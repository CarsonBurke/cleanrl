...
OMP_NUM_THREADS=1 xvfb-run -a uv run python -m cleanrl_utils.benchmark \
    ...
    --command "uv run python cleanrl/qdagger_dqn_atari_impalacnn.py --track --capture_video --offline-checkpoint-interval-seconds 600" \
    --num-seeds 3 \
    --workers 1
...
xvfb-run -a uv run python -m cleanrl_utils.benchmark \
    --env-ids PongNoFrameskip-v4 BeamRiderNoFrameskip-v4 BreakoutNoFrameskip-v4 \
    --command "uv run python cleanrl/qdagger_dqn_atari_jax_impalacnn.py --track --capture_video --offline-checkpoint-interval-seconds 600" \
    --num-seeds 3 \
    --workers 1
