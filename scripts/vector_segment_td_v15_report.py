"""Summarize the fresh v15 run and only its established local references."""
import json
import statistics
...
RUNS = ROOT / 'runs'
paths = list(RUNS.glob('HalfCheetah-v4__vector_segment_td_v15_8M__1__*/progress.json'))
if len(paths) != 1:
    raise RuntimeError(f'Expected one actual v15 run, got {len(paths)}')
run = paths[0].parent
...
references = [
    ...
    ('v14 one-step vector TD', RUNS/'HalfCheetah-v4__vector_td_response_v14_8M__1__1789583093813423555/result.json'),
...
axes[0].plot(steps,[p[ret] for p in progress],label='v15 32-step vector TD',linewidth=2)
...
for ax in axes:
    ...
    ax.grid(alpha=.2)
fig.suptitle('Vector segment TD v15 — fresh seed 1, HalfCheetah-v4')
output = ROOT/'docs/vector-segment-td-v15-results'
