"""Summarize the fresh v17 run and only its established local references."""
import json
import statistics
...
RUNS = ROOT / 'runs'
paths = list(RUNS.glob('HalfCheetah-v4__vector_predictive_distribution_v17_8M__1__*/progress.json'))
if len(paths) != 1:
    raise RuntimeError(f'Expected one actual v17 run, got {len(paths)}')
run = paths[0].parent
...
references = [
    ...
    ('v16 unified vector TD', RUNS/'HalfCheetah-v4__vector_unified_td_v16_8M__1__1789585442529891760/result.json'),
...
axes[0].plot(steps,[p[ret] for p in progress],label='v17 predictive distribution',linewidth=2)
...
for key,label in [('critic/component_td_rms','Post-fit mean TD residual'),
                  ('targets/component_rms','Mean TD target'),
                  ('credit/observed_residual_rms','Pre-fit sampled TD residual')]:
...
for ax in axes:
    ...
    ax.grid(alpha=.2)
fig.suptitle('Predictive distribution v17 — fresh seed 1, HalfCheetah-v4')
output = ROOT/'docs/vector-predictive-distribution-v17-results'
