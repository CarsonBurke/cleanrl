...
def centering_interventions(actor, observations):
    """Frozen-policy interventions: change only channel centering, never weights.

    This is functional reliance, not a counterfactual retraining score. KL is
    between the complete baseline and intervened Beta action distributions.
    """
    from torch.distributions import Beta, kl_divergence

    trunk = actor[0]
    base_alpha, base_beta = (F.softplus(actor(observations)) + 1.0).chunk(2, -1)
    reference = Beta(base_alpha, base_beta)
    mean_reference = 2 * base_alpha / (base_alpha + base_beta) - 1
    original_norms, original_final = trunk.block_norms, trunk.final_norm
    target_kind = "layer" if trunk.norm_kind == "rms" else "rms"
    norm_class = nn.LayerNorm if target_kind == "layer" else nn.RMSNorm
    results = {"original_norm": trunk.norm_kind, "intervened_norm": target_kind, "cases": {}}
    for name, change_inputs, change_final in (("branch_inputs_only", True, False),
                                               ("readout_only", False, True),
                                               ("branch_inputs_and_readout", True, True)):
        try:
            if change_inputs:
                trunk.block_norms = nn.ModuleList(norm_class(trunk.width, eps=1e-5, elementwise_affine=False)
                                                  for _ in original_norms).to("cuda")
            if change_final:
                trunk.final_norm = norm_class(trunk.width, eps=1e-5, elementwise_affine=False).to("cuda")
            alpha, beta = (F.softplus(actor(observations)) + 1.0).chunk(2, -1)
            kl = kl_divergence(reference, Beta(alpha, beta)).sum(-1)
            mean_delta = (2 * alpha / (alpha + beta) - 1 - mean_reference).abs()
            results["cases"][name] = {
                "joint_action_kl_mean": scalar(kl.mean()),
                "joint_action_kl_median": scalar(kl.median()),
                "joint_action_kl_p95": scalar(torch.quantile(kl, 0.95)),
                "physical_action_mean_abs_change": scalar(mean_delta.mean()),
                "physical_action_mean_max_change": scalar(mean_delta.max()),
            }
        finally:
            trunk.block_norms, trunk.final_norm = original_norms, original_final
    return results
...
def collect_checkpoint_observations(path):
    ...
    return actor, torch.from_numpy(observations).to("cuda"), provenance
...
def main():
    ...
    if args.checkpoint is not None:
        trained_actor, observations, trained_provenance = collect_checkpoint_observations(args.checkpoint)
        trained = trained_actor[0]
    ...
    report = {
        ...
        "source_sha256": {str(path): sha256(ROOT / path) for path in
                          (Path("cleanrl/shared/host_actor.py"), Path("cleanrl/shared/norm_residual.py"),
                           Path("cleanrl/shared/host_graph.py"), Path("scripts/analyze_activation_geometry_v2.py"))},
    ...
    if args.checkpoint is not None:
        report["frozen_policy_centering_interventions"] = {
            name: centering_interventions(trained_actor, observations)
            for name, observations in datasets.items()
        }
