...
for label, name, job in [('Full batch + fixed target, old LR', 'fullbatch_fixed_low_lr', 7546),
                          ('Full batch + fixed target, 3x LR', 'fullbatch_fixed_3x_lr', 7547)]:
    matches = list(RUNS.glob(f'HalfCheetah-v4__latent_costate_v12_{name}_8M__1__*/progress.json'))
    if len(matches) != 1:
        raise RuntimeError(f'Expected one run for {name}, got {len(matches)}')
    result_path = matches[0].parent / 'result.json'
    if not result_path.exists() and not (matches[0].parent / 'experiment_outcome.json').exists():
...
fig.suptitle('Full-batch vector costate, one fixed target — fresh seed 1, HalfCheetah-v4')
output = ROOT / 'docs/latent-costate-fullbatch-v12-results'
