"""v12 gate-pole extension: gate_beta {.9995, .9999} for the gated families, structured_ood only.
...
plan = {**v12, "suite": [scenario], "families": families, "scenario_families": {"structured_ood": families},
        ...
        "selection": v12["selection"] + " Extension: gate_beta {.9995, .9999} only; compared against the v12 parents.",
        "inherited_from": "benchmarks/plasticity/optimizer_proxy_v12_plan.json"}
Path("benchmarks/plasticity/optimizer_proxy_v12_ext_plan.json").write_text(json.dumps(plan, indent=1) + "\n")
