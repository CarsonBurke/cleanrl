"""LSTD Explore: Exploration Reward Bonus

Fork of lstd_linear_tanh with intrinsic reward bonus proportional to action_std.

**Hypothesis**: Adding alpha * action_std.mean() to the reward during rollouts
incentivizes the policy to maintain exploration through the value function. States
where the policy is stochastic get higher returns, so the value function learns to
prefer stochastic states. Linear annealing removes the bonus over training so the
policy converges to exploitation.

Key changes from base:
1. get_action_and_value returns action_std as 5th value during rollouts (return_std=True)
2. explore_bonus = coef * action_std.mean(dim=-1).detach() added to reward after env step
3. Linear annealing of bonus coefficient over training
4. Training-phase get_action_and_value unchanged (still 4 returns)
5. New args: explore_bonus_coef (default 0.1), explore_bonus_anneal (default True)
"""
import os
import random
import time
...
class Args:
    ...
    explore_bonus_coef: float = 0.1
    """coefficient for exploration reward bonus (alpha * action_std.mean)"""
    explore_bonus_anneal: bool = True
    """whether to linearly anneal the exploration bonus to 0"""
    vf_coef: float = 0.5
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None, return_std=False):
        ...
        if action is None:
            action = probs.sample()
        base = (action, probs.log_prob(action).sum(1), probs.entropy().sum(1), self.get_value(x))
        if return_std:
            return base + (action_std,)
        return base
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Compute current explore bonus coefficient (with optional annealing)
        if args.explore_bonus_anneal:
            bonus_frac = 1.0 - (iteration - 1.0) / args.num_iterations
            current_bonus_coef = args.explore_bonus_coef * bonus_frac
        else:
            current_bonus_coef = args.explore_bonus_coef
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, action_std = agent.get_action_and_value(next_obs, return_std=True)
                values[step] = value.flatten()
                # Compute exploration bonus: mean std across action dims
                explore_bonus = current_bonus_coef * action_std.mean(dim=-1)  # (num_envs,)
            actions[step] = action
            ...
            rewards[step] = reward_tensor + explore_bonus
        ...
        writer.add_scalar("explore/bonus_coef", current_bonus_coef, global_step)
