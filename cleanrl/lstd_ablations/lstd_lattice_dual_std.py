...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Separate std for log_prob (well-behaved) and sampling (more expressive)
        self.log_std_param = nn.Parameter(torch.zeros(hidden_dim, act_dim))      # for log_prob
        self.log_std_sample = nn.Parameter(torch.zeros(hidden_dim, act_dim))     # for noise sampling
    ...
    def _get_noise(self, h):
        """Lattice-style noise with separate sampling std.
        Sampling std can explore more freely without destabilizing importance ratios."""
        sde_latent, _ = self._sde_latent_and_std(h)
        # Use separate sampling std
        log_std_s = (self.log_std_sample + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
        std_s = log_std_s.exp()
        exploration_mat = torch.randn_like(std_s) * std_s  # (hidden_dim, act_dim)
