"""LSTD latent CFM with positive-advantage power weighting.
...
base covariance and softly modulates low-rank amplitudes. The final policy is
still an exact LowRankMultivariateNormal, so PPO logprobs remain valid.

This variant replaces minibatch softmax weighting with normalized positive
advantage weights to test whether softmax concentration is helping or harming.
It raises positive-advantage weights to 1.5 before renormalization, between
broad no-softmax weighting and sharp softmax weighting.
"""
import os
import random
import time
...
class Args:
    ...
    cfm_adv_power: float = 1.5
    """power applied to positive-advantage CFM weights"""
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.cfm_adv_power = float(args.cfm_adv_power)
    ...
    def cfm_loss(self, x, actions, advantages, weight_clip):
        ...
        with torch.no_grad():
            ...
            weights = (1.0 + F.relu(adv)).pow(self.cfm_adv_power)
