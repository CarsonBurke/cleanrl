"""LSTD sigmoid-gated noise: single-head state→noise from actor features

Radical simplification of the SDE noise path. Instead of a separate 3-layer
network (sde_fc → norm → sde_fc2 → tanh → fc3/quadratic), use a single
linear head off the actor backbone features:

  log_std = LOG_STD_MIN + (LOG_STD_MAX - LOG_STD_MIN) * sigmoid(std_head(h))

Why this should be more responsive:
1. Shortest gradient path: loss → log_std → std_head → actor backbone
2. Starts in the MIDDLE of the range (sigmoid(0) = 0.5), not pinned to one end
3. A weight change of ±2 swings 75% of the range — maximally responsive
4. No clamping needed — sigmoid naturally maps to valid [min, max]
5. Wider range [-4, 0] allows both near-deterministic (std≈0.018) and
   full unit-scale exploration (std=1.0)

Previous ablation results on HalfCheetah @ 8M:
- sdefc3 (3-layer, tanh, signed): ~3850
- sdefc3_sq (3-layer, tanh, squared): ~3700
"""
import os
import random
import time
...
LOG_STD_MIN = -4.0   # std ≈ 0.018 — near-deterministic
LOG_STD_MAX = 0.0    # std = 1.0 — full unit-scale exploration
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Noise head: single linear from actor features → per-action sigmoid gate
        # Small init so sigmoid(~0) ≈ 0.5 → starts at midpoint of [LOG_STD_MIN, LOG_STD_MAX]
        self.std_head = layer_init(nn.Linear(hidden_dim, act_dim), std=0.01)
        nn.init.constant_(self.std_head.bias, 0.0)
    ...
    def _get_action_std(self, h):
        """State-dependent action std via sigmoid gate on actor features."""
        raw = self.std_head(h)  # (batch, act_dim)
        log_std = LOG_STD_MIN + (LOG_STD_MAX - LOG_STD_MIN) * torch.sigmoid(raw)
