"""Ablation 3: Higher initial noise levels.
Changes:
  - FLOOR_LOG_STD: -3.0 -> -1.5 (std 0.05 -> 0.22)
  - dir_head bias: 0.0 -> 1.0 (sigmoid(1)=0.73, so higher initial amp)
  - LOG_AMP_MAX: 0.0 -> 1.0 (max amp exp(1)=2.7 instead of 1.0)
Hypothesis: The original starts too quiet — both directional and floor noise are small.
More initial exploration helps PPO find good reward signal faster.
"""
import os
import random
import time
...
LOG_AMP_MIN = -4.0
# ABLATION: higher max amplitude
LOG_AMP_MAX = 1.0
# ABLATION: higher floor std
FLOOR_LOG_STD = -1.5
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # ABLATION: higher initial bias -> sigmoid(1.0) = 0.73 -> more initial exploration
        nn.init.constant_(self.dir_head.bias, 1.0)
