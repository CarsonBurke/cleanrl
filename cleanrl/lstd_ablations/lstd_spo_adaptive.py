"""LSTD + SPO with adaptive ε via policy loss feedback

Ablation: instead of dual descent / targets, ε adapts based on whether the
policy loss improves or degrades across the K update epochs.

After each iteration's K epochs:
- If pg_loss improved (decreased): trust region was appropriate → slowly grow ε
- If pg_loss degraded (increased): overshooting → shrink ε

This is analogous to TRPO's line search — adapt the trust region size based on
actual optimization quality, not ratio statistics. No target, no dual descent,
no separate optimizer.

eps_high and eps_low are updated independently based on their contribution to
the policy loss change.
"""
import os
import random
import time
...
class Args:
    ...
    eps_grow: float = 0.01
    """log_eps increment when policy loss improves"""
    eps_shrink: float = 0.05
    """log_eps decrement when policy loss degrades (asymmetric: shrink faster than grow)"""
    eps_min: float = 0.05
    """minimum ε (prevents collapse)"""
    eps_max: float = 5.0
    """maximum ε (prevents unbounded growth)"""
    ent_coef: float = 0.0
...
if __name__ == "__main__":
    ...
    # Adaptive ε — adjusted per iteration based on policy loss trajectory
    log_eps_high = np.log(args.clip_coef_high)
    log_eps_low = np.log(args.clip_coef_low)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        clipfracs = []
        spo_penalty_sum = 0.0
        eps_high = np.clip(np.exp(log_eps_high), args.eps_min, args.eps_max)
        eps_low = np.clip(np.exp(log_eps_low), args.eps_min, args.eps_max)
        eps_high_t = torch.tensor(eps_high, dtype=torch.float32, device=device)
        eps_low_t = torch.tensor(eps_low, dtype=torch.float32, device=device)
        pg_loss_epoch0 = None  # track first epoch's pg_loss
        pg_loss_epochK = None  # track last epoch's pg_loss
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # SPO with fixed eps for this iteration (adapted between iterations)
                eps = torch.where(mb_advantages >= 0, eps_high_t, eps_low_t)
            ...
            # Track pg_loss at first and last epoch
            avg_epoch_pg = epoch_pg_loss / max(n_mb, 1)
            if epoch == 0:
                pg_loss_epoch0 = avg_epoch_pg
            pg_loss_epochK = avg_epoch_pg

            if args.target_kl is not None and approx_kl > args.target_kl:
                break

        # Adaptive eps: compare policy loss at epoch 0 vs epoch K
        # If loss improved (decreased): grow eps (allow more)
        # If loss degraded (increased): shrink eps (constrain more)
        pg_improved = pg_loss_epochK < pg_loss_epoch0
        if pg_improved:
            log_eps_high += args.eps_grow
            log_eps_low += args.eps_grow
        else:
            log_eps_high -= args.eps_shrink
            log_eps_low -= args.eps_shrink
        ...
        writer.add_scalar("spo/eps_high", np.clip(np.exp(log_eps_high), args.eps_min, args.eps_max), global_step)
        writer.add_scalar("spo/eps_low", np.clip(np.exp(log_eps_low), args.eps_min, args.eps_max), global_step)
        writer.add_scalar("spo/penalty_mean", spo_penalty_sum / max(n_updates, 1), global_step)
        writer.add_scalar("spo/pg_loss_epoch0", pg_loss_epoch0, global_step)
        writer.add_scalar("spo/pg_loss_epochK", pg_loss_epochK, global_step)
        writer.add_scalar("spo/pg_improved", float(pg_improved), global_step)
