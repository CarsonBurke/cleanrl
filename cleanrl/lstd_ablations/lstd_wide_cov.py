"""lstd_wide_cov — Ablation: wider SDE path (128 dims vs 64)

Fork of tbot_scalar_lstd. The ONLY change is widening the SDE covariance path
from hidden_dim=64 to sde_dim=128:

- sde_fc: Linear(hidden_dim, 128)
- sde_norm: RMSNorm(128)
- log_std_param: [128, act_dim]
- action_var normalized by /128 to keep initial noise magnitude matched to 64-dim baseline

This tests whether a wider covariance bottleneck improves exploration expressiveness
without changing the actor backbone or mean path.
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.sde_fc = layer_init(nn.Linear(hidden_dim, sde_dim), std=1.0)
        self.sde_norm = RMSNorm(sde_dim)
        # Learned per-element noise magnitude: log_std = (param + LOG_STD_INIT).clamp(min, max)
        self.log_std_param = nn.Parameter(torch.zeros(sde_dim, act_dim))
    ...
    def _get_action_std(self, h):
        ...
        # SDE variance: sum over sde_dim of (latent^2 * std^2)
