"""Ablation: Fixed small constant floor (built on directamp).
Change: Replace learnable log_std_floor with a fixed 1e-4 constant.
At full rank the low-rank component is a full covariance — the floor is
just numerical safety. No parameters wasted on something redundant.
"""
import os
import random
import time
...
FLOOR_VAR = 1e-4  # fixed, not learnable
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # No learnable floor — register fixed constant
        self.register_buffer('cov_diag_fixed', torch.full((act_dim,), FLOOR_VAR))
    ...
    def _get_distribution(self, h, action_mean):
        ...
        cov_diag = self.cov_diag_fixed.expand(batch, act_dim)
