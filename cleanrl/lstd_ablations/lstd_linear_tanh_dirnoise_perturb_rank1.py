"""Residual rank-1 perturbation updates.
Change vs perturb: Instead of a single Linear(64, rank*dim) perturbation head,
decompose the perturbation as K rank-1 updates: delta = sum_k u_k(s) v_k(s)^T.
u_k is (rank,), v_k is (dim,), both state-dependent. Multiplicative structure
gives richer interaction patterns per parameter than additive linear map.
K=4 rank-1 updates produce the same output shape but with structured low-rank bias.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Rank-1 perturbation: K rank-1 updates u_k(s) @ v_k(s)^T
        self.num_rank1 = 4
        self.u_head = layer_init(nn.Linear(hidden_dim, self.num_rank1 * self.noise_rank), std=0.01)
        nn.init.constant_(self.u_head.bias, 0.0)
        self.v_head = layer_init(nn.Linear(hidden_dim, self.num_rank1 * act_dim), std=0.01)
    ...
    def _get_distribution(self, h, action_mean):
        ...
        raw = self.dir_head(h)
        # Direct sigmoid amplitude: maps to [0.05, 1.0]
        amp = 0.05 + 0.95 * torch.sigmoid(raw)
        # Rank-1 perturbation: delta = sum_k u_k v_k^T
        u = self.u_head(h).reshape(batch, self.num_rank1, self.noise_rank)   # (B, K, rank)
        v = self.v_head(h).reshape(batch, self.num_rank1, act_dim)           # (B, K, dim)
        perturb = torch.einsum('bki,bkj->bij', u, v) * 0.1                  # (B, rank, dim)
