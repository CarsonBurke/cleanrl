"""Ablation 4: Direct sigmoid amplitude instead of double transform.
Change: Replace sigmoid->log_amp->exp with direct: amp = 0.05 + 0.95 * sigmoid(raw)
Hypothesis: The double transform (sigmoid to get log_amp, then exp) is unnecessarily
complex. A direct sigmoid mapping to [0.05, 1.0] is simpler, has better gradient flow
(no exp), and achieves the same bounded positive amplitude.
"""
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def _get_distribution(self, h, action_mean):
        ...
        raw = self.dir_head(h)
        # ABLATION: direct sigmoid amplitude instead of sigmoid->log->exp
        # Maps to [0.05, 1.0] directly. At bias=0, sigmoid(0)=0.5 -> amp=0.525
