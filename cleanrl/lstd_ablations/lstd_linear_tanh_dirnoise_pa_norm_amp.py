"""Norm-derived amplitude: amplitude IS the norm of each perturbed basis row.
Remove the separate amplitude head. The perturbation head (36 outputs) controls
both direction and magnitude through the geometry of the perturbed vectors.
effective_basis rows are decomposed into direction (unit norm) and amplitude (norm).
Perturbation scale 0.5 so norms can vary meaningfully.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Perturbation head — controls both direction and magnitude
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Perturbation with larger scale so norms vary meaningfully
        perturb = self.perturb_head(h).reshape(batch, self.noise_rank, act_dim) * 0.5
        effective_basis = self.noise_basis.unsqueeze(0) + perturb  # (batch, noise_rank, act_dim)
        # Decompose into direction (unit norm) and amplitude (row norm)
        row_norms = effective_basis.norm(dim=-1, keepdim=True).clamp(min=1e-6)  # (batch, rank, 1)
        dirs = effective_basis / row_norms  # unit direction vectors
        amp = row_norms.squeeze(-1)  # (batch, rank) — amplitude is the geometry
        cov_factor = dirs.transpose(-2, -1) * amp.unsqueeze(1)  # (batch, act_dim, noise_rank)
