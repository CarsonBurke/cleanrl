"""Diagonal only: no low-rank component at all. Diagonal Normal with bounded
state-dependent std using amprange pattern. Tests whether our bounded sigmoid + learned
range mechanism is competitive with the baseline's SDE approach as a pure diagonal policy.
Init std range ~[0.15, 0.65] to match baseline's effective range.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # State-dependent std with amprange pattern
        # Init: softplus(floor) ≈ 0.15, softplus(scale) ≈ 0.5
        # So std range ~[0.15, 0.65], matching baseline's effective range
        self.std_head = layer_init(nn.Linear(hidden_dim, act_dim), std=0.01)
        nn.init.constant_(self.std_head.bias, 0.0)
        self.std_floor = nn.Parameter(torch.full((act_dim,), float(np.log(np.exp(0.15) - 1))))
        self.std_scale = nn.Parameter(torch.full((act_dim,), float(np.log(np.exp(0.5) - 1))))
    ...
    def _get_distribution(self, h, action_mean):
        # Bounded state-dependent std: amprange pattern
        action_std = F.softplus(self.std_floor) + F.softplus(self.std_scale) * torch.sigmoid(self.std_head(h))
