"""Ablation E: Learned amplitude range per noise direction.
Change: Replace fixed amp = 0.05 + 0.95 * sigmoid(raw) with learned per-direction
floor and scale: amp_floor (noise_rank,) init 0.05, amp_scale (noise_rank,) init 0.95,
both as nn.Parameter. amp = softplus(amp_floor) + softplus(amp_scale) * sigmoid(raw).
This lets the model learn to widen or narrow the amplitude range per direction.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # ABLATION E: learned per-direction amplitude floor and scale
        # softplus(amp_floor) ~= 0.05 at init, softplus(amp_scale) ~= 0.95 at init
        # We use inverse softplus to find init values: inv_softplus(x) = log(exp(x) - 1)
    ...
    def _get_distribution(self, h, action_mean):
        ...
        raw = self.dir_head(h)
        # ABLATION E: learned amplitude range per direction
        # amp = softplus(amp_floor) + softplus(amp_scale) * sigmoid(raw)
