"""Ablation: SDE from shared backbone instead of actor backbone.

Hypothesis: In lstd_linear_tanh, SDE features come from the actor backbone,
meaning exploration is shaped entirely by policy-specific representations.
This ablation uses a SHARED backbone for actor, critic, AND SDE. The SDE
features thus reflect a more general state representation trained by both
policy and value gradients, rather than being actor-specific.

This tests whether exploration benefits from richer representations (value
gradient signal) or whether coupling SDE to the actor is important for
policy-aligned exploration.

Changes from lstd_linear_tanh:
- Single shared backbone replaces separate actor/critic backbones
- Actor mean head, critic value head, and SDE head all branch from shared features
- SDE fc layers take shared features as input instead of actor features
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Shared backbone: obs -> hidden features (used by actor, critic, AND SDE)
        self.shared_fc1 = layer_init(nn.Linear(obs_dim, hidden_dim))
        self.shared_norm1 = RMSNorm(hidden_dim)
        self.shared_fc2 = layer_init(nn.Linear(hidden_dim, hidden_dim))
        self.shared_norm2 = RMSNorm(hidden_dim)

        # Actor mean head
        self.actor_out = layer_init(nn.Linear(hidden_dim, act_dim), std=1.0)
        self.mean_scale = nn.Parameter(torch.tensor(0.01))

        # SDE noise head (branches from shared features)
    ...
    def _shared_features(self, x):
        h = F.silu(self.shared_norm1(self.shared_fc1(x)))
        h = F.silu(self.shared_norm2(self.shared_fc2(h)))
        return h
