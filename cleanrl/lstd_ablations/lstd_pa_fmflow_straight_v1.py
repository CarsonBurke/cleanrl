"""LSTD perturb-amprange with a straightened time-indexed action flow.

This variant uses the same exact time-indexed coupling flow but adds a coupled
straight-path regularizer. For sampled source noise x0 and final action x1, it
penalizes intermediate flow states for deviating from the linear CFM path
(1-t)x0 + t x1. This keeps PPO likelihood exact while taking the paper's
straight/coupled low-variance transport lesson seriously.
"""
import os
import random
import time
...
class Args:
    ...
    """maximum tanh-bounded affine-coupling log scale"""
    flow_steps: int = 4
    """number of time-indexed coupling steps"""
    flow_straightness_coef: float = 0.01
    """coefficient for the coupled straight-path regularizer"""
    batch_size: int = 0
...
class TimeCouplingBlock(nn.Module):
    def __init__(self, action_dim, cond_dim, flip=False, flow_scale=1.0):
        ...
        left_dim = self.action_dim - self.split if flip else self.split
        right_dim = self.split if flip else self.action_dim - self.split
        hidden_dim = 64
    ...
    def _params(self, left, cond, t):
        t_feat = torch.full((left.shape[0], 1), float(t), device=left.device, dtype=left.dtype)
        shift, raw_scale = self.net(torch.cat((cond, left, t_feat), dim=-1)).chunk(2, dim=-1)
        return shift, self.flow_scale * torch.tanh(raw_scale)

    def forward(self, x, cond, t):
        left, right = self._split(x)
        shift, log_scale = self._params(left, cond, t)
        y = self._merge(left, right * torch.exp(log_scale) + shift)
        return y, log_scale.sum(dim=-1)

    def inverse(self, y, cond, t):
        left, transformed = self._split(y)
        shift, log_scale = self._params(left, cond, t)
        x = self._merge(left, (transformed - shift) * torch.exp(-log_scale))
        return x, -log_scale.sum(dim=-1)


class TimeActionFlow(nn.Module):
    def __init__(self, action_dim, cond_dim, flow_scale, flow_steps):
        super().__init__()
        self.flow_steps = int(flow_steps)
        self.blocks = nn.ModuleList(
            [
                TimeCouplingBlock(action_dim, cond_dim, flip=bool(i % 2), flow_scale=flow_scale)
                for i in range(self.flow_steps)
            ]
        )

    def _time(self, index):
        return (index + 0.5) / max(self.flow_steps, 1)

    def forward(self, x, cond, return_path=False):
        logdet = torch.zeros(x.shape[0], device=x.device, dtype=x.dtype)
        path = [x] if return_path else None
        for i, block in enumerate(self.blocks):
            x, delta = block(x, cond, self._time(i))
            logdet = logdet + delta
            if return_path:
                path.append(x)
        if return_path:
            return x, logdet, path
        return x, logdet

    def inverse(self, y, cond):
        logdet = torch.zeros(y.shape[0], device=y.device, dtype=y.dtype)
        for i in reversed(range(len(self.blocks))):
            y, delta = self.blocks[i].inverse(y, cond, self._time(i))
            logdet = logdet + delta
        return y, logdet
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.action_flow = TimeActionFlow(act_dim, hidden_dim, args.flow_scale, args.flow_steps)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            base_action = dist.sample()
            action, logdet = self.action_flow(base_action, h)
            logprob = dist.log_prob(base_action) - logdet
            entropy = dist.entropy() + logdet
        else:
            base_action, inv_logdet = self.action_flow.inverse(action, h)
            logprob = dist.log_prob(base_action) + inv_logdet
            entropy = dist.entropy()
        return action, logprob, entropy, self.get_value(x)

    def flow_straightness_loss(self, x):
        h = self._actor_features(x)
        action_mean = self.actor_out(h) * self.mean_scale
        dist = self._get_distribution(h, action_mean)
        base_action = dist.sample()
        final_action, _, path = self.action_flow(base_action, h, return_path=True)
        losses = []
        num_segments = len(path) - 1
        for i, intermediate in enumerate(path[1:-1], start=1):
            t = i / num_segments
            target = base_action + t * (final_action.detach() - base_action)
            losses.append((intermediate - target).square().mean())
        if not losses:
            return base_action.new_tensor(0.0)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                straightness_loss = agent.flow_straightness_loss(b_obs[mb_inds])
                loss = loss + args.flow_straightness_coef * straightness_loss
                optimizer.zero_grad()
                loss.backward()
        ...
        writer.add_scalar("losses/flow_straightness_loss", straightness_loss.item(), global_step)
