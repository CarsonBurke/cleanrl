"""LSTD linear_tanh + tanh surrogate ablation

Replaces standard PPO clipped surrogate with tanh-based surrogate:
  pg_loss = -advantage * tanh(log_ratio) * SURROGATE_SCALE

Key properties vs standard PPO clip:
- Bounded by construction: output in (-scale, +scale), no hard clip needed
- Symmetric around zero: equal gradient for probability increases/decreases
- Soft saturation: smooth gradient everywhere, no zero-gradient dead zones at clip boundary
- Implicit tight trust region: 0.3 scale caps effective surrogate at ±0.3 * advantage

Inspired by trading bot observation that this speeds up early learning significantly.
...
# tanh(lr * k) / k surrogate: gradient=1 at lr=0, saturates at ±1/k
# k=4 saturates ~|lr|=0.5 (ratio ~0.6 to 1.6), max surrogate ±0.25
# Asymmetric: k_pos for increasing prob, k_neg for decreasing
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Asymmetric tanh surrogate: tanh(lr * k) / k
                # gradient=1 at lr=0, saturates at ±1/k, no clip needed
                k = torch.where(logratio > 0, TANH_K_POS, TANH_K_NEG)
                surrogate = (logratio * k).tanh() / k
                pg_loss = -(mb_advantages * surrogate).mean()
