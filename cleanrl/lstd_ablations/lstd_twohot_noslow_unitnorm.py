...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        """State-dependent action std: unit-norm SDE latent (direction) + log_std_param (magnitude)."""
        sde_raw = self.sde_fc(h)
        sde_latent = self.sde_fc2(self.sde_norm(sde_raw))
        sde_latent = F.normalize(sde_latent, dim=-1)  # unit norm: direction only
