"""Direct state-dependent cov_factor with amplitude range control.
Removes the B + perturb*0.1 decomposition entirely. A single head outputs full
direction vectors per state, normalized to unit length, then scaled by amplitude range.
Bias initialized to orthogonal basis for stable start. Over training the network can
move arbitrarily far from init — no 0.1 scale constraint, no fixed basis parameter.
Cleaner separation: directions are purely directional (unit norm), amplitude is purely
magnitude (learned range). Fewer components, more expressiveness.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Direct cov_factor head: outputs full (act_dim, noise_rank) direction matrix
        # Bias initialized to orthogonal basis (transposed) for stable start
        basis = torch.empty(self.noise_rank, act_dim)
        nn.init.orthogonal_(basis)
        self.cov_head = nn.Linear(hidden_dim, act_dim * self.noise_rank)
        nn.init.normal_(self.cov_head.weight, std=0.01)
        self.cov_head.bias.data.copy_(basis.T.reshape(-1))  # (act_dim * noise_rank,)

        # Amplitude: state-dependent with learned per-direction range
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Direct state-dependent directions, normalized to unit column vectors
        raw_dirs = self.cov_head(h).reshape(batch, act_dim, self.noise_rank)
        dirs = raw_dirs / (raw_dirs.norm(dim=1, keepdim=True) + 1e-8)
        # Amplitude with learned per-direction range
        amp = F.softplus(self.amp_floor) + F.softplus(self.amp_scale) * torch.sigmoid(self.dir_head(h))
        cov_factor = dirs * amp.unsqueeze(1)  # (batch, act_dim, noise_rank)
