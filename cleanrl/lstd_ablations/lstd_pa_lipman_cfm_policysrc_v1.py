"""LSTD perturb-amprange with policy-source Lipman OT-CFM.

This keeps the successful LSTD perturb-amprange PPO policy intact and adds a
simulation-free OT-CFM objective over rollout actions. The CFM target is the
advantage-tilted action distribution: sample x0, sample rollout action x1,
sample t, regress v_theta(obs, x_t, t) to the Lipman OT velocity
x1 - (1 - sigma_min) * x0. In this conservative variant the vector field is
uses detached current-policy samples as x0 rather than standard Gaussian noise.
This asks the vector field to transport the policy's own action distribution
toward advantage-tilted rollout actions, then feeds a small t=0 velocity
residual back into the policy mean. PPO likelihoods remain exact.
"""
import os
import random
import time
