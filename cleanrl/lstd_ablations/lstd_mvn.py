"""LSTD-MVN Ablation: Multivariate Normal (cross-action correlated noise)

Fork of ppo_continuous_action_tbot_scalar_lstd.py replacing independent per-dim
diagonal Gaussian sampling with MultivariateNormal. Instead of computing per-action
std and using Normal(mean, std), we build a factor matrix A = sde_latent * std and
form covariance Sigma = A^T @ A + eps*I, then sample from MVN(mean, Sigma).

This introduces cross-action noise correlation: the covariance is a full matrix
(low-rank + diagonal), not just diagonal. The rank of the noise correlation is
min(hidden_dim, act_dim).

Key difference from lstd diagonal:
- _get_action_std removed entirely
- Factor matrix A: (batch, hidden, act) = sde_latent.unsqueeze(-1) * std.unsqueeze(0)
- Covariance: Sigma = A^T @ A + eps*I  (batch, act, act)
- MultivariateNormal log_prob/entropy are already summed across dims
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        # Build noise factor matrix from SDE path
        sde_raw = self.sde_fc(h)
        sde_latent = (self.sde_norm(sde_raw) / SDE_PRESCALE).tanh()  # (batch, hidden)

        log_std = (self.log_std_param + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
        std = log_std.exp()  # (hidden, act)

        # Factor matrix A: (batch, hidden, act)
        A = sde_latent.unsqueeze(-1) * std.unsqueeze(0)

        # Covariance: Sigma = A^T @ A + eps*I
        Sigma = torch.bmm(A.transpose(1, 2), A)
