...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Fully separate noise trunk (own obs→hidden→hidden path)
        self.noise_fc1 = layer_init(nn.Linear(obs_dim, hidden_dim))
        self.noise_norm1 = RMSNorm(hidden_dim)
        self.noise_fc2 = layer_init(nn.Linear(hidden_dim, hidden_dim))
        self.noise_norm2 = RMSNorm(hidden_dim)
    ...
    def _noise_features(self, x):
        """Separate noise backbone — no gradient sharing with actor."""
        h = F.silu(self.noise_norm1(self.noise_fc1(x)))
        h = F.silu(self.noise_norm2(self.noise_fc2(h)))
        return h

    def _sde_latent_and_std(self, x):
        """Compute SDE latent features from separate trunk and std matrix."""
        h = self._noise_features(x)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            ...
            action_std = self._get_action_std(x)
