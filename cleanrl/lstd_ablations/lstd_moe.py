"""PPO tbot_scalar_lstd — MoE Noise Ablation

Fork of tbot_scalar_lstd replacing the single log_std_param matrix with a
Mixture-of-Experts (MoE) noise computation:
- SDE latent is split into n_experts=4 chunks of expert_dim=16
- Each expert projects its chunk to action-space variance via learned expert_proj
- A state-dependent gate (from backbone features h) routes expert variances
- Final action_var = weighted sum of expert variances

This tests whether specialized, state-routed noise experts improve over a
monolithic learned noise matrix.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # SDE noise with MoE variance computation
        self.sde_fc = layer_init(nn.Linear(hidden_dim, hidden_dim), std=1.0)
        self.sde_norm = RMSNorm(hidden_dim)
        # MoE: split 64-dim SDE latent into 4 experts of 16 dims each
        self.n_experts = 4
        self.expert_dim = hidden_dim // self.n_experts  # 16
        # Each expert has its own projection from its 16 dims to action space
        self.expert_proj = nn.Parameter(torch.randn(self.n_experts, self.expert_dim, act_dim) * 0.1)
        # Gate uses backbone features h (not sde features) to route experts
        self.gate = layer_init(nn.Linear(hidden_dim, self.n_experts), std=0.01)
    ...
    def _get_action_std(self, h):
        """State-dependent action std via MoE over SDE latent chunks."""
        sde_raw = self.sde_fc(h)  # (batch, hidden_dim)
        # RMSNorm → scale → tanh: bounded in [-1, 1] with stable gradients
        sde_latent = (self.sde_norm(sde_raw) / SDE_PRESCALE).tanh()  # (batch, hidden_dim)

        # Split into expert groups
        chunks = sde_latent.view(-1, self.n_experts, self.expert_dim)  # (B, 4, 16)
        # Each expert: variance from its 16 dims
        expert_var = torch.einsum('bke,kea->bka', chunks.pow(2), self.expert_proj.pow(2))  # (B, 4, act)
        # State-dependent routing from backbone features
        gate_weights = F.softmax(self.gate(h), dim=-1)  # (B, 4)
        # Weighted sum of expert variances
        action_var = torch.einsum('bka,bk->ba', expert_var, gate_weights)  # (B, act_dim)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            gate_logits = agent.gate(h) if 'h' in dir() else None
        writer.add_scalar("tbot/mean_scale", agent.mean_scale.item(), global_step)
        writer.add_scalar("tbot/expert_proj_norm", agent.expert_proj.norm().item(), global_step)
