"""LSTD Ablation: State-Dependent Noise Gating (lstd_gated)

Fork of tbot_scalar_lstd. Single change: adds a state-dependent gate on SDE latent
features to allow per-hidden-dimension amplification or suppression of noise.

The gate is a linear layer from actor features h (NOT sde_raw) to hidden_dim,
initialized to zeros so it starts as identity (exp(0)=1). The gate output is
exp(clamp(linear(h), -2, 2)), giving a multiplicative factor in [exp(-2), exp(2)]
≈ [0.135, 7.389] per hidden dimension.

Key insight: the gate uses actor features h as input, giving it a different
information pathway than the SDE features (which go through sde_fc). This lets
the network learn state-dependent noise magnitude control orthogonal to the
noise direction/structure.

All other components identical to tbot_scalar_lstd.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # State-dependent noise gate: maps actor features -> per-hidden-dim scale
        # Zero-init so gate starts as identity (exp(0) = 1.0)
    ...
    def _get_action_std(self, h):
        """State-dependent action std via RMSNorm→tanh SDE + learned log_std_param + gating."""
        ...
        # State-dependent gate: exp(offset) centered at 1.0, can amplify or suppress
        gate = torch.exp(self.noise_gate(h).clamp(-2.0, 2.0))  # (batch, hidden_dim)
        gated_latent = sde_latent * gate
        ...
        action_var = (gated_latent.pow(2)) @ std_sq  # (batch, act_dim)
