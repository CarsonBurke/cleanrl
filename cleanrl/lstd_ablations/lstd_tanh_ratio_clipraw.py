    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                ratio_raw = logratio_raw.exp()
                ratio_soft = (logratio_raw.tanh() * 0.3).exp()

                with torch.no_grad():
                    old_approx_kl = (-logratio_raw).mean()
                    approx_kl = ((ratio_raw - 1) - logratio_raw).mean()
                    clipfracs += [
                        ((ratio_raw < (1 - args.clip_coef_low)) | (ratio_raw > (1 + args.clip_coef_high)))
                        .float()
                        .mean()
                        .item()
                    ]
                ...
                # Policy loss: soft ratio for gradient, raw ratio for clipping
                pg_loss1 = -mb_advantages * ratio_soft
                pg_loss2 = -mb_advantages * torch.clamp(ratio_raw, 1 - args.clip_coef_low, 1 + args.clip_coef_high)
