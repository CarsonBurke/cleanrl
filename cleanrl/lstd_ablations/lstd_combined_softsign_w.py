...
LOG_STD_INIT = 0.1  # compensate for per-element W² (~1/64 → +2.08 from baseline -2.0)
LOG_STD_MIN = -3.0
LOG_STD_MAX = -0.5
SDE_EPS = 1e-6
...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        ...
        # RMSNorm → linear → soft-sign * 2: wider dynamic range, never fully saturates
        x = self.sde_fc2(self.sde_norm(sde_raw))
        sde_latent = x / (x.abs() + 1.0)  # (batch, hidden_dim)

        # Learned per-element log-std, no clamping
        log_std = self.log_std_param + LOG_STD_INIT
        std_sq = log_std.exp().pow(2)  # (hidden_dim, act_dim)

        # SDE variance with shared W coupling for policy-aligned exploration
