...
class Args:
    ...
    """Toggle learning rate annealing (disabled — using KL-adaptive LR instead)"""
    kl_target: float = 0.02
    """target KL divergence per update — LR adjusts to track this"""
    kl_max: float = 0.06
    """hard KL bound — early-stop epoch if exceeded"""
    lr_min: float = 1e-5
    """minimum learning rate"""
    lr_max: float = 3e-3
    """maximum learning rate"""
    kl_lr_adapt: float = 1.5
    """multiplicative factor for KL-based LR adjustment"""
    gamma: float = 0.99
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        # KL-adaptive LR: no schedule, LR is adjusted based on observed KL after each epoch
        if args.anneal_lr:
        ...
        for epoch in range(args.update_epochs):
            ...
            # Hard KL bound: early-stop this update if KL too high
            if approx_kl > args.kl_max:
                break

        # KL-adaptive LR: adjust once per update (after all epochs)
        # Proportional control: scale LR by target/actual KL ratio, clamped
        with torch.no_grad():
            kl_ratio = (args.kl_target / max(approx_kl.item(), 1e-8))
            lr_scale = max(min(kl_ratio, args.kl_lr_adapt), 1.0 / args.kl_lr_adapt)
            current_lr = max(min(current_lr * lr_scale, args.lr_max), args.lr_min)
            optimizer.param_groups[0]["lr"] = current_lr
