"""LSTD backbone + v5 independent Beta action distribution + ReLUSq.

This keeps the perturb_amprange SiLU/RMS actor and critic backbone plus
asymmetric PPO update, but replaces the low-rank Gaussian exploration model with
the v5 independent alpha/beta Beta distribution. It isolates whether the Beta
distribution can benefit from the stronger LSTD backbone and clipping recipe.

v2 keeps that Beta-centered setup and swaps only the actor/critic hidden
activation from SiLU to ReLU-squared.
"""
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def _actor_features(self, x):
        h = relu_sq(self.actor_norm1(self.actor_fc1(x)))
        h = relu_sq(self.actor_norm2(self.actor_fc2(h)))
        return h
    ...
    def _critic_features(self, x):
        h = relu_sq(self.critic_norm1(self.critic_fc1(x)))
        h = relu_sq(self.critic_norm2(self.critic_fc2(h)))
        return h
