"""PPO lstd_linear_tanh + weight-space trust region

Fork of lstd_linear_tanh with per-parameter weight delta clamping:

After each optimizer step, clamp (param_new - param_old) to a fraction of
the parameter's norm. This prevents any single weight from lurching too far
in one update, which can trap the policy in a narrow basin.

The insight: policy collapse isn't caused by large log-ratios per se, but by
specific parameters moving too fast. Global gradient clipping (max_grad_norm)
doesn't help because one parameter can eat the entire gradient budget.
Weight-space clamping directly constrains how far each parameter can move.
...
class Args:
    ...
    weight_delta_frac: float = 0.1
    """max per-parameter delta as fraction of parameter norm per minibatch step"""
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Policy loss (unclipped surrogate — trust region via weight clamping only)
                pg_loss = -(mb_advantages * ratio).mean()

                # Value loss (unclipped — trust region via weight clamping)
                ...
                param_snapshots = {name: p.data.clone() for name, p in agent.named_parameters() if p.requires_grad}
                ...
                # Weight-space trust region: clamp per-parameter deltas
                with torch.no_grad():
                    for name, p in agent.named_parameters():
                        if not p.requires_grad:
                            continue
                        old = param_snapshots[name]
                        delta = p.data - old
                        max_delta = args.weight_delta_frac * old.norm().clamp(min=1e-6)
                        delta_norm = delta.norm()
                        if delta_norm > max_delta:
                            p.data.copy_(old + delta * (max_delta / delta_norm))
