...
SDE_EPS = 1e-6
SDE_PRESCALE = 1.5
MIN_NOISE_OFFSET = 0.01  # soft lower bound on noise_offset² to prevent collapse
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # SDE noise: end-to-end learned, quadratic form for non-negativity
        ...
        # std=1.0 → W elements ~1/sqrt(64) ~0.125 → W² ~0.016 → matches gSDE exp(-2)²=0.018
        self.sde_out = layer_init(nn.Linear(hidden_dim, act_dim), std=1.0)
        # Learnable additive noise floor — squared for non-negativity + symmetric gradients
        # Init so noise_offset² = exp(-2.0) ≈ 0.1353, matching standard LOG_STD_INIT
        self.noise_offset = nn.Parameter(torch.full((act_dim,), math.exp(-1.0)))
    ...
    def _get_action_std(self, h):
        """State-dependent action std via SDE network + quadratic form.

        Uses latent² @ W² — guaranteed non-negative, sign-invariant in latent,
        smooth gradients everywhere (no saturation like softplus), and matches
        the gSDE variance structure (latent² @ std²) but end-to-end learned.
        """
        sde_raw = self.sde_fc(h)
        sde_latent = (self.sde_fc2(self.sde_norm(sde_raw)) / SDE_PRESCALE).tanh()
        action_var = (sde_latent.pow(2)) @ (self.sde_out.weight.T.pow(2))
        action_std = (action_var + SDE_EPS).sqrt() + self.noise_offset.pow(2) + MIN_NOISE_OFFSET
        return action_std
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        writer.add_scalar("tbot/noise_offset_mean", agent.noise_offset.mean().item(), global_step)
        writer.add_scalar("tbot/noise_offset_std", agent.noise_offset.std().item(), global_step)
