"""Perturb_amprange + action-conditioned covariance: feed action_mean (detached)
into the perturbation head so covariance adapts to where in action space the policy
is pointing. Near action boundaries needs different noise geometry than near zero.
Detach prevents covariance gradients from corrupting mean learning.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Perturbation head: takes hidden + detached action_mean
        self.perturb_head = layer_init(nn.Linear(hidden_dim + act_dim, self.noise_rank * act_dim), std=0.01)
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Action-conditioned perturbation: covariance sees where mean is pointing
        perturb_input = torch.cat([h, action_mean.detach()], dim=-1)
        perturb = self.perturb_head(perturb_input).reshape(batch, self.noise_rank, act_dim) * 0.1
