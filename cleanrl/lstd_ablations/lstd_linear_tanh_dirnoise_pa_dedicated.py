"""Dedicated noise layers: add a 2-layer sub-network after the backbone that
produces noise-specific features for the amplitude and perturbation heads.
This decouples noise feature learning from mean feature learning, matching
the baseline's architecture pattern. Static floor unchanged.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Dedicated noise sub-network: produces noise-specific features
        self.noise_fc = layer_init(nn.Linear(hidden_dim, hidden_dim), std=1.0)
        self.noise_norm = RMSNorm(hidden_dim)
        self.noise_fc2 = layer_init(nn.Linear(hidden_dim, hidden_dim), std=1.0)

        # Amplitude and perturbation heads read from noise features
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Dedicated noise features
        hn = F.silu(self.noise_norm(self.noise_fc(h)))
        hn = F.silu(self.noise_fc2(hn))
        raw = self.dir_head(hn)
        ...
        perturb = self.perturb_head(hn).reshape(batch, self.noise_rank, act_dim) * 0.1
