"""State-dependent diagonal: replace static log_std_floor with bounded state-dependent
diagonal using the same amprange pattern. The independent noise channel (sqrt(cov_diag)*eps)
was ~0.0025 vs correlated channel ~0.1-0.5 — essentially dead. This makes it meaningful
and state-adaptive. Init ~0.03 variance so it complements rather than overwhelms the low-rank.
cov_factor@z handles correlated exploration, sqrt(cov_diag)*eps handles independent fine control.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # State-dependent diagonal with amprange pattern
        # Init: softplus(floor_raw) ≈ 0.01, softplus(scale_raw) ≈ 0.05
        # So cov_diag range is ~[0.01, 0.06] — meaningful alongside low-rank ~0.1-0.25
        self.diag_head = layer_init(nn.Linear(hidden_dim, act_dim), std=0.01)
        nn.init.constant_(self.diag_head.bias, 0.0)
        self.diag_floor = nn.Parameter(torch.full((act_dim,), float(np.log(np.exp(0.01) - 1))))
        self.diag_scale = nn.Parameter(torch.full((act_dim,), float(np.log(np.exp(0.05) - 1))))
