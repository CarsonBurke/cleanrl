"""LSTD Explore: Curiosity-Driven Noise

Fork of lstd_linear_tanh with auxiliary loss pushing noise up where value prediction
error is high.

**Hypothesis**: States where the value function has high prediction error are poorly
understood. An auxiliary loss that encourages higher action_std in these states drives
exploration where it's most needed. The value error is detached so only noise params
receive gradients from this term.

Key changes from base:
1. get_action_and_value returns action_std as 5th value
2. v_error = |newvalue - b_returns|, normalized to [0,1] range, detached
3. curiosity_loss = -curiosity_coef * (action_std.mean(-1) * v_error_norm).mean()
4. Added to total loss — only noise params get gradients (v_error detached)
5. New arg: curiosity_coef (default 0.1)
"""
import os
import random
import time
...
class Args:
    ...
    """coefficient for curiosity-driven noise bonus"""
    vf_coef: float = 0.5
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, action_std = agent.get_action_and_value(
                    b_obs[mb_inds], b_actions[mb_inds], return_std=True
                )
                ...
                # Curiosity: push noise up where value prediction error is high
                with torch.no_grad():
                    v_error = (newvalue.detach() - b_returns[mb_inds]).abs()
                    v_error_norm = v_error / (v_error.max() + 1e-8)  # normalize to [0, 1]
                curiosity_loss = -args.curiosity_coef * (action_std.mean(-1) * v_error_norm).mean()

                loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef + curiosity_loss
