"""LSTD Beta policy with no-softmax logit CFM mean/concentration residual.

This keeps the perturb_amprange SiLU/RMS actor and critic backbone plus
asymmetric PPO update and the v5 independent alpha/beta Beta distribution.

The CFM head learns bounded-action transport in logit action space using the
same normalized positive-advantage weighting that won in latent_cov_nosoftmax.
At policy time, the CFM velocity shifts the Beta mean logit and modulates
concentration, while final PPO logprobs remain exact independent-Beta values.
"""
import os
import random
import time
...
class Args:
    ...
    cfm_coef: float = 0.03
    """coefficient for detached logit-space CFM auxiliary loss"""
    cfm_weight_clip: float = 5.0
    """maximum normalized positive-advantage CFM weight"""
    cfm_mean_coef: float = 0.25
    """Beta mean-logit residual from the learned CFM velocity"""
    cfm_conc_coef: float = 0.05
    """Beta concentration modulation from the learned CFM velocity"""
    batch_size: int = 0
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.cfm_conc_coef = float(args.cfm_conc_coef)
        ...
        self.cfm_velocity_head = nn.Sequential(
    ...
    def _cfm_velocity(self, h, y_t, t):
        if t.ndim == 1:
            t = t.unsqueeze(-1)
        return self.cfm_velocity_head(torch.cat((h, y_t, t), dim=-1))
    ...
    def _get_distribution(self, h):
        ...
        if self.cfm_mean_coef != 0.0 or self.cfm_conc_coef != 0.0:
            mean = alpha / (alpha + beta)
            concentration = (alpha + beta - 2.0).clamp_min(1e-4)
            mean_logit = safe_logit(mean)
            t0 = torch.zeros((h.shape[0], 1), device=h.device, dtype=h.dtype)
            velocity = self._cfm_velocity(h, mean_logit, t0)
            mean = torch.sigmoid(mean_logit + self.cfm_mean_coef * velocity)
            concentration = concentration * torch.exp(self.cfm_conc_coef * torch.tanh(velocity))
    ...
    def cfm_loss(self, x, actions, advantages, weight_clip):
        ...
        pred_velocity = self._cfm_velocity(h, y_t, t)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        return action, dist.log_prob(z).sum(1), dist.entropy().sum(1), self.get_value(x)
