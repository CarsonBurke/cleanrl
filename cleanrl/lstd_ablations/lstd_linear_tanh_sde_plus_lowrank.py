"""SDE + Low-Rank: Baseline's SDE diagonal variance as cov_diag in LowRankMVN,
plus a small perturb_amprange low-rank factor for explicit cross-action correlation.

Hypothesis: The baseline's strong SDE mechanism (384 params, dedicated layers,
rank-64 implicit covariance) handles per-action variance well but lacks explicit
cross-action correlation. Adding a small rank-6 low-rank factor provides
coordinated exploration across actuators.

Key design: SDE diagonal is primary (same scale as baseline), low-rank is
complementary (small amplitude range [0.02, 0.12] so it doesn't overwhelm).

Previous failures:
- pa_sde_diag (-155): Added FULL SDE variance to existing LARGE low-rank → overflow
- Here: SDE is primary, low-rank is small complement → balanced
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Actor backbone (same as baseline)
        ...
        # SDE mechanism for diagonal variance (identical to baseline)
        ...
        # Small low-rank for cross-action correlation (from perturb_amprange)
        ...
        # Small amplitude range: low-rank is complementary, not primary
        # amp ∈ [0.02, 0.12] at init → variance contribution ~0.002-0.08 total
        self.amp_floor = nn.Parameter(torch.full((self.noise_rank,), float(np.log(np.exp(0.02) - 1))))
        self.amp_scale = nn.Parameter(torch.full((self.noise_rank,), float(np.log(np.exp(0.1) - 1))))

        # Critic (same as baseline)
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # SDE diagonal variance (same as baseline)
        ...
        cov_diag = sde_latent.pow(2) @ std_sq + SDE_EPS  # (batch, act_dim)

        # Small low-rank factor for cross-action correlation
        raw = self.dir_head(h)
        ...
        effective_basis = self.noise_basis.unsqueeze(0) + perturb
        cov_factor = effective_basis.transpose(-2, -1) * amp.unsqueeze(1)  # (batch, act_dim, rank)
