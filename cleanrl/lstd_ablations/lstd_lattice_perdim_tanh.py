...
# Per-dim tanh surrogate: tanh(lr * k) / k, gradient=1 at lr=0
TANH_K_POS = 3.5   # 1/3.5 ≈ 0.286, matches clip_coef_high=0.28
TANH_K_NEG = 5.0   # 1/5.0 = 0.20, matches clip_coef_low=0.2
...
class Agent(nn.Module):
    ...
    def _sde_latent_and_std(self, h):
        """Compute SDE latent features and std matrix."""
        ...
        return sde_latent, std

    def _get_action_std(self, h):
        """Analytical diagonal std for log_prob (same L2 variance as original)."""
        sde_latent, std = self._sde_latent_and_std(h)
        action_var = (sde_latent.pow(2)) @ std.pow(2)  # (batch, act_dim)
        action_std = (action_var + SDE_EPS).sqrt()
        return action_std

    def _get_noise(self, h):
        """Lattice-style noise: latent @ N(0, std). Fresh sample each step.
        Cross-action correlation from shared latent; no temporal persistence."""
        sde_latent, std = self._sde_latent_and_std(h)
        exploration_mat = torch.randn_like(std) * std  # (hidden_dim, act_dim)
        return sde_latent @ exploration_mat  # (batch, act_dim)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            ...
            # Rollout: return summed log_prob for storage
            return action, probs.log_prob(action), probs.entropy().sum(1), self.get_value(x)
        else:
            # Training: return per-dim log_prob (batch, act_dim) for per-dim clipping
            return action, probs.log_prob(action), probs.entropy().sum(1), self.get_value(x)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Per-dim log ratios and ratios: (minibatch, act_dim)
                logratio = newlogprob - b_logprobs[mb_inds]  # (mb, act_dim)
                ratio = logratio.exp()  # (mb, act_dim)

                with torch.no_grad():
                    joint_logratio = logratio.sum(-1)  # for KL logging
                ...
                # Per-dim tanh surrogate: tanh(lr * k) / k, no clip needed
                # mb_advantages is (mb,), broadcast to (mb, act_dim)
                k = torch.where(logratio > 0, TANH_K_POS, TANH_K_NEG)
                surrogate = (logratio * k).tanh() / k
                pg_loss = -(mb_advantages.unsqueeze(-1) * surrogate).mean()
