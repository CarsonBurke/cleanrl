...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        """Analytical diagonal std with shared W scaling — policy-aligned exploration."""
        sde_latent, std = self._sde_latent_and_std(h)
        action_var = (sde_latent.pow(2)) @ std.pow(2)  # (batch, act_dim)
        # Scale by actor_out W² norm — couples noise magnitude to mean projection
        W = self.actor_out.weight  # (act_dim, hidden_dim)
        w_sq_norm = (W ** 2).sum(dim=1)  # (act_dim,)
        action_var = action_var * w_sq_norm
        action_std = (action_var + SDE_EPS).sqrt()
        return action_std

    def _get_noise(self, h):
        """Lattice noise scaled by W norm — policy-aligned exploration."""
        sde_latent, std = self._sde_latent_and_std(h)
        exploration_mat = torch.randn_like(std) * std  # (hidden_dim, act_dim)
        raw_noise = sde_latent @ exploration_mat  # (batch, act_dim)
        # Scale noise by sqrt(W² norm) to match variance formula
        W = self.actor_out.weight
        w_norm = (W ** 2).sum(dim=1).sqrt()  # (act_dim,)
        return raw_noise * w_norm
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Per-dim PPO clipping: clip each action dim's ratio independently
