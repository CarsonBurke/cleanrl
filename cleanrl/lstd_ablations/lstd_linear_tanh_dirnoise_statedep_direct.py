"""Ablation A: Direct head cov_factor — fully state-dependent low-rank covariance.
Change: Replace fixed noise_basis + dir_head with a single linear head that outputs
the entire cov_factor from actor features: head(h).reshape(batch, act_dim, noise_rank).
64->36 linear (act_dim * noise_rank). Init with small std.
The whole cov_factor is now state-dependent — no fixed basis matrix.
Keep direct sigmoid amplitude concept: the head output goes through sigmoid to bound it,
then serves as the full cov_factor. Keep mean_scale, FLOOR_LOG_STD=-3.0, etc from directamp.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Single head that outputs the entire cov_factor: hidden_dim -> act_dim * noise_rank
        self.cov_factor_head = layer_init(
            nn.Linear(hidden_dim, act_dim * self.noise_rank), std=0.01
        )
        nn.init.constant_(self.cov_factor_head.bias, 0.0)
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Direct state-dependent cov_factor: head outputs full factor
        raw = self.cov_factor_head(h)  # (batch, act_dim * noise_rank)
        # Apply direct sigmoid amplitude bounding to [0.05, 1.0]
        cov_factor = 0.05 + 0.95 * torch.sigmoid(raw)
        cov_factor = cov_factor.reshape(batch, act_dim, self.noise_rank)
