"""Combined ablation: direct amplitude + no mean_scale + low rank.
Combines the two strongest signals:
- Direct sigmoid amplitude (0.05 + 0.95*sigmoid) instead of exp(log_amp) — eliminates
  gradient bottleneck at low amplitudes, 4x higher initial exploration
- Remove mean_scale scalar, use std=0.01 layer init — eliminates shared scalar bottleneck
Also tests low rank (4) with proper amplitude, since rank reduction was inconclusive
when the amplitude parameterization was broken.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        hidden_dim = 64
        # Low rank: force structured exploration directions
        self.noise_rank = min(act_dim, 4)
        ...
        # No mean_scale — std=0.01 layer init directly
    ...
    def _get_distribution(self, h, action_mean):
        ...
        raw = self.dir_head(h)
        # Direct sigmoid amplitude — no exp bottleneck
