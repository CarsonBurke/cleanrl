"""LSTD directional noise — mean-aware ablation

Mean-aware noise: dir_head takes both actor features h AND the action mean as
input, letting the network learn to explore differently based on what action is
being proposed (e.g., orthogonal to the mean direction).

Base covariance structure is unchanged (low-rank + diagonal floor):
  action ~ N(mean, B^T diag(amp(h, mean)²) B + diag(floor²))

The only difference from the base dirnoise variant is that dir_head receives
[h; action_mean] instead of just h.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.dir_head = layer_init(nn.Linear(hidden_dim + act_dim, self.noise_rank), std=0.01)
    ...
    def _get_distribution(self, h, action_mean):
        ...
        dir_input = torch.cat([h, action_mean], dim=-1)  # (batch, hidden_dim + act_dim)
        raw = self.dir_head(dir_input)  # (batch, noise_rank)
