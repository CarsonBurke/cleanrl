"""SDE Abs: Replace sde_latent.pow(2) with sde_latent.abs() in variance computation.

The baseline computes: action_var = sde_latent.pow(2) @ std_sq
This creates a gradient dead zone: when sde_latent[k] ≈ 0, the gradient
through pow(2) is 2*latent[k] ≈ 0, so that noise dimension can't reactivate.

With abs(): action_var = sde_latent.abs() @ std_sq
The gradient through abs is sign(latent[k]) * std_sq, which has constant
magnitude regardless of latent value. All noise dimensions maintain equal
gradient flow, potentially enabling more efficient use of the rank-64 capacity.

Note: abs() still guarantees non-negative variance (abs >= 0, std_sq >= 0).

Minimal change: only sde_latent.pow(2) → sde_latent.abs() in one line.
"""
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        ...
        # KEY CHANGE: abs() instead of pow(2) — removes gradient dead zone
        action_var = sde_latent.abs() @ std_sq
