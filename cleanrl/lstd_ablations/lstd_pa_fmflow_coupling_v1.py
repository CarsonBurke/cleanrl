"""LSTD perturb-amprange with an exact residual coupling flow.

The TD-Flow paper's useful lesson for PPO actions is to model transport from a
simple source to a richer target while preserving tractable likelihoods. This
variant keeps the high-performing LSTD low-rank Gaussian as the source and adds
two identity-initialized affine coupling blocks over raw pre-clip actions. PPO
ratios remain exact via the change-of-variables formula.
"""
import os
import random
import time
...
class AffineCouplingBlock(nn.Module):
    def __init__(self, action_dim, cond_dim, flip=False, flow_scale=1.5):
        super().__init__()
    ...
    def forward(self, x, cond):
        ...
        shift, raw_scale = self.net(torch.cat((cond, left), dim=-1)).chunk(2, dim=-1)
        log_scale = self.flow_scale * torch.tanh(raw_scale)
        transformed = right * torch.exp(log_scale) + shift
        y = self._merge(left, transformed)
    ...
    def inverse(self, y, cond):
        ...
        shift, raw_scale = self.net(torch.cat((cond, left), dim=-1)).chunk(2, dim=-1)
        log_scale = self.flow_scale * torch.tanh(raw_scale)
        right = (transformed - shift) * torch.exp(-log_scale)
        x = self._merge(left, right)
        return x, -log_scale.sum(dim=-1)


class ActionFlow(nn.Module):
    def __init__(self, action_dim, cond_dim, flow_scale):
        super().__init__()
        self.blocks = nn.ModuleList(
            [
                AffineCouplingBlock(action_dim, cond_dim, flip=False, flow_scale=flow_scale),
                AffineCouplingBlock(action_dim, cond_dim, flip=True, flow_scale=flow_scale),
            ]
        )
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.action_flow = ActionFlow(act_dim, hidden_dim, args.flow_scale)
