"""Perturb + learned scale: per-direction learned perturbation scale instead of
fixed 0.1. Different basis directions may need different amounts of state-dependent
rotation. Uses softplus to keep scales positive.
Change vs perturb: Replace fixed * 0.1 with * softplus(perturb_scale) where
perturb_scale is nn.Parameter((noise_rank,)) initialized so softplus ≈ 0.1.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Learned per-direction perturbation scale (replaces fixed 0.1)
        # inv_softplus(0.1) = log(exp(0.1) - 1) ≈ -2.25
        self.perturb_scale = nn.Parameter(torch.full((self.noise_rank,), float(np.log(np.exp(0.1) - 1))))
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # State-dependent perturbation with learned per-direction scale
        scale = F.softplus(self.perturb_scale).unsqueeze(0).unsqueeze(-1)  # (1, noise_rank, 1)
        perturb = self.perturb_head(h).reshape(batch, self.noise_rank, act_dim) * scale
