"""Nonlinear amplitude head: replace the single linear dir_head + learned range
with a small MLP (64→16→SiLU→6→softplus). This gives the amplitude its own
nonlinearity and more capacity to express rich state-dependent scaling, making
the static learned floor/scale parameters unnecessary.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Nonlinear amplitude MLP: h → 16 → SiLU → noise_rank → softplus
        self.amp_hidden = layer_init(nn.Linear(hidden_dim, 16), std=0.01)
        self.amp_out = nn.Linear(16, self.noise_rank)
        # Init amp_out so softplus(output) ≈ 0.5 at start (mid-range)
        nn.init.constant_(self.amp_out.weight, 0.0)
        nn.init.constant_(self.amp_out.bias, float(np.log(np.exp(0.5) - 1)))
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Nonlinear amplitude: MLP with its own hidden layer
        amp = F.softplus(self.amp_out(F.silu(self.amp_hidden(h))))
