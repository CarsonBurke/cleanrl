"""LSTD directional noise + advantage-supervised basis learning

Fork of dirnoise. Adds an auxiliary loss that gives the noise basis a direct
gradient signal from advantages, bypassing the z²-1 zero-mean gradient problem.

Problem: PG gradients to noise parameters average to zero (E[z²-1]=0 for
any exponential family). The basis drifts randomly, exploring dangerous
Walker2d directions (balance disruption) as much as safe ones (speed).

Solution: during PPO updates, compute the deviation of each action from the
current mean, project onto basis directions, and weight by advantage:

  noise_dir = normalize(action - mean)
  proj = noise_dir @ basis.T
  noise_dir_loss = -(advantage * proj).mean()

This directly teaches the basis: "actions that deviated in THIS direction got
high advantage — align a basis vector there." No z²-1 cancellation because
this loss doesn't go through log_prob.

Previous results @ 8M:
- dirnoise HalfCheetah: ~5150
- dirnoise Walker2d: ~1200 (high variance, balance collapses)
"""
import os
import random
import time
...
FLOOR_LOG_STD = -3.0  # diagonal floor std ≈ 0.05
...
class Args:
    ...
    noise_dir_coef: float = 0.1
    """coefficient for the advantage-supervised noise direction loss"""
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Diagonal noise floor (small per-action baseline)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        return action, dist.log_prob(action), dist.entropy(), self.get_value(x), action_mean
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, action_mean = agent.get_action_and_value(
                ...
                # Advantage-supervised noise direction loss
                noise = (b_actions[mb_inds] - action_mean).detach()
                noise_dir = noise / (noise.norm(dim=-1, keepdim=True) + 1e-8)
                # Project onto basis directions (basis gradients flow here)
                proj = noise_dir @ agent.noise_basis.T  # (batch, K)
                noise_dir_loss = -(mb_advantages.unsqueeze(-1) * proj).mean()

                loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef + args.noise_dir_coef * noise_dir_loss
        ...
        writer.add_scalar("losses/noise_dir_loss", noise_dir_loss.item(), global_step)
        ...
        with torch.no_grad():
            ...
            writer.add_scalar("noise/floor_log_std", agent.log_std_floor.mean().item(), global_step)
