"""LSTD directional noise ablation: basis normalization

Normalize noise_basis rows to unit norm before computing cov_factor, preventing
basis norm from growing unboundedly and removing the redundancy between basis
magnitude and amplitude scaling.

Base: lstd_linear_tanh_dirnoise (low-rank multivariate normal with state-dependent
directional amplitudes). This ablation adds a single normalization step so that
only the amplitude head controls the scale of each exploration direction.
"""
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Normalize basis to unit rows so only amplitude controls scale
        basis_normed = self.noise_basis / (self.noise_basis.norm(dim=-1, keepdim=True) + 1e-8)
        cov_factor = basis_normed.T.unsqueeze(0) * amp.unsqueeze(1)
