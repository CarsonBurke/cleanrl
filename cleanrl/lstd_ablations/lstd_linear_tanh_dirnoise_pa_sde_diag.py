"""SDE diagonal: replace static learnable floor with the baseline's SDE-style
state-dependent diagonal variance. Dedicated 2-layer sub-network produces noise
features, then latent² @ std² gives per-action state-dependent variance.
Keeps our low-rank directional structure (basis + perturb + amprange) on top.
cov = B_eff^T diag(amp²) B_eff + diag(sde_var)
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # SDE diagonal: dedicated sub-network for state-dependent variance
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # SDE diagonal: dedicated layers → tanh latent → variance via matrix multiply
        sde_latent = (self.sde_fc2(self.sde_norm(self.sde_fc(h))) / SDE_PRESCALE).tanh()
        log_std = (self.log_std_param + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
        cov_diag = sde_latent.pow(2) @ log_std.exp().pow(2) + SDE_EPS
