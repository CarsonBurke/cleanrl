...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Per-action noise scale (decouples magnitude from latent norm)
        # Init ~sqrt(E[||latent||²]) to match original variance at start
        self.noise_scale = nn.Parameter(torch.full((act_dim,), np.sqrt(hidden_dim * 0.34)))
    ...
    def _get_action_std(self, h):
        """Analytical diagonal std with L2-normalized latent."""
        sde_latent, std = self._sde_latent_and_std(h)
        latent_norm = sde_latent / (sde_latent.norm(dim=-1, keepdim=True) + 1e-6)
        action_var = (latent_norm.pow(2)) @ std.pow(2) * self.noise_scale.pow(2)
        action_std = (action_var + SDE_EPS).sqrt()
        return action_std

    def _get_noise(self, h):
        """Lattice noise with L2-normalized latent — cascade-proof.
        Direction is state-dependent, magnitude from noise_scale + std."""
        sde_latent, std = self._sde_latent_and_std(h)
        latent_norm = sde_latent / (sde_latent.norm(dim=-1, keepdim=True) + 1e-6)
        exploration_mat = torch.randn_like(std) * std
        return (latent_norm @ exploration_mat) * self.noise_scale
