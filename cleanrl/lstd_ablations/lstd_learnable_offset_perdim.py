...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Learned per-element noise magnitude: log_std = (param + log_std_offset).clamp(min, max)
        self.log_std_param = nn.Parameter(torch.zeros(hidden_dim, act_dim))
        # Per-dim learnable offset (replaces fixed LOG_STD_INIT=-2.0), init at same value
        self.log_std_offset = nn.Parameter(torch.full((act_dim,), LOG_STD_INIT))
    ...
    def _get_action_std(self, h):
        ...
        log_std = (self.log_std_param + self.log_std_offset).clamp(LOG_STD_MIN, LOG_STD_MAX)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                logratio = logratio_raw.tanh() * 0.3
                ratio = logratio.exp()

                with torch.no_grad():
                    # calculate approx_kl http://joschu.net/blog/kl-approx.html
                    old_approx_kl = (-logratio_raw).mean()
                    approx_kl = ((logratio_raw.exp() - 1) - logratio_raw).mean()
                    clipfracs += [
        ...
        log_std_eff = (agent.log_std_param + agent.log_std_offset).clamp(LOG_STD_MIN, LOG_STD_MAX)
        ...
        writer.add_scalar("tbot/log_std_offset_mean", agent.log_std_offset.mean().item(), global_step)
        writer.add_scalar("tbot/log_std_offset_std", agent.log_std_offset.std().item(), global_step)
