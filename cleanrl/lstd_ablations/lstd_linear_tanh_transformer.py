...
class TransformerBlock(nn.Module):
    """Pre-norm Transformer++ block: MHSA + SwiGLU FFN with residual connections."""
    def __init__(self, d_token=32, n_heads=4, ffn_hidden=64):
        ...
        # QKV packed projection (no bias, standard transformer convention)
        self.qkv = nn.Linear(d_token, 3 * d_token, bias=False)
        # Output projection — small init so residual starts near identity
        ...
        # SwiGLU: projects to 2x intermediate (gate + value), then back
