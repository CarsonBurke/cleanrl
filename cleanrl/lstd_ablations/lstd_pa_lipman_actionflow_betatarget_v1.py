"""Beta-logit action flow with Beta-target bounded-space Flow Matching.

The policy replaces the multivariate normal with an independent Beta source
followed by a state-conditioned additive coupling flow in logit action space.
The flow is exactly invertible, so PPO ratios use the true transformed density.

Flow matching is applied to the same coupling velocity blocks. Source samples
come from the policy Beta distribution. Targets are not hard rollout actions;
they are local Beta distributions centered on rollout actions, so the endpoint
remains bounded and smooth instead of becoming a brittle point mass.
"""
import os
import random
import time
...
class Args:
    ...
    flow_steps: int = 4
    """number of additive coupling Euler steps in logit action space"""
    cfm_coef: float = 0.03
    ...
    cfm_target_concentration: float = 64.0
    """Beta concentration around each rollout action endpoint"""
...
class AdditiveVelocityCouplingBlock(nn.Module):
    ...
    def _velocity(self, left, cond, t):
        if t.ndim == 0:
            ...
            t = t.unsqueeze(-1)
        return self.net(torch.cat((cond, left, t.to(dtype=left.dtype, device=left.device)), dim=-1))
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.cfm_target_concentration = float(args.cfm_target_concentration)
        ...
        self.action_flow = BetaLogitActionFlow(act_dim, hidden_dim, args.flow_steps)
    ...
    def cfm_loss(self, x, actions, advantages, weight_clip):
        ...
        with torch.no_grad():
            ...
            y0 = safe_logit(z0)
            action_z = self._action_to_z(actions.detach())
            target_conc = torch.as_tensor(self.cfm_target_concentration, device=action_z.device, dtype=action_z.dtype)
            target_alpha = 1.0 + target_conc * action_z
            target_beta = 1.0 + target_conc * (1.0 - action_z)
            target_z = Beta(target_alpha, target_beta).sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            y1 = safe_logit(target_z)
