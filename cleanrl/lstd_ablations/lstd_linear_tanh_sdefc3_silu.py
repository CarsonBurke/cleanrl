"""LSTD linear-tanh + sde_fc3 with SiLU instead of tanh

Fork of lstd_linear_tanh_sdefc3. Key change:

**Replace tanh with SiLU in the SDE path.** tanh bounds to [-1,1] but
compresses dynamic range and saturates gradients — redundant given RMSNorm
(controls input magnitude) and clamp (controls output). SiLU is unbounded,
preserves gradients, and has smooth nonlinearity. The clamp on log_std
after fc3 is the actual safety net.

Architecture:
  sde_fc → sde_norm → sde_fc2 → SiLU → sde_fc3 → + LOG_STD_INIT → clamp → exp → std

Results from sdefc3 (tanh): HalfCheetah ~3850 @ 8M
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # SDE noise path: fc → norm → fc2 → SiLU → fc3 → per-action log-std
    ...
    def _get_action_std(self, h):
        ...
        sde_latent = F.silu(self.sde_fc2(self.sde_norm(sde_raw)))  # (batch, hidden_dim) — unbounded, smooth
