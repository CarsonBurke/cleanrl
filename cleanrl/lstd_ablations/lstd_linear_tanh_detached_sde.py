"""Ablation: SDE gets its own independent backbone (3 separate backbones).

Hypothesis: shared_sde underperforms because value gradients distort SDE features.
The original lstd_linear_tanh has SDE branching from actor features, meaning SDE
is shaped by policy gradients. But when shared with critic, value loss corrupts
the SDE representations.

This ablation goes the other direction: SDE gets its OWN 2-layer MLP backbone,
completely independent from both actor and critic. The SDE backbone is trained
only through policy gradients (via log_prob -> std -> SDE features), giving it
maximum freedom to learn exploration-specific representations.

3 backbones: actor, critic, SDE (each independent 2-layer MLP).
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # SDE backbone (independent — NOT shared with actor or critic)
        self.sde_backbone_fc1 = layer_init(nn.Linear(obs_dim, hidden_dim))
        self.sde_backbone_norm1 = RMSNorm(hidden_dim)
        self.sde_backbone_fc2 = layer_init(nn.Linear(hidden_dim, hidden_dim))
        self.sde_backbone_norm2 = RMSNorm(hidden_dim)
        # SDE head
    ...
    def _sde_features(self, x):
        h = F.silu(self.sde_backbone_norm1(self.sde_backbone_fc1(x)))
        h = F.silu(self.sde_backbone_norm2(self.sde_backbone_fc2(h)))
        return h

    def _get_action_std(self, sde_h):
        sde_raw = self.sde_fc(sde_h)
    ...
    def get_action_and_value(self, x, action=None):
        actor_h = self._actor_features(x)
        sde_h = self._sde_features(x)
        action_mean = self.actor_out(actor_h) * self.mean_scale
        action_std = self._get_action_std(sde_h)
