"""lstd_linear_tanh with shared encoder backbone.

Fork of lstd_linear_tanh that replaces separate actor/critic backbones with a
single shared encoder. Actor SDE head and critic value head both operate on
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Shared encoder backbone: obs -> hidden features (used by both actor and critic)
        self.encoder_fc1 = layer_init(nn.Linear(obs_dim, hidden_dim))
        self.encoder_norm1 = RMSNorm(hidden_dim)
        self.encoder_fc2 = layer_init(nn.Linear(hidden_dim, hidden_dim))
        self.encoder_norm2 = RMSNorm(hidden_dim)
        ...
        # Scalar value head (operates on shared encoder output)
        self.value_out = layer_init(nn.Linear(hidden_dim, 1), std=1.0)

    def _encoder_features(self, x):
        h = F.silu(self.encoder_norm1(self.encoder_fc1(x)))
        h = F.silu(self.encoder_norm2(self.encoder_fc2(h)))
        return h
    ...
    def get_value(self, x):
        return self.value_out(self._encoder_features(x))

    def get_action_and_value(self, x, action=None):
        h = self._encoder_features(x)
