"""Ablation B: State-dependent perturbation of fixed basis.
Change: Add a state-dependent perturbation to the fixed orthogonal basis.
effective_basis = B_fixed + perturb_head(h).reshape(noise_rank, act_dim) * 0.1
Then cov_factor = effective_basis.T * amp.unsqueeze(1) as before.
Hypothesis: The fixed basis limits expressiveness. A small state-dependent perturbation
allows the covariance structure to adapt to the current state while maintaining the
stability of the orthogonal initialization.
"""
import os
import random
import time
