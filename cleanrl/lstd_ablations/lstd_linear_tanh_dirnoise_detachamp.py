"""Ablation: Detach amplitude from basis gradient path (built on directamp).
Change: Use amp.detach() when scaling the basis in cov_factor, so basis always gets
unit-scale gradients regardless of amplitude. Amplitude still gets gradients normally.
This breaks the chicken-and-egg problem where low-amplitude directions get vanishing
basis gradients and can never improve.
"""
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # ABLATION: detach amp from basis gradient path
        # Basis gets gradients as if amp=1 (unit scale), amp still learns normally
        # cov_factor uses amp for forward pass but amp.detach() for basis grads
        cov_factor = self.noise_basis.T.unsqueeze(0) * amp.detach().unsqueeze(1) \
                   + (self.noise_basis.T.unsqueeze(0)).detach() * (amp - amp.detach()).unsqueeze(1)
