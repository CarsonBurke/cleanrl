"""Ablation D: Overcomplete basis mixing with state-dependent selection.
Change: Create K=12 fixed orthogonal-init basis vectors (overcomplete for act_dim=6).
Output mixing weights from actor features: weights = head(h).reshape(noise_rank, K),
apply softmax, blend: effective_basis = softmax(weights) @ all_basis.
noise_rank stays = act_dim = 6. Keep direct sigmoid amplitude from directamp.
Hypothesis: An overcomplete basis set gives the agent more directional options to choose
from. State-dependent softmax mixing lets the policy adaptively combine basis vectors,
potentially discovering richer exploration directions than a fixed noise_rank-sized basis.
"""
import os
import random
import time
...
FLOOR_LOG_STD = -3.0
NUM_BASIS = 12  # K: overcomplete basis size
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Overcomplete basis: K=12 vectors of dimension act_dim, fixed (not learned)
        basis = torch.empty(self.num_basis, act_dim)
        nn.init.orthogonal_(basis)
        self.register_buffer('noise_basis', basis)  # fixed, not a Parameter

        # State-dependent mixing: outputs noise_rank * K weights to mix basis vectors
        self.mix_head = layer_init(nn.Linear(hidden_dim, self.noise_rank * self.num_basis), std=0.01)
        nn.init.constant_(self.mix_head.bias, 0.0)

        # Direct sigmoid amplitude head (from directamp)
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # State-dependent mixing of overcomplete basis
        mix_logits = self.mix_head(h).reshape(batch, self.noise_rank, self.num_basis)
        mix_weights = F.softmax(mix_logits, dim=-1)  # (batch, noise_rank, K)
        # effective_basis: (batch, noise_rank, act_dim)
        effective_basis = torch.matmul(mix_weights, self.noise_basis)  # (batch, noise_rank, K) @ (K, act_dim)

        # Direct sigmoid amplitude (from directamp)
        raw = self.dir_head(h)
        amp = 0.05 + 0.95 * torch.sigmoid(raw)  # (batch, noise_rank)

        # cov_factor: (batch, act_dim, noise_rank)
        # effective_basis is (batch, noise_rank, act_dim), transpose last two dims
        cov_factor = effective_basis.transpose(1, 2) * amp.unsqueeze(1)  # (batch, act_dim, noise_rank)
