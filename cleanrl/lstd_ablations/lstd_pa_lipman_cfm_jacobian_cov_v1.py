"""LSTD perturb-amprange with CFM mean and Jacobian covariance.

This keeps the successful LSTD perturb-amprange PPO policy intact and adds a
simulation-free OT-CFM objective over rollout actions. The learned CFM velocity
still shifts the policy mean, but now its local finite-difference Jacobian near
the mean also perturbs the low-rank covariance basis. This lets the vector field
shape correlated exploration without replacing PPO's explicit Gaussian policy.
"""
import os
import random
import time
...
class Args:
    ...
    """policy mean residual from the learned CFM velocity field"""
    cfm_cov_coef: float = 0.05
    """coefficient for CFM Jacobian low-rank basis perturbation"""
    cfm_probe_eps: float = 0.05
    """finite-difference probe size for local CFM stretch directions"""
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.cfm_probe_eps = float(args.cfm_probe_eps)
    ...
    def _get_distribution(self, h, action_mean, basis_delta=None):
        batch = h.shape[0]
        ...
        if basis_delta is not None:
            effective_basis = effective_basis + basis_delta
    ...
    def _cfm_policy_terms(self, h, base_mean):
        ...
        base_velocity = self._cfm_velocity(h, base_mean, t0)
        action_mean = base_mean + self.cfm_mean_coef * base_velocity
        basis_delta = None
        basis_delta_norm = base_mean.new_tensor(0.0)
        if self.cfm_cov_coef != 0.0:
            eye = torch.eye(self.act_dim, device=h.device, dtype=h.dtype)
            probes = base_mean.unsqueeze(1) + self.cfm_probe_eps * eye.unsqueeze(0)
            probe_h = h.unsqueeze(1).expand(-1, self.act_dim, -1).reshape(-1, h.shape[-1])
            probe_t = torch.zeros((h.shape[0] * self.act_dim, 1), device=h.device, dtype=h.dtype)
            probe_v = self._cfm_velocity(probe_h, probes.reshape(-1, self.act_dim), probe_t)
            probe_v = probe_v.reshape(h.shape[0], self.act_dim, self.act_dim)
            jacobian_cols = (probe_v - base_velocity.unsqueeze(1)) / self.cfm_probe_eps
            basis_delta = self.cfm_cov_coef * torch.tanh(jacobian_cols)
            basis_delta_norm = basis_delta.norm(dim=(-2, -1)).mean()
        return action_mean, basis_delta, (self.cfm_mean_coef * base_velocity).norm(dim=-1).mean(), basis_delta_norm

    def cfm_loss(self, x, actions, advantages, weight_clip):
        h = self._actor_features(x)
        x1 = actions.detach()
        if self.cfm_policy_source:
            with torch.no_grad():
                source_mean = self.actor_out(h.detach()) * self.mean_scale.detach()
                source_dist = self._get_distribution(h.detach(), source_mean)
                x0 = source_dist.sample()
        else:
            x0 = torch.randn_like(x1)
        t = torch.rand((x1.shape[0], 1), device=x1.device, dtype=x1.dtype)
        sigma_t = 1.0 - (1.0 - self.cfm_sigma_min) * t
        x_t = sigma_t * x0 + t * x1
        target_velocity = x1 - (1.0 - self.cfm_sigma_min) * x0
        pred_velocity = self._cfm_velocity(h, x_t, t)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        action_mean, basis_delta, _, _ = self._cfm_policy_terms(h, base_mean)
        dist = self._get_distribution(h, action_mean, basis_delta=basis_delta)
        if action is None:
            action = dist.sample()
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            h_diag = agent._actor_features(b_obs[: min(1024, b_obs.shape[0])])
            base_mean = agent.actor_out(h_diag) * agent.mean_scale
            _, _, residual_norm, basis_delta_norm = agent._cfm_policy_terms(h_diag, base_mean)
        ...
        writer.add_scalar("diagnostics/cfm_basis_delta_norm", basis_delta_norm.item(), global_step)
