"""Perturb + amprange: combine state-dependent basis perturbation with learned
per-direction amplitude range. Perturb rotates directions, amprange optimizes
amplitude scaling — orthogonal improvements.
Change vs perturb: Replace fixed amp = 0.05 + 0.95 * sigmoid(raw) with
amp = softplus(amp_floor) + softplus(amp_scale) * sigmoid(raw), both learnable per-direction.
"""
import os
import random
import time
