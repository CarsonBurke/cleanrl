"""Perturb + normed v2: normalize effective basis directions to unit norm, bounded
state-dependent amplitude via 0.05 + 0.95 * sigmoid(amp_head(h)).
v1 used softplus (unbounded) and scored 6656. v2 bounds amplitude to [0.05, 1.0] — same
range as amprange init but fully state-dependent with no slow learnable range params.
Change vs perturb_amprange: remove amp_floor/amp_scale, normalize basis rows, bounded sigmoid amp.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Amplitude head: fully state-dependent, sigmoid output bounded to [0.05, 1.0]
        self.amp_head = layer_init(nn.Linear(hidden_dim, self.noise_rank), std=0.01)
        nn.init.constant_(self.amp_head.bias, 0.0)  # sigmoid(0)=0.5 → amp ≈ 0.525

        # Perturbation head: state-dependent rotation of basis
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Fully state-dependent amplitude, bounded to [0.05, 1.0]
        amp = 0.05 + 0.95 * torch.sigmoid(self.amp_head(h))  # (batch, noise_rank)
        ...
        effective_basis = F.normalize(effective_basis, dim=-1)
