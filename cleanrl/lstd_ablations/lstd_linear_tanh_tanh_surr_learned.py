"""LSTD linear_tanh + tanh surrogate with LEARNED scale

Same as tanh_surrogate but the 0.3 scale is a learnable parameter (init 0.3).
The agent can widen or tighten its own trust region during training.
Uses softplus to keep the scale positive.
...
SURROGATE_SCALE_INIT = 0.3  # initial tanh surrogate output scale
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Learnable surrogate scale (softplus to keep positive, init matches 0.3)
        # softplus_inv(0.3) ≈ -0.847 so softplus(-0.847) ≈ 0.3
        self.surrogate_scale_raw = nn.Parameter(torch.tensor(float(np.log(np.exp(SURROGATE_SCALE_INIT) - 1))))
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                logratio_raw = newlogprob - b_logprobs[mb_inds]
                surr_scale = F.softplus(agent.surrogate_scale_raw)
                logratio = logratio_raw.tanh() * surr_scale
                ratio = logratio.exp()
                ...
                # Policy loss — standard PPO clip on tanh-compressed ratio
        ...
        writer.add_scalar("losses/surrogate_scale", F.softplus(agent.surrogate_scale_raw).item(), global_step)
