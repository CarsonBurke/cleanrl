"""Cross-direction attention perturbation.
Change vs perturb: Before applying perturbation, let noise directions attend to each
other via a tiny rank×rank attention matrix. This lets directions coordinate — e.g.
"if direction 3 covers hip torque, direction 5 should avoid it."
The raw perturbation V comes from perturb_head, but is mixed across directions via
learned attention weights before being added to the basis.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Cross-direction attention: learned Q/K embeddings for basis rows
        d_head = 8
        self.basis_q = nn.Parameter(torch.randn(self.noise_rank, d_head) * 0.1)
        self.basis_k = nn.Parameter(torch.randn(self.noise_rank, d_head) * 0.1)
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Cross-direction attention: mix perturbations across directions
        V = self.perturb_head(h).reshape(batch, self.noise_rank, act_dim)  # (batch, rank, act_dim)
        attn = (self.basis_q @ self.basis_k.T) * self.attn_scale  # (rank, rank)
        attn = torch.softmax(attn, dim=-1)
        perturb = torch.einsum('ij,bjd->bid', attn, V) * 0.1  # (batch, rank, act_dim)
