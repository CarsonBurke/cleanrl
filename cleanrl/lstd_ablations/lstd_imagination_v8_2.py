"""Imagination v8.2 — v8.1 + Model-Based Value Expansion (MVE)

Builds on v8.1 (PPO + World Model Auxiliary Loss + standard diagonal Gaussian actor)
by adding Model-Based Value Expansion (MVE) to improve value targets:

- Standard diagonal Gaussian actor (actor_mean Sequential + actor_logstd parameter)
- Standard critic on latents, symmetric clip_coef=0.2
- Encoder with target encoder EMA, world model (transition + reward prediction)
- MVE: after GAE, for each timestep, unroll world model H steps from encoded latent,
  accumulate discounted predicted rewards, bootstrap from critic at final latent.
  Blend: returns = (1-mve_coef)*standard_returns + mve_coef*mve_returns
"""
import os
import random
import time
...
class Args:
    ...
    mve_horizon: int = 5
    """Number of world model rollout steps for MVE targets"""
    mve_coef: float = 0.3
    """Blending coefficient: target = (1-mve_coef)*standard + mve_coef*mve"""
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Standard actor on latents (diagonal Gaussian)
    ...
    def get_value_from_latent(self, latent):
        """Compute value directly from a latent vector (no encoding step)."""
...
def compute_mve_returns(agent, obs, actions, dones, next_obs_final, next_done_final,
                        values, rewards, args, device):
    """Compute MVE-blended returns.

    For each timestep t, unrolls the world model H steps forward from the encoded
    latent at obs[t], accumulating discounted predicted rewards and bootstrapping
    from the critic at the final latent. Then blends with standard GAE returns.

    Returns:
        returns: (num_steps, num_envs) blended returns
        advantages: (num_steps, num_envs) advantages
    """
    ...
    with torch.no_grad():
        # --- Standard GAE ---
        next_value = agent.get_value(next_obs_final).reshape(1, -1)
        advantages = torch.zeros_like(rewards)
        lastgaelam = 0
        for t in reversed(range(num_steps)):
            if t == num_steps - 1:
                nextnonterminal = 1.0 - next_done_final
        ...
        standard_returns = advantages + values

        # --- MVE returns ---
        # Encode all observations using target encoder for stability
        all_latents = agent.encode_target(obs.reshape(-1, *obs.shape[2:]))
        all_latents = all_latents.reshape(num_steps, num_envs, -1)
        all_actions = agent.clamp_action(actions)

        mve_values = torch.zeros((num_steps, num_envs), device=device)
        for t in range(num_steps):
            latent = all_latents[t]  # (num_envs, latent_dim)
            cumulative_reward = torch.zeros(num_envs, device=device)
            discount = torch.ones(num_envs, device=device)
            ...
            for h in range(H):
                ...
                act = all_actions[future_t]
                pred_mean, _ = agent.transition_params(latent, act)
                next_latent = pred_mean

                pred_reward = agent.predict_reward(latent, act, next_latent)

                if future_t + 1 < num_steps:
                    nonterminal = 1.0 - dones[future_t + 1]
                else:
                    nonterminal = 1.0 - next_done_final

                cumulative_reward += discount * pred_reward
                discount *= gamma * nonterminal
                latent = next_latent

            final_value = agent.get_value_from_latent(latent).squeeze(-1)
            mve_values[t] = cumulative_reward + discount * final_value

        # Blend: returns = (1-mve_coef)*standard_returns + mve_coef*mve_returns
        blended_returns = (1 - args.mve_coef) * standard_returns + args.mve_coef * mve_values
        blended_advantages = blended_returns - values

    return blended_returns, blended_advantages
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Compute MVE-blended returns and advantages
        returns, advantages = compute_mve_returns(
            agent, obs, actions, dones, next_obs, next_done,
            values, rewards, args, device)
