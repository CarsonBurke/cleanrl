"""SDE Correlated: Same SDE mechanism as baseline (lstd_linear_tanh) but with
correlated sampling via MultivariateNormal instead of independent Normal.

The baseline computes action_var = sde_latent² @ std² and samples independently
per action dimension. But the sde_latent is shared across actions, creating an
implicit rank-64 covariance structure. By sampling from MultivariateNormal
instead of diagonal Normal, we add cross-action correlation for free using
the EXACT SAME parameters.

Math: cov_factor[b,i,k] = sde_latent[b,k] * std[k,i]
      cov[b,i,j] = sum_k(sde_latent[b,k]² * std[k,i] * std[k,j])
      Marginal variance = sum_k(sde_latent[b,k]² * std[k,i]²)  -- same as baseline
      Cross-action cov = sum_k(sde_latent[b,k]² * std[k,i] * std[k,j])  -- NEW

Hypothesis: Coordinated exploration (correlated noise across actions) helps
locomotion tasks where limbs need to move in coordination.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # SDE mechanism (identical to baseline)
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # SDE latent (same as baseline)
        sde_raw = self.sde_fc(h)
        sde_latent = (self.sde_fc2(self.sde_norm(sde_raw)) / SDE_PRESCALE).tanh()

        # Learned per-element std (same as baseline)
        log_std = (self.log_std_param + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
        std = log_std.exp()  # (hidden_dim, act_dim)

        # Build cov_factor: (batch, act_dim, hidden_dim)
        # cov_factor[b,i,k] = sde_latent[b,k] * std[k,i]
        cov_factor = std.T.unsqueeze(0) * sde_latent.unsqueeze(1)

        # Full covariance: FF^T + eps*I (6x6, cheap to compute)
        cov = cov_factor @ cov_factor.transpose(-2, -1)
        cov = cov + SDE_EPS * torch.eye(act_dim, device=h.device).unsqueeze(0)

        return MultivariateNormal(action_mean, cov)
