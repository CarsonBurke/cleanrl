...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None, use_rpo=False):
        # Actor
        h = self._actor_features(x)
        action_mean = self.actor_out(h) * self.mean_scale

        # RPO: add fixed uniform noise to mean during training
        if use_rpo:
            action_mean = action_mean + torch.empty_like(action_mean).uniform_(-0.2, 0.2)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(
                    b_obs[mb_inds], b_actions[mb_inds], use_rpo=True
                )
