...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # SDE noise (no log_std_param — per-dim learnable offset instead)
        ...
        # Per-dim learnable offset on action_std, replaces log_std_param
    ...
    def _get_action_std(self, h):
        """State-dependent action std via SDE network + per-dim learnable offset."""
        sde_raw = self.sde_fc(h)
        sde_latent = (self.sde_fc2(self.sde_norm(sde_raw)) / SDE_PRESCALE).tanh()
        action_var = (sde_latent.pow(2)) @ (self.sde_out.weight.T.pow(2))  # (batch, act_dim)
        action_std = (action_var + SDE_EPS).sqrt() + self.log_std_offset.exp()
        return action_std
