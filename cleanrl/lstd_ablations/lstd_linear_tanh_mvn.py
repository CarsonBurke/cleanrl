...
class Agent(nn.Module):
    ...
    def _get_action_dist(self, h):
        """State-dependent MVN covariance via linear_tanh SDE pipeline.

        Builds full covariance Sigma = A^T @ A + eps*I where A = diag(sde_latent) @ std.
        This gives cross-action correlation — actions are NOT independent.
        Uses linear_tanh's shared-trunk SDE (no separate cov trunk, no W coupling).
        """
        ...
        # Factor matrix A: (batch, hidden_dim, act_dim)
        A = sde_latent.unsqueeze(-1) * std.unsqueeze(0)

        # Covariance: Sigma = A^T @ A + eps*I, shape (batch, act_dim, act_dim)
