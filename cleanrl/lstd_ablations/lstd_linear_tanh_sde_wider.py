"""SDE Wider: Widen the SDE sub-network from hidden_dim=64 to sde_dim=128.

The baseline's SDE mechanism uses a rank-64 variance structure with 384 noise params.
This variant doubles that to rank-128 with 768 noise params, giving the SDE more
capacity to represent state-dependent variance patterns.

The actor and critic networks remain at hidden_dim=64. Only the SDE sub-network
(sde_fc, sde_norm, sde_fc2, log_std_param) is widened.

Hypothesis: More noise capacity allows finer-grained state-dependent exploration,
potentially improving sample efficiency and final performance.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.sde_fc2 = layer_init(nn.Linear(sde_dim, sde_dim), std=1.0)
