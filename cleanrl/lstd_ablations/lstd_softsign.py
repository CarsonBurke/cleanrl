...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        ...
        # RMSNorm → soft-sign: bounded [-1, 1] but never fully saturates (unlike tanh)
        sde_normed = self.sde_norm(sde_raw)
        sde_latent = sde_normed / (sde_normed.abs() + 1.0)  # (batch, hidden_dim)
