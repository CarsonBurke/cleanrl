"""LSTD Beta policy with detached logit-space Conditional Flow Matching.
...
The auxiliary CFM head learns bounded-action transport in logit action space:
sample z0 from the current Beta policy, map to y0=logit(z0), map rollout
actions to y1=logit((a-low)/(high-low)), and regress v(obs, y_t, t) to y1-y0.
The policy is not driven by this loss; actor features and source samples are
detached so PPO remains the only policy-changing objective.
"""
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def cfm_loss(self, x, actions, advantages, weight_clip):
        ...
        pred_velocity = self.cfm_velocity_head(torch.cat((h.detach(), y_t, t), dim=-1))
