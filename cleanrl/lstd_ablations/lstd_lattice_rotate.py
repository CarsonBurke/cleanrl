...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Learned correlation rotation: can create anti-correlations and decouple per-action magnitude
        self.R = nn.Parameter(torch.eye(act_dim))
    ...
    def _get_action_std(self, h):
        """Analytical diagonal std with R rotation applied."""
        sde_latent, std = self._sde_latent_and_std(h)
        # Diagonal of R @ diag(latent² @ std²) @ R^T
        raw_var = (sde_latent.pow(2)) @ std.pow(2)  # (batch, act_dim)
        # action_var_j = sum_k R_jk² * raw_var_k
        action_var = (raw_var.unsqueeze(-1) * self.R.T.unsqueeze(0).pow(2)).sum(-2)  # (batch, act_dim)
        action_std = (action_var + SDE_EPS).sqrt()
        return action_std

    def _get_noise(self, h):
        """Lattice noise with learned correlation rotation R.
        R can create anti-correlations and decouple per-action magnitude from shared latent."""
        sde_latent, std = self._sde_latent_and_std(h)
        exploration_mat = torch.randn_like(std) * std  # (hidden_dim, act_dim)
        raw_noise = sde_latent @ exploration_mat  # (batch, act_dim)
        return F.linear(raw_noise, self.R)  # (batch, act_dim)
