"""SDE Sigmoid: Replace tanh().pow(2) with sigmoid() in variance computation.

The baseline computes: sde_latent = tanh(x), then action_var = sde_latent² @ std_sq
This creates a gradient dead zone at x=0: d(tanh(x)²)/dx = 2*tanh(x)*(1-tanh²(x)) = 0.

With sigmoid: sde_weights = sigmoid(x), then action_var = sde_weights @ std_sq
- sigmoid output is directly in [0,1], no squaring needed
- Gradient: σ(x)(1-σ(x)), peaks at x=0 (value 0.25), no dead zone at center
- Only dies at extreme saturation (|x| >> 0)
- Same non-negativity guarantee (sigmoid >= 0, std_sq >= 0)

Key advantage: At initialization (x ≈ 0), sigmoid gives weight ≈ 0.5 with maximum
gradient, vs tanh.pow(2) gives weight ≈ 0 with zero gradient. This means all 64
noise dimensions are active from the start.

Minimal change: only activation function in _get_action_std.
"""
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        ...
        sde_activation = self.sde_fc2(self.sde_norm(sde_raw)) / SDE_PRESCALE
        # KEY CHANGE: sigmoid() instead of tanh().pow(2) — no gradient dead zone
        sde_weights = torch.sigmoid(sde_activation)  # (batch, 64) in [0,1]
        log_std = (self.log_std_param + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
        std_sq = log_std.exp().pow(2)
        action_var = sde_weights @ std_sq
