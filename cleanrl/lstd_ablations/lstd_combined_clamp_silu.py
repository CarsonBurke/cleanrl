...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        ...
        # RMSNorm → linear → tanh * 2: wider dynamic range with learned pre-tanh transform
        sde_latent = F.silu(self.sde_fc2(self.sde_norm(sde_raw)))  # (batch, hidden_dim)

        # Learned per-element log-std with clamping
