"""LSTD directional noise: state-dependent noise DIRECTION, not just magnitude

Instead of diagonal N(mean, diag(std²)), use a low-rank multivariate normal:
  action ~ N(mean, B^T diag(amp(h)²) B + diag(floor²))
...
- floor: (act_dim,) small learned diagonal baseline

Why this should improve over diagonal noise:
1. Correlated exploration: the agent can explore in "meaningful directions" in
   action space (e.g., coordinated joint movements) rather than random per-axis jitter
2. State-dependent direction: different states activate different basis directions
   via the mixing head, so the agent explores differently in different states
3. Responsive: single linear head from actor features, sigmoid-gated amplitude,
   shortest gradient path — same responsiveness as the gate variant
4. Noise directly improves the objective because structured exploration finds
   better actions faster than random per-axis perturbation

Log-prob is tractable via Woodbury identity (PyTorch LowRankMultivariateNormal).

Previous results on HalfCheetah @ 8M:
- sdefc3 gate (diagonal, sigmoid): ~3850 (extrapolated)
