...
LOG_STD_INIT = -2.7  # compensate for 2x tanh scale (4x variance → -ln(2) offset)
LOG_STD_MIN = -3.0
LOG_STD_MAX = -0.5
SDE_EPS = 1e-6
...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        ...
        # RMSNorm → scale → tanh → 2x: bounded in [-2, 2], larger dynamic range
        sde_latent = (self.sde_norm(sde_raw) / SDE_PRESCALE).tanh() * 2.0  # (batch, hidden_dim)
