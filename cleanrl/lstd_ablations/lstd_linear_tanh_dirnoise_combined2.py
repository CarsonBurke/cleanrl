"""Combined: fixedfloor + amprange (built on directamp).
- Fixed constant diagonal floor (1e-4) — no learnable floor params
- Learned per-direction amplitude range: amp = softplus(floor) + softplus(scale) * sigmoid(raw)
Both were independently positive (+3% and +6.1% over directamp on HalfCheetah).
"""
import os
import random
import time
...
FLOOR_VAR = 1e-4  # fixed diagonal, not learnable
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Fixed diagonal floor (not learnable)
