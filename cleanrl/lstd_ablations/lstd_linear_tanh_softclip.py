"""LSTD linear_tanh + soft clip ablation

Replaces hard torch.clamp in PPO surrogate with smooth softplus-based clamp.
Keeps the pessimistic max(unclipped, clipped) structure identical to standard PPO,
but the clip boundary is a smooth sigmoid transition instead of a gradient discontinuity.

softclamp(x, lo, hi) = hi - softplus(hi - (lo + softplus(x - lo, beta=K) / K), beta=K) / K

K controls sharpness: K→∞ recovers hard clamp, K=20 gives smooth transition.
...
SOFTCLIP_K = 20.0   # sharpness of soft clamp (higher = closer to hard clamp)


def softclamp(x, lo, hi):
    """Smooth differentiable clamp using softplus."""
    # smooth max(x, lo)
    x = lo + F.softplus(x - lo, beta=SOFTCLIP_K) / SOFTCLIP_K
    # smooth min(x, hi)
    x = hi - F.softplus(hi - x, beta=SOFTCLIP_K) / SOFTCLIP_K
    return x
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Policy loss — soft clip (smooth clamp, same pessimistic max)
                pg_loss1 = -mb_advantages * ratio
                pg_loss2 = -mb_advantages * softclamp(ratio, 1 - args.clip_coef_low, 1 + args.clip_coef_high)
