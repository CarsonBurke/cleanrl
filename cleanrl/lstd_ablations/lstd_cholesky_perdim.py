...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # SDE noise — state-dependent Cholesky factor
        ...
        # Predict Cholesky entries: act_dim*(act_dim+1)/2 free parameters
        self.act_dim = act_dim
        n_chol = act_dim * (act_dim + 1) // 2  # 21 for act_dim=6
        self.chol_proj = layer_init(nn.Linear(hidden_dim, n_chol), std=0.01)
        # Precompute lower-triangular indices
        self.register_buffer('tril_indices', torch.tril_indices(act_dim, act_dim))
        # Diagonal bias — init to LOG_STD_INIT so initial noise magnitude matches baseline
        self.register_buffer('diag_indices', torch.arange(act_dim))
    ...
    def _get_cholesky(self, h):
        """State-dependent Cholesky factor L from actor features.
        Returns L: (batch, act_dim, act_dim) lower-triangular."""
        sde_raw = self.sde_fc(h)
        sde_latent = (self.sde_fc2(self.sde_norm(sde_raw)) / SDE_PRESCALE).tanh()
        chol_entries = self.chol_proj(sde_latent)  # (batch, n_chol)

        batch = h.shape[0]
        L = torch.zeros(batch, self.act_dim, self.act_dim, device=h.device)
        L[:, self.tril_indices[0], self.tril_indices[1]] = chol_entries
        # Positive diagonal via exp, clamped like baseline log_std
        # Cholesky diagonal init: target ~0.6 to match baseline action_std
        # (baseline sums 64 terms; Cholesky has only ~6 per row, so need larger per-entry values)
        CHOL_DIAG_INIT = -0.5
        diag_raw = L[:, self.diag_indices, self.diag_indices] + CHOL_DIAG_INIT
        L[:, self.diag_indices, self.diag_indices] = diag_raw.clamp(LOG_STD_MIN, LOG_STD_MAX).exp()
        return L  # (batch, act_dim, act_dim)

    def _get_action_std(self, h):
        """Diagonal std from Cholesky: diag(L @ L^T) = sum of squares per row of L."""
        L = self._get_cholesky(h)
        action_var = (L ** 2).sum(dim=-1)  # (batch, act_dim)
        action_std = (action_var + SDE_EPS).sqrt()
        return action_std

    def _get_noise(self, h):
        """Sample noise via Cholesky: noise = L @ eps, eps ~ N(0, I).
        Full covariance, anti-correlations, state-dependent."""
        L = self._get_cholesky(h)
        eps = torch.randn(h.shape[0], self.act_dim, 1, device=h.device)
        return (L @ eps).squeeze(-1)  # (batch, act_dim)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            # Rollout: use lattice-style noise (exploration_mat must be sampled beforehand)
            noise = self._get_noise(h)
            action = action_mean + noise
            # Compute diagonal std for log_prob
            action_std = self._get_action_std(h)
            probs = Normal(action_mean, action_std)
            return action, probs.log_prob(action), probs.entropy().sum(1), self.get_value(x)
        else:
            # Training: use diagonal Normal for log_prob (analytically correct for diagonal)
            action_std = self._get_action_std(h)
            probs = Normal(action_mean, action_std)
            return action, probs.log_prob(action), probs.entropy().sum(1), self.get_value(x)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Cholesky-specific metrics
        with torch.no_grad():
            h = agent._actor_features(b_obs[:16])
            L = agent._get_cholesky(h)
            writer.add_scalar("tbot/chol_diag_mean", L[:, agent.diag_indices, agent.diag_indices].mean().item(), global_step)
            writer.add_scalar("tbot/chol_offdiag_mean", (L.sum() - L[:, agent.diag_indices, agent.diag_indices].sum()).item() / (16 * 15), global_step)
