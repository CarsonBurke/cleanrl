"""Diagonal-primary LSTD with Lipman OT-CFM mean residual.

This combines the best old covariance backbone (large state-dependent diagonal
as primary noise, small low-rank correlation as complement) with the winning
CFM mean-residual mechanism from lstd_pa_lipman_cfm_mean_v1.

The policy remains an exact LowRankMultivariateNormal for PPO. A time-conditioned
CFM head is trained on advantage-weighted OT paths from standard Gaussian source
actions to rollout actions, and its t=0 velocity is added as a learned residual
to the policy mean.
"""
import os
import random
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Small low-rank amplitude range (complement, not primary)
        # Init: floor ≈ 0.02, scale ≈ 0.15, so amp range ~[0.02, 0.17]
        self.amp_floor = nn.Parameter(torch.full((self.noise_rank,), float(np.log(np.exp(0.02) - 1))))
        self.amp_scale = nn.Parameter(torch.full((self.noise_rank,), float(np.log(np.exp(0.15) - 1))))

        # Large state-dependent diagonal (primary noise source)
        # Init: floor ≈ 0.1, scale ≈ 0.3, so cov_diag range ~[0.1, 0.4]
        self.diag_head = layer_init(nn.Linear(hidden_dim, act_dim), std=0.01)
        nn.init.constant_(self.diag_head.bias, 0.0)
        self.diag_floor = nn.Parameter(torch.full((act_dim,), float(np.log(np.exp(0.1) - 1))))
        self.diag_scale = nn.Parameter(torch.full((act_dim,), float(np.log(np.exp(0.3) - 1))))
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # State-dependent diagonal: bounded, amprange pattern
        cov_diag = F.softplus(self.diag_floor) + F.softplus(self.diag_scale) * torch.sigmoid(self.diag_head(h))
    ...
    def cfm_residual_norm(self, h, action_mean):
        if self.cfm_mean_coef == 0.0:
            return action_mean.new_tensor(0.0)
        t0 = torch.zeros((h.shape[0], 1), device=h.device, dtype=h.dtype)
        residual = self.cfm_mean_coef * self._cfm_velocity(h, action_mean, t0)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if self.cfm_mean_coef != 0.0:
            t0 = torch.zeros((h.shape[0], 1), device=h.device, dtype=h.dtype)
            action_mean = action_mean + self.cfm_mean_coef * self._cfm_velocity(h, action_mean, t0)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            residual_norm = agent.cfm_residual_norm(h_diag, base_mean)
