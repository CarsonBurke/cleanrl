"""Ablation: softplus diagonal floor instead of exp().square() (built on directamp).
Change: Replace log_std_floor with a raw param + softplus for the diagonal std.
The original exp(x)^2 = exp(2x) has vanishing gradients at init (-3.0) — gradient is
2*exp(2*-3) = 0.005. Softplus gives near-linear gradient flow for positive values.
Init softplus param to produce std ~0.2.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # ABLATION: raw param for softplus floor std, init so softplus(x) ≈ 0.2
        # softplus(x) = log(1+exp(x)), inverse: x = log(exp(0.2)-1) ≈ -1.4
        self.floor_std_raw = nn.Parameter(torch.full((act_dim,), -1.4))
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # ABLATION: softplus for floor std, then square for variance
        floor_std = F.softplus(self.floor_std_raw)
        cov_diag = floor_std.square().expand(batch, act_dim)
