"""Implicit amplitude: remove the separate amplitude head entirely. Let the
perturbation control both direction AND magnitude. The perturb_head outputs 36
values per state (vs 6 for dir_head) — far more capacity for state-dependent
amplitude. Scale perturbation to 0.5 so basis norms can meaningfully vary.
cov_factor = effective_basis.T directly, no per-direction amplitude scaling.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Perturbation head only — no separate amplitude head
        # Larger scale (0.5) so perturbation can meaningfully change basis norms
    ...
    def _get_distribution(self, h, action_mean):
        ...
        # Perturbation controls both direction and magnitude
        ...
        cov_factor = effective_basis.transpose(-2, -1)  # (batch, act_dim, noise_rank)
