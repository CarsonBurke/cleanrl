# PMPO-D4 + ReluSq + Beta — SPO v1 (Simple Policy Optimization).
#
# Sibling to clipdapo_v1 (PPO clip-higher). Same Beta + ReluSq + adv_norm
# backbone; trust region swapped from PPO ratio clipping to the SPO
# quadratic-penalty surrogate from arXiv 2401.16025v9:
#
#   J(θ) = E[ r·A − (|A|/(2ε)) · (r − 1)² ]
#   ⇒ pg_loss = -E[ r·A − (|A|/(2ε)) · (r − 1)² ]
#
# Key property vs PPO clip. PPO clipping zeros the gradient when the ratio
# leaves [1−ε, 1+ε], discarding signal from "out-of-band" samples. SPO
# applies a smooth quadratic pull-back instead — every sample always
# contributes a non-zero gradient, smoothly directed back toward the
# constraint boundary. Fixed points coincide with PPO clip:
#   ∂/∂r: A − sign(A)·|A|·(r−1)/ε = 0  ⇒  r* = 1 + sign(A)·ε
# i.e., A > 0 prefers r = 1+ε, A < 0 prefers r = 1−ε, identical to clip.
#
# Hyperparameter: single ε = 0.2 (paper default). Clip-higher's asymmetric
# 0.20/0.28 has no SPO equivalent; the paper does not propose asymmetry.
#
# What's swapped vs clipdapo_v1:
#   - clip_coef_low / clip_coef_high   →  spo_eps (single ε)
#   - max(pg_loss1, pg_loss2) clipped surrogate  →  SPO quadratic objective
#   - losses/clipfrac diagnostic       →  diag/spo_penalty (penalty mean)
#   - value-loss clip restored to symmetric ±clip_coef (PPO-standard;
#     value drift isn't a likelihood ratio so asymmetry has no motivation
#     here even in clipdapo, but we kept it there to match the lstd
#     reference; SPO is fresh so we go canonical)
#
# Hypothesis. SPO's smooth gradient on out-of-band samples should extract
# more signal per minibatch than PPO clip's gradient-zero region,
# especially in low-clipfrac regimes (clipdapo settled at 6-8% clipfrac;
# those zeroed-gradient samples now contribute under SPO). Expect parity
# or modest gain over clipdapo on HC.
...
class Args:
    ...
    spo_eps: float = 0.2
    """SPO quadratic-penalty bound ε; objective J = E[r·A − |A|/(2ε)·(r−1)²]"""
    ent_coef: float = 0.0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # SPO objective: J = r·A − (|A|/(2ε)) · (r − 1)²
                # Loss = -J. Penalty is a smooth quadratic pull-back toward
                # r = 1 ± ε (sign chosen by sign(A)); unlike PPO clip, no
                # gradient is zeroed for out-of-band samples.
                pg_surrogate = mb_advantages * ratio
                spo_penalty = mb_advantages.abs() * (ratio - 1.0).pow(2) / (2.0 * args.spo_eps)
