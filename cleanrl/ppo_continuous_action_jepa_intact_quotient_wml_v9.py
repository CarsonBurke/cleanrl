"""INTACT action-quotient weighted maximum likelihood (AWR-style comparator).

A deterministic prescriber selects a latent displacement, interpreted by the
learned conditional Beta action law. Real GAE exponentially tilts empirical
transition weights under a categorical KL constraint; actor fitting maximizes
weighted likelihood of executed actions through the frozen inverse law. This
learns only distinctions visible in action space, without a latent endpoint
density objective or imagined dynamics/value derivatives. It is an existing
weighted-likelihood optimization principle, not a new optimizer.

World learning retains attached JEPA/local inverse learning and unweighted
hindsight goal-law fitting. The real-return critic and world parameters have
separate owners; actor derivatives pass through the law's inputs only. Uniform
weights isolate reward selection. Neither the deterministic prescribed goal
nor the hindsight inverse law has a reachability guarantee; shared law and
encoder drift also prevent a monotonic policy improvement claim.
"""
import os
import random
import copy
...
def main():
    ...
    try:
        ...
        writer.add_text("method", "AWR-style action quotient likelihood through frozen INTACT law; no imagined control objective or endpoint density fit.")
        ...
        def loss_model(observations, native, weights, returns, old_values,
            ...
            return training_loss(agent, observations, native, weights, returns, old_values,
                                 following, goals, rewards, terminations, args)
                ...
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss, metrics = loss_model(
                            ...
                            b_goals[indices], b_rewards[indices], b_terminations[indices],
                        )
