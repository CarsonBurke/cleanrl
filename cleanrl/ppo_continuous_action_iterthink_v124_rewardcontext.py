...
#   calibration without giving the model an autoregressive target leak or exposing
#   outcome readout tokens to the PPO agent.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Direct latent control state for actor/critic: obs tokens only. Reward
        # and continuation semantic tokens remain inside the WM and outcome heads
        # but are not exposed to the PPO agent in this ablation.
    ...
    def agent_input_from_latent(self, latent):
        ...
        if latent.shape[-2] == self.world_model.num_latent_tokens:
            latent = latent[..., self.world_model.obs_token_start :, :]
        elif latent.shape[-2] != self.world_model.num_obs_tokens:
            raise ValueError(
                f"agent latent must have {self.world_model.num_latent_tokens} full tokens "
                f"or {self.world_model.num_obs_tokens} obs tokens, got {latent.shape[-2]}"
            )
