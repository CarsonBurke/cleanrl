# AdaBelief PPO, v1. Base: ppo_normres_indclip_v4_norm_mse_valueclip_50M.
# https://arxiv.org/html/2010.07468v5, Algorithm 2 (no rectification/weight decay).
# Replace joint Adam with EMA squared gradient-prediction residuals for both networks.
# Hypothesis: persistent gradients benefit from larger steps; nonstationary PPO
# gradients may instead make the unchanged 0.0096 LR too aggressive.
# Preserve architecture, clipping, normalization, batches and 50M annealing schedule.
# Paper epsilon=1e-8 is added both inside the variance recurrence and outside sqrt;
# reusing Adam's 1e-5 here would introduce a much stronger variance floor.
import os
import random
import time
...
class Args:
    ...
    """32x the 3e-4 PPO default; one AdaBelief covers actor and critic"""
    adabelief_eps: float = 1e-8
    """Algorithm 2 epsilon, added to the variance each step and to the denominator"""
    num_envs: int = 16
...
def adabelief_update(params, grads, moments, variances, steps, lr, beta1, beta2, eps):
    """Algorithm 2: residual uses the updated, uncorrected first moment."""
    for parameter, gradient, moment, variance, step in zip(params, grads, moments, variances, steps):
        step.add_(1)
        moment.lerp_(gradient, 1.0 - beta1)
        residual = gradient - moment
        # The paper adds epsilon to the accumulator EVERY step, not just the denominator.
        variance.mul_(beta2).addcmul_(residual, residual, value=1.0 - beta2).add_(eps)
        corrected_moment = moment / (1.0 - beta1 ** step)
        denominator = (variance / (1.0 - beta2 ** step)).sqrt().add_(eps)
        parameter.add_(-lr * corrected_moment / denominator)


class AdaBelief(optim.Optimizer):
    """Unrectified, no-weight-decay AdaBelief; compiled updates and device-side clocks."""

    def __init__(self, params, lr=9.6e-3, betas=(0.9, 0.999), eps=1e-8, compile=True):
        params = list(params)
        if not params or not np.isfinite(lr) or lr < 0 or not np.isfinite(eps) or eps <= 0:
            raise ValueError("AdaBelief requires parameters, nonnegative finite LR and positive finite epsilon")
        if not all(0 <= beta < 1 for beta in betas):
            raise ValueError("AdaBelief betas must be in [0, 1)")
        device = params[0].device
        if device.type != "cuda" or any(parameter.device != device for parameter in params):
            raise ValueError("AdaBelief requires parameters on one CUDA device")
        super().__init__(params, dict(lr=torch.tensor(lr, device=device), betas=betas, eps=eps))
        self._update = (
            torch.compile(adabelief_update, fullgraph=True, dynamic=False, options={"triton.cudagraphs": False})
            if compile else adabelief_update
        )
    ...
    def step(self, closure: Callable[[], float] | None = None) -> float | None:
        ...
        if closure is not None:
            with torch.enable_grad():
                loss = closure()
        for group in self.param_groups:
            params, grads, moments, variances, steps = [], [], [], [], []
            for parameter in group["params"]:
                if parameter.grad is None:
                    continue
                if parameter.grad.is_sparse:
                    raise RuntimeError("AdaBelief requires dense gradients")
                ...
                variances.append(state["exp_avg_var"])
                steps.append(state["step"])
            if params:
                beta1, beta2 = group["betas"]
                self._update(params, grads, moments, variances, steps, group["lr"], beta1, beta2, group["eps"])
        return loss
...
def main():
    ...
    try:
        ...
        optimizer = AdaBelief(agent.parameters(), lr=args.learning_rate, eps=args.adabelief_eps, compile=args.compile)
