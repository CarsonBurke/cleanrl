# PPO + IterThink v140 raw-directtokens-k6 (v137 with direct raw scalar tokens).
#
# Key change from v137:
#   - Remove Gym's stateful NormalizeObservation/clip wrappers.
#   - Kill observation symlog and the scalar MLP precursor. Each raw scalar is
#     projected directly to token dim with a shared Linear(1, D), then gets a
#     learned feature slot embedding. No input embed norm; encoder pre-norm and
#     encoder_norm remain responsible for scale control.
...
#
# Hypothesis:
#   v137/v139 underperformed because symlog and early embedding normalization
#   compressed raw scalar magnitude before the transformer could use it. Direct
#   raw scalar tokens test the simplest scalable tokenizer.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # Direct raw-observation tokenizer. A shared scalar projection provides
        # value-dependent token magnitude/direction; learned slot embeddings
        # provide feature identity. There is intentionally no input embedding
        # norm here.
