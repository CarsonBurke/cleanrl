# PPO + IterThink v141 raw-multiscale-embed-k6 (v137 with GPT-style scalar embeddings).
#
# Key change from v137:
#   - Remove Gym's stateful NormalizeObservation/clip wrappers.
#   - Kill observation symlog. Encode each raw scalar with a fixed multiscale
#     basis [clip(x/s), tanh(x/s)] for s in {1, 5, 25}, project that basis
#     directly to token dim, add learned feature slot embedding, then apply
#     embedding RMSNorm before the encoder stack.
...
#
# Hypothesis:
#   GPT-style input handling first creates a learned embedding vector, then
#   normalizes the residual stream. For scalar observations, magnitude must be
#   encoded as direction before normalization. A fixed multiscale basis gives the
#   token projection bounded but magnitude-sensitive channels without env obs norm
#   or symlog compression.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # GPT-style raw-observation tokenizer. A fixed multiscale scalar basis is
        # projected directly to token dim; learned slot embeddings provide feature
        # identity; embedding RMSNorm is applied after value and slot embeddings
        # are combined.
        ...
        self.obs_scalar_in = xavier_linear(nn.Linear(6, self.dim))
    ...
    def encode_summary(self, obs):
        ...
        scales = self.obs_basis_scales.to(device=obs_flat.device, dtype=obs_flat.dtype)
        scaled = obs_flat.unsqueeze(-1) / scales.view(1, 1, -1)
        obs_basis = torch.cat([scaled.clamp(-10.0, 10.0), torch.tanh(scaled)], dim=-1)
        obs_tokens = self.obs_scalar_in(obs_basis)
