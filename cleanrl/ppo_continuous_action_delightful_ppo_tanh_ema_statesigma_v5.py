# Delightful PPO with GAE-calibrated standard-rollout actor updates v5.
#
...
# (1-gamma*lambda), turning dense GAE accumulation into an order-one weighted
# average analogous to bounded episodic outcomes. Advantages and delight are
# not normalized or whitened.
...
#
# Hypothesis: order-one GAE scale will keep delight responsive rather than
# saturated, while sigma=1 prevents negative density-surprisal from reversing
# DG without sacrificing PPO's established rollout horizon and diversity.
...
class Args:
    ...
    scale_gae_kernel: bool = True
    """multiply unit-RMS dense rewards by (1-gamma*lambda) for order-one GAE"""
...
class EMARewardNormalizer:
    """EMA-normalize immediate rewards and calibrate their dense-GAE scale."""
...
def gae_kernel_scale(gamma, gae_lambda):
    """Normalize the total mass of the exponentially weighted GAE kernel."""
    scale = 1.0 - gamma * gae_lambda
    if not 0.0 < scale <= 1.0:
        raise ValueError("gamma * gae_lambda must be in [0, 1)")
    return scale
...
if __name__ == "__main__":
    ...
    reward_scale = (
        gae_kernel_scale(args.gamma, args.gae_lambda)
        if args.scale_gae_kernel
        else 1.0
    )
