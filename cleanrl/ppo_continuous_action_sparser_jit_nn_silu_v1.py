# Sparser-JIT NN SiLU PPO: fixed sparse MLPs with configurable capacity.
# The actor and critic use fixed 50%-density connections, SiLU hidden
# activations, and torch.compile learner graphs. Width and hidden-layer count
# are CLI controls so depth and capacity can be tested without changing PPO.
# Hypothesis: SiLU improves gradient flow through sparse pathways; extra width
# or depth may recover the coordination lost by fixed random connectivity.
import os
import random
import time
...
class Args:
    ...
    hidden_width: int = 64
    """hidden width for sparse actor and critic trunks"""
    hidden_layers: int = 2
    """number of sparse hidden layers before each output head"""
    sparse_density: float = 0.5
    """fraction of hidden-layer connections retained by fixed masks"""
    num_envs: int = 32
...
class SparseLinear(nn.Linear):
    """A fixed-mask Linear whose inactive edges stay zero under Adam."""

    def __init__(self, in_features, out_features, density):
        super().__init__(in_features, out_features)
        if not 0.0 < density <= 1.0:
            raise ValueError("sparse_density must be in (0, 1]")
        keep = max(1, int(round(in_features * density)))
        scores = torch.rand(out_features, in_features)
        ...
        self.weight.register_hook(lambda gradient: gradient * self.sparsity_mask)
        self.apply_sparsity()

    @torch.no_grad()
    def apply_sparsity(self):
        self.weight.mul_(self.sparsity_mask)
...
def sparse_mlp(observation_dim, output_dim, width, layers, density, output_std):
    modules = []
    input_dim = observation_dim
    for _ in range(layers):
        modules.extend((layer_init(SparseLinear(input_dim, width, density)), nn.SiLU()))
        input_dim = width
    modules.append(layer_init(nn.Linear(width, output_dim), std=output_std))
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        ...
        width = 48 if args is None else args.hidden_width
        layers = 2 if args is None else args.hidden_layers
        density = 0.5 if args is None else args.sparse_density
        ...
        self.critic = sparse_mlp(
            observation_dim, 1, width, layers, density, 1.0
        )
        self.actor = sparse_mlp(
            observation_dim, 2 * self.action_dim, width, layers, density, 0.01
        )
...
def validate_args(args):
    if min(
        args.num_envs, args.num_steps, args.num_minibatches, args.update_epochs,
        args.hidden_width, args.hidden_layers,
    ) <= 0:
        raise ValueError("environment, rollout, minibatch, epoch, width and depth counts must be positive")
    if not 0.0 < args.sparse_density <= 1.0:
        raise ValueError("sparse_density must be in (0, 1]")
...
def main():
    ...
    try:
        ...
        writer.add_text(
            "policy",
            f"Beta PPO; SiLU; sparse width-{args.hidden_width}, depth-{args.hidden_layers}, "
            f"density-{args.sparse_density:.2f}; masked gradients; FP32; "
