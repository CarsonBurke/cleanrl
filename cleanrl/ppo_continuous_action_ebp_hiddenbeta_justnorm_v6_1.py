# PPO + Energy-Based Prediction Hidden-Beta JustNorm v6.1.
#
# Predictive-coding variant of base continuous PPO. Hidden activations are explicit
# signed state variables, not an ordinary end-to-end MLP path. Each local hidden
# predictor outputs a Beta distribution over z=(h+1)/2. v6.1 replaces hard
# per-coordinate clipping with nGPT-style JustNorm projection over the hidden
# vector, so h lies on or inside the unit L2 ball and every coordinate remains in
# [-1, 1] for the signed Beta transform.
#
...
class PCHierarchy(nn.Module):
    ...
    def initial_states(self, x, args):
        ...
        h2_dist = self.edge1(h1_mean, args.pc_hidden_beta_min_concentration)
    ...
    def local_energies(self, x, states, args):
        h1, h2 = states
        h1 = justnorm(h1, args.pc_justnorm_eps)
        h2 = justnorm(h2, args.pc_justnorm_eps)
        z1 = (h1 + 1.0) * 0.5
        z2 = (h2 + 1.0) * 0.5
...
class Agent(nn.Module):
    ...
    def settle_states(self, hierarchy, x, initial_states, args, terminal_energy_fn, steps=None):
        ...
        states = [justnorm(state.detach(), args.pc_justnorm_eps).requires_grad_(True) for state in initial_states]
        first_energy = None
        last_energy = None
