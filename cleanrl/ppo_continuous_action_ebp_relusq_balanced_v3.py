# PPO + Energy-Based Prediction ReLUSq balanced v3.
#
# v3 keeps v2's structural fixes while rebalancing local predictive energy after
# switching from feature-mean to feature-summed NLL. With hidden_size=64, v3 uses
# 1/64 as the default local coefficient, so the summed local energy has the same
# default scale as v1's feature-mean energy while preserving per-sample NLL form.
#
# v2 fixed three v1 issues while preserving the no end-to-end-hidden-backprop
# constraint:
# - local predictors use ReLU-squared hidden features with linear, unbounded mean
#   outputs, so settled states are not outside the amortizer's support;
# - local Gaussian NLL is a feature-summed sample energy, not a full tensor mean;
# - activation settling uses unclipped terminal energies: actor policy-gradient
#   energy plus old/new Beta KL, critic Huber-to-return plus old-value anchor.
#
...
class Args:
    ...
    """absolute clamp for inferred hidden states; <=0 disables clipping"""
    pc_local_energy_coef: float = 0.015625
    ...
    pc_actor_inference_kl_coef: float = 0.03
    """old/new Beta KL coefficient for actor activation settling"""
    pc_critic_inference_huber_delta: float = 10.0
    """Huber beta for critic activation settling against returns"""
    pc_critic_inference_old_value_coef: float = 0.1
    """old-value anchor coefficient for critic activation settling"""
...
class LocalPredictor(nn.Module):
    def __init__(self, in_dim, out_dim, fixed_logvar=0.0, learn_logvar=False):
        super().__init__()
        self.fc = layer_init(nn.Linear(in_dim, out_dim))
        self.act = ReluSq()
        self.mean = layer_init(nn.Linear(out_dim, out_dim), std=0.1)
        self.learn_logvar = learn_logvar
        if learn_logvar:
            self.logvar = layer_init(nn.Linear(out_dim, out_dim), std=0.01, bias_const=fixed_logvar)
        else:
    ...
    def forward(self, x, logvar_min, logvar_max):
        ...
        if self.learn_logvar:
            logvar = self.logvar(feat).clamp(logvar_min, logvar_max)
        else:
...
class PCHierarchy(nn.Module):
    def __init__(self, input_dim, hidden_size, args):
        super().__init__()
        self.edge0 = LocalPredictor(input_dim, hidden_size, args.pc_fixed_logvar, args.pc_learn_logvar)
        self.edge1 = LocalPredictor(hidden_size, hidden_size, args.pc_fixed_logvar, args.pc_learn_logvar)
    ...
    def local_energies(self, x, states, args):
        h1, h2 = states
        mean1, logvar1 = self.edge0(x, args.pc_logvar_min, args.pc_logvar_max)
        mean2, logvar2 = self.edge1(h1, args.pc_logvar_min, args.pc_logvar_max)
        e1 = 0.5 * (((h1 - mean1) ** 2) * torch.exp(-logvar1) + logvar1)
        e2 = 0.5 * (((h2 - mean2) ** 2) * torch.exp(-logvar2) + logvar2)
        return e1.sum(dim=1).mean(), e2.sum(dim=1).mean()
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, args, action_z=None):
        ...
        return self.action_from_z(action_z), action_z, log_prob, entropy, self.value_from_state(critic_states[-1]), alpha, beta
    ...
    def actor_inference_terminal_energy(self, h, action_z, old_alpha, old_beta, advantages, args):
        dist = self.actor_dist_from_state(h)
        action_z = action_z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        newlogprob = dist.log_prob(action_z.detach()).sum(1)
        old_dist = Beta(old_alpha.detach(), old_beta.detach())
        kl = kl_divergence(old_dist, dist).sum(1).mean()
        return (-(advantages.detach()) * newlogprob).mean() + args.pc_actor_inference_kl_coef * kl

    def critic_inference_terminal_energy(self, h, returns, old_values, args):
        newvalue = self.value_from_state(h).view(-1)
        ret_loss = F.smooth_l1_loss(
            newvalue,
            returns.detach(),
            beta=args.pc_critic_inference_huber_delta,
        )
        old_anchor = 0.5 * ((newvalue - old_values.detach()) ** 2).mean()
        return ret_loss + args.pc_critic_inference_old_value_coef * old_anchor
    ...
    def settle_states(self, hierarchy, x, initial_states, args, terminal_energy_fn, steps=None):
        ...
        for _ in range(steps):
            ...
            for state, grad in zip(states, grads):
                ...
                if args.pc_state_clip > 0:
                    next_state = next_state.clamp(-args.pc_state_clip, args.pc_state_clip)
    ...
    def infer_actor_update_states(self, x, action_z, old_alpha, old_beta, advantages, args):
        with torch.no_grad():
            initial_states = self.actor_pc.initial_states(x, args)

        def terminal(h):
            return args.pc_actor_terminal_coef * self.actor_inference_terminal_energy(
                h,
                action_z,
                old_alpha,
                old_beta,
    ...
    def infer_critic_update_states(self, x, returns, old_values, args):
        ...
        def terminal(h):
            return args.pc_critic_terminal_coef * self.critic_inference_terminal_energy(h, returns, old_values, args)
...
if __name__ == "__main__":
    ...
    actor_alphas = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device)
    actor_betas = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, action_z, logprob, _, value, alpha, beta = agent.get_action_and_value(next_obs, args)
                values[step] = value.flatten()
            actions[step] = action
            action_zs[step] = action_z
            actor_alphas[step] = alpha
            actor_betas[step] = beta
        ...
        b_actor_alphas = actor_alphas.reshape((-1,) + envs.single_action_space.shape)
        b_actor_betas = actor_betas.reshape((-1,) + envs.single_action_space.shape)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                actor_states, actor_energy = agent.infer_actor_update_states(
                    b_obs[mb_inds],
                    b_action_zs[mb_inds],
                    b_actor_alphas[mb_inds],
                    b_actor_betas[mb_inds],
                    mb_advantages,
                    args,
                )
