# nGPT spherical PPO, v20: project weights after Adam, not just activations.
# Unit hidden states use nGPT's learned positive, uncapped interpolation steps;
# 4x-width SwiGLU and readout gains restore explicit scale degrees of freedom.
# Continuous observations use a homogeneous coordinate instead of token embeddings.
# Keep the Peri comparison's PPO/normalization recipe and full-GAE scalar critic.
import os
import random
import time
...
from cleanrl.shared.ngpt import NGPTHead, NGPTTrunk, project_ngpt_weights
...
class Agent(nn.Module):
    ...
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(NGPTTrunk(observation_dim, 64), NGPTHead(64, 1))
        self.actor = nn.Sequential(NGPTTrunk(observation_dim, 64), NGPTHead(64, 2 * self.action_dim))
...
def critic_step(agent, optimizer, observations, targets, old_values, args, loss_model=None):
    ...
    optimizer.step()
    project_ngpt_weights(agent.critic)
    return grad_norm
...
@torch.no_grad()
def geometry_metrics(agent):
    """Post-update constraints and learned scales, in the same units as nGPT."""
    metrics = {}
    for name, network in (("actor", agent.actor), ("critic", agent.critic)):
        trunk, head = network
        errors = [(trunk.in_proj.weight.norm(dim=1) - 1.0).abs().max(),
                  (head.weight.norm(dim=1) - 1.0).abs().max()]
        for block in trunk.blocks:
            errors.extend((weight.norm(dim=axis) - 1.0).abs().max() for weight, axis in (
                (block.gate.weight, 1), (block.up.weight, 1), (block.down.weight, 0)
            ))
        alpha = torch.stack([block.alpha.abs() * (0.05 * trunk.width ** 0.5)
                             for block in trunk.blocks])
        scale = head.scale * trunk.width ** 0.5
        metrics.update({
            f"geometry/{name}_weight_norm_error": torch.stack(errors).max(),
            f"geometry/{name}_alpha_mean": alpha.mean(),
            f"geometry/{name}_alpha_max": alpha.max(),
            f"geometry/{name}_head_scale_min": scale.min(),
            f"geometry/{name}_head_scale_max": scale.max(),
        })
    return metrics
...
def main():
    ...
    try:
        ...
        writer.add_text(
            "policy",
            f"Beta fused host actor; nGPT projected weights, spherical residuals, SwiGLU; LR={args.learning_rate}; "
            f"epochs={args.update_epochs}; grad_clip={args.grad_clip}; "
            f"clip_heads={args.clip_heads}; full policy GAE; "
            f"critic_trace_steps={args.critic_trace_steps}; critic_minibatches={args.critic_minibatches}",
        )
        envs = make_training_env(args, run_name)
        resources.callback(envs.close)
        agent = Agent(envs).to(device)
        writer.add_text("architecture", "nGPT MLP: width=64, blocks=3, SwiGLU hidden=256; "
                        "homogeneous observation embedding; unit weight rows/down columns; "
                        "learned residual/MLP/head scales; no attention or SiTU caps.")
        writer.add_scalar("architecture/parameters", sum(p.numel() for p in agent.parameters()), 0)
        ...
        def rollout_statistics(observations, native, next_observations):
            ...
            return value.flatten(), agent.action_logprob(alpha, beta, native), next_value.flatten()
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_values, b_logprobs, b_next_values = rollout_statistics(b_obs, b_native, b_next_obs)
