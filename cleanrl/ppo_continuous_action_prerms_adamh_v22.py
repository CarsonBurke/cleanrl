# AdamH / Hyperball on the best completed v6 scalar-PPO recipe, not Peri or nGPT.
# Preserve the architecture, initialization, scalar-clipped MSE and ten-epoch loop.
# Hidden SiTU-GLU gate/up/down matrices keep their INITIAL whole-matrix Frobenius
# radii; embeddings, readouts, residual gates and biases retain ordinary Adam.
# Fixed-gain RMS and SiTU do not make every matrix exactly scale-invariant:
# this tests the Hyperball optimizer wrapper, not a scale-invariance guarantee.
import os
import random
import time
...
class Args:
    ...
    optimizer_kind: Literal["adam", "adamh"] = "adamh"
    """original one-Adam control, or AdamH hidden matrices plus auxiliary Adam"""
    hyperball_learning_rate: float = 0.018
    """relative whole-matrix Hyperball step size before linear annealing"""
    hyperball_beta2: float = 0.999
    """match baseline Adam by default; 0.95 optionally matches the donor recipe"""
    num_envs: int = 16
    ...
    for network in (agent.actor, agent.critic):
        for branch in network[0].blocks:
            if not isinstance(branch, SiTUGLUBranch):
                raise ValueError("AdamH requires SiTU-GLU branches (activation='stiglu')")
...
def make_optimizers(agent, args):
    ...
    if args.optimizer_kind != "adamh":
        raise ValueError("optimizer_kind must be 'adam' or 'adamh'")
    ...
    hyperball_optimizer = AdamH(
        ...
        betas=(0.9, args.hyperball_beta2),
        eps=1e-5,
...
def validate_args(args):
    ...
    if args.optimizer_kind == "adamh" and args.activation != "stiglu":
        raise ValueError("AdamH requires activation='stiglu'")
