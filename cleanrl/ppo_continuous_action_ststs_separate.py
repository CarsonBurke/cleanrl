# PPO with separate axial space-time transformer backbones for actor and critic.
#
...
#
# Ablation:
# - separate STSTS backbones for actor and critic
# - flatten each final token grid
# - separate linear projections into actor and critic heads
#
# PPO base does not include an SDE head, so this variant compares actor/critic
# representation decoupling only.
import os
import random
import time
...
class STSTSBackbone(nn.Module):
    ...
    def forward(self, obs_seq):
        tokens = self.value_proj(obs_seq.unsqueeze(-1))
        tokens = tokens + self.dim_id_embed(self.dim_indices).view(1, 1, self.obs_dim, EMBED_DIM)
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        self.context_len = CONTEXT_LEN
        flat_dim = self.context_len * obs_dim * EMBED_DIM

        self.actor_backbone = STSTSBackbone(obs_dim, self.context_len)
        self.critic_backbone = STSTSBackbone(obs_dim, self.context_len)
        self.actor_proj = layer_init(nn.Linear(flat_dim, EMBED_DIM))
        self.critic_proj = layer_init(nn.Linear(flat_dim, EMBED_DIM))
    ...
    def _actor_latent(self, obs_seq):
        tokens = self.actor_backbone(obs_seq)
        return torch.tanh(self.actor_proj(tokens.reshape(tokens.shape[0], -1)))

    def _critic_latent(self, obs_seq):
        tokens = self.critic_backbone(obs_seq)
        return torch.tanh(self.critic_proj(tokens.reshape(tokens.shape[0], -1)))

    def get_value(self, obs_seq):
        return self.critic(self._critic_latent(obs_seq))

    def get_action_and_value(self, obs_seq, action=None):
        actor_latent = self._actor_latent(obs_seq)
        critic_latent = self._critic_latent(obs_seq)
