# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + DREAMER4 STATE-ENTROPY v2.
#
...
class Args:
    ...
    state_pred_eps: float = 1e-3              # clamp the (0,1) Beta target off the open boundary
    state_obs_clip: float = 10.0              # obs wrapper clip bound; affine-rescales obs->(0,1) for the Beta target
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # dims, masked to non-reset transitions. v2: targets map s' into (0,1) by
                # AFFINE rescale of the wrapper-clipped obs range [-clip,clip] -> (x/clip+1)/2,
                # clamped off the boundary -- dreamer4's (x+1)/2 with NO added nonlinearity
                # (v1's tanh saturated large-|obs| dims, suppressing the novelty signal).
                if args.state_entropy:
                    fwd_dist = agent.state_model.dist(b_obs[mb_inds], b_actions[mb_inds])
                    tgt = ((b_next_obs[mb_inds] / args.state_obs_clip + 1.0) * 0.5).clamp(
