# PPO + IterThink v24.18 (BETA LOGIT ACTION FLOW, EXACT Y-REPLAY + POSITIVE CFM).
# From v24.14 and v24.13.
#
# This is the low-risk comparator for the no-squish/no-clamp ablations. It keeps
# v24.14's exact y-replay logit-flow path: rollout stores post-flow logit y1, and
# the executed action is computed from sigmoid(y1) without an extra clamp. It also
# uses v24.13's positive-only CFM weighting so the auxiliary imitates only
# above-average rollout actions instead of applying a baseline imitation loss to
# neutral/bad samples.
#
