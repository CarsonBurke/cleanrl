# Delightful PPO with latent-Gaussian surprisal and tanh actions v9.
#
# DG multiplies each on-policy score term by sigmoid(A * ell / eta), where ell
# is the clipped joint surprisal of the latent diagonal Gaussian. Actions retain
# SAC's smooth tanh transform; PPO ratios use the exact bounded-action density.
# The transform is policy-independent, so its Jacobian cancels from ratios and
# score gradients, but excluding it from delight matches the Gaussian density
# coordinate specified by DG. Standard rollout collection uses 16 environments
# for 128 steps; the actor takes one fresh full-batch step, while only the critic
# uses minibatches.
# Each immediate reward exactly matches DeepMind Control Suite's cheetah:run:
# forward speed projected directly to [0, 1], without EMA or other scaling.
# Advantages and delight are not normalized or whitened. State-dependent log
# sigma uses SAC's smooth bounds with log sigma=-1.5 initialization.
#
# Hypothesis: retaining smooth bounded actions while gating on the paper's
# Gaussian surprisal restores DG's intended selectivity without clipping actions.
...
def control_suite_cheetah_reward(forward_speed):
    """Reproduce dm_control cheetah:run's linear tolerance reward exactly."""
    forward_speed = np.asarray(forward_speed)
    return np.clip(
        forward_speed / CONTROL_SUITE_RUN_SPEED,
        REWARD_LOWER_BOUND,
        REWARD_UPPER_BOUND,
    )


def extract_forward_speeds(infos, num_envs):
    """Read speeds from ordinary and Gymnasium auto-reset transition info."""
    forward_speeds = np.zeros(num_envs, dtype=np.float32)

    if "x_velocity" in infos:
        values = np.asarray(infos["x_velocity"], dtype=np.float32)
        mask = np.asarray(
            infos.get("_x_velocity", np.ones(num_envs, dtype=bool)), dtype=bool
        )
        forward_speeds[mask] = values[mask]

    # Gymnasium moves the terminal transition's info under final_info after
    # automatically resetting that environment. It is still the transition
    # whose reward is being stored, so its terminal speed must take precedence.
    for index, final_info in enumerate(infos.get("final_info", ())):
        if final_info is not None and "x_velocity" in final_info:
            forward_speeds[index] = final_info["x_velocity"]

    return forward_speeds
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None, raw_action=None):
        ...
        probs, _ = self.get_action_distribution(x)
        transformed_action, logprob, _ = self._action_and_logprobs_from_raw(
            probs, raw_action
        )
        if action is None:
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            "compiled action and value functions "
    ...
    gaussian_logprobs = torch.zeros_like(logprobs)
    rewards = torch.zeros((args.num_steps, args.num_envs)).to(device)
    raw_rewards = torch.zeros_like(rewards)
    forward_speeds = torch.zeros_like(rewards)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(args.num_steps):
            ...
            gaussian_logprobs[step] = gaussian_logprob

            next_obs_np, raw_reward, terminations, truncations, infos = envs.step(
                action.cpu().numpy()
            )
            raw_rewards[step] = torch.as_tensor(
                raw_reward, dtype=torch.float32, device=device
            )
            forward_speed = extract_forward_speeds(infos, args.num_envs)
            forward_speeds[step] = torch.as_tensor(
                forward_speed, dtype=torch.float32, device=device
            )
            rewards[step] = torch.as_tensor(
                control_suite_cheetah_reward(forward_speed),
                dtype=torch.float32,
                device=device,
            )
        ...
        forward_speed_mean = forward_speeds.mean().item()
        forward_speed_min = forward_speeds.min().item()
        forward_speed_max = forward_speeds.max().item()
        zero_reward_fraction = (forward_speeds <= 0.0).float().mean().item()
        target_reward_fraction = (
            (forward_speeds >= CONTROL_SUITE_RUN_SPEED).float().mean().item()
        )
        ...
        b_gaussian_logprobs = gaussian_logprobs.reshape(-1)
        ...
        # One actor epoch and one full-batch step. The rollout Gaussian density
        # is fresh under the current actor, so the gate never consumes stale data.
        if args.compile:
            torch.compiler.cudagraph_mark_step_begin()
        _, newlogprob, _, _ = action_value_function(
        ...
        with torch.no_grad():
            if args.compile:
                torch.compiler.cudagraph_mark_step_begin()
            _, post_logprob, _, _ = action_value_function(
                b_obs, b_actions, raw_action=b_raw_actions
            )
        ...
        writer.add_scalar(
            "diagnostics/latent_gaussian_surprisal_mean",
            (-b_gaussian_logprobs).mean().item(),
            global_step,
        )
        writer.add_scalar(
            "diagnostics/squashed_action_surprisal_mean",
            (-b_logprobs).mean().item(),
            global_step,
        )
        writer.add_scalar(
            "diagnostics/change_of_variables_logprob_delta_mean",
            (b_logprobs - b_gaussian_logprobs).mean().item(),
            global_step,
        )
        ...
        writer.add_scalar(
            "diagnostics/bounded_reward_rms",
            rewards.square().mean().sqrt().item(),
            global_step,
        )
        writer.add_scalar("diagnostics/raw_reward_mean", raw_reward_mean, global_step)
        writer.add_scalar("diagnostics/raw_reward_min", raw_reward_min, global_step)
        writer.add_scalar("diagnostics/raw_reward_max", raw_reward_max, global_step)
        writer.add_scalar(
            "diagnostics/forward_speed_mean", forward_speed_mean, global_step
        )
        writer.add_scalar(
            "diagnostics/forward_speed_min", forward_speed_min, global_step
        )
        writer.add_scalar(
            "diagnostics/forward_speed_max", forward_speed_max, global_step
        )
        writer.add_scalar(
            "diagnostics/zero_reward_fraction",
            zero_reward_fraction,
            global_step,
        )
        writer.add_scalar(
            "diagnostics/target_reward_fraction",
            target_reward_fraction,
            global_step,
        )
        writer.add_scalar(
            "diagnostics/bounded_reward_min", rewards.min().item(), global_step
        )
        writer.add_scalar(
            "diagnostics/bounded_reward_max", rewards.max().item(), global_step
        )
        writer.add_scalar(
