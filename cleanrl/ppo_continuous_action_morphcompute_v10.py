...
#
# V10. Make entmax alpha a learned per-distribution field, bounded to a
# conservative range. Each observation and destination cell predicts separate
# alpha values for current-tick and historical routing. Alpha controls the
# sparsity law itself: lower alpha is smoother/denser, higher alpha is
# sharper/sparser.
#
...
class Args:
    ...
    """initial entmax alpha for learned sparse routing"""
    entmax_alpha_min: float = 1.2
    """lower bound for learned entmax alpha"""
    entmax_alpha_max: float = 1.8
    """upper bound for learned entmax alpha"""
...
def entmax_bisect(logits, alpha, dim=-1, n_iter=24):
    ...
    logits = logits - logits.max(dim=dim, keepdim=True).values
    alpha_minus_one = (alpha - 1.0).clamp_min(1e-6)
    power = 1.0 / alpha_minus_one
    logits = logits * alpha_minus_one
    tau_lo = logits.min(dim=dim, keepdim=True).values - 1.0
    tau_hi = logits.max(dim=dim, keepdim=True).values
    for _ in range(n_iter):
        tau_mid = (tau_lo + tau_hi) / 2.0
        probs = torch.clamp(logits - tau_mid, min=0.0).pow(power)
        needs_more_mass = probs.sum(dim=dim, keepdim=True) < 1.0
        tau_hi = torch.where(needs_more_mass, tau_mid, tau_hi)
        tau_lo = torch.where(needs_more_mass, tau_lo, tau_mid)
    probs = torch.clamp(logits - tau_hi, min=0.0).pow(power)
...
def entmax_route_with_floor(logits, dense_floor, alpha, dim=-1):
    sparse_route = entmax_bisect(logits, alpha, dim=dim)
    if dense_floor <= 0.0:
...
class MorphogenicSubstrate(nn.Module):
    ...
    def __init__(self, obs_dim, args):
        ...
        self.entmax_alpha_min = args.entmax_alpha_min
        self.entmax_alpha_max = args.entmax_alpha_max
        ...
        alpha_init_frac = (args.entmax_alpha_init - args.entmax_alpha_min) / (
            args.entmax_alpha_max - args.entmax_alpha_min
        )
        alpha_init_frac = float(np.clip(alpha_init_frac, 1e-4, 1.0 - 1e-4))
        alpha_init_logit = np.log(alpha_init_frac / (1.0 - alpha_init_frac))
        self.current_alpha = layer_init(nn.Linear(self.D, self.K), std=0.01, bias_const=alpha_init_logit)
        self.lookback_alpha = layer_init(nn.Linear(self.D, self.K), std=0.01, bias_const=alpha_init_logit)
    ...
    def forward(self, x):
        ...
        current_alpha = self.entmax_alpha_min + (
            self.entmax_alpha_max - self.entmax_alpha_min
        ) * torch.sigmoid(self.current_alpha(base))
        lookback_alpha = self.entmax_alpha_min + (
            self.entmax_alpha_max - self.entmax_alpha_min
        ) * torch.sigmoid(self.lookback_alpha(base))
        ...
        stats = {
            ...
            "entmax_alpha": current_alpha.mean(dim=1),
            "current_entmax_alpha": current_alpha.mean(dim=1),
            "lookback_entmax_alpha": lookback_alpha.mean(dim=1),
        }
        return out, stats
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if last_stats is not None:
            ...
            writer.add_scalar("morph/actor_entmax_alpha", mean_stat(last_stats, "actor", "entmax_alpha"), global_step)
            writer.add_scalar("morph/critic_entmax_alpha", mean_stat(last_stats, "critic", "entmax_alpha"), global_step)
            writer.add_scalar("morph/actor_current_entmax_alpha", mean_stat(last_stats, "actor", "current_entmax_alpha"), global_step)
            writer.add_scalar("morph/critic_current_entmax_alpha", mean_stat(last_stats, "critic", "current_entmax_alpha"), global_step)
            writer.add_scalar("morph/actor_lookback_entmax_alpha", mean_stat(last_stats, "actor", "lookback_entmax_alpha"), global_step)
            writer.add_scalar("morph/critic_lookback_entmax_alpha", mean_stat(last_stats, "critic", "lookback_entmax_alpha"), global_step)
