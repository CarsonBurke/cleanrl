# PPO + IterThink v211 affine-RMS hierarchical Gaussian options.
#
# Preserve v198's low-level Beta PPO, raw affine-RMS encoder, attached
# LeJEPA/SIGReg world model, detached belief interface, and 4-D/64-step coherent
# exploration channel. Replace random resampling with an agent-owned Gaussian
# option policy over that same channel. A detached high token reads the v198
# belief only at option boundaries; completed real semi-Markov segments train a
# separate high actor/critic with duration-aware returns and exact Gaussian KL.
# This borrows Dreamer4's one-way agent/token separation, not its pixel tokenizer,
# dynamics objective, or policy algorithm. Low V(s,c) gets a zero-init option
# residual while the low actor and WM remain unchanged. v198's imagined LOW
# critic updates remain intact; imagined data never trains the hierarchy.
#
# -----------------------------------------------------------------------------
...
class Args:
    ...
    # v211 semi-Markov option policy. The option dimension and duration are fixed
    # to v198's successful random exploration process for an exact behavioral
    # N(0,I) initialization.
    option_horizon: int = 64
    high_learning_rate: float = 3e-4
    high_update_epochs: int = 10
    high_target_kl: float = 0.01
    high_gae_lambda: float = 0.95
    high_min_segments: int = 16
    high_actor_warmup_steps: int = 100000
    high_actor_warmup_segments: int = 256
    high_actor_grad_clip: float = 0.25
    high_critic_grad_clip: float = 0.5
    ...
    imagine_enable: bool = True       # preserve v198 low critic; hierarchy itself remains real-only
    ...
    imagine_prompt_bank_rollouts: int = 6  # preserve v198's rolling real-context dream seeds
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Agent-owned hierarchy. Forked initialization preserves every v198 low
        # actor/critic/WM parameter and the global rollout RNG lineage. Zero heads
        # make the initial option exactly N(0,I), and the low critic residual is
        # exactly inactive until value learning discovers code dependence.
        with torch.random.fork_rng():
            torch.manual_seed(args.seed + 60000)
            self.critic_code_proj = layer_init(
                nn.Linear(self.explore_code_dim, args.lewm_dim), std=0.0
            )
            self.high_actor_readout = AgentQueryReadout(
                obs_dim, args.lewm_dim, args.lewm_heads, args.lewm_ffn_mult
            )
            self.high_critic_readout = AgentQueryReadout(
                obs_dim, args.lewm_dim, args.lewm_heads, args.lewm_ffn_mult
            )
            self.high_actor_trunk = ThinkTrunk(
                args.lewm_dim, H, args.k_blocks, args.n_experts
            )
            self.high_critic_trunk = ThinkTrunk(
                args.lewm_dim, H, args.k_blocks, args.n_experts
            )
            self.high_actor_mean = layer_init(
                nn.Linear(H, self.explore_code_dim), std=0.0
            )
            self.high_actor_logstd = nn.Parameter(
                torch.zeros(self.explore_code_dim)
            )
            self.high_critic_head = layer_init(nn.Linear(H, 1), std=0.0)
    ...
    def _trunks_from_agent_input(self, obs, agent_input, explore_code=None):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        critic_feat = self._critic_feat_from_agent_input(agent_input, explore_code)
        return self._actor_feat_from_agent_input(agent_input), critic_feat
    ...
    def _critic_feat_from_agent_input(self, agent_input, explore_code=None):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        # Insert c before the nonlinear critic trunk. Adding it after the trunk
        # would only learn a global code preference and could not represent the
        # state-dependent value of an exploration direction.
        critic_input = self.critic_readout(agent_input)
        if explore_code is not None:
            critic_input = critic_input + self.critic_code_proj(explore_code.detach())
    ...
    def get_value_from_agent_input(self, obs, agent_input, explore_code=None):
        critic_feat = self._critic_feat_from_agent_input(agent_input, explore_code)
        return self.critic_head(critic_feat).view(-1, self.critic_mtp_horizon, self.num_bins)

    def high_policy_params(self, agent_input):
        agent_input = agent_input.detach()
        feat = self.high_actor_trunk(self.high_actor_readout(agent_input))
        mean = self.high_actor_mean(feat)
        # Smooth finite range prevents a slowly drifting option variance from
        # turning persistent codes into rare catastrophic action shifts.
        bounded_logstd = 2.0 * torch.tanh(self.high_actor_logstd.to(mean.dtype) / 2.0)
        logstd = bounded_logstd.expand_as(mean)
        return mean, logstd

    def sample_high_option(self, agent_input, generator):
        mean, logstd = self.high_policy_params(agent_input)
        eps = torch.randn(
            mean.shape, device=mean.device, dtype=mean.dtype, generator=generator
        )
        code = mean + logstd.exp() * eps
        logprob = Normal(mean, logstd.exp()).log_prob(code).sum(dim=-1)
        return code, logprob, mean, logstd

    def high_value_symlog(self, agent_input):
        agent_input = agent_input.detach()
        feat = self.high_critic_trunk(self.high_critic_readout(agent_input))
        return self.high_critic_head(feat).squeeze(-1)

    def high_value(self, agent_input):
        return symexp(self.high_value_symlog(agent_input))
    ...
    def get_action_and_value_from_agent_input(
        ...
        actor_feat, critic_feat = self._trunks_from_agent_input(
            obs, agent_input, explore_code
        )
    ...
    def critic_parameters(self):
        ...
        return (
            ...
            + list(self.critic_code_proj.parameters())
        )

    def high_actor_parameters(self):
        return (
            list(self.high_actor_readout.parameters())
            + list(self.high_actor_trunk.parameters())
            + list(self.high_actor_mean.parameters())
            + [self.high_actor_logstd]
        )

    def high_critic_parameters(self):
        return (
            list(self.high_critic_readout.parameters())
            + list(self.high_critic_trunk.parameters())
            + list(self.high_critic_head.parameters())
        )
...
if __name__ == "__main__":
    ...
    assert args.explore_code_dim == 4
    assert args.option_horizon == args.explore_resample_steps == 64
    assert args.high_target_kl > 0.0 and args.high_min_segments > 1
    assert args.high_actor_warmup_steps >= 0
    assert args.high_actor_warmup_segments >= args.high_min_segments
    ...
    high_actor_params = agent.high_actor_parameters()
    high_critic_params = agent.high_critic_parameters()
    high_param_ids = {id(p) for p in high_actor_params + high_critic_params}
    non_wm_params = [
        p for p in agent.parameters()
        if id(p) not in wm_param_ids and id(p) not in high_param_ids
    ]
    ...
    assert high_param_ids.isdisjoint(wm_param_ids | actor_param_ids | {id(p) for p in critic_params})
    high_actor_optimizer = optim.Adam(
        high_actor_params, lr=args.high_learning_rate, eps=1e-5
    )
    high_critic_optimizer = optim.Adam(
        high_critic_params, lr=args.high_learning_rate, eps=1e-5
    )

    def snapshot_high_actor_optimizer_state():
        param_values = [p.detach().clone() for p in high_actor_params]
        state_values = []
        for p in high_actor_params:
            state_values.append({
                key: value.detach().clone() if torch.is_tensor(value) else value
                for key, value in high_actor_optimizer.state.get(p, {}).items()
            })
        return param_values, state_values

    @torch.no_grad()
    def restore_high_actor_optimizer_state(snapshot):
        param_values, state_values = snapshot
        for p, value, saved_state in zip(high_actor_params, param_values, state_values):
            p.copy_(value)
            current_state = high_actor_optimizer.state[p]
            current_state.clear()
    ...
    high_bootstrap_override_mask = torch.zeros(
        (args.num_steps, args.num_envs), dtype=torch.bool, device=device
    )
    high_bootstrap_override_value = torch.zeros(
    ...
    option_active = torch.zeros(args.num_envs, dtype=torch.bool, device=device)
    option_duration = torch.zeros(args.num_envs, dtype=torch.long, device=device)
    option_discounted_reward = torch.zeros(args.num_envs, device=device)
    option_start_input = torch.zeros(
        args.num_envs, agent.agent_input_dim, device=device
    )
    option_behavior_logprob = torch.zeros(args.num_envs, device=device)
    option_behavior_mean = torch.zeros(
        args.num_envs, args.explore_code_dim, device=device
    )
    option_behavior_logstd = torch.zeros_like(option_behavior_mean)
    total_completed_option_segments = 0
    imagine_prompt_bank = []
    ...
    @torch.no_grad()
    def activate_options(mask, boundary_agent_input):
        if not bool(mask.any()):
            return
        code, logprob, mean, logstd = agent.sample_high_option(
            boundary_agent_input[mask], explore_generator
        )
        current_explore_code[mask] = code
        option_start_input[mask] = boundary_agent_input[mask].detach()
        option_behavior_logprob[mask] = logprob
        option_behavior_mean[mask] = mean
        option_behavior_logstd[mask] = logstd
        option_duration[mask] = 0
        option_discounted_reward[mask] = 0.0
        option_active[mask] = True
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            for pg in optimizer.param_groups:  # v103: anneal both the non-WM and WM groups
                pg["lr"] = lrnow
            high_lrnow = frac * args.high_learning_rate
            for high_optimizer in (high_actor_optimizer, high_critic_optimizer):
                for pg in high_optimizer.param_groups:
                    pg["lr"] = high_lrnow

        rollout_actor_active = global_step >= args.wm_warmup_steps
        completed_option_segments = []
        high_bootstrap_override_mask.zero_()
        high_bootstrap_override_value.zero_()
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                activate_options(~option_active, agent_input)
                ...
                if not rollout_actor_active and args.actor_dist == "beta":
                    ...
                    value_logits = agent.get_value_from_agent_input(
                        next_obs, agent_input, current_explore_code
                    )
                else:
                    (
                        action,
            ...
            with torch.no_grad():
                ...
                # Complete option rewards in semi-Markov units. An option ends at
                # 64 real steps or an episode boundary; rollout tails do neither,
                # so their accumulators persist into the next rollout unchanged.
                finishing_code = current_explore_code.clone()
                option_discounted_reward += (
                    args.gamma ** option_duration.to(reward_tensor.dtype)
                ) * reward_tensor
                option_duration += 1
                option_complete = (option_duration >= args.option_horizon) | boundary_tensor
                for env_idx in option_complete.nonzero(as_tuple=False).squeeze(-1).tolist():
                    completed_option_segments.append({
                        "env": env_idx,
                        "start_input": option_start_input[env_idx].detach().clone(),
                        "code": finishing_code[env_idx].detach().clone(),
                        "logprob": option_behavior_logprob[env_idx].detach().clone(),
                        "mean": option_behavior_mean[env_idx].detach().clone(),
                        "logstd": option_behavior_logstd[env_idx].detach().clone(),
                        "reward": option_discounted_reward[env_idx].detach().clone(),
                        "duration": int(option_duration[env_idx].item()),
                        "termination": termination_tensor[env_idx].detach().clone(),
                        "bootstrap_valid": transition_valids[
                            step, env_idx
                        ].detach().clone(),
                        "boundary": boundary_tensor[env_idx].detach().clone(),
                        "next_input": next_transition_agent_input[env_idx].detach().clone(),
                    })
                option_active[option_complete] = False

                # At a realized 64-step decision boundary, sample c_next at s'
                # immediately so low V(s',c_next) and the next action agree.
                # Mid-option truncations continue under the finishing code. At a
                # valid time-limit exactly at 64, the decision-level successor is
                # marginalized by detached V_H(final_observation); record a scalar
                # override below rather than sampling an unrealized option. The
                # actual reset always samples fresh. True terminations are masked.
                duration_boundary = option_duration >= args.option_horizon
                normal_option_boundary = duration_boundary & (~boundary_tensor)
                activate_options(normal_option_boundary, next_transition_agent_input)
                successor_code = finishing_code
                successor_code[normal_option_boundary] = current_explore_code[
                    normal_option_boundary
                ]
                horizon_truncation = (
                    duration_boundary
                    & boundary_tensor
                    & (termination_tensor < 0.5)
                    & (transition_valids[step] > 0.5)
                )
                if bool(horizon_truncation.any()):
                    high_bootstrap_override_mask[step, horizon_truncation] = True
                    high_bootstrap_override_value[step, horizon_truncation] = (
                        agent.high_value(
                            next_transition_agent_input[horizon_truncation]
                        ).detach()
                    )
                next_transition_explore_codes[step] = successor_code
        ...
        with torch.no_grad():
            next_transition_value_logits = agent.get_value_from_agent_input(
                ...
                next_transition_explore_codes.reshape(-1, args.explore_code_dim),
            )[:, 0]  # horizon 0 = V(s')
            ...
            # Exact-horizon time limits transition at the option level: low GAE
            # bootstraps their valid final observation with V_H, not V_L(c_old).
            next_transition_values = torch.where(
                high_bootstrap_override_mask,
                high_bootstrap_override_value,
                ...
                with torch.no_grad():
                    ...
                    value_probs_H = torch.softmax(
                        agent.get_value_from_agent_input(
                            None,
                            agent.agent_input_from_latent(belief_H),
                            im_explore_code,
                        )[:, 0],  # horizon 0 = V(s_H)
                        dim=-1,
                    )
        ...
        # ======================= REAL SEMI-MARKOV OPTIONS =======================
        high_segment_count = len(completed_option_segments)
        total_completed_option_segments += high_segment_count
        high_actor_updates_used = 0
        high_actor_updates_rejected = 0
        high_exact_kl = 0.0
        high_policy_loss = 0.0
        high_value_loss = 0.0
        high_actor_grad_norm = 0.0
        high_critic_grad_norm = 0.0
        high_duration_mean = 0.0
        high_return_mean = 0.0
        high_adv_mean = 0.0
        high_option_mean_abs = 0.0
        high_option_std_mean = 1.0
        high_ratio_p95 = 1.0
        high_ratio_ess = 1.0
        high_entropy = 0.0
        high_selector_mean = 0.0
        high_selector_ci_low = 0.0
        high_value_explained_variance = 0.0
        high_boundary_fraction = 0.0
        high_termination_segments = sum(
            int(segment["termination"].item() > 0.5)
            for segment in completed_option_segments
        )
        high_boundary_segments = sum(
            int(bool(segment["boundary"]))
            for segment in completed_option_segments
        )
        high_truncation_segments = high_boundary_segments - high_termination_segments
        high_horizon_segments = high_segment_count - high_boundary_segments
        if high_segment_count >= args.high_min_segments:
            high_start_inputs = torch.stack([
                segment["start_input"] for segment in completed_option_segments
            ])
            high_next_inputs = torch.stack([
                segment["next_input"] for segment in completed_option_segments
            ])
            high_codes = torch.stack([
                segment["code"] for segment in completed_option_segments
            ])
            high_old_logprobs = torch.stack([
                segment["logprob"] for segment in completed_option_segments
            ])
            high_old_means = torch.stack([
                segment["mean"] for segment in completed_option_segments
            ])
            high_old_logstds = torch.stack([
                segment["logstd"] for segment in completed_option_segments
            ])
            high_rewards = torch.stack([
                segment["reward"] for segment in completed_option_segments
            ])
            high_durations = torch.tensor(
                [segment["duration"] for segment in completed_option_segments],
                device=device,
                dtype=torch.float32,
            )
            high_terminations = torch.stack([
                segment["termination"] for segment in completed_option_segments
            ])
            high_bootstrap_valids = torch.stack([
                segment["bootstrap_valid"] for segment in completed_option_segments
            ])
            high_boundaries = torch.stack([
                segment["boundary"] for segment in completed_option_segments
            ]).bool()
            high_envs = [segment["env"] for segment in completed_option_segments]

            with torch.no_grad():
                high_values = agent.high_value(high_start_inputs)
                high_next_values = agent.high_value(high_next_inputs)
                high_discounts = args.gamma ** high_durations
                # A mid-option time-limit is an artificial cut through a
                # continuing (s,c) process, so bootstrap it with V_L(s,c).
                # If truncation lands exactly on the 64-step decision, V_H(s)
                # is the correct marginal successor value instead.
                mid_option_truncations = (
                    high_boundaries
                    & (high_terminations < 0.5)
                    & (high_durations < args.option_horizon)
                )
                if bool(mid_option_truncations.any()):
                    low_bootstrap_logits = agent.get_value_from_agent_input(
                        None,
                        high_next_inputs[mid_option_truncations],
                        high_codes[mid_option_truncations],
                    )[:, 0]
                    high_next_values[mid_option_truncations] = value_logits_to_scalar(
                        low_bootstrap_logits
                    )
                high_deltas = (
                    high_rewards
                    + high_discounts
                    * (1.0 - high_terminations)
                    * high_bootstrap_valids
                    * high_next_values
                    - high_values
                )
                high_advantages = torch.zeros_like(high_deltas)
                next_advantage_by_env = {}
                for idx in reversed(range(high_segment_count)):
                    env_idx = high_envs[idx]
                    next_advantage = (
                        0.0
                        if bool(high_boundaries[idx])
                        else next_advantage_by_env.get(env_idx, 0.0)
                    )
                    high_advantages[idx] = (
                        high_deltas[idx]
                        + high_discounts[idx]
                        * args.high_gae_lambda
                        * next_advantage
                    )
                    next_advantage_by_env[env_idx] = high_advantages[idx]
                high_returns = high_advantages + high_values
                high_actor_advantages = (
                    high_advantages - high_advantages.mean()
                ) / (high_advantages.std() + 1e-8)
                high_return_var = high_returns.var(unbiased=False)
                if high_return_var > 0:
                    high_value_explained_variance = float(
                        1.0
                        - (high_returns - high_values).var(unbiased=False)
                        / high_return_var
                    )
                prior_logprob = Normal(
                    torch.zeros_like(high_codes), torch.ones_like(high_codes)
                ).log_prob(high_codes).sum(dim=-1)
                selector_samples = high_advantages * (
                    high_old_logprobs - prior_logprob
                )
                high_selector_mean = selector_samples.mean().item()
                if high_segment_count > 1:
                    high_selector_ci_low = (
                        selector_samples.mean()
                        - 1.96
                        * selector_samples.std(unbiased=True)
                        / high_segment_count ** 0.5
                    ).item()

            # Detached high critic: symlog regression keeps long segment returns
            # well scaled while scalar bootstrap values remain in raw reward units.
            for _ in range(args.high_update_epochs):
                high_value_pred = agent.high_value_symlog(high_start_inputs)
                high_v_loss = 0.5 * (
                    high_value_pred - symlog(high_returns)
                ).square().mean()
                high_critic_optimizer.zero_grad(set_to_none=True)
                high_v_loss.backward()
                high_critic_gn = nn.utils.clip_grad_norm_(
                    high_critic_params, args.high_critic_grad_clip
                )
                high_critic_optimizer.step()
                high_value_loss = high_v_loss.item()
                high_critic_grad_norm = float(high_critic_gn)

            old_high_dist = Normal(high_old_means, high_old_logstds.exp())
            high_actor_warm = (
                global_step >= args.high_actor_warmup_steps
                and total_completed_option_segments >= args.high_actor_warmup_segments
            )
            if high_actor_warm:
                for _ in range(args.high_update_epochs):
                    high_mean, high_logstd = agent.high_policy_params(high_start_inputs)
                    high_dist = Normal(high_mean, high_logstd.exp())
                    high_new_logprob = high_dist.log_prob(high_codes).sum(dim=-1)
                    high_ratio = (high_new_logprob - high_old_logprobs).exp()
                    high_pg1 = -high_actor_advantages * high_ratio
                    high_pg2 = -high_actor_advantages * torch.clamp(
                        high_ratio, 1.0 - args.clip_coef, 1.0 + args.clip_coef
                    )
                    high_pg_loss = torch.maximum(high_pg1, high_pg2).mean()
                    with torch.no_grad():
                        pre_high_kl = torch.distributions.kl_divergence(
                            old_high_dist, high_dist
                        ).sum(dim=-1).mean()
                    high_exact_kl = float(pre_high_kl)
                    if not torch.isfinite(pre_high_kl) or pre_high_kl > args.high_target_kl:
                        high_actor_updates_rejected += 1
                        break
                    high_actor_snapshot = snapshot_high_actor_optimizer_state()
                    high_actor_optimizer.zero_grad(set_to_none=True)
                    high_pg_loss.backward()
                    high_actor_gn = nn.utils.clip_grad_norm_(
                        high_actor_params, args.high_actor_grad_clip
                    )
                    high_actor_optimizer.step()
                    with torch.no_grad():
                        post_mean, post_logstd = agent.high_policy_params(high_start_inputs)
                        post_high_dist = Normal(post_mean, post_logstd.exp())
                        post_high_kl = torch.distributions.kl_divergence(
                            old_high_dist, post_high_dist
                        ).sum(dim=-1).mean()
                    if (
                        not torch.isfinite(post_high_kl)
                        or post_high_kl > args.high_target_kl
                    ):
                        restore_high_actor_optimizer_state(high_actor_snapshot)
                        high_actor_updates_rejected += 1
                        high_exact_kl = float(pre_high_kl)
                        break
                    high_actor_updates_used += 1
                    high_exact_kl = float(post_high_kl)
                    high_policy_loss = high_pg_loss.item()
                    high_actor_grad_norm = float(high_actor_gn)

            with torch.no_grad():
                final_high_mean, final_high_logstd = agent.high_policy_params(
                    high_start_inputs
                )
                final_high_dist = Normal(
                    final_high_mean, final_high_logstd.exp()
                )
                final_high_logprob = final_high_dist.log_prob(high_codes).sum(dim=-1)
                final_high_ratio = (final_high_logprob - high_old_logprobs).exp()
                high_exact_kl = float(
                    torch.distributions.kl_divergence(
                        old_high_dist, final_high_dist
                    ).sum(dim=-1).mean()
                )
                high_ratio_p95 = torch.quantile(final_high_ratio, 0.95).item()
                high_ratio_ess = (
                    final_high_ratio.sum().square()
                    / (
                        high_segment_count
                        * final_high_ratio.square().sum().clamp_min(1e-8)
                    )
                ).item()
                high_entropy = final_high_dist.entropy().sum(dim=-1).mean().item()
            high_duration_mean = high_durations.mean().item()
            high_return_mean = high_returns.mean().item()
            high_adv_mean = high_advantages.mean().item()
            high_option_mean_abs = final_high_mean.abs().mean().item()
            high_option_std_mean = final_high_logstd.exp().mean().item()
            high_boundary_fraction = high_boundaries.float().mean().item()
        ...
        with torch.no_grad():
            ...
            actor_feat_diag, critic_code_feat_diag = agent._trunks_from_agent_input(
                None, b_agent_inputs[:diag_n], b_explore_codes[:diag_n]
            )
            _, critic_zero_feat_diag = agent._trunks_from_agent_input(
                None, b_agent_inputs[:diag_n], torch.zeros_like(b_explore_codes[:diag_n])
            )
            ...
            critic_code_value = value_logits_to_scalar(
                agent.critic_head(critic_code_feat_diag).view(
                    -1, agent.critic_mtp_horizon, agent.num_bins
                )[:, 0]
            )
            critic_zero_value = value_logits_to_scalar(
                agent.critic_head(critic_zero_feat_diag).view(
                    -1, agent.critic_mtp_horizon, agent.num_bins
                )[:, 0]
            )
            critic_code_sensitivity = (
                critic_code_value - critic_zero_value
            ).abs().mean()

        writer.add_scalar("charts/learning_rate", optimizer.param_groups[0]["lr"], global_step)
        writer.add_scalar(
            "charts/high_learning_rate",
            high_actor_optimizer.param_groups[0]["lr"],
            global_step,
        )
        ...
        writer.add_scalar(
            "options/low_critic_code_sensitivity",
            critic_code_sensitivity.item(),
            global_step,
        )
        ...
        writer.add_scalar("options/completed_segments", high_segment_count, global_step)
        writer.add_scalar("options/pending_segments", option_active.float().sum().item(), global_step)
        writer.add_scalar(
            "options/pending_duration_mean",
            option_duration[option_active].float().mean().item()
            if bool(option_active.any()) else 0.0,
            global_step,
        )
        writer.add_scalar("options/duration_mean", high_duration_mean, global_step)
        writer.add_scalar("options/return_mean", high_return_mean, global_step)
        writer.add_scalar("options/advantage_mean", high_adv_mean, global_step)
        writer.add_scalar("options/exact_gaussian_kl", high_exact_kl, global_step)
        writer.add_scalar("options/policy_loss", high_policy_loss, global_step)
        writer.add_scalar("options/value_loss", high_value_loss, global_step)
        writer.add_scalar("options/actor_updates_used", high_actor_updates_used, global_step)
        writer.add_scalar("options/actor_updates_rejected", high_actor_updates_rejected, global_step)
        writer.add_scalar("options/actor_grad_norm", high_actor_grad_norm, global_step)
        writer.add_scalar("options/critic_grad_norm", high_critic_grad_norm, global_step)
        writer.add_scalar("options/policy_mean_abs", high_option_mean_abs, global_step)
        writer.add_scalar("options/policy_std_mean", high_option_std_mean, global_step)
        writer.add_scalar("options/ratio_p95", high_ratio_p95, global_step)
        writer.add_scalar("options/ratio_ess_fraction", high_ratio_ess, global_step)
        writer.add_scalar("options/entropy", high_entropy, global_step)
        writer.add_scalar("options/selector_adv_logdensity", high_selector_mean, global_step)
        writer.add_scalar("options/selector_adv_logdensity_ci_low", high_selector_ci_low, global_step)
        writer.add_scalar("options/value_explained_variance", high_value_explained_variance, global_step)
        writer.add_scalar(
            "options/actor_warmup_active",
            float(
                global_step < args.high_actor_warmup_steps
                or total_completed_option_segments < args.high_actor_warmup_segments
            ),
            global_step,
        )
        writer.add_scalar(
            "options/total_completed_segments",
            total_completed_option_segments,
            global_step,
        )
        writer.add_scalar("options/episode_boundary_fraction", high_boundary_fraction, global_step)
        writer.add_scalar("options/horizon_segments", high_horizon_segments, global_step)
        writer.add_scalar("options/termination_segments", high_termination_segments, global_step)
        writer.add_scalar("options/truncation_segments", high_truncation_segments, global_step)
        if wm_losses:
            writer.add_scalar("lewm/loss", float(np.mean(wm_losses)), global_step)
            writer.add_scalar("lewm/obs_latent_mse", float(np.mean(wm_latent_losses)), global_step)
            writer.add_scalar("lewm/reward_ce", float(np.mean(wm_reward_losses)), global_step)
