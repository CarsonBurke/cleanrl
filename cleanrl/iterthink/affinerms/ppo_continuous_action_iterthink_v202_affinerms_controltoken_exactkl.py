# PPO + IterThink v202 affine-RMS persistent control-token world model.
#
# Keep v198's observation-only affine-RMS encoder, attached encoder-side SIGReg,
# action-free temporal state trunk, and outbound transition decoder unchanged.
# Add a WM-owned one-way control stream: a local query reads the current encoded
# and temporal state plus shifted inbound a_(t-1), its single slot mixes causally
# through time, and a final query rereads the history-enriched current state. PPO
# consumes only this detached persistent control token through disjoint actor and
# critic trunks. Attached outbound-action auxiliaries make the token sufficient
# for next-state tokens, reward, and continuation without EMA targets, target
# detach, representation normalization, or SIGReg on the control stream.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    share_backbone: bool = False     # v202: separate trunks consume the same detached WM control token
    ...
    lewm_predictor_layers: int = 2   # v202 retains v198's action-free temporal state trunk
    lewm_heads: int = 4
    lewm_kv_heads: int = 2
    lewm_ffn_mult: int = 2
    lewm_context: int = 5
    lewm_dyn_horizon: int = 5
    lewm_mtp_len: int = 1            # v202: only offset 0 has the required outbound-action conditioning
    ...
    # Attached control-token sufficiency objective. Internally it weights next-state
    # MSE at 1 and reward/continuation at the existing 0.5/0.5 ratio.
    lewm_control_aux_coef: float = 0.5
...
class ControlTemporalBlock(nn.Module):
    """Causal single-slot history with exact episode/padding key masking."""
    ...
    def forward(self, control, frame_valids):
        # control: (B,T,D), frame_valids: (B,T). Invalid pre-boundary/padded
        # frames are neither keys nor values and are kept exactly zero.
...
class ControlQueryBlock(nn.Module):
    """One-way query update: the control slot reads memory but cannot write to it."""
...
class PersistentControlStream(nn.Module):
    """Dreamer4-inspired WM-owned control slot with explicit inbound action ownership."""
    ...
    def __init__(self, dim, act_dim, num_heads, ffn_mult, rope_fraction):
        ...
        self.inbound_action_embed = xavier_linear(nn.Linear(act_dim, dim))
    ...
    def forward(self, encoded_states, temporal_states, previous_actions, frame_valids=None):
        # All tensors are aligned at state time t. previous_actions[..., t] owns
        # the inbound a_(t-1); the outbound a_t is intentionally absent here.
        batch, time_len, num_tokens, width = temporal_states.shape
        if encoded_states.shape != temporal_states.shape:
            raise ValueError("encoded and temporal control memories must have identical shapes")
        if previous_actions.shape[:2] != (batch, time_len):
            raise ValueError("previous actions must align one-for-one with state frames")
        if frame_valids is None:
            frame_valids = torch.ones(batch, time_len, dtype=torch.bool, device=temporal_states.device)
        elif frame_valids.shape != (batch, time_len):
            raise ValueError("control frame validity mask has the wrong shape")
        action_token = self.inbound_action_embed(previous_actions).unsqueeze(2)
        ...
        local_control = self.local_read(query, flat_memory).reshape(batch, time_len, 1, width)
        local_control = local_control[:, :, 0] * frame_valids.unsqueeze(-1).to(local_control.dtype)
        history_control = self.temporal(local_control, frame_valids).unsqueeze(2)
        final_query = history_control.reshape(batch * time_len, 1, width)
        final_memory = temporal_states.reshape(batch * time_len, num_tokens, width)
        control = self.final_read(final_query, final_memory)
        control = control.reshape(batch, time_len, width)
        return control * frame_valids.unsqueeze(-1).to(control.dtype)


class ControlAuxiliaryHeads(nn.Module):
    """Outbound-action probes that make the WM control token outcome sufficient."""

    def __init__(self, dim, act_dim, num_obs_tokens, reward_num_bins, ffn_mult):
        super().__init__()
        self.num_obs_tokens = num_obs_tokens
        self.dim = dim
        self.action_embed = xavier_linear(nn.Linear(act_dim, dim))
        self.fuse1 = xavier_linear(nn.Linear(dim, dim * ffn_mult, bias=False))
        self.fuse2 = xavier_linear(nn.Linear(dim * ffn_mult, dim, bias=False))
        self.next_state = xavier_linear(nn.Linear(dim, num_obs_tokens * dim))
        self.reward = xavier_linear(nn.Linear(dim, reward_num_bins))
        self.continuation = xavier_linear(nn.Linear(dim, 1))

    def forward(self, control, outbound_actions):
        features = control + self.action_embed(outbound_actions)
        features = features + self.fuse2(relu_sq(self.fuse1(features)))
        next_state = self.next_state(features).reshape(
            *features.shape[:-1], self.num_obs_tokens, self.dim
        )
        return next_state, self.reward(features), self.continuation(features).squeeze(-1)
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.control_stream = PersistentControlStream(
            self.dim,
            self.act_dim,
            args.lewm_heads,
            args.lewm_ffn_mult,
        ...
        self.control_aux = ControlAuxiliaryHeads(
            self.dim,
            self.act_dim,
            self.num_obs_tokens,
    ...
    def control_from_state_memory(
        self, latent_history, state_memory, previous_actions, frame_valids=None
    ):
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
            state_memory = state_memory[:, -self.context :]
            previous_actions = previous_actions[:, -self.context :]
            if frame_valids is not None:
                frame_valids = frame_valids[:, -self.context :]
        # Every finite causal window owns its left edge: no action immediately
        # preceding the retained window is observable. Replay and online already
        # construct this BOS sentinel; enforce it centrally so sliding dream
        # windows cannot retain the dropped frame's outbound action.
        previous_actions = previous_actions.clone()
        previous_actions[:, 0] = 0.0
        return self.control_stream(
            latent_history, state_memory, previous_actions, frame_valids
        )

    def control_from_history(self, latent_history, previous_actions, frame_valids=None):
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
            previous_actions = previous_actions[:, -self.context :]
            if frame_valids is not None:
        ...
        return self.control_from_state_memory(
            latent_history, state_memory, previous_actions, frame_valids
        )[:, -1]

    def predict_control_aux(self, control, outbound_actions):
        return self.control_aux(control, outbound_actions)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if not self.detach_world_model_from_agent:
            raise ValueError("v202 requires a strictly detached world-model/agent interface")
        self.share_backbone = args.share_backbone
        if self.share_backbone:
            raise ValueError("v202 requires separate actor/critic trunks")
        self.explore_code_dim = args.explore_code_dim
        self.explore_logit_scale = args.explore_logit_scale
        # PPO buffers the detached WM-owned control token. There is deliberately no
        # trainable agent-side query that can reinterpret stale state-token memory.
        self.agent_latent_dim = args.lewm_dim
        ...
        assert self.agent_input_dim == self.world_model.dim
    ...
    def agent_input_from_control(self, control):
        if self.detach_world_model_from_agent:
            control = control.detach()
        return control.reshape(control.shape[0], -1)

    def agent_input_from_history(self, latent_history, previous_actions, frame_valids=None):
        control = self.world_model.control_from_history(
            latent_history, previous_actions, frame_valids
        )
        return self.agent_input_from_control(control)
    ...
    def agent_input_from_obs(self, x):
        ...
        previous_action = torch.zeros(
            latent.shape[0], 1, self.world_model.act_dim, device=latent.device, dtype=latent.dtype
        )
        return self.agent_input_from_history(latent.unsqueeze(1), previous_action)
    ...
    def _actor_feat_from_agent_input(self, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        return self.actor_trunk(agent_input)

    def _critic_feat_from_agent_input(self, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        return self.critic_trunk(agent_input)
...
if __name__ == "__main__":
    ...
    assert args.lewm_mtp_len == 1, "v202 transition queries condition only the one-step outbound action"
    assert args.detach_world_model_from_agent, "v202 requires PPO to be detached from the WM"
    ...
    assert not args.share_backbone, "v202 separates actor/critic trunks"
    ...
    # Param groups for decoupled clipping. v202 deliberately has no shared
    ...
    assert not shared_trunk_params, "v202 actor and critic paths must be disjoint"
    ...
    def build_control_agent_input(obs_tensor):
        ...
        if n_past > 0:
            ...
            # Every stored action is outbound from the identically indexed past
            # state and therefore inbound to the next state. Invalidating history
            # at an episode boundary also removes the crossing action.
            past_actions = [
        ...
        sentinel = torch.zeros(
            obs_tensor.shape[0], agent.world_model.act_dim, device=obs_tensor.device, dtype=current_latent.dtype
        )
        previous_actions = torch.stack([sentinel] + past_actions, dim=1)
        current_valid = torch.ones(obs_tensor.shape[0], dtype=torch.bool, device=obs_tensor.device)
        frame_valids = torch.stack(past_valids + [current_valid], dim=1)
        return (
            agent.agent_input_from_history(belief_latents, previous_actions, frame_valids),
            current_latent,
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                agent_input, current_latent = build_control_agent_input(next_obs)
            ...
            with torch.no_grad():
                next_transition_agent_input, _ = build_control_agent_input(transition_next_obs_t)
        ...
        wm_control_latent_losses = []
        wm_control_reward_losses = []
        wm_control_termination_losses = []
        wm_sigreg_losses = []
        wm_grad_norms = []
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    pred_mtp, state_memory = agent.world_model.predict_mtp_from_history(
                        teacher_history, future_actions, return_beliefs=True
                    )

                    # Align inbound actions with state ownership. The first frame
                    # in a finite context has the same zero sentinel used by online
                    # and dream windows; every later frame receives the preceding
                    # outbound action, never its own a_t.
                    teacher_previous_actions = torch.zeros_like(future_actions)
                    if horizon > 1:
                        teacher_previous_actions[:, 1:] = (
                            future_actions[:, :-1]
                            * (1.0 - future_boundaries[:, :-1]).unsqueeze(-1)
                        )
                    ...
                    control_sequence = agent.world_model.control_from_state_memory(
                        teacher_history,
                        state_memory,
                        teacher_previous_actions,
                        frame_valids=step_weight.bool(),
                    )
                    (
                        control_next_state,
                        control_reward_logits,
                        control_continuation_logits,
                    ) = agent.world_model.predict_control_aux(control_sequence, future_actions)
                    ...
                    # The control token and encoder target stay attached on both
                    # sides, exactly like the LeJEPA latent objective. There is no
                    # control EMA, detach, target normalization, or control SIGReg.
                    control_latent_error = F.mse_loss(
                        control_next_state,
                        one_step_target,
                        reduction="none",
                    ).mean(dim=(-1, -2))
                    wm_control_latent_loss = (
                        control_latent_error * one_step_valid
                    ).sum() / one_step_denom
                    control_reward_loss = -(
                        reward_target_probs * torch.log_softmax(control_reward_logits, dim=-1)
                    ).sum(dim=-1)
                    wm_control_reward_loss = (
                        control_reward_loss * one_step_valid
                    ).sum() / one_step_denom
                    control_termination_loss = F.binary_cross_entropy_with_logits(
                        control_continuation_logits,
                        1.0 - future_terminations,
                        reduction="none",
                    )
                    wm_control_termination_loss = (
                        control_termination_loss * one_step_valid
                    ).sum() / one_step_denom
                    wm_control_aux_loss = (
                        wm_control_latent_loss
                        + args.lewm_reward_loss_coef * wm_control_reward_loss
                        + args.lewm_termination_loss_coef * wm_control_termination_loss
                    )
                    ...
                    wm_loss = (
                        ...
                        + args.lewm_control_aux_coef * wm_control_aux_loss
                    ...
                    wm_control_latent_losses.append(wm_control_latent_loss.item())
                    wm_control_reward_losses.append(wm_control_reward_loss.item())
                    wm_control_termination_losses.append(wm_control_termination_loss.item())
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/control_next_mse", float(np.mean(wm_control_latent_losses)), global_step)
                writer.add_scalar("lewm/control_reward_ce", float(np.mean(wm_control_reward_losses)), global_step)
                writer.add_scalar("lewm/control_continuation_bce", float(np.mean(wm_control_termination_losses)), global_step)
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            for _ in range(args.imagine_batches):
                ...
                with torch.no_grad():
                    ...
                    def dream_previous_actions(action_history, batch_size, dtype):
                        sentinel = torch.zeros(
                            batch_size,
                        ...
                        return torch.stack([sentinel] + action_history, dim=1)

                    for t in range(H_im):
                        latent_hist = torch.stack(hist_latents, dim=1)               # (B, t+1, obs_tok, dim)
                        # Control owns the shifted inbound action sequence only.
                        # Outbound a_t is introduced after sampling, exclusively to
                        # transition generation and the control sufficiency probes.
                        previous_actions = dream_previous_actions(
                            hist_actions, B_im, latent_hist.dtype
                        )
                        control = agent.world_model.control_from_history(
                            latent_hist, previous_actions
                        )
                        agent_input = agent.agent_input_from_control(control)
                    ...
                    control_H = agent.world_model.control_from_history(
                        latent_hist, previous_actions
                    )
                    value_probs_H = torch.softmax(
                        agent.get_value_from_agent_input(
                            None, agent.agent_input_from_control(control_H)
                        )[:, 0],  # horizon 0 = V(s_H)
                        dim=-1,
                    )
