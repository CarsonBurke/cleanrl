# PPO + IterThink v210 affine-RMS SPO with KL-scaled Adam directions.
#
# Preserve v209's v198-lineage rollout, WM, critic, attached LeJEPA/SIGReg,
# detached affine-RMS beliefs, exact full-row SPO objective, and dedicated actor
# Adam. Take exactly one clipped full-batch Adam step per rollout, then treat its
# parameter delta as a search direction. Joint objective/Armijo/KL admission
# expands that direction geometrically through scale 64 and bisects the final
# feasible/infeasible bracket. A committed scale retains the one Adam candidate's
# moments; failure transactionally restores both parameters and moments.
#
# -----------------------------------------------------------------------------
# PPO + IterThink v209 affine-RMS full-batch SPO with projected Adam.
#
# Preserve v198's raw affine-RMS encoder, attached LeJEPA/SIGReg, action-free
# temporal WM, detached actor/critic queries, exact Beta policy, and persistent
# 4-D/64-step exploration code. Replace minibatch clipped PPO actor reuse with a
# full-rollout SPO objective. A dedicated persistent actor Adam proposes each
# step; exact full-rollout forward KL and objective tests either accept it,
# project it along the Adam step, or transactionally restore parameters and
# moments. Critic and WM retain their v198 AdamW training and never update actor
# parameters. Imagination is explicitly disabled.
#
# -----------------------------------------------------------------------------
...
class Args:
    ...
    spo_epsilon: float = 0.2         # quadratic SPO ratio penalty scale
    spo_chunk_size: int = 1024       # exact full-rollout sums accumulated in bounded chunks
    spo_projection_evals: int = 6    # bisection evaluations after an infeasible Adam step
    spo_armijo_coef: float = 0.1      # required fraction of local predicted ascent
    spo_expansion_max_scale: float = 64.0
    ...
    norm_adv_scope: str = "batch"       # v210: normalize once, then hold fixed for the SPO direction
    ...
    imagine_enable: bool = False      # v210: real full-rollout SPO only
    ...
    imagine_prompt_bank_rollouts: int = 0  # v210: do not retain unused dream prompt buffers
...
if __name__ == "__main__":
    ...
    assert args.norm_adv and args.norm_adv_scope == "batch", (
        "v210 normalizes plain real GAE exactly once over the full rollout"
    )
    ...
    assert args.explore_code_dim == 4 and args.explore_resample_steps == 64, (
        "v210 preserves v198's 4-D persistent code held for 64 environment steps"
    )
    assert not args.auto_entropy and args.ent_coef == 0.0, "v210 actor objective is SPO only"
    assert not args.imagine_enable, "v210 explicitly disables dreamed optimizer updates"
    assert args.update_epochs == 10 and args.num_minibatches == 32, (
        "v210 preserves v198's 10x32 critic update schedule"
    )
    assert args.actor_learning_rate == 3e-4
    assert args.actor_grad_clip == 0.25, "v210 preserves v198 actor gradient clipping"
    assert args.spo_epsilon == 0.2 and args.spo_chunk_size == 1024
    assert args.spo_projection_evals == 6 and 0.0 <= args.spo_armijo_coef <= 1.0
    assert args.spo_expansion_max_scale == 64.0
    ...
    assert actor_param_ids.isdisjoint(critic_param_ids), "v210 actor and critic must be disjoint"
    assert actor_param_ids.isdisjoint(wm_param_ids), "v210 actor must be detached from the WM optimizer"
    assert critic_param_ids.isdisjoint(wm_param_ids), "critic consumes detached WM memories"
    # Critic + WM preserve v198 AdamW behavior. Actor parameters are intentionally
    # absent: the persistent actor Adam below is the only writer to the policy.
    optimizer = optim.AdamW(
        [
            {"params": critic_params, "weight_decay": 0.0},
    ...
    actor_optimizer = optim.Adam(
        actor_params, lr=args.actor_learning_rate, eps=1e-5, weight_decay=0.0
    )
            ...
            state_values.append(
                {
                    ...
                    for key, value in actor_optimizer.state.get(p, {}).items()
                }
            )
    ...
    def restore_actor_optimizer_state(snapshot):
        ...
        for p, value, saved_state in zip(actor_params, param_values, state_values):
            p.copy_(value)
            current_state = actor_optimizer.state[p]
            current_state.clear()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Dedicated SPO advantage: plain real GAE standardized once over the full
        # rollout and then held immutable for every gradient/evaluation chunk.
        gae = advantages.reshape(-1)
        b_policy_adv = shape_advantage(gae, b_sigma, b_u, args, device)
        spo_adv = (b_policy_adv - b_policy_adv.mean()) / (b_policy_adv.std() + 1e-8)
        # Diagnostics: how different is the shaped advantage from raw GAE?
        az = (gae - gae.mean()) / (gae.std() + 1e-8)
        pz = (spo_adv - spo_adv.mean()) / (spo_adv.std() + 1e-8)
        adv_corr = (az * pz).mean().item()
        adv_sign_agree = (torch.sign(az) == torch.sign(pz)).float().mean().item()

        b_inds = np.arange(args.batch_size)

        def actor_logprob_and_beta(indices):
            alpha, beta = agent.get_beta_params_from_agent_input(
                b_agent_inputs[indices], b_explore_codes[indices]
            )
            dist = Beta(alpha, beta)
            return dist.log_prob(b_latent_zs[indices]).sum(dim=-1), alpha, beta

        @torch.no_grad()
        def evaluate_spo_policy():
            objective_sum = torch.zeros((), device=device, dtype=torch.float64)
            kl_sum = torch.zeros((), device=device, dtype=torch.float64)
            old_kl_sum = torch.zeros((), device=device, dtype=torch.float64)
            approx_kl_sum = torch.zeros((), device=device, dtype=torch.float64)
            finite_rows = 0
            ratio_parts = []
            for start in range(0, args.batch_size, args.spo_chunk_size):
                end = min(start + args.spo_chunk_size, args.batch_size)
                inds = slice(start, end)
                newlogprob, alpha, beta = actor_logprob_and_beta(inds)
                logratio = newlogprob - b_logprobs[inds]
                ratio = logratio.exp()
                adv = spo_adv[inds]
                spo_row = ratio * adv - adv.abs() * (ratio - 1.0).square() / (
                    2.0 * args.spo_epsilon
                )
                behavior_dist = Beta(b_behavior_alphas[inds], b_behavior_betas[inds])
                current_dist = Beta(alpha, beta)
                kl_row = torch.distributions.kl_divergence(
                    behavior_dist, current_dist
                ).sum(dim=-1)
                finite_rows += int(
                    (torch.isfinite(spo_row) & torch.isfinite(kl_row)).sum().item()
                )
                objective_sum += spo_row.double().sum()
                kl_sum += kl_row.double().sum()
                old_kl_sum += (-logratio).double().sum()
                approx_kl_sum += ((ratio - 1.0) - logratio).double().sum()
                ratio_parts.append(ratio.detach())
            ratios = torch.cat(ratio_parts)
            safe_ratios = torch.nan_to_num(
                ratios, nan=0.0, posinf=1e12, neginf=0.0
            )
            ratio_sum = safe_ratios.double().sum()
            ratio_sq_sum = safe_ratios.double().square().sum()
            return {
                "j": float((objective_sum / args.batch_size).item()),
                "kl": float((kl_sum / args.batch_size).item()),
                "old_approx_kl": float((old_kl_sum / args.batch_size).item()),
                "approx_kl": float((approx_kl_sum / args.batch_size).item()),
                "ratio_p95": float(torch.quantile(safe_ratios, 0.95).item()),
                "ratio_p99": float(torch.quantile(safe_ratios, 0.99).item()),
                "ratio_ess": float(
                    (
                        ratio_sum.square()
                        / (args.batch_size * ratio_sq_sum.clamp_min(1e-30))
                    ).item()
                ),
                "ratio_in_band": float(
                    ((safe_ratios - 1.0).abs() <= args.spo_epsilon)
                    .float()
                    .mean()
                    .item()
                ),
                "row_coverage": finite_rows / float(args.batch_size),
            }

        def accumulate_full_batch_spo_gradient():
            actor_optimizer.zero_grad(set_to_none=True)
            for start in range(0, args.batch_size, args.spo_chunk_size):
                end = min(start + args.spo_chunk_size, args.batch_size)
                inds = slice(start, end)
                newlogprob, _, _ = actor_logprob_and_beta(inds)
                ratio = (newlogprob - b_logprobs[inds]).exp()
                adv = spo_adv[inds]
                spo_row = ratio * adv - adv.abs() * (ratio - 1.0).square() / (
                    2.0 * args.spo_epsilon
                )
                loss_chunk = -spo_row.sum() / args.batch_size
                if not torch.isfinite(loss_chunk):
                    actor_optimizer.zero_grad(set_to_none=True)
                    return float("nan"), None
                loss_chunk.backward()
            # Preserve v198's actor max-norm before the persistent Adam consumes
            # the gradient. This is especially important for projected steps,
            # whose candidate moments are intentionally retained.
            ...
            for p in actor_params:
                ...
                grads.append(grad)
            return float(preclip_grad_norm.item()), grads

        @torch.no_grad()
        def interpolate_actor_parameters(previous, candidate, fraction):
            for p, p0, p1 in zip(actor_params, previous, candidate):
                p.copy_(p0 + fraction * (p1 - p0))

        # Exact v198 value-update volume: ten shuffled epochs by 32 minibatches.
        # The critic/WM AdamW contains no actor parameters; detached buffered
        # memories mean this value-only pass also cannot update the WM.
        ...
        # One actor Adam transaction per rollout. Adam supplies a preconditioned
        # base direction; exact real-rollout SPO/KL admission chooses its scale.
        initial_spo_eval = evaluate_spo_policy()
        actor_grad_norms = []
        exact_beta_kls = [initial_spo_eval["kl"]]
        actor_updates_used = 0
        actor_updates_rejected = 0
        spo_base_candidate_kl = initial_spo_eval["kl"]
        spo_expansion_max = 0.0
        spo_final_scale = 0.0
        spo_projected = False

        actor_grad_norm, actor_grads = accumulate_full_batch_spo_gradient()
        actor_grad_norms.append(actor_grad_norm)
        if actor_grads is None or not np.isfinite(actor_grad_norm):
            actor_updates_rejected = 1
            final_spo_eval = initial_spo_eval
        else:
            actor_snapshot = snapshot_actor_optimizer_state()
            previous_params = [p.detach().clone() for p in actor_params]
            previous_j = initial_spo_eval["j"]
            actor_optimizer.step()
            candidate_params = [p.detach().clone() for p in actor_params]
            predicted_gain = 0.0
            for grad, previous, candidate in zip(
                actor_grads, previous_params, candidate_params
            ):
                if grad is not None:
                    predicted_gain -= float(
                        (grad.double() * (candidate - previous).double()).sum().item()
                    )

            def jointly_admissible(policy_eval, scale):
                armijo_floor = (
                    previous_j
                    + args.spo_armijo_coef
                    * scale
                    * max(predicted_gain, 0.0)
                )
                return (
                    np.isfinite(policy_eval["kl"])
                    and policy_eval["kl"] <= args.target_kl
                    and np.isfinite(policy_eval["j"])
                    and policy_eval["j"] > previous_j
                    and policy_eval["j"] >= armijo_floor
                )

            # Scale 1 is the sole actual Adam step. Subsequent evaluations move
            # parameters along its fixed delta without touching optimizer state.
            base_candidate_eval = evaluate_spo_policy()
            spo_base_candidate_kl = base_candidate_eval["kl"]
            spo_expansion_max = 1.0
            exact_beta_kls.append(base_candidate_eval["kl"])
            best_scale = 0.0
            best_eval = None
            if jointly_admissible(base_candidate_eval, 1.0):
                best_scale = 1.0
                best_eval = base_candidate_eval
                lo = 1.0
                hi = None
                scale = 2.0
                while scale <= args.spo_expansion_max_scale:
                    interpolate_actor_parameters(
                        previous_params, candidate_params, scale
                    )
                    expanded_eval = evaluate_spo_policy()
                    spo_expansion_max = scale
                    exact_beta_kls.append(expanded_eval["kl"])
                    if jointly_admissible(expanded_eval, scale):
                        best_scale = scale
                        best_eval = expanded_eval
                        lo = scale
                        scale *= 2.0
                    else:
                        hi = scale
                ...
                # Even a KL-feasible base can fail the objective/Armijo test; a
                # smaller positive fraction may still be an admissible ascent.
                lo, hi = 0.0, 1.0

            # If expansion found a bad endpoint (or scale 1 was bad), refine the
            # jointly admissible boundary. When scale 64 is still good, it is the
            # configured maximum and already the largest allowed point.
            if hi is not None:
                for _ in range(args.spo_projection_evals):
                    scale = 0.5 * (lo + hi)
                    interpolate_actor_parameters(
                        previous_params, candidate_params, scale
                    )
                    scaled_eval = evaluate_spo_policy()
                    exact_beta_kls.append(scaled_eval["kl"])
                    if jointly_admissible(scaled_eval, scale):
                        lo = scale
                        best_scale = scale
            ...
            if best_eval is not None and best_scale > 0.0:
                interpolate_actor_parameters(
                    previous_params, candidate_params, best_scale
                )
                final_spo_eval = best_eval
                actor_updates_used = 1
                spo_final_scale = best_scale
                spo_projected = best_scale != 1.0
            else:
                restore_actor_optimizer_state(actor_snapshot)
                final_spo_eval = evaluate_spo_policy()
                actor_updates_rejected = 1

        exact_beta_kls.append(final_spo_eval["kl"])
        real_actor_active = bool(
            np.isfinite(final_spo_eval["kl"])
            and final_spo_eval["kl"] <= args.target_kl
        )
        old_approx_kl = torch.tensor(
            final_spo_eval["old_approx_kl"], device=device
        )
        approx_kl = torch.tensor(final_spo_eval["approx_kl"], device=device)
        pg_loss = torch.tensor(-final_spo_eval["j"], device=device)
        entropy_loss = torch.zeros((), device=device)
        clipfracs = [1.0 - final_spo_eval["ratio_in_band"]]

        # v210 has no dreamed data path: all actor acceptance statistics are
        # measured exactly on the real behavior rollout.
        ...
        writer.add_scalar("spo/accepted_steps", actor_updates_used, global_step)
        writer.add_scalar("spo/rejected_steps", actor_updates_rejected, global_step)
        writer.add_scalar("spo/projected", float(spo_projected), global_step)
        writer.add_scalar("spo/projection_fraction", spo_final_scale, global_step)
        writer.add_scalar("spo/final_scale", spo_final_scale, global_step)
        writer.add_scalar("spo/expansion_max", spo_expansion_max, global_step)
        writer.add_scalar(
            "spo/kl_utilization", final_spo_eval["kl"] / args.target_kl, global_step
        )
        writer.add_scalar("spo/objective_initial", initial_spo_eval["j"], global_step)
        writer.add_scalar("spo/objective_final", final_spo_eval["j"], global_step)
        writer.add_scalar(
            "spo/objective_improvement",
            final_spo_eval["j"] - initial_spo_eval["j"],
            global_step,
        )
        writer.add_scalar("spo/candidate_kl", spo_base_candidate_kl, global_step)
        writer.add_scalar("spo/base_candidate_kl", spo_base_candidate_kl, global_step)
        writer.add_scalar("spo/ratio_p95", final_spo_eval["ratio_p95"], global_step)
        writer.add_scalar("spo/ratio_p99", final_spo_eval["ratio_p99"], global_step)
        writer.add_scalar("spo/ratio_ess_fraction", final_spo_eval["ratio_ess"], global_step)
        writer.add_scalar("spo/ratio_in_band_fraction", final_spo_eval["ratio_in_band"], global_step)
        writer.add_scalar("spo/row_coverage", final_spo_eval["row_coverage"], global_step)
        writer.add_scalar(
            "spo/gradient_norm_last",
            actor_grad_norms[-1] if actor_grad_norms else 0.0,
            global_step,
        )
