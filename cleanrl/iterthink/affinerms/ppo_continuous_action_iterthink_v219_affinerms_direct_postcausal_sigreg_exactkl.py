...
#
# v218 failed by 256k: returns stayed -237 versus v213's +15.9, while working
# latent MSE collapsed 0.689 -> 0.0026 and projected SIGReg fell 6.51 -> 1.31.
# The disposable LeVJEPA projector could satisfy distribution matching off the
# control path while the unprojected temporal state converged to a trivially
# predictable representation. LeVJEPA avoids that split because invariance and
# SIGReg both act in projector space; our tokenwise dynamics MSE does not.
#
# v219 enforces the user's original placement directly: after every causal
# temporal block and predictor_norm, before transition/policy queries and before
# any projector. The longest valid action-free state row is flattened across all
# observation tokens and jointly SIGReg'd, so the actual recurrent/control
# representation must retain full-state diversity and cross-token rank. The
# disposable projector is removed completely. Direct working-space strength
# returns to the established LeWM coefficient 0.09. Exact effective/stable rank
# and per-coordinate spread of this same state population are logged each update.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    lewm_sigreg_coef: float = 0.09    # v219: direct working-state LeWM strength
