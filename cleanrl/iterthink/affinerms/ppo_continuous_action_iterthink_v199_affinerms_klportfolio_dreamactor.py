# PPO + IterThink v199 affine-RMS KL-portfolio dream actor.
#
# Keep v198's causal temporal-query WM and detached actor/critic query readouts.
# Real PPO receives a strict sub-budget of the total exact-Beta-KL trust region,
# reserving policy movement for model-generated experience. Each fresh dream block
# also stores its behavior Beta distribution and accepts an actor step only when
# both the fixed real-state anchor and its own dream-state anchor remain inside
# their exact KL budgets. This turns the previously inert imagination actor into
# a guaranteed trust-region participant without relaxing the total real-state KL.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    real_target_kl: float = 0.02     # v199: reserve one third of total real-anchor KL for dream policy updates
    dream_target_kl: float = 0.02    # exact KL to each freshly generated dream behavior distribution
...
if __name__ == "__main__":
    ...
    assert 0 < args.real_target_kl < args.target_kl
    assert args.dream_target_kl > 0
    ...
    for iteration in range(1, args.num_iterations + 1):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with torch.no_grad():
                    ...
                    if real_actor_active and (
                        not torch.isfinite(exact_beta_kl) or exact_beta_kl > args.real_target_kl
                    ):
                ...
                if args.separate_grad_clip:
                    ...
                    if attempted_actor_update:
                        ...
                        if (
                            ...
                            or post_step_beta_kl > args.real_target_kl
                        ):
        ...
        if (
            ...
            or post_real_anchor_beta_kl > args.real_target_kl
        ):
            real_actor_active = False
        # v199: exhausting the real sub-budget does not disable imagination.
        # Dreams may use the deliberately reserved headroom up to the unchanged
        # global real-state anchor budget.
        dream_actor_active = bool(
            torch.isfinite(post_real_anchor_beta_kl)
            and post_real_anchor_beta_kl <= args.target_kl
        )
        dream_start_kl_headroom = max(
            0.0, args.target_kl - float(post_real_anchor_beta_kl)
        )
        ...
        dream_actor_updates_attempted = 0
        dream_exact_beta_kls = []
        dream_attempted_exact_beta_kls = []
        final_anchor_beta_kl = post_real_anchor_beta_kl
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            for _ in range(args.imagine_batches):
                ...
                with torch.no_grad():
                    ...
                    ag_inputs, im_zs, im_lps, im_alphas, im_betas = [], [], [], [], []
                    im_rs, im_cs, im_vps = [], [], []
                    ...
                    for t in range(H_im):
                        ...
                        action, z, logprob, _, value_logits, im_alpha, im_beta = (
                            agent.get_action_and_value_from_agent_input(
                                None,
                                agent_input,
                                explore_code=im_explore_code,
                                return_beta_params=True,
                            )
                        )
                    ...
                    f_behavior_alpha = torch.stack(im_alphas, dim=0).reshape(H_im * B_im, -1)
                    f_behavior_beta = torch.stack(im_betas, dim=0).reshape(H_im * B_im, -1)
                ...
                dream_anchor_n = min(4096, f_ag.shape[0])
                dream_anchor_inds = torch.linspace(
                    0, f_ag.shape[0] - 1, dream_anchor_n, device=device
                ).long()

                @torch.no_grad()
                def measure_dream_anchor_beta_kl():
                    measured_alpha, measured_beta = agent.get_beta_params_from_agent_input(
                        f_ag[dream_anchor_inds], f_explore_code[dream_anchor_inds]
                    )
                    behavior_dist = Beta(
                        f_behavior_alpha[dream_anchor_inds], f_behavior_beta[dream_anchor_inds]
                    )
                    measured_dist = Beta(measured_alpha, measured_beta)
                    return torch.distributions.kl_divergence(
                        behavior_dist, measured_dist
                ...
                pre_dream_exact_kl = measure_dream_anchor_beta_kl()
                dream_exact_beta_kls.append(pre_dream_exact_kl.item())
                block_dream_actor_active = dream_actor_active and not (
                    not torch.isfinite(pre_dream_exact_kl)
                    or pre_dream_exact_kl > args.dream_target_kl
                )
                ...
                attempted_dream_actor_update = block_dream_actor_active
                dream_actor_updates_attempted += int(attempted_dream_actor_update)
                ...
                if args.separate_grad_clip:
                    ...
                    if not block_dream_actor_active:
                        for p in shared_trunk_params:
                            p.grad = None
                    ...
                    if attempted_dream_actor_update:
                        ...
                        post_dream_behavior_kl = measure_dream_anchor_beta_kl()
                        exact_beta_kls.append(post_dream_anchor_kl.item())
                        dream_attempted_exact_beta_kls.append(post_dream_behavior_kl.item())
                        global_violation = (
                            not torch.isfinite(post_dream_anchor_kl)
                            or post_dream_anchor_kl > args.target_kl
                        )
                        local_violation = (
                            not torch.isfinite(post_dream_behavior_kl)
                            or post_dream_behavior_kl > args.dream_target_kl
                        )
                        if global_violation or local_violation:
                            restore_actor_optimizer_state(dream_actor_snapshot)
                            # A local dream-anchor overshoot invalidates only this
                            # freshly generated block. Rollback restores the safe
                            # actor, so later fresh blocks may still use the reserved
                            # global KL headroom. A global violation stops the portfolio.
                            if global_violation:
                                dream_actor_active = False
                            restored_dream_kl = measure_dream_anchor_beta_kl()
                            dream_exact_beta_kls.append(restored_dream_kl.item())
                            dream_actor_updates_rejected += 1
                        else:
                            dream_exact_beta_kls.append(post_dream_behavior_kl.item())
        ...
        writer.add_scalar("losses/post_real_exact_beta_kl", post_real_anchor_beta_kl.item(), global_step)
        writer.add_scalar("losses/dream_start_kl_headroom", dream_start_kl_headroom, global_step)
        writer.add_scalar(
        ...
        writer.add_scalar(
            "losses/dream_actor_updates_attempted", dream_actor_updates_attempted, global_step
        )
        writer.add_scalar(
            "losses/dream_actor_updates_rejected", dream_actor_updates_rejected, global_step
        )
        writer.add_scalar(
            "losses/dream_exact_beta_kl",
            dream_exact_beta_kls[-1] if dream_exact_beta_kls else 0.0,
            global_step,
        )
        writer.add_scalar(
            "losses/dream_exact_beta_kl_max",
            max(dream_attempted_exact_beta_kls) if dream_attempted_exact_beta_kls else 0.0,
            global_step,
        )
        writer.add_scalar(
            "losses/real_trust_subbudget_exhausted", float(not real_actor_active), global_step
        )
        writer.add_scalar(
