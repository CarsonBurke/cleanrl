# PPO + IterThink v201 affine-RMS one-step model-gradient policy improvement.
#
# Preserve v198's observation encoder, encoder-side SIGReg, action-free temporal
# memory, outbound transition queries, real PPO, and exact-Beta-KL rollback. Add
# a deterministic one-step value-gradient objective on every eligible real actor
# minibatch: the current Beta mean action is differentiated through the learned
# transition, predicted reward/continuation, a T=1 next-state temporal pass, and
# the frozen critic. autograd.grad requests actor parameters only, so neither the
# world model nor critic receives this objective's gradients. Unlike imagined
# PPO, this directly teaches the policy how its action changes predicted return
# without fitting likelihood ratios to synthetic trajectories.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    # v201 replaces imagined actor PPO with the lower-bias one-step model
    # gradient. Keep the legacy dream code available for explicit diagnostics,
    # but do not train actor or critic on synthetic PPO data by default.
    imagine_enable: bool = True
    imagine_actor_enable: bool = False  # retain v198 dream critic, forbid the harmful synthetic actor gradient
    ...
    # v201: differentiable one-step policy improvement through the WM. Q is in
    # raw return units, so divide by the real rollout's return std (with a floor)
    # before applying a conservative coefficient. Its gradient is combined with
    # PPO and covered by the same actor clip and exact-KL rollback.
    model_grad_coef: float = 0.05
    # Wait until both the supervised transition and real-data critic are useful;
    # an early model gradient would optimize arbitrary initial extrapolation.
    model_grad_warmup_steps: int = 500000
    model_grad_return_scale_floor: float = 100.0
...
class LeWMBackbone(nn.Module):
    ...
    def model_grad_step_from_memory(self, state_memory, action):
        """One differentiable transition from already-buffered temporal memory.

        `state_memory` is v198's detached action-free current-frame memory. The
        outbound action is the sole actor-dependent input. The returned next
        memory deliberately takes the predicted observation tokens through the
        same action-free temporal trunk at T=1 before the critic reads them.
        Parameters remain ordinary trainable WM parameters for supervised WM
        updates; the caller isolates this path with autograd.grad(actor_params).
        """
        if state_memory.ndim != 3:
            raise ValueError("state_memory must have shape (batch, obs_tokens, dim)")
        action_features = self.action_cond(action).unsqueeze(1)
        features = self.transition_readout(
            state_memory.unsqueeze(1), action_features
        )[:, 0]
        next_summary = self.pred_next_proj(features)
        reward_logits, continuation_logits = self.decode_belief_outcomes(
            next_summary, action, detach_belief=False
        )
        next_obs_tokens = next_summary[:, self.obs_token_start :]
        next_memory = self.belief_from_history(next_obs_tokens.unsqueeze(1))
        return next_memory, reward_logits, continuation_logits
...
class Agent(nn.Module):
    ...
    def get_differentiable_value_from_agent_input(self, agent_input):
        """Evaluate V while retaining only its derivative with respect to input.

        This intentionally bypasses the normal agent-interface detach. The v201
        caller uses autograd.grad with actor parameters as the only requested
        leaves, making the critic a fixed differentiable function for this path.
        """
        critic_feat = self.critic_trunk(self.critic_readout(agent_input))
        return self.critic_head(critic_feat).view(
...
if __name__ == "__main__":
    ...
    assert args.actor_dist == "beta", "v201 model-gradient and exact-KL path requires the Beta actor"
    assert args.model_grad_coef >= 0.0
    assert args.model_grad_return_scale_floor > 0.0
    ...
    def one_step_model_gradient_terms(agent_input, alpha, beta, return_scale):
        """Build v201's differentiable Q1 objective without detaching the action.

        The rollout memory itself is detached. WM and critic parameters appear as
        fixed functions in this graph; only actor leaves are later requested from
        autograd.grad. The Beta mean avoids sampling noise and gives both
        concentration heads a smooth path to predicted one-step return.
        """
        native_mean = alpha / (alpha + beta)
        mean_action = agent.action_low + (agent.action_high - agent.action_low) * native_mean
        state_memory = agent_input.reshape(
            agent_input.shape[0], agent.world_model.num_obs_tokens, agent.world_model.dim
        ).detach()
        next_memory, reward_logits, continuation_logits = (
            agent.world_model.model_grad_step_from_memory(state_memory, mean_action)
        )
        next_agent_input = next_memory.reshape(next_memory.shape[0], -1)
        next_value_logits = agent.get_differentiable_value_from_agent_input(
            next_agent_input
        )[:, 0]
        predicted_reward = reward_logits_to_scalar(reward_logits)
        predicted_continuation = torch.sigmoid(continuation_logits)
        predicted_next_value = value_logits_to_scalar(next_value_logits)
        # Continuation is a discount/confidence gate, not an action-value channel.
        # HalfCheetah's target is essentially constant one; detaching the gate
        # prevents the actor from exploiting its small calibration errors while
        # preserving action gradients through reward and next-state value.
        predicted_q1 = (
            predicted_reward
            + args.gamma * predicted_continuation.detach() * predicted_next_value
        )
        loss = -args.model_grad_coef * predicted_q1.mean() / return_scale
        return (
            loss,
            predicted_q1,
            predicted_reward,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        b_u = u.reshape(-1)
        # Q1 has raw reward/return units. A rollout-wide detached scale keeps the
        # auxiliary gradient comparable across learning phases without changing
        # its action-gradient direction inside a minibatch.
        model_grad_return_scale = b_returns.detach().std().clamp_min(
            args.model_grad_return_scale_floor
        )
        ...
        model_grad_q1s = []
        model_grad_rewards = []
        model_grad_continuations = []
        model_grad_next_values = []
        model_grad_action_magnitudes = []
        model_grad_norms = []
        model_grad_ppo_cosines = []
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                use_model_grad = (
                    attempted_actor_update
                    and args.model_grad_coef > 0.0
                    and global_step >= args.model_grad_warmup_steps
                )
                model_grad_loss = None
                if use_model_grad:
                    (
                        model_grad_loss,
                        model_q1,
                        ...
                        model_next_value,
                        model_mean_action,
                    ) = one_step_model_gradient_terms(
                        b_agent_inputs[mb_inds],
                        current_alpha,
                        current_beta,
                        model_grad_return_scale,
                    )
                    model_grad_losses.append(model_grad_loss.detach().item())
                    model_grad_q1s.append(model_q1.detach().mean().item())
                    model_grad_rewards.append(model_reward.detach().mean().item())
                    model_grad_continuations.append(
                        model_continuation.detach().mean().item()
                    )
                    model_grad_next_values.append(model_next_value.detach().mean().item())
                    model_grad_action_magnitudes.append(
                        model_mean_action.detach().abs().mean().item()
                    )
                if args.separate_grad_clip:
                    ...
                    if attempted_actor_update:
                        model_grads = []
                        if use_model_grad:
                            raw_model_grads = torch.autograd.grad(
                                model_grad_loss,
                                actor_params,
                                retain_graph=True,
                                allow_unused=False,
                            ...
                            model_grads = [
                                (p, g.detach())
                                for p, g in zip(actor_params, raw_model_grads)
                                if g is not None
                            ]
                            # autograd.grad must not populate parameter .grad on
                            # either fixed differentiable subsystem.
                            assert all(p.grad is None for p in world_model_params)
                            assert all(p.grad is None for p in critic_params)
                            model_gn = torch.sqrt(
                                sum(g.float().square().sum() for _, g in model_grads)
                            )
                            model_grad_norms.append(model_gn.item())
                        (pg_loss - ent_coef_eff * entropy_loss).backward()
                        if model_grads:
                            ppo_sq = torch.zeros((), device=device)
                            model_sq = torch.zeros((), device=device)
                            dot = torch.zeros((), device=device)
                            for p, g in model_grads:
                                if p.grad is not None:
                                    pg = p.grad.detach().float()
                                    gf = g.float()
                                    ppo_sq += pg.square().sum()
                                    model_sq += gf.square().sum()
                                    dot += (pg * gf).sum()
                                p.grad = g if p.grad is None else p.grad + g
                            cosine = dot / (
                                ppo_sq.sqrt() * model_sq.sqrt()
                            ).clamp_min(1e-12)
                            model_grad_ppo_cosines.append(cosine.item())
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            for _ in range(args.imagine_batches):
                ...
                attempted_dream_actor_update = args.imagine_actor_enable and real_actor_active
        ...
        writer.add_scalar("model_grad/coef", args.model_grad_coef, global_step)
        writer.add_scalar(
            "model_grad/return_scale", model_grad_return_scale.item(), global_step
        )
        writer.add_scalar(
            "model_grad/loss",
            float(np.mean(model_grad_losses)) if model_grad_losses else 0.0,
            global_step,
        )
        writer.add_scalar(
            "model_grad/predicted_q1",
            float(np.mean(model_grad_q1s)) if model_grad_q1s else 0.0,
            global_step,
        )
        writer.add_scalar(
            "model_grad/predicted_reward",
            float(np.mean(model_grad_rewards)) if model_grad_rewards else 0.0,
            global_step,
        )
        writer.add_scalar(
            "model_grad/predicted_continuation",
            float(np.mean(model_grad_continuations)) if model_grad_continuations else 0.0,
            global_step,
        )
        writer.add_scalar(
            "model_grad/predicted_next_value",
            float(np.mean(model_grad_next_values)) if model_grad_next_values else 0.0,
            global_step,
        )
        writer.add_scalar(
            "model_grad/mean_action_abs",
            float(np.mean(model_grad_action_magnitudes))
            if model_grad_action_magnitudes
            else 0.0,
            global_step,
        )
        writer.add_scalar(
            "model_grad/grad_norm",
            float(np.mean(model_grad_norms)) if model_grad_norms else 0.0,
            global_step,
        )
        writer.add_scalar(
            "model_grad/ppo_grad_cosine",
            float(np.mean(model_grad_ppo_cosines))
            if model_grad_ppo_cosines
            else 0.0,
            global_step,
        )
        if wm_losses:
