# PPO + IterThink v208 phase-option exploration + exact-Beta TRPO.
#
# Replace v207's piecewise-constant random code with a persistent 128-step plan:
# sampled coefficients modulate deterministic DC and sin/cos harmonics at periods
# 16/32/64, augmented by phase and remaining-age coordinates. The critic receives
# the complete Markov plan feature through its own zero-init projection. The WM is
# unchanged and strictly detached; v207's full-rollout exact-KL TRPO remains.
#
# Preserve v198's raw affine-RMS encoder, attached LeJEPA/SIGReg world model,
# action-free causal belief, strict detached and separate actor/critic paths,
# persistent Beta exploration, real GAE, and categorical MTP critic. Replace
# Adam/PPO actor minibatches with one rollout-wide natural-gradient step: exact
# Beta KL curvature, conjugate gradient, and an exact-KL backtracking line search.
# Critic and WM retain their Adam schedules; synthetic actor/critic updates are off.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    explore_code_dim: int = 9        # DC + 3 sin/cos pairs + remaining age + phase
    explore_resample_steps: int = 128
    ...
    imagine_prompt_bank_rollouts: int = 0

    trpo_cg_iters: int = 10
    trpo_cg_tolerance: float = 1e-10
    trpo_damping: float = 0.1
    trpo_anchor_size: int = 2048
    trpo_surrogate_chunk_size: int = 1024
    trpo_backtrack_iters: int = 10
    trpo_backtrack_coeff: float = 0.5
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Preserve v207 actor/critic initialization lineage: constructing this
        # zero-init addition must not advance the main parameter RNG stream.
        main_rng_state_plan = torch.random.get_rng_state()
        self.critic_plan_proj = nn.Linear(args.explore_code_dim, args.lewm_dim, bias=False)
        nn.init.zeros_(self.critic_plan_proj.weight)
        torch.random.set_rng_state(main_rng_state_plan)
    ...
    def _critic_feat_from_agent_input(self, agent_input, explore_code=None):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        feat = self.critic_readout(agent_input)
        if explore_code is not None:
            feat = feat + self.critic_plan_proj(explore_code)
        return self.critic_trunk(feat)
    ...
    def get_beta_logprob_from_agent_input(self, agent_input, z, explore_code):
        actor_feat = self._actor_feat_from_agent_input(agent_input)
        dist, _, _ = self._actor_dist(actor_feat, explore_code)
        logprob = dist.log_prob(z).sum(dim=-1)
        return logprob, dist.concentration1, dist.concentration0
    ...
    def get_action_and_value_from_agent_input(
        ...
        actor_feat = self._actor_feat_from_agent_input(agent_input)
        critic_feat = self._critic_feat_from_agent_input(
            agent_input, explore_code if critic_plan_state is None else critic_plan_state
        )
    ...
    def critic_parameters(self):
        ...
        return (
            ...
            + list(self.critic_plan_proj.parameters())
...
if __name__ == "__main__":
    ...
    assert not args.imagine_enable, "v207 forbids imagined actor/critic optimizer updates"
    assert 0 < args.trpo_anchor_size <= args.batch_size
    assert args.trpo_cg_iters > 0 and args.trpo_damping > 0
    ...
    assert not ({id(p) for p in critic_params} & actor_param_ids), (
        "v207 actor and critic paths must be disjoint"
    )
    ...
    non_wm_params = [
        ...
        if id(p) not in wm_param_ids and id(p) not in actor_param_ids
    ]
    ...
    assert all(id(p) not in actor_param_ids for group in optimizer.param_groups for p in group["params"])
    ...
    critic_plan_states = torch.zeros_like(explore_codes)
    next_transition_plan_states = torch.zeros_like(explore_codes)
    ...
    plan_coeff = torch.randn(args.num_envs, 7, device=device, generator=explore_generator)
    plan_age = torch.zeros(args.num_envs, device=device, dtype=torch.long)
    plan_resets_total = 0
    imagine_prompt_bank = []

    def phase_plan_features(coeff, age):
        age_f = age.to(torch.float32)
        cols = [coeff[:, 0]]
        ci = 1
        for period in (16.0, 32.0, 64.0):
            angle = 2.0 * torch.pi * age_f / period
            cols.extend([
                (2.0 ** 0.5) * coeff[:, ci] * torch.sin(angle),
                (2.0 ** 0.5) * coeff[:, ci + 1] * torch.cos(angle),
            ])
            ci += 2
        cols.extend([
            1.0 - age_f / float(args.explore_resample_steps - 1),
            2.0 * age_f / float(args.explore_resample_steps - 1) - 1.0,
        ])
        return torch.stack(cols, dim=-1)

    def raw_plan_state(coeff, age):
        age_f = age.to(torch.float32)
        return torch.cat([
            coeff,
            (1.0 - age_f / float(args.explore_resample_steps - 1)).unsqueeze(-1),
            (2.0 * age_f / float(args.explore_resample_steps - 1) - 1.0).unsqueeze(-1),
        ], dim=-1)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            current_explore_code = phase_plan_features(plan_coeff, plan_age)
            current_plan_state = raw_plan_state(plan_coeff, plan_age)
            global_step += args.num_envs
            obs[step] = next_obs
            dones[step] = next_done
            explore_codes[step] = current_explore_code
            critic_plan_states[step] = current_plan_state
            ...
            with torch.no_grad():
                ...
                if not rollout_actor_active and args.actor_dist == "beta":
                    ...
                    value_logits = agent.get_value_from_agent_input(next_obs, agent_input, current_plan_state)
                else:
                    (
                        action,
                        z,
                        ...
                        critic_plan_state=current_plan_state,
                    )
            ...
            successor_age = plan_age + 1
            successor_coeff = plan_coeff.clone()
            segment_reset = successor_age >= args.explore_resample_steps
            if bool(segment_reset.any()):
                successor_age = successor_age.clone()
                successor_age[segment_reset] = 0
                successor_coeff[segment_reset] = torch.randn(
                    int(segment_reset.sum()), 7, device=device, generator=explore_generator
                )
                plan_resets_total += int(segment_reset.sum())
            # A truncation's final observation receives the advanced old plan;
            # actual reset observations below receive a newly sampled plan.
            next_transition_explore_codes[step] = phase_plan_features(
                successor_coeff, successor_age
            )
            next_transition_plan_states[step] = raw_plan_state(successor_coeff, successor_age)
            ...
            if bool(boundary_tensor.any()):
                ...
                successor_age[boundary_tensor] = 0
                successor_coeff[boundary_tensor] = torch.randn(
                    int(boundary_tensor.sum()), 7, device=device, generator=explore_generator
                )
                plan_resets_total += int((boundary_tensor & (~segment_reset)).sum())
            plan_age = successor_age
            plan_coeff = successor_coeff
        ...
        with torch.no_grad():
            next_transition_value_logits = agent.get_value_from_agent_input(
                ...
                next_transition_plan_states.reshape(-1, args.explore_code_dim),
            )[:, 0]  # horizon 0 = V(s')
        ...
        b_critic_plan_states = critic_plan_states.reshape(-1, args.explore_code_dim)
        ...
        trust_anchor_n = min(args.trpo_anchor_size, args.batch_size)
        # Deterministic time×env stratification avoids flatten-stride aliases and
        # covers every environment plus all persistent-code time segments.
        anchor_times_n = max(1, trust_anchor_n // args.num_envs)
        anchor_times = np.linspace(
            0, args.num_steps - 1, anchor_times_n, dtype=np.int64
        )
        anchor_grid = (
            anchor_times[:, None] * args.num_envs
            + np.arange(args.num_envs, dtype=np.int64)[None, :]
        ).reshape(-1)
        trust_anchor_inds = torch.as_tensor(
            anchor_grid[:trust_anchor_n], device=device, dtype=torch.long
        )

        # ------------------------ rollout-wide exact-Beta TRPO ------------------------
        # Raw real GAE, normalized once over the full rollout. The surrogate gradient
        # is accumulated in deterministic chunks but is mathematically the full-batch
        # mean over all 32768 transitions.
        trpo_adv = (b_advantages - b_advantages.mean()) / (b_advantages.std() + 1e-8)
        actor_numel = sum(p.numel() for p in actor_params)

        # Pre-TRPO behavior-policy diagnostics (never confounded by the update).
        with torch.no_grad():
            useful_shift_pre = torch.empty(args.batch_size, device=device)
            mean_shift_pre = torch.empty_like(b_latent_zs)
            for shift_start in range(0, args.batch_size, args.trpo_surrogate_chunk_size):
                shift_end = min(shift_start + args.trpo_surrogate_chunk_size, args.batch_size)
                sl = slice(shift_start, shift_end)
                zero_code = torch.zeros_like(b_explore_codes[sl])
                log_zero, _, _ = agent.get_beta_logprob_from_agent_input(
                    b_agent_inputs[sl], b_latent_zs[sl], zero_code
                )
                useful_shift_pre[sl] = trpo_adv[sl] * (b_logprobs[sl] - log_zero)
                feat = agent._actor_feat_from_agent_input(b_agent_inputs[sl])
                dist_struct, to_action_shift, _ = agent._actor_dist(feat, b_explore_codes[sl])
                dist_zero_shift, _, _ = agent._actor_dist(feat, None)
                mean_shift_pre[sl] = (
                    to_action_shift(dist_struct.mean) - to_action_shift(dist_zero_shift.mean)
                )
            useful_env = useful_shift_pre.reshape(args.num_steps, args.num_envs).mean(dim=0)
            useful_shift_mean = useful_env.mean()
            useful_shift_ci95 = 2.131 * useful_env.std(unbiased=True) / (args.num_envs ** 0.5)
            shift_time = mean_shift_pre.reshape(args.num_steps, args.num_envs, -1)
            phase_time = critic_plan_states[..., -1]
            complete_blocks = []
            expected_phase = torch.linspace(-1.0, 1.0, args.explore_resample_steps, device=device)
            candidate_starts = torch.nonzero(
                (phase_time + 1.0).abs() < 1e-6, as_tuple=False
            ).cpu().tolist()
            for block_start, env_idx in candidate_starts:
                if block_start + args.explore_resample_steps > args.num_steps:
                    continue
                phase_block = phase_time[
                    block_start : block_start + args.explore_resample_steps, env_idx
                ]
                if torch.allclose(phase_block, expected_phase, atol=1e-5, rtol=0):
            ...
            if complete_blocks:
                spectrum = torch.fft.rfft(torch.stack(complete_blocks), dim=1).abs().square()
                total_non_dc = spectrum[:, 1:].sum().clamp_min(1e-12)
                designed_harmonic_fraction = spectrum[:, [2, 4, 8]].sum() / total_non_dc
                dc_spectral_power = spectrum[:, 0].mean()
            else:
                designed_harmonic_fraction = torch.zeros((), device=device)
                dc_spectral_power = torch.zeros((), device=device)

        def flatten_tensors(tensors):
            return torch.cat([t.reshape(-1) for t in tensors])

        def flatten_actor_grads(grads):
            return torch.cat([
                (torch.zeros_like(param) if grad is None else grad).reshape(-1)
                for param, grad in zip(actor_params, grads)
            ])

        def actor_parameter_vector():
            return torch.nn.utils.parameters_to_vector(actor_params).detach().clone()

        @torch.no_grad()
        def set_actor_parameter_vector(vector):
            torch.nn.utils.vector_to_parameters(vector, actor_params)

        def surrogate_gradient():
            grad = torch.zeros(actor_numel, device=device)
            chunk = args.trpo_surrogate_chunk_size
            for start in range(0, args.batch_size, chunk):
                inds = slice(start, min(start + chunk, args.batch_size))
                new_logprob, _, _ = agent.get_beta_logprob_from_agent_input(
                    b_agent_inputs[inds], b_latent_zs[inds], b_explore_codes[inds]
                )
                objective = (
                    torch.exp(new_logprob - b_logprobs[inds]) * trpo_adv[inds]
                ).sum() / args.batch_size
                chunk_grad = torch.autograd.grad(
                    objective, actor_params, allow_unused=True
                )
                grad.add_(flatten_actor_grads(chunk_grad).detach())
            return grad
        ...
        def evaluate_surrogate():
            ...
            return total / args.batch_size

        def anchor_exact_kl(create_graph=False):
            current_alpha, current_beta = agent.get_beta_params_from_agent_input(
                b_agent_inputs[trust_anchor_inds], b_explore_codes[trust_anchor_inds]
            )
            behavior = Beta(
                b_behavior_alphas[trust_anchor_inds], b_behavior_betas[trust_anchor_inds]
            )
            current = Beta(current_alpha, current_beta)
            kl = torch.distributions.kl_divergence(behavior, current).sum(dim=-1).mean()
            return kl

        @torch.no_grad()
        def evaluate_full_exact_kl():
            total = torch.zeros((), device=device)
            chunk = args.trpo_surrogate_chunk_size
            for start in range(0, args.batch_size, chunk):
                inds = slice(start, min(start + chunk, args.batch_size))
                current_alpha, current_beta = agent.get_beta_params_from_agent_input(
                    b_agent_inputs[inds], b_explore_codes[inds]
                )
                behavior = Beta(b_behavior_alphas[inds], b_behavior_betas[inds])
                current = Beta(current_alpha, current_beta)
                total += torch.distributions.kl_divergence(
                    behavior, current
                ).sum(dim=-1).sum()
            return total / args.batch_size

        def fisher_vector_product(vector, add_damping=True):
            # Flash/memory-efficient SDPA kernels may not implement double
            # backward through the actor query attention. Fisher products force
            # the CUDA math kernel; surrogate and line-search forwards stay fast.
            with torch.backends.cuda.sdp_kernel(
                enable_flash=False, enable_math=True, enable_mem_efficient=False
            ):
                kl = anchor_exact_kl(create_graph=True)
                kl_grad = torch.autograd.grad(
                    kl, actor_params, create_graph=True, allow_unused=True
                )
                directional = (flatten_actor_grads(kl_grad) * vector).sum()
                hvp = torch.autograd.grad(
                    directional, actor_params, allow_unused=True
                )
            result = flatten_actor_grads(hvp).detach()
            if add_damping:
                result = result + args.trpo_damping * vector
            return result

        def conjugate_gradient(rhs):
            x = torch.zeros_like(rhs)
            residual = rhs.clone()
            direction = residual.clone()
            residual_sq = torch.dot(residual, residual)
            initial_residual = residual_sq.sqrt()
            iterations = 0
            for cg_iter in range(args.trpo_cg_iters):
                f_direction = fisher_vector_product(direction)
                denom = torch.dot(direction, f_direction)
                if not torch.isfinite(denom) or denom <= 1e-12:
                    break
                alpha = residual_sq / denom
                x = x + alpha * direction
                new_residual = residual - alpha * f_direction
                new_residual_sq = torch.dot(new_residual, new_residual)
                iterations = cg_iter + 1
                if new_residual_sq <= args.trpo_cg_tolerance:
                    residual = new_residual
                    residual_sq = new_residual_sq
                    break
                beta_cg = new_residual_sq / residual_sq.clamp_min(1e-20)
                direction = new_residual + beta_cg * direction
                residual = new_residual
                residual_sq = new_residual_sq
            relative_residual = residual_sq.sqrt() / initial_residual.clamp_min(1e-12)
            return x, relative_residual, iterations

        old_actor_vector = actor_parameter_vector()
        old_surrogate = evaluate_surrogate()
        policy_gradient = surrogate_gradient()
        natural_direction, trpo_cg_residual, trpo_cg_iterations = conjugate_gradient(
            policy_gradient
        )
        # CG solves the damped system for numerical stability, while the step
        # scale uses the undamped exact-KL curvature. The line search remains the
        # authoritative feasibility test.
        trpo_curvature = torch.dot(
            natural_direction,
            fisher_vector_product(natural_direction, add_damping=False),
        )
        trpo_predicted_improvement = torch.zeros((), device=device)
        trpo_actual_improvement = torch.zeros((), device=device)
        trpo_accepted_fraction = 0.0
        trpo_backtracks = args.trpo_backtrack_iters
        trpo_accepted = False
        if (
            torch.isfinite(policy_gradient).all()
            and torch.isfinite(natural_direction).all()
            and torch.isfinite(trpo_curvature)
            and trpo_curvature > 1e-12
        ):
            scale = torch.sqrt(2.0 * args.target_kl / trpo_curvature)
            full_step = natural_direction * scale
            trpo_predicted_improvement = torch.dot(policy_gradient, full_step)
            if torch.isfinite(trpo_predicted_improvement) and trpo_predicted_improvement > 0:
                for backtrack in range(args.trpo_backtrack_iters):
                    fraction = args.trpo_backtrack_coeff ** backtrack
                    set_actor_parameter_vector(old_actor_vector + fraction * full_step)
                    candidate_surrogate = evaluate_surrogate()
                    candidate_kl = evaluate_full_exact_kl()
                    improvement = candidate_surrogate - old_surrogate
                    if (
                        torch.isfinite(candidate_surrogate)
                        and torch.isfinite(candidate_kl)
                        and candidate_kl <= args.target_kl
                        and improvement
                        >= 0.1 * fraction * trpo_predicted_improvement
                    ):
                        trpo_actual_improvement = improvement
                        trpo_accepted_fraction = fraction
                        trpo_backtracks = backtrack
                        trpo_accepted = True
                        if backtrack > 0:
                            lower = fraction
                            upper = args.trpo_backtrack_coeff ** (backtrack - 1)
                            for _ in range(6):
                                trial = 0.5 * (lower + upper)
                                set_actor_parameter_vector(old_actor_vector + trial * full_step)
                                trial_surrogate = evaluate_surrogate()
                                trial_kl = evaluate_full_exact_kl()
                                trial_improvement = trial_surrogate - old_surrogate
                                feasible = bool(
                                    torch.isfinite(trial_surrogate)
                                    and torch.isfinite(trial_kl)
                                    and trial_kl <= args.target_kl
                                    and trial_improvement
                                    >= 0.1 * trial * trpo_predicted_improvement
                                )
                                if feasible:
                                    lower = trial
                                    trpo_accepted_fraction = trial
                                    trpo_actual_improvement = trial_improvement
                                else:
                                    upper = trial
                            set_actor_parameter_vector(
                                old_actor_vector + trpo_accepted_fraction * full_step
                            )
                        break
        if not trpo_accepted:
            set_actor_parameter_vector(old_actor_vector)
        post_real_anchor_beta_kl = evaluate_full_exact_kl()
        exact_beta_kls = [post_real_anchor_beta_kl.item()]
        actor_grad_norms = [policy_gradient.norm().item()]
        actor_updates_used = int(trpo_accepted)
        actor_updates_rejected = int(not trpo_accepted)
        real_actor_active = trpo_accepted
        clipfracs = []

        # Critic keeps v198's exact 10x32 Adam minibatch schedule. Actor parameters
        # are absent from Adam and actor computation is absent from this loop.
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                value_logits = agent.get_value_from_agent_input(
                    b_obs[mb_inds], b_agent_inputs[mb_inds], b_critic_plan_states[mb_inds]
                )
                ...
                optimizer.step()
        actor_gn = policy_gradient.norm()
        with torch.no_grad():
            final_surrogate = evaluate_surrogate()
            _, diag_alpha, diag_beta = agent.get_beta_logprob_from_agent_input(
                b_agent_inputs[trust_anchor_inds],
                b_latent_zs[trust_anchor_inds],
                b_explore_codes[trust_anchor_inds],
            )
            entropy_loss = Beta(diag_alpha, diag_beta).entropy().sum(dim=-1).mean()
        pg_loss = -final_surrogate
        old_approx_kl = post_real_anchor_beta_kl
        approx_kl = post_real_anchor_beta_kl
        clipfracs = [0.0]
        ...
        writer.add_scalar("explore/useful_shift", useful_shift_mean.item(), global_step)
        writer.add_scalar("explore/useful_shift_env_block_ci95", useful_shift_ci95.item(), global_step)
        writer.add_scalar("explore/dc_spectral_power", dc_spectral_power.item(), global_step)
        writer.add_scalar(
            "explore/designed_harmonic_rfft_fraction",
            designed_harmonic_fraction.item(),
            global_step,
        )
        writer.add_scalar("explore/plan_resets_total", plan_resets_total, global_step)
        ...
        writer.add_scalar(
            "losses/trust_region_exhausted",
            float(post_real_anchor_beta_kl >= args.target_kl),
            global_step,
        )
        writer.add_scalar(
            "losses/actor_update_fraction",
            float(actor_updates_used),
            global_step,
        )
        writer.add_scalar("losses/clipfrac", np.mean(clipfracs), global_step)
        writer.add_scalar("trpo/cg_relative_residual", trpo_cg_residual.item(), global_step)
        writer.add_scalar("trpo/cg_iterations", trpo_cg_iterations, global_step)
        writer.add_scalar("trpo/exact_curvature", trpo_curvature.item(), global_step)
        writer.add_scalar(
            "trpo/predicted_surrogate_improvement",
            trpo_predicted_improvement.item(),
            global_step,
        )
        writer.add_scalar(
            "trpo/actual_surrogate_improvement",
            trpo_actual_improvement.item(),
            global_step,
        )
        writer.add_scalar("trpo/accepted_fraction", trpo_accepted_fraction, global_step)
        writer.add_scalar("trpo/backtracks", trpo_backtracks, global_step)
        writer.add_scalar("trpo/accepted", float(trpo_accepted), global_step)
        writer.add_scalar("trpo/line_failed", float(not trpo_accepted), global_step)
        writer.add_scalar("trpo/exact_beta_kl", post_real_anchor_beta_kl.item(), global_step)
        writer.add_scalar(
            "trpo/kl_utilization",
            post_real_anchor_beta_kl.item() / args.target_kl,
            global_step,
        )
