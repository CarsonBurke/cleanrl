# PPO + IterThink v194 affine-RMSNorm joint-direct-SIGReg k1.
#
# v194 fixes v193's cross-token redundancy: observation tokens are flattened per
# frame and regularized jointly, so SIGReg sees and penalizes their cross-covariance.
# Outcome tokens remain separate because reward/continuation should correlate with
# state. SIGReg and predictive MSE act directly on the attached working latent; the
# PPO agent alone is stop-gradient isolated. Raw observations, affine RMSNorm, and
# the single genuinely trained next-state head are retained.
#
# -----------------------------------------------------------------------------
...
class Args:
    ...
    lewm_mtp_len: int = 1  # only the genuinely trained next-state projection
    ...
    lewm_sigreg_num_proj: int = 4096  # joint obs space is 17*64D; dense random slicing matters
    lewm_sigreg_knots: int = 17
    lewm_sigreg_min_valid: int = 32
    lewm_sigreg_ref_n: int = 128  # preserve le-wm's estimator and coefficient calibration
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if not self.detach_world_model_from_agent:
            raise ValueError("v194 requires a strictly detached world-model/agent interface")
...
if __name__ == "__main__":
    ...
    assert args.lewm_mtp_len == 1, "v194 deliberately supports only the trained one-step WM head"
    assert args.obs_cond_mode == "none", "v194 isolates the detached belief-only agent interface"
    ...
    def masked_token_sigreg_static(token_latents, token_valids, A_obs, A_out):
        # Observation slots are one joint random vector, preventing the encoder from
        # satisfying every marginal with duplicated tokens. Outcome slots are kept
        # separate: reward and continuation are allowed to remain functions of state.
        os = agent.world_model.obs_token_start
        obs_loss = _sigreg_group_static(token_latents[:, :, os:, :], token_valids, A_obs)
        outcome_losses = torch.stack(
            [
                _sigreg_group_static(token_latents[:, :, i : i + 1, :], token_valids, A_out)
                for i in range(os)
            ]
        )
        return (
            agent.world_model.num_obs_tokens * obs_loss + outcome_losses.sum()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for _ in range(args.lewm_update_epochs):
            ...
            for start in range(0, args.batch_size, wm_minibatch_size):
                ...
                # v194: joint observation-token sketches expose cross-token redundancy;
                # outcome tokens use a separate D-dimensional sketch basis.
