...
class Args:
    ...
    norm_adv_scope: str = "batch"       # v209: normalize once, then hold fixed for all SPO steps
    ...
    imagine_enable: bool = False      # v209: real full-rollout SPO only
    ...
    imagine_prompt_bank_rollouts: int = 0  # v209: do not retain unused dream prompt buffers
...
if __name__ == "__main__":
    ...
    assert args.norm_adv and args.norm_adv_scope == "batch", (
        "v209 normalizes plain real GAE exactly once over the full rollout"
    )
    ...
    assert args.explore_code_dim == 4 and args.explore_resample_steps == 64, (
        "v209 preserves v198's 4-D persistent code held for 64 environment steps"
    )
    assert not args.auto_entropy and args.ent_coef == 0.0, "v209 actor objective is SPO only"
    assert not args.imagine_enable, "v209 explicitly disables dreamed optimizer updates"
    assert args.update_epochs == 10 and args.num_minibatches == 32, (
        "v209 preserves v198's 10x32 critic update schedule"
    )
    assert args.actor_learning_rate == 3e-4
    assert args.actor_grad_clip == 0.25, "v209 preserves v198 actor gradient clipping"
    ...
    assert actor_param_ids.isdisjoint(critic_param_ids), "v209 actor and critic must be disjoint"
    assert actor_param_ids.isdisjoint(wm_param_ids), "v209 actor must be detached from the WM optimizer"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Each actor step is transactional. The full candidate is accepted only
        # when its exact rollout objective improves and forward KL is feasible.
        # An infeasible candidate is projected along the Adam parameter step; its
        # new moments are retained only when a projected point is committed.
        initial_spo_eval = evaluate_spo_policy()
        current_spo_eval = initial_spo_eval
        actor_grad_norms = []
        exact_beta_kls = [initial_spo_eval["kl"]]
        actor_updates_used = 0
        actor_updates_rejected = 0
        spo_projection_fraction = 0.0
        spo_projected = False
        spo_last_candidate_kl = initial_spo_eval["kl"]
        for _ in range(args.update_epochs):
            ...
            candidate_eval = evaluate_spo_policy()
            spo_last_candidate_kl = candidate_eval["kl"]
            exact_beta_kls.append(candidate_eval["kl"])
            candidate_finite = np.isfinite(candidate_eval["j"]) and np.isfinite(
                candidate_eval["kl"]
            )
            if (
                candidate_finite
                and candidate_eval["kl"] <= args.target_kl
                and candidate_eval["j"] > previous_j
            ):
                current_spo_eval = candidate_eval
                actor_updates_used += 1
                spo_projection_fraction = 1.0
                continue

            # Every rejected full step gets a transactional line search. This
            # includes KL-feasible candidates whose objective failed: a smaller
            # point can still be an improving ascent step. The bisection lower
            # bound advances only when *all* acceptance conditions hold.
            lo, hi = 0.0, 1.0
            best_fraction = 0.0
            best_eval = None
            for _ in range(args.spo_projection_evals):
                ...
                interpolate_actor_parameters(
                    previous_params, candidate_params, fraction
                )
                projected_eval = evaluate_spo_policy()
                exact_beta_kls.append(projected_eval["kl"])
                armijo_floor = (
                    previous_j
                ...
                admissible = (
                    np.isfinite(projected_eval["kl"])
                    and projected_eval["kl"] <= args.target_kl
                    and np.isfinite(projected_eval["j"])
                    and projected_eval["j"] > previous_j
                    and projected_eval["j"] >= armijo_floor
                )
                if admissible:
                    lo = fraction
                    best_fraction = fraction
                    best_eval = projected_eval
                else:
                    hi = fraction
            if best_eval is not None:
                interpolate_actor_parameters(
                    previous_params, candidate_params, best_fraction
                )
                current_spo_eval = best_eval
                actor_updates_used += 1
                spo_projection_fraction = best_fraction
                spo_projected = True
                break

            restore_actor_optimizer_state(actor_snapshot)
            current_spo_eval = evaluate_spo_policy()
            actor_updates_rejected += 1
            break
        ...
        # v209 has no dreamed data path: all actor acceptance statistics are
        ...
        writer.add_scalar(
            ...
            actor_updates_used / float(args.update_epochs),
            global_step,
        )
        ...
        writer.add_scalar("spo/projection_fraction", spo_projection_fraction, global_step)
        writer.add_scalar(
        ...
        writer.add_scalar("spo/candidate_kl", spo_last_candidate_kl, global_step)
