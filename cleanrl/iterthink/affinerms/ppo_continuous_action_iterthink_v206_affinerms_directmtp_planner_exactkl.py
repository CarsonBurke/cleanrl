# PPO + IterThink v206 direct action-prefix MTP + trust-limited behavior planning.
#
# Preserve v198's raw affine-RMS encoder, attached LeJEPA context/targets and
# encoder-side SIGReg before its action-free temporal memory.  Add a non-self-fed
# H=6 predictor: every future state/reward/continuation is decoded directly from
# the same real history and a causally masked prefix of real actions.  Once a
# deterministic heldout split demonstrates useful discounted-reward calibration,
# a small batched shooting planner refines only the behavior Beta under a strict
# exact-KL(q||pi) cap.  PPO stores and replays q exactly; all advantages remain
# real-environment GAE and no synthetic actor/critic updates are performed.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    lewm_ffn_mult: int = 2
    lewm_context: int = 5
    lewm_dyn_horizon: int = 6
    lewm_mtp_len: int = 6            # v206: direct, non-self-fed action-prefix predictions
    lewm_update_epochs: int = 2       # 1->2: WM was getting 10x fewer grad steps than the agent
    # H6 direct targets plus five attached real-history frames materially increase
    # activation memory; four 256-row microbatches preserve effective batch 1024.
    ...
    # v206 never trains the actor or critic on synthetic returns. The legacy AR
    # paths remain detached diagnostics only.
    imagine_enable: bool = False
    ...
    imagine_prompt_bank_rollouts: int = 0  # v206 has no synthetic optimization/prompt bank

    # Direct action-prefix MTP and model-predictive behavior policy. Planning is
    # admitted only by an out-of-training-split real-reward calibration test.
    planner_candidates: int = 16
    planner_horizon: int = 6
    planner_discount: float = 0.99
    planner_action_correlation: float = 0.75
    planner_elites: int = 4
    planner_temperature: float = 0.5
    planner_kl_cap: float = 0.0075
    planner_corr_threshold: float = 0.5
    planner_relative_mae_threshold: float = 0.5
    planner_min_calibration: int = 128
    planner_warmup_steps: int = 100000
    planner_holdout_modulus: int = 10
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # Direct MTP has no recurrent predicted-latent input.  Position k sees the
        # fixed real state memory and only actions a_t..a_{t+k}; causal prefix
        # attention makes future-action leakage structurally impossible.
        self.direct_horizon_embed = nn.Parameter(torch.empty(self.mtp_len, self.dim))
        nn.init.xavier_uniform_(self.direct_horizon_embed)
        self.direct_prefix_layers = nn.ModuleList(
            [
                TemporalStateBlock(
                    self.dim, args.lewm_heads, args.lewm_ffn_mult, args.lewm_rope_fraction
                )
        ...
        self.direct_prefix_norm = nn.RMSNorm(self.dim)
        self.direct_pred_proj = xavier_linear(nn.Linear(self.dim, self.dim))
    ...
    def direct_mtp_from_history(self, latent_history, action_prefix):
        """Predict each prefix endpoint directly, never feeding a prediction back.

        latent_history: (B,C,S,D), containing real encoded observations only.
        action_prefix: (B,H,A), where output k may see indices <= k only.
        Returns (B,H,semantic+obs,D).
        """
        if action_prefix.shape[1] > self.mtp_len:
            raise ValueError(f"direct prefix exceeds configured horizon {self.mtp_len}")
        state_memory = self._temporal_state_trunk(latent_history)
        horizon = action_prefix.shape[1]
        prefix = self.action_cond(action_prefix)
        prefix = prefix + self.direct_horizon_embed[:horizon].to(prefix.dtype).unsqueeze(0)
        prefix = prefix.unsqueeze(2)  # one causal token stream over prefix time
        for layer in self.direct_prefix_layers:
            prefix = layer(prefix)
        prefix = self.direct_prefix_norm(prefix[:, :, 0])
        # Every horizon reads the identical final real-history memory. Only the
        # causal action-prefix representation changes with k.
        fixed_memory = state_memory[:, -1:].expand(-1, horizon, -1, -1)
        features = self.transition_readout(fixed_memory, prefix)
        return self.direct_pred_proj(features)

    def imagine_step_from_history(self, latent_history, action_history):
        # Detached AR diagnostic only: repeatedly apply the trained direct k=0
        # endpoint while feeding its latent prediction back into history. This is
        # never an actor/critic training simulator in v206.
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
        last_action = action_history[:, -1:]
        next_summary = self.direct_mtp_from_history(latent_history, last_action)[:, 0]
        reward_logits, continuation_logits = self.decode_belief_outcomes(
            next_summary, action=None, detach_belief=False
        )
...
if __name__ == "__main__":
    ...
    assert args.lewm_mtp_len == args.planner_horizon == 6
    ...
    assert not args.imagine_enable, "v206 forbids synthetic actor/critic optimizer updates"
    assert 0.0 < args.planner_kl_cap < args.target_kl
    assert args.planner_candidates >= args.planner_elites >= 2
    assert args.planner_holdout_modulus >= 2
    ...
    planner_pi_logprobs = torch.zeros((args.num_steps, args.num_envs), device=device)
    planner_pi_entropies = torch.zeros((args.num_steps, args.num_envs), device=device)
    planner_behavior_kls = torch.zeros((args.num_steps, args.num_envs), device=device)
    planner_score_spreads = torch.zeros((args.num_steps, args.num_envs), device=device)
    planner_score_noise_ratios = torch.zeros((args.num_steps, args.num_envs), device=device)
    planner_used = torch.zeros((args.num_steps, args.num_envs), device=device)
    ...
    planner_calibration_corr = float("nan")
    planner_calibration_relative_mae = float("inf")
    planner_calibration_mae = float("inf")
    planner_action_sensitivity_ratio = 0.0
    ...
    def build_belief_agent_input(obs_tensor):
        ...
        return agent.agent_input_from_history(belief_latents), current_latent, belief_latents

    @torch.no_grad()
    def plan_beta_behavior(real_history, pi_alpha, pi_beta):
        """One batched direct-MTP shooting call, returning a KL-capped Beta q."""
        batch, act_dim = pi_alpha.shape
        n, horizon = args.planner_candidates, args.planner_horizon
        raw = Beta(pi_alpha, pi_beta).sample((n, horizon)).permute(2, 0, 1, 3).contiguous()
        # Candidate zero is the policy mean; other candidates use an AR(1)-like
        # convex correlation in native Beta space to produce coherent controls.
        pi_mean = pi_alpha / (pi_alpha + pi_beta)
        raw[:, 0] = pi_mean[:, None, :].expand(-1, horizon, -1)
        rho = args.planner_action_correlation
        for k in range(1, horizon):
            raw[:, 1:, k] = rho * raw[:, 1:, k - 1] + (1.0 - rho) * raw[:, 1:, k]
        candidate_actions = agent.action_low + (agent.action_high - agent.action_low) * raw
        expanded_history = real_history[:, None].expand(
            -1, n, -1, -1, -1
        ).reshape(
            batch * n,
            real_history.shape[1],
            real_history.shape[2],
            real_history.shape[3],
        )
        direct_pred = agent.world_model.direct_mtp_from_history(
            expanded_history,
            candidate_actions.reshape(batch * n, horizon, act_dim),
        )
        reward_logits, _ = agent.world_model.decode_belief_outcomes(
            direct_pred, action=None, detach_belief=False
        )
        pred_rewards = reward_logits_to_scalar(reward_logits).reshape(batch, n, horizon)
        discounts = args.planner_discount ** torch.arange(horizon, device=device, dtype=pred_rewards.dtype)
        scores = (pred_rewards * discounts).sum(dim=-1)
        score_spread = scores.std(dim=1, unbiased=False)
        # calibration_mae is already measured in discounted H-return units.
        noise_scale = max(planner_calibration_mae, 1e-6)
        score_noise_ratio = score_spread / noise_scale

        elite_scores, elite_idx = scores.topk(args.planner_elites, dim=1)
        elite_first = raw[:, :, 0].gather(
            1, elite_idx.unsqueeze(-1).expand(-1, -1, act_dim)
        )
        scale = elite_scores.std(dim=1, keepdim=True, unbiased=False).clamp_min(1e-4)
        elite_weight = torch.softmax(
            (elite_scores - elite_scores.max(dim=1, keepdim=True).values)
            / (args.planner_temperature * scale),
            dim=1,
        )
        q_mean = (elite_weight.unsqueeze(-1) * elite_first).sum(dim=1).clamp(1e-3, 1.0 - 1e-3)
        q_var = (
            elite_weight.unsqueeze(-1) * (elite_first - q_mean[:, None]).square()
        ).sum(dim=1).clamp_min(1e-5)
        concentration = q_mean * (1.0 - q_mean) / q_var - 1.0
        # Unimodal Beta requires alpha,beta > 1. Fit the nearest concentration
        # along the moment-fit mean ray when elite variance is too broad.
        min_concentration = torch.maximum(1.001 / q_mean, 1.001 / (1.0 - q_mean))
        concentration = torch.maximum(concentration, min_concentration).clamp(max=1e4)
        target_alpha = q_mean * concentration
        target_beta = (1.0 - q_mean) * concentration

        # State-wise bisection interpolates concentration parameters back toward
        # pi until the exact multivariate KL(q||pi) meets the behavior cap.
        lo = torch.zeros(batch, 1, device=device)
        hi = torch.ones(batch, 1, device=device)
        for _ in range(18):
            mid = (lo + hi) * 0.5
            mid_alpha = pi_alpha + mid * (target_alpha - pi_alpha)
            mid_beta = pi_beta + mid * (target_beta - pi_beta)
            mid_kl = torch.distributions.kl_divergence(
                Beta(mid_alpha, mid_beta), Beta(pi_alpha, pi_beta)
            ).sum(dim=-1, keepdim=True)
            admissible = torch.isfinite(mid_kl) & (mid_kl <= args.planner_kl_cap)
            lo = torch.where(admissible, mid, lo)
            hi = torch.where(admissible, hi, mid)
        q_alpha = pi_alpha + lo * (target_alpha - pi_alpha)
        q_beta = pi_beta + lo * (target_beta - pi_beta)
        q_kl = torch.distributions.kl_divergence(
            Beta(q_alpha, q_beta), Beta(pi_alpha, pi_beta)
        ).sum(dim=-1)
        finite = (
            torch.isfinite(q_alpha).all(dim=-1)
            & torch.isfinite(q_beta).all(dim=-1)
            & torch.isfinite(q_kl)
            & (q_kl <= args.planner_kl_cap * 1.001)
            & (score_noise_ratio > 1.0)
        )
        q_alpha = torch.where(finite[:, None], q_alpha, pi_alpha)
        q_beta = torch.where(finite[:, None], q_beta, pi_beta)
        q_kl = torch.where(finite, q_kl, torch.zeros_like(q_kl))
        q_dist = Beta(q_alpha, q_beta)
        q_z = q_dist.sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        q_action = agent.action_low + (agent.action_high - agent.action_low) * q_z
        q_logprob = q_dist.log_prob(q_z).sum(dim=-1)
        pi_logprob = Beta(pi_alpha, pi_beta).log_prob(q_z).sum(dim=-1)
        return (
            q_action,
            q_z,
            q_logprob,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                agent_input, current_latent, real_history = build_belief_agent_input(next_obs)
                if not rollout_actor_active and args.actor_dist == "beta":
                    ...
                    pi_behavior_logprob = logprob
                    planner_kl = torch.zeros_like(logprob)
                    planner_spread = torch.zeros_like(logprob)
                    planner_noise_ratio = torch.zeros_like(logprob)
                    planned_mask = torch.zeros_like(logprob, dtype=torch.bool)
                    ent = torch.zeros_like(logprob)
                    ...
                    planner_noise_ratio = torch.zeros_like(logprob)
                    planned_mask = torch.zeros_like(logprob, dtype=torch.bool)
                    if planner_enabled and global_step >= args.planner_warmup_steps:
                        (
                            action,
                            z,
                            logprob,
                            ...
                            planner_noise_ratio,
                            planned_mask,
                        ) = plan_beta_behavior(real_history, behavior_alpha, behavior_beta)
            ...
            planner_pi_logprobs[step] = pi_behavior_logprob
            planner_pi_entropies[step] = ent
            planner_behavior_kls[step] = planner_kl
            planner_score_spreads[step] = planner_spread
            planner_score_noise_ratios[step] = planner_noise_ratio
            planner_used[step] = planned_mask.float()
            ...
            with torch.no_grad():
                next_transition_agent_input, _, _ = build_belief_agent_input(transition_next_obs_t)
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                _, _, _, boot_entropy, _ = agent.get_action_and_value_from_agent_input(
                    next_obses[-1],
                ...
                next_value_bonus[:-1] = alpha_r * planner_pi_entropies[1:]
                next_value_bonus[-1] = alpha_r * boot_entropy
            else:
            ...
            # Real-only estimate of the behavior planner's incremental gain.
            # q generated the action, so (1-pi_old/q)A is an unbiased q-sample
            # contrast against the collection-time policy. Aggregate by env to
            # expose an env-block confidence interval rather than treating time
            # steps as independent.
            planner_gain_terms = (
                1.0 - torch.exp(planner_pi_logprobs - logprobs)
            ) * advantages
            env_gain_blocks = []
            for env_idx in range(args.num_envs):
                env_mask = planner_used[:, env_idx] > 0.5
                if bool(env_mask.any()):
                    env_gain_blocks.append(planner_gain_terms[env_mask, env_idx].mean())
            if env_gain_blocks:
                env_gain_blocks_t = torch.stack(env_gain_blocks)
                planner_real_gain = float(env_gain_blocks_t.mean())
                planner_real_gain_ci95 = float(
                    1.96
                    * env_gain_blocks_t.std(unbiased=False)
                    / max(1.0, env_gain_blocks_t.numel() ** 0.5)
                )
            else:
                planner_real_gain = 0.0
                planner_real_gain_ci95 = 0.0
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # Fixed PAST real context ending at s_t, exactly matching online
                    # planning. Invalid left-padding and history crossing an episode
                    # boundary are zeroed; no future target state enters this input.
                    obs_start = agent.world_model.obs_token_start
                    context = agent.world_model.context
                    past_offset = torch.arange(context - 1, -1, -1, device=device)
                    past_step_raw = mb_step_inds[:, None] - past_offset[None, :]
                    past_in_rollout = past_step_raw >= 0
                    past_steps = past_step_raw.clamp(0, max_start)
                    past_envs = mb_env_inds[:, None].expand_as(past_steps)
                    history_valid = past_in_rollout.clone()
                    for hist_pos in range(context - 1):
                        # dones[j]==1 means obs[j] begins after a boundary.
                        begin = int(context - 1 - hist_pos)
                        for d in range(begin - 1, -1, -1):
                            check_step = (mb_step_inds - d).clamp(0, max_start)
                            history_valid[:, hist_pos] &= dones[check_step, mb_env_inds] == 0
                    history_summaries = agent.world_model.encode_summary(
                        obs[past_steps, past_envs].reshape((-1,) + obs_shape)
                    ).reshape(
                        mb_size, context, agent.world_model.num_latent_tokens, agent.world_model.dim
                    )
                    fixed_history = history_summaries[:, :, obs_start:] * history_valid[
                        :, :, None, None
                    ].to(history_summaries.dtype)
                    direct_pred = agent.world_model.direct_mtp_from_history(fixed_history, future_actions)
                    ...
                    # Hold out whole environment trajectories so overlapping H-step
                    # windows cannot leak a heldout transition through a neighboring
                    # training start. With 16 envs/modulus 10 this selects envs 0,10.
                    heldout_row = (mb_env_inds % args.planner_holdout_modulus) == 0
                    train_row = (~heldout_row).to(step_weight.dtype).unsqueeze(1)
                    latent_weight = step_weight * future_valids * train_row
                    ...
                    direct_denom = latent_weight.sum().clamp_min(1.0)
                    direct_target = target_summaries[:, :, obs_start:]
                    direct_latent_error = F.mse_loss(
                        direct_pred[:, :, obs_start:], direct_target, reduction="none"
                    ).mean(dim=(-1, -2))
                    wm_latent_loss = (direct_latent_error * latent_weight).sum() / direct_denom
                    ...
                    direct_reward_ce = -(
                        ...
                        * torch.log_softmax(pred_reward_logits_all, dim=-1)
                    ).sum(dim=-1)
                    direct_cont_bce = F.binary_cross_entropy_with_logits(
                        pred_continuation_logits_all,
                        1.0 - future_terminations,
                        reduction="none",
                    )
                    wm_reward_loss = (direct_reward_ce * latent_weight).sum() / direct_denom
                    wm_termination_loss = (direct_cont_bce * latent_weight).sum() / direct_denom
                    with torch.no_grad():
                        pred_reward_scalar = reward_logits_to_scalar(pred_reward_logits_all)
                        reward_error = pred_reward_scalar - future_rewards
                        pred_reward_probs = torch.softmax(pred_reward_logits_all, dim=-1)
                        pred_edge_mass = pred_reward_probs[..., 0] + pred_reward_probs[..., -1]
                        target_edge_frac = (
                            (future_rewards <= args.lewm_reward_raw_v_min)
                            | (future_rewards >= args.lewm_reward_raw_v_max)
                        ).to(latent_weight.dtype)
                        for mtp_idx in range(mtp_len):
                            w_h = latent_weight[:, mtp_idx]
                            denom_h = w_h.sum()
                            wm_reward_abs_error_sums[mtp_idx] += (
                                reward_error[:, mtp_idx].abs() * w_h
                            ).sum()
                            wm_reward_error_sums[mtp_idx] += (
                                reward_error[:, mtp_idx] * w_h
                            ).sum()
                            wm_reward_pred_edge_sums[mtp_idx] += (
                                pred_edge_mass[:, mtp_idx] * w_h
                            ).sum()
                            wm_reward_target_edge_sums[mtp_idx] += (
                                target_edge_frac[:, mtp_idx] * w_h
                            ).sum()
                            wm_reward_probe_weights[mtp_idx] += denom_h

                    # Attached SIGReg covers the exact real encoder contexts and
                    # targets used by direct MTP; predictions themselves are never
                    # normalized or SIGReg'd.
                    sigreg_latents = torch.cat(
                        [
                            history_summaries[:, :, obs_start:],
                            target_summaries[:, :, obs_start:],
                        ],
                        dim=1,
                    )
                    sigreg_weight = torch.cat(
                        [
                            history_valid.to(latent_weight.dtype) * train_row,
                            latent_weight,
                        ],
                        dim=1,
                    )
        ...
        # Final-model, no-grad calibration on whole heldout environment
        # trajectories. These rows were excluded from every WM loss above.
        with torch.no_grad():
            heldout_envs = torch.arange(args.num_envs, device=device)
            heldout_envs = heldout_envs[(heldout_envs % args.planner_holdout_modulus) == 0]
            calib_pred, calib_true, calib_permuted = [], [], []
            if heldout_envs.numel() > 0:
                calib_steps_grid = torch.arange(
                    0, args.num_steps - args.planner_horizon + 1, device=device
                )
                calib_steps = calib_steps_grid[:, None].expand(-1, heldout_envs.numel()).reshape(-1)
                calib_env = heldout_envs[None, :].expand(calib_steps_grid.numel(), -1).reshape(-1)
                h_off = torch.arange(args.planner_horizon, device=device)
                f_steps = calib_steps[:, None] + h_off[None, :]
                f_env = calib_env[:, None].expand_as(f_steps)
                f_boundary = transition_boundaries[f_steps, f_env]
                f_valid = transition_valids[f_steps, f_env]
                prefix_alive = torch.cumprod(
                    torch.cat(
                        [torch.ones(f_boundary.shape[0], 1, device=device), 1.0 - f_boundary[:, :-1]],
                        dim=1,
                    ),
                    dim=1,
                )
                full_valid = ((prefix_alive * f_valid).min(dim=1).values > 0.5)
                calib_steps = calib_steps[full_valid]
                calib_env = calib_env[full_valid]
                discounts = args.planner_discount ** torch.arange(
                    args.planner_horizon, device=device, dtype=rewards.dtype
                )
                calib_batch = max(64, args.lewm_minibatch_size)
                for c_start in range(0, calib_steps.numel(), calib_batch):
                    c_step = calib_steps[c_start : c_start + calib_batch]
                    c_env = calib_env[c_start : c_start + calib_batch]
                    cb = c_step.numel()
                    if cb == 0:
                        continue
                    past_off = torch.arange(
                        agent.world_model.context - 1, -1, -1, device=device
                    )
                    past_raw = c_step[:, None] - past_off[None, :]
                    past_in = past_raw >= 0
                    past_idx = past_raw.clamp(0, args.num_steps - 1)
                    past_env = c_env[:, None].expand_as(past_idx)
                    c_hist_valid = past_in.clone()
                    for hist_pos in range(agent.world_model.context - 1):
                        begin = agent.world_model.context - 1 - hist_pos
                        for d in range(begin - 1, -1, -1):
                            check = (c_step - d).clamp(0, args.num_steps - 1)
                            c_hist_valid[:, hist_pos] &= dones[check, c_env] == 0
                    c_summ = agent.world_model.encode_summary(
                        obs[past_idx, past_env].reshape((-1,) + obs_shape)
                    ).reshape(
                        cb,
                    ...
                    c_hist = c_summ[:, :, agent.world_model.obs_token_start :] * c_hist_valid[
                        :, :, None, None
                    ].to(c_summ.dtype)
                    c_future_step = c_step[:, None] + h_off[None, :]
                    c_future_env = c_env[:, None].expand_as(c_future_step)
                    c_actions = actions[c_future_step, c_future_env]
                    c_rewards = rewards[c_future_step, c_future_env]
                    c_pred = agent.world_model.direct_mtp_from_history(c_hist, c_actions)
                    c_reward_logits, _ = agent.world_model.decode_belief_outcomes(
                        c_pred, action=None, detach_belief=False
                    )
                    c_pred_score = (
                        reward_logits_to_scalar(c_reward_logits) * discounts
                    ).sum(dim=1)
                    c_true_score = (c_rewards * discounts).sum(dim=1)
                    # Deterministic action-prefix permutation, same state histories.
                    perm_actions = torch.roll(c_actions, shifts=1, dims=0)
                    perm_pred = agent.world_model.direct_mtp_from_history(c_hist, perm_actions)
                    perm_logits, _ = agent.world_model.decode_belief_outcomes(
                        perm_pred, action=None, detach_belief=False
                    )
                    perm_score = (reward_logits_to_scalar(perm_logits) * discounts).sum(dim=1)
                    calib_pred.append(c_pred_score)
                    calib_true.append(c_true_score)
                    calib_permuted.append(perm_score)
            calibration_count = sum(x.numel() for x in calib_true)
            if calibration_count >= args.planner_min_calibration:
                pred_score = torch.cat(calib_pred)
                true_score = torch.cat(calib_true)
                perm_score = torch.cat(calib_permuted)
                pred_centered = pred_score - pred_score.mean()
                true_centered = true_score - true_score.mean()
                corr_denom = (
                    pred_centered.square().mean().sqrt()
                    * true_centered.square().mean().sqrt()
                ).clamp_min(1e-8)
                planner_calibration_corr = float(
                    (pred_centered * true_centered).mean() / corr_denom
                )
                abs_error = (pred_score - true_score).abs()
                planner_calibration_mae = float(abs_error.mean())
                variation = (true_score - true_score.mean()).abs().mean().clamp_min(1e-8)
                planner_calibration_relative_mae = float(abs_error.mean() / variation)
                planner_action_sensitivity_ratio = float(
                    (pred_score - perm_score).abs().mean()
                    / abs_error.mean().clamp_min(1e-8)
                )
            else:
                planner_calibration_corr = float("nan")
                planner_calibration_relative_mae = float("inf")
            ...
            planner_enabled = bool(
                calibration_count >= args.planner_min_calibration
                and np.isfinite(planner_calibration_corr)
                and planner_calibration_corr > args.planner_corr_threshold
                and planner_calibration_relative_mae < args.planner_relative_mae_threshold
                and planner_action_sensitivity_ratio > 1.0
            )
        ...
        writer.add_scalar("planner/enabled", float(planner_enabled), global_step)
        writer.add_scalar("planner/calibration_count", float(calibration_count), global_step)
        writer.add_scalar("planner/heldout_discounted_reward_corr", planner_calibration_corr, global_step)
        writer.add_scalar(
            "planner/heldout_discounted_reward_relative_mae",
            planner_calibration_relative_mae,
            global_step,
        )
        writer.add_scalar("planner/heldout_discounted_reward_mae", planner_calibration_mae, global_step)
        writer.add_scalar(
            "planner/heldout_action_sensitivity_noise_ratio",
            planner_action_sensitivity_ratio,
            global_step,
        )
        writer.add_scalar("planner/used_fraction", planner_used.mean().item(), global_step)
        used_weight = planner_used.sum().clamp_min(1.0)
        writer.add_scalar(
            "planner/exact_kl_q_pi_mean",
            (planner_behavior_kls * planner_used).sum().item() / used_weight.item(),
            global_step,
        )
        writer.add_scalar("planner/exact_kl_q_pi_max", planner_behavior_kls.max().item(), global_step)
        writer.add_scalar(
            "planner/predicted_score_spread",
            (planner_score_spreads * planner_used).sum().item() / used_weight.item(),
            global_step,
        )
        writer.add_scalar(
            "planner/predicted_score_spread_noise_ratio",
            (planner_score_noise_ratios * planner_used).sum().item() / used_weight.item(),
            global_step,
        )
        writer.add_scalar("planner/real_gain_estimate", planner_real_gain, global_step)
        writer.add_scalar("planner/real_gain_env_block_ci95", planner_real_gain_ci95, global_step)
