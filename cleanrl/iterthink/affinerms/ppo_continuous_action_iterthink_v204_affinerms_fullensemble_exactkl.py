# PPO + IterThink v204 affine-RMS full epistemic transition ensemble.
#
# Preserve the shared affine-RMS spatial encoder, attached LeJEPA targets,
# encoder-only SIGReg, raw observations, detached policy, plain GAE, and exact
# Beta-KL accounting. Fix v203's shallow epistemic ensemble: each of three fixed
# 65%-bootstrap members now owns its two-block causal temporal trunk, norm, action
# conditioner, transition queries/projection, and reward/continuation decoder. A
# separate canonical v198 temporal trunk is fitted on the non-calibration training
# split and is the only recurrent state exposed to PPO, so bootstrap membership
# cannot randomly move the policy representation.
#
# Final-model support calibration is componentwise on deterministic held-out real
# transitions. Imagination keeps one member per trajectory and makes rejection
# absorbing. A whole dream block is skipped -- including its critic update -- unless
# it contains at least max(256, 20% of H*B) accepted transitions, starts with >=20%
# support, and has mean supported prefix >=2. Admitted actor steps retain both the
# fixed real-policy and supported-dream exact-KL anchors. This uses Dreamer4's idea
# of coherent stochastic rollouts without diffusion, normalization, target EMA, or
# any policy-to-world-model gradient.
#
...
ENSEMBLE_SIZE = 3
BOOTSTRAP_KEEP_PROB = 0.65


def mix_integer_ids(ids, salt):
    """Deterministic nonlinear mixer whose products stay inside signed int64."""
    modulus = 2_147_483_647
    x = (ids.to(torch.long).remainder(modulus) + salt).remainder(modulus)
    x = (x * 1_103_515_245).remainder(modulus)
    x = torch.bitwise_xor(x, x >> 16)
    x = (x * 2_246_822_519).remainder(modulus)
    return torch.bitwise_xor(x, x >> 13).remainder(modulus)
...
class Args:
    ...
    real_actor_kl_budget: float = 0.025  # reserve 0.005 total budget for supported dreams
    dream_anchor_kl_budget: float = 0.005
    ...
    lewm_ensemble_size: int = ENSEMBLE_SIZE  # fixed full temporal/dynamics ensemble
    lewm_bootstrap_keep_prob: float = BOOTSTRAP_KEEP_PROB  # fixed deterministic masks
    ...
    imagine_support_quantile: float = 0.95  # real-transition epistemic support boundary
    imagine_calibration_min_corr: float = 0.0  # disagreement must at least rank risk positively
    imagine_calibration_min_tail_capture: float = 0.15  # disagreement must catch residual q95 tail
    imagine_continue_min_class_count: int = 16  # omit continuation gate without both labels
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.ensemble_size = args.lewm_ensemble_size
        ...
        # recurrent stream.
        self.reward_token_norms = nn.ModuleList(
            [nn.RMSNorm(self.dim) for _ in range(self.ensemble_size)]
        )
        self.continuation_token_norms = nn.ModuleList(
            [nn.RMSNorm(self.dim) for _ in range(self.ensemble_size)]
        )
        self.reward_action_projs = nn.ModuleList(
            [xavier_linear(nn.Linear(self.act_dim, self.dim, bias=False)) for _ in range(self.ensemble_size)]
        )
        self.reward_token_unprojs = nn.ModuleList(
            [xavier_linear(nn.Linear(self.dim, self.reward_num_bins)) for _ in range(self.ensemble_size)]
        )
        self.continuation_token_unprojs = nn.ModuleList(
            [xavier_linear(nn.Linear(self.dim, 1)) for _ in range(self.ensemble_size)]
        )

        # Canonical v198 path: this full-training-split temporal state is the only
        # recurrent representation exposed to PPO. It does not receive any member
        # bootstrap mask. Its private transition head supplies a dynamics gradient
        # only to canonical-path parameters; both encoder input and target are
        # detached so the ensemble remains the single attached LeJEPA objective.
        self.canonical_action_cond = xavier_linear(nn.Linear(act_dim, self.dim))
        self.canonical_predictor_layers = nn.ModuleList(
            [
                TemporalStateBlock(
        ...
        self.canonical_predictor_norm = nn.RMSNorm(self.dim)
        self.canonical_transition_readout = TransitionQueryReadout(
            self.num_obs_tokens,
        ...
        self.canonical_pred_next_proj = xavier_linear(nn.Linear(self.dim, self.dim))

        # v204 epistemic path: diversity begins before causal history aggregation.
        # Each member owns the complete temporal/action/transition/outcome path;
        # only the observation-only spatial encoder and its attached targets remain
        # shared. Two temporal blocks per member match the canonical v198 topology.
        self.member_action_conds = nn.ModuleList(
            [xavier_linear(nn.Linear(act_dim, self.dim)) for _ in range(self.ensemble_size)]
        )
        self.member_predictor_layers = nn.ModuleList(
            [
                nn.ModuleList(
                    [
                        TemporalStateBlock(
        ...
        self.member_predictor_norms = nn.ModuleList(
            [nn.RMSNorm(self.dim) for _ in range(self.ensemble_size)]
        )
        self.transition_readouts = nn.ModuleList(
            [
                TransitionQueryReadout(
                    self.num_obs_tokens,
                    self.num_semantic_tokens,
                    self.dim,
                    args.lewm_heads,
        ...
        self.pred_next_projs = nn.ModuleList(
            [xavier_linear(nn.Linear(self.dim, self.dim)) for _ in range(self.ensemble_size)]
        )
    ...
    def decode_belief_outcomes(self, belief, action=None, detach_belief=True, member_idx=0):
        if detach_belief:
            belief = belief.detach()
        reward_feat = self.reward_token_norms[member_idx](belief[..., self.reward_token_idx, :])
        continuation_feat = self.continuation_token_norms[member_idx](
            belief[..., self.continuation_token_idx, :]
        )
        if action is not None:
            reward_feat = reward_feat + self.reward_action_projs[member_idx](action)
        reward_logits = self.reward_token_unprojs[member_idx](reward_feat)
        continuation_logits = self.continuation_token_unprojs[member_idx](continuation_feat).squeeze(-1)
        return reward_logits, continuation_logits

    def decode_ensemble_outcomes(self, ensemble_beliefs, action=None, detach_belief=True):
        """Decode K independent outcome heads; K is the leading dimension."""
        rewards, continuations = [], []
        for member_idx in range(self.ensemble_size):
            reward, continuation = self.decode_belief_outcomes(
                ensemble_beliefs[member_idx], action, detach_belief, member_idx
            )
            rewards.append(reward)
            continuations.append(continuation)
        return torch.stack(rewards), torch.stack(continuations)

    def _run_temporal_state_trunk(self, latent_history, layers, norm):
        # Observation-only recurrent stream. Attention remains causal over time
        # independently per already-spatially-mixed observation token.
        context_len = latent_history.shape[1]
    ...
    def _canonical_temporal_state_trunk(self, latent_history):
        return self._run_temporal_state_trunk(
            latent_history, self.canonical_predictor_layers, self.canonical_predictor_norm
        )

    def _member_temporal_state_trunk(self, latent_history, member_idx):
        return self._run_temporal_state_trunk(
            latent_history,
            self.member_predictor_layers[member_idx],
            self.member_predictor_norms[member_idx],
        )

    def belief_from_history(self, latent_history):
        # PPO and its bootstrap critic see only this canonical, non-bootstrapped
        # recurrent state. Member selection affects simulated transitions, not the
        # representation used to choose an action at a given latent history.
        return self._canonical_temporal_state_trunk(latent_history)[:, -1]

    def predict_canonical_from_history(self, latent_history, action_history):
        """Full-training-split v198 transition used to fit the policy state trunk."""
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
            action_history = action_history[:, -self.context :]
        state_memory = self._canonical_temporal_state_trunk(latent_history)
        action_features = self.canonical_action_cond(action_history)
        features = self.canonical_transition_readout(state_memory, action_features)
        return self.canonical_pred_next_proj(features).unsqueeze(2)

    def predict_ensemble_from_history(self, latent_history, action_history, return_beliefs=False):
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
            action_history = action_history[:, -self.context :]
        preds, state_memories = [], []
        for member_idx, (action_cond, readout, projection) in enumerate(
            zip(self.member_action_conds, self.transition_readouts, self.pred_next_projs)
        ):
            state_memory = self._member_temporal_state_trunk(latent_history, member_idx)
            action_features = action_cond(action_history)
            features = readout(state_memory, action_features)
            preds.append(projection(features).unsqueeze(2))
            state_memories.append(state_memory)
        preds = torch.stack(preds)  # (K,B,T,1,L,D)
        if return_beliefs:
            return preds, torch.stack(state_memories)
        return preds

    def predict_mtp_from_history(self, latent_history, action_history, return_beliefs=False):
        # Compatibility path for diagnostics: the ensemble mean is the minimum-MSE
        # point estimate. Imagination samples one persistent member instead.
        result = self.predict_ensemble_from_history(latent_history, action_history, return_beliefs)
        if return_beliefs:
            preds, _ = result
            return preds.mean(dim=0), self._canonical_temporal_state_trunk(latent_history)
        return result.mean(dim=0)

    def imagine_step_all_from_history(self, latent_history, action_history):
        """Return every member for epistemic scoring and persistent-member rollout."""
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
        ...
        for member_idx, (action_cond, readout, projection) in enumerate(
            ...
            state_memory = self._member_temporal_state_trunk(latent_history, member_idx)
            action_features = action_cond(last_action)
            features = readout(state_memory[:, -1:], action_features)[:, -1]
            next_summaries.append(projection(features))
        next_summaries = torch.stack(next_summaries)
        reward_logits, continuation_logits = self.decode_ensemble_outcomes(
            next_summaries, last_action[:, -1], detach_belief=False
        )
        return next_summaries, reward_logits, continuation_logits

    def imagine_step_from_history(self, latent_history, action_history):
        # Mean prediction is used only for labeled diagnostics. Consumed dreams
        # call imagine_step_all_from_history and select one fixed member/trajectory.
        next_summaries, reward_logits, continuation_logits = self.imagine_step_all_from_history(
            latent_history, action_history
        )
        return next_summaries.mean(0), reward_logits.mean(0), continuation_logits.mean(0)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        assert self.agent_input_dim == self.world_model.num_obs_tokens * self.world_model.dim
    ...
    def agent_input_from_history(self, latent_history):
    ...
    def _actor_feat_from_agent_input(self, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        return self.actor_trunk(self.actor_readout(agent_input))

    def _critic_feat_from_agent_input(self, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        return self.critic_trunk(self.critic_readout(agent_input))
    ...
    def actor_parameters(self):
        ...
        return list(self.actor_readout.parameters()) + list(trunk.parameters()) + heads
...
if __name__ == "__main__":
    ...
    assert args.lewm_ensemble_size == ENSEMBLE_SIZE, "v204 fixes K=3 for calibrated support"
    assert args.lewm_predictor_layers == 2, "v204 gives every member exactly two temporal blocks"
    assert abs(args.lewm_bootstrap_keep_prob - BOOTSTRAP_KEEP_PROB) < 1e-12, (
        "v204 fixes the member bootstrap keep probability at 0.65"
    )
    assert 0.5 < args.imagine_support_quantile < 1.0
    assert -1.0 <= args.imagine_calibration_min_corr <= 1.0
    assert 0.0 <= args.imagine_calibration_min_tail_capture <= 1.0
    assert args.imagine_continue_min_class_count > 0
    ...
    assert 0.0 < args.real_actor_kl_budget < args.target_kl
    assert 0.0 < args.dream_anchor_kl_budget <= args.target_kl
    ...
    wm_outcome_io_params = (
        list(agent.world_model.reward_token_norms.parameters())
        + list(agent.world_model.continuation_token_norms.parameters())
        + list(agent.world_model.reward_action_projs.parameters())
        + list(agent.world_model.reward_token_unprojs.parameters())
        + list(agent.world_model.continuation_token_unprojs.parameters())
    )
    ...
    with torch.no_grad():
        ...
        reward_prior = reward_support.project_to_logprobs(
            torch.zeros(1, device=device), eps=args.lewm_reward_prior_floor
        )[0]
        for reward_head in agent.world_model.reward_token_unprojs:
            reward_head.weight.zero_()
            reward_head.bias.copy_(reward_prior)
    ...
    imagine_prompt_bank = []
    # Componentwise final-model calibration state: direct held-out disagreement
    # quantiles [latent, reward, continue]. Continuation can be inactive when the
    # held-out split lacks both classes; correlations/tail capture gate imagination
    # globally and never rescale an individual dream transition.
    real_support_thresholds = None
    real_support_components_active = None
    support_calibration_ready = False
    ...
    def build_belief_agent_input(obs_tensor):
        ...
        return agent.agent_input_from_history(belief_latents), current_latent
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        wm_losses = []
        wm_latent_losses = []
        wm_canonical_latent_losses = []
        wm_ensemble_latent_losses = []
        wm_reward_losses = []
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    canonical_pred = agent.world_model.predict_canonical_from_history(
                        teacher_history.detach(), future_actions
                    )
                    pred_ensemble = agent.world_model.predict_ensemble_from_history(
                        teacher_history, future_actions
                    )
                    ...
                    # Deterministic bootstrap assignment. Nonlinear mixing happens
                    # after combining transition and member identity; unlike v203's
                    # affine member offset, these are not shifted copies of one mask.
                    # The canonical PPO-state trunk sees the complete non-calibration
                    # training split and never receives these member masks.
                    transition_ids = (
                        global_step * (args.num_steps * args.num_envs)
                        + safe_hist_step_inds * args.num_envs
                        + env_inds
                    ).long()
                    member_ids = torch.arange(
                        args.lewm_ensemble_size, device=device, dtype=torch.long
                    )[:, None, None]
                    bootstrap_hash = mix_integer_ids(
                        transition_ids.unsqueeze(0), (member_ids + 1) * 1_000_003
                    )
                    calibration_hash = mix_integer_ids(transition_ids, 97_531)
                    calibration_mask = calibration_hash.remainder(10) == 0
                    bootstrap_masks = (
                        bootstrap_hash
                        < int(BOOTSTRAP_KEEP_PROB * 2_147_483_647)
                    ) & ~calibration_mask.unsqueeze(0)
                    one_step_valid = latent_weight.unsqueeze(0) * bootstrap_masks.to(latent_weight.dtype)
                    one_step_pred = pred_ensemble[:, :, :, 0, agent.world_model.obs_token_start :]
                    canonical_one_step_pred = canonical_pred[:, :, 0, agent.world_model.obs_token_start :]
                    one_step_target = target_summaries[:, :, agent.world_model.obs_token_start :]
                    canonical_weight = latent_weight * (~calibration_mask).to(latent_weight.dtype)
                    canonical_latent_error = F.mse_loss(
                        canonical_one_step_pred,
                        one_step_target.detach(),
                        reduction="none",
                    ).mean(dim=(-1, -2))
                    canonical_latent_loss = (
                        (canonical_latent_error * canonical_weight).sum()
                        / canonical_weight.sum().clamp_min(1.0)
                    )
                    one_step_latent_loss = F.mse_loss(
                        one_step_pred,
                        one_step_target.unsqueeze(0).expand_as(one_step_pred),
                        reduction="none",
                    ).mean(dim=(-1, -2))
                    member_latent_losses = []
                    for member_idx in range(args.lewm_ensemble_size):
                        member_weight = one_step_valid[member_idx]
                        member_latent_losses.append(
                            (one_step_latent_loss[member_idx] * member_weight).sum()
                            / member_weight.sum().clamp_min(1.0)
                        )
                    # Preserve full-strength fitting for the canonical PPO state and
                    # the independently bootstrapped simulator ensemble. Canonical
                    # inputs/targets are detached, so this sum does not double the
                    # ensemble's attached encoder-target pressure.
                    ensemble_latent_loss = torch.stack(member_latent_losses).mean()
                    wm_latent_loss = canonical_latent_loss + ensemble_latent_loss
                    ...
                    pred_reward_logits_all, pred_continuation_logits_all = agent.world_model.decode_ensemble_outcomes(
                        pred_ensemble,
                    ...
                    for mtp_idx in range(mtp_len):
                        ...
                        if valid_horizon <= 0:
                            continue
                        offset_valid = one_step_valid[:, :, mtp_idx:]
                        reward_loss = -(
                            reward_target_probs[None, :, mtp_idx:].detach()
                            * torch.log_softmax(
                        ...
                        continuation_loss = F.binary_cross_entropy_with_logits(
                            ...
                            (1.0 - future_terminations[:, mtp_idx:]).unsqueeze(0).expand_as(
                                pred_continuation_logits_all[:, :, :valid_horizon, mtp_idx]
                            ),
                            reduction="none",
                        )
                        for member_idx in range(args.lewm_ensemble_size):
                            member_weight = offset_valid[member_idx]
                            denom = member_weight.sum().clamp_min(1.0)
                            reward_losses.append(
                                (reward_loss[member_idx] * member_weight).sum() / denom
                            )
                            termination_losses.append(
                                (continuation_loss[member_idx] * member_weight).sum() / denom
                            )
                        with torch.no_grad():
                            pred_reward_members = reward_logits_to_scalar(
                                pred_reward_logits_all[:, :, :valid_horizon, mtp_idx]
                            )
                            pred_reward_scalar = pred_reward_members.mean(0)
                            target_reward_scalar = future_rewards[:, mtp_idx:]
                            reward_error = pred_reward_scalar - target_reward_scalar
                            metric_weight = latent_weight[:, mtp_idx:]
                            metric_denom = metric_weight.sum().clamp_min(1.0)
                            wm_reward_abs_error_sums[mtp_idx] += (reward_error.abs() * metric_weight).sum()
                            wm_reward_error_sums[mtp_idx] += (reward_error * metric_weight).sum()
                            pred_reward_probs = torch.softmax(
                                pred_reward_logits_all[:, :, :valid_horizon, mtp_idx], dim=-1
                            ).mean(0)
                            pred_edge_mass = pred_reward_probs[..., 0] + pred_reward_probs[..., -1]
                            wm_reward_pred_edge_sums[mtp_idx] += (pred_edge_mass * metric_weight).sum()
                            target_edge_frac = (
                                (target_reward_scalar <= args.lewm_reward_raw_v_min)
                                | (target_reward_scalar >= args.lewm_reward_raw_v_max)
                            ).to(metric_weight.dtype)
                            wm_reward_target_edge_sums[mtp_idx] += (target_edge_frac * metric_weight).sum()
                            wm_reward_probe_weights[mtp_idx] += metric_denom
                    ...
                    wm_canonical_latent_losses.append(canonical_latent_loss.item())
                    wm_ensemble_latent_losses.append(ensemble_latent_loss.item())
        ...
        # Final-model calibration is deliberately separate from optimizer
        # minibatches: support describes the exact model dreams will consume, not
        # a mixture of earlier parameter states. The deterministic 10% split was
        # excluded from every supervised transition/member loss. Direct held-out
        # disagreement quantiles define componentwise support; residual correlation
        # and high-residual tail capture are only global readiness checks.
        # They never multiply or divide a per-transition disagreement (v203's ratio
        # calibration collapsed support when disagreement was numerically small).
        current_support_quantiles = None
        current_teacher_residual = None
        current_calibration_correlations = None
        current_calibration_tail_captures = None
        current_support_components_active = None
        support_calibration_ready = False
        with torch.no_grad():
            W_cal = min(agent.world_model.context, args.num_steps)
            cal_step = torch.arange(args.num_steps, device=device)[:, None].expand(-1, args.num_envs)
            cal_env = torch.arange(args.num_envs, device=device)[None, :].expand(args.num_steps, -1)
            cal_clean = cal_step >= (W_cal - 1)
            for d in range(W_cal - 1):
                cal_clean &= dones[(cal_step - d).clamp_min(0), cal_env] == 0
            cal_clean &= transition_valids.bool()
            cal_ids = global_step * (args.num_steps * args.num_envs) + cal_step * args.num_envs + cal_env
            cal_hash = mix_integer_ids(cal_ids, 97_531)
            cal_clean &= cal_hash.remainder(10) == 0
            cal_pairs = cal_clean.nonzero(as_tuple=False)
            if cal_pairs.shape[0] > 4096:
                cal_pairs = cal_pairs[:4096]
            if cal_pairs.shape[0] >= 32:
                cal_t, cal_e = cal_pairs[:, 0], cal_pairs[:, 1]
                cal_offsets = torch.arange(W_cal - 1, -1, -1, device=device)
                cal_win_t = cal_t[:, None] - cal_offsets[None, :]
                cal_win_e = cal_e[:, None].expand_as(cal_win_t)
                cal_summaries = agent.world_model.encode_summary(
                    obs[cal_win_t, cal_win_e].reshape((-1,) + obs_shape)
                ).reshape(
                    cal_pairs.shape[0], W_cal,
                    agent.world_model.num_latent_tokens, agent.world_model.dim,
                )
                cal_history = cal_summaries[:, :, agent.world_model.obs_token_start :]
                cal_action = actions[cal_t, cal_e].unsqueeze(1)
                cal_next_all, cal_reward_logits_all, cal_continue_logits_all = (
                    agent.world_model.imagine_step_all_from_history(cal_history, cal_action)
                )
                cal_target = agent.world_model.encode_summary(
                    next_obses[cal_t, cal_e]
                )[:, agent.world_model.obs_token_start :]
                cal_next_obs = cal_next_all[:, :, agent.world_model.obs_token_start :]
                cal_reward_members = reward_logits_to_scalar(cal_reward_logits_all)
                cal_continue_members = torch.sigmoid(cal_continue_logits_all)
                cal_component_disagreements = [
                    (cal_next_obs - cal_next_obs.mean(0, keepdim=True)).square().mean(dim=(0, -1, -2)),
                    cal_reward_members.var(dim=0, unbiased=False),
                    cal_continue_members.var(dim=0, unbiased=False),
                ]
                cal_component_residuals = [
                    (cal_next_obs.mean(0) - cal_target).square().mean(dim=(-1, -2)),
                    (cal_reward_members.mean(0) - rewards[cal_t, cal_e]).square(),
                    (
                        cal_continue_members.mean(0)
                        - (1.0 - transition_terminations[cal_t, cal_e])
                    ).square(),
                ]
                cal_continue_targets = 1.0 - transition_terminations[cal_t, cal_e]
                continue_zeros = int((cal_continue_targets < 0.5).sum().item())
                continue_ones = int((cal_continue_targets >= 0.5).sum().item())
                component_active = [
                    True,
                    True,
                    min(continue_zeros, continue_ones) >= args.imagine_continue_min_class_count,
                ]
                new_thresholds, new_corrs, new_tail_captures = [], [], []
                for disagreement, residual, active in zip(
                    cal_component_disagreements, cal_component_residuals, component_active
                ):
                    disagreement_threshold = torch.quantile(
                        disagreement, args.imagine_support_quantile
                    )
                    residual_limit = torch.quantile(residual, args.imagine_support_quantile)
                    new_thresholds.append(disagreement_threshold.item())
                    dx = disagreement - disagreement.mean()
                    ry = residual - residual.mean()
                    corr = (dx * ry).mean() / (
                        dx.square().mean().sqrt() * ry.square().mean().sqrt()
                    ).clamp_min(1e-8)
                    new_corrs.append(corr.item())
                    residual_tail = residual > residual_limit
                    # Does the disagreement q95 actually identify large-error cases?
                    # Random independent disagreement captures ~5% of this tail;
                    # require materially better-than-random tail capture globally.
                    tail_capture = (
                        (disagreement[residual_tail] > disagreement_threshold).float().mean()
                        if bool(residual_tail.any())
                        else torch.zeros((), device=device)
                    )
                    new_tail_captures.append(tail_capture.item())
                current_support_quantiles = new_thresholds
                current_teacher_residual = [x.mean().item() for x in cal_component_residuals]
                current_calibration_correlations = new_corrs
                current_calibration_tail_captures = new_tail_captures
                current_support_components_active = component_active
                active_checks = [
                    active
                    and np.isfinite(threshold)
                    and threshold > 0.0
                    and np.isfinite(corr)
                    and corr >= args.imagine_calibration_min_corr
                    and np.isfinite(tail_capture)
                    and tail_capture >= args.imagine_calibration_min_tail_capture
                    for active, threshold, corr, tail_capture in zip(
                        component_active, new_thresholds, new_corrs, new_tail_captures
                    )
                ]
                # Latent and reward are mandatory. Continuation participates only
                # when both labels are sufficiently represented in held-out data.
                support_calibration_ready = bool(
                    active_checks[0]
                    and active_checks[1]
                    and (not component_active[2] or active_checks[2])
                )
                # Use this exact final model's heldout quantiles. Carrying an EMA
                # across older models recreates a stale, indirect support boundary.
                real_support_thresholds = new_thresholds
                real_support_components_active = component_active

        world_model_only = not rollout_actor_active
        if world_model_only:
            writer.add_scalar("charts/world_model_only", 1.0, global_step)
            writer.add_scalar(
                "lewm/calibration_ready", float(support_calibration_ready), global_step
            )
            if wm_losses:
                writer.add_scalar("lewm/loss", float(np.mean(wm_losses)), global_step)
                writer.add_scalar(
                    "lewm/obs_latent_mse", float(np.mean(wm_ensemble_latent_losses)), global_step
                )
                writer.add_scalar(
                    "lewm/latent_objective", float(np.mean(wm_latent_losses)), global_step
                )
                writer.add_scalar(
                    "lewm/canonical_latent_mse",
                    float(np.mean(wm_canonical_latent_losses)),
                    global_step,
                )
                writer.add_scalar(
                    "lewm/ensemble_latent_mse",
                    float(np.mean(wm_ensemble_latent_losses)),
                    global_step,
                )
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with torch.no_grad():
                    ...
                    if real_actor_active and (
                        ...
                        or exact_beta_kl > args.real_actor_kl_budget
                    ):
                ...
                if args.separate_grad_clip:
                    ...
                    if attempted_actor_update:
                        ...
                        if (
                            ...
                            or post_step_beta_kl > args.real_actor_kl_budget
                        ):
        ...
        @torch.no_grad()
        def measure_real_anchor_beta_kl():
            return measure_behavior_beta_kl(trust_anchor_inds)

        post_real_anchor_beta_kl = measure_real_anchor_beta_kl()
        exact_beta_kls.append(post_real_anchor_beta_kl.item())
        # Real PPO stops at 0.025, reserving the last 0.005 of the total rollout
        # trust region for supported dreams. This flag is intentionally separate:
        # exhausting the real subbudget must not make the gated dream test inert.
        dream_actor_active = bool(
            torch.isfinite(post_real_anchor_beta_kl)
            and post_real_anchor_beta_kl <= args.target_kl
        )
        ...
        if (
            args.imagine_enable
            and global_step >= args.imagine_warmup_steps
            and support_calibration_ready
            and real_support_thresholds is not None
            and real_support_components_active is not None
        ):
            ...
            dream_blocks_admitted = 0
            dream_blocks_skipped = 0
            dream_blocks_low_count = 0
            dream_blocks_low_initial = 0
            dream_blocks_short_prefix = 0
            ...
            im_support_sum = torch.zeros((), device=device)
            im_support_depth_sum = torch.zeros((), device=device)
            im_initial_support_sum = torch.zeros((), device=device)
            im_disagreement_sum = torch.zeros((), device=device)
            ...
            for _ in range(args.imagine_batches):
                anchor_beta_kl = measure_real_anchor_beta_kl()
                exact_beta_kls.append(anchor_beta_kl.item())
                if dream_actor_active and (
                    not torch.isfinite(anchor_beta_kl) or anchor_beta_kl > args.target_kl
                ):
                    dream_actor_active = False
                with torch.no_grad():
                    ...
                    im_supports, im_disagreements = [], []
                    # One member per trajectory, held fixed for the entire dream.
                    # This preserves coherent model bias instead of averaging into
                    # unrealistically smooth dynamics or resampling noise each step.
                    im_member_ids = torch.randint(
                        0, args.lewm_ensemble_size, (B_im,), device=device
                    )
                    im_batch_ids = torch.arange(B_im, device=device)
                    trajectory_supported = torch.ones(B_im, dtype=torch.bool, device=device)
                    ...
                    for t in range(H_im):
                        ...
                        next_s_all, reward_logits_all, cont_logits_all = (
                            agent.world_model.imagine_step_all_from_history(latent_hist, dyn_actions)
                        )
                        reward_members = reward_logits_to_scalar(reward_logits_all)
                        latent_member_mean = next_s_all[:, :, obs_start:].mean(0)
                        latent_disagreement = (
                            next_s_all[:, :, obs_start:] - latent_member_mean.unsqueeze(0)
                        ).square().mean(dim=(0, -1, -2))
                        reward_disagreement = reward_members.var(dim=0, unbiased=False)
                        continue_members = torch.sigmoid(cont_logits_all)
                        continue_disagreement = continue_members.var(dim=0, unbiased=False)
                        component_disagreements = [
                            latent_disagreement, reward_disagreement, continue_disagreement
                        ]
                        # Direct final-heldout disagreement quantiles. The relative
                        # values are a diagnostic only; support compares raw member
                        # variance to the matching raw heldout quantile. Continuation
                        # is omitted when heldout data lacks both classes.
                        component_relative_disagreements = [
                            component / max(threshold, 1e-12)
                            for threshold, component in zip(
                                real_support_thresholds, component_disagreements
                            )
                        ]
                        active_relative = [
                            relative
                            for active, relative in zip(
                                real_support_components_active,
                                component_relative_disagreements,
                            )
                            if active
                        ]
                        disagreement = torch.stack(active_relative).amax(dim=0)
                        step_supported = torch.stack(
                            [
                                component <= threshold
                                for active, threshold, component in zip(
                                    real_support_components_active,
                                    real_support_thresholds,
                                    component_disagreements,
                                )
                                if active
                            ]
                        ).all(dim=0)
                        trajectory_supported &= step_supported
                        next_s = next_s_all[im_member_ids, im_batch_ids]
                        reward_logits = reward_logits_all[im_member_ids, im_batch_ids]
                        cont_logits = cont_logits_all[im_member_ids, im_batch_ids]
                        ...
                        im_supports.append(trajectory_supported.clone())
                        im_disagreements.append(disagreement)
                    ...
                    im_support = torch.stack(im_supports, dim=0)                # prefix support (H,B)
                    im_disagreement = torch.stack(im_disagreements, dim=0)
                    ...
                    for t in reversed(range(H_im)):
                        nonterminal = im_continues[t] * im_support[t].to(im_continues.dtype)
                        delta = im_rewards[t] + args.gamma * nonterminal * next_values[t] - values_t[t]
                        proposed_gae = delta + args.gamma * args.gae_lambda * nonterminal * lastgae
                        lastgae = torch.where(im_support[t], proposed_gae, torch.zeros_like(proposed_gae))
                        adv[t] = lastgae
                    ...
                    for h in range(mtp):
                        ...
                        return_mtp_mask_im[:valid_len, :, h] = im_support[h : h + valid_len]
                    ...
                    f_support = im_support.reshape(-1)
                    accepted_transition_count = int(f_support.sum().item())
                    required_transition_count = max(256, (H_im * B_im + 4) // 5)
                    initial_support_fraction = float(im_support[0].float().mean().item())
                    mean_supported_prefix = float(im_support.float().sum(dim=0).mean().item())
                    enough_transitions = accepted_transition_count >= required_transition_count
                    enough_initial_support = initial_support_fraction >= 0.20
                    enough_prefix = mean_supported_prefix >= 2.0
                    dream_block_admitted = bool(
                        enough_transitions and enough_initial_support and enough_prefix
                    )
                    supported_flat_inds = f_support.nonzero(as_tuple=False).squeeze(-1)
                    if dream_block_admitted:
                        dream_anchor_n = min(4096, supported_flat_inds.numel())
                        dream_anchor_pos = torch.linspace(
                            0,
                            supported_flat_inds.numel() - 1,
                            dream_anchor_n,
                            device=device,
                        ).long()
                        dream_anchor_inds = supported_flat_inds[dream_anchor_pos]
                        dream_anchor_agent_inputs = f_ag[dream_anchor_inds]
                        dream_anchor_explore_codes = f_explore_code[dream_anchor_inds]
                        dream_anchor_alpha, dream_anchor_beta = agent.get_beta_params_from_agent_input(
                            dream_anchor_agent_inputs, dream_anchor_explore_codes
                        )
                    else:
                        dream_anchor_inds = None
                    consumed_mask = im_support.to(returns_t.dtype)
                    consumed_denom = consumed_mask.sum().clamp_min(1.0)
                    im_return_sum += (returns_t * consumed_mask).sum() / consumed_denom
                    im_return_sq_sum += (returns_t.square() * consumed_mask).sum() / consumed_denom
                    im_reward_sum += (im_rewards * consumed_mask).sum() / consumed_denom
                    im_reward_sq_sum += (im_rewards.square() * consumed_mask).sum() / consumed_denom
                    im_continue_sum += (im_continues * consumed_mask).sum() / consumed_denom
                    im_support_sum += consumed_mask.mean()
                    im_support_depth_sum += consumed_mask.sum(dim=0).mean()
                    im_initial_support_sum += consumed_mask[0].mean()
                    im_disagreement_sum += im_disagreement.mean()
                ...
                # Hard all-or-nothing admission. Do not even build an actor/critic
                # backward graph for zero or marginal support: an AdamW step would
                # otherwise move both heads through momentum/weight decay despite a
                # tiny masked batch. The three tests constrain volume, first-step
                # breadth, and useful absorbing-prefix depth independently.
                if not dream_block_admitted:
                    dream_blocks_skipped += 1
                    dream_blocks_low_count += int(not enough_transitions)
                    dream_blocks_low_initial += int(not enough_initial_support)
                    dream_blocks_short_prefix += int(not enough_prefix)
                    im_pg_losses.append(0.0)
                    im_v_losses.append(0.0)
                    im_entropies.append(0.0)
                    im_actor_gns.append(0.0)
                    im_critic_gns.append(0.0)
                    continue
                dream_blocks_admitted += 1

                @torch.no_grad()
                def measure_supported_dream_beta_kl():
                    if dream_anchor_inds is None:
                        return torch.zeros((), device=device)
                    measured_alpha, measured_beta = agent.get_beta_params_from_agent_input(
                        dream_anchor_agent_inputs, dream_anchor_explore_codes
                    )
                    anchor_dist = Beta(dream_anchor_alpha, dream_anchor_beta)
                    measured_dist = Beta(measured_alpha, measured_beta)
                    return torch.distributions.kl_divergence(
                        anchor_dist, measured_dist
                    ).sum(dim=-1).mean()
                ...
                supported_count = f_support.sum()
                if args.norm_adv and supported_count > 1:
                    supported_adv = shaped[f_support]
                    shaped = (shaped - supported_adv.mean()) / (supported_adv.std() + 1e-8)
                clip_hi = args.clip_coef if args.clip_coef_high is None else args.clip_coef_high
                pg_loss1 = -shaped * ratio
                pg_loss2 = -shaped * torch.clamp(ratio, 1 - args.clip_coef, 1 + clip_hi)
                actor_mask = f_support.to(pg_loss1.dtype)
                pg_loss_im = (
                    torch.max(pg_loss1, pg_loss2) * actor_mask
                ).sum() / actor_mask.sum().clamp_min(1.0)
                value_log_probs_im = torch.log_softmax(value_logits, dim=-1)
                value_ce_im = -(f_target * value_log_probs_im).sum(dim=-1)
                critic_mask = f_target_mask.to(value_ce_im.dtype)
                v_loss_im = (value_ce_im * critic_mask).sum() / critic_mask.sum().clamp_min(1.0)
                entropy_loss_im = (entropy * actor_mask).sum() / actor_mask.sum().clamp_min(1.0)
                attempted_dream_actor_update = dream_actor_active
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.step()
                    if attempted_dream_actor_update:
                        post_dream_anchor_kl = measure_real_anchor_beta_kl()
                        post_supported_dream_kl = measure_supported_dream_beta_kl()
                        ...
                        if (
                            ...
                            or not torch.isfinite(post_supported_dream_kl)
                            or post_supported_dream_kl > args.dream_anchor_kl_budget
                        ):
            ...
            final_anchor_beta_kl = measure_real_anchor_beta_kl()
            ...
            imagine_metrics = {
                ...
                "imagine/support_fraction": (im_support_sum / n_im).item(),
                "imagine/support_depth": (im_support_depth_sum / n_im).item(),
                "imagine/initial_support_fraction": (im_initial_support_sum / n_im).item(),
                "imagine/disagreement_mean": (im_disagreement_sum / n_im).item(),
                ...
                "imagine/blocks_admitted": float(dream_blocks_admitted),
                "imagine/blocks_skipped": float(dream_blocks_skipped),
                "imagine/blocks_low_transition_count": float(dream_blocks_low_count),
                "imagine/blocks_low_initial_support": float(dream_blocks_low_initial),
                "imagine/blocks_short_prefix": float(dream_blocks_short_prefix),
            }
            for component_idx, component_name in enumerate(("latent", "reward", "continue")):
                imagine_metrics[f"imagine/{component_name}_support_threshold"] = float(
                    real_support_thresholds[component_idx]
                )
                imagine_metrics[f"imagine/{component_name}_support_active"] = float(
                    real_support_components_active[component_idx]
                )
        ...
        writer.add_scalar(
            "losses/real_subbudget_exhausted", float(not real_actor_active), global_step
        )
        writer.add_scalar(
            "losses/trust_region_exhausted", float(not dream_actor_active), global_step
        )
        writer.add_scalar(
        ...
        if wm_losses:
            ...
            if current_calibration_correlations is not None:
                for component_idx, component_name in enumerate(("latent", "reward", "continue")):
                    writer.add_scalar(
                        f"lewm/calibration_{component_name}_corr",
                        current_calibration_correlations[component_idx],
                        global_step,
                    )
                    writer.add_scalar(
                        f"lewm/calibration_{component_name}_residual",
                        current_teacher_residual[component_idx],
                        global_step,
                    )
                    writer.add_scalar(
                        f"lewm/calibration_{component_name}_tail_capture",
                        current_calibration_tail_captures[component_idx],
                        global_step,
                    )
                    writer.add_scalar(
                        f"lewm/calibration_{component_name}_disagreement_q",
                        current_support_quantiles[component_idx],
                        global_step,
                    )
                    writer.add_scalar(
                        f"lewm/calibration_{component_name}_active",
                        float(current_support_components_active[component_idx]),
                        global_step,
                    )
            total_probe_weight = wm_reward_probe_weights.sum()
            if total_probe_weight.item() > 0:
