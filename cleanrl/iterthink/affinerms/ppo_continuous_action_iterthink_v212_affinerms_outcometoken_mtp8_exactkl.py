# PPO + IterThink v212 affine-RMS outcome-token MTP8.
#
# Preserve v198's raw affine-RMS spatial encoder, attached encoder-side SIGReg,
# action-free per-observation temporal memory, outbound transition decoder,
# exact-Beta PPO/KL, persistent 4D/64 exploration, and imagination. Add a
# WM-owned read-only control token: it reads encoded and temporal observation
# slots plus shifted incoming a_(t-1), mixes causally through time, then rereads
# the current temporal slots. It never writes into the state stream and has no
# state-reconstruction/SPR loss. Real replay trains it only through independent
# H=8 reward-categorical and continuation heads conditioned on actual outbound
# a_t (future actions remain hidden). PPO receives detached [v198 memory, control]
# and zero-initialized actor/critic control residual adapters. Initial policy and
# value behavior is bit-exact to v198 while preserving strict WM/agent gradient
# isolation. A direct raw-state critic side channel is intentionally omitted: the
# inspected D4 HalfCheetah path does not train its apparent embedder and dreams
# cannot supply raw state, so copying it would create conflicting critic modes.
#
# -----------------------------------------------------------------------------
...
class Args:
    ...
    control_outcome_horizon: int = 8 # v212: separate real-only reward/continuation MTP horizon
    # Each head is normalized on its own valid rows, then horizons are averaged;
    # these coefficients therefore retain roughly v198's one-outcome-head scale.
    control_reward_loss_coef: float = 0.5
    control_continuation_loss_coef: float = 0.5
...
class _RoPEAttention(nn.Module):
    ...
    def forward(self, x, causal=True, key_valid=None):
        ...
        if key_valid is None:
            ...
            if key_valid.shape != (batch, seq_len):
                raise ValueError("temporal key validity mask has the wrong shape")
            allowed = key_valid[:, None, None, :].bool()
            if causal:
                causal_allowed = torch.ones(
                    seq_len, seq_len, dtype=torch.bool, device=x.device
                ).tril()
                allowed = allowed & causal_allowed.view(1, 1, seq_len, seq_len)
...
class ControlQueryBlock(nn.Module):
    """One-way learned query: control reads memory; memory is never updated."""
    ...
    def forward(self, query, memory):
        ...
        attn_out, _ = self.cross_attn(
            self.query_norm(query), memory_n, memory_n, need_weights=False
        )
        query = query + attn_out
        return query + self.w2(relu_sq(self.w1(self.ffn_norm(query))))


class ControlTemporalBlock(nn.Module):
    """Causal history for the single control slot with episode-safe key masking."""
    ...
    def forward(self, control, frame_valids):
        valid = frame_valids.unsqueeze(-1).to(control.dtype)
        control = control * valid
        control = control + self.attn_scale.view(1, 1, -1).to(control.dtype) * self.attn(
            self.attn_norm(control), causal=True, key_valid=frame_valids
        )
        control = control * valid
        control = control + self.ffn_scale.view(1, 1, -1).to(control.dtype) * self.w2(
            relu_sq(self.w1(self.ffn_norm(control)))
        )
        return control * valid


class OutcomeControlStream(nn.Module):
    """Read-only D4-style local-read -> causal-time -> final-read control token."""

    def __init__(self, dim, act_dim, num_heads, ffn_mult, rope_fraction):
        super().__init__()
        self.dim = dim
        self.query = nn.Parameter(torch.empty(1, 1, dim))
        nn.init.xavier_uniform_(self.query)
        self.incoming_action = xavier_linear(nn.Linear(act_dim, dim, bias=False))
        self.local_read = ControlQueryBlock(dim, num_heads, ffn_mult)
        self.temporal = ControlTemporalBlock(dim, num_heads, ffn_mult, rope_fraction)
        self.final_read = ControlQueryBlock(dim, num_heads, ffn_mult)

    def forward(self, encoded_states, temporal_states, previous_actions, frame_valids=None):
        batch, time_len, num_tokens, width = temporal_states.shape
        if encoded_states.shape != temporal_states.shape:
            raise ValueError("encoded and temporal control memories must match")
        if previous_actions.shape[:2] != (batch, time_len):
            raise ValueError("incoming actions must align with state frames")
        if frame_valids is None:
            frame_valids = torch.ones(
                batch, time_len, dtype=torch.bool, device=temporal_states.device
            )
        elif frame_valids.shape != (batch, time_len):
            raise ValueError("control validity mask has the wrong shape")

        action_token = self.incoming_action(previous_actions).unsqueeze(2)
        local_memory = torch.cat([encoded_states, temporal_states, action_token], dim=2)
        flat_memory = local_memory.reshape(batch * time_len, 2 * num_tokens + 1, width)
        query = self.query.to(flat_memory.dtype).expand(batch * time_len, -1, -1)
        control = self.local_read(query, flat_memory).reshape(batch, time_len, width)
        control = self.temporal(control, frame_valids)

        final_query = control.reshape(batch * time_len, 1, width)
        final_memory = temporal_states.reshape(batch * time_len, num_tokens, width)
        control = self.final_read(final_query, final_memory).reshape(batch, time_len, width)
        return control * frame_valids.unsqueeze(-1).to(control.dtype)


class ControlOutcomeMTPHeads(nn.Module):
    """Actual-a_t-conditioned independent future reward/continuation heads."""

    def __init__(self, dim, act_dim, horizon, reward_num_bins, ffn_mult):
        super().__init__()
        self.horizon = horizon
        self.action_embed = xavier_linear(nn.Linear(act_dim, dim, bias=False))
        self.fuse1 = xavier_linear(nn.Linear(2 * dim, dim * ffn_mult, bias=False))
        self.fuse2 = xavier_linear(nn.Linear(dim * ffn_mult, dim, bias=False))
        self.reward_heads = nn.ModuleList(
            [xavier_linear(nn.Linear(dim, reward_num_bins)) for _ in range(horizon)]
        )
        self.continuation_heads = nn.ModuleList(
            [xavier_linear(nn.Linear(dim, 1)) for _ in range(horizon)]
        )

    def forward(self, control, outbound_action):
        action = self.action_embed(outbound_action)
        fused = self.fuse2(relu_sq(self.fuse1(torch.cat([control, action], dim=-1))))
        reward_logits = torch.stack([head(fused) for head in self.reward_heads], dim=-2)
        continuation_logits = torch.cat(
            [head(fused) for head in self.continuation_heads], dim=-1
        )
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # Forked construction preserves every v198 parameter and global RNG draw.
        with torch.random.fork_rng():
            torch.manual_seed(args.seed + 70000)
            self.control_stream = OutcomeControlStream(
                self.dim,
                act_dim,
                args.lewm_heads,
                args.lewm_ffn_mult,
            ...
            self.control_outcome_heads = ControlOutcomeMTPHeads(
                self.dim,
                act_dim,
                args.control_outcome_horizon,
                self.reward_num_bins,
                args.lewm_ffn_mult,
            )
    ...
    def belief_and_control_from_history(
        self, latent_history, previous_actions, frame_valids=None, return_sequence=False
    ):
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
            previous_actions = previous_actions[:, -self.context :]
            if frame_valids is not None:
                frame_valids = frame_valids[:, -self.context :]
        temporal_states = self._temporal_state_trunk(latent_history)
        control = self.control_stream(
            latent_history, temporal_states, previous_actions, frame_valids
        )
        if return_sequence:
            return temporal_states, control
        return temporal_states[:, -1], control[:, -1]

    def control_outcomes(self, control, outbound_action):
        return self.control_outcome_heads(control, outbound_action)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Preserve v198's flat detached memory verbatim and append one detached
        # WM-owned control vector. Zero residual adapters make the extra vector
        # exactly inert at initialization.
        self.agent_latent_dim = obs_dim * args.lewm_dim
        self.control_dim = args.lewm_dim
        self.agent_input_dim = self.agent_latent_dim + self.control_dim
        ...
        with torch.random.fork_rng():
            torch.manual_seed(args.seed + 71000)
            self.actor_control_residual = layer_init(
                nn.Linear(self.control_dim, args.lewm_dim, bias=False), std=0.0
            )
            self.critic_control_residual = layer_init(
                nn.Linear(self.control_dim, args.lewm_dim, bias=False), std=0.0
            )
    ...
    def pack_agent_input(self, latent, control):
        if self.detach_world_model_from_agent:
            latent = latent.detach()
            control = control.detach()
        return torch.cat([latent.reshape(latent.shape[0], -1), control], dim=-1)
    ...
    def split_agent_input(self, agent_input):
        ...
        return agent_input[..., : self.agent_latent_dim], agent_input[..., self.agent_latent_dim :]

    def agent_input_from_latent(self, latent, control=None):
        if control is None:
            control = latent.new_zeros(latent.shape[0], self.control_dim)
        return self.pack_agent_input(latent, control)

    def agent_input_from_history(self, latent_history, previous_actions=None, frame_valids=None):
        if previous_actions is None:
            previous_actions = latent_history.new_zeros(
                latent_history.shape[0], latent_history.shape[1], self.world_model.act_dim
            )
        belief, control = self.world_model.belief_and_control_from_history(
            latent_history, previous_actions, frame_valids
        )
        return self.pack_agent_input(belief, control)
    ...
    def _trunks_from_agent_input(self, obs, agent_input):
        ...
        return self._actor_feat_from_agent_input(agent_input), self._critic_feat_from_agent_input(obs, agent_input)

    def _actor_feat_from_agent_input(self, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        memory, control = self.split_agent_input(agent_input)
        trunk_input = self.actor_readout(memory) + self.actor_control_residual(control)
        return self.actor_trunk(trunk_input)

    def _critic_feat_from_agent_input(self, obs, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        memory, control = self.split_agent_input(agent_input)
        trunk_input = self.critic_readout(memory) + self.critic_control_residual(control)
    ...
    def get_value(self, x):
        ...
        critic_feat = self._critic_feat_from_agent_input(x, self.agent_input_from_obs(x))
    ...
    def get_value_from_agent_input(self, obs, agent_input):
        critic_feat = self._critic_feat_from_agent_input(obs, agent_input)
        return self.critic_head(critic_feat).view(-1, self.critic_mtp_horizon, self.num_bins)
    ...
    def actor_parameters(self):
        ...
        return (
            ...
            + list(self.actor_control_residual.parameters())
    ...
    def critic_parameters(self):
        ...
        return (
            ...
            + list(self.critic_control_residual.parameters())
            + list(self.critic_trunk.parameters())
            + list(self.critic_head.parameters())
        )

    @torch.no_grad()
    def assert_zero_residual_init(self):
        for name, module in (
            ("actor control", self.actor_control_residual),
            ("critic control", self.critic_control_residual),
        ):
            if torch.count_nonzero(module.weight).item() != 0:
                raise AssertionError(f"{name} residual must initialize exactly zero")
            if module.bias is not None and torch.count_nonzero(module.bias).item() != 0:
                raise AssertionError(f"{name} residual bias must initialize exactly zero")
...
if __name__ == "__main__":
    ...
    assert args.lewm_mtp_len == 1, "v212 keeps state prediction one-step only"
    assert args.control_outcome_horizon == 8, "v212 control outcomes are fixed H=8"
    assert args.control_outcome_horizon <= args.num_steps
    assert args.detach_world_model_from_agent, "v212 requires PPO to be detached from WM/control"
    ...
    agent.assert_zero_residual_init()
    ...
    control_wm_params = (
        list(agent.world_model.control_stream.parameters())
        + list(agent.world_model.control_outcome_heads.parameters())
    )
    control_wm_param_ids = {id(p) for p in control_wm_params}
    base_wm_params = [p for p in wm_latent_params if id(p) not in control_wm_param_ids]

    def unclipped_grad_norm(params):
    ...
    control_diag_generator = torch.Generator(device=device)
    control_diag_generator.manual_seed(args.seed + 72000)
    ...
    def build_belief_agent_input(obs_tensor):
        ...
        oldest_incoming = torch.zeros(
            obs_tensor.shape[0], agent.world_model.act_dim,
            device=obs_tensor.device, dtype=current_latent.dtype,
        )
        # Retain one extra source transition so the oldest state in a full
        # sliding context keeps its true incoming action. At rollout start or
        # after reset, the unavailable/cross-boundary action remains BOS zero.
        if n_past > 0 and len(rollout_action_history) > n_past:
            source_idx = -(n_past + 1)
            source_valid = rollout_history_valid[source_idx]
            oldest_incoming = rollout_action_history[source_idx] * source_valid.view(
                -1, 1
            ).to(current_latent.dtype)
        previous_actions = torch.stack([oldest_incoming] + past_actions, dim=1)
        current_valid = torch.ones(
            obs_tensor.shape[0], dtype=torch.bool, device=obs_tensor.device
        )
        frame_valids = torch.stack(past_valids + [current_valid], dim=1)
        return (
            agent.agent_input_from_history(
                belief_latents, previous_actions, frame_valids
            ),
            current_latent,
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if len(rollout_latent_history) > args.lewm_context:
                rollout_latent_history = rollout_latent_history[-args.lewm_context :]
                rollout_action_history = rollout_action_history[-args.lewm_context :]
                rollout_history_valid = rollout_history_valid[-args.lewm_context :]
        ...
        wm_sigreg_losses = []
        control_reward_losses = []
        control_continuation_losses = []
        control_outbound_shuffle_degradations = []
        control_incoming_shuffle_degradations = []
        control_wm_grad_norms = []
        base_wm_grad_norms = []
        wm_grad_norms = []
        ...
        control_reward_abs_error_sums = torch.zeros(args.control_outcome_horizon, device=device)
        control_reward_error_sums = torch.zeros(args.control_outcome_horizon, device=device)
        control_probe_weights = torch.zeros(args.control_outcome_horizon, device=device)
        ...
        if (
            ...
            or args.control_reward_loss_coef > 0.0
            or args.control_continuation_loss_coef > 0.0
        ):
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # v212 control representation uses the actual causal context
                    # ending at s_t. Its shifted action rows are [BOS, ..., a_(t-1)];
                    # the current outbound a_t is withheld until the outcome heads.
                    control_context = min(agent.world_model.context, args.num_steps)
                    context_lags = torch.arange(
                        control_context - 1, -1, -1, device=device
                    )
                    context_steps = mb_step_inds[:, None] - context_lags[None, :]
                    context_in_rollout = context_steps >= 0
                    safe_context_steps = context_steps.clamp(0, max_start)
                    context_envs = mb_env_inds[:, None].expand_as(safe_context_steps)
                    context_valid = context_in_rollout.clone()
                    # dones[j] marks that s_j starts a new episode. A historical
                    # row is valid for current s_t only if no later row through t
                    # is an episode start.
                    for context_pos in range(control_context - 1):
                        for later_pos in range(context_pos + 1, control_context):
                            context_valid[:, context_pos] &= (
                                dones[
                                    safe_context_steps[:, later_pos],
                                    mb_env_inds,
                                ]
                                == 0
                            )
                    context_summaries = agent.world_model.encode_summary(
                        obs[safe_context_steps, context_envs].reshape((-1,) + obs_shape)
                    ).reshape(
                        mb_size,
                        control_context,
                        agent.world_model.num_latent_tokens,
                    ...
                    control_latents = context_summaries[:, :, obs_start:]
                    control_latents = control_latents * context_valid[..., None, None].to(
                        control_latents.dtype
                    )
                    control_incoming_actions = future_actions.new_zeros(
                        mb_size, control_context, future_actions.shape[-1]
                    )
                    oldest_source_steps = context_steps[:, 0] - 1
                    oldest_source_available = oldest_source_steps >= 0
                    safe_oldest_source_steps = oldest_source_steps.clamp(0, max_start)
                    oldest_source_valid = (
                        oldest_source_available
                        & context_valid[:, 0]
                        & (
                            dones[
                                safe_context_steps[:, 0],
                                mb_env_inds,
                            ]
                            == 0
                        )
                    )
                    control_incoming_actions[:, 0] = actions[
                        safe_oldest_source_steps, mb_env_inds
                    ] * oldest_source_valid.unsqueeze(-1).to(future_actions.dtype)
                    if control_context > 1:
                        shifted_actions = actions[
                            safe_context_steps[:, :-1], context_envs[:, :-1]
                        ]
                        shifted_valid = context_valid[:, :-1] & context_valid[:, 1:]
                        control_incoming_actions[:, 1:] = shifted_actions * shifted_valid.unsqueeze(-1).to(
                            shifted_actions.dtype
                        )
                    control_temporal_states, control_sequence = agent.world_model.belief_and_control_from_history(
                        control_latents,
                        control_incoming_actions,
                        context_valid,
                        return_sequence=True,
                    )
                    control_t = control_sequence[:, -1]

                    outcome_horizon = args.control_outcome_horizon
                    outcome_offsets = torch.arange(outcome_horizon, device=device)
                    outcome_steps = mb_step_inds[:, None] + outcome_offsets[None, :]
                    outcome_in_rollout = outcome_steps < args.num_steps
                    safe_outcome_steps = outcome_steps.clamp(max=max_start)
                    outcome_envs = mb_env_inds[:, None].expand_as(safe_outcome_steps)
                    outcome_rewards = rewards[safe_outcome_steps, outcome_envs]
                    outcome_terminations = transition_terminations[
                        safe_outcome_steps, outcome_envs
                    ]
                    outcome_boundaries = transition_boundaries[
                        safe_outcome_steps, outcome_envs
                    ]
                    outcome_valid = outcome_in_rollout.clone()
                    for outcome_offset in range(1, outcome_horizon):
                        for prior_offset in range(outcome_offset):
                            outcome_valid[:, outcome_offset] &= (
                                outcome_boundaries[:, prior_offset] == 0
                            )

                    control_reward_logits, control_continuation_logits = (
                        agent.world_model.control_outcomes(
                            control_t, actions[mb_step_inds, mb_env_inds]
                        )
                    )
                    control_reward_targets = reward_support.project(outcome_rewards)
                    control_reward_ce = -(
                        control_reward_targets.detach()
                        * torch.log_softmax(control_reward_logits, dim=-1)
                    ).sum(dim=-1)
                    control_continuation_bce = F.binary_cross_entropy_with_logits(
                        control_continuation_logits,
                        1.0 - outcome_terminations,
                        reduction="none",
                    )
                    control_reward_per_horizon = []
                    control_cont_per_horizon = []
                    for outcome_offset in range(outcome_horizon):
                        valid_h = outcome_valid[:, outcome_offset].to(control_reward_ce.dtype)
                        denom_h = valid_h.sum().clamp_min(1.0)
                        control_reward_per_horizon.append(
                            (control_reward_ce[:, outcome_offset] * valid_h).sum() / denom_h
                        )
                        control_cont_per_horizon.append(
                            (control_continuation_bce[:, outcome_offset] * valid_h).sum() / denom_h
                        )
                    control_reward_loss = torch.stack(control_reward_per_horizon).mean()
                    control_continuation_loss = torch.stack(control_cont_per_horizon).mean()

                    with torch.no_grad():
                        control_reward_scalar = reward_logits_to_scalar(control_reward_logits)
                        control_reward_error = control_reward_scalar - outcome_rewards
                        for outcome_offset in range(outcome_horizon):
                            valid_h = outcome_valid[:, outcome_offset].to(control_reward_error.dtype)
                            control_reward_abs_error_sums[outcome_offset] += (
                                control_reward_error[:, outcome_offset].abs() * valid_h
                            ).sum()
                            control_reward_error_sums[outcome_offset] += (
                                control_reward_error[:, outcome_offset] * valid_h
                            ).sum()
                            control_probe_weights[outcome_offset] += valid_h.sum()

                        shuffle = torch.randperm(
                            mb_size, device=device, generator=control_diag_generator
                        )
                        shuffled_reward_logits, shuffled_cont_logits = (
                            agent.world_model.control_outcomes(
                                control_t, actions[mb_step_inds, mb_env_inds][shuffle]
                            )
                        )
                        shuffled_reward_ce = -(
                            control_reward_targets
                            * torch.log_softmax(shuffled_reward_logits, dim=-1)
                        ).sum(dim=-1)
                        shuffled_cont_bce = F.binary_cross_entropy_with_logits(
                            shuffled_cont_logits,
                            1.0 - outcome_terminations,
                            reduction="none",
                        )
                        valid_float = outcome_valid.to(shuffled_reward_ce.dtype)
                        valid_denom = valid_float.sum().clamp_min(1.0)
                        true_joint = (
                            (control_reward_ce + control_continuation_bce) * valid_float
                        ).sum() / valid_denom
                        shuffled_joint = (
                            (shuffled_reward_ce + shuffled_cont_bce) * valid_float
                        ).sum() / valid_denom
                        control_outbound_shuffle_degradations.append(
                            (shuffled_joint - true_joint).item()
                        )

                        if not control_incoming_shuffle_degradations:
                            shuffled_incoming_control_seq = agent.world_model.control_stream(
                                control_latents,
                                control_temporal_states,
                                control_incoming_actions[shuffle],
                                context_valid,
                            )
                            shuffled_incoming_reward, shuffled_incoming_cont = (
                                agent.world_model.control_outcomes(
                                    shuffled_incoming_control_seq[:, -1],
                                    actions[mb_step_inds, mb_env_inds],
                                )
                            )
                            shuffled_incoming_reward_ce = -(
                                control_reward_targets
                                * torch.log_softmax(shuffled_incoming_reward, dim=-1)
                            ).sum(dim=-1)
                            shuffled_incoming_cont_bce = F.binary_cross_entropy_with_logits(
                                shuffled_incoming_cont,
                                1.0 - outcome_terminations,
                                reduction="none",
                            )
                            shuffled_incoming_joint = (
                                (shuffled_incoming_reward_ce + shuffled_incoming_cont_bce)
                                * valid_float
                            ).sum() / valid_denom
                            control_incoming_shuffle_degradations.append(
                                (shuffled_incoming_joint - true_joint).item()
                            )
                    ...
                    wm_loss = (
                        ...
                        + args.control_reward_loss_coef * control_reward_loss
                        + args.control_continuation_loss_coef * control_continuation_loss
                    )
                    if wm_accum_count == 0:
                    ...
                    if wm_accum_count >= wm_accum_steps or end >= args.batch_size:
                        # One representative pre-clip split is enough to detect
                        # scale mismatch without adding two CUDA syncs per WM step.
                        if not control_wm_grad_norms:
                            control_wm_grad_norms.append(
                                float(unclipped_grad_norm(control_wm_params))
                            )
                            base_wm_grad_norms.append(
                                float(unclipped_grad_norm(base_wm_params))
                            )
                        # Keep the new branch's private parameters from consuming
                        # v198's base-WM norm budget. Shared encoder/temporal gradients
                        # remain in base_wm_params, so the attached outcome objective
                        # still shapes the representation as intended.
                        wm_gn = nn.utils.clip_grad_norm_(base_wm_params, args.lewm_grad_clip)
                        nn.utils.clip_grad_norm_(control_wm_params, args.lewm_grad_clip)
                    ...
                    control_reward_losses.append(control_reward_loss.item())
                    control_continuation_losses.append(control_continuation_loss.item())

        def log_control_outcome_diagnostics():
            if not control_reward_losses:
                return
            writer.add_scalar(
                "control/reward_ce_h8mean",
                float(np.mean(control_reward_losses)),
                global_step,
            )
            writer.add_scalar(
                "control/continuation_bce_h8mean",
                float(np.mean(control_continuation_losses)),
                global_step,
            )
            if control_outbound_shuffle_degradations:
                writer.add_scalar(
                    "control/outbound_action_shuffle_loss_degradation",
                    float(np.mean(control_outbound_shuffle_degradations)),
                    global_step,
                )
            if control_incoming_shuffle_degradations:
                writer.add_scalar(
                    "control/incoming_action_shuffle_loss_degradation",
                    float(np.mean(control_incoming_shuffle_degradations)),
                    global_step,
                )
            if control_wm_grad_norms and base_wm_grad_norms:
                control_gn = float(np.mean(control_wm_grad_norms))
                base_gn = float(np.mean(base_wm_grad_norms))
                writer.add_scalar("control/wm_control_grad_norm", control_gn, global_step)
                writer.add_scalar("control/wm_base_grad_norm", base_gn, global_step)
                writer.add_scalar(
                    "control/wm_control_to_base_grad_ratio",
                    control_gn / max(base_gn, 1e-12),
                    global_step,
                )
            for offset in (0, 3, 7):
                weight = control_probe_weights[offset]
                if weight.item() <= 0:
                    continue
                writer.add_scalar(
                    f"control/reward_mae_h{offset + 1}",
                    (control_reward_abs_error_sums[offset] / weight).item(),
                    global_step,
                )
                writer.add_scalar(
                    f"control/reward_bias_h{offset + 1}",
                    (control_reward_error_sums[offset] / weight).item(),
                    global_step,
                )
        ...
        if world_model_only:
            ...
            if wm_losses:
                log_control_outcome_diagnostics()
        ...
        ppo_wm_grad_norm_max = 0.0

        def audit_wm_gradient_isolation():
            # The detached interface should leave every WM grad slot as None.
            # This host-side identity check has no CUDA reduction/synchronization
            # cost on the normal path; compute a norm only when isolation fails.
            leaked = [param for param in world_model_params if param.grad is not None]
            if leaked:
                grad_sq = torch.stack(
                    [param.grad.detach().float().square().sum() for param in leaked]
                ).sum()
                grad_norm = float(grad_sq.sqrt())
                raise RuntimeError(
                    f"PPO/value gradient leaked into WM/control (norm={grad_norm})"
                )
            return 0.0
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with torch.no_grad():
                    ...
                    if real_actor_active and (
                        not torch.isfinite(exact_beta_kl) or exact_beta_kl > args.target_kl
                    ):
                ...
                if args.separate_grad_clip:
                    ...
                    ppo_wm_grad_norm_max = max(
                        ppo_wm_grad_norm_max, audit_wm_gradient_isolation()
                    )
            ...
            for _ in range(args.imagine_batches):
                ...
                with torch.no_grad():
                    ...
                    if clean_flat.numel() > 0:
                        ...
                        oldest_steps = win_steps[0]
                        prior_steps = oldest_steps - 1
                        prior_available = prior_steps >= 0
                        safe_prior_steps = prior_steps.clamp_min(0)
                        oldest_is_continuing = (
                            bank_dones[rollout_idx, oldest_steps, env_idx] == 0
                        )
                        prior_valid = prior_available & oldest_is_continuing
                        control_prior_action = bank_actions[
                            rollout_idx, safe_prior_steps, env_idx
                        ] * prior_valid.unsqueeze(-1).to(bank_actions.dtype)
                    else:
                        ...
                        control_prior_action = torch.zeros(
                            B_im,
                            agent.world_model.act_dim,
                            device=device,
                            dtype=bank_actions.dtype,
                        )
                    for t in range(H_im):
                        latent_hist = torch.stack(hist_latents, dim=1)               # (B, t+1, obs_tok, dim)
                        # The control stream sees only shifted incoming actions. Build
                        # alignment over the full history before context cropping so a
                        # sliding window retains the known inbound action of its oldest row.
                        previous_actions = latent_hist.new_zeros(
                            B_im, latent_hist.shape[1], agent.world_model.act_dim
                        )
                        previous_actions[:, 0] = control_prior_action
                        if hist_actions:
                            previous_actions[:, 1:] = torch.stack(hist_actions, dim=1)
                        agent_input = agent.agent_input_from_history(
                            latent_hist, previous_actions
                        )
                    ...
                    if hist_actions:
                        previous_actions[:, 1:] = torch.stack(hist_actions, dim=1)
                    agent_input_H = agent.agent_input_from_history(
                        latent_hist, previous_actions
                    )
        ...
        with torch.no_grad():
            ...
            diag_memory, diag_control = agent.split_agent_input(b_agent_inputs[:diag_n])
            actor_base_input = agent.actor_readout(diag_memory)
            actor_control_delta = agent.actor_control_residual(diag_control)
            critic_base_input = agent.critic_readout(diag_memory)
            critic_control_delta = agent.critic_control_residual(diag_control)

            def residual_rms_ratio(delta, base):
                return (
                    delta.float().square().mean().sqrt()
                    / base.float().square().mean().sqrt().clamp_min(1e-8)
                )

            actor_control_ratio = residual_rms_ratio(
                actor_control_delta, actor_base_input
            )
            critic_control_ratio = residual_rms_ratio(
                critic_control_delta, critic_base_input
            )
        ...
        writer.add_scalar("control/actor_residual_rms_ratio", actor_control_ratio.item(), global_step)
        writer.add_scalar("control/critic_residual_rms_ratio", critic_control_ratio.item(), global_step)
        writer.add_scalar("control/ppo_wm_grad_norm", ppo_wm_grad_norm_max, global_step)
