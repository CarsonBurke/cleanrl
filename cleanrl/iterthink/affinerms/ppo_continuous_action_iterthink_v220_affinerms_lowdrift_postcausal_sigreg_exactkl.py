# PPO + IterThink v220 low-drift direct post-causal SIGReg.
#
# v219 fixed rank collapse, but its working-representation updates consumed the
# policy's trust budget before PPO could act. At 688k, causal effective/stable
# rank reached 143/41.5 with mean coordinate std 0.95, while pre-PPO behavior KL
# repeatedly reached 0.027-0.039 against target 0.03; actor update fraction was
# usually 0-0.03, return regressed to -60, value loss rose to 18.4, and explained
# variance was 0.37 versus v213's 0.92. Raw WM grad norm stayed 11-16 and clipped
# every update. Reward/continue were not the bottleneck: one-step reward MAE fell
# to 0.206, continuation BCE to 2.4e-4, and rank kept increasing.
#
# v220 retains v219's correct direct joint post-causal placement but reduces
# avoidable representation churn. SIGReg 0.09 -> 0.02 makes its weighted loss
# comparable to latent MSE instead of ~5x larger, while already-healthy rank has
# margin. A separate WM LR 1e-4 (agent remains 3e-4), annealed on the same schedule,
# reduces the 64 WM optimizer steps per rollout from monopolizing the exact-KL
# budget and destabilizing the critic interface. Both learning rates are logged.
#
# -----------------------------------------------------------------------------
# Previous header:
# PPO + IterThink v219 direct joint post-causal SIGReg.
#
# Previous header:
# PPO + IterThink v218 LeVJEPA-style post-causal SIGReg.
#
# Previous header:
...
class Args:
    ...
    lewm_sigreg_coef: float = 0.02    # v220: rank is healthy; balance direct SIGReg with latent MSE
...
class LeWMBackbone(nn.Module):
    ...
    def predict_mtp_from_history(self, latent_history, action_history, return_state_memory=False):
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
...
if __name__ == "__main__":
    ...
    def direct_causal_state_sigreg(state_memory, token_valids):
        # Regularize the representation recurrence and control actually consume:
        # final normalized causal state, before every projector/query head.
        valid_lengths = token_valids.bool().sum(dim=1)
        valid_sequences = valid_lengths > 0
        if int(valid_sequences.sum().item()) < args.lewm_sigreg_min_valid:
            return state_memory.sum() * 0.0

        batch_ids = torch.arange(state_memory.shape[0], device=state_memory.device)
        last_valid = (valid_lengths - 1).clamp_min(0)
        final_states = state_memory[batch_ids, last_valid][valid_sequences]
        joint_states = final_states.flatten(start_dim=1)

        # One exact sample-covariance spectrum per rollout update. This observes
        # the same unprojected population SIGReg constrains, not an off-path proxy.
        if not wm_causal_effective_ranks:
            with torch.no_grad():
                centered = joint_states.float() - joint_states.float().mean(dim=0, keepdim=True)
                ...
                stable_rank = spectrum_sum / spectrum.max().clamp_min(1e-12)
                coordinate_std = joint_states.float().std(dim=0, correction=0)
                wm_causal_effective_ranks.append(effective_rank.item())
                wm_causal_stable_ranks.append(stable_rank.item())
                wm_causal_mean_stds.append(coordinate_std.mean().item())
                wm_causal_min_stds.append(coordinate_std.min().item())
        ...
        return sigreg(joint_states.unsqueeze(0), A=A)
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            optimizer.param_groups[1]["lr"] = frac * args.lewm_learning_rate
        writer.add_scalar("charts/learning_rate", optimizer.param_groups[0]["lr"], global_step)
        writer.add_scalar("charts/lewm_learning_rate", optimizer.param_groups[1]["lr"], global_step)
        ...
        wm_sigreg_losses = []
        wm_causal_effective_ranks = []
        wm_causal_stable_ranks = []
        wm_causal_mean_stds = []
        wm_causal_min_stds = []
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    pred_mtp, state_memory = agent.world_model.predict_mtp_from_history(
                        teacher_history, future_actions, return_state_memory=True
                    )
                    ...
                    # v219: directly constrain the joint working causal state,
                    # after predictor_norm and before every query/projector head.
                    wm_sigreg_loss = direct_causal_state_sigreg(
                        state_memory,
                        latent_weight,
                    )
                    wm_loss = (
        ...
        def log_causal_rank_diagnostics():
            if not wm_causal_effective_ranks:
                return
            writer.add_scalar(
                "lewm/causal_state_effective_rank",
                float(np.mean(wm_causal_effective_ranks)),
                global_step,
            )
            writer.add_scalar(
                "lewm/causal_state_stable_rank",
                float(np.mean(wm_causal_stable_ranks)),
                global_step,
            )
            writer.add_scalar(
                "lewm/causal_state_mean_coordinate_std",
                float(np.mean(wm_causal_mean_stds)),
                global_step,
            )
            writer.add_scalar(
                "lewm/causal_state_min_coordinate_std",
                float(np.mean(wm_causal_min_stds)),
                global_step,
            )
