# PPO + IterThink v25 (SAC-FAITHFUL SQUASHED-GAUSSIAN MAX-ENTROPY). From v24.
#
# v25 is a SAC-alignment pass on v24's gaussian soft-max-entropy branch. We audited
# sac_continuous_action.py component-by-component and fixed the three places the
# on-policy port silently diverged from SAC's actual mechanism:
#
#   (1) MAGNITUDE PRESERVED. SAC's soft-RL guarantee is that alpha*H enters the
#       VALUE at full scale. v24 folded alpha*H into the policy advantage and then
#       ran it through `rankgauss`, which replaces each advantage by the Gaussian
#       quantile of its RANK -> the entropy magnitude is discarded and only a faint
#       reordering survives (v24's header even claimed "magnitude is incidental" --
#       that was the bug). v25 defaults adv_transform="clip_z" (winsorized z-score:
#       linear/magnitude-preserving in the bulk, outlier-safe) so alpha*H survives.
#   (2) SAC ENTROPY HORIZON. SAC bakes -alpha*logpi into EVERY Bellman backup, so
#       future entropy H(s_{t+1+j}) is discounted at gamma^{j+1}. v24's one-step GAE
#       injection weights it at (gamma*lambda)^l*gamma -- matching SAC only at
#       lambda=1. v25 runs the ENTROPY channel of the soft-advantage GAE at
#       ent_gae_lambda=1.0 (reward GAE keeps gae_lambda=0.95).
#   (3) CONSISTENT ESTIMATOR. SAC uses one fresh rsample logpi for actor loss AND
#       dual. v25 targets the dual on the SAME fresh rsample squashed entropy the
#       actor bonus maximizes (v24 used the replayed-z newlogprob). log-std bounds
#       tightened to SAC's [-5,2] (logvar [-10,4]; was std up to ~54).
#
# DELIBERATELY KEPT FROM v24 (verified correct, NOT SAC-misalignments): single
# categorical critic regressing the RAW entropy-free return (making it learn the
# soft value overflows the fixed support [v_min,v_max] -- the softboot failure:
# edge_mass->0.9); no twin-Q / no target net (on-policy lambda-returns don't
# max-bootstrap, so SAC's overestimation controls are unnecessary). Entropy is
# supplied to the POLICY only: future-entropy credit via the soft-advantage GAE,
# current-state entropy via the explicit alpha*H actor bonus. target_entropy=-|A|,
# SAC's exact dual. --no-auto-entropy => the plain v24 gaussian control.
#
...
class Args:
    ...
    alpha_lr: float = 1e-3
    ent_gae_lambda: float = 1.0     # v25 fix #2: lambda for the ENTROPY channel of the
    # soft-advantage GAE. SAC discounts future entropy at gamma^{j+1}; a one-step bonus
    # in GAE weights it at (gamma*lambda)^l*gamma, matching SAC ONLY at lambda=1. The
    # reward GAE keeps gae_lambda (0.95); only the entropy bonus uses this.
    vf_coef: float = 0.5
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # SOFT-ADVANTAGE (POLICY ONLY) = reward advantage + a SEPARATE entropy advantage.
            # GAE is linear in the per-step deltas, so the entropy bonus α·H(s') gets its OWN
            # recursion at ent_gae_lambda (fix #2) while the reward advantage keeps gae_lambda
            # (=0.95). The entropy-only delta is δ^e_t = γ·b_t·nonterminal with b_t=α·H(s_{t+1}),
            # giving ent_adv_t = Σ_l (γ·λ_e)^l·γ·α·H(s_{t+l+1}). At λ_e=1 this is Σ_l γ^{l+1}·α·H
            # = SAC's EXACT γ-discounted future entropy; mixing it into the combined recursion
            # at the reward λ would (a) under-count entropy AND (b) inflate the reward variance
            # to λ=1. clip_z at the optim step then preserves the entropy MAGNITUDE (fix #1).
            if auto_alpha:
                ent_adv = torch.zeros_like(rewards).to(device)
                lastgaelam = 0
                for t in reversed(range(args.num_steps)):
                    nextnonterminal = (1.0 - next_done) if t == args.num_steps - 1 else (1.0 - dones[t + 1])
                    delta_e = args.gamma * next_value_bonus[t] * nextnonterminal
                    ent_adv[t] = lastgaelam = delta_e + args.gamma * args.ent_gae_lambda * nextnonterminal * lastgaelam
                policy_adv = advantages + ent_adv
            else:
