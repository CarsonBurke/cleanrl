# PPO + IterThink v24 COLORED-OU exploration v1.
#
# METHOD (the ONLY change vs the ~9785 symlog_v1 base): replace the per-step
# i.i.d. Beta draw in the ROLLOUT with a temporally-correlated (colored / OU)
# draw, to inject coherent low-frequency action perturbations that survive over a
# stride. HalfCheetah reward = forward velocity = the time-integral of a near-
# periodic gait; white per-step noise has a flat spectrum and CANCELS over a
# stride, so it can never push the policy into the basin of a faster limit-cycle.
# An OU residual concentrates exploratory power at the gait fundamental, so
# consecutive steps push the same joints the same direction for ~1/(1-rho) steps,
# producing rollouts with a fat upper tail of genuinely-faster gaits that
# rankgauss promotes and the distributional critic represents.
#
# FORMULATION (rollout only; nothing about the policy being optimized changes):
#   Maintain a persistent per-env OU residual e in R^{num_envs x act_dim}, init 0.
#   Each rollout step, BEFORE env.step, with the SAME Beta(alpha,beta) heads:
#       mu_z   = alpha / (alpha + beta)                     # Beta mean, in (0,1)
#       s_beta = sqrt(alpha*beta / ((a+b)^2 (a+b+1)))       # Beta std, native z
#       e      = rho*e + sqrt(1-rho^2)*randn_like(e)        # unit-stationary-var OU
#       e      = e * (1 - next_done)[:,None]                # reset corr on episode end
#       z      = clamp(mu_z + k*s_beta*e, EPS, 1-EPS)       # colored draw
#   CRITICAL: the stored logprob is STILL Beta(alpha,beta).log_prob(z), so the PPO
#   importance ratio pi_new/pi_old is EXACT (the OU overlay has no parameters and
#   no gradient; it is a correlated PROPOSAL on a factored Beta target). ou_k=0
#   recovers z = mu_z exactly (the deterministic Beta-mean draw): a clean baseline
#   superset. rho in [0.9,0.97]; k=1 matches injected variance to the Beta's own.
#
# Heads, rankgauss advantage, HL-Gauss symlog critic, dual-backward PPO update,
# clip/target_kl are ALL untouched. Single-variable, additive, falsifiable.
#
# ---- base file header (symlog critic) ----
...
class Args:
    ...
    # Colored-OU exploration (THE variant). Replaces the i.i.d. Beta rollout draw
    # with z = clamp(mu_z + ou_k*s_beta*e, EPS, 1-EPS), e <- ou_rho*e +
    # sqrt(1-ou_rho^2)*randn (reset on episode boundary). logprob stays the exact
    # Beta logprob so the PPO ratio is unchanged. ou_k=0 => z=mu_z (Beta-mean draw,
    # baseline superset). Only active for actor_dist="beta".
    ou_rho: float = 0.95             # OU correlation; ~1/(1-rho) step coherence (~20 steps)
    ou_k: float = 1.0                # injected std as a multiple of the Beta's own std
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, z=None, ou_e=None, ou_k=0.0):
        ...
        #
        # Colored-OU draw (beta only): when ou_e is given AND z is None, draw the
        # native sample as z = clamp(mu_z + ou_k*s_beta*ou_e, EPS, 1-EPS) instead of
        # dist.sample(), where mu_z/s_beta are the Beta mean/std. ou_e carries the
        # persistent per-env OU residual (unit stationary variance). The log_prob
        # below is UNCHANGED (dist.log_prob(z)), so the PPO ratio stays exact; the OU
        # overlay is only a correlated proposal on the same factored Beta target.
        # ou_k=0 => z=mu_z (the deterministic Beta-mean draw, baseline superset).
        actor_feat, critic_feat = self._trunks(x)
        dist, to_action, log_det_fn = self._actor_dist(actor_feat)
        if z is None:
            if ou_e is not None and self.actor_dist == "beta":
                # dist.mean / dist.stddev are alpha/(a+b) and the Beta std in z-space.
                z = dist.mean + ou_k * dist.stddev * ou_e
...
if __name__ == "__main__":
    ...
    # Colored-OU exploration residual: persists ACROSS rollout iterations (init once).
    # Shape (num_envs, act_dim); unit stationary variance. Only used for beta actor.
    act_dim = int(np.prod(envs.single_action_space.shape))
    use_ou = args.actor_dist == "beta"
    ou_e = torch.zeros((args.num_envs, act_dim), device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                if use_ou:
                    # Advance OU BEFORE sampling; reset correlation where the env that
                    # produced next_obs just terminated/truncated (next_done is the done
                    # flag of the CURRENT next_obs). ou_k=0 => ou_e is multiplied out and
                    # z collapses to the Beta mean (exact baseline draw).
                    ou_e = args.ou_rho * ou_e + sqrt(1.0 - args.ou_rho**2) * torch.randn_like(ou_e)
                    ou_e = ou_e * (1.0 - next_done).unsqueeze(1)
                    action, z, logprob, ent, value_logits = agent.get_action_and_value(
                        next_obs, ou_e=ou_e, ou_k=args.ou_k
                    )
                else:
        ...
        # Colored-OU diagnostics. (1) gait_lag1_autocorr: lag-1 autocorrelation of the
        # EXECUTED actions per dim, averaged, across step transitions that DON'T cross
        # an episode boundary (dones[t+1]==0). Should rise above the white-noise
        # baseline when coloring is active — the direct mechanism check. (2) beta_std:
        # mean of the Beta std over the rollout (auto-anneal probe: should decay).
        with torch.no_grad():
            a = actions  # (T, E, A)
            cont = (1.0 - dones[1:]).unsqueeze(-1)  # (T-1, E, 1): t->t+1 stays in-episode
            x0 = a[:-1]
            x1 = a[1:]
            w = cont.expand_as(x0)
            ...
            m0 = (x0 * w).sum() / wsum
            m1 = (x1 * w).sum() / wsum
            cov = (w * (x0 - m0) * (x1 - m1)).sum() / wsum
            v0 = (w * (x0 - m0) ** 2).sum() / wsum
            v1 = (w * (x1 - m1) ** 2).sum() / wsum
            gait_lag1_autocorr = (cov / (v0 * v1).clamp_min(1e-12).sqrt()).item()
            if use_ou:
                # Cheap probe over a subsample of the rollout obs (same stat used in
                # the OU draw: dist.stddev = Beta std in native z-space).
                probe = b_obs[: min(4096, b_obs.shape[0])]
                actor_feat, _ = agent._trunks(probe)
                dist_probe, _, _ = agent._actor_dist(actor_feat)
                beta_std_mean = dist_probe.stddev.mean().item()
            else:
                beta_std_mean = float("nan")
        ...
        # Colored-OU exploration probes.
        writer.add_scalar("explore/gait_lag1_autocorr", gait_lag1_autocorr, global_step)
        writer.add_scalar("explore/beta_std_mean", beta_std_mean, global_step)
