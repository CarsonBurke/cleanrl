# PPO + IterThink v24 — ADVNORM EMA-RMS v1. From the v24_dist base.
#
# ADVANTAGE NORMALIZATION STUDY (variant #3 of the advnorm ladder): replace the
# default rankgauss rank-transform with a LINEAR, magnitude-preserving advantage
# scale driven by a slow global EMA of the advantage RMS — "reward-norm-style"
# normalization applied to the advantage instead of the reward.
#
# WHY. NormalizeReward divides rewards by the running std of the discounted return,
# so as the policy improves the return scale grows and raw GAE magnitude SHRINKS
# over training — the policy gradient would fade. Standard per-minibatch norm
# re-inflates magnitude, but its divisor (instantaneous minibatch std) is noisy,
# outlier-sensitive, rescales each minibatch independently (so the SAME true
# advantage gets a different push depending on its minibatch's spread — relative
# differences across the batch are NOT respected), and amplifies genuine
# spread-collapse into noise. rankgauss is scale-stable and outlier-immune but
# DISCARDS magnitude ratios (rank-only) and anchors the sign at the median.
#
# EMA-RMS is the sweet spot: divide by a slow GLOBAL EMA of the batch advantage
# RMS and rescale to a fixed target. Magnitude stays HIGH and ~constant over
# training (cancels the reward-norm drift); the transform is LINEAR so relative
# magnitude differences are preserved exactly across the whole batch; the EMA
# smooths within-batch outliers and spread-collapse. No recentering (the value
# baseline already centers GAE; sign = raw GAE sign). Set adv_scale_norm="off"
# (with adv_transform/norm_adv restored) to recover the base byte-for-byte.
#
...
class Args:
    ...
    norm_adv: bool = False           # ADVNORM v1: off — EMA-RMS scaling replaces per-mb standardize
    ...
    adv_transform: str = "v10"       # ADVNORM v1: identity — EMA-RMS scaling does the normalization
    ...
    adv_scale_norm: str = "ema_rms"      # "off" | "ema_rms"  (ADVNORM v1 default = ema_rms)
