# PPO + IterThink v107 tightrew (v105 + reward support tightened to the realized range).
#
# Change from v105:
#   - REWARD SUPPORT [-20,50] -> [-5,20]. Measured: random-policy rewards span
#     [-2.8, +2.0] (std 0.65) and the trained policy averages ~+6.2/step, so the
#     realized envelope occupied ~24% of the old support: 76% dead bins, bin width
#     0.7 LARGER than the early-training reward std, smoothing sigma 0.35 blurring
#     half of it, and a +15 uniform/zero-logit decode midpoint. The old range
#     handicapped every reward-head variant tried on it (CDF/BCE: ~76 of 100
#     thresholds permanently saturated, ~4x extra signal dilution; pure scalar:
#     weight-decay attractor decodes to +15, matching its AR bias drift; CE:
#     elevated MAE floor from the coarse sigma). New geometry: bin 0.25, sigma
#     0.125 (2.8x finer), midpoint +7.5. The head/loss is unchanged (softmax
#     HL-Gauss CE, scalar decode for consumption -- the v105 winner). Watch
#     lewm/reward_target_edge_frac_* for clamping; widen v_max if it rises.
#
...
class Args:
    ...
    lewm_reward_v_min: float = -5.0
    lewm_reward_v_max: float = 20.0
