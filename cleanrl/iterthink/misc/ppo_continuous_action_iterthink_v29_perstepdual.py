# PPO + IterThink v29 (PER-STEP DUAL + alpha floor). From v28.
#
# v28 fixed the alpha UNITS (scale bonus by 1/return_std) and learned cleanly, but stepped the
# dual once/epoch on the ROLLOUT-MEAN BUFFERED entropy -> stale (old-policy) and artificially
# smooth/monotone. v29 makes the dual SAC-faithful: step it PER MINIBATCH on the FRESH current-
# policy squashed entropy (entropy = -logπ at a fresh rsample == SAC's log_pi). SAC steps alpha
# every gradient step (~50x more often than once/epoch); step COUNT never caused the v26
# collapse -- the non-binding target did. Per-step gives a current, responsive (higher-variance,
# SAC-like) signal. Because per-step bleeds alpha ~32x faster during the LONG one-sided pre-
# saturation transient (H_sq=+4 >> -6 until the on-policy policy saturates), we add a small
# alpha FLOOR (alpha_min, raw units) so the temperature survives to bind once H_sq crosses -6
# (then the dual goes two-sided and alpha rises -- the SAC behavior). Everything else == v28.
#
# ---- inherited v28 notes ----
...
# THE FIX (this file): inject entropy in RAW units by scaling the bonus by 1/return_std, so the
...
class Args:
    ...
    target_entropy: Optional[float] = None  # SAC heuristic default = -|A| (act_dim) = -6 for HC
    alpha_lr: float = 1e-3
    alpha_init: float = 0.2         # v28: init alpha in SAC RAW-reward units (bonus is /return_std)
    alpha_min: float = 0.05         # v29: floor on alpha (raw) so the temperature survives the long
                                    #      one-sided transient and can rise once the policy binds (<-6)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                # v28 EQUIVALENT-ALPHA: alpha_r is in SAC RAW-reward units; divide by return_std
                # so alpha_r*H lands in the SAME normalized scale as the reward the loop sees
                # (reward_seen = reward_raw / return_std). alpha_eff = alpha_r / return_std is the
                # effective (normalized) entropy weight; with alpha_r~0.2, return_std~16-100 =>
                # alpha_eff~0.002-0.012 ~ SAC's entropy/reward ratio. Keeps the entropy a SMALL
                # perturbation so the reward gradient can saturate the policy (no peak-pinning).
