# PPO + IterThink v183 policypred-actionmtp v3.
#
# RESULT (NEGATIVE -- and conclusive for the whole policypred line; seed 1
# HalfCheetah, stopped 2M): windowed return 266 @2M (regressing from 569 @1.5M) vs
# parent v182's 1672; entropy -5.21 @2M vs parent -2.65 -- the FASTEST entropy
# collapse of v1/v2/v3 even though the WM is untouched here. Across all three
# placements (WM trunk k>=0, WM trunk k>=1, actor trunk k>=1) the signature is
# identical: aux CE improves only ~0.7 nats over uniform while policy entropy
# collapses early and returns stall. CONCLUSION: predicting your own future
# actions is an anti-exploration objective in ON-POLICY RL. The agent controls
# the data distribution, so the joint fixed point of {minimize self-prediction
# CE, maximize return} lowers entropy to make future actions predictable --
# CE can be reduced by becoming predictable instead of by predicting better.
# Dreamer 4's policy prediction works because its targets are EXOGENOUS (a fixed
# human-contractor dataset): predictability of the data is not controllable, so
# the only way down is genuine prediction. That mechanism does not transfer to
# on-policy PPO. Do not revisit without exogenous or off-policy action targets
# (e.g. a frozen prior's actions, expert data, or a lagged policy snapshot).
#
# v3 = policy prediction moved to the AGENT trunk (Dreamer 4-faithful isolation).
#   The v2 RESULT below is NEGATIVE: putting the future-action MTP head on the WORLD
#   MODEL's predicted summaries (non-detached, shaping the WM trunk) collapsed policy
#   entropy early -- feeding "what the policy will do" into the embedding the actor
#   READS is a self-imitation loop. Dreamer 4 S3.3 guards against exactly this: the
#   policy-prediction head lives on a read-only AGENT-token stream and never shapes
#   the dynamics representation. v3 does the same here: the WM reverts EXACTLY to
#   v182 (all WM-side action machinery removed), and a new action-MTP head hangs off
#   the ACTOR trunk. It reads the actor features and predicts the policy's own future
#   executed actions a_{t+k}, k=1..aux_action_mtp_len, as per-action-dim HL-Gauss
#   logits; the CE (coef aux_action_coef=0.1) flows ONLY through the actor group's
#   grad clip. Zero-init head -> zero trunk gradient at step 0, ramping in as it
#   learns. Same boundary/cumprod validity masking as the WM MTP loop.
#   HYPOTHESIS. Action-MTP on the actor pathway shapes the policy/value features with
#   a forward-looking self-model signal WITHOUT feeding habits back through the world
#   model, so it can help control instead of collapsing exploration.
#
# -----------------------------------------------------------------------------
# PPO + IterThink v183 policypred-actionmtp v2.
#
# RESULT (NEGATIVE, seed 1 HalfCheetah, stopped 3.5M): 1862 @3.5M vs parent v182's
# 3027 at matched steps; stalled 2M->2.5M (1422->1380) while parent gained +834.
# v2's fixes DID protect the WM (obs_latent_cosine 0.6815 vs parent 0.6847 @3.5M) --
# the damage is NOT latent quality. Diagnosis: (1) action_ce only reached ~3.1
# nats/offset vs ln51=3.93 uniform -- future actions of a stochastic Beta policy are
# mostly irreducible sampling noise, so the trunk pressure carries little signal;
# (2) policy entropy collapsed much earlier than parent (-3.77 vs -2.65 @2M):
# pushing "what the policy will do" into the embedding the actor READS feeds the
# policy its own habits -- a self-imitation loop that kills exploration. Dreamer 4
# S3.3 guards against exactly this: agent tokens read all modalities but NOTHING
# attends back, so policy prediction never shapes the dynamics representation.
# LESSON: policy prediction must live on the AGENT pathway (v3), not the WM.
#
# v2 = v1 with TWO changes, both targeting v1's diagnosed failure (at 2M: return
# 1184 vs parent v182's 1673; lewm/obs_latent_cosine 0.363 vs 0.575 -- the
# non-detached action CE was over-pressuring the very embedding the agent reads):
#   1. EXCLUDE offset k=0 from the action CE (lewm_action_mtp_min_offset=1). The
#      k=0 target a_{t+j} is FULLY determined by the AdaLN conditioning, so that
#      term is a pure readback task whose gradient rewards writing the raw current
#      action into pred_mtp[:, :, 0] -- which IS the agent's input embed
#      (unit_norm(pred_next_proj(belief))). Offsets k>=1 remain genuine policy
#      prediction. Initial action_ce ~ (mtp_len-1)*ln(51) ~ 11.8.
#   2. lewm_action_loss_coef 0.5 -> 0.25: halve the total representation pressure.
#
# -----------------------------------------------------------------------------
# PPO + IterThink v183 policypred-actionmtp.
#
# v183 = v182 + Dreamer 4 (§3.3) POLICY PREDICTION: an action multi-token-prediction
# head on the WM's predicted summaries.
#   IDEA. The WM predicts next latents, reward and continuation -- all CONDITIONED on
#   executed actions -- but never PREDICTS actions. Dreamer 4 trains the trunk to
#   predict the agent's own future actions a_{t+n} (MTP, eq. 9) jointly with the
#   dynamics loss, letting the policy-prediction gradient shape the shared
#   representation. v183 adds that head here: at every (position j, offset k) MTP slot
#   the full flattened 19-token predicted summary pred_mtp[:, j, k] (the summary of
#   frame t+1+j+k) is decoded to per-action-dim HL-Gauss logits (51 uniform bins on
#   [-1,1], sigma 0.75*binwidth) and trained by CE against the executed action
#   a_{t+j+k} -- the action that produced the predicted frame -- with the SAME
#   latent_weight masking as the reward MTP. Offsets k>=1 are genuine policy
#   prediction (the MTP projections receive no intermediate actions); k=0 is partly
#   AdaLN readback, kept for parity with Dreamer 4's n=0 term.
#   NOVELTY vs v182 (deliberate departure from the detached-probe protocol): the
#   action head reads the belief NON-detached -- this is the only outcome-style loss
#   allowed to shape the trunk. Reward/continuation probes stay detached as before.
#   Coef 0.5 (mean over act_dim => initial CE ~ ln 51, same scale as reward CE); head
#   zero-init (uniform initial action dist, zero trunk grad at step 0) and excluded
#   from the WM weight unit-norm; head params live in the 25x probe-LR group.
#   HYPOTHESIS. Forcing the predicted summary to be sufficient to predict the policy's
#   future actions injects policy-relevant structure into EXACTLY the embedding the
#   actor/critic consume (they read detached unit_norm(pred_next_proj(belief))),
#   improving control on top of the JEPA+SIGReg-shaped representation.
#
# -----------------------------------------------------------------------------
# PPO + IterThink v182 detachedpredembedinput-lninput-clip075-probehighlr-lnprobe-pretemporalwm-ppoadvnorm-actor1-critic10-cliphi-criticsigma075-dreamer3bucket-hlgauss-wmloss1024-cudagraph-contdisc-k6.
#
...
class Args:
    ...
    # v3: Dreamer4-S3.3 policy prediction moved to the AGENT trunk (action MTP head on
    # the actor features). Per-action-dim HL-Gauss CE, mean over act_dim (initial CE
    # ~ ln(num_bins)). The HL-Gauss support params below are reused by the agent head.
    lewm_action_num_bins: int = 51
    lewm_action_v_min: float = -1.0
    lewm_action_v_max: float = 1.0
    lewm_action_sigma_ratio: float = 0.75
    # v3: agent-trunk action MTP. Head reads the actor features and predicts future
    # executed actions a_{t+k}, k=1..aux_action_mtp_len; CE flows into the actor group.
    aux_action_coef: float = 0.1
    aux_action_mtp_len: int = 3
...
class _SpaceCrossReadout(nn.Module):
    # v174: read the semantic (reward/continuation) query tokens off the time-mixed
    # obs belief via a single non-causal CROSS-attention. This is the only cross-token
    # op left in the predictor: the spatial (space-axis) mixing was moved into the
    # encoder (pre-SIGReg). This readout is auxiliary -- it feeds the outcome heads
    # only, sits OUTSIDE the recurrent obs stream, and never matches a SIGReg'd target.
    # q = semantic queries, k/v = obs belief tokens. nGPT conventions: unit-norm q/k
    # per head; no positional code (feature tokens have no order). norm lives on the
    # output (unit_norm in _predictor_trunk) since the query carries no residual state.
    def __init__(self, dim, num_heads):
        super().__init__()
        if dim % num_heads != 0:
            raise ValueError(f"dim {dim} not divisible by num_heads {num_heads}")
        ...
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
        self.to_q = xavier_linear(nn.Linear(dim, dim, bias=False))
        self.to_kv = xavier_linear(nn.Linear(dim, 2 * dim, bias=False))
        self.proj = xavier_linear(nn.Linear(dim, dim, bias=False))
        self.sqk = nn.Parameter((dim ** -0.5) * torch.ones(dim))

    def forward(self, sem, obs):
        # sem: (N, Q, D) queries; obs: (N, S, D) keys/values. Returns (N, Q, D).
        n, q_len, width = sem.shape
        s_len = obs.shape[1]
        q = self.to_q(sem).reshape(n, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        kv = self.to_kv(obs).reshape(n, s_len, 2, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        sqk = (self.sqk * (1.0 / (self.dim ** -0.5))).to(q.dtype).view(1, self.num_heads, 1, self.head_dim)
        q = sqk * unit_norm(q)
        k = sqk * unit_norm(k)
        attn_out = F.scaled_dot_product_attention(q, k, v, scale=self.head_dim ** 0.5)
        attn_out = attn_out.transpose(1, 2).reshape(n, q_len, width)
        return self.proj(attn_out)
...
class LeWMBackbone(nn.Module):
    ...
    def encode_summary(self, obs, incoming_outcome):
        ...
        outcome_tokens = unit_norm(self.outcome_target_proj(tokens[:, : self.num_semantic_tokens]))
        obs_tokens = unit_norm(self.obs_target_proj(tokens[:, self.obs_token_start :]))
        return torch.cat([outcome_tokens, obs_tokens], dim=1)  # (B,19,D)
    ...
    def _predictor_trunk(self, latent_history, action_history):
        ...
        tokens = unit_norm(latent_history)
        ...
        belief = unit_norm(self.predictor_norm(tokens))  # (B, T, 19, dim)
        return belief
    ...
    def imagine_step_from_history(self, latent_history, action_history):
        ...
        next_summary = unit_norm(self.pred_next_proj(belief))                    # (B,19,D) full
...
class BeliefConditioner(nn.Module):
    ...
    def forward(self, belief, obs_embed):
        ...
        return unit_norm(belief * (1.0 + gamma) + beta)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # (unit_norm(pred_next_proj(belief))) rather than the raw belief. This
        ...
        # v3: agent-trunk action MTP head (Dreamer 4 S3.3 policy prediction, isolated on
        # the ACTOR pathway -- nothing feeds back through the world model). Reads the
        # actor trunk features and predicts future executed actions a_{t+k}, k=1..len,
        # as per-action-dim HL-Gauss logits. Zero-init -> zero trunk gradient at step 0
        # (the agent has no unit-norm projection loop, so the zero init survives), so
        # the forward-looking self-model signal ramps in only as the head learns.
        self.aux_action_mtp_len = args.aux_action_mtp_len
        self.action_mtp_num_bins = args.lewm_action_num_bins
        self.action_mtp_head = nn.Linear(
            H, args.aux_action_mtp_len * act_dim * args.lewm_action_num_bins, bias=False
        )
        nn.init.zeros_(self.action_mtp_head.weight)
    ...
    def combine_agent_input(self, obs_embed_latent, belief_latent):
        ...
        # v182: project the belief into the WM's PREDICTED next-summary embed --
        # pred_next_proj(belief) -- BEFORE buffering, so the agent acts on the
        # prediction, not the raw belief. This runs at rollout/dream buffer time;
        # agent_input_from_latent then .detach()es it, so pred_next_proj's WM params
        # never receive agent gradient (detached predicted embed).
        ...
        # mirrors the v179.2 probe protocol ("LayerNorm replaces unit_norm"). The WM's
        # own next_summary (imagine_step) keeps its unit_norm for the unit-sphere
        # dynamics history; only the LN-normalized agent-input path drops it.
        belief_latent = self.world_model.pred_next_proj(belief_latent)
    ...
    def agent_input_from_obs(self, x):
        ...
        belief = self.world_model.belief_from_history(latent_hist, neutral)
    ...
    def predict_future_actions_from_agent_input(self, obs, agent_input):
        # v3: agent-trunk action MTP. Recomputes the actor features from the buffered
        # agent input and decodes per-offset, per-action-dim HL-Gauss logits over the
        # policy's future executed actions a_{t+k}. Called EAGERLY (not torch.compiled):
        # the actor/value callables are cudagraph-captured and must not share params
        # with a second compiled callable. Recomputes the trunk once per actor epoch.
        actor_feat, _ = self._trunks_from_agent_input(obs, agent_input)
        logits = self.action_mtp_head(actor_feat)
        return logits.view(-1, self.aux_action_mtp_len, self.act_dim, self.action_mtp_num_bins)
    ...
    def actor_parameters(self):
        ...
        # v3: the action MTP head receives the policy-epoch gradient (actor group), NOT
        # the critic group.
        return list(trunk.parameters()) + heads + list(self.action_mtp_head.parameters()) + self._fusion_params()
...
if __name__ == "__main__":
    ...
    optimizer = optim.AdamW(
        [
            ...
            {"params": wm_latent_params, "weight_decay": 0.0},
            {"params": wm_outcome_io_params, "weight_decay": 0.0,
    ...
    # v183: uniform bins on the [-1,1] action range (symlog spacing is pointless here).
    action_support = HLGaussSupport(
        args.lewm_action_num_bins,
        args.lewm_action_v_min,
        args.lewm_action_v_max,
        args.lewm_action_sigma_ratio,
        device,
    )
    ...
    def masked_token_sigreg_static(token_latents, token_valids, A_obs, A_out):
        # v179.1: SEPARATE SIGReg for outcome vs obs tokens. The obs tokens
        # ([obs_token_start:], K=num_obs_tokens) are whitened JOINTLY (flatten K*D, A_obs)
        # -- this keeps v175's cross-obs-token decorrelation. Each OUTCOME token
        # ([<obs_token_start], reward / continuation) is whitened SEPARATELY over its own
        # D dims (A_out), and is NOT cross-whitened with obs. Rationale: joint 19-token
        # SIGReg pushes outcome dims to be statistically INDEPENDENT of obs dims, but
        # reward/continuation are genuine FUNCTIONS of state -- so joint whitening fights
        # the very signal the detached probe must read. Decoupling lets the outcome tokens
        # correlate with obs while still being anti-collapsed within themselves.
        # token_latents: (B, H, S, D); A_obs: (num_obs*D, P); A_out: (D, P).
        os = agent.world_model.obs_token_start
        obs_loss = _sigreg_group_static(token_latents[:, :, os:, :], token_valids, A_obs)
        out_losses = torch.stack(
            [_sigreg_group_static(token_latents[:, :, i : i + 1, :], token_valids, A_out) for i in range(os)]
        )
        # Token-count-weighted average: the obs joint statistic counts as num_obs tokens,
        # each outcome token as one. Keeps total SIGReg magnitude ~unchanged vs the
        # 19-token joint, so this ablation isolates the DECOUPLING, not the strength.
        return (
            agent.world_model.num_obs_tokens * obs_loss + out_losses.sum()
    ...
    def wm_supervised_loss_forward(
        ...
        pred_mtp = wm_supervised_predict_mtp_from_history(teacher_history, future_actions)

        one_step_denom = latent_weight.sum().clamp_min(1.0)
        one_step_pred = pred_mtp[:, :, 0, :]
        one_step_latent_cos = (one_step_pred * target_summaries).sum(dim=-1).mean(dim=-1)
        one_step_latent_loss = 1.0 - one_step_latent_cos
        one_step_latent_mse = F.mse_loss(one_step_pred, target_summaries, reduction="none").mean(dim=(-1, -2))
        wm_latent_loss = (one_step_latent_loss * latent_weight).sum() / one_step_denom
        wm_latent_mse = (one_step_latent_mse * latent_weight).sum() / one_step_denom
        wm_latent_cosine = (one_step_latent_cos * latent_weight).sum() / one_step_denom

        # v179: DETACHED probe -- pred_mtp is stop-grad inside decode, so CE/BCE reach
        # only the probe heads, never the encoder/predictor.
        ...
        # v179: SIGReg over the FULL 19-token summary (joint flatten = 19*D).
    ...
    def wm_supervised_metrics_forward(
        ...
        wm_latent_mse = (one_step_latent_mse * latent_weight).sum() / denom_latent
        wm_latent_cosine = (one_step_latent_cos * latent_weight).sum() / denom_latent
        ...
        # v179: SIGReg over the FULL 19-token summary.
        sigreg_latents = torch.cat([initial_summary.unsqueeze(1), target_summaries], dim=1)
        sigreg_latents = sigreg_latents * (sigreg_latents.shape[-1] ** 0.5)
        wm_sigreg_loss = masked_token_sigreg_static(sigreg_latents, sigreg_weight, sigreg_A_obs, sigreg_A_out)
        wm_discount_bce = discount_bce_sum
        wm_discount_bce_excess = discount_bce_sum - discount_entropy_sum
    ...
    def build_belief_agent_input(obs_tensor, incoming_outcome):
        ...
        belief = agent.world_model.belief_from_history(belief_latents, belief_actions)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for _ in range(args.lewm_update_epochs):
            ...
            for start in range(0, args.batch_size, wm_minibatch_size):
                ...
                # v179.1: SEPARATE projections -- obs group (num_obs*D) and per-outcome-token (D).
                sigreg_A_obs = sigreg.sample_projection(
                    agent.world_model.num_obs_tokens * agent.world_model.dim,
                    device,
        ...
        aux_action_loss = torch.zeros((), device=device)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if actor_active:
                    ...
                    # v3: agent-trunk action MTP (Dreamer 4 S3.3 policy prediction).
                    # Predict the policy's OWN future executed actions a_{t+k}, k=1..K,
                    # from the actor features; CE flows into the actor group so the
                    # actor pathway carries a forward-looking self-model signal without
                    # feeding habits back through the world model. Targets/masks reuse
                    # the WM loop's boundary/cumprod semantics: slot k is valid iff no
                    # episode boundary in [t, t+k) AND t+k is a real in-rollout step.
                    aux_K = agent.aux_action_mtp_len
                    mb_step_inds_a = torch.as_tensor(
                        mb_inds // args.num_envs, device=device, dtype=torch.long
                    )
                    mb_env_inds_a = torch.as_tensor(
                        mb_inds % args.num_envs, device=device, dtype=torch.long
                    )
                    aux_max_start = args.num_steps - 1
                    # Boundary cumprod over transitions [t, t+K): column (k-1) holds
                    # prod_{i=0}^{k-1}(1 - boundary[t+i]) -> the mask for offset k.
                    aux_bnd_offsets = torch.arange(aux_K, device=device)
                    aux_bnd_steps = (mb_step_inds_a[:, None] + aux_bnd_offsets[None, :]).clamp(max=aux_max_start)
                    aux_bnd_env = mb_env_inds_a[:, None].expand_as(aux_bnd_steps)
                    aux_no_boundary = torch.cumprod(
                        1.0 - transition_boundaries[aux_bnd_steps, aux_bnd_env], dim=1
                    )
                    # Target action steps t+k, k=1..K.
                    aux_tgt_offsets = torch.arange(1, aux_K + 1, device=device)
                    aux_tgt_steps_raw = mb_step_inds_a[:, None] + aux_tgt_offsets[None, :]
                    aux_in_rollout = (aux_tgt_steps_raw < args.num_steps).to(aux_no_boundary.dtype)
                    aux_tgt_steps = aux_tgt_steps_raw.clamp(max=aux_max_start)
                    aux_tgt_env = mb_env_inds_a[:, None].expand_as(aux_tgt_steps)
                    aux_mask = aux_no_boundary * aux_in_rollout * transition_valids[aux_tgt_steps, aux_tgt_env]
                    aux_target_actions = actions[aux_tgt_steps, aux_tgt_env]      # (mb, K, act_dim)
                    aux_target_probs = action_support.project(aux_target_actions)  # (mb, K, act_dim, bins)
                    aux_logits = agent.predict_future_actions_from_agent_input(
                        b_obs[mb_inds], b_agent_inputs[mb_inds]
                    )                                                             # (mb, K, act_dim, bins)
                    aux_ce = -(aux_target_probs * torch.log_softmax(aux_logits, dim=-1)).sum(-1).mean(-1)  # (mb, K)
                    aux_offset_ce = (aux_ce * aux_mask).sum(dim=0) / aux_mask.sum(dim=0).clamp_min(1.0)     # (K,)
                    aux_action_loss = aux_offset_ce.mean()
                else:
                ...
                if args.separate_grad_clip:
                    ...
                    if actor_active:
                        optimizer.zero_grad(set_to_none=True)
                        # v3: aux action-MTP CE rides the actor group -> actor grad clip.
                        (pg_loss - ent_coef_eff * entropy_loss + args.aux_action_coef * aux_action_loss).backward()
                    ...
                    if actor_active:
                        loss = loss + pg_loss - ent_coef_eff * entropy_loss + args.aux_action_coef * aux_action_loss
        ...
        writer.add_scalar("losses/aux_action_ce", aux_action_loss.item(), global_step)
