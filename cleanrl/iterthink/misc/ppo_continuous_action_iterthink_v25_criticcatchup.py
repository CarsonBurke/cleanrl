# PPO + IterThink v25 (CRITIC CATCH-UP: decouple the value budget from the KL leash). From v24.
#
# WHY v25. v24_beta won big (rankgauss = 9498, a new high), but every MAGNITUDE-aware
# advantage (advnorm/raw) collapsed (advnorm ~3150). That gap is the symptom of a
# critic whose VALUE MAGNITUDES lag the moving policy: rankgauss only needs the
# advantage ORDERING, so it tolerates a bad critic; advnorm needs the magnitudes and
# the lagging critic can't supply them. So rankgauss is a crutch for a poorly-informed
# critic. v25 attacks the CAUSE, not the symptom.
#
# THE MECHANISM. The epoch loop trains policy AND critic together, and `target_kl`
# (0.03) breaks the WHOLE loop. During fast policy improvement approx_kl crosses 0.03
# after only 1-3 of the 10 epochs, so the loop breaks early. The policy is protected
# (intended) but the CRITIC is starved of updates at exactly the moment its target
# (V^pi) is moving fastest -> explained_variance collapses. The KL leash that guards
# the policy on the shared trunk inadvertently shackles the critic to it.
#
# THE FIX (critic catch-up). Decouple the critic's optimization budget from the
# policy's KL leash. The policy still early-stops at target_kl. But then we run the
# REMAINING epochs as VALUE-ONLY passes so the critic always gets `critic_epochs`
# (default = update_epochs = 10) value updates regardless of where the policy stopped.
# Because the trunk is shared, a bare value-only pass would reshape the trunk and drift
# the (now-frozen) policy -- defeating the KL stop. So each catch-up pass adds a KL
# ANCHOR (PPG-style): KL(pi_frozen || pi_new), where pi_frozen is snapshotted right
# after the policy phase. The anchor's gradient pulls the policy back through the trunk
# while the value CE reshapes it for value -- the critic converges, the policy holds.
# Reuses the v24 dual-backward clip verbatim (value grad -> critic_grad_clip, KL grad
# -> actor_grad_clip). Clean ablation: if target_kl never trips, NO catch-up runs and
# v25 == v24 exactly. Test: does the un-starved critic let advnorm match rankgauss?
#
# --- inherited v24 notes ---
...
class Args:
    ...
    # v25 critic catch-up. The critic gets `critic_epochs` TOTAL value updates per
    # iteration; the policy still early-stops at target_kl. Any epochs the policy
    # skips become VALUE-ONLY passes (policy held by a KL anchor). Default == update_
    # epochs => the critic is simply un-starved (never under-trained relative to the
    # no-early-stop case). Set critic_catchup=False to recover v24 exactly.
    critic_catchup: bool = True
    critic_epochs: int = 10          # total value-update epochs/iter (>= effective policy epochs)
    aux_kl_coef: float = 1.0         # KL(pi_frozen || pi_new) anchor weight in value-only passes
    norm_adv: bool = True
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # v25 CRITIC CATCH-UP. The policy stopped after `policy_epochs_done` epochs
        # (early if target_kl tripped). Give the critic the remaining value updates so
        # it is never under-trained relative to the no-early-stop case, holding the
        # (now frozen) policy in place with a KL anchor so reshaping the shared trunk
        # for value cannot drift it. When the policy used the full budget, NO catch-up
        # runs => identical to v24.
        policy_epochs_done = epoch + 1
        n_catchup = max(0, args.critic_epochs - policy_epochs_done) if args.critic_catchup else 0
        aux_v_loss = aux_kl = 0.0
        if n_catchup > 0:
            with torch.no_grad():
                af_all, _ = agent._trunks(b_obs)
                frozen, _, _ = agent._actor_dist(af_all)
                if args.actor_dist == "beta":
                    fz1, fz0 = frozen.concentration1.detach(), frozen.concentration0.detach()
                else:
                    fz_loc, fz_scale = frozen.loc.detach(), frozen.scale.detach()
            for _ in range(n_catchup):
                np.random.shuffle(b_inds)
                for start in range(0, args.batch_size, args.minibatch_size):
                    end = start + args.minibatch_size
                    mb_inds = b_inds[start:end]
                    actor_feat, critic_feat = agent._trunks(b_obs[mb_inds])
                    value_logits = agent.critic_head(critic_feat)
                    value_log_probs = torch.log_softmax(value_logits, dim=-1)
                    v_loss = -(b_target_probs[mb_inds] * value_log_probs).sum(dim=-1).mean()
                    new_dist, _, _ = agent._actor_dist(actor_feat)
                    if args.actor_dist == "beta":
                        frozen_mb = Beta(fz1[mb_inds], fz0[mb_inds])
                    else:
                        frozen_mb = Normal(fz_loc[mb_inds], fz_scale[mb_inds])
                    kl_anchor = kl_divergence(frozen_mb, new_dist).sum(1).mean()
                    if args.separate_grad_clip:
                        # Same dual-backward as the joint phase: value CE clipped to
                        # critic_grad_clip, the KL-anchor (a policy-space grad) clipped
                        # to actor_grad_clip, summed on the shared trunk.
                        optimizer.zero_grad(set_to_none=True)
                        (args.vf_coef * v_loss).backward(retain_graph=True)
                        nn.utils.clip_grad_norm_(critic_params, args.critic_grad_clip)
                        value_grads = [(p, p.grad.detach().clone()) for p in critic_params if p.grad is not None]
                        optimizer.zero_grad(set_to_none=True)
                        (args.aux_kl_coef * kl_anchor).backward()
                        nn.utils.clip_grad_norm_(actor_params, args.actor_grad_clip)
                        for p, g in value_grads:
                            p.grad = g if p.grad is None else p.grad + g
                        optimizer.step()
                    else:
                        loss = args.vf_coef * v_loss + args.aux_kl_coef * kl_anchor
                        optimizer.zero_grad()
                        loss.backward()
                        nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                        optimizer.step()
            aux_v_loss, aux_kl = float(v_loss.item()), float(kl_anchor.item())
        ...
        # v25 catch-up diagnostics: how many epochs the policy actually ran before the
        # KL leash tripped, and how many value-only catch-up epochs the critic got.
        writer.add_scalar("debug/policy_epochs", policy_epochs_done, global_step)
        writer.add_scalar("debug/critic_catchup_epochs", n_catchup, global_step)
        writer.add_scalar("debug/aux_value_loss", aux_v_loss, global_step)
        writer.add_scalar("debug/aux_anchor_kl", aux_kl, global_step)
