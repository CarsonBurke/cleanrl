# PPO + IterThink v27 (SAC SOFT CRITIC + ALPHA-COLLAPSE FIX). From v26.
#
# v26's max-ent objective was sound but its temperature dual was INERT. The live run showed
# alpha collapsing 0.097 -> 0.003 (dead by ~460k steps) while debug/squashed_entropy never
# left [-3, +0.06]. Root cause = TARGET MAGNITUDE MISMATCH: target_entropy=-|A|=-6 is SAC's
# heuristic for its OFF-POLICY-converged tanh policy (~-1 nat/dim); our ON-POLICY squashed
# policy is ~2x more entropic (~-0.5 nat/dim) and never reaches -6, so (-H + target) is
# constant-sign for all 8M steps and the dual can ONLY push alpha down -- one-sided bleed,
# never the SAC-like 0.1->0.3 rise. Two fixes, alpha-logic ONLY:
#   (A) CALIBRATED TARGET. Set target_entropy ABOVE the policy's pure-reward (alpha->0)
#       entropy asymptote (~-4..-5) so the dual must HOLD entropy UP -> two-sided feedback ->
#       alpha equilibrates/rises (SAC's actual mechanism). Default -|A|/2=-3; bracket {-2,-3,-4}.
#   (B) CADENCE. v26 stepped the dual 320x/rollout (10 epochs x 32 minibatches) on ONE reused
#       batch -- all same-sign, ~320x over-correction. Now: alpha held constant within a
#       rollout (= alpha_r, used in both soft reward and actor bonus), dual stepped ONCE PER
#       EPOCH on that epoch's mean fresh entropy -> stable equilibration, ample adapt range.
# Everything else (clip_z, value_symlog, C51 backup, network) is UNCHANGED from v26 so this
# isolates the alpha fix.
#
# ----- v26 (inherited) -----
#
# v25 injected entropy as a POLICY-ONLY soft advantage (hard critic + a separate future-
# entropy GAE channel). Five audits of the live run found that decomposition both BIASED
# (un-baselined: the soft advantage's baseline V(s) lacked the future entropy its target
# carried) and CATASTROPHICALLY MIS-SCALED: at alpha=1 init the entropy advantage was
# 30-186x the reward advantage (debug/soft_adv_std_ratio), so post-norm the PG chased
# entropy and return collapsed (-47 -> -300, vs v24-gaussian +4868). Root cause = scale:
# SAC's alpha lives in RAW-reward units, but here NormalizeReward makes the on-par alpha
# ~0.1, and the lambda=1 entropy horizon multiplied the per-step over-scale ~100x.
#
# v26 returns to the PRINCIPLED max-ent objective: fold the entropy into the reward,
#       soft_reward_t = r_t + alpha * H(s_t),
# and learn ONE distributional critic + ONE GAE on it. The critic now learns the SOFT
# value Z_soft; the advantage is the consistent A = Q_soft - V_soft (the persistent
# alpha*H/(1-gamma) level sits in BOTH return and baseline and cancels, leaving the
# future-entropy credit -- exactly SAC's soft value, correctly baselined). Fixes vs v25:
#   (1) SCALE. alpha_init=0.1 (was 1.0) so alpha*H ~ the unit normalized reward, not 100x.
#       Probe debug/ent_bonus_vs_reward (target ~O(1)). The dual adapts alpha from there.
#   (2) BASELINING. Entropy in BOTH G and V => the soft advantage is unbiased (no separate
#       un-baselined future-only bonus); SAC's gamma-horizon falls out of the soft critic
#       for free (no lambda=1 hack needed).
#   (3) SUPPORT. The soft value is LARGE (~alpha*H/(1-gamma) ~ tens). value_symlog=True puts
#       the C51 support in symlog space ([-10,10] -> +-22000 real) so it never overflows --
#       the headroom the v24 hard-support softboot lacked. The C51 lambda-return backup is
#       KEPT IN FULL (more powerful than a scalar target): the Bellman shift Tz=r+gamma*z is
#       done in REAL space then mapped back to symlog before projection (DreamerV3-grade).
#
# KEPT: squashed-Gaussian + SAC tanh Jacobian; SAC log-std bounds [-5,2]; dual on the fresh
# rsample squashed entropy (target_entropy=-|A|); explicit alpha*H actor bonus supplies the
# CURRENT-state entropy gradient (H(s_t) is action-independent so it cancels in the advantage
# -- no double count); single critic, no twin-Q/target net (on-policy lambda-returns don't
# max-bootstrap). adv_transform=clip_z (magnitude-preserving). --no-auto-entropy => plain
# gaussian control (soft_reward == reward, hard critic).
#
...
#           lv  = rescale(tanh(raw_lv/(hi-lo)), (-1,1), (lo,hi))   # v25: (lo,hi)=(-10,4) => lv=-3, std=0.223 at init (SAC parity)
#           std = exp(0.5 * lv)
...
class Args:
    ...
    auto_entropy: bool = True       # v25: SAC-faithful soft max-entropy ON by default
    target_entropy: Optional[float] = None  # SAC heuristic default = -|A| (act_dim)
    alpha_lr: float = 1e-3
    alpha_init: float = 0.1         # v26: initial α (was implicitly 1.0). See scale fix.
    vf_coef: float = 0.5
    ...
    # Distributional critic support. v26: support is in SYMLOG space (value_symlog=True),
    # so [-10,10] covers ±(e^10-1)≈±22000 real — easily holds the SOFT value (~α·H/(1-γ)≈
    # tens) that the entropy-augmented reward induces, the headroom the hard-support
    # softboot lacked. C51 backup shifts in real space (see distributional_lambda_returns).
    num_bins: int = 511
    v_min: float = -10.0
    v_max: float = 10.0
    critic_init_tau: float = 0.5   # init Z ≈ N(0, tau^2), sharp at 0
    value_symlog: bool = True      # v26: symlog support for the soft (entropy-augmented) value
    ...
    adv_transform: str = "clip_z"    # v25 fix #1: magnitude-PRESERVING (was rankgauss,
    # which discards the alpha*H magnitude that SAC's soft value depends on)
    ...
    actor_dist: str = "gaussian"     # v25: squashed-Gaussian (SAC-native); maxent requires it
    logvar_min: float = -10.0        # v25 fix #3: SAC log-std bounds [-5, 2] => logvar [-10, 4]
    logvar_max: float = 4.0          # std in [exp(-5), exp(2)] = [0.0067, 7.39] (SAC parity)
...
def distributional_lambda_returns(
    rewards, dones, next_done, value_probs, bootstrap_probs, support, v_min, v_max, bin_width,
    gamma, gae_lambda, use_symlog=False,
):
    """Backward recursion for the distributional λ-return G^λ (probs per step).

        G^λ_t =_D r_t + γ·nonterm·[ (1-λ)·Z(s_{t+1}) + λ·G^λ_{t+1} ]

    Full C51 categorical backup. Mean-matches the scalar GAE λ-return. Shapes:
    rewards/dones (T, B); value_probs (T, B, n); bootstrap_probs (B, n) = Z(s_T).
    Returns (T, B, n). `rewards` is the SOFT reward r_t + α·H(s_t) when max-ent is on
    (the critic learns the SOFT value distribution; see --auto-entropy).

    v26: when use_symlog, `support` lives in SYMLOG space. The Bellman shift Tz = r + γz
    must be done in REAL value space (symlog is non-linear, you cannot add a real reward
    to a symlog atom position), so we symexp the support to real, shift, then symlog the
    result back before projecting onto the fixed symlog support (DreamerV3-grade). This
    keeps the full distributional backup while giving symlog's ~unbounded range so the
    persistent α·H/(1-γ) soft-value offset never overflows [v_min,v_max]."""
    T = rewards.shape[0]
    z_real = symexp(support) if use_symlog else support  # (n,) real atom values
    ...
    for t in reversed(range(T)):
        ...
        atoms_real = rewards[t].unsqueeze(-1) + gn * z_real  # (B, n) REAL-space Bellman shift
        atoms = symlog(atoms_real) if use_symlog else atoms_real
...
if __name__ == "__main__":
    ...
    if auto_alpha:
        act_dim = int(np.prod(envs.single_action_space.shape))
        # v27 CALIBRATED TARGET. SAC's heuristic is target = -|A| = -6 for HalfCheetah, tuned
        # to its OFF-POLICY-converged tanh policy (~-1 nat/dim as Q-learning saturates actions).
        # The v26 live run proved this is UNREACHABLE for our on-policy squashed-PPO policy:
        # debug/squashed_entropy never leaves [-3, +0.06] (~-0.5 nat/dim, ~2x more entropic),
        # so (-H + target) is constant-sign for all 8M steps and the dual ONE-SIDEDLY bleeds
        # alpha 0.097 -> 0.003 (never binds, never rises like SAC). The constraint must sit
        # ABOVE the policy's pure-reward (alpha->0) entropy asymptote (~-4..-5) so the dual has
        # to hold entropy UP -> two-sided feedback -> alpha equilibrates/rises (SAC's mechanism).
        # Default -|A|/2 = -3; bracket {-2,-3,-4} via --target-entropy.
        target_entropy = args.target_entropy if args.target_entropy is not None else -0.5 * float(act_dim)
        # v26 scale fix: init α LOW (≈0.1, not 1.0). SAC inits log_alpha=0 (α=1) but on RAW
        # rewards; here rewards are NormalizeReward-scaled (std≈0.5) so the on-par α is ≈0.1-0.15
        # (α·H≈reward). Starting at α=1 made the entropy bonus swamp reward ~30-186× and collapsed
        # the policy (v25). The dual still adapts α from this gentle start.
    ...
    hl_support = HLGaussSupport(
        ...
        args.v_min,
        args.v_max,
        0.5,  # sigma_ratio unused (hard C51 two-point projection, no Gaussian smoothing)
        device,
        use_symlog=args.value_symlog,
    )
    support = hl_support.support                       # (num_bins,) support (SYMLOG space if value_symlog)
    bin_width = hl_support.bin_width
    # Real-space atom values: symexp(support) under symlog, else the support itself. Used for
    # the expected scalar value E[Z]=Σp·z_real (GAE) and the real-space Bellman shift.
    z_real = symexp(support) if args.value_symlog else support
    ...
    entropies = torch.zeros((args.num_steps, args.num_envs)).to(device)  # H(s_t), buffered for the soft reward
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                value_probs[step] = p
                values[step] = (p * z_real).sum(dim=-1)   # E[Z] in REAL value space
            actions[step] = action
            latent_zs[step] = z
            logprobs[step] = logprob
            entropies[step] = ent                          # H(s_t) (squashed), data for the soft reward
        ...
        with torch.no_grad():
            # v26 UNIFIED SOFT CRITIC (SAC max-ent, on-policy). Entropy is folded into the
            # reward — soft_reward_t = r_t + α·H(s_t) — and fed through ONE GAE and ONE
            # distributional critic, so the critic learns the SOFT value Z_soft and the
            # advantage is the CONSISTENT soft advantage A = Q_soft − V_soft. The persistent
            # α·H/(1-γ) level appears in BOTH the return G_t and the baseline V(s_t) and
            # cancels, leaving the future-entropy CREDIT — exactly SAC's soft value, NOT
            # v25's un-baselined, rank-stripped, λ=1 future-only bonus that swamped reward
            # 30-186×. H(s_t) is action-independent so it contributes ~0 to the PG; the
            # CURRENT-state entropy gradient is supplied by the explicit α·H actor bonus below
            # (no double count). Symlog support holds the large soft value.
            if auto_alpha:
                alpha_r = log_alpha.exp().detach()
                soft_rewards = rewards + alpha_r * entropies     # r_t + α·H(s_t)
            else:
                soft_rewards = rewards
            bootstrap_probs = torch.softmax(agent.get_value(next_obs), dim=-1)   # (B, n) = Z_soft(s_T)
            next_value = (bootstrap_probs * z_real).sum(dim=-1).reshape(1, -1)   # REAL space
            # SOFT GAE: advantage + return from the entropy-augmented reward; real-space values.
            ...
            # Critic target: distributional SOFT λ-return (full C51 backup on the soft reward,
            # real-space Bellman shift under symlog).
            target_probs = distributional_lambda_returns(
                soft_rewards, dones, next_done, value_probs, bootstrap_probs,
                support, args.v_min, args.v_max, bin_width, args.gamma, args.gae_lambda,
                use_symlog=args.value_symlog,
            )
            # Per-state return std sigma(s_t) from the OLD rollout Z(s_t), REAL space.
            sigma = (value_probs * (z_real - values.unsqueeze(-1)) ** 2).sum(-1).clamp_min(0).sqrt()  # (T,B)
            sigma = sigma.clamp_min(args.sigma_floor_bins * bin_width)
            # CDF-rank u (cdf_probit path + calibration probe), REAL space.
            cdf_frac = ((returns.unsqueeze(-1) - z_real) / bin_width + 0.5).clamp(0.0, 1.0)  # (T,B,n)
        ...
        clipfracs = []
        # v27 CADENCE FIX. v26 stepped the dual 320x/rollout (10 epochs x 32 minibatches)
        # on the SAME reused batch -- all same-sign updates over one batch's mean-entropy,
        # so the dual over-corrects ~320x and (with the non-binding -6 target) collapses alpha
        # in ~14 rollouts. SAC steps alpha once per fresh gradient step around a BINDING target,
        # so its steps balance. Here: hold alpha CONSTANT within a rollout (alpha_r is the
        # data-for-this-rollout temperature used in BOTH the soft reward and the actor bonus),
        # and step the dual ONCE PER EPOCH on that epoch's mean fresh entropy -> stable
        # equilibration with ample adaptation range (10 steps/rollout x lr).
        ent_coef_eff = alpha_r if auto_alpha else args.ent_coef
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if auto_alpha:
                    # Accumulate this epoch's fresh squashed entropy; the dual steps once
                    # after the epoch (below) on the epoch mean. ent_coef_eff stays = alpha_r
                    # (held constant within the rollout), matching the soft reward's alpha.
                    epoch_entropies.append(entropy.detach().mean())
            ...
            if auto_alpha:
                # SAC's exact temperature dual (sac_continuous_action.py:292), stepped ONCE
                # per epoch on this epoch's mean fresh squashed entropy:
                #   alpha_loss = -α·(logπ + target_entropy)  with  -logπ == H_squashed.
                # Two-sided once H crosses target -> α equilibrates instead of one-sided bleed.
                mean_entropy = torch.stack(epoch_entropies).mean()
                alpha_loss = (-log_alpha.exp() * (-mean_entropy + target_entropy))
        ...
        if auto_alpha:
            ...
            # squashed entropy H(s) (buffered rollout estimate); the per-step soft-reward
            # entropy bonus α·H (should be ON PAR with the ~unit normalized reward, NOT 30-186×);
            # and the entropy share of the soft reward (sanity that it doesn't dominate).
            writer.add_scalar("debug/squashed_entropy", entropies.mean().item(), global_step)
            ent_bonus = (alpha_r * entropies)
            writer.add_scalar("debug/soft_reward_bonus", ent_bonus.mean().item(), global_step)
            writer.add_scalar("debug/ent_bonus_vs_reward", (ent_bonus.abs().mean() / (rewards.abs().mean() + 1e-8)).item(), global_step)
