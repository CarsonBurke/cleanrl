# PPO + IterThink v28 (MULTI-TOKEN VALUE PREDICTION: predict the next T value targets). From v24.
#
# WHY v28. v24_beta+rankgauss is SOTA (9498). The action-conditioned-critic detour (v27)
# confirmed the lesson: on-policy the per-action advantage magnitude is a high-variance
# single sample, and learned-Q replacements collapse -- so we stop fighting the advantage
# transform and instead make the CRITIC's REPRESENTATION richer. A better-conditioned
# critic -> a lower-variance V(s) -> cleaner GAE, with the proven GAE+rankgauss policy
# path UNTOUCHED.
#
# THE IDEA (multi-token prediction, Gloeckle et al. / DeepSeek-V3, ported to value). Instead
# of the critic predicting only V(s_t), it predicts the next T=8 value targets from s_t's
# features with T PARALLEL distributional heads off the shared trunk:
#     head_k(feat_t)  regresses (CE) to the distributional lambda-return target G_{t+k}
# k=0 is the ordinary value head (used for V, GAE, bootstrap, sigma -- everything is v24);
# k=1..7 are AUXILIARY: they force s_t's representation to encode the value trajectory of
# the next 8 steps, a strong temporal-predictivity signal that should sharpen V(s_t) itself.
# Loss = EQUAL weight per head (mean over heads of each head's masked CE). Heads are masked
# past the rollout end and across episode-boundary (done) crossings, so a head never trains
# on a return from a different episode. Hypothesis: the multi-step auxiliary lifts EV and
# the score above 9498. KNOB: mtp_T (=1 recovers v24 exactly). Policy/advantage unchanged.
#
# --- inherited v24 notes ---
...
class Args:
    ...
    num_bins: int = 511
    mtp_T: int = 8                   # multi-token value prediction horizon (heads predict G_t..G_{t+T-1}); 1 == v24
    v_min: float = -10.0
    v_max: float = 10.0
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v28 multi-token value head: T parallel distributional heads (one Linear emitting
        # T*num_bins, reshaped). head_0 is the ordinary value; head_k predicts G_{t+k}.
        self.num_bins = args.num_bins
        self.mtp_T = args.mtp_T
        self.critic_head = layer_init(nn.Linear(H, args.mtp_T * args.num_bins), std=0.1)
        with torch.no_grad():
            z = torch.linspace(args.v_min, args.v_max, args.num_bins)
            peaked = -0.5 * (z / args.critic_init_tau) ** 2          # (num_bins,)
            self.critic_head.bias.copy_(peaked.repeat(args.mtp_T))   # same peaked init per head
    ...
    def _value_mtp(self, critic_feat):
        # (B, T, num_bins): the T multi-token value heads.
        return self.critic_head(critic_feat).view(-1, self.mtp_T, self.num_bins)

    def get_value(self, x):
        # Returns head_0 value LOGITS (B, num_bins) for V/bootstrap; caller converts via support.
        _, critic_feat = self._trunks(x)
        return self._value_mtp(critic_feat)[:, 0]
    ...
    def get_action_and_value(self, x, z=None):
        ...
        value_logits = self._value_mtp(critic_feat)   # (B, T, num_bins); head_0 is V
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                p = torch.softmax(value_logits[:, 0], dim=-1)   # head_0 = V(s_t)
                value_probs[step] = p
        ...
        # v28 multi-token targets: for each sample (t, env) and head k, the target is the
        # distributional λ-return G_{t+k} == b_target_probs[(t+k)*B + env]. We store flat
        # GATHER indices (cheap) instead of materializing (S,B,T,n). A head is masked past
        # the rollout end (t+k >= S) and across any episode boundary (cumulative no-done).
        S, B, Tk = args.num_steps, args.num_envs, args.mtp_T
        base = (torch.arange(S, device=device).unsqueeze(1) * B
                + torch.arange(B, device=device).unsqueeze(0))            # (S,B): t*B+env
        mtp_src = torch.zeros(S, B, Tk, dtype=torch.long, device=device)
        mtp_mask = torch.zeros(S, B, Tk, device=device)
        mtp_src[:, :, 0] = base
        mtp_mask[:, :, 0] = 1.0
        for k in range(1, Tk):
            mtp_src[: S - k, :, k] = base[: S - k] + k * B                 # index of (t+k, env)
            # valid iff valid at k-1 AND s_{t+k} did not start a new episode (dones[t+k]==0)
            mtp_mask[: S - k, :, k] = mtp_mask[: S - k, :, k - 1] * (1.0 - dones[k:])
        b_mtp_src = mtp_src.reshape(-1, Tk)                                # (batch, T)
        b_mtp_mask = mtp_mask.reshape(-1, Tk)                              # (batch, T)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v28 multi-token value loss: each of the T heads regresses (CE) to its
                # future λ-return target G_{t+k}, EQUAL weight per head (per-head mean over
                # valid samples, then mean over heads). value_logits is (mb, T, num_bins);
                # head_0 is the ordinary value used for EV/diagnostics. No value clipping.
                mb_mask = b_mtp_mask[mb_inds]                              # (mb, T)
                mb_targets = b_target_probs[b_mtp_src[mb_inds]]            # (mb, T, num_bins)
                value_log_probs = torch.log_softmax(value_logits, dim=-1)  # (mb, T, num_bins)
                ce = -(mb_targets * value_log_probs).sum(dim=-1)          # (mb, T)
                per_head = (ce * mb_mask).sum(0) / mb_mask.sum(0).clamp_min(1.0)  # (T,)
                v_loss = per_head.mean()
                v_loss_head0 = per_head[0].detach()                       # for logging
        ...
        writer.add_scalar("losses/value_loss_head0", float(v_loss_head0), global_step)
        writer.add_scalar("debug/mtp_mask_frac", b_mtp_mask.mean().item(), global_step)
