...
class Args:
    ...
    # v183: Dreamer4-S3.3 policy prediction -- action MTP head on the NON-detached
    # predicted summaries. Per-action-dim HL-Gauss CE, mean over act_dim (initial CE
    # ~ ln(num_bins), same scale as reward CE).
    lewm_action_loss_coef: float = 0.25  # v2: 0.5 -> 0.25 (halve latent-cosine pressure)
    # v2: skip pure-readback offsets. k=0's target is already in the AdaLN
    # conditioning; only k >= min_offset slots contribute to the action CE.
    lewm_action_mtp_min_offset: int = 1
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # v183: Dreamer4-S3.3 policy-prediction head. Reads the FULL flattened
        # 19-token predicted summary (actions are a function of the whole state; no
        # dedicated action token) NON-detached -- the one loss allowed to shape the
        # trunk. Per-action-dim HL-Gauss logits, one shared head across MTP offsets.
        self.action_num_bins = args.lewm_action_num_bins
        self.action_readout_ln = nn.LayerNorm(self.dim * self.num_latent_tokens)
        self.action_unproj = nn.Linear(self.dim * self.num_latent_tokens, act_dim * self.action_num_bins, bias=False)
    ...
    def normalize_world_model_matrices(self):
        ...
        for name, param in self.named_parameters():
            ...
            if name.endswith("reward_token_unproj.weight") or name.endswith("continuation_token_unproj.weight"):
                continue
            # v183: the action head must keep its zero-init (and later its learned
            # scale) -- unit-norming its rows every step would destroy both.
            if name.endswith("action_unproj.weight"):
                continue
    ...
    def decode_belief_actions(self, belief):
        # v183: policy-prediction head (Dreamer 4 S3.3). Intentionally NO detach --
        # unlike the outcome probes, the action CE is meant to back-prop through the
        # pred_next_proj/pred_mtp_projs into the trunk and shape the very embedding
        # the agent consumes. belief: (..., num_latent_tokens, D) -> logits
        # (..., act_dim, action_num_bins).
        feat = self.action_readout_ln(belief.flatten(-2))
        logits = self.action_unproj(feat)
        return logits.reshape(logits.shape[:-1] + (self.act_dim, self.action_num_bins))
...
if __name__ == "__main__":
    ...
    wm_outcome_io_params = (
        ...
        # v183: the action head shares the probe LR budget; its trunk gradient
        # (non-detached) still lands in wm_latent_params at base LR.
        + list(agent.world_model.action_readout_ln.parameters())
        + list(agent.world_model.action_unproj.parameters())
    )
    ...
    with torch.no_grad():
        ...
        # v183: uniform initial action dist AND zero trunk gradient at step 0
        # (dL/dbelief = W^T(p-q) = 0), so the non-detached representation pressure
        # ramps in only as the head learns. Safe: excluded from the WM unit-norm.
        agent.world_model.action_unproj.weight.zero_()
    ...
    def wm_supervised_loss_forward(
        ...
        # v183: policy prediction reads pred_mtp NON-detached -- the action CE is the
        # one loss allowed to shape the trunk (through pred_next_proj/pred_mtp_projs).
        pred_action_logits_all = agent.world_model.decode_belief_actions(pred_mtp)

        reward_loss_sum = pred_mtp.new_zeros(())
        action_loss_sum = pred_mtp.new_zeros(())
        ...
        for mtp_idx in range(mtp_len):
            ...
            # v183: action CE at slot (j, k): predict a_{t+j+k}, the action that
            # produced the predicted frame. Mean over act_dim keeps the initial CE
            # ~ ln(num_bins), the same scale as the reward CE.
            # v2: k < min_offset is pure AdaLN readback -- excluded.
            if mtp_idx >= args.lewm_action_mtp_min_offset:
                action_loss = -(
                    action_target_probs[:, mtp_idx:].detach()
                    * torch.log_softmax(pred_action_logits_all[:, :valid_horizon, mtp_idx], dim=-1)
                ).sum(dim=-1).mean(dim=-1)
                action_loss_sum = action_loss_sum + (action_loss * offset_valid).sum() / denom
        ...
        wm_loss = (
            ...
            + args.lewm_action_loss_coef * action_loss_sum
    ...
    def wm_supervised_metrics_forward(
        ...
        reward_loss_sum = pred_mtp.new_zeros(())
        action_loss_sum = pred_mtp.new_zeros(())
        action_abs_error_sum = pred_mtp.new_zeros(())
        discount_bce_sum = pred_mtp.new_zeros(())
        discount_entropy_sum = pred_mtp.new_zeros(())
        reward_abs_error_sums = []
        reward_error_sums = []
        ...
        for mtp_idx in range(mtp_len):
            ...
            reward_loss_sum = reward_loss_sum + (reward_loss * offset_valid).sum() / denom
            # v183: action CE + scalar MAE diagnostics (mean over act_dim).
            # v2: gated like the loss so the metric tracks what actually trains.
            if mtp_idx >= args.lewm_action_mtp_min_offset:
                action_loss = -(
                    action_target_probs[:, mtp_idx:]
                    * torch.log_softmax(pred_action_logits_all[:, :valid_horizon, mtp_idx], dim=-1)
                ).sum(dim=-1).mean(dim=-1)
                action_loss_sum = action_loss_sum + (action_loss * offset_valid).sum() / denom
                pred_action_scalar = action_support.to_scalar(pred_action_logits_all[:, :valid_horizon, mtp_idx])
                action_abs_error = (pred_action_scalar - future_actions[:, mtp_idx:]).abs().mean(dim=-1)
                action_abs_error_sum = action_abs_error_sum + (action_abs_error * offset_valid).sum() / denom
            discount_target = future_discounts[:, mtp_idx:]
            discount_bce = F.binary_cross_entropy_with_logits(
                pred_continuation_logits_all[:, :valid_horizon, mtp_idx],
                discount_target,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for _ in range(args.lewm_update_epochs):
            ...
            for start in range(0, args.batch_size, wm_minibatch_size):
                ...
                # v183: HL-Gauss action targets (project broadcasts over leading dims)
                # -> (mb, horizon, act_dim, num_bins).
                action_target_probs = action_support.project(future_actions)
                ...
                wm_action_losses.append(wm_action_loss.item())
                wm_action_abs_errors.append(wm_action_abs_error.item())
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/action_ce", float(np.mean(wm_action_losses)), global_step)
                writer.add_scalar("lewm/action_scalar_mae", float(np.mean(wm_action_abs_errors)), global_step)
