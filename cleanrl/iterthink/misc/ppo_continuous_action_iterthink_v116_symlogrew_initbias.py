...
#     [-20, 50], while scalar reward consumption decodes as E[symexp(bin)] like
#     the critic's symlog scalar decode.
#   - The reward bucket head is initialized to a smoothed zero-reward prior:
#     final weights are zero and bias is log(project(0) + eps). This removes the
...
class Args:
    ...
    lewm_reward_prior_smoothing: float = 1e-4
...
if __name__ == "__main__":
    ...
    scalar_reward_support = symexp(reward_support.support)

    def reward_logits_to_scalar(logits):
        return (torch.softmax(logits, dim=-1) * scalar_reward_support).sum(dim=-1)

    with torch.no_grad():
        zero_reward_target = reward_support.project(torch.zeros(1, device=device))[0]
        zero_reward_target = zero_reward_target + args.lewm_reward_prior_smoothing
        zero_reward_target = zero_reward_target / zero_reward_target.sum()
        agent.world_model.belief_reward_unproj.weight.zero_()
        agent.world_model.belief_reward_unproj.bias.copy_(
            torch.log(zero_reward_target)
        )
