# PPO + IterThink v24 densepercept v1: DenseNet reach-back that also concats
# REUSED PERCEPTRONS. Built byte-for-byte on the strongest v24 base (symlog_v1,
# ~9785 on HalfCheetah); the ONLY change is ThinkTrunk. v24's DenseNet feeds each
# block the concat of all previous block OUTPUTS; here each block ALSO sees every
# earlier block's reusable perceptron re-applied to the CURRENT state (recall_j on
# RMSNorm(latest)), concatenated into the same bank. Motivation: across this
# session, replacing v24's trunk always lost, but (a) v24's concat+linear reach-
# back is the proven winner and (b) perceptron reuse durably helped at narrow
# width (v24's H=64 regime). This fuses them: a cumulative perceptron library
# composed by CONCAT (not the softmax router that decayed), enriching the proven
# DenseNet. Hypothesis: fresh re-compositions of earlier circuits give the block
# strictly more useful reach-back than cached outputs alone -> beats v24.
#
# ---- v24 symlog base (unchanged below) ----
...
class ThinkTrunk(nn.Module):
    """DenseNet trunk (v24) whose reach-back bank ALSO carries reused perceptrons.

    v24 fed block k the concat of all previous block OUTPUTS [x0, out_0..out_{k-1}]
    (cached representations). This variant additionally concatenates, for every
    earlier block j<k, that block's reusable perceptron RE-APPLIED to the current
    running state: recall_j(RMSNorm(feats[-1])). So block k sees both the cached
    DenseNet outputs and fresh re-compositions of every earlier circuit on the
    latest representation, combined by its learned in_proj (concat + linear, the
    mechanism v24 already shows works) rather than a softmax router (which decayed
    in the perceptronreuse experiments). recall_j is introduced at block j and
    reused by all later blocks: a cumulative perceptron library composed by concat.

    Block k input width: H*(k+1) [outputs] + H*k [reused perceptrons] = H*(2k+1).
    The final readout is unchanged from v24 (concat of block outputs only), so the
    only change is a richer reach-back; the reused info reaches the readout via the
    block outputs that integrate it.
    """
    ...
    def __init__(self, in_dim, H, K, n_experts):
        ...
        for k in range(K):
            # [x0, out_0..out_{k-1}] = H*(k+1) ; reused recall_0..recall_{k-1} = H*k
            block_in_dim = H * (2 * k + 1)
            self.blocks.append(ThinkBlock(block_in_dim, H, n_experts))
        # Reusable perceptron library: recall_j introduced at block j (j<K-1), reused
        # by every later block. Block K-1 introduces no recall (nothing reuses it).
        self.recall = nn.ModuleList([_branch_body(H) for _ in range(max(K - 1, 0))])
        self.recall_norm = nn.RMSNorm(H, elementwise_affine=False)
        cat_dim = H * (K + 1)
    ...
    def forward(self, x):
        ...
        for k, block in enumerate(self.blocks):
            # Re-apply every earlier block's perceptron to the CURRENT state, concat
            # alongside the cached DenseNet outputs (perceptron reuse via concat).
            z = self.recall_norm(feats[-1])
            reused = [self.recall[j](z) for j in range(k)]    # k tensors (B, H)
            cat_feats = torch.cat(feats + reused, dim=-1)      # (B, H*(2k+1))
            feats.append(block(cat_feats, x0))
