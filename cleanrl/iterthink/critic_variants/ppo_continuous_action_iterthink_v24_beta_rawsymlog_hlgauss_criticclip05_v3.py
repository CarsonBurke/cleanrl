# PPO + IterThink v24 Beta raw-return symlog HL-Gauss critic, critic-clip v3.
#
# Ablation hypothesis: keep iterthink_v24_beta_s1 / v24 Beta policy machinery,
# but remove Gym return/reward normalization and train the critic directly on
# raw HalfCheetah returns with an Orbit-Wars-style symlog HL-Gauss head. Raw
# HalfCheetah scores stay below ~20k, so the support uses raw ±20k endpoints
# transformed into symlog bin coordinates rather than Orbit-Wars' much wider
# ±100k margin range.
#
# v2 fix: match Orbit-Wars' value-head cold start. The v1 raw-return port kept
# the normalized-return variant's sharp Gaussian logit bias around zero; on
# symlog ±20k support that makes the critic overconfident before raw λ-returns
# have even reached the hundreds. This variant zero-inits the final critic head
# with no bias, giving uniform logits and decoded scalar 0 at step 0.
#
# v3 fix: raw-return HL-Gauss produces larger critic gradients than the
# normalized-return line. Keep actor clip unchanged, but relax the separate
# critic gradient cap from 0.25 to 0.5 so the critic can learn state-dependent
# spread instead of tracking only the batch mean.
#
# Variant hypothesis: keep v24 beta PPO machinery and vf_coef=0.5, but replace
# the critic target with Dreamer4-style scalar-return HL-Gauss:
#   scalar GAE return -> Gaussian-smoothed categorical target over value bins.
# This removes v24's self-referential distributional lambda-return recursion,
# which may be suppressing critic gradient scale.
#
...
def value_support_bounds(args):
    """Return critic support endpoints in the coordinate system used by the bins.

    Args.v_min/v_max remain raw return bounds. With symlog enabled, Orbit-Wars
    semantics transform those raw endpoints before constructing the histogram
    support; targets are likewise symlogged by HLGaussSupport.project().
    """
...
class Args:
    ...
    critic_grad_clip: float = 0.5    # raw-return critic needs more head/trunk update than normalized-return PPO

    # Distributional critic support. Raw HalfCheetah returns stay below ~20k;
    # with value_symlog=True these raw endpoints are transformed into symlog
    # support coordinates to match Orbit-Wars / Dreamer-style HL-Gauss.
    num_bins: int = 511
    v_min: float = -20000.0
    v_max: float = 20000.0
    value_symlog: bool = True
    value_sigma_to_bin_ratio: float = 0.5  # Orbit-Wars adapter default
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Categorical critic with Orbit-Wars-style uniform cold start: logits are
        # all zero, so the decoded scalar is still 0 on the symmetric support,
        # but the critic is not initialized as a false low-variance zero-return
        # predictor before it has seen raw λ-return targets.
        self.critic_head = nn.Linear(H, args.num_bins, bias=False)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                next_value_logits = agent.get_value(next_obs)
                next_value_bonus = None
            next_value = hl_support.to_scalar(next_value_logits).reshape(1, -1)
            ...
            # raw return units. Center around E[symexp(z)] for a mathematically
            # consistent raw-scale distribution std; the scalar value used by GAE
            # remains Orbit-Wars' symexp(E[z]).
            raw_value_mean = (value_probs * raw_support).sum(-1)
            sigma = (value_probs * (raw_support - raw_value_mean.unsqueeze(-1)) ** 2).sum(-1).clamp_min(0).sqrt()  # (T,B)
            if args.value_symlog:
                local_sigma_floor = args.sigma_floor_bins * bin_width * (raw_value_mean.abs() + 1.0)
                sigma = torch.maximum(sigma, local_sigma_floor)
            else:
        ...
        b_raw_value_mean = raw_value_mean.reshape(-1)
        ...
        writer.add_scalar("debug/values_mean", b_values.mean().item(), global_step)
        writer.add_scalar("debug/values_std", b_values.std().item(), global_step)
        writer.add_scalar("debug/raw_value_mean", b_raw_value_mean.mean().item(), global_step)
        writer.add_scalar("debug/gae_mean", gae.mean().item(), global_step)
        writer.add_scalar("debug/gae_std", gae.std().item(), global_step)
