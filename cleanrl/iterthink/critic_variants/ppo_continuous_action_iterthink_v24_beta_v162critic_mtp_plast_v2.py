...
# on the plain-MLP base), retested here on the iterthink base with an EXPANSIVE per-neuron
# clamp [0.0625,16] so the global LR is free to swing widely (up to 16x / down to 1/16x).
# Selected by --plast-gate-mode {relative,absolute}. See plast_v3 for the drift-bounded
# variant (decoupled shape, but the layer-mean gate held within a bounded band).
