# PPO + IterThink v24 BETA — CRITIC-10 + HL-Gauss symlog critic v1.
#
# iterthink_v24 beta ARCHITECTURE (heavy DenseNet ThinkTrunk + soft-MoE backbone,
# unimodal Beta policy) trained with an ASYMMETRIC actor/critic schedule and a
# DreamerV3-style HL-Gauss symlog distributional critic. (The schedule + critic are
# borrowed from the beta_v3 line run with its gate OFF — there is NO delight gate here;
# the actor is a plain policy-gradient surrogate, gate disabled / not implemented.)
#
# THE RECIPE:
#   - ACTOR 1 epoch / CRITIC 10 epochs, in two separate update loops with their own
#     epoch counts. Actor: one on-policy pass per rollout (z-replay). Critic: many
#     epochs of pure distributional regression to keep the value — and hence the
#     advantages — well fit.
#   - SHARED BACKBONE (share_backbone=True): ONE ThinkTrunk feeds both heads, updated by
#     BOTH loops through a single coherent optimizer. Because the critic loop steps the
#     shared trunk 10x per rollout vs the actor's 1x, the value gradient dominates the
#     shared features — the classic shared-trunk contention. (Set share_backbone=False
#     for separate actor/critic trunks that decouple them.)
#   - NO RATIO CLIP (clip_ratio=False): plain importance-weighted PG surrogate
#     -A*ratio, no PPO clamp. With a shared trunk the 10 critic epochs shift the trunk
#     BEFORE the actor pass, so even the first actor minibatch is slightly off-policy;
#     the (unclipped) ratio importance-corrects. The only step leash is max_grad_norm.
#   - NO KL EARLY-STOP (target_kl=None).
#   - NO ADVANTAGE NORMALIZATION AT ALL: norm_adv=False, no EMA-RMS scaling. Raw GAE.
#   - HL-GAUSS SYMLOG DISTRIBUTIONAL CRITIC (replaces iterthink's linear-C51 head +
#     distributional Bellman backup). num_bins=101, support [-10,10] in SYMLOG space
#     (symexp(10)≈22026 raw, so any return scale is covered with fine resolution near
#     0), sigma_ratio=0.75. The target is the HL-Gauss projection of the SCALAR GAE
#     lambda-return (returns = advantages + values), trained by cross-entropy and
#     decoded as symexp(E[symlog bin]). No distributional bootstrap => no C51 blowup,
#     so the critic head uses a plain init (no peaked-at-0 bias trick).
#   - max_grad_norm=0.5.
#
# KEPT FROM iterthink_v24 beta:
#   - ThinkTrunk (entry -> K dense-concat ThinkBlocks {dense + softmax-MoE} -> proj).
#   - Unimodal Beta actor: alpha,beta = 1 + softplus(head); native z in (0,1) linearly
#     rescaled to the env action range; constant Jacobian drops out of the ratio.
#   - z-replay (buffer the native Beta sample; recompute log_prob at the same draw).
#   - NormalizeReward wrapper kept (only ADVANTAGE normalization was removed). NOTE:
#     reward-norm + no-adv-norm reintroduces late-training advantage-shrinkage (the
#     scale compresses as the policy improves with nothing to re-inflate it); watch
#     debug/raw_adv_rms drift.
#   - num_steps=2048 (iterthink); beta_v3 used 128 -> more frequent updates, a knob.
#
# HYPOTHESIS: the iterthink representation + a clean HL-Gauss critic refit 10x, feeding
# raw advantages to a single-pass unclipped PG actor, learns well. Bar: the gate-off
# beta_v3 reference and the iterthink line.
import os
import random
import time
...
class Args:
    ...
    actor_epochs: int = 1            # DG recipe: ONE faithful on-policy actor pass per rollout
    critic_epochs: int = 10          # DG recipe: refit the critic many times (pure regression)
    ...
    # Backbone sharing. True => ONE ThinkTrunk for both heads, updated by both loops via
    # a SINGLE coherent optimizer (the critic's 10 epochs dominate the shared trunk).
    # False => separate actor/critic trunks with their own optimizers (decoupled).
    share_backbone: bool = True

    # HL-Gauss symlog distributional critic (from dg_beta_v3).
    critic_num_bins: int = 101
    critic_v_min: float = -10.0      # SYMLOG-space support min: symexp(-10) ≈ -2.2e4 raw
    critic_v_max: float = 10.0       # SYMLOG-space support max
    critic_sigma_ratio: float = 0.75 # HL-Gauss label sigma as a fraction of bin width
    critic_symlog: bool = True       # symlog-scale targets (DreamerV3-style) -> any value range
...
class Agent(nn.Module):
    """Separate (or optionally shared) ThinkTrunks: unimodal-Beta actor + HL-Gauss
    distributional critic. Actor and critic are queried independently so the two
    ...
    def __init__(self, envs, args):
        ...
        # HL-Gauss distributional value head: num_bins logits over the symlog support.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # HL-Gauss target probs for the scalar returns (Gaussian-smoothed two-hot in symlog space).
        ...
        # ---- Actor update: ONE PPO clipped-ratio pass on raw advantages (z-replay) ----
        actor_gns, clipfracs = [], []
        ...
        # Watch this drift DOWN: reward-norm compresses the reward scale as the policy
        # improves and there is no adv-norm to re-inflate it (the dg_beta_v3 v3 concern).
