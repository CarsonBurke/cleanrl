# PPO + IterThink v24 Beta + v162 critic, Dreamer3-bucket HL-Gauss MTP v2.
#
...
# changing only the support geometry to Dreamer3-style nonuniform buckets:
#   - raw-reward HL-Gauss labels over symexp(linspace(v_min, v_max)) centers
#   - Gaussian mass integrated over midpoint bucket cells in symlog coordinate
#   - expected-scalar decode E[raw bin center] for values and bootstraps
...
# This isolates whether Dreamer3's wide, nonuniform value buckets improve the
# v162 critic geometry without importing the v162 world model, CUDA graph path,
# or imagined updates.
#
...
from cleanrl.shared.hl_gauss import symlog, symexp

SAMPLE_EPS = 1e-6  # clamp Beta samples off the open-interval boundary (avoid log(0))


def value_support_bounds(args):
    """Return Dreamer3 bucket coordinate endpoints."""
    return args.v_min, args.v_max


class Dreamer3HLGaussSupport:
    """Dreamer3 symexp-spaced value buckets with HL-Gauss labels.

    Dreamer3's `symexp_twohot` head uses raw centers built by mirroring
    `symexp(linspace(-coord_max, 0))` and decodes as E[raw bin]. This class
    keeps that scalar consumption but replaces two-hot interpolation with
    Gaussian mass over midpoint bucket cells in symlog coordinate. Targets are
    raw returns; only the histogram coordinate is symlog.
    """

    def __init__(self, num_bins, coord_min, coord_max, sigma_ratio, device, eps=1e-10):
        if num_bins % 2 != 1:
            raise ValueError("Dreamer3HLGaussSupport requires an odd num_bins for a single zero bucket")
        if not np.isclose(abs(coord_min), abs(coord_max)):
            raise ValueError("Dreamer3HLGaussSupport expects symmetric coordinate endpoints")
        self.num_bins = num_bins
        self.coord_min = coord_min
        self.coord_max = coord_max
        self.eps = eps
        half_count = (num_bins - 1) // 2 + 1
        coord_half = torch.linspace(coord_min, 0.0, half_count, device=device)
        self.coord_support = torch.cat([coord_half, -coord_half[:-1].flip(0)], dim=0)
        self.coord_bin_width = (coord_max - coord_min) / (num_bins - 1)
        half_width = 0.5 * self.coord_bin_width
        self.coord_edges = torch.linspace(
            coord_min - half_width,
            coord_max + half_width,
            num_bins + 1,
            device=device,
        )
        self.sigma = sigma_ratio * self.coord_bin_width
        self.support = symexp(self.coord_support)
        self.raw_min = self.support[0].item()
        self.raw_max = self.support[-1].item()

    def probs_to_expected_scalar(self, probs):
        # Symmetric summation matches Dreamer3's zero-at-uniform decode behavior.
        n = self.num_bins
        m = (n - 1) // 2
        p_neg = probs[..., :m]
        p_zero = probs[..., m : m + 1]
        p_pos = probs[..., m + 1 :]
        b_neg = self.support[:m]
        b_zero = self.support[m : m + 1]
        b_pos = self.support[m + 1 :]
        return (p_zero * b_zero).sum(-1) + ((p_neg * b_neg).flip(-1) + p_pos * b_pos).sum(-1)
    ...
    def project(self, targets):
        raw_targets = targets.clamp(self.raw_min, self.raw_max)
        coord_targets = symlog(raw_targets).clamp(self.coord_min, self.coord_max).unsqueeze(-1)
        cdf_evals = torch.erf(
            (self.coord_edges.unsqueeze(0) - coord_targets) / (self.sigma * np.sqrt(2.0))
        )
...
class Args:
    ...
    # v162 critic with Dreamer3 bucket geometry. The endpoints are symlog
    # coordinates; raw centers are symexp(linspace(v_min, v_max, num_bins)).
    # Defaults preserve v162's symmetric +/-20k raw-return range.
    num_bins: int = 511
...
if __name__ == "__main__":
    ...
    if not args.value_symlog:
        raise ValueError("Dreamer3 bucket HL-Gauss requires --value-symlog=true")
    support_min, support_max = value_support_bounds(args)
    hl_support = Dreamer3HLGaussSupport(
        args.num_bins,
        support_min,
        support_max,
    ...
    support = hl_support.coord_support                 # bin centers in symlog coordinate
    bin_width = hl_support.coord_bin_width
    raw_support = hl_support.support                    # Dreamer3 raw bin centers

    def value_logits_to_scalar(logits):
        return hl_support.to_expected_scalar(logits)

    raw_bin_width_floor = (raw_support[1:] - raw_support[:-1]).abs().min().item()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # CDF-rank u in the critic's uniform Dreamer3 bucket coordinate.
            returns_coord = symlog(returns)
