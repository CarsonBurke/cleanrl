# PPO + IterThink v24 beta + normalized feedforward backbone (nff_v1).
#
# KEY IDEA. Variant from iterthink_v24_beta_simbav2_v1: keep the v24_beta PPO,
# Beta actor, rankgauss advantage, and symlog HL-Gauss critic, but replace the
# SimBaV2 ReLU HyperMLP trunk with lucidrains' nFF stack
# (x_mlps_pytorch/nff.py, arXiv:2410.01131). The hypothesis is that SiLU-gated
# normalized feedforwards give smoother policy/critic features than SimBaV2's
# ReLU inverted bottleneck while retaining the important hyperspherical dynamics.
#
# nFF backbone (ported from lucidrains/x-mlps-pytorch nff.py):
#   input: optional const c_shift append -> l2norm -> normalized projection -> l2norm
#   block: x <- l2norm(lerp(x, nFF(x), alpha)); nFF = SiLU(gate) * hidden, all
#          through normalized bias-free Linear layers with learned per-channel scales.
#   default shared trunk: H=512, depth=2, expand=4, input_preserve_magnitude=True.
#
# OPTIMIZER: inherited from simbav2_v1 (plain Adam by default, optional AdamW).
# nFF uses parametrized normalized weights and we explicitly re-normalize them
# after each step, so weight decay defaults to 0.
#
...
# This isolates the NFF BACKBONE as the only lever vs the v24 bar (~9785).
# v2 candidates (deferred, kept fixed here to avoid confounds): normalized actor
# heads, NFF critic readout, RSNorm input, reward scaling.
#
...
class Args:
    ...
    weight_decay: float = 0.0             # nFF normalized weights default to no decay; >0 -> AdamW
    # DECOUPLED actor/critic learning rates. The normalized actor needs a high PPO lr to
    ...
    # nFF backbone geometry. share_backbone=True (inherited Args) -> ONE shared trunk
    # (shared_hidden/shared_blocks) feeds both heads with decoupled dual-clip (v24's v21
    # winner); False -> SEPARATE asymmetric actor/critic trunks.
    ...
    critic_blocks: int = 2
    nff_expand_factor: float = 4.0
    nff_input_preserve_magnitude: bool = True
    nff_manual_norm_weights: bool = False
    c_shift: float = 3.0       # constant feature appended before l2norm when preserving magnitude
    critic_head: str = "linear"  # "linear" (plain) | "nff" (normalized value readout)
...
# ============================ normalized feedforward backbone ============================
# Ported from lucidrains/x-mlps-pytorch/x_mlps_pytorch/nff.py. Every hidden state is
# l2-normalized, and every NormLinear effective weight is l2-normalized by PyTorch's
# parametrization API unless manual_norm_weights=True.
...
def norm_nff_weights_(parent_module: nn.Module):
...
class Residual(nn.Module):
    ...
    def forward(self, x):
        residual = x
        out = l2normalize(self.fn(x))
        return l2normalize(residual.lerp(out, self.branch_scale()))
...
class L2Norm(nn.Module):
    ...
    def forward(self, t):
        return l2normalize(t, dim=self.dim)
    ...
    def __init__(
        ...
        *,
        ...
        for (
            ...
            self.layers.append(
                Residual(ff, dim, default(alpha_ff_init_, alpha_init), default(alpha_ff_scale_, dim ** -0.5))
            )
        ...
        if self.need_proj_out:
            ...
            self.proj_out_scale = Scale(dim_out, 1.0, dim ** -0.5)

    @torch.no_grad()
    def norm_weights_(self):
        norm_nff_weights_(self)
...
class NFFBackbone(nn.Module):
    """Drop-in trunk: forward maps (B, in_dim) -> (B, hidden_dim)."""

    def __init__(self, in_dim, hidden_dim, depth, expand_factor, input_preserve_magnitude, c_shift, manual_norm_weights):
        super().__init__()
        self.net = NFeedforwards(
            hidden_dim,
            depth,
            dim_in=in_dim,
            ff_expand_factor=expand_factor,
            input_preserve_magnitude=input_preserve_magnitude,
            constant_shift=c_shift,
            manual_norm_weights=manual_norm_weights,
        )
...
class NFFCategoricalHead(nn.Module):
    """Normalized value readout. Bias is kept separate for the zero-return HL-Gauss prior."""

    def __init__(self, hidden_dim, num_bins, expand_factor, manual_norm_weights):
        super().__init__()
        self.net = NFeedforwards(
            hidden_dim,
            1,
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if self.share_backbone:
            # ONE normalized trunk feeds both heads (v24's v21 winner). The decoupled
            # dual-backward clip keeps the value gradient from swamping the policy features.
            actor_H = critic_H = args.shared_hidden
            self.trunk = NFFBackbone(
                obs_dim,
                args.shared_hidden,
                args.shared_blocks,
                args.nff_expand_factor,
                args.nff_input_preserve_magnitude,
                args.c_shift,
                args.nff_manual_norm_weights,
            )
        else:
            # SEPARATE asymmetric actor/critic backbones.
            actor_H = args.actor_hidden
            critic_H = args.critic_hidden
            self.actor_trunk = NFFBackbone(
                obs_dim,
                actor_H,
                args.actor_blocks,
                args.nff_expand_factor,
        ...
        if args.critic_head in ("nff", "hyperspherical"):
            self.critic_head = NFFCategoricalHead(
                critic_H,
                args.num_bins,
                args.nff_expand_factor,
                args.nff_manual_norm_weights,
    ...
    @torch.no_grad()
    def norm_nff_weights_(self):
        # Keep NormLinear originals on the unit sphere after optimizer updates. With
        # parametrization this is redundant for forward math but prevents unbounded
        # original-weight norms; with manual_norm_weights it is required.
        norm_nff_weights_(self)
...
if __name__ == "__main__":
    ...
    agent.norm_nff_weights_()  # nFF: unit-sphere effective weights at init
    # Disjoint actor/critic param partition (fully separate backbones, no sharing).
    actor_params = agent.actor_parameters()
    critic_params = agent.critic_parameters()
    # nFF optimizer: plain Adam + per-step normalized-weight refresh (NO weight decay).
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.step()
                    agent.norm_nff_weights_()  # nFF: refresh normalized weights
                else:
