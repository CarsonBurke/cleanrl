# PPO + IterThink v24.21 (STATE-CONDITIONED INNOVATION AUTOREG BETA). From v24.16.
#
# Hypothesis: v24.16 works because it adds small conditional action covariance in
# native bounded Beta space. v24.20 underperformed because the residual MLP could
# become a duplicate state-only actor head. v24.21 keeps the exact teacher-forced
# AR-Beta density but makes only the triangular coefficients state-dependent:
# Policy:
#   base Beta gives mean m(s)
#   residual_i = sum_{j<i} W_ij(s) * (z_j - m_j(s))
#   p(z|s) = prod_i Beta(z_i | alpha_i(s,z_<i), beta_i(s,z_<i)).
# Dimension 0 has exactly zero residual, and if previous actions equal their base
# means then every residual is zero. The coefficient head is zero-initialized, so
# startup is exactly v24; rollout stores native z and PPO replay teacher-forces z.
import os
import random
import time
...
class Args:
    ...
    # v24.21: state-conditioned triangular innovation residual. Coefficients
    # depend on state, but residuals only multiply previous action innovations.
    autoreg_coef_hidden: int = 64
    autoreg_coef_scale: float = 0.25
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v24.21 autoregressive Beta. The base heads are exactly v24's independent
        # Beta logits. A state-conditioned coefficient head predicts bounded
        # triangular weights on previous action innovations; zero init makes
        # startup exactly v24.
        self.act_dim = act_dim
        self.actor_alpha_head = layer_init(nn.Linear(H, act_dim), std=0.01)
        self.actor_beta_head = layer_init(nn.Linear(H, act_dim), std=0.01)
        self.autoreg_coef_scale = float(args.autoreg_coef_scale)
        self.autoreg_coef_head = nn.Sequential(
            layer_init(nn.Linear(H, args.autoreg_coef_hidden), std=0.5),
            nn.SiLU(),
            layer_init(nn.Linear(args.autoreg_coef_hidden, 2 * act_dim * act_dim), std=0.0),
        )
    ...
    def _base_logits_and_mean(self, actor_feat):
        base_alpha = self.actor_alpha_head(actor_feat)
        base_beta = self.actor_beta_head(actor_feat)
        base_dist = Beta(1.0 + F.softplus(base_alpha), 1.0 + F.softplus(base_beta))
        return base_alpha, base_beta, base_dist.mean.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)

    def _autoreg_coefficients(self, actor_feat):
        B = actor_feat.shape[0]
        raw = self.autoreg_coef_head(actor_feat).view(B, 2, self.act_dim, self.act_dim)
        coeff = self.autoreg_coef_scale * raw.tanh() * self.autoreg_mask.view(1, 1, self.act_dim, self.act_dim)
        return coeff[:, 0], coeff[:, 1]

    def _autoreg_residual(self, actor_feat, z, base_mean=None):
        if base_mean is None:
            _, _, base_mean = self._base_logits_and_mean(actor_feat)
        innovation = z - base_mean
        alpha_coeff, beta_coeff = self._autoreg_coefficients(actor_feat)
        alpha_resid = torch.einsum("bij,bj->bi", alpha_coeff, innovation)
        beta_resid = torch.einsum("bij,bj->bi", beta_coeff, innovation)
        return alpha_resid, beta_resid, alpha_coeff, beta_coeff

    def _beta_params(self, actor_feat, z):
        base_alpha, base_beta, base_mean = self._base_logits_and_mean(actor_feat)
        alpha_resid, beta_resid, _, _ = self._autoreg_residual(actor_feat, z, base_mean)
    ...
    def _sample_autoreg_z(self, actor_feat):
        ...
        for i in range(self.act_dim):
            innovation = z - base_mean
            alpha_resid_i = (alpha_coeff[:, i] * innovation).sum(dim=-1)
            beta_resid_i = (beta_coeff[:, i] * innovation).sum(dim=-1)
            alpha_i = 1.0 + F.softplus(base_alpha[:, i] + alpha_resid_i)
            beta_i = 1.0 + F.softplus(base_beta[:, i] + beta_resid_i)
            zi = Beta(alpha_i, beta_i).sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            z[:, i] = zi
        return z
    ...
    def actor_parameters(self):
        ...
        heads = (
            ...
            + list(self.autoreg_coef_head.parameters())
        )
    ...
    def actor_diagnostics(self, x, z):
        ...
        alpha_coeff, beta_coeff = self._autoreg_coefficients(actor_feat)
        coeff = torch.cat([alpha_coeff, beta_coeff], dim=1)
        return {
            "cond_residual_rms": resid.pow(2).mean().sqrt(),
            "cond_coeff_rms": coeff.pow(2).mean().sqrt(),
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        writer.add_scalar("debug/cond_coeff_rms", actor_diag["cond_coeff_rms"].item(), global_step)
