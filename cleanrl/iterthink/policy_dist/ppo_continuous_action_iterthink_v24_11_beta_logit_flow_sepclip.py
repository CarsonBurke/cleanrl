# PPO + IterThink v24.11 (BETA LOGIT ACTION FLOW, SEPARATE CFM CLIP). From v24.9.
#
# Isolated ablation: keep v24.9's flow and CFM target unchanged, but give the
# CFM auxiliary its own gradient budget over `action_flow` parameters. PPO actor
# gradients are clipped first as in v24.9, then CFM gradients are clipped
# separately and added only to the flow blocks before the optimizer step.
#
...
class Args:
    ...
    beta_flow_cfm_grad_clip: float = 0.05
...
class Agent(nn.Module):
    ...
    def beta_flow_cfm_loss(self, x, target_z, advantages, weight_clip):
        if self.actor_dist != "beta_flow":
        ...
        with torch.no_grad():
            ...
            y0 = safe_logit(z0)
            target_z = target_z.detach().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        if self.actor_dist == "beta_flow":
            ...
            if z is None:
                ...
                y0 = safe_logit(z0)
                y1 = self.action_flow(y0, actor_feat)
                z = torch.sigmoid(y1).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            else:
                z = z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                y1 = safe_logit(z)
    ...
    def beta_flow_parameters(self):
        if self.actor_dist != "beta_flow":
            return []
        return list(self.action_flow.parameters())
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        clipfracs = []
        cfm_losses = []
        cfm_gn = torch.tensor(0.0, device=device)
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    actor_grads = [(p, p.grad.detach().clone()) for p in actor_params if p.grad is not None]
                    cfm_grads = []
                    if args.actor_dist == "beta_flow" and args.beta_flow_cfm_coef > 0.0:
                        flow_params = agent.beta_flow_parameters()
                        optimizer.zero_grad(set_to_none=True)
                        (args.beta_flow_cfm_coef * cfm_loss).backward()
                        cfm_gn = nn.utils.clip_grad_norm_(flow_params, args.beta_flow_cfm_grad_clip)
                        cfm_grads = [(p, p.grad.detach().clone()) for p in flow_params if p.grad is not None]
                        optimizer.zero_grad(set_to_none=True)
                    for p, g in actor_grads:
                        p.grad = g
                    for p, g in cfm_grads:
        ...
        if args.actor_dist == "beta_flow":
            writer.add_scalar("losses/beta_flow_cfm_grad_norm", float(cfm_gn), global_step)
