# PPO + IterThink v24.10 (BETA LOGIT ACTION FLOW, SCALED CFM TARGET). From v24.9.
#
# Isolated ablation: fix the CFM target scale for additive half-coupling blocks.
# v24.9's flow uses `flow_steps` blocks, each updates only half the action dims
# with dt=1/flow_steps. With alternating masks, each dimension is updated
# flow_steps/2 times, so training each block against the full straight-line
# velocity under-scales the supervised transport. v24.10 multiplies each block's
# per-dimension target by flow_steps / updates_per_dim (=2 for the default even
# flow_steps) inside the CFM auxiliary only.
#
...
class BetaLogitActionFlow(nn.Module):
    ...
    def cfm_loss(self, y0, y1, cond, weights):
        ...
        losses = []
        updates_per_dim = max(self.flow_steps // 2, 1)
        target_scale = float(self.flow_steps) / float(updates_per_dim)
        for i, block in enumerate(self.blocks):
            t = self._time(i, y0.shape[0], y0.device, y0.dtype)
            y_t = (1.0 - t) * y0 + t * y1
            losses.append(block.cfm_loss(y_t, cond, t, target_scale * target_velocity, weights))
