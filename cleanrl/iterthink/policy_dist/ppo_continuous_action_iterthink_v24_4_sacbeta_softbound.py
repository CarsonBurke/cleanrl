# PPO + IterThink v24.4 (SAC-BETA SHIFTED LOG-CONCENTRATIONS, SOFT-BOUNDED). From v24.3.
#
# v24.4 keeps v24.3's SAC-Beta shifted log-concentration family but swaps the hard
# clamp for the smooth tanh-rescale bound used in the Gaussian log_std path. This
# preserves the bounded concave/unimodal Beta:
#     alpha = exp(log_shift_a) + 1
#     beta  = exp(log_shift_b) + 1
# while avoiding zero gradients when a concentration head hits the clip boundary.
# It is a PPO-idiomatic refinement of the exact ../sac-beta mapping.
#
...
# remains calibrated. With auto-entropy off, the PPO update is unchanged; v24.4 changes
...
#
#   actor_dist="beta"  (DEFAULT, soft-bounded SAC-Beta shifted log-concentration path):
#       unimodal Beta, with a smooth bound around ../sac-beta's shifted-log concentrations:
#           log_shift_a = rescale(tanh(head_a), (-1,1), (beta_log_shift_min,beta_log_shift_max))
#           log_shift_b = rescale(tanh(head_b), (-1,1), (beta_log_shift_min,beta_log_shift_max))
#           alpha = 1 + exp(log_shift_a);  beta = 1 + exp(log_shift_b)
...
# softplus(head)+1 -> exp(soft-bounded shifted-log head)+1.
...
class Args:
    ...
    # v24.4: action distribution. "beta" (soft-bounded SAC-Beta shifted log concentration) | "gaussian"
    ...
    beta_log_shift_min: float = -20.0  # beta: lower bound for soft shifted log-concentration
    beta_log_shift_max: float = 2.0    # beta: exp(max)+1 ~= 8.39 concentration cap
    beta_log_shift_init: float = 0.0   # beta: initial shifted log-concentration target (0 => alpha=beta=2)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v24.4: soft-bounded SAC-Beta shifted-log-concentration default plus SAC-style Gaussian path.
        ...
        if self.actor_dist == "gaussian":
            ...
            # SAC-Beta family with a smooth tanh-rescale bound on shifted log-concentrations.
            if not (args.beta_log_shift_min < args.beta_log_shift_init < args.beta_log_shift_max):
                raise ValueError(
                    "beta_log_shift_init must lie inside "
                    f"({args.beta_log_shift_min}, {args.beta_log_shift_max}); got {args.beta_log_shift_init}"
                )
            raw_beta_logshift_init = float(
                np.arctanh(
                    rescale(
                        args.beta_log_shift_init,
                        (args.beta_log_shift_min, args.beta_log_shift_max),
                        (-1.0, 1.0),
                    )
                )
            )
            self.actor_alpha_logshift_head = layer_init(
                nn.Linear(H, act_dim), std=0.01, bias_const=raw_beta_logshift_init
            )
            self.actor_beta_logshift_head = layer_init(
                nn.Linear(H, act_dim), std=0.01, bias_const=raw_beta_logshift_init
            )
    ...
    def _actor_dist(self, actor_feat):
        ...
        # beta: SAC-Beta shifted log-concentrations with smooth bounds.
        log_shift_alpha = rescale(
            self.actor_alpha_logshift_head(actor_feat).tanh(),
            (-1.0, 1.0),
            (self.beta_log_shift_min, self.beta_log_shift_max),
        )
        log_shift_beta = rescale(
            self.actor_beta_logshift_head(actor_feat).tanh(),
            (-1.0, 1.0),
            (self.beta_log_shift_min, self.beta_log_shift_max),
        )
