# PPO + IterThink v98 (v24 Beta + small root-action TPO auxiliary). From v24.
#
# Failed pure-TPO variants replaced PPO's dense MuJoCo policy gradient with a
# sampled-simplex cross-entropy target and learned far too slowly. v98 keeps the
# v24 PPO actor/critic path intact and adds TPO only as a small local preference
# regularizer over root action candidates sampled from the rollout policy. The
# executed action is candidate 0; other candidates are scored by a cheap local
# advantage proxy A(s,a0) * exp(logp_old(a_k|s)-logp_old(a0|s)), clipped. This
# preserves PPO's credit assignment while letting TPO gently redistribute
# probability mass among nearby old-policy actions.
#
# Hypothesis: in dense-reward MuJoCo, TPO is useful as local action-ranking
# pressure, not as a replacement actor objective.
#
# --- inherited v24 notes ---
...
class Args:
    ...
    # v98: small TPO auxiliary. PPO remains the primary actor objective.
    tpo_group_size: int = 8
    tpo_eta: float = 2.0
    tpo_coef: float = 0.05
    tpo_coef_warmup_steps: int = 1000000
    tpo_score_floor: float = 0.1
    tpo_score_clip: float = 3.0
    tpo_ratio_clip: float = 3.0
    tpo_min_adv_abs: float = 0.05
    tpo_only_positive_adv: bool = False
...
class Agent(nn.Module):
    ...
    def get_action_candidates_and_value(self, x, num_candidates):
        # Candidate 0 is the behavior action executed in the real env. Remaining
        # candidates are old-policy alternatives used only by the TPO auxiliary.
        actor_feat, critic_feat = self._trunks(x)
        ...
        action = to_action(z_candidates[:, 0])
        value_logits = self.critic_head(critic_feat)
        if self.actor_dist == "gaussian":
        ...
        return action, z_candidates[:, 0], z_candidates, candidate_log_probs, entropy, value_logits

    def get_candidate_log_probs_and_value(self, x, z_candidates):
        # Re-score a stored candidate set under the current policy.
        actor_feat, critic_feat = self._trunks(x)
...
if __name__ == "__main__":
    ...
    assert args.tpo_group_size >= 2, "TPO auxiliary needs at least two candidates"
    assert args.tpo_eta > 0.0, "tpo_eta must be positive"
    assert args.tpo_coef >= 0.0, "tpo_coef must be non-negative"
    assert args.tpo_coef_warmup_steps >= 0, "tpo_coef_warmup_steps must be non-negative"
    ...
    assert args.tpo_ratio_clip >= 1.0, "tpo_ratio_clip must be at least one"
    ...
    tpo_candidate_zs = torch.zeros(
        (args.num_steps, args.num_envs, args.tpo_group_size) + envs.single_action_space.shape
    ).to(device)
    tpo_candidate_logprobs = torch.zeros((args.num_steps, args.num_envs, args.tpo_group_size)).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, z_candidates, candidate_logprobs, ent, value_logits = agent.get_action_candidates_and_value(
            ...
            actions[step] = action
            latent_zs[step] = z
            tpo_candidate_zs[step] = z_candidates
            tpo_candidate_logprobs[step] = candidate_logprobs
            logprobs[step] = candidate_logprobs[:, 0]
        ...
        b_tpo_candidate_zs = tpo_candidate_zs.reshape(
            (-1, args.tpo_group_size) + envs.single_action_space.shape
        )
        b_tpo_candidate_logprobs = tpo_candidate_logprobs.reshape(-1, args.tpo_group_size)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                new_candidate_logprobs, entropy, value_logits = agent.get_candidate_log_probs_and_value(
                    b_obs[mb_inds], b_tpo_candidate_zs[mb_inds]
                )
                ...
                with torch.no_grad():
                    old_candidate_logprobs = b_tpo_candidate_logprobs[mb_inds]
                    old_delta = (old_candidate_logprobs - old_candidate_logprobs[:, 0:1]).clamp(
                        -log(args.tpo_ratio_clip), log(args.tpo_ratio_clip)
                    )
                    proxy_ratio = old_delta.exp()
                    tpo_scores_raw = mb_raw_adv.detach().unsqueeze(1) * proxy_ratio
                    tpo_scores_raw[:, 0] = mb_raw_adv.detach()
                    tpo_score_clip_frac = (
                        (tpo_scores_raw.abs() > args.tpo_score_clip).float().mean().item()
                    )
                    tpo_scores = tpo_scores_raw.clamp(-args.tpo_score_clip, args.tpo_score_clip)
                    tpo_context_mask = (mb_raw_adv.detach().abs() >= args.tpo_min_adv_abs).to(torch.float32)
                    if args.tpo_only_positive_adv:
                        tpo_context_mask = tpo_context_mask * (mb_raw_adv.detach() > 0).to(torch.float32)
                ...
                if tpo_context_mask.any():
                    tpo_loss = (per_context_tpo_loss * tpo_context_mask).sum() / tpo_context_mask.sum()
                else:
                    tpo_loss = per_context_tpo_loss.mean() * 0.0
                if args.tpo_coef_warmup_steps > 0:
                    tpo_coef_eff = args.tpo_coef * min(global_step / args.tpo_coef_warmup_steps, 1.0)
                else:
                    tpo_coef_eff = args.tpo_coef
                actor_loss = pg_loss + tpo_coef_eff * tpo_loss
                with torch.no_grad():
                    tpo_losses.append(tpo_loss.item())
                    tpo_coefs.append(tpo_coef_eff)
                    ...
                    tpo_context_fracs.append(tpo_context_mask.mean().item())
        ...
        writer.add_scalar("losses/ppo_policy_loss", pg_loss.item(), global_step)
        writer.add_scalar("losses/tpo_loss", np.mean(tpo_losses), global_step)
        writer.add_scalar("losses/tpo_coef", np.mean(tpo_coefs), global_step)
        ...
        writer.add_scalar("debug/tpo_context_frac", np.mean(tpo_context_fracs), global_step)
        writer.add_scalar("debug/rollout_adv_mean_abs", b_policy_adv.abs().mean().item(), global_step)
