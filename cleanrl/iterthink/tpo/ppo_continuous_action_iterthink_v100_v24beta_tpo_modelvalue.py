# PPO + IterThink v100 (v24 Beta + full TPO with model-value credit). From v99.
#
# Pure TPO failed when candidate scores came from immediate reward, short branch
# probes, or executed-only action-advantage heads. v100 keeps the actor objective
# as full TPO, but scores candidates through learned one-step reward and next-value
# predictors:
#     score(s,a) = r_hat(s,a) + gamma * vnext_hat(s,a) - V_old(s)
# Candidate 0 is anchored to the realized one-step TD error. The model is trained
# on executed transitions to old normalized reward and old V(s_next), so the value
# critic supplies long-horizon credit without reconstructing full observations.
#
# Hypothesis: dense MuJoCo needs TPO's target to be built from action-value credit,
# and a supervised one-step model can generalize local counterfactual actions
# better than an executed-only advantage regressor.
#
# --- inherited v24 notes ---
...
def tpo_cross_entropy_loss(new_log_scores, old_log_scores, scores, eta, score_floor=0.0):
    ...
    return -(q * log_p).sum(dim=-1).mean(), q
...
class Args:
    ...
    # v100: pure TPO actor objective; one-step model + value critic supplies candidate credit.
    tpo_group_size: int = 16
    tpo_eta: float = 2.0
    tpo_score_floor: float = 0.05
    tpo_score_clip: float = 2.5
    tpo_min_score_std: float = 0.03
    tpo_anchor_candidate0: bool = True
    model_hidden: int = 128
    model_reward_coef: float = 1.0
    model_vnext_coef: float = 1.0
    model_huber_delta: float = 2.0
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        mh = args.model_hidden
        self.model_body1 = nn.Sequential(
            layer_init(nn.Linear(obs_dim + act_dim, mh)),
            ReLUSquared(),
            layer_init(nn.Linear(mh, mh)),
            ReLUSquared(),
        )
        self.model_reward_head1 = layer_init(nn.Linear(mh, 1), std=0.01)
        self.model_vnext_head1 = layer_init(nn.Linear(mh, 1), std=0.01)
        self.model_body2 = nn.Sequential(
            layer_init(nn.Linear(obs_dim + act_dim, mh)),
            ReLUSquared(),
            layer_init(nn.Linear(mh, mh)),
            ReLUSquared(),
        )
        self.model_reward_head2 = layer_init(nn.Linear(mh, 1), std=0.01)
        self.model_vnext_head2 = layer_init(nn.Linear(mh, 1), std=0.01)
    ...
    def predict_reward_vnext(self, x, actions):
        if actions.dim() == 2:
            model_in = torch.cat([x, actions], dim=-1)
            feat1 = self.model_body1(model_in)
            feat2 = self.model_body2(model_in)
            return (
                self.model_reward_head1(feat1).squeeze(-1),
                self.model_vnext_head1(feat1).squeeze(-1),
                self.model_reward_head2(feat2).squeeze(-1),
                self.model_vnext_head2(feat2).squeeze(-1),
            )
        num_candidates = actions.shape[1]
        obs = x.unsqueeze(1).expand(-1, num_candidates, -1)
    ...
    def get_action_candidates_and_value(self, x, num_candidates):
        # Candidate 0 is the behavior action executed in the env. Candidates are
        # sampled from the rollout policy and replayed during TPO updates.
        actor_feat, critic_feat = self._trunks(x)
        ...
        if num_candidates > 1:
            z_candidates = z_candidates.clone()
            z_candidates[:, 1] = dist.mean[:, 1].detach()
        if self.actor_dist == "beta":
    ...
    def get_candidate_log_probs_and_actions(self, x, z_candidates):
        # Re-score root-action candidates and return env-space actions for model scoring.
        actor_feat, critic_feat = self._trunks(x)
        ...
        if self.actor_dist == "gaussian":
            ...
            entropy = dist.entropy()[:, 0].sum(1)
        return candidate_log_probs, candidate_actions, entropy, value_logits
    ...
    def critic_parameters(self):
        ...
        return (
            ...
            + list(self.model_body1.parameters())
            + list(self.model_reward_head1.parameters())
            + list(self.model_vnext_head1.parameters())
            + list(self.model_body2.parameters())
            + list(self.model_reward_head2.parameters())
            + list(self.model_vnext_head2.parameters())
        )
...
if __name__ == "__main__":
    ...
    assert args.tpo_score_floor >= 0.0, "tpo_score_floor must be non-negative"
    assert args.tpo_score_clip > 0.0, "tpo_score_clip must be positive"
    assert args.tpo_min_score_std >= 0.0, "tpo_min_score_std must be non-negative"
    assert args.model_reward_coef >= 0.0, "model_reward_coef must be non-negative"
    assert args.model_vnext_coef >= 0.0, "model_vnext_coef must be non-negative"
    assert args.model_huber_delta > 0.0, "model_huber_delta must be positive"
    ...
    candidate_zs = torch.zeros(
        (args.num_steps, args.num_envs, args.tpo_group_size) + envs.single_action_space.shape
    ).to(device)
    candidate_logprobs = torch.zeros((args.num_steps, args.num_envs, args.tpo_group_size)).to(device)
    logprobs = torch.zeros((args.num_steps, args.num_envs)).to(device)
    rewards = torch.zeros((args.num_steps, args.num_envs)).to(device)
    dones = torch.zeros((args.num_steps, args.num_envs)).to(device)
    td_errors = torch.zeros((args.num_steps, args.num_envs)).to(device)
    next_value_targets = torch.zeros((args.num_steps, args.num_envs)).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, _, z_cands, _, cand_logprobs, ent, value_logits = agent.get_action_candidates_and_value(
                    next_obs, args.tpo_group_size
                )
                p = torch.softmax(value_logits, dim=-1)
                value_probs[step] = p
                values[step] = (p * support).sum(dim=-1)
            candidate_zs[step] = z_cands
            candidate_logprobs[step] = cand_logprobs
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.num_steps)):
                ...
                next_value_targets[t] = nextvalues.reshape(-1) * nextnonterminal
        ...
        b_candidate_zs = candidate_zs.reshape(
            (-1, args.tpo_group_size) + envs.single_action_space.shape
        )
        b_candidate_logprobs = candidate_logprobs.reshape(-1, args.tpo_group_size)
        b_advantages = policy_adv.reshape(-1)            # policy GAE (soft when auto_alpha; else raw)
        b_td_errors = td_errors.reshape(-1)
        b_next_value_targets = next_value_targets.reshape(-1)
        ...
        b_tpo_scores = torch.zeros((args.batch_size, args.tpo_group_size), device=device)
        score_chunk = args.minibatch_size
        with torch.no_grad():
            for start in range(0, args.batch_size, score_chunk):
                end = start + score_chunk
                obs_chunk = b_obs[start:end]
                _, candidate_actions, _, _ = agent.get_candidate_log_probs_and_actions(
                    obs_chunk, b_candidate_zs[start:end]
                )
                r1, vn1, r2, vn2 = agent.predict_reward_vnext(obs_chunk, candidate_actions)
                score1 = r1 + args.gamma * vn1 - b_values[start:end].unsqueeze(1)
                score2 = r2 + args.gamma * vn2 - b_values[start:end].unsqueeze(1)
                scores = torch.minimum(score1, score2)
                if args.tpo_anchor_candidate0:
                    scores[:, 0] = b_td_errors[start:end]
                scores = scores.clamp(-args.tpo_score_clip, args.tpo_score_clip)
                low_std = scores.std(dim=-1, unbiased=False) < args.tpo_min_score_std
                if low_std.any():
                    scores[low_std] = 0.0
                b_tpo_scores[start:end] = scores
        ...
        tpo_q_maxes = []
        tpo_score_stds = []
        tpo_score_clip_fracs = []
        tpo_skipped_low_std_fracs = []
        candidate0_ranks = []
        model_reward_losses = []
        model_vnext_losses = []
        model_score_td_corrs = []
        model_reward_corrs = []
        model_vnext_corrs = []
        model_twin_disagreements = []
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                new_candidate_logprobs, candidate_actions, entropy, value_logits = (
                    agent.get_candidate_log_probs_and_actions(b_obs[mb_inds], b_candidate_zs[mb_inds])
                )
                newlogprob = new_candidate_logprobs[:, 0]
                ...
                with torch.no_grad():
                    ...
                    old_group_logp = F.log_softmax(b_candidate_logprobs[mb_inds], dim=-1)
                    new_group_logp = F.log_softmax(new_candidate_logprobs, dim=-1)
                    group_kl = (old_group_logp.exp() * (old_group_logp - new_group_logp)).sum(dim=-1).mean()
                    clipfracs += [((ratio - 1.0).abs() > args.clip_coef).float().mean().item()]
                    group_kls.append(group_kl.item())

                with torch.no_grad():
                    tpo_scores = b_tpo_scores[mb_inds]
                    tpo_score_clip_frac = (tpo_scores.abs() >= args.tpo_score_clip).float().mean().item()
                    score_std = tpo_scores.std(dim=-1, unbiased=False)
                    low_std = score_std < args.tpo_min_score_std
                    candidate0_ranks.append(
                        (tpo_scores <= tpo_scores[:, 0:1]).float().mean(dim=-1).mean().item()
                    )
                policy_loss, tpo_q = tpo_cross_entropy_loss(
                    new_candidate_logprobs,
                    b_candidate_logprobs[mb_inds],
                    tpo_scores,
                    args.tpo_eta,
                ...
                r1_exec, vn1_exec, r2_exec, vn2_exec = agent.predict_reward_vnext(
                    b_obs[mb_inds], candidate_actions[:, 0]
                )
                reward_target = rewards.reshape(-1)[mb_inds].detach()
                vnext_target = b_next_value_targets[mb_inds].detach()
                model_reward_loss = 0.5 * (
                    F.smooth_l1_loss(r1_exec, reward_target, beta=args.model_huber_delta)
                    + F.smooth_l1_loss(r2_exec, reward_target, beta=args.model_huber_delta)
                )
                model_vnext_loss = 0.5 * (
                    F.smooth_l1_loss(vn1_exec, vnext_target, beta=args.model_huber_delta)
                    + F.smooth_l1_loss(vn2_exec, vnext_target, beta=args.model_huber_delta)
                )
                critic_loss = (
                    args.vf_coef * v_loss
                    + args.model_reward_coef * model_reward_loss
                    + args.model_vnext_coef * model_vnext_loss
                )
                ...
                with torch.no_grad():
                    ...
                    tpo_score_stds.append(score_std.mean().item())
                    tpo_score_clip_fracs.append(tpo_score_clip_frac)
                    tpo_skipped_low_std_fracs.append(low_std.float().mean().item())
                    model_reward_losses.append(model_reward_loss.item())
                    model_vnext_losses.append(model_vnext_loss.item())
                    score0 = tpo_scores[:, 0]
                    score0z = (score0 - score0.mean()) / (score0.std() + 1e-8)
                    tdz = (b_td_errors[mb_inds] - b_td_errors[mb_inds].mean()) / (b_td_errors[mb_inds].std() + 1e-8)
                    model_score_td_corrs.append((score0z * tdz).mean().item())
                    rpred = 0.5 * (r1_exec + r2_exec)
                    vpred = 0.5 * (vn1_exec + vn2_exec)
                    rpredz = (rpred - rpred.mean()) / (rpred.std() + 1e-8)
                    rtargetz = (reward_target - reward_target.mean()) / (reward_target.std() + 1e-8)
                    model_reward_corrs.append((rpredz * rtargetz).mean().item())
                    vpredz = (vpred - vpred.mean()) / (vpred.std() + 1e-8)
                    vtargetz = (vnext_target - vnext_target.mean()) / (vnext_target.std() + 1e-8)
                    model_vnext_corrs.append((vpredz * vtargetz).mean().item())
                    score1_exec = r1_exec + args.gamma * vn1_exec - b_values[mb_inds]
                    score2_exec = r2_exec + args.gamma * vn2_exec - b_values[mb_inds]
                    model_twin_disagreements.append((score1_exec - score2_exec).abs().mean().item())
                ...
                if args.separate_grad_clip:
                    ...
                    loss = policy_loss - ent_coef_eff * entropy_loss + critic_loss
                    optimizer.zero_grad()
                    loss.backward()
        ...
        writer.add_scalar("losses/tpo_actor_loss", policy_loss.item(), global_step)
        writer.add_scalar("losses/model_reward_loss", np.mean(model_reward_losses), global_step)
        writer.add_scalar("losses/model_vnext_loss", np.mean(model_vnext_losses), global_step)
        ...
        writer.add_scalar("debug/tpo_candidate_group_kl", np.mean(group_kls), global_step)
        ...
        writer.add_scalar("debug/tpo_score_std", np.mean(tpo_score_stds), global_step)
        writer.add_scalar("debug/tpo_score_clip_frac", np.mean(tpo_score_clip_fracs), global_step)
        writer.add_scalar("debug/tpo_skipped_low_std_frac", np.mean(tpo_skipped_low_std_fracs), global_step)
        writer.add_scalar("debug/model_score_td_corr", np.mean(model_score_td_corrs), global_step)
        writer.add_scalar("debug/model_reward_corr", np.mean(model_reward_corrs), global_step)
        writer.add_scalar("debug/model_vnext_corr", np.mean(model_vnext_corrs), global_step)
        writer.add_scalar("debug/model_twin_disagreement", np.mean(model_twin_disagreements), global_step)
        writer.add_scalar("debug/tpo_candidate0_rank", np.mean(candidate0_ranks), global_step)
