# PPO + IterThink v99 (v24 Beta + full TPO with action-Q credit). From v24.
#
# Pure TPO failed when candidate scores came from immediate reward, short branch
# probes, or an unconstrained scorer. v99 keeps the actor objective as full TPO,
# but gives TPO a real counterfactual credit model: a separate action-conditioned
# Q critic trained on executed (s,a) samples to the scalar lambda-return. Candidate
# scores are A_Q(s,a_i)=Q(s,a_i)-V(s), with candidate 0 anchored to the realized
# lambda-return advantage. The value critic still trains as v24; the Q critic
# exists only to assign credit across the sampled candidate set.
#
# Hypothesis: dense MuJoCo needs TPO's target to be built from action-value credit,
# not from reward-only bandit scores and not from PPO blending.
#
# --- inherited v24 notes ---
...
class Args:
    ...
    # v99: pure TPO actor objective; twin action-advantage critics supply candidate credit.
    tpo_group_size: int = 16
    tpo_eta: float = 2.0
    tpo_score_floor: float = 0.05
    tpo_score_clip: float = 3.0
    tpo_min_score_std: float = 0.05
    tpo_anchor_candidate0: bool = True
    adv_q_coef: float = 0.5
    adv_q_hidden: int = 128
    adv_q_huber_delta: float = 5.0
    adv_zero_mean_coef: float = 0.01
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        act_dim = int(np.prod(envs.single_action_space.shape))
        qh = args.adv_q_hidden
        self.adv_head1 = nn.Sequential(
            layer_init(nn.Linear(H + act_dim, qh)),
            ReLUSquared(),
            layer_init(nn.Linear(qh, qh)),
            ReLUSquared(),
            layer_init(nn.Linear(qh, 1), std=0.01),
        )
        self.adv_head2 = nn.Sequential(
            layer_init(nn.Linear(H + act_dim, qh)),
            ReLUSquared(),
            layer_init(nn.Linear(qh, qh)),
            ReLUSquared(),
    ...
    def _adv_from_feat(self, critic_feat, actions):
        if actions.dim() == 2:
            q_in = torch.cat([critic_feat, actions], dim=-1)
            return self.adv_head1(q_in).squeeze(-1), self.adv_head2(q_in).squeeze(-1)
        num_candidates = actions.shape[1]
        feat = critic_feat.unsqueeze(1).expand(-1, num_candidates, -1)
        q_in = torch.cat([feat, actions], dim=-1)
        return self.adv_head1(q_in).squeeze(-1), self.adv_head2(q_in).squeeze(-1)

    def get_action_advantages(self, x, actions):
        _, critic_feat = self._trunks(x)
        return self._adv_from_feat(critic_feat, actions)
    ...
    def get_candidate_log_probs_value_advantages(self, x, z_candidates):
        # Re-score root-action candidates and estimate their action advantages.
        actor_feat, critic_feat = self._trunks(x)
        ...
        candidate_log_probs = (dist.log_prob(z_candidates) - log_det_fn(z_candidates)).sum(-1)
        candidate_actions = to_action(z_candidates)
        adv1, adv2 = self._adv_from_feat(critic_feat, candidate_actions)
        value_logits = self.critic_head(critic_feat)
        if self.actor_dist == "gaussian":
            dist_1, _, log_det_1 = self._actor_dist(actor_feat)
            zr = dist_1.rsample()
            ...
            entropy = dist.entropy()[:, 0].sum(1)
        return candidate_log_probs, candidate_actions, adv1, adv2, entropy, value_logits
    ...
    def critic_parameters(self):
        ...
        return (
            ...
            + list(self.adv_head1.parameters())
            + list(self.adv_head2.parameters())
        )
...
if __name__ == "__main__":
    ...
    assert args.adv_q_coef >= 0.0, "adv_q_coef must be non-negative"
    assert args.adv_zero_mean_coef >= 0.0, "adv_zero_mean_coef must be non-negative"
    assert args.adv_q_huber_delta > 0.0, "adv_q_huber_delta must be positive"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        candidate0_ranks = []
        adv_q_losses = []
        adv_zero_mean_losses = []
        adv_head_corrs = []
        adv_twin_disagreements = []
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                new_candidate_logprobs, _, adv1, adv2, entropy, value_logits = (
                    agent.get_candidate_log_probs_value_advantages(b_obs[mb_inds], b_candidate_zs[mb_inds])
                )
                ...
                adv_target = (b_returns[mb_inds] - b_values[mb_inds]).detach()
                with torch.no_grad():
                    tpo_scores = torch.minimum(adv1, adv2).detach()
                    if args.tpo_anchor_candidate0:
                        tpo_scores[:, 0] = adv_target
                    tpo_score_clip_frac = (tpo_scores.abs() > args.tpo_score_clip).float().mean().item()
                    tpo_scores = tpo_scores.clamp(-args.tpo_score_clip, args.tpo_score_clip)
                    score_std = tpo_scores.std(dim=-1, unbiased=False)
                    low_std = score_std < args.tpo_min_score_std
                    if low_std.any():
                        tpo_scores[low_std] = 0.0
                ...
                adv_q_loss = 0.5 * (
                    F.smooth_l1_loss(adv1[:, 0], adv_target, beta=args.adv_q_huber_delta)
                    + F.smooth_l1_loss(adv2[:, 0], adv_target, beta=args.adv_q_huber_delta)
                )
                adv_zero_mean_loss = 0.5 * (adv1.mean(dim=1).square().mean() + adv2.mean(dim=1).square().mean())
                critic_loss = (
                    args.vf_coef * v_loss
                    + args.adv_q_coef * adv_q_loss
                    + args.adv_zero_mean_coef * adv_zero_mean_loss
                )
                ...
                with torch.no_grad():
                    ...
                    adv_q_losses.append(adv_q_loss.item())
                    adv_zero_mean_losses.append(adv_zero_mean_loss.item())
                    a0 = 0.5 * (adv1[:, 0] + adv2[:, 0])
                    a0z = (a0 - a0.mean()) / (a0.std() + 1e-8)
                    gtz = (adv_target - adv_target.mean()) / (adv_target.std() + 1e-8)
                    adv_head_corrs.append((a0z * gtz).mean().item())
                    adv_twin_disagreements.append((adv1 - adv2).abs().mean().item())
        ...
        writer.add_scalar("losses/advantage_q_loss", np.mean(adv_q_losses), global_step)
        writer.add_scalar("losses/advantage_zero_mean_loss", np.mean(adv_zero_mean_losses), global_step)
        ...
        writer.add_scalar("debug/adv_head_executed_corr_with_gae", np.mean(adv_head_corrs), global_step)
        writer.add_scalar("debug/adv_head_twin_disagreement", np.mean(adv_twin_disagreements), global_step)
