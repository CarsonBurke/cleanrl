# PPO + IterThink v9 (distributional λ-return critic). Branched from v7_hlgauss.
#
# v7 made the critic CATEGORICAL but its target is still a SCALAR λ-return G_t
# smeared by a FIXED Gaussian — i.e. label-smoothed regression of the mean. The
# spread is a hyperparameter, not information. v9 makes the TARGET itself a real
# distribution: the return distribution Z(s), via a distributional Bellman
# backup that bootstraps the critic's OWN predicted distribution at s'.
#
# Hypothesis ("A Distributional Perspective on RL", Bellemare et al. 2017):
# learning the full return distribution is a richer, better-conditioned training
# signal that shapes representations and improves control EVEN WHEN we still act
# on the mean. Analogy: regressing pixels with MSE collapses to a blurry mean;
# modeling the distribution preserves structure. Our high-capacity trunk
# (densenet + 16-expert MoE) is exactly where a richer auxiliary target should
# help most. (It is also the prerequisite for the spread to be *meaningful* —
# v8's precision weighting failed because v7's spread was fake label-smoothing.)
#
# What changes vs v7 — ONLY the critic target:
#   - We keep the SCALAR GAE advantage (V(s)=E[Z|s] is the correct baseline; the
#     distributional payoff is representational, not a different point estimate).
#   - The critic CE target is the distributional λ-return G^λ, a distribution
#     built by a backward recursion that mirrors GAE's λ exactly:
#         G^λ_t  =_D  r_t + γ·nonterm·[ (1-λ)·Z(s_{t+1}) + λ·G^λ_{t+1} ]
#     The bracket is a MIXTURE of two categoricals (weighted prob vectors); the
#     affine x↦r+γ·nonterm·x is applied to the support atoms and reprojected onto
#     the fixed support via the C51 projection (mass split between neighbouring
#     bins). Mean-preserving (modulo edge clamping), so mean(G^λ) == scalar G_t —
#     the comparison to v7 isolates exactly "distributional vs scalar target".
#   - Same support (±20, 255 bins), same horizon (λ), 4 update epochs.
#
# Ablation control: ppo_continuous_action_iterthink_v7_hlgauss (scalar target).
import os
import random
import time
...
class Args:
    ...
    # HL-Gauss / distributional critic support.
    num_bins: int = 255
    v_min: float = -20.0
    v_max: float = 20.0
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Categorical critic: logits over the value-support.
