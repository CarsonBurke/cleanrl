# PPO + IterThink v11 (distributional λ-return critic + mean-tracking warmup). From v10.
#
# v10 made the distributional λ-return target work (EV 2-5x better than the v7
# HL-Gauss scalar-target control from ~750k on; v7's EV collapses 0.89->0.10 as
# returns grow while v10 stays well-conditioned). BUT v10 LOSES on return early
# (-33% @500k) and only crosses v7 at ~4M. Two subagents independently localized
# the cause to the SAME mechanism:
#
#   Early lag (200k-1M) = categorical MASS-TRANSPORT lag. A peaked categorical
#   head moves its value MEAN by transporting probability mass across dozens of
#   near-empty bins; the cross-entropy gradient in that corridor is ~10x weaker
#   than a flat/scalar head's. During the fast-rising (non-stationary) early
#   return phase the mean chronically lags the climbing target. This barely dents
#   aggregate EV (variance-weighted, blind to a small coherent offset) but biases
#   ADVANTAGE DIRECTION -> worse policy updates (KL/clipfrac identical to v7, so
#   it is direction, not step size). Late training is stationary -> lag vanishes
#   and v10's distributional accuracy wins.
#
# v11 fix: give the value MEAN a direct point-shift gradient during the rapid-rise
# window via a HYBRID value loss that anneals scalar -> distributional:
#
#       w = min(1, global_step / value_warmup_steps)
#       v_loss = (1 - w) * 0.5*MSE(mean, return)  +  w * CE(target_dist, logits)
#
#   - At w=0 the value head is a plain scalar regressor on its mean (== the proven
#     v6 critic): the MSE gradient shifts the whole distribution's mean in one step,
#     eliminating the transport lag while the target is far and moving fast.
#   - At w=1 it is EXACTLY v10's distributional CE, recovering the late EV/return
#     win. So any late difference vs v10 is attributable purely to the warmup, and
#     the worst case cannot regress below v10.
#   - The two losses are CONSISTENT (the C51 target mean == the scalar GAE return,
#     verified in v10), so the handoff has no objective conflict, only a
#     conditioning/tracking-speed trade. warmup ends (~1M) right where v10's EV
#     advantage begins, so MSE covers the lag region and CE owns the win region.
#
# Peaked init (tau=0.5) KEPT: it prevents the v9 distributional-bootstrap blowup.
# Everything else (separate trunks, scalar GAE advantage, support, recursion) is
# v10 unchanged. Control: ppo_continuous_action_iterthink_v10_distreturn.
import os
import random
import time
...
class Args:
    ...
    # Hybrid value-loss warmup: scalar MSE-on-mean -> distributional CE over
    # the first `value_warmup_steps` env steps. ~1M covers the rapid-return-rise
    # (lag) window and hands off to CE right where v10's EV advantage begins.
    value_warmup_steps: int = 1000000
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Hybrid value-loss weight: scalar MSE-on-mean (w=0) -> distributional CE
        # (w=1) over the first `value_warmup_steps` env steps. Held fixed across
        # this iteration's epochs (the rollout is on-policy at this global_step).
        w = min(1.0, global_step / args.value_warmup_steps)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Hybrid value loss: convex blend of scalar MSE-on-mean and the
                # distributional CE target. The MSE term gives the mean a direct
                # point-shift gradient (fixing the early mass-transport lag); the
                # CE term carries the distributional signal that wins late. The
                # two are consistent (C51 target mean == scalar return).
                value_log_probs = torch.log_softmax(value_logits, dim=-1)
                v_loss_dist = -(b_target_probs[mb_inds] * value_log_probs).sum(dim=-1).mean()
                v_mean = (value_log_probs.exp() * support).sum(dim=-1)
                v_loss_scalar = 0.5 * ((v_mean - b_returns[mb_inds]) ** 2).mean()
                v_loss = (1.0 - w) * v_loss_scalar + w * v_loss_dist
        ...
        writer.add_scalar("losses/value_loss_scalar", v_loss_scalar.item(), global_step)
        writer.add_scalar("losses/value_loss_dist", v_loss_dist.item(), global_step)
        writer.add_scalar("losses/value_warmup_w", w, global_step)
