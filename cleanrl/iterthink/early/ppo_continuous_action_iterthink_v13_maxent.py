# PPO + IterThink v13 (MAXIMUM-ENTROPY soft distributional value). From v12.
#
# v12's location-scale critic (scalar value mu + auxiliary distributional residual
# shape head) reaches ~6000-6800 peak on HalfCheetah. v13 adds SAC-style MAXIMUM
# ENTROPY by making the value a SOFT value: the entropy bonus is folded into the
# reward that the distributional Bellman backs up.
#
#   soft reward:   r~_t = r_t - alpha * log pi(a_t | s_t)
#
# This single substitution makes the whole stack max-entropy. GAE on r~ yields the
# SOFT advantage, so the PPO clip objective maximizes the soft (entropy-augmented)
# return -> a maximum-entropy policy. And because v12's residual recursion runs on
# the GAE deltas, the distributional backup propagates the SOFT return distribution
# for free -- the categorical Bellman needs no change beyond the reward it sees.
# No Q(s,a) is required: on-policy max-ent works with V + the policy gradient.
#
#   - log pi already carries the tanh log-det (stable form), so it is the correct
#     entropy estimate for the squashed policy.
#   - Temperature alpha is auto-tuned SAC-style (dual gradient on a target entropy
#     H_bar = -act_dim): alpha rises when policy entropy < H_bar and falls when it
#     exceeds it. Principled, self-regulating exploration replaces a fixed bonus,
#     and may counter v12's premature value-collapse (a determinism failure mode
#     that drove its seed-variance: good seed kept entropy/EV, bad seed collapsed).
#   - No separate ent_coef bonus (entropy now lives inside the soft advantage).
#   - Reported episodic_return stays the RAW env return, so the benchmark metric is
#     unaffected by the entropy augmentation.
# Control: iterthink_v12_locscale.
import os
import random
import time
...
class Args:
    ...
    # Maximum-entropy (v13). Soft reward r~ = r - alpha*logpi; alpha auto-tuned to
    # a target policy entropy (SAC dual). target_entropy defaults to -act_dim.
    alpha: float = 0.1            # initial temperature
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # SAC temperature dual update: drive E[-logpi] (entropy) toward target_entropy.
        if args.autotune_alpha:
            alpha_loss = -(log_alpha * (b_logprobs + target_entropy).detach()).mean()
        ...
        writer.add_scalar("maxent/alpha", alpha, global_step)
