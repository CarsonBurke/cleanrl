# PPO + IterThink v132 rawsymlog-embednorm-k6 (v129 + no running obs norm).
#
# Key change from v129:
#   - Remove Gym's stateful NormalizeObservation/clip wrappers.
#   - Feed raw flattened MuJoCo observations into a fixed symlog scalar tokenizer:
#       z = symlog(raw_obs)
#       token_i = RMSNorm(sum_k f_k(z_i) * W_{i,k} + b_i)
#     where f_k = [z, |z|, z^2]. This bounds token scale without a running
#     normalizer and keeps scalar magnitude encoded in token direction.
...
#   symlog tokenizer plus embed norm may keep optimization stable while making
#   replayed prompt windows invariant to when they are sampled.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # RMSNorm bounds token scale while magnitude remains in token direction.
        ...
        self.obs_embed_norm = nn.RMSNorm(self.dim)
    ...
    def encode_summary(self, obs):
        ...
        obs_tokens = self.obs_embed_norm(obs_tokens + self.obs_feature_bias.to(obs_tokens.dtype))
