# PPO + IterThink v131 promptbank-rawobs-rms-k6 (v130 + collection-stat prompt normalization).
#
# Key change from v130:
#   - The rolling imagination prompt bank now stores flattened pre-normalization
#     observations plus the per-env NormalizeObservation RMS mean/variance from
#     the exact step that produced each normalized network input.
#   - Sampled prompt windows are reconstructed with those stored collection-time
#     stats, preserving the WM input coordinates while keeping raw physical obs.
#
# Hypothesis:
#   Raw-only prompt banking discarded the normalizer state, so old prompts were
#   affine-warped by current RMS stats. Storing raw obs together with collection
#   RMS should recover v129's exact prompt semantics without stale normalized-only
#   storage.
#
# -----------------------------------------------------------------------------
# Previous header:
# PPO + IterThink v129 promptbank-k6 (v128 + rolling raw imagination prompt bank).
#
# Key change from v128:
#   - Keep symmetric reward/critic supports, but seed imagination from a rolling
#     bank of the most recent 6 raw rollout buffers instead of only the current
#     rollout. The bank stores raw normalized obs, actions, and episode-start
#     dones, then re-encodes sampled prompt windows with the current WM.
#
# Hypothesis:
#   Current-rollout-only prompts make imagined return metrics and imagined PPO
#   updates sensitive to a single weak on-policy rollout. A recent raw prompt
#   bank gives more prompt diversity and smoother dream starts at negligible
#   memory cost while keeping dreamed actions on the current policy.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class RawObservationInfo(gym.Wrapper):
    """Expose flattened pre-normalization observations for prompt banking."""

    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        info["raw_observation"] = np.array(obs, copy=True, dtype=np.float32)
        return obs, info
...
if __name__ == "__main__":
    ...
    raw_obs = torch.zeros((args.num_steps, args.num_envs) + obs_shape).to(device)
    obs_rms_means = torch.zeros((args.num_steps, args.num_envs) + obs_shape, device=device)
    obs_rms_vars = torch.zeros((args.num_steps, args.num_envs) + obs_shape, device=device)
    ...
    def current_obs_rms_np():
        means, vars_ = [], []
        eps = None
        for env in envs.envs:
            obs_rms = env.get_wrapper_attr("obs_rms")
            means.append(np.asarray(obs_rms.mean, dtype=np.float32))
            vars_.append(np.asarray(obs_rms.var, dtype=np.float32))
            eps = float(env.get_wrapper_attr("epsilon"))
        return np.stack(means, axis=0), np.stack(vars_, axis=0), eps

    def unnormalize_obs_np(norm_obs_np):
        mean, var, eps = current_obs_rms_np()
        return norm_obs_np.astype(np.float32) * np.sqrt(var + eps) + mean

    def raw_obs_from_infos(infos, norm_obs_np):
        raw = infos.get("raw_observation") if isinstance(infos, dict) else None
        if raw is None:
            return unnormalize_obs_np(norm_obs_np)
        try:
            raw_arr = np.asarray(raw, dtype=np.float32)
            if raw_arr.shape == norm_obs_np.shape:
                return raw_arr
        except (TypeError, ValueError):
            pass
        # Gymnasium vector infos can occasionally carry object arrays when
        # individual infos are heterogeneous; fall back to per-env stacking.
        try:
            stacked = np.stack(list(raw), axis=0).astype(np.float32)
            if stacked.shape == norm_obs_np.shape:
                return stacked
        except Exception:
            pass
        return unnormalize_obs_np(norm_obs_np)

    def normalize_raw_obs_tensor(raw_tensor, obs_rms_mean, obs_rms_var):
        normed = (raw_tensor - obs_rms_mean) * torch.rsqrt(obs_rms_var + obs_norm_eps)
        return normed.clamp(-10.0, 10.0)

    next_raw_obs = torch.as_tensor(
        raw_obs_from_infos(reset_infos, next_obs),
        device=device,
        dtype=torch.float32,
    )
    obs_norm_eps = current_obs_rms_np()[2]
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            obs_rms_mean_np, obs_rms_var_np, _ = current_obs_rms_np()
            obs[step] = next_obs
            raw_obs[step] = next_raw_obs
            obs_rms_means[step] = torch.as_tensor(obs_rms_mean_np, device=device, dtype=torch.float32)
            obs_rms_vars[step] = torch.as_tensor(obs_rms_var_np, device=device, dtype=torch.float32)
            ...
            next_raw_obs_np = raw_obs_from_infos(infos, next_obs_np)
            ...
            next_raw_obs = torch.as_tensor(next_raw_obs_np, device=device, dtype=torch.float32)
        ...
        if args.imagine_prompt_bank_rollouts > 0:
            imagine_prompt_bank.append(
                {
                    "raw_obs": raw_obs.detach().clone(),
                    "obs_rms_mean": obs_rms_means.detach().clone(),
                    "obs_rms_var": obs_rms_vars.detach().clone(),
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            if imagine_prompt_bank:
                bank_raw_obs = torch.stack([entry["raw_obs"] for entry in imagine_prompt_bank], dim=0)
                bank_obs_rms_mean = torch.stack([entry["obs_rms_mean"] for entry in imagine_prompt_bank], dim=0)
                bank_obs_rms_var = torch.stack([entry["obs_rms_var"] for entry in imagine_prompt_bank], dim=0)
                ...
                bank_raw_obs = raw_obs.unsqueeze(0)
                bank_obs_rms_mean = obs_rms_means.unsqueeze(0)
                bank_obs_rms_var = obs_rms_vars.unsqueeze(0)
                bank_actions = actions.unsqueeze(0)
                bank_dones = dones.unsqueeze(0)
            bank_rollouts = bank_raw_obs.shape[0]
            ...
            for _ in range(args.imagine_batches):
                with torch.no_grad():
                    ...
                    if clean_flat.numel() > 0:
                        ...
                        win_raw_obs = bank_raw_obs[win_rollouts, win_steps, win_envs]
                        win_obs = normalize_raw_obs_tensor(
                            win_raw_obs,
                            bank_obs_rms_mean[win_rollouts, win_steps, win_envs],
                            bank_obs_rms_var[win_rollouts, win_steps, win_envs],
                        )
                        win_summ = agent.world_model.encode_summary(
                            win_obs.reshape((-1,) + obs_shape),
                        ).detach().reshape(
                        ...
                        seed_raw_obs = bank_raw_obs[rollout_idx, step_idx, env_idx]
                        seed_obs = normalize_raw_obs_tensor(
                            seed_raw_obs,
                            bank_obs_rms_mean[rollout_idx, step_idx, env_idx],
                            bank_obs_rms_var[rollout_idx, step_idx, env_idx],
                        )
