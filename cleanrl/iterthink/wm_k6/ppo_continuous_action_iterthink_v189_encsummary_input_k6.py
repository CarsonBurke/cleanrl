# PPO + IterThink v189 encsummary-input-k6 (v186 with a PRE-PREDICTOR agent input).
#
# v189 = v186 with ONE architectural change to the AGENT INPUT (reverting v182's swap in a
#   new direction): the actor/critic act on the ENCODER SUMMARY of the CURRENT state --
#   encode_summary(obs, incoming_outcome), the 19-token latent computed BEFORE the
#   autoregressive temporal predictor -- instead of the detached predicted next-summary
#   embed pred_next_proj(belief_from_history(...)).
#   IDEA. Both v179.x (raw belief) and v182/v186 (predicted embed) read the agent input
#   off the OUTPUT of the AR predictor, so the control signal inherits whatever the
#   temporal stack has (or hasn't) learned -- v184/v186 showed that a predictor
#   regression drags the POLICY down with it. The encoder summary is the WM's grounded,
#   Markov state representation: it is the JEPA prediction TARGET (what the predictor is
#   trained to reach), it is SIGReg'd to stay high-variance/anti-collapsed, and it does
#   not depend on the predictor at all. Acting on it decouples policy quality from
#   predictor quality; the predictor still trains (JEPA/MTP) and still drives dreams.
#   MECHANISM.
#     - combine_agent_input(summary) buffers the DETACHED current-state summary (no
#       pred_next_proj, no belief). agent_input_ln (per-token LayerNorm, grad-enabled
#       update forward) standardizes it for the trunks, exactly as before.
#     - agent_input_space: "token" (default) feeds the raw encoder-summary tokens --
#       PRE-SIGReg space; "emb" first pushes them through the WM's jepa_projector, i.e.
#       the exact BatchNorm'd emb space the JEPA MSE + SIGReg act on ("SIGReg'd" space).
#       Both are (19, D) per state; agent_input_dim is unchanged. Either way the input
#       is pre-autoregressive.
#     - The rollout-time belief construction is GONE: build_summary_agent_input encodes
#       the obs and buffers it directly; the per-step rollout latent/action history
#       lists, the neutral-action belief call, and the dream-loop neutral-action
#       belief_from_history calls (act + bootstrap) are all removed (the dream dynamics
#       still run imagine_step_from_history; in dreams the imagined next summary stands
#       in for the encoder summary, as it is trained to).
#     - obs_cond_mode (film/concat fusion of obs embed INTO the belief) is meaningless
#       once the input IS the obs summary; the flag and BeliefConditioner are removed.
#   GRADIENT ISOLATION unchanged: the buffered summary is stop-grad, so encoder (and
#   projector, under "emb") params never receive policy/value gradient; agent_input_ln
#   stays agent-side in both clip groups.
#   RISK. The input loses the belief's action-conditioned temporal context (context=5
#   recurrence) -- the agent becomes purely Markov in the current summary (MuJoCo states
#   are ~Markov, so this should cost little and may reduce noise). Under "emb" the
#   projector BatchNorm (track_running_stats=False) makes the input depend on rollout
#   batch composition -- "token" is the clean default.
#
# -----------------------------------------------------------------------------
# PPO + IterThink v186 doublenorm-predwarmstart-k6 (fix B for v184's regression vs nGPT).
#
...
class Args:
    ...
    lewm_projector_hidden: int = 512  # v183: le-wm projector hidden width (le-wm: 192->2048 ~10.7x; here D=64 -> 512 = 8x).
    ...
    # v189: which pre-predictor space the agent reads the current-state summary in.
    #   "token": raw encoder-summary tokens (pre-SIGReg space).
    #   "emb":   jepa_projector(summary) -- the BatchNorm'd emb space JEPA-MSE/SIGReg act on.
    agent_input_space: str = "token"
...
class JEPAProjector(nn.Module):
    """le-wm projector head (Linear -> BatchNorm1d -> GELU -> Linear).

    v183: the JEPA prediction MSE and SIGReg operate on this projector's emb -- a
    BatchNorm'd (NOT unit-sphere) representation -- so SIGReg's "push toward isotropic
    N(0,I)" target is well-posed (unit-norm tokens are stuck on the sphere and cannot be
    Gaussian). Mirrors le-wm's encoder->projector->emb path; only the JEPA OBJECTIVE
    lives in emb space, while the recurrent/belief/agent path keeps the unit-norm
    summary. track_running_stats=False: this head is only ever run in train mode (the WM
    is never .eval()'d on this path), so batch-stat normalization is identical to le-wm's
    default BN here, while avoiding running-stat buffer mutations under torch.compile.
    Input/output are flat (N, dim); the caller pools all token instances into N so the
    predicted and target summaries share one BN call (consistent normalization)."""
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # v183: le-wm projector head. The JEPA prediction MSE + SIGReg are computed on
        # this projector's BatchNorm'd emb (see JEPAProjector). Per-token (operates on
        # the D-dim token vectors); applied only in the WM supervised loss / metrics.
        self.jepa_projector = JEPAProjector(self.dim, args.lewm_projector_hidden)
    ...
    def _predictor_trunk(self, latent_history, action_history):
        ...
        # latent history (encoder targets at train time, pred_next_proj outputs at imagine
        # time -- both ~unit-variance from xavier projections) feeds straight in. The action
    ...
    def predict_mtp_from_history(self, latent_history, action_history, return_beliefs=False):
        ...
        preds = [self.pred_next_proj(features)]  # v184: raw projection (no unit-norm)
    ...
    def imagine_step_from_history(self, latent_history, action_history):
        ...
        next_summary = self.pred_next_proj(belief)                              # (B,19,D) full, raw (v184)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v189: the agent acts on the DETACHED ENCODER SUMMARY of the current state
        # (pre-predictor), in token or projector-emb space. Either space is 19 tokens
        # of D dims, so the buffer width is always agent_latent_dim.
        self.agent_input_space = args.agent_input_space
        assert self.agent_input_space in ("token", "emb")
        self.agent_input_dim = self.agent_latent_dim
        trunk_in_dim = self.agent_latent_dim
        # v182 (retained): a per-token LayerNorm standardises the buffered (stop-grad)
        # agent input at the grad-enabled update forward -- lejepa probe protocol -- so
        # the LN trains with the agent while the summary stays stop-grad w.r.t. the WM.
    ...
    def combine_agent_input(self, summary_latent):
        # v189: BUFFER representation handed to get_*_from_agent_input -- the agent acts
        # on the ENCODER SUMMARY of the current state (pre-autoregressive; no belief, no
        # pred_next_proj). Under agent_input_space=="emb" the summary is first pushed
        # through the WM's jepa_projector, i.e. the exact BatchNorm'd emb space the JEPA
        # MSE + SIGReg act on ("SIGReg'd" space); "token" reads the raw pre-SIGReg
        # summary tokens. This runs at rollout/dream buffer time; agent_input_from_latent
        # then .detach()es the result, so encoder/projector WM params never receive agent
        # (policy/value) gradient.
        ...
        # mirrors the v179.2 probe protocol ("LayerNorm replaces unit_norm").
        if self.agent_input_space == "emb":
            wm = self.world_model
            flat = summary_latent.reshape(-1, wm.dim)
            summary_latent = wm.jepa_projector(flat).reshape(summary_latent.shape)
        return self.agent_input_from_latent(summary_latent)

    def agent_input_from_obs(self, x):
        # Standalone single-obs convenience path: no incoming transition is available,
        # so ground the outcome tokens with zeros (episode-start convention). v189: the
        # FULL 19-token summary IS the agent input -- no belief construction needed.
        ...
        return self.combine_agent_input(latent)

    def _trunks_from_agent_input(self, obs, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        # v182 (retained): LayerNorm the buffered (detached) summary tokens per token
        # (dim D) here in the grad-enabled update forward, so the LN trains with the
        # agent. The buffer width is num_latent_tokens*D, so reshape->LN->flat.
        wm = self.world_model
    ...
    def _fusion_params(self):
        # v182: agent_input_ln is a SHARED pre-trunk module (it standardises the
        # summary input feeding both trunks), so — exactly like the shared backbone —
        # it belongs in BOTH the actor and critic clip groups; otherwise the decoupled
        # dual backward would drop its value grad and apply its policy grad unclipped.
        return list(self.agent_input_ln.parameters())
...
if __name__ == "__main__":
    ...
    def wm_supervised_loss_forward(
        ...
        # v183: JEPA loss + SIGReg in le-wm projector (BatchNorm'd) emb space. Project the
        # encoder summaries AND the predicted next-token THROUGH ONE BatchNorm call (concat
        # along the pooled token axis) so pred_emb and target_emb share batch stats and are
        # consistently normalized. The recurrent/belief/agent path keeps the unit-norm
        # tokens; only this objective moves to emb space. le-wm uses MSE (replaces v179's
        # token-space cosine as the training loss); cosine is kept only as a diagnostic.
        num_tok = agent.world_model.num_latent_tokens
        tok_dim = agent.world_model.dim
        n_seq = encoded_sequence.reshape(-1, tok_dim)   # (mb*(H+1)*S, D)
        n_pred = one_step_pred.reshape(-1, tok_dim)     # (mb*H*S, D)
        emb_all = agent.world_model.jepa_projector(torch.cat([n_seq, n_pred], dim=0))
        emb_seq_flat, pred_emb_flat = emb_all.split([n_seq.shape[0], n_pred.shape[0]], dim=0)
        emb_sequence = emb_seq_flat.reshape(mb_size, horizon + 1, num_tok, tok_dim)
        pred_emb = pred_emb_flat.reshape(mb_size, horizon, num_tok, tok_dim)
        target_emb = emb_sequence[:, 1:]
        one_step_latent_mse = F.mse_loss(pred_emb, target_emb, reduction="none").mean(dim=(-1, -2))
        one_step_latent_loss = one_step_latent_mse
        ...
        # v183: SIGReg over the FULL summary in le-wm projector emb space. emb_sequence is
        # the BatchNorm'd projection of [initial, targets] (from the shared projector call
        # above), so SIGReg drives the SAME emb the MSE target lives in toward isotropic
        # N(0,I). No sqrt(D) rescale -- that was a hack to lift unit-norm tokens to ~unit
        # per-dim variance; emb is shaped to N(0,I) by SIGReg itself.
        sigreg_latents = emb_sequence
    ...
    def wm_supervised_metrics_forward(
        ...
        one_step_pred = pred_mtp[:, :, 0, :]
        # v183: mirror supervised_loss -- JEPA MSE + SIGReg in le-wm projector emb space via
        # one shared BatchNorm projection of [encoder summaries, predicted next-token].
        num_tok = agent.world_model.num_latent_tokens
        tok_dim = agent.world_model.dim
        n_seq = encoded_sequence.reshape(-1, tok_dim)
        n_pred = one_step_pred.reshape(-1, tok_dim)
        emb_all = agent.world_model.jepa_projector(torch.cat([n_seq, n_pred], dim=0))
        emb_seq_flat, pred_emb_flat = emb_all.split([n_seq.shape[0], n_pred.shape[0]], dim=0)
        emb_sequence = emb_seq_flat.reshape(mb_size, horizon + 1, num_tok, tok_dim)
        pred_emb = pred_emb_flat.reshape(mb_size, horizon, num_tok, tok_dim)
        ...
        # v183: SIGReg over the FULL summary in le-wm projector emb space (mirror).
        sigreg_latents = emb_sequence
        wm_sigreg_loss = masked_token_sigreg_static(sigreg_latents, sigreg_weight, sigreg_A_obs, sigreg_A_out)
        wm_discount_bce = discount_bce_sum
        wm_discount_bce_excess = discount_bce_sum - discount_entropy_sum
    ...
    # v189: no rollout latent/action history -- the agent input is the current-state
    # summary, so nothing recurrent is carried between rollout steps.
    imagine_prompt_bank = []
    ...
    def build_summary_agent_input(obs_tensor, incoming_outcome):
        # v189: the agent input is the (detached) encoder summary of the CURRENT state.
        # No belief construction at rollout time -- the AR predictor runs only in the
        # WM loss (teacher-forced) and in imagination (dynamics).
        current_latent = agent.world_model.encode_summary(obs_tensor, incoming_outcome)
        if agent.detach_world_model_from_agent:
            current_latent = current_latent.detach()
        return agent.combine_agent_input(current_latent)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                agent_input = build_summary_agent_input(next_obs, incoming)
            ...
            with torch.no_grad():
                ...
                next_transition_agent_inputs[step] = build_summary_agent_input(
            ...
            for _ in range(args.imagine_batches):
                with torch.no_grad():
                    ...
                    for t in range(H_im):
                        ...
                        # v189: the actor acts on the current (dreamed) summary alone --
                        # no neutral-action belief pass. In dreams the imagined next
                        # summary stands in for the encoder summary (its JEPA target).
                        agent_input = agent.combine_agent_input(cur_full)
                    ...
                    # Bootstrap value at the final imagined state s_H (its dreamed summary).
                    value_probs_H = torch.softmax(
                        agent.get_value_from_agent_input(
                            None, agent.combine_agent_input(cur_full)
                        )[:, 0],  # horizon 0 = V(s_H)
                        dim=-1,
                    )
