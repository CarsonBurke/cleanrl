# PPO + IterThink v152.2 d3linbins-k6 (v152 + Dreamer3-style linear scalar bins).
#
# Key change from v152:
#   - Replace symlog-coordinate HL-Gauss supports for critic values and WM
#     rewards with scalar-space HL-Gauss supports whose centers are linear
#     values spaced by `symexp(linspace(z_min, z_max))`. Targets are projected
#     by Gaussian CDF mass over scalar-space bin edges using the hl-gauss-pytorch
#     convention of one global sigma from mean bin width, and scalar consumption
#     is exactly E[linear_center]. This avoids the `symexp(E[z])` Jensen issue
#     while preserving wide symlog-like resolution.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class D3LinearHLGaussSupport:
    """HL-Gauss on linear scalar bins spaced uniformly in symlog coordinates."""

    def __init__(self, num_bins, z_min, z_max, sigma_ratio, device, eps=1e-10):
        self.num_bins = num_bins
        self.z_min = z_min
        self.z_max = z_max
        self.sigma_ratio = sigma_ratio
        self.eps = eps
        coords = torch.linspace(z_min, z_max, num_bins, device=device)
        self.support = symexp(coords)
        self.v_min = self.support[0].item()
        self.v_max = self.support[-1].item()
        diffs = self.support[1:] - self.support[:-1]
        left = self.support[:1] - diffs[:1] / 2.0
        mids = (self.support[:-1] + self.support[1:]) / 2.0
        right = self.support[-1:] + diffs[-1:] / 2.0
        self.edges = torch.cat([left, mids, right], dim=0)
        self.bin_width = diffs.abs().min().item()
        self.local_widths = (self.edges[1:] - self.edges[:-1]).clamp_min(eps)
        # Match hl-gauss-pytorch semantics: one scalar sigma from the mean bin
        # size, shared for all CDF edge evaluations.
        self.sigma = sigma_ratio * self.local_widths.mean()

    def _symmetric_expectation(self, probs):
        bins = self.support.to(device=probs.device, dtype=probs.dtype)
        n = bins.shape[0]
        if n % 2 == 1:
            m = (n - 1) // 2
    ...
    def probs_to_expected_scalar(self, probs):
        return self._symmetric_expectation(probs)

    def to_expected_scalar(self, logits):
        return self.probs_to_expected_scalar(torch.softmax(logits, dim=-1))

    # API-compatible aliases. Both intentionally decode as E[linear_bin].
    probs_to_scalar = probs_to_expected_scalar
    to_scalar = to_expected_scalar

    def project(self, targets):
        bins = self.support.to(device=targets.device, dtype=targets.dtype)
        edges = self.edges.to(device=targets.device, dtype=targets.dtype)
        flat_targets = targets.clamp(bins[0], bins[-1]).reshape(-1)
        sigma = self.sigma.to(device=targets.device, dtype=targets.dtype).clamp_min(self.eps)
        centered_edges = edges.unsqueeze(0) - flat_targets.unsqueeze(-1)
        cdf_evals = torch.erf(centered_edges / (sigma * np.sqrt(2)))
        probs = cdf_evals[..., 1:] - cdf_evals[..., :-1]
        z = probs.sum(dim=-1, keepdim=True)
        return (probs / z.clamp_min(self.eps)).reshape(targets.shape + (self.num_bins,))

    def cdf_position(self, probs, targets):
        bins = self.support.to(device=targets.device, dtype=targets.dtype)
        edges = self.edges.to(device=targets.device, dtype=targets.dtype)
        widths = self.local_widths.to(device=targets.device, dtype=targets.dtype)
        flat_targets_raw = targets.reshape(-1)
        flat_targets = flat_targets_raw.clamp(edges[0], edges[-1])
        flat_probs = probs.reshape(-1, self.num_bins)
        bin_idx = (torch.searchsorted(edges, flat_targets, right=True) - 1).clamp(0, self.num_bins - 1)
        cum = flat_probs.cumsum(dim=-1)
        gather_idx = (bin_idx - 1).clamp_min(0)
        gathered = cum.gather(-1, gather_idx.unsqueeze(-1)).squeeze(-1)
        below_mass = torch.where(
            bin_idx > 0,
            gathered,
            torch.zeros_like(flat_targets),
        )
        frac = ((flat_targets - edges[bin_idx]) / widths[bin_idx]).clamp(0.0, 1.0)
        interval_mass = flat_probs.gather(-1, bin_idx.unsqueeze(-1)).squeeze(-1)
        out = below_mass + frac * interval_mass
        out = torch.where(flat_targets_raw <= edges[0], torch.zeros_like(out), out)
        out = torch.where(flat_targets_raw >= edges[-1], torch.ones_like(out), out)
...
if __name__ == "__main__":
    ...
    assert args.value_symlog, "v152.2 D3 linear bins require symlog-coordinate support endpoints"
    ...
    hl_support = D3LinearHLGaussSupport(
        args.num_bins,
        args.v_min,
        args.v_max,
    ...
    support = hl_support.support                       # (num_bins,) linear scalar bins
    scalar_support = support
    ...
    reward_support = D3LinearHLGaussSupport(
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            u = hl_support.cdf_position(value_probs, returns)  # (T,B) CDF position in scalar space
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    for mtp_idx in range(mtp_len):
                        ...
                        with torch.no_grad():
                            ...
                            reward_low = reward_support.support[0].to(
                                device=target_reward_scalar.device, dtype=target_reward_scalar.dtype
                            )
                            reward_high = reward_support.support[-1].to(
                                device=target_reward_scalar.device, dtype=target_reward_scalar.dtype
                            )
                            target_edge_frac = (
                                (target_reward_scalar <= reward_low)
                                | (target_reward_scalar >= reward_high)
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            for _ in range(args.imagine_batches):
                with torch.no_grad():
                    ...
                    u_im = hl_support.cdf_position(value_probs_t, returns_t)
