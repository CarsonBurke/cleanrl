# PPO + IterThink v172 obsembedconcat-batchpercnorm-actor1-critic10-cliphi-criticsigma075-dreamer3bucket-hlgauss-wmloss1024-cudagraph-contdisc-k6.
#
# v171, but feed the actor/critic trunk BOTH the raw current-obs encoder summary
# AND the history-conditioned WM belief, concatenated, instead of the belief alone.
#   - MOTIVATION. Until now the agent input was ONLY `belief_from_history` (the
#     predictor trunk read at the current step with a neutral last action). The
#     belief is history-mixed and the predictor is trained to predict-NEXT, so it
#     can smear/drift from the precise current observation. The encoder summary
#     `encode_summary(obs)` is the clean, un-predicted readout of "where am I now."
#     Concatenating both gives the trunk a direct current-state view alongside the
#     contextual belief — Dreamer-like (deterministic h + direct latent z).
#   - SHAPES. Both components are (B, num_latent_tokens, lewm_dim) and flatten to
#     agent_latent_dim = (2+obs_dim)*lewm_dim. The concatenated agent input is thus
#     2*agent_latent_dim; the ThinkTrunk input width doubles accordingly.
#   - REAL path: build_belief_agent_input concats the full encode_summary(obs) with
#     the belief. DREAM path: at each imagined step, concat the current full latent
#     (real encode_summary at t=0, the dreamed next_summary for t>0) with the belief;
#     the s_H bootstrap mirrors this. Both detach per detach_world_model_from_agent.
#   - Flag `concat_obs_embed` (default True); set False to recover v171 (belief-only).
#
# -----------------------------------------------------------------------------
...
class Args:
    ...
    # v172: feed the trunk concat(current encode_summary, history-conditioned belief)
    # instead of the belief alone, so the agent has a direct current-state view.
    concat_obs_embed: bool = True
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v172: optionally concat the raw current-obs encoder summary with the belief,
        # doubling the trunk input width. concat_obs_embed=False recovers v171.
        self.concat_obs_embed = args.concat_obs_embed
        self.agent_input_dim = self.agent_latent_dim * (2 if self.concat_obs_embed else 1)
    ...
    def combine_agent_input(self, obs_embed_latent, belief_latent):
        # v172: trunk input = concat(current encode_summary, history belief). When
        # concat_obs_embed is off, this is the belief alone (v171 behavior). Both
        # components are flattened+detached identically via agent_input_from_latent.
        belief_in = self.agent_input_from_latent(belief_latent)
        if not self.concat_obs_embed:
            return belief_in
...
if __name__ == "__main__":
    ...
    def build_belief_agent_input(obs_tensor):
        ...
        # v172: the FULL summary is the raw current-state embed concatenated into the
        # agent input. Recurrent stream stores obs tokens only; semantic slots never fed back.
