# PPO + IterThink v136 shared-scalar-tokens-k6 (v133 + scalable shared tokenizer).
#
...
#       z_i = symlog(raw_i)
#       f_i = [z_i, |z_i|, z_i^2]
#       h_i = Linear_shared(f_i) -> per-token RMSNorm -> ReLU^2
...
#   encoding without the unscalable per-feature weights of v133 or the full dense
#   remapping of v135. Per-token hidden RMSNorm stabilizes the tokenizer without
#   globally coupling unrelated sensors.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.obs_scalar_in = xavier_linear(nn.Linear(3, self.dim))
    ...
    def encode_summary(self, obs):
        ...
        z = symlog(obs_flat)
        obs_scalar_features = torch.stack([z, z.abs(), z.square()], dim=-1)
        obs_tokens = self.obs_scalar_in(obs_scalar_features)
