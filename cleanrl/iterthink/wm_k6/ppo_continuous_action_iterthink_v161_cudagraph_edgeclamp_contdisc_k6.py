...
def compile_agent_callables(agent, args):
    ...
    # Keep eager handles for WM supervised training. That training path builds one
    # loss from several WM forwards before a single backward; CUDA graphs are only
    # safe there if the whole loss is one compiled region. The compiled methods
    # below are still used for rollout/imagination/PPO hot paths.
    ...
    # Keep this unwrapped: imagine_step_from_history calls decode_belief_outcomes
    # internally, and nested CUDA-graph compiled callables can hand graph-owned
    # outputs across graph boundaries. The decode head is small and fuses into the
    # compiled imagine_step graph through the normal Python method call.
...
class Args:
    ...
    lewm_minibatch_size: int = 512    # v161: fixed microbatch for CUDA graph capture without v160's WM OOM peak.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        wm_train_encode_summary = getattr(
            agent.world_model, "_eager_encode_summary", agent.world_model.encode_summary
        )
        wm_train_predict_mtp_from_history = getattr(
            agent.world_model,
        ...
        for _ in range(args.lewm_update_epochs):
            ...
            for start in range(0, args.batch_size, wm_minibatch_size):
                ...
                pred_mtp = wm_train_predict_mtp_from_history(teacher_history, future_actions)
                ...
                with torch.no_grad():
                    ...
                    if args.diagnostics_warm_ar:
                        ...
                        if bool(warm_valid.any()):
                            ...
                            win_summaries = wm_train_encode_summary(
