# PPO + IterThink v139 raw-nosymlog-embednorm-k6 (v137 without symlog).
#
# Key change from v137:
#   - Remove Gym's stateful NormalizeObservation/clip wrappers.
#   - Kill observation symlog. Encode each raw scalar directly with a shared
#     scalar MLP, add a learned feature slot embedding, then apply a standard
#     token embedding RMSNorm before the encoder stack.
...
#
# Hypothesis:
#   v137's raw symlog compression is hurting reward learning. A direct raw-scalar
#   tokenizer with conventional post-embedding normalization keeps magnitude
#   information available to the scalar MLP while still presenting well-scaled
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # Raw-observation tokenizer. The scalar MLP is shared across obs
        # dimensions for scalability; learned slot embeddings provide feature
        # identity. Apply a standard embedding RMSNorm after value and slot
        # embeddings are combined, not before the scalar value is represented.
    ...
    def encode_summary(self, obs):
        ...
        obs_tokens = self.obs_scalar_in(obs_flat.unsqueeze(-1))
