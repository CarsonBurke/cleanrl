...
class LeWMBackbone(nn.Module):
    ...
    def normalize_world_model_matrices(self):
        ...
        zero-action-conditioning path and explicit output priors are preserved.
        """
        output_projection_names = (
            "attn.proj.weight",
            "w2.weight",
            "semantic_target_proj.weight",
            "obs_target_proj.weight",
            "pred_next_proj.weight",
        )
        for name, param in self.named_parameters():
            if param.ndim < 2 or "adaLN" in name or "bias" in name:
                continue
            if (
                name.endswith("reward_action_proj.weight")
                or name.endswith("reward_token_unproj.weight")
                or name.endswith("continuation_token_unproj.weight")
            ):
                continue
            norm_dim = 0 if name.endswith(output_projection_names) or "pred_mtp_projs" in name else -1
