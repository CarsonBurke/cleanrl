...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.reward_logit_scale = nn.Parameter((self.dim ** -0.5) * torch.ones(self.reward_num_bins))
        self.continuation_token_unproj = xavier_linear(nn.Linear(self.dim, 1))
        self.continuation_logit_scale = nn.Parameter(torch.ones(1))
    ...
    def decode_belief_outcomes(self, belief, action=None, detach_belief=True):
        ...
        reward_feat = unit_norm(self.reward_token_norm(belief[..., self.reward_token_idx, :]))
        if action is not None:
            reward_feat = reward_feat + self.reward_action_proj(action)
        reward_feat = unit_norm(reward_feat)
        reward_scale = self.reward_logit_scale.abs().to(reward_feat.dtype) * (self.dim ** 0.5)
        reward_logits = self.reward_token_unproj(reward_feat) * reward_scale
        continuation_feat = unit_norm(
            self.continuation_token_norm(belief[..., self.continuation_token_idx, :])
        )
        continuation_logits = (
            self.continuation_token_unproj(continuation_feat).squeeze(-1)
            * self.continuation_logit_scale.abs().to(continuation_feat.dtype)
        )
...
if __name__ == "__main__":
    ...
    wm_outcome_io_params = (
        ...
        + [agent.world_model.reward_logit_scale]
        + list(agent.world_model.continuation_token_unproj.parameters())
        + [agent.world_model.continuation_logit_scale]
    )
