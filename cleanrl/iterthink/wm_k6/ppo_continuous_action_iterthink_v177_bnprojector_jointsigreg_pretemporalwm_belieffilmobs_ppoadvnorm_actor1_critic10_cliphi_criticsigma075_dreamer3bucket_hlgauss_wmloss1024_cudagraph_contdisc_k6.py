# PPO + IterThink v177 bnprojector-jointsigreg-pretemporalwm-belieffilmobs-ppoadvnorm-actor1-critic10-cliphi-criticsigma075-dreamer3bucket-hlgauss-wmloss1024-cudagraph-contdisc-k6.
#
# v176, but add le-wm-style BatchNorm PROJECTOR heads on the two embedding
# productions, replacing the single nGPT Linear projections (obs_target_proj,
# pred_next_proj) with the le-wm `module.MLP` structure
# (config/train/model/lewm.yaml:29-45; module.py ~218-235):
#     Linear(D -> H) -> BatchNorm1d(H) -> GELU -> Linear(H -> D),  H = lewm_projector_hidden.
#   - WHERE (mapping le-wm jepa.py:39/53 onto our token-factored WM):
#       * "projector"  (jepa.py:39 `emb = self.projector(pixels_emb)`): the TARGET-embedding
#         production in encode_summary. Replaces obs_target_proj on the obs tokens -- its
#         output is the SIGReg'd target AND the one-step prediction target AND the obs
#         latent the belief/agent consume.
#       * "pred_proj"  (jepa.py:53 `preds = self.pred_proj(...)`): the PREDICTED-embedding
#         production. Replaces pred_next_proj (predictor features -> predicted next obs emb,
#         compared in the WM one-step cosine/MSE loss). The MTP heads (pred_mtp_projs) stay
#         plain nGPT Linears -- le-wm has a SINGLE pred_proj and the one-step head is the
#         primary WM latent loss; keeping MTP linear is the minimal, faithful mapping.
#   - BN ON THE HIDDEN ONLY. BatchNorm normalizes the H-dim hidden, NOT the D-dim output;
#     the output is left un-BN'd so the downstream unit_norm + SIGReg still shape it,
#     exactly like le-wm (which leaves the projector output un-normalized for SIGReg). H is
#     4*lewm_dim (256) -- le-wm uses H=2048 over embed_dim of a few hundred; we keep a
#     comparable ~4x ratio at our lewm_dim=64 scale.
#   - TRAIN vs EVAL BatchNorm (the imagination/decision subtlety). The BN train/running-stat
#     choice is made EXPLICITLY per call via a `bn_training` flag, NOT module.training
#     (there is no global .train()/.eval() in this file, and the cudagraph-captured agent
#     forwards never touch these heads). Decision: bn_training=True ONLY inside the
#     WM-supervised loss forward (le-wm whitens the projector hidden over the B*T sample
#     batch, so running stats track the representation-learning distribution); EVERYWHERE
#     ELSE -- real-rollout encode_summary (build_belief_agent_input / agent_input_from_obs),
#     imagination seeding (encode_summary), and the imagined dream rollout
#     (imagine_step_from_history -> pred_proj) -- uses bn_training=False (running stats).
#     WHY eval-mode off the training path: (a) the projector output FEEDS the actor/critic
#     at decision time (encode_summary -> obs latent -> belief), so train-mode BN would make
#     env i's action depend on the other envs in the rollout batch (a cross-sample leak) and
#     could see tiny batches at deployment; running stats give a deterministic,
#     batch-composition-independent embedding. (b) The dream is a simulator generating
#     training data; per-sample deterministic dynamics are the coherent choice. The
#     predicted embeddings (pred_proj output) are NOT consumed by the actor/critic at
#     decision time -- belief_from_history reads the predictor TRUNK features, never
#     pred_next_proj -- so pred_proj's BN only matters in the WM loss (train) and the dream
#     rollout (eval). Running stats are well-populated long before imagine_warmup_steps.
#   - nGPT. The projector/pred_proj fc1/fc2 Linears are EXCLUDED from
#     normalize_world_model_matrices (guarded by "projector" in the param name): their
#     normalization discipline is BatchNorm, not the nGPT unit-sphere, and unit-normalizing
#     their rows would fight BN's learned scale. BN weight/bias are 1-D (weight skipped by
#     ndim<2, bias skipped by the "bias" guard) and running_mean/var are buffers (not in
#     named_parameters), so no BatchNorm tensor ever reaches the nGPT normalizer.
#   - Args: lewm_bn_projector=True (toggle; False recovers v176's Linear heads),
#     lewm_projector_hidden=256.
#
# -----------------------------------------------------------------------------
# v176 (inherited): standard PPO per-minibatch advantage normalization.
#
# v175, but swap the percentile return-range advantage normalizer (batchpercnorm /
...
class Args:
    ...
    # v177: le-wm-style BatchNorm projector heads on the target/predicted embedding
    # productions. lewm_bn_projector toggles them (False recovers v176's nGPT Linear
    # obs_target_proj / pred_next_proj); lewm_projector_hidden is the BN hidden width H
    # (~4*lewm_dim, mirroring le-wm's H=2048 over an embed_dim of a few hundred).
    lewm_bn_projector: bool = True
    lewm_projector_hidden: int = 256
...
class BNProjector(nn.Module):
    # v177: le-wm projector / pred_proj head (config/train/model/lewm.yaml:29-45 ->
    # module.MLP, module.py ~218-235):  Linear(D -> H) -> BatchNorm1d(H) -> GELU ->
    # Linear(H -> D). BatchNorm normalizes the HIDDEN activation only; the D-dim output is
    # left un-normalized so the downstream nGPT unit_norm + SIGReg still shape it (le-wm
    # leaves the projector output un-BN'd and lets SIGReg act on it).
    #
    # The BatchNorm running/batch-stat choice is made EXPLICITLY per call via `bn_training`,
    # NOT module.training: the WM-supervised loss passes True (le-wm whitens the projector
    # hidden over the B*T sample batch, so running stats track the representation-learning
    # distribution), while the rollout/imagination decision path passes False (running
    # stats) so each state's embedding is a deterministic, batch-composition-independent
    # function -- no cross-env leak at action time and no batch-size-1 hazard. See the
    # header for the full rationale. F.batch_norm with an explicit `training` arg keeps this
    # decoupled from module.training (no global .train()/.eval() in this file).
    ...
    def forward(self, x, bn_training):
        lead = x.shape[:-1]
        h = self.fc1(x).reshape(-1, self.fc1.out_features)
        h = F.batch_norm(
            h,
        ...
        h = F.gelu(h)
        return self.fc2(h).reshape(*lead, self.fc2.out_features)
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # v177: "projector" head on the TARGET-embedding production (le-wm jepa.py:39).
        # BN MLP when lewm_bn_projector, else the v176 nGPT Linear. Excluded from the nGPT
        # matrix normalizer via the "projector" name guard (BN supplies the normalization).
        self.bn_projector = args.lewm_bn_projector
        if self.bn_projector:
            self.obs_projector = BNProjector(self.dim, args.lewm_projector_hidden)
        else:
        ...
        # v177: "pred_proj" head on the PREDICTED-embedding production (le-wm jepa.py:53);
        # the one-step predicted obs emb compared in the WM latent cosine/MSE loss. BN MLP
        # when lewm_bn_projector, else the v176 nGPT Linear. The MTP heads stay plain nGPT
        # Linears (le-wm has a single pred_proj; the one-step head is the primary loss).
        if self.bn_projector:
            self.pred_projector = BNProjector(self.dim, args.lewm_projector_hidden)
        else:
    ...
    def normalize_world_model_matrices(self):
        ...
        for name, param in self.named_parameters():
            ...
            if name.endswith("reward_token_unproj.weight") or name.endswith("continuation_token_unproj.weight"):
                continue
            # v177: the le-wm BatchNorm projector / pred_proj fc1/fc2 weights are NOT nGPT
            # unit-sphere matrices -- their normalization discipline is BatchNorm, so
            # unit-normalizing their rows would fight BN's learned scale. (BN weight is 1-D
            # -> skipped by ndim<2; BN bias by the "bias" guard; running_mean/var are buffers,
            # not parameters, so no BatchNorm tensor reaches this loop.)
            if "projector" in name:
                continue
            norm_dim = 0 if name.endswith(output_projection_names) else -1
            param.copy_(unit_norm(param, dim=norm_dim))

    def encode_summary(self, obs, bn_training=False):
        # v177: bn_training selects BatchNorm batch-vs-running stats for the obs projector
        # head. Default False (running stats) -- correct for the rollout/imagination/decision
        # path; the WM-supervised loss passes True. No-op when lewm_bn_projector is False.
        ...
        obs_encoded = tokens[:, self.obs_token_start :]
        if self.bn_projector:
            obs_tokens = unit_norm(self.obs_projector(obs_encoded, bn_training))
        else:
            obs_tokens = unit_norm(self.obs_target_proj(obs_encoded))
    ...
    def predict_mtp_from_history(self, latent_history, action_history, return_beliefs=False, bn_training=False):
        features = self._predictor_trunk(latent_history, action_history)
        if self.bn_projector:
            preds = [unit_norm(self.pred_projector(features, bn_training))]
        else:
    ...
    def imagine_step_from_history(self, latent_history, action_history):
        ...
        # v177: pred_proj in EVAL-mode BN (running stats) -- the dream is a simulator
        # generating training data; per-sample deterministic dynamics are the coherent
        # choice (and avoid cross-dream-sample coupling). bn_training=False is also the
        # default; passed explicitly here for clarity.
        if self.bn_projector:
            next_summary = unit_norm(self.pred_projector(features, False))
        else:
...
if __name__ == "__main__":
    ...
    def wm_supervised_loss_forward(
        ...
        # v177: bn_training=True -- this is the le-wm representation-learning forward where
        # BatchNorm whitens the projector hidden over the B*T sample batch and updates the
        # running stats every other (rollout/imagination) path then reads in eval mode.
        encoded_sequence = wm_supervised_encode_summary(
            obs_sequence.reshape((-1,) + obs_anchor.shape[1:]), bn_training=True
        ).reshape(
            mb_size,
            horizon + 1,
        ...
        pred_mtp = wm_supervised_predict_mtp_from_history(teacher_history, future_actions, bn_training=True)
