# PPO + IterThink v130 promptbank-rawobs-k6 (v129 + current-stat prompt normalization).
#
# Key change from v129:
#   - The rolling imagination prompt bank now stores flattened pre-normalization
#     observations captured before NormalizeObservation/clip, then re-normalizes
#     sampled prompt windows with the current per-env observation RMS stats.
#   - This keeps prompt storage cheap and avoids stale normalized coordinates as
#     observation normalization statistics drift.
#
# Hypothesis:
#   Recent prompt diversity is useful, but banking already-normalized obs bakes
#   old normalizer coordinates into dream starts. Storing raw flattened obs and
#   re-normalizing on sample should preserve v129's diversity while making prompts
#   consistent with the current policy/WM input space.
#
# -----------------------------------------------------------------------------
# Previous header:
...
if __name__ == "__main__":
    ...
    def current_obs_rms_tensors(dtype=torch.float32):
        mean_np, var_np, eps = current_obs_rms_np()
        mean = torch.as_tensor(mean_np, device=device, dtype=dtype)
        invstd = torch.rsqrt(torch.as_tensor(var_np, device=device, dtype=dtype) + eps)
        return mean, invstd

    def normalize_raw_obs_tensor(raw_tensor, env_indices, obs_rms_mean, obs_rms_invstd):
        normed = (raw_tensor - obs_rms_mean[env_indices]) * obs_rms_invstd[env_indices]
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            obs_rms_mean, obs_rms_invstd = current_obs_rms_tensors(dtype=bank_raw_obs.dtype)
            for _ in range(args.imagine_batches):
                with torch.no_grad():
                    ...
                    if clean_flat.numel() > 0:
                        ...
                        win_obs = normalize_raw_obs_tensor(win_raw_obs, win_envs, obs_rms_mean, obs_rms_invstd)
                        ...
                        seed_obs = normalize_raw_obs_tensor(seed_raw_obs, env_idx, obs_rms_mean, obs_rms_invstd)
