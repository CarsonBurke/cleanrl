...
if __name__ == "__main__":
    ...
    support = hl_support.support                       # (num_bins,) bin centers in transformed coordinate
    support_edges = hl_support.edges                   # (num_bins+1,) bin edges in transformed coordinate
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Per-state return spread in the same transformed support coordinate
            # used by hl-gauss-pytorch's centers. Keep this auxiliary statistic
            # aligned with transform_from_probs instead of mixing in E[symexp(z)].
            sigma_center = (value_probs * support).sum(dim=-1)
            sigma = (
                value_probs * (support - sigma_center.unsqueeze(-1)) ** 2
            ...
            cdf_frac = ((returns_coord.unsqueeze(-1) - support_edges[:-1]) / bin_width).clamp(0.0, 1.0)
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            for _ in range(args.imagine_batches):
                with torch.no_grad():
                    ...
                    sigma_center_im = (value_probs_t * support).sum(dim=-1)
                    sigma_im = (
                        value_probs_t * (support - sigma_center_im.unsqueeze(-1)) ** 2
                    ...
                    cdf_frac_im = ((returns_coord_im.unsqueeze(-1) - support_edges[:-1]) / bin_width).clamp(0.0, 1.0)
