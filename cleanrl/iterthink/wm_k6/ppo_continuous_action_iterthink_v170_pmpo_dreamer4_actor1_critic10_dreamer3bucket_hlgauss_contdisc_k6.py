# PPO + IterThink v170 pmpo-dreamer4-actor1-critic10-dreamer3bucket-hlgauss-contdisc-k6.
#
# v169 + replace the PPO clipped-ratio actor objective with Dreamer4's PMPO
# (eq. 10 of arXiv:2410.04166) in BOTH the real and imagined actor updates.
#
# PMPO is NOT a ratio objective. Per sample:
#     gate   = sigmoid((-logπ · A) / τ).detach()         # "delight" gating (optional)
#     scaled = logπ · gate · |tanh(A)|                     # bounded-magnitude weighting
#     pos_loss = Σ scaled[A ≥ 0];  neg_loss = Σ scaled[A < 0]
#     pg_loss  = -α · (pos_loss - neg_loss) / N  (+ β · KL(π_old ‖ π_new))
# It pushes up logπ of positive-advantage actions and down for negative ones.
# The two PPO safety rails are replaced by PMPO's own:
#   - |tanh(A)| bounds advantage magnitude -> outlier-robust -> NO advantage
#     normalization (raw GAE advantage feeds tanh; sign-dominant by design).
#   - An analytic KL(π_old ‖ π_new) penalty (β=0.3, reverse-KL) on the action
#     distribution is the soft trust region that replaces the ratio clip. We plumb
#     the OLD action-dist params (Beta α/β or Gaussian μ/σ) through the real
#     rollout buffer and the dream-generation buffer, and compute torch
#     kl_divergence per minibatch (the base, pre-squash dist for gaussian).
# Entropy bonus 0.01 (dreamer4 default) on real + dream. Reward/critic targets
# stay raw Dreamer3-bucket/HL-Gauss; no return/reward normalization is added.
# Flag `policy_objective` ("ppo" | "pmpo", default "pmpo"); v169's `ema_adv_norm`
# now defaults off and only affects the ppo objective.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    ent_coef: float = 0.01           # dreamer4 PMPO entropy bonus (applied to real + dream actor)
    ...
    ema_adv_norm: bool = False        # v170: PMPO uses raw advantage; this only affects policy_objective="ppo"
    ema_adv_decay: float = 0.95       # EMA retention per outer iteration (lerp = 1-decay toward new return stats)
    ema_adv_quantile_lo: float = 0.05  # clamp returns to [lo, hi] quantiles before measuring spread (outlier-robust)
    ema_adv_quantile_hi: float = 0.95
    # Dreamer4 PMPO actor objective (eq. 10 of arXiv:2410.04166). "ppo" keeps the
    # clipped-ratio path; "pmpo" uses sign-split |tanh(A)|-weighted log-prob + KL.
    policy_objective: str = "pmpo"
    pmpo_pos_to_neg_weight: float = 0.5    # alpha: scales the (pos_loss - neg_loss) term
    pmpo_kl_weight: float = 0.3            # beta: weight of the KL(old||new) trust-region penalty
    pmpo_reverse_kl: bool = True           # True => KL(old || new) (dreamer4 default); else KL(new || old)
    pmpo_delight_gating: bool = True       # gate logπ by sigmoid((-logπ·A)/τ).detach()
    pmpo_delight_temperature: float = 1.0
...
class Agent(nn.Module):
    ...
    def _actor_dist(self, actor_feat):
        ...
        # Returns (dist, to_action, log_det_fn, p1, p2) where:
        ...
        #   p1, p2: the dist's raw parameters (gaussian mean/std or beta alpha/beta),
        #           exposed for PMPO's analytic KL trust region.
        if self.actor_dist == "gaussian":
            ...
            return dist, to_action, log_det_fn, mean, std
        # beta
        ...
        return dist, to_action, log_det_fn, alpha, beta
    ...
    def get_action_and_value_from_agent_input(self, obs, agent_input, z=None):
        ...
        dist, to_action, log_det_fn, dist_p1, dist_p2 = self._actor_dist(actor_feat)
        if z is None:
            z = dist.sample()
        ...
        return action, z, log_prob, entropy, value_logits, dist_p1, dist_p2

    def actor_kl_to_old(self, new_p1, new_p2, old_p1, old_p2, reverse=True):
        # Analytic KL between the current and OLD action distribution — PMPO's soft
        # trust region (replaces the PPO ratio clip). Built on the base dist
        # (pre-squash for gaussian); old params are detached buffer values. Gradient
        # always flows through the current dist. reverse=True => KL(old || new), the
        # dreamer4 default; else KL(new || old). Summed over action dims -> (B,).
        old_p1, old_p2 = old_p1.detach(), old_p2.detach()  # old policy is a fixed target
        if self.actor_dist == "gaussian":
            new_d, old_d = Normal(new_p1, new_p2), Normal(old_p1, old_p2)
        else:
            new_d, old_d = Beta(new_p1, new_p2), Beta(old_p1, old_p2)
        kl = (
            torch.distributions.kl_divergence(old_d, new_d)
            if reverse
            else torch.distributions.kl_divergence(new_d, old_d)
        )
        return kl.sum(-1)
...
if __name__ == "__main__":
    ...
    assert not (args.norm_adv and args.ema_adv_norm), \
        "use either norm_adv (CleanRL per-minibatch) or ema_adv_norm (Dreamer4 return-std), not both"
    ...
    def masked_token_sigreg_static(token_latents, token_valids, A):
        # Static-shape variant for the compiled WM supervised loss. This keeps
        # le-wm's per-step batch marginal, but replaces Python valid-count
        # branching with tensor masks so CUDA graph capture sees one stable graph.
        # token_latents: (B, H, S, D), token_valids: (B, H), A: (D, P)
        step_losses = []
        ...
        for h in range(token_latents.shape[1]):
            step_tokens = token_latents[:, h].transpose(0, 1).contiguous()  # (S, B, D)
            valid = token_valids[:, h].to(dtype=token_latents.dtype)
            valid_count = valid.sum().clamp_min(1.0)
            valid_view = valid.view(1, -1, 1, 1)
            x_t = (step_tokens @ A).unsqueeze(-1) * t
            cos_mean = (x_t.cos() * valid_view).sum(dim=1) / valid_count
            sin_mean = (x_t.sin() * valid_view).sum(dim=1) / valid_count
    ...
    # PMPO: store the OLD action-dist params per step (beta alpha/beta or gaussian mu/std)
    # so the analytic KL trust region can compare new-vs-old in the update.
    old_dist_p1 = torch.zeros((args.num_steps, args.num_envs, action_dim)).to(device)
    old_dist_p2 = torch.zeros((args.num_steps, args.num_envs, action_dim)).to(device)
    ...
    ema_returns_mean = torch.zeros((), device=device)
    ema_returns_var = torch.ones((), device=device)
    ema_returns_std = torch.ones((), device=device)
    ema_returns_initialized = False
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if not rollout_actor_active and args.actor_dist == "beta":
                    ...
                    action, z, logprob, ent, value_logits, dist_p1, dist_p2 = agent.get_action_and_value_from_agent_input(
                        next_obs,
                        agent_input,
                    )
            ...
            old_dist_p1[step] = dist_p1
            old_dist_p2[step] = dist_p2
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                _, _, boot_logprob, _, _, _, _ = agent.get_action_and_value_from_agent_input(
                    next_obses[-1],
        ...
        b_u = u.reshape(-1)
        b_old_p1 = old_dist_p1.reshape(-1, action_dim)   # PMPO old action-dist params
        b_old_p2 = old_dist_p2.reshape(-1, action_dim)
        ...
        # Dreamer4-style EMA advantage normalizer: slow EMA of the (quantile-clipped)
        # real-return spread. We later divide both real and imagined advantages by
        # ema_returns_std. Mean is tracked for parity/diagnostics; it cancels in
        # (returns - values), so only the std scales the advantage.
        if args.ema_adv_norm:
            with torch.no_grad():
                r_stats = b_returns
                lo = torch.quantile(r_stats, args.ema_adv_quantile_lo)
                hi = torch.quantile(r_stats, args.ema_adv_quantile_hi)
                r_stats = r_stats.clamp(lo, hi)
                batch_mean = r_stats.mean()
                batch_var = r_stats.var(unbiased=False)
                if ema_returns_initialized:
                    lerp = 1.0 - args.ema_adv_decay
                    ema_returns_mean.lerp_(batch_mean, lerp)
                    ema_returns_var.lerp_(batch_var, lerp)
                else:
                    ema_returns_mean.copy_(batch_mean)
                    ema_returns_var.copy_(batch_var)
                    ema_returns_initialized = True
                ema_returns_std = ema_returns_var.clamp_min(1e-5).sqrt()
        ...
        pmpo_kl_val = torch.zeros((), device=device)
        pmpo_pos_frac = torch.zeros((), device=device)
        pmpo_sat_frac = torch.zeros((), device=device)
        pmpo_gate_mean = torch.zeros((), device=device)
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if actor_active:
                    _, _, newlogprob, entropy, value_logits, new_p1, new_p2 = agent.get_action_and_value_from_agent_input(
                        b_obs[mb_inds],
                    ...
                    if args.policy_objective == "pmpo":
                        # Dreamer4 PMPO (eq. 10): raw advantage into |tanh|, sign-split
                        # log-prob, optional delight gate, + KL(old||new) trust region.
                        A = mb_raw_adv
                        if args.pmpo_delight_gating:
                            gate = ((-newlogprob * A) / args.pmpo_delight_temperature).sigmoid().detach()
                        else:
                            gate = 1.0
                        scaled = newlogprob * gate * A.tanh().abs()
                        pos = A >= 0
                        n_adv = float(A.numel())
                        pg_loss = -args.pmpo_pos_to_neg_weight * (scaled[pos].sum() - scaled[~pos].sum()) / n_adv
                        if args.pmpo_kl_weight > 0.0:
                            pmpo_kl = agent.actor_kl_to_old(
                                new_p1, new_p2, b_old_p1[mb_inds], b_old_p2[mb_inds], args.pmpo_reverse_kl
                            ).mean()
                            pg_loss = pg_loss + args.pmpo_kl_weight * pmpo_kl
                        else:
                            pmpo_kl = torch.zeros((), device=device)
                        entropy_loss = entropy.mean()
                        with torch.no_grad():
                            pmpo_kl_val = pmpo_kl
                            pmpo_pos_frac = pos.float().mean()
                            pmpo_sat_frac = (A.tanh().abs() > 0.99).float().mean()
                            pmpo_gate_mean = gate.mean() if args.pmpo_delight_gating else torch.ones((), device=device)
                    else:
                        ...
                        if args.ema_adv_norm:
                            # Dreamer4 scale-only: divide advantage by the slow EMA return std.
                            mb_advantages = mb_advantages / ema_returns_std
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            for _ in range(args.imagine_batches):
                with torch.no_grad():
                    ...
                    im_dps1, im_dps2 = [], []   # PMPO: old dream action-dist params per step
                    ...
                    for t in range(H_im):
                        ...
                        action, z, logprob, _, value_logits, dist_p1, dist_p2 = (
                            agent.get_action_and_value_from_agent_input(None, agent_input)
                        )
                        im_dps1.append(dist_p1)
                        im_dps2.append(dist_p2)
                    ...
                    f_p1 = torch.stack(im_dps1, dim=0).reshape(H_im * B_im, action_dim)  # PMPO old dream params
                    f_p2 = torch.stack(im_dps2, dim=0).reshape(H_im * B_im, action_dim)
                ...
                for im_epoch in range(args.update_epochs):
                    ...
                    if actor_active_im:
                        _, _, newlogprob, entropy, value_logits, new_p1_im, new_p2_im = agent.get_action_and_value_from_agent_input(
                            None, f_ag, f_z
                        )
                        logratio = newlogprob - f_lp
                        ratio = logratio.exp()
                        if args.policy_objective == "pmpo":
                            # PMPO on the dream: raw imagined advantage into |tanh|,
                            # sign-split log-prob, delight gate, + KL(old||new) to the
                            # dream-generation policy.
                            A_im = shape_advantage(f_adv, f_sigma, f_u, args, device)
                            if args.pmpo_delight_gating:
                                gate_im = ((-newlogprob * A_im) / args.pmpo_delight_temperature).sigmoid().detach()
                            else:
                                gate_im = 1.0
                            scaled_im = newlogprob * gate_im * A_im.tanh().abs()
                            pos_im = A_im >= 0
                            n_adv_im = float(A_im.numel())
                            pg_loss_im = -args.pmpo_pos_to_neg_weight * (scaled_im[pos_im].sum() - scaled_im[~pos_im].sum()) / n_adv_im
                            if args.pmpo_kl_weight > 0.0:
                                pg_loss_im = pg_loss_im + args.pmpo_kl_weight * agent.actor_kl_to_old(
                                    new_p1_im, new_p2_im, f_p1, f_p2, args.pmpo_reverse_kl
                                ).mean()
                            ...
                            if args.ema_adv_norm:
                                # Same EMA return std as the real path (shared raw return units).
                                shaped = shaped / ema_returns_std
                            ratio_clamped = torch.clamp(
                                ratio,
                                1.0 - args.clip_coef,
                                1.0 + clip_hi,
                    ...
                    if actor_active_im:
                        with torch.no_grad():
                            _, _, post_logprob, _, _, _, _ = agent.get_action_and_value_from_agent_input(
                                None, f_ag, f_z
                            )
        ...
        if args.ema_adv_norm:
            writer.add_scalar("losses/ema_returns_std", float(ema_returns_std), global_step)
            writer.add_scalar("losses/ema_returns_mean", float(ema_returns_mean), global_step)
        if args.policy_objective == "pmpo":
            writer.add_scalar("pmpo/kl_to_old", float(pmpo_kl_val), global_step)
            writer.add_scalar("pmpo/pos_frac", float(pmpo_pos_frac), global_step)
            writer.add_scalar("pmpo/tanh_saturation", float(pmpo_sat_frac), global_step)
            writer.add_scalar("pmpo/delight_gate_mean", float(pmpo_gate_mean), global_step)
        if wm_losses:
            writer.add_scalar("lewm/loss", float(np.mean(wm_losses)), global_step)
            writer.add_scalar("lewm/obs_latent_loss", float(np.mean(wm_latent_losses)), global_step)
            writer.add_scalar("lewm/obs_latent_cosine_loss", float(np.mean(wm_latent_losses)), global_step)
