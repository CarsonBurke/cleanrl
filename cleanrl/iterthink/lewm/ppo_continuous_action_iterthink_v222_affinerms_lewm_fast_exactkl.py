# PPO + IterThink v222: launch-amortized affine-RMS LeWM with exact KL.
#
# Measured baseline: 552 SPS on RTX 5090. The profile is dominated by 512 WM
# micros, a warm AR diagnostic on every micro, 3072 SIGReg calls, about 1.38M
# expert Linear launches, 2048 rollout/repair rows, and repeated 86,016-state KL
# sweeps after actor steps; GPU SM utilization is typically only 30--45%.
#
# Exact behavior-preserving optimizations:
#   - stacked dense soft-MoE experts (two batched GEMMs), fused real/dream policy
#     forwards, fused behavior/base actor parameters, full-population large-batch
#     exact-Beta KL, batched successor targets/repair critic/private-refresh targets,
#     temporal-state reuse, one labeled AR diagnostic sample, deferred metric
#     synchronization, fused CUDA AdamW, and optional compilation of pure tensor
#     actor/critic regions.
# Controlled throughput adaptations (explicitly not mathematical equivalences):
#   - async 128 envs x 256 steps (still 32,768 transitions), projector BN physical
#     B=512, one stochastic reference-B128/time SIGReg sample per effective B=1024
#     update, and BF16 frozen-WM inference. PPO epochs, training population,
#     exact-KL rollback protection, model width/depth, and SIGReg coefficient .09
#     are unchanged. Hypothesis: launch amortization and higher occupancy reach
#     at least 5,000 SPS on RTX 5090.
import os
import random
import time
...
class Args:
    ...
    # Controlled projector-BN population B=512; two physical batches retain the
    # established effective B=1024 and exactly 64 WM Adam steps per default rollout.
...
if __name__ == "__main__":
    ...
    writer.add_text(
        "v222/exact_optimizations",
        "stacked dense experts; fused actor/base and real/dream forwards; "
        "full-population large-batch exact KL; batched successor/repair value "
        "evaluation; temporal-state reuse; deferred metric synchronization",
    )
    writer.add_text(
        "v222/controlled_adaptations",
        f"async_envs={args.async_envs}; envs_x_steps={args.num_envs}x{args.num_steps}; "
        f"projector_bn_batch={args.lewm_minibatch_size}; stochastic_sigreg_reference="
        f"{args.lewm_sigreg_reference_size}; frozen_wm_bf16={args.frozen_wm_bf16}; "
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("v222 requires CUDA")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            # Repair already encoded this exact post-WM rollout population with
            # frozen projector BN; gather prompts instead of encoding 20,480 rows again.
            dream_summary = repaired_current_summaries[win_steps, win_envs]
