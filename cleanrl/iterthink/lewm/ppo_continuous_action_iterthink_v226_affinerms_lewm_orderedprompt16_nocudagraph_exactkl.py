# PPO + IterThink v226: ordered prompt PPO with compile-safe shape polymorphism.
#
# v225's first dream backward hit a TorchInductor CUDA-graph tree lifetime
# invariant: compiled actor/critic regions alternate inference/training and
# real-B128/dream-B80 shapes, but reduce-overhead records fixed-lifetime graphs.
# v226 keeps those regions compiled and selects max-autotune-no-cudagraphs,
# retaining optimized kernels without unsafe graph replay. The causal order is:
# rollout -> WM optimization -> repaired real PPO -> dream generation -> dream PPO.
# Both PPO phases use 16 minibatches x 4 epochs; real updates enforce rollout
# behavior/base exact-KL anchors, and dream updates additionally enforce their
# fresh post-real dream-behavior anchor.
import os
import random
import time
...
if __name__ == "__main__":
    ...
    writer.add_text(
        "v226/ordered_real_to_imagination",
        "rollout -> WM optimization -> repaired real PPO -> dream generation -> "
        "dream PPO; 16 minibatches x 4 epochs per phase; rolling current-WM cache",
    )
    writer.add_text(
        "v226/compile_contract",
        "max-autotune-no-cudagraphs preserves compiled actor/critic kernels across "
        "alternating B128/B80 inference and gradient phases without graph replay",
    )
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("v226 requires CUDA")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if clean_dream_flat.numel() == 0:
            raise RuntimeError("v226 prompt cache has no clean full-context state")
