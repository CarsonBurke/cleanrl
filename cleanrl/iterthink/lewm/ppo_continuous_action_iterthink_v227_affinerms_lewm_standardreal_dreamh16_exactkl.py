# PPO + IterThink v227: v24-style real PPO with H16 cached-prompt dreams.
#
# Real experience follows IterThink v24: 16 envs x 2,048 steps with gamma 0.99.
# After WM optimization and post-WM repair, real PPO runs 16 minibatches x
# 4 epochs over 32,768 transitions (B2,048/update). Only imagination uses a
# short horizon: 1,280 cached prompts each generate H16 current-policy
# trajectories, preserving the established 20,480 dream transitions
# (B1,280/update). Dream generation follows real optimization; dream PPO
# enforces its fresh behavior anchor plus both original real exact-KL anchors.
# Compiled B2,048/B1,280 gradient phases use max-autotune without CUDA graphs.
import os
import random
import time
...
class Args:
    ...
    imagine_enable: bool = False       # v227 uses the ordered H16 dream PPO below
...
if __name__ == "__main__":
    ...
    assert not args.imagine_enable, "v227 disables the legacy H8 imagination path"
    ...
    writer.add_text(
        "v227/v24_real_ordered_h16_dreams",
        "16x2048 v24-style real rollout -> WM -> 16x4 real PPO -> 1280 cached "
        "prompts x H16 generation -> 16x4 dream PPO",
    )
    writer.add_text(
        "v227/compile_contract",
        "max-autotune-no-cudagraphs preserves compiled B2048 real and B1280 dream "
        "gradient phases without unsafe graph replay",
    )
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("v227 requires CUDA")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if clean_dream_flat.numel() == 0:
            raise RuntimeError("v227 prompt cache has no clean full-context state")
