# PPO + IterThink v225: ordered real-to-imagination PPO with rolling prompts.
#
# Each iteration now follows the causal optimization order:
# rollout -> WM optimization -> repaired real PPO -> dream generation -> dream PPO.
# Real PPO uses 16 minibatches x 4 epochs (128 samples/update). Only after those
# updates do we generate 256 current-policy H5 dreams from the rolling raw cache.
# Dream PPO then uses 16 minibatches x 4 epochs (80 samples/update). Both phases
# are full-strength and independently normalized. Real updates retain behavior
# and base-policy exact-KL rollback; dream updates additionally retain their fresh
# dream-behavior anchor and continue enforcing both real anchors.
# Hypothesis: dreams track the improved current policy instead of a stale pre-real
# policy, yielding cleaner on-policy imagination credit.
import os
import random
import time
...
if __name__ == "__main__":
    ...
    writer.add_text(
        "v225/ordered_real_to_imagination",
        "rollout -> WM optimization -> repaired real PPO -> dream generation -> "
        "dream PPO; 16 minibatches x 4 epochs per PPO phase; rolling current-WM "
        "prompt cache; persistent 64-step exploration codes",
    )
    writer.add_text(
        "v225/trust_region_contract",
        "real PPO enforces rollout behavior and base anchors; dream PPO enforces "
        "fresh dream behavior plus both real anchors after every actor update",
    )
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("v225 requires CUDA")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if clean_dream_flat.numel() == 0:
            raise RuntimeError("v225 prompt cache has no clean full-context state")
