...
#
# Preserve v217's correction away from pre-temporal encoder embeddings, but match
# LeVJEPA's exact ordering. LeVJEPA runs block-causal transformer layers, applies
# the encoder's final norm, reads the resulting clip representation, then sends
# that representation through a disposable Linear -> BatchNorm -> GELU -> Linear
# projector; SIGReg acts on the projector output. Accordingly, v218 regularizes
# the final valid action-free state_memory row produced by _temporal_state_trunk
# (including predictor_norm), before any action-conditioned transition/policy
# query. Its observation-token frame is flattened into one clip-level embedding,
# projected to 256 dimensions, and regularized across the batch. This replaces
# v217's direct per-token SIGReg after transition_readout, which was both after a
# task head LeVJEPA does not have and blind to cross-token collapse. The projector
# remains training-only; rollout, imagination, and PPO consume unprojected state.
# LeVJEPA defaults are retained: hidden 2,048, 1,024 directions, 17 knots, and
# lambda 0.02.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    lewm_sigreg_coef: float = 0.02    # v218: LeVJEPA default for projected embeddings
    lewm_sigreg_num_proj: int = 1024 # LeVJEPA: 1,024 random directions per step
    lewm_sigreg_knots: int = 17       # LeVJEPA: trapezoidal quadrature on [0, 3]
    lewm_sigreg_min_valid: int = 32
    lewm_sigreg_projector_hidden: int = 2048
    lewm_sigreg_projector_dim: int = 256
...
class LeVJEPASIGRegProjector(nn.Module):
    """Disposable loss-space projector used by LeVJEPA before SIGReg."""

    def __init__(self, input_dim, hidden_dim, output_dim):
        super().__init__()
        self.output_dim = output_dim
        self.net = nn.Sequential(
            xavier_linear(nn.Linear(input_dim, hidden_dim)),
            nn.BatchNorm1d(hidden_dim, affine=True, track_running_stats=False),
            nn.GELU(),
            xavier_linear(nn.Linear(hidden_dim, output_dim)),
        )
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # v218: LeVJEPA applies SIGReg only after a disposable MLP projector.
        # Flattening the observation slots makes one full-state embedding, analogous
        # to its projected CLS token, so random directions also see cross-token collapse.
        self.sigreg_projector = LeVJEPASIGRegProjector(
            self.num_obs_tokens * self.dim,
            args.lewm_sigreg_projector_hidden,
            args.lewm_sigreg_projector_dim,
        )
...
if __name__ == "__main__":
    ...
    def projected_causal_state_sigreg(state_memory, token_valids):
        # LeVJEPA regularizes one projected clip representation per view after all
        # block-causal layers. Each rollout row is a causal prefix, so select each
        # sample's longest valid prefix and form one full-state embedding.
        ...
        projected = agent.world_model.sigreg_projector(final_states.flatten(start_dim=1))
        A = sigreg.sample_projection(
            agent.world_model.sigreg_projector.output_dim,
            projected.device,
            projected.dtype,
            generator=wm_torch_generator,
        )
        return sigreg(projected.unsqueeze(0), A=A)
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # v218: LeVJEPA ordering is causal backbone -> final norm ->
                    # disposable projector -> SIGReg. Regularize the longest valid
                    # action-free causal state, before transition/policy query heads.
                    wm_sigreg_loss = projected_causal_state_sigreg(
                        state_memory,
                        latent_weight,
                    )
                    wm_loss = (
