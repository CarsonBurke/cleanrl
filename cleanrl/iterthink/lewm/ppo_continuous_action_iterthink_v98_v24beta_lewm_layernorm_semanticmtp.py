# PPO + IterThink v98 (v24 Beta + LayerNorm semantic MTP). From v97.
#
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.encoder_norm = nn.LayerNorm(self.dim)
        ...
        self.outcome_norm = nn.LayerNorm(self.dim)
        ...
        self.predictor_norm = nn.LayerNorm(self.dim)
    ...
    def encode_summary_with_outcomes(self, obs, reward_probs=None, continuations=None, value_probs=None):
        batch = obs.shape[0]
    ...
    def _outcome_feature(self, summary, detach_summary=True):
        if detach_summary:
            summary = summary.detach()
        return self.outcome_norm(summary)

    def decode_outcomes(self, summary, action=None, detach_summary=True):
        tokens = self._outcome_feature(summary, detach_summary)
        reward_feat = tokens[:, self.reward_token_idx]
        continuation_feat = tokens[:, self.continuation_token_idx]
        if action is not None:
            reward_feat = reward_feat + self.reward_action_proj(action)
        reward_logits = self.reward_outcome_unproj(reward_feat)
        continuation_logits = self.continuation_outcome_unproj(continuation_feat).squeeze(-1)
        return reward_logits, continuation_logits

    def decode_value(self, summary, detach_summary=True):
        tokens = self._outcome_feature(summary, detach_summary)
        return self.value_outcome_unproj(tokens[:, self.value_token_idx])
    ...
    def imagine_step(self, summary, action):
        ...
        reward_logits, continuation_logits = self.decode_outcomes(
            next_summary, action, detach_summary=False
        )
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.wm_adapter_token_norm = nn.LayerNorm(self.world_model.dim)
    ...
    def _obs_with_wm_residual(self, obs, agent_input):
        ...
        latent = agent_input.reshape(
            agent_input.shape[0],
            self.world_model.num_latent_tokens,
            self.world_model.dim,
        )
        latent = self.wm_adapter_token_norm(latent).reshape(agent_input.shape[0], -1)
        wm_residual = self.wm_adapter_scale * self.wm_to_obs(latent)
    ...
    def actor_parameters(self):
        ...
        adapter = list(self.wm_adapter_token_norm.parameters()) + list(self.wm_to_obs.parameters())
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    with torch.no_grad():
                        ...
                        imagine_latent_mse = F.mse_loss(
                            imagine_next_summary[:, agent.world_model.obs_token_start :],
                            target_summaries[:, 0, agent.world_model.obs_token_start :],
                            reduction="none",
                        ).mean(dim=(-1, -2))
                    ...
                    target_reward_logits, target_continuation_logits = agent.world_model.decode_outcomes(
                        ...
                        future_actions.reshape(mb_size * horizon, -1),
                    ...
                    target_termination_loss = (
                        target_termination_loss * latent_weight
                    ).sum() / target_denom

                    # Predicted outcome heads train from action-conditioned MTP
                    # semantic CLS latents; labels are not recurrent inputs.
                    ...
                    pred_reward_logits_all, pred_continuation_logits_all = agent.world_model.decode_outcomes(
                        ...
                        pred_outcome_actions.reshape(mb_size * horizon * mtp_len, -1),
                        detach_summary=False,
                    )
                    ...
                    wm_pred_value_loss = torch.stack(value_losses).sum()
                    ...
                    wm_value_loss = wm_pred_value_loss + target_value_loss
