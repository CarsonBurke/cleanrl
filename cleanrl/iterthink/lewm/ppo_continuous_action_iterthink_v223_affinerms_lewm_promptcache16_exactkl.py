# PPO + IterThink v223: short-rollout rolling-prompt LeWM with exact KL.
#
# v223 uses standard-scale 128 x 16 real PPO batches while preserving v222's
# exact trust-region, model, and per-transition real:dream training ratios.
# Dreams start from a rolling raw-experience cache spanning 16 recent rollouts.
# Each prompt is re-encoded by the current frozen WM with its full context; one
# extra predecessor state reconstructs the current incoming policy forecast
# without stale cached latents. Exploration codes retain their 64-step cadence
# across iteration boundaries. Hypothesis: frequent trust-region policy refresh
# plus diverse recent prompts improves score despite higher launch overhead.
import os
import random
import time
...
if __name__ == "__main__":
    ...
    writer.add_text(
        "v223/short_rollout_prompt_cache",
        "128x16 real rollouts; rolling raw-experience prompt cache; current-WM "
        "full-context re-encoding; reconstructed incoming forecast; persistent "
        "64-step exploration codes; preserved 0.625 dream:real transition ratio",
    )
    writer.add_text(
        "v223/exact_optimizations",
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("v223 requires CUDA")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if clean_dream_flat.numel() == 0:
            raise RuntimeError("v223 prompt cache has no clean full-context state")
