# PPO + IterThink v221 LeWM embedding-space SIGReg.
#
# v219 regularized the post-causal control state, but that is not the objective
# implemented by le-wm: LeVJEPA and le-wm both apply invariance and SIGReg to the
# same full-frame online projector embeddings. v221 ports that core contract:
# one joint attached online sequence is projected by Linear-BN-GELU-Linear,
# shifted targets meet a separate prediction projector under masked global MSE,
# and shared-projection .09 SIGReg covers only valid online frames. CleanRL's
# observation-token frame is the flattened raw-affine encoder block; semantic
# outcomes use a detached private transition branch and the policy forecast reads
# detached state. Projector BN updates only during gradient-enabled teacher-forced
# WM work. Dedicated fixed-lr WM AdamW uses physical B=128/effective B=1024,
# while PPO keeps its independently annealed optimizer and exact-KL semantics.
#
# -----------------------------------------------------------------------------
...
class Args:
    ...
    # Physical SIGReg population matches le-wm; eight microbatches preserve the
    # established 1024-row effective optimizer update.
    lewm_minibatch_size: int = 128
...
class LeWMBackbone(nn.Module):
    ...
    def policy_forecast_from_history(self, latent_history, outbound_action):
        ...
        return self.policy_moment_outcome(
            state_memory[:, -1:].detach(), outbound_action[:, None].detach()
        )
    ...
    def imagine_step_from_history(self, latent_history, action_history):
        ...
        obs_features = self.obs_transition_readout(state_memory[:, -1:], last_action)
        next_obs = self._project_prediction_frames(obs_features)[:, -1]
        semantic_features = self.outcome_transition_readout(
            state_memory[:, -1:].detach(), last_action.detach()
        )
...
if __name__ == "__main__":
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("v221 requires CUDA")
    ...
    def masked_online_embedding_sigreg(online_frames, valid_mask):
        # online_frames: (B,T,D). A is sampled once and reused for every time
        # population, while each row keeps le-wm's actual valid-B extensive scale.
        ...
        time_losses = []
        for t in range(online_frames.shape[1]):
            population = online_frames[valid_mask[:, t], t]
            if population.shape[0] >= args.lewm_sigreg_min_valid:
                time_losses.append(sigreg(population.unsqueeze(0), A=A))
        if not time_losses:
            return online_frames.sum() * 0.0
        return torch.stack(time_losses).mean()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                        def accumulate_labeled_ar(
                            ...
                            for h in range(horizon):
                                ...
                                w_h = (latent_weight[:, h] * extra_weight).float()
                                ar_latent_mse = F.mse_loss(
                                    ar_next_summary[:, obs_start:].float(),
                                    target_summaries[:, h, obs_start:].float(),
                                    reduction="none",
                                ).mean(dim=(-1, -2))
                                ar_reward_scalar = reward_logits_to_scalar(
                                    ar_reward_logits.float()
                                )
                                ar_reward_error = ar_reward_scalar - future_rewards[:, h].float()
                                ar_continuation_bce = F.binary_cross_entropy_with_logits(
                                    ar_continuation_logits.float(),
                                    1.0 - future_terminations[:, h].float(),
                                    reduction="none",
                                )
                    ...
                    wm_sigreg_loss = masked_online_embedding_sigreg(
                        online_embedding_frames,
                        online_valid,
                    )
                    wm_loss = (
        ...
        def log_policy_forecast_diagnostics():
            ...
            if wm_policy_forecast_grad_norms and wm_grad_norms:
                ...
                wm_gn = float(np.mean(wm_grad_norms))
                writer.add_scalar(
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/online_embedding_sigreg", float(np.mean(wm_sigreg_losses)), global_step)
