# PPO + IterThink v72 (v24 Beta + LeWM belief -> v24 trunk). From v24.
#
...
# teacher-forced latent/action history predicts future obs+reward target
# embeddings, optimized by masked MSE against detached targets. The v24 agent
# trunk reads detached predictor-trunk belief latents, matching the latest le-wm
# control path more closely than a raw encoder-summary feed.
#
# --- inherited v24 notes ---
#
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.num_reward_tokens = 1
        self.num_latent_tokens = self.num_obs_tokens + self.num_reward_tokens
        ...
        self.reward_embed = xavier_linear(nn.Linear(1, self.dim))
        ...
        self.reward_readout = xavier_linear(nn.Linear(self.dim, 1))

    def encode_summary(self, obs, reward=None):
        batch = obs.shape[0]
        ...
        if reward is None:
            reward = obs_flat.new_zeros(batch)
        reward_token = self.reward_embed(reward.reshape(batch, 1)).unsqueeze(1)
        return torch.cat([obs_tokens, reward_token], dim=1)
...
class Agent(nn.Module):
    ...
    def agent_input_from_latent(self, latent):
        ...
        return self.agent_input_norm(latent.reshape(latent.shape[0], -1))
...
if __name__ == "__main__":
    ...
    def build_belief_agent_input(obs_tensor, done_tensor):
        ...
        done_mask = done_tensor.bool().view(-1, 1, 1)
        done_action_mask = done_tensor.bool().view(-1, 1)
        ...
        if n_past > 0 and done_mask.any():
            past_latents = [torch.where(done_mask, torch.zeros_like(latent), latent) for latent in past_latents]
            past_actions = [torch.where(done_action_mask, torch.zeros_like(action), action) for action in past_actions]
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                agent_input, current_latent = build_belief_agent_input(next_obs, next_done)
                if global_step < args.wm_warmup_steps and args.actor_dist == "beta":
        ...
        with torch.no_grad():
            next_agent_input, _ = build_belief_agent_input(next_obs, next_done)
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            horizon = min(args.lewm_dyn_horizon, args.lewm_context)
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    hist_obs = obs[safe_hist_step_inds, env_inds]
                    hist_actions = actions[safe_hist_step_inds, env_inds]
                    hist_rewards = rewards.new_zeros((mb_size, horizon))
                    hist_summaries = agent.world_model.encode_summary(
                        hist_obs.reshape((-1,) + obs_shape),
                        hist_rewards.reshape(-1),
                    ).reshape(
                        mb_size,
                        horizon,
                    ...
                    pred_mtp = agent.world_model.predict_mtp_from_history(hist_summaries, hist_actions)

                    transition_alive = (1.0 - transition_dones[safe_hist_step_inds, env_inds]) * hist_in_rollout.float()
                    alive_prefix = torch.cumprod(transition_alive, dim=1)
                    latent_losses = []
                    reward_losses = []
                    for mtp_idx in range(mtp_len):
                        ...
                        if valid_horizon <= 0:
                            continue
                        target_step_inds = hist_step_inds[:, :valid_horizon] + mtp_idx
                        target_in_rollout = target_step_inds < args.num_steps
                        safe_target_step_inds = target_step_inds.clamp(max=max_start)
                        target_env_inds = mb_env_inds[:, None].expand_as(safe_target_step_inds)
                        target_next_obs = next_obses[safe_target_step_inds, target_env_inds]
                        target_rewards = rewards[safe_target_step_inds, target_env_inds]
                        with torch.no_grad():
                            target_summaries = agent.world_model.encode_summary(
                                target_next_obs.reshape((-1,) + obs_shape),
                        ...
                        offset_alive = alive_prefix[:, mtp_idx : mtp_idx + valid_horizon]
                        offset_valid = target_in_rollout.float() * offset_alive
                        denom = offset_valid.sum().clamp_min(1.0)
                        pred = pred_mtp[:, :valid_horizon, mtp_idx]
                        latent_loss = F.mse_loss(pred, target_summaries, reduction="none").mean(dim=(-1, -2))
                        latent_losses.append((latent_loss * offset_valid).sum() / denom)
                        pred_reward = agent.world_model.reward_readout(pred[:, :, -1]).squeeze(-1)
                        reward_loss = (pred_reward - target_rewards).square()
                        reward_losses.append((reward_loss * offset_valid).sum() / denom)
                    if not latent_losses:
                        continue
                    wm_latent_loss = torch.stack(latent_losses).mean()
                    wm_reward_loss = torch.stack(reward_losses).mean()
                    wm_loss = args.lewm_loss_coef * wm_latent_loss + args.lewm_reward_loss_coef * wm_reward_loss
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/reward_mse", float(np.mean(wm_reward_losses)), global_step)
