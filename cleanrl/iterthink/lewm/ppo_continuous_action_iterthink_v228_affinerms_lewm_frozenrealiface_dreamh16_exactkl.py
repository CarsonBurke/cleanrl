# PPO + IterThink v228: frozen real PPO interface with ordered H16 dreams.
#
# v227 peaked near 800k then decayed because the WM update remapped stored real
# agent inputs before PPO: rollout-entry behavior/base KL reached 0.0309/0.0294
# against a 0.01 budget at 800k, dream actor updates fell to zero, and by 1.5M
# all actor updates were frozen. v228 keeps the required WM-before-agent order,
# but standard real PPO consumes the exact detached inputs, values, advantages,
# and targets recorded during collection. Current-WM encoding is reserved for
# cached H16 dream prompts generated after real PPO. This restores a zero-drift
# real entry anchor while retaining every real/dream post-update exact-KL check.
import os
import random
import time
...
class Args:
    ...
    num_minibatches: int = 16         # 2,048 real + 1,280 dream samples/update
    update_epochs: int = 4            # user-selected schedule for both PPO phases
    norm_adv: bool = True
    ...
    # Physical B=512 with effective B=1024 retains 64 WM Adam steps per
    # 32,768-transition v24-style rollout.
    ...
    imagine_enable: bool = False       # v228 uses the ordered H16 dream PPO below
    imagine_batches: int = 3
    imagine_batch_size: int = 4096
    imagine_horizon: int = 8
    imagine_warmup_steps: int = 100000
    joint_prompt_cache_rollouts: int = 16  # 32,768 recent steps per environment
    joint_imagine_horizon: int = 16
    joint_imagine_batch_size: int = 1280    # 1,280 x H16 = 20,480 dream transitions
...
if __name__ == "__main__":
    ...
    assert not args.imagine_enable, "v228 disables the legacy H8 imagination path"
    assert args.joint_imagine_horizon == 16 and args.joint_imagine_batch_size > 0
    ...
    writer.add_text(
        "v228/frozen_real_interface",
        "16x2048 v24-style rollout -> WM update -> PPO on frozen rollout inputs -> "
        "1280 current-WM cached prompts x H16 -> dream PPO",
    )
    writer.add_text(
        "v228/interface_contract",
        "WM optimization cannot remap stored real behavior before PPO; only dreams "
        "consume newly encoded current-WM state",
    )
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("v228 requires CUDA")
    ...
    compile_uses_cudagraphs = args.compile and args.compile_mode in {
        "reduce-overhead",
        "max-autotune",
    }
    ...
    imagine_prompt_bank = []  # unreachable legacy H8 branch; ordered H16 uses the ring above
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ====================== FROZEN ROLLOUT PPO INTERFACE ==================
        # Standard PPO optimizes the exact agent inputs, values, advantages, and
        # targets that generated the rollout. The detached WM may update first,
        # but it cannot silently remap stored real behavior before its KL anchor
        # is measured. Dreams below still encode raw cached prompts with the
        # newly updated WM.
        rollout_next_agent_inputs = next_transition_agent_inputs
        rollout_invalid_fraction = 1.0 - transition_valids.mean().item()
        ...
        def run_ppo_phase(
            *,
            ...
            phase_codes,
            phase_logprobs,
            phase_behavior_alphas,
            phase_behavior_betas,
            phase_target_probs,
            phase_target_mask,
            phase_advantages,
            phase_sigma,
            phase_u,
            phase_minibatch_size,
            actor_active,
            full_kl_fn,
            pg_losses,
            value_losses,
            minibatch_kls,
            update_alpha,
            phase_base_alphas=None,
            phase_base_betas=None,
        ):
            phase_size = phase_inputs.shape[0]
            phase_inds = np.arange(phase_size)
            updates_used = 0
            updates_rejected = 0
            last_pg_loss = torch.zeros((), device=device)
            last_v_loss = torch.zeros((), device=device)
            last_entropy_loss = torch.zeros((), device=device)
            last_old_approx_kl = torch.zeros((), device=device)
            last_approx_kl = torch.zeros((), device=device)
            last_critic_gn = torch.zeros((), device=device)

            for _ in range(args.update_epochs):
                np.random.shuffle(phase_inds)
                for start in range(0, phase_size, phase_minibatch_size):
                    mb_inds = phase_inds[start : start + phase_minibatch_size]
                    mark_cudagraph_step()
                    (
                        _,
                        _,
                        ...
                        None,
                        phase_inputs[mb_inds],
                        phase_zs[mb_inds],
                        phase_codes[mb_inds],
                        return_beta_params=True,
                    )
                    assert current_alpha is not None and current_beta is not None
                    assert (
                        current_base_alpha is not None
                        and current_base_beta is not None
                    )
                    logratio = newlogprob - phase_logprobs[mb_inds]
                    ratio = logratio.exp()

                    with torch.no_grad():
                        last_old_approx_kl = (-logratio).mean()
                        last_approx_kl = ((ratio - 1) - logratio).mean()
                        minibatch_behavior_kl = torch.distributions.kl_divergence(
                            Beta(
                                phase_behavior_alphas[mb_inds],
                                phase_behavior_betas[mb_inds],
                            ),
                            Beta(current_alpha, current_beta),
                        ).sum(dim=-1).mean()
                        minibatch_kls.append(minibatch_behavior_kl.detach())
                        minibatch_base_kl = None
                        if phase_base_alphas is not None:
                            assert phase_base_betas is not None
                            minibatch_base_kl = torch.distributions.kl_divergence(
                                Beta(
                                    phase_base_alphas[mb_inds],
                                    phase_base_betas[mb_inds],
                                ),
                                Beta(current_base_alpha, current_base_beta),
                            ).sum(dim=-1).mean()
                        if actor_active and (
                            not torch.isfinite(minibatch_behavior_kl)
                            or minibatch_behavior_kl > args.target_kl
                            or (
                                minibatch_base_kl is not None
                                and (
                                    not torch.isfinite(minibatch_base_kl)
                                    or minibatch_base_kl > args.target_kl
                                )
                            )
                        ):
                            actor_active = False
                    ...
                    pg_losses.append(last_pg_loss.detach())

                    value_log_probs = torch.log_softmax(value_logits, dim=-1)
                    value_ce = -(
                        phase_target_probs[mb_inds] * value_log_probs
                    ).sum(dim=-1)
                    value_mask = phase_target_mask[mb_inds].to(
                        dtype=value_ce.dtype
                    )
                    last_v_loss = (
                        value_ce * value_mask
                    ).sum(dim=-1).mean()
                    value_losses.append(last_v_loss.detach())
                    last_entropy_loss = entropy.mean()

                    if auto_alpha and update_alpha:
                    ...
                    attempted_actor_update = actor_active
                    actor_snapshot = (
                    ...
                    (args.vf_coef * last_v_loss).backward(
                        retain_graph=attempted_actor_update
                    )
                    audit_wm_gradient_isolation()
                    last_critic_gn = nn.utils.clip_grad_norm_(
                    ...
                    if attempted_actor_update:
                        (
                            last_pg_loss
                            - ent_coef_eff * last_entropy_loss
                        ).backward()
                        audit_wm_gradient_isolation()
                    ...
                    if attempted_actor_update:
                        ...
                        exact_beta_kls.append(post_real_kl.item())
                        base_exact_beta_kls.append(post_base_kl.item())
                        if post_dream_kl is not None:
                            joint_dream_full_kls.append(
                                post_dream_kl.item()
                            )
                        if (
                            not torch.isfinite(post_real_kl)
                            or post_real_kl > args.target_kl
                            or not torch.isfinite(post_base_kl)
                            or post_base_kl > args.target_kl
                            or (
                                post_dream_kl is not None
                                and (
                                    not torch.isfinite(post_dream_kl)
                                    or post_dream_kl > args.target_kl
                                )
                            )
                        ):
            ...
            return {
                ...
                "v_loss": last_v_loss,
                "entropy_loss": last_entropy_loss,
                "old_approx_kl": last_old_approx_kl,
                "approx_kl": last_approx_kl,
                "critic_gn": last_critic_gn,
            }

        rollout_entry_behavior_kl, rollout_entry_base_kl = (
            measure_full_real_anchor_beta_kls()
        )

        boundary_time = phase_boundary_time()
        frozen_interface_seconds = boundary_time - phase_started_at
        phase_started_at = boundary_time

        real_minibatch_kls = []

        @torch.no_grad()
        def measure_real_phase_full_kls():
            behavior_kl, base_kl = measure_full_real_anchor_beta_kls()
            return behavior_kl, base_kl, None

        real_actor_active = bool(
            torch.isfinite(rollout_entry_behavior_kl)
            and rollout_entry_behavior_kl <= args.target_kl
            and torch.isfinite(rollout_entry_base_kl)
            and rollout_entry_base_kl <= args.target_kl
        )
        real_ppo_result = run_ppo_phase(
            phase_inputs=b_agent_inputs,
            phase_zs=b_latent_zs,
            phase_codes=b_explore_codes,
            phase_logprobs=b_logprobs,
            phase_behavior_alphas=b_behavior_alphas,
            phase_behavior_betas=b_behavior_betas,
            phase_target_probs=b_target_probs,
            phase_target_mask=b_target_mask,
            phase_advantages=b_advantages,
            phase_sigma=b_sigma,
            phase_u=b_u,
            phase_minibatch_size=args.minibatch_size,
            actor_active=real_actor_active,
            full_kl_fn=measure_real_phase_full_kls,
            pg_losses=joint_real_pg_losses,
            value_losses=joint_real_value_losses,
            minibatch_kls=real_minibatch_kls,
            update_alpha=True,
            phase_base_alphas=b_base_behavior_alphas,
            phase_base_betas=b_base_behavior_betas,
        )
        actor_updates_used += real_ppo_result["updates_used"]
        actor_updates_rejected += real_ppo_result["updates_rejected"]
        real_actor_active = real_ppo_result["actor_active"]
        boundary_time = phase_boundary_time()
        real_ppo_seconds = boundary_time - phase_started_at
        phase_started_at = boundary_time

        # ======================= FROZEN-WM H16 DREAM DATA ======================
        ...
        if clean_dream_flat.numel() == 0:
            raise RuntimeError("v228 prompt cache has no clean full-context state")
        ...
        def measure_full_dream_behavior_kl():
            ...
            for dream_start in range(
                ...
                kl_sum += torch.distributions.kl_divergence(
                    Beta(
                        d_alphas[dream_start:dream_end],
                        d_betas[dream_start:dream_end],
                    ),
                    Beta(alpha, beta),
                ).sum(dim=-1).sum()
        ...
        @torch.no_grad()
        def measure_dream_phase_full_kls():
            behavior_kl, base_kl = measure_full_real_anchor_beta_kls()
            dream_kl = measure_full_dream_behavior_kl()
            return behavior_kl, base_kl, dream_kl

        dream_entry_behavior_kl = measure_full_dream_behavior_kl()
        (
            post_real_phase_behavior_kl,
            post_real_phase_base_kl,
        ) = measure_full_real_anchor_beta_kls()
        dream_actor_active = bool(
            real_ppo_result["actor_active"]
            and torch.isfinite(post_real_phase_behavior_kl)
            and post_real_phase_behavior_kl <= args.target_kl
            and torch.isfinite(post_real_phase_base_kl)
            and post_real_phase_base_kl <= args.target_kl
        ...
        dream_ppo_result = run_ppo_phase(
            phase_inputs=d_agent_inputs,
            phase_zs=d_zs,
            phase_codes=d_codes,
            phase_logprobs=d_logprobs,
            phase_behavior_alphas=d_alphas,
            phase_behavior_betas=d_betas,
            phase_target_probs=d_target_probs,
            phase_target_mask=d_target_mask,
            phase_advantages=d_advantages,
            phase_sigma=d_sigma,
            phase_u=d_u,
            phase_minibatch_size=dream_minibatch_size,
            actor_active=dream_actor_active,
            full_kl_fn=measure_dream_phase_full_kls,
            pg_losses=joint_dream_pg_losses,
            value_losses=joint_dream_value_losses,
            minibatch_kls=joint_dream_minibatch_kls,
            update_alpha=False,
        )
        actor_updates_used += dream_ppo_result["updates_used"]
        actor_updates_rejected += dream_ppo_result["updates_rejected"]
        real_actor_active = dream_ppo_result["actor_active"]

        (
            post_real_anchor_beta_kl,
            post_real_anchor_base_kl,
        ) = measure_full_real_anchor_beta_kls()
        ...
        joint_dream_full_kls.append(post_joint_dream_kl.item())

        joint_pg_loss = 0.5 * (
            real_ppo_result["pg_loss"] + dream_ppo_result["pg_loss"]
        )
        joint_v_loss = 0.5 * (
            real_ppo_result["v_loss"] + dream_ppo_result["v_loss"]
        )
        joint_entropy_loss = 0.5 * (
            real_ppo_result["entropy_loss"]
            + dream_ppo_result["entropy_loss"]
        )
        old_approx_kl = real_ppo_result["old_approx_kl"]
        approx_kl = real_ppo_result["approx_kl"]
        critic_gn = dream_ppo_result["critic_gn"]
        ...
        boundary_time = phase_boundary_time()
        dream_ppo_seconds = boundary_time - phase_started_at
        phase_started_at = boundary_time
        ...
        refresh_next_inputs = rollout_next_agent_inputs.reshape(
        ...
        writer.add_scalar(
            "rollout_interface/entry_behavior_kl",
            rollout_entry_behavior_kl.item(),
            global_step,
        )
        writer.add_scalar(
            "rollout_interface/entry_base_kl",
            rollout_entry_base_kl.item(),
            global_step,
        )
        writer.add_scalar(
            "rollout_interface/invalid_fraction",
            rollout_invalid_fraction,
            global_step,
        )
        dream_pg_abs = (
            device_metric_abs_mean(joint_dream_pg_losses)
            if joint_dream_pg_losses
            else 0.0
        )
        real_pg_abs = (
            device_metric_abs_mean(joint_real_pg_losses)
            if joint_real_pg_losses
            else 0.0
        )
        writer.add_scalar(
            "ordered_real/policy_loss",
            device_metric_mean(joint_real_pg_losses)
            if joint_real_pg_losses
            else 0.0,
            global_step,
        )
        writer.add_scalar(
            "ordered_real/value_loss",
            device_metric_mean(joint_real_value_losses)
            if joint_real_value_losses
            else 0.0,
            global_step,
        )
        writer.add_scalar(
            "ordered_real/minibatch_behavior_kl",
            device_metric_mean(real_minibatch_kls)
            if real_minibatch_kls
            else 0.0,
            global_step,
        )
        writer.add_scalar(
            "ordered_real/post_phase_behavior_kl",
            post_real_phase_behavior_kl.item(),
            global_step,
        )
        writer.add_scalar(
            "ordered_real/actor_updates_used",
            real_ppo_result["updates_used"],
            global_step,
        )
        writer.add_scalar(
            "ordered_real/actor_updates_rejected",
            real_ppo_result["updates_rejected"],
            global_step,
        )
        writer.add_scalar(
            "ordered_imagine/policy_loss",
            device_metric_mean(joint_dream_pg_losses)
            if joint_dream_pg_losses
            else 0.0,
            global_step,
        )
        writer.add_scalar(
            "ordered_imagine/value_loss",
            device_metric_mean(joint_dream_value_losses)
            if joint_dream_value_losses
            else 0.0,
            global_step,
        )
        writer.add_scalar(
            "ordered_imagine/minibatch_behavior_kl",
            device_metric_mean(joint_dream_minibatch_kls)
            if joint_dream_minibatch_kls
            else 0.0,
            global_step,
        )
        writer.add_scalar(
            "ordered_imagine/entry_behavior_kl",
            dream_entry_behavior_kl.item(),
            global_step,
        )
        writer.add_scalar(
            "ordered_imagine/full_behavior_kl",
            post_joint_dream_kl.item(),
            global_step,
        )
        writer.add_scalar(
            "ordered_imagine/policy_abs_fraction",
            dream_pg_abs / max(1e-12, dream_pg_abs + real_pg_abs),
            global_step,
        )
        writer.add_scalar(
            "ordered_imagine/actor_updates_used",
            dream_ppo_result["updates_used"],
            global_step,
        )
        writer.add_scalar(
            "ordered_imagine/actor_updates_rejected",
            dream_ppo_result["updates_rejected"],
            global_step,
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            ...
            actor_updates_used
            / float(2 * args.update_epochs * args.num_minibatches),
            global_step,
        )
        ...
        phase_seconds = {
            ...
            "gae_pre_wm": pre_wm_seconds,
            "wm": wm_seconds,
            "frozen_real_interface": frozen_interface_seconds,
            "real_ppo": real_ppo_seconds,
            "dream_generation": dream_generation_seconds,
            "dream_ppo": dream_ppo_seconds,
