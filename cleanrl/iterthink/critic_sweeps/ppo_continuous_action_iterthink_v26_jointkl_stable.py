# PPO + IterThink v26 (JOINT-KL STABLE). From v25.
#
# Resolution to v25: the fixed PMPO coefficient (0.3) plus v24's 10-epoch update was
# too loose under constant LR: sampled KL ran ~0.08-0.14 against target_kl=0.03 and
# returns lagged v24. v26 keeps the behavior-policy joint KL but tightens the optimizer:
#   * actor_kl_coef=1.0 so KL is a real restoring force at rankgauss unit scale.
#   * update_epochs=3 to reduce stale-batch over-optimization.
#   * minibatch-level target_kl stop so we stop as soon as an update crosses the leash,
#     rather than finishing the rest of an epoch after the violation is visible.
#
# --- inherited v25 notes ---
# PPO + IterThink v25 (JOINT-KL TRUST REGION). From v24.
#
# Replaces PPO's hard ratio clip with the dreamer4/PMPO-style distributional trust
# region:
#     L_actor = -E[A * pi_new(a|s) / pi_old(a|s)] + actor_kl_coef * KL(old || new)
# The KL is analytic and summed over action dimensions. For beta this is the exact
# factorized Beta joint KL using behavior concentrations stashed at rollout time;
# for gaussian it is the base-Normal KL, equivalent under the shared tanh bijection.
# Hypothesis: no LR schedule + a smooth joint-KL restoring force should constrain
# policy movement and concentration collapse more cleanly than per-sample clipping
# plus an early-stop leash.
#
# --- inherited v24 notes ---
...
class Args:
    ...
    actor_trust_region: str = "joint_kl"  # "joint_kl" (default) | "clip" (v24 PPO clip)
    actor_kl_coef: float = 1.0       # v26: stronger than dreamer4 PMPO default for no-schedule PPO
    actor_kl_reverse: bool = True    # True: KL(old || new), False: KL(new || old)
    ent_coef: float = 0.0
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, z=None, return_dist=False):
...
if __name__ == "__main__":
    ...
    assert args.actor_trust_region in ("joint_kl", "clip")
    ...
    behavior_param1 = torch.zeros_like(latent_zs)
    behavior_param2 = torch.zeros_like(latent_zs)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, value_logits, behavior_dist = agent.get_action_and_value(
                    next_obs, return_dist=True
                )
                p = torch.softmax(value_logits, dim=-1)
                value_probs[step] = p
                values[step] = (p * support).sum(dim=-1)
                if args.actor_dist == "beta":
                    behavior_param1[step] = behavior_dist.concentration1
                    behavior_param2[step] = behavior_dist.concentration0
                else:
                    behavior_param1[step] = behavior_dist.loc
                    behavior_param2[step] = behavior_dist.scale
            actions[step] = action
            latent_zs[step] = z
        ...
        b_behavior_param1 = behavior_param1.reshape((-1,) + envs.single_action_space.shape)
        b_behavior_param2 = behavior_param2.reshape((-1,) + envs.single_action_space.shape)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_logits, actor_dist = agent.get_action_and_value(
                    b_obs[mb_inds], b_latent_zs[mb_inds], return_dist=True
                )
                ...
                if args.actor_trust_region == "joint_kl":
                    # Joint-KL trust region: unclipped likelihood-ratio PG plus an
                    # analytic distribution KL to the behavior policy used for this rollout.
                    # For factorized policies, the joint KL is the per-dimension KL sum.
                    if args.actor_dist == "beta":
                        old_dist = Beta(b_behavior_param1[mb_inds], b_behavior_param2[mb_inds])
                        actor_concentrations.append(
                            (actor_dist.concentration1 + actor_dist.concentration0).mean().item()
                        )
                    else:
                        old_dist = Normal(b_behavior_param1[mb_inds], b_behavior_param2[mb_inds])
                    if args.actor_kl_reverse:
                        kl_per_dim = kl_divergence(old_dist, actor_dist)
                    else:
                        kl_per_dim = kl_divergence(actor_dist, old_dist)
                    actor_kl_penalty = kl_per_dim.sum(-1).mean()
                    pg_loss = (-mb_advantages * ratio).mean() + args.actor_kl_coef * actor_kl_penalty
                    actor_kls.append(actor_kl_penalty.item())
                else:
                    # v24 asymmetric "clip-higher": looser upper bound gives positive-
                    # advantage actions more room to grow (counters collapse).
