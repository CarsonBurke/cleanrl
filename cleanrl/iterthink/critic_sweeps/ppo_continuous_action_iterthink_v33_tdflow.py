# PPO + IterThink v33 (TD-FLOW CRITIC — AdaLN-Zero FiLM conditioning + shorter horizon). From v32.
#
# v32's TD-CFM fix DID escape the v=0 collapse and the policy learned (returns climbed), but
# value_std collapsed to ~0.09: at flow_gamma=0.99 only ~1% of CFM targets are state-conditional
# (the immediate-next-obs/direct component) and CONCATENATING the state/time conditioning modulates
# the body only weakly, so the flow fit a near-MARGINAL (state-independent) successor measure ->
# the value barely discriminates between states. v33 attacks value discrimination two ways:
# (a) AdaLN-Zero FiLM conditioning (paper §C.3.1 uses FiLM/AdaLN): the state+flow-time vector c
# now MULTIPLICATIVELY modulates (shift/scale/gate) each hidden block via zero-init AdaLN, so the
# velocity field is strongly per-state conditional yet starts as the identity / near-zero field
# (matching v32's out std=0.1 small-init); (b) flow_gamma 0.99 -> 0.97 gives ~3x more direct
# (state-conditional) grounding and a shorter ~33-step horizon. PPO/GAE gamma stays 0.99.
#
# --- inherited v32 notes ---
...
class AdaLNBlock(nn.Module):
    """A DiT-style AdaLN-Zero residual block. The conditioning vector c (state + flow-time)
    MULTIPLICATIVELY modulates a LayerNorm'd hidden state via (shift, scale, gate). The AdaLN
    Linear is zero-initialized (weight AND bias) so shift=scale=gate=0 at init -> the block is
    the identity, and the whole net starts as a near-zero velocity field (the v32 out std=0.1
    small-init philosophy). h <- h + gate * mlp(norm(h)*(1+scale) + shift)."""
    def __init__(self, hidden):
        super().__init__()
        self.norm = nn.LayerNorm(hidden, elementwise_affine=False)
        self.mlp = nn.Sequential(
            layer_init(nn.Linear(hidden, hidden)), nn.SiLU(),
            layer_init(nn.Linear(hidden, hidden)),
        )
        self.adaLN = nn.Linear(hidden, 3 * hidden)
        nn.init.zeros_(self.adaLN.weight)
        nn.init.zeros_(self.adaLN.bias)

    def forward(self, h, c):
        shift, scale, gate = self.adaLN(F.silu(c)).chunk(3, dim=-1)
        return h + gate * self.mlp(self.norm(h) * (1.0 + scale) + shift)


class FlowVelocity(nn.Module):
    """Rectified-flow velocity v(x_t, t | s) for the TD-flow successor measure over
    OBSERVATIONS, with DiT-style AdaLN-Zero (FiLM) conditioning (paper arXiv 2503.09817 §C.3.1).
    The conditioning vector c = t_mlp(sinusoidal_time_embed(t)) + cond_enc(s) MULTIPLICATIVELY
    modulates each hidden block (shift/scale/gate), so the field is strongly per-STATE
    conditional — vs v32's weak CONCATENATION that fit a near-marginal (state-independent)
    successor measure and collapsed value_std to ~0.09. All AdaLN modulations are zero-init
    (AdaLN-Zero) so every block starts as the identity and the net starts as a near-zero
    velocity field (matching v32's out std=0.1 small init). t_mlp / cond_enc / sinusoidal time
    embedding are UNCHANGED from v32."""
    def __init__(self, obs_dim, hidden, t_dim=128):
        ...
        self.blocks = nn.ModuleList([AdaLNBlock(hidden) for _ in range(2)])
        self.norm_final = nn.LayerNorm(hidden, elementwise_affine=False)
        self.final_adaLN = nn.Linear(hidden, 2 * hidden)
    ...
    def forward(self, x, t, cond_obs):
        c = self.t_mlp(sinusoidal_time_embed(t, self.t_dim)) + self.cond_enc(cond_obs)
        h = self.x_in(x)
        for block in self.blocks:
            h = block(h, c)
        shift, scale = self.final_adaLN(F.silu(c)).chunk(2, dim=-1)
        h = self.norm_final(h) * (1.0 + scale) + shift
        return self.out(h)
...
def tdflow_loss(net, target_net, obs, next_obs, mask, gamma, steps):
    """TD-CFM rectified-flow loss (arXiv 2503.09817, Eq. 6) for the discounted-future-state
    successor measure m(.|s).

    WHY NOT TD2-CFM (Eq. 8): the TD2 bootstrap matches the online field's velocity to its own
    EMA copy's velocity. v==0 satisfies that *trivially* (online == target), so at gamma=0.99
    the bootstrap term (weight gamma) pins the net at the degenerate v=0 fixed point while the
    real-data direct term (weight 1-gamma=0.01) is far too weak to escape. Online (few grad
    steps) this collapses: direct stays at the predict-zero loss forever (empirically ~34).

    TD-CFM instead regresses (single, full-weight CFM term) toward a future state X1 *sampled*
    from the Bellman mixture
        X1 ~ (1-gamma) * delta_{next_obs}  +  gamma * m_target(.|next_obs),
    i.e. with prob (1-gamma) the immediate next state S', else a full sample drawn from the
    EMA-target successor flow conditioned on S'. Same successor-measure fixed point, but the
    target is an actual nonzero state, so v=0 is NOT a solution -> no collapse. The flow is
    conditioned on the current state `obs` (its own encoder, independent of the policy trunk)."""
    B = next_obs.shape[0]
    with torch.no_grad():
        b0 = torch.randn_like(next_obs)
        ones = torch.ones(B, 1, device=next_obs.device)
        boot_sample = integrate_flow(target_net, next_obs, b0, ones, steps)  # ~ m_target(.|S')
    ...
    xt = (1.0 - t) * x0 + t * x1
    cfm = ((net(xt, t, obs) - (x1 - x0)) ** 2).sum(-1)
...
class Args:
    ...
    flow_gamma: float = 0.97          # ~3x more state-conditional ("direct") grounding + shorter ~33-step horizon (PPO/GAE gamma stays 0.99)
