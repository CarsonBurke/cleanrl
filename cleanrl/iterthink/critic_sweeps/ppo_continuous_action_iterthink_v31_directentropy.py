# PPO + IterThink v31 (DIRECT auto-entropy -- the standard mechanism). From v30.
#
# THE STANDARD AUTO-ENTROPY WE WERE MISSING. v28/v29/v30 all inject entropy via an INDIRECT
# channel: alpha*H(s') folded into a SEPARATE soft-advantage GAE, then run through rankgauss
# (rank-only) and norm_adv (unit-rescale). That laundering crushed alpha to inert
# (soft_adv_std_ratio ~ 1.00 the whole run in v28/v29). v30 rescaled it but still rides the same
# indirect path. NONE of them had the textbook mechanism: an alpha*H term added DIRECTLY to the
# actor objective, with alpha auto-tuned by the SAC dual. That is what SAC does (its actor loss
# maximizes Q + alpha*H) and what PPO does with ent_coef -- just dual-tuned instead of fixed.
#
# v31 adds exactly that:
#   - DIRECT bonus: actor loss = pg_loss - alpha.detach() * entropy.mean(). entropy carries grad
#     so the policy gets the entropy-maximizing gradient straight, NOT laundered through
#     GAE/rankgauss/norm_adv. This is the potent channel where alpha's MAGNITUDE actually bites.
#   - Soft-advantage channel OFF (direct_entropy=True => policy_adv = pure reward advantage).
#   - alpha auto-tuned per-minibatch by the SAC dual toward target_entropy (inherited v29 dual).
#   - Inherited from v30: alpha in the NORMALIZED frame (no /return_std), no real floor
#     (alpha_min ~ 1e-6 numerical only), target_entropy = -2.0 (binding exploration floor on the
#     summed squashed scale; free descent reaches ~-4, so -2 binds and the dual HOLDS H there).
#     alpha_init = 0.05 so the bonus starts small (no early peak-pinning); the dual grows it.
# Hypothesis: a direct, correctly-scaled, dual-tuned entropy bonus -- the mechanism we never
# actually had -- lets alpha genuinely shape exploration and matches/beats v29's free descent
# (which already beats the 4868 beta champion at ~6100). This is the clean test of "does a real
# auto-entropy help?", and v31-vs-v30 isolates DIRECT vs INDIRECT injection at the same target.
#
# ---- inherited v28 notes ----
