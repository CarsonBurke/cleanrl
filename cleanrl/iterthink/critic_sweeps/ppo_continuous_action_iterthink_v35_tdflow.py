# PPO + IterThink v35 (TD-FLOW CRITIC — replay + DECOUPLED flow updates). From v34.
#
# v34 (Q-style (s,a) conditioning + TD-CFM(c) coupled noise) lifted episodic return 739 ->
# 1382 at 1.22M — a real improvement — but EXPLAINED VARIANCE plateaued at +0.2 (a useful
# PPO critic needs >0.8) and value_std drifted toward marginal-ish. Validator diagnosis:
# on-policy PPO data has tiny support; even an (s,a)-conditioned flow generalizes as
# near-marginal away from the narrow visited (s,a) slice. The paper sidesteps this by
# training off-policy on ~10M wide-coverage transitions for 3-8M flow gradient steps with
# a FROZEN policy (decoupling the flow's data efficiency from the policy's).
#
# v35 ports the closest on-policy adaptation: a FIFO REPLAY BUFFER over the last 8 rollouts
# (262k tuples) and a DECOUPLED FLOW UPDATE LOOP that runs BEFORE the PPO epochs — 256 flow
# grad-steps per iter, batch 1024 sampled uniformly from replay, against a SEPARATE Adam
# optimizer (lr 3e-4) over flow + reward params only. The PPO update loop now optimizes
# ONLY the actor (single grad-clip), so the actor & critic are also fully decoupled. Flow
# net, TD-CFM(c) coupled loss, EMA target, Q-style (s,a) conditioning, value readout,
# beta/rankgauss/clip-higher policy, and every other hyperparam are BYTE-IDENTICAL to v34.
# From v34:
# v34 ports the paper-faithful Q-style flow (arXiv 2503.09817, §5 / Alg. 1): the velocity
# conditions on (s, a) instead of just s. The direct target s' is now structurally
# (s,a)-dependent (s' = env(s,a)), so the marginal-collapse symmetry is BROKEN and v=0 / a
# constant marginal flow is no longer a Bellman fixed point. The bootstrap conditions on
# (s', a'=pi(s')) with a' sampled stochastically (paper Alg. 1 line 8), and the rollout /
# GAE value reads V(s) = E_{a~pi}[Q(s,a)] (stochastic a). The CFM noise is also COUPLED
# (TD-CFM(c), paper Eq. 7 / Fig. 1): the regression x0 seeds the bootstrap integration so
# the regression target has lower transport cost.
# From v32:
# PPO + IterThink v32 (TD-FLOW CRITIC — TD-CFM target-mixture loss, fixes the v=0 collapse). From v31.
#
# v31 added common-random-numbers value + per-term diagnostics and they were DECISIVE: an
# isolation unit test proved the FlowVelocity net fits a CFM target perfectly (direct loss
# 34 -> 3 in 100 steps), yet in the real run flow_direct stayed pinned at ~34 with grad norm
# ~0.15. Root cause: the TD2-CFM bootstrap (Eq. 8) matches the online field's velocity to its
# own EMA copy's velocity, and v==0 satisfies that TRIVIALLY (online == target). At gamma=0.99
# the bootstrap term (weight gamma) pins the net at the degenerate v=0 fixed point; the
# real-data direct term (weight 1-gamma=0.01) is 100x too weak to escape, so online (few grad
# steps per state) the flow never leaves zero and the value is useless.
#
# v32 switches to TD-CFM (arXiv 2503.09817, Eq. 6): a SINGLE full-weight CFM term regressing
# toward a future state X1 SAMPLED from the Bellman mixture (1-g)*delta_{s'} + g*m_target(.|s').
# Same successor-measure fixed point, but the target is a real nonzero state, so v=0 is no
# longer a solution -> no collapse. CRN value, decoupled flow encoder, sinusoidal time MLP,
# diagnostics and the 1/(1-g) value readout are all UNCHANGED.
#
...
class FlowVelocity(nn.Module):
    """Rectified-flow velocity v(x_t, t | s, a) for the TD-flow successor measure over
    OBSERVATIONS. v34: conditioning is Q-style — cat([obs, action]) — so the direct target
    s' is structurally (s,a)-dependent and the marginal-collapse symmetry of v32 is broken
    (paper arXiv 2503.09817 §5 / Alg. 1). Proven recipe otherwise (paper + repo v120):
    SINUSOIDAL time embedding through a nonlinear MLP, a DEDICATED conditioning encoder
    (independent of the policy trunk, so the flow rides a stable representation),
    CONCATENATED (x, t, cond) features, SiLU activations and LayerNorm."""
    def __init__(self, obs_dim, hidden, act_dim, t_dim=128):
        super().__init__()
        self.t_dim = t_dim
        ...
        # Q-style conditioning: cond_in = cat([obs, action]) along the feature axis.
        self.cond_enc = nn.Sequential(
            layer_init(nn.Linear(obs_dim + act_dim, hidden)), nn.SiLU(),
            layer_init(nn.Linear(hidden, hidden)),
        )
...
def integrate_flow_sa(net, cond_obs, cond_act, x0, end_t, steps):
    ...
    per-sample end_t (B,1). Q-style (s,a)-conditioned variant of v32's integrate_flow."""
    x = x0
    dt = end_t / steps
...
class FlowReplay:
    """CUDA-resident FIFO replay buffer for the decoupled TD-flow updates. Stores raw
    on-policy transitions (obs, action, next_obs, reward, flow_mask) flattened across the
    rollout's (T, num_envs). Sampled uniformly with replacement during the decoupled
    flow-update loop so the flow gradient sees ~8x more transitions than a single rollout
    can supply (the paper's off-policy data efficiency, on-policy-adapted)."""

    def __init__(self, cap, obs_dim, act_dim, device):
        self.cap = int(cap)
        self.device = device
        self.obs = torch.zeros((self.cap, obs_dim), device=device)
        self.action = torch.zeros((self.cap, act_dim), device=device)
        self.next_obs = torch.zeros((self.cap, obs_dim), device=device)
        self.reward = torch.zeros((self.cap,), device=device)
        self.mask = torch.zeros((self.cap,), device=device)
        self.ptr = 0
        self.size = 0

    def push(self, obs, action, next_obs, reward, mask):
        n = obs.shape[0]
    ...
    def sample(self, batch_size, device):
        ...
        return (
            ...
            self.mask[idx].to(device),
        )


def tdflow_loss(net, target_net, agent, obs, action, next_obs, mask, gamma, steps):
    """TD-CFM(c) rectified-flow loss for the Q-style discounted-future-state successor
    measure m(.|s,a) (paper arXiv 2503.09817 §5 / Alg. 1 + Eq. 7 coupled-noise variant).

    v34 changes vs v32:
      - Q-style conditioning: net(xt, t, obs, action). The direct target s' is structurally
        (s,a)-dependent (s' = env(s,a)) so the flow MUST be (s,a)-conditional, breaking
        v32's V-style state-marginal collapse where v=0 (i.e. m^pi(s')) was a Bellman fp.
      - Bootstrap conditions on (s', a'=pi(s')) with a' sampled stochastically from the
        current policy (Alg. 1 line 8). a' is drawn under no_grad.
      - TD-CFM(c) coupled noise (paper Eq. 7 / Fig. 1): the regression x0 ALSO seeds the
        bootstrap integration (instead of an independent b0), giving a tighter regression
        target with lower transport cost.

    The mixture target is still
        X1 ~ (1-gamma) * delta_{next_obs}  +  gamma * m_target(.|s', a'),
    so v=0 / a constant marginal flow is not a fixed point under Q-style conditioning."""
    B = next_obs.shape[0]
    ones = torch.ones(B, 1, device=next_obs.device)
    x0 = torch.randn_like(next_obs)                     # regression noise (also seeds bootstrap: coupled)
    with torch.no_grad():
        # a' ~ pi(.|s') stochastic (paper Alg. 1 line 8); use the action only.
        next_action, _, _, _ = agent.get_action_and_value(next_obs)
        boot_sample = integrate_flow_sa(target_net, next_obs, next_action, x0, ones, steps)  # ~ m_target(.|s',a')
        use_boot = (torch.rand(B, 1, device=next_obs.device) < gamma).float()
        x1 = use_boot * boot_sample + (1.0 - use_boot) * next_obs  # mixture target
    t = torch.rand(B, 1, device=next_obs.device)
    xt = (1.0 - t) * x0 + t * x1
    cfm = ((net(xt, t, obs, action) - (x1 - x0)) ** 2).sum(-1)
    denom = mask.sum().clamp_min(1.0)
    # diagnostics: split the CFM loss by which mixture component supplied the target
    ub = use_boot.squeeze(-1)
    direct_d = (cfm * (1.0 - ub) * mask).sum() / ((1.0 - ub) * mask).sum().clamp_min(1.0)
    boot_d = (cfm * ub * mask).sum() / (ub * mask).sum().clamp_min(1.0)
    return (cfm * mask).sum() / denom, direct_d.detach(), boot_d.detach()
...
class Args:
    ...
    # v35: decoupled-flow knobs. FIFO replay over the last 8 rollouts (262144 = 8*16*2048)
    # plus a separate Adam optimizer over flow+reward params lets the flow take many more
    # gradient steps per env step than PPO would otherwise allow. Defaults match the
    # paper's "many flow steps per policy step" recipe, adapted on-policy.
    replay_capacity: int = 262144      # FIFO cap = 8 rollouts (8 * 16 envs * 2048 steps)
    flow_warmup_size: int = 32768      # wait until >=1 full rollout (16*2048) is buffered
    flow_extra_steps: int = 256        # decoupled flow grad-steps per training iteration
    flow_extra_batch: int = 1024       # batch size for replay-sampled flow updates
    flow_learning_rate: float = 3e-4   # Adam lr for the separate flow+reward optimizer
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v34 TD-FLOW CRITIC (Q-style). Value = 1/(1-g) * E_{a~pi}[ E_{s'~m(.|s,a)}[ r_hat(s') ] ],
        # no categorical head. Reward model stays s-only (action-conditioning r_hat is a secondary
        # improvement per the validator; the dominant collapse fix is the (s,a)-conditional flow).
        self.flow_gamma = args.flow_gamma
        self.obs_dim = obs_dim
        self.act_dim = act_dim
        self.flow_net = FlowVelocity(obs_dim, args.flow_hidden, act_dim, t_dim=args.flow_t_dim)
    ...
    def flow_value(self, x, a, steps):
        """Q(s,a) via the successor-measure flow with COMMON RANDOM NUMBERS: every (s,a) is
        integrated from the SAME fixed noise basis `self.value_noise` (refreshed once per
        iteration), so Q is a deterministic function of (s,a) within an iteration. This
        makes GAE's delta = r + gamma*V(s') - V(s) reflect true value DIFFERENCES rather
        than two independent Monte-Carlo draws, whose 1/(1-gamma)=100x-amplified noise
        otherwise swamps the O(1) reward signal in every advantage.

        V(s) at the call site = E_{a~pi}[Q(s,a)] (one stochastic action sample per state)."""
        z = self.value_noise                         # (K, d) fixed this iteration
        K = z.shape[0]
        B = x.shape[0]
        cond_rep_obs = x.repeat_interleave(K, dim=0)  # (B*K, d): [s0xK, s1xK, ...]
        cond_rep_act = a.repeat_interleave(K, dim=0)  # (B*K, a): [a0xK, a1xK, ...]
        z_rep = z.repeat(B, 1)                        # (B*K, d): [z0..z_{K-1}] tiled per state
        end = torch.ones(B * K, 1, device=x.device)
        s_future = integrate_flow_sa(self.flow_net, cond_rep_obs, cond_rep_act, z_rep, end, steps)
...
if __name__ == "__main__":
    ...
    # v35: actor and flow+reward are optimized by SEPARATE Adam optimizers, so the flow's
    # data efficiency (replay + many grad steps per iter) is decoupled from PPO's update
    # cadence. Anneal applies to the actor lr only (PPO convention); flow lr stays constant.
    actor_params = agent.actor_parameters()
    critic_params = agent.critic_parameters()
    flow_target = copy.deepcopy(agent.flow_net)
    for p in flow_target.parameters():
        p.requires_grad_(False)
    flow_params = list(agent.flow_net.parameters())  # online flow only (flow_target has no grads)
    reward_params = list(agent.reward_model.parameters())
    optimizer = optim.Adam(actor_params, lr=args.learning_rate, eps=1e-5)
    flow_optimizer = optim.Adam(flow_params + reward_params, lr=args.flow_learning_rate, eps=1e-5)

    # v35: CUDA-resident FIFO replay buffer for the decoupled flow updates.
    obs_dim_replay = int(np.array(envs.single_observation_space.shape).prod())
    act_dim_replay = int(np.prod(envs.single_action_space.shape))
    replay = FlowReplay(args.replay_capacity, obs_dim_replay, act_dim_replay, device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                # V(s) = E_{a~pi}[Q(s,a)] via a single stochastic action sample (paper Alg. 1).
                # Reuses the rollout action to save a forward pass and stay on-policy.
                values[step] = agent.flow_value(next_obs, action, args.flow_value_steps).flatten()
            actions[step] = action
            latent_zs[step] = z
        ...
        with torch.no_grad():
            # Bootstrap V(s_T) = E_{a'~pi(.|s_T)}[Q(s_T, a')] with a single stochastic a' sample.
            boot_action, _, _, _ = agent.get_action_and_value(next_obs)
            next_value = agent.flow_value(next_obs, boot_action, args.flow_value_steps).reshape(1, -1)
        ...
        # v35: push the rollout into the FIFO replay so the decoupled flow update sees a
        # wider (s,a) support than a single rollout would provide.
        replay.push(b_obs, b_actions, b_next_obs, b_rewards, b_flow_mask)

        # v35: DECOUPLED flow update loop. Runs BEFORE the PPO epochs (PPO uses the rollout's
        # GAE values that were already computed, so this iter's PPO still sees the OLD flow's
        # values; the freshly updated flow shows up at the NEXT rollout). Many small grad-steps
        # over uniformly sampled replay transitions = the paper's data-efficient recipe ported
        # to on-policy. Logged diagnostics are averages over the inner steps.
        flow_loss_acc = 0.0
        flow_direct_acc = 0.0
        flow_boot_acc = 0.0
        reward_loss_acc = 0.0
        ...
        reward_gn_acc = 0.0
        n_flow_steps = 0
        if replay.size >= args.flow_warmup_size:
            for _ in range(args.flow_extra_steps):
                s_b, a_b, sn_b, r_b, m_b = replay.sample(args.flow_extra_batch, device)
                flow_loss, fdir, fboot = tdflow_loss(
                    agent.flow_net, flow_target, agent,
                    s_b, a_b, sn_b, m_b, args.flow_gamma, args.flow_steps,
                )
                r_pred = agent.reward_model(sn_b).squeeze(-1)
                denom = m_b.sum().clamp_min(1.0)
                reward_loss = (((r_pred - r_b) ** 2) * m_b).sum() / denom
                total = args.flow_coef * flow_loss + args.reward_coef * reward_loss
                flow_optimizer.zero_grad(set_to_none=True)
                total.backward()
                fgn = nn.utils.clip_grad_norm_(flow_params, args.flow_grad_clip)
                rgn = nn.utils.clip_grad_norm_(reward_params, args.reward_grad_clip)
                flow_optimizer.step()
                ema_update(flow_target, agent.flow_net, args.flow_target_decay)
                flow_loss_acc += float(flow_loss.item())
                flow_direct_acc += float(fdir.item())
                flow_boot_acc += float(fboot.item())
                reward_loss_acc += float(reward_loss.item())
                flow_gn_acc += float(fgn)
                reward_gn_acc += float(rgn)
                n_flow_steps += 1
        # value_std / value_mean diagnostics from a single fresh probe on a random replay
        # sample of 1024 (pre-PPO state of the flow), kept off the inner loop to avoid noise.
        if replay.size >= args.flow_warmup_size:
            with torch.no_grad():
                s_probe, a_probe, _, _, _ = replay.sample(min(1024, replay.size), device)
                v_probe = agent.flow_value(s_probe, a_probe, args.flow_value_steps)
                value_std_post = float(v_probe.std().item())
                value_mean_post = float(v_probe.mean().item())
        else:
            value_std_post = float(values.std().item())
            value_mean_post = float(values.mean().item())
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v35: PPO update optimizes ONLY the actor. Flow + reward are updated in a
                # separate decoupled loop on replay BEFORE the PPO epochs (above), so the
                # PPO graph here is the actor-only graph.
        ...
        # v35: flow diagnostics are MEANS over the per-iter decoupled flow grad-steps.
        if n_flow_steps > 0:
            writer.add_scalar("losses/flow_loss", flow_loss_acc / n_flow_steps, global_step)
            writer.add_scalar("losses/flow_direct", flow_direct_acc / n_flow_steps, global_step)
            writer.add_scalar("losses/flow_boot", flow_boot_acc / n_flow_steps, global_step)
            writer.add_scalar("losses/reward_loss", reward_loss_acc / n_flow_steps, global_step)
            writer.add_scalar("losses/flow_grad_norm", flow_gn_acc / n_flow_steps, global_step)
            writer.add_scalar("losses/reward_grad_norm", reward_gn_acc / n_flow_steps, global_step)
        writer.add_scalar("losses/value_std", value_std_post, global_step)
        writer.add_scalar("losses/value_mean", value_mean_post, global_step)
