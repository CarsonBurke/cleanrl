...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        clipfracs = []
        # Buffered rollout squashed entropy H = E[-logπ] (the SAME quantity the soft-adv
        # bonus used); the once-per-epoch dual regulates this toward target_entropy.
        if auto_alpha:
            buffered_entropy = (-logprobs).mean().detach()
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                entropy_loss = entropy.mean()  # logging only (v28 has NO actor entropy bonus)
            ...
            if auto_alpha:
                # v28 dual: step ONCE per epoch (not per-minibatch -> no 320x over-drain) on the
                # BUFFERED rollout squashed entropy (the SAME H that went into next_value_bonus,
                # so the temperature regulates exactly the entropy the bonus injected). SAC's
                # exact form: alpha_loss = -alpha*(logpi + target). When the policy SATURATES,
                # H drops below target and the dual turns two-sided (alpha rises) instead of
                # bleeding to 0.  alpha is in RAW units; its EFFECT is alpha/return_std.
                alpha_loss = (-log_alpha.exp() * (-buffered_entropy + target_entropy))
