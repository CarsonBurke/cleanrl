# PPO + IterThink v29 (TD-FLOW CRITIC — value derived ENTIRELY from a successor measure). From v24.
#
...
class FlowVelocity(nn.Module):
    """Rectified-flow velocity v(x_tau, tau | cond) over OBSERVATIONS for the TD-flow
    successor measure. Additive (FiLM-lite) conditioning on the trunk feature."""
    def __init__(self, cond_dim, x_dim, hidden):
        super().__init__()
        self.x_in = layer_init(nn.Linear(x_dim, hidden))
        self.t_in = layer_init(nn.Linear(1, hidden))
        self.cond_in = layer_init(nn.Linear(cond_dim, hidden))
        self.body = nn.Sequential(
            nn.Tanh(),
            layer_init(nn.Linear(hidden, hidden)), nn.Tanh(),
            layer_init(nn.Linear(hidden, x_dim), std=0.1),
        )

    def forward(self, x, t, cond):
        return self.body(self.x_in(x) + self.t_in(t) + self.cond_in(cond))
...
def tdflow_loss(net, target_net, cond_t, cond_next, next_obs, mask, gamma, steps):
    """TD2 rectified conditional-flow-matching loss (arXiv 2503.09817) for the discounted-
    future-observation successor measure. cond_t carries gradient (shapes the trunk -> the
    critic's representation signal); cond_next is detached (target side)."""
    B = next_obs.shape[0]
    t = torch.rand(B, 1, device=next_obs.device)
    x0 = torch.randn_like(next_obs)
    xt = (1.0 - t) * x0 + t * next_obs
    direct = ((net(xt, t, cond_t) - (next_obs - x0)) ** 2).sum(-1)
    with torch.no_grad():
        b0 = torch.randn_like(next_obs)
        bxt = integrate_flow(target_net, cond_next, b0, t, steps)
        btgt = target_net(bxt, t, cond_next)
    boot = ((net(bxt, t, cond_t) - btgt) ** 2).sum(-1)
...
class Args:
    ...
    flow_steps: int = 4               # Euler steps for the TD bootstrap integration
    flow_value_steps: int = 8         # Euler steps when sampling future obs for V(s)
    flow_value_samples: int = 16      # # future-obs samples averaged for V(s)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.flow_net = FlowVelocity(cond_dim=H, x_dim=obs_dim, hidden=args.flow_hidden)
    ...
    def flow_value(self, x, n_samples, steps):
        """V(s) via the successor-measure flow: sample n_samples future observations by
        integrating the flow to tau=1, score each with the reward model, average, scale by
        1/(1-gamma). Intended to be called under no_grad (rollout / bootstrap)."""
        cond = self._trunks(x)[1]
        B, d = x.shape[0], x.shape[1]
        cond_rep = cond.repeat_interleave(n_samples, dim=0)
        z = torch.randn(B * n_samples, d, device=x.device)
        end = torch.ones(B * n_samples, 1, device=x.device)
        s_future = integrate_flow(self.flow_net, cond_rep, z, end, steps)
        r = self.reward_model(s_future).squeeze(-1).view(B, n_samples).mean(dim=1)
...
if __name__ == "__main__":
    ...
    trunk_mod = agent.trunk if agent.share_backbone else agent.critic_trunk
    flow_params = list(trunk_mod.parameters()) + list(agent.flow_net.parameters())
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                values[step] = agent.flow_value(next_obs, args.flow_value_samples, args.flow_value_steps).flatten()
            actions[step] = action
            latent_zs[step] = z
        ...
        with torch.no_grad():
            next_value = agent.flow_value(next_obs, args.flow_value_samples, args.flow_value_steps).reshape(1, -1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v29 TD-FLOW CRITIC losses (separate graphs from the policy forward).
                cf_t = agent._trunks(b_obs[mb_inds])[1]                 # (mb,H) WITH grad -> shapes trunk
                cf_next = agent._trunks(b_next_obs[mb_inds])[1].detach()
                flow_loss = tdflow_loss(
                    agent.flow_net, flow_target, cf_t, cf_next,
                    b_next_obs[mb_inds], b_flow_mask[mb_inds],
                ...
                if args.separate_grad_clip:
                    # TRIPLE decoupled clip: flow (trunk+flow_net) / reward (reward_model) /
                    # policy (trunk+actor). All three losses are on INDEPENDENT graphs (each from
                    # its own forward), so no retain_graph is needed; we backward each, clip its
                    # own group, stash, then sum on the shared trunk before one optimizer.step().
                    optimizer.zero_grad(set_to_none=True)
                    (args.reward_coef * reward_loss).backward()
                    reward_gn = nn.utils.clip_grad_norm_(reward_params, args.reward_grad_clip)
                    reward_grads = [(p, p.grad.detach().clone()) for p in reward_params if p.grad is not None]
                    optimizer.zero_grad(set_to_none=True)
                    (args.flow_coef * flow_loss).backward()
                    flow_gn = nn.utils.clip_grad_norm_(flow_params, args.flow_grad_clip)
                    flow_grads = [(p, p.grad.detach().clone()) for p in flow_params if p.grad is not None]
                    optimizer.zero_grad(set_to_none=True)
                    (pg_loss - args.ent_coef * entropy_loss).backward()
                    actor_gn = nn.utils.clip_grad_norm_(actor_params, args.actor_grad_clip)
                    for p, g in flow_grads:
                        p.grad = g if p.grad is None else p.grad + g
                    for p, g in reward_grads:
                        p.grad = g if p.grad is None else p.grad + g
                    optimizer.step()
                else:
                    loss = pg_loss - args.ent_coef * entropy_loss + args.flow_coef * flow_loss + args.reward_coef * reward_loss
                    optimizer.zero_grad()
                    loss.backward()
                    actor_gn = flow_gn = reward_gn = nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                    optimizer.step()
