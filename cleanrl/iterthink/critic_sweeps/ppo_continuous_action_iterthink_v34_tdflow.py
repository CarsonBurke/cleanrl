# PPO + IterThink v34 (TD-FLOW CRITIC — Q-style (s,a) conditioning + TD-CFM(c) coupled noise). From v32.
#
# v32 RAN but the value collapsed to the on-policy STATE-MARGINAL successor measure (value_std
# -> 0.04 at 1M steps, episodic return ~739 vs the categorical baseline 9498). Root cause
# (validator-confirmed): V-style conditioning of the flow on `s` only lets the gamma=0.99
# bootstrap self-reinforce the marginal — the direct target s' depends on s only via the
# narrow on-policy action distribution, so the marginal m^pi(s') is itself a fixed point.
# v33 (AdaLN + gamma=0.97) refuted the conditioning-expressiveness / grounding-fraction
# hypotheses; the structural fix is the conditioning set itself.
#
...
# the regression target has lower transport cost. Replay / widening / FiLM / action-cond
# reward model are deferred to v35+ per the validator's "premature before the structural
# fix" call.
# From v32:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v34 TD-FLOW CRITIC loss. Flow conditions on RAW (obs, action) via its own
                # encoder (decoupled from the policy trunk) -> a fully independent graph. The
                # bootstrap target uses a' ~ pi(.|s') sampled fresh under no_grad (paper Alg. 1).
                flow_loss, flow_direct, flow_boot = tdflow_loss(
                    agent.flow_net, flow_target, agent,
                    b_obs[mb_inds], b_actions[mb_inds], b_next_obs[mb_inds], b_flow_mask[mb_inds],
