# PPO + IterThink v26 (SAC SOFT CRITIC: squashed-Gaussian max-entropy, unified). From v25.
#
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if auto_alpha:
                    # SAC's exact temperature dual (sac_continuous_action.py:292), on the
                    # SQUASHED log-prob: alpha_loss = (-α·(logπ + target_entropy)).mean().
                    # Drives E[-logπ_squashed] -> -target_entropy, i.e. H_squashed -> -|A|.
                    # v25 fix #3: dual targets the SAME fresh-rsample squashed entropy the
                    # actor bonus maximizes (entropy = -logπ at a fresh rsample) instead of
                    # v24's replayed-z newlogprob -- SAC uses one consistent fresh sample for
                    # both. The SAME α weights the explicit CURRENT-step actor entropy bonus
                    # below (the soft return's current-state entropy is action-independent =>
                    # zero in the PG term, so the bonus supplies the actual entropy gradient).
                    ent_coef_eff = log_alpha.exp().detach()
                    alpha_loss = (-log_alpha.exp() * (-entropy.detach() + target_entropy)).mean()
