# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + Delightful Policy Gradient v1.
#
# Variant hypothesis: keep the current best v24 beta PPO + symlog d4-HL-Gauss
# critic unchanged, and add Delightful Policy Gradient only to the actor loss:
#   U = existing rankgauss/minibatch-normalized policy advantage.
#   ell = clip(-log pi_theta(a|s), -10, 10) from the current action density.
#   w = sigmoid(U * ell / eta), eta=1.
#   PPO's clipped policy terms are multiplied by stopgrad(w).
