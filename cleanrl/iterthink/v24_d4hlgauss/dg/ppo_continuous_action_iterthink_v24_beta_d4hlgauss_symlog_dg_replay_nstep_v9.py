# IterThink v24 beta d4-HL-Gauss symlog critic + replay Delightful PG, N-STEP v9.
#
# WHY v9. The v7/v8 pair isolated the advantage bottleneck precisely:
#   - v7 (U = TD(0) under current target critic): stable, scales with actor
#     throughput, but decelerates vs baseline (2946 vs 4063 @1M at full step
#     parity) — TD(0) = GAE(lambda=0) is maximally critic-bias-limited, and
#     that bias dominates as true advantages shrink with policy improvement.
#   - v8 (U = collection-time lambda=0.95 GAE, replayed): STALLED (941@1M at
#     matched throughput) despite 0.07-nat staleness. Stored U credits actions
#     against an OLD critic baseline; once the policy improves, replayed
#     "good" actions are merely average yet still reinforced. U must be
#     re-evaluated under a CURRENT critic — exactly why the paper uses
#     Retrace-Q re-evaluated at sample time.
# v9 takes both lessons: REWARDS from the buffer (data, never stale) +
# BASELINE fresh at sample time. n-step TD under the current target critic:
#   U = sum_{k<h} gamma^k r_{t+k} + gamma^h * alive_h * V_tgt(s_{t+h}) - V_tgt(s_t)
# with h = min(nstep_horizon, steps to done/rollout end), windows precomputed
# at insertion. The off-policy bias from intermediate stale actions is small
# at our measured 0.4-nat staleness — and suppressing what remains is the
# delight gate's job (Algorithm 2's IS-free bet). dg_u_source:
# "nstep" (default) | "td0" (v7) | "stored_gae" (v8, kept for the trail).
#
...
class Args:
    ...
    # v9: where the actor's U comes from. "nstep" = buffered n-step rewards +
    # fresh target-critic baseline (see header); "td0" = v7's fresh TD(0);
    # "stored_gae" = v8's collection-time GAE (stalls, kept for the trail).
    dg_u_source: str = "nstep"
    nstep_horizon: int = 5
...
if __name__ == "__main__":
    ...
    assert args.dg_u_source in ("nstep", "stored_gae", "td0")
    ...
    # v9: n-step window per transition — discounted reward sum, window-end obs,
    # and bootstrap discount gamma^h * alive (0 if the window hit a done).
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            # v9: precompute each transition's n-step window within this rollout.
            # Entry (t,e): R_n accumulates gamma^k r_{t+k} while no done intervenes
            # and t+k stays inside the rollout; nobs/ndisc track the window-end
            # bootstrap point and its discount gamma^h (zeroed if the window
            # ended on a done). Tail entries get shorter windows (valid n-step
            # returns of smaller h), not a bias.
            nR = rewards.clone()
            ...
            for k in range(1, args.nstep_horizon):
                ...
                if head <= 0:
                    break
                ext = alive[:head]                      # window can absorb step t+k
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with torch.no_grad():
                    if args.dg_u_source == "nstep":
                        # v9: buffered n-step rewards + FRESH baseline. Cuts the
                        # TD(0) critic-bias bottleneck without v8's stale-credit
                        # stall (the baseline is re-evaluated every sample).
                        v_s = hl_support.to_scalar(target_agent.get_value(rs))
                        v_b = hl_support.to_scalar(target_agent.get_value(r_nobs[ridx]))
                        td_adv_raw = r_nR[ridx] + r_ndisc[ridx] * v_b - v_s
                    elif args.dg_u_source == "stored_gae":
                        # v8: lambda=0.95 GAE computed on-policy at collection.
                        # Stalls — credit goes stale against an improving critic.
