# IterThink v24 beta d4-HL-Gauss symlog critic + raw-gate/norm-update Delightful PG v1.
#
# Variant hypothesis: strict paper-DG in this PPO/HL-Gauss stack under-learns
# because raw normalized-reward GAE is too small for the actor coefficient. Keep
# the DG gate's delight semantics raw:
#   U_gate = raw policy advantage estimate.
#   ell = clip(-log pi_theta(a|s), -10, 10) from the current action density.
#   w = sigmoid(U_gate * ell / eta), eta=1.
# But use minibatch-normalized advantage as the actor score coefficient:
#   policy loss = -(stopgrad(w) * norm(U_gate) * log pi_theta(a|s)).mean().
...
def delightful_gate(advantages, logprob, eta=1.0, clip_bound=10.0):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.delightful_pg:
                    ...
                    with torch.no_grad():
                        pos_mask = mb_raw_adv > 0
                        neg_mask = mb_raw_adv < 0
                        ...
                        dg_update_adv_stds.append(mb_advantages.std(unbiased=False).item())
                else:
                ...
                pg_loss = -(dg_weight * mb_advantages * newlogprob).mean()
