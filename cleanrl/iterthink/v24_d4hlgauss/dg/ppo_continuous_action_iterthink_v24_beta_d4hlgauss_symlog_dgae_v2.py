# PPO + IterThink v24 beta d4-HL-Gauss symlog DGAE v2 (DISTRIBUTIONAL lambda-return critic).
#
# WHY v2. v1 review (arXiv:2507.17530) found the critic was only *nominally*
# distributional: its target was the SCALAR GAE lambda-return projected by
# HL-Gauss, i.e. a smoothed point mass at one number. With a near-point-mass Z(s),
# every distribution-aware quantity (DGAE advantage, any risk measure rho(Z)) is
# degenerate ~ mean, which is exactly why the v1 DGAE advantage was a near no-op.
# v2 fixes the FOUNDATION: the critic now regresses a genuine DISTRIBUTIONAL
# lambda-return target via the distributional TD(lambda) backward recursion, so
# Z(s) can learn a real return distribution (spread, skew), not a point mass.
#
#   Distributional TD(lambda) target (categorical, on the fixed symlog support):
#     Z^lambda_t = Pi( r_t + gamma * nonterm * [ (1-lambda) Z(s_{t+1})
#                                                 + lambda    Z^lambda_{t+1} ] )
#   where the convex combo is a MIXTURE of categoricals (weighted-average probs),
#   `gamma*. + r` moves each atom from raw location z_i to r_t + gamma*z_i, and Pi
#   is the per-atom HL-Gauss projection back onto the fixed bins (each Bellman
#   image atom -> a sigma=2-bin Gaussian; the mixture is their probability sum).
#   This GENERALIZES v1's scalar target: a point-mass Z' makes the target identical
#   to v1's project(scalar lambda-return), so v2 vs v1 is a clean A/B isolating
#   "distributional return target" alone. Mean is preserved up to the symlog-grid
#   projection (tracked by explained_variance + the decode-MAE diagnostics).
#
# UNCHANGED vs v1: the POLICY advantage path (DGAE / adv_transform), bootstrapping
# (symexp(E[z])), reward norm/clip, support, critic init, PPO settings. `--dist-
# lambda-target` (default True) toggles the new target back to v1's scalar target
# for the A/B. Note: with the default rankgauss adv_transform the policy only sees
# ranks, so to read v2's effect on the *advantage* run --adv-transform v10.
#
# CALIBRATION WATCH: a genuinely wide Z in SYMLOG space makes the raw-mean decode
# E[symexp(z)] drift above symexp(E[z]) (Jensen, symexp convex) as spread grows.
# v2 tracks target_dist_std, dgae_rawmean_decode_mae, and explained_variance so
# this is observable; if it bites, a raw-space (non-symlog) distributional support
# is the natural follow-up (v3).
#
# --- inherited v1 notes ---
# Keep the strong d4-HL-Gauss symlog critic, use Distributional GAE
# (arXiv:2507.17530) for the POLICY advantage. The actor's advantage recursion
# swaps scalar TD errors for the paper's distributional TD error:
#   delta_G = r + d(gamma Z(s'), Z(s))
# For our one-dimensional categorical critic and the paper's linear directional
# inverse-CDF metric, the exact matched-quantile average collapses to
#   d(gamma Z', Z) = gamma E[Z'] - E[Z].
# This is computed in raw normalized-return units from raw bin centers
# symexp(symlog_center). That is intentionally distinct from the symlog scalar
# decode used by this file's critic bootstrap, symexp(E[symlog_center]).
#
...
def distributional_lambda_target(
    rewards, dones, next_done, value_probs, bootstrap_probs, raw_support, gamma, lam, hl_support
):
    """Distributional TD(lambda) return target as a categorical over the fixed support.

    Backward recursion (the distributional analog of the scalar GAE lambda-return):
        Z^lambda_t = Pi( r_t + gamma * nonterm * [ (1-lambda) Z(s_{t+1})
                                                   + lambda    Z^lambda_{t+1} ] )
    The convex combination is a mixture of categoricals on the SAME support
    (probability-weighted average), `gamma*. + r` shifts/scales each atom in RAW
    return units (atom z_i -> r_t + gamma*z_i), and Pi is the per-atom HL-Gauss
    projection back onto the fixed bins (project() symlogs + clamps internally, so
    raw atom locations are correct to pass in). When the next state is terminal
    (nonterm=0) every atom collapses to r_t, so the mixture weights wash out and
    the target is exactly project(r_t) -- a point mass at the terminal reward.

    Returns lam_probs (T, B, num_bins) and the per-state target std in RAW units.
    """
    ...
    raw = raw_support.unsqueeze(0)  # (1, n)
    for t in reversed(range(T)):
        nonterminal = (1.0 - next_done) if t == T - 1 else (1.0 - dones[t + 1])  # (B,)
        m = (1.0 - lam) * next_val + lam * next_lam                              # (B, n) mixture
        loc = rewards[t].unsqueeze(-1) + gamma * nonterminal.unsqueeze(-1) * raw  # (B, n) raw atom locs
        proj = hl_support.project(loc)                                           # (B, n, num_bins)
        probs_t = (m.unsqueeze(-1) * proj).sum(dim=1)                            # (B, num_bins)
        lam_probs[t] = probs_t
        mean_raw = (probs_t * raw_support).sum(-1, keepdim=True)
        target_std[t] = (probs_t * (raw_support - mean_raw) ** 2).sum(-1).clamp_min(0).sqrt()
        next_lam = probs_t
...
class Args:
    ...
    dist_lambda_target: bool = True        # v2: distributional TD(lambda) critic target (else v1 scalar-return)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Critic target. v2: a genuine DISTRIBUTIONAL TD(lambda) return target
            # (categorical Bellman mixture, see distributional_lambda_target) so Z(s)
            # learns a real return distribution. v1 fallback: the scalar lambda-return
            # projected by HL-Gauss (a smoothed point mass).
            if args.dist_lambda_target:
                target_probs, target_dist_std = distributional_lambda_target(
                    rewards, dones, next_done, value_probs, bootstrap_probs,
                    raw_support, args.gamma, args.gae_lambda, hl_support,
                )
            else:
