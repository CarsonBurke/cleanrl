# PPO + IterThink v24 beta d4-HL-Gauss critic + AMPLIFY-ONLY CONFIDENCE GATE v17.
#
# WHY v17 (the sign flip). The entire delight-gate family — reward the
# SURPRISING action — was falsified across v13/v14 (relative: clip reskin, below
# base), v15 (absolute symmetric: collapse to uniform), v16 (amplify-only:
# unproductive churn). One mechanism explains all three: in dense-reward
# continuous control the critic and policy co-adapt, so GOOD ACTIONS ARE THE
# TYPICAL ONES => cov(U, ell~) < 0. A delight gate keyed on U*ell~ therefore puts
# its weight on rare-bad / surprising samples (noise), starving the
# confident-good gradient PPO rides. v17 flips the sign (chi = -U*ell~): reward
# CONFIDENT-CORRECT, prune surprising-bad noise. This is a bounded (<=2x)
# variance-reducing PG reweight aligned WITH the env's structure, amplify-only so
# the worst case is base PPO. Clean: on-policy, single actor loss, no
# aux/EMA/replay/clip/IS-ratio. If this too caps at base, both gate signs are
# exhausted and the gate-as-PG-modulator idea is closed for this regime.
#
# (Historical) WHY v16. Two faithful delight-gate forms failed for orthogonal reasons:
#   - RELATIVE surprisal (v13/v14): ell_rel=-logratio cancels typicality, leaving
#     a Fisher pull toward pi_old = a soft-clip RESKIN. Capped BELOW base at every
#     checkpoint (8637 vs 9785 @8M).
#   - ABSOLUTE symmetric (v15): 2*sigmoid(U*ell~/eta) COLLAPSED to the uniform
#     max-entropy policy. Diagnosed mechanistically: in dense-reward continuous
#     control the critic and policy co-adapt so good actions are TYPICAL ones,
#     hence cov(U, ell~)<0. Then U>0 => ell~<0 => chi<0 => gate<0.5: the gate
#     SUPPRESSES the confident-good gradient PPO rides. Measured collapse:
#     entropy->0, KL->1e-6, actor_grad_norm 1.33->0.003 by 400k. The delight
#     "reinforce-the-surprising" bias is anti-aligned with this regime.
#
# v16 keeps the delight signal but makes it NON-SUPPRESSING: clamp the gate to
# >=1 so it can only ADD reinforcement, never cut below standard PG. The uniform
# equilibrium is then unreachable (confident-good always keeps full weight), and
# the gate reduces to a bounded, self-annealing delight BONUS on good-surprises
# (U>0,ell~>0) and on fleeing typical-bad (U<0,ell~<0). On-policy, single actor
# loss, no aux/EMA/replay/clip/IS-ratio. Worst case ~= base PPO (bonus rarely
# fires once good==typical); upside = stronger early exploration out of
# HalfCheetah's mediocre-gait basins. eta smaller => stronger bonus when it fires.
#
# (Historical note: the relative-surprisal rationale that motivated v13.)
# WHY v15. The relative-surprisal gate (v13/v14) was proven empirically to cap
# at clip-equivalence and UNDERPERFORM base IterThink at every checkpoint
# (8637 vs 9785 @8M). v13's own docstring states why: ell_rel = -logratio
# CANCELS every per-state typicality offset, leaving an even-in-U component that
# is a pure Fisher-metric pull toward pi_old — a soft trust region, i.e. a clip
# RESKIN with no mechanism to exceed PPO. The "delight" of the Delight Gate is
# precisely the absolute-rarity signal that cancellation destroyed.
#
# v15 RESTORES the faithful Algorithm-2 mechanism, on-policy, no aux/EMA/replay,
# gate REPLACES the PG term (single actor loss):
#   ell~ = (-logpi_theta(a|s)) - H(s)        # entropy-referenced surprisal
#   chi  = clip(U, +-u_clip) * (ell~ / std(ell~))     # dimensionless
#   w    = sigmoid(chi / eta), detached;  U = minibatch-normalized GAE
#   pg_loss = -(2 * w * U * logpi_theta).mean()       # no ratio, no clip
#
# WHY ell~ (entropy-referenced), NOT raw -logpi: raw surprisal sits at H(s) per
# state, so on-policy it is a near-degenerate sign gate whose even-in-U part is
# a NON-annealing entropy bonus this env punishes (the v1/v2 failures). ell~ =
# s - H is per-state ZERO-MEAN (E[-logpi]=H), so it carries genuine WITHIN-state
# typicality: ell~>0 the realized action was a tail/surprising draw, ell~<0 it
# was modal. std-normalizing makes eta a clean trust width and stops a few
# Beta-boundary samples from saturating the gate. No centering (would move the
# typical/tail zero-crossing).
#
# THE DELIGHT QUADRANTS (the genuine mechanism vs clip, which weights all
# in-region samples uniformly by ratio*U):
#   U>0 & ell~>0 (pleasant surprise = DELIGHT): w>0.5 -> AMPLIFY. Chase good
#       actions the policy currently finds unlikely -> exploration drive.
#   U>0 & ell~<0 (good but already typical):    w<0.5 -> damp (already learned).
#   U<0 & ell~>0 (bad surprise, likely noise):  w<0.5 -> damp punishment.
#   U<0 & ell~<0 (consistently-bad modal):      w>0.5 -> AMPLIFY flight.
# SELF-ANNEALING: a reinforced good-surprise rises in probability -> becomes
# typical (ell~->0-) -> its amplification fades. The even-in-U entropy force is
# scaled by U^2/2eta and so vanishes as the critic converges (U->0) -- an
# exploration bonus that auto-decays, unlike the raw v1/v2 bonus. target_kl 0.03
# remains the carrier-standard drift backstop (no extra trust region added).
#
# Health signatures (log + watch): relgate_epoch_first ~ 0.5; pos_mean > 0.5 and
# neg_mean < 0.5 (the delight asymmetry, the opposite of a symmetric trust
# region); entropy should still fall (escaping, not freezing) -- if it pins high
# the entropy force is too strong, raise eta. Entropy past -3 by ~1.5M en route
# toward base's deep-decisive -8 by 8M.
#
...
class Args:
    ...
    dg_eta: float = 0.5   # v15 absdelight temperature on std-normalized chi (~unit scale)
    ...
    # v13: relative-surprisal delight gate REPLACES the PPO clipped surrogate
    # (see the pg_loss block). eta sets the gate's trust-region softness:
    # transition width in logratio is eta/|U|; 0.1 saturates ~where PPO clips.
    dg_relgate: bool = False
    # v15: absolute entropy-referenced surprisal delight gate REPLACES the PPO
    # clipped surrogate (takes precedence over dg_relgate). See pg_loss block.
    dg_absdelight: bool = True
    dg_u_clip: float = 2.0   # |U| cap inside chi only (trust-radius floor eta/2)
    dg_s_mask: float = 4.0   # clamp std-normalized ell~ into chi (gate-saturation guard)
...
if __name__ == "__main__":
    ...
    assert args.dg_eta > 0, "dg_eta must be positive"
    assert args.dg_u_clip > 0 and args.dg_s_mask > 0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        clipfracs = []
        relgate_epoch_sums = [0.0] * args.update_epochs
        relgate_epoch_counts = [0] * args.update_epochs
        relgate_pos_means = []
        relgate_neg_means = []
        relgate_recover_fracs = []
        relgate_mask_fracs = []
        dg_delight_means = []
        dg_delight_stds = []
        dg_surprisal_means = []
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.dg_absdelight:
                    # v17: AMPLIFY-ONLY CONFIDENCE gate (sign-flipped delight).
                    # ell~ = (-logpi_theta) - H(s) is the realized within-state
                    # typicality (zero-mean per state): ell~<0 modal/confident,
                    # ell~>0 a surprising draw. std-normalize; clamp into chi.
                    #
                    # WHY THE FLIP: the v13/v15/v16 delight family (reward the
                    # SURPRISING) was falsified mechanistically. In dense-reward
                    # CC the critic+policy co-adapt so good actions are TYPICAL:
                    # cov(U, ell~)<0. The delight sign therefore amplified noise
                    # (rare-bad, surprising) over signal and collapsed/churned the
                    # policy toward uniform. v17 uses chi = -U*ell~ instead, so the
                    # gate rewards CONFIDENT-CORRECT, the COMMON aligned quadrant:
                    #   U>0 & ell~<0 (confident-good): -U*ell~>0 -> amplify (signal)
                    #   U<0 & ell~>0 (surprising-bad/noise): -U*ell~>0 -> amplify
                    #       its punishment (prune exploration noise)
                    #   U>0 & ell~>0 (lucky-rare-good) and U<0 & ell~<0
                    #       (typical-bad): chi<0 -> clamp to 1 = full PG, never cut.
                    # Net: a bounded (<=2x) VARIANCE-REDUCING reweight that leans
                    # the update toward samples where policy-confidence and
                    # advantage AGREE, down-weighting high-variance atypical draws.
                    # amplify-only clamp(>=1) keeps base PPO intact (worst case ~=
                    # base, equilibrium unreachable). No ratio, no clip; single
                    # actor loss; target_kl is the only drift backstop. Watch
                    # entropy: if it freefalls vs base => over-sharpening, raise eta.
                    ell_abs = (-newlogprob - entropy).detach()
                    ell_std = ell_abs / (ell_abs.std(unbiased=False) + 1e-8)
                    ell_std = ell_std.clamp(-args.dg_s_mask, args.dg_s_mask)
                    chi = mb_advantages.clamp(-args.dg_u_clip, args.dg_u_clip) * ell_std
                    # sign flip (-chi): reward confident-correct, not surprising.
                    relgate = (2.0 * torch.sigmoid(-chi / args.dg_eta)).clamp(min=1.0).detach()
                    pg_loss = -(dg_weight * relgate * mb_advantages * newlogprob).mean()
                    with torch.no_grad():
                        relgate_epoch_sums[epoch] += relgate.mean().item()
                        relgate_epoch_counts[epoch] += 1
                        rg_pos = mb_advantages > 0
                        if rg_pos.any():
                            relgate_pos_means.append(relgate[rg_pos].mean().item())
                        if (~rg_pos).any():
                            relgate_neg_means.append(relgate[~rg_pos].mean().item())
                        # recover_frac here = share of samples where the delight
                        # bonus actually fired (post-clamp weight strictly > 1).
                        relgate_recover_fracs.append(
                            (relgate > 1.0).float().mean().item()
                        )
                        # mask_frac repurposed: share hitting the s_mask clamp
                        relgate_mask_fracs.append(
                            (ell_std.abs() >= args.dg_s_mask).float().mean().item()
                        )
                elif args.dg_relgate:
                    # v13: the RELATIVE-SURPRISAL delight gate REPLACES the PPO
                    # clipped surrogate. ell_rel = logpi_old - logpi_theta at the
                    # replayed native z: every per-state typicality offset (H(s),
                    # Beta rescale Jacobian) cancels EXACTLY, so unlike absolute
                    # surprisal there is no implicit entropy force — the gate's
                    # even-in-U component is -(1/8eta) * grad[U^2 * logratio^2],
                    # a Fisher-metric pull toward pi_old (soft trust region).
                    # Epoch 1 (ratio=1): w=0.5, and the 2x rescale makes the
                    # update EXACTLY vanilla PG = PPO's own first minibatch.
                    # Later epochs: U>0 samples whose prob already rose are
                    # damped (w->0 exponentially, self-limiting); U>0 samples
                    # LOSING probability are amplified up to 2x — recovery of
                    # suppressed-good actions, which PPO's clip+IS weight starve.
                    # U<0 mirrors. No ratio multiplies the gradient; no clip.
                    ell_rel = -logratio
                    # Validity mask, not a clamp: |s|>4 means the density ratio
                    # is off >50x — for a Beta policy these sit near the support
                    # boundary where grad logpi diverges, and the gate's amplified
                    # quadrant (U>0, abandoned action) would blow up exactly where
                    # PPO's IS weight e^{-s} would have suppressed exponentially.
                    # Zero their gradient (PPO clip-dead-zone analog).
                    rg_valid = (ell_rel.abs() <= args.dg_s_mask).float().detach()
                    # clip(U,±2) inside chi ONLY (multiplier keeps full U): floors
                    # the per-sample trust radius at eta/2 so heavy-tail advantage
                    # samples are not frozen an order of magnitude before PPO would.
                    chi = mb_advantages.clamp(-args.dg_u_clip, args.dg_u_clip) * ell_rel
                    relgate = torch.sigmoid(chi / args.dg_eta).detach()
                    pg_loss = -(2.0 * dg_weight * rg_valid * relgate * mb_advantages * newlogprob).mean()
                    with torch.no_grad():
                        relgate_epoch_sums[epoch] += relgate.mean().item()
                        relgate_epoch_counts[epoch] += 1
                        rg_pos = mb_advantages > 0
                        ...
                        if (~rg_pos).any():
                            relgate_neg_means.append(relgate[~rg_pos].mean().item())
                        relgate_recover_fracs.append(
                            ((relgate > 0.9) & rg_pos).float().mean().item()
                        )
                        relgate_mask_fracs.append(rg_valid.mean().item())
                else:
        ...
        if args.dg_relgate or args.dg_absdelight:
            ep_means = [
                relgate_epoch_sums[e] / relgate_epoch_counts[e]
                for e in range(args.update_epochs)
                if relgate_epoch_counts[e] > 0
            ]
            writer.add_scalar("debug/relgate_epoch_first", ep_means[0], global_step)
            writer.add_scalar("debug/relgate_epoch_last", ep_means[-1], global_step)
            writer.add_scalar("debug/relgate_mean", float(np.mean(ep_means)), global_step)
            writer.add_scalar("debug/relgate_pos_mean", list_mean(relgate_pos_means, 0.5), global_step)
            writer.add_scalar("debug/relgate_neg_mean", list_mean(relgate_neg_means, 0.5), global_step)
            writer.add_scalar("debug/relgate_recover_frac", list_mean(relgate_recover_fracs), global_step)
            writer.add_scalar("debug/relgate_mask_frac", list_mean(relgate_mask_fracs, 1.0), global_step)
