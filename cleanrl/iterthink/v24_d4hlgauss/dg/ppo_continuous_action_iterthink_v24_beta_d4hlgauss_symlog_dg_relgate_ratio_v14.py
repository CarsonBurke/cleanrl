# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + RELGATE-ON-RATIO v14.
#
# WHY v14. v13 (gate REPLACING the whole estimator, plain grad-logpi carrier)
# reproduced the replay-line lesson ON-POLICY at 1M: gated vanilla PG lacks
# PPO's ratio-multiplied sharpening. eta=0.1 over-tightened (KL 0.022 < 0.03
# budget unused, entropy stuck at -1.1, 35% of control); eta=0.3 was healthy
# (entropy -3.0, gate gap 0.02) but still -15% vs matched control at both
# 500k and 1M. Mask never fired; recover_frac ~1% (on-policy drift at KL 0.03
# is too small for the recovery channel to matter).
# v14: the gate replaces PPO's CLIP, not the estimator — keep the IS ratio:
#   ell_rel = logpi_old - logpi_theta  (= -log ratio = -s)
#   w = sigmoid(clip(U,+-2) * ell_rel / eta), detached
#   pg_loss = -(2 * w * U * ratio * m).mean()    (single loss, no aux/EMA)
# Gradient: 2 w U ratio grad-logpi. Anchors:
#   - epoch-1 first minibatch: ratio=1, w=0.5, x2 => EXACTLY vanilla PG;
#   - U>0, ratio rising past trust radius: w -> 0 smoothly (PPO's upper clip,
#     but smooth and advantage-adaptive);
#   - U<0, ratio -> 0 (escaped bad action): w -> 0 AND ratio -> 0 — double
#     suppression; ratio=e^{-s} dominates any Beta-boundary grad-logpi
#     divergence, so this quadrant is safer than PPO's (which merely clips);
#   - U>0, ratio < 1 (suppressed good action): w in (0.5,1] amplifies up to
#     2x against the fading ratio — partial recovery, milder than v13;
#   - U<0, ratio > 1 (drifting toward bad action): w -> 1, 2x corrective.
# WHERE THE SPREAD COMES FROM: Algorithm 2 needs surprisal spread, and prior
# attempts showed absolute density surprisal has none on-policy (it sits at
# H(s) per state; the gate degenerates to a sign gate whose even-in-U
# component is an entropy bonus this env punishes — v1/v2 failures). PPO's own
# multi-epoch reuse IS an off-policy regime in miniature: by epoch 2+ the
# minibatch actions are mildly stale and ell_rel has real, growing spread.
# Referencing surprisal to the ROLLOUT policy at the same native z cancels
# every typicality offset exactly (H(s), Beta rescale Jacobian), so:
#   - even-in-U component = -(1/8eta) grad[U^2 * logratio^2]: a Fisher-metric
#     pull toward pi_old — soft trust region, NOT an entropy force (verified
#     analytically + numerically by a red-team pass);
#   - self-limiting: U>0 samples whose prob already rose gate to 0
#     exponentially (drift ~ log t); U<0 suppression mirrors. Strictly more
#     bounded than PPO's unclipped U<0 branch;
#   - epoch-1 first minibatch (ratio=1): w=0.5, 2x rescale => EXACTLY vanilla
#     PG, identical to PPO's own first minibatch;
#   - the novel behavior vs PPO's clip: suppressed-good-action RECOVERY.
#     PPO starves U>0 samples whose ratio fell (IS weight -> 0, clip silent);
#     the gate amplifies them up to 2x. Two-sided smooth advantage-adaptive
#     trust region overall.
# Honest scope note: relative surprisal cancels the paper's absolute-rarity
# mechanisms (Prop 1/2); what is kept is Algorithm 2's gated score-function
# FORM, operating on update-drift instead of typicality. The paper itself
# only ever validated continuous control off replay (App E.1).
# eta: with the ratio supplying sharpening, the gate only needs to act where
# PPO's clip would. Half-power at logratio eta/|U|; |U|~0.8 typical after
# norm => eta=0.15 puts half-power near logratio 0.19, between the carrier's
# clip bounds (0.2/0.28) — single arm. target_kl 0.03 kept (carrier-standard).
# Health signatures (log + watch):
#   relgate_epoch0 ~ 0.5 always; relgate_epochN falling to 0.25-0.40 healthy;
#   pinned at 0.5 => eta too big; ~0 by epoch 3 => eta too small.
#   |pos-neg gate gap| > 0.05 persistent => an asymmetric force re-emerged.
#   Entropy must pass -3 by ~1M en route to -6 by ~2M (baseline trajectory).
#   approx_kl should now ride the 0.03 budget like PPO does (v13 eta=0.1
#   sat at 0.022 — the signature of the gate, not the budget, binding).
#
...
class Args:
    ...
    dg_eta: float = 0.15   # v14 gate temperature (half-power at logratio eta/|U|)
    ...
    # v14: relative-surprisal delight gate replaces PPO's CLIP on the
    # IS-weighted surrogate: loss = -(2 w U ratio); see header.
    dg_relgate: bool = True
    dg_u_clip: float = 2.0   # |U| cap inside chi only (trust-radius floor eta/2)
    dg_s_mask: float = 4.0   # drop samples with |log ratio| beyond this (grad blowup guard)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.dg_relgate:
                    ...
                    # v14: keep the IS ratio (grad path: ratio = exp(newlogprob
                    # - old)); the detached gate is the smooth two-sided clip.
                    # Saturate the exp at the mask bound: identical for every
                    # unmasked sample, turns masked samples' 0*inf into 0.
                    safe_ratio = logratio.clamp(-args.dg_s_mask, args.dg_s_mask).exp()
                    pg_loss = -(2.0 * dg_weight * rg_valid * relgate * mb_advantages * safe_ratio).mean()
                    with torch.no_grad():
