# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return idx-transfer + TPO-MD v9
# (DYNAMIC trust region, v5) + CEM-GUIDED CANDIDATES (break the candidate-quality ceiling).
#
# v9 HYPOTHESIS: v5's pure-TPO plateau (~10.5k from ~5M on) is a CANDIDATE-QUALITY ceiling,
# not a normalizer or step-size problem (v8 proved sigma is redundant under the eta-solver;
# v5's KL step held at ~0.03 straight through the plateau). The K candidates are i.i.d.
# samples from the rollout policy, so as the policy concentrates they collapse to near-
# identical actions and the oracle has no genuinely-better action to imitate. Broadening
# the proposal RANDOMLY (v6) regressed — random wide samples are mostly WORSE. v9 instead
# uses the critic-oracle to make candidates BETTER: each step, probe+score the round-0
# samples, refit a z-space Gaussian to the top-`elite_frac` elites, resample, and probe the
# refined set (tpo_cem_rounds of this). The TPO target still anchors on the TRUE rollout
# policy (q ∝ p_old·exp(u/eta), trust region intact) and candidate 0 stays the executed
# action — so CEM only injects genuinely higher-value actions into the imitation target.
# COST: ~2x probe per CEM round. tpo_cem_rounds=0 is bit-identical to v5.
#
...
class Args:
    ...
    tpo_cem_rounds: int = 1          # CEM refinement rounds AFTER the initial sample (0 => bit-identical to v5)
    tpo_cem_elite_frac: float = 0.5  # fraction of K kept as elites for the refit (>=2 elites enforced)
    tpo_cem_temp: float = 1.0        # std multiplier when resampling from the refit Gaussian
    tpo_cem_min_std: float = 0.05    # z-space std floor so collapsed elites still spread (avoid degenerate refit)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # v9: with tpo_cem_rounds>0 the candidate set is CEM-refined toward high-value
            # regions before the final probe; tpo_cem_rounds=0 reproduces v5 exactly.
            ...
            with torch.no_grad():
                ...
                with torch.random.fork_rng(devices=[device]):
                    ...
                    # --- CEM refinement: probe+score, refit a z-space Gaussian to elites, resample ---
                    n_elite = max(2, int(round(args.tpo_cem_elite_frac * args.tpo_k)))
                    for _cem_round in range(args.tpo_cem_rounds):
                        ...
                        elite_idx = cem_scores.topk(n_elite, dim=1).indices  # (N, n_elite)
                        elite_z = torch.gather(
                            cand_z, 1, elite_idx.unsqueeze(-1).expand(-1, -1, cand_z.shape[-1])
                        )                                                 # (N, n_elite, A)
                        mu = elite_z.mean(dim=1)                          # (N, A)
                        std = elite_z.std(dim=1, unbiased=False).clamp_min(args.tpo_cem_min_std)
                        std = std * args.tpo_cem_temp
                        noise = torch.randn((args.num_envs, args.tpo_k) + act_shape, device=device)
                        cand_z = mu.unsqueeze(1) + std.unsqueeze(1) * noise
                        if args.actor_dist == "beta":
                            cand_z = cand_z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
