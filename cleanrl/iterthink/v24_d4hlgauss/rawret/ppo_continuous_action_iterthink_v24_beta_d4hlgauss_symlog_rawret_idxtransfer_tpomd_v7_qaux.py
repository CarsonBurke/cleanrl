# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return idx-transfer + TPO-MD v7
# (v5 dyn-trust mirror descent PLUS an action-conditioned Q-head auxiliary that
# ATTACHES the probe transitions to the shared backbone).
#
# === v7 ADDITION: action-conditioned Q-head auxiliary (probe attachment) ===
# In v5 the K sim-probe transitions per state are scored with the FROZEN pre-update
# critic: scores_k = r_k + gamma*(1-term_k)*V_old(s'_k). Those scores build the
# detached TPO target b_tpo_q and are then DISCARDED — the probe data is a read-only
# frozen-critic oracle that contributes ZERO gradient to the trunk. v7 "attaches" it
# via a small action-conditioned Q-head Q(s, a) on the SHARED trunk, trained by CE
# against the SAME unbiased one-step targets (project(scores_k)). The Q targets are
# return-scale (V predicts returns, so r+gamma*V ~ return-scale), so the Q-head reuses
# the existing HL-Gauss support / projection / decode exactly. The Q-head feeds
# concat(trunk_feature, candidate_action) -> num_bins HL-Gauss logits.
#
# DECOUPLED ISOLATION (critic-branch only, cannot corrupt actor features): the Q-head
# params live in critic_params; the Q-aux loss is added to the CRITIC backward ONLY
# (vf_coef*v_loss + tpo_qaux_coef*qaux_loss), so on the shared trunk the Q-aux gradient
# is summed into the critic branch and clipped under critic_grad_clip — apart from the
# actor branch, which is textually UNCHANGED. The Q-aux gradient NEVER reaches
# actor_params.
#
# CANDIDATE REPRESENTATION (z vs action): the Q-head is fed the env ACTION of each
# candidate, obtained by mapping the SAME stored latent candidates b_tpo_zs
# (candidate 0 = the executed action) through the SAME to_action used in the probe.
# This conditions Q on exactly the action that produced each score, so the 0..K
# alignment with b_tpo_scores is exact.
#
# SCOPE: the Q-head is a representation-shaping AUXILIARY ONLY. The TPO scoring/target
# is UNCHANGED — b_tpo_q is still built from the sim-probe `scores`, still detached,
# still frozen across epochs. The Q-head does NOT feed scoring in this version; it sets
# up a future probe-free Q-scoring variant (telemetry debug/q_score_corr tracks how well
# Q predicts the oracle). tpo_qaux=False OR tpo_qaux_coef=0 => bit-identical to v5
# (no Q-head forward, nothing added to any backward; b_tpo_scores is a no-op alias of
# the already-computed scores).
#
# BIT-IDENTITY of the init RNG: the Q-head MODULE is constructed LAST in __init__, after
# the trunk, value head and all actor heads have drawn their orthogonal-init RNG, so its
# own init draws are APPENDED to the stream and shift NO existing module's parameters
# (verified: 279 shared params, max-abs-diff 0.0 vs v5 under the same seed). The init
# runs on CPU tensors (pre-.to(device)) so it advances only the CPU generator; the beta/
# gaussian rollout dist.sample() runs on CUDA tensors and draws from the CUDA generator,
# which the q_head CPU init does not touch — so the FIRST rollout sample is bit-identical
# to v5 (verified: max-abs-diff 0.0). The q_head params are added to the optimizer but,
# off, receive grad=None every step (skipped by Adam and by clip_grad_norm_), so the
# optimizer trajectory is identical to v5.
#
# --- inherited v5 notes ---
...
class Args:
    ...
    # v7: action-conditioned Q-head auxiliary on the SHARED trunk, trained on the
    # probe transitions (CE to project(scores_k)). Routed through the CRITIC backward
    # ONLY (decoupled-clipped with V), so it shapes the trunk representation without
    # ever touching the actor branch. Representation-shaping auxiliary only; does NOT
    # feed the TPO scoring/target in this version.
    tpo_qaux: bool = True            # master switch; False => bit-identical to v5
    tpo_qaux_coef: float = 1.0       # weight of the Q-aux CE within the CRITIC branch; 0.0 => bit-identical to v5
    qhead_hidden: int = 64           # Q-head MLP hidden width (mirrors trunk H)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v7: action-conditioned Q-head on the SHARED trunk. Trained on the probe
        # transitions (CE to project(scores_k)); routed through the CRITIC branch
        # only. Mirrors the value head's HL-Gauss output dim (num_bins) and reuses
        # hl_support for projection/decode. Input is concat(trunk_feature, action)
        # (action dim added to the first layer's input), a small MLP -> num_bins.
        #
        # BIT-IDENTITY: this module is constructed LAST, after the trunk, value head
        # and all actor heads have drawn their init RNG, so its parameter init draws
        # NO random numbers ahead of any existing module. With tpo_qaux off the head
        # is never forwarded and receives no gradient, so the optimizer's update is
        # identical to v5 (Adam state for these params stays at zero grad => no step),
        # and the only externally observable difference — the RNG consumed by the
        # head's own init — is appended AFTER v5's stream, leaving v5's init untouched.
        self.tpo_qaux = args.tpo_qaux
        self.act_dim = act_dim
        qh = args.qhead_hidden
        self.q_head = nn.Sequential(
            layer_init(nn.Linear(H + act_dim, qh)),
            nn.Tanh(),
            layer_init(nn.Linear(qh, args.num_bins, bias=False), std=0.1),
        )
        with torch.no_grad():
    ...
    def z_to_action(self, z):
        # State-INDEPENDENT native-z -> env-action map (the SAME transform the rollout
        # dist's to_action applies). For gaussian it is tanh; for beta it is the fixed
        # linear rescale from (0,1) to [low, high]. Used by the Q-aux head to condition
        # on the env action of each stored candidate latent (bit-consistent with the
        # actions the probe scored). Independent of any trunk forward / RNG.
        if self.actor_dist == "gaussian":
            return torch.tanh(z)
        return self.action_low + (self.action_high - self.action_low) * z

    def get_q(self, features, actions):
        # Action-conditioned Q-head on the SHARED trunk.
        #   features (B, H): trunk output for the B states (the SAME critic_feat used
        #     by the value head this forward — threaded in to avoid recomputing the
        #     trunk; the Q-aux gradient therefore flows into the shared trunk via the
        #     critic branch).
        #   actions  (B, K, A): env actions of the K candidates per state.
        # Returns HL-Gauss logits (B, K, num_bins). Broadcasts the per-state feature
        # across the K candidate axis so each candidate conditions on the same state.
        B, K, A = actions.shape
        feat = features.unsqueeze(1).expand(B, K, features.shape[-1])  # (B, K, H)
        qin = torch.cat([feat, actions], dim=-1)                       # (B, K, H+A)
        return self.q_head(qin)                                        # (B, K, num_bins)

    def get_action_and_value(self, x, z=None, candidate_zs=None, return_dist=False, return_critic_feat=False):
        ...
        if return_critic_feat:
            # v7: thread the shared critic feature out so the Q-aux head can be
            # evaluated WITHOUT recomputing the trunk. Appended last; only requested
            # by the update loop when tpo_qaux is on, so off-path output is unchanged.
            out = out + (critic_feat,)
        return out
    ...
    def critic_parameters(self):
        # Params receiving the VALUE gradient (incl. the shared trunk). v7: the
        # action-conditioned Q-head joins the CRITIC group so its Q-aux gradient is
        # clipped under critic_grad_clip alongside V — NOT in actor_params, so the
        # Q-aux objective can never reach the actor heads.
        trunk = self.trunk if self.share_backbone else self.critic_trunk
        return list(trunk.parameters()) + list(self.critic_head.parameters()) + list(self.q_head.parameters())
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v7: keep the raw per-candidate one-step targets for the Q-aux head.
            # These are the DETACHED Q targets (return-scale; same support as V).
            # Built under no_grad here; .detach() in the update loop is belt-and-braces.
            b_tpo_scores = scores                                            # (B, K)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # SAME dist (no second trunk pass, consumes no RNG). v7: when the
                # Q-aux is on, also thread out the shared critic feature so get_q
                # reuses this forward's trunk output (no extra trunk pass).
                qaux_on = args.tpo_qaux and args.tpo_qaux_coef != 0.0
                if qaux_on:
                    _, _, newlogprob, entropy, value_logits, new_cand_logprobs, mb_critic_feat = (
                        agent.get_action_and_value(
                            b_obs[mb_inds], b_latent_zs[mb_inds], candidate_zs=b_tpo_zs[mb_inds],
                            return_critic_feat=True,
                        )
                    )
                else:
                ...
                # v7: action-conditioned Q-aux loss (CRITIC branch only). The Q-head
                # regresses (HL-Gauss CE) to the DETACHED one-step probe targets
                # scores_k = r_k + gamma*(1-term_k)*V_old(s'_k), conditioned on the env
                # ACTION of each stored candidate latent (candidate 0 = executed action;
                # SAME mb_inds + K ordering as b_tpo_scores => exact alignment). Reuses
                # this forward's shared critic feature (no extra trunk pass) and the
                # SAME hl_support projection as V (scores are return-scale). Detached
                # target => no gradient flows back through the target.
                if qaux_on:
                    mb_cand_actions = agent.z_to_action(b_tpo_zs[mb_inds])        # (mb, K, A)
                    q_logits = agent.get_q(mb_critic_feat, mb_cand_actions)       # (mb, K, num_bins)
                    mb_scores = b_tpo_scores[mb_inds].detach()                    # (mb, K)
                    q_target = hl_support.project(mb_scores.reshape(-1)).reshape(
                        q_logits.shape
                    ).detach()                                                   # (mb, K, num_bins)
                    q_log_probs = torch.log_softmax(q_logits, dim=-1)
                    qaux_loss = -(q_target * q_log_probs).sum(dim=-1).mean()      # over mb*K
                    with torch.no_grad():
                        # Diagnostic: how well does the Q-head's DECODED scalar track the
                        # oracle target (sets up a future probe-free Q-scoring variant)?
                        # Pearson r over the mb*K flattened pairs.
                        q_scalar = value_logits_to_scalar(q_logits).reshape(-1)   # (mb*K,)
                        s_flat = mb_scores.reshape(-1)
                        qm = q_scalar - q_scalar.mean()
                        sm = s_flat - s_flat.mean()
                        denom = (qm.pow(2).sum().sqrt() * sm.pow(2).sum().sqrt()).clamp_min(1e-12)
                        q_score_corr = ((qm * sm).sum() / denom).item()
                else:
                    qaux_loss = None
                ...
                # v7: fold the Q-aux into the CRITIC-weighted term. The Q-head lives in
                # critic_params, so this gradient is clipped under critic_grad_clip with
                # V and summed into the shared trunk's CRITIC branch — never the actor.
                critic_branch_loss = args.vf_coef * v_loss
                if qaux_on:
                    critic_branch_loss = critic_branch_loss + args.tpo_qaux_coef * qaux_loss
                ...
                if args.separate_grad_clip:
                    ...
                    if actor_active:
                        ...
                        critic_branch_loss.backward(retain_graph=True)
                        ...
                        # shared trunk receives only the critic branch (V + Q-aux),
                        # clipped to its own budget exactly as in the dual path.
                        critic_branch_loss.backward()
                        critic_gn = nn.utils.clip_grad_norm_(critic_params, args.critic_grad_clip)
                        actor_gn = 0.0
                    optimizer.step()
                else:
                    # Global (non-decoupled) clip: fold Q-aux into the single loss with
                    # the same critic-weighted term it gets in the dual path.
                    loss = critic_branch_loss
                    if actor_active:
        ...
        if args.tpo_qaux and args.tpo_qaux_coef != 0.0:
            # Last-minibatch Q-aux loss + how well Q tracks the sim-probe oracle.
            writer.add_scalar("losses/qaux_loss", qaux_loss.item(), global_step)
            writer.add_scalar("debug/q_score_corr", q_score_corr, global_step)
