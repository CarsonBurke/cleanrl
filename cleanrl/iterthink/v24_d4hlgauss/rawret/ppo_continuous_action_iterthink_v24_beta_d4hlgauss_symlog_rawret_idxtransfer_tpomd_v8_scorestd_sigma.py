# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return idx-transfer + TPO-MD v8
# (DYNAMIC trust region, v5) + SCORE-STD sigma (v8 fix for the late-training plateau).
#
# v8 CHANGE (tpo_sigma_scorestd, default True): the global score sigma is now
# coef * EMA(mean within-state candidate score std), NOT coef * EMA(executed-
# transition TD-residual RMS). DIAGNOSIS: the TD-RMS normalizer is keyed to the
# ABSOLUTE return scale, which grows ~5x as the policy climbs (sigma_global 1.15->5.9
# over 7M) while the within-state advantage spread it normalizes only grows ~2x
# (score_std 0.77->1.51). The denominator outruns the numerator -> u_scores shrink ->
# tpo_kl_base collapses to the cap -> the trust-region step degenerates to noise ->
# the run plateaus past ~6M (and v1's forced-KL dual DECLINES). Keying sigma to the
# within-state score std tracks the quantity actually normalized, holding u ~ O(1).
# Still ONE global scalar (batch mean), so cross-state magnitude survives. The only
# plateau-immune sibling, the PG-hybrid, bypasses this sigma entirely -> confirms the
# normalizer, not gradients/weights/critic-capacity (EV stays 0.95+), is the cause.
#
...
class Args:
    ...
    tpo_sigma_scorestd: bool = True    # v8: drive sigma_global from EMA(mean within-state candidate score std)
    #   instead of the executed-transition TD-residual RMS. TD-RMS is keyed to ABSOLUTE return scale,
    #   which grows ~5x as the policy climbs (1.15->5.9) and outruns the within-state advantage spread
    #   (~2x), self-suppressing u_scores -> kl_base collapses to the cap -> late-training plateau. The
    #   within-state score std tracks the quantity actually normalized, keeping u ~ O(1). Still ONE
    #   global scalar (preserves cross-state magnitude; NOT per-group z-scoring). False => exact v5.
...
if __name__ == "__main__":
    ...
    score_std_ema = None  # v8: EMA (decay 0.99) of the mean within-state candidate score std
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v5 default: sigma keyed to executed-transition TD-RMS (computed here for telemetry/v5 mode).
            # v8 overrides it below from the within-state score std once `scores` exist.
            ...
            group_std = scores.std(dim=-1, unbiased=False)                   # (B,) within-state spread
            # v8: drive the GLOBAL sigma from the within-state candidate score std (the quantity actually
            # being normalized), not the executed-transition TD-RMS (which tracks absolute return scale and
            # self-suppresses u as the policy climbs). Mean over the batch -> still ONE global scalar, so
            # cross-state advantage MAGNITUDE survives (a uniformly tiny-spread state stays tiny).
            if args.tpo_sigma_scorestd:
                score_std_now = group_std.mean().item()
                score_std_ema = (
                    score_std_now if score_std_ema is None else 0.99 * score_std_ema + 0.01 * score_std_now
                )
                tpo_sigma_global = max(args.tpo_sigma_scale_coef * score_std_ema, 1e-6)
