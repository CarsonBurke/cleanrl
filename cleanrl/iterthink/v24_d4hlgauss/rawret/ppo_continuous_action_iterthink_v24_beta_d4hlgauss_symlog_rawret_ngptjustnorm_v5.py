# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic nGPT-justnorm v5.
#
...
# target rather than v149's MTP critic.
#
# This sibling adds nGPT-style justnorm matrix projection and hypersphere residual
# updates to the IterThink backbone only. Linear input maps are row-normalized,
# branch/output maps are column-normalized, router gates and actor/critic output
# heads stay unconstrained, and branch residuals update unit features with learned
# per-channel eigen learning rates use the reference nGPT parameterization:
# parameters are stored at H^-0.5 and converted to an effective residual LR by
# abs(alpha * 0.05 / H^-0.5). v3 combined
# the dense and MoE branches into one normalized residual target so max-strength
# initialization did not let the later branch overwrite the earlier branch's
# gradient; v4 keeps that branch mix but uses the reference-small update scale.
# Observation handling remains the rawret env-normalized/clipped
# observation vector; we normalize the first observation-entry matrix, not the
# observation vector itself, to avoid discarding state magnitude.
#
...
def justnorm(x, dim=-1, eps=1e-12):
    """nGPT-style L2 projection, reduced in fp32 and cast back to input dtype."""
    dtype = x.dtype
    x = x.float()
    return (x / x.norm(p=2, dim=dim, keepdim=True).clamp_min(eps)).to(dtype)


NGPT_ALPHA_INIT_VALUE = 0.05


def ngpt_residual(x, branch, alpha, alpha_init_value, alpha_init_scaling):
    lr = (alpha * (alpha_init_value / alpha_init_scaling)).abs()
    lr = lr.to(dtype=x.dtype).view(*([1] * (x.ndim - 1)), x.shape[-1])
    x_u = justnorm(x)
    return justnorm(x_u + lr * (justnorm(branch) - x_u))
...
class ThinkBlock(nn.Module):
    ...
    def __init__(self, in_dim, H, n_experts):
        ...
        self.ngpt_input_linears = [self.in_proj]
        self.alpha_init_value = NGPT_ALPHA_INIT_VALUE
        self.alpha_init_scaling = H ** -0.5
        self.input_alpha = nn.Parameter(torch.full((H,), self.alpha_init_scaling))
        self.branch_alpha = nn.Parameter(torch.full((H,), self.alpha_init_scaling))

        # Dense branch. RMSNorm without learnable affine (no free per-channel γ).
        self.dense_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.dense = _branch_body(H)
        self.ngpt_input_linears.append(self.dense[0])
        self.ngpt_output_linears = [self.dense[2]]
        ...
        for expert in self.experts:
            self.ngpt_input_linears.append(expert[0])
            self.ngpt_output_linears.append(expert[2])

    def forward(self, cat_feats, x0):
        x_in = ngpt_residual(
            x0,
            self.in_proj(cat_feats),
            self.input_alpha,
        ...
        branch = justnorm(d_dense) + justnorm(d_moe)
        return ngpt_residual(
            x_in,
            branch,
            self.branch_alpha,
            self.alpha_init_value,
...
class ThinkTrunk(nn.Module):
    ...
    def forward(self, x):
        x0 = justnorm(self.entry(x))
        feats = [x0]
        for block in self.blocks:
            feats.append(block(torch.cat(feats, dim=-1), x0))
        return justnorm(self.out_proj(self.out_norm(torch.cat(feats, dim=-1))))

    @torch.no_grad()
    def normalize_ngpt_weights(self):
        """Project backbone matrices onto nGPT-style unit directions.

        Biases and scalar gates are excluded. The first observation-entry matrix
        is row-normalized like other input maps, which preserves the normalized
        observation vector's magnitude while bounding each hidden unit's direction.
        """
        self.entry.weight.copy_(justnorm(self.entry.weight, dim=1))
        for block in self.blocks:
            for module in block.ngpt_input_linears:
                module.weight.copy_(justnorm(module.weight, dim=1))
            for module in block.ngpt_output_linears:
                module.weight.copy_(justnorm(module.weight, dim=0))
...
class Agent(nn.Module):
    ...
    def normalize_ngpt_weights(self):
        if self.share_backbone:
            self.trunk.normalize_ngpt_weights()
        else:
            self.actor_trunk.normalize_ngpt_weights()
            self.critic_trunk.normalize_ngpt_weights()
