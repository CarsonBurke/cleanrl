# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic idx-transfer LatentMoE v1.
#
...
#   - Default compression=4 expands expert count 16->64 and active top-k 2->8,
...
class LatentIndexedTransferMoE(nn.Module):
    ...
    def __init__(self, H, history_dim, n_experts, compression, base_top_k, expert_multiplier):
        super().__init__()
        if compression < 1:
        ...
        self.expert_current_weight = nn.Parameter(torch.empty(self.n_experts, H, self.latent_dim))
        self.expert_current_bias = nn.Parameter(torch.zeros(self.n_experts, H))
        self.expert_out_weight = nn.Parameter(torch.empty(self.n_experts, self.latent_dim, H))
        self.expert_out_bias = nn.Parameter(torch.zeros(self.n_experts, self.latent_dim))
        self.expert_history_weight = nn.Parameter(torch.empty(self.n_experts, self.history_slots, H))
        for e in range(self.n_experts):
            nn.init.orthogonal_(self.expert_current_weight[e], np.sqrt(2))
            nn.init.orthogonal_(self.expert_out_weight[e], np.sqrt(2))
        nn.init.normal_(self.expert_history_weight, mean=0.0, std=np.sqrt(2.0 / (H + self.history_slots)))

    def _expert_bank(self, z, latent_history, expert_idx):
        current_weight = self.expert_current_weight[expert_idx]  # (B,K,H,L)
        current_bias = self.expert_current_bias[expert_idx]      # (B,K,H)
        out_weight = self.expert_out_weight[expert_idx]          # (B,K,L,H)
        out_bias = self.expert_out_bias[expert_idx]              # (B,K,L)
        history_weight = self.expert_history_weight[expert_idx]  # (B,K,S,H)

        z_sel = z.unsqueeze(1).expand(-1, self.top_k, -1)         # (B,K,L)
        preact = torch.einsum("bkl,bkhl->bkh", z_sel, current_weight) + current_bias
        ...
        expanded_history = expanded_history.unsqueeze(1)         # (B,1,S,H)
        ...
        return torch.einsum("bkh,bklh->bkl", hidden, out_weight) + out_bias
