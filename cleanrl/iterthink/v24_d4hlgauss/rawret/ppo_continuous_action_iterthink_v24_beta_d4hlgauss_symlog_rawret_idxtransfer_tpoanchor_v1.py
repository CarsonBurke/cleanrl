# PPO + IterThink v24 beta d4hlgauss symlog rawret idx-transfer + ANCHORED-TPO v1.
#
# TPO (../tpo: anchored target-policy optimization) integrated at PPO's native
# per-sample granularity, on top of the UNCHANGED idxtransfer_v1 PPO path
# (base run 9879 @ 8M). Why this shape: every prior TPO port (v92-v100)
# replaced the PPO surrogate and scored UNEXECUTED candidate actions (learned
# scorers, sim probes, one-step models) — all lost >6k to the base because
# counterfactual scores in dense MuJoCo are untrustworthy; AWR-style tilt-only
# actors (advweight_awr) failed the same way. So: executed transitions only,
# scored by the same trusted rankgauss-shaped GAE the PPO loss consumes.
#
# POINTWISE anchored target. TPO's mechanism is (1) a frozen KL-anchored
# target q(a|s) ∝ π_old(a|s)·exp(u/η) and (2) a gradient that self-
# extinguishes when the policy reaches it. A candidate-softmax over groups is
# only TPO's device for normalizing q on a sampled simplex; across unrelated
# states it would couple each sample's gradient to K-1 random neighbors
# (grouping noise, granularity loss). Instead, write the target per sample in
# log-ratio space:
#     logratio_i = logπ_θ(a_i|s_i) − logπ_old(a_i|s_i)  →  u_i/η − 1/(2η²)
# where u is the post-norm shaped advantage (rankgauss ≈ N(0,1)) and the
# constant is log E[exp(u/η)] under u~N(0,1) — the per-state normalizer that
# keeps the tilted density normalized in expectation (E_{π_old}[ratio] → 1).
# The loss is a frozen-target regression  L = ½·(logratio − t_i)², gradient
# (logratio − t_i): at rollout start (logratio = 0) it is the vanilla PG
# direction with per-sample magnitude |u/η − 1/(2η²)| — PPO-granular — and it
# decays linearly to ZERO as each sample realizes its own tilt. Anchored
# (t built from rollout u and logπ_old), self-extinguishing, multi-epoch
# stable, no clipping, no counterfactuals, no cross-sample coupling. Note the
# normalizer makes the median sample's target slightly NEGATIVE (−1/(2η²) at
# u=0): average actions cede a little log-mass to the upper tail, which is
# exactly what keeps E_{π_old}[ratio] → 1 under the tilt.
#
# η IS THE TRUST RADIUS, not a free temperature: q = argmax_r E_r[u] − η·KL,
# and under u~N(0,1) the anchored target costs KL(π_old‖q) = 1/(2η²). The
# paper's η=1 would demand per-iteration KL ≈ 0.5 — 15x this run's
# target_kl=0.03 leash — so the term would blow the KL budget and starve the
# PPO loss of its update epochs. Calibrate instead to the machinery's own
# trust radius: η=3.5 ⇒ KL(π_old‖q) ≈ 0.041 ≈ the leash, max tilt ratio
# e^{3.3/3.5} ≈ 2.6 for the top-ranked samples (vs the 1.28 clip ceiling).
#
# Hypothesis: PPO's clipped surrogate discards gradient once |ratio−1| > clip
# even when the rank signal is strong, and its unclipped branch never lets go
# (gradient persists at any ratio). The anchored term is complementary on
# both ends: self-limiting pressure that keeps extracting past the clip but
# STOPS at the target (and pulls BACK overshoot, which PPO never does),
# layered on the tuned clip/rankgauss/target-kl machinery at matched KL
# budget. tpo_coef=0 recovers the base exactly.
#
# RESULT (negative, 2026-06-12): all configurations underperformed and the line
# is closed. Pure mode (tpo_pure, η∈{1,3.5,5}, shaped and zscore): inert —
# returns 53-184 @ ~400k vs base ~1400 @ 500k; approx_kl 0.003-0.007, far
# below both leash and anchor radius; logratio moved only ~0.4x target rms.
# Root causes (verified against ../tpo): (1) pointwise targets embed GAE noise
# — the regression self-extinguishes at the conditional-mean projection, not
# the target (ref scores are deterministic); (2) η=3.5-5 sits past the ref's
# own η=4 degradation point (Appendix D basin is [0.25,2]); (3) the paper's
# ablations credit TPO's gains to within-context redistribution among K
# candidates of one input, which has no pointwise continuous analogue, and
# candidate-based ports need counterfactual scores (fatal: v92-v100). Aux mode
# (η∈{1,2,3.5}, coef 1) tracked the base at 1M but never beat it. Together
# with v92-v100 and advweight_awr: TPO does not transplant to dense-reward
# continuous control with stochastic credit.
#
# --- inherited idxtransfer_v1 notes ---
#
...
class Args:
    ...
    # Anchored-TPO auxiliary actor objective (see header): per-sample frozen-
    # target regression L = ½·(logratio − (u/η − 1/(2η²)))².
    #   tpo_coef: weight added to the actor loss (0 => exact base file).
    #   tpo_eta: KL radius of the anchored target — KL(π_old‖q) = 1/(2η²)
    #     for u ~ N(0,1) (rankgauss + norm_adv make this near-exact). Default
    #     3.5 matches the target's KL cost (0.041) to the run's target_kl
    #     leash (0.03); the paper's η=1 would cost KL ≈ 0.5 and starve the
    #     PPO epochs via early-stop.
    #   tpo_score: score source for the target's u.
    #     "shaped" (default): the post-norm shaped advantage the PPO loss
    #       consumes (rankgauss => u ~ N(0,1) exactly; the calibration above).
    #     "zscore": ref parity (../tpo Appendix A) — plain z-score of the RAW
    #       GAE advantage per minibatch, population std, zero-variance guard.
    #       Heavy-tailed: a tail sample can z-score to |u| >> 3.3 and gets an
    #       exp(u/η) tilt target rankgauss would have bounded. The PPO path
    #       still uses the shaped advantage; only the TPO target changes.
    #   tpo_pure: ref parity for the OBJECTIVE — the policy is trained by the
    #     anchored-TPO loss alone (pg_loss dropped from the actor backward, as
    #     in ../tpo where the CE fit is the sole policy objective). The critic,
    #     GAE, dual-clip machinery, and target_kl leash are unchanged: the
    #     critic is the score function here (the ref scores with raw task
    #     rewards, which dense MuJoCo control cannot), not a policy auxiliary.
    tpo_coef: float = 1.0
    tpo_eta: float = 3.5
    tpo_score: str = "shaped"
    tpo_pure: bool = False
...
if __name__ == "__main__":
    ...
    assert args.tpo_coef >= 0, "tpo_coef must be non-negative"
    assert args.tpo_score in ("shaped", "zscore")
    assert not (args.tpo_pure and args.tpo_coef == 0), "tpo_pure requires tpo_coef > 0"
    # The anchored target's normalizer 1/(2η²) and its KL calibration assume the
    # score u is ~N(0,1); rankgauss-family shaping + norm_adv guarantee that.
    # tpo_score="zscore" (ref parity) intentionally relaxes this: z-scored raw
    # GAE is standardized but heavy-tailed.
    assert args.tpo_coef == 0 or args.tpo_score == "zscore" or (
        args.norm_adv and args.adv_transform.startswith("rankgauss")
    ), "tpo_coef > 0 with tpo_score=shaped requires norm_adv and a rankgauss-family adv_transform"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Anchored-TPO target score, captured BEFORE pos_neg reweighting
                # (that is a PG loss weight, not a change to the score the
                # anchored target tilts on). "shaped" = the normalized shaped
                # advantage; "zscore" = ref parity (../tpo Appendix A): plain
                # z-score of the raw GAE per minibatch, population std. A
                # zero-variance minibatch falls back to u = 0, where the target
                # is the normalizer -1/(2eta^2), not exactly neutral (unreachable
                # in practice: needs 1024 identical GAE values).
                if args.tpo_score == "zscore":
                    raw_gae = b_advantages[mb_inds]
                    gae_std = raw_gae.std(unbiased=False)
                    tpo_u = (
                        (raw_gae - raw_gae.mean()) / gae_std
                        if gae_std > 1e-6
                        else torch.zeros_like(raw_gae)
                    )
                else:
                    tpo_u = mb_advantages
                ...
                # Anchored-TPO auxiliary loss (header): per-sample regression of
                # the log-ratio onto the frozen KL-anchored tilt target
                # t_i = u_i/eta − 1/(2·eta²). Gradient (logratio − t_i) starts as
                # the vanilla PG direction scaled |u|/eta and decays linearly to
                # zero as each sample realizes its own exp(u/eta) tilt — TPO's
                # self-extinguishing fit, at PPO's per-sample granularity. The
                # target is built from frozen rollout quantities (u, logπ_old),
                # so the term is stable across update epochs.
                if args.tpo_coef > 0:
                    tpo_target = (tpo_u / args.tpo_eta) - 1.0 / (2.0 * args.tpo_eta**2)
                    tpo_loss = 0.5 * (logratio - tpo_target).pow(2).mean()
                    with torch.no_grad():
                        tpo_gap = (logratio - tpo_target).abs().mean()
                else:
                    tpo_loss = torch.zeros((), device=device)
                    tpo_gap = torch.zeros((), device=device)
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    actor_loss = args.tpo_coef * tpo_loss - ent_coef_eff * entropy_loss
                    if not args.tpo_pure:
                        actor_loss = actor_loss + pg_loss
                    actor_loss.backward()
                    ...
                    loss = args.tpo_coef * tpo_loss - ent_coef_eff * entropy_loss + v_loss * args.vf_coef
                    if not args.tpo_pure:
                        loss = loss + pg_loss
                    optimizer.zero_grad()
                    loss.backward()
        ...
        writer.add_scalar("losses/tpo_loss", tpo_loss.item(), global_step)
        writer.add_scalar("debug/tpo_target_gap", tpo_gap.item(), global_step)
