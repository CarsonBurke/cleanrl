# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return idx-transfer + TPO-MD v6
# (WIDE-CAND proposal + DYNAMIC, signal-determined trust region; one-sided KL cap).
#
# v6 = v3 (WIDE-CANDIDATE PROPOSAL) + v5 (DYNAMIC TRUST REGION), two ORTHOGONAL
# changes on the shared tpomd_v1 base, combined. The two mechanisms target the SAME
# late-training plateau from opposite ends: WIDE CANDIDATES (A) break the plateau by
# letting the oracle probe discover better-than-on-policy actions once on-policy
# noise stops producing improving candidates; the DYNAMIC TRUST REGION (B) tames the
# variance that a broader, now-noisier candidate signal introduces by SHRINKING the
# step on its own whenever the signal is weak (instead of forcing a constant 0.03 KL
# that would amplify noise back up). A widens what we look at; B controls how hard we
# move given what we found. With tpo_cand_temp=1.0 AND tpo_dyn_trust=False, v6 is
# bit-identical to tpomd_v1 (proposal reuses roll_dist; solver uses the original
# fixed-KL-target dual). The two block-level changes touch disjoint code (candidate
# sampling in the rollout vs. the eta solver in the update), so they compose cleanly.
#
# --- CHANGE A (from tpomd_v3): WIDE-CANDIDATE PROPOSAL ---
# v3 WIDE-CANDIDATE PROPOSAL (over tpomd_v1): tpomd_v1 samples all K candidates
# ON-POLICY, so the cross-entropy target can only redistribute mass toward actions
# the current policy already samples — if every candidate is mediocre the target is
# mediocre (the M-step reweights on-policy noise). FIX: propose candidates 1..K-1
# from a BROADENED (higher-entropy) version of the rollout policy so the oracle
# probe can DISCOVER better-than-on-policy actions, while the ANCHOR p_old used to
# build the target stays the TRUE rollout policy. The proposal is widened but NOT
# the anchor: for the default Beta actor we shrink the excess concentration above
# the uniform prior by tpo_cand_temp (alpha'=1+(alpha-1)/t, beta'=1+(beta-1)/t,
# flattening toward Uniform; gaussian widens std by t). Candidate 0 is still the
# executed action with its true logprob. The within-group anchor logprobs and the
# at-update new-policy CE logprobs are ALL evaluated under the un-broadened policy,
# so q = softmax(anchor_logp + u/eta) reweights the true anchor over a wider
# candidate SET — coherent, just better-covered. tpo_cand_temp=1.0 -> bit-identical
# to tpomd_v1 (identity param transform, same probe RNG stream, anchor unchanged).
#
...
#
# --- CHANGE B (from tpomd_v5): DYNAMIC TRUST REGION (one-sided KL cap) ---
...
# OWN when signal is weak. (This matters MORE in v6: wide candidates make the
# candidate signal noisier, so a self-shrinking step is exactly the right governor.)
#
...
#
# Set tpo_dyn_trust=False to reproduce tpomd_v1's solver EXACTLY (the original
# fixed-KL-target bisection over [1e-2, 1e4]; eta_base unused).
#
...
class Args:
    ...
    tpo_cand_temp: float = 1.5   # PROPOSAL broadening factor for candidates 1..K-1 (1.0 == tpomd_v1, bit-identical).
                                 # >1 flattens the Beta/Normal used to DRAW candidates so the oracle probe can
                                 # discover better-than-on-policy actions; the ANCHOR (p_old) stays the TRUE rollout policy.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                # WIDE-CANDIDATE PROPOSAL (v3): draw candidates 1..K-1 from a BROADENED
                # (higher-entropy) version of roll_dist so the oracle probe can discover
                # better-than-on-policy actions. The ANCHOR p_old used to build the
                # target stays the TRUE rollout policy (roll_dist) — see cand_logprob
                # below. At tpo_cand_temp == 1.0 propose_dist's params equal roll_dist's
                # exactly, so sampling is bit-identical to tpomd_v1.
                if args.tpo_cand_temp == 1.0:
                    propose_dist = roll_dist
                elif args.actor_dist == "beta":
                    # Beta(alpha,beta) with alpha,beta>=1 (1+softplus). Higher entropy
                    # == SMALLER concentration. Scale the excess concentration above the
                    # uniform prior (conc-1) DOWN by the temperature: alpha' = 1+(alpha-1)/t,
                    # beta' = 1+(beta-1)/t. t>1 flattens toward Uniform(0,1); t=1 is identity.
                    a_b = 1.0 + (roll_dist.concentration1 - 1.0) / args.tpo_cand_temp
                    b_b = 1.0 + (roll_dist.concentration0 - 1.0) / args.tpo_cand_temp
                    propose_dist = Beta(a_b, b_b)
                else:
                    # gaussian: widen by scaling std (a higher-variance Normal).
                    propose_dist = Normal(roll_dist.loc, roll_dist.scale * args.tpo_cand_temp)
                ...
                with torch.random.fork_rng(devices=[device]):
                    ...
                    cand_z = propose_dist.sample(torch.Size([args.tpo_k]))  # (K, N, A)
                ...
                # ANCHOR logprobs: evaluated under the TRUE rollout policy (roll_dist),
                # NOT propose_dist. This is the key proposal-vs-anchor split — the target
                # q reweights the true within-group anchor by oracle scores; the candidate
                # SET just has wider coverage. Candidate 0's logprob is the executed
                # action's true logprob, unchanged.
