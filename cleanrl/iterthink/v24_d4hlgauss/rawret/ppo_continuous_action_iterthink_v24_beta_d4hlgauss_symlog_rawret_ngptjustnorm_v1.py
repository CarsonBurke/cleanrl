# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic nGPT-justnorm v1.
#
...
#
# This sibling adds nGPT-style justnorm matrix projection to the IterThink
# backbone only. Linear input maps are row-normalized, branch/output maps are
# column-normalized, and actor/critic output heads stay unconstrained so policy
# scale, log-variance/concentration scale, and the zeroed value prior remain free.
# Observation handling remains the rawret env-normalized/clipped observation
# vector; we normalize the first observation-entry matrix, not the observation
# vector itself, to avoid discarding state magnitude.
#
...
class ThinkBlock(nn.Module):
    ...
    def __init__(self, in_dim, H, n_experts):
        ...
        self.ngpt_input_linears.append(self.gate)
