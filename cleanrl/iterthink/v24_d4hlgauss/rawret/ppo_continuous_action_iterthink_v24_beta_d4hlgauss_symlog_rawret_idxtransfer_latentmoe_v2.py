# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic idx-transfer LatentMoE v2.
#
# v2 change: keep compression ratio 4 but restore more active/nonlinear routed
# capacity: total experts 16->64, active top-k=16, expert latent input/output
# width 16, and expert intermediate width 128. This follows the paper's useful
# distinction that latent compression should target expert input/output while
# preserving or expanding expert intermediate capacity.
#
# LatentMoE change vs the source run:
#   - Keep the raw-return idx-transfer PPO baseline exactly otherwise:
#     HalfCheetah-v4__iterthink_v24_beta_d4hlgauss_symlog_rawret_idxtransfer_v1_sameindex_sigma05_vf1_hc8m__1__1781250944.
#   - Replace each block's soft all-expert MoE branch with a LatentMoE branch
#     following arXiv:2601.18089v1: route from the original hidden state, project
#     selected-token computation into a lower shared latent dimension, run top-k
#     routed experts in that latent space, aggregate there, then project back.
#   - Default compression=4 expands expert count 16->64 and active top-k to 16,
#     using the paper's recommendation to spend the saved latent expert cost on
#     more expert diversity and more active combinations.
#
# Hypothesis: the current all-expert H-dimensional MoE overpays for broad dense
# averaging. Latent top-k experts should preserve the useful same-index transfer
# inductive bias while increasing specialization/combinatorial diversity per unit
# expert cost, improving HalfCheetah raw-return PPO performance.
#
...
#
# Base run: HalfCheetah-v4__iterthink_v24_beta_d4hlgauss_symlog_rawret_idxtransfer_v1_sameindex_sigma05_vf1_hc8m__1__1781250944.
#
...
class Args:
    ...
    k_blocks: int = 3
    n_experts: int = 16
    latent_moe_compression: int = 4
    latent_moe_base_top_k: int = 4
    latent_moe_expert_multiplier: int = 0  # <=0 uses latent_moe_compression
    latent_moe_intermediate: int = 128
...
class LatentIndexedTransferMoE(nn.Module):
    """LatentMoE branch with same-index transfer experts.

    Router logits are computed in the original hidden dimension. Expert inputs
    and same-index history are projected into a lower latent dimension before
    top-k dispatch. Experts use latent input/output but keep H intermediate
    perceptrons, matching LatentMoE's preserved nonlinear budget.
    """

    def __init__(self, H, history_dim, n_experts, compression, base_top_k, expert_multiplier, intermediate_dim):
        super().__init__()
        if compression < 1:
            raise ValueError(f"latent_moe_compression must be >= 1, got {compression}")
        if H % compression != 0:
            raise ValueError(f"hidden={H} must be divisible by latent_moe_compression={compression}")
        if history_dim % H != 0:
            raise ValueError(f"history_dim={history_dim} must be divisible by hidden={H}")
        multiplier = compression if expert_multiplier <= 0 else expert_multiplier
        self.H = H
        self.latent_dim = H // compression
        if intermediate_dim % self.latent_dim != 0:
            raise ValueError(
                f"latent_moe_intermediate={intermediate_dim} must be divisible by latent_dim={self.latent_dim}"
            )
        self.intermediate_dim = intermediate_dim
        self.history_slots = history_dim // H
        self.n_experts = n_experts * multiplier
        self.top_k = min(self.n_experts, max(1, base_top_k * compression))
        self.expand = intermediate_dim // self.latent_dim

        self.router = layer_init(nn.Linear(H, self.n_experts))
        self.down = layer_init(nn.Linear(H, self.latent_dim))
        self.up = layer_init(nn.Linear(self.latent_dim, H))
        self.expert_current_weight = nn.Parameter(torch.empty(self.n_experts, intermediate_dim, self.latent_dim))
        self.expert_current_bias = nn.Parameter(torch.zeros(self.n_experts, intermediate_dim))
        self.expert_out_weight = nn.Parameter(torch.empty(self.n_experts, self.latent_dim, intermediate_dim))
        self.expert_out_bias = nn.Parameter(torch.zeros(self.n_experts, self.latent_dim))
        self.expert_history_weight = nn.Parameter(torch.empty(self.n_experts, self.history_slots, intermediate_dim))
        for e in range(self.n_experts):
            nn.init.orthogonal_(self.expert_current_weight[e], np.sqrt(2))
            nn.init.orthogonal_(self.expert_out_weight[e], np.sqrt(2))
        nn.init.normal_(
            self.expert_history_weight,
            mean=0.0,
            std=np.sqrt(2.0 / (intermediate_dim + self.history_slots)),
        )

    def _expert_bank(self, z, latent_history, expert_idx):
        current_weight = self.expert_current_weight[expert_idx]  # (B,K,I,L)
        current_bias = self.expert_current_bias[expert_idx]      # (B,K,I)
        out_weight = self.expert_out_weight[expert_idx]          # (B,K,L,I)
        out_bias = self.expert_out_bias[expert_idx]              # (B,K,L)
        history_weight = self.expert_history_weight[expert_idx]  # (B,K,S,I)

        z_sel = z.unsqueeze(1).expand(-1, self.top_k, -1)         # (B,K,L)
        preact = torch.einsum("bkl,bkil->bki", z_sel, current_weight) + current_bias
        latent_history = latent_history.reshape(latent_history.shape[0], self.history_slots, self.latent_dim)
        expanded_history = latent_history.repeat_interleave(self.expand, dim=-1)
        expanded_history = expanded_history.unsqueeze(1)         # (B,1,S,I)
        transfer = (expanded_history * history_weight.to(dtype=expanded_history.dtype)).sum(dim=2)
        hidden = torch.relu(preact + transfer).pow(2)
        return torch.einsum("bki,bkli->bkl", hidden, out_weight) + out_bias

    def forward(self, router_input, expert_input, history):
        logits = self.router(router_input)
        z = self.down(expert_input)
        history = history.reshape(history.shape[0], self.history_slots, self.H)
        latent_history = self.down(history).reshape(history.shape[0], self.history_slots * self.latent_dim)

        topk_logits, topk_idx = torch.topk(logits, k=self.top_k, dim=-1)
        topk_weights = torch.softmax(topk_logits, dim=-1)
        expert_out = self._expert_bank(z, latent_history, topk_idx)
        mixed = (topk_weights.unsqueeze(-1) * expert_out).sum(dim=1)
        return self.up(mixed)


class ThinkBlock(nn.Module):
    """Bounded convex residual mix of x and x0, then parallel dense + LatentMoE."""
    ...
    def __init__(
        ...
        H,
        n_experts,
        latent_moe_compression,
        latent_moe_base_top_k,
        latent_moe_expert_multiplier,
        latent_moe_intermediate,
    ):
        super().__init__()
        ...
        # LatentMoE branch: full-H router, top-k latent experts, latent aggregation.
        self.moe_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.moe = LatentIndexedTransferMoE(
            H,
            in_dim,
            n_experts,
            latent_moe_compression,
    ...
    def forward(self, cat_feats, x0):
        ...
        d_moe = self.moe(m_in, m_in, cat_feats)                      # (B, H)
    ...
    def __init__(self, envs, args):
        ...
        if self.share_backbone:
            ...
            self.trunk = ThinkTrunk(
                ...
                args.k_blocks,
                args.n_experts,
                args.latent_moe_compression,
                args.latent_moe_base_top_k,
                args.latent_moe_expert_multiplier,
                args.latent_moe_intermediate,
            )
        else:
