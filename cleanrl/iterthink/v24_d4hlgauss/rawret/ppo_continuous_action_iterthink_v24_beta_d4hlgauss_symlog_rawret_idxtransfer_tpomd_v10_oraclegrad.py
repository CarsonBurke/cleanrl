# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return idx-transfer + TPO-MD v10
# (DYNAMIC trust region, v5) + ORACLE-GRADIENT CANDIDATES (break the candidate-quality ceiling).
#
# v10 HYPOTHESIS: v5's pure-TPO plateau (~10.5k from ~5M on) is a CANDIDATE-QUALITY ceiling
# (v8 proved sigma is redundant under the eta-solver; v5's KL step held ~0.03 through the
# plateau). The K candidates are i.i.d. rollout-policy samples, so as the policy concentrates
# they collapse to near-identical actions and the oracle has no better action to imitate.
# v9 attacked this with CEM (resample around elites) — but CEM only INTERPOLATES among the
# sampled actions. v10 goes further: each round it probes+scores the candidates, fits a LOCAL
# LINEAR surrogate of the oracle (score ~= b + g.(z - zbar)) by ridge least-squares, and steps
# the candidate cloud along the ascent direction g. The gradient EXTRAPOLATES beyond the
# sampled actions toward actions better than ANY sampled — a sharper ceiling attack, and it is
# differentiable-free / cold-start-free (the surrogate is refit per-state from the oracle, no
# learned Q to warm up). Anchor logprob stays under the TRUE rollout policy (q ∝ p_old·exp(u/eta),
# trust region intact); candidate 0 stays the executed action. ~2x probe per ascent round.
# tpo_grad_rounds=0 is bit-identical to v5.
#
...
class Args:
    ...
    # --- v9 CEM-guided candidate refinement: break the candidate-quality ceiling ---
    # Pure-TPO candidates are i.i.d. samples from the rollout policy, so as the policy
    # concentrates the K candidates collapse to near-identical actions -> the oracle has
    # no genuinely-better action to point at -> the ~10.5k plateau. CEM REFINES the
    # candidate set toward high-value regions: probe round-0 samples, score them with the
    # (pre-update) critic, refit a Gaussian in z-space to the top elites, resample, and
    # probe the refined set. The TPO anchor logprob stays under the TRUE rollout policy
    # (trust-region geometry intact); candidate 0 is always the executed action.
    # v10 ORACLE-GRADIENT ascent (replaces CEM resampling). Each round, fit a local LINEAR
    # surrogate score ~= b + g.(z - zbar) by ridge least-squares over the K probed (z, score)
    # pairs, then step the candidate cloud along the ascent direction g. Unlike CEM (which only
    # interpolates among sampled actions), the gradient EXTRAPOLATES beyond them toward actions
    # better than any sampled -> a sharper attack on the candidate-quality ceiling. Differentiable-
    # free and cold-start-free (no learned Q): the surrogate is refit per-state from the oracle.
    tpo_grad_rounds: int = 1         # ascent rounds AFTER the initial sample (0 => bit-identical to v5)
    tpo_grad_step: float = 1.0       # ascent step in units of the round's per-dim candidate std
    tpo_grad_spread: float = 0.5     # resample spread around the ascended center, in candidate-std units
    tpo_grad_ridge: float = 1e-3     # ridge on the (z^T z) normal-equations matrix (stabilizes the fit)
    tpo_grad_min_std: float = 0.05   # z-space candidate-std floor (avoid degenerate step/spread scale)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # v10: with tpo_grad_rounds>0 the candidate set is refined by oracle-gradient ascent
            # toward high-value regions before the final probe; tpo_grad_rounds=0 reproduces v5 exactly.
            probe_start = time.time()

            def _probe_block(cand_actions_np):
                # Bit-exact sim restore + one raw physics step per candidate. Returns
                # (next_obs (N,K,obs), rew (N,K), term (N,K)). Every candidate restores
                # qpos/qvel/qacc_warmstart/time, and each env is restored again afterwards,
                # so the REAL env.step below is identical to an unprobed run regardless of
                # how many times this runs per step. obs_rms is frozen (raw env never updates it).
                cand_actions_np = np.clip(cand_actions_np, probe_action_low, probe_action_high)
                nobs = np.zeros(
                    (args.num_envs, args.tpo_k) + envs.single_observation_space.shape, dtype=np.float32
                )
                nrew = np.zeros((args.num_envs, args.tpo_k), dtype=np.float32)
                nterm = np.zeros((args.num_envs, args.tpo_k), dtype=np.float32)
                for env_i in range(args.num_envs):
                    ...
                    for cand_i in range(args.tpo_k):
                        ...
                        nobs[env_i, cand_i] = np.clip(norm_obs, -10.0, 10.0)
                        nrew[env_i, cand_i] = probe_rew                       # RAW reward (base is raw-return)
                        nterm[env_i, cand_i] = float(probe_term)
                    base_env.data.qpos[:] = saved_qpos
                    base_env.data.qvel[:] = saved_qvel
                    base_env.data.qacc_warmstart[:] = saved_warm
                    base_env.data.time = saved_time
                return nobs, nrew, nterm

            act_shape = envs.single_action_space.shape
            with torch.no_grad():
                # Candidate sampling rides the PERSISTENT probe RNG stream inside fork_rng:
                # restore probe state, sample (+ any CEM resamples), save state back. The main
                # stream is untouched, so the executed trajectory stays bit-identical.
                with torch.random.fork_rng(devices=[device]):
                    ...
                    # --- oracle-gradient ascent: probe+score, fit a local linear surrogate by
                    #     ridge least-squares, step the candidate cloud along the ascent direction ---
                    A_dim = cand_z.shape[-1]
                    eye_A = torch.eye(A_dim, device=device).unsqueeze(0)   # (1, A, A)
                    for _grad_round in range(args.tpo_grad_rounds):
                        c_act = roll_to_action(cand_z).cpu().numpy()
                        c_obs, c_rew, c_term = _probe_block(c_act)
                        # score with the CURRENT critic (== the frozen pre-update critic used at
                        # update time: it does not change between rollout and update start).
                        c_obs_t = torch.as_tensor(
                            c_obs.reshape(-1, *envs.single_observation_space.shape), device=device
                        )
                        v_cand = torch.cat(
                            [value_logits_to_scalar(agent.get_value(ch)) for ch in c_obs_t.split(65536)]
                        ).reshape(args.num_envs, args.tpo_k)
                        g_scores = (
                            torch.as_tensor(c_rew, device=device)
                            + args.gamma * (1.0 - torch.as_tensor(c_term, device=device)) * v_cand
                        )                                                 # (N, K)
                        # Local linear surrogate score ~= b + g.(z - zbar); ridge normal equations
                        # g = (Zc^T Zc + lambda I)^-1 Zc^T sc, batched over envs.
                        z_bar = cand_z.mean(dim=1, keepdim=True)          # (N, 1, A)
                        Zc = cand_z - z_bar                               # (N, K, A) centered
                        sc = (g_scores - g_scores.mean(dim=1, keepdim=True)).unsqueeze(-1)  # (N, K, 1)
                        ZtZ = Zc.transpose(1, 2) @ Zc                     # (N, A, A)
                        ZtS = Zc.transpose(1, 2) @ sc                     # (N, A, 1)
                        g = torch.linalg.solve(ZtZ + args.tpo_grad_ridge * eye_A, ZtS).squeeze(-1)  # (N, A)
                        g_unit = g / (g.norm(dim=-1, keepdim=True) + 1e-8)
                        # Per-dim candidate scale sets the step/spread magnitude (keeps z-space units sane).
                        cand_std = cand_z.std(dim=1, unbiased=False).clamp_min(args.tpo_grad_min_std)  # (N, A)
                        z_center = z_bar.squeeze(1) + args.tpo_grad_step * cand_std * g_unit  # (N, A)
                        noise = torch.randn((args.num_envs, args.tpo_k) + act_shape, device=device)
                        cand_z = z_center.unsqueeze(1) + args.tpo_grad_spread * cand_std.unsqueeze(1) * noise
                        if args.actor_dist == "beta":
                            cand_z = cand_z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                        cand_z[:, 0] = z                                  # keep the executed action as candidate 0
                    probe_cpu_rng_state = torch.get_rng_state()
                    probe_cuda_rng_state = torch.cuda.get_rng_state(device)
                # Anchor logprob under the TRUE rollout policy (mirror-descent geometry is
                # preserved even for off-policy CEM candidates: q ∝ p_old · exp(u/eta)).
            ...
            # Final probe of the (possibly refined) candidate set -> the TPO target inputs.
            tpo_next_obs_np[step], tpo_rewards_np[step], tpo_terms_np[step] = _probe_block(cand_actions_np)
        ...
        with torch.no_grad():
            ...
            # v9 ceiling probe: does any candidate beat the executed action (candidate 0)?
            # If CEM works, cem_beat_frac stays high and cem_score_gain stays positive late,
            # where v5's on-policy candidates can no longer out-score what the policy already did.
            best_scores = scores.max(dim=-1).values
            cem_beat_frac = (scores[:, 1:].max(dim=-1).values > scores[:, 0]).float().mean().item()
            cem_score_gain = (best_scores - scores[:, 0]).mean().item()
        ...
        writer.add_scalar("debug/cem_beat_frac", cem_beat_frac, global_step)
        writer.add_scalar("debug/cem_score_gain", cem_score_gain, global_step)
