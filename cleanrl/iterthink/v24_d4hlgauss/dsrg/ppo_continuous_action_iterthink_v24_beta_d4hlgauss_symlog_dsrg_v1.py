# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v1.
#
...
class Args:
    ...
    gate_tau: float = 0.5            # CONSTANT gate temperature g=sigmoid(logit/tau) (no anneal)
    straight_through: bool = False   # hard {0,1} forward, soft backward (true binary routing)
    route_target_active: float = 0.15  # target fraction of open gates (route-balance penalty)
    lambda_route: float = 1e-2       # weight on (mean_active - target)^2
    lambda_gate_ent: float = 1e-3    # gate-entropy BONUS (prevents premature saturation)
    carry_bias_init: float = 1.0     # f-gate bias init (>0 => start near-identity highway)
    grad_checkpoint: bool = True     # checkpoint each tick to bound BPTT activation memory
...
class RoutedGraphCell(nn.Module):
    ...
    def __init__(self, n, d, src, args):
        ...
        # Routing: receiver query from state(+tick), learned per-source key.
    ...
    def forward(self, h, tick_t, Ksrc):
        ...
        logit = logit.transpose(0, 1) / (sqrt(self.dk) * self.tau)        # (B, N, K)
...
class DSRGTrunk(nn.Module):
    ...
    def __init__(self, obs_dim, H, args):
        ...
        self.cell = RoutedGraphCell(args.n_neurons, args.node_dim, src, args)
    ...
    def forward(self, x):
        ...
        ent_sum = x.new_zeros(())
        Ksrc = self.cell.key[self.cell.src]   # (N, K, dk) tick-invariant; gathered once
        for t in range(self.T):
            tick_t = self.tick_emb[t]
            if self.grad_checkpoint and self.training:
                h, active_t, ent_t = checkpoint(self.cell, h, tick_t, Ksrc, use_reentrant=False)
            else:
                h, active_t, ent_t = self.cell(h, tick_t, Ksrc)
            p_sum = p_sum + active_t
            ent_sum = ent_sum + ent_t

        active = p_sum / self.T
        gate_ent = ent_sum / self.T
        # route-balance penalty toward target open-fraction, minus an entropy bonus
        # (keeps gates from saturating all-open/all-closed early).
