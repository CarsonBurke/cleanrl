# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v11.
#
# WHAT CHANGES vs DSRG v7: the message NONLINEARITY only -- SiLU -> ReLU^2 (squared ReLU).
# This is the SAME activation swap as v10 (which applied it to v8's additive residual), but
# here on v7's LSTM-style gated carry `norm((1-f)*hhat + f*h)`. Run in parallel with v10 so
# the ReLU^2 win can be attributed independently of the carry type (additive vs gated). All
# else (competitive routing, obs-conditional readout, load-balance, triple-backward, the
# carry gate) is byte-for-byte v7. ReLU^2 = max(0,x)^2 zeros negatives exactly and is super-
# linear above 0 -- a sparser, harder activation matching the sparse top-m routing theme.
#
# --- v7 lineage note ---
# WHAT CHANGED vs DSRG v6: the READOUT becomes COMPETITIVE and obs-CONDITIONAL. v6 fixed
...
class RoutedGraphCell(nn.Module):
    ...
    def forward(self, h, tick_t, Ksrc):
        ...
        # v11: ReLU^2 (squared ReLU) instead of SiLU -- the same activation swap as v10,
        # but on v7's LSTM-style gated carry rather than v8's additive residual, to test
        # whether the harder/sparser activation helps independently of the carry type.
        pre = m + tick_t
