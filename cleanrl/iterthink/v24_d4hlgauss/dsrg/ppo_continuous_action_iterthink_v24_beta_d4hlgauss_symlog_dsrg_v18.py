# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v18.
#
# WHAT CHANGES vs v15: NOTHING in the graph. v18 is a pure CONFIG experiment on the SAME
# DSRG trunk. Motivation: ~17 architecturally-diverse DSRG variants all plateaued in a tight
# 6900-7400 band. A plateau invariant to wildly different trunks means the binding constraint
# is something held FIXED across all of them -- the OPTIMIZATION config, not the representation.
# Three throttles identified (diagnostics: critic EV ~0.95 but clipfrac 0.30 vs 0.11 baseline,
# KL hitting its cap after ~1-2 of 10 epochs = the policy was leashed):
#   (1) target_kl 0.03 -> None  (was early-stopping ~3/4 of the update epochs)
#   (2) actor/critic grad-clip 0.25 -> 0.5  (0.25 under-moved the policy per minibatch)
#   (3) adv_transform rankgauss -> none  (rankgauss flattened advantage MAGNITUDE)
# HYPOTHESIS: removing the policy-step throttle lifts the SAME graph off the 7000 plateau.
# This is the attribution control for the simba_v1 MLP run (does config-fix alone explain the
# gap, or does capacity/width+norm also matter?).
#
...
class Args:
    ...
    target_kl: Optional[float] = None  # v18: KL early-stop DISABLED (was 0.03 -- it cut effective epochs to ~1/4; diagnostics showed clipfrac 0.30 vs 0.11 baseline = the policy was leashed)
    ...
    actor_grad_clip: float = 0.5     # v18: loosened 0.25 -> 0.5 (the 0.25 leash under-moved the policy; 0.5 matches the standard global PPO clip)
    critic_grad_clip: float = 0.5    # v18: loosened 0.25 -> 0.5
    ...
    # Advantage shaping. v18: DISABLED (was rankgauss) -- rank-Gaussianizing flattens
    # advantage MAGNITUDE, removing the signal that large positive advantages should
    # drive large policy updates. "none" = raw GAE; plain per-minibatch norm_adv still applies.
