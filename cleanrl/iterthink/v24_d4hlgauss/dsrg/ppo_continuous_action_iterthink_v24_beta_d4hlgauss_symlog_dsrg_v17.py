# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v17.
#
# WHAT CHANGES vs DSRG v14: the direct obs->actor skip becomes NONLINEAR -- a 2-layer MLP
# (obs -> SiLU -> H) instead of v14's single Linear. v14 (the linear skip) was the first
# variant to beat v8 at every mature checkpoint (4755/6077/7343 vs 4500/5683/6931, +6% @8M),
# establishing that a direct obs->action path is the actor's missing lever. v17 asks whether
# that path benefits from its OWN nonlinear capacity: products/thresholds of joint angles &
# velocities that a single Linear cannot form. The output layer keeps v14's small orthogonal
# init (gain 0.1) so the skip still starts as a gentle ~10% contribution and anneals up only
# as it earns return -- the graph trunk still drives the actor early. Clean single-variable
# test of linear-vs-nonlinear skip on the proven v14 lever. Critic untouched. Everything
# else is v8.
#
# --- v14 obs->actor lineage note ---
# WHAT CHANGED vs DSRG v8: a DIRECT obs->actor-feature residual (a single linear skip).
# DIAGNOSIS: across v6-v13 the critic is strong (explained-variance ~0.85+) but returns sit
# well under the dense-MLP baseline, and the v12/v13 conditionality experiments confirmed
# the gap is on the ACTOR, not the graph (more conditional routing churned the actor's
# features and HURT return while critic EV stayed fine -- conditionality is NOT the lever).
# The actor's only obs->action path is obs -> sparse graph (T ticks) -> sparse 64-neuron
# obs-gated readout -> H=64 -> Beta heads: lossy, and its features shift as the graph state
# evolves. The dense baseline's edge is a DIRECT, high-capacity obs->action path. v14 gives
# the actor that: actor_feat += Linear(obs) (orthogonal init gain 0.1 -> ~10% contribution
# at start, grows as it earns return = a built-in clipfrac-safe anneal). CRITIC is left
# untouched (already excellent -> clean attribution). Everything else is v8.
#
# --- v8 lineage note ---
...
# carry path. (v7 and v8 tracked closely; v7 ~5% ahead late. ReLU^2 vs SiLU was a wash.)
#
# --- v7 lineage note ---
...
class Args:
    ...
    obs_actor_std: float = 0.1       # v17: orthogonal-init gain of the OUTPUT layer of the nonlinear obs->actor MLP skip
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v17: NONLINEAR obs->actor-feature residual (vs v14's linear skip). A 2-layer MLP
        # obs -> SiLU -> H gives the direct obs->action path its own nonlinear capacity to
        # extract action-relevant structure (e.g. products/thresholds of joint angles &
        # velocities) that a single Linear cannot. The OUTPUT layer keeps the small
        # orthogonal init (gain obs_actor_std) so the skip still starts as a gentle ~10%
        # contribution and grows only as it earns return -- same anneal as v14, so the
        # graph trunk still drives the actor early. First layer gets standard init.
        self.obs_actor = nn.Sequential(
            layer_init(nn.Linear(obs_dim, H), std=np.sqrt(2)),
            nn.SiLU(),
            layer_init(nn.Linear(H, H), std=args.obs_actor_std),
        )
    ...
    def _trunks(self, x):
        ...
        if self.share_backbone:
            pooled = self.trunk(x)
            actor_feat, critic_feat = pooled[:, 0], pooled[:, 1]
        else:
            actor_feat, critic_feat = self.actor_trunk(x)[:, 0], self.critic_trunk(x)[:, 1]
        # v14: DIRECT obs->actor-feature residual. The actor's only obs->action path was
        # obs -> sparse graph -> sparse 64-neuron readout -> H=64 -> Beta heads -- a lossy,
        # churny route, and diagnostics put the gap to the dense baseline on the ACTOR (the
        # critic EV is already high). A plain linear obs->H skip gives the policy direct,
        # low-variance access to the raw observation (joint angles/velocities, which for
        # locomotion largely determine good actions) without laundering it through the
        # graph. Small orthogonal init => starts as a ~10% contribution and grows only as
        # it earns return (a built-in anneal -> bounded clipfrac risk). Actor ONLY: the
        # critic is already excellent, so it is left untouched for clean attribution.
        actor_feat = actor_feat + self.obs_actor(x)
    ...
    def actor_parameters(self):
        ...
        # v14: the direct obs->actor residual is an ACTOR param -> include it so its gradient
        # is part of the decoupled actor grad-clip (else it would step UN-clipped).
        return list(trunk.parameters()) + heads + list(self.obs_actor.parameters())
