# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v13.
#
# WHAT CHANGES vs DSRG v12: + QK-NORM on the routing gate. The gate logit is q.key/sqrt(dk):
# in v12 neither q (=q_proj(h+tick)) nor key was normalized, so the dot-product magnitude --
# which sets the softmax competition SHARPNESS -- drifted with the learned q_proj/key/tick
# scales and was uncontrolled. That bites hardest with route_bias=0 (v12), where routing is
# ENTIRELY q.key: at init the logit std is only ~0.25 -> a near-flat softmax over the K=16
# candidates -> top-m selection decided by noise (near-random, weak conditional signal).
# v13 RMS-normalizes q and key (over dk) BEFORE their dot product, pinning the logit scale
# to ~unit so tau is the sole, clean control of competition sharpness. This is the proper
# (un-confounded) test of whether conditional routing helps: v12 isolates route_bias=0, v13
# adds the scale control that route_bias=0 needs to produce a meaningful competition. QK-norm
# has no affine -> it controls SCALE only, forces no particular routing (still emergent).
#
# --- v12 lineage note ---
# WHAT CHANGED vs DSRG v8: route_bias init 0.5 -> 0 (and a per-edge release-probability
# metric). DIAGNOSIS: across v6-v11 the routing was nearly input-INDEPENDENT -- measured
# conditionality ~0.007, diversity ~0.05 (the active circuit is ~95% identical for every
# observation), i.e. DSRG was running as a STATIC sparse net, the likely reason it trails
# the dense baseline by ~30% despite the conditional-computation machinery. MECHANISM: the
# routing logit is `q.key/sqrt(dk) + route_bias`; route_bias was seeded at std 0.5 while the
# input-dependent q.key term is only ~0.25, so the STATIC per-edge bias dominated from step
# 0 and the optimizer never needed the state-dependent path. FIX (emergent, NOT forced): we
# do NOT penalize route_bias toward 0 (that would just engineer the result, the anti-pattern
# v6 removed). We init it to ZERO so it no longer pre-empts q.key; symmetry is broken by the
# random per-source key dotted with the state query (each edge gets a distinct affinity at
# step 0). route_bias stays learnable -- static per-edge structure can re-emerge if useful,
# but conditional, state-driven routing is now free to win. Everything else is v8.
#
# Pr METRIC (biological release-probability analog): per edge, Pr = fraction of inputs it is
# selected into the active top-m. A STATIC graph has every edge at Pr~0/1 (always the same
# choice); a CONDITIONAL graph (like biology's unreliable synapses) has edges at INTERMEDIATE
# Pr that switch with input. Logged as circuit/pr_mean, pr_cond_frac, pr_entropy.
#
# --- v8 lineage note ---
# v8 made the per-tick CARRY a PURE ADDITIVE RESIDUAL `norm(h + hhat)` (vs v7's LSTM-style
# gated carry `norm((1-f)*hhat + f*h)`): an unconditional identity highway, no gate that can
# saturate to zero out the update/carry path. (v7 and v8 tracked closely; v7 ~5% ahead late.)
#
# --- v7 lineage note ---
...
class Args:
    ...
    route_grad_clip: float = 0.1     # max-norm for the DECOUPLED route-structural gradient (routing params only)
    ...
    route_bias_init: float = 0.0     # v12: ZERO init (was 0.5). The static per-edge bias no longer
                                     # starts dominant, so it can't pre-empt the input-dependent q.key
                                     # path -- conditional routing is free to EMERGE (or not) from the
                                     # task gradient. route_bias stays learnable, so static per-edge
                                     # structure can also re-emerge if useful; nothing is forced.
                                     # Symmetry is broken by the random per-source key . state-query.
...
class RoutedGraphCell(nn.Module):
    ...
    def __init__(self, n, d, src, args):
        ...
        # Routing: receiver query from state(+tick) dotted with a learned per-source key,
        # PLUS a learnable per-EDGE bias. v12 init's route_bias to ZERO (was std 0.5): the
        # symmetry breaker is now the RANDOM per-source key dotted with the (state-derived)
        # query -- each candidate edge gets a distinct q.key affinity at step 0, so gates
        # start differentiated WITHOUT a static per-edge constant pre-empting them. Earlier
        # versions seeded a large route_bias so it carried routing from step 0, which is
        # exactly why routing stayed input-INDEPENDENT (the static knob dominated the q.key
        # term). route_bias remains learnable, so static per-edge structure can re-emerge
        # if the task rewards it -- but conditional, state-driven routing is now free to win.
        ...
        # v13: QK-norm. RMS-normalize the routing query and key (over dk) BEFORE their dot
        # product, so the gate logit scale -- hence the softmax competition sharpness -- is
        # PINNED (~unit) instead of drifting with the learned q_proj/key/tick magnitudes.
        # Matters most with route_bias=0 (v12): routing is then ENTIRELY q.key, and without
        # this the init logit std is only ~0.25 (near-flat softmax -> near-random top-m).
        # tau becomes the sole, clean control of sharpness. No affine -> no forced routing.
        self.qk_norm = nn.RMSNorm(self.dk, elementwise_affine=False)
    ...
    def gate_probs(self, h, tick_t, Ksrc):
        ...
        q = self.qk_norm(self.q_proj(h + tick_t))                # v13: QK-norm (mirrors forward)
        k = self.qk_norm(Ksrc)
        logit = torch.bmm(q.transpose(0, 1), k.transpose(1, 2)).transpose(0, 1) / sqrt(self.dk)
    ...
    def forward(self, h, tick_t, Ksrc):
        ...
        q = self.qk_norm(self.q_proj(h + tick_t))                         # (B, N, dk) QK-normed query
        k = self.qk_norm(Ksrc)                                            # (N, K, dk) QK-normed keys
        logit = torch.bmm(q.transpose(0, 1), k.transpose(1, 2))           # (N, B, K)
...
class DSRGTrunk(nn.Module):
    """obs -> input-neuron states -> T weight-tied routed ticks -> PMA readout -> (actor_feat, critic_feat)."""
    ...
    def __init__(self, obs_dim, H, args):
        ...
        self.lambda_balance = args.lambda_balance               # v6: Switch load-balance weight
        self.active_frac = min(args.route_top_m, args.fan_in) / args.fan_in  # structural m/K
        ...
        # Pathway readout: 2 output nodes (actor, critic) each read K_read fixed source
        # perceptrons via gated pathways -> (K_read*d -> H). Fan-in-K sparse readout
        # consistent with the graph; replaces the v1-v3 flatten and the v4 PMA.
        self.readout = PathwayReadout(self.n, self.d, args.n_read, H, obs_dim, n_out=2,
                                      m=(args.read_m_actor, args.read_m_critic),
                                      dk=args.read_dk, tau=args.read_tau, seed=args.seed)
    ...
    def forward(self, x):
        ...
        lb_sum = x.new_zeros(())         # accumulate load-balance / competition entropy over ticks
        ent_sum = x.new_zeros(())
        for t in range(self.T):
            ...
            if self.grad_checkpoint and self.training:
                h, lb_t, ent_t = checkpoint(cell, h, tick_t, Ksrc, use_reentrant=False)
            else:
                h, lb_t, ent_t = cell(h, tick_t, Ksrc)
            lb_sum = lb_sum + lb_t
            ent_sum = ent_sum + ent_t

        lb = lb_sum / self.T
        route_ent = ent_sum / self.T
        # v6 route_loss = ONLY the Switch/GShard load-balance aux (uniform source usage).
        # Sparsity is structural (top-m), crispness is competition -> no open-mean budget
        # and no entropy penalty. Task gradient reaches the routing params via the actor
        # backward; this aux (decoupled-clipped) just keeps the population evenly used.
        # v12 does NOT add any conditionality penalty -- conditional routing is left to
        # EMERGE from the q.key path (see route_bias_init=0 in the cell): no forced target.
        self.route_loss = self.lambda_balance * lb
        self._active = self.active_frac       # structural m/K (constant)
        self._gate_ent = route_ent.detach()   # competition entropy gauge (logged as route_gate_entropy)
        self._lb = lb.detach()                # load-balance gauge

        pooled = self.readout(h, x)     # (B, 2, H): [:,0]=actor feat, [:,1]=critic feat (obs-conditional)
        return pooled

    @torch.no_grad()
    def probe_gates(self, x):
        # Re-run the T-tick rollout eagerly, capturing the per-edge gate probs each
        # tick -> (B, T, N, K). Used (periodically, off the hot path) for circuit
        # instrumentation. Returns the gates and the (N, K) source map for one cell
        # (topology is shared across ticks).
        B = x.shape[0]
        h = self.h0.unsqueeze(0).expand(B, -1, -1).contiguous()
        h[:, :self.n_input] = self.encoder(x).view(B, self.n_input, self.d)
        gates = []
        for t in range(self.T):
            cell = self.cells[t]
            tick_t = self.tick_emb[t]
            Ksrc = cell.key[cell.src]
            gates.append(cell.gate_probs(h, tick_t, Ksrc))    # (B, N, K)
            h, _, _ = cell(h, tick_t, Ksrc)
        return torch.stack(gates, dim=1), self.cells[0].src   # (B, T, N, K), (N, K)
...
def circuit_metrics(gates, src, n):
    ...
    gates: (B, T, N, K) gate probs; src: (N, K) source-perceptron idx; n: #perceptrons.
      ...
      pr_*           : v12 SYNAPTIC RELEASE-PROBABILITY analog. Per edge, Pr = fraction of
                       inputs (x ticks) it is selected into the active top-m set -- the
                       direct analog of a biological synapse's release probability (how
                       often it transmits given a presynaptic spike). The DISTRIBUTION is
                       the tell: a STATIC graph has every edge at Pr~0 or Pr~1 (each
                       receiver always picks the same candidates regardless of input); a
                       CONDITIONAL graph (like biology's unreliable synapses) has many
                       edges at INTERMEDIATE Pr that switch on/off with the input.
                         pr_mean     : mean Pr over edges (~ top_m/K = structural sanity).
                         pr_cond_frac: fraction of edges with 0.05<Pr<0.95 (genuinely
                                       input-switched edges; ~0 => static routing).
                         pr_entropy  : mean per-edge Bernoulli entropy H(Pr) (smooth
                                       version of cond_frac; 0 => deterministic edges)."""
    B, T, N, K = gates.shape
    ...
    # v12: per-edge release probability Pr (selection frequency over batch x ticks)
    Pr = m.mean(dim=(0, 1))                                   # (N, K) in [0,1]
    pr_mean = Pr.mean().item()
    pr_cond_frac = ((Pr > 0.05) & (Pr < 0.95)).float().mean().item()
    p = Pr.clamp(1e-6, 1 - 1e-6)
    pr_entropy = (-(p * p.log() + (1 - p) * (1 - p).log())).mean().item()
    return dict(conditionality=conditionality, tick_dynamism=tick_dynamism,
                ...
                pr_mean=pr_mean, pr_cond_frac=pr_cond_frac, pr_entropy=pr_entropy)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        cm = circuit_metrics(*route_trunk.probe_gates(b_obs[:256]), args.n_neurons)
