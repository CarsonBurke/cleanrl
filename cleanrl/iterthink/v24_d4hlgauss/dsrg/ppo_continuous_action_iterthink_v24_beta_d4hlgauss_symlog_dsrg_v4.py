# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v4.
#
# WHAT CHANGES vs DSRG v3 (diagnosed across the v1->v3 sweep on HC-8M: the per-tick
...
class Args:
    ...
    # --- PMA readout (Set-Transformer Pooling by Multihead Attention) ---
    readout_dim: int = 64            # d_model the perceptron states are projected to for the pool (== hidden H)
    readout_heads: int = 4           # MHA heads in the pool (readout_dim must divide by this)
    # n_seeds is FIXED at 2: seed 0 -> actor feature, seed 1 -> critic feature.
...
class PMAReadout(nn.Module):
    """Set-Transformer Pooling by Multihead Attention. k learned SEED queries
    multihead-attend over the N perceptron states (keys/values), returning k pooled
    H-vectors. Here k=2: seed 0 -> actor feature, seed 1 -> critic feature, so one
    shared graph trunk yields DECOUPLED actor/critic readouts. The attention softmax
    over N is a low-variance, permutation-invariant soft-selection of perceptrons --
    no active-subset masking (DSRG updates all N densely; sparsity is in the edges).

        X = proj(h)                       # (B, N, H)
        A = MultiheadAttn(S, X, X)        # (B, k, H), S = learned seeds (queries)
        Hh = LN(S + A);  out = LN(Hh + FF(Hh))   # Set-Transformer MAB block
    """

    def __init__(self, d_node, d_model, n_heads, n_seeds=2):
        super().__init__()
        self.inp = layer_init(nn.Linear(d_node, d_model), std=1.0)
        self.seeds = nn.Parameter(torch.randn(n_seeds, d_model) / sqrt(d_model))
        self.attn = nn.MultiheadAttention(d_model, n_heads, batch_first=True)
        self.ln0 = nn.LayerNorm(d_model)
        self.ff = nn.Sequential(
            layer_init(nn.Linear(d_model, d_model), std=sqrt(2)),
            nn.GELU(),
            layer_init(nn.Linear(d_model, d_model), std=1.0),
        )
        self.ln1 = nn.LayerNorm(d_model)

    def forward(self, h):                          # h: (B, N, d_node)
        x = self.inp(h)                            # (B, N, d_model)
        s = self.seeds.unsqueeze(0).expand(x.shape[0], -1, -1)   # (B, k, d_model)
        a, _ = self.attn(s, x, x, need_weights=False)            # (B, k, d_model)
        hh = self.ln0(s + a)
        out = self.ln1(hh + self.ff(hh))           # (B, k, d_model)
        return out                                  # split by seed outside
...
class DSRGTrunk(nn.Module):
    ...
    def __init__(self, obs_dim, H, args):
        ...
        # PMA readout: 2 seed queries pool the N perceptron states -> (actor, critic)
        # features. Replaces the v1-v3 flatten(N*d)->Linear, which was a high-variance
        # policy feature and forced actor/critic to share one readout.
        self.readout = PMAReadout(self.d, args.readout_dim, args.readout_heads, n_seeds=2)
        assert args.readout_dim == H, "PMA readout_dim must equal hidden H (head feature width)"
