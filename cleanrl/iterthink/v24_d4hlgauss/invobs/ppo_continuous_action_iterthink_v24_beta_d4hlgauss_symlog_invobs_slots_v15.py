# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted-attention slot transformer v15.
#
# WHY v15 (ReLU^2 SLOT FFN, GAIN-AFTER-SQUARE, ABLATION OVER v14). Identical to v14
# (squared-ReLU FFN, matched params, no gating) EXCEPT the learned per-channel gain `su`
# is moved to AFTER the square: v14 computes relu(su*h)**2 (== su^2 * relu(h)**2), so the
# H^0.5 (=8x) gain enters QUADRATICALLY -> ~64x preactivation-variance blowup before the
# downstream justnorm/eigen-lr, reshaping the update direction to be spiky/low-entropy
# even though justnorm rescales magnitude. v15 computes su * relu(h)**2 -- the gain is now
# LINEAR, matching the nGPT variance calibration that `su`/`alpha` init (1/sqrt(H)) assumes
# for the SwiGLU recipe. HYPOTHESIS: gain-before-square is a recipe-miscalibration confound;
# moving it after the square recovers squared-ReLU toward parity with SwiGLU (v11) -- which
# is what Primer (2109.08668) predicts for a NON-causal trunk like ours (its squared-ReLU
# beats SwiGLU only for decoder-only AR-LM; for encoders it merely matches). Width UNCHANGED
# (no wider d_ff). The only delta vs v14 is the placement of the gain. Base: v14.
#
# WHY v14 (ReLU^2 SLOT FFN, ABLATION OVER v12). Replaces the slot-block FFN nonlinearity:
# nGPT SwiGLU (u * silu(v)) -> squared-ReLU (relu(h)**2), at MATCHED parameter count.
# The single-gate SwiGLU's 2*mult*H up-projection is swapped for a plain up-projection to
# hidden d = int(1.5*mult*H) (=288 at mult=3, H=64), so 128*d == 3*mult*H^2 == the SwiGLU
# FFN's param count exactly. The nGPT-style learned per-channel pre-activation gain is
# kept (now `su` over the un-gated hidden), so the on-sphere scaling philosophy is
# unchanged -- the ONLY real change is the nonlinearity+gating (gated SiLU vs squared
# ReLU). Tests whether gating matters for this on-sphere proprioceptive trunk. Base: v12.
#
...
class SlotMLP(nn.Module):
    """nGPT ReLU^2 FFN (v15, GAIN-AFTER-SQUARE). A single up-projection to hidden
    d = int(1.5*mult*H), the squared-ReLU nonlinearity relu(h)**2, THEN scaled by a learned
    per-channel `su` gain (init 1). Unlike v14 -- which applied the gain BEFORE the square
    (relu(su*h)**2 == su^2 * relu(h)**2, squaring the H^0.5 gain) -- v15 applies it AFTER,
    so the gain stays LINEAR (su * relu(h)**2). No gating, no bias. Hidden width is chosen so
    the FFN param count (128*d == 3*mult*H^2) matches the v12 SwiGLU FFN exactly. Weights are
    sphere-normalized after each optimizer step (block.mlp.fc/proj.weight)."""

    def __init__(self, H, mult, base_scale):
        super().__init__()
        self.H = H
        d = int(1.5 * mult * H)            # matched params to SwiGLU (128*d == 3*mult*H^2)
        self.fc = nn.Linear(H, d, bias=False)
        self.proj = nn.Linear(d, H, bias=False)
        self.su_init_value = 1.0
        self.su_init_scaling = 1.0
        self.su = nn.Parameter(self.su_init_scaling * torch.ones(d))

    def forward(self, x):
        h = self.fc(x)
        su = self.su * ((self.su_init_value / self.su_init_scaling) * (self.H ** 0.5))
        return self.proj(su * F.relu(h) ** 2)   # gain AFTER square -> linear, not squared
