# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted obs slot transformer v4.
#
...
# invariance. Actor and critic use separate 4-slot, 4-block inverted-attention stacks. Every attention
# layer is paper-aligned inverted cross-attention: tokens assign mass across slots
# (softmax over slots), then slots renormalize over tokens. No GRU, no temporal
# recurrence, no normal self-attention/PMA.
#
...
# weights. This is not Parameter Golf's causal self-attention dataflow: skips and
# residual anchors operate on slot states, while observations enter through inverted
# cross-attention. Environment-side observation normalization supplies the input
# scale; attention still uses prenormalized Q/K/V inputs. The PPO actor/critic heads
# consume the full flattened slot state (4*64=256).
#
...
class InvertedCrossAttention(nn.Module):
    ...
    def __init__(self, H, heads, qk_gain_init):
        super().__init__()
        ...
        self.proj = zero_init_linear(nn.Linear(H, H, bias=False))
...
class InvertedSlotBlock(nn.Module):
    def __init__(self, H, heads, mlp_mult, qk_gain_init, attn_scale_init, mlp_scale_init):
        ...
        self.attn = InvertedCrossAttention(H, heads, qk_gain_init)
...
class InvertedSlotStack(nn.Module):
    def __init__(self, n_slots, H, n_blocks, n_heads, mlp_mult, qk_gain_init, attn_scale_init, mlp_scale_init):
        super().__init__()
        assert n_blocks >= 1
