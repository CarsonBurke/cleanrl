# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted-attention slot transformer v21.
#
# WHY v21 (vs v20): WIDER ROUTING BOTTLENECK, slot_d_qk 8 -> 16. v20's per-action design
# requires each action's slots to ROUTE to that muscle's obs tokens (its joint angle+vel)
# out of 17 tokens. Routing happens in the low-rank d_qk space. At d_qk=8 the Welch coherence
# bound for 17 unit keys is 0.265 (~half-overcomplete, tight) -- routing is feasible but
# learning-limited and crowded at init (random-init max key cos ~0.82; some muscle pairs 0.90).
# d_qk=16 drops the bound to 0.06 (17 keys near-orthogonalizable), giving the per-muscle router
# ample headroom. Everything else (inverted attention, per-action tokens, critic<-actor) is v20.
# Hypothesis: if per-action specialization was routing-limited, the wider QK lifts the ceiling.
#
# WHY v20 (PER-ACTION ROLE TOKENS: 2 slots per action dim, read out action-locally). The
# v6..v19 actor used N GENERIC slots flattened into one vector that a dense Linear(N*H,
# act_dim) regresses ALL action params from -- the slots are not tied to actions, and the
# readout is an order-flattened bag. v20 instead gives the ACTOR `tokens_per_action`(=2)
# dedicated slot tokens PER ACTION DIMENSION (so n_actor = 2*act_dim, e.g. 12 for
# HalfCheetah; NO global actor token). Each action dim's distribution is read out from ITS
# OWN two tokens by a shared per-role head:
#     slots -> (B, act_dim, 2, H);   p0 = headA(slot[:, :, 0]);   p1 = headB(slot[:, :, 1])
#   beta:     alpha = 1 + softplus(p0),  beta = 1 + softplus(p1)   (per action dim)
#   gaussian: mean  = p0,               raw_logvar = p1
# headA/headB are Linear(H, 1) SHARED across action dims (weight-tied per param-role); the
# per-action specialization lives entirely in the learned per-action tokens. In inverted
# attention each of the 2*act_dim tokens COMPETES to explain the obs tokens, so an action's
# alpha/beta token binds to the proprioceptive dims relevant to THAT actuator -- a far
# stronger structural prior than pooling, and the readout is one-token -> one-scalar
# (permutation-correct, no flatten). This is the invobs analog of the satransformer line's
# action role tokens. Built on v19 (separate per-role weights + asymmetric critic<-actor
# bridge); the critic is UNCHANGED (generic critic_slots, flattened, one value head) and
# still reads sg(the 2*act_dim actor tokens). Runs in parallel with v19 as the clean A/B
# for "per-action tokens vs pooled slots". Base: v19.
#
# --- inherited v19 notes ---
#
# WHY v19 (ASYMMETRIC CRITIC<-ACTOR CHANNEL, SEPARATE PER-ROLE WEIGHTS). v16 (and the
# whole v6..v18 line) gives the actor and critic two separate slot stacks that share only
# the obs tokenizer, with NO information flow between them. v19 keeps the separate-weights
# structure (each role's q/k/v/proj/mlp see ONLY their own loss's gradient -> zero
# policy/value gradient conflict on any trunk weight) but adds a one-directional
# information bridge so the critic can condition on the actor's representation:
#   - Actor and critic each keep their OWN slot tokens AND their OWN block weights. We do
#     NOT share weights between the roles: sharing would put both the policy and value
#     gradient on the same matrices (the classic shared-backbone conflict). Keeping them
#     separate means the actor trunk is shaped purely by the policy gradient and the critic
#     trunk purely by the value gradient.
#   - ASYMMETRIC INFO CHANNEL (critic_sees_actor, default on): the critic slots inverted-
#     attend over the scene [obs tokens ; stop_grad(final actor slots)] -- the actor's
#     finished parse becomes extra "scene" tokens the critic explains. This is native
#     slot-attention (the explainable set is arbitrary; we just enlarge the critic's
#     scene). Direction is critic <- actor ONLY:
#       * V(s)=E_{a~pi}[.] depends on the policy, so conditioning value on the actor's
#         representation is a genuine sufficient-statistic gain (lower-variance value).
#       * The actor never reads the critic, and the read is DETACHED, so value-loss
#         gradients NEVER reach the actor: the actor leads (gradient-pure), the critic
#         follows. The only weight both roles touch is the obs tokenizer (shared since
#         v6; the dual-backward decoupled clip already balances that one).
#   - They do NOT share an inverted softmax either. Inverted attention softmaxes over the
#     SLOT axis (slots COMPETE per token), so one merged actor+critic competition would
#     let one role STARVE the other of obs tokens. Each role runs its own pass.
# The actor/critic readers are SEPARATE compiled callables, so the decoupled dual-clip
# backprops each loss on its own single-output graph (v12's per-stack-compile requirement);
# the stop-grad keeps the two backward graphs disjoint. Everything else (nGPT hypersphere,
# inverted attention, SwiGLU, v16 depth-aware eigen-lr 1/(2N+1), per-step weight justnorm,
# PPO/critic/advantage stack) is unchanged. Base: v16. c_shift (v18) is dropped -- regressed.
#
# --- inherited v16 notes ---
#
# WHY v16 (DEPTH-AWARE EIGEN-LR INIT, from SimbaV2). v11/v12 inherited nGPT's fixed
# eigen-lr init of 0.05 on the normalized residual lerp  x <- justnorm(x + alpha*(f(x)-x)).
# 0.05 is tuned for VERY deep LMs; SimbaV2 (same hyperspherical lerp, an RL SOTA result)
# instead ties the step to depth: alpha_init = 1/(num_blocks+1), giving 0.33-0.5 for its
# 1-2 block nets. Our stack is 4 blocks = 8 lerp updates, so 0.05 leaves the residual
# stream barely moving and under-uses the slot blocks. v16 sets alpha_init = 1/(2*n_blocks+1)
# (= 1/9 ~= 0.111 for n_blocks=4), the SimbaV2 heuristic counting our 2 sublayers/block.
# This is the ONLY change vs v12 (mechanism, attention, FFN/SwiGLU, weight justnorm, PPO
# loop all identical). HYPOTHESIS: a depth-calibrated step unlocks the slot stack's capacity
# and improves over v11. Base: v12 (compiled, behavior-identical to v11 champion).
#
...
class Args:
    ...
    critic_slots: int = 4
    slot_blocks: int = 4
    slot_d_qk: int = 16      # v21: 8->16. Routing of 17 obs tokens happens in this low-rank
                             # QK space; at d_qk=8 the Welch coherence bound is 0.265 (tight,
                             # ~half-overcomplete for 17 tokens) so per-muscle routing is
                             # learning-limited. d_qk=16 drops the bound to 0.06 (17 tokens
                             # become near-orthogonalizable) -> ample muscle-routing headroom.
    slot_mlp_mult: int = 3   # v10: wider pointwise FFN (params reallocated from Q/K)
    critic_sees_actor: bool = True  # v19: critic slots also attend over sg(actor slots);
                                    # False = pure gradient-decoupling, no info channel (ablation)
    per_action_actor: bool = True   # v20: actor uses tokens_per_action slots PER action dim,
                                    # read out action-locally (vs N generic slots + dense head)
    tokens_per_action: int = 2      # v20: slots per action dim (one per distribution param)
...
class InvertedSlotBlock(nn.Module):
    ...
    def __init__(self, H, d_qk, mlp_mult, base_scale, alpha_init):
        super().__init__()
        # nGPT: no LayerNorm. Each sublayer is a normalized lerp toward its (sphere-
        # projected) output with a learned per-channel eigen learning rate. v16: the
        # init is DEPTH-AWARE (SimbaV2's 1/(N+1) heuristic, passed in) rather than
        # nGPT's fixed 0.05, which is calibrated for very deep LMs and leaves our
        # shallow 4-block stack barely moving the residual stream.
...
class InvertedSlotStack(nn.Module):
    ...
    def __init__(self, n_slots, H, n_blocks, d_qk, mlp_mult, base_scale):
        ...
        # v16: SimbaV2-style depth-aware eigen-lr init. The residual stream passes through
        # 2*n_blocks lerp updates (attn + mlp per block); SimbaV2's 1/(N+1) heuristic keeps
        # cumulative movement calibrated to depth. For n_blocks=4 -> 1/9 ~= 0.111 (vs nGPT's
        # fixed 0.05), so each block actually moves the representation a meaningful amount.
        alpha_init = 1.0 / (2 * n_blocks + 1)
        self.blocks = nn.ModuleList(
            [InvertedSlotBlock(H, d_qk, mlp_mult, base_scale, alpha_init) for _ in range(n_blocks)]
        )
...
class _SlotReader(nn.Module):
    """v19: refines ONE slot group over a token scene using its OWN block stack (weights
    NOT shared with the other role, so each role's matrices receive only their own loss's
    gradient). Owns its slot tokens and its blocks. When `extra` is supplied (the detached
    actor slots), it is appended to the scene the slots explain. This is the unit that gets
    torch.compile'd separately per role, so the dual-backward keeps each loss on its own
    single-output graph (mirrors v12's per-stack compile)."""
    ...
    def __init__(self, n_slots, H, n_blocks, d_qk, mlp_mult, base_scale):
        ...
        self.H = H
        self.slots = nn.Parameter(torch.randn(n_slots, H) / sqrt(H))
        alpha_init = 1.0 / (2 * n_blocks + 1)   # v16 depth-aware eigen-lr init
        self.blocks = nn.ModuleList(
            [InvertedSlotBlock(H, d_qk, mlp_mult, base_scale, alpha_init) for _ in range(n_blocks)]
        )
    ...
    def forward(self, tokens, extra=None):
        # extra (the detached actor slots) is appended to the scene when present. tokens
        # and extra are both unit-sphere H-vectors, so they sit in one inverted-attention
        # competition cleanly. Returns slots (B, S, H) on the sphere (scaled/flattened by
        # the caller) so the role head differentiates a single compiled output.
        B = tokens.shape[0]
        scene = tokens if extra is None else torch.cat([tokens, extra], dim=1)
        s = justnorm(self.slots).unsqueeze(0).expand(B, -1, -1)
        for block in self.blocks:
            s = block(s, scene)
        return s

    @torch.no_grad()
    def assign_entropy(self, tokens, extra=None):
        B = tokens.shape[0]
        scene = tokens if extra is None else torch.cat([tokens, extra], dim=1)
        s = justnorm(self.slots).unsqueeze(0).expand(B, -1, -1)
        ents = []
        for block in self.blocks:
            ents.append(block.attn.assign_entropy(s, scene))
            s = block(s, scene)
        return torch.stack(ents).mean()


class UnifiedSlotStack(nn.Module):
    """v19: actor and critic each have their OWN slot tokens AND their OWN block weights
    (NO weight sharing -> no policy/value gradient conflict on any trunk matrix). They are
    coupled only by the asymmetric, stop-gradded critic<-actor scene channel: the critic
    slots inverted-attend over [obs ; sg(final actor slots)] (or obs only when disabled).
    The actor never reads the critic and the read is detached, so value-loss gradients
    never reach the actor (actor leads gradient-pure; critic follows). See file header."""

    def __init__(self, actor_slots, critic_slots, H, n_blocks, d_qk, mlp_mult, base_scale,
                 critic_sees_actor=True):
        super().__init__()
        self.H = H
        self.n_actor = actor_slots
        self.n_critic = critic_slots
        self.critic_sees_actor = critic_sees_actor
        self.actor_reader = _SlotReader(actor_slots, H, n_blocks, d_qk, mlp_mult, base_scale)
        self.critic_reader = _SlotReader(critic_slots, H, n_blocks, d_qk, mlp_mult, base_scale)

    @property
    def actor_out_dim(self):
        return self.n_actor * self.H

    @property
    def critic_out_dim(self):
        return self.n_critic * self.H

    def actor_forward(self, tokens):
        # Actor pass: scene = obs tokens only (never sees the critic). Compiled separately.
        return self.actor_reader(tokens)

    def critic_forward(self, tokens, actor_slots_sg):
        # Critic pass: scene = [obs ; sg(actor slots)] (or obs only if disabled). The actor
        # slots arrive already detached. Compiled separately.
        extra = actor_slots_sg if self.critic_sees_actor else None
        return self.critic_reader(tokens, extra)

    @torch.no_grad()
    def assign_entropy(self, tokens):
        # Diagnostic, eager: (actor entropy over obs, critic entropy over its scene).
        a_ent = self.actor_reader.assign_entropy(tokens)
        if self.critic_sees_actor:
            sa = self.actor_reader(tokens)
            c_ent = self.critic_reader.assign_entropy(tokens, sa)
        else:
            c_ent = self.critic_reader.assign_entropy(tokens)
        return a_ent, c_ent


class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, actor_slots, critic_slots, slot_blocks, slot_d_qk, slot_mlp_mult,
                 critic_sees_actor=True, per_action_actor=False):
        super().__init__()
        base_scale = 1.0 / sqrt(H)   # nGPT base scale (single scale for alpha/sqk/suv params)
        self.H = H
        # v20: when set, the actor readout stays UNFLATTENED (B, n_actor, H) so each
        # per-action slot can be read locally by a shared head. Critic readout is
        # always flattened. The critic<-actor bridge passes raw on-sphere slots and
        # is unaffected by this flag.
        self.per_action_actor = per_action_actor
        self.tokenizer = ObsTokenizer(in_dim, H)
        # v19: when both heads are present (the share_backbone path) use ONE unified stack
        # with shared blocks + two slot groups + the asymmetric critic<-actor channel.
        # The single-head fallback (non-shared path) keeps the v16 InvertedSlotStack.
        self.unified = actor_slots is not None and critic_slots is not None
        if self.unified:
            self.stack = UnifiedSlotStack(
                actor_slots, critic_slots, H, slot_blocks, slot_d_qk, slot_mlp_mult,
                base_scale, critic_sees_actor,
            )
            self.actor_stack = None
    ...
    def normalize_weights(self):
        ...
        block_lists = (
            [self.stack.actor_reader.blocks, self.stack.critic_reader.blocks]
            if self.unified
            else [s.blocks for s in (self.actor_stack, self.critic_stack) if s is not None]
        )
    ...
    def _readout(self, slots):
        # nGPT: rescale on-sphere slots by sqrt(H) so head-input per-channel RMS ~= 1.
        return (slots * sqrt(self.H)).flatten(1)

    def _readout_actor(self, slots):
        # v20 per-action mode keeps the slot axis (B, n_actor, H) for action-local
        # heads; otherwise behaves like the flat critic readout.
        if self.per_action_actor:
            return slots * sqrt(self.H)
        return self._readout(slots)

    @property
    def actor_out_dim(self):
        return self.stack.actor_out_dim if self.unified else self.actor_stack.out_dim

    @property
    def critic_out_dim(self):
        return self.stack.critic_out_dim if self.unified else self.critic_stack.out_dim

    def forward_actor(self, x):
        if self.unified:
            return self._readout_actor(self.stack.actor_forward(self.encode_tokens(x)))
        return self.actor_stack(self.encode_tokens(x))

    def forward_critic(self, x):
        if self.unified:
            tokens = self.encode_tokens(x)
            sa = self.stack.actor_forward(tokens)
            return self._readout(self.stack.critic_forward(tokens, sa.detach()))
        return self.critic_stack(self.encode_tokens(x))

    def forward(self, x):
        tokens = self.encode_tokens(x)
        if self.unified:
            sa = self.stack.actor_forward(tokens)                  # actor leads
            sc = self.stack.critic_forward(tokens, sa.detach())    # critic follows (detached)
            return self._readout_actor(sa), self._readout(sc)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v20: per-action role tokens. The actor gets tokens_per_action slots PER action
        # dim (no global actor token); each action's distribution is read action-locally
        # from its own slots by shared heads. tokens_per_action==2 -> slot 0 feeds the
        # first concentration/mean, slot 1 the second concentration/log-variance.
        self.per_action_actor = args.per_action_actor
        self.tokens_per_action = args.tokens_per_action
        self.act_dim = act_dim
        self.H = H
        if self.per_action_actor:
            assert args.tokens_per_action == 2, "v20 reads param-0 from slot 0, param-1 from slot 1"
            n_actor = args.tokens_per_action * act_dim
        else:
            n_actor = args.actor_slots
        if self.share_backbone:
            ...
            self.trunk = ThinkTrunk(
                ...
                args.slot_d_qk,
                args.slot_mlp_mult,
                args.critic_sees_actor,
                args.per_action_actor,
            )
            ...
            self.actor_trunk = ThinkTrunk(
                ...
                args.slot_d_qk,
                args.slot_mlp_mult,
                per_action_actor=args.per_action_actor,
            )
        ...
        # v20: in per-action mode the two distribution heads read a SINGLE slot (H->1)
        # and are shared across action dims (applied to (B, act_dim, H) -> (B, act_dim)).
        # Otherwise they read the flat actor feature (actor_feat_dim -> act_dim).
        head_in = H if self.per_action_actor else actor_feat_dim
        head_out = 1 if self.per_action_actor else act_dim
        if self.actor_dist == "gaussian":
            # dreamer4 Gaussian: mean head + state-dependent log-VARIANCE head.
            self.actor_head = layer_init(nn.Linear(head_in, head_out), std=0.01)
            self.actor_logvar_head = layer_init(nn.Linear(head_in, head_out), std=0.01)
            ...
            self.actor_alpha_head = layer_init(nn.Linear(head_in, head_out), std=0.01)
            self.actor_beta_head = layer_init(nn.Linear(head_in, head_out), std=0.01)
    ...
    def _actor_dist(self, actor_feat):
        ...
        # v20: split the per-action slots into the two param sources. p0 feeds the
        # first head (mean / alpha), p1 the second (log-variance / beta). Shared heads
        # map (B, act_dim, H) -> (B, act_dim).
        if self.per_action_actor:
            feat = actor_feat.view(actor_feat.shape[0], self.act_dim, self.tokens_per_action, self.H)
            p0, p1 = feat[:, :, 0, :], feat[:, :, 1, :]
        else:
            p0 = p1 = actor_feat
        if self.actor_dist == "gaussian":
            mean = self.actor_head(p0).squeeze(-1) if self.per_action_actor else self.actor_head(p0)
            raw_lv = self.actor_logvar_head(p1).squeeze(-1) if self.per_action_actor else self.actor_logvar_head(p1)
        ...
        # beta
        a_raw = self.actor_alpha_head(p0).squeeze(-1) if self.per_action_actor else self.actor_alpha_head(p0)
        b_raw = self.actor_beta_head(p1).squeeze(-1) if self.per_action_actor else self.actor_beta_head(p1)
        alpha = 1.0 + F.softplus(a_raw)
        beta = 1.0 + F.softplus(b_raw)
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        def _compile_stacks(trunk):
            if trunk.unified:
                # v19: compile the two _SlotReader modules separately (they share the block
                # params). Each role's loss then backwards on its own single-output graph;
                # the critic's stop-gradded actor input keeps the graphs disjoint.
                trunk.stack.actor_reader = torch.compile(trunk.stack.actor_reader)
                trunk.stack.critic_reader = torch.compile(trunk.stack.critic_reader)
            else:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if args.share_backbone:
                ...
                # v19: one unified stack returns both role entropies in one eager pass.
                actor_ent, critic_ent = agent.trunk.stack.assign_entropy(stat_toks)
            else:
        ...
        # v20: per-action slot differentiation. The whole point of per-action tokens is
        # that each action dim's slots specialize; if they collapse, v20 degenerates to a
        # single shared token read by a shared head. We measure, on real obs:
        #  - action_dispersion: mean over the batch of (1 - mean pairwise cosine) between
        #    the 6 per-action slot reps. ~0 => collapsed/identical actions; higher => the
        #    slots carry action-specific content.
        #  - intra_action_cos: mean cosine between an action's two slots (1 => redundant
        #    pair carrying the same info; lower => the two params see complementary state).
        #  - policy_action_dispersion: std across action dims of the DETERMINISTIC action
        #    (the bottom line: do the dims actually request different controls).
        if args.per_action_actor and args.share_backbone:
            with torch.no_grad():
                sa = agent.trunk.stack.actor_forward(stat_toks)          # (B, n_actor, H)
                B = sa.shape[0]
                a_dim = agent.act_dim
                pa = sa.view(B, a_dim, args.tokens_per_action, args.hidden)
                rep = F.normalize(pa.reshape(B, a_dim, -1), dim=-1)      # per-action unit rep
                cos = torch.matmul(rep, rep.transpose(1, 2))             # (B, a_dim, a_dim)
                off = ~torch.eye(a_dim, dtype=torch.bool, device=cos.device)
                action_dispersion = (1.0 - cos[:, off].mean()).item()
                s0 = F.normalize(pa[:, :, 0, :], dim=-1)
                s1 = F.normalize(pa[:, :, 1, :], dim=-1)
                intra_cos = (s0 * s1).sum(-1).mean().item()
                feat = agent.trunk._readout_actor(sa)
                dist, to_action, _ = agent._actor_dist(feat)
                act_det = to_action(dist.mean)                          # (B, act_dim)
                policy_disp = act_det.std(dim=1).mean().item()
            writer.add_scalar("debug/slot_action_dispersion", action_dispersion, global_step)
            writer.add_scalar("debug/slot_intra_action_cos", intra_cos, global_step)
            writer.add_scalar("debug/policy_action_dispersion", policy_disp, global_step)
