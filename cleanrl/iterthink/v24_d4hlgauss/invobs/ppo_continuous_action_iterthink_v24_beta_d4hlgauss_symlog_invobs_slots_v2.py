# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted obs slot transformer v2.
#
...
# Each scalar observation dimension is an env-normalized token (Linear(1,H) +
# learned position; no immediate token RMSNorm that erases magnitude). Actor and
# critic use separate 4-slot, 4-block inverted-attention stacks. Every attention
...
#
# The block uses prenorm residuals, multi-head QK-norm with learned head gain,
# ReLU^2 FFNs, and parameter-golf-style residual mixing against the initial slots
# plus U-Net encoder/decoder skip weights. The PPO actor/critic heads consume the
# full flattened slot state (4*64=256) instead of compressing back to 64.
#
...
class ObsTokenizer(nn.Module):
    """Env-normalized scalar observation dimensions -> ordered hidden tokens."""

    def __init__(self, obs_dim, H):
        super().__init__()
        self.value = layer_init(nn.Linear(1, H))
        self.pos = nn.Parameter(torch.randn(obs_dim, H) / sqrt(H))

    def forward(self, x):
        return self.value(x.unsqueeze(-1)) + self.pos.unsqueeze(0)
...
class InvertedSlotBlock(nn.Module):
    def __init__(self, H, heads, mlp_mult, qk_gain_init):
        super().__init__()
        ...
        self.attn_scale = nn.Parameter(torch.ones(H, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.ones(H, dtype=torch.float32))
    ...
    def forward(self, slots, tokens, slots0):
        ...
        attn_out = self.attn(self.slot_attn_norm(x), self.token_attn_norm(tokens))
...
class InvertedSlotStack(nn.Module):
    def __init__(self, n_slots, H, n_blocks, n_heads, mlp_mult, qk_gain_init):
        super().__init__()
        assert n_blocks >= 1
        ...
        self.blocks = nn.ModuleList(
            [InvertedSlotBlock(H, n_heads, mlp_mult, qk_gain_init) for _ in range(n_blocks)]
        )
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, actor_slots, critic_slots, slot_blocks, slot_heads, slot_mlp_mult, qk_gain_init):
        super().__init__()
        self.tokenizer = ObsTokenizer(in_dim, H)
        self.actor_stack = (
            InvertedSlotStack(actor_slots, H, slot_blocks, slot_heads, slot_mlp_mult, qk_gain_init)
            if actor_slots is not None
            else None
        )
        self.critic_stack = (
            InvertedSlotStack(critic_slots, H, slot_blocks, slot_heads, slot_mlp_mult, qk_gain_init)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        H = args.hidden
        assert args.slot_dim == H, "slot_dim must equal hidden in v2"
