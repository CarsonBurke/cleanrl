# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted-attention slot transformer v18.
#
# WHY v18 (SimbaV2 c_shift: MAGNITUDE-PRESERVING SPHERE EMBEDDING). Single-variable
# ablation over the v12 champion (NOT stacked on v17, whose ortho-init + signed-lr came
# in neutral-to-slightly-negative @<=2.7M; this isolates c_shift alone). nGPT's justnorm
# at the tokenizer projects each obs token onto the unit sphere, which DISCARDS the
# token's magnitude |t_i| = |x_i*w_i + pos_i| -- i.e. throws away how large each scalar
# observation is, keeping only its direction. SimbaV2's HyperEmbedder fixes exactly this:
# it APPENDS a constant channel c_shift before the L2 projection,
#     t_i' = [ t_i ; c_shift ] in R^(H+1);   t_hat_i = t_i' / ||t_i'||
# so the H real coordinates are scaled by 1/sqrt(|t_i|^2 + c_shift^2) -- a monotone
# function of |t_i| -- and the appended coordinate carries c_shift/||t_i'||. Magnitude is
# no longer destroyed: it survives as the real-coords' shrinkage relative to the fixed
# constant axis. The constant axis is consumed downstream by k and v (whose input dim
# becomes H+1), exactly as SimbaV2's Dense consumes the (H+1)-normalized embedding.
# HYPOTHESIS: restoring per-token magnitude to the on-sphere representation lifts the
# asymptote/early learning, since proprioceptive control needs the *size* of joint
# positions/velocities, not only their direction. c_shift=3.0 matches SimbaV2 (our tokens
# are O(1) like their RSNorm'd input, so the ratio regime transfers). The slots, eigen-lr
# lerp, SwiGLU, inverted attention, per-step weight justnorm, and the entire PPO/critic/
# advantage stack are byte-identical to v12/v11; ONLY the tokenizer gains the c_shift
# channel and k/v widen from H to H+1 to read it. Base: v12 (compiled, == v11 champion).
#
# --- inherited v12 notes ---
#
...
class Args:
    ...
    c_shift: float = 3.0     # v18: SimbaV2 constant channel appended before tokenizer sphere
                             #      projection (preserves per-token magnitude). 0 disables (==v12).
...
class ObsTokenizer(nn.Module):
    ...
    v18: before the sphere projection we APPEND a constant channel c_shift to each token
    (SimbaV2's HyperEmbedder). Plain justnorm is scale-invariant and so destroys the
    token magnitude |t_i| = |x_i*w_i + pos_i| (the size of obs dim i); appending a fixed
    c_shift breaks that invariance, so after normalizing over H+1 the magnitude survives
    as the real coords' shrinkage 1/sqrt(|t_i|^2 + c_shift^2) relative to the constant
    axis. Output dim is therefore H+1 (consumed by the attention k/v). c_shift<=0 falls
    back to the v12 H-dim justnorm (no constant channel)."""

    def __init__(self, obs_dim, H, c_shift):
        super().__init__()
        self.value_weight = nn.Parameter(torch.randn(obs_dim, H) / sqrt(H))
        self.pos = nn.Parameter(torch.randn(obs_dim, H) / sqrt(H))
        self.c_shift = c_shift
        self.out_dim = H + 1 if c_shift > 0 else H

    def forward(self, x):
        tokens = x.unsqueeze(-1) * self.value_weight.unsqueeze(0) + self.pos.unsqueeze(0)
        if self.c_shift > 0:
            const = tokens.new_full(tokens.shape[:-1] + (1,), self.c_shift)
            tokens = torch.cat([tokens, const], dim=-1)
...
class InvertedCrossAttention(nn.Module):
    ...
    def __init__(self, H, d_qk, base_scale, tok_dim=None):
        super().__init__()
        self.H = H
        self.d_qk = d_qk
        # v18: tokens carry the appended c_shift channel, so k/v read tok_dim (H+1)
        # instead of H. Queries come from the slots (still H). tok_dim defaults to H.
        tok_dim = H if tok_dim is None else tok_dim
        self.q = nn.Linear(H, d_qk, bias=False)        # low-rank, single-head assignment
        self.k = nn.Linear(tok_dim, d_qk, bias=False)
        self.v = nn.Linear(tok_dim, H, bias=False)     # full-rank value (carries info)
...
class InvertedSlotBlock(nn.Module):
    ...
    def __init__(self, H, d_qk, mlp_mult, base_scale, tok_dim=None):
        super().__init__()
        ...
        self.attn = InvertedCrossAttention(H, d_qk, base_scale, tok_dim)
...
class InvertedSlotStack(nn.Module):
    ...
    def __init__(self, n_slots, H, n_blocks, d_qk, mlp_mult, base_scale, tok_dim=None):
        super().__init__()
        assert n_blocks >= 1
        ...
        self.blocks = nn.ModuleList(
            [InvertedSlotBlock(H, d_qk, mlp_mult, base_scale, tok_dim) for _ in range(n_blocks)]
        )
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, actor_slots, critic_slots, slot_blocks, slot_d_qk, slot_mlp_mult, c_shift=0.0):
        super().__init__()
        base_scale = 1.0 / sqrt(H)   # nGPT base scale (single scale for alpha/sqk/suv params)
        self.tokenizer = ObsTokenizer(in_dim, H, c_shift)
        tok_dim = self.tokenizer.out_dim   # H+1 with c_shift, else H (k/v input width)
        self.actor_stack = (
            InvertedSlotStack(actor_slots, H, slot_blocks, slot_d_qk, slot_mlp_mult, base_scale, tok_dim)
            if actor_slots is not None
            else None
        )
        self.critic_stack = (
            InvertedSlotStack(critic_slots, H, slot_blocks, slot_d_qk, slot_mlp_mult, base_scale, tok_dim)
