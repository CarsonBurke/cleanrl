# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted-attention slot transformer v14.
#
...
class SlotMLP(nn.Module):
    """nGPT ReLU^2 FFN (v14). A single up-projection to hidden d = int(1.5*mult*H),
    scaled by a learned per-channel `su` gain (init 1, base_scale-free parameterization
    so the raw param has unit gradient dynamics), then the squared-ReLU nonlinearity:
    relu(h)**2. No gating, no bias. Hidden width is chosen so the FFN param count
    (128*d == 3*mult*H^2) matches the v12 SwiGLU FFN exactly. Weights are
