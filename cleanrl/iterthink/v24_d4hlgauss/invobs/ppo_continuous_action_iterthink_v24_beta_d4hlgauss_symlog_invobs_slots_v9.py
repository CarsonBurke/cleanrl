# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted-attention slot transformer v9.
#
# WHY v9 (one variable on top of v6: DEPTH-MIXING READOUT). Independent single-variable
# sibling of v8 (which probes the tokenizer); v9 probes the READOUT, both branching
# from the v6 baseline (8561 +/-376 HalfCheetah @8M) so their effects compose cleanly.
#
#   v6's PPO head reads only final_norm(final_slots). Because the block is a plain
#   residual (slots += attn; slots += ffn), the final slot is a FIXED-WEIGHT SUM over
#   depths: slots_final = slots_0 + sum_k (attn_k + ffn_k). The head therefore sees
#   every depth's contribution collapsed with weight 1 -- it cannot, e.g., subtract an
#   early iterate from a late one to read "what the refinement changed", nor weight a
#   coarse-assignment depth differently from the converged one. For an ITERATIVE-
#   refinement stack (which is exactly what inverted attention is) the per-depth
#   iterates carry distinct, non-redundant signal.
#
#   v9 concatenates the initial slots + every block's output (n_blocks+1 depths) per
#   slot, LayerNorms, and LINEAR-PROJECTS back to H. The PPO head input dim is
#   UNCHANGED (n_slots*H), so this is a clean test of depth-MIXING expressivity, NOT a
#   width/capacity increase: the only new capability is that the head can apply its own
#   learned weights to each refinement depth. This is the one v24 (DenseNet concat-
#   readout) ingredient prior analysis endorsed as genuine depth-mixing rather than a
#   prosthetic. Everything else is byte-identical to v6.
#
...
class InvertedSlotStack(nn.Module):
    ...
    def __init__(self, n_slots, H, n_blocks, n_heads, mlp_mult):
        ...
        # v9: depth-mixing readout. Concat the initial slots + every block's output
        # (n_blocks+1 depths) per slot, then LN + linear-project back to H so the PPO
        # head input dim is UNCHANGED from v6 (n_slots*H). This isolates the
        # depth-mixing capability (head can weight each refinement depth with its own
        # weights, e.g. late-minus-early) from any width/capacity increase. v6 read
        # only final_norm(final_slots), a fixed-weight residual sum over depths.
        self.read_norm = nn.LayerNorm(H * (n_blocks + 1))
        self.read_proj = nn.Linear(H * (n_blocks + 1), H)
    ...
    def forward(self, tokens):
        ...
        depths = [slots]                       # initial slots (learned prior)
        for block in self.blocks:
            slots = block(slots, tokens)
            depths.append(slots)               # each refinement depth
            self._accumulate_block_stats(block)
        cat = torch.cat(depths, dim=-1)        # (B, n_slots, H*(n_blocks+1))
        out = self.read_proj(self.read_norm(cat))  # depth-mix -> (B, n_slots, H)
