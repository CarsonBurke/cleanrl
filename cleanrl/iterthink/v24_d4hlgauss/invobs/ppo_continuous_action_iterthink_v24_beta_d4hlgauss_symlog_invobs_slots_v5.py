# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted obs slot transformer v5.
#
# Variant hypothesis: use the TF-Inv / Slot-Attention normalization-axis change as
# the full backbone, but make the block modern and deep enough for MuJoCo control.
# Each scalar observation dimension is an env-normalized, dimension-specific token
# (x_i * value_weight_i + learned position_i), with no convolutional/shared scalar
# invariance. Actor and critic use separate 4-slot inverted-attention stacks. Each
# stack first builds input-dependent initial slots with inverted cross-attention,
# then applies four zero-init residual refinement blocks. Every attention layer is
# paper-aligned inverted cross-attention: tokens assign mass across slots (softmax
# over slots), then slots renormalize over tokens. No GRU, no temporal recurrence,
# no normal self-attention/PMA.
#
# The block uses prenorm residuals, multi-head QK-norm with learned head gain init
# 1.0, and ReLU^2 FFNs. The residual branch mechanics borrow from Parameter Golf:
# learned current-vs-initial residual mixing, one-valued residual branch scales,
# zero-initialized attention/MLP output projections, and learned LIFO U-Net skip
# weights. This is not Parameter Golf's causal self-attention dataflow: skips operate
# on slot states, while observations enter through inverted cross-attention. Unlike
# v4, the residual anchor is input-dependent (`slots0`) so zero-init residual blocks
# still leave an observation-dependent path, matching the important PG initialization
# invariant. Environment-side observation normalization supplies the input scale; the
# PPO actor/critic heads consume the full flattened slot state (4*64=256).
#
...
class Args:
    ...
    slot_seed_scale_init: float = 1.0
    slot_attn_scale_init: float = 1.0
    slot_mlp_scale_init: float = 1.0
...
class ObsTokenizer(nn.Module):
    ...
    def forward(self, x):
        return x.unsqueeze(-1) * self.value_weight.unsqueeze(0) + self.pos.unsqueeze(0)


def zero_init_linear(layer):
...
class SlotMLP(nn.Module):
    def __init__(self, H, mult):
        super().__init__()
        self.fc = nn.Linear(H, mult * H, bias=False)
        self.proj = zero_init_linear(nn.Linear(mult * H, H, bias=False))
...
class InvertedCrossAttention(nn.Module):
    """Multi-head inverted cross-attention: slots compete for each token."""

    def __init__(self, H, heads, qk_gain_init, zero_proj=True):
        super().__init__()
        ...
        if zero_proj:
            zero_init_linear(self.proj)
        self.q_gain = nn.Parameter(torch.full((heads,), qk_gain_init, dtype=torch.float32))
        self._assign_entropy = None

    def forward(self, slots, key_tokens, value_tokens):
        B, S, _ = slots.shape
        T = key_tokens.shape[1]
        q = self.q(slots).view(B, S, self.heads, self.head_dim).transpose(1, 2)   # (B, h, S, d)
        k = self.k(key_tokens).view(B, T, self.heads, self.head_dim).transpose(1, 2)  # (B, h, T, d)
        v = self.v(value_tokens).view(B, T, self.heads, self.head_dim).transpose(1, 2)  # (B, h, T, d)
        q = F.rms_norm(q, (self.head_dim,))
        k = F.rms_norm(k, (self.head_dim,))
        q = q * self.q_gain.to(dtype=q.dtype)[None, :, None, None]
        logits = torch.matmul(q, k.transpose(-1, -2)) / sqrt(self.head_dim)       # (B, h, S, T)
        assign = torch.softmax(logits, dim=-2)                                   # token -> competing slots
        attn = assign / (assign.sum(dim=-1, keepdim=True) + 1e-6)                # slot -> token renorm
...
class InvertedSlotBlock(nn.Module):
    def __init__(self, H, heads, mlp_mult, qk_gain_init, attn_scale_init, mlp_scale_init):
        super().__init__()
        self.slot_attn_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.token_attn_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.mlp_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.attn = InvertedCrossAttention(H, heads, qk_gain_init, zero_proj=True)
        self.mlp = SlotMLP(H, mlp_mult)
        self.attn_scale = nn.Parameter(torch.full((H,), attn_scale_init, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.full((H,), mlp_scale_init, dtype=torch.float32))
        self.resid_mix = nn.Parameter(torch.stack((torch.ones(H), torch.zeros(H))).float())

    def forward(self, slots, tokens, slots0):
        mix = self.resid_mix.to(dtype=slots.dtype)
        x = mix[0][None, None, :] * slots + mix[1][None, None, :] * slots0
        attn_tokens = self.token_attn_norm(tokens)
        attn_out = self.attn(self.slot_attn_norm(x), attn_tokens, attn_tokens)
...
class InvertedSlotStack(nn.Module):
    def __init__(
        ...
        self.seed_attn_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.seed_token_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.seed_attn = InvertedCrossAttention(H, n_heads, qk_gain_init, zero_proj=False)
        self.seed_scale = nn.Parameter(torch.full((H,), seed_scale_init, dtype=torch.float32))
        self.blocks = nn.ModuleList(
            [
                InvertedSlotBlock(H, n_heads, mlp_mult, qk_gain_init, attn_scale_init, mlp_scale_init)
                for _ in range(n_blocks)
            ]
        )
        self.num_encoder_blocks = n_blocks // 2
        self.num_decoder_blocks = n_blocks - self.num_encoder_blocks
        self.num_skip_weights = min(self.num_encoder_blocks, self.num_decoder_blocks)
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, H, dtype=torch.float32))
        self.final_norm = nn.RMSNorm(H, elementwise_affine=False)
    ...
    def forward(self, tokens):
        B = tokens.shape[0]
        learned_slots = self.slots.unsqueeze(0).expand(B, -1, -1)
        seed_tokens = self.seed_token_norm(tokens)
        seed_update = self.seed_attn(self.seed_attn_norm(learned_slots), seed_tokens, seed_tokens)
        slots = learned_slots + self.seed_scale.to(dtype=learned_slots.dtype)[None, None, :] * seed_update
        slots0 = slots
        skips = []
        for i in range(self.num_encoder_blocks):
            slots = self.blocks[i](slots, tokens, slots0)
            self._accumulate_block_stats(self.blocks[i])
            skips.append(slots)
        for i in range(self.num_decoder_blocks):
            bi = self.num_encoder_blocks + i
            if i < self.num_skip_weights and skips:
                slots = slots + self.skip_weights[i].to(dtype=slots.dtype)[None, None, :] * skips.pop()
            slots = self.blocks[bi](slots, tokens, slots0)
            self._accumulate_block_stats(self.blocks[bi])
        return self.final_norm(slots).flatten(1)
...
class ThinkTrunk(nn.Module):
    def __init__(
        ...
        slot_mlp_mult,
        qk_gain_init,
        slot_seed_scale_init,
        slot_attn_scale_init,
        slot_mlp_scale_init,
    ):
        super().__init__()
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        H = args.hidden
        assert args.slot_dim == H, "slot_dim must equal hidden in v5"
        ...
        if self.share_backbone:
            ...
            self.trunk = ThinkTrunk(
                ...
                args.slot_mlp_mult,
                args.qk_gain_init,
                args.slot_seed_scale_init,
                args.slot_attn_scale_init,
                args.slot_mlp_scale_init,
            )
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.share_backbone:
            actor_ent = agent.trunk.actor_stack.consume_stats()
            critic_ent = agent.trunk.critic_stack.consume_stats()
            if actor_ent is not None:
                writer.add_scalar("debug/invslot_actor_assign_entropy", actor_ent.item(), global_step)
            if critic_ent is not None:
