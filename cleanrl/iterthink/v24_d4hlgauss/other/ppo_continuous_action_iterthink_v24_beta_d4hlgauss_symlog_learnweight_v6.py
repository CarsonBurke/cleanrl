# PPO + LEARNED-AWR ADVANTAGE WEIGHT (softmax over advantages) (v6).
#
# GOAL. Replace the STATIC rankgauss advantage transform with a LEARNED reshape of the
# PPO advantages. The weight is an AWR / exp-tilt SOFTMAX over the batch advantages, but
# the softmax LOGIT for each sample is a LEARNED per-sample reweight of its advantage:
#   mu_i = phi(actor_feat_i, A_i, value_i)            (learned logit; signed A_i in)
#   z_i ~ Normal(mu_i, sigma)                         (Gaussian exploration on the logit)
#   w_i = N * softmax(z_i)  over the batch            (AWR weight, mean 1)
...
#
# WHY SOFTMAX (v4/v5 -> v6). v4/v5 used a mean-1 MULTIPLICATIVE weight. Empirically inert
# (corr(w,|A|)~0, returns = base) because: norm_adv -> Adam (scale-invariant) -> max-norm
# grad-clip (binds 100%, global rescale) LAUNDER any magnitude effect; only a RELATIVE
# reshape (which samples dominate => gradient DIRECTION) survives -- and a near-uniform
# mean-1 weight (ESS~0.82) is a tiny relative reshape. rankgauss works through the SAME
# pipeline precisely because it is a STRONG relative reshape (tail compression). A SOFTMAX
# can concentrate hard (ESS -> small) => a strong relative-reshape lever, the thing the
# mean-1 weight lacked. The realized reward (below) then regulates how far to concentrate.
#
# TEACHER: REALIZED held-out RETURNS DIFFERENCE (kept from v5). Per minibatch split
# T (train) / V (held-out):
#   g_w = grad_theta sum_{i in T} w_i * L_i^PPO     (softmax-weighted train-loss gradient)
#   g_0 = grad_theta sum_{i in T}       L_i^PPO     (uniform baseline gradient)
#   reward = J_V(theta - a*g_w) - J_V(theta - a*g_0)            (a = lw_probe_lr)
# i.e. does this weighting's step improve the held-out PPO surrogate J_V MORE than the
# uniform step? Realized (2nd-order-accurate): amplifying a noisy outlier overshoots and
# LOWERS J_V, so compression can win -- the curvature term IS the trust-region signal
# rankgauss exploits. SIGN-NEUTRAL: compress or amplify, whatever raises J_V wins. The
# strong softmax lever makes reward INFORMATIVE (v5's mean-1 lever gave reward ~ 1e-4 noise).
#
# REINFORCE on the logits:
#   L_w = - ( mean_{i in T} log Normal(z_i; mu_i, sigma) ) * reward
# No separate baseline: J_V(uniform) does not depend on z, so the paired subtraction is an
# exact control variate (reward already centered vs uniform). reward is ONE scalar per
# minibatch; the SHARED-PARAMETER phi (few params) carries the per-sample structure, learned
# statistically across minibatches -- so per-sample feature conditioning does NOT blow up
# credit the way v5's per-sample stochastic weights would have.
#
# THE WEIGHT HEAD.
#   mu = weight_logit_head(tanh(weight_body([actor_feat, A, value])))   (small init => mu~0
#   => softmax≈uniform => w≈1 at start = standard PPO). Applied/probe softmax are over the
#   FULL minibatch (real update) / over T only (probe) so sum(w_T)=|T|=sum(uniform) and the
#   probe compares step DIRECTION/concentration, not size. w is detached when applied
#   (mean-1 => only RE-WEIGHTS, no learning-rate confound).
#
...
class Args:
    ...
    # ---- Learned-AWR advantage weight: w = N*softmax(phi(feat,A,value)+noise) ----
    # Trained by REINFORCE on the REALIZED held-out returns difference (vs uniform step).
    ...
    lw_logit_sigma: float = 0.5  # exploration std on the softmax logit z ~ Normal(mu, sigma)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.learnweight:
                    ...
                    # Learned-AWR weight: per-sample softmax LOGIT mu = phi(feat, A, value),
                    # with Gaussian exploration z ~ Normal(mu, sigma). Inputs all detached
                    ...
                    dist_w = Normal(mu, args.lw_logit_sigma)
                    z_w = dist_w.sample().detach()                               # (M,) sampled logit
                    # Applied weighting for the REAL update + diagnostics: AWR/exp-tilt
                    # w = N * softmax(z) over the full minibatch (mean 1; CAN concentrate =>
                    # strong relative reshape, the lever a mean-1 multiplicative weight lacked).
                    w_full = (M * torch.softmax(z_w, dim=0)).detach()            # mean 1 over M
                    # Probe weighting: softmax over T ONLY, so the reward is a pure function of
                    # z_w[T] (exact control variate + exact REINFORCE credit on logp_w_T), and
                    # sum(w_T) = |T| = sum(uniform) so |g_w| ~ |g_0| -- the probe compares step
                    # DIRECTION/concentration, not step size.
                    nT = M - nV
                    w_T = (nT * torch.softmax(z_w[T_pos], dim=0)).detach()       # mean 1 over T
                    ...
                    logp_w_T = dist_w.log_prob(z_w)[T_pos]                        # Normal log-prob
