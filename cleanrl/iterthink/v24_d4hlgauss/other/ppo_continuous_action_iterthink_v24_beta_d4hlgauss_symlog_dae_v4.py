# PPO + IterThink v24 beta d4-HL-Gauss symlog + DIRECT ADVANTAGE ESTIMATION (DAE) v4.
#
# v4 is calibrated against the OFFICIAL DAE code (github.com/hrpan/dae,
...
class Args:
    ...
    dae_nstep: int = 128             # n-step horizon = paper's full-trajectory value (Table 4). Large n grounds
                                     # A_hat in real multi-step returns -> true-advantage magnitude. Must divide num_steps.
    ...
    dae_adv_rms_norm: bool = True    # v4: divide A_hat by its POLICY-WEIGHTED RMS sqrt(E_s E_{a~mu}[A^2]) before
                                     # the PPO surrogate. This is the official `advantage_normalization: true`
                                     # (a single global SCALE, NOT z-scoring). DAE's A_hat scale is arbitrary by
                                     # design and rescaled to unit scale at the policy step; raw A_hat~0.02 was
                                     # starving the policy gradient. THE key v4 fix.
