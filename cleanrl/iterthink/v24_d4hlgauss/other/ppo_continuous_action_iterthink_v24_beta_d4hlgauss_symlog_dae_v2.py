# PPO + IterThink v24 beta d4-HL-Gauss symlog + DIRECT ADVANTAGE ESTIMATION (DAE) v2.
#
# v2 FIXES v1's fatal flaw. v1 trained the HL-Gauss value SEPARATELY (CE to the GAE
...
class Args:
    ...
    dae_k_center: int = 4            # MC samples a'~mu per state for the pi-centering E_mu[f(s,a')]
...
class Agent(nn.Module):
    ...
    def act_with_center(self, x, n_center):
        # Rollout step: sample the action AND n_center extra centering actions from
        # the SAME (behaviour=mu) distribution. Returns the native samples so the
        # update pass can recompute f at exactly mu's sampled actions (Eq: E_mu[f]).
        actor_feat, critic_feat = self._trunks(x)
        dist, to_action, log_det_fn = self._actor_dist(actor_feat)
        z = dist.sample()
        if self.actor_dist == "beta":
        ...
        log_prob = (dist.log_prob(z) - log_det_fn(z)).sum(1)
        value_logits = self.critic_head(critic_feat)
        cz = dist.sample((n_center,))                      # (K, B, A)
        if self.actor_dist == "beta":
            cz = cz.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        center_z = cz.permute(1, 0, 2).contiguous()        # (B, K, A)
        return action, z, log_prob, value_logits, center_z

    def get_action_value_adv(self, x, z, center_z):
        # Update pass: recompute log_prob at the replayed native sample z, the value
        # logits, and the pi-centered DAE advantage A_hat = f(s,a) - mean_k f(s,a'_k).
        actor_feat, critic_feat = self._trunks(x)
        dist, to_action, log_det_fn = self._actor_dist(actor_feat)
        log_prob = (dist.log_prob(z) - log_det_fn(z)).sum(1)
        if self.actor_dist == "gaussian":
        ...
        center = self._adv_f_multi(critic_feat, center_z).mean(dim=1)  # (B,)
...
if __name__ == "__main__":
    ...
    # DAE pi-centering: K native actions sampled from mu (the rollout policy) per step.
    center_zs = torch.zeros(
        (args.num_steps, args.num_envs, args.dae_k_center) + envs.single_action_space.shape
    ).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, value_logits, center_z = agent.act_with_center(
                    next_obs, args.dae_k_center
                )
        ...
        center_v = seg_view(center_zs)                 # (nstep, NS, K, *act)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, n_segments, seg_per_mb):
                ...
                mb_cz = center_v[:, mb_seg].reshape((nstep * S, args.dae_k_center) + act_shape)

                newlogprob, entropy, value_logits, adv = agent.get_action_value_adv(mb_obs, mb_z, mb_cz)
        ...
        with torch.no_grad():
            _, _, _, dbg_adv = agent.get_action_value_adv(
                ...
                latent_zs.reshape((-1,) + act_shape),
                center_zs.reshape((-1, args.dae_k_center) + act_shape),
            )
