# PPO + IterThink v24 beta d4-HL-Gauss symlog critic, raw-spaced bins v1.
#
# Variant hypothesis: keep v24 beta PPO machinery and d4-HL-Gauss behavior, but
# use raw-spaced value buckets with symlog-space Gaussian smoothing:
#   raw bin edges are linear in [v_min, v_max].
#   scalar GAE return -> symlog -> Gaussian mass integrated over symlog(raw edges).
#   logits -> E[raw bin center] -> scalar value for GAE/bootstrap.
...
# are linear in raw normalized-return space; only target smoothing happens in
# symlog coordinates.
#
...
from cleanrl.shared.hl_gauss import symlog
...
class RawSpacedSymlogHLGaussSupport:
    """HL-Gauss over raw-spaced bins with Gaussian smoothing in symlog space.

    The categorical variable indexes raw value intervals. A scalar target `x`
    is transformed to `symlog(x)`, and each raw bin receives the Gaussian CDF
    mass between `symlog(raw_edge_i)` and `symlog(raw_edge_{i+1})`. Decode is
    therefore E[raw bin center], not symexp(E[symlog center]).
    """

    def __init__(self, num_bins, v_min, v_max, sigma_ratio, device, eps=1e-10):
        ...
        self.coord_edges = symlog(self.edges)
        self.coord_widths = self.coord_edges[1:] - self.coord_edges[:-1]
        self.coord_support = symlog(self.support)

    def _target_sigma(self, targets):
        raw_targets = targets.clamp(self.v_min, self.v_max)
        idx = ((raw_targets - self.v_min) / self.bin_width).floor().long().clamp(0, self.num_bins - 1)
        return (self.sigma_ratio * self.coord_widths[idx]).clamp_min(self.eps)
    ...
    def project(self, targets):
        raw_targets = targets.clamp(self.v_min, self.v_max)
        coord_targets = symlog(raw_targets)
        sigma = self._target_sigma(raw_targets)
        coord_edges = self.coord_edges

        coord_targets = coord_targets.unsqueeze(-1)
        sigma = sigma.unsqueeze(-1)
        cdf_evals = torch.erf((coord_edges.unsqueeze(0) - coord_targets) / (sigma * np.sqrt(2)))
    ...
    def cdf_fraction(self, targets):
        raw_targets = targets.clamp(self.v_min, self.v_max)
        coord_targets = symlog(raw_targets).unsqueeze(-1)
        lo = self.coord_edges[:-1]
        frac = (coord_targets - lo) / self.coord_widths.clamp_min(self.eps)
        return frac.clamp(0.0, 1.0)

    def gaussian_prior_logits(self, tau):
        sigma = torch.as_tensor(tau, device=self.edges.device, dtype=self.edges.dtype)
        cdf = 0.5 * (1.0 + torch.erf(self.coord_edges / (sigma * np.sqrt(2))))
        mass = (cdf[1:] - cdf[:-1]).clamp_min(self.eps)
        return mass.log()
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # prior over symlog coordinates, integrated over the raw-spaced bins.
        # For uniform transformed bins this is equivalent to the old center
        # density bias up to a constant; with raw-spaced bins the exact bin mass
        # accounts for nonuniform symlog widths.
        self.critic_head = layer_init(nn.Linear(H, args.num_bins), std=0.1)
        with torch.no_grad():
            init_support = RawSpacedSymlogHLGaussSupport(
                args.num_bins,
                args.v_min,
                args.v_max,
            ...
            self.critic_head.bias.copy_(init_support.gaussian_prior_logits(args.critic_init_tau))
...
if __name__ == "__main__":
    ...
    hl_support = RawSpacedSymlogHLGaussSupport(
        args.num_bins,
        args.v_min,
        args.v_max,
        args.value_sigma_to_bin_ratio,
    ...
    support = hl_support.support                       # (num_bins,) raw bin centers
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # GAE scale consumed by tanh_std. Bins represent raw intervals; only
            # target smoothing / CDF diagnostics use symlog coordinates.
