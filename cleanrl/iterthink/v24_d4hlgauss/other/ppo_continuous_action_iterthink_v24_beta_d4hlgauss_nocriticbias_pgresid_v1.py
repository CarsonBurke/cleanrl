# PPO + IterThink v24 beta d4-HL-Gauss no-critic-bias pg-resid v1.
#
# Variant hypothesis: keep v24 beta PPO machinery, vf_coef=0.5, D4-style
# scalar-return HL-Gauss, and no critic-head bias, but replace the IterThink
# DenseNet trunk with a parameter-golf-baseline residual trunk:
#   - fixed-width residual stream instead of growing feature concatenation
#   - raw per-channel resid_mix initialized to [1, 0] for x/x0
#   - per-channel dense/moe residual scales initialized to 1
#   - zero-initialized residual branch output projections
#   - encoder/decoder learned additive skip weights
# This isolates whether parameter-golf's identity-start residual parameterization
# gives PPO a better-conditioned shared actor/critic representation.
#
# Inherited critic target:
...
# which may be suppressing critic gradient scale. This ablation also removes
# the peaked zero-return critic-head bias to test whether the D4-HL-Gauss target
# benefits from an unbiased initial categorical head.
#
# Base: ppo_continuous_action_iterthink_v24_beta_d4hlgauss_nocriticbias_v1.py.
# Only intended additional algorithmic change: parameter-golf-baseline residual
# trunk alignment.
#
...
def _branch_body(H, zero_init_output=False):
    # Parameter-golf baseline zero-inits residual output projections so each
    # block starts as an identity/carry path and learns residual deltas.
    out_std = 0.0 if zero_init_output else np.sqrt(2)
    return nn.Sequential(
        layer_init(nn.Linear(H, H)),
        ReLUSquared(),
        layer_init(nn.Linear(H, H), std=out_std),
    )


class ThinkBlock(nn.Module):
    """Parameter-golf-style residual mixer plus scaled dense/MoE deltas."""
    ...
    def __init__(self, H, n_experts):
        ...
        self.dense_scale = nn.Parameter(torch.ones(H, dtype=torch.float32))
        self.moe_scale = nn.Parameter(torch.ones(H, dtype=torch.float32))

        self.dense_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.dense = _branch_body(H, zero_init_output=True)

        self.moe_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.gate = layer_init(nn.Linear(H, n_experts))
        self.experts = nn.ModuleList([_branch_body(H, zero_init_output=True) for _ in range(n_experts)])
    ...
    def forward(self, x, x0):
        ...
        x_mid = x_in + self.dense_scale.to(dtype=x.dtype)[None, :] * d_dense

        m_in = self.moe_norm(x_mid)
        ...
        return x_mid + self.moe_scale.to(dtype=x.dtype)[None, :] * d_moe
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, n_experts):
        ...
        self.entry_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.num_encoder_layers = K // 2
        self.num_decoder_layers = K - self.num_encoder_layers
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers)
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, H, dtype=torch.float32))
        self.blocks = nn.ModuleList([ThinkBlock(H, n_experts) for _ in range(K)])
        self.final_norm = nn.RMSNorm(H, elementwise_affine=False)

    def forward(self, x):
        x = self.entry_norm(self.entry(x))
        x0 = x
        skips = []
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Categorical critic. This ablation deliberately leaves the layer_init
        # zero bias in place instead of applying v24's peaked zero-return prior.
