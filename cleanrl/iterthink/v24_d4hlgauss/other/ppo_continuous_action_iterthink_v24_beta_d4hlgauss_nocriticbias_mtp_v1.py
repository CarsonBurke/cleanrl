# PPO + IterThink v24 beta d4-HL-Gauss no-critic-bias MTP v1.
#
...
# benefits from an unbiased initial categorical head. It adds Orbit Wars style
# critic MTP: one critic token predicts H=6 future return rows, and CE is summed
# across valid horizons for each rollout row.
#
# Base: ppo_continuous_action_iterthink_v24_beta_d4hlgauss_v1.py.
# Only intended algorithmic changes: D4-style critic target, no critic bias, and
# critic MTP with summed horizon loss.
#
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.critic_head = layer_init(nn.Linear(H, args.critic_mtp_horizon * args.num_bins), std=0.1)
    ...
    def get_value(self, x):
        # Returns value LOGITS (B, H, num_bins); horizon 0 is V(s_t), later
        # horizons are critic-only MTP predictions.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                bootstrap_probs = torch.softmax(boot_logits[:, 0], dim=-1)      # (B, n) = Z(s_T)
                ...
                bootstrap_probs = torch.softmax(agent.get_value(next_obs)[:, 0], dim=-1)   # (B, n) = Z(s_T)
            ...
            # Critic target: Orbit Wars style MTP over Dreamer4-style scalar
            # HL-Gauss targets. Horizon 0 predicts returns[t]; later horizons
            # predict returns[t+h] from the same critic features. Episode-boundary
            # and rollout-tail horizons are masked out. The loss sums valid
            # horizons per row, matching orbit-wars.
            return_mtp = returns.new_zeros((*returns.shape, args.critic_mtp_horizon))
            return_mtp_mask = torch.zeros(
                (*returns.shape, args.critic_mtp_horizon),
                dtype=torch.bool,
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # HL-Gauss MTP value loss: per-horizon CE, summed across valid
                # future horizons per row. No value clipping.
        ...
        edge_mass = (
            (edge_per_h * b_target_mask.to(dtype=edge_per_h.dtype)).sum()
            / b_target_mask.sum().clamp_min(1).to(dtype=edge_per_h.dtype)
        ).item()
