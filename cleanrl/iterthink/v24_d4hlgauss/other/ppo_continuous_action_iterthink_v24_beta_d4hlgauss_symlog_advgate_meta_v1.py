# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + meta advantage gate v1.
#
# Adds a Beta-distributed advantage gate that predicts a per-sample multiplier
# in (0, 1) from value features and the sampled native action. The gate is NOT
# trained to imitate rankgauss. It is trained by a first-order meta objective:
# split each PPO minibatch, compute the standard PPO actor gradient on the held
# out split, then train the gate so its weighted actor gradient on the train
# split aligns with that held-out gradient. The actual actor update uses the
# detached gate mean, preventing direct surrogate-loss collapse. The gate loss
# is attached to the value/critic feature trunk (but not the critic output head)
# so the model can shape the representation used for learned sample weighting.
#
...
class Args:
    ...
    # Meta advantage gate. Predicts a Beta mean in (0, 1) used as a detached
    # multiplier on policy advantages. The gate is trained to make weighted
    # train-split policy gradients align with held-out PPO gradients.
    adv_gate: bool = True
    adv_gate_coef: float = 0.02
    adv_gate_grad_clip: float = 0.25
    adv_gate_action_init: float = 0.1
    adv_gate_eps: float = 1e-8
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Beta advantage gate. It reads critic/value features plus the replayed
        # native action sample and outputs a multiplier mean in (0, 1).
        self.adv_gate_action_proj = layer_init(nn.Linear(act_dim, H), std=args.adv_gate_action_init)
        self.adv_gate_alpha_head = layer_init(nn.Linear(H, 1), std=0.01)
        self.adv_gate_beta_head = layer_init(nn.Linear(H, 1), std=0.01)
    ...
    def get_adv_gate_dist(self, x, z, detach_feat=False):
        _, critic_feat = self._trunks(x)
        if detach_feat:
            critic_feat = critic_feat.detach()
        gate_feat = torch.tanh(critic_feat + self.adv_gate_action_proj(z.detach()))
        alpha = 1.0 + F.softplus(self.adv_gate_alpha_head(gate_feat))
        beta = 1.0 + F.softplus(self.adv_gate_beta_head(gate_feat))
    ...
    def adv_gate_parameters(self, include_backbone=False):
        params = (
            list(self.adv_gate_action_proj.parameters())
            + list(self.adv_gate_alpha_head.parameters())
            + list(self.adv_gate_beta_head.parameters())
        )
        if include_backbone:
            trunk = self.trunk if self.share_backbone else self.critic_trunk
            params = list(trunk.parameters()) + params
        return list(dict.fromkeys(params))


def ppo_actor_loss_from_adv(advantages, ratio, clip_lo, clip_hi):
    pg_loss1 = -advantages * ratio
    pg_loss2 = -advantages * torch.clamp(ratio, 1 - clip_lo, 1 + clip_hi)
    return torch.max(pg_loss1, pg_loss2).mean()


def gradient_alignment_loss(weighted_train_loss, valid_loss, actor_params, eps):
    ...
    dot = valid_loss.new_zeros(())
    train_norm = valid_loss.new_zeros(())
    valid_norm = valid_loss.new_zeros(())
    for train_grad, valid_grad in zip(train_grads, valid_grads):
        if train_grad is None or valid_grad is None:
            continue
        valid_grad = valid_grad.detach()
        dot = dot + (train_grad * valid_grad).sum()
        train_norm = train_norm + train_grad.pow(2).sum()
        valid_norm = valid_norm + valid_grad.pow(2).sum()
    return -(dot / (train_norm.sqrt() * valid_norm.sqrt() + eps))
...
if __name__ == "__main__":
    ...
    assert not (args.adv_gate and args.norm_adv and args.norm_adv_scope == "batch"), \
        "adv_gate v1 recomputes gate weights per minibatch; use minibatch norm_adv_scope"
    ...
    adv_gate_params = agent.adv_gate_parameters(include_backbone=True)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.adv_gate:
                    gate_dist = agent.get_adv_gate_dist(b_obs[mb_inds], b_latent_zs[mb_inds])
                    gate_mean = gate_dist.mean.squeeze(-1)
                    half = mb_advantages.numel() // 2
                    train_slice = slice(0, half)
                    valid_slice = slice(half, mb_advantages.numel())
                    weighted_train_loss = ppo_actor_loss_from_adv(
                        mb_advantages[train_slice] * gate_mean[train_slice],
                        ratio[train_slice],
                        args.clip_coef,
                        clip_hi,
                    )
                    valid_loss = ppo_actor_loss_from_adv(
                        mb_advantages[valid_slice],
                        ratio[valid_slice],
                        args.clip_coef,
                        clip_hi,
                    )
                    gate_loss = gradient_alignment_loss(
                        weighted_train_loss,
                        valid_loss,
                        actor_params,
                        args.adv_gate_eps,
                    )
                    mb_advantages = mb_advantages * gate_mean.detach()
                    gate_losses.append(gate_loss.detach().item())
                    gate_means.append(gate_mean.detach().mean().item())
                    gate_alignments.append((-gate_loss.detach()).item())
                ...
                pg_loss = ppo_actor_loss_from_adv(mb_advantages, ratio, args.clip_coef, clip_hi)
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    if args.adv_gate:
                        (args.adv_gate_coef * gate_loss).backward(retain_graph=True)
                        gate_gn = nn.utils.clip_grad_norm_(adv_gate_params, args.adv_gate_grad_clip)
                        gate_grads = [(p, p.grad.detach().clone()) for p in adv_gate_params if p.grad is not None]
                        optimizer.zero_grad(set_to_none=True)
                    else:
                        gate_gn = torch.zeros((), device=device)
                    (pg_loss - ent_coef_eff * entropy_loss).backward()
                    actor_gn = nn.utils.clip_grad_norm_(actor_params, args.actor_grad_clip)
                    # trunk.grad currently = clip_actor(d pg / d trunk); add the
                    # stashed clip_critic(d vl / d trunk) and clip_gate(d meta / d
                    # trunk). Critic head gets value grad only; gate heads get gate
                    # grad only.
                    for p, g in value_grads:
                        p.grad = g if p.grad is None else p.grad + g
                    if args.adv_gate:
                        for p, g in gate_grads:
                            p.grad = g if p.grad is None else p.grad + g
                    optimizer.step()
                else:
                    loss = (
        ...
        writer.add_scalar("losses/adv_gate_grad_norm", float(gate_gn), global_step)
        if args.adv_gate and gate_losses:
            writer.add_scalar("losses/adv_gate_loss", float(np.mean(gate_losses)), global_step)
            writer.add_scalar("debug/adv_gate_mean", float(np.mean(gate_means)), global_step)
            writer.add_scalar("debug/adv_gate_alignment", float(np.mean(gate_alignments)), global_step)
