# PPO + LEARNED-AWR ADVANTAGE WEIGHT, REALIZED HYPERGRADIENT trainer (v7_hyper).
#
...
#
# WHY SOFTMAX (v4/v5 -> v6/v7). v4/v5 used a mean-1 MULTIPLICATIVE weight. Empirically inert
...
# mean-1 weight lacked. The realized hypergradient (below) regulates how far to concentrate.
#
# WHY HYPERGRADIENT (v6 -> v7). v6 trained phi by REINFORCE on the realized held-out
# returns difference. That FAILS: w = N*softmax(z) over a batch of B dilutes each sample's
# score-function credit by ~1/B, so the REINFORCE estimate drowns in variance (learnability
# test stalled at corr~0.15 even when the optimum was a direct input feature). The teacher
# (realized held-out returns) is sound; the ESTIMATOR was wrong. Here we make phi's update
# a DETERMINISTIC analytic meta-gradient of the SAME realized objective -- no sampling.
#
# TEACHER + EXACT HYPERGRADIENT. Per minibatch split T (train) / V (held-out). Maximise the
# held-out PPO surrogate after the weighted train step:
#   J_V( theta - a*g_w(phi) ),   g_w(phi) = grad_theta sum_{i in T} w_i(phi) * L_i^PPO
# J_V depends on phi ONLY through theta' = theta - a*g_w(phi), so the chain rule is EXACT and
# closed-form (no functional_call / create_graph needed):
#   d J_V / d phi = h'^T (d theta'/d phi) = -a * grad_phi sum_{i in T} w_i(phi) * (G_i . h')
#   where  G_i = grad_theta L_i^PPO   and   h' = grad_theta J_V  EVALUATED AT theta'.
# So per-sample scalar  c_i = G_i . h'  (directional derivative of L_i along the held-out
# improvement direction, taken AT THE PERTURBED POINT theta'), detached; then phi ascends
#   grad_phi [ - sum_{i in T} w_i(phi) * c_i ]    ( = the meta-gradient, low variance).
#
# WHY THE PERTURBED POINT = the 2nd-order content. h' = grad J_V(theta - a*g_w)
#   ~ h - a*H_V*g_w  (h = grad J_V at theta, H_V = held-out Hessian). The -a*H_V*g_w term
# is exactly the curvature/cross-sample interaction: amplifying a noisy outlier overshoots,
# H_V bends h' away from it, c_i flips sign, the meta-grad PULLS weight back. This is the
# trust-region/anti-overconcentration signal first-order (h at theta, the v7_foa sibling)
# lacks -- v7_foa's softmax has nothing to stop it collapsing onto one sample.
#
# THE WEIGHT HEAD.
...
#   minibatch (real update); the probe/meta-grad use softmax over T only so sum(w_T)=|T|=
#   sum(uniform) and the step compares DIRECTION/concentration, not size. w is detached when
#   applied (mean-1 => only RE-WEIGHTS, no learning-rate confound).
#
...
class Args:
    ...
    lw_probe_lr: float = 3e-3    # probe step size a: theta' = theta - a*g_w (where h' is taken)
    lw_eps: float = 0.01         # finite-difference step for c_i = G_i . h' (dir. deriv of L_i)
...
class Agent(nn.Module):
    ...
    def weight_logit(self, actor_feat, adv, value):
        ...
        The applied weight is w = N * softmax(mu) (deterministic; trained by the
        realized hypergradient, NOT by sampling)."""
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.learnweight:
                    # ===== Learned-AWR softmax weight, REALIZED HYPERGRADIENT =====
                    # Deterministic analytic meta-grad of J_V(theta - a*g_w(phi)).
                    M = len(mb_inds)
                    nV = min(M - 1, max(1, int(round(args.lw_holdout * M))))
                    nT = M - nV
                    T_pos = slice(0, nT)            # train head (gets the weighting + step)
                    V_pos = slice(nT, M)            # held-out tail (defines h')
                    ...
                    # Learned softmax logit mu = phi(feat, A, value). DIFFERENTIABLE in phi
                    # (mu NOT detached); inputs detached (no grad to actor/critic).
                    mu = agent.weight_logit(actor_feat1.detach(), adv1.detach(), value_s)  # (M,)

                    # Per-sample PPO TRAIN loss on T (graph -> actor params).
                    ...
                    # g_w = grad_theta sum_i w_i*L_i with w detached at the CURRENT softmax over T.
                    # This is the actual step we PROBE; phi's dependence enters analytically below.
                    # sum(w_T)=|T|=sum(uniform) so the step probes DIRECTION/concentration, not size.
                    w_T_det = nT * torch.softmax(mu.detach()[T_pos], dim=0)      # mean-1 over T
                    L_w = (w_T_det * loss_T).mean()
                    g_w = torch.autograd.grad(L_w, actor_params, retain_graph=False, allow_unused=True)
                    g_w = [torch.zeros_like(p) if g is None else g for p, g in zip(actor_params, g_w)]

                    # h' = grad_theta J_V evaluated AT theta' = theta - a*g_w (the 2nd-order point;
                    # h' ~ h - a*H_V*g_w, the curvature term that regulates over-concentration).
                    a = args.lw_probe_lr
                    ...
                    with torch.no_grad():                       # step theta -> theta'
                        for p, g in zip(actor_params, g_w):
                            p.add_(g, alpha=-a)
                    jv_thetaprime = _surr_V()                   # grad-enabled forward at theta'
                    hprime = torch.autograd.grad(jv_thetaprime, actor_params, allow_unused=True)
                    hprime = [torch.zeros_like(p) if g is None else g for p, g in zip(actor_params, hprime)]
                    with torch.no_grad():                       # restore theta
                        for p, g in zip(actor_params, g_w):
                            p.add_(g, alpha=a)

                    # c_i = G_i . hat(h'): directional derivative of L_i (at theta) along the unit
                    # held-out direction, via one finite-difference forward at theta + eps*hat(h').
                    ...
                    with torch.no_grad():
                        ...
                        lp_p = agent.lw_logprob(obs_T, z_T)
                        r_p = (lp_p - oldlp_T).exp()
                        pg1_p = adv_T * r_p
                        pg2_p = adv_T * torch.clamp(r_p, 1 - args.clip_coef, 1 + clip_hi)
                        loss_T_pert = -torch.min(pg1_p, pg2_p)
                        for p, g in zip(actor_params, hprime):
                            p.add_(g, alpha=-scale)             # restore
                    c = ((loss_T_pert - loss_T0) / args.lw_eps).detach()    # (nT,) = G_i . hat(h')

                    # DETERMINISTIC meta-gradient on phi. J_V improvement (1st-order at theta')
                    #   = -a*|h'| * sum_i w_i(phi)*c_i   (a,|h'| > 0 consts => folded into lr).
                    # Maximise it => minimise lw_loss = mean_i w_i(phi)*c_i (softmax diff in phi).
                    w_T_phi = nT * torch.softmax(mu[T_pos], dim=0)          # mean-1 over T, grad to phi
                    ...
                    with torch.no_grad():
                        # APPLIED weight for the real update: softmax over the FULL minibatch.
                        w_full = M * torch.softmax(mu.detach(), dim=0)      # mean-1 over M
                        lw_loss_val = lw_loss.item()
                        lw_w_std = w_full.std().item()
                        # Realized improvement of THIS weighting OVER uniform (1st-order, at theta').
                        # >0 => weighting beats uniform on the held-out surrogate. (w_T-1) has mean 0.
                        lw_reward = -((w_T_phi.detach() - 1.0) * c).mean().item()
                        lw_entropy = mu.std().item()    # spread of learned logits (0 => uniform)
                        # ESS fraction of the learned weighting: (sum w)^2 / (n sum w^2).
                        # 1 => uniform; ->0 => mass collapsed onto a few samples. The curvature
                        # term in c should keep this from collapsing (unlike v7_foa first-order).
                        lw_ess = (w_full.sum() ** 2 / (w_full.pow(2).sum() + 1e-8) / w_full.numel()).item()
                        # DECISIVE diagnostic: corr(applied w, |A|). NEGATIVE => compresses
                        # outliers (rankgauss-like). POSITIVE => amplifies them.
