# PPO + LEARNED ADVANTAGE WEIGHT as a STOCHASTIC BETA meta-policy (v3).
#
...
#
# WHY A BETA (the v1->v3 change). v1 made w_i a DETERMINISTIC head trained by a linear
# objective -(w*A*s). That is brittle: it commits a point estimate every minibatch to a
# NOISY reward (s_i is a one-sample finite-difference of a one-minibatch direction), and
# the actor sees a razor-sharp reweighting. v3 makes the weight a STOCHASTIC meta-policy
# with the SAME template as the actor -- a Beta whose own sampling IS the exploration
...
#   alpha,beta = 1 + softplus(head([actor_feat, A]))   (unimodal Beta, like the actor)
#   b_i ~ Beta(alpha_i, beta_i) in (0,1);  w_i = b_i * N / sum(b)   (mean-1, APPLIED detached)
# The reward for a drawn weight is its first-order held-out policy improvement:
...
#   L_w = - mean_{i in T}( log pi_w(b_i) * (r_i - mean r) )  -  lw_ent * H[Beta]
# i.e. push the Beta toward weights that helped V, keep entropy (noise) alive so it never
# collapses to a brittle deterministic weighting. The actor takes its normal PPO step with
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if args.learnweight:
            ...
            self.weight_body = nn.Sequential(nn.Linear(H + 1, args.lw_hidden), nn.Tanh())
    ...
    def weight_dist(self, actor_feat, adv):
        """Beta meta-policy over the per-sample weight, from [detached actor feature,
        advantage]. Unimodal (alpha,beta >= 1), same template as the actor."""
        h = torch.cat([actor_feat, adv.unsqueeze(-1)], dim=-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.learnweight:
                    ...
                    # Forward #1 (signal graph): logpi, actor feature at current params.
                    logp1, _, actor_feat1 = agent.lw_forward(b_obs[mb_inds], b_latent_zs[mb_inds])
                    ...
                    # Beta weight meta-policy over the full minibatch. Sample b (the noise),
                    # map to a softmax-normalized mean-1 weight via the logit (so the Beta's
                    # mean sets each sample's up/down lean and exp-tilting nests AWR).
                    dist_w = agent.weight_dist(actor_feat1.detach(), adv1.detach())
                    b_w = dist_w.sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)     # (M,), detached
                    logit_w = torch.log(b_w) - torch.log1p(-b_w)
                    w_full = (torch.softmax(logit_w, dim=0) * M).detach()         # mean 1, APPLIED detached
