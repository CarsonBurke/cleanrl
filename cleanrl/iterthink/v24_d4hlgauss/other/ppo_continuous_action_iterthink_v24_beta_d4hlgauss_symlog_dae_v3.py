# PPO + IterThink v24 beta d4-HL-Gauss symlog + DIRECT ADVANTAGE ESTIMATION (DAE) v3.
#
# v3 FIXES v2's centering noise (the real reason v2 stalled at ~450 on HalfCheetah).
...
class Args:
    ...
    dae_coef: float = 1.0            # weight (beta_V analog) on the joint DAE squared loss
    dae_grad_clip: float = 0.25      # unused in v2 (dual-backward; value+advantage share one clip group)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # NATIVE action z, mixes them through a tanh, reads out a scalar. Small
        # readout init so A_hat ~= 0 at start (the centered advantage begins near
        # zero and is grown by the DAE loss). The centering E_mu[f(s,a')] makes the
        # network's RANKING/MAGNITUDE over actions the learned advantage.
        self.adv_action_proj = layer_init(nn.Linear(act_dim, H))
        self.adv_head = layer_init(nn.Linear(H, 1), std=0.01)
    ...
    def _adv_f(self, critic_feat, z):
        ...
        h = torch.tanh(critic_feat + self.adv_action_proj(z))
    ...
    def _adv_f_multi(self, critic_feat, z_multi):
        ...
        h = torch.tanh(critic_feat.unsqueeze(1) + self.adv_action_proj(z_multi))
        return self.adv_head(h).squeeze(-1)
...
if __name__ == "__main__":
    ...
    # v2: value and advantage are ONE joint DAE objective -> one gradient group
    # (shared trunk + HL-Gauss value head + scalar advantage head).
    value_adv_params = (
        agent.critic_parameters()
        + list(agent.adv_action_proj.parameters())
        + list(agent.adv_head.parameters())
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, n_segments, seg_per_mb):
                ...
                # STOP-GRADIENT, used RAW (paper Eq 49; no advantage normalization). ---
                ...
                if args.separate_grad_clip:
                    # DUAL-BACKWARD decoupled clipping: the joint value+advantage DAE
                    # objective and the policy objective are each backpropped and clipped
                    # to their own max-norm, then summed on the shared trunk.
                    optimizer.zero_grad(set_to_none=True)
                    (args.dae_coef * dae_loss + args.vf_coef * vdist_loss).backward(retain_graph=True)
                    critic_gn = nn.utils.clip_grad_norm_(value_adv_params, args.critic_grad_clip)
                    value_grads = [(p, p.grad.detach().clone()) for p in value_adv_params if p.grad is not None]
                    optimizer.zero_grad(set_to_none=True)
