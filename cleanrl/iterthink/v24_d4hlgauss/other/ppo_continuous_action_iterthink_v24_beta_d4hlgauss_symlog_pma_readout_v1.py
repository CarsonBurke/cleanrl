# PPO + IterThink v24 beta d4-HL-Gauss symlog + PMA depth readout v1.
#
# Variant hypothesis: keep the high-performing symlog_v1 PPO machinery unchanged,
# but replace the single concat->projection readout with Set-Transformer PMA over
# the IterThink depth states. Tokens are the natural staged features
# [x0, block1, ..., blockK], not obs fields or expert internals. Two learned seed
# queries read the same shared token set: seed 0 -> actor feature, seed 1 -> critic
# feature. This preserves the successful shared backbone + decoupled grad clipping
# while letting policy/value heads select different computation depths.
#
# A small learned residual gate keeps the initial feature close to the proven
# concat readout, so the ablation tests actor/critic readout specialization without
# deliberately shocking PPO at initialization. Attention entropy and per-depth means
# are logged to diagnose whether PMA actually specializes or degenerates to the old
# shared readout.
#
...
class Args:
    ...
    k_blocks: int = 3
    n_experts: int = 16
    readout_heads: int = 4           # PMA heads over the K+1 depth tokens
    readout_resid_init: float = -4.0 # sigmoid gate on PMA delta; starts near baseline readout
...
class PMADepthReadout(nn.Module):
    """PMA readout over ordered IterThink depth tokens.

    The baseline concat readout remains as an initialization anchor. PMA produces
    actor/critic-specific deltas from two learned seed queries, and a small learned
    gate lets the model move away from the shared baseline feature when useful.
    """

    def __init__(self, H, n_tokens, n_heads, resid_init):
        super().__init__()
        assert H % n_heads == 0, "hidden must be divisible by readout_heads"
        self.n_tokens = n_tokens
        self.token_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.depth_emb = nn.Parameter(torch.zeros(n_tokens, H))
        self.seeds = nn.Parameter(torch.randn(2, H) / sqrt(H))
        self.attn = nn.MultiheadAttention(H, n_heads, batch_first=True)
        self.seed_norm = nn.LayerNorm(H)
        self.ff = nn.Sequential(
            layer_init(nn.Linear(H, H)),
            nn.GELU(),
            layer_init(nn.Linear(H, H), std=1.0),
        )
        self.out_norm = nn.LayerNorm(H)
        self.resid_gate = nn.Parameter(torch.full((2, H), resid_init))
        self._attn_entropy = None
        self._attn_mean = None
        self._attn_entropy_sum = None
        self._attn_mean_sum = None
        self._attn_count = 0

    def consume_stats(self):
        if self._attn_count == 0:
            return None, None
        entropy = self._attn_entropy_sum / self._attn_count
        mean = self._attn_mean_sum / self._attn_count
        self._attn_entropy_sum = None
        self._attn_mean_sum = None
        self._attn_count = 0
        return entropy, mean

    def forward(self, tokens, base_feat):
        x = self.token_norm(tokens) + self.depth_emb.unsqueeze(0)
        seeds = self.seeds.unsqueeze(0).expand(tokens.shape[0], -1, -1)
        ...
        h = self.seed_norm(seeds + pooled)
        delta = self.out_norm(h + self.ff(h))
        gate = torch.sigmoid(self.resid_gate).unsqueeze(0)

        with torch.no_grad():
            p = attn.detach().clamp_min(1e-8)
            self._attn_entropy = -(p * p.log()).sum(dim=-1).mean()
            self._attn_mean = attn.detach().mean(dim=(0, 1))
            if self._attn_entropy_sum is None:
                self._attn_entropy_sum = self._attn_entropy.clone()
                self._attn_mean_sum = self._attn_mean.clone()
            else:
                self._attn_entropy_sum = self._attn_entropy_sum + self._attn_entropy
                self._attn_mean_sum = self._attn_mean_sum + self._attn_mean
            self._attn_count += 1

        return base_feat.unsqueeze(1) + gate * delta


class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, n_experts, readout_heads, readout_resid_init):
        super().__init__()
        self.K = K
        ...
        self.readout = PMADepthReadout(H, K + 1, readout_heads, readout_resid_init)
    ...
    def forward(self, x):
        ...
        base_feat = self.out_proj(self.out_norm(cat))
        tokens = torch.stack(feats, dim=1)
        pooled = self.readout(tokens, base_feat)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.share_backbone:
            readout = agent.trunk.readout
            attn_entropy, attn_mean = readout.consume_stats()
            if attn_entropy is not None:
                writer.add_scalar("debug/pma_attn_entropy", attn_entropy.item(), global_step)
                gate = torch.sigmoid(readout.resid_gate.detach()).mean(dim=1)
                writer.add_scalar("debug/pma_actor_gate", gate[0].item(), global_step)
                writer.add_scalar("debug/pma_critic_gate", gate[1].item(), global_step)
                for seed_idx, seed_name in enumerate(("actor", "critic")):
                    for depth_idx in range(args.k_blocks + 1):
                        writer.add_scalar(
                            f"debug/pma_{seed_name}_depth{depth_idx}",
                            attn_mean[seed_idx, depth_idx].item(),
                            global_step,
                        )
