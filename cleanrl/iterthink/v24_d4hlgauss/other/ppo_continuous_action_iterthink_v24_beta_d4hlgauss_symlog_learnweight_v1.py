# PPO + LEARNED ADVANTAGE WEIGHT via first-order policy-improvement signal (v1).
#
# GOAL. Replace the STATIC rankgauss advantage transform with a LEARNED, per-sample
# weight head g_phi that multiplies the (standardized) advantage in the PPO
# surrogate: effective adv = w_i * A_i. A weight reproduces rankgauss exactly when
# w_i = rankgauss(A_i)/A_i, so this is a strict generalization -- but we do NOT
# distill rankgauss; the head learns the weighting itself, on-policy.
#
# TRAINING SIGNAL -- "policy-delta loss" (held-out, first-order, NON-meta).
# We want g_phi trained on how much its weighting IMPROVES the policy. The naive
# same-batch version degenerates (the biggest same-batch loss drop is just w->inf,
# a learning-rate amplifier). The fix is HELD-OUT improvement: split each minibatch
# into T (train) and V (held-out). The policy update direction from weighting T is
#   delta_theta ~ sum_{i in T} w_i A_i grad log pi_i.
# Its first-order effect on the held-out PPO surrogate J_V is
#   Delta J_V ~ sum_i w_i A_i s_i,   s_i = grad_theta log pi_i . h,   h = grad_theta J_V.
# s_i ("does pushing sample i help the held-out batch?") is obtained with ONE extra
# forward by finite-difference along h (no second-order graph -- this is the TracIn /
# influence-function trick, not full meta). The weight-head loss is then
#   L_phi = - mean_{i in T}( w_i * A_i * s_i ) + lw_reg * mean((w-1)^2),
# i.e. g_phi learns to UP-WEIGHT advantages whose gradient GENERALIZES to improving
# the policy, and down-weight those that don't -- a dynamic, learned rankgauss.
# The policy then takes its normal PPO step with w detached (mean-normalized to 1 so
# the head only RE-WEIGHTS, never rescales the step = no learning-rate confound).
#
# Why this dodges the earlier dead-ends: held-out breaks the null-gradient/herd trap;
# first-order finite-diff avoids second-order meta; the weight is learned from actual
# policy improvement, NOT from critic uncertainty (a rare/low-EV state is weighted by
# whether learning from it helps, never by the critic's confidence).
#
...
class Args:
    ...
    # ---- Learned advantage weight via first-order policy-improvement signal ----
    ...
    lw_reg: float = 0.1          # L2 pull of weights toward 1 (keeps weighting from collapsing)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if args.learnweight:
            self.weight_head = nn.Sequential(
                nn.Linear(H + 1, args.lw_hidden),
                nn.Tanh(),
                layer_init(nn.Linear(args.lw_hidden, 1), std=0.01, bias_const=0.0),
            )
    ...
    def weight_raw(self, actor_feat, adv):
        """Positive per-sample weight from [detached actor feature, advantage]."""
        h = torch.cat([actor_feat, adv.unsqueeze(-1)], dim=-1)
        return F.softplus(self.weight_head(h)).squeeze(-1)
...
if __name__ == "__main__":
    ...
    main_params = [p for n, p in agent.named_parameters() if not n.startswith("weight_head")]
    ...
    if args.learnweight:
        weight_optimizer = optim.Adam(agent.weight_head.parameters(), lr=args.lw_lr, eps=1e-5)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                lw_loss_val = lw_w_std = lw_corr_ws = lw_s_mean = lw_ess = lw_corr_w_absA = 0.0
                if args.learnweight:
                    # ===== Learned advantage weight via held-out policy improvement =====
                    M = len(mb_inds)
                    ...
                    # weight-head loss: up-weight advantages whose gradient generalizes
                    # to improving the held-out surrogate; L2-pull to 1 prevents collapse.
                    w_raw_T = agent.weight_raw(actor_feat1[T_pos].detach(), adv1[T_pos].detach())
                    w_T = w_raw_T * (w_raw_T.numel() / (w_raw_T.sum() + 1e-8))    # mean 1
                    lw_loss = -(w_T * adv1[T_pos].detach() * s).mean() + args.lw_reg * ((w_T - 1.0) ** 2).mean()
                    weight_optimizer.zero_grad(set_to_none=True)
                    lw_loss.backward()
                    weight_optimizer.step()

                    with torch.no_grad():
                        w_raw = agent.weight_raw(actor_feat1.detach(), adv1.detach())
                        w_full = w_raw * (w_raw.numel() / (w_raw.sum() + 1e-8))   # mean 1, detached
                        lw_loss_val = lw_loss.item()
                        lw_w_std = w_full.std().item()
                        lw_s_mean = s.mean().item()
                        ws = torch.stack([w_T.detach(), adv1[T_pos].detach() * s])
                        lw_corr_ws = torch.corrcoef(ws)[0, 1].item() if ws.shape[1] > 1 else 0.0
                        ...
                        # The DECISIVE diagnostic: corr(learned w, |A|). NEGATIVE => the head
                        # compresses outliers (rankgauss-like, the goal). POSITIVE => it
                        # AMPLIFIES high-|A| samples (the adversarial failure mode).
        ...
        if args.learnweight:
            ...
            writer.add_scalar("lw/corr_w_vs_As", lw_corr_ws, global_step)
            writer.add_scalar("lw/s_mean", lw_s_mean, global_step)
