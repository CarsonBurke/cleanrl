# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + LEARNED ADVANTAGE v1.
#
# KEY IDEA. Replace the GAE-based POLICY advantage with a LEARNED, action-
# conditioned advantage A(s,a) = Q(s,a) - V(s). Q is a NEW distributional head
# that mirrors the critic (same HL-Gauss support / peaked init) but reads the
# critic feature plus a projection of the sampled native action z. It is GROUNDED
# by the PRESENT value loss: cross-entropy to the SAME HL-Gauss return target the
# critic already regresses (`target_probs`), NOT regressed to GAE directly.
#
# The GAE / lambda-return is retained ONLY as the value/Q regression TARGET
# (`returns` -> `target_probs`); it is NOT used as the policy advantage when
# `learned_adv` is on. A_used = blend*A_learned + (1-blend)*A_GAE (blend=1 =>
# pure learned). No meta-learning, no held-out split.
#
# RATIONALE. A value-grounded advantage is GAE-family in its TARGET (both chase
# the lambda-return), so this is not a different objective. The learned head's
# genuine addition over GAE is COUNTERFACTUAL action-generalization: Q(s,a)
# estimates the value of actions including unsampled ones, letting A vary with the
# action argument rather than only with the realized trajectory.
# RISK. If Q simply memorizes the sampled return at the sampled action, A
# degenerates to the Monte-Carlo advantage (return - V), i.e. high-variance GAE
# with lambda~1. The qv_gap / corr_with_gae diagnostics watch for this.
#
# Base: ppo_continuous_action_iterthink_v24_beta_d4hlgauss_symlog_v1.py.
# Only intended algorithmic change: learned action-conditioned policy advantage.
#
...
class Args:
    ...
    # LEARNED action-conditioned advantage. A(s,a) = Q(s,a) - V(s), where Q is a
    # distributional head grounded by the SAME HL-Gauss return target as V (CE),
    # NOT regressed to GAE. GAE/lambda-return is kept only as the value/Q target.
    learned_adv: bool = True              # use the learned Q-V advantage for the policy
    learned_adv_blend: float = 1.0        # A_used = blend*A_learned + (1-blend)*A_GAE (1=pure learned)
    q_action_init: float = 0.1            # std for the Q action-projection init
    q_vf_coef: float = 0.5                # weight on the Q cross-entropy loss (mirror vf_coef)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        with torch.no_grad():
            ...
            peaked_bias = -0.5 * (z / args.critic_init_tau) ** 2
            self.critic_head.bias.copy_(peaked_bias)
        # LEARNED action-conditioned Q head: mirrors the critic (same support /
        # peaked Gaussian-logit bias init), but reads the critic feature combined
        # with a projection of the native action sample z. Trained by the value-CE
        # gradient (q_loss) only -> sits in the critic clip group.
        self.q_action_proj = layer_init(nn.Linear(act_dim, H), std=args.q_action_init)
        self.q_head = layer_init(nn.Linear(H, args.num_bins), std=0.1)
        with torch.no_grad():
            self.q_head.bias.copy_(peaked_bias)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        # Action-conditioned Q logits: critic feature + projected native action z.
        # z is detached so the policy gets no gradient through the Q head (Q is a
        # value estimator; the policy advantage A=Q-V is detached at the use site).
        # During rollout z is sampled under no_grad; on the update pass z is the
        # buffered replay sample (a constant) -> the detach is a no-op-safe guard.
        q_feat = torch.tanh(critic_feat + self.q_action_proj(z.detach()))
        q_logits = self.q_head(q_feat)
        ...
        return action, z, log_prob, entropy, value_logits, q_logits
    ...
    def critic_parameters(self):
        ...
        # and its action projection are trained by the value-CE gradient (q_loss),
        # so they belong to the critic clip group.
        trunk = self.trunk if self.share_backbone else self.critic_trunk
        return (
            list(trunk.parameters())
            + list(self.critic_head.parameters())
            + list(self.q_head.parameters())
            + list(self.q_action_proj.parameters())
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # LEARNED action-conditioned advantage A = Q(s,a) - V(s). Q and V
                # are converted to scalars off the SAME HL-Gauss support; A is
                # DETACHED (it is a target weight for the PG, not differentiated).
                q_value = hl_support.to_scalar(q_logits)
                v_value_mb = hl_support.to_scalar(value_logits)
                a_learned = (q_value - v_value_mb).detach()
                a_learned_normed = a_learned
                if args.norm_adv:
                    # Standardize over the minibatch (no rankgauss): keep the
                    # learned advantage's raw magnitude structure, just rescale it.
                    a_learned_normed = (a_learned - a_learned.mean()) / (a_learned.std() + 1e-8)
                # Diagnostic: does the learned advantage actually differ from GAE?
                # Standardize both and take the mean product = Pearson correlation.
                with torch.no_grad():
                    gae_z = (mb_advantages - mb_advantages.mean()) / (mb_advantages.std() + 1e-8)
                    lrn_z = (a_learned - a_learned.mean()) / (a_learned.std() + 1e-8)
                    learnedadv_corr = (gae_z * lrn_z).mean().item()
                    last_learned_adv_mean = a_learned.mean().item()
                    last_learned_adv_std = a_learned.std().item()
                    last_qv_gap_absmean = (q_value - v_value_mb).abs().mean().item()
                    last_learnedadv_corr = learnedadv_corr
                # Replace the GAE policy advantage with the learned (blended) one so
                # ALL downstream PG transforms (pos_neg, clip, pg_loss) pick it up.
                # learned_adv=False keeps the unchanged GAE baseline behavior.
                if args.learned_adv:
                    a_used = (
                        args.learned_adv_blend * a_learned_normed
                        + (1.0 - args.learned_adv_blend) * mb_advantages
                    )
                    mb_advantages = a_used
                ...
                # Q GROUNDING: same HL-Gauss return target as the critic (CE), NOT
                # GAE. This anchors Q on the value-target scale; its only genuine
                # addition over V is action-conditioning (counterfactual generalization).
                ...
                if args.separate_grad_clip:
                    ...
                    # Fold the Q grounding loss into the critic backward so it flows
                    # to q_head/q_action_proj and the shared trunk, all covered by
                    # the critic clip group (critic_params now includes the Q params).
                    ((args.vf_coef * v_loss) + (args.q_vf_coef * q_loss)).backward(retain_graph=True)
