# PPO + LEARNED ADVANTAGE WEIGHT as a STOCHASTIC BETA meta-policy (v4).
#
...
# on-policy from how much it IMPROVES the policy.
#
# WHY A BETA. v1 made w_i DETERMINISTIC, trained by a linear -(w*A*s): brittle (commits a
# point estimate every minibatch to a NOISY reward). v3+ makes the weight a STOCHASTIC
# meta-policy with the SAME template as the actor -- a Beta whose own sampling IS the
# noise -- trained by log-likelihood (REINFORCE), so it learns "weights like this paid
# off" rather than overfitting c_i, and the actor averages over noisy weights.
#
# v3 -> v4 CHANGES. v3 used softmax(logit(b)) and inputs [actor_feat, A]; it
# OVER-CONCENTRATED (ESS ~10%, weight_std 2-10) and was under-informed. v4:
#   1. LINEAR mean-1 normalization  w_i = b_i * N / sum(b)  -- bounded by the Beta
#      range, so it cannot exp-over-concentrate the way softmax did.
#   2. RICHER inputs. The reward A_i*s_i depends on the SAMPLED ACTION (s_i =
#      grad logpi(z_i|s).h), which v3 could not see. Head input is now
#      [actor_feat, advantage, value, sampled_action z]  (no entropy bonus -- the
#      Beta sampling is the noise, like the actor's ent_coef=0).
#
# THE WEIGHT META-POLICY.
#   alpha,beta = 1 + softplus(head([actor_feat, A, value, z]))   (unimodal Beta)
#   b_i ~ Beta(alpha_i, beta_i) in (0,1);  w_i = b_i * N / sum(b)   (mean-1, APPLIED detached)
# Reward = first-order held-out policy improvement:
#   split each minibatch T (train) / V (held-out);  h = grad_theta J_V (held-out PPO surrogate);
#   s_i = grad_theta log pi_i . h  (ONE finite-difference forward -- TracIn/influence, NOT meta);
#   r_i = w_i * A_i * s_i   (realized contribution of sample i to held-out improvement).
# REINFORCE on the weight policy (advantage = reward - baseline):
#   L_w = - mean_{i in T}( log pi_w(b_i) * (r_i - mean r) )
# i.e. push the Beta toward weights that helped V. The actor takes its normal PPO step with
# the sampled w detached (mean-1 => only RE-WEIGHTS, no learning-rate confound).
#
# Why this dodges the earlier dead-ends: held-out breaks the null-gradient/herd trap;
# first-order finite-diff avoids second-order meta; stochastic Beta + log-likelihood
# avoids the v1 brittleness; the weight is learned from actual policy improvement, NOT
# from critic uncertainty.
#
...
class Args:
    ...
    lw_holdout: float = 0.5      # fraction of each minibatch held out (V) for the signal
    lw_eps: float = 0.01         # finite-difference step along the unit held-out direction
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                lw_loss_val = lw_w_std = lw_ess = lw_corr_w_absA = lw_entropy = lw_adv_w = 0.0
                if args.learnweight:
                    # ===== Stochastic Beta weight meta-policy + REINFORCE =====
                    M = len(mb_inds)
                    nV = min(M - 1, max(1, int(round(args.lw_holdout * M))))
                    T_pos = slice(0, M - nV)         # weighted-signal (train) head
                    V_pos = slice(M - nV, M)         # held-out tail
                    ...
                    # held-out PPO surrogate to MAXIMIZE; h = grad wrt actor params (the
                    # direction that improves the policy on V). No retain: this graph is
                    # consumed here so the in-place finite-diff below cannot corrupt it.
                    ...
                    h = torch.autograd.grad(surr_V, actor_params, allow_unused=True)
                    h = [torch.zeros_like(p) if g is None else g for p, g in zip(actor_params, h)]
                    h_norm = torch.sqrt(sum((g * g).sum() for g in h)) + 1e-12

                    # first-order alignment s_i = grad logpi_i . (h/|h|) via finite diff
                    # (one perturbed forward; NO second-order graph).
                    with torch.no_grad():
                        scale = (args.lw_eps / h_norm).item()
                        for p, g in zip(actor_params, h):
                            p.add_(g, alpha=scale)
                        logp_pert = agent.lw_logprob(b_obs[mb_inds][T_pos], b_latent_zs[mb_inds][T_pos])
                        for p, g in zip(actor_params, h):
                            p.add_(g, alpha=-scale)
                    s = (logp_pert - logp1[T_pos].detach()) / args.lw_eps        # detached

                    # Beta weight meta-policy over the full minibatch. Inputs (all detached:
                    # no grad to actor/critic) include the SAMPLED ACTION z, which the reward
                    # A*s depends on. value = critic scalar at this state.
                    ...
                    # LINEAR mean-1 normalization (bounded: cannot over-concentrate like softmax).
                    w_full = (b_w * (M / (b_w.sum() + 1e-8))).detach()           # mean 1, APPLIED detached

                    # REINFORCE on T: reward = realized held-out contribution w_i*A_i*s_i;
                    # raise the Beta toward weights that beat the batch-mean contribution.
                    reward_T = (w_full[T_pos] * adv1[T_pos].detach() * s)         # detached
                    adv_w = reward_T - reward_T.mean()                           # baseline = mean
                    logp_w_T = dist_w.log_prob(b_w)[T_pos]
                    ent_w = dist_w.entropy()                                      # diagnostic only
                    lw_loss = -(logp_w_T * adv_w).mean()
                    ...
                    with torch.no_grad():
                        ...
                        # outliers (rankgauss-like, the goal). POSITIVE => amplifies them.
        ...
        if args.learnweight:
            ...
            # corr_ws: corr(learned w, A*s) over the train split -> is the head actually
            #          tracking which advantages help the held-out surrogate?
            writer.add_scalar("lw/loss", lw_loss_val, global_step)
            writer.add_scalar("lw/weight_std", lw_w_std, global_step)
            writer.add_scalar("lw/adv_w_absmean", lw_adv_w, global_step)
