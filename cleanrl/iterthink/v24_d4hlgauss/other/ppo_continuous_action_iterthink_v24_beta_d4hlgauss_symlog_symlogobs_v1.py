# PPO + IterThink v24 beta d4-HL-Gauss symlog-observation critic v1.
#
# Variant hypothesis: remove Gym's stateful observation normalization/clipping
# and feed a fixed symlog transform of raw flattened MuJoCo observations instead.
# This keeps reward normalization/clipping, value support, PPO, and the beta
# actor matched to the source run while testing whether symlog obs can replace
# NormalizeObservation's running RMS without stale-stat or cross-run effects.
#
...
def symlog_obs(obs):
    return np.sign(obs) * np.log1p(np.abs(obs))


def transform_observation(env, fn):
    try:
...
def make_env(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        ...
        env = transform_observation(env, symlog_obs)
