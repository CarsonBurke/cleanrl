...
# the `iterthink_v24_beta_s1` run). The ONLY change is the actor update: replace the PPO
# clipped surrogate with the paper's FAITHFUL gated score-function (Algorithm 2).
#
#   ell  = action surprisal;  chi = U * ell  ("delight");  w = sigmoid(chi / eta), eta=1.
#   actor loss = -( stopgrad(w) * U * log pi_theta(a|s) ).mean()     # no ratio, no clip.
#
# The gate w in (0,1) reallocates gradient budget toward RARE SUCCESSES (high advantage on a
# tail action) and away from RARE FAILURES, as derived from maximizing chi*w + eta*H(w).
# The gate is DETACHED so only the score function carries gradient.
#
# SCALE (paper-faithful, no extra machinery). The paper's ONLY scale tool is clipping the raw
# log-density to [-10,10]; eta=1 is fixed, U is NOT standardized, chi is NOT whitened, and there
# is no renorm by mean(w) (reward EMA-normalization is in the env wrapper, applied to all). So
# the default surprisal here is the paper-literal  ell = clip(-log pi(a|s), -C, C)  summed over
# action dims (`--dg-surprisal raw_clip`). For this base's unimodal Beta, raw -log pi already
# carries an O(1) std at init (~0.78 over 6 dims), so the clip is the right and sufficient
# treatment. EARLIER MISTAKE (now an opt-in ablation, `--dg-surprisal entref`): subtracting the
# per-state entropy, ell~ = -log pi - H. It leaves the std untouched but re-centers the mean to
# ~0, scrambling sign(chi)=sign(U) -- the breakthrough/blunder asymmetry the gate is built on --
# so it is NOT the default. EMA / batch-whitening of the surprisal or advantage were dead ends
# (the paper does none) and have been removed.
#
...
class Args:
    ...
    dg_surprisal: str = "raw_clip"    # "raw_clip" (paper-literal: clip(-logpi,-C,C)) | "entref" (ablation: -logpi - H)
...
def dg_raw_surprisal(logprob, entropy, args):
    """Raw (un-scaled, un-clipped) surprisal ell. `entref` subtracts the policy's own
    per-state entropy H so the zero is "a typical draw from this state" (a sample-
    independent, parameterization-invariant anchor), not the arbitrary density=1 point.
    For Beta, H is closed-form & differentiable; E_a[-logp]=H => E[ell~]=0 per state."""
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.dg_enable:
                    ...
                    dg_surp_raw = dg_raw_surprisal(newlogprob, entropy, args)
                    if args.dg_surprisal_norm:
