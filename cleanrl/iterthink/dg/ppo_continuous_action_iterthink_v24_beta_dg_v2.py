...
#
#   ell  = action surprisal;  chi = U * ell  ("delight");  w = sigmoid(chi / eta), eta=1.
#   score mode:    loss = -( stopgrad(w) * U * log pi(a|s) )          # faithful Algorithm 2.
#   gate_ppo mode: loss =    stopgrad(w) * PPO_clipped_surrogate      # gate x PPO trust region.
#
# The gate w in (0,1) reallocates gradient budget toward RARE SUCCESSES (high advantage on a
# tail action) and away from RARE FAILURES (derived from maximizing chi*w + eta*H(w)). It is
# DETACHED so only the score / surrogate carries gradient.
#
# v2 HEADLINE -- MODE-REFERENCED Beta surprisal (`--dg-surprisal mode_ref`, DEFAULT).
# v1 used the paper-literal raw_clip surprisal ell = clip(-log pi, -C, C). The gate diagnostics
# exposed a hard Beta-specific failure: for the bounded unimodal Beta the density routinely
# EXCEEDS 1, so raw -log pi is NEGATIVE ~65% of the time (and ~100% once the policy sharpens).
# chi = U*ell then flips sign and the gate INVERTS -- down-weighting rare successes, up-weighting
# rare failures (measured sign_agree fell to 0.26-0.37, worsening as the Beta concentrated).
# The paper's propositions require ell >= 0; that holds for its Gaussian (in its sigma regime),
# not for bounded Beta.
#   FIX:  ell = log pi(mode) - log pi(a)  >= 0   (summed over dims; mode = (a-1)/(a+b-2)).
# This is exactly what the paper's raw -log pi reduces to once the sign-flipping log-normalizer
# is dropped: for a Gaussian, -log pi(a) - (-log pi(mode)) = 0.5||z||^2, the Mahalanobis tail
# distance. Mode-referencing is the faithful generalization of that effective surprisal to
# bounded Beta. It is >= 0 by construction (so sign(chi)=sign(U) holds), is zero at the mode,
# and GROWS with both tail-ness and policy confidence (std ~0.8 at init -> ~1.6 when sharp) --
# the intended "this action was surprising for a now-confident policy" signal. `raw_clip` and
# `entref` remain as ablation flags. eta=1 fixed; U not standardized; no whiten/EMA/renorm.
#
...
class Args:
    ...
    dg_surprisal: str = "mode_ref"    # "mode_ref" (logp(mode)-logp(a)>=0, Beta-faithful) | "raw_clip" (paper-literal) | "entref"
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, z=None):
        ...
        if self.actor_dist == "beta":
            ...
            # NaN-safe: a_c can round to exactly 1.0 in fp32 (softplus underflow) -> 0/0; floor denom.
        ...
        return action, z, log_prob, entropy, value_logits, logp_peak
...
def dg_raw_surprisal(logprob, entropy, args, logp_peak=None):
    """Raw (un-scaled, un-clipped) surprisal ell.

    `mode_ref` (DEFAULT): ell = logp(mode) - logp(a) >= 0. The bounded-Beta analog of the
    Gaussian's Mahalanobis tail distance (= what the paper's raw -logp reduces to once the
    sign-flipping log-normalizer is dropped). Non-negative by construction, so the paper's
    propositions hold: ell>=0 => sign(chi)=sign(U) (breakthrough/blunder asymmetry intact).
    Zero at the mode, grows with tail-ness AND with policy confidence -- the intended signal.

    `raw_clip`: paper-literal ell = -logp. WRONG for Beta -- bounded support lets the density
    exceed 1, so -logp is negative ~65% of the time and INVERTS the gate (sign_agree ~0.3).
    `entref`: ell~ = -logp - H, mean-zero per state but also breaks ell>=0 (allows sign flip)."""
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_logits, logp_peak = agent.get_action_and_value(
                ...
                if args.dg_enable:
                    ...
                    dg_surp_raw = dg_raw_surprisal(newlogprob, entropy, args, logp_peak)
                    if args.dg_surprisal_norm:
