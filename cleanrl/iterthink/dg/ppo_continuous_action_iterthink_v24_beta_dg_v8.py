...
#
# ============================ DG variant (v8) =================================
# v8 = STOP CENTERING THE SURPRISAL. Use the paper's ell verbatim.
# Algorithm 1: ell_t = -log pi(A_t|H_t);  chi_t = U_t * ell_t;  w_t = sigmoid(chi_t / eta);
#              dtheta += w_t * U_t * grad log pi(A_t).  eta=1.  ell clipped to [-10,10].
#
#  THE v3-v7 MISTAKE -- the gate had no magnitude. To "fix" Beta's scale I defaulted the
#  surprisal to concentration-invariant forms (cdf_tail E~1; entref ell=-logpi-H is mean-0).
#  That is COVERT WHITENING: it forces E[ell]~0/1 with tiny spread, so chi = U*ell ~ U*(small)
#  ~ 0, and w = sigmoid(~0) sits in a thin band around 0.5. A gate pinned at ~0.5 is just a
#  constant 0.5x on the score gradient -- the delight reallocation does NOTHING. Measured:
#  with raw U std~0.3 and cdf_tail ell E~1, chi~0.3 => w in [0.43,0.57]. Inert.
#
#  THE FIX -- the paper's ell = -log pi(a|s), SUMMED over action dims (joint surprisal),
#  clipped to [-10,10]. For a 6-dim Beta this is O(several), so chi = U*ell is O(1-3) and the
#  gate ACTUALLY swings [0.05,0.95]: rare success (U>0, high ell) -> w->1 (amplify); rare
#  blunder (U<0, high ell) -> w->0 (suppress); typical action (low ell) -> w~0.5. No whitening,
#  no surprisal-norm, no entropy reference, no per-minibatch chi-whiten -- all OFF.
#  (`--dg-surprisal raw_clip` is the default; cdf_tail/entref/critic/hybrid/mode_ref remain as
#  the CENTERED ablations whose inertness this version diagnoses.)
#
...
class Args:
    ...
    dg_surprisal: str = "raw_clip"    # v8 (PAPER-FAITHFUL): ell = -log pi(a|s) summed over dims, clipped [-10,10] -- gives chi=U*ell real magnitude so the gate swings [0,1]. CENTERED ablations (inert -- chi~0, gate pinned at 0.5): "cdf_tail"(E~1) | "entref"(mean-0) | "critic" | "hybrid" | "mode_ref"
