...
#
# ============================ DG variant (v9) =================================
# v9 = MODE-REFERENCED SURPRISAL. The paper's ell = -log pi INVERTS the gate on a Beta;
# this is the minimal, sign-correct repair that keeps the paper's mechanism and magnitude.
# Algorithm 1: chi_t = U_t * ell_t;  w_t = sigmoid(chi_t / eta);  dtheta += w_t * U_t * grad log pi.
#
#  THE v8 FINDING (paper ell = -log pi, run live, BOTH arms):  the gate INVERTED.
#  sign_agree = frac[ sign(chi)==sign(U) ] = frac[ ell>0 ] collapsed to 0.0 (nogate) / 0.37 (gate),
#  with surprisal_mean -9.5. Cause: a Beta CONCENTRATES, so its sampled actions sit where the
#  density EXCEEDS 1 => -log pi < 0. The paper (Gaussian) implicitly assumes ell>=0 ("rare =>
#  high ell"); on a bounded Beta that assumption fails and chi = U*ell carries the OPPOSITE sign
#  of U -- the gate then SUPPRESSES breakthroughs and AMPLIFIES blunders. Verified in isolation:
#  for Beta concentration {1,5,50}, frac[-log pi >= 0] = {0.32, 0.03, 0.00}.
#
#  THE FIX -- reference the surprisal to the policy's OWN PEAK, not an absolute zero:
#      ell = log pi(mode) - log pi(a)  >= 0   (mode = the per-state max-density action)
#  This is the paper's -log pi with the per-state normalization OFFSET removed. It restores
#  ell>=0 (=> sign(chi)=sign(U), no inversion) at EVERY concentration (frac>=0 = 1.00), while
#  keeping the SAME spread as -log pi (std identical: it subtracts the per-state MAX, a constant,
#  not the MEAN). So unlike the v3-v7 cdf_tail/entref CENTERING (which subtracts the mean/entropy
#  and zeroes the magnitude => gate pinned at 0.5, inert), this preserves real, concentration-
#  RESPONSIVE magnitude: mean(ell) {1.4,2.6,3.0} and std {0.56,1.46,1.71} GROW as the Beta
#  sharpens, so the gate swings [0,1] in the CORRECT direction. For beta log_det_fn=0, so
#  ell = (dist.log_prob(mode) - dist.log_prob(z)).sum is computed in pure native z-space (>=0
#  exactly). Mildly skew-biased on asymmetric Beta -- accepted vs inversion/inertness.
#  (`--dg-surprisal mode_ref` default; raw_clip = paper-literal/inverts; cdf_tail/entref = centered/inert.)
#
#  Carried from v7: raw advantage U=R-b drives BOTH gate and score (adv_transform="v10",
#  norm_adv=False). Faithful single actor epoch (--actor-epochs 1, each sample used once, no
#  ratio/clip/KL-leash) with a decoupled 10-epoch critic refit (--update-epochs 10) so EV stays
#  high without biasing the on-policy actor. KL leash stays OFF (ablation only, --dg-kl-coef>0).
#
# ============================ DG variant (v6) =================================
...
class Args:
    ...
    norm_adv: bool = False           # v7: OFF -> the gate sees raw U=R-b (the paper's Algorithm 1),
    #   not a per-minibatch-standardized advantage. See the v7 header (root cause #1).
    ...
    ent_coef: float = 0.0            # faithful (base/paper-DG use 0). No entropy crutch.
    ...
    dg_surprisal: str = "mode_ref"    # v9: ell = logp(mode)-logp(a) >=0, peak-referenced -- sign-correct (no inversion) AND magnitude-preserving (std grows with concentration). "raw_clip"=paper -logpi (INVERTS on sharp Beta) | "cdf_tail"/"entref"=CENTERED (inert, gate~0.5) | "critic"/"hybrid"
    dg_eta: float = 1.0               # temperature eta in w = sigmoid(chi/eta)
    dg_clip: float = 10.0             # symmetric clip on the surprisal ell/ell~
    # v7 TRUST REGION (score mode): the faithful on-policy analog of the paper's TARGET NETWORK.
    # Penalize KL(pi_old || pi_theta) (k3 estimator (ratio-1)-logratio, >=0, =0 on-policy) so the
    # single faithful score pass cannot diverge (v6 no-gate hit approx_kl~437). This is the
    # PPO-PENALTY / TRPO trust region, NOT the PPO ratio-CLIP -> "no clip" is preserved. beta_kl
    # is auto-tuned (Schulman adaptive-beta) to hold approx_kl ~ target_kl. 0 disables (=v6 score).
    dg_kl_coef: float = 0.0           # OFF by default: faithful Alg.1 with 1 actor epoch needs no
    #   trust region (each sample is used ONCE; the rankgauss/norm_adv bug, not the missing leash,
    #   caused v6's no-gate KL~437). Set >0 only to ablate a PPO-penalty trust region (score mode).
    dg_kl_adapt: bool = True          # adapt beta_kl each iteration toward target_kl (only if coef>0)
    dg_kl_target: float = 0.03        # target approx_kl for the leash (defaults to target_kl scale)
    ...
    adv_transform: str = "v10"        # v7: raw GAE identity -> the gate's chi=U*ell uses the raw
    #   breakthrough/blunder magnitude R-b (Algorithm 1), not rankgauss's batch-RANK. (root cause #1)
...
if __name__ == "__main__":
    ...
    dg_kl_beta = args.dg_kl_coef  # v7: adaptive KL-to-rollout penalty coefficient (score-mode trust region)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        clipfracs = []
        # Defensive init so the per-iteration KL-beta adaptation / logging never hit an
        # UnboundLocalError in an all-critic-only epoch config (approx_kl is otherwise only
        # bound inside the actor minibatch path).
        approx_kl = old_approx_kl = torch.zeros((), device=device)
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.dg_enable:
                    ...
                    if args.dg_mode == "score":
                        # Faithful Algorithm 1: gated score-function, no ratio, no clip.
                        per_sample = -(w * mb_advantages * newlogprob)
                        # v7 TRUST REGION: + beta_kl * KL(pi_old || pi_theta). k3 estimator
                        # (ratio-1)-logratio is >=0, =0 on-policy, and its grad ((ratio-1)*d logp)
                        # pulls pi_theta back toward the rollout policy -- the on-policy analog of
                        # the paper's target network. NOT the PPO ratio-clip ("no clip" preserved).
                        if dg_kl_beta > 0.0:
                            per_sample = per_sample + dg_kl_beta * ((ratio - 1.0) - logratio)
        ...
        # v7: adapt the score-mode KL leash (Schulman adaptive-beta) toward the target. Done once
        # per iteration on the last-epoch approx_kl so beta_kl converges to hold KL ~ dg_kl_target.
        if args.dg_enable and args.dg_mode == "score" and args.dg_kl_adapt and args.dg_kl_coef > 0.0:
            if approx_kl > 2.0 * args.dg_kl_target:
                dg_kl_beta = min(dg_kl_beta * 1.5, 1e4)
            elif approx_kl < 0.5 * args.dg_kl_target:
                dg_kl_beta = max(dg_kl_beta / 1.5, 1e-4)
        ...
        if args.dg_enable:
            ...
            writer.add_scalar("charts/dg_kl_beta", dg_kl_beta, global_step)
        if auto_alpha:
