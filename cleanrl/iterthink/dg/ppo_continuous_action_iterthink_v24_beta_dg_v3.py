...
class Args:
    ...
    dg_surprisal: str = "cdf_tail"    # "cdf_tail" (-log 2*min(F,1-F), true rarity) | "mode_ref" | "raw_clip" | "entref"
...
def dg_raw_surprisal(logprob, entropy, args, logp_peak=None, cdf_ell=None):
    """Raw (un-scaled, un-clipped) surprisal ell.

    `cdf_tail` (DEFAULT, v3): ell = -log(2*min(F(a),1-F(a))) -- true two-sided tail mass /
    p-value. >= 0, zero at the median, concentration-invariant and mass-symmetric. Best per
    the expert review (measures probabilistic RARITY, not density-distance-from-mode).
    `mode_ref` (v2): ell = logp(mode) - logp(a) >= 0. Beta analog of the Gaussian Mahalanobis
    tail distance; >= 0 so sign(chi)=sign(U) holds, but it tracks the action's own SAMPLING
    QUANTILE (not rarity) and is skew-biased on asymmetric Beta. Kept as ablation.
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_logits, logp_peak, cdf_ell = agent.get_action_and_value(
                    b_obs[mb_inds], b_latent_zs[mb_inds], compute_cdf=(args.dg_enable and args.dg_surprisal == "cdf_tail")
                )
                ...
                if args.dg_enable:
                    ...
                    dg_surp_raw = dg_raw_surprisal(newlogprob, entropy, args, logp_peak, cdf_ell)
                    if args.dg_surprisal_norm:
