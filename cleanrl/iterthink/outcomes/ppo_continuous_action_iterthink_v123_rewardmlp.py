# PPO + IterThink v123 rewardmlp (v122 + nonlinear reward readout).
#
# Key change from v122:
#   - Replace the linear reward token/action readout with a small nonlinear head:
#     MLP([reward_token, action, action^2]) -> projection -> RMSNorm -> bucket
#     prediction projection.
#   - The final prediction projection maps directly to the HL-Gauss reward logit
#     buckets.
#
# Hypothesis:
#   Reward MAE is not primarily biased, but the old head was too linear: it could
#   add a linear action projection to the reward token, then project directly to
#   buckets. HalfCheetah reward contains action-cost curvature and state-action
#   interactions, so v123 gives the outcome head enough local capacity without
#   forcing the predictor transformer to synthesize all nonlinear reward features.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        reward_in_dim = self.dim + 2 * self.act_dim
        reward_hidden = 2 * self.dim
        self.reward_readout_mlp = nn.Sequential(
            xavier_linear(nn.Linear(reward_in_dim, reward_hidden)),
            ReLUSquared(),
            xavier_linear(nn.Linear(reward_hidden, reward_hidden)),
            ReLUSquared(),
        )
        self.reward_readout_proj = xavier_linear(nn.Linear(reward_hidden, self.dim))
        self.reward_pred_norm = nn.RMSNorm(self.dim)
        self.reward_pred_proj = xavier_linear(nn.Linear(self.dim, self.reward_num_bins))
    ...
    def decode_belief_outcomes(self, belief, action=None, detach_belief=True):
        ...
        reward_token = belief[..., self.reward_token_idx, :]
        continuation_feat = self.continuation_token_norm(
            belief[..., self.continuation_token_idx, :]
        )
        if action is None:
            action = reward_token.new_zeros((*reward_token.shape[:-1], self.act_dim))
        reward_input = torch.cat([reward_token, action, action.square()], dim=-1)
        reward_feat = self.reward_readout_proj(self.reward_readout_mlp(reward_input))
        reward_logits = self.reward_pred_proj(self.reward_pred_norm(reward_feat))
...
if __name__ == "__main__":
    ...
    wm_outcome_io_params = (
        list(agent.world_model.reward_readout_mlp.parameters())
        + list(agent.world_model.reward_readout_proj.parameters())
        + list(agent.world_model.reward_pred_norm.parameters())
        + list(agent.world_model.reward_pred_proj.parameters())
    ...
    with torch.no_grad():
        ...
        agent.world_model.reward_pred_proj.weight.zero_()
        agent.world_model.reward_pred_proj.bias.copy_(
