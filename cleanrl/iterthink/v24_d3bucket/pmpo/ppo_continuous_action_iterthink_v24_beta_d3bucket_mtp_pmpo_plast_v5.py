# PPO + IterThink v24 Beta + d3bucket MTP + BetaPlast v5: PURE SIGN PPO (no tanh, no advnorm).
# =====================================================================================
# v5 change over v4 (ablation): signneg_pos_mode="sign" -- BOTH sides use pure sign, so the
# PPO clipped-ratio surrogate runs on eff_adv = sign(A) in {+1,-1}, with NO advantage
# normalization of any kind (no tanh, no std/percentile norm; raw GAE only used for its sign).
# This is the strict-sign policy update under a clip trust region: every positive-advantage
# action pushed up, every negative one down, all with unit magnitude. Contrast points:
#   - vs v3 (also pure sign) : v3 uses the PMPO log-prob objective + KL-to-rollout leash;
#     v5 uses the PPO ratio-clip leash. Isolates the trust-region mechanism at equal (sign) signal.
#   - vs v4 : v4 keeps proportional magnitude for positives; v5 drops it -> tests if any
#     magnitude at all helps over strict sign. Plasticity gate = v2's relative [0.25,4].
#
# --- inherited v4 notes ---
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if pmpo_active:
                    ...
                    if args.signneg_pos_mode == "ppo":
                        ...
                        eff_adv = torch.where(mb_adv_raw >= 0, pos_mag, torch.full_like(mb_adv_raw, -1.0))
                    elif args.signneg_pos_mode == "sign":
                        # pure sign everywhere: eff_adv in {+1, -1} (0 -> 0). NO advnorm.
                        eff_adv = torch.sign(mb_adv_raw)
                    else:
