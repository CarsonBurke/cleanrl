...
class Args:
    ...
    pmpo_kl_coef: float = 0.1        # FIXED reverse KL(old||new) trust-region penalty coefficient
    pmpo_reverse_kl: bool = True     # True: KL(old||new) (reverse); False: KL(new||old) (ablation)
    # v2: analytical-KL trust-region control. The soft reverse-KL penalty is the primary
    # regulator; the analytical-KL early-stop is a soft backstop (standard PPO-style break,
    # NOT a hard rescale/clamp) so analytical KL ideally stays <= pmpo_target_kl.
    pmpo_kl_adapt: bool = False      # adaptive coef OFF (fixed coef). If True: x2/div2 toward pmpo_kl_setpoint
    pmpo_kl_setpoint: float = 0.03   # (only if pmpo_kl_adapt) target iteration-mean analytical reverse-KL
    pmpo_kl_coef_min: float = 1e-3   # adaptive-coef clamp (lower)
    pmpo_kl_coef_max: float = 10.0   # adaptive-coef clamp (upper)
    pmpo_target_kl: float = 0.03     # analytical reverse-KL soft early-stop backstop (PMPO path)
    pmpo_ent_coef: float = 0.0       # NO entropy bonus
...
if __name__ == "__main__":
    ...
    # v2: mutable adaptive PMPO trust-region coefficient, regulated toward pmpo_kl_setpoint
    # by the realized iteration-mean analytical reverse-KL (Schulman 2017 x2/div2 rule).
    pmpo_kl_coef_cur = args.pmpo_kl_coef
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # v2: accumulate the analytical reverse-KL over the minibatch steps ACTUALLY taken
        # this iteration (respects the early-stop break), so kl_meas is the realized mean.
        pmpo_kl_sum = 0.0
        pmpo_kl_count = 0
        beta_conc_mean_val = 0.0  # representative mean(alpha+beta), set on the last PMPO minibatch
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if pmpo_active:
                    ...
                    # Track the realized analytical reverse-KL over the steps actually taken
                    # (drives the adaptive coef + early-stop, both on analytical KL not the ratio).
                    pmpo_kl_sum += float(reverse_kl_val.item())
                    pmpo_kl_count += 1
                    # Representative Beta concentration (alpha+beta) for collapse diagnostics.
                    beta_conc_mean_val = float(
                        (new_dist.concentration1 + new_dist.concentration0).mean().item()
                    )
                    # Entropy/anti-collapse bonus: MAXIMIZE Beta entropy => subtract from the
                    # minimized actor loss. Single term on the PMPO path; ent_coef_eff is zeroed
                    # below for pmpo so the backprop's (- ent_coef_eff * entropy_loss) cannot
                    # double-count this bonus.
                    pg_loss = pg_loss - args.pmpo_ent_coef * entropy.mean()
                    if pmpo_kl_coef_cur > 0.0:
                        kl_loss = reverse_kl if args.pmpo_reverse_kl else kl_divergence(new_dist, old_dist).sum(-1).mean()
                        pg_loss = pg_loss + pmpo_kl_coef_cur * kl_loss
                else:
                ...
                # v2: PMPO carries its OWN entropy bonus folded into pg_loss above (coef
                # pmpo_ent_coef). Zero the generic backprop entropy term on the PMPO path so
                # the bonus is applied exactly once (no double-count).
                if pmpo_active:
                    ent_coef_eff = 0.0
            ...
            if pmpo_active:
                # PMPO early-stop on the LOOSE ANALYTICAL reverse Beta-KL (closed-form, exact),
                # not the ratio-based approx_kl. reverse_kl holds the last minibatch's value,
                # consistent with approx_kl on the ppo path.
                if reverse_kl_val.item() > args.pmpo_target_kl:
                    break
            elif args.target_kl is not None and approx_kl > args.target_kl:
                break

        # v2: regulate the adaptive trust-region coef toward pmpo_kl_setpoint using THIS
        # iteration's realized mean analytical reverse-KL (Schulman 2017 x2/div2 rule).
        # Direction: KL too HIGH -> coef UP -> stronger penalty -> less KL next iteration.
        kl_meas = pmpo_kl_sum / pmpo_kl_count if pmpo_kl_count > 0 else 0.0
        if pmpo_active and args.pmpo_kl_adapt:
            if kl_meas > 1.5 * args.pmpo_kl_setpoint:
                pmpo_kl_coef_cur *= 2.0
            elif kl_meas < args.pmpo_kl_setpoint / 1.5:
                pmpo_kl_coef_cur /= 2.0
            pmpo_kl_coef_cur = float(min(max(pmpo_kl_coef_cur, args.pmpo_kl_coef_min), args.pmpo_kl_coef_max))
        ...
        if pmpo_active:
            ...
            # v2 adaptive trust-region + collapse diagnostics.
            writer.add_scalar("losses/pmpo_kl_coef", pmpo_kl_coef_cur, global_step)
            writer.add_scalar("losses/analytical_kl", kl_meas, global_step)
            writer.add_scalar("losses/beta_conc_mean", beta_conc_mean_val, global_step)
            with torch.no_grad():
                # Fraction of replayed Beta-native latents pinned near the (0,1) support edges.
                edge_pinned = ((b_latent_zs < 0.01) | (b_latent_zs > 0.99)).float().mean().item()
            writer.add_scalar("charts/edge_pinned_frac", edge_pinned, global_step)
