# PPO + IterThink v24 Beta + v162 critic + Dreamer3-bucket HL-Gauss MTP + DREAMER4 PMPO v3.
# =====================================================================================
# v3 = v1 + an ANALYTICAL early-stop and THREE toggleable objective-shaping axes (all default-OFF, so
# with default flags v3 reproduces v1 EXACTLY -- a strict superset / clean A/B sibling):
#   (0) ANALYTICAL EARLY-STOP (always on under PMPO, not a flag). The epoch break under PMPO now
#       triggers on the closed-form reverse Beta-KL(old||new) -- the SAME analytical value the PMPO
#       trust region already computes -- exceeding target_kl, instead of the ratio-based approx_kl.
#       The ratio approx_kl is a noisy MC estimate of the wrong (forward) KL for a sign-based update;
#       the analytical reverse-KL is exactly the quantity the PMPO leash regularizes, so the leash and
#       the early-stop now measure the same trust-region radius. The ppo path keeps approx_kl.
#   (1) POS/NEG TILT (pmpo_pos_to_neg_weight, already a v1 flag, default 0.5 = symmetric): weight the
#       positive-advantage log-prob push vs. the negative-advantage one. >0.5 emphasizes reinforcing
#       good actions over suppressing bad ones.
#   (2) DISTRIBUTIONAL-CRITIC VARIANCE GATE (pmpo_var_gate, default OFF). Down-weight each sample's
#       policy-gradient contribution by its critic's predictive uncertainty:
#           gate = 1 / (1 + std_V / (c * mean(std_V)))      (detached, no grad through the gate)
#       where std_V is the per-sample dispersion of the 511-bin distributional value distribution
#       (sqrt(sum p*(atoms-mean)^2)) over the SAME symexp raw bucket centers the critic decodes with,
#       so units match the value. High-uncertainty states (the critic disagrees with itself) get a
#       smaller, more cautious policy step; the gate multiplies into signed_log_probs before the
#       balanced pos/neg sums. Strictly on-policy, V-only.
#   (3) MAGNITUDE-BAND RESCALE (pmpo_adv_band, default 0.0 = OFF). Replace adv.tanh().abs() with
#       (adv / (band * sigma_adv)).tanh().abs(), where sigma_adv = std of the full-batch raw policy
#       advantages. band<1 sharpens toward sign; band>1 keeps more graded magnitude inside tanh's
#       responsive band so the soft sign is less saturated. band==0 is exact v1 behavior.
...
class Args:
    ...
    # v3 toggleable objective-shaping axes (all default-OFF => v1 behavior exactly).
    # (2) Distributional-critic variance gate: down-weight a sample's policy-gradient contribution by
    #     its critic dispersion. gate = 1/(1 + std_V/(c*mean(std_V))), detached. std_V is the per-sample
    #     std of the 511-bin value distribution over the SAME symexp raw bucket centers used to decode V.
    pmpo_var_gate: bool = False
    pmpo_var_gate_c: float = 1.0
    pmpo_gate_renorm: bool = False  # v3 (2b): divide the variance gate by its mean -> relative reweighting only (preserves effective LR)
    # (3) Magnitude-band rescale: replace adv.tanh().abs() with (adv/(band*sigma_adv)).tanh().abs(),
    #     sigma_adv = std of the full-batch raw policy advantages. 0.0 == v1 (plain adv.tanh().abs()).
    pmpo_adv_band: float = 0.0
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # v3 (3) magnitude-band rescale: std of the FULL-batch raw policy advantages, computed once per
        # iteration so dividing by band*sigma_adv keeps advantages inside tanh's responsive band.
        sigma_adv = b_advantages.std()
        ...
        var_gate_mean_val = torch.ones((), device=device)  # v3 (2): gate.mean() (1.0 when gate off)
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if pmpo_active:
                    ...
                    # v3 (3) magnitude-band rescale: divide by band*sigma_adv before the tanh-sign so
                    # advantages land in tanh's responsive band (band==0.0 => exact v1 adv.tanh().abs()).
                    if args.pmpo_adv_band > 0.0:
                        adv_weight = (
                            mb_advantages / (args.pmpo_adv_band * sigma_adv + 1e-8)
                        ).tanh().abs().unsqueeze(-1)
                    else:
                        adv_weight = mb_advantages.tanh().abs().unsqueeze(-1)
                    signed_log_probs = log_prob_per_dim * adv_weight
                    # v3 (2) distributional-critic variance gate: down-weight each sample's policy-gradient
                    # contribution by its critic dispersion std_V (per-sample std of the 511-bin value
                    # distribution over the SAME symexp raw bucket centers `raw_support` used to decode V,
                    # so units match the value). gate detached -> no grad through the gate. gate scales the
                    # per-sample contribution => multiplied into signed_log_probs before the pos/neg sums.
                    if args.pmpo_var_gate:
                        with torch.no_grad():
                            p_v = torch.softmax(value_logits[:, 0], dim=-1)           # (mb, num_bins)
                            mean_v = (p_v * raw_support).sum(-1, keepdim=True)
                            std_v = (p_v * (raw_support - mean_v) ** 2).sum(-1).clamp_min(0).add(1e-8).sqrt()
                            gate = 1.0 / (1.0 + std_v / (args.pmpo_var_gate_c * std_v.mean()))
                            # v3: mean-preserving renorm -> relative reweighting only, effective step size preserved
                            # (the raw gate averages ~0.5 when the critic is uniformly uncertain, which silently halves the LR).
                            if args.pmpo_gate_renorm:
                                gate = gate / gate.mean().clamp_min(1e-8)
                        signed_log_probs = signed_log_probs * gate.unsqueeze(-1)
                        var_gate_mean_val = gate.mean().detach()
            ...
            # v3 (0) ANALYTICAL EARLY-STOP (always on under PMPO). Break on the closed-form reverse
            # Beta-KL(old||new) -- the SAME analytical value the PMPO trust region regularizes (last
            # minibatch's reverse_kl_val) -- instead of the ratio-based approx_kl. The ppo path is
            # unchanged (ratio approx_kl). reverse_kl_val is in scope (set before the epoch loop and
            # refreshed in the PMPO branch each minibatch).
            if args.target_kl is not None:
                kl_for_stop = reverse_kl_val if pmpo_active else approx_kl
                if kl_for_stop > args.target_kl:
                    break
        ...
        if pmpo_active:
            ...
            if args.pmpo_var_gate:
                writer.add_scalar("losses/var_gate_mean", var_gate_mean_val.item(), global_step)
            if args.pmpo_adv_band > 0.0:
                writer.add_scalar("losses/adv_band_sigma", sigma_adv.item(), global_step)
