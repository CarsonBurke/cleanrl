# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT v12_env128.
# =====================================================================================
# DOUBLE ENVS: v11 config (actor 1e-2 / critic 1e-3 / aux 3e-4, 1 epoch) at
# 128 envs x 39 steps = 4992 batch. Batch-size ablation at fixed epoch=1
# against running v11 (64x39): isolates whether the frozen actor was a batch
# (gradient-noise) or step-count problem. Stagger spacing 1000/128 ~= 7.8;
# warmup costs 128k (12.8% of 1M); ~174 iterations. LRs deliberately NOT
# rescaled -- the batch effect is the variable under test.
# Deltas from v11: num_envs 64 -> 128 only.
# (v11 LR-REBALANCE context: v10 left the actor frozen, aux streams noisy.)
...
# Inherited from v11/v10 (unchanged): single joint update (1 epoch, whole
# batch, KL leash non-binding), unshared backbone, 3-group Adam (actor /
# critic / aux, asserted disjoint at build), debug/epochs_run counter.
...
class Args:
    ...
    actor_lr: float = 1e-2          # v11: stronger actor steps (v10 KL peaked at 1.2e-7)
    critic_lr: float = 1e-3         # v10: lower LR for the supervised critic side
    nextlat_lr: float = 3e-4        # v11: aux predictors step 10x below the actor
    num_envs: int = 128             # v12: doubled envs (4992 batch); fallback would be 64x78
...
if __name__ == "__main__":
    ...
    # v11: one Adam, three LR groups (actor core / critic core / aux predictors).
    # Aux-only params are derived by set difference (nextlat_* minus core_*),
    # so the Agent needs no new methods. Requires disjoint param sets -- i.e.
    # an unshared backbone -- so no parameter is stepped twice.
    actor_opt_params = list(agent.actor_parameters())
    critic_opt_params = list(agent.critic_parameters())
    _actor_ids = {id(p) for p in actor_opt_params}
    _critic_ids = {id(p) for p in critic_opt_params}
    aux_opt_params = [p for p in agent.nextlat_actor_parameters() if id(p) not in _actor_ids]
    aux_opt_params += [p for p in agent.nextlat_critic_parameters() if id(p) not in _critic_ids]
    _aux_ids = {id(p) for p in aux_opt_params}
    if _actor_ids & _critic_ids:
        raise ValueError("v11 LR split needs disjoint actor/critic groups (unshare the backbone)")
    if (_actor_ids & _aux_ids) or (_critic_ids & _aux_ids):
        raise ValueError("v11 aux group must be disjoint from actor/critic groups")
    if {id(p) for p in agent.parameters()} != (_actor_ids | _critic_ids | _aux_ids):
        raise ValueError("v11 optimizer groups must cover all parameters exactly once")
    optimizer = optim.Adam(
        [
            {"params": actor_opt_params, "lr": args.actor_lr},
            {"params": critic_opt_params, "lr": args.critic_lr},
            {"params": aux_opt_params, "lr": args.nextlat_lr},
        ],
        eps=1e-5,
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            optimizer.param_groups[2]["lr"] = frac * args.nextlat_lr
        ...
        writer.add_scalar("charts/learning_rate_nextlat", optimizer.param_groups[2]["lr"], global_step)
