# RESPC v7: RESIDUAL PREDICTIVE CODING.  (NextLat, but coding the residual.)
# =====================================================================================
...
class Args:
    ...
    nextlat_grad_clip: float = 0.25  # v7 uses the SWEEP WINNER (clip025), not v1's 0.1, so
                                     # the only difference from the incumbent is the loss
    nextlat_cascade_coef: float = 0.25  # weight on the LINEAR stage's own prediction error.
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    if args.nextlat:
                        ...
                        # Pure DIAGNOSTIC decomposition, measured before the single joint
                        # clip below actually fires. v5 discovered the dynamics MLP was
                        # carrying ~98.6% of this norm, meaning every joint-clip run was
                        # spending its budget on aux-only params that Adam renormalizes
                        # anyway. Keep watching it -- but clip jointly, exactly as the
                        # incumbent does, so v7 vs clip025 isolates the LOSS alone.
                        with torch.no_grad():
                            def _gn(ps):
                                sq = [p.grad.pow(2).sum() for p in ps if p.grad is not None]
                                return torch.stack(sq).sum().sqrt() if sq else torch.zeros((), device=device)
                            nextlat_trunk_gn = _gn(nextlat_trunk_diag)
                            nextlat_pred_gn = _gn(nextlat_pred_diag)
                            nextlat_head_gn = _gn(nextlat_head_diag)
                        nextlat_gn = nn.utils.clip_grad_norm_(nextlat_params, args.nextlat_grad_clip)
