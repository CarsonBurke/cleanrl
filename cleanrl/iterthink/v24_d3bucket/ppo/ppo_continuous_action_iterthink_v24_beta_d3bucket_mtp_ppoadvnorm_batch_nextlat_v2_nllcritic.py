# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT + NLL-CRITIC v2.
# =====================================================================================
# V2 DELTA (vs nextlat_v1): the Dreamer3 511-bucket HL-Gauss categorical critic is
# replaced by a GAUSSIAN NLL critic -- NO Q-learning, NO distributional buckets.
#   - Head: one Linear(H -> 2*MTP): per horizon h, (mean_h, logvar_h) regressing
#     returns[t+h] under 0.5*(logvar + (ret-mean)^2/exp(logvar)), masked by the same
#     reset-boundary MTP masks. Mean init 0, logvar init `critic_logvar_init`,
#     logvar clamped to [critic_logvar_min, critic_logvar_max] (the clamp is the
#     only variance guard; there is no annealing, EMA target, or second head).
#   - Uncertainty -> policy weight: the h0 std sigma(s)=exp(0.5*logvar_0) scales
#     each sample's GAE BEFORE shaping/normalization via a precision-style weight
#         w = 1 / (1 + unc_weight_coef * (sigma(s)/std(returns))^2),
#     so high-aleatoric states contribute less direction while batch norm_adv
#     restores the global step size (relative reweighting, not downscaling).
#     `--no-unc-weight-adv` recovers unweighted advantages exactly.
# HYPOTHESIS: raw returns are heavy-tailed/heteroscedastic; NLL learns a calibrated
# per-state spread where MSE would average it away, and precision-weighting the
# advantages steers PPO steps toward states the critic actually understands --
# fewer garbage gradients from unpredictable transitions, higher returns. Falsifiable:
# if v2 <= v1, the bucket geometry (near-zero resolution + wide tails) was load-bearing
# and a single Gaussian mode cannot cover the return shape.
# DEFAULTS BAKE IN the v1 run config: norm_adv=True/scope=batch, nextlat on with
# nextlat_grad_clip=0.25 (the value passed on the v1 command line, not v1's 0.1 file
# default), clip-higher 0.2/0.28, raw rewards, beta actor.
# Watch: losses/value_nll, debug/critic_sigma_mean, debug/nll_mse (mean-vs-return MSE),
# debug/sigma_over_retstd (calibration: ~1 means the spread matches return scale),
# debug/unc_weight_mean (effective data kept), debug/latent_batch_std.
# =====================================================================================
#
# --- inherited nextlat_v1 notes (NEXTLAT unchanged below) ---
...
class Args:
    ...
    # return; a SEPARATE soft-advantage GAE adds the bootstrap bonus b_t = α·H_sq(s_{t+1}),
    # estimated as the single-sample SQUASHED entropy -logπ_sq(a|s), matching SAC's
    # next_state_log_pi units. Rationale: forcing the critic to learn the soft value wastes
    # capacity and distorts its uncertainty (the softboot failure). alpha is
    ...
    # Gaussian NLL MTP critic (v2): per horizon h, (mean_h, logvar_h) regressing
    # returns[t+h] under Gaussian NLL. No buckets, no support, no Q-learning.
    critic_mtp_horizon: int = 6
    critic_logvar_init: float = 4.0   # initial log-variance bias (std ~7.4 at start)
    critic_logvar_min: float = -6.0   # variance clamp (sole guard against collapse)
    critic_logvar_max: float = 14.0   # std up to ~1096 -- covers raw return scales
    critic_sigma_floor: float = 1.0   # floor on sigma(s) in raw-return units
    # Uncertainty-weighted advantages: w = 1/(1 + coef*(sigma(s)/std(returns))^2).
    unc_weight_adv: bool = True
    unc_weight_coef: float = 1.0
    ...
    nextlat_grad_clip: float = 0.25  # own dual-backward budget (v1 cmdline value; trunk total 0.25+0.25+0.25)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v2 NLL critic: one Linear(H -> 2*MTP); per horizon h, channel 2h is the
        # return mean and 2h+1 the raw log-variance. Neutral init: zeroed weights,
        # mean bias 0, logvar bias critic_logvar_init -- the value starts at 0 with
        # a broad fixed spread, and NLL calibrates the spread from data.
        self.critic_mtp_horizon = args.critic_mtp_horizon
        self.critic_logvar_min = args.critic_logvar_min
        self.critic_logvar_max = args.critic_logvar_max
        self.critic_head = layer_init(
            nn.Linear(H, args.critic_mtp_horizon * 2, bias=True), std=0.1
        )
        with torch.no_grad():
            self.critic_head.weight.zero_()
            self.critic_head.bias.zero_()
            self.critic_head.bias[1::2].fill_(args.critic_logvar_init)
    ...
    def _decode_value_stats(self, head_out):
        # Split the head into (mean, clamped logvar) per MTP horizon.
        # Returns (B, mtp, 2) with [..., 0] = mean, [..., 1] = logvar.
        stats = head_out.view(-1, self.critic_mtp_horizon, 2)
        mean = stats[..., 0]
        logvar = stats[..., 1].clamp(self.critic_logvar_min, self.critic_logvar_max)
        return torch.stack([mean, logvar], dim=-1)

    def get_value(self, x):
        # Returns value STATS (B, mtp, 2); horizon 0 is V(s_t).
        _, critic_feat = self._trunks(x)
        return self._decode_value_stats(self.critic_head(critic_feat))
    ...
    def get_action_and_value(self, x, z=None):
        ...
        value_stats = self._decode_value_stats(self.critic_head(critic_feat))
        if self.actor_dist == "gaussian":
        ...
        return action, z, log_prob, entropy, value_stats, actor_feat
...
if __name__ == "__main__":
    ...
    # v2 NLL critic: no buckets/support. Values are head means; uncertainty is the
    # h0 std sigma(s) = exp(0.5 * logvar_0), floored in raw-return units.
    sigma_floor = args.critic_sigma_floor
    ...
    value_logvars = torch.zeros((args.num_steps, args.num_envs)).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, value_stats, actor_feat = agent.get_action_and_value(next_obs)
                values[step] = value_stats[:, 0, 0]
                value_logvars[step] = value_stats[:, 0, 1]
        ...
        with torch.no_grad():
            next_transition_value_stats = agent.get_value(
                next_obses.reshape((-1,) + envs.single_observation_space.shape)
            )[:, 0]
            next_transition_values = next_transition_value_stats[:, 0].reshape(
            ...
            # next_state_log_pi. The NLL critic regresses to the RAW reward return and
            # the entropy is added to a SEPARATE soft advantage used only for the PG.
            if auto_alpha:
            ...
            # v2 NLL critic target: scalar-return MTP. Horizon 0 regresses
            ...
            # Targets stay on GPU: (T,B,MTP) floats, no bucket projection.
            ...
            # Per-state uncertainty from the OLD rollout NLL critic: sigma(s) is the
            # h0 predicted std in raw-return units. Feeds tanh_std shaping AND the
            # precision-style advantage weight below.
            sigma = (0.5 * value_logvars).exp().clamp_min(sigma_floor)  # (T,B)
            # Gaussian-CDF position u = Phi((G - mean)/sigma): keeps the cdf_probit
            # shaping toggle functional without buckets.
            zscore = ((returns - values) / sigma).clamp(-5.0, 5.0)
            u = 0.5 * (1.0 + torch.erf(zscore / (2.0 ** 0.5)))  # (T,B)
            u_edge_frac = ((u < 0.05) | (u > 0.95)).float().mean().item()  # calib probe (uniform≈0.1)
            # UNCERTAINTY-WEIGHTED ADVANTAGES (v2): precision-style per-sample weight
            # w = 1/(1 + coef*(sigma(s)/std(returns))^2), applied to the policy GAE
            # BEFORE shaping/normalization. Relative reweighting: certain states steer
            # the update, uncertain ones are trusted less; batch norm_adv afterwards
            # restores the global step size so this cannot collapse the update scale.
            ret_std = returns.std().clamp_min(1.0)
            sigma_over_retstd = (sigma / ret_std).mean().item()
            if args.unc_weight_adv:
                rel = sigma / ret_std
                unc_w = 1.0 / (1.0 + args.unc_weight_coef * rel * rel)
                policy_adv = policy_adv * unc_w
                unc_weight_mean = unc_w.mean().item()
            else:
                unc_weight_mean = 1.0
        ...
        b_return_mtp = return_mtp.reshape(-1, args.critic_mtp_horizon)
        b_return_mtp_mask = return_mtp_mask.reshape(-1, args.critic_mtp_horizon)
        b_u = u.reshape(-1)
        if args.nextlat:
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_stats_mb, mb_actor_feat = agent.get_action_and_value(
                ...
                # v2 Gaussian-NLL MTP value loss: per-horizon NLL of the scalar
                # return under N(mean_h, exp(logvar_h)), summed across valid
                # horizons per row. No value clipping; the learned variance IS the
                # outlier model (large residuals widen sigma instead of yanking mean).
                v_mean = value_stats_mb[..., 0]
                v_logvar = value_stats_mb[..., 1]
                ret_mb = b_return_mtp[mb_inds]
                nll = 0.5 * (v_logvar + (ret_mb - v_mean) ** 2 * torch.exp(-v_logvar))
                value_mask = b_return_mtp_mask[mb_inds].to(dtype=nll.dtype)
                v_loss = (nll * value_mask).sum(dim=-1).mean()
                ...
                if args.separate_grad_clip:
                    ...
                    # nextlat, and policy gradients separately, clip each to its own
        ...
        # NLL-critic calibration probes: raw MSE of the h0 mean (scale probe),
        # mean predicted spread, and whether the spread matches the return scale.
        nll_mse = ((b_values - b_returns) ** 2).mean().item()
        critic_sigma_mean = b_sigma.mean().item()
        ...
        writer.add_scalar("debug/nll_mse", nll_mse, global_step)
        writer.add_scalar("debug/critic_sigma_mean", critic_sigma_mean, global_step)
        writer.add_scalar("debug/sigma_over_retstd", sigma_over_retstd, global_step)
        writer.add_scalar("debug/unc_weight_mean", unc_weight_mean, global_step)
