# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT PER-LAYER v2.
# =====================================================================================
# WHAT'S NEW vs nextlat_v1: supervise EVERY trunk block latent, not just the output.
#   v1 applies the next-latent loss only to the ThinkTrunk OUTPUT, so the K interior
#   blocks are shaped by the aux only indirectly through backprop. v2 gives each block
#   latent its own dynamics MLP and its own stop-grad target (the same block's latent
#   at t+i), averaged with the output term so total aux scale is unchanged. feats[0] is
#   entry(x), a linear map of the observation, so its target is the most grounded in
#   the stack: predict the next OBSERVATION embedding.
# Hypothesis: deeper supervision distributes the dynamics constraint across the whole
#   trunk instead of concentrating it at the readout, giving every block a reason to
#   encode controllable state. Risk: early-block targets may be easy and saturate, in
#   which case the extra terms dilute the (still-sharpening) output term.
# Watch: losses/nextlat_pred_loss (should sit HIGHER than v1 if the layer targets are
#   non-trivial; a drop means the easy layers are diluting the signal).
# =====================================================================================
#
...
class Args:
    ...
    nextlat_perlayer: bool = True    # v2: supervise EVERY trunk block latent, not just the output
    nextlat_layer_grad_clip: float = 0.1  # OWN budget for the per-layer predictors, so the
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # PER-LAYER (v2). v1 supervises only the trunk OUTPUT latent, so the interior
        # blocks are shaped by the aux only indirectly, through backprop. Here every
        # block latent gets its own dynamics MLP and its own prediction target, so each
        # block must itself be action-conditionally predictive. Note feats[0] = entry(x)
        # is a linear map of the observation, making its target the most grounded one in
        # the stack: predict the next OBSERVATION embedding. Initialized LAST for RNG
        # parity with the base run.
        self.nextlat_perlayer = args.nextlat_perlayer
        if self.nextlat_perlayer:
            self.nextlat_layer_predictors = nn.ModuleList(
                [
                    nn.Sequential(
                        layer_init(nn.Linear(H + act_dim, H)),
                        ReLUSquared(),
    ...
    def nextlat_layer_parameters(self):
        ...
        return list(self.nextlat_layer_predictors.parameters()) if self.nextlat_perlayer else []
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.nextlat:
                    ...
                    # the averaged loss.
                    comp_losses = [[] for _ in range(args.k_blocks + 2)] if args.nextlat_perlayer else []
                    for i in range(1, args.nextlat_depth + 1):
                        ...
                        with torch.no_grad():
                            if args.nextlat_perlayer:
                                tgt_feat, tgt_feats = agent.get_actor_feats_all(b_obs[tgt_idx])
                            else:
                        ...
                        if args.nextlat_perlayer:
                            # Each block latent is rolled by its OWN dynamics MLP against
                            # the stop-grad block latent of the true future obs. Averaged
                            # with the output term so the aux's total scale is unchanged.
                            layer_terms = [pred_l]
                            for k, layer_pred in enumerate(agent.nextlat_layer_predictors):
                                h_hat_layers[k] = layer_pred(
                                    torch.cat([h_hat_layers[k], act_i], dim=-1)
                                )
                                layer_terms.append(
                                    F.smooth_l1_loss(
                                        h_hat_layers[k], tgt_feats[k], reduction="none"
                                    ).mean(-1)
                                )
        ...
        if args.nextlat:
            ...
            if args.nextlat_perlayer:
                # comp 0 = trunk output (the v1 term); comp 1 = entry(x) embedding;
                # comp 2.. = ThinkTrunk block latents.
