# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + LATENT-TD v2 SPECLADDER.
# =====================================================================================
# LATENT-TD v2, MULTI-GAMMA SPECTRAL LADDER. v1's single-gamma(0.99) EWMA target was a
# low-pass filter that crushed all latent structure faster than ~100 steps (gait-frequency
# attenuation ~30-40x): psi solved the mushy target to ~3% error, the aux gradient fell
# below its clip budget, and trunk shaping faded (confirmed in v1's run diagnostics).
# v2 keeps the TD bootstrap (infinite horizon at O(1)) but predicts the latent EWMA at
# FOUR timescales simultaneously:
#     psi_k(s_t) ~= E[ (1-g_k) * sum_j g_k^j * c_{t+j} ],   g_k in (0.6, 0.9, 0.97, 0.99)
# A bank of low-pass filters at staggered cutoffs (~2.5/10/33/100-step horizons) is a
# coarse SPECTRAL DECOMPOSITION: differences between adjacent bands are band-pass, so the
# trunk must encode WHEN things happen at every timescale, not just the long-run average.
# The gait cycle lives in the 0.9 band; the regime average in the 0.99 band.
# REWARD ANCHOR: the pseudo-reward vector c_t = [LN(z_t), symlog(r_t)] appends the true
# reward as one channel, so one coordinate of each psi_k is a bona fide (normalized,
# horizon-g_k) value function inside the actor trunk — a non-learnable, policy-coupled
# channel that cannot saturate while the policy still improves (the property that made
# nextlat's KL term effective).
# Mechanics otherwise inherited from v1: vector lambda-returns via the reward-GAE
# recursion/masks (snapshot targets fixed per iteration), LN latents, own dual-backward
# clip budget (0.1 vs actor 0.25 / critic 0.25). --compile wraps the hot forward paths
# (torch.compile dynamic=False + cudagraph_mark_step_begin, reduce-overhead default).
# Watch: losses/latent_td_loss (should NOT collapse to ~0 like v1), losses/latent_grad_norm
# (should stay near the 0.1 budget), debug/psi_target_std_g* (info content per band —
# the short-gamma bands should carry visibly more variance than v1's single 0.99 band),
# debug/latent_zn_batch_std (collapse probe).
#
...
def symlog(x):
    # Dreamer-style signed log compression; keeps the reward-anchor channel at the
    # same O(1) scale as the LayerNormed latent channels.
...
class Args:
    ...
    # LATENT-TD v2 (spectral ladder): multi-gamma TD/lambda-return self-prediction of
    # [LN(z), symlog(r)] (see header). Targets are snapshot lambda-returns computed once
    # per iteration with the SAME masks/recursion as the reward GAE, one bank per gamma.
    latent_td: bool = True
    latent_td_gammas: tuple[float, ...] = (0.6, 0.9, 0.97, 0.99)  # ~2.5/10/33/100-step bands
    latent_td_lambda: float = 0.95   # lambda-return mix; high => targets mostly real latents
    latent_td_coef: float = 1.0      # MSE weight (the dedicated grad clip is the real ceiling)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # LATENT-TD v2 psi head: K gamma-banks x C channels (C = H LN-latent channels +
        # 1 symlog-reward anchor channel). Small final init => psi starts ~0, so early
        # lambda-return targets are dominated by real observed pseudo-rewards, not
        # bootstrap. Initialized LAST so all base-shared parameters draw identical
        # initial weights from the seeded RNG as the ppoadvnorm_batch base run.
        self.latent_td_k = len(args.latent_td_gammas)
        self.latent_td_c = H + 1
        self.psi_head = nn.Sequential(
            layer_init(nn.Linear(H, H)),
            ReLUSquared(),
            layer_init(nn.Linear(H, self.latent_td_k * self.latent_td_c), std=0.1),
        )
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # LATENT-TD v2 (spectral ladder) targets: per-gamma vector lambda-returns
            # over the pseudo-reward stream c_t = [LN(z_t), symlog(r_t)], with the exact
            # reward-GAE recursion/masks vectorized over (K gamma-banks, C channels).
            # Each bank predicts the (1-g_k)-normalized EWMA at its own timescale. All
            # quantities here are rollout SNAPSHOTS (old trunk, old psi) — targets are
            # fixed across the epoch loop, exactly like the critic's `returns`.
            if args.latent_td:
                K_lat, C_lat = agent.latent_td_k, agent.latent_td_c
                zn = F.layer_norm(latents, (args.hidden,))                     # (T,B,H)
                pseudo_r = torch.cat([zn, symlog(rewards).unsqueeze(-1)], dim=-1)  # (T,B,C)
                g_lat = torch.tensor(
                    args.latent_td_gammas, device=device, dtype=torch.float32
                ).view(1, K_lat, 1)                                            # (1,K,1)
                psi_roll = agent.psi_head(latents).view(
                    args.num_steps, args.num_envs, K_lat, C_lat
                )
                ...
                lam_lat = args.latent_td_lambda
                psi_adv = torch.zeros_like(psi_roll)                           # (T,B,K,C)
                lastpsilam = 0.0
                for t in reversed(range(args.num_steps)):
                    boot_nonterm = ((1.0 - transition_terminations[t]) * transition_valids[t]).view(-1, 1, 1)
                    lam_nonterm = (1.0 - transition_boundaries[t]).view(-1, 1, 1)
                    psi_delta = (
                        (1.0 - g_lat) * pseudo_r[t].unsqueeze(1)               # (B,1,C)->(B,K,C)
                        + g_lat * psi_next[t] * boot_nonterm
                        - psi_roll[t]
                    )
                    psi_adv[t] = lastpsilam = psi_delta + g_lat * lam_lat * lam_nonterm * lastpsilam
                psi_targets = psi_adv + psi_roll                               # (T,B,K,C)
                # Diagnostics: per-band target std (info content per timescale — short
                # gammas should carry visibly more variance than the 0.99 band) and the
                # collapse probe (cross-batch per-dim std of LN latents; -> 0 = collapse).
                psi_target_std_per_g = psi_targets.reshape(-1, K_lat, C_lat).std(dim=0).mean(dim=-1)  # (K,)
        ...
        b_u = u.reshape(-1)
        if args.latent_td:
            b_psi_targets = psi_targets.reshape(-1, agent.latent_td_k, agent.latent_td_c)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # LATENT-TD v2: regress the CURRENT psi(trunk(s)) onto the fixed snapshot
                # multi-gamma lambda-return targets. Gradient flows through psi_head INTO
                # the trunk — that is the point of the aux.
                if args.latent_td:
                    psi_pred = agent.psi_head(mb_actor_feat).view(
                        -1, agent.latent_td_k, agent.latent_td_c
                    )
                    latent_td_loss = F.mse_loss(psi_pred, b_psi_targets[mb_inds])
                else:
                    latent_td_loss = None
        ...
        if args.latent_td:
            ...
            writer.add_scalar("debug/psi_target_absmean", psi_targets.abs().mean().item(), global_step)
            writer.add_scalar("debug/psi_absmean", psi_roll.abs().mean().item(), global_step)
            for gi, gv in enumerate(args.latent_td_gammas):
                writer.add_scalar(f"debug/psi_target_std_g{gv}", psi_target_std_per_g[gi].item(), global_step)
        if auto_alpha:
