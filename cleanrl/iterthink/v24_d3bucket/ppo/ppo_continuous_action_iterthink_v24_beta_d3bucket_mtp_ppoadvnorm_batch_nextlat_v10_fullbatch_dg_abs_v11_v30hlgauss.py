# PPO + IterThink v24 Beta + d3bucket + NEXTLAT v10_fullbatch + TRUE DG GATE + VMPO-v30 HLGAUSS (v11).
# v11 delta vs v10: v30's critic config ported in -- 51 buckets (was 511) with
# project_moment_matched labels (was plain project). Same +-9.9 support, same
# 0.75 sigma ratio, same single horizon. Tests whether v30's coarse
# moment-exact critic beats our fine plain one inside our stack.
# v10 delta vs v9: critic_mtp_horizon 6 -> 1 (single-horizon HL-Gauss value
# head; the nextlat aux carries the multi-step signal instead). Also replaces
# the nextlat critic categorical KL with a manual clamped-log-softmax form
# after a transient logged NaN at ~947k in v9. Proven note: the SAME step's
# critic pred loss (same z_hat/tgt tensors) was finite, and torch's KL is
# mathematically NaN-proof on finite inputs (p=0 bins explicitly zeroed), so
# the NaN is a downstream logging artifact, not a training blowup. The manual
# form is kept anyway: verified 9e-7-identical in-range and it caps torch's
# +inf spikes (teacher mass on underflowed student bins) at 20 nats.
# =====================================================================================
# v6 delta vs DG-ABS v1: norm_adv=False -- the gate-weighted RAW shaped advantage
# feeds PPO directly with NO batch standardization (weight-then-norm and
# norm-then-weight both exonerated as slow: v1 402/614/1294, v5-normfirst
# 298/503/653 @500k/1M/2M vs baseline 938/2265; normfirst shrank steps to ~0.5x
# by never restoring unit variance). HYPOTHESIS: the re-norm bar itself is the
# slowdown (pos-heavy weighted mean flips common-good actions negative =
# neg-erasure-by-normalization). Raw scale varies with returns, so steps are
# uncalibrated -- that is the accepted cost of this test, not a bug.
# v7 delta vs v6: normalize_reward=True (shared VectorRewardNorm, unclipped) so
# reward scale -- and hence GAE/TD-error scale -- stays stationary as raw
# returns grow; support stays WIDE +-9.9 (unlike v4's +-5) to avoid the
# edge-binding confound. v6 @500k=276 @1M=475 (slowest sibling) says advnorm
# absence is NOT the speedup; v7 tests whether scale stationarity recovers it.
# DG-ABS: delight-gate advantage weighting with ABSOLUTE (density) rareness on
# the v10_fullbatch base (64x39 rollouts, single full-batch update, unshared
# trunks, split LRs, head-split auxes). Same asymmetric maps as DG-REL, but
# rareness comes from the sample's OWN policy density, no batch comparison:
#   v9: paper-faithful gate w = sigmoid(U*l/eta), l = clip(-log-density, -10, 10).
# JACOBIAN NOTE (load-bearing): the buffered logprob DROPS the constant linear
# rescale Jacobian (it cancels in the PPO ratio), so absolute density restores
# it: beta true log-density = stored - sum(log(high-low)) (HalfCheetah: -4.16);
# gaussian stored logprob already includes the tanh Jacobian and is used as-is.
# Without this, every density would be off by 64x and the "absolute" gate would
# silently behave as all-rare. Rank-based DG-REL never needed it (constants
# don't affect ranks) -- this is THE structural difference under test.
# Consequence: unlike rank gating, this gate goes QUIET when the policy
# sharpens (everything clips to common) instead of forcing contrast forever --
# the rank-vs-absolute difference flagged in the v4 chart comparison.
# `--no-dg` is bit-identical to the v10 base. update_epochs=1 => zero staleness.
# HYPOTHESIS: absolute surprise matches the DG plots' p-axis literally and
# avoids spurious late-training contrast. Falsifiable vs DG-REL on the same
# base: if ABS <= REL, the chart's p-axis needs rank-style contrast to matter.
# Watch: debug/dg_w_mean, debug/dg_w_std, debug/dg_pos_mass_frac.
# =====================================================================================
#
# SPEED (8M): torch.compile ON by default (mode=reduce-overhead) over the agent
# forwards (rollout + update) and the custom reset-gated valids-GAE loop (fused;
# shared compute_gae cannot express valids-gating, so the loop is kept and
# compiled). Obs/reward norms, staggered warmup, device minibatches,
# explained_variance and gather_metrics are the shared vectorized implementations
# (already in v10). First iteration pays compilation; --no-compile = eager parity.
# --- inherited v10_fullbatch header (unchanged below) ---
# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT v10_fullbatch.
# =====================================================================================
# VMPO-MATCHED: v8 head-split reconfigured to VMPO v30's rollout/update shape
# (filename is a stale trail marker -- v10 started as 64x2048 full-batch).
# Deltas from v8:
...
class Args:
    ...
    seed: int = 1
    # Speed (semantics-preserving): compile agent forwards + valids-GAE loop.
    compile: bool = True
    ...
    actor_lr: float = 3e-3          # v10: split LRs (hilr winner for the actor side)
    critic_lr: float = 1e-3         # v10: lower LR for the supervised critic side
    ...
    norm_adv: bool = False           # v6 NO-ADVNORM: gate-weighted raw advantage straight into PPO, no standardization
    ...
    num_bins: int = 51             # v11: v30's coarse buckets (was 511)
    v_min: float = -9.90353755128617
    v_max: float = 9.90353755128617
    value_sigma_to_bin_ratio: float = 0.75
    critic_mtp_horizon: int = 1        # v10: NO critic MTP (nextlat carries multi-step signal)
    ...
    normalize_reward: bool = True    # v7: shared VectorRewardNorm ON (unclipped, wide support kept)
    ...
    # Paper-faithful Delightful Policy Gradient gate (arXiv:2603.14608, Alg. 2):
    # w = sigmoid(U*l/eta), l = clip(-true log-density, -C, C), eta = 1.
    # No whitening, no sign piecewise map: magnitude response, smooth blunder
    # attenuation (never hard-zero), and dense-success downweighting all fall
    # out of the product. U is the shaped policy advantage (raw GAE here);
    # reward norm keeps it unit-scale so eta = 1 operates as in the paper
    # (the paper also normalizes rewards for all methods, App. E.1).
    dg: bool = True
    dg_eta: float = 1.0              # paper temperature (fixed at 1 throughout)
    dg_surprisal_clip: float = 10.0  # paper clip bound C on surprisal
...
def gae_valids_loop(rewards, values, next_values, terms, valids, bounds, bonus, gamma, lam):
    """Reset-gated GAE backward recursion (valids-GAE semantics).

    Closed over tensors with a fixed trip count and no breaks so torch.compile
    can fuse the per-step launches. bonus=None gives the reward (critic)
    recursion; bonus=<tensor> gives the entropy-augmented soft policy recursion.
    """
    advantages = torch.zeros_like(rewards)
    lastgaelam = 0.0
    for t in range(rewards.shape[0] - 1, -1, -1):
        boot = (1.0 - terms[t]) * valids[t]
        lnt = 1.0 - bounds[t]
        nv = next_values[t] if bonus is None else next_values[t] + bonus[t]
        delta = rewards[t] + gamma * nv * boot - values[t]
        lastgaelam = delta + gamma * lam * lnt * lastgaelam
...
if __name__ == "__main__":
    ...
    if args.compile:
        print(f"torch.compile ON (mode={args.compile_mode}): agent forwards + valids-GAE loop")
        agent.get_action_and_value = torch.compile(agent.get_action_and_value, mode=args.compile_mode)
        agent.get_value = torch.compile(agent.get_value, mode=args.compile_mode)
    gae_loop_fn = (
        torch.compile(gae_valids_loop, mode=args.compile_mode) if args.compile
        else gae_valids_loop
    )
    # v10: one Adam, two LR groups (actor side / critic side). Requires disjoint
    # param sets -- i.e. an unshared backbone -- so no parameter is stepped twice.
    ...
    if _actor_ids & _critic_ids:
        raise ValueError("v10 LR split needs disjoint actor/critic groups (unshare the backbone)")
    if {id(p) for p in agent.parameters()} != (_actor_ids | _critic_ids):
        raise ValueError("v10 optimizer groups must cover all parameters exactly once")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            advantages, returns = gae_loop_fn(
                rewards, values, next_transition_values,
                transition_terminations, transition_valids, transition_boundaries,
                None, args.gamma, args.gae_lambda,
            )
            # cudagraph discipline (cf. v162 compiled lineage): compiled outputs
            # are graph-owned -- clone before they escape, or the caching
            # allocator recycles their memory for later allocations in-iteration.
            advantages, returns = advantages.clone(), returns.clone()
            ...
            if auto_alpha:
                policy_adv, _ = gae_loop_fn(
                    rewards, values, next_transition_values,
                    transition_terminations, transition_valids, transition_boundaries,
                    next_value_bonus, args.gamma, args.gae_lambda,
                )
                policy_adv = policy_adv.clone()  # cudagraph discipline, as above
            else:
                policy_adv = advantages  # already an owned clone from the reward path
            ...
            target_probs = hl_support_cpu.project_moment_matched(return_mtp.detach().cpu())
        ...
        # TRUE DG GATE (sole delta, arXiv:2603.14608 Alg. 2): w = sigmoid(U*l/eta)
        # with l = clip(-true log-density, -C, C). No whitening, no sign map.
        # The buffered logprob drops the constant rescale Jacobian (cancels in
        # the PPO ratio) so beta restores it from the action bounds; gaussian
        # stored logprob already includes tanh Jacobian.
        if args.dg:
            if args.actor_dist == "beta":
                beta_logdet = (agent.action_high - agent.action_low).log().sum()
                lp_true = b_logprobs - beta_logdet
            else:
                lp_true = b_logprobs
            dg_surprisal = (-lp_true).clamp(
                min=-args.dg_surprisal_clip, max=args.dg_surprisal_clip
            )
            dg_delight = b_policy_adv * dg_surprisal  # raw product, unwhitened
            dg_w = torch.sigmoid(dg_delight / args.dg_eta)
            b_policy_adv = b_policy_adv * dg_w
        else:
            dg_w = torch.ones_like(b_policy_adv)
            dg_surprisal = torch.zeros_like(b_policy_adv)
            dg_delight = torch.zeros_like(b_policy_adv)
        dg_w_mean = dg_w.mean().item()
        dg_w_std = dg_w.std().item()
        dg_pos_mass_frac = (
            (dg_w * (b_policy_adv > 0).to(dg_w.dtype)).sum() / dg_w.sum().clamp_min(1e-12)
        ).item()
        dg_delight_absmean = dg_delight.abs().mean().item()
        dg_sat_frac = (dg_delight.abs() > 3.0).float().mean().item()
            ...
            for mb_inds_cpu in device_minibatches(
                ...
                if args.nextlat:
                    ...
                    for i in range(1, args.nextlat_depth + 1):
                        ...
                        # Manual categorical KL (9e-7-identical to kl_divergence
                        # in-range; caps +inf tail spikes at 20 nats).
                        tgt_logp = torch.log_softmax(tgt_logits, dim=-1).clamp_min(-20.0)
                        stu_logp = torch.log_softmax(z_hat, dim=-1).clamp_min(-20.0)
                        ckl_l = (tgt_logp.exp() * (tgt_logp - stu_logp)).sum(-1)
        ...
        log_tensors["dg_w_mean"] = torch.as_tensor(dg_w_mean, device=device)
        log_tensors["dg_w_std"] = torch.as_tensor(dg_w_std, device=device)
        log_tensors["dg_pos_mass_frac"] = torch.as_tensor(dg_pos_mass_frac, device=device)
        log_tensors["dg_delight_absmean"] = torch.as_tensor(dg_delight_absmean, device=device)
        log_tensors["dg_sat_frac"] = torch.as_tensor(dg_sat_frac, device=device)
        if args.nextlat:
        ...
        writer.add_scalar("debug/dg_w_mean", log_vals["dg_w_mean"], global_step)
        writer.add_scalar("debug/dg_w_std", log_vals["dg_w_std"], global_step)
        writer.add_scalar("debug/dg_pos_mass_frac", log_vals["dg_pos_mass_frac"], global_step)
        writer.add_scalar("debug/dg_delight_absmean", log_vals["dg_delight_absmean"], global_step)
        writer.add_scalar("debug/dg_sat_frac", log_vals["dg_sat_frac"], global_step)
