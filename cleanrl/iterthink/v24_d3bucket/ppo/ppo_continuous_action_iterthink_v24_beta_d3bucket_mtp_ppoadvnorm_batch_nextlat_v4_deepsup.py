# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT DEEP-SUP v4.
# =====================================================================================
# WHAT'S NEW vs nextlat_v1: every trunk block latent must predict the future OUTPUT
# latent -- classic deep supervision (GoogLeNet-style aux heads), not self-prediction.
#   v2_perlayer had each block predict ITSELF at t+i. That target is too easy at the
#   bottom of the stack: feats[0] = entry(x) is a linear map of the observation, and
#   MuJoCo obs evolve near-linearly (obs + dt*velocity), so a 2-layer MLP solves it
#   almost immediately, saturates, and dilutes the one term that was still learning.
#   Here every block instead targets h(t+i), the OUTPUT latent the policy actually
#   consumes. That is strictly harder for early blocks -- entry(x_t) must already carry
#   enough information to pin down the behaviourally relevant future representation.
#   Rollout structure: P_k lifts block-k's latent into predicted OUTPUT space one step
#   ahead, then the SHARED output dynamics model P_out continues the chain:
#       g_k^(1) = P_k(feats_k(t),  a_t)                 -> target h(t+1)
#       g_k^(i) = P_out(g_k^(i-1), a_{t+i-1})           -> target h(t+i)
#   So each P_k is purely a depth-k -> output-space lift and the temporal dynamics are
#   shared, which also gives P_out extra starting points to learn from. Input spaces
#   stay consistent at every step (the alternative, chaining each P_k on its own
#   output, would feed it block-space vectors at i=1 and output-space ones after).
#   All components share ONE target per depth, so targets cost one trunk forward, not
#   a full feats stack. SmoothL1 only on the block terms (the KL stays on the output
#   chain as in v1), so v4 differs from v2_perlayer in exactly one thing: WHAT the
#   block latents are asked to predict.
# Hypothesis: deep supervision toward the future output latent avoids the saturation
#   that self-prediction invites at the bottom of the stack, and constrains every block
#   to encode the future of the representation the policy reads.
# Watch: losses/nextlat_pred_layer0 (the entry embedding). Under v2_perlayer this was
#   expected to collapse toward 0; here it should stay comparable to the output term.
# =====================================================================================
#
# INHERITED BASE (nextlat_v1) BELOW.
...
# Watch: losses/nextlat_pred_out vs losses/nextlat_pred_layer0..K, nextlat_kl_loss
...
class Args:
    ...
    nextlat_grad_clip: float = 0.1   # own dual-backward budget (trunk total: 0.25+0.25+0.1)
    nextlat_deepsup: bool = True     # v4: every block latent predicts the future OUTPUT latent
    nextlat_layer_grad_clip: float = 0.1  # OWN budget for the lift predictors, so the
                                     # shared trunk keeps the same 0.1 aux budget as v1 and
                                     # this arm isolates aux DIRECTION, not magnitude
...
class ThinkTrunk(nn.Module):
    ...
    def forward_feats(self, x):
        # (trunk output, [entry latent, block_1 latent, ...]). All are width H.
        x0 = self.entry(x)
        feats = [x0]
        for block in self.blocks:
            feats.append(block(torch.cat(feats, dim=-1), x0))
        return self.out_proj(self.out_norm(torch.cat(feats, dim=-1))), feats
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # DEEP SUPERVISION (v4). One LIFT model per block latent: P_k maps block-k's
        # latent at t into the predicted OUTPUT latent at t+1. From there the shared
        # output dynamics model carries the chain, so every block is graded against
        # h(t+i) -- the representation the policy actually reads -- rather than against
        # its own future, which is near-trivial at the bottom of the stack. Initialized
        # LAST for RNG parity with the base run.
        self.nextlat_deepsup = args.nextlat_deepsup
        if self.nextlat_deepsup:
            self.nextlat_lift_predictors = nn.ModuleList(
                [
                    nn.Sequential(
                        layer_init(nn.Linear(H + act_dim, H)),
                        ReLUSquared(),
                    ...
                    for _ in range(args.k_blocks + 1)
                ]
            )

    def get_actor_feats_all(self, x):
        # (trunk output, per-block latents) from the ACTOR-side trunk.
        trunk = self.trunk if self.share_backbone else self.actor_trunk
        return trunk.forward_feats(x)
    ...
    def nextlat_layer_parameters(self):
        # The per-layer dynamics MLPs get their OWN clip group. Deliberate: nextlat_grad_clip
        # is a GROUP norm, so folding these into the main group would shrink the shared
        # trunk's share of the 0.1 budget and confound "deep supervision helps" with
        # "the trunk got less aux gradient". Split this way, the trunk's aux budget is
        # identical to v1 and only the DIRECTION of its aux gradient changes.
        return list(self.nextlat_lift_predictors.parameters()) if self.nextlat_deepsup else []
...
if __name__ == "__main__":
    ...
    nextlat_layer_params = agent.nextlat_layer_parameters()

    # Compiled hot paths. Separate wrappers per call site so each sees ONE static shape
    # (dynamic=False): rollout policy (num_envs rows), update policy (minibatch rows),
    # flat value bootstrap (batch rows). Call sites use cudagraph_mark_step_begin().
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.nextlat:
                    if args.nextlat_deepsup:
                        # One extra actor-trunk forward to expose the interior latents.
                        # Its output equals mb_actor_feat; a separate graph is fine since
                        # the aux already takes its own dual-backward pass.
                        mb_out, mb_feats = agent.get_actor_feats_all(b_obs[mb_inds])
                        h_hat = mb_out
                        g_layers = None  # per-block predictions, in OUTPUT latent space
                    else:
                        h_hat = mb_actor_feat
                    pred_losses, kl_losses = [], []
                    # Per-component accumulators (index 0 = trunk output term, 1..K+1 =
                    # block latents). Logged separately: without this a null result is
                    # uninterpretable -- "deep supervision doesn't help" and "the block
                    # targets were trivial and contributed nothing" look identical in
                    # the averaged loss.
                    comp_losses = [[] for _ in range(args.k_blocks + 2)] if args.nextlat_deepsup else []
                    for i in range(1, args.nextlat_depth + 1):
                        act_idx = np.clip(mb_inds + (i - 1) * args.num_envs, 0, args.batch_size - 1)
                        tgt_idx = np.clip(mb_inds + i * args.num_envs, 0, args.batch_size - 1)
                        act_i = b_actions[act_idx]
                        h_hat = agent.nextlat_predictor(torch.cat([h_hat, act_i], dim=-1))
                        with torch.no_grad():
                            # ONE target per depth now: every component -- the output
                            # chain and all block lifts -- is graded against h(t+i).
                        ...
                        if args.nextlat_deepsup:
                            # i=1: lift each block latent into predicted OUTPUT space.
                            # i>1: the SHARED output dynamics model continues each chain,
                            # so every predictor only ever sees inputs from the space it
                            # was trained on. All graded against the same h(t+i).
                            if g_layers is None:
                                g_layers = [
                                    lift(torch.cat([mb_feats[k], act_i], dim=-1))
                                    for k, lift in enumerate(agent.nextlat_lift_predictors)
                                ]
                            else:
                                g_layers = [
                                    agent.nextlat_predictor(torch.cat([g, act_i], dim=-1))
                                    for g in g_layers
                                ]
                            layer_terms = [pred_l]
                            for g in g_layers:
                                layer_terms.append(
                                    F.smooth_l1_loss(g, tgt_feat, reduction="none").mean(-1)
                                )
                            for c, term in enumerate(layer_terms):
                                comp_losses[c].append((term * mask_i).sum() / denom)
                            pred_l = torch.stack(layer_terms, dim=0).mean(dim=0)
                    ...
                    if args.nextlat_deepsup:
                        nextlat_comp = [torch.stack(v).mean().detach() for v in comp_losses]
                ...
                if args.separate_grad_clip:
                    ...
                    if args.nextlat:
                        ...
                        nextlat_layer_gn = torch.zeros((), device=device)
                        if nextlat_layer_params:
                            # Disjoint group, clipped separately from the same backward.
                            nextlat_layer_gn = nn.utils.clip_grad_norm_(
                                nextlat_layer_params, args.nextlat_layer_grad_clip
                            )
                        nextlat_grads = [
                            (p, p.grad.detach().clone())
                            for p in (nextlat_params + nextlat_layer_params)
        ...
        if args.nextlat:
            ...
            if args.nextlat_deepsup:
                # comp 0 = output chain (the v1 term); comp 1 = lift from entry(x);
                # comp 2.. = lifts from each ThinkTrunk block latent. All vs h(t+i).
                # Raw (pre-clip) norm of the lift group: if this falls under
                # nextlat_layer_grad_clip the lifts have stopped learning.
                writer.add_scalar("losses/nextlat_layer_grad_norm", float(nextlat_layer_gn), global_step)
                writer.add_scalar("losses/nextlat_pred_out", nextlat_comp[0].item(), global_step)
                for _k in range(1, len(nextlat_comp)):
                    writer.add_scalar(f"losses/nextlat_pred_layer{_k - 1}", nextlat_comp[_k].item(), global_step)
