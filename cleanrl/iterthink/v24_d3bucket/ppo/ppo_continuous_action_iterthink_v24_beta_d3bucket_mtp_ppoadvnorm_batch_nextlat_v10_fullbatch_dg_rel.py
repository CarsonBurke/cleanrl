# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT v10_fullbatch + DG-REL.
# =====================================================================================
# DG-REL: delight-gate advantage weighting with RELATIVE (batch-rank) rareness on
# the v10_fullbatch base (64x39 rollouts, single full-batch update, unshared
# trunks, split LRs, head-split auxes). Same gate as v4_dg: rareness q in (0,1]
# (1 = rarest) is the RANK of the behavior log-density over the rollout, and
#   w = pos_common + (pos_rare - pos_common)*q   if A >= 0
#   w = neg_common * (1 - q)                      if A <  0
# defaults (0.5/1.0/0.5) reproduce the DG plots. Applied to the shaped policy
# advantage BEFORE batch norm_adv (relative shaping preserved). `--no-dg` is
# bit-identical to the v10 base (no new params, no RNG draws -- argsort is
# deterministic). Structural bonus of this base: update_epochs=1 over the full
# batch, so behavior-rareness has ZERO staleness by construction (the v4
# epochs->1 staleness question is moot here).
# HYPOTHESIS: same as v4 (reinforce delights, never punish failed exploration),
# now on the stronger v10 base where the aux/trunk confounds are gone by
# construction. Falsifiable vs batch_nextlat_v10_vmpo (@1M 2265): if DG-REL <=
# it, the asymmetry costs more signal than it saves on this base too.
...
class Args:
    ...
    # gating of the shaped policy advantage before batch norm_adv. REL version:
    # q = batch rank of behavior log-density (1 = rarest). w_pos = pos_common +
    # (pos_rare - pos_common)*q; w_neg = neg_common*(1-q). Defaults match DG plots.
    dg: bool = True
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # DG-REL (sole delta): sign-asymmetric RANK rareness gating BEFORE batch
        # norm_adv (relative shaping preserved). q from behavior log-density over
        # the rollout; rank => scale- and Jacobian-proof. Single full-batch
        # update => zero staleness by construction.
        if args.dg:
            flat_lp = b_logprobs
            n_dg = flat_lp.numel()
            r_dg = flat_lp.argsort().argsort().to(torch.float32)  # 0 = rarest
            q_dg = 1.0 - r_dg / max(n_dg - 1, 1)  # 1 = rarest in rollout
