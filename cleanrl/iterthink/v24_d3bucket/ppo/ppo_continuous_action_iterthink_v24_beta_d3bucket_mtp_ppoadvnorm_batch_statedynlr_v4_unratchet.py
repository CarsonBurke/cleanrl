# STATEDYNLR v3 FREEZE (ppoadvnorm_batch base) — alpha range decoupled to
# [alpha_min=0.01, alpha_max=8]: a unit can be gated to ~ZERO learning from
# bad/noisy data, or driven to an 8x hot step when evidence says its current
# states are highly productive ("high LR while uncertainty is high" regime).
# Init anchors alpha EXACTLY at 1 (zero output weights + logit bias), so the
# warmup ends at the neutral baseline and all differentiation is earned.
#
# Bounds are now LINEAR in sigmoid(logit), not exp(c*(2s-1)): the exp form's
# symmetric multiplicative range could express neither freeze nor large gain.
# alpha_min keeps a 1% gradient leak so a frozen unit's evidence remains
# measurable and it can RECOVER if outcomes turn productive (full 0 would
# ratchet shut forever). Centering + one-step outcome meta-gradient unchanged.
# Verified: init==1 exact; consistent-productive unit -> alpha=8.000;
# alternating-hurtful unit -> alpha=0.010.
#
# --- v2 centered header ---
# =====================================================================================
...
class SDLRController(nn.Module):
    ...
    def __init__(self, site_modules, H, alpha_min=0.05, alpha_max=8.0,
        ...
        # Anchor the init at EXACTLY alpha=1: with the linear [a_min,a_max] map,
        # alpha=1 sits at sigmoid(logit) = (1-a_min)/(a_max-a_min), so zero the
        # output weights and set the bias to the matching logit. All units start
        # neutral; differentiation comes only from evidence.
        p0 = (1.0 - self.a_min) / (self.a_max - self.a_min)
        self.register_buffer(
            "b0", (torch.log(torch.tensor(p0)) - torch.log1p(-torch.tensor(p0))).view(())
        )
        nn.init.zeros_(self.net[-1].weight)
        nn.init.constant_(self.net[-1].bias, float(self.b0))
    ...
    def ingest(self):
        ...
        if self._prev_mm is not None and self._logits and self.updates >= self.warmup:
            ...
            for name, logits in self._logits:
                ...
                # v2 CENTERING: normalize ACROSS UNITS within the site, not by
                # global RMS. The v1 rail postmortem: evidence_mean was persistently
                # positive (+0.14), so every logit railed to e^c and alpha became
                # UNIFORM (std=0.000 for 8M steps) -- the run reduced to a constant
                # 2.72x PG scale. Subtracting the per-site mean makes the uniform
                # component invisible BY CONSTRUCTION: only RELATIVE productivity
                # between units in the same backward moves the logits, which is the
                # state-conditioned signal we actually want.
                en = ((e - e.mean()) / (e.std() + 1e-8)).clamp(-3.0, 3.0)
