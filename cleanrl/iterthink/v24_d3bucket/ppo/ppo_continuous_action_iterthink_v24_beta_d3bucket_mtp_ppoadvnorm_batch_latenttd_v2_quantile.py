# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + LATENT-TD v2 QUANTILE.
# =====================================================================================
# LATENT-TD v2, DISTRIBUTIONAL VECTOR TD (quantile successor). v1 regressed the MEAN of
# the discounted latent future — an EWMA whose low-pass filtering crushed all structure
# faster than ~100 steps; psi solved the mushy target and the aux gradient faded
# (confirmed in v1's diagnostics). The mush IS the first moment. v2 predicts the
# DISTRIBUTION of the gamma-model object instead: the geometric-horizon future latent
#     G_t = LN(z_{t+M}),  M ~ Geometric(1-gamma_lat)  (M >= 0, includes the present)
# whose mean is exactly v1's EWMA but whose spread carries everything the mean averaged
# away ("goes left or right" = two quantile branches, not a blurred zero). Note G is a
# future LATENT, not a sum — no (1-gamma) scaling needed; it lives at latent scale by
# construction. psi outputs N marginal quantiles per latent channel, trained by
# QR-DQN-style quantile-Huber TD on the distributional Bellman recursion:
#     G_t  =d  LN(z_t) w.p. (1-g)   |   G_{t+1} w.p. g
# TARGET ATOMS per state: atom0 = LN(z_t) with weight (1-g) + g*(1-bootstrap_mask)
# (termination collapses the tail mass onto the present), atoms 1..N = the SNAPSHOT
# psi quantiles at s_{t+1}, each with weight g*bootstrap_mask/N. One-step distributional
# bootstrap (no lambda — atoms are sample-sharp already); targets fixed per iteration
# from rollout-time trunk+psi, exactly like the critic's `returns`.
# HYPOTHESIS: keeping TD's infinite horizon while replacing the information-destroying
# expectation with quantiles yields a NON-SATURATING horizon-aware aux (v1's reach,
# nextlat's sharpness). A distributional successor representation as an actor auxiliary.
# Mechanics otherwise inherited from v1: own dual-backward clip budget (0.1 vs actor
# 0.25 / critic 0.25), LN latents, snapshot targets. --compile wraps the hot forward
# paths (torch.compile dynamic=False + cudagraph_mark_step_begin, reduce-overhead).
# Watch: losses/latent_td_loss (should NOT collapse to ~0), losses/latent_grad_norm
# (should hold near the 0.1 budget), debug/psi_quantile_std (spread across quantiles —
# -> 0 means degeneration to v1's mean prediction), debug/latent_zn_batch_std.
#
# CONFIG NOTE: this file bakes in the "ppoadvnorm_batch" base config as defaults
# (norm_adv=True, norm_adv_scope="batch", ret_percnorm=False) — i.e. the exact config of run
# HalfCheetah-v4__iterthink_v24_beta_d3bucket_mtp_ppoadvnorm_batch_v1__1__1782008060, which is
# the bar to beat. --no-latent-td reproduces that base's algorithm/config (shared params get
# bit-identical init; the extra psi head draws shift the RNG stream afterward, so the exact
# seed-1 trajectory is not bit-reproduced).
# =====================================================================================
#
...
class Args:
    ...
    # LATENT-TD v2 (quantile successor): distributional vector TD on the geometric-
    # horizon future latent (see header). Snapshot one-step distributional bootstrap;
    # quantile-Huber (QR-DQN) loss per latent channel.
    latent_td: bool = True
    latent_td_gamma: float = 0.99    # geometric horizon of the future-latent index (~100 steps)
    latent_td_quantiles: int = 8     # N marginal quantiles per latent channel
    latent_td_kappa: float = 0.25    # quantile-Huber width; LN latents are unit-scale, so
                                     # kappa=1 (QR-DQN default) would sit fully in the quadratic
                                     # regime and degrade toward expectile regression
    latent_td_coef: float = 1.0      # loss weight (the dedicated grad clip is the real ceiling)
    latent_td_grad_clip: float = 0.1 # own dual-backward budget (trunk total: 0.25+0.25+0.1)

    # torch.compile of the hot forward paths (reduce-overhead => CUDA graphs).
    compile: bool = False
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # LATENT-TD v2 psi head: N marginal quantiles per latent channel of the
        # geometric-horizon future LN latent. Small final init => quantiles start ~0
        # and tight, so early bootstrap atoms contribute little vs the real-latent
        # atom. Initialized LAST so all base-shared parameters draw identical initial
        # weights from the seeded RNG as the ppoadvnorm_batch base run.
        self.latent_td_n = args.latent_td_quantiles
        self.psi_head = nn.Sequential(
            layer_init(nn.Linear(H, H)),
            ReLUSquared(),
            layer_init(nn.Linear(H, H * args.latent_td_quantiles), std=0.1),
        )
    ...
    def get_feat_and_value(self, x):
        # Actor-side trunk feature AND value logits in one pass (one trunk forward when
        # shared). Used post-rollout on next_obses for the value bootstrap + psi bootstrap.
        actor_feat, critic_feat = self._trunks(x)
        return actor_feat, self.critic_head(critic_feat).view(-1, self.critic_mtp_horizon, self.num_bins)
    ...
    def latent_td_parameters(self):
        # Params receiving the LATENT-TD gradient: the ACTOR-side trunk (the representation
        # this aux exists to shape) plus the psi head. Clipped as its own dual-backward group.
        trunk = self.trunk if self.share_backbone else self.actor_trunk
...
if __name__ == "__main__":
    ...
    latent_params = agent.latent_td_parameters()

    # Quantile midpoints tau_i = (i+0.5)/N, shaped for the (M,H,N,N+1) pinball term.
    psi_taus = (
        (torch.arange(args.latent_td_quantiles, device=device, dtype=torch.float32) + 0.5)
        / args.latent_td_quantiles
    ).view(1, 1, -1, 1)

    # Compiled hot paths (one static shape each, dynamic=False): rollout policy
    # (num_envs rows), update policy (minibatch rows), post-rollout feat+value bootstrap
    # (batch rows). Call sites use torch.compiler.cudagraph_mark_step_begin()
    ...
    def feat_value_fn(obs_):
        return agent.get_feat_and_value(obs_)
    ...
    if args.compile:
        ...
        feat_value_fn = torch.compile(feat_value_fn, mode=args.compile_mode, dynamic=False)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            if args.compile:
                torch.compiler.cudagraph_mark_step_begin()
            next_actor_feats_flat, next_value_logits_all = feat_value_fn(
                next_obses.reshape((-1,) + envs.single_observation_space.shape)
            )
            next_transition_value_logits = next_value_logits_all[:, 0]
            ...
            # LATENT-TD v2 (quantile successor) targets: distributional Bellman atoms
            # for the geometric-horizon future latent G_t. Atom 0 is the REAL present
            # latent LN(z_t); atoms 1..N are the SNAPSHOT psi quantiles at s_{t+1}
            # (rollout-time trunk+psi = the natural lagged target). Termination folds
            # the bootstrap tail mass onto the present atom via the weights. Targets
            # are fixed across the epoch loop, exactly like the critic's `returns`.
            if args.latent_td:
                N_lat = agent.latent_td_n
                g_lat = args.latent_td_gamma
                zn = F.layer_norm(latents, (args.hidden,))                     # (T,B,H)
                next_actor_feats = next_actor_feats_flat.reshape(
                    args.num_steps, args.num_envs, args.hidden
                )
                psi_next = agent.psi_head(next_actor_feats).view(
                    args.num_steps, args.num_envs, args.hidden, N_lat
                )
                boot_mask = (1.0 - transition_terminations) * transition_valids  # (T,B)
                psi_atoms = torch.cat([zn.unsqueeze(-1), psi_next], dim=-1)    # (T,B,H,N+1)
                psi_w0 = (1.0 - g_lat) + g_lat * (1.0 - boot_mask)             # (T,B) present-atom weight
                psi_wboot = g_lat * boot_mask / N_lat                          # (T,B) per-bootstrap-atom weight
                # Diagnostics: quantile spread of the snapshot psi (-> 0 = degenerate to
                # mean prediction, i.e. v1 behavior) and the latent collapse probe.
                psi_quantile_std = psi_next.std(dim=-1).mean().item()
                latent_zn_batch_std = zn.reshape(-1, args.hidden).std(dim=0).mean().item()
        ...
        b_u = u.reshape(-1)
        if args.latent_td:
            b_psi_atoms = psi_atoms.reshape(-1, args.hidden, N_lat + 1)
            b_psi_w0 = psi_w0.reshape(-1)
            b_psi_wboot = psi_wboot.reshape(-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # LATENT-TD v2: quantile-Huber (QR-DQN) regression of the CURRENT psi
                # quantiles onto the fixed snapshot atom sets, weighted by the atom
                # probabilities. Gradient flows through psi_head INTO the trunk — that
                # is the point of the aux.
                if args.latent_td:
                    M_mb = len(mb_inds)
                    psi_q = agent.psi_head(mb_actor_feat).view(M_mb, args.hidden, N_lat)
                    atoms = b_psi_atoms[mb_inds]                               # (M,H,N+1)
                    # u[m,h,i,j] = atom_j - quantile_i
                    u = atoms.unsqueeze(2) - psi_q.unsqueeze(3)                # (M,H,N,N+1)
                    k_h = args.latent_td_kappa
                    huber = torch.where(u.abs() <= k_h, 0.5 * u * u / k_h, u.abs() - 0.5 * k_h)
                    rho = (psi_taus - (u.detach() < 0).float()).abs() * huber  # (M,H,N,N+1)
                    atom_w = torch.cat(
                        [
                            b_psi_w0[mb_inds].view(M_mb, 1, 1, 1),
                            b_psi_wboot[mb_inds].view(M_mb, 1, 1, 1).expand(M_mb, 1, 1, N_lat),
                        ],
                        dim=3,
                    )                                                          # (M,1,1,N+1), sums to 1
                    latent_td_loss = (rho * atom_w).sum(dim=3).mean()
                else:
                    latent_td_loss = None
                ...
                if args.separate_grad_clip:
                    ...
                    if args.latent_td:
                        optimizer.zero_grad(set_to_none=True)
                        (args.latent_td_coef * latent_td_loss).backward(retain_graph=True)
                        latent_gn = nn.utils.clip_grad_norm_(latent_params, args.latent_td_grad_clip)
                        latent_grads = [(p, p.grad.detach().clone()) for p in latent_params if p.grad is not None]
                    else:
                        latent_grads = []
                    optimizer.zero_grad(set_to_none=True)
                    ...
                    # clip_critic(d vl / d trunk) and clip_latent(d psi / d trunk).
                    # critic_head / psi_head get only their own loss's gradient.
                    for p, g in value_grads:
                        p.grad = g if p.grad is None else p.grad + g
                    for p, g in latent_grads:
                        p.grad = g if p.grad is None else p.grad + g
                    optimizer.step()
                else:
                    loss = pg_loss - ent_coef_eff * entropy_loss + v_loss * args.vf_coef
                    if args.latent_td:
                        loss = loss + args.latent_td_coef * latent_td_loss
                    optimizer.zero_grad()
                    loss.backward()
        ...
        if args.latent_td:
            writer.add_scalar("losses/latent_td_loss", latent_td_loss.item(), global_step)
            writer.add_scalar("losses/latent_grad_norm", float(latent_gn) if args.separate_grad_clip else 0.0, global_step)
            writer.add_scalar("debug/latent_zn_batch_std", latent_zn_batch_std, global_step)
            writer.add_scalar("debug/psi_quantile_std", psi_quantile_std, global_step)
            writer.add_scalar("debug/psi_atom_absmean", psi_atoms.abs().mean().item(), global_step)
        if auto_alpha:
