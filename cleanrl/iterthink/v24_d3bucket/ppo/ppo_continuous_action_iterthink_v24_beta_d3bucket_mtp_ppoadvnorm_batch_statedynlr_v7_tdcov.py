# STATEDYNLR V7 TDCOV on the PPOADVNORM(batch) base — CLOSED-FORM per-perceptron
# TD-covariance gain. NO learned controller.
# =====================================================================================
# LINE CONTEXT: v1 railed (sample-independent supervision => constant optimal); v2/v3
# regression found gradient-alignment outcomes unpredictable from any small feature set
# (pred_corr 0.001-0.017). v6 measured per-unit "consistency" as EMA[mean_i(g_ij)] /
# sqrt(EMA[mean_i(g_ij^2)]) -- but advantages are zero-mean standardized, so the
# unweighted batch-mean gradient is ~0 BY CONSTRUCTION: that statistic could never
# grow. Its near-zero readout was a measurement bug, not falsification. v6's early
# curve tracked the reference closely (+12% @200k) while being ~identity, so the
# modulation itself was untested.
#
# V7 fixes the statistic: accumulate the ADVANTAGE-WEIGHTED gradient -- each unit's
# persistent policy-gradient component, i.e. its actual TD-linked throughput:
#     c1_j = EMA[ mean_i(adv_i * g_ij) ]       signed signal
#     c2_j = EMA[ mean_i((adv_i * g_ij)^2) ]   fluctuation scale
#     gain_j = clamp(c1_j / sqrt(c2_j), -1, 1) signed per-unit SNR
#     w_ij  = exp(k * gain_j)
# A unit keeps positive gain only if its incoming gradients REPEATEDLY align with the
# advantage across updates -- temporal accumulation of TD covariance, information no
# raw-gradient second moment (Adam) captures. w redistributes policy-gradient direction
# toward high-TD-covariance units (under the binding actor norm clip, total magnitude
# is renormalized; direction is what changes). Per-perceptron granularity; identity at
# init; engaged from update 0; k=1 => w in [e^-1, e]. Applied in the actor backward
# only; stats captured pre-modulation/pre-clip. Sites: trunk.entry, ThinkBlock
# outputs, trunk.out_proj.
#
# HYPOTHESIS: TD-covariance-weighted allocation of policy-gradient magnitude across
# trunk units improves learning on this base.
...
# FAIL: < reference trajectory at two consecutive checkpoints, or gain_absmean ~ 0
#       sustained past ~1M steps (no persistent per-unit TD covariance exists).
# WATCH: debug/sdlr_gain_mean/_absmean/_max/_min (does the statistic come alive?),
#        debug/sdlr_alpha_mean/_std, debug/actor_clip_frac.
# =====================================================================================
...
class Args:
    ...
    # --- STATEDYNLR v7 TDCOV: CLOSED-FORM per-perceptron TD-covariance gain
    #     w_j = exp(k * gain_j), gain_j = clamp(EMA[mean_i(adv*g)] / sqrt(EMA[mean_i((adv*g)^2)]))
    statedynlr: bool = True          # modulate the POLICY gradient per trunk unit
    sdlr_k: float = 1.0              # gain: w in [exp(-k), exp(+k)]
    sdlr_ema_beta: float = 0.98      # EMA decay for the TD-covariance statistics c1/c2
    sdlr_lr: float = 1e-3            # UNUSED in v7 (kept for CLI compatibility; no optimizer)
    sdlr_warmup: int = 0             # UNUSED in v7: identity at init (gain starts at 0), no dead phase
...
class SDLRController(nn.Module):
    """CLOSED-FORM per-perceptron TD-covariance gain (v7 TDCOV). NO learned
    controller, NO regression, NO optimizer.

    V6 MEASUREMENT BUG (why its "cons ~ 0" verdict was wrong): cons_j accumulated the
    UNWEIGHTED batch-mean gradient m_j = EMA[mean_i(g_ij)]. Advantages are zero-mean
    standardized, so E[mean_i(g_ij)] ~ 0 BY CONSTRUCTION -- that statistic can never
    grow, regardless of per-unit structure. It measured nothing.

    V7 accumulates what actually carries the signal -- the ADVANTAGE-WEIGHTED
    gradient, i.e. each unit's persistent policy-gradient component:
        c1_j = EMA[ mean_i(adv_i * g_ij) ]          (signed TD-linked throughput)
        c2_j = EMA[ mean_i((adv_i * g_ij)^2) ]      (its fluctuation scale)
        gain_j = clamp(c1_j / sqrt(c2_j + eps), -1, 1)
    gain_j is stable-positive only for units whose incoming gradients REPEATEDLY
    align with the advantage across updates -- temporal accumulation of TD covariance,
    information no Adam-style raw-second-moment statistic captures. Modulation:
        w_ij = exp(k * gain_j)
    applied elementwise to the POLICY gradient in the actor backward only. Per-unit
    (per-perceptron) granularity; identity at init; engaged from update 0; k=1 =>
    w in [e^-1, e]. With actor_clip_frac=1 the norm clip renormalizes total magnitude,
    so w acts by REDISTRIBUTING gradient direction toward high-TD-covariance units --
    a TD-derived preconditioner.

    Stats captured pre-modulation/pre-clip in the hook; EMAs updated in ingest from
    the hook-captured grads and this pass's standardized advantage. Sites:
    trunk.entry, ThinkBlock outputs, trunk.out_proj.
    """

    def __init__(self, site_modules, H, k=1.0, ema_beta=0.98):
        super().__init__()
        ...
        for name in self.site_names:
            # advantage-weighted grad: signed mean and square-mean EMAs
            self.register_buffer(f"c1_{name}", torch.zeros(H))
            self.register_buffer(f"c2_{name}", torch.zeros(H))
        ...
        self._last_w = {}          # site -> weights of the LAST pass (for diagnostics)
        self._last_gain = None     # last flattened per-unit gain (for diagnostics)
        self._handles = []
    ...
    def _make_bwd(self, name):
        def hook(g):
            if not self.pg_pass or self._adv is None:
                return None
            self._grads[name] = g.detach()
            w_row = self._gain.get(name)
            if w_row is None:
                return g
            return g * w_row                      # (H,) broadcasts over (B, H)
        return hook

    def _gain_for(self, name):
        c1 = getattr(self, f"c1_{name}")
        c2 = getattr(self, f"c2_{name}")
        return (c1 / (c2.sqrt() + 1e-8)).clamp(-1.0, 1.0)   # (H,)

    def prepare(self, adv):
        """Store this minibatch's standardized advantage; freeze this pass's per-unit
        gains from the current EMAs."""
        self._adv = adv.detach()
        gains = []
        for name in self.site_names:
            g = self._gain_for(name)
            self._gain[name] = g.detach()
            gains.append(g)
        self._last_gain = torch.cat(gains)

    def ingest(self, adv=None):
        """After optimizer.step(): fold this backward's grads into the TD-covariance
        EMAs using the stored standardized advantage. Returns None by design."""
        adv = self._adv
        if adv is not None:
            aw = adv.unsqueeze(1)                              # (B, 1)
            for name in self.site_names:
                gd = self._grads.get(name)
                if gd is None:
                    continue
                ag = aw * gd                                   # (B, H) advantage-weighted
                c1 = getattr(self, f"c1_{name}")
                setattr(self, f"c1_{name}", self.beta * c1 + (1.0 - self.beta) * ag.mean(dim=0))
                c2 = getattr(self, f"c2_{name}")
                setattr(self, f"c2_{name}", self.beta * c2 + (1.0 - self.beta) * (ag * ag).mean(dim=0))
        self.updates += 1
        self._last_w = {name: torch.exp(self.k * gmat) for name, gmat in self._gain.items()}
        self._grads.clear()
        self._gain.clear()
        self._adv = None
        return None
