# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT LOOKAHEAD v3.
# =====================================================================================
# WHAT'S NEW vs nextlat_v1: the policy USES the world model at act time.
#   nextlat trains an action-conditioned latent dynamics MLP as a pure REGULARIZER and
#   then discards it at inference -- the policy remains a single feedforward pass
#   s_t -> a_t. v3 rolls that same predictor M steps forward under the policy's own
#   mean action, pools the predicted latents in LayerNorm space, and gates the result
#   into the actor feature:  h_aug = h_0 + g * W(mean_i LN(h_hat_i)).
#   The rollout runs under no_grad, so the predictor is trained ONLY by the aux; the
#   policy learns to READ the model and cannot corrupt it into a flattering feature
#   extractor. g is a ReZero-style zero-init scalar => at step 0 the acting
#   distribution is bit-identical to nextlat_v1, and the branch earns influence only
#   as the model becomes accurate. Downside is bounded: g can simply stay near 0.
#   The auxiliary itself is UNCHANGED from v1 (raw actor_feat is still what it
#   predicts and targets), so this arm isolates exactly one variable.
# Hypothesis: nextlat's measured gains come from a latent world model that is accurate
#   enough to be worth consulting; making the policy model-predictive rather than
#   model-regularized should compound that, most visibly late in training where
#   pred_loss/KL show the model is still sharpening (0.58->1.49 / 0.08->2.61 by 4M).
# Watch: debug/la_gate (does the policy choose to use the model?), losses/nextlat_*.
# =====================================================================================
#
...
class Args:
    ...
    nextlat_lookahead: bool = True   # v3: the POLICY reads the world model at act time
    nextlat_lookahead_steps: int = 3 # M-step deterministic latent rollout fed to the head
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # LOOKAHEAD (v3). nextlat trains an action-conditioned latent world model and
        # then THROWS IT AWAY at inference: the policy is still one feedforward pass.
        # Here the policy READS the model -- it rolls the predictor M steps under its
        # own mean action and conditions on where it is heading. ReZero-style scalar
        # gate is zero-init, so at step 0 the acting distribution is bit-identical to
        # nextlat_v1 and the branch only gains influence as the model earns it.
        # Initialized LAST for RNG parity with the base run.
        # Gated on nextlat too: with --no-nextlat the predictor never receives a
        # gradient, and LayerNorm would rescale its untrained output back to unit
        # magnitude -- the "base" control would then be base + structured noise.
        self.lookahead_steps = (
            args.nextlat_lookahead_steps if (args.nextlat_lookahead and args.nextlat) else 0
        )
        self.la_proj = layer_init(nn.Linear(H, H), std=0.1)
        self.la_gate = nn.Parameter(torch.zeros(1))

    def _lookahead(self, h0):
        # Deterministic M-step latent rollout, pooled in LayerNorm space and gated into
        # the actor feature. Run under no_grad: the lookahead is a READ of the world
        # model, never a training signal for it. That matters -- if the policy gradient
        # reached the predictor it could corrupt the model into a free-form feature
        # extractor that flatters the policy instead of predicting the future. The
        # model is trained ONLY by the aux; the policy only learns how to USE it.
        ...
        with torch.no_grad():
            ...
            for _ in range(self.lookahead_steps):
                dist, to_action, _ = self._actor_dist(h)
                # env-space action: matches what the aux conditions on (b_actions).
                a = to_action(dist.mean)
                h = self.nextlat_predictor(torch.cat([h, a], dim=-1))
                hn = F.layer_norm(h, (h.shape[-1],))
                acc = hn if acc is None else acc + hn
            u = acc / self.lookahead_steps
        return h0 + self.la_gate * self.la_proj(u)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        # Act from the LOOKAHEAD-augmented feature. The RAW actor_feat is what the
        # nextlat aux predicts and targets, so the auxiliary is unchanged from v1 and
        # this arm isolates exactly one variable: does the policy benefit from reading
        # its own world model?
        dist, to_action, log_det_fn = self._actor_dist(self._lookahead(actor_feat))
        if z is None:
            z = dist.sample()
    ...
    def actor_parameters(self):
        ...
        # la_gate/la_proj take the POLICY gradient only (the rollout they read is
        # detached), so they belong to the actor clip group.
        return list(trunk.parameters()) + heads + [self.la_gate] + list(self.la_proj.parameters())
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        writer.add_scalar("debug/la_gate", agent.la_gate.item(), global_step)
        writer.add_scalar("debug/la_steps", float(agent.lookahead_steps), global_step)
