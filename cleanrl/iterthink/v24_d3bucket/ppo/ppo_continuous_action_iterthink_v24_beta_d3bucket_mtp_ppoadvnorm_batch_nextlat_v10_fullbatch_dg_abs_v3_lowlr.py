# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT v10_fullbatch + DG-ABS, LOW LR (v3).
# v3 delta vs DG-ABS: 5x lower LRs (actor 3e-3->6e-4, critic 1e-3->2e-4, ratio
# kept, annealing schedule unchanged). Tests the high-LR hypothesis: the rel
# freeze (aux-capture -> head softplus-death) and the abs 3x slowdown are
# attributed to oversized steps, not to the gate or the aux per se. If v3
# tracks baseline speed with a healthy gate, LR was the whole story.
# =====================================================================================
...
class Args:
    ...
    actor_lr: float = 6e-4          # v3: 5x reduction (3e-3/5); ratio to critic kept
    critic_lr: float = 2e-4         # v3: 5x reduction (1e-3/5)
