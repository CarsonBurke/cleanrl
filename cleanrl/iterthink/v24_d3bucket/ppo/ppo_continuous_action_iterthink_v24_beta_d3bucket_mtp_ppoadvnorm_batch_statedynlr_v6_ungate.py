# STATEDYNLR v6 UNGATE (ppoadvnorm_batch base) — DERIVED uncertainty x
# informativeness plasticity gate. No learned controller, no gradient evidence,
# nothing trained.
# =====================================================================================
# RETHINK RATIONALE. Six controller variants shared one load-bearing component:
# outcome evidence e = <g_{t+1}, u_t> (consecutive-minibatch gradient products).
# That number is noise-dominated for trunk units (~0 correlation, measured in
# v1); every normalization (RMS, zero-sum centering, decay) rearranged deck
# chairs around it. Meanwhile the quantities that actually encode "should this
# unit learn from this transition right now?" were already computed by the base
# and unused:
#
#   UNCERTAINTY  U(s)    -- the distributional critic's per-state return std
#                           sigma(s). Uncertain state => high plasticity;
#                           familiar/confident state => damp toward floor.
#                           ("LR super high until my uncertainty diminishes")
#   INFORMATIVENESS I(t) -- |advantage| of the transition. Near-zero |A|
#                           carries nothing to learn => gate to EXACT 0 (this
#                           kills pure noise, not error: large-negative-A still
#                           learns at full weight -- error magnitude is signal,
#                           not valence).
#   PARTICIPATION P_ij   -- |activation| of unit j for sample i, normalized to
#                           mean-1 over units: units not representing the
#                           current state receive little plasticity. This is
#                           the per-perceptron axis; every unit is judged on
#                           its own participation in its own sample --
#                           non-competitive.
#
#   alpha_ij(s,t) = clamp( w_i * P_ij / calib , [0, alpha_max] )
#   w_i = U(s_i)^p * I(t_i), both renormalized to batch-mean 1
#   calib = EMA of batch-mean raw alpha  => AVERAGE applied scale stays 1
#
# Scale-neutrality by construction (the lesson of the whole line: railed v1's
# win was an accidental uniform LR boost; centered's win was variance reduction
# without scale distortion; v5's 8x drift destroyed early learning). The gate
# redistributes the existing policy-gradient budget toward (uncertain state x
# informative transition x participating unit) and away from everything else,
# hitting exact 0 and the extremes without any rail, ratchet, or meta-gradient
# to stabilize.
#
# Recipe unchanged: ppoadvnorm_batch (norm_adv=True, norm_adv_scope="batch",
# ret_percnorm=False, adv_transform="v10"), Beta actor, d3bucket MTP critic,
# shared trunk + dual-backward decoupled clip. Score to beat: railed control
# 9899; trajectory benchmark: centered 3472/5121 @1M/1.5M.
# PASS: > base 8454 AND > railed 9899.
# FAIL: <= base at 2 consecutive checkpoints past 2M.
# WATCH: debug/ung_alpha_mean/_std (allocation spread),
#        debug/ung_U_gt1_frac (how often uncertainty raises plasticity),
#        debug/ung_I_zero_frac (how much data is gated off as noise),
...
class Args:
    ...
    k_blocks: int = 3
    n_experts: int = 16
    # --- STATEDYNLR v6 UNGATE: derived uncertainty x informativeness x participation gate ---
    statedynlr: bool = True          # gate the POLICY gradient per (sample, trunk unit)
    ung_k: float = 2.0               # uncertainty sigmoid slope on centered log sigma(s)
    ung_power: float = 1.0           # exponent on U (p>1 sharpens the uncertain-state focus)
    ung_alpha_max: float = 4.0       # per-(sample, unit) ceiling; floor is exact 0
    ung_ema_rate: float = 0.05       # EMA rate of the scale-calibration constant
...
class UncertaintyGate(nn.Module):
    """v6 UNGATE -- DERIVED plasticity gate; nothing trained, no gradient evidence.

    alpha_ij(s,t) = clamp(w_i * P_ij / calib, [0, alpha_max])
      w_i  = U(s_i)^p * I(t_i), U and I each renormalized to batch-mean 1
      U(s) = 2*sigmoid(k*(log sigma(s) - batch-median log sigma))     in (0,2)
      I(t) = clamp(|A_t|/P90(|A|), 0, 2)                              exact 0 for noise
      P_ij = |a_ij| / mean_j |a_ij|                                   participation
      calib= EMA of batch-mean raw alpha => average applied scale stays ~1

    Applied elementwise to the POLICY gradient during the actor backward only.
    """

    def __init__(self, site_modules, k=2.0, power=1.0, alpha_max=4.0, ema_rate=0.05):
        super().__init__()
        ...
        self.register_buffer("calib", torch.ones(()))
        self.active = False       # hooks scale ONLY during the policy backward
        self._acts = {}           # site -> detached activations from this forward
        self._alphas = {}
        self._last_stats = {}
        self._handles = []
        # no parameters => no RNG consumption => init stream matches the base exactly
    ...
    @torch.no_grad()
    def prepare(self, log_sigma, adv):
        """log_sigma (B,), adv (B,) RAW GAE of this minibatch."""
        med = log_sigma.median()
        U = 2.0 * torch.sigmoid(self.k * (log_sigma - med))
        U = U / U.mean().clamp_min(1e-8)
        info = adv.abs()
        q90 = torch.quantile(info, 0.9)
        I = torch.clamp(info / (q90 + 1e-8), 0.0, 2.0)
        # explicit noise cutoff: |A| below 1% of P90 is noise BY DEFINITION --
        # those transitions gate to EXACT 0 (not just ~0).
        I = torch.where(info < 0.01 * (q90 + 1e-8), torch.zeros_like(I), I)
        m = I.mean()
        I = torch.ones_like(I) if m < 1e-8 else I / m
        w = (U**self.power) * I
        ...
        for name in self.site_names:
            ...
            if act is None:
                continue
            P = act.abs() / (act.abs().mean(dim=1, keepdim=True) + 1e-8)
            raws[name] = w.unsqueeze(-1) * P
        batch_mean = torch.stack([r.mean() for r in raws.values()]).mean()
        self.calib.mul_(1.0 - self.ema_rate).add_(batch_mean.detach(), alpha=self.ema_rate)
        for name, r in raws.items():
            self._alphas[name] = (r / self.calib).clamp(0.0, self.alpha_max)
        self._last_stats = {
            "debug/ung_U_gt1_frac": (U > 1.0).float().mean().item(),
            "debug/ung_I_zero_frac": (I <= 1e-6).float().mean().item(),
        }
        self._acts.clear()

    def diagnostics(self):
        out = dict(self._last_stats)
        alphas = [a for a in self._alphas.values() if a.numel() > 1]
        if alphas:
            all_a = torch.cat([a.reshape(-1) for a in alphas])
            out["debug/ung_alpha_mean"] = all_a.mean().item()
            out["debug/ung_alpha_std"] = all_a.std().item()
            for name in self.site_names:
                a = self._alphas.get(name)
                if a is not None and a.numel() > 1:
                    out[f"debug/ung_alpha_site_{name}"] = a.mean().item()
        return out
...
if __name__ == "__main__":
    ...
    if args.statedynlr:
        ...
        ung = UncertaintyGate(
            site_modules,
            k=args.ung_k,
            power=args.ung_power,
            alpha_max=args.ung_alpha_max,
            ema_rate=args.ung_ema_rate,
        ).to(device)
        # no parameters => the init RNG stream is untouched; --no-statedynlr still
        # matches the base bit-for-bit at the same seed.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    if ung is not None:
                        # Derive the per-(sample, unit) gate from uncertainty x
                        # informativeness x participation, then let the hooks
                        # scale the POLICY gradient during the backward below.
                        ung.prepare(torch.log(b_sigma[mb_inds].clamp_min(1e-6)),
                    ...
                    # ung alpha would be cancelled exactly by this clip)
        ...
        if ung is not None:
            for k, v in ung.diagnostics().items():
