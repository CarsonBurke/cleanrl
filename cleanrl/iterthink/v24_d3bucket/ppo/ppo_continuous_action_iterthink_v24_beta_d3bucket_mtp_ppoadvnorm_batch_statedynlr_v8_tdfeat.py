# STATEDYNLR V8 TDFEAT on the PPOADVNORM(batch) base — CLOSED-FORM per-perceptron
# TD-feature-relevance gain. NO learned controller.
# =====================================================================================
# TOTAL RETHINK: every prior variant correlated GRADIENTS with the advantage --
# within-batch (noise-dominated by action sampling, SNR ~0.002: v2/v3 pred_corr
# 0.001-0.017; v6/v7 closed-form statistics at or below their own noise floor), or
# accumulated across updates (gradient directions rotate; nothing settles). The
# quantity never measured is the STABLE one: covariance between a unit's ACTIVATION
# and the TD(lambda) advantage. Activations are smooth state functions -- their
# correlation with the return signal persists across minibatches and updates. This
# is the same information the repo-dominant td7/SALE line exploits (TD-driven
# selection of reward-relevant latent features), recast as a per-perceptron LR gain.
#
# MECHANISM:
#     c_j    = mean_i( xbar_ij * adv_i )            activation-advantage covariance
#     chat_j = clamp( c_j / RMS_EMA_j(c), -3, 3 )   self-normalized over time
#     w_ij   = exp( k * chat_j )                    per-unit LR gain
# Units whose activations track how good the state is -- which PREDICT the TD
# signal -- get amplified policy-gradient throughput; TD-uninformative units sit at
# chat ~ 0 => w ~ 1. Rail-proof by construction: self-normalization keeps chat O(1)
# even if all covariances grow together; identity at init. Applied in the actor
# backward only (norm clip renormalizes magnitude => direction redistribution toward
# TD-informative units). Sites: trunk.entry, ThinkBlock outputs, trunk.out_proj.
# k=1 => w in [e^-1, e].
#
# HYPOTHESIS: concentrating policy-gradient throughput on TD-relevant representation
# units improves learning on this base.
# PASS: >= +20% over the reference trajectory at matched steps (5015/6634/7602
#       @2/4/6M, 8242 end) -- below that is unresolvable single-seed noise.
# FAIL: < reference trajectory at two consecutive checkpoints, or gain_absmean < 0.05
#       sustained past ~1M steps (activations carry no TD-relevant structure the
#       gain can act on).
# WATCH: debug/sdlr_gain_mean/_absmean/_max/_min, debug/sdlr_alpha_mean/_std,
...
class Args:
    ...
    # --- STATEDYNLR v8 TDFEAT: CLOSED-FORM per-perceptron TD-feature-relevance gain
    #     w_j = exp(k * chat_j), chat_j = clamp(cov(h_j, adv) / RMS_EMA(cov), -3, 3)
    statedynlr: bool = True          # modulate the POLICY gradient per trunk unit
    sdlr_k: float = 1.0              # gain: w in [exp(-k), exp(+k)]
    sdlr_ema_beta: float = 0.98      # EMA decay for the covariance self-normalizer
    sdlr_lr: float = 1e-3            # UNUSED in v8 (kept for CLI compatibility; no optimizer)
    sdlr_warmup: int = 50            # passes before gains engage (RMS EMA must see real data)
...
class SDLRController(nn.Module):
    """CLOSED-FORM per-perceptron TD-FEATURE-RELEVANCE gain (v8 TDFEAT). NO learned
    controller, NO regression, NO optimizer.

    TOTAL RETHINK RATIONALE: every prior variant correlated GRADIENTS with the
    advantage -- either within-batch (noise-dominated by action sampling, SNR ~0.002)
    or accumulated over time (gradient directions rotate, never settle). None measured
    the stable, low-frequency quantity: covariance between a unit's ACTIVATION and the
    TD(lambda) advantage. Activations are smooth state functions; their correlation
    with the return signal persists across minibatches. This is the same information
    the repo-dominant td7/SALE line exploits (TD-driven selection of reward-relevant
    latent features), applied as a per-perceptron learning-rate gain.

    Mechanism:
        c_j     = mean_i( xbar_ij * adv_i )          activation-advantage covariance
        chat_j  = clamp( c_j / rmsEMA_j(c), -3, 3 )  self-normalized over time
        w_ij    = exp( k * chat_j )                  per-unit LR gain
    xbar_ij is the batch-RMS-normalized activation of unit j for sample i (captured in
    the forward hook). Units whose activations track how good the state is -- i.e.
    which predict the TD signal -- get amplified policy-gradient throughput;
    TD-uninformative units decay toward w=1 (chat->0 since c->0). Self-normalization
    by the running RMS of c makes the scale-free statistic rail-proof: if all c_j grow,
    chat_j stays O(1); identity at init (EMAs start at zero => division floor).

    Applied elementwise to the POLICY gradient in the actor backward only; under the
    binding actor norm clip this redistributes gradient DIRECTION toward
    TD-informative units. Sites: trunk.entry, ThinkBlock outputs, trunk.out_proj.
    """

    def __init__(self, site_modules, H, k=1.0, ema_beta=0.98, warmup=50):
        super().__init__()
        self.site_names = list(site_modules.keys())
        self.H = H
        self.k = k
        self.beta = ema_beta
        self.warmup = warmup   # passes before gains engage: RMS EMA must see real data
        self.updates = 0
        for name in self.site_names:
            # running first/second moment of the raw covariance c_j (for z-scoring)
            self.register_buffer(f"c_{name}", torch.zeros(H))   # EMA of c_j
            self.register_buffer(f"c2_{name}", torch.zeros(H))  # EMA of c_j^2
        # runtime state (not parameters)
        self.pg_pass = False       # hooks act ONLY during the policy backward
        self._acts = {}            # site -> detached activations from THIS forward
        self._gain = {}            # site -> detached (H,) gain used THIS pass
        self._last_w = {}          # site -> weights of the LAST pass (diagnostics)
        self._last_gain = None     # last flattened per-unit gain (diagnostics)
        self._pending_c = []       # [(site, c_j)] captured this pass, folded in ingest
        self._handles = []
        # NOTE: register_full_backward_hook cannot MODIFY grads on torch 2.12, so the
        # modulation attaches a TENSOR hook to each site's output tensor during the
        # update forward instead -- same semantics, supported API.
    ...
    def _make_bwd(self, name):
        def hook(g):
            if not self.pg_pass:
                return None
            w_row = self._gain.get(name)
            return g if w_row is None else g * w_row   # (H,) broadcasts over (B, H)
        return hook

    def prepare(self, adv):
        """Compute this pass's per-unit gains: batch activation-advantage covariance,
        self-normalized by its running RMS. Stashes raw covariances for ingest."""
        advd = adv.detach()
        ...
        for name in self.site_names:
            ...
            if act is None:
                continue
            x = act / (act.pow(2).mean().sqrt() + 1e-6)      # batch-RMS normalize
            c = (x * advd.unsqueeze(1)).mean(dim=0)          # (H,) raw covariance
            if modulate:
                rms = getattr(self, f"c2_{name}").sqrt()     # running RMS of c
                chat = (c / (rms + 1e-6)).clamp(-3.0, 3.0)
                self._gain[name] = chat.detach()
                gains.append(chat)
            else:
                self._gain[name] = torch.zeros_like(c)       # identity while warming
                gains.append(self._gain[name])
            pending.append((name, c))
        self._last_gain = torch.cat(gains) if gains else None
        self._pending_c = pending

    def ingest(self, adv=None):
        """After optimizer.step(): fold this pass's raw covariances into the running
        first/second-moment EMAs used for self-normalization."""
        for name, c in self._pending_c:
            cm = getattr(self, f"c_{name}")
            setattr(self, f"c_{name}", self.beta * cm + (1.0 - self.beta) * c)
            cv = getattr(self, f"c2_{name}")
            setattr(self, f"c2_{name}", self.beta * cv + (1.0 - self.beta) * (c * c))
        self.updates += 1
        self._last_w = {name: torch.exp(self.k * g) for name, g in self._gain.items()}
        self._acts.clear()
        self._gain.clear()
        self._pending_c = []
        return None
    ...
    def diagnostics(self):
        ...
        if self._last_gain is not None:
            out["debug/sdlr_gain_mean"] = self._last_gain.mean().item()
            out["debug/sdlr_gain_absmean"] = self._last_gain.abs().mean().item()
            out["debug/sdlr_gain_max"] = self._last_gain.max().item()
            out["debug/sdlr_gain_min"] = self._last_gain.min().item()
        alphas = [a for a in self._last_w.values() if a.numel() > 1]
        if not alphas:
            return out
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    if sdlr is not None:
                        ...
                        sdlr.prepare(mb_advantages.detach())
                        sdlr.pg_pass = True
