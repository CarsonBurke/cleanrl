# STATEDYNLR v9 UNITID — make per-perceptron productivity identifiable
# =====================================================================================
# v8 POSTADAM finished 9519.8 (9531 matched at 8M), beating the differentiated
# v4 incumbent's 9133.6 and the base's 8454.7. It also trained faster: 37.9 min
# versus v4's 50.2 min, and led v4 9519.8 to 8553.4 at 38 wall-clock minutes.
# The remaining gap is v1's uniform-railed 9762.7.
#
# DIAGNOSIS: v8's target rate std stayed ~0.28-0.31, but realized rate std was
# only 0.005 @0.5M, 0.009 @4M, and 0.013 @8M; data-alpha std ended at 0.016 and
# controller loss at 0.0053. The controller mostly predicted neutral despite a
# persistent differentiated target. Root cause: the target is per perceptron,
# but v8 gives f_phi only activation/gradient features and a SITE embedding.
# There is no unit identity with which to learn persistent productivity.
#
# ONE CHANGE: add a learned embedding for every (site, unit) pair to f_phi.
# This makes the target identifiable while retaining state features, exact
# mean-one target rates, split pre/post-Adam application, bounds, optimizer,
# warmup, and mean reversion unchanged.
#
# PASS: beat v8 at matched steps/final while rate std grows materially toward
# target std, rate mean stays ~1, and rate-effect MAE remains numerical noise.
# FAIL: two consecutive checkpoints >5% below v8, rate std remains <0.02 after
# 2M (identity did not resolve underfit), or rates rail despite mean-one targets.
# WATCH: sdlr_rate_std versus sdlr_target_rate_std, sdlr_data_alpha_std,
# sdlr_target_rate_mean, sdlr_rate_effect_mae, controller loss, return, SPS.
# =====================================================================================
#
# STATEDYNLR v8 POSTADAM — per-perceptron learning rates that survive Adam
# =====================================================================================
# v7 applied each unit's mean alpha before Adam. Its run reached only 1869 at
# 1.584M steps, while debug/sdlr_dw_corr stayed negative (-0.36 at 1.5M):
# Adam's second moment did not merely cancel the intended row-wise rate; it
# transiently inverted its relationship with the realized update norm.
#
# v8 separates the controller output alpha_ij(s) into two independently useful
# factors:
#   data_ij = alpha_ij / mean_i(alpha_ij)  (pre-Adam, column mean exactly 1)
#   rate_j = mean_i(alpha_ij)              (post-Adam, exact row step multiplier)
# data_ij retains state-dependent sample reweighting without feeding a uniform
# row scale into Adam. rate_j rescales the optimizer's applied weight-row AND
# bias update after optimizer.step(), so no downstream moment normalization can
# erase it. The gated layers belong to the shared trunk; consequently rate_j
# scales the combined, separately clipped policy+critic update, while data_ij
# still modulates only the policy backward.
#
# SUPERVISION: snapshot every gated perceptron's parameters and combined gradient
# immediately before optimizer.step(). After Adam, use the UNGATED optimizer
# delta to measure rate-independent linearized productivity:
#   e_j = -<grad_weight_j, raw_delta_weight_j> - grad_bias_j * raw_delta_bias_j.
# Center and standardize e across units within each site. Recenter after
# outlier clipping, map the zero-mean direction into bounded rate space, and
# regress onto a target whose mean rate is exactly 1 at every site. This removes
# the uniform-LR confound. Using the final gated delta would instead multiply e
# by rate_j and mechanically reward already-large rates. Output mean reversion
# makes stale hot/frozen rates recover toward 1.
#
# Bounds are [0.1, 4]. Warmup is 100 policy updates with exact rate=1.
# EVIDENCE: ppoadvnorm_batch base 8455; uniform-railed v1 9763; TDADV v2 9735.
# PASS: beat the base at matched steps with nonzero rate dispersion and exact
# post-Adam effect (debug/sdlr_rate_effect_mae near zero).
# FAIL: >5% below base at two consecutive checkpoints, rates pinned at bounds,
# non-finite controller metrics, or rate-effect error materially above roundoff.
# WATCH: debug/sdlr_rate_mean/_std, debug/sdlr_data_alpha_std,
# debug/sdlr_rate_effect_corr/_mae, debug/sdlr_progress_mean/_std,
# losses/sdlr_ctrl_loss, debug/actor_clip_frac.
# =====================================================================================
# STATEDYNLR v2 CENTERED (ppoadvnorm_batch base) — outcome evidence normalized
# ACROSS UNITS per site, killing the uniform rail that v1 converged to.
# =====================================================================================
# V1 RAIL POSTMORTEM (run 3204, still running as control): the one-step
# meta-gradient saw persistently POSITIVE evidence (mean +0.09..+0.16 all run),
# so every logit railed: alpha_mean = e^1 = 2.718 and alpha_std = 0.000 at EVERY
# checkpoint from 0.3M to 5.9M. The mechanism was inert-as-designed; the run is
# effectively "uniform 2.72x policy-gradient scale" -- and STILL beats the 8454
# base (9329 @5.7M). Same story on retstd_batch (metastep finished 8224 vs 6428
# base). Uniform-scale boost is real; state-conditioning never engaged.
#
# FIX: normalize evidence ACROSS UNITS within each site (z-score, then clamp +-3)
# instead of by global RMS. The per-backward MEAN evidence component -- the thing
# that railed v1 -- is removed BY CONSTRUCTION; only RELATIVE productivity
# between units of the same site drives logits. Zero-sum pressure means some
# units' alphas must fall for others to rise: genuine per-unit differentiation
# is now the only solution that reduces L_meta.
#
# Mechanism otherwise unchanged from v1 on this base (ppoadvnorm_batch recipe:
# norm_adv=True, norm_adv_scope="batch", ret_percnorm=False, adv_transform="v10";
# bar to beat: 8454, and v1's railed arm is itself the score to beat).
#
# PASS: > ppoadvnorm_batch reference 8454 AND > v1's railed arm.
# FAIL: alpha_std -> 0 again with alpha_mean railing (centering failed), or
#       >5% below both references at two consecutive checkpoints.
# WATCH: debug/sdlr_alpha_std (must be >> 0 now), debug/sdlr_evidence_mean
#        (~0 BY CONSTRUCTION now; watch _std instead), losses/sdlr_ctrl_loss,
#        debug/actor_clip_frac.
#
# --- v1 header (mechanism description) ---
# =====================================================================================
# BASE CHANGE vs the retstd_batch statedynlr line: the advantage scaler is now the
# ppoadvnorm_batch reference recipe -- norm_adv=True, norm_adv_scope="batch"
# (standard PPO standardization ONCE over the whole rollout), ret_percnorm=False,
# adv_transform="v10". This is the exact config of run
# HalfCheetah-v4__iterthink_v24_beta_d3bucket_mtp_ppoadvnorm_batch_v1__1__1782008060,
# whose 8454 is the ABSOLUTE bar (stronger base than retstd_batch's 6428: batch
# standardization re-centers advantages per rollout, which also changes the PG
# magnitude profile the controller's evidence signal sees).
#
# WHY THIS BASE: metastep did OK on retstd_batch; the question here is whether
# outcome-trained state-conditioned plasticity survives (and helps) on the
# stronger normalization, where the advantage is zero-mean per rollout and the
# per-unit PG statistics differ.
#
# MECHANISM (unchanged from retstd_batch statedynlr v3 metastep): hypergradient-
# ascent on phi, no regression target. Per-unit outcome evidence
#     e_j = r_j(t+1) * mm_j(t)
# where mm_j(t) = mean_i(alpha_ij * g_ij) is the MODULATED batch-mean gradient
# that entered update t (captured in the hook, pre-clip) and r_j(t+1) is the next
# backward's RAW batch-mean gradient. e_j > 0: the step proved productive =>
# bigger alpha would have helped more; e_j < 0: shrink. phi descends
#     L_meta = -mean( en_j * logit_ij(t) ),   en = clamp(e / rms(e), +-3)
# -- the dL_{t+1}/dphi ~= -<g_{t+1}, u_t> * dalpha/dphi term from the one-step
# unroll, collapsed to per-unit scalar evidence, RMS-normalized, applied to the
# CURRENT minibatch's logits (PPO epoch-reuse makes f(s_t) ~= f(s_{t+1}) where it
# matters; keeping the stale graph alive across opt.step() breaks). Inputs to
# f_phi stay purely state features (activation, sign consistency, z-scored
# engagement, site emb). alpha_ij(s) = exp(c*(2*sigmoid(logit)-1)) in [e^-c, e^c]
# multiplies the POLICY gradient elementwise during the actor backward only
# (critic backward and heads untouched). Sites: trunk.entry, ThinkBlock outputs,
# trunk.out_proj. Warmup (100 pg updates): stats only, no modulation, no meta.
#
# KNOWN RISKS: if per-unit PG directions decorrelate between minibatches, e_j is
# noise and the meta-gradient random-walks the logits (RMS normalization keeps it
# zero-mean rather than railing; v1's sign-SNR regression instead collapsed to a
# constant and railed uniformly, ~50% behind reference, cancelled at 1.15M).
# HYPOTHESIS: outcome-trained state-conditioned plasticity helps on the
# ppoadvnorm_batch base too.
# PASS: >= ppoadvnorm_batch reference (8454 end; check --at 1M/2M against its
#       trajectory) at matched steps.
# FAIL: >5% below reference at two consecutive checkpoints, or alpha rails at a
#       bound with debug/sdlr_evidence_std < 1e-3 sustained.
# WATCH: debug/sdlr_evidence_mean/_std (is the outcome signal alive?),
#        losses/sdlr_ctrl_loss (the meta loss), debug/sdlr_alpha_mean/_std,
#        debug/sdlr_alpha_site_{entry,block0..2,out_proj}, debug/actor_clip_frac.
# =====================================================================================
...
class Args:
    ...
    # --- STATEDYNLR v8: split sample weighting and post-Adam per-unit rates ---
    statedynlr: bool = True          # enable controller on the shared trunk update
    sdlr_alpha_min: float = 0.1      # lower post-Adam per-perceptron rate
    sdlr_alpha_max: float = 4.0      # upper post-Adam per-perceptron rate
    sdlr_logit_decay: float = 0.01   # mean-revert the controller output layer toward rate=1;
    #                                  hot/frozen rates must be continually re-earned
    sdlr_ema_beta: float = 0.98      # EMA decay for grad-sign consistency m and magnitude v
    sdlr_lr: float = 1e-3            # controller learning rate (own Adam, self-supervised only)
    sdlr_warmup: int = 100           # pg-updates before modulation engages (stats converge first)
...
class SDLRController(nn.Module):
    """Per-perceptron plasticity controller with split pre/post-Adam action.

    For controller output alpha_ij(s), the policy backward receives the
    per-sample factor alpha_ij / mean_i(alpha_ij), while the optimizer's applied
    update to perceptron j is multiplied after Adam by mean_i(alpha_ij).

    Supervision is the centered per-site, rate-independent Adam productivity
        e_j = -<grad_weight_j, raw_delta_weight_j> - grad_bias_j * raw_delta_bias_j.
    Every site's target rates have mean exactly one, so relative evidence cannot
    inject a uniform learning-rate change.
    """

    def __init__(self, site_modules, unit_layers, H, alpha_min=0.1, alpha_max=4.0,
                 ema_beta=0.98, lr=1e-3, warmup=100, logit_decay=0.01):
        """unit_layers maps each hooked site to the Linear producing its units."""
        super().__init__()
        self.unit_layers = dict(unit_layers)
        self.site_names = list(unit_layers.keys())
        self.H = H
        assert set(site_modules) == set(unit_layers)
        for name, layer in self.unit_layers.items():
            assert site_modules[name] is layer, f"{name} hook must be on its producing Linear"
            assert isinstance(layer, nn.Linear) and layer.out_features == H
            assert layer.bias is not None, f"{name} must expose a complete weight+bias perceptron"
        assert 0.0 <= alpha_min < 1.0 < alpha_max
        assert 0.0 <= logit_decay < 1.0
        self.a_min = alpha_min
        self.a_max = alpha_max
        self.decay = logit_decay
        self.beta = ema_beta
        self.warmup = warmup
        self.updates = 0
        site_emb_dim = 8
        unit_emb_dim = 8
        self.site_emb = nn.Embedding(len(self.site_names), site_emb_dim)
        self.unit_emb = nn.Embedding(len(self.site_names) * H, unit_emb_dim)
        self.register_buffer("site_ids", torch.arange(len(self.site_names)), persistent=False)
        self.register_buffer(
            "unit_ids",
            torch.arange(len(self.site_names) * H).view(len(self.site_names), H),
            persistent=False,
        )
        # inputs: activation | sign consistency | log magnitude | site ID | unit ID
        input_dim = 3 + site_emb_dim + unit_emb_dim
        self.net = nn.Sequential(nn.Linear(input_dim, 32), nn.ReLU(), nn.Linear(32, 1))
        # Anchor the init at EXACTLY alpha=1, through the TANH LOGIT CAP below:
        # the raw output is squashed to bounded_logit = cap*tanh(raw/cap) before
        # the sigmoid. The cap keeps sigmoid' > 0 at every operating point -- a
        # railed unit's negative evidence must still reach its parameters (v5
        # pre-fix failure: saturated sigmoid zeroed the meta-gradient and a
        # hurtful unit could never fall). alpha=1 needs bounded_logit = b0 =
        # logit((1-a_min)/(a_max-a_min)), so raw_bias = cap*atanh(b0/cap).
        self.logit_cap = 4.0
        p0 = (1.0 - self.a_min) / (self.a_max - self.a_min)
        b0 = torch.log(torch.tensor(p0)) - torch.log1p(-torch.tensor(p0))
        self.register_buffer("b0", b0.view(()))
        raw_b0 = self.logit_cap * torch.atanh(self.b0 / self.logit_cap)
        self.register_buffer("raw_b0", raw_b0.view(()))
        reachable_min = self.a_min + (self.a_max - self.a_min) * torch.sigmoid(
            -self.b0.new_tensor(self.logit_cap)
        )
        reachable_max = self.a_min + (self.a_max - self.a_min) * torch.sigmoid(
            self.b0.new_tensor(self.logit_cap)
        )
        self.target_span = min(1.0 - float(reachable_min), float(reachable_max) - 1.0)
        nn.init.zeros_(self.net[-1].weight)
        nn.init.constant_(self.net[-1].bias, float(self.raw_b0))
        ...
        self._pending = []         # [(site, raw batch-mean grad, batch mean-square, modulated batch-mean)]
        self._alphas = {}          # site -> detached (B,H) mean-1 data modulation
        self._rates = {}           # site -> detached (H,) post-Adam step rate
        self._logits = []          # [(site, logits with graph)] for THIS minibatch
        self._last = {}            # site -> raw alpha (B,H) from the last prepare
        self._last_rates = {}      # site -> post-Adam rate (H,) from the last prepare
        self._last_en = None       # last normalized progress (S,H), detached
        self._handles = []
    ...
    def _make_bwd(self, name):
        def hook(g):
            ...
            # modulated batch-mean: what actually entered this unit's param grad
            mm = gmean if a is None else (gd * a).mean(dim=0)
            self._pending.append((name, gmean, gd.pow(2).mean(dim=0), mm.detach()))
            return None if a is None else g * a
        return hook

    def prepare(self):
        """Predict this minibatch's data weights and post-Adam row rates."""
        self._alphas.clear()
        self._rates.clear()
        self._logits.clear()
        self._last.clear()
        ...
        site_embs = self.site_emb(self.site_ids)
        unit_embs = self.unit_emb(self.unit_ids)
        for idx, name in enumerate(self.site_names):
            ...
            feat_act = x / (1.0 + x.abs())
            sd = getattr(self, f"v_{name}").sqrt()
            cons = (getattr(self, f"m_{name}") / (sd + 1e-8)).clamp(-1, 1)
            log_rms = (sd + 1e-12).log()
            mu = getattr(self, f"mu_{name}")
            sig = (getattr(self, f"m2_{name}") - mu.pow(2)).clamp_min(1e-8).sqrt().clamp_min(0.25)
            mag = ((log_rms - mu) / sig).clamp(-4.0, 4.0)
            rows = torch.cat(
                [
                    feat_act.unsqueeze(-1),
                    cons.expand(B, -1).unsqueeze(-1),
                    mag.expand(B, -1).unsqueeze(-1),
                    site_embs[idx].expand(B, self.H, -1),
                    unit_embs[idx].unsqueeze(0).expand(B, -1, -1),
                ],
                dim=-1,
            )
            logits = self.logit_cap * torch.tanh(self.net(rows).squeeze(-1) / self.logit_cap)
            self._logits.append((name, logits))
            if modulate:
                alpha = self.a_min + (self.a_max - self.a_min) * torch.sigmoid(logits)
            else:
                alpha = torch.ones_like(logits)
            alpha_d = alpha.detach()
            rate = alpha_d.mean(dim=0)
            self._rates[name] = rate
            self._alphas[name] = alpha_d / rate.unsqueeze(0)
            self._last[name] = alpha_d
            self._last_rates[name] = rate

    @torch.no_grad()
    def stash(self):
        """Update features and snapshot each complete perceptron before Adam."""
        for name, gmean, gms, _mm in self._pending:
            m = getattr(self, f"m_{name}")
            setattr(self, f"m_{name}", self.beta * m + (1.0 - self.beta) * gmean)
            v_old = getattr(self, f"v_{name}")
            v_new = self.beta * v_old + (1.0 - self.beta) * gms
            setattr(self, f"v_{name}", v_new)
            log_rms = (v_new + 1e-12).sqrt().log()
            mu = getattr(self, f"mu_{name}")
            m2 = getattr(self, f"m2_{name}")
            setattr(self, f"mu_{name}", self.beta * mu + (1.0 - self.beta) * log_rms)
            setattr(self, f"m2_{name}", self.beta * m2 + (1.0 - self.beta) * log_rms.pow(2))
        self._pending.clear()
        self._snap = {}
        self.updates += 1
        for name, layer in self.unit_layers.items():
    ...
    def _mean_revert(self):
        self.net[-1].weight.mul_(1.0 - self.decay)
        self.net[-1].bias.mul_(1.0 - self.decay).add_(self.raw_b0, alpha=self.decay)

    def observe(self):
        """Apply row rates after Adam, measure ungated progress, and train the controller."""
        with torch.no_grad():
            progress, valid = [], []
            rate_values, final_norms = [], []
            effect_rates, effect_ratios = [], []
            logits_by_name = dict(self._logits)
            self._dw_corr = float("nan")
            self._rate_effect_corr = float("nan")
            self._rate_effect_mae = float("nan")
            self._last_en = None
            self._target_rate_mean = float("nan")
            self._target_rate_std = float("nan")
            for name, (w_before, b_before, grad_w, grad_b) in getattr(self, "_snap", {}).items():
                layer = self.unit_layers[name]
                assert layer.bias is not None
                raw_dw = layer.weight.detach() - w_before
                raw_db = layer.bias.detach() - b_before
                rate = self._rates.get(name)
                if rate is not None:
                    layer.weight.copy_(w_before + rate.unsqueeze(1) * raw_dw)
                    layer.bias.copy_(b_before + rate * raw_db)
                final_dw = layer.weight.detach() - w_before
                final_db = layer.bias.detach() - b_before

                e = -(grad_w * raw_dw).sum(dim=1) - grad_b * raw_db
                centered = e - e.mean()
                sd = centered.std(unbiased=False)
                if (
                    name in logits_by_name
                    and torch.isfinite(centered).all()
                    and torch.isfinite(sd)
                    and sd >= 1e-12
                ):
                    normalized = (centered / sd).clamp(-3.0, 3.0)
                    progress.append(normalized - normalized.mean())
                    valid.append(name)

                if rate is not None:
                    raw_norm_sq = raw_dw.pow(2).sum(dim=1) + raw_db.pow(2)
                    final_norm_sq = final_dw.pow(2).sum(dim=1) + final_db.pow(2)
                    raw_norm = raw_norm_sq.sqrt()
                    final_norm = final_norm_sq.sqrt()
                    rate_values.append(rate)
                    final_norms.append(final_norm)
                    nz = raw_norm > 1e-12
                    effect_rates.append(rate[nz])
                    effect_ratios.append(final_norm[nz] / raw_norm[nz])

            if rate_values:
                rate_vec = torch.cat(rate_values)
                final_norm_vec = torch.cat(final_norms)
                if rate_vec.std() > 1e-9 and final_norm_vec.std() > 1e-12:
                    self._dw_corr = float(torch.corrcoef(torch.stack((rate_vec, final_norm_vec)))[0, 1])
            if effect_rates and any(x.numel() for x in effect_rates):
                expected = torch.cat(effect_rates)
                observed = torch.cat(effect_ratios)
                self._rate_effect_mae = float((expected - observed).abs().mean())
                if expected.numel() > 1 and expected.std() > 1e-9 and observed.std() > 1e-9:
                    self._rate_effect_corr = float(torch.corrcoef(torch.stack((expected, observed)))[0, 1])

            if not progress or self.updates < self.warmup:
                self._snap = {}
                if self.updates >= self.warmup:
                    self._mean_revert()
                return None
            en_all = torch.stack(progress)
            direction = en_all / en_all.abs().amax(dim=1, keepdim=True).clamp_min(1.0)
            target_rate = 1.0 + self.target_span * direction
            target = (target_rate - self.a_min) / (self.a_max - self.a_min)
            self._target_rate_mean = float(target_rate.mean())
            self._target_rate_std = float(target_rate.std())
            self._last_en = en_all.detach()
            self._snap = {}

        pred = torch.stack([logits_by_name[name] for name in valid])
        meta_loss = F.mse_loss(torch.sigmoid(pred), target.unsqueeze(1).expand_as(pred))
    ...
    def diagnostics(self):
        out = {
            "debug/sdlr_progress_mean": (
                self._last_en.mean().item() if self._last_en is not None else float("nan")
            ),
            "debug/sdlr_progress_std": (
                self._last_en.std().item() if self._last_en is not None else float("nan")
            ),
            "debug/sdlr_target_rate_mean": float(getattr(self, "_target_rate_mean", float("nan"))),
            "debug/sdlr_target_rate_std": float(getattr(self, "_target_rate_std", float("nan"))),
            "debug/sdlr_dw_corr": float(getattr(self, "_dw_corr", float("nan"))),
            "debug/sdlr_rate_effect_corr": float(getattr(self, "_rate_effect_corr", float("nan"))),
            "debug/sdlr_rate_effect_mae": float(getattr(self, "_rate_effect_mae", float("nan"))),
        }
        alphas = [a for a in self._last.values() if a.numel() > 1]
        rates = [r for r in self._last_rates.values() if r.numel() > 1]
        if not alphas or not rates:
            return out
        all_alpha = torch.cat([a.reshape(-1) for a in alphas])
        all_rate = torch.cat(rates)
        all_data = torch.cat([(a / a.mean(dim=0, keepdim=True)).reshape(-1) for a in alphas])
        out["debug/sdlr_alpha_mean"] = all_alpha.mean().item()
        out["debug/sdlr_alpha_std"] = all_alpha.std().item()
        out["debug/sdlr_rate_mean"] = all_rate.mean().item()
        out["debug/sdlr_rate_std"] = all_rate.std().item()
        out["debug/sdlr_data_alpha_std"] = all_data.std().item()
        for name in self.site_names:
            rate = self._last_rates.get(name)
            if rate is not None and rate.numel() > 1:
                out[f"debug/sdlr_rate_site_{name}"] = rate.mean().item()
        return out
...
if __name__ == "__main__":
    ...
    source_paths = [Path(__file__).resolve(), repo_root / "cleanrl/shared/hl_gauss.py"]
    manifest = {
        "argv": sys.argv,
        "args": vars(args),
        "versions": {
    ...
    (run_dir / "manifest.json").write_text(manifest_json + "\n")
    print("RUN_MANIFEST:", json.dumps(manifest, sort_keys=True))
    writer = SummaryWriter(run_dir)
    writer.add_text("run_manifest", f"```json\n{manifest_json}\n```")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s"
    ...
    # STATEDYNLR: per-perceptron sample weighting on the policy backward plus an
    # exact post-Adam row rate on each producing Linear in the shared trunk.
    sdlr = None
    if args.statedynlr:
        assert args.share_backbone and args.separate_grad_clip, \
            "statedynlr requires the shared trunk and separate gradient clipping"
        site_modules = {"entry": agent.trunk.entry}
        unit_layers = {"entry": agent.trunk.entry}
        for k, blk in enumerate(agent.trunk.blocks):
            site_modules[f"block{k}"] = blk.in_proj
            unit_layers[f"block{k}"] = blk.in_proj
        site_modules["out_proj"] = agent.trunk.out_proj
        unit_layers["out_proj"] = agent.trunk.out_proj
        # Construct on CPU without shifting the seeded rollout stream.
        _rng_state = torch.get_rng_state()
        sdlr = SDLRController(
            site_modules,
            unit_layers,
            args.hidden,
            alpha_min=args.sdlr_alpha_min,
            alpha_max=args.sdlr_alpha_max,
            logit_decay=args.sdlr_logit_decay,
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    # Probe whether the actor budget binds after the mean-one
                    # per-sample data weighting.
                    ...
                    if sdlr is not None:
                        # Snapshot complete controlled perceptrons and their
                        # combined clipped gradients before Adam.
                        sdlr.stash()
                    optimizer.step()
                    if sdlr is not None:
                        # Apply the predicted row rates to Adam's weight+bias
                        # deltas, then train on rate-independent raw-Adam
                        # productivity for this same step.
                        sdlr_ctrl_loss = sdlr.observe()
                else:
