# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + DIFFUSIONBLOCKS VALUE v1.
# =====================================================================================
...
#   B blocks f_b(z, cond, t_emb): u = in_proj([RMSNorm(z), RMSNorm(cond)]),
#            u = u*(1+scale(t)) + shift(t),  F = out_proj(ReLU(u)^2)
...
#   blocks:              3      6     12     24
#   e2e        peak MiB  222    274    379    592     step ms  17.6  11.8  15.9  21.6
#   dblock-all peak MiB  304    480    829   1531     step ms  10.0   8.9   8.7  12.2
#   dblock-one peak MiB  188    189    192    199     step ms   8.7   9.4   8.6   9.7
# dblock-one (--no-dblock-train-all-blocks, the paper's mode) is FLAT in depth: 24 blocks
# cost 3% more memory than 3, against 2.7x for end-to-end, and it is 2.2x faster per step at
# depth 24. dblock-all keeps B graphs alive so it trades that memory back for a full update
# of every block per minibatch, and is still faster than e2e (no depth-serial backward).
# Default is dblock-all at B=3: at this scale samples, not bytes, are the binding constraint.
#
...
class Args:
    ...
    dblock: bool = True
    dblock_num_blocks: int = 3        # depth of the value stack; block 0 owns the LARGEST sigmas
    dblock_mult: int = 2              # block MLP width = mult * hidden
...
class BlockStack(nn.Module):
    ...
    attention mixer over 6 independent value distributions would buy nothing.
        ...
        u = in_proj([RMSNorm(z), RMSNorm(cond)])          # z: diffusion state, cond: obs code
        u = u * (1 + scale(t)) + shift(t)                 # adaLN noise conditioning, ZERO-init
        F = out_proj(ReLU(u)^2)                           # out_proj ZERO-init  =>  F == 0 at init

    Every parameter carries a leading (num_blocks,) axis and every op is an einsum over it,
    so TRAINING ALL BLOCKS IS ONE BATCHED CALL. That batching is only legal because the
    blocks never compose (block b's loss never flows through block b'): it is the same
    property that removes the depth-serial backward, expressed in the forward. A residual
    stack forbids it. Inference indexes one block per Euler step (params[ids]).
    """
    ...
    def __init__(self, num_blocks, H, mult):
        ...
        # scales bounded before the projection.
...
class ValueNet(nn.Module):
    ...
    def __init__(self, H, num_bins, mtp, coord_support, coord_max, args):
        ...
        self.blocks = BlockStack(self.num_blocks, H, args.dblock_mult)
