# STATEDYNLR V6 TDGAIN on the PPOADVNORM(batch) base — CLOSED-FORM per-(sample, unit)
# TD-gain modulation. NO learned controller.
# =====================================================================================
# LINE POSTMORTEM (why v6 abandons learned supervision entirely):
#   v1 (metastep): per-unit evidence e_j = gmean(t+1)*mm(t) is sample-independent =>
#     the optimal f_phi output is a per-unit CONSTANT; run converged exactly there
#     (alpha_std=0, all sites railed at e), and actor_clip_frac=1.0 made the uniform
#     rail an exact no-op. Its +20% lead over reference was early-transient/drift.
#   v2 (tdadv): per-(sample,unit) regression onto clamp(Â_i*g_ij/RMS_j): targets
#     zero-mean (no rail -- worked as designed) but pred_corr ~0.001: given unit
#     activations alone, advantage-gradient alignment is UNPREDICTABLE noise.
#   v3 (actcond): + action/logp/entropy inputs: pred_corr ~0.017, alpha_std ~0.05 --
#     still nothing. Learned outcome supervision of micro-plasticity is dead on this
#     base at predictor scale <= 32 hidden.
#   ALSO: v1-control / v2 / reference are mechanically IDENTICAL post-warmup yet span
#     8242-9704 final (+-18%): the single-seed noise floor. Only an intervention that
#     ENGAGES from step 0 and moves matched-step returns >20% is measurable.
#
# V6 MECHANISM: replace prediction with measurement.
#     w_ij = exp( k * tanh(adv_i) * cons_j )
#   adv_i = minibatch batch-standardized GAE advantage (the base's TD(lambda) signal).
#   cons_j = clamp(m_j/sqrt(v_j), -1, 1): unit j's running EMA sign-consistency
#     between its RAW incoming policy gradient and the advantage direction (updated in
#     ingest from hook-captured grads; beta=0.98). Positive-advantage samples amplify
#     advantage-aligned units and damp noisy ones; negative-advantage samples do the
#     symmetric sharpening of suppression. Per-(sample,unit) granularity; identity at
#     init (cons=0 => w=1); active from update 0; k=1 => w in [e^-1, e].
#   Applied elementwise to the POLICY gradient during the actor backward only (critic
#   backward and heads untouched); stats captured pre-modulation/pre-clip. Sites:
#   trunk.entry, ThinkBlock outputs, trunk.out_proj.
#
# HYPOTHESIS: SNR-weighted allocation of policy-gradient magnitude across trunk units
# improves learning enough to clear the noise floor. If NOT, the entire per-unit PG
# reweighting class is falsified on this base -- stop the line either way.
# PASS: >= +20% over the reference trajectory at matched steps (5015/6634/7602 @2/4/6M,
#       8242 end) -- anything less is unresolvable noise.
# FAIL: < reference trajectory at two consecutive checkpoints, or cons_absmean ~ 0
#       sustained (no consistent units exist => mechanism has no handle).
# WATCH: debug/sdlr_cons_mean/_absmean (is there signal to gate on?),
#        debug/sdlr_alpha_mean/_std (+ per-site), debug/actor_clip_frac.
# =====================================================================================
...
class Args:
    ...
    # --- STATEDYNLR v6 TDGAIN: CLOSED-FORM per-(sample, unit) modulation
    #     w_ij = exp(k * tanh(adv_i) * cons_j). No learned controller (see header) ---
    statedynlr: bool = True          # modulate the POLICY gradient per (sample, trunk unit)
    sdlr_k: float = 1.0              # gain: w in [exp(-k), exp(+k)]
    sdlr_ema_beta: float = 0.98      # EMA decay for per-unit sign-consistency m/sqrt(v)
    sdlr_lr: float = 1e-3            # UNUSED in v6 (kept for CLI compatibility; no optimizer)
    sdlr_warmup: int = 0             # UNUSED in v6: identity at init (cons starts at 0), no dead phase
...
class SDLRController(nn.Module):
    """CLOSED-FORM per-(sample, unit) TD-gain modulation (v6 TDGAIN). NO learned
    controller, NO regression, NO optimizer: nothing to fail to learn, nothing to rail.

    Why: three learned variants (per-unit hypergradient -> constant rail; per-sample
    Â*g regression from activations -> pred_corr 0.001; + action/logp/entropy ->
    0.017) show gradient-alignment outcomes are UNPREDICTABLE from any input set this
    small. v6 replaces prediction with measurement:
        w_ij = exp(k * tanh(adv_i) * cons_j)
    where adv_i is the minibatch's batch-standardized GAE advantage (the TD(lambda)
    signal) and cons_j = clamp(m_j/sqrt(v_j), -1, 1) is unit j's running
    sign-consistency between its raw incoming policy gradient and the advantage
    direction (EMAs updated in ingest). Positive-advantage sample: amplify
    advantage-aligned units, damp noisy ones; negative-advantage: symmetric (sharpens
    suppression). Identity at init (cons=0 => w=1), engaged from update 0.

    The hook applies w elementwise to the POLICY gradient in the actor backward only;
    stats are captured BEFORE the site's own scaling. Sites: trunk.entry, ThinkBlock
    outputs, trunk.out_proj. Falsification: if per-unit PG reweighting cannot beat
    the +-18% single-seed noise floor as a closed-form rule, the hypothesis class is
    dead on this base.
    """
    ...
    def __init__(self, site_modules, H, k=1.0, ema_beta=0.98):
        ...
        for name in self.site_names:
            self.register_buffer(f"m_{name}", torch.zeros(H))   # signed batch-mean grad EMA
        ...
        self._adv = None           # this minibatch's standardized advantage (set in prepare)
        self._last_w = {}          # site -> weights of the LAST pass (for diagnostics)
        self._last_cons = None     # last flattened per-unit consistency (for diagnostics)
        self._handles = []
    ...
    def _make_bwd(self, name):
        def hook(g):
            ...
            cons = (getattr(self, f"m_{name}") / (sd + 1e-8)).clamp(-1.0, 1.0)   # (H,)
            w = torch.exp(self.k * torch.tanh(self._adv).unsqueeze(1) * cons.unsqueeze(0))  # (B, H)
            self._last_w[name] = w
            return g * w
        return hook

    def prepare(self, adv):
        """Store this minibatch's standardized advantage; weights are computed in the
        backward hooks (cons is current, adv is fixed for the pass)."""
        self._adv = adv.detach()
        cons_all = []
        for name in self.site_names:
            sd = getattr(self, f"v_{name}").sqrt()
            cons_all.append((getattr(self, f"m_{name}") / (sd + 1e-8)).clamp(-1.0, 1.0))
        self._last_cons = torch.cat(cons_all)

    def ingest(self, adv=None):
        """After optimizer.step(): fold this backward's grad stats into the per-unit
        sign-consistency EMAs. Returns None (no control loss by design)."""
        for name in self.site_names:
            gd = self._grads.get(name)
            if gd is None:
                continue
            m = getattr(self, f"m_{name}")
            setattr(self, f"m_{name}", self.beta * m + (1.0 - self.beta) * gd.mean(dim=0))
            v = getattr(self, f"v_{name}")
            setattr(self, f"v_{name}", self.beta * v + (1.0 - self.beta) * gd.pow(2).mean(dim=0))
        self.updates += 1
        self._grads.clear()
        self._adv = None
        return None
    ...
    def diagnostics(self):
        out = {}
        if self._last_cons is not None:
            out["debug/sdlr_cons_mean"] = self._last_cons.mean().item()
            out["debug/sdlr_cons_absmean"] = self._last_cons.abs().mean().item()
