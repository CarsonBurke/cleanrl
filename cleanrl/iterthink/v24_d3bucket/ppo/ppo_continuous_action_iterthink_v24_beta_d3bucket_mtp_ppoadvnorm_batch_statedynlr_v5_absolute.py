# STATEDYNLR v5 ABSOLUTE (ppoadvnorm_batch base) — per-perceptron, NON-COMPETITIVE
# outcome accountability.
# =====================================================================================
# DESIGN: each hidden unit's alpha is judged ONLY on its own realized outcome
# evidence e_j = r_j(t+1) * mm_j(t) (next backward's raw batch-mean grad times
# this step's modulated batch-mean), RMS-normalized, clamped +-3. NO per-site
# mean subtraction: units do NOT fight for a relative budget. A unit earns a
# bigger or smaller alpha independently of its neighbours; simultaneous
# productivity legitimately raises several units together.
#
# WHY: v2_centered (zero-sum z-scored evidence) had the best TRAJECTORY at
# matched steps (5431 @1.6M vs ~4900 for everything else) but was cancelled
# early; finished runs ranked railed-v1 9899 > v4-unratchet (competitive +
# decay) 9337 > base 8454. The zero-sum constraint keeps only relative
# differences and discards each unit's absolute productivity -- the actual
# "this unit deserves to learn faster right now" signal.
#
# RAIL PROTECTION (v1's failure: all logits pinned at e^c for 8M steps) now
# comes from v4's logit mean-reversion instead of centering: bounded en (+-3)
# pushing up vs decay pulling the output layer toward the neutral anchor every
# update => logits settle where meta-push == decay-pull, per unit, at
# evidence-justified levels rather than at a shared bound.
#
# Otherwise identical to v4: alpha in [0.05, 8] linear-in-sigmoid, init alpha=1
# exact, warmup 100 pg updates, one-step outcome meta-gradient applied to the
# current minibatch's logits, policy-gradient-only modulation at 5 trunk sites.
#
# PASS: > railed v1 (9899) -- the score to beat.
# FAIL: alpha rails to a bound with evidence_mean pinned away from 0 (decay
#       failed to hold equilibrium), or >5% below v1 at two checkpoints.
# WATCH: debug/sdlr_alpha_mean/_std, debug/sdlr_alpha_site_*, 
#        debug/sdlr_evidence_mean/_std, losses/sdlr_ctrl_loss,
...
class SDLRController(nn.Module):
    ...
    v4 UNRATCHET: the controller output mean-reverts toward the neutral anchor
    each update (logit_decay), so freeze/hot gates must be CONTINUALLY re-earned
    from evidence -- a frozen unit's logit creeps back into contention instead of
    the whole net strangling itself shut (v3 postmortem).
    """

    def __init__(self, site_modules, H, alpha_min=0.05, alpha_max=8.0,
        ...
        self._prev_mm = None       # site -> previous step's modulated batch-mean (H,)
    ...
    def ingest(self):
        """After optimizer.step(): (1) fold grad stats into the feature EMAs;
        (2) build per-unit outcome evidence e_j = r_j(now) * mm_j(prev step),
        RMS-normalize per site, clamp to +-3; (3) descend
            L_meta = -mean( en_j * logit_ij(prev step) )
        so productive steps raise their logits and harmful steps lower them;
        (4) stash this step's modulated means and logits for the next round.
        Returns the meta loss (or None)."""
        meta_loss = None
        cur = {}
        for name, gmean, gms, mm in self._pending:
            cur[name] = (gmean, mm)
        ...
        if self._prev_mm is not None and self._logits and self.updates >= self.warmup:
            # The evidence describes step t's modulation; we apply its gradient to
            # the CURRENT logits. Rationale: keeping step t's graph alive across
            # opt.step() breaks (params modified in-place), and in PPO the same
            # states recur across epochs, so f(s_t) ~= f(s_{t+1}) where it matters.
            terms, evids = [], []
            for name, logits in self._logits:
                prev_mm = self._prev_mm.get(name)
                if prev_mm is None or name not in cur:
                    continue
                e = cur[name][0] * prev_mm                 # outcome evidence (H,)
                # v5 ABSOLUTE: PER-PERCEPTRION, NON-COMPETITIVE accountability.
                # Each unit is judged ONLY on its own realized outcome e_j =
                # r_j(t+1) * mm_j(t), RMS-normalized and clamped. No mean
                # subtraction: v2_centered's z-score made the site zero-sum
                # (units FIGHT for a fixed relative budget) -- and although
                # centered had the best trajectory at matched steps, the zero-sum
                # constraint discards each unit's absolute productivity, which is
                # exactly the signal "this unit deserves to learn faster/slower
                # right now". Here a unit earns alpha up or down INDEPENDENTLY of
                # its neighbours; if several units are productive simultaneously
                # they all legitimately rise together -- that is per-unit
                # accounting, not a global LR knob.
                # Rail protection comes from the v4 logit mean-reversion (below),
                # not from centering: with bounded en (+-3) and decay pulling the
                # output layer toward the neutral anchor every update, logits sit
                # at an equilibrium where meta-push == decay-pull instead of
                # pinning at the bound like v1.
                en = (e / (e.pow(2).mean().sqrt() + 1e-8)).clamp(-3.0, 3.0)
                evids.append(en.detach())
                terms.append(-(en.unsqueeze(0) * logits).mean())
            if terms:
                meta_loss = torch.stack(terms).mean()
                self.opt.zero_grad(set_to_none=True)
                meta_loss.backward()
                self.opt.step()
                self._last_en = torch.stack(evids).detach()
                # v4 UNRATCHET: mean-revert the output layer toward the neutral
                # anchor. Gates must be continually re-earned from fresh evidence;
                # without this, one bad outcome epoch permanently freezes a unit
                # (its mm ~ 0 starves its own future evidence) and the net slowly
                # strangles itself shut -- exactly the v3 failure at 0.8M steps.
                with torch.no_grad():
                    lam = self.decay
                    self.net[-1].weight.mul_(1.0 - lam)
                    self.net[-1].bias.mul_(1.0 - lam).add_(self.b0, alpha=lam)
        # carry this step's state into the next one-step unroll
        self._prev_mm = {name: mm for name, (_, mm) in cur.items()}
        self.updates += 1
        self._last = dict(self._alphas)
        self._pending.clear()
        self._acts.clear()
        self._alphas.clear()
        self._logits = []          # rebind, NOT clear: prev_logits holds the tensors
        return meta_loss

    def diagnostics(self):
        out = {}
        if self._last_en is not None:
            out["debug/sdlr_evidence_mean"] = self._last_en.mean().item()
            out["debug/sdlr_evidence_std"] = self._last_en.std().item()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.step()
                    if sdlr is not None:
                        # Fold raw-grad stats into the EMAs and regress the
                        # predictions onto the realized post-update gradient SNR.
                        sdlr_ctrl_loss = sdlr.ingest()
                else:
