# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT + DG v4.
# =====================================================================================
# V4 DELTA (vs nextlat_v1): DELIGHT-GATE (DG) advantage weighting -- asymmetric,
# action-surprise gating that replaces the symmetric state-uncertainty weighting
# v3 tried. Reading of the DG plots: effective coefficient omega(U, p) with
# U = advantage, p = policy probability of the taken action:
#   U >= 0: full weight when rare (p->0), ~half when common (p->1) -- reinforce
#           surprising successes at full strength, attenuate expected ones;
#   U <  0: ~zero when rare, ~half when common -- NEVER punish failed
#           exploration, only mildly suppress known-bad actions.
# Port: rareness q in (0,1] (1 = rarest) is the batch RANK of the behavior
# log-density (rank => scale- and Jacobian-proof, exact from step 0 -- unlike a
# learned sigma, surprise needs no calibration):
#   w = pos_common + (pos_rare - pos_common)*q   if A >= 0
#   w = neg_common * (1 - q)                      if A <  0
# defaults (pos_common=0.5, pos_rare=1.0, neg_common=0.5) reproduce both plots.
# Applied to the policy GAE BEFORE shaping/normalization; batch norm_adv then
# restores global scale (relative shaping preserved). `--no-dg` recovers the
# clip025 configuration (this file defaults nextlat_grad_clip=0.25).
# WHY NOT v3's weighting: v3 keyed on critic sigma(s) -- (1) symmetric, so it
# suppressed rare-good DELIGHTS in uncertain states, the exact exploration
# successes that must be reinforced; (2) state-aleatoric, not action-surprise --
# high-sigma states are often the frontier, and suppressing them suppresses
# exploration itself; (3) uncalibrated early (sigma/retstd ~12-30), so trust
# concentrated on noise and the run stalled. DG inverts all three: keep every
# delight, punish only the familiar failures, gate on an exact quantity.
# HYPOTHESIS: the PG variance that kills exploration lives in 1/p rare-action
# terms on the NEGATIVE side; gating those while keeping rare positives intact
# stabilizes without dulling discovery. Falsifiable vs nextlat_v1_clip025
# (@4M 8535): if v4 <= it, the asymmetry costs more signal than it saves.
# Watch: debug/dg_w_mean, debug/dg_w_std, debug/dg_pos_mass_frac.
# =====================================================================================
#
# --- inherited nextlat_v1 header (unchanged below) ---
...
class Args:
    ...
    # Delight-gate advantage weighting (v4, the SOLE delta): sign-asymmetric
    # rareness gating of the policy GAE before shaping/norm. q = batch rank of
    # behavior log-density (1 = rarest). w_pos = pos_common + (pos_rare -
    # pos_common)*q; w_neg = neg_common*(1-q). Defaults reproduce the DG plots.
    dg: bool = True
    ...
    nextlat_grad_clip: float = 0.25  # own dual-backward budget (clip025 run value; trunk total 0.25+0.25+0.25)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # V4 DELIGHT-GATE (sole delta): sign-asymmetric rareness gating of the
            # policy GAE BEFORE shaping/normalization. q = batch rank of the
            # BEHAVIOR log-density (1 = rarest sample in the rollout; rank makes
            # it scale- and Jacobian-proof, exact from step 0). Positives keep
            # full weight when rare and half when common; negatives are zeroed
            # when rare and halved when common. Sign is split on the GAE itself
            # (all scalings so far are divide-only, so this is the raw sign).
            # Batch norm_adv afterwards restores global scale.
            if args.dg:
                flat_lp = logprobs.reshape(-1)
                n_dg = flat_lp.numel()
                r_dg = flat_lp.argsort().argsort().to(torch.float32)  # 0 = rarest
                q_dg = (1.0 - r_dg / max(n_dg - 1, 1)).reshape_as(policy_adv)
                pos_w = args.dg_pos_common + (args.dg_pos_rare - args.dg_pos_common) * q_dg
                neg_w = args.dg_neg_common * (1.0 - q_dg)
                dg_w = torch.where(policy_adv >= 0, pos_w, neg_w)
                policy_adv = policy_adv * dg_w
            else:
                dg_w = torch.ones_like(policy_adv)
            dg_w_mean = dg_w.mean().item()
            dg_w_std = dg_w.std().item()
            dg_pos_mass_frac = (
                (dg_w * (policy_adv > 0).to(dg_w.dtype)).sum() / dg_w.sum().clamp_min(1e-12)
            ).item()
        ...
        writer.add_scalar("debug/dg_w_mean", dg_w_mean, global_step)
        writer.add_scalar("debug/dg_w_std", dg_w_std, global_step)
        writer.add_scalar("debug/dg_pos_mass_frac", dg_pos_mass_frac, global_step)
