# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT v9_klleash.
# =====================================================================================
# KLLEASH: v7_std plus epochs_run observability and an optional redundant
# epoch-level KL break. Correction of an earlier misreading: the inherited
# target_kl break (line ~1287) sits at EPOCH-loop level (after the minibatch
# loop), so it DOES bind at num_minibatches=1 -- the e50 run already tested
# "50 epochs or until KL 0.03". --kl-epoch-break re-checks the same condition
# (no-op when the first break fires; backstop otherwise). Default off = v7-exact.
# The real addition is debug/epochs_run: how many epochs actually ran per iter.
# First use: full-batch 10 epochs at 10x LR (3e-3) with the leash on, testing
# whether clean full-batch gradients + big steps recover minibatch progress.
# =====================================================================================
...
class Args:
    ...
    kl_epoch_break: bool = False    # v9: enforce the leash at epoch level (the only
                                    # level that binds at num_minibatches=1); off = v7-exact
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            if args.target_kl is not None and approx_kl > args.target_kl:
                break
            # v9: epoch-level leash (binds at num_minibatches=1, where the
            # minibatch-level break above is a single-chunk no-op).
            if args.kl_epoch_break and args.target_kl is not None and approx_kl > args.target_kl:
                break
        ...
        if args.nextlat:
            log_tensors["nextlat_pred_loss"] = nextlat_pred_loss
            log_tensors["nextlat_kl_loss"] = nextlat_kl_loss
        ...
        if args.nextlat:
            writer.add_scalar("losses/nextlat_pred_loss", log_vals["nextlat_pred_loss"], global_step)
            writer.add_scalar("losses/nextlat_kl_loss", log_vals["nextlat_kl_loss"], global_step)
