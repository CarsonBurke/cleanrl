...
class Args:
    ...
    sdlr_alpha_min: float = 0.01     # freeze bound: alpha can gate a unit to ~0 (no learning from
    #                                  bad/noisy data). Tiny floor keeps evidence measurable so a
    #                                  frozen unit can recover if its outcomes turn productive.
...
class SDLRController(nn.Module):
    ...
    bounded by alpha in [a_min, a_max].
    """

    def __init__(self, site_modules, H, alpha_min=0.01, alpha_max=8.0,
        ...
        nn.init.constant_(self.net[-1].bias, float(b0))
