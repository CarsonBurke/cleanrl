# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT-JEPA v1.
# =====================================================================================
# NEXTLAT-JEPA: nextlat_v1's multi-step action-conditioned latent prediction, but with
# the target side ATTACHED (JEPA-style joint embedding, both encoder sides in the
# graph) instead of stop-gradded. Rationale: JEPA/BYOL need EMA/sg targets because
# nothing else grounds the representation; HERE the shared trunk is grounded by the
# policy-gradient and critic losses, so the aux may pull BOTH ends together — the
# prediction toward the future latent AND the future latent toward predictability-
# from-the-past (a direct temporal-coherence pressure the sg version lacks).
# TWO diffs vs nextlat_v1 (everything else identical: depth-4 rollout, action
# conditioning, KL distillation with DETACHED teacher, masks, 0.1 aux clip budget):
#   (1) ATTACHED TARGETS: tgt latents computed in-graph; nextlat_target_grad in [0,1]
#       scales the target-side gradient (1.0 = full symmetry; 0.2 = Dreamer-style
#       KL-balancing asymmetry; 0.0 = nextlat_v1's sg semantics). Default 1.0 — the
#       "grounded, so attach both sides" bet.
#   (2) LAYERNORM LOSS SPACE (forced by (1)): Smooth L1 is computed between
#       LayerNormed latents on BOTH sides. With attached raw targets the aux has a
#       scale cheat — uniformly shrinking latent norms shrinks the loss to zero while
#       the heads rescale to compensate, and the PG/critic grounding does NOT penalize
#       it. LN removes the scale degree of freedom (the sg version didn't need this:
#       a fixed-scale target anchors the prediction side).
# Residual risk the grounding does NOT cover: premature compression — latent dims
# irrelevant to the CURRENT policy/critic get a live gradient toward smoothness. If
# debug/latent_batch_std (raw-latent cross-batch std) decays, lower
# --nextlat-target-grad toward 0.2.
# HYPOTHESIS: attached targets accelerate representation alignment vs nextlat_v1's
# one-sided prediction, at matched gradient budget.
...
# --compile wraps the four hot forward paths (rollout policy, update policy, flat value
# bootstrap, aux target trunk) in torch.compile(dynamic=False) with
# cudagraph_mark_step_begin discipline at each call site (reduce-overhead default).
# =====================================================================================
#
...
class Args:
    ...
    # JEPA attachment: scale of the TARGET-side gradient in the prediction loss.
    # 1.0 = fully attached (symmetric joint embedding), 0.2 = Dreamer-KL-balancing-style
    # asymmetry, 0.0 = nextlat_v1's stop-grad semantics. Forward value is unchanged.
    nextlat_target_grad: float = 1.0
...
if __name__ == "__main__":
    ...
    # flat value bootstrap (batch rows), aux target trunk (minibatch rows). Call sites
    # use torch.compiler.cudagraph_mark_step_begin() (reduce-overhead => CUDA graphs).
    ...
    if args.compile:
        ...
        actor_feat_fn = torch.compile(actor_feat_fn, mode=args.compile_mode, dynamic=False)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # NEXTLAT-JEPA: roll the latent dynamics MLP to depth d from the CURRENT
                # trunk latent, conditioned on the buffered actions. The future trunk
                # latent is computed IN-GRAPH (attached target); Smooth L1 is taken in
                # LayerNorm space on both sides (kills the scale cheat — see header),
                # with the target-side gradient scaled by nextlat_target_grad. The KL
                # teacher stays DETACHED (distillation is one-way into the student).
                # T-major flattening => index + i*num_envs is (t+i, same env); masks
                # exclude boundary-crossing and rollout-tail rows.
                if args.nextlat:
                    h_hat = mb_actor_feat
                    ...
                    for i in range(1, args.nextlat_depth + 1):
                        ...
                        tgt_feat = actor_feat_fn(b_obs[tgt_idx])        # ATTACHED (JEPA)
                        with torch.no_grad():
                            t_dist, _, _ = agent._actor_dist(tgt_feat)  # teacher (sg)
                        mask_i = b_nextlat_mask[mb_inds, i - 1]
                        denom = mask_i.sum().clamp_min(1.0)
                        tgt_n = F.layer_norm(tgt_feat, (args.hidden,))
                        w = args.nextlat_target_grad
                        if w == 0.0:
                            tgt_n = tgt_n.detach()  # true sg semantics: drop the target graph
                        elif w < 1.0:
                            tgt_n = w * tgt_n + (1.0 - w) * tgt_n.detach()  # scale target-side grad
                        pred_l = F.smooth_l1_loss(
                            F.layer_norm(h_hat, (args.hidden,)), tgt_n, reduction="none"
                        ).mean(-1)
