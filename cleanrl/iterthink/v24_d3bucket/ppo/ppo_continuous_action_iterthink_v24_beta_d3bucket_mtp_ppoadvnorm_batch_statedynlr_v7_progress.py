# STATEDYNLR v7 PROGRESS (ppoadvnorm_batch base) — per-perceptron learning rate
# trained on PER-STEP realized progress, Adam-aware.
# =====================================================================================
# ADAM INTERACTION (resolved): a quasi-static per-parameter gradient rescale
# cancels inside Adam (m and sqrt(v) scale identically). Two consequences,
# both handled by construction:
#   1. alpha_ij(s,t) varies per SAMPLE and per STEP; only that variation
#      survives Adam -- which is exactly the part this method uses. The target
#      refreshes EVERY optimizer step, so the trained alpha can never settle
#      into the quasi-static regime Adam would nullify.
#   2. Penetration is MEASURED, not assumed: debug/sdlr_dw_corr logs the
#      per-unit correlation between mean alpha_j and |dW_j| (the realized
#      update norm). If cancellation ever eats the signal, this goes to ~0.
#
# EVIDENCE: next-step granularity with exact credit. Right before each
# optimizer step we snapshot every unit's parameter row and gradient row;
# right after it we compute
#     e_j = -<grad_row_j, dW_row_j>
# where dW is the ACTUAL applied update (Adam moments + decoupled clip +
# everything). e_j is the linearized loss decrease attributed to unit j's
# weight movement THIS step. Normalized per site (z-score, clamp +-3), mapped
# through sigmoid to a target in (0,1); f_phi's current-logit predictions are
# regressed onto it (MSE). Prediction made at t is scored at t.
#
# Otherwise: bounded alpha in [0.1, 4] linear-in-sigmoid via tanh logit cap
# (sigmoid never saturates => meta-gradient alive everywhere), init alpha=1
# exact, warmup 100 pg updates, policy-gradient-only modulation at the 5 trunk
# sites (entry, block0-2 in_proj rows, out_proj rows).
#
# PASS: > railed v1 control (9899) -- the score to beat; base = 8454.
# FAIL: >5% below base at two consecutive checkpoints, or sdlr_dw_corr ~ 0
#       sustained (mechanism not penetrating Adam), or alpha pinned at bounds.
# WATCH: debug/sdlr_dw_corr, debug/sdlr_progress_mean/_std,
#        debug/sdlr_alpha_mean/_std, losses/sdlr_ctrl_loss, actor_clip_frac.
# =====================================================================================
...
class Args:
    ...
    # --- STATEDYNLR v1: per-perceptron state-dependent learning rate (self-supervised) ---
    statedynlr: bool = True          # modulate the POLICY gradient per (sample, trunk unit)
    sdlr_alpha_min: float = 0.05     # freeze bound: near-zero learning from bad/noisy data, but
    #                                  enough leak that evidence stays measurable and freeze is
    #                                  REVERSIBLE (v3 postmortem: 0.01 + absorbing freeze let
    #                                  the whole net strangle itself to alpha_mean=0.02 by 0.8M).
    sdlr_alpha_max: float = 8.0      # hot bound: alpha up to 8x for high-evidence units/states.
    sdlr_logit_decay: float = 0.01   # per-update mean-reversion of the controller output toward
    #                                  the neutral anchor: freeze/hot must be CONTINUALLY
    #                                  re-earned from evidence, stale gates decay away.
...
class SDLRController(nn.Module):
    """Per-perceptron state-dependent LR modulator trained by DIRECT one-step
    meta-gradient (v3 METASTEP). NO forecast, NO regression target.
    ...
    engagement, learned site embedding -- and emits a bounded modulation
        alpha_ij(s) = exp(c*(2*sigmoid(logit_ij)-1))   in [e^-c, e^c],
    applied elementwise to the POLICY gradient in the actor backward only.

    SUPERVISION = realized step outcome (one-step hypergradient, collapsed to
    per-unit scalars): after step t with modulated batch-mean mm_j(t) =
    mean_i(alpha_ij g_ij) and the NEXT backward's raw batch-mean r_j(t+1),
        e_j = r_j(t+1) * mm_j(t)
    is positive iff stepping along alpha's direction reduced the next loss.
    phi descends  L_meta = -mean( en_j * logit_ij(t) ),  en = clamp(e/rms(e),+-3):
    productive steps push their logits up (bigger future alpha there), harmful
    steps push them down. This is dL_{t+1}/dphi ~= -<g_{t+1}, u_t> dalpha/dphi
    without keeping any cross-step autograd graph.

    Stats are captured in the same backward hook BEFORE the site's own scaling.
    v7 PROGRESS: supervision is the per-step realized progress e_j =
    -<grad_j, dW_j> computed from each unit's ACTUAL applied update -- next-step
    granularity, Adam/clip effects included by construction.
    """

    def __init__(self, site_modules, unit_params, H, alpha_min=0.1, alpha_max=4.0,
                 ema_beta=0.98, lr=1e-3, warmup=100):
        """unit_params: {site: (weight_tensor, unit_dim)} -- the parameter slice
        whose rows ARE the perceptrons (Linear weight rows / block in_proj rows).
        Used for per-step realized-progress evidence: e_j = -<grad_j, dW_j>.
        Adam-aware BY CONSTRUCTION: dW is the actual post-Adam post-clip update,
        so the credit already includes every optimizer effect; and because the
        target refreshes EVERY step, the alpha it trains is never quasi-static --
        the component Adam would cancel is exactly the part we do not use."""
        super().__init__()
        self.unit_params = dict(unit_params)
        self.site_names = list(unit_params.keys())
        self.H = H
        ...
        raw_b0 = self.logit_cap * torch.atanh(self.b0.clamp(-0.999 * self.logit_cap) / self.logit_cap)
        nn.init.zeros_(self.net[-1].weight)
        nn.init.constant_(self.net[-1].bias, float(raw_b0))
        ...
        self._last_en = None       # last normalized evidence (S, H) detached
        self._handles = []
    ...
    def prepare(self):
        """Run f_phi on this minibatch's stored activations; stash detached alphas
        for the backward hooks and keep the logits' graph for the control loss."""
        self._alphas.clear()
        self._logits.clear()
        ...
        for idx, name in enumerate(self.site_names):
            ...
            # TANH LOGIT CAP: keeps sigmoid' > 0 at the extremes so outcome
            # evidence can always move a unit's gate back (see init note).
            logits = self.logit_cap * torch.tanh(self.net(rows).squeeze(-1) / self.logit_cap)
            self._logits.append((name, logits))
            if modulate:
                # v3 FREEZE: linear map from sigmoid to [a_min, a_max] instead of
                # exp(c*(2s-1)) in [e^-c, e^c]. The floor gates a unit to ~0 (no
                # learning from bad/noisy data); the ceiling allows a hot step for
                # high-evidence states. Init bias anchors alpha=1 exactly.
    ...
    @torch.no_grad()
    def stash(self):
        """Call right BEFORE optimizer.step(): (0) fold this backward's grad
        stats into the per-unit feature EMAs (they feed f_phi's inputs);
        (1) snapshot each unit's parameter row and gradient row. After the step,
        delta = w_after - w_before is the ACTUAL applied update (Adam moments +
        clip + everything), so the progress credit is exact and per-step."""
        ...
        self._snap = {}
        self.updates += 1
        for name, (w, dim) in self.unit_params.items():
            g = w.grad
            if g is None:
                continue
            if dim == 0:
                self._snap[name] = (w.detach().clone(), g.detach().clone())

    def observe(self):
        """Call right AFTER optimizer.step(): compute per-perceptron realized
        progress e_j = -<grad_row_j, dW_row_j> (the linearized loss decrease
        attributed to unit j's weight movement this step), normalize per site,
        and train f_phi on THIS step's logits against it. Next-step granularity:
        prediction made at t is scored on t's own realized outcome."""
        with torch.no_grad():
            prog, valid = [], []
            for name, snap in getattr(self, "_snap", {}).items():
                w_before, g = snap
                w = self.unit_params[name][0]
                dW = w.detach() - w_before                   # applied update rows
                e = -(g * dW).sum(dim=1)                     # (H,) >= ~0 when healthy
                sd = e.std()
                if not torch.isfinite(sd) or sd < 1e-12:
                    continue
                lmap = dict(self._logits)
                if name not in lmap:
                    continue
                prog.append((e / sd).clamp(-3.0, 3.0))
                valid.append(name)
            if not prog or self.updates < self.warmup:
                self._snap = {}
                return None
            en_all = torch.stack(prog)                       # (S, H)
            tgt = torch.sigmoid(en_all)                      # in (0,1), 0.5 == neutral
            # Adam-penetration probe: does alpha actually move the applied
            # update? corr over units between mean alpha_j and |dW_row_j|.
            # Quasi-static alpha components cancel under Adam; this diagnostic
            # makes that visible instead of assumed away.
            try:
                a_means, dnorms = [], []
                for n in valid:
                    a = self._alphas.get(n)
                    if a is None:
                        continue
                    a_means.append(a.float().mean(dim=0))
                    dnorms.append((self.unit_params[n][0].detach() - self._snap[n][0]).norm(dim=1))
                av = torch.cat(a_means); dv = torch.cat(dnorms)
                if av.std() > 1e-9 and dv.std() > 1e-12:
                    av = (av - av.mean()) / av.std()
                    dv = (dv - dv.mean()) / dv.std()
                    self._dw_corr = float((av * dv).mean().item())
            except Exception:
                pass
            self._last_en = en_all.detach()
            self._snap = {}
        # OUTSIDE no_grad: regress this step's logits onto its own outcome.
        pred = torch.stack([dict(self._logits)[n] for n in valid])   # (S, B, H), graph alive
        meta_loss = F.mse_loss(torch.sigmoid(pred), tgt.unsqueeze(1).expand_as(pred))
    ...
    def diagnostics(self):
        out = {}
        if self._last_en is not None:
            out["debug/sdlr_progress_mean"] = self._last_en.mean().item()
            out["debug/sdlr_progress_std"] = self._last_en.std().item()
        out["debug/sdlr_dw_corr"] = float(getattr(self, "_dw_corr", float("nan")))
...
if __name__ == "__main__":
    ...
    if args.statedynlr:
        ...
        unit_params = {"entry": (agent.trunk.entry.weight, 0)}
        for k_, blk in enumerate(agent.trunk.blocks):
            unit_params[f"block{k_}"] = (blk.in_proj.weight, 0)
        unit_params["out_proj"] = (agent.trunk.out_proj.weight, 0)
        sdlr = SDLRController(
            site_modules,
            unit_params,
            args.hidden,
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    if sdlr is not None:
                        # Snapshot unit rows + grads BEFORE the step so the
                        # realized update dW can be measured after it.
                        sdlr.stash()
                    optimizer.step()
                    if sdlr is not None:
                        # Per-step realized progress e_j = -<grad_j, dW_j>:
                        # train f_phi on THIS step's outcome (next-step
                        # granularity), with Adam/clip effects included in dW.
