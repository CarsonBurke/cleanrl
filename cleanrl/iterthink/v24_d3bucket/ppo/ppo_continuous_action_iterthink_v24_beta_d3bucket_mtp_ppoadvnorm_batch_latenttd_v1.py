# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + LATENT-TD (psi) v1.
# =====================================================================================
# LATENT-TD: a TD/lambda-return SELF-prediction auxiliary for the ACTOR's representation.
# The trunk feature z_t (the same H-dim feature the Beta action heads consume) becomes a
# VECTOR pseudo-reward, and a psi head on the trunk learns its discounted future stream:
#     psi(s_t) ~= E[ (1-gamma_lat) * sum_k gamma_lat^k * LN(z_{t+k}) ]
# i.e. the successor-feature recursion psi(s_t) = (1-g)*LN(z_t) + g*psi(s_{t+1}) trained with
# lambda-returns. This extends one-step next-latent prediction (NextLat-style, arXiv:2511.05963)
# to a bootstrapped HORIZON: instead of "what is my next latent", the trunk must know "where is
# my representation heading over the next ~1/(1-gamma_lat) steps" — the vector analog of a
# critic, whose fixed points are the slow eigenfunctions of the policy's transition operator.
# HYPOTHESIS: PPO's actor trunk is representation-starved (only the sparse PG signal reaches
# it); a horizon-matched dense vector-TD aux organizes trunk features around long-run,
# value-relevant structure and beats/no-ops the base. Collapse ("make z smooth so psi is easy")
# is held off by (a) the PG+critic losses anchoring the shared trunk, (b) SNAPSHOT targets:
# psi lambda-returns are built ONCE per iteration from rollout-time latents and rollout-time
# psi (the natural lagged-target analog of the critic's fixed `returns`), (c) lambda=0.95
# keeping targets dominated by real observed latents rather than bootstrap, (d) LayerNorm on
# the pseudo-reward latents. The psi loss gets ITS OWN dual-backward clip budget
# (latent_td_grad_clip=0.1, vs actor 0.25 / critic 0.25) so it can never swamp the policy
# gradient on the shared trunk. Watch: losses/latent_td_loss, debug/latent_zn_batch_std
# (collapse probe: per-dim cross-batch std of LN latents; healthy O(1), collapse -> 0).
#
...
class Args:
    ...
    # LATENT-TD (psi) auxiliary: TD/lambda-return self-prediction of the actor trunk's
    # own LayerNormed latents (see header). Targets are snapshot lambda-returns computed
    # once per iteration with the SAME masks/recursion as the reward GAE.
    latent_td: bool = True
    latent_td_gamma: float = 0.99    # horizon of the latent stream (~100 steps)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # LATENT-TD psi head: predicts the (1-gamma_lat)-scaled discounted stream of the
        # trunk's own LayerNormed latents. Small final init => psi starts ~0, so early
        # lambda-return targets are dominated by real observed latents, not bootstrap.
        # Initialized LAST so all base-shared parameters draw identical initial weights
        # from the seeded RNG as the ppoadvnorm_batch base run.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            next_actor_feats_flat, next_value_logits_all = agent.get_feat_and_value(
            ...
            # LATENT-TD psi targets: lambda-returns over the trunk's OWN LayerNormed
            # latents, with the exact reward-GAE recursion/masks vectorized over the H
            # latent channels. Pseudo-reward is (1-gamma_lat)*LN(z_t), so psi predicts a
            # discount-weighted AVERAGE of future latents (stays at latent scale). All
            ...
            if args.latent_td:
                ...
                psi_roll = agent.psi_head(latents)                             # (T,B,H)
                next_actor_feats = next_actor_feats_flat.reshape(
                    args.num_steps, args.num_envs, args.hidden
                )
                psi_next = agent.psi_head(next_actor_feats)                    # (T,B,H)
                g_lat, lam_lat = args.latent_td_gamma, args.latent_td_lambda
                psi_adv = torch.zeros_like(zn)
                lastpsilam = 0.0
                for t in reversed(range(args.num_steps)):
                    boot_nonterm = ((1.0 - transition_terminations[t]) * transition_valids[t]).unsqueeze(-1)
                    lam_nonterm = (1.0 - transition_boundaries[t]).unsqueeze(-1)
                    psi_delta = (1.0 - g_lat) * zn[t] + g_lat * psi_next[t] * boot_nonterm - psi_roll[t]
                    psi_adv[t] = lastpsilam = psi_delta + g_lat * lam_lat * lam_nonterm * lastpsilam
                psi_targets = psi_adv + psi_roll                               # (T,B,H)
                # Collapse probe: cross-batch per-dim std of the LN latents. LayerNorm fixes
                # each SAMPLE's scale, so identical-latents collapse shows up here as -> 0.
        ...
        b_u = u.reshape(-1)
        if args.latent_td:
            b_psi_targets = psi_targets.reshape(-1, args.hidden)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # LATENT-TD: regress the CURRENT psi(trunk(s)) onto the fixed snapshot
                # lambda-return targets. Gradient flows through psi_head INTO the trunk —
                # that is the point of the aux.
                if args.latent_td:
                    latent_td_loss = F.mse_loss(agent.psi_head(mb_actor_feat), b_psi_targets[mb_inds])
                else:
                    latent_td_loss = None
