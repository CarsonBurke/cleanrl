# BetaCE v1 — fixed per-state Beta targets from a single sampled action.
#
# The categorical proposal q ∝ pi_old * exp(A / eta) cannot be copied literally
# to continuous control: changing one point of a density has zero measure. This
# variant uses the policy-head analogue. For each sampled native Beta action z,
# it takes one stopped score step in the OLD alpha/beta head-logit space:
#
#   u = clamp(zscore_batch(GAE) / eta, -tilt_clip, tilt_clip)
#   h_target = h_old + scale * u * grad_h log pi_old(z | s)
#
# alpha_target,beta_target = 1 + softplus(h_target), so every target remains in
# the same unimodal Beta family as the actor. A positive advantage increases the
# sampled action's density; a negative advantage suppresses it. u=0 gives the
# old policy exactly. A rollout-wide bisection chooses `scale` so
# mean KL(Beta_old || Beta_target) matches `target_policy_kl`; eta therefore
# changes the tilt direction without confounding target step size.
#
# The actor M-step is pure analytic cross-entropy, implemented as
# KL(Beta_target || Beta_theta). The target is frozen for all update epochs, the
# gradient self-extinguishes at a match, and PPO ratios/clipping/early stopping
# are absent. The critic remains IterThink v24's shared trunk plus Dreamer3
# 511-bucket HL-Gauss MTP head. Hot fixed-shape rollout, update, and bootstrap
# forwards use torch.compile(dynamic=False, mode="reduce-overhead") with explicit
# CUDA-graph step markers.
#
# Hypothesis: a fixed, reachable distribution target preserves signed GAE
# information while avoiding PPO's moving ratio geometry and PMPO's discarded
# magnitude. Primary comparison: ppoadvnorm_batch_v1 (8278 @8M); PMPO v1 (7636).
#
import os
import random
import time
...
def inverse_softplus(x):
    """Stable inverse of softplus for strictly positive Beta excess concentration."""
    return x + torch.log(-torch.expm1(-x))


@torch.no_grad()
def build_beta_ce_target(old_alpha, old_beta, z, advantage, args):
    """Build a stopped, reachable Beta target at a fixed reverse-KL radius."""
    old_alpha = old_alpha.float()
    old_beta = old_beta.float()
    z = z.float().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
    normalized_advantage = (advantage.float() - advantage.float().mean()) / (
        advantage.float().std() + 1e-8
    )
    tilt = (normalized_advantage / args.beta_ce_eta).clamp(
        -args.beta_ce_tilt_clip, args.beta_ce_tilt_clip
    )

    raw_alpha = inverse_softplus((old_alpha - 1.0).clamp_min(1e-8))
    raw_beta = inverse_softplus((old_beta - 1.0).clamp_min(1e-8))
    digamma_sum = torch.digamma(old_alpha + old_beta)
    score_alpha = (
        z.log() - torch.digamma(old_alpha) + digamma_sum
    ) * torch.sigmoid(raw_alpha)
    score_beta = (
        torch.log1p(-z) - torch.digamma(old_beta) + digamma_sum
    ) * torch.sigmoid(raw_beta)
    direction_alpha = tilt.unsqueeze(-1) * score_alpha
    direction_beta = tilt.unsqueeze(-1) * score_beta
    old_dist = Beta(old_alpha, old_beta)

    def target_at(scale):
        target_alpha = 1.0 + F.softplus(raw_alpha + scale * direction_alpha)
        target_beta = 1.0 + F.softplus(raw_beta + scale * direction_beta)
        target_dist = Beta(target_alpha, target_beta)
        target_kl = kl_divergence(old_dist, target_dist).sum(-1).mean()
        return target_alpha, target_beta, target_kl

    low = torch.zeros((), device=old_alpha.device)
    high = torch.as_tensor(args.beta_ce_max_scale, device=old_alpha.device)
    _, _, high_kl = target_at(high)
    for _ in range(args.beta_ce_bisection_steps):
        midpoint = (low + high) * 0.5
        _, _, midpoint_kl = target_at(midpoint)
        below_budget = midpoint_kl < args.target_policy_kl
        low = torch.where(below_budget, midpoint, low)
        high = torch.where(below_budget, high, midpoint)
    scale = torch.where(high_kl < args.target_policy_kl, high, (low + high) * 0.5)
    target_alpha, target_beta, target_kl = target_at(scale)
    saturation = (normalized_advantage.abs() / args.beta_ce_eta >= args.beta_ce_tilt_clip).float().mean()
    return target_alpha.detach(), target_beta.detach(), target_kl.detach(), scale.detach(), saturation.detach()
...
class Args:
    ...
    # BetaCE uses one rollout-wide z-score, independent of legacy PPO shaping.
    norm_adv: bool = True
    ret_percnorm: bool = False
    ret_perc_scope: str = "batch"
    ret_perc_rate: float = 0.01
    ret_perc_lo: float = 0.05
    ret_perc_hi: float = 0.95
    ret_perc_floor: float = 1.0
    clip_coef: float = 0.2  # diagnostics only; BetaCE has no ratio clipping
    ...
    target_kl: Optional[float] = None  # no moving-policy early stop

    # Fixed-target E-step. The KL budget fixes target distance; eta changes only
    # the clipped advantage-tilt direction.
    target_policy_kl: float = 0.03
    beta_ce_eta: float = 1.0
    beta_ce_tilt_clip: float = 3.0
    beta_ce_max_scale: float = 16.0
    beta_ce_bisection_steps: int = 16
...
class Agent(nn.Module):
    ...
    def get_beta_parameters(self, x):
        actor_feat = self.trunk(x) if self.share_backbone else self.actor_trunk(x)
...
if __name__ == "__main__":
    ...
    if args.actor_dist != "beta":
        raise ValueError("BetaCE requires actor_dist='beta'")
    if args.beta_ce_eta <= 0.0 or args.target_policy_kl <= 0.0:
        raise ValueError("beta_ce_eta and target_policy_kl must be positive")
    if args.beta_ce_tilt_clip <= 0.0 or args.beta_ce_max_scale <= 0.0:
        raise ValueError("beta_ce_tilt_clip and beta_ce_max_scale must be positive")
    if args.beta_ce_bisection_steps < 1:
        raise ValueError("beta_ce_bisection_steps must be at least one")
    ...
    def policy_rollout_fn(observation):
        return agent.get_action_and_value(observation)

    def policy_update_fn(observation, native_action):
        return agent.get_action_and_value(observation, native_action)

    def value_flat_fn(observation):
        return agent.get_value(observation)

    def beta_params_fn(observation):
        return agent.get_beta_parameters(observation)
    ...
    if args.compile:
        ...
        value_flat_fn = torch.compile(
            value_flat_fn, mode=args.compile_mode, dynamic=False
        )
        beta_params_fn = torch.compile(
            beta_params_fn, mode=args.compile_mode, dynamic=False
        )
        print(f"compiled fixed-shape forward paths ({args.compile_mode})")
    ...
    old_alphas = torch.zeros_like(actions)
    old_betas = torch.zeros_like(actions)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                action, z, logprob, ent, value_logits, old_alpha, old_beta = policy_rollout_fn(next_obs)
            ...
            old_alphas[step] = old_alpha
            old_betas[step] = old_beta
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                _, _, boot_logprob, _, _, _, _ = policy_rollout_fn(next_obses[-1])
        ...
        (
            ...
            beta_target_kl,
            beta_target_scale,
            beta_tilt_saturation,
        ) = build_beta_ce_target(
            b_old_alphas,
            b_old_betas,
            b_latent_zs,
            b_advantages,
        ...
        clipfracs = []
        policy_reverse_kl = torch.zeros((), device=device)
        target_fit_kl = torch.zeros((), device=device)
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                (
                    ...
                    new_alpha,
                    new_beta,
                ) = policy_update_fn(b_obs[mb_inds], b_latent_zs[mb_inds])
                new_dist = Beta(new_alpha, new_beta)
                old_dist = Beta(b_old_alphas[mb_inds], b_old_betas[mb_inds])
                target_dist = Beta(b_target_alphas[mb_inds], b_target_betas[mb_inds])

                # Analytic cross-entropy up to the fixed target's entropy.
                # No sampled ratio, clipping, or moving-policy term enters this loss.
                pg_loss = kl_divergence(target_dist, new_dist).sum(-1).mean()
                with torch.no_grad():
        ...
        # Evaluate the completed M-step over the full rollout, not a stale
        # pre-step shuffled minibatch.
        with torch.no_grad():
            reverse_kl_sum = torch.zeros((), device=device)
            target_fit_kl_sum = torch.zeros((), device=device)
            for start in range(0, args.batch_size, args.minibatch_size):
                end = start + args.minibatch_size
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                final_alpha, final_beta = beta_params_fn(b_obs[start:end])
                final_dist = Beta(final_alpha, final_beta)
                old_dist = Beta(b_old_alphas[start:end], b_old_betas[start:end])
                target_dist = Beta(b_target_alphas[start:end], b_target_betas[start:end])
                reverse_kl_sum += kl_divergence(old_dist, final_dist).sum(-1).sum()
                target_fit_kl_sum += kl_divergence(target_dist, final_dist).sum(-1).sum()
            policy_reverse_kl = reverse_kl_sum / args.batch_size
            target_fit_kl = target_fit_kl_sum / args.batch_size
        ...
        writer.add_scalar("losses/policy_reverse_kl", policy_reverse_kl.item(), global_step)
        writer.add_scalar("losses/beta_target_fit_kl", target_fit_kl.item(), global_step)
        writer.add_scalar("debug/beta_target_kl", beta_target_kl.item(), global_step)
        writer.add_scalar("debug/beta_target_scale", beta_target_scale.item(), global_step)
        writer.add_scalar("debug/beta_tilt_saturation", beta_tilt_saturation.item(), global_step)
