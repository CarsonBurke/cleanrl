# RESPC v8: SPEND THE AUX BUDGET ON THE TRUNK -- now that the trunk cannot cheat.
# =====================================================================================
# v5 asked "what if the representation were mostly world-model shaped?" and got collapse:
# raising the trunk's aux clip monotonically shrank the latent scale (std 3.07 -> 1.24)
# and return fell with it, because the trunk was the one party to the aux able to lower
# the loss by making the TARGET easier rather than the prediction better.
# v7 removed that: the loss is the fraction of the latent's ACTUAL MOVEMENT left
# unexplained, which is exactly invariant to h -> c*h and to making the latent sluggish,
# and the KL is divided by the same scale so no term rewards shrinking. Measured on the
# winning v7 arm: nmse 0.064 (93.6% of the movement explained) with latent_batch_std
# GROWING 1.5 -> 2.8. The cheat is priced out and the world model is real.
# So v5's question is now well-posed for the first time -- and it needs asking, because
# v7 also measured where its budget actually goes:
#     trunk 0.0296   pred 0.2478   head 0.0128     (delivered by the joint clip 0.25)
# 99% of the aux norm lands on the aux-only predictor, which Adam renormalizes anyway,
# while the trunk -- the entire point of the auxiliary -- gets 0.0296 against the actor's
# 0.25, leaving the representation only ~11% world-model shaped. That is very likely why
# a 14x better world model bought only ~3% return.
# v8 = v7's winning arm (cascade_coef 0) + v5's disjoint clip groups, with ONE variable
# raised: the trunk's. pred and head stay pinned at v7's delivered levels -- the head
# especially, since v5 already proved head aux is what stalls PPO (approx_kl 0.014 vs
# 0.031) and that result does not need re-deriving.
# HYPOTHESIS: return improves with trunk budget now, reversing v5's sign, because the
# gradient the trunk receives is finally about prediction rather than about shrinking.
# FALSIFIER: if debug/latent_batch_std falls with trunk budget again, some cheat survived
# the reformulation and the invariance argument is wrong somewhere. Watch it first.
# =====================================================================================
#
# --- inherited RESPC v7 notes ---
# =====================================================================================
# NextLat asks the latent to predict its own future. The measurements say it is mostly
# answering a question nobody needed asked.
#
# WHAT THE INCUMBENT IS ACTUALLY DOING. At 2M on clip025, pred_loss 0.803 against
# latent_batch_std 3.07. SmoothL1 is quadratic here, so per-dim MSE ~= 2*0.803 = 1.61
# against per-dim variance 9.42 -- a normalized error of ~0.17, i.e. 83% of the target
# "explained". But the IDENTITY predictor (h_hat = h_t, "the state stays put") already
# scores 2(1-rho_i) on a signal with lag-i autocorrelation rho, and MuJoCo latents at
# dt=0.05 are extremely smooth. An average rho of ~0.91 over depths 1..4 reproduces 0.17
# exactly. So the incumbent's aux loss is consistent with a predictor that has learned
# little beyond IDENTITY -- and v1's predictor is an MLP with std=0.1 output init, so it
# begins by predicting ~0 against a target of ~h_t and must burn capacity climbing to
# the trivial solution before it can learn any dynamics at all.
# That is the opportunity: the informative part of the signal is the ~17% RESIDUAL, and
# it is buried under a trivial component worth 6x more.
#
# THE METHOD. Code the residual, at three levels:
#  1. RESIDUAL ARCHITECTURE. h_{t+1} = h_t + g(h_t,a_t), both output layers ZERO-init, so
#     the rollout starts exactly at the identity baseline and every gradient from step 0
#     is about the CHANGE. It also closes v5's second cheat channel: h_hat inherits h's
#     scale structurally instead of being a fresh Linear output with a free scale.
#  2. RESIDUAL NORMALIZATION. Divide both sides by s_i = RMS of the ACTUAL movement
#     d_i = sg[h_{t+i}] - sg[h_t] (one detached scalar per depth -- a unit change, not a
#     whitening). The loss becomes "fraction of the movement left unexplained", which is
#     invariant to BOTH measured pathologies: h -> c*h scales error and d together, so
#     v5's shrink cheat pays nothing; and making the latent temporally sluggish shrinks d
#     exactly as fast as the error, so the low-pass cheat pays nothing either. It also
#     equalizes depths -- d grows with i, so depth 4 used to drown out depth 1.
#  3. EXPLAINING-AWAY CASCADE (the predictive-coding part). A LINEAR stage predicts the
#     delta; the nonlinear stage sees the linear stage's answer and models only what it
#     left over. Both are supervised against the same target, so the MLP cannot spend
#     itself re-deriving linear structure.
#
# WHY THIS SHOULD HELP WHERE v5 DID NOT. v5 tried to buy more aux INFLUENCE and got
# collapse, because the trunk was the one party able to make the target easier. v7
# changes what the aux MEASURES so that the easy answers score zero, then leaves the
# budget exactly where the sweep put it (joint clip 0.25 = clip025). One variable.
#
# THE FALSIFIER IS BUILT IN. losses/nextlat_nmse is 1 - R^2 against the identity
# baseline: 1.0 means the dynamics model is worth nothing over "the state stays put".
# If it sits near 1.0, the aux never had predictive content and the whole NextLat line
# has been running on the KL term alone -- which would be the single most useful thing
# this run could tell us, win or lose. The lin_nmse/nmse gap prices the nonlinearity.
# ARMS: cascade_coef 0.25 (full method) vs 0.0 (residual coding without the cascade).
# =====================================================================================
#
# --- inherited nextlat v1 notes ---
# =====================================================================================
...
class Args:
    ...
    # v8: DISJOINT aux clip groups. v7 measured what the joint 0.25 actually delivers --
    # trunk 0.0296 / pred 0.2478 / head 0.0128 -- i.e. 99% of the aux norm goes to the
    # aux-only predictor (inert under Adam) and the trunk, the thing the aux exists to
    # shape, ends up only ~11% world-model-shaped against the actor's 0.25.
    nextlat_trunk_grad_norm: float = 0.15  # THE variable. RENORMALIZED (up or down) to
                                     # exactly this, because a clip cannot raise a budget
                                     # -- see _renorm_grads_. v7 delivered 0.0296 (full-run
                                     # mean; 0.036 over the recent window), so 0.15 is
                                     # ~4-5x and 0.50 is ~14-17x. The raw trunk norm is
                                     # only ~0.36, so 0.50 is genuinely UNREACHABLE by
                                     # clipping and needs the gain.
    nextlat_trunk_max_gain: float = 4.0  # cap on amplification, so a collapsed aux
                                     # gradient (mostly noise) is never blown up to target
    nextlat_pred_grad_clip: float = 0.25   # pinned at v7's delivered level (expected inert)
    nextlat_head_grad_clip: float = 0.018  # v7's DELIVERED head mean (0.0176), not the
                                     # end-of-window 0.0128 -- pinning at the latter would
                                     # have tightened the head ~26% below the arm we are
                                     # building on, adding a second variable to a study
                                     # meant to isolate the trunk. Still imperfect: v7's
                                     # delivered head DECAYED (~0.033 early -> 0.013 late)
                                     # and no flat value reproduces that. NOT raised --
                                     # v5 established head aux is what stalls PPO
                                     # (approx_kl 0.014 vs 0.031).
    nextlat_cascade_coef: float = 0.0   # v8 default is v7's WINNER. Supervising the linear
                                     # stage cost ~17% return (4414 vs 5331 @1.5M): it
                                     # forces a GREEDY stagewise split, and the best
                                     # (linear, nonlinear) decomposition of a sum is not
                                     # the one whose linear part is individually optimal.
                                     # Unsupervised, g_lin is a linear SKIP FEATURE the
                                     # MLP consumes, and the joint fit is strictly better
                                     # (nmse 0.064 vs 0.072). Original doc follows.
                                     # weight on the LINEAR stage's own prediction error.
                                     # 0.0 => pure residual prediction (no explaining-away
                                     # cascade); the two arms A/B exactly this.
    nextlat_normalize: bool = True   # divide both sides by the actual-movement RMS s_i.
                                     # --no-nextlat-normalize + huber-beta 1.0 gives the
                                     # residual ARCHITECTURE alone, isolating (1) from (2).
    nextlat_huber_beta: float = 0.1  # SmoothL1 knee, and the reason the aux does not
                                     # self-anneal. v1's RAW residual sat ABOVE the beta=1
                                     # knee, so its gradient was L1-saturated -- constant
                                     # magnitude regardless of fit -- and clip025 therefore
                                     # injected a steady 0.25-norm aux for all 8M steps.
                                     # Normalizing puts the residual at RMS 1.0 and then
                                     # drives it DOWN, so at beta=1 we would slide into the
                                     # quadratic regime where grad ~ sqrt(nmse): the aux
                                     # would decay ~100x and switch itself off, and a null
                                     # result would be unreadable ("loss is neutral" vs "we
                                     # stopped training it"). beta=0.1 keeps |u|>0.1 -- the
                                     # whole plausible range, nmse 1.0 down to 0.01 -- in
                                     # the LINEAR regime, so the gradient stays fit-
                                     # independent and the clip keeps binding, exactly as
                                     # it does for the incumbent.
...
def _renorm_grads_(params, target, max_gain, device):
    """Rescale a param group's gradient to EXACTLY `target` norm, in place.

    Unlike clip_grad_norm_ this scales UP as well as down, which is required to push a
    group's budget above the loss's own gradient magnitude. `max_gain` caps the
    amplification so that if the aux gradient ever collapses (a near-perfect world model
    leaves mostly noise) we do not blow that noise up to the target norm. Returns the
    PRE-scale norm, matching clip_grad_norm_'s convention so the logged series stays
    comparable to every earlier run.
    """
    sq = [p.grad.pow(2).sum() for p in params if p.grad is not None]
    if not sq:
        return torch.zeros((), device=device)
    raw = torch.stack(sq).sum().sqrt()
    scale = (target / raw.clamp_min(1e-12)).clamp(max=max_gain)
    for p in params:
...
class ResidualDynamics(nn.Module):
    """Action-conditioned latent dynamics as a two-stage RESIDUAL cascade.

    Predicts the CHANGE, never the state: h_{t+1} = h_t + g(h_t, a_t). Two consequences.
    (1) The identity map is free, so no capacity is spent holding h_{t+1} ~= h_t (v1's
    MLP had to learn it, and its std=0.1 output init starts it predicting ~0 against a
    target of ~h_t -- the worst possible starting point).
    (2) h_hat inherits h's scale structurally instead of being a fresh Linear output with
    a free scale, closing one of the two channels that let v5 cheat by shrinking.

    The cascade is the predictive-coding part: a LINEAR stage explains what it can, and
    the nonlinear stage models only what the linear stage left over. `lin` sees [h, a];
    `mlp` additionally sees the linear stage's own answer, so it can subtract it. Both
    output layers are ZERO-initialized => g == 0 at step 0 => the model starts exactly at
    the identity baseline and every subsequent gradient is about the residual. (Zero
    weights still receive gradient: dL/dW = delta (x) x, unlike a zero-init gate.)
    """

    def __init__(self, H, act_dim):
        super().__init__()
        self.lin = layer_init(nn.Linear(H + act_dim, H), std=0.0)
        self.mlp = nn.Sequential(
            layer_init(nn.Linear(H + act_dim + H, H)),
            ReLUSquared(),
            layer_init(nn.Linear(H, H), std=0.0),
        )

    def forward(self, h, a):
        ha = torch.cat([h, a], dim=-1)
        g_lin = self.lin(ha)
        g_res = self.mlp(torch.cat([ha, g_lin], dim=-1))
        # (linear-only delta, full delta) -- both are supervised against the same target.
        return g_lin, g_lin + g_res
...
class Agent(nn.Module):
    ...
    def nextlat_parameters(self):
        # Params receiving the NEXTLAT gradient: the ACTOR-side trunk (the representation
        # this aux exists to shape), the latent dynamics MLP, and the action heads (the
        # KL distillation's student path decodes through them). One dual-backward group.
        trunk = self.trunk if self.share_backbone else self.actor_trunk
        if self.actor_dist == "gaussian":
            heads = list(self.actor_head.parameters()) + list(self.actor_logvar_head.parameters())
        else:
...
if __name__ == "__main__":
    ...
    # Disjoint slices of nextlat_params, used ONLY to log the norm decomposition.
    _trunk_mod = agent.trunk if agent.share_backbone else agent.actor_trunk
    nextlat_trunk_diag = list(_trunk_mod.parameters())
    nextlat_pred_diag = list(agent.nextlat_predictor.parameters())
    _pd_ids = set(map(id, nextlat_trunk_diag)) | set(map(id, nextlat_pred_diag))
    nextlat_head_diag = [p for p in nextlat_params if id(p) not in _pd_ids]
    assert len(nextlat_trunk_diag) + len(nextlat_pred_diag) + len(nextlat_head_diag) == len(
        nextlat_params
    ), "v8 clip groups must PARTITION the aux params (no param dropped or double-clipped)"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.nextlat:
                    h_hat = mb_actor_feat
                    h0_sg = mb_actor_feat.detach()   # constant anchor for the normalizer
                    pred_losses, lin_losses, kl_losses, nmses, lin_nmses = [], [], [], [], []
                    for i in range(1, args.nextlat_depth + 1):
                        act_idx = np.clip(mb_inds + (i - 1) * args.num_envs, 0, args.batch_size - 1)
                        tgt_idx = np.clip(mb_inds + i * args.num_envs, 0, args.batch_size - 1)
                        # RESIDUAL rollout: the chain carries the state, the net supplies
                        # only the increment. g_lin is the linear stage's answer alone.
                        g_lin, g_full = agent.nextlat_predictor(h_hat, b_actions[act_idx])
                        h_lin = h_hat + g_lin
                        h_hat = h_hat + g_full
                        with torch.no_grad():
                        ...
                        # --- RESIDUAL PREDICTIVE CODING normalizer -------------------
                        # d_i = how far the latent ACTUALLY moved over i steps, fully
                        # detached. s_i = its masked RMS: one scalar per depth, so no
                        # per-dim reweighting (this is a unit change, not a whitening).
                        # Dividing by it makes the loss "fraction of the MOVEMENT left
                        # unexplained", which is invariant to BOTH pathologies we
                        # measured: h -> c*h scales error and d alike (kills v5's shrink
                        # cheat), and making the latent temporally sluggish shrinks d
                        # just as fast as the error (kills the low-pass cheat). The only
                        # way down is to actually predict where the state is going.
                        # It also equalizes depths, which the raw loss never did: d grows
                        # with i, so depth 4 used to drown out depth 1 in the mean.
                        d_i = tgt_feat - h0_sg
                        d_sq = (d_i.pow(2).mean(-1) * mask_i).sum() / denom
                        if args.nextlat_normalize:
                            # Floor s_i RELATIVE to the latent's own RMS. An absolute floor
                            # would be dimensionally wrong: s_i's natural scale tracks h
                            # (~3 here), so a fixed 1e-4 never guards a genuinely quiet
                            # minibatch, and if it ever did fire it would inject a ~1/s
                            # direction huge enough to eat the whole clip budget for that
                            # depth. A relative floor also preserves scale invariance,
                            # since it scales with h exactly as s_i does.
                            h_rms = h0_sg.pow(2).mean().sqrt()
                            s_i = d_sq.clamp_min(0.0).sqrt().clamp_min(1e-3 * h_rms + 1e-12)
                        else:
                            s_i = torch.ones((), device=d_sq.device, dtype=d_sq.dtype)

                        beta = args.nextlat_huber_beta
                        pred_l = F.smooth_l1_loss(
                            h_hat / s_i, tgt_feat / s_i, reduction="none", beta=beta
                        ).mean(-1)
                        pred_losses.append((pred_l * mask_i).sum() / denom)
                        lin_l = F.smooth_l1_loss(
                            h_lin / s_i, tgt_feat / s_i, reduction="none", beta=beta
                        ).mean(-1)
                        lin_losses.append((lin_l * mask_i).sum() / denom)

                        # Pure diagnostics (no grad): 1 - R^2 against the identity
                        # baseline. 1.0 == "no better than assuming the state stays put",
                        # 0.0 == perfect. This is the number that says whether the
                        # dynamics model has ever actually done anything.
                        with torch.no_grad():
                            e_sq = ((h_hat - tgt_feat).pow(2).mean(-1) * mask_i).sum() / denom
                            l_sq = ((h_lin - tgt_feat).pow(2).mean(-1) * mask_i).sum() / denom
                            dn = d_sq.clamp_min(1e-8)
                            # Clamped: the ratio is unbounded, and one degenerate depth in
                            # one minibatch would otherwise drag the headline mean to ~1e8
                            # and make the series unreadable for the rest of the run.
                            nmses.append((e_sq / dn).clamp(0.0, 10.0))
                            lin_nmses.append((l_sq / dn).clamp(0.0, 10.0))

                        s_dist, _, _ = agent._actor_dist(h_hat)         # student
                        kl_l = torch.distributions.kl_divergence(t_dist, s_dist).sum(-1)
                        # The KL is locally quadratic in the latent error (KL ~= e' J' F J e
                        # for the head Jacobian J and Fisher F), so under h -> c*h it scales
                        # EXACTLY as c^2. Left raw it would be the only scale-sensitive term
                        # left, and it points DOWNHILL in c -- so making the pred term
                        # perfectly invariant would have concentrated 100% of the aux's
                        # shrink pressure into the KL and could have made v5's collapse
                        # worse per unit of budget, not better. Dividing by the same
                        # detached s_i^2 cancels the c^2 exactly. This does NOT touch the
                        # auto-curriculum, which comes from sigma shrinking as the policy
                        # sharpens -- an entirely different quantity from the latent's
                        # movement scale.
                        if args.nextlat_normalize:
                            kl_l = kl_l / s_i.pow(2)
                        kl_losses.append((kl_l * mask_i).sum() / denom)
                    nextlat_pred_loss = torch.stack(pred_losses).mean()
                    nextlat_lin_loss = torch.stack(lin_losses).mean()
                    nextlat_kl_loss = torch.stack(kl_losses).mean()
                    nextlat_nmse = torch.stack(nmses).mean()
                    nextlat_lin_nmse = torch.stack(lin_nmses).mean()
                    nextlat_loss = (
                        nextlat_pred_loss
                        + args.nextlat_cascade_coef * nextlat_lin_loss
                        + args.nextlat_kl_coef * nextlat_kl_loss
                    )
                else:
                    nextlat_loss = None
                ...
                if args.separate_grad_clip:
                    # DUAL-BACKWARD decoupled clipping (now 3-way). Backprop value,
                    # latent-TD, and policy gradients separately, clip each to its own
                    # max-norm, then sum on the (possibly shared) trunk so no single
                    # loss can swamp the others' contribution to shared features.
                    ...
                    if args.nextlat:
                        ...
                        # v8: three DISJOINT groups, partitioning nextlat_params exactly,
                        # so each rescale touches an independent slice and the summed
                        # result below is structurally unchanged.
                        #
                        # The TRUNK is RENORMALIZED, not clipped, and that distinction is
                        # the whole experiment. clip_grad_norm_ only ever scales DOWN, and
                        # v7's raw trunk norm is ~0.36 (max 0.715) -- so any cap above
                        # ~0.75 is bit-identical to no clip, and a "0.5 cap" would bind on
                        # under 10% of steps. A clip therefore CANNOT raise this budget
                        # above the loss's own gradient magnitude; it can only pin it. It
                        # would also make two arms differ in KIND (pinned-constant vs
                        # free-floating) rather than in degree, and the raw norm decays
                        # over training, so the high arm's effective budget would drift
                        # down while the low arm stayed fixed. Renormalizing to a target
                        # delivers exactly that norm either way, which is what makes
                        # "budget" a real, sweepable variable.
                        nextlat_trunk_gn = _renorm_grads_(
                            nextlat_trunk_diag,
                            args.nextlat_trunk_grad_norm,
                            args.nextlat_trunk_max_gain,
                            device,
                        )
                        nextlat_pred_gn = nn.utils.clip_grad_norm_(
                            nextlat_pred_diag, args.nextlat_pred_grad_clip
                        )
                        nextlat_head_gn = nn.utils.clip_grad_norm_(
                            nextlat_head_diag, args.nextlat_head_grad_clip
                        )
                        nextlat_gn = torch.sqrt(
                            nextlat_trunk_gn ** 2 + nextlat_pred_gn ** 2 + nextlat_head_gn ** 2
                        )
                        # DELIVERED (post-rescale) norms. The pre-rescale series above is
                        # kept for continuity with v7/clip025, but it no longer implies
                        # what each group actually received -- and the budget IS the
                        # variable under study, so measure it directly rather than
                        # inferring it from the cap.
                        with torch.no_grad():
                            def _post(ps):
                                sq = [p.grad.pow(2).sum() for p in ps if p.grad is not None]
                                return torch.stack(sq).sum().sqrt() if sq else torch.zeros((), device=device)
                            nextlat_trunk_deliv = _post(nextlat_trunk_diag)
                            nextlat_head_deliv = _post(nextlat_head_diag)
                        nextlat_grads = [(p, p.grad.detach().clone()) for p in nextlat_params if p.grad is not None]
                    else:
                        nextlat_grads = []
                    optimizer.zero_grad(set_to_none=True)
                    ...
                    # clip_critic(d vl / d trunk) and clip_nextlat(d aux / d trunk).
                    # critic_head / nextlat_predictor get only their own loss's gradient;
                    # the action heads get policy + clipped KL-distill gradients.
                    for p, g in value_grads:
                        p.grad = g if p.grad is None else p.grad + g
                    for p, g in nextlat_grads:
                        p.grad = g if p.grad is None else p.grad + g
                    optimizer.step()
                else:
                    loss = pg_loss - ent_coef_eff * entropy_loss + v_loss * args.vf_coef
                    if args.nextlat:
                        loss = loss + args.nextlat_coef * nextlat_loss
                    optimizer.zero_grad()
                    loss.backward()
        ...
        if args.nextlat:
            ...
            writer.add_scalar("losses/nextlat_lin_loss", nextlat_lin_loss.item(), global_step)
            writer.add_scalar("losses/nextlat_kl_loss", nextlat_kl_loss.item(), global_step)
            # THE headline metric: fraction of the latent's actual movement left
            # unexplained, averaged over depths. 1.0 == the dynamics model is worth
            # nothing over "the state stays put".
            writer.add_scalar("losses/nextlat_nmse", nextlat_nmse.item(), global_step)
            # CAUTION on the "linear" series: h_lin is the full nonlinear chain with only
            # the LAST step taken linearly, so the lin_nmse/nmse gap prices nonlinearity
            # cleanly ONLY at depth 1. The d1 pair below is the honest reading; the depth
            # -averaged pair above is a trend, not a decomposition.
            writer.add_scalar("losses/nextlat_lin_nmse", nextlat_lin_nmse.item(), global_step)
            writer.add_scalar("losses/nextlat_nmse_d1", nmses[0].item(), global_step)
            writer.add_scalar("losses/nextlat_lin_nmse_d1", lin_nmses[0].item(), global_step)
            writer.add_scalar("losses/nextlat_nmse_dmax", nmses[-1].item(), global_step)
            if args.separate_grad_clip:
                writer.add_scalar("losses/nextlat_trunk_grad_norm", float(nextlat_trunk_gn), global_step)
                writer.add_scalar("losses/nextlat_pred_grad_norm", float(nextlat_pred_gn), global_step)
                writer.add_scalar("losses/nextlat_head_grad_norm", float(nextlat_head_gn), global_step)
                writer.add_scalar("losses/nextlat_trunk_delivered", float(nextlat_trunk_deliv), global_step)
                writer.add_scalar("losses/nextlat_head_delivered", float(nextlat_head_deliv), global_step)
            writer.add_scalar("losses/nextlat_grad_norm", float(nextlat_gn) if args.separate_grad_clip else 0.0, global_step)
