# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT v10_fullbatch + DG-ABS, REWNORM (v4).
# v4 delta vs DG-ABS: normalize_reward=True (shared VectorRewardNorm) and critic
# support tightened to symlog +-5 (was +-9.9/symlog20k). RMS-normalized
# discounted returns sit at std ~7, so +-5 covers to +-74 (~10 sigma) with 2x
# the bin resolution where the mass lives; edge_mass will tell us if it binds.
# Tests whether the abs slowdown is a value-scale artifact (raw returns inflate
# as the policy beats its history, coarsening the fixed wide support).
# =====================================================================================
...
class Args:
    ...
    num_bins: int = 511
    v_min: float = -5.0   # v4: tightened to the normalized-return scale (was symlog20k +-9.9)
    v_max: float = 5.0    # covers normalized returns to +-74 (~10 sigma); watch target_edge_mass
    ...
    normalize_reward: bool = True   # v4: shared VectorRewardNorm ON (was raw)
