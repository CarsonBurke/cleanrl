# NEXTLAT v6: SCALE-ANCHORED PREDICTION LOSS.  [WRITTEN, NOT YET RUN]
# =====================================================================================
# v5 (trunk-heavy split budget) FAILED, and the autopsy is specific and reusable.
# Measured at 2M on HalfCheetah, sweeping ONLY the trunk's aux clip:
#     trunk clip   latent_batch_std   pred_loss   return
#     0.038 (=clip025 incumbent)  3.07    0.803    6550
#     0.25                        2.01    0.492    5173
#     1.00                        1.24    0.175    5015
# Every column is monotone and return runs the WRONG way. The tell is the normalized
# error pred_loss/std^2: 0.085 for the incumbent vs 0.11-0.12 for both v5 arms. Per unit
# of latent spread the trunk-heavy arms predict slightly WORSE -- so the 4.6x pred_loss
# reduction bought no predictive skill at all. It was bought by SHRINKING THE LATENT
# SCALE 2.5x.
# WHY THE TRUNK CAN DO THIS: ThinkTrunk applies RMSNorm BEFORE out_proj, so the emitted
# latent h is an unnormalized Linear output whose scale is a free parameter, and the aux
# target is the trunk's OWN output. SmoothL1 is not scale-invariant, so h -> c*h with
# c<1 lowers the loss quadratically for free. The trunk is the ONLY party to the aux that
# can cheat this way (the predictor can only predict; the heads only decode) -- which is
# exactly why handing IT the budget was the worst possible allocation.
# THE FIX: measure both sides in units of the target batch's own spread. sigma is a
# SCALAR (mean over dims of the per-dim cross-batch std) read off the already-detached
# target, so it contributes no gradient path and no per-dim reweighting -- a pure unit
# change, NOT a whitening/rotation. Under h -> c*h numerator and sigma scale together and
# the loss is exactly invariant, so shrinking pays nothing and the only way down is real
# prediction. This makes the trunk-budget knob meaningful for the first time.
# STATUS: not submitted. The v5 result only says trunk-heavy fails GIVEN the cheat; this
# file is what makes that question askable. Re-run the 0.038 / 1.0 trunk pair under it.
# WATCH: debug/nextlat_tgt_scale should now drift freely WITHOUT pulling pred_loss down.
# If pred_loss still falls in lockstep with it, the anchor leaked and this is wrong.
# =====================================================================================
#
# --- inherited v5 notes (its budget-split machinery is retained verbatim) ---
# =====================================================================================
# WHY. The aux clip is the only live knob (nextlat_coef is provably inert: the raw aux
# grad is 13.6-150 vs a clip of 0.1-0.25, so the clip ALWAYS binds and rescales the
# whole group back to the max-norm, dividing the coef straight out). Sweeping that one
# knob gave a NON-MONOTONIC response on HalfCheetah:
#     clip 0.10 (v1)      @1M 3114  @2M 5889  @3M 7518
#     clip 0.25 (clip025) @1M 3457  @2M 6550  @3M 7631   <- best
#     clip inf  (noclip)  @500k 881 vs ~1500  -> collapse
# The collapse mechanism is visible in the diagnostics and is specifically a HEAD
# problem, not a representation problem: approx_kl fell to 0.014 (vs 0.031) and entropy
# decayed 2x slower, i.e. the policy stopped moving. Adam normalizes per-parameter, so
# once the aux gradient on the ACTION HEADS dwarfs the policy gradient there, the update
# DIRECTION at the decoder is ~all aux and PPO gets a vote it cannot win.
#
# THE FIX. v1-v4 govern the whole aux with ONE number, so "let the representation be
# world-model shaped" and "let the aux rewrite the policy's decoder" are the same dial
# and the good half is capped by the bad half. v5 makes them DISJOINT clip groups --
# three clip_grad_norm_ calls on a PARTITION of v1's single list after ONE aux backward,
# each slice rescaled to its own max-norm, then all summed onto the shared params as
# before. Every param that got aux gradient still gets it; only the ceilings differ.
#
# SIZING THE GROUPS (the non-obvious part). A joint clip does NOT split evenly: it scales
# the whole vector by c/||g||, so each group's delivered budget is its SHARE of the raw
# norm. Per-group norms measured on a FRESH agent with random inputs are
#     trunk 0.290   pred 1.125   head 0.011   (quadrature 1.167)
# i.e. the dynamics MLP carries ~97% and the decoder ~1%. The ABSOLUTE scale here is an
# init artifact and does not transfer -- the clip025 run logs a union norm of 56-185, ~50x
# larger -- but only the RATIO enters the arithmetic below, and the scale cancels in it.
# What clip025 (joint 0.25) therefore delivered per group was
#     trunk 0.062   pred 0.242   head 0.0024
# so a "small-looking" head clip of 0.05 would have been 21x MORE head aux than the
# incumbent: a step TOWARD the failure mode, not away from it. Head and pred are pinned at
# clip025's delivered levels (0.0025 / 0.25) and ONLY the trunk is raised.
# CAVEAT, stated because it is the likeliest thing to misread later: the head pin is only
# a null change at init. The heads receive aux gradient EXCLUSIVELY through the KL term
# (pred_l compares h_hat to tgt_feat and never touches a head), and kl_loss climbs
# 0.006 -> 2.18 over the clip025 run while pred_loss only goes 0.83 -> 1.20. So the head's
# share of the union grows by orders of magnitude, and under a JOINT clip its delivered
# budget floated up with it -- which is precisely the drift the failure diagnosis blames.
# Pinning it is deliberate, but it is a SECOND, time-varying change vs the incumbent, and
# it is the most likely source of any LATE divergence from clip025. Do not reflexively
# attribute a post-3M gap to the trunk; check losses/nextlat_head_grad_norm first.
# DOSE-RESPONSE: two arms differing ONLY in nextlat_trunk_grad_clip --
#     0.25 = 4x clip025's trunk, still <= the actor's whole-group budget
#     1.00 = 16x clip025's trunk, 4x the actor's ENTIRE budget (world-model-dominated)
# Note the KL's Fisher weighting survives a small head budget: the head Jacobian reweights
# the LATENT gradient whether or not the heads themselves are updated, so the
# auto-curriculum is kept and only the decoder rewrite is dropped.
# RISK: the critic reads the same shared trunk, so a world-model-shaped trunk also moves
# value features (its own 0.25 is unchanged). Could cut either way; explained_variance
# is the probe.
# HYPOTHESIS: >= clip025 early and strictly better past 2M, because the 0.1->0.25 gain
# came from the trunk half of the budget and 0.25->inf lost on the head half; splitting
# lets us push past the joint optimum on the side that was actually paying. FALSIFIER:
# if both trunk arms track clip025, the joint clip was never the binding constraint and
# the aux uplift is saturated -- stop tuning the budget and change the TARGET instead.
# NEW INSTRUMENTATION: clip_grad_norm_ returns the PRE-clip norm, so
# losses/nextlat_{trunk,pred,head}_grad_norm give the raw per-group decomposition of the
# aux gradient we have never been able to see -- this run is informative even if it loses.
# =====================================================================================
#
...
class Args:
    ...
    # v5: the aux budget is SPLIT across three DISJOINT param groups so the
    # representation and the policy's decoder are governed independently (see header).
    nextlat_trunk_grad_clip: float = 1.0  # shared ThinkTrunk: 4x the actor's whole-group
                                          # 0.25 => the trunk is world-model-dominated
    nextlat_pred_grad_clip: float = 0.25  # latent dynamics MLP. Pinned at clip025's
                                          # delivered level. EXPECTED INERT for the same
                                          # reason nextlat_coef is: these params are
                                          # aux-only, so the clip always binds to a
                                          # CONSTANT magnitude and Adam's per-parameter
                                          # normalization divides it out. Pinned anyway so
                                          # a v5-vs-clip025 delta cannot be blamed on it.
    nextlat_head_grad_clip: float = 0.0025  # action heads: HELD AT THE clip025 LEVEL, not
                                          # raised -- see the budget arithmetic in the header
...
class Agent(nn.Module):
    ...
    # --- v5: the NEXTLAT gradient is clipped as THREE DISJOINT groups (see header).
    # Their union is exactly the single aux list v1 clipped as one group, so nothing that
    # received the aux gradient stops receiving it; only the per-group ceilings differ.

    def nextlat_trunk_parameters(self):
        # The representation this aux exists to shape. Gets the LARGE budget.
        trunk = self.trunk if self.share_backbone else self.actor_trunk
        return list(trunk.parameters())

    def nextlat_pred_parameters(self):
        # The latent dynamics MLP. Aux-only params (no other loss touches them).
        return list(self.nextlat_predictor.parameters())

    def nextlat_head_parameters(self):
        # The KL distillation's student path decodes through the action heads. Gets the
        # SMALL budget: the aux may reshape the features the policy reads, not the map
        # from features to actions.
        if self.actor_dist == "gaussian":
...
if __name__ == "__main__":
    ...
    nextlat_pred_params = agent.nextlat_pred_parameters()
    nextlat_head_params = agent.nextlat_head_parameters()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.nextlat:
                    h_hat = mb_actor_feat
                    pred_losses, kl_losses, tgt_scales = [], [], []
                    for i in range(1, args.nextlat_depth + 1):
                        ...
                        # v6 SCALE ANCHOR. Measure both sides in units of the target
                        # batch's own spread. sigma is a SCALAR (mean over dims of the
                        # per-dim cross-batch std) taken from the already-detached target,
                        # so it adds no gradient path and no per-dim reweighting -- it is
                        # a pure unit change, not a whitening. Under h -> c*h the trunk
                        # scales numerator and sigma alike, so the loss is INVARIANT and
                        # the shrink cheat pays exactly nothing.
                        sigma = tgt_feat.std(dim=0).mean().clamp_min(1e-3)
                        pred_l = F.smooth_l1_loss(
                            h_hat / sigma, tgt_feat / sigma, reduction="none"
                        ).mean(-1)
                        pred_losses.append((pred_l * mask_i).sum() / denom)
                        tgt_scales.append(sigma)
                    ...
                    nextlat_tgt_scale = torch.stack(tgt_scales).mean()
                ...
                if args.separate_grad_clip:
                    ...
                    if args.nextlat:
                        ...
                        # v5: ONE aux backward, THREE disjoint clip groups. The groups
                        # partition v1's single list, so each call rescales an independent
                        # slice of the same gradient vector to its own max-norm. Returned
                        # values are the PRE-clip norms => free decomposition of the raw
                        # aux gradient (trunk vs dynamics MLP vs decoder).
                        ...
                        nextlat_pred_gn = nn.utils.clip_grad_norm_(
                            nextlat_pred_params, args.nextlat_pred_grad_clip
                        )
                        nextlat_head_gn = nn.utils.clip_grad_norm_(
                            nextlat_head_params, args.nextlat_head_grad_clip
                        )
                        nextlat_grads = [
                            (p, p.grad.detach().clone())
                            for p in (nextlat_trunk_params + nextlat_pred_params + nextlat_head_params)
        ...
        if args.nextlat:
            ...
            # PRE-clip norms per group. Read them together: whichever group's raw norm
            # sits BELOW its clip has stopped being budget-limited, and the ratio
            # trunk:head is the aux gradient's own idea of where it wants to act.
            if args.separate_grad_clip:
                ...
                # The groups PARTITION v1's single list, so the quadrature of their norms
                # is exactly the union norm v1/clip025 logged under this tag -- keep
                # emitting it so v5 stays comparable to every prior nextlat run.
                union_gn = torch.sqrt(nextlat_trunk_gn ** 2 + nextlat_pred_gn ** 2 + nextlat_head_gn ** 2)
                writer.add_scalar("losses/nextlat_grad_norm", float(union_gn), global_step)
            else:
                writer.add_scalar("losses/nextlat_grad_norm", 0.0, global_step)
            writer.add_scalar("debug/latent_batch_std", latent_batch_std, global_step)
            # The scale the trunk was previously shrinking to cheat the loss. Under v6 it
            # should now be FREE to drift without lowering nextlat_pred_loss; if pred_loss
            # still falls while this falls, the anchor did not work.
            writer.add_scalar("debug/nextlat_tgt_scale", nextlat_tgt_scale.item(), global_step)
        if auto_alpha:
