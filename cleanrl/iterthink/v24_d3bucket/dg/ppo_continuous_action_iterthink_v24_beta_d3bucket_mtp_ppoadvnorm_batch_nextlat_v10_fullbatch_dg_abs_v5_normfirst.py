# PPO + IterThink v24 Beta + d3bucket MTP + PPO-ADVNORM(batch) + NEXTLAT v10_fullbatch + DG-ABS, NORM-FIRST (v5).
# v5 delta vs DG-ABS: swap the order -- standardize the RAW shaped advantage
# FIRST (fair PPO bar), gate-weight SECOND with NO re-standardization. v1's
# weight-then-norm rigs the bar by rare weights (pos-heavy mean flips
# common-good actions negative, blocking consolidation = the 3x slowdown).
# Here the gate keeps direction AND step-size control; at convergence weights
# go uniform and this is identical to PPO, same as v1.
# REWARD SETUP mirrors v4 (normalize_reward=True, support +-5) so v5-vs-v4
# isolates the norm order from the reward scale.
# =====================================================================================
...
#   p_abs = clip(exp(true log-density), 0, 1);  q = 1 - p_abs  (1 = rarest).
...
class Args:
    ...
    num_bins: int = 511
    v_min: float = -5.0   # v5: mirrors v4 normalized scale (was symlog20k +-9.9)
    v_max: float = 5.0
    ...
    normalize_reward: bool = True   # v5: shared VectorRewardNorm ON (mirrors v4)
    ...
    # Delight-gate advantage weighting (sole delta): sign-asymmetric rareness
    # gating of the shaped policy advantage before batch norm_adv. ABS version:
    # q = 1 - clip(exp(true log-density), 0, 1) (1 = rarest). w_pos =
    # pos_common + (pos_rare - pos_common)*q; w_neg = neg_common*(1-q).
    dg: bool = True
    dg_pos_common: float = 0.5
    dg_pos_rare: float = 1.0
    dg_neg_common: float = 0.5
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # v5 NORM-FIRST: standardize the RAW shaped advantage here (fair PPO
        # bar at the unweighted mean), then gate-weight below with NO
        # re-standardization, so common-good actions keep their positive sign
        # and the gate controls step size as well as direction.
        if args.norm_adv and args.norm_adv_scope == "batch":
            b_policy_adv = (b_policy_adv - b_policy_adv.mean()) / (b_policy_adv.std() + 1e-8)
            ...
            b_policy_adv = b_policy_adv / b_returns.std().clamp(min=args.ret_perc_floor)
        # DG-ABS: sign-asymmetric ABSOLUTE-density rareness gating AFTER the
        # norm (v5 order). q = 1 - clip(exp(true log-density), 0, 1), no batch
        # comparison: the gate quiets as the policy sharpens instead of forcing
        # contrast. The buffered logprob drops the constant rescale Jacobian
        # (cancels in the PPO ratio) so beta restores it from the action
        # bounds; gaussian stored logprob already includes tanh Jacobian.
        if args.dg:
            if args.actor_dist == "beta":
                ...
                lp_true = b_logprobs
            p_abs = lp_true.clamp(max=0.0).exp()  # (0,1]: 1 = fully common
            q_dg = 1.0 - p_abs  # 1 = rarest (vanishing density)
            pos_w = args.dg_pos_common + (args.dg_pos_rare - args.dg_pos_common) * q_dg
            neg_w = args.dg_neg_common * (1.0 - q_dg)
            dg_w = torch.where(b_policy_adv >= 0, pos_w, neg_w)
            b_policy_adv_normed = b_policy_adv * dg_w
        else:
            dg_w = torch.ones_like(b_policy_adv)
            b_policy_adv_normed = b_policy_adv
        ...
        pz = (b_policy_adv_normed - b_policy_adv_normed.mean()) / (b_policy_adv_normed.std() + 1e-8)
