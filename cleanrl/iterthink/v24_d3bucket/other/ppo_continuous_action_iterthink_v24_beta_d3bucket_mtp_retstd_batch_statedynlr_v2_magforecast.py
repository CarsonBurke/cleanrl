# STATEDYNLR v2 MAGFORECAST — per-perceptron state-dependent LR, magnitude-forecast target.
# =====================================================================================
# v1 POSTMORTEM (run 3194, cancelled @1.15M): the v1 target was the per-unit
# gradient SIGN SNR rho = m^2/(v+eps). Measured reality: trunk-unit policy
# gradients fully decorrelate between minibatches (rho -> 3.8e-7), so the
# controller regressed onto a constant zero, alpha pinned at the e^-c floor
# UNIFORMLY (alpha_std 3.7e-4), and with the actor clip binding (frac=1.0) the
# mechanism was inert at best, ~50% behind the retstd_batch reference at 500k/1M
# (598 vs 1222, 1406 vs 2430). Lesson: in this PPO the informative, non-degenerate
# per-unit quantity is gradient MAGNITUDE (which units carry the clipped budget),
# not gradient sign consistency.
#
# v2 CHANGE: the regression target is now a SCALE-INVARIANT per-unit engagement
# forecast. Stats per unit (EMAs, beta): signed batch-mean grad m (INPUT feature
# only: m/sqrt(v)), batch mean-square v, and running mean/second-moment of
# z_j := log rms_j (rms_j = sqrt(v_j)). After each optimizer step the target is
#     y_j = sigmoid( (z_j - mu_j) / sigma_j )     in (0,1)
# i.e. "will this unit be hotter than its own baseline in the state it is in
# now?" -- always variance left to explain, so no constant-collapse solution.
# alpha_ij(s) = exp(c*(2*rhat_ij-1)) in [e^-c, e^c] as before: predicted-hot
# units get a bigger share of the (clip-normalized) policy-gradient budget in
# that state; predicted-cold units are protected. Same self-supervised online
# MSE, no meta-gradient, alphas detached in the main backward.
#
# Idea (unchanged from v1): the effective learning rate of a hidden unit should
# be a function of the CURRENT state, not just parameter history -- conditional
# metaplasticity. Per (sample i, unit j) of each trunk site, applied elementwise
# to the POLICY gradient during the actor backward (critic backward and heads
# untouched). Stats are captured before the site's own scaling; modulation at
# upstream sites leaks into deeper sites' grads within one backward, a bias
# bounded by alpha in [e^-c, e^c].
#
# Sites: trunk.entry, each ThinkBlock output, trunk.out_proj (all (B,H=64)).
# Warmup (100 pg updates): hooks record stats but return the gradient unchanged.
#
# HYPOTHESIS: state-conditioned budget allocation onto predicted-engaged units
# beats the flat-LR reference. Falsifiable: if debug/sdlr_alpha_std ~ 0 again
# (uniform alpha) or alpha_mean rails at a bound, the state signal carries no
# engagement information and this line is dead.
# PASS: >= retstd_batch reference (6428 end, 2430 @1M, 1222 @500k) at matched steps.
# FAIL: >5% below reference at two consecutive checkpoints, or uniform-alpha
#       collapse (debug/sdlr_alpha_std < 1e-2 sustained).
# WATCH: debug/sdlr_alpha_mean/_std, debug/sdlr_target_mean, losses/sdlr_ctrl_loss
#        (must NOT sit at ~0 while alpha rails), debug/sdlr_cons_mean,
#        debug/sdlr_alpha_site_{entry,block0..2,out_proj}, debug/actor_clip_frac.
#
# Base (ent_alpha forced to 0 => bit-identical to the retstd_batch reference
# recipe): PPO + IterThink v24 Beta + Dreamer3-bucket HL-Gauss MTP critic +
# MB PERCNORM v2 + v21 shared backbone / dual-backward decoupled clip.
# =====================================================================================
...
class Args:
    ...
    norm_adv_scope: str = "batch_retstd"   # retstd_batch reference recipe (was "minibatch")
...
class SDLRController(nn.Module):
    """Per-perceptron state-dependent learning-rate modulator (v2 MAGFORECAST).

    Self-supervised conditional metaplasticity (NO meta-gradient, NO outer loss).
    Each hooked trunk site exposes H hidden units ("perceptrons"). A tiny shared
    net f_phi reads per-(sample, unit) features -- the unit's activation (state),
    its running grad-sign consistency m/sqrt(v) (input only; v1 showed trunk PG
    sign-SNR is ~0, so it is NOT the target), its z-scored log gradient RMS
    (engagement relative to its own baseline), and a learned site embedding.
    After each optimizer step the regression target is the realized post-update
        y_j = sigmoid( (log rms_j - mu_j) / sigma_j )    in (0,1)
    -- a SCALE-INVARIANT engagement forecast: will this unit be hotter than its
    own baseline? mu/sigma are EMAs of the per-unit log-rms. Always variance
    left to explain => no constant-collapse solution (v1's failure mode).
    Stats are captured in the same backward hook, BEFORE the site's own scaling;
    modulation applied at upstream sites does propagate into deeper sites'
    gradients within one backward, a bias bounded by alpha in [e^-c, e^c].
    The prediction sets a bounded multiplicative learning rate
        alpha_ij(s) = exp(c*(2*sigmoid(logit_ij)-1))   in [e^-c, e^c],
    applied elementwise to the POLICY gradient at that unit during the actor
    backward only. L_PR = MSE: predicted-hot units get more of the clip-normalized
    budget in that state; predicted-cold units are protected.
    """
    ...
    def __init__(self, site_modules, H, c=1.0, ema_beta=0.98, lr=1e-3, warmup=100):
        ...
        self._pending = []         # [(site, signed batch-mean grad, batch mean-square)] per backward
        self._alphas = {}          # site -> detached (B,H) modulation
        self._logits = []          # [(site, logits with graph)] for the controller loss
    ...
    def _make_bwd(self, name):
        def hook(g):
            if not self.pg_pass:
                return None
            # stats measure the gradient AT THIS SITE before its own scaling.
            # KNOWN COUPLING: modulation applied at an upstream site (e.g.
            # out_proj, whose hook fires first) propagates into the gradients
            # seen by deeper sites within the SAME backward, so their stats are
            # not perfectly raw; the bias is bounded by alpha in [e^-c, e^c].
            gd = g.detach()
            self._pending.append((name, gd.mean(dim=0), gd.pow(2).mean(dim=0)))
    ...
    def _targets(self):
        """Realized post-update engagement targets y_j in (0,1), scale-invariant."""
        out = []
        for name in self.site_names:
            lr = (getattr(self, f"v_{name}") + 1e-12).sqrt().log()
            mu = getattr(self, f"mu_{name}")
            sig = (getattr(self, f"m2_{name}") - mu.pow(2)).clamp_min(1e-8).sqrt().clamp_min(0.25)
            out.append(torch.sigmoid((lr - mu) / sig))
        return torch.stack(out)                              # (S, H)
    ...
    def ingest(self):
        """After optimizer.step(): fold this backward's grad stats into the EMAs
        (including the log-rms running stats), regress the stashed predictions onto
        the realized post-update engagement targets, step the controller, clear
        per-minibatch state. Returns the control loss (or None)."""
        ctrl_loss = None
        for name, gmean, gms in self._pending:
            ...
            # log-rms running stats (targets are z-scored against these)
        ...
        if self._logits and self.updates >= self.warmup:
            tgt = self._targets()                                # (S, H)
            pred = torch.stack([lg for _, lg in self._logits])   # (S, B, H)
            ctrl_loss = F.mse_loss(torch.sigmoid(pred), tgt.unsqueeze(1).expand_as(pred))
    ...
    def diagnostics(self):
        # cons (sign consistency) and target spread are available from the EMAs
        # even during warmup (no alphas yet).
        cons = [
            (getattr(self, f"m_{n}") / (getattr(self, f"v_{n}") + 1e-12).sqrt()).abs().mean().item()
            for n in self.site_names
        ]
        out = {
            "debug/sdlr_cons_mean": sum(cons) / len(cons),
            "debug/sdlr_target_mean": self._targets().mean().item(),
            "debug/sdlr_target_std": self._targets().std().item(),
        }
