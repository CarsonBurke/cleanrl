...
# =====================================================================================
# Idea: the effective learning rate of a hidden unit should be a function of the
# CURRENT state, not just of the parameter's gradient history. A unit that is
# currently processing predictable structure can afford a big step; a unit in a
# high-noise regime should freeze. Conditional metaplasticity:
#     theta <- theta - alpha_ij(s) * g_ij,   alpha_ij(s) = exp(c*(2*rhat_ij-1)) in [e^-c, e^c]
# per (sample i, unit j) of each trunk site, applied elementwise to the POLICY
# gradient during the actor backward (critic backward and heads untouched; the
# decoupled actor clip then renormalizes the total budget, so SDLR REDISTRIBUTES
# gradient across units/states instead of rescaling the step globally).
#
# NO META-LEARNING. The controller f_phi (shared across all units/sites; inputs:
# the unit's activation as the state feature, its running grad-sign consistency
# m/sqrt(v), its grad magnitude log1p(sqrt(v)) -- delta-bar-delta-style history
# -- and a learned site embedding) predicts the unit's gradient SNR
#     rho_j = m_j^2 / (v_j + eps)   in (0,1]   (Cauchy-Schwarz: m^2 <= v)
# where m is the EMA (beta=0.98) of the per-unit SIGNED batch-mean gradient and v
# the EMA of the batch mean-square, captured in the same backward hook before the
# site's own scaling (modulation at upstream sites does leak into deeper sites'
# grads within one backward; that bias is bounded by alpha in [e^-c, e^c]).
# f_phi trains ONLINE from the same stream: after each optimizer step the realized
# post-update rho becomes the regression
# target,  L_PR = (sigmoid(logit) - rho_next)^2  -- the IDBD trick (differentiate
# the ordinary loss wrt a learning parameter) replaced by plain self-supervision.
# No outer loss, no unrolling, no second-order terms: the alphas are detached in
# the main backward. High predicted predictability => bigger step in that state;
# low => shrink. Warmup (100 pg updates): hooks record stats but return the
# gradient unchanged while the EMAs converge.
#
# Sites: trunk.entry, each ThinkBlock output, trunk.out_proj (all (B,H=64)).
# Controller cost per minibatch: (B*H*n_sites=328k) rows through an 11->32->1 MLP.
#
# HYPOTHESIS: state-conditioned plasticity beats the flat-LR reference by letting
# the policy spend its clipped gradient budget on units whose current-state
# gradient flow is predictable, and protect noisy units. Falsifiable: if
# debug/sdlr_alpha collapses to ~1 everywhere (or variance explodes without
# score change), the state signal carries no plasticity information here.
# PASS: >= retstd_batch reference (6364 @7.2M) at matched steps.
# FAIL: >5% below reference at two consecutive checkpoints.
# WATCH: debug/sdlr_alpha_mean/_std (modulation magnitude; mean~1 healthy),
#        debug/sdlr_rho_mean (realized gradient SNR), losses/sdlr_ctrl_loss,
#        debug/sdlr_alpha_site_{entry,block0..2,out_proj}, debug/actor_clip_frac
#        (a UNIFORM alpha is cancelled exactly by the actor norm clip).
#
...
class SDLRController(nn.Module):
    """Per-perceptron state-dependent learning-rate modulator for the PPO trunk.
    ...
    its running grad-sign consistency m/sqrt(v) and magnitude log1p(sqrt(v))
    (delta-bar-delta-style history), and a learned site embedding -- and predicts
    the unit's gradient SNR rho = m^2/(v+eps) in (0,1] -- m is the EMA of the
    per-unit SIGNED batch-mean gradient, v the EMA of the batch mean-square, so
    rho is true sign-consistency (Cauchy-Schwarz: m^2 <= v; sign flips -> rho~0).
    ...
    backward only. After each optimizer step the realized post-update rho is the
    regression target (L_PR = MSE): high predicted predictability => bigger step.
    """
    ...
    def __init__(self, site_modules, H, c=1.0, ema_beta=0.98, lr=1e-3, warmup=100):
        ...
        for name in self.site_names:
            self.register_buffer(f"m_{name}", torch.zeros(H))
            self.register_buffer(f"v_{name}", torch.zeros(H))
    ...
    def _make_bwd(self, name):
        def hook(g):
            ...
            # Signed batch-mean (drives m, sign consistency) + batch mean-square
            # (drives v): rho = m^2/(v+eps) is then a true gradient SNR in
            # (0,1] by Cauchy-Schwarz -- sign-flipping units score rho ~ 0.
            gd = g.detach()
    ...
    def prepare(self):
        ...
        for idx, name in enumerate(self.site_names):
            ...
            cons = (getattr(self, f"m_{name}") / (sd + 1e-8)).clamp(-1, 1)   # (H,)
            mag = torch.log1p(sd).clamp(max=10.0)                # (H,)
            rows = torch.cat(
                [
    ...
    def ingest(self):
        """After optimizer.step(): fold this backward's raw-grad stats into the EMAs,
        regress the stashed predictions onto the realized post-update rho, step the
        controller, clear per-minibatch state. Returns the control loss (or None)."""
        ctrl_loss = None
        for name, gmean, gms in self._pending:
            m, v = getattr(self, f"m_{name}"), getattr(self, f"v_{name}")
            setattr(self, f"m_{name}", self.beta * m + (1.0 - self.beta) * gmean)
            setattr(self, f"v_{name}", self.beta * v + (1.0 - self.beta) * gms)
        if self._logits and self.updates >= self.warmup:
            targets = []
            for name, _ in self._logits:
                m, v = getattr(self, f"m_{name}"), getattr(self, f"v_{name}")
                targets.append(m.pow(2) / (v + 1e-8))        # realized rho in (0,1]
            tgt = torch.stack(targets)                           # (S, H)
    ...
    def diagnostics(self):
        # rho is available from the EMAs even during warmup (no alphas yet).
        rhos = [
            (getattr(self, f"m_{n}").pow(2) / (getattr(self, f"v_{n}") + 1e-8)).mean().item()
            for n in self.site_names
        ]
        out = {"debug/sdlr_rho_mean": sum(rhos) / len(rhos)}
