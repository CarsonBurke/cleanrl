# STATEDYNLR v3 METASTEP — state-dependent LR trained by DIRECT one-step meta-gradient.
# =====================================================================================
# DESIGN FORK (user-directed): v2 predicts the FUTURE (engagement forecast); v3
# trains the controller on the OPTIMUM criterion -- did the step it chose actually
# improve the next policy loss -- without unrolling the optimizer.
#
# MECHANISM (hypergradient-ascent on phi, no regression target):
# Under a linear model of the post-step loss, the per-unit evidence that the
# applied modulation alpha helped is
#     e_j = <g_next_j, u_t,j>  ~  r_j(t+1) * mm_j(t)
# where mm_j(t) = mean_i(alpha_ij * g_ij) is the per-unit MODULATED batch-mean
# gradient that entered the update at step t (captured in the hook, pre-clip),
# and r_j(t+1) is the next backward's per-unit RAW batch-mean gradient. e_j > 0:
# the step direction proved productive => a LARGER alpha would have helped more;
# e_j < 0: the step hurt => shrink. phi is updated by gradient descent on
#     L_meta = -mean( en_j * logit_ij(t) ),   en = clamp(e / rms(e), +-3)
# i.e. exactly the dL_{t+1}/dphi ~= -<g_{t+1}, u_t> * dalpha/dphi term from the
# one-step unroll, with the <.,.> collapsed to per-unit scalar evidence and
# RMS-normalized so the signal is scale-free. Inputs to f_phi stay purely
# state features (activation, sign consistency, z-scored engagement, site emb).
# No forecast, no regression onto gradient statistics: the ONLY supervision is
# realized step outcome.
#
# KNOWN RISKS (from v1's postmortem): if per-unit PG directions decorrelate
# between minibatches, e_j is pure noise and en ~ symmetric -- the meta-gradient
# then random-walks the logits. The RMS normalization keeps it zero-mean instead
# of collapsing to a rail; watch debug/sdlr_evidence_std and the FAIL rule.
#
# v1 POSTMORTEM (kept for context): sign-SNR regression target collapsed to a
# constant (rho -> 4e-7), alpha railed uniformly, run was ~50% behind reference
# at 500k/1M and was cancelled at 1.15M.
#
# Idea (as in v1/v2): per (sample i, unit j) of each trunk site, alpha_ij(s) =
# exp(c*(2*sigmoid(logit)-1)) in [e^-c, e^c] multiplies the POLICY gradient
# elementwise during the actor backward (critic backward and heads untouched).
# Stats captured before the site's own scaling; upstream modulation leaks into
# deeper sites' grads within one backward, bounded by alpha in [e^-c, e^c].
# Sites: trunk.entry, each ThinkBlock output, trunk.out_proj (all (B,H=64)).
# Warmup (100 pg updates): hooks record stats, no modulation, no meta-update.
#
# HYPOTHESIS: outcome-trained state-conditioned plasticity beats both the flat-LR
# reference and v2's forecast objective.
...
#       bound with evidence signal ~0 (debug/sdlr_evidence_std < 1e-3 sustained).
# WATCH: debug/sdlr_evidence_mean/_std (is the outcome signal alive?),
#        losses/sdlr_meta_loss, debug/sdlr_alpha_mean/_std,
