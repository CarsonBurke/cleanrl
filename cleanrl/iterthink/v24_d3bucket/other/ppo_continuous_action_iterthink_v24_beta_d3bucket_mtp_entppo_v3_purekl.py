# ENT-PPO v3 — exact KL as a SECOND trust region beside the clip (adaptive β).
# =====================================================================================
# v3 was built to test "the exact KL REPLACES the ratio clip". That hypothesis is
# FALSIFIED by its own run (entppo_v3_purekl_a03_b03init, --no-pg-clip). Recorded here
# because the failure is structural, not a tuning miss, and it explains what the clip is
# actually for.
#
# WHAT WAS MEASURED (51 iterations, vs v2 over 244):
#     losses/approx_kl        p95 1.72   max 7.92    (v2: p95 0.036  max 0.055)
#     losses/actor_grad_norm  p95  374   max  962    (v2: p95 5.78   max 9.67)
#     losses/exact_kl                    max 0.122   <- the KL never noticed
#     kl_breaker_trips        25 of 51 iterations (49%)
#     debug/kl_beta           0.3 -> 4.8 while realized KL ROSE 0.037 -> 0.046
# At global_step 753664 a single iteration had exact_kl = 0.034 (on target) and
# approx_kl = 7.92 with actor_grad_norm = 962 SIMULTANEOUSLY.
#
# WHY, PRECISELY. The two objects constrain different things and neither implies the other:
#   * KL(π_new‖π_old) is a MEAN of a PARAMETRIC divergence. It bounds how far the
#     DISTRIBUTION moved.
#   * The clip bounds the per-SAMPLE importance ratio. It bounds the VARIANCE of the IS
#     estimator on the batch actually being reused.
# A heavy-tailed ratio distribution has both a small mean KL and an enormous max ratio.
# Here the tail is concrete: Beta z-replay stores z clamped only to SAMPLE_EPS = 1e-6, so
# one near-boundary dimension moves logratio by up to ~13.8 nats, ×6 dims, while the mean
# analytic KL barely registers it. The clip was never a crude proxy for the KL; it is the
# per-sample IS bound that the KL structurally cannot supply. They are COMPLEMENTARY.
#
# WHAT ALSO BROKE, AND IS FIXED HERE (both were mechanism defects, not hypothesis evidence):
#   1. The breaker checked once per EPOCH, i.e. after 32 further unclipped updates, and only
#      on the last minibatch's approx_kl. It now checks per MINIBATCH. Its cumulative
#      counter also made the "trips ≈ 0" criterion unevaluable -> debug/kl_breaker_tripped
#      is now a per-iteration 0/1.
#   2. β had NO authority over step size. actor_grad_norm exceeds actor_grad_clip=0.25 in
#      100.0% of iterations across all five runs measured (medians 1.2-3.8), so clipping the
#      SUM of (PG + β·KL) to a fixed norm let β only ROTATE a constant-length step. That is
#      why β railed 16x while drift rose. --kl-grad-group (default on) clips the objective
#      and the restraint in separate groups and sums WITHOUT renormalizing, so β changes
#      magnitude as its control law assumes.
#   Also: debug/ratio_max is now logged (the reference logs max_importance_weight at
#   PPO_hypergrid.py:329,338; this port had no tail instrument at all), the β adaptation
#   reads max(epoch_kls) rather than the last executed epoch (whose horizon varies with the
#   breaker, so it understated drift exactly on runaway iterations), debug/kl_beta logs the β
#   actually used rather than the post-adaptation value, and `--kl-beta 0` no longer inverts
#   (0 is a fixed point only of the increase branch, so low drift used to raise β off zero).
#
# DEFAULTS THEREFORE: pg_clip=True, target_kl=0.03 (v2's leash). v3-as-shipped is v2 plus a
# corrected adaptive β and the missing diagnostics -- the KL as a distributional constraint
# ALONGSIDE the clip's per-sample one.
# ABLATIONS: --pg-clip --no-kl-adapt --kl-beta 0.3 --target-kl 0.03 --no-kl-grad-group
#            restores v2 exactly. --no-pg-clip reproduces the falsified pure-KL arm.
# PASS: ≥ v2's 10269 @8M (≥ 5443 @2M / 8207 @4M) with debug/ratio_max bounded and
# kl_breaker_tripped rare. FAIL: β railing to kl_beta_max again, or no gain over v2, in
# which case the corrected β control adds nothing beyond v2's fixed 0.3.
#
# ALIGNMENT WITH THE REFERENCE TRUST REGION (read the config, not the paper prose:
# ent-ppo/gfnx/baselines/PPO_hypergrid.py + configs/PPO_hypergrid.yaml). Theirs is:
#   clip_eps=0.2 KEPT, KL coefficient FIXED at exactly 1, advantages RAW (unnormalized),
#   NO KL early stop, ppo_policy_epochs=4, and optax.adam with NO gradient clipping.
# The clip-kept part is now corroborated by our own falsification above. The only meaningful
# cross-comparison of the coefficient is dimensionless: their 1 multiplies advantages in
# NATS (reward = log R + log P_B − log P_F), so in normalized-advantage units it is
# ~1/std(Â_nats), i.e. order 1. Against that bar:
#   v1  α/S ≈ 0.012 (measured median 0.0077) -> ~2 orders too weak; WHY it was inert.
#   v2  β = 0.3 fixed                        -> within ~3x, and it worked (10269 @8M).
#   v3  β adaptive; in the pure-KL arm it passed order 1 and kept climbing to 4.8, but that
#       arm had no step-size authority, so it is not evidence about the right scale.
# Their no-grad-clipping is what let a fixed coefficient work: with no norm budget the
# penalty never competes with the objective. Our clip_grad_norm_ is why kl_grad_group is
# needed to reproduce that property.
#
...
class Args:
    ...
    kl_grad_group: bool = True
    kl_grad_clip: float = 1.0
    kl_adapt: bool = True            # PPO-penalty rule on the EXACT KL (Schulman 2017 §4)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # CIRCUIT BREAKER (not the trust region): abort if drift ran away before β
                # could react. Checked PER MINIBATCH -- an epoch-level check lets 32 further
                # updates land first, which is exactly how approx_kl reached 7.9 in the
                # --no-pg-clip run while the mean exact KL still read 0.034.
