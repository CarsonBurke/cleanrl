# PPO + IterThink v54 (v24 Beta + stratified role-memory loop-bank). From v24.
#
...
#     parameter shapes.
#   - Actor and critic get separate memory/read token streams to reduce direct
#     role-token gradient leakage, while sharing observation embeddings and
#     recurrent block weights.
#   - The ledger readout is stratified and role-local: actor and critic have
#     separate final norm/projection matrices over only their role read tokens
#     across loop depth. Obs/memory tokens still shape the read tokens through
#     attention, but do not directly dominate the final projection.
#
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, args):
        ...
        self.actor_memory_tokens = nn.Parameter(torch.empty(self.num_memory_tokens, H))
        self.critic_memory_tokens = nn.Parameter(torch.empty(self.num_memory_tokens, H))
        ...
        nn.init.normal_(self.actor_memory_tokens, std=0.02)
        nn.init.normal_(self.critic_memory_tokens, std=0.02)
        ...
        bank_tokens = in_dim + self.num_memory_tokens + self.num_read_tokens
        self.read_start = bank_tokens - self.num_read_tokens
    ...
    def _initial_bank(self, x, role):
        B = x.shape[0]
        x0 = self.entry(x)
        if role == "actor":
            role_idx = 0
            memory = self.actor_memory_tokens
            read = self.actor_read_token
        elif role == "critic":
            role_idx = 1
            memory = self.critic_memory_tokens
            read = self.critic_read_token
        else:
            raise ValueError(f"unknown role {role}")
        role_bias = self.role_embed[role_idx].view(1, 1, -1)
        obs_tokens = (
        ...
        memory_tokens = (
            memory.unsqueeze(0).expand(B, -1, -1)
            + self.type_embed[1].view(1, 1, -1)
            + role_bias
            + x0.unsqueeze(1)
        )
        read_tokens = (
            read.unsqueeze(0).expand(B, -1, -1)
            + self.type_embed[2].view(1, 1, -1)
            + role_bias
            + x0.unsqueeze(1)
        )
        return torch.cat([obs_tokens, memory_tokens, read_tokens], dim=1)

    def _run_stream(self, initial, role):
        current = initial
        ledger = [current]
        ...
        flat = torch.cat([
            bank[:, self.read_start:].reshape(bank.shape[0], -1)
            for bank in ledger
        ], dim=-1)
        if role == "actor":
    ...
    def forward(self, x):
        ...
        with ctx:
            actor_feat = self._run_stream(self._initial_bank(x, "actor"), "actor")
            critic_feat = self._run_stream(self._initial_bank(x, "critic"), "critic")
