# PPO + IterThink v51 (v24 Beta + action-token U-Net transformer trunk). From v24.
#
...
#   - Replace the dense/MoE ThinkTrunk with pre-norm token self-attention blocks.
#   - Use per-observation affine tokens, latent scratch tokens, actor/critic tokens,
#     and one action token per action dimension from layer zero.
#   - Read Beta/Gaussian action parameters from per-action tokens via shared scalar
#     heads; read value logits from the critic token.
#   - Use ReLUSquared FFNs, QK norm, BF16 autocast on CUDA, PyTorch flash SDPA, and
#     U-Net-style encoder-to-decoder residual skip mixing.
#
# This file intentionally does NOT use v24.5's SAC-Beta shifted-log concentrations;
# that family significantly underperformed the original v24 Beta reference.
#
# --- inherited v24 notes ---
#
...
class Args:
    ...
    hidden: int = 64
    k_blocks: int = 3               # encoder depth; decoder has the same depth.
    n_experts: int = 16             # legacy v24 field, unused by the transformer trunk.
    transformer_heads: int = 4
    transformer_latents: int = 4
    transformer_ffn_mult: int = 7    # near parameter-match to v24's dense + 16-expert trunk.
...
class PerActionLinear(nn.Module):
    """One scalar linear readout per action token, initialized like Linear(H, act_dim)."""

    def __init__(self, H, act_dim, std=1.0, bias_const=0.0):
        super().__init__()
        self.weight = nn.Parameter(torch.empty(act_dim, H))
        self.bias = nn.Parameter(torch.empty(act_dim))
        nn.init.orthogonal_(self.weight, std)
        nn.init.constant_(self.bias, bias_const)
...
class ThinkTrunk(nn.Module):
    ...
    def actor_role_parameters(self):
        return [self.actor_token, self.action_tokens]

    def critic_role_parameters(self):
        return [self.critic_token]

    def shared_parameters(self):
        role_ids = {id(self.actor_token), id(self.action_tokens), id(self.critic_token)}
        return [p for p in self.parameters() if id(p) not in role_ids]

    def _run_unet(self, tokens):
        skips = []
    ...
    def forward(self, x):
        ...
        with ctx:
            ...
            actor_tokens = self._run_unet(torch.cat([latents, actor, actions, obs_tokens], dim=1))
            critic_tokens = self._run_unet(torch.cat([latents, critic, obs_tokens], dim=1))
            action_start = self.latent_tokens.shape[0] + 1
            action_end = action_start + self.act_dim
            critic_idx = self.latent_tokens.shape[0]
            actor_action_feats = self.actor_out_norm(actor_tokens[:, action_start:action_end])
            critic_feat = self.critic_out_norm(critic_tokens[:, critic_idx])
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if self.actor_dist == "gaussian":
            ...
            self.actor_head = PerActionLinear(H, act_dim, std=0.01)
            self.actor_logvar_head = PerActionLinear(H, act_dim, std=0.01)
            ...
            self.actor_alpha_head = PerActionLinear(H, act_dim, std=0.01)
            self.actor_beta_head = PerActionLinear(H, act_dim, std=0.01)
    ...
    def actor_parameters(self):
        ...
        trunk_params = trunk.shared_parameters() + trunk.actor_role_parameters()
        if self.actor_dist == "gaussian":
        ...
        return trunk_params + heads

    def critic_parameters(self):
        # Params receiving the VALUE gradient (incl. the shared trunk).
        trunk = self.trunk if self.share_backbone else self.critic_trunk
        return trunk.shared_parameters() + trunk.critic_role_parameters() + list(self.critic_head.parameters())
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    for p in agent.parameters():
                        if p.grad is not None and id(p) not in critic_param_ids:
                            p.grad = None
                    ...
                    for p in agent.parameters():
                        if p.grad is not None and id(p) not in actor_param_ids:
                            p.grad = None
