# PPO + IterThink v69 (v56 + Beta concentration leash). From v24.
#
# Ablation base: HalfCheetah-v4__iterthink_v24_beta_s1__1__1780040819 / v24_dist.
# Preserve the strong v24 PPO loop, Dreamer-style unimodal Beta actor
# (alpha,beta = 1 + softplus(head)), C51 critic, rankgauss shaping, clip-higher,
# target-KL leash, shared backbone, and decoupled actor/critic gradient clipping.
#
# Architectural hypothesis:
#   - v24's cat(feats) works because it keeps a lossless widening ledger, while
#     fixed-width residuals overwrite useful prior-layer information.
#   - v51's token transformer regressed because it lost both v24's dense numeric
#     anchor and the IterThink loop/ledger. This trunk restores both.
#   - Each loop reuses the SAME transformer block. The ledger grows in token
#     length, not channel width, so every current obs/memory token can attend to
#     all previous loop-bank tokens plus the current bank without changing block
#     parameter shapes.
#   - The recurrent loop bank contains only observation tokens plus shared,
#     non-stratified memory tokens. These are the tokens that carry information
#     forward through loop depth.
#   - Actor and critic use separate CLS/read tokens only after the recurrent loop:
#     the role CLS tokens independently query each loop-prefix ledger for readout
#     traces, but are never appended to the future memory ledger and do not carry
#     a recurrent role-specific reader state across depths.
#   - Final projections are role-specific and consume only actor/critic CLS traces.
#   - v69 keeps the v56 architecture unchanged. The 8M v56 rerun learns faster
#     than v24 but over-sharpens late (entropy ~-11.6, concentration ~50), causing
#     final variance/regression. The entropy floor only engages very late, so add a
#     weak Beta-only concentration leash that activates around the observed plateau
#     region and directly limits distribution curvature / PPO ratio sensitivity.
#
# No RoPE: MuJoCo observation coordinates are named slots, not a translation-
# invariant sequence. Use learned coordinate/type embeddings and a dense x0 anchor.
#
# --- inherited v24 notes ---
#
...
class Args:
    ...
    ent_coef: float = 0.0
    beta_entropy_floor_per_dim: float = -1.75  # HalfCheetah act_dim=6 => total floor -10.5; inactive until late v56 oversharpening
    beta_entropy_floor_coef: float = 0.005     # summed per-dim squared hinge; gradient scale is independent of act_dim
    beta_concentration_cap: float = 35.0       # leash alpha+beta per action dim near v63's good final regime
    beta_concentration_coef: float = 0.02      # log-space squared hinge; actor-only
    ...
    hidden: int = 64
    k_blocks: int = 3
    loop_memory_tokens: int = 8
    transformer_heads: int = 4
    transformer_ffn_mult: int = 4
    transformer_resid_init: float = 0.10
    transformer_q_gain_init: float = 1.5
...
def init_linear_(module, init_scale=1.0):
    nn.init.orthogonal_(module.weight, init_scale)
...
class LoopBankBlock(nn.Module):
    """Shared cross-attention block: current loop tokens query the append-only ledger."""

    def __init__(self, H, num_heads, ffn_mult, resid_init, q_gain_init):
        super().__init__()
        assert H % num_heads == 0
        self.num_heads = num_heads
        self.head_dim = H // num_heads
        self.current_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.context_norm = nn.RMSNorm(H, elementwise_affine=False)
        ...
        self.q_gain = nn.Parameter(torch.full((num_heads, 1, 1), q_gain_init))
    ...
    def forward(self, current, context, initial):
        B, S, H = current.shape
        C = context.shape[1]
        g = torch.sigmoid(self.resid_gate).view(1, 1, H)
        x_in = g * current + (1.0 - g) * initial

        q_in = self.current_norm(x_in)
        kv_in = self.context_norm(context)
        q = self.wq(q_in).view(B, S, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.wk(kv_in).view(B, C, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.wv(context).view(B, C, self.num_heads, self.head_dim).transpose(1, 2)
        q = self.q_norm(q) * self.q_gain.view(1, self.num_heads, 1, 1)
        k = self.k_norm(k)
        attn = flash_attention(q, k, v).transpose(1, 2).reshape(B, S, H)

        ffn_in = self.ffn_norm(x_in)
        ffn = self.ff2(torch.relu(self.ff1(ffn_in)).pow(2))
        return x_in + self.attn_scale.view(1, 1, H) * self.wo(attn) + self.ffn_scale.view(1, 1, H) * ffn


class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, args):
        super().__init__()
        self.in_dim = in_dim
        self.H = H
        self.K = K
        self.num_memory_tokens = args.loop_memory_tokens
        self.num_read_tokens = 1
        ...
        self.obs_coord_embed = nn.Parameter(torch.empty(in_dim, H))
        self.shared_memory_tokens = nn.Parameter(torch.empty(self.num_memory_tokens, H))
        self.actor_read_token = nn.Parameter(torch.empty(self.num_read_tokens, H))
        self.critic_read_token = nn.Parameter(torch.empty(self.num_read_tokens, H))
        self.type_embed = nn.Parameter(torch.empty(3, H))
        self.role_embed = nn.Parameter(torch.empty(2, H))
        nn.init.normal_(self.obs_embed_w, std=0.02)
        nn.init.normal_(self.obs_embed_b, std=0.02)
        nn.init.normal_(self.obs_coord_embed, std=0.02)
        nn.init.normal_(self.shared_memory_tokens, std=0.02)
        nn.init.normal_(self.actor_read_token, std=0.02)
        nn.init.normal_(self.critic_read_token, std=0.02)
        nn.init.normal_(self.type_embed, std=0.02)
        nn.init.normal_(self.role_embed, std=0.02)
        self.block = LoopBankBlock(
            H,
            args.transformer_heads,
            args.transformer_ffn_mult,
            args.transformer_resid_init,
            args.transformer_q_gain_init,
        )
        self.read_block = LoopBankBlock(
            H,
            args.transformer_heads,
            args.transformer_ffn_mult,
            args.transformer_resid_init,
        ...
        read_dim = self.num_read_tokens * H * (K + 1)
        self.actor_out_norm = nn.RMSNorm(read_dim, elementwise_affine=False)
        self.critic_out_norm = nn.RMSNorm(read_dim, elementwise_affine=False)
        self.actor_out_proj = layer_init(nn.Linear(read_dim, H))
        self.critic_out_proj = layer_init(nn.Linear(read_dim, H))

    def _initial_work_bank(self, x):
        B = x.shape[0]
        x0 = self.entry(x)
        obs_tokens = (
            x.unsqueeze(-1) * self.obs_embed_w.unsqueeze(0)
            + self.obs_embed_b.unsqueeze(0)
            + self.obs_coord_embed.unsqueeze(0)
            + self.type_embed[0].view(1, 1, -1)
            + 0.25 * x0.unsqueeze(1)
        )
        memory_tokens = (
            self.shared_memory_tokens.unsqueeze(0).expand(B, -1, -1)
            + self.type_embed[1].view(1, 1, -1)
            + x0.unsqueeze(1)
        )
        return torch.cat([obs_tokens, memory_tokens], dim=1), x0

    def _run_work_loop(self, initial):
        current = initial
        ledger = [current]
        for _ in range(self.K):
            context = torch.cat(ledger, dim=1)
            current = self.block(current, context, initial)
            ledger.append(current)
        return ledger

    def _read_trace(self, ledger, x0, role):
        B = x0.shape[0]
        if role == "actor":
            role_idx = 0
            read_token = self.actor_read_token
            out_norm = self.actor_out_norm
            out_proj = self.actor_out_proj
        elif role == "critic":
            role_idx = 1
            read_token = self.critic_read_token
            out_norm = self.critic_out_norm
            out_proj = self.critic_out_proj
        else:
            raise ValueError(f"unknown role {role}")

        initial_read = (
            read_token.unsqueeze(0).expand(B, -1, -1)
            + self.type_embed[2].view(1, 1, -1)
            + self.role_embed[role_idx].view(1, 1, -1)
            + x0.unsqueeze(1)
        )
        traces = []
        for depth in range(len(ledger)):
            context = torch.cat(ledger[: depth + 1], dim=1)
            read = initial_read
            read = self.read_block(read, context, initial_read)
            traces.append(read)
        flat = torch.cat([r.reshape(B, -1) for r in traces], dim=-1)
        return out_proj(out_norm(flat))

    def forward(self, x):
        use_bf16 = self.use_bf16 and x.is_cuda and torch.cuda.is_bf16_supported()
        ctx = torch.autocast(device_type="cuda", dtype=torch.bfloat16) if use_bf16 else nullcontext()
        with ctx:
            initial, x0 = self._initial_work_bank(x)
            ledger = self._run_work_loop(initial)
            actor_feat = self._read_trace(ledger, x0, "actor")
            critic_feat = self._read_trace(ledger, x0, "critic")
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if self.share_backbone:
            ...
            self.trunk = ThinkTrunk(obs_dim, H, args.k_blocks, args)
        else:
            self.critic_trunk = ThinkTrunk(obs_dim, H, args.k_blocks, args)
            self.actor_trunk = ThinkTrunk(obs_dim, H, args.k_blocks, args)
    ...
    def _trunks(self, x):
        # Return (actor_feat, critic_feat). With a shared backbone, one trunk
        # computes separate actor/critic role-memory streams with shared weights.
        if self.share_backbone:
            return self.trunk(x)
    ...
    def get_action_and_value(self, x, z=None, return_beta_concentration=False):
        ...
        if return_beta_concentration and self.actor_dist == "beta":
            beta_concentration = dist.concentration1 + dist.concentration0
        if z is None:
            z = dist.sample()
        ...
        if return_beta_concentration:
            return action, z, log_prob, entropy, value_logits, beta_concentration
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        clipfracs = []
        beta_entropy_floor_loss_sum = 0.0
        beta_entropy_floor_active_sum = 0.0
        beta_concentration_loss_sum = 0.0
        beta_concentration_active_sum = 0.0
        beta_concentration_mean_sum = 0.0
        beta_concentration_max = 0.0
        beta_entropy_floor_mb_count = 0
        beta_entropy_floor = args.beta_entropy_floor_per_dim * float(act_dim)
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_logits, beta_concentration = agent.get_action_and_value(
                    b_obs[mb_inds], b_latent_zs[mb_inds], return_beta_concentration=True
                )
                ...
                if args.actor_dist == "beta" and args.beta_entropy_floor_coef > 0.0:
                    entropy_per_dim = entropy_loss / float(act_dim)
                    beta_entropy_floor_loss = (
                        args.beta_entropy_floor_coef
                        * float(act_dim)
                        * (args.beta_entropy_floor_per_dim - entropy_per_dim).clamp_min(0.0).pow(2)
                    )
                else:
                    beta_entropy_floor_loss = entropy_loss.new_zeros(())
                if (
                    args.actor_dist == "beta"
                    and beta_concentration is not None
                    and args.beta_concentration_coef > 0.0
                ):
                    beta_concentration_hinge = (
                        torch.log(beta_concentration.clamp_min(1e-6)) - log(args.beta_concentration_cap)
                    ).clamp_min(0.0)
                    beta_concentration_loss = args.beta_concentration_coef * beta_concentration_hinge.pow(2).mean()
                    beta_concentration_active = (beta_concentration_hinge > 0).float().mean().detach().item()
                    beta_concentration_mean = beta_concentration.detach().mean().item()
                    beta_concentration_max_mb = beta_concentration.detach().max().item()
                else:
                    beta_concentration_loss = entropy_loss.new_zeros(())
                    beta_concentration_active = 0.0
                    beta_concentration_mean = 0.0
                    beta_concentration_max_mb = 0.0
                beta_entropy_floor_loss_sum += beta_entropy_floor_loss.detach().item()
                beta_entropy_floor_active_sum += float((beta_entropy_floor_loss.detach() > 0).item())
                beta_concentration_loss_sum += beta_concentration_loss.detach().item()
                beta_concentration_active_sum += beta_concentration_active
                beta_concentration_mean_sum += beta_concentration_mean
                beta_concentration_max = max(beta_concentration_max, beta_concentration_max_mb)
                beta_entropy_floor_mb_count += 1
        ...
        writer.add_scalar(
            "losses/beta_entropy_floor_loss",
            beta_entropy_floor_loss_sum / max(beta_entropy_floor_mb_count, 1),
            global_step,
        )
        writer.add_scalar(
            "debug/beta_entropy_floor_active_frac",
            beta_entropy_floor_active_sum / max(beta_entropy_floor_mb_count, 1),
            global_step,
        )
        writer.add_scalar("debug/beta_entropy_floor", beta_entropy_floor, global_step)
        writer.add_scalar(
            "losses/beta_concentration_loss",
            beta_concentration_loss_sum / max(beta_entropy_floor_mb_count, 1),
            global_step,
        )
        writer.add_scalar(
            "debug/beta_concentration_leash_active_frac",
            beta_concentration_active_sum / max(beta_entropy_floor_mb_count, 1),
            global_step,
        )
        writer.add_scalar(
            "debug/beta_concentration_leash_mean",
            beta_concentration_mean_sum / max(beta_entropy_floor_mb_count, 1),
            global_step,
        )
        writer.add_scalar("debug/beta_concentration_leash_max", beta_concentration_max, global_step)
        if auto_alpha:
        ...
        writer.add_scalar("losses/actor_clip_scale", min(1.0, args.actor_grad_clip / (float(actor_gn) + 1e-12)), global_step)
        writer.add_scalar("losses/critic_clip_scale", min(1.0, args.critic_grad_clip / (float(critic_gn) + 1e-12)), global_step)
        if args.actor_dist == "beta":
            with torch.no_grad():
                actor_probe, _ = agent._trunks(b_obs[: args.minibatch_size])
                beta_probe, _, _ = agent._actor_dist(actor_probe)
                beta_alpha = beta_probe.concentration1
                beta_beta = beta_probe.concentration0
                beta_total = beta_alpha + beta_beta
                ...
                writer.add_scalar("debug/beta_concentration_mean", beta_total.mean().item(), global_step)
                writer.add_scalar("debug/beta_concentration_max", beta_total.max().item(), global_step)
