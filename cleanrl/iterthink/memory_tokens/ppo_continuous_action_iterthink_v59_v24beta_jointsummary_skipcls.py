# PPO + IterThink v59 (v24 Beta + full-layer skip/CLS summaries). From v24.
#
...
#     parameter shapes.
#   - The loop current state contains observation/work tokens. Each layer also has
#     an append-only skip packet written from the full current layer context
#     (work + prior skips). The skip packet is then part of that layer for actor
#     and critic CLS reads and for future loop attention, but it never becomes the
#     next recurrent work state.
#   - Actor and critic CLS tokens read the full current layer (work + prior skips
#     + newly written skip) at every depth. Their outputs are collected as traces,
#     but never appended to future shared context.
#   - Final projections are role-specific and consume only actor/critic CLS traces.
#   - Use the v56-style conservative normalization/residual setup: non-affine
#     RMSNorm, residual branches initialized at 0.1, and output projections scaled
#     by the same residual init.
#
...
class SkipWriter(nn.Module):
    """Write immutable skip summaries by soft-pooling the current work tokens."""

    def __init__(self, H, num_heads, resid_init):
        super().__init__()
        assert H % num_heads == 0
        self.num_heads = num_heads
        self.head_dim = H // num_heads
        self.query_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.context_norm = nn.RMSNorm(H, elementwise_affine=False)
        ...
        self.wv = nn.Linear(H, H, bias=False)
        self.wo = nn.Linear(H, H, bias=False)
        self.skip_scale = nn.Parameter(torch.full((H,), resid_init))
        for module in (self.wq, self.wk, self.wv):
            init_linear_(module, init_scale=1.0)
        init_linear_(self.wo, init_scale=resid_init)

    def forward(self, skip_seed, current):
        B, Q, H = skip_seed.shape
        S = current.shape[1]
        q_in = self.query_norm(skip_seed)
        k_in = self.context_norm(current)
        q = self.wq(q_in).view(B, Q, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.wk(k_in).view(B, S, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.wv(current).view(B, S, self.num_heads, self.head_dim).transpose(1, 2)
        q = self.q_norm(q)
        k = self.k_norm(k)
        pooled = flash_attention(q, k, v).transpose(1, 2).reshape(B, Q, H)
        pooled = self.out_norm(self.wo(pooled))
        return skip_seed + self.skip_scale.view(1, 1, H) * pooled
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, args):
        ...
        self.H = H
        self.K = K
        self.num_skip_tokens = args.loop_skip_tokens
        ...
        self.skip_tokens = nn.Parameter(torch.empty(self.num_skip_tokens, H))
        ...
        nn.init.normal_(self.skip_tokens, std=0.02)
    ...
    def _initial_work_bank(self, x):
        ...
        return obs_tokens, x0

    def _skip_seed(self, x0):
        B = x0.shape[0]
        return (
            self.skip_tokens.unsqueeze(0).expand(B, -1, -1)
            + self.type_embed[1].view(1, 1, -1)
            + x0.unsqueeze(1)
        )

    def _read_seed(self, x0, role):
        B = x0.shape[0]
        if role == "actor":
            role_idx = 0
    ...
    def _read_role(self, layer_context, read_seed):
        return self.read_block(read_seed, layer_context, read_seed)

    def _project_trace(self, traces, role):
        B = traces[0].shape[0]
        flat = torch.cat([r.reshape(B, -1) for r in traces], dim=-1)
        if role == "actor":
            return self.actor_out_proj(self.actor_out_norm(flat))
        if role == "critic":
            return self.critic_out_proj(self.critic_out_norm(flat))
        raise ValueError(f"unknown role {role}")

    def _summarize_layer(self, current, skip_ledger, skip_seed, actor_seed, critic_seed):
        context_parts = [current]
        if skip_ledger:
            context_parts.append(torch.cat(skip_ledger, dim=1))
        pre_skip_context = torch.cat(context_parts, dim=1)
        new_skip = self.skip_writer(skip_seed, pre_skip_context)
        # Actor/critic read the full current layer, including the newly written
        # skip packet. Only the skip packet is appended for future layers.
        full_layer_context = torch.cat([pre_skip_context, new_skip], dim=1)
        actor_read = self._read_role(full_layer_context, actor_seed)
        critic_read = self._read_role(full_layer_context, critic_seed)
        return new_skip, actor_read, critic_read

    def _run_work_loop(self, initial, x0):
        current = initial
        skip_seed = self._skip_seed(x0)
        actor_seed = self._read_seed(x0, "actor")
        critic_seed = self._read_seed(x0, "critic")
        skip_ledger = []
        actor_traces = []
        critic_traces = []

        new_skip, actor_read, critic_read = self._summarize_layer(
            current, skip_ledger, skip_seed, actor_seed, critic_seed
        )
        skip_ledger.append(new_skip)
        actor_traces.append(actor_read)
        critic_traces.append(critic_read)

        for _ in range(self.K):
            context = torch.cat([current, torch.cat(skip_ledger, dim=1)], dim=1)
            ...
            actor_traces.append(actor_read)
            critic_traces.append(critic_read)
        return actor_traces, critic_traces
    ...
    def forward(self, x):
        ...
        with ctx:
            ...
            actor_traces, critic_traces = self._run_work_loop(initial, x0)
            actor_feat = self._project_trace(actor_traces, "actor")
            critic_feat = self._project_trace(critic_traces, "critic")
