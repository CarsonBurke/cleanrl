# PPO + IterThink v52 (v24 Beta + shared loop-bank transformer ledger). From v24.
#
...
class Args:
    ...
    hidden: int = 64
    k_blocks: int = 3
    n_experts: int = 16  # legacy v24 CLI parity; unused by the loop-bank transformer trunk
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, args):
        ...
        self.memory_tokens = nn.Parameter(torch.empty(self.num_memory_tokens, H))
        self.read_tokens = nn.Parameter(torch.empty(self.num_read_tokens, H))
    ...
    def _initial_bank(self, x):
        ...
        memory_tokens = (
            self.memory_tokens.unsqueeze(0).expand(B, -1, -1)
            + self.type_embed[1].view(1, 1, -1)
            + x0.unsqueeze(1)
        )
        read_tokens = (
            self.read_tokens.unsqueeze(0).expand(B, -1, -1)
    ...
    def forward(self, x):
        ...
        with ctx:
            ...
            feat = self.out_proj(self.out_norm(flat))
        return feat.float()
