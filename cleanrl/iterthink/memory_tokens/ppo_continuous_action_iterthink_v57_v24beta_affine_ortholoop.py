# PPO + IterThink v57 (v24 Beta + affine-norm orthogonal loop memory). From v24.
#
...
#   - Compared with v56, token RMSNorms use learned affine scale, residual branch
#     scales start at 1.0, and attention/FFN output projections use full
#     orthogonal initialization instead of double-damped 0.1 residual branches.
#
...
class LoopBankBlock(nn.Module):
    ...
    def __init__(self, H, num_heads, ffn_mult, resid_init, q_gain_init):
        ...
        self.current_norm = nn.RMSNorm(H, elementwise_affine=True)
        self.context_norm = nn.RMSNorm(H, elementwise_affine=True)
        self.ffn_norm = nn.RMSNorm(H, elementwise_affine=True)
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, args):
        ...
        self.actor_out_norm = nn.RMSNorm(read_dim, elementwise_affine=True)
        self.critic_out_norm = nn.RMSNorm(read_dim, elementwise_affine=True)
