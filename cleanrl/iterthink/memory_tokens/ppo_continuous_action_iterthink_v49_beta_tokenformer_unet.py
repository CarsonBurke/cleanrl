# PPO + IterThink v49 (SAC-Beta + BF16 token U-Net transformer trunk). From v24.5.
#
# Changes from v24.5:
#   - Replace ThinkTrunk's dense/MoE MLP backend with a token transformer trunk.
...
class Args:
    ...
    hidden: int = 128
    k_blocks: int = 4               # v49: encoder depth; decoder has the same depth.
...
class CrossAttentionReadout(nn.Module):
    """Pre-norm PMA-style single-token readout over transformed observation tokens."""
    ...
    def __init__(self, H, num_heads, ffn_mult, resid_init):
        ...
        self.num_heads = num_heads
        self.head_dim = H // num_heads
        self.q_norm_in = nn.RMSNorm(H, elementwise_affine=False)
        self.kv_norm_in = nn.RMSNorm(H, elementwise_affine=False)
        self.ffn_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.q_norm = nn.RMSNorm(self.head_dim, elementwise_affine=False)
        self.k_norm = nn.RMSNorm(self.head_dim, elementwise_affine=False)
        self.wq = nn.Linear(H, H, bias=False)
    ...
    def forward(self, seed, tokens):
        B, S_q, H = seed.shape
        S_kv = tokens.shape[1]
        q_in = self.q_norm_in(seed)
        kv_in = self.kv_norm_in(tokens)
        q = self.wq(q_in).view(B, S_q, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.wk(kv_in).view(B, S_kv, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.wv(kv_in).view(B, S_kv, self.num_heads, self.head_dim).transpose(1, 2)
        q = self.q_norm(q)
        k = self.k_norm(k)
        attn = flash_attention(q, k, v).transpose(1, 2).reshape(B, S_q, H)
        seed = seed + self.attn_scale.view(1, 1, H) * self.wo(attn)
        h = self.ffn_norm(seed)
        seed = seed + self.ffn_scale.view(1, 1, H) * self.ff2(torch.relu(self.ff1(h)).pow(2))
        return seed
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, args):
        ...
        self.summary_token = nn.Parameter(torch.randn(1, H) * 0.02)
        self.readout = CrossAttentionReadout(
            H, args.transformer_heads, args.transformer_ffn_mult, args.transformer_resid_init
        )
    ...
    def forward(self, x):
        ...
        with ctx:
            ...
            tokens = torch.cat([latents, obs_tokens], dim=1)
            skips = []
            ...
            for mixer, block, skip in zip(self.skip_mixers, self.decoder, reversed(skips)):
                ...
                tokens = block(tokens)
            seed = self.summary_token.unsqueeze(0).expand(B, -1, -1)
            seed = self.readout(seed, tokens)
            out = self.out_norm(seed[:, 0])
        return out.float()
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if self.actor_dist == "gaussian":
            ...
            self.actor_logstd_head = layer_init(nn.Linear(H, act_dim), std=0.01, bias_const=raw_logstd_init)
    ...
    def _actor_dist(self, actor_feat):
        ...
        if self.actor_dist == "gaussian":
            ...
            raw_logstd = self.actor_logstd_head(actor_feat)
