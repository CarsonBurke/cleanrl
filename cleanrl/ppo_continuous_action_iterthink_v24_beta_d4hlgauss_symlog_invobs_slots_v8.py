# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted-attention slot transformer v8.
#
# WHY v8 (one variable on top of v6). v6 is the clean paper-faithful TF-Inv baseline
# (8561 +/-376 on HalfCheetah @8M, only ~1.1k behind the v24 MLP winner). v7 tried to
# beat it but bundled FOUR changes at once -- a nonlinear tokenizer that ALSO dropped
# v6's per-dim value directions, LayerScale(0.1) residual scaling, a ReLU^2 FFN swap,
# and a shared-trunk restructure. It regressed ~15-20% at every matched step (the
# LayerScale(0.1) throttle on every block is the prime suspect for the early-learning
# lag), and the 4-way confound made attribution impossible. v8 discards that bundle
# and tests the ONLY genuine architectural thesis in isolation:
#
#   THE BOTTLENECK IS THE TOKEN, NOT CAPACITY. v6's tokenizer is rank-1 per dim:
#   token_i = x_i * w_i + p_i, a scalar slide along ONE fixed direction w_i. The
#   key/query are linear in this, so slot<->token similarity is AFFINE in x_i -- a
#   token can encode "how much" of dim i but every value of x_i maps to the SAME
#   direction in H-space. Inverted attention's job is nonlinear cross-feature mixing,
#   but with affine-in-value tokens it has little nonlinear structure to mix. v8 adds
#   a shared per-scalar nonlinear lift (1->H GELU ->H) as a RESIDUAL on top of v6's
#   linear token, so each dim's token can trace a nonlinear CURVE in H-space:
#       token_i = (x_i * w_i + p_i)            # v6 path, keeps per-dim direction
#                 + fc2(gelu(fc1(x_i)))        # shared nonlinear value code
#   This is a STRICT SUPERSET of v6: the linear path (and its per-dim directions) is
#   untouched. token_norm (LayerNorm) renormalizes each token before attention, so the
#   additive branch only enriches token DIRECTION content, not scale -- no gate or
#   zero-init needed, and unlike LayerScale it cannot throttle block contributions.
#   Everything else is byte-identical to v6 (GELU FFN, plain residuals, separate
#   actor/critic stacks, flatten readout, all PPO machinery).
#
# WHY v6 (inherited). v4 grafted the inverted-attention idea (Wu et al. 2024, "Inverted-Attention
...
class ObsTokenizer(nn.Module):
    ...
    v8: v6's rank-1 linear token (per-dim direction) PLUS a shared per-scalar nonlinear
    lift added as a residual, so each dim's token traces a nonlinear curve in H-space
    instead of an affine line. Strict superset of v6 -- the linear path is untouched.
    The nonlinear branch (fc1: 1->H, GELU, fc2: H->H) is shared across all obs dims;
    per-dim identity still comes from value_weight/pos. token_norm downstream
    renormalizes scale, so this only enriches token direction content."""
    ...
    def forward(self, x):
        xx = x.unsqueeze(-1)                                                # (B, obs_dim, 1)
        lin = xx * self.value_weight.unsqueeze(0) + self.pos.unsqueeze(0)   # v6 token
        nl = self.fc2(F.gelu(self.fc1(xx)))                                 # shared nonlinear lift
        return lin + nl


class SlotMLP(nn.Module):
    """Standard transformer FFN (GELU), per the paper's pre-LN decoder block."""
...
class InvertedCrossAttention(nn.Module):
    """Multi-head INVERTED cross-attention (Wu et al. 2024). The ONLY change from a
    standard Transformer-decoder cross-attention is the normalization axis: softmax
    over the SLOT (query) axis so slots COMPETE to explain each token, then each
    slot's weights are renormalized over tokens into a convex combination. Plain
    scaled dot-product otherwise -- no QK-norm, no head gains, no zero-init."""
...
class InvertedSlotBlock(nn.Module):
    ...
    no learned residual scales."""

    def __init__(self, H, heads, mlp_mult):
        super().__init__()
    ...
    def forward(self, slots, tokens):
        slots = slots + self.attn(self.slot_norm(slots), self.token_norm(tokens))
...
class InvertedSlotStack(nn.Module):
    ...
    def __init__(self, n_slots, H, n_blocks, n_heads, mlp_mult):
        super().__init__()
        assert n_blocks >= 1
        ...
        self.blocks = nn.ModuleList(
            [InvertedSlotBlock(H, n_heads, mlp_mult) for _ in range(n_blocks)]
        )
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, actor_slots, critic_slots, slot_blocks, slot_heads, slot_mlp_mult):
        super().__init__()
        self.tokenizer = ObsTokenizer(in_dim, H)
        self.actor_stack = (
            InvertedSlotStack(actor_slots, H, slot_blocks, slot_heads, slot_mlp_mult)
            if actor_slots is not None
            else None
        )
        self.critic_stack = (
            InvertedSlotStack(critic_slots, H, slot_blocks, slot_heads, slot_mlp_mult)
