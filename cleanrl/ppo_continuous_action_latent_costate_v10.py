# Latent costate v10: vector differential Bellman prediction for return maximization.
...
# Model Jacobian accuracy and undiscounted derivative stability remain hypotheses.
import json
import math
import random
import time
...
@dataclass
class Args:
    exp_name: str = 'latent_costate_v10'
    seed: int = 1
...
def differential_targets(model, actor, observations, physical_actions, beta_jacobian,
                         next_costate, continuation, control_weight):
    """Vector Bellman target; next costate is held constant inside every VJP."""
    cotangent = torch.cat((next_costate.detach() * continuation[:, None],
...
def main():
    ...
    if batch_size % args.minibatch_size or args.num_steps < 1000:
        raise ValueError('fixed minibatches must divide a rollout of at least one horizon')
    if min(args.total_timesteps, args.model_epochs, args.critic_sweeps, args.model_learning_rate,
    ...
    with ExitStack() as resources:
        ...
        def target_function(obs, action, beta_jac, next_value, continuation):
            return differential_targets(model, agent.actor, obs, action, beta_jac, next_value, continuation, control_weight)
        ...
        for iteration in range(1, iterations + 1):
            ...
            with timer.span('update'):
                ...
                for _ in range(args.model_epochs):
                    for indices in device_minibatches(batch_size, args.minibatch_size, device, model_shuffle):
                        if args.compile:
                            torch.compiler.cudagraph_mark_step_begin()
                ...
                critic_metrics = []
                for _ in range(args.critic_sweeps):
                    with torch.no_grad():
                        if args.compile:
                            torch.compiler.cudagraph_mark_step_begin()
                ...
                with torch.no_grad():
                    if args.compile:
                        torch.compiler.cudagraph_mark_step_begin()
                    diagnostics.update(trust.step(observations, logits, factor, eta_credit, weights, measure))
        ...
        report = dict(args=vars(args), fresh_initialization=True, checkpoint_loaded=False,
                      ...
                      objective='Undiscounted fixed-horizon episodic reward',
                      limitations='Single seed and training returns. Learned dynamics derivatives may be inaccurate. Differential Bellman recursion need not contract. Fixed affine normalization after stochastic warmup. State occupancy includes ongoing-episode policy changes. Natural actor solve is approximate.')
