# Intra-trajectory TPO, Beta policy, SiTU-GLU trunks, HL-Gauss critic. v4.
#
# v4 is v2 (ppo_continuous_action_tpo_intra_beta_v2.py) with four changes:
#   1. SiTU-GLU trunks replace the ReluSq MLP bodies in both actor and critic,
#      with the learned cap allocation from sf_vlam_v8_learnedbeta_situglu.
#   2. A distributional HL-Gauss critic on Dreamer3 symexp-spaced buckets
#      replaces the scalar critic and its clipped MSE.
#   3. eta = 4.
#   4. The actor and critic networks are each compiled separately with
#      reduce-overhead, with the distribution math left in eager.
#
# The TPO objective is unchanged from v2. Recap: every timestep is its own TPO
# problem over "the executed action vs. the entire rest of the action space".
# Give the executed action a cell of width delta per dimension so it carries
# genuine mass p_old = pi_old(a_t|s_t)*delta^d; the anchored target is
...
#
# Why HL-Gauss here specifically. TPO's target is built from the critic's
# advantage and nothing else -- there is no clip to absorb a bad advantage, and
# the exponential exp(u/eta) *amplifies* the critic's tail errors rather than
# truncating them. A regression critic trained on symlog-free returns has to
# spend capacity on scale; the categorical HL-Gauss head instead learns a
# distribution over symexp-spaced buckets under a cross-entropy loss, which is
# far better conditioned in the tails. So the critic quality that TPO is unusually
# sensitive to is exactly what this head is meant to improve. Value clipping is
# dropped: the categorical loss is a cross-entropy against a projected target,
# and there is no scalar residual to clip.
#
# Why SiTU-GLU. The gated branch has a different Jacobian geometry than ReluSq
# at matched initial output scale, and the learned log_beta_delta moves
# reciprocal cap scale between the tanh gate and the tanh up-projection while
# holding their product invariant, so the total activation range is fixed and
# only its allocation is learned.
#
# Note on depth: both trunks are two stacked SiTU-GLU branches, matching v2's two
# nonlinear stages. A linear input projection feeding the branch's linear gate/up
# would compose to a single linear map, so there is no bare projection layer.
# This costs roughly 2x the hidden parameters of v2's 64->64 stage.
#
# Hypothesis: TPO's sensitivity to critic quality makes a distributional critic
# worth more here than it is for clipped PPO, and the gated trunk supplies the
# capacity to use it.
...
# --- HL-Gauss critic support -------------------------------------------------


def normalized_return_abs_bound(args) -> float:
    if args.gamma >= 1.0:
...
def hlgauss_coord_bounds(args):
    bound = normalized_return_abs_bound(args)
    coord_max = float(symlog(torch.tensor(bound, dtype=torch.float32)).item())
...
def make_hlgauss_support(args, device):
    ...
    return Dreamer3BucketHLGaussSupport(
        args.num_bins, coord_min, coord_max, args.value_sigma_to_bin_ratio, device
    )


def hl_value_loss(value_logits, returns, hl_support):
    """Cross entropy to the HL-Gauss projection of the scalar return target."""
...
class Agent(nn.Module):
    """Beta policy on the native action box with an HL-Gauss categorical critic.
    ...
    def __init__(self, envs, num_bins, hidden_dim=64):
        super().__init__()
    ...
    def critic_logits(self, x):
        """Compiled critic network: observations -> HL-Gauss bucket logits."""
...
if __name__ == "__main__":
    ...
    agent = Agent(envs, args.num_bins, args.hidden_dim).to(device)
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)
    hl_support = make_hlgauss_support(args, device)
    ...
    critic_logits = agent.critic_logits
    if args.compile:
        policy_params = torch.compile(policy_params, mode=args.compile_mode, dynamic=False)
        critic_logits = torch.compile(critic_logits, mode=args.compile_mode, dynamic=False)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                values[step] = hl_support.to_scalar(critic_logits(next_obs))
            actions[step] = action
            zs[step] = z
        ...
        with torch.no_grad():
            step_begin()
            next_value = hl_support.to_scalar(critic_logits(next_obs)).reshape(1, -1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                newvalue_logits = critic_logits(b_obs[mb_inds])
                ...
                # Value loss: HL-Gauss cross entropy. There is no scalar residual
                # to clip, so PPO's clipped value loss does not apply.
