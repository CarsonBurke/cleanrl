# TPO-MD HORIZON-TD TAPE v23: graph-free multistep latent predictive coding.
#
# One shared horizon-conditioned predictor estimates a dense H=16 tape of future latent
# innovations.  A coherent actor/predictor snapshot supplies local stopped TD errors:
# y_1=eta_t and y_h=p^-_{h-1}(s_{t+1},a_{t+1}).  Cumulative tape entries therefore obey
# q_h=eta_t+q^-_{h-1}(t+1), without rolling a learned state through time or retaining a
# temporal autograd graph.  The horizon shift is H-step nilpotent (acyclic), not an
# ordinary discounted contraction; unlike gamma-weighted successor averages it preserves
# gait phase and expected endpoint displacement.  One rollout innovation RMS changes loss units
# only.  The clipped trunk error joins actor+critic once in task Adam; the predictor keeps
# private Adam moments and continues after the TPO breaker detaches the trunk.
# Falsifiers: the tape must beat direct-MC zero/persistence baselines, retain delayed phase,
# respond to source and action replacements, and improve return without worsening the
# rolling lower tail.  Low Bellman error alone is not evidence of useful representation.
import copy
import os
import random
import time
...
class TapeTargets:
    """Stopped one-step TD labels in delayed-innovation coordinates."""
...
def build_next_action_table(
    ...
    """Choose the SARSA action at each stored transition's actual next state.

    Interior rows use the next stored behavior action.  Truncations and the artificial
    rollout edge use one frozen-policy sample supplied by the caller.  Terminal and
    invalid rows are exactly zero, so reset actions and poisoned placeholders cannot leak.
    """
    if behavior_actions.shape != sampled_next_actions.shape or behavior_actions.ndim != 3:
        raise ValueError("actions must have matching [time, env, action] shapes")
    expected = behavior_actions.shape[:2]
    if any(tensor.shape != expected for tensor in (
        transition_terminations, transition_boundaries, transition_valids
    )):
        raise ValueError("transition masks must match the action table")
    time_dim = behavior_actions.shape[0]
    terminations = transition_terminations.detach().bool()
    boundaries = transition_boundaries.detach().bool()
    valids = transition_valids.detach().bool()
    sampled_rows = next_state_bootstrap_rows(terminations, boundaries, valids)
    interior = valids & ~boundaries
    interior[-1] = False
    table = torch.zeros_like(behavior_actions)
    if time_dim > 1:
        table[:-1] = torch.where(
            interior[:-1].unsqueeze(-1),
            behavior_actions.detach()[1:],
            torch.zeros_like(table[:-1]),
        )
    table = torch.where(
        sampled_rows.unsqueeze(-1),
        sampled_next_actions.detach(),
        table,
    )
    usable = (interior | sampled_rows) & ~terminations & valids
    table = torch.where(usable.unsqueeze(-1), table, torch.zeros_like(table))
    return table.detach(), sampled_rows.detach()


def build_horizon_td_tape_targets(
    innovations: torch.Tensor,
    next_predictions: torch.Tensor,
    transition_terminations: torch.Tensor,
    transition_valids: torch.Tensor,
) -> TapeTargets:
    """Apply the stopped, lower-shift Bellman operator in delayed coordinates."""
    if innovations.ndim != 3 or next_predictions.ndim != 4:
        raise ValueError("innovations and predictions must be [T,N,D] and [T,N,H,D]")
    if innovations.shape[:2] != next_predictions.shape[:2]:
        raise ValueError("innovation and prediction rows do not match")
    if innovations.shape[-1] != next_predictions.shape[-1]:
        raise ValueError("innovation and prediction latent dimensions do not match")
    if next_predictions.shape[-2] < 1:
        raise ValueError("the tape horizon must be positive")
    expected = innovations.shape[:2]
    if transition_terminations.shape != expected or transition_valids.shape != expected:
        raise ValueError("transition masks have the wrong shape")
    valid = transition_valids.detach().bool()
    continuation = valid & ~transition_terminations.detach().bool()
    safe_innovation = torch.where(
        valid.unsqueeze(-1), innovations.detach(), torch.zeros_like(innovations)
    )
    safe_next = torch.where(
        continuation[..., None, None],
        next_predictions.detach(),
        torch.zeros_like(next_predictions),
    )
    values = torch.zeros_like(next_predictions)
    values[..., 0, :] = safe_innovation
    if values.shape[-2] > 1:
        values[..., 1:, :] = safe_next[..., :-1, :]
    masks = valid.unsqueeze(-1).expand(*valid.shape, values.shape[-2]).clone()
    return TapeTargets(
        values=values.detach(),
        masks=masks.detach(),
        innovations=safe_innovation.detach(),
        continuations=continuation.detach(),
    )
...
def lower_shift_tape(tape: torch.Tensor) -> torch.Tensor:
    """The nilpotent horizon operator L: [x1,...,xH] -> [0,x1,...,xH-1]."""
    if tape.ndim < 2 or tape.shape[-2] < 1:
        raise ValueError("tape needs a nonempty horizon dimension")
    shifted = torch.zeros_like(tape)
    if tape.shape[-2] > 1:
        shifted[..., 1:, :] = tape[..., :-1, :]
    return shifted
...
def build_direct_delayed_innovation_targets(
    ...
    if innovations.ndim != 3 or horizon < 1:
        raise ValueError("innovations must be [T,N,D] and horizon must be positive")
    expected = innovations.shape[:2]
    ...
    masks = torch.zeros((time_dim, num_envs, horizon), dtype=torch.bool, device=innovations.device)
...
def horizon_td_tape_loss(
    ...
    *,
    ...
    """Equal-head SmoothL1 using one detached rollout innovation unit."""
    ...
    if target.ndim < 3 or target.shape[-2] < 1:
        raise ValueError("target needs nonempty horizon and latent dimensions")
    ...
    if scale is None:
        ...
        denominator = (innovation_mask.sum() * target.shape[-1]).clamp_min(1.0)
        innovation_rms = (
            safe_innovation.square().mul(innovation_mask).sum() / denominator
        ).sqrt()
        if latent_rms is None:
            floor = innovation_rms.new_tensor(torch.finfo(target.dtype).eps)
        else:
            if latent_rms.numel() != 1:
                raise ValueError("latent_rms must be scalar")
            floor = latent_rms.detach().to(target).reshape(()) * scale_floor_ratio
        scale = innovation_rms.clamp_min(floor.clamp_min(torch.finfo(target.dtype).eps))
    else:
        if scale.numel() != 1:
            raise ValueError("scale must be one scalar shared by every tape head")
        scale = scale.detach().to(target).reshape(()).clamp_min(torch.finfo(target.dtype).eps)
    ...
    channel_error = F.smooth_l1_loss(
        safe_prediction / scale, safe_target / scale, reduction="none"
    )
    reduce_dims = tuple(range(target.ndim - 2)) + (target.ndim - 1,)
    denominator = (
        mask.to(target.dtype).sum(dim=tuple(range(mask.ndim - 1))) * target.shape[-1]
    ).clamp_min(1.0)
    per_head = (channel_error * weights).sum(dim=reduce_dims) / denominator
    return per_head.mean(), scale.detach(), per_head.detach()


def normalized_tape_rms(
    residual: torch.Tensor, mask: torch.Tensor, scale: torch.Tensor
) -> torch.Tensor:
    """Per-head RMS in the single rollout innovation unit."""
    if residual.shape[:-1] != mask.shape or scale.numel() != 1:
        raise ValueError("residual, mask, and scale shapes are inconsistent")
    weights = mask.to(residual.dtype).unsqueeze(-1)
    sample_dims = tuple(range(mask.ndim - 1))
    ...
    normalized_square = (safe_residual / scale.detach().reshape(())).square() * weights
...
def tape_cross_head_statistics(
    ...
    """Adjacent-head cosine, normalized energy, and alternating/phase energy."""
    if prediction.shape[:-1] != mask.shape or scale.numel() != 1:
        raise ValueError("prediction, mask, and scale shapes are inconsistent")
    safe = torch.where(mask.unsqueeze(-1), prediction.detach(), torch.zeros_like(prediction))
    weights = mask.to(prediction.dtype).unsqueeze(-1)
    energy = ((safe / scale).square() * weights).sum() / (
    ...
    left = torch.where(joint, safe[..., :-1, :], torch.zeros_like(safe[..., :-1, :]))
    right = torch.where(joint, safe[..., 1:, :], torch.zeros_like(safe[..., 1:, :]))
    dot = (left * right).sum()
    cosine = dot / (left.square().sum().sqrt() * right.square().sum().sqrt()).clamp_min(1e-12)
    alternating = ((right - left) / scale).square().sum() / (
...
def masked_head_cosine(
    ...
    """Per-head cosine against realized delayed innovations on matched rows."""
    if prediction.shape != target.shape or prediction.shape[:-1] != mask.shape:
        raise ValueError("prediction, target, and mask shapes are inconsistent")
    weights = mask.to(prediction.dtype).unsqueeze(-1)
    ...
    reduce_dims = tuple(range(mask.ndim - 1)) + (prediction.ndim - 1,)
    dot = (safe_prediction * safe_target * weights).sum(dim=reduce_dims)
    prediction_norm = (safe_prediction.square() * weights).sum(dim=reduce_dims).sqrt()
...
class Args:
    ...
    # Graph-free dense delayed-innovation TD tape.  Every row supervises every head;
    # horizon is an acyclic index, not a temporal unroll in the optimizer graph.
    tape: bool = True
    tape_horizon: int = 16
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if self.tape_enabled:
            with torch.random.fork_rng(devices=[]):
                self.tape_predictor = HorizonTapePredictor(
                    H,
                    act_dim,
                    args.tape_horizon,
    ...
    def target_actor_feature(self, observations):
        ...
        return self.target_actor_trunk(observations)
...
def target_actor_feat_forward(agent, observations):
    """Static hard-snapshot encoder wrapper."""
...
def build_sparse_frozen_next_action_table(
    ...
    *,
    ...
    """Sample only truncation/rollout-edge actions from the coherent target policy."""
    if bootstrap_rows.ndim != 2 or next_actor_features.shape[:2] != bootstrap_rows.shape:
        raise ValueError("next features and bootstrap rows must share [time, env]")
    if next_actor_features.ndim != 3 or next_actor_features.shape[-1] != agent.tape_feature_dim:
        raise ValueError("next features have the wrong latent dimension")
    ...
    flat_next_features = next_actor_features.reshape(
        flat_rows.numel(), next_actor_features.shape[-1]
    )
    selected_actor_features = flat_next_features.index_select(0, row_indices)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if args.tape:
                ...
                frozen_actor_feats = torch.cat(current_feature_chunks).reshape(
                    args.num_steps, args.num_envs, agent.tape_feature_dim
                )
                frozen_next_actor_feats = torch.cat(next_feature_chunks).reshape(
                    args.num_steps, args.num_envs, agent.tape_feature_dim
                )
                valid_feature_rows = transition_valids.bool().unsqueeze(-1)
                ...
                # Interior targets use actual behavior a_{t+1}. Truncations and the
                # artificial rollout edge get exactly one target-policy action from a
                # persistent auxiliary RNG stream; terminal/invalid rows remain zero.
                ...
                next_action_table, tape_sampled_action_rows = build_next_action_table(
                    actions,
                    sampled_next_actions,
                    transition_terminations,
                    transition_boundaries,
                ...
                target_next_prediction_chunks = []
                for next_feature_chunk, next_action_chunk in zip(
                    frozen_next_actor_feats.reshape(
                        -1, agent.tape_feature_dim
                    ).split(args.tape_target_batch_size),
                    next_action_table.reshape(
                    ...
                    if args.compile:
                        torch.compiler.cudagraph_mark_step_begin()
                    target_next_prediction_chunks.append(
                        retain_graph_output(
                            target_tape_fn(next_feature_chunk, next_action_chunk),
                            compiled=args.compile,
                        )
                    )
                target_next_predictions = torch.cat(
                    target_next_prediction_chunks
                ).reshape(
                    args.num_steps,
                    args.num_envs,
                ...
                tape_targets = build_horizon_td_tape_targets(
                    tape_innovations,
                    target_next_predictions,
                    transition_terminations,
                    transition_valids,
                )
                latent_rms = frozen_actor_feats.detach().square().mean().sqrt()
                ...
                tape_target_rms_over_eta = tape_target_rms / tape_target_scale
                tape_propagated_head_fraction = (
                    tape_target_rms
                    > tape_target_scale
                    * max(args.tape_scale_floor_ratio, torch.finfo(torch.float32).eps)
                ).float().mean()
                tape_propagated_head_count = tape_propagated_head_fraction * args.tape_horizon
        ...
        if args.tape:
            ...
            b_tape_mask = tape_targets.masks.reshape(
                args.batch_size, args.tape_horizon
            )
            b_tape_direct_values = tape_direct_values.reshape(
                args.batch_size, args.tape_horizon, agent.tape_feature_dim
            )
        ...
        if args.tape:
            ...
            tape_bellman_errors = torch.zeros(args.tape_horizon, device=device)
            tape_prediction_rms = torch.zeros(args.tape_horizon, device=device)
            tape_direct_mc_errors = torch.zeros(args.tape_horizon, device=device)
            tape_direct_coverage = torch.zeros(args.tape_horizon, device=device)
            tape_direct_zero_baseline = torch.zeros(args.tape_horizon, device=device)
            tape_direct_nmse = torch.zeros(args.tape_horizon, device=device)
            tape_direct_cosine = torch.zeros(args.tape_horizon, device=device)
            tape_endpoint_errors = torch.zeros(args.tape_horizon, device=device)
            tape_endpoint_persistence_baseline = torch.zeros(
                args.tape_horizon, device=device
            )
            tape_endpoint_nmse = torch.zeros(args.tape_horizon, device=device)
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.tape and (
                    ...
                    with torch.no_grad():
                        ...
                        prediction_energy = normalized_tape_rms(
                            tape_prediction_for_sensitivity,
                            b_tape_mask[mb_inds],
                            tape_target_scale,
                        ).square().mean().sqrt().clamp_min(1e-12)
                        tape_action_replacement_ratio = normalized_tape_rms(
                            action_replacement_prediction
                            - tape_prediction_for_sensitivity,
                            b_tape_mask[mb_inds],
                            tape_target_scale,
                        ).square().mean().sqrt() / prediction_energy
                        tape_source_replacement_ratio = normalized_tape_rms(
        ...
        if args.tape:
            device_telemetry.update(
                {
                    ...
                    "tape/innovation_loss_scale": tape_target_scale,
                    ...
                    "tape/propagated_head_count": tape_propagated_head_count,
                    "tape/propagated_head_fraction": tape_propagated_head_fraction,
            ...
            device_telemetry.update(
                build_tape_head_telemetry(
                    head_losses=tape_head_losses,
                    bellman_errors=tape_bellman_errors,
                    target_rms=tape_target_rms,
                    target_rms_over_eta=tape_target_rms_over_eta,
                    prediction_rms=tape_prediction_rms,
                    direct_mc_errors=tape_direct_mc_errors,
                    direct_coverage=tape_direct_coverage,
                    direct_zero_baseline=tape_direct_zero_baseline,
                    direct_nmse=tape_direct_nmse,
                    direct_cosine=tape_direct_cosine,
                    endpoint_errors=tape_endpoint_errors,
                    endpoint_persistence_baseline=tape_endpoint_persistence_baseline,
                    endpoint_nmse=tape_endpoint_nmse,
                    rollout_zero_baseline=tape_rollout_zero_baseline,
                    rollout_persistence_baseline=tape_rollout_persistence_baseline,
                )
            )
