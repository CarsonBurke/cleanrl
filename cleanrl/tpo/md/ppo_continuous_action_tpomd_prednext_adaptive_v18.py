# TPO-MD PredNext Adaptive CUDA v18: signal-aware optimizer-level predictive coding.
#
# The exact CUDA v16 TPO control/optimizer chassis is retained: K=8 raw-MuJoCo probes,
# one-sided dynamic KL, pure mirror descent, HL-Gauss critic, static compiled paths,
# CUDA-resident labels/indices, packed telemetry, async fail-loud clipping, and TF32.
# After the same actor/critic active-set projection, predictive Adam proposals receive
# 5% block trust if either task constraint conflicts, 10% if both are nonconflicting,
# and 15% only when both descent cosines exceed 0.1. A strict global 10% cap against
# the actual task-Adam trunk delta prevents aggregate representation overreach.
import os
import random
import time
...
class Args:
    ...
    # Actual-update trust. Descent cosine is -g_task.dot(delta_predictive),
    # normalized per logical block; positive values help the task to first order.
    prednext_conflict_ratio: float = 0.05
    prednext_nonconflict_ratio: float = 0.10
    prednext_strong_align_ratio: float = 0.15
    prednext_strong_align_cosine: float = 0.10
    prednext_global_ratio: float = 0.10
...
def _zero_admission_result(task_updates, layout, *, nonfinite_veto):
    ...
    return [torch.zeros_like(update) for update in task_updates], {
        ...
        "block_actor_descent_cosine": block_zeros,
        "block_critic_descent_cosine": block_zeros,
        "block_local_ratio": block_zeros,
        "block_strong_alignment": block_zeros.bool(),
...
@torch.no_grad()
def adaptive_block_trust_ratios(
    actor_dot,
    critic_dot,
    actor_sq,
    critic_sq,
    ...
    *,
    ...
    """Choose each logical block's actual-update allowance.

    ``actor_dot`` and ``critic_dot`` are task-loss-gradient dot predictive-update.
    Positive values conflict with descent. Descent cosine negates that convention, so
    positive cosine means the predictive parameter delta helps the task to first order.
    """
    if not 0.0 <= conflict_ratio <= nonconflict_ratio <= strong_align_ratio <= 1.0:
        raise ValueError(
            "adaptive trust ratios must satisfy 0 <= conflict <= ordinary <= strong <= 1"
        )
    if not -1.0 <= strong_align_cosine <= 1.0:
        raise ValueError("strong-alignment cosine threshold must lie in [-1, 1]")
    if not (
        actor_dot.shape
        == critic_dot.shape
        == actor_sq.shape
        ...
        raise ValueError("adaptive trust statistics must have identical block shapes")
    if len(
        {
            tensor.device
            for tensor in (
        ...
        raise ValueError("adaptive trust statistics must share one device")

    zero = torch.zeros_like(actor_dot)
    actor_denom = (actor_sq * predictive_sq).clamp_min(0.0).sqrt()
    critic_denom = (critic_sq * predictive_sq).clamp_min(0.0).sqrt()
    actor_descent_cosine = torch.where(
        (actor_sq > 0.0) & (predictive_sq > 0.0),
        -actor_dot / actor_denom.clamp_min(torch.finfo(actor_dot.dtype).tiny),
        zero,
    ).clamp(-1.0, 1.0)
    critic_descent_cosine = torch.where(
        (critic_sq > 0.0) & (predictive_sq > 0.0),
        -critic_dot / critic_denom.clamp_min(torch.finfo(critic_dot.dtype).tiny),
        zero,
    ).clamp(-1.0, 1.0)
    conflict = (actor_dot > 0.0) | (critic_dot > 0.0)
    strong_alignment = (
        ~conflict
        & (actor_descent_cosine > strong_align_cosine)
        & (critic_descent_cosine > strong_align_cosine)
    )
    local_ratio = torch.where(
        conflict,
        actor_dot.new_full(actor_dot.shape, conflict_ratio),
        torch.where(
            strong_alignment,
            actor_dot.new_full(actor_dot.shape, strong_align_ratio),
            actor_dot.new_full(actor_dot.shape, nonconflict_ratio),
        ),
    )
    return local_ratio, {
        "actor_descent_cosine": actor_descent_cosine,
        "critic_descent_cosine": critic_descent_cosine,
        "conflict": conflict,
...
@torch.no_grad()
def install_fail_closed_optimizer_gradients(parameters, gradients):
    """Pack and sanitize one all-or-none optimizer group with constant GPU work."""
    parameters = list(parameters)
    gradients = list(gradients)
    if not parameters or len(parameters) != len(gradients):
        raise ValueError("one optimizer gradient is required per parameter")
    reference_device = parameters[0].device
    reference_dtype = parameters[0].dtype
    for parameter, gradient in zip(parameters, gradients):
        if (
            parameter.shape != gradient.shape
            or parameter.device != reference_device
            or gradient.device != reference_device
            or parameter.dtype != reference_dtype
            or gradient.dtype != reference_dtype
        ):
            raise ValueError(
                "optimizer parameters and gradients must share shape, device, and dtype"
            )

    parameter_lengths = tuple(parameter.numel() for parameter in parameters)
    packed_gradient = torch.cat([gradient.reshape(-1) for gradient in gradients])
    finite = torch.isfinite(packed_gradient).all()
    # One scalar-indexed fill kernel sanitizes the packed buffer in place. This avoids
    # per-parameter isfinite/where/zeros kernels and avoids allocating a second full-size
    # zero buffer. Finite inputs are unchanged bit-for-bit; any bad element zeros the group.
    packed_gradient.masked_fill_(~finite, 0.0)
    for parameter, flat_gradient in zip(
        parameters, packed_gradient.split(parameter_lengths)
    ):
        parameter.grad = flat_gradient.view_as(parameter)
    return finite
...
def admit_predictive_updates(
    ...
    *,
    ...
    """Project a predictive Adam proposal, then apply adaptive actual-update trust."""
    ...
    if not 0.0 <= global_ratio <= 1.0:
        raise ValueError("global trust ratio must lie in [0, 1]")
    ...
    block_local_ratio, adaptive_stats = adaptive_block_trust_ratios(
        actor_pred,
        critic_pred,
        actor_sq,
        critic_sq,
        ...
        nonconflict_ratio=nonconflict_ratio,
        strong_align_ratio=strong_align_ratio,
        strong_align_cosine=strong_align_cosine,
    )
    ...
    local_scale = torch.clamp(
        block_local_ratio * task_norm_block
        / projected_norm_block.clamp_min(1e-20),
        max=1.0,
    )
    ...
    global_scale = torch.clamp(
        global_ratio * task_norm / local_norm.clamp_min(1e-20), max=1.0
    )
    ...
    stats = {
        ...
        "block_actor_descent_cosine": adaptive_stats["actor_descent_cosine"],
        "block_critic_descent_cosine": adaptive_stats["critic_descent_cosine"],
        "block_local_ratio": block_local_ratio,
        "block_strong_alignment": adaptive_stats["strong_alignment"],
        "block_lengths": block_lengths_tuple,
...
def apply_predictive_trunk_transaction(
    ...
    *,
    ...
    gradient_finite = install_fail_closed_optimizer_gradients(parameters, gradients)
    optimizer.step()
    raw_updates = [
    ...
    raw_max_norms = torch.stack(torch._foreach_norm(raw_updates, float("inf")))
    transaction_finite = gradient_finite & torch.isfinite(raw_max_norms).all()
    if bool(transaction_finite.item()):
...
def apply_private_optimizer_step(parameters, optimizer, gradients):
    """Apply one fail-closed private Adam transaction."""
    if not parameters or len(parameters) != len(gradients):
    ...
    optimizer.zero_grad(set_to_none=True)
    return update_sq.sqrt(), gradient_finite.to(dtype=update_sq.dtype)
...
if __name__ == "__main__":
    ...
    assert (
        0.0
        <= args.prednext_conflict_ratio
        <= args.prednext_nonconflict_ratio
        <= args.prednext_strong_align_ratio
        <= 1.0
    )
    assert -1.0 <= args.prednext_strong_align_cosine <= 1.0
    assert 0.0 <= args.prednext_global_ratio <= 1.0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.nextlat:
                    ...
                    agent.zero_grad(set_to_none=True)
                    predictor_step_norm, predictor_numeric_valid = (
                    ...
                    if actor_active:
                        (
                            ...
                            conflict_ratio=args.prednext_conflict_ratio,
                            nonconflict_ratio=args.prednext_nonconflict_ratio,
                            strong_align_ratio=args.prednext_strong_align_ratio,
                            strong_align_cosine=args.prednext_strong_align_cosine,
                            global_ratio=args.prednext_global_ratio,
                        )
                    else:
        ...
        if args.nextlat:
            device_telemetry.update(
                {
                    ...
                    "prednext/predictor_numeric_valid": predictor_numeric_valid,
                    ...
                    "prednext/block_strong_alignment_fraction": prednext_stats[
                        "block_strong_alignment"
                    ].float().mean(),
                    "prednext/local_ratio_mean": prednext_stats[
                        "block_local_ratio"
                    ].mean(),
                }
            )
            for block_index, block_name in enumerate(prednext_block_names):
                ...
                device_telemetry[
                    f"prednext_blocks/{block_name}_actor_descent_cosine"
                ] = prednext_stats["block_actor_descent_cosine"][block_index]
                device_telemetry[
                    f"prednext_blocks/{block_name}_critic_descent_cosine"
                ] = prednext_stats["block_critic_descent_cosine"][block_index]
                device_telemetry[
                    f"prednext_blocks/{block_name}_local_ratio"
                ] = prednext_stats["block_local_ratio"][block_index]
                device_telemetry[
                    f"prednext_blocks/{block_name}_strong_alignment"
                ] = prednext_stats["block_strong_alignment"][block_index]
