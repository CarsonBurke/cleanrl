# TPO-MD SPREAD-TEMP + DIFFUSIONBLOCKS POLICY HEAD v1.
# =====================================================================================
...
#                   L_b   = lambda(sigma) * [ CE(q, pi_{D_b}) + anchor*||D_b - z0||^2 ]
...
#       instead of predicting it -- the paper's normalize()-with-sigma_data=0.5 geometry.
...
# 3. LATENT ANCHOR: the per-block loss adds the paper's own Eq. (2) inner loss ||D-z0||^2 to
...
#    cos(D, z0) ~ 0.45 -- the ODE state leaves the latent manifold it is supposed to
#    integrate on. --dblock-latent-anchor 0 restores CE-only.
...
# policy that acts. losses/dblock_actor_ce_b{0,1,2} is each block's own CE; block 0 owns the
# LARGEST sigmas, so b0 == "predict the target policy from the observation alone" == the base
# head's job, and the series should be monotonically DECREASING in b. debug/tpo_kl_achieved
# and debug/tpo_cap_engaged must keep behaving as in v28: the trust region is unchanged, and
# if the ODE policy cannot track its own mirror-descent target the cap will stop binding.
#
...
class Args:
    ...
    dblock_latent_norm: float = 1.0   # ||phi(a)|| scale: sets how much of the target a block
                                      # can READ off its noised input instead of predicting
                                      # it from the observation. 1.0 == the paper's geometry
                                      # (normalize() with sigma_data left at 0.5).
    dblock_latent_anchor: float = 1.0 # weight of the EDM L2 term ||D-z0||^2 that pins the
                                      # ODE state to the latent manifold (see ActorHead)
    ...
    dblock_inference_steps: int = 0   # Euler ladder length; 0 => dblock_num_blocks
...
class ActorHead(nn.Module):
    ...
    TARGET POLICY at those frequencies. Properties carried over from the value side:
      (a) ||phi(a)|| == 1 EXACTLY for every action (cos^2 + sin^2 per row), so the
          ...
          input instead of predicting it is set by ONE number for every action, as the
          paper's normalize()-with-sigma_data geometry intends.
    ...
    local objective is introduced):
        L_b = lambda(sigma) * [ CE(q, log_softmax_i log pi_{D_b}(a_i|s)) + anchor*||D_b-z0||^2 ]
    ...
    (one extra squared distance, no second decode) and needs no invertible decode.
    ...
    def _ode_state(self, cond):
        ...
        Euler over ladder[:-1], then one final denoise at the smallest sigma."""
        N = cond.shape[0]
        ...
        for i, sigma in enumerate(self.ladder):
            ...
            if i + 1 == len(self.ladder):
                return D.squeeze(0)
            z = z + (self.ladder[i + 1] - sigma) * (z - D) / sigma
    ...
    def block_losses(self, cond, q, cand_z, block_ids, ce_fn):
        ...
        anchor = (D - z0).pow(2).mean(-1)                                 # (G,N)
        w = self.edm_weight(sigma.squeeze(-1))                            # (G,N)
        if self.weight_normalize:
            w = w / w.mean(dim=-1, keepdim=True).clamp_min(1e-8)
        loss = (w * (ce + self.anchor * anchor)).mean(-1)
        return loss, ce.mean(-1).detach()

    def e2e_ce(self, cond, ce_fn):
        """END-TO-END control loss: the base's TPO CE on the top-of-stack readout,
        backpropped through all B composed blocks. Same loss, same reduction."""
        return ce_fn(self.raw_params(cond)).mean()
...
if __name__ == "__main__":
    ...
    dblock_ce = [float("nan")] * agent.actor_head_net.num_blocks
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.dblock_actor:
                    ...
                    for b, ce in zip(block_ids.tolist(), per_block_ce.tolist()):
                        dblock_ce[b] = ce
                else:
        ...
        if args.dblock_actor:
            for b, ce in enumerate(dblock_ce):
