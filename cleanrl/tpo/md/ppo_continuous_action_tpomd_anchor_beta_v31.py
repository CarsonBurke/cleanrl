# TPO-MD SLOT-PRIOR EXPONENT v31: TPO's operator is variance-limited, so find the optimum.
#
# Chassis is v25 (all-layer residual-TD tape; best run in the repo, 11,015 @7.46M
# HalfCheetah).  One scalar changes: anchor = log_softmax(beta * log pi_old(a_i)) over the
# K=8 probed candidate slots.  beta=1 is v25.
#
# WHAT v30 ESTABLISHED.  The MPO/AWR derivation says the slot prior should be UNIFORM: the
# candidates are drawn from pi_old, so the sampling measure already supplies the pi_old
# factor of q ∝ pi_old*exp(u/eta), and v25's log_softmax(log pi_old) applies it a second
# time (effective prior pi_old^2).  v30 removed it, with v29's controller pinning the
# REALIZED policy KL at 0.033 so the step SIZE was matched to v25's early 0.027-0.031.
#
#   tpo_q_entropy   1.855-1.879 (v25: 1.54-1.58, ln 8 = 2.079)   <- change engaged
#   realized KL     0.030-0.039 (v25: 0.027-0.031)               <- size matched
#   return @~620k   708         (v25 ~2,880, v29 2,690)          <- 3.8x COLLAPSE
#
# The derivation is right and the estimator is unusable.  With K=8 samples from a 6-dim
# Beta whose log-probs spread ~1.7 nats, softmax(u/eta) alone lets one deep-tail sample own
# the target; its 1-step probe reward is exact but r + gamma*V(s') is a poor proxy for that
# action's long-run value, so the fit chases score error into the tail.  The pi_old factor
# is not an artefact -- it is the variance control that keeps effective sample size up and
# the target near the sampling distribution.  Bias is cheap here; variance is not.
#
# THE QUESTION THIS RAISES.  If pi_old^1 beats pi_old^0 by 4x, beta=1 is not obviously the
# optimum -- it is just the value that falls out of writing the anchor as a plain
# log_softmax.  Nothing has ever tested the beta>1 side, where the target is pulled further
# toward the sampling distribution and effective sample size rises further.  beta is a pure
# bias/ESS dial: low beta = aggressive, chases outliers; high beta = conservative, ignores
# tail candidates the probe measured exactly.  The trust region and the realized-KL
# controller bound the step SIZE independently, so beta only sets the DIRECTION.
#
# HYPOTHESIS.  The return is not monotone in beta and beta=1 is not the argmax; with step
# size held fixed by the controller, some beta > 1 buys return by spending tail-candidate
# influence on effective sample size.  Sweep beta in {0.5, 2.0} against the known beta=0
# (708) and beta=1 (2,690, same controller) points, judged at ~640k where beta=0 already
# separated by 3.8x.  FALSIFIERS: beta=2 <= beta=1 AND beta=0.5 <= beta=1 => beta=1 is the
# argmax, this dial is exhausted, and the lever moves to the SCORES themselves (a
# variance-limited operator wants a better r + gamma*V per candidate, e.g. a second exact
# physics step per candidate, not a more aggressive target).  Monotone increase through
# beta=2 => push beta higher and re-check that tpo_anchor_ess is not saturating at K.
# Watch debug/tpo_anchor_ess (effective slots, 1/sum p_i^2, in [1, 8]) and
# debug/tpo_q_entropy alongside return.
#
...
class Args:
    ...
    # v31: exponent on the slot prior, anchor = log_softmax(beta * log pi_old).
    #   1.0 = v25 · 0.0 = uniform / exact MPO E-step (falsified, 708 @640k) · >1 = untested.
    tpo_anchor_beta: float = 1.0
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v31: the slot prior is pi_old^beta, i.e. anchor = log_softmax(beta * log pi_old).
            # beta=1 is v25; beta=0 is v30's uniform (exact MPO E-step over pi_old samples),
            # which COLLAPSED to 708 @640k against v25's ~2,880 at matched realized KL. So the
            # extra pi_old factor is not a double-count artefact to be removed but a variance
            # control: it keeps the K-slot target near the sampling distribution and stops the
            # CE fit from chasing a lone deep-tail sample whose exact 1-step probe reward is a
            # poor proxy for its long-run value. beta therefore interpolates bias (low beta,
            # aggressive, high variance) against effective sample size (high beta, conservative).
            # beta > 1 is the untested side, and the beta=0 vs beta=1 gap says the gradient at
            # beta=1 may well still point upward.
            if args.tpo_anchor_beta == 0.0:
                anchor_logp = tpo_logprobs.new_full(
                    (args.batch_size, args.tpo_k), -log(float(args.tpo_k))
                )
            else:
                anchor_logp = F.log_softmax(
                    args.tpo_anchor_beta * tpo_logprobs.reshape(-1, args.tpo_k), dim=-1
                )
            anchor_ess = (
                1.0 / anchor_logp.exp().pow(2).sum(-1).clamp_min(1e-12)
            ).mean().item()
        ...
        for tag, value in {
            ...
            "debug/tpo_anchor_ess": anchor_ess,
