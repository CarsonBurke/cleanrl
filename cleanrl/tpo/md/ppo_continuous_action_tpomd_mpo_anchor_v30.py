# TPO-MD MPO-EXACT ANCHOR v30: stop double-counting pi_old in the target.
#
# Chassis is v25 (all-layer residual-TD tape; best run in the repo, 11,015 @7.46M
# HalfCheetah).  One object changes: the prior over the K candidate slots.
#
# THE DEFECT, FROM THE DERIVATION.  TPO-MD's target is q(a) ∝ pi_old(a) exp(u(a)/eta),
# fitted by weighted cross-entropy over the K probed candidates.  The candidates are
# SAMPLED FROM pi_old, so the sampling measure already supplies the pi_old factor.  The
# exact E-step is therefore
#
#     E_q[log pi] = Int pi_old(a) [exp(u/eta)/Z] log pi(a) da
#                ~= sum_i softmax(u_i/eta)_i log pi(a_i),      a_i ~ pi_old
#
# i.e. a UNIFORM prior over the sampled slots.  v25 instead builds the slot prior as
# log_softmax(log pi_old(a_i)), multiplying by pi_old a second time; the effective prior
# is pi_old^2.  With a 6-dim Beta policy the K sampled log-probs spread over ~1.7 nats, so
# this is not a small correction: it up-weights the near-mode candidates and discounts the
# tail candidates -- exactly the ones whose 1-step reward is measured EXACTLY by the MuJoCo
# probe and whose whole purpose is to find improvement outside the current mode.  It is a
# systematic pull toward the incumbent mode on top of the trust region, and it worsens as
# the policy sharpens (v25's entropy runs -0.66 -> -10.7).
#
# THE FIX.  anchor_logp = -log K.  A constant cancels inside both softmaxes, so
# tpo_restricted_target degenerates to softmax(u/eta) (the AWR/MPO weights) and the dual
# statistic becomes KL(uniform || q), still monotone decreasing in eta -- the bisection,
# the breaker and the CE loss are untouched.  `--tpo-anchor pi_old` recovers v25/v29.
#
# WHY THE v29 CONTROLLER STAYS ON, AND WHY IT IS NOT A CONFOUND.  Changing the anchor
# changes what eps MEANS: the surrogate KL is now measured from a uniform prior, not from
# pi_old, so a fixed surrogate cap would silently rescale the step and the comparison would
# confound DIRECTION with SIZE.  The v29 closed loop pins the REALIZED policy KL
# (max_epoch_approx_kl) at 0.033 -- v25's own value over its fastest-improving phase, and
# measured on v29 its value over 0-1M as well.  Both runs therefore take the same size step
# and differ only in where it points.  That is why it is included.
#
# HYPOTHESIS.  Removing the pi_old^2 prior lets the exactly-measured tail candidates carry
# their true weight, so the same-size step points somewhere better and the gain shows up
# IMMEDIATELY -- unlike v28/v29, whose effect is confined to the post-4M step-decay window
# and which were consequently untestable inside a 1-2M budget.  FALSIFIERS: no separation
# from v25 by 1M => the slot prior does not matter and the pi_old weighting was harmless;
# WORSE than v25 => the pi_old factor was doing useful conservative work beyond the trust
# region, and the next question is a partial exponent pi_old^beta, beta in (0,1), rather
# than either endpoint.  Watch debug/tpo_q_entropy: it should RISE toward ln 8 = 2.079
# against v25's 1.35-1.58, since the target is no longer pre-peaked by pi_old.
#
...
class Args:
    ...
    # v29 REALIZED-KL CONTROLLER. The dyn-trust cap regulates KL(p_old||q) in the
    # DISCRETE K-way simplex; the quantity that actually matters is the continuous
    # policy KL the CE fit realizes. On v25 those two decoupled by ~6x: the discrete
    # KL sat pinned at eps=0.03 all run while max_epoch_approx_kl fell 0.0339 (3.7M)
    # -> 0.0059 (7.4M). So the closed loop multiplies the scores by u_gain and adapts
    # u_gain in log space to hold the realized KL at tpo_kl_target. With the controller
    # on, tpo_eps is demoted to a loose surrogate guard (default 0.5) and the hard
    # safety is the unchanged 0.09 breaker on the realized epoch-mean KL.
    tpo_kl_control: bool = True
    tpo_kl_target: float = 0.033   # v25's realized KL during its fastest-improving phase (1.9M-3.7M)
    tpo_kl_gain: float = 0.3       # log-space controller gain per iteration (~244 iterations per 8M)
    tpo_u_gain_min: float = 0.05
    tpo_u_gain_max: float = 500.0
    # Surrogate-KL guard used INSTEAD of tpo_eps while the controller is on, so the
    # tpo_eps field keeps its exact v25 meaning and `--no-tpo-kl-control` is v25.
    tpo_eps_control: float = 0.5

    # v30: prior over the K sampled candidate slots inside the mirror-descent target.
    #   "uniform" (default) = exact MPO/AWR E-step; the pi_old factor comes from sampling.
    #   "pi_old"            = v25/v29 behaviour (candidate log-probs re-weight the slots).
    tpo_anchor: str = "uniform"
...
if __name__ == "__main__":
    ...
    # v29 closed-loop score gain. 1.0 == exact v25 scaling at iteration 0.
    u_gain = 1.0
    tpo_eps_eff = args.tpo_eps_control if args.tpo_kl_control else args.tpo_eps
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v29: u_gain is the CLOSED-LOOP score gain (== 1/temperature multiplier),
            # driven by the controller below so that the REALIZED continuous policy KL
            # tracks tpo_kl_target. The +-5 clamp is kept as the saturation guard: at
            # large gain the target degenerates to argmax over the K probed candidates,
            # which is the greedy limit of an exactly-measured 1-step score.
            u_scores = (
                u_gain * (scores - scores.mean(dim=-1, keepdim=True)) / tpo_sigma_global
            ).clamp(-5.0, 5.0)
            group_std = scores.std(dim=-1, unbiased=False)                   # (B,) diagnostics only
            # v30: the K candidates are ALREADY drawn from pi_old, so the sampling supplies
            # the pi_old factor of q ∝ pi_old * exp(u/eta). Weighting them by pi_old(a_i) a
            # SECOND time (v25's log_softmax of the candidate log-probs) is a double count:
            # the effective prior becomes pi_old^2, which over-weights near-mode candidates
            # and discounts exactly the tail candidates whose measured probe advantage is
            # the reason for probing. The exact MPO/AWR E-step over pi_old samples is
            #     E_q[log pi] ~= sum_i softmax(u_i/eta)_i * log pi(a_i),
            # i.e. a UNIFORM anchor over the sampled slots. A constant anchor cancels inside
            # both softmaxes, so tpo_restricted_target degenerates to softmax(u/eta) and the
            # dual statistic becomes KL(uniform || q) -- still monotone decreasing in eta, so
            # the bisection is unchanged.
            if args.tpo_anchor == "uniform":
                anchor_logp = tpo_logprobs.new_full(
                    (args.batch_size, args.tpo_k), -log(float(args.tpo_k))
                )
            elif args.tpo_anchor == "pi_old":
                anchor_logp = F.log_softmax(tpo_logprobs.reshape(-1, args.tpo_k), dim=-1)
            else:
                raise ValueError(f"unknown tpo_anchor {args.tpo_anchor!r}")
            ...
            if u_scores.abs().max().item() < 1e-8:
                ...
                if tpo_kl_base <= tpo_eps_eff:
                    ...
                    if tpo_mean_kl(float(np.exp(log_hi))) > tpo_eps_eff:
                        ...
                        for _ in range(40):
                            ...
                            if tpo_mean_kl(float(np.exp(log_mid))) > tpo_eps_eff:
                ...
                if tpo_mean_kl(float(np.exp(log_lo))) < tpo_eps_eff:
                    tpo_eta_solved = float(np.exp(log_lo))   # weak scores: even max-strength < eps
                elif tpo_mean_kl(float(np.exp(log_hi))) > tpo_eps_eff:
        ...
        # --- v29 realized-KL controller (one lagged multiplicative step per iteration) ---
        # Regulates the object that matters: the continuous policy KL the CE fit actually
        # realizes (max_epoch_approx_kl), not the discrete K-way surrogate. A breaker trip
        # means the step overshot, so it forces a decrease unconditionally -- without that
        # branch a trip would LOWER the measured KL and perversely raise the gain.
        if args.tpo_kl_control:
            tripped = actor_epochs_completed < args.update_epochs
            realized_kl = float(max_epoch_approx_kl.item())
            if tripped or actor_epochs_completed == 0:
                log_err = -1.0
            else:
                log_err = log(args.tpo_kl_target) - log(max(realized_kl, 1e-8))
                log_err = float(np.clip(log_err, -2.0, 2.0))
            u_gain = float(
                np.clip(
                    u_gain * float(np.exp(args.tpo_kl_gain * log_err)),
                    args.tpo_u_gain_min,
                    args.tpo_u_gain_max,
                )
            )
        ...
        for tag, value in {
            ...
            "debug/tpo_u_gain": u_gain,
            "debug/tpo_eps_eff": tpo_eps_eff,
