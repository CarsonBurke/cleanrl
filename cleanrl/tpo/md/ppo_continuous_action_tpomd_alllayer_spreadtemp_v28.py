# TPO-MD SPREAD-REFERENCED TEMPERATURE v28: fix the mirror-descent step's unit.
#
# Chassis is v25 (all-layer residual-TD tape, unchanged, still the best run in the repo:
# 11,015 @7.46M HalfCheetah).  ONE line of the TPO target changes: the return-unit scale
# that group-centered probe scores are divided by before the softmax.
#
# THE DEFECT, MEASURED ON v25's OWN LOGS.  TPO ranks K=8 probed candidate actions per
# state by score_i = r(s,a_i) + gamma*V(s'_i), centers within the group, then divides by
# ONE GLOBAL sigma so that cross-state advantage magnitude survives.  v25 sets that sigma
# to an EMA of the one-step TD-residual RMS.  Both objects carry return units, but they
# scale with different things:
#
#   within-group spread  = local Q-curvature across K actions at ONE state
#                          -> 1.39 / 1.68 / 1.65 at 0.2M / 3.7M / 7.4M   (FLAT)
#   td-residual RMS      = critic error across DIFFERENT states, grows with return scale
#                          -> 0.67 / 4.01 / 7.15 at 0.2M / 3.7M / 7.4M   (11x)
#
# So u_scores shrank ~9x (2.1 -> 0.23) purely because the agent got better at the task,
# and tpo_kl_base fell 1.37 -> 0.031 -- onto the eps=0.03 trust cap.  tpo_cap_engaged was
# 1.0 at EVERY iteration, i.e. the cap had been the binding constraint all run, and it
# stops binding right where v25's return curve flattens (10,026 @3.7M -> 11,313 @7.4M,
# +13% over 3.7M steps after +30% over the prior 1.8M).  Late training is step-starved by
# a scale drift that says nothing about how well the candidates are ranked.
#
# THE FIX.  Reference the quantity being ranked: sigma = EMA(batch-mean within-group score
# std).  Still ONE global divisor, so cross-state magnitude weighting is preserved exactly;
# now ||u|| stays O(1) for the whole run, tpo_kl_base stays well above eps, and the TRUST
# REGION governs the step at 8M as it did at 0.2M.  `--tpo-sigma-mode td_rms` recovers v25
# bit-for-bit, so the diff is one knob.
#
# HYPOTHESIS.  The second half of the curve is step-starved, not task-saturated: holding
# the KL cap binding to 8M lifts the final return above 11,015.  FALSIFIERS: tpo_kl_base
# no longer decaying yet return unchanged => v25's late flattening is genuine saturation
# and the trust cap is not the lever.  Return DROPS with a permanently-binding 0.03 cap =>
# the decay was acting as a useful late anneal; the next move is then an explicit eps
# schedule rather than a fixed cap.  Watch debug/tpo_spread_ema vs debug/tpo_td_rms_ema.
#
