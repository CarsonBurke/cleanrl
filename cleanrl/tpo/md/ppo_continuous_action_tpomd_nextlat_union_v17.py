# TPO-MD NEXTLAT UNION v17: simple same-Adam predictive-trunk composition.
#
# This is the simple extreme composition: TPO-MD v5 remains exact (K=8 raw-MuJoCo
# probes, anchored q, one-sided dynamic KL, pure actor CE, forked probe RNG, and its
# HL-Gauss critic). NextLat's v13 loss is unchanged: an absolute action-conditioned latent
# predictor, stopped online targets, and a fully frozen action decoder for both teacher and
# student. Its trunk gradient is independently clipped to 0.025 and added to TPO actor +
# critic gradients BEFORE their existing task-Adam step. The predictor keeps private Adam
# moments and its 0.25 clip. There is no predictive-trunk Adam, projection, live-head path,
# or post-step transaction: the isolated intervention is shared task-Adam moment mixing.
# When TPO's breaker disables actor learning, the auxiliary trunk path is suppressed so the
# policy truly freezes; the private predictor continues from detached features without a
# trunk backward/clip/clone scan. Forked predictor initialization preserves every base
# parameter/RNG state. Static forwards/projector use reduce-overhead CUDA graphs; retained
# outputs are cloned, labels/indices stay on CUDA, telemetry syncs once.
import os
import random
import time
...
class Args:
    ...
    # Depth-4 v13 NextLat loss. Only the trunk gradient joins task Adam; the predictor
    # remains private so the experiment isolates shared-trunk moment mixing.
    nextlat: bool = True
    nextlat_depth: int = 4
...
class Agent(nn.Module):
    ...
    def task_parameters(self):
        predictor_ids = {
            id(parameter) for parameter in self.nextlat_predictor.parameters()
        }
        return [
            parameter
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.nextlat:
            writer.add_scalar(
                "nextlat_union/predictor_only_delivery",
