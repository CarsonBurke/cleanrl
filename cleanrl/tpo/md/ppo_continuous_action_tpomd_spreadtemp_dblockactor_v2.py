# TPO-MD SPREAD-TEMP + DIFFUSIONBLOCKS POLICY HEAD v2 (CALIBRATED GEOMETRY).
# =====================================================================================
# DiffusionBlocks (ICLR 2026, arXiv:2506.14202) applied to the ACTOR, with a built-in
# CONTROL: --dblock-actor / --no-dblock-actor select block-local vs end-to-end training of
# the SAME policy head, so "is DiffusionBlocks helping" is one flag, not two codebases.
#
...
# block becomes a denoiser for one interval of the noise level. Denoisers at different
# noise levels have INDEPENDENT training problems (score matching factorizes over sigma),
# so each block trains alone on the FULL task loss applied to its own one-step denoise --
# never on another block's output. No backward through depth, no cross-block activations,
# every block's update computable in parallel. B=1 recovers ordinary training.
#
# WHY THE ACTOR IS WELL-POSED *HERE*, AND NOT UNDER PPO. Eq. (6) needs a per-example target
# y: it goes INSIDE the block's input (y + sigma*eps) and is the loss target, and block
# independence is derived from every block predicting the SAME y. A PPO actor has no y --
# only a sampled action carrying a scalar advantage -- so converting a PPO actor means
# inventing a target and replacing the ratio with advantage-weighted diffusion BC, i.e. a
# different algorithm. THIS chassis needs none of that: TPO-MD's actor loss already IS
#       CE(q, log_softmax_i log pi(a_i|s)),   q = softmax(anchor_logp + u_scores/eta)
# a cross-entropy against a FROZEN target distribution q over the K=8 probed candidates,
# solved once post-rollout by the mirror-descent temperature under its KL cap. That is
# exactly the paper's classification setting with q as the label distribution, so the port
# changes only HOW the head's gradient is scheduled -- the objective, the trust region, the
# probe machinery, the critic and the tape are the base's, untouched.
#
# THE ARCHITECTURE (identical parameters, count and readout in both regimes)
#   cond      = actor trunk output feature (H)          -- the paper's re-read input embedding
#   B blocks f_b, each L/B adaLN sub-layers deep, residual WITHIN the block, siblings ACROSS
#   decoder   = Linear(H, 2*act_dim, bias=False), ZERO-init, shared by every block
#             -> alpha, beta = 1 + softplus(raw), the base's Beta family unchanged, and
#                raw == 0 at init => alpha == beta == 1.693 for every state: a symmetric,
#                mean-zero policy, the same neutral start as the base's std=0.01 heads.
#   --no-dblock-actor  z_0 = z_start; z_b = z_{b-1} + f_b(z_{b-1}, cond, t(0)); raw = dec(z_B)
#                ONE backward through B composed blocks, loss = the base's TPO CE verbatim.
#                This is the base algorithm with a deeper policy head.
#   --dblock-actor     z0 = lat(q); block b owns the b-th EQUAL-MASS interval of the EDM
#                training lognormal and is trained ALONE on
#                   sigma ~ lognormal | bin_b ,  z_t = z0 + sigma*eps
#                   D_b   = c_skip z_t + c_out f_b(c_in z_t, cond, c_noise)    (EDM precond)
#                   L_b   = CE(q, pi_{D_b}) + anchor*(1+|W_dec|)*lambda(sigma)*||D_b - z0||^2
#                ACTING is the Euler solve of the same ODE from pure noise down the sigma
#                ladder, visiting the block that owns each sigma (6 steps, 2 per block).
#
# THE ACTION LATENT. The value-side port diffused lat(p) = p @ E with E a table of FIXED
# Fourier features of each value bin's coordinate. The actor analog replaces the 1-D bin
# coordinate with the A-dim action and the fixed table with a frozen multi-bandwidth random
# Fourier matrix:  phi(a) = sqrt(2/H)[cos(W u), sin(W u)], u = 2a-1;  z0 = norm * sum_i q_i phi(a_i).
# So z0 is the empirical CHARACTERISTIC FUNCTION of the target policy at H/2 frequencies:
#   (a) ||phi(a)|| == 1 EXACTLY for every action (cos^2+sin^2 per row), so one number sets
#       the discriminability that decides how much of the answer a block reads off its input
#       instead of predicting it.
#   (b) z0 is LINEAR in q, so a soft target maps to its exact conditional-mean latent and the
#       denoiser does progressive refinement of the target DISTRIBUTION, coarse frequencies
#       first. A diffuse q has a smaller-norm latent -- a decaying characteristic function is
#       exactly the signal that the target is spread out.
#
# v2 vs v1: THREE MEASURED GEOMETRY DEFECTS, all in the SAME direction -- v1's block-local
# regime was structurally unable to beat its own control, for reasons that had nothing to do
# with the paper's claim. Each was found by probing the head, not by reading it.
#   (i) THE LATENT WAS 5x TOO SMALL. EDM preconditions for a signal of per-COORDINATE std
#       sigma_data; ||phi(a)|| == 1 is a TOTAL norm over H coordinates, so v1's latent_norm=1
#       put the signal at 0.098/coordinate against sigma_data=0.5 and left EDM's own
#       normalization target E||F_target||^2/H at 0.32/0.73/0.94 for blocks 0/1/2 instead of
#       1. Every c_skip/c_out/c_in and the whole sigma range were calibrated for a signal
#       that was not there. latent_norm now AUTO-scales to sigma_data*sqrt(H); measured
#       per-coordinate std 0.39 and E||F_target||^2/H = 0.83/0.93/0.99.
#  (ii) THE 3-STEP LADDER MADE BLOCK 2 VESTIGIAL. With num_steps == num_blocks the ladder
#       ends at sigma_min, where c_out == 0.002, so the final block's learned function enters
#       the decoded latent with weight 0.002. Solving the Euler recursion for the net
#       coefficient on each block's F: shares 58.7 / 41.0 / 0.32 %. One THIRD of the head's
#       training compute moved the acting policy by nothing while still pushing gradient into
#       the shared trunk and decoder. dblock_inference_steps=6 gives 2 visits per block and
#       shares 53 / 32 / 15 %, and each block is now evaluated at several sigmas inside the
#       bin it trains on rather than one point.
# (iii) THE EDM WEIGHT WAS ON THE WRONG TERM (see block_losses). lambda belongs to the L2,
#       where lambda*c_out^2 == 1 makes it the unit-scale regression ||F-F_target||^2. On the
#       CE it had no such justification and collapsed block 2's effective sample size to 4.7%
#       of the minibatch. The CE is now unweighted, which also makes the per-block CE probes
#       comparable, and dblock_weight_normalize is gone.
# Two v1 findings were investigated and REJECTED as non-defects: the c_noise embedding's
# apparent near-orthogonality at sigma_max is an artifact of the randomly initialised
# TimeEmbed MLP, not of the frequency ladder (f_max=8 measures HIGHER cosine to block 0's
# training quantiles than f_max=2), and sigma_max=80 with negligible training density there
# is EDM's own standard sampler start.
#
...
#    free and sample efficiency is everything. Gradients stay strictly block-local either
#    way -- blocks are siblings, never composed -- so this is the same estimator, more
#    samples per step. Every parameter carries a leading (block, layer) axis and every op is
#    an einsum over the block axis, so all blocks train in ONE batched call.
# 2. COMMON RANDOM NUMBER for the ODE (one frozen seed, drawn at construction). pi(.|s) MUST
#    be a deterministic function of s: the rollout stores log pi and the anchor logprobs that
#    define q AND the trust region, and the update re-evaluates them. A resampled start would
#    make the two disagree and inject noise straight into the mirror-descent target.
# 3. LATENT ANCHOR: the per-block loss adds the paper's own Eq. (2) inner loss, weighted, to
#    the task CE. Measured need, from the value-side port: the CE reaches D only through
#    decoder(D), whose nullspace it cannot see, so CE alone let ||D|| inflate 80-140x with
#    cos(D, z0) ~ 0.45 -- the ODE state leaves the latent manifold it integrates on. The
#    coefficient TRACKS THE DECODER NORM, because the CE's pull on D scales with the decoder
#    gain while a fixed anchor does not: measured force ratio ||d(L2)/dD||/||d(CE)/dD|| for a
#    fixed coefficient decays 0.097 -> 0.001 between unit gain and gain 10, i.e. the guard
#    evaporates exactly as the policy sharpens and the decoder has to grow. With the tracking
#    the ratio is 0.25/0.44/2.16 at unit gain and 0.10/0.16/0.77 at gain 20 -- subordinate to
#    the task loss, and bounded away from zero. --dblock-latent-anchor 0 restores CE-only.
# 4. TIME-EMBEDDING FREQUENCY RANGE re-cut to [0.5, 8]: DiT's ladder is calibrated for
#    timesteps in [0, 1000], while c_noise = 0.25*log(sigma) spans only [-1.55, 1.10] here.
# 5. BLOCK OWNERSHIP is bucketed on the UNWIDENED equal-mass edges, like the reference
#    estimate_target_layer. The gamma-widened training bins overlap, so bucketing on those
#    misroutes any ladder sigma landing in an overlap to its larger-sigma neighbour.
#
# THE A/B, AND ITS ONE CONFOUND. Three arms, all seed 1 / HalfCheetah / 8M:
#   A  --no-dblock-actor                          control: 3 COMPOSED block calls
#   B  --dblock-actor                             method:  6 ODE block calls (2 per block)
#   C  --dblock-actor --dblock-inference-steps 3  method:  3 ODE block calls
# All three are EXACTLY parameter-matched: the ladder indexes params[block_id] and revisits
# the same B blocks, so more steps add zero parameters and zero per-block capacity. What 6
# steps DO add is acting-time compute -- measured 1270.7 us per readout against the control's
# 649.9 us, i.e. 1.96x -- so B vs A alone would confound "block-local training" with "twice
# the inference FLOPs". Hence C: A vs C is the FLOP-matched test of block-local training, and
# C vs B prices the extra ladder steps. The confound is small in wallclock (the step is
# rollout- and trunk-dominated; whole-run SPS is 619-634 for B against 644-657 for A, ~4%)
# but it is a real difference and the arms above decompose it rather than argue about it.
#
# WATCH: losses/tpo_ce is the COMPOSED (acting) ODE policy's CE and NOTHING optimizes it in
# the dblock regime -- it is the honest test of whether block-local training improves the
# policy that acts. losses/dblock_actor_ce_b{0,1,2} is each block's own CE, now an
# iteration-mean and, with the EDM weight off the CE, directly comparable across blocks;
# block 0 owns the LARGEST sigmas, so b0 == "predict the target policy from the observation
# alone" == the base head's job, and the series should be monotonically DECREASING in b.
# debug/tpo_kl_achieved and debug/tpo_cap_engaged must keep behaving as in v28: the trust
# region is unchanged, and if the ODE policy cannot track its own mirror-descent target the
# cap will stop binding. tape/actor_head_weight_norm is the DECODER norm (v28-comparable):
# it is the direct read on whether the gain-tracked anchor is holding the latent gauge.
#

# --- v25 chassis, unchanged below this line ---
...
# heads retain substantial detached TD-bootstrap mass (over half by head 16).
import copy
import os
import random
import time
...
def actor_head_weight_norm(agent) -> torch.Tensor:
    ...
    if agent.actor_dist == "gaussian":
        ...
        # The DECODER only. The block stack is 255k numbers against the decoder's 768, so
        # packing the whole head would swamp the signal this probe exists for -- a decoder
        # inflating while the latent shrinks -- and would break comparability with v28,
        # whose series covered exactly the readout weights.
        heads = (agent.actor_head_net.decoder,)
    return torch.cat(
...
class Args:
    ...
    tpo_sigma_scale_coef: float = 1.0  # global score sigma = coef * EMA(reference scale)
    # v28: which return-unit scale the group-centered probe scores are divided by.
    #   "group_spread" (default) = EMA of the batch-mean WITHIN-GROUP score std.
    #   "td_rms"                 = EMA of the one-step TD-residual RMS (exact v25).
    tpo_sigma_mode: str = "group_spread"
    ...
    # --- DiffusionBlocks POLICY head (arXiv:2506.14202) -------------------------------
    # THE A/B. Both settings use the SAME head architecture, the same parameter count and
    # the same TPO CE; only the way the head's gradient is scheduled changes.
    #   --dblock-actor     block-local: each block is an EDM denoiser of the TPO target's
    #                      action latent, trained ALONE on the full TPO CE of its own
    #                      one-step denoise. No backward through depth. Acting = ODE solve.
    #   --no-dblock-actor  the residual stack it converts, trained end-to-end == the base
    #                      algorithm exactly, with a deeper policy head (the control).
    dblock_actor: bool = True
    dblock_num_blocks: int = 3        # B: number of blocks. The ODE ladder length is
                                      # dblock_inference_steps, NOT B; block 0 owns the
                                      # LARGEST sigmas.
    # LAYERS PER BLOCK (L/B) -- the capacity of one Euler step. The paper's configs are 4
    # (ViT/DiT/MD4 at L=12,B=3), 8 (DiT-L/2) and 3 (AR LM, B=4); its Table 8 sweep at L=12
    # fixed reads FID 35.47 (L/B=6, beats end-to-end 39.83) < 38.03 (4) << 45.43 (3) <<
    # 53.32 (2), i.e. quality tracks per-block capacity, not the block count. The value-side
    # port ran L/B=1 and lost 32% return to its own end-to-end control; do not go below 2.
    ...
    dblock_latent_norm: float = 0.0   # 0 => AUTO = sigma_data * sqrt(hidden). EDM's whole
                                      # preconditioning assumes the diffused signal has
                                      # per-COORDINATE std sigma_data; ||phi(a)|| == 1 is a
                                      # TOTAL norm, so v1's 1.0 put the latent at 0.098 per
                                      # coordinate against sigma_data 0.5 and left
                                      # E||F_target||^2/H at 0.32/0.73/0.94 for blocks 0/1/2
                                      # instead of EDM's intended 1. Auto measures
                                      # 0.83/0.93/0.99.
    dblock_latent_anchor: float = 0.05 # weight of the EDM L2 term lambda*||D-z0||^2 that pins
                                      # the ODE state to the latent manifold, scaled by the
                                      # decoder gain so it cannot decay away (see ActorHead).
                                      # Calibrated by MEASURED force ratio in D-space,
                                      # ||d(L2)/dD|| / ||d(CE)/dD||: 0.25/0.44/2.2 for blocks
                                      # 0/1/2 at unit decoder gain. Subordinate to the task
                                      # loss where the CE has real pull, dominant only at
                                      # block 2's tiny sigmas where lambda ~ 1/sigma^2 and
                                      # matching z0 is nearly free anyway.
    ...
    dblock_gamma: float = 0.05        # widen each block's sigma bin by this fraction of its
                                      # log-range on both sides (paper default)
    dblock_inference_steps: int = 6   # Euler ladder length. NOT num_blocks: with a 3-step
                                      # ladder the final step's c_out == sigma_min == 0.002,
                                      # so block 2 contributed 0.32% of the decoded latent
                                      # (measured) and the "3-block" ODE was really 2 blocks.
                                      # 6 gives 2 visits per block and shares 53/32/15%.
    dblock_train_all_blocks: bool = True   # False => paper-faithful ONE block per minibatch
    dblock_rff_min: float = 0.5       # multi-bandwidth random-Fourier frequency range for
    dblock_rff_max: float = 8.0       # the action features phi(a)
...
# ---------------------------------------------------------------------------------------
# DiffusionBlocks (arXiv:2506.14202) machinery, ported from the value-side file. Verbatim:
# equal-mass sigma partitioning, the DESCENDING inference ladder, the DiT timestep embedder
# with the corrected [0.5, 8] frequency range, and the block stack whose every parameter
...
class ActorHead(nn.Module):
    """The POLICY head: ONE architecture, TWO training regimes selected by args.dblock_actor.

    Replaces the base's two Linear(H, act_dim) Beta-concentration heads with `num_blocks`
    blocks over an ACTION-DISTRIBUTION latent, read out by a shared zero-init decoder.

    WHAT MAKES THIS WELL-POSED ON AN ACTOR. DiffusionBlocks needs a per-example TARGET y:
    Eq. (6) puts y inside the block's input (y + sigma*eps) and uses it as the loss target,
    and block independence follows from every block predicting the SAME y. A PPO actor has
    no y -- only a sampled action with a scalar advantage -- which is why the value-side port
    left the actor alone. THIS chassis is different: TPO-MD's actor loss is already
        CE(q, log_softmax_i log pi(a_i|s))
    a cross-entropy against a FROZEN target distribution q over the K probed candidates
    (solved once post-rollout by the mirror-descent temperature). That is exactly the
    paper's classification setting, with q playing the role of the label distribution.

    THE ACTION LATENT. The value-side file diffused lat(p) = p @ E, E a table of FIXED
    Fourier features of each bin's coordinate -- the characteristic function of the target
    value distribution sampled at fixed frequencies, hence LINEAR in p. The actor analog,
    with the 1-D bin coordinate replaced by the A-dim action:
        phi(a) = sqrt(2/H) [cos(W u), sin(W u)],  u = 2a_native - 1 in [-1,1]^A
        z0     = dblock_latent_norm * sum_i q_i phi(a_i)
    W is a FROZEN multi-bandwidth random-Fourier matrix, so phi is a Monte-Carlo sample of
    the characteristic function and z0 is the empirical characteristic function of the
    TARGET POLICY at those frequencies. Properties carried over from the value side:
      (a) ||phi(a)|| == 1 EXACTLY for every action (cos^2 + sin^2 per row), so the
          discriminability that decides how much of the answer a block can read off its
          input instead of predicting it is set by ONE number for every action.
      (b) z0 is LINEAR in q, so a soft target maps to its exact conditional-mean latent and
          the denoiser is doing progressive refinement of the target DISTRIBUTION, coarse
          frequencies first. A concentrated q has a larger-norm latent than a diffuse one --
          the characteristic function decaying is the signal that the target is spread out.
    dblock_latent_norm AUTO-scales to sigma_data*sqrt(H), because (a) is a TOTAL norm while
    every EDM preconditioner is written for a signal of that PER-COORDINATE std. v1 left it
    at 1.0, which diffused a signal 5x smaller than the schedule assumed.

    THE PER-BLOCK LOSS is the FULL task loss on the block's own one-step denoise, as the
    paper requires (its whole claim against Forward-Forward/NoProp is that no surrogate
    local objective is introduced):
        L_b = CE(q, log_softmax_i log pi_{D_b}(a_i|s))
              + anchor*(1 + ||W_dec||) * lambda(sigma) * ||D_b - z0||^2
    The L2 term is the paper's own inner loss (Eq. 2) and it is here for a MEASURED reason:
    the CE reaches D only through decoder(D) -> softplus -> Beta, which is nearly flat in
    the component of D that the decoder's nullspace absorbs, so CE alone does not pin the
    ODE state to the latent manifold. On the value side the same hole showed up as ||D||
    inflating 80-140x with cos(D, z0) ~ 0.45 and had to be closed by reprojecting the Euler
    state through the decoded distribution. Anchoring D directly is the cheaper fix here
    (one extra squared distance, no second decode) and needs no invertible decode. lambda
    sits on the L2 and NOT on the CE (see block_losses), and the coefficient tracks the
    decoder norm so the guard does not evaporate as the policy sharpens.

    args.dblock_actor == False  (END-TO-END CONTROL, == the base algorithm, deeper head)
        z_0 = z_start; z_b = z_{b-1} + F_b(z_{b-1}, cond, t(0)); raw = decoder(z_B)
        ONE backward through all B composed blocks, loss = the base's TPO CE, unmodified.
    args.dblock_actor == True   (DIFFUSIONBLOCKS)
        Blocks are siblings, never composed; ACTING is the Euler solve of the probability-
        flow ODE from pure noise down the sigma ladder, visiting the block owning each sigma.

    COMMON RANDOM NUMBER. The ODE's start state is a single frozen z_seed drawn at
    construction, so pi(.|s) is a DETERMINISTIC function of s. This is not optional: the
    rollout stores log pi and the anchor logprobs that define q and the trust region, and
    the update re-evaluates them; a resampled start would make the two disagree and inject
    noise straight into the mirror-descent target.
    """

    def __init__(self, H, act_dim, args):
        super().__init__()
        self.H, self.act_dim = H, act_dim
        self.dblock = args.dblock_actor
        ...
        # EDM preconditions for a signal whose PER-COORDINATE std is sigma_data. ||phi(a)||
        # is 1 as a TOTAL norm over H coordinates, so the latent must be scaled by sqrt(H)
        # to land there; without it every c_skip/c_out/c_in and the whole sigma range are
        # calibrated for a signal 5x larger than the one actually being diffused.
        self.latent_norm = args.dblock_latent_norm or args.dblock_sigma_data * sqrt(H)
        self.anchor = args.dblock_latent_anchor
        # ---- EDM schedule: ASCENDING sigma endpoints carving the training lognormal into
        # num_blocks EQUAL-PROBABILITY-MASS bins. Block 0 owns the LARGEST sigmas, so the
        # descending inference ladder visits 0, 1, ... in order and block 0's job is
        # "predict the target policy from the observation alone" == the base head's job.
        ...
        # The ladder is FIXED at construction, so everything about the solve except the block
        # calls themselves is a constant: precompute the c_noise inputs (one batched t_embed
        # call for the whole ladder instead of one 1-element MLP forward per step), the owning
        # block ids, the EDM preconditioners and the Euler coefficients. _ode_state runs on
        # every rollout step and every minibatch policy forward, so this removes S device
        # allocations, S-1 kernel launches and a python bucket search from each of them.
        self.register_buffer(
            "ladder_cnoise", torch.tensor([0.25 * log(s) for s in self.ladder])
        )
        self.ladder_precond = []
        ...
        # z <- z + (sigma_next - sigma)*(z - D)/sigma  ==  (1+d)*z - d*D
        self.ladder_dt = [
            (self.ladder[i + 1] - self.ladder[i]) / self.ladder[i]
            for i in range(len(self.ladder) - 1)
        ]
        self.register_buffer(
            "ladder_ids",
            torch.tensor([self.block_for_sigma(s) for s in self.ladder], dtype=torch.long),
        )
        # Per-block sigma bin, WIDENED by dblock_gamma of its log-range on both sides
        # (reference get_weights/model.py:161-169) and clamped to the support, so the
        # inference ladder's endpoints are not sitting exactly on a bin edge.
        lo_hi = []
        ...
        self.register_buffer(
            "cdf_lo",
            torch.tensor([_cdf_scalar((log(lo) - self.p_mean) / self.p_std) for lo, _ in self.block_bins]),
        )
        self.register_buffer(
            "cdf_hi",
            torch.tensor([_cdf_scalar((log(hi) - self.p_mean) / self.p_std) for _, hi in self.block_bins]),
        )
        # ---- FROZEN multi-bandwidth random Fourier features of the action.
        n_freq = H // 2
        scales = torch.logspace(
            log(args.dblock_rff_min) / log(10.0), log(args.dblock_rff_max) / log(10.0), n_freq
        )
        self.register_buffer(
            "rff_w", torch.randn(n_freq, act_dim) * scales.unsqueeze(-1) / sqrt(act_dim)
        )
        self.t_embed = TimeEmbed(H)
        self.blocks = BlockStack(
            self.num_blocks, args.dblock_layers_per_block, H, args.dblock_mult
        )
        # Shared readout -> (alpha_raw, beta_raw). ZERO-init, so raw == 0 at init and
        # alpha == beta == 1 + softplus(0) == 1.693 for every state: a symmetric, mean-zero
        # action distribution, the same neutral start as the base's std=0.01 linear heads.
        self.decoder = nn.Linear(H, 2 * act_dim, bias=False)
        with torch.no_grad():
            self.decoder.weight.zero_()
        # Start-of-stack state for the e2e control (unused by the ODE, which starts from
        # noise). MUST be nonzero: with a zero start and a zero-init decoder the control
        # sits on an all-zero fixed point and receives exactly zero gradient forever.
        self.z_start = nn.Parameter(torch.randn(H))
        self.register_buffer("z_seed", torch.randn(1, H))   # frozen CRN for the ODE

    # ---- latent -----------------------------------------------------------------------
    def action_features(self, z_native):
        """(..., A) native Beta sample in (0,1) -> (..., H) unit-norm Fourier features."""
        ang = (2.0 * z_native - 1.0) @ self.rff_w.t()
        return sqrt(2.0 / self.H) * torch.cat([ang.cos(), ang.sin()], dim=-1)

    def latent_of(self, q, cand_z):
        """(N,K) target distribution + (N,K,A) candidates -> (N,H). LINEAR in q."""
        return self.latent_norm * torch.einsum("nk,nkh->nh", q, self.action_features(cand_z))

    # ---- schedule helpers -------------------------------------------------------------
    def block_for_sigma(self, sigma):
        """Which block OWNS this sigma. Buckets on the UNWIDENED equal-mass edges, like the
        reference estimate_target_layer (model.py:172-179). The gamma-widened bins OVERLAP,
        so walking them instead hands a sigma to the larger-sigma neighbour whenever it falls
        in the overlap: at a 6-step ladder that misroutes 0.408 and 0.222 to blocks 0 and 1
        rather than the blocks whose bins actually contain them."""
        # self.edges is ASCENDING and block 0 owns the LARGEST sigmas.
        for b in range(self.num_blocks):
    ...
    def denoise(self, cond, zt, sigma, ids):
        """EDM-preconditioned denoise: D_theta(zt; cond, sigma), the block's estimate of z0."""
        sd = self.sigma_data
        s2 = sigma**2 + sd**2
    ...
    def raw_params(self, cond):
        """(..., H) actor feature -> (..., 2*A) Beta raw concentrations. The ONLY policy
        readout: rollout, update, probes and the trust-region statistics all come through
        here, so the network is always evaluated as it is actually trained. Leading dims are
        flattened for the solve -- callers pass (B,H) on the task path and (T,B,H) from the
        frozen-policy tape tail, and the ODE is per-state either way."""
        lead = cond.shape[:-1]
        flat = cond.reshape(-1, self.H)
        z = self._ode_state(flat) if self.dblock else self._e2e_state(flat)
        return self.decoder(z).view(*lead, 2 * self.act_dim)

    # `forward` is the readout, so torch.func.functional_call can run this head with
    # detached parameters (the frozen-head decode path) without duplicating any arithmetic.
    forward = raw_params

    def _e2e_state(self, cond):
        N = cond.shape[0]
        t_emb = self.t_embed(torch.zeros(N, device=cond.device)).unsqueeze(0)   # sigma == 1
        z = self.z_start.expand(N, self.H).unsqueeze(0)
    ...
    def _ode_state(self, cond):
        ...
        Euler over ladder[:-1], then one final denoise at the smallest sigma. Everything but
        the block calls is precomputed at construction (see __init__), so the only per-step
        work is the block itself."""
        N = cond.shape[0]
        z = (self.z_seed * self.ladder[0]).expand(N, self.H).unsqueeze(0)
        t_all = self.t_embed(self.ladder_cnoise).view(-1, 1, 1, self.H)
        last = len(self.ladder) - 1
        for i in range(last + 1):
            c_skip, c_out, c_in = self.ladder_precond[i]
            D = c_skip * z + c_out * self.blocks(
    ...
    # ---- training --------------------------------------------------------------------
    def block_losses(self, cond, q, cand_z, block_ids, ce_fn):
        """Per-block DiffusionBlocks losses, ALL BLOCKS IN ONE BATCHED CALL. Returns
        (losses (G,), ce_probe (G,)): block b's loss touches ONLY block b's parameters
        (plus the shared conditioning trunk and decoder, which the paper also trains from
        every block). ce_fn(raw) -> (N,) is the base's TPO cross-entropy, unmodified.

        WEIGHTING (v2). The EDM weight lambda(sigma) = (sigma^2+sd^2)/(sigma*sd)^2 belongs to
        the L2 term and ONLY there: the identity lambda*c_out^2 == 1 is what turns
        lambda*||D-z0||^2 into the unit-scale regression ||F - F_target||^2, which is why that
        term needs no per-block normalization at all. Carrying the same lambda on the CE has
        no such identity behind it and is actively harmful -- lambda spans [23.8, 2.5e5]
        inside block 2's bin, which collapsed block 2's effective sample size to 4.7% of the
        minibatch (one draw carrying a median 9.4% of its whole loss) and put the 100x
        amplification exactly on the draws where z_t ~= z0 and the CE carries no signal. So
        the CE enters UNWEIGHTED. Two consequences beyond the variance: the per-block CE
        probes become directly comparable across blocks (they are the same quantity now), and
        dblock_weight_normalize -- which rescued the magnitude but not the relative weighting
        -- is gone."""
        N = cond.shape[0]
        G = block_ids.shape[0]
        z0 = self.latent_of(q, cand_z).unsqueeze(0)                       # (1,N,H)
        sigma = self.sample_sigma(block_ids, (G, N, 1), cond.device)      # (G,N,1)
        zt = z0 + sigma * torch.randn(G, N, self.H, device=cond.device)
        D = self.denoise(cond, zt, sigma, block_ids)                      # (G,N,H)
        ce = torch.stack([ce_fn(self.decoder(D[g])) for g in range(G)])   # (G,N)
        # lambda * mean_h (D-z0)^2 == ||F - F_target||^2 / H, sigma-stable by construction.
        l2 = self.edm_weight(sigma.squeeze(-1)) * (D - z0).pow(2).mean(-1)
        # The CE's pull on D grows with the decoder gain, so a FIXED anchor coefficient decays
        # as ~gain^-1 with NO floor (measured force ratio for block 0: 0.216 at unit gain,
        # 0.0055 at gain 20, 1e-5 at gain 20000) and the guard evaporates exactly as the
        # policy sharpens and the decoder has to grow. Multiplying by the DETACHED decoder
        # norm cancels that: the ratio falls 0.43 -> 0.12 over gain 1..20 and then FLATTENS
        # onto a strictly positive floor, 0.061/0.110/0.461 for blocks 0/1/2, flat to three
        # digits from gain 1000 to 20000. Bounded away from zero for any decoder norm.
        gain = self.decoder.weight.detach().norm()
        loss = (ce + (self.anchor * (1.0 + gain)) * l2).mean(-1)
        return loss, ce.mean(-1).detach()
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if self.actor_dist == "gaussian":
            ...
            # DIFFUSIONBLOCKS policy head: the base's two Linear(H, act_dim) concentration
            # heads are replaced by a stack of independently trained EDM denoisers over an
            # action-distribution latent, read out by one shared zero-init decoder. Both
            # regimes (block-local / end-to-end) live in this one module; alpha, beta =
            # 1 + softplus(raw) is unchanged, so the distribution family is the base's.
            self.actor_head_net = ActorHead(H, act_dim, args)
            self.register_buffer(
        ...
        if self.tape_enabled:
            ...
            if self.actor_dist == "gaussian":
                ...
                self.target_actor_head_net = copy.deepcopy(self.actor_head_net)
    ...
    def _actor_dist(self, actor_feat):
        ...
        # beta
        alpha_raw, beta_raw = self.actor_head_net.raw_params(actor_feat).chunk(2, dim=-1)
    ...
    def _actor_dist_frozen_head(self, actor_feat):
        ...
        # Detached-parameter functional call: identical arithmetic to the live head with
        # every head weight cut out of the graph, so the gradient reaches only actor_feat.
        # (The base spelled this out as F.linear(..., w.detach()); the head is no longer a
        # single Linear, so do it once for the whole module.)
        raw = torch.func.functional_call(
            self.actor_head_net,
            (
                {k: v.detach() for k, v in self.actor_head_net.named_parameters()},
                dict(self.actor_head_net.named_buffers()),
            ),
            (actor_feat,),
            {},
        )
    ...
    def actor_parameters(self):
        ...
        if self.actor_dist == "gaussian":
            ...
            # Every block, the timestep embedder, the shared decoder and the e2e start state
            # sit in the ONE actor clip group, exactly as the two linear heads they replace.
    ...
    def target_bundle_modules(self):
        ...
        if self.actor_dist == "gaussian":
            ...
            modules.append(self.target_actor_head_net)
    ...
    def _target_actor_dist(self, actor_feature):
        ...
        raw = self.target_actor_head_net.raw_params(actor_feature)
        alpha_raw, beta_raw = raw.chunk(2, dim=-1)
        alpha = 1.0 + F.softplus(alpha_raw)
        beta = 1.0 + F.softplus(beta_raw)
        return (
    ...
    def snapshot_tape_target(self):
        ...
        if self.actor_dist == "gaussian":
            ...
            self.target_actor_head_net.load_state_dict(self.actor_head_net.state_dict())
    ...
    def target_snapshot_lag(self):
        ...
        if self.actor_dist == "gaussian":
            ...
            pairs.append((self.actor_head_net, self.target_actor_head_net))
...
def policy_model_forward(agent, observations):
    ...
    if agent.actor_dist == "gaussian":
        ...
        alpha_raw, beta_raw = agent.actor_head_net.raw_params(actor_output).chunk(2, dim=-1)
        first = 1.0 + F.softplus(alpha_raw)
        second = 1.0 + F.softplus(beta_raw)
...
if __name__ == "__main__":
    ...
    spread_ema = None  # EMA (decay 0.99) of the batch-mean within-group score spread
    ...
    assert args.actor_dist == "beta", (
        "the DiffusionBlocks policy head replaces the Beta concentration heads; "
        "the gaussian path is the base's untouched linear head"
    )
    # Every block's id, for the one batched all-blocks training call.
    all_block_ids = torch.arange(agent.actor_head_net.num_blocks, device=device)
    # Per-block CE accumulated on DEVICE over each iteration and reduced once at logging:
    # no per-minibatch host sync, and an iteration MEAN rather than whichever minibatch
    # happened to be last (with --no-dblock-train-all-blocks, a block that no minibatch
    # sampled would otherwise re-log the previous iteration's value).
    nb = agent.actor_head_net.num_blocks
    dblock_ce_sum = torch.zeros(nb, device=device)
    dblock_ce_n = torch.zeros(nb, device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            group_std = scores.std(dim=-1, unbiased=False)                   # (B,)
            # v28: the temperature reference is the WITHIN-GROUP score spread, not the
            # global TD-residual RMS. Both quantities carry return units, but they scale
            # with different things: group spread measures local Q-curvature across K
            # candidate actions at ONE state (flat over training, 1.4-1.7 raw units),
            # while td_rms tracks the critic's error across DIFFERENT states and grows
            # with the return scale (0.67 -> 7.15 over v25's 8M run). Dividing a
            # scale-free ranking signal by the latter shrank u_scores ~9x and drove
            # tpo_kl_base 1.37 -> 0.031, i.e. down to the eps=0.03 trust cap, so the cap
            # stopped binding right where v25's return curve flattened. Referencing the
            # spread keeps ||u|| ~ O(1) for the whole run, so the TRUST REGION -- not an
            # incidental scale drift -- governs the mirror-descent step at every step.
            spread_batch = group_std.mean().item()
            spread_ema = (
                spread_batch if spread_ema is None else 0.99 * spread_ema + 0.01 * spread_batch
            )
            if args.tpo_sigma_mode == "group_spread":
                tpo_sigma_global = max(args.tpo_sigma_scale_coef * spread_ema, 1e-6)
            elif args.tpo_sigma_mode != "td_rms":
                raise ValueError(f"unknown tpo_sigma_mode {args.tpo_sigma_mode!r}")
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # In BOTH regimes this is the composed policy's CE -- in the dblock regime
                # nothing optimizes it directly, which makes it the honest diagnostic for
                # whether block-local training actually improves the acting ODE policy.
                mb_logp_new = F.log_softmax(new_cand_logprobs, dim=-1)
                tpo_ce = (-(b_tpo_q[mb_inds] * mb_logp_new).sum(-1)).mean()
                mb_q = b_tpo_q[mb_inds]
                mb_cand_z = b_tpo_zs[mb_inds]
                if args.dblock_actor:
                    # DIFFUSIONBLOCKS actor loss: the SAME TPO CE -- same frozen targets q,
                    # same K candidates, same reduction -- but each block optimizes it on ITS
                    # OWN one-step EDM denoise of the target policy's action latent, at a
                    # sigma drawn from its own equal-mass bin. The blocks are siblings, never
                    # composed, so no gradient crosses a block boundary and the actor's
                    # backward never traverses the head's depth. The loss is the MEAN over
                    # blocks, so its magnitude stays comparable to the control's single CE.
                    def dblock_ce_fn(raw):
                        alpha_raw, beta_raw = raw.chunk(2, dim=-1)
                        block_dist = Beta(
                            1.0 + F.softplus(alpha_raw), 1.0 + F.softplus(beta_raw)
                        )
                        # (K,N,A) so the (N,A) batch shape broadcasts, then back to (N,K).
                        cand_logp = (
                            block_dist.log_prob(mb_cand_z.transpose(0, 1))
                            .sum(-1)
                            .transpose(0, 1)
                        )
                        return -(mb_q * F.log_softmax(cand_logp, dim=-1)).sum(-1)

                    block_ids = (
                        all_block_ids
                        if args.dblock_train_all_blocks
                        else all_block_ids[
                            torch.randint(0, agent.actor_head_net.num_blocks, (1,), device=device)
                        ]
                    )
                    # mb_actor_feat is the FULL per-layer stack (B, num_layers, H) whenever
                    # the tape is on; the head conditions on the OUTPUT layer, exactly the
                    # feature policy_model_forward reads out.
                    mb_cond = (
                        mb_actor_feat[..., -1, :] if agent.tape_enabled else mb_actor_feat
                    )
                    per_block_loss, per_block_ce = agent.actor_head_net.block_losses(
                        mb_cond, mb_q, mb_cand_z, block_ids, dblock_ce_fn
                    )
                    actor_loss = args.tpo_coef * per_block_loss.mean()
                    # Accumulate on-DEVICE. A .tolist() here would sync the host every
                    # minibatch (~11% of the step in a launch-bound H=64 run) for a number
                    # the writer only needs once per iteration; the running mean over the
                    # whole iteration is also a far less noisy monotonicity probe than one
                    # arbitrary minibatch, and cannot re-log a stale block.
                    dblock_ce_sum.index_add_(0, block_ids, per_block_ce.detach())
                    dblock_ce_n.index_add_(
                        0, block_ids, torch.ones_like(per_block_ce, dtype=dblock_ce_n.dtype)
                    )
                else:
                    # PURE mirror descent: the CE is the entire actor objective (the base).
                ...
                actor_grads = {}
                if actor_active:
                    # Pure TPO CE remains the entire actor backward. Skip the entropy term
                    # when its coefficient is exactly zero: with the block-diffusion head
                    # that graph is the full 3-step Euler ODE plus a second trunk backward
                    # (~26% of the actor step) accumulating exact zeros, and 0 * inf would
                    # inject a NaN into the clip.
                    if ent_coef_eff == 0.0:
                        actor_loss.backward()
                    else:
        ...
        for tag, value in {
            ...
            "debug/tpo_spread_ema": spread_ema,
            "debug/tpo_td_rms_ema": td_rms_ema,
        ...
        # DiffusionBlocks probes. dblock_actor_ce_b{b} is block b's UNWEIGHTED TPO CE on its
        # own one-step denoise. Block 0 owns the LARGEST sigmas, i.e. "predict the target
        # policy from the observation alone" == exactly the base head's job, and each later
        # block refines a cleaner target latent, so the series should be monotonically
        # DECREASING in b. Compare the whole series against losses/tpo_ce, which is the
        # COMPOSED (acting) ODE policy's CE: block-local training only pays off if driving
        # the per-block series down also drives the composed CE down.
        if args.dblock_actor:
            # NaN, not 0.0, for a block no minibatch sampled: 0.0 is a plausible CE and would
            # read as a block that had solved its task. Only reachable under
            # --no-dblock-train-all-blocks.
            means = torch.where(
                dblock_ce_n > 0,
