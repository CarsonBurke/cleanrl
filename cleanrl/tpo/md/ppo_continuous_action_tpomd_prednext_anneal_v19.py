# TPO-MD PredNext Anneal v19: retain the early 5% gain, remove late trunk drift.
#
# v13's 5%-of-task-step predictive trunk admission is +645 return at 2M versus TPO,
# but its edge decays to -5 by 3.5M as prediction/KL losses rise and raw task cosine
# stays near zero. v19 changes exactly one causal lever: keep 0.05 through rollout 64
# (2,097,152 steps), half-cosine only the admitted trunk delta to zero over 32 rollouts,
# and leave TPO, targets, frozen decoder, predictive/private Adam states, and private
# predictor learning untouched. At zero the task trunk update is exact, while continuing
# the predictor preserves useful dynamics learning and makes this a clean representation-
# intervention schedule. The optimized
# v16 chassis remains: RNG-free reduce-overhead graphs, retained-output clones, CUDA-
# resident labels/indices, packed telemetry, async finite checks, and TF32-high matmuls.
import os
import random
import time
...
def scheduled_prednext_trust_ratio(
    global_step,
    base_ratio,
    anneal_start,
    anneal_end,
    ...
    """Deterministically anneal only the admitted predictive-trunk trust ratio."""
    if base_ratio < 0.0:
        raise ValueError("base trust ratio must be non-negative")
    if anneal_start < 0 or anneal_end <= anneal_start:
        raise ValueError("anneal bounds must satisfy 0 <= start < end")
    if shape not in ("cosine", "linear"):
        raise ValueError("anneal shape must be 'cosine' or 'linear'")
    if global_step <= anneal_start:
        return float(base_ratio)
    if global_step >= anneal_end:
        return 0.0
    progress = (float(global_step) - anneal_start) / (anneal_end - anneal_start)
    multiplier = (
        0.5 * (1.0 + cos(pi * progress))
        if shape == "cosine"
        else 1.0 - progress
    )
    return float(base_ratio) * multiplier


def rollout_prednext_trust_ratio(
    rollout_start_step,
    rollout_size,
    base_ratio,
    anneal_start,
    ...
    """Schedule an update by the completed rollout step it is about to optimize."""
    if rollout_size <= 0:
        raise ValueError("rollout size must be positive")
    return scheduled_prednext_trust_ratio(
        rollout_start_step + rollout_size,
        base_ratio,
        anneal_start,
        anneal_end,
        shape,
...
class Args:
    ...
    # Full v13 admission early; only the admitted trunk delta fades. Predictor
    # learning and both predictive Adam state machines remain live after the fade.
    prednext_anneal_start: int = 2_097_152  # 64 full 32,768-step rollouts
    prednext_anneal_end: int = 3_145_728  # 32-rollout half-cosine fade
    prednext_anneal_shape: str = "cosine"  # "cosine" | "linear"
...
if __name__ == "__main__":
    ...
    # Central validation uses the same helper as training, avoiding divergent CLI
    # and runtime interpretations of schedule boundaries/shapes.
    scheduled_prednext_trust_ratio(
        0,
        args.prednext_trust_ratio,
        args.prednext_anneal_start,
        args.prednext_anneal_end,
        args.prednext_anneal_shape,
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        effective_prednext_trust_ratio = rollout_prednext_trust_ratio(
            global_step,
            args.batch_size,
            args.prednext_trust_ratio,
            args.prednext_anneal_start,
                ...
                if args.nextlat:
                    ...
                    if actor_active:
                        (
                            ...
                            nextlat_trunk_grads,
                            task_updates,
                            effective_prednext_trust_ratio,
            ...
            device_telemetry.update(
                {
                    ...
                    "prednext/effective_trust_ratio": prednext_stats["task_norm"].new_tensor(
                        effective_prednext_trust_ratio
                    ),
                    "prednext/admitted_to_task_ratio": prednext_stats["admitted_norm"]
                    / prednext_stats["task_norm"].clamp_min(1e-20),
