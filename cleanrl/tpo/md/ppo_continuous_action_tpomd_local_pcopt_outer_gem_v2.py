# TPO-MD local predictive-coding optimizer, outer generalized-EM v2.
#
# The probe-scored TPO target and the HL-Gauss TD(lambda) target are the only task
# evidence and are frozen once per rollout.  Ten generalized-EM outer cycles each
# re-forward the current actor and critic, run ten detached reverse Gauss-Seidel
# inference sweeps, and form fresh whole-rollout float64 sufficient statistics.
# Every edge then takes its residual C+ conditional M-step correction; actor and
# critic corrections are captured before one atomic mutation.  Repeating this
# local E/M pair lets a newly learned output edge carry task evidence into earlier
# hidden edges during the same immutable rollout.
...
def proposed_actor_kl(
    ...
    for obs_chunk, latent_chunk, old_chunk in zip(
        ...
        estimates.append(torch.expm1(logratio) - logratio)
...
class OuterCycleDiagnostics(NamedTuple):
    actor_energy_per_row: torch.Tensor
    critic_energy_per_row: torch.Tensor
    actor_stationarity_rms: torch.Tensor
    critic_stationarity_rms: torch.Tensor
    ...
    actor_proposed_behavior_kl: torch.Tensor
    actor_sse_before: torch.Tensor
    actor_sse_after: torch.Tensor
    critic_sse_before: torch.Tensor
    critic_sse_after: torch.Tensor
    correction_norm: torch.Tensor
    actor_entry_delta_norm: torch.Tensor
    critic_entry_delta_norm: torch.Tensor
    actor_min_cov_rank_fraction: torch.Tensor
    actor_max_cov_condition: torch.Tensor
    critic_min_cov_rank_fraction: torch.Tensor
    critic_max_cov_condition: torch.Tensor


class OuterCycleResult(NamedTuple):
    cycle_index: int
    diagnostics: OuterCycleDiagnostics
    actor_statistics: tuple[EdgeStatistics, ...]
    critic_statistics: tuple[EdgeStatistics, ...]
    actor_deltas: dict[str, torch.Tensor]
    critic_deltas: dict[str, torch.Tensor]


ActorSettleCore = Callable[[torch.Tensor, torch.Tensor, torch.Tensor], SettleResult]
CriticSettleCore = Callable[[torch.Tensor, torch.Tensor], SettleResult]


def _settle_failure_flag(result: SettleResult) -> torch.Tensor:
    finite = torch.stack(
        [
            *(activity.isfinite().all() for activity in result.activities),
            result.energies.isfinite().all(),
            result.stationarity_rms.isfinite(),
        ]
    ).all()
    systematic_growth = (
    ...
    return ~finite | systematic_growth


def propose_outer_gem_cycle(
    agent: Agent,
    observations: torch.Tensor,
    candidate_zs: torch.Tensor,
    ...
    chunk_size: int,
    actor_settle_core: ActorSettleCore,
    critic_settle_core: CriticSettleCore,
    cycle_index: int,
) -> OuterCycleResult:
    """Fresh whole-rollout E-step and simultaneous actor/critic M-step proposal."""

    if chunk_size <= 0:
        raise ValueError("chunk_size must be positive")
    rows = observations.shape[0]
    if rows == 0:
        raise ValueError("outer-GEM needs a non-empty rollout")
    expected_rows = (
    ...
    if any(item != rows for item in expected_rows):
        raise ValueError("outer-GEM rollout tensors must have the same leading dimension")

    # Statistics are intentionally recreated here, inside every outer cycle.  No
    # stopped activities or factor residuals survive a live parameter mutation.
    actor_statistics = empty_chain_statistics(agent.actor_chain, observations.device)
    critic_statistics = empty_chain_statistics(agent.critic_chain, observations.device)
    actor_energy_per_row = torch.zeros(
        PC_INFERENCE_STEPS + 1, device=observations.device, dtype=observations.dtype
    )
    critic_energy_per_row = torch.zeros_like(actor_energy_per_row)
    actor_stationarity_squared = torch.zeros((), device=observations.device)
    critic_stationarity_squared = torch.zeros_like(actor_stationarity_squared)
    settle_failed = torch.zeros((), dtype=torch.bool, device=observations.device)

    for start in range(0, rows, chunk_size):
        end = min(start + chunk_size, rows)
        chunk_observations = observations[start:end]
        ...
        settle_failed |= _settle_failure_flag(actor_settled)
        # Consume actor graph outputs before invoking the independently compiled
        # critic graph, whose CUDA pool may reuse temporary storage.
        actor_energy_per_row += actor_settled.energies / rows
        row_fraction = (end - start) / rows
        actor_stationarity_squared += (
            row_fraction * actor_settled.stationarity_rms.square()
        )
        ...
        settle_failed |= _settle_failure_flag(critic_settled)
        critic_energy_per_row += critic_settled.energies / rows
        critic_stationarity_squared += (
            row_fraction * critic_settled.stationarity_rms.square()
        )
    ...
    if any(
        stats.covariance.dtype != torch.float64
        or stats.residual_cross.dtype != torch.float64
        or stats.residual_sse.dtype != torch.float64
        or stats.rows != rows
        for stats in (*actor_statistics, *critic_statistics)
    ):
        raise RuntimeError("outer-GEM statistics lost whole-rollout float64 precision")
    ...
    with torch.no_grad():
        actor_proposed_behavior_kl = proposed_actor_kl(
            agent,
            observations,
            rollout_latent_zs,
            rollout_logprobs,
    ...
    correction_norm = torch.stack(
        [
            delta.square().sum()
            for delta in (*actor_deltas.values(), *critic_deltas.values())
        ]
    ).sum().sqrt()
    actor_entry_delta_norm = (
        actor_deltas["edges.0.weight"].square().sum()
        + actor_deltas["edges.0.bias"].square().sum()
    ).sqrt()
    critic_entry_delta_norm = (
        critic_deltas["edges.0.weight"].square().sum()
        + critic_deltas["edges.0.bias"].square().sum()
    ).sqrt()
    actor_stationarity = actor_stationarity_squared.sqrt()
    critic_stationarity = critic_stationarity_squared.sqrt()
    _require_finite(
        "outer-GEM cycle diagnostics",
        actor_energy_per_row,
        critic_energy_per_row,
        actor_stationarity,
        critic_stationarity,
        actor_ce_before,
        actor_ce_after,
    ...
    return OuterCycleResult(
        cycle_index,
        diagnostics,
        tuple(actor_statistics),
        tuple(critic_statistics),
        actor_deltas,
        critic_deltas,
    )


def run_outer_gem_cycles(
    agent: Agent,
    observations: torch.Tensor,
    candidate_zs: torch.Tensor,
    q_targets: torch.Tensor,
    ...
    rollout_logprobs: torch.Tensor,
    chunk_size: int,
    actor_settle_core: Optional[ActorSettleCore] = None,
    critic_settle_core: Optional[CriticSettleCore] = None,
) -> tuple[OuterCycleResult, ...]:
    """Run exactly ten generalized-EM cycles against one immutable rollout."""

    if actor_settle_core is None:
        actor_settle_core = lambda obs, candidates, target: _settle_actor_chain_core(
            agent, obs, candidates, target, PC_INFERENCE_STEPS
        )
    if critic_settle_core is None:
        critic_settle_core = lambda obs, target: _settle_critic_chain_core(
            agent, obs, target, PC_INFERENCE_STEPS
        )
    _require_finite(
        "immutable outer-GEM rollout evidence",
        observations,
        candidate_zs,
        q_targets,
        value_targets,
    ...
    for cycle_index in range(OUTER_CYCLES):
        result = propose_outer_gem_cycle(
            agent,
            observations,
            candidate_zs,
            q_targets,
        ...
        if (q_targets._version, value_targets._version) != target_versions:
            raise RuntimeError("outer-GEM target evidence was mutated during the update")
        apply_atomic_chain_deltas(agent, result.actor_deltas, result.critic_deltas)
...
if __name__ == "__main__":
    ...
    if args.share_backbone:
        raise ValueError("local PC outer-GEM v2 uses separate actor and critic chains")
    if args.pc_hidden_layers != 4:
        raise ValueError("v2 is the fixed four-hidden-edge intervention")
    if args.pc_inference_steps != PC_INFERENCE_STEPS:
        raise ValueError("v2 is the fixed ten-sweep intervention")
    if args.outer_cycles != OUTER_CYCLES:
        raise ValueError("v2 requires exactly ten outer generalized-EM cycles")
    if args.pc_chunk_size <= 0:
        raise ValueError("pc_chunk_size must be positive")
    if args.batch_size % args.pc_chunk_size:
        raise ValueError("compiled v2 requires batch_size divisible by pc_chunk_size")
    if args.tpo_k < 2:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        outer_cycle_results = run_outer_gem_cycles(
            agent,
            flat_observations,
            flat_candidates,
            rollout_q_targets,
        ...
        actor_statistics = final_cycle.actor_statistics
        critic_statistics = final_cycle.critic_statistics
        actor_deltas = final_cycle.actor_deltas
        critic_deltas = final_cycle.critic_deltas
        actor_energy_per_row = final_diagnostics.actor_energy_per_row
        critic_energy_per_row = final_diagnostics.critic_energy_per_row
        actor_stationarity = final_diagnostics.actor_stationarity_rms
        critic_stationarity = final_diagnostics.critic_stationarity_rms
        actor_proposed_kl = final_diagnostics.actor_proposed_behavior_kl
        actor_ce_before = final_diagnostics.actor_boundary_ce_before
        actor_ce_after = final_diagnostics.actor_boundary_ce_after
        critic_ce_before = final_diagnostics.critic_boundary_ce_before
        critic_ce_after = final_diagnostics.critic_boundary_ce_after
        actor_sse_before = final_diagnostics.actor_sse_before
        actor_sse_after = final_diagnostics.actor_sse_after
        critic_sse_before = final_diagnostics.critic_sse_before
        critic_sse_after = final_diagnostics.critic_sse_after
        actor_rank_fraction = final_diagnostics.actor_min_cov_rank_fraction
        actor_condition = final_diagnostics.actor_max_cov_condition
        critic_rank_fraction = final_diagnostics.critic_min_cov_rank_fraction
        critic_condition = final_diagnostics.critic_max_cov_condition
        ...
        delta_norm = final_diagnostics.correction_norm
        ...
        for cycle in outer_cycle_results:
            ...
            writer.add_scalar(
                ...
                float(diagnostics.actor_energy_per_row[0]),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/actor_energy_settled",
                float(diagnostics.actor_energy_per_row[-1]),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/critic_energy_free",
                float(diagnostics.critic_energy_per_row[0]),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/critic_energy_settled",
                float(diagnostics.critic_energy_per_row[-1]),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/actor_stationarity_rms",
                float(diagnostics.actor_stationarity_rms),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/critic_stationarity_rms",
                float(diagnostics.critic_stationarity_rms),
                global_step,
            )
            writer.add_scalar(
            ...
            writer.add_scalar(
                f"{prefix}/actor_proposed_behavior_kl",
                float(diagnostics.actor_proposed_behavior_kl),
                global_step,
            )
            writer.add_scalar(
            ...
            writer.add_scalar(
                f"{prefix}/actor_entry_delta_norm",
                float(diagnostics.actor_entry_delta_norm),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/critic_entry_delta_norm",
                float(diagnostics.critic_entry_delta_norm),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/actor_min_cov_rank_fraction",
                float(diagnostics.actor_min_cov_rank_fraction),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/actor_max_cov_condition",
                float(diagnostics.actor_max_cov_condition),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/critic_min_cov_rank_fraction",
                float(diagnostics.critic_min_cov_rank_fraction),
                global_step,
            )
            writer.add_scalar(
                f"{prefix}/critic_max_cov_condition",
                float(diagnostics.critic_max_cov_condition),
                global_step,
            )
        ...
        for chain_name, statistics, deltas, edge_sse_before, edge_sse_after in (
            ("actor", actor_statistics, actor_deltas, actor_sse_before, actor_sse_after),
            ("critic", critic_statistics, critic_deltas, critic_sse_before, critic_sse_after),
        ):
            for edge_index, edge_statistics in enumerate(statistics):
                ...
                final_prefix = f"pc_final_edge/{chain_name}_edge_{edge_index}"
                writer.add_scalar(
                    f"{final_prefix}/rank_fraction", float(rank_fraction), global_step
                )
                writer.add_scalar(
                    f"{final_prefix}/min_positive_eigenvalue",
                    float(min_eigenvalue),
                    global_step,
                )
                writer.add_scalar(
                    f"{final_prefix}/condition", float(condition), global_step
                )
                writer.add_scalar(
                    f"{final_prefix}/correction_norm",
                    float(correction_norm),
                    global_step,
                )
                writer.add_scalar(
                    f"{final_prefix}/residual_sse_per_row",
                    float(edge_statistics.residual_sse / edge_statistics.rows),
                    global_step,
                )
                writer.add_scalar(
                    f"{final_prefix}/projected_sse_ratio",
                    float(
                        edge_sse_after[edge_index]
                        / edge_sse_before[edge_index].clamp_min(1e-30)
                    ),
                    global_step,
                )
        writer.add_scalar(
