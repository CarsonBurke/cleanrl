# TPO-MD PredNext v13: optimizer-level predictive coding on the 10.41k TPO-MD v5 lineage.
#
# The control algorithm is unchanged: K=8 raw-MuJoCo one-step probes, the signal-
# determined one-sided KL cap, pure mirror-descent actor, scalar symlog HL-Gauss
# critic, and indexed-transfer trunk. A depth-4 action-conditioned dynamics model
# predicts stopped online future latents with SmoothL1 plus frozen policy-space KL.
# Its private predictor keeps a 0.25 gradient budget. Predictive trunk Adam proposals
# are projected against the current TPO and critic gradients per logical trunk block,
# locally and globally capped to 5% of the actual task-Adam trunk step, then admitted.
# Policy heads never receive auxiliary updates; predictor initialization uses a forked
# RNG stream so base initialization and rollout/probe RNG semantics remain intact.
# Hypothesis: NextLat's useful representation direction compounds TPO-MD's stronger
# actor target when admitted as a small optimizer-level preconditioner, without
# corrupting either task objective or the proven late-training dynamic trust behavior.
import os
import random
import time
...
class Args:
    ...
    # Only stopped target-latent encoding is compiled; TPO rollout/probe paths stay
    # exactly on the evidenced eager implementation.
    compile: bool = False
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(f"compiled NextLat target encoder ({args.compile_mode})")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.nextlat:
            ...
            with torch.no_grad():
                for target_obs in b_obs.split(args.nextlat_target_batch_size):
                    ...
                    # clone protects retained tables from compiled CUDA-graph output reuse
                    target_chunks.append(target_actor_feat_fn(target_obs).clone())
                frozen_actor_feats = torch.cat(target_chunks)
                latent_batch_std = frozen_actor_feats.std(dim=0).mean().item()
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.nextlat:
                    action_indices, target_indices = make_nextlat_indices(
                        mb_inds,
                        args.num_envs,
                        args.batch_size,
                        args.nextlat_depth,
        ...
        if args.nextlat:
            writer.add_scalar(
                "losses/nextlat_prediction", nextlat_pred_loss.item(), global_step
            )
            writer.add_scalar(
                "losses/nextlat_policy_kl", nextlat_kl_loss.item(), global_step
            )
            writer.add_scalar(
            ...
            writer.add_scalar("debug/nextlat_latent_std", latent_batch_std, global_step)
            writer.add_scalar(
                "prednext/task_step_norm", float(prednext_stats["task_norm"]), global_step
            )
            writer.add_scalar(
                "prednext/raw_predictive_step_norm",
                float(prednext_stats["predictive_norm"]),
                global_step,
            )
            writer.add_scalar(
                "prednext/admitted_step_norm",
                float(prednext_stats["admitted_norm"]),
                global_step,
            )
            writer.add_scalar(
                "prednext/predictor_step_norm", float(predictor_step_norm), global_step
            )
            writer.add_scalar(
                "prednext/raw_task_cosine", float(prednext_stats["raw_cosine"]), global_step
            )
            writer.add_scalar(
                "prednext/accepted_fraction",
                float(prednext_stats["accepted_fraction"]),
                global_step,
            )
            writer.add_scalar(
                "prednext/global_cap_scale",
                float(prednext_stats["global_cap_scale"]),
                global_step,
            )
            writer.add_scalar(
                "prednext/nonfinite_veto",
                float(prednext_stats["nonfinite_veto"]),
                global_step,
            )
            writer.add_scalar(
                "prednext/actor_first_order",
                float(prednext_stats["actor_first_order"]),
                global_step,
            )
            writer.add_scalar(
                "prednext/critic_first_order",
                float(prednext_stats["critic_first_order"]),
                global_step,
            )
            writer.add_scalar(
                "prednext/block_actor_conflict_fraction",
                float(prednext_stats["block_actor_conflict"].float().mean()),
                global_step,
            )
            writer.add_scalar(
                "prednext/block_critic_conflict_fraction",
                float(prednext_stats["block_critic_conflict"].float().mean()),
                global_step,
            )
            for block_index, block_name in enumerate(prednext_block_names):
                writer.add_scalar(
                    f"prednext_blocks/{block_name}_actor_conflict",
                    float(prednext_stats["block_actor_conflict"][block_index]),
                    global_step,
                )
                writer.add_scalar(
                    f"prednext_blocks/{block_name}_critic_conflict",
                    float(prednext_stats["block_critic_conflict"][block_index]),
                    global_step,
                )
                writer.add_scalar(
                    f"prednext_blocks/{block_name}_admitted_ratio",
                    float(prednext_stats["block_admitted_ratio"][block_index]),
                    global_step,
                )
