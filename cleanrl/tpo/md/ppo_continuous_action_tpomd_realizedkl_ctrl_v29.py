# TPO-MD REALIZED-KL CONTROLLER v29: regulate the step that actually happens.
#
# Chassis is v25 (all-layer residual-TD tape, unchanged; best run in the repo, 11,015
# @7.46M HalfCheetah).  The change is a closed loop on the policy step size.
#
# THE DEFECT, MEASURED ON v25's OWN LOGS.  TPO-MD solves a temperature eta so that
# KL(p_old || q) = eps in the DISCRETE K-way simplex over the 8 probed candidates, then
# fits the continuous policy to q by cross-entropy.  Those are different objects, and on
# v25 they decoupled by ~6x over training:
#
#   discrete surrogate  tpo_cap_engaged = 1.0 and kl_achieved = 0.0300 at EVERY iteration
#   realized policy KL  max_epoch_approx_kl = 0.0163 / 0.0328 / 0.0339 / 0.0175 / 0.0058
#                       at 0.2M / 1.9M / 3.7M / 5.6M / 7.4M
#
# The surrogate was pinned at its cap for the entire run while the policy's actual step
# collapsed 5.7x from its peak.  The breaker never fired (actor_epochs_completed = 10
# throughout), so nothing was being restrained -- the TARGET had decayed onto the anchor.
# Return flattens over exactly that window (10,026 @3.7M -> 11,313 @7.4M, +13% after +30%
# over the prior 1.8M steps).  Two independent drifts drive it: u_scores shrink ~9x
# because sigma_global tracks the growing return scale (see v28, which fixes only this),
# and -- not fixable by rescaling -- as the Beta policy sharpens (entropy -0.66 -> -10.7)
# the K candidates collapse together in ACTION space, so a fixed discrete reweighting of
# near-identical actions moves the fitted continuous policy less and less.  A cap on the
# surrogate cannot see either drift, and a trust region only bounds steps from ABOVE.
#
# THE FIX.  Close the loop on the measured quantity.  Scores are multiplied by u_gain,
# adapted once per iteration in log space,
#
#   u_gain <- clip( u_gain * exp( kappa * ( log kl_target - log max_epoch_approx_kl ) ) )
#
# with kappa = 0.3, kl_target = 0.033 (v25's realized KL during its fastest-improving
# phase), and u_gain in [0.05, 500].  A breaker trip forces a decrease outright, so an
# overshoot cannot be misread as a too-small step.  This subsumes v28's open-loop fix: any
# scale drift, whatever its source, is absorbed.  The surrogate cap is demoted to a loose
# guard (tpo_eps_control = 0.5) because at eps = 0.03 the cap and the controller fight and
# the discrete KL stays pinned regardless; the HARD safety is the unchanged 0.09 breaker on
# the realized epoch-mean KL.  `--no-tpo-kl-control` recovers v25 bit-for-bit.
#
# HYPOTHESIS.  Holding the realized step at v25's own best-phase value for the whole run
# extends the fast-improvement phase past 3.7M and lifts the final return above 11,015.
# FALSIFIERS: u_gain saturates at 500 with realized KL still under target => the candidate
# set is too collapsed for ANY reweighting to move the policy, and the next move is the
# candidate proposal, not the temperature.  Return drops with the step held up => v25's
# decay was a useful late anneal and kl_target belongs on a schedule.  Watch
# debug/tpo_u_gain against losses/max_epoch_approx_kl.
#
