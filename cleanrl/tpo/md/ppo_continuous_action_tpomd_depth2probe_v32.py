# TPO-MD DEPTH-2 EXACT PROBE v32: a variance-limited operator wants better scores.
#
# Chassis is v25 (all-layer residual-TD tape; best run in the repo, 11,015 @7.46M
# HalfCheetah), with its dual and trust region untouched.  One thing changes: each probed
# candidate gets TWO exact physics steps instead of one.
#
# WHY, FROM THE v30/v31 RESULT.  TPO ranks K=8 candidates by score_i = r_i + gamma*V(s'_i)
# and fits the policy to softmax(beta*log pi_old + u/eta).  A sweep of the slot-prior
# exponent beta, with realized policy KL pinned at 0.033 so step SIZE was matched, gave:
#
#   beta   0      0.5     1 (v25)   2
#   ret    552    1,460   3,003     -65          (all at ~630k, HalfCheetah, seed 1)
#   ESS    8.0    7.30    -         2.51  of 8
#
# Monotone up to beta=1, and beta=2 diverges for a separate structural reason (a sharp
# anchor is mode-seeking, realized KL ran to 0.262 with the gain controller pinned at its
# floor -- the controller scales the advantage tilt and has no authority over the anchor).
# So beta=1 is the argmax and the meaning of beta=0's collapse is the finding: with K=8
# samples from a 6-dim Beta, softmax(u/eta) alone lets a single deep-tail sample own the
# target, and its score is trusted far beyond what it is worth.  THE OPERATOR IS
# VARIANCE-LIMITED, NOT BIAS-LIMITED.  Every remaining knob on the target -- temperature
# (v28), step size (v29), slot prior (v30/v31) -- has now been tried and none of them beat
# v25, because none of them make the scores more trustworthy.
#
# THE MOVE.  Attack the score, which is where the variance actually lives.  r_i is measured
# EXACTLY by the MuJoCo probe; V(s'_i) is the frozen critic evaluated at 8 states one
# physics step apart, where its error is large relative to the differences that decide the
# ranking.  A second exact step under the POLICY MEAN gives
#
#   score_i = r_i + gamma*(1-term_i)*[ r'_i + gamma*(1-term'_i)*V(s''_i) ]
#
# which (a) adds a second exactly-measured reward difference between candidates, (b) cuts
# the critic's weight from gamma to gamma^2, and (c) doubles the state separation at which
# V is read, so its differences rise against its error.  The continuation is deterministic
# and identical in law across all K branches, so it adds no sampling noise of its own -- it
# is pure common-random-numbers variance reduction on an exactly-simulated model.
#
# COST.  2K physics steps per (env, step) instead of K, plus ONE batched policy forward over
# the N*K intermediate states.  Probes already dominate wall time (probe_sps_overhead ~7.6),
# so expect roughly half of v25's SPS.  `--probe-depth 1` recovers v25 bit-for-bit.
#
# HYPOTHESIS.  Better-conditioned scores lift return from the first iterations, because the
# same trust-region step now points using two exact rewards instead of one.  FALSIFIERS: no
# separation from v25 by 1M => the candidate ranking was never critic-limited and the 1-step
# probe was already sufficient, which would retire the whole score-quality direction and
# leave the tape/representation side as the only live lever.  WORSE than v25 => the policy
# mean is a bad continuation (the executed policy is stochastic, so a mean continuation
# evaluates an off-policy tail), and the fix is to continue with a SAMPLED action sharing one
# common random draw across the K branches.  Watch charts/probe_sps_overhead for the true
# cost and debug/tpo_score_std_mean, which should RISE above v25's flat 1.4-1.7 if the second
# exact reward is adding real discrimination.
import copy
import os
import random
import time
...
class Args:
    ...
    # v32: exact physics steps per probed candidate. 1 == v25. At 2 the continuation is the
    # POLICY MEAN, so a second exactly-measured reward enters every candidate score and the
    # frozen critic's weight drops from gamma to gamma^2.
    probe_depth: int = 2
...
def policy_mean_action(agent, model_outputs):
    """Deterministic continuation action for the v32 depth-2 probe.

    The second probe step must not inject fresh policy noise: the point of the extra step
    is to accumulate one more EXACTLY measured reward difference between candidates, so the
    continuation is held at the policy mean and is identical in law across all K branches.
    """
    _, _, first, second = model_outputs
    if agent.actor_dist == "gaussian":
        return torch.tanh(first)
    z = (first / (first + second)).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
    return agent.action_low + (agent.action_high - agent.action_low) * z
...
if __name__ == "__main__":
    ...
    # NEVER compiled: called with a (num_envs * tpo_k) batch, a different static shape from
    # policy_rollout_fn's (num_envs), so sharing a reduce-overhead CUDA graph would be wrong.
    def probe_policy_fn(obs_):
        return policy_model_forward(agent, obs_)
    ...
    # v32 depth-2 probe storage. Only the CURRENT step's intermediate physics state is
    # needed, so these are (num_envs, K, ...) and reused every step; the per-step reward
    # and termination tapes are full-length like the depth-1 ones.
    probe_obs_dim = int(np.array(envs.single_observation_space.shape).prod())
    probe_nq = int(probe_base_envs[0].model.nq)
    probe_nv = int(probe_base_envs[0].model.nv)
    tpo_rewards2_np = np.zeros((args.num_steps, args.num_envs, args.tpo_k), dtype=np.float32)
    tpo_terms2_np = np.zeros((args.num_steps, args.num_envs, args.tpo_k), dtype=np.float32)
    probe_mid_obs_np = np.zeros((args.num_envs, args.tpo_k, probe_obs_dim), dtype=np.float32)
    probe_mid_qpos = np.zeros((args.num_envs, args.tpo_k, probe_nq), dtype=np.float64)
    probe_mid_qvel = np.zeros((args.num_envs, args.tpo_k, probe_nv), dtype=np.float64)
    probe_mid_warm = np.zeros((args.num_envs, args.tpo_k, probe_nv), dtype=np.float64)
    probe_mid_time = np.zeros((args.num_envs, args.tpo_k), dtype=np.float64)
    probe_saved_qpos = np.zeros((args.num_envs, probe_nq), dtype=np.float64)
    probe_saved_qvel = np.zeros((args.num_envs, probe_nv), dtype=np.float64)
    probe_saved_warm = np.zeros((args.num_envs, probe_nv), dtype=np.float64)
    probe_saved_time = np.zeros((args.num_envs,), dtype=np.float64)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # Phase A: one exact step per candidate, stashing the post-step physics state.
            for env_i in range(args.num_envs):
                base_env = probe_base_envs[env_i]
                obs_rms = probe_obs_wrappers[env_i].obs_rms
                probe_saved_qpos[env_i] = base_env.data.qpos
                probe_saved_qvel[env_i] = base_env.data.qvel
                probe_saved_warm[env_i] = base_env.data.qacc_warmstart
                probe_saved_time[env_i] = base_env.data.time
                for cand_i in range(args.tpo_k):
                    ...
                    base_env.data.qpos[:] = probe_saved_qpos[env_i]
                    base_env.data.qvel[:] = probe_saved_qvel[env_i]
                    base_env.data.qacc_warmstart[:] = probe_saved_warm[env_i]
                    base_env.data.time = probe_saved_time[env_i]
                    ...
                    probe_mid_obs_np[env_i, cand_i] = np.clip(norm_obs, -10.0, 10.0)
                    tpo_rewards_np[step, env_i, cand_i] = probe_rew       # RAW reward (base is raw-return)
                    tpo_terms_np[step, env_i, cand_i] = float(probe_term)
                    probe_mid_qpos[env_i, cand_i] = base_env.data.qpos
                    probe_mid_qvel[env_i, cand_i] = base_env.data.qvel
                    probe_mid_warm[env_i, cand_i] = base_env.data.qacc_warmstart
                    probe_mid_time[env_i, cand_i] = base_env.data.time

            if args.probe_depth == 1:
                # Exact v25 semantics: bootstrap straight off the depth-1 successor.
                tpo_next_obs_np[step] = probe_mid_obs_np
                tpo_rewards2_np[step] = 0.0
                tpo_terms2_np[step] = 0.0
                for env_i in range(args.num_envs):
                    base_env = probe_base_envs[env_i]
                    base_env.data.qpos[:] = probe_saved_qpos[env_i]
                    base_env.data.qvel[:] = probe_saved_qvel[env_i]
                ...
                # Phase B: ONE batched deterministic policy forward over all N*K mid states.
                with torch.no_grad():
                    cont_actions_np = (
                        policy_mean_action(
                            agent,
                            probe_policy_fn(
                                torch.as_tensor(
                                    probe_mid_obs_np.reshape(-1, probe_obs_dim),
                                    device=device,
                                )
                            ),
                        )
                        .cpu()
                        .numpy()
                        .reshape(args.num_envs, args.tpo_k, -1)
                    )
                cont_actions_np = np.clip(cont_actions_np, probe_action_low, probe_action_high)

                # Phase C: second exact step from each stashed mid state, then restore the
                # ORIGINAL pre-probe state so the real envs.step below is bit-unaffected.
                for env_i in range(args.num_envs):
                    base_env = probe_base_envs[env_i]
                    obs_rms = probe_obs_wrappers[env_i].obs_rms
                    for cand_i in range(args.tpo_k):
                        if tpo_terms_np[step, env_i, cand_i] > 0.0:
                            # Branch already terminated: no second reward exists and the
                            # gamma*(1-term1) factor zeroes the whole tail anyway.
                            tpo_next_obs_np[step, env_i, cand_i] = probe_mid_obs_np[env_i, cand_i]
                            tpo_rewards2_np[step, env_i, cand_i] = 0.0
                            tpo_terms2_np[step, env_i, cand_i] = 1.0
                            continue
                        base_env.data.qpos[:] = probe_mid_qpos[env_i, cand_i]
                        base_env.data.qvel[:] = probe_mid_qvel[env_i, cand_i]
                        base_env.data.qacc_warmstart[:] = probe_mid_warm[env_i, cand_i]
                        base_env.data.time = probe_mid_time[env_i, cand_i]
                        probe_obs2, probe_rew2, probe_term2, _, _ = base_env.step(
                            cont_actions_np[env_i, cand_i]
                        )
                        norm_obs2 = (
                            (probe_obs2 - obs_rms.mean) / np.sqrt(obs_rms.var + 1e-8)
                        ).astype(np.float32)
                        tpo_next_obs_np[step, env_i, cand_i] = np.clip(norm_obs2, -10.0, 10.0)
                        tpo_rewards2_np[step, env_i, cand_i] = probe_rew2
                        tpo_terms2_np[step, env_i, cand_i] = float(probe_term2)
                    base_env.data.qpos[:] = probe_saved_qpos[env_i]
                    base_env.data.qvel[:] = probe_saved_qvel[env_i]
                    base_env.data.qacc_warmstart[:] = probe_saved_warm[env_i]
                    base_env.data.time = probe_saved_time[env_i]
        ...
        with torch.no_grad():
            ...
            # Oracle score: exactly-measured probe rewards, then a bootstrapped frozen value
            # at the LAST probed state. At probe_depth=1 the r2/term2 tapes are zero and this
            # is v25's r + gamma*(1-term)*V(s') identically.
            r2_probe = torch.as_tensor(tpo_rewards2_np.reshape(-1, args.tpo_k), device=device)
            term2_probe = torch.as_tensor(tpo_terms2_np.reshape(-1, args.tpo_k), device=device)
            if args.probe_depth == 1:
                scores = r_probe + args.gamma * (1.0 - term_probe) * v_next  # (B, K)
            else:
                scores = r_probe + args.gamma * (1.0 - term_probe) * (
                    r2_probe + args.gamma * (1.0 - term2_probe) * v_next
                )
