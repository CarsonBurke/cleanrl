# TPO-MD HORIZON-TD(lambda) TAPE v24: graph-free multistep predictive coding.
#
# One shared horizon-conditioned predictor estimates a dense H=16 tape of delayed latent
# innovations.  A coherent pre-update snapshot supplies stopped finite-horizon lambda
# returns: Y1=eta_t and Yh=(1-lambda)P^-_{h-1}(t+1)+lambda Y_{h-1}(t+1).
# Horizon, rather than a learned state, is shifted: there is no BPTT, learned-state rollout,
# or temporal optimizer graph.  lambda=0 is local TD; lambda=1 is a realized delayed label
# until a real trajectory cutoff, where the frozen policy bootstraps exactly once.  Thus
# each target estimates one delayed innovation (never a discounted successor average), and
# its cumulative tape estimates endpoint displacement without double counting.  One rollout
# innovation RMS changes loss units only.  The clipped trunk error joins actor+critic once
# in task Adam; the predictor keeps private Adam moments after the TPO breaker detaches the
# trunk.  Return and lower-tail behavior, not target fit alone, remain the falsifiers.
import copy
import os
import random
import time
...
def build_sparse_frozen_edge_predictions(
    ...
    *,
    ...
    """Evaluate and return compact frozen predictions only at trajectory cutoffs.

    This deliberately stays eager: the selected row count changes by rollout, while
    the full current-state predictor path remains fixed-shape and CUDA-graph compiled.
    Avoiding an H-head forward over every next state is much cheaper than recompiling a
    dynamic graph or padding the sparse cutoff set to the full rollout.
    """
    if batch_size < 1:
        raise ValueError("batch_size must be positive")
    if next_actor_features.ndim != 3 or sampled_next_actions.ndim != 3:
        raise ValueError("features and actions must be [time, env, channel]")
    ...
    if next_actor_features.shape[-1] != agent.tape_feature_dim:
    ...
    if row_indices.numel() == 0:
        return next_actor_features.new_zeros(
            0, agent.tape_horizon, agent.tape_feature_dim
        )
...
class Args:
    ...
    # Graph-free dense delayed-innovation TD(lambda) tape. Every valid row supervises
    # every head; horizon is an acyclic label index, not an optimizer-time unroll.
    tape: bool = True
    tape_horizon: int = 16
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if args.tape:
                ...
                tape_targets = build_horizon_td_lambda_tape_targets(
                    tape_innovations,
                ...
                valid_head_count = tape_targets.masks.float().sum(dim=(0, 1)).clamp_min(1.0)
        ...
        if args.tape:
            ...
            device_telemetry.update(
                build_tape_head_telemetry(
                    ...
                    teacher_direct_error=tape_teacher_direct_error,
                    teacher_direct_nmse=tape_teacher_direct_nmse,
                    teacher_direct_cosine=tape_teacher_direct_cosine,
                )
            )
