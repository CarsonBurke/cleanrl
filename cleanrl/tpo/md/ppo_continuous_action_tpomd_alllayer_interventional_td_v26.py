# TPO-MD ALL-LAYER INTERVENTIONAL RESIDUAL-TD v26: graph-free predictive supervision.
#
...
# heads retain substantial detached TD-bootstrap mass (over half by head 16).  v26 adds a
# causal K-branch contrast: one deterministic query branch of the frozen shared trunk's
# probed next-state features is contrasted against the other branches' mean, and a
# shared-weight H=1 predictor learns that contrast.  The intervention is graph-free and
# does not alter TPO scores, rollout actions, or RNG. Return/lower-tail regressions,
# layerwise collapse, or missing early-block delivery remain falsifiers.
import copy
import os
import random
import time
...
class Args:
    ...
    # v26 causal interventional contrast.  The executed TPO candidate remains
    # candidate 0; query rotation is deterministic and independent of RNG.
    tape_intervention: bool = True
    tape_intervention_coef: float = 1.0
...
class HorizonTapePredictor(nn.Module):
    ...
    def forward_candidate_h1(self, actor_feature, actions):
        """Predict H=1 for K actions while sharing the source projection.

        The ordinary head evaluates ``source([feature, action])`` independently
        for every flattened candidate.  Splitting that linear layer lets the
        feature projection run once per source row and broadcasts it across K;
        the action projection remains candidate-specific.  Horizon embedding 0
        is used deliberately: this is the exact head-1 branch of ``forward``.
        """
        if actor_feature.ndim != 2 or actions.ndim != 3:
            raise ValueError("candidate H1 inputs must be [M,D] and [M,K,A]")
        if actor_feature.shape[0] != actions.shape[0]:
            raise ValueError("candidate source and action batch sizes differ")
        if actor_feature.shape[-1] != self.feature_dim:
            raise ValueError("candidate source feature dimension does not match predictor")
        if actions.shape[-1] != self.source.in_features - self.feature_dim:
            raise ValueError("candidate action dimension does not match predictor")
        source_weight = self.source.weight
        base = F.linear(
            actor_feature,
            source_weight[:, : self.feature_dim],
            self.source.bias,
        ).unsqueeze(1)
        action_term = F.linear(
            actions,
            source_weight[:, self.feature_dim :],
            None,
        )
        base = self.activation(base + action_term)
        horizon = self.horizon_embedding.weight[0].view(1, 1, -1)
        conditioned = self.activation(
            self.conditioned(torch.cat((base, horizon.expand(base.shape[0], base.shape[1], -1)), dim=-1))
        )
        return self.output(conditioned)

    # Explicit aliases make the H=1 contract discoverable to small diagnostic
    # callers without creating another implementation or parameter path.
    forward_h1_candidates = forward_candidate_h1
...
class AllLayerHorizonTapePredictor(nn.Module):
    ...
    def forward_candidate_h1(self, actor_features, actions):
        """Shared-weight H=1 candidate predictions, shaped ``[M,K,L,D]``."""
        if actor_features.ndim != 3 or actions.ndim != 3:
            raise ValueError("candidate all-layer inputs must be [M,L,D] and [M,K,A]")
        if actor_features.shape[0] != actions.shape[0]:
            raise ValueError("candidate all-layer batch sizes differ")
        if actor_features.shape[1:] != (self.num_layers, self.feature_dim):
            raise ValueError("candidate all-layer feature channels do not match predictor")
        predictions = [
            predictor.forward_candidate_h1(actor_features[:, layer], actions)
            for layer, predictor in enumerate(self.predictors)
        ]
...
def intervention_query_indices(
    global_rows: torch.Tensor | int,
    candidate_count: int,
    iteration: int,
    *,
    device: Optional[torch.device] = None,
) -> torch.Tensor:
    """Deterministic rotating query branch for a batch of rollout rows.

    ``global_rows`` are zero-based rows in the full rollout, not minibatch-local
    positions.  Candidate zero is always the executed action; rotation only
    selects which branch supplies the positive contrast target.
    """
    if candidate_count < 2:
        raise ValueError("intervention requires at least two candidates")
    if iteration < 1:
        raise ValueError("iteration must be one-based and positive")
    if isinstance(global_rows, int):
        if global_rows < 0:
            raise ValueError("number of rollout rows must be nonnegative")
        rows = torch.arange(global_rows, device=device, dtype=torch.long)
    else:
        rows = global_rows.detach().to(device=device, dtype=torch.long)
        if rows.ndim != 1:
            raise ValueError("global rollout rows must be rank one")
        torch._assert_async(
            torch.all(rows >= 0),
            "global rollout rows must be nonnegative",
        )
    return (rows + int(iteration) - 1) % candidate_count


def _finite_or_raise(value: torch.Tensor, name: str) -> torch.Tensor:
    """Fail at the producing boundary instead of training on a poisoned label."""
    torch._assert_async(
        torch.isfinite(value).all(),
        f"non-finite {name} in interventional tape",
    )
    return value


def reduce_candidate_contrast(
    candidate_features: torch.Tensor,
    query_indices: torch.Tensor,
) -> torch.Tensor:
    """Return query minus the mean of all other candidate branches.

    Inputs are ``[M,K,L,D]`` and ``[M]``.  The reduction is exact for K>=2,
    avoids a Python candidate loop, and is used for both frozen feature targets
    and live H=1 predictions.
    """
    if candidate_features.ndim != 4 or query_indices.ndim != 1:
        raise ValueError("candidate contrast expects [M,K,L,D] and [M] query indices")
    rows, candidates = candidate_features.shape[:2]
    if query_indices.shape != (rows,):
        raise ValueError("query indices must have one entry per candidate row")
    if candidates < 2:
        raise ValueError("intervention requires at least two candidates")
    query_indices = query_indices.to(device=candidate_features.device, dtype=torch.long)
    torch._assert_async(
        torch.all((query_indices >= 0) & (query_indices < candidates)),
        "query indices are outside the candidate axis",
    )
    row_indices = torch.arange(rows, device=candidate_features.device)
    query = candidate_features[row_indices, query_indices]
    others = (candidate_features.sum(dim=1) - query) / float(candidates - 1)
    return query - others


def build_intervention_target_contrast(
    ...
    *,
    ...
    """Reduce one candidate-fast probe chunk and return its query schedule.

    ``flat_candidate_features`` is the graph-owned ``[G*K,L,D]`` output from
    one probe forward.  ``global_row_offset`` is measured in rollout rows G,
    not flattened candidate rows; this makes chunking and replay offsets
    explicit and testable.
    """
    if flat_candidate_features.ndim != 3:
        raise ValueError("flat candidate features must be [G*K,L,D]")
    if candidate_count < 2:
        raise ValueError("intervention requires at least two candidates")
    if flat_candidate_features.shape[0] % candidate_count != 0:
        raise ValueError("candidate feature chunk must be divisible by candidate count")
    if global_row_offset < 0:
        raise ValueError("global row offset must be nonnegative")
    rows = flat_candidate_features.shape[0] // candidate_count
    global_rows = torch.arange(
        global_row_offset,
        global_row_offset + rows,
        device=flat_candidate_features.device,
        dtype=torch.long,
    )
    query_indices = intervention_query_indices(
        global_rows, candidate_count, iteration
    )
    grouped = flat_candidate_features.reshape(rows, candidate_count, *flat_candidate_features.shape[1:])
    contrast = reduce_candidate_contrast(grouped, query_indices).detach().clone()
    _finite_or_raise(contrast, "probe feature contrast")
    return contrast, query_indices


def intervention_contrast_scale(
    target_contrast: torch.Tensor,
    latent_rms: Optional[torch.Tensor] = None,
    floor_ratio: float = 1e-3,
) -> torch.Tensor:
    """Per-layer RMS contrast unit with a latent-relative floor."""
    if target_contrast.ndim != 3:
        raise ValueError("intervention target contrast must be [M,L,D]")
    if target_contrast.shape[0] < 1 or target_contrast.shape[1] < 1:
        raise ValueError("intervention target contrast must be nonempty")
    if floor_ratio <= 0.0:
        raise ValueError("intervention scale floor ratio must be positive")
    _finite_or_raise(target_contrast.detach(), "contrast target")
    scale = target_contrast.detach().square().mean(dim=(0, 2)).sqrt()
    if latent_rms is not None:
        if latent_rms.shape != (target_contrast.shape[1],):
            raise ValueError("latent_rms must contain one scalar per layer")
        _finite_or_raise(latent_rms.detach(), "latent RMS")
        scale = scale.clamp_min(latent_rms.detach().to(scale) * floor_ratio)
    return scale.clamp_min(torch.finfo(target_contrast.dtype).eps).detach()


def intervention_contrast_loss(
    ...
    *,
    ...
    """SmoothL1 loss for query-vs-other candidate contrasts.

    Loss is averaged equally over layers after normalization in detached target
    units.  The returned per-layer terms are detached diagnostics.
    """
    if candidate_prediction.ndim != 4 or target_contrast.ndim != 3:
        raise ValueError("intervention prediction/target shapes must be [M,K,L,D]/[M,L,D]")
    if candidate_prediction.shape[0] != target_contrast.shape[0]:
        raise ValueError("intervention prediction and target row counts differ")
    if candidate_prediction.shape[2:] != target_contrast.shape[1:]:
        raise ValueError("intervention prediction and target channels differ")
    target_contrast = _finite_or_raise(target_contrast.detach(), "contrast target")
    prediction_contrast = reduce_candidate_contrast(candidate_prediction, query_indices)
    _finite_or_raise(prediction_contrast.detach(), "contrast prediction")
    if scale is None:
        scale = intervention_contrast_scale(target_contrast, latent_rms, floor_ratio)
    elif scale.shape != (target_contrast.shape[1],):
        raise ValueError("intervention scale must contain one scalar per layer")
    scale = _finite_or_raise(scale.detach().to(target_contrast), "contrast scale")
    scale = scale.clamp_min(torch.finfo(target_contrast.dtype).eps)
    normalized_prediction = prediction_contrast / scale.view(1, -1, 1)
    normalized_target = target_contrast / scale.view(1, -1, 1)
    terms = F.smooth_l1_loss(
        normalized_prediction,
        normalized_target,
        reduction="none",
    ).mean(dim=(0, 2))
    return terms.mean(), scale.detach(), terms.detach()


@torch.no_grad()
def intervention_contrast_metrics(
    candidate_prediction: torch.Tensor,
    target_contrast: torch.Tensor,
    query_indices: torch.Tensor,
) -> dict[str, torch.Tensor]:
    """Full-rollout frozen H=1 contrast diagnostics, including a query shuffle."""
    prediction = reduce_candidate_contrast(candidate_prediction, query_indices)
    target_contrast = _finite_or_raise(target_contrast.detach(), "contrast target")
    prediction = _finite_or_raise(prediction.detach(), "contrast prediction")
    error = prediction - target_contrast
    target_power = target_contrast.square().mean(dim=(0, 2)).clamp_min(1e-12)
    mse = error.square().mean(dim=(0, 2))
    nmse = mse / target_power
    dot = (prediction * target_contrast).sum(dim=(0, 2))
    cosine = dot / (
        prediction.square().sum(dim=(0, 2)).sqrt()
        * target_contrast.square().sum(dim=(0, 2)).sqrt()
    ).clamp_min(1e-12)
    shuffled_queries = (query_indices.to(torch.long) + 1) % candidate_prediction.shape[1]
    shuffled = reduce_candidate_contrast(candidate_prediction, shuffled_queries)
    shuffled_nmse = (shuffled - target_contrast).square().mean(dim=(0, 2)) / target_power
    return {
        "mse": mse,
        "nmse": nmse,
        "cosine": cosine,
        "shuffled_nmse": shuffled_nmse,
        "shuffled_ratio": shuffled_nmse / nmse.clamp_min(1e-12),
    }
...
class Agent(nn.Module):
    ...
    def predict_intervention_tapes(self, actor_feature, candidate_actions):
        if not self.tape_enabled:
            raise RuntimeError("the horizon-TD tape is disabled")
        return self.tape_predictor.forward_candidate_h1(
            actor_feature, candidate_actions
        )

    def predict_target_tapes(self, actor_feature, action):
        return self.target_tape_predictor(actor_feature, action)

    def predict_target_intervention_tapes(self, actor_feature, candidate_actions):
        if not self.tape_enabled:
            raise RuntimeError("the horizon-TD tape is disabled")
        return self.target_tape_predictor.forward_candidate_h1(
            actor_feature, candidate_actions
        )
...
def probe_value_and_actor_features_forward(agent, observations):
    """Shared-trunk probe forward retaining value logits and all actor layers."""
    if not agent.share_backbone:
        raise RuntimeError("interventional probe requires a shared backbone")
    actor_features, critic_features = agent._trunks_all(observations)
    return agent.critic_head(critic_features), actor_features
...
def tape_intervention_forward(agent, actor_feature, candidate_actions):
    """Static live H=1 candidate path sharing the source feature projection."""
    return agent.predict_intervention_tapes(actor_feature, candidate_actions)


def tape_auxiliary_forward(agent, actor_feature, action, candidate_actions):
    """Single live auxiliary graph: ordinary H16 and candidate H1 outputs."""
    return (
        agent.predict_tapes(actor_feature, action),
        agent.predict_intervention_tapes(actor_feature, candidate_actions),
    )
...
def target_tape_intervention_forward(agent, actor_feature, candidate_actions):
    """Static frozen H=1 candidate path for pre-update diagnostics."""
    return agent.predict_target_intervention_tapes(actor_feature, candidate_actions)
...
if __name__ == "__main__":
    ...
    assert args.tape_intervention_coef >= 0.0
    if args.tape_intervention:
        assert args.tape, "interventional tape requires the base tape"
        assert args.share_backbone, "interventional tape requires a shared backbone"
        assert args.tpo_k >= 2, "interventional tape requires at least two candidates"
    ...
    def probe_value_fn(obs_):
        if args.tape_intervention:
            return probe_value_and_actor_features_forward(agent, obs_)
        return value_forward(agent, obs_)
    ...
    def tape_auxiliary_update_fn(actor_feature_, action_, candidate_actions_):
        return tape_auxiliary_forward(
            agent, actor_feature_, action_, candidate_actions_
        )

    def target_tape_fn(actor_feature_, action_):
        return target_tape_predictor_forward(agent, actor_feature_, action_)

    def target_tape_intervention_fn(actor_feature_, candidate_actions_):
        return target_tape_intervention_forward(
            agent, actor_feature_, candidate_actions_
        )
    ...
    if args.compile:
        ...
        if args.tape_intervention:
            tape_auxiliary_update_fn = torch.compile(
                tape_auxiliary_update_fn, mode=args.compile_mode, dynamic=False
            )
        ...
        if args.tape_intervention:
            target_tape_intervention_fn = torch.compile(
                target_tape_intervention_fn, mode=args.compile_mode, dynamic=False
            )
    ...
    if args.tape_intervention:
        # Candidate actions are captured at the exact native-z -> environment-action
        # conversion used by TPO.  This keeps candidate 0 bit-identical to the
        # executed action and avoids any live-policy recomputation in the update.
        intervention_actions = torch.zeros(
            (args.num_steps, args.num_envs, args.tpo_k)
            + envs.single_action_space.shape,
            device=device,
        )
        intervention_target_contrast = torch.zeros(
            args.batch_size,
            agent.tape_num_layers,
            agent.tape_feature_dim,
            device=device,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                cand_actions = roll_to_action(cand_z)
                if args.tape_intervention:
                    intervention_actions[step].copy_(cand_actions)
                cand_actions_np = cand_actions.cpu().numpy()
        ...
        with torch.no_grad():
            ...
            # Candidate order is fastest in flat_probe_obs.  Interventional probe
            # chunks must therefore contain complete K-candidate groups so that the
            # graph-owned feature output can be reduced before the next replay.
            probe_chunk_size = 65536
            if args.tape_intervention:
                probe_chunk_size = (probe_chunk_size // args.tpo_k) * args.tpo_k
                if probe_chunk_size < args.tpo_k:
                    raise ValueError("candidate count exceeds probe chunk capacity")
                _finite_or_raise(flat_probe_obs, "probe state")
            probe_value_chunks = []
            for chunk_number, chunk in enumerate(flat_probe_obs.split(probe_chunk_size)):
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                probe_outputs = probe_value_fn(chunk)
                if args.tape_intervention:
                    chunk_logits, chunk_actor_features = probe_outputs
                    if chunk.shape[0] % args.tpo_k != 0:
                        raise RuntimeError(
                            "interventional probe chunk is not divisible by candidate count"
                        )
                    _finite_or_raise(chunk_actor_features, "probe actor features")
                    # Do not clone the full graph-owned feature table.  Reduce each
                    # [G,K,L,D] chunk immediately, clone only [G,L,D], and discard
                    # the graph output before the next compiled probe replay.
                    global_group_start = (chunk_number * probe_chunk_size) // args.tpo_k
                    contrast, _ = build_intervention_target_contrast(
                        chunk_actor_features,
                        args.tpo_k,
                        iteration,
                        global_row_offset=global_group_start,
                    )
                    groups = chunk.shape[0] // args.tpo_k
                    global_rows = torch.arange(
                        global_group_start,
                        global_group_start + groups,
                        device=device,
                        dtype=torch.long,
                    )
                    intervention_target_contrast.index_copy_(
                        0, global_rows, contrast
                    )
                    del chunk_actor_features, contrast
                else:
                    chunk_logits = probe_outputs
                # Value logits are replayed by the CUDA graph, so retain their own
                # storage before the next chunk.  In off mode this is exactly v25.
            ...
            if args.tape:
                ...
                target_intervention_prediction_chunks = []
                ...
                if args.tape_intervention:
                    for feature_chunk, candidate_action_chunk in zip(
                        frozen_actor_feats.reshape(
                            -1, agent.tape_num_layers, agent.tape_feature_dim
                        ).split(args.tape_target_batch_size),
                        intervention_actions.reshape(
                            -1, args.tpo_k, agent.tape_action_dim
                        ).split(args.tape_target_batch_size),
                        strict=True,
                    ):
                        if args.compile:
                            torch.compiler.cudagraph_mark_step_begin()
                        target_intervention_prediction_chunks.append(
                            retain_graph_output(
                                target_tape_intervention_fn(
                                    feature_chunk, candidate_action_chunk
                                ),
                                compiled=args.compile,
                            )
                        )
                ...
                if args.tape_intervention:
                    target_intervention_predictions = torch.cat(
                        target_intervention_prediction_chunks
                    ).reshape(
                        args.batch_size,
                        args.tpo_k,
                    ...
                    intervention_query = intervention_query_indices(
                        args.batch_size, args.tpo_k, iteration, device=device
                    )
                    intervention_scale = intervention_contrast_scale(
                        intervention_target_contrast,
                        frozen_actor_feats.reshape(
                            args.batch_size,
                            agent.tape_num_layers,
                            agent.tape_feature_dim,
                        ).square().mean(dim=(0, 2)).sqrt(),
                        args.tape_scale_floor_ratio,
                    )
                    intervention_preupdate_metrics = intervention_contrast_metrics(
                        target_intervention_predictions,
                        intervention_target_contrast,
                        intervention_query,
                    )
                    del target_intervention_prediction_chunks, target_intervention_predictions
        ...
        if args.tape:
            ...
            if args.tape_intervention:
                b_intervention_actions = intervention_actions.reshape(
                    args.batch_size,
                    args.tpo_k,
                    agent.tape_action_dim,
                )
                b_intervention_target = intervention_target_contrast
                b_intervention_query = intervention_query
        ...
        if args.tape:
            ...
            if args.tape_intervention:
                intervention_loss = torch.zeros((), device=device)
                intervention_head_losses = torch.zeros(
                    agent.tape_num_layers, device=device
                )
                intervention_metric_mse = intervention_preupdate_metrics["mse"]
                intervention_metric_nmse = intervention_preupdate_metrics["nmse"]
                intervention_metric_cosine = intervention_preupdate_metrics["cosine"]
                intervention_metric_shuffled_nmse = intervention_preupdate_metrics[
                    "shuffled_nmse"
                ]
                intervention_metric_shuffled_ratio = intervention_preupdate_metrics[
                    "shuffled_ratio"
                ]
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.tape:
                    ...
                    if args.tape_intervention:
                        # Exactly one live auxiliary forward owns both outputs.  In
                        # compiled mode this prevents replaying a second graph from
                        # invalidating the ordinary H16 activation before backward.
                        tape_prediction, intervention_prediction = (
                            tape_auxiliary_update_fn(
                                tape_source,
                                b_actions[mb_inds],
                                b_intervention_actions[mb_inds],
                            )
                        )
                    else:
                    ...
                    if args.tape_intervention:
                        intervention_query = b_intervention_query[mb_inds]
                        (
                            intervention_loss,
                            _,
                            intervention_head_losses,
                        ) = intervention_contrast_loss(
                            intervention_prediction,
                            b_intervention_target[mb_inds],
                            intervention_query,
                ...
                if args.tape:
                    agent.zero_grad(set_to_none=True)
                    auxiliary_loss = args.tape_coef * tape_loss
                    if args.tape_intervention:
                        auxiliary_loss = auxiliary_loss + (
                            args.tape_intervention_coef * intervention_loss
                        )
        ...
        if args.tape:
            ...
            if args.tape_intervention:
                device_telemetry.update(
                    {
                        "losses/tape_intervention": intervention_loss,
                        "tape_intervention/target_scale": intervention_scale.mean(),
                        "tape_intervention/preupdate_mse": intervention_metric_mse.mean(),
                        "tape_intervention/preupdate_nmse": intervention_metric_nmse.mean(),
                        "tape_intervention/preupdate_cosine": intervention_metric_cosine.mean(),
                        "tape_intervention/preupdate_shuffled_nmse": intervention_metric_shuffled_nmse.mean(),
                        "tape_intervention/preupdate_shuffled_ratio": intervention_metric_shuffled_ratio.mean(),
                    }
                )
            for layer_index, layer_name in enumerate(agent.tape_layer_names):
                ...
                if args.tape_intervention:
                    device_telemetry.update(
                        {
                            f"tape_intervention/layers/{layer_name}/loss": intervention_head_losses[
                                layer_index
                            ],
                            f"tape_intervention/layers/{layer_name}/preupdate_mse": intervention_metric_mse[
                                layer_index
                            ],
                            f"tape_intervention/layers/{layer_name}/preupdate_nmse": intervention_metric_nmse[
                                layer_index
                            ],
                            f"tape_intervention/layers/{layer_name}/preupdate_cosine": intervention_metric_cosine[
                                layer_index
                            ],
                            f"tape_intervention/layers/{layer_name}/preupdate_shuffled_nmse": intervention_metric_shuffled_nmse[
                                layer_index
                            ],
                            f"tape_intervention/layers/{layer_name}/preupdate_shuffled_ratio": intervention_metric_shuffled_ratio[
                                layer_index
                            ],
                        }
                    )
