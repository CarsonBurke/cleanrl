# TPO-MD GROUNDED GVF v21: action-conditioned predictive state at four time scales.
#
# A private model predicts normalized discounted sums of fixed observable cumulants:
# normalized-observation change, raw reward, control energy, and continuation. Gamma
# defines each question; a forward-view prefix observes enough steps to leave <=10% tail
# mass, then bootstraps exactly once from a rollout-frozen predictor/encoder. Per-channel
# target RMS changes only loss units, never Bellman coordinates. The globally clipped GVF
# trunk gradient joins actor+critic in their existing task Adam; the predictor has private
# Adam, and the TPO breaker gates only the trunk path. TPO, task initialization/RNG,
# CUDA-resident labels, and compiled static paths remain the exact union-v17 chassis.
# Falsifier: lower grounded band error and nonzero action sensitivity must accompany
# better return/lower-tail CVaR at the fixed trunk budget. Fitting fixed cumulants without
# task gains means these predictive questions are behaviorally irrelevant.
import copy
import os
import random
import time
...
class GroundedCumulants:
    """Fixed, observable cumulants in their raw semantic coordinates."""
...
@dataclass(frozen=True)
class GVFTargets:
    """Stopped forward-view labels and coverage diagnostics."""

    values: torch.Tensor
    masks: torch.Tensor
    bootstrap_masks: torch.Tensor
    observed_mass: torch.Tensor
    bootstrap_mass: torch.Tensor


def gvf_prefix_horizons(
    gammas: Sequence[float], residual_tail_mass: float
) -> tuple[int, ...]:
    """Smallest n for each gamma with gamma**n <= residual_tail_mass."""
    if not 0.0 < residual_tail_mass < 1.0:
        raise ValueError("residual_tail_mass must lie strictly between zero and one")
    horizons = []
    for gamma in gammas:
        if not 0.0 < gamma < 1.0:
            raise ValueError("GVF gammas must lie strictly between zero and one")
        horizons.append(max(1, ceil(log(residual_tail_mass) / log(gamma))))
    if not horizons:
        raise ValueError("at least one GVF gamma is required")
    return tuple(horizons)


def build_grounded_cumulants(
    ...
    """Construct encoder-independent cumulants from stored transitions.

    Observations are already normalized and clipped by the environment wrappers. Reward
    remains in raw task units; detached per-channel RMS is applied only by the loss.
    Invalid rows (a boundary with no final observation) are zeroed and masked downstream.
    """
    if observations.shape != next_observations.shape or observations.ndim != 3:
        raise ValueError("observations must have matching [time, env, feature] shapes")
    if actions.ndim != 3 or actions.shape[:2] != observations.shape[:2]:
        raise ValueError("actions must have shape [time, env, action]")
    expected = observations.shape[:2]
    if any(tensor.shape != expected for tensor in (rewards, terminations, transition_valids)):
        raise ValueError("reward and transition masks must match [time, env]")
    valid = transition_valids.detach().bool().unsqueeze(-1)
    obs_delta = next_observations.detach() - observations.detach()
    reward = rewards.detach().unsqueeze(-1)
    control_energy = actions.detach().square().mean(dim=-1, keepdim=True)
    continuation = (1.0 - terminations.detach()).unsqueeze(-1)
    values = torch.cat((obs_delta, reward, control_energy, continuation), dim=-1)
    values = torch.where(valid, values, torch.zeros_like(values)).detach()
    obs_dim = observations.shape[-1]
    return GroundedCumulants(
        values=values,
        obs_dim=obs_dim,
        reward_index=obs_dim,
        control_index=obs_dim + 1,
        continuation_index=obs_dim + 2,
    )
...
def build_grounded_gvf_targets(
    ...
    """Build normalized n-step GVFs with observed prefixes and exactly one tail.

    A terminal transition contributes its final cumulant and stops. A truncation uses
    its final observation and an isolated frozen-policy action sample. The rollout edge
    does the same. A later transition missing its final observation falls back one state
    earlier to the known (state, behavior-action) tail instead of zero-filling or crossing
    the reset. Every returned label is a semi-gradient target.
    """
    if cumulants.ndim != 3:
        raise ValueError("cumulants must have shape [time, env, feature]")
    time_dim, num_envs, feature_dim = cumulants.shape
    num_bands = len(gammas)
    if len(prefix_horizons) != num_bands or num_bands == 0:
        raise ValueError("gammas and prefix_horizons must have the same nonzero length")
    if tail_predictions.shape != (time_dim, num_envs, num_bands, feature_dim):
        raise ValueError("tail_predictions has the wrong shape")
    if next_state_tail_predictions.shape != tail_predictions.shape:
        raise ValueError("next-state tail predictions have the wrong shape")
    if any(tensor.shape != (time_dim, num_envs) for tensor in (
        transition_terminations, transition_boundaries, transition_valids
    )):
        raise ValueError("transition masks have the wrong shape")
    if any(not 0.0 < gamma < 1.0 for gamma in gammas):
        raise ValueError("GVF gammas must lie strictly between zero and one")
    if any(horizon < 1 for horizon in prefix_horizons):
        raise ValueError("prefix horizons must be positive")
    if trace_lambda != 1.0:
        raise ValueError(
            "grounded_gvf_v21 currently implements the unbiased n-step forward view; "
            "trace_lambda must remain 1.0"
        )

    cumulants = cumulants.detach()
    tails = tail_predictions.detach()
    next_tails = next_state_tail_predictions.detach()
    terminations = transition_terminations.detach().bool()
    boundaries = transition_boundaries.detach().bool()
    valids = transition_valids.detach().bool()
    values = cumulants.new_zeros((time_dim, num_envs, num_bands, feature_dim))
    bootstrap_masks = torch.zeros(
        (time_dim, num_envs, num_bands), dtype=torch.bool, device=cumulants.device
    )
    observed_mass = cumulants.new_zeros((time_dim, num_envs, num_bands))
    bootstrap_mass = cumulants.new_zeros((time_dim, num_envs, num_bands))

    for band, (gamma, horizon) in enumerate(zip(gammas, prefix_horizons)):
        alive = torch.ones((time_dim, num_envs), dtype=torch.bool, device=cumulants.device)
        discount = 1.0
        for offset in range(min(horizon, time_dim)):
            ...
            cumulant_weight = (1.0 - gamma) * discount
            values[:valid_len, :, band] += (
                cumulant_weight * cumulants[offset:] * step_ok.unsqueeze(-1)
            )
            observed_mass[:valid_len, :, band] += cumulant_weight * step_ok

            # The transition at this offset has no usable final observation. Tail from
            # its known source state/action before consuming the unknown cumulant.
            fallback = source_alive & ~valids[offset:] & valids[:valid_len]
            values[:valid_len, :, band] += (
                discount * tails[offset:, :, band] * fallback.unsqueeze(-1)
            )
            bootstrap_masks[:valid_len, :, band] |= fallback
            bootstrap_mass[:valid_len, :, band] += discount * fallback
            ...
            values[:valid_len, :, band] += (
                next_discount
                * next_tails[offset:, :, band]
                * next_state_bootstrap.unsqueeze(-1)
            )
            bootstrap_masks[:valid_len, :, band] |= next_state_bootstrap
            bootstrap_mass[:valid_len, :, band] += (
                next_discount * next_state_bootstrap
            )

            if offset + 1 == horizon and valid_len > 1:
                interior = step_ok[:-1] & ~boundaries[offset:-1]
                values[: valid_len - 1, :, band] += (
                    next_discount
                    * tails[offset + 1 :, :, band]
                    * interior.unsqueeze(-1)
                )
                bootstrap_masks[: valid_len - 1, :, band] |= interior
                bootstrap_mass[: valid_len - 1, :, band] += next_discount * interior
    ...
    return GVFTargets(
        values.detach(),
        masks.detach(),
        bootstrap_masks.detach(),
        observed_mass.detach(),
        bootstrap_mass.detach(),
    )


def grounded_gvf_loss(
    ...
    *,
    ...
    """Equal-average bands after detached per-channel RMS preconditioning."""
    ...
    if target.ndim < 3 or target.shape[-1] != obs_dim + 3:
        raise ValueError("target must contain obs deltas plus reward/control/continuation")
    if min(reward_weight, control_weight, continuation_weight) < 0.0:
        raise ValueError("semantic loss weights must be non-negative")
    weights = mask.to(target.dtype).unsqueeze(-1)
    reduce_dims = tuple(range(target.ndim - 2))
    if scale is None:
        denominator = weights.sum(dim=reduce_dims).clamp_min(1.0)
        scale = (
            (target.detach().square() * weights).sum(dim=reduce_dims) / denominator
        ).sqrt().clamp_min(min_scale)
    else:
        if scale.shape != target.shape[-2:]:
            raise ValueError("scale must have [band, channel] shape")
        scale = scale.detach().to(target).clamp_min(min_scale)
    view = scale.view(*([1] * (target.ndim - 2)), *scale.shape)
    channel_error = F.smooth_l1_loss(
        prediction / view, target / view, reduction="none"
    )
    row_denominator = mask.to(target.dtype).sum(dim=reduce_dims).clamp_min(1.0)
    per_band_channel = (
        channel_error * weights
    ).sum(dim=reduce_dims) / row_denominator.unsqueeze(-1)
    obs_loss = per_band_channel[:, :obs_dim].mean(dim=-1)
    reward_loss = per_band_channel[:, obs_dim]
    control_loss = per_band_channel[:, obs_dim + 1]
    continuation_loss = per_band_channel[:, obs_dim + 2]
    total_weight = 1.0 + reward_weight + control_weight + continuation_weight
    per_band = (
        obs_loss
    ...
    semantic = torch.stack(
        (obs_loss.mean(), reward_loss.mean(), control_loss.mean(), continuation_loss.mean())
    )
    return per_band.mean(), scale.detach(), per_band.detach(), semantic.detach()


def normalized_gvf_rms(
    residual: torch.Tensor, mask: torch.Tensor, scale: torch.Tensor
) -> torch.Tensor:
    """Per-band RMS reduced over every valid sample and cumulant channel."""
    if residual.shape[:-1] != mask.shape or scale.shape != residual.shape[-2:]:
        raise ValueError("residual, mask, and scale shapes are inconsistent")
    weights = mask.to(residual.dtype).unsqueeze(-1)
    sample_dims = tuple(range(residual.ndim - 2))
    reduce_dims = sample_dims + (residual.ndim - 1,)
    denominator = (
        mask.to(residual.dtype).sum(dim=sample_dims) * residual.shape[-1]
    ).clamp_min(1.0)
    view = scale.detach().view(*([1] * (residual.ndim - 2)), *scale.shape)
    normalized_square = (residual.detach() / view).square() * weights
    return (normalized_square.sum(dim=reduce_dims) / denominator).sqrt()


def build_gvf_band_telemetry(
    gammas: Sequence[float],
    *,
    band_losses: torch.Tensor,
    band_errors: torch.Tensor,
    target_channel_rms: torch.Tensor,
    zero_baseline: torch.Tensor,
    mean_baseline: torch.Tensor,
    signflip_action_sensitivity: torch.Tensor,
    observed_target_mass: torch.Tensor,
    frozen_tail_bootstrap_mass: torch.Tensor,
    ...
    """Build the exact scalar per-band map consumed by packed telemetry."""
    num_bands = len(gammas)
    vectors = {
        ...
        "signflip_action_sensitivity": signflip_action_sensitivity,
        "observed_target_mass": observed_target_mass,
        "frozen_tail_bootstrap_mass": frozen_tail_bootstrap_mass,
        "bootstrap_fraction": bootstrap_fraction,
    }
    if any(value.shape != (num_bands,) for value in vectors.values()):
        raise ValueError("every per-band diagnostic must have shape [num_bands]")
    if target_channel_rms.ndim != 2 or target_channel_rms.shape[0] != num_bands:
        raise ValueError("target_channel_rms must have shape [num_bands, channels]")
    channel_dim = target_channel_rms.shape[1]
    if not (
        0 < obs_dim <= channel_dim
        and reward_index == obs_dim
        and control_index == obs_dim + 1
        and continuation_index == obs_dim + 2
        and continuation_index < channel_dim
    ):
        raise ValueError("grounded cumulant channel indices are inconsistent")

    pooled_target_rms = target_channel_rms.square().mean(dim=-1).sqrt()
    telemetry = {}
    for band, gamma in enumerate(gammas):
        tag = str(gamma).replace(".", "p")
        telemetry.update(
            {
                f"gvf/band_{tag}_loss": band_losses[band],
                f"gvf/band_{tag}_error_rms": band_errors[band],
                f"gvf/band_{tag}_pooled_target_rms": pooled_target_rms[band],
                f"gvf/band_{tag}_obs_target_rms_mean": target_channel_rms[
                    band, :obs_dim
                ].mean(),
                f"gvf/band_{tag}_reward_target_rms": target_channel_rms[
                    band, reward_index
                ],
                f"gvf/band_{tag}_control_target_rms": target_channel_rms[
                    band, control_index
                ],
                f"gvf/band_{tag}_continuation_target_rms": target_channel_rms[
                    band, continuation_index
                ],
                f"gvf/band_{tag}_zero_baseline": zero_baseline[band],
                f"gvf/band_{tag}_mean_baseline": mean_baseline[band],
                f"gvf/band_{tag}_signflip_action_sensitivity": (
                    signflip_action_sensitivity[band]
                ),
                f"gvf/band_{tag}_observed_target_mass": observed_target_mass[band],
                f"gvf/band_{tag}_frozen_tail_bootstrap_mass": (
                    frozen_tail_bootstrap_mass[band]
                ),
                f"gvf/band_{tag}_bootstrap_fraction": bootstrap_fraction[band],
            }
        )
        for channel in range(obs_dim):
            telemetry[
                f"gvf_channels/band_{tag}_obs_{channel}_target_rms"
            ] = target_channel_rms[band, channel]
    if any(value.numel() != 1 for value in telemetry.values()):
        raise RuntimeError("GVF per-band telemetry must contain only scalar tensors")
    return telemetry
...
class Args:
    ...
    # Grounded multi-timescale GVFs. Gamma defines the semantic timescale. Each
    # automatically derived prefix leaves at most ``gvf_residual_tail_mass`` for one
    # frozen bootstrap (defaults -> 4, 22, 76, 230 observed transitions).
    gvf: bool = True
    gvf_gammas: tuple[float, ...] = (0.50, 0.90, 0.97, 0.99)
    gvf_residual_tail_mass: float = 0.10
    gvf_trace_lambda: float = 1.0
    gvf_coef: float = 1.0
    gvf_reward_loss_weight: float = 0.25
    gvf_control_loss_weight: float = 0.10
    gvf_continuation_loss_weight: float = 0.10
    gvf_trunk_grad_clip: float = 0.025
    gvf_predictor_grad_clip: float = 0.25
    gvf_target_batch_size: int = 8192
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        with torch.random.fork_rng(devices=[]):
            self.gvf_num_bands = len(args.gvf_gammas)
            self.gvf_feature_dim = obs_dim + 3
            self.gvf_predictor = nn.Sequential(
                layer_init(nn.Linear(H + act_dim, H)),
                ReLUSquared(),
                layer_init(
                    nn.Linear(H, self.gvf_num_bands * self.gvf_feature_dim),
                    std=0.01,
                ),
            )
            # A random gamma=.99 tail would inject its arbitrary output into 10% of
            # every first-rollout target. Zero output gives a conservative finite-prefix
            # initialization; hidden/predictor gradients begin once the output head moves.
            with torch.no_grad():
                self.gvf_predictor[-1].weight.zero_()
                self.gvf_predictor[-1].bias.zero_()
            self.target_gvf_predictor = copy.deepcopy(self.gvf_predictor)
            self.target_gvf_predictor.requires_grad_(False)
    ...
    def gvf_trunk_parameters(self):
        return [parameter for block in self.gvf_trunk_blocks() for parameter in block]

    def gvf_predictor_parameters(self):
        return list(self.gvf_predictor.parameters())

    def predict_gvfs(self, actor_feature, action):
        prediction = self.gvf_predictor(torch.cat((actor_feature, action), dim=-1))
        return prediction.view(-1, self.gvf_num_bands, self.gvf_feature_dim)

    def predict_target_gvfs(self, actor_feature, action):
        prediction = self.target_gvf_predictor(
            torch.cat((actor_feature, action), dim=-1)
        )
        return prediction.view(-1, self.gvf_num_bands, self.gvf_feature_dim)

    @torch.no_grad()
    def snapshot_gvf_target(self):
        self.target_gvf_predictor.load_state_dict(self.gvf_predictor.state_dict())

    def gvf_parameters(self):
        return self.gvf_trunk_parameters() + self.gvf_predictor_parameters()

    def task_parameters(self):
        predictor_ids = {id(parameter) for parameter in self.gvf_predictor.parameters()}
        target_ids = {
            id(parameter) for parameter in self.target_gvf_predictor.parameters()
        }
        return [
            parameter
            ...
            and id(parameter) not in target_ids
        ]
...
def gvf_predictor_forward(agent, actor_feature, action):
    """Static live predictor path; gradients may flow into the source feature."""
    return agent.predict_gvfs(actor_feature, action)


def target_gvf_predictor_forward(agent, actor_feature, action):
    """Static rollout-frozen tail predictor path."""
    return agent.predict_target_gvfs(actor_feature, action)
...
def frozen_policy_sample_action_isolated(
    ...
    *,
    ...
    expectation in a GVF tail; using more samples is deliberately left out of v21.
    """
    ...
    with torch.random.fork_rng(devices=fork_devices):
        ...
        dist, to_action, _ = agent._actor_dist_frozen_head(actor_features)
...
@torch.no_grad()
def build_sparse_next_state_tail_table(
    agent,
    next_observations: torch.Tensor,
    ...
    *,
    ...
    """Predict frozen-policy tails only for required final/rollout-edge states."""
    if bootstrap_rows.ndim != 2 or next_observations.shape[:2] != bootstrap_rows.shape:
        raise ValueError("next observations and bootstrap rows must share [time, env]")
    if next_observations.device != bootstrap_rows.device:
        raise ValueError("next observations and bootstrap rows must share a device")

    flat_rows = bootstrap_rows.detach().bool().reshape(-1)
    row_indices = flat_rows.nonzero(as_tuple=False).squeeze(-1)
    full_tails = next_observations.new_zeros(
        (flat_rows.numel(), agent.gvf_num_bands, agent.gvf_feature_dim)
    )
    if row_indices.numel() == 0:
        return (
            full_tails.reshape(*bootstrap_rows.shape, *full_tails.shape[1:]),
            cpu_rng_state,
            device_rng_state,
        )

    flat_next_observations = next_observations.reshape(
        flat_rows.numel(), *next_observations.shape[2:]
    )
    selected_observations = flat_next_observations.index_select(0, row_indices)
    selected_actor_features = target_actor_feat_forward(agent, selected_observations)
    ...
    selected_tails = target_gvf_predictor_forward(
        agent, selected_actor_features, selected_actions
    )
    full_tails.index_copy_(0, row_indices, selected_tails)
    return (
        full_tails.reshape(*bootstrap_rows.shape, *full_tails.shape[1:]).detach(),
        cpu_rng_state,
        device_rng_state,
    )
...
def gvf_source_feature(actor_feature, *, trunk_active):
    """Root GVF at the trunk only while policy updates are permitted."""
    return actor_feature if trunk_active else actor_feature.detach()


@torch.no_grad()
def gvf_gradient_telemetry(parameters, auxiliary, actor, critic):
...
@torch.no_grad()
def capture_gvf_gradient_groups(
    trunk_parameters,
...
if __name__ == "__main__":
    ...
    prefix_horizons = gvf_prefix_horizons(
        args.gvf_gammas, args.gvf_residual_tail_mass
    )
    assert args.gvf_target_batch_size >= 1, "gvf_target_batch_size must be positive"
    assert args.gvf_trunk_grad_clip >= 0.0
    assert args.gvf_predictor_grad_clip >= 0.0
    assert min(
        args.gvf_reward_loss_weight,
        args.gvf_control_loss_weight,
        args.gvf_continuation_loss_weight,
    ) >= 0.0
    assert args.gvf_trace_lambda == 1.0, (
        "v21 exposes lambda explicitly but uses a single full-prefix n-step target; "
        "set gvf_trace_lambda=1"
    )
    assert args.tail_risk_window >= 1
    assert len(args.tail_risk_thresholds) >= 1
    assert args.separate_grad_clip, "GVF union requires separate gradient groups"
    ...
    assert not args.clip_reward, "grounded reward cumulants require unclipped rewards"
    ...
    # Independent persistent stream for the one-sample frozen-policy GVF tail.
    # fork_rng restores the main generator, and these states never alias probe state.
    gvf_aux_cpu_rng_state = None
    gvf_aux_cuda_rng_state = None
    ...
    gvf_params = agent.gvf_parameters()
    gvf_trunk_params = agent.gvf_trunk_parameters()
    gvf_predictor_params = agent.gvf_predictor_parameters()
    assert {id(parameter) for parameter in gvf_params} == {
        id(parameter)
        for parameter in (gvf_trunk_params + gvf_predictor_params)
    }
    predictor_optimizer = optim.Adam(
        gvf_predictor_params,
    ...
    def gvf_update_fn(actor_feature_, action_):
        return gvf_predictor_forward(agent, actor_feature_, action_)

    def target_gvf_fn(actor_feature_, action_):
        return target_gvf_predictor_forward(agent, actor_feature_, action_)
    ...
    if args.compile:
        ...
        gvf_update_fn = torch.compile(
            gvf_update_fn, mode=args.compile_mode, dynamic=False
        )
        target_gvf_fn = torch.compile(
            target_gvf_fn, mode=args.compile_mode, dynamic=False
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if args.gvf:
                ...
                grounded = build_grounded_cumulants(
                    obs,
                    next_transition_obses,
                    rewards,
                    actions,
                    transition_terminations,
                    transition_valids,
                )
                # Freeze the pre-update source encoder table and predictor for every PPO
                # epoch. Current (state, behavior-action) tails are dense; next-state
                # tails are sparse because only truncations and rollout edges need them.
                current_feature_chunks = []
                for current_chunk in flat_observations.split(
                    args.gvf_target_batch_size
                ):
                    if args.compile:
                        torch.compiler.cudagraph_mark_step_begin()
                ...
                frozen_actor_feats = torch.cat(current_feature_chunks)

                # Stored behavior actions define interior on-policy tails. At final and
                # rollout-edge states, one frozen-policy sample is an unbiased estimator
                # of the action expectation. Its persistent RNG stream is isolated from
                # both PPO and TPO probe sampling. Variable sparse sizes stay eager to
                # avoid recompiling the static dense target graphs.
                target_actions = flat_actions
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                target_tails = retain_graph_output(
                    target_gvf_fn(frozen_actor_feats, target_actions),
                    compiled=args.compile,
                ).reshape(
                    args.num_steps,
                    args.num_envs,
                    len(args.gvf_gammas),
                    grounded.values.shape[-1],
                )
                gvf_next_state_rows = next_state_bootstrap_rows(
                    transition_terminations,
                ...
                (
                    next_state_tails,
                    gvf_aux_cpu_rng_state,
                    gvf_aux_cuda_rng_state,
                ) = build_sparse_next_state_tail_table(
                    agent,
                    next_transition_obses,
                    gvf_next_state_rows,
                    seed=args.seed + 2_000_003,
                    cpu_rng_state=gvf_aux_cpu_rng_state,
                    device_rng_state=gvf_aux_cuda_rng_state,
                )
                gvf_targets = build_grounded_gvf_targets(
                    grounded.values,
                    target_tails,
                    next_state_tails,
                    transition_terminations,
                    ...
                    args.gvf_gammas,
                    prefix_horizons,
                    args.gvf_trace_lambda,
                )
                _, gvf_target_scale, _, _ = grounded_gvf_loss(
                    torch.zeros_like(gvf_targets.values),
                    gvf_targets.values,
                    gvf_targets.masks,
                    grounded.obs_dim,
                    reward_weight=args.gvf_reward_loss_weight,
                    control_weight=args.gvf_control_loss_weight,
                    continuation_weight=args.gvf_continuation_loss_weight,
                )
                gvf_zero_baseline = normalized_gvf_rms(
                    gvf_targets.values,
                    gvf_targets.masks,
                    gvf_target_scale,
                )
                target_mean = (
                    gvf_targets.values * gvf_targets.masks.unsqueeze(-1)
                ).sum(dim=(0, 1)) / gvf_targets.masks.sum(dim=(0, 1)).clamp_min(1).unsqueeze(-1)
                gvf_mean_baseline = normalized_gvf_rms(
                    target_mean.view(1, 1, *target_mean.shape)
                    - gvf_targets.values,
                    gvf_targets.masks,
                    gvf_target_scale,
                )
                gvf_valid_count = gvf_targets.masks.sum(dim=(0, 1)).clamp_min(1)
                gvf_bootstrap_fraction = (
                    gvf_targets.bootstrap_masks & gvf_targets.masks
                ).sum(dim=(0, 1)) / gvf_valid_count
                gvf_observed_mass = (
                    gvf_targets.observed_mass * gvf_targets.masks
                ).sum(dim=(0, 1)) / gvf_valid_count
                gvf_frozen_tail_bootstrap_mass = (
                    gvf_targets.bootstrap_mass * gvf_targets.masks
                ).sum(dim=(0, 1)) / gvf_valid_count
        ...
        b_u = u.reshape(-1)
        if args.gvf:
            b_gvf_targets = gvf_targets.values.reshape(
                args.batch_size,
                len(args.gvf_gammas),
                grounded.values.shape[-1],
            )
            b_gvf_mask = gvf_targets.masks.reshape(
                args.batch_size, len(args.gvf_gammas)
            )
        ...
        if args.gvf:
            gvf_action_sensitivity = torch.zeros(
                len(args.gvf_gammas), device=device
            )
            gvf_band_errors = torch.zeros(len(args.gvf_gammas), device=device)
            gvf_grad_telemetry = {
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.gvf:
                    # Once the breaker fires, GVF becomes a predictor-only
                    ...
                    gvf_source = gvf_source_feature(
                        mb_actor_feat,
                        trunk_active=actor_active,
                    )
                    gvf_prediction = gvf_update_fn(
                        gvf_source, b_actions[mb_inds]
                    )
                    (
                        gvf_loss,
                        _,
                        ...
                        gvf_prediction,
                        b_gvf_targets[mb_inds],
                        b_gvf_mask[mb_inds],
                        grounded.obs_dim,
                        reward_weight=args.gvf_reward_loss_weight,
                        control_weight=args.gvf_control_loss_weight,
                        continuation_weight=args.gvf_continuation_loss_weight,
                        scale=gvf_target_scale,
                    )
                    if (
                        epoch + 1 == args.update_epochs
                        and start + args.minibatch_size >= args.batch_size
                    ):
                        gvf_band_errors = normalized_gvf_rms(
                            gvf_prediction - b_gvf_targets[mb_inds],
                            b_gvf_mask[mb_inds],
                            gvf_target_scale,
                        )
                        ...
                        gvf_prediction_for_sensitivity = gvf_prediction.detach().clone()
                ...
                if args.gvf:
                    agent.zero_grad(set_to_none=True)
                    (args.gvf_coef * gvf_loss).backward(
                        retain_graph=actor_active
                    )
                    (
                        gvf_trunk_gn,
                        ...
                        gvf_trunk_grads,
                        gvf_predictor_grads,
                    ) = capture_gvf_gradient_groups(
                        gvf_trunk_params,
                        gvf_predictor_params,
                        trunk_active=actor_active,
                        trunk_max_norm=args.gvf_trunk_grad_clip,
                        predictor_max_norm=args.gvf_predictor_grad_clip,
                    )
                agent.zero_grad(set_to_none=True)
                actor_grads = {}
                if actor_active:
                ...
                if args.gvf and (
                    ...
                    # compiled policy output has completed critic, GVF, and actor
                    ...
                    with torch.no_grad():
                        diagnostic_gvf_source = gvf_source.detach().clone()
                        signflip_prediction = gvf_update_fn(
                            diagnostic_gvf_source, -b_actions[mb_inds]
                        )
                        gvf_action_sensitivity = normalized_gvf_rms(
                            signflip_prediction - gvf_prediction_for_sensitivity,
                            b_gvf_mask[mb_inds],
                            gvf_target_scale,
                        )
                ...
                if args.gvf and (
                    ...
                    gvf_grad_telemetry = gvf_gradient_telemetry(
                        gvf_trunk_params,
                        gvf_trunk_grads,
                        actor_grads,
                        value_grads,
                ...
                apply_union_optimizer_step(
                    ...
                    auxiliary_gradients=gvf_trunk_grads,
        ...
        if args.gvf:
            # The target snapshot advances once per rollout, after every loss in this
            # update used the same frozen pre-update tail model.
        ...
        if args.gvf:
            device_telemetry.update(
                {
                    "losses/gvf_prediction": gvf_loss,
                    "losses/gvf_obs_innovation": gvf_semantic_losses[0],
                    "losses/gvf_reward": gvf_semantic_losses[1],
                    "losses/gvf_control_energy": gvf_semantic_losses[2],
                    "losses/gvf_continuation": gvf_semantic_losses[3],
                    "losses/gvf_grad_norm": gvf_trunk_gn,
                    "losses/gvf_trunk_grad_norm": gvf_trunk_gn,
                    "losses/gvf_predictor_grad_norm": gvf_predictor_gn,
                    "gvf/delivered_trunk_norm": gvf_grad_telemetry["delivered_norm"],
                    "gvf/actor_cosine": gvf_grad_telemetry["actor_cosine"],
                    "gvf/critic_cosine": gvf_grad_telemetry["critic_cosine"],
                    "gvf/task_cosine": gvf_grad_telemetry["task_cosine"],
                }
            )
            device_telemetry.update(
                build_gvf_band_telemetry(
                    args.gvf_gammas,
                    band_losses=gvf_band_losses,
                    band_errors=gvf_band_errors,
                    target_channel_rms=gvf_target_scale,
                    zero_baseline=gvf_zero_baseline,
                    mean_baseline=gvf_mean_baseline,
                    signflip_action_sensitivity=gvf_action_sensitivity,
                    observed_target_mass=gvf_observed_mass,
                    frozen_tail_bootstrap_mass=gvf_frozen_tail_bootstrap_mass,
                    bootstrap_fraction=gvf_bootstrap_fraction,
                    obs_dim=grounded.obs_dim,
                    reward_index=grounded.reward_index,
                    control_index=grounded.control_index,
                    continuation_index=grounded.continuation_index,
                )
            )
        ...
        if args.gvf:
            writer.add_scalar(
                "gvf_union/predictor_only_delivery",
