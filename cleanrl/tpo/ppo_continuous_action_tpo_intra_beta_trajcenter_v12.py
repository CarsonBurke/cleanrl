# Intra-trajectory TPO, self-normalised target, per-trajectory-centred utility. v12.
#
# v12 = v10 with exactly one change, inside the utility transformation:
#   v10:  u = (A - mean_over_the_whole_rollout(A)) / std_batch,  clipped to +/-3
#   v12:  u = (A - mean_over_that_env's_column(A))  / std_batch,  clipped to +/-3
# The group is unchanged. eta is still solved batch-wide on the full flattened
# rollout, the log-partition is still computed once per rollout after that solve,
# the target is still fixed for all 10 epochs and all 32 minibatches, and the
# target ratios still average to exactly 1. This is a utility transformation and
# nothing else. `--no-traj-center` recovers v10 bit-for-bit.
#
# Why the global mean is nearly not worth removing. Under v10's self-normalised
# target the log-ratio is u/eta - logZ, and logZ = log mean exp(u/eta), so adding a
# constant c to every u shifts u/eta by c/eta and shifts logZ by exactly the same
# amount: the target is invariant. Softmax is shift-invariant, so global centring is
# a no-op on what the actor is fitted to -- exactly so if the clip does not bind, and
# approximately so otherwise, since clamp(-3, 3) is symmetric and therefore not
# shift-equivariant (a level shift moves how much of each tail is truncated; the
# centred and uncentred targets differ by at most 0.0007 in ratio on a clean rollout,
# 0.03 with one 40-sigma outlier, 0.16 with one 200-sigma outlier). Either way
# the level is close to free, and the only centring that can change the target by an
# order more than that is one which shifts trajectories *differentially*.
#
# Why differential shifts are the ones with a bias story. The utility is a GAE
# advantage, so it inherits the critic's error. A critic that misestimates V in the
# region one env happens to be occupying adds a roughly common offset to every
# advantage in that env's 128-step column -- the error is a smooth function of state
# and the column is a short, highly autocorrelated stretch of one trajectory. That
# offset is pure critic bias, not policy-improvement signal, and v10 tilts the whole
# column up or down because of it. Subtracting the column mean deletes it exactly.
#
# The trade, stated honestly. This is not free.
#  * It deletes cross-trajectory ranking. After centring, every column contributes
#    the same total (zero) utility, so the policy can no longer prefer a good
#    rollout over a bad one -- only good-within-each-rollout. If a real return
#    difference between envs is carrying signal, v12 throws it away. On HalfCheetah
#    all 16 envs run the same non-terminating task from the same policy, so the
#    between-env spread should be mostly critic error rather than real; on an env
#    where one seed genuinely finds a better mode, this is the wrong move.
#  * It injects estimation noise. Each column mean is estimated from num_steps=128
#    samples, so it carries its own error of order 0.086 sigma under the iid null
#    below -- and rather more here, because GAE returns within a column
#    are strongly autocorrelated, so the effective sample size is well below 128.
#    We remove a bias of unknown size and pay a known variance for it.
#  * It amplifies the clip. Deleting the between-column variance and then
#    re-whitening rescales the within-column signal by 1/sqrt(1 - spread^2), so
#    utility_clip=3 truncates a strictly larger fraction of the within-column tail
#    than it does in v10. Measured on rollouts built to hold the within-column part
#    fixed: at spread 0.31 the clip binds on 0.34% of samples versus v10's 0.24%, at
#    0.76 on 0.34% versus 0.05%, at 0.90 on 0.34% versus 0.00%. Negligible at small
#    spread (the amplification is 1.05x at spread 0.3, 1.53x at 0.76), and
#    material exactly in the high-spread regime v12 is built for -- so this is a
#    second behavioural change riding along with the centring, not literally nothing
#    else moving. tpo/utility_clip_frac logs it directly; compare it against a
#    --no-traj-center control rather than against a remembered number.
# The diagnostic below is what decides whether the first bullet is even in play.
#
# Diagnostic: tpo/traj_mean_spread = std over envs of the per-column advantage
# means, in units of the batch advantage std. By the law of total variance (equal
# column lengths, biased std) its square is exactly the fraction of total advantage
# variance that per-trajectory centring deletes, which also bounds it above by 1. If
# it logs ~0.02, v12 is removing 0.04% of the variance and cannot possibly matter --
# read it in the first hundred iterations and stop the run if so.
# tpo/traj_mean_spread_z is the same number divided by its iid null,
# sqrt((num_envs - 1) / (num_envs * num_steps)) -- note the (C-1)/C, since the
# numerator is a population std of C column means about their own grand mean;
# dropping it biases z low by 3% at 16 envs and by 22% at 2. z ~ 1 means the
# differential shifts are indistinguishable from sampling noise and v12 is trading
# nothing for noise. z >> 1 is necessary but NOT sufficient for real per-env bias:
# GAE advantages within a 128-step column are strongly autocorrelated, which inflates
# z on its own, so z is an UPPER bound on the genuine-bias content, not a lower one.
# A z near 1 is therefore the conclusive reading (nothing there); a large z only says
# "worth looking at", and the thing to look at is losses/explained_variance.
#
# Episode boundaries. A column is one env's 128 consecutive steps, not one episode.
# HalfCheetah-v4 never terminates and truncates at 1000 steps, so roughly one column
# in eight straddles a reset and its "trajectory" is really two. This is deliberate
# and NOT hidden behind a flag: segmenting on `dones` would produce segments as
# short as one step, and a one-sample segment has its own mean subtracted from
# itself, giving utility exactly 0 -- the sample is silently deleted from the
# update. The failure mode of the simple version (one column in eight mixes two
# episodes' offsets, weakening the correction there) is far milder than the failure
# mode of the correct-looking one (short segments get zero gradient), so the simple
# version is the only one implemented.
#
# The std divisor stays batch-wide, and stays the std of the *centred* utilities, so
# Var(u) = 1 exactly as in v10 and utility_clip=3 keeps its calibration in utility
# units -- though not, per the third bullet above, in advantage units. Per-column std
# would instead be a per-column rescale, which the dual solve largely absorbs anyway,
# and would add a second noisy estimate for nothing.
#
# Inherited caveat, found while testing and left alone deliberately. v10's header
# claims the max_target_ratio floor still bounds the self-normalised target, "since
# exp(u_max/eta - logZ) <= exp(u_max/eta)". That needs logZ >= 0, which Jensen gives
# only when the POST-CLIP utility mean is >= 0. A rollout with a large positive
# outlier is truncated asymmetrically by the clip, the mean goes slightly negative,
# logZ goes with it, and the realised max ratio exceeds the cap by exp(-logZ) -- 10.13
# instead of 10.0 on a deliberately adversarial rollout. v12 does not make this worse
# (it measures 10.11 on the same rollout, marginally better), so it is left as v10 has
# it rather than silently changing a second thing; tpo/target_ratio_max already logs
# the realised value and is the number to watch.
#
# Risk. If tpo/traj_mean_spread is small this is a null result at best and a small
# noise injection at worst. If it is large, the sign is genuinely uncertain: large
# spread means either large critic bias (v12 helps) or large real between-env return
# differences (v12 hurts, because it deletes them). Watch traj_mean_spread against
# losses/explained_variance -- low explained variance with high spread is the case
# v12 is built for.
#
# Hypothesis: v12 >= v10 (5894 at 8M on HalfCheetah), driven by removing per-column
# critic bias from the utility, with tpo/traj_mean_spread logging comfortably above
# the 1/sqrt(128) noise floor.
#
# --- v10, on which this is built, unchanged below -------------------------------
#
# Intra-trajectory TPO with a self-normalised target ratio. v10.
#
# v10 = v7 with one change: the per-sample target ratio is divided by its own
# batch mean, w_i = exp(u_i/eta) / mean_j exp(u_j/eta). Nothing else moves.
#
# Why. A density ratio has to integrate to one against the measure it is a ratio
# for: E_pi_old[q/pi_old] = 1, necessarily. v7's target does not. After whitening
# the utilities have mean zero, but Jensen puts mean(exp(u/eta)) above 1 anyway --
# measured 1.069 at eta 2.33 on this codebase -- so v7 asks, on average, for every
# executed action to become 6.9% more likely. No policy can grant that; a normalised
# density can only approximate it by sharpening everywhere. Dividing by the batch
# mean makes the demand exactly consistent, and it is not an ad-hoc fix: the result
# w_i = N*q_i for q = softmax(u/eta) is precisely the self-normalised importance
# estimate of the REPS target ratio q(s,a)/pi_old(s,a) under the pooled measure.
# v7 was using the unnormalised, biased version of the same estimator.
#
# What this is a controlled test of, given v9. v9 replaced the whole loss with the
# TPO paper's group cross entropy and lost badly (1028 vs 3331 at 400k on
# HalfCheetah, at matched realized KL 0.028, with entropy -4.87 vs -8.97 and Beta
# concentration 15.6 vs 48.5 -- it moved as far and converted almost none of it).
# But v9 changed two things at once:
#   (A) it zeroed the coherent sharpening drive, since softmax coefficients p - q
#       sum to zero identically;
#   (B) it replaced a per-state absolute target ("raise this action's density by
#       w_i") with a cross-state share ("raise this action's share of the batch"),
#       which with one action per state is a constraint on unrelated states that no
#       policy improvement corresponds to.
# v10 changes (A) alone and leaves (B) as v7 had it. Its coefficients sum to zero
# at logratio = 0 exactly as v9's do, but each sample still carries an absolute
# per-state target, and as the policy moves the sum is free again. So:
#   v10 ~= v7  =>  v9 died of (B), the cross-state normalisation, and the paper's
#                  group form simply needs several samples per context.
#   v10 << v7  =>  v9 died of (A), and the Jensen inflation in v7's target is not a
#                  defect but the active ingredient -- an implicit entropy penalty
#                  doing real work at ent_coef = 0.
# Either answer is worth knowing, and the second would be the more surprising.
#
# Read it alongside v8, which moves (A) the other way by keeping the advantage
# level: target_ratio_mean 1.04-1.16 against v7's ~1.07 and v10's exactly 1.0. The
# three runs are a sweep in the level with everything else held fixed.
#
# Implementation notes. The partition is computed once per rollout on the full
# batch, like the utilities and eta, so the target stays a single fixed quantity for
# the whole update. It is a shift in log-target-ratio space, so the solved eta is
# untouched: KL(target ‖ behaviour) is a property of the normalised target and does
# not see the constant. The eta floor is left at v7's value; since the true maximum
# ratio is now exp(u_max/eta - logZ) <= exp(u_max/eta), that floor is still a valid
# bound, just a slightly conservative one. tpo/target_ratio_mean should log exactly
# 1.0 and is a free correctness check on the whole path.
#
# Hypothesis: v10 matches or beats v7 kl3 (5431 at 8M). If it does, TPO's target
# should be self-normalised everywhere, including in the LLM setting, and v9's
# failure localises cleanly to the grouping rather than to the level.
#
...
class Args:
    ...
    traj_center: bool = True
    """subtract each env-column's own advantage mean instead of the rollout's, before
    the batch-wide whitening. The v12 change; False is an exact v10 control.

    Only the *centring* moves. eta, the log-partition and the clip are untouched and
    remain batch-wide and once-per-rollout"""
...
def trajectory_mean_spread(advantages: torch.Tensor) -> float:
    """Std over env-columns of the per-column advantage means, in batch-std units.

    The single number that says whether v12 can matter at all. ``advantages`` is
    ``(num_steps, num_envs)``; each column is one env's rollout segment.

    With equal-length columns and biased (population) variance the law of total
    variance is an identity on the empirical distribution,

        Var(A) = mean_c Var(A_c) + Var_c(mean(A_c)),

    so the square of this quantity is *exactly* the fraction of total advantage
    variance that per-trajectory centring deletes, and is bounded above by 1. Near
    0 means v12 is a no-op. Compare against :func:`trajectory_spread_null`.

    Returns NaN, not 0, on a non-finite rollout: 0 is the "nothing to remove"
    reading and must not be produced by the rollout that is about to kill the run.
    """
    a = advantages.double()
    ...
    if float(std) <= 0.0:
        return 0.0
    return float(a.mean(dim=0).std(unbiased=False) / std)


def trajectory_spread_null(num_steps: int, num_envs: int) -> float:
    """Expected :func:`trajectory_mean_spread` under iid advantages.

    The numerator is a *population* std of ``num_envs`` column means about their own
    grand mean, so its expectation is ``sigma^2 * (C - 1) / (C * T)``, not
    ``sigma^2 / T``. Dropping the ``(C - 1) / C`` biases the reported z low by 3% at
    16 envs and 22% at 2, which matters precisely when someone lowers ``--num-envs``
    and then trusts "z ~ 1 means noise".

    This is only the *iid* null. Within-column autocorrelation, which GAE returns
    have in abundance, inflates the realised spread above it with no critic bias at
    all, so z > 1 is necessary and not sufficient evidence of per-env bias.
    """
    if num_envs < 2 or num_steps < 1:
        raise ValueError(f"need >=2 envs and >=1 step, got {num_envs}, {num_steps}")
    return math.sqrt((num_envs - 1) / (num_envs * num_steps))


def tpo_utility_trajectory(
    advantages: torch.Tensor, utility_clip: float | None, traj_center: bool
) -> torch.Tensor:
    """v12's utility: centre per env-column, then whiten and clip batch-wide.

    ``advantages`` is the unflattened ``(num_steps, num_envs)`` rollout. The return
    is flattened with ``reshape(-1)`` from that same shape, so it is aligned
    element-for-element with ``advantages.reshape(-1)`` -- i.e. with ``b_obs``,
    ``b_zs`` and ``b_logprobs``, which are flattened the same way. Centring is done
    *before* the reshape precisely so the alignment cannot drift.

    ``traj_center=False`` delegates to :func:`tpo_utility` on the flattened batch,
    which is v10 exactly, not an approximation of it.

    Why the divisor is still batch-wide: the eta bracket, ``utility_clip`` and the
    ``max_target_ratio`` floor are all calibrated to unit-variance utilities. The
    numerator here already has global mean exactly 0 (each column's residuals sum
    to 0), so dividing by its batch std restores Var(u) = 1 and leaves the clip in
    the same relative position as in v10. A per-column std would instead be a
    per-column rescale -- a second noisy estimate, and one the dual solve mostly
    absorbs anyway.
    """
    if advantages.dim() != 2:
        raise ValueError(f"expected (num_steps, num_envs) advantages, got {tuple(advantages.shape)}")
    if not traj_center:
        return tpo_utility(advantages.reshape(-1), utility_clip)
    if advantages.shape[0] < 2:
        # A length-1 column has its own mean subtracted from itself, so every utility
        # is exactly 0, the solver returns eta_max on the constant-utility guard, and
        # every target ratio is 1: the actor silently freezes for the whole run with
        # no error and no NaN. This is the same degeneracy the header rejects
        # per-episode segmentation for; fail loudly rather than train a no-op.
        raise ValueError("traj_center needs num_steps >= 2; a 1-step column centres to exactly 0")
    centered = (advantages - advantages.mean(dim=0, keepdim=True)).reshape(-1)
    # Biased std, matching tpo_utility, so a degenerate rollout stays finite.
    std = centered.std(unbiased=False)
    utility = torch.where(std > 1e-6, centered / std.clamp_min(1e-6), centered)
    if utility_clip is not None:
        utility = utility.clamp(-utility_clip, utility_clip)
...
def tpo_log_partition(utility: torch.Tensor, eta: float) -> float:
    ...
    exactly 1, which is the defining property of a density ratio and the one v7's
    target violated. Equivalently it turns ``w_i`` into ``N * softmax(u/eta)_i``,
    the self-normalised importance estimate of ``q(s,a) / pi_old(s,a)``.

    Computed through ``logsumexp`` so it is finite for any eta in the bracket, and
    once per rollout so the target does not move during the update.
    """
    utility = utility.detach().double()
...
def tpo_intra_loss(
    ...
    ratio ``w = exp(utility / eta - log_partition)``, which averages to 1 over the
    rollout. Non-negative, convex in ``logratio``, and zero exactly at ``r = w``.

    Note ``utility = 0`` no longer gives zero gradient at ``r = 1``: a sample of
    exactly average utility is asked for ``exp(-log_partition) < 1``, because the
    tilt has to take mass from somewhere to give it to the samples above average.
    That is the correction, not a side effect of it.
...
if __name__ == "__main__":
    ...
    # Constant for the run; the denominator tpo/traj_mean_spread must beat to mean
    # anything. Computed once so the logged z is comparable across configurations.
    spread_null = trajectory_spread_null(args.num_steps, args.num_envs)
    print(f"per-trajectory centring {'on' if args.traj_center else 'off'}; iid spread null {spread_null:.4f}")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # No b_advantages: v12's utility is built from the *unflattened* advantages
        # so the per-column centring happens before the reshape. Both flatten the same
        # (num_steps, num_envs) -> row-major way, and b_obs/b_zs/b_logprobs above use
        # the identical reshape, so flat index k is (k // num_envs, k % num_envs)
        # everywhere and the utilities stay aligned with the samples they label.
        ...
        #
        # v12: the centring is per env-column, taken from the unflattened advantages
        # so that the result is aligned with the flattened batch by construction.
        # Everything downstream -- whitening, clip, eta, partition -- is still
        # batch-wide and still computed exactly once here.
        traj_spread = trajectory_mean_spread(advantages)
        b_utilities = tpo_utility_trajectory(advantages, args.utility_clip, args.traj_center)
        eta, solved_kl = tpo_solve_eta(
            b_utilities, kl_budget, solver_eta_min, args.eta_max, args.eta_solver_iters
        )
        # A shift in log-target-ratio space, so it cannot move the solved eta; it is
        # computed after the solve purely because it depends on it.
        ...
        target_ratios = (b_utilities.double() / eta - log_partition).exp()
        writer.add_scalar("tpo/target_ratio_max", float(target_ratios.max()), global_step)
        # Exactly 1 by construction. A free end-to-end check that the partition, the
        # utilities and eta all refer to the same batch; drift here is a real bug.
        writer.add_scalar("tpo/target_ratio_mean", float(target_ratios.mean()), global_step)
        writer.add_scalar("tpo/log_partition", log_partition, global_step)
        # v12's decisive diagnostic. Its square is the fraction of advantage variance
        # that per-trajectory centring deletes, so a value near 0 means the variant is
        # a no-op regardless of how the return curve looks. The _z form divides by the
        # iid null: z ~ 1 is conclusive sampling noise; z >> 1 only says "worth
        # looking at", since within-column autocorrelation inflates z with no critic
        # bias at all. Both are logged under --no-traj-center too, so the control run
        # reports the same numbers without acting on them.
        writer.add_scalar("tpo/traj_mean_spread", traj_spread, global_step)
        writer.add_scalar("tpo/traj_mean_spread_z", traj_spread / spread_null, global_step)
        # The clip amplification the centring drags along: re-whitening after removing
        # the between-column variance scales the within-column signal by
        # 1/sqrt(1 - spread^2), so utility_clip truncates more. Compare against a
        # --no-traj-center control on the same env.
        if args.utility_clip is not None:
            writer.add_scalar(
                "tpo/utility_clip_frac",
                float((b_utilities.abs() >= args.utility_clip * (1 - 1e-6)).float().mean()),
                global_step,
            )
