# Intra-trajectory TPO with a Beta action policy and a dual-solved eta. v6.
#
# v6 = v2 with the eta hyperparameter eliminated. Everything else -- Beta policy,
# ReluSq trunks, batch-scope utility whitening, the guarded loss -- is v2 verbatim.
#
...
class Args:
    ...
    kl_budget: float = 0.125
    """the trust region, in nats: KL(target ‖ behaviour) over the pooled rollout.
    eta is solved from this each iteration instead of being set directly. The
    default is 1/(2*2^2), the KL that fixed eta=2 implies for Gaussian utilities,
    so the run is directly comparable to the fixed-eta=2 reference"""
    eta_min: float = 0.05
    """lower bracket for the eta bisection; the solver returns this when the
    budget exceeds the largest KL the clipped utilities can express"""
    eta_max: float = 50.0
...
def tpo_solve_eta(
    ...
    Both brackets are saturating rather than error conditions. Hitting ``eta_min``
    means the budget is larger than the clipped utilities can express, so the
    solver returns the most aggressive target available. Hitting ``eta_max`` means
    the rollout carries no advantage signal; the utilities are then all zero, every
    target ratio is 1, and the actor is exactly neutral regardless of eta.
    """
    ...
    # A variance-free rollout gives KL == 0 at every eta and no root exists.
    if float(utility.abs().max()) <= 1e-12:
        return eta_max, 0.0
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        eta, solved_kl = tpo_solve_eta(
            b_utilities, args.kl_budget, args.eta_min, args.eta_max, args.eta_solver_iters
        )
        ...
        # How much of the demanded move the Beta family can actually realise. A
        # drifting ratio means the budget is not controlling the true policy step.
        writer.add_scalar(
            "tpo/kl_efficiency",
            approx_kl.item() / solved_kl if solved_kl > 0.0 else 0.0,
            global_step,
        )
