# TPO on a latent-successor critic: v23's vector-preference chassis, v10's TPO actor.
#
# The two halves of the HalfCheetah plateau diagnosis, combined. The TPO line
# (tpo_intra_beta_*) plateaus for two independent reasons: its SHAPE is set by the
# actor -- an unnormalised target ratio w = exp(u/eta) whose mean exceeds 1 by
# Jensen, a coherent self-imitation push that collapsed the Beta from entropy
# -0.5 to -22.8 while healthy on-policy runs end near -10 -- and its LEVEL is set
# by the critic: at explained variance ~0.6 (bottom quartile of this repo's
# on-policy 8M runs) 5400 is exactly the median return of that EV band.
#
# v10 fixed the shape by self-normalising the target (w = N*softmax(u/eta), mean
# exactly 1) and reached 5115 by 2.1M -- within 320 of v7's 8M final -- carrying
# 6x less Beta concentration. It is still stuck at the same critic. This file
# keeps v10's actor and swaps the stock scalar-MSE critic for v23's, which is the
# best on-policy 8M result in the repo (9141).
#
# What changes relative to v23, and nothing else:
#   1. The clipped surrogate max(-A*r, -A*clamp(r, 1-c, 1+c_hi)) is replaced by
#      the Bregman fit L = w*(x - log x - 1), x = r/w, whose gradient is (r - w)
#      and whose fixed point is r = w exactly. No ratio clip.
#   2. The trust region moves from a fixed clip width to a dual-solved temperature:
#      eta solves KL(target || behaviour) = kl_budget each rollout, and a
#      proportional controller drives the realised approx_kl to realized_kl_target.
#   3. target_kl rises 0.03 -> 0.09. It was v23's trust region; it is now only a
#      safety leash, because TPO's controller already targets 0.03 and a leash at
#      the same value would truncate every update and hide the controller's error.
#
# The utility is v23's vector policy credit -- covector . vector-TD-trace -- whitened
# and clipped at 3 sigma. v23 deliberately scales that credit without centring it;
# under a self-normalised target neither choice reaches the actor: softmax is
# shift-invariant, the dual solve is scale-equivariant (KL(c*u, c*eta) = KL(u, eta)),
# and the clip is applied after whitening, so it lands in the same place either way.
# The actor is invariant to any positive affine map of v23's credit.
#
# Hypothesis: the plateau's shape and level are separable defects, so fixing both
# multiplies rather than overlaps. TPO's early-training advantage is real (it led
# every top on-policy run at 1M) and was being spent down by entropy collapse;
# on a critic with EV ~0.95 that lead should compound instead. Target: beat v23's
# 9141 at 8M, and beat it by more at 2M than at 8M.
#
# Risk 1: TPO carries no ratio clip, and a controlled probe put the clip's entropy
# protection at ~4.5x on pure-noise advantages. v23's credit is a learned preference
# direction dotted into a TD trace, which is noisier than a GAE advantage. If entropy
# collapses here it is the clip's absence, not the Jensen level, and losses/entropy
# against v23's own trace is the readout.
#
# Risk 2: the eta bracket is an absolute constant while the credit's shape is not. The
# floor (utility_clip / log max_target_ratio = 1.3029) can serve ~0.29 nats on whitened
# gaussian utilities but only ~0.06 at 1% sparsity and ~0.005 at 0.1%. If v23's credit
# is spiky enough that tpo/kl_at_floor drops below realized_kl_target, no budget can
# reach the setpoint and tpo/kl_budget ratchets into its 1.0 bound. Watch
# tpo/kl_at_floor, tpo/eta_at_floor and tpo/utility_clip_fraction over the first ~50
# iterations; the fix is raising max_target_ratio, which lowers the floor.
#
# ===================================================================================
# Vector-valued policy credit from a LeJEPA latent-successor critic.
#
# The critic predicts one rich discounted-state embedding. Its vector TD(lambda)
# residual is kept vector-valued through the trace. A detached state-conditioned
# preference field supplies a direction in that space; its dot product with the vector
# trace is used directly for policy credit, without learning or decoding V(s). The
# preference field learns only pairwise return ordering between equal-horizon rollouts,
# never return magnitudes, and cannot shape the critic, trunk, or LeJEPA. Procrustes
# transport pins LeJEPA's coordinate frame without an EMA teacher.
#
# Base: v22 detached-HL-Gauss. The categorical evaluator and every scalar-value
# bootstrap are removed. Historical base notes below are superseded where they describe
# scalar evaluation.
#
...
class Args:
    ...
    # TPO replaces the clip entirely; this is only the |ratio-1| threshold the
    # losses/clipfrac diagnostic counts, kept at v23's clip width so the series
    # stays readable across the two files.
    move_threshold: float = 0.2
    ent_coef: float = 0.0
    # --- TPO actor (verbatim from tpo_intra_beta_selfnorm_v10) ---------------------
    realized_kl_target: float | None = 0.03  # approx_kl the controller drives the update to
    kl_budget: float = 0.08          # initial nats asked of the dual solve; controller owns it after
    kl_budget_gain: float = 0.3      # proportional gain on log(realized / target)
    kl_ratio_clip: float = 3.0       # bound on one iteration's influence over the budget
    kl_budget_bounds: tuple[float, float] = (1e-4, 1.0)
    max_target_ratio: float = 10.0   # per-sample cap; sets the eta floor via utility_clip
    eta_min: float = 0.05
    eta_max: float = 50.0
    eta_solver_iters: int = 40
    utility_clip: float | None = 3.0  # sigmas, applied after whitening the policy credit
    logratio_guard: float = 20.0     # linearise exp above this rather than clamping it
    ...
    target_kl: float = 0.09          # SAFETY leash only (3x realized_kl_target). Under PPO this WAS
                                     # the trust region; under TPO the dual solve is, and a leash at
                                     # 0.03 would cut every update at exactly the value the controller
                                     # is trying to measure, masking its error signal.
    ...
    # --- Vector preference policy credit --------------------------------------------
    preference_hidden: int = 128
    preference_lr: float = 3e-4
    preference_epochs: int = 10
    preference_time_batch: int = 256
    preference_grad_clip: float = 1.0
    preference_temperature: float = 0.2
    preference_smoothness: float = 0.01
    preference_horizon_tolerance: int = 32
    # The preference field is fit from comparisons between environments at the same
    # rollout timestep. This holds the remaining horizon fixed on the fixed-length
    # MuJoCo tasks instead of teaching the field that "earlier in an episode" is value.
    # Only trajectories whose next reset boundary is present in this rollout are used.
    # Its input and vector trace are hard-detached, so ordinal supervision cannot turn
    # the representation into a disguised scalar critic.
...
# =====================================================================================
# TPO actor: target construction, dual solve, KL controller and Bregman loss.
# Copied verbatim from ppo_continuous_action_tpo_intra_beta_selfnorm_v10.py so the
# two files can be diffed for the actor and differ only in the critic.
# =====================================================================================


def tpo_utility(advantages, utility_clip):
    """Whiten advantages into TPO utilities, then clip.

    A batch with no advantage variance maps to all-zero utility, making the actor
    exactly neutral on it -- which is what iteration 1 needs here, since v23 holds
    the policy fixed by zeroing the preference direction during warm-up.
    """
...
def tpo_target_kl(utility, eta):
    """KL(target || behaviour) for the anchored TPO target at temperature ``eta``.

    One action per state degenerates the per-state expectation to a single draw, so
    the batch is pooled into one expectation over (s, a) ~ pi_old, as in REPS. With
    ``w_i ∝ exp(u_i / eta)``, KL(eta) = E_w[u]/eta - log E_pi_old[exp(u/eta)],
    strictly decreasing in eta.
    """
    z = utility / eta
...
def tpo_solve_eta(utility, kl_budget, eta_min, eta_max, iters):
    """Solve ``KL(eta) = kl_budget``. Returns (eta, KL).

    Stationary point of the convex dual g(eta) = eta*kl_budget +
    eta*log E[exp(u/eta)]; since KL is monotone, bisecting it directly is
    equivalent to minimising g and is not tripped up by the dual's flatness.
    Bisection runs in log-eta. Both brackets saturate rather than erroring and the
    returned KL is then not the budget, so callers that care must compare.
    """
    ...
    # KL is identically 0 for *constant* utilities, not merely zero ones.
...
def eta_floor(utility_clip, max_target_ratio, eta_min):
...
def kl_budget_update(budget, realized_kl, realized_kl_target, gain, ratio_clip, bounds):
    """Proportional control of the KL budget on ``log(realized / target)``.

    Log space, so the correction is relative and behaves the same at budget 0.3 or
    0.003. Monotone: overshooting shrinks the budget, which raises eta, which demands
    a smaller move. ``ratio_clip`` bounds one iteration's influence so a degenerate
    rollout cannot slam the budget into a bound and strand it there.
    """
...
def tpo_log_partition(utility, eta):
    """``log mean_i exp(u_i / eta)`` over the pooled rollout, in float64.

    Subtracting this from every ``u_i / eta`` makes the target ratios average to
    exactly 1 -- the defining property of a density ratio, and the one v7 violated.
    Equivalently it turns ``w_i`` into ``N * softmax(u/eta)_i``, the self-normalised
    importance estimate of ``q(s,a) / pi_old(s,a)``. This is the whole of v10.
    """
    utility = utility.detach().double()
    return float(torch.logsumexp(utility / eta, 0) - math.log(utility.numel()))


def tpo_intra_loss(logratio, utility, eta, logratio_guard, log_partition):
    """Per-sample TPO loss with gradient (ratio - target).

    ``L = w * (x - log x - 1)`` for ``x = r/w``, ``r = exp(logratio)`` and target
    ``w = exp(utility/eta - log_partition)``, which averages to 1 over the rollout.
    Non-negative, convex in ``logratio``, zero exactly at ``r = w``.

    A sample of exactly average utility is asked for ``exp(-log_partition) < 1``,
    not 1: the tilt has to take mass from somewhere to give it to the samples above
    average. That is the correction, not a side effect of it.

    Only ``exp`` is guarded, by linearisation rather than clamping, so above the
    guard the gradient saturates at ``exp(guard) - w`` instead of vanishing. The
    linear ``-w * logratio`` term is never guarded, so a heavily suppressed sample
    keeps its full restoring ``-w`` gradient however far it has been pushed.
    """
    log_target_ratio = utility.detach() / eta - log_partition
...
class VectorPreferenceField(nn.Module):
    """A state-conditioned unit covector over the rich value space.

    Both inputs are detached here, rather than relying on callers to preserve the
    one-way boundary. Ordinal reward supervision may orient this field, but it cannot
    scalarize or reshape either the shared state representation or the vector trace.
    """

    def __init__(self, feature_dim, hidden, value_dim):
        super().__init__()
        self.body = nn.Sequential(
            layer_init(nn.Linear(feature_dim, hidden)),
            LeakyReLUSquared(),
            layer_init(nn.Linear(hidden, value_dim), std=0.01),
        )

    def direction(self, state_features):
        return F.normalize(self.body(state_features.detach()), dim=-1, eps=1e-6)

    def forward(self, state_features, vector_trace):
        direction = self.direction(state_features)
        score = (direction * vector_trace.detach()).sum(-1)
        return score, direction


def local_covector_preference_loss(
    preference,
    embeddings,
    vector_traces,
    returns,
    ...
    """Orient a local covector using only ordinal labels on nearby state pairs."""
    directions = preference.direction(embeddings)
    partner_directions = directions.gather(
        1, partner_env.unsqueeze(-1).expand_as(directions)
    )
    partner_traces = vector_traces.detach().gather(
        1, partner_env.unsqueeze(-1).expand_as(vector_traces)
    )
    ...
    if not valid.any():
        zero = directions.sum() * 0.0
        return zero, zero, valid, directions
    trace_difference = F.normalize(
        vector_traces.detach() - partner_traces, dim=-1, eps=1e-6
    )
    local_direction = 0.5 * (directions + partner_directions)
    margin = (local_direction * trace_difference).sum(-1)
    signed_margin = return_difference.sign() * margin
    ordinal_loss = F.softplus(-signed_margin[valid] / temperature).mean()
    smoothness = (
        1.0 - (directions * partner_directions).sum(-1)
    )[valid].mean()
    return ordinal_loss + smoothness_weight * smoothness, ordinal_loss, valid, directions


@torch.no_grad()
def local_preference_metrics(
    directions, vector_traces, returns, complete, partner_env, temperature
):
    partner_directions = directions.gather(
        1, partner_env.unsqueeze(-1).expand_as(directions)
    )
    partner_traces = vector_traces.gather(
        1, partner_env.unsqueeze(-1).expand_as(vector_traces)
    )
    partner_returns = returns.gather(1, partner_env)
    partner_complete = complete.gather(1, partner_env)
    return_difference = returns - partner_returns
    valid = complete & partner_complete & return_difference.ne(0)
    trace_difference = F.normalize(vector_traces - partner_traces, dim=-1, eps=1e-6)
    local_direction = 0.5 * (directions + partner_directions)
    margin = (local_direction * trace_difference).sum(-1)
    signed_margin = return_difference.sign() * margin
    accuracy = float((signed_margin[valid] > 0).float().mean()) if valid.any() else float("nan")
    loss = (
        float(F.softplus(-signed_margin[valid] / temperature).mean())
        if valid.any()
        else float("nan")
    )
    return accuracy, loss, valid, signed_margin


@torch.no_grad()
def quartile_pair_accuracies(ordering_variable, signed_margin, valid):
    values = ordering_variable[valid]
    margins = signed_margin[valid]
    if values.numel() == 0:
        return [float("nan")] * 4
    cuts = torch.quantile(values.float(), torch.tensor([0.25, 0.5, 0.75], device=values.device))
    bins = torch.bucketize(values, cuts)
    return [
        float((margins[bins == quartile] > 0).float().mean())
        if (bins == quartile).any()
        else float("nan")
...
if __name__ == "__main__":
    ...
    if args.num_envs < 2:
        raise ValueError("vector preference learning needs at least two parallel environments")
    if args.preference_temperature <= 0:
        raise ValueError("preference_temperature must be positive")
    if not args.separate_grad_clip:
        raise ValueError("vector-preference warm-up requires separate_grad_clip")
    ...
    if auto_alpha:
        raise ValueError(
            ...
            "vector-preference design; use the Beta actor or disable auto_entropy"
        )
    if auto_alpha:
        act_dim = int(np.prod(envs.single_action_space.shape))
    ...
    # preference field so actor sampling remains paired with the strongest control.
    ssl = LeJepaSSL(obs_dim, act_dim_sf, args).to(device)
    with torch.random.fork_rng(devices=[]):
        preference = VectorPreferenceField(
            args.emb_dim, args.preference_hidden, args.emb_dim
        ).to(device)
    ...
    preference_optimizer = optim.Adam(
        preference.parameters(), lr=args.preference_lr, eps=1e-5
    )
    preference_generator = torch.Generator(device=device)
    preference_generator.manual_seed(args.seed + 17021)
    ...
    if args.compile:
        ...
        preference.body = CompiledModule(
            preference.body, mode=args.compile_mode, cudagraphs=False
        )
    ...
    # TPO controller state: the nats the dual solve is asked for. Carried across
    # iterations because what it corrects for -- how much of the demanded move the
    # Beta family can actually realise under actor_grad_clip -- drifts with training.
    ...
    print(
        f"eta bracket [{solver_eta_min:.4f}, {args.eta_max}] caps the target ratio "
        f"at {args.max_target_ratio}"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            preference_optimizer.param_groups[0]["lr"] = frac * args.preference_lr
        ...
        with torch.no_grad():
            ...
            # Nearest cross-environment neighbor at the same rollout timestep. LeJEPA's
            # fixed-frame Euclidean geometry supplies locality; same-t pairing controls
            # remaining horizon without introducing a scalar state-value baseline.
            preference_partner, has_preference_partner, neighbor_distance = (
                nearest_cross_env_pairs(
                    emb_buf,
                    mc_avail,
                    mc_complete,
                    args.preference_horizon_tolerance,
                )
            )
            preference_sample_valid = mc_complete & has_preference_partner

            frozen_preference_direction = preference.direction(
                emb_buf.reshape(-1, args.emb_dim)
            ).reshape(args.num_steps, args.num_envs, sf_dim)
            if preference_ready:
                policy_credit = (
                    frozen_preference_direction * vector_policy_trace
                ).sum(-1)
            else:
                # Iteration one trains the vector critic and the preference field but
                # deliberately leaves the policy fixed. A random covector is not a
                # defensible exploration objective.
                frozen_preference_direction.zero_()
                policy_credit = torch.zeros_like(rewards)
        ...
        # v23's per-minibatch `(direction * trace).sum(-1) / rms` is exactly
        # b_policy_credit up to that rms, and the rms is gone: tpo_utility whitens, so
        # the actor is invariant to any positive affine map of the credit. The batch
        # tensors that fed only the old surrogate (b_vector_trace,
        # b_preference_direction, policy_credit_rms) are dropped with it; the logging
        # block below re-derives its own view from vector_policy_trace.

        # ALGO LOGIC (TPO): the target is fixed for the whole update, so the utility
        # is built once over the rollout rather than per minibatch. eta is then solved
        # from those exact post-clip values -- the clipped values are what the target
        # is actually built from -- so the target sits kl_budget nats from the
        # behaviour policy by construction rather than by whatever a fixed clip implies.
        #
        # v23 scales the credit without centring it. Under the self-normalised target
        # neither choice reaches the actor at all: softmax is shift-invariant, the dual
        # solve is scale-equivariant (KL(c*u, c*eta) = KL(u, eta)), and the 3-sigma clip
        # is applied AFTER whitening, so even the clip lands in the same place.
        b_utilities = tpo_utility(b_policy_credit, args.utility_clip)
        eta, solved_kl = tpo_solve_eta(
            b_utilities, kl_budget, solver_eta_min, args.eta_max, args.eta_solver_iters
        )
        # A shift in log-target-ratio space, so it cannot move the solved eta; computed
        # after the solve purely because it depends on it.
        log_partition = tpo_log_partition(b_utilities, eta)
        eta_at_floor = eta <= solver_eta_min * (1.0 + 1e-6)
        eta_at_ceiling = eta >= args.eta_max * (1.0 - 1e-6)
        # The largest KL the bracket can serve for THIS rollout's utilities. Whitened
        # gaussian credit puts it near 0.29, far above realized_kl_target, but a spiky
        # covector-dot-trace credit drives it down fast (~0.06 at 1% sparsity, ~0.005 at
        # 0.1%). If this falls under realized_kl_target the controller cannot reach its
        # target at any budget and will ratchet kl_budget into its upper bound; that is
        # a bracket mis-specification, not a controller fault, and this is how to see it.
        kl_at_floor = tpo_target_kl(b_utilities.detach().double(), solver_eta_min)
        ...
        active_critic_params = (
            critic_params if preference_ready else list(agent.critic_head.parameters())
        )
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Guarded to match the loss, which linearises exp above the guard.
                ratio = logratio.clamp_max(args.logratio_guard).exp()

                with torch.no_grad():
                    old_approx_kl = (-logratio).mean()
                    approx_kl = ((ratio - 1) - logratio).mean()
                    clipfracs += [((ratio - 1.0).abs() > args.move_threshold).float().mean().item()]
                    approx_kls += [approx_kl.item()]

                # Policy loss: fit the executed action's density to the anchored TPO
                # target pi_old * exp(u/eta), self-normalised so the target ratios average
                # to exactly 1 over the rollout. No clipping -- the trust region is the
                # solved eta, not a ratio bound.
                mb_utilities = b_utilities[mb_inds]
                pg_loss = tpo_intra_loss(
                    logratio, mb_utilities, eta, args.logratio_guard, log_partition
                ).mean()

                with torch.no_grad():
                    # |ratio - target| is the TPO fit error; it should shrink across
                    # epochs because the target does not move. Its max is the per-sample
                    # gradient coefficient, so a large value means one sample is
                    # dominating the minibatch direction.
                    coefficient = ratio - (mb_utilities / eta - log_partition).exp()
        ...
        # The minibatches of an epoch partition the batch, so the mean approx_kl over
        # the last epoch is the full-batch KL of the post-update policy against the
        # behaviour policy -- the move that actually happened. The target_kl break is at
        # epoch level and batch_size is a multiple of minibatch_size, so a partial epoch
        # is unreachable; min() only keeps the slice honest if that ever stops holding.
        # Inherited caveat from v10: each minibatch's approx_kl is measured at that
        # minibatch's parameters, so this under-reads the true post-update KL by about
        # half an epoch of movement. The controller's setpoint absorbs a constant bias.
        realized_kl = float(np.mean(approx_kls[-min(args.num_minibatches, len(approx_kls)) :]))
        budget_used = kl_budget
        # Iteration one zeroes the preference direction on purpose, so its utilities are
        # identically zero and its realised KL is ~0. Integrating that would be the
        # controller reacting to a deliberately frozen policy, so skip it.
        if preference_ready:
        ...
        # ---- local ordinal preference field -----------------------------------------
        # The actor consumed the field as it existed BEFORE seeing this rollout. Fit on
        # current local comparisons only after the policy update, for use next iteration.
        # This one-iteration cross-fit prevents the field from explaining action noise and
        # immediately feeding that memorized explanation back as policy credit.
        frozen_pair_accuracy, frozen_pair_loss, preference_valid, frozen_signed_margin = (
            local_preference_metrics(
                frozen_preference_direction,
                vector_policy_trace,
                returns,
                preference_sample_valid,
                preference_partner,
                args.preference_temperature,
            )
        )
        preference_loss_sum = preference_ordinal_sum = preference_grad_norm_sum = 0.0
        preference_steps = 0
        for _ in range(args.preference_epochs):
            time_permutation = torch.randperm(
                args.num_steps, device=device, generator=preference_generator
            )
            for preference_start in range(0, args.num_steps, args.preference_time_batch):
                time_indices = time_permutation[
                    preference_start : preference_start + args.preference_time_batch
                ]
                preference_loss, ordinal_loss, valid_pairs, _ = local_covector_preference_loss(
                    preference,
                    emb_buf[time_indices],
                    vector_policy_trace[time_indices],
                    returns[time_indices],
                    preference_sample_valid[time_indices],
                    preference_partner[time_indices],
                    args.preference_temperature,
                    args.preference_smoothness,
                )
                if not valid_pairs.any():
                    continue
                preference_optimizer.zero_grad(set_to_none=True)
                preference_loss.backward()
                preference_grad_norm = nn.utils.clip_grad_norm_(
                    preference.parameters(), args.preference_grad_clip
                )
                preference_optimizer.step()
                preference_loss_sum += preference_loss.item()
                preference_ordinal_sum += ordinal_loss.item()
                preference_grad_norm_sum += float(preference_grad_norm)
                preference_steps += 1
        preference_loss_mean = preference_loss_sum / max(preference_steps, 1)
        preference_ordinal_mean = preference_ordinal_sum / max(preference_steps, 1)
        preference_grad_norm_mean = preference_grad_norm_sum / max(preference_steps, 1)
        preference_ready = preference_steps > 0 or preference_ready

        with torch.no_grad():
            trained_preference_direction = preference.direction(
                emb_buf.reshape(-1, args.emb_dim)
            ).reshape(args.num_steps, args.num_envs, sf_dim)
            trained_pair_accuracy, trained_pair_loss, _, trained_signed_margin = (
                local_preference_metrics(
                    trained_preference_direction,
                    vector_policy_trace,
                    returns,
                    preference_sample_valid,
                    preference_partner,
        ...
        with torch.no_grad():
            ...
            preference_score = (
                frozen_preference_direction.reshape(-1, sf_dim) * vector_trace_flat
            ).sum(-1)
            preference_score_rms = preference_score.square().mean().sqrt()
            preference_score_cosine = preference_score / vector_trace_norm.clamp_min(1e-8)
            coordinate_energy = (
                frozen_preference_direction.reshape(-1, sf_dim) * vector_trace_flat
            ).square()
            ...
            selected_neighbor_distance = neighbor_distance.gather(
                2, preference_partner.unsqueeze(-1)
            ).squeeze(-1)
            valid_neighbor_distance = selected_neighbor_distance[preference_valid]
            neighbor_p50 = (
            ...
            paired_returns = returns.gather(1, preference_partner)
            return_gap = (returns - paired_returns).abs()
            neighbor_quartile_accuracy = quartile_pair_accuracies(
                selected_neighbor_distance, frozen_signed_margin, preference_valid
            )
            return_gap_quartile_accuracy = quartile_pair_accuracies(
                return_gap, frozen_signed_margin, preference_valid
            )
            preference_direction_rank = effective_rank(
                trained_preference_direction.reshape(-1, sf_dim)
            )
            preference_resultant = float(
                trained_preference_direction.reshape(-1, sf_dim).mean(0).norm()
            )
            preference_update_cosine = float(
                (
                    frozen_preference_direction * trained_preference_direction
                ).sum(-1).mean()
            )
        ...
        writer.add_scalar("tpo/eta_at_floor", float(eta_at_floor), global_step)
        writer.add_scalar("tpo/eta_at_ceiling", float(eta_at_ceiling), global_step)
        writer.add_scalar("tpo/kl_at_floor", kl_at_floor, global_step)
        # Spikiness of the credit, read directly: a whitened gaussian clips 0.27% of
        # samples at 3 sigma, so a much larger number is the warning that goes with a
        # small tpo/kl_at_floor.
        writer.add_scalar(
            "tpo/utility_clip_fraction",
            float((b_utilities.abs() >= args.utility_clip - 1e-6).float().mean())
            if args.utility_clip is not None
            else 0.0,
            global_step,
        )
        writer.add_scalar("tpo/kl_budget", budget_used, global_step)
        writer.add_scalar("tpo/solved_kl", solved_kl, global_step)
        writer.add_scalar("tpo/realized_kl", realized_kl, global_step)
        writer.add_scalar("tpo/log_partition", log_partition, global_step)
        writer.add_scalar("tpo/utility_std", float(b_utilities.std(unbiased=False)), global_step)
        # Exactly 1.0 by construction; a drift means eta and the partition disagree.
        writer.add_scalar(
            "tpo/target_ratio_mean",
            float((b_utilities.double() / eta - log_partition).exp().mean()),
            global_step,
        )
        writer.add_scalar(
            "tpo/target_ratio_max",
            float(math.exp(float(b_utilities.max()) / eta - log_partition)),
            global_step,
        )
        if residuals:
            writer.add_scalar(
                "tpo/residual_first_epoch",
                float(np.mean(residuals[: args.num_minibatches])),
                global_step,
            )
            writer.add_scalar(
                "tpo/residual_last_epoch",
                float(np.mean(residuals[-min(args.num_minibatches, len(residuals)) :])),
                global_step,
            )
            writer.add_scalar("tpo/coefficient_max", float(np.max(coef_maxes)), global_step)
        ...
        # ---- vector policy credit and local preference geometry -----------------
        ...
        writer.add_scalar("preference/loss", preference_loss_mean, global_step)
        writer.add_scalar("preference/ordinal_loss", preference_ordinal_mean, global_step)
        writer.add_scalar("preference/grad_norm", preference_grad_norm_mean, global_step)
        writer.add_scalar("preference/steps", preference_steps, global_step)
        writer.add_scalar("preference/one_iteration_ahead_pair_accuracy", frozen_pair_accuracy, global_step)
        writer.add_scalar("preference/one_iteration_ahead_pair_loss", frozen_pair_loss, global_step)
        writer.add_scalar("preference/training_pair_accuracy", trained_pair_accuracy, global_step)
        writer.add_scalar("preference/training_pair_loss", trained_pair_loss, global_step)
        writer.add_scalar("preference/valid_pairs", preference_valid.sum().item(), global_step)
        writer.add_scalar("preference/neighbor_distance_p50", neighbor_p50, global_step)
        writer.add_scalar("preference/neighbor_distance_p90", neighbor_p90, global_step)
        for quartile, accuracy in enumerate(neighbor_quartile_accuracy, start=1):
            writer.add_scalar(
                f"preference/pair_accuracy_neighbor_q{quartile}", accuracy, global_step
            )
        for quartile, accuracy in enumerate(return_gap_quartile_accuracy, start=1):
            writer.add_scalar(
                f"preference/pair_accuracy_return_gap_q{quartile}", accuracy, global_step
            )
        writer.add_scalar("preference/direction_effective_rank", preference_direction_rank, global_step)
        writer.add_scalar("preference/direction_resultant_length", preference_resultant, global_step)
        writer.add_scalar("preference/direction_update_cosine", preference_update_cosine, global_step)
        writer.add_scalar("preference/score_mean", preference_score.mean().item(), global_step)
        writer.add_scalar("preference/score_rms", preference_score_rms.item(), global_step)
        writer.add_scalar("preference/score_positive_fraction", (preference_score > 0).float().mean().item(), global_step)
        writer.add_scalar("preference/score_trace_cosine", preference_score_cosine.mean().item(), global_step)
