# Intra-trajectory TPO on *raw* advantages: no advantage normalisation. v8.
#
# v8 = v7 (dual-solved eta under closed-loop realized-KL control) with the
# centring of advantages removed. The loss, the solver, the eta floor, the agent
# and every default are v7's, so that a run of this file against the v7 kl3 run
# (5431 at 8M) differs in exactly one thing. The anti-windup fix described below
# is present but *off by default* for precisely that reason.
#
# The point of the ablation, and why it is narrower than it looks. Under a
# dual-solved eta, dividing the advantages by their batch standard deviation is
# *exactly* a no-op, not approximately one. KL(eta) is scale-equivariant:
#   z = (c*u)/(c*eta) = u/eta, so the softmax weights, the log-partition and
#   E_w[u]/eta are all unchanged, hence KL(c*u, c*eta) = KL(u, eta).
# So if whitened utilities solve to eta*, raw ones solve to c*eta* for c = std(A),
# and the target ratios exp(u/eta) come out bit-identical. The +/-3 clip is
# equivariant too once it is expressed in standard deviations. The dual solve has
# therefore already absorbed the entire scale question, which is exactly what it
# was built to do -- and it means the *only* part of "advantage normalisation"
# that can change anything at all here is the mean subtraction.
#
# So v8 is precisely: do not centre. u_t = clip(A_t / std(A), +/- 3). The division
# is kept even though it provably cannot matter, because it holds eta, the eta
# bracket and the eta floor in exactly the units v7 used, so this is a genuine
# one-variable ablation rather than a change of units wearing an ablation's name.
#
# What that actually changes. Centring forces sum_t u_t = 0, so the anchored
# target pi_old*exp(u/eta) always asks for exactly as much density to go up as
# down. Without it, a rollout in which every executed action beat its value
# baseline gets a uniformly inflated target, exp(mean(A)/eta) > 1 across the board:
# the target says "make all of these more likely". For a normalised density that is
# only satisfiable by sharpening, so a persistently positive advantage mean becomes
# an entropy-reduction pressure, and a negative one an entropy-increase pressure.
# Whitening deletes that level information and keeps only the within-batch ranking.
#
# How large is the level, actually. Simulated through this file's own GAE recursion
# over 400 rollouts at num_envs=16, num_steps=128: mean(A)/std(A) has mean +0.003,
# std 0.111, median magnitude 0.075, p95 0.21, max 0.41. The spread is 5.7x the
# naive 1/sqrt(2048), because GAE smears each TD residual over ~1/(1-gamma*lambda)
# = 17 steps, so the 2048 samples are worth ~120 independent ones. At the eta this
# codebase actually runs (2.15 early, 1.30 late) that is a 6% median and 17% p95
# coherent upward push on every executed action's density, every iteration.
#
# So this is not a null experiment -- but note what it is and is not. The mean of
# the level across rollouts is +0.003, i.e. essentially zero: under a calibrated
# critic the level is estimator noise, not a property of the batch. A constant
# critic bias b reaches mean(A) damped by (1-gamma)/(1-gamma*lambda) = 0.168, so
# even a poorly fit critic contributes little systematic level. The analogy to the
# paper's group-normalised rewards is therefore weaker than it looks: there a group
# is many samples from *one prompt* and its mean is a real property of that prompt,
# whereas here the group is one rollout of a single continuous task and mean(A) has
# no per-group semantics. What v8 measures is closer to "does a coherent,
# sign-random 6-17% density push per iteration help or hurt", and the honest prior
# is neutral-to-negative given that v7 kl3 already collapses hard on its own
# (beta_concentration 3.4 -> 2437, entropy -0.48 -> -22.6).
#
# Containment, and its limits. The eta floor bounds the uniform push at
# max_target_ratio, since |mean(u)| <= max|u| <= utility_clip -- but that bound is
# 10x, which is not containment of anything that matters; a uniform 10x push *is*
# the entropy collapse the bound is supposed to exclude. The real containment is
# the realized-KL controller, and it lags: escaping the floor means driving the
# budget from 1.0 down below the achievable KL, ~11 iterations at gain 0.3.
#
# There is also a saturation cascade at large levels, which the measured
# distribution puts ~25 sigma away but which is worth naming because the failure is
# silent. The clip is applied after the division, so the level eats the clip
# budget: at mean/std = 3 half the samples pin to +3, the post-clip std falls to
# 0.58, eta hits the floor, and the update is a 7x uniform push with little ranking
# content left; at mean/std = 5 the std is 0.05 and the ranking is gone entirely.
# tpo/utility_std goes to zero along that path and tpo/target_ratio_mean to 10, so
# the diagnostics name it if it ever happens.
#
...
#
# Hypothesis: keeping the advantage level improves return on HalfCheetah-v4 over
# v7 kl3 (5431 at 8M), or -- if it does not -- shows that whitening's contribution
# is entirely in the mean and not in the scale, which the argument above already
# proves it must be.
#
...
# eta is the trust region: u_t is the rollout advantage (uncentred here), in units of
...
#
# Utilities are formed once over the full rollout, not per minibatch, so the
...
class Args:
    ...
    Correct, and off by default only so that this file is a one-variable ablation
    against the v7 kl3 run, which did not have it. That matters here rather than
    being pedantic: eta sat at its floor for 24.4% of that run's iterations and
    continuously past 4.5M steps, which is exactly the regime this changes. Turn it
    on for any run not being compared against that reference."""
    ...
    Uncentred utilities put the batch's mean push under the same cap.
    ...
    eta_min: float = 0.05
    """hard floor on the eta bisection bracket, under the max_target_ratio floor
    that normally binds first. Reached only if the rollout's utilities are tiny"""
    eta_max: float = 50.0
    ...
    """clip uncentred utilities to +/- this many standard deviations, bounding the
...
def tpo_utility(advantages: torch.Tensor, utility_clip: float | None) -> torch.Tensor:
    """Uncentred advantages in units of the batch standard deviation, then clipped.

    v7 subtracted the mean and divided by the std. The division is exactly undone by
    the dual solve (see the header), so it changes nothing about the target and is
    kept here purely to hold eta, the eta bracket and the eta floor in the same
    units as v7 -- which makes this a one-variable ablation. The mean subtraction is
    that one variable, and dropping it lets the *level* of the batch's advantages
    survive into the target as a uniform push on every executed action's density.

    A batch with no advantage spread carries no ranking, and its level cannot be
    told apart from a value-function bias, so it still maps to all-zero utility and
    leaves the actor exactly neutral -- the continuous analogue of skipping a prompt
    group with no reward variance.
    """
    # Biased std keeps a length-1 input finite instead of NaN.
    std = advantages.std(unbiased=False)
    utility = torch.where(
        std > 1e-6, advantages / std.clamp_min(1e-6), torch.zeros_like(advantages)
    )
...
def tpo_target_kl(utility: torch.Tensor, eta: float) -> float:
    ...
    and E_w[u] tends to the batch mean of u) and to log(N / n_max) as eta shrinks
...
def eta_floor(utility_clip: float | None, max_target_ratio: float, eta_min: float) -> float:
    ...
    Unchanged from v7: the utilities are still in units of the batch standard
    deviation, so the clip is still the largest |u| and the floor still binds at the
    same 1.303. Because the utilities are uncentred the cap now also bounds the
    uniform push, since |mean(u)| <= max|u| <= utility_clip.
    """
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # are formed once over the rollout rather than per minibatch. The
        ...
        # The true max, not the exp(utility_clip/eta) bound v7 logged: uncentred
        # utilities need not reach the clip on both sides, and with a negative level
        # the bound overstates the largest demand several-fold.
        writer.add_scalar("tpo/target_ratio_max", float(np.exp(float(b_utilities.max()) / eta)), global_step)
        # The whole content of the ablation: the level whitening would have deleted.
        # Measured on the *advantages*, since post-clip moments are not the quantity
        # -- at a large level the clip flattens the spread and mean(u)/std(u) runs
        # away from mean(A)/std(A) (56 versus 5 at the extreme).
        advantage_std = float(b_advantages.std(unbiased=False))
        writer.add_scalar(
            "tpo/utility_mean_sigmas",
            float(b_advantages.mean()) / advantage_std if advantage_std > 0.0 else 0.0,
            global_step,
        )
        # The uniform density push the level buys: above 1 the target asks for every
        # executed action to become more likely, which a normalised density can only
        # grant by sharpening. This is the mechanism under test.
        writer.add_scalar(
            "tpo/target_ratio_mean", float(np.exp(float(b_utilities.mean()) / eta)), global_step
        )
        # Falls below 1 as the level eats the clip budget and the ranking signal
        # saturates; a sustained collapse here means the update has become a pure
        # uniform push with no ranking content left.
