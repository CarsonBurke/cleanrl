# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted-attention slot transformer v10.
#
...
class InvertedSlotStack(nn.Module):
    ...
    def __init__(self, n_slots, H, n_blocks, d_qk, mlp_mult):
        super().__init__()
        assert n_blocks >= 1
        ...
        self.blocks = nn.ModuleList(
            [InvertedSlotBlock(H, d_qk, mlp_mult) for _ in range(n_blocks)]
        )
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, actor_slots, critic_slots, slot_blocks, slot_d_qk, slot_mlp_mult):
        ...
        self.actor_stack = (
            InvertedSlotStack(actor_slots, H, slot_blocks, slot_d_qk, slot_mlp_mult)
            if actor_slots is not None
            else None
        )
        self.critic_stack = (
            InvertedSlotStack(critic_slots, H, slot_blocks, slot_d_qk, slot_mlp_mult)
