# Pan Goal Solver v11: magnitude-aware local inverse control toward an open goal.
#
...
# scale-invariant reward head learns MSE from detached predicted-next latents;
# freezing it while maximizing R(G) revises the goal head every training cycle.
# There is no PPO, action critic, policy gradient, search, MPC, or rollout.
# Hypothesis: bounded delta features preserve action magnitude on real one-step
# goals while converging to direction-only control for the unreachable ideal.
...
def train_goal_step(agent, goal_optimizer, reward_optimizer, batch, args, device):
    ...
    reward_prediction = agent.reward_predictor(predicted_next_goal.detach())
    ...
    predicted_goal_reward = agent.reward_predictor(goal)
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        if revision_anchor_batch is not None:
            with torch.no_grad():
                ...
                anchor_goal_rewards = agent.reward_predictor(anchor_goals)
                writer.add_scalar(
