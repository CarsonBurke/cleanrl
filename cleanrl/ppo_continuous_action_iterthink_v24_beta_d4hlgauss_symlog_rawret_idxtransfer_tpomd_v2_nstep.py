# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return idx-transfer + TPO-MD v2 (n-step).
#
...
# gamma^n instead of gamma. tpo_probe_steps=1 reproduces the v1 (one-step) probe
# bit-exactly. The pure mirror-descent actor update (adaptive-eta MPO target,
# global-sigma scaling, decoupled dual-backward clipping, circuit breaker) is
# UNCHANGED from TPO-MD v1.
#
# --- inherited TPO-MD v1 notes ---
...
class Args:
    ...
    tpo_probe_steps: int = 2     # n-step probe horizon; step 0 = candidate action, steps 1..n-1 = policy mean action; 1 => exact one-step (v1) behavior
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # TPO-MD probes EVERY env (no state-frac mask): probe_env_ids spans all envs.
            probe_env_ids = np.arange(args.num_envs)
            n_probe = args.tpo_probe_steps
            P = int(probe_env_ids.size)
            # Initial sim snapshot per env: the restore target for every candidate's
            # step 0 AND the post-probe restore.
            init_states = []
        ...
        with torch.no_grad():
            ...
            # bootstrapped frozen value (term_probe=1 iff the probe terminated at
            # ANY of its n steps => bootstrap dropped, sum already truncated).
            scores = r_probe + (args.gamma ** args.tpo_probe_steps) * (1.0 - term_probe) * v_next  # (B, K)
