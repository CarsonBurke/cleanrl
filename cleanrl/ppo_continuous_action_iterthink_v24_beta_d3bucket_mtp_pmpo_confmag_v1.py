# PPO + IterThink v24 Beta + d3bucket MTP + PMPO + CONFMAG v1 (critic-confidence-scaled magpos).
# =====================================================================================
# NOVELTY: magpos weights POSITIVE advantages by the UNBOUNDED magnitude |A|/std. That won
#   HalfCheetah (+18.5%, 9221) but REGRESSED Walker ~3x because it over-amplifies HIGH-VARIANCE
#   advantages near the termination (fall-vs-continue) boundary. This variant scales the positive
#   magnitude weight DOWN exactly where the critic is UNCERTAIN. The v162 HL-Gauss distributional
#   critic already emits, per state, a categorical over a fixed value-bin support, so its predictive
#   STD (crit_std = sqrt(sum_b p_b (support_b - mean)^2)) is a FREE per-state uncertainty signal --
#   already computed as `sigma`/`b_sigma` in this file. We reuse b_sigma[mb_inds] (detached, a
#   constant coefficient like adv_std) and multiply w_pos by conf = 1/(1 + (crit_std/mean(crit_std))
#   * pmpo_conf_scale) in (0,1]: above-average uncertainty => damped, below-average => ~full.
# HYPOTHESIS: damping positive amplification under high critic spread keeps HalfCheetah's gain
#   (dense, low-variance, confident) AND tames Walker's instability (bimodal returns at the fall
#   boundary) => a GENERAL win, not a per-env static knob. Negatives are UNTOUCHED; scaling is
#   POSITIVE-side only. DEFAULTS (pmpo_conf_scale=0.0) reproduce magpos_v1 EXACTLY (byte-for-byte
#   behavior + RNG; no extra tensor ops when off). pmpo_conf_mode="inv" is the implemented scaling.
#
...
class Args:
    ...
    # CONFMAG: critic-confidence scaling of the POSITIVE magnitude weight. 0.0 = OFF (reproduces
    # magpos_v1 exactly). >0 multiplies w_pos by conf = 1/(1 + (crit_std/mean(crit_std))*scale) in
    # (0,1], where crit_std is the HL-Gauss critic predictive std (b_sigma). Higher-than-average
    # per-state uncertainty => conf<1 (damped amplification); lower => conf~1. Negatives untouched.
    pmpo_conf_scale: float = 0.0
    # Confidence scaling mode (reserved for alternatives). "inv" = the 1/(1+z*scale) form above.
    pmpo_conf_mode: str = "inv"
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if pmpo_active:
                    ...
                    if args.pmpo_pos_weight == "tanh":
                        ...
                        if args.pmpo_conf_scale != 0.0:
                            # CONFMAG: damp the positive magnitude weight where the HL-Gauss critic
                            # is uncertain. crit_std is the per-state predictive std (b_sigma, a
                            # detached constant coefficient), normalized by its minibatch mean so
                            # pmpo_conf_scale is a dimensionless knob. Above-average uncertainty =>
                            # conf<1 (Walker fall-vs-continue), below-average => conf~1 (confident).
                            crit_std = b_sigma[mb_inds].detach()
                            conf = 1.0 / (1.0 + (crit_std / (crit_std.mean() + 1e-8)) * args.pmpo_conf_scale)
                            w_pos = w_pos * conf
                    else:
