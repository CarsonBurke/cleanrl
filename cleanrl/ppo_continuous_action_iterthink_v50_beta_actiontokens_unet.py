# PPO + IterThink v50 (SAC-Beta + action-token U-Net transformer trunk). From v49.
#
# Changes from v49:
#   - Actor, critic, and one token per action dimension enter the transformer from
#     layer zero. Action tokens attend to observations throughout the encoder/decoder
#     instead of being produced only by a final linear head.
#   - Beta concentration logits are read directly from per-action tokens with shared
#     scalar heads, giving each actuator its own recurrent attention path while keeping
#     v24.5's shifted log-concentration policy geometry.
#   - Critic uses the critic token; the actor summary token remains in-stream as a
#     global policy workspace that action tokens can attend to.
#
# Inherited from v49:
#   - Token transformer trunk in place of ThinkTrunk's dense/MoE MLP backend.
#   - Per-observation-dimension affine tokens plus learnable latent scratch tokens.
#   - Pre-norm self-attention blocks with ReLUSquared FFNs, QK norm, PyTorch SDPA
#     flash attention, and BF16 autocast on CUDA.
#   - U-Net-style depth residual mixing: encoder token states are injected into
#     matching decoder layers through learned residual skip mixers.
#   - Preserve v24.5's PPO loop, Beta shifted log-concentration actor, C51 critic,
#     rankgauss shaping, clip-higher, and decoupled actor/critic gradient clipping.
#
# --- inherited v24.5 notes ---
#
# v24.3/v24.4 underfit HalfCheetah because the SAC/Tianshou shifted-log cap of 2
# limits each Beta concentration to exp(2)+1≈8.39. At ~1-1.6M steps v24.3 entropy
# stayed around -3.9 while v24 reaches ~-10 late, so the policy cannot become sharp
# enough for PPO's deterministic locomotion phase. v24.5 keeps the SAC-Beta mapping
# and hard-clamp geometry, but widens only the upper shifted-log cap to 8:
#     alpha,beta <= exp(8)+1≈2982
# This preserves the alpha=beta=2 init and tests whether the shifted-log parameter
# family is useful once it is not artificially capped at a high-noise regime.
#
# --- inherited v24.3 notes ---
#
# PPO + IterThink v24.3 (SAC-BETA SHIFTED LOG-CONCENTRATIONS). From v24.2.
#
# v24.3 ports the actual SAC-Beta policy parameterization from Della Libera (2024)
# and ../sac-beta into the default Beta actor. SAC-Beta does NOT decouple mode and
# concentration. Its policy network returns two independent shifted log-concentration
# heads, clips them to the same interval used for log_std-like scale parameters, then
# shifts by +1 to force a concave/unimodal Beta:
#     log_shift_a = clamp(head_a, lo, hi)
#     log_shift_b = clamp(head_b, lo, hi)
#     alpha = exp(log_shift_a) + 1
#     beta  = exp(log_shift_b) + 1
# This is close to v24's independent alpha/beta heads, but replaces softplus with
# clipped shifted-log concentrations. The Gaussian toggle remains v24.2's SAC-style
# log_std control path.
#
# --- inherited v24.2 notes ---
#
# PPO + IterThink v24.2 (SAC LOG-STD WITH STD=1 INIT). From v24.1.
#
# v24.2 keeps v24.1's SAC-style state-dependent log_std parameterization, but fixes
# the PPO initialization mismatch. v24.1's [-5, 2] log_std bounds plus zero head bias
# initialize at the midpoint log_std=-1.5 (std≈0.22), much quieter than v24's std=1
# log-variance init. PPO has no random-action replay warmup, so that low sigma makes
# score gradients hot. v24.2 adds --log-std-init and initializes the bounded head's
# bias through inverse tanh-rescale so the initial policy can start at std=1:
#     log_std = rescale(tanh(raw_logstd), (-1,1), (LOG_STD_MIN, LOG_STD_MAX))
#     std     = exp(log_std)
#     bias    = atanh(rescale(log_std_init, (LOG_STD_MIN, LOG_STD_MAX), (-1,1)))
# while preserving v24's PPO update, native-sample replay, squashed log-prob Jacobian,
# beta distribution path, and optional on-policy SAC-style entropy terms.
#
...
# remains calibrated. With auto-entropy off, the PPO update is unchanged; v24.5 changes
# only the default Beta actor parameterization.
#
...
#
#   actor_dist="beta"  (DEFAULT, SAC-Beta shifted log-concentration path, PPO-wide cap):
#       unimodal Beta, matching ../sac-beta except for a wider upper concentration cap:
#           alpha = 1 + exp(clamp(head_a, beta_log_shift_min, beta_log_shift_max))
#           beta  = 1 + exp(clamp(head_b, beta_log_shift_min, beta_log_shift_max))
...
#
#   actor_dist="gaussian"  (the matched control = SAC-style state-dependent log_std):
#       SAC/SpinUp/Yarats tanh-Gaussian readout. A state-dependent log-STD head
#       (not a flat Parameter, not log-variance), SOFT-bounded by tanh-rescale:
#           log_std = rescale(tanh(raw_logstd), (-1,1), (lo,hi))
#           std     = exp(log_std)
#       then the iterthink/SAC tanh-squash + stable Jacobian on the sample (mean
#       stays raw). Bounds default to SAC's [-5, 2].
#
...
# gaussian path differs from v24 in the scale head: log-variance -> log_std, with
# an explicit std=1 initialization target. Beta differs in concentration mapping:
# softplus(head)+1 -> exp(clipped head)+1.
...
SAMPLE_EPS = 1e-7  # SAC-Beta AD / paper clamp for Beta samples off the open-interval boundary.
...
class Args:
    ...
    # v24.5: action distribution. "beta" (SAC-Beta shifted log concentration, PPO-wide cap) | "gaussian"
    # (SAC-style state-dependent log_std head with soft tanh-rescale bound).
    actor_dist: str = "beta"
    log_std_min: float = -5.0        # gaussian: SAC-style soft log_std lower bound
    log_std_max: float = 2.0         # gaussian: SAC-style soft log_std upper bound
    log_std_init: float = 0.0        # gaussian: initial bounded log_std target (0 => std=1)
    beta_log_shift_min: float = -20.0  # beta: SAC-Beta/Tianshou shifted log-concentration clip
    ...
    hidden: int = 128
    k_blocks: int = 4               # v50: encoder depth; decoder has the same depth.
    n_experts: int = 16             # legacy v24 field, unused by the transformer trunk.
    transformer_heads: int = 4
    transformer_latents: int = 4
    transformer_ffn_mult: int = 2
    transformer_resid_init: float = 0.1
    transformer_skip_init: float = -2.0  # sigmoid ~= 0.12 initial U-Net skip strength.
    transformer_bf16: bool = True
...
def flash_attention(q, k, v):
    # Force FlashAttention on CUDA bf16/fp16 so runtime regressions are not hidden.
    if q.is_cuda and q.dtype in (torch.float16, torch.bfloat16) and sdpa_kernel is not None:
...
class TokenTransformerBlock(nn.Module):
    """Pre-norm self-attention + ReLUSquared FFN with small residual multipliers."""

    def __init__(self, H, num_heads, ffn_mult, resid_init):
        super().__init__()
        assert H % num_heads == 0
        self.num_heads = num_heads
        self.head_dim = H // num_heads
        self.attn_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.ffn_norm = nn.RMSNorm(H, elementwise_affine=False)
        ...
        self.wo = nn.Linear(H, H, bias=False)
        ffn_dim = H * ffn_mult
        self.ff1 = nn.Linear(H, ffn_dim)
        self.ff2 = nn.Linear(ffn_dim, H)
        self.attn_scale = nn.Parameter(torch.full((H,), resid_init))
        self.ffn_scale = nn.Parameter(torch.full((H,), resid_init))
        for module in (self.wq, self.wk, self.wv, self.ff1):
            init_linear_(module, init_scale=1.0)
        for module in (self.wo, self.ff2):
            init_linear_(module, init_scale=resid_init)
    ...
    def forward(self, x):
        ...
        q = self.q_norm(q)
        k = self.k_norm(k)
        attn = flash_attention(q, k, v).transpose(1, 2).reshape(B, S, H)
        x = x + self.attn_scale.view(1, 1, H) * self.wo(attn)
        h = self.ffn_norm(x)
        x = x + self.ffn_scale.view(1, 1, H) * self.ff2(torch.relu(self.ff1(h)).pow(2))
        return x


class UNetSkipMixer(nn.Module):
    """Depth-symmetric residual skip mixer from encoder tokens into decoder tokens."""

    def __init__(self, H, skip_init, resid_init):
        super().__init__()
        self.x_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.skip_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.proj = nn.Linear(2 * H, H)
        self.gate = nn.Parameter(torch.full((H,), skip_init))
        init_linear_(self.proj, init_scale=resid_init)

    def forward(self, x, skip):
        mixed = self.proj(torch.cat([self.x_norm(x), self.skip_norm(skip)], dim=-1))
        return x + torch.sigmoid(self.gate).view(1, 1, -1) * mixed


class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, args, act_dim):
        super().__init__()
        self.in_dim = in_dim
        self.act_dim = act_dim
        self.use_bf16 = args.transformer_bf16
        self.obs_embed_w = nn.Parameter(torch.empty(in_dim, H))
        self.obs_embed_b = nn.Parameter(torch.empty(in_dim, H))
        nn.init.normal_(self.obs_embed_w, std=0.02)
        nn.init.normal_(self.obs_embed_b, std=0.02)
        self.latent_tokens = nn.Parameter(torch.randn(args.transformer_latents, H) * 0.02)
        self.actor_token = nn.Parameter(torch.randn(1, H) * 0.02)
        self.critic_token = nn.Parameter(torch.randn(1, H) * 0.02)
        self.action_tokens = nn.Parameter(torch.randn(act_dim, H) * 0.02)
        self.encoder = nn.ModuleList([
            TokenTransformerBlock(H, args.transformer_heads, args.transformer_ffn_mult, args.transformer_resid_init)
            for _ in range(K)
        ])
        self.skip_mixers = nn.ModuleList([
            UNetSkipMixer(H, args.transformer_skip_init, args.transformer_resid_init)
            for _ in range(K)
        ])
        ...
        self.actor_out_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.critic_out_norm = nn.RMSNorm(H, elementwise_affine=False)

    def forward(self, x):
        ctx = torch.autocast(device_type="cuda", dtype=torch.bfloat16) if self.use_bf16 and x.is_cuda else nullcontext()
        with ctx:
            B = x.shape[0]
            obs_tokens = x.unsqueeze(-1) * self.obs_embed_w.unsqueeze(0) + self.obs_embed_b.unsqueeze(0)
            latents = self.latent_tokens.unsqueeze(0).expand(B, -1, -1)
            actor = self.actor_token.unsqueeze(0).expand(B, -1, -1)
            critic = self.critic_token.unsqueeze(0).expand(B, -1, -1)
            actions = self.action_tokens.unsqueeze(0).expand(B, -1, -1)
            tokens = torch.cat([latents, actor, critic, actions, obs_tokens], dim=1)
            skips = []
            for block in self.encoder:
                skips.append(tokens)
                tokens = block(tokens)
            for mixer, block, skip in zip(self.skip_mixers, self.decoder, reversed(skips)):
                tokens = mixer(tokens, skip)
                tokens = block(tokens)
            actor_idx = self.latent_tokens.shape[0]
            critic_idx = actor_idx + 1
            action_start = critic_idx + 1
            action_end = action_start + self.act_dim
            actor_action_feats = self.actor_out_norm(tokens[:, action_start:action_end])
            critic_feat = self.critic_out_norm(tokens[:, critic_idx])
        return actor_action_feats.float(), critic_feat.float()
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if self.share_backbone:
            ...
            self.trunk = ThinkTrunk(obs_dim, H, args.k_blocks, args, act_dim)
        else:
            self.critic_trunk = ThinkTrunk(obs_dim, H, args.k_blocks, args, act_dim)
            self.actor_trunk = ThinkTrunk(obs_dim, H, args.k_blocks, args, act_dim)
        ...
        # v24.5: SAC-Beta shifted-log-concentration default plus SAC-style Gaussian control path.
        self.actor_dist = args.actor_dist
        if self.actor_dist == "gaussian":
            # SAC-style Gaussian: mean head + state-dependent log_std head.
            self.actor_head = layer_init(nn.Linear(H, 1), std=0.01)
            if not (args.log_std_min < args.log_std_init < args.log_std_max):
                raise ValueError(
                    f"log_std_init must lie inside ({args.log_std_min}, {args.log_std_max}); "
                    f"got {args.log_std_init}"
                )
            raw_logstd_init = float(
                np.arctanh(rescale(args.log_std_init, (args.log_std_min, args.log_std_max), (-1.0, 1.0)))
            )
            self.actor_logstd_head = layer_init(nn.Linear(H, 1), std=0.01, bias_const=raw_logstd_init)
            self.log_std_min, self.log_std_max = args.log_std_min, args.log_std_max
        elif self.actor_dist == "beta":
            # SAC-Beta: network returns shifted log-concentrations, then exp(.) + 1.
            self.actor_alpha_logshift_head = layer_init(nn.Linear(H, 1), std=0.01)
            self.actor_beta_logshift_head = layer_init(nn.Linear(H, 1), std=0.01)
    ...
    def _actor_dist(self, actor_feat):
        ...
        if self.actor_dist == "gaussian":
            ...
            raw_logstd = self.actor_logstd_head(actor_feat).squeeze(-1)
            # SAC-style state-dependent log_std with a smooth tanh-rescale bound.
            log_std = rescale(
                raw_logstd.tanh(),
                (-1.0, 1.0),
        ...
        # beta: SAC-Beta shifted log-concentrations. Hard clamp matches ../sac-beta.
        alpha = self.actor_alpha_logshift_head(actor_feat).squeeze(-1).clamp(
            min=self.beta_log_shift_min, max=self.beta_log_shift_max
        ).exp() + 1.0
        beta = self.actor_beta_logshift_head(actor_feat).squeeze(-1).clamp(
            min=self.beta_log_shift_min, max=self.beta_log_shift_max
        ).exp() + 1.0
    ...
    def get_beta_stats(self, x):
        ...
        raw_alpha = self.actor_alpha_logshift_head(actor_feat).squeeze(-1)
        raw_beta = self.actor_beta_logshift_head(actor_feat).squeeze(-1)
    ...
    def _trunks(self, x):
        ...
        if self.share_backbone:
            return self.trunk(x)
        actor_feat, _ = self.actor_trunk(x)
        _, critic_feat = self.critic_trunk(x)
        return actor_feat, critic_feat
    ...
    def actor_parameters(self):
        ...
        # decoupled clip; no separate std budget — gaussian's log_std head and
        # both beta shifted-log-concentration heads sit in the same group).
        trunk = self.trunk if self.share_backbone else self.actor_trunk
        if self.actor_dist == "gaussian":
            heads = list(self.actor_head.parameters()) + list(self.actor_logstd_head.parameters())
        else:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.actor_dist == "beta":
            with torch.no_grad():
                ...
                writer.add_scalar("debug/beta_log_shift_alpha_min", log_shift_alpha.min().item(), global_step)
                writer.add_scalar("debug/beta_log_shift_beta_min", log_shift_beta.min().item(), global_step)
                ...
                writer.add_scalar("debug/beta_log_shift_alpha_max", log_shift_alpha.max().item(), global_step)
                writer.add_scalar("debug/beta_log_shift_beta_max", log_shift_beta.max().item(), global_step)
                ...
                writer.add_scalar(
                    "debug/beta_log_shift_alpha_hi_frac",
                    alpha_hi.float().mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "debug/beta_log_shift_beta_hi_frac",
                    beta_hi.float().mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "debug/beta_log_shift_sample_hi_frac",
                    (alpha_hi | beta_hi).any(dim=1).float().mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "debug/beta_log_shift_alpha_lo_frac",
                    alpha_lo.float().mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "debug/beta_log_shift_beta_lo_frac",
                    beta_lo.float().mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "debug/beta_log_shift_sample_lo_frac",
                    (alpha_lo | beta_lo).any(dim=1).float().mean().item(),
                    global_step,
                )
                writer.add_scalar(
                    "debug/beta_z_edge_frac",
                    (
                        (b_latent_zs <= SAMPLE_EPS * 10.0)
                        | (b_latent_zs >= 1.0 - SAMPLE_EPS * 10.0)
                    ).float().mean().item(),
                    global_step,
                )
                writer.add_scalar("debug/action_std", b_actions.std().item(), global_step)
