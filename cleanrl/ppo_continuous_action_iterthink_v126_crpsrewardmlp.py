# PPO + IterThink v126 crpsrewardmlp (v121 + post-action ReLU^2 reward readout + CRPS).
#
# Key change from v121:
#   - Keep v120/v121's stable outcome-token layout: reward and continuation
#     predictor query tokens are visible to the agent, and only obs tokens are
#     recurrent world-model state.
#   - Replace the linear reward-token + action projection with a post-action
#     ReLU^2 readout MLP over [reward_token, action, action^2], followed by a
#     projection, RMSNorm, and HL-Gauss bucket logits.
#   - Train reward buckets with an unnormalized symlog-bucket CDF-distance loss
#     instead of pointwise distribution CE. This preserves distributional
#     prediction while making scalar-relevant mass displacement pay in proportion
#     to distance without implicitly downweighting reward learning by bin width.
#
# Hypothesis:
#   v105's CDF reward head had much better early dream reward scale but diluted
#   threshold BCE and weak late scalar fit. v126 ports the useful geometry into
#   the existing softmax HL-Gauss reward head, while adding enough post-action
#   capacity for HalfCheetah's action-sensitive reward terms.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # are role-specific query/readout tokens: token 0 -> reward bucket CDF
        # loss, token 1 -> continuation BCE. Obs tokens still carry dynamics
        # state and are the only recurrent stream.
        self.continuation_token_norm = nn.RMSNorm(self.dim)
        reward_readout_hidden = 2 * self.dim
        self.reward_readout_mlp = nn.Sequential(
            xavier_linear(nn.Linear(self.dim + 2 * self.act_dim, reward_readout_hidden)),
            ReLUSquared(),
            xavier_linear(nn.Linear(reward_readout_hidden, reward_readout_hidden)),
            ReLUSquared(),
        )
        self.reward_readout_proj = xavier_linear(nn.Linear(reward_readout_hidden, self.dim))
...
class Agent(nn.Module):
    ...
    def agent_input_from_obs(self, x):
        ...
        return self.agent_input_from_history(obs_latent.unsqueeze(1), neutral_action.unsqueeze(1))
...
if __name__ == "__main__":
    ...
    def reward_crps_loss(logits, target_probs):
        pred_cdf = torch.softmax(logits, dim=-1).cumsum(dim=-1)
        target_cdf = target_probs.cumsum(dim=-1)
        # Bucket-unit CRPS-style loss. Do not multiply by bin_width here: for
        # this fixed 101-bin symlog support, the integral scale would mostly
        # downweight reward learning relative to the v121 CE objective.
        return (pred_cdf - target_cdf).square().sum(dim=-1)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/reward_crps", float(np.mean(wm_reward_losses)), global_step)
