# PPO + IterThink v183 lewmprojbn-wmwd1e3-predembedinput-k6 (on v182's agent-input + clips).
#
    ...
    def normalize_world_model_matrices(self):
        ...
        for name, param in self.named_parameters():
            ...
            if name.endswith("reward_token_unproj.weight") or name.endswith("continuation_token_unproj.weight"):
                continue
            # v183: the le-wm JEPA projector deliberately lives OFF the unit sphere -- its
            # MSE/SIGReg target is the BatchNorm'd (non-sphere) emb. Renormalizing its
            # Linear weights every step would clamp emb geometry and cancel the new wd, so
            # exclude it (mirrors le-wm's plain MLP head).
...
if __name__ == "__main__":
    ...
    def wm_supervised_loss_forward(
        ...
        one_step_latent_cos = F.cosine_similarity(pred_emb, target_emb, dim=-1).mean(dim=-1)
