# PPO + IterThink v203 affine-RMS epistemic transition-query ensemble.
#
# Preserve v198's action-free temporal state, attached LeJEPA encoder targets,
# encoder-only SIGReg, raw observations, detached actor/critic queries, Beta PPO,
# and exact real-anchor KL accounting. Replace the single outbound transition
# query with three independently bootstrapped query/next-state/outcome decoders.
# Each imagined trajectory keeps one sampled member for temporally coherent model
# error, while every member is evaluated at each step. Actor and critic learning
# stops at the first transition whose ensemble disagreement exceeds a running
# 95th-percentile threshold measured on real teacher-forced transitions. This
# imports Dreamer4's stochastic/recovery role without diffusion or policy-to-WM
# gradients and prevents unsupported dreams from spending the real PPO KL budget.
#
...
class Args:
    ...
    lewm_ensemble_size: int = 3      # independent transition-query/outcome members
    lewm_bootstrap_keep_prob: float = 0.65  # deterministic per-transition bootstrap masks
    ...
    imagine_support_ema: float = 0.9        # smooth quantile between iterations
...
class LeWMBackbone(nn.Module):
    ...
    def predict_ensemble_from_history(self, latent_history, action_history, return_beliefs=False):
        ...
        preds = []
        for readout, projection in zip(self.transition_readouts, self.pred_next_projs):
    ...
    def predict_mtp_from_history(self, latent_history, action_history, return_beliefs=False):
        ...
        if return_beliefs:
            preds, state_memory = result
            return preds.mean(dim=0), state_memory
    ...
    def imagine_step_all_from_history(self, latent_history, action_history):
        ...
        action_features = self.action_cond(last_action)
        next_summaries = []
        for readout, projection in zip(self.transition_readouts, self.pred_next_projs):
...
if __name__ == "__main__":
    ...
    assert args.lewm_ensemble_size == 3, "v203 uses a fixed three-member epistemic ensemble"
    assert 0.0 < args.lewm_bootstrap_keep_prob <= 1.0
    assert 0.5 < args.imagine_support_quantile < 1.0
    assert 0.0 <= args.imagine_support_ema < 1.0
    ...
    imagine_prompt_bank = []
    # Componentwise, final-model calibration state: [latent, reward, continue].
    real_support_thresholds = None
    real_disagreement_scales = None
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # Deterministic bootstrap assignment. The same current-rollout
                    # transition gets the same mask in both WM epochs, while each
                    # member sees a different 65% subset. Data changes on the next
                    # iteration through global_step, so no permanent env/time slot is
                    ...
                    bootstrap_hash = (
                        transition_ids.unsqueeze(0) * 1103515245
                        + member_ids * 12345
                        + 1013904223
                    ) & 0x7FFFFFFF
                    calibration_hash = (
                        transition_ids * 214013 + 2531011
                    ) & 0x7FFFFFFF
                    calibration_mask = calibration_hash.remainder(10) == 0
                    bootstrap_masks = (
                        bootstrap_hash.remainder(100000)
                        < int(args.lewm_bootstrap_keep_prob * 100000)
                    ...
                    wm_latent_loss = torch.stack(member_latent_losses).mean()
        ...
        # Final-model calibration is deliberately separate from the optimizer
        # minibatches: support must describe the model dreams will actually use,
        # not a mixture of earlier parameter states. A deterministic 10% decoder-
        # held-out slice is evaluated from clean warm contexts. Latent, reward and
        # continuation are calibrated independently so one raw scale cannot hide
        # another component's epistemic failure.
        ...
        with torch.no_grad():
            ...
            cal_hash = (cal_ids * 214013 + 2531011) & 0x7FFFFFFF
            ...
            if cal_pairs.shape[0] >= 32:
                ...
                new_thresholds, new_scales, new_corrs = [], [], []
                for disagreement, residual in zip(
                    cal_component_disagreements, cal_component_residuals
                ):
                    denom_floor = disagreement.median().clamp_min(1e-8) * 1e-3
                    ratio = residual / disagreement.clamp_min(denom_floor)
                    new_scales.append(torch.quantile(ratio, args.imagine_support_quantile).item())
                    new_thresholds.append(torch.quantile(residual, args.imagine_support_quantile).item())
                ...
                if real_support_thresholds is None:
                    real_support_thresholds = new_thresholds
                    real_disagreement_scales = new_scales
                else:
                    real_support_thresholds = [
                        args.imagine_support_ema * old + (1.0 - args.imagine_support_ema) * new
                        for old, new in zip(real_support_thresholds, new_thresholds)
                    ]
                    real_disagreement_scales = [
                        args.imagine_support_ema * old + (1.0 - args.imagine_support_ema) * new
                        for old, new in zip(real_disagreement_scales, new_scales)
                    ]
        ...
        if (
            ...
            and real_disagreement_scales is not None
        ):
            ...
            for _ in range(args.imagine_batches):
                ...
                with torch.no_grad():
                    ...
                    for t in range(H_im):
                        ...
                        component_risk_ratios = [
                            scale * component / max(threshold, 1e-8)
                            for scale, threshold, component in zip(
                                real_disagreement_scales,
                                real_support_thresholds,
                                component_disagreements,
                            )
                        ]
                        disagreement = torch.stack(component_risk_ratios).amax(dim=0)
                        step_supported = torch.stack(
                            [risk <= 1.0 for risk in component_risk_ratios]
                        ).all(dim=0)
                    ...
                    if supported_flat_inds.numel() > 0:
                ...
                if supported_count == 0:
                    # Do not call optimizer.step with empty support: AdamW momentum
                    # would otherwise move both heads despite the gate.
                    ...
                    continue
                attempted_dream_actor_update = dream_actor_active and bool(supported_count > 0)
            ...
            for component_idx, component_name in enumerate(("latent", "reward", "continue")):
                ...
                imagine_metrics[f"imagine/{component_name}_disagreement_scale"] = float(
                    real_disagreement_scales[component_idx]
                )
