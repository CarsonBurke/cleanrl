# 32x LR + 1 minibatch + V-MPO v30 target-critic hold, no HL-Gauss.
# Frozen target critic evaluates V(s') on each transition's actual next
# observation (explicit-next-value GAE). Online critic scores V(s) and trains
# with ordinary clipped MSE. Target copies every 100 rollouts, matching v30's
# target_update_period. Scalar critic, no percentile scaling, no duals.
import copy
import os
import random
import time
...
from cleanrl.shared.ppo_loop import (
    device_minibatches, explained_variance, gather_metrics, get_gae_fn,
)
...
class Args:
    ...
    """copy the online critic into the frozen target, matching V-MPO v30"""
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; frozen target critic for V(s'); explicit-next GAE")
        envs = make_training_env(args, run_name)
        resources.callback(envs.close)
        agent = Agent(envs).to(device)
        target_critic = copy.deepcopy(agent.critic).requires_grad_(False)
        optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5, fused=True)

        def target_value(observations):
            return target_critic(observations).flatten()
        ...
        if args.compile:
            ...
            target_value = graph_compile(target_value)
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            for step in range(args.num_steps):
                ...
                with timer.span("normalize_transfer", use_cuda=False):
                    ...
                    transfer.push(step, reward, terms, truncs, transition_obs,
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                next_values = target_value(batch.transition_observations.flatten(0, 1)).view(
            ...
            target_promoted = target_age_batches >= args.target_update_period
            if target_promoted:
                target_critic.load_state_dict(agent.critic.state_dict())
                target_age_batches = 0
                target_promotions += 1
            ...
            writer.add_scalar("vmpo/target_age_batches", target_age_batches, global_step)
            writer.add_scalar("vmpo/target_promoted", float(target_promoted), global_step)
