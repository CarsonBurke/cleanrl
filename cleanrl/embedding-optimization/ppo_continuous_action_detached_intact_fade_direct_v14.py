# Detached INTACT Fade Direct v14
#
# V11's detached physical H12 NLL scaffolds the shared law early, then linearly
# fades away so persistent self-imitation cannot oppose frozen-world reward
# optimization. World learning and deterministic runtime control are unchanged.
import math
import os
import random
import time
...
class Args:
    ...
    intact_hold_steps: int = 1_250_000
    intact_fade_steps: int = 250_000
...
def scheduled_detached_intact_coefficient(
    global_step,
    base_coefficient,
    hold_steps,
    fade_steps,
    *,
    ...
    """Hold the physical NLL coefficient, then linearly fade it to zero."""

    if base_coefficient < 0.0:
        raise ValueError("detached INTACT coefficient must be nonnegative")
    if hold_steps < 0:
        raise ValueError("INTACT hold steps must be nonnegative")
    if fade_steps <= 0:
        raise ValueError("INTACT fade steps must be positive")
    step = torch.as_tensor(global_step, device=device, dtype=dtype)
    progress = ((step - hold_steps) / fade_steps).clamp(0.0, 1.0)
    return step.new_tensor(base_coefficient) * (1.0 - progress)
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "detached INTACT World Direct v14 uses 64-D latents"
    ...
    assert args.intact_hold_steps >= 0
    assert args.intact_fade_steps > 0
    ...
    if args.compile:
        ...
        print(
            "[detached_intact_fade_direct_v14] "
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        effective_detached_intact_coef = scheduled_detached_intact_coefficient(
            global_step,
            args.detached_intact_coef,
            args.intact_hold_steps,
            args.intact_fade_steps,
            device=device,
        )
        update_metrics = []
        ...
        writer.add_scalar(
            "intact/effective_detached_coefficient",
            effective_detached_intact_coef,
            global_step,
        )
        writer.add_scalar(
