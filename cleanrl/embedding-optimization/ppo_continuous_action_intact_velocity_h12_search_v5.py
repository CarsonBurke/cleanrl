# INTACT Velocity H12 Search v5
#
# Policy-free online HalfCheetah control with a LeJEPA world model and INTACT
# representation shaping. A residual head predicts exact interval velocity and
# an H12 controller uses either projected Adam or CEM on the identical reward
# rollout, without a terminal value. A separate switch isolates continued random
# action support. The hypothesis is that global search or data coverage, rather
# than reward definition, explains the remaining planning gap.
import math
import os
import random
import time
...
def action_sequence_population_score(
    ...
    """Evaluate [batch,population,horizon,action] through the shared scorer."""

    if action_sequences.ndim != 4:
        raise ValueError(
            "action_sequences must have shape [batch,population,horizon,action]"
        )
    batch, population, horizon, action_dim = action_sequences.shape
    if z.shape[0] != batch:
        raise ValueError("latent batch does not match candidate batch")
    expanded_z = z[:, None].expand(-1, population, -1).reshape(
        batch * population, -1
    )
    flat_sequences = action_sequences.reshape(
        batch * population, horizon, action_dim
    )
    scores, velocities = action_sequence_score(
        agent, expanded_z, flat_sequences, action_cost_coef
    )
    scores = scores.reshape(batch, population)
    velocities = velocities.reshape(batch, population, horizon)
...
def sample_cem_action_sequences(
    ...
    """Sample bounded local and global CEM candidates around [B,H,A]."""
    ...
    if incumbent is not None and incumbent.shape != center.shape:
        raise ValueError("incumbent must have the same shape as center")
    local[:, 0] = incumbent if incumbent is not None else center
    if incumbent is not None and local_count > 1:
        local[:, 1] = center
    if global_count:
        ...
        local = torch.cat([local, global_samples], dim=1)
    return local.clamp(-1.0, 1.0)
...
def cem_sequence_update(
    ...
    """Refit all sequence coordinates from the per-environment elite set."""
...
def optimize_action_sequence(agent, z, warm_start, args):
    ...
    diagnostics = {
        ...
        "score_spread": torch.zeros_like(best_score),
        "elite_action_std": torch.zeros_like(best_score),
        "planner_advantage": best_score - initial_score,
    }
    return best_sequence.detach(), diagnostics


@torch.no_grad()
def optimize_action_sequence_cem(
    agent,
    z,
    warm_start,
    args,
    generator=None,
    score_function=action_sequence_population_score,
):
    """Batched CEM over the exact same H12 reward rollout used by Adam."""

    center = warm_start.detach().clone()
    ...
    initial_score = incumbent_score.clone()
    ...
    for _ in range(args.cem_iterations):
        ...
        scores = score_function(agent, z, candidates, args.action_cost_coef)
        ...
        incumbent = torch.where(
            improved[:, None, None], iteration_best, incumbent
        )
        incumbent_score = torch.maximum(incumbent_score, iteration_best_score)
        final_scores = scores
        final_elites = elites

    _, predicted_interval_velocities = score_function(
        agent,
        z,
        incumbent[:, None],
    ...
    diagnostics = {
        "optimizer_improvement": incumbent_score - initial_score,
        "initial_gradient_norm": torch.zeros_like(incumbent_score),
        "action_change": (incumbent[:, 0] - warm_start[:, 0]).norm(dim=-1),
        "sequence_change": (incumbent - warm_start).flatten(1).norm(dim=-1),
        "predicted_interval_velocity": predicted_interval_velocities[:, 0, 0],
        "selected_score": incumbent_score,
        "score_spread": final_scores.std(dim=1, unbiased=False),
        "elite_action_std": final_elites.std(dim=1, unbiased=False).mean(
            dim=(-1, -2)
        ),
...
def plan_action_sequence(
    ...
    args,
    generator=None,
    cem_score_function=action_sequence_population_score,
):
    """Dispatch the configured planner without changing its reward model."""

    if args.planner == "adam":
        return optimize_action_sequence(agent, z, warm_start, args)
    if args.planner == "cem":
        return optimize_action_sequence_cem(
            agent,
            z,
            warm_start,
            args,
...
@torch.no_grad()
def apply_random_action_replacement(
    action,
    planned_sequence,
    probability,
    generator=None,
):
    """Replace selected executed actions and clear only their warm plans."""

    if not 0.0 <= probability <= 1.0:
        raise ValueError("random action probability must be in [0,1]")
    replaced = torch.zeros(action.shape[0], dtype=torch.bool, device=action.device)
    if probability == 0.0:
        return action, planned_sequence, replaced
    replaced = (
        torch.rand(action.shape[0], device=action.device, generator=generator)
        < probability
    )
    random_action = torch.empty_like(action).uniform_(
        -1.0, 1.0, generator=generator
    )
    action = torch.where(replaced[:, None], random_action, action)
    planned_sequence = torch.where(
        replaced[:, None, None],
        torch.zeros_like(planned_sequence),
        planned_sequence,
    )
    return action, planned_sequence, replaced
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "INTACT velocity H12 Search v5 uses 64-D latents"
    assert args.model_horizon >= 1
    assert args.planner_horizon >= 1
    assert args.planner_horizon <= args.model_horizon
    assert args.planner in ("adam", "cem")
    ...
    assert 0.0 <= args.random_action_probability <= 1.0
    assert args.cem_iterations >= 1
    assert 1 <= args.cem_elites < args.cem_population
    assert 0.0 <= args.cem_global_fraction < 1.0
    assert args.cem_population * args.cem_global_fraction >= 1
    assert args.cem_initial_std > 0.0
    assert args.cem_min_std > 0.0
    assert 0.0 < args.cem_update_rate <= 1.0
    ...
    if args.compile:
        ...
        cem_score_function = torch.compile(
            cem_score_function, mode=args.compile_mode, dynamic=False
        )
        print(
            "[intact_velocity_h12_search_v5] "
    ...
    cem_generator = torch.Generator(device=device).manual_seed(args.seed + 1_000_003)
    exploration_generator = torch.Generator(device=device).manual_seed(
        args.seed + 2_000_003
    )
    global_step = 0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        rollout_elite_action_std = torch.zeros((), device=device)
        rollout_planner_advantage = torch.zeros((), device=device)
        rollout_random_action_fraction = torch.zeros((), device=device)
        ...
        for _ in range(args.num_steps):
            ...
            if global_step < args.warmup_steps:
                ...
                with torch.no_grad():
                    diagnostics = {
                        ...
                        "selected_score": zero,
                        "score_spread": zero,
                        "elite_action_std": zero,
                        "planner_advantage": zero,
                        "random_action_fraction": zero,
                    }
                planned_action_sequence = warm_start_sequence
            else:
                planned_action_sequence, diagnostics = plan_action_sequence(
                    agent,
                    z,
                    warm_start_sequence,
                    args,
                    generator=cem_generator,
                    cem_score_function=cem_score_function,
                )
                action = planned_action_sequence[:, 0]
                action, planned_action_sequence, replaced = (
                    apply_random_action_replacement(
                        action,
                        planned_action_sequence,
                        args.random_action_probability,
                        exploration_generator,
                    )
                )
                with torch.no_grad():
                    ...
                    diagnostics["action_change"] = (
                        action - warm_start_sequence[:, 0]
                    ).norm(dim=-1)
                    diagnostics["sequence_change"] = (
                        planned_action_sequence - warm_start_sequence
                    ).flatten(1).norm(dim=-1)
                    diagnostics["random_action_fraction"] = replaced.float()
            ...
            rollout_elite_action_std += diagnostics["elite_action_std"].mean()
            rollout_planner_advantage += diagnostics["planner_advantage"].mean()
            rollout_random_action_fraction += diagnostics[
        ...
        writer.add_scalar(
            "planner/score_spread",
            rollout_score_spread / args.num_steps,
            global_step,
        )
        writer.add_scalar(
            "planner/elite_action_std",
            rollout_elite_action_std / args.num_steps,
            global_step,
        )
        writer.add_scalar(
            "planner/advantage",
            rollout_planner_advantage / args.num_steps,
            global_step,
        )
        writer.add_scalar(
            "diagnostics/random_action_fraction",
            rollout_random_action_fraction / args.num_steps,
            global_step,
        )
