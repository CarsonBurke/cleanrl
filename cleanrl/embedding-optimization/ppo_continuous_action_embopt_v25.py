# Embedding Optimization v25 (embopt_v25) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
...
#       CONSTRUCTION (far goals neutral by the projection; g~z neutral by the
#       gap-norm FLOOR — the raw projection is degree-0 in the gap, so the
#       floor, not the projection, creates the interior optimum), the
...
class Args:
    ...
    rate_window: int = 16
    """forward window (steps) for the qrate action-credit target (what() now
    regresses reward efficiency, not this)"""
    ...
    learning is the family's point). Projection units in v25 — do not carry
    intuitions from v16-v24"""
    prop_reach_coef: float = 1.0
    """weight of the reachability term q(z, act(z,g), g) in the proposer score:
    aspiration must be pursuable from the current state (restores z-dependence;
    belief-only scoring collapsed to one constant goal in v22). Read against
    goal_grad_belief/goal_grad_reach"""
    eff_window: int = 16
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # over the last eff_window steps en route to z (state-local backward
        # window, not the whole journey)
...
if __name__ == "__main__":
    ...
    def wm_loss_fn(o, a, o2, r, rate, rv, eff, ev, pair_valid):
        z = agent.encode(o)
        z2 = agent.encode(o2)
    ...
    def proposer_loss_fn(z):
        # score = the BELIEF map what(g): predicted reward efficiency of the goal
        # state (mean reward over the last eff_window steps en route — NOTE: only
        # the FINAL eff_window steps of travel are priced, so this is close to
        # v22's local rate with a backward shift, not v24's whole-journey
        # average; the tempering claim rests on the latter and is under test).
        # Grounded by the pursuit loop: what() regresses measured efficiencies
        # at real states.
        # + reachability (from v23, safe under the projection target): the goal
        # must also be pursuable from HERE — restores z-dependence (belief-only
        # scoring collapsed to one constant goal in v22). Far goals no longer buy
        # progress magnitude, so this term cannot fuel the v16 treadmill
        g = agent.propose(z)
        w = agent.what(g)
    ...
    if args.compile:
        ...
        print(f"[embopt_v25] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for _ in range(args.wm_updates):
            ...
            mark_step()
            loss, pl, sl, rl, wl, qrl = wm_loss_fn(o, a, o2, r, rate, rv, eff, ev, pv)
        ...
        with torch.no_grad():
            ...
            # gap-norm floor at 10% of the shell radius: goals closer than that are
            # effectively reached — target goes NEUTRAL (not explosive) as g -> z,
            # instead of dividing by ~0 when the reachability incentive parks
            # goals near the current state. No effect on the normal regime
            # (typical gap ~ sqrt(2*dz) >> 0.1*sqrt(dz))
        ...
        # measured balance of the proposer's two terms at the goal — unscaled,
        # same idiom: prop_reach_coef must be read against these, not assumed
        gd = agent.propose(zb).detach()
        g1 = gd.clone().requires_grad_(True)
        grad_w = torch.autograd.grad(agent.what(g1).sum(), g1)[0]
        g2 = gd.clone().requires_grad_(True)
        grad_p = torch.autograd.grad(agent.qprog(zb, agent.act(zb, g2), g2).sum(), g2)[0]
        # TANGENTIAL components only: propose() shell-normalizes, so radial score
        # gradients are annihilated by the normalization Jacobian; full norms
        # overstate whichever term has the larger radial part (typically belief)
        gh = gd / gd.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        grad_w = grad_w - (grad_w * gh).sum(-1, keepdim=True) * gh
        grad_p = grad_p - (grad_p * gh).sum(-1, keepdim=True) * gh
        goal_grad_belief = grad_w.norm(dim=-1).mean().item()
        goal_grad_reach = grad_p.norm(dim=-1).mean().item()
        ...
        writer.add_scalar("losses/eff_mse", wm_m[3], global_step)
        ...
        writer.add_scalar("diagnostics/goal_grad_reach", goal_grad_reach, global_step)
