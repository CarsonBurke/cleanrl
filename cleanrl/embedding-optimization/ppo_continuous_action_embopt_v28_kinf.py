# Embedding Optimization v28_kinf (embopt_v28_kinf) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v28_fixg -> v28_kinf: PROBE ARM C1-NEUTRALIZED — PURE KINETIC TARGET ON THE
# FROZEN-GOAL CHASSIS. v28_kin (job 745) left the proposer live: g = propose(z)
# is a second encoding of z the head can exploit, and with a goal-independent
# target nothing constrains how the policy uses a drifting g input (reviewer).
# This arm removes both leaks via the fixg plumbing (g frozen shell point,
# proposer_updates=0, [z,g,g-z] a reparameterization of z) and swaps ONLY the
# qprog target to the goal-independent per-dim squared latent step
# ||z'-z||^2/dz. True "kinetic bonus, no goal channel". Reads against
# v28_fixg (anchored direction, same chassis) and v22 (+584) / vc0 (-149).
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(f"[embopt_v28_kinf] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # PURE KINETIC target, goal-independent: per-dim squared latent step —
            # the quantity the v25/v26 projection overpaid, now with no live goal
            # channel to confound it
            tgt_pool = (Zs[:, wend_idx] - Zs[:, :T]).square().mean(-1)  # (K,T,E)
        ...
        with torch.no_grad():
            ...
            # alignment measurement: correlation of the kinetic target (per-step
            # ||d||^2, rotation-invariant) with env reward. wm_delta_var is only a
            # screen (it also moves for world-model reasons); this is the clean
            # per-step version — decay toward 0 over training = representational
            # decoupling (encoder stops spending variance on physical velocity)
            mgf = m_pool[0].bool()
            kp_c = tgt_pool[0][mgf] - tgt_pool[0][mgf].mean()
            rw_c = buf_rew[fresh_slot][mgf] - buf_rew[fresh_slot][mgf].mean()
            kin_reward_corr = (
                (kp_c * rw_c).sum() / (kp_c.norm() * rw_c.norm()).clamp_min(1e-8)
            ).item()
        ...
        writer.add_scalar("diagnostics/kin_reward_corr", kin_reward_corr, global_step)
