# Embedding Optimization: Frozen-Chart Free Goals v3
#
...
# The representation is trained only during an exact 262,144-step factual
# warmup, then frozen permanently. This isolates deterministic goal learning
# from nonlinear chart drift while retaining LeJEPA's two-term objective.
...
def freeze_world_model(world):
    """Permanently freeze a settled chart without blocking action derivatives."""
    for parameter in world.parameters():
        parameter.requires_grad_(False)


def controlled_learning_started(global_step, warmup_steps):
    return global_step >= warmup_steps


def should_update_representation(controlled_iteration):
    """LeJEPA updates are permitted only for factual warmup batches."""
    return not controlled_iteration


def warmup_boundary_reached(
    controlled_iteration, global_step, warmup_steps
):
    """The just-collected factual batch ends exactly at the control boundary."""
    return not controlled_iteration and global_step >= warmup_steps
...
def factual_st_actor_loss(
    ...
    with frozen_parameters(agent.world, agent.reward, agent.value):
        ...
        return_loss = -factual_return.mean()
        delivery_loss = args.goal_delivery_coef * delivery_mse.mean()
        action_cost_loss = args.action_cost_coef * action_cost.mean()
        uncertainty_loss = args.uncertainty_coef * uncertainty.mean()
    ...
    if diagnose_routes:
        goal_parameters = list(agent.goal.parameters())
        policy_parameters = list(agent.policy.parameters())
        return_gradients = torch.autograd.grad(
            return_loss,
            goal_parameters + policy_parameters,
            retain_graph=True,
            allow_unused=True,
        )
        goal_return_gradients = return_gradients[: len(goal_parameters)]
        policy_return_gradients = return_gradients[len(goal_parameters) :]
        policy_delivery_gradients = torch.autograd.grad(
            delivery_loss,
            policy_parameters,
            retain_graph=True,
            allow_unused=True,
        )
        policy_action_cost_gradients = torch.autograd.grad(
            action_cost_loss,
            policy_parameters,
            retain_graph=True,
            allow_unused=True,
        )
        policy_uncertainty_gradients = torch.autograd.grad(
            uncertainty_loss,
            policy_parameters,
            retain_graph=True,
            allow_unused=True,
        )
        goal_return_norm = gradient_route_norm(goal_return_gradients)
        policy_return_norm = gradient_route_norm(policy_return_gradients)
        policy_delivery_norm = gradient_route_norm(policy_delivery_gradients)
        policy_action_cost_norm = gradient_route_norm(
            policy_action_cost_gradients
        )
        policy_uncertainty_norm = gradient_route_norm(
            policy_uncertainty_gradients
        )
        metrics.update(
            {
                "goal_return_gradient_norm": goal_return_norm.detach(),
                "policy_return_gradient_norm": policy_return_norm.detach(),
                "policy_delivery_gradient_norm": policy_delivery_norm.detach(),
                "policy_action_cost_gradient_norm": (
                    policy_action_cost_norm.detach()
                ),
                "policy_uncertainty_gradient_norm": (
                    policy_uncertainty_norm.detach()
                ),
                "policy_return_delivery_gradient_cosine": gradient_route_cosine(
                    policy_return_gradients, policy_delivery_gradients
                ).detach(),
                "policy_return_action_cost_gradient_cosine": (
                    gradient_route_cosine(
                        policy_return_gradients,
                        policy_action_cost_gradients,
                    ).detach()
                ),
                "policy_return_uncertainty_gradient_cosine": (
                    gradient_route_cosine(
                        policy_return_gradients,
                        policy_uncertainty_gradients,
                    ).detach()
...
def gradient_route_norm(gradients):
    squared_norm = sum(
        gradient.square().sum()
...
def gradient_route_cosine(left, right):
    dot = sum(
    ...
    if not isinstance(dot, torch.Tensor):
        return torch.tensor(0.0)
    denominator = gradient_route_norm(left) * gradient_route_norm(right)
    return dot / denominator.clamp_min(1e-12)
...
def main():
    ...
    if args.warmup_steps % args.batch_size:
        raise ValueError(
            "warmup_steps must be divisible by num_envs * num_steps"
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        controlled_iteration = controlled_learning_started(
        ...
        if (
            should_update_representation(controlled_iteration)
            and replay.size >= args.minibatch_size
        ):
            totals = {}
        ...
        if (
            not chart_frozen
            and warmup_boundary_reached(
                controlled_iteration, global_step, args.warmup_steps
            )
        ):
            representation_optimizer.zero_grad(set_to_none=True)
            freeze_world_model(agent.world)
            chart_frozen = True
            writer.add_scalar(
                "model/chart_freeze_transition", 1.0, global_step
            )
        writer.add_scalar(
            "model/chart_frozen", float(chart_frozen), global_step
        )
        writer.add_scalar(
            "model/representation_updates",
