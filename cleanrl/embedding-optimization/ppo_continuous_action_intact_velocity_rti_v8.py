# INTACT Velocity RTI v8
#
# Policy-free online HalfCheetah control with a LeJEPA world model and INTACT
# representation shaping. Deployment shifts its H12 plan and takes one guarded,
# sign-normalized gradient step on cumulative exact interval velocity minus
# control cost. The hypothesis is that receding-horizon real-time iteration
# preserves useful gait momentum without iterative search or action-optimizer state;
# INTACT only shapes encoder geometry and action recoverability.
import math
import os
import random
import time
...
class Args:
    ...
    rti_step_size: float = 0.16
    warm_start_mode: Literal["repeat", "zero_tail", "reset"] = "repeat"
...
def rti_action_sequence(
    ...
    with torch.no_grad():
        ...
        selected_velocities = torch.where(
            ...
            initial_velocities.detach(),
        )
    diagnostics = {
        "optimizer_improvement": selected_score - initial_score.detach(),
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "INTACT velocity RTI v8 uses 64-D latents"
