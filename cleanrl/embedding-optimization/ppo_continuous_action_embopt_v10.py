# Embedding Optimization v10 (embopt_v10) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v9 -> v10: recover the unroll's compute cost without touching the mechanism (v9 @2.4M:
# +1154 return, first locomotion in family history, but SPS 7800 -> 6000). Insight: one
# H=8 unrolled update is ~8 depths of action credit per backward (every a_i receives
# gradient from all downstream terms), so v9 at 16 updates carries ~8x v8's per-iteration
# signal — halving update counts still leaves ~4x. H stays 8 (it IS the breakthrough).
# (1) policy_updates 16 -> 8, proposer_updates 8 -> 4.
# (2) rollout proposal computed ONLY on refresh steps: the old single compiled step
#     evaluated torch.where(refresh, propose(z), cur_goal) — under CUDA graphs both
#     branches run, so 15/16 of the 16-step proposal unrolls were wasted. Split into
#     rollout_act (every step) + rollout_propose (every goal_refresh steps). Semantic
#     delta: an env that resets mid-window keeps its stale goal for <= goal_refresh-1
#     steps (HalfCheetah: one reset per 1000 steps; negligible).
# --- v9 header follows ---
...
class Args:
    ...
    proposer_updates: int = 4
    """proposer minibatch updates per iteration (each carries a full 16-step unroll)"""
    policy_updates: int = 8
    """policy minibatch updates per iteration (each carries 8 depths of action credit)"""
    ...
    policy_unroll: int = 8
    """imagined rollout horizon H for the policy loss (SVG(H) through frozen f)"""
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # goal-seeker: an action policy unrolled through frozen f; the proposed goal is
        # its imagined endpoint, hence on f's manifold and exactly goal_refresh
        # model-steps away by construction
        self.proposer = mlp(dz, dh, da, out_std=0.01)
        self.policy = mlp(3 * dz, dh, da, out_std=0.01)
        self.dz = dz
        self.goal_unroll = args.goal_refresh
    ...
    def seek(self, z):
        return torch.tanh(self.proposer(z))

    def propose_path(self, z):
        # imagined goal_refresh-step rollout of the seeker through f; returns the
        # endpoint (the goal) and the discounted imagined path reward
        path_r = torch.zeros(z.shape[:-1], device=z.device)
        for i in range(self.goal_unroll):
            a = self.seek(z)
            path_r = path_r + (self.gamma_goal**i) * self.rhat(z, a)
            z = self.forward_dyn(z, a)
        return z, path_r

    def propose(self, z):
        return self.propose_path(z)[0]
...
if __name__ == "__main__":
    ...
    def rollout_act(obs, goal):
        return agent.act(agent.encode(obs), goal)

    def rollout_propose(obs):
        return agent.propose(agent.encode(obs))
    ...
    def critic_loss_fn(z, g, target):
        ...
        # an unrelated goal): V's ONLY off-manifold signal — what "far" means
        neg_target = args.gamma_goal**args.num_steps
        neg = (agent.value(z, torch.roll(g, 1, 0)) - neg_target).square().mean()
        return main + args.neg_goal_coef * neg, main.detach(), v.mean().detach()

    def proposer_loss_fn(z, r_lo, r_hi):
        # goal = imagined endpoint of the seeker unrolled through frozen f: reachability
        # is structural (no V term, no band). Payoff = discounted imagined path reward
        # + endpoint reward-rate, support-clamped (fantasy capped at the best OBSERVED
        # regime; the cap ratchets up with performance). rate -> discounted-window-sum
        # scaling puts both terms in the same units.
        g, path_r = agent.propose_path(z)
        w = agent.what(g).clamp(r_lo, r_hi)
        w_scale = (1.0 - args.gamma_goal**args.rate_window) / (1.0 - args.gamma_goal)
        score = path_r + (args.gamma_goal**args.goal_refresh) * w_scale * (w - r_lo)
        # v is telemetry only: V's opinion of a goal that is structurally 16 steps away
        # — deviation from gamma^16 ~ 0.72 measures V's error at model-image queries
        v = agent.value(z, g)
        return -score.mean(), w.mean().detach(), v.mean().detach()

    def policy_loss_fn(z, g):
        # SVG(H): unroll the policy H steps through frozen f, r_hat credit along the
        # imagined path, frozen V terminal. Compounding model error makes one-step
        # fantasy self-punishing; gait fits inside the policy's own horizon.
        r_sum = torch.zeros(z.shape[:-1], device=z.device)
        r0 = None
        for i in range(args.policy_unroll):
            a = agent.act(z, g)
            r_i = agent.rhat(z, a)
            if i == 0:
                r0 = r_i.mean().detach()
            r_sum = r_sum + (args.gamma_goal**i) * r_i
            z = agent.forward_dyn(z, a)
        v_term = agent.value(z, g)
        obj = args.reward_coef * r_sum + args.value_coef * (args.gamma_goal**args.policy_unroll) * v_term
        return -obj.mean(), r0, v_term.mean().detach()

    if args.compile:
        rollout_act = torch.compile(rollout_act, mode=args.compile_mode, dynamic=False)
        rollout_propose = torch.compile(rollout_propose, mode=args.compile_mode, dynamic=False)
        ...
        print(f"[embopt_v10] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(T):
            ...
            with torch.no_grad():
                if step % args.goal_refresh == 0:
                    mark_step()
                    cur_goal = rollout_propose(nobs(next_obs)).clone()
                roll_goals[step] = cur_goal
                mark_step()
                action = rollout_act(nobs(next_obs), cur_goal).clone()
        ...
        with torch.no_grad():
            ...
            Z = agent.encode(nobs(seg_obs.reshape(-1, do))).reshape(S, T + 1, E, dz)
        ...
        # ---- proposer updates: seeker ascends imagined-path payoff through frozen f ------
        prop_stats = []
        ...
        for _ in range(args.policy_updates):
            ...
            with torch.no_grad():
                zp = agent.encode(nobs(buf_obs[flat // (T * E), (flat // E) % T, flat % E]))
                gp = agent.propose(zp)
