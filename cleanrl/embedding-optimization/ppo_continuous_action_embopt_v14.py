# Embedding Optimization v14 (embopt_v14) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v13 -> v14: EXACT-k HINDSIGHT TARGETS (the family spec's lambda=1 special case, applied
# precisely where the evidence is exact). v13 @1.6M: consistently learning (-32 best,
# promise/delivery loop functioning, proposer on the band target) but dist_calib_abs ~ 28
# on distances averaging ~45 — the head is right on average, wrong per pair. Cause: the
# lambda=0.95 recursion trains hindsight pairs mostly on BOOTSTRAP (the exact distance
# enters only at the arrival reset and diffuses backward through V's own miscalibrated
# predictions) while the exact label k = j - t sits unused. Fix: after the recursion,
# hindsight-column targets are overwritten with gamma^k exactly — pure regression to the
# realized path distance (V^pi semantics: the policy's own behavioral distance, the right
# quantity for its improvement). Commanded columns keep the lambda-bootstrap ratchet at
# cmd_weight 0.15 (they have no arrival and need the bootstrap). Expected:
# dist_calib_abs -> single digits, making the policy's dominant d-gradient point at truth.
# --- v13 header follows ---
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(f"[embopt_v14] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # hindsight columns get EXACT targets: the realized path distance k = j - t
            # is known, so regress to gamma^k directly (lambda=1 / Phase-A special case)
            # instead of the bootstrap-diffused lambda-return; commanded columns (no
            # arrival, bootstrap required) keep the recursion's output
            M0 = args.hindsight_goals
            k_exact = (j[:, None, :, :M0] - t_range).float()  # (S,T,E,M0); valid where > 0
            targets[:, :, :, :M0] = args.gamma_goal**k_exact
