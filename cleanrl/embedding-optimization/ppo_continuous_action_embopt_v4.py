# Embedding Optimization v4 (embopt_v4) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v3 -> v4: v3's shell constraint fixed the norm collapse but exposed the next exploit,
# measured at 1M: proposer_what = 5-8 (goals scoring 6+ sigma above any real reward rate)
# while rollout_goal_dv < 0 — the proposer walked along the shell into the measure-zero
# complement of the data submanifold where the frozen W_hat extrapolates fantasy, the
# commanded goals were unreachable, and the policy learned from the myopic r_hat term only
# (limited, stochastic return improvement; goal_removal decaying as the policy learned to
# ignore useless goals).
# v4 applies the SAME medicine that fixed V's extrapolation in v1 review, symmetrically:
# W_hat gets negative anchors — random shell points (in 63-dim, almost surely far from the
# data submanifold) regressed toward the minibatch-minimum rate. Family principle, now
# standing: EVERY frozen head that gets ascended needs its extrapolation anchored on the
# complement of the data. Adds the blind-spot telemetry that would have caught this in one
# glance: W_hat on real states (mean/max) next to W_hat on proposed goals.
#
