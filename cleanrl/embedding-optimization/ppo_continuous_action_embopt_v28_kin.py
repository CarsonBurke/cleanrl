# Embedding Optimization v28_kin (embopt_v28_kin) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v22 -> v28_kin: PROBE ARM B — PURE KINETIC BONUS, GOAL CONTENT REMOVED.
...
# target becomes the goal-INDEPENDENT per-dim squared latent step
# ||z'-z||^2/dz (the quantity the v25/v26 projection overpaid; the head keeps
# its (z,g,g-z,a) input and is free to ignore g; proposer/goal machinery
# untouched so everything else — inputs, shapes, schedules — is byte-identical
# to v22). Question: is a pure latent-speed bonus an adjuvant at all? If
# +584-ish, the goal's directional content never mattered (the adjuvant was
# always "move fast in latent space"). If -149-ish/worse, kinetic-bonus is
# dead as a mechanism and DIRECTION is what the adjuvant provides.
# --- v22 header follows ---
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(f"[embopt_v28_kin] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # PURE KINETIC target: per-dim squared latent step, goal-independent —
            # exactly the quantity the v25/v26 projection target overpaid. Tests
            # whether latent speed alone is the adjuvant.
