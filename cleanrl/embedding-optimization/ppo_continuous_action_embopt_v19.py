# Embedding Optimization v19 (embopt_v19) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
...
if __name__ == "__main__":
    ...
    def policy_loss_fn(z, g):
        # one differentiable scalar, no model step: both terms are frozen regression
        # surfaces of measured outcomes, so the ascent direction stays grounded on
        # the executed-action ball (expl_noise) that keeps labeling them
        a = agent.act(z, g)
        r = agent.rhat(z, a)
        qp = agent.qprog(z, a, g)
        obj = args.reward_coef * r + args.value_coef * qp
        return -obj.mean(), r.mean().detach(), qp.mean().detach()
    ...
    if args.compile:
        ...
        print(f"[embopt_v19] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
