# Embedding Optimization v5 (embopt_v5) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v4 -> v5, two failure fronts measured at 1.75M and one reference audit (../le-wm):
# (A) v4's random-shell W_hat anchors FAILED to close the fantasy gap (proposer_what 6-11 vs
#     what_real_max <= 2.6): you cannot cap the max of an unconstrained MLP over a 63-dim
#     shell by pinning samples — ascent finds ridges between anchors. V's own grounding held
#     (proposer_v ~ 0), but the ADDITIVE score lets W_hat=10 outvote V=0. v5 makes
#     reachability multiplicative and clamps predicted rate to observed support:
#         score = V(z,g) * (clamp(W_hat(g), r_lo, r_hi) - r_lo)
#     At the clamp the W gradient dies and only V's pull remains (drags goals back toward
#     reachable) — self-correcting, and r_hi ratchets up with real performance.
# (B) wm_pred plateau, diagnosed against le-wm:
#     1) SIGReg statistic scales linearly with batch N; coef 0.09 was balanced at batch 128,
#        our 2048 minibatch made it ~16x too strong (isotropy pressure crushing predictive
#        structure). Fixed by pinning the N-factor to sigreg_ref_n=128 (batch-invariant).
#     2) Stale normalization: le-wm z-scores with FROZEN dataset-global stats; we replayed
#        observations warped by gym's drifting running stats. v5 stores RAW obs and
#        normalizes with the CURRENT running stats at every use site.
#     3) WM optimizer AdamW(1e-4, wd=1e-3) + LayerNorm in encoder/dynamics hidden layers
#        (le-wm: AdamW 5e-5 wd 1e-3, norm everywhere; our Adam 3e-4 bare — matches the
#        observed embedding-scale drift). le-wm's BN asymmetric pred head deliberately
#        skipped: collapse is not our failure mode and BN mutates state under cudagraphs.
#
...
class Args:
    ...
    what_neg_coef: float = 0.25
    """weight of W_hat negative anchors at random shell points (anti-fantasy grounding)"""
...
if __name__ == "__main__":
    ...
    def wm_loss_fn(o, a, o2, r, rate, pair_valid):
        ...
        # anti-fantasy grounding: random shell points are (w.h.p.) far off the data
        # submanifold — pin W_hat there to the worst observed rate so proposer ascent
        # cannot harvest extrapolated optimism (same medicine as the critic's negatives)
        rn = torch.randn_like(zd)
        g_tilde = (zd.shape[-1] ** 0.5) * rn / rn.norm(dim=-1, keepdim=True).clamp_min(1e-6)
        w_neg_loss = (agent.what(g_tilde) - rate.min().detach()).square().mean()
        loss = (
            pred_loss
            + args.sigreg_coef * sig_loss
            + r_loss
            + w_loss
            + args.what_neg_coef * w_neg_loss
        )
    ...
    def proposer_loss_fn(z, r_lo, r_hi):
        g = agent.propose(z)
        # reachability-gated, support-clamped score: predicted rate can never exceed the
        # best OBSERVED rate (fantasy capped; the ceiling ratchets up with performance),
        # and at the clamp only V's gradient survives, pulling goals back toward reachable
