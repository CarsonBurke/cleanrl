# Embedding Optimization v28 (embopt_v28) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v22 -> v28: PROBE ARM A — EXACT-REDUCTION GOAL TERM ON THE FULL v22 CHASSIS.
# 2x2 decomposition of the goal term (rungs: v22 dd +584, v22_vc0 none -149,
# v28 exact ?, v28_kin kinetic ?). ONE substantive delta from v22: the qprog
# target changes from the distance-delta ldist(z,g) - ldist(z',g) (D^2 units,
# telescoping, contains a kinetic PENALTY -||d||^2/dz) to the EXACT one-step
# distance reduction (||g-z|| - ||g-z'||)/sqrt(dz) (D units, telescoping,
# exploit-free — the v27 target). value_coef default raised 1.0 -> 3.0 to
# scale-match action gradients: dd's d/dcos = 2*sqrt(l*v) ~ 3.4 vs exact's
# 0.9-1.3. Question: does the honest adjuvant retain v22's +584? If yes, v27
# froze only for lacking reward heads and the exact target becomes the family
# standard. If -149-ish, dd's D^2/kinetic-penalty structure was load-bearing.
# --- v22 header follows ---
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(f"[embopt_v28] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # EXACT one-step distance reduction (v27 target): telescopes for a
            # fixed g (orbit-proof) and |tgt| <= ||d||/sqrt(dz) (no distance
            # inflation) — the honest replacement for the D^2 distance-delta
            tgt_pool = (
                (g_pool - Zs[:, :T]).norm(dim=-1) - (g_pool - Zs[:, wend_idx]).norm(dim=-1)
            ) / dz**0.5  # (K,T,E)
