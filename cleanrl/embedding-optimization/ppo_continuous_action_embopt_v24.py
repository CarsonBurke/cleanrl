# Embedding Optimization v24 (embopt_v24) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # belief map what(z): regresses measured REWARD EFFICIENCY — cumulative
        # reward en route to z divided by steps to reach it (joy per unit time)
...
if __name__ == "__main__":
    ...
    def wm_loss_fn(o, a, o2, r, rate, rv, eff, ev, pair_valid):
        ...
        # belief map: measured reward EFFICIENCY of reaching this state (cumulative
        # reward / steps since episode start); ev masks reset states (0-step paths)
    ...
    def proposer_loss_fn(z):
        # score = the BELIEF map what(g): the predicted reward EFFICIENCY of the
        # goal state (cumulative reward en route / steps to reach), extrapolating
        # beyond experience. Efficiency prices travel cost into the aspiration
        # itself: a distant state only scores high if the whole journey pays.
    ...
    if args.compile:
        ...
        print(f"[embopt_v24] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    buf_eff = torch.zeros((R, T, E), device=device)  # cum reward / steps-to-reach at obs[t]
    buf_effv = torch.zeros((R, T, E), device=device)  # 0 at reset states (0-step path)
    ...
    # reward-efficiency accumulators: cumulative reward and steps since episode
    # start, per env — the efficiency label of the state REACHED (before acting)
    ep_cum = torch.zeros(E, device=device)
    ep_len = torch.zeros(E, device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(T):
            ...
            buf_eff[s, step] = ep_cum / ep_len.clamp_min(1.0)
            buf_effv[s, step] = (ep_len > 0).float()
            with torch.no_grad():
                mark_step()
            ...
            # advance accumulators; zero on done (autoreset: next_obs starts a new path)
            ep_cum = (ep_cum + buf_rew[s, step]) * (1.0 - next_done)
            ep_len = (ep_len + 1.0) * (1.0 - next_done)
