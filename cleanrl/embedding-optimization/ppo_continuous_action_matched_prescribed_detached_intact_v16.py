# Matched Prescribed Detached INTACT v16
#
# A single-action INTACT law is matched on detached predicted-local and factual
# H12-goal intents. A separate prescriber learns through the frozen law/world;
# runtime remains one deterministic prescriber/law pass.
import math
import os
import random
import time
...
class Args:
    ...
    physical_intact_fraction: float = 0.5
...
class IntentActionLaw(nn.Module):
    """Shared fixed-variance INTACT Gaussian over one action."""
    ...
    def nll(self, z, intent, previous_action, action):
        if action.shape[-1] != self.action_dim:
            raise ValueError("action has incompatible size")
        return -self(z, intent, previous_action).log_prob(action).sum(dim=-1)


class IntentPrescriber(nn.Module):
    """State-dependent bounded latent-intent direction."""
    ...
    def forward(self, z, previous_action, intent_scale):
        if z.shape[:-1] != previous_action.shape[:-1]:
            raise ValueError("state and previous action batches must match")
        raw_direction = self.network(torch.cat([z, previous_action], dim=-1))
        direction = raw_direction / raw_direction.norm(
            dim=-1, keepdim=True
        ).clamp_min(1e-8)
        return direction * intent_scale, raw_direction


class Agent(nn.Module):
    """Prediction model, detached inverse law, and direct intent prescriber."""
    ...
    def __init__(self, obs_dim, action_dim, args):
        ...
        self.intent_prescriber = IntentPrescriber(
    ...
    def prescribe_intent(self, z, previous_action):
        scale = self.factual_intent_mean_square.clamp_min(1e-12).sqrt()
        return self.intent_prescriber(z, previous_action, scale)
...
def matched_intact_objective(agent, factual_z, previous_action, actions, args):
    """Match one shared law on detached predicted-local and factual-goal intents."""

    z_start = factual_z[:, 0].detach()
    demonstrated_action = actions[:, 0].detach()
    previous_action = previous_action.detach()
    with torch.no_grad():
        predicted_z1 = agent.predict_next(z_start, demonstrated_action)
    physical_intent = (predicted_z1.detach() - z_start).detach()
    goal_intent = (factual_z[:, -1].detach() - z_start).detach()
    physical_mean, physical_log_std = agent.intact_action_law.parameters_for(
        z_start, physical_intent, previous_action
    )
    goal_mean, goal_log_std = agent.intact_action_law.parameters_for(
        z_start, goal_intent, previous_action
    )
    physical_nll = agent.intact_action_law.nll(
        z_start, physical_intent, previous_action, demonstrated_action
    ).mean()
    goal_nll = agent.intact_action_law.nll(
        z_start, goal_intent, previous_action, demonstrated_action
    ).mean()
    shuffled_goal_mean, _ = agent.intact_action_law.parameters_for(
        z_start, goal_intent.roll(1, dims=0), previous_action
    )
    total = (
        args.physical_intact_fraction * physical_nll
        + (1.0 - args.physical_intact_fraction) * goal_nll
    )
    metrics = {
        "matched_intact_loss": total.detach(),
        "physical_nll": physical_nll.detach(),
        "goal_nll": goal_nll.detach(),
        "physical_action_mae": (
            physical_mean - demonstrated_action
        ).abs().mean().detach(),
        "goal_action_mae": (
            goal_mean - demonstrated_action
        ).abs().mean().detach(),
        "goal_physical_action_gap": (
            goal_mean - physical_mean
        ).abs().mean().detach(),
        "goal_physical_action_alignment": nn.functional.cosine_similarity(
            goal_mean, physical_mean, dim=-1
        ).mean().detach(),
        "goal_intent_use": (
            goal_mean - shuffled_goal_mean
        ).abs().mean().detach(),
        "fixed_std": (
            0.5 * (physical_log_std.exp() + goal_log_std.exp())
        ).mean().detach(),
    }
    return total, metrics
...
def closed_loop_direct_score(
    ...
    law_state = _detached_module_state(agent.intact_action_law)
    score = torch.zeros(
    ...
    for _ in range(horizon):
        prescribed_intent, _ = agent.prescribe_intent(
            predicted_z, previous_action
        )
        flat_mean = functional_call(
            agent.intact_action_law,
            law_state,
            (predicted_z, prescribed_intent, previous_action),
        ).mean
        raw_action = flat_mean
...
def direct_controller_objective(agent, z_start, previous_action, args):
    ...
    previous_action = previous_action.detach()
    prescribed_intent, raw_direction = agent.prescribe_intent(
        z_start, previous_action
    )
    shuffled_intent, _ = agent.prescribe_intent(
        z_start.roll(1, dims=0), previous_action
    )
    zero_previous_intent, _ = agent.prescribe_intent(
        z_start, torch.zeros_like(previous_action)
    )
    ...
    metrics = {
        ...
        "prescribed_intent_norm": prescribed_intent.norm(dim=-1).mean().detach(),
        "prescriber_raw_norm": raw_direction.norm(dim=-1).mean().detach(),
        "prescriber_state_use": (
            prescribed_intent - shuffled_intent
        ).abs().mean().detach(),
        "prescriber_previous_action_use": (
            prescribed_intent - zero_previous_intent
        ).abs().mean().detach(),
...
def prescriber_parameters(agent):
    return tuple(agent.intent_prescriber.parameters())
...
@torch.no_grad()
def direct_action(agent, z, previous_action):
    """Decode one prescribed-intent action mean deterministically."""

    prescribed_intent, raw_direction = agent.prescribe_intent(
        z.detach(), previous_action.detach()
    )
    flat_mean, log_std = agent.intact_action_law.parameters_for(
        z, prescribed_intent, previous_action
    )
    action = flat_mean.clamp(-1.0, 1.0)
    diagnostics = {
        "direct_mean_magnitude": flat_mean.abs().mean(dim=-1),
        "direct_std_mean": log_std.exp().mean(dim=-1),
        "prescribed_intent_norm": prescribed_intent.norm(dim=-1),
        "prescriber_raw_norm": raw_direction.norm(dim=-1),
    }
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "detached INTACT World Direct v16 uses 64-D latents"
    assert args.model_horizon == 12
    assert 1 <= args.controller_horizon <= args.model_horizon
    assert args.intact_fixed_std > 0.0
    assert 0.0 <= args.physical_intact_fraction <= 1.0
    ...
    prescriber_params = prescriber_parameters(agent)
    parameter_groups = [
        {id(parameter) for parameter in parameters}
        for parameters in (wm_parameters, law_parameters, prescriber_params)
    ]
    assert not (parameter_groups[0] & parameter_groups[1])
    assert not (parameter_groups[0] & parameter_groups[2])
    assert not (parameter_groups[1] & parameter_groups[2])
    ...
    law_optimizer = optim.AdamW(
        law_parameters,
        lr=args.controller_lr,
        weight_decay=args.weight_decay,
        eps=1e-5,
    )
    prescriber_optimizer = optim.AdamW(
        prescriber_params,
        lr=args.controller_lr,
        # Output normalization makes radial shrinkage meaningless and can drive
        # the raw direction toward the numerical normalization floor.
        weight_decay=0.0,
        eps=1e-5,
    )
    wm_loss_function = world_model_objective
    intact_loss_function = matched_intact_objective
    direct_loss_function = direct_controller_objective
    if args.compile:
        wm_loss_function = torch.compile(
            wm_loss_function, mode=args.compile_mode, dynamic=False
        )
        intact_loss_function = torch.compile(
            intact_loss_function, mode=args.compile_mode, dynamic=False
        )
        direct_loss_function = torch.compile(
            direct_loss_function, mode=args.compile_mode, dynamic=False
        )
        print(
            "[matched_prescribed_detached_intact_v16] "
            f"compiled three isolated objectives (mode={args.compile_mode!r})"
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            law_optimizer.param_groups[0]["lr"] = fraction * args.controller_lr
            prescriber_optimizer.param_groups[0]["lr"] = (
                fraction * args.controller_lr
            )
        ...
        rollout_prescribed_intent_norm = torch.zeros((), device=device)
        rollout_prescriber_raw_norm = torch.zeros((), device=device)
        ...
        for _ in range(args.num_steps):
            ...
            if global_step < args.warmup_steps:
                ...
                diagnostics = {
                    ...
                    "prescribed_intent_norm": zero,
                    "prescriber_raw_norm": zero,
                }
            else:
                action, diagnostics = direct_action(
                    agent,
                    z,
                    previous_action,
                )
            ...
            rollout_prescribed_intent_norm += diagnostics[
                "prescribed_intent_norm"
            ].mean()
            rollout_prescriber_raw_norm += diagnostics[
                "prescriber_raw_norm"
            ].mean()
        ...
        if replay.size >= required_replay:
            ...
            for batch in training_batches:
                ...
                law_optimizer.zero_grad(set_to_none=True)
                prescriber_optimizer.zero_grad(set_to_none=True)
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                wm_loss, wm_metrics = wm_loss_function(agent, sigreg, batch, args)
                wm_loss.backward()
                assert_parameters_have_no_gradients(law_parameters, "INTACT law")
                assert_parameters_have_no_gradients(
                    prescriber_params, "intent prescriber"
                )
                ...
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                intact_loss, intact_metrics = intact_loss_function(
                    agent,
                    factual_z,
                ...
                law_optimizer.step()
                intact_metrics = {
                    key: value.clone() for key, value in intact_metrics.items()
                }
                law_optimizer.zero_grad(set_to_none=True)
                ...
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                direct_loss, direct_metrics = direct_loss_function(
                    agent,
                    factual_z[:, 0],
                    batch.previous_actions[:, 0],
                    args,
                ...
                assert_parameters_have_no_gradients(wm_parameters, "world model")
                assert_parameters_have_no_gradients(law_parameters, "INTACT law")
                nn.utils.clip_grad_norm_(prescriber_params, args.max_grad_norm)
                prescriber_optimizer.step()
                direct_metrics = {
                    key: value.clone() for key, value in direct_metrics.items()
                }
                prescriber_optimizer.zero_grad(set_to_none=True)
                update_metrics.append(
                    {**wm_metrics, **intact_metrics, **direct_metrics}
                )
        ...
        writer.add_scalar(
            "direct/prescribed_intent_norm",
            rollout_prescribed_intent_norm / args.num_steps,
            global_step,
        )
        writer.add_scalar(
            "direct/prescriber_raw_norm",
            rollout_prescriber_raw_norm / args.num_steps,
            global_step,
        )
