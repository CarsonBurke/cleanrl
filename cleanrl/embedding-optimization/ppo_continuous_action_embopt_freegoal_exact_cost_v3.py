# Embedding Optimization: Exact-Cost Factual Straight-Through Free Goals v3
#
...
# A forward-reward ensemble models state-transition reward while the environment's
# action-squared cost is differentiated exactly from its configured coefficient.
...
class Args:
    ...
    route_diagnostics_interval: int = 32
...
def exact_control_cost(actions, ctrl_cost_weight, reward_scale=1.0):
    """Environment-exact action cost, optionally in scaled reward units."""
    return (
        reward_scale
        * ctrl_cost_weight
...
def forward_reward_from_total(total_reward, actions, ctrl_cost_weight):
    """Invert total = forward - control without learning the action cost."""
    return total_reward + exact_control_cost(actions, ctrl_cost_weight)


def configured_ctrl_cost_weight(vector_env):
    weights = {
        float(env.unwrapped._ctrl_cost_weight) for env in vector_env.envs
    }
    if len(weights) != 1:
        raise RuntimeError(
            "all vector environments must share one control-cost weight"
        )
    return weights.pop()


def pressure_norm(pressure):
    return pressure.norm(dim=-1).mean()


def pressure_cosine(first, second):
...
def factual_st_actor_loss(
    ...
    with frozen_parameters(agent.world, agent.reward, agent.value):
        ...
        forward_reward_heads = agent.reward(beliefs, training_next)
        next_value_heads = agent.value(training_next)
        forward_value_route = pessimistic_value(
            forward_reward_heads + args.gamma * next_value_heads,
            args.pessimism_coef,
        )
        scaled_exact_cost = exact_control_cost(
            action, ctrl_cost_weight, args.reward_scale
        )
        factual_return = forward_value_route - scaled_exact_cost

        # A separate route detaches the goal before the policy. Delivery,
        # uncertainty therefore shape pi but cannot update G.
    ...
    metrics = {
        ...
        "learned_scaled_forward_reward": (
            forward_reward_heads.mean().detach()
        ),
        "exact_scaled_control_cost": scaled_exact_cost.mean().detach(),
        "learned_scaled_total_reward": (
            forward_reward_heads.mean(dim=0) - scaled_exact_cost
        ).mean().detach(),
    ...
    if diagnose_routes:
        # Each tensor is the ascent pressure contributed by exactly one route.
        # autograd.grad is observational: it does not populate parameter .grad.
        return_action_pressure = torch.autograd.grad(
            forward_value_route.sum(),
            action,
            retain_graph=True,
        )[0]
        delivery_action_pressure = torch.autograd.grad(
            (-args.goal_delivery_coef * delivery_mse).sum(),
            delivery_action,
            retain_graph=True,
        )[0]
        exact_cost_action_pressure = torch.autograd.grad(
            (-scaled_exact_cost).sum(),
            action,
            retain_graph=True,
        )[0]
        uncertainty_action_pressure = torch.autograd.grad(
            (-args.uncertainty_coef * uncertainty).sum(),
            delivery_action,
            retain_graph=True,
        )[0]
        return_raw_pressure = return_action_pressure * (
            1 - action.square()
        )
        delivery_raw_pressure = delivery_action_pressure * (
            1 - delivery_action.square()
        )
        exact_cost_raw_pressure = exact_cost_action_pressure * (
            1 - action.square()
        )
        uncertainty_raw_pressure = uncertainty_action_pressure * (
            1 - delivery_action.square()
        )
        metrics.update(
            {
                "return_action_pressure_norm": (
                    pressure_norm(return_action_pressure).detach()
                ),
                "delivery_action_pressure_norm": (
                    pressure_norm(delivery_action_pressure).detach()
                ),
                "exact_cost_action_pressure_norm": (
                    pressure_norm(exact_cost_action_pressure).detach()
                ),
                "uncertainty_action_pressure_norm": (
                    pressure_norm(uncertainty_action_pressure).detach()
                ),
                "return_raw_policy_pressure_norm": (
                    pressure_norm(return_raw_pressure).detach()
                ),
                "delivery_raw_policy_pressure_norm": (
                    pressure_norm(delivery_raw_pressure).detach()
                ),
                "exact_cost_raw_policy_pressure_norm": (
                    pressure_norm(exact_cost_raw_pressure).detach()
                ),
                "uncertainty_raw_policy_pressure_norm": (
                    pressure_norm(uncertainty_raw_pressure).detach()
                ),
                "return_exact_cost_pressure_cosine": pressure_cosine(
                    return_action_pressure, exact_cost_action_pressure
                ).detach(),
                "return_delivery_pressure_cosine": pressure_cosine(
                    return_action_pressure, delivery_action_pressure
                ).detach(),
                "return_uncertainty_pressure_cosine": pressure_cosine(
                    return_action_pressure, uncertainty_action_pressure
                ).detach(),
            }
        )
...
def main():
    ...
    if args.route_diagnostics_interval <= 0:
        raise ValueError("route_diagnostics_interval must be positive")
    ...
    ctrl_cost_weight = configured_ctrl_cost_weight(envs)
    writer.add_scalar(
        "environment/ctrl_cost_weight", ctrl_cost_weight, 0
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if controlled_iteration:
            ...
            actor_loss, actor_metrics, recomputed_actions = factual_st_actor_loss(
                ...
                args,
                diagnose_routes=(
                    iteration % args.route_diagnostics_interval == 0
                ),
            )
        ...
        rollout_action_tensor = torch.as_tensor(
            rollout_actions.reshape(-1, action_dim), device=device
        )
        raw_total_reward = torch.as_tensor(
            rollout_rewards.reshape(-1), device=device
        )
        raw_control_cost = exact_control_cost(
            rollout_action_tensor, ctrl_cost_weight
        )
        raw_forward_reward = forward_reward_from_total(
            raw_total_reward,
            rollout_action_tensor,
            ctrl_cost_weight,
        )
        reconstructed_total_reward = raw_forward_reward - raw_control_cost
        writer.add_scalar(
            "rewards/raw_total_reward",
            raw_total_reward.mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rewards/raw_forward_reward",
            raw_forward_reward.mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rewards/raw_exact_control_cost",
            raw_control_cost.mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rewards/scaled_total_reward",
            (args.reward_scale * raw_total_reward).mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rewards/scaled_forward_reward",
            (args.reward_scale * raw_forward_reward).mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rewards/scaled_exact_control_cost",
            (args.reward_scale * raw_control_cost).mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rewards/total_reconstruction_mse",
            (
                reconstructed_total_reward - raw_total_reward
            ).square().mean().item(),
            global_step,
        )
        ...
        factual_reward_targets = args.reward_scale * raw_forward_reward
        ...
        reward_optimizer.zero_grad(set_to_none=True)
        writer.add_scalar(
            "reward/factual_forward_mse", reward_loss.item(), global_step
        )
        writer.add_scalar(
