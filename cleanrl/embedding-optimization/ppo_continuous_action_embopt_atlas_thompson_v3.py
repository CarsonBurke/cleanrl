# Embedding Optimization: Factual Atlas Thompson Navigator v3
#
...
# segments. Their realized optimal-stopping reward rate directly drives selection,
# while bootstrapped applicability and support correct transport to the current
# state. Persistent Thompson heads diversify causal interventions. A factual
# multimode bridge, state-only baseline, and exact inverse model measure and
# execute conditional control. There is no PPO, Q-value, TD target, EMA, fixed
# horizon, or learned goal proposer.
import math
import os
import random
import time
...
class Args:
    ...
    controlled_bridge_updates: int = 8
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.state_bridge = mlp(
            dy, hidden, args.navigator_modes * dy, out_std=0.01
        )
    ...
    def navigate_state_only(self, y):
        shape = y.shape[:-1] + (self.navigator_modes, self.goal_dim)
        return self.state_bridge(y.detach()).reshape(shape)
...
def leave_one_out(values, excluded_head):
    """Evaluator ensemble excluding the head that selected a command."""
...
if __name__ == "__main__":
    ...
    pair_params = (
        ...
        + list(agent.state_bridge.parameters())
    ...
    if args.compile:
        ...
        print(
            f"[embopt_atlas_thompson_v3] torch.compile("
    ...
    buffer_command_head = torch.zeros((R, T, E), dtype=torch.long, device=device)
    buffer_filled = 0
    buffer_pointer = 0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(T):
            ...
            with torch.no_grad():
                ...
                if evidence_ready:
                    ...
                    source_rates = motif_realized_rate[nearest]
                    ...
                    effective_candidate_rates = (
                        (1.0 - args.applicability_coef)
                        * source_rates.unsqueeze(-1)
                        + args.applicability_coef * candidate_rates
                    )
                    ...
                    candidate_rate_heads = evaluator_rates[batch, best]
                    candidate_support_heads = evaluator_supports[batch, best]
                    ...
                    current_evaluator_rates = leave_one_out(
                        current_rates, active_head
                    )
                    current_evaluator_logs = leave_one_out(
                        current_logs, active_head
                    )
                    current_evaluator_supports = leave_one_out(
                        current_supports, active_head
                    )
                    requested_switch, arrived, challenger = endogenous_goal_switch(
                        current_evaluator_rates,
                        current_evaluator_logs,
                        current_evaluator_supports,
                    ...
                    active_head = torch.where(install, proposal_head, active_head)
                    ...
                    chosen_rate = leave_one_out(
                        chosen_rates, active_head
                    ).mean(-1)
                    chosen_log_duration = leave_one_out(
                        chosen_logs, active_head
                    ).mean(-1)
                    chosen_support = leave_one_out(
                        chosen_supports, active_head
                    ).mean(-1).sigmoid()
                    ...
                    mode_head = active_head.unsqueeze(1).expand(
                        E, args.navigator_modes
                    ).reshape(-1)
                    mode_scores, _, _ = factual_thompson_score(
                        torch.zeros_like(gather_head(mode_rates, mode_head)),
                        gather_head(mode_rates, mode_head),
                        gather_head(mode_supports, mode_head),
                        1.0,
                        args.support_score_coef,
                        args.minimum_support,
                    )
            ...
            buffer_command_head[slot, step] = active_head
        ...
        for _ in range(args.pair_updates):
            ...
            state_bridge_candidates = agent.navigate_state_only(y0)
            state_bridge_loss, state_bridge_fit, _, _ = navigator_loss(
                state_bridge_candidates,
                target_displacement,
                valid,
                args.navigator_balance_coef,
            )
            ...
            pair_loss = value_loss + nav_loss + state_bridge_loss + inverse_loss
            ...
            pair_stats.append(
                (
                    ...
                    nav_fit.item(),
                    state_bridge_fit.item(),
                    inverse_loss.item(),
                    usage.std(unbiased=False).item(),
                )
            )

        controlled_y = agent.goal_encode(z_sequence).detach()
        controlled_next_y = agent.goal_encode(next_z_sequence).detach()
        controlled_goal = buffer_goal[recent_slots].reshape(sequence_length, E, dy)
        controlled_goal_delta = buffer_goal_delta[recent_slots].reshape(
            sequence_length, E, dy
        )
        controlled_valid = buffer_control_gate[recent_slots].reshape(
            sequence_length, E
        ).bool()
        controlled_index = torch.where(controlled_valid.reshape(-1))[0]
        controlled_bridge_stats = []
        if controlled_index.numel():
            for _ in range(args.controlled_bridge_updates):
                sampled = controlled_index[
                    torch.randint(
                        controlled_index.numel(),
                        (mb,),
                        device=device,
                    )
                ]
                sampled_y = controlled_y.reshape(-1, dy)[sampled]
                sampled_next_y = controlled_next_y.reshape(-1, dy)[sampled]
                sampled_goal = controlled_goal.reshape(-1, dy)[sampled]
                sampled_goal_delta = controlled_goal_delta.reshape(-1, dy)[sampled]
                sampled_z = z_sequence.reshape(-1, dz)[sampled]
                sampled_action = action_sequence.reshape(-1, action_dim)[sampled]
                target_displacement = (sampled_next_y - sampled_y).detach()
                bridge_candidates = agent.navigate(
                    sampled_y, sampled_goal, sampled_goal_delta
                )
                controlled_nav_loss, controlled_nav_fit, _, _ = navigator_loss(
                    bridge_candidates,
                    target_displacement,
                    torch.ones(mb, device=device),
                    args.navigator_balance_coef,
                )
                state_candidates = agent.navigate_state_only(sampled_y)
                controlled_state_loss, controlled_state_fit, _, _ = navigator_loss(
                    state_candidates,
                    target_displacement,
                    torch.ones(mb, device=device),
                    args.navigator_balance_coef,
                )
                predicted_action = agent.act(sampled_z, target_displacement)
                controlled_inverse_loss = (
                    predicted_action - sampled_action
                ).square().mean()
                controlled_loss = (
                    controlled_nav_loss
                    + controlled_state_loss
                    + controlled_inverse_loss
                )
                pair_opt.zero_grad(set_to_none=True)
                controlled_loss.backward(inputs=pair_params)
                nn.utils.clip_grad_norm_(pair_params, args.max_grad_norm)
                pair_opt.step()
                controlled_bridge_stats.append(
                    (
                        controlled_nav_fit.item(),
                        controlled_state_fit.item(),
                        controlled_inverse_loss.item(),
                    )
                )
        ...
        with torch.no_grad():
            ...
            state_bridge_error = (
                agent.navigate_state_only(y0) - nav_target.unsqueeze(1)
            ).square().mean(-1).min(-1).values
            heldout_goal_bridge_error = masked_mean(nav_best, valid)
            heldout_state_bridge_error = masked_mean(state_bridge_error, valid)
            heldout_conditional_gain = (
                heldout_state_bridge_error - heldout_goal_bridge_error
            ) / heldout_state_bridge_error.clamp_min(1e-8)
            nav_error_vs_zero = (
                heldout_goal_bridge_error
                / masked_mean(nav_zero, valid).clamp_min(1e-8)
            )
            ...
            controlled_heads = buffer_command_head[fresh_slot][delivery_mask]
            if controlled_heads.numel():
                active_head_histogram = torch.bincount(
                    controlled_heads,
                    minlength=args.value_heads,
                ).float()
                active_head_probability = (
                    active_head_histogram / active_head_histogram.sum()
                )
                active_head_entropy = -(
                    active_head_probability
                    * active_head_probability.clamp_min(1e-8).log()
                ).sum() / math.log(args.value_heads)
            else:
                active_head_entropy = torch.zeros((), device=device)

        wm_mean = np.mean(wm_stats, axis=0)
        pair_mean = np.mean(pair_stats, axis=0)
        controlled_bridge_mean = (
            np.mean(controlled_bridge_stats, axis=0)
            if controlled_bridge_stats
            else np.zeros(3)
        )
        ...
        writer.add_scalar("losses/state_bridge", pair_mean[4], global_step)
        writer.add_scalar("losses/inverse", pair_mean[5], global_step)
        writer.add_scalar("losses/controlled_bridge", controlled_bridge_mean[0], global_step)
        writer.add_scalar("losses/controlled_state_bridge", controlled_bridge_mean[1], global_step)
        writer.add_scalar("losses/controlled_inverse", controlled_bridge_mean[2], global_step)
        writer.add_scalar(
        ...
        writer.add_scalar("navigator/heldout_goal_bridge_mse", heldout_goal_bridge_error, global_step)
        writer.add_scalar("navigator/heldout_state_bridge_mse", heldout_state_bridge_error, global_step)
        writer.add_scalar("navigator/heldout_conditional_gain", heldout_conditional_gain, global_step)
        ...
        writer.add_scalar("navigator/mode_usage_std", pair_mean[6], global_step)
        ...
        writer.add_scalar("commitment/thompson_head_entropy", active_head_entropy, global_step)
        writer.add_scalar(
