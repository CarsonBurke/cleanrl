# Embedding Optimization v8 (embopt_v8) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
...
if __name__ == "__main__":
    ...
    def critic_loss_fn(z, a, g, target, target_next):
        ...
        # an unrelated goal): V's ONLY off-manifold signal — what "far" means; the band
        # penalty can only reject fantasy goals because of this term
        neg_target = args.gamma_goal**args.num_steps
        neg = (agent.value(z, torch.roll(g, 1, 0)) - neg_target).square().mean()
        return (
            main + args.vf_consistency_coef * consistency + args.neg_goal_coef * neg,
            main.detach(),
            v.mean().detach(),
        )
