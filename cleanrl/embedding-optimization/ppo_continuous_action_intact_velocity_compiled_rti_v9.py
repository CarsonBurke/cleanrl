# INTACT Velocity Compiled RTI v9
#
# Policy-free online HalfCheetah control with a LeJEPA world model and INTACT
# representation shaping. Deployment zero-tail shifts its H12 plan and takes one
# guarded, sign-normalized gradient step on cumulative exact interval velocity
# minus control cost. Only the rollout scorer is compiled; RTI remains eager and
# snapshots compiled graph outputs before reuse. The hypothesis is that the
# validated persistent planner can retain its behavior at lower wall-clock cost.
import math
import os
import random
import time
...
class Args:
    ...
    rti_step_size: float = 0.20
    warm_start_mode: Literal["repeat", "zero_tail", "reset"] = "zero_tail"
    compile_planner: bool = True
...
def shift_action_sequence(previous_sequence, reset, mode="repeat"):
    """Construct the configured warm start and clear reset environments."""
    ...
    if mode == "repeat":
        ...
        shifted = torch.cat(
            [previous_sequence[:, 1:], torch.zeros_like(previous_sequence[:, -1:])],
            dim=1,
        )
    elif mode == "reset":
        shifted = torch.zeros_like(previous_sequence)
    else:
...
@torch.enable_grad()
def rti_action_sequence(
    agent,
    z,
    warm_start,
    args,
    score_function=action_sequence_score,
    compiled_score=False,
):
    """Take one guarded, sign-normalized receding-horizon gradient step."""

    z = z.detach()
    warm_sequence = warm_start.detach().clone().requires_grad_(True)
    if compiled_score:
        torch.compiler.cudagraph_mark_step_begin()
    initial_score, initial_velocities = score_function(
        agent, z, warm_sequence, args.action_cost_coef
    )
    (gradient,) = torch.autograd.grad(initial_score.sum(), warm_sequence)
    # reduce-overhead CUDA graphs reuse output storage on the next invocation.
    # Snapshot every first-call value needed by the eager RTI before that reuse.
    initial_score_snapshot = initial_score.detach().clone()
    initial_velocities_snapshot = initial_velocities.detach().clone()
    gradient_snapshot = gradient.detach().clone()
    with torch.no_grad():
        candidate = (
            warm_sequence
            + args.rti_step_size * gradient_snapshot.sign()
        ).clamp(-1.0, 1.0)
        if compiled_score:
            torch.compiler.cudagraph_mark_step_begin()
        candidate_score, candidate_velocities = score_function(
            agent, z, candidate, args.action_cost_coef
        )
        accepted = candidate_score > initial_score_snapshot
        selected_sequence = torch.where(
            accepted[:, None, None], candidate, warm_sequence.detach()
        ).clone()
        ...
        selected_velocities = torch.where(
            ...
            initial_velocities_snapshot,
        ).clone()
    diagnostics = {
        "optimizer_improvement": selected_score - initial_score_snapshot,
        "gradient_norm": gradient_snapshot.flatten(1).norm(dim=-1),
        "gradient_sign_active_fraction": (
            gradient_snapshot != 0.0
        ).float().flatten(1).mean(dim=-1),
        "accept_fraction": accepted.float(),
        "action_change": (
            selected_sequence[:, 0] - warm_start[:, 0]
        ).norm(dim=-1),
        "sequence_change": (
            selected_sequence - warm_start
        ).flatten(1).norm(dim=-1),
        "predicted_interval_velocity": selected_velocities[:, 0],
        "selected_score": selected_score,
    }
    return selected_sequence.detach(), diagnostics
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "INTACT velocity compiled RTI v9 uses 64-D latents"
    assert args.model_horizon >= 1
    assert args.planner_horizon >= 1
    assert args.planner_horizon <= args.model_horizon
    assert args.rti_step_size > 0.0
    assert args.warm_start_mode in ("repeat", "zero_tail", "reset")
    ...
    planner_score_function = action_sequence_score
    planner_score_is_compiled = args.compile and args.compile_planner
    if args.compile:
        loss_function = torch.compile(
            loss_function, mode=args.compile_mode, dynamic=False
        )
        print(
            "[intact_velocity_compiled_rti_v9] "
            f"compiled learner (mode={args.compile_mode!r}, dynamic=False)"
        )
    if planner_score_is_compiled:
        planner_score_function = torch.compile(
            action_sequence_score, mode=args.compile_mode, dynamic=False
        )
        print(
            "[intact_velocity_compiled_rti_v9] "
            f"compiled planner scorer (mode={args.compile_mode!r}, dynamic=False)"
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        rollout_sign_active_fraction = torch.zeros((), device=device)
        rollout_accept_fraction = torch.zeros((), device=device)
        ...
        for _ in range(args.num_steps):
            ...
            warm_start_sequence = shift_action_sequence(
                planned_action_sequence, reset, args.warm_start_mode
            )
            ...
            if global_step < args.warmup_steps:
                ...
                with torch.no_grad():
                    diagnostics = {
                        ...
                        "gradient_norm": zero,
                        "gradient_sign_active_fraction": zero,
                        "accept_fraction": zero,
                        "action_change": (
                ...
                planned_action_sequence, diagnostics = rti_action_sequence(
                    agent,
                    z,
                    warm_start_sequence,
                    args,
                    score_function=planner_score_function,
                    compiled_score=planner_score_is_compiled,
                )
            ...
            rollout_gradient_norm += diagnostics["gradient_norm"].mean()
            rollout_sign_active_fraction += diagnostics[
                "gradient_sign_active_fraction"
            ].mean()
            rollout_accept_fraction += diagnostics["accept_fraction"].mean()
        ...
        writer.add_scalar(
            "rti/gradient_sign_active_fraction",
            rollout_sign_active_fraction / args.num_steps,
            global_step,
        )
        writer.add_scalar(
            "rti/accept_fraction",
            rollout_accept_fraction / args.num_steps,
            global_step,
        )
        writer.add_scalar(
            "rti/action_change",
