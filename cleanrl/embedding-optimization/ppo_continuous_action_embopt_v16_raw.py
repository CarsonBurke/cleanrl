# Embedding Optimization v16 (embopt_v16) — family: cleanrl/embedding-optimization/ (see FAMILY.md)

# RAW BACKPORT (embopt_v16_raw): identical algorithm to v16 with the v19
...
class Args:
    ...
    prog_coef: float = 1.0
    """weight of predicted window progress h(z,g) in the proposer score"""
    imag_probe_h: int = 8
...
if __name__ == "__main__":
    ...
    def proposer_loss_fn(z):
        g = agent.propose(z)
        # UNCLAMPED scalar critic (standard). W_hat is grounded at goal points instead:
        # every commanded goal's promised rate is regressed toward its measured
        # delivered rate (see wm_loss_fn), so ascending the surface into fantasy is
        # corrected by facts rather than masked by a clamp — and the proposer keeps a
        # live gradient above the best observed rate. h adds predicted measurable
        # progress toward g (reachability).
        w = agent.what(g)
        p = agent.prog(z, g)
        score = w + args.prog_coef * p
        return -score.mean(), w.mean().detach(), p.mean().detach()
    ...
    if args.compile:
        ...
        print(f"[embopt_v16_raw] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
