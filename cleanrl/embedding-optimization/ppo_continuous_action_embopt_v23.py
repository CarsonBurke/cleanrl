# Embedding Optimization v23 (embopt_v23) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v22 -> v23: PESSIMISTIC TWIN PROGRESS CRITIC + REACHABILITY-AWARE ASPIRATION.
# v22 took off (+575 last-20 @2.5M, family best) but its telemetry exposed two
# structural defects it succeeded DESPITE:
#   (1) qprog over-claims exactly where the policy queries it: policy_prog 0.33
#       vs realized rollout_goal_prog ~0, while in-distribution calibration is
#       clean (prog_calib_bias ~0). Extrapolation error at the executed-action
#       ball edge — the same pathology as v19's dynamics cross-term, relocated
#       from model error into the regression head. Fix: TWIN progress heads
#       trained on the SAME measured targets from different random inits; every
#       consumer (policy, proposer, telemetry) reads min(q1, q2). This is pure
#       regression pessimism — no bootstrap, no target nets, no TD: two
#       independent fits agree on the labeled ball and disagree most exactly
#       off-distribution, so the min prunes fantasy directions while leaving
#       grounded credit intact. qprog_gap = |q1 - q2| tracks where the policy
#       is querying relative to the data.
#   (2) The proposer objective -what(propose(z)) had NO z-dependence, so it
#       collapsed to one constant shell point (proposer_out_std ~0.004).
#       Benign in v22 — persistence from a stable belief is the design intent —
#       but it discards "the best next step from HERE" (the doctor analogy has
#       both a destination AND a current-state-dependent next step). Fix:
#       L_prop = -[ what(g) + prop_reach_coef * min-q(z, act(z,g), g) ]:
#       aspiration scored by the belief AND by pursuability from the current
#       state. z re-enters the objective, so per-state goal variety returns
#       where reachability differs; the reachability term is read through the
#       pessimistic min, so the proposer cannot harvest critic fantasy either.
# Hypothesis: v22's takeoff under a constant fantasy pull and an exploitable
# critic is a LOWER BOUND for this design. Pruning the off-ball over-claim and
# making goals reachability-aware turns the goal channel from a static bias
# into functional per-state guidance. Decisive metric: rollout_goal_prog must
# lift off ~0 (realized pursuit, flat zero in every version so far).
# --- v22 header follows ---
...
class Args:
    ...
    learning is the family's point)"""
    prop_reach_coef: float = 1.0
    """weight of the pessimistic reachability term min-q(z, act(z,g), g) in the
    proposer score: aspiration must be pursuable from the current state. Restores
    z-dependence (v22's belief-only score collapsed to one constant goal)"""
    imag_probe_h: int = 8
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # TWIN action-conditioned progress heads q(z,a,g): same measured-outcome
        # regression targets, different random inits. Consumers read min(q1,q2) —
        # the two fits agree on the labeled ball and diverge off-distribution, so
        # the min prunes exactly the extrapolation over-claim v22 measured
        # (policy_prog 0.33 vs realized ~0 at clean in-distribution calibration)
        self.critic = mlp(3 * dz + da, dh, 1)
        self.critic2 = mlp(3 * dz + da, dh, 1)
    ...
    def qprog2(self, z, a, g):
        return self.critic2(torch.cat([z, g, g - z, a], -1)).squeeze(-1)

    def qprog_min(self, z, a, g):
        # pessimistic read for every consumer that ASCENDS the surface (policy,
        # proposer, telemetry); training regresses both heads separately
        x = torch.cat([z, g, g - z, a], -1)
        return torch.minimum(self.critic(x).squeeze(-1), self.critic2(x).squeeze(-1))
...
if __name__ == "__main__":
    ...
    critic_params = list(agent.critic.parameters()) + list(agent.critic2.parameters())
    ...
    def critic_loss_fn(z, a, g, target, m):
        ...
        pred1 = agent.qprog(z, a, g)
        pred2 = agent.qprog2(z, a, g)
        denom = m.sum().clamp_min(1.0)
        loss = (
            (m * nn.functional.smooth_l1_loss(pred1, target, reduction="none")).sum()
            + (m * nn.functional.smooth_l1_loss(pred2, target, reduction="none")).sum()
        ) / denom
        pmin = torch.minimum(pred1, pred2)
        return loss, ((pmin * m).sum() / denom).detach()

    def proposer_loss_fn(z):
        # score = the BELIEF map what(g): the predicted reward rate OF THE GOAL
        # STATE ITSELF, extrapolating beyond experience — aspiration, not the
        # delivered-rate association credit that made v16-v19 thrash. Grounded by
        # the pursuit loop: what() regresses measured rates at real states, and
        # pursuit moves the visited distribution toward the goal region.
        # + reachability: the goal must also be pursuable from HERE, read through
        # the pessimistic min so the proposer cannot harvest critic fantasy. This
        # restores z-dependence (v22's belief-only score collapsed to one constant
        # shell point) — the doctor analogy has a destination AND a next step
        g = agent.propose(z)
        w = agent.what(g)
        p = agent.qprog_min(z, agent.act(z, g), g)
        return -(w + args.prop_reach_coef * p).mean(), w.mean().detach(), p.mean().detach()
    ...
    if args.compile:
        ...
        print(f"[embopt_v23] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # -> realized decrease in embed MSE to that goal over the step. Goals vary
        ...
        with torch.no_grad():
            ...
            # twin disagreement at the policy's own query points: large gap = the
            # policy is ascending into off-distribution territory the min prunes
            qprog_gap = (
                (agent.qprog(zb, a_matched, gb) - agent.qprog2(zb, a_matched, gb)).abs().mean().item()
            )
        ...
        grad_v = torch.autograd.grad(agent.qprog_min(zb, a2, gb).sum(), a2)[0]
        ...
        writer.add_scalar("diagnostics/qprog_gap", qprog_gap, global_step)
