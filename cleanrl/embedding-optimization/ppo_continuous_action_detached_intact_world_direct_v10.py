# Detached INTACT World Direct v10
#
# A detached physical INTACT auxiliary teaches one fixed-variance H12 chunk law
# coherent replay actions without shaping the world representation. A learned
# forward-objective token is trained through that frozen law and a frozen world
# model, replanning on every predicted state exactly as at deployment.
# The hypothesis is that direct amortized control preserves H12 behavior support
# while avoiding runtime rollout, search, sampling, and action optimization.
import math
import os
import random
import time
...
class Args:
    ...
    detached_intact_coef: float = 0.1
...
def direct_controller_objective(agent, z_start, previous_action, args):
    ...
    law_state = _detached_module_state(agent.intact_action_law)
    objective_mean = functional_call(
        agent.intact_action_law,
        law_state,
        (z_start.detach(), objective_intent, previous_action.detach()),
    ).mean
    zero_mean = functional_call(
        agent.intact_action_law,
        law_state,
        (
            z_start.detach(),
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "detached INTACT World Direct v10 uses 64-D latents"
    ...
    parameter_groups = [
        ...
        for parameters in (wm_parameters, law_parameters, token_parameters)
    ]
    ...
    token_optimizer = optim.AdamW(
        token_parameters,
        lr=args.controller_lr,
        # The controller parameter is a direction; radial decay is meaningless
        # after normalization and would only drive it toward the singular origin.
        weight_decay=0.0,
        eps=1e-5,
    )
    ...
    if args.compile:
        ...
        print(
            "[detached_intact_world_direct_v10] "
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            token_optimizer.param_groups[0]["lr"] = fraction * args.controller_lr
        ...
        if replay.size >= required_replay:
            ...
            for batch in training_batches:
                ...
                assert_parameters_have_no_gradients(token_parameters, "objective token")
                ...
                (args.detached_intact_coef * intact_loss).backward()
                ...
                (args.direct_objective_coef * direct_loss).backward()
                assert_parameters_have_no_gradients(wm_parameters, "world model")
                assert_parameters_have_no_gradients(law_parameters, "INTACT law")
