# Prescribed Detached INTACT Direct v13
#
# Three isolated learners: a prediction-only world model, a detached physical
# H12 INTACT law, and a state-dependent intent prescriber optimized through the
# frozen law and world model. Runtime is one deterministic prescriber/law pass.
import math
import os
import random
import time
...
def detached_intact_objective(
    ...
    """Fit physical H12 chunks while treating all world latents as constants."""
    ...
    nll = agent.intact_action_law.nll(
        z_start, local_intent, previous_action, action_chunk
    ).mean()
    mean_chunk = flat_mean.reshape_as(action_chunk)
    metrics = {
        "detached_local_nll": nll.detach(),
        "detached_first_action_mae": (
            mean_chunk[:, 0] - action_chunk[:, 0]
        ).abs().mean().detach(),
        "detached_chunk_mean_mae": (
            mean_chunk - action_chunk
        ).abs().mean().detach(),
        "fixed_std": log_std.exp().mean().detach(),
    }
    return nll, metrics
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "detached INTACT World Direct v13 uses 64-D latents"
    ...
    intact_loss_function = detached_intact_objective
    ...
    if args.compile:
        ...
        print(
            "[prescribed_detached_intact_direct_v13] "
