# EmbOpt v29 POTENTIAL-SHAPING v1. Chassis = the tpoprobe_v1 frontier file, UNCHANGED.
#
# METHOD: potential-based reward shaping (Ng et al. 1999). An independent value
# network Phi (2-layer Tanh MLP on the same normalized observations, its own Adam,
# no parameters shared with the PPO critic) is regressed on the rollout's raw-scale
# bootstrapped lambda-return in SYMLOG coordinates. Between the rollout and the GAE,
# the reward buffer receives, in RAW RETURN UNITS with NO normalization:
#     F_t = gamma * (1 - terminated_t) * Phi(s'_t) - Phi(s_t)
#     rewards_t += shaping_coef * F_t
#
# THE MECHANISM (this is the whole registered claim). Potential shaping leaves every
# GAE advantage EXACTLY invariant: once the critic represents V - c*Phi, the TD
# residual is unchanged term by term, delta'_t = delta_t. So the advantage channel
# contributes exactly ZERO and the sole channel is the CRITIC'S FITTING PROBLEM,
# which changes from V to V - c*Phi. Helps iff that residual is easier to fit than V
# and the value-error reduction improves advantage quality. The primary kill gate is
# early value-fit quality vs the control, NOT returns.
#
# LIVE vs USED Phi. Two objects, and the split is NOT uniform. The live net trains
# every iteration; a FROZEN COPY (`phi_used`) supplies every quantity that reasons
# about WHAT THE CRITIC ABSORBED, because that copy generated the shaping the critic
# was trained on. Refreshed at iteration 2, then every `phi_refresh_every` (=16), so
# refreshes land at 2, 16, 32, 48, ... (iteration 2 rather than 16 because the
# iteration-0 copy is an initialized net and the earliest gate window would
# otherwise score an inert treatment; any Phi is a valid potential, so an early
# refresh costs nothing theoretically). Piecewise-constant Phi => exact invariance
# within each interval, and a fixed timescale for the critic to absorb.
#     shaping term written into rewards ........ USED
#     phi_psi_s / phi_psi_next (de-shaped target) USED
#     TPO probe-score correction ............... USED
#     Psi in the absorption_beta estimator ..... USED
#     the forward pass being optimized ......... LIVE
#
# EXPECTED SIGNATURES — READ BEFORE CALLING ANYTHING A FAILURE.
#   * At c=1.0 with a well-fit Phi the shaped reward IS the one-step TD residual:
#     Phi=V_true => F = gamma*V(s')-V(s) = delta - r, so r_shaped = delta exactly.
#     Therefore shaping/reward_frac -> 1.0 and the STOCK losses/explained_variance
#     -> 0. BOTH ARE SUCCESS SIGNATURES. The critic's target becomes V_true(now) -
#     V_true(last refresh), near zero over 16 iterations; explained variance is a
#     RATIO and normalizes away exactly the absolute error reduction being claimed.
#     Anyone glancing at the dashboard will read the expl-var collapse as
#     catastrophe. It is the design working.
#   * The advantage degenerates gracefully into GAE on the snapshot's TD residuals.
#     That is correct, not a degenerate case.
#   * Near-zero targets sit in the FINEST part of the symlog support, far from the
#     edges, so debug/target_edge_mass should be QUIET. Elevated edge_mass in the
#     shaped arm is a real signal, not noise.
#   * TD-residual RMS spikes at shaping-enable by the same order as the residual
#     itself (the critic still represents unshifted V, so delta' = delta + c*F ~
#     2*delta - r at c=1). Refresh transients are far smaller (Phi moves only by 16
#     iterations of critic improvement). Only PERSISTENT divergence away from
#     refresh boundaries indicates a bug.
#
# THE GATE IS JOINT, AND NATIVE METRICS CANNOT READ IT. Shaping shrinks the target
# variance by orders of magnitude, so native expl-var / value-loss are not
# comparable between arms. Reconstructed metrics score V_hat = V' + c*Phi against
# the UNSHAPED returns, computed identically to the chassis expl-var so the curves
# overlay the control:
#   shaping/critic_residual_rms  PRIMARY. RMS(returns - values) in return units;
#       unit-comparable across arms because the potential shifts target and
#       prediction alike. Mechanism present iff the shaped arm sits BELOW the
#       control's derived curve returns_std*sqrt(1 - explained_variance).
#   shaping/expl_var_reconstructed, shaping/expl_var_phi, and the control's stock
#       expl-var are THREE curves. Two cannot separate "critic adds value on top of
#       the snapshot" from "critic fit noise and the snapshot alone was fine" —
#       both show V_hat tracking the control. V_hat > Phi and > control = mechanism
#       present; V_hat ~ Phi = the critic contributed nothing. Plausible cause if
#       that fires: shrinking the target shrinks the SIGNAL toward zero while the
#       TD-target sampling-noise floor stays put, so SNR worsens even as the target
#       gets smaller, and the residual — a difference of two strongly correlated
#       functions — cancels the shared smooth structure and keeps the rough part.
#       Smaller can be harder.
#   shaping/absorption_beta  beta = -Cov(values - R_unshaped, Psi)/Var(Psi). The
#       critic represents V_true - beta*Psi during adaptation. EVERY approximate
#       correction here is exact only at beta=1: the probe correction leaves a
#       candidate-VARYING residual (1-beta)*c*gamma*Phi(s'_cand), and the GAE leaves
#       (1-beta)*F in the advantage, which is unlabeled ADVANTAGE shaping — the
#       rejected design. Both residuals push toward high-Phi states, so the compound
#       failure mode MIMICS improvement. Early reconstructed gains count as
#       mechanism evidence ONLY if beta ~ 1 in the same window. beta > 1 =
#       over-absorption, report it. The estimator is clean to first order: Phi is a
#       frozen snapshot from earlier iterations and cannot correlate with the
#       current rollout's return sampling noise.
#
# TRAPS HONORED:
#   1. Phi(terminal) = 0, via the TERMINATIONS-ONLY mask (transition_terminations),
#      never the merged next_done / transition_boundaries. THE SEVERE ONE, and it
#      lives at TWO sites (GAE and probe) — HalfCheetah never terminates, so a wrong
#      mask is invisible on the tuning env and only surfaces cross-env.
#   2. Truncation != termination: truncated steps bootstrap through Phi(final_obs),
#      reusing the chassis's own next_transition_obses buffer.
#   3. gamma is exactly args.gamma at every site; no second discount.
#   (The old traps 4 and 5 — F normalization and NormalizeReward ordering — are moot
#   on this chassis: no running normalization of any kind, and normalize_reward is
#   False and asserted off whenever tpo_coef>0.) A FIXED scale factor would have been
#   harmless (c*F/k is shaping with potential Phi/k); what is banned is a TIME-VARYING
#   scale, which breaks the telescoping because consecutive steps scale differently
#   and no single potential generates the terms.
#
# CONSISTENCY SITES THE SHAPING FORCES ON THE CHASSIS (all guarded, all no-ops at
# shaping_coef=0). With the critic representing V' = V - c*Phi:
#   a. Both GAE loops (reward GAE and the dormant soft-advantage GAE) are covered by
#      construction — shaping is a SINGLE in-place mutation of the rewards buffer
#      before the GAE section, so every consumer reads shaped rewards automatically.
#   b. The TPO probe is NOT covered: it generates candidate rewards fresh from
#      physics, so the buffer mutation never reaches them. An unshaped candidate
#      score r_cand + gamma*(1-term)*V'(s'_cand) is corrupted by a candidate-VARYING
#      -c*gamma*Phi(s'_cand). The shaped candidate reward is applied explicitly; the
#      -c*Phi(s) half is candidate-common and cancels in the anchored softmax.
#   c. Phi's own regression target de-shapes the critic (V_true = V' + c*Phi_used)
#      and uses pre-shaping rewards, otherwise Phi chases V - c*Phi_prev. Using the
#      LIVE net here would de-shape by the wrong potential, an error of exactly
#      c*(Phi_live - Phi_used) — i.e. shaping/phi_drift would be reading its own cause.
#
# BIT-IDENTITY at shaping_coef=0: same discipline as the chassis's own tpo_coef=0
# guarantee (line ~239) — construction may run, every USE is guarded. Phi's
# construction additionally runs inside torch.random.fork_rng(devices=[device]) and
# its minibatch permutations come from a dedicated torch.Generator, so no global
# CPU/CUDA/numpy stream is touched. Verified empirically, not by inspection.
#
# --- inherited base notes ---
# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return idx-transfer + TPO-PROBE v1.
#
# METHOD: paper-faithful per-state dense TPO via oracle sim probes, AUXILIARY to
# PPO. At every probed rollout state, K=8 candidates are drawn from the rollout
# policy (candidate 0 = the executed action). Each candidate is stepped one tick
# through the RAW MuJoCo physics (sim state direct-assign restored afterwards, so
# the real trajectory is bit-identical to an unprobed run) and scored with the
# FROZEN pre-update critic: s_k = r_k + gamma * (1 - term) * V_old(s'_k). Scores
# are z-scored WITHIN the group with a sigma floor (0.5x the running EMA of the
# one-step TD-residual RMS), the target q = softmax(log p_old + u/eta) shifts
# mass toward better candidates around the rollout-policy anchor, and the actor
# adds tpo_coef * CE(q, K-restricted softmax of new logprobs) beside the
# untouched PPO surrogate. tpo_coef=0 => bit-identical to the base file.
#
# WHY THE PRIOR FAILURES ARE ADDRESSED:
#   - v93 scored with the LIVE critic and hand-rebuilt wrapper normalization
#     (its set_state restore and reward normalization were wrong) -> here: frozen
#     rollout critic, RAW probe rewards (this base is raw-return), frozen
#     NormalizeObservation stats, no mj_forward / no MujocoEnv.set_state.
#   - v94 myopia (pure one-step reward score) -> the gamma*V_old(s') bootstrap
#     restores long-horizon credit.
#   - pointwise-noise learned scorers -> oracle physics scores + within-group
#     redistribution: only RELATIVE candidate quality at the same state matters.
#   - eta calibrated to the trust region: measured group KL(q||p_old) ~= 0.012 at
#     eta=6 for the 6-dim Beta with K=8 (inside the target_kl=0.03 leash).
#   - perpetual unit-variance renormalization would push full-strength updates on
#     noise-level score spreads and collapse entropy -> the TD-residual-RMS sigma
#     floor sends u ~= 0 (q -> anchor) when the group spread is below noise scale.
#
# KILL CRITERIA: debug/tpo_floored_frac > 0.8 after 1M steps; debug/
# epochs_completed pinned <= 2; entropy freefall vs the base run.
#
# --- inherited base notes ---
...
class Args:
    ...
    # TPO-probe: per-state-candidate Target Policy Optimization with oracle
    # one-step sim-probe scores, auxiliary to the PPO surrogate.
    tpo_coef: float = 1.0        # weight of the TPO CE in the actor loss; 0 => exact base behavior
    tpo_eta: float = 6.0         # anchor temperature; measured group KL(q||p_old) ~= 0.012 at eta=6 (6-dim Beta, K=8)
    tpo_k: int = 8               # candidates per state (ALL probed, incl. the executed action as candidate 0)
    tpo_state_frac: float = 1.0  # fraction of (env, step) cells probed (per-cell Bernoulli via a dedicated rng)
    tpo_score_floor_coef: float = 0.5  # sigma floor = coef * running RMS of the one-step TD residual
    tpo_pure: bool = False       # ablation: drop pg_loss from the actor backward (requires tpo_coef>0)
    tpo_uniform_anchor: bool = False  # ablation: q = softmax(u/eta) instead of anchored on p_old

    # v29: potential-based reward shaping with an INDEPENDENT raw-obs value MLP.
    # c in rewards += c * (gamma*(1-term)*Phi(s') - Phi(s)), RAW return units, no
    # normalization. 0.0 => the executed op sequence is identical to the chassis.
    shaping_coef: float = 1.0
    # The Phi USED for shaping is a frozen copy of the live net, refreshed at
    # iteration 2 and then every phi_refresh_every iterations (2, 16, 32, ...).
    phi_refresh_every: int = 16
    phi_lr: float = 1e-3
    phi_epochs: int = 4
    phi_minibatches: int = 8
    phi_hidden: int = 256
...
class PotentialNet(nn.Module):
    """Independent potential Phi(s): plain Tanh MLP on the normalized observation.

    Shares NO parameters with the PPO critic, so its value surface is an
    independent estimate rather than a re-read of the critic's. Orthogonal init
    matches the chassis's other MLPs; the output layer uses the std=1.0 value-head
    convention.

    SYMLOG PARAMETRIZATION. `forward` emits a SYMLOG coordinate and `value` returns
    symexp of it, so the net regresses symlog(raw return) and Phi lands in raw
    return units. This is a FIXED invertible transform — no running statistics — and
    a fixed invertible transform of a state function is still a valid potential. It
    is what lets a plain MSE head at lr=1e-3 cover raw MuJoCo returns of O(1e2-1e3),
    which it could not traverse in 244 iterations regressing raw scale directly.

    NOTE the decode differs from the chassis critic's on purpose: V uses
    E[symexp(bin)] over a categorical head (HLGaussSupport.to_expected_scalar),
    Phi uses symexp(scalar) because it has no distribution to average over. Both
    land in raw return units, which is all the shaping algebra needs; Phi is not
    required to equal V.
    """
    ...
    def forward(self, x):
        # Symlog coordinate (the regression space).
        return self.net(x).squeeze(-1)

    def value(self, x):
        # Phi(s) in raw return units.
        return symexp(self.forward(x))
...
if __name__ == "__main__":
    ...
    assert args.tpo_coef >= 0.0, "tpo_coef must be non-negative"
    assert args.tpo_k >= 2, "TPO needs at least two candidates per group"
    assert 0.0 < args.tpo_state_frac <= 1.0, "tpo_state_frac must be in (0, 1]"
    assert not (args.tpo_pure and args.tpo_coef == 0.0), "tpo_pure requires tpo_coef > 0"
    assert args.phi_epochs >= 1 and args.phi_minibatches >= 1, "phi_epochs/phi_minibatches must be >= 1"
    # Probe rewards are RAW physics rewards; the critic must live in the same units.
    assert not (args.tpo_coef > 0.0 and args.normalize_reward), "TPO probe scores require raw rewards"
    ...
    # TPO-probe machinery. EVERYTHING here is gated on tpo_coef > 0 so a
    # tpo_coef=0 run executes exactly the base code path (bit-identical).
    tpo_enabled = args.tpo_coef > 0.0
    if tpo_enabled:
        # Dedicated probe-mask rng: NEVER the global np.random (the update loop's
        # minibatch shuffle consumes that stream).
    ...
    # --- v29 potential-based shaping machinery -------------------------------
    # Construction is unconditional but RNG-isolated: fork_rng means Phi's
    # orthogonal init consumes NONE of the global CPU stream, so a
    # shaping_coef=0 run draws exactly the same numbers as the chassis. Every
    # USE of phi_net below is additionally guarded by `shaping_enabled`.
    shaping_enabled = args.shaping_coef != 0.0
    phi_obs_dim = int(np.array(envs.single_observation_space.shape).prod())
    # NOTE: devices=[device] is REQUIRED — torch.manual_seed also reseeds every
    # CUDA generator, so forking the CPU state alone would permanently clobber the
    # CUDA stream that the rollout's dist.sample() draws from.
    with torch.random.fork_rng(devices=[device]):
        torch.manual_seed(args.seed + 7_000_011)
        phi_net = PotentialNet(phi_obs_dim, args.phi_hidden)
    phi_net = phi_net.to(device)
    # phi_used: the FROZEN copy that supplies the potential actually applied. See
    # the LIVE/USED table in the header — everything that reasons about what the
    # critic absorbed reads this, only the optimized forward reads phi_net.
    phi_used = copy.deepcopy(phi_net)
    for p_ in phi_used.parameters():
        p_.requires_grad_(False)
    phi_snapshot_iter = 0
    phi_optimizer = optim.Adam(phi_net.parameters(), lr=args.phi_lr, eps=1e-5)
    # Dedicated permutation stream for Phi's minibatches: NEVER np.random (the PPO
    # update loop's shuffle owns that) and never the global torch stream.
    phi_generator = torch.Generator()
    phi_generator.manual_seed(args.seed + 7_000_017)
    phi_minibatch_size = max(1, args.batch_size // args.phi_minibatches)
    # Spread of the TPO probe-score correction relative to the group spread it is
    # capped against; only written when TPO probing is also on.
    tpo_shaping_corr_ratio = 0.0
    phi_loss_val = 0.0
    phi_drift_val = 0.0
    ...
    if tpo_enabled:
        ...
        tpo_mask_np = np.zeros((args.num_steps, args.num_envs), dtype=bool)
        tpo_zs = torch.zeros(
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if tpo_enabled:
                # --- TPO probe: K candidates per masked env, one raw physics step each ---
                probe_start = time.time()
                step_mask = probe_rng.random(args.num_envs) < args.tpo_state_frac
                tpo_mask_np[step] = step_mask
                with torch.no_grad():
        ...
        # --- v29: potential-based reward shaping. A SINGLE in-place mutation of the
        # rewards buffer, placed after the rollout loop and before the GAE section,
        # so BOTH GAE loops (the reward GAE and the dormant soft-advantage GAE) read
        # shaped rewards automatically — consistency by construction rather than by
        # remembering to patch two recursions. The potential is phi_used (frozen,
        # piecewise-constant between refreshes), never the live net.
        if shaping_enabled:
            with torch.no_grad():
                # Phi's regression target must be the UNSHAPED return, so stash the
                # pre-shaping rewards. Reusing the chassis's post-GAE `returns`
                # (raw-scale, = advantages + values) would make Phi regress on a
                # return that already contains c*F, i.e. Phi chasing V - c*Phi_old.
                # This copy also feeds the reconstructed gate metrics.
                phi_target_rewards = rewards.clone()
                # Age of the copy AS USED for this iteration's shaping. Captured here,
                # not at logging time: the refresh happens later in the same iteration
                # and would otherwise make refresh iterations report age 0 for a
                # potential that was actually phi_refresh_every iterations old.
                phi_used_age = iteration - phi_snapshot_iter
                phi_s = phi_used.value(obs.reshape(-1, phi_obs_dim)).reshape(args.num_steps, args.num_envs)
                phi_next = phi_used.value(
                    next_transition_obses.reshape(-1, phi_obs_dim)
                ).reshape(args.num_steps, args.num_envs)
                # TERMINATIONS-ONLY mask (Phi(terminal) = 0). transition_valids
                # mirrors the chassis GAE's own bootstrap gate exactly: it drops the
                # bootstrap on boundary steps where no final_observation was
                # available, where next_transition_obses holds the RESET obs.
                shaping_bootstrap = (1.0 - transition_terminations) * transition_valids
                shaping_f = args.gamma * shaping_bootstrap * phi_next - phi_s
                shaping_term = args.shaping_coef * shaping_f
                # Psi = c * Phi_used exactly: the offset the critic is being asked to
                # absorb. No scale factor of any kind — a time-varying one would
                # break the telescoping (see the header).
                phi_psi_s = args.shaping_coef * phi_s
                phi_psi_next = args.shaping_coef * phi_next
                shaping_reward_frac = (
                    shaping_term.abs().mean() / (rewards.abs().mean() + 1e-8)
                ).item()
                shaping_f_mean = shaping_f.mean().item()
                shaping_f_std_log = shaping_f.std().item()
                rewards += shaping_term
        ...
        with torch.no_grad():
            ...
            if tpo_enabled:
                # --- TPO scoring (frozen pre-update critic; targets fixed across epochs) ---
                # Running one-step TD-residual RMS over EXECUTED transitions -> sigma floor.
                td_resid = (
                    rewards
                ...
                tpo_sigma_floor = args.tpo_score_floor_coef * td_rms_ema

                flat_mask_np = tpo_mask_np.reshape(-1)
                b_tpo_mask = torch.as_tensor(flat_mask_np, device=device)
                b_tpo_zs = tpo_zs.reshape((-1, args.tpo_k) + envs.single_action_space.shape)
                b_tpo_q = torch.full((args.batch_size, args.tpo_k), 1.0 / args.tpo_k, device=device)
                num_probed = int(flat_mask_np.sum())
                tpo_group_kl = tpo_q_entropy = tpo_score_std_mean = tpo_floored_frac = 0.0
                tpo_shaping_corr_ratio = 0.0  # reset with its siblings (num_probed==0 => 0, not stale)
                if num_probed > 0:
                    obs_dim = int(np.array(envs.single_observation_space.shape).prod())
                    flat_probe_obs = torch.as_tensor(
                        tpo_next_obs_np.reshape(-1, args.tpo_k, obs_dim)[flat_mask_np], device=device
                    ).reshape(-1, obs_dim)
                    ...
                    ).reshape(num_probed, args.tpo_k)
                    r_probe = torch.as_tensor(tpo_rewards_np.reshape(-1, args.tpo_k)[flat_mask_np], device=device)
                    term_probe = torch.as_tensor(tpo_terms_np.reshape(-1, args.tpo_k)[flat_mask_np], device=device)
                    # Oracle score: raw probe reward + bootstrapped frozen value.
                    scores = r_probe + args.gamma * (1.0 - term_probe) * v_next      # (P, K)
                    if shaping_enabled:
                        # v29 PROBE-SITE SHAPING. The rewards-buffer mutation cannot
                        # reach these: probe candidate rewards are generated fresh
                        # from physics. But v_next comes from a critic that now
                        # represents V' = V_true - Psi, so an unshaped candidate score
                        # is Q_true - c*gamma*(1-term_k)*Phi(s'_k) — a candidate-
                        # VARYING corruption that would make TPO rank candidates
                        # AGAINST exactly the successors the shaping rewards, with no
                        # error raised. Apply the shaped candidate reward
                        #     r'_k = r_k + c*(gamma*(1-term_k)*Phi(s'_k) - Phi(s))
                        # giving shaped_score = true_score - c*Phi(s), a candidate-
                        # COMMON shift that cancels in the anchored softmax.
                        #
                        # Phi(s'_k) is read from flat_probe_obs — the SAME preprocessed
                        # tensor the critic's v_next used. Never re-derive from the raw
                        # probe observation: the obs path has TWO stages (frozen-stat
                        # NormalizeObservation AND a separate clip to [-10,10]) and
                        # re-deriving would likely drop the clip, feeding the net
                        # out-of-distribution inputs on exactly the tail states where
                        # candidate ordering matters most.
                        #
                        # (1-term_probe) is trap 1 at the probe site: Phi(terminal)=0
                        # must hold HERE as well as in the GAE, and HalfCheetah never
                        # terminates so a wrong mask is invisible on the tuning env.
                        phi_probe = torch.cat(
                            [phi_used.value(chunk) for chunk in flat_probe_obs.split(65536)]
                        ).reshape(num_probed, args.tpo_k)
                        # Same mask tensor the chassis uses to gather anchor_logp, so
                        # the P rows line up with phi_probe's row ordering.
                        phi_probe_s = phi_s.reshape(-1)[b_tpo_mask].unsqueeze(-1)   # (P,1), candidate-common
                        score_corr = args.shaping_coef * (
                            args.gamma * (1.0 - term_probe) * phi_probe - phi_probe_s
                        )
                        # Centering drops the candidate-common -c*Phi(s) exactly (it
                        # also cancels in the z-score below) and is REQUIRED before the
                        # cap, since clamp is nonlinear in the absolute value.
                        #
                        # CAP at 1x the uncorrected group spread. The correction is
                        # exact only in the ABSORBED limit: while the critic has
                        # absorbed only beta*Psi it leaves a candidate-varying
                        # (1-beta)*c*gamma*Phi(s'_k). Structurally mild here — shaping
                        # is on from iteration 1 (no mid-training enable shock) and Phi
                        # is frozen between refreshes, so the absorption gap is only
                        # c*(Phi_new - Phi_old) per refresh — but unbounded in
                        # principle. Watch shaping/tpo_corr_ratio against
                        # shaping/absorption_beta, and debug/tpo_score_std_mean /
                        # tpo_sigma_floor / tpo_floored_frac vs the control.
                        score_corr = score_corr - score_corr.mean(dim=-1, keepdim=True)
                        corr_cap = scores.std(dim=-1, unbiased=False, keepdim=True)
                        tpo_shaping_corr_ratio = (
                            score_corr.std(dim=-1, unbiased=False) / corr_cap.squeeze(-1).clamp_min(1e-8)
                        ).mean().item()
                        scores = scores + score_corr.clamp(min=-corr_cap, max=corr_cap)
                    # Within-group z-score with the sigma FLOOR: when the group spread is
                    # below noise scale, u ~= 0 and q ~= anchor (no hard zero-var branch).
                    group_std = scores.std(dim=-1, unbiased=False, keepdim=True)     # population std over K
                    u_scores = (
                        (scores - scores.mean(dim=-1, keepdim=True))
                        / group_std.clamp_min(max(tpo_sigma_floor, 1e-8))  # eps: 0/0 guard if EMA is exactly 0
                    ).clamp(-5.0, 5.0)
                    anchor_logp = F.log_softmax(tpo_logprobs.reshape(-1, args.tpo_k)[b_tpo_mask], dim=-1)
                    if args.tpo_uniform_anchor:
                        tpo_q_probed = torch.softmax(u_scores / args.tpo_eta, dim=-1)
                        anchor_for_kl = torch.full_like(anchor_logp, -float(np.log(args.tpo_k)))
                    else:
                        tpo_q_probed = torch.softmax(anchor_logp + u_scores / args.tpo_eta, dim=-1)
                        anchor_for_kl = anchor_logp
                    b_tpo_q[b_tpo_mask] = tpo_q_probed
                    log_q = tpo_q_probed.clamp_min(1e-12).log()
                    tpo_group_kl = (tpo_q_probed * (log_q - anchor_for_kl)).sum(-1).mean().item()
                    tpo_q_entropy = (-(tpo_q_probed * log_q).sum(-1)).mean().item()
                    tpo_score_std_mean = group_std.mean().item()
                    tpo_floored_frac = (group_std < tpo_sigma_floor).float().mean().item()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # TPO CE on the K-restricted softmax, masked-mean over probed states.
                # Targets q are frozen (computed once post-rollout).
                if tpo_enabled:
                    mb_tpo_mask = b_tpo_mask[mb_inds]
                    if bool(mb_tpo_mask.any()):
                        mb_logp_new = F.log_softmax(new_cand_logprobs, dim=-1)
                        tpo_ce = (-(b_tpo_q[mb_inds] * mb_logp_new).sum(-1))[mb_tpo_mask].mean()
                    else:
                        tpo_ce = torch.zeros((), device=device)
                    if args.tpo_pure:
                        actor_loss = args.tpo_coef * tpo_ce
                    else:
                        actor_loss = pg_loss + args.tpo_coef * tpo_ce
                else:
                    actor_loss = pg_loss
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    # The TPO term rides the actor backward, clipped under actor_grad_clip.
        ...
        # --- v29: train Phi, AFTER the PPO update (so the shaping applied above
        # used a fully frozen, previous-iteration potential).
        if shaping_enabled:
            with torch.no_grad():
                # Re-run the chassis's GAE recursion verbatim, but on the UNSHAPED
                # rewards AND on DE-SHAPED critic values, to recover the raw-scale
                # bootstrapped lambda-return of the TRUE reward.
                #   - unshaped rewards: reusing the chassis's `returns` would feed
                #     back this iteration's own lambda*F into Phi's target.
                #   - de-shaped values: the critic regresses the shaped return, so
                #     V_true = V_shaped + Psi (Ng's identity, exact for a converged
                #     critic). Without this the bootstrap alone reintroduces the
                #     loop Phi <- V - c*Phi_prev.
                # The residual inexactness is the critic's own error plus partial
                # absorption (shaping/absorption_beta measures exactly that).
                #
                # HIGHEST-RISK LIVE/USED SITE: phi_psi_* MUST come from phi_used, the
                # copy that generated the shaping the critic actually trained on.
                # De-shaping by the LIVE potential would be wrong by exactly
                # c*(Phi_live - Phi_used) — shaping/phi_drift would be reading its own
                # cause. Only phi_pred below uses the live net.
                phi_v = values + phi_psi_s
                phi_v_next = next_transition_values + phi_psi_next
                phi_adv = torch.zeros_like(phi_target_rewards)
                lastgaelam = 0
                for t in reversed(range(args.num_steps)):
                    bootstrap_nonterminal = (1.0 - transition_terminations[t]) * transition_valids[t]
                    lambda_nonterminal = 1.0 - transition_boundaries[t]
                    delta = (
                        phi_target_rewards[t]
                        + args.gamma * phi_v_next[t] * bootstrap_nonterminal
                        - phi_v[t]
                    )
                    phi_adv[t] = lastgaelam = (
                        delta + args.gamma * args.gae_lambda * lambda_nonterminal * lastgaelam
                    )
                # R_unshaped: the raw-return estimate. Phi's regression target AND the
                # denominator of every reconstructed gate metric below.
                phi_raw_targets = (phi_adv + phi_v).reshape(-1)
                # FIXED symlog parametrization — no running statistics of any kind.
                # The net regresses symlog(raw return) and Phi = symexp(net), the same
                # treatment as the chassis's value_symlog, which is what lets a plain
                # MSE head at lr=1e-3 cover raw returns of O(1e2-1e3).
                phi_targets = symlog(phi_raw_targets)

                # --- GATE INSTRUMENTS. All scored against the UNSHAPED returns and
                # computed with the same numpy expression as the chassis's own
                # explained variance, so the curves overlay the control run's.
                psi_flat = phi_psi_s.reshape(-1)                 # USED-copy Psi
                v_hat = phi_v.reshape(-1)                        # V' + c*Phi, raw units
                r_unshaped_np = phi_raw_targets.cpu().numpy()
                var_unshaped = np.var(r_unshaped_np)
                v_hat_np = v_hat.cpu().numpy()
                phi_s_np = phi_s.reshape(-1).cpu().numpy()
                shaping_expl_var_recon = (
                    np.nan if var_unshaped == 0
                    else 1 - np.var(r_unshaped_np - v_hat_np) / var_unshaped
                )
                shaping_expl_var_phi = (
                    np.nan if var_unshaped == 0
                    else 1 - np.var(r_unshaped_np - phi_s_np) / var_unshaped
                )
                shaping_value_loss_recon = float(np.mean((r_unshaped_np - v_hat_np) ** 2))
                # absorption_beta: the critic represents V_true - beta*Psi during
                # adaptation, so (values - R_unshaped) ~ -beta*Psi and
                #   beta = -Cov(values - R_unshaped, Psi) / Var(Psi).
                # Clean to first order: Psi comes from a snapshot taken in earlier
                # iterations, so it cannot correlate with THIS rollout's return noise.
                psi_var = psi_flat.var(unbiased=False)
                resid_v = b_values - phi_raw_targets
                shaping_absorption_beta = float(
                    (
                        -((resid_v - resid_v.mean()) * (psi_flat - psi_flat.mean())).mean()
                        / psi_var.clamp_min(1e-8)
                    ).item()
                )
                # PRIMARY: absolute fit residual in return units. Unit-comparable
                # across arms (the potential shifts target and prediction alike),
                # unlike explained variance, which is a ratio and normalizes away the
                # absolute error reduction that is the entire claim. The control's
                # comparable curve is returns_std * sqrt(1 - explained_variance).
                shaping_critic_residual_rms = float(
                    (b_returns - b_values).pow(2).mean().sqrt().item()
                )
                # f_std_batch is identical to f_std now that no normalization exists
                # (shaping_f IS raw F); kept under the registered tag name. phi_value_std
                # is the other half of the derived roughness ratio std(F)/std(Phi).
                shaping_f_std_batch = shaping_f_std_log
                shaping_phi_value_std = float(phi_s.std().item())

            phi_loss_sum, phi_loss_n = 0.0, 0
            for _ in range(args.phi_epochs):
                # Dedicated generator: consumes NO global torch/numpy randomness.
                phi_perm = torch.randperm(args.batch_size, generator=phi_generator)
                for phi_start in range(0, args.batch_size, phi_minibatch_size):
                    phi_inds = phi_perm[phi_start : phi_start + phi_minibatch_size].to(device)
                    phi_pred = phi_net(b_obs[phi_inds])            # LIVE net, symlog space
                    phi_loss = F.mse_loss(phi_pred, phi_targets[phi_inds])
                    phi_optimizer.zero_grad(set_to_none=True)
                    phi_loss.backward()
                    nn.utils.clip_grad_norm_(phi_net.parameters(), 10.0)
                    phi_optimizer.step()
                    phi_loss_sum += phi_loss.item()
                    phi_loss_n += 1
            phi_loss_val = phi_loss_sum / max(phi_loss_n, 1)

            # --- Refresh the USED copy. Schedule: iteration 2, then every
            # phi_refresh_every (2, 16, 32, 48, ...). Placed AFTER training so
            # "refresh at iteration 2" means the copy has had 2 iterations of
            # gradient steps; it takes effect for the NEXT iteration's shaping.
            phi_refresh_now = iteration == 2 or iteration % args.phi_refresh_every == 0
            if phi_refresh_now:
                with torch.no_grad():
                    # Drift is measured BEFORE the overwrite: how far the live net has
                    # moved from the potential the critic has been absorbing.
                    phi_drift_val = (
                        (phi_net.value(b_obs) - phi_used.value(b_obs)).abs().mean().item()
                    )
                    phi_used.load_state_dict(phi_net.state_dict())
                phi_snapshot_iter = iteration
        ...
        if tpo_enabled:
            ...
            writer.add_scalar("debug/tpo_floored_frac", tpo_floored_frac, global_step)
            writer.add_scalar("debug/tpo_sigma_floor", tpo_sigma_floor, global_step)
            ...
            # Per-iteration TD-residual RMS. The chassis only exposes the EMA
            # indirectly (debug/tpo_sigma_floor = tpo_score_floor_coef * td_rms_ema);
            # the invariance self-test needs the raw per-iteration value. Logged
            # unconditionally so the shaping_coef=0 control arm carries it too — a
            # pure logging addition, no computation and no RNG.
            writer.add_scalar("debug/td_rms", td_rms, global_step)
        if shaping_enabled:
            writer.add_scalar("shaping/phi_loss", phi_loss_val, global_step)
            writer.add_scalar("shaping/f_mean", shaping_f_mean, global_step)
            writer.add_scalar("shaping/f_std", shaping_f_std_log, global_step)
            writer.add_scalar("shaping/f_std_batch", shaping_f_std_batch, global_step)
            writer.add_scalar("shaping/phi_value_std", shaping_phi_value_std, global_step)
            writer.add_scalar("shaping/reward_frac", shaping_reward_frac, global_step)
            writer.add_scalar("shaping/tpo_corr_ratio", tpo_shaping_corr_ratio, global_step)
            # Snapshot bookkeeping: age, the refresh marker (so refresh transients in
            # debug/td_rms are identifiable post-hoc), and how far the live net had
            # drifted at the moment of the last refresh.
            writer.add_scalar("shaping/phi_snapshot_age_iters", phi_used_age, global_step)
            writer.add_scalar("shaping/phi_refresh", 1.0 if phi_refresh_now else 0.0, global_step)
            writer.add_scalar("shaping/phi_drift", phi_drift_val, global_step)
            # THE GATE. See the header: native expl-var is uninterpretable in the
            # regime this design creates, and the read is JOINT with absorption_beta.
            writer.add_scalar("shaping/critic_residual_rms", shaping_critic_residual_rms, global_step)
            writer.add_scalar("shaping/expl_var_reconstructed", shaping_expl_var_recon, global_step)
            writer.add_scalar("shaping/expl_var_phi", shaping_expl_var_phi, global_step)
            writer.add_scalar("shaping/value_loss_reconstructed", shaping_value_loss_recon, global_step)
            writer.add_scalar("shaping/absorption_beta", shaping_absorption_beta, global_step)
