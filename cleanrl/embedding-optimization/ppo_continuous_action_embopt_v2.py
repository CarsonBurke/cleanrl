# Embedding Optimization v2 (embopt_v2) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v1 -> v2: fixes the measured v1 failure (1.3M autopsy): the free-gradient-ascent proposer
# collapsed to a near-zero constant goal (proposer_goal_sqnorm ~0.5 vs on-manifold ||z||^2 ~ 64;
# proposed_goal_sensitivity ~1e-4) because the 0.5*||g||^2 mode-seeking prior overpowers W_hat
# ascent — and the origin of a whitened space is the mean replay state, i.e. "stand still".
# The policy obeyed it faithfully (goal channel measured ALIVE: goal_removal ~0.04, grad terms
# balanced), so returns tracked the bad goal, not a broken mechanism.
# v2 proposer is the literal reading of the family spec: REGRESS toward the embedding
# maximizing the value-based score — per-state argmax over a pool of real replay embeddings
# scored by actual windowed reward rate + alpha_reach * V(z, cand). On-manifold by
# construction, state-dependent through V, no norm prior, collapse-proof.
# Also adds goal-reaching telemetry: V(z_t, commanded g) mean and its per-step climb.
#
...
class Args:
    ...
    """weight of the V(z,cand) reachability term in the candidate score"""
    cand_pool: int = 256
    """replay embeddings scored as goal candidates per iteration"""
    proposer_minibatch: int = 512
    """proposer minibatch size (B x cand_pool value evals per update)"""
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.proposer = mlp(dz, dh, dz, out_std=0.01)
...
if __name__ == "__main__":
    ...
    def proposer_loss_fn(z, g_star):
        g = agent.propose(z)
        loss = (g - g_star).square().mean()
        return loss, g.square().sum(-1).mean().detach()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ---- proposer updates: regress toward the score-maximizing replay embedding ------
        C, Bp = args.cand_pool, args.proposer_minibatch
        cflat = torch.randint(0, n_flat, (C,), device=device)
        with torch.no_grad():
            zc = agent.encode(buf_obs[cflat // (T * E), (cflat // E) % T, cflat % E])  # (C,dz)
            rate_c = buf_rate[cflat // (T * E), (cflat // E) % T, cflat % E] / r_scale  # (C,)
        prop_stats = []
        for _ in range(args.proposer_updates):
            flat = torch.randint(0, n_flat, (Bp,), device=device)
            with torch.no_grad():
                zb = agent.encode(buf_obs[flat // (T * E), (flat // E) % T, flat % E])
                v_zc = agent.value(
                    zb[:, None, :].expand(Bp, C, dz).reshape(-1, dz),
                    zc[None, :, :].expand(Bp, C, dz).reshape(-1, dz),
                ).reshape(Bp, C)
                scores = rate_c[None, :] + args.alpha_reach * v_zc
                best = scores.argmax(1)
                g_star = zc[best]
            mark_step()
            loss, gnorm = proposer_loss_fn(zb, g_star)
            ...
            proposer_opt.step()
            prop_stats.append(
                (loss.item(), rate_c[best].mean().item(), g_star.std(0).mean().item(), gnorm.item())
            )
        ...
        writer.add_scalar("losses/proposer_mse", pr_m[0], global_step)
        writer.add_scalar("diagnostics/chosen_cand_rate", pr_m[1], global_step)
        writer.add_scalar("diagnostics/gstar_dispersion", pr_m[2], global_step)
        writer.add_scalar("diagnostics/proposer_goal_sqnorm", pr_m[3], global_step)
