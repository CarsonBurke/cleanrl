...
# interval velocity from (z_t, a_t), and deterministic projected Adam directly
# maximizes its H1 score minus the environment's analytic control cost. The
# hypothesis is that exact one-step credit avoids long-rollout model error while
# retaining INTACT's controllability bias; INTACT is never used for acting.
import math
import os
import random
import time
...
def forward_delta_target(observations, interval_velocities):
    """Action-controllable interval-velocity residual used by the H1 head."""
...
def interval_velocity_score(agent, z, action, action_cost_coef=0.1):
    """Exact H1 benchmark score predicted from state velocity plus its residual."""

    return agent.predict_interval_velocity(z, action) - (
        action_cost_coef * action.square().sum(dim=-1)
    )


@torch.enable_grad()
def optimize_action_h1(agent, z, warm_start, args):
    """Deterministic projected Adam ascent over one bounded action per state."""

    action = warm_start.detach().clone()
    z = z.detach()
    with torch.no_grad():
        initial_score = interval_velocity_score(
            agent, z, action, args.action_cost_coef
        )
        best_score = initial_score.clone()
        best_action = action.clone()
    first_gradient_norm = None
    first_moment = torch.zeros_like(action)
    second_moment = torch.zeros_like(action)
    for iteration in range(1, args.action_opt_iterations + 1):
        action.requires_grad_(True)
        score = interval_velocity_score(agent, z, action, args.action_cost_coef)
        (gradient,) = torch.autograd.grad(score.sum(), action)
        if first_gradient_norm is None:
            first_gradient_norm = gradient.norm(dim=-1)
        with torch.no_grad():
            improved = score > best_score
            best_score = torch.where(improved, score, best_score)
            best_action = torch.where(improved[:, None], action, best_action)
    ...
    with torch.no_grad():
        candidate_score = interval_velocity_score(
            agent, z, action, args.action_cost_coef
        )
        improved = candidate_score > best_score
        best_score = torch.where(improved, candidate_score, best_score)
        best_action = torch.where(improved[:, None], action, best_action)
        predicted_interval_velocity = agent.predict_interval_velocity(
            z, best_action
        )
    diagnostics = {
        "optimizer_improvement": best_score - initial_score,
        "initial_gradient_norm": first_gradient_norm,
        "action_change": (best_action - warm_start).norm(dim=-1),
        "predicted_interval_velocity": predicted_interval_velocity,
        "selected_score": best_score,
    }
    return best_action.detach(), diagnostics
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "INTACT velocity delta v2 uses 64-D latents"
            ...
            if global_step < args.warmup_steps:
                ...
                with torch.no_grad():
                    diagnostics = {
                        ...
                        "action_change": (action - previous_action).norm(dim=-1),
                        "predicted_interval_velocity": (
                            agent.predict_interval_velocity(z, action)
                        ),
                    }
            else:
                action, diagnostics = optimize_action_h1(
                    agent, z, previous_action, args
                )
