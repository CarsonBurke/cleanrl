# Velocity-Conditioned Chunk INTACT Direct v6
#
# A shared INTACT law learns supported H12 action chunks from attached endpoint
# intents and exact achieved-velocity conditions. Deployment deterministically
# decodes the Gaussian mean for a replay-supported velocity command and replans
# every step. The hypothesis is that chunk prediction preserves gait structure
# without action search, terminal values, or post-warmup exploration.
import math
import os
import random
import time
...
class Args:
    ...
    target_velocity_quantile: float = 0.9
    target_velocity_ema: float = 0.99
    target_velocity_min: float = 0.0
    target_velocity_max: float = 8.0
    command_normalizer_ema: float = 0.99
...
def velocity_command_features(current_velocity, commanded_velocity):
    """Detached [current, command, command-current] conditioning labels."""
...
class Agent(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, args):
        ...
        self.velocity_goal_encoder = mlp(
            3, hidden_dim, latent_dim, final_std=0.01, layers=2
        )
        ...
        self.action_dim = action_dim
        self.register_buffer("command_feature_mean", torch.zeros(3))
        self.register_buffer("command_feature_variance", torch.ones(3))
        self.register_buffer(
            "command_normalizer_initialized", torch.tensor(False)
        )
    ...
    def encode_velocity_goal(self, current_velocity, commanded_velocity):
        features = velocity_command_features(current_velocity, commanded_velocity)
        normalized = (features - self.command_feature_mean) / (
            self.command_feature_variance + 1e-4
        ).sqrt()
        return self.velocity_goal_encoder(normalized)

    @torch.no_grad()
    def update_command_normalizer(self, features, ema):
        if features.ndim != 2 or features.shape[-1] != 3:
            raise ValueError("command features must have shape [batch,3]")
        if not 0.0 <= ema < 1.0:
            raise ValueError("normalizer EMA must be in [0,1)")
        batch_mean = features.detach().mean(dim=0)
        batch_variance = features.detach().var(dim=0, unbiased=False)
        if self.command_normalizer_initialized:
            self.command_feature_mean.lerp_(batch_mean, 1.0 - ema)
            self.command_feature_variance.lerp_(batch_variance, 1.0 - ema)
        else:
            self.command_feature_mean.copy_(batch_mean)
            self.command_feature_variance.copy_(batch_variance)
            self.command_normalizer_initialized.fill_(True)
...
def chunk_intact_nll_losses(
    ...
    """Shared chunk likelihood for attached endpoints and achieved velocity."""

    local_intent = z_future - z_start
    goal_condition = agent.encode_velocity_goal(
        current_velocity, achieved_velocity
    )
    ...
    goal_nll = agent.intact_action_law.nll(
        z_start, goal_condition, previous_action, action_chunk
    )
...
def training_objective(agent, sigreg, batch, args):
    ...
    current_velocity = forward_velocity(batch.observations[:, 0])
    achieved_velocity = batch.interval_velocities.mean(dim=1)
    local_intent = z_future - z_start
    goal_condition = agent.encode_velocity_goal(
        current_velocity, achieved_velocity
    )
    ...
    goal_mean, goal_log_std = agent.intact_action_law.parameters_for(
        z_start, goal_condition, previous_action
    )
    ...
    shuffled_goal_condition = goal_condition.roll(1, dims=0)
    shuffled_goal_mean, _ = agent.intact_action_law.parameters_for(
        z_start, shuffled_goal_condition, previous_action
    )
    ...
    metrics = {
        ...
        "achieved_chunk_velocity": achieved_velocity.mean().detach(),
    }
    return total, metrics


class VelocityCommandTracker:
    """Deterministic quantile EMA bounded by currently observed support."""

    def __init__(self, quantile, ema, minimum, maximum, device="cpu"):
        if not 0.0 <= quantile <= 1.0:
            raise ValueError("target quantile must be in [0,1]")
        if not 0.0 <= ema < 1.0:
            raise ValueError("target EMA must be in [0,1)")
        if minimum > maximum:
            raise ValueError("target minimum must not exceed maximum")
        self.quantile = quantile
        self.ema = ema
        self.minimum = minimum
        self.maximum = maximum
        self.target = torch.zeros((), dtype=torch.float32, device=device)
        self.initialized = False

    @torch.no_grad()
    def update(self, achieved_mean_velocities):
        if achieved_mean_velocities.numel() < 1:
            raise ValueError("velocity command update requires observations")
        achieved = achieved_mean_velocities.detach().to(
            device=self.target.device, dtype=self.target.dtype
        )
        observed_maximum = achieved.max().clamp(self.minimum, self.maximum)
        candidate = torch.quantile(achieved, self.quantile).clamp(
            self.minimum, self.maximum
        )
        candidate = torch.minimum(candidate, observed_maximum)
        if self.initialized:
            self.target.lerp_(candidate, 1.0 - self.ema)
            # Downward EMA lag must not leave the command above support after
            # old replay windows have been overwritten.
            self.target.copy_(torch.minimum(self.target, observed_maximum))
        else:
            self.target.copy_(candidate)
            self.initialized = True
        return self.target.clone()

    def batch(self, batch_size, *, device, dtype):
        return self.target.to(device=device, dtype=dtype).expand(batch_size)


@torch.no_grad()
def direct_action_chunk(
    agent, z, current_velocity, target_velocity, previous_action
):
    """Decode the shared Gaussian mean; no search or action sampling."""

    goal_condition = agent.encode_velocity_goal(
        current_velocity, target_velocity
    )
    flat_mean, log_std = agent.intact_action_law.parameters_for(
        z, goal_condition, previous_action
    )
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "chunk INTACT Direct v6 uses 64-D latents"
    ...
    assert 0.0 <= args.target_velocity_quantile <= 1.0
    assert 0.0 <= args.target_velocity_ema < 1.0
    assert args.target_velocity_min <= args.target_velocity_max
    assert 0.0 <= args.command_normalizer_ema < 1.0
    ...
    if args.compile:
        ...
        print(
            "[velocity_chunk_intact_direct_v6] "
    ...
    command_tracker = VelocityCommandTracker(
        args.target_velocity_quantile,
        args.target_velocity_ema,
        args.target_velocity_min,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        rollout_target_velocity = torch.zeros((), device=device)
        rollout_command_gap = torch.zeros((), device=device)
        ...
        for _ in range(args.num_steps):
            ...
            with torch.no_grad():
                ...
                target_velocity = command_tracker.batch(
                    args.num_envs, device=device, dtype=z.dtype
                )
            ...
            rollout_target_velocity += target_velocity.mean()
            rollout_command_gap += (
                target_velocity - realized_interval_velocity
            ).mean()
        ...
        update_metrics = []
        command_feature_samples = []
        achieved_velocity_samples = []
        ...
        if replay.size >= required_replay:
            ...
            for _ in range(args.gradient_updates):
                ...
                batch_current_velocity = forward_velocity(
                    batch.observations[:, 0]
                )
                batch_achieved_velocity = batch.interval_velocities.mean(dim=1)
                command_feature_samples.append(
                    velocity_command_features(
                        batch_current_velocity, batch_achieved_velocity
                    )
                )
                achieved_velocity_samples.append(batch_achieved_velocity.detach())

            # Update normalization from exactly the pooled labels used below,
            # before the goal encoder is trained or deployed with that transform.
            agent.update_command_normalizer(
                torch.cat(command_feature_samples), args.command_normalizer_ema
            )
            ...
            # One statistics update per rollout makes the command EMA
            # independent of optimizer-update count. Pooling all sampled
            # windows also makes high-quantile estimates less minibatch-noisy.
            command_tracker.update(torch.cat(achieved_velocity_samples))
        ...
        writer.add_scalar(
            ...
            rollout_target_velocity / args.num_steps,
            global_step,
        )
        writer.add_scalar(
            "command/gap",
            rollout_command_gap / args.num_steps,
            global_step,
        )
        writer.add_scalar(
