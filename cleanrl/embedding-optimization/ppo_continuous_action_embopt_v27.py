# Embedding Optimization v27 (embopt_v27) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v26 -> v27: EXACT-REDUCTION TARGET — THE FARM-FREE PURSUIT SIGNAL. v26 (pure
# pursuit, projection target) was killed at 4.5M: all four farm-gate signals
# fired (constant goal, claimed progress ~0.7/step vs a ~1.4 TOTAL honest
# budget, flat goal distance, returns -703 << the -149 reward-only backstop).
# Root cause (reviewer analysis): the projection is only the FIRST-ORDER term
# of distance reduction; the second-order excess (perp displacement)^2 / 2D is
# non-negative and does NOT telescope, so closed orbits near the goal collect
# ~||d||^2/(2 D sqrt(dz)) ~ 0.5/step forever — worth ~45x genuine pursuit at
# this latent's step size (per-step ||d||^2 ~ 91 vs pair distance^2 ~ 128; one
# step covers ~2/3 of a typical separation, so the linearization is invalid).
# v27 regresses the EXACT one-step distance reduction instead:
#     tgt = (||g - z|| - ||g - z'||) / sqrt(dz)
#   - TELESCOPES for a FIXED goal: closed loops collect exactly zero and the
#     segment total is bounded by the initial distance (~1.4). Per-step
#     re-proposal COULD re-open a carrot farm (~1.19/step), but the guard is
#     architectural: the proposer's only loss is -what(g) — blind to this
#     target — and it collapses near-constant, under which the sum telescopes
#     exactly. proposer_out_std is therefore SAFETY-CRITICAL telemetry; never
#     reintroduce pursuit-derived terms into the proposer score.
#   - BOUNDED: |tgt| <= ||d||/sqrt(dz) exactly (reverse triangle inequality):
#     no v16 distance inflation either — both known exploits die by construction.
#   - No denominator, so no floor needed; g=z is strictly REPULSIVE (gapn1=0
#     forces tgt = -||g-z'||/sqrt(dz) < 0 — you can only move away).
# diagnostics/prog_farm_rate logs (first-order projection - exact) — the
# measured farm rate of the OLD target, expected ~0.5 early; and
# rollout_goal_prog is now in exact-reduction units (comparable to nothing
# before it). READ AGAINST THE RANDOM BASELINE, NOT ZERO: random motion scores
# sqrt(rollout_goal_dist) - sqrt(rollout_goal_dist + wm_delta_var) — recompute
# from the contemporaneous pair at every read (scales with latent step size;
# ~-0.44 only at dist 2.0/dvar 1.42); ~0 = parked at closest approach; >0
# sustained requires falling dist. Everything else is v26 unchanged: one loss per module, belief
# what(z) = backward-window efficiency, proposer = max what(g), policy = max
# qprog. This is the CLEAN goal-only ablation arm (v22 both-channels +584;
# v22_vc0 reward-only -149; v26's arm was confounded by the farmable target).
# --- v26 header follows ---
...
# the pursuit loop is miscalibrated, nothing else catches the fall.
# Watch: goal_removal_action_mse (goal conditioning must be alive), pursuit
# metrics in exact-reduction units, and returns vs v25 (the blended-objective
# reference on the same chassis: final +682.6, family best).
# --- v25 header follows ---
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # regression on measured exact one-step distance reduction toward g
    ...
    def qprog(self, z, a, g):
        # predicted EXACT one-step distance reduction toward g — (||g-z||-||g-z'||)
        # per sqrt(dz) — as a function of the action taken NOW: a regression
        # surface of measured outcomes. The policy ascends dq/da; no dynamics sit
        # between the action and the goal credit, and the target telescopes, so
        # neither far goals (v16) nor orbiting near goals (v26) buy anything
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(f"[embopt_v27] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # -> realized EXACT distance reduction toward the goal over the step. Goals vary
        ...
        with torch.no_grad():
            ...
            # EXACT-REDUCTION target: (||g-z|| - ||g-z'||) / sqrt(dz). Telescopes
            # (closed loops collect zero -> no perpendicular-motion farm, the
            # exploit that killed v26) and is bounded by ||d||/sqrt(dz) (no v16
            # distance inflation). The true measured approach, nothing else.
            disp = Zs[:, wend_idx] - Zs[:, :T]  # (K,T,E,dz)
            gap = g_pool - Zs[:, :T]  # (K,T,E,dz)
            gapn1 = gap.norm(dim=-1)  # (K,T,E)
            gapn2 = (g_pool - Zs[:, wend_idx]).norm(dim=-1)  # (K,T,E)
            tgt_pool = (gapn1 - gapn2) / dz**0.5  # (K,T,E)
            # farm-rate diagnostic: the OLD first-order projection minus the exact
            # reduction = the non-telescoping excess the old target paid per step
            proj_pool = (gap * disp).sum(-1) / (gapn1.clamp_min(0.1 * dz**0.5) * (dz**0.5))
            prog_farm_rate = (
                ((proj_pool[0] - tgt_pool[0]) * m_pool[0]).sum() / m_pool[0].sum().clamp_min(1.0)
            ).item()
        ...
        with torch.no_grad():
            ...
            # no-change baseline for wm_pred: per-dim E[(z'-z)^2] — the raw UNCENTERED
            # second moment, deliberately (coherent drift included): it is the ||d||^2/dz
            # the prog_random_baseline formula needs. If wm_pred is not well below this,
            # the dynamics has learned nothing beyond identity
            dstep = Zf[1:] - Zf[:-1]
        ...
        writer.add_scalar("diagnostics/prog_farm_rate", prog_farm_rate, global_step)
