# Embedding Optimization v28_dirf (embopt_v28_dirf) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v28_kinf -> v28_dirf: PROBE ARM C2-NEUTRALIZED — PURE CONSTANT-DIRECTION
# TARGET ON THE FROZEN-GOAL CHASSIS. The qprog target is u.(z'-z) with u ONE
# fixed random unit latent direction: LINEAR, zero magnitude content, and it
# TELESCOPES (sum over a closed loop = u.(z_end - z_start) = 0; a steady gait
# is periodic in egocentric obs, hence in latent) — orbit-proof like dd, so
# read it in the gradient frame: realized target mean should sit ~0; a
# SUSTAINED positive mean = net latent drift along u = the policy stopped
# cycling — treat as an ANOMALY signal, not success. Scale: d(u.d)/dd norm =
# 1.0; the kinf twin runs at --value-coef 3.4 to match (its 2||d||/dz ~ 0.30).
# kinf@3.4 vs dirf@1.0 is the CLEAN C1-vs-C2 discrimination (matched gradient,
# matched chassis); the live pair 745/746 is scale-confounded (~3.4x) and
# drift-leaky — deprecated for mutual comparison. Reads against v28_fixg
# (anchored pull + damping) to separate pure-direction from point-anchor.
# --- v28_fixg header follows ---
# v22 -> v28_fixg: PROBE ARM D — FROZEN RANDOM GOAL, APPARATUS REMOVED
# (reviewer design; the highest-information arm). v22's final telemetry:
# proposer_out_std 0.0025 (goal effectively constant), rollout_goal_prog
# -0.002 (realized pursuit zero) — the goal channel COLLECTED nothing across
# 8M and still moved -149 -> +584. The dd gradient is d(dd)/dd = 2(g-z')/dz:
# a persistent latent-direction pull with magnitude ~D plus velocity damping
# -2d/dz — a regularizer/symmetry-breaker, not a goal. This arm tests whether
# that is ALL the apparatus was: dd target UNCHANGED, value_coef 1.0
# UNCHANGED, but g = ONE random shell point frozen at init; proposer updates
# skipped (proposer_updates=0; with g constant, [z,g,g-z] reparameterizes z,
# so policy/critic are effectively unconditioned). If this reproduces ~+584,
# the proposer/belief/efficiency machinery from v20 forward was decorative
# and v29 builds on "reward heads + constant latent-direction bias". If
# ~-149, goal CONTENT matters and the aspiration line is alive.
# --- v22 header follows ---
...
if __name__ == "__main__":
    ...
    # ONE random shell point, frozen for the whole run — the entire goal channel
    g_fix = torch.randn(args.latent_dim, device=device)
    g_fix = (args.latent_dim**0.5) * g_fix / g_fix.norm()
    # ONE fixed random unit direction for the C2 target u.(z'-z)
    u_dir = torch.randn(args.latent_dim, device=device)
    ...
    if args.compile:
        ...
        print(f"[embopt_v28_dirf] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # PURE CONSTANT-DIRECTION target: u.(z'-z), linear, no magnitude
            # content; telescopes over closed loops (see header anomaly rule)
            tgt_pool = ((Zs[:, wend_idx] - Zs[:, :T]) * u_dir).sum(-1)  # (K,T,E)
        ...
        with torch.no_grad():
            ...
            # control-cost term of the reward (0.1*sum a^2): separates passive-park
            # (~0) from futile-push (~|return|/1000 when velocity ~0) directly
            ctrl_cost = (0.1 * buf_act[fresh_slot].square().sum(-1)).mean().item()
            # alignment measurement: correlation of u_dir displacement with env
            # reward. The encoder keeps training, so the physical meaning of the
            # fixed latent direction can rotate — a sign change here over training
            # is direct evidence of an alignment flip (vs structural direction value)
            mgf = m_pool[0].bool()
            dp_c = tgt_pool[0][mgf] - tgt_pool[0][mgf].mean()
            rw_c = buf_rew[fresh_slot][mgf] - buf_rew[fresh_slot][mgf].mean()
            dir_reward_corr = (
                (dp_c * rw_c).sum() / (dp_c.norm() * rw_c.norm()).clamp_min(1e-8)
            ).item()
        ...
        pr_m = np.mean(prop_stats, axis=0) if prop_stats else np.array([float("nan"), float("nan")])
        ...
        writer.add_scalar("diagnostics/ctrl_cost", ctrl_cost, global_step)
        writer.add_scalar("diagnostics/dir_reward_corr", dir_reward_corr, global_step)
