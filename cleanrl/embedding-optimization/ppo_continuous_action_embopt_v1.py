# Embedding Optimization v1 (embopt_v1) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
...
class Args:
    ...
    goal_norm_coef: float = 0.5
    """weight of the ||g||^2 prior on proposed goals (0.5 = N(0,I) log-density, the SIGReg'd marginal)"""
...
if __name__ == "__main__":
    ...
    def proposer_loss_fn(z):
        ...
        v = agent.value(z, g)
        gn = g.square().sum(-1)
        score = w + args.alpha_reach * v - args.goal_norm_coef * gn
        return -score.mean(), w.mean().detach(), v.mean().detach(), gn.mean().detach()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ---- proposer updates (frozen W_hat, V) ------------------------------------------
        prop_stats = []
        for _ in range(args.proposer_updates):
            ...
            mark_step()
            loss, wmean, vmean, gnorm = proposer_loss_fn(zb)
            ...
            proposer_opt.step()
            prop_stats.append((wmean.item(), vmean.item(), gnorm.item()))
        ...
        writer.add_scalar("diagnostics/proposer_goal_sqnorm", pr_m[2], global_step)
