# Embedding Optimization v28_dir (embopt_v28_dir) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
# v22 -> v28_dir: PROBE ARM C2 — DIRECTIONAL-LINEAR CONTROL (reviewer design).
# Discriminator for the kinetic-vs-perturbation question that arm B (v28_kin)
# alone cannot decide: the qprog target becomes u.(z'-z)/sqrt(dz) with u a
# FIXED random unit latent direction — a linear term with ZERO magnitude
# content (rewarding motion along an arbitrary constant direction; the goal
# machinery still runs untouched, the head may ignore (z,g)). Both hypotheses
# predict v28_kin can help; they SPLIT here: if v28_kin >> v28_dir, the
# adjuvant is genuinely kinetic (magnitude pressure). If v28_kin ~ v28_dir,
# the goal term was a structured gradient perturbation all along and any
# large decorrelated gradient escapes the -149 myopia plateau. Note v22's own
# dd target expands to 2(gap.d)/dz - ||d||^2/dz = directional-linear MINUS
# kinetic — structurally closer to THIS arm than to v28_kin.
# --- v22 header follows ---
...
if __name__ == "__main__":
    ...
    # C2 control direction: ONE fixed random unit vector for the whole run — the
    # qprog target becomes u.(z'-z), a LINEAR directional term with zero kinetic
    # content. Discriminates "structured gradient perturbation" from "kinetic
    # bonus" (v28_kin) at matched telemetry
    ...
    if args.compile:
        ...
        print(f"[embopt_v28_dir] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # C2 DIRECTIONAL-LINEAR target: u.(z'-z), u fixed random unit — motion
            # along an arbitrary constant latent direction; no magnitude content
            # whatsoever (linear, sign-symmetric around the plane). No extra
            # normalizer: u.d has std ~ sqrt(wm_delta_var) ~ 1.2, already matching
            # the kin arm's target scale (~1.4). The goal machinery still runs
            # untouched; the head is free to ignore (z,g)
