# Detached INTACT Anchor Direct v15
#
# V11's detached physical H12 NLL is treated as a stabilizing anchor: its weight
# holds early, then ramps upward around the observed collapse window. World
# learning, direct reward optimization, and deterministic runtime are unchanged.
import math
import os
import random
import time
...
class Args:
    ...
    intact_anchor_start_steps: int = 1_250_000
    intact_anchor_ramp_steps: int = 250_000
    detached_intact_anchor_coef: float = 0.06
...
def scheduled_detached_intact_anchor_coefficient(
    global_step,
    base_coefficient,
    anchor_start_steps,
    anchor_ramp_steps,
    ...
    *,
    ...
    """Hold the physical NLL coefficient, then ramp to a persistent anchor."""

    if base_coefficient < 0.0:
        raise ValueError("detached INTACT coefficient must be nonnegative")
    if anchor_coefficient < 0.0:
        raise ValueError("detached INTACT anchor coefficient must be nonnegative")
    if anchor_start_steps < 0:
        raise ValueError("INTACT anchor start steps must be nonnegative")
    if anchor_ramp_steps <= 0:
        raise ValueError("INTACT anchor ramp steps must be positive")
    step = torch.as_tensor(global_step, device=device, dtype=dtype)
    progress = (
        (step - anchor_start_steps) / anchor_ramp_steps
    ).clamp(0.0, 1.0)
    base = step.new_tensor(base_coefficient)
    return base + progress * (step.new_tensor(anchor_coefficient) - base)
...
def combined_controller_objective(
    ...
    total = (
        detached_intact_coefficient * detached_nll
        + args.direct_objective_coef * direct_loss
    )
    metrics = {
        "controller_loss": total.detach(),
        "effective_detached_intact_coef": (
            detached_intact_coefficient.detach()
        ),
        **detached_metrics,
        **direct_metrics,
    }
...
if __name__ == "__main__":
    ...
    assert args.latent_dim == 64, "detached INTACT World Direct v15 uses 64-D latents"
    ...
    assert args.intact_anchor_start_steps >= 0
    assert args.intact_anchor_ramp_steps > 0
    assert args.detached_intact_anchor_coef >= 0.0
    ...
    if args.compile:
        ...
        print(
            "[detached_intact_anchor_direct_v15] "
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        effective_detached_intact_coef = scheduled_detached_intact_anchor_coefficient(
            global_step,
            args.detached_intact_coef,
            args.intact_anchor_start_steps,
            args.intact_anchor_ramp_steps,
            args.detached_intact_anchor_coef,
            device=device,
        )
        update_metrics = []
        ...
        writer.add_scalar(
            "intact/effective_detached_anchor_coefficient",
            effective_detached_intact_coef,
            global_step,
        )
        writer.add_scalar(
