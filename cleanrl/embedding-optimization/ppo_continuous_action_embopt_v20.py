# Embedding Optimization v20 (embopt_v20) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
...
class Args:
    ...
    """weight of the goal-progress term in the policy loss (default 0: the v19 sweep
    showed instability monotone in this coefficient — spurious goal attribution; the
    goal machinery still trains for telemetry and future re-entry)"""
    imag_probe_h: int = 8
...
if __name__ == "__main__":
    ...
    def wm_loss_fn(o, a, o2, r, rate, rv, pair_valid, gcmd, grate):
        z = agent.encode(o)
        z2 = agent.encode(o2)
        ...
        # policy gradient directly, unlike the what/gwhat telemetry surfaces)
        ...
        loss = pred_loss + args.sigreg_coef * sig_loss + r_loss + w_loss + qr_loss + wg_loss
        return (
            loss,
            pred_loss.detach(),
            sig_loss.detach(),
    ...
    def proposer_loss_fn(z):
        ...
        # Every proposer incentive is a measured outcome; fantasy has no head to
        # live in. q is computed for telemetry only.
        w = agent.gwhat(g)
    ...
    if args.compile:
        ...
        print(f"[embopt_v20] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for _ in range(args.wm_updates):
            ...
            grate = buf_rate[g_f2, g_t2, g_e2]
            mark_step()
            loss, pl, sl, rl, wl, qrl, wgl = wm_loss_fn(o, a, o2, r, rate, rv, pv, gcmd, grate)
            ...
            wm_opt.step()
            wm_stats.append((pl.item(), sl.item(), rl.item(), wl.item(), qrl.item(), wgl.item()))
        ...
        writer.add_scalar("losses/rate_goal_mse", wm_m[5], global_step)
