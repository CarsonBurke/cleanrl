# Embedding Optimization v7 (embopt_v7) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
...
class Args:
    ...
    vf_consistency_coef: float = 0.5
    """weight of the V(f(z,a),g) consistency term (trains V where the policy queries it)"""
    ...
    """(unused since v6; kept for CLI compat) former additive V weight"""
    beta_band: float = 16.0
    """weight of the (V - gamma_g^goal_refresh)^2 aspiration-band penalty in the proposer score"""
...
if __name__ == "__main__":
    ...
    def critic_loss_fn(z, a, g, target, target_next):
        v = agent.value(z, g)
        main = (v - target).square().mean()
        # consistency: value the MODEL-predicted next latent (the exact query point the
        # policy differentiates) toward the next step's lambda-return target; f is
        # traversed, not updated (backward restricted to critic params)
        v_model = agent.value(agent.forward_dyn(z, a), g)
        consistency = (v_model - target_next).square().mean()
        ...
        return (
            main
            + args.vf_consistency_coef * consistency
            + args.self_goal_coef * anchor
            + args.neg_goal_coef * neg,
            main.detach(),
            v.mean().detach(),
        )
    ...
    def proposer_loss_fn(z, r_lo, r_hi):
        ...
        # both triviality (V ~ 1, the self-anchor exploit) and fantasy (V ~ floor) cost
        w = agent.what(g).clamp(r_lo, r_hi)
        v = agent.value(z, g)
        v_target = args.gamma_goal**args.goal_refresh
        score = (w - r_lo) - args.beta_band * (v - v_target).square()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # next-step targets for the V(f(z_t,a_t),g) consistency term: lambda-return at
            # t+1, with arrival (t+1 == j) -> 1
            targets_next = torch.zeros_like(targets)
            targets_next[:, :-1] = targets[:, 1:]
            targets_next = torch.where(j[:, None] == (t_range + 1), torch.ones_like(targets_next), targets_next)
            seg_act = buf_act[seg]  # (S,T,E,da)
        ...
        def sample_pairs(n):
            ...
            return (
                ...
                goals[ps, pe, pm],
                targets[ps, pt, pe, pm],
                targets_next[ps, pt, pe, pm],
            )

        # ---- critic updates --------------------------------------------------------------
        critic_stats = []
        for _ in range(args.critic_updates):
            zb, ab, gb, tb, tnb = sample_pairs(mb)
            mark_step()
            loss, main, vmean = critic_loss_fn(zb, ab, gb, tb, tnb)
        ...
        for _ in range(args.policy_updates):
            zh, _, gh, _, _ = sample_pairs(mb - n_prop)
