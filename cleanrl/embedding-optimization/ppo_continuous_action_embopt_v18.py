# Embedding Optimization v18 (embopt_v18) — family: cleanrl/embedding-optimization/ (see FAMILY.md)
#
...
class Args:
    ...
    goal_noise: float = 0.25
    """per-dim std of Gaussian exploration noise added to the commanded goal before
    shell re-projection (grounds dh/dg and the delivery map off the proposer's
    output manifold; the noisy goal IS the commanded goal, so labels stay factual)"""
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # delivery map: rate realized around a commanded goal's pursuit — separate
        # from rate_head so the state-rate fit and the goal map don't fight over one
        # set of parameters (their inputs collide at ||x|| ~ sqrt(dz) under SIGReg)
        self.goal_rate_head = mlp(dz, dh, 1)
        self.dz = dz
    ...
    def gwhat(self, g):
        return self.goal_rate_head(g).squeeze(-1)
...
if __name__ == "__main__":
    ...
    wm_params = (
        ...
        + list(agent.goal_rate_head.parameters())
    )
    ...
    def rollout_forward(obs, cur_goal, refresh, gnoise):
        z = agent.encode(obs)
        goal = torch.where(refresh, agent.propose(z), cur_goal)
        # goal-channel exploration: perturb on the shell; the perturbed goal IS the
        # commanded goal, so every downstream label stays a measured outcome
        gn = goal + gnoise
        goal = (agent.dz**0.5) * gn / gn.norm(dim=-1, keepdim=True).clamp_min(1e-6)
    ...
    def wm_loss_fn(o, a, o2, r, rate, pair_valid, gcmd, grate):
        ...
        # delivery map (separate head): promised rate at each commanded goal regressed
        # to the rate actually delivered around its pursuit — the fact surface the
        # proposer ascends
        wg_loss = (agent.gwhat(gcmd) - grate).square().mean()
    ...
    def proposer_loss_fn(z):
        g = agent.propose(z)
        # score = the delivery map ONLY: "goals like this empirically delivered X",
        # hill-climbed under goal noise. Every proposer incentive is a measured
        # outcome; fantasy has no head to live in. h is computed for telemetry only.
        w = agent.gwhat(g)
        p = agent.prog(z, g)
    ...
    if args.compile:
        ...
        print(f"[embopt_v18] torch.compile(mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(T):
            ...
            with torch.no_grad():
                gnoise = args.goal_noise * torch.randn((E, dz), device=device)
                mark_step()
                _, goal, action = rollout_forward(nobs(next_obs), cur_goal, refresh.unsqueeze(-1), gnoise)
                cur_goal = goal.clone()
        ...
        with torch.no_grad():
            ...
            goal_promise = agent.gwhat(roll_goals[w_idx]).mean().item()
