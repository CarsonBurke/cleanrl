# PPO + IterThink v24 Beta + d3bucket MTP + BetaPlast v9: v8 + SHARPER graded negative.
# =====================================================================================
# v9 change over v8: std-normalize the advantage before the negative tanh and SHARPEN it
# (signneg_neg_temp=2.0, signneg_neg_norm=True): neg_eff = tanh(2.0 * A/std). v8 (raw-tanh
# graded negative) beat v4 (hard sign) at every matched step, so graded > hard -- but v7
# (uniformly weaker suppression) was worst, so the win is SHAPE, not weakness. v9 brackets the
# sweet spot from the HARD side: still graded for near-zero advantages, but saturates to full
# -1 suppression faster (~1 std). Scale-robust (std-normalized). If v9 > v8, the optimum sits
# between v8 and v4. Positives keep divide-by-std magnitude; plasticity gate relative [0.25,4].
#
# --- inherited v8 notes ---
