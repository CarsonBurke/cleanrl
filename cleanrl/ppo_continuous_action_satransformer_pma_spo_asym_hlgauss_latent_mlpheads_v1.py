...
#
# mlpheads_v1 diff vs hlgauss_latent_v1:
#   The base transformer has just a single Linear(64, 6) on top of the encoder
#   for actor mean and Linear(64, num_bins) for the critic. The MLP baseline,
#   in contrast, has 2 hidden layers (3 linears, 2 nonlinearities) all the
#   way to its readout. Hypothesis: the encoder's PMA-seed feature is rich
#   but a single-linear readout is bottlenecking the translation to actions
#   and value, so the transformer's representational advantage isn't visible
#   on score. Replace the three heads with 2-layer MLPs (Linear → ReluSq →
#   Linear), keeping the rest identical.
...
class ReluSq(nn.Module):
    """Primer-style ReLU²: torch.relu(x) ** 2. Smooth, monotone, unbounded."""
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        # 2-layer MLP heads (Linear → ReluSq → Linear). Hidden dim = MODEL_DIM.
        # Hidden Linear keeps orthogonal(gain=√2) (default of layer_init); the
        # readout Linear keeps the small init (std=0.01 actor/sde, 1.0 critic)
        # so the policy still starts near-zero mean / near-zero log_std and
        # the critic readout starts well-conditioned.
        self.actor_mean_head = nn.Sequential(
            layer_init(nn.Linear(MODEL_DIM, MODEL_DIM)),
            ReluSq(),
            layer_init(nn.Linear(MODEL_DIM, self.action_dim), std=0.01),
        )
        self.sde_logstd_head = nn.Sequential(
            layer_init(nn.Linear(MODEL_DIM, MODEL_DIM)),
            ReluSq(),
            layer_init(nn.Linear(MODEL_DIM, self.action_dim), std=0.01),
        )
        self.critic_head = nn.Sequential(
            layer_init(nn.Linear(MODEL_DIM, MODEL_DIM)),
            ReluSq(),
            layer_init(nn.Linear(MODEL_DIM, num_bins), std=1.0),
        )
