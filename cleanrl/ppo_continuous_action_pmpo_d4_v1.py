# PMPO-D4 for continuous-action PPO.
#
# Hypothesis: replace PPO's ratio-clipped actor update with D4-style PMPO for
# smoother exploration std dynamics: raw-advantage sign split, tanh(abs(adv))
# weighting, per-action log-prob terms, and reverse KL(old rollout policy || new
# policy). `policy_objective=ppo` keeps the base CleanRL PPO path for reference.
