...
#
# Preserve v213's attached LeJEPA/SIGReg WM, grounded detached belief+policy
# forecast agent interface, and exact Beta trust region. After every WM update,
# causally reconstruct the rollout interface under the current WM and recompute
# values/GAE/HL targets, placing WM representation drift inside the same total
# KL budget as PPO. Generate H5 frozen-WM dreams from clean real prompts and mix
# one dream minibatch into every real PPO optimizer step with identical PPO,
# advantage, critic, clipping, and .03 transactional trust-region semantics.
# After real PPO, refresh only the private forecast head against the current
# successor pure-base actor. No EMA, normalization, rank transform, or agent-WM
