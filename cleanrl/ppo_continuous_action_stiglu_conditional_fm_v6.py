"""Flow-MPO: detached finite-candidate improvement, angular CFM projection.

The KL constraint is mean_s KL(w_s || Uniform(K)) on a sampled E-step ONLY.
It is not a density/KL/return guarantee for the fitted flow or Gaussian actor.
The principal uncertainty is action-dependent GAE predictiveness at tiny noise.
Unit-native coordinates u decode as bias + scale*tanh(initial_std*u).
"""
import hashlib
import json
import math
...
def sample_native(agent, observations, noise, steps):
    """Batched device Heun solver; no sampler graph enters policy projection."""
    observations, noise = observations.detach(), noise.detach()
    if agent.policy == "gaussian":
        mean, log_std = agent.gaussian_parameters(observations)
        return (mean + log_std.exp() * noise).detach()
    native = noise.clone()
    h = -1.0 / steps
...
class HostSampler:
    """Borrowed permanent buffers. Stage/copy outputs before invoking again.

    The host mirror's output is also borrowed: copy Heun k1 before computing k2.
    No CUDA inference, random draw, or transfer occurs inside __call__.
    """
    def __init__(self, agent, num_rows, steps):
        ...
        self.first_velocity, self.work = (np.empty(shape, dtype=np.float32) for _ in range(2))
    ...
    def __call__(self, observations, rng, *, noise=None):
        ...
        observations = observations.reshape(self.native.shape[0], self.obs_dim)
        if self.policy == "gaussian":
            mean, log_std = np.split(self.actor(observations), 2, axis=-1)
            np.exp(log_std, out=self.work)
...
def projection_loss(agent, observations, native, noise, times):
    ...
    if agent.policy == "flow":
        states, target = flow_interpolant(native, noise, times)
        return (agent.velocity(observations, states, times.detach()) - target).square().mean()
    mean, log_std = agent.gaussian_parameters(observations)
...
def validate_args(args):
    if min(args.num_envs, args.num_steps, args.num_minibatches, args.update_epochs,
           args.flow_steps, args.diagnostic_interval, args.critic_baseline_epochs,
    ...
    if not math.isfinite(args.initial_std) or args.initial_std != INITIAL_STD:
        raise ValueError(f"v6 hard initial native std must be {INITIAL_STD}")
...
def main():
    ...
    with ExitStack() as resources:
        ...
        provenance = {
            ...
            "path": "cos(pi*t/2)*data+sin(pi*t/2)*noise; source1,data0; backward Heun",
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            for optimizer in (actor_optimizer, value_optimizer, baseline_optimizer, action_optimizer):
            ...
            with timer.span("diagnostics"), torch.no_grad():
                ...
                if diagnose:
                    ...
                    prefix = "flow" if args.policy == "flow" else "gaussian"
            ...
            logged.update({"charts/learning_rate": learning_rate,
                           ...
                           "sampling/nfe": 2 * args.flow_steps if args.policy == "flow" else 1,
