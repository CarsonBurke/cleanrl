# Self-supervised STSTS tokenizer pretraining for the v5 world-model variant.
#
# Mirrors `pretrain_ststs_tokenizer_v1.py` but instantiates the tokenizer with
# `obs_token_tanh=True` so its per-cell observation tokens land in (-1, 1) — the
# support consumed by the v5 Beta-NLL state head. The dynamics CLS shaping
# (current-state recon, next-state recon, reward, continue, transition) is
# unchanged, so the tokenizer remains a frozen target for the RL loop while the
# downstream Beta head can decode latents in their bounded range.
import os
import random
import sys
import time
...
from cleanrl.wm.wm_sde_stateent_twohot_v6_relusq_hlgauss_v5 import (
    CONTEXT_LEN,
    DYN_TOKEN_COUNT,
    STSTSCLSBackbone,
    TOK_EMBED_DIM,
    TOK_FFN_MULT,
    TOK_NUM_HEADS,
    TOK_NUM_SPATIAL_BLOCKS,
    TOK_NUM_TEMPORAL_BLOCKS,
    WM_CLS_NAMES,
    make_env,
    relusq_mlp,
...
class TokenizerTrainer(nn.Module):
    def __init__(self, obs_dim, action_dim):
        super().__init__()
        self.backbone = STSTSCLSBackbone(
            obs_dim,
            CONTEXT_LEN,
            WM_CLS_NAMES,
        ...
        self.obs_decoder = nn.Linear(TOK_EMBED_DIM, 1)
    ...
    def forward_loss(self, obs_seq, next_obs_seq, actions, rewards, continues, args):
        ...
        next_dyn_pred = self.transition(za).view(-1, DYN_TOKEN_COUNT, TOK_EMBED_DIM)
