# V7 GATED: Sigmoid-gated low-rank noise bases with explicit floor.
# Replaces the quadratic SDE (tanh(z)^2 @ W^2) with bounded sigmoid gates
# over rank-8 noise basis vectors + learned floor std.
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # SDE: sigmoid-gated rank-8 noise bases + floor
        NOISE_RANK = 8
        self.gate_proj = layer_init(nn.Linear(EMBED_DIM, NOISE_RANK), std=0.01)
        self.noise_basis = nn.Parameter(torch.randn(NOISE_RANK, action_dim) * 0.1)
    ...
    def _get_action_std(self, sde_latent):
        gate = torch.sigmoid(self.gate_proj(sde_latent))  # (batch, 8), bounded [0,1]
        basis_sq = self.noise_basis.pow(2)  # (8, action_dim)
        var = gate.pow(2) @ basis_sq  # (batch, action_dim)
        floor_var = self.base_log_std.clamp(LOG_STD_MIN, LOG_STD_MAX).exp().pow(2)
        return (var + floor_var).sqrt()

    def _action_std_fixed(self):
        # Imagination: gates at sigmoid midpoint (0.5)
        basis_sq = self.noise_basis.pow(2)
        var = 0.25 * basis_sq.sum(0)
        floor_var = self.base_log_std.clamp(LOG_STD_MIN, LOG_STD_MAX).exp().pow(2)
        return (var + floor_var).sqrt()
