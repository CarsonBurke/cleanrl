# V7B NORMALIZED: Original quadratic but softmax-normalized weights.
# var = tanh(z)² @ softmax(W², dim=0) * scale + floor
# Softmax over embed dim makes weights sum to 1 per action → bounded output.
# Learned scale controls magnitude. Full rank-32, no drift.
#
# PPO + WM + SDE + state entropy + D4-style two-hot value/reward encoding.
#
# Fork of worldmodel_sde4cls_stateent (best performer ~5500 at 8M).
# Adds SymExpTwoHot encoding for critic and reward heads via dedicated CLS tokens.
# 7 CLS tokens: actor, critic, dynamics, sde, state_pred, critic_bins, reward_bins.
# The bin CLS tokens are trained only by two-hot CE losses.
# The scalar critic/reward heads (on critic/dynamics CLS) handle imagination.
import os
import random
import time
...
class SymExpTwoHot(nn.Module):
    """D4-style symlog two-hot encoding. Bins uniform in symlog space, stored in real space."""

    def __init__(self, num_bins=NUM_BINS, value_range=REWARD_RANGE):
        super().__init__()
        min_val, max_val = value_range
        symlog_grid = torch.linspace(min_val, max_val, num_bins)
        bin_values = symlog_grid.sign() * (symlog_grid.abs().exp() - 1.0)
        self.num_bins = num_bins
        self.register_buffer("bin_values", bin_values)

    def encode(self, values):
        """Encode scalar values to two-hot vectors. values: (...) -> (..., num_bins)"""
        values_clamped = values.clamp(min=self.bin_values[0], max=self.bin_values[-1])
        indices = torch.searchsorted(self.bin_values, values_clamped.contiguous())
        left = (indices - 1).clamp(min=0)
        right = left.clamp(max=self.num_bins - 2) + 1

        left_vals = self.bin_values[left]
        right_vals = self.bin_values[right]
        total_dist = (right_vals - left_vals).clamp(min=1e-8)
        right_weight = ((values_clamped - left_vals) / total_dist).clamp(0.0, 1.0)
        left_weight = 1.0 - right_weight

        encoded = torch.zeros(*values.shape, self.num_bins, device=values.device)
        encoded.scatter_(-1, left.unsqueeze(-1), left_weight.unsqueeze(-1))
        encoded.scatter_(-1, right.unsqueeze(-1), right_weight.unsqueeze(-1))
        return encoded

    def decode(self, logits):
        """Decode bin logits to scalar. No softmax — matches D4's normalize=False default."""
        return (logits.softmax(dim=-1) * self.bin_values).sum(dim=-1)
...
class STSTSCLSBackbone(nn.Module):
    def __init__(self, obs_dim, context_len):
        ...
        # 7 CLS: actor, critic, dynamics, sde, state_pred, critic_bins, reward_bins
        self.cls_names = ['actor', 'critic', 'dynamics', 'sde', 'state_pred', 'critic_bins', 'reward_bins']
        ...
        self.cls_params = nn.ParameterList([
            nn.Parameter(torch.empty(EMBED_DIM)) for _ in range(NUM_CLS_TOKENS)
        ])
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # Two-hot encoder
        self.twohot = SymExpTwoHot(NUM_BINS, REWARD_RANGE)

        # Actor (on actor CLS)
        self.actor_mean = layer_init(nn.Linear(EMBED_DIM, action_dim), std=0.01)

        # Scalar critic (on critic CLS) — used for GAE + imagination
        self.critic = layer_init(nn.Linear(EMBED_DIM, 1), std=1.0)

        # Bin critic (on critic_bins CLS) — trained by two-hot CE
        self.critic_bins = layer_init(nn.Linear(EMBED_DIM, NUM_BINS), std=1.0)

        # SDE: normalized quadratic — softmax weights + learned scale + floor
        self.log_std_param = nn.Parameter(torch.zeros(EMBED_DIM, action_dim))
        self.var_scale = nn.Parameter(torch.ones(action_dim))  # learned per-action scale
        self.base_log_std = nn.Parameter(torch.full((action_dim,), LOG_STD_INIT))

        # State prediction (on state_pred CLS)
        self.to_state_pred = nn.Sequential(
            RMSNorm(EMBED_DIM),
            nn.Linear(EMBED_DIM, EMBED_DIM * 2),
        )

        # WM heads (on dynamics CLS)
        self.transition = nn.Sequential(
            nn.Linear(EMBED_DIM + action_dim, 256), nn.SiLU(),
            nn.Linear(256, 256), nn.SiLU(),
            nn.Linear(256, EMBED_DIM),
        )
        # Scalar reward (on dynamics CLS) — used in imagination
        self.reward_head = nn.Sequential(
            nn.Linear(EMBED_DIM + action_dim, 128), nn.SiLU(),
            nn.Linear(128, 1),
        )
        # Bin reward (on reward_bins CLS) — trained by two-hot CE
        self.reward_head_bins = nn.Sequential(
            nn.Linear(EMBED_DIM + action_dim, 128), nn.SiLU(),
    ...
    def _get_action_std(self, sde_latent):
        z = torch.tanh(sde_latent)  # (batch, 32)
        # Softmax-normalized weights: sum to 1 per action dim → bounded output
        weights = F.softmax(self.log_std_param, dim=0)  # (32, action_dim), normalized over embed
        action_var = z.pow(2) @ weights  # (batch, action_dim), bounded [0, 1]
        action_var = action_var * self.var_scale.pow(2)  # learned scale
        floor_var = self.base_log_std.clamp(LOG_STD_MIN, LOG_STD_MAX).exp().pow(2)
        return (action_var + floor_var).sqrt()

    def _action_std_fixed(self):
        weights = F.softmax(self.log_std_param, dim=0)
        avg_z_sq = 0.5  # E[tanh(z)²] ≈ 0.5 for typical activations
        var = avg_z_sq * weights.sum(0) * self.var_scale.pow(2)
        floor_var = self.base_log_std.clamp(LOG_STD_MIN, LOG_STD_MAX).exp().pow(2)
        return (var + floor_var).sqrt()
    ...
    def get_value(self, obs_seq):
        cls = self._encode(obs_seq)
        return self.critic(cls['critic'])

    def get_action_and_value(self, obs_seq, action=None):
        cls = self._encode(obs_seq)
        action_mean = self.actor_mean(cls['actor'])
        action_std = self._get_action_std(cls['sde'])
        probs = Normal(action_mean, action_std)
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), self.critic(cls['critic']), cls['dynamics'], cls['state_pred']
    ...
    def get_all_for_update(self, obs_seq, action):
        ...
        return (
            ...
            probs.entropy().sum(1),
            self.critic(cls['critic']),          # scalar value for GAE
            self.critic_bins(cls['critic_bins']), # bin logits for two-hot CE
            cls['dynamics'],                      # for WM scalar losses
            cls['reward_bins'],                   # for reward bin CE
            cls['state_pred'],
        )

    def imagine(self, z_start, horizon, gamma, gae_lambda):
        """Differentiable imagination — scalar heads only."""
        action_std = self._action_std_fixed()
        z = z_start
        rewards = []
        values = []
        continues = []
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Pre-encode two-hot targets
        with torch.no_grad():
            b_return_bins = agent.twohot.encode(b_returns)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                newlogprob, entropy, newvalue, value_logits, dynamics_latent, reward_bins_latent, state_pred_latent = agent.get_all_for_update(b_obs_seqs[mb_inds], b_actions[mb_inds])
                ...
                # Scalar value loss (on critic CLS — used for GAE + imagination)
                ...
                # Two-hot value loss (on critic_bins CLS — richer gradients)
                mb_return_bins = b_return_bins[mb_inds]
                v_loss_bins = F.cross_entropy(value_logits, mb_return_bins, reduction='mean')

                entropy_loss = entropy.mean()

                # World model losses on fresh latents
                with torch.no_grad():
                    next_cls = agent._encode(b_next_obs_seqs[mb_inds])
                    next_dynamics = next_cls['dynamics']
                mb_za = torch.cat([dynamics_latent, b_actions[mb_inds]], dim=-1)
                ...
                transition_loss = (((mb_z_pred - next_dynamics) ** 2).mean(-1) * mb_valid).sum() / (mb_valid.sum() + 1e-8)
                reward_loss = ((mb_r_pred - b_rewards[mb_inds]) ** 2 * mb_valid).sum() / (mb_valid.sum() + 1e-8)

                # Two-hot reward loss (on reward_bins CLS)
                mb_rba = torch.cat([reward_bins_latent, b_actions[mb_inds]], dim=-1)
                mb_r_logits = agent.reward_head_bins(mb_rba)
                mb_reward_bins = agent.twohot.encode(b_rewards[mb_inds])
                reward_ce = F.cross_entropy(mb_r_logits, mb_reward_bins, reduction='none')
                reward_loss_bins = (reward_ce * mb_valid).sum() / (mb_valid.sum() + 1e-8)
                ...
                loss = pg_loss - args.ent_coef * entropy_loss + args.vf_coef * v_loss + v_loss_bins + WM_COEF * wm_loss + STATE_PRED_LOSS_COEF * state_pred_loss
        ...
        # Imagination phase (scalar heads only)
        n_seeds = min(N_IMAGINE_SEEDS, args.batch_size)
        seed_inds = torch.randperm(args.batch_size, device=device)[:n_seeds]
        with torch.no_grad():
            seed_cls = agent._encode(b_obs_seqs[seed_inds])
            z_seeds = seed_cls['dynamics']
        z_seeds = z_seeds.detach()
        ...
        with torch.no_grad():
            ...
            cls = agent._encode(agent.obs_history)
            sp_pred = agent.to_state_pred(cls['state_pred'])
