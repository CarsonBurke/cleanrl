# PPO + WM + split actor mean/log-var heads + Dreamer4-style state prediction bonus.
#
...
# smaller real-env parallelism by default, larger update ratios, and replay-seeded
# repeated imagination updates after each real rollout. The policy no longer uses
# a separate SDE CLS; this ablation keeps a single actor latent but routes it
# through separate mean and log-variance heads. State prediction is upgraded to
# predict a Gaussian over the full next state-token tensor, and the intrinsic
# bonus uses summed predicted log-variance as in Dreamer4.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        self.policy_mean = layer_init(nn.Linear(EMBED_DIM, action_dim), std=0.01)
        self.policy_log_var = layer_init(nn.Linear(EMBED_DIM, action_dim), std=0.01)
        with torch.no_grad():
            self.policy_log_var.bias.fill_(2.0 * LOG_STD_INIT)
        self.critic = layer_init(nn.Linear(EMBED_DIM, 1), std=1.0)

        # State prediction head: Gaussian (mean, log_var) over next state-token tensor.
        self.to_state_pred = nn.Sequential(
            RMSNorm(EMBED_DIM),
            nn.Linear(EMBED_DIM, obs_dim * EMBED_DIM * 2),
        )
    ...
    def _get_policy_dist(self, actor_latent):
        action_mean = self.policy_mean(actor_latent)
        action_log_var = self.policy_log_var(actor_latent)
    ...
    def _predict_state_tokens_distribution(self, state_pred_latent):
        pred = self.to_state_pred(state_pred_latent)
        pred = pred.view(-1, self.obs_dim, EMBED_DIM, 2)
        pred_mean = pred[..., 0]
        pred_log_var = pred[..., 1]
        return pred_mean, pred_log_var

    def get_state_entropy_bonus(self, state_pred_latent):
        _, pred_log_var = self._predict_state_tokens_distribution(state_pred_latent)
        return pred_log_var.sum(dim=(-1, -2)) * STATE_ENT_COEF
    ...
    def get_action_and_value(self, obs_seq, action_seq, reward_seq, action=None):
        ...
        probs, _, _, _ = self._get_policy_dist(actor_latent)
        if action is None:
            action = probs.sample()
        return (
            action,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # State prediction loss: Gaussian NLL on next state-token target
                pred_mean, pred_log_var = agent._predict_state_tokens_distribution(state_pred_latent)
                pred_var = pred_log_var.exp()
                sp_loss_raw = F.gaussian_nll_loss(pred_mean, next_state_tokens_target, var=pred_var, reduction='none')
                state_pred_loss = (sp_loss_raw.mean(dim=(-1, -2)) * mb_valid).sum() / (mb_valid.sum() + 1e-8)
        ...
        # Imagination phase: run multiple replay-seeded updates to shift more compute onto the GPU.
        imagine_actor_loss = torch.zeros((), device=device)
        imagine_critic_loss = torch.zeros((), device=device)
        ...
        for _ in range(args.imagine_updates_per_iter):
            ...
            one_imagine_actor_loss = -torch.stack(lambda_returns).mean()

            one_imagine_critic_loss = torch.zeros((), device=device)
            for t in range(args.imagination_horizon):
                one_imagine_critic_loss = one_imagine_critic_loss + ((imagined_values[t] - lambda_returns[t].detach()) ** 2).mean()
            one_imagine_critic_loss = one_imagine_critic_loss / args.imagination_horizon

            imagine_loss = IMAGINE_COEF * one_imagine_actor_loss + IMAGINE_CRITIC_COEF * one_imagine_critic_loss
            ...
            imagine_actor_loss = imagine_actor_loss + one_imagine_actor_loss.detach()
            imagine_critic_loss = imagine_critic_loss + one_imagine_critic_loss.detach()
            imagine_return_mean = imagine_return_mean + torch.stack(lambda_returns).mean().detach()
            imagine_updates_done += 1

        if imagine_updates_done > 0:
            imagine_actor_loss = imagine_actor_loss / imagine_updates_done
            imagine_critic_loss = imagine_critic_loss / imagine_updates_done
        ...
        with torch.no_grad():
            ...
            _, sp_log_var = agent._predict_state_tokens_distribution(sp_latent)
            writer.add_scalar("state_pred/log_var_mean", sp_log_var.mean().item(), global_step)
            writer.add_scalar("state_pred/entropy_bonus_mean", (sp_log_var.sum(dim=(-1, -2)) * STATE_ENT_COEF).mean().item(), global_step)
