# V7 LOWRANK: Original quadratic SDE but with rank-8 bottleneck instead of rank-32.
# Projects SDE latent (32-dim) to 8-dim before tanh^2 @ W^2.
# Tests whether the full 32-dim aggregation is needed or if 8 dims suffice.
# No state entropy bonus (confirmed not needed in ablation study).
#
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # SDE: original quadratic but rank-8 bottleneck + floor
        NOISE_RANK = 8
        self.rank_proj = nn.Linear(EMBED_DIM, NOISE_RANK)
        # Init rank_proj so output has reasonable magnitude
        fan_in = EMBED_DIM
        std = 1.0 / fan_in**0.5
        nn.init.trunc_normal_(self.rank_proj.weight, std=std, a=-2*std, b=2*std)
        nn.init.zeros_(self.rank_proj.bias)
        self.log_std_param = nn.Parameter(torch.zeros(NOISE_RANK, action_dim))
    ...
    def _get_action_std(self, sde_latent):
        z = torch.tanh(self.rank_proj(sde_latent))  # (batch, 8)
        log_std = (self.log_std_param + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
        std_sq = log_std.exp().pow(2)
        action_var = z.pow(2) @ std_sq  # 8-dim sum instead of 32
