# PPO + WM + Dreamer4-style policy mean/log-var head + D4-style state entropy.
#
# Fork of wm_sde_stateent_base_v3.
# The policy no longer uses a separate SDE CLS for exploration scale. Instead,
# the actor CLS emits both action mean and action log-var, matching Dreamer4's
# "one policy embedding -> Gaussian parameters" structure for continuous actions.
# The state-entropy path also follows Dreamer4 more closely: the state_pred CLS
# predicts a Gaussian over the full next state-token tensor, and the exploration
# bonus uses the summed predicted log-variance.
import os
import random
import time
...
class SymExpTwoHot(nn.Module):
    """D4-style symlog two-hot encoding with bins stored in real value space."""

    def __init__(self, num_bins=NUM_BINS, value_range=VALUE_RANGE):
        super().__init__()
...
class STSTSCLSBackbone(nn.Module):
    def __init__(self, obs_dim, action_dim, context_len):
        ...
        self.critic_bins_cls = nn.Parameter(torch.empty(EMBED_DIM))
    ...
    def forward_state_tokens(self, state_tokens, action_seq, reward_seq):
        ...
        cls_tokens = torch.stack(
            [self.actor_cls, self.critic_cls, self.critic_bins_cls, self.dynamics_cls, self.state_pred_cls], dim=0
        )
        ...
        critic_bins_cls = cls_tokens[:, -1, 2]
        dynamics_cls = cls_tokens[:, -1, 3]
        state_pred_cls = cls_tokens[:, -1, 4]
        return actor_cls, critic_cls, critic_bins_cls, dynamics_cls, state_pred_cls, state_tokens
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        self.twohot = SymExpTwoHot(NUM_BINS, VALUE_RANGE)

        # Dreamer4-style state prediction: Gaussian over the full next state-token tensor.
    ...
    def _get_policy_mean_log_var(self, actor_latent):
    ...
    def _get_policy_dist(self, actor_latent):
        action_mean, action_log_var = self._get_policy_mean_log_var(actor_latent)
    ...
    def get_action_and_value(self, obs_seq, action_seq, reward_seq, action=None):
        actor_latent, critic_latent, _, dynamics_latent, state_pred_latent, _ = self._encode(
            obs_seq, action_seq, reward_seq
        )
        probs, _, _ = self._get_policy_dist(actor_latent)
        if action is None:
            action = probs.sample()
        return (
            action,
    ...
    def get_all_for_update(self, obs_seq, action_seq, reward_seq, action):
        actor_latent, critic_latent, critic_bins_latent, dynamics_latent, state_pred_latent, state_tokens = self._encode(
            obs_seq, action_seq, reward_seq
        )
        probs, _, _ = self._get_policy_dist(actor_latent)
        return (
            ...
            probs.entropy().sum(1),
            self.critic(critic_latent),
            self.critic_bins(critic_bins_latent),
            dynamics_latent,
            state_pred_latent,
            state_tokens[:, -1],
        )
    ...
    def imagine(self, state_token_seq, action_seq, reward_seq, horizon, gamma, gae_lambda):
        ...
        for _ in range(horizon):
            actor_latent, critic_latent, _, dynamics_latent, _, state_tokens = self._encode_state_tokens(
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                newlogprob, entropy, newvalue, value_logits, dynamics_latent, state_pred_latent, state_tokens = agent.get_all_for_update(
                    b_obs_seqs[mb_inds],
                ...
                mb_return_bins = agent.twohot.encode(b_returns[mb_inds])
                ...
                # Dreamer4-style state prediction loss on the full next state-token tensor.
        ...
        with torch.no_grad():
            ...
            actor_latent, _, _, _, _, _ = agent._encode(agent.obs_history, agent.action_history, agent.reward_history)
            _, _, action_std = agent._get_policy_dist(actor_latent)
