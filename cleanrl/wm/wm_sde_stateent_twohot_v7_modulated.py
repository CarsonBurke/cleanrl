# V7 MODULATED: Base std with bounded state-dependent multiplicative modulation.
# Learned per-action base_std modulated by tanh(features @ basis) * alpha.
# State controls WHERE to explore more/less, base controls HOW MUCH overall.
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # SDE: base std × bounded modulation
        NOISE_RANK = 8
        self.mod_proj = layer_init(nn.Linear(EMBED_DIM, NOISE_RANK), std=0.01)
        self.mod_basis = nn.Parameter(torch.randn(NOISE_RANK, action_dim) * 0.1)
        self.base_log_std = nn.Parameter(torch.full((action_dim,), LOG_STD_INIT))
        self.mod_alpha = 0.5  # modulation range ±50%
    ...
    def _get_action_std(self, sde_latent):
        features = torch.tanh(self.mod_proj(sde_latent))  # (batch, 8)
        modulation = torch.tanh(features @ self.mod_basis)  # (batch, action_dim), bounded [-1,1]
        base_std = self.base_log_std.clamp(LOG_STD_MIN, LOG_STD_MAX).exp()
        return base_std * (1.0 + self.mod_alpha * modulation)

    def _action_std_fixed(self):
        # Imagination: no modulation (tanh(0)=0)
        return self.base_log_std.clamp(LOG_STD_MIN, LOG_STD_MAX).exp()
