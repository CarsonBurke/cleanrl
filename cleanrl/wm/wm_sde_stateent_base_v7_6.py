# PPO + WM + dream-only PMPO/value updates after a world-model warmup.
#
# Fork of wm_sde_stateent_base_v2.
# This version keeps the v7 modeling and PMPO path, but changes the default
# cadence to small real rollouts and uses only dreamed PMPO/value updates for
# the agent. The world model trains first on real data alone for a fixed warmup
# period before any imagined agent updates are allowed.
import os
import random
import time
...
DEFAULT_IMAGINE_BATCH_SIZE = 256
STATE_ENT_COEF = 0.01
...
class Args:
    ...
    imagine_updates_per_iter: int = 1
    imagine_batch_size: int = 16
    imagine_replay_capacity: int = 128
    warmup_world_model_steps: int = 10000
...
def masked_mean(t, mask=None):
    ...
    if mask.shape != t.shape:
        mask = torch.broadcast_to(mask, t.shape)
    if not mask.any():
        return t[mask].sum()
    return t[mask].mean()
...
class Agent(nn.Module):
    ...
    def _get_action_dist(self, actor_latent):
        action_mean, action_log_var = self.policy_mean_log_var(actor_latent).chunk(2, dim=-1)
        action_log_var = action_log_var.clamp(2.0 * LOG_STD_MIN, 2.0 * LOG_STD_MAX)
        action_std = (0.5 * action_log_var).exp()
        return Normal(action_mean, action_std), action_mean, action_log_var
    ...
    def get_action_and_value(self, obs_seq, action_seq, reward_seq, action=None):
        ...
        probs, action_mean, action_log_var = self._get_action_dist(actor_latent)
        if action is None:
            action = probs.sample()
        return (
            action,
    ...
    def get_all_for_update_state_tokens(self, state_token_seq, action_seq, reward_seq, action):
        actor_latent, critic_latent, dynamics_latent, state_pred_latent, state_tokens = self._encode_state_tokens(
    ...
    def imagine(self, state_token_seq, action_seq, reward_seq, horizon, gamma, gae_lambda):
        ...
        for _ in range(horizon):
            ...
            probs, _, _ = self._get_action_dist(actor_latent)
            a = probs.rsample()
    ...
    @torch.no_grad()
    def imagine_experience(self, state_token_seq, action_seq, reward_seq, horizon):
        state_token_seqs = []
        action_seqs = []
        reward_seqs = []
        actions = []
        ...
        for _ in range(horizon):
            state_token_seqs.append(state_token_seq)
            action_seqs.append(action_seq)
            reward_seqs.append(reward_seq)

            actor_latent, critic_latent, _, state_pred_latent, state_tokens = self._encode_state_tokens(
                state_token_seq, action_seq, reward_seq
            )
            probs, action_mean, action_log_var = self._get_action_dist(actor_latent)
            action = probs.sample()
            next_state_tokens = self.predict_next_state_tokens(state_tokens[:, -1])
            reward_latent, continue_latent = self.readout_next_state(next_state_tokens)
            intrinsic_bonus = self.get_state_entropy_bonus(state_pred_latent)
            reward = self.reward_head(reward_latent).squeeze(-1) + intrinsic_bonus
            cont = self.continue_head(continue_latent).sigmoid().squeeze(-1)
            value = self.critic(critic_latent).squeeze(-1)

            actions.append(action)
            action_logprobs.append(probs.log_prob(action).sum(-1))
            action_means.append(action_mean)
            action_log_vars.append(action_log_var)
            rewards.append(reward)
            continues.append(cont)
            values.append(value)

            state_token_seq = torch.cat([state_token_seq[:, 1:], next_state_tokens.unsqueeze(1)], dim=1)
            action_seq = torch.cat([action_seq[:, 1:], action.unsqueeze(1)], dim=1)
            reward_seq = torch.cat([reward_seq[:, 1:], reward.unsqueeze(1).unsqueeze(-1)], dim=1)

        _, critic_latent, _, _, _ = self._encode_state_tokens(state_token_seq, action_seq, reward_seq)
        bootstrap_value = self.critic(critic_latent).squeeze(-1)

        return (
            torch.stack(state_token_seqs),
            torch.stack(action_seqs),
            torch.stack(reward_seqs),
            torch.stack(actions),
            torch.stack(action_logprobs),
            torch.stack(action_means),
            torch.stack(action_log_vars),
            torch.stack(rewards),
...
def compute_gae_from_rewards_values(rewards, values, continues, gamma, gae_lambda, bootstrap_value):
    advantages = torch.zeros_like(rewards)
    lastgaelam = torch.zeros_like(bootstrap_value)
    nextvalues = bootstrap_value
    for t in reversed(range(rewards.shape[0])):
        delta = rewards[t] + gamma * continues[t] * nextvalues - values[t]
        lastgaelam = delta + gamma * gae_lambda * continues[t] * lastgaelam
...
def build_dream_valid_mask(continues, threshold=0.5):
    valid = torch.zeros_like(continues, dtype=torch.bool)
    still_alive = torch.ones_like(continues[0], dtype=torch.bool)
    for t in range(continues.shape[0]):
        valid[t] = still_alive
        still_alive = still_alive & (continues[t] >= threshold)
    return valid


def compute_policy_value_losses(
    agent,
    args,
    action_seqs,
    reward_seqs,
    ...
    returns,
    obs_seqs=None,
    state_token_seqs=None,
    mask=None,
):
    prefix_shape = actions.shape[:-1]
    flat_batch = int(np.prod(prefix_shape))

    flat_action_seqs = action_seqs.reshape(flat_batch, *action_seqs.shape[len(prefix_shape):])
    flat_reward_seqs = reward_seqs.reshape(flat_batch, *reward_seqs.shape[len(prefix_shape):])
    flat_actions = actions.reshape(flat_batch, actions.shape[-1])

    if obs_seqs is not None:
        flat_obs_seqs = obs_seqs.reshape(flat_batch, *obs_seqs.shape[len(prefix_shape):])
        new_action_logprobs, new_action_entropies, newvalue, _, _, _, new_action_mean, new_action_log_var = agent.get_all_for_update(
            flat_obs_seqs,
            flat_action_seqs,
            flat_reward_seqs,
            flat_actions,
        ...
        flat_state_token_seqs = state_token_seqs.reshape(flat_batch, *state_token_seqs.shape[len(prefix_shape):])
        new_action_logprobs, new_action_entropies, newvalue, _, _, _, new_action_mean, new_action_log_var = agent.get_all_for_update_state_tokens(
            flat_state_token_seqs,
            flat_action_seqs,
            flat_reward_seqs,
            flat_actions,
        )

    action_dim = new_action_logprobs.shape[-1]
    new_action_logprobs = new_action_logprobs.reshape(*prefix_shape, action_dim)
    new_action_entropies = new_action_entropies.reshape(*prefix_shape, action_dim)
    new_action_mean = new_action_mean.reshape(*prefix_shape, action_dim)
    new_action_log_var = new_action_log_var.reshape(*prefix_shape, action_dim)
    newvalue = newvalue.reshape(*prefix_shape)

    newlogprob = new_action_logprobs.sum(-1)
    logratio = newlogprob - old_logprobs
    ratio = logratio.exp()

    with torch.no_grad():
        old_approx_kl = masked_mean(-logratio, mask)
        approx_kl = masked_mean((ratio - 1.0) - logratio, mask)
        clipfrac = masked_mean(((ratio - 1.0).abs() > args.clip_coef).float(), mask).item()
    ...
    if args.norm_adv:
        if mask is None:
            ...
            valid_adv = mb_advantages[mask]
            mb_advantages = (mb_advantages - valid_adv.mean()) / (valid_adv.std() + 1e-8)

    if args.use_pmpo:
        pos_advantage_mask = mb_advantages >= 0.0
        neg_advantage_mask = ~pos_advantage_mask
        if mask is not None:
            pos_advantage_mask &= mask
            neg_advantage_mask &= mask

        pos = masked_mean(new_action_logprobs, pos_advantage_mask[..., None])
        neg = -masked_mean(new_action_logprobs, neg_advantage_mask[..., None])

        pg_loss = -(args.pmpo_pos_to_neg_weight * pos + (1.0 - args.pmpo_pos_to_neg_weight) * neg)

        if args.pmpo_kl_div_loss_weight > 0.0:
            old_action_std = (0.5 * old_action_log_vars).exp()
            old_dist = Normal(old_action_means, old_action_std)
            new_action_std = (0.5 * new_action_log_var).exp()
            new_dist = Normal(new_action_mean, new_action_std)

            kl_src, kl_tgt = new_dist, old_dist
            if args.pmpo_reverse_kl:
                kl_src, kl_tgt = kl_tgt, kl_src

            kl_div_loss = masked_mean(torch.distributions.kl.kl_divergence(kl_src, kl_tgt).sum(-1), mask)
            pg_loss = pg_loss + kl_div_loss * args.pmpo_kl_div_loss_weight
    else:
    ...
    if args.clip_vloss:
        ...
        v_clipped = old_values + torch.clamp(newvalue - old_values, -args.value_clip, args.value_clip)
        v_loss = 0.5 * masked_mean(torch.max(v_loss_unclipped, (v_clipped - returns) ** 2), mask)
    else:
        v_loss = 0.5 * masked_mean((newvalue - returns) ** 2, mask)

    entropy_loss = masked_mean(-new_action_entropies.sum(-1), mask)
    rl_step_loss = pg_loss + args.ent_coef * entropy_loss + args.vf_coef * v_loss

    return rl_step_loss, pg_loss, v_loss, entropy_loss, old_approx_kl, approx_kl, clipfrac
...
if __name__ == "__main__":
    ...
    old_action_means = torch.zeros((args.num_steps, args.num_envs, action_dim), device=device)
    old_action_log_vars = torch.zeros((args.num_steps, args.num_envs, action_dim), device=device)
    logprobs = torch.zeros((args.num_steps, args.num_envs), device=device)
    rewards = torch.zeros((args.num_steps, args.num_envs), device=device)
    extrinsic_rewards = torch.zeros((args.num_steps, args.num_envs), device=device)
    intrinsic_bonuses = torch.zeros((args.num_steps, args.num_envs), device=device)
    dones = torch.zeros((args.num_steps, args.num_envs), device=device)
    next_dones = torch.zeros((args.num_steps, args.num_envs), device=device)
    values = torch.zeros((args.num_steps, args.num_envs), device=device)
    imagine_replay_obs = torch.zeros((args.imagine_replay_capacity, agent.context_len, obs_dim), device=device)
    imagine_replay_actions = torch.zeros((args.imagine_replay_capacity, agent.context_len, action_dim), device=device)
    imagine_replay_rewards = torch.zeros((args.imagine_replay_capacity, agent.context_len, 1), device=device)
    imagine_replay_size = 0
    imagine_replay_ptr = 0
    ...
    recent_episodic_returns = deque(maxlen=50)
    recent_episodic_lengths = deque(maxlen=50)
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            frac = 1.0 - (iteration - 1.0) / args.num_iterations
            optimizer.param_groups[0]["lr"] = frac * args.learning_rate
        completed_episodes_this_iter = 0
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, _, state_pred_latent, action_mean, action_log_var = agent.get_action_and_value(
                    agent.obs_history,
            ...
            actions[step] = action
            old_action_means[step] = action_mean
            old_action_log_vars[step] = action_log_var
            ...
            intrinsic_bonuses[step] = ent_bonus
            ...
            history_reward = extrinsic_rewards[step].unsqueeze(-1)
            if next_done.any():
            ...
            if "final_info" in infos:
                for info in infos["final_info"]:
                    if info and "episode" in info:
                        completed_episodes_this_iter += 1
                        recent_episodic_returns.append(float(info["episode"]["r"]))
                        recent_episodic_lengths.append(float(info["episode"]["l"]))
                        print(f"global_step={global_step}, episodic_return={info['episode']['r']}")
                        writer.add_scalar("charts/episodic_return_raw", info["episode"]["r"], global_step)
        ...
        b_old_action_means = old_action_means.reshape(-1, action_dim)
        b_old_action_log_vars = old_action_log_vars.reshape(-1, action_dim)
        b_actions = actions.reshape((-1,) + envs.single_action_space.shape)
        b_advantages = advantages.reshape(-1)
        b_returns = returns.reshape(-1)
        b_values = values.reshape(-1)
        b_rewards = rewards.reshape(-1)
        b_extrinsic_rewards = extrinsic_rewards.reshape(-1)
        b_intrinsic_bonuses = intrinsic_bonuses.reshape(-1)
        b_dones = dones.reshape(-1)

        # Keep a GPU-resident replay of context windows for repeated imagination updates.
        replay_batch_n = b_obs_seqs.shape[0]
        if replay_batch_n >= args.imagine_replay_capacity:
            replay_obs_batch = b_obs_seqs[-args.imagine_replay_capacity:]
            replay_action_batch = b_action_seqs[-args.imagine_replay_capacity:]
            replay_reward_batch = b_reward_seqs[-args.imagine_replay_capacity:]
            replay_batch_n = args.imagine_replay_capacity
        else:
            replay_obs_batch = b_obs_seqs
            replay_action_batch = b_action_seqs
            replay_reward_batch = b_reward_seqs

        replay_write = min(replay_batch_n, args.imagine_replay_capacity - imagine_replay_ptr)
        imagine_replay_obs[imagine_replay_ptr:imagine_replay_ptr + replay_write] = replay_obs_batch[:replay_write]
        imagine_replay_actions[imagine_replay_ptr:imagine_replay_ptr + replay_write] = replay_action_batch[:replay_write]
        imagine_replay_rewards[imagine_replay_ptr:imagine_replay_ptr + replay_write] = replay_reward_batch[:replay_write]
        if replay_write < replay_batch_n:
            wrap = replay_batch_n - replay_write
            imagine_replay_obs[:wrap] = replay_obs_batch[replay_write:]
            imagine_replay_actions[:wrap] = replay_action_batch[replay_write:]
            imagine_replay_rewards[:wrap] = replay_reward_batch[replay_write:]
        imagine_replay_ptr = (imagine_replay_ptr + replay_batch_n) % args.imagine_replay_capacity
        imagine_replay_size = min(args.imagine_replay_capacity, imagine_replay_size + replay_batch_n)
        ...
        # World-model-only updates on the real batch.
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # World model step.
                _, _, dynamics_latent, state_pred_latent, state_tokens = agent._encode(
                    b_obs_seqs[mb_inds],
                ...
                reward_loss = ((mb_r_pred - b_extrinsic_rewards[mb_inds]) ** 2 * mb_valid).sum() / (mb_valid.sum() + 1e-8)
                ...
                wm_step_loss = WM_COEF * wm_loss + STATE_PRED_LOSS_COEF * state_pred_loss
        ...
        # Imagination phase: generate imagined experience and run the same PMPO/value update path on it.
        imagine_pg_loss = torch.zeros((), device=device)
        imagine_v_loss = torch.zeros((), device=device)
        imagine_entropy_loss = torch.zeros((), device=device)
        imagine_return_mean = torch.zeros((), device=device)
        imagine_updates_done = 0
        dreams_enabled = global_step >= args.warmup_world_model_steps

        for _ in range(args.imagine_updates_per_iter if dreams_enabled else 0):
            if imagine_replay_size == 0:
                break

            n_seeds = min(args.imagine_batch_size, imagine_replay_size)
            seed_inds = torch.randint(0, imagine_replay_size, (n_seeds,), device=device)

            imagine_obs_seqs = imagine_replay_obs[seed_inds]
            imagine_action_seqs = imagine_replay_actions[seed_inds]
            imagine_reward_seqs = imagine_replay_rewards[seed_inds]

            with torch.no_grad():
                state_token_seeds = agent.encode_state(imagine_obs_seqs).detach()
                (
                    imagine_state_token_seqs,
                    imagine_action_seqs_hist,
                    imagine_reward_seqs_hist,
                    imagine_actions,
                    imagine_old_logprobs,
                    imagine_old_action_means,
                    imagine_old_action_log_vars,
                    imagine_rewards,
                    imagine_continues,
                    imagine_values,
                    imagine_bootstrap_value,
                ) = agent.imagine_experience(
                    state_token_seeds,
                    imagine_action_seqs.detach(),
                    imagine_reward_seqs.detach(),
                    args.imagination_horizon,
                )
                imagine_advantages, imagine_returns = compute_gae_from_rewards_values(
                    imagine_rewards,
                    imagine_values,
                    imagine_continues,
                    args.gamma,
            ...
            imagine_mask = build_dream_valid_mask(imagine_continues)

            rl_step_loss, one_imagine_pg_loss, one_imagine_v_loss, one_imagine_entropy_loss, _, _, _ = compute_policy_value_losses(
                agent,
                args,
                imagine_action_seqs_hist,
                imagine_reward_seqs_hist,
                ...
                imagine_advantages,
                imagine_returns,
                state_token_seqs=imagine_state_token_seqs,
                mask=imagine_mask,
            )

            optimizer.zero_grad()
            rl_step_loss.backward()
            nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
            optimizer.step()

            imagine_pg_loss = imagine_pg_loss + one_imagine_pg_loss.detach()
            imagine_v_loss = imagine_v_loss + one_imagine_v_loss.detach()
            imagine_entropy_loss = imagine_entropy_loss + one_imagine_entropy_loss.detach()
            imagine_return_mean = imagine_return_mean + masked_mean(imagine_returns, imagine_mask).detach()
            imagine_updates_done += 1

        if imagine_updates_done > 0:
            imagine_pg_loss = imagine_pg_loss / imagine_updates_done
            imagine_v_loss = imagine_v_loss / imagine_updates_done
            imagine_entropy_loss = imagine_entropy_loss / imagine_updates_done
            imagine_return_mean = imagine_return_mean / imagine_updates_done
        ...
        writer.add_scalar("charts/completed_episodes_per_iter", completed_episodes_this_iter, global_step)
        writer.add_scalar("charts/rollout_extrinsic_reward_mean", b_extrinsic_rewards.mean().item(), global_step)
        writer.add_scalar("charts/rollout_intrinsic_bonus_mean", b_intrinsic_bonuses.mean().item(), global_step)
        if recent_episodic_returns:
            writer.add_scalar("charts/episodic_return", float(np.mean(recent_episodic_returns)), global_step)
            writer.add_scalar("charts/episodic_return_recent_mean", float(np.mean(recent_episodic_returns)), global_step)
            writer.add_scalar("charts/episodic_length_recent_mean", float(np.mean(recent_episodic_lengths)), global_step)
        writer.add_scalar("imagination/policy_loss", imagine_pg_loss.item(), global_step)
        writer.add_scalar("imagination/value_loss", imagine_v_loss.item(), global_step)
        writer.add_scalar("imagination/entropy_loss", imagine_entropy_loss.item(), global_step)
        writer.add_scalar("imagination/updates", imagine_updates_done, global_step)
        writer.add_scalar("imagination/enabled", float(dreams_enabled), global_step)
        with torch.no_grad():
            writer.add_scalar("imagination/mean_return", imagine_return_mean.item(), global_step)
            actor_latent, _, _, _, _ = agent._encode(agent.obs_history, agent.action_history, agent.reward_history)
            _, _, action_log_var = agent._get_action_dist(actor_latent)
            action_std = (0.5 * action_log_var).exp()
