# PPO + WM + direct-var SDE + state entropy bonus + logits-aware critic w/ shaping.
#
# Fork of wm_sde_stateent_base_v2 with a logits-aware critic: V(critic_cls, μ, σ).
# The critic head receives the critic CLS token concatenated with the policy's
# detached mean and std, giving it information about the current policy distribution.
# An auxiliary shaping loss maximizes V w.r.t. μ and σ (with frozen critic weights),
# providing ∂V/∂μ and ∂V/∂σ directional gradients to the actor.
import os
import random
import time
...
SHAPING_COEF = 0.05
SHAPING_WARMUP_FRAC = 0.1
SHAPING_DECAY_FRAC = 0.8
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # Logits-aware critic: input is critic_cls concat with μ and σ
        self.critic = layer_init(nn.Linear(EMBED_DIM + action_dim * 2, 1), std=1.0)
    ...
    def _critic_forward(self, critic_latent, action_mean, action_std, detach_logits=True):
        """V(critic_cls, μ, σ). Detach logits by default for value loss."""
        if detach_logits:
            action_mean = action_mean.detach()
            action_std = action_std.detach()
        critic_input = torch.cat([critic_latent, action_mean, action_std], dim=-1)
        return self.critic(critic_input)

    def _critic_forward_shaping(self, critic_latent, action_mean, action_std):
        """V with attached logits but frozen critic weights for shaping."""
        critic_input = torch.cat([critic_latent.detach(), action_mean, action_std], dim=-1)
    ...
    def get_value(self, obs_seq, action_seq, reward_seq):
        actor_latent, critic_latent, _, sde_latent, _, _ = self._encode(obs_seq, action_seq, reward_seq)
        action_mean = self.actor_mean(actor_latent)
        action_std = self._get_action_std(sde_latent)
        return self._critic_forward(critic_latent, action_mean, action_std)
    ...
    def get_action_and_value(self, obs_seq, action_seq, reward_seq, action=None):
        ...
        return (
            ...
            probs.entropy().sum(1),
            self._critic_forward(critic_latent, action_mean, action_std),
            dynamics_latent,
            state_pred_latent,
        )
    ...
    def imagine(self, state_token_seq, action_seq, reward_seq, horizon, gamma, gae_lambda):
        ...
        for _ in range(horizon):
            ...
            values.append(self._critic_forward(critic_latent, a_mean, a_std).squeeze(-1))
        ...
        actor_latent, critic_latent, _, sde_latent, _, _ = self._encode_state_tokens(state_token_seq, action_seq, reward_seq)
        a_mean = self.actor_mean(actor_latent)
        a_std = self._get_action_std(sde_latent)
        values.append(self._critic_forward(critic_latent, a_mean, a_std).squeeze(-1))
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if progress < SHAPING_WARMUP_FRAC:
            shaping_scale = progress / max(SHAPING_WARMUP_FRAC, 1e-8)
        elif progress > SHAPING_DECAY_FRAC:
            shaping_scale = max(0.0, 1.0 - (progress - SHAPING_DECAY_FRAC) / max(1.0 - SHAPING_DECAY_FRAC, 1e-8))
        else:
            shaping_scale = 1.0
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                (newlogprob, entropy, newvalue, dynamics_latent, state_pred_latent,
                 state_tokens, critic_latent, action_mean, action_std) = agent.get_all_for_update(
                    b_obs_seqs[mb_inds],
                ...
                # Shaping loss: maximize V w.r.t. μ, σ with frozen critic
                shaping_loss = torch.zeros(1, device=device)
                if SHAPING_COEF > 0 and shaping_scale > 0:
                    shaping_value = agent._critic_forward_shaping(critic_latent, action_mean, action_std)
                    shaping_loss = -shaping_scale * SHAPING_COEF * shaping_value.mean()

                loss = pg_loss - args.ent_coef * entropy_loss + args.vf_coef * v_loss + WM_COEF * wm_loss + STATE_PRED_LOSS_COEF * state_pred_loss + shaping_loss
