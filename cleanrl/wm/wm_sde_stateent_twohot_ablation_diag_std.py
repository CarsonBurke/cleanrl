# ABLATION: diag_std — Replace SDE (state-dependent quadratic noise aggregation) with
# a simple learned diagonal log_std per action dim. Tests whether the SDE mechanism
# and its 32-dim-to-action-dim variance aggregation contribute to performance,
# or if a simpler noise parameterization suffices.
#
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # ABLATION: simple diagonal log_std instead of SDE matrix
        self.action_log_std = nn.Parameter(torch.full((action_dim,), LOG_STD_INIT))
    ...
    def _get_action_std(self, sde_latent=None):
        """Simple diagonal std — ignores sde_latent entirely."""
        log_std = self.action_log_std.clamp(LOG_STD_MIN, LOG_STD_MAX)
        return log_std.exp()

    def _action_std_fixed(self):
        return self._get_action_std()
