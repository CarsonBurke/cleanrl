# ABLATION: direct_std — Replace the quadratic noise aggregation (sde_latent.pow(2) @ std_sq)
# with a direct linear projection from SDE CLS to per-action-dim log_std.
# Still state-dependent, but avoids the 32-dim quadratic aggregation that risks noise drift.
# Tests whether state-dependent noise matters but simpler parameterization is sufficient.
#
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # SDE (on sde CLS) — direct linear projection
        self.std_proj = layer_init(nn.Linear(EMBED_DIM, action_dim), std=0.01)
    ...
    def _get_action_std(self, sde_latent):
        """Direct linear projection from SDE latent to per-action log_std."""
        log_std = self.std_proj(sde_latent) + LOG_STD_INIT
        log_std = log_std.clamp(LOG_STD_MIN, LOG_STD_MAX)
        return log_std.exp()

    def _action_std_fixed(self):
        """For imagination — use mean of projection bias as fixed std."""
        log_std = (self.std_proj.bias + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
        return log_std.exp()
