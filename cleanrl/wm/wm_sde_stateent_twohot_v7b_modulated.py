# V7B MODULATED: Base std × bounded full-rank modulation.
# std = base_std * (1 + alpha * tanh(tanh(z) @ basis))
# Full 32-dim SDE CLS, no projection. Tanh bounds modulation to [-1,1].
# Base std provides stability, modulation provides state-dependence.
#
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # SDE: base std × full-rank bounded modulation
        self.mod_basis = nn.Parameter(torch.randn(EMBED_DIM, action_dim) * (1.0 / EMBED_DIM**0.5))
        self.base_log_std = nn.Parameter(torch.full((action_dim,), LOG_STD_INIT))
        self.mod_alpha = 0.5  # ±50% modulation range
    ...
    def _get_action_std(self, sde_latent):
        z = torch.tanh(sde_latent)  # (batch, 32) — full rank, bounded
        modulation = torch.tanh(z @ self.mod_basis)  # (batch, action_dim), bounded [-1,1]
