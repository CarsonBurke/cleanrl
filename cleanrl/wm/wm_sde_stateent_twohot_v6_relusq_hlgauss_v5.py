...
#     agent, cls]` with PAST-SHIFTED action tokens (slot k holds a_{k-1},
...
#   - Reward and continue heads also read `dyn_flat` at each frame.
...
class TokenActionSTSBackbone(nn.Module):
    ...
    def encode_obs_tokens(
        ...
        if return_dyn_seq:
            ...
            # autoregressively decoding (next_obs, reward, continue) under
...
class Agent(nn.Module):
    def __init__(
        ...
        self.reward_head_bins = relusq_mlp(WM_DYN_FLAT_DIM, reward_num_bins, REWARD_HIDDEN)
    ...
    def _predictions_from_dyn_flat(self, dyn_flat):
        """Apply per-frame heads to one or many dyn-flat readouts.

        Accepts either:
          - (B, D): single-frame readout used by the dream loop;
          - (B, T, D): per-frame readout used by extended-window WM training.
        Returns predictions of corresponding rank. The state head emits
        unimodal Beta(alpha, beta) parameters; reward / continue heads emit
        bin logits and a scalar logit. The caller is responsible for selecting
        / masking the valid prediction frames.
        """
        state_params = self.to_state_pred(dyn_flat)  # (..., 2 * tok_obs_embed_dim)
        reward_logits = self.reward_head_bins(dyn_flat)
        continue_logits = self.continue_head(dyn_flat).squeeze(-1)
        return state_params, reward_logits, continue_logits
    ...
    def _predict_from_obs_window(self, obs_tokens, action_history, reward_history):
        """Single-window predict (s_{k+1}, r_k, c_k) from frame k's dyn.
        ...
        return self._predictions_from_dyn_flat(dyn_flat)
...
def clip_agent_grad_norms(agent, max_grad_norm):
    ...
    groups = {
        ...
        "dyn_v": list(agent.dynamics_value_bins.parameters()),
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            if not stop_policy_updates:
                ...
                optimizer.step()
                with torch.no_grad():
                    adv_abs = b_advantages[mb_inds].abs().mean().item()
                print(
                    f"[real] iter={iteration} epoch={epoch} step={global_step} "
                    f"v={v_loss_bins.item():.4f} ent={entropy_loss.item():.4f} kl={approx_kl.item():.5f} "
                    f"adv|.|={adv_abs:.3f} act_gn={grad_norms['actor']:.3f} "
                    f"crit_gn={grad_norms['critic']:.3f} {_ep_summary()}",
                    flush=True,
                )
        ...
        for wm_epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, wm_batch_size):
                ...
                with bf16_autocast(device):
                    ...
                    state_params, pred_reward_logits, pred_continue_logits = (
                        agent._predictions_from_dyn_flat(predict_dyn_flats)
                    )
            ...
            for _dream_batch in range(args.dream_batches):
                with torch.no_grad(), bf16_autocast(device):
                    ...
                    for _ in range(args.imagination_horizon):
                        ...
                        state_params, reward_logits, continue_logits = agent._predict_from_obs_window(
                            obs_token_window,
                            action_window,
                            reward_window,
                        )
                ...
                for _ in range(args.imagine_update_epochs):
                    ...
                    print(
                        ...
                        f"critic={imagine_critic_loss.item():.4f} kl={dream_kl_mean:.5f} "
