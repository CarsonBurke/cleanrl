...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # World model heads
        self.next_state_token_delta = nn.Sequential(
            RMSNorm(EMBED_DIM),
            nn.Linear(EMBED_DIM, EMBED_DIM),
        )
        self.next_state_dynamics = layer_init(nn.Linear(EMBED_DIM, EMBED_DIM), std=0.01)
    ...
    def predict_next_state_tokens(self, state_tokens, dynamics_latent):
        token_delta = self.next_state_token_delta(state_tokens)
        dynamics_bias = self.next_state_dynamics(dynamics_latent).unsqueeze(1)
        return state_tokens + token_delta + dynamics_bias
    ...
    def imagine(self, state_token_seq, action_seq, reward_seq, horizon, gamma, gae_lambda):
        ...
        for _ in range(horizon):
            ...
            za = torch.cat([dynamics_latent, a], dim=-1)
            next_state_tokens = self.predict_next_state_tokens(state_tokens[:, -1], dynamics_latent)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_state_pred = agent.predict_next_state_tokens(state_tokens, dynamics_latent)
