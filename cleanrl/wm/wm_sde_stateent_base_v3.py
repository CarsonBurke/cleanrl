...
class STSTSCLSBackbone(nn.Module):
    ...
    def forward_state_tokens(self, state_tokens, action_seq, reward_seq):
        ...
        cls_tokens = torch.stack(
            [self.actor_cls, self.critic_cls, self.critic_bins_cls, self.dynamics_cls, self.sde_cls, self.state_pred_cls], dim=0
        )
        ...
        sde_cls = cls_tokens[:, -1, 4]
        state_pred_cls = cls_tokens[:, -1, 5]
        return actor_cls, critic_cls, critic_bins_cls, dynamics_cls, sde_cls, state_pred_cls, state_tokens
...
class Agent(nn.Module):
    ...
    def get_value(self, obs_seq, action_seq, reward_seq):
        _, critic_latent, _, _, _, _, _ = self._encode(obs_seq, action_seq, reward_seq)
        return self.critic(critic_latent)

    def get_action_and_value(self, obs_seq, action_seq, reward_seq, action=None):
        actor_latent, critic_latent, _, dynamics_latent, sde_latent, state_pred_latent, _ = self._encode(
    ...
    def get_all_for_update(self, obs_seq, action_seq, reward_seq, action):
        actor_latent, critic_latent, critic_bins_latent, dynamics_latent, sde_latent, state_pred_latent, state_tokens = self._encode(
            obs_seq, action_seq, reward_seq
        )
        action_mean = self.actor_mean(actor_latent)
        action_std = self._get_action_std(sde_latent)
    ...
    def imagine(self, state_token_seq, action_seq, reward_seq, horizon, gamma, gae_lambda):
        ...
        for _ in range(horizon):
            actor_latent, critic_latent, _, dynamics_latent, sde_latent, _, state_tokens = self._encode_state_tokens(
        ...
        _, critic_latent, _, _, _, _, _ = self._encode_state_tokens(state_token_seq, action_seq, reward_seq)
        values.append(self.critic(critic_latent).squeeze(-1))
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with torch.no_grad():
                    ...
                    _, _, _, next_dynamics, _, _, _ = agent._encode(
        ...
        with torch.no_grad():
            ...
            _, _, _, _, sde_latent, _, _ = agent._encode(agent.obs_history, agent.action_history, agent.reward_history)
            action_std = agent._get_action_std(sde_latent)
            writer.add_scalar("policy/action_std_mean", action_std.mean().item(), global_step)
            _, _, _, _, _, sp_latent, _ = agent._encode(agent.obs_history, agent.action_history, agent.reward_history)
