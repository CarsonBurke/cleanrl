# V7B GATED: Sigmoid-gated full-rank (32-dim) noise with floor.
# sigmoid(z)² @ basis² + floor — no projection bottleneck, all 32 dims preserved.
# Sigmoid bounds each gate to [0,1] preventing drift from unbounded sum.
#
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # SDE: sigmoid-gated full-rank noise with floor
        self.noise_basis = nn.Parameter(torch.randn(EMBED_DIM, action_dim) * 0.1)
    ...
    def _get_action_std(self, sde_latent):
        gate = torch.sigmoid(sde_latent)  # (batch, 32), bounded [0,1] — no projection
        basis_sq = self.noise_basis.pow(2)  # (32, action_dim)
    ...
    def _action_std_fixed(self):
        ...
        var = 0.25 * basis_sq.sum(0)  # sigmoid midpoint
        floor_var = self.base_log_std.clamp(LOG_STD_MIN, LOG_STD_MAX).exp().pow(2)
        return (var + floor_var).sqrt()
