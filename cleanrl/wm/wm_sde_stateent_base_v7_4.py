# PPO + WM + direct-var SDE + state entropy bonus, with a more GPU-dense regime.
#
# Fork of wm_sde_stateent_base_v2.
# This version keeps the stronger base_v2 modeling choices, but shifts more of the
# compute budget away from MuJoCo stepping and into batched model-side updates:
# smaller real-env parallelism by default, larger update ratios, replay-seeded
# repeated imagination updates after each real rollout, and Dreamer4-style PMPO
# with reverse-KL regularization on the policy update.
import os
import random
import time
...
class Args:
    ...
    imagine_updates_per_iter: int = 2
    imagine_batch_size: int = DEFAULT_IMAGINE_BATCH_SIZE
    imagine_replay_capacity: int = 16384
...
class Agent(nn.Module):
    ...
    def imagine_experience(self, state_token_seq, action_seq, reward_seq, horizon):
        ...
        for _ in range(horizon):
            ...
            actor_latent, critic_latent, _, _, state_tokens = self._encode_state_tokens(
                state_token_seq, action_seq, reward_seq
            )
            probs, action_mean, action_log_var = self._get_action_dist(actor_latent)
            action = probs.sample()
            next_state_tokens = self.predict_next_state_tokens(state_tokens[:, -1])
            reward_latent, continue_latent = self.readout_next_state(next_state_tokens)
            reward = self.reward_head(reward_latent).squeeze(-1)
...
def compute_policy_value_losses(agent, args, action_seqs, reward_seqs, actions, old_logprobs, old_action_means, old_action_log_vars, old_values, advantages, returns, obs_seqs=None, state_token_seqs=None):
    ...
    if args.use_pmpo:
        ...
        if args.pmpo_kl_div_loss_weight > 0.0:
            ...
            kl_div_loss = torch.distributions.kl.kl_divergence(kl_src, kl_tgt).sum(-1).mean()
    ...
    entropy_loss = -new_action_entropies.sum(-1).mean()
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Separate world-model and RL updates on the shared backbone.
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Policy/value step.
                rl_step_loss, pg_loss, v_loss, entropy_loss, old_approx_kl, approx_kl, clipfrac = compute_policy_value_losses(
                    agent,
                    args,
                    b_action_seqs[mb_inds],
                    b_reward_seqs[mb_inds],
                    b_actions[mb_inds],
                    b_logprobs[mb_inds],
                    b_old_action_means[mb_inds],
                    b_old_action_log_vars[mb_inds],
                    b_values[mb_inds],
                    b_advantages[mb_inds],
                    b_returns[mb_inds],
                    obs_seqs=b_obs_seqs[mb_inds],
                )
                clipfracs += [clipfrac]
        ...
        for _ in range(args.imagine_updates_per_iter):
            ...
            flat_imagine_state_tokens = imagine_state_token_seqs.reshape(-1, agent.context_len, obs_dim, EMBED_DIM)
            flat_imagine_action_seqs = imagine_action_seqs_hist.reshape(-1, agent.context_len, action_dim)
            flat_imagine_reward_seqs = imagine_reward_seqs_hist.reshape(-1, agent.context_len, 1)
            flat_imagine_actions = imagine_actions.reshape(-1, action_dim)
            flat_imagine_old_logprobs = imagine_old_logprobs.reshape(-1)
            flat_imagine_old_action_means = imagine_old_action_means.reshape(-1, action_dim)
            flat_imagine_old_action_log_vars = imagine_old_action_log_vars.reshape(-1, action_dim)
            flat_imagine_values = imagine_values.reshape(-1)
            flat_imagine_advantages = imagine_advantages.reshape(-1)
            flat_imagine_returns = imagine_returns.reshape(-1)

            rl_step_loss, one_imagine_pg_loss, one_imagine_v_loss, one_imagine_entropy_loss, _, _, _ = compute_policy_value_losses(
                agent,
                args,
                flat_imagine_action_seqs,
                flat_imagine_reward_seqs,
                flat_imagine_actions,
                flat_imagine_old_logprobs,
                flat_imagine_old_action_means,
                flat_imagine_old_action_log_vars,
                flat_imagine_values,
                flat_imagine_advantages,
                flat_imagine_returns,
                state_token_seqs=flat_imagine_state_tokens,
            )
            ...
            imagine_return_mean = imagine_return_mean + imagine_returns.mean().detach()
        ...
        writer.add_scalar("charts/rollout_intrinsic_bonus_mean", (b_rewards - b_extrinsic_rewards).mean().item(), global_step)
