...
class STSTSCLSBackbone(nn.Module):
    ...
    def forward_state_tokens(self, state_tokens, action_seq, reward_seq):
        ...
        cls_tokens = torch.stack(
            [self.actor_cls, self.critic_cls, self.dynamics_cls, self.state_pred_cls, self.critic_bins_cls], dim=0
        )
        ...
        critic_bins_cls = cls_tokens[:, -1, 4]
        return actor_cls, critic_cls, dynamics_cls, state_pred_cls, critic_bins_cls, state_tokens
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, obs_seq, action_seq, reward_seq, action=None):
        actor_latent, critic_latent, dynamics_latent, state_pred_latent, _, _ = self._encode(
    ...
    def get_all_for_update(self, obs_seq, action_seq, reward_seq, action):
        actor_latent, critic_latent, dynamics_latent, state_pred_latent, critic_bins_latent, state_tokens = self._encode(
            obs_seq, action_seq, reward_seq
        )
        probs, action_mean, action_log_var, action_std = self._get_policy_dist(actor_latent)
        return (
    ...
    def imagine(self, state_token_seq, action_seq, reward_seq, horizon, gamma, gae_lambda):
        ...
        for _ in range(horizon):
            actor_latent, critic_latent, dynamics_latent, _, _, state_tokens = self._encode_state_tokens(
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        writer.add_scalar("losses/v_loss_bins", v_loss_bins.item(), global_step)
        ...
        with torch.no_grad():
            ...
            _, _, _, sp_latent, _, _ = agent._encode(agent.obs_history, agent.action_history, agent.reward_history)
