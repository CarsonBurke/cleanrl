# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted-attention slot transformer v13.
#
# WHY v13 (ORTHONORMAL TRUNK WEIGHTS instead of L2 row/col weight-norm). v12 normalizes
# every trunk matrix to unit rows/cols (justnorm) after each optimizer step -- this fixes
# each row's NORM but leaves the angles between rows free. v13 replaces that with the
# nearest semi-orthogonal matrix via reduced SVD (W <- U @ Vh), i.e. a Stiefel/Procrustes
# projection. Square matrices (v, attn.proj) become fully orthogonal; rectangular ones
# (q, k, mlp.fc, mlp.proj) become semi-orthogonal (orthonormal rows if wide, columns if
# tall). Each weight is then a true ISOMETRY of the hypersphere -- it preserves angles,
# not just norms -- which is strictly stronger conditioning than v12's unit-norm rows.
# Note this is NOT scale-neutral for rectangular matrices (UVh changes the singular-value
# scale, not just per-row norms), so the learned eigen-lr / suv gains must absorb the
# shift -- making this a genuine A/B against v12, not a reparametrization. Everything else
# is byte-identical to v12 (activation justnorm, FFN, attention, residuals, PPO loop, and
# the per-step normalize call frequency are all unchanged). Base: v12.
#
# --- inherited v12 notes ---
#
...
def orthonormalize(W, steps=8, piters=6, eps=1e-7):
    # Nearest semi-orthogonal matrix (Stiefel projection): the polar factor U @ Vh,
    # i.e. all singular values driven to exactly 1 -- a true isometry (orthonormal
    # ROWS when wide, COLUMNS when tall), strictly stronger than unit-row L2 norm.
    #
    # Computed by the CONVERGENT cubic Newton-Schulz iteration  X <- 1.5 X - 0.5 X X^T X
    # (each singular value sigma -> 1.5 sigma - 0.5 sigma^3, fixed point sigma=1).
    # matmul-ONLY -> stays on-GPU with NO device sync, unlike torch.linalg.svd, whose
    # per-call GPU->CPU sync made every optimizer step ~5x slower in our launch-bound
    # regime. (NB: Muon's *quintic* NS is deliberately non-convergent -- it parks
    # singular values in [~0.7,1.3] for fast gradient whitening -- so it is the WRONG
    # tool for an exact projection; we use the convergent cubic instead.)
    #
    # Convergence needs spectral norm <= 1 going in. Frobenius-normalizing would shrink
    # an already-orthogonal matrix's singular values to 1/sqrt(k) every call (k = #rows),
    # forcing many steps forever. Instead we estimate the spectral norm with a few power
    # iterations on X X^T (sync-free) and divide by it (x1.1 safety so spectral < 1
    # strictly; cubic NS converges for spectral in (0, sqrt(3))). Then a near-orthogonal
    # matrix re-converges in a handful of steps; cold/ill-conditioned matrices (the
    # square q/k/v/proj at init) need more, hence the larger `steps` on the init call.
    X = W
    transpose = X.shape[-2] > X.shape[-1]
    if transpose:
        X = X.mT                                   # iterate on rows<=cols (drive X X^T -> I)
    G = X @ X.mT                                    # rows x rows (small)
    v = G.sum(dim=-1, keepdim=True) + eps           # deterministic nonzero init (no rng/sync);
                                                    # +eps breaks any degenerate orthogonality
                                                    # to the top eigenvector.
    for _ in range(piters):
        v = G @ v
        v = v / (v.norm() + eps)
    sigma2 = (v * (G @ v)).sum() / ((v * v).sum() + eps)   # Rayleigh quotient ~ lambda_max
    # x1.5 margin: Rayleigh is a *lower* bound on lambda_max, so the divisor must be padded
    # enough that the result's spectral norm stays well under sqrt(3) (cubic-NS convergence
    # radius) even when power iteration under-estimates on a near-degenerate spectrum. 1.5
    # gives a large buffer (needs Rayleigh within ~38% of true) at zero extra steady steps.
    X = X / (sigma2.clamp_min(eps).sqrt() * 1.5 + eps)     # spectral norm < 1
...
class ThinkTrunk(nn.Module):
    ...
    @torch.no_grad()
    def normalize_weights(self, steps=8):
        """nGPT weight normalization: after each optimizer step, project every trunk
        matrix back onto the Stiefel manifold (orthonormal rows/cols) -- replaces weight
        decay; keeps the eigen-lr step sizes calibrated. `steps` is the cubic-NS count
        per matrix (8 suffices for the small per-step drift; the init call passes a
        larger value to fully orthogonalize the cold, ill-conditioned square matrices)."""
        self.tokenizer.value_weight.copy_(orthonormalize(self.tokenizer.value_weight, steps))
        self.tokenizer.pos.copy_(orthonormalize(self.tokenizer.pos, steps))
        for stack in (self.actor_stack, self.critic_stack):
            if stack is None:
                continue
            for block in stack.blocks:
                block.attn.q.weight.copy_(orthonormalize(block.attn.q.weight, steps))
                block.attn.k.weight.copy_(orthonormalize(block.attn.k.weight, steps))
                block.attn.v.weight.copy_(orthonormalize(block.attn.v.weight, steps))
                block.attn.proj.weight.copy_(orthonormalize(block.attn.proj.weight, steps))
                block.mlp.fc.weight.copy_(orthonormalize(block.mlp.fc.weight, steps))
                block.mlp.proj.weight.copy_(orthonormalize(block.mlp.proj.weight, steps))
...
class Agent(nn.Module):
    ...
    @torch.no_grad()
    def normalize_trunk_weights(self, steps=8):
        """nGPT weight re-normalization, applied to every trunk after each optimizer
        step (and once at init, with a larger `steps` for the cold-start projection).
        Heads are deliberately excluded."""
        if self.share_backbone:
            self.trunk.normalize_weights(steps)
        else:
            self.actor_trunk.normalize_weights(steps)
            self.critic_trunk.normalize_weights(steps)
...
if __name__ == "__main__":
    ...
    agent.normalize_trunk_weights(steps=45)   # nGPT: matrices start orthonormal (cold-start NS)
    if args.compile:
