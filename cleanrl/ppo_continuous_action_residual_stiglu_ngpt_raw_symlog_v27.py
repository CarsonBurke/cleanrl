# Raw-reward symlog-mean clipping ablations of the residual SiTU-GLU baseline.
# Preserve the v26 runtime and actor PPO (.2/.2, GAE lambda=.95). The scalar
# mean-consistent symlog critic estimates expected return after symexp decoding.
# Reward normalization is bypassed in warmup and training; raw rewards are neither
# rescaled nor clipped. Observation normalization stays on. Global gradient
# clipping and value clipping are independent ablations. Value clipping bounds
# decoded displacement by +/-0.2 raw-return units, never symlog coordinates.
import json
import math
import os
import random
...
class Args:
    ...
    clip_coef: float = 0.2
    """lower policy ratio clipping coefficient and decoded-value clipping radius (raw units by default)"""
    clip_coef_upper: float = 0.2
    """upper policy ratio clipping coefficient"""
    clip_vloss: bool = True
    """clip decoded value displacement before comparing per-example critic losses"""
    critic_loss: Literal["mse", "symlog_mean", "symlog_mse"] = "symlog_mean"
    """scalar objective; symlog_mean decodes to expected return in training reward units"""
...
def symlog(x):
    """Signed log1p without saturation, with derivative one at the origin."""
    return torch.where(x >= 0, torch.log1p(x.clamp_min(0)), -torch.log1p((-x).clamp_min(0)))


def symexp(z):
    """Inverse symlog without saturation, with derivative one at the origin."""
    return torch.where(z >= 0, torch.expm1(z.clamp_min(0)), -torch.expm1((-z).clamp_min(0)))
...
def _scalar_value_loss_elements(prediction, targets, mode):
    """Elementwise objectives with the quadratic half factor left for reduction."""
    if mode == "mse":
        return (prediction - targets) ** 2
    if mode == "symlog_mse":
        return (prediction - symlog(targets)) ** 2
    if mode == "symlog_mean":
        ...
        return potential - targets * prediction + conjugate
...
def scalar_value_loss(prediction, targets, mode, mask=None):
    """Unclipped scalar objective over selected detached return targets.
    ...
    loss = _masked_mean(_scalar_value_loss_elements(prediction, targets, mode), mask)
    return loss if mode == "symlog_mean" else 0.5 * loss


def value_loss(prediction, targets, old_values, args, mask=None):
    """PPO critic loss with optional clipping in decoded return units.

    Targets and old decoded values are frozen. Compare the original and clipped
    objectives per example before reducing the valid mask. For symlog critics,
    only changed decoded values are re-encoded; inactive clipping retains the
    original coordinate and its gradient, avoiding a numerical round trip.
    """
    if not args.clip_vloss:
        return scalar_value_loss(prediction, targets, args.critic_loss, mask)
    targets = targets.detach()
    ...
    if mask is not None:
        ...
        prediction = torch.where(mask, prediction, 0.0)
        targets = torch.where(mask, targets, 0.0)
        old_values = torch.where(mask, old_values, 0.0)
    unclipped = _scalar_value_loss_elements(prediction, targets, args.critic_loss)
    decoded = prediction if args.critic_loss == "mse" else symexp(prediction)
    displacement = decoded - old_values
    clipped_value = old_values + torch.clamp(displacement, -args.clip_coef, args.clip_coef)
    if args.critic_loss == "mse":
        clipped_prediction = clipped_value
    else:
        active = (displacement < -args.clip_coef) | (displacement > args.clip_coef)
        clipped_prediction = torch.where(active, symlog(clipped_value), prediction)
    clipped = _scalar_value_loss_elements(clipped_prediction, targets, args.critic_loss)
    loss = _masked_mean(torch.max(unclipped, clipped), mask)
    # Keep v7's exact MSE order: half the mean of the pointwise squared-error max.
    return loss if args.critic_loss == "symlog_mean" else 0.5 * loss
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        super().__init__()
        self.critic_loss = Args.critic_loss if args is None else args.critic_loss
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args,
    ...
    v_loss = value_loss(newvalue, returns, old_values, args, critic_mask)
...
def main():
    ...
    try:
        ...
        writer.add_text("critic_objective", f"{args.critic_loss}; gradient clipping={args.grad_clip}; reward normalization={args.norm_reward}; value clipping={args.clip_vloss}; decoded GAE/V and value-clip radius in training reward units")
