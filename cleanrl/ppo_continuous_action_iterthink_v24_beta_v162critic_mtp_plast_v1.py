...
class Args:
    ...
    plast_gate_min: float = 0.0625   # lower clamp on the relative per-neuron post-Adam step gate (expansive: 1/16x)
    plast_gate_max: float = 16.0     # upper clamp on the relative per-neuron post-Adam step gate (expansive: 16x)
...
def plasticity_gates(agent, args):
    ...
    # Normalized to the layer mean and clamped: reallocates LR across neurons,
    # never globally choked. Returned as a one-element list (one gained layer).
    with torch.no_grad():
        c = agent.concentration()
        proxy = 2.0 / ((c + 1.0) * (c + 2.0))
        return [(proxy / proxy.mean()).clamp(args.plast_gate_min, args.plast_gate_max)]
