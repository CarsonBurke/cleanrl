# PPO + IterThink v105 CDF-reward variant (ARCHIVED -- regression).
#
# OUTCOME (HalfCheetah, seed 1, vs v105_rawrew softmax HL-Gauss baseline): led at
# every matched step to ~2.7M (3304 vs 2274 @2M) then plateaued/declined at ~3.7k
# while the baseline reached 6.2k @7.4M. Probes: decoded reward MAE converged 2.7x
# WORSE (0.27 vs 0.10 mtp1; AR dream 0.35 vs 0.20), bias ~0 for both. Diagnosis:
# per-threshold BCE meaned over K-1=100 thresholds gives the ~5 informative
# thresholds 1/100 weight each, so effective learning pressure on the decode is
# ~20x weaker than softmax-CE concentrated on its ~6 active bins. The geometry
# advantage (decode-error-proportional gradients) won early; the diluted fit lost
# late. Kept for reference; main line (v105_rawrew file) reverted to softmax CE.
#
# Change from v104:
...
#   - CDF REWARD HEAD ("HL-Gauss in CDF space"): the WM reward head is K-1 sigmoid
#     threshold logits predicting survival probabilities s_k = P(r > e_k) at the
#     K-1 interior bin EDGES e_k, trained with per-threshold BCE against the
#     smoothed labels y_k = Phi((r - e_k)/sigma) (the HL-Gauss target's survival
#     function) and decoded by the exact tail-sum identity
#     E[r] = v_min + bin_width * sum_k s_k. Rationale: softmax-CE prices an
#     eps mass misplacement at ~eps nats INDEPENDENT of how far it shifts the
#     expectation decode (eps*distance), so the loss is nearly blind to exactly the
#     errors the dream consumes; per-threshold BCE decomposes the decode error
#     exactly (d loss/d logit_k = s_k - y_k, decode error = bin_width * sum_k
#     (s_k - y_k)) while each threshold stays a proper scoring rule, so the full
#     CDF is still calibrated at optimum and the sigma smoothing is kept. Loss is
#     MEANED over thresholds (gradient l1 <= 1, comparable to softmax-CE's <= 2),
#     so lewm_reward_loss_coef is unchanged. sigma_ratio 0.5 -> 0.75 (HL-Gauss
#     paper-recommended smoothing). The critic keeps softmax HL-Gauss (its
#     range/signal ratio is mild; change one thing at a time).
...
from cleanrl.shared.hl_gauss import HLGaussCDFSupport, HLGaussSupport, symlog, symexp
...
class Args:
    ...
    lewm_reward_num_bins: int = 101   # K bin centers => K-1 CDF thresholds
    lewm_reward_v_min: float = -20.0
    lewm_reward_v_max: float = 50.0
    lewm_reward_sigma_ratio: float = 0.75  # HL-Gauss label smoothing (sigma / bin_width)
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.reward_num_thresholds = args.lewm_reward_num_bins - 1
        ...
        # CDF reward head: K-1 threshold logits (survival probabilities), not K bin logits.
        self.reward_outcome_unproj = xavier_linear(nn.Linear(self.dim, self.reward_num_thresholds))
    ...
    def imagine_step_from_history(self, latent_history, action_history):
        ...
        # current state, which both conditions the next-state prediction and decodes
        # its reward/continuation. _predictor_trunk auto-truncates to self.context.
        ...
        reward_logits, continuation_logits = self.decode_outcomes(
            next_summary, action_history[:, -1], detach_summary=False
        )
...
if __name__ == "__main__":
    ...
    # CDF reward head support: K-1 sigmoid thresholds, BCE vs smoothed survival
    # labels, exact tail-sum mean decode (see header).
    reward_support = HLGaussCDFSupport(
        args.lewm_reward_num_bins,
        args.lewm_reward_v_min,
        args.lewm_reward_v_max,
        args.lewm_reward_sigma_ratio,
        device,
        use_symlog=False,
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # Smoothed survival labels y_k = Phi((r - z_k)/sigma) for the CDF head.
                    reward_target_labels_flat = reward_support.cdf_labels(future_rewards.reshape(-1))
                    ...
                    with torch.no_grad():
                        # AUTOREGRESSIVE rollout along the REAL action sequence, feeding
                        ...
                        for h in range(horizon):
                            ...
                            (
                                probe_next_summary,
                                probe_reward_logits,
                                probe_continuation_logits,
                            ) = agent.world_model.imagine_step_from_history(
                                torch.stack(probe_hist_lat, dim=1),
                                torch.stack(probe_hist_act, dim=1),
                            )
                            w_h = latent_weight[:, h]
                            probe_latent_mse = F.mse_loss(
                                probe_next_summary[:, obs_start:],
                                target_summaries[:, h, obs_start:],
                                reduction="none",
                            ).mean(dim=(-1, -2))
                            probe_reward_scalar = reward_support.to_scalar(probe_reward_logits)
                            probe_reward_error = probe_reward_scalar - future_rewards[:, h]
                            probe_continuation_bce = F.binary_cross_entropy_with_logits(
                                probe_continuation_logits,
                                1.0 - future_terminations[:, h],
                                reduction="none",
                            )
                            wm_imagine_latent_mse_sum[h] += (probe_latent_mse * w_h).sum()
                            wm_imagine_reward_abs_error_sum[h] += (probe_reward_error.abs() * w_h).sum()
                    ...
                    reward_target_labels = reward_target_labels_flat.reshape(mb_size, horizon, -1)
                    ...
                    # CDF head: per-threshold BCE vs survival labels, MEANED over
                    # thresholds (gradient l1 <= 1, comparable to softmax-CE's <= 2).
                    target_reward_loss = F.binary_cross_entropy_with_logits(
                        target_reward_logits,
                        reward_target_labels.detach(),
                        reduction="none",
                    ).mean(dim=-1)
                    ...
                    for mtp_idx in range(mtp_len):
                        ...
                        reward_loss = F.binary_cross_entropy_with_logits(
                            ...
                            reward_target_labels[:, mtp_idx:].detach(),
                            reduction="none",
                        ).mean(dim=-1)
                        ...
                        with torch.no_grad():
                            ...
                            # CDF analogue of edge mass: implied probability below the
                            # first threshold plus above the last one.
                            pred_surv = torch.sigmoid(
                                pred_reward_logits_all[:, :valid_horizon, mtp_idx]
                            )
                            pred_edge_mass = (1.0 - pred_surv[..., 0]) + pred_surv[..., -1]
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/reward_bce", float(np.mean(wm_reward_losses)), global_step)
