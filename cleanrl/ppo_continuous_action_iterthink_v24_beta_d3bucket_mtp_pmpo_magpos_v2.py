# PPO + IterThink v24 Beta + d3bucket MTP + PMPO + MAGPOS v2 (bounded-proportional pos weight).
# =====================================================================================
# v2 MOTIVATION: magpos_v1 (pos=magnitude, UNBOUNDED |A|/std) won HalfCheetah +18.5% (9221 vs 7780)
# but REGRESSED ~3x on Walker2d (matched @3M: 738 vs baseline 2049). Diagnosis: HalfCheetah (no
# early termination, dense reward) rewards unbounded amplification of the best actions; Walker
# (termination-sensitive gait) is destabilized by it -- unbounded weighting over-amplifies high-
# variance near-fall episodes; the tanh BOUND is protective. The two envs want OPPOSITE things, so
# no static UNbounded knob generalizes. v2 HYPOTHESIS: a PROPORTIONAL-but-BOUNDED positive weight
# keeps HalfCheetah's "reinforce better actions more" edge while restoring Walker's tail bound.
# Two new pmpo_pos_weight options (negatives + everything else IDENTICAL to v1/pmpo_v1):
#   "rank"    = percentile rank among positive advantages, in (0,1] -- proportional, inherently
#               bounded, robust to fat tails (a termination outlier -> rank ~1.0, not an unbounded wt).
#   "stdtanh" = tanh(|A|/std) -- std-normalized then bounded; proportional in the bulk, saturating tail.
# TARGET: BEAT the tanh baseline on BOTH HalfCheetah (7780) and Walker2d simultaneously -- a general
# win, even if below v1's HalfCheetah-specific 9221. If bounding erases the HalfCheetah gain entirely,
# the gain was fundamentally the unbounded tail and the conflict is irreducible with a static weight.
#
# === RESULT (seed 1, matched-step): CONFLICT IS IRREDUCIBLE. Neither bounded variant is a general win. ===
#   HalfCheetah (magpos-unbounded ref @2M/3M/4M = 3513/5040/6026 -> 9221; tanh baseline 7780):
#     rank    @2M/3M/4M = 2573/3737/4388 (mean ~6116 @6.9M -> heading ~6.5k, BELOW baseline 7780)
#     stdtanh @2M/3M/4M = 2479/3428/4147 (mean ~4931 @7.0M -> heading ~5.5k, WELL below baseline)
#     => bounding the positive weight doesn't just shed magpos's gain, it lands STRICTLY BELOW the
#        plain tanh baseline. The HalfCheetah gain IS the unbounded tail; rank/stdtanh over-weight
#        MARGINAL positives (rank gives the median positive ~0.5 regardless of |A|; stdtanh inflates
#        small |A|/std), which is a worse general shape than tanh's proportional-near-0 + bounded-tail.
#   Walker2d (tanh baseline @2M/3M = 2120/2049; magpos-unbounded 594/738):
#     rank @2M/3M = 563/730 -- as broken as magpos early; partially recovers late (~1848 @6M) but never
#     cleanly beats the baseline at matched steps.
#   CONCLUSION: NO static positive-weight reshape beats the tanh baseline on BOTH envs. The advantage-
#     weighting axis is EXHAUSTED. tanh (proportional near 0, bounded tail) is the best GENERAL positive
#     shape; magnitude/unbounded (magpos_v1) is the best HalfCheetah-SPECIFIC shape (+18.5%, 9221) but
#     Walker cannot tolerate it. A genuinely general gain needs a different lever -- a real EXPLORATION
#     mechanism (temporally-coherent/state-correlated noise, critic-uncertainty intrinsic reward), not
#     advantage reshaping. DELIVERABLE stands: magpos_v1 tanhneg = 9221 on HalfCheetah (+18.5%).
# =====================================================================================
...
class Args:
    ...
    #     reinforces good actions proportionally -- HalfCheetah-specific win, hurts Walker),
    #     "rank": percentile rank among positives in (0,1] (proportional+bounded, v2 general attempt),
    #     "stdtanh": tanh(|A|/std) (std-normalized then bounded, v2 general attempt)}
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if pmpo_active:
                    ...
                    elif args.pmpo_pos_weight == "stdtanh":
                        # std-normalized THEN bounded: proportional in the bulk, saturates the tail.
                        # A middle ground between tanh-baseline (bounded, no magnitude) and magnitude
                        # (unbounded). Aims to keep HalfCheetah's proportional reinforcement while
                        # restoring the tail bound that protects Walker.
                        w_pos = (absA / adv_std).tanh()
                        if args.pmpo_pos_mag_pow != 1.0:
                            w_pos = w_pos.pow(args.pmpo_pos_mag_pow)
                    elif args.pmpo_pos_weight == "rank":
                        # Percentile rank among POSITIVE advantages, in (0,1]: proportional (higher A
                        # -> higher weight, like magnitude) but INHERENTLY BOUNDED and robust to fat
                        # tails -- a termination-driven outlier gets rank ~1.0, not an unbounded weight.
                        # Directly targets the HalfCheetah(wants unbounded)-vs-Walker(wants bounded)
                        # conflict that made pos=magnitude HalfCheetah-specific.
                        w_pos = torch.zeros_like(absA)
                        pos_sel = mb_advantages >= 0
                        if pos_sel.any():
                            pa = mb_advantages[pos_sel]
                            r = pa.argsort().argsort().float()  # dense ranks 0..P-1 ascending
                            w_pos[pos_sel] = (r + 1.0) / pa.numel()  # (0,1], largest positive -> 1.0
                        if args.pmpo_pos_mag_pow != 1.0:
                            w_pos = w_pos.pow(args.pmpo_pos_mag_pow)
                    else:
