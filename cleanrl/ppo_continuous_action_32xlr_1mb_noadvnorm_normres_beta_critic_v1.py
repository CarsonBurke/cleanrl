# Ctrl-base PPO with Beta-conditioned critic, v1.
#
# Keep ctrl_base's additive pre-RMS SiTU-GLU trunk, Beta policy, full-batch
# updates, raw GAE, and 32x learning rate. The critic receives the current
# observation plus the actor's emitted Beta parameters (alpha, beta) for that
# observation: V(s, alpha_pi(s), beta_pi(s)). The distribution features are
# detached before entering the critic, so value regression cannot directly
# update the actor through the critic input path.
#
# Hypothesis: the actor's concentration and mean information gives the critic
# a policy-aware baseline without adding recurrent state or sampled-action
# variance.
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, placement="pre", norm_kind="layer", activation="lrelusq"):
        ...
        critic_input_dim = observation_dim + 2 * self.action_dim
        self.critic = nn.Sequential(
            make_norm_residual_trunk(critic_input_dim, 64, placement=placement,
    ...
    def _policy_parameters(self, observations):
        return (F.softplus(self.actor(observations)) + 1.0).chunk(2, dim=-1)

    def _critic_input(self, observations, alpha, beta):
        return torch.cat((observations.flatten(1), alpha.detach(), beta.detach()), dim=-1)

    def get_value(self, observations):
        alpha, beta = self._policy_parameters(observations)
        return self.critic(self._critic_input(observations, alpha, beta))

    def get_policy_and_value(self, observations):
        alpha, beta = self._policy_parameters(observations)
        value = self.critic(self._critic_input(observations, alpha, beta))
    ...
    def get_action_and_value(self, observations, action=None):
        """Public API uses physical actions; critic sees current Beta parameters."""
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", f"Beta host actor; critic sees detached alpha/beta; "
