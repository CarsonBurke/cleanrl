# PPO + IterThink v178 PMPO-revkl-actorsym10-beliefonly-jointsigreg-pretemporalwm-dreamer3bucket-hlgauss-wmloss1024-cudagraph-contdisc-k6.
#
# v178-PMPO = v178 (belief-only, no batchnorm, symmetric actor_update_epochs=10) with the
# PPO ratio-clip actor objective REPLACED by PMPO (Probability-Matching Policy Optimization).
#   - HYPOTHESIS. v166's symmetric 10-epoch actor regime worked because the PPO clip-higher
#     ratio AND the KL/clipfrac leash kept the policy on-distribution across 10 epochs. v167
#     cut actor epochs 1 and removed that load-bearing KL leash; the regression localizes there.
#     v178 restored 10 epochs but leaned on PPO's clip alone. PMPO restores the leash in a
#     principled, importance-ratio-FREE form: the actor loss is a sign/magnitude-split
#     log-likelihood plus an EXPLICIT reverse-KL(old||new) trust region.
#   - CHANGE. policy_objective="pmpo". The actor loss per minibatch is
#       pg = -w * mean_pos(|tanh(adv)| * logp_dim) + (1-w) * mean_neg(|tanh(adv)| * logp_dim)
#            + pmpo_kl_coef * KL(old || new)
#     NO importance-sampling ratio, NO clipping. The advantage enters ONLY via (a) the sign,
#     splitting samples into a positive group (pulled UP) and a negative group (pushed DOWN),
#     and (b) a tanh-saturated |magnitude| weight. The reverse-KL(old||new) penalty IS the
#     trust region that makes symmetric actor_update_epochs=10 safe (replacing the clip leash).
#     Standard advantage normalization (norm_adv/ret_percnorm) is NOT applied to the PG; PMPO
#     uses the true (pre-norm) advantage sign + tanh magnitude only. Applied to BOTH the real
#     and imagined PG streams; the imagined "old" policy is the dream-generation policy, so the
#     imagined reverse-KL is faithful (old-dist params buffered at dream time).
#   - DISTRIBUTION-AGNOSTIC. KL/old-dist handling works for the Beta default AND the Gaussian
#     branch via torch.distributions.kl.kl_divergence (Beta||Beta, Normal||Normal) and the
#     make_actor_dist/actor_dist_params helpers (Beta: concentration1/0; Normal: loc/scale).
#   - A/B. policy_objective="ppo" recovers the exact v178 clip-higher objective (the new code
#     is gated on the flag), so this file is an internal A/B of PMPO vs PPO-clip.
#   - belief-only agent input (obs_cond_mode="none"), NO batchnorm, joint SIGReg, purely
#     temporal world model: otherwise identical to v176.1.
#
# -----------------------------------------------------------------------------
# v176.1 (inherited): BELIEF-ONLY agent input (obs_cond_mode="none"), reverting the
...
class Args:
    ...
    actor_update_epochs: int = 10  # v178: SYMMETRIC actor/critic epochs (revert v167's actor1 asymmetry, like v166)
    ...
    # v178-PMPO: actor objective selector and PMPO trust-region knobs.
    #   "pmpo": sign/tanh|adv|-split log-likelihood + reverse-KL(old||new) trust region
    #           (NO importance ratio, NO clip; norm_adv/ret_percnorm NOT applied to the PG).
    #   "ppo":  recovers the v178 clip-higher ratio objective (internal A/B fallback).
    policy_objective: str = "pmpo"
    pmpo_kl_coef: float = 0.3        # reverse-KL(old||new) trust-region weight (PMPO's leash)
    pmpo_reverse_kl: bool = True     # True=KL(old||new) (reverse); False=KL(new||old) (forward)
    pmpo_pos_to_neg_weight: float = 0.5  # pos/neg log-likelihood weighting w (d4 default 0.5)
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # v174: auxiliary cross-attention that reads the reward/continuation semantic
        # tokens off the time-mixed obs belief (outside the recurrent stream).
        self.semantic_readout = _SpaceCrossReadout(self.dim, args.lewm_heads)
    ...
    def normalize_world_model_matrices(self):
        ...
        output_projection_names = (
            "attn.proj.weight",
            "w2.weight",
            "semantic_readout.proj.weight",  # v174: readout output proj normalizes on dim=0
        )
    ...
    def _predictor_trunk(self, latent_history, action_history):
        # v174: PURELY TEMPORAL predictor over the OBS-ONLY recurrent stream
        # (B, T, num_obs_tokens, dim); semantic tokens are never stored here. The obs
        # tokens are propagated per token across time (cross-token mixing was moved to
        # the encoder, pre-SIGReg). The reward/continuation semantic tokens are then
        # READ OFF the time-mixed obs belief by an auxiliary cross-attention -- outside
        # the recurrent stream and downstream of the obs prediction, so the obs targets
        # stay a pure temporal function and the SIGReg'd representation is untouched.
        ...
        # Time-only dynamics on the obs tokens. The action is injected as AdaLN
        # conditioning (c), never as an input token.
        obs_tokens = unit_norm(latent_history)
        action_features = unit_norm(self.action_cond(action_history))
        for layer in self.predictor_layers:
            obs_tokens = layer(obs_tokens, action_features)
        obs_belief = unit_norm(self.predictor_norm(obs_tokens))  # (B, T, num_obs, dim)
        # Auxiliary readout: semantic queries cross-attend the obs belief per (B, T).
        sem_query = self.predictor_semantic_query.to(obs_belief.dtype)
        sem_query = sem_query.view(1, self.num_semantic_tokens, width).expand(
            batch * context_len, -1, -1
        )
        obs_flat = obs_belief.reshape(batch * context_len, num_obs, width)
        sem_belief = unit_norm(self.semantic_readout(sem_query, obs_flat))
        sem_belief = sem_belief.reshape(batch, context_len, self.num_semantic_tokens, width)
        # belief = [semantic readout, obs belief] -- num_latent_tokens wide, same
        # contract as before (semantic slots first, then obs).
        return torch.cat([sem_belief, obs_belief], dim=2)
...
class Agent(nn.Module):
    ...
    def actor_dist_params(self, dist):
        # v178-PMPO: distribution-agnostic (p1, p2) extraction so old actor dists can be
        # buffered as plain tensors and rebuilt later for the reverse-KL trust region.
        # Beta -> (alpha=concentration1, beta=concentration0); Normal -> (loc, scale).
        if self.actor_dist == "gaussian":
            return dist.loc, dist.scale
        return dist.concentration1, dist.concentration0

    def make_actor_dist(self, p1, p2):
        # v178-PMPO: inverse of actor_dist_params. Rebuild a Beta/Normal from buffered
        # params (used for KL(old||new) and KL(new||old); both supported by kl_divergence).
        if self.actor_dist == "gaussian":
            return Normal(p1, p2)
        return Beta(p1, p2)
    ...
    def agent_input_from_obs(self, x):
        ...
        latent = self.world_model.encode_summary(x)
        obs_only = latent[:, self.world_model.obs_token_start :].unsqueeze(1)
        neutral = x.new_zeros(x.shape[0], 1, self.act_dim)
        belief = self.world_model.belief_from_history(obs_only, neutral)
    ...
    def get_action_and_value_from_agent_input(self, obs, agent_input, z=None, return_dist=False):
        ...
        # v178-PMPO: return_dist=True additionally returns (p1, p2, log_prob_per_dim) where
        # (p1, p2) are the actor-dist params (tensors, NOT a dist object — this forward is
        # torch.compile/cudagraph-wrapped, so it must return tensors; the dist is rebuilt
        # outside via make_actor_dist) and log_prob_per_dim is the PER-DIMENSION native
        # log-prob (dist.log_prob(z) - log_det_fn(z), before the .sum(1)) for PMPO's
        # sign/magnitude split. Default call sites are unchanged (5-tuple).
        ...
        action = to_action(z)
        log_prob_per_dim = dist.log_prob(z) - log_det_fn(z)
        ...
        if return_dist:
            p1, p2 = self.actor_dist_params(dist)
            return action, z, log_prob, entropy, value_logits, p1, p2, log_prob_per_dim
    ...
    def _fusion_params(self):
        ...
        return list(self.belief_conditioner.parameters()) if self.obs_cond_mode == "film" else []
...
if __name__ == "__main__":
    ...
    assert args.policy_objective in ("pmpo", "ppo")
    ...
    def masked_token_sigreg_static(token_latents, token_valids, A):
        # v175: JOINT (flattened) SIGReg. Per rollout step, flatten the S obs-token
        # slots into one (S*D) per-frame vector and whiten its JOINT batch-marginal,
        # restoring le-wm's "whiten the whole per-frame latent" semantics (le-wm runs
        # SIGReg on (T, B, D_frame); here D_frame is factored into S*D and concatenated
        # back). Random projections mix dims ACROSS tokens, so cross-token correlation
        # is penalized (per-slot v174 could not see it). Static-shape / tensor-masked
        # for CUDA-graph capture; per-step batch marginal preserved, averaged over steps.
        # token_latents: (B, H, S, D), token_valids: (B, H), A: (S*D, P)
        step_losses = []
        ...
        for h in range(token_latents.shape[1]):
            step_flat = token_latents[:, h].reshape(batch, -1)  # (B, S*D)
    ...
    def wm_supervised_loss_forward(
        ...
        encoded_sequence = wm_supervised_encode_summary(obs_sequence.reshape((-1,) + obs_anchor.shape[1:])).reshape(
            mb_size,
            horizon + 1,
        ...
        initial_obs_summary = initial_summary[:, obs_start:]
        target_obs_summaries = target_summaries[:, :, obs_start:]
        teacher_history = torch.cat(
            [
                initial_obs_summary.unsqueeze(1),
                target_obs_summaries[:, :-1],
            ],
            dim=1,
        )
        ...
        one_step_pred = pred_mtp[:, :, 0, obs_start:]
        one_step_latent_cos = (one_step_pred * target_obs_summaries).sum(dim=-1).mean(dim=-1)
        one_step_latent_loss = 1.0 - one_step_latent_cos
        one_step_latent_mse = F.mse_loss(one_step_pred, target_obs_summaries, reduction="none").mean(dim=(-1, -2))
        ...
        sigreg_latents = torch.cat(
            [
                initial_obs_summary.unsqueeze(1),
                target_obs_summaries,
            ],
            dim=1,
        )
        sigreg_latents = sigreg_latents * (sigreg_latents.shape[-1] ** 0.5)
        wm_sigreg_loss = masked_token_sigreg_static(sigreg_latents, sigreg_weight, sigreg_A)
    ...
    def wm_supervised_metrics_forward(
        ...
        sigreg_latents = torch.cat([initial_obs_summary.unsqueeze(1), target_obs_summaries], dim=1)
        sigreg_latents = sigreg_latents * (sigreg_latents.shape[-1] ** 0.5)
        wm_sigreg_loss = masked_token_sigreg_static(sigreg_latents, sigreg_weight, sigreg_A)
        wm_discount_bce = discount_bce_sum
        wm_discount_bce_excess = discount_bce_sum - discount_entropy_sum
    ...
    # v178-PMPO: buffer the OLD actor-dist params (the policy that generated each action) so
    # the reverse-KL(old||new) trust region can be evaluated in the update. Distribution
    # agnostic (p1,p2)=Beta(alpha,beta) or Normal(loc,scale); shaped (num_steps,num_envs,act_dim).
    old_action_p1 = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device)
    old_action_p2 = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device)
    ...
    def build_belief_agent_input(obs_tensor):
        current_latent_full = agent.world_model.encode_summary(obs_tensor)
        ...
        # Recurrent stream stores obs tokens only; semantic slots never fed back.
        current_latent = current_latent_full[:, agent.world_model.obs_token_start :]
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if not rollout_actor_active and args.actor_dist == "beta":
                    ...
                    # WM-warmup rows never reach the PPO/PMPO update (world_model_only skips it);
                    # store neutral Beta(1,1)=uniform params defensively so the buffer stays valid.
                    old_p1 = torch.ones_like(action)
                    old_p2 = torch.ones_like(action)
                else:
                    action, z, logprob, ent, value_logits, old_p1, old_p2, _ = (
            ...
            actions[step] = action
            latent_zs[step] = z
            old_action_p1[step] = old_p1
            old_action_p2[step] = old_p2
        ...
        for _ in range(args.lewm_update_epochs):
            ...
            for start in range(0, args.batch_size, wm_minibatch_size):
                ...
                sigreg_A = sigreg.sample_projection(
                    # v175: joint SIGReg projects the flattened obs-token frame vector.
                ...
                # v175 probe: cross-token effective rank of the encoder obs summaries.
                # joint = rank of the flattened (S*D) frame vector; pertoken = mean
                # per-slot (D) rank. joint/pertoken in [1, S] => cross-token diversity.
                _summ = target_obs_summaries[:, 0]               # (B, S, D), anchor-next step
        ...
        b_old_p1 = old_action_p1.reshape((-1,) + envs.single_action_space.shape)  # v178-PMPO: old-dist params
        b_old_p2 = old_action_p2.reshape((-1,) + envs.single_action_space.shape)
        ...
        reverse_kl = torch.zeros((), device=device)        # v178-PMPO: mean reverse-KL(old||new)
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if actor_active:
                    if args.policy_objective == "pmpo":
                        # v178-PMPO: forward also returns the new actor-dist params (rebuilt to a
                        # dist below for KL) and the per-dimension native log-prob.
                        (_, _, newlogprob, entropy, value_logits,
                         new_p1, new_p2, log_prob_per_dim) = agent.get_action_and_value_from_agent_input(
                            b_obs[mb_inds],
                    ...
                    with torch.no_grad():
                        # Diagnostics only (logged for BOTH objectives); PMPO does NOT use the ratio.
                    ...
                    # Pre-normalization (true-sign) shaped advantage: PMPO's sign split and the
                    # PPO pos/neg weighting both key off this; PPO additionally normalizes it.
                    ...
                    if args.policy_objective == "pmpo":
                        # PMPO actor loss: sign/magnitude-split log-likelihood + reverse-KL
                        # trust region. NO importance ratio, NO clip; norm_adv/ret_percnorm are
                        # NOT applied (the advantage enters ONLY via its true sign and a
                        # tanh-saturated |magnitude| weight).
                        adv_weight = mb_raw_adv.tanh().abs().unsqueeze(-1)
                        signed_log_probs = log_prob_per_dim * adv_weight
                        pos_mask = (mb_raw_adv >= 0).unsqueeze(-1).expand_as(log_prob_per_dim)
                        neg_mask = (mb_raw_adv < 0).unsqueeze(-1).expand_as(log_prob_per_dim)
                        ...
                        # Reverse-KL(old||new) trust region (distribution-agnostic). new dist is
                        # rebuilt OUTSIDE the compiled forward from its returned params so grad
                        # still flows to the actor heads; old dist is rebuilt from the buffer.
                        new_dist = agent.make_actor_dist(new_p1, new_p2)
                        old_dist = agent.make_actor_dist(b_old_p1[mb_inds], b_old_p2[mb_inds])
                        reverse_kl_mb = kl_divergence(old_dist, new_dist).sum(-1).mean()
                        reverse_kl_sum = reverse_kl_sum + reverse_kl_mb.detach() * len(mb_inds)
                        reverse_kl_count += len(mb_inds)
                        if args.pmpo_kl_coef > 0.0:
                            if args.pmpo_reverse_kl:
                                kl_loss = reverse_kl_mb
                            else:
        ...
        if reverse_kl_count > 0:
            reverse_kl = reverse_kl_sum / reverse_kl_count   # v178-PMPO: mean over minibatches
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            im_reverse_kls = []   # v178-PMPO: imagined reverse-KL(old||new) per actor update
            im_blocks = 0
            im_actor_updates = 0
            im_critic_updates = 0
            for _ in range(args.imagine_batches):
                with torch.no_grad():
                    ...
                    im_p1s, im_p2s = [], []   # v178-PMPO: dream-generation (old) actor-dist params
                    ...
                    for t in range(H_im):
                        ...
                        action, z, logprob, _, value_logits, gen_p1, gen_p2, _ = (
                            agent.get_action_and_value_from_agent_input(None, agent_input, return_dist=True)
                        )
                        im_p1s.append(gen_p1)   # v178-PMPO: old (dream-generation) dist params
                        im_p2s.append(gen_p2)
                    ...
                    f_old_p1 = torch.stack(im_p1s, dim=0).reshape(H_im * B_im, -1)  # v178-PMPO old dist
                    f_old_p2 = torch.stack(im_p2s, dim=0).reshape(H_im * B_im, -1)
                ...
                for im_epoch in range(args.update_epochs):
                    ...
                    reverse_kl_im = torch.zeros((), device=device)
                    if actor_active_im:
                        if args.policy_objective == "pmpo":
                            (_, _, newlogprob, entropy, value_logits,
                             new_p1_im, new_p2_im, log_prob_per_dim_im) = (
                                agent.get_action_and_value_from_agent_input(
                                    None, f_ag, f_z, return_dist=True
                                )
                            )
                        else:
                            _, _, newlogprob, entropy, value_logits = agent.get_action_and_value_from_agent_input(
                        ...
                        ratio = logratio.exp()
                        # Pre-normalization (true-sign) shaped imagined advantage.
                        im_raw_adv = shape_advantage(f_adv, f_sigma, f_u, args, device)
                        if args.policy_objective == "pmpo":
                            # PMPO on the dream stream: same sign/tanh|adv|-split log-likelihood,
                            # NO ratio/clip, NO advantage normalization. The reverse-KL "old" policy
                            # is the dream-GENERATION policy (params buffered at dream time), so the
                            # imagined trust region is fully faithful (NOT omitted).
                            adv_weight_im = im_raw_adv.tanh().abs().unsqueeze(-1)
                            signed_log_probs_im = log_prob_per_dim_im * adv_weight_im
                            pos_mask_im = (im_raw_adv >= 0).unsqueeze(-1).expand_as(log_prob_per_dim_im)
                            neg_mask_im = (im_raw_adv < 0).unsqueeze(-1).expand_as(log_prob_per_dim_im)
                            pos_count_im = pos_mask_im.float().sum().clamp_min(1.0)
                            neg_count_im = neg_mask_im.float().sum().clamp_min(1.0)
                            pos_loss_im = signed_log_probs_im[pos_mask_im].sum() / pos_count_im
                            neg_loss_im = signed_log_probs_im[neg_mask_im].sum() / neg_count_im
                            pg_loss_im = (
                                -args.pmpo_pos_to_neg_weight * pos_loss_im
                                + (1.0 - args.pmpo_pos_to_neg_weight) * neg_loss_im
                            )
                            new_dist_im = agent.make_actor_dist(new_p1_im, new_p2_im)
                            old_dist_im = agent.make_actor_dist(f_old_p1, f_old_p2)
                            reverse_kl_im = kl_divergence(old_dist_im, new_dist_im).sum(-1).mean()
                            if args.pmpo_kl_coef > 0.0:
                                if args.pmpo_reverse_kl:
                                    kl_loss_im = reverse_kl_im
                                else:
                                    kl_loss_im = kl_divergence(new_dist_im, old_dist_im).sum(-1).mean()
                                pg_loss_im = pg_loss_im + args.pmpo_kl_coef * kl_loss_im
                    ...
                    if actor_active_im:
                        ...
                        im_reverse_kls.append(reverse_kl_im.item())   # v178-PMPO
                        im_actor_updates += 1
            ...
            imagine_metrics = {
                ...
                "charts/reverse_kl_imagined": float(np.mean(im_reverse_kls)) if im_reverse_kls else 0.0,
        ...
        writer.add_scalar("charts/reverse_kl", reverse_kl.item(), global_step)   # v178-PMPO trust-region size
