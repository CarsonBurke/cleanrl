# PPO + IterThink v196 affine-RMSNorm persistent Beta exploration + plain GAE.
#
# Preserve the v145 world model exactly: raw observations, learned affine
# scalar tokens, RMSNorm, attached prediction targets/SIGReg, and a detached agent.
# Retain v195's actor-only persistent Gaussian code. A learned state-dependent bilinear
# basis turns that code into an antisymmetric shift of the Beta concentration
# logits, creating coherent exploration without changing the WM. Replace RankGauss
# entirely with raw GAE followed by canonical per-minibatch mean/std normalization;
# advantage magnitudes and zero crossing remain part of PPO's credit assignment.
#
# -----------------------------------------------------------------------------
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if not self.detach_world_model_from_agent:
            raise ValueError("v196 requires a strictly detached world-model/agent interface")
    ...
    def get_action_and_value_from_agent_input(
        self, obs, agent_input, z=None, explore_code=None
    ):
...
def shape_advantage(gae, sigma, u, args, device):
    ...
    if args.adv_transform != "v10":
        raise ValueError("v196 supports plain GAE only")
    return gae
...
if __name__ == "__main__":
    ...
    assert args.detach_world_model_from_agent, "v196 requires PPO to be detached from the WM"
    assert args.adv_transform == "v10", "v196 deliberately forbids ranked advantage transforms"
    assert args.norm_adv and args.norm_adv_scope == "minibatch", (
        "v196 uses canonical per-minibatch advantage normalization"
    )
    assert args.pos_neg_alpha == 0.5, "v196 preserves the plain GAE sign and weighting"
