# PPO + IterThink v24.fpo.3 (BOUNDED-ENDPOINT FPO + v24 CONTROLS). From v24.fpo.2.
#
# v1 faithfully used reference FPO's raw action endpoint and applied tanh only for
# the environment. In Gym MuJoCo + v24 machinery that let raw actions drift to large
# magnitudes while tanh saturated, so CFM chased an unidentifiable raw coordinate.
# v2 treated the bounded env action itself as the flow endpoint: x_1 ~ N(0,I),
# x_0 in [-1,1]. The stored action, CFM target, and env action are the same bounded
# tensor. v3 keeps that endpoint but restores the v24 beta-control machinery
# (shared trunk, decoupled clipping, rankgauss, minibatch norm, KL leash) because v2's
# FPO-parity defaults collapsed harder than raw-action v1.
#
# Hypothesis: FPO's weak point here is ratio/action geometry, not v24's robust policy
# machinery. Restoring v24 controls should make the bounded CFM ratio trainable enough
# to test whether coordinated flow exploration adds value over Beta.
#
...
class Args:
    ...
    share_backbone: bool = True      # v3: restore the beta-control representation path
    separate_grad_clip: bool = True  # v3: restore decoupled actor/critic clipping
    ...
    # v24.fpo.2: "fpo" is the default bounded-endpoint flow-matching policy. "beta" and "gaussian"
    # remain as controls from v24.
    actor_dist: str = "fpo"
    # FPO policy parameters, ported from ../fpo/playground with CleanRL shapes.
    ...
    fpo_clipping_epsilon: float = 0.05
    fpo_target_kl: Optional[float] = 0.03
    fpo_output_mode: str = "u_but_supervise_as_eps"  # "u" | "u_but_supervise_as_eps"

    # v24 controls. "beta" (unimodal, dreamer4 default) | "gaussian"
...
class FPOFlowPolicy(nn.Module):
    """Time-conditioned velocity field used by Flow Policy Optimization."""

    def __init__(self, feature_dim, action_dim, timestep_embed_dim, hidden, output_scale):
        super().__init__()
        ...
        self.net = nn.Sequential(
            layer_init(nn.Linear(feature_dim + action_dim + timestep_embed_dim, hidden)),
            nn.SiLU(),
    ...
    def forward(self, actor_feat, x_t, t):
        ...
        return self.net(torch.cat([actor_feat, x_t, t_embed], dim=-1)) * self.output_scale
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v24.fpo.1: FPO flow default, with v24 beta/diagonal controls.
        self.actor_dist = args.actor_dist
        if self.actor_dist == "fpo":
            self.fpo_flow = FPOFlowPolicy(
                H,
                act_dim,
                args.fpo_timestep_embed_dim,
                H,
                args.fpo_policy_output_scale,
            )
            self.fpo_flow_steps = int(args.fpo_flow_steps)
            self.fpo_n_samples_per_action = int(args.fpo_n_samples_per_action)
            self.fpo_average_losses_before_exp = bool(args.fpo_average_losses_before_exp)
            self.fpo_discretize_t_for_training = bool(args.fpo_discretize_t_for_training)
            self.fpo_source_std = float(args.fpo_source_std)
            self.fpo_feather_std = float(args.fpo_feather_std)
            self.fpo_loss_clip = float(args.fpo_loss_clip)
            self.fpo_output_mode = args.fpo_output_mode
            if self.fpo_output_mode not in ("u", "u_but_supervise_as_eps"):
                raise ValueError(f"unknown fpo_output_mode {self.fpo_output_mode}")
    ...
    def _fpo_schedule(self, device, dtype):
        return torch.linspace(1.0, 0.0, self.fpo_flow_steps + 1, device=device, dtype=dtype)

    def _fpo_cfm_loss_from_feat(self, actor_feat, action, eps, t):
        x_t = t * eps + (1.0 - t) * action.unsqueeze(-2)
        velocity_pred = self.fpo_flow(actor_feat, x_t, t)
        velocity_gt = eps - action.unsqueeze(-2)
        if self.fpo_output_mode == "u":
            loss = (velocity_pred - velocity_gt).pow(2).mean(dim=-1)
        else:
            x0_pred = x_t - t * velocity_pred
            x1_pred = x0_pred + velocity_pred
            loss = (eps - x1_pred).pow(2).mean(dim=-1)
        return loss

    def _sample_fpo_action(self, actor_feat, deterministic=False):
        batch_shape = actor_feat.shape[:-1]
        device, dtype = actor_feat.device, actor_feat.dtype
        schedule = self._fpo_schedule(device, dtype)
        x_t = torch.randn(*batch_shape, self.fpo_flow.action_dim, device=device, dtype=dtype) * self.fpo_source_std
        for i in range(self.fpo_flow_steps):
            t_now = schedule[i].expand(*batch_shape, 1)
            dt = schedule[i + 1] - schedule[i]
            x_t = x_t + dt * self.fpo_flow(actor_feat, x_t, t_now)
        if (not deterministic) and self.fpo_feather_std > 0.0:
            x_t = x_t + torch.randn_like(x_t) * self.fpo_feather_std
        return x_t.clamp(-1.0 + SAMPLE_EPS, 1.0 - SAMPLE_EPS)

    def get_fpo_endpoint_stats(self, x):
        ...
        for i in range(self.fpo_flow_steps):
            ...
            dt = schedule[i + 1] - schedule[i]
            x_t = x_t + dt * self.fpo_flow(actor_feat, x_t, t_now)
        bounded = x_t.clamp(-1.0 + SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        return {
            "raw_std": x_t.std(),
            "raw_absmax": x_t.abs().max(),
            "raw_clip_frac": (x_t.abs() > 1.0).float().mean(),
            "bounded_saturation_frac": (bounded.abs() > 0.99).float().mean(),
        }

    def _sample_fpo_loss_terms(self, actor_feat, action):
        batch_shape = action.shape[:-1]
        device, dtype = action.device, action.dtype
        sample_shape = (*batch_shape, self.fpo_n_samples_per_action)
        eps = torch.randn(*sample_shape, self.fpo_flow.action_dim, device=device, dtype=dtype) * self.fpo_source_std
        if self.fpo_discretize_t_for_training:
            schedule = self._fpo_schedule(device, dtype)[:-1]
            idx = torch.randint(self.fpo_flow_steps, (*sample_shape, 1), device=device)
            t = schedule[idx]
        else:
            t = torch.rand(*sample_shape, 1, device=device, dtype=dtype)
        initial_loss = self._fpo_cfm_loss_from_feat(actor_feat, action, eps, t)
        return eps, t, initial_loss

    def evaluate_fpo_action_and_value(self, x, action, eps, t, old_cfm_loss):
        actor_feat, critic_feat = self._trunks(x)
        cfm_loss = self._fpo_cfm_loss_from_feat(actor_feat, action, eps, t)
        if self.fpo_average_losses_before_exp:
            logratio = old_cfm_loss.mean(dim=-1, keepdim=True) - cfm_loss.mean(dim=-1, keepdim=True)
        else:
            logratio = old_cfm_loss - cfm_loss
            logratio = logratio.clamp(-self.fpo_loss_clip, self.fpo_loss_clip)
        ratio = logratio.exp()
        entropy = torch.zeros(action.shape[0], device=action.device, dtype=action.dtype)
        value_logits = self.critic_head(critic_feat)
        return ratio, logratio, entropy, value_logits, cfm_loss
    ...
    def get_action_and_value(self, x, z=None):
        ...
        actor_feat, critic_feat = self._trunks(x)
        if self.actor_dist == "fpo":
            action = self._sample_fpo_action(actor_feat, deterministic=False)
            eps, t, initial_cfm_loss = self._sample_fpo_loss_terms(actor_feat, action)
            log_prob = torch.zeros(action.shape[0], device=action.device, dtype=action.dtype)
            entropy = torch.zeros_like(log_prob)
            value_logits = self.critic_head(critic_feat)
            return action, action, log_prob, entropy, value_logits, eps, t, initial_cfm_loss
    ...
    def actor_parameters(self):
        ...
        if self.actor_dist == "fpo":
            heads = list(self.fpo_flow.parameters())
        elif self.actor_dist == "gaussian":
...
if __name__ == "__main__":
    ...
    assert torch.cuda.is_available() and args.cuda, "v24.fpo.1 is configured for CUDA-only runs"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                if is_fpo:
                    action, z, logprob, ent, value_logits, loss_eps, loss_t, old_cfm_loss = agent.get_action_and_value(next_obs)
        ...
        if is_fpo:
            b_fpo_eps = fpo_eps.reshape((-1, args.fpo_n_samples_per_action) + envs.single_action_space.shape)
            b_fpo_t = fpo_t.reshape(-1, args.fpo_n_samples_per_action, 1)
            b_fpo_old_cfm_loss = fpo_old_cfm_loss.reshape(-1, args.fpo_n_samples_per_action)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if is_fpo:
                    ratio, logratio, entropy, value_logits, cfm_loss = agent.evaluate_fpo_action_and_value(
                        b_obs[mb_inds],
                        b_actions[mb_inds],
                        b_fpo_eps[mb_inds],
                        b_fpo_t[mb_inds],
                        b_fpo_old_cfm_loss[mb_inds],
                    )
                    fpo_cfm_losses.append(cfm_loss.mean().item())
                    fpo_ratio_means.append(ratio.mean().item())
                else:
                    ...
                    ratio = logratio.exp()
                clip_eps = args.fpo_clipping_epsilon if is_fpo else args.clip_coef
                ...
                clip_hi = clip_eps if is_fpo or args.clip_coef_high is None else args.clip_coef_high
                ratio_advantages = mb_advantages.unsqueeze(-1) if is_fpo else mb_advantages
                pg_loss1 = -ratio_advantages * ratio
                pg_loss2 = -ratio_advantages * torch.clamp(ratio, 1 - clip_eps, 1 + clip_hi)
            ...
            kl_stop = args.fpo_target_kl if is_fpo else args.target_kl
            if kl_stop is not None and approx_kl > kl_stop:
                break
        ...
        if is_fpo:
            writer.add_scalar("losses/fpo_cfm_loss", float(np.mean(fpo_cfm_losses)) if fpo_cfm_losses else 0.0, global_step)
            writer.add_scalar("debug/fpo_ratio_mean", float(np.mean(fpo_ratio_means)) if fpo_ratio_means else 1.0, global_step)
            writer.add_scalar("debug/fpo_old_cfm_loss", b_fpo_old_cfm_loss.mean().item(), global_step)
            writer.add_scalar("debug/action_std", b_actions.std().item(), global_step)
            with torch.no_grad():
                fpo_stats = agent.get_fpo_endpoint_stats(b_obs[: min(4096, b_obs.shape[0])])
            writer.add_scalar("debug/fpo_raw_endpoint_std", fpo_stats["raw_std"].item(), global_step)
            writer.add_scalar("debug/fpo_raw_endpoint_absmax", fpo_stats["raw_absmax"].item(), global_step)
            writer.add_scalar("debug/fpo_raw_clip_frac", fpo_stats["raw_clip_frac"].item(), global_step)
            writer.add_scalar("debug/fpo_bounded_saturation_frac", fpo_stats["bounded_saturation_frac"].item(), global_step)
        if auto_alpha:
