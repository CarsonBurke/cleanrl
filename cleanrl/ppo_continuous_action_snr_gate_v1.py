# Uncertainty-gated Adam on base PPO, v1.
# Gate = mu^2 / (mu^2 + tau * (Var[g] + eps)), tau=1; multiply Adam's step.
# Gate mean and second moment share beta1=0.9 and are bias-corrected. Keep a
# separate beta2=0.999 second moment for Adam scaling; mixed EMA windows cannot
# be subtracted to estimate variance. No feedback of gated gradients into state.
# Hypothesis: suppress oscillating coordinates without shrinking consistent ones.
# Risk: minibatch disagreement, policy drift and weak useful signals all look noisy;
# this is gradient-history consistency, not calibrated epistemic uncertainty.
# Base tanh/Beta PPO, 3e-4 LR, joint clipping and 8M annealing remain unchanged.
import os
import random
import time
...
def uncertainty_gate(mean, second_moment, tau, eps):
    """Matched-decay, bias-corrected moments; eps has squared-gradient units."""
    signal = mean.square()
    ...
    return signal / (signal + tau * (variance + eps))
...
def snr_adam_update(params, grads, moments, seconds, gate_seconds, steps, lr,
    ...
    for parameter, gradient, moment, second, gate_second, step in zip(
        ...
        gate = uncertainty_gate(mean, gate_second / correction1, gate_tau, gate_eps)
        denominator = (second / (1.0 - beta2 ** step)).sqrt().add_(eps)
        parameter.add_(-lr * gate * mean / denominator)


class SNRGatedAdam(optim.Optimizer):
    """Adam preconditioning with matched-moment multiplicative uncertainty gating."""
