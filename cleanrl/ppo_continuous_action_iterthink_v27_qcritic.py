# PPO + IterThink v27 (ACTION-CONDITIONED CRITIC: give the critic the action). From v24.
#
# WHY v27. v24_beta won with rankgauss (9498) but every MAGNITUDE-aware advantage
# (advnorm/raw) collapsed (~3150). v25's critic catch-up (more value updates) did NOT
# rescue advnorm (2700 @1.18M vs rankgauss 4000), so the bottleneck is NOT critic
# under-optimization. The real cause: the critic is a STATE value V(s) -- it is
# action-blind. The advantage A(s,a)=Q(s,a)-V(s) needs to know how good the SPECIFIC
# action was, but the only action-specific signal in the PPO advantage is the single
# realized return R_t (high variance). V(s) cannot sharpen the per-action MAGNITUDE
# because it never sees a. So advnorm (needs magnitudes) drowns in return noise, while
# rankgauss (needs only the ORDERING) survives -- exactly the crutch we observed.
# Adding stabilizers (EMA target / value clipping) only smooths V(s); it cannot add
# information the critic structurally lacks (and both would cap the score anyway).
#
# THE FIX (action-conditioned value). Add a distributional Q-head Q(s,a) that takes the
# critic features AND the policy's action mean+std (its INTENT) AND the sampled action,
# trained by CE to the SAME distributional lambda-return target as V. Conditioning on
# (mean, std, action) lets Q value the action by its normalized deviation (a-mean)/std.
# The policy advantage becomes the LEARNED, low-variance, action-conditioned
#     A_Q(s,a) = E[Q(s,a)] - V(s)              (adv_source="q_minus_v")
# computed once per rollout (rollout-time critic), then fed through the SAME adv_transform
# pipeline (advnorm/rankgauss/...). Because V(s)=E_{a~pi}[Q(s,a)] holds in expectation
# when both regress to the same returns, A_Q is a proper (mean-zero-over-actions)
# advantage -- but now its MAGNITUDE reflects the critic's knowledge of the action, not
# one noisy return sample. Test: does A_Q make magnitude-aware advnorm finally work
# (approach/beat rankgauss)? If so, rankgauss was a crutch for an action-blind critic.
#
# KNOBS. adv_source={gae(=v24),q_minus_v}. q_detach_trunk=True (default): the Q-head is
# a readout on detached critic features, so the shared trunk dynamics stay EXACTLY v24
# (clean isolation -- any gain is from action-conditioning, not trunk changes). The Q CE
# loss is summed with V's and shares the critic_grad_clip budget. On-policy gives one
# action per state, but the beta policy's spread supplies the action diversity Q needs
# to interpolate Q(s,.) across the action it is "going to perform".
#
# --- inherited v24 notes ---
...
class Args:
    ...
    # v27 action-conditioned advantage. "gae" == v24 (V-based GAE). "q_minus_v" uses the
    # learned A_Q = E[Q(s,a)] - V(s) as the policy advantage (then adv_transform on top).
    adv_source: str = "q_minus_v"
    q_detach_trunk: bool = True      # Q-head reads DETACHED critic features (trunk stays v24)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v27 action-conditioned (distributional) Q-head: Q(s,a) from critic features +
        # the action. Same categorical support + peaked init as the V-head. The advantage
        # A_Q = E[Q(s,a)] - V(s) supplies the per-action MAGNITUDE that V(s) alone cannot.
        self.q_detach_trunk = args.q_detach_trunk
        # Q sees the policy's INTENT at this state -- the action-distribution mean and std
        # (what it is "going to perform" + its spread) -- plus the ACTUAL sampled action.
        # So Q can value the action via its normalized deviation (a-mean)/std from intent.
        self.q_head = nn.Sequential(
            layer_init(nn.Linear(H + 3 * act_dim, H)),
            nn.Tanh(),
            layer_init(nn.Linear(H, args.num_bins), std=0.1),
        )
        with torch.no_grad():
            self.q_head[-1].bias.copy_(-0.5 * (z / args.critic_init_tau) ** 2)
    ...
    def get_q(self, critic_feat, action, pol_mean, pol_std):
        # Action-conditioned distributional Q(s,a) LOGITS (B, num_bins), conditioned on the
        # policy's env-space mean & std AND the actual env-space action [-1,1]. By default
        # the critic features are DETACHED so the Q gradient does not reshape the (v24)
        # trunk -- Q is a pure action-aware readout.
        feat = critic_feat.detach() if self.q_detach_trunk else critic_feat
        return self.q_head(torch.cat([feat, pol_mean, pol_std, action], dim=-1))
    ...
    def get_action_and_value(self, x, z=None):
        ...
        # Policy intent (env-space mean & std) for the Q-head. DETACHED: the intent is a
        # constant INPUT to the critic, so the Q CE loss trains only the Q-head (+ trunk
        # iff q_detach_trunk=False) and never leaks a DPG-style gradient into the policy.
        if self.actor_dist == "beta":
            pol_mean = to_action(dist.mean).detach()                                  # affine map
            pol_std = ((self.action_high - self.action_low) * dist.stddev).detach()   # affine scale
        else:  # gaussian (pre-tanh); delta-method through the tanh squash
            m = dist.mean
            pol_mean = torch.tanh(m).detach()
            pol_std = (dist.stddev * (1.0 - torch.tanh(m) ** 2)).detach()
        q_logits = self.get_q(critic_feat, action, pol_mean, pol_std)
        return action, z, log_prob, dist.entropy().sum(1), value_logits, q_logits
    ...
    def critic_parameters(self):
        # Params receiving the VALUE gradient (incl. the shared trunk + the Q-head).
        # NOTE: when q_detach_trunk, the Q gradient stops at the Q-head (the trunk is
        # detached for Q), so including the trunk here only carries V's gradient.
...
if __name__ == "__main__":
    ...
    q_values = torch.zeros((args.num_steps, args.num_envs)).to(device)  # v27: E[Q(s_t, a_t)]
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, _, value_logits, q_logits = agent.get_action_and_value(next_obs)
                p = torch.softmax(value_logits, dim=-1)
                value_probs[step] = p
                values[step] = (p * support).sum(dim=-1)
                q_values[step] = (torch.softmax(q_logits, dim=-1) * support).sum(dim=-1)
            actions[step] = action
            latent_zs[step] = z
        ...
        # v27: choose the POLICY-advantage SOURCE before shaping. "q_minus_v" replaces the
        # action-blind GAE (single-sample R_t - V) with the learned action-conditioned
        # A_Q = E[Q(s,a)] - V(s) (rollout-time critic, fixed for the epochs). Both V and Q
        # regress to the same lambda-return target, so E_a[Q] = V => A_Q is mean-zero over
        # actions, but its MAGNITUDE now reflects the action. Feeds the SAME transforms.
        a_q = b_q_values - b_values
        adv_for_policy = a_q if args.adv_source == "q_minus_v" else b_advantages
        ...
        # v27: how much does the action-conditioned A_Q differ from action-blind GAE?
        # corr→1 means Q added no per-action info beyond V; lower means it did.
        gaez = (b_advantages - b_advantages.mean()) / (b_advantages.std() + 1e-8)
        aqz = (a_q - a_q.mean()) / (a_q.std() + 1e-8)
        aq_gae_corr = (gaez * aqz).mean().item()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_logits, q_logits = agent.get_action_and_value(
                ...
                # v27: action-conditioned Q regresses to the SAME target, given (s, a_taken).
                # Summed with v_loss => one critic objective sharing the critic_grad_clip.
                q_log_probs = torch.log_softmax(q_logits, dim=-1)
                q_loss = -(b_target_probs[mb_inds] * q_log_probs).sum(dim=-1).mean()
                ...
                if args.separate_grad_clip:
                    ...
                    (args.vf_coef * critic_loss).backward(retain_graph=True)
                    ...
                    loss = pg_loss - args.ent_coef * entropy_loss + critic_loss * args.vf_coef
                    optimizer.zero_grad()
                    loss.backward()
        ...
        writer.add_scalar("debug/aq_gae_corr", aq_gae_corr, global_step)
        writer.add_scalar("debug/a_q_std", a_q.std().item(), global_step)
