# PPO + IterThink v24 beta d4-HL-Gauss symlog + channel-split readout v1.
#
# Variant hypothesis: keep the high-performing symlog_v1 PPO machinery and
# multidepth IterThink computation, but reduce direct actor/critic readout
# contention by splitting every depth feature in half along channels. The actor
# reads concat([x0_a, ..., xK_a]) and the critic reads concat([x0_c, ..., xK_c]).
# This is intentionally simpler than PMA: no attention, no learned routing, and
# no extra auxiliary losses. It preserves shared computation and decoupled
# dual-backward clipping while preventing the two heads from projecting from the
# exact same readout channels.
#
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, n_experts):
        super().__init__()
        assert H % 2 == 0, "channel-split readout requires an even hidden size"
        self.H = H
        self.half = H // 2
        ...
        split_cat_dim = self.half * (K + 1)
        self.actor_out_norm = nn.RMSNorm(split_cat_dim, elementwise_affine=False)
        self.critic_out_norm = nn.RMSNorm(split_cat_dim, elementwise_affine=False)
        self.actor_out_proj = layer_init(nn.Linear(split_cat_dim, H))
        self.critic_out_proj = layer_init(nn.Linear(split_cat_dim, H))
    ...
    def forward(self, x):
        ...
        actor_cat = torch.cat([feat[:, : self.half] for feat in feats], dim=-1)
        critic_cat = torch.cat([feat[:, self.half :] for feat in feats], dim=-1)
        actor_feat = self.actor_out_proj(self.actor_out_norm(actor_cat))
        critic_feat = self.critic_out_proj(self.critic_out_norm(critic_cat))
...
class Agent(nn.Module):
    ...
    def _trunks(self, x):
        # Return (actor_feat, critic_feat). When sharing, one trunk computes both
        # role-specific channel-split readouts in a single forward.
        if self.share_backbone:
            return self.trunk(x)
