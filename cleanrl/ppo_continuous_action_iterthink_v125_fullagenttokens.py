# PPO + IterThink v125 fullagenttokens (v124 + restore outcome-token PPO input).
#
# Key change from v124:
#   - Actor and critic again consume the full pre-action predictor belief:
#     reward readout token, continuation readout token, and obs tokens. The
#     recurrent world-model stream remains obs-only; outcome tokens are learned
#     query/readout outputs for the current neutral-action belief.
#
# Hypothesis:
#   With causal previous-reward context in the WM, exposing reward/continue
#   readout latents to PPO should help value/policy use the same compact outcome
#   features the WM trains, closer to Dreamer-style shared agent features.
#
# -----------------------------------------------------------------------------
# Previous header:
# PPO + IterThink v124 rewardcontext (v122 + D4-style shifted reward context).
#
# Key change from v122:
#   - Add an actual reward-label context token to the world model predictor,
#     projected from the same symlog HL-Gauss reward labels used for reward CE.
#   - The token is shifted for this predictor's indexing: state/action position t
#     can see only r_{t-1} or older, never the r_t label it is trained to
#     predict. This follows D4's causal reward-history rule, but is not a literal
#     copy of D4's action-conditioned post-transition contract: D4 predicts r_t
#     from agent[t+1] and therefore uses a training-time two-slot reward shift.
#     Our reward head predicts r_t from the current-position predicted next
#     summary, so one-slot previous-reward context is the aligned no-leak shift.
#   - Imagination feeds back the model's predicted reward distribution as the
#     next state's previous-reward context. Real rollouts and WM training use
#     real previous rewards where they are causally available.
#
# Hypothesis:
#   Previous reward is a useful compact history feature in many environments, but
#   it must be causal. A shifted context token should improve reward/continuation
#   calibration without giving the model an autoregressive target leak.
#
# -----------------------------------------------------------------------------
# Previous header:
# PPO + IterThink v122 agentobsonly (v121 + hide outcome readout tokens from agent).
#
# Key change from v121:
#   - The world model still creates reward and continuation semantic readout
#     tokens, and reward/continue heads still read only those tokens.
#   - Actor and critic no longer receive those semantic tokens in `agent_input`.
#     `agent_input_from_latent` slices them away and flattens obs tokens only.
#   - This tests whether the agent was overusing outcome-readout latent features
#     rather than learning from the dynamics/observation belief.
#
# Hypothesis:
#   D3/D4 normally feed policy/value the same model feature that reward is
#   decoded from, so full belief is defensible. But our semantic slots are more
#   explicitly role-specialized than Dreamer agent embeddings. Hiding them from
#   the agent may improve value/policy robustness if they were acting as stale or
#   self-referential outcome shortcuts in imagined training.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    lewm_reward_raw_v_min: float = -20.0
    lewm_reward_raw_v_max: float = 50.0
    # symlog(-20)=-3.0445224377, symlog(50)=3.9318256327.
    lewm_reward_v_min: float = -3.044522437723423
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.reward_context_proj = xavier_linear(nn.Linear(self.reward_num_bins, self.dim, bias=False))
        self.reward_context_embed = nn.Parameter(torch.empty(self.dim))
        nn.init.xavier_uniform_(self.reward_context_embed.unsqueeze(0))
    ...
    def _predictor_trunk(self, latent_history, action_history, reward_context_probs=None):
        ...
        if context_len > self.context:
            ...
            if reward_context_probs is not None:
                reward_context_probs = reward_context_probs[:, -self.context :]
            context_len = self.context
        if reward_context_probs is None:
            reward_context_probs = latent_history.new_zeros(batch, context_len, self.reward_num_bins)
        else:
            reward_context_probs = reward_context_probs.to(device=latent_history.device, dtype=latent_history.dtype)
        ...
        # Tokens are [semantic queries, obs latents, previous-reward context].
        # The reward token is input-only: it is appended after the returned latent
        # slots, so pred_next/probes/PPO never consume it directly.
        reward_context = self.reward_context_proj(reward_context_probs)
        reward_context = reward_context + self.reward_context_embed.to(reward_context.dtype)
        reward_context = reward_context.unsqueeze(2)
        tokens = torch.cat([sem_query, latent_history, reward_context], dim=2)
    ...
    def belief_from_history(self, latent_history, action_history, reward_context_probs=None):
        return self._predictor_trunk(latent_history, action_history, reward_context_probs)[:, -1]

    def predict_mtp_from_history(self, latent_history, action_history, reward_context_probs=None, return_beliefs=False):
        features = self._predictor_trunk(latent_history, action_history, reward_context_probs)
    ...
    def imagine_step_from_history(self, latent_history, action_history, reward_context_probs=None):
        ...
        features = self._predictor_trunk(latent_history, action_history, reward_context_probs)[:, -1]
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Direct latent control state for actor/critic: full pre-action belief.
        # The WM recurrent stream is still obs-only, but the current predictor
        # belief includes reward and continuation query/readout tokens.
        self.agent_latent_dim = (obs_dim + 2) * args.lewm_dim
    ...
    def agent_input_from_latent(self, latent):
        ...
        if latent.shape[-2] != self.world_model.num_latent_tokens:
            raise ValueError(
                f"agent latent must have {self.world_model.num_latent_tokens} full tokens, got {latent.shape[-2]}"
            )
        return latent.reshape(latent.shape[0], -1)

    def agent_input_from_history(self, latent_history, action_history, reward_context_probs=None):
        latent = self.world_model.belief_from_history(latent_history, action_history, reward_context_probs)
        return self.agent_input_from_latent(latent)

    def agent_input_from_obs(self, x):
        summary = self.world_model.encode_summary(x)
        obs_latent = summary[:, self.world_model.obs_token_start :]
        neutral_action = torch.zeros(
            summary.shape[0],
            self.world_model.act_dim,
            device=summary.device,
            dtype=summary.dtype,
        )
        zero_reward_context = torch.zeros(
            summary.shape[0],
            self.world_model.reward_num_bins,
            device=summary.device,
            dtype=summary.dtype,
        )
        return self.agent_input_from_history(
            obs_latent.unsqueeze(1),
            neutral_action.unsqueeze(1),
            zero_reward_context.unsqueeze(1),
        )
...
if __name__ == "__main__":
    ...
    def reward_values_to_context_probs(reward_values, valid=None):
        probs = reward_support.project(reward_values.reshape(-1)).reshape(*reward_values.shape, -1)
        if valid is not None:
            probs = probs * valid.to(dtype=probs.dtype).unsqueeze(-1)
        return probs
    ...
    state_reward_contexts = torch.zeros(
        (args.num_steps, args.num_envs, args.lewm_reward_num_bins),
        device=device,
    )
    ...
    rollout_reward_context_history = []
    rollout_history_valid = []
    neutral_action = torch.zeros(args.num_envs, int(np.prod(envs.single_action_space.shape)), device=device)
    zero_reward_context = torch.zeros(args.num_envs, args.lewm_reward_num_bins, device=device)
    last_reward_context = zero_reward_context.clone()

    def build_belief_agent_input(obs_tensor, current_reward_context=None):
        ...
        if current_reward_context is None:
            current_reward_context = last_reward_context
        ...
        past_reward_contexts = rollout_reward_context_history[-n_past:] if n_past > 0 else []
        ...
        if n_past > 0:
            ...
            past_reward_contexts = [
                reward_context * valid.view(-1, 1).to(reward_context.dtype)
                for reward_context, valid in zip(past_reward_contexts, past_valids)
            ]
        belief_latents = torch.stack(past_latents + [current_latent], dim=1)
        belief_actions = torch.stack(past_actions + [neutral_action], dim=1)
        belief_rewards = torch.stack(past_reward_contexts + [current_reward_context], dim=1)
        return agent.agent_input_from_history(belief_latents, belief_actions, belief_rewards), current_latent, current_reward_context
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                agent_input, current_latent, current_reward_context = build_belief_agent_input(next_obs)
            ...
            state_reward_contexts[step] = current_reward_context
            rollout_latent_history.append(current_latent.detach())
            rollout_action_history.append(action.detach())
            rollout_reward_context_history.append(current_reward_context.detach())
            ...
            if len(rollout_latent_history) > args.lewm_context - 1:
                ...
                rollout_reward_context_history = rollout_reward_context_history[-(args.lewm_context - 1) :]
            ...
            transition_reward_context = reward_values_to_context_probs(reward_tensor)
            ...
            with torch.no_grad():
                next_transition_agent_input, _, _ = build_belief_agent_input(
                    ...
                    transition_reward_context,
                )
                next_transition_agent_inputs[step] = next_transition_agent_input
            last_reward_context = transition_reward_context * (~boundary_tensor).to(transition_reward_context.dtype).unsqueeze(-1)
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # Actual causal reward context used at each collected state.
                    # This preserves previous-reward context across rollout
                    # boundaries when the episode is still live, matching rollout
                    # policy/value conditioning.
                    prev_reward_context = state_reward_contexts[safe_hist_step_inds, env_inds]
                    ...
                    pred_mtp = agent.world_model.predict_mtp_from_history(
                        teacher_history,
                        future_actions,
                        prev_reward_context,
                    )
                    ...
                    with torch.no_grad():
                        def accumulate_labeled_ar(
                            ...
                            for h in range(horizon):
                                ...
                                (
                                    ...
                                    torch.stack(hist_reward_contexts, dim=1),
                                )
                                ...
                                next_reward_context = torch.softmax(ar_reward_logits, dim=-1)
                                next_reward_context = next_reward_context * (1.0 - future_boundaries[:, h]).unsqueeze(-1)
                                hist_reward_contexts.append(next_reward_context.detach())
                        ...
                        if args.diagnostics_warm_ar:
                            ...
                            if bool(warm_valid.any()):
                                ...
                                win_reward_context = state_reward_contexts[win_steps, win_envs]
                                accumulate_labeled_ar(
                                    [win_summaries[:, d, obs_start:].detach() for d in range(warm_context)],
                                    [win_actions[:, d] for d in range(max(0, warm_context - 1))],
                                    [win_reward_context[:, d].detach() for d in range(warm_context)],
            ...
            for _ in range(args.imagine_batches):
                with torch.no_grad():
                    ...
                    if clean_flat.numel() > 0:
                        ...
                        win_reward_context = state_reward_contexts[
                            win_steps,
                            env_idx.unsqueeze(0).expand_as(win_steps),
                        ]
                        hist_reward_contexts = [win_reward_context[d] for d in range(W_im)]
                    else:
                        ...
                        hist_reward_contexts = [state_reward_contexts[step_idx, env_idx]]
                    for t in range(H_im):
                        latent_hist = torch.stack(hist_latents, dim=1)               # (B, t+1, obs_tok, dim)
                        reward_hist = torch.stack(hist_reward_contexts, dim=1)       # (B, t+1, reward_bins)
                        ...
                        belief = agent.world_model.belief_from_history(latent_hist, belief_actions, reward_hist)
                        ...
                        next_s, reward_logits, cont_logits = (
                            agent.world_model.imagine_step_from_history(latent_hist, dyn_actions, reward_hist)
                        )
                        ...
                        hist_reward_contexts.append(torch.softmax(reward_logits, dim=-1).detach())
                    # Bootstrap value at the final imagined state s_H (same recurrent belief).
                    latent_hist = torch.stack(hist_latents, dim=1)
                    reward_hist = torch.stack(hist_reward_contexts, dim=1)
                    belief_actions_H = torch.stack(hist_actions + [imagine_neutral], dim=1)
                    belief_H = agent.world_model.belief_from_history(latent_hist, belief_actions_H, reward_hist)
