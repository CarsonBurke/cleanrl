# Future control v19: use the unified return embedding to remove later-action
# noise from real-segment TD targets. Frozen, centered action predictions are
# transported through the observed reward prefix in every embedding coordinate.
# Their conditional mean is zero; source-action credit and target means survive.
# An action-value martingale estimator, related to finite unit-weight TD traces;
# no scalar GAE head, lambda mixture, replay, decoder, or Bellman target sweeps.
import json
import math
import random
import time
...
@dataclass
class Args:
    exp_name: str = 'vector_future_control_v19'
    seed: int = 1
    ...
    credit_score_weighting: bool = False
...
def td_segments(components, ends, td_steps=32):
    ...
    prefix = time_prefix_sum(components)
...
def time_prefix_sum(values):
    """Inclusive FP64 prefix per environment/channel, with short compiled scans."""
    steps, environments, channels = values.shape
    streams = values.double().permute(1, 2, 0).contiguous().flatten(0, 1)
    block = 128
    blocks = (steps + block - 1) // block
    tiles = F.pad(streams, (0, blocks * block - steps)).reshape(-1, block).cumsum(-1)
    tiles = tiles.view(streams.shape[0], blocks, block)
    totals = tiles[..., -1]
    prefix = (tiles + (totals.cumsum(-1) - totals)[..., None]).flatten(1)[:, :steps]
    return prefix.view(environments, channels, steps).permute(2, 0, 1)


def future_action_control_sum(components, predicted, policy_average, consumed, frequencies, horizon=1000):
    """Transport centered future-action predictions; exclude the source action.

    Inputs are pre-fit embeddings at the real rollout states. Policy averages
    use independently sampled collection-policy actions. For each source t,
    sum j=t+1..t+m-1, where m is its terminal-clipped segment length. Means
    add directly; complex coordinates rotate by the observed reward before j.
    Absolute-prefix rotation makes this a pair of scans, not a 32-step gather.
    No projection/clipping of corrected targets is permitted: it biases means.
    """
    steps, environments, channels = components.shape
    count = frequencies.shape[0]
    difference = (predicted.detach().double() - policy_average.detach().double()).view(steps, environments, -1)
    means, cosine, sine = difference.split((channels, count, count), -1)
    prefix_reward = time_prefix_sum(components) - components.double()
    phase = (prefix_reward / horizon) @ frequencies.double().T
    real, imag = phase.cos(), phase.sin()
    transported = torch.cat((means, real * cosine - imag * sine,
                            imag * cosine + real * sine), -1)
    prefix = time_prefix_sum(transported)
    positions = torch.arange(steps, device=components.device)[:, None]
    environment_index = torch.arange(environments, device=components.device)[None]
    last = positions + consumed.view(steps, environments) - 1
    # Inclusive prefix[last] - prefix[t] excludes t even for one-step segments.
    accumulated = prefix[last, environment_index] - prefix
    means, cosine, sine = accumulated.split((channels, count, count), -1)
    result = torch.cat((means, real * cosine + imag * sine,
                        real * sine - imag * cosine), -1)
    return result.flatten(0, 1).to(predicted.dtype).detach()
...
def main():
    ...
    with ExitStack() as resources:
        ...
        writer.add_text('method', 'Unified future embedding with frozen future-action controls transported through real reward prefixes; full-rank component/utility fitting; source-action residual correction; no scalar GAE, lambda mixture, replay, decoder or target sweeps.')
        ...
        def candidate_predictions(obs, age, actions):
            count = actions.shape[1]
            expanded_obs = obs[:, None].expand(-1, count, -1).flatten(0, 1)
            expanded_age = age[:, None].expand(-1, count).flatten()
            return critic.predict(expanded_obs, expanded_age, actions.flatten(0, 1)).view(
        ...
        control_function = future_action_control_sum
        if args.compile:
            ...
            candidate_predictions = torch.compile(candidate_predictions, fullgraph=True, mode=args.compile_mode)
            control_function = torch.compile(control_function, fullgraph=True, mode=args.compile_mode)
        ...
        for iteration in range(1, iterations + 1):
            ...
            with timer.span('targets'), torch.no_grad():
                ...
                prefit_predictions = candidate_predictions(observations, age, candidate_actions).clone()
                mean_scale = horizon * math.sqrt(critic.components_count)
                prefit_components = prefit_predictions[..., :critic.components_count] * mean_scale
                ordinary_target = target
                controls = control_function(
                    components.view(args.num_steps, args.num_envs, -1), prefit_predictions[:, -1],
                    prefit_predictions[:, :-1].mean(1), consumed, critic.frequencies, horizon).clone()
                target = ordinary_target - controls
                target_components = target[:, :critic.components_count] * mean_scale
                ordinary_residual = ordinary_target[:, :critic.components_count] * mean_scale - prefit_components[:, -1]
                controlled_residual = target_components - prefit_components[:, -1]
                diagnostics['control/mean_vector_rms'] = (controls[:, :critic.components_count] * mean_scale).square().mean().sqrt()
                diagnostics['control/moment_rms'] = controls[:, critic.components_count:].square().mean().sqrt()
                diagnostics['control/raw_residual_vector_rms'] = (ordinary_residual.square().mean(-1) * weights).sum().div(weights.sum()).sqrt()
                diagnostics['control/controlled_residual_vector_rms'] = (controlled_residual.square().mean(-1) * weights).sum().div(weights.sum()).sqrt()
                natural_score_energy = whitened_score.square().mean((-2, -1))
                for label, delta in (('raw', ordinary_residual), ('controlled', controlled_residual)):
                    diagnostics[f'control/{label}_whitened_residual_credit_rms'] = (
                        delta.sum(-1).square() * natural_score_energy * weights).sum().div(weights.sum()).sqrt()
                count = critic.frequencies.shape[0]
                cosine, sine = target[:, critic.components_count:].split(count, -1)
                diagnostics['control/target_outside_unit_disk_fraction'] = (
                    (cosine.square() + sine.square()) * count > 1 + 1e-5).float().mean()
        ...
        report = dict(args=vars(args), status='cancelled' if stop_requested else 'completed',
                      ...
                      future_action_controls=True, q_td_trace_equivalence=True,
                      ...
                      objective='Unified future return embedding with transported zero-mean future-action controls and observed-residual-corrected actor; full-batch unweighted component/utility fitting',
                      limitations='Single seed, training returns. Future-action controls can increase variance with inaccurate predictions or finite action integration. This Q-TD martingale estimator is related to finite unit-weight TD traces. Characteristic coordinates still affect risk-neutral actor credit indirectly through representation learning. Finite characteristic features and unit-disk constraints do not identify a complete joint distribution. Actor correction removes current-action model bias for fixed candidates, not continuation bias or finite-sample selection error. Unclipped importance ratios can have high variance. Natural-gradient solves are approximate.')
