# Dense-SiLU Beta PPO baseline on the shared native MuJoCo trainer.
# This is an activation-only control against the current dense Tanh baseline:
# width, depth, initialization, optimizer, rollout topology, and PPO objective
# remain unchanged. Hypothesis: non-saturating SiLU improves gradient flow.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(observation_dim, 64)), nn.SiLU(),
            layer_init(nn.Linear(64, 64)), nn.SiLU(),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta PPO; dense width-64 SiLU trunk; FP32; native-action storage; host actor mirror; torch.compile")
