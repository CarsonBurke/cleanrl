# TD7 + rank-weighted soft-greedification actor ("rank-greedy").
#
# Single change vs td7_continuous_action.py: the actor's deterministic policy-gradient loss
# (-Q(s, pi(s)) via SALE embeddings) is replaced by a k-sample rank-weighted distillation loss.
# Everything else — twin critics, target policy smoothing, LAP prioritized replay, SALE encoder,
# policy checkpointing, target value clipping, update frequencies, exploration noise, and all
# hyperparameters — is byte-identical to the baseline.
#
# Framing (translator view of policy improvement):
#   The critic Q is the policy-improvement operator. TD7's DPG actor follows dQ/da to move pi(s)
#   toward the Q-argmax — a first-order local ascent that trusts the critic's gradient exactly.
#   Here we instead treat improvement as *distillation from a proposed action distribution*: sample
#   k candidate actions around pi(s), let the critic rank them, and regress pi(s) toward the highly
#   ranked ones. This is a soft-greedification step that is agnostic to the shape of the critic
#   surface (it needs only an ordering, not a usable gradient), which is friendlier to noisy /
#   stochastic-friendly improvement operators than raw dQ/da.
#
# Why RANK weighting (not raw Q softmax):
#   Weights come from each candidate's *rank* among the k (argsort of argsort -> {0..k-1}), not from
#   raw Q magnitudes. A Q overestimation spike on one candidate can therefore only ever make it
#   "best of k": its influence is bounded by the top rank weight, never blown up by an unbounded Q
#   value. This ports the rank-advantage-shaping win from the on-policy streaming track into TD7's
#   off-policy actor update.
#
# Candidates per state: cand 0 = pi(s) (detached); cands 1..k-3 = pi(s)+cand_sigma*eps (local);
#   last 2 = pi(s)+cand_sigma_broad*eps (broad); all clamped to [-1, 1]. Scoring uses the same SALE
#   embedding plumbing and the same head reduction (mean of the twin Q heads) that the baseline
#   actor loss uses, under no_grad (candidates/weights are detached targets). Loss is
#   sum_j w_j * ||pi(s) - a_j||^2 averaged over states, with pi(s) the differentiable actor output.
#
# Hypothesis: matches or beats TD7's DPG actor on continuous control by using a bounded-influence,
# ordering-only improvement operator that is robust to critic overestimation spikes.
import copy
import os
import random
import time
...
class Args:
    ...
    # rank-weighted soft-greedification actor arguments
    rank_k: int = 8
    """number of candidate actions scored per state for the rank-weighted actor loss"""
    cand_sigma: float = 0.2
    """std of the local Gaussian proposals around pi(s) (candidates 1..k-3)"""
    cand_sigma_broad: float = 0.5
    """std of the broad Gaussian proposals around pi(s) (the last 2 candidates)"""
    rank_tau: float = 0.25
    """temperature of the softmax over normalized ranks that forms the candidate weights"""
...
class TD7Agent:
    ...
    def train(self):
        ...
        # update actor (delayed): rank-weighted soft-greedification.
        # Build k candidate actions around pi(s), score them with the critic (same SALE plumbing and
        # head reduction as the baseline actor loss), convert per-state scores to weights by RANK,
        # and regress the differentiable pi(s) toward the highly ranked candidates.
        if self.training_steps % self.args.policy_freq == 0:
            actor_action = self.actor(state, fixed_zs)  # differentiable pi(s), shape [B, act_dim]

            with torch.no_grad():
                k = self.args.rank_k
                B = actor_action.shape[0]
                base = actor_action.detach()  # cand 0 anchor, [B, act_dim]

                # per-candidate proposal std: cand 0 = 0 (exactly pi(s)); locals = cand_sigma;
                # last 2 = cand_sigma_broad. shape [k, 1] to broadcast over [B, k, act_dim].
                sigma = torch.full((k,), self.args.cand_sigma, device=self.device)
                sigma[0] = 0.0
                sigma[-2:] = self.args.cand_sigma_broad
                sigma = sigma.view(1, k, 1)

                eps = torch.randn(B, k, self.action_dim, device=self.device)
                candidates = (base.unsqueeze(1) + sigma * eps).clamp(-1, 1)  # [B, k, act_dim]

                # score every candidate with the critic, mirroring the baseline actor plumbing:
                # zsa = fixed_encoder.zsa(fixed_zs, a), Q = critic(state, a, zsa, fixed_zs), then
                # reduce the twin heads by mean (baseline uses -critic(...).mean()).
                cand_flat = candidates.reshape(B * k, self.action_dim)
                state_rep = state.repeat_interleave(k, dim=0)
                fixed_zs_rep = fixed_zs.repeat_interleave(k, dim=0)
                cand_zsa = self.fixed_encoder.zsa(fixed_zs_rep, cand_flat)
                cand_Q = self.critic(state_rep, cand_flat, cand_zsa, fixed_zs_rep)  # [B*k, 2]
                scores = cand_Q.mean(1).view(B, k)  # [B, k]

                # rank-based weights: rank_j in {0..k-1} via argsort of argsort (0 = worst),
                # rank_norm in [0, 1], w = softmax(rank_norm / rank_tau) over the k candidates.
                ranks = scores.argsort(dim=1).argsort(dim=1).float()  # [B, k]
                rank_norm = ranks / (k - 1)
                weights = torch.softmax(rank_norm / self.args.rank_tau, dim=1)  # [B, k]

            # weighted regression of pi(s) toward the candidates; only actor_action carries grad.
            dist_sq = (actor_action.unsqueeze(1) - candidates).pow(2).sum(-1)  # [B, k]
            actor_loss = (weights * dist_sq).sum(1).mean()
            ...
            if self.training_steps % 500 == 0:
                ...
                # how often the current action already wins (mean weight on candidate 0)
                self.writer.add_scalar("charts/rank_w_cand0", weights[:, 0].mean().item(), self.training_steps)
                # mean Q-gap between the best candidate and pi(s) (candidate 0)
                q_gap = (scores.max(1)[0] - scores[:, 0]).mean().item()
                self.writer.add_scalar("charts/rank_q_gap", q_gap, self.training_steps)
