# PPO + IterThink v24.15 (MIXTURE OF PRODUCT BETAS). From v24.
#
# Actor-only ablation: replace v24's single independent Beta policy with a K-way
# categorical mixture of product Betas. A rollout stores only the native action
# coordinate z in (0,1); PPO replay recomputes the exact mixture log-prob with
# logsumexp_k(log pi_k + sum_d log Beta_k(z_d)). Actions are the linear map from z
# to the Box bounds. There is no tanh/sigmoid action squish and no actor-side action
# clipping beyond the numerical Beta sample clamp to [eps, 1-eps].
#
# Hypothesis: near-identical components preserve v24's stable bounded-support start,
# while the mixture can later allocate modes to state-conditioned exploration niches.
import os
import random
import time
...
class Args:
    ...
    ent_coef: float = 0.0
    # Retained for CLI compatibility with v24; disabled for this Beta-mixture ablation.
    ...
    # v24.15 actor: K-way categorical mixture of product Betas.
    beta_mixture_components: int = 4
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.beta_mixture_components = args.beta_mixture_components
        if self.beta_mixture_components < 1:
            raise ValueError("beta_mixture_components must be >= 1")
        # Base heads match v24's single-Beta initialization. Tiny component offsets
        # break symmetry without giving the mixture an early asymmetric policy.
        self.actor_alpha_base = layer_init(nn.Linear(H, act_dim), std=0.01)
        self.actor_beta_base = layer_init(nn.Linear(H, act_dim), std=0.01)
        self.actor_alpha_delta = layer_init(
            nn.Linear(H, self.beta_mixture_components * act_dim),
            std=1e-4,
        )
        self.actor_beta_delta = layer_init(
            nn.Linear(H, self.beta_mixture_components * act_dim),
            std=1e-4,
        )
        self.actor_mixture_logits = layer_init(nn.Linear(H, self.beta_mixture_components), std=1e-4)
    ...
    def _beta_mixture_params(self, actor_feat):
        B = actor_feat.shape[0]
        K = self.beta_mixture_components
        alpha_base = self.actor_alpha_base(actor_feat).unsqueeze(1)
        beta_base = self.actor_beta_base(actor_feat).unsqueeze(1)
        alpha_delta = self.actor_alpha_delta(actor_feat).view(B, K, self.act_dim)
        beta_delta = self.actor_beta_delta(actor_feat).view(B, K, self.act_dim)
        alpha = 1.0 + F.softplus(alpha_base + alpha_delta)
        beta = 1.0 + F.softplus(beta_base + beta_delta)
        mixture_logits = self.actor_mixture_logits(actor_feat)
        return mixture_logits, alpha, beta

    def _beta_mixture_log_prob(self, z, mixture_logits, alpha, beta):
        z = z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        component_logprob = Beta(alpha, beta).log_prob(z.unsqueeze(1)).sum(-1)
        log_mix = F.log_softmax(mixture_logits, dim=-1)
        return torch.logsumexp(log_mix + component_logprob, dim=-1), component_logprob, log_mix

    def _sample_beta_mixture(self, mixture_logits, alpha, beta):
        mixture_probs = torch.softmax(mixture_logits, dim=-1)
        component = torch.distributions.Categorical(probs=mixture_probs).sample()
        batch = torch.arange(component.shape[0], device=component.device)
        z = Beta(alpha[batch, component], beta[batch, component]).sample()
        return z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)

    def _to_action(self, z):
    ...
    def get_action_and_value(self, x, z=None):
        # z is the native Beta-mixture sample in (0,1). PPO replay passes the same
        # z back in and recomputes the exact mixture density via logsumexp.
        actor_feat, critic_feat = self._trunks(x)
        mixture_logits, alpha, beta = self._beta_mixture_params(actor_feat)
        if z is None:
            z = self._sample_beta_mixture(mixture_logits, alpha, beta)
        action = self._to_action(z)
        log_prob, _, _ = self._beta_mixture_log_prob(z, mixture_logits, alpha, beta)
        value_logits = self.critic_head(critic_feat)
        entropy_z = self._sample_beta_mixture(mixture_logits, alpha, beta)
        entropy = -self._beta_mixture_log_prob(entropy_z, mixture_logits, alpha, beta)[0]
        return action, z, log_prob, entropy, value_logits

    def get_beta_mixture_diagnostics(self, x, z):
        actor_feat, _ = self._trunks(x)
        mixture_logits, alpha, beta = self._beta_mixture_params(actor_feat)
        log_prob, component_logprob, log_mix = self._beta_mixture_log_prob(z, mixture_logits, alpha, beta)
        mixture_probs = torch.softmax(mixture_logits, dim=-1)
        responsibilities = torch.softmax(log_mix + component_logprob, dim=-1)
        mixture_entropy = torch.distributions.Categorical(probs=mixture_probs).entropy()
        return {
            "mixture_entropy": mixture_entropy.mean(),
            "sample_entropy": (-log_prob).mean(),
            "max_mixture_probability": mixture_probs.max(dim=-1).values.mean(),
            "max_responsibility": responsibilities.max(dim=-1).values.mean(),
    ...
    def actor_parameters(self):
        ...
        heads = (
            list(self.actor_alpha_base.parameters())
            + list(self.actor_beta_base.parameters())
            + list(self.actor_alpha_delta.parameters())
            + list(self.actor_beta_delta.parameters())
            + list(self.actor_mixture_logits.parameters())
        )
...
if __name__ == "__main__":
    ...
    # v24's Gaussian-only auto-entropy path is intentionally disabled here; this
    # ablation changes only the actor distribution to a Beta mixture.
    auto_alpha = False
    if auto_alpha:
        act_dim = int(np.prod(envs.single_action_space.shape))
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            beta_mixture_diag = agent.get_beta_mixture_diagnostics(b_obs, b_latent_zs)
        ...
        writer.add_scalar("debug/beta_mixture_entropy", beta_mixture_diag["mixture_entropy"].item(), global_step)
        writer.add_scalar("debug/beta_mixture_sample_entropy", beta_mixture_diag["sample_entropy"].item(), global_step)
        writer.add_scalar(
            "debug/beta_mixture_max_probability",
            beta_mixture_diag["max_mixture_probability"].item(),
            global_step,
        )
        writer.add_scalar(
            "debug/beta_mixture_max_responsibility",
            beta_mixture_diag["max_responsibility"].item(),
            global_step,
        )
        writer.add_scalar("debug/beta_mixture_alpha_mean", beta_mixture_diag["alpha_mean"].item(), global_step)
        writer.add_scalar("debug/beta_mixture_beta_mean", beta_mixture_diag["beta_mean"].item(), global_step)
