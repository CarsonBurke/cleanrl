# Raw-mean-preserving categorical critic on normalized residual SiTU PPO v13.
# Raw rewards, fixed +/-20000 support, no return standardization or EMA.
# Gaussian labels are KL-tilted to preserve the RAW mean after symexp decoding;
# Dreamer raw-space twohot is the matched-support control. This removes the
# systematic value bias of symlog smoothing without swapping our architecture.
# Default: 501 nodes, sigma 1.5 bins, fullbatch asymmetric PPO, no adv/value clip.
# Hypothesis: unbiased critic targets prevent persistent negative GAE drift.
import math
import os
import random
import time
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", f"original residual SiTU-GLU trunks and matrix projection; raw rewards; fixed symexp-spaced raw nodes; {args.value_target} mean-preserving labels; paired raw-expectation decode; no return frame or EMA")
