"""FPO reference-actor correction, arXiv:2507.21053v2 / official Playground code.

v1 substituted a PPO actor and tanh decoder without an ablation. Restore the
reference 4x32 SiLU velocity field, LeCun-uniform weights, eight sinusoidal time
features, and explicit .25 output scaling. Use the reference reverse convention:
noise at t=1, data at t=0, u=eps-action, Euler dt=-1/K. Epsilon estimate is
x_t+(1-t)*u. Mean action error, then mean MC error, then exp(old-new).

Default actions are raw ODE outputs, clipped only by the environment. Always
store ORIGINAL ODE targets, never clipped actions. Optional tanh decoding is an
explicit controlled ablation, not a density correction. Fixed MC pairs and signed
clipping are unchanged. Discrete epsilon training needs at least two flow steps.

Retain CleanRL's critic, GAE, normalization, optimizer, gradient norm .5, rollout
sizes, and learning-rate schedule to isolate actor corrections. This remains a
Gym MuJoCo adaptation, NOT the paper's MJX setup or a claimed reproduction.
"""
...
class Args:
    ...
    action_transform: Literal["raw", "tanh"] = "raw"
