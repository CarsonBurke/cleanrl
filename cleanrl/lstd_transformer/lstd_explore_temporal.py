"""Temporal transformer: attention over last CONTEXT_LEN timesteps.

Each timestep's obs (17 dims) is embedded via MLP to EMBED_DIM, giving one token
per timestep. Transformer attends over the temporal sequence (5 tokens), giving
attention a meaningful job: modeling temporal dynamics and dependencies.

The agent maintains a rolling history buffer of past observations per env.
On episode reset, the buffer is cleared (zero-padded).

Architecture:
- Per-step MLP embed: obs(17) -> Linear(17,EMBED_DIM) -> SiLU -> Linear(EMBED_DIM,EMBED_DIM)
- 3x GQA transformer over CONTEXT_LEN temporal tokens
- Use last token's output (most recent obs) for action/value heads
...
class Agent(nn.Module):
    ...
    def reset_history(self, env_mask=None):
        """Reset history for environments that terminated. env_mask: (num_envs,) bool tensor."""
        if env_mask is None:
    ...
    def update_history(self, obs):
        """Push new obs into history buffer. obs: (num_envs, obs_dim)."""
        # Shift left, insert new obs at end
        self.obs_history = torch.cat([self.obs_history[:, 1:], obs.unsqueeze(1)], dim=1)

    def _make_backbone(self, obs_dim):
        """Create one transformer backbone with per-step MLP embeddings."""
        # Per-step MLP: obs(17) -> EMBED_DIM
        embed_fc1 = nn.Linear(obs_dim, EMBED_DIM)
        embed_fc2 = nn.Linear(EMBED_DIM, EMBED_DIM)
        std1 = 1.0 / obs_dim ** 0.5
        nn.init.trunc_normal_(embed_fc1.weight, std=std1, a=-2*std1, b=2*std1)
        std2 = 1.0 / EMBED_DIM ** 0.5
        nn.init.trunc_normal_(embed_fc2.weight, std=std2, a=-2*std2, b=2*std2)
        ...
        return embed_fc1, embed_fc2, embed_norm, layers, final_norm

    def _forward_backbone(self, obs_seq, embed_fc1, embed_fc2, embed_norm, layers, final_norm):
        """Forward pass. obs_seq: (B, CONTEXT_LEN, obs_dim). Returns last token features (B, EMBED_DIM)."""
        # Per-step MLP embedding: (B, CONTEXT_LEN, obs_dim) -> (B, CONTEXT_LEN, EMBED_DIM)
        tokens = F.silu(embed_fc1(obs_seq))
        tokens = embed_fc2(tokens)
        ...
        # Take last token (most recent timestep)
        return tokens[:, -1]

    def get_value(self, obs_seq):
        critic_features = self._forward_backbone(
            obs_seq, self.critic_embed_fc1, self.critic_embed_fc2, self.critic_embed_norm,
            self.critic_layers, self.critic_final_norm)
    ...
    def get_action_and_value(self, obs_seq, action=None):
        actor_features = self._forward_backbone(
            obs_seq, self.actor_embed_fc1, self.actor_embed_fc2, self.actor_embed_norm,
            self.actor_layers, self.actor_final_norm)
        critic_features = self._forward_backbone(
            obs_seq, self.critic_embed_fc1, self.critic_embed_fc2, self.critic_embed_norm,
            self.critic_layers, self.critic_final_norm)
...
if __name__ == "__main__":
    ...
    obs_dim = np.array(envs.single_observation_space.shape).prod()
    # Store full obs sequences for each step
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # Reset history for terminated envs, then push new obs
            if next_done.any():
