"""CLS token readout instead of flatten-project. Each backbone has a learnable
CLS token prepended to obs tokens. After transformer, CLS embedding is the
feature for that head.

Architecture:
- Actor backbone:  embed -> prepend CLS -> transformer -> CLS out -> action mean
- Critic backbone: embed -> prepend CLS -> transformer -> CLS out -> value
...
identical to lstd_explore_3backbone_simple.py except as noted above.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # RoPE cache: obs_dim + 1 to cover CLS position
        head_dim = EMBED_DIM // NUM_Q_HEADS
        rope_cos, rope_sin = build_rope_cache(obs_dim + 1, head_dim, torch.device('cpu'))
        ...
        (self.actor_embed_fc1, self.actor_embed_fc2, self.actor_embed_norm,
         self.actor_layers, self.actor_final_norm, self.actor_cls) = self._make_backbone(obs_dim)

        (self.critic_embed_fc1, self.critic_embed_fc2, self.critic_embed_norm,
         self.critic_layers, self.critic_final_norm, self.critic_cls) = self._make_backbone(obs_dim)

        # Actor: CLS output (EMBED_DIM) -> act_dim
        self.actor_out = layer_init(nn.Linear(EMBED_DIM, act_dim), std=0.01)

        # Critic: CLS output (EMBED_DIM) -> 1
    ...
    def _make_backbone(self, obs_dim):
        """Create one transformer backbone with per-dim MLP embeddings + CLS."""
        embed_fc1 = nn.Linear(1, EMBED_DIM)
        ...
        cls_token = nn.Parameter(torch.randn(1, 1, EMBED_DIM) * embed_std)

        return embed_fc1, embed_fc2, embed_norm, layers, final_norm, cls_token

    def _forward_backbone(self, x, embed_fc1, embed_fc2, embed_norm, layers, final_norm, cls_token, attn_gate=1.0):
        """Forward pass through one backbone. Returns CLS token output (B, EMBED_DIM)."""
        B = x.shape[0]

        # Per-dim MLP embedding: (B, obs_dim, 1) -> MLP -> (B, obs_dim, EMBED_DIM)
        tokens = x.unsqueeze(-1)
        tokens = F.silu(embed_fc1(tokens))
        tokens = embed_fc2(tokens)

        tokens = embed_norm(tokens)

        # Prepend CLS token: (B, 1+obs_dim, EMBED_DIM)
    ...
    def get_value(self, x, attn_gate=1.0):
        critic_features = self._forward_backbone(
            x, self.critic_embed_fc1, self.critic_embed_fc2, self.critic_embed_norm,
            self.critic_layers, self.critic_final_norm, self.critic_cls, attn_gate=attn_gate)
    ...
    def get_action_and_value(self, x, action=None, attn_gate=1.0):
        actor_features = self._forward_backbone(
            x, self.actor_embed_fc1, self.actor_embed_fc2, self.actor_embed_norm,
            self.actor_layers, self.actor_final_norm, self.actor_cls, attn_gate=attn_gate)
        critic_features = self._forward_backbone(
            x, self.critic_embed_fc1, self.critic_embed_fc2, self.critic_embed_norm,
            self.critic_layers, self.critic_final_norm, self.critic_cls, attn_gate=attn_gate)
