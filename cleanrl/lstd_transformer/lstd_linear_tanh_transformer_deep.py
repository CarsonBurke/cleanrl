"""Deep transformer backbone (3 layers) with pure linear actor/critic heads.

SOTA Transformer++ design following Gemma 2 / DeepSeek v3 conventions:
- Sandwich norm: pre-norm on input + post-norm on branch, clean additive residual
- QK-norm (RMSNorm on Q,K per-head before RoPE) — critical for head_dim=8
- RoPE shared across all layers (computed once in Agent)
- Depth-scaled output: branch outputs scaled by 1/sqrt(2*num_layers)
- Embedding norm after tokenizer
- Final norm before pooling
- SwiGLU FFN
- GQA (4 Q heads, 2 KV heads)
- Hypersphere projection (L2-norm + learnable scale) after pooling

Actor and critic are pure linear projections from transformer features.
SDE noise uses the standard 2-layer head.
"""
import os
import random
import time
...
class GQATransformerBlock(nn.Module):
    ...
    def forward(self, x, rope_cos, rope_sin):
        ...
        # Post-norm on branch, then clean additive residual (no runtime scaling —
        # PostNorm already bounds branch magnitude, init_scale handles early stability)
