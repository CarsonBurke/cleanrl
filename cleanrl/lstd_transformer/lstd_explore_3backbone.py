"""3 separate transformer backbones: actor, critic, and SDE each get their own
per-dim embeddings, embed_norm, transformer layers, and final_norm.

Ablation to test whether decoupling representations fully (instead of sharing
a single backbone with separate readout heads) improves performance. The three
backbones share only the RoPE cache.

Architecture:
- Actor backbone:  embed -> transformer -> flatten -> action mean
- Critic backbone: embed -> transformer -> flatten -> value
- SDE backbone:    embed -> transformer -> flatten -> SDE latent + variance offset

All other constants, hyperparams, GQATransformerBlock, training loop, etc. are
identical to lstd_explore_decoupled.py.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Three separate transformer backbones
        ...
        (self.sde_embed_w, self.sde_embed_b, self.sde_embed_norm,
         self.sde_layers, self.sde_final_norm) = self._make_backbone(obs_dim)
    ...
    def _get_action_std(self, sde_features):
        """SDE with state-dependent latent + state-dependent variance offset floor."""
        sde_latent = (self.sde_proj(sde_features) / SDE_PRESCALE).tanh()
        ...
        # State-dependent variance offset (exploration floor)
        log_std_offset = self.log_std_offset_proj(sde_features)
    ...
    def get_action_and_value(self, x, action=None, attn_gate=1.0):
        ...
        critic_features = self._forward_backbone(
            x, self.critic_embed_w, self.critic_embed_b, self.critic_embed_norm,
            self.critic_layers, self.critic_final_norm, attn_gate=attn_gate)
        sde_features = self._forward_backbone(
            x, self.sde_embed_w, self.sde_embed_b, self.sde_embed_norm,
            self.sde_layers, self.sde_final_norm, attn_gate=attn_gate)

        action_mean = self.actor_out(actor_features) * self.mean_scale
        action_std = self._get_action_std(sde_features)
