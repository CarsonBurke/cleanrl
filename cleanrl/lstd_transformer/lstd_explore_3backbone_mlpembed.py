"""Per-dim MLP embedding + transformer. Each obs scalar gets a nonlinear,
full-rank embedding via Linear(1,E)->SiLU->Linear(E,E) before self-attention.
This mirrors how LLMs get rich per-token embeddings before attention does mixing.

Architecture:
- Per-dim MLP embed: obs[i] -> Linear(1,EMBED_DIM) -> SiLU -> Linear(EMBED_DIM,EMBED_DIM)
- Actor backbone:  mlp_embed -> transformer -> flatten -> action mean
- Critic backbone: mlp_embed -> transformer -> flatten -> value
...
class Agent(nn.Module):
    ...
    def _make_backbone(self, obs_dim):
        """Create one transformer backbone with per-dim MLP embeddings."""
        # Per-dim MLP: (B, obs_dim, 1) -> (B, obs_dim, EMBED_DIM) -> SiLU -> (B, obs_dim, EMBED_DIM)
        # Implemented as grouped linear: weight shape (obs_dim, EMBED_DIM, 1) and (obs_dim, EMBED_DIM, EMBED_DIM)
        embed_fc1 = nn.Linear(1, EMBED_DIM)  # shared across dims, broadcast
        embed_fc2 = nn.Linear(EMBED_DIM, EMBED_DIM)
        # Init with fan-in scaling
        std1 = 1.0  # fan_in=1
    ...
    def _forward_backbone(self, x, embed_fc1, embed_fc2, embed_norm, layers, final_norm, attn_gate=1.0):
        """Forward pass through one backbone. Returns flattened obs features."""
        B = x.shape[0]

        # Per-dim MLP embedding: (B, obs_dim) -> (B, obs_dim, 1) -> MLP -> (B, obs_dim, EMBED_DIM)
        tokens = x.unsqueeze(-1)  # (B, obs_dim, 1)
        tokens = F.silu(embed_fc1(tokens))  # (B, obs_dim, EMBED_DIM)
        tokens = embed_fc2(tokens)  # (B, obs_dim, EMBED_DIM)
