"""2-backbone ablation: actor+SDE shared backbone, critic separate backbone.

Instead of one shared transformer backbone for all heads, this variant uses
TWO separate transformer backbones:
1. Actor+SDE backbone (shared) — features used for both action mean and SDE noise
2. Critic backbone (separate) — features used only for value estimation

Each backbone has its own per-dimension embedding, transformer layers, and final norm.
They share a single RoPE cache. This tests whether decoupling critic features from
actor/SDE features improves performance by removing gradient interference.

Architecture:
- actor_sde_embed_w/b, actor_sde_embed_norm, actor_sde_layers, actor_sde_final_norm
- critic_embed_w/b, critic_embed_norm, critic_layers, critic_final_norm
- Shared RoPE cache (rope_cos, rope_sin)

All other design choices (GQA, SwiGLU, sandwich norm, QK-norm, RoPE, gated attention,
SDE with variance offset, asymmetric clipping) are identical to lstd_explore_decoupled.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        (self.actor_sde_embed_w, self.actor_sde_embed_b, self.actor_sde_embed_norm,
         self.actor_sde_layers, self.actor_sde_final_norm) = self._make_backbone(obs_dim)
    ...
    def _make_backbone(self, obs_dim):
        """Create a full transformer backbone: embedding + layers + final norm."""
    ...
    def _forward_backbone(self, x, embed_w, embed_b, embed_norm, layers, final_norm, attn_gate=1.0):
        """Forward through a single backbone. Returns flattened obs features."""
        B = x.shape[0]
    ...
    def get_action_and_value(self, x, action=None, attn_gate=1.0):
        # Actor+SDE backbone
        actor_sde_features = self._forward_backbone(
            x, self.actor_sde_embed_w, self.actor_sde_embed_b, self.actor_sde_embed_norm,
            self.actor_sde_layers, self.actor_sde_final_norm, attn_gate=attn_gate)
        ...
        action_mean = self.actor_out(actor_sde_features) * self.mean_scale
        action_std = self._get_action_std(actor_sde_features)
