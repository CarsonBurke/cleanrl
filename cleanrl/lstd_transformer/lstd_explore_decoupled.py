"""Per-dimension transformer: each obs dim is a token, flatten->project to ALL heads.

Each observation dimension (joint angle, velocity, etc.) becomes its own token.
Self-attention learns real cross-dimension interactions (e.g., hip angle attending
to knee velocity). After the transformer, tokens are flattened and linearly
projected to each head (actor, critic, SDE) — no mean-pooling, no information loss.

Architecture:
- Per-dim embedding: each scalar obs[i] -> embed_dim via per-dimension weights
- RoPE across all obs tokens
- 3x GQA transformer blocks (sandwich norm, QK-norm, RoPE)
- Flatten (obs_dim * embed_dim) -> linear projection to actor, critic, AND SDE

v4: Removed SDE tokens. All heads (actor, critic, SDE) use flattened obs tokens.
    embed_dim=32 (head_dim=8). Gated attention (sigmoid gate, init ≈ 0.12).
    Restored state-dependent variance offset as exploration floor.
"""
import os
import random
import time
...
FFN_MULT = 2
NUM_LAYERS = 3
GATE_WARMUP_FRAC = 0.1  # linear ramp 0→1 over first 10% of training
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Per-dimension embedding: each scalar obs[i] -> EMBED_DIM
        # x_i * w_i + b_i for each dimension independently
        embed_std = 1.0 / EMBED_DIM ** 0.5
        self.dim_embed_w = nn.Parameter(torch.empty(obs_dim, EMBED_DIM))
        nn.init.trunc_normal_(self.dim_embed_w, std=embed_std, a=-2*embed_std, b=2*embed_std)
        self.dim_embed_b = nn.Parameter(torch.zeros(obs_dim, EMBED_DIM))
        ...
        # Flattened dim for readout (all heads use same flattened obs tokens)
        ...
        # SDE latent: flatten obs tokens -> linear -> sde_hidden
        sde_hidden = obs_flat_dim
        self.sde_proj = layer_init(nn.Linear(obs_flat_dim, sde_hidden), std=1.0)
        self.log_std_param = nn.Parameter(torch.zeros(sde_hidden, act_dim))

        # State-dependent variance offset (exploration floor)
        self.log_std_offset_proj = layer_init(nn.Linear(obs_flat_dim, act_dim), std=0.01)

    def _forward_transformer(self, x, attn_gate=1.0):
        """Returns flattened obs features."""
        B = x.shape[0]

        # Per-dimension embedding: (B, obs_dim) -> (B, obs_dim, EMBED_DIM)
        tokens = x.unsqueeze(-1) * self.dim_embed_w + self.dim_embed_b
    ...
    def _get_action_std(self, features):
        """SDE with state-dependent latent + state-dependent variance offset floor."""
        sde_latent = (self.sde_proj(features) / SDE_PRESCALE).tanh()

        # Base log_std (fixed learned matrix)
        log_std = (self.log_std_param + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
        std_sq = log_std.exp().pow(2)

        # SDE variance from latent
        action_var = (sde_latent.pow(2)) @ std_sq

        # State-dependent variance offset (exploration floor)
        log_std_offset = self.log_std_offset_proj(features)
        log_std_state = (log_std_offset + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
    ...
    def get_value(self, x, attn_gate=1.0):
        features = self._forward_transformer(x, attn_gate=attn_gate)
        return self.value_out(features)

    def get_action_and_value(self, x, action=None, attn_gate=1.0):
        features = self._forward_transformer(x, attn_gate=attn_gate)

        action_mean = self.actor_out(features) * self.mean_scale
        action_std = self._get_action_std(features)

        probs = Normal(action_mean, action_std)
        if action is None:
            action = probs.sample()

        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), self.value_out(features)
...
if __name__ == "__main__":
    ...
    gate_warmup_iters = int(GATE_WARMUP_FRAC * args.num_iterations)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        attn_gate = min(1.0, iteration / max(gate_warmup_iters, 1))
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(next_obs, attn_gate=attn_gate)
        ...
        with torch.no_grad():
            next_value = agent.get_value(next_obs, attn_gate=attn_gate).reshape(1, -1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(b_obs[mb_inds], b_actions[mb_inds], attn_gate=attn_gate)
        ...
        writer.add_scalar("gate/attn_gate", attn_gate, global_step)
