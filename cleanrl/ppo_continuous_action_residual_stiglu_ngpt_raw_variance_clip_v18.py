# Gradient-clipping ablations of raw variance-matched residual SiTU PPO v18.
# Keep v17 K101/sigma0.5, raw rewards, detached variance gain and shared Adam.
# Compare joint clipping, actor-only clipping (critic free), and no clipping.
# Hypothesis: critic variance gain can improve value fitting without using
# joint clipping to suppress actor updates. Preserve both trunks and projection.
import math
import os
import random
import time
...
class Args:
    ...
    grad_clip: Literal["global", "actor", "none"] = "global"
...
def categorical_value_terms(logits, target_probs, histogram):
    """CE scaled by detached batch-mean variance in raw discounted-return units."""
    ...
    with torch.no_grad():
        ...
        gain = (probabilities * (histogram.support - mean).square()).sum(dim=-1).mean()
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", f"original residual SiTU-GLU trunks and matrix projection; raw rewards; fixed symexp-spaced raw nodes; {args.value_target} mean-preserving labels; paired raw-expectation decode; detached predicted raw-variance CE gain; gradient clipping={args.grad_clip}; no return frame or EMA")
