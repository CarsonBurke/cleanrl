# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + DREAMER4 STATE-ENTROPY v1.
#
# ADD-ON over symlog_v1 (DEFAULT OFF => byte-identical to v1 unless --state-entropy):
# dreamer4's unimodal-Beta state-entropy exploration bonus, ported faithfully.
#
#   dreamer4 mechanism (dreamer4/dreamer4.py + discrete_continuous_embed_readout):
#     * BetaDist(unimodal=True): raw head params -> a = 1 + softplus(raw_a),
#       b = 1 + softplus(raw_b)  (both >= 1 => concave/unimodal Beta on (0,1)),
#       one Beta per state dimension. This is EXACTLY the same parameterization the
#       v24 actor already uses for actions; here it models the NEXT STATE.
#     * A state-prediction head (to_state_pred / to_agent_state_pred) emits those
#       params to predict s' as a per-dim Beta. Targets are tanh-bounded latents
#       rescaled to (0,1) via (x+1)/2 then clamped to [eps, 1-eps]; the head is
#       trained by NLL  -Beta.log_prob(target)  (state_pred_loss_weight, def 0.1).
#     * state_entropy_bonus_weight (def 0.05) * H[Beta]  -- the differential entropy
#       of the predicted next-state distribution -- is the intrinsic reward: the
#       agent is paid to reach states where the forward model is most uncertain
#       (curiosity / Plan2Explore-style epistemic drive). Unimodal-Beta H lives in
#       (-inf, 0]; only its per-state VARIATION is the exploration signal (the
#       constant offset is absorbed by the value baseline / rank-normalization).
#
#   This port:
#     * A SELF-CONTAINED forward model p(s'|s,a) = prod_i Beta(a_i,b_i) (its own MLP
#       encoder over [obs, action]; dim state_pred_hidden). It is DECOUPLED from the
#       actor/critic ThinkTrunks, so state_entropy=False leaves v1 untouched and the
#       A/B is clean. Trained by masked NLL (terminal transitions excluded: their s'
#       is a fresh reset, not a real successor).
#     * The bonus enters the POLICY ADVANTAGE ONLY, via the SAME soft-GAE machinery
#       this file already uses for the auto-entropy path -- NEVER the HL-Gauss critic
#       target. This honors v1's central thesis (keep the bounded categorical critic
#       regressing the RAW reward return so its fixed support stays calibrated,
#       edge_mass~=0) while still giving the bonus MULTI-STEP credit through the GAE
#       recursion. With state_entropy_norm (def True) the per-rollout bonus is
#       standardized before weighting so it can never overflow anything; rankgauss
#       advantage shaping then preserves the entropy-induced reordering.
#
...
class Args:
    ...
    state_entropy_bonus_weight: float = 0.05  # dreamer4 default
    ...
    state_entropy_norm: bool = True           # standardize the bonus over the rollout before weighting
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # dims, masked to non-reset transitions. Targets map s' into (0,1) via
                # dreamer4's (tanh(x)+1)/2 squash, clamped off the open boundary.
                if args.state_entropy:
                    fwd_dist = agent.state_model.dist(b_obs[mb_inds], b_actions[mb_inds])
                    tgt = ((b_next_obs[mb_inds].tanh() + 1.0) * 0.5).clamp(
        ...
        if args.state_entropy:
            ...
            writer.add_scalar("debug/state_bonus_mean", state_bonus.mean().item(), global_step)
