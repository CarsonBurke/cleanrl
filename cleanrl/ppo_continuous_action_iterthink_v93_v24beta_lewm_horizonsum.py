# PPO + IterThink v93 (v24 Beta + obs-state LeWM horizon-sum heads). From v92.
#
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.num_latent_tokens = self.num_obs_tokens
    ...
    def _outcome_feature(self, summary, detach_summary=True):
        ...
        return self.outcome_norm(summary).mean(dim=1)

    def decode_outcomes(self, summary, detach_summary=True):
        feat = self._outcome_feature(summary, detach_summary)
        reward_logits = self.reward_outcome_unproj(feat)
        continuation_logits = self.continuation_outcome_unproj(feat).squeeze(-1)
        return reward_logits, continuation_logits

    def decode_value(self, summary, detach_summary=True):
        feat = self._outcome_feature(summary, detach_summary)
        return self.value_outcome_unproj(feat)
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # Outcome heads train from action-conditioned predicted obs
                    # latents; outcomes are not recurrent tokens.
                    ...
                    wm_value_loss = wm_pred_value_loss
