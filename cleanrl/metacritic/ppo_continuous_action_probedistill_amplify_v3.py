# PROBE-DISTILL + TEACHER AMPLIFICATION v3.
#
# Ancestor: ppo_continuous_action_iterthink_v24_beta_d4hlgauss_symlog_rawret_
# idxtransfer_tpomd_v5_dyntrust.py (10411 on HalfCheetah-v4; best pure-distillation
# actor in the repo). This file is that file plus EXACTLY ONE new mechanism,
# gated by --amplify / --no-amplify. --no-amplify is bit-identical to the ancestor
# (verified: all 30 non-wall-clock TensorBoard scalars match exactly over 3
# iterations at seed 1).
#
# THE BOTTLENECK. The ancestor's actor loss is entirely
# tpo_coef * CE(q_frozen, softmax(new logprobs restricted to K candidates)) with
# K = tpo_k = 8. Both the teacher's softmax and the student's CE therefore live on
# 8 sampled atoms: the update can only reshape RELATIVE preference among 8 points
# of a 6-dimensional continuous action space. It is a K-sample Monte-Carlo estimator
# of a full-support KL projection, and its variance is set by K, not by the quality
# of the scores. Raising K is not an option — every atom costs one real MuJoCo step
# with a save/restore, and the probe already dominates wall clock (18.2 s of the
# rollout at the launch config).
#
# THE MECHANISM. The probe hands us, for free, K=8 EXACT one-step oracle labels per
# state: s_k = r_k + gamma*(1-term_k)*V_old(s'_k), real physics + frozen critic.
# Regress a small head on those labels, then use the head to score
# tpo_k + amplify_extra = 8 + 56 = 64 candidates per state at ZERO physics cost
# (the extra 56 are fresh draws from the STORED rollout policy at that state). The
# teacher softmax, the eta bisection and the actor CE then run over 64 atoms with
# the ancestor's formulas unchanged. Physics cost is untouched: still exactly 8
# probed candidates per state.
#
# WHY ONE ESTIMATOR FOR ALL 64, not oracle-for-8 + learned-for-56. A softmax is a
# comparison. Feeding it 8 exact values and 56 values from a different, biased
# estimator makes the comparison depend on the gap between the two estimators, not
# on action quality: whichever family is systematically optimistic collects the
# target mass, and the resulting policy drift has nothing to do with the reward.
# Scoring all 64 with the head keeps the ORDERING (the only thing the teacher
# consumes) internally consistent. The oracle's job is to TRAIN the estimator, not
# to score alongside it. The measured cost of this choice is visible and small:
# debug/amplify_extra_weight_frac sits at 0.8755 +/- 0.0015, i.e. within 0.1% of
# the neutral 56/64 = 0.875, so the teacher shows no probed-vs-extra subset bias.
#
# WHY THE HEAD IS DETACHED FROM THE TRUNK. OracleQHead.forward() calls
# critic_feat.detach() internally (single enforcement point). The shared ThinkTrunk
# is the proven chassis of the 10411 run; letting a brand-new auxiliary regression
# push gradient into it would change the representation and confound the one thing
# this file is meant to measure. Verified numerically: backwarding the Q-head loss
# alone leaves all 279 non-Q-head parameters with grad None (zero contribution) —
# through get_qhead() and through get_action_and_value(qhead_zs=...) — while all 6
# Q-head tensors get nonzero grad. The head still rides the CRITIC's grad clip and
# the critic's backward, since it is a value-side object.
#
# TWO DEFECTS FOUND AND FIXED AFTER THE FIRST IMPLEMENTATION. The mechanism above was
# built to spec and was measurably ALIVE BUT USELESS: rank_corr averaged +0.061 over 20
# iterations while the IDENTICAL head fitted offline with its own optimizer reached
# held-out +0.51. That gap was the whole story, and it had two causes.
#
# FIX 1 -- THE Q HEAD HAD NO GRADIENT BUDGET OF ITS OWN. It was placed in
# critic_parameters() and therefore renormalized by clip_grad_norm_(critic_params, 0.25)
# TOGETHER with the value CE, whose measured pre-clip norm is 84..358 -- a shared factor
# of ~0.002. This file already clips the actor and the critic separately for exactly this
# reason ("the critic's CE gradient cannot swamp the policy's contribution"), so a third
# objective sharing the second one's budget was an omission, not a design. critic_params
# is still the routing/optimizer set, but for CLIPPING it is now partitioned (asserted, at
# startup) into value_only_params and qhead_params with independent max-norms.
# MEASURED EFFECT, same tiny config: rank_corr 0.00 -> 0.07 -> 0.23 -> 0.33 within FOUR
# iterations (was +0.061 mean over twenty), and losses/qhead_loss now falls monotonically
# 0.242 -> 0.120 -> 0.084 -> 0.035 where before it ROSE ~4x. The diagnosis was correct.
#
# FIX 2 -- THE COLD START WAS FATAL. An untrained head orders candidates by noise, so the
# 64-atom teacher started at 95% of maximum entropy (tpo_q_entropy 3.968 of log64 = 4.159)
# with eta pinned at its floor and kl_base ~ 0: an information-free teacher. The ancestor
# is informative from step 1 and reaches 1375 by 400k, so a run whose actor idles until the
# head warms up never catches up. FIX: a HELD-OUT COMPETENCE GATE. rank_corr is already
# computed with the FROZEN pre-update head on THIS rollout's fresh states and actions,
# BEFORE the update loop trains on them -- so it is a genuine out-of-sample read, free.
# When it is below amplify_min_rank the iteration falls back to the ancestor's exact 8-atom
# ORACLE teacher; the head keeps training regardless, so the gate opens by itself. No
# step-count schedule, no annealing knob: the oracle labels do double duty as training
# signal AND as the competence certificate that licenses their own replacement.
#
# THE SAFETY PROPERTY THIS BUYS, PROVEN NOT ARGUED. With the gate closed the file is
# BIT-EXACT to the ancestor: forcing it shut (--amplify-min-rank 1.1) and diffing every
# one of the 32 numeric TensorBoard scalars against an ancestor run at the same seed gives
# ZERO differing tags, while losses/qhead_loss and rank_corr confirm the head is still
# learning off to the side. That holds because the head is detached from the trunk, has its
# own clip group, and its parameters touch nothing else. So this file cannot underperform
# the ancestor through incompetence -- only through the deliberate, measured act of
# switching to a teacher that has already demonstrated out-of-sample ordering skill.
# Observed gate behaviour at the tiny config: k_total = 8, 8, 8, 64, 8 -- it switched on
# exactly when rank_corr crossed the bar (0.333) and back off when it fell (0.183).
#
# MEASURED FIDELITY (HalfCheetah-v4, seed 1, num_envs 4, num_steps 256; TINY config, 0.06%
# of the launch budget -- treat as a mechanism check, not a performance claim):
#   debug/qhead_rank_corr     : -0.008, +0.071, +0.229, +0.333, +0.276 over 5 iterations.
#       Per-iteration SE with K=8 over 1024 groups is ~0.012, so +0.33 is ~27 sigma.
#   debug/qhead_err_vs_spread : stays high (~8). The floor of this metric is NOT 1.0: a
#       level-perfect but action-BLIND predictor (the per-state oracle group mean) scores
#       0.836. It is dominated by per-state LEVEL error, which the teacher never consumes
#       -- the teacher only compares candidates WITHIN a state. rank_corr is the honest
#       kill metric; err_vs_spread is kept only because a level blow-up would show there.
#   Offline control (identical head, own Adam(1e-3), one frozen batch, 80/20 state split):
#       held-out rank_corr +0.486 @1000 steps, +0.514 @2000. So ~+0.5 is the realistic
#       ceiling for this feature set, and the gate's 0.25 bar is reachable but not free.
#
# FALSIFIABLE PREDICTIONS (launch config: num_envs 16, num_steps 2048, 8M steps):
#   P1. debug/amplify_active spends the MAJORITY of iterations at 1.0 after 2M steps. If it
#       is still mostly 0 the head never became competent at scale and this file is just
#       the ancestor with wasted compute -- a clean, honest negative, not a silent one.
#   P2. debug/amplify_extra_weight_frac stays inside [0.80, 0.94] whenever the gate is
#       open. ~0 means the head systematically under-scores unprobed actions (inert); ~1
#       means it over-scores them and the teacher is chasing its own optimism.
#   P3. Return beats the ancestor's 10411. Given the safety property, the only way to lose
#       materially is if an OPEN gate actively hurts -- i.e. a head at rank_corr 0.3-0.5 is
#       worse than exact oracle ordering over 8 atoms. That is the real scientific question
#       this file asks: does 64 approximately-ordered candidates beat 8 exactly-ordered
#       ones? If P1 holds and P3 fails, the answer is no, and the bar should be raised
#       rather than the mechanism abandoned.
#
# NOT VERIFIED / KNOWN GAPS:
#   * No return/score claim from any short run. Nothing here should be read as an
#     improvement over 10411 until the 8M run reports.
#   * The gate flickers at small batch (1024 states). At the launch config there are 32768
#     states per iteration, ~5.7x smaller SE, so less flicker is expected but not verified.
#   * Only actor_dist="beta" was exercised. The gaussian path stores (loc, scale)
#     and reuses the ancestor's tanh log-Jacobian, but was not run.
#
# ------------------------------------------------------------------------------
# --- inherited ancestor notes (unchanged) ---
...
class Args:
    ...
    # --- TEACHER AMPLIFICATION (the one new mechanism) ---
    # The probe already pays real physics for K=8 EXACT one-step oracle Q labels per
    # state. Regress a small head on those labels, then use the head to score
    # tpo_k + amplify_extra candidates at ZERO physics cost, so the teacher softmax
    # and the actor CE run over a dense candidate set instead of 8 sampled atoms.
    # --no-amplify disables ALL of it (head is built but never trained, never read),
    # making this file bit-identical to the ancestor.
    amplify: bool = True
    amplify_extra: int = 56      # extra rollout-policy draws per state scored by the head (total = tpo_k + this)
    qhead_hidden: int = 0        # <=0 => args.hidden
    qhead_coef: float = 1.0      # weight of the Q-head regression inside the CRITIC's gradient budget
    # Held-out competence gate. rank_corr is measured with the FROZEN pre-update head on
    # THIS rollout's fresh states/actions, before the update loop trains on them, so it is
    # an honest out-of-sample read of whether the head can order actions at all. Below the
    # bar the 64-atom teacher is replaced by the ancestor's exact 8-atom ORACLE teacher, so
    # the run can never be worse-informed than the ancestor. 0.25 is ~20x the K=8/1024-group
    # sampling SE (0.0118), i.e. "demonstrably ordering", not "not quite chance".
    amplify_min_rank: float = 0.25
    # The Q head is a NEW parameter group. Sharing the value head's single max-norm meant a
    # measured pre-clip critic norm of 84..358 renormalized the whole group by ~0.002; this
    # file already clips actor and critic separately for exactly this reason, so the head
    # gets its own budget rather than riding one calibrated for a different objective.
    qhead_grad_clip: float = 0.25
...
class OracleQHead(nn.Module):
    """(critic_feat, action_z) -> scalar prediction of symlog(one-step oracle Q).

    Trained ONLY on the K probed candidates, whose labels
    s_k = r_k + gamma*(1-term_k)*V_old(s'_k) are EXACT (real physics + frozen
    critic), then queried on arbitrarily many extra actions at zero physics cost.

    Two deliberate design constraints:
      * critic_feat is DETACHED inside forward(). The head can never backprop into
        the shared trunk: the trunk is the proven chassis of the 10411 ancestor and
        any perturbation of it would confound the amplification experiment. The
        detach lives here (single enforcement point) rather than at the call sites.
      * The head regresses in SYMLOG coordinates. Raw oracle Q on HalfCheetah is
        O(1e3) with O(1) within-group spread, which is hopeless for a small MLP in
        raw units; symlog is the same remedy the critic already uses in this file.
        Predictions are mapped back with symexp before being used as scores.
    """

    def __init__(self, feat_dim, act_dim, hidden):
        super().__init__()
        self.fc1 = layer_init(nn.Linear(feat_dim + act_dim, hidden))
        self.fc2 = layer_init(nn.Linear(hidden, hidden))
        self.act = ReLUSquared()
        # Zero-init readout => the head outputs EXACTLY 0 at init, so symexp(0)=0
        # scores are perfectly flat and the amplified teacher starts as the anchor.
        self.out = layer_init(nn.Linear(hidden, 1), std=0.1)
        with torch.no_grad():
            self.out.weight.zero_()
            self.out.bias.zero_()

    def forward(self, critic_feat, cand_zs):
        # critic_feat (B, F), cand_zs (B, C, A) -> (B, C) in symlog coordinates.
        feat = critic_feat.detach().unsqueeze(1).expand(-1, cand_zs.shape[1], -1)
        h = torch.cat([feat, cand_zs], dim=-1)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # TEACHER AMPLIFICATION: oracle-Q head. Built LAST so every parameter above
        # is initialized from the exact same RNG draws as the ancestor.
        self.qhead = OracleQHead(H, act_dim, args.qhead_hidden)
    ...
    def get_qhead(self, x, cand_zs):
        # Oracle-Q head prediction in SYMLOG coords, (B, C). Used post-rollout with
        # the frozen pre-update net to score the enlarged candidate set. The trunk
        # gradient path is severed inside OracleQHead.forward.
        _, critic_feat = self._trunks(x)
        return self.qhead(critic_feat, cand_zs)

    def get_action_and_value(self, x, z=None, candidate_zs=None, qhead_zs=None, return_dist=False):
        ...
        #   qhead_zs (B, C, A): also return the oracle-Q head's symlog prediction
        #     (B, C) from the SAME (detached) critic feature — no extra trunk pass,
        #     no RNG, and no gradient path into the trunk.
        ...
        if qhead_zs is not None:
            out = out + (self.qhead(critic_feat, qhead_zs),)
        if return_dist:
    ...
    def critic_parameters(self):
        # Params receiving the VALUE gradient (incl. the shared trunk). The oracle-Q
        # head is a value-side object, so it rides the CRITIC's clip budget and the
        # critic backward. It is NOT in actor_parameters(): the actor must never see
        # a Q-head gradient.
        trunk = self.trunk if self.share_backbone else self.critic_trunk
        return (
            list(trunk.parameters())
            + list(self.critic_head.parameters())
            + list(self.qhead.parameters())
        )
...
def spearman_rows(a, b):
    """Row-wise Spearman rank correlation over the last axis, (..., C) -> (...).

    The teacher consumes only the ORDER of candidate scores, so rank fidelity —
    not squared error — is the honest measure of whether the Q head is usable.
    Ties are broken by argsort order; with continuous predictions exact ties only
    occur at init (all-zero head), where the resulting correlation is genuinely
    arbitrary and averages to ~0 across states, which is the correct reading.
    """
    n = a.shape[-1]
    ra = a.argsort(dim=-1).argsort(dim=-1).to(torch.float32)
    rb = b.argsort(dim=-1).argsort(dim=-1).to(torch.float32)
    return 1.0 - 6.0 * (ra - rb).pow(2).sum(-1) / (n * (n * n - 1))
...
if __name__ == "__main__":
    ...
    if args.qhead_hidden <= 0:
        args.qhead_hidden = args.hidden
    ...
    assert args.amplify_extra >= 0, "amplify_extra must be >= 0"
    assert args.qhead_coef >= 0.0, "qhead_coef must be >= 0"
    # Total candidate count seen by the teacher softmax / actor CE.
    tpo_k_total = args.tpo_k + args.amplify_extra if args.amplify else args.tpo_k
    # Disjoint parameter groups for clipping. critic_params stays the OPTIMIZER/routing set
    # (unchanged); value_only_params is critic_params minus the Q head, so the two objectives
    # get independent max-norms instead of one shared renormalization factor.
    ...
    # AMPLIFY: a THIRD, independent persistent RNG stream for the extra
    # (physics-free) candidate draws — same save/restore-inside-fork_rng discipline
    # as the probe stream. Separate state means enabling --amplify perturbs neither
    # the MAIN stream (executed actions stay bit-identical) nor the PROBE stream
    # (the 8 probed candidates stay bit-identical).
    amp_cpu_rng_state = None
    amp_cuda_rng_state = None
    ...
    # Clip groups. critic_params stays the routing/optimizer set; for CLIPPING it is split
    # so the Q-head regression and the value CE cannot renormalize each other.
    qhead_params = list(agent.qhead.parameters())
    _qhead_ids = {id(q) for q in qhead_params}
    value_only_params = [q for q in critic_params if id(q) not in _qhead_ids]
    assert len(value_only_params) + len(qhead_params) == len(critic_params), (
        "clip groups must partition critic_params exactly"
    )
    ...
    # AMPLIFY: the rollout policy's per-state distribution parameters. The ancestor
    # keeps only the K sampled candidates, which is not enough to draw MORE actions
    # from the same rollout distribution after the fact, so store the two natural
    # parameters: (alpha, beta) for beta, (loc, scale) for gaussian. Reconstructing
    # the dist from these gives a bit-identical distribution object, hence bit-
    # identical log-densities for the K probed candidates.
    if args.amplify:
        tpo_dist_p1 = torch.zeros(
            (args.num_steps, args.num_envs) + envs.single_action_space.shape
        ).to(device)
        tpo_dist_p2 = torch.zeros(
            (args.num_steps, args.num_envs) + envs.single_action_space.shape
        ).to(device)

        def rollout_dist_from_params(p1, p2):
            # Rebuild the rollout distribution and its native->action log-Jacobian.
            if args.actor_dist == "beta":
                return Beta(p1, p2), (lambda z: 0.0)
            return Normal(p1, p2), (lambda z: 2.0 * (log(2.0) - z - F.softplus(-2.0 * z)))
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if args.amplify:
                    # Natural parameters of the rollout dist at this state (no RNG,
                    # no graph: pure bookkeeping for the post-rollout extra draws).
                    if args.actor_dist == "beta":
                        tpo_dist_p1[step] = roll_dist.concentration1
                        tpo_dist_p2[step] = roll_dist.concentration0
                    else:
                        tpo_dist_p1[step] = roll_dist.loc
                        tpo_dist_p2[step] = roll_dist.scale
        ...
        with torch.no_grad():
            ...
            # `scores` (B, K) are EXACT one-step oracle Q labels that real physics
            # already paid for. They serve two roles here: (a) the Q head's
            # regression target, in symlog coordinates, and (b) with --amplify, the
            # thing the head is trained to imitate so it can score an ENLARGED
            # candidate set for free.
            #
            # The head scores ALL candidates, INCLUDING the K probed ones whose
            # exact values we already know. That is deliberate. A softmax over a
            # mixture of 8 exact and 56 learned values compares two differently
            # biased estimators, so any optimism gap between them tilts the whole
            # target toward one subset for reasons that have nothing to do with
            # action quality. One estimator for every candidate makes the ordering
            # honest; the oracle's job is to TRAIN the estimator, not to score
            # alongside it.
            if args.amplify:
                b_tpo_scores_symlog = symlog(scores)                         # (B, K) labels
                flat_obs = obs.reshape(-1, obs_dim)
                # 1) EXTRA candidates: fresh draws from the STORED rollout policy at
                #    each state, on the dedicated amplify RNG stream (main + probe
                #    streams untouched => executed actions and probed candidates are
                #    bit-identical to a --no-amplify run).
                amp_p1 = tpo_dist_p1.reshape((-1,) + envs.single_action_space.shape)
                amp_p2 = tpo_dist_p2.reshape((-1,) + envs.single_action_space.shape)
                amp_dist, amp_log_det_fn = rollout_dist_from_params(amp_p1, amp_p2)
                with torch.random.fork_rng(devices=[device]):
                    if amp_cpu_rng_state is None:
                        torch.manual_seed(args.seed + 2_000_003)
                    else:
                        torch.set_rng_state(amp_cpu_rng_state)
                        torch.cuda.set_rng_state(amp_cuda_rng_state, device)
                    extra_z = amp_dist.sample(torch.Size([args.amplify_extra]))  # (E, B, A)
                    amp_cpu_rng_state = torch.get_rng_state()
                    amp_cuda_rng_state = torch.cuda.get_rng_state(device)
                extra_z = extra_z.permute(1, 0, 2).contiguous()               # (B, E, A)
                if args.actor_dist == "beta":
                    extra_z = extra_z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                # 2) Enlarged set = probed K first (candidate 0 is STILL the executed
                #    action), then the extras.
                b_tpo_zs = torch.cat([b_tpo_zs, extra_z], dim=1)              # (B, Ktot, A)
                ez = extra_z.transpose(0, 1)                                  # (E, B, A)
                extra_logprob = (
                    amp_dist.log_prob(ez) - amp_log_det_fn(ez)
                ).sum(-1).transpose(0, 1)                                     # (B, E)
                tpo_cand_logprobs = torch.cat(
                    [tpo_logprobs.reshape(-1, args.tpo_k), extra_logprob], dim=1
                )                                                             # (B, Ktot)
                # 3) Score EVERY candidate with the head (frozen pre-update net),
                #    chunked over states to bound the (B, Ktot, H+A) activation.
                qhead_pred_all = torch.cat(
                    [
                        agent.get_qhead(o_chunk, z_chunk)
                        for o_chunk, z_chunk in zip(flat_obs.split(4096), b_tpo_zs.split(4096))
                    ]
                )                                                             # (B, Ktot) symlog
                # 4) Fidelity telemetry on the probed atoms, i.e. exactly where the
                #    ground truth is known. qhead_err_vs_spread is the load-bearing
                #    number: >= 1 means the head cannot resolve the differences the
                #    teacher is trying to exploit and the amplification is noise.
                pred_probed = qhead_pred_all[:, : args.tpo_k]
                abs_err = (pred_probed - b_tpo_scores_symlog).abs()
                qhead_err_symlog = abs_err.mean().item()
                label_spread = b_tpo_scores_symlog.std(dim=-1, unbiased=False)  # (B,)
                qhead_err_vs_spread = (
                    abs_err.mean() / label_spread.mean().clamp_min(1e-12)
                ).item()
                qhead_rank_corr = spearman_rows(pred_probed, b_tpo_scores_symlog).mean().item()
                amplify_active = qhead_rank_corr >= args.amplify_min_rank
            if args.amplify and amplify_active:
                teacher_scores = symexp(qhead_pred_all)                       # raw return units
                anchor_logp = F.log_softmax(tpo_cand_logprobs, dim=-1)
            elif args.amplify:
                # GATED OFF this iteration: the head has not demonstrated, on labels it has
                # not yet trained on, that it can order actions. Fall back to the exact
                # oracle teacher over the 8 PROBED atoms (candidate 0 still the executed
                # action). The head keeps training on the oracle labels regardless, so the
                # gate opens by itself once it is competent -- no step-count schedule.
                b_tpo_zs = b_tpo_zs[:, : args.tpo_k].contiguous()
                teacher_scores = scores
                anchor_logp = F.log_softmax(tpo_logprobs.reshape(-1, args.tpo_k), dim=-1)
            else:
                b_tpo_scores_symlog = None
            ...
            u_scores = (
                (teacher_scores - teacher_scores.mean(dim=-1, keepdim=True)) / tpo_sigma_global
            ).clamp(-5.0, 5.0)
            ...
            if args.amplify:
                # Fraction of the teacher's target mass sitting on the NEW,
                # never-probed candidates. ~0 => the amplification is inert; ~1 =>
                # the teacher has abandoned the oracle-anchored actions entirely.
                amplify_extra_weight_frac = b_tpo_q[:, args.tpo_k :].sum(-1).mean().item()
            tpo_k_actual = b_tpo_q.shape[-1]
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_cand_zs = b_tpo_zs[mb_inds]
                if args.amplify:
                    # The Q head trains ONLY on the K probed candidates — the only
                    # actions with exact labels — while the CE above runs over the
                    # full enlarged set.
                    (
                        _, _, newlogprob, entropy, value_logits, new_cand_logprobs, qhead_pred,
                    ) = agent.get_action_and_value(
                        b_obs[mb_inds],
                        b_latent_zs[mb_inds],
                        candidate_zs=mb_cand_zs,
                        qhead_zs=mb_cand_zs[:, : args.tpo_k],
                    )
                    qhead_loss = F.smooth_l1_loss(
                        qhead_pred, b_tpo_scores_symlog[mb_inds].detach()
                    )
                else:
                    _, _, newlogprob, entropy, value_logits, new_cand_logprobs = agent.get_action_and_value(
                        b_obs[mb_inds], b_latent_zs[mb_inds], candidate_zs=mb_cand_zs
                    )
                    qhead_loss = None
                ...
                # The Q head is a VALUE-side object: its regression rides the critic's
                # backward and the critic's grad clip. critic_feat is detached inside
                # OracleQHead.forward, so this term adds EXACTLY zero gradient on the
                # trunk / critic head — it only fills the Q head's own parameters.
                critic_loss = args.vf_coef * v_loss
                if qhead_loss is not None:
                    critic_loss = critic_loss + args.qhead_coef * qhead_loss
                ...
                if args.separate_grad_clip:
                    ...
                    if actor_active:
                        ...
                        critic_gn = nn.utils.clip_grad_norm_(value_only_params, args.critic_grad_clip)
                        qhead_gn = nn.utils.clip_grad_norm_(qhead_params, args.qhead_grad_clip) if qhead_params else 0.0
        ...
        if args.amplify:
            # THE LOAD-BEARING DIAGNOSTIC IS qhead_err_vs_spread. The teacher only
            # gains from amplification if the head can resolve differences SMALLER
            # than the within-group spread it is asked to rank; >= ~1 means the
            # amplified candidates are being ordered by noise.
            writer.add_scalar("debug/qhead_err_vs_spread", qhead_err_vs_spread, global_step)
            writer.add_scalar("debug/qhead_err_symlog", qhead_err_symlog, global_step)
            writer.add_scalar("debug/qhead_rank_corr", qhead_rank_corr, global_step)
            writer.add_scalar("debug/amplify_extra_weight_frac", amplify_extra_weight_frac, global_step)
            writer.add_scalar("losses/qhead_loss", qhead_loss.item(), global_step)
            writer.add_scalar("debug/tpo_k_total", tpo_k_actual, global_step)
            writer.add_scalar("debug/amplify_active", float(amplify_active), global_step)
