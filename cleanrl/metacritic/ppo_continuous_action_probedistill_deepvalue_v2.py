# PROBE-DISTILL + DEEP VALUE v2 -- v1 plus the EXPECTED-BACKUP CRITIC AUXILIARY.
# One lever over v1. Gated by --probe-backup / --no-probe-backup; with the gate off this
# file is bit-exact v1 (proven below, not assumed). HalfCheetah-v4 only. Actor untouched:
# still pure CE(q_frozen, pi) distillation, no ratio, no PG, no clone term.
# =====================================================================================
# THE OBSERVATION. The probe already steps K=8 candidate actions through real physics and
# builds scores_k = r_k + gamma(1-term_k) V_old(s'_k). Those K numbers are then CENTERED
# per group -- and the group MEAN, which centering discards, is thrown away. But the K
# candidates are iid draws from the rollout policy, so
#
#     mean_k [ r_k + gamma(1-term_k) V_old(s'_k) ]  =  a K-sample EXPECTED-SARSA backup
#                                                     of V_{pi_old}(s)
#
# i.e. a valid Bellman backup of the SAME V the lambda-return targets, with sqrt(K) times
# less sampling noise than the single-sample TD(0) term inside that lambda-return. It costs
# ZERO extra physics steps, zero extra forwards: it is a byproduct that was being deleted.
#
# WHY THIS AND NOT SOMETHING ELSE. The measured ordering of every lever in this lineage says
# the binding constraint is the TEACHER'S INFORMATION, and the teacher's scores are exactly
# r + gamma V(s'): their noise floor IS the critic's error. My advantage-conditioned teacher
# (one action, one scalar credit) tops out at 8718 and its edge over PPO decays monotonically
# +24% -> +6.4%; swapping in this oracle probe teacher (eight actions, real one-step outcomes)
# reaches 10411. So sharpening V sharpens the teacher, which is upstream of the actor.
#
# NOT A NEW OBJECTIVE. Both targets are backups of the same fixed point (lambda=0.95 with one
# sample over the executed trajectory; lambda=0 with K samples here), so mixing them cannot
# move where the critic converges -- only how fast and how noisily. That is why this needs no
# trust region, no annealing, and no separate head.
#
# HONEST TENSION, measured not hidden: `losses/explained_variance` scores the critic against
# the LAMBDA-RETURN, so it is structurally biased AGAINST this change -- spending capacity on
# a second target must cost lambda-return fit. In the 3-iteration check the probe CE fell
# 4.296 -> 2.690 while EV read -0.001 (on) against +0.009 (off), both indistinguishable from
# zero at 3 iterations but pointing the wrong way. THEREFORE: this file is falsified or
# confirmed on RETURN, not on EV. Do not read EV here as the verdict.
#
# probe_backup_coef = 1.0 is UNTUNED and deliberately not searched: equal weight is the only
# non-arbitrary choice without an inverse-variance estimator. If v2 loses to v1, the two
# candidate explanations are "TD(0) bias dominates" and "coefficient too high"; those are
# separated by one run at 0.25, not by a grid.
#
# VERIFIED BEFORE LAUNCH, not assumed. All numbers below are from
# `--num-envs 4 --num-steps 256 --num-minibatches 4 --total-timesteps 3072 --seed 1`
# on HalfCheetah-v4 (3 iterations); stating the config so the digits are reproducible.
#   * --no-probe-backup reproduces v1 EXACTLY: value_loss [4.421194, 4.195661, 3.849508] and
#     tpo_ce [1.863611, 1.817242, 1.800047] are identical to a v1 reference run, same seed.
#   * the lever is NOT inert with the gate on: value_loss moves to [4.755, 4.535, 4.398] and
#     probe CE is nonzero and falling.
#   * the backup is a DIFFERENT target from the lambda-return: debug/probe_backup_gap is
#     nonzero throughout (4.39 -> 2.53); had it been ~0 the term would have been a no-op.
#   * HLGaussSupport.project is elementwise and order-preserving (verified: projecting a
#     flattened tensor and reshaping equals projecting the unflattened one), so reusing the
#     probe's (T,N,K)->(B,K) flattening for a (T,N)->(B) target cannot misalign states.
#   * sqrt(K) variance reduction is real, not asserted: single-sample sd 119.6 vs K=8-mean
#     sd 42.5, ratio 2.817 against sqrt(8)=2.828.
#
# INDEPENDENTLY RED-TEAMED before launch (read-only audit, 8 checks, all PASS, no CRITICAL
# or MAJOR). Two of its results are worth keeping because they are stronger than mine:
#   * ALIGNMENT PROVEN, not argued. Probe candidate 0 IS the executed action, so its raw
#     probe reward must equal the executed transition's reward at the same (t, env). Under
#     the shipped (T,N,K)->(B,K) flattening that match is BIT-EXACT (max abs error 0.0)
#     across every row; under the transposed (env, step) hypothesis the error is 1.94. So
#     row i of `scores` is provably the same state as row i of b_obs/b_target_probs.
#   * A MISSED SQUEEZE WOULD HAVE BEEN SILENT, not loud: `b_target_probs[mb] * log_softmax`
#     of an unsqueezed (B,1,bins) broadcasts to (mb, mb, bins) with NO exception, giving a
#     plausible-looking but wrong v_loss (6.7257 correct vs 6.7485 broadcast). The audit
#     enumerated all four consumption sites and confirmed each is squeezed.
# Its MINOR findings are fixed here: the (B,num_bins) projection is now skipped entirely
# when the gate is off (it was allocating ~134MB for a tensor never read), the new target
# now has its own support-adequacy telemetry (debug/probe_edge_mass), and the redundant
# debug/probe_backup_std was REMOVED -- it was exactly debug/tpo_score_std_mean times
# sqrt(K/(K-1)) = 1.069045, measured constant to 6dp, so it carried no new information.
#
# FALSIFIABLE PREDICTIONS:
#   1. return beats v1 at matched steps. If it does not, the critic's noise was NOT the
#      teacher's binding constraint and this whole line of attack is wrong.
#   2. debug/tpo_score_std_mean (spread of oracle Q within a group; NOT a new scalar --
#      v1 already logs it) SHRINKS relative to v1 as the policy improves: the teacher
#      should run out of local disagreement once V stops injecting noise into its scores.
#   3. debug/tpo_eta_solved should sit LOWER than v1's at matched steps: cleaner scores mean
#      the KL cap binds at a smaller temperature. If eta is unchanged, V was not the noise
#      source and prediction 1 will also fail.
#   4. debug/probe_edge_mass stays ~0. The audit measured edge saturation beginning only at
#      raw |V| ~ 20000, about 19x above this task's plausible operating point (~1040), so a
#      nonzero reading means the backup has diverged, not that the support is too small.
#
# Inherited v1 notes below, unchanged.
# =====================================================================================
...
class Args:
    ...
    alpha_lr: float = 1e-3
    vf_coef: float = 1.0
    # --- v2: expected-backup critic auxiliary (the probe's discarded byproduct) ---
    probe_backup: bool = True     # regress V(s) on the K-sample expected-SARSA backup too
    probe_backup_coef: float = 1.0  # UNTUNED. 1.0 == equal weight with the lambda-return CE
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # --- v2: the probe's K candidates are iid draws from the ROLLOUT policy, so
            # mean_k[r_k + gamma(1-term_k)V_old(s'_k)] is a K-sample EXPECTED-SARSA backup
            # of V_{pi_old}(s): an unbiased Bellman target with ~K times less sampling
            # variance than the single-sample TD(0) term inside the lambda-return. It is
            # already computed for the teacher and then thrown away. Same fixed point as
            # the lambda-return (both are valid backups of the same V), so adding it is
            # a variance reduction, not a new objective.
            probe_backup_target = scores.mean(dim=-1)                          # (B,) raw return units
            probe_backup_gap = (probe_backup_target - returns.reshape(-1)).abs().mean().item()
            # Only the (B, num_bins) projection is expensive (67MB + a 67MB cdf temp at
            # batch 32768 x 511 bins), so it is the part that is gated; the scalar gap
            # diagnostic above is free and stays live in both regimes for comparison.
            if args.probe_backup:
                b_probe_target_probs = hl_support.project(probe_backup_target)  # (B, num_bins)
                # Support adequacy for the NEW target specifically: the existing
                # debug/target_edge_mass only watches the lambda-return target. A backup
                # target pinned to the support edges would be a silent regression.
                probe_edge_mass = (
                    b_probe_target_probs[:, 0] + b_probe_target_probs[:, -1]
                ).mean().item()
            else:
                b_probe_target_probs = None
                probe_edge_mass = 0.0
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.probe_backup:
                    v_loss_probe = -(b_probe_target_probs[mb_inds] * value_log_probs).sum(dim=-1).mean()
                    v_loss_opt = v_loss + args.probe_backup_coef * v_loss_probe
                else:
                    v_loss_probe = torch.zeros((), device=device)
                    v_loss_opt = v_loss
                ...
                if args.separate_grad_clip:
                    ...
                    if actor_active:
                        ...
                        (args.vf_coef * v_loss_opt).backward(retain_graph=True)
                        ...
                        (args.vf_coef * v_loss_opt).backward()
                        critic_gn = nn.utils.clip_grad_norm_(critic_params, args.critic_grad_clip)
                        actor_gn = 0.0
                    optimizer.step()
                else:
                    loss = v_loss_opt * args.vf_coef
                    if actor_active:
        ...
        writer.add_scalar("losses/value_loss_probe", v_loss_probe.item(), global_step)
        writer.add_scalar("debug/probe_backup_gap", probe_backup_gap, global_step)
        writer.add_scalar("debug/probe_edge_mass", probe_edge_mass, global_step)
