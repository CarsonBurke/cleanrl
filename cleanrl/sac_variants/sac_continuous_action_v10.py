# SAC v10: v4 HL-Gauss critic + late alpha cap, no auxiliary loss.
#
# Keeps v4's support, optimistic initialization, pure CE critic objective, and
# scalar clipped double-Q target. The only algorithmic change is a late cap on
# autotuned alpha to reduce high-return crash episodes once the policy has
# learned a strong gait.
#
# Diagnostics log alpha cap state plus support/projection health. CUDA required.
import math
import os
import random
import time
...
class Args:
    ...
    alpha_cap: float = 0.20
    """maximum autotuned alpha after alpha_cap_start"""
    alpha_cap_start: int = 200000
    """global step after which to cap autotuned alpha"""
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.policy_frequency == 0:
                for _ in range(args.policy_frequency):
                    ...
                    if args.autotune:
                        ...
                        alpha_loss.backward()
                        a_optimizer.step()
                        if global_step >= args.alpha_cap_start:
                            log_alpha.data.clamp_(max=math.log(args.alpha_cap))
                        alpha = log_alpha.exp().item()
            ...
            if global_step % 100 == 0:
                ...
                writer.add_scalar("losses/alpha_cap_active", float(global_step >= args.alpha_cap_start), global_step)
