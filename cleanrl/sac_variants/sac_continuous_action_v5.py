# SAC v5: v4 + neutral HL prior + distributional HL-Gauss Bellman target.
#
# Critic predicts num_bins logits over a fixed support. Unlike v4, the target
# preserves the next critic distribution: choose the lower-mean target critic,
# shift every support atom by r + gamma * (z - alpha log pi), project each atom
# through HL-Gauss, then mix by next probabilities. This keeps the SAC actor on
# scalar expected Q while letting critic training propagate distribution shape.
#
# The HL head bias is initialized to the projection of Q=0 to remove v4's
# optimistic uniform-logit prior at the support midpoint. Diagnostics log target
# clipping, edge mass, entropy, and decoded scalar residuals.
import math
import os
import random
import time
...
def init_hl_head_for_zero_q(qf, hl_support):
    """Make the initial decoded critic prior approximately Q=0."""
    with torch.no_grad():
        target = hl_support.project(torch.zeros(1, device=hl_support.support.device))[0]
        qf.head.bias.copy_(target.clamp_min(hl_support.eps).log())
        nn.init.uniform_(qf.head.weight, -1e-3, 1e-3)


def distributional_hl_bellman_target(rewards, dones, next_log_pi, next_logits, hl_support, gamma, alpha):
    """Project the full SAC Bellman-shifted next distribution onto HL support."""
    ...
    shifted_atoms = (
        rewards.view(-1, 1)
        + (1.0 - dones.view(-1, 1)) * gamma * (support_atoms.view(1, -1) - alpha * next_log_pi.view(-1, 1))
    )
    projected_atoms = hl_support.project(shifted_atoms.reshape(-1)).view(
        shifted_atoms.shape[0], hl_support.num_bins, hl_support.num_bins
    )
    target_probs = (next_probs.unsqueeze(-1) * projected_atoms).sum(dim=1)
    return target_probs / target_probs.sum(dim=-1, keepdim=True).clamp_min(hl_support.eps), shifted_atoms


def transformed_clip_fraction(values, hl_support):
    if hl_support.use_symlog:
        values = symlog(values)
    clipped = (values < hl_support.v_min) | (values > hl_support.v_max)
...
if __name__ == "__main__":
    ...
    init_hl_head_for_zero_q(qf1, hl_support)
    init_hl_head_for_zero_q(qf2, hl_support)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                use_qf1_next = qf1_next <= qf2_next
                selected_qf1_frac = use_qf1_next.float().mean()
                next_logits = torch.where(use_qf1_next.unsqueeze(-1), qf1_next_logits, qf2_next_logits)
                target_probs, shifted_atoms = distributional_hl_bellman_target(
            ...
            # Cross-entropy vs projected distributional SAC Bellman target.
            ...
            if global_step % 100 == 0:
                with torch.no_grad():
                    qf1_probs = torch.softmax(qf1_logits, dim=-1)
                    qf2_probs = torch.softmax(qf2_logits, dim=-1)
                    ...
                    target_scalar = target_scalar_values.mean()
                    scalar_td_error = 0.5 * (
                    ...
                    target_clip_frac = transformed_clip_fraction(shifted_atoms, hl_support)
                    target_p95 = shifted_atoms.flatten().quantile(0.95)
                writer.add_scalar("losses/qf1_values", q1_scalar, global_step)
                writer.add_scalar("losses/qf2_values", q2_scalar, global_step)
                writer.add_scalar("losses/target_values", target_scalar.item(), global_step)
                writer.add_scalar("losses/qf1_loss", qf1_loss.item(), global_step)
                writer.add_scalar("losses/qf2_loss", qf2_loss.item(), global_step)
                writer.add_scalar("losses/qf_loss", qf_loss.item() / 2.0, global_step)
                writer.add_scalar("losses/actor_loss", actor_loss.item(), global_step)
                writer.add_scalar("losses/alpha", alpha, global_step)
                writer.add_scalar("losses/scalar_td_abs", scalar_td_error.item(), global_step)
                ...
                writer.add_scalar("losses/target_min", shifted_atoms.min().item(), global_step)
                writer.add_scalar("losses/target_max", shifted_atoms.max().item(), global_step)
                writer.add_scalar("losses/target_p95", target_p95.item(), global_step)
                writer.add_scalar("losses/selected_qf1_frac", selected_qf1_frac.item(), global_step)
