# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v5.
#
# WHAT CHANGES vs DSRG v4: the READOUT. v4 used PMA (attention pool) -- an output
...
class Args:
    ...
    # v4 bakes the v3b strong-lambda values that made routing FUNCTIONAL: the route
    # loss is a mean over B*N*K ~1e7 gates, so its raw gradient is ~1e-4 -- only a
    # large lambda saturates the (tight) decoupled route grad-clip, which then
    # regulates the actual step. With these, active->0.15 and gate-entropy->0.
    lambda_route: float = 100.0      # weight on (mean_active - target)^2 budget (binds the open mean)
    lambda_gate_ent: float = 10.0    # gate-entropy PENALTY (pushes each gate to {0,1} -> commit)
    ...
    n_read: int = 64                 # K_read: source perceptrons per output node (sparse, << N)
...
class RoutedGraphCell(nn.Module):
    ...
    def gate_probs(self, h, tick_t, Ksrc):
        # Eager recompute of the per-edge gate probabilities p (B, N, K) for circuit
...
class PathwayReadout(nn.Module):
    """Fan-in-K sparse gated readout (v5). n_out output nodes (0->actor, 1->critic),
    each reading a FIXED bounded set of K_read source perceptrons via learnable gated
    pathways, then a per-output projection (K_read*d -> H). Mirrors the graph's own
    fan-in-K sparse-pathway structure (biological output neurons read a bounded gated
    set of upstream projections, not a dense readout of the whole population). Separate
    source sets + projection per output => decoupled actor/critic readout off one
    shared graph trunk. Low-variance (only K_read*d inputs per head) and it forces the
    network to ROUTE task-relevant info into those specific source perceptrons.

        hs = h[:, src]                       # (B, n_out, K_read, d)  gather bounded sources
        g  = sigmoid(gate_logit)             # (n_out, K_read)        learnable pathway gates
        out[o] = (g . hs).reshape(K_read*d) @ proj[o] + bias[o]      # (B, n_out, H)
    """

    def __init__(self, n, d, k_read, H, n_out=2, seed=0):
        super().__init__()
        self.n_out, self.K, self.d = n_out, k_read, d
        # each output reads k_read distinct source perceptrons (separate pathways/out).
        src = build_topology(n, k_read, seed + 9973)[:n_out].clone()   # (n_out, K_read)
        self.register_buffer("src", src)
        self.gate_logit = nn.Parameter(torch.zeros(n_out, k_read))      # sigmoid->0.5 init (all pathways usable)
    ...
    def forward(self, h):                          # h: (B, N, d)
        hs = h[:, self.src]                        # (B, n_out, K_read, d)
        g = torch.sigmoid(self.gate_logit)         # (n_out, K_read) gated pathways
        hs = g.unsqueeze(0).unsqueeze(-1) * hs     # (B, n_out, K_read, d)
        feat = hs.reshape(hs.shape[0], self.n_out, self.K * self.d)    # (B, n_out, K_read*d)
...
class DSRGTrunk(nn.Module):
    ...
    def __init__(self, obs_dim, H, args):
        ...
        self.readout = PathwayReadout(self.n, self.d, args.n_read, H, n_out=2, seed=args.seed)
    ...
    def forward(self, x):
        ...
        pooled = self.readout(h)        # (B, 2, H): [:,0]=actor feat, [:,1]=critic feat
        return pooled
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    # TRIPLE-BACKWARD decoupled clipping (v3). Value, ROUTE-structural,
                    # and policy gradients are each backpropped + clipped to their own
                    # max-norm, then summed. The route grad is now decoupled from the
                    # policy clip (v2 buried it in the actor backward, where the ~12x
                    # actor down-scale starved it and the gates stayed at 0.5). It is
                    # applied to ONLY the routing params; route_loss's incidental grad
                    # on the transformation weights (via the recurrent state) is dropped
                    # by the zero_grad before the policy backward.
