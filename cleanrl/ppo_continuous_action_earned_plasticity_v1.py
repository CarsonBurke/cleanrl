# Earned-Plasticity optimizer PPO v1, evaluated against Adam in one trainer.
# Persistent history is the strongest earned-plasticity control from the source
# optimizer. The PPO objective, Beta MLP architecture, clipping, rollout path,
# normalization and max_grad_norm remain unchanged; only optimizer selection and
# persistent stake vary for the matched HalfCheetah study.
import os
import random
import time
...
from cleanrl.plasticity.earned_plasticity_optimizer_v1 import EarnedPlasticity
...
class Args:
    ...
    """Adam learning rate; ignored by EarnedPlasticity"""
    optimizer: Literal["adam", "earned_plasticity"] = "adam"
    """optimizer arm; EarnedPlasticity always uses persistent control"""
    initial_stake: float = 1e-4
    """EarnedPlasticity persistent stake in parameter units"""
    num_envs: int = 1
...
def validate_args(args):
    ...
    if args.optimizer not in {"adam", "earned_plasticity"}:
        raise ValueError("optimizer must be adam or earned_plasticity")
    if not np.isfinite(args.initial_stake) or args.initial_stake <= 0:
...
def main():
    ...
    try:
        ...
        writer.add_text("optimizer", f"{args.optimizer}; control=persistent; initial_stake={args.initial_stake}")
        ...
        if args.optimizer == "adam":
            ...
            optimizer = EarnedPlasticity(agent.parameters(), initial_stake=args.initial_stake, control="persistent")
        optimizer_step = optimizer.step
        if args.compile and args.optimizer == "earned_plasticity":
            optimizer_step = torch.compile(optimizer.step, options={"triton.cudagraphs": False})
        ...
        for iteration in range(1, args.num_iterations + 1):
            if args.anneal_lr and args.optimizer == "adam":
            ...
            writer.add_scalar(
                ...
                optimizer.param_groups[0].get("lr", 0.0) if args.optimizer == "adam" else 0.0,
                global_step,
            )
