# Predictive distribution v17: one joint state-action embedding of future return.
# Predict signed component means and finite joint characteristic moments using
# one shared network and one frozen real-segment TD target per fresh rollout.
# Actor credit integrates the pre-fit critic and corrects it with observed vector
# TD residuals. No GAE, replay, contrastive learning, decoding, or target sweeps.
import json
import math
import random
import time
...
@dataclass
class Args:
    exp_name: str = 'vector_predictive_distribution_v17'
    seed: int = 1
...
def main():
    ...
    with ExitStack() as resources:
        ...
        writer.add_text('method', 'Unified joint characteristic embedding of remaining reward components; pre-fit model integration plus observed vector TD correction; fresh full batches; no GAE/replay/decoding/contrastive loss/target sweeps.')
        ...
        def critic_loss(obs, age, actions, target, weights):
            residual = critic.predict(obs, age, actions) - target
            return .5 * (residual.square().sum(-1) * weights).sum() / weights.sum()
        ...
        for iteration in range(1, iterations + 1):
            ...
            with timer.span('update'):
                ...
                for _ in range(args.critic_epochs):
                    ...
                    loss = critic_loss(observations, age, native_actions, target, weights)
                    loss.backward()
