# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v9.
#
# WHAT CHANGES vs DSRG v8: the additive residual moves from POST-norm to PRE-norm (and
# NOTHING else). v8 used `h_new = norm(h + hhat)` -- it re-normalized the residual stream
# every tick, so the gradient w.r.t. the prior tick's h had to pass through the RMSNorm
# Jacobian each step; whenever rms(h+hhat)>1 the +h identity path is attenuated, and the
# per-tick rescales COMPOUND across T, so the highway iterthink relies on was not actually
# preserved (which fit the observation that v7's LSTM carry overtook v8 later in training).
# v9 keeps the residual stream UN-normalized -- `h_new = h + hhat` (Jacobian = I, a true
# identity highway across all T ticks) -- and normalizes only the cell's INPUTS: each tick
# computes `h_in = norm(h)` and reads h_in for routing query, messages, and the low-rank
# path (the transformer pre-norm pattern). A single final RMSNorm in DSRGTrunk rescales the
# accumulated stream once before the readout. This isolates norm PLACEMENT as the variable
# vs v8; everything else (competitive routing, obs-conditional readout, load-balance,
# triple-backward) is unchanged from v7/v8.
#
# --- v7 lineage note ---
...
class RoutedGraphCell(nn.Module):
    ...
    def gate_probs(self, h, tick_t, Ksrc):
        ...
        q = self.q_proj(self.norm(h) + tick_t)            # v9: pre-norm view, mirrors forward()
    ...
    def forward(self, h, tick_t, Ksrc):
        ...
        N, K = self.n, self.K
        src = self.src
        h_in = self.norm(h)          # v9: PRE-norm. The cell reads a unit-scale VIEW of h
                                     # (stable routing dot-products / message scale); the
                                     # residual STREAM h itself stays un-normalized so
                                     # h_new = h + hhat is a true identity highway (the
                                     # gradient to earlier ticks is undamped, unlike v8's
                                     # norm-the-sum which divided the path by rms>1 each tick).
        ...
        q = self.q_proj(h_in + tick_t)                                    # (B, N, dk)
        ...
        w = torch.bmm(h_in.transpose(0, 1), self.V.transpose(1, 2)).transpose(0, 1)   # (B, N, r)
        ws = w[:, src]                                                    # (B, N, K, r)
        ...
        # --- PRE-NORM additive residual (v9) ---
        # v8 used a POST-norm additive residual `norm(h + hhat)`. That re-normalizes the
        # residual stream every tick, so the gradient w.r.t. the previous tick's h passes
        # through the RMSNorm Jacobian each step: when rms(h+hhat)>1 the identity path is
        # attenuated, and the per-tick rescales compound across T -- the highway iterthink
        # relies on is not actually preserved. v9 moves the norm to the INPUT instead:
        # h_in = norm(h) is the unit-scale view consumed by routing/messages above, while
        # the residual stream itself stays un-normalized -- `h_new = h + hhat`. The +h term
        # is now a clean identity (Jacobian = I), so gradient flows straight back across all
        # T ticks to the obs-injected perceptrons; normalization only ever conditions the
        # cell's inputs, never the carried state. A single final norm before the readout
        # (in DSRGTrunk) rescales the accumulated stream once.
        h_new = h + hhat
...
class DSRGTrunk(nn.Module):
    ...
    def __init__(self, obs_dim, H, args):
        ...
        # v9: the residual stream is un-normalized inside the cells (pre-norm), so the
        # accumulated h after T ticks can drift in scale. A single final RMSNorm rescales
        # it once before the readout -- the iterthink pattern (norm once at the end of the
        # residual stack, not per layer).
        self.out_norm = nn.RMSNorm(self.d, elementwise_affine=False)
    ...
    def forward(self, x):
        ...
        h = self.out_norm(h)            # v9: single final norm of the un-normalized residual stream
