...
- Supported architectures: ``Sequential(<trunk>, Linear)`` for every trunk in
  ...
  ``LReluSphereTrunk``, ``LReluResTrunk``), and plain ``nn.Sequential`` stacks
  of ``Linear``/``Tanh``/``ReLU``/``LeakyReluSq``/``SiTUGLUBranch`` (what
  ``HostMLP`` accepts). Anything else raises ``TypeError``/``ValueError`` and
  the caller keeps using the NumPy mirror.
...
from cleanrl.shared.host_actor import (
    ...
    HostSiTUResActor, HostSiTUSphereActor, LeakyReluSq, LReluResTrunk,
    LReluSphereTrunk, LReluSqPair, SiTUDenseTrunk, SiTUGLUBranch,
    SiTUResTrunk, SiTUSphereTrunk,
)

# Must match the enum in host_kernel.c.
_OP_LINEAR = 0
_OP_TANH = 1
_OP_RELU = 2
_OP_LRELUSQ = 3
...
_BUILD_FLAGS = ("-O3", "-march=native", "-std=c11", "-fPIC", "-shared")
...
class HostGraphActor:
    ...
    def __init__(self, sequential, num_rows):
        ...
        self._op_stream = np.array(self._ops, dtype=np.int32)
        self._buf_table = (ctypes.c_void_p * len(self._arrays))()
        for slot, array in enumerate(self._arrays):
            self._buf_table[slot] = array.ctypes.data
        self._graph = _HostGraph(
            ops=self._op_stream.ctypes.data,
            bufs=ctypes.addressof(self._buf_table),
            n_ops=len(self._ops) // _OP_STRIDE,
            x_slot=self._x_slot,
            out_slot=self._out_slot,
        )
        self._graph_address = ctypes.addressof(self._graph)
    ...
    def _lrelu_pair_branch(self, src, block):
        """Emit ``lin2(leaky_relu(lin1(x), 0.5)^2)``, returning the output slot.

        Both linears are biased in every shipped ``LReluSqPair``; ``_linear``
        emits whatever bias is present, so a bias is never dropped.
        """
        if not isinstance(block.act, LeakyReluSq):
            raise TypeError(
                f"LReluSqPair activation must be LeakyReluSq, not {type(block.act).__name__}")
        ...
        self._op(_OP_LRELUSQ, hidden, hidden, 0, 0, self.num_rows * block.dim)
        out = self._scratch(block.dim)
        self._linear(out, hidden, block.lin2)
        return out

    def _branch(self, src, block):
        """Emit one trunk block of either family, returning its output slot."""
        if isinstance(block, SiTUGLUBranch):
    ...
    def _build_trunk(self, sequential, kind):
        ...
        skip_index = [(k, j) for k in range(1, n_blocks + 1) for j in range(k - 1)]
        ...
        if len(gate_params) != n_blocks + len(skip_index):
        ...
        for k in range(1, n_blocks + 1):
            previous = streams[-1]
            branch = self._branch(previous, blocks[k - 1])
