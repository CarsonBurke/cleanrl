# PPO + IterThink v195 affine-RMSNorm persistent Beta exploration.
#
# Preserve the proven v145 world model exactly: raw observations, learned affine
# scalar tokens, RMSNorm, attached prediction targets/SIGReg, and a detached agent.
# Add an actor-only persistent Gaussian code. A learned state-dependent bilinear
# basis turns that code into an antisymmetric shift of the Beta concentration
# logits, creating coherent exploration without changing the WM. Codes are held
# across real/dream trajectories and replayed in PPO, so action likelihood ratios
# are evaluated under the same conditional policy that generated each action.
#
# -----------------------------------------------------------------------------
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if not self.detach_world_model_from_agent:
            raise ValueError("v195 requires a strictly detached world-model/agent interface")
...
if __name__ == "__main__":
    ...
    assert args.detach_world_model_from_agent, "v195 requires PPO to be detached from the WM"
