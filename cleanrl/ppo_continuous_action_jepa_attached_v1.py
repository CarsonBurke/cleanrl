# Attached JEPA PPO v1: predict the next first-layer latent from state and action.
# Actor/critic each retain baseline 64x64 tanh trunks and PPO gradients throughout.
# Enabled branches share their encoder across time; both sides receive MSE gradients.
# No target network, stop-gradient, EMA, or anti-collapse loss. Hypothesis: PPO
# preserves useful latent variation while one-step prediction improves features.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, jepa_mode="both"):
        super().__init__()
        ...
        self.actor_predictor = self.make_predictor() if jepa_mode in {"actor", "both"} else None
        self.critic_predictor = self.make_predictor() if jepa_mode in {"critic", "both"} else None
    ...
    @staticmethod
    def branch_prediction(trunk, predictor, current, next_observations, native_actions):
        # Reuse PPO's current latent; encode the next state with the same attached layer.
        target = trunk[1](trunk[0](next_observations))
        prediction = predictor(torch.cat((current, 2.0 * native_actions - 1.0), dim=-1))
    ...
    def jepa_loss(self, actor_latent, critic_latent, next_observations, native_actions):
        ...
        if self.actor_predictor is not None:
            actor_loss, actor_std = self.branch_prediction(
                self.actor, self.actor_predictor, actor_latent, next_observations, native_actions,
            )
        if self.critic_predictor is not None:
            critic_loss, critic_std = self.branch_prediction(
                self.critic, self.critic_predictor, critic_latent, next_observations, native_actions,
            )
    ...
    def get_policy_value_latents(self, x):
        ...
        logits = self.actor[4](self.actor[3](self.actor[2](actor_latent)))
        value = self.critic[4](self.critic[3](self.critic[2](critic_latent)))
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args, next_observations=None):
    """Baseline clipped PPO plus attached prediction on the selected trunks."""
    ...
    if args.jepa_mode != "none":
        auxiliary, jepa_metrics = agent.jepa_loss(actor_latent, critic_latent, next_observations, native_actions)
        loss = loss + args.jepa_coef * auxiliary
...
def main():
    ...
    try:
        ...
        agent = Agent(envs, args.jepa_mode).to(device)
