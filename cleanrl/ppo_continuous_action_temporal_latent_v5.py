# Temporal-latent critic v5: fresh initialization, no checkpoint loading.
# Predict full future-transition latent distributions at an explicit integer horizon.
# Balance vector supervision across time bands; integrate actor utility with the
# original geometric probabilities. Reward never trains the predictive critic.
...
# no scalar GAE enters that estimator.
...
HORIZON_LOWER = (0, 1, 2, 4, 8, 16, 32, 64, 128, 256)


def horizon_features(horizons):
    time = horizons.float()[..., None]
    scales = torch.tensor((1., 2., 4., 8., 16., 32., 64., 128., 256.), device=time.device)
    return torch.cat((torch.log1p(time) / math.log(257), torch.exp(-time / scales), (time == 0).float()), -1)


def sample_horizon_bands(shape, gamma, device):
    """Equal-band training draws; actor weights recover the original discount law.

    Each finite band is [lower, next_lower); the final band has an unbounded
    geometric tail. A counterfactual pair must share its horizon and weight.
    """
    lower = torch.tensor(HORIZON_LOWER, device=device, dtype=torch.long)
    band = torch.randint(len(HORIZON_LOWER), shape, device=device)
    widths = torch.cat((lower[1:] - lower[:-1], torch.ones_like(lower[:1])))[band]
    tail = band == len(HORIZON_LOWER) - 1
    probability = torch.where(tail, torch.ones_like(widths, dtype=torch.float64),
                              -torch.expm1(widths.double() * math.log(gamma)))
    uniform = torch.rand(shape, device=device, dtype=torch.float64)
    residual = (torch.log1p(-uniform * probability) / math.log(gamma)).floor().long()
    # Protect finite-band endpoints against inverse-CDF roundoff only.
    residual = torch.where(tail, residual, torch.minimum(residual, widths - 1))
    horizons = lower[band] + residual
    weights = len(HORIZON_LOWER) * gamma ** lower[band].double() * probability
    return horizons, weights.float()


@dataclass
class Args:
    exp_name: str = "temporal_latent_v5_fresh"
    seed: int = 1
...
class SuccessorCritic(nn.Module):
    """Sample a joint transition latent at the requested integer future horizon."""
    def __init__(self, observation_dim, action_dim, noise_dim=32, width=128, latent_dim=None):
        ...
        self.network = nn.Sequential(
            layer_init(nn.Linear(observation_dim + action_dim + noise_dim + 11, width)), nn.SiLU(),
            layer_init(nn.Linear(width, width)), nn.SiLU(),
            layer_init(nn.Linear(width, width)), nn.SiLU(),
            layer_init(nn.Linear(width, self.latent_dim), std=0.1),
        )

    def forward(self, observations, actions, noise, horizons):
        return self.network(torch.cat((observations, 2 * actions - 1, noise, horizon_features(horizons)), -1))
...
def counterfactual_advantages(observations, actions, donors, noise, horizons, latent_model, reward_model, gamma):
    ...
    latent = latent_model(states, current.flatten(0, 1), flat_noise, horizons.flatten())
    reference = latent_model(states, donors.flatten(0, 1), flat_noise, horizons.flatten())
...
def main():
    ...
    try:
        ...
        def successor_loss(observations, actions, first_noise, second_noise, target, horizons):
            first = successor(observations, actions, first_noise, horizons)
            second = successor(observations, actions, second_noise, horizons)
        ...
        def counterfactual_model(observations, actions, donors, noise, horizons):
            return counterfactual_advantages(observations, actions, donors, noise, horizons, successor, reward_decoder, args.gamma)
        ...
        sampler_rng = np.random.default_rng(np.random.SeedSequence([args.seed, 5]))
        ...
        for iteration in range(1, iterations + 1):
            ...
            if iteration == args.calibration_rollouts + args.fit_rollouts + 1:
                ...
                frozen_models = [(model, copy.deepcopy(model.state_dict())) for model in (successor, target_successor, reward_decoder, representation)]
            if stage == 'fit':
            ...
            def bellman_target():
                with torch.no_grad():
                    sampled_horizon, _ = sample_horizon_bands((batch_size,), args.gamma, device)
                    ...
                    remaining = (sampled_horizon - spans).clamp_min(0)
                    bootstrapped = target_model(next_states, next_actions, next_noise, remaining).clone()
                    return select_successor_target(observed, selected, latents, flat_terms, bootstrapped), sampled_horizon
                ...
                with timer.span('update'):
                    ...
                    for epoch in range(args.update_epochs):
                        ...
                        if stage == 'calibrate':
                            ...
                            target, horizons = bellman_target()
                            first_noise, second_noise = torch.randn(2, batch_size, args.noise_dim, device=device).unbind()
                            loss = successor_loss(observations, actions, first_noise, second_noise, target, horizons)
                ...
                with timer.span('gradient_diagnostics'):
                    ...
                    for _ in range(args.eval_samples):
                        with torch.no_grad():
                            ...
                            counterfactual_horizons, importance = sample_horizon_bands((batch_size, agent.action_dim), args.gamma, device)
                            vector_action_advantages, outcome_difference = (x.clone() for x in counterfactual_model(
                                observations, actions, donors, counterfactual_noise, counterfactual_horizons))
                            vector_action_advantages.mul_(importance)
                    ...
                    with torch.no_grad():
                        ...
                        predicted1 = latent_model(observations, actions, noise1, horizons).clone()
                        predicted2 = latent_model(observations, actions, noise2, horizons).clone()
                        ...
                        diagnostics.update({
                            ...
                            'latent/sampled_bootstrap_fraction': (horizons >= spans).float().mean(),
                        })
                        ...
                        # >=512 real transitions before any reset; retain sampled
                        # balanced-band horizons below 512 and report the excluded
                        # fraction. This is not a geometric full-law energy test.
                        ...
                        same_raw_count = same_states.sum()
                        same_count = same_raw_count.clamp_min(1)
                        ...
                        # These forecasts and targets must use the SAME horizon.
                        usable = (full_spans >= 512) & (horizons < 512)
                        _, observed_indices = successor_indices(full_spans, horizons, args.num_envs)
                        ...
                        diagnostics.update({
                            ...
                            'latent/observed_excluded_horizon_fraction': (horizons >= 512).float().mean(),
                        })
                        horizon_gradients = {}
                        for requested in (0, 4, 16, 64):
                            fixed_horizons = torch.full((batch_size, agent.action_dim), requested, device=device, dtype=torch.long)
                            donors = Beta(alpha, beta, validate_args=False).sample((agent.action_dim,)).movedim(0, 1).contiguous()
                            fixed_noise = torch.randn(batch_size, agent.action_dim, args.noise_dim, device=device)
                            fixed_advantage, _ = counterfactual_model(observations, actions, donors, fixed_noise, fixed_horizons)
                            # Counterfactual model returns full-return units; an
                            # individual-horizon check compares r_(t+h), without
                            # geometric weights or the 1/(1-gamma) conversion.
                            horizon_model = (coordinate_scores * ((1 - args.gamma) * fixed_advantage)[:, None, :]).sum(-1)
                            selected = (torch.arange(batch_size, device=device) + requested * args.num_envs).clamp_max(batch_size - 1)
                            horizon_reference = score * batch.rewards.flatten()[selected, None]
                            horizon_gradients[str(requested)] = {
                                'model': ((horizon_model * same_states[:, None]).sum(0) / same_count).cpu().tolist(),
                                'reference': ((horizon_reference * same_states[:, None]).sum(0) / same_count).cpu().tolist(),
                            }
                            fixed_prediction = latent_model(observations, actions, noise1, fixed_horizons[:, 0]).clone()
                            predicted_utility = reward_model(fixed_prediction).clone()
                            # One forecast sample versus one physical outcome;
                            # this includes sampling variance, not just mean error.
                            diagnostics[f'horizon/{requested}_observed_reward_mse'] = (((predicted_utility - batch.rewards.flatten()[selected]).square()) * same_states).sum() / same_count
                        ...
                        row = {'rollout': len(rows) + 1, 'step': global_step,
                               ...
                               'same_state_count': float(same_raw_count.cpu()),
                               'horizon_gradients': horizon_gradients}
                        del tangents, reference, model, coordinate_scores
        ...
        report.update(args=vars(args), fresh_initialization=True, checkpoint_loaded=False, actor_unchanged=True, models_frozen_for_holdout=True, holdout=rows,
                      ...
                      primary_estimator='coordinate-counterfactual advantages from nonlinear decoded future-latent samples; explicit future horizons with geometric importance integration')
