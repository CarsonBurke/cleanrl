...
class Agent(nn.Module):
    ...
    def decomposed_score_logprobs(self, x, raw_action):
        ...
        scale_score_logprob = Normal(mean.detach(), std).log_prob(raw_action).sum(dim=-1)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                ...
                action, raw_action, logprob, gaussian_logprob, value = (
        ...
        writer.add_scalar(
            "tail/normalized_delight_mean", normalized_delight.mean().item(), global_step
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "tail/gate_high_saturation", (gate > 0.99).float().mean().item(), global_step
        )
        writer.add_scalar(
        ...
        writer.add_scalar("grad/actor_mean_head", actor_mean_grad_norm.item(), global_step)
        writer.add_scalar(
