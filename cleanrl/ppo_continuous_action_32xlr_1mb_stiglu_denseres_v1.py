# 32x LR + 1 minibatch with a dense-residual SiTU-GLU trunk (sTiGLU) instead
# of LeakyReluSq. Same host-actor trainer as ppo_continuous_action_32xlr_1mb
# _v1: lr=9.6e-3, num_minibatches=1; PPO loss, clip, critic and Beta head are
# unchanged. The trunk is in_proj -> three SiTU blocks where every previous
# stream feeds every later stage (DenseNet-style adds), each contribution
# gated by a learned PER-CHANNEL vector (init sigmoid(-1.5) ~= 0.18) so every
# perceptron routes its own effective depth -- no layer-wide scalars, no
# normalization. Residuals keep trunk outputs O(1) (measured E[x^2] ~= 2.5,
# the lrelusq regime) so PPO's trust region holds. Hypothesis: per-unit depth
# routing lets some channels stay shallow (stable policy mean) while others
# compose deep multiplicative features, beating one-depth-fits-all trunks.
import os
import random
import time
...
from cleanrl.shared.host_actor import HostSiTUDenseActor, make_situ_dense_trunk
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            make_situ_dense_trunk(observation_dim, 64),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); FP32; dense per-channel-gated SiTU-GLU trunk; native-action storage; host actor mirror")
        ...
        host_actor = HostSiTUDenseActor(agent.actor, args.num_envs)
