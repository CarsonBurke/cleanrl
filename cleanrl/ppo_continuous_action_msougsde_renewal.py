# MSOU-gSDE (Renewal): Multi-Scale OU gSDE with Jump-Renewal Resets
#
# Hypothesis:
# - Balance tasks (Hopper/Walker) can be harmed by long correlated exploration
#   drifts that repeatedly push the agent into failure modes.
# - Add a light-tailed "renewal" component: with small probability per step (per
#   env), reset the OU exploration matrices to fresh draws. This yields a
#   jump-diffusion style exploration: mostly smooth, occasionally re-randomized.
#
# Keeps the same marginal gSDE log_prob approximation as the lineage; changes
# only the exploration state dynamics.
import math
import os
import random
import time
...
class MSOUGSDEDistribution:
    ...
    def reset_envs(self, log_std, env_mask):
        # env_mask: (B,) bool
...
class Agent(nn.Module):
    ...
    def reset_envs(self, env_mask):
        self.gsde.reset_envs(self.log_std, env_mask=env_mask)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # Renewal: random resets per env (on the OU update cadence, to keep interpretation clean).
            if update_noise and args.renewal_prob > 0:
