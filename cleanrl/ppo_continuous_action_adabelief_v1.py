# AdaBelief on base PPO, v1: https://arxiv.org/html/2010.07468v5, Algorithm 2.
# Preserve base PPO's tanh MLPs, Beta policy, 3e-4 LR, normalized advantages,
# 32 minibatches and JOINT 0.5 gradient clipping; replace only Adam.
# Hypothesis: standard PPO's smaller updates avoid the high-LR normres collapse.
# Epsilon is added to the variance each step AND outside sqrt. Expose it because
# the paper's 1e-8 and the authors' HalfCheetah 1e-12 give very different floors.
import os
import random
import time
...
class Args:
    ...
    adabelief_eps: float = 1e-12
    """variance and denominator epsilon; use 1e-8 for the paper-default comparison"""
    num_envs: int = 32
...
class AdaBelief(optim.Optimizer):
    ...
    def __init__(self, params, lr=3e-4, betas=(0.9, 0.999), eps=1e-12, compile=True):
        params = list(params)
