# PPO + IterThink v15 (STATE-DEPENDENT std, SAC-style). From v14 (max-ent).
#
# v14 keeps a single GLOBAL learnable logstd (one scalar exploration budget per
# action dim, identical in every state). SAC instead emits a HETEROSCEDASTIC std
# logsigma(s) from the actor features (clamped to [LOG_STD_MIN, LOG_STD_MAX]) so
# the policy is uncertain where it has not learned and confident where it has.
# In pure PPO a state-dependent std is known to be unstable / prone to premature
# collapse -- which is exactly why we add it ONLY now: v14's auto-tuned max-ent
# alpha enforces an entropy FLOOR on the *mean* entropy, so the per-state head can
# freely shape the noise across states while the dual prevents global collapse.
# This is the SAC pairing (max-ent + heteroscedastic Gaussian) ported to on-policy.
# Hypothesis: per-state noise shaping + the entropy floor improves the peak and/or
# tames v12's late EV collapse vs the global-logstd v14. Control: v14_maxent.
# ===== inherited v14 max-ent notes =====
#
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # STATE-DEPENDENT logstd head (SAC). Small-weight + zero-bias init so the
        # initial std ≈ 1 everywhere, matching v14's global-logstd starting point.
        self.actor_logstd = layer_init(nn.Linear(H, act_dim), std=0.01)
    ...
    def get_action_and_value(self, x, z=None):
        fa = self.actor_trunk(x)
        mean = self.actor_head(fa)
        logstd = self.actor_logstd(fa).clamp(LOG_STD_MIN, LOG_STD_MAX)
        std = logstd.exp()
