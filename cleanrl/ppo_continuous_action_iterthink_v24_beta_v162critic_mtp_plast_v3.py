# PPO + IterThink v24 Beta + v162 critic MTP + BetaPlast v3.
#
# NEW IN v3: DRIFT-BOUNDED absolute gate — decoupled per-neuron (non-competitive)
# but with a bounded global LR drift, fixing the failure that sank betaplast_v2_1.
# KEY INSIGHT: "non-competitive" and "global-LR-stable" are the same coin. v1's gate
# is competitive PRECISELY BECAUSE it divides by the layer mean (sum pinned => no
# global drift). v2's absolute gate removes that => gates decouple (good, per-neuron)
# but the LAYER-MEAN gate now floats freely; when the whole layer stiffens EVERY gate
# hits the floor and the global LR COLLAPSES, compounding with the LR anneal (v2_1's
# death). v3 keeps the ABSOLUTE (decoupled) shape but rescales it so the layer-mean
# gate is pulled into a tight band [gate_mean_min, gate_mean_max]:
#     g_i = proxy_i / proxy(c_init)                          # absolute, non-competitive
#     g  <- g * clamp(mean(g), mmin, mmax) / mean(g)         # bound the GLOBAL drift
#     gate_i = clamp(g_i, gmin, gmax)                        # bound per-neuron extremes
# The RELATIVE shape (which neurons are more plastic) is preserved exactly, but the
# aggregate step scale can only breathe within +/-(band), never collapse. At init
# proxy is uniform => g==1 everywhere => mean 1 => unchanged (matched A/B with v1/v2).
# Selected by --plast-gate-mode {relative,absolute,bounded_absolute}.
# Hypothesis: the LOCAL decoupling (letting a plastic neuron keep a big step without
# taxing its neighbors) is the useful part of "non-competitive"; the GLOBAL float is
# the harmful part. Bounding only the global drift keeps the benefit and removes the
# v2_1 failure, so it should beat both the pinned-mean v1 and the free-float v2.
#
# --- inherited v2 notes ---
# PPO + IterThink v24 Beta + v162 critic MTP + BetaPlast v2.
#
# NEW IN v2: NON-COMPETITIVE (decoupled/absolute) plasticity step gate.
# v1's gate = clamp(proxy_i / mean_j(proxy_j), gmin, gmax) divides by the LAYER
# MEAN, so its pre-clamp mean is exactly 1: a zero-sum REALLOCATION of a fixed LR
# budget (one neuron up forces others down), blind to the overall plasticity level.
# v2 makes the gate ABSOLUTE: gate_i = clamp(proxy_i / proxy(c_init), gmin, gmax),
# referenced to the INIT concentration so gate=1 everywhere at init (a matched A/B
# with v1), but each neuron's step scale now depends ONLY on its own concentration.
# The competition is gone; the global effective LR floats within the clamp [gmin,gmax].
# NOTE: this is the same mechanism as the betaplast_v2_1 ablation (which underperformed
# on the plain-MLP base), retested here on the iterthink base with a TIGHTER clamp
# [0.25,4] (vs v2_1's [0.1,10]) that limits the global-LR swing to +/-4x. Selected by
# --plast-gate-mode {relative,absolute}. See plast_v3 for the drift-bounded variant.
# Hypothesis: letting the gate see the ABSOLUTE plasticity level (not just its shape)
# lets it globally throttle/re-inject learning; the risk (v2_1's failure) is that when
# the whole layer stiffens the global LR collapses and compounds with the LR anneal.
#
# --- inherited v1 notes ---
# PPO + IterThink v24 Beta + v162 critic MTP + BetaPlast v1.
#
# NEW IN THIS VARIANT (plasticity add-on, grafted from betaplast_v2):
# Give the v24_beta_v162critic_mtp base per-neuron learned plasticity via
# signed-Beta multiplicative gains on the ACTOR's hidden feature. Mechanism
# (unchanged in spirit from betaplast; adapted to the ThinkTrunk actor):
#   s_i = 1 + delta * r_i * (1 - u_i^(1/c_i)),  r_i ~ {-1,+1}, u_i ~ U(0,1)
# a signed Beta(1, c_i) perturbation on (1-delta, 1+delta) with mean exactly 1,
# multiplying the i-th neuron of the trunk's ACTOR-side output feature (dim H).
# c_i = 1 + (c_max-1)*sigmoid(rho_i) is a learned per-neuron concentration
# (bounded both ends, gradient alive everywhere). Three coupled roles, no meta:
#   1. Coherent parameter-space exploration: (u, r) are drawn ONCE per iteration
#      per env (antithetic sign pairs across env pairs) and frozen for the rollout
#      AND all update epochs, so PPO ratios stay exactly consistent (old/new share
#      the same gain noise). The perturbed actor IS the policy for that iteration.
#   2. Pathwise plasticity learning: x = 1 - u^(1/c) is differentiable in c with
#      frozen noise, so c_i is trained by the ordinary clipped surrogate; the mean-1
#      pin kills the first-order term so c descends local curvature (sharp neurons
#      stiffen, flat/shifting neurons stay plastic).
#   3. Plasticity gates the step size: each gained neuron's POST-Adam update to the
#      layer that PRODUCES it (out_proj weight row + bias) is scaled by its relative
#      gain variance, clamped. Post-Adam (pre-Adam scaling is undone by Adam); relative
#      so LR is reallocated across neurons, never globally choked.
# ADAPTATION NOTES vs betaplast_v2: betaplast gained two 64-d MLP hidden layers; the
# base actor exposes a SINGLE H-dim hidden feature (the ThinkTrunk output feeding the
# actor heads), so plasticity collapses to ONE gained layer here. The gain is applied
# only on the ACTOR path (after the actor/critic split), so the critic value target is
# untouched. CAVEAT: with share_backbone=True the gated producing layer (trunk.out_proj)
# is SHARED, so its post-Adam step gate mildly reallocates LR for the critic too; the
# gate is relative (mean 1) and clamped to [gate_min, gate_max], so the effect is bounded.
# Reward normalization stays OFF (the v162 fixed symlog support needs raw returns).
# Hypothesis: temporally-coherent per-neuron parameter noise + curvature-tracking
# per-neuron LR add exploration/plasticity on top of the strong v24 base without runaway
# (bounded by delta and the gate clamp; rigidity floored by the concentration cap).
#
# --- inherited base notes ---
# Hypothesis: keep iterthink_v24_beta_s1's PPO/Beta actor and ThinkTrunk, but
# replace the v24 distributional lambda-return critic with the v162 critic:
#   - raw-reward symlog HL-Gauss support over symmetric +/-20k returns
...
# This isolates whether v162's critic geometry improves v24_beta_s1 without
# importing the v162 world model, CUDA graph path, or imagined updates.
#
...
# calibrated. In this variant the target is v162 scalar-return HL-Gauss MTP.
#
...
# v24's distributional lambda-return to v162 scalar-return HL-Gauss MTP; only
# the policy advantage transforms are selected by `adv_transform`. sigma(s) is
# the std of the OLD rollout Z(s_t), floored at `sigma_floor_bins` bins.
import os
import random
import time
...
class Args:
    ...
    plast_gate_min: float = 0.0625   # lower clamp on the per-neuron post-Adam step gate (expansive: 1/16x)
    plast_gate_max: float = 16.0     # upper clamp on the per-neuron post-Adam step gate (expansive: 16x)
    # Gate normalization. "relative" (v1): gate_i = proxy_i / mean_j(proxy_j) -> zero-sum
    # reallocation, mean pinned to 1 (competitive). "absolute" (v2): gate_i =
    # proxy_i / proxy(conc_init) -> decoupled per-neuron, global LR floats within the clamp.
    # "bounded_absolute" (v3): absolute shape, but the layer-MEAN gate is rescaled into
    # [gate_mean_min, gate_mean_max] so the global LR drift is bounded (no collapse).
    plast_gate_mode: str = "bounded_absolute"
    plast_gate_mean_min: float = 0.25  # v3: lower bound on the LAYER-MEAN gate (expansive global LR floor: 1/4x)
    plast_gate_mean_max: float = 4.0   # v3: upper bound on the LAYER-MEAN gate (expansive global LR ceiling: 4x)

    # v162 critic: raw HalfCheetah returns use symmetric +/-20k endpoints in
    # symlog coordinates, with Dreamer4/hl-gauss-pytorch sigma.
    num_bins: int = 511
...
class Agent(nn.Module):
    ...
    def resample_gain_noise(self, num_envs, device):
        ...
        # Frozen for the whole rollout AND all update epochs so PPO ratios stay exact.
...
def plasticity_gates(agent, args):
    ...
    #   "relative": normalize to the layer mean -> zero-sum reallocation (mean pinned
    #     to 1, competitive; blind to the overall plasticity level).
    #   "absolute": normalize to the INIT concentration's variance -> decoupled per
    #     neuron (gate=1 at init), global LR free to float within the clamp.
...
if __name__ == "__main__":
    ...
    # trunk's out_proj. With share_backbone=True this is the shared trunk (see header
    # caveat: the gate then mildly reallocates LR for the critic too, bounded by the
    # relative-mean-1 clamp).
    actor_trunk = agent.trunk if agent.share_backbone else agent.actor_trunk
    gated_layers = [actor_trunk.out_proj]
    ...
    support = hl_support.support                       # bin centers; symlog coordinates when value_symlog
    bin_width = hl_support.bin_width
    raw_support = symexp(support) if args.value_symlog else support

    def value_logits_to_scalar(logits):
        return hl_support.to_expected_scalar(logits)

    raw_bin_width_floor = (
        (raw_support[1:] - raw_support[:-1]).abs().min().item()
        if args.value_symlog
        else bin_width
    )
    sigma_floor = args.sigma_floor_bins * raw_bin_width_floor
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, value_logits = agent.get_action_and_value(
        ...
        with torch.no_grad():
            ...
            # CDF-rank u in the critic's bin coordinate system (symlog when enabled).
        ...
        with torch.no_grad():
            ...
            # gate_mean probes the (de)inflation: relative pins ~1; absolute floats.
