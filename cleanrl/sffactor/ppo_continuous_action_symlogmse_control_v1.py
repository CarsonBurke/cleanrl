# PPO + IterThink v24 Beta + SYMLOG-MSE SCALAR CRITIC CONTROL (SF-line falsifier).
# =====================================================================================
# DISCRIMINATING CONTROL from the sffactor red team. Identical to the scalarmse
# control (same "ppoadvnorm_batch_v1" base: Beta actor, shared ThinkTrunk, dual-
# backward clip 0.25/0.25, batch adv z-score, clip 0.2/0.28, tkl03; scalar head,
# no value clipping, zero-init) EXCEPT the head predicts in SYMLOG space:
#
#     u(s) = Linear(H, 1)  V(s) = symexp(u)   loss = MSE(u, symlog(returns))
#
# WHY: sffactor_v1 (vector-TD SF critic) beat the raw-MSE scalar control by ~17%
# @4M, which we read as "vector-TD targets shape the representation". The red team's
# alternative: raw MSE on ~1e6-scale return targets permanently saturates the 0.25
# critic clip -> constant full-norm trunk churn -> KL leash trips earlier -> control
# got 2/10 policy epochs vs v1's 4/10. Under that reading v1's edge is purely LOSS
# SCALE, not the vector signal. Symlog-MSE fixes the scale with NO vector signal:
#   - symlogmse ~= sffactor_v1 (4372 @2M / 5798 @4M)  => scale story wins; vector-TD
#     shaping is dead; retire the SF line entirely.
#   - symlogmse ~= raw-MSE control (3856 / 4947)      => scale was not the confound;
#     the vector-TD representation effect is real; build the hardened SF-aux variant.
#   - in between: both mechanisms contribute; judge SF-aux expected value accordingly.
# Watch debug/epochs_completed (the mediator) and losses/critic_grad_norm (clip
# saturation should vanish vs the raw-MSE control).
# =====================================================================================
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # CONTROL: scalar critic head predicting in SYMLOG space, zero-init
        # (symlog-space 0 => raw V=0 prior, matching the base's neutral bucket head).
    ...
    def get_value(self, x):
        ...
        return symexp(self.critic_head(critic_feat).squeeze(-1))
    ...
    def get_action_and_value(self, x, z=None):
        ...
        # SYMLOG-space head output; callers symexp where raw values are needed.
        # (Loss consumes this directly — a symlog(symexp(u)) round trip has autograd
        # gradient exactly 0 at u=0 because of sign(0), which with the zero-init head
        # would kill the critic permanently.)
        value_symlog = self.critic_head(critic_feat).squeeze(-1)
        if self.actor_dist == "gaussian":
            zr = dist.rsample()
            entropy = (dist.log_prob(zr) - log_det_fn(zr)).sum(1).neg()
        else:
            entropy = dist.entropy().sum(1)
        return action, z, log_prob, entropy, value_symlog
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, value_symlog = agent.get_action_and_value(next_obs)
                values[step] = symexp(value_symlog)
            actions[step] = action
            latent_zs[step] = z
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # CONTROL value loss, no clipping. Symlog-space MSE directly on the
                # head output (symexp never enters the training graph).
                # Targets ~1e3 raw compress to ~7 in symlog => the 0.25 critic clip
                # is no longer saturated by target scale (the confound under test).
                v_loss = args.vf_coef * F.mse_loss(newvalue, symlog(b_returns[mb_inds]))
