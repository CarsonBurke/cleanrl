# PPO + IterThink v187 flowmatch-shortcut-dit-k6 (stochastic next-latent via shortcut flow matching).
#
...
class Args:
    ...
    # v187: shortcut flow-matching next-latent head (dreamer4 shortcut forcing).
    ...
    lewm_flow_cond_dim: int = 128           # conditioning MLP width (belief tok + signal + step embeds)
    lewm_flow_signal_levels: int = 64       # K_MAX: discrete signal grid, t = k / K_MAX (must be power of 2)
    lewm_flow_signal_embed_dim: int = 16    # learned Embedding(K_MAX, this)
    lewm_flow_step_embed_dim: int = 16      # learned Embedding(log2(K_MAX), this), indexed by log2(d)
    ...
    lewm_flow_meanpath_prob: float = 0.1    # fraction of rows forced to the EXACT policy-input query (z=0, signal=0, d=1): anchors f(0|belief) to E[x|belief] (training otherwise only sees z_t=eps at signal 0)
    lewm_flow_consistency_coef: float = 1.0 # weight on the (1-t)^2 shortcut self-consistency term (grounding is always 1.0)
    lewm_flow_sample_steps: int = 4         # Euler steps for sampled paths (d = K_MAX / steps)
    lewm_flow_dream_temperature: float = 1.0  # dream-loop generation: 0 => one-shot conditional mean, >0 => z0 ~ N(0,I)*tau + 4-step Euler
...
class FlowMatchingHead(nn.Module):
    """v187: 3-block AdaLN-zero DiT flow head (MLP-only, per-token), x-parameterization.

    Predicts the CLEAN next summary token x from a noised input z_t = (1-t)*eps + t*x,
    conditioned on c = MLP(belief token, signal-level embed, step-size embed). The signal
    level k in {0..K_MAX-1} discretizes time t = k/K_MAX (k=0 pure noise, k->K_MAX clean);
    the step size d = 2^step_log2 conditions the shortcut objective (one d-step must match
    two d/2-steps). No attention / token mixing: the belief already integrated the context,
    so the head is a pure conditional per-token denoiser. mod/final_mod/out_proj are
    ZERO-INIT (weights and biases): at init x_pred == bound*tanh(0) == 0 and conditioning
    has no influence -- the AdaLN-zero identity start. Velocity is recovered analytically
    as v = (x_pred - z_t) / (1 - t); callers keep t <= (K_MAX-1)/K_MAX so 1-t >= 1/K_MAX."""

    def __init__(self, dim, args):
        super().__init__()
        self.dim = dim
        self.k_max = args.lewm_flow_signal_levels
        assert self.k_max >= 2 and (self.k_max & (self.k_max - 1)) == 0, "K_MAX must be a power of 2"
        self.num_step_sizes = int(log2(self.k_max))  # log2(d) in {0..num_step_sizes-1}
        ...
        self.step_embed = nn.Embedding(self.num_step_sizes, args.lewm_flow_step_embed_dim)
        cond_in = dim + args.lewm_flow_signal_embed_dim + args.lewm_flow_step_embed_dim
        ...
        self.block_mods = nn.ModuleList(
            [zero_linear(nn.Linear(cond_dim, 3 * dim)) for _ in range(args.lewm_flow_blocks)]
        )
        ...
        self.out_proj = zero_linear(nn.Linear(dim, dim))

    def forward(self, z, signal, step_log2, ctx):
        # z, ctx: (..., D); signal, step_log2: long tensors of shape z.shape[:-1].
        cond_in = torch.cat([ctx, self.signal_embed(signal), self.step_embed(step_log2)], dim=-1)
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # v183's JEPA projector (Linear->BatchNorm->GELU->Linear) is GONE: the flow
        # grounding MSE lives directly in bounded token space and SIGReg acts on the
        # clean tanh-bounded encoder tokens (trading-bot recipe).
    ...
    def _predictor_trunk(self, latent_history, action_history, ctx_noise=None, mix_ctx=True):
        ...
        # v187: 10% noisy context on the trunk INPUT tokens (train AND inference). The
        # compiled WM loss passes ctx_noise explicitly (its region must stay
        # randomness-free); eager callers (rollout / dream / bootstrap) draw fresh noise.
        # The self-conditioning pre-pass disables the mix entirely (mix_ctx=False).
    ...
    def flow_mean_next(self, belief):
        # v187 TEMP-0 CONDITIONAL MEAN: one-shot x-prediction at (z=0, signal=0, d=1).
        # Trained at full uniform weight by the always-on grounding term (signal 0 rows),
        # so this single forward IS the conditional mean of the next-summary distribution.
        # Deterministic, one MLP pass: the policy-input / probe / diagnostic path.
        zeros = torch.zeros(belief.shape[:-1], dtype=torch.long, device=belief.device)
        return self.flow_head(torch.zeros_like(belief), zeros, zeros, belief)

    def flow_sample_next(self, belief, temperature=1.0, num_steps=None):
        # v187 sampled path: z0 ~ N(0, I) * temperature, then a deterministic shortcut
        # Euler ODE with d = K_MAX / num_steps (signals 0, d, 2d, ...). x-parameterization:
        # v = (x_pred - z) / (1 - t). The final step lands exactly on the (bounded) x_pred.
        # The init draw is the ONLY stochasticity (dreamer4: no inter-step noise injection).
        if temperature <= 0.0:
            return self.flow_mean_next(belief)
        if num_steps is None:
            num_steps = self.flow_sample_steps
        k_max = self.flow_head.k_max
        # d must be a trained step size: power of 2, 1 <= d <= k_max/2 (step_embed has
        # log2(k_max) rows), so num_steps in {2, 4, ..., k_max} and a power of 2.
        assert (
            2 <= num_steps <= k_max and (num_steps & (num_steps - 1)) == 0
        ), f"num_steps must be a power of 2 in [2, {k_max}], got {num_steps}"
        d = k_max // num_steps
        step_log2 = int(log2(d))
        lead_shape = belief.shape[:-1]
        step_cond = torch.full(lead_shape, step_log2, dtype=torch.long, device=belief.device)
        z = torch.randn_like(belief) * temperature
        for step in range(num_steps):
            sig_val = min(step * d, k_max - 1)
            signal = torch.full(lead_shape, sig_val, dtype=torch.long, device=belief.device)
            x_pred = self.flow_head(z, signal, step_cond, belief)
            t = sig_val / k_max
    ...
    def imagine_step_from_history(self, latent_history, action_history, temperature=None):
        ...
        # v187: generation is a flow sample. temperature=None -> the configured dream
        # temperature (default 1.0: sampled 4-step Euler); 0.0 -> one-shot conditional
        # mean (deterministic; used by the labeled AR diagnostics).
...
if __name__ == "__main__":
    ...
    def wm_supervised_loss_forward(
        ...
        # v187: ALL flow randomness is sampled OUTSIDE this (compiled) region and passed
        # in as tensors (same pattern as the SIGReg projections): flow_eps (mb,H,S,D)
        # standard normal, flow_signal (mb,H) long in [0, K_MAX) already SNAPPED down to a
        # multiple of d=2^flow_step_log2, flow_step_log2 (mb,H) long ~ U{0..log2(K_MAX)-1},
        # flow_selfcond_mask (mb,H) bool ~ Bernoulli(p_selfcond), flow_ctx_noise (mb,H,S,D)
        # standard normal for the 10% noisy-context mix on the training trunk pass.
        ...
        # v187 SHORTCUT FLOW-MATCHING loss (replaces the v183 JEPA-projector MSE).
        # Targets are the clean bounded encoder summaries, ATTACHED (encoder gradient
        # flows through both the x-target and the noised input, le-wm/JEPA style).
        # Diffusion forcing: signal level independent per (row, timestep), shared across
        # the frame's tokens. GROUNDING: x-space MSE at FULL UNIFORM weight over signal
        # levels, on every row. SELF-CONSISTENCY (d>=2 rows): the one-full-step student
        # velocity matches the stop-grad average of two half-step velocities from the
        # SAME online net (no EMA teacher), weighted (1-t)^2.
        ...
        d_float = torch.exp2(flow_step_log2.to(targets.dtype))                # (mb,H)
        half_long = (d_float * 0.5).to(flow_step_log2.dtype)                  # d//2 (0 when d=1)
        ...
        step_tok = flow_step_log2[:, :, None].expand(-1, -1, num_tok)
        x_pred = flow_head(z_t, sig_tok, step_tok, belief_features)
        grounding = F.mse_loss(x_pred, targets, reduction="none").mean(dim=(-1, -2))  # (mb,H)
        with torch.no_grad():
            half_step_tok = (flow_step_log2 - 1).clamp_min(0)[:, :, None].expand(-1, -1, num_tok)
            x1 = flow_head(z_t, sig_tok, half_step_tok, belief_features)
            v1 = (x1 - z_t) / (1.0 - t1)
            z_mid = z_t + v1 * (d_float[:, :, None, None] * 0.5 / k_max)
            sig2 = flow_signal + half_long
            t2 = (sig2.to(targets.dtype) / k_max)[:, :, None, None]
            sig2_tok = sig2[:, :, None].expand(-1, -1, num_tok)
            x2 = flow_head(z_mid, sig2_tok, half_step_tok, belief_features)
            v2 = (x2 - z_mid) / (1.0 - t2)
            v_target = 0.5 * (v1 + v2)
        v_pred = (x_pred - z_t) / (1.0 - t1)
        consistency = F.mse_loss(v_pred, v_target.detach(), reduction="none").mean(dim=(-1, -2))
        consistency = consistency * (1.0 - t1[:, :, 0, 0]) ** 2               # (1-t)^2 weight
        shortcut_mask = (flow_step_log2 >= 1).to(targets.dtype)
        one_step_latent_loss = grounding + args.lewm_flow_consistency_coef * consistency * shortcut_mask
    ...
    def wm_supervised_metrics_forward(
        ...
        # v187: mirror supervised_loss -- self-conditioning pre-pass, then the shortcut
        ...
        d_float = torch.exp2(flow_step_log2.to(targets.dtype))
        half_long = (d_float * 0.5).to(flow_step_log2.dtype)
        ...
        v_target = 0.5 * (v1 + v2)
        v_pred = (x_pred - z_t) / (1.0 - t1)
        consistency = F.mse_loss(v_pred, v_target, reduction="none").mean(dim=(-1, -2))
        consistency = consistency * (1.0 - t1[:, :, 0, 0]) ** 2
        shortcut_mask = (flow_step_log2 >= 1).to(targets.dtype)
        one_step_latent_loss = grounding + args.lewm_flow_consistency_coef * consistency * shortcut_mask
        wm_latent_loss = (one_step_latent_loss * latent_weight).sum() / denom_latent
        wm_latent_mse = (grounding * latent_weight).sum() / denom_latent
        wm_flow_consistency = (consistency * shortcut_mask * latent_weight).sum() / denom_latent
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for _ in range(args.lewm_update_epochs):
            ...
            for start in range(0, args.batch_size, wm_minibatch_size):
                ...
                # v187: flow randomness, sampled OUTSIDE the compiled loss region (same
                # pattern as the SIGReg projections) so the region stays graph-safe and
                # deterministic. step_log2 ~ U{0..log2(K)-1} per (row, timestep) unifies
                # the trading-bot batch-Bernoulli(5/6) branch (identical marginal: d=1
                # "plain" rows w.p. 1/6, shortcut d in {2..K/2} rows w.p. 5/6); the
                # signal is snapped down to a multiple of d (shortcut grid alignment).
                n_tok = agent.world_model.num_latent_tokens
                tok_d = agent.world_model.dim
                flow_k_max = agent.world_model.flow_head.k_max
                flow_step_log2 = torch.randint(
                    0, agent.world_model.flow_head.num_step_sizes, (mb_size, horizon),
                    device=device, generator=wm_torch_generator,
                )
                flow_d_long = torch.pow(2, flow_step_log2)
                flow_signal = (
                    torch.randint(
                ...
                # self-conditioning all consume f(z=0, signal=0, d=1), but the flow loss
                ...
                # (signal=0, d=1, eps=0): their grounding term becomes a direct
                # regression f(0|belief) -> x, anchoring the temp-0 mean path.
                if args.lewm_flow_meanpath_prob > 0.0:
                    ...
                    flow_step_log2 = torch.where(
                        meanpath_mask, torch.zeros_like(flow_step_log2), flow_step_log2
                    )
                ...
                with torch.no_grad():
                    ...
                    # v187: TEMP-0 CONDITIONAL-MEAN path vs K sampled 4-step Euler paths
                ...
                wm_flow_consistency_losses.append(wm_flow_consistency.item())
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/flow_consistency", float(np.mean(wm_flow_consistency_losses)), global_step)
