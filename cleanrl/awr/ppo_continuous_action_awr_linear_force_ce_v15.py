# AWR linear-force CE v15: the KL-optimal target exp(z/eta), fitted by a weighted generalized-KL
# M-step whose per-sample weights make its initial gradient the linear policy gradient. Base: v14.
# Diagnosis (ppo_chi2_weights_v1 ablation, scripts/awr_update_bench.py bins):
# - PPO driven by v10's chi^2 weights (A shifted by -lambda, floored at -eta) ends at 11.0k (eta 1)
#   and 9.5k (eta 0.6) against PPO's 13.6k: discarding negative-advantage magnitudes is the target's
#   cost, and HalfCheetah's advantages are heavily negative-skewed (skew ~-4.5, kurtosis ~60).
# - Any r* >= 0 fitted by E[r - r* log r] pushes a bad sample with force r - r* <= 1, but pulls a good
#   one with r* - 1 (up to ~15): in-sample, v10 moves the z<-4 bin by -140e-3 (PPO -331e-3) and the
#   z>4 bin by +616e-3 (PPO +325e-3).
# Fix: r* = exp(z/eta) keeps every magnitude as a finite, positive fixed point, and the weight
# w = x/expm1(x), x = z/eta, makes the gradient at theta_old exactly -(z/eta) grad log pi, both tails
# intact; each sample's pull still ends at its own target. Bench at eta 2: both tails pushed like PPO
# (-491e-3 / +530e-3), held-out dJ tied with PPO. Keeps v14's beta2 = 0.9 (the M-step is curved).
# Hypothesis: with negative magnitudes restored, CE closes the gap to PPO.
import os
import random
import time
...
class Args:
    ...
    """eta on z-scored advantages: the ratio target is pi/pi_old = exp(z / eta)"""
...
def awr_loss(agent, observations, native_actions, old_logprobs, old_alpha, old_beta,
    ...
    """Weighted generalized-KL projection onto the fixed ratio targets exp(z/eta), plus unclipped value MSE.
    ...
    ratio = logratio.exp()
    # d/dlogratio = w (ratio - r*): -(z/eta) at theta_old, zero at each sample's own target.
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); weighted generalized-KL M-step E[w (r - r* log r)], r* = exp(z/eta), w = x/expm1(x)")
        ...
        def prepare_targets(advantages):
            """Per-sample (weight, ratio target) = (x/expm1(x), exp(x)), x = z/eta, fixed for this rollout."""
            advantage_std = advantages.std()
            z = (advantages - advantages.mean()) / advantage_std
            x = z / args.advantage_temperature
