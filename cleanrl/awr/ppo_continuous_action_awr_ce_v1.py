# AWR cross-entropy v1 on the residual SiTU-GLU nGPT base (v7, no adv norm, no value clip).
...
def awr_loss(agent, observations, native_actions, old_logprobs, weights, returns, args):
    """Weighted CE to the fixed batch target plus the base's unclipped value MSE.

    ``weights`` are the E-step softmax scaled by the batch size (mean one), so a
    minibatch mean is an unbiased estimate of the batch-wide weighted sum.
    """
    ...
    policy_loss = -(weights * newlogprob).mean()
    ...
    metrics = torch.stack((policy_loss.detach(), value_loss.detach(), entropy.mean().detach(),
                           old_approx_kl, approx_kl, target_gain))
    return loss, metrics
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); batch-wide AWR CE target, eta solved to KL(w||u)=kl_budget")
        ...
        def loss_model(observations, native, old_logprobs, weights, returns):
            return awr_loss(agent, observations, native, old_logprobs, weights, returns, args)
