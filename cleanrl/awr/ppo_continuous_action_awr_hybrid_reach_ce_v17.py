# AWR hybrid-reach CE v17: weighted generalized-KL M-step E[w (r - r* log r)] with force z at theta_old
# on every sample; good samples get a fixed reach, bad samples an exp reach. Base: v16.
# A CE rule is the IS surrogate plus a per-sample reverse-KL anchor k = w r* (a -log r barrier).
# - v16 (sign reach both sides, r* = e^up / e^-down, w = |z|/|r* - 1|) ties PPO on held-out gain but
#   its anchor on bad samples is 1.2|z|, largest on the catastrophic tail (skew ~-5, kurtosis ~100):
#   on its own 20M/35M plateau snapshots each update concentrates the policy less than PPO's
#   (paired dH +7e-3 per update); training entropy stalls near -5.7 while PPO reaches -9.4.
# - v14/v15 (chi2 / exp targets) have anchors that vanish on the bad tail, but over-concentrate
#   (paired dH -8e-3 to -20e-3), and exp reach on good samples fits positive outliers' noise.
# Fix: good samples r* = e^up, w = z/expm1(up) (reach independent of |A|); bad samples
# r* = exp(z/eta), w = z/expm1(z/eta) (anchor -> 0 on the tail). Bench on v16 snapshots: PPO-level
# concentration (dH-ref +2 / -2 +- 3e-3) and held-out gain.
# Hypothesis: with the bad-tail brake removed, v16's early lead carries through the late plateau.
import math
import os
import random
import time
...
class Args:
    ...
    up_reach: float = 0.7
    """log-ratio target for positive-advantage samples: r* = exp(up_reach)"""
    down_temperature: float = 1.0
    """eta for negative-advantage samples: r* = exp(z / eta)"""
    actor_adam_beta2: float = 0.9
    """actor Adam beta2: short enough for the second moment to track one update's 10 epochs"""
    vf_coef: float = 0.5
...
def awr_loss(agent, observations, native_actions, old_logprobs, old_alpha, old_beta,
    ...
    """Weighted generalized-KL projection onto the targets exp(up) (z > 0) / exp(z/eta) (z <= 0), plus unclipped value MSE.
    targets[:, 0] holds the per-sample weight, targets[:, 1] the ratio target."""
    weight, target = targets.unbind(-1)
    ...
    ratio = logratio.exp()
    # d/dlogratio = w (ratio - r*): -z at theta_old, zero at each sample's own target.
    policy_loss = (weight * (ratio - target * logratio)).mean()
    ...
    with torch.no_grad():
        ...
        fit_r2 = 1.0 - (weight * (ratio - target).square()).mean() / (weight * (target - 1.0).square()).mean()
        target_gain = (weight * (target - 1.0) * excess).mean()
...
def validate_args(args):
    ...
    if not (args.up_reach > 0.0 and args.down_temperature > 0.0):
        raise ValueError("up_reach and down_temperature must be positive")
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); weighted generalized-KL M-step E[w (r - r* log r)], r* = exp(up) if z > 0 else exp(z/eta), w = z/(r* - 1)")
        ...
        normalize_matrices()
        # The actor's second moment tracks its update; the critic keeps the default long average.
        optimizer = optim.Adam([{"params": agent.actor.parameters(), "betas": (0.9, args.actor_adam_beta2)},
        ...
        def prepare_targets(advantages):
            """Per-sample (weight, ratio target) = (z/(r* - 1), r*): r* = e^up for z > 0, exp(z/eta) otherwise."""
            advantage_std = advantages.std()
            z = (advantages - advantages.mean()) / advantage_std
            up = z > 0
            x = torch.where(up, args.up_reach, z / args.down_temperature)
            # Bad side: z / expm1(z/eta) = eta * x / expm1(x), in its stable form near x = 0.
            down_weight = args.down_temperature * torch.where(x.abs() < 1e-4, 1.0 - 0.5 * x, x / torch.expm1(x))
            weight = torch.where(up, z / math.expm1(args.up_reach), down_weight)
            target = x.exp()
            diagnostics = {
                ...
                "awr/up_share": up.float().mean(),
                "awr/min_target": target.min(),
                "awr/max_weight": weight.max(),
            }
            return torch.stack((weight, target), -1), diagnostics
        ...
        for iteration in range(1, args.num_iterations + 1):
            if args.anneal_lr:
                for group in optimizer.param_groups:
                    group["lr"] = (1.0 - (iteration - 1.0) / args.num_iterations) * args.learning_rate
            bootstraps.reset()
            host_actor.refresh()
