# REFUTED BEFORE RUNNING (red team, CPU policy simulation). Under normalization, alpha only splits
# eta between shift and rescale, so the geometry is neutral. The v13 loss on v10's floored target
# matches v10. Dropping the floor lost 0.06-0.08 J in every noise regime, at 2-4x the KL.
# v10's per-sample floor is useful winsorization (init gradient cosine 0.995 vs 0.95-0.98),
# not a harmful bias. Kept as a record; not submitted.
# AWR mixture-Bregman CE v13: the chi^2 posterior, projected unbiasedly under advantage noise. Base: v10.
# The M-step regresses its per-sample target over (s, a), so the fitted policy only sees
# E[target | s, a]. Only a target linear in A_hat survives that average unbiased:
# E[exp(A_hat/eta) | s,a] = exp(A/eta) * MGF(noise), which is biased, and infinite for heavy tails (CE v1-v5).
# Among mirror-descent posteriors, chi^2 is the one whose target 1 + (A - lambda)/eta is linear
# before its positivity floor. v10 applies that floor to each noisy sample, and
# E[(1 + A_hat/eta)_+ | s,a] > (1 + A/eta)_+ wherever the noise is negatively skewed (z skewness -3.5 to -9).
# v13 keeps the raw linear target y = 1 + z/eta and moves the floor into the loss.
# A per-sample loss linear in y has a population minimizer that depends only on E[y | s,a], for any noise.
# In that family:
#   L = E_old[(m - m* log m)] / alpha^2,  m = alpha r + 1 - alpha,  m* = alpha y + 1 - alpha,  alpha = 1/2
# i.e. CE between the half-mixtures of (pi_theta, pi_old) and (pi*, pi_old).
# - Population minimizer (E[y | s,a])_+: the exact chi^2 posterior on the true advantage, with no floor on the data.
# - The d/dlog r weight r (r - y) / m equals the policy gradient at theta_old, so tails lead (v8).
#   It saturates as r -> 0 (removal is bounded: m >= 1 - alpha) and is capped at |r - y| / alpha (no v9 blow-up).
# - alpha -> 0 recovers v9's least squares, and alpha = 1 v10's generalized KL (invalid for y < 0).
# Hypothesis: v10's gain came from the ratio-space geometry, and its floor bias is a cost that v13 removes.
import os
import random
import time
...
class Args:
    ...
    """eta on z-scored advantages: the unfloored ratio target is y = 1 + z / eta"""
    target_mix: float = 0.5
    """alpha: CE between alpha-mixtures of (pi_theta, pi_old) and (pi*, pi_old); 0 < alpha < 1"""
    vf_coef: float = 0.5
...
def awr_loss(agent, observations, native_actions, old_logprobs, old_alpha, old_beta,
    ...
    """Mixture-Bregman projection onto the fixed linear chi^2 ratio targets, plus unclipped value MSE."""
    ...
    ratio = logratio.exp()
    # d/dlogratio = ratio (ratio - targets) / mixture: linear in the target, saturating as ratio -> 0.
    mix = args.target_mix
    mixture = mix * ratio + (1.0 - mix)
    target_mixture = mix * targets + (1.0 - mix)
    policy_loss = (mixture - target_mixture * mixture.log()).mean() / mix**2
...
def validate_args(args):
    ...
    if not 0.0 < args.target_mix < 1.0:
        raise ValueError("target_mix must lie in (0, 1): alpha = 1 is unbounded below for negative targets")
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); mixture-Bregman CE onto unfloored chi^2 targets y = 1 + z/eta, population minimizer (E[y|s,a])_+")
        ...
        def prepare_targets(advantages):
            """The unfloored linear chi^2 ratio targets 1 + z / eta, fixed for this rollout."""
            advantage_std = advantages.std()
            z = (advantages - advantages.mean()) / advantage_std
            targets = 1.0 + z / args.advantage_temperature
            diagnostics = {
                "awr/advantage_std": advantage_std,
                "awr/max_abs_z": z.abs().max(),
                "awr/z_kurtosis": z.pow(4).mean(),
                "awr/z_skewness": z.pow(3).mean(),
                # Samples whose own target is negative: v10 floored these; v13 lets the average decide.
                "awr/negative_target_share": (targets < 0).float().mean(),
                "awr/negative_target_mass": F.relu(-targets).mean(),
            }
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_targets, target_diagnostics = prepare_targets(advantages.flatten())
            updates = steps = 0
