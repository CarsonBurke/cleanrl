# AWR tracked-moment CE v14: v10's chi^2 target and generalized-KL M-step, with an actor Adam whose
# second moment tracks the update it is taking. Base: v10.
# Diagnosis (scripts/awr_update_bench.py, real snapshots with the trainer's Adam state replayed):
# - v10 cannot take PPO-sized steps. Its M-step E[r - r* log r] has log-ratio curvature r, so an
#   overshooting sample steepens its own gradient. With beta2 = 0.999, Adam's second moment is
#   ~frozen across an update's 10 deterministic full-batch epochs, so the growing gradient passes
#   through unnormalized: at 1.5-2x lr one update runs away (KL 0.01 -> 0.5-2.8, entropy +3-7,
#   the same with permuted advantages). In training, 1-2% of v10 updates exceed KL 0.1 against
#   0.2% for PPO, and a larger step (eta 0.6) buys more of them.
# - PPO's clip zeroes overshooting samples, so its gradient shrinks instead; it has no cliff.
# - beta2 = 0.9 lets the second moment follow the update's own gradient scale within ~10 steps.
#   On the bench it turns CE's KL-vs-lr curve smooth (0.019, 0.031, 0.055 at 1, 1.5, 2x lr, entropy
#   flat) with no loss of signal share; PPO is unchanged by it. The target and M-step are untouched.
# Hypothesis: without the runaway updates, CE can take PPO-sized steps and stop plateauing below PPO.
import os
import random
import time
...
class Args:
    ...
    """eta on z-scored advantages: the ratio target is pi/pi_old = (1 + (z - lambda) / eta)_+"""
...
def awr_loss(agent, observations, native_actions, old_logprobs, old_alpha, old_beta,
    ...
    """Generalized-KL (weighted MLE) projection onto the fixed chi^2 ratio targets, plus unclipped value MSE."""
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); generalized-KL M-step E[r - r* log r] onto chi^2 targets r* = (1 + (z - lambda)/eta)_+")
        ...
        def prepare_targets(advantages):
            """The chi^2 mirror-descent ratio targets (1 + (z - lambda) / eta)_+, fixed for this rollout."""
            advantage_std = advantages.std()
            z = (advantages - advantages.mean()) / advantage_std
            eta = args.advantage_temperature
            # mean((1 + (z - lambda)/eta)_+) falls monotonically from >= 1 at lambda = 0 (mean z = 0)
            # to 0 at lambda = max z + eta; bisect the normalizer on that bracket.
            low, high = torch.zeros_like(advantage_std), z.max() + eta
            for _ in range(40):
                middle = 0.5 * (low + high)
                above = F.relu(1.0 + (z - middle) / eta).mean() > 1.0
                low, high = torch.where(above, middle, low), torch.where(above, high, middle)
            lam = 0.5 * (low + high)
            targets = F.relu(1.0 + (z - lam) / eta)
            diagnostics = {
