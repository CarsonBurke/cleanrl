# AWR chi-square ratio regression v9: fit the ratio pi/pi_old to the chi^2 mirror-descent target. Base: v6.
# v6 fits log-ratios to z/eta: a second-order (Fisher) model of KL mirror descent whose
# linear term E[z log r] is only the first-order policy improvement. Evidence that this
# model breaks at PPO-sized steps: at matched small KL (~0.017) v6 eta2 and PPO lr/2 tie
# (12.2k vs 11.8k at 48M); at larger steps PPO reaches 14.1k, while v6 eta1 takes still
# larger steps (KL 0.045-0.053 over 8-14M) that raise entropy (-5.1 -> -4.7) as the
# return stalls. Its log-space targets ask a z = -30 action for log r = -30; fit_r2 ~ 0.1 means
# no target is ever reached, so 10 epochs keep pulling bad actions down at full weight.
# Pearson chi^2 is the divergence whose mirror descent and projection are both exact:
#   max_pi E_pi[A] - (eta/2) chi2(pi || pi_old)  =>  pi*/pi_old = (1 + (A - lambda(s))/eta)_+
#   projection: min E_old[(r - r*)^2],  r = pi_theta/pi_old
#   L = 0.5 E_old[(r - 1 - z/eta)^2] = -E[z (r - 1)]/eta + 0.5 E[(r - 1)^2] + const.
# - The linear term is the exact importance-sampled improvement PPO clips; the quadratic is an
#   unbiased sample chi^2 trust region. Neither is a Taylor model in theta (the state
#   distribution is still frozen, as in PPO), so large steps stay faithful.
# - E_old[r | s] = 1 for every theta, so the per-state normalizer lambda(s) (and critic bias)
#   is orthogonal to the fit without v6's closed-form KL add-back.
# - The l-gradient (r - r*) r vanishes as r -> 0: a bad action saturates once removed, which is
#   the (.)_+ of the target, reached by the parametrization. The gradient at theta_old is still
#   the full policy gradient of z/eta, so large advantages keep their weight (v8's lesson).
# Hypothesis: the same regression, made exact, keeps PPO's late gains without PPO's clip.
import os
import random
import time
...
class Args:
    ...
    """eta on z-scored advantages: the ratio target is pi/pi_old = 1 + z / eta"""
    vf_coef: float = 0.5
...
def awr_loss(agent, observations, native_actions, old_logprobs, old_alpha, old_beta,
    ...
    """Least-squares fit of the ratio excess r - 1 to the fixed chi^2 targets, plus unclipped value MSE."""
    ...
    ratio = logratio.exp()
    # E_{pi_old}[r - 1 | s] = 0 for every theta: the excess is centered per state by construction.
    excess = ratio - 1.0
    ...
    with torch.no_grad():
        ...
        target_gain = (targets * excess).mean()
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); least-squares fit of pi/pi_old - 1 to z(A) / eta (chi^2 mirror descent)")
        ...
        def prepare_targets(advantages):
            """The batch-wide ratio-excess target z(A) / eta, fixed for every epoch of this rollout."""
            ...
            diagnostics = {
                ...
                # Share of samples whose chi^2 target (1 + z/eta)_+ is zero: actions to remove.
                "awr/removed_share": (z < -args.advantage_temperature).float().mean(),
            }
            return z / args.advantage_temperature, diagnostics
