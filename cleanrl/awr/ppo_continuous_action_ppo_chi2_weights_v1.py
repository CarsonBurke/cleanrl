# Diagnostic ablation: PPO's clipped fit, driven by the chi^2 CE target's per-sample weights.
# awr_chi2_ce_v10 fits r* = (1 + (z - lambda)/eta)_+ with a generalized-KL M-step and plateaus
# below PPO. Its pull on sample i, r*_i - 1, is (z_i - lambda)/eta floored at -1: CE shifts every
# advantage by -lambda (the normalizer) and discards the magnitude of the worst ~15-25% of samples
# (z < lambda - eta), where HalfCheetah's heavily negative-skewed advantages (skew ~ -4.5,
# kurtosis ~ 60) carry much of their mass. Here PPO sees A' = std(A) * max(z - lambda, -eta):
# the chi^2 target's weights at PPO's slope, everything else identical to
# ppo_sepclip_control_v1. Tracking PPO means the target is fine and CE's M-step is the gap;
# tracking v10 means the target's negative-tail truncation is.
# Run with --no-norm-adv --no-clip-vloss --clip-coef-upper 0.2 to match the 50M reference.
import os
import random
import time
...
class Args:
    ...
    clip_coef: float = 0.2
    """lower policy ratio clipping coefficient, also the value clipping radius"""
    ...
    """eta of the chi^2 target whose floored, lambda-shifted weights replace the advantages"""
...
def validate_args(args):
    ...
    if args.norm_adv:
        raise ValueError("per-minibatch normalization would undo the chi^2 weights' shift; pass --no-norm-adv")
    return args
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", "residual SiTU-GLU; unit-L2 matrices and residual directions; sqrt(width) branch/readout scale; learned output gains; no learned mixing")
        ...
        @torch.no_grad()
        def chi2_weights(advantages):
            """std(A) * (r* - 1) * eta = std(A) * max(z - lambda, -eta), lambda normalizing mean(r*) to 1."""
            ...
            lam = 0.5 * (low + high)
            shifted = z - lam
            diagnostics = torch.stack((lam, (shifted < -eta).float().mean()))
            return advantage_std * shifted.clamp_min(-eta), diagnostics

        if args.compile:
            chi2_weights = torch.compile(chi2_weights, fullgraph=True, dynamic=False)
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_advantages, weight_diagnostics = chi2_weights(advantages.flatten())
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss.backward()
                        grad_norms[updates, 0] = nn.utils.clip_grad_norm_(actor_parameters, args.max_grad_norm, foreach=True)
                        grad_norms[updates, 1] = nn.utils.clip_grad_norm_(critic_parameters, args.max_grad_norm, foreach=True)
                        optimizer.step()
                        normalize_matrices()
            ...
            logged = gather_metrics({
                ...
                "grad/critic_preclip_norm": grad_norms[:updates, 1].mean(),
                "awr/lambda": weight_diagnostics[0], "awr/removed_share": weight_diagnostics[1],
            })
