# AWR quantile posterior v11: CE onto the posterior of "beats a fresh draw from pi_old". Base: v10.
# A CE target is a posterior pi(a|s,O) = pi_old(a|s) p(O|s,a) / p(O|s). The event decides what
# the target does with rare outcomes, and whether p(O|s) is computable from one action per state:
# - exp(A/eta) (AWR, CE v1-v5, MaxRL ports v1-v3) is not a likelihood: unbounded above, so rare
#   positives collapse the ESS, and p(O|s) = E[exp(A/eta)|s] varies by state with the noise spread.
# - (1 + A/eta)_+ (v9/v10) is unbounded above, and eta is an exchange rate between advantage and nats.
# - TPO and MaxRL are bounded (simplex; binary success), but their exact p(O|s) needs groups.
# Here O = "a beats k independent draws a' ~ pi_old(.|s)": p(O|s,a) = F_s(A)^k, where F_s is the
# advantage CDF under pi_old at s. p(O|s) = 1/(k+1) at EVERY state, so the posterior is exact and
# normalized without groups:  r* = pi*/pi_old = (k+1) F_s(A)^k,  KL(pi*||pi_old) = log(k+1) - k/(k+1).
# - Rare negatives go to r* -> 0, draining at most their own share, and they never lead the step.
#   Rare positives are capped at r* = k+1, a probability rather than an outlier's magnitude.
#   The weight is monotone, not redescending (v8), so the tails still carry the most weight.
# - No temperature: the event fixes the step (0.19 nats at k = 1), and k is MaxRL's pass@k order.
# - F_s is the batch rank (per-state scale is not predictable, v7). Ranks are exactly
#   scale-free and immune to kurtosis; a critic offset only rescales pi*(.|s), which normalization removes.
# M-step unchanged from v10: L = E_old[r - r* log r], gradient r - r*, fixed point r = r*.
# Hypothesis: bounded, exactly normalized CE targets fit the bulk instead of chasing GAE tails,
# so the late steps stay productive with no trust region anywhere.
import os
import random
import time
...
class Args:
    ...
    beat_draws: int = 1
    """k: the success event is beating k draws from pi_old, so the ratio target is (k+1) F(A)^k"""
    vf_coef: float = 0.5
...
def awr_loss(agent, observations, native_actions, old_logprobs, old_alpha, old_beta,
    ...
    """Generalized-KL (weighted MLE) projection onto the fixed posterior ratio targets, plus unclipped value MSE."""
...
def validate_args(args):
    ...
    if args.beat_draws < 1:
        raise ValueError("beat_draws must be at least 1")
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); generalized-KL M-step E[r - r* log r] onto quantile-posterior targets r* = (k+1) F(A)^k")
        ...
        rank_grid = (torch.arange(args.batch_size, device=device, dtype=torch.float32) + 0.5) / args.batch_size

        @torch.no_grad()
        def prepare_targets(advantages):
            """Posterior ratio targets (k+1) F(A)^k from batch ranks, fixed for this rollout."""
            order = advantages.argsort()
            quantile = torch.empty_like(rank_grid).scatter_(0, order, rank_grid)
            targets = quantile.pow(args.beat_draws)
            # The continuum mean is exactly 1/(k+1); normalize the discrete grid to mean one.
            ...
            diagnostics = {
                ...
                # How much of the z-scored signal the bounded target keeps (Pearson correlation).
                "awr/target_z_corr": ((targets - 1.0) * z).mean() / (targets - 1.0).square().mean().sqrt(),
