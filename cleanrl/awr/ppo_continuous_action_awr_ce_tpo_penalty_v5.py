# AWR cross-entropy v5: TPO's fixed-temperature penalty as an alternative to the KL budget, no clip. Base: v4.
# TPO (Eq. 4) forms q = argmax_r <r, u> - eta KL(r || p_old) with fixed eta = 1 on
# z-scored scores: a KL *penalty*. V-MPO instead solves eta so that KL(w || u) meets a
# *constraint*. For Gaussian advantages they coincide at eta = std(A) / sqrt(2 eps).
# Under heavy tails the constraint raises eta and shrinks every typical sample's tilt:
# v4's realized policy KL decayed 0.023 -> 0.012 as ESS fell 0.44 -> 0.23. The penalty keeps
# the typical tilt fixed and lets tail samples take more weight instead.
# --advantage-temperature c sets eta = c * std(A) (TPO: c = 1); leave it unset to solve kl_budget.
# Gradient clipping is off by default: it flattens the self-limiting TPO gradient
# (v4's no-clip run beat the clipped one at 16M and 24M). v4 notes follow.
# AWR cross-entropy v4: v3 with separate actor and critic gradient clipping. Base: awr_ce_tpo_batch_v3.
# A single clip over both networks lets critic gradient spikes shrink the actor's
# step (the critic took about 0.46 of the 0.5 post-clip norm in v12). The TPO policy gradient is
# O(1/N) per sample, so under the joint clip it is the critic that decides the
# clip. Each network now gets its own max_grad_norm budget, which also makes
# vf_coef inert. Pre-clip norms are logged per network. v3 notes follow.
# AWR cross-entropy v3: TPO-style batch target matching. Base: awr_ce_rb_anchor_v2.
# The CE is taken between two distributions over the rollout's samples: the fixed
# target w = softmax(A / eta) and the policy's batch distribution
# p_theta = softmax_i(log pi_theta(a_i|s_i) - log pi_old(a_i|s_i)).
#   L = -sum_i w_i log p_theta,i,   dL/dlogratio_i = p_theta,i - w_i.
# At theta_old, p = 1/N, so the gradient is the centered signal alone. v1's maximum-likelihood noise
# drops out exactly, because a common shift of every log-ratio leaves p unchanged. The
# gradient vanishes once the ratios reproduce w, so the target is a fixed attractor
# across epochs (Kaddour 2026, TPO), with the old-policy anchor built into w.
# v2's exact Beta KL(pi_old || pi) is kept only as a diagnostic.
# Hypothesis: a self-limiting target step reuses the 10 epochs with PPO-like KL.
import os
import random
import time
...
# Geometric bisection over [span * 1e-6, span / kl_budget]: 40 halvings of a
# ~16-nat bracket resolve log(eta) far below FP32 noise in the weights.
TEMPERATURE_ITERATIONS = 40
TEMPERATURE_LOW_FRACTION = 1e-6
...
class Args:
    ...
    kl_budget: float = 0.1
    """E-step constraint KL(w || uniform) over the rollout; sets eta"""
    advantage_temperature: float | None = None
    """if set, eta = advantage_temperature * std(A) (TPO's fixed penalty on z-scores) instead of the budget"""
    vf_coef: float = 0.5
...
def fixed_temperature(advantages, temperature):
    """TPO penalty form: softmax of z-scored advantages at a fixed temperature."""
    eta = temperature * advantages.std()
    log_weights = ((advantages - advantages.mean()) / eta).log_softmax(dim=0)
    weights = log_weights.exp()
    kl = (weights * (log_weights + log(advantages.numel()))).sum()
    return weights, eta, kl


def solve_temperature(advantages, kl_budget):
    """Batch-wide E-step: the eta with KL(softmax(A / eta) || uniform) = kl_budget.

    KL falls monotonically in eta. At eta = span / kl_budget every logit lies in
    a range of kl_budget, which bounds the KL by kl_budget, so the upper bracket
    is always feasible; the returned eta is the feasible side of the bracket.
    Logits are centered at the maximum so FP32 keeps near-maximum differences.
    """
    log_n = log(advantages.numel())
    centered = advantages - advantages.max()
    span = (-centered.min()).clamp_min(1e-12)
    log_low = span.log() + log(TEMPERATURE_LOW_FRACTION)
    log_high = (span / kl_budget).log()
    for _ in range(TEMPERATURE_ITERATIONS):
        log_mid = 0.5 * (log_low + log_high)
        log_weights = (centered / log_mid.exp()).log_softmax(dim=0)
        kl = (log_weights.exp() * (log_weights + log_n)).sum()
        infeasible = kl > kl_budget
        log_low = torch.where(infeasible, log_mid, log_low)
        log_high = torch.where(infeasible, log_high, log_mid)
    eta = log_high.exp()
    log_weights = (centered / eta).log_softmax(dim=0)
    weights = log_weights.exp()
    kl = (weights * (log_weights + log_n)).sum()
    return weights, eta, kl
...
def awr_loss(agent, observations, native_actions, old_logprobs, old_alpha, old_beta,
             weights, returns, args):
    """CE from the batch target to the policy's batch distribution, plus unclipped value MSE.

    ``weights`` are the E-step softmax scaled by the batch size (mean one); they
    are renormalized over the minibatch, which is the whole rollout by default.
    """
    ...
    policy_loss = -(target * logratio.log_softmax(dim=0)).sum()
    value_loss = 0.5 * (newvalue - returns).square().mean()
    loss = policy_loss + args.vf_coef * value_loss
    with torch.no_grad():
        anchor_kl = beta_kl(old_alpha, old_beta, alpha, beta).sum(-1).mean()
        approx_kl = (logratio.exp() - 1 - logratio).mean()
        # Log-likelihood moved onto the target beyond the uniform (anchor) share.
        target_gain = ((weights - 1.0) * logratio).mean()
    metrics = torch.stack((policy_loss.detach(), value_loss.detach(), entropy.mean().detach(),
                           anchor_kl.detach(), approx_kl, target_gain))
    return loss, metrics
...
def validate_args(args):
    ...
    if not 0.0 < args.kl_budget < log(args.batch_size):
        raise ValueError("kl_budget must lie in (0, log(batch_size))")
    if args.advantage_temperature is not None and not args.advantage_temperature > 0.0:
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); batch-wide AWR CE target, eta solved to KL(w||u)=kl_budget; TPO batch target matching, CE(w, softmax(logratio))")
        ...
        def loss_model(observations, native, old_logprobs, old_alpha, old_beta, weights, returns):
            return awr_loss(agent, observations, native, old_logprobs, old_alpha, old_beta,
                            weights, returns, args)

        @torch.no_grad()
        def prepare_targets(advantages):
            """The batch-wide policy target, fixed for every epoch of this rollout."""
            if args.advantage_temperature is None:
                weights, eta, estep_kl = solve_temperature(advantages, args.kl_budget)
            else:
                weights, eta, estep_kl = fixed_temperature(advantages, args.advantage_temperature)
            diagnostics = {
                "awr/eta": eta, "awr/estep_kl": estep_kl,
                "awr/ess_fraction": weights.square().sum().reciprocal() / weights.numel(),
                "awr/max_weight_x_n": weights.max() * weights.numel(),
                "awr/advantage_std": advantages.std(),
            }
            return weights * weights.numel(), diagnostics
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_weights, target_diagnostics = prepare_targets(advantages.flatten())
            updates = steps = 0
                    ...
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        if epoch < args.actor_epochs:
                            loss, metrics = loss_model(
                                ...
                                b_weights[indices], b_returns[indices],
                            )
            ...
            logged = gather_metrics({
                ...
                "losses/approx_kl": last[4], "awr/target_gain": last[5],
