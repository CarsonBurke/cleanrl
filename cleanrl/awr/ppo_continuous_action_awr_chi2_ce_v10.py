# AWR chi-square CE v10: the AWR/MPO forward-KL M-step onto the clipped chi^2 E-step target. Base: v9.
# E-step (chi^2 mirror descent): r* = pi*/pi_old = (1 + (z - lambda)/eta)_+, with lambda set so mean(r*) = 1.
# M-step (generalized KL(pi* || pi) projection): L = E_old[r - r* log r], r = pi_theta/pi_old.
# Against the rest of the family:
# - CE v1-v5 weight by exp(A/eta): ESS ~ 1/kurtosis (~1% at kurtosis 100). Here r* is linear in z.
# - v6 fits log r to z/eta in L2. Unreached targets (z = -30 asks for log r = -30) pull at full
#   weight for all 10 epochs. Here a removed action (r* = 0) has l-gradient r -> 0 and saturates.
# - v9 fits r to r* in L2. Its l-weight (r* - r) r peaks at r*^2/4, so the positive tail collapses
#   the ESS in later epochs. Here the l-gradient is r - r*: linear, convex (d2/dl2 = r), optimum r = r*.
# - E_old[r | s] = 1 for a normalized policy, so the r term is a zero-mean importance-sampled
#   baseline. In expectation L is AWR's weighted MLE -E[r* log pi], and any per-state offset
#   in r* only rescales pi*(.|s), which the normalization removes.
# - At theta_old the gradient is the policy gradient of (z - lambda)/eta, with removed actions at
#   weight 1, and the Gauss-Newton curvature is the Fisher.
# Hypothesis: CE with an exact, linear, self-saturating target fits the bulk and the tails at
# PPO-sized steps without a clip.
import os
import random
import time
