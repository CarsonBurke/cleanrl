# AWR sign-reach CE v16: a weighted generalized-KL M-step E[w (r - r* log r)] whose weights carry
# the advantage magnitude and whose targets carry only its sign. Base: v15.
# Derivation: a CE rule with force z at theta_old (w (r* - 1) = z) equals the importance-sampled
# surrogate -E[z (r - 1)] plus a per-sample reverse-KL penalty k_i E[r - 1 - log r], k_i = w r*.
# - Uniform k (w = k - z, r* = k/(k - z): the "natural" weighting) spends the step on the tails:
#   bench dJ-ref -0.1 to -0.3e-3, in-sample gain ~half PPO's, the bulk bins barely separated.
# - v15's exp target has k_i -> z/eta for good samples, so its reach grows with |A|: highest
#   in-sample gain, held-out dJ below PPO (noise fit by outliers).
# - k_i proportional to |z| makes each sample's fixed point independent of |A| (PPO's structure):
#   w = z / (r* - 1), r* = e^up (z > 0) or e^-down (z < 0). Magnitude sets how hard a sample pulls
#   in the shared-parameter compromise, never how far. Bench: the only CE family tied with PPO;
#   reach up 1.0 / down 0.6 at the edge of the grid (larger reach kept helping).
# Hypothesis: with PPO's reach structure but CE's smooth restoring fit, AWR matches or beats PPO.
import math
import os
import random
import time
...
class Args:
    ...
    down_reach: float = 0.6
    """negative log-ratio target for negative-advantage samples: r* = exp(-down_reach)"""
...
def awr_loss(agent, observations, native_actions, old_logprobs, old_alpha, old_beta,
    ...
    """Weighted generalized-KL projection onto the sign targets exp(up) / exp(-down), plus unclipped value MSE.
...
def validate_args(args):
    ...
    if not (args.up_reach > 0.0 and args.down_reach > 0.0):
        raise ValueError("up_reach and down_reach must be positive")
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); weighted generalized-KL M-step E[w (r - r* log r)], r* = exp(up) if z > 0 else exp(-down), w = z/(r* - 1)")
        ...
        def prepare_targets(advantages):
            """Per-sample (weight, ratio target) = (z/(r* - 1), r*), r* = e^up or e^-down by the sign of z."""
            advantage_std = advantages.std()
            z = (advantages - advantages.mean()) / advantage_std
            up = z > 0
            target = torch.where(up, math.exp(args.up_reach), math.exp(-args.down_reach))
            weight = z.abs() / torch.where(up, math.expm1(args.up_reach), -math.expm1(-args.down_reach))
            diagnostics = {
