# AWR cross-entropy v2: Rao-Blackwellized anchor. Base: awr_ce_v1.
# v1 fell short because of noise, not the target. With W_i = N w_i,
#   -sum_i w_i log pi_i = -mean((W_i - 1) log pi_i) - mean(log pi_i),
# and the second term is maximum likelihood on one sampled action per state: zero-mean
# gradient noise that Adam scales up to a full lr step. v1's policy KL was 0.12-0.23 (PPO 0.024)
# and was largest at the smallest budget. v2 replaces that term by its exact
# expectation over a ~ pi_old, KL(pi_old(.|s) || pi(.|s)) + H(pi_old), which is closed
# form for Beta. The objective is unchanged in expectation; only sampling noise is removed.
# v1 notes follow.
# Policy: the clipped surrogate is replaced by the MPO/AWR E-step target over the
# whole rollout, w_i = softmax_i(A_i / eta), with eta solved exactly so that
# KL(w || uniform) = kl_budget, then fit by weighted CE, -sum_i w_i log pi(a_i|s_i).
# Near w = 1/N this is (1/N) sum [1 + (A_i - mean A)/eta] grad log pi: a PPO-like
# signal plus a sample estimate of KL(pi_old || pi), so the fixed target itself is
# the trust region and eta makes the step invariant to advantage scale.
# Critic unchanged from the base: scalar head, unclipped MSE to GAE lambda-returns.
# Hypothesis: a fixed, KL-bounded batch target reuses the rollout's 10 epochs
# more safely and efficiently than ratio clipping.
import os
import random
import time
...
def awr_loss(agent, observations, native_actions, old_logprobs, old_alpha, old_beta,
    ...
    """Rao-Blackwellized weighted CE to the fixed batch target plus unclipped value MSE.

    ``weights`` are the E-step softmax scaled by the batch size (mean one). The
    uniform share of the CE is replaced by its expectation under pi_old: the exact
    per-state KL(pi_old || pi), up to the constant H(pi_old).
    """
    ...
    policy_loss = anchor_kl - ((weights - 1.0) * newlogprob).mean()
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); batch-wide AWR CE target, eta solved to KL(w||u)=kl_budget; Rao-Blackwellized uniform share as exact Beta KL")
