# HOPSD v16 — precision-weighted tilt: privileged information enters through the
# WEIGHTS, the one channel where it provably cannot backfire.
# =====================================================================================
# Ablation ladder so far @1.5M: no-phi 4726 < pooled-phi 5288; richer contexts FAILED
# (ordered lags 4107 — target jitter; Q(s,z,phi) dead — phi drowned the action signal).
# Lesson: privileged info helps, but pushing more of it into the teacher's INPUT either
# jitters the target or explains the advantage away. v16 routes it into the tilt
# weights instead: sigma_T(s,phi) = the privileged teacher critic's predictive spread =
# return noise that even hindsight cannot explain. Tilt on the t-statistic
# adv / sigma_T (then the unchanged v12.2 winsorize + KL-targeted dual): statistically
# confident advantages tilt at full strength, lucky-looking ones get shrunk — the
# winner's-curse defense, which should also make higher tilt_eps safe later.
# Per-sample down-scaling of a weight cannot re-rank actions within a state nor
# rationalize an advantage (it never enters the conditional), so the v1/v13/v15
# failure modes are structurally unavailable. Iteration 1 degrades exactly to v12.2.
# Watch debug/precision_sigma_med (spread of the sigma field = how much re-ranking is
# happening) and matched-step return vs control (2206@500k / 4159@1M / 5288@1.5M).
# =====================================================================================
#
...
class Args:
    ...
    precision_weight: bool = True    # v16: tilt on adv/sigma_T (t-statistic) instead of raw adv
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v16: precision weighting. sigma_T(s, phi) from the previous iteration's
            # privileged teacher critic (chunked, no_grad); delta method maps symlog
            # moments to a raw-space spread. Iteration 1: zero-init head -> uniform
            # bins -> constant sigma -> the z-score below removes it -> exact v12.2.
        ...
        if args.precision_weight:
            ...
            writer.add_scalar(
                "debug/precision_sigma_cv",
                (sigma_raw.std() / (sigma_raw.mean() + 1e-8)).item(),
                global_step,
            )
        with torch.no_grad():
            _p = awr_w / awr_w.sum()
