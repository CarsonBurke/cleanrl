# MuonH = Hyperball(Muon) on the same scalar-PPO recipe; --optimizer-kind adam is
# the unchanged fused-Adam control. Mechanism: plain-EMA Muon momentum (no bias
# correction, no Nesterov), Newton-Schulz msign as the direction, then Hyperball --
# normalize it, step eta*R, project the whole matrix back to its INITIAL Frobenius
# radius. Novelty vs the AdamH sibling: the direction has equal singular values, so
# eta is a per-matrix ANGULAR step, and every per-layer Muon gain (s_mu, Moonlight,
# the NS output scale) cancels exactly under that normalization. No weight decay is
# possible: the projection is purely radial and erases decay exactly.
# Constrained set = the 18 SiTU-GLU gate/up/down matrices of both trunks, chosen by
# module traversal. in_proj, block gates, biases and both heads stay on fused Adam;
# the actor head is initialized at std=0.01, so its radius would be ~0.02 and
# Hyperball would freeze it at a near-zero norm.
# Honesty: the paper's expressivity argument (Sec. 4.1) needs a TRAINABLE norm gain
# to absorb the fixed matrix scale, and this trunk uses non-affine RMSNorm with a
# fixed width**-0.5 readout, so pinning ||W|| genuinely restricts the function class
# here. The paper also reports Hyperball starting slightly WORSE and overtaking only
# as the LR decays, and all of its evidence is LLM pretraining -- zero RL evidence.
# Hypothesis: removing the silent 1/||W|| angular-LR collapse of unconstrained Muon
# buys late-training progress at 8M steps; it may simply cost early return.
import os
import random
import time
...
class Args:
    ...
    """32x the 3e-4 PPO default; Adam rate for unconstrained parameters"""
    optimizer_kind: Literal["adam", "muonh"] = "muonh"
    """original one-Adam control, or MuonH hidden matrices plus auxiliary Adam"""
    hyperball_learning_rate: float = 0.018
    """relative whole-matrix Hyperball step size before linear annealing"""
    hyperball_momentum: float = 0.9
    """Muon momentum EMA pole; no bias correction and no Nesterov lookahead"""
    hyperball_ns_steps: int = 5
    """Newton-Schulz iterations approximating msign; scale is irrelevant here"""
    num_envs: int = 16
    ...
    grad_clip: bool = True
    """independent actor/critic clipping, matching the indclip_v4 baseline budget"""
...
def hidden_branch_parameters(agent):
    """Constrain only explicit SiTU-GLU branch matrices, never all 2-D tensors."""
    parameters = []
    for network in (agent.actor, agent.critic):
        for branch in network[0].blocks:
            if not isinstance(branch, SiTUGLUBranch):
                raise ValueError("MuonH requires SiTU-GLU branches (activation='stiglu')")
            parameters.extend((branch.gate.weight, branch.up.weight, branch.down.weight))
    return tuple(parameters)


def make_optimizers(agent, args):
    """Keep the original fused-Adam control, or split disjoint optimizer ownership."""
    if args.optimizer_kind == "adam":
        return optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5, fused=True), None
    if args.optimizer_kind != "muonh":
        raise ValueError("optimizer_kind must be 'adam' or 'muonh'")
    hidden = hidden_branch_parameters(agent)
    hidden_ids = {id(parameter) for parameter in hidden}
    auxiliary = [parameter for parameter in agent.parameters() if id(parameter) not in hidden_ids]
    optimizer = optim.Adam(auxiliary, lr=args.learning_rate, eps=1e-5, fused=True)
    hyperball_optimizer = MuonH(
        hidden,
        lr=args.hyperball_learning_rate,
        momentum=args.hyperball_momentum,
        ns_steps=args.hyperball_ns_steps,
        compile=args.compile,
    )
    return optimizer, hyperball_optimizer


@torch.no_grad()
def geometry_metrics(hyperball_optimizer):
    """Gather fixed-initial-radius drift and the actual device learning rate once per rollout."""
    if hyperball_optimizer is None:
        return {}
    errors = []
    for group in hyperball_optimizer.param_groups:
        for parameter in group["params"]:
            radius = hyperball_optimizer.state[parameter]["radius"]
            # A zero-radius sphere is a singleton; use its absolute norm error.
            denominator = torch.where(radius > 0, radius, torch.ones_like(radius))
            errors.append((torch.linalg.vector_norm(parameter) - radius).abs() / denominator)
    return {
        "geometry/max_radius_relative_error": torch.stack(errors).max(),
        "charts/hyperball_learning_rate": hyperball_optimizer.param_groups[0]["lr"],
    }


@torch.no_grad()
def stable_rank_metrics(hyperball_optimizer):
    """Mean ||W||_F^2 / ||W||_op^2: Frobenius control tracks spectral control only when this is high."""
    if hyperball_optimizer is None:
        return {}
    ranks = []
    for group in hyperball_optimizer.param_groups:
        for parameter in group["params"]:
            spectral = torch.linalg.matrix_norm(parameter, ord=2)
            safe = torch.where(spectral > 0, spectral, torch.ones_like(spectral))
            ranks.append(torch.linalg.vector_norm(parameter) ** 2 / safe ** 2)
    return {"geometry/mean_stable_rank": torch.stack(ranks).mean()}
...
def validate_args(args):
    ...
    if args.optimizer_kind == "muonh" and args.activation != "stiglu":
        raise ValueError("MuonH requires activation='stiglu'")
...
def main():
    ...
    try:
        ...
        writer.add_text(
            ...
            f"optimizer={args.optimizer_kind}; Hyperball LR={args.hyperball_learning_rate}; "
            f"momentum={args.hyperball_momentum}; ns_steps={args.hyperball_ns_steps}; "
            f"epochs={args.update_epochs}; grad_clip={args.grad_clip}; "
            f"clip_heads={args.clip_heads}; 1 minibatch; raw GAE",
        )
        envs = make_training_env(args, run_name)
        resources.callback(envs.close)
        agent = Agent(envs, placement=args.placement, norm_kind=args.norm_kind, activation=args.activation).to(device)
        optimizer, hyperball_optimizer = make_optimizers(agent, args)
        ...
        for iteration in range(1, args.num_iterations + 1):
            if args.anneal_lr:
                ...
                if hyperball_optimizer is not None:
            ...
            logged = gather_metrics(
                {
                    **geometry_metrics(hyperball_optimizer),
                    **stable_rank_metrics(hyperball_optimizer),
            ...
            torch.save(
                {
                    ...
                    "hyperball_optimizer": (
                        hyperball_optimizer.state_dict() if hyperball_optimizer is not None else None
                    ),
                    "obs_norm": {
