# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + DREAMER4 STATE-ENTROPY v3.
#
# ADD-ON over symlog_v1 (DEFAULT OFF => byte-identical to v1 unless --state-entropy):
# dreamer4's unimodal-Beta state-prediction-entropy exploration bonus.
#
#   ============================ v3: FIX OVER v2 ============================
#   v2 stopped the collapse but still slightly UNDERPERFORMED base (1M: 3351 vs 4063):
#   the bonus was a near-neutral drag because the Beta TARGET was degenerate. v2 mapped
#   obs -> (0,1) via (obs/CLIP + 1)/2 with CLIP=10 -- but 10 is the obs CLAMP bound, not
#   its SCALE. Normalized obs are ~N(0,1), so obs/10 ~ N(0,0.1) and the target collapses
#   to ~N(0.5, 0.05): >99% of mass in [0.28,0.71], the full (0,1) range almost unused.
#   The unimodal Beta then becomes trivially confident (a=b~50, NLL ~ -27) for
#   REPRESENTATIONAL reasons, not learned dynamics, so H[Beta] carries almost no novelty
#   information -- the surviving entropy spread is ~noise (bonus std ~0.028, ~1-3% of the
#   advantage), which rankgauss (scale-free) then re-amplifies into advantage RANKINGS.
#   v3: scale the affine target by the obs STD (~1), not the clamp. `state_obs_scale=3`
#   (~3 sigma) maps obs/3 -> N(0,0.33) -> target N(0.5,0.167) so +-3 sigma fills the full
#   (0,1) range. The predictor now faces a genuine prediction task with real dynamic
#   range; its confidence/entropy reflects learned predictability differences, making
#   the bonus an INFORMATIVE epistemic signal instead of compressed-target noise.
#   Also tightens state_pred_eps 1e-3 -> 1e-6 for exact dreamer4 eps_latent_pred parity.
#
#   HONEST PROVENANCE: dreamer4 DEFINES `state_entropy_bonus_weight` (=0.05) and
#   `add_state_entropy_bonus` (dreamer4.py:3597-3598) but NEVER CONSUMES them -- grep
#   shows zero reads; the only use of the state Beta is its NLL training loss
#   (dreamer4.py:5931-5938). So the BONUS WIRING here realizes dreamer4's evident
#   *intent*, not a wired mechanism. What IS faithfully ported is the distribution:
#     * BetaDist(unimodal=True): a = 1 + softplus(raw_a), b = 1 + softplus(raw_b)
#       (both >= 1 => concave/unimodal Beta on (0,1)), one Beta per state dim. Same
#       parameterization the v24 actor uses for actions; here it models the next state.
#     * NLL  -Beta.log_prob(target)  with the target mapped into (0,1) -- see v2 fix.
#   The bonus is  state_entropy_bonus_weight * H[Beta]  (differential entropy of the
#   predicted next-state Beta): pay the agent to reach states the forward model finds
#   uncertain (curiosity / Plan2Explore epistemic drive). Unimodal-Beta H is <= 0;
#   only its per-state VARIATION is signal (the offset cancels in advantage norm).
#
#   ============================ v2: FIXES OVER v1 ============================
#   v1 (stateentropy_v1) ACTIVELY HURT on HalfCheetah: identical to base until ~200k,
#   then policy entropy collapsed FASTER than base and return cratered (294 vs 1160 at
#   400k). Two root causes, both fixed here:
#
#   (1) PER-ROLLOUT STANDARDIZATION amplified noise. v1 z-scored the bonus to unit std
#       every rollout, then x0.05. Once the forward model gets confident (NLL plateaus
#       ~ -25 by 200k), H[Beta] is near-zero-VARIANCE -- the residual is mostly noise.
#       Z-scoring rescaled that noise to unit std and injected it into the advantage;
#       PPO+rankgauss then reinforced spurious rankings => entropy collapse + reward
#       crater. It ALSO destroyed self-annealing (a good curiosity signal must VANISH
#       as the model learns; standardization force-revives it as noise).
#       v2: default state_entropy_norm=False. The bonus is only CENTERED (offset is
#       meaningless), never variance-normalized, so its natural scale SELF-ANNEALS:
#       large meaningful spread early, ~0 once the model is confident => worst case
#       neutral, never noise injection.
#
#   (2) tanh ON RAW OBS saturated the novel dims. dreamer4 applies NO nonlinearity --
#       (x+1)/2 is an affine rescale of ALREADY-bounded latents. v1 applied tanh to
#       raw normalized obs (= sigmoid(2*obs)), saturating |obs|>~2 to ~1: exactly the
#       large-magnitude / most-novel dims collapse to one value, suppressing the
#       uncertainty signal where it matters. v2: faithful AFFINE rescale of the
#       wrapper-CLIPPED obs range [-clip,clip] -> (0,1):  (obs/clip + 1)/2, clamped.
#       No saturation; distinct large-|obs| successors stay distinct.
#
#   PORT STRUCTURE (unchanged from v1):
#     * SELF-CONTAINED forward model p(s'|s,a) = prod_i Beta(a_i,b_i) (own MLP over
#       [obs, action]; dim state_pred_hidden), DECOUPLED from the actor/critic
#       ThinkTrunks so state_entropy=False leaves v1/v1-base untouched (clean A/B).
#       Trained by masked NLL (terminal transitions excluded: their s' is a reset).
#       NB: this is action-conditioned p(s'|s,a) -- dreamer4's bonus FLAG points at the
#       sequence-conditioned world-model head, but p(s'|s,a) is the standard/better
#       object for an exploration bonus (rewards action choices with uncertain outcome).
#     * The bonus enters the POLICY ADVANTAGE ONLY, via the SAME soft-GAE machinery
#       this file uses for the auto-entropy path -- NEVER the HL-Gauss critic target,
#       honoring v1's thesis (bounded categorical critic regresses RAW reward return,
#       support stays calibrated) while giving the bonus MULTI-STEP GAE credit.
#
...
class Args:
    ...
    # Dreamer4 state-entropy exploration bonus (DEFAULT OFF; clean superset of v1).
    # A self-contained unimodal-Beta forward model p(s'|s,a) (dreamer4's BetaDist,
    # unimodal=True: a,b = 1+softplus(raw)) is trained by NLL to predict the NEXT
    # normalized observation, mapped to (0,1) via dreamer4's (tanh(x)+1)/2 squash.
    # Its per-dim differential entropy H[p(s'|s,a)] (summed over obs dims), weighted
    # by state_entropy_bonus_weight, is added to the POLICY-advantage soft GAE only
    # (the HL-Gauss critic target stays on the raw reward return). See header.
    state_entropy: bool = False
    state_entropy_bonus_weight: float = 0.05  # dreamer4 default; scales the (centered) entropy bonus
    state_pred_loss_weight: float = 0.1       # dreamer4 default; weights the forward-model NLL
    state_pred_hidden: int = 256              # forward-model MLP width (independent of policy `hidden`)
    state_pred_eps: float = 1e-6              # clamp the (0,1) Beta target off the boundary (dreamer4 eps_latent_pred)
    state_obs_scale: float = 3.0              # obs std-scale mapped to the Beta (0,1) edges: target=(obs/scale+1)/2 (~3 sigma fills (0,1))
    # v2: default OFF. v1 z-scored the bonus to unit std each rollout, which amplified
    # near-zero-variance noise once the forward model converged (entropy collapse). The
    # bonus is always CENTERED; set True to ALSO variance-normalize (v1 behavior, kept
    # for ablation only — NOT recommended, it removes the self-annealing safety).
    state_entropy_norm: bool = False
...
class StateForwardModel(nn.Module):
    """Dreamer4 state-entropy forward model: p(s' | s, a) = prod_i Beta(a_i, b_i).

    A self-contained MLP over [obs, action] emitting two raw concentrations per
    observation dim, mapped to a UNIMODAL Beta exactly as dreamer4's
    BetaDist(unimodal=True): alpha = 1 + softplus(raw_a), beta = 1 + softplus(raw_b)
    (both >= 1 => concave Beta on (0,1)). Decoupled from the actor/critic trunks so
    it never perturbs the PPO gradients. Its per-dim differential entropy is the
    state-entropy exploration bonus; its NLL trains the prediction.
    """
    ...
    def __init__(self, obs_dim, act_dim, H):
        ...
        self.net = nn.Sequential(
            layer_init(nn.Linear(obs_dim + act_dim, H)),
            nn.Tanh(),
            layer_init(nn.Linear(H, H)),
            nn.Tanh(),
        )
        # Two concentrations per obs dim; small init => near-uniform Beta at start.
        self.head = layer_init(nn.Linear(H, obs_dim * 2), std=0.01)

    def dist(self, obs, action):
        h = self.net(torch.cat([obs, action], dim=-1))
        a, b = self.head(h).view(*h.shape[:-1], self.obs_dim, 2).unbind(-1)
        # dreamer4 BetaDist(unimodal=True): offset 1.0 => alpha,beta >= 1 (unimodal).
        alpha = 1.0 + F.softplus(a)
        beta = 1.0 + F.softplus(b)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Dreamer4 state-entropy forward model (decoupled; only built when enabled).
        self.state_entropy = args.state_entropy
        if self.state_entropy:
            self.state_model = StateForwardModel(obs_dim, act_dim, args.state_pred_hidden)

    def state_params(self):
        # Params of the decoupled forward model (disjoint from actor/critic groups).
        return list(self.state_model.parameters()) if self.state_entropy else []
...
if __name__ == "__main__":
    ...
    # Decoupled state-entropy forward model (disjoint from actor/critic groups). It is
    # in agent.parameters() => the global-clip path trains it; the separate-clip path
    # backprops its NLL explicitly below.
    state_params = agent.state_params()
    ...
    if args.state_entropy:
        # Forward-model targets s_{t+1} and a validity mask (0 across episode resets,
        # whose s' is a fresh reset state rather than a real successor), plus the
        # per-step raw entropy bonus collected at rollout time.
        next_obs_buf = torch.zeros_like(obs)
        fwd_mask = torch.zeros((args.num_steps, args.num_envs)).to(device)
        raw_state_bonus = torch.zeros((args.num_steps, args.num_envs)).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if args.state_entropy:
                    # Curiosity bonus = H[p(s'|s,a)] of the (pre-update) forward model,
                    # summed over obs dims. Stored raw; standardized/weighted post-rollout.
                    raw_state_bonus[step] = agent.state_model.dist(obs[step], action).entropy().sum(-1)
            actions[step] = action
            latent_zs[step] = z
            ...
            if args.state_entropy:
                # Forward-model target s_{t+1}; mask out transitions that just reset
                # (next_done) so we never regress to a post-reset observation.
                next_obs_buf[step] = next_obs
                fwd_mask[step] = 1.0 - next_done
        ...
        with torch.no_grad():
            ...
            # DREAMER4 STATE-ENTROPY per-step reward bonus (policy advantage only). The
            # raw forward-model entropy is standardized over the rollout (its absolute
            # offset is meaningless; only per-state variation drives exploration) and
            # scaled by state_entropy_bonus_weight. The HL-Gauss critic target (returns,
            # above) is deliberately left bonus-free so its fixed support stays calibrated.
            if args.state_entropy:
                # Center (the absolute offset of H[Beta] is meaningless). v2: do NOT
                # variance-normalize by default -- the natural per-state spread SELF-
                # ANNEALS as the forward model converges (=> neutral, not noise).
                rb = raw_state_bonus - raw_state_bonus.mean()
                if args.state_entropy_norm:
                    rb = rb / (rb.std() + 1e-8)
                state_bonus = args.state_entropy_bonus_weight * rb
            else:
                state_bonus = None
            # SOFT-ADVANTAGE GAE (POLICY ONLY): same recursion with the entropy-augmented
            # reward (r + state-entropy bonus) and/or next value V(s')+α·H(s'). Unbiased
            # (state-dependent baseline); its large magnitude is harmless — rankgauss at the
            # optim step rank-normalizes it while PRESERVING the entropy-induced reordering
            # (the actual exploration signal).
            if auto_alpha or args.state_entropy:
                ...
                for t in reversed(range(args.num_steps)):
                    if t == args.num_steps - 1:
                        ...
                        nextnonterminal = 1.0 - dones[t + 1]
                        nextvalues = values[t + 1]
                    r_t = rewards[t] + (state_bonus[t] if args.state_entropy else 0.0)
                    nv_t = nextvalues + (next_value_bonus[t] if auto_alpha else 0.0)
                    delta = r_t + args.gamma * nv_t * nextnonterminal - values[t]
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Dreamer4 forward-model NLL: -log Beta(s'_(0,1) | s, a), summed over obs
                # dims, masked to non-reset transitions. v3: targets map s' into (0,1) by
                # AFFINE rescale at the obs STD-scale -> (obs/state_obs_scale + 1)/2, clamped
                # off the boundary -- dreamer4's (x+1)/2 with NO nonlinearity, but scaled so
                # ~3 sigma of the normalized obs fills the full (0,1) range (v2 used the
                # clamp bound 10 => target degenerate at ~0.5, predictor trivially confident).
                if args.state_entropy:
                    fwd_dist = agent.state_model.dist(b_obs[mb_inds], b_actions[mb_inds])
                    tgt = ((b_next_obs[mb_inds] / args.state_obs_scale + 1.0) * 0.5).clamp(
                        args.state_pred_eps, 1.0 - args.state_pred_eps
                    )
                    nll = -fwd_dist.log_prob(tgt).sum(-1)
                    m = b_fwd_mask[mb_inds]
                    fwd_loss = (nll * m).sum() / m.sum().clamp_min(1.0)
                ...
                if args.separate_grad_clip:
                    ...
                    # Forward model is disjoint from actor/critic params: backprop its
                    # NLL onto its own grads and let the same optimizer.step() update it.
                    if args.state_entropy:
                        (args.state_pred_loss_weight * fwd_loss).backward()
                        nn.utils.clip_grad_norm_(state_params, args.max_grad_norm)
                    optimizer.step()
                else:
                    loss = pg_loss - ent_coef_eff * entropy_loss + v_loss * args.vf_coef
                    if args.state_entropy:
                        loss = loss + args.state_pred_loss_weight * fwd_loss
                    optimizer.zero_grad()
                    loss.backward()
        ...
        if args.state_entropy:
            # Forward-model NLL and the raw (pre-standardize) per-step entropy bonus.
            writer.add_scalar("losses/state_pred_nll", fwd_loss.item(), global_step)
            writer.add_scalar("debug/state_entropy_raw", raw_state_bonus.mean().item(), global_step)
            # Bonus std vs reward-advantage std: the actual exploration signal strength;
            # should DECAY toward 0 as the forward model converges (v2 self-annealing).
            writer.add_scalar("debug/state_bonus_std", state_bonus.std().item(), global_step)
            writer.add_scalar("debug/state_bonus_to_adv_ratio", (state_bonus.std() / (advantages.std() + 1e-8)).item(), global_step)
