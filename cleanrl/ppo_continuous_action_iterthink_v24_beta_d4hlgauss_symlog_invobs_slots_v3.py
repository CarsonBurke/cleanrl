# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted obs slot transformer v3.
#
...
# 1.0, ReLU^2 FFNs, and parameter-golf-style residual mixing against the initial
# slots plus U-Net encoder/decoder skip weights. QK uses normalized tokens, but V
# uses raw magnitude-preserving tokens. The PPO actor/critic heads consume the full
# flattened slot state (4*64=256) instead of compressing back to 64.
#
...
class Args:
    ...
    slot_attn_scale_init: float = 0.25
    slot_mlp_scale_init: float = 0.25
...
class SlotMLP(nn.Module):
    def __init__(self, H, mult):
        super().__init__()
        self.fc = layer_init(nn.Linear(H, mult * H))
        self.proj = layer_init(nn.Linear(mult * H, H), std=1.0)
...
class InvertedCrossAttention(nn.Module):
    ...
    def __init__(self, H, heads, qk_gain_init):
        ...
        self.q = layer_init(nn.Linear(H, H, bias=False), std=1.0)
        self.k = layer_init(nn.Linear(H, H, bias=False), std=1.0)
        self.v = layer_init(nn.Linear(H, H, bias=False), std=1.0)
        self.proj = layer_init(nn.Linear(H, H, bias=False), std=1.0)
...
class InvertedSlotBlock(nn.Module):
    ...
    def forward(self, slots, tokens, slots0):
        ...
        key_tokens = self.token_attn_norm(tokens)
        attn_out = self.attn(self.slot_attn_norm(x), key_tokens, tokens)
...
class ThinkTrunk(nn.Module):
    ...
    def forward_actor(self, x):
        assert self.actor_stack is not None
        return self.actor_stack(self.tokenizer(x))

    def forward_critic(self, x):
        assert self.critic_stack is not None
        return self.critic_stack(self.tokenizer(x))
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        H = args.hidden
        assert args.slot_dim == H, "slot_dim must equal hidden in v3"
