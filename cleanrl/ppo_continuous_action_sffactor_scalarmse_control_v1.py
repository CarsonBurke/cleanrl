# PPO + IterThink v24 Beta + SCALAR-MSE CRITIC CONTROL (for sffactor v1).
# =====================================================================================
# CONTROL RUN for ppo_continuous_action_sffactor_v1.py. Same base as sffactor v1
# (the "ppoadvnorm_batch_v1" reference config, 8278 @8M: Beta actor, shared
# ThinkTrunk, decoupled dual-backward grad clip 0.25/0.25, batch-scope advantage
# z-score, clip 0.2/0.28, tkl03), but the critic is the SIMPLEST possible head:
#
#     V(s) = Linear(H, 1)   trained by plain MSE to the scalar GAE returns,
#                           vf_coef=0.5, no value clipping, zero-init (V=0 prior).
#
# WHY THIS EXISTS: the baseline's critic was a 511-bucket HL-Gauss CE MTP head.
# sffactor v1 swaps BOTH the factorization AND the loss family (CE -> MSE on raw
# ~1e2-1e3-scale targets, which saturates the 0.25 critic grad clip and changes the
# trunk's gradient regime). This control isolates the loss-family/scale confound:
#   - control ~= 8278 baseline AND sffactor ~= control  => plumbing fine, MSE fine.
#   - control ~= 8278 but sffactor low                  => the SF factorization
#     (psi TD indirection / w regression bias) is what loses value accuracy.
#   - control well below 8278                           => CE->MSE is the confound;
#     8278 was never the right bar for ANY MSE-family critic on this base, and
#     sffactor must be judged against THIS run, not the baseline.
# =====================================================================================
import os
import random
import time
...
class Args:
    ...
    # ppoadvnorm_batch_v1 config, baked in.
    norm_adv: bool = True
    norm_adv_scope: str = "batch"    # "batch" (reference) | "minibatch" (idiomatic PPO)
    clip_coef: float = 0.2
    clip_coef_high: float = 0.28
    ent_coef: float = 0.0
    vf_coef: float = 0.5             # weight on the scalar MSE value loss
    max_grad_norm: float = 0.5       # used only when separate_grad_clip=False
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # CONTROL: plain scalar critic head, zero-init (V=0 prior, matching both the
        # base's neutral bucket head and sffactor's zeroed psi/w).
        self.critic_head = layer_init(nn.Linear(H, 1, bias=True), std=0.1)
        with torch.no_grad():
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # CONTROL value loss: plain scalar MSE to returns, no clipping.
                v_loss = args.vf_coef * F.mse_loss(newvalue, b_returns[mb_inds])
        ...
        n_mb = max(acc["n"], 1)
        writer.add_scalar("losses/value_loss", acc["v"] / n_mb, global_step)
