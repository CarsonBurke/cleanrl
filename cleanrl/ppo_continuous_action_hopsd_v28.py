# HOPSD v28 — target-entropy dual: hold the student's entropy up so the operator
# keeps a usable step size.
# =====================================================================================
# Panel-convergent diagnosis of the 5.5k band: student entropy falls monotonically
# (-0.8 early, -4 by 1.5M, -6 by 2.3M) with NO counterforce — ent_coef has been 0.0
# in every variant, so the plumbed entropy term is inert. As the Beta concentrates,
# the Fisher metric blows up and a fixed-KL improvement step buys a vanishing mean
# shift (champion approx_kl decays all run); the operator starves regardless of how
# good the teacher's signal is. This is the structural gap to SAC, whose target-
# entropy temperature holds the metric bounded forever.
# v28 ports exactly that: a scalar dual variable ent_alpha replaces the fixed
# ent_coef, updated multiplicatively once per iteration toward target_entropy
# (z-space Beta entropy summed over dims): alpha *= exp(eta*(H_target - H)); clamp
# [1e-6, 0.5]. Early (H ~ -1 > target -4) alpha decays to the floor and the run is
# ~identical to v19; once H approaches the target (~1M+), the floor engages and
# entropy is held, keeping late-training steps alive.
# Pre-registered: expect only +0..200 @1.5M (floor barely engaged) — the real read
# is the 2.5M+ slope, where v19's curve bends. Tells: ent_alpha rising to its cap
# with returns sagging = target too high (fighting the tilt); distill_kl inflating
# while entropy holds = teacher sharper than a student now forbidden to sharpen —
# pair with a teacher concentration cap in v28.1 if seen.
# =====================================================================================
#
...
class Args:
    ...
    ent_coef: float = 0.0            # superseded by the v28 dual (kept for lineage)
    target_entropy: float = -4.0     # v28: z-space Beta entropy target (sum over dims)
    ent_alpha_init: float = 1e-3     # v28: initial dual temperature
    ent_alpha_eta: float = 0.1       # v28: multiplicative dual step per iteration
    ent_alpha_max: float = 0.5       # v28: dual temperature cap
    vf_coef: float = 0.5
...
if __name__ == "__main__":
    ...
    ent_alpha = args.ent_alpha_init  # v28: entropy dual temperature
    prev_student_entropy = None
    for iteration in range(1, args.num_iterations + 1):
        # v28: dual ascent on the entropy constraint, once per iteration from the
        # previous iteration's full-batch student entropy.
        if prev_student_entropy is not None:
            ent_alpha = float(
                np.clip(
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    (pg_loss - ent_alpha * entropy_loss).backward()
        ...
        prev_student_entropy = float(np.mean(s_ents))  # v28: feeds next iteration's dual
