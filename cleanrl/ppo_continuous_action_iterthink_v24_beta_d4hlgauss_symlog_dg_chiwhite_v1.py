# IterThink v24 beta d4-HL-Gauss symlog critic + chi-whitened Delightful Policy Gradient v1.
#
# Variant hypothesis: keep the current best v24 beta + symlog d4-HL-Gauss critic,
# but make the actor update match Algorithm 2 from arXiv:2603.14608v1:
#   U = the raw policy advantage estimate.
#   ell = clip(-log pi_theta(a|s), -10, 10) from the current action density.
#   chi = whiten(U * ell) within the minibatch.
#   w = sigmoid(chi / eta), eta=1.
#   policy loss = -(stopgrad(w) * U * log pi_theta(a|s)).mean().
# There is no PPO importance ratio and no PPO clipped surrogate in the actor loss.
# No PPO-style advantage normalization is applied by default; the paper describes
# reward normalization for critic stability but uses U_t directly in Algorithm 2.
# This ablation keeps that actor coefficient raw and only whitens the sigmoid
# input, matching the paper's continuous-action scale-sensitivity note without
# turning DG into advantage normalization.
...
class Args:
    ...
    # Delightful Policy Gradient (arXiv:2603.14608v1, Algorithm 2).
    delightful_pg: bool = True
    dg_eta: float = 1.0
    dg_surprisal_clip: float = 10.0
    dg_surprisal_source: str = "current"  # "current" (paper) | "old" (PPO-stability ablation)
    dg_chi_whiten: bool = True
    dg_chi_whiten_center: bool = True
    dg_chi_whiten_eps: float = 1e-8
...
def delightful_gate(
    ...
    The training loss detaches w so the gradient is w * U * grad log pi, not
    the gradient of w * U * log pi. When whiten_chi is enabled, only the
    sigmoid input is standardized; the policy-gradient coefficient U is not.
    """
    if eta <= 0:
    ...
    if whiten_eps <= 0:
        raise ValueError("dg_chi_whiten_eps must be positive")
    raw_surprisal = -logprob
    clipped_surprisal = raw_surprisal.clamp(-clip_bound, clip_bound)
    chi_raw = advantages * clipped_surprisal
    chi_gate = chi_raw
    if whiten_chi:
        if center_chi:
            chi_gate = chi_gate - chi_gate.mean()
        chi_gate = chi_gate / (chi_gate.std(unbiased=False) + whiten_eps)
    gate = torch.sigmoid(chi_gate / eta)
    return gate, clipped_surprisal, raw_surprisal, chi_raw, chi_gate
...
def dg_policy_loss(update_advantages, logprob, gate):
    """Minimization loss for the detached DG score-function estimator."""
    return -(gate.detach() * update_advantages * logprob).mean()
...
if __name__ == "__main__":
    ...
    assert args.dg_surprisal_source in ("current", "old")
    assert args.dg_chi_whiten_eps > 0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.delightful_pg:
                    dg_update_advantages = mb_raw_adv
                    dg_logprob = newlogprob if args.dg_surprisal_source == "current" else b_logprobs[mb_inds]
                    dg_logprob = dg_action_density_logprob(agent, dg_logprob)
                    dg_gate, dg_surprisal, dg_raw_surprisal, dg_chi_raw, dg_chi_gate = delightful_gate(
                        dg_update_advantages,
                        dg_logprob.detach(),
                        eta=args.dg_eta,
                        clip_bound=args.dg_surprisal_clip,
                        whiten_chi=args.dg_chi_whiten,
                        center_chi=args.dg_chi_whiten_center,
                        whiten_eps=args.dg_chi_whiten_eps,
                    )
                    dg_weight = dg_gate.detach()
                    with torch.no_grad():
                        pos_mask = dg_update_advantages > 0
                        neg_mask = dg_update_advantages < 0
                        nonzero_chi = dg_chi_raw != 0
                        dg_update_adv_stds.append(dg_update_advantages.std(unbiased=False).item())
                        dg_delight_means.append(dg_chi_raw.mean().item())
                        dg_delight_stds.append(dg_chi_raw.std(unbiased=False).item())
                        ...
                        dg_chi_raw_means.append(dg_chi_raw.mean().item())
                        dg_chi_raw_stds.append(dg_chi_raw.std(unbiased=False).item())
                        dg_chi_gate_means.append(dg_chi_gate.mean().item())
                        dg_chi_gate_stds.append(dg_chi_gate.std(unbiased=False).item())
                        if nonzero_chi.any():
                            dg_chi_sign_flip_fracs.append(
                                ((dg_chi_raw.sign() != dg_chi_gate.sign()) & nonzero_chi).float().mean().item()
                            )
                    ...
                    dg_update_advantages = mb_advantages

                # Paper-faithful DG actor update: no importance ratio and no PPO
                # clipped surrogate. This implements
                #   sum_t stopgrad(w_t) * U_t * grad log pi_theta(a_t|s_t)
                # as a minimization loss. Ratio/clipfrac above are diagnostics only.
                pg_loss = dg_policy_loss(dg_update_advantages, newlogprob, dg_weight)
        ...
        if args.delightful_pg:
            ...
            writer.add_scalar("debug/dg_chi_raw_mean", list_mean(dg_chi_raw_means), global_step)
            writer.add_scalar("debug/dg_chi_raw_std", list_mean(dg_chi_raw_stds), global_step)
            writer.add_scalar("debug/dg_chi_gate_mean", list_mean(dg_chi_gate_means), global_step)
            writer.add_scalar("debug/dg_chi_gate_std", list_mean(dg_chi_gate_stds), global_step)
            writer.add_scalar("debug/dg_chi_sign_flip_frac", list_mean(dg_chi_sign_flip_fracs), global_step)
            writer.add_scalar("debug/dg_update_adv_std", list_mean(dg_update_adv_stds), global_step)
