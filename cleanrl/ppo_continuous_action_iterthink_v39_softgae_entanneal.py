# PPO + IterThink v39 (SAC max-entropy IN GAE -- ANNEALED entropy target: explore then exploit). From v38.
#
# WHY v39. v38 (scale-controlled soft channel + BINDING FLOOR target=+3.0) ran clean to 3M on all three:
# the soft-channel bug is fully fixed (soft_adv_std_ratio pinned ~1.18, no collapse). But it exposed a
# TASK-DEPENDENCE the fixed FLOOR cannot satisfy at once: HalfCheetah's squashed entropy is HELD at the
# +3.0 floor (alpha ~0.08, H pinned ~2.9-3.0) which FORBIDS the deterministic high-speed gait => HC
# PLATEAUS ~2450 (vs v32's 8800), while the SAME floor sustains the exploration that lets Hopper (~1130)
# and Walker (~880, max 3500, still climbing) beat their v35 marks. HC wants LOW entropy (exploit),
# terminating envs want HIGH entropy (explore). SAC reconciles this by EVALUATING deterministically; the
# on-policy analogue (we score the stochastic policy) is to ANNEAL: explore early, exploit late. v39
# linearly anneals the per-dim entropy target from the +0.5 floor DOWN to the -1.0 SAC-parity level over
# the first 50% of training, then holds. Late, H >> target => the dual bleeds alpha => the direct entropy
# term vanishes => entropy FALLS => the policy becomes near-deterministic and exploits (HC -> its fast
# gait), while the early floor still buys the exploration that helps all envs escape poor local optima.
# The scale-controlled soft channel (coef 0.25, alpha-independent) persists throughout as a small nudge.
# Hypothesis: HC recovers toward ~8800 (entropy free to fall late) AND Hopper/Walker keep v38's gains
# (exploration banked early). One isolated change from v38: target is annealed, not a fixed floor.
#
# ---- inherited v38 notes ----
...
class Args:
    ...
    target_entropy_coef: float = 0.5  # v35 legacy (used only if start/end coefs are left at their defaults
                                    #      AND you want a constant target -- superseded by the v39 anneal below).
    # v39 ENTROPY-TARGET ANNEAL. v38 showed the binding FLOOR rescues the terminating envs (sustained
    # exploration) but CAPS HalfCheetah (~2450 vs v32's 8800): HC's entropy is held at the +3.0 floor,
    # forbidding the deterministic high-speed gait. SAC sidesteps this by EVALUATING deterministically;
    # the on-policy analogue is exploration-THEN-exploitation -- anneal the target from a high floor
    # (early: explore) DOWN to a negative SAC-parity level (late: H >> target => alpha bleeds => entropy
    # FALLS => the stochastic-return policy becomes near-deterministic and exploits). This is env-agnostic:
    # every task gets exploration early and exploitation late, so HC can converge to its fast gait while
    # Hopper/Walker keep their early-exploration gains.
    target_entropy_start_coef: float = 0.5   # per-dim target at iteration 0 (the v35 floor).
    target_entropy_end_coef: float = -1.0    # per-dim target after the anneal (SAC's -|A|; lets H free-fall).
    target_anneal_frac: float = 0.5          # fraction of training over which to anneal start->end, then hold.
    alpha_lr: float = 1e-3
...
if __name__ == "__main__":
    ...
    if auto_alpha:
        ...
        # v39: target_entropy is now ANNEALED per-iteration (see compute_target_entropy in the loop).
        # Initialize to the start level for any pre-loop logging.
        target_entropy = (args.target_entropy if args.target_entropy is not None
                          else args.target_entropy_start_coef * float(act_dim))
        log_alpha = torch.full((1,), float(np.log(args.alpha_init)), requires_grad=True, device=device)
        alpha_optimizer = optim.Adam([log_alpha], lr=args.alpha_lr)

    def compute_target_entropy(iteration):
        # v39: linearly anneal the per-dim target coef from start -> end over target_anneal_frac of
        # training, then hold at end. An absolute target override (if set) stays constant.
        if args.target_entropy is not None:
            return args.target_entropy
        adim = int(np.prod(envs.single_action_space.shape))
        frac = min(1.0, (iteration - 1.0) / max(1.0, args.num_iterations * args.target_anneal_frac))
        coef = args.target_entropy_start_coef + frac * (args.target_entropy_end_coef - args.target_entropy_start_coef)
        return coef * float(adim)
    ...
    for iteration in range(1, args.num_iterations + 1):
        if auto_alpha:
            # v39: refresh the annealed entropy target (the dual's _step_dual closes over this).
            target_entropy = compute_target_entropy(iteration)
        if args.anneal_lr:
        ...
        with torch.no_grad():
            ...
            if auto_alpha and args.soft_adv:
                ...
                ent_returns = ent_adv + ev_denorm           # TRUE-scale future-entropy lambda-return (V_ent target)
                # v37 CENTER GUARANTEE. v36 relied on V_ent's absolute level to center the entropy advantage,
                # but on HalfCheetah the (detached, linear) V_ent fits poorly (ent EV~0: entropy is ~constant
                # across HC states => no per-state value to learn) so ent_adv kept a large POSITIVE mean
                # (~60 >> std ~9) -- the v35 pedestal returned and flipped rankgauss's sign (sign_agree~0.35,
                # HC stuck ~-250). Mean-centering ent_adv per rollout GUARANTEES a zero-mean entropy advantage
                # (kills the pedestal) regardless of V_ent fit, while PRESERVING the per-state variation that
                # carries the real "this state leads to higher-than-average future entropy" exploration signal.
                # V_ent is kept: where it DOES fit (terminating envs, ent EV~0.2-0.3) it reduces ent_adv's
                # variance, so centering removes a smaller offset. The MSE target (ent_returns) stays uncentered
                # so V_ent remains a valid value estimator.
                ent_adv = ent_adv - ent_adv.mean()
                # v38 SCALE CONTROL. v37 centered the entropy advantage (killed the pedestal) but its
                # VARIANCE ran away: alpha barely moves (the once/epoch dual is ~20x too slow to anneal
                # it) while the GAE-accumulated single-sample-logprob entropy noise GROWS with the horizon
                # AND the reward advantage SHRINKS as the critic fits -- so soft_adv_std_ratio climbed to
                # ~8.8 on HC (entropy std 9x the reward-adv std => entropy swamped the policy gradient,
                # HC stuck ~-220, Walker stuck ~2). This is the v28-37 tension: alpha-weighting the
                # GAE-accumulated entropy is scale-UNSTABLE (inert when alpha bleeds, dominant when it
                # doesn't). v38 sets the scale EXPLICITLY: rescale the centered entropy advantage to the
                # reward advantage's std, then weight by a FIXED soft_adv_coef, so the future-entropy
                # reorder is ALWAYS ~soft_adv_coef of the reward scale (soft_adv_std_ratio ~ sqrt(1+c^2),
                # bounded forever) regardless of alpha, horizon, or env. The alpha dual still governs the
                # DIRECT -alpha*logpi term (the sigma-maintenance / true max-ent channel); the in-GAE
                # channel becomes a bounded, controllable exploration nudge toward higher-future-entropy
                # states (real signal where entropy varies by state, minor noise where it is ~constant).
                ent_adv = ent_adv / (ent_adv.std() + 1e-8) * advantages.std()
                policy_adv = advantages + args.soft_adv_coef * ent_adv
