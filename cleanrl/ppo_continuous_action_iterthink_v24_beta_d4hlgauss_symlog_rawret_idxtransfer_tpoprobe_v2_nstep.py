# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return idx-transfer + TPO-PROBE v2 (n-step).
#
# v2 DELTA: probe scores become n-step (tpo_probe_steps, default 2). Each
# candidate's first sim step is unchanged; intermediate states are then rolled
# with the policy's DETERMINISTIC mean action (Beta mean a/(a+b), same z->action
# map + clip, frozen obs normalization, current rollout policy), batched as ONE
# trunk forward per probe step. Score: s_k = sum_t gamma^t r_t + gamma^n *
# (1-term_n) * V_old(s_n), truncated at termination (bootstrap dropped). This
# shrinks the critic-bias share of the score: score noise from V appears at
# gamma^n instead of gamma. tpo_probe_steps=1 reproduces v1 bit-exactly.
#
# --- inherited v1 notes ---
...
class Args:
    ...
    tpo_probe_steps: int = 2     # n-step probe horizon; step 0 = candidate action, steps 1..n-1 = policy mean action; 1 => exact v1 behavior
...
if __name__ == "__main__":
    ...
    assert args.tpo_probe_steps >= 1, "tpo_probe_steps must be >= 1"
    ...
    if tpo_enabled:
        ...
        probe_obs_dim = int(np.array(envs.single_observation_space.shape).prod())
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if tpo_enabled:
                ...
                probe_env_ids = np.nonzero(step_mask)[0]
                if probe_env_ids.size > 0:
                    n_probe = args.tpo_probe_steps
                    P = int(probe_env_ids.size)
                    # Initial sim snapshot per masked env: the restore target for every
                    # candidate's step 0 AND the post-probe restore.
                    init_states = []
                    for env_i in probe_env_ids:
                        d = probe_base_envs[env_i].data
                        init_states.append((d.qpos.copy(), d.qvel.copy(), d.qacc_warmstart.copy(), d.time))
                    probe_disc_rew = np.zeros((P, args.tpo_k), dtype=np.float64)
                    probe_terminated = np.zeros((P, args.tpo_k), dtype=bool)
                    probe_norm_obs = np.zeros((P, args.tpo_k, probe_obs_dim), dtype=np.float32)
                    # Per-(env, candidate) sim snapshot carried between probe steps.
                    cand_states = [[None] * args.tpo_k for _ in range(P)]
                    next_actions_np = None  # (P, K, A) policy mean actions for probe steps t >= 1
                    for t in range(n_probe):
                        disc_t = args.gamma ** t
                        for pi in range(P):
                            env_i = int(probe_env_ids[pi])
                            base_env = probe_base_envs[env_i]
                            obs_rms = probe_obs_wrappers[env_i].obs_rms
                            for cand_i in range(args.tpo_k):
                                if probe_terminated[pi, cand_i]:
                                    continue
                                ...
                                qpos_s, qvel_s, warm_s, time_s = (
                                    init_states[pi] if t == 0 else cand_states[pi][cand_i]
                                )
                                base_env.data.qpos[:] = qpos_s
                                base_env.data.qvel[:] = qvel_s
                                base_env.data.qacc_warmstart[:] = warm_s
                                base_env.data.time = time_s
                                act = cand_actions_np[env_i, cand_i] if t == 0 else next_actions_np[pi, cand_i]
                                probe_obs, probe_rew, probe_term, _, _ = base_env.step(act)
                                # RAW reward (base is raw-return), discounted into the n-step sum.
                                probe_disc_rew[pi, cand_i] += disc_t * probe_rew
                                ...
                                probe_norm_obs[pi, cand_i] = np.clip(norm_obs, -10.0, 10.0)
                                if probe_term:
                                    # Truncate the sum here; the gamma^n bootstrap is dropped
                                    # via term_probe in the scoring block.
                                    probe_terminated[pi, cand_i] = True
                                elif t < n_probe - 1:
                                    d = base_env.data
                                    cand_states[pi][cand_i] = (
                                        d.qpos.copy(), d.qvel.copy(), d.qacc_warmstart.copy(), d.time
                                    )
                        if t < n_probe - 1 and not probe_terminated.all():
                            # Intermediate probe actions: the policy's DETERMINISTIC mean
                            # (Beta mean a/(a+b); dist.mean generally), through the SAME
                            # z->action map + clip as the sampled candidates, on the frozen
                            # obs normalization. Batched over all (env, candidate) pairs:
                            # ONE trunk forward per probe step, consumes no RNG.
                            with torch.no_grad():
                                probe_obs_t = torch.as_tensor(
                                    probe_norm_obs.reshape(P * args.tpo_k, probe_obs_dim), device=device
                                )
                                probe_actor_feat, _ = agent._trunks(probe_obs_t)
                                probe_dist, probe_to_action, _ = agent._actor_dist(probe_actor_feat)
                                mean_z = probe_dist.mean
                                if args.actor_dist == "beta":
                                    mean_z = mean_z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                                next_actions_np = (
                                    probe_to_action(mean_z).cpu().numpy().reshape(P, args.tpo_k, -1)
                                )
                            next_actions_np = np.clip(next_actions_np, probe_action_low, probe_action_high)
                    # Restore every probed env to its pre-probe state so the REAL
                    # rollout step below matches an unprobed run exactly.
                    for pi in range(P):
                        base_env = probe_base_envs[int(probe_env_ids[pi])]
                        qpos_s, qvel_s, warm_s, time_s = init_states[pi]
                        base_env.data.qpos[:] = qpos_s
                        base_env.data.qvel[:] = qvel_s
                        base_env.data.qacc_warmstart[:] = warm_s
                        base_env.data.time = time_s
                    tpo_next_obs_np[step, probe_env_ids] = probe_norm_obs
                    tpo_rewards_np[step, probe_env_ids] = probe_disc_rew.astype(np.float32)
                    tpo_terms_np[step, probe_env_ids] = probe_terminated.astype(np.float32)
        ...
        with torch.no_grad():
            ...
            if tpo_enabled:
                ...
                if num_probed > 0:
                    ...
                    # Oracle score: n-step discounted raw probe rewards + gamma^n
                    # bootstrapped frozen value (term_probe=1 iff the probe terminated
                    # at ANY of its n steps => bootstrap dropped, sum already truncated).
                    scores = r_probe + (args.gamma ** args.tpo_probe_steps) * (1.0 - term_probe) * v_next  # (P, K)
