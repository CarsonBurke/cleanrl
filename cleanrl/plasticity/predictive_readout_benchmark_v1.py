"""Sequential CUDA-only unit-candidate predictive-readout benchmark, without model averaging.
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = PredictiveReadoutState(self.weight[rows], control=control)
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__predictive_readout_v1__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("predictive_readout_benchmark_v1.py", "predictive_readout_optimizer_v1.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "predictive_readout_v1",
                ...
                "readout_rule": "A+=old_memory*(h*live_displacement-g); B+=h*old_memory^2; allocation=argmin on[0,1] of .5*B*a^2-A*a. Then memory=sum(h*live_displacement-g)/sum(h). Unit candidate=allocation*memory. Exact current-example quadratic line search handles interference.",
