...
CONTROLS = ("coarse_first", "recheck", "fine_first", "coordinate", "block", "unvalidated", "shuffled")
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = PredictiveCommitmentState(self.weight[rows], control=control)
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__predictive_commitment_v5__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("predictive_commitment_benchmark_v5.py", "predictive_commitment_v5.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "predictive_commitment_v5",
                ...
                "control_semantics": {"coarse_first": "Validated aggregate takes precedence; admitted coordinates remain eligible until recent predictive covariance is nonpositive", "recheck": "Same aggregate precedence, but coordinate admission must re-pass its discovery threshold at every step", "fine_first": "Persistent coordinate admission takes precedence over aggregate", "coordinate": "Persistent fast coordinate admission only", "block": "Slow normalized aggregate only", "unvalidated": "Fast coordinate amplitude without admission", "shuffled": "Wrong-coordinate forecasts, with aggregate precedence and commitment"},
                "admission_retirement": "A failed renewal of strong discovery evidence is not counterevidence. Keep an admitted coordinate while A>0; the fitted amplitude still updates every observation. The block still uses its cumulative admission boundary.",
