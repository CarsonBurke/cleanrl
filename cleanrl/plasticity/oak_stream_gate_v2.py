"""Oak stream v2: v1 grids shifted one decade down (v1 bests sat at the low-LR edge), IDBD alpha0 1e-3, decay {1, 3}.
...
class Args:
    ...
    steps: int = 200000
    window: int = 1500
    sgd_lrs: tuple[float, ...] = (0.0003, 0.001, 0.003)
    adam_lrs: tuple[float, ...] = (0.0001, 0.0003, 0.001)
    idbd_thetas: tuple[float, ...] = (0.001, 0.01, 0.1)
    idbd_alpha0: float = 0.001
    gate_lrs: tuple[float, ...] = (0.0001, 0.0003, 0.001)
    gate_decays: tuple[float, ...] = (1.0, 3.0)
...
def main():
    ...
    root = Path(a.output or f"runs/OakStreamGate__v2__{a.seed}__{time.time_ns()}")
