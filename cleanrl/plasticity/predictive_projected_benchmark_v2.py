"""Sequential CUDA-only observation-constrained predictive-readout benchmark, without model averaging.
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = ProjectedReadoutState(self.weight[rows], control=control)
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__predictive_projected_v2__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("predictive_projected_benchmark_v2.py", "predictive_projected_readout_v2.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "predictive_projected_v2",
                ...
                "readout_rule": "A+=old_memory*(h*live_displacement-g); B+=h*old_memory^2; allocation=argmin on[0,1] of .5*B*a^2-A*a. Then memory=sum(h*live_displacement-g)/sum(h). Unit candidate=allocation*memory. Candidate displacement is first projected onto the observed example Jacobian; exact quadratic line search then handles interference. Unobserved nullspace changes are never written.",
                "control_semantics": {"validated": "Only later outcomes validate older memory; rejected memory is retracted from live parameters", "open": "Allocation always1; memory is written without independent validation", "shuffled": "Old memory used for validation belongs to an adjacent coordinate"},
                "controller_grid": "None: one fixed unit-candidate configuration per mechanism; no globally tuned learning rate or validation threshold.",
                ...
                "limitations": ["One seed and linear models; no universal optimizer-superiority claim.",
                    ...
                    "Validation allocation is constrained to[0,1] by definition. New memory cannot validate itself; cold-start behavior and full noise movement are measured.",
