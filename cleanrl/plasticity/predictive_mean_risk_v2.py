...
and switching rows share exactly the same independently trained experts.

SparsePosterior remains a factorized assumed-density approximation; residual
EMAs conflate noise and model error. Neither objective guarantees sparse
recovery, calibrated uncertainty, or transfer to stocks or neural training.
No clean targets, support information, oracle noise, or evaluation data enter
this model. Callers own compilation, CUDA graph capture, and finite input
validation; all update inputs must be FP32 CUDA tensors on the model device.
"""
...
class PredictiveMeanRisk:
    ...
    def state_tensors(self) -> list[torch.Tensor]:
        ...
        return [*self.filter.state_tensors(), self.log_weights, self.noise, self.energy]
    ...
    def update(self, x, y) -> torch.Tensor:
        ...
        # R is still the previous residual EMA, not the current squared error.
        ...
        self.noise.lerp_(residual_squared, self.noise_rate)
        self.energy.lerp_(y.square(), self.noise_rate)
        return prediction
