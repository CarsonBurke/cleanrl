"""Predictive Commitment v5: validate direction separately from write amplitude.
...
Discovery and retirement differ: once admitted, a coordinate remains eligible while
its recent predictive covariance is positive; its fitted amplitude can still fall.
A validated block readout takes precedence: coordinate evidence against zero does
not establish improvement over an already predictive aggregate.
...
class PredictiveCommitmentState:
    ...
    slow row-wide direction. Discovery needs the significance boundary; retirement
    needs nonpositive recent predictive covariance. recheck ablates that hysteresis,
    while fine_first ablates validated-block precedence. Each forecast is scored on the next observation,
    ...
    def __init__(self, weight, *, control="coarse_first"):
    ...
    def step(self, weight, grad, curvature):
        ...
        discovered = self.validation_target > (2 * self.log_tests * self.validation_square).sqrt()
        coordinate_admitted = (discovered if self.control == "recheck" else
                               discovered | (self.coordinate_committed & (self.validation_target > 0)))
