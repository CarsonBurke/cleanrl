"""Sequential CUDA-only hierarchically admitted predictive-readout benchmark, without model averaging.
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = PredictiveAdmissionState(self.weight[rows], control=control)
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__predictive_admission_v3__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("predictive_admission_benchmark_v3.py", "predictive_admission_v3.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "predictive_admission_v3",
                ...
                "readout_rule": "Old memory produces coordinate and row-wide forecasts. Accumulate forecast*partial_target A, squared design B, and squared product V. Admission needs A>sqrt(2*V*log(2*(D+1))). After admission, use the fitted A/B on[0,1], NOT a shrunken lower bound. Coordinate admission overrides block allocation. Candidate writing is projected only through admitted or previously written coordinates.",
                "control_semantics": {"hierarchical": "Coordinate or row-wide admission supports sparse and distributed structure", "coordinate": "Only coordinate admission", "block": "Only row-wide admission", "unvalidated": "Coordinate readout without admission boundary", "shuffled": "Wrong-coordinate old memory used in both forecast scores"},
                "controller_grid": "None: one unit-candidate configuration per mechanism; boundary uses only the number of tested memories, with no LR or threshold sweep.",
                "statistical_caution": "The self-normalized boundary is a diagnostic heuristic, not a proven anytime p-value or false-discovery guarantee under repeated testing, serial dependence and changing forecasts.",
                ...
                "limitations": ["One seed and linear models; no universal optimizer-superiority claim.",
                    ...
                    "Validation allocation is constrained to[0,1] by definition. Evidence uses older forecasts. Lifetime evidence may delay reopening after genuine change; full regime tests are required.",
