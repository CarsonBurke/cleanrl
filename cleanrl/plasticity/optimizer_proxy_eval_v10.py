"""v10 protocol: identical to optimizer_proxy_eval_v9 except that it drives optimizer_proxy_model_v10
(adds look_polar) and names its runs OptimizerProxy__v10_*. See the v9 docstring below for the protocol.
...
from cleanrl.plasticity import optimizer_proxy_model_v10 as optimizer
...
class Args:
    ...
    test: int = 8192
    output: str = ""
    plan: str = "benchmarks/plasticity/optimizer_proxy_v10_plan.json"
...
def main():
    ...
    root = Path(args.output or f"runs/OptimizerProxy__v10_{args.scenario}{tag}__{args.seed}__{time.time_ns()}")
    ...
    sources = [Path(__file__), Path(optimizer.__file__), Path(optimizer.base.__file__),
               ...
               Path(model.__file__), Path(reference.__file__),
