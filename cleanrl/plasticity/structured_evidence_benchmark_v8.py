...
from cleanrl.plasticity.structured_evidence_v8 import StructuredEvidenceState
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = StructuredEvidenceState(self.weight[rows], control=control)
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__structured_evidence_v8__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("structured_evidence_benchmark_v8.py", "structured_evidence_v8.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "structured_evidence_v8",
                ...
                "block_horizon": "Lifetime shared-coefficient and learned-contrast regression. Old contrast direction is scored before its memory is updated.",
                "control_semantics": {"structured": "Shared, coordinate and validated contrast components", "renewed": "Same with renewable coordinate regression", "no_shared": "Remove shared coefficient structure", "no_dense": "Remove learned dense contrast", "no_gate": "Remove prequential admission", "misaligned": "Wrong-coordinate gradients and curvature; constant shared component is permutation invariant"},
                ...
                "limitations": ["One seed and linear models; no universal optimizer-superiority claim.",
                    ...
                    "Signed amplitudes are learned without rounding. Shared coefficients favor constant-pattern structure; alternating-sign controls expose this inductive bias. Lifetime statistics can lag regime changes.",
