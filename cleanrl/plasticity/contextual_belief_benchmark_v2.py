"""Sequential CUDA-only per-parameter predictive-belief benchmark.

Candidates receive the observed residual, analytic prediction Jacobian, and
local target-free context. Clean synthetic targets and support are evaluator-only.
First-half observed-loss selections are frozen before second-half evaluation;
nulls use their signal stream's selected hyperparameters. All controls complete
the full horizon, including underperforming ones. No network layers are added.
"""
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = ContextualBeliefState(self.weight[rows], control=control)
        ...
        def step(x, target_rows, clean_rows):
            ...
            for control, state in self.controllers.items():
                ...
                context = torch.stack((presynaptic_context.expand_as(candidate_weight),
                                       prediction[rows, None].tanh().expand_as(candidate_weight)), dim=-1)
                state.step(candidate_weight, grad[rows], jacobian, context, residual[rows])
    ...
    def results(self, observed, elapsed, final_support=None):
        ...
        for control, state in self.controllers.items():
            ...
            for label, accumulator in (
                ("calibration_nll", state.nll_sum),
                ("calibration_standardized_square", state.standardized_square_sum),
                ("mean_predicted_variance", state.predicted_variance_sum),
                ("mean_effective_gain", state.gain_sum),
                ("mean_bias_probability", state.bias_probability_sum),
                ("mean_change_probability", state.change_probability_sum),
            ):
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__contextual_belief_v2__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("contextual_belief_benchmark_v2.py", "contextual_belief_v2.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "contextual_belief_v2",
                "optimizer_interface": "step(weight, gradient, prediction_jacobian, target_free_context, observed_residual). Gradient is residual times Jacobian. No clean labels, support, hidden targets, Hessian, paired gradients, or extra network layers.",
                "calibration": "Per-parameter predictive-belief model of the observed scalar downstream residual, with calibrated-zero, persistent conditional-error, and changed-error hypotheses. Gaussian coefficient uncertainty describes predictive error, not weights or hidden-neuron targets. Continuous hypothesis probabilities and a learned revision hazard are updated by approximate moment-matched filtering. Predictive scores and all gains use beliefs before the current outcome updates calibration.",
                "context": "Two separate target-free features: tanh(presynaptic activation) and tanh(postsynaptic preactivation). Linear output preactivation is the current prediction; the stock intercept's first context feature is exactly one. Context never contains the current residual or target.",
                "plasticity": "State-dependent bounded Bayes-risk gain M/(M+R), not inverse variance, a gradient predictor, or a learned learning rate. M is the inferred correctable predictive-error second moment and R the estimated observation-noise variance. Normalized LMS write: -gain_i*gradient_i/sum(Jacobian_i^2); a zero Jacobian row makes no writes. The linearized output gain is a Jacobian-energy-weighted average, not a sum of predictive confidence. No candidate reference scales or exact historical gradient credit.",
                "control_semantics": {
                    "belief": "Full predictive-belief filter with conditional error, revision, and epistemic uncertainty in the gain",
                    "unconditional": "Zero both context features, retaining per-parameter unconditional inference",
                    "no_change": "Disable the changed-error hypothesis while retaining conditional inference",
                    "no_epistemic": "Learn the same beliefs but omit posterior coefficient variance from the gain",
                    "noise_only": "Use gain 1/(1+R), bypassing mean-belief evidence; otherwise retain common inference"},
                "initialization": "Fixed common coefficient, covariance, conditional-variance, hypothesis, and revision-hazard priors defined in contextual_belief_v2.py. No candidate learning-rate argument, reference scale, or candidate sweep; network weights retain original zero initialization.",
                "controller_grid": "No LR grid for candidates; five causal controls. Baselines retain fifteen rates.",
                "prediction_registry": "runs/contextual_belief_v2_preregistration.json",
                "prediction_registry_sha256": hashlib.sha256(Path("runs/contextual_belief_v2_preregistration.json").read_bytes()).hexdigest(),
                "cost": "One analytic prediction/Jacobian evaluation per observation; 21 FP64 optimizer-state scalars per parameter (168 bytes) and seven FP64 diagnostic counters per model (56 bytes). Network weights remain FP32. Covariance has shape (models, parameters, 3, 3). Experimental cost, not LLM-ready. No gradient accumulation, model averaging, or replay.",
                "calibration_metric_caution": "Calibration NLL, standardized residual square, predicted variance, effective gain, and bias/change probabilities average over active parameters per example, then active observations on each learner's own trajectory. Counters divided by observation_count are None at zero count. They are not common-state cross-method rankings or evidence of model-learning success by themselves.",
                ...
                "limitations": ["One seed; repeated development streams are not an untouched confirmatory holdout.",
                    ...
                    "Gaussian coefficient uncertainty is in predictive error, not weights, hidden-neuron targets, or a general RL/LLM likelihood.",
                    "The per-parameter filters reuse a shared scalar residual; their evidence is dependent, not a jointly exact posterior.",
                    "Local context is deliberately limited; conditional variance can absorb model misspecification. Lower NLL alone does not demonstrate improved learning.",
                    "Hypothesis filtering is approximately moment-matched. Learned revision hazards and continuous belief probabilities do not recover exact historical gradient credit.",
                    "State-dependent gain and shared Jacobian normalization do not guarantee arbitrary nonlinear finite-step safety or parameterization invariance.",
                    "Conditional-variance adaptation retains cumulative accumulator state, which can be stale under drift. Fixed priors remain design choices, not a parameter-free claim.",
