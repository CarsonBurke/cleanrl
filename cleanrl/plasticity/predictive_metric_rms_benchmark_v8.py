"""Full sequential CUDA proxy for standalone Predictive Metric RMS v8.

Original network, data, baselines, frozen half-selection and evaluator retained.
Candidate update has no underlying Adam; v4 is an explicit standalone control.
"""
...
from cleanrl.plasticity.predictive_metric_rms_v8 import CONTROLS, PredictiveMetricState
...
from cleanrl.plasticity.predictive_transport_benchmark_v2 import (
    best_row, comparisons, log_result, named_buffers, save_json, score_metrics,
)
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = (PredictiveNeedState(self.weight[rows], control="predictive_need")
                     if control == "v4_reference" else
                     PredictiveMetricState(self.weight[rows], control=control))
    ...
    def results(self, observed, elapsed, final_support=None):
        ...
        for control, state in self.controllers.items():
            ...
            for label, accumulator in (
                ("mean_effective_gain" if control == "v4_reference" else "mean_linearized_output_fraction", state.gain_sum),
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__predictive_metric_rms_v8__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("predictive_metric_rms_benchmark_v8.py", "predictive_metric_rms_v8.py",
                    "predictive_need_v4.py", "predictive_transport_benchmark_v2.py",
                    "noisy_stream_diagnostic.py", "stock_stream.py")
    metadata = {
        "arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
        ...
        "run_label": "predictive_metric_rms_v8",
        "optimizer_interface": "Original standalone scalar-loss interface: weight, ordinary gradient, Jacobian, target-free local feature, observed target, actual residual and pre-update prediction. Candidate never calls Adam; AdamW/SGD are separate baseline rows only.",
        "evidence_model": "Frozen v4 centered target-function moments, g=n conditional model and composite empirical-Bayes inclusion. Inheritance reuses statistics and the in-place state installer, NOT v4's parameter transition. All target-dependent gain statistics precede current target assimilation.",
        "curvature": "The square of the stored RMS Jacobian is the cumulative mean H_i, including the current target-independent Jacobian. Stable hypot updates and J/RMS ratios avoid explicit overflowing squares or inverse curvature. This is function-space geometry, not a gradient-noise second moment or a probability about weight values.",
        "geometry": "D_i=p_i/H_i on identified coordinates, B=sum D_i*J_i^2. Let c=max active p_i. Primary gain_i=c*q*D_i/(1+B). The unscaled direction solves min_delta .5*(e+J*delta)^2+.5*sum delta_i^2/D_i on supported coordinates; c*q shrinks this proximal step. Expected diagonal curvature is an approximation, not full function covariance.",
        "risk": "Mi=(prediction-local_target_mean_i)^2+Ui, Ri=local conditional observation variance. With energy weights D_i*J_i^2, q=sum energy_i*Mi / sum energy_i*(Mi+Ri). This is the scalar risk optimum for the stated working mixture of local H1 models, not a joint calibrated posterior. The extra confidence ceiling c is an explicit conservative heuristic.",
        "controls": {
            "metric": "Primary standalone curvature metric, pooled predictive-risk fraction and confidence ceiling",
            "local_need": "Same metric and ceiling, coordinate-local v4 risk fractions instead of pooled moments",
            "uncapped": "Same metric and pooled risk, remove the ceiling c to test cancellation of small inclusion probabilities",
            "no_need": "Same metric and confidence ceiling, q=1 to isolate predictive-risk noise refusal",
            "v4_reference": "Exact frozen PredictiveNeedState(control=predictive_need), including original unit ridge; separate reference rows"
        },
        "state_cost": "Candidate32 bytes/parameter plus72 bytes/model: v4's three FP64 parameter moments and nine scalar statistics, plus one FP64 RMS-Jacobian vector. V4 reference retains24 bytes/parameter plus72 bytes/model. No momentum, Adam state, free base LR, schedule, gradient table, extra inference network, target substitution or replay in candidate.",
        "prediction_registry": "runs/predictive_metric_rms_v8_preregistration.json",
        "prediction_registry_sha256": hashlib.sha256(Path("runs/predictive_metric_rms_v8_preregistration.json").read_bytes()).hexdigest(),
        "selection_protocol": "Unchanged15-rate SGD/AdamW grids. Freeze each method/stream's first-half noisy-MSE selection before second-half targets. Null uses matched signal-selected configuration. Primary remains metric irrespective of ablation outcomes.",
        "measurement": "Original exact clean risk, full-stream prequential clean error, signal coefficient, nuisance/null power and actual useful/noise parameter writes. Evidence counters retain v4 definitions. Candidate mean_linearized_output_fraction is the average bounded output correction fraction; v4_reference alone retains its original mean_effective_gain units. Numerical checks cover local update geometry and compiled execution, not nonlinear finite-step guarantees; proxy itself is the unchanged original linear predictor.",
        "primary_objective": "Full20k seed1 stationary sparse+matched-noise proxy. Test that removing unit-metric attenuation improves resident signal memory without buying the result through nuisance diffusion or noisy large writes. No claim of standalone RL qualification.",
        "limitations": [
            "Requires scalar-output prediction Jacobians and local features, not a general gradient-only/vector-loss interface.",
            "Cumulative target evidence still cannot reliably revise unknown regime changes; no phase boundary reaches the optimizer.",
            "Diagonal curvature misses cross-coordinate correlations, including duplicated features; correction is bounded, not exactly duplication invariant.",
            "Max inclusion is a conservative confidence ceiling, not a joint probability or multiplicity guarantee.",
            "Local univariate predictive distributions may miss interactions and dense weak effects. Constant features cannot earn association evidence.",
            "Only linearized output contraction is guaranteed; arbitrary nonlinear finite steps are unqualified.",
            "A tiny current Jacobian relative to historical curvature cannot cause inverse-Jacobian blowup; wholly unseen future scales are not bounded by this statement.",
            "One reused development seed; no independent-seed statistical confidence. Other inherited CLI protocols are not automatically registered or qualified."
        ]}
