"""Arithmetic-fidelity iteration of the v6 proxy; unchanged optimizer algorithms.

Hypothesis: hidden-matrix direction geometry, rather than additional momentum
transport, improves adaptation. Compare polar-normalized Nesterov momentum with
an exactly Frobenius-matched control; biases and the output head use AdamW. Tune
AdamW equally; choose on noisy validation, lock all choices before test scoring,
and retain negative results. Not an RL benchmark or a guarantee that synthetic
rankings transfer to MuJoCo.
"""
...
from cleanrl.plasticity import optimizer_proxy_model_v6 as optimizer
...
class Args:
    ...
    test: int = 8192
    output: str = ""
    plan: str = "benchmarks/plasticity/optimizer_proxy_v7_plan.json"
...
class RoundLearner:
    ...
    def compute(self):
        ...
        return optimizer.transition(self.weights, self.previous, self.m, self.v,
                                    self.step, grads, corrections, lr, beta1, beta2, decay,
                                    self.method, self.scenario["epochs"])
    ...
    def capture(self):
        ...
        try:
            prepare = torch.compile(self.prepare, fullgraph=True, options={"triton.cudagraphs": False, "eager_numerics.division_rounding": True, "eager_numerics.use_pytorch_libdevice": True})
            compute = torch.compile(self.compute, fullgraph=True, options={"triton.cudagraphs": False, "eager_numerics.division_rounding": True, "eager_numerics.use_pytorch_libdevice": True})
...
def retain_selected_checkpoints(root, result, generated_checkpoints):
            ...
            if selections:
                ...
                for name, choice in selections:
                    ...
                    state = {key: [tensor[selected].clone() for tensor in checkpoint[key]]
                             for key in ("weights", "previous", "m", "v")}
    ...
    result["checkpoint_retention"] = "locked selections only; full candidate curves and invalid masks retained"
...
def main():
    ...
    grid = (plan["configurations"] if "configurations" in plan else
            [dict(zip(keys, values)) for values in itertools.product(*(plan["grid"][key] for key in keys))])
    families = plan.get("scenario_families", {}).get(args.scenario, plan["families"])
    root = Path(args.output or f"runs/OptimizerProxy__v7_{args.scenario}__{args.seed}__{time.time_ns()}")
    root.mkdir(parents=True, exist_ok=False)
    project = Path(__file__).resolve().parents[2]
    sources = [Path(__file__), Path(model.__file__), Path(optimizer.base.__file__),
               Path(optimizer.__file__), Path(reference.__file__),
    ...
    result = {"args": asdict(args), "scenario": scenario, "grid": grid, "status": "training",
              ...
              "deployment_gradient_diagnostic_semantics": {
                  ...
                  "applied_step": "(previous weights - current weights) / learning rate; includes matrix weight decay",
              },
    ...
    try:
        ...
        for name, choice in result["selections"].items():
            ...
            for index, curve in enumerate(result["methods"][choice["source_method"]]["curves"]):
                ...
                applied_step = [(old - current) / choice["config"]["lr"]
                                for old, current in zip(previous, weights)]
