"""Full batch-one nonlinear parity experiment for contextual precision.
...
from cleanrl.plasticity.contextual_precision_benchmark_v1 import (
...
def local_context(weight, x, prediction):
    """Flatten target-free synaptic contexts in the predictor's weight order."""
    ...
    return torch.cat((
        (hidden_activation[:, :, None] * x.tanh()[None, None, :]).flatten(1),
        hidden_activation,
        hidden_activation.tanh() * output_context,
        output_context,
    ), dim=1)
...
class Experiment:
    def __init__(self, args, initial, reference, configs, groups):
        ...
        self.controllers = {
            name: ContextualPrecisionState(self.weight[groups[name]], control=name, reference=reference)
        ...
        for name, state in self.controllers.items():
            ...
            for value in state.buffers().values():
                ...
                if value.ndim:
                    ...
                    finite[rows] &= valid
            values = {
                "mean_step_size": state.log_step.exp().mean(1).cpu().tolist(),
                "baseline_context_variance": state.logvar_bias.exp().mean(1).cpu().tolist(),
...
def main():
    ...
    directory = Path("runs") / f"nonlinear__contextual_precision_v1__1__{stamp}"
    directory.mkdir(parents=True)
    experiment = Experiment(args, initial, reference, configs, groups)
    source_names = ("contextual_precision_v1.py", "contextual_precision_nonlinear_v1.py", "contextual_precision_benchmark_v1.py")
    metadata = {
        "arguments": vars(args), "run_stamp": stamp, "run_label": "contextual_precision_v1", "task": "nonlinear parity",
        ...
        "cost": "One autograd loss-gradient evaluation for every learner; an extra direct prediction-Jacobian pass and local-context activation reconstruction only for candidates. No Hessian supplied. Compiled CUDA-graph execution; FP32 learner state and FP64 score sums.",
        ...
        "contexts": "Input weights tanh(input)*tanh(hidden preactivation); hidden biases tanh(hidden preactivation); output weights tanh(hidden activation)*tanh(output preactivation); output bias tanh(output preactivation). No target enters context.",
        "calibration": "Per-parameter conditional mean/variance of the downstream scalar predictive residual; not fictitious hidden-neuron supervision or a general RL/LLM likelihood. Statistical coefficients are optimizer metadata, not model layers or outputs. Diagnostics are on each learner's own trajectory, not common-state rankings; baseline_context_variance is exp(logvar_bias) at context zero.",
    ...
    try:
        ...
        torch.save({
            ...
            "reference": reference.cpu(), "frozen_selection": selection,
