...
from cleanrl.plasticity.predictive_transport_v1 import PredictiveTransportState


CHUNK = 100
LR_GRID = tuple(float(value) for value in np.logspace(-8, -1, 15))
CONTROLS = ("predictive", "fixed_trust", "no_transport", "fixed_write", "raw_gradient", "one_step_credit", "miscredited")
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        self.forecast_squared_error = torch.zeros(n, dtype=torch.float64, device=device)
        self.zero_forecast_squared_error = torch.zeros_like(self.forecast_squared_error)
        self.future_write_loss_change = torch.zeros_like(self.forecast_squared_error)
        self.controllers = {}
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = PredictiveTransportState(self.weight[rows], control=control)
        ...
        self.states.extend((self.forecast_squared_error, self.zero_forecast_squared_error, self.future_write_loss_change))
        ...
        def step(x, target_rows, clean_rows):
            ...
            for control, state in self.controllers.items():
                rows = groups[control]
                # Same new observation, evaluated at the previous parameters.
                # Only the paired gradients reach the optimizer, never x/y/h.
                old_prediction = (state.previous_weight * x).sum(1)
                old_residual = old_prediction - targets[rows]
                previous_gradient = old_residual[:, None] * x
                error = previous_gradient - state.forecast
                self.forecast_squared_error[rows].add_(error.double().square().sum(1))
                self.zero_forecast_squared_error[rows].add_(previous_gradient.double().square().sum(1))
                self.future_write_loss_change[rows].add_(0.5 * (residual[rows].double().square() - old_residual.double().square()))
                state.step(self.weight[rows], grad[rows], previous_gradient)
            sgd = groups["sgd"]
    ...
    def results(self, observed, elapsed, final_support=None):
        ...
        for control, state in self.controllers.items():
            ...
            statistics = {}
            for label, value in (("forecast", state.forecast), ("assimilation", state.trust_logit.sigmoid()),
                                 ("local_write_gain", state.write_log_gain.exp()),
                                 ("block_write_gain", state.group_log_gain.exp())):
        ...
        forecast_error = self.forecast_squared_error.cpu().tolist()
        zero_forecast_error = self.zero_forecast_squared_error.cpu().tolist()
        write_effect = self.future_write_loss_change.cpu().tolist()
        rows = []
        for index, config in enumerate(self.configs):
            ...
            if config["method"] in CONTROLS:
                row["gradient_forecast_MSE_ratio_to_zero"] = (forecast_error[index] / zero_forecast_error[index] if zero_forecast_error[index] else None)
                row["mean_next_example_write_loss_change"] = write_effect[index] / observed if observed else None
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__predictive_transport_v1__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("predictive_transport_benchmark_v1.py", "predictive_transport_v1.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "predictive_transport_v1",
                "optimizer_interface": "First-order paired gradients: step(weight, gradient_on_current_example_at_current_parameters, gradient_on_THE_SAME_example_at_previous_parameters). No supplied Hessian, curvature, feature vectors, clean labels, or support.",
                "predictor": "Same-example parameter-motion correction plus learned sigmoid innovation assimilation. The variance normalizer uses learned assimilation and participates only when a gradient is nonzero; it is a scale estimate, not calibrated uncertainty.",
                "write_credit": "Separate future-loss credit for rescaling current pending parameter displacements. Local anchors survive until that parameter participates again. Block credit uses the last global update. Neither is claimed to be the exact historical multi-step hypergradient.",
                "control_semantics": {"predictive": "Full predictive transport and future-write credit", "fixed_trust": "Assimilation fixed at .5", "no_transport": "Omit same-example parameter-motion correction", "fixed_write": "Write multipliers fixed at one", "raw_gradient": "Write from the raw gradient instead of its forecast", "one_step_credit": "Credit only the last global displacement, not pending coordinate changes", "miscredited": "Assign innovation and write credit to the wrong coordinates"},
                "initialization": "Assimilation .5; local/block write multiplier one; reference parameter scale 1/sqrt(parameter count) for these linear models. Meta step .1 fixed before runs, no sweep. Not parameter-free.",
                "controller_grid": "No LR grid for the candidate; causal ablations only.",
                "prediction_registry": "runs/predictive_transport_v1_preregistration.json",
                "prediction_registry_sha256": hashlib.sha256(Path("runs/predictive_transport_v1_preregistration.json").read_bytes()).hexdigest(),
                "cost": "Two gradient evaluations per new example for candidate methods, one for baselines. Linear gradients are evaluated analytically; separate nonlinear experiment uses autograd. No gradient accumulation.",
                "forecast_metric_caution": "Forecast MSE is relative to a zero forecast on each learner's own trajectory at previous parameters. It is not itself task improvement, nor a common-parameter cross-method comparison.",
                ...
                "limitations": ["One seed; repeated development streams are not an untouched confirmatory holdout.",
                    "This is not a standard one-backward gradient-only optimizer; it requires paired same-example gradients.",
                    "No exact long-delay credit or context retrieval mechanism. No parameterization-invariance guarantee.",
                    "Stochastic layers require identical paired randomness and careful mutable-buffer handling; current experiments are deterministic networks.",
                    "Meta step and reference-scale initialization remain design constants; no claim of a parameter-free rule.",
                    "Forecast accuracy can improve while task loss worsens; both are measured separately.",
