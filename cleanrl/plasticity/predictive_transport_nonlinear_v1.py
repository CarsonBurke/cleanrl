...
from cleanrl.plasticity.predictive_transport_v1 import CONTROLS, PredictiveTransportState
from cleanrl.plasticity.predictive_transport_benchmark_v1 import (
    best_row, comparisons, configurations, json_safe, log_result, save_json, score_metrics,
)
...
class Experiment:
    def __init__(self, args, initial, reference, configs, groups):
        ...
        self.states = [self.weight, self.m, self.v, self.t, self.count, self.error, self.excess,
        ...
        def step(x, target_rows, clean_rows):
            ...
            for name, state in self.controllers.items():
                ...
                state.step(self.weight[rows], gradient[rows], g_old)
        ...
        self.compiled = torch.compile(step, fullgraph=True, dynamic=False, options={"triton.cudagraphs": False})
...
def main():
    ...
    directory = Path("runs") / f"nonlinear__predictive_transport_v1__1__{stamp}"
    directory.mkdir(parents=True)
    experiment = Experiment(args, initial, reference, configs, groups)
    source_names = ("predictive_transport_v1.py", "predictive_transport_nonlinear_v1.py", "predictive_transport_benchmark_v1.py")
