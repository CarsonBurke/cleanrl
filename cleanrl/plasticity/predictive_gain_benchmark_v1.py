"""Sequential CUDA-only fixed-unit predictive-gain benchmark, without model averaging.
...
observed loss and frozen before the second half is consumed. New controllers
start at unit gain with no LR grid; future gradients learn predictive plasticity. All controls
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = PredictiveGainState(self.weight[rows], control=control)
    ...
    def results(self, observed, elapsed, final_support=None):
        ...
        for control, state in self.controllers.items():
            ...
            statistics = {}
            for label, value in (("gain", state.log_gain.exp()), ("log_gain", state.log_gain),
                                 ("trace", state.trace), ("credit_scale", state.credit_scale)):
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__predictive_gain_v1__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("predictive_gain_benchmark_v1.py", "predictive_gain_optimizer_v1.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "predictive_gain_v1",
                "optimizer_interface": "Curvature-assisted step(weight, grad, diagonal_curvature); no targets, support, clean labels, posterior, covariance matrix or model ensemble enters the optimizer. Exact linear half-square diagonal curvature is supplied.",
                "gain_rule": "All coordinates start at alpha=1. Older eligibility receives credit -g*trace. Log gain moves by fixed .1 times normalized credit. Sample-curvature stability enforces sum(alpha*h)<=1, reducing only participating gains.",
                "control_semantics": {"predictive": "IDBD/AutoStep-inspired magnitude-normalized causal credit", "sign": "credit sign only, testing reopening after old large credit", "frozen": "no credit adaptation; only sample stability normalization", "shuffled": "eligibility belongs to the adjacent coordinate, negative credit-assignment control"},
                "controller_grid": "None: one prespecified unit-initial-gain configuration per mechanism; no globally tuned learning rate.",
                ...
                "limitations": ["One seed and linear models; no universal optimizer-superiority claim.",
                    "IDBD/AutoStep-inspired credit control is an established foundation; no novelty theorem claimed. Eligibility omits cross-Hessian and normalization derivatives.",
                    "Unit initial gain can absorb noise before credit develops; all cold-start movement and losses remain reported.",
