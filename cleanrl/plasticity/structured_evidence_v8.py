"""Structured Evidence v8: keep estimation separate from prequential admission.

v7's refitted residual variance could certify a few accidentally similar noisy
samples. Score OLD directions on NEW partial targets instead. That score decides
whether to write, never the fitted amplitude: direct regression still supplies the
full signed coefficient. Shared structure, learned contrasts, and coordinate
signals coexist in one live model; every update consumes one observation.
"""
import math
...
class StructuredEvidenceState:
    def __init__(self, weight, *, control="structured"):
        if weight.device.type != "cuda" or weight.dtype != torch.float32:
            raise ValueError("structured evidence requires CUDA float32 parameters")
        ...
        self.common = tuple(torch.zeros_like(self.common_weight) for _ in range(4))
        # For the old learned contrast, regression numerator is also its score.
        self.dense = tuple(torch.zeros_like(self.common_weight) for _ in range(3))
