"""Sequential CUDA-only dual-geometry benchmark, without model averaging.
...
observed loss and frozen before the second half is consumed. Lifetime dual
accumulation is fixed; controls isolate the diagonal metric used to solve it. All controls
...
CONTROLS = ("shared", "rms", "variance", "strong")
...
def configurations(streams):
    ...
    for method in ("sgd", "adamw", *CONTROLS):
        ...
        if method in ("sgd", "adamw"):
            ...
            settings = [{"lr": lr} for lr in TRACKING_LRS]
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = DualGeometryState(self.weight[rows], lr=self.lr[rows], control=control)
    ...
    def results(self, observed, elapsed, final_support=None):
        ...
        for control, state in self.controllers.items():
            ...
            statistics = {}
            from cleanrl.plasticity.dual_geometry_optimizer_v2 import POWERS
            squares = state.square_sum.detach().double()
            shared = squares.mean(-1, keepdim=True)
            denominator = shared.sqrt() * (squares / shared.clamp_min(1e-300)).pow(POWERS[control])
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__dual_geometry_v2__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("dual_geometry_benchmark_v2.py", "dual_geometry_optimizer_v2.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "dual_rule": "G=sum gradients, Q=sum gradient squares; q=coordinate mean of Q; D=sqrt(q)*(Q/q)^power; weight=anchor-lr*G/D.",
                "control_semantics": {"shared": "power0, shared scale", "rms": "power0.5, exact v1 lifetime coordinate RMS control", "variance": "power1, relative second-moment geometry", "strong": "power1.5, stronger coordinate normalization"},
                "controller_grid": {"lr": TRACKING_LRS, "memory": "lifetime for all controls"},
                ...
                "limitations": ["One seed and linear models; no universal optimizer-superiority claim.",
                    "Diagonal metrics and dual averaging are established optimization forms; no novelty theorem claimed.",
                    "Large metric powers amplify rare coordinates; full cold-start behavior is measured without clipping.",
                    ...
                    "Four geometry controls each use nine scales; SGD/AdamW use fifteen learning rates.",
