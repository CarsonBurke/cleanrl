"""Direct signed-return prediction; full chronological CUDA streams, not volatility accuracy.
...
from cleanrl.plasticity.panel_return_representation_v5 import GROUPS, LRS, Learner, configurations
...
PRIMARY = "memory_aux"
HORIZONS = (1, 4, 16, 32)
SIGN_NAMESPACE = "panel_return_representation_v5_signed_label_rademacher_seed1"
...
def persist_prefix_locks(root, raw, result):
    ...
    proof = {"version": 5, "source_sha256": result["source_sha256"], "data_identity": identity,
...
def load_reference(path, hashes, configs):
    ...
    if (source.get("version"), source.get("stage"), source.get("view"), source.get("status")) != (5, "development", "real", "completed"):
    ...
    if (proof.get("version") != 5 or proof.get("source_sha256") != hashes or proof.get("data_identity") != identity
...
def main():
    ...
    result = {"version": 5, "stage": a.stage, "view": a.view, "status": "initializing", "args": asdict(a),
