"""Sequential CUDA-only state-conditioned residual-calibration benchmark.
...
from cleanrl.plasticity.contextual_precision_v1 import CONTROLS, ContextualPrecisionState
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = ContextualPrecisionState(self.weight[rows], control=control)
        ...
        def step(x, target_rows, clean_rows):
            ...
            for control, state in self.controllers.items():
                ...
                context = presynaptic_context * prediction[rows, None].tanh()
    ...
    def results(self, observed, elapsed, final_support=None):
        ...
        for control, state in self.controllers.items():
            ...
            statistics = {
                "mean_step_size": state.log_step.detach().double().exp().mean(1).cpu().tolist(),
                "baseline_context_variance": state.logvar_bias.detach().double().exp().mean(1).cpu().tolist(),
            }
            count = state.observation_count.detach().double()
            for label, accumulator in (
                ...
                average = torch.where(count > 0, accumulator.double() / count, float("nan"))
                statistics[label] = average.cpu().tolist()
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__contextual_precision_v1__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("contextual_precision_benchmark_v1.py", "contextual_precision_v1.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "contextual_precision_v1",
                ...
                "calibration": "Per-parameter Gaussian observed scalar residual: mean=m0+m1*c, variance=exp(v0+v1*c). Predictive scores and precision use coefficients before the current observation updates them. Four coefficient arrays are optimizer metadata, not model outputs or a parameter posterior.",
                "context": "tanh(presynaptic activation)*tanh(postsynaptic preactivation). Linear output preactivation is the current prediction; the stock intercept uses presynaptic activation one. Context never contains the current residual or target.",
                "plasticity": "Learned per-parameter log steps receive local sensitivity-based AdaGrad credit. A shared scalar Gauss-Newton denominator 1+sum(exp(log_step)*precision*Jacobian^2) normalizes simultaneous writes. The sensitivity is a diagonal stop-statistic approximation, not exact historical gradient credit.",
                "control_semantics": {
                    "conditional": "Full state-conditioned residual calibration and adaptive local steps",
                    "unconditional": "Zero context, retaining per-parameter unconditional residual calibration",
                    "no_precision": "Learn conditional calibration but use unit precision in writes and local log-step credit",
                    "fixed_plasticity": "Learn conditional calibration but hold log steps at initialization",
                    "uncentered_noise": "Hold conditional mean zero so correctable bias can enter the variance estimate"},
                "initialization": "All four calibration coefficients zero: mean zero, variance one. Reference parameter scale 1/sqrt(parameter count), initial step reference squared. Calibration AdaGrad step .1; log-step AdaGrad step1.0; zero accumulators, denominator epsilon1e-12. Fixed before runs, no candidate sweep.",
                "controller_grid": "No LR grid for candidates; five causal controls. Baselines retain fifteen rates.",
                "prediction_registry": "runs/contextual_precision_v1_preregistration.json",
                "prediction_registry_sha256": hashlib.sha256(Path("runs/contextual_precision_v1_preregistration.json").read_bytes()).hexdigest(),
                "cost": "One analytic prediction/Jacobian evaluation per observation; candidate calibration and local sensitivity use eleven FP32 optimizer-state scalars per parameter plus per-model diagnostic counters. No gradient accumulation, model averaging, or replay.",
                "calibration_metric_caution": "Calibration NLL, standardized residual square, predicted variance and effective gain average over active parameters per example, then over active observations on each learner's own trajectory. They are not common-state cross-method rankings or model-learning success. baseline_context_variance means mean exp(logvar_bias), the variance at context zero, not the trajectory-averaged conditional variance; mean_step_size means mean exp(log_step).",
                ...
                "limitations": ["One seed; repeated development streams are not an untouched confirmatory holdout.",
                    ...
                    "Gaussian scalar residual variance is not a weight posterior, hidden-neuron target uncertainty, or a general RL/LLM likelihood.",
                    ...
                    "Diagonal sensitivity omits cross-parameter transport, residual curvature, calibration-state derivatives and meta-update derivatives; shared normalization does not guarantee nonlinear finite-step safety.",
                    "Cumulative AdaGrad state may be stale under drift. Meta steps and reference scale remain design constants; no parameter-free or parameterization-invariance claim.",
