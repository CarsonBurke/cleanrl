"""Fused Triton predictive momentum transport on the paired dense stream.
...
Production uses one fused 16-sample Triton kernel per CUDA graph replay; the v2
CUDA FP32 eager learner is a temporary correctness oracle, never a fallback.
"""
...
from cleanrl.plasticity import predictive_transport_stream_v2 as audit_reference
...
def train_arm(a, config, data, root, writer, result):
    ...
    try:
        ...
        row["resources"]["mutable_tensor_bytes"] = sum({
            t.untyped_storage().data_ptr(): t.untyped_storage().nbytes() for t in learner.mutable
        }.values())
        ...
        row["resources"]["startup_compute_scope"] = "explicit CUDA FP32 v2 eager local audit, single-step/fused-block parity, Triton warmup, capture and exact replay; excludes Triton compiler internals"
        ...
        for step in range(learner.capture_steps, a.samples + 1, learner.capture_steps):
            ...
            row["resources"].update(consumed_samples=int(learner.index.item()), learner_steps=float(learner.steps.item()),
                                    candidate_consumed_samples=learner.candidate_indices.cpu().tolist(),
            ...
            if (row["resources"]["consumed_samples"] != step or row["resources"]["learner_steps"] != step
                    or any(consumed != step for consumed in row["resources"]["candidate_consumed_samples"])):
...
def time_selected_replay(a, name, data, root, row, project, source_hashes):
    ...
    Capture audits and compiles the fused single-candidate kernel outside timing. The
    ...
    benchmark = row["fixed_selected_replay"] = {
        ...
        "comparison_limit": "single-candidate fused Triton shape is benchmarked independently; not the joint-grid training/report throughput or a second accuracy estimate",
    }
    ...
    try:
        ...
        benchmark["startup_compute_scope"] = "explicit CUDA FP32 v2 eager local audit, single-step/fused-block parity, Triton warmup, capture and exact replay; excludes Triton compiler internals"
...
def main():
    a = tyro.cli(Args)
    validate_args(a)
    root = Path(a.output) if a.output else Path("runs") / f"DenseStream__predictive_transport_v3__{a.seed}__{time.time_ns()}"
    ...
    source_paths = sorted({Path(__file__).resolve(), Path(transport.__file__).resolve(),
                           Path(audit_reference.__file__).resolve(), Path(reference.__file__).resolve(),
    ...
    result = {
        ...
        "information_fairness": {
            ...
            "transport": "m=beta1*m+(1-beta1)*g+beta1*(1-beta1**(t-1))*C with stable expm1 moment masses; previous weights retain the pre-update current weights explicitly inside each fused sample transition",
    ...
    try:
        ...
        result["resources"] = {"device": torch.cuda.get_device_name(), "torch": str(torch.__version__),
                               ...
                               "execution": "fused Triton CUDA graph replay; one CTA per candidate processes 16 sequential samples per kernel with candidate-owned indices; no torch.compile or eager production fallback; graph_steps=16",
