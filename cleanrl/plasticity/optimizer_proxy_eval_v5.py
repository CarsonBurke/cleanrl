...
Hypothesis: counting reused batches as independent observations destroys the
variance reduction of transported momentum. Advance round-family EMA clocks only
for fresh batches and transport moments between reuse updates. Tune AdamW equally; choose on noisy validation,
...
from cleanrl.plasticity import optimizer_proxy_model_v5 as optimizer
...
class Args:
    ...
    test: int = 8192
    output: str = ""
    plan: str = "benchmarks/plasticity/optimizer_proxy_v5_plan.json"
    ...
    def capture(self):
        ...
        try:
            ...
            for _ in range(self.scenario["epochs"]):
                ...
                self.commit(compute())
                self.audit_equal(self.mutable, eager)
            self.restore(before)
...
def main():
    ...
    root = Path(args.output or f"runs/OptimizerProxy__v5_{args.scenario}__{args.seed}__{time.time_ns()}")
    root.mkdir(parents=True, exist_ok=False)
    project = Path(__file__).resolve().parents[2]
    sources = [Path(__file__), Path(model.__file__), Path(optimizer.__file__), Path(reference.__file__),
