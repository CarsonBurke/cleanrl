...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = LearnedSharingState(self.weight[rows], control=control)
    ...
    def results(self, observed, elapsed, final_support=None):
        ...
        for control, state in self.controllers.items():
            ...
            for label, value in (("memory", state.memory), ("allocation", state.allocation),
                                 ("regression_curvature", state.fine[1])):
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__learned_sharing_v9__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("learned_sharing_benchmark_v9.py", "learned_sharing_v9.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "coordinate_horizon": "Lifetime sufficient statistics; renewed control discounts on feature participation by exp(-1/128). No gradient accumulation or postponed minibatch update.",
                "block_horizon": "Shared coefficient uses lifetime regression. Learned signed contrast discounts prior evidence according to squared agreement when its direction changes; fixed_evidence removes this discount. Global regression moments use FP64, model weights FP32.",
                "control_semantics": {"structured": "Shared, coordinate and sign-pooled validated contrast components", "renewed": "Same with renewable coordinate regression", "no_shared": "Remove shared coefficient structure", "no_dense": "Remove learned dense contrast", "no_gate": "Remove prequential admission", "misaligned": "Wrong-coordinate gradients and curvature; constant shared component is permutation invariant", "unpooled": "Continuous contrast magnitudes instead of sign-pooled magnitudes", "fixed_evidence": "Sign sharing without geometry-dependent evidence renewal"},
