"""Projected Predictive Readout v2: learn what to write, not merely how hard to write.
...
The candidate is projected onto the observed example Jacobian before a unit write;
no unobserved/nullspace component is committed. All candidate writes have unit scale. No LR sweep, posterior, covariance matrix,
...
class ProjectedReadoutState:
    ...
    A unit candidate can have cross-coordinate interference and unobserved components.
    Project onto the sample Jacobian, then use exact current-example quadratic line
    minimization on[0,1]. This avoids unsupported writes and overshoot, without an LR. sign(g)*sqrt(h) reconstructs x up to a common
    ...
    def step(self, weight, grad, curvature):
        ...
        desired = candidate - displacement
        signed_features = grad.sign() * curvature.sqrt()
        output_shift = (signed_features * desired).sum(-1, keepdim=True)
        feature_norm = curvature.sum(-1, keepdim=True)
        # Write only the candidate's observed output change. Copying its whole
        # vector also writes unconstrained nullspace directions, which v1 showed
        # can explode despite reducing the loss of the current example.
        direction = signed_features * (output_shift / feature_norm.clamp_min(tiny))
        slope = (grad * direction).sum(-1, keepdim=True)
        line_curvature = output_shift.square()
