"""Full sequential CUDA proxy for standalone Predictive Metric v7.
...
from cleanrl.plasticity.predictive_metric_v7 import CONTROLS, PredictiveMetricState
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__predictive_metric_v7__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("predictive_metric_benchmark_v7.py", "predictive_metric_v7.py",
    ...
    metadata = {
        ...
        "run_label": "predictive_metric_v7",
        ...
        "curvature": "H_i is the cumulative mean of squared current prediction Jacobians, including the current target-independent Jacobian. This is function-space geometry, not a gradient-noise second moment or a probability about weight values.",
        ...
        "state_cost": "Candidate32 bytes/parameter plus72 bytes/model: v4's three FP64 parameter moments and nine scalar statistics, plus one FP64 mean-squared-Jacobian vector. V4 reference retains24 bytes/parameter plus72 bytes/model. No momentum, Adam state, free base LR, schedule, gradient table, extra inference network, target substitution or replay in candidate.",
        "prediction_registry": "runs/predictive_metric_v7_preregistration.json",
        "prediction_registry_sha256": hashlib.sha256(Path("runs/predictive_metric_v7_preregistration.json").read_bytes()).hexdigest(),
        ...
        "measurement": "Original exact clean risk, full-stream prequential clean error, signal coefficient, nuisance/null power and actual useful/noise parameter writes. Inherited gain/evidence counters retain v4 definitions. Numerical checks cover local update geometry and compiled execution, not nonlinear finite-step guarantees; proxy itself is the unchanged original linear predictor.",
