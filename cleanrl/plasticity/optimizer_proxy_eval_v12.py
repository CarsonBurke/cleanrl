"""v12 protocol: the v11 protocol driving optimizer_proxy_model_v12 (adds gate_adamw / gate_polar, the
resistance-gated decay; gated families carry a `gate_beta` axis and keep their EMAs in aux slots 2 and 3),
named OptimizerProxy__v12_*. The v11 text follows. v11 protocol: the v10 protocol driving
optimizer_proxy_model_v11 (adds polar_svag / rms_svag), plus one new regime kind. A scenario carrying `relevant` uses structured teachers
that read only the first `relevant` inputs (zeroed first-layer columns, kept columns rescaled so the
pre-activation variance is unchanged); its selection is unchanged (noisy in-distribution validation), and
its locked test scoring additionally reports clean excess risk on two off-marginal held-out sets built
from the same test draws: irrelevant inputs scaled by `distractor_scale` (teacher output unchanged) and
relevant inputs scaled by `relevant_scale`. The promotion metric for such a regime is the sustained
distractor risk; the in-distribution risk is reported for the no-regression check. See the v9 docstring
below for the rest of the protocol.
...
from cleanrl.plasticity import optimizer_proxy_model_v12 as optimizer
...
class Args:
    ...
    test: int = 8192
    output: str = ""
    plan: str = "benchmarks/plasticity/optimizer_proxy_v12_plan.json"
    families: str = ""  # comma-separated override of the plan's family list (one family per job keeps runs short)


HYPER_KEYS = ("lr", "beta1", "beta2", "weight_decay", "head_lr_scale", *optimizer.TIER_KEYS, *optimizer.GATE_KEYS)
...
def structure_teacher(teacher, relevant, input_dim):
    """Teacher reading only the first `relevant` inputs; pre-activation variance preserved."""
    structured = [layer.clone() for layer in teacher]
    structured[0][:, relevant:] = 0
    structured[0][:, :relevant] *= math.sqrt(input_dim / relevant)
    return structured


def off_marginal(x, scenario):
    """Held-out inputs off the training marginal: distractor (teacher-invariant) and relevant scalings."""
    relevant = scenario["relevant"]
    distractor, scaled = x.clone(), x.clone()
    distractor[:, relevant:] *= scenario["distractor_scale"]
    scaled[:, :relevant] *= scenario["relevant_scale"]
    return {"distractor": distractor, "relevant": scaled}
...
class RoundLearner:
    ...
    def __init__(self, initial, grid, scenario, method):
        ...
        if optimizer.is_gated(method):
            for key in optimizer.GATE_KEYS:
                assert key in self.hyper, f"missing gate axis: {key}"
...
def main():
    ...
    if "relevant" in scenario:
        teachers = [structure_teacher(teacher, scenario["relevant"], args.input_dim) for teacher in teachers]
    ...
    root = Path(args.output or f"runs/OptimizerProxy__v12_{args.scenario}{tag}__{args.seed}__{time.time_ns()}")
    root.mkdir(parents=True, exist_ok=False)
    project = Path(__file__).resolve().parents[2]
    sources = [Path(__file__)]
    module = optimizer
    while module is not None:  # every optimizer version in the delegation chain hashes into the record
        sources.append(Path(module.__file__))
        module = getattr(module, "base", None)
    sources += [Path(model.__file__), Path(reference.__file__), Path(reporting.__file__), Path(args.plan), Path(runtime.__file__)]
    ...
    result = {"args": asdict(args), "scenario": scenario, "grids": grids, "status": "training",
              ...
              "tier_semantics": "tier families: weights are the transient tier within a block and the consolidated tier at block boundaries; aux slots are phi, gain, lag-1 innovation covariance, innovation power, previous innovation, velocity; gated families keep the gradient mean and power EMAs in slots 2 and 3",
              "parameter_groups": {
    ...
    try:
        ...
        for name, choice in result["selections"].items():
            ...
            for index, curve in enumerate(result["methods"][choice["source_method"]]["curves"]):
                ...
                off_marginal_risk = {}
                if "relevant" in scenario:
                    for label, ox in off_marginal(test_draws[index][0], scenario).items():
                        ox, oy = task_at(teachers, ox, curve["progress"], scenario["drift"])
                        off_marginal_risk[label] = float(observation_loss(weights, ox, oy, noise, pnoise,
                                                                          scenario["objective"])[1][0])
                        if not math.isfinite(off_marginal_risk[label]):
                            raise FloatingPointError(f"nonfinite off-marginal score: {name}/{label}")
                selected = slice(choice["index"], choice["index"] + 1)
                ...
                test_curve.append({"step": curve["step"], "observed_loss": float(observed[0]),
                                   ...
                                   **{f"clean_excess_risk_{label}": value for label, value in off_marginal_risk.items()}})
            ...
            for key in [k for k in test_curve[-1] if k.startswith("clean_excess_risk_")]:
                label = key[len("clean_excess_risk_"):]
                choice[f"test_sustained_risk_{label}"] = sum(c[key] for c in test_curve) / len(test_curve)
                choice[f"test_endpoint_risk_{label}"] = test_curve[-1][key]
