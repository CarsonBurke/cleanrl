"""Sequential CUDA-only dual-tracking benchmark, without model averaging.
...
observed loss and frozen before the second half is consumed. Dual averaging
re-solves accumulated gradients; horizon controls isolate lifetime memory. All controls
...
TRACKING_LRS = tuple(10 ** (-4 + k / 2) for k in range(9))
MEMORIES = (0, 1000, 10000, 100000)
CONTROLS = ("dual", "mirror", "innovation")
...
def configurations(streams):
    ...
    for method in ("sgd", "adamw", *CONTROLS):
        ...
        if method in ("sgd", "adamw"):
            ...
            settings = [{"lr": lr, "memory": memory} for lr in TRACKING_LRS for memory in MEMORIES]
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            memory = torch.tensor([cfg["memory"] for cfg in configs[rows]], device=device)[:, None]
            state = DualTrackingState(self.weight[rows], lr=self.lr[rows],
    ...
    def results(self, observed, elapsed, final_support=None):
        ...
        for control, state in self.controllers.items():
            ...
            statistics = {}
            denominator = state.square_sum.detach().double().sqrt()
            effective_step = torch.where(denominator > 0, state.rate / denominator.clamp_min(1e-300), 0.0)
            for label, value in (("gradient_sum", state.gradient_sum), ("square_sum", state.square_sum),
                                 ("mass", state.mass), ("effective_step", effective_step)):
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__dual_tracking_v1__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("dual_tracking_benchmark_v1.py", "dual_tracking_optimizer_v1.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "dual_rule": "G=decay*G+g; Q=decay*Q+g*g; weight=anchor-lr*G/sqrt(Q). Memory=0 means no discount, otherwise decay=exp(-1/memory).",
                "control_semantics": {"dual": "Re-solves all accumulated linearized losses.",
                                      "mirror": "Same gradient-square denominator, incremental steps, no anchor re-solving.",
                                      "innovation": "Dual updates using squares of gradient surprise g-G_old/N_old instead of g."},
                "controller_grid": {"lr": TRACKING_LRS, "memory": MEMORIES},
                "primary_objective": "Original stock targets alpha=0; planted signal cannot substitute for this objective.",
                ...
                "limitations": ["One seed and linear models; no universal optimizer-superiority claim.",
                    "Dual averaging and mirror controls are established optimization forms; no novelty theorem claimed.",
                    "Innovation normalization can grow unstable when gradients become highly predictable.",
                    ...
                    "Three tracking controls each use nine scales and four memory horizons; SGD/AdamW use fifteen learning rates.",
