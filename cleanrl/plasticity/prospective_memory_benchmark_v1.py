"""Sequential CUDA-only prospective-plasticity benchmark, without model averaging.

Every learner receives only observed gradients and diagonal curvature. Clean
synthetic targets and support are evaluator-only. Configurations are selected
using first-half observed loss and frozen before the second half is consumed.
The prospective controller uses diagonal eligibility approximations, not full
RTRL. All controls complete the full horizon, including underperforming ones.
"""
...
from cleanrl.plasticity.prospective_memory_optimizer_v1 import ProspectiveMemoryState


CHUNK = 100
LR_GRID = tuple(float(value) for value in np.logspace(-8, -1, 15))
INITIAL_LRS = (0.0001, 0.001, 0.01, 0.1)
META_LRS = (0.001, 0.01, 0.1)
RETAIN_RATES = (0.0001, 0.001, 0.01)
CONTROLS = ("full", "write", "retain", "fixed", "shuffle", "wall")
...
def configurations(streams):
    ...
    for method in ("sgd", "adamw", *CONTROLS):
        ...
        if method in ("sgd", "adamw"):
            ...
            settings = [{"initial_lr": lr, "meta_lr": meta}
                        for lr, meta in product(INITIAL_LRS, META_LRS)]
        elif method == "fixed":
            settings = [{"initial_lr": lr, "retain_rate": retention}
                        for lr, retention in product(INITIAL_LRS, RETAIN_RATES)]
        else:
            settings = [{"initial_lr": lr, "meta_lr": meta, "retain_rate": retention}
                        for lr, meta, retention in product(INITIAL_LRS, META_LRS, RETAIN_RATES)]
...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            parameters = {
                name: torch.tensor([cfg.get(name, default) for cfg in configs[rows]],
                                   device=device)[:, None]
                for name, default in (("initial_lr", INITIAL_LRS[0]),
                                      ("meta_lr", META_LRS[0]),
                                      ("retain_rate", RETAIN_RATES[0]))
            }
            state = ProspectiveMemoryState(self.weight[rows], control=control,
                                           meta_beta=0.99, **parameters)
    ...
    def results(self, observed, elapsed, final_support=None):
        ...
        for control, state in self.controllers.items():
            ...
            statistics = {}
            for attribute, label in (("log_write", "log_write_rate"), ("retain_logit", "retention")):
                ...
                if isinstance(value, torch.Tensor) and value.ndim and value.shape[0] == size:
                    ...
                    if attribute == "retain_logit":
                        forgetting = torch.zeros_like(value) if control == "write" else (-value).sigmoid()
                        statistics["forgetting_rate_mean"] = forgetting.mean(1).cpu().tolist()
                        statistics["forgetting_rate_std"] = forgetting.std(1, correction=0).cpu().tolist()
                        value = torch.ones_like(value) if control == "write" else value.sigmoid()
...
def matched_config(rows, selected, target_stream):
    if selected is None:
        return None
    keys = ("method", "lr", "initial_lr", "meta_lr", "retain_rate")
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__prospective_memory_v1__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("prospective_memory_benchmark_v1.py", "prospective_memory_optimizer_v1.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "meta_beta": 0.99,
                "meta_normalization": "Bias-corrected hypergradient RMS; write credit includes previous normalization actions affecting observed peers.",
                "retain_rate_semantics": "Initial forgetting fraction per participating coordinate observation; wall ablation forgets on every wall-time step.",
                ...
                "limitations": ["One seed and linear models; no universal optimizer-superiority claim.",
                    "Latest-action normalization credit is exact; historical eligibility remains diagonal, not full RTRL.",
                    "Synthetic sparse/dense targets are uncentered; dense signal has a large predictable mean.",
                    "Controller grids differ in cardinality because irrelevant control hyperparameters are omitted.",
