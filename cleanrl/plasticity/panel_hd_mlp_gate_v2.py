"""Streaming MLP on the high-dimensional vol panel, v2: adds gate_adam to v1's adam / gate_polar arms.
...
learnable). One (method, config) per job; results.json under runs/.
  ...
  gate_adam   : GatePolar with every group on the Adam direction: the gate alone, on the direction that wins here

    .venv/bin/python cleanrl/plasticity/panel_hd_mlp_gate_v2.py --method gate_polar --lr 3e-4 --weight-decay 0.3 --gate-beta 0.9999
"""
import json
import sys
import time
...
def make_optimizer(a, net):
    ...
    if a.method == "gate_polar":
        return GatePolar(mlp_groups(net, head_lr_scale=a.head_lr_scale), lr=a.lr, betas=(0.9, 0.999), eps=1e-8,
                         ...
                         compile=a.compile)
    if a.method == "gate_adam":
        return GatePolar([{"params": list(net.parameters()), "polar": False}], lr=a.lr, betas=(0.9, 0.999), eps=1e-8,
                         weight_decay=a.weight_decay, gate_beta=a.gate_beta, compile=a.compile)
...
def main():
    ...
    root = Path(a.output or f"runs/PanelHDMLPGate__v2__{tag}__{a.seed}__{time.time_ns()}")
