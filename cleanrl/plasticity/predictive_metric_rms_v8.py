"""Predictive Metric RMS v8: standalone evidence-controlled proximal updates.
...
class PredictiveMetricState(PredictiveNeedState):
    def __init__(self, weight, *, control="metric"):
        ...
        self.rms_jacobian = torch.zeros_like(weight, dtype=torch.float64)
    ...
    def transition(self, weight, gradient, jacobian, feature, target, residual, prediction):
        ...
        old_fraction = self.observation_count[:, None] / count[:, None]
        rms = torch.hypot(self.rms_jacobian * old_fraction.sqrt(), j / count[:, None].sqrt())
        safe_rms = torch.where(rms > 0, rms, 1.)
        normalized_j = j / safe_rms
        valid = stats["noise_ready"] & (self.m2_x > 0)
        participation = torch.where(valid, stats["participation"], 0.)
        # J / RMS is bounded by sqrt(n+1); neither J^2 nor 1/H is formed.
        energy = participation * normalized_j.square()
        ...
        write_fraction = confidence * fraction / (1 + total_energy)
        update = (write_fraction * participation * (gradient.double() / safe_rms)) / safe_rms
        next_weight = weight - update.to(weight.dtype)
        ...
        # Report a bounded functional fraction, not an inverse-square-unit
        # parameter gain that can be unrepresentable despite a finite update.
        updated["gain_sum"] = self.gain_sum + (write_fraction * energy).sum(1)
        for name, value in (("participation_sum", stats["participation"]),
