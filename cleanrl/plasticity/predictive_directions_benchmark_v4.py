...
class Runner:
    def __init__(self, args, dim, streams, total, score_after, configs, groups):
        ...
        for control in CONTROLS:
            rows = groups[control]
            state = PredictiveDirectionsState(self.weight[rows], control=control)
...
def main():
    ...
    directory = Path(args.output_dir) / f"{args.task}__predictive_directions_v4__{args.seed}__{stamp}"
    directory.mkdir(parents=True, exist_ok=False)
    source_files = ("predictive_directions_benchmark_v4.py", "predictive_directions_v4.py",
    ...
    metadata = {"arguments": vars(args), "source_arguments": asdict(source_args), "seed": args.seed,
                ...
                "run_label": "predictive_directions_v4",
                ...
                "control_semantics": {"hierarchical": "Admitted fast coordinate directions override the admitted slow block direction", "coordinate": "Fast coordinate evidence only", "block": "Slow normalized direction only", "unvalidated": "Fast coordinate amplitude without admission test", "shuffled": "Wrong-coordinate old directions used for validation"},
