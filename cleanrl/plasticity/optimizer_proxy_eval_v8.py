...
Hypothesis: hidden-matrix direction geometry improves adaptation beyond tuned
head learning rates. Compare polar iteration counts with Frobenius-matched
momentum and Adam-direction controls; matrix-family biases and heads use AdamW.
Tune the same head-rate grid for every family; choose on noisy validation, lock
all choices before test scoring,
...
from cleanrl.plasticity import optimizer_proxy_model_v8 as optimizer
...
class Args:
    ...
    test: int = 8192
    output: str = ""
    plan: str = "benchmarks/plasticity/optimizer_proxy_v8_plan.json"
...
class RoundLearner:
    ...
    def __init__(self, initial, grid, scenario, method):
        ...
        self.head_lr_scale = torch.tensor([g.get("head_lr_scale", 1.0) for g in grid],
    ...
    def compute(self):
        ...
        return optimizer.transition(self.weights, self.previous, self.m, self.v,
                                    ...
                                    self.method, self.scenario["epochs"], self.head_lr_scale)
...
def main():
    ...
    grid_axes = {**plan["grid"], "head_lr_scale": plan["grid"].get("head_lr_scale", [1.0])}
    grid = ([{**config, "head_lr_scale": config.get("head_lr_scale", 1.0)}
             for config in plan["configurations"]] if "configurations" in plan else
            [dict(zip(keys, values)) for values in itertools.product(*(grid_axes[key] for key in keys))])
    if "head_lr_scale" not in plan["grid"]:
        grid_axes["head_lr_scale"] = sorted({config["head_lr_scale"] for config in grid})
    families = plan.get("scenario_families", {}).get(args.scenario, plan["families"])
    root = Path(args.output or f"runs/OptimizerProxy__v8_{args.scenario}__{args.seed}__{time.time_ns()}")
    root.mkdir(parents=True, exist_ok=False)
    project = Path(__file__).resolve().parents[2]
    sources = [Path(__file__), Path(optimizer.__file__), Path(optimizer.base.__file__),
               Path(optimizer.base.base.__file__), Path(model.__file__), Path(reference.__file__),
        ...
        for method in families:
            ...
            for name in (["adamw", "adam"] if method == "adamw" else [method]):
                ...
                choice = {"status": "selected", "source_method": method, "index": index, "config": grid[index],
                          "validation": float(scores[index]), "edge_flags": edge_flags(grid[index], grid_axes)}
