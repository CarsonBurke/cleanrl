# HL-Gauss + Direct Log-Std + Shared Encoder for PPO
#
# Combines:
#   - HL-Gauss categorical value head (no value clipping)
#   - State-dependent log_std (Dreamer4-style)
#   - Shared first-layer encoder between actor and critic
#   - Single optimizer with combined loss (vf_coef balances gradients through shared encoder)
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        # Shared encoder: first layer common to actor and critic
        self.shared_encoder = nn.Sequential(
            layer_init(nn.Linear(obs_dim, 64)),
            nn.Tanh(),
        )
        # Critic head: second layer + categorical output
        self.critic_head = nn.Sequential(
            layer_init(nn.Linear(64, 64)),
            nn.Tanh(),
            layer_init(nn.Linear(64, num_bins), std=1.0),
        )
        # Actor head: second layer + mean/logstd outputs
    ...
    def get_value_logits(self, x):
        ...
        return self.critic_head(self.shared_encoder(x))
    ...
    def get_action_and_value(self, x, hl_support, action=None):
        ...
        hidden = self.actor_hidden(enc)
        action_mean = self.actor_mean_head(hidden)
        action_logstd = self.actor_logstd_head(hidden)
        action_logstd = torch.clamp(action_logstd, -5.0, 2.0)
        action_std = torch.exp(action_logstd)
        probs = Normal(action_mean, action_std)
        if action is None:
            action = probs.sample()
        critic_logits = self.critic_head(enc)
        value = hl_support.to_scalar(critic_logits)
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), value, critic_logits
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, _ = agent.get_action_and_value(next_obs, hl_support)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, newvalue_logits = agent.get_action_and_value(b_obs[mb_inds], hl_support, b_actions[mb_inds])
                ...
                # Value loss: HL-Gauss cross-entropy (logits from shared forward pass)
                if args.clip_vloss:
