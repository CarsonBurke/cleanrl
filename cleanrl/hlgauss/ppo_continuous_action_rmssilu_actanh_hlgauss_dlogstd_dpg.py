# RMSNorm+SiLU + Tanh actor output + HL-Gauss (no vclip) + Direct Log-Std + DPG
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        # Actor: shared trunk with RMSNorm+SiLU, then separate mean (Tanh) and logstd heads
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # DPG gate: compute from RAW advantages BEFORE normalization
                ...
                # THEN normalize advantages for the surrogate loss
                if args.norm_adv:
                    mb_advantages = (mb_advantages - mb_advantages.mean()) / (mb_advantages.std() + 1e-8)

                # Policy loss with DPG gating
                pg_loss1 = -mb_advantages * ratio
                pg_loss2 = -mb_advantages * torch.clamp(ratio, 1 - args.clip_coef, 1 + args.clip_coef)
                pg_loss = (dg_weight * torch.max(pg_loss1, pg_loss2)).mean()

                # Value loss: HL-Gauss cross-entropy (no value clipping)
                ...
                v_loss = -(target_probs * log_probs).sum(-1).mean()
