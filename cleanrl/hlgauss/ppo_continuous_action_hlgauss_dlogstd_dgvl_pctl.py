# HL-Gauss + Direct Log-Std + DG with Percentile-Normalized Value-Loss Surprisal
#
# Clamps delight to 5th/95th percentile, rescales to [-4, 4] for sigmoid.
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with torch.no_grad():
                    ...
                    # Percentile clamp + rescale to [-4, 4]
                    lo = torch.quantile(delight, 0.05)
                    hi = torch.quantile(delight, 0.95)
                    delight = torch.clamp(delight, lo, hi)
                    spread = (hi - lo).clamp(min=1e-8)
                    delight = (delight - (hi + lo) / 2) / (spread / 8)  # maps [lo,hi] → [-4,4]
                    dg_weight = torch.sigmoid(delight)
