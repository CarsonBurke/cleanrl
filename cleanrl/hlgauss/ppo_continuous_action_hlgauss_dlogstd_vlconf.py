# HL-Gauss + Direct Log-Std + Value-Loss Confidence Weighting for PPO
#
# Policy loss weighted by confidence = sigmoid(per_sample_CE_loss / eta).
# High CE → high weight, independent of advantage sign.
...
class Args:
    ...
    vl_eta: float = 1.0
    """temperature for sigmoid confidence weighting"""
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Compute per-sample value loss first (needed for policy scaling)
                ...
                # Value-loss scaling: amplify high-CE samples
                with torch.no_grad():
                    scale = torch.sigmoid(v_loss_per_sample.detach() / args.vl_eta)

                # Policy loss with value-loss scaling
                pg_loss1 = -mb_advantages * ratio
                pg_loss2 = -mb_advantages * torch.clamp(ratio, 1 - args.clip_coef, 1 + args.clip_coef)
                pg_loss = (scale * torch.max(pg_loss1, pg_loss2)).mean()
