# v144: direct H=1 differentiable WM action planner.
#
# v140-v143 underused the world model: the free-goal variants produced tiny
# action-distance improvements, and planner takeover disabled the PPO actor
# learning anchor. v144 returns to the direct action-gradient baseline and makes
# it observable:
# - rollout planning calls the direct frozen-WM action solver, not GoalSolver;
# - the solver uses stronger H=1 defaults (`S=4`, 16 Adam steps);
# - real PPO/SPO actor learning stays active after planner takeover;
# - diagnostics log raw cost delta, action movement, and action-gradient norm.
#
...
class Args:
    ...
    inner_solver_objective_scale: float = 10.0
    """multiplier on the direct action-solver objective before backward; diagnostics keep raw costs"""
    ...
    goal_solver_steps: int = 4
    """Adam steps for optimizing the free next obs-latent goal embedding"""
    goal_solver_lr: float = 0.05
    """Adam learning rate for the free next obs-latent goal embedding"""
    goal_defect_coef: float = 1.0
    """coefficient on summed squared anchor defect tying free g to the action-seeded WM next obs-latent"""
    goal_action_steps: int = 4
...
if __name__ == "__main__":
    ...
    def run_inner_action_solver(start_summary, init_actions, return_diagnostics=False):
        ...
        try:
            ...
            for _ in range(args.inner_solver_steps):
                ...
                raw_cost = agent.cost_from_action(
                    flat_start_summary,
                    action,
                    hl_support,
                    reward_support,
                ...
                cost = raw_cost.mean() * args.inner_solver_objective_scale
                inner_opt.zero_grad(set_to_none=True)
                cost.backward()
                with torch.no_grad():
                    if action_u.grad is not None:
                        last_grad_norm = action_u.grad.detach().reshape(
                            batch_size,
                            num_samples,
                            -1,
                        ).norm(dim=-1)
            ...
            with torch.no_grad():
                ...
                action_delta = final_action - candidate_actions
                action_abs_delta = action_delta.abs().mean(dim=(-1, -2))
                action_l2_delta = action_delta.flatten(2).norm(dim=-1)
                if last_grad_norm is None:
                    last_grad_norm = torch.zeros_like(final_cost)
                ...
                diagnostics = {
                    "action_abs_delta": action_abs_delta[batch_idx, best_idx].detach(),
                    "action_l2_delta": action_l2_delta[batch_idx, best_idx].detach(),
                    "last_grad_norm": last_grad_norm[batch_idx, best_idx].detach(),
                }
        finally:
            agent.train(was_training)
            restore_agent_grad(prev_states)
        if return_diagnostics:
            return best_action.detach(), best_initial_cost, best_final_cost, diagnostics
        return best_action.detach(), best_initial_cost, best_final_cost

    def run_goal_embedding_solver(start_summary, init_actions):
        """Two-stage H=1 planner: solve target obs-latent g, then solve action to reach g.

        GoalSolver optimizes only a free next obs-token embedding. It is seeded
        from current obs tokens plus the policy/action-predicted next obs delta.
        ActionSolver then minimizes only distance from WM(z, a) to the solved
        target; return is not used in the action objective.
        """
        if args.inner_solver_horizon != 1:
            raise ValueError("v140 goal-embedding solver expects --inner-solver-horizon 1")
        prev_states = freeze_agent_grad()
        was_training = agent.training
        agent.eval()
        try:
            ...
            with torch.no_grad():
                init_pred = predict_one(flat_start_summary, flat_init_actions)
                current_obs = flat_start_summary[:, :NUM_OBS_TOKENS].detach()
                anchor_obs = init_pred[:, :NUM_OBS_TOKENS].detach()
                init_goal_delta = anchor_obs - current_obs
            goal_delta = init_goal_delta.clone().requires_grad_(True)
            goal_opt = optim.Adam([goal_delta], lr=args.goal_solver_lr)

            def goal_energy():
                goal_obs = current_obs + goal_delta
                critic_input = agent.critic_input_norm(goal_obs.reshape(goal_obs.shape[0], -1))
                goal_value = hl_support.to_scalar(agent.critic(critic_input))
                defect = (goal_obs - anchor_obs).square().sum(dim=(-1, -2))
                defect_mean = defect / float(NUM_OBS_TOKENS * MODEL_DIM)
                model_return = goal_value
                energy = -model_return + args.goal_defect_coef * defect
                return energy, model_return, goal_value, defect_mean

            with torch.no_grad():
                initial_goal_energy, _, _, _ = goal_energy()
            for _ in range(args.goal_solver_steps):
                energy, _, _, _ = goal_energy()
                goal_opt.zero_grad(set_to_none=True)
                energy.mean().backward()
                goal_opt.step()

            with torch.no_grad():
                final_goal_energy, goal_return, goal_value, goal_defect = goal_energy()
                final_goal_obs = (current_obs + goal_delta).detach()
            ...
            def action_distance():
                ...
                return (pred_obs - best_goal_obs).square().mean(dim=(-1, -2))
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        rollout_direct_initial_costs = []
        rollout_direct_final_costs = []
        rollout_direct_action_abs_deltas = []
        rollout_direct_action_l2_deltas = []
        rollout_direct_grad_norms = []
        ...
        for step in range(0, args.num_steps):
            ...
            if planner_active:
                ...
                with torch.no_grad():
                    ...
                    rollout_direct_initial_costs.append(planner_initial_cost.mean().item())
                    rollout_direct_final_costs.append(planner_final_cost.mean().item())
                    rollout_direct_action_abs_deltas.append(
                        planner_diagnostics["action_abs_delta"].mean().item()
                    )
                    rollout_direct_action_l2_deltas.append(
                        planner_diagnostics["action_l2_delta"].mean().item()
                    )
                    rollout_direct_grad_norms.append(
                        planner_diagnostics["last_grad_norm"].mean().item()
                    )
        ...
        if rollout_direct_initial_costs:
            writer.add_scalar("direct_solver/initial_cost", safe_mean(rollout_direct_initial_costs), global_step)
            writer.add_scalar("direct_solver/final_cost", safe_mean(rollout_direct_final_costs), global_step)
            writer.add_scalar(
                "direct_solver/cost_delta",
                safe_mean(
                    [
                        final_cost - initial_cost
                        for initial_cost, final_cost in zip(
                            rollout_direct_initial_costs,
                            rollout_direct_final_costs,
                        )
                    ]
            ...
            writer.add_scalar(
                "direct_solver/action_abs_delta",
                safe_mean(rollout_direct_action_abs_deltas),
                global_step,
            )
            writer.add_scalar(
                "direct_solver/action_l2_delta",
                safe_mean(rollout_direct_action_l2_deltas),
                global_step,
            )
            writer.add_scalar(
                "direct_solver/last_grad_norm",
                safe_mean(rollout_direct_grad_norms),
                global_step,
            )
