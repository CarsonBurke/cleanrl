# v178: CLIP-HIGHER (asymmetric PPO clip). Decouples the PPO ratio clip bounds: lower
# stays 1 - clip_coef (0.20), upper becomes 1 + clip_coef_high (0.28, DAPO-style). The
# wider upper bound lets the policy keep shifting probability mass toward advantageous
# actions before the surrogate saturates -- a mild antidote to the Beta concentration
# runaway / premature entropy collapse diagnosed in v176 (actor_entropy fell monotonically
# with no floor). clipfrac now counts ratios outside the asymmetric band.
#
# REWARD NORMALIZATION (reverted): v177's raw-reward experiment regressed vs the
# normalized v176 champion, so NormalizeReward + the [-10,10] reward clip are RESTORED
# and the symlog support is reverted to the v176 grid (value v_min/v_max +-5, reward +-3).
# The returns/real_target "plateau" that motivated raw rewards is now understood as
# mostly the running-std rescale (a logging artifact) plus Beta entropy collapse -- the
# latter is what clip-higher targets directly. Net: v178 = v176 (normalized Beta champion)
# + the SIGReg-as-embed-norm cleanup (no hard in-agent embed norms) + clip-higher. The
# outcome-token input norms (reward/continuation_outcome_input_norm) are UNCHANGED: they
# normalize the length-51 reward-bin probability vector and the continuation scalar, both
# of which are invariant to whether env rewards are raw or normalized.
#
...
# tokens (unchanged). Input embed norms (reward/continuation_outcome_input_norm) kept.
...
class Agent(nn.Module):
    def __init__(
        ...
        # v177: outcome tokens use norm-BEFORE-projection, so the target token is
        # proj(norm(raw)) -- a non-unit embedding, structurally equivalent to the obs
        # latent target dyn_next_proj(dyn_final_norm(...)) = proj(norm(.)). This is the
        # idiomatic embed norm for these tokens: it must NOT be a post-projection norm
        # (that would force the target onto the unit sphere and break symmetry with the
        # un-normed obs latent target -- the v174 invariant). The norm sits on the raw
        # input (51 reward bins / scalar continuation) ahead of the projection.
    ...
    def _reward_outcome_token(self, reward_probs):
        # v177: proj(norm(raw)) -- non-unit target, mirrors the obs latent target
        # dyn_next_proj(dyn_final_norm(.)). Norm before projection (NOT a unit embed norm).
        return self.reward_outcome_proj(self.reward_outcome_input_norm(reward_probs))

    def _continuation_outcome_token(self, continuations):
        continuation_input = continuations.unsqueeze(-1)
        # v177: proj(norm(raw)) -- non-unit target, mirrors the obs latent target.
        return self.continuation_outcome_proj(self.continuation_outcome_input_norm(continuation_input))
