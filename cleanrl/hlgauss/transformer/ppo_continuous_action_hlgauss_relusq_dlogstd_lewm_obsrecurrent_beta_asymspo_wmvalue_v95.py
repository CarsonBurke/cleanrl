...
class Agent(nn.Module):
    def __init__(
        ...
        self.wm_value_input_norm = RMSNorm(NUM_LATENT_TOKENS * MODEL_DIM)
        self.wm_value_head = ReluSqRMSHead(NUM_LATENT_TOKENS * MODEL_DIM, AGENT_HIDDEN_DIM, num_bins, output_std=1.0)
    ...
    def _wm_value_features_from_latents(self, latent_tokens):
        return self.wm_value_input_norm(latent_tokens.reshape(latent_tokens.shape[0], -1))
    ...
    def get_wm_value_logits(self, summary_tokens, detach_summary=True):
        latent_tokens = summary_tokens.detach() if detach_summary else summary_tokens
        return self.wm_value_head(self._wm_value_features_from_latents(latent_tokens))

    def get_wm_value(self, summary_tokens, hl_support, detach_summary=True):
        return hl_support.to_scalar(self.get_wm_value_logits(summary_tokens, detach_summary=detach_summary))
    ...
    def build_dream_batch(prompt_summary_history, prompt_action_history, prompt_valids):
        ...
        with torch.no_grad():
            for _ in range(args.imagine_horizon):
                ...
                value = agent.get_wm_value(summary_state, hl_support)
            ...
            bootstrap_value = agent.get_wm_value(summary_history[-1].detach(), hl_support)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                initial_value_logits = agent.get_wm_value_logits(initial_summary)
                target_value_logits = agent.get_wm_value_logits(
                    target_next_latents.reshape(mb_size * args.dyn_horizon, NUM_LATENT_TOKENS, MODEL_DIM),
                ).reshape(mb_size, args.dyn_horizon, -1)
                pred_value_logits = agent.get_wm_value_logits(
                    pred_next_latents.reshape(mb_size * args.dyn_horizon, NUM_LATENT_TOKENS, MODEL_DIM),
                ).reshape(mb_size, args.dyn_horizon, -1)
                initial_value_loss = -(
                ...
                closed_loop_value_logits = agent.get_wm_value_logits(
                    closed_loop_next_latents.reshape(mb_size * args.dyn_horizon, NUM_LATENT_TOKENS, MODEL_DIM),
                ).reshape(mb_size, args.dyn_horizon, -1)
        ...
        with torch.no_grad():
            ...
            rollout_probe_values = agent.get_wm_value(target_summaries, hl_support).reshape(
                args.num_steps,
                args.num_envs,
            )
