# v139: official-style MPC warm start for unknown-goal best-policy control.
#
# v138 directly executed the differentiable return planner, but also trained
# the actor to clone every planner action. That created a delayed feedback loop:
# the planner initialized from an actor that was itself chasing stale planner
# outputs. This variant keeps the unknown-goal objective (maximize predicted
# reward + terminal value), but treats the actor as a prior/initializer rather
# than the control target after MPC starts.
#
# Key changes from v138:
...
# - planner initialization warm-starts from the shifted previous action plan
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        rollout_planner_initial_costs = []
        rollout_planner_final_costs = []
        ...
        for step in range(0, args.num_steps):
            ...
            if planner_active:
                ...
                planner_action_seq, planner_initial_cost, planner_final_cost = run_inner_action_solver(
                    planner_latents,
                    planner_init_seq,
                )
                ...
                with torch.no_grad():
                    ...
                    rollout_planner_initial_costs.append(planner_initial_cost.mean().item())
                    rollout_planner_final_costs.append(planner_final_cost.mean().item())
        ...
        if rollout_planner_initial_costs:
            writer.add_scalar("planner/rollout_initial_cost", safe_mean(rollout_planner_initial_costs), global_step)
            writer.add_scalar("planner/rollout_final_cost", safe_mean(rollout_planner_final_costs), global_step)
            writer.add_scalar(
                "planner/rollout_cost_delta",
                safe_mean(
                    [
