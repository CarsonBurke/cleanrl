...
class Agent(nn.Module):
    def __init__(
        ...
        # v184: RMSNorm conditioning on the outcome token embeddings. The outcome tokens are
        # removed from the SIGReg pool (v184) -- their anti-collapse now comes from the
        # UN-DETACHED reward/continuation decode (label-grounded sufficiency), not from SIGReg.
        # SIGReg therefore no longer sets their scale, so this norm keeps them at unit RMS,
        # comparable to the ~N(0,I) SIGReg'd obs tokens, for well-conditioned actor/critic reads
        # and a balanced JEPA latent-MSE. (Unlike the v174 outcome_token_norm this is paired
        # with SIGReg removal + decode grounding, so it conditions scale without being the sole
        # anti-collapse force.)
    ...
    def _outcome_tokens_from_labels(self, reward_probs, continuations):
        ...
        # v184: unit-RMS conditioning (per token, over MODEL_DIM); see outcome_token_norm note.
        return self.outcome_token_norm(torch.stack([reward_token, continuation_token], dim=1))
