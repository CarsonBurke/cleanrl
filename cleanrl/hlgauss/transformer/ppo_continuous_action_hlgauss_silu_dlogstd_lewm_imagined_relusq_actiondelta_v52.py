...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True, actor_mean_scale=3.0):
        ...
        self.pred_next_proj = nn.Sequential(
            xavier_init_linear(nn.Linear(MODEL_DIM, MODEL_DIM * 4)),
            nn.SiLU(),
            xavier_init_linear(nn.Linear(MODEL_DIM * 4, MODEL_DIM)),
        )
    ...
    def predict_next_latents_from_history(self, latent_history, action_history):
        ...
        pred_latents = pred_latents.reshape(batch, context_len, num_latents, width)
...
if __name__ == "__main__":
            ...
            for _ in range(args.imagine_horizon):
                ...
                if diagnostic_n > 0:
                    ...
                    try:
                        ...
                        diag_next_latents = diag_next_summary[:, :NUM_DYN_LATENT_TOKENS].reshape(
                            diagnostic_n,
                            sensitivity_k,
                            -1,
                        )
    ...
    def dynamics_diagnostics(flat_obs, rollout_rewards, rollout_actions, rollout_terminations, rollout_boundaries, rollout_valids):
        ...
        try:
            ...
            sensitivity_latents = sensitivity_next_summary[:, :NUM_DYN_LATENT_TOKENS].reshape(sensitivity_n, sensitivity_k, -1)
            metrics.update(
