...
# - v122: replaces one-step TD2-CFM with truncated geometric successor anchors
#   over the teacher window and adds a detached predicted-source TDF term so the
#   tail model is trained on the latent distribution used by dreams.
import copy
import os
import random
import sys
...
class Args:
    ...
    """weight on the detached geometric TD2-CFM full successor-summary flow"""
    successor_pred_source_coef: float = 0.5
    """relative weight for detached predicted-summary sources in successor-flow training"""
...
class Agent(nn.Module):
    def __init__(
        ...
        successor_flow_dim = NUM_LATENT_TOKENS * MODEL_DIM
    ...
    def _successor_flow_velocity(self, condition_tokens, successor_xt, flow_t, use_target=False):
        ...
        return velocity.reshape(condition_tokens.shape[0], NUM_LATENT_TOKENS, MODEL_DIM)
    ...
    def integrate_successor_flow(
        ...
        if start_tokens is None:
            successor_tokens = torch.randn_like(condition_tokens)
        else:
            successor_tokens = start_tokens.reshape(condition_tokens.shape[0], NUM_LATENT_TOKENS, MODEL_DIM)
    ...
    def successor_td2_flow_loss(
        ...
        flat_absorbing = absorbing_summary_tokens.reshape(-1, NUM_LATENT_TOKENS, MODEL_DIM).detach()
    ...
    def successor_geometric_td2_flow_loss(
        self,
        source_summary_tokens,
        future_summary_tokens,
        absorbing_summary_tokens,
        terminal_mask,
        sample_weight,
        gamma,
        ...
        batch, horizon = source_summary_tokens.shape[:2]
        if horizon <= 0:
            zero = source_summary_tokens.sum() * 0.0
            if return_components:
                return zero, zero.detach(), zero.detach()
            return zero

        source_denom = sample_weight.sum().clamp_min(1.0)
        direct_numer = source_summary_tokens.sum() * 0.0
        for offset in range(horizon):
            valid_horizon = horizon - offset
            flat_source = source_summary_tokens[:, :valid_horizon].reshape(
                -1,
                NUM_LATENT_TOKENS,
                MODEL_DIM,
            ).detach()
            flat_target = future_summary_tokens[:, offset:].reshape(
                -1,
                NUM_LATENT_TOKENS,
                MODEL_DIM,
            ).detach()
            flat_weight = sample_weight[:, offset:].reshape(-1)
            flow_t = torch.rand(flat_source.shape[0], device=flat_source.device, dtype=flat_source.dtype)
            source_noise = torch.randn_like(flat_target)
            path_t = flow_t.reshape(-1, 1, 1)
            direct_xt = source_noise.lerp(flat_target, path_t)
            direct_target_velocity = flat_target - source_noise
            direct_velocity = self._successor_flow_velocity(
                flat_source,
                direct_xt,
                flow_t,
            ...
            coeff = (1.0 - gamma) * (gamma**offset)
            direct_numer = direct_numer + coeff * (offset_loss * flat_weight).sum()
        direct_loss = direct_numer / source_denom

        gamma_tensor = source_summary_tokens.new_tensor(float(gamma))
        batch_idx = torch.arange(batch, device=source_summary_tokens.device)
        boot_sources = []
        boot_tails = []
        boot_absorbing = []
        boot_terminals = []
        boot_weights = []
        tail_coeffs = []
        valid_targets = sample_weight > 0.0
        for source_offset in range(horizon):
            future_valid = valid_targets[:, source_offset:]
            available_steps = future_valid.long().sum(dim=1)
            has_tail = available_steps > 0
            if not bool(has_tail.any()):
                continue
            last_future_idx = source_offset + (available_steps - 1).clamp_min(0)
            valid_batch_idx = batch_idx[has_tail]
            valid_last_idx = last_future_idx[has_tail]
            boot_sources.append(source_summary_tokens[has_tail, source_offset])
            boot_tails.append(future_summary_tokens[valid_batch_idx, valid_last_idx])
            boot_absorbing.append(absorbing_summary_tokens[valid_batch_idx, valid_last_idx])
            boot_terminals.append(terminal_mask[valid_batch_idx, valid_last_idx])
            boot_weights.append(sample_weight[valid_batch_idx, valid_last_idx])
            tail_coeffs.append(torch.pow(gamma_tensor, available_steps[has_tail].to(dtype=source_summary_tokens.dtype)))
        if boot_sources:
            flat_source = torch.cat(boot_sources, dim=0).detach()
            flat_tail = torch.cat(boot_tails, dim=0).detach()
            flat_absorbing = torch.cat(boot_absorbing, dim=0).detach()
            flat_terminal = torch.cat(boot_terminals, dim=0).bool()
            flat_weight = torch.cat(boot_weights, dim=0)
            tail_coeffs = torch.cat(tail_coeffs, dim=0)
        else:
            boot_loss = source_summary_tokens.sum() * 0.0
            total_loss = direct_loss + boot_loss
            if return_components:
                return total_loss, direct_loss.detach(), boot_loss.detach()
            return total_loss
        flow_t = torch.rand(flat_source.shape[0], device=flat_source.device, dtype=flat_source.dtype)
        boot_noise = torch.randn_like(flat_tail)
        with torch.no_grad():
            boot_xt = self.integrate_successor_flow(
                flat_tail,
                start_tokens=boot_noise,
        ...
        boot_loss_per_sample = F.mse_loss(
            boot_velocity,
            boot_target_velocity.detach(),
            reduction="none",
        ).mean(dim=(-1, -2))
        boot_loss = (boot_loss_per_sample * flat_weight * tail_coeffs).sum() / source_denom
        total_loss = direct_loss + boot_loss
        if return_components:
            return total_loss, direct_loss.detach(), boot_loss.detach()
        return total_loss
    ...
    def successor_value(self, summary_tokens, reward_support, gamma, samples=1, steps=4, chunk_size=0):
        ...
        if chunk_size <= 0 or total <= chunk_size:
            ...
            reward_logits, _ = self.decode_outcomes(successor_tokens, detach_summary=True)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        successor_real_flow_losses = []
        successor_pred_source_flow_losses = []
        successor_direct_flow_losses = []
        successor_boot_flow_losses = []
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                (
                    real_successor_flow_loss,
                    real_successor_direct_loss,
                    real_successor_boot_loss,
                ) = agent.successor_geometric_td2_flow_loss(
                ...
                pred_source_flow_loss = real_successor_flow_loss * 0.0
                if args.successor_pred_source_coef != 0.0 and args.dyn_horizon > 1:
                    pred_source_flow_loss = agent.successor_geometric_td2_flow_loss(
                        pred_next_latents[:, :-1].detach(),
                        target_core_summaries[:, 1:],
                        absorbing_core_summaries[:, 1:],
                        future_terminations[:, 1:],
                        latent_weight[:, 1:],
                        args.gamma,
                        args.successor_flow_train_steps,
                    )
                successor_flow_loss = (
                    real_successor_flow_loss
                    + args.successor_pred_source_coef * pred_source_flow_loss
                )
                ...
                successor_real_flow_losses.append(real_successor_flow_loss.item())
                successor_pred_source_flow_losses.append(pred_source_flow_loss.item())
                successor_direct_flow_losses.append(real_successor_direct_loss.item())
                successor_boot_flow_losses.append(real_successor_boot_loss.item())
                if run_diagnostics:
                    with torch.no_grad():
        ...
        successor_real_flow_loss_mean = safe_mean(successor_real_flow_losses)
        successor_pred_source_flow_loss_mean = safe_mean(successor_pred_source_flow_losses)
        successor_direct_flow_loss_mean = safe_mean(successor_direct_flow_losses)
        successor_boot_flow_loss_mean = safe_mean(successor_boot_flow_losses)
        ...
        writer.add_scalar("dynamics/successor_real_flow_loss", successor_real_flow_loss_mean, global_step)
        writer.add_scalar(
            "dynamics/successor_pred_source_flow_loss",
            successor_pred_source_flow_loss_mean,
            global_step,
        )
        writer.add_scalar("dynamics/successor_direct_flow_loss", successor_direct_flow_loss_mean, global_step)
        writer.add_scalar("dynamics/successor_boot_flow_loss", successor_boot_flow_loss_mean, global_step)
        if dyn_reward_mses:
