# v130: v129's (μ_z, κ) reparameterization, but the β-NLL amortization
# is reverted to v124-style MSE on the mean.
#
# Why: v128 and v129 both collapsed in returns. Two compounding bugs in
# the β-NLL amortization:
#   (a) Reduction mismatch — v128/v129 used `(weight * gauss_nll).sum(-1).mean()`,
#       which sums over the action dim. v124's `F.mse_loss(...)` averages
#       over all elements. For HalfCheetah's 6-dim action this makes the
#       β-NLL term ~6× larger than v124's MSE, so `amortize_coef=1.0` is
#       effectively 6×, drowning the SPO/PG signal.
#   (b) Conceptual: `a_star_first` is the inner-solver's deterministic
#       improvement of the current policy. There is no irreducible noise
#       on this target, so β-NLL's optimal σ is |y − μ| → 0 as the
#       policy aligns. β=0.5 inverse-std reweighting only damps, not
#       prevents, the σ → 0 collapse. Empirically: amortize_loss went
#       negative (1.7 → −0.9), actor_std fell 0.48 → 0.16, returns
#       collapsed (v128 peaked +43 at 608k → −445 by 2M).
#
# Fix: drop β-NLL entirely from the amortization. Keep v129's (μ_z, κ)
# split-head reparam — not load-bearing for the bug fix but useful as
# (i) a clean σ knob for upcoming direct-log-std / UCB variants and
# (ii) it keeps the PG gradient on μ_z separable from κ.
#
# v129 → v130 diff: one line in the amortize block — revert to
# `F.mse_loss(actor_mean, target)`. Hyperparams unchanged
# (inner_solver_horizon=1, init=policy_mean, real_spo_coef=1.0,
# amortize_coef=1.0, wm_warmup=100k, ent_coef=0).
#
# v129 header (kept for context):
#   v128 + (μ_z, κ) Beta reparam. `actor_mean_logit_head` (A outputs →
#   sigmoid → μ_z) controls location; `actor_concentration_log_head`
#   (A outputs → softplus → κ) controls dispersion. Then
#     α = 1 + μ_z · κ,   β = 1 + (1 − μ_z) · κ
#   keeps α, β ≥ 1 (unimodal Beta everywhere). The Beta mean is
#   (1 + μ_z·κ)/(2 + κ), so it tracks μ_z exactly only as κ → ∞ —
#   a soft coupling, not an issue once amortize is MSE.
#
        ...
        if not world_model_only:
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    # v130 amortization: v124-style MSE on the Beta mean
                    # (action-space). β-NLL removed — see header.
