...
#   value tokens, and predicted value tokens are appended to dream history. JEPA value
#   MSE uses stop-grad targets; target-token CE grounds the target projection, while
#   predicted/read-token CE is detached decoder calibration.
import os
import random
import sys
import time
...
NUM_VALUE_TOKENS = 1
NUM_RECURRENT_TOKENS = NUM_LATENT_TOKENS + NUM_VALUE_TOKENS
SCALAR_EMBED_DIM = 32
...
class Args:
    ...
    """weight on detached world-model value readout calibration"""
...
class Agent(nn.Module):
    ...
    def recurrent_summary(self, core_summary_tokens, value_tokens):
        return torch.cat([core_summary_tokens, value_tokens], dim=1)
    ...
    def predict_next_latents(self, latent_tokens, action):
        pred_next_summary, _, _, _, _ = self.dynamics_step(latent_tokens, action)
        return pred_next_summary

    def predict_next_latents_and_values_all_from_history(self, latent_history, action_history):
        ...
        pred_latents = pred_latents[:, :, self.act_dim :]
        pred_value_tokens = pred_latents[:, :, NUM_LATENT_TOKENS:]
        pred_latents = pred_latents[:, :, :NUM_LATENT_TOKENS]
    ...
    def dynamics_teacher_forced(self, latent_history, action_history):
        ...
        pred_value_logits = self.decode_value_token(
            pred_value_tokens.reshape(batch * horizon, NUM_VALUE_TOKENS, width),
        )
        ...
        pred_value_logits = pred_value_logits.reshape(batch, horizon, -1)
        return pred_next_latents, pred_value_tokens, pred_reward_logits, pred_termination_logits, pred_value_logits

    def dynamics_step_from_history(self, summary_history, action_history):
        if summary_history.shape[2] == NUM_LATENT_TOKENS:
            batch, context_len, _, width = summary_history.shape
            flat_core = summary_history.reshape(batch * context_len, NUM_LATENT_TOKENS, width)
            flat_value = self.read_value_token(flat_core).detach()
            summary_history = self.recurrent_summary(flat_core, flat_value).reshape(
                batch,
                context_len,
        ...
        pred_value_logits = self.decode_value_token(pred_value_token)
        return pred_next_summary, pred_value_token, pred_reward_logits, pred_termination_logits, pred_value_logits
...
if __name__ == "__main__":
    ...
    @torch.no_grad()
    def build_dream_prompt_context(flat_obs, flat_prev_reward_probs, flat_prev_continues, rollout_actions, rollout_boundaries):
        ...
        for offset in range(prompt_len):
            ...
            prompt_summary_history.append(
                agent.recurrent_summary(core_summary, agent.read_value_token(core_summary)).detach()
            )
    ...
    def build_dream_batch(prompt_summary_history, prompt_action_history, prompt_valids):
        ...
        with torch.no_grad():
            for _ in range(args.imagine_horizon):
                ...
                dream_action, dream_action_z, old_logprob, _, _agent_value = agent.get_imagined_action_and_value(
                    summary_state, hl_support
                )
                current_value_token = summary_state[:, NUM_LATENT_TOKENS:]
                ...
                if diagnostic_n > 0:
                    ...
                    try:
                        ...
                        diag_next_summary, _, diag_reward_logits, _, _ = agent.dynamics_step_from_history(
                            diag_pred_context,
                            diag_action_context,
                        )
                ...
                summary_history.append(agent.recurrent_summary(pred_next_summary, pred_value_token).detach())
    ...
    def dynamics_diagnostics(
        ...
        try:
            ...
            summary_history = [
                agent.recurrent_summary(initial_core_summary, agent.read_value_token(initial_core_summary)).detach()
            ]
            action_history = []
            ...
            sensitivity_next_summary, _, sensitivity_reward_logits, _, _ = agent.dynamics_step_from_history(
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        dyn_value_losses = []
        target_value_ground_losses = []
        predicted_value_decode_losses = []
        value_token_losses = []
        read_value_decode_losses = []
        pred_value_token_losses = []
        closed_loop_value_token_losses = []
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                initial_summary = agent.recurrent_summary(initial_core_summary, initial_value_read_token)
                sigreg_initial_summary = agent.recurrent_summary(initial_core_summary, initial_value_target_token)
                ...
                with torch.no_grad():
                    teacher_value_history = agent.read_value_token(
                        teacher_core_history.reshape(mb_size * args.dyn_horizon, NUM_LATENT_TOKENS, MODEL_DIM),
                    ).reshape(mb_size, args.dyn_horizon, NUM_VALUE_TOKENS, MODEL_DIM)
                teacher_window_latents = agent.recurrent_summary(
                    teacher_core_history.reshape(mb_size * args.dyn_horizon, NUM_LATENT_TOKENS, MODEL_DIM),
                    teacher_value_history.reshape(mb_size * args.dyn_horizon, NUM_VALUE_TOKENS, MODEL_DIM),
                ).reshape(mb_size, args.dyn_horizon, NUM_RECURRENT_TOKENS, MODEL_DIM)
                target_future_summaries = agent.recurrent_summary(
                ...
                sigreg_window_latents = torch.cat(
                    [sigreg_initial_summary.unsqueeze(1), target_future_summaries],
                    dim=1,
                )
                ...
                read_initial_value_logits = agent.decode_value_token(initial_value_read_token)
                ...
                read_initial_value_loss = -(
                    initial_value_target_probs.detach() * torch.log_softmax(read_initial_value_logits, dim=-1)
                ).sum(dim=-1).mean()
                pred_value_loss = -(
                    value_target_probs.detach() * torch.log_softmax(pred_value_logits, dim=-1)
                ).sum(dim=-1)
                dyn_target_value_loss = (
                    target_value_loss * latent_weight
                ).sum() / latent_weight.sum().clamp_min(1.0)
                dyn_pred_value_loss = (
                    pred_value_loss * latent_weight
                ).sum() / latent_weight.sum().clamp_min(1.0)
                ...
                for horizon_idx in range(args.dyn_horizon):
                    ...
                    summary_history.append(
                        agent.recurrent_summary(closed_loop_summary, closed_loop_value_token).detach()
                    )
                ...
                closed_loop_value_loss_per_step = -(
                    value_target_probs.detach() * torch.log_softmax(closed_loop_value_logits, dim=-1)
                ).sum(dim=-1)
                closed_loop_value_loss = (
                    closed_loop_value_loss_per_step * latent_weight
                ).sum() / latent_weight.sum().clamp_min(1.0)
                ...
                target_value_ground_loss = 0.5 * (initial_target_value_loss + dyn_target_value_loss)
                predicted_value_decode_loss = 0.5 * (
                    0.5 * (read_initial_value_loss + dyn_pred_value_loss)
                    + closed_loop_value_loss
                )
                value_token_loss = 0.5 * (
                    0.5 * (initial_value_token_loss + pred_value_token_loss)
                    + closed_loop_value_token_loss
                )
                ...
                dyn_value_loss = (
                    target_value_ground_loss
                    + predicted_value_decode_loss
                    + value_token_loss
                ) / 3.0
                ...
                pred_value_losses.append(dyn_pred_value_loss.item())
                target_value_ground_losses.append(target_value_ground_loss.item())
                predicted_value_decode_losses.append(predicted_value_decode_loss.item())
                value_token_losses.append(value_token_loss.item())
                read_value_decode_losses.append(read_initial_value_loss.item())
                pred_value_token_losses.append(pred_value_token_loss.item())
                closed_loop_value_token_losses.append(closed_loop_value_token_loss.item())
                ...
                closed_loop_value_losses.append(closed_loop_value_loss.item())
                with torch.no_grad():
        ...
        target_value_ground_loss_mean = safe_mean(target_value_ground_losses)
        predicted_value_decode_loss_mean = safe_mean(predicted_value_decode_losses)
        value_token_loss_mean = safe_mean(value_token_losses)
        read_value_decode_loss_mean = safe_mean(read_value_decode_losses)
        pred_value_token_loss_mean = safe_mean(pred_value_token_losses)
        closed_loop_value_token_loss_mean = safe_mean(closed_loop_value_token_losses)
        ...
        pred_value_loss_mean = safe_mean(pred_value_losses)
        ...
        writer.add_scalar("dynamics/value_target_ground_loss", target_value_ground_loss_mean, global_step)
        writer.add_scalar("dynamics/value_decode_loss", predicted_value_decode_loss_mean, global_step)
        writer.add_scalar("dynamics/value_token_loss", value_token_loss_mean, global_step)
        writer.add_scalar("dynamics/read_value_loss", read_value_decode_loss_mean, global_step)
        writer.add_scalar("dynamics/pred_value_loss", pred_value_loss_mean, global_step)
        writer.add_scalar("dynamics/pred_value_token_loss", pred_value_token_loss_mean, global_step)
        writer.add_scalar("dynamics/closed_loop_reward_loss", safe_mean(closed_loop_reward_losses), global_step)
        writer.add_scalar("dynamics/closed_loop_value_loss", safe_mean(closed_loop_value_losses), global_step)
        writer.add_scalar(
            "dynamics/closed_loop_value_token_loss",
            closed_loop_value_token_loss_mean,
            global_step,
        )
