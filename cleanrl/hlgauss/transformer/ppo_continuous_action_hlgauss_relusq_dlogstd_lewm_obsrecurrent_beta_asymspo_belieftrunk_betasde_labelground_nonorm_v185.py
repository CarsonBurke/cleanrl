# v185: v184 MINUS the outcome RMSNorm. Single-axis ablation isolating the hard per-token
# outcome norm. Reasoning (subagent-audited): the un-detached reward-CE/continuation-BCE is a
# LOCAL autoencoder on the outcome proj (cannot reach obs/transformer -- outcome tokens are
# concatenated, never mixed, and decode slices only outcome positions), so it grounds outcome
# embeds for SUFFICIENCY without distorting the state representation. The v184 RMSNorm on top
# was a mild net negative: it removed the per-sample magnitude DOF the CE wants, was redundant
# with the un-detached JEPA-MSE target (which already bounds scale against CE blow-up), and was
# asymmetric with the un-normed obs JEPA targets. v185 drops it -> proj(label) directly. A/B vs
# v184 confirms whether the norm helps or hurts; watch outcome decode loss / outcome_pred_mse
# for scale drift (if it drifts, prefer a SOFT L2-on-norm over reinstating the hard RMSNorm).
# (Rejected alternative: a separate SIGReg pool on the 2 outcome tokens -- repeats the v183
# low-rank collapse since reward is ~1-D + continuation constant -> unsatisfiable N(0,I_64),
# and SIGReg optimizes spread not decodability, the wrong property for a label token.)
#
# v184: LABEL-GROUNDED OUTCOME-TOKEN EMBEDS. One conceptual change over v182 (joint-KL KEPT,
# run on the mb8/s500 config). DIAGNOSIS of the SIGReg dead-ends (v183 per-token collapse,
# the separate-pool idea): the 2 outcome tokens (reward simplex + continuation scalar) were
# grounded by EXACTLY the same forces as obs tokens -- JEPA latent-MSE (collapse-prone) +
# SIGReg -- because the reward CE / continuation BCE decode heads are .detach()'d (they train
# only the readout, never the embedding). So SIGReg was their only real anti-collapse, and
# because each is a low-rank single-scalar-derived token (continuation is literally constant on
# HalfCheetah), it cannot be Gaussianized in isolation -> pooling with obs was load-bearing but
# is the wrong prior. KEY ASYMMETRY we were missing: outcome tokens have an OBSERVABLE LABEL
# (the true reward dist / continuation); obs tokens do not (JEPA, no reconstruction).
#   FIX (use the label, not Gaussianity):
#   1. UN-DETACH the target outcome decode (detach_summary=False) -> the HL-Gauss reward CE and
#      continuation BCE flow into the encoder's outcome embedding, forcing it to be a SUFFICIENT
#      STATISTIC for the true outcome. Supervised, information-preserving anti-collapse, active
#      exactly in proportion to signal variation (reward varies -> strong; continuation constant
#      -> trivially satisfied, token becomes a correct constant, nothing to collapse).
#   2. REMOVE the 2 outcome tokens from the SIGReg pool -> SIGReg is obs-only (full-rank,
#      Gaussianizable), which is also the le-wm-faithful design (le-wm SIGReg's the state
#      embedding; it has no reward representation at all).
#   3. RMSNorm the outcome tokens for scale conditioning (SIGReg used to set their scale).
# Grounding weight = dyn_reward_coef*0.5 = 0.125 > the 0.09 SIGReg removed, so it is adequate.
# Hypothesis: cleaner, label-faithful outcome embeds + a non-degenerate SIGReg target; the
# actor/critic read reward/continuation tokens that actually ENCODE the predicted outcome.
# Expected gain modest (WM is not the binding constraint) but the design is finally principled.
#
# v182: JOINT-KL TRUST REGION INSTEAD OF THE PPO CLIP (dreamer4 PMPO style). One change
# over v181. Diagnosis: v181 (and the whole lineage) plateaus on HalfCheetah ~3.0k because
# the Beta policy COLLAPSES -- actor_entropy falls monotonically (-0.5 -> -4.1), the
# concentration α+β runs to ~1.6e4 (near-deterministic), and returns/real_target flat-lines
# at the halfway mark exactly as entropy dies. The asymmetric clip (v178 clip-higher) is
# only a per-step RATE-LIMITER on the ratio: it caps how fast the policy moves but applies
# NO restoring force, so concentration ratchets up every epoch unchecked.
#   FIX (as ../dreamer4 does): drop the clip; constrain the policy with a soft KL trust
#   region. The actor objective becomes
#       L = -E[Â · r]  +  actor_kl_coef · KL(old‖new)         (UNCLIPPED PG + KL penalty)
#   where r = π_new/π_old, old = the behavior (data-generating) Beta, and KL(old‖new) is the
#   ANALYTIC joint KL of the factorized Beta policy (closed form, summed over action dims).
#   actor_kl_coef = 0.3 and reverse KL(old‖new) match dreamer4's PMPO defaults
#   (pmpo_kl_div_loss_weight=0.3, pmpo_reverse_kl=True). Why reverse: KL(old‖new) is
#   mass-covering w.r.t. the behavior policy -- it BLOWS UP when π_new gets sharper than
#   π_old (assigns low mass where old had mass), so it directly penalizes spread reduction,
#   a genuine anti-collapse restoring force the clip lacked. The behavior concentrations
#   (α_old, β_old) are stashed per step at acting time (like behavior_logprobs) so "old" is
#   exactly the data-generating policy. New diagnostics: losses/actor_kl_penalty and
#   losses/actor_concentration (mean α+β) make the collapse directly observable.
#   Everything else (per-dim tokenizer, Beta-SDE policy, HL-Gauss critic, attached JEPA,
#   LeJEPA, batch-adv-norm, 2 epochs) is unchanged. clip_coef* args are kept only for the
#   clipfrac drift diagnostic; they no longer gate the objective.
#
# v181: PER-DIMENSION OBS TOKENIZER (FT-Transformer feature tokenizer) + clean pre-norm
# dyn stack. TWO changes over v180.
#   (1) EACH OBSERVATION SCALAR IS ITS OWN TOKEN. v180 (and all priors) densely mixed the
#       whole obs vector into a FIXED 8 tokens via obs_mix_proj = Linear(obs_dim, 8*D).
#       That dense mix (a) entangles all sensors into every token before any attention,
#       pre-empting the backbone's own mixing, and (b) caps obs resolution at 8 latent
#       slots regardless of obs_dim. v181 adopts the le-wm/LLM tokenizer stance: obs_dim
#       tokens, one per sensor, via a learned PER-FEATURE (weight, bias) embedding:
#           token_i = obs_i * W[i] + b[i],   W, b in R^{obs_dim x MODEL_DIM}.
#       Each sensor gets a DISTINCT content direction (the continuous analog of an LLM
#       vocab row), so the backbone can disambiguate features even at equal scalar values.
#       A purely SHARED scalar->D projection would be degenerate (all tokens colinear,
#       differing only in magnitude) and RoPE -- which rotates q/k, not values -- could not
#       separate them; the per-feature W/b is what makes the tokens content-distinct.
#       Order is carried by RoPE in dyn_layers (NO learned positional embedding; feature
#       identity already lives in W/b). NUM_OBS_TOKENS now == obs_dim, so NUM_LATENT_TOKENS,
#       NUM_RECURRENT_TOKENS, NUM_SIGREG_TOKENS, AGENT_INPUT_DIM and the predictor space
#       axis all scale with obs_dim (rebound in main() before any allocation). For
#       HalfCheetah obs_dim=17: 17 obs + 2 outcome = 19 latent tokens (was 10);
#       AGENT_INPUT_DIM 640 -> 1216.
#   (2) DROP THE dyn_x0 RAW-INPUT SKIP. v180's TransformerBlock began each layer with a
#       learnable x = resid_mix0*x + resid_mix1*x0 blend of the layer input and the raw
#       token embedding. With a per-feature tokenizer the raw embedding is already a clean,
#       well-conditioned per-token signal, and the skip just re-injected pre-attention
#       content. v181 removes resid_mix/x0 -> a standard clean pre-norm residual stack
#       (x = x + sublayer(norm(x))), the SOTA-default structure (cf. parameter-golf).
#   Lineage: v180 (batch-adv-norm + belief-trunk + 2 epochs) with the obs encoder swapped
#   from dense-8-patch to per-feature tokens and the dyn skip dropped. Everything else
#   (Beta-SDE policy, clip-higher, HL-Gauss critic, attached JEPA, LeJEPA) is unchanged.
#
# v180: THREE changes over v179.
#   (1) BATCH ADVANTAGE NORM like base PPO. The actor-active-SUBSET advantage z-norm was
#       high-variance when n_active was small (the warmup->actor boundary), injecting a
#       spurious advantage scale that amplified ratio drift / KL. v180 normalizes over the
#       FULL minibatch (cleanrl/ppo_continuous_action.py convention); inactive rows are
#       masked from the PG loss regardless, and post-warmup this is identical to base PPO.
#   (2) AGENT READS THE BELIEF TRUNK (B), reverting v175. belief_features_from_history now
#       returns the predictor trunk pred_final_norm(.) instead of pred_next_proj(trunk).
#       v175 had the agent consume a forward NEXT-state prediction that is action-blind at
#       decision time (the current action slot is neutral, but pred_next_proj was trained
#       with the real action) and optimized for predictability, not current-state control
#       sufficiency. The trunk fuses s_t with history a_{<t} -- a current-state filter, the
#       representation le-wm's controller actually reads. pred_next_proj stays as the JEPA
#       prediction head (WM target); only the agent's belief path drops it.
#   (3) agent_update_epochs 4 -> 2. No KL early-stop exists, so each extra PPO epoch
#       re-pushes Beta concentration on the same minibatch (the cross-epoch ratchet the
#       opt-review flagged as the entropy-collapse engine); halving epochs cuts the
#       per-iteration policy movement, slowing collapse and lowering KL.
#
# v179: IDIOMATIC OUTCOME-TOKEN EMBEDDING -- drop the outcome input RMSNorms. Single-axis
# change over v178 (clip-higher KEPT). v174-v178 fed the outcome tokens through
# proj(RMSNorm(raw)). RMSNorm is wrong for these inputs:
#   - reward_outcome_input_norm = RMSNorm(51) on the reward-bin SIMPLEX. A length-51
#     probability vector has RMS ~0.140 when peaked (one-hot) down to ~0.0196 when uniform,
#     so RMSNorm multiplies it by ~7x (confident) up to ~51x (uncertain). This (a) destroys
#     the concentration/entropy signal -- a confident and a diffuse reward prediction both
#     map to RMS=1 -- and (b) amplifies the uncertain case ~51x, blowing tiny per-bin noise
#     into the outcome token the agent reads.
#   - continuation_outcome_input_norm = RMSNorm(1) on a SCALAR is ~sign(x): for any
#     continuation in (0,1] it saturates to ~weight (a constant), so on HalfCheetah (never
#     terminates, continuation==1) the continuation token is literally constant and carries
#     zero information.
# Fix: project the raw simplex / scalar directly. reward_outcome_proj(reward_probs) is a
# soft embedding lookup over the reward codebook (sum_i p_i * W[:,i]) -- concentration
# preserved; continuation_outcome_proj(continuation) is a scalar gate on an embedding. Both
# the token-construction and the WM prediction TARGET use the same methods, so the change
# is symmetric (no train/target mismatch).
#
# Lineage: v176 (normalized Beta champion) + SIGReg-as-embed-norm cleanup + clip-higher
# (asymmetric 1-0.20 / 1+0.28, kept) + idiomatic outcome embedding. Normalized rewards.
#
# v176: STATE-DEPENDENT BETA POLICY ("beta + SDE"). Replaces v175's tanh-squashed
# Gaussian (global learnable log-std) with the unimodal Beta from iterthink_v24_dist,
# which found Beta "performs much better." TWO state-dependent heads emit the
# concentrations alpha = 1 + softplus(.), beta = 1 + softplus(.) (>= 1 => always
# unimodal); the latent z ~ Beta(alpha, beta) in (0, 1) maps to the action box by a
# CONSTANT linear rescale action = low + (high - low) * z whose log|Jacobian| cancels
# in the PPO ratio (log_det = 0, exactly as v24). The spread is now a STATE FUNCTION
# (both alpha and beta vary with the belief) -- this IS the state-dependent exploration
# ("SDE") character the experiment asked for; there is NO global log-std and NO separate
# noise module. In full v24 parity, all actor params (both concentration heads + the
# actor input norm) are clipped JOINTLY in a single grad-norm group (no separate std
# budget). NOTE on the v171 collapse concern: v176 deliberately re-adopts Beta
# despite v171's diagnosis that Beta entropy reg is structurally weak -- here ent_coef=0
# and the state-dependent concentrations are the exploration lever. Everything
# else (belief-(3) reads, symmetric outcome tokens, attached JEPA, HL-Gauss critic, WM,
# LeJEPA) is identical to v175. Single-axis change vs v175: the policy distribution.
#
# v175: AGENT READS THE PREDICTED LATENT (3), not the predictor trunk (2). Built on v174
# (un-detached/"attached" JEPA + symmetric outcome tokens) -- only the agent's READ point
# changes. le-wm grounding: its controller (eval.py rollout/criterion) consumes `emb` =
# projector output = the EXACT prediction target; it never reads the predictor's internal
# pre-pred_proj activations. cleanrl had the agent read pred_final_norm(trunk) BEFORE
# pred_next_proj (the docstring says so explicitly), i.e. one layer short of the space the
# JEPA MSE shapes. Hypothesis: un-detaching shaped (3) but the agent consumed (2), so the
# predictability gradient was laundered through pred_next_proj and could even make the
# trunk (2) carry less (easier-to-predict target => less work for the trunk). v175 applies
# pred_next_proj to the belief so the agent reads (3) -- the predictability-trained space --
# everywhere (acting, PPO, GAE, SVG). Predictor I/O is already in (3) (encoded summaries),
# so the recurrence loop is now 3->3 with the agent on 3, mirroring le-wm. Single change
# vs v174.
#
# v174: SYMMETRIC OUTCOME-TOKEN TREATMENT. v173 still hard-normed the 2 TARGET outcome
# tokens with outcome_token_norm (unit-RMS), while the prediction side and the 8 obs
# target tokens were un-normed -- a 2/10-token asymmetry in the JEPA MSE. v174 removes
# outcome_token_norm entirely, so target outcome tokens are proj(input_norm(label))
# with NO output norm, exactly matching the obs tokens (dyn_next_proj, un-normed) and
# le-wm (raw, un-normalized targets). Predictions are proj(rmsnorm(trunk)) for all 10
# tokens (unchanged). [v179: the outcome input embed norms are now REMOVED -- see top.]
# Now all 10 target tokens are regularized only softly toward N(0,1) by SIGReg, like
# le-wm. Otherwise identical to v173. Supersedes v173.
#
# v173: LE-WM PARITY FOR THE JEPA OBJECTIVE. v172 (like every prior version) detached
# BOTH the context AND the target of the latent-prediction MSE, so the encoder received
# ZERO forward-predictability gradient -- it was shaped only by SIGReg + outcome decode.
# le-wm detaches NEITHER: context and target are two slices of the same differentiable
# embedding, and SIGReg alone prevents collapse. v173 removes both .detach()s
# (context @ teacher_core_history, target @ mtp_latent_loss), so the encoder is now
# trained to produce forward-PREDICTABLE latents -- the suspected root of the
# obs_prediction_mse rebound / return plateau. No EMA target here (target uses the
# online encoder), and successor_td2_flow_loss re-detaches internally, so the ONLY new
# gradient path is JEPA-predictor -> encoder. Targets remain UN-normalized (le-wm style).
# Everything else identical to v172. Clean one-axis ablation: v171 -> v172 (+outcome
# tokens) -> v173 (+JEPA encoder gradient).
#
# v172: OUTCOME-TOKEN AGENT INPUT. The actor and critic now read the FULL belief
# trunk -- all NUM_LATENT_TOKENS (8 obs + 2 outcome) tokens -- instead of only the
# 8 obs tokens. Hypothesis: v171 plateaued (~4.5-4.6k @ 4.7M) because the agent
# discarded the 2 outcome (value/return-bearing) latent tokens the WM produces,
# starving the policy/value heads of the WM's outcome-predictive representation.
# AGENT_INPUT_DIM goes 512 -> 640. Prediction TARGETS are deliberately NOT
# RMS-normalized (le-wm uses raw-MSE targets with no normalization; SIGReg alone
# regularizes the embedding scale). Everything else is identical to v171.
#
# v171: TANH-SQUASHED GAUSSIAN POLICY (adopted from iterthink_v21) on top of the
# v170 PG-only belief-trunk baseline. Diagnosis behind the swap: v170's return
# plateaued because the Beta policy's concentration ran away unboundedly -> the
# policy collapsed toward determinism (actor_entropy fell monotonically past -7,
# no floor) -> exploration died. The codebase already documented (v135) that Beta
# entropy regularization is structurally too weak to stop this: dH/d(alpha+beta) ~
# -1/(alpha+beta) VANISHES as concentration grows, and the linear concentration
# penalty + cap were only ever wired to the (now-dormant) PrefPoE branch, never the
# MAIN policy. The WM (rew_mse~0, term_acc=1.0) and critic (EV~0.9) were healthy, so
# the bottleneck was exploration, not modeling.
#
# THE FIX: replace the main Beta with a tanh-squashed Gaussian whose spread is a
# single GLOBAL learnable log-std per action dim. A Gaussian's dH/dlogstd = 1 is
# CONSTANT, so the std cannot collapse without bound and (when desired) ent_coef can
# hold a genuine entropy floor. Mechanics: z ~ Normal(mean(s), exp(logstd));
# action = tanh(z) in [-1,1]; log-prob carries the tanh change-of-variables Jacobian
# log(1 - tanh(z)^2); entropy is the pre-squash Gaussian entropy (PPO-standard).
# `action_z` (stored for the PPO ratio recompute) is now the pre-squash z. PrefPoE
# is disabled (lambda_pref=0; Beta-PoE has no Gaussian analogue) and SVG/amortize
# remain off, so their Beta paths are dead. Everything else -- belief-trunk reads,
# HL-Gauss critic, world model, LeJEPA -- is unchanged. ent_coef kept at 0.0 (the
# iterthink_v21 default); the structural fix alone should arrest the collapse, and
# ent_coef is now an effective lever if more exploration is needed.
#
# --- inherited v170 / v169 design notes ---
...
#
# Defaults: svg_horizon=3, svg_ent_coef=1e-3, agent_update_epochs=2 (v180; was 4).
#
...
# v181: per-dimension obs tokenization -- each observation scalar becomes its OWN
# token (FT-Transformer feature tokenizer), so NUM_OBS_TOKENS now equals obs_dim.
# These are placeholders; main() rebinds them from the actual obs_dim BEFORE the
# Agent is built or any tensor is allocated (see the `# v181: derive token counts`
# block). All five constants are pure functions of NUM_OBS_TOKENS, so rebinding
# them together keeps the agent/predictor token geometry consistent.
NUM_OBS_TOKENS = 8
...
MTP_PRED_LEN = 4
SCALAR_EMBED_DIM = 32
AGENT_INPUT_DIM = NUM_LATENT_TOKENS * MODEL_DIM  # v172: actor+critic read obs+outcome tokens
AGENT_HIDDEN_DIM = 256
SIGREG_CHUNK_SIZE = 2048
SAMPLE_EPS = 1e-7
ATANH_EPS = 1e-6  # v171: keep atanh(action) finite when recovering pre-tanh z from a clamped action
...
class Args:
    ...
    agent_update_epochs: int = 2
    """number of PPO epochs per rollout iteration once agent training is enabled. v180:
    dropped 4 -> 2. With no KL early-stop, each extra epoch re-pushes the Beta concentration
    on the same minibatch (the cross-epoch ratchet the opt-review flagged); fewer epochs =
    less per-iteration policy movement = slower entropy collapse and lower KL."""
    norm_adv: bool = True
    """Toggles advantages normalization"""
    clip_coef: float = 0.2
    """PPO clip LOWER bound: ratio is clamped below at 1 - clip_coef (unchanged)."""
    clip_coef_high: float = 0.28
    """v178 clip-higher: PPO clip UPPER bound = 1 + clip_coef_high, decoupled from the
    lower bound (DAPO-style 0.20/0.28). A wider upper bound lets the policy keep moving
    probability mass toward advantageous actions before the surrogate saturates -- a mild
    antidote to the Beta concentration runaway / premature entropy collapse seen in v176."""
    ...
    """v182: weight on the joint-KL trust-region penalty that REPLACES the PPO ratio clip.
       Matches dreamer4's PMPO pmpo_kl_div_loss_weight=0.3. The actor objective is now the
       UNCLIPPED advantage-weighted PG term plus actor_kl_coef * KL(old‖new) summed over
       action dims -- a soft KL trust region instead of the hard asymmetric clip."""
    actor_kl_reverse: bool = True
    """v182: if True use KL(old‖new) (dreamer4 pmpo_reverse_kl=True default); else KL(new‖old).
       KL(old‖new) is mass-covering w.r.t. the behavior policy -- it explodes when the new
       policy gets SHARPER than old (low new-mass where old had mass), giving a stronger
       restoring force against Beta concentration collapse than the clip ever did."""
    vf_coef: float = 0.5
    ...
    v_min: float = -5.0
    """minimum value of the support (in symlog space). v178 reverted to -5 (v176 grid):
    rewards are NormalizeReward'd again, so returns live in normalized symlog space."""
    v_max: float = 5.0
    """maximum value of the support (in symlog space). v178 reverted to 5 (v176 grid)
    since raw rewards regressed and reward normalization is restored."""
    ...
    """minimum symlog reward support for the learned reward outcome token. v178 reverted
    to -3 (v176 grid): NormalizeReward restored, so per-step rewards are ~unit-std again."""
    reward_v_max: float = 3.0
    """maximum symlog reward support for the learned reward outcome token. v178 reverted
    to 3 (v176 grid) since reward normalization is restored."""
    ...
    # v171: DISABLED. PrefPoE is a closed-form Beta product-of-experts; it has no
    # meaning for the tanh-Gaussian policy. lambda_pref=0 makes `_action_distributions`
    # short-circuit to (main, main, main), so only the Gaussian main policy is used.
...
def make_env(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        ...
        # v178 (reverted): raw rewards regressed vs the normalized champion (v176), so
        # reward normalization + the [-10,10] reward clip are restored. The symlog
        # HL-Gauss critic regresses NORMALIZED discounted returns again; support widths
        # are reverted to the v176 grid (value +-5, reward +-3) accordingly.
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_q_heads, num_kv_heads, ffn_mult=2):
        ...
        # v181: dropped the learnable `resid_mix` raw-input (x0) skip. The dyn stack
        # is now a clean pre-norm residual transformer -- x = x + sublayer(norm(x)).
...
class Agent(nn.Module):
    def __init__(
        ...
        # v177 cleanup: obs ingestion is just a projection into token space. No in-agent
        # hard embed norm: the embedding regularization is SIGReg (applied to the output
        # latent summary tokens), which shapes scale/isotropy softly toward N(0,1) WITHOUT
        # pinning any token to unit RMS -- the LeJEPA-idiomatic embed norm. Dropped:
        #   obs_input_norm   (redundant; the NormalizeObservation wrapper standardizes obs)
        #   dyn_embed_norm   (back-to-back with obs_token_norm; RMSNorm(RMSNorm(x)) no-op)
        #   obs_token_norm   (a hard post-proj unit-RMS norm -- SIGReg replaces it)
        # The encoder's internal block pre-norms keep the un-normed input well-conditioned.
        # v181: FT-Transformer-style per-feature tokenizer. Each observation scalar
        # becomes its own token via a learned per-feature (weight, bias) embedding
        # pair: token_i = obs_i * W[i] + b[i], with W, b in R^{obs_dim x MODEL_DIM}.
        # This gives every sensor a DISTINCT content direction (so the backbone can
        # tell feature 3 from feature 7 even at equal scalar values) -- the continuous
        # analog of an LLM vocab embedding, where each token-id maps to a distinct
        # row. A purely shared scalar->D projection would be degenerate (all tokens
        # colinear, differing only in magnitude); RoPE rotates q/k not values, so it
        # could not disambiguate them. Position/order is carried solely by RoPE in
        # dyn_layers -- no learned positional embedding (feature identity lives in W/b).
        # NUM_OBS_TOKENS == obs_dim (rebound in main()).
        self.obs_feature_weight = nn.Parameter(torch.empty(obs_dim, MODEL_DIM))
        self.obs_feature_bias = nn.Parameter(torch.empty(obs_dim, MODEL_DIM))
        ...
        # v176: unimodal Beta policy (adopted from iterthink_v24_dist, which found it
        # "performs much better" than the tanh-Gaussian). In parity with v24: TWO
        # state-dependent heads emit the concentrations directly,
        # alpha = 1 + softplus(.), beta = 1 + softplus(.) (>= 1 -> always unimodal,
        # finite boundary density). The action latent z ~ Beta(alpha, beta) lives in
        # (0, 1) and maps to the action box by a CONSTANT linear rescale
        # action = low + (high - low) * z, whose log|Jacobian| is a constant that
        # drops out of the PPO ratio (so log_det == 0, exactly as in v24). Because the
        # concentrations are state functions, the spread is STATE-DEPENDENT (the "SDE"
        # character) -- no global log-std and no separate noise module. The two heads
        # are clipped as their own grad-norm group (see the actor update step).
        self.actor_alpha_head = ReluSqRMSHead(AGENT_INPUT_DIM, AGENT_HIDDEN_DIM, act_dim, output_std=0.01)
        self.actor_beta_head = ReluSqRMSHead(AGENT_INPUT_DIM, AGENT_HIDDEN_DIM, act_dim, output_std=0.01)
        ...
        # v177 cleanup: removed dead ret_ema_lo/ret_ema_hi buffers (v164 Dreamer-v3
        # return-normalization EMAs). They were never read or updated anywhere in the
        # file -- pure dead state.
        ...
        # v179: NO input norm on the outcome tokens. The reward-bin simplex and the
        # continuation scalar are projected directly. proj(reward_probs) is a soft embedding
        # lookup over the reward codebook (concentration preserved); RMSNorm on a simplex
        # would fold out the concentration signal and amplify diffuse predictions ~51x, and
        # RMSNorm(1) on the scalar continuation collapses it to a near-constant. Still a
        # non-unit embedding, so the symmetry with the (also non-unit) obs latent target
        # dyn_next_proj(dyn_final_norm(.)) is preserved.
        ...
        # v185: NO outcome_token_norm (dropped from v184). The hard per-token RMSNorm removed
        # the per-sample magnitude DOF that the un-detached reward-CE wants for confidence, was
        # redundant with the un-detached JEPA-MSE target (which already bounds scale against CE
        # blow-up), and was asymmetric with the un-normed obs JEPA targets (dyn_next_proj output
        # is not unit-normed on the target side). Outcome tokens are now proj(label) directly,
        # symmetric with obs and le-wm-faithful; scale is set by the AE-CE / JEPA-MSE equilibrium.

    def _action_distribution(self, agent_input):
        # v176: state-dependent unimodal Beta (v24 parity). Two heads emit the
        # concentrations directly; `1 + softplus` keeps alpha, beta >= 1 (always
        # unimodal, finite density at the boundaries). The latent z ~ Beta lives in
        # (0, 1); the constant linear rescale to the action box is applied in
        # `_z_to_action` (Jacobian is a constant and cancels in the PPO ratio, so the
        # log-prob below omits it).
        alpha = 1.0 + F.softplus(self.actor_alpha_head(agent_input))
        beta = 1.0 + F.softplus(self.actor_beta_head(agent_input))
    ...
    def _z_to_action(self, action_z):
        # v176: constant linear rescale of the Beta latent z in (0, 1) onto the
        # action box [low, high]. Affine -> Jacobian is constant in z, so it cancels
        # in the PPO importance ratio (the log-prob omits it). The env wrapper's
        # clamp to [-1, 1] is a no-op since z in (0, 1) maps inside the box.
        return self.action_low + (self.action_high - self.action_low) * action_z

    def _action_to_z(self, action):
        # v176: inverse linear rescale, action box -> (0, 1). Clamped to the open
        # interval so a boundary action stays a valid Beta support point. Used for
        # the special non-policy action sources (warmup midpoint, planner, random
        # bootstrap), whose steps are excluded from the PG update anyway.
    ...
    def _beta_action_logprob_entropy(self, dist, action=None, action_z=None, sum_logprob=True):
        # v176: unimodal Beta. `action_z` is the latent z ~ Beta in (0, 1);
        # `action = low + (high - low) * z`. The affine rescale has a constant
        # log|Jacobian| that cancels in the PPO ratio, so the log-prob is simply
        # log Beta(z) (no Jacobian term, exactly as in iterthink_v24). Sampled z is
        # clamped to the open interval so log_prob/entropy stay finite at boundaries.
        if action_z is None:
            if action is not None:
                action_z = self._action_to_z(action)
            else:
                action_z = dist.rsample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
    ...
    def _action_mean_from_dist(self, dist):
        # Deterministic action = rescaled Beta mean alpha / (alpha + beta) in (0, 1).
        return self._z_to_action(dist.mean)

    def _action_std_from_dist(self, dist):
        # Beta std in latent (0, 1) space (diagnostic logging only).
        return dist.stddev

    def _encode_dynamics_tokens(self, x):
        batch = x.shape[0]
        # v181: per-feature tokenization. (batch, obs_dim) -> (batch, obs_dim, MODEL_DIM)
        # via token_i = obs_i * W[i] + b[i]. obs_dim == NUM_OBS_TOKENS.
        obs_flat = x.reshape(batch, -1)
        dyn_tokens = obs_flat.unsqueeze(-1) * self.obs_feature_weight + self.obs_feature_bias

        # v181: clean pre-norm residual stack (dropped the v180 dyn_x0 raw-input skip).
    ...
    def _reward_outcome_token(self, reward_probs):
        # v179: direct projection of the reward-bin simplex -- a soft embedding lookup over
        # the reward codebook (sum_i p_i * W[:,i]). Non-unit, no input norm.
        return self.reward_outcome_proj(reward_probs)

    def _continuation_outcome_token(self, continuations):
        continuation_input = continuations.unsqueeze(-1)
        # v179: direct projection of the continuation scalar (scalar gate on an embedding).
        return self.continuation_outcome_proj(continuation_input)
    ...
    def _outcome_tokens_from_labels(self, reward_probs, continuations):
        ...
        # v185: no outcome_token_norm -- direct proj(label) stack (see __init__ note).
    ...
    def _actor_features_from_latents(self, latent_tokens):
        agent_tokens = latent_tokens[:, :NUM_LATENT_TOKENS]  # v172: obs + outcome tokens
        return self.actor_input_norm(agent_tokens.reshape(agent_tokens.shape[0], -1))

    def _critic_features_from_latents(self, latent_tokens):
        agent_tokens = latent_tokens[:, :NUM_LATENT_TOKENS]  # v172: obs + outcome tokens
        return self.critic_input_norm(agent_tokens.reshape(agent_tokens.shape[0], -1))
    ...
    def target_value(self, latent, hl_support):
        ...
        """
        agent_tokens = latent[:, :NUM_LATENT_TOKENS]  # v172: obs + outcome tokens
        critic_input = self.target_critic_input_norm(agent_tokens.reshape(agent_tokens.shape[0], -1))
    ...
    def online_value(self, latent, hl_support):
        ...
        """
        agent_tokens = latent[:, :NUM_LATENT_TOKENS]  # v172: obs + outcome tokens
        critic_input = self.critic_input_norm(agent_tokens.reshape(agent_tokens.shape[0], -1))
    ...
    def belief_features_from_history(self, latent_history, action_history):
        ...
        # v180 (B, reverting v175): the agent reads the predictor TRUNK (2) =
        # pred_final_norm(pred_tokens), NOT pred_next_proj(trunk) (3). v175 had the agent
        # consume the forward NEXT-state prediction, which (a) is action-blind at decision
        # time -- the current slot holds a NEUTRAL action, but pred_next_proj was trained
        # with the REAL action there -- and (b) optimizes the read representation for
        # next-step predictability rather than current-state sufficiency for control.
        # The trunk fuses the current obs s_t with the obs/action history a_{<t}: it is a
        # history-conditioned CURRENT-state belief (a one-step filter), which is what
        # le-wm's controller actually reads (its encoder current-state encoding), never a
        # forward prediction. pred_next_proj remains the JEPA prediction head (line ~2074),
        # trained on the next-latent target with the real action -- only the AGENT'S belief
        # path drops it.
    ...
    def get_imagined_raw_action_mean(self, summary_tokens):
        # v176: diagnostic only. "Raw" pre-rescale action center = the Beta latent
        # mean alpha / (alpha + beta) in (0, 1) (no linear rescale applied).
        latent_tokens = summary_tokens
        if self.detach_world_model_from_agent:
            latent_tokens = latent_tokens.detach()
        agent_input = self._actor_features_from_latents(latent_tokens)
        return self._action_distribution(agent_input).mean
    ...
    def imagined_actor_objective(
        ...
        for _ in range(horizon):
            ...
            action = self._z_to_action(action_z)  # v171: tanh squash (no [0,1] clamp; z is real-valued)
...
if __name__ == "__main__":
    ...
    # v181: derive token counts from obs_dim. Each observation scalar is its own
    # token (per-feature tokenizer), so NUM_OBS_TOKENS == obs_dim. Rebind here --
    # at module scope, before the Agent is built or any rollout tensor is allocated
    # -- so every downstream reference (agent heads, predictor space axis, RoPE
    # caches, SIGReg, rollout buffers) sees the correct geometry.
    NUM_OBS_TOKENS = int(np.array(envs.single_observation_space.shape).prod())
    NUM_LATENT_TOKENS = NUM_OBS_TOKENS + NUM_OUTCOME_TOKENS
    NUM_RECURRENT_TOKENS = NUM_LATENT_TOKENS
    NUM_SIGREG_TOKENS = NUM_LATENT_TOKENS
    AGENT_INPUT_DIM = NUM_LATENT_TOKENS * MODEL_DIM
    print(
        f"[v181] per-dimension tokenizer: obs_dim={NUM_OBS_TOKENS} obs tokens "
        f"+ {NUM_OUTCOME_TOKENS} outcome = {NUM_LATENT_TOKENS} latent tokens, "
        f"AGENT_INPUT_DIM={AGENT_INPUT_DIM}"
    )
    ...
    # gradient. PrefPoE stays off, so its head is frozen; the Beta concentration
    # heads and actor_input_norm are LEARNED, but by a SEPARATE actor_optimizer.
    agent.actor_pref_beta.requires_grad_(False)
    # v176: actor params = both Beta concentration heads + the shared actor input
    # norm (NOT actor_pref_beta). No global log-std any more (spread is state-dependent).
    # v24 parity: a single clip group over all actor params (no separate std budget).
    actor_params = [
        p
        for p in (
    ...
    grad_clip_groups = [
        (
            "wm",
            [
                agent.obs_feature_weight,
                agent.obs_feature_bias,
                *module_parameters(agent.dyn_layers, agent.dyn_final_norm, agent.dyn_next_proj),
                *module_parameters(
        ...
        # v176: NOTE the actor heads are intentionally absent from this list. These
        # groups are clipped only on the WM/critic `optimizer` step, which carries no
        # actor grads (actor params live in the separate actor_optimizer). The actor's
        # OWN clip -- a single joint clip over all actor_params (v24 parity, no separate
        # std budget) -- is applied at the actor update step.
            ...
            for _ in range(args.imagine_horizon):
                ...
                if diagnostic_n > 0:
                    ...
                    try:
                        ...
                        # v171: pre-squash z is real-valued; no [0,1] clamp.
    ...
    # v182: store the OLD (behavior) Beta concentrations per action dim so the actor
    # update can form the analytic joint KL(old‖new) trust-region penalty that
    # replaces the PPO clip. Captured at acting time (detached), exactly like
    # behavior_logprobs, so the "old" policy is the data-generating policy.
    # Initialized to ONES (not zeros): at the warmup->actor boundary iteration a
    # minibatch can mix active rows with not-yet-written inactive rows; Beta(0,0) is
    # invalid (raises / NaNs), whereas Beta(1,1) is the valid uniform and its KL is
    # masked out of the penalty by mb_active anyway.
    behavior_alphas = torch.ones((args.num_steps, args.num_envs) + envs.single_action_space.shape, device=device)
    behavior_betas = torch.ones((args.num_steps, args.num_envs) + envs.single_action_space.shape, device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if amortized_actor_active:
                ...
                with torch.no_grad():
                    ...
                    # v171: sample the pre-squash z inside the helper (no [0,1] clamp);
                    # action = tanh(z), logprob carries the tanh Jacobian.
                    action, action_z, logprob, _ = agent._beta_action_logprob_entropy(
                        dist, sum_logprob=False
                    )
                    ...
                    # v182: stash the behavior Beta concentrations for the joint-KL penalty.
                    behavior_alphas[step] = dist.concentration1
                    behavior_betas[step] = dist.concentration0
        ...
        # v182: behavior Beta concentrations for the joint-KL trust region.
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                teacher_core_history = torch.cat(
                    # v173 (le-wm parity): context NOT detached -> the JEPA prediction
                    # loss now flows gradient into the encoder through the context, like
                    # le-wm's ctx_emb = emb[:, :n_ctx]. successor_td2_flow_loss re-detaches
                    # its inputs internally, so the only new grad path is JEPA->encoder.
                    [initial_core_summary.unsqueeze(1), target_core_summaries[:, :-1]],
                    dim=1,
                )
                ...
                # v184: detach_summary=False -- the reward CE / continuation BCE now flow into
                # the TARGET (encoder) outcome token embedding, label-grounding it. This is the
                # anti-collapse force that replaces SIGReg for the outcome tokens (which are
                # removed from the SIGReg pool below). Effective weight on this grounding is
                # dyn_reward_coef*0.5 = 0.125 (and dyn_termination_coef*0.5) > the 0.09 SIGReg
                # it replaces. The predicted (MTP) decode at 4342 stays detached (readout only).
                ...
                for mtp_idx in range(MTP_PRED_LEN):
                    ...
                    mtp_latent_loss = F.mse_loss(
                        offset_pred_latents,
                        offset_target_latents,  # v173 (le-wm parity): target NOT detached
                        reduction="none",
                    ).mean(dim=(-1, -2))
                ...
                # v184: SIGReg is now OBS-ONLY. The 2 outcome tokens are removed from the pool;
                # they are anti-collapsed by the un-detached label decode instead (see 4362).
                # This makes SIGReg regularize only the (unlabeled, full-rank) state tokens --
                # which is also the le-wm-faithful design (le-wm SIGReg's the state embedding and
                # has no reward in its representation at all).
                dyn_sigreg_loss = masked_token_sigreg(
                    target_future_summaries[:, :, :NUM_OBS_TOKENS, :], latent_weight > 0.0
                )
        ...
        actor_entropies = []
        actor_losses = []
        actor_kls = []  # v182: joint KL(old‖new) trust-region penalty per minibatch
        actor_concentrations = []  # v182: mean α+β of the live actor (collapse monitor)
            ...
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    if n_active > 0:
                        ...
                        if args.norm_adv:
                            # v180: normalize over the FULL minibatch, exactly like base
                            # PPO (cleanrl/ppo_continuous_action.py) -- NOT the actor-active
                            # subset. The subset stats were high-variance whenever n_active
                            # was small (the warmup->actor boundary minibatch), injecting a
                            # spurious advantage scale that amplified ratio drift. Over the
                            # full 1024-row minibatch the std is stable, so no singleton-NaN
                            # guard is needed; post-warmup mb_active is all-True so this is
                            # identical to base PPO, and inactive rows are masked from the
                            # PG loss below regardless.
                            mb_adv = (mb_adv - mb_adv.mean()) / (mb_adv.std() + 1e-8)
                        # v182: UNCLIPPED advantage-weighted PG. The hard asymmetric clip
                        # (v178 clip-higher) is replaced by the joint-KL trust region below
                        # (dreamer4 PMPO style). The clip only rate-limited per-step ratio
                        # drift; the KL penalty is a smooth distributional trust region that
                        # also penalizes spread reduction (anti-collapse).
                        pg_per = -mb_adv * ratio
                        pg_loss = (pg_per * mb_active).sum() / n_active
                        entropy_term = (pg_entropy * mb_active).sum() / n_active

                        # v182: joint KL(old‖new) trust region, summed over action dims
                        # (the factorized Beta policy's joint KL). old = behavior policy.
                        old_dist = Beta(b_behavior_alphas[mb_inds], b_behavior_betas[mb_inds])
                        if args.actor_kl_reverse:
                            kl_per_dim = kl_divergence(old_dist, actor_dist)  # KL(old‖new)
                        else:
                            kl_per_dim = kl_divergence(actor_dist, old_dist)  # KL(new‖old)
                        kl_joint = kl_per_dim.sum(-1)
                        kl_penalty = (kl_joint * mb_active).sum() / n_active
                        ...
                        for p, g in zip(actor_params, actor_grads):
                            p.grad = g
                        # v176: v24 parity -- single joint clip over all actor params.
                        ...
                        with torch.no_grad():
                            ...
                            # v178 clip-higher: count ratios outside the ASYMMETRIC band
                            # [1 - clip_coef, 1 + clip_coef_high].
                            clipped_mask = (ratio < 1.0 - args.clip_coef) | (ratio > 1.0 + args.clip_coef_high)
                            clipfracs.append(
                                (clipped_mask.float() * mb_active.float()).sum().item()
                                / max(n_active, 1)
                            )
                        actor_losses.append(actor_loss.item())
                        actor_entropies.append(entropy_term.item())
                        actor_kls.append(kl_penalty.item())
                        with torch.no_grad():
                            conc = (actor_dist.concentration1 + actor_dist.concentration0)
                            actor_concentrations.append(
                                (conc.mean(-1) * mb_active).sum().item() / max(n_active, 1)
                            )
        ...
        # plateau. v177: rewards are RAW (no NormalizeReward), so this is the raw
        # discounted-return target -- directly comparable in scale to the raw GAE
        # returns the symlog HL-Gauss critic regresses.
        ...
        if actor_losses:
            ...
            writer.add_scalar("losses/actor_kl_penalty", np.mean(actor_kls), global_step)
            writer.add_scalar("losses/actor_concentration", np.mean(actor_concentrations), global_step)
