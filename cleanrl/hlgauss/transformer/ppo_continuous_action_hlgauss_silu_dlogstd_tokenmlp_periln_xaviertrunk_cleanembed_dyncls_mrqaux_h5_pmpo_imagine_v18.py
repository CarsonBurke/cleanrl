# PMPO + HL-Gauss with an LLM-style shared self-attention backbone.
#
# Key ideas:
...
# - optional PMPO actor loss on top of PPO; default is standard normalized-advantage PPO
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        self.dyn_actor_proj = xavier_init_linear(nn.Linear(MODEL_DIM, MODEL_DIM))
        self.dyn_sde_proj = xavier_init_linear(nn.Linear(MODEL_DIM, MODEL_DIM))
        self.dyn_critic_proj = xavier_init_linear(nn.Linear(MODEL_DIM, MODEL_DIM))
    ...
    def get_imagined_action_dist(self, dyn_features):
        actor_features = F.silu(self.dyn_actor_proj(dyn_features))
        sde_features = F.silu(self.dyn_sde_proj(dyn_features))
    ...
    def get_imagined_value_logits(self, dyn_features):
        critic_features = F.silu(self.dyn_critic_proj(dyn_features))
        return self.critic_head(critic_features)

    def get_imagined_value(self, dyn_features, hl_support):
        return hl_support.to_scalar(self.get_imagined_value_logits(dyn_features))
...
if __name__ == "__main__":
    ...
    def build_dream_targets(initial_dyn):
        states = []
        rewards_hat = []
        continues_hat = []
        dyn_state = initial_dyn
        with torch.no_grad():
            for _ in range(args.imagine_horizon):
                states.append(dyn_state)
                dream_dist = agent.get_imagined_action_dist(dyn_state)
                dream_action = torch.clamp(dream_dist.sample(), action_low, action_high)
                pred_next_dyn, pred_reward_logits, pred_done_logits = agent.dynamics_step(dyn_state, dream_action)
                rewards_hat.append(reward_support.to_scalar(pred_reward_logits))
                continues_hat.append(1.0 - torch.sigmoid(pred_done_logits))
                dyn_state = pred_next_dyn
            bootstrap = agent.get_imagined_value(dyn_state, hl_support)
    ...
    def dreamed_actor_objective(initial_dyn):
        dyn_state = initial_dyn
        rewards_hat = []
        continues_hat = []
        for _ in range(args.imagine_horizon):
        ...
        with torch.no_grad():
            bootstrap = agent.get_imagined_value(dyn_state.detach(), hl_support)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                for horizon_idx in range(args.dyn_horizon):
                    ...
                    with torch.no_grad():
                        target_next_dyn = agent.get_dyn_target(future_next_obs)
                ...
                if global_step >= args.imagine_start_step:
                    initial_dyn = agent.get_dyn_target(b_obs[mb_inds])
                    dream_states, dream_returns = build_dream_targets(initial_dyn.detach())
                    ...
                    optimizer.zero_grad()
                    initial_dyn = agent.get_dyn_target(b_obs[mb_inds])
                    imagine_actor_loss, imagine_return = dreamed_actor_objective(initial_dyn)
