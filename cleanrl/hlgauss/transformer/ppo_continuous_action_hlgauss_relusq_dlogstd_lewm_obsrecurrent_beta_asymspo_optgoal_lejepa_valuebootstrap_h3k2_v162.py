# v162: value-bootstrapped lookahead-return imagined-goal MPC.
#
# Diagnosis of v161's failure (it scored ~random, -400 on HalfCheetah): v161 set
# planner_terminal_value_coef=0, so the GoalSolver optimized a PURE H+K-step
# discounted reward sum with no value bootstrap. Diagnostics showed why this is a
# dead objective:
#   - reward_action_sensitivity ~= 0.0005 (and shrinking): the one-step reward
#     decode is ~flat in the action-controlled latent directions. HalfCheetah
#     reward is velocity-based; a single action barely moves THIS step's reward,
#     so a finite reward-only sum is flat and the action-gradient collapses to
#     ~1e-4 (measured exec_action_grad_norm). The planner sat at model_return~=0.
#   - latent_action_sensitivity ~= 0.27 (stable): the predicted next-latent IS
#     richly action-sensitive. The action's payoff lives in the FUTURE state, not
#     this step's scalar reward. The only operator that turns that action-sensitive
#     latent into a return signal is a value function V(s_H).
#
# v162 fix: re-enable the terminal-value bootstrap (planner_terminal_value_coef=1)
# so the GoalSolver maximizes  sum_t gamma^t (prod continue) r_hat_t + gamma^H V(s_H).
# V is `self.critic`, the working HL-Gauss critic: CE-trained on hl_support-projected
# real returns and read out via to_scalar (the same recipe as the high-scoring
# pmpo_d4_hlgauss variants). This is distinct from `successor_value` (the TD2-CFM
# flow head, MAE~4.8) which the planner does NOT use. The value provides the
# long-horizon, action-sensitive return signal that one-step reward structurally
# cannot, restoring a usable gradient off the already-good latent (the SVG/Dreamer
# value-gradient mechanism).
#
# Also: train self.critic DURING the WM warmup phase (not only after planner start)
# so the bootstrap value is calibrated by the time the planner begins to use it,
# instead of cold-starting from a random-init critic.
#
# Defaults: H=3, K=2, S=4, 8 GoalSolver Adam steps, 16 ActionSolver Adam steps,
# planner_terminal_value_coef=1.0.
#
...
class Args:
    ...
    planner_terminal_value_coef: float = 1.0
    """v162: terminal HL-Gauss critic bootstrap weight; gamma^H V(s_H) added to the GoalSolver return so the action-sensitive latent yields a usable gradient (one-step reward alone is action-flat)"""
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # v162: fit the HL-Gauss critic every iteration, including during WM warmup,
        # so the planner's gamma^H V(s_H) bootstrap is calibrated before planner_start_step
        # rather than cold-starting from a random-init critic. Critic fitting is the only
        # agent-side update in this pure-MPC variant.
