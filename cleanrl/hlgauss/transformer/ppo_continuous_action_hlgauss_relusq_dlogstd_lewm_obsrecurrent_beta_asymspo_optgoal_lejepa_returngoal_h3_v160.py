...
#
# TD-MPC2 does not plan from short-horizon reward alone; it adds terminal Q.
# v160 keeps the v159/v156 two-stage "choose high-return imagined latent, then
# GradientSolver to g" structure, but GoalSolver scores
#   sum_h gamma^h r_hat_h + gamma^H V_hat(z_H)
# using the existing HL-Gauss critic as terminal return prediction. A coefficient
# and optional clamp are exposed because this is a single critic, not TD-MPC2's
# conservative Q ensemble.
#
...
class Args:
    ...
    """weight on terminal HL-Gauss critic return in the GoalSolver objective"""
...
class Agent(nn.Module):
    ...
    def cost_from_action(
        ...
        Returns a tensor of shape (B,) with reward/return-probe cost
            C = -(sum_{k<H} gamma^k r_hat_k + c_v gamma^H V_hat(z_H))
    ...
    def run_goal_embedding_solver(start_summary, init_actions):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                terminal_value_logits = agent.critic(agent._critic_features_from_latents(final_pred))
                terminal_value = hl_support.to_scalar(terminal_value_logits)
                if args.planner_terminal_value_clip > 0.0:
                    terminal_value = terminal_value.clamp(
                        -args.planner_terminal_value_clip,
                        args.planner_terminal_value_clip,
                    )
                goal_return = -final_goal_energy
                goal_value = terminal_value
