...
class Agent(nn.Module):
    ...
    def belief_features_from_history(self, latent_history, action_history):
        ...
        # v175: read the PREDICTED latent (3) = pred_next_proj(trunk), NOT the
        # pre-projection trunk (2). The JEPA MSE target lives in the pred_next_proj
        # space (offset-0 of the MTP prediction), so this makes the agent consume the
        # exact representation that is trained for predictability -- matching le-wm,
        # whose controller reads emb (= the prediction target), never the predictor's
        # internal pre-pred_proj activations. The predictor's input context is already
        # in this space (encoded summaries), so the loop is now 3->3 with the agent on 3.
        belief_features = self.pred_next_proj(belief_features)
