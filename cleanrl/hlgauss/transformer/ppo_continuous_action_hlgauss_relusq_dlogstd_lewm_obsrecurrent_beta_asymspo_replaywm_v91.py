...
# - v90: repairs generated outcome tokens before recurrent dream reuse by decoding
#   predicted reward/termination and re-embedding through the canonical learned
#   outcome projection, matching Dreamer-style scalar reward feedback
# - v91: samples world-model minibatches and dream prompts from a small rollout
#   replay buffer instead of only the latest on-policy rollout, matching D4's
#   replay-trained imagination loop and reducing distribution-chasing collapse
import os
import random
import sys
import time
...
class Args:
    ...
    replay_size: int = 16
    """number of recent full rollouts used for world-model training and dream prompts"""
...
class Agent(nn.Module):
    ...
    def repair_summary_outcomes(self, summary_tokens, reward_logits, termination_logits, reward_support):
        """Put generated outcome slots back on the learned target-token manifold."""
        leading_shape = summary_tokens.shape[:-2]
        flat_summary = summary_tokens.reshape(-1, NUM_LATENT_TOKENS, MODEL_DIM)
        flat_reward_logits = reward_logits.reshape(-1, self.reward_num_bins)
        flat_termination_logits = termination_logits.reshape(-1)
        reward_scalar = reward_support.to_scalar(flat_reward_logits)
        reward_probs = reward_support.project(reward_scalar)
        continues = 1.0 - torch.sigmoid(flat_termination_logits)
        outcome_tokens = self._outcome_tokens_from_labels(reward_probs, continues)
        repaired = torch.cat([flat_summary[:, :NUM_OBS_TOKENS], outcome_tokens], dim=1)
        return repaired.reshape(*leading_shape, NUM_LATENT_TOKENS, MODEL_DIM)
...
if __name__ == "__main__":
    ...
    def build_dream_batch(prompt_summary_history, prompt_action_history, prompt_valids):
        ...
        with torch.no_grad():
            for _ in range(args.imagine_horizon):
                ...
                recurrent_next_summary = agent.repair_summary_outcomes(
                    pred_next_summary,
                    pred_reward_logits,
                ...
                summary_history.append(recurrent_next_summary.detach())
    ...
    transition_replay = deque(maxlen=max(1, args.replay_size))

    def make_rollout_record():
        return dict(
            obs=obs.detach().cpu().clone(),
            next_obses=next_obses.detach().cpu().clone(),
            prev_reward_probs=prev_reward_probs.detach().cpu().clone(),
            prev_continues=prev_outcome_continues.detach().cpu().clone(),
            rewards=rewards.detach().cpu().clone(),
            actions=transition_actions.detach().cpu().clone(),
            terminations=transition_terminations.detach().cpu().clone(),
            boundaries=transition_boundaries.detach().cpu().clone(),
            valids=transition_valids.detach().cpu().clone(),
        )

    def sample_replay_record():
        return random.choice(transition_replay)

    def replay_tensor(record, key):
        return record[key].to(device, non_blocking=True)

    def sample_replay_starts(batch_size):
        step_inds = torch.randint(0, args.num_steps, (batch_size,), device=device)
        env_inds = torch.randint(0, args.num_envs, (batch_size,), device=device)
        return step_inds, env_inds

    @torch.no_grad()
    def build_replay_dream_prompt_context():
        record = sample_replay_record()
        rollout_obs = replay_tensor(record, "obs")
        rollout_prev_reward_probs = replay_tensor(record, "prev_reward_probs")
        rollout_prev_continues = replay_tensor(record, "prev_continues")
        rollout_actions = replay_tensor(record, "actions")
        rollout_boundaries = replay_tensor(record, "boundaries")

        prompt_len = max(1, min(args.dream_prompt_len, args.pred_context))
        step_inds, env_inds = sample_replay_starts(args.batch_size)
        prompt_valids = step_inds >= (prompt_len - 1)
        ...
        for offset in range(prompt_len):
            ...
            prompt_summary_history.append(
                agent.encode_summary_with_outcomes(
                    rollout_obs[safe_hist_step, env_inds],
                    rollout_prev_reward_probs[safe_hist_step, env_inds],
                    rollout_prev_continues[safe_hist_step, env_inds],
                ).detach()
            )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        transition_replay.append(make_rollout_record())
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_size = len(mb_inds)
                wm_record = sample_replay_record()
                wm_obs = replay_tensor(wm_record, "obs")
                wm_next_obses = replay_tensor(wm_record, "next_obses")
                wm_prev_reward_probs = replay_tensor(wm_record, "prev_reward_probs")
                wm_prev_continues = replay_tensor(wm_record, "prev_continues")
                wm_rewards = replay_tensor(wm_record, "rewards")
                wm_actions = replay_tensor(wm_record, "actions")
                wm_terminations = replay_tensor(wm_record, "terminations")
                wm_boundaries = replay_tensor(wm_record, "boundaries")
                wm_valids = replay_tensor(wm_record, "valids")
                mb_step_inds, mb_env_inds = sample_replay_starts(mb_size)
                ...
                future_actions = wm_actions[safe_step_inds, env_inds]
                future_rewards = wm_rewards[safe_step_inds, env_inds]
                future_terminations = wm_terminations[safe_step_inds, env_inds]
                future_boundaries = wm_boundaries[safe_step_inds, env_inds]
                future_valids = wm_valids[safe_step_inds, env_inds]
                future_next_obs = wm_next_obses[safe_step_inds, env_inds]

                initial_summary = agent.encode_summary_with_outcomes(
                    wm_obs[mb_step_inds, mb_env_inds],
                    wm_prev_reward_probs[mb_step_inds, mb_env_inds],
                    wm_prev_continues[mb_step_inds, mb_env_inds],
                )
                ...
                for horizon_idx in range(args.dyn_horizon):
                    ...
                    closed_loop_context_summary = agent.repair_summary_outcomes(
                        closed_loop_summary,
                        closed_loop_reward_logit,
                        closed_loop_termination_logit,
                        reward_support,
                    )
                    summary_history.append(closed_loop_context_summary.detach())
        ...
        if (not world_model_only) and use_imagination_updates and global_step >= args.imagine_start_step:
            with torch.no_grad():
                prompt_summary_history, prompt_action_history, prompt_valids = build_replay_dream_prompt_context()
        ...
        writer.add_scalar("dynamics/replay_size", len(transition_replay), global_step)
