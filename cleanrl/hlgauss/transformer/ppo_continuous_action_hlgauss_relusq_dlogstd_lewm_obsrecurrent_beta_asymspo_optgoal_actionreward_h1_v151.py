# v151: pure optimal-goal MPC with action-aware transition reward.
#
# v149/v150 plateau near zero because the MPC return objective decodes reward
# from predicted outcome tokens. Those tokens are part of the optimized latent
# summary and can become optimistic without forcing physically useful next obs.
# v151 keeps the pure-MPC controller but computes planner reward with a separate
# action-aware transition readout r(z_obs_t, a_t, z_obs_{t+1}).
#
...
class Agent(nn.Module):
    def __init__(
        ...
        transition_reward_dim = 2 * NUM_OBS_TOKENS * MODEL_DIM + 2 * act_dim + 1
        self.transition_reward_input_norm = RMSNorm(transition_reward_dim)
        self.transition_reward_head = ReluSqRMSHead(
            transition_reward_dim,
            AGENT_HIDDEN_DIM,
            reward_num_bins,
            output_std=1.0,
        )
    ...
    def transition_reward_logits(self, current_summary, action, next_summary, detach_inputs=False):
        current_obs = current_summary[:, :NUM_OBS_TOKENS]
        next_obs = next_summary[:, :NUM_OBS_TOKENS]
        if detach_inputs:
        ...
        reward_input = torch.cat(
            [
                current_obs.reshape(current_obs.shape[0], -1),
                next_obs.reshape(next_obs.shape[0], -1),
                action,
                action.square(),
                action.square().sum(dim=-1, keepdim=True),
            ],
            dim=-1,
        )
        return self.transition_reward_head(self.transition_reward_input_norm(reward_input))
    ...
    def dynamics_step_from_history_with_cache(self, summary_history, action_history):
        ...
        current_summary = summary_history[:, -1]
        current_action = action_history[:, -1]
        pred_next_summary = pred_next_latents[:, -1]
        pred_reward_logits = self.transition_reward_logits(
            current_summary,
            current_action,
            pred_next_summary,
            detach_inputs=False,
        )
        _, pred_termination_logits = self.decode_outcomes(pred_next_summary)
    ...
    def dynamics_step_from_cache(self, latent_tokens, action, predictor_cache=None, max_context=PRED_CONTEXT):
        ...
        _, pred_termination_logits = self.decode_outcomes(pred_next_latents)
    ...
    def cost_from_action(
        ...
        for step_idx in range(horizon):
            ...
            # Reward is action-aware: r(z_obs_t, a_t, z_obs_{t+1}). Continuation
            # still comes from the predicted outcome token.
            outcome_tokens = pred_next_summary[:, NUM_OBS_TOKENS:]
            reward_logits = self.transition_reward_logits(
                last_summary,
                action_step,
                pred_next_summary,
                detach_inputs=False,
    ...
    grad_clip_groups = [
        ...
        (
            ...
            module_parameters(
                ...
                agent.transition_reward_input_norm,
                agent.transition_reward_head,
    ...
    def run_goal_embedding_solver(start_summary, init_actions):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                reward_logits = agent.transition_reward_logits(
                    flat_start_summary,
                    final_goal_action[:, 0],
                    final_pred,
                    detach_inputs=False,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                transition_reward_logits = agent.transition_reward_logits(
                    teacher_window_latents.reshape(
                        mb_size * args.dyn_horizon,
                        NUM_LATENT_TOKENS,
                        MODEL_DIM,
                    ),
                    future_actions.reshape(mb_size * args.dyn_horizon, agent.act_dim),
                    pred_next_latents.reshape(
                        mb_size * args.dyn_horizon,
                        NUM_LATENT_TOKENS,
                        MODEL_DIM,
                    ),
                ...
                transition_reward_loss = -(
                    reward_target_probs.detach() * torch.log_softmax(transition_reward_logits, dim=-1)
                ).sum(dim=-1)
                transition_reward_loss = (
                    transition_reward_loss * latent_weight
                ).sum() / latent_weight.sum().clamp_min(1.0)
                ...
                teacher_forced_reward_loss = 0.5 * (dyn_target_reward_loss + transition_reward_loss)
                ...
                transition_reward_losses.append(transition_reward_loss.item())
        ...
        transition_reward_loss_mean = safe_mean(transition_reward_losses)
        ...
        writer.add_scalar("dynamics/transition_reward_loss", transition_reward_loss_mean, global_step)
