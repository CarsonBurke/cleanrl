...
# - imagined actor uses clipped PPO on detached world-model latent rollouts
...
# - v57: aligned real rollout actor training with imagined actor training by using
#   the same asymmetric SPO objective; v66 switches both back to clipped PPO
...
class Args:
    ...
    clip_coef: float = 0.2
    """PPO policy and value clipping coefficient"""
    spo_eps_low: float = 0.40
    """retained for CLI compatibility; v66 uses clipped PPO, not SPO"""
    ...
    use_pmpo: bool = False
    """retained for CLI compatibility; v66 uses clipped PPO, not PMPO"""
...
if __name__ == "__main__":
    ...
    def build_dream_batch(prompt_summary_history, prompt_action_history, prompt_valids):
        ...
        return states, raw_actions, action_zs, old_logprobs, values, advantages, returns, learn_masks, diagnostics
        ...
        if prompt_summary_history is not None:
            ...
            for epoch in range(args.imagine_update_epochs):
                ...
                for start in range(0, dream_states.shape[0], dream_minibatch_size):
                    ...
                    with torch.no_grad():
                        if has_dream_targets:
                            ...
                            with torch.no_grad():
                                ...
                                imagine_raw_actor_beta_head_abs_means.append(
                                    raw_dream_beta_head.abs().mean().item()
                                )
                                imagine_raw_actor_beta_head_max_abses.append(
                                    raw_dream_beta_head.abs().max().item()
                                )
                                imagine_actor_stds.append(
                                    agent.get_imagined_action_std(valid_dream_states).mean().item()
                                )
                        else:
                    ...
                    dream_policy_loss1 = -mb_dream_advantages * dream_ratio
                    dream_policy_loss2 = -mb_dream_advantages * torch.clamp(
                        dream_ratio,
                        1.0 - args.clip_coef,
                        1.0 + args.clip_coef,
                    )
                    imagine_policy_loss = torch.max(dream_policy_loss1, dream_policy_loss2)
                    if has_dream_targets:
