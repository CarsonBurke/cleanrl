...
# - a second 2-layer agent transformer over latent/actor/critic/SDE CLS tokens produced by the dynamics stack
...
# - dreamed actor/critic updates from the learned dynamics model at a 1:5 real-to-fake transition ratio
# - dreamed actions come from a learned Gaussian policy over the dynamics token, so std/exploration can learn too
# - attached variant with a separate latent CLS to reduce direct conflict with the dynamics-only CLS
import os
import random
import sys
import time
...
DYN_NUM_LAYERS = 2
AGENT_NUM_LAYERS = 2
NUM_DYN_SPECIAL_TOKENS = 5
NUM_AGENT_SPECIAL_TOKENS = 4
SCALAR_EMBED_DIM = 32
...
class Args:
    ...
    detach_world_model_from_agent: bool = False
    ...
    """dream rollout horizon; one dreamed pass per minibatch gives a 1:5 real-to-fake transition ratio"""
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True):
        ...
        self.actor_cls = nn.Parameter(summary_cls[2:3].unsqueeze(0))
        self.critic_cls = nn.Parameter(summary_cls[3:4].unsqueeze(0))
        self.sde_cls = nn.Parameter(summary_cls[4:5].unsqueeze(0))
    ...
    def _encode_agent_tokens(self, agent_summary_tokens):
        agent_tokens = agent_summary_tokens
        ...
        return agent_tokens[:, 0], agent_tokens[:, 1], agent_tokens[:, 2], agent_tokens[:, 3]
    ...
    def _encode_agent_features(self, x):
        ...
        agent_summary_tokens = summary_tokens[:, 1:]
        if self.detach_world_model_from_agent:
            agent_summary_tokens = agent_summary_tokens.detach()
        _, actor_features, critic_features, sde_features = self._encode_agent_tokens(agent_summary_tokens)
    ...
    def dynamics_step(self, summary_tokens, action):
        ...
        pred_done_logits = self.dyn_done_head(dyn_latent[:, 0]).squeeze(-1)
        return pred_next_summary, pred_reward_logits, pred_done_logits

    def get_imagined_action_dist(self, summary_tokens):
        agent_summary_tokens = summary_tokens[:, 1:]
        if self.detach_world_model_from_agent:
            agent_summary_tokens = agent_summary_tokens.detach()
        _, actor_features, _, sde_features = self._encode_agent_tokens(agent_summary_tokens)
    ...
    def get_imagined_value_logits(self, summary_tokens):
        ...
        if self.detach_world_model_from_agent:
            agent_summary_tokens = agent_summary_tokens.detach()
        _, _, critic_features, _ = self._encode_agent_tokens(agent_summary_tokens)
...
if __name__ == "__main__":
    ...
    world_model_modules = [
        agent.dyn_action_proj,
        agent.dyn_joint_proj,
        agent.dyn_next_head,
        agent.dyn_reward_head,
        agent.dyn_done_head,
    ]

    def build_dream_targets(initial_summary):
        states = []
        rewards_hat = []
        continues_hat = []
        summary_state = initial_summary
        with torch.no_grad():
            for _ in range(args.imagine_horizon):
                states.append(summary_state)
                dream_dist = agent.get_imagined_action_dist(summary_state)
                dream_action = torch.clamp(dream_dist.sample(), action_low, action_high)
                pred_next_summary, pred_reward_logits, pred_done_logits = agent.dynamics_step(summary_state, dream_action)
                rewards_hat.append(reward_support.to_scalar(pred_reward_logits))
                continues_hat.append(1.0 - torch.sigmoid(pred_done_logits))
    ...
    def dreamed_actor_objective(initial_summary):
        summary_state = initial_summary
        rewards_hat = []
        continues_hat = []
        for _ in range(args.imagine_horizon):
            dream_dist = agent.get_imagined_action_dist(summary_state)
            dream_action = torch.clamp(dream_dist.rsample(), action_low, action_high)
            pred_next_summary, pred_reward_logits, pred_done_logits = agent.dynamics_step(summary_state, dream_action)
            rewards_hat.append(reward_support.to_scalar(pred_reward_logits))
            continues_hat.append(1.0 - torch.sigmoid(pred_done_logits))
            summary_state = pred_next_summary
        ...
        with torch.no_grad():
            bootstrap = agent.get_imagined_value(summary_state.detach(), hl_support)
        ...
        return -returns.mean(), returns.mean().detach()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                for horizon_idx in range(args.dyn_horizon):
                    ...
                    pred_next_summary, pred_reward_logits, pred_done_logits = agent.dynamics_step(
                ...
                if global_step >= args.imagine_start_step:
                    ...
                    dream_states, dream_returns = build_dream_targets(initial_summary.detach())
                    imagined_steps += len(mb_inds) * args.imagine_horizon

                    dream_state_batch = torch.cat(dream_states, dim=0)
                    dream_return_batch = torch.cat(dream_returns, dim=0)
                    dream_value_logits = agent.get_imagined_value_logits(dream_state_batch)
                    dream_target_probs = hl_support.project(dream_return_batch)
                    ...
                    optimizer.zero_grad()
                    (args.imagine_critic_coef * imagine_critic_loss).backward()
                    nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                    optimizer.step()
                    imagine_critic_losses.append(imagine_critic_loss.item())

                    set_requires_grad(world_model_modules, False)
                    optimizer.zero_grad()
                    initial_summary = agent.get_summary_targets(b_obs[mb_inds])
                    imagine_actor_loss, imagine_return = dreamed_actor_objective(initial_summary)
                    (args.imagine_actor_coef * imagine_actor_loss).backward()
                    nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                    optimizer.step()
                    set_requires_grad(world_model_modules, True)
                    imagine_actor_losses.append(imagine_actor_loss.item())
                    imagine_actor_returns.append(imagine_return.item())
