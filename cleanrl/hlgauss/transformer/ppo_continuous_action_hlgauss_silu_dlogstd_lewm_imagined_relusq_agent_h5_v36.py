...
# - imagined actor/critic use the standard PPO relu-squared MLP agent on detached world-model latents
import os
import random
import sys
import time
...
class Agent(nn.Module):
    ...
    def get_value_logits(self, x):
        return self.get_value(x, None).unsqueeze(-1)
...
if __name__ == "__main__":
    ...
    def build_dream_batch(initial_summary):
        ...
        returns = []
        next_return = torch.zeros_like(values[-1])
        ...
        if dream_batch is not None:
            ...
            for epoch in range(args.imagine_update_epochs):
                ...
                for start in range(0, dream_states.shape[0], dream_minibatch_size):
                    ...
                    dream_newvalue = agent.get_imagined_value(dream_states[mb_inds])
                    imagine_critic_loss = 0.5 * ((dream_newvalue - dream_returns[mb_inds]) ** 2).mean()
