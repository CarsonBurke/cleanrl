# v125: LeWM-faithful inner action solver. Critical fidelity fixes vs v124
# from the fidelity reviewer's deviation report:
#   1) horizon collapse fix: planning horizon defaults to H=5 (matches
#      LeWM's pusht.yaml horizon=5), not H=1.
#   2) random restarts: each minibatch state runs S=8 parallel candidates
#      with uniform-random unconstrained-u init; argmin cost across restarts
#      picks the amortization target (LeWM gradient solver uses 100 candidates).
#   3) critic distribution-shift fix: drop the HL-Gauss critic from the
#      cost. Reward + continuation probes are trained on predicted outcome
#      tokens in this WM and are in-distribution under the inner solve.
#      Cost = -sum_{k<H} gamma^k * continue_carry_k * r_hat_k.
#   4) Seitzer 2022 β-NLL amortize (arxiv 2203.09168): variance-weighted
#      Gaussian NLL using the actor Beta's first two moments. β=0.5 by
#      default = inverse-std weighting; avoids vanilla NLL's pitfall of
#      shrinking gradients on poorly-fit points (which would otherwise
#      bias the actor toward "ignoring" hard states). MSE retained as a
#      flag option (equivalent to β=1).
#   5) tanh/sigmoid action reparam: optimize unconstrained u, map via
#      sigmoid into the action box (no clamp_ killing Adam momentum).
#   6) agent.eval() inside the inner solver (defensive against any running
#      stats; all current norms are RMSNorm so this is belt-and-suspenders).
#
# Per real rollout state we run an inner Adam loop on free actions through
# the frozen WM. The actor probe is updated by NLL of the argmin-cost
# action under the actor's Beta dist (amortizer probe), alongside the
# real-rollout SPO anchor. WM training and probes are unchanged from v123.
# Imagined SPO dream batch is removed.
#
...
class Args:
    ...
    # v125 LeWM-faithful inner-Adam action solver
    ...
    """Adam learning rate on the free action variable (in unconstrained 'u' space)"""
    inner_solver_horizon: int = 5
    """planning horizon for the inner solver (matches LeWM pusht.yaml horizon=5)"""
    inner_solver_num_samples: int = 4
    """random restarts per state; argmin cost across restarts picks the amortization target"""
    inner_solver_init: str = "uniform_random"
    """initialization for restarts: 'uniform_random' (uniform in box) or 'policy_mean' (anchor at policy)"""
    amortize_coef: float = 0.1
    """weight on the actor amortization loss; scaled so it's comparable to typical pg_loss"""
    amortize_loss_type: str = "beta_nll"
    """amortization loss: 'beta_nll' = Seitzer 2022 β-NLL (variance-weighted Gaussian NLL using
       the actor Beta's first two moments); 'mse' = MSE on the squashed Beta mean"""
    nll_beta: float = 0.5
    """β-NLL variance-weighting exponent (Seitzer 2022, β=0.5 = inverse-std weighting, recommended)"""
...
class Agent(nn.Module):
    ...
    def cost_from_action(
        ...
        v125 drops the HL-Gauss critic bootstrap from the cost: the critic
        was trained on encoded latents but the inner solver evaluates it on
        predicted latents, opening an OOD attack surface (Adam finds
        predicted-z that score adversarially high). Reward + continuation
        probes in this WM are trained on predicted outcome tokens directly,
        so they're in-distribution under the inner solve.

        Cost: C = -sum_{k<H} gamma^k * continue_carry_k * r_hat_k.
        """
        if action.dim() == 2:
        ...
        gamma_t = 1.0
        # Gradient checkpoint each predictor step so peak activations during
        # the inner Adam backward are bounded by ONE predictor forward instead
        # of H. Cost is one extra forward pass per Adam step.
        for step_idx in range(horizon):
            ...
            pred_next_summary = torch.utils.checkpoint.checkpoint(
                self.predict_next_latents_from_history,
                pred_context,
                action_context,
                use_reentrant=False,
            )
...
if __name__ == "__main__":
    ...
    def _u_to_action(u):
        """Map unconstrained 'u' to action via sigmoid into the action box.

        sigmoid avoids the Adam-momentum-at-boundary failure of clamp_().
        """
        z = torch.sigmoid(u)
        return action_low + (action_high - action_low) * z

    def run_inner_action_solver(start_summary, init_u):
        """LeWM-style first-order solver with multi-restart parallelism.

        start_summary: (B*S, NUM_LATENT_TOKENS, MODEL_DIM), detached. Caller
            tiles the per-state latent across S restarts.
        init_u: (B*S, H, A) unconstrained initial actions in pre-sigmoid space.

        Returns:
            action_seq_star: (B*S, H, A) optimized actions in the action box, detached.
            initial_cost: (B*S,) detached cost at iteration 0.
            final_cost:   (B*S,) detached cost after `inner_solver_steps`.

        agent.eval() is set so any norm with running stats does not update.
        """
        prev_states = freeze_agent_grad()
        was_training = agent.training
        agent.eval()
        try:
            u_var = init_u.detach().clone().requires_grad_(True)
            inner_opt = optim.Adam([u_var], lr=args.inner_solver_lr)
            with torch.no_grad():
                initial_cost = agent.cost_from_action(
                    start_summary,
                    _u_to_action(u_var.detach()),
                    reward_support,
                    args.gamma,
                    horizon=args.inner_solver_horizon,
                ).detach()
            for _ in range(args.inner_solver_steps):
                action_seq = _u_to_action(u_var)
                ...
                inner_opt.step()
            with torch.no_grad():
                final_action_seq = _u_to_action(u_var.detach())
        ...
        return final_action_seq.detach(), initial_cost, final_cost
        ...
        if not world_model_only:
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    # v125 LeWM-faithful planner: tile each mb state across S
                    # random restarts, run Adam on unconstrained u (sigmoid
                    # reparam), pick argmin-cost restart per state. Memory is
                    # bounded by gradient checkpointing inside cost_from_action.
                    mb_latents = b_agent_latents[mb_inds].detach()
                    mb_size = mb_latents.shape[0]
                    S = args.inner_solver_num_samples
                    H = args.inner_solver_horizon
                    A = agent.act_dim
                    tiled_latents = (
                        mb_latents.unsqueeze(1)
                        .expand(-1, S, -1, -1)
                        .reshape(mb_size * S, *mb_latents.shape[1:])
                        .contiguous()
                    )
                    with torch.no_grad():
                        if args.inner_solver_init == "policy_mean":
                            ...
                            init_u_per_state = (
                                init_action_z.log() - (1.0 - init_action_z).log()
                            )
                            init_u = (
                                init_u_per_state.unsqueeze(1)
                                .unsqueeze(2)
                                .expand(-1, S, H, -1)
                                .reshape(mb_size * S, H, A)
                                .contiguous()
                            )
                            uniform_z = torch.rand(
                                mb_size * S, H, A, device=device, dtype=mb_latents.dtype
                            ).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                            uniform_u = uniform_z.log() - (1.0 - uniform_z).log()
                            mix_w = torch.rand(
                                mb_size * S, 1, 1, device=device, dtype=mb_latents.dtype
                            )
                            init_u = mix_w * init_u + (1.0 - mix_w) * uniform_u
                        else:
                            # uniform_random: action_z ~ U(0, 1), u = logit(z)
                            uniform_z = torch.rand(
                                mb_size * S, H, A, device=device, dtype=mb_latents.dtype
                            ).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                            init_u = uniform_z.log() - (1.0 - uniform_z).log()
                    a_star_seq, inner_initial_cost, inner_final_cost = run_inner_action_solver(
                        tiled_latents, init_u
                    )
                    final_cost_per_state = inner_final_cost.view(mb_size, S)
                    initial_cost_per_state = inner_initial_cost.view(mb_size, S)
                    best_idx = final_cost_per_state.argmin(dim=1)
                    a_star_seq = a_star_seq.view(mb_size, S, H, A)
                    row_idx = torch.arange(mb_size, device=device)
                    a_star_first = a_star_seq[row_idx, best_idx, 0]
                    inner_initial_cost_mean = initial_cost_per_state[row_idx, best_idx]
                    inner_final_cost_mean = final_cost_per_state[row_idx, best_idx]
                    ...
                    # v125 amortization: regress the actor toward the
                    # argmin-cost optimized first-step action. Default is
                    # Seitzer 2022 β-NLL (variance-weighted Gaussian NLL
                    # using Beta's first two moments), which avoids the
                    # vanilla-NLL pitfall of underweighting hard-to-fit
                    # points by scaling NLL with sg(σ^{2β}). β=0.5 is the
                    # paper-recommended sweet spot (inverse-std weighting).
                    actor_dist = agent._action_distribution(
                        agent._actor_features_from_latents(mb_latents)
                    )
                    actor_mean = agent._z_to_action(actor_dist.mean)
                    actor_var = (actor_dist.variance * (
                        (agent.action_high - agent.action_low) ** 2
                    )).clamp_min(1e-8)
                    target = a_star_first.detach()
                    if args.amortize_loss_type == "beta_nll":
                        sq_err = (target - actor_mean).square()
                        gauss_nll = 0.5 * actor_var.log() + 0.5 * sq_err / actor_var
                        weight = actor_var.detach().pow(args.nll_beta)
                        amortize_loss = (weight * gauss_nll).sum(-1).mean()
                    elif args.amortize_loss_type == "mse":
                        amortize_loss = F.mse_loss(actor_mean, target)
                    else:
                        raise ValueError(
                            f"unknown amortize_loss_type {args.amortize_loss_type!r}"
                        )

                    loss = (
                        args.real_spo_coef * pg_loss
                        - args.ent_coef * entropy_loss
                        + args.vf_coef * v_loss
                        + args.amortize_coef * amortize_loss
                    )
                    ...
                    inner_initial_costs.append(inner_initial_cost_mean.mean().item())
                    inner_final_costs.append(inner_final_cost_mean.mean().item())
                    inner_cost_deltas.append(
                        (inner_final_cost_mean - inner_initial_cost_mean).mean().item()
                    )
                    amortize_losses.append(amortize_loss.item())
                    amortize_action_errors.append(
                        (actor_mean - a_star_first).abs().mean().item()
                    )
