...
# - v72: applies SIGReg token-wise to obs and outcome target tokens
...
# - v77: removes predictor dropout, uses partial RoPE, and restores same-step
#   predictor slot position by applying RoPE in local axial attention.
# - v78: actor and critic each read 4 Perceiver tokens flattened into the
#   256-hidden MLP heads, obs scalar tokens get learned dimension embeddings,
#   SIGReg returns to full-summary embeddings, and current summaries carry
#   previous outcomes when those outcomes exist in the same episode.
# - v79: fixes outcome-aware value bootstrapping: when real rollout values and
#   agent updates use summaries with previous outcome tokens, GAE now bootstraps
#   from matching next summaries instead of neutral-outcome next obs encodings.
# - v80: makes current-summary outcome usage consistent for rollout, WM training,
#   and diagnostics; dreamed rollout feedback re-tokenizes decoded reward/continue
#   outcomes before feeding predicted summaries back into the predictor.
# - v81: restores calibrated outcome prediction as detached linear probes from
#   predicted reward and termination outcome tokens. Probe CE/BCE trains only the
#   heads, leaving LEJEPA token geometry governed by prediction MSE + SIGReg.
import os
import random
import sys
import time
...
AGENT_READOUT_TOKENS = 4
AGENT_INPUT_DIM = AGENT_READOUT_TOKENS * MODEL_DIM
...
class Args:
    ...
    """retained for CLI compatibility; v81 decodes outcomes through detached trained probes"""
...
def build_rope_cache(num_obs_tokens, num_special_tokens, head_dim, device, base=10000.0, rope_fraction=ROPE_FRACTION):
    ...
    theta = 1.0 / (base ** (torch.arange(0, rope_dim, 2, device=device).float() / rope_dim))
    positions = torch.arange(num_obs_tokens, device=device).float()
    freqs = torch.outer(positions, theta)

    cos = torch.ones(total_tokens, rope_dim // 2, device=device)
    sin = torch.zeros(total_tokens, rope_dim // 2, device=device)
...
def attention(q, k, v, attn_mask=None, enable_gqa=False, is_causal=False):
...
class PredictorSelfAttentionBlock(nn.Module):
    ...
    def step(self, x, cache, rope_cos, rope_sin, max_context):
        ...
        attn_out = attention(q, k_attn, v_all, enable_gqa=self.kv_group_size > 1)
...
class PredictorAxialBlock(nn.Module):
    def __init__(self, dim, num_q_heads, num_kv_heads, ffn_mult=2):
        super().__init__()
        self.local_in = PredictorSelfAttentionBlock(dim, num_q_heads, num_kv_heads, ffn_mult)
        self.temporal = PredictorSelfAttentionBlock(dim, num_q_heads, num_kv_heads, ffn_mult)
        self.local_out = PredictorSelfAttentionBlock(dim, num_q_heads, num_kv_heads, ffn_mult)
    ...
    def forward(
        ...
        space_rope_sin,
        temporal_mask=None,
        temporal_is_causal=False,
        return_cache=False,
    ):
        batch, context_len, tokens_per_step, width = x.shape
        x = self.local_in(
            x.reshape(batch * context_len, tokens_per_step, width),
            space_rope_cos,
            space_rope_sin,
        )
    ...
    def step(self, x, temporal_cache, time_rope_cos, time_rope_sin, space_rope_cos, space_rope_sin, max_context):
        batch, tokens_per_step, width = x.shape
        x = self.local_in(x, space_rope_cos, space_rope_sin)
        ...
        x = self.local_out(x, space_rope_cos, space_rope_sin)
        return x, new_cache
...
class Agent(nn.Module):
    def __init__(
        ...
        self.pred_layers = nn.ModuleList(
            [
                PredictorAxialBlock(MODEL_DIM, NUM_Q_HEADS, NUM_KV_HEADS, FFN_MULT)
        ...
        self.agent_readout_queries = nn.Parameter(torch.empty(2 * AGENT_READOUT_TOKENS, MODEL_DIM))
    ...
    def _encode_dynamics_tokens(self, x):
        ...
        obs_tokens = obs_tokens + self.obs_dim_embed.to(dtype=x.dtype, device=x.device).unsqueeze(0)
    ...
    def encode_summary_with_optional_outcome(self, x, reward_probs, continuations, has_outcome):
        obs_tokens = self._encode_obs_latent_tokens(x)
        outcome_tokens = self._outcome_tokens(obs_tokens, reward_probs, continuations)
        if has_outcome is not None:
            neutral_outcome_tokens = self._neutral_outcome_tokens(obs_tokens)
            outcome_mask = has_outcome.to(device=x.device, dtype=torch.bool).view(-1, 1, 1)
            outcome_tokens = torch.where(outcome_mask, outcome_tokens, neutral_outcome_tokens)
        return self._latent_from_obs(obs_tokens, outcome_tokens)

    def decode_outcomes(self, summary_tokens, detach_summary=True):
        outcome_tokens = summary_tokens[:, NUM_OBS_TOKENS:]
        # Detached probes calibrate predicted outcome tokens without letting
        # CE/BCE gradients shape the LEJEPA embedding space.
    ...
    def repair_predicted_outcome_tokens(self, summary_tokens, reward_probs, continuations):
        reward_probs = reward_probs.to(dtype=summary_tokens.dtype)
        continuations = continuations.to(dtype=summary_tokens.dtype)
        reward_token = self._reward_outcome_token(reward_probs)
        ...
        continuation_token = torch.lerp(
            continuation_codebook[0].unsqueeze(0),
            continuation_codebook[1].unsqueeze(0),
            continuations.clamp(0.0, 1.0).unsqueeze(-1),
        )
        outcome_tokens = torch.stack([reward_token, continuation_token], dim=1)
        return self._latent_from_obs(summary_tokens[:, :NUM_OBS_TOKENS], outcome_tokens)
    ...
    def _agent_readout_features_from_latents(self, latent_tokens):
        ...
        queries = queries.unsqueeze(0).expand(latent_tokens.shape[0], -1, -1)
        return self.agent_readout_final_norm(self.agent_readout(queries, latent_tokens))
    ...
    def _actor_features_from_latents(self, latent_tokens):
        ...
        actor_tokens = readout_tokens[:, :AGENT_READOUT_TOKENS]
        return self.actor_input_norm(actor_tokens.reshape(actor_tokens.shape[0], -1))

    def _critic_features_from_latents(self, latent_tokens):
        readout_tokens = self._agent_readout_features_from_latents(latent_tokens)
        critic_tokens = readout_tokens[:, AGENT_READOUT_TOKENS:]
        return self.critic_input_norm(critic_tokens.reshape(critic_tokens.shape[0], -1))
    ...
    def _actor_critic_features_from_latents(self, latent_tokens):
        ...
        critic_tokens = readout_tokens[:, AGENT_READOUT_TOKENS:]
        return (
            self.actor_input_norm(actor_tokens.reshape(actor_tokens.shape[0], -1)),
            self.critic_input_norm(critic_tokens.reshape(critic_tokens.shape[0], -1)),
        )
    ...
    def _predictor_forward_parallel(self, pred_tokens, return_cache=False):
        ...
        tokens_per_step = pred_tokens.shape[2]
    ...
    def dynamics_step_with_cache(self, summary_tokens, action_features, predictor_cache, max_context=PRED_CONTEXT):
        ...
        for layer, layer_cache in zip(self.pred_layers, predictor_cache):
            pred_tokens, layer_cache = layer.step(
                ...
                self.pred_space_rope_cos[: pred_tokens.shape[1]],
                self.pred_space_rope_sin[: pred_tokens.shape[1]],
                max_context,
            )
...
if __name__ == "__main__":
    ...
    def build_dream_batch(prompt_summary_history, prompt_action_feature_history, prompt_valids):
        ...
        rewards_hat = []
        continues_hat = []
        model_horizon_returns = torch.zeros_like(prompt_valids, dtype=torch.float32)
        ...
        with torch.no_grad():
            ...
            for _ in range(args.imagine_horizon):
                ...
                feedback_next_summary = agent.repair_predicted_outcome_tokens(
                    pred_next_summary,
                    reward_support.project(pred_reward),
                    pred_continue,
                )
                rewards_hat.append(pred_reward)
                continues_hat.append(pred_continue)
                model_horizon_returns = model_horizon_returns + pred_reward * alive
                alive = alive * pred_continue
                summary_history.append(feedback_next_summary.detach())
        ...
        with torch.no_grad():
            ...
            diagnostics = {
                ...
                "model_horizon_return": model_horizon_returns[prompt_valids].mean().item()
    ...
    def build_dream_batch_eval(prompt_summary_history, prompt_action_feature_history, prompt_valids):
        ...
        try:
            ...
            for start in range(0, total_starts, chunk_size):
                ...
                for key, value in chunk_batch[9].items():
                    if key.endswith("_max"):
                        diagnostic_maxes[key] = max(diagnostic_maxes.get(key, -float("inf")), value)
                    else:
            ...
            diagnostics.update(diagnostic_maxes)
    ...
    def dynamics_diagnostics(
        ...
        try:
            ...
            summary_history = [
                agent.encode_summary_with_optional_outcome(
                    flat_obs[sample_inds],
                    reward_support.project(flat_prev_rewards[sample_inds]),
                    flat_prev_continues[sample_inds],
                    flat_has_prev_outcomes[sample_inds],
                ).detach()
            ]
    ...
    last_outcome_reward = torch.zeros(args.num_envs, device=device)
    last_outcome_continue = torch.ones(args.num_envs, device=device)
    last_has_outcome = torch.zeros(args.num_envs, device=device, dtype=torch.bool)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        rollout_start_prev_reward = last_outcome_reward.clone()
        rollout_start_prev_continue = last_outcome_continue.clone()
        rollout_start_has_outcome = last_has_outcome.clone() & (~next_done.bool())
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                rollout_latents = agent.encode_summary_with_optional_outcome(
                    next_obs,
                    reward_support.project(last_outcome_reward),
                    last_outcome_continue,
                    last_has_outcome & (~next_done.bool()),
                )
            ...
            reward_t = torch.tensor(reward, device=device, dtype=torch.float32).view(-1)
            termination_t = torch.tensor(transition_termination, device=device, dtype=torch.float32)
            boundary_t = torch.tensor(transition_boundary, device=device, dtype=torch.float32)
            valid_t = torch.tensor(transition_valid, device=device)
            rewards[step] = reward_t
            transition_terminations[step] = termination_t
            transition_boundaries[step] = boundary_t
            transition_valids[step] = valid_t
            next_obses[step] = torch.tensor(transition_next_obs, device=device)
            next_obs, next_done = torch.Tensor(next_obs).to(device), torch.Tensor(next_done).to(device)
            last_outcome_reward = reward_t
            last_outcome_continue = 1.0 - termination_t
            last_has_outcome = valid_t.bool() & (~boundary_t.bool())
        ...
        with torch.no_grad():
            next_transition_reward_probs = reward_support.project(rewards.reshape(-1))
            next_transition_continues = (1.0 - transition_terminations).reshape(-1)
            next_transition_summaries = agent.encode_target_summary(
                next_obses.reshape((-1,) + envs.single_observation_space.shape),
                next_transition_reward_probs,
                next_transition_continues,
            ).reshape(args.num_steps, args.num_envs, NUM_LATENT_TOKENS, MODEL_DIM)
            next_transition_values = agent.get_imagined_value(
                next_transition_summaries.reshape(-1, NUM_LATENT_TOKENS, MODEL_DIM),
                hl_support,
        ...
        current_prev_rewards = torch.zeros_like(rewards)
        current_prev_continues = torch.ones_like(rewards)
        current_has_prev_outcomes = torch.zeros_like(transition_valids, dtype=torch.bool)
        current_prev_rewards[0] = rollout_start_prev_reward
        current_prev_continues[0] = rollout_start_prev_continue
        current_has_prev_outcomes[0] = rollout_start_has_outcome
        if args.num_steps > 1:
            current_prev_rewards[1:] = rewards[:-1]
            current_prev_continues[1:] = 1.0 - transition_terminations[:-1]
            current_has_prev_outcomes[1:] = transition_valids[:-1].bool() & (~transition_boundaries[:-1].bool())
        b_current_prev_rewards = current_prev_rewards.reshape(-1)
        b_current_prev_continues = current_prev_continues.reshape(-1)
        b_current_has_prev_outcomes = current_has_prev_outcomes.reshape(-1)
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                initial_has_outcome = b_current_has_prev_outcomes[mb_inds]
                initial_summary = agent.encode_summary_with_optional_outcome(
                    b_obs[mb_inds],
                    reward_support.project(b_current_prev_rewards[mb_inds]),
                    b_current_prev_continues[mb_inds],
                    initial_has_outcome,
                )
                ...
                sigreg_window_flat = sigreg_window_latents.reshape(mb_size, args.dyn_horizon + 1, -1)
                sigreg_latents = [sigreg_window_flat[:, 0]]
                sigreg_valids = [initial_has_outcome]
                sigreg_latents.extend(
                    [
                        sigreg_window_flat[:, horizon_idx + 1]
                ...
                outcome_probe_loss = (
                    args.dyn_reward_coef * outcome_reward_probe_ce
                    + args.dyn_termination_coef * outcome_termination_probe_bce
                )

                optimizer.zero_grad()
                (lejepa_loss + outcome_probe_loss).backward()
                clip_grad_groups()
                optimizer.step()

                wm_losses.append((lejepa_loss + outcome_probe_loss).item())
        ...
        dyn_diagnostics = dynamics_diagnostics(
            b_obs,
            b_current_prev_rewards,
            b_current_prev_continues,
            b_current_has_prev_outcomes,
            rewards,
        ...
        if not world_model_only:
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    with torch.no_grad():
                        writer.add_scalar(
                            "returns/real_return_minibatch",
                            b_returns[mb_inds].mean().item(),
                            real_minibatch_step,
                        )
                        writer.add_scalar(
                            "returns/real_advantage_minibatch",
                            b_advantages[mb_inds].mean().item(),
                            real_minibatch_step,
                        )
            ...
            for epoch in range(args.imagine_update_epochs):
                ...
                for start in range(0, dream_states.shape[0], dream_minibatch_size):
                    ...
                    with torch.no_grad():
                        ...
                        writer.add_scalar(
                            "returns/dream_advantage_minibatch",
                            weighted_mean(
                                mb_dream_advantages[mb_dream_learn_mask],
                                valid_weights,
                            ).item(),
                            imagination_minibatch_step,
                        )
                    ...
                    with torch.no_grad():
                        ...
                        dream_approx_kl = ((valid_dream_ratio - 1) - valid_dream_logratio).mean()
                        dream_clipfracs += [
        ...
        if v_loss is not None:
            ...
            if real_logratio_abs_means:
                ...
                writer.add_scalar("real_rollout/logratio_max_abs", np.max(real_logratio_max_abses), global_step)
                writer.add_scalar(
        ...
        if imagine_logratio_abs_means:
            ...
            writer.add_scalar("imagination/logratio_max_abs", np.max(imagine_logratio_max_abses), global_step)
            writer.add_scalar(
            ...
            writer.add_scalar("imagination/actor_mean_max_abs", np.max(imagine_actor_mean_max_abses), global_step)
            writer.add_scalar(
            ...
            writer.add_scalar(
                ...
                np.max(imagine_raw_actor_beta_head_max_abses),
                global_step,
            )
