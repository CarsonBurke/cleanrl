...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True, actor_mean_scale=3.0):
        super().__init__()
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        dream_inds = np.arange(args.batch_size * args.imagine_horizon) if initial_summary is not None else None
        dream_minibatch_size = args.minibatch_size * args.imagine_horizon if initial_summary is not None else None
        clipfracs = []
        dream_clipfracs = []
        ...
        if initial_summary is not None:
            for epoch in range(args.imagine_update_epochs):
                ...
                for start in range(0, dream_states.shape[0], dream_minibatch_size):
                    ...
                    dream_bounded_action_logratio = dream_action_logratio.clamp(-20.0, 20.0)
                    dream_action_ratio = dream_bounded_action_logratio.exp()
                    dream_logratio = dream_bounded_action_logratio.sum(1).clamp(-20.0, 20.0)
