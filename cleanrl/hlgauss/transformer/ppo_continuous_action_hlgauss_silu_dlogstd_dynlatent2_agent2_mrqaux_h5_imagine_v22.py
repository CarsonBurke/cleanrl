...
# - a second 2-layer agent transformer over predicted latent state plus learned actor/critic/SDE CLS tokens
...
# - Dreamer-style split: the world model predicts latent state, and the agent is a downstream function of that latent
import os
import random
import sys
import time
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True):
        ...
        agent_cls = torch.empty(NUM_AGENT_SPECIAL_TOKENS - 1, MODEL_DIM)
    ...
    def _encode_agent_tokens(self, latent_features):
        batch = latent_features.shape[0]
        agent_tokens = torch.cat(
            [
                latent_features.unsqueeze(1),
    ...
    def _encode_agent_features(self, x):
        ...
        latent_features = summary_tokens[:, 1]
        if self.detach_world_model_from_agent:
            latent_features = latent_features.detach()
        _, actor_features, critic_features, sde_features = self._encode_agent_tokens(latent_features)
    ...
    def get_imagined_action_dist(self, summary_tokens):
        ...
        if self.detach_world_model_from_agent:
            latent_features = latent_features.detach()
        _, actor_features, _, sde_features = self._encode_agent_tokens(latent_features)
    ...
    def get_imagined_value_logits(self, summary_tokens):
        ...
        if self.detach_world_model_from_agent:
            latent_features = latent_features.detach()
        _, _, critic_features, _ = self._encode_agent_tokens(latent_features)
