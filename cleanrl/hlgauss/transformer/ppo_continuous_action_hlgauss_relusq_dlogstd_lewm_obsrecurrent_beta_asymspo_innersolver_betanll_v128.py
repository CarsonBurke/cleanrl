# v128: v124 scaffold + Seitzer 2022 β-NLL amortization in place of MSE.
#
# Motivation: the inner solver returns a noisy point estimate of the local
# argmin-cost action. v124's MSE loss treats every `a_star_first` as a
# perfectly clean label and only updates the actor mean. β-NLL treats it
# as a Gaussian-noisy observation, weights the per-state NLL by sg(σ^{2β}),
# and lets the actor's own Beta variance both (a) absorb residual target
# noise and (b) act as a learned uncertainty signal. β=0.5 (inverse-std
# weighting) is Seitzer's recommended setting — it interpolates between
# raw NLL (β=0, variance-collapse-prone) and MSE (β=1, variance-blind).
#
