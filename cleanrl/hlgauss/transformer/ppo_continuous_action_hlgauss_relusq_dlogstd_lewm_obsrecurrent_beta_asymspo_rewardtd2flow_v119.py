...
class Args:
    ...
    """weight on the detached TD2-CFM successor reward-token flow"""
    successor_flow_steps: int = 4
    """Euler midpoint steps used to sample successor reward tokens for WM values"""
    ...
    """number of reward-token successor-flow samples averaged for imagined WM value estimates"""
...
class Agent(nn.Module):
    ...
    def integrate_successor_flow(
        ...
        if start_tokens is None:
            ...
            successor_tokens = start_tokens[:, 0] if start_tokens.dim() == 3 else start_tokens
        if torch.is_tensor(end_t):
            end_t_tensor = end_t.reshape(-1, 1).to(
                device=condition_tokens.device,
                dtype=condition_tokens.dtype,
            )
        else:
            end_t_tensor = condition_tokens.new_full((condition_tokens.shape[0], 1), float(end_t))
        dt = end_t_tensor / max(1, steps)
        for step_idx in range(max(1, steps)):
            mid_t = (step_idx + 0.5) * dt.squeeze(-1)
    ...
    def successor_td2_flow_loss(
        ...
        flat_next_reward = flat_next[:, NUM_OBS_TOKENS]
        ...
        source_noise = torch.randn_like(flat_next_reward)
        path_t = flow_t.reshape(-1, 1)
        direct_xt = source_noise.lerp(flat_next_reward, path_t)
        direct_target_velocity = flat_next_reward - source_noise
        ...
        boot_noise = torch.randn_like(flat_next_reward)
        with torch.no_grad():
    ...
    def successor_value(self, summary_tokens, reward_support, gamma, samples=1, steps=4):
        ...
        successor_reward_tokens = self.integrate_successor_flow(
            condition_tokens,
            steps=steps,
            use_target=False,
        )
        reward_logits = self.reward_outcome_unproj(successor_reward_tokens.detach())
