# v134: v133 PrefPoE with α_entropy retuned (0.5 → 0.05).
#
# v133 underperformance (HC 1.1M run):
#   Returns plateaued at +1144 (vs v124 baseline +2104 at 1.376M).
#   pref_entropy stuck near −0.2 nat (Beta α+β ≈ 2.5/dim ≈ uniform); pref
#   provided no preference signal. KL(fused,main) ≈ 0.16 → fused ≈ main →
#   we paid the cost (extra head, gradients, cons_loss) for no benefit.
#   Cause: with 6 action dims, summed entropy reg = 6·0.5 = 3.0 dominated
#   the β₁=1.0 advantage-weighted likelihood term, holding pref uniform.
#
# v134 fix:
#   prefpoe_alpha_entropy default 0.5 → 0.05 (10× lower).
#   Cap stays at 50 as the primary safety backstop (entropy reg now gentle
#   pressure, not hard regularization). Expect pref_entropy ∈ [−5, −1]
#   instead of v133's ≈ −0.2.
#
# Earlier history:
...
#   v134 (α_entropy=0.05): targeting balance — paper Theorem 1 Boltzmann
#                          temperature α/β₁=0.05 gives sharp-but-stable pref.
#
...
class Args:
    ...
    """α in the paper — entropy regularization on the preference head.
       v132 (α=0.0): pref ran away to α+β≈500/dim → Dirac collapse @500k.
       v133 (α=0.5): pref stuck uniform (6·0.5 entropy reg dominated β₁=1.0
                     advantage signal) → no preference signal → underperform.
       v134 (α=0.05): gentle entropy pressure; cap=50 is primary safety."""
