# v132: v124 + PrefPoE (Lin et al., arXiv 2511.08241) adapted from
# Gaussian to Beta via the closed-form Beta-PoE.
#
...
#
# v132 adaptation (Beta, since v124 already uses state-dependent Beta on
# normalized action coords):
#   For two Betas Beta(α_θ, β_θ) and Beta(α_pref, β_pref):
#     Beta(α_θ, β_θ)(x) · Beta(α_pref, β_pref)(x)^λ
#         ∝ x^{α_θ−1+λ(α_pref−1)} · (1−x)^{β_θ−1+λ(β_pref−1)}
#         = Beta(α_θ + λ(α_pref−1),  β_θ + λ(β_pref−1))   (closed-form)
#   With v124's `α = 1 + softplus(.)` parameterization both α, β ≥ 1, so
#   for any λ ∈ [0, 1] the fused parameters are also ≥ 1 — fused stays a
#   proper unimodal Beta. λ=0 ⇒ fused = main; λ=1 ⇒ full PoE.
#
# Why Beta-PoE over the paper's Gaussian-PoE:
#   - v124's Beta policy has bounded support [action_low, action_high] and
#     state-dependent skew — replacing it with Gaussian would discard a
#     known-good property of the v124 stack.
#   - The Beta-PoE closed form preserves the variance-reducing property
#     (Theorem 2 in the paper): fused concentration α_fused+β_fused ≥
#     α_θ+β_θ, so var(fused) ≤ var(main) componentwise — the same "soft
#     trust region" effect as in the Gaussian case.
#
# Defaults (paper body does not give exact hyperparams; appendix-only):
#   lambda_pref=0.5, prefpoe_beta1=1.0, prefpoe_alpha_entropy=0.0,
#   w_pref=1.0, w_cons=0.1.
#
...
class Args:
    ...
    # v132 PrefPoE (arXiv 2511.08241) — Beta-PoE adaptation.
    ...
    """α in the paper — entropy regularization on the preference head. 0.0 disables."""
...
class Agent(nn.Module):
    ...
    def _pref_action_distribution(self, agent_input):
        # v132 PrefPoE: advantage-guided preference Beta head.
