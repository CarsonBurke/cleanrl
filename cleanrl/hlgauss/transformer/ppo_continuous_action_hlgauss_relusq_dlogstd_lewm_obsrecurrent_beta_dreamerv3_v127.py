# v127: same Dreamer V3-style pure-dream actor+critic as v126, with two
# changes motivated by v126's collapse on HalfCheetah:
#   - 200k env-step WM warmup. v126 launched the actor against an
#     uncalibrated reward head; the WM Jacobian early on pointed in
#     random directions and the Beta concentration ran away to extreme
#     values, locking the actor into a low-entropy corner that even a
#     later-correct Jacobian could not move out of. A 200k warmup lets
#     the reward head finish coarse calibration before the actor opens.
#   - dream_ent_coef = 0. There is no PPO ratio in the dream actor loss
#     (pure reparam through frozen WM), and the entropy term in v126
#     was the dominant driver of the Beta concentration runaway when
#     coupled with std normalization. With no entropy term the actor
#     is shaped purely by ∂λ/∂a via the WM Jacobian.
#
# Everything else from v126 is unchanged:
...
#   - Critic is HL-Gauss on PREDICTED latents only. The only value
#     signal is the dream λ-return computed inside imagined rollouts.
#   - Target critic via EMA decay (0.995 per minibatch) for the bootstrap
#     V(z_H), preventing the actor → WM → critic → actor closed loop.
...
#   - Real-rollout SPO anchor stays at 0 by default (pure-dream).
#
...
class Args:
    ...
    dream_ent_coef: float = 0.0
    """entropy bonus on the Beta actor inside the dream rollout. v127 disables this:
       the actor loss is shaped purely by ∂λ/∂a via the WM Jacobian, no PPO ratio,
       so an explicit entropy bonus is unnecessary and was destabilizing in v126."""
