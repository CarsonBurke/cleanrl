...
class Agent(nn.Module):
    def __init__(
        ...
        # v171: tanh-squashed Gaussian policy (adopted from iterthink_v21). The head
        # now emits only the per-state MEAN; the spread is a single GLOBAL learnable
        # log-std per action dim (state-independent, like classic PPO-continuous).
        # This replaces the Beta whose concentration ran away unboundedly (entropy
        # had no floor) because Beta entropy reg vanishes as alpha+beta grows; a
        # Gaussian's dH/dlogstd = 1 is constant, so std cannot collapse without bound.
        self.actor_beta = ReluSqRMSHead(AGENT_INPUT_DIM, AGENT_HIDDEN_DIM, act_dim, output_std=0.01)
    ...
    def _action_distribution(self, agent_input):
        # v171: pre-squash Gaussian. mean = state-dependent head; std = global
        # learnable log-std. The action is tanh(z) where z ~ this Normal (squashing
        # + Jacobian handled in `_beta_action_logprob_entropy`/`_z_to_action`).
        mean = self.actor_beta(agent_input)
    ...
    def _z_to_action(self, action_z):
        # v171: tanh squashing maps the real-valued Gaussian sample z to (-1, 1),
        # which IS the action space ([-1, 1]); the env wrapper's clamp is a no-op.
        return torch.tanh(action_z)

    def _action_to_z(self, action):
        # v171: inverse squashing (atanh). Only used for the special non-policy
        # action sources (warmup midpoint, planner, random bootstrap), whose steps
        # are excluded from the PG update, so the clamp's tiny boundary error is
        # irrelevant there.
        y = action.clamp(-1.0 + ATANH_EPS, 1.0 - ATANH_EPS)
    ...
    def _beta_action_logprob_entropy(self, dist, action=None, action_z=None, sum_logprob=True):
        # v171: tanh-squashed Gaussian. `action_z` is the PRE-squash Gaussian sample;
        # `action = tanh(action_z)`. The log-prob carries the tanh change-of-variables
        # correction; the entropy is the pre-squash Gaussian entropy (PPO-standard).
        if action_z is None:
        ...
        # log|d action / d z| = log(1 - tanh(z)^2), numerically stable form.
        log_det = 2.0 * (math.log(2.0) - action_z - F.softplus(-2.0 * action_z))
        logprob_per_dim = dist.log_prob(action_z) - log_det
    ...
    def _action_mean_from_dist(self, dist):
        # Deterministic action = squashed mean.
        return torch.tanh(dist.mean)

    def _action_std_from_dist(self, dist):
        # Pre-squash Gaussian std (diagnostic logging only).
        return dist.stddev
    ...
    actor_params = [
        ...
        for p in (
            ...
            *agent.actor_input_norm.parameters(),
            agent.actor_logstd,  # v171: global learnable log-std receives the policy gradient
        )
        if p.requires_grad
    ]
