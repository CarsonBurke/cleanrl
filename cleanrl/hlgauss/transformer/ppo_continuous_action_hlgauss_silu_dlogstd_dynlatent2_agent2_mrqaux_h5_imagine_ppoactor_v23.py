...
# - dreamed critic updates from model-predicted latent rollouts and PPO-native dreamed actor updates on detached latent states
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if global_step >= args.imagine_start_step:
                    ...
                    dream_states, dream_actions, dream_old_logprobs, dream_advantages, dream_returns = build_dream_batch(
                        initial_summary
                    )
                    imagined_steps += len(mb_inds) * args.imagine_horizon

                    dream_value_logits = agent.get_imagined_value_logits(dream_states)
                    dream_target_probs = hl_support.project(dream_returns)
                    ...
                    _, dream_newlogprob, dream_entropy, _ = agent.get_imagined_action_and_value(
                        dream_states, hl_support, dream_actions
                    )
                    dream_logratio = dream_newlogprob - dream_old_logprobs
                    dream_ratio = dream_logratio.exp()
                    imagine_pg_loss1 = -dream_advantages * dream_ratio
                    imagine_pg_loss2 = -dream_advantages * torch.clamp(
                    ...
                    imagine_return = dream_returns.mean().detach()
