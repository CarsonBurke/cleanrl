...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True, actor_mean_scale=3.0):
        ...
        pred_tokens_per_step = act_dim + NUM_DYN_LATENT_TOKENS
    ...
    def predict_next_latents_from_history(self, latent_history, action_history):
        ...
        action_tokens = self.pred_action_out_proj(F.silu(self.pred_action_in_proj(action_history.unsqueeze(-1))))
        ...
        for layer in self.pred_layers:
            pred_tokens = layer(pred_tokens, rope_cos, rope_sin, attn_mask=attn_mask)
        ...
        return pred_latents[:, -1, self.act_dim :]
