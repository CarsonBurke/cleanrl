...
# - a standard relu-squared MLP PPO agent acts on detached world-model state tokens
...
#   the same asymmetric SPO objective; clip_coef remains a diagnostic reference
...
PRED_DROPOUT = 0.1
PRED_CONTEXT = 1
DEFAULT_PRED_CONTEXT = 1
NUM_STATE_TOKENS = 6
NUM_OUTCOME_TOKENS = 2
NUM_LATENT_TOKENS = NUM_STATE_TOKENS + NUM_OUTCOME_TOKENS
SCALAR_EMBED_DIM = 32
AGENT_INPUT_DIM = NUM_STATE_TOKENS * MODEL_DIM
...
class Args:
    ...
    """retained for CLI compatibility; v56+ uses an uncapped baseline-style actor mean"""
    use_pmpo: bool = False
    ...
    """number of summary steps the predictor can attend over; v59 defaults to current state/action only"""
    dyn_latent_coef: float = 1.0
    """weight on next-dynamics-token prediction"""
    dyn_reward_coef: float = 0.25
    """weight on scalar reward decoder calibration from outcome tokens"""
    dyn_termination_coef: float = 0.25
    """weight on scalar termination decoder calibration from outcome tokens"""
...
class Agent(nn.Module):
    def __init__(
        ...
        state_cls = torch.empty(NUM_STATE_TOKENS, MODEL_DIM)
        nn.init.xavier_uniform_(state_cls)
        self.state_cls = nn.Parameter(state_cls.unsqueeze(0))
        ...
        dyn_rope_cos, dyn_rope_sin = build_rope_cache(
            obs_dim, NUM_STATE_TOKENS, head_dim, torch.device("cpu")
        )
        ...
        self.actor_mean = ReluSqRMSHead(AGENT_INPUT_DIM, AGENT_HIDDEN_DIM, act_dim, output_std=0.01)
        self.actor_logstd = nn.Parameter(torch.zeros(1, act_dim))
        self.actor_input_norm = RMSNorm(AGENT_INPUT_DIM)
        self.critic_input_norm = RMSNorm(AGENT_INPUT_DIM)
        outcome_input_dim = MODEL_DIM * NUM_STATE_TOKENS + reward_num_bins + 1
    ...
    def _action_mean(self, agent_input):
    ...
    def _encode_dynamics_tokens(self, x):
        ...
        state_tokens = self.state_cls.expand(batch, -1, -1)
        dyn_tokens = torch.cat([state_tokens, obs_tokens], dim=1)
    ...
    def _encode_state_tokens(self, x):
        state_tokens = self._encode_dynamics_tokens(x)[:, :NUM_STATE_TOKENS]
        return self.dyn_next_proj(state_tokens)

    def _outcome_tokens(self, state_tokens, reward_probs, continuations):
        state_flat = state_tokens.detach().reshape(state_tokens.shape[0], -1)
        outcome_input = torch.cat([state_flat, reward_probs, continuations.unsqueeze(-1)], dim=-1)
        outcome_tokens = self.outcome_target_proj(self.outcome_input_norm(outcome_input))
        outcome_tokens = outcome_tokens.reshape(state_tokens.shape[0], NUM_OUTCOME_TOKENS, MODEL_DIM)
        return self.outcome_token_norm(outcome_tokens)

    def _neutral_outcome_tokens(self, state_tokens):
        reward_probs = state_tokens.new_zeros((state_tokens.shape[0], self.reward_num_bins))
        reward_probs[:, self.reward_num_bins // 2] = 1.0
        continuations = state_tokens.new_ones((state_tokens.shape[0],))
        return self._outcome_tokens(state_tokens, reward_probs, continuations)

    def _summary_from_state(self, state_tokens, outcome_tokens):
        return torch.cat([state_tokens, outcome_tokens], dim=1)

    def _encode_online_summary(self, x):
        state_tokens = self._encode_state_tokens(x)
        return self._summary_from_state(state_tokens, self._neutral_outcome_tokens(state_tokens))

    def encode_target_summary(self, x, reward_probs, continuations):
        state_tokens = self._encode_state_tokens(x)
        outcome_tokens = self._outcome_tokens(state_tokens, reward_probs, continuations)
        return self._summary_from_state(state_tokens, outcome_tokens)

    def decode_outcomes(self, summary_tokens, detach_summary=True):
        outcome_tokens = summary_tokens[:, NUM_STATE_TOKENS:]
        if detach_summary:
    ...
    def _actor_features_from_latents(self, latent_tokens):
        state_tokens = latent_tokens[:, :NUM_STATE_TOKENS]
        return self.actor_input_norm(state_tokens.reshape(state_tokens.shape[0], -1))

    def _critic_features_from_latents(self, latent_tokens):
        state_tokens = latent_tokens[:, :NUM_STATE_TOKENS]
        return self.critic_input_norm(state_tokens.reshape(state_tokens.shape[0], -1))
    ...
    def dynamics_teacher_forced(self, latent_history, action_history):
        ...
        flat_latent_history = latent_history.reshape(batch * horizon, 1, num_latents, width)
        flat_action_history = action_history.reshape(batch * horizon, 1, self.act_dim)
        pred_next_latents = self.predict_next_latents_from_history(flat_latent_history, flat_action_history)
        pred_reward_logits, pred_termination_logits = self.decode_outcomes(pred_next_latents)
        pred_next_latents = pred_next_latents.reshape(batch, horizon, num_latents, width)
    ...
    def get_imagined_action_dist(self, summary_tokens):
        ...
        action_mean = self._action_mean(agent_input)
    ...
    def get_imagined_action_and_value(self, summary_tokens, hl_support, action=None, gaussian_action=None):
        ...
        action_mean = self._action_mean(actor_input)
        action_std = torch.exp(self.actor_logstd.expand_as(action_mean))
    ...
    def get_action_logprob_entropy_from_latents(
        ...
        action_std = torch.exp(self.actor_logstd.expand_as(action_mean))
        probs = Normal(action_mean, action_std)
        return self._tanh_action_logprob_entropy(
            probs,
            action=action,
            gaussian_action=gaussian_action,
            sum_logprob=sum_logprob,
...
if __name__ == "__main__":
    ...
    grad_clip_groups = [
        ...
        (
            "reward",
            module_parameters(
                agent.outcome_readout_norm,
                agent.outcome_readout_proj,
                agent.dyn_reward_head,
            ),
        ),
        (
            "termination",
            module_parameters(agent.dyn_termination_head),
        ),
        ("actor", module_parameters(agent.actor_input_norm, agent.actor_mean) + [agent.actor_logstd]),
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                action, gaussian_action, logprob, _, value = agent.get_action_and_value_from_latents(
                    rollout_latents,
                    hl_support,
                    sum_logprob=False,
                )
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                window_latents = torch.cat([initial_summary.unsqueeze(1), target_future_summaries], dim=1)
                ...
                sigreg_latents = [
                    window_latents[:, horizon_idx, :NUM_STATE_TOKENS].reshape(mb_size, -1)
                ...
                target_reward_loss = -(reward_target_probs * torch.log_softmax(target_reward_logits, dim=-1)).sum(dim=-1)
                ...
                per_step_termination_loss = 0.5 * (
                    per_step_termination_loss + target_step_termination_loss
                )
                ...
                with torch.no_grad():
                    ...
                    state_pred_mse = (
                        F.mse_loss(
                            pred_next_latents[:, :, :NUM_STATE_TOKENS],
                            target_next_latents[:, :, :NUM_STATE_TOKENS].detach(),
                            reduction="none",
                        ).mean(dim=(-1, -2))
                        * latent_weight
                    ).sum() / latent_weight.sum().clamp_min(1.0)
                    ...
                    lejepa_state_pred_mses.append(state_pred_mse.item())
            ...
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    with torch.no_grad():
                        ...
                        real_dist = agent.get_imagined_action_dist(b_agent_latents[mb_inds])
                        real_actor_stds.append(real_dist.stddev.mean().item())
                    ...
                    spo_eps = torch.where(
                        (policy_weight * ratio_diff) > 0,
                        torch.full_like(policy_weight, args.spo_eps_high),
                        torch.full_like(policy_weight, args.spo_eps_low),
                    )
                    spo_penalty = policy_weight.abs() * ratio_diff.square() / (2.0 * spo_eps)
                    pg_loss = -(policy_weight * ratio - spo_penalty).mean()
        ...
        writer.add_scalar("lejepa/state_prediction_mse", safe_mean(lejepa_state_pred_mses), global_step)
