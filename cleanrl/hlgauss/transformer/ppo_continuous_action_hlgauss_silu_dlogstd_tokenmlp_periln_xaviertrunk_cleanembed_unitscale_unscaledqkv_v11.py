...
# - 2-layer shared transformer with RoPE, unit-scale attention, scaled QK norm, and unscaled V norm
...
class UnscaledRMSNorm(nn.Module):
            ...
            try:
                with sdpa_kernel(backends=[SDPBackend.FLASH_ATTENTION]):
                    out = F.scaled_dot_product_attention(
                        q.to(attn_dtype), k.to(attn_dtype), v.to(attn_dtype), dropout_p=0.0, scale=1.0
                    )
                return out.to(q.dtype)
            except RuntimeError:
                pass
    return F.scaled_dot_product_attention(q, k, v, dropout_p=0.0, scale=1.0)
