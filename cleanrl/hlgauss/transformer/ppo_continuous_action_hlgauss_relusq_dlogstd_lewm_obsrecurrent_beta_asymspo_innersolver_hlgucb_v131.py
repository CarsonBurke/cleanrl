# v131: v124 + HL-Gauss critic σ as UCB-style optimistic-bootstrap bonus.
#
# Motivation: the HL-Gauss critic outputs a full 51-bin categorical posterior
# over returns. v124 collapsed this to its scalar mean via `hl_support.to_scalar`
# and used only that for GAE. The std of the categorical (in actual value
# units after symexp) is a free epistemic-uncertainty signal: states where
# the critic disagrees-with-itself across bins are where exploration is most
# valuable. v131 adds an optimistic bootstrap to GAE,
#     V̂_opt(s_{t+1}) = V̂(s_{t+1}) + ucb_coef · decay(global_step) · σ_V(s_{t+1})
# where σ_V is the std of the HL-Gauss categorical (after symexp). decay is
# linear from 1.0 → 0.0 over `ucb_decay_steps` so the asymptotic critic
# target is the env-only return (no permanent bias).
#
# Why a bonus on the bootstrap rather than on the raw reward:
#   - cleanest interaction with PPO ratio / SPO trust region — only the
#     advantage and return targets shift, the actor's log-prob computation
#     is unchanged
#   - the bonus is naturally state-conditioned (no per-step intrinsic that
#     fights env reward at terminal states)
#   - symmetric with the existing critic semantics: `returns = advantages + values`
#     still holds; the critic still learns to predict the augmented return
#     during the bonus phase, then converges to env-only as decay → 0
#
...
def hl_value_std(logits, hl_support):
    """Standard deviation of the HL-Gauss categorical posterior in actual
    value space — i.e. Var[symexp(Z)]^½ when symlog is on, Var[Z]^½ otherwise.
    Used as a free epistemic uncertainty signal alongside v124's existing
    `hl_support.to_scalar` mean (which is intentionally NOT replaced — that
    is `symexp(E[Z_symlog])`, a Jensen-biased statistic v124 relies on).
    """
    probs = F.softmax(logits, dim=-1)
    eff_support = symexp(hl_support.support) if hl_support.use_symlog else hl_support.support
    mean_eff = (probs * eff_support).sum(dim=-1)
    var = (probs * (eff_support - mean_eff.unsqueeze(-1)).square()).sum(dim=-1)
...
class Args:
    ...
    ucb_coef: float = 0.05
    """weight on the HL-Gauss critic-σ optimistic bootstrap added to GAE (UCB-style exploration bonus).
       The bonus is applied to V̂(s_{t+1}) inside the GAE delta, decayed linearly to 0 over
       `ucb_decay_steps`. 0.0 disables the bonus."""
    ucb_decay_steps: int = 2_000_000
    """env steps over which `ucb_coef` linearly decays to 0. 0 disables decay (constant bonus)."""
...
class Agent(nn.Module):
    ...
    def get_value_logits(self, x):
        # v131: raw-obs version of `get_value_logits_from_latents` for the
        # GAE bootstrap, which needs the categorical logits (for σ_V) at the
        # last next_obs without re-collapsing through `to_scalar`.
        agent_input = self._encode_critic_features(x)
        return self.critic(agent_input)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.inference_mode():
                ...
                # v131: compute σ_V(s) from the HL-Gauss categorical for the UCB
                # bootstrap below. One extra critic-head forward; negligible cost.
                rollout_value_logits = agent.get_value_logits_from_latents(rollout_latents)
                rollout_value_std = hl_value_std(rollout_value_logits, hl_support)
                values[step] = value.flatten()
                value_stds[step] = rollout_value_std.flatten()
        ...
        with torch.inference_mode():
            next_obs_flat = next_obses.reshape((-1,) + envs.single_observation_space.shape)
            next_transition_value_logits = agent.get_value_logits(next_obs_flat)
            next_transition_values = hl_support.to_scalar(next_transition_value_logits).reshape(
                args.num_steps, args.num_envs
            )
            # v131: optimistic bootstrap. Augment V̂(s_{t+1}) with
            # `ucb_coef · decay · σ_V(s_{t+1})` inside the GAE delta only — the
            # critic still trains on env-only returns post-decay, since the
            # bonus magnitude linearly anneals to 0 over `ucb_decay_steps`.
            next_transition_value_stds = hl_value_std(
                next_transition_value_logits, hl_support
            ).reshape(args.num_steps, args.num_envs)
            if args.ucb_decay_steps > 0:
                ucb_decay = max(0.0, 1.0 - global_step / args.ucb_decay_steps)
            else:
                ucb_decay = 1.0
            ucb_now = args.ucb_coef * ucb_decay
            next_value_optimistic = next_transition_values + ucb_now * next_transition_value_stds
            ...
            for t in reversed(range(args.num_steps)):
                ...
                delta = rewards[t] + args.gamma * next_value_optimistic[t] * bootstrap_nonterminal - values[t]
        ...
        if not world_model_only:
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    # v124 amortization: regress actor mean toward optimized
                    # first-step action from the inner solver above.
                    actor_dist = agent._action_distribution(
                        agent._actor_features_from_latents(mb_latents)
                    )
                    actor_mean = agent._z_to_action(actor_dist.mean)
                    amortize_loss = F.mse_loss(actor_mean, a_star_first.detach())
        ...
        # v131: UCB diagnostics — std of the HL-Gauss categorical (in value
        # space) and the effective coefficient after decay.
        writer.add_scalar("ucb/value_std_mean", value_stds.mean().item(), global_step)
        writer.add_scalar("ucb/next_value_std_mean", next_transition_value_stds.mean().item(), global_step)
        writer.add_scalar("ucb/coef_now", ucb_now, global_step)
        writer.add_scalar(
            "ucb/bonus_mean", (ucb_now * next_transition_value_stds).mean().item(), global_step
        )
