...
class Agent(nn.Module):
    ...
    def predict_next_latents_and_values_all_from_history(self, latent_history, action_history, return_mtp=False):
        ...
        pred_latent_features = pred_tokens.reshape(batch, context_len, tokens_per_step, width)
        pred_latent_features = pred_latent_features[:, :, self.act_dim :]
...
if __name__ == "__main__":
    ...
    if args.dyn_horizon < MTP_PRED_LEN:
        raise ValueError(f"--dyn-horizon must be at least {MTP_PRED_LEN} for v104 MTP supervision")
    if args.imagine_horizon != args.dyn_horizon:
        raise ValueError("--imagine-horizon must equal --dyn-horizon for v104 horizon parity")
    if args.dream_behavior_prefix_len != args.dyn_horizon:
        raise ValueError("--dream-behavior-prefix-len must equal --dyn-horizon for v104 horizon parity")
