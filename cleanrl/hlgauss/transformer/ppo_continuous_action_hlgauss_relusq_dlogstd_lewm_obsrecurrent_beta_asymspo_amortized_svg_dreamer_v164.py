# v164: Dreamer-v3-stabilized amortized analytic value-gradient actor.
#
# v163 LEARNED then COLLAPSED: with low svg_ent_coef the Beta policy went
# deterministic and diverged back to random (~-350); with high entropy it never
# learned. Root cause: the analytic value gradient was too noisy/weak. v163 used
# a SINGLE H-step (H=3) imagined return with ONE terminal V(s_H) bootstrap read
# from the LIVE (non-target) HL-Gauss critic, and the unnormalized return scale
# made the entropy coefficient impossible to balance against the policy-gradient
# magnitude (which itself drifts as the critic moves).
#
# v164 applies the Dreamer-v3 actor recipe to stabilize the SAME amortized SVG
# actor (single-forward-pass Beta at rollout time, WM/critic training unchanged):
#   1. TARGET CRITIC: the per-step/terminal value used inside the imagined
#      rollout is read from an EMA copy of the value critic (self.critic +
#      self.critic_input_norm), decay critic_target_decay=0.98, updated once per
#      iteration after the critic CE step. This decouples the actor gradient from
#      the fast-moving online critic. The latent input stays differentiable so
#      the value gradient still flows into the actions; only the critic PARAMS
#      are frozen (a separate non-grad module, in NEITHER optimizer).
#   2. LAMBDA-RETURNS: instead of one H-step return + single bootstrap, compute a
#      Dreamer GAE-lambda return over the imagined rollout. At every step t we
#      read the target value v_t and form ret_t = r_t + gamma*continue_t *
#      ((1-lam) v_{t+1} + lam ret_{t+1}); the actor maximizes the MEAN over steps
#      of these per-step lambda-returns (sum_t ret_t / H). lam = svg_lambda=0.95.
#      This spreads the bootstrap across the whole horizon and lowers variance.
#   3. RETURN NORMALIZATION: an EMA of the 5th/95th percentiles of the (detached)
#      batch lambda-returns gives a scale S = max(1, hi - lo); the policy-gradient
#      term is divided by S before adding entropy, so svg_ent_coef is now scale-
#      invariant. S is treated as a constant (detached) in the loss.
#   4. LONGER HORIZON + balanced entropy: svg_horizon=15, svg_ent_coef=3e-3, and
#      entropy is the MEAN per-step entropy (horizon-invariant), not the raw sum.
# Gradient isolation, optimizer split, and the zero/step/zero recipe are
# unchanged; the target critic is excluded from both optimizers and updated only
# by EMA.
#
...
class Agent(nn.Module):
    ...
    def imagined_actor_objective(
        ...
        summary_history = [start_summary]
        action_history = []
        entropy_sum = start_summary.new_zeros(batch)
        last_summary = start_summary
        # Per-step rewards r_t and continuation gates continue_t for t=0..H-1.
        rewards = []
        continues = []
        # Target values v_t at every visited latent t=0..H (start + each predicted).
        values = [self.target_value(last_summary, hl_support)]
        for _ in range(horizon):
            # Live latent (no detach): gradient must flow through the trajectory.
            dist = self._action_distribution(self._actor_features_from_latents(last_summary))
            ...
            last_summary = pred_next_summary
            summary_history.append(pred_next_summary)
            values.append(self.target_value(last_summary, hl_support))
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.inference_mode():
                ...
                value_logits = agent.get_value_logits_from_latents(rollout_latents)
            ...
            if amortized_actor_active:
                ...
                with torch.no_grad():
                    dist = agent.get_imagined_action_dist(rollout_latents)
                    action_z = dist.sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        ...
        if not world_model_only:
            ...
            # v164: EMA-update the target value critic once per iteration, after
            # the online critic CE update (mirrors update_successor_flow_target).
            agent.update_critic_target(args.critic_target_decay)

            # v164 Dreamer-stabilized amortized SVG actor update. Imagine
            # svg_horizon steps through the FROZEN world model from a bounded,
            # randomly-subsampled set of real start summaries (decouples actor
            # imagination memory from the value minibatch — full minibatches OOM
            # at long horizons), form a Dreamer GAE-lambda return off the TARGET
            # critic, normalize it by an EMA percentile scale, and backprop the
            # analytic value gradient into the actor only. WM/critic/target are
            # frozen via the separate optimizer split plus the zero/step/zero
            # recipe below.
            imagine_batch = min(args.svg_imagine_batch, args.batch_size)
            actor_steps_per_epoch = max(1, args.batch_size // imagine_batch)
            for epoch in range(args.agent_update_epochs):
                for _ in range(actor_steps_per_epoch):
                    # Fresh random subset of real start states each actor step.
                    mb_inds = np.random.choice(args.batch_size, size=imagine_batch, replace=False)

                    start_summary = b_rollout_summaries[mb_inds].detach()
                    objective, entropy = agent.imagined_actor_objective(
                        start_summary,
                        args.svg_horizon,
                        args.gamma,
                        hl_support,
                    ...
                    # v164 Dreamer-v3 return normalization. Track an EMA of the
                    # 5th/95th percentiles of the (DETACHED) lambda-returns; the
                    # scale S = max(1, hi - lo) divides the policy-gradient term
                    # only and is treated as a constant in the loss.
                    with torch.no_grad():
                        det_obj = objective.detach()
                        lo = torch.quantile(det_obj, 0.05)
                        hi = torch.quantile(det_obj, 0.95)
                        agent.ret_ema_lo.lerp_(lo, 1.0 - 0.99)
                        agent.ret_ema_hi.lerp_(hi, 1.0 - 0.99)
                        ret_scale = (agent.ret_ema_hi - agent.ret_ema_lo).clamp_min(1.0)
                    actor_loss = -(objective / ret_scale + args.svg_ent_coef * entropy).mean()

                    # Strict gradient isolation: only actor_optimizer steps the
                    # actor; WM/critic/target grads produced by this backward are
                    # discarded (target params are non-grad so they carry none).
                    ...
                    actor_objectives.append(objective.mean().item())
                    actor_entropies.append(entropy.mean().item())
