# v129: v128 + (μ_z, κ) Beta reparameterization. In v128 the actor's
# Beta was parameterized by a single 2*A head producing (α_logit, β_logit),
# which entangles mean and variance: any gradient on σ² via β-NLL also
# moves α/β and therefore the mean output. v129 splits the head in two —
# `actor_mean_logit_head` (A outputs → sigmoid → μ_z) controls the
# location, and `actor_concentration_log_head` (A outputs → softplus → κ)
# controls the dispersion. We then form
#     α = 1 + μ_z · κ,   β = 1 + (1 − μ_z) · κ
# which keeps α, β ≥ 1 (unimodal Beta everywhere) and approximately
# decouples mean preference from concentration. The β-NLL σ² gradient
# now flows almost entirely through κ, leaving μ_z's regression toward
# `a_star_first` clean and MSE-like.
#
# Everything else from v128 / v124 is preserved (inner_solver_horizon=1,
# init=policy_mean, real_spo_coef=1.0, amortize_coef=1.0, wm_warmup=100k,
# β-NLL with β=0.5).
#
# Original v128 notes for context (Seitzer 2022 β-NLL amortization
# replacing v124's MSE):
#   - The inner solver returns a noisy point estimate of the local
#     argmin-cost action. β-NLL treats every `a_star_first` as a
#     Gaussian-noisy observation, weights the per-state NLL by sg(σ^{2β}),
#     and lets the actor's own Beta variance both (a) absorb residual
#     target noise and (b) act as a learned uncertainty signal. β=0.5
#     (inverse-std weighting) is Seitzer's recommended setting.
#
# Everything else from v124 is preserved:
#   - inner_solver_horizon=1, no restarts, init=policy_mean — small,
#     smooth, monotonic action-space refinement on top of the policy.
#   - cost = -(r_hat + gamma * V_hat(pred)) — HL-Gauss critic is part of
#     the cost gradient.
#   - amortize_coef=1.0, real_spo_coef=1.0 — the real-rollout SPO on
#     env GAE is still the primary policy driver; the amortization is a
#     WM-Jacobian regularizer that pulls the mean toward locally-improved
#     actions.
#   - wm_warmup_steps=100000 — the actor only opens once the WM's reward
#     head has finished coarse calibration.
#   - HL-Gauss critic, outcome / continuation / successor-flow probes,
#     SIGReg, MTP, etc. all from v123 unchanged.
#
...
class Args:
    ...
    """planning horizon over which the inner solver minimizes cost; 1 = single-step value gradient"""
    ...
    """amortization loss type: 'beta_nll' = Seitzer 2022 β-NLL (variance-weighted Gaussian NLL
       using the actor Beta's first two moments — treats `a_star_first` as a noisy target and
       lets the actor variance absorb residual noise); 'mse' = v124's MSE on the squashed
       Beta mean (no variance signal)."""
    nll_beta: float = 0.5
    """β-NLL variance-weighting exponent (Seitzer 2022, β=0.5 = inverse-std weighting, recommended;
       β=0 is vanilla NLL (variance-collapse-prone), β=1 is equivalent to MSE)"""
...
class Agent(nn.Module):
    def __init__(
        ...
        # v129: (μ_z, κ) reparam — split actor head in two so mean preference
        # and concentration are produced by independent weights.
        self.actor_mean_logit_head = ReluSqRMSHead(
            AGENT_INPUT_DIM, AGENT_HIDDEN_DIM, act_dim, output_std=0.01
        )
        self.actor_concentration_log_head = ReluSqRMSHead(
            AGENT_INPUT_DIM, AGENT_HIDDEN_DIM, act_dim, output_std=0.01
        )
    ...
    def _action_distribution(self, agent_input):
        # v129 (μ_z, κ) reparam: two independent heads.
        #   μ_z = sigmoid(actor_mean_logit_head(x))     in (0, 1)
        #   κ_raw = softplus(actor_concentration_log_head(x))   ≥ 0
        #   α = 1 + μ_z · κ_raw,   β = 1 + (1 − μ_z) · κ_raw
        # Keeps α, β ≥ 1 (unimodal Beta) and lets the concentration head
        # adjust dispersion without moving the mean preference head.
        mu_z = torch.sigmoid(self.actor_mean_logit_head(agent_input))
        kappa_raw = F.softplus(self.actor_concentration_log_head(agent_input))
        alpha = 1.0 + mu_z * kappa_raw
        beta = 1.0 + (1.0 - mu_z) * kappa_raw
    ...
    def get_imagined_raw_action_mean(self, summary_tokens):
        ...
        if self.detach_world_model_from_agent:
            latent_tokens = latent_tokens.detach()
        agent_input = self._actor_features_from_latents(latent_tokens)
        # v129: concatenate the two split heads so the diagnostic abs_mean /
        # abs_max scalars at the bottom of the iteration keep the same shape.
        mu_z_logit = self.actor_mean_logit_head(agent_input)
        concentration_log = self.actor_concentration_log_head(agent_input)
        return torch.cat([mu_z_logit, concentration_log], dim=-1)
...
if __name__ == "__main__":
    ...
    grad_clip_groups = [
        ...
        (
            "actor",
            module_parameters(
                agent.actor_input_norm,
                agent.actor_mean_logit_head,
                agent.actor_concentration_log_head,
            ),
        ),
    ...
    def run_inner_action_solver(start_summary, init_actions):
        """Adam on a free action variable through the frozen WM.

        start_summary: (B, NUM_LATENT_TOKENS, MODEL_DIM), detached.
        init_actions: (B, H, A) initial actions; clamped to action box.
        Returns (a_star, initial_cost, final_cost), all detached.
        """
        prev_states = freeze_agent_grad()
        try:
            action_var = init_actions.detach().clone().requires_grad_(True)
            inner_opt = optim.Adam([action_var], lr=args.inner_solver_lr)
            with torch.no_grad():
            ...
            for _ in range(args.inner_solver_steps):
                ...
                inner_opt.step()
                with torch.no_grad():
                    action_var.clamp_(action_low, action_high)
            with torch.no_grad():
        ...
        return action_var.detach(), initial_cost, final_cost
        ...
        if not world_model_only:
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    # v124: run the inner-Adam action solver BEFORE building any
                    # outer-graph losses. freeze_agent_grad / restore_agent_grad
                    # toggle requires_grad on the agent's leaves; running the
                    # solver first guarantees every loss graph below is built
                    # under the fully-restored requires_grad state.
                    ...
                    # v128 amortization: Seitzer 2022 β-NLL on the actor's
                    # Beta moments — treat `a_star_first` as a noisy
                    # observation, weight the per-state Gaussian NLL by
                    # sg(σ^{2β}). β=0.5 is the inverse-std-weighting
                    # recommended default. Falls back to v124's MSE when
                    # `amortize_loss_type == 'mse'`.
                    ...
                    if args.amortize_loss_type == "beta_nll":
                        action_scale = agent.action_high - agent.action_low
                        actor_var = (actor_dist.variance * (action_scale ** 2)).clamp_min(1e-8)
