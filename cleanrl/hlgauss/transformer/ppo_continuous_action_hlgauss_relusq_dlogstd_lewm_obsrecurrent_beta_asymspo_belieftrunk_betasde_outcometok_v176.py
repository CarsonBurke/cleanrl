...
class Agent(nn.Module):
    def __init__(
        ...
        # v174: NO output norm on target outcome tokens -- symmetric with obs tokens
        # (dyn_next_proj, un-normed) and with le-wm (raw, un-normalized targets). The
        # input embed norms (reward/continuation_outcome_input_norm) are kept.
    ...
    def _reward_outcome_token(self, reward_probs):
        # v174: proj(input_norm(.)) -- no output token norm, matching obs tokens.
