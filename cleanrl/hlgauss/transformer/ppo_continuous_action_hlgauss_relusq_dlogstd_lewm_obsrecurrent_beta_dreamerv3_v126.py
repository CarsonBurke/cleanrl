# v126: Dreamer V3-style pure-dream actor+critic learning. v125's inner
# Adam solver + amortization regression is removed entirely. The actor
# learns by reparameterized H-step rollouts through a frozen WM, with
# gradient flowing actor → WM Jacobian → reward/continuation/critic
# heads → loss. WM params do NOT accumulate from the actor loss thanks
# to requires_grad gating (frozen during the actor/critic phase).
#
# Key design choices (Dreamer V3-faithful, Hafner et al. 2023):
#   - Actor is a Beta(α, β); rsample() uses PyTorch's implicit-reparam
#     gradient (Figurnov et al. 2018). Bounded support means no squashing.
#   - Critic is HL-Gauss on PREDICTED latents only. Real-env returns are
#     not used to train the critic; the only value signal is the dream
#     λ-return computed inside imagined rollouts. This is the "agent
#     learns from its own predictions" formulation — biased early when
#     the WM is rough but converges as the WM does.
#   - Target critic via EMA decay for the bootstrap V(z_H), preventing
#     the actor → WM → critic → actor closed-loop instability.
#   - WM training (encoder, predictor, reward/continuation probes) is
#     unchanged from v123 and runs every step alongside actor+critic.
#     No wm_warmup gating by default — agent and WM co-train from step 0.
#   - Reward + continuation probes are trained on predicted tokens in
#     v123, so they are in-distribution under the dream rollout.
#   - Real-rollout SPO anchor is retained behind a `real_spo_coef`
#     CLI knob, defaulted to 0. Pure-dream learning by default.
#
# Compute: 1 forward + 1 backward through H=5 predictor calls per
# minibatch — vs v125's 8 inner Adam steps × 4 restarts × 5 horizon =
# 160 predictor backwards. ~20-30x faster than v125, ~5x slower than
# v124 (which had H=1, S=1 → not LeWM-faithful at all).
#
...
class Args:
    ...
    # v126 Dreamer V3-style pure-dream actor/critic
    dream_horizon: int = 5
    """imagined rollout horizon H for the dream actor/critic update (predictor calls per minibatch state)"""
    dream_lambda: float = 0.95
    """λ for the Dreamer V3 λ-return target used by both actor and critic on the dream rollout"""
    dream_gamma: float = 0.997
    """discount used inside the dream λ-return (Dreamer V3 default; combined with predicted continuation)"""
    dream_ent_coef: float = 3e-4
    """entropy bonus on the Beta actor inside the dream rollout (Dreamer V3 default range)"""
    dream_critic_coef: float = 1.0
    """weight on the dream critic HL-Gauss loss"""
    target_critic_decay: float = 0.995
    """EMA decay for the target critic used to bootstrap λ-returns through the dream
       rollout. Applied per-minibatch update (we have ~hundreds of agent grad-steps
       per env iteration, so this is slower than Dreamer V3's per-iteration 0.98)"""
    real_spo_coef: float = 0.0
    """weight on the real-rollout SPO actor anchor (0 by default — pure dream learning)"""
    dream_advantage_normalize: bool = True
    """if True, dream advantage (return - V_pred) is standardized per-update before the actor loss"""
...
if __name__ == "__main__":
    ...
    # --- v126 Dreamer V3 pure-dream actor/critic helpers ---

    # Collect WM-side params (encoder, predictor, outcome heads) once so the
    # dream phase can flip requires_grad on them cheaply. The actor and critic
    # param groups stay trainable throughout.
    _wm_phase_param_groups = ("wm", "outcome_inverse")
    wm_phase_params = []
    for group_name, params in grad_clip_groups:
        if group_name in _wm_phase_param_groups:
            wm_phase_params.extend(params)

    @contextlib.contextmanager
    def freeze_wm_for_actor():
        """Freeze WM + outcome_inverse params so the actor/critic backward
        flows gradients THROUGH the WM Jacobian but does not accumulate into
        WM parameters. Restores prior requires_grad on exit."""
        prev = [(p, p.requires_grad) for p in wm_phase_params]
        for p in wm_phase_params:
            p.requires_grad_(False)
        try:
            yield
        finally:
            for p, was_req in prev:
                p.requires_grad_(was_req)

    # Target critic for λ-return bootstrap. Mirrors the online critic head and
    # its dedicated input norm. Updated by EMA after each agent minibatch.
    target_critic_input_norm = copy.deepcopy(agent.critic_input_norm).to(device)
    target_critic = copy.deepcopy(agent.critic).to(device)
    for p in list(target_critic_input_norm.parameters()) + list(target_critic.parameters()):
        p.requires_grad_(False)

    @torch.no_grad()
    def update_target_critic(decay):
        for target_p, online_p in zip(target_critic_input_norm.parameters(), agent.critic_input_norm.parameters()):
            target_p.lerp_(online_p.detach(), 1.0 - decay)
        for target_p, online_p in zip(target_critic.parameters(), agent.critic.parameters()):
            target_p.lerp_(online_p.detach(), 1.0 - decay)
        for target_b, online_b in zip(target_critic_input_norm.buffers(), agent.critic_input_norm.buffers()):
            target_b.copy_(online_b)
        for target_b, online_b in zip(target_critic.buffers(), agent.critic.buffers()):
            target_b.copy_(online_b)

    def target_value_from_latents(latent_tokens):
        obs_tokens = latent_tokens[:, :NUM_OBS_TOKENS]
        agent_input = target_critic_input_norm(obs_tokens.reshape(obs_tokens.shape[0], -1))
        return hl_support.to_scalar(target_critic(agent_input))

    def dream_rollout(start_latents, horizon):
        """Reparametrized H-step dream rollout through the (frozen) WM.

        start_latents: (B, NUM_LATENT_TOKENS, MODEL_DIM), detached real-encoded latents.

        Returns lists indexed by step. z_traj has length H+1 (z_0..z_H); the
        rest have length H. Gradient flows actor -> WM Jacobian -> reward /
        continuation / next-z -> caller's loss.
        """
        z_traj = [start_latents]
        action_z_list = []
        reward_list = []
        continue_list = []
        log_prob_list = []
        entropy_list = []
        summary_history = [start_latents]
        action_history = []
        for _ in range(horizon):
            z_t = z_traj[-1]
            actor_input = agent._actor_features_from_latents(z_t)
            dist = agent._action_distribution(actor_input)
            ...
            z_next = agent.predict_next_latents_from_history(pred_context, action_context)
            outcome_tokens = z_next[:, NUM_OBS_TOKENS:]
            ...
            reward_list.append(reward_hat)
            continue_list.append(continue_hat)
            log_prob_list.append(log_prob)
            entropy_list.append(entropy)
            action_z_list.append(action_z)
            z_traj.append(z_next)
            summary_history.append(z_next)
        return z_traj, action_z_list, reward_list, continue_list, log_prob_list, entropy_list

    def compute_dream_lambda_returns(rewards_h, continues_h, values_hp1, gamma, lam):
        """Dreamer V3 λ-return computed over an H-step dream trajectory.

        rewards_h:    list/length H, each (B,)
        continues_h:  list/length H, each (B,) (predicted continuation)
        values_hp1:   list/length H+1, each (B,) (target critic at z_0..z_H)

        Returns: tensor (H, B) of λ-returns λ_0..λ_{H-1}.

        Backward: r_t and c_t carry gradient to actor; values are detached
        target-critic outputs but still depend on z_t (which depends on actor).
        Caller stop-grads as needed for the critic loss.
        """
        H = len(rewards_h)
        next_lambda = values_hp1[H]
        returns = [None] * H
        for t in reversed(range(H)):
            target = rewards_h[t] + gamma * continues_h[t] * (
                (1.0 - lam) * values_hp1[t + 1] + lam * next_lambda
            )
            returns[t] = target
            next_lambda = target
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Agent phase: pure Dreamer V3-style dream rollout. Real rollouts only
        # supply detached start latents (no real-env PPO anchor by default;
        # `real_spo_coef` is 0 by default for full Dreamer fidelity).
        ...
        dream_return_stds = []
        dream_reward_means = []
        dream_continue_means = []
        dream_value_means = []
        dream_actor_std_means = []
        dream_advantage_scales = []
        pg_loss = None
        old_approx_kl = None
        approx_kl = None
        ...
        if not world_model_only:
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    # Dream rollout + actor/critic loss under a WM freeze. The
                    # WM Jacobian still propagates gradient back to the actor
                    # because we backprop through the (now grad-less) WM ops;
                    # only WM parameter updates are blocked.
                    with freeze_wm_for_actor():
                        z_traj, _, reward_list, continue_list, log_prob_list, entropy_list = dream_rollout(
                            mb_latents, args.dream_horizon
                        )
                        # Target-critic bootstrap. We detach the latents going
                        # in so the target critic Jacobian does NOT itself
                        # feed gradient — only its scalar output matters as a
                        # bootstrap. (Actor gradient comes through r_t, c_t
                        # and the chain to z_t.)
                        target_values_hp1 = [target_value_from_latents(z.detach()) for z in z_traj]
                        lambda_returns = compute_dream_lambda_returns(
                            reward_list,
                            continue_list,
                            target_values_hp1,
                            args.dream_gamma,
                        ...
                        # Critic loss: HL-Gauss CE between online critic at
                        # sg(z_t) and sg(λ_t). Online critic stays trainable.
                        z_critic = torch.stack([z.detach() for z in z_traj[:-1]], dim=0)  # (H, B, T, D)
                        z_critic_flat = z_critic.reshape(-1, z_critic.shape[-2], z_critic.shape[-1])
                        critic_logits_flat = agent.get_value_logits_from_latents(z_critic_flat)
                        return_targets_flat = lambda_returns.detach().reshape(-1)
                        return_probs_flat = hl_support.project(return_targets_flat)
                        dream_critic_loss_t = -(
                            return_probs_flat * torch.log_softmax(critic_logits_flat, dim=-1)
                        ).sum(dim=-1).mean()

                        # Actor loss: maximize sum-of-λ-returns via reparam
                        # backprop. Optional std normalization (detached) keeps
                        # the actor-LR sensible across reward scales.
                        if args.dream_advantage_normalize:
                            adv_scale = lambda_returns.detach().std(unbiased=False).clamp_min(1e-3)
                            actor_objective = lambda_returns / adv_scale
                        else:
                            adv_scale = lambda_returns.new_ones(())
                            actor_objective = lambda_returns
                        stacked_entropy = torch.stack(entropy_list, dim=0)  # (H, B)
                        dream_actor_loss_t = -actor_objective.mean() - args.dream_ent_coef * stacked_entropy.mean()

                        total_loss = (
                            dream_actor_loss_t
                            + args.dream_critic_coef * dream_critic_loss_t
                        )

                        # Optional real-rollout SPO anchor (Dreamer-V3-faithful
                        # default is 0). When >0 we add a small SPO term to
                        # keep the actor near the behavior that produced the
                        # rollout — useful as a stability knob.
                        if args.real_spo_coef > 0.0:
                            _, _, newlogprob, real_entropy = agent.get_action_logprob_entropy_from_latents(
                            ...
                            total_loss = total_loss + args.real_spo_coef * pg_loss
                    ...
                    update_target_critic(args.target_critic_decay)

                    with torch.no_grad():
                        dream_actor_losses.append(dream_actor_loss_t.item())
                        dream_critic_losses.append(dream_critic_loss_t.item())
                        dream_entropy_means.append(stacked_entropy.mean().item())
                        dream_return_means.append(lambda_returns.mean().item())
                        dream_return_stds.append(lambda_returns.std(unbiased=False).item())
                        dream_reward_means.append(torch.stack(reward_list).mean().item())
                        dream_continue_means.append(torch.stack(continue_list).mean().item())
                        dream_value_means.append(torch.stack(target_values_hp1).mean().item())
                        dream_actor_std_means.append(
                            agent.get_imagined_action_std(mb_latents).mean().item()
                        )
                        dream_advantage_scales.append(adv_scale.item() if adv_scale.numel() == 1 else 1.0)
                        real_actor_stds.append(dream_actor_std_means[-1])
        ...
        if dream_actor_losses:
            writer.add_scalar("losses/dream_actor_loss", np.mean(dream_actor_losses), global_step)
            writer.add_scalar("losses/dream_critic_loss", np.mean(dream_critic_losses), global_step)
            writer.add_scalar("losses/entropy", np.mean(dream_entropy_means), global_step)
            writer.add_scalar("imagination/dream_return_mean", np.mean(dream_return_means), global_step)
            writer.add_scalar("imagination/dream_return_std", np.mean(dream_return_stds), global_step)
            writer.add_scalar("imagination/dream_reward_mean", np.mean(dream_reward_means), global_step)
            writer.add_scalar("imagination/dream_continue_mean", np.mean(dream_continue_means), global_step)
            writer.add_scalar("imagination/dream_target_value_mean", np.mean(dream_value_means), global_step)
            writer.add_scalar("imagination/actor_std_mean", np.mean(dream_actor_std_means), global_step)
            writer.add_scalar("imagination/advantage_scale", np.mean(dream_advantage_scales), global_step)
