# v157: faster multi-step LEJEPA reward-imagined goal MPC.
#
# Same objective as v156, but removes avoidable online planner cost:
# - solve an H-step plan and execute a short action block before replanning;
# - initialize the goal-reaching ActionSolver from the action sequence that
#   generated the target latent, then use a small refinement budget;
# - defer planner diagnostic host syncs until the rollout summary.
#
...
class Args:
    ...
    """planning horizon over which the inner solver minimizes cost"""
    ...
    planner_action_block: int = 3
    """number of optimized actions to execute from each solve before replanning"""
...
class Agent(nn.Module):
    ...
    def cost_from_action(
        ...
        Returns a tensor of shape (B,) with reward-probe cost
...
if __name__ == "__main__":
    ...
    def run_goal_embedding_solver(start_summary, init_actions):
        ...
        try:
            ...
            flat_init_actions = candidate_actions.reshape(batch_size * num_samples, horizon, -1)

            def predict_rollout_final(summary, action_seq):
                if action_seq.dim() == 2:
            ...
            with torch.no_grad():
                ...
                final_seed_action = flat_init_actions.reshape(batch_size, num_samples, horizon, -1)
            ...
            action_init_norm = ((best_goal_action - action_mid) / action_scale).clamp(
                -1.0 + SAMPLE_EPS,
                1.0 - SAMPLE_EPS,
            )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if planner_active:
                if planner_action_buffer is not None and planner_action_buffer.shape[1] > 0:
                    action = planner_action_buffer[:, 0]
                    planner_action_buffer = planner_action_buffer[:, 1:].detach().clone()
                    rollout_planner_solve_flags.append(0.0)
                    with torch.no_grad():
                        action_z = agent._action_to_z(action)
                        logprob = torch.zeros_like(action)
                else:
                    ...
                    action_block = min(
                        max(1, args.planner_action_block),
                        args.inner_solver_horizon,
                    )
                    planner_action_buffer = planner_action_seq[:, 1:action_block].detach().clone()
                    if args.planner_warm_start:
                        warm_consumed = min(
                            max(1, args.planner_receding_horizon),
                            args.inner_solver_horizon,
                        )
                        consumed = max(action_block, warm_consumed)
                        planner_next_init = planner_action_seq[:, consumed:].detach().clone()
                    rollout_planner_solve_flags.append(1.0)
                    with torch.no_grad():
                        action_z = agent._action_to_z(action)
                        logprob = torch.zeros_like(action)
                        ...
                        rollout_goal_values.append(planner_diagnostics["goal_value"].detach().mean())
            ...
            if planner_action_buffer is not None and bool(boundary_tensor.any()):
                # Avoid replaying an old plan after any vector env reset. This is
                # conservative but boundaries are infrequent in MuJoCo.
                planner_action_buffer = None
        ...
        if rollout_planner_solve_flags:
            writer.add_scalar("planner/solve_frac", safe_mean(rollout_planner_solve_flags), global_step)
