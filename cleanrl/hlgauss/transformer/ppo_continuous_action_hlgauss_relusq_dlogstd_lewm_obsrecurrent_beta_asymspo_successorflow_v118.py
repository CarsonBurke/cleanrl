...
class Args:
    ...
    """weight on the detached TD2-CFM successor-summary flow probe"""
    successor_flow_steps: int = 4
    """Euler midpoint steps used to sample successor summaries for WM values"""
    ...
    successor_value_samples: int = 2
    """number of successor-flow samples averaged for imagined WM value estimates"""
...
class Agent(nn.Module):
    def __init__(
        ...
        self.successor_flow_cond_norm = RMSNorm(successor_flow_dim)
    ...
    def successor_td2_flow_loss(
        ...
        direct_loss = (direct_loss * flat_weight).sum() / flat_weight.sum().clamp_min(1.0)
        ...
        boot_loss = (boot_loss * flat_bootstrap_weight).sum() / flat_bootstrap_weight.sum().clamp_min(1.0)
        return (1.0 - gamma) * direct_loss + gamma * boot_loss

    def successor_value(self, summary_tokens, reward_support, gamma, samples=1, steps=4):
        ...
        if samples > 1:
            condition_tokens = summary_tokens[:, None].expand(
                ...
                NUM_LATENT_TOKENS,
                MODEL_DIM,
            ).reshape(batch * samples, NUM_LATENT_TOKENS, MODEL_DIM)
        else:
    ...
    def predict_next_latents_and_values_all_from_history(self, latent_history, action_history, return_mtp=False, return_cache=False):
        ...
        if return_mtp:
            ...
            if return_cache:
                return pred_latents, pred_mtp_latents, pred_mtp_value_tokens, predictor_cache
            return pred_latents, pred_mtp_latents, pred_mtp_value_tokens
...
if __name__ == "__main__":
    ...
    def build_dream_batch(prompt_summary_history, prompt_action_history, prompt_valids, run_diagnostics=False):
        ...
        with torch.inference_mode():
            ...
            bootstrap_value = agent.successor_value(
                summary_history[-1].detach(),
                reward_support,
                args.gamma,
                samples=args.successor_value_samples,
                steps=args.successor_flow_steps,
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                successor_flow_loss = agent.successor_td2_flow_loss(
                    ...
                    latent_weight,
                    latent_weight * (1.0 - future_terminations),
                    args.gamma,
