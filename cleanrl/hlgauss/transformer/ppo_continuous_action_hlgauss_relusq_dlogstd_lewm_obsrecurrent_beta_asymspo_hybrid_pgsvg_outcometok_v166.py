# v166: identical to v165 EXCEPT the actor and critic now read ALL latent tokens
# (obs + outcome), not just the obs tokens.
#
# Rationale: the WM splits its latent into NUM_OBS_TOKENS obs tokens (trained by
# dynamics/JEPA/MTP -> next-state-predictive structure) and NUM_OUTCOME_TOKENS
# outcome tokens (the reward token, trained by reward CE, and the continuation
# token, trained by termination BCE). v165's actor/critic sliced OFF the outcome
# tokens, so the value head was denied EXACTLY the two features the WM was
# optimized to make value-relevant (immediate reward + survival), forcing it to
# re-derive them from a dynamics-oriented representation. v166 feeds both heads
# the full NUM_LATENT_TOKENS latent (Dreamer/MuZero-style sharing). AGENT_INPUT_DIM
# grows from NUM_OBS_TOKENS*MODEL_DIM to NUM_LATENT_TOKENS*MODEL_DIM; the two
# _*_features_from_latents helpers stop slicing. Everything else (hybrid PPO+SVG
# actor, HL-Gauss critic, gradient isolation via torch.autograd.grad, ret_scale
# EMA, target critic, per-iteration returns/real_target logging) is unchanged.
# Hypothesis: termination-aware value helps most on Hopper/Walker (value cliff);
# the reward token is a clean distilled velocity-reward feature on all three.
#
# --- prior history (v165 and earlier) ---
#
...
MTP_PRED_LEN = 4
SCALAR_EMBED_DIM = 32
# v166: actor/critic read the FULL latent (obs + outcome tokens), so the agent
# input spans all NUM_LATENT_TOKENS, not just the obs tokens.
...
class Agent(nn.Module):
    ...
    def _actor_features_from_latents(self, latent_tokens):
        # v166: use the FULL latent (obs + outcome tokens), no slicing.
        return self.actor_input_norm(latent_tokens.reshape(latent_tokens.shape[0], -1))

    def _critic_features_from_latents(self, latent_tokens):
        # v166: use the FULL latent (obs + outcome tokens), no slicing.
        return self.critic_input_norm(latent_tokens.reshape(latent_tokens.shape[0], -1))
    ...
    def target_value(self, latent, hl_support):
        ...
        """
        # v166: target critic reads the FULL latent (obs + outcome tokens), matching
        # the online critic's _critic_features_from_latents. No obs-only slice.
        critic_input = self.target_critic_input_norm(latent.reshape(latent.shape[0], -1))
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    if n_active > 0:
                        ...
                        if args.svg_coef > 0.0:
                            ...
                            # Dreamer-v3 percentile-EMA scale (persistent agent
                            # buffers ret_ema_lo/hi -> persists across iterations).
                            with torch.no_grad():
                                obj_det = objective.detach()
                                lo_new = torch.quantile(obj_det, 0.05)
                                hi_new = torch.quantile(obj_det, 0.95)
                                agent.ret_ema_lo.mul_(0.99).add_(0.01 * lo_new)
                                agent.ret_ema_hi.mul_(0.99).add_(0.01 * hi_new)
        ...
        # Log EVERY minibatch's GAE return target as its own point, each at an
        # interpolated global_step spanning this rollout's [base, base+batch]
        # range. No aggregation, no smoothing: this is the exact per-update target
        # the critic is trained on, at full resolution on the real-step axis.
        # (Normalized-reward space, so its scale differs from charts/episodic_return.)
        if real_return_means:
            vals = torch.stack(real_return_means).cpu().tolist()
            n = len(vals)
            base_step = global_step - args.batch_size
            for i, v in enumerate(vals):
                step_i = int(base_step + (i + 1) / n * args.batch_size)
                writer.add_scalar("returns/real_target", v, step_i)
