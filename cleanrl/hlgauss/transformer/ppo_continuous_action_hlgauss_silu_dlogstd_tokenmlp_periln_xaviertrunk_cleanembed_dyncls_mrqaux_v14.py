...
class Agent(nn.Module):
    ...
    def predict_dynamics(self, x, action):
        _, _, _, dyn_features = self._encode(x)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                pred_next_dyn, pred_reward_logits, pred_done_logits = agent.predict_dynamics(
                    b_obs[mb_inds], b_transition_actions[mb_inds]
                )
                with torch.no_grad():
                    target_next_dyn = target_agent.get_dyn_target(b_next_obs[mb_inds])
                dyn_latent_loss = (
                    F.mse_loss(pred_next_dyn, target_next_dyn, reduction="none").mean(dim=-1) * b_transition_valids[mb_inds]
                ).mean()
                reward_target_probs = reward_support.project(b_rewards[mb_inds])
                dyn_reward_loss = -(reward_target_probs * torch.log_softmax(pred_reward_logits, dim=-1)).sum(dim=-1).mean()
                dyn_done_loss = F.binary_cross_entropy_with_logits(pred_done_logits, b_transition_dones[mb_inds])
                dyn_loss = (
