...
# - v116: replaces the direct value-token readout with a TD2-CFM value-token flow.
#   The learned value target token remains HL-Gauss grounded, while dreamed value
#   bootstraps come from integrating the target-network-bootstrapped value flow.
# - v117: tests the long-horizon TD-flow hypothesis directly: D4-parity gamma=0.997,
#   slower value-flow EMA, and deterministic zero-source value-flow training to match
#   the inference source used for dreamed value bootstraps.
import copy
import os
import random
import sys
...
class Args:
    ...
    value_flow_steps: int = 4
    """Euler midpoint steps used to generate value tokens from the TD-flow"""
    value_flow_train_steps: int = 2
    """Euler midpoint steps used when sampling target bootstrap flow paths during training"""
    value_flow_target_decay: float = 0.999
    """EMA decay for the target value-flow vector field"""
    value_flow_zero_source_prob: float = 1.0
    """probability of training value-flow paths from the zero source used at inference"""
    value_flow_td_coef: float = 0.1
    """weight on the target-network TD value-flow consistency term"""
...
class Agent(nn.Module):
    def __init__(
        ...
        self.value_flow_steps = value_flow_steps
        ...
        self.value_flow_cond_norm = RMSNorm(NUM_LATENT_TOKENS * MODEL_DIM)
        self.value_flow_cond_proj = ReluSqRMSHead(
            NUM_LATENT_TOKENS * MODEL_DIM,
            AGENT_HIDDEN_DIM,
            MODEL_DIM,
            output_std=0.5,
        )
        self.value_flow_token_norm = RMSNorm(MODEL_DIM)
        self.value_flow_time_proj = nn.Sequential(
            xavier_init_linear(nn.Linear(1, MODEL_DIM)),
            ReluSq(),
            RMSNorm(MODEL_DIM),
            xavier_init_linear(nn.Linear(MODEL_DIM, MODEL_DIM)),
        )
        self.value_flow_input_norm = RMSNorm(3 * MODEL_DIM)
        self.value_flow_head = ReluSqRMSHead(
            3 * MODEL_DIM,
            AGENT_HIDDEN_DIM,
            MODEL_DIM,
            output_std=0.1,
        )
        self.target_value_flow_cond_norm = copy.deepcopy(self.value_flow_cond_norm)
        self.target_value_flow_cond_proj = copy.deepcopy(self.value_flow_cond_proj)
        self.target_value_flow_token_norm = copy.deepcopy(self.value_flow_token_norm)
        self.target_value_flow_time_proj = copy.deepcopy(self.value_flow_time_proj)
        self.target_value_flow_input_norm = copy.deepcopy(self.value_flow_input_norm)
        self.target_value_flow_head = copy.deepcopy(self.value_flow_head)
        for module in self.target_value_flow_modules():
    ...
    def value_flow_modules(self):
        return (
            self.value_flow_cond_norm,
            self.value_flow_cond_proj,
            self.value_flow_token_norm,
            self.value_flow_time_proj,
            self.value_flow_input_norm,
            self.value_flow_head,
        )

    def target_value_flow_modules(self):
        return (
            self.target_value_flow_cond_norm,
            self.target_value_flow_cond_proj,
            self.target_value_flow_token_norm,
            self.target_value_flow_time_proj,
            self.target_value_flow_input_norm,
            self.target_value_flow_head,
        )

    @torch.no_grad()
    def update_value_flow_target(self, decay):
        for target_module, online_module in zip(self.target_value_flow_modules(), self.value_flow_modules()):
    ...
    def _value_flow_velocity(self, summary_tokens, value_xt, flow_t, use_target=False, detach_summary=True):
        if value_xt.dim() == 3:
            value_xt = value_xt[:, 0]
        summary = summary_tokens.detach() if detach_summary else summary_tokens
        if flow_t.dim() == 0:
            flow_t = flow_t.expand(summary.shape[0])
        flow_t = flow_t.reshape(-1, 1).to(device=summary.device, dtype=summary.dtype)
        if use_target:
            cond_norm = self.target_value_flow_cond_norm
            cond_proj = self.target_value_flow_cond_proj
            token_norm = self.target_value_flow_token_norm
            time_proj = self.target_value_flow_time_proj
            input_norm = self.target_value_flow_input_norm
            flow_head = self.target_value_flow_head
        else:
            cond_norm = self.value_flow_cond_norm
            cond_proj = self.value_flow_cond_proj
            token_norm = self.value_flow_token_norm
            time_proj = self.value_flow_time_proj
            input_norm = self.value_flow_input_norm
            flow_head = self.value_flow_head
        cond = cond_proj(cond_norm(summary.reshape(summary.shape[0], -1)))
        xt = token_norm(value_xt)
        t_embed = time_proj(flow_t)
        flow_input = input_norm(torch.cat([cond, xt, t_embed], dim=-1))
        return flow_head(flow_input).unsqueeze(1)
    ...
    def integrate_value_flow(
        ...
        if start_token is None:
            value_token = summary_tokens.new_zeros(summary_tokens.shape[0], NUM_VALUE_TOKENS, MODEL_DIM)
        else:
            value_token = start_token
        if torch.is_tensor(end_t):
            end_t_tensor = end_t.reshape(-1, 1, 1).to(device=summary_tokens.device, dtype=summary_tokens.dtype)
        else:
            end_t_tensor = summary_tokens.new_full((summary_tokens.shape[0], 1, 1), float(end_t))
        dt = end_t_tensor / max(1, steps)
        for step_idx in range(max(1, steps)):
            mid_t = (step_idx + 0.5) * dt.squeeze(-1).squeeze(-1)
            velocity = self._value_flow_velocity(
                summary_tokens,
                value_token,
                mid_t,
                use_target=use_target,
                detach_summary=detach_summary,
            )
            value_token = value_token + dt * velocity
        if normalize_output:
            return self.value_token_norm(value_token)
        return value_token

    def predict_value_token(self, summary_tokens, detach_summary=True, steps=None):
        if steps is None:
            steps = self.value_flow_steps
        return self.integrate_value_flow(
            summary_tokens,
            steps=steps,
            use_target=False,
            detach_summary=detach_summary,
    ...
    def _sample_value_flow_source(self, target_value_tokens, zero_source_prob):
        source_tokens = torch.randn_like(target_value_tokens)
        if zero_source_prob > 0.0:
            zero_mask = (
                torch.rand(
                    target_value_tokens.shape[0],
                    1,
                    1,
                    device=target_value_tokens.device,
                    dtype=target_value_tokens.dtype,
                )
                < zero_source_prob
            )
            source_tokens = torch.where(zero_mask, torch.zeros_like(source_tokens), source_tokens)
        return source_tokens

    def value_flow_direct_loss(
        self,
        summary_tokens,
        target_value_tokens,
        sample_weight=None,
        detach_summary=True,
        zero_source_prob=0.0,
    ):
        if summary_tokens.dim() == 4:
            summary_tokens = summary_tokens.reshape(-1, NUM_LATENT_TOKENS, MODEL_DIM)
        if target_value_tokens.dim() == 4:
            target_value_tokens = target_value_tokens.reshape(-1, NUM_VALUE_TOKENS, MODEL_DIM)
        if summary_tokens.dim() != 3 or target_value_tokens.dim() != 3:
            raise ValueError(
                f"value_flow_direct_loss expected summary [N,T,D] and value [N,1,D], got {tuple(summary_tokens.shape)} and {tuple(target_value_tokens.shape)}"
            )
        target_value_tokens = target_value_tokens.detach()
        flow_t = torch.rand(summary_tokens.shape[0], device=summary_tokens.device, dtype=summary_tokens.dtype)
        source_tokens = self._sample_value_flow_source(target_value_tokens, zero_source_prob)
        path_t = flow_t.reshape(-1, 1, 1)
        value_xt = source_tokens.lerp(target_value_tokens, path_t)
        target_velocity = target_value_tokens - source_tokens
        pred_velocity = self._value_flow_velocity(
            summary_tokens,
            value_xt,
            flow_t,
            use_target=False,
        ...
        loss = F.mse_loss(pred_velocity, target_velocity, reduction="none").mean(dim=(-1, -2))
        if sample_weight is None:
            return loss
        flat_weight = sample_weight.reshape(-1)
        return (loss * flat_weight).sum() / flat_weight.sum().clamp_min(1.0)

    def value_td_bootstrap_flow_loss(
        self,
        ...
        flat_source = source_summary_tokens.reshape(-1, NUM_LATENT_TOKENS, MODEL_DIM)
        flat_next = next_summary_tokens.reshape(-1, NUM_LATENT_TOKENS, MODEL_DIM).detach()
        flat_weight = sample_weight.reshape(-1)
        flat_rewards = rewards.reshape(-1)
        flat_terminations = terminations.reshape(-1)
        with torch.no_grad():
            next_value_token = self.integrate_value_flow(
                flat_next,
                steps=target_flow_steps,
                use_target=True,
                detach_summary=True,
            )
            next_value = hl_support.to_scalar(self.decode_value_token(next_value_token))
            td_return = flat_rewards + gamma * (1.0 - flat_terminations) * next_value
            td_target_probs = hl_support.project(td_return)
            td_target_tokens = self.value_target_token(td_target_probs)
        return self.value_flow_direct_loss(
            flat_source,
            td_target_tokens,
            sample_weight=flat_weight,
            detach_summary=False,
            zero_source_prob=zero_source_prob,
        )

    def read_value(self, summary_tokens, hl_support, detach_summary=True):
        return hl_support.to_scalar(
            self.decode_value_token(self.predict_value_token(summary_tokens, detach_summary=detach_summary))
        )
    ...
    def predict_next_latents_and_values_all_from_history(self, latent_history, action_history, return_mtp=False, return_cache=False):
        ...
        pred_value_tokens = self.predict_value_token(
    ...
    def predict_next_latents_and_value_cached(self, latent_tokens, action, predictor_cache=None, max_context=PRED_CONTEXT):
        ...
        pred_latents = self.pred_next_proj(pred_latent_features)
        pred_value_tokens = self.predict_value_token(pred_latents, detach_summary=False)
...
if __name__ == "__main__":
    ...
    grad_clip_groups = [
        ...
        (
            ...
            module_parameters(
                ...
                agent.value_flow_cond_norm,
                agent.value_flow_cond_proj,
                agent.value_flow_token_norm,
                agent.value_flow_time_proj,
                agent.value_flow_input_norm,
                agent.value_flow_head,
    ...
    def build_dream_batch(prompt_summary_history, prompt_action_history, prompt_valids, run_diagnostics=False):
        ...
        with torch.inference_mode():
            for _ in range(args.imagine_horizon):
                ...
                current_value_token = agent.predict_value_token(summary_state)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                for mtp_idx in range(MTP_PRED_LEN):
                    ...
                    mtp_value_loss = agent.value_flow_direct_loss(
                        offset_pred_latents,
                        target_value_tokens[:, mtp_idx:],
                        offset_weight,
                        detach_summary=False,
                        zero_source_prob=args.value_flow_zero_source_prob,
                    )
                    mtp_value_offset_losses.append(
                        mtp_value_loss
                    )
                    if valid_horizon > 1:
                        td_weight = offset_weight[:, :-1] * latent_weight[:, mtp_idx + 1 :]
                        mtp_value_td_offset_losses.append(
                            agent.value_td_bootstrap_flow_loss(
                                offset_pred_latents[:, :-1],
                                target_next_latents[:, mtp_idx + 1 :],
                                future_rewards[:, mtp_idx + 1 :],
                                future_terminations[:, mtp_idx + 1 :],
                                td_weight,
                                hl_support,
                                args.gamma,
                                args.value_flow_train_steps,
                                args.value_flow_zero_source_prob,
                            )
                        )
                ...
                pred_value_cfm_loss = torch.stack(mtp_value_offset_losses).mean()
                pred_value_td_loss = (
                    torch.stack(mtp_value_td_offset_losses).mean()
                    if mtp_value_td_offset_losses
                    else pred_value_cfm_loss.detach() * 0.0
                )
                pred_value_token_loss = pred_value_cfm_loss + args.value_flow_td_coef * pred_value_td_loss
                ...
                initial_value_token_loss = agent.value_flow_direct_loss(
                    initial_core_summary,
                    initial_value_target_token,
                    detach_summary=True,
                    zero_source_prob=args.value_flow_zero_source_prob,
                ).mean()
                ...
                value_token_loss = 0.5 * (initial_value_token_loss + pred_value_token_loss)
                ...
                clip_grad_groups()
                optimizer.step()
                agent.update_value_flow_target(args.value_flow_target_decay)
        ...
        if prompt_summary_history is not None:
            ...
            imagined_learnable_steps += int(dream_batch[6].sum().item())
            (
                dream_states,
                dream_actions,
                dream_action_zs,
