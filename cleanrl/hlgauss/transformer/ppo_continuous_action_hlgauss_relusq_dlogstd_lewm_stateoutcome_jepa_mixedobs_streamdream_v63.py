...
#
# Key ideas:
# - a linear observation mixer maps the full raw observation vector to 8 encoder tokens
...
class Args:
    ...
    dream_build_batch_size: int = 4096
...
class Agent(nn.Module):
    def __init__(
        ...
        dyn_rope_cos, dyn_rope_sin = build_rope_cache(
            NUM_OBS_TOKENS, NUM_STATE_TOKENS, head_dim, torch.device("cpu")
        )
        ...
        if prompt_summary_history is not None:
            ...
            for epoch in range(args.imagine_update_epochs):
                ...
                for start in range(0, dream_states.shape[0], dream_minibatch_size):
                    ...
                    mb_dream_gaussian_actions = dream_gaussian_actions[mb_inds_t].to(device)
                    ...
                    if has_dream_targets:
                        imagine_actor_returns.append(mb_dream_returns[mb_dream_learn_mask].mean().item())
