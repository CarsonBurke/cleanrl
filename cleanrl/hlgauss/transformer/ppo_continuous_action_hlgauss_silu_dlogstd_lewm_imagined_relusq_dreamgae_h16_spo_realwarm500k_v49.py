...
# - imagined critic uses a scalar ReLU-squared value head for Dreamer-style lambda returns
...
# - v46: agent critic is a standard scalar ReLU-squared value head to avoid early HL-Gauss
#   support-edge bootstraps poisoning imagined PPO
...
# - v49: standard clipped PPO trains the agent on real rollouts through 500k env steps,
#   then the agent switches back to fixed-buffer imagined PPO only
import os
import random
import sys
import time
...
class Args:
    ...
    """number of real-rollout PPO epochs per rollout iteration during warm-start"""
    norm_adv: bool = True
    ...
    real_ppo_warmup_steps: int = 500000
    """global step cutoff for real-rollout PPO warm-start before imagination-only agent updates"""
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True, actor_mean_scale=3.0):
        ...
        reward_input_dim = MODEL_DIM * NUM_DYN_LATENT_TOKENS * 2 + act_dim + 1
        self.dyn_reward_input_norm = RMSNorm(reward_input_dim)
        self.dyn_reward_input_proj = xavier_init_linear(nn.Linear(reward_input_dim, MODEL_DIM))
    ...
    def _reward_features(self, current_latents, action, next_latents):
        ...
        reward_input = torch.cat([current_flat, action, action_energy, next_flat], dim=-1)
        reward_input = self.dyn_reward_input_norm(reward_input)
        return F.silu(self.dyn_reward_input_proj(reward_input))
    ...
    def get_imagined_value(self, summary_tokens, hl_support=None):
        ...
        if self.detach_world_model_from_agent:
            latent_tokens = latent_tokens.detach()
        agent_input = self._agent_features_from_latents(latent_tokens)
        return self.critic(agent_input).squeeze(-1)
    ...
    def get_imagined_action_and_value(self, summary_tokens, hl_support, action=None, gaussian_action=None):
        ...
        value = self.critic(agent_input).squeeze(-1)
...
if __name__ == "__main__":
    ...
    def build_dream_batch(initial_summary):
        ...
        diagnostic_n = min(args.imagination_diagnostic_batch, initial_summary.shape[0])
            ...
            for _ in range(args.imagine_horizon):
                ...
                if diagnostic_n > 0:
                    ...
                    try:
                        ...
                        _, diag_reward_logits, _ = agent.dynamics_step_from_history(
                            diag_pred_context,
                            diag_action_context,
                        )
    ...
    def build_dream_batch_eval(initial_summary):
        was_training = agent.training
        agent.eval()
        try:
            return build_dream_batch(initial_summary)
        finally:
    ...
    def dynamics_diagnostics(flat_obs, rollout_rewards, rollout_actions, rollout_terminations, rollout_boundaries, rollout_valids):
        ...
        try:
            ...
            _, sensitivity_reward_logits, _ = agent.dynamics_step_from_history(
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        real_ppo_warmup_active = global_step <= args.real_ppo_warmup_steps and args.agent_update_epochs > 0
        world_model_only = global_step < args.wm_warmup_steps and not real_ppo_warmup_active
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                lejepa_loss = args.dyn_coef * args.dyn_latent_coef * dyn_latent_loss + args.sigreg_coef * dyn_sigreg_loss
                ...
                optimizer.zero_grad()
                (args.dyn_coef * args.dyn_latent_coef * dyn_latent_loss + args.sigreg_coef * dyn_sigreg_loss).backward()
                (args.dyn_coef * dynamics_readout_loss).backward()
                nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                optimizer.step()
        ...
        if (
            (not world_model_only)
            and (not real_ppo_warmup_active)
            and use_imagination_updates
            and global_step >= args.imagine_start_step
        ):
            with torch.no_grad():
                initial_summary = agent.get_summary_targets(b_obs)

        # After the real PPO warm-start, actor/critic PPO updates come from
        # imagination while real rollouts continue training the WM/readouts.
        ...
        if initial_summary is not None:
            dream_batch = build_dream_batch_eval(initial_summary)
            ...
            for epoch in range(args.imagine_update_epochs):
                ...
                for start in range(0, dream_states.shape[0], dream_minibatch_size):
                    ...
                    dream_newvalue = agent.get_imagined_value(dream_states[mb_inds], hl_support)
                    imagine_unclipped_value_loss = (dream_newvalue - dream_returns[mb_inds]).square()
                    imagine_value_loss = imagine_unclipped_value_loss
                    if args.clip_vloss:
                        dream_oldvalue = dream_values[mb_inds]
                        dream_clipped_value = dream_oldvalue + (dream_newvalue - dream_oldvalue).clamp(
                            -args.clip_coef, args.clip_coef
                        )
                        imagine_clipped_value_loss = (dream_clipped_value - dream_returns[mb_inds]).square()
                        imagine_value_loss = torch.maximum(imagine_value_loss, imagine_clipped_value_loss)
                    imagine_value_loss = 0.5 * imagine_value_loss
                    if has_dream_targets:
        ...
        writer.add_scalar("charts/real_ppo_warmup_active", float(real_ppo_warmup_active), global_step)
        writer.add_scalar("charts/imagination_active", float(initial_summary is not None), global_step)
