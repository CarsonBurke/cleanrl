# v133: v132 PrefPoE + entropy regularization on pref + concentration cap.
#
# v132 collapse diagnosis (HC 8M run):
#   Peaked at +537 returns near 400k steps, then crashed to −511 by 600k
#   and never recovered. TB scalars at the crash:
#     pref_entropy:   −5.86 (400k) → −20.4  (600k)   [Beta α+β ~ 500/dim]
#     fused_entropy:  −3.51        → −15.1
#     KL(fused‖pref): 7.8          →  18.9
#     approx_kl:      0.10         →  0.99           [policy lurch]
#   Cause: v132 shipped with `prefpoe_alpha_entropy = 0.0`, dropping the
#   `−α·H(π_pref)` term from the paper's L_pref. Advantage-weighted log-
#   likelihood then drives α_pref+β_pref to infinity unboundedly; the
#   PoE fusion `α_fused = α_θ + λ(α_pref−1)` drags the sampling
#   distribution into a Dirac, killing exploration.
#
# v133 fixes:
#   (1) prefpoe_alpha_entropy default 0.0 → 0.5
#       Paper §3.2 eq (2) and §4 explicitly require this term for
#       positive-definiteness (Theorem 2) and non-collapse. With β_1=1,
#       α/β_1=0.5 sets a Boltzmann temperature of order 1 — sharp but
#       not Dirac. Paper Fig 2(a) shows HC pref_entropy stable in [2.5,10]
#       for Gaussian; with our Beta, we expect pref_entropy ∈ [−6, −1].
#   (2) Hard cap on Beta concentration in `_pref_action_distribution`:
#       α_pref ≤ pref_concentration_cap, same for β_pref. Defensive
#       backstop in case entropy reg alone is insufficient. Default cap
#       = 49 ⇒ α_pref+β_pref ≤ 100 per dim ⇒ Beta entropy ≥ −2.3/dim
#       ≥ −13.8 total for HC-6d. No clamp on main_dist (separation).
#   (3) Header doc updated; no other algorithmic changes from v132.
#
...
#
# v133 defaults:
#   lambda_pref=0.5, prefpoe_beta1=1.0, prefpoe_alpha_entropy=0.5,
...
class Args:
    ...
    """β_1 in the paper — strength of advantage guidance on the preference loss
       L_pref = −β_1·E[A_norm·log π_pref(a|s)] − α·H(π_pref)."""
    prefpoe_alpha_entropy: float = 0.5
    """α in the paper — entropy regularization on the preference head.
       v132 shipped with 0.0 → pref Beta concentration ran away to ~500/dim and
       fused collapsed to Dirac (returns crashed at 500k). With β_1=1, α/β_1=0.5
       sets a Boltzmann temperature of order 1 (paper Theorem 1)."""
        ...
        if not world_model_only:
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    # v132 PrefPoE losses (paper eq. 2 + KL consistency):
                    #   L_pref = −β_1·E[A_norm·log π_pref(a_z|s)] − α·H(π_pref)
                    #   L_cons = KL(π_fused ‖ π_pref)
                    # Both gated by warmup and zero-coef short-circuit.
                    ...
                    if prefpoe_active and args.prefpoe_w_cons > 0.0:
                        cons_loss = torch.distributions.kl.kl_divergence(
