# v154: direct reward MPC with one-step scalar reward supervision.
#
# v153 direct reward MPC collapsed while diagnostics showed a nearly
# action-flat reward readout. Review found that scalar MTP reward/termination
# CE trains every offset from a shared timestep representation, including
# future rewards without the intervening future actions. v154 keeps latent MTP
# but applies reward/termination CE only to mtp_idx == 0 so the scalar planner
# interface is trained as the immediate action-conditioned transition reward.
#
# v153: direct reward-only MPC through the differentiable world model.
#
# v152 fixed the predicted-reward detach bug, but the H=1 two-stage goal solver
# is redundant: the optimized witness action already reaches its own predicted
# goal. v153 uses the existing frozen-WM action GradientSolver directly,
# executes the optimized action sequence, increases the default horizon to 3,
# and removes critic bootstrapping from the planner cost so early MPC is not
# dominated by an untrained value function.
#
# v152: full-summary optimal-goal MPC with reward-gradient dynamics fix.
#
# v150 preserved the GoalSolver return through ActionSolver but still plateaued
# near zero. The scalar reward/termination losses decoded predicted latents with
# detach_summary=True, training only inverse heads while the action-conditioned
# predictor never received reward/termination gradient. v152 keeps v150's
# full-summary pure-MPC controller and changes only that math: predicted
# reward/termination decode losses now backprop into the predicted WM latents.
#
...
class Args:
    ...
    """planning horizon over which the direct MPC solver maximizes predicted reward"""
...
class Agent(nn.Module):
    ...
    def cost_from_action(
        ...
        Returns a tensor of shape (B,) with reward-only cost
            C = -sum_{k<H} gamma^k r_hat_k
...
if __name__ == "__main__":
    ...
    # v148: the actor is not an amortized controller or a behavior policy. Freeze
    # it so the experiment is WM/value learning plus differentiable MPC only.
    for module in (agent.actor_input_norm, agent.actor_beta, agent.actor_pref_beta):
        module.requires_grad_(False)
    ...
    def run_inner_action_solver(start_summary, init_actions):
        ...
        Returns (a_star, initial_cost, final_cost, diagnostics), all detached.
        """
        ...
        try:
            ...
            with torch.no_grad():
                initial_action = bounded_action().detach()
            ...
            for _ in range(args.inner_solver_steps):
                ...
                with torch.no_grad():
                    if action_u.grad is not None:
                        last_grad_norm = action_u.grad.detach().flatten(1).norm(dim=-1)
                inner_opt.step()
            with torch.no_grad():
                final_action = bounded_action().detach()
                ...
                initial_action = initial_action.reshape(
                    batch_size,
                    num_samples,
                    args.inner_solver_horizon,
                    -1,
                )
                best_initial_action = initial_action[batch_idx, best_idx]
                best_initial_cost = initial_cost[batch_idx, best_idx]
                best_final_cost = final_cost[batch_idx, best_idx]
                if last_grad_norm is None:
                    last_grad_norm = torch.zeros_like(final_cost.reshape(-1))
                last_grad_norm = last_grad_norm.reshape(batch_size, num_samples)[batch_idx, best_idx]
                diagnostics = {
                    "goal_initial_energy": best_initial_cost.detach(),
                    "goal_final_energy": best_final_cost.detach(),
                    "goal_return": (-best_final_cost).detach(),
                    "goal_value": torch.zeros_like(best_final_cost),
                    "goal_defect": torch.zeros_like(best_final_cost),
                    "exec_return": (-best_final_cost).detach(),
                    "return_loss": torch.zeros_like(best_final_cost),
                    "goal_action_abs_delta": (
                        best_action - best_initial_action
                    ).abs().mean(dim=(-1, -2)).detach(),
                    "action_abs_delta": (
                        best_action - best_initial_action
                    ).abs().mean(dim=(-1, -2)).detach(),
                    "action_l2_delta": (
                        best_action - best_initial_action
                    ).flatten(1).norm(dim=-1).detach(),
                    "action_grad_norm": last_grad_norm.detach(),
                }
        finally:
    ...
    def run_goal_embedding_solver(start_summary, init_actions):
        ...
        defines g as the resulting predicted next latent summary. ActionSolver
        then minimizes summed final embedding distance from WM(z, a) to
        stopgrad(g); return is not used in the executed-action objective.
        """
        ...
        try:
            ...
            with torch.no_grad():
                ...
                final_goal_summary = final_pred.detach()
            ...
            def action_distance():
                ...
                return (pred_next - best_goal_summary).square().sum(dim=(-1, -2))
            ...
            with torch.no_grad():
                ...
                exec_energy = agent.cost_from_action(
                    start_summary,
                    final_action,
                    hl_support,
                    reward_support,
        ...
        for step in range(0, args.num_steps):
            ...
            if planner_active:
                ...
                with torch.no_grad():
                    ...
                    rollout_exec_returns.append(planner_diagnostics["exec_return"].mean().item())
                    rollout_return_losses.append(planner_diagnostics["return_loss"].mean().item())
