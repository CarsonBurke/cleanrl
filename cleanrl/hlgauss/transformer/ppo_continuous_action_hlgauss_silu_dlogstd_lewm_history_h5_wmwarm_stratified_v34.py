...
# - a second 2-layer agent transformer over the 2-token shared latent summary plus learned actor/critic/SDE CLS tokens
...
# - 5-step teacher-forced dynamics training masked across episode boundaries
# - LeWM-style SIGReg regularizes the full real rollout latent tensor toward an isotropic Gaussian
# - agent losses see detached world-model latents; the LeWM objective still trains encoder+predictor end-to-end
import os
import random
import sys
import time
...
NUM_AGENT_HEAD_TOKENS = 3
NUM_AGENT_SPECIAL_TOKENS = NUM_DYN_LATENT_TOKENS + NUM_AGENT_HEAD_TOKENS
SCALAR_EMBED_DIM = 32
...
class Args:
    ...
    dyn_reward_coef: float = 0.0
    """optional weight on reward prediction; default 0 keeps the LeWM objective reward-free"""
    dyn_termination_coef: float = 0.0
    """optional weight on true termination prediction"""
    ...
    imagine_actor_coef: float = 0.0
    """optional weight on the dreamed actor objective"""
    imagine_critic_coef: float = 0.0
    """optional weight on the dreamed critic objective"""
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True):
        ...
        agent_cls = torch.empty(NUM_AGENT_HEAD_TOKENS, MODEL_DIM)
    ...
    def _encode_agent_tokens(self, latent_tokens):
    ...
    def _encode_agent_features(self, x):
        ...
        _, _, actor_features, critic_features, sde_features = self._encode_agent_tokens(latent_tokens)
        reward_features, termination_features = self._dyn_head_features(dyn_head_tokens)
        return reward_features, termination_features, actor_features, critic_features, sde_features
    ...
    def get_imagined_action_dist(self, summary_tokens):
        ...
        if self.detach_world_model_from_agent:
            latent_tokens = latent_tokens.detach()
        _, _, actor_features, _, sde_features = self._encode_agent_tokens(latent_tokens)
    ...
    def get_imagined_value_logits(self, summary_tokens):
        ...
        if self.detach_world_model_from_agent:
            latent_tokens = latent_tokens.detach()
        _, _, _, critic_features, _ = self._encode_agent_tokens(latent_tokens)
    ...
    def get_value_logits(self, x):
        _, _, _, critic_features, _ = self._encode_agent_features(x)
        return self.critic_head(critic_features)
    ...
    def get_action_and_value(self, x, hl_support, action=None):
        _, _, actor_features, critic_features, sde_features = self._encode_agent_features(x)
        action_mean = self.actor_mean_head(actor_features)
        action_logstd = torch.clamp(self.sde_logstd_head(sde_features), -5.0, 2.0)
        action_std = torch.exp(action_logstd)
        probs = Normal(action_mean, action_std)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                sigreg_samples = torch.cat(
                    [latents[valids] for latents, valids in zip(sigreg_latents, sigreg_valids)], dim=0
                )
                if sigreg_samples.shape[0] > 1:
                    dyn_sigreg_loss = sigreg(sigreg_samples)
                else:
                    dyn_sigreg_loss = sigreg_samples.sum() * 0.0
                dyn_loss = (
        ...
        # Agent phase on a frozen world-model interface
            ...
            for epoch in range(args.agent_update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    _, newlogprob, entropy, newvalue = agent.get_action_and_value(
                        b_obs[mb_inds], hl_support, b_actions[mb_inds]
                    )
        ...
        writer.add_scalar("losses/dyn_value_loss", np.mean(dyn_value_losses), global_step)
