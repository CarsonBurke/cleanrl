# PPO + HL-Gauss with a LeWM-style action-conditioned latent predictor.
#
# Key ideas:
...
# - a separate predictor transformer rolls latent tokens forward conditioned on clipped env actions
...
# - a world-model-side value head trains the shared latent pair to preserve control-relevant return structure
...
class Args:
    ...
    """weight on world-model value prediction from the shared latent tokens"""
...
class AdaLNTransformerBlock(nn.Module):
    ...
    def forward(self, x, action_features, rope_cos, rope_sin):
        batch, seq_len, width = x.shape
        ...
        attn_out = attention(q, k, v, dropout_p=dropout_p)
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True):
        ...
        pred_rope_cos, pred_rope_sin = build_rope_cache(NUM_DYN_LATENT_TOKENS, 0, head_dim, torch.device("cpu"))
    ...
    def predict_dynamics(self, x, action):
        summary_tokens = self.get_summary_targets(x).detach()
        return self.dynamics_step(summary_tokens, action)

    def predict_next_latents(self, latent_tokens, action):
        action_features = F.silu(self.pred_action_proj(action))
        pred_tokens = latent_tokens
        for layer in self.pred_layers:
            pred_tokens = layer(pred_tokens, action_features, self.pred_rope_cos, self.pred_rope_sin)
        pred_delta = self.pred_delta_head(self.pred_final_norm(pred_tokens))
        return latent_tokens + pred_delta

    def dynamics_step(self, summary_tokens, action):
        latent_tokens, dyn_head_tokens = self._split_summary_tokens(summary_tokens)
        pred_next_latents = self.predict_next_latents(latent_tokens, action)
        pred_next_summary = torch.cat([pred_next_latents, dyn_head_tokens.detach().new_zeros(dyn_head_tokens.shape)], dim=1)
        predicted_latent_features = self._latent_value_features(pred_next_latents)
        reward_features = predicted_latent_features
        termination_features = predicted_latent_features
        pred_reward_logits = self.dyn_reward_head(reward_features)
        pred_termination_logits = self.dyn_termination_head(termination_features).squeeze(-1)
    ...
    def get_dyn_value_logits_from_summary(self, summary_tokens):
        latent_tokens, _ = self._split_summary_tokens(summary_tokens)
        value_features = self._latent_value_features(latent_tokens)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        rollout_latent_tokens = rollout_summary_tokens[:, :, :NUM_DYN_LATENT_TOKENS, :].reshape(
            args.num_steps, args.num_envs * NUM_DYN_LATENT_TOKENS, MODEL_DIM
        )
        ...
        optimizer.zero_grad()
        dyn_sigreg_loss = sigreg(rollout_latent_tokens)
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Keep dynamics losses from reshaping the encoder/agent latents.
                # The predictor learns on top of the detached encoder space, then
                # remains differentiable through its own autoregressive rollout.
                summary_tokens = agent.get_summary_targets(b_obs[mb_inds]).detach()
                ...
                for horizon_idx in range(args.dyn_horizon):
                    ...
                    with torch.no_grad():
                        target_next_summary = agent.get_summary_targets(future_next_obs).detach()
                ...
                optimizer.zero_grad()
                (args.dyn_coef * dyn_loss).backward()
                nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                optimizer.step()
        ...
        dream_batch = None
        if (not world_model_only) and global_step >= args.imagine_start_step:
            with torch.no_grad():
