...
# - an encoder transformer maps observations to 2 compact latent tokens
...
# - imagined actor uses a standard PPO relu-squared MLP on detached world-model latents
# - imagined critic uses a relu-squared HL-Gauss value head for Dreamer-style lambda returns
import os
import random
import sys
import time
...
PRED_DROPOUT = 0.1
PRED_CONTEXT = 4
NUM_DYN_LATENT_TOKENS = 2
NUM_DYN_HEAD_TOKENS = 2
NUM_DYN_SPECIAL_TOKENS = NUM_DYN_LATENT_TOKENS + NUM_DYN_HEAD_TOKENS
SCALAR_EMBED_DIM = 32
AGENT_INPUT_DIM = NUM_DYN_LATENT_TOKENS * MODEL_DIM
...
class Args:
    ...
    clip_coef: float = 0.2
    """value-loss clipping coefficient; SPO replaces the imagined actor hard clip"""
    ...
    dyn_value_coef: float = 0.0
    """optional weight on world-model value prediction"""
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True):
        ...
        self.latent_cls = nn.Parameter(summary_cls[:NUM_DYN_LATENT_TOKENS].unsqueeze(0))
        self.dyn_head_cls = nn.Parameter(summary_cls[NUM_DYN_LATENT_TOKENS:].unsqueeze(0))
        ...
        self.pred_action_proj = xavier_init_linear(nn.Linear(act_dim, MODEL_DIM))
        self.pred_layers = nn.ModuleList(
            [
                AdaLNTransformerBlock(MODEL_DIM, NUM_Q_HEADS, NUM_KV_HEADS, FFN_MULT, PRED_DROPOUT)
                for _ in range(PRED_NUM_LAYERS)
            ]
        )
        self.pred_final_norm = RMSNorm(MODEL_DIM)
        self.pred_delta_head = nn.Linear(MODEL_DIM, MODEL_DIM)
        nn.init.zeros_(self.pred_delta_head.weight)
        nn.init.zeros_(self.pred_delta_head.bias)
        ...
        pred_rope_cos, pred_rope_sin = build_rope_cache(
            PRED_CONTEXT * NUM_DYN_LATENT_TOKENS, 0, head_dim, torch.device("cpu")
        )
        ...
        self.latent_value_norm = RMSNorm(MODEL_DIM * NUM_DYN_LATENT_TOKENS)
        self.latent_value_proj = xavier_init_linear(nn.Linear(MODEL_DIM * NUM_DYN_LATENT_TOKENS, MODEL_DIM))
        self.dyn_value_head = layer_init(nn.Linear(MODEL_DIM, num_bins), std=1.0)
        self.dyn_head_norm = RMSNorm(MODEL_DIM * NUM_DYN_HEAD_TOKENS)
        self.dyn_reward_proj = xavier_init_linear(nn.Linear(MODEL_DIM * NUM_DYN_HEAD_TOKENS, MODEL_DIM))
        self.dyn_termination_proj = xavier_init_linear(nn.Linear(MODEL_DIM * NUM_DYN_HEAD_TOKENS, MODEL_DIM))
    ...
    def _encode_dynamics_tokens(self, x):
        ...
        summary_tokens = torch.cat(
            [
                ...
                self.dyn_head_cls.expand(batch, -1, -1),
            ],
            dim=1,
        )
    ...
    def _split_summary_tokens(self, summary_tokens):
        latent_tokens = summary_tokens[:, :NUM_DYN_LATENT_TOKENS]
        dyn_head_tokens = summary_tokens[:, NUM_DYN_LATENT_TOKENS:]
        return latent_tokens, dyn_head_tokens

    def _dyn_head_features(self, dyn_head_tokens):
        flat_dyn_heads = dyn_head_tokens.reshape(dyn_head_tokens.shape[0], -1)
        flat_dyn_heads = self.dyn_head_norm(flat_dyn_heads)
        reward_features = F.silu(self.dyn_reward_proj(flat_dyn_heads))
        termination_features = F.silu(self.dyn_termination_proj(flat_dyn_heads))
        return reward_features, termination_features
    ...
    def _latent_value_features(self, latent_tokens):
        ...
        return F.silu(self.latent_value_proj(flat_latents))
    ...
    def _encode_agent_features(self, x):
        ...
        latent_tokens, dyn_head_tokens = self._split_summary_tokens(summary_tokens)
        if self.detach_world_model_from_agent:
            latent_tokens = latent_tokens.detach()
        return latent_tokens.reshape(latent_tokens.shape[0], -1)
    ...
    def predict_next_latents_from_history(self, latent_history, action_history):
        ...
        pred_tokens = latent_history.reshape(batch, context_len * num_latents, width)
        action_features = F.silu(self.pred_action_proj(action_history))
        action_features = action_features.unsqueeze(2).expand(-1, -1, num_latents, -1)
        action_features = action_features.reshape(batch, context_len * num_latents, width)
        rope_cos = self.pred_rope_cos[: context_len * num_latents]
        rope_sin = self.pred_rope_sin[: context_len * num_latents]
        attn_mask = temporal_causal_mask(context_len, num_latents, pred_tokens.device)
        for layer in self.pred_layers:
            pred_tokens = layer(pred_tokens, action_features, rope_cos, rope_sin, attn_mask=attn_mask)
        pred_delta = self.pred_delta_head(self.pred_final_norm(pred_tokens))
        pred_latents = latent_history + pred_delta.reshape(batch, context_len, num_latents, width)
        return pred_latents[:, -1]

    def dynamics_step_from_history(self, summary_history, action_history):
        latent_history = summary_history[:, :, :NUM_DYN_LATENT_TOKENS]
        dyn_head_tokens = summary_history[:, -1, NUM_DYN_LATENT_TOKENS:]
        pred_next_latents = self.predict_next_latents_from_history(latent_history, action_history)
        zero_dyn_heads = dyn_head_tokens.detach().new_zeros(dyn_head_tokens.shape)
        pred_next_summary = torch.cat([pred_next_latents, zero_dyn_heads], dim=1)
        predicted_latent_features = self._latent_value_features(pred_next_latents.detach())
        pred_reward_logits = self.dyn_reward_head(predicted_latent_features)
    ...
    def get_imagined_action_dist(self, summary_tokens):
        latent_tokens, _ = self._split_summary_tokens(summary_tokens)
        if self.detach_world_model_from_agent:
            latent_tokens = latent_tokens.detach()
        agent_input = latent_tokens.reshape(latent_tokens.shape[0], -1)
        action_mean = self.actor_mean(agent_input)
        action_std = torch.exp(self.actor_logstd.expand_as(action_mean))
        return Normal(action_mean, action_std)

    def _tanh_action_logprob_entropy(self, probs, action=None, gaussian_action=None):
        if gaussian_action is None:
            ...
            action = torch.tanh(gaussian_action)
        logprob_per_dim = probs.log_prob(gaussian_action) - torch.log(1 - action.square() + 1e-6)
    ...
    def get_imagined_action_and_value(self, summary_tokens, hl_support, action=None, gaussian_action=None):
        ...
        value = hl_support.to_scalar(self.critic(agent_input))
    ...
    def get_dyn_value_logits_from_summary(self, summary_tokens):
        latent_tokens, _ = self._split_summary_tokens(summary_tokens)
        value_features = self._latent_value_features(latent_tokens.detach())
        return self.dyn_value_head(value_features)
...
if __name__ == "__main__":
    ...
    def masked_time_major_sigreg(sigreg_latents, sigreg_valids):
        ...
        for latents, valids in zip(sigreg_latents, sigreg_valids):
            ...
            if valid_latents.shape[0] > 1:
                sigreg_losses.append(sigreg(valid_latents.unsqueeze(0)))
        if sigreg_losses:
            return torch.stack(sigreg_losses).mean()
        return sigreg_latents[0].sum() * 0.0

    def imagined_lambda_returns(rewards_hat, continues_hat, values_hat):
        returns = []
    ...
    def build_dream_batch(initial_summary):
        ...
        action_history = []
        alive = torch.ones(initial_summary.shape[0], device=initial_summary.device)
        with torch.no_grad():
            for _ in range(args.imagine_horizon):
                ...
                pred_continue = (1.0 - termination_prob) * alive * (1.0 - sampled_terminal)
        ...
        returns = imagined_lambda_returns(rewards_hat, continues_hat, values + [bootstrap_value])
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                sigreg_latents = [summary_tokens[:, :NUM_DYN_LATENT_TOKENS].reshape(len(mb_inds), -1)]
                sigreg_valids = [torch.ones(len(mb_inds), device=device, dtype=torch.bool)]
                action_history = []
                ...
                dyn_value_logits = agent.get_dyn_value_logits_from_summary(summary_tokens)
                dyn_value_target_probs = hl_support.project(b_returns[mb_inds])
                dyn_value_log_probs = torch.log_softmax(dyn_value_logits, dim=-1)
                dyn_value_loss = -(dyn_value_target_probs * dyn_value_log_probs).sum(dim=-1).mean()
                for horizon_idx in range(args.dyn_horizon):
                    ...
                    target_next_latents = target_next_summary[:, :NUM_DYN_LATENT_TOKENS]
                    sigreg_latents.append(target_next_latents.reshape(len(mb_inds), -1))

                    step_weight = dyn_alive * in_rollout
                    latent_weight = step_weight * future_valids
                    sigreg_valids.append(latent_weight > 0.0)
                    pred_next_latents = pred_next_summary[:, :NUM_DYN_LATENT_TOKENS]
                    dyn_latent_loss = dyn_latent_loss + (
                        F.mse_loss(pred_next_latents, target_next_latents, reduction="none").mean(dim=(-1, -2))
                        * latent_weight
                    ).mean()
                    ...
                    with torch.no_grad():
                        ...
                        reward_mse = ((reward_pred - future_rewards).square() * step_weight).sum() / step_weight.sum().clamp_min(1.0)
                        termination_pred = (torch.sigmoid(pred_termination_logits) >= 0.5).float()
                        termination_acc = (
                            (termination_pred == future_terminations).float() * step_weight
                    ...
                    summary_history.append(target_next_summary)
                ...
                optimizer.zero_grad()
                (args.dyn_coef * dyn_loss + args.sigreg_coef * dyn_sigreg_loss).backward()
        ...
        if dream_batch is not None:
            ...
            for epoch in range(args.imagine_update_epochs):
                ...
                for start in range(0, dream_states.shape[0], dream_minibatch_size):
                    ...
                    _, _, dream_newlogprob, dream_entropy, _ = agent.get_imagined_action_and_value(
                    ...
                    with torch.no_grad():
                        if has_dream_targets:
                            ...
                            with torch.no_grad():
                                dream_dist = agent.get_imagined_action_dist(dream_states[mb_inds][mb_dream_learn_mask])
                    ...
                    if has_dream_targets:
                        ...
                        imagine_actor_loss = imagine_actor_loss - args.imagine_actor_ent_coef * dream_entropy[
                            mb_dream_learn_mask
                        ].mean()
                    else:
                        imagine_actor_loss = imagine_policy_loss.sum() * 0.0

                    dream_newvalue_logits = agent.get_imagined_value_logits(dream_states[mb_inds])
                    dream_return_target_probs = hl_support.project(dream_returns[mb_inds])
                    dream_value_log_probs = torch.log_softmax(dream_newvalue_logits, dim=-1)
                    imagine_value_loss = -(dream_return_target_probs * dream_value_log_probs).sum(dim=-1)
                    if args.clip_vloss:
                        dream_newvalue = hl_support.to_scalar(dream_newvalue_logits)
                        dream_oldvalue = dream_values[mb_inds]
                        dream_clipped_value = dream_oldvalue + (dream_newvalue - dream_oldvalue).clamp(
                            -args.clip_coef, args.clip_coef
                        )
                        dream_clipped_log_probs = torch.log(hl_support.project(dream_clipped_value).clamp_min(1e-8))
                        imagine_clipped_value_loss = -(
                            dream_return_target_probs * dream_clipped_log_probs
                        ).sum(dim=-1)
                        imagine_value_loss = torch.maximum(imagine_value_loss, imagine_clipped_value_loss)
                    if has_dream_targets:
            ...
            with torch.no_grad():
                ...
                valid_values = dream_values[dream_learn_masks]
                if valid_returns.numel() > 1:
                    var_returns = torch.var(valid_returns, unbiased=False)
                    if var_returns == 0:
                        imagine_explained_var = np.nan
                    else:
                        imagine_explained_var = (
        ...
        writer.add_scalar("losses/dyn_value_loss", dyn_value_loss_mean, global_step)
        writer.add_scalar("losses/dyn_sigreg_loss", dyn_sigreg_loss_mean, global_step)
        writer.add_scalar("lejepa/loss", dyn_latent_loss_mean + args.sigreg_coef * dyn_sigreg_loss_mean, global_step)
        ...
        writer.add_scalar("dynamics/value_loss", dyn_value_loss_mean, global_step)
        ...
        for key, value in dream_diagnostics.items():
            writer.add_scalar(f"imagination/{key}", value, global_step)
