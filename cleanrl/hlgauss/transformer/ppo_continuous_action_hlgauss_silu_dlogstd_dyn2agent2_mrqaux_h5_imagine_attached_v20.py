...
# - a second 2-layer agent transformer over the actor/critic/SDE CLS tokens produced by the dynamics stack
...
# - attached variant: PPO and dreamed agent losses are allowed to shape the world-model summary tokens
import os
import random
import sys
import time
...
DYN_NUM_LAYERS = 2
AGENT_NUM_LAYERS = 2
NUM_DYN_SPECIAL_TOKENS = 4
NUM_AGENT_SPECIAL_TOKENS = 3
SCALAR_EMBED_DIM = 32
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True):
        ...
        self.actor_cls = nn.Parameter(summary_cls[1:2].unsqueeze(0))
        self.critic_cls = nn.Parameter(summary_cls[2:3].unsqueeze(0))
        self.sde_cls = nn.Parameter(summary_cls[3:4].unsqueeze(0))
    ...
    def _encode_agent_tokens(self, agent_summary_tokens):
        ...
        return agent_tokens[:, 0], agent_tokens[:, 1], agent_tokens[:, 2]
    ...
    def _encode_agent_features(self, x):
        ...
        actor_features, critic_features, sde_features = self._encode_agent_tokens(agent_summary_tokens)
    ...
    def get_imagined_action_dist(self, summary_tokens):
        ...
        if self.detach_world_model_from_agent:
            agent_summary_tokens = agent_summary_tokens.detach()
        actor_features, _, sde_features = self._encode_agent_tokens(agent_summary_tokens)
    ...
    def get_imagined_value_logits(self, summary_tokens):
        ...
        if self.detach_world_model_from_agent:
            agent_summary_tokens = agent_summary_tokens.detach()
        _, critic_features, _ = self._encode_agent_tokens(agent_summary_tokens)
