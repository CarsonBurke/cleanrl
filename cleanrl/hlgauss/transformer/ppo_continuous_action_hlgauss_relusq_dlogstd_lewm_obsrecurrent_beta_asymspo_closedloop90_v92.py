...
# - v92: keeps v89's outcome semantics but weights world-model losses 90% toward
#   closed-loop autoregressive predictions to target dream-distribution error
import os
import random
import sys
import time
...
class Args:
    ...
    closed_loop_loss_weight: float = 0.9
    """fraction of dynamics/reward/termination loss assigned to closed-loop predictions"""
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.wm_update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                closed_loop_weight = args.closed_loop_loss_weight
                teacher_forced_weight = 1.0 - closed_loop_weight
                dyn_latent_loss = (
                    teacher_forced_weight * teacher_forced_latent_loss
                    + closed_loop_weight * closed_loop_latent_loss
                )
                dyn_reward_loss = (
                    teacher_forced_weight * teacher_forced_reward_loss
                    + closed_loop_weight * closed_loop_reward_loss
                )
                dyn_termination_loss = (
                    teacher_forced_weight * teacher_forced_termination_loss
                    + closed_loop_weight * closed_loop_termination_loss
                )
        ...
        writer.add_scalar("lejepa/closed_loop_loss_weight", args.closed_loop_loss_weight, global_step)
