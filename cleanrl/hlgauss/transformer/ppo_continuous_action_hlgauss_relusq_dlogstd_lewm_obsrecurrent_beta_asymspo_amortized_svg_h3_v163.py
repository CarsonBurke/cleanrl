...
class Args:
    ...
    # v163 amortized SVG / Dreamer-continuous actor
    svg_horizon: int = 3
    """short analytic-gradient horizon imagined through the frozen WM each actor update;
       the full terminal value V(s_H) carries the tail beyond H. Kept short to avoid
       exploding gradients through chaotic dynamics."""
    svg_ent_coef: float = 1e-3
    """entropy bonus on the imagined Beta actor rollout (maximized alongside the return)."""
...
class Agent(nn.Module):
    ...
    def imagined_actor_objective(
        ...
        """v163 SVG / Dreamer-continuous amortized actor objective.
        ...
        differentiable, continuation-gated discounted return (plus a full terminal
        value V(s_H)) back into the actor parameters through the trajectory
        latents. Mirrors `rollout_return_components`' WM stepping exactly.
        ...
        Returns (objective, entropy_sum) where
            objective = discounted_reward_sum + gamma^H * continue * V(s_H),
        both differentiable w.r.t. actor params via the WM.
        """
        batch = start_summary.shape[0]
        summary_history = [start_summary]
        action_history = []
        discounted_reward_sum = start_summary.new_zeros(batch)
        entropy_sum = start_summary.new_zeros(batch)
        continue_carry = start_summary.new_ones(batch)
        gamma_t = 1.0
        ...
        # Full terminal value bootstrap (coef 1.0): V(s_H) carries the tail.
        critic_input = self._critic_features_from_latents(last_summary)
        value_hat = hl_support.to_scalar(self.critic(critic_input))
        objective = discounted_reward_sum + gamma_t * continue_carry * value_hat
        return objective, entropy_sum
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if not world_model_only:
            ...
            # v163 amortized SVG actor update: imagine short rollouts through the
            # FROZEN world model from real start summaries, backprop the analytic
            # value gradient into the actor only. WM/critic are frozen via the
            # separate optimizer split plus the zero/step/zero recipe below.
            for epoch in range(args.agent_update_epochs):
                np.random.shuffle(b_inds)
                for start in range(0, args.batch_size, args.minibatch_size):
                    end = start + args.minibatch_size
                    ...
                    actor_loss = -(objective + args.svg_ent_coef * entropy).mean()

                    # Strict gradient isolation: only actor_optimizer steps the
                    # actor; WM/critic grads produced by this backward are discarded.
