...
# - the agent reads the 2-token world-model summary directly through shallow per-head projections
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins, detach_world_model_from_agent=True):
        ...
        self.agent_summary_norm = RMSNorm(MODEL_DIM * NUM_DYN_SPECIAL_TOKENS)
        self.actor_proj = xavier_init_linear(nn.Linear(MODEL_DIM * NUM_DYN_SPECIAL_TOKENS, MODEL_DIM))
        self.critic_proj = xavier_init_linear(nn.Linear(MODEL_DIM * NUM_DYN_SPECIAL_TOKENS, MODEL_DIM))
        self.sde_proj = xavier_init_linear(nn.Linear(MODEL_DIM * NUM_DYN_SPECIAL_TOKENS, MODEL_DIM))
    ...
    def _encode_agent_features_from_summary(self, summary_tokens):
        flat_summary = summary_tokens.reshape(summary_tokens.shape[0], -1)
        flat_summary = self.agent_summary_norm(flat_summary)
        actor_features = F.silu(self.actor_proj(flat_summary))
        critic_features = F.silu(self.critic_proj(flat_summary))
        sde_features = F.silu(self.sde_proj(flat_summary))
        return actor_features, critic_features, sde_features
    ...
    def _encode_agent_features(self, x):
        ...
        actor_features, critic_features, sde_features = self._encode_agent_features_from_summary(summary_tokens)
    ...
    def get_imagined_action_dist(self, summary_tokens):
        if self.detach_world_model_from_agent:
            summary_tokens = summary_tokens.detach()
        actor_features, _, sde_features = self._encode_agent_features_from_summary(summary_tokens)
    ...
    def get_imagined_value_logits(self, summary_tokens):
        if self.detach_world_model_from_agent:
            summary_tokens = summary_tokens.detach()
        _, critic_features, _ = self._encode_agent_features_from_summary(summary_tokens)
