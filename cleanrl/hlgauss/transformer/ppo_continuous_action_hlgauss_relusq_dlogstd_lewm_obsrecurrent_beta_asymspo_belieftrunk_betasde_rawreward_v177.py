# v177: RAW REWARDS (no reward normalization). Diagnosis from v176: returns/real_target
# plateaued at ~2.6 because the NormalizeReward wrapper's running return-std grows as the
# policy improves, so the NORMALIZED critic target saturates even while raw episodic_return
# keeps climbing -- the normalization was masking the learning signal. The HL-Gauss critic
# is distributional + symlog, so its (widened) binned support can represent raw discounted
# returns directly; reward normalization is redundant (this is the Dreamer-v3 stance). v177
# removes BOTH NormalizeReward and the [-10,10] reward clip, and widens the symlog support
# (value v_min/v_max +-5 -> +-8 ; reward +-3 -> +-5) so raw per-step rewards (~15-20) and
# raw discounted returns (~hundreds) sit inside the support with margin. PPO advantage
# z-normalization (per-minibatch) still handles the policy-gradient scale, so only the
# critic-target / reward-model scale changes. Observation normalization is KEPT. Beta
# policy and everything else identical to v176. Single-axis change vs v176: reward scaling.
#
...
class Args:
    ...
    v_min: float = -8.0
    """minimum value of the support (in symlog space). v177: widened from -5 to -8 to
    cover RAW (un-normalized) discounted returns -- symlog(-8) ~= -2980 in raw units."""
    v_max: float = 8.0
    """maximum value of the support (in symlog space). v177: widened from 5 to 8 to
    cover RAW discounted returns (symlog(8) ~= 2980 raw); strong HalfCheetah values
    (episodic ~9k -> discounted V ~900, symlog ~6.8) sit comfortably inside."""
    ...
    """minimum symlog reward support for the learned reward outcome token. v177: widened
    from -3 to -5 for RAW per-step rewards (symlog(-5) ~= -147 raw)."""
    reward_v_max: float = 5.0
    """maximum symlog reward support for the learned reward outcome token. v177: widened
    from 3 to 5 -- raw HalfCheetah per-step reward can reach ~15-20 (symlog ~3), so the
    old +3 support (symlog(3) ~= 19 raw) was right at the clip edge; +5 -> ~147 raw."""
...
def make_env(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        ...
        # v177: NO reward normalization/clip. The symlog HL-Gauss critic (widened
        # support, v_min/v_max = +-8) regresses RAW discounted returns directly, so the
        # NormalizeReward running-std (which was saturating returns/real_target as the
        # policy improved) and the [-10,10] clip (which would clip raw HalfCheetah
        # rewards) are both removed. Observation normalization is kept.
        return env
