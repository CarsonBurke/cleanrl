...
# - reward and continuation in GoalSolver are decoded from WM(z_t, a_seed), not
#   from free g, so the solver cannot directly edit reward tokens.
...
class Args:
    ...
    """coefficient on summed squared reachability defect tying free g to WM-predicted next obs-latent"""
...
if __name__ == "__main__":
    ...
    def run_goal_embedding_solver(start_summary, init_actions):
        ...
        GoalSolver optimizes a free next obs-token embedding with a WM transition
        defect. ActionSolver then minimizes only distance from WM(z, a) to the
        solved target; return is not used in the action objective.
        """
        ...
        try:
            ...
            seed_action_u = torch.atanh(init_norm).requires_grad_(True)

            def bounded_seed_action():
                return action_mid + action_scale * torch.tanh(seed_action_u)
            ...
            with torch.no_grad():
                ...
                init_goal_obs = init_pred[:, :NUM_OBS_TOKENS].detach().clone()
            goal_obs = init_goal_obs.requires_grad_(True)
            goal_opt = optim.Adam([goal_obs, seed_action_u], lr=args.goal_solver_lr)

            def goal_energy():
                pred_next = predict_one(flat_start_summary, bounded_seed_action())
                pred_obs = pred_next[:, :NUM_OBS_TOKENS]
                outcome_tokens = pred_next[:, NUM_OBS_TOKENS:]
                ...
                defect = (goal_obs - pred_obs).square().sum(dim=(-1, -2))
                defect_mean = defect / float(NUM_OBS_TOKENS * MODEL_DIM)
                model_return = reward_hat + args.gamma * continue_hat * goal_value
            ...
            with torch.no_grad():
                ...
                final_seed_action = bounded_seed_action().detach()
                final_goal_obs = goal_obs.detach()
                ...
                final_seed_action = final_seed_action.reshape(batch_size, num_samples, 1, -1)
