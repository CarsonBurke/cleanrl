# v183: PER-TOKEN-POSITION SIGReg. One change over v182 (joint-KL trust region KEPT, run on
# the winning mb8/s500 config). The SIGReg anti-collapse term previously POOLED all
# NUM_SIGREG_TOKENS token positions -- 17 obs-dim tokens + reward + continuation -- into one
# cloud and fit a SINGLE shared N(0,I). That fits a Gaussian over the obs+outcome MIXTURE, so
# the 2 heterogeneous outcome tokens are regularized against the obs-dominated reference
# distribution rather than their own marginal. v183 keeps the token-position axis SEPARATE:
# each position gets its own N(0,I) target, with the empirical CF reduced over the valid
# (B,H) axis only (this is exactly le-wm's reduction, stratified over the token axis instead
# of time). The constant continuation token is ISOLATED in its own pool, NOT excluded: on
# terminating envs (Hopper/Walker) that preserves real anti-collapse on the {0,1} signal,
# while on HalfCheetah (continuation==1) it is vacuous-but-harmless and no longer pollutes
# the obs cloud with a spurious delta spike. Hypothesis: cleaner per-type latent geometry
# (esp. a properly Gaussianized reward token the actor/critic read) gives a small WM-side
# gain. NOTE: this is principled-but-modest -- the binding constraint is policy-side, not WM.
# Only masked_token_sigreg changed; gradient magnitude per token is O(1) in pool size either
# way (the *N scaling cancels the 1/N mean), so this is a wrong-target fix, not a dilution fix.
#
...
if __name__ == "__main__":
    ...
    def masked_token_sigreg(token_latents, token_valids):
        # v183: PER-TOKEN-POSITION SIGReg. Instead of pooling all NUM_SIGREG_TOKENS token
        # positions (obs dims + reward + continuation) into one cloud and fitting a single
        # shared N(0,I), we keep the token-position (T) axis SEPARATE: each position gets its
        # own N(0,I) target, with the empirical CF reduced over the valid (B,H) axis only.
        # This is the le-wm reduction stratified over the token axis (le-wm stratifies over
        # time; we stratify over token position). Rationale: obs-dim tokens and the
        # reward/continuation outcome tokens are heterogeneous marginals -- pooling fits one
        # Gaussian over the *mixture*, regularizing the outcome tokens against the wrong
        # (obs-dominated) reference. Per-position Gaussianizes each against its own marginal
        # (Cramer-Wold per type). The constant continuation token is ISOLATED in its own
        # position pool (not excluded): on terminating envs it keeps genuine anti-collapse on
        # the {0,1} signal; on HalfCheetah (continuation==1) it is vacuous but harmless and
        # no longer injects a spurious spike into the obs cloud.
        ...
        valid_tokens = token_latents[token_valids]  # [M, T, D] -- M valid (B,H) positions
        valid_count = valid_tokens.shape[0]  # per-position sample count
        num_tok = valid_tokens.shape[1]
        ...
        cos_sum = valid_tokens.new_zeros(num_tok, A.shape[1], t.numel())  # [T, P, K]
        sin_sum = valid_tokens.new_zeros(num_tok, A.shape[1], t.numel())

        def chunk_trig(chunk_tokens):
            # chunk_tokens: [m, T, D] -> [m, T, P] -> [m, T, P, K]; reduce over m only.
            x_t = (chunk_tokens @ A).unsqueeze(-1) * t
            return x_t.cos().sum(dim=0), x_t.sin().sum(dim=0)

        # Chunk over the M axis but keep T separate; size the chunk so the per-chunk trig
        # tensor (m * T * P * K) matches the pooled implementation's (SIGREG_CHUNK_SIZE * P * K).
        m_chunk = max(1, SIGREG_CHUNK_SIZE // num_tok)
        for token_chunk in valid_tokens.split(m_chunk):
        ...
        cos_mean = cos_sum / valid_count  # [T, P, K]
        sin_mean = sin_sum / valid_count
        err = (cos_mean - phi).square() + sin_mean.square()  # [T, P, K]
        statistic = (err @ weights) * valid_count  # [T, P]
        return statistic.mean()  # mean over token positions and projections
