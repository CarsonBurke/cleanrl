# SiLU + HL-Gauss + Direct Log-Std + DG(value-loss surprisal, std-normalized)
#
# Combines:
#   - SiLU activations (no RMSNorm) in hidden layers
#   - HL-Gauss categorical value head (no value clipping)
#   - State-dependent direct log-std (clamped [-5, 2])
#   - DG with value-loss surprisal, normalized by std (no mean shift, no eta)
...
class Args:
    ...
    dg_eta: float = 1.0
    """temperature for the delight gate (unused in std-norm variant, kept for compatibility)"""
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        # Shared actor trunk with SiLU hidden layers
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Compute per-sample CE value loss BEFORE policy loss
                ...
                # DG gate: std-normalized (no mean shift, no eta)
                with torch.no_grad():
                    vl_surprisal = v_loss_per_sample.detach()
                    delight = mb_advantages * vl_surprisal  # raw advantages, before normalization
                    dg_weight = torch.sigmoid(delight / (delight.std() + 1e-8))

                # Normalize advantages AFTER computing DG gate
                if args.norm_adv:
