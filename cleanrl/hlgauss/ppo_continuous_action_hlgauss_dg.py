# HL-Gauss + Delightful Policy Gradient for PPO
# HL-Gauss: Farebrother et al., 2024 (https://arxiv.org/abs/2403.03950)
# DG: https://arxiv.org/abs/2603.14608
#
# Key idea: Replace the scalar MSE critic with a categorical distribution over
# discretized value bins. Targets are constructed by projecting the scalar return
# onto the bin support using a Gaussian kernel (HL-Gauss), then training with
# cross-entropy loss. This yields better representation learning and more stable
# value estimates compared to standard MSE regression.
#
# Changes from ppo_continuous_action.py:
#   - Critic head outputs `num_bins` logits instead of 1 scalar
#   - Value extraction via E[z] = sum(softmax(logits) * bin_centers)
#   - Value loss: cross-entropy with HL-Gauss projected targets
#   - Value clipping applied to expected values, then re-projected
...
class Args:
    ...
    dg_eta: float = 1.0
    """temperature for the delight gate"""
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Delight gate: uses RAW advantages (before normalization)
                surprisal = torch.clamp(-b_logprobs[mb_inds], -10.0, 10.0)
                ...
                # Policy loss with delight gating
