# HOPSD v11.1 — v11 with a PURE-DISTILLATION student critic (full policy-path symmetry).
# =====================================================================================
# Single change vs v11: the student critic's horizon-0 head learns ONLY from the
# teacher's distribution (lambda-return CE removed for horizon 0; MTP horizons 1..5
# keep lambda CE as the auxiliary representation task; distillation now covers ALL
# states, not just MC-valid ones, since rollout cuts are arbitrary w.r.t. state).
# This makes the value path bootstrap-free end to end: teacher <- windowed MC ground
# truth, student <- teacher — exactly the actor's dependency shape.
# The experiment's real question (Rao-Blackwell caveat): with teacher EV ~0.95, the
# hindsight conditional mean tracks G_MC per sample, so this approaches smoothed
# CE-to-MC-returns — i.e. v11.1 tests whether the lambda-targets' BOOTSTRAP BIAS costs
# more than MC VARIANCE. Prediction: noisier critic/GAE early (v11 ahead early),
# converging equal-or-better late; if the student EV stays clearly below v11's
# throughout, the lambda anchor earns its place and v11's hybrid stands.
# =====================================================================================
#
# --- v11 method (retained below) ---
# Base = pure no-leash champion config (v2, awr_temp 0.5, target_kl inert at 1e6);
# v10_noleash is exactly this file minus the critic mechanism (clean control).
#
# v8.1 failed structurally, not conceptually: the teacher critic's targets were
# lambda-returns built from the STUDENT critic's bootstrap, then V_T was distilled back
# into V_S — a closed loop with no ground-truth anchor; both EVs sagged (0.83-0.86 vs
# 0.96-0.97 control). The policy path never has this problem: policy teacher <- realized
# ACTIONS (ground truth), student <- teacher. v11 restores the same dependency shape for
# values:
#   TEACHER CRITIC <- windowed MONTE-CARLO returns (ground truth): within a 2048-step
#   rollout, sum gamma^k r_{t+k} until episode cut/rollout edge; the target is valid
#   where the unobserved tail mass gamma^K <= mc_tail_max (0.02, K >= ~387 steps) or the
#   episode truly terminated. No critic bootstraps anything here. Given the privileged
#   future context, V_T(s, phi) explains realized luck -> sharp AND accurate.
#   STUDENT CRITIC <- primary CE to lambda-return targets (unchanged anchor) + auxiliary
#   CE (coef 0.3) toward softmax(V_T logits) on MC-valid states, teacher detached.
#   Per-sample CE toward the sharp hindsight distribution averages over futures to the
#   correct wide marginal p(G|s): a Rao-Blackwellized MC signal — unbiased like MC,
#   variance-reduced by hindsight; complementary to the biased/low-var lambda targets.
# Predictions: student EV rises faster than v10_noleash at matched steps with returns
# >= v10; teacher_ev high on MC-valid region. Risks: MC-valid fraction (~60-80%) skews
# toward early-episode states (log debug/mc_valid_frac); if returns lag v10, the distill
# coef is the rollback knob. Next symmetry step if this validates: AWR-weight the
# teacher critic CE (improvement-consistent value — the critic analog of the tilt).
# =====================================================================================
#
...
class Args:
    ...
    critic_distill_coef: float = 1.0 # v11.1: horizon-0 is distill-ONLY, full weight
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        distill_kls, distill_clipfracs, teacher_nlls, critic_distills = [], [], [], []
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v11.1: horizon 0 learns ONLY from the teacher (full policy-path
                # symmetry); lambda-return CE remains on MTP horizons 1..5 as the
                # auxiliary representation task.
                value_mask = value_mask.clone()
                value_mask[:, 0] = 0.0
                v_loss = (value_ce * value_mask).sum(dim=-1).mean()

                # v11.1: horizon-0 distillation on ALL states (rollout-edge/truncation
                # cuts are arbitrary w.r.t. state, so the MC-trained teacher generalizes
                # there; leaving 20% of states with no horizon-0 signal would be worse).
                with torch.no_grad():
                    t_cd_probs = torch.softmax(
                        teacher.critic_logits(b_teacher_critic_in[mb_inds]), dim=-1
                    )
                cd_ce = -(t_cd_probs * value_log_probs[:, 0]).sum(-1)
                cd_loss = cd_ce.mean()
                v_loss = v_loss + args.critic_distill_coef * cd_loss
                critic_distills.append(cd_loss.item())
        ...
        writer.add_scalar("losses/critic_distill", np.mean(critic_distills), global_step)
