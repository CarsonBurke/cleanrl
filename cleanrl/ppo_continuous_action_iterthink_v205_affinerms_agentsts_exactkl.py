# PPO + IterThink v205 affine-RMS agent-owned S-T-S policy memory.
#
# Preserve v198's raw affine-RMS spatial encoder, attached LeJEPA/SIGReg,
# action-free WM temporal trunk, outbound-action transition query, exact Beta KL,
# and strict WM/agent detach.  The policy and critic now each own a one-way S-T-S
# stream over the last five detached v198 belief memories: a per-frame agent read,
# causal temporal mixing with shifted incoming actions, then a current-frame
# reread.  WM/state tokens never attend to agent tokens.  Zero residual gates make
# both paths exactly v198 at initialization while allowing PPO gradients to turn
# on history, action, and final-reread deltas.  Imagination is disabled because
# constructing episode-safe dreamed incoming-action histories would otherwise
# change the intended real-policy experiment.
#
# -----------------------------------------------------------------------------
...
class Args:
    ...
    imagine_enable: bool = False      # v205: real agent-history alignment only; no malformed dream histories
...
class AgentTemporalSpecialBlock(nn.Module):
    """Causal time mixing for agent tokens; state tokens are never modified."""
    ...
    def forward(self, tokens, valid):
        # A permanent zero/null prefix prevents all-masked attention rows in the
        # left padding. Invalid history rows are zeroed after every block, so they
        # can neither write useful content nor become keys in a later block.
        batch, time_len, width = tokens.shape
        null = tokens.new_zeros(batch, 1, width)
        y = torch.cat([null, tokens], dim=1)
        null_valid = torch.ones(batch, 1, dtype=torch.bool, device=tokens.device)
        key_valid = torch.cat([null_valid, valid], dim=1)
        seq = time_len + 1
        causal = torch.ones(seq, seq, dtype=torch.bool, device=tokens.device).triu(1)
        h = self.attn_norm(y)
        attn, _ = self.attn(
            h, h, h,
            attn_mask=causal,
        ...
        y = y + attn
        y = y + self.w2(relu_sq(self.w1(self.ffn_norm(y))))
        y = y[:, 1:]
        return y * valid.unsqueeze(-1).to(y.dtype)


class AgentSTSReadout(nn.Module):
    """One-way space-time-space readout over detached v198 beliefs."""

    def __init__(self, num_tokens, dim, act_dim, context, num_heads, ffn_mult, branch_seed):
        super().__init__()
        self.num_tokens = num_tokens
        self.dim = dim
        self.act_dim = act_dim
        self.context = context
        # This is structurally the exact v198 readout and supplies the parity path.
        self.frame_read = AgentQueryReadout(num_tokens, dim, num_heads, ffn_mult)
        # Enhancement initialization must not perturb the v198 RNG lineage of the
        # parity readouts/trunks/heads. Actor and critic use different fork seeds.
        with torch.random.fork_rng():
            torch.manual_seed(branch_seed)
            self.incoming_action = xavier_linear(nn.Linear(act_dim, dim, bias=False))
            self.lag_embedding = nn.Parameter(torch.empty(context, dim))
            nn.init.xavier_uniform_(self.lag_embedding)
            self.temporal_blocks = nn.ModuleList(
                [AgentTemporalSpecialBlock(dim, num_heads, ffn_mult) for _ in range(2)]
            )
            self.temporal_gate = nn.Parameter(torch.zeros(dim))

            # Final S: the temporally mixed agent token rereads current state memory.
            # It is a query-only path: no output is ever written into WM memory.
            self.reread_query_norm = nn.RMSNorm(dim)
            self.reread_memory_norm = nn.RMSNorm(dim)
            self.reread_attn = nn.MultiheadAttention(
                dim, num_heads, batch_first=True, bias=False
            )
            for name, param in self.reread_attn.named_parameters():
                if "weight" in name:
                    nn.init.xavier_uniform_(param)
            self.reread_ffn_norm = nn.RMSNorm(dim)
            self.reread_w1 = xavier_linear(nn.Linear(dim, dim * ffn_mult, bias=False))
            self.reread_w2 = xavier_linear(nn.Linear(dim * ffn_mult, dim, bias=False))
            self.reread_gate = nn.Parameter(torch.zeros(dim))

    def forward(self, memories, incoming_actions, valid):
        # memories (B,W,S,D), incoming_actions (B,W,A), valid (B,W).
        # The last row is always the current state. Incoming action at row t is
        # a_{t-1}; it is zero at an episode start.
        batch, window, num_tokens, width = memories.shape
        if (window, num_tokens, width) != (self.context, self.num_tokens, self.dim):
            raise ValueError("malformed fixed agent S-T-S history")
        frame = self.frame_read(memories.reshape(batch * window, num_tokens * width))
        frame = frame.reshape(batch, window, width)
        current_base = frame[:, -1]
        temporal = frame + self.incoming_action(incoming_actions)
        temporal = temporal + self.lag_embedding.to(temporal.dtype).unsqueeze(0)
        temporal = temporal * valid.unsqueeze(-1).to(temporal.dtype)
        for block in self.temporal_blocks:
            temporal = block(temporal, valid)
        temporal_current = temporal[:, -1]
        mixed = current_base + self.temporal_gate.to(current_base.dtype) * (
            temporal_current - current_base
        )

        current_memory = memories[:, -1]
        q = mixed.unsqueeze(1)
        reread_attn, _ = self.reread_attn(
            self.reread_query_norm(q),
            self.reread_memory_norm(current_memory),
            self.reread_memory_norm(current_memory),
            need_weights=False,
        )
        reread = q + reread_attn
        reread = reread + self.reread_w2(
            relu_sq(self.reread_w1(self.reread_ffn_norm(reread)))
        )
        reread = reread[:, 0]
        return mixed + self.reread_gate.to(mixed.dtype) * (reread - mixed)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if not self.detach_world_model_from_agent:
            raise ValueError("v205 requires a strictly detached world-model/agent interface")
        self.share_backbone = args.share_backbone
        if self.share_backbone:
            raise ValueError("v205 requires separate actor/critic S-T-S streams and trunks")
        self.explore_code_dim = args.explore_code_dim
        self.explore_logit_scale = args.explore_logit_scale
        self.agent_context = args.lewm_context
        self.act_dim = act_dim
        # Fixed packed PPO input: W detached v198 belief-token memories, W shifted
        # incoming actions, and W validity bits. Keeping the whole policy-owned
        # history in the buffer makes every PPO replay deterministic and avoids
        # reconstructing episode boundaries during update epochs.
        self.agent_latent_dim = obs_dim * args.lewm_dim
        self.agent_memory_input_dim = self.agent_context * self.agent_latent_dim
        self.agent_action_input_dim = self.agent_context * act_dim
        self.agent_input_dim = (
            self.agent_memory_input_dim + self.agent_action_input_dim + self.agent_context
        )
        self.actor_readout = AgentSTSReadout(
            obs_dim, args.lewm_dim, act_dim, self.agent_context,
            args.lewm_heads, args.lewm_ffn_mult, args.seed + 51000,
        )
        self.critic_readout = AgentSTSReadout(
            obs_dim, args.lewm_dim, act_dim, self.agent_context,
            args.lewm_heads, args.lewm_ffn_mult, args.seed + 52000,
        )
    ...
    def pack_agent_input(self, memories, incoming_actions, valid):
        if self.detach_world_model_from_agent:
            memories = memories.detach()
        batch = memories.shape[0]
        return torch.cat(
            [
                memories.reshape(batch, self.agent_memory_input_dim),
                incoming_actions.detach().reshape(batch, self.agent_action_input_dim),
                valid.to(memories.dtype).reshape(batch, self.agent_context),
            ],
            dim=-1,
        )

    def unpack_agent_input(self, agent_input):
        batch = agent_input.shape[0]
        memory_end = self.agent_memory_input_dim
        action_end = memory_end + self.agent_action_input_dim
        memories = agent_input[:, :memory_end].reshape(
            batch, self.agent_context, self.world_model.num_obs_tokens, self.world_model.dim
        )
        incoming_actions = agent_input[:, memory_end:action_end].reshape(
            batch, self.agent_context, self.act_dim
        )
        valid = agent_input[:, action_end:].reshape(batch, self.agent_context) > 0.5
        return memories, incoming_actions, valid

    def agent_input_from_latent(self, latent):
        # Compatibility for observation-only callers. The current belief occupies
        # the final row; earlier rows and incoming a_{t-1} are invalid sentinels.
        if self.detach_world_model_from_agent:
            latent = latent.detach()
        batch = latent.shape[0]
        memories = latent.new_zeros(
            batch, self.agent_context, self.world_model.num_obs_tokens, self.world_model.dim
        )
        memories[:, -1] = latent
        incoming_actions = latent.new_zeros(batch, self.agent_context, self.act_dim)
        valid = torch.zeros(batch, self.agent_context, dtype=torch.bool, device=latent.device)
        valid[:, -1] = True
        return self.pack_agent_input(memories, incoming_actions, valid)
    ...
    def _actor_feat_from_agent_input(self, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        memories, incoming_actions, valid = self.unpack_agent_input(agent_input)
        return self.actor_trunk(self.actor_readout(memories, incoming_actions, valid))

    def _critic_feat_from_agent_input(self, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        memories, incoming_actions, valid = self.unpack_agent_input(agent_input)
        return self.critic_trunk(self.critic_readout(memories, incoming_actions, valid))
...
if __name__ == "__main__":
    ...
    assert args.lewm_context == 5, "v205 fixes the policy S-T-S window at five belief frames"
    ...
    assert args.detach_world_model_from_agent, "v205 requires PPO to be detached from the WM"
    ...
    assert not args.imagine_enable, (
        "v205 disables imagined optimizer updates: dream S-T-S action/history alignment is intentionally absent"
    )
    ...
    rollout_policy_memory_history = []
    rollout_policy_action_history = []
    rollout_policy_history_valid = []
    rollout_previous_action = torch.zeros(
        args.num_envs, agent.act_dim, device=device
    )
    rollout_previous_action_valid = torch.zeros(
    ...
    def pack_policy_history(current_belief, current_incoming_action, current_valid):
        # Left-pad a fixed W-frame policy stream. Each action is shifted: the
        # action beside belief/state s_t is a_{t-1}. Per-env validity is retained
        # for old rows so vector-env episode resets cannot leak across boundaries.
        W = agent.agent_context
        n_past = min(W - 1, len(rollout_policy_memory_history))
        past_mem = rollout_policy_memory_history[-n_past:] if n_past else []
        past_act = rollout_policy_action_history[-n_past:] if n_past else []
        past_valid = rollout_policy_history_valid[-n_past:] if n_past else []
        pad = W - 1 - n_past
        memory_rows = [torch.zeros_like(current_belief) for _ in range(pad)] + past_mem + [current_belief]
        action_rows = [torch.zeros_like(current_incoming_action) for _ in range(pad)] + past_act + [current_incoming_action]
        valid_rows = [torch.zeros_like(current_valid) for _ in range(pad)] + past_valid + [current_valid]
        memories = torch.stack(
            [m * v.view(-1, 1, 1).to(m.dtype) for m, v in zip(memory_rows, valid_rows)], dim=1
        )
        incoming_actions = torch.stack(
            [a * v.view(-1, 1).to(a.dtype) for a, v in zip(action_rows, valid_rows)], dim=1
        )
        valid = torch.stack(valid_rows, dim=1)
        return agent.pack_agent_input(memories, incoming_actions, valid)

    def build_belief_agent_input(
        obs_tensor, current_incoming_action=None, current_incoming_valid=None
    ):
        ...
        belief = agent.world_model.belief_from_history(belief_latents)
        if current_incoming_action is None:
            current_incoming_action = rollout_previous_action
        if current_incoming_valid is None:
            current_incoming_valid = rollout_previous_action_valid
        # State validity and incoming-action availability are different. The
        # current state always exists; an episode-start action is a zero sentinel.
        current_incoming_action = current_incoming_action * current_incoming_valid.view(-1, 1).to(
            current_incoming_action.dtype
        )
        current_state_valid = torch.ones_like(current_incoming_valid)
        return (
            pack_policy_history(belief, current_incoming_action, current_state_valid),
            current_latent,
            belief,
            current_incoming_action,
            current_state_valid,
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                (
                    agent_input,
                    current_latent,
                    current_policy_belief,
                    current_policy_incoming_action,
                    current_policy_valid,
                ) = build_belief_agent_input(next_obs)
            ...
            rollout_policy_memory_history.append(current_policy_belief.detach())
            rollout_policy_action_history.append(current_policy_incoming_action.detach())
            rollout_policy_history_valid.append(current_policy_valid.detach())
            if len(rollout_latent_history) > args.lewm_context - 1:
                ...
                rollout_policy_memory_history = rollout_policy_memory_history[-(args.lewm_context - 1) :]
                rollout_policy_action_history = rollout_policy_action_history[-(args.lewm_context - 1) :]
                rollout_policy_history_valid = rollout_policy_history_valid[-(args.lewm_context - 1) :]
            ...
            with torch.no_grad():
                (
                    ...
                    current_incoming_valid=torch.ones_like(boundary_tensor),
                )
                next_transition_agent_inputs[step] = next_transition_agent_input
            rollout_previous_action = action.detach().clone()
            rollout_previous_action_valid = torch.ones_like(boundary_tensor)
            if bool(boundary_tensor.any()):
                for hist_idx in range(len(rollout_history_valid)):
                    ...
                    rollout_policy_history_valid[hist_idx] = (
                        rollout_policy_history_valid[hist_idx] & (~boundary_tensor)
                    )
                rollout_previous_action[boundary_tensor] = 0.0
                rollout_previous_action_valid[boundary_tensor] = False
        ...
        with torch.no_grad():
            ...
            diag_inputs = b_agent_inputs[:diag_n]
            actor_feat_diag, critic_feat_diag = agent._trunks_from_agent_input(
                None, diag_inputs
            )
            ...
            diag_mem, diag_incoming, diag_valid = agent.unpack_agent_input(diag_inputs)
            zero_action_inputs = agent.pack_agent_input(
                diag_mem, torch.zeros_like(diag_incoming), diag_valid
            )
            zero_action_actor, zero_action_critic = agent._trunks_from_agent_input(
                None, zero_action_inputs
            )
            zero_action_dist, _, _ = agent._actor_dist(
                zero_action_actor, b_explore_codes[:diag_n]
            )
            prev_action_sensitivity = (
                to_action_diag(dist_code.mean) - to_action_diag(zero_action_dist.mean)
            ).abs().mean()
            base_value_diag = value_logits_to_scalar(
                agent.critic_head(critic_feat_diag).view(
                    -1, agent.critic_mtp_horizon, agent.num_bins
                )[:, 0]
            )
            zero_action_value = value_logits_to_scalar(
                agent.critic_head(zero_action_critic).view(
                    -1, agent.critic_mtp_horizon, agent.num_bins
                )[:, 0]
            )
            prev_action_value_sensitivity = (base_value_diag - zero_action_value).abs().mean()

            # Reverse only past rows, keeping memory/action/validity aligned and
            # the current state fixed. This measures use of temporal ordering,
            # not sensitivity to corrupt or cross-episode content.
            past_perm = torch.arange(agent.agent_context - 2, -1, -1, device=device)
            shuffled_mem = torch.cat([diag_mem[:, past_perm], diag_mem[:, -1:]], dim=1)
            shuffled_incoming = torch.cat(
                [diag_incoming[:, past_perm], diag_incoming[:, -1:]], dim=1
            )
            shuffled_valid = torch.cat(
                [diag_valid[:, past_perm], diag_valid[:, -1:]], dim=1
            )
            shuffled_inputs = agent.pack_agent_input(
                shuffled_mem, shuffled_incoming, shuffled_valid
            )
            shuffled_actor, shuffled_critic = agent._trunks_from_agent_input(
                None, shuffled_inputs
            )
            shuffled_dist, _, _ = agent._actor_dist(
                shuffled_actor, b_explore_codes[:diag_n]
            )
            shuffled_history_sensitivity = (
                to_action_diag(dist_code.mean) - to_action_diag(shuffled_dist.mean)
            ).abs().mean()
            shuffled_value = value_logits_to_scalar(
                agent.critic_head(shuffled_critic).view(
                    -1, agent.critic_mtp_horizon, agent.num_bins
                )[:, 0]
            )
            shuffled_history_value_sensitivity = (
                base_value_diag - shuffled_value
            ).abs().mean()
        ...
        writer.add_scalar(
            "agent_sts/actor_temporal_gate_abs",
            agent.actor_readout.temporal_gate.abs().mean().item(), global_step,
        )
        writer.add_scalar(
            "agent_sts/actor_reread_gate_abs",
            agent.actor_readout.reread_gate.abs().mean().item(), global_step,
        )
        writer.add_scalar(
            "agent_sts/critic_temporal_gate_abs",
            agent.critic_readout.temporal_gate.abs().mean().item(), global_step,
        )
        writer.add_scalar(
            "agent_sts/critic_reread_gate_abs",
            agent.critic_readout.reread_gate.abs().mean().item(), global_step,
        )
        writer.add_scalar(
            "agent_sts/previous_action_policy_sensitivity",
            prev_action_sensitivity.item(), global_step,
        )
        writer.add_scalar(
            "agent_sts/previous_action_value_sensitivity",
            prev_action_value_sensitivity.item(), global_step,
        )
        writer.add_scalar(
            "agent_sts/shuffled_history_policy_sensitivity",
            shuffled_history_sensitivity.item(), global_step,
        )
        writer.add_scalar(
            "agent_sts/shuffled_history_value_sensitivity",
            shuffled_history_value_sensitivity.item(), global_step,
        )
