# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + ABSOLUTE-SURPRISAL DELIGHT GATE v15.
#
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.dg_absdelight:
                    # v15: ABSOLUTE entropy-referenced surprisal delight gate,
                    # REPLACES the PPO clipped surrogate. ell~ = (-logpi_theta)
                    # - H(s) is the realized WITHIN-state typicality of the
                    # sampled action (zero-mean per state since E[-logpi]=H):
                    # ell~>0 a tail/surprising draw, ell~<0 modal. std-normalize
                    # so eta is a clean dimensionless trust width; clamp into chi
                    # (NOT zero-mask) so Beta-boundary samples cannot saturate
                    # the gate yet the delight quadrant keeps its gradient. No
                    # ratio, no clip; target_kl is the only drift backstop.
                    # newlogprob and entropy are both summed over action dims
                    # (matching units), so ell~ is exactly per-state centered.
                    ...
                    pg_loss = -(2.0 * dg_weight * relgate * mb_advantages * newlogprob).mean()
                    with torch.no_grad():
                        ...
                        # recover_frac here = share of U>0 samples fully amplified
