# LeJEPA Policy-Gradient-Risk Trace Vector Policy Optimization v9.
#
...
# A single geometric trace horizon is learned from cross-environment vector
# policy-gradient risk against the longest reliable suffix, then hard-lagged one rollout.
# This corrects v8's failure: per-transition return MSE priced irreducible noise as if PPO
# used one sample and collapsed the realized trace to 1.4 steps. With only 16 independent
# trajectories, a global horizon is identifiable where a state hazard is not. No
# Q-function, endpoint mixture, return reconstruction, or EMA is used.
#
...
TRACE_CANDIDATE_HORIZONS = (1, 2, 4, 8, 12, 16.8067, 25, 37, 55, 75, 100)
...
class Args:
    ...
    trace_risk_epochs: int = 0  # obsolete v8 return-risk head is disabled in v9
    trace_risk_batch: int = 1024
    trace_risk_min_samples: int = 4096
    trace_variance_floor: float = 1e-4
    trace_pg_sketch_dim: int = 64
    trace_validation_margin: float = 0.02
    ...
    ssl_encoder_lr: float = 2e-6     # transactional hard-snapshot encoder rate
...
def learned_vector_gae_candidates(
    delta,
    continuation,
    gamma,
    candidate_state_lambda,
):
    """Evaluate many state-hazard traces in one reverse-time recurrence."""
    if candidate_state_lambda.shape[1:] != continuation.shape:
        raise ValueError("candidate lambdas must have shape (K,T,B)")
    candidate_count = candidate_state_lambda.shape[0]
    next_lambda = torch.zeros_like(candidate_state_lambda)
    next_lambda[:, :-1] = candidate_state_lambda[:, 1:]
    out = delta.new_zeros(
        (candidate_count,) + tuple(delta.shape)
    )
    running = delta.new_zeros(
        (candidate_count,) + tuple(delta.shape[1:])
    )
    for t in reversed(range(delta.shape[0])):
        running = delta[t].unsqueeze(0) + (
            gamma
            * continuation[t].view(1, -1, 1)
            * next_lambda[:, t].unsqueeze(-1)
            * running
        )
        out[:, t] = running
...
def lambda_from_effective_horizon(horizon, gamma):
    """Map discounted geometric trace mass to its constant continuation lambda."""
    return min(1.0, max(0.0, (1.0 - 1.0 / horizon) / gamma))
...
def optimal_recursive_lambda(
    ...
    denominator = (
        continuation_direction.square()
        + variance_temperature * recursive_variance
    ).mean(-1).clamp_min(1e-12)
    return (numerator / denominator).clamp(0.0, 1.0)


def environment_gradient_tensors(
    score_sketch,
    vector_advantage,
    valid_mask,
):
    """One vector policy-gradient sketch per independent environment."""
    if score_sketch.shape[:2] != vector_advantage.shape[:2]:
        raise ValueError("score and advantage time/environment axes must match")
    tensors = []
    for environment in range(score_sketch.shape[1]):
        valid = valid_mask[:, environment]
        count = valid.sum().clamp_min(1)
        tensors.append(
            score_sketch[valid, environment].T
            @ vector_advantage[valid, environment]
            / count
        )
    return torch.stack(tensors)


def environment_routed_gradient_tensors(
    projected_action_score,
    routed_vector_advantage,
    valid_mask,
):
    """Per-environment sketched Beta-head gradients with actuator-specific credit."""
    tensors = []
    for environment in range(projected_action_score.shape[1]):
        valid = valid_mask[:, environment]
        count = valid.sum().clamp_min(1)
        tensors.append(
            torch.einsum(
                "tak,tad->kd",
                projected_action_score[valid, environment],
                routed_vector_advantage[valid, environment],
            )
            / count
        )
    return torch.stack(tensors)
...
def cross_environment_gradient_risk(candidate, reference, environment_mask):
    """U-statistic squared bias plus variance of the environment-mean gradient."""
    candidate = candidate[environment_mask]
    reference = reference[environment_mask]
    environment_count = candidate.shape[0]
    if environment_count < 2:
        raise ValueError("policy-gradient risk needs at least two environments")
    difference = candidate - reference
    difference_sum = difference.sum(0)
    squared_bias = (
        difference_sum.square().sum()
        - difference.square().sum()
    ) / (environment_count * (environment_count - 1))
    centered = candidate - candidate.mean(0, keepdim=True)
    mean_variance = centered.square().sum() / (
        environment_count * (environment_count - 1)
    )
    return squared_bias, mean_variance, squared_bias + mean_variance
...
if __name__ == "__main__":
    ...
    if args.control_encoder_tau != 1.0:
        raise ValueError("control_encoder_tau must be 1: v9 forbids encoder EMA")
    ...
    if args.norm_adv or args.adv_transform != "v10":
        raise ValueError("v9 requires vector whitening and forbids scalar advantage scaling")
    if args.auto_entropy:
        raise ValueError("v9 does not mix scalar entropy returns into its vector trace")
    if args.ent_coef != 0.0:
        raise ValueError("v9 forbids a scalar entropy term in its vector policy objective")
    if args.trace_risk_epochs != 0:
        raise ValueError("v9 replaces and disables the obsolete per-return risk head")
    if args.actor_dist != "beta":
        raise ValueError("v9's vector policy path currently requires the Beta actor")
    if not args.separate_grad_clip:
        raise ValueError("v9 requires separate_grad_clip for vector/critic gradient isolation")
    if args.num_envs < 4:
        raise ValueError(
            "policy-gradient risk requires at least two environments in each parity fold"
        )
    if (
        ...
        or args.trace_pg_sketch_dim < 1
        or args.trace_validation_margin < 0.0
    ):
    ...
    encoder_parameters = list(ssl.encoder.parameters())
    ...
    ssl_optimizer = optim.AdamW(
        [
            {"params": encoder_parameters, "lr": args.ssl_encoder_lr},
            {"params": predictor_parameters, "lr": args.ssl_lr},
        ],
    ...
    learned_trace_lambda = args.gae_lambda
    trace_use_learned = False
    sketch_generator = torch.Generator(device=device)
    sketch_generator.manual_seed(args.seed + 104729)
    policy_gradient_sketch = (
        torch.randint(
            0,
            2,
            (
                2 * act_dim_sf * (args.hidden + 1),
                args.trace_pg_sketch_dim,
            ),
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            ssl_optimizer.param_groups[0]["lr"] = frac * args.ssl_encoder_lr
            ssl_optimizer.param_groups[1]["lr"] = frac * args.ssl_lr
        ...
        with torch.no_grad():
            ...
            trace_learned_was_enabled = trace_use_learned
            trace_lambda_used = learned_trace_lambda
            if trace_was_ready:
                (
                    direct_mean,
                    direct_variance,
            ...
            state_lambda = (
                torch.full_like(rewards, learned_trace_lambda)
                if args.learn_trace and trace_use_learned
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    raise RuntimeError("v9 requires separate_grad_clip")

            if args.target_kl is not None and kl_loss > args.target_kl:
                break

        # Learn the geometric lambda for the NEXT rollout from the object PPO
        # actually averages: a vector policy-gradient estimate. The longest reliable
        # suffix is the reference. A fixed random sketch makes the exact Beta-head
        # weight/bias gradient tractable without projecting away the outcome embedding.
        pg_risk_train = pg_risk_validation = pg_risk_fixed_validation = float("nan")
        pg_risk_bias = pg_risk_variance = float("nan")
        selected_trace_lambda = learned_trace_lambda
        selected_trace_lambda_mean = float(args.gae_lambda)
        next_trace_use_learned = False
        next_trace_vector_advantage = fixed_vector_advantage
        valid_tb = mc_mask.reshape(args.num_steps, args.num_envs)
        environments_with_reference = valid_tb.sum(0) > 0
        selection_train_environments = (
            torch.arange(args.num_envs, device=device).remainder(2)
            == iteration % 2
        ) & environments_with_reference
        selection_validation_environments = (
            ~(
                torch.arange(args.num_envs, device=device).remainder(2)
                == iteration % 2
            )
        ) & environments_with_reference
        selection_has_two_folds = bool(
            selection_train_environments.sum() >= 2
            and selection_validation_environments.sum() >= 2
        )
        if (
            n_mc >= args.trace_risk_min_samples
            and selection_has_two_folds
        ):
            with torch.no_grad():
                direct_vector_advantage = torch.cat(
                    [
                        mc_phi - psi_raw(successor_cur),
                        mc_resid.unsqueeze(-1),
                    ],
                    dim=-1,
                )
                train_environments = selection_train_environments
                validation_environments = selection_validation_environments
                train_valid_tb = (
                    valid_tb & train_environments.unsqueeze(0)
                )
                direct_train = direct_vector_advantage[train_valid_tb]
                (
                    _,
                    _,
                    _,
                    pg_inv_sqrt_covariance,
                    _,
                ) = whiten_vector_advantage(
                    direct_train,
                    reward_covector,
                    args.vector_whiten_ridge,
                )
                direct_center = direct_train.mean(0, keepdim=True)
                direct_white = torch.zeros_like(direct_vector_advantage)
                direct_white[valid_tb] = (
                    direct_vector_advantage[valid_tb] - direct_center
                ) @ pg_inv_sqrt_covariance

                alpha_jacobian = 1.0 - (
                    -(old_alphas.reshape(-1, act_dim_sf) - 1.0)
                ).exp()
                beta_jacobian = 1.0 - (
                    -(old_betas.reshape(-1, act_dim_sf) - 1.0)
                ).exp()
                raw_head_score = action_score * torch.stack(
                    [alpha_jacobian, beta_jacobian], dim=2
                )
                raw_actor_features = torch.cat(
                    [
                        actor_feature_buffer,
                        actor_feature_buffer.new_ones(
                            actor_feature_buffer.shape[0], 1
                        ),
                    ],
                    dim=-1,
                )
                projected_action_score = torch.einsum(
                    "nac,nf,acfk->nak",
                    raw_head_score,
                    raw_actor_features,
                    policy_gradient_sketch.view(
                        act_dim_sf,
                        2,
                        args.hidden + 1,
                        args.trace_pg_sketch_dim,
                ...
                if router_ready:
                    direct_routed, _, _ = routed_vector_advantage(
                        lagged_router_features,
                        action_score,
                        router_coefficients,
                        pg_inv_sqrt_covariance,
                        direct_white.reshape(-1, sf_dim + 1),
                        args.router_max_fraction,
                    )
                else:
                    direct_routed = direct_white.reshape(
                        -1, sf_dim + 1
                    ).unsqueeze(1).expand(-1, act_dim_sf, -1)
                direct_gradient = environment_routed_gradient_tensors(
                    projected_action_score,
                    direct_routed.reshape(
                        args.num_steps,
                        args.num_envs,
                        act_dim_sf,
                        sf_dim + 1,
                ...
                candidate_lambda_values = torch.as_tensor(
                    [
                        lambda_from_effective_horizon(horizon, args.gamma)
                        for horizon in TRACE_CANDIDATE_HORIZONS
                    ],
                    device=device,
                    dtype=rewards.dtype,
                )
                candidate_lambdas = candidate_lambda_values.view(
                    -1, 1, 1
                ).expand(-1, args.num_steps, args.num_envs)
                candidate_advantages = learned_vector_gae_candidates(
                    delta_vector,
                    continuation,
                    args.gamma,
                    candidate_lambdas,
                ...
                candidate_gradients = []
                train_risks = []
                for candidate_advantage in candidate_advantages:
                    candidate_valid = candidate_advantage[valid_tb]
                    candidate_train_center = candidate_advantage[
                        train_valid_tb
                    ].mean(0, keepdim=True)
                    candidate_white = torch.zeros_like(candidate_advantage)
                    candidate_white[valid_tb] = (
                        candidate_valid - candidate_train_center
                    ) @ pg_inv_sqrt_covariance
                    if router_ready:
                        candidate_routed, _, _ = routed_vector_advantage(
                            lagged_router_features,
                            action_score,
                            router_coefficients,
                            pg_inv_sqrt_covariance,
                            candidate_white.reshape(-1, sf_dim + 1),
                            args.router_max_fraction,
                        )
                    else:
                        candidate_routed = candidate_white.reshape(
                            -1, sf_dim + 1
                        ).unsqueeze(1).expand(-1, act_dim_sf, -1)
                    candidate_gradient = environment_routed_gradient_tensors(
                        projected_action_score,
                        candidate_routed.reshape(
                            args.num_steps,
                            args.num_envs,
                            act_dim_sf,
                            sf_dim + 1,
                    ...
                    train_risks.append(
                        cross_environment_gradient_risk(
                            candidate_gradient,
                            direct_gradient,
                            train_environments,
                        )[2]
                    )
                stacked_train_risks = torch.stack(train_risks)
                best_train_risk = stacked_train_risks.min()
                statistically_tied = stacked_train_risks <= (
                    best_train_risk
                    + args.trace_validation_margin
                    * best_train_risk.abs().clamp_min(1e-12)
                )
                selected_index = statistically_tied.nonzero(
                    as_tuple=True
                )[0].max()
                selected_index_int = int(selected_index)
                selected_gradient = candidate_gradients[selected_index_int]
                selected_trace_lambda = float(
                    candidate_lambda_values[selected_index_int]
                )
                selected_trace_lambda_mean = selected_trace_lambda
                selected_bias, selected_variance, selected_validation = (
                    cross_environment_gradient_risk(
                        selected_gradient,
                        direct_gradient,
                ...
                fixed_white = (
                    fixed_vector_advantage
                    - fixed_vector_advantage[train_valid_tb].mean(
                        0, keepdim=True
                    )
                ) @ pg_inv_sqrt_covariance
                if router_ready:
                    fixed_routed, _, _ = routed_vector_advantage(
                        lagged_router_features,
                        action_score,
                        router_coefficients,
                        pg_inv_sqrt_covariance,
                        fixed_white.reshape(-1, sf_dim + 1),
                        args.router_max_fraction,
                    )
                else:
                    fixed_routed = fixed_white.reshape(
                        -1, sf_dim + 1
                    ).unsqueeze(1).expand(-1, act_dim_sf, -1)
                fixed_gradient = environment_routed_gradient_tensors(
                    projected_action_score,
                    fixed_routed.reshape(
                        args.num_steps,
                        args.num_envs,
                        act_dim_sf,
                        sf_dim + 1,
                ...
                fixed_validation = cross_environment_gradient_risk(
                    fixed_gradient,
                    direct_gradient,
                    validation_environments,
                )[2]
                pg_risk_train = float(train_risks[selected_index_int])
                pg_risk_validation = float(selected_validation)
                pg_risk_fixed_validation = float(fixed_validation)
                pg_risk_bias = float(selected_bias)
                pg_risk_variance = float(selected_variance)
                validation_improvement = (
                    fixed_validation - selected_validation
                )
                next_trace_use_learned = bool(
                    torch.isfinite(selected_validation)
                    and validation_improvement
                    > args.trace_validation_margin
                    * fixed_validation.abs().clamp_min(1e-12)
                )
                next_trace_vector_advantage = (
                    candidate_advantages[selected_index_int]
                    if next_trace_use_learned
                    else fixed_vector_advantage
                )
        if not args.learn_trace:
            next_trace_use_learned = False
            next_trace_vector_advantage = fixed_vector_advantage
            selected_trace_lambda = args.gae_lambda
        learned_trace_lambda = selected_trace_lambda
        trace_use_learned = next_trace_use_learned
        ...
        with torch.no_grad():
            (
                ...
                _,
                _,
                next_router_inv_sqrt_covariance,
                next_router_sqrt_covariance,
            ) = whiten_vector_advantage(
                next_trace_vector_advantage,
                reward_covector,
                args.vector_whiten_ridge,
            ...
            (
                ...
                fit_router_features,
                action_score,
                next_trace_vector_advantage.reshape(-1, sf_dim + 1)
                - next_trace_vector_advantage.reshape(
                    -1, sf_dim + 1
                ).mean(0, keepdim=True),
                args.router_ridge,
                router_train_mask,
            ...
            router_ready = bool(router_fit_r2 > 0.0)
        ...
        old_online_encoder_params = [
            parameter.detach().clone() for parameter in ssl.encoder.parameters()
        ]
        old_encoder_optimizer_state = {
            parameter: {
                key: value.detach().clone()
                if torch.is_tensor(value)
                else copy.deepcopy(value)
                for key, value in ssl_optimizer.state.get(parameter, {}).items()
            }
        ...
        with torch.no_grad():
            ...
            if accepted:
                ...
                for parameter, old_parameter in zip(
                    ssl.encoder.parameters(), old_online_encoder_params
                ):
                    parameter.copy_(old_parameter)
                for parameter in encoder_parameters:
                    ssl_optimizer.state[parameter] = old_encoder_optimizer_state[
                        parameter
                    ]
                probe_control_after = probe_control_before
        ...
        writer.add_scalar(
            "actor_risk/lambda_used", trace_lambda_used, global_step
        )
        writer.add_scalar(
            "actor_risk/selected_lambda",
            selected_trace_lambda,
            global_step,
        )
        writer.add_scalar(
            "actor_risk/selected_lambda_mean",
            selected_trace_lambda_mean,
            global_step,
        )
        writer.add_scalar(
            "actor_risk/learned_used",
            float(trace_learned_was_enabled and args.learn_trace),
            global_step,
        )
        writer.add_scalar(
            "actor_risk/next_learned_enabled",
            float(next_trace_use_learned and args.learn_trace),
            global_step,
        )
        writer.add_scalar(
            "actor_risk/valid_fold_pair",
            float(selection_has_two_folds),
            global_step,
        )
        writer.add_scalar(
            "actor_risk/train_total", pg_risk_train, global_step
        )
        writer.add_scalar(
            "actor_risk/validation_total", pg_risk_validation, global_step
        )
        writer.add_scalar(
            "actor_risk/fixed95_validation_total",
            pg_risk_fixed_validation,
            global_step,
        )
        writer.add_scalar(
            "actor_risk/validation_bias", pg_risk_bias, global_step
        )
        writer.add_scalar(
            "actor_risk/validation_variance", pg_risk_variance, global_step
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "ssl/encoder_learning_rate",
            ssl_optimizer.param_groups[0]["lr"],
            global_step,
        )
        writer.add_scalar(
            "ssl/predictor_learning_rate",
            ssl_optimizer.param_groups[1]["lr"],
            global_step,
        )
