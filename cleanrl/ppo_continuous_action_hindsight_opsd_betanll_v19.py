...
#
# OPSD v19: ReLU positive-advantage AWR weighting.
#
# v19 note: exp(adv_z) weights every sample ~1, so AWR is mostly uniform BC of the
# batch (~0 KL) plus a tiny tilt -> under-movement. ReLU(adv_z) zeroes the
# below-average half, dropping the dead uniform component for pure ascent.
#
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v19: positive-part weighting. exp(adv_z) weights every sample ~1, so the
                # AWR gradient is mostly uniform BC of the batch (already matched -> ~0 KL)
                # plus a small tilt -> under-movement (approx_kl~0.005). ReLU zeroes the
                # below-average half, removing the dead uniform component: pure ascent.
                awr_weights = torch.relu(mb_advantages)
