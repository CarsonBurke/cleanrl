# PPO + IterThink v45 (REVERT the entropy target to SAC's -|A|; keep v44's plain soft GAE). From v44.
# Clean soft-actor-critic structure (unchanged):
...
#     A_soft     = R_soft_lambda - V_soft(s_t)                      (the policy advantage, used DIRECTLY)
# ONE algorithmic change from v44 (a clean single-variable test of the v41 WALKER REGRESSION):
#   (1) target_entropy_coef 0.0 -> -1.0 (SAC's -dim; = v40's setting). DIAGNOSIS: the v41 move to target 0
#       was the regression, NOT the advantage transform. Evidence: v40 (-dim, alpha bleeds to floor) reached
#       Walker 4964 as a LATE bloomer (637->1405->2188 @4M -> ~5000); v44 (target 0, BINDING -> dual forces
#       entropy UP) left Walker DEAD FLAT (425->459 over 4M). Target 0 sits ABOVE the policy's natural squashed
#       H on every env, so the dual cranks alpha to inject exploration -- fatal for a sensitive balancer that
#       must SHARPEN to converge. -dim is (intentionally) unreachable: H stays above it, alpha -> alpha_min,
#       max-ent pressure gets OUT OF THE WAY, reward gradient drives the policy deterministic. NOTE this makes
#       the soft-entropy channel ~inert (alpha at floor); the apparatus is retained only for a clean diff vs
#       v44. If Walker recovers, a follow-up can strip the now-dormant soft machinery.
# KEPT from v44: plain v10 GAE (gave Hopper 1235 >> v40's 779 -- a genuine gain worth preserving; the
# Walker-vs-Hopper transform tension, v43-rankgauss-Walker-854 > v44-plain-463, is a SEPARATE follow-up if
# the target fix alone doesn't restore Walker). direct_entropy True; --diag instrumentation (raw A_soft
# moments, reward/entropy-return std split, pre-tanh |z| quantiles, mu/sigma min-max, per-head grad norms).
...
class Args:
    ...
    adv_transform: str = "v10"      # v44: PLAIN soft GAE -- A_soft = R_soft_lambda - V_soft(s_t), used
                                    #      directly. Drops v34's rankgauss_signmag rank-shaping, which
                                    #      discarded the advantage MAGNITUDE (rank-only) and is suspected in
                                    #      the Walker regression. norm_adv still z-scores it (standard PPO).
