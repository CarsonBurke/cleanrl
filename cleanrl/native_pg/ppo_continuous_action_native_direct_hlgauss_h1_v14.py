# Native direct policy gradient with a single-head symlog HL-Gauss critic v14.
#
# Replaces raw-return MSE with a neutral-initialized categorical critic over
# symlog-spaced raw returns. A single head directly optimizes the current value;
# expected raw values still drive unchanged GAE. The state-dependent direct
# actor is unchanged from v13, isolating whether MTP caused value-fit failures.
# Hypothesis: the primary value head fits better without auxiliary horizons.
...
def interpolate_parameters(parameters, old_parameters, candidate_parameters, fraction):
    with torch.no_grad():
        for parameter, old, candidate in zip(parameters, old_parameters, candidate_parameters, strict=True):
            if fraction == 0.0:
                parameter.copy_(old)
            else:
...
class Agent(nn.Module):
    def __init__(self, envs, args, raw_value_support=None):
        super().__init__()
        ...
        if raw_value_support is None:
            value_edges = torch.linspace(args.value_min, args.value_max, args.value_num_bins + 1)
            raw_value_support = symexp((value_edges[:-1] + value_edges[1:]) / 2.0)
        if raw_value_support.shape != (args.value_num_bins,):
            raise ValueError("raw_value_support must contain one scalar per value bin")
        self.register_buffer("raw_value_support", raw_value_support.detach().clone())
    ...
    def decode_value_logits(self, logits):
        return (torch.softmax(logits, dim=-1) * self.raw_value_support).sum(dim=-1)
...
if __name__ == "__main__":
    ...
    agent = Agent(
        envs,
        args,
        raw_value_support=symexp(hlgauss_support.support),
    ).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.compile:
            torch.compiler.cudagraph_mark_step_begin()
        gaussian_logprob, _, old_mean_graph, old_logstd_graph = action_logprobs(b_obs, b_raw_actions)
        ...
        candidate_parameters = [parameter.detach().clone() for parameter in actor_parameters]
        proposal_delta_norm = parameter_delta_norm(candidate_parameters, old_parameters)

        def evaluate_policy_change():
            if args.compile:
                torch.compiler.cudagraph_mark_step_begin()
            new_gaussian_logprob, new_action_logprob, new_mean, new_logstd = action_logprobs(b_obs, b_raw_actions)
            ...
            exact_kl = kl_divergence(Normal(old_mean, old_std), Normal(new_mean, new_logstd.exp())).sum(dim=-1)
            return (
        ...
        writer.add_scalar("update/proposal_delta_norm", proposal_delta_norm.item(), global_step)
        writer.add_scalar(
