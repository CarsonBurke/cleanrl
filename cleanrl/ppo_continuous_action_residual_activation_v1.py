# Residual activation PPO v1, based only on ppo_continuous_action.py.
# Actor and critic: h = stage1(obs); h = h + stage2(h); output = head(h).
# Compare SiLU, tanh, SiTU-GLU, ReLU^2, and 0.5 * LeakyReLU(x, 0.5)^2.
# No added normalization, gates, depth, or PPO changes. Hypothesis: the identity
# path preserves useful features while activation choice controls branch growth.
# SiTU-GLU uses the existing gated FFN/init, so it is not parameter-matched to
# a single Linear+activation stage. Host mirrors compose existing fused stages.
import os
import random
import time
...
class Args:
    ...
    activation: str = "tanh"
    """silu, tanh, stiglu, relusq, or leakyrelusq (scaled by 0.5)"""
    num_envs: int = 32
...
class ResidualMLP(nn.Module):
    """Default two-stage width-64 trunk with one identity skip."""

    def __init__(self, observation_dim, output_dim, activation, output_std):
        super().__init__()
        self.activation_scale = 0.5 if activation == "leakyrelusq" else 1.0
        ...
        def stage(in_dim):
            ...
            nonlinearities = {
                ...
                "relusq": ReluSq, "leakyrelusq": LeakyReluSq,
            }
            return nn.Sequential(layer_init(nn.Linear(in_dim, 64)), nonlinearities[activation]())
    ...
    def forward(self, x):
        h = self.first(x) * self.activation_scale
        return self.head(h + self.second(h) * self.activation_scale)
...
class ResidualHostMirror:
    ...
    def __init__(self, actor, num_envs):
        ...
        self.scale = actor.activation_scale
    ...
    def __call__(self, observations):
        np.multiply(self.first(observations), self.scale, out=self.hidden)
        np.multiply(self.second(self.hidden), self.scale, out=self.branch)
        np.add(self.hidden, self.branch, out=self.hidden)
        return self.head(self.hidden)


class Agent(nn.Module):
    def __init__(self, envs, activation="tanh"):
        super().__init__()
        ...
        self.critic = ResidualMLP(observation_dim, 1, activation, output_std=1.0)
        self.actor = ResidualMLP(observation_dim, 2 * self.action_dim, activation, output_std=0.01)
...
def validate_args(args):
    if args.activation not in {"silu", "tanh", "stiglu", "relusq", "leakyrelusq"}:
        raise ValueError("activation must be silu, tanh, stiglu, relusq or leakyrelusq")
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", f"two width-64 stages; one residual skip; activation={args.activation}")
        envs = make_training_env(args, run_name)
        resources.callback(envs.close)
        agent = Agent(envs, args.activation).to(device)
        ...
        if args.save_model:
            ...
            episodic_returns = evaluate(
                ...
                run_name=f"{run_name}-eval", Model=lambda envs: Agent(envs, args.activation), device=device, gamma=args.gamma,
            )
