# Per-channel LayerScale atop scale-restored normalized SiTU-GLU v8.
# https://arxiv.org/pdf/2103.17239v2, section 2: learn a diagonal residual gain.
# h=N(first(obs)); b=N(second(8*h)); output=g*head(8*N(h+lambda*b)).
# lambda starts at 0.1, is unconstrained, and acts AFTER branch normalization.
# Keep the unit-weight projection and scale corrections; lambda is not projected.
# Adaptation retains post-sum normalization. PPO: no advnorm, no value clip,
# policy ratio bounds [0.80,1.28]. Hypothesis: small channelwise residual updates
# improve optimization over equal-weight addition without reducing the skip term.
import os
import random
import time
...
class ResidualMLP(nn.Module):
    ...
    def __init__(self, observation_dim, output_dim, output_std):
        ...
        self.layer_scale = nn.Parameter(torch.full((64,), 0.1))

    def forward(self, x):
        h = F.normalize(self.first(x), p=2, dim=-1)
        branch = F.normalize(self.second(8.0 * h), p=2, dim=-1)
        return self.readout_gain * self.head(8.0 * F.normalize(h + self.layer_scale * branch, p=2, dim=-1))
...
class ResidualHostMirror:
    ...
    def __init__(self, actor, num_envs):
        ...
        self.layer_scale = np.empty(64, dtype=np.float32)
        self.refresh()
    ...
    def refresh(self):
        ...
        np.copyto(self.layer_scale, self.actor.layer_scale.detach().cpu().numpy())
    ...
    def __call__(self, observations):
        ...
        np.multiply(self.branch, self.layer_scale, out=self.branch)
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", "residual SiTU-GLU; unit-L2 matrices and residual directions; sqrt(width) branch/readout scale; learned output gains; per-channel LayerScale initialized to 0.1 after branch normalization")
