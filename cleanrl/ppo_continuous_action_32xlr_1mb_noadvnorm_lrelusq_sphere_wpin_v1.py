...
# Fix 2 of 2 for the LeakyReluSq sphere trunk: keep the activation, pin every
# scale-invariant trunk matrix (in_proj, each block's lin1/lin2) to its
# initial Frobenius norm after every optimizer step.
# Measured: lin2(f(lin1(u))) with f positively homogeneous of degree 2, wrapped
# in justnorm on both sides, makes the trunk exactly invariant to radial weight
# scaling (cos(y, y | W1 *= 3) = 1.0000 against 0.9989 for SiTU-GLU, which the
# tanh caps make non-homogeneous). The radial direction carries no gradient, yet
# Adam without weight decay still random-walks ||W|| upward, and for a
# scale-invariant map the functional step per unit update decays like 1/||W||.
# Reference-run evidence over 5M -> 30M: clipfrac falls to 0.63x its 5-7M level
# (0.228 -> 0.145) while the SiTU sphere holds 0.96x (0.184 -> 0.177), and both
# runs' approx_kl decays by the same 0.63-0.65x that LR annealing alone buys.
# Equal mean KL with a collapsing clipped fraction is what a shrinking
# functional step per update looks like.
# Pinning removes exactly that one null direction per matrix and nothing else:
# directions still learn freely, and the activation stays at its v = 2 design
# point instead of drifting.
# Hypothesis: late-training plasticity (approx_kl, clipfrac) stops decaying and
# the run keeps climbing past 30M.
...
from cleanrl.shared.host_actor import make_lrelu_sphere_trunk
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            make_lrelu_sphere_trunk(observation_dim, 64),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; 32x LR; 1 minibatch; hypersphered residual LeakyReluSq trunk, Frobenius-pinned; no advantage normalization; trunk probes")
        ...
        pins = [WeightNormPin(agent.actor[0]), WeightNormPin(agent.critic[0])]
