# Pan Goal Solver v4: Pan hindsight plus aspirational latent-progress improvement.
#
# A reward-free LeJEPA learns observation tokens and temporal control belief.
# Hindsight and discounted occupancy train general goal following. Locally
# matched real interventions additionally teach the first action that made more
# Euclidean latent-MSE progress toward the frozen aspirational goal. Reward
# trains only a global affine goal ray P(y)=mu+s(y)d;
...
class Args:
    ...
    short_hindsight_coef: float = 0.25
    aspirational_action_coef: float = 4.0
    aspirational_rank_coef: float = 1.0
    aspirational_match_radius: float = 0.75
    aspirational_rank_margin: float = 0.02
...
class VectorReplayBuffer:
    ...
    def sample_hindsight(
        ...
        return {
            ...
            "diagnostic_reward": self.reward[env, end % self.capacity],
...
class FourierExplorer:
    ...
    def resample(self, mask):
        ...
        harmonic_amplitude = self.rng.uniform(
            -0.8, 0.8, size=(len(indices), 3, 1)
        )
        joint_amplitude = self.rng.uniform(
            0.5, 1.2, size=(len(indices), 1, self.act_dim)
        )
        self.amplitude[indices] = harmonic_amplitude * joint_amplitude
        self.amplitude[indices] /= np.arange(1, 4, dtype=np.float32)[None, :, None]
        base_phase = self.rng.uniform(0, 2 * np.pi, size=(len(indices), 3, 1))
        phase_lag = self.rng.uniform(-np.pi, np.pi, size=(len(indices), 1, 1))
        joint_index = np.arange(self.act_dim, dtype=np.float32)[None, None]
        self.phase[indices] = base_phase + phase_lag * joint_index
...
def train_follower_step(
    agent, optimizer, batch, args, device, rng, train_aspiration=True
):
    obs = as_tensor(batch["obs"], device)
    incoming = as_tensor(batch["incoming"], device)
    next_obs = as_tensor(batch["next_obs"], device)
    ...
    action_loss = one_step_action_loss + args.short_hindsight_coef * future_action_loss
    ...
    with torch.no_grad():
        ...
        raw_aspirational_progress = (
            (current_goal - aspiration_goal).square().mean(-1)
            - (next_goal - aspiration_goal).square().mean(-1)
        )
        goal_radius = (current_goal - aspiration_goal).square().mean(-1).sqrt()
        aspirational_progress = raw_aspirational_progress / (
            2 * goal_radius + 1e-6
        )
        ...
        state_match = (
            ...
            & (nearest_distance <= args.aspirational_match_radius)
        )
        partner_progress = aspirational_progress[matched]
        winner_progress = torch.maximum(aspirational_progress, partner_progress)
        progress_gap_all = (aspirational_progress - partner_progress).abs()
        positive_match = state_match & (winner_progress > 0)
        if positive_match.any():
            gap_threshold = torch.quantile(progress_gap_all[positive_match], 0.5)
            valid_match = positive_match & (progress_gap_all >= gap_threshold)
        else:
            valid_match = positive_match
    if valid_match.any():
        selected = torch.nonzero(valid_match, as_tuple=False).squeeze(1)
        partner = matched[selected]
        pair_prefers_source = (
            aspirational_progress[selected] >= aspirational_progress[partner]
        )
        preferred_action = torch.where(
            pair_prefers_source[:, None], action[selected], action[partner]
        )
        rejected_action = torch.where(
            pair_prefers_source[:, None], action[partner], action[selected]
        )
        progress_gap = (
            aspirational_progress[selected] - aspirational_progress[partner]
        ).abs()
        preference_weight = (
            progress_gap / progress_gap.mean().clamp_min(1e-6)
        ).clamp(max=4.0)
        anchor_action = agent.follower(
            belief[selected],
            current_goal[selected],
            aspiration_goal.expand(selected.shape[0], -1),
        )
        partner_action = agent.follower(
            belief[partner],
            current_goal[partner],
            aspiration_goal.expand(selected.shape[0], -1),
        )
        aspirational_pair_loss = (
            preference_weight
            * 0.5
            * (
                (anchor_action - preferred_action).square().mean(-1)
                + (partner_action - preferred_action).square().mean(-1)
            )
        ).mean()
        winner_error = (anchor_action - preferred_action).square().mean(-1)
        loser_error = (anchor_action - rejected_action).square().mean(-1)
        aspirational_rank_loss = F.relu(
            args.aspirational_rank_margin + winner_error - loser_error
        ).mean()
        aspirational_action_abs = 0.5 * (
            anchor_action.abs().mean() + partner_action.abs().mean()
        )
    else:
        aspirational_pair_loss = action_loss.new_zeros(())
        aspirational_rank_loss = action_loss.new_zeros(())
        aspirational_action_abs = action_loss.new_zeros(())
        progress_gap = action_loss.new_zeros(1)
    loss = (
        args.action_coef * action_loss
        + args.aspirational_action_coef * float(train_aspiration) * aspirational_pair_loss
        + args.aspirational_rank_coef * float(train_aspiration) * aspirational_rank_loss
    ...
    with torch.no_grad():
        ...
        # External validation only: reward never participates in selection,
        # loss construction, or gradients.
        diagnostic_reward = as_tensor(batch["diagnostic_reward"], device)
        centered_progress = aspirational_progress - aspirational_progress.mean()
        centered_reward = diagnostic_reward - diagnostic_reward.mean()
        progress_reward_correlation = (
            (centered_progress * centered_reward).mean()
            / (
                centered_progress.square().mean().sqrt()
    ...
    return {
        ...
        "follower/aspirational_action_loss": aspirational_pair_loss.item(),
        "follower/aspirational_pair_action_loss": aspirational_pair_loss.item(),
        "follower/aspirational_rank_loss": aspirational_rank_loss.item(),
        "follower/aspirational_pair_progress_gap": progress_gap.mean().item(),
        "follower/aspirational_action_abs_mean": aspirational_action_abs.item(),
        "follower/occupancy_loss": occupancy_loss.item(),
        "follower/aspirational_match_fraction": valid_match.float().mean().item(),
        ...
        "diagnostics/aspirational_progress_reward_correlation": progress_reward_correlation.item(),
            ...
            for _ in range(args.follower_updates_per_iteration):
                ...
                metrics.append(
                    train_follower_step(
                        ...
                        train_aspiration=(
                            global_step
                            >= args.random_warmup_steps + args.num_envs * args.num_steps
                        ),
                    )
                )
        if global_step >= args.random_warmup_steps:
