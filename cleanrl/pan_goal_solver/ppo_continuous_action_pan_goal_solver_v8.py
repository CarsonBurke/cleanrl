# Pan Goal Solver v8: calibrated one-step MSE descent toward an evolving ideal.
#
...
class Args:
    ...
    goal_descent_coef: float = 0.5
    goal_descent_start: int = 100_000
    goal_descent_ramp_steps: int = 200_000
...
def train_follower_step(
    agent, optimizer, batch, args, device, rng, goal_descent_weight=1.0
):
    obs = as_tensor(batch["obs"], device)
    incoming = as_tensor(batch["incoming"], device)
    next_obs = as_tensor(batch["next_obs"], device)
    ...
    goal_descent_loss = (
        predicted_distance / current_distance.clamp_min(1e-3)
    ).mean()
    loss = (
        args.action_coef * action_loss
        + args.occupancy_coef * occupancy_loss
        + args.goal_descent_coef * goal_descent_weight * goal_descent_loss
    )
    optimizer.zero_grad(set_to_none=True)
    loss.backward()
    for parameter in transition_parameters:
    ...
    return {
        ...
        "follower/goal_descent_weight": float(goal_descent_weight),
...
def main():
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        if global_step >= args.follower_warmup_steps:
            goal_descent_weight = float(
                np.clip(
                    (global_step - args.goal_descent_start)
                    / max(1, args.goal_descent_ramp_steps),
                    0.0,
                    1.0,
                )
            )
            for _ in range(args.follower_updates_per_iteration):
                ...
                metrics.append(
                    train_follower_step(
                        ...
                        device,
                        rng,
                        goal_descent_weight=goal_descent_weight,
                    )
                )
