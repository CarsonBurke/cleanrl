# Pan Goal Solver v22: goal-diverse collection — the identifiability fix.
#
# v19, v20, AND v21 (waypoint successor targets) all measured the goal
# channel dead (goal_removal_action_mse 0.0002-0.001, frozen_policy_mse flat
# ~2.3). Diagnosis: this is a DATA problem no architecture can fix. The
# behavior mixture carries almost no conditioning signal — half the replay
# is a scripted Fourier explorer whose future is screened off by the
# action-history-bearing belief, and the policy half has only ever been
# commanded ONE fixed goal, so given the belief its action variation is
# noise. I(action; goal | belief) ~ 0 in the data, so hindsight regression
# has nothing to extract. Pan-1 never faces this: its data is goal-directed
# human play. The GCSL loop needs goal-DIVERSE collection to bootstrap.
#
# v22 changes:
# 1. Per-episode commanded goals for policy envs, drawn from a mixture:
#    replay-achieved regimes (50%), the unscaled anchor (25%), the
#    velocity-scaled aspiration (25%). Raw goal histories are stored and
#    re-encoded online every iteration (fresh coordinates, no snapshots).
#    Any goal-dependence the policy exhibits now propagates into the data,
#    which hindsight training then amplifies — the standard GCSL bootstrap.
# 2. Achieved-goal reaching eval with a shuffle control: command each eval
#    env a random replay-achieved regime and measure whether the rollout's
#    goal latent stays closer to its own command than to another env's
#    (servo evidence); a bias-token follower shows no matched/shuffled gap.
#    This is the follower's unit test, gating all aspiration work.
# 3. The successor is conditioned on the waypoint horizon j (v21 omitted
#    it, so S's optimum blurred phase over 1-3 gait cycles).
# 4. Composed loss detached through S (restores v19 factorization
#    semantics; conditioning now comes from data, not gradient plumbing).
#
# ---- v19 header (architecture unchanged) ----
...
class Args:
    ...
    # Waypoint horizon: S predicts a frame j steps ahead (j uniform in
    # [1, min(hindsight offset, this cap)]) instead of one step, so the
    # target's goal-dependence is ~j-fold stronger than the one-step
    # conditional, which is nearly goal-blind under the behavior mixture.
    waypoint_horizon: int = 16
    waypoint_inverse_coef: float = 1.0
    # Goal-diverse collection: every policy episode is commanded a goal drawn
    # from this mixture (achieved = a random replay regime re-encoded online;
    # base = the unscaled anchor; remainder = the velocity-scaled aspiration).
    # Diverse commands are what make I(action; goal | belief) > 0 in the
    # collected data — the signal every hindsight loss extracts.
    goal_mix_achieved: float = 0.5
    goal_mix_base: float = 0.25
...
class DirectGoalFollower(nn.Module):
    ...
    def __init__(self, observation_tokens, belief_dim, goal_dim, act_dim, hidden):
        ...
        # +1 input: the normalized waypoint horizon j / horizon_cap. Without
        # it (v21) S's optimum is the mean frame over j in [1, 16] — one to
        # three full gait cycles — which collapses phase.
        self.successor_net = nn.Sequential(
            nn.RMSNorm(belief_dim + goal_dim + 1),
            xavier_linear(nn.Linear(belief_dim + goal_dim + 1, hidden)),
            nn.SiLU(),
    ...
    def desired_successor(self, belief, current_frame, current_goal, desired_goal, horizon):
        """Predict the frame summary j steps ahead en route to the goal
        (horizon = j / horizon_cap in (0, 1]). The goal conditions in
        goal-latent space; the prediction lives in frame space, where env
        steps carry full-strength action information (the goal latent is
        deliberately phase-robust and near one-step-invariant, so it cannot
        serve as the intermediate)."""
        features = self.goal_features(current_goal, desired_goal)
        return current_frame + self.successor_net(
            torch.cat([belief, features, horizon], dim=-1)
        )
    ...
    def action(self, belief, current_frame, current_goal, desired_goal, horizon=None):
        if horizon is None:
            # Act time plots the full-horizon waypoint: the strongest
            # goal-dependence S was trained with.
            horizon = belief.new_ones(belief.shape[0], 1)
        successor = self.desired_successor(
            belief, current_frame, current_goal, desired_goal, horizon
        )
...
class VectorReplayBuffer:
    ...
    def sample_goal_histories(self, count, history, rng):
        """Random achieved goal histories: raw obs windows re-encodable online
        each iteration. Uniform over the buffer, so explorer and policy
        regimes are both commanded — diversity is the point."""
        accepted = []
        while sum(len(x[0]) for x in accepted) < count:
            env, end = self._sample_base(count * 2, history, 0, rng)
            valid = self._valid_end(env, end)
            accepted.append((env[valid], end[valid]))
        env = np.concatenate([x[0] for x in accepted])[:count]
        end = np.concatenate([x[1] for x in accepted])[:count]
        return self._padded_history(self.obs, env, end, history)
    ...
    def sample_hindsight(
        ...
        # Waypoint: an intermediate frame on the actual path to the hindsight
        # goal. end and future are same-episode and contiguous in the ring,
        # so every index between them is valid.
        span = np.minimum(future - end, waypoint_horizon)
        waypoint = end + rng.integers(1, span + 1)
        occupancy = []
        ...
        return {
            ...
            "incoming": self._padded_history(self.incoming_action, env, end, history, zero_pad=True),
            "next_obs": self._padded_history(self.obs, env, end + 1, history),
            "waypoint_obs": self._padded_history(self.obs, env, waypoint, history),
            ...
            "waypoint_offset": (waypoint - end).astype(np.float32),
...
def train_follower_step(agent, optimizer, batch, args, device):
    ...
    action = as_tensor(batch["action"], device)
    waypoint_obs = as_tensor(batch["waypoint_obs"], device)
    with torch.no_grad():
        all_tokens, all_goals = agent.encode_goal(
            torch.cat(
                [obs, next_obs, waypoint_obs, future_obs, occupancy_future_obs],
                dim=0,
            )
        )
        current_goal, next_goal, _, future_goal, occupancy_future_goal = (
            all_goals.chunk(5)
        )
        ...
        # of the waypoint history, f_{t+j} on the actual path to the goal.
        all_frames = all_tokens.mean(dim=-2)[:, -1]
        current_frame, next_frame, waypoint_frame, _, _ = all_frames.chunk(5)
    ...
    # followed while en route, at the sampled waypoint horizon j (given to S
    # so distinct horizons are not averaged into a phase-free smear).
    # Conditioning on the current belief makes this a feedback law: wherever
    # the agent is, S re-plots the waypoint.
    horizon = as_tensor(batch["waypoint_offset"], device).unsqueeze(-1) / max(
        args.waypoint_horizon, 1
    )
    successor_prediction = agent.follower.desired_successor(
        belief, current_frame, current_goal, future_goal, horizon
    )
    successor_loss = F.mse_loss(successor_prediction, waypoint_frame)
    # Inverse dynamics, two horizons: the one-step term is dense and exact;
    # the waypoint term is first-action-toward-a-far-frame — the first-order
    # hindsight conditioning that v16/v17 had in goal space, now in frame
    # space where I also serves as the act-time servo.
    ...
    waypoint_prediction = agent.follower.inverse_action(
        belief, current_frame, waypoint_frame
    )
    waypoint_inverse_loss = F.mse_loss(waypoint_prediction, action)
    # Composed act-time path on the same hindsight tuples, DETACHED through
    # the successor: S keeps waypoint-prediction semantics (v20/v21 measured
    # that attaching cannot conjure a conditioning signal the data lacks,
    # and it lets S degenerate into an uninterpretable feature bottleneck).
    # The composed term trains I on S's actual output distribution.
    composed_prediction = agent.follower.inverse_action(
        belief, current_frame, successor_prediction.detach()
    )
    ...
    loss = (
        ...
        + args.waypoint_inverse_coef * waypoint_inverse_loss
    ...
    return {
        ...
        "follower/successor_identity_mse": F.mse_loss(current_frame, waypoint_frame).item(),
        "follower/inverse_action_mse": inverse_loss.item(),
        "follower/waypoint_inverse_mse": waypoint_inverse_loss.item(),
...
@torch.no_grad()
def evaluate_goal_reaching(agent, args, device, run_name, goals):
    """The follower's unit test: command each eval env a replay-ACHIEVED
    regime latent and measure whether the rollout's own goal latent stays
    closer to its command (matched) than to another env's command (shuffled
    control). Genuine servoing requires matched < shuffled; a bias-token
    follower — actions depend on the goal without pursuing it — shows no
    gap. Distances accumulate over each env's first episode only."""
    envs = gym.vector.SyncVectorEnv(
    ...
    shuffled_goals = goals.roll(1, dims=0)
    ...
    returns = np.zeros(args.eval_envs)
    while active.any():
        action, current_goal, _ = agent.act(
            as_tensor(obs_history, device), as_tensor(incoming_history, device), goals
        )
        matched = (current_goal - goals).square().mean(-1).cpu().numpy()
        shuffled = (current_goal - shuffled_goals).square().mean(-1).cpu().numpy()
        ...
        if "final_info" in infos:
            for env_index, info in enumerate(infos["final_info"]):
                if info and "episode" in info and active[env_index]:
                    returns[env_index] = float(info["episode"]["r"])
        active &= ~done
        next_obs = np.asarray(next_obs, np.float32)
        obs_history = np.roll(obs_history, -1, axis=1)
        obs_history[:, -1] = next_obs
        ...
        if done.any():
            ...
            incoming_history[done] = 0
    envs.close()
    matched_mse = float((matched_sum / steps).mean())
    shuffled_mse = float((shuffled_sum / steps).mean())
    return matched_mse, shuffled_mse, float(returns.mean())
...
def main():
    ...
    if not (
        0 <= args.goal_mix_achieved
        and 0 <= args.goal_mix_base
        and args.goal_mix_achieved + args.goal_mix_base <= 1
    ):
        raise ValueError("goal mixture probabilities must be a sub-distribution")
    if args.eval_envs < 2:
    ...
    # Goal-diverse collection state. Kinds: -1 unset, 0 achieved replay
    # regime (raw history stored, re-encoded online), 1 unscaled anchor,
    # 2 velocity-scaled aspiration. Explorer envs carry goals too (unused
    # for acting) so an explorer->policy reset needs no special case.
    env_goal_kind = np.full(args.num_envs, -1, np.int64)
    env_goal_history = np.zeros((args.num_envs, args.history, obs_dim), np.float32)
    env_goals = torch.zeros(args.num_envs, args.latent_dim, device=device)

    @torch.no_grad()
    def refresh_env_goals():
        """Re-encode every env's commanded goal with the current encoder
        (fresh online coordinates each iteration; fixed batch shape so
        compiled encoders see one shape). Anchor kinds read the latents
        computed from the current anchor this iteration."""
        _, achieved_goals = agent.encode_goal(as_tensor(env_goal_history, device))
        kinds = torch.as_tensor(env_goal_kind, device=device)
        env_goals.copy_(achieved_goals)
        if base_goal is not None:
            env_goals[kinds == 1] = base_goal
            env_goals[kinds == 2] = commanded_goal

    def resample_env_goals(indices):
        if not len(indices):
            return
        draw = rng.random(len(indices))
        kinds = np.where(
            draw < args.goal_mix_achieved,
            0,
            np.where(draw < args.goal_mix_achieved + args.goal_mix_base, 1, 2),
        )
        if anchor_history is None:
            kinds[:] = 0
        env_goal_kind[indices] = kinds
        env_goal_history[indices] = replay.sample_goal_histories(
            len(indices), args.history, rng
        )
        refresh_env_goals()
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        step_kind = np.zeros((rollout_steps, args.num_envs), np.int64)
        policy_action_abs = []
        for step in range(rollout_steps):
            ...
            with torch.no_grad():
                ...
                policy_action, current_goal, step_goal = agent.act(
                    obs_tensor, incoming_tensor, env_goals
                )
                step_kind[step] = env_goal_kind
                if step > 0:
            ...
            if done.any():
                ...
                elif replay.total_steps >= args.history:
                    episode_explorer[done] = rng.random(done.sum()) < args.explorer_fraction
                    # Fresh commanded goal for every new episode: the
                    # diversity that makes hindsight conditioning
                    # identifiable in the collected data.
                    resample_env_goals(np.flatnonzero(done))
                else:
                    # Replay too small to sample a goal: stay explorer so no
                    # policy env ever acts on an unset (zero) goal.
                    episode_explorer[done] = True
        ...
        # Per-iteration goal refresh: initialize any still-unset env goals
        # once the policy phase begins, and re-encode every commanded goal in
        # the fresh coordinates (the anchor latents above were just redone).
        if global_step >= args.random_warmup_steps and replay.total_steps >= args.history:
            unset = np.flatnonzero(env_goal_kind < 0)
            if len(unset):
                resample_env_goals(unset)
            else:
                refresh_env_goals()
        if global_step >= args.random_warmup_steps and rollout_steps > 1:
            ...
            # Pursuit error split by commanded-goal kind: achieved goals are
            # on-manifold and must become reachable first; the aspiration is
            # extrapolated and may carry an irreducible floor.
            for kind_value, kind_name in ((0, "achieved"), (1, "base"), (2, "aspiration")):
                kind_mask = policy_transition & (step_kind == kind_value)
                if kind_mask.any():
                    writer.add_scalar(
                        f"goal/pursuit_mse_{kind_name}",
                        float(goal_mse_before[kind_mask].mean()),
                        global_step,
                    )
            if progress.size:
        ...
        if args.eval_interval > 0 and global_step >= next_eval_step and global_step >= args.random_warmup_steps:
            ...
            # Achieved-goal reaching: command random replay regimes and test
            # for a matched-vs-shuffled servo gap. This gates all aspiration
            # claims — a follower that cannot reach demonstrated regimes has
            # nothing to extrapolate from.
            if replay.total_steps >= args.history:
                reach_histories = replay.sample_goal_histories(
                    args.eval_envs, args.history, rng
                )
                with torch.no_grad():
                    _, reach_goals = agent.encode_goal(
                        as_tensor(reach_histories, device)
                    )
                matched_mse, shuffled_mse, reach_return = evaluate_goal_reaching(
                    agent, args, device, run_name, reach_goals
                )
                writer.add_scalar("evaluation/reach_matched_mse", matched_mse, global_step)
                writer.add_scalar("evaluation/reach_shuffled_mse", shuffled_mse, global_step)
                writer.add_scalar(
                    "evaluation/reach_servo_gap", shuffled_mse - matched_mse, global_step
                )
                writer.add_scalar(
                    "evaluation/reach_episodic_return", reach_return, global_step
                )
            next_eval_step += args.eval_interval
        print(
