# Pan Goal Solver v14: velocity-scaled replay-anchor goal (nonparametric proposer).
#
# A reward-free LeJEPA learns observation tokens and temporal control belief.
# Pan hindsight and occupancy ground the follower in real trajectories. v11's
# learned goal head trained by ascent through a learned reward head is removed:
# that gradient path invites reward-model exploitation (off-manifold latents the
# frozen head scores optimistically). The proposer is now programmatic: track
# the replay state with the best sustained windowed reward rate, take its raw
# observation history, scale every velocity coordinate by alpha > 1 (the same
# gait, time-compressed), and encode it with the EMA target encoder as the one
# commanded goal latent. The anchor ratchets on new bests, so the goal stays a
# fixed velocity margin ahead of demonstrated capability: an auto-curriculum
# with directed pressure that replay-argmax alone lacks. Reward touches only
# anchor selection and a detached windowed reward-rate head kept for goal
# semantics diagnostics; world model and follower stay reward-free.
# There is no PPO, action critic, policy gradient, search, MPC, or rollout.
# Proposer contract (FAMILY.md): input = replay rewards + one stored raw obs
# history; objective = argmax sustained rate, aspiration variable = velocity
# scale alpha; safeguards = on-manifold anchor, monotonicity sweep diagnostics.
# Hypothesis: obs-space velocity scaling of a real gait is a physically
# grounded aspiration the encoder extrapolates smoothly, and the follower's
# direction-only far-goal control turns it into sustained speed pressure.
...
class Args:
    ...
    # Velocity aspiration: commanded goal = best anchor with velocities scaled
    # by this factor, so the target stays a fixed margin ahead of best-achieved.
    velocity_scale: float = 1.3
...
class PanGoalSolver(nn.Module):
    ...
    def encode_scaled_anchor(self, anchor_history, velocity_dims, scales, device):
        ...
        _, goals = self.target_encode(scaled)
        return goals
...
ALPHA_SWEEP = (1.0, 1.1, 1.3, 1.5, 2.0)
...
def main():
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        # the EMA target encoder is frozen until the next block, so these
        # latents are exact for the upcoming rollout, the diagnostics below,
        # and eval alike (no encoder-snapshot mixing).
        ...
        if anchor_history is not None:
            anchor_goals = agent.encode_scaled_anchor(
                anchor_history, velocity_dims, (1.0, args.velocity_scale), device
            )
            ...
            writer.add_scalar(
                ...
                float(anchor_history[:, -velocity_dims].mean()),
                global_step,
            )
        if anchor_history is not None:
            with torch.no_grad():
