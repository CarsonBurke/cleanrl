# Pan Goal Solver v16: v14 with the three failures measured at 1.1M steps fixed.
#
# v14 evidence: (1) the Fourier explorer (periods 20-80 steps) cannot excite
# HalfCheetah's ~5-15 step gait frequency, so no fast window was ever
# experienced (lifetime best sustained rate 0.395; a scripted 6-30 step-period
# explorer reaches ~0.95 in 30k steps). (2) Multiplicative velocity aspiration
# is a no-op on a near-zero-velocity anchor (aspiration action MSE ~80x below
# goal action power). (3) The linear direction-Jacobian follower shows 50%
# frozen-goal progress even toward achieved in-distribution latents.
# v16 changes, one per failure: gait-frequency explorer periods (6-30);
# additive forward-velocity aspiration (uniform scale =
# clip((v_fwd + delta) / v_fwd, 1, cap), interpolating additive pressure from
# slow anchors to multiplicative near fast ones); and a nonlinear follower
# head, MLP over (belief, bounded goal delta), replacing the bilinear servo.
# Proposer stays nonparametric (best sustained-rate replay anchor, EMA target
# encoding); reward touches only anchor selection and the detached rate probe.
# There is no PPO, action critic, policy gradient, search, MPC, or rollout.
# Hypothesis: with gait-frequency exploration seeding real fast windows,
# meaningful aspiration pressure from any anchor speed, and a follower head
# expressive enough to track commanded latents, the ratchet finally engages.
