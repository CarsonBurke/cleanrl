...
#     their own per-step masked SIGReg term (lewm/ar_sigreg), scaled under
#     lewm_ar_loss_coef. FULL token set: fed-back obs tokens AND the semantic
...
#     the projected scalar label, continuation BCE. Weighted by lewm_ar_loss_coef
...
class Args:
    ...
    lewm_ar_loss_coef: float = 0.5  # weight on the autoregressive-rollout training losses
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        wm_sigreg_losses = []
        wm_ar_sigreg_losses = []
        wm_target_reward_diag_losses = []
        wm_target_termination_diag_losses = []
        wm_grad_norms = []
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # v110: AUTOREGRESSIVE rollout along the REAL action sequence as
                    # a training path, DAgger-style, NO BPTT: the seed summary and
                    # every fed-back prediction are detached, so step h trains only
                    # its own predictor application + outcome decode on the corrupted
                    # history its own consumption produces. The per-step latent MSE
                    # keeps its ATTACHED real target (encoder feedback, as in the
                    # teacher-forced path); reward CE vs projected labels and
                    # continuation BCE are unchanged. The same pass still feeds the
                    # per-horizon drift probes (train metrics, not held-out).
                    ar_hist_lat = [initial_summary[:, obs_start:].detach()]
                    ar_hist_act = []
                    ar_pred_summaries = []
                    ar_denom = latent_weight.sum().clamp_min(1.0)
                    ar_latent_loss = torch.zeros((), device=device)
                    ar_reward_loss = torch.zeros((), device=device)
                    ar_termination_loss = torch.zeros((), device=device)
                    for h in range(horizon):
                        ...
                        ar_reward_ce = -(
                            reward_target_probs[:, h].detach()
                            * torch.log_softmax(ar_reward_logits, dim=-1)
                        ).sum(dim=-1)
                        ...
                        ar_latent_loss = ar_latent_loss + (ar_latent_mse * w_h).sum() / ar_denom
                        ar_reward_loss = ar_reward_loss + (ar_reward_ce * w_h).sum() / ar_denom
                        ar_termination_loss = (
                            ar_termination_loss + (ar_continuation_bce * w_h).sum() / ar_denom
                        )
                        with torch.no_grad():
                            ar_reward_scalar = reward_support.to_scalar(ar_reward_logits)
                            ar_reward_error = ar_reward_scalar - future_rewards[:, h]
                            wm_imagine_latent_mse_sum[h] += (ar_latent_mse.detach() * w_h).sum()
                            ...
                            wm_imagine_continuation_bce_sum[h] += (
                                ar_continuation_bce.detach() * w_h
                            ).sum()
                            wm_imagine_probe_weight[h] += w_h.sum()
                        ar_pred_summaries.append(ar_next_summary[:, obs_start:])
                        ar_hist_lat.append(ar_next_summary[:, obs_start:].detach())
                    # v108: target-side outcome decode is diagnostic only. The
                    # imagination simulator consumes outcomes decoded from predicted
                    # summaries, so reward/continuation gradients should train that
                    # path rather than an easier real-target-summary readout.
                    with torch.no_grad():
                    ...
                    # v110: SIGReg the AR-predicted summaries too -- the population
                    # imagination actually consumes, which otherwise gets isotropy
                    # pressure only secondhand (MSE toward SIGReg'd encoder targets).
                    # v111: OBS TOKENS ONLY, matching the encoder-side pool. SIGReg on
                    # the semantic outcome tokens fought their CE/BCE heads (the
                    # continuation token wants a constant -- zero-variance -- embedding
                    # on a no-termination env; forcing it Gaussian injects logit
                    # noise). Attached tensors (pre-detach), under lewm_ar_loss_coef.
                    ar_sigreg_latents = torch.stack(ar_pred_summaries, dim=1)
                    wm_ar_sigreg_loss = masked_token_sigreg(ar_sigreg_latents, latent_weight)
                    wm_loss = (
                        ...
                        + args.lewm_ar_loss_coef
                        * (
                            args.lewm_loss_coef * ar_latent_loss
                            + args.lewm_reward_loss_coef * ar_reward_loss
                            + args.lewm_termination_loss_coef * ar_termination_loss
                            + args.lewm_sigreg_coef * wm_ar_sigreg_loss
                        )
                    )
                    if wm_accum_count == 0:
                    ...
                    wm_ar_latent_losses.append(ar_latent_loss.item())
                    wm_ar_reward_losses.append(ar_reward_loss.item())
                    wm_target_reward_diag_losses.append(target_reward_loss.item())
                    wm_target_termination_diag_losses.append(target_termination_loss.item())
                    wm_sigreg_losses.append(wm_sigreg_loss.item())
                    wm_ar_sigreg_losses.append(wm_ar_sigreg_loss.item())
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/ar_latent_mse_loss", float(np.mean(wm_ar_latent_losses)), global_step)
                writer.add_scalar("lewm/ar_reward_ce", float(np.mean(wm_ar_reward_losses)), global_step)
                writer.add_scalar("lewm/target_reward_ce_diag", float(np.mean(wm_target_reward_diag_losses)), global_step)
                writer.add_scalar("lewm/target_continuation_bce_diag", float(np.mean(wm_target_termination_diag_losses)), global_step)
                writer.add_scalar("lewm/obs_sigreg", float(np.mean(wm_sigreg_losses)), global_step)
                writer.add_scalar("lewm/ar_sigreg", float(np.mean(wm_ar_sigreg_losses)), global_step)
