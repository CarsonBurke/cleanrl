# SPO-asym + transformer + HL-Gauss + latent_2sa + separate actor/critic clip.
#
# Architecture diff vs hlgauss_latent_2sa_v1: ONE addition only — the single
# global clip
#     clip_grad_norm_(agent.parameters(), 0.5)
# is replaced by an orbit-wars-style separate-clip pattern:
#     - actor heads (actor_mean_head + sde_logstd_head): only see pg + ent grad
#     - critic head (critic_head): only sees vf grad
#     - shared trunk (obs_embed, embed_norm, latent_tokens, seeds, sa_blocks×2,
#                     pma_block, final_norm): receives gradients from BOTH
# Two backwards (retain_graph=True on the first), each followed by an
# independent clip on (head + shared) for that loss; the actor's shared-trunk
# grads are cloned and re-added after the critic's pass so the trunk update
# is the *sum of two pre-clipped* gradient contributions rather than one
# global-clipped sum. Pattern adapted from
# orbit-wars/src/owars/training/ppo.py:_backward_actor_critic_with_separate_clips.
#
# Hypothesis. The transformer trunk is exactly the case where this should
# matter most: a shared encoder feeds 3 distinct readouts, the HL-Gauss
# CE loss has a fundamentally different scale from the SPO policy loss,
# and a single global clip makes one term governor for the trunk's step
# size. With separate clips the trunk update is the sum of two clipped
# contributions — neither term can starve the other.
#
# Architecture diff vs hlgauss_latent_v1 (carried from latent_2sa_v1):
#   - Two SA blocks stacked sequentially over [latent_3 | obs] before PMA
#     (vs. one SA block in v1). Same RoPE cache reused across both blocks.
#     NUM_BLOCKS bumped to 3 (2 SA + 1 PMA) so output-projection init scale
#     1/sqrt(2*N) accounts for the deeper residual stream.
#
# Architecture diff vs hlgauss_v1 (carried from latent_v1):
...
NUM_HEADS = 4
FFN_MULT = 2
NUM_BLOCKS = 3  # two SA + one PMA — used for output-projection init scale
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        init_scale = 1.0 / (2 * NUM_BLOCKS) ** 0.5  # 1/sqrt(6) ≈ 0.408
        self.sa_blocks = nn.ModuleList(
            [SABlock(MODEL_DIM, NUM_HEADS, FFN_MULT, init_scale) for _ in range(2)]
        )
    ...
    def _encode(self, x):
        ...
        Q over full SA output K/V) → final_norm. Returns the three PMA-seed
        features (actor, sde, critic) each shape (B, MODEL_DIM)."""
        B = x.shape[0]
        ...
        for sa in self.sa_blocks:
            sa_tokens = sa(sa_tokens, self.rope_cos, self.rope_sin)
...
def _grad_clip_groups(agent):
    """Partition agent.parameters() into (actor_head, critic_head, shared).
       Iterating named_parameters() once means each Parameter lands in
       exactly one bucket. Cached on the agent.
    """
    cache = getattr(agent, "_sepclip_groups", None)
    if cache is not None:
        return cache
    actor, critic, shared = [], [], []
    for name, p in agent.named_parameters():
        if name.startswith("actor_mean_head") or name.startswith("sde_logstd_head"):
            actor.append(p)
    ...
    agent._sepclip_groups = cache
    return cache


def _clone_grads(params):
    """Deep-clone .grad of each param that has one, returning [(p, g_clone), ...]."""
    return [(p, p.grad.detach().clone()) for p in params if p.grad is not None]
...
def _sepclip_backward(agent, actor_loss, critic_loss, max_norm):
    """orbit-wars two-backward: actor (with retain_graph) → clip on
    (actor_heads + shared) → save + clear; critic → clip on
    (critic_head + shared) → re-add saved actor grads. Returns
    (actor_norm_pre_clip, critic_norm_pre_clip).
    """
    actor_heads, critic_head, shared = _grad_clip_groups(agent)
    actor_params = actor_heads + shared
    critic_params = critic_head + shared

    actor_loss.backward(retain_graph=True)
    actor_norm = nn.utils.clip_grad_norm_(actor_params, max_norm)
    saved = _clone_grads(actor_params)
    _clear_grads(agent.parameters())

    critic_loss.backward()
    critic_norm = nn.utils.clip_grad_norm_(critic_params, max_norm)
    _add_grads(saved)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # orbit-wars-style separate actor/critic clipping. The trunk
                # is shared across both losses; the actor backward is retained
                # so we can clip actor+shared, save those grads, then run the
                # critic backward, clip critic+shared, and re-add the saved
                # actor grads. Trunk update = sum of two independently-clipped
                # contributions instead of a single global-clipped sum.
                optimizer.zero_grad()
                actor_grad_norm, critic_grad_norm = _sepclip_backward(
                    agent, actor_loss, critic_loss, args.max_grad_norm,
                )
                optimizer.step()
        ...
        # Sliced to a single minibatch_size to avoid a large allocation
        # right after the sepclip update epoch (whose retain_graph +
        # clone-grad path already pushes memory pressure higher than the
        # base 2sa variant). On a driver with a flaky NVML, the full-batch
        # forward intermittently triggers a CUDACachingAllocator NVML init.
        with torch.no_grad():
            _, std_diag = agent.get_action_distribution(b_obs[: args.minibatch_size])
