# Predict-then-check Adam on base PPO, v1.
# pred = eta * <bias-corrected Adam momentum, Adam direction>. After stepping,
# re-evaluate the SAME minibatch with frozen PPO targets, then EMA(actual/pred).
# rho>0.75: eta*=1.05; rho<0.25: eta*=0.8. No annealing, LR bounds or rollback.
# Proportional layer attribution cancels: actual*(pred_l/sum(pred))/pred_l
# equals actual/sum(pred), so identical starting LRs need one global controller.
# Hypothesis: model agreement supplies the schedule. Risk: clipped, historical
# momentum is not the current loss gradient; its calibration and EMA lag can
# cause runaway growth or decay. This is not an environment world model.
# The check costs an extra no-grad forward; cross-minibatch loss changes are NOT
# valid measurements. The EMA starts from the first valid observation; no
# fabricated zero-rho warmup. Zero predicted change leaves the controller alone.
import os
import random
import time
...
class Args:
    ...
    trust_beta: float = 0.9
    """decay for the bias-corrected EMA of actual/predicted same-batch decrease"""
    num_envs: int = 32
...
def adam_predict_update(params, grads, moments, seconds, steps, lr, prediction, applied_lr, beta1, beta2, eps):
    contributions = []
    applied_lr.copy_(lr)
    for parameter, gradient, moment, second, step in zip(params, grads, moments, seconds, steps):
        step.add_(1)
        moment.lerp_(gradient, 1.0 - beta1)
        second.mul_(beta2).addcmul_(gradient, gradient, value=1.0 - beta2)
        mean = moment / (1.0 - beta1 ** step)
        direction = mean / ((second / (1.0 - beta2 ** step)).sqrt() + eps)
        contributions.append((mean * direction).sum())
        parameter.add_(-lr * direction)
    prediction.copy_(lr * torch.stack(contributions).sum())


def adapt_learning_rate(lr, ema_sum, ema_weight, before, after, prediction, applied_lr, beta):
    """Check the just-applied step on its SAME fixed objective; no step rejection."""
    actual = before - after
    positive_prediction = prediction > 0
    ratio = actual / torch.where(positive_prediction, prediction, torch.ones_like(prediction))
    valid = positive_prediction & torch.isfinite(prediction) & torch.isfinite(ratio)
    ema_sum.copy_(torch.where(valid, beta * ema_sum + (1.0 - beta) * ratio, ema_sum))
    ema_weight.copy_(torch.where(valid, beta * ema_weight + (1.0 - beta), ema_weight))
    rho = ema_sum / torch.where(ema_weight > 0, ema_weight, torch.ones_like(ema_weight))
    multiplier = torch.where(rho > 0.75, 1.05, torch.where(rho < 0.25, 0.8, 1.0))
    multiplier = torch.where(valid, multiplier, torch.ones_like(multiplier))
    lr.mul_(multiplier)
    # Preserve nonfinite measurements for the trainer's existing fail-fast check.
    reported_ratio = torch.where(positive_prediction, ratio, torch.zeros_like(ratio))
    return torch.stack((prediction, actual, reported_ratio, rho, applied_lr, lr,
                        multiplier, valid.float(), after))


class PredictCheckAdam(optim.Optimizer):
    """Default Adam with a single shared, prediction-controlled learning rate."""

    def __init__(self, params, lr=3e-4, betas=(0.9, 0.999), eps=1e-5, trust_beta=0.9, compile=True):
        params = list(params)
        if not params or not np.isfinite(lr) or lr <= 0 or not np.isfinite(eps) or eps <= 0:
            raise ValueError("PredictCheckAdam requires parameters and positive finite LR and epsilon")
        if not all(0 <= beta < 1 for beta in (*betas, trust_beta)):
            raise ValueError("Adam and trust EMA decays must be in [0, 1)")
        device = params[0].device
        if device.type != "cuda" or any(parameter.device != device for parameter in params):
            raise ValueError("PredictCheckAdam requires parameters on one CUDA device")
        super().__init__(params, dict(lr=torch.tensor(lr, device=device), betas=betas, eps=eps,
                                     trust_beta=trust_beta, rho_sum=torch.zeros((), device=device),
                                     rho_weight=torch.zeros((), device=device)))
        self.predicted_decrease = torch.zeros((), device=device)
        self.applied_lr = torch.zeros((), device=device)
        self._update = (
            torch.compile(adam_predict_update, fullgraph=True, dynamic=False, options={"triton.cudagraphs": False})
            if compile else adam_predict_update
        )
        self._adapt = (
            torch.compile(adapt_learning_rate, fullgraph=True, dynamic=False, options={"triton.cudagraphs": False})
            if compile else adapt_learning_rate
        )
    ...
    def step(self, closure: Callable[[], float] | None = None) -> float | None:
        ...
        if closure is not None:
            with torch.enable_grad():
                loss = closure()
        # Layerwise proportional attribution cancels to one global ratio. The
        # trainer therefore owns one flat parameter group, not fake layer models.
        group = self.param_groups[0]
        params, grads, moments, seconds, steps = [], [], [], [], []
        for parameter in group["params"]:
            if parameter.grad is None:
                continue
            if parameter.grad.is_sparse:
                raise RuntimeError("PredictCheckAdam requires dense gradients")
        ...
        if params:
            ...
            self._update(params, grads, moments, seconds, steps, group["lr"], self.predicted_decrease,
                         self.applied_lr, beta1, beta2, group["eps"])
        else:
            self.predicted_decrease.zero_()
            self.applied_lr.copy_(group["lr"])
        return loss

    @torch.no_grad()
    def observe_loss(self, before, after):
        """Call exactly once after step, with the same minibatch and frozen targets."""
        group = self.param_groups[0]
        return self._adapt(group["lr"], group["rho_sum"], group["rho_weight"], before, after,
                           self.predicted_decrease, self.applied_lr, group["trust_beta"])
...
def main():
    ...
    try:
        ...
        optimizer = PredictCheckAdam(agent.parameters(), lr=args.learning_rate,
                                     trust_beta=args.trust_beta, compile=args.compile)
        ...
        def check_model(observations, native, old_logprobs, advantages, returns, old_values):
            return ppo_loss(agent, observations, native, old_logprobs, advantages, returns, old_values, args)[0]
        ...
        prediction_metrics = torch.empty((max_updates, 9), device=device)
        timer = PhaseTimer()
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        optimizer.step()
                        with torch.no_grad():
                            after_loss = check_model(*minibatch)
                            prediction_metrics[updates].copy_(optimizer.observe_loss(before_loss, after_loss))
                        updates += 1
            ...
            logged = gather_metrics({
                ...
                "predict/predicted_decrease": checks[:, 0].mean(),
                "predict/actual_decrease": checks[:, 1].mean(),
                "predict/rho": checks[:, 2].mean(), "predict/rho_ema": checks[-1, 3],
                "predict/lr_used_min": checks[:, 4].min(), "predict/lr_used_max": checks[:, 4].max(),
                "predict/next_lr": checks[-1, 5],
                "predict/grow_fraction": (checks[:, 6] > 1).float().mean(),
                "predict/shrink_fraction": (checks[:, 6] < 1).float().mean(),
                "predict/valid_fraction": checks[:, 7].mean(),
                "predict/loss_increase_fraction": (checks[:, 1] < 0).float().mean(),
                "predict/post_loss_max": checks[:, 8].abs().max(),
            })
            ...
            writer.add_scalar("charts/learning_rate", logged["predict/next_lr"], global_step)
