# Peri-Derf scalar PPO, v11: replace only the v8 branch input/output RMS norms.
# Hypothesis: learned pointwise saturation controls branch outliers without
# cross-channel moments; additive streams remain unprojected, no final norms.
# Derf(x) = erf(alpha*x + shift)*weight + bias; alpha=.5, shift=0, weight=1, bias=0.
# Source: https://github.com/zlab-princeton/Derf/blob/2d068e910da7cd1938b3371f7132c42c6ea24e3d/ViT/dynamic_erf.py
# Paper: https://arxiv.org/abs/2512.10938
# One epoch, LR=.0096, reward normalization, clipped scalar MSE, no grad clipping.
import os
import random
import time
...
class Args:
    ...
    norm_kind: Literal["layer", "rms", "sphere", "derf"] = "derf"
    """Derf by default: learned erf(alpha*x+shift) with channel affine at both branch sites"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, placement="peri", norm_kind="derf", activation="stiglu"):
        super().__init__()
