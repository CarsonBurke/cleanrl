# JEPA geometry/drift v7: PPO -> actor/critic only (encoder consumers detached).
# Prediction -> current encoder/projector/predictor; future projector/encoder
# attached by default, optionally stopped AFTER projection for prediction only.
# SIGReg -> BOTH online encoder/projector branches, including with target-stop.
# Reward -> detached-input head by default; never a control/planning input.
# Rollout critic caching freezes coordinates ONLY within an update, not across
# rollouts; actor/SSL stay online. Huber = 2*smooth_l1(beta=1), otherwise v6 MSE.
import os
import random
import time
...
class LeWMBranch(nn.Module):
    ...
    def forward(self, current, following, native_actions):
        ...
        target = embeddings[1].detach() if self.prediction_target_gradient == "stopped" else embeddings[1]
        prediction_loss = prediction_objective(prediction, target, self.prediction_loss)
        # Target detachment must never detach the future SIGReg branch.
        ...
        with torch.no_grad():
            ...
            metrics = {
                ...
                "ssl/shared_prediction_raw_mse": squared_error.mean(),
                "ssl/shared_residual_gt1_fraction": tails.float().mean(),
                "ssl/shared_residual_gt1_squared_error_share":
                    (squared_error * tails).sum() / squared_error.sum().clamp_min(1e-12),
                "ssl/shared_sigreg_loss": regularization.detach(),
                "ssl/shared_projected_std": embeddings.std(dim=1, correction=0).mean(),
                "ssl/shared_backbone_std": following.std(dim=0, correction=0).mean(),
            }
...
def joint_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values,
               next_observations, rewards, args, *, critic_features=None):
...
def drift_probe_indices(batch_size, device):
    """Fixed evenly spaced rows, independent of every training random stream."""
    count = min(128, batch_size)
    return torch.arange(count, device=device) * batch_size // count
...
def main():
    ...
    try:
        ...
        def joint_model(observations, native, old_logprobs, advantages, returns, old_values,
                        next_observations, rewards, critic_features):
            return joint_loss(
        ...
        probe_indices = drift_probe_indices(args.batch_size, device)
        ...
        writer.add_text("ablation", f"{args.task_activation=}, {args.task_residual=}, "
                        ...
                        f"{args.prediction_target_gradient=}, {args.critic_feature_updates=}; fixed SSL512; "
