# HybridNorm observation-token PPO, v16.
# Shared learned token offsets start N(0,1), avoiding an all-zero RMS fixed point.
# Hypothesis: per-head Q/K/V normalization and normalized FFN identity paths
# stabilize high-LR attention PPO. Eq. 6/Algorithm 1 use z=RMS(y), x=z+gate*FFN(z),
# not RMS(y+gate*FFN(y)); no final norm.
...
class Args:
    ...
    norm_method: Literal["peri", "siamese", "span", "hybrid"] = "hybrid"
...
class Agent(nn.Module):
    ...
    def __init__(
        self, envs, *, norm_method="hybrid", token_width=32, n_tokens=4, n_heads=2, n_blocks=3
    ):
        super().__init__()
