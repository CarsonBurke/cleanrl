...
class Args:
    ...
    alpha_max: float = 1.0          # v41: upper clamp (RAW units) -- bounds the entropy contribution to the
                                    #      soft return so it stays inside the C51 support [v_min,v_max]. Monitor
                                    #      debug/edge_mass; if it climbs, lower alpha_max or widen the support.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if auto_alpha and args.soft_adv:
                ...
                # SUPPORT-SAFE SCALING. The entropy contribution to the soft VALUE is alpha_eff*E[sum_k g^k H],
                # whose LEVEL (~alpha_eff * mean discounted entropy return) must fit the fixed C51 support.
                # The discounted entropy return amplifies the per-step H by ~1/(1-gamma) (~100), so dividing by
                # the REWARD return_std (v40's bug) under-normalizes by ~25x and overflows. Instead normalize
                # by the entropy return's own LEVEL: ent_level = mean(discounted entropy return). Then the
                # entropy value level == alpha_r (<= alpha_max), bounded inside the support FOR ANY env/horizon.
                # The constant level cancels in the advantage (it is in both the return and the V_soft baseline),
                # so the per-state future-entropy EXPLORATION SIGNAL (the variation) is preserved undistorted.
                ent_ret = torch.zeros_like(rewards)
                last = torch.zeros(args.num_envs, device=device)
                for t in reversed(range(args.num_steps)):
                    nt = (1.0 - next_done) if t == args.num_steps - 1 else (1.0 - dones[t + 1])
                    ent_ret[t] = last = ent_bonus[t] + args.gamma * nt * last
                ent_level = ent_ret.mean().clamp_min(1.0)                # ~hundreds; positive (H>=0)
                alpha_eff = alpha_r / ent_level                          # entropy value level -> ~alpha_r
                soft_rewards = rewards + alpha_eff * ent_bonus
            else:
            ...
            # alpha_eff (=alpha/ent_level) + alpha_max keep the soft return inside the support; watch edge_mass.
        ...
        with torch.no_grad():
            if auto_alpha and args.soft_adv:
                ...
                ent_level_log = float(ent_level)
                alpha_eff_log = float(alpha_eff)
            else:
                ent_bonus_frac, ent_level_log, alpha_eff_log = 0.0, 0.0, 0.0
        ...
        if auto_alpha:
            ...
            writer.add_scalar("losses/alpha_eff", alpha_eff_log, global_step)               # alpha_r / ent_level
            writer.add_scalar("debug/ent_level", ent_level_log, global_step)                # mean discounted entropy return
