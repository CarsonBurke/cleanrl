# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + AWR ADVANTAGE WEIGHT v1.
#
# KEY IDEA. Advantage-Weighted Regression (Peng et al. 2019) gives the closed-form,
# non-meta answer to "what per-sample weight on the advantages MAXIMIZES return?":
# the trust-region optimum  max_pi E[A] s.t. KL(pi||pi_old)<=eps  is
#       pi*(a|s) proportional to pi_old(a|s) * exp(A(s,a)/beta),
# i.e. the optimal weight is  w_i = exp(A_i/beta)  -- an ANALYTIC function of the
# advantage we already estimate. No learned head, no REINFORCE, no meta, no policy
# dual. We layer this on PPO's clipped surrogate as a non-negative per-sample
# multiplier (w>=0 => commutes with the clip max), with the advantage standardized
# so beta is in std units. The weight is softmax-normalized over the minibatch
# (mean 1, numerically stable: w_i = B * softmax(A/beta)_i) so it cannot rescale
# the overall step (no learning-rate confound) -- it only RELATIVELY reweights.
#
# beta is auto-tuned per minibatch (1-D bisection) to hit a target effective-
# sample-size ESS = (sum w)^2/sum w^2 (CRR/AWR-practical style). This is a scalar
# root-find on the advantages -- NOT the MPO policy-space Lagrangian dual, and not
# meta. Set awr_target_ess=0 to use a fixed awr_beta instead. awr_wmax clamps the
# weight (CRR found clamped beats full-exp), renormalized back to mean 1.
#
# HYPOTHESIS / TENSION. AWR's exp() AMPLIFIES large advantages; our proven winner
# rankgauss COMPRESSES them. AWR is off-policy regression (un-weighted = all actions
# pulled equally, so exp is needed); we are on-policy clipped PG where A already
# multiplies the gradient, so re-amplifying heavy-tailed A may destabilize. This run
# tests WHERE on the compress<->amplify spectrum the optimum sits for on-policy
# clipped PG with a distributional critic. AWR also asymmetrically SUPPRESSES
# negative-advantage gradients (never flips sign) -- a distinct axis from rankgauss.
# Default adv_transform="v10" (identity) so the AWR weight is the ONLY shaping, for
# a clean A/B against the rankgauss baseline (symlog_v1).
#
# Base: symlog_v1 (standard GAE + rankgauss + HL-Gauss symlog critic + beta actor +
# dual-backward decoupled grad clip + clip-higher 0.2/0.28 + target_kl 0.03).
#
...
class Args:
    ...
    # AWR default: "v10" (identity) so the AWR weight is the ONLY advantage shaping.
    adv_transform: str = "v10"

    # AWR advantage weighting (Peng et al. 2019). w_i = B * softmax(A_i/beta)_i,
    # a non-negative per-sample multiplier on the PPO surrogate (mean 1 => no LR
    # confound; commutes with the clip since w>=0). A is the standardized advantage.
    awr: bool = True
    awr_beta: float = 1.0            # fixed temperature (std units) when awr_target_ess<=0
    awr_target_ess: float = 0.5      # >0: auto-tune beta per minibatch to this ESS fraction
    awr_wmax: float = 20.0           # CRR-style clamp on the weight (renormalized to mean 1)
...
def awr_weights(adv, beta, target_ess, wmax, n_iter=25):
    """AWR per-sample weight w = B * softmax(adv/beta) over the (mini)batch.

    Non-negative, mean ~= 1 (numerically stable softmax form, so it relatively
    reweights without rescaling the overall step). If target_ess>0, beta is solved
    by bisection so the effective-sample-size fraction ESS/B = (sum w)^2/(B*sum w^2)
    hits target_ess (ESS is monotone increasing in beta: larger beta => flatter =>
    higher ESS). Optional CRR-style clamp at wmax, then renormalized to mean 1.
    Returns (w, beta_eff, ess_frac). adv is detached (the weight is a fixed
    multiplier on the surrogate, like rankgauss)."""
    a = adv.detach()
    n = a.numel()

    def ess_frac(b):
        p = torch.softmax(a / b, dim=0)            # sums to 1
        return (1.0 / (p.pow(2).sum() * n)).item()  # in (0, 1]

    if target_ess and target_ess > 0.0:
        lo, hi = 1e-3, 1e3
        for _ in range(n_iter):
            mid = (lo * hi) ** 0.5                  # geometric-mean bisection
            if ess_frac(mid) < target_ess:         # too peaked -> raise beta
                lo = mid
            else:
                hi = mid
        beta = (lo * hi) ** 0.5

    w = torch.softmax(a / beta, dim=0) * n          # mean exactly 1
    if wmax and wmax > 0:
        # CRR-style cap on amplification. softmax already gives mean 1; clamping
        # only REDUCES (caps the largest weights), so mean stays <=1 and we never
        # re-amplify a capped entry. Rarely binds for standardized advantages.
        w = w.clamp(max=wmax)
    ess = (w.sum() ** 2 / (w.pow(2).sum() * n + 1e-8)).item()
    return w, float(beta), ess
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # AWR weight: non-negative per-sample multiplier on the clipped
                # surrogate (w>=0 commutes with the max). Built from the standardized
                # advantage so beta is in std units. mean(w)~=1 => no LR confound.
                if args.awr:
                    w_awr, beta_eff, ess_frac = awr_weights(
                        mb_advantages, args.awr_beta, args.awr_target_ess, args.awr_wmax
                    )
                else:
                    w_awr = torch.ones_like(mb_advantages)
                    beta_eff, ess_frac = 0.0, 1.0
                pg_loss = (w_awr * torch.max(pg_loss1, pg_loss2)).mean()
        ...
        if args.awr:
            # AWR diagnostics from the last minibatch: where on the compress<->amplify
            # spectrum we landed. ess_frac near target => beta tracking; w_max high =>
            # peaked/amplifying. corr(w, signed A) ~1 by construction; corr(w, |A|)
            # reveals asymmetry (exp up-weights +A, suppresses -A => should be < 1).
            with torch.no_grad():
                wa = w_awr.detach()
                ad = mb_advantages.detach()
                wz = (wa - wa.mean()) / (wa.std() + 1e-8)
                corr_w_signed = (wz * ((ad - ad.mean()) / (ad.std() + 1e-8))).mean().item()
                absa = ad.abs()
                corr_w_absadv = (wz * ((absa - absa.mean()) / (absa.std() + 1e-8))).mean().item()
            writer.add_scalar("awr/beta", beta_eff, global_step)
            writer.add_scalar("awr/ess_frac", ess_frac, global_step)
            writer.add_scalar("awr/w_max", wa.max().item(), global_step)
            writer.add_scalar("awr/corr_w_signed_adv", corr_w_signed, global_step)
            writer.add_scalar("awr/corr_w_abs_adv", corr_w_absadv, global_step)
        if auto_alpha:
