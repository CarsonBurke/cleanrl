# 32x LR + 1 minibatch with a SiTU-GLU trunk (sTiGLU) instead of LeakyReluSq.
# Same host-actor trainer as ppo_continuous_action_32xlr_1mb_v1: lr=9.6e-3,
# num_minibatches=1; PPO loss, clip, critic and Beta head are unchanged. Each
# Linear+LeakyReluSq stage is replaced by a SiTUGLUBranch (Kimi K3 product
# down(situ_glu(gate(x), up(x))), fixed caps 4/25), two stacked per trunk with
# the first consuming observations directly -- the tpo_intra_beta_situglu_v5
# layout. Each block's down projection is scaled to match the replaced
# LeakyReluSq stage's initial output scale. Hypothesis: the soft-capped
# multiplicative gate keeps coordinated joint updates alive at 32x LR where
# the squared leaky-ReLU saturates or kills gradients.
import os
import random
import time
...
from cleanrl.shared.host_actor import HostMLP, SITU_GLU_MEAN_SQUARE, SiTUGLUBranch
...
def stiglu_branch(in_dim, out_dim):
    """One SiTU-GLU block; two stack per trunk (tpo v5 layout)."""
    branch = SiTUGLUBranch(in_dim, out_dim)
    # layer_init assumes a bias; the branch linears are bias-free, so init the
    # weights directly with the same orthogonal scheme (sqrt(2) gate/up gain).
    torch.nn.init.orthogonal_(branch.gate.weight, np.sqrt(2))
    torch.nn.init.orthogonal_(branch.up.weight, np.sqrt(2))
    # Match the replaced LeakyReluSq stage's initial output scale, as in
    # tpo_intra_beta_situglu_v5 (theirs targets ReLU^2's 12.0): E[f(N(0,2))^2]
    # is 6.375 for f=leaky_relu(.,0.5)^2 vs SITU_GLU_MEAN_SQUARE for SiTU, and
    # a rectangular orthogonal M->H down projection contributes M/H on average.
    down_std = np.sqrt(6.375 * out_dim / (branch.hidden_dim * SITU_GLU_MEAN_SQUARE))
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            stiglu_branch(observation_dim, 64), stiglu_branch(64, 64),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); FP32; SiTU-GLU trunk; native-action storage; host actor mirror")
