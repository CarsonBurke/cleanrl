# HOPSD v21.1 — upside-down teacher WITHOUT screening: outcome-only context (s, g).
# =====================================================================================
# v21 autopsy: grade_sensitivity pinned at 0.012 (~1% of the z action range) -> the
# g-conditional was INERT and distillation degenerated to behavior cloning (1524@1.5M).
# Mechanism: the future-action windows SCREEN OFF the outcome — given s and the
# executed plan around t, a_t is nearly conditionally independent of how well things
# went. The mirror image of v1: there outcome-context explained away the advantage in
# the WEIGHTS; here action-context explains away the outcome in the CONDITIONAL.
# v21.1 completes the rethink ladder by removing the screen: teacher actor context is
# (s, g) ONLY — no windows — so all outcome correlation must route through g. This is
# iterated reward-conditioned supervised learning distilled through a hindsight
# teacher; the query at g*=+1.0 is the improvement operator. Teacher critic keeps its
# windowed privileged input (diagnostics only; GAE is anchored by the student critic).
# Prediction: grade_sensitivity must rise >=10x from 0.012, else g simply cannot carry
# improvement signal via regression and the WEIGHT channel (tilt, v19=5531) is
# cemented as the only carrier. Read @1.5M vs v19 5531 / control 5288 / BC-floor ~1500.
# =====================================================================================
#
# --- v21 rationale (retained below; superseded by the autopsy above) ---
# Ab initio rethink. The tilted teacher is a SELECTION operator: it never sees an
# outcome (phi = future actions only) and "how it played out" enters solely as a
# scalar sample weight — imitate the lucky. v1 taught that outcome context + tilt is
# degenerate (context explains the advantage -> weights constant within cells ->
# posterior reconstruction). But that theorem cuts both ways: outcome context with NO
# tilt is perfectly well-posed. v21 inverts the architecture:
#   TRAIN: teacher = plain-MLE conditional pi_T(a | s, windows, g) with the realized
#     z-scored return g IN THE CONTEXT (teacher_sees_g=True, weights gone). It learns
#     the behavior manifold indexed by outcome: what the policy does on good vs bad
#     days at states like this. Plain MLE cannot collapse — nothing to explain away.
#   DISTILL: the student pulls toward pi_T queried at g* = query_grade (+1.0 sigma,
#     inside the data: ~16% of samples exceed it) — "given where you were and the plan
#     you were executing, what would you have done had it played out well?" The
#     improvement operator moves from the weight channel to the query channel;
#     "what it did wrong" = the realized-vs-starred conditional gap (logged).
# Trust-region property preserved: teacher is still a smooth tracking learner (plain
# MLE is lower-variance than tilted MLE); student still acts obs-only.
# Kill-tells: debug/grade_sensitivity ~ 0 => the g-channel is inert and this is mere
# behavior cloning (returns flatline at policy level, visible <500k); distill_kl
# inflating => starred extrapolation jitter. Reference @1.5M: v19 tilt champion 5531,
# control 5288, no-phi 4726.
# =====================================================================================
#
...
class Args:
    ...
    teacher_sees_g: bool = True      # v21.1: INERT — the (s, g) context is hardcoded
    query_grade: float = 1.0         # v21: distill from the teacher queried at g* = this many sigma
...
class HindsightTeacher(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, args):
        super().__init__()
        H = args.hidden
        actor_in = obs_dim + 1  # v21.1: (s, g) only — windows removed to unscreen g
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v21.1: outcome-only actor context — no windows, no valid_frac; the
            # counterfactual query context is identical except g -> g*.
            teacher_actor_in = torch.cat([obs, g.unsqueeze(-1)], dim=-1)
            teacher_actor_in_star = torch.cat(
                [obs, torch.full_like(g, args.query_grade).unsqueeze(-1)], dim=-1
            )
        ...
        b_teacher_actor_in_star = teacher_actor_in_star.reshape(-1, teacher_actor_in_star.shape[-1])
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                teacher_actor_loss = t_nll.mean()  # v21: PLAIN MLE — no tilt anywhere
                ...
                # v21: distill from the STARRED query — "had it played out well" —
                # evaluated with the just-updated teacher (fresher conditional).
                with torch.no_grad():
                    t_alpha_star, t_beta_star = teacher.actor_params_for(
                        b_teacher_actor_in_star[mb_inds]
                    )
                kl_dims = beta_kl_per_dim(t_alpha_star, t_beta_star, s_alpha, s_beta)
        ...
        # teacher/student entropies, mean-action gap vs the STARRED target, and
        # grade sensitivity = |mu_T(g*) - mu_T(g_realized)| — the v21 kill-tell:
        # ~0 means the g-channel is inert and distillation is mere behavior cloning.
        with torch.no_grad():
            t_vals, t_ents, s_ents, gaps, grade_sens = [], [], [], [], []
            for start in range(0, args.batch_size, args.minibatch_size):
                sl = slice(start, start + args.minibatch_size)
                ta, tb = teacher.actor_params_for(b_teacher_actor_in[sl])
                ta_s, tb_s = teacher.actor_params_for(b_teacher_actor_in_star[sl])
                ...
                gaps.append((ta_s / (ta_s + tb_s) - sa / (sa + sb)).abs().mean().item())
                grade_sens.append((ta_s / (ta_s + tb_s) - ta / (ta + tb)).abs().mean().item())
        ...
        writer.add_scalar("debug/grade_sensitivity", np.mean(grade_sens), global_step)
