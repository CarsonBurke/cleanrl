# Scale-restored normalized residual SiTU-GLU v6.
# h=N(stage1(obs)); b=N(stage2(sqrt(64)*h)); y=g*head(sqrt(64)*N(h+b)).
# Unit directions need sqrt(width) scaling for order-one gate/up coordinates.
# Learned per-output g removes v5's fixed readout range; initialize g=1.
# Keep unit-L2 matrix projection, additive mixing, and full-batch 8x-LR PPO.
# Hypothesis: v5 discarded useful scale, not evidence against normalized geometry.
import os
import random
import time
