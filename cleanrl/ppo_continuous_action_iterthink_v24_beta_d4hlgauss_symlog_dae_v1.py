# PPO + IterThink v24 beta d4-HL-Gauss symlog + DIRECT ADVANTAGE ESTIMATION (DAE) v1.
#
...
# arXiv:2109.06093), adapted to continuous control and kept faithful otherwise.
#
...
#
# FAITHFUL DAE DETAILS. A_hat is STOP-GRADIENT in the PPO policy loss (it is an
# advantage estimate, not a thing the policy maximizes through). The baseline and
# bootstrap V_tgt are the SNAPSHOT rollout value (detached) — Eq 12 proves A_hat*
# depends only on V_tgt, justifying the detach. f shares the trunk with the critic.
# The advantage is standardized per minibatch (norm_adv=True): an AFFINE map that
# keeps the agent's learned RANKING and RELATIVE magnitudes intact and only rescales
# the arbitrary global scale. With a fixed-snapshot V_tgt the DAE minimizer is the
# (small, ~1e-3-scale) 1-step TD residual, which used raw cannot drive the policy
# against shared-trunk value/DAE gradient coupling; standardization restores the
# O(1) scale PPO needs. Expect DAE to start slower than GAE (biased early), win late.
#
# HL-GAUSS KEPT. The value V_hat is still the Dreamer-style symlog HL-Gauss
# distributional critic, trained by cross-entropy to the scalar lambda-return target
# (which estimates V^pi — the same target DAE's value would converge to). DAE adds
# only the advantage head; the value path is unchanged. Decoupled (now TRIPLE-
# backward) gradient clipping balances policy / value-CE / DAE gradients on the
# shared trunk. Run config: HalfCheetah-v4 only, 8M timesteps.
#
# Base: ppo_continuous_action_iterthink_v24_beta_d4hlgauss_symlog_v1.py.
#
...
class Args:
    ...
    norm_adv: bool = True            # standardize A_hat per minibatch. This is AFFINE -> it preserves the
                                     # agent's LEARNED ranking and RELATIVE magnitudes; it only rescales the
                                     # arbitrary global scale so PPO can use it. Raw A_hat (=norm_adv False)
                                     # is ~1e-3 in normalized-reward units (the 1-step TD residual the DAE
                                     # minimizer collapses to when V_tgt is a fixed snapshot), far too small
                                     # to drive the policy against shared-trunk value/DAE gradient coupling.
    ...
    dae_nstep: int = 16              # n-step horizon of the DAE shaped-return recursion (replaces gae_lambda for the advantage); must divide num_steps
    dae_k_center: int = 4            # MC samples a'~mu per state for the pi-centering E_mu[f(s,a')]
    dae_coef: float = 1.0            # weight on the DAE advantage loss L_A in the (triple) backward
    dae_grad_clip: float = 0.25      # max-norm for the DAE gradient (adv head + its trunk part)
...
if __name__ == "__main__":
    ...
    dae_params = agent.dae_parameters()        # advantage head + shared trunk (DAE L_A gradient)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        valbase_v = seg_view(values)                   # (nstep, NS) snapshot V_tgt(s_t) baseline (detached)
        tgtprob_v = seg_view(target_probs)             # (nstep, NS, num_bins)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, n_segments, seg_per_mb):
                ...
                # --- DAE advantage loss L_A (Eq 13): minimize the variance of the
                # advantage-shaped n-step return. Backward recursion over the segment
                # with (r - A_hat) as the per-step quantity; snapshot value is both the
                # bootstrap and the baseline (Eq 12: A_hat* depends only on V_tgt). ---
                adv_seg = adv.reshape(nstep, S)
                shaped = rew_v[:, mb_seg] - adv_seg                # (nstep, S)
                running = boot_v[mb_seg]                            # (S,) = R after the last step
                R_rows = [None] * nstep
                for j in reversed(range(nstep)):
                    ...
                    R_rows[j] = running
                R = torch.stack(R_rows, dim=0)                     # (nstep, S)
                dae_loss = ((R - valbase_v[:, mb_seg]) ** 2).mean()

                # --- Policy loss: PPO clip on the LEARNED, pi-centered advantage,
                # STOP-GRADIENT (A_hat is an estimate, not maximized through). Used RAW
                # to preserve the agent's own magnitude unless norm_adv. ---
                ...
                # HL-Gauss value loss: CE to the fixed scalar-return projection target.
                mb_target_probs = tgtprob_v[:, mb_seg].reshape(-1, args.num_bins)
                value_log_probs = torch.log_softmax(value_logits, dim=-1)
                v_loss = -(mb_target_probs * value_log_probs).sum(dim=-1).mean()

                entropy_loss = entropy.mean()
                ent_coef_eff = args.ent_coef

                if args.separate_grad_clip:
                    # TRIPLE-BACKWARD decoupled clipping: value-CE, DAE-advantage, and
                    # policy gradients are each backpropped and clipped to their own
                    # max-norm, then summed on the shared trunk so no single objective
                    # swamps the shared features.
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    (args.dae_coef * dae_loss).backward(retain_graph=True)
                    dae_gn = nn.utils.clip_grad_norm_(dae_params, args.dae_grad_clip)
                    dae_grads = [(p, p.grad.detach().clone()) for p in dae_params if p.grad is not None]
                    optimizer.zero_grad(set_to_none=True)
                    ...
                    loss = pg_loss - ent_coef_eff * entropy_loss + v_loss * args.vf_coef + args.dae_coef * dae_loss
                    optimizer.zero_grad()
                    loss.backward()
                    critic_gn = actor_gn = dae_gn = nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                    optimizer.step()
        ...
        writer.add_scalar("losses/dae_grad_norm", float(dae_gn), global_step)
