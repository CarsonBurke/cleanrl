# V-MPO v57 — v54 with a satisfiable concentration budget (single factor).
#
# Only change from v54: epsilon_alpha_concentration 1.58e-5 -> 3e-4, the
# measured single-Adam-step concentration-KL floor in the v53 50M run
# (conc-KL sat at 2-8e-4 all run: the old bound was unreachable at lr 3e-4).
# Deliberately above the paper Gym range top (5e-5); the range was tuned
# for Gaussian covariance KLs, not this Beta concentration analog.
# Hypothesis: a bindable bound stops the unbounded dual ramp and the
# late-run over-penalization of all concentration change.
# lr, target hold, global top-k, reward norm, model are unchanged.
...
class Args:
    ...
    epsilon_alpha_concentration: float = 3e-4
