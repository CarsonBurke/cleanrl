# V-MPO v51 — v50 (unshared + nextlat) + TRUE DG GATE combined into the M-step.
#
# Sole delta vs v50: the M-step weighted-MLE target is tilted by the delight
# gate (arXiv:2603.14608 Alg. 2): psi = normalize(weights * sigmoid(U*l/eta)),
# l = clip(-OLD log-density, -10, 10), eta = 1, no whitening. E-step (top-k,
# eta solve, temperature) runs UNCHANGED on raw advantages; the gate only
# reallocates the fixed M-step budget (psi sums to 1) inside the same single
# joint backward. U is the reward-normalized GAE advantage. --no-dg recovers
# exact v50.
...
class Args:
    ...
    dg: bool = True                  # v51: delight-tilt the M-step weights
...
if __name__ == "__main__":
    ...
    def update_loss_model(
        ...
        with torch.no_grad():
            ...
            # v51 DG TILT (sole delta): reallocate the fixed M-step budget by
            # delight. E-step weights/eta/temperature above are UNCHANGED; psi
            # still sums to 1. U is the reward-normalized GAE advantage.
            if args.dg:
                old_log_prob = beta_log_prob(
                    old_alpha.float(), old_beta.float(), native_actions
                )
                dg_surprisal = (-old_log_prob).clamp(
                    min=-args.dg_surprisal_clip, max=args.dg_surprisal_clip
                )
                dg_delight = advantages * dg_surprisal
                dg_w = torch.sigmoid(dg_delight / args.dg_eta)
                gated_weights = weights * dg_w
                psi_weights = gated_weights / gated_weights.sum().clamp_min(1e-12)
            else:
                dg_w = torch.ones_like(weights)
                dg_delight = torch.zeros_like(weights)
                psi_weights = weights
            dg_w_mean = dg_w.mean()
            dg_w_std = dg_w.std()
            dg_delight_absmean = dg_delight.abs().mean()
            dg_sat_frac = (dg_delight.abs() > 3.0).float().mean()

        log_prob = beta_log_prob(new_alpha, new_beta, native_actions)
        policy_loss = -(psi_weights * log_prob).sum()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if should_log:
            ...
            writer.add_scalar("debug/dg_w_mean", dg_w_mean_value, global_step)
            writer.add_scalar("debug/dg_w_std", dg_w_std_value, global_step)
            writer.add_scalar(
                "debug/dg_delight_absmean", dg_delight_absmean_value, global_step
            )
            writer.add_scalar("debug/dg_sat_frac", dg_sat_frac_value, global_step)
