# V-MPO v38 — maximum-feasible full-KL target projection.
#
# Controlled derivative of v30 replacing its stale mean-KL/post-step hard copy.
# At the existing synchronized target checkpoint, the exact pre-step learner is
# deployed directly when feasible; otherwise a parameter-segment bisection moves
# the behavior target as far as possible without exceeding the full forward Beta
# KL radius. The critic, normalization, V-MPO loss, cadence, and optimizer are
# unchanged.
...
class Args:
    ...
    target_kl_threshold: float = 0.007071067811865476
    target_projection_iterations: int = 12
...
if __name__ == "__main__":
    ...
    if args.target_kl_threshold <= 0.0:
        raise ValueError("target_kl_threshold must be positive")
    if args.target_projection_iterations <= 0:
        raise ValueError("target_projection_iterations must be positive")
    ...
    def policy_parameters(model):
        return (
            tuple(model.trunk.parameters())
            + tuple(model.policy_mlp.parameters())
            + tuple(model.actor_alpha.parameters())
            + tuple(model.actor_beta.parameters())
        )

    online_policy_parameters = policy_parameters(agent)
    target_policy_parameters = policy_parameters(target_agent)
    if len(online_policy_parameters) != len(target_policy_parameters):
        raise RuntimeError("online and target policy parameter structures differ")
    duals = nn.Parameter(
        torch.tensor(
            [
    ...
    def target_policy_model(observations):
        with torch.autocast(device_type="cuda", dtype=autocast_dtype):
            target_alpha, target_beta = target_agent.policy(observations)
        return target_alpha.float(), target_beta.float()


    @torch.no_grad()
    def deploy_target_fraction(start_parameters, end_parameters, fraction):
        for target_parameter, start_parameter, end_parameter in zip(
    ...
    if args.compile:
        ...
        target_policy_model = torch.compile(
            target_policy_model,
            mode=args.compile_mode,
            dynamic=False,
            fullgraph=True,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        flat_native_actions = native_actions.reshape(
        ...
        should_check_target = iteration % args.log_interval == 0
        should_log = should_check_target or iteration == 1
        duals_before = duals.detach().clone() if should_log else None
        target_age_batches += 1
        target_promoted = False
        promote_for_full_kl = False
        promote_for_max_age = False
        promotion_requested = False
        target_projection_fraction_value = 0.0
        target_deployed_full_kl_value = 0.0
        pre_step_online_parameters = None

        if should_check_target:
            candidate_full_kl_value = float(metrics[5].detach().item())
            promote_for_full_kl = (
                candidate_full_kl_value >= args.target_kl_threshold
            )
            promote_for_max_age = (
                target_age_batches >= args.target_update_period
            )
            promotion_requested = promote_for_full_kl or promote_for_max_age
            if promotion_requested:
                pre_step_online_parameters = tuple(
                    parameter.detach().clone()
                    for parameter in online_policy_parameters
                )
        ...
        if promotion_requested:
            assert pre_step_online_parameters is not None
            start_parameters = tuple(
                parameter.detach().clone()
                for parameter in target_policy_parameters
            )
            full_fraction = torch.ones((), device=device)
            deploy_target_fraction(
                start_parameters,
                pre_step_online_parameters,
                full_fraction,
            )
            if args.compile:
                torch.compiler.cudagraph_mark_step_begin()
            candidate_alpha, candidate_beta = target_policy_model(
                flat_observations
            )
            exact_candidate_full_kl = beta_kl(
                flat_old_alphas,
                flat_old_betas,
                candidate_alpha,
                candidate_beta,
            ...
            if (
                float(exact_candidate_full_kl.item())
                <= args.target_kl_threshold
            ):
                deployed_fraction = full_fraction
                deployed_full_kl = exact_candidate_full_kl
            else:
                feasible_fraction = torch.zeros((), device=device)
                infeasible_fraction = full_fraction
                best_feasible_fraction = feasible_fraction
                for _ in range(args.target_projection_iterations):
                    trial_fraction = 0.5 * (
                    ...
                    if args.compile:
                        torch.compiler.cudagraph_mark_step_begin()
                    trial_alpha, trial_beta = target_policy_model(
                        flat_observations
                    )
                    trial_full_kl = beta_kl(
                        flat_old_alphas,
                        flat_old_betas,
                        trial_alpha,
                        trial_beta,
                    ...
                    feasible = (
                        trial_full_kl <= args.target_kl_threshold
                    )
                    best_feasible_fraction = torch.where(
                        feasible,
                        torch.maximum(
                            best_feasible_fraction,
                            trial_fraction,
                        ),
                        best_feasible_fraction,
                    )
                    feasible_fraction = torch.where(
                        feasible,
                        trial_fraction,
                        feasible_fraction,
                    )
                    infeasible_fraction = torch.where(
                        feasible,
                        infeasible_fraction,
                        trial_fraction,
                    )
                deployed_fraction = best_feasible_fraction
                deploy_target_fraction(
                    start_parameters,
                    pre_step_online_parameters,
                    deployed_fraction,
                ...
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                deployed_alpha, deployed_beta = target_policy_model(
                    flat_observations
                )
                deployed_full_kl = beta_kl(
                    flat_old_alphas,
                    flat_old_betas,
                    deployed_alpha,
                    deployed_beta,
            ...
            (
                target_projection_fraction_value,
                target_deployed_full_kl_value,
            ) = (
                float(deployed_fraction.item()),
                float(deployed_full_kl.item()),
            )
            target_promoted = (
                target_projection_fraction_value > 0.0
                and target_deployed_full_kl_value
                <= args.target_kl_threshold
            )
            if target_promoted:
                target_age_batches = 0
                ...
                zero_fraction = torch.zeros((), device=device)
                deploy_target_fraction(
                    start_parameters,
                    pre_step_online_parameters,
                    zero_fraction,
        ...
        if should_log:
            ...
            writer.add_scalar(
                "vmpo/target_promoted_for_full_kl",
                float(promote_for_full_kl),
                global_step,
            )
            writer.add_scalar(
            ...
            writer.add_scalar(
                "vmpo/target_projection_fraction",
                target_projection_fraction_value,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_deployed_full_kl",
                target_deployed_full_kl_value,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_kl_threshold",
                args.target_kl_threshold,
                global_step,
            )
            writer.add_scalar(
