# Projected-target v19 — candidate-free, single-sample policy improvement.
#
# Each fresh rollout supplies one Beta action and GAE advantage per state. The
# standardized advantage scales the action score in the Beta mean coordinate;
# its local inverse Fisher maps that score to a natural policy direction. A
# batch-global line search turns those directions into stopped Beta targets at
# a fixed behavior-to-target KL budget. The actor fits the explicit targets with
# target-to-online Beta KL, whose gradient vanishes when the target is reached.
#
# Concentration is held fixed in this first projected-target experiment because
# a one-action concentration score has high variance. There is no candidate
# sampling, top-k E-step, temperature, KL dual, or long-lived target network.
# GAE, PopArt, staggered phases, vectorized per-environment observation
# normalization, and the full-width ReLU-squared IterThink model remain.
...
if __name__ == "__main__":
    ...
    def update_loss_model(
        ...
        # This forward KL is a proper distribution-fitting objective. Unlike
        # weighted sampled log likelihood, its actor gradient is exactly zero
        # when the online policy reaches the stopped target distribution.
        # Dividing by the detached target scale recovers the ordinary projected
        # policy-gradient magnitude at the behavior policy; the KL budget then
        # controls the finite target rather than starving the sole actor update.
