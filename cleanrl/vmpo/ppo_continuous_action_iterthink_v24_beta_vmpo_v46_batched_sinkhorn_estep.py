# V-MPO v46 — batched online Sinkhorn E-step for Beta policies.
#
# V45 restored the strong per-rollout balanced transport target, but solved
# every eta serially: roughly 40 bisections plus floor bracketing, with 64
# log-domain sweeps and 64 post-balance sweeps per eta.  That produced thousands
# of tiny GPU launches and host synchronizations for each 2,496-transition
# rollout.  Its four epochs over two actor minibatches also required eight
# backward steps per rollout.  V46 evaluates eta grids in parallel, refines the
# grid three times without per-candidate synchronization, and defaults to one
# full-batch actor step.  It retains positive weighted likelihood, exact
# marginal preservation, full analytic Beta-KL projection, and atomic
# promotion, without Q estimates, replay, simulator branches, counterfactual
# actions, top-k selection, signed weights, or contrastive loss.
...
class Args:
    ...
    # The default is one [num_steps, num_envs] transport per rollout.
    window_batches: int = 1
    # log(2) from v30's top-half selection plus 0.01 nat of extra tilt.
    epsilon_eta: float = 0.7031471805599453
    sinkhorn_iterations: int = 32
    eta_grid_size: int = 64
    eta_grid_refinements: int = 3
    epsilon_policy_kl: float = 0.01
    m_step_minibatches: int = 2496
    m_step_epochs: int = 1
...
if __name__ == "__main__":
    ...
    if type(args.eta_grid_size) is not int or args.eta_grid_size < 2:
        raise ValueError("eta_grid_size must be an integer of at least two")
    if type(args.eta_grid_refinements) is not int or args.eta_grid_refinements < 1:
        raise ValueError("eta_grid_refinements must be a positive integer")
    ...
    @torch.no_grad()
    def transport_for_etas(centered_advantages, eta_values):
        """Evaluate a full eta grid as one batched balanced transport solve."""
        time_rows, environment_columns = centered_advantages.shape
        eta_count = eta_values.shape[0]
        log_kernel = centered_advantages.unsqueeze(0) / eta_values[:, None, None]
        log_row_mass = math.log(environment_columns)
        log_column_mass = math.log(time_rows)
        log_row_scale = torch.zeros(
            (eta_count, time_rows), device=device, dtype=torch.float64
        )
        log_column_scale = torch.zeros(
            (eta_count, environment_columns), device=device, dtype=torch.float64
        )

        for _ in range(args.sinkhorn_iterations):
            log_row_scale = log_row_mass - torch.logsumexp(
                log_kernel + log_column_scale[:, None, :], dim=2
            )
            log_column_scale = log_column_mass - torch.logsumexp(
                log_kernel + log_row_scale[:, :, None], dim=1
            )
            # Recenter each candidate's dual gauge independently without
            # changing the represented transport.
            gauge = log_row_scale.mean(dim=1, keepdim=True)
        ...
        log_weights = (
            log_kernel
            + log_row_scale[:, :, None]
            + log_column_scale[:, None, :]
        )
        # Preserve mathematically positive support after float64 underflow, then
        # restore both marginals for every eta in the same batched sweeps.
        tiny = torch.finfo(torch.float64).tiny
        weights = torch.exp(log_weights.clamp_min(math.log(tiny)))
        for _ in range(args.sinkhorn_iterations):
            weights = weights * (
                environment_columns / weights.sum(dim=2, keepdim=True)
            )
            weights = weights * (
                time_rows / weights.sum(dim=1, keepdim=True)
            )
        weights.clamp_min_(tiny)

        row_residual = (weights.mean(dim=2) - 1.0).abs().amax(dim=1)
        column_residual = (weights.mean(dim=1) - 1.0).abs().amax(dim=1)
        global_mean_residual = (weights.mean(dim=(1, 2)) - 1.0).abs()
        e_step_kl = torch.special.xlogy(weights, weights).mean(dim=(1, 2))
        weight_sums = weights.sum(dim=(1, 2))
        ess_fraction = weight_sums.square() / (
            weights[0].numel() * weights.square().sum(dim=(1, 2))
        )
        max_weight = weights.amax(dim=(1, 2))
        weighted_advantage_gain = (
            weights * centered_advantages.unsqueeze(0)
        ).mean(dim=(1, 2))
        return (
            weights,
            e_step_kl,
    ...
    def solve_transport_estep(advantage_matrix):
        ...
        advantage_scale = centered_advantages.std(correction=0).clamp_min(1e-6)
        eta_values = advantage_scale * torch.logspace(
            -4.0,
            2.0,
            args.eta_grid_size,
            device=device,
        ...
        grid_coordinates = torch.linspace(
            0.0,
            1.0,
            args.eta_grid_size,
            device=device,
        ...
        final_result = None
        selected_index = None
        advantage_is_finite = torch.isfinite(advantage_matrix).all()

        for refinement_index in range(args.eta_grid_refinements):
            result = transport_for_etas(centered_advantages, eta_values)
            (
                candidate_weights,
                candidate_kl,
                candidate_ess,
                ...
                candidate_gain,
                candidate_row_residual,
                candidate_column_residual,
                candidate_global_residual,
            ) = result
            candidate_metrics = torch.stack(
                (
                    eta_values,
                    candidate_kl,
                    candidate_ess,
            ...
            candidate_is_finite = (
                advantage_is_finite
                & torch.isfinite(candidate_metrics).all(dim=1)
                & torch.isfinite(candidate_weights).all(dim=(1, 2))
                & (candidate_weights > 0.0).all(dim=(1, 2))
            )
            candidate_is_feasible = candidate_is_finite & (
                candidate_kl <= args.epsilon_eta
            )
            budget_distance = (candidate_kl - args.epsilon_eta).abs()
            largest_finite_distance = torch.where(
                candidate_is_finite,
                budget_distance,
                torch.zeros_like(budget_distance),
            ).amax()
            infeasible_penalty = largest_finite_distance + 1.0
            selection_score = torch.where(
                candidate_is_feasible,
                budget_distance,
                budget_distance + infeasible_penalty,
            )
            selection_score = torch.where(
                candidate_is_finite,
                selection_score,
                torch.full_like(selection_score, torch.inf),
            )
            selected_index = selection_score.argmin()
            final_result = result

            if refinement_index + 1 < args.eta_grid_refinements:
                lower_index = (selected_index - 1).clamp_min(0)
                upper_index = (selected_index + 1).clamp_max(
                    args.eta_grid_size - 1
                )
                lower_log_eta = eta_values[lower_index].log()
                upper_log_eta = eta_values[upper_index].log()
                eta_values = torch.exp(
                    torch.lerp(lower_log_eta, upper_log_eta, grid_coordinates)
                )

        assert final_result is not None and selected_index is not None
        (
            candidate_weights,
            candidate_kl,
            candidate_ess,
        ...
        eta = eta_values[selected_index]
        weights = candidate_weights[selected_index].clone()
        e_step_kl = candidate_kl[selected_index]
        ess_fraction = candidate_ess[selected_index]
        max_weight = candidate_max_weight[selected_index]
        weighted_advantage_gain = candidate_gain[selected_index]
        row_residual = candidate_row_residual[selected_index]
        column_residual = candidate_column_residual[selected_index]
        global_mean_residual = candidate_global_residual[selected_index]

        def selected_validation_values():
            return torch.stack(
                (
                    advantage_is_finite.double(),
                    candidate_is_finite.sum().double(),
                    candidate_is_feasible.sum().double(),
                    eta,
                    e_step_kl,
                    ess_fraction,
                    max_weight,
                    ...
                    global_mean_residual,
                    weights.min(),
                    torch.isfinite(weights).all().double(),
                    (weights > 0.0).all().double(),
                )
            ).tolist()

        validation_values = selected_validation_values()
        (
            advantages_finite,
            finite_candidates,
            feasible_candidates,
            ...
            gain_value,
            row_residual_value,
            column_residual_value,
            global_residual_value,
            min_weight_value,
            weights_finite,
            weights_positive,
        ) = validation_values
        if finite_candidates == 0.0 or feasible_candidates == 0.0:
            raise FloatingPointError(
                "transport grid has no finite budget-feasible candidate: "
                f"advantages_finite={bool(advantages_finite)}, "
                f"finite_candidates={int(finite_candidates)}, "
                f"feasible_candidates={int(feasible_candidates)}, "
                f"eta={eta_value:.12g}, kl={kl_value:.12g}, "
                f"row_residual={row_residual_value:.12g}, "
                f"column_residual={column_residual_value:.12g}"
            )
        ...
        if max(
            ...
            # Only the chosen [T, E] transport is revisited; all grid candidates
            # have already completed the same number of batched balance sweeps.
            time_rows, environment_columns = weights.shape
            for _ in range(args.sinkhorn_iterations):
                weights = weights * (
                    environment_columns / weights.sum(dim=1, keepdim=True)
                )
                weights = weights * (
                    time_rows / weights.sum(dim=0, keepdim=True)
                )
        ...
        transport_valid = (
            bool(advantages_finite)
            and bool(weights_finite)
            and bool(weights_positive)
            and row_residual_value <= MARGINAL_TOLERANCE
            and column_residual_value <= MARGINAL_TOLERANCE
            and global_residual_value <= MARGINAL_TOLERANCE
            and kl_value <= args.epsilon_eta + ETA_KL_TOLERANCE
        )
        if not transport_valid:
            raise FloatingPointError(
                "transport projection invalid: "
                f"eta={eta_value:.12g}, kl={kl_value:.12g}, "
                f"ess={ess_value:.12g}, gain={gain_value:.12g}, "
                f"row_residual={row_residual_value:.12g}, "
                f"column_residual={column_residual_value:.12g}, "
                f"global_residual={global_residual_value:.12g}, "
                f"min_weight={min_weight_value:.12g}, "
                f"max_weight={max_weight_value:.12g}"
            )
        return (
            eta.float(),
    ...
    def fit_candidate(
        ...
        for epoch in range(args.m_step_epochs):
            # The default window is one 2,496-element slice, so this executes
            # exactly one actor backward without a pointless full-batch shuffle.
            for start in range(0, args.window_size, args.m_step_minibatches):
                stop = min(start + args.m_step_minibatches, args.window_size)
                mark_compiled_step()
                new_alpha, new_beta = candidate_policy_model(
                    flat_observations[start:stop]
                )
                log_prob = beta_log_prob(
                    new_alpha, new_beta, flat_actions[start:stop]
                )
                policy_loss = -(weights[start:stop] * log_prob).mean()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # only the first two axes gives [num_steps, num_envs] at the default
            # one-rollout cadence; flattening that matrix therefore matches
            # observation/action flattening exactly, with environment fastest.
            advantage_matrix = (
