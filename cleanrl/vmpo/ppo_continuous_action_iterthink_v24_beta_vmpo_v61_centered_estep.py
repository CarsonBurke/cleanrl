# V-MPO v61 — E-step ablations on the v60 online-actor / EMA-anchor trainer.
#
# v30's E-step is a median cut with a 1% KL tilt: perplexity >= 99% of the top
# half, so weights are binary up to ~15% and advantage magnitude is discarded.
# Three switchable changes, each defaulting to v60 behaviour:
#
# --center-weights     M-step weights become psi - 1/n over the whole batch.
#                      The subtracted term is the batch mean score, a control
#                      variate that is exact at the rollout policy: same
#                      expected direction, half the variance, and an identically
#                      zero update when psi is uninformative. This is TPO's
#                      p - q written for one action per state.
# --topk-fraction 1.0  No truncation; with --epsilon-eta ~0.7 the perplexity
# --epsilon-eta 0.7    floor is ~50% of the full batch, the same effective
#                      count as the median cut, but graded by magnitude.
# --segment-normalize  Top-k, softmax and eta are computed per environment
#                      segment (columns of the [T, N] rollout), each segment
#                      carrying mass 1/N. Adjacent GAE advantages are ~0.94
#                      correlated, so a batch-level ranking mostly ranks
#                      segments; per-segment centering cancels that component.
#                      The per-segment top-k count is floor(T * fraction), so
#                      with T=39 a 0.5 cut keeps 19/39 rather than the batch
#                      median; irrelevant at --topk-fraction 1.0. The logged
#                      top_advantage_min becomes the mean of per-segment cuts.
#
# Diagnostics: debug/value_mean, and debug/policy_grad_split_cosine, the cosine
# between policy-parameter gradients from even and odd environments on log
# iterations, a direct signal-to-noise readout of the E-step target.
...
if __name__ == "__main__":
    ...
    if args.segment_normalize:
        estep_rows, estep_columns = args.num_steps, args.num_envs
    else:
        estep_rows, estep_columns = args.batch_size, 1
    args.topk_size = int(estep_rows * args.topk_fraction)
    if not 0 < args.topk_size <= estep_rows:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.compile:
            torch.compiler.cudagraph_mark_step_begin()
        flat_observations = observations.reshape((args.batch_size,) + observation_shape)
        flat_actions = native_actions.reshape((args.batch_size,) + action_shape)
        ...
        if should_log and args.num_envs >= 2:
        ...
        if should_log:
            ...
            writer.add_scalar("vmpo/anchor_full_kl", full_kl_value, global_step)
            writer.add_scalar(
                "vmpo/rollout_full_kl", rollout_full_kl_value, global_step
            )
            writer.add_scalar("debug/value_mean", value_mean_value, global_step)
            if split_cosine is not None:
                writer.add_scalar(
                    "debug/policy_grad_split_cosine",
                    float(split_cosine),
                    global_step,
                )
            writer.add_scalar(
