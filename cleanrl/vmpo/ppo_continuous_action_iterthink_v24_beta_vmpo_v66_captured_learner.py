# V-MPO v66 — v62's algorithm with the launch overhead taken out of both loops.
#
# Profiling v62 at 64x39 (job 8879/8889): the captured policy step was 115
# kernels and ~120 us of GPU time per env step, a third of it autocast
# re-casting the fp32 parameters to bf16 on every replay; the learner was 573
# kernels with 1.6 ms of GPU time inside ~5 ms of wall, i.e. CPU launch bound.
# Three execution changes, each behind a flag, none touching the objective:
# --rollout-mirror     act from a bf16 copy of exactly the tensors autocast
#                      casts (Linear weights/biases, expert einsum weights),
#                      refreshed with one foreach copy after each update. Eager
#                      outputs are bitwise identical to autocast on fp32
#                      parameters; 115 -> 72 kernels per step.
# --inductor-cpp-wrapper  Inductor emits a C++ launcher for the compiled learner
#                      graphs: same kernels, ~1.4 ms less Python per update.
# --capture-learner    GAE + M-step + Adam + anchor EMA + mirror refresh replay
#                      as one manually captured CUDA graph over the collector's
#                      static rollout storage. Log iterations that want the
#                      split-cosine diagnostic run the same code eagerly so the
#                      diagnostic keeps v62's pre-update ordering. Adam is
#                      capturable only when this is on. torch.quantile syncs on a
#                      tensor of levels, so the percentile scale uses an explicit
#                      sort/lerp that is checked against torch.quantile at start.
# With all three off, v66 runs v62's exact computation.
#
# ---- v62 notes ----
# V-MPO v62 — v61's learner on the shared fast rollout stack.
#
...
class Args:
    ...
    inductor_cpp_wrapper: bool = True  # C++ launcher for compiled graphs (same kernels)
    rollout_mirror: bool = True  # act from a bf16 copy of the autocast-cast parameters
    capture_learner: bool = True  # replay GAE + update as one CUDA graph
    learner_warmup_iterations: int = 3  # eager learner iterations before capture
    log_interval: int = 10
...
def autocast_operands(module):
    """Tensors bf16 autocast casts in the forward: Linear weights/biases and the
    expert einsum weights. Expert biases, residual gates and norms stay fp32
    because autocast leaves their ops in fp32."""
    tensors = []
    for child in module.modules():
        if isinstance(child, nn.Linear):
            tensors.append(child.weight)
            if child.bias is not None:
                tensors.append(child.bias)
        elif isinstance(child, FusedExperts):
            tensors.extend([child.weight1, child.weight2])
    return tensors


def linear_quantiles(values, levels):
    """``torch.quantile(values, levels)`` (linear interpolation) without the host
    sync torch.quantile performs to validate a tensor of levels; capturable."""
    sorted_values = values.sort().values
    ranks = levels * (values.numel() - 1)
    below = ranks.long()
    weights = ranks - below
    above = ranks.ceil().long()
    return torch.lerp(sorted_values[below], sorted_values[above], weights)
...
def validate_args(args):
    ...
    if args.learner_warmup_iterations < 2:
        raise ValueError("learner_warmup_iterations must be at least 2 so compilation and lazy optimizer state creation happen before capture")
    if args.capture_learner and args.compile and args.compile_mode == "reduce-overhead":
        raise ValueError("--capture-learner needs a non-cudagraph compile mode (cudagraph trees cannot be captured inside a manual CUDA graph)")
...
def main():
    ...
    # Imported only after configure_runtime: importing torch._inductor resolves
    # and memoizes the inductor cache directory, and configure_runtime is what
    # points it at the shared cache. An earlier import lands in /var/tmp with
    # its own autotune history (different reduction kernels, ulp-level
    # differences in the policy step) and breaks bitwise agreement with v62.
    from torch._inductor import config as inductor_config

    inductor_config.cpp_wrapper = args.inductor_cpp_wrapper
    ...
    try:
        ...
        # The rollout graph acts from ``rollout_agent``. With --rollout-mirror it
        # is a copy whose autocast operands are already bf16, so the captured
        # step has no per-replay casts; one foreach copy after each update
        # (fp32 -> bf16 for those tensors, plain copies for the rest) keeps it
        # equal to what autocast would compute from the online parameters.
        rollout_agent = agent
        mirror_pairs = None
        if args.rollout_mirror:
            rollout_agent = copy.deepcopy(agent).requires_grad_(False)
            for parameter in autocast_operands(rollout_agent):
                parameter.data = parameter.data.to(autocast_dtype)
            mirror_pairs = (list(rollout_agent.parameters()), online_params)
        duals = nn.Parameter(
            torch.tensor(
        ...
        optimizer = optim.Adam(
            ...
            eps=1e-8,
            fused=True,
            capturable=args.capture_learner,
        )

        def rollout_model(observations):
            with torch.autocast(device_type="cuda", dtype=autocast_dtype):
                alpha, beta, value_logits = rollout_agent(observations)
        ...
        # Private generator: the default CUDA generator feeds the rollout graph's
        # Beta samples, and touching it here would shift every draw after this.
        probe = torch.randn(
            args.batch_size, device=device, generator=torch.Generator(device=device).manual_seed(0)
        )
        if not torch.equal(linear_quantiles(probe, percentile_levels), torch.quantile(probe, percentile_levels)):
            raise RuntimeError("linear_quantiles does not reproduce torch.quantile on this build")
        del probe

        def learner_step(batch, diagnostic):
            """v62's GAE + M-step for one rollout, in v62's order. ``diagnostic``
            (log iterations only) is measured at the pre-update parameters."""
            with torch.no_grad():
                ...
                percentiles = linear_quantiles(value_targets, percentile_levels)
            ...
            with torch.no_grad():
                ...
                if mirror_pairs is not None:
                    torch._foreach_copy_(*mirror_pairs)
            # Detached: the captured outputs are kept for the run, not their autograd graph.
            return total_loss.detach(), metrics, loss_weights, return_percentile_scale, split_cosine

        def learner_inputs(batch):
            return (
                batch.observations,
                batch.transitions.transition_observations,
                batch.transitions.rewards,
                ...
                batch.policy["alpha"],
                batch.policy["beta"],
                batch.policy["native_action"],
            )

        learner_graph = None
        learner_static = None  # (input data_ptrs, outputs) of the captured learner
        # Every learner step, eager or captured, runs on this stream. Autograd
        # latches a leaf's AccumulateGrad node to the stream that first created
        # it, and a node that survives across iterations (the duals' does) makes
        # the capture wait on that stream. Being pulled into a capture is legal
        # for a side stream and fatal for the legacy default stream, so the eager
        # warmup steps must already run where the capture will.
        learner_stream = torch.cuda.Stream(device=device)

        def run_learner(batch, diagnostic, iteration):
            """Replay the captured learner; eager for warmup and diagnostic iterations."""
            nonlocal learner_graph, learner_static
            rollout_stream = torch.cuda.current_stream(device)
            learner_stream.wait_stream(rollout_stream)
            with torch.cuda.stream(learner_stream):
                if not args.capture_learner or diagnostic:
                    outputs = learner_step(batch, diagnostic)
                elif learner_graph is None and iteration <= args.learner_warmup_iterations:
                    outputs = learner_step(batch, False)
                else:
                    if learner_graph is None:
                        # Capture over the collector's permanent storage; the
                        # capture itself applies this iteration's update.
                        pointers = tuple(tensor.data_ptr() for tensor in learner_inputs(batch))
                        graph = torch.cuda.CUDAGraph()
                        with torch.cuda.graph(graph, stream=learner_stream):
                            captured = learner_step(batch, False)
                        learner_graph, learner_static = graph, (pointers, captured)
                        print(f"captured learner graph at iteration {iteration}", flush=True)
                    pointers, outputs = learner_static
                    if tuple(tensor.data_ptr() for tensor in learner_inputs(batch)) != pointers:
                        raise RuntimeError("rollout storage moved; the captured learner reads stale buffers")
                    learner_graph.replay()
            rollout_stream.wait_stream(learner_stream)
            return outputs
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            diagnostic = should_log and args.split_cosine_diagnostic and args.num_envs >= 2
            with timer.span("learner"):
                total_loss, metrics, loss_weights, return_percentile_scale, split_cosine = run_learner(
                    batch, diagnostic, iteration
                )
