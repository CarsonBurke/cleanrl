# V-MPO v63 — v62 with a scalar critic: no distributional head, no symlog.
#
# Algorithm is v62's learner (online actor, EMA KL anchor, exact eta by
# bisection, decoupled Beta trust region with Lagrangian duals, percentile-
# scaled GAE advantages, one full-batch Adam step per rollout, switchable
# E-step options) on the shared fast rollout stack, with one change: the
# critic is a single linear output trained by half squared error on the raw
# GAE returns. Rewards stay normalised by the shared VectorRewardNorm, so
# targets are O(1-10) and need no symlog compression or bucketed support.
#
# Why test it. The v30/v61/v62 critic is a 51-bin HL-Gauss head on a symexp
# support with moment-matched labels. It carried three coupled choices:
# symlog compression (which flattens gradients on large returns exactly where
# a 50M-step HalfCheetah policy lives), a bounded support, and cross-entropy
# scale for the value loss relative to the policy loss. With normalised
# rewards none of those are needed, and each is a candidate for why v30
# stalls near 9k. The scalar critic removes all three at once.
#
# As in v62 there is no gradient-norm clipping and no value clipping: the
# only clamps are the dual floor and the advantage percentile-scale floor.
#
# Execution (native batched MuJoCo, fused normalisers, one captured CUDA graph
# per rollout step, packed rollout upload, full-batch next-state critic for
# GAE) is unchanged from v62; learner paths compile in mode "default" because
# reduce-overhead cudagraph trees invalidate the captured rollout graph on
# torch 2.12 (see v62).
#
import copy
import math
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        # Scalar critic. Zero-initialised so the first values are exactly 0,
        # matching the HL-Gauss head's symmetric-logit start.
        self.value_head = nn.Linear(256, 1, bias=False)
        with torch.no_grad():
    ...
    def forward(self, observations):
        ...
        value = self._scalar_value(features)
        return alpha, beta, value

    def _scalar_value(self, features):
        # The trunk and value MLP run under bf16 autocast; the final linear
        # runs in fp32 so the emitted scalar keeps fp32 precision. A bf16 value
        # near 10 is quantised in steps of 0.06, which lands directly in the
        # TD residual r + gamma*V' - V. (The HL-Gauss head averaged 51 bins in
        # fp32 and did not have this problem.)
        hidden = self.value_mlp(features)
        with torch.autocast(device_type="cuda", enabled=False):
            return self.value_head(hidden.float()).squeeze(-1)

    def value(self, observations):
        return self._scalar_value(self.trunk(observations))
    ...
    try:
        ...
        def rollout_model(observations):
            with torch.autocast(device_type="cuda", dtype=autocast_dtype):
                alpha, beta, value = agent(observations)
            return alpha.float(), beta.float(), value.float()
        ...
        def update_loss_model(
            ...
            with torch.autocast(device_type="cuda", dtype=autocast_dtype):
                new_alpha, new_beta, value = agent(observations)
                with torch.no_grad():
            ...
            # Half squared error on raw normalised returns (paper V-MPO's critic
            # loss); no symlog, no support, no value clipping.
