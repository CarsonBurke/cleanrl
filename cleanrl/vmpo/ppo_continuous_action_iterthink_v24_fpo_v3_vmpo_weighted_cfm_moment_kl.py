# FPO v3 — V-MPO M-step as advantage-weighted flow matching (McAllister et al.
# 2025, arXiv:2507.21053) on the v30 IterThink stack.
#
# v30 is kept intact: hard-frozen target policy generating rollouts, promoted
# every 100 batches or when the mean trust region is spent; top-50% global
# softmax weights with the eta bisection; learned Lagrange duals; Dreamer
# moment-matched HL-Gauss critic; per-env obs/reward normalization; GAE;
# percentile advantage scale; phase staggering; one full-batch Adam step per
# rollout; BF16 compiled full-graph training. Only the carrier changes.
#
# Carrier: a forward-time OT flow in pre-tanh space R^A. z ~ N(0, I) at t=0 is
# integrated with 10 Euler steps of a velocity MLP conditioned on the shared
# trunk's policy features; env action = box-affine tanh(x). The rollout stores
# the pre-tanh sample x, which is the only policy-specific buffer.
#
# M-step: V-MPO's weighted negative log-likelihood becomes the weighted
# conditional flow matching loss, sum_i w_i * L_i, with L_i the mean over
# stratified (t, eps) pairs of 0.5 * ||eps_hat - eps||^2 (paper's eps-MSE;
# equals t^2 * velocity MSE on the OT path). FPO's ratio/clip machinery is
# unnecessary here: V-MPO weights are nonnegative and sum to one, the CFM loss
# is a -ELBO surrogate for -log pi up to a theta-independent constant, so
# weighted CFM is the exact FPO first-order update with no importance ratio.
#
# Trust region: flows have no analytic KL. The decoupled Beta KLs are replaced
# by decoupled Gaussian-moment KLs of the flow pushforward in pre-tanh space,
# estimated per state from K coupled noise draws pushed through the online and
# target flows (shared z makes the estimator exactly zero at promotion):
#   mean_kl   = 0.5 * sum_d (mu_new - mu_old)^2 / var_old
#   spread_kl = 0.5 * sum_d (var_new/var_old - 1 - log(var_new/var_old))
# using v30's eta/alpha budgets. KL is reparameterization-invariant, so the
# pre-tanh moments are the faithful place to measure it; action-space moments
# would explode under saturation. The FPO ELBO ratio, mean(L_online - L_target)
# on shared pairs, is logged as a diagnostic KL surrogate but not constrained.
#
# Hypothesis: the flow's mode-seeking, non-parametric M-step fits the tilted
# target q ∝ pi_old exp(A/eta) more faithfully than a factorized Beta whose
# alpha, beta >= 1 forbids sharp edge modes, while the moment trust region
# preserves v30's stability. Failure of FPO v1 (NaN by 90k steps) came from an
# unbounded reverse-time flow with a large-init ReLU^2 velocity head and an
# unclipped ratio over four reuse epochs; none of those survive here.
import copy
import os
import random
import time
...
if __name__ == "__main__":
    ...
    if args.cfm_samples <= 0:
        raise ValueError("cfm_samples must be positive")
    ...
    def update_loss_model(
        ...
        with torch.autocast(device_type="cuda", dtype=autocast_dtype):
            ...
            # Stratified flow times: one draw per stratum of width 1/N per
            # sample, so every action's CFM estimate covers the whole path.
            cfm_noise = torch.randn(
                batch,
                args.cfm_samples,
                agent.action_dim,
        ...
        # Weighted CFM is the FPO first-order M-step: -ELBO stands in for -log pi.
        policy_loss = (weights * online_cfm).sum()
        ...
        with torch.no_grad():
            ...
            cfm_gap = online_cfm - target_cfm
            elbo_kl = cfm_gap.mean()
            weighted_elbo_kl = (weights * cfm_gap).sum()
