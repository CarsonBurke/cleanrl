# Projected-target v21 — realizable one-dimensional Beta carrier.
#
# V20's strict mean loss still changed concentration indirectly because the
# network emitted alpha and beta in Euclidean coordinates. V21 emits one bounded
# mean and deterministically maps it to a smooth alpha,beta > 1 manifold. Target
# score, Fisher information, line search, and fitting KL all follow that actual
# one-dimensional carrier tangent, so every target is realizable and no
# independent concentration shortcut exists.
#
# The mean is bounded to [0.05, 0.95]. Concentration is a smooth maximum of the
# two shape-bound requirements plus fixed excess chosen to reproduce the old
# 3.386 initial concentration. GAE, PopArt, phase staggering, vectorized
# observation normalization, and the one-update IterThink schedule are retained.
...
ADVANTAGE_EPS = 1e-8
FISHER_EPS = 1e-8
POLICY_MEAN_MIN = 0.05
TARGET_MEAN_MIN = POLICY_MEAN_MIN + 1e-4
CARRIER_SMOOTH_TEMPERATURE = 0.1
CARRIER_EXCESS = 1.9 * np.log(2.0)
...
def carrier_concentration(mean):
    inverse_means = torch.stack(
        (mean.reciprocal(), (1.0 - mean).reciprocal()),
        dim=0,
    )
    return (
        CARRIER_SMOOTH_TEMPERATURE
        * torch.logsumexp(
            inverse_means / CARRIER_SMOOTH_TEMPERATURE,
            dim=0,
        )
        + CARRIER_EXCESS
    )
...
def carrier_beta_shapes(mean):
    concentration = carrier_concentration(mean)
...
class Agent(nn.Module):
    ...
    def policy(self, observations):
        ...
        mean_logits = self.actor_mean(features).float()
        mean = POLICY_MEAN_MIN + (
            1.0 - 2.0 * POLICY_MEAN_MIN
        ) * torch.sigmoid(mean_logits)
        return carrier_beta_shapes(mean)

    def forward(self, observations):
        features = self.trunk(observations)
        policy_features = self.policy_mlp(features)
        mean_logits = self.actor_mean(policy_features).float()
        mean = POLICY_MEAN_MIN + (
            1.0 - 2.0 * POLICY_MEAN_MIN
        ) * torch.sigmoid(mean_logits)
        alpha, beta = carrier_beta_shapes(mean)
...
if __name__ == "__main__":
    ...
    def update_loss_model(
        ...
        with torch.no_grad():
            ...
            # Differentiate the actual one-dimensional carrier. Concentration
            # is a smooth function of mean, so both Beta shapes contribute to
            # the tangent score and Fisher information.
            inverse_mean = old_mean.reciprocal()
            inverse_complement = (1.0 - old_mean).reciprocal()
            smooth_weights = torch.softmax(
                torch.stack((inverse_mean, inverse_complement), dim=0)
                / CARRIER_SMOOTH_TEMPERATURE,
                dim=0,
            )
            concentration_derivative = (
                -smooth_weights[0] * inverse_mean.square()
                + smooth_weights[1] * inverse_complement.square()
            )
            alpha_derivative = (
                old_concentration + old_mean * concentration_derivative
            )
            beta_derivative = (
                -old_concentration
                + (1.0 - old_mean) * concentration_derivative
            )

            digamma_concentration = torch.digamma(old_concentration)
            alpha_score = (
                clamped_actions.log()
                - torch.digamma(old_alpha)
                + digamma_concentration
            )
            beta_score = (
            ...
            mean_score = (
                alpha_derivative * alpha_score
                + beta_derivative * beta_score
            )

            trigamma_concentration = torch.polygamma(1, old_concentration)
            fisher_alpha = (
                torch.polygamma(1, old_alpha) - trigamma_concentration
            )
            fisher_beta = (
                torch.polygamma(1, old_beta) - trigamma_concentration
            )
            fisher_cross = -trigamma_concentration
            mean_fisher = (
                alpha_derivative.square() * fisher_alpha
                + 2.0
                * alpha_derivative
                * beta_derivative
                * fisher_cross
                + beta_derivative.square() * fisher_beta
            )
            mean_direction = (
            ...
            # The local quadratic model supplies the initial global bracket.
            # Every candidate is mapped back onto the realizable carrier before
            # exact behavior-to-target KL is evaluated.
            quadratic_kl = (
            ...
            for _ in range(args.target_bracket_steps):
                ...
                ).clamp(TARGET_MEAN_MIN, 1.0 - TARGET_MEAN_MIN)
                bracket_alpha, bracket_beta = carrier_beta_shapes(bracket_mean)
                bracket_kl = beta_kl(
            ...
            for _ in range(args.target_bisection_steps):
                ...
                candidate_alpha, candidate_beta = carrier_beta_shapes(
                    candidate_mean
                )
            ...
            target_mean = target_unclamped_mean.clamp(
                TARGET_MEAN_MIN,
                1.0 - TARGET_MEAN_MIN,
            )
            target_alpha, target_beta = carrier_beta_shapes(target_mean)
            ...
            target_saturation_fraction = (
                (target_unclamped_mean <= TARGET_MEAN_MIN)
                | (target_unclamped_mean >= 1.0 - TARGET_MEAN_MIN)
            ).float().mean()
        ...
        # Target and online policies share the same one-dimensional carrier, so
        # this full forward KL has no independent concentration direction.
        target_fit_kl = beta_kl(
            target_alpha, target_beta, new_alpha, new_beta
        ).mean()
