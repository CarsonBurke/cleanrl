# V-MPO v27 — bias-free 51-bin HL-Gauss symlog critic.
#
# Controlled derivative of v24 replacing only its scalar MSE critic with a
# categorical HL-Gauss critic. Fifty-one bins partition the symmetric scalar
# range [-10, 10] uniformly in symlog space; sigma is 0.75 transformed-bin
# widths. The output head has no bias and starts with zero weights, giving an
# exactly uniform, zero-mean prior. Values decode as E[symexp(bin)] rather than
# symexp(E[bin]), avoiding the Jensen error. Reward normalization, D3 advantage
# scaling, and all policy machinery remain unchanged.
...
class Args:
    ...
    value_support_limit: float = 10.0
...
if __name__ == "__main__":
    ...
    def gae_model(
        ...
        next_values = hl_support.to_expected_scalar(
            next_value_logits.float()
