# V-MPO v3 FAST-DUALS — fixed-budget dual convergence on IterThink v24 Beta.
#
# v2 reproduced the paper's shared 1e-4 Adam but received only 3,205 learner
# updates in the 8M-step CleanRL budget. Eta moved at the Adam step-size ceiling,
# leaving the selected-action weights concentrated on 6-26 effective samples;
# the Beta concentration KL also remained 40-50x above its constraint. The
# critic converged, but the E-step and M-step duals never reached their intended
# optimization regime.
#
# v3 changes one dimension: agent parameters retain the paper's fixed 1e-4
# learning rate while eta and both KL multipliers use a 1e-2 parameter group.
# The V-MPO objective, raw n-step advantages, top-half selection, target period,
# batch, PopArt critic, and one-update-per-fresh-rollout contract are unchanged.
# This is an explicit 8M-budget optimizer adaptation, not a reported paper
# hyperparameter.
#
# Diagnostics now expose the exact E-step KL and eta stationarity residual,
# weight concentration, both KL residuals, dual update sizes, critic RMSE and
# explained variance, and Beta concentration/variance. Network matmuls remain
# BF16; duals, distribution math, PopArt, returns, and diagnostics remain FP32.
