# V-MPO v29 — Dreamer-aligned moment-matched HL-Gauss critic.
#
# Controlled derivative of v28 correcting its categorical coordinates and
# target mean. Fifty-one bucket centers are symexp-spaced from -20,000 through
# exact zero to 20,000, matching Dreamer3's bucket construction. Scalar values
# use Dreamer3's symmetric paired expectation, so a uniform prediction is
# exactly zero. Gaussian labels remain 0.75 coordinate-bin widths, then receive
# a minimum-KL exponential tilt that preserves the raw scalar target exactly,
# removing the symlog Jensen bias. Reward normalization and PopArt remain absent.
