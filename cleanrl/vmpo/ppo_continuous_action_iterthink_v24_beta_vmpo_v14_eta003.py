# V-MPO v14 eta=.03 — wider nonparametric KL budget ablation of v10 GAE.
#
# This isolated ablation raises epsilon_eta from 0.01 to 0.03. The hypothesis is
# that a wider nonparametric KL budget gives the exact eta solve enough freedom
# to concentrate more weight on the top-half advantages and improve policy
# progress without changing the parametric trust-region constraints.
#
# Each temporal-difference residual bootstraps from the online critic at the
# transition's actual next observation. Recursion stops at both termination and
# truncation boundaries; termination suppresses value bootstrap, while
# truncation retains bootstrap from the final pre-reset observation. The critic
# regresses the matching lambda return, `GAE + rollout value`.
#
# Exact eta, seeded phase staggering, the one-epoch joint update, target-policy
# ownership, PopArt, and the full-width ReLU-squared IterThink Beta model remain
# unchanged.
