# V-MPO v32 — KL-gated Polyak target.
#
# Controlled derivative of v31 replacing frequent hard copies with a standard
# Polyak target update. When fresh-rollout full KL(target || learner) reaches the
# behavior radius, the target moves 10% toward the exact pre-step learner. This
# preserves V31's immediate KL feedback without resetting the trust anchor.
# A learner inside the radius is copied only at the 100-rollout fallback. The
# critic, reward normalization, and all other policy machinery are unchanged.
...
class Args:
    ...
    # V30's successful adaptive target moved about once per ten batches.
    target_polyak_tau: float = 0.1
...
if __name__ == "__main__":
    ...
    if not 0.0 < args.target_polyak_tau < 1.0:
        raise ValueError("target_polyak_tau must be in (0, 1)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        candidate_kl_is_finite = np.isfinite(candidate_full_kl_value)
        update_for_full_kl = (
            candidate_kl_is_finite
            and candidate_full_kl_value >= args.target_kl_threshold
        )
        update_for_max_age = (
            candidate_kl_is_finite
            and not update_for_full_kl
            and target_rollout_batches + 1 >= args.target_update_period
        )
        target_updated = update_for_full_kl or update_for_max_age
        target_update_fraction = 0.0
        if target_updated:
            target_update_fraction = (
                args.target_polyak_tau if update_for_full_kl else 1.0
            )
            with torch.no_grad():
                for target_parameter, candidate_parameter in zip(
        ...
        if target_updated:
            # The target moved toward the pre-step candidate; this optimizer
            # step leaves the online policy one completed update beyond it.
        ...
        if should_log:
            ...
            writer.add_scalar(
                "vmpo/target_updated",
                float(target_updated),
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_updated_for_full_kl",
                float(update_for_full_kl),
                global_step,
            )
            writer.add_scalar(
            ...
            writer.add_scalar(
                "vmpo/target_update_fraction",
                target_update_fraction,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_updated_for_max_age",
                float(update_for_max_age),
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_updates",
