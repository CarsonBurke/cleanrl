# V-MPO (Song, Abdolmaleki et al. 2019, arXiv:1909.12238) on the v24 beta
# HL-Gauss symlog network. FAITHFUL to the paper's policy improvement, replacing
# PPO's clipped policy gradient entirely with V-MPO's E-step / M-step.
#
# WHAT V-MPO IS (and what PPO's clipped surrogate is NOT). V-MPO does NOT reshape
# advantages and feed them to a policy gradient. It is a weighted MAXIMUM-LIKELIHOOD
# (behavioral-cloning-toward-elite-actions) update derived from EM on an RL-as-
# inference objective:
#
#   E-step (nonparametric):  keep the TOP-HALF advantages in the (mini)batch (D~),
#     form sample weights  psi(s,a) = softmax_{D~}( A(s,a) / eta )  (normalized over
#     the top half; bottom half gets EXACTLY zero weight).
#   M-step (parametric):  Lpi(theta) = - sum_{D~} psi(s,a) * log pi_theta(a|s)
#     -- the weight multiplies LOG-PI, there is no advantage factor and no ratio.
#
# Temperature eta is LEARNED via the convex dual (Eq.4):
#   Leta(eta) = eta*eps_eta + eta*log( mean_{D~} exp(A/eta) ),
# which adapts eta to a target KL eps_eta on the nonparametric distribution.
#
# A decoupled KL TRUST REGION replaces PPO's clip (Eq.5), with its own learned
# Lagrange multiplier alpha and target eps_alpha:
#   Lalpha = mean_D [ alpha*(eps_alpha - sg[KL]) + sg[alpha]*KL ],  KL = KL(pi_old || pi_theta).
# (Paper splits mean/cov KL for GAUSSIAN policies; we use a BETA policy so a single
# combined Beta-KL with one alpha, like the discrete case.)
#
# Advantage A = G - V (we reuse the existing GAE advantage = n-step-return analogue),
# used RAW: no rankgauss, no norm_adv -- eta sets the scale. Value head keeps the
# d4-HL-Gauss symlog distributional critic (the analogue of the paper's PopArt
# n-step value regression). Top-half + softmax normalization are per-MINIBATCH,
# exactly as the paper's implementation note prescribes. Target policy = the rollout
# (old) policy, fixed across the update epochs (the paper's target network, whose
# update period maps to one PPO iteration). Continuous-control inits from Table 1/7:
# eta0=1.0, alpha0=1.0, eps_eta~0.1, eps_alpha~0.01. No entropy bonus, no PPO clip,
# no target_kl early-stop (the alpha trust region subsumes it).
#
# The shared-trunk dual-backward decoupled grad clip (value vs policy) is kept --
# it balances grads on the shared backbone and is orthogonal to the V-MPO objective.
#
...
class Args:
    ...
    norm_adv: bool = False           # V-MPO: raw advantages into exp(A/eta); eta sets scale
    ...
    # V-MPO uses RAW advantages (eta sets the scale): identity transform, no norm.
    adv_transform: str = "v10"

    # ---- V-MPO (arXiv:1909.12238). Replaces the PPO clipped surrogate. ----
    vmpo: bool = True
    vmpo_topk_frac: float = 0.5      # E-step: keep this fraction of highest advantages (D~)
    vmpo_eps_eta: float = 0.1        # temperature KL target eps_eta (Eq.4); Table 7 continuous
    vmpo_eps_alpha: float = 0.01     # trust-region KL target eps_alpha (Eq.5); single Beta-KL
    vmpo_init_eta: float = 1.0       # initial temperature (Table 1, continuous)
    vmpo_init_alpha: float = 1.0     # initial trust-region multiplier (Table 1, continuous)
    vmpo_eta_floor: float = 1e-8     # project eta > 0 after each dual step
    vmpo_alpha_floor: float = 1e-8   # project alpha > 0 after each dual step
    vmpo_dual_lr: float = 1e-2       # lr for the (eta, alpha) dual optimizer
...
class Agent(nn.Module):
    ...
    def beta_params(self, x):
        """Beta(alpha, beta) concentrations for the current policy (V-MPO old-policy
        snapshot during rollout)."""
        actor_feat, _ = self._trunks(x)
        alpha = 1.0 + F.softplus(self.actor_alpha_head(actor_feat))
        beta = 1.0 + F.softplus(self.actor_beta_head(actor_feat))
        return alpha, beta

    def vmpo_forward(self, x, z):
        """V-MPO update forward: recompute log pi_theta(z|s), the value logits, and
        the current Beta(alpha, beta) distribution (needed for the KL trust region).
        Beta only (V-MPO variant); the constant rescale Jacobian drops out of log_prob
        differences/KL so we use the native-z Beta log_prob directly."""
        actor_feat, critic_feat = self._trunks(x)
        alpha = 1.0 + F.softplus(self.actor_alpha_head(actor_feat))
        beta = 1.0 + F.softplus(self.actor_beta_head(actor_feat))
        dist = Beta(alpha, beta)
        ...
        value_logits = self.critic_head(critic_feat)
        return log_prob, value_logits, dist
...
if __name__ == "__main__":
    ...
    # V-MPO Lagrange multipliers: temperature eta (Eq.4) and trust-region alpha
    # (Eq.5). Learned by their own dual losses; projected to stay positive.
    if args.vmpo:
        vmpo_eta = torch.tensor(float(args.vmpo_init_eta), device=device, requires_grad=True)
        vmpo_alpha = torch.tensor(float(args.vmpo_init_alpha), device=device, requires_grad=True)
        vmpo_dual_optimizer = optim.Adam([vmpo_eta, vmpo_alpha], lr=args.vmpo_dual_lr)
    ...
    # V-MPO: snapshot the OLD (rollout) Beta policy params for the KL trust region.
    old_alphas = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device)
    old_betas = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if args.vmpo:
                    a_old, b_old = agent.beta_params(next_obs)
                    old_alphas[step] = a_old
                    old_betas[step] = b_old
            actions[step] = action
            latent_zs[step] = z
        ...
        if args.vmpo:
            ...
            b_old_alphas = old_alphas.reshape((-1,) + act_shape)
            b_old_betas = old_betas.reshape((-1,) + act_shape)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.vmpo:
                    # === V-MPO E-step / M-step (replaces the PPO clipped surrogate) ===
                    newlogprob, value_logits, new_dist = agent.vmpo_forward(
                    ...
                    # E-step: keep the TOP-HALF advantages (D~); softmax weights psi over
                    # them (sums to 1; bottom half -> exactly zero). Per-minibatch, per the
                    # paper's implementation note. psi is a fixed weight => fully detached.
                    adv_mb = b_advantages[mb_inds].detach()
                    k = max(1, int(round(args.vmpo_topk_frac * adv_mb.numel())))
                    top_adv, top_idx = torch.topk(adv_mb, k)
                    # Use float() snapshots of the duals as fixed coefficients in the
                    # theta-graph: the dual optimizer.step()/clamp_ mutate eta/alpha
                    # in-place below, which would otherwise version-clash with tensors
                    # saved for pg_loss.backward() (a python float shares no storage).
                    eta_coef = float(vmpo_eta.detach())
                    alpha_coef = float(vmpo_alpha.detach())
                    psi = torch.softmax(top_adv / eta_coef, dim=0).detach()
                    l_pi = -(psi * newlogprob[top_idx]).sum()   # weighted max-likelihood (Eq.3)

                    # Temperature dual (Eq.4): trains eta only (advantages detached).
                    # log mean exp(A/eta) = logsumexp(A/eta) - log(k), numerically stable.
                    l_eta = vmpo_eta * args.vmpo_eps_eta + vmpo_eta * (
                        torch.logsumexp(top_adv / vmpo_eta, dim=0) - log(k)
                    )

                    # KL trust region (Eq.5) over the FULL minibatch, KL(pi_old || pi_theta).
                    old_dist = Beta(b_old_alphas[mb_inds], b_old_betas[mb_inds])
                    kl = kl_divergence(old_dist, new_dist).sum(1)
                    l_kl_theta = (alpha_coef * kl).mean()                               # trains theta
                    l_alpha = (vmpo_alpha * (args.vmpo_eps_alpha - kl.detach())).mean()  # trains alpha

                    # Dual step (eta, alpha) on their own graph (disjoint from theta).
                    vmpo_dual_optimizer.zero_grad(set_to_none=True)
                    (l_eta + l_alpha).backward()
                    vmpo_dual_optimizer.step()
                    with torch.no_grad():
                        vmpo_eta.clamp_(min=args.vmpo_eta_floor)
                        vmpo_alpha.clamp_(min=args.vmpo_alpha_floor)

                    # Main (theta) policy loss = weighted ML + KL penalty (alpha detached).
                    pg_loss = l_pi + l_kl_theta
                    vmpo_kl_mean = kl.mean().detach()
                    vmpo_ess = 1.0 / (psi.pow(2).sum() * k + 1e-8)
                else:
            ...
            if not args.vmpo and args.target_kl is not None and approx_kl > args.target_kl:
                break
        ...
        if args.vmpo:
            # eta = temperature (should settle so the nonparametric KL ~ eps_eta);
            # alpha = trust-region multiplier (rises if KL>eps_alpha); vmpo_kl =
            # realized KL(old||new); ess = effective weight count / k (1=uniform top
            # half, ->0 = winner-take-all).
            writer.add_scalar("vmpo/eta", vmpo_eta.item(), global_step)
            writer.add_scalar("vmpo/alpha", vmpo_alpha.item(), global_step)
            writer.add_scalar("vmpo/kl_old_new", vmpo_kl_mean.item(), global_step)
            writer.add_scalar("vmpo/weight_ess_frac", vmpo_ess.item(), global_step)
            writer.add_scalar("vmpo/topk_adv_min", top_adv.min().item(), global_step)
        if auto_alpha:
