# V-MPO v60 — online actor with an EMA KL anchor.
#
# v30 acts with a frozen target that is copied from the learner every ten
# batches. At PPO batch geometry the copy trigger never fires and the actor
# stays at initialization for millions of steps (v59 periodic). Removing the
# target entirely (v59 per-batch) deletes the trust region: the KL gradient at
# the anchor is zero, both duals decay, and the Beta concentration runs away.
#
# v60 separates the two roles. The online policy always acts, so data is never
# stale, and the M-step prior is an exponential moving average of the online
# parameters (tau per learner update) that is only ever used to evaluate the
# decoupled Beta KL. A lagged anchor resists consistent drift such as variance
# collapse quadratically in the lag while barely resisting zero-mean noise,
# which is what the frozen target was silently doing in v30. tau = 1 recovers
# the per-batch trust region exactly.
#
# Everything else is v30: single full-batch step per rollout, exact eta
# bisection, decoupled mean/concentration duals, HL-Gauss critic.
