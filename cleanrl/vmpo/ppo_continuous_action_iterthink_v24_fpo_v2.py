# FPO v2 — one-step advantage-weighted OT flow on the v30 IterThink stack.
#
# v1 NaN'd by ~90k steps. Reverse-time Euler on ReLU^2 velocities produced
# |a|~100, then four PPO-clip epochs on an unclipped CFM ratio exploded the
# weights. ClipAction cannot save a NaN controller.
#
# This version uses FPO's actual first-order update, which is what a v30-style
# single full-batch step implements: stop-grad advantages times the CFM loss.
# No likelihood ratio, no clip, no reuse. Flow is forward OT (t=0 noise to
# t=1 action) with a velocity target. The ODE lives in pre-tanh space; env
# actions are affine-tanh mapped into the Box. The last velocity layer uses
# v30's actor scale.
#
# Kept from v30: IterThink trunk, Dreamer HL-Gauss critic, observation and
# reward normalization, GAE, percentile advantage scale, phase staggering,
# one Adam step per rollout, BF16 compiled full-graph training.
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        self.velocity_out = layer_init(nn.Linear(256, action_dim), std=0.01)
        ...
        t_path = torch.linspace(0.0, 1.0, args.flow_steps + 1)
