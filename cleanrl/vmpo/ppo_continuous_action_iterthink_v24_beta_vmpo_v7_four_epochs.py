# V-MPO v7 FOUR-EPOCH — repeated constrained fitting on fresh IterThink data.
#
# The V-MPO paper defines one joint value-plus-policy-improvement objective over
# a shared representation. SEED RL exposes generic `epochs_per_step` reuse for
# V-MPO by repeating that complete objective, not by applying unconstrained
# value-only gradients through the policy's shared trunk. This version performs
# four full-batch epochs per fresh rollout.
#
# Every epoch recomputes online policy/value outputs and all V-MPO dual losses
# against fixed rollout actions, target-policy parameters, advantages, returns,
# and PopArt statistics. The target policy still advances every 100 fresh
# rollout batches. This gives the critic and the under-equilibrated duals four
# optimizer steps per 2,496 samples while keeping shared-trunk policy movement
# inside the KL-constrained objective.
#
# The model remains full-width IterThink Beta with ReLU-squared heads, global
# RLax E-step reductions, Adam at 3e-4, and no separate policy evaluator.
...
if __name__ == "__main__":
    ...
    def bootstrap_returns_model(
        ...
        for reverse_step in reversed(range(args.num_steps)):
            ...
            continuing_return = reward_batch[reverse_step] + args.gamma * running_return
    ...
    def update_loss_model(
        ...
        # RLax selects every sample at or above the kth threshold, so ties at
        # the boundary can make the selected set larger than the nominal k.
        topk_threshold = torch.sort(advantages).values[-args.topk_size].detach()
        selected = advantages >= topk_threshold
        selected_count = selected.sum().to(advantages.dtype)

        policy_logits = torch.where(
            selected,
            advantages / eta.detach(),
            -torch.inf,
        )
        ...
        temperature_logits = torch.where(
            selected,
            advantages / eta,
            -torch.inf,
        )
        temperature_loss = eta * (
            args.epsilon_eta
            + torch.logsumexp(temperature_logits, dim=0)
            - selected_count.log()
        )
        ...
        selected_log_weights = torch.where(selected, log_weights.detach(), 0.0)
        temperature_kl = (
            weights * (selected_log_weights + selected_count.log())
        ).sum()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for _ in range(1, args.update_epochs):
            del metrics
            if args.compile:
                torch.compiler.cudagraph_mark_step_begin()
            total_loss, metrics = update_loss_model(
        ...
        if should_log:
            ...
            writer.add_scalar(
                "vmpo/optimizer_updates", iteration * args.update_epochs, global_step
            )
            writer.add_scalar("vmpo/learner_batches", iteration, global_step)
