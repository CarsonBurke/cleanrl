# V-MPO v17 vectorized observation normalization — performance-only v13.
#
# This implementation is behaviorally equivalent to v13 observation
# normalization, but keeps its 64 independent per-environment running statistics
# in arrays and updates all ordinary environment rows in one NumPy hot path.
# Terminal observations are updated and normalized before same-step autoreset
# observations, matching the removed per-environment Gymnasium wrappers.
#
# Observations are clipped to [-10, 10]. Rewards remain unnormalized so PopArt
# continues to normalize critic targets while preserving raw return units.
#
# Exact eta, GAE (including truncation bootstrap observations), seeded phase
# staggering, the one-epoch joint update, target-policy ownership, PopArt, and
# the full-width ReLU-squared IterThink Beta model remain unchanged.
