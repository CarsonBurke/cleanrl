# V-MPO v63 — packed asynchronous bootstrap staging, unchanged v62 algorithm.
# Isolated v60 optimizer change: covariance alpha leaves Adam and follows
# alpha <- max(floor, alpha + (KL/epsilon - 1)/target_period).
# A unit relative violation changes alpha by one over a target hold, instead
# of raw-unit Adam's slow ~1e-4 steps. This is projected dual ascent, NOT an
# exact alpha solve or the paper's optimizer. Mean alpha/actor Adam unchanged.
# Optional v30-compatible reward normalization is a separate paired ablation.
# Keep 64x39, eight replicas, fixed100 holds, std .3, solved eta and batch PopArt.
# Runtime-only successor: event-protected pinned transfers replace blocking
# tail/index/final-observation uploads. Preserve every value-forward batch shape
# and the shifted GAE recurrence; retain PERI's return-phase graph boundary.
...
class Agent(nn.Module):
    def __init__(self, obs_dim, action_dim, initial_std=0.3):
        super().__init__()
...
@torch.no_grad()
def update_covariance_dual(alpha, covariance_kl, epsilon, target_period):
    """Projected ascent of the same dual, with step 1/(period * epsilon).

    Consume pre-optimizer KL from the fresh loss, after its backward completes.
    Unlike Adam, this update responds to violation magnitude in budget units.
    """
    alpha.add_(covariance_kl.detach() / epsilon - 1.0, alpha=1.0 / target_period)
    return alpha.clamp_(min=DUAL_FLOOR)


def vmpo_loss(agent, alpha_mean, alpha_covariance, observations, actions, old_mean, old_std,
              advantages, normalized_returns, args):
    mean, std, values = agent(observations)
    alpha_mean = alpha_mean.clamp_min(DUAL_FLOOR)
    ...
    covariance_penalty = alpha_covariance.detach() * covariance_kl
...
def main():
    ...
    with ExitStack() as resources:
        ...
        writer.add_text("paper_reference", "arXiv:1909.12238, Eqs 1-6/25-27, Appendix F/Table 7. "
                        f"Raw observations; reward normalization={args.normalize_rewards}; n-step (lambda=1); "
                        f"solved shared eta; {args.num_replicas} env-axis replicas; "
                        ...
                        "PopArt uses one batch-moment EMA per learner update, before fitting; initial moments (0,1). "
                        "v60 solved shared eta retained. v62 covariance alpha uses projected ascent "
                        "(KL/epsilon - 1)/target_period outside Adam; pre-step KL, no extra policy updates. "
                        "Optional reward normalization follows v30: per-env discounted-return RMS and clip10, "
                        "including phase warmup; episode and evaluation returns remain raw. "
                        "Runtime departures: Gymnasium-v4, phase warmup, host FP32 actor, CUDA FP32 learner. "
                        "v63 runtime: packed pinned tail/truncation uploads, event-protected asynchronous rollout "
                        "transfer; unchanged FP32 value batch shapes and shifted return recurrence. "
        ...
        alpha_mean = nn.Parameter(torch.tensor(args.initial_alpha_mean, device=device))
        alpha_covariance = torch.tensor(args.initial_alpha_covariance, device=device).clamp_min(DUAL_FLOOR)
        optimizer = optim.Adam([*agent.parameters(), alpha_mean], lr=args.learning_rate,
                               betas=(0.9, 0.999), eps=1e-8, fused=True)
        reward_normalizer = VectorRewardNorm(args.num_envs, args.gamma) if args.normalize_rewards else None
        ...
        transfer = RolloutTransfer(args.num_steps, args.num_envs, obs_shape, device, non_blocking=True,
        ...
        def loss_fn(observations, actions, mean, std, advantages, targets):
            return vmpo_loss(agent, alpha_mean, alpha_covariance, observations, actions,
                             mean, std, advantages, targets, args)
        ...
        covariance_dual_fn = update_covariance_dual
        if args.compile:
            ...
            covariance_dual_fn = graph_compile(covariance_dual_fn)
        host_actor.refresh()
        ...
        warm = run_phase_warmup(envs, obs_norm=None, rew_norm=reward_normalizer,
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            for step in range(args.num_steps):
                ...
                with timer.span("normalization", use_cuda=False):
                    learning_rewards = (reward_normalizer.normalize(rewards, terms)
                                        if reward_normalizer is not None else rewards)
                with timer.span("transfer", use_cuda=False):
                    ...
                    transfer.push(step, learning_rewards, terms, truncs, observations=observations,
            ...
            with timer.span("returns"), torch.no_grad():
                ...
                tail_values, truncation_values = bootstraps.resolve_with_tail(
                    value_fn, next_obs, device, batch_size=args.num_envs)
                ...
                normalized_returns = popart_fn(returns, args.popart_rate,
                                                args.popart_std_min, args.popart_std_max)
            ...
            with timer.span("update"):
                ...
                optimizer.step()
                with torch.no_grad():
                    alpha_mean.clamp_(min=DUAL_FLOOR)
                    covariance_dual_fn(alpha_covariance, metrics[4], args.epsilon_alpha_covariance,
                                       args.target_update_period)
            if should_log:
                with timer.span("diagnostics"), torch.no_grad():
                    ...
                    logged = gather_metrics({**dict(zip(metric_names, metrics)), **dict(zip(post_names, post)),
                                             "vmpo/alpha_mean": alpha_mean,
                                             "vmpo/alpha_covariance": alpha_covariance,
                ...
                if reward_normalizer is not None:
                    logged["reward_norm/scale_mean"] = float(np.sqrt(
                        reward_normalizer.variances + reward_normalizer.epsilon).mean())
            ...
            if iteration % args.eval_interval == 0 or iteration == args.num_iterations:
                ...
                if args.save_model:
                    torch.save({"args": asdict(args), "agent": agent.state_dict(),
                                ...
                                "alpha_mean": alpha_mean.detach(), "alpha_covariance": alpha_covariance,
                                "reward_normalizer": None if reward_normalizer is None else {
                                    key: getattr(reward_normalizer, key).copy()
                                    for key in ("returns", "means", "variances", "counts")},
