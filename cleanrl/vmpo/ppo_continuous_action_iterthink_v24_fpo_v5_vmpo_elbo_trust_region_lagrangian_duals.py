# FPO v5 — ELBO-gap trust region with real Lagrangian duals on the v4 stack
# (McAllister et al. 2025, arXiv:2507.21053; V-MPO v30 base).
#
# v4 post-mortem (job 4552: 75 @4M vs v30's 5127, then collapse from 6M).
# Slow: v30's Gaussian M-step crosses the 0.0071 mean-KL budget within the
# first 10 batches of every window (promotion at age 9, 110 promotions by
# 4M). v4's flow plateaued at mean KL ~0.005 inside each window and promoted
# only at max age 100 (18 promotions by 4M). The plateau is an equilibrium
# between the ELBO-ratio objective, whose moment gradient is ~E[t^2]=1/3 of a
# log-likelihood's, and the moment penalty at alpha~1: the duals sit in the
# policy Adam at lr 3e-4 and never move, so the trust region is a fixed
# penalty, not a constraint. v30 is fast despite it; the flow is not.
# Collapse: on negatively weighted samples the ratio objective rewards
# raising L_online without bound; the gradient decays as exp(-L) but Adam
# renormalizes it into a constant push. The velocity net inflated its error
# at the target's (x_t, t) points (online CFM 265 vs target 5, |pre-tanh| up
# to 40), invisible to the K-draw moment KL until the sampling paths broke
# (moment KL 143 with alpha_mean still 1.02).
#
# v5:
# 1. ELBO-gap trust region: mean_i (L_online,i - L_target,i) <= eps on the
#    same shared pairs as the objective (FPO's KL(target||online) estimate).
#    Per sample the loss coefficient on grad L_online,i is (c_i r_i + a/B):
#    for a pushed-down sample it flips sign at r_i = alpha_elbo, so the
#    push-away halts at a ratio floor of alpha_elbo instead of running to
#    infinity. That is PPO's lower clip derived from the Lagrangian, with the
#    floor set by the constraint rather than a constant.
# 2. Real duals: alpha_mean, alpha_spread, alpha_elbo leave the policy Adam
#    and take projected dual ascent in relative units after each step,
#      alpha <- max(alpha + kappa (KL / eps - 1), 0),  kappa = 0.1,
#    so a slack constraint releases the objective within ~10 batches and a
#    violated one builds a restoring force at a rate set by the violation.
# 3. Promotion is checked every batch instead of every log interval; the
#    mean-KL trigger is otherwise v30's.
# 4. 32 CFM samples (16 antithetic pairs): the coupled moment draws dominate
#    velocity-net cost (K x 10 Euler steps x 2 flows), so this is near free.
#
# Hypothesis: with the constraint actually enforced and the moment penalty
# released while slack, the flow M-step reaches the promotion budget in
# about ten batches like v30 and the run tracks v30's promotion cadence
# without the push-away blow-up.
import copy
import os
import random
import time
...
class Args:
    ...
    epsilon_alpha_elbo: float = 0.007071067811865476
    initial_alpha_elbo: float = 1.0
    # Projected dual ascent step in relative units (KL / eps - 1) per batch.
...
if __name__ == "__main__":
    ...
    duals = torch.tensor(
        [
            ...
            args.initial_alpha_elbo,
        ],
        device=device,
    )
    dual_budgets = torch.tensor(
        [
            args.epsilon_alpha_mean,
            args.epsilon_alpha_spread,
            args.epsilon_alpha_elbo,
        ],
        device=device,
    )
    ...
    MEAN_KL_METRIC_INDEX = METRIC_NAMES.index("vmpo/mean_kl")
    DUAL_METRIC_INDICES = torch.tensor(
        [
            MEAN_KL_METRIC_INDEX,
            METRIC_NAMES.index("vmpo/spread_kl"),
            METRIC_NAMES.index("fpo/elbo_kl"),
        ],
        device=device,
    )
    ...
    def update_loss_model(
        ...
        # ELBO gap = FPO's KL(target || online) estimate on the same pairs;
        # its penalty gives every pushed-down sample a ratio floor of alpha.
        elbo_kl = -log_ratio.mean()
        elbo_constraint_loss = alpha_elbo * elbo_kl
        ...
        spread_constraint_loss = alpha_spread * spread_kl_average
        with torch.no_grad():
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        total_loss.backward()
        optimizer.step()
        # Projected dual ascent on the batch's own constraint measurements.
        with torch.no_grad():
            measured_kls = metrics[DUAL_METRIC_INDICES]
            duals.add_(args.dual_step_size * (measured_kls / dual_budgets - 1.0))
            duals.clamp_(min=0.0)
        target_age_batches += 1

        promote_for_mean_kl = bool(
            metrics[MEAN_KL_METRIC_INDEX].item() >= args.epsilon_alpha_mean
        )
        ...
        if should_log:
            ...
            writer.add_scalar("vmpo/alpha_elbo", alpha_elbo_value, global_step)
            writer.add_scalar(
                "vmpo/alpha_elbo_delta", alpha_elbo_delta_value, global_step
            )
            writer.add_scalar(
