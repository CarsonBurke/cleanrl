# V-MPO v64 — v62 with K M-step epochs per rollout batch.
#
# Algorithm is v62's (online actor, EMA KL anchor, exact eta by bisection,
# decoupled Beta trust region with Lagrangian duals, HL-Gauss critic,
# percentile-scaled GAE advantages, switchable E-step options) with the
# update split into its two natural parts:
#   estep_model(advantages)      -> loss_weights (once per batch, no grad)
#   mstep_model(batch, weights)  -> loss          (--update-epochs times)
# With --update-epochs 1 the run is v62 op for op. The duals step with every
# M-step; the anchor moves once per batch, so inside a batch the trust region
# is measured against a fixed prior, as in PPO's clipped ratio to the rollout
# policy across epochs. --anneal-lr decays the agent learning rate linearly.
#
# Why. Every 50M V-MPO run in this lineage plateaus at 9.3-9.5k while the
# repo's PPO baselines on the same stack pass 14k and are still climbing.
# At 8M the two are level (~7.3k). The dominant structural difference is the
# optimisation budget per sample: PPO takes ten full-batch steps per 32768-
# sample batch at 32x the learning rate; this V-MPO takes one Adam step per
# 2496-sample batch. The nonparametric E-step does not depend on the online
# policy, so repeating the M-step against fixed weights is the paper's
# objective optimised further, not a different objective; the KL trust
# region is what makes repeated steps safe.
#
# Execution is v62's fast stack unchanged (mode "default" compile; see v62).
#
import copy
import math
import os
...
class Args:
    ...
    # M-step gradient steps per rollout batch. 1 reproduces v62 exactly; the
    # E-step weights are computed once per batch and reused by every epoch.
    update_epochs: int = 1
    # Linear learning-rate decay to zero over training (agent group only;
    # the dual learning rate is held fixed).
...
# Orders match the two metric stacks returned by estep_model / mstep_model.
ESTEP_METRIC_NAMES = (
...
def validate_args(args):
    ...
    if args.compile and args.compile_mode == "reduce-overhead":
        # Cudagraph trees invalidate the captured rollout graph (see v62) and,
        # here, would also recycle estep_model's outputs before mstep_model
        # consumes them across epochs.
        raise ValueError("compile_mode reduce-overhead is unsupported; use default")
...
def main():
    ...
    try:
        ...
        def estep_model(advantages):
            """Nonparametric E-step for one batch: top-k selection, exact eta by
            bisection, optionally centered loss weights. Independent of the
            online policy, so it runs once per batch and its weights are reused
            by every M-step epoch."""
            with torch.no_grad():
            ...
            return loss_weights, estep_metrics
        ...
        def mstep_model(
            ...
            """One M-step gradient evaluation: weighted max-likelihood under the
            fixed E-step weights, decoupled KL trust region to the anchor with
            Lagrangian duals, and the critic loss."""
            with torch.autocast(device_type="cuda", dtype=autocast_dtype):
            ...
            return total_loss, mstep_metrics
        ...
        if args.compile:
            ...
            estep_model = torch.compile(
                estep_model, mode=args.compile_mode, fullgraph=True, dynamic=False
            )
            mstep_model = torch.compile(
                mstep_model, mode=args.compile_mode, fullgraph=True, dynamic=False
            )
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            split_cosine = None
            if args.anneal_lr:
                lr_now = args.learning_rate * (1.0 - (iteration - 1) / args.num_iterations)
            ...
            with timer.span("update"):
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                loss_weights, estep_metrics = estep_model(advantages)
                ...
                old_alpha = batch.policy["alpha"].flatten(0, 1)
                old_beta = batch.policy["beta"].flatten(0, 1)
                for _epoch in range(args.update_epochs):
                    if args.compile:
                        torch.compiler.cudagraph_mark_step_begin()
                    total_loss, mstep_metrics = mstep_model(
                        flat_observations,
                        flat_actions,
                        old_alpha,
                        old_beta,
                ...
                with torch.no_grad():
                    # The anchor moves once per batch, so the trust region within
                    # a batch is measured against a fixed prior across epochs.
                    torch._foreach_lerp_(anchor_params, online_params, args.anchor_ema_tau)

            if should_log:
                assert duals_before is not None
                named = (
                    dict(zip(ESTEP_METRIC_NAMES, estep_metrics))
                    | dict(zip(MSTEP_METRIC_NAMES, mstep_metrics))
                ) | {
