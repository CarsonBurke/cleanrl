# V-MPO v53 — v30 without percentile advantage scaling.
#
# Controlled v30 derivative removing only the D3-style divide of policy
# advantages by max(1, P95-P5 of rollout returns). Hypothesis: the exact
# fixed-KL eta solver is scale-equivariant, so the divide was near-neutral
# except for its floor, which breaks equivariance when the return spread is
# below 1. Raw GAE advantages now feed top-k selection and the E-step
# directly, matching the paper's unscaled advantages. Reward normalization,
# observation normalization, and the Dreamer moment-matched HL-Gauss critic
# are unchanged (see paper-alignment notes in FAMILY.md).
