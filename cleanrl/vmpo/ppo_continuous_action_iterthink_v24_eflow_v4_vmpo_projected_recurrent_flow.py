# EFLOW v4 — KL-projected recurrent spline flow on the V-MPO v30 carrier.
#
# eflow_v3 kept the exact flow likelihood and decoupled context, but its delayed
# projected dual was a bang-bang controller: alpha_transform repeatedly hit zero
# while the flow accumulated a large warp (|log det| 1.48, 6.5% saturation), then
# rebounded to 7.6. It reached only 6,088 at 8M versus v30's 7,746.
#
# v4 stores each rollout's target-base draw. The Beta carrier is fit directly
# to that latent action while a disjoint residual-flow loss fits the native
# action under the frozen target Beta, removing the coordinate competition that
# left v3's carrier too broad. The transform dual is replaced by post-step hard
# feasibility: accept the proposal, geometrically backtrack its parameter delta,
# or restore it according to exact-density KL on common target samples.
#
# The policy and critic also receive the previous native action as an augmented
# Markov state. The spline can therefore model cross-joint and temporal
# dependence with an exact conditional likelihood instead of temporally white
# shape noise. Episode boundaries reset the previous action to box center.
import copy
import os
import random
import time
...
class Args:
    ...
    # Maximum transform KL for the post-step functional projection.
    ...
    kl_samples: int = 8
    flow_backtrack_steps: int = 5
    flow_backtrack_ratio: float = 0.5
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        self.trunk = ThinkTrunk(
            augmented_dim, args.hidden, args.k_blocks, args.n_experts
        )
        ...
        self.flow_context = nn.Sequential(
            layer_init(nn.Linear(augmented_dim, 256)),
            ReLUSquared(),
    ...
    @staticmethod
    def augmented_input(observations, previous_native_actions):
        # Native actions are in [0, 1]; centering preserves the zero-input
        # initialization convention at episode boundaries.
        return torch.cat((observations, 2.0 * previous_native_actions - 1.0), dim=-1)

    def context(self, observations, previous_native_actions):
        augmented = self.augmented_input(observations, previous_native_actions)
        return self.flow_context(augmented)

    def policy(self, observations, previous_native_actions):
        augmented = self.augmented_input(observations, previous_native_actions)
        features = self.policy_mlp(self.trunk(augmented))
        alpha = 1.0 + F.softplus(self.actor_alpha(features))
        beta = 1.0 + F.softplus(self.actor_beta(features))
        return alpha, beta, self.flow_context(augmented)

    def forward(self, observations, previous_native_actions):
        augmented = self.augmented_input(observations, previous_native_actions)
        features = self.trunk(augmented)
        ...
        return alpha, beta, self.flow_context(augmented), value_logits

    def value_logits(self, observations, previous_native_actions):
        augmented = self.augmented_input(observations, previous_native_actions)
        features = self.value_mlp(self.trunk(augmented))
...
if __name__ == "__main__":
    ...
    if args.epsilon_alpha_transform <= 0.0:
        raise ValueError("epsilon_alpha_transform must be positive")
    if args.flow_backtrack_steps <= 0:
        raise ValueError("flow_backtrack_steps must be positive")
    if not 0.0 < args.flow_backtrack_ratio < 1.0:
        raise ValueError("flow_backtrack_ratio must be in (0, 1)")
    ...
    flow_parameters = tuple(agent.flow_context.parameters()) + tuple(
        agent.flow.parameters()
    )
    target_flow_parameters = tuple(target_agent.flow_context.parameters()) + tuple(
        target_agent.flow.parameters()
    )
    flow_candidates = tuple(torch.empty_like(parameter) for parameter in flow_parameters)
    flow_snapshots = tuple(torch.empty_like(parameter) for parameter in flow_parameters)

    autocast_dtype = torch.bfloat16

    def rollout_model(observations, previous_native_actions):
        with torch.autocast(device_type="cuda", dtype=autocast_dtype):
            target_alpha, target_beta, target_context = target_agent.policy(
                observations, previous_native_actions
            )
            value_logits = agent.value_logits(observations, previous_native_actions)
    ...
    def gae_model(
        ...
        transition_next_previous_actions,
        reward_batch,
        value_batch,
        terminations,
        boundaries,
        ...
        with torch.autocast(device_type="cuda", dtype=autocast_dtype):
            next_value_logits = agent.value_logits(
                ...
                transition_next_previous_actions.reshape(
                    (args.batch_size,) + transition_next_previous_actions.shape[2:]
                ),
            )
    ...
    def update_loss_model(
        observations,
        previous_native_actions,
        native_actions,
        rollout_base_actions,
        old_alpha,
        old_beta,
        old_log_probs,
        kl_base_samples,
        ...
        with torch.autocast(device_type="cuda", dtype=autocast_dtype):
            new_alpha, new_beta, new_context, value_logits = agent(
                observations, previous_native_actions
            )
            with torch.no_grad():
                old_context = target_agent.context(
                    observations, previous_native_actions
                )
            online_rollout_base, inverse_log_abs_det = agent.flow.inverse(
        ...
        base_policy_log_prob = beta_log_prob(
            new_alpha, new_beta, rollout_base_actions
        )
        flow_policy_log_prob = (
            beta_log_prob(old_alpha, old_beta, online_rollout_base)
            + inverse_log_abs_det
        )
        base_policy_loss = -(weights * base_policy_log_prob).sum()
        flow_policy_loss = -(weights * flow_policy_log_prob).sum()
        policy_loss = base_policy_loss + flow_policy_loss
        ...
        base_coordinate_shift = (
            online_rollout_base - rollout_base_actions
        ).abs().mean()
        ...
        metrics = torch.stack(
            (
                ...
                base_policy_loss.detach(),
                flow_policy_loss.detach(),
                base_coordinate_shift.detach(),
            )
        )
        return total_loss, metrics, transform_kl_average.detach()

    def transform_kl_model(
        observations,
        previous_native_actions,
        old_alpha,
        old_beta,
        ...
        """Measure target-base KL to the post-step online transform."""
        with torch.autocast(device_type="cuda", dtype=autocast_dtype):
            with torch.no_grad():
                old_context = target_agent.context(
                    observations, previous_native_actions
                )
            new_context = agent.context(observations, previous_native_actions)
            kl_count = kl_base_samples.shape[1]
            with torch.no_grad():
                kl_actions, target_forward_log_abs_det = target_agent.flow(
                    kl_base_samples,
                ...
                kl_actions = kl_actions.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            online_base, online_inverse_log_abs_det = agent.flow.inverse(
                kl_actions, new_context.unsqueeze(1).expand(-1, kl_count, -1)
            )
        old_alpha = old_alpha.unsqueeze(1)
        old_beta = old_beta.unsqueeze(1)
        with torch.no_grad():
            target_log_prob = (
                beta_log_prob(old_alpha, old_beta, kl_base_samples)
                - target_forward_log_abs_det
            )
        online_log_prob = (
            beta_log_prob(old_alpha, old_beta, online_base)
            + online_inverse_log_abs_det
        )
        return (target_log_prob - online_log_prob).mean()
    ...
    if args.compile:
        ...
        transform_kl_model = torch.compile(
            transform_kl_model,
            mode=args.compile_mode,
            dynamic=False,
            fullgraph=True,
    ...
    rollout_base_actions = torch.empty_like(native_actions)
    previous_actions = torch.empty(rollout_shape + action_shape, device=device)
    ...
    transition_next_previous_actions = torch.empty_like(previous_actions)
    ...
    native_action_center = torch.full(
        (args.num_envs,) + action_shape, 0.5, device=device
    )
    next_previous_action = native_action_center.clone()
    ...
    for warmup_step in range(1, args.initial_phase_warmup_steps + 1):
        ...
        with torch.no_grad():
            ...
            warmup_alpha, warmup_beta, warmup_context, _ = rollout_model(
                warmup_observation, next_previous_action
            )
        ...
        warmup_boundary = torch.as_tensor(
            np.logical_or(warmup_terminations, warmup_truncations),
            device=device,
            dtype=torch.bool,
        ).reshape((args.num_envs,) + (1,) * len(action_shape))
        next_previous_action.copy_(
            torch.where(
                warmup_boundary,
                native_action_center,
                warmup_native_action,
            )
        )
        ...
        for env_index in np.flatnonzero(scheduled_resets == warmup_step):
            ...
            next_previous_action[env_index].copy_(native_action_center[env_index])
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(args.num_steps):
            ...
            previous_actions[step].copy_(next_previous_action)
            with torch.no_grad():
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                old_alpha, old_beta, old_context, value = rollout_model(
                    next_observation, next_previous_action
                )
            ...
            rollout_base_actions[step].copy_(base_sample)
            ...
            transition_next_previous_actions[step].copy_(native_action)
            next_observation = torch.as_tensor(
                next_observation_np, device=device, dtype=torch.float32
            )
            boundary_tensor = torch.as_tensor(
                boundary, device=device, dtype=torch.bool
            ).reshape((args.num_envs,) + (1,) * len(action_shape))
            next_previous_action.copy_(
                torch.where(boundary_tensor, native_action_center, native_action)
            )
        ...
        if args.compile:
            torch.compiler.cudagraph_mark_step_begin()
        total_loss, metrics, transform_kl_measured = update_loss_model(
            observations.reshape((args.batch_size,) + observation_shape),
            previous_actions.reshape((args.batch_size,) + action_shape),
            native_actions.reshape((args.batch_size,) + action_shape),
            rollout_base_actions.reshape((args.batch_size,) + action_shape),
            flat_old_alphas,
            flat_old_betas,
        ...
        metrics = metrics.detach().clone()
        transform_kl_measured = transform_kl_measured.detach().clone()
        should_log = iteration % args.log_interval == 0 or iteration == 1
        duals_before = duals.detach().clone() if should_log else None
        pre_transform_kl_value = float(transform_kl_measured)
        if not np.isfinite(pre_transform_kl_value):
            raise RuntimeError("non-finite pre-step transform KL")
        optimizer.zero_grad(set_to_none=True)
        total_loss.backward()
        with torch.no_grad():
            for snapshot, parameter in zip(flow_snapshots, flow_parameters):
                snapshot.copy_(parameter)
        optimizer.step()
        with torch.no_grad():
            duals.clamp_(min=DUAL_FLOOR)
            for candidate, parameter in zip(flow_candidates, flow_parameters):
                candidate.copy_(parameter)

        flat_observations = observations.reshape(
            (args.batch_size,) + observation_shape
        )
        flat_previous_actions = previous_actions.reshape(
            (args.batch_size,) + action_shape
        )
        if args.compile:
            torch.compiler.cudagraph_mark_step_begin()
        with torch.no_grad():
            proposal_transform_kl = transform_kl_model(
                flat_observations,
        ...
        proposal_transform_kl_value = float(proposal_transform_kl)
        if not np.isfinite(proposal_transform_kl_value):
            raise RuntimeError("non-finite post-step transform KL")

        pre_transform_feasible = (
            pre_transform_kl_value <= args.epsilon_alpha_transform
        )
        projection_origins = (
            flow_snapshots if pre_transform_feasible else target_flow_parameters
        )
        flow_step_scale = 1.0
        accepted_transform_kl = proposal_transform_kl
        update_rejected = 0.0
        proposal_accepted = (
            proposal_transform_kl_value <= args.epsilon_alpha_transform
        )
        if not proposal_accepted:
            for backtrack_step in range(args.flow_backtrack_steps):
                flow_step_scale = args.flow_backtrack_ratio ** (backtrack_step + 1)
                with torch.no_grad():
                    for parameter, origin, candidate in zip(
                        flow_parameters, projection_origins, flow_candidates
                    ):
                        parameter.copy_(candidate).sub_(origin).mul_(
                            flow_step_scale
                        ).add_(origin)
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                with torch.no_grad():
                    trial_transform_kl = transform_kl_model(
                        flat_observations,
                        flat_previous_actions,
                        flat_old_alphas,
                        flat_old_betas,
                ...
                trial_transform_kl_value = float(trial_transform_kl)
                trial_accepted = (
                    np.isfinite(trial_transform_kl_value)
                    and trial_transform_kl_value <= args.epsilon_alpha_transform
                )
                if trial_accepted:
                    accepted_transform_kl = trial_transform_kl
                    break
            else:
                with torch.no_grad():
                    for parameter, origin in zip(
                        flow_parameters, projection_origins
                    ):
                        parameter.copy_(origin)
                flow_step_scale = 0.0
                accepted_transform_kl = (
                    transform_kl_measured
                    if pre_transform_feasible
                    else torch.zeros_like(transform_kl_measured)
                )
                update_rejected = 1.0
        flow_step_scale_tensor = torch.tensor(flow_step_scale, device=device)
        update_rejected_tensor = torch.tensor(update_rejected, device=device)
        ...
        if should_log:
            ...
            packed = torch.cat(
                (
                    ...
                    duals.detach(),
                    dual_delta,
                    proposal_transform_kl.view(1),
                    accepted_transform_kl.view(1),
                    flow_step_scale_tensor.view(1),
                    update_rejected_tensor.view(1),
            ...
            (
                ...
                base_policy_loss_value,
                flow_policy_loss_value,
                base_coordinate_shift_value,
                alpha_mean_value,
                alpha_concentration_value,
                alpha_mean_delta_value,
                alpha_concentration_delta_value,
                proposal_transform_kl_value,
                accepted_transform_kl_value,
                flow_step_scale_value,
                update_rejected_value,
                return_percentile_scale_value,
            ) = packed
            promote_for_mean_kl = mean_kl_value >= args.epsilon_alpha_mean
            promote_for_max_age = (
            ...
            writer.add_scalar(
                "flow/proposal_transform_kl",
                proposal_transform_kl_value,
                global_step,
            )
            writer.add_scalar(
                "flow/accepted_transform_kl",
                accepted_transform_kl_value,
                global_step,
            )
            writer.add_scalar("flow/step_scale", flow_step_scale_value, global_step)
            writer.add_scalar(
                "flow/update_rejected", update_rejected_value, global_step
            )
            writer.add_scalar(
                "flow/base_coordinate_shift",
                base_coordinate_shift_value,
                global_step,
            )
            writer.add_scalar(
                "losses/base_policy_loss", base_policy_loss_value, global_step
            )
            writer.add_scalar(
                "losses/flow_policy_loss", flow_policy_loss_value, global_step
            )
