# V-MPO v49 — v30 with UNSHARED actor/critic backbones.
#
# Sole delta vs v30 (which shares one ThinkTrunk between policy and value):
# separate actor_trunk / critic_trunk so the growing policy-NLL term cannot
# starve the critic representation late in training (the shared-trunk
# imbalance). Joint single-Adam update otherwise unchanged. 50M-step run to
# test whether backbone separation lifts the ~10k crawl.
...
class Agent(nn.Module):
    ...
    def policy(self, observations):
        features = self.policy_mlp(self.actor_trunk(observations))
        alpha = 1.0 + F.softplus(self.actor_alpha(features))
        beta = 1.0 + F.softplus(self.actor_beta(features))
        return alpha, beta

    def forward(self, observations):
        actor_features = self.actor_trunk(observations)
        policy_features = self.policy_mlp(actor_features)
