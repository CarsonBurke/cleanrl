# V-MPO v8 EXACT-ETA — one network epoch with an equilibrated E-step.
#
# A learned temperature eta controls the KL from the top-half nonparametric
# action weighting to the uniform behavior-sample distribution. With one Adam
# step per 2,496 transitions, eta can move only about one learning rate per
# rollout: v6 reached eta=1.879 and KL=4.992 against epsilon=0.01 after 8M
# steps. Four repeated epochs still left KL=2.246.
#
# The E-step temperature objective is scalar and convex. This version solves its
# stationarity condition, KL(weights || uniform)=epsilon_eta, by fixed-iteration
# bisection on every fresh rollout. The policy/value network and the two
# parametric-KL multipliers still receive exactly one joint Adam update. This
# removes optimizer-time-scale dependence from the E-step without replaying
# rollout data or changing the IterThink Beta model.
