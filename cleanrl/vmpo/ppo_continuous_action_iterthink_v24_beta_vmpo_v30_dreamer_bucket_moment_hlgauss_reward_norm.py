# V-MPO v30 — reward-normalized Dreamer moment-matched HL-Gauss critic.
#
# Controlled derivative of v29 restoring per-environment reward normalization
# while retaining its target-mean correction. Fifty-one bucket centers are
# symexp-spaced from -20,000 through exact zero to 20,000. Scalar values use
# Dreamer3's symmetric paired expectation, so a uniform prediction is exactly
# zero. Gaussian labels are minimum-KL tilted to preserve the raw scalar target,
# removing symlog Jensen bias. PopArt remains absent.
