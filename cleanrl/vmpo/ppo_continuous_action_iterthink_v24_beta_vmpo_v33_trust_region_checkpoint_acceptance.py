# V-MPO v33 — trust-region checkpoint acceptance.
#
# Controlled derivative of v30/v31 replacing distance-triggered target updates
# with conservative policy iteration. On each fresh target rollout, the exact
# pre-step learner is accepted only when full KL(target || learner) is within the
# behavior radius and its trajectory-level lower confidence bound improves the
# current V-MPO weighted log-likelihood objective. Infeasible or non-improving
# learners leave the target fixed so the KL dual can correct them.
...
if __name__ == "__main__":
    ...
    if args.num_envs < 2:
        raise ValueError("target gain confidence requires at least two environments")
    if args.target_kl_threshold <= 0.0:
        raise ValueError("target_kl_threshold must be positive")
    if args.target_gain_lcb_z < 0.0:
        raise ValueError("target_gain_lcb_z must be nonnegative")
    ...
    def update_loss_model(
        ...
        with torch.no_grad():
            target_log_prob = beta_log_prob(
                old_alpha,
                old_beta,
                native_actions,
            )
            weighted_gain_samples = weights * (
                log_prob.detach() - target_log_prob
            )
            target_surrogate_gain = weighted_gain_samples.sum()
            trajectory_gains = weighted_gain_samples.reshape(
                args.num_steps,
                args.num_envs,
            ).sum(dim=0)
            target_surrogate_gain_standard_error = (
                trajectory_gains.std(unbiased=True) * np.sqrt(args.num_envs)
            )
            target_surrogate_gain_lcb = (
                target_surrogate_gain
                - args.target_gain_lcb_z
                * target_surrogate_gain_standard_error
            )
        ...
        metrics = torch.stack(
            (
                ...
                concentration_kl_average.detach(),
                full_kl.detach(),
                target_surrogate_gain.detach(),
                target_surrogate_gain_standard_error.detach(),
                target_surrogate_gain_lcb.detach(),
    ...
    target_rollout_batches = 0
    target_acceptances = 0


    for iteration in range(1, args.num_iterations + 1):
        rollout_target_age_batches = target_age_batches
        rollout_target_rollout_batches = target_rollout_batches
        ...
        candidate_full_kl_value = float(metrics[5])
        candidate_gain_lcb_value = float(metrics[8])
        candidate_kl_feasible = (
            np.isfinite(candidate_full_kl_value)
            and candidate_full_kl_value <= args.target_kl_threshold
        )
        candidate_gain_confident = (
            np.isfinite(candidate_gain_lcb_value)
            and candidate_gain_lcb_value > 0.0
        )
        target_accepted = candidate_kl_feasible and candidate_gain_confident
        if target_accepted:
            with torch.no_grad():
                target_agent.load_state_dict(agent.state_dict())
            target_acceptances += 1

        optimizer.step()
        with torch.no_grad():
            duals.clamp_(min=DUAL_FLOOR)
        if target_accepted:
            # The candidate was copied before this optimizer step, so the
            # resulting online policy is one completed update beyond it.
            target_age_batches = 1
            target_rollout_batches = 0
        else:
            target_age_batches += 1
            target_rollout_batches += 1
        ...
        if should_log:
            ...
            (
                ...
                full_kl_value,
                target_surrogate_gain_value,
                target_surrogate_gain_standard_error_value,
                target_surrogate_gain_lcb_value,
                ess_fraction_value,
                top_advantage_min,
                advantage_mean,
                advantage_std,
            ...
            writer.add_scalar(
                "vmpo/target_rollout_batches",
                rollout_target_rollout_batches,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_rollout_transitions",
                rollout_target_rollout_batches * args.batch_size,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_checkpoint_accepted",
                float(target_accepted),
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_candidate_kl_feasible",
                float(candidate_kl_feasible),
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_candidate_gain_confident",
                float(candidate_gain_confident),
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_candidate_full_kl",
                candidate_full_kl_value,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_kl_threshold_residual",
                candidate_full_kl_value - args.target_kl_threshold,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_surrogate_gain",
                target_surrogate_gain_value,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_surrogate_gain_standard_error",
                target_surrogate_gain_standard_error_value,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_surrogate_gain_lcb",
                target_surrogate_gain_lcb_value,
                global_step,
            )
            writer.add_scalar(
                "vmpo/target_checkpoint_acceptances",
                target_acceptances,
                global_step,
            )
