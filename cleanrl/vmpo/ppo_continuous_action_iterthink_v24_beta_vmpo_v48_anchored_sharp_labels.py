# V-MPO v48 — anchored HL-Gauss with v30's label sharpness.
#
# Controlled derivative of v47 changing only sigma_to_bin_ratio from 1.0 to
# 0.75, matching v30's label entropy exactly. v47 finished 8M seed-1
# HalfCheetah steps at 5907.9 final-20 vs v30's 7790.4 with equal-or-better
# critic RMSE/EV in every window, identical E-step machinery, and zero
# clamping -- so coverage, accuracy, and loss scale are eliminated, leaving
# only ranking-relevant, scale-free channels (the E-step is scale-equivariant
# and Adam scale-adaptive by construction). The one persistent steady-state
# signature is ~20% tighter late advantages under floored rescaling with lower
# eta, consistent with v47's wider labels (sigma one full bin width)
# smoothing the learned value surface and weakening top-k contrast.
# Hypothesis: restoring v30's sharper labels restores selection contrast and
# closes the gap; if v48 also lands ~6000, the transfer is exhausted and the
# gap is selection path-dependence, not labels.
...
def anchored_symmetric_geometry(half_range, interior_bins, margin_bins):
    """Support geometry whose bin centers include exactly -R, 0.0, and +R.

    ``interior_bins`` uniform divisions from 0 to ``half_range`` set the bin
    width R/interior_bins; the grid is laid out so -R, 0, and +R are bin
    CENTERS (Dreamer3's exact-zero bucket, extended to both range anchors),
    with ``margin_bins`` extra bins beyond each anchor. Without the margin a
    boundary target's Gaussian is truncated at the support edge and its
    renormalized label decodes ~0.8*sigma inward; margin + 0.5 half bin-widths
    of slack keep that bias below the truncated tail mass.
    ...
    if half_range <= 0.0:
        raise ValueError("anchored support half_range must be positive")
    width = half_range / interior_bins
    num_bins = 2 * (interior_bins + margin_bins) + 1
    edge = (interior_bins + margin_bins + 0.5) * width
    return num_bins, -edge, edge
...
class Args:
    ...
    value_half_range: float = 20.0
    value_interior_bins: int = 80
    value_margin_bins: int = 8
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        self.value_head = nn.Linear(256, args.num_value_bins, bias=True)
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    if args.value_half_range <= 0.0:
        raise ValueError("value_half_range must be positive")
    if args.value_interior_bins < 1:
        raise ValueError("value_interior_bins must be positive")
    if args.value_margin_bins < 0:
        raise ValueError("value_margin_bins must be non-negative")
    if args.value_sigma_to_bin_ratio <= 0.0:
        raise ValueError("value_sigma_to_bin_ratio must be positive")
    args.num_value_bins, args.value_v_min, args.value_v_max = (
        anchored_symmetric_geometry(
            args.value_half_range,
            args.value_interior_bins,
            args.value_margin_bins,
        )
    )
    ...
    # Zero-anchored init: the head starts predicting exactly ~0 with a minimal
    # decoder Jacobian, mirroring the vapo solution's log-probs-of-zero bias.
    with torch.no_grad():
        agent.value_head.weight.zero_()
        agent.value_head.bias.copy_(
            hl_support.project_to_logprobs(
                torch.zeros((), device=device), eps=1e-6
            ).reshape(-1)
        )
    ...
    def rollout_model(observations):
        ...
        values = hl_support.to_expected_scalar(value_logits.float())
    ...
    def gae_model(
        ...
        next_values = hl_support.to_expected_scalar(next_value_logits.float()).view(
    ...
    def update_loss_model(
        ...
        values = hl_support.probs_to_expected_scalar(value_probs)
        ...
        with torch.no_grad():
            value_target_probs = hl_support.project(value_targets)
        ...
        target_outside_support = (
            (value_targets < args.value_v_min) | (value_targets > args.value_v_max)
        ).float().mean()
        target_edge_mass = (
