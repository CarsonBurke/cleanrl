# V-MPO v56 — v54 with per-minibatch top-k E-step (single-factor ablation).
#
# Only factor from v54: top-half selection, eta solve and weight
# normalization run independently inside each of num_estep_shards (8)
# contiguous minibatches, per the paper's App. F implementation note (one
# TPU replica each), instead of one global top-k + global softmax.
# Per-shard losses/KL/ESS are averaged so gradient scale matches v54.
# Hypothesis: global normalization diffuses weight mass over 2,496 samples
# (max_weight ~0.004); per-shard normalization concentrates selection
# pressure 8x and preserves per-shard diversity.
# lr, target hold, epsilons, reward norm, model are unchanged.
...
if __name__ == "__main__":
    ...
    if args.batch_size % args.num_estep_shards != 0:
        raise ValueError("batch_size must split evenly into num_estep_shards")
    shard_size = args.batch_size // args.num_estep_shards
    if not 0 < int(shard_size * args.topk_fraction) <= shard_size:
        raise ValueError("topk_fraction produces an invalid per-shard top-k size")
    ...
    def update_loss_model(
        ...
        with torch.no_grad():
            # Paper App. F: top-half selection and E-step weight normalization
            # are per-minibatch (one replica each), not global. Shard the
            # batch into num_estep_shards contiguous minibatches; every
            # reduction below runs over dim=1. Cutoff ties stay inclusive
            # within each shard, as before.
            shard_advantages = advantages.reshape(args.num_estep_shards, -1)
            shard_topk_size = int(shard_advantages.shape[1] * args.topk_fraction)
            topk_threshold = (
                torch.sort(shard_advantages, dim=1)
                .values[:, -shard_topk_size]
                .unsqueeze(1)
            )
            selected = shard_advantages >= topk_threshold
            selected_count = selected.sum(dim=1).to(shard_advantages.dtype)
            log_selected_count = selected_count.log().unsqueeze(1)
            ...
            maximum_advantage = shard_advantages.max(dim=1, keepdim=True).values
            centered_advantages = shard_advantages - maximum_advantage
            ...
            for _ in range(32):
                ...
                mid_log_weights = mid_logits - torch.logsumexp(
                    mid_logits, dim=1, keepdim=True
                )
            ...
            log_weights = policy_logits - torch.logsumexp(
                policy_logits, dim=1, keepdim=True
            )
            weights = log_weights.exp()
            selected_log_weights = torch.where(selected, log_weights, 0.0)
            # Per-shard KL is pinned at epsilon; the logged scalar is the mean.
            temperature_kl = (
                weights * (selected_log_weights + log_selected_count)
            ).sum(dim=1).mean()
            temperature_loss = (
                maximum_advantage.squeeze(1)
                + eta.squeeze(1)
                * (
                    args.epsilon_eta
                    + torch.logsumexp(policy_logits, dim=1)
                    - log_selected_count.squeeze(1)
                )
            ).mean()

        log_prob = beta_log_prob(new_alpha, new_beta, native_actions)
        # Mean over shards keeps the gradient scale comparable to the former
        # global-softmax loss (each shard's weights sum to 1).
        shard_log_prob = log_prob.reshape(
            weights.shape[0], weights.shape[1], -1
        )
        policy_loss = -(
            weights.unsqueeze(-1) * shard_log_prob
        ).sum(dim=(1, 2)).mean()
        ...
        # Per-shard ESS, averaged; each shard normalizes its own weights.
        ess_per_shard = weights.square().sum(dim=1).reciprocal()
        effective_sample_size = ess_per_shard.mean()
        effective_sample_fraction = (ess_per_shard / selected_count).mean()
