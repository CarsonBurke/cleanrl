# V-MPO v59 — paper-reference Gaussian policy iteration (arXiv:1909.12238).
# Shared state MLP, scalar PopArt, raw n-step returns, learned eta/alpha duals,
# replica-local top-half likelihoods, Adam 1e-4 and fixed 100-update behavior.
# Hypothesis: restore the published credit/constraint geometry before judging
# the Beta adaptations. No GAE(lambda<1), reward scaling, entropy bonus or PPO.
# Gym-specific architecture, initialization and batch count are not fully
# reported: use the paper's state-control 512/256 trunk + 256-unit heads,
# 64 trajectories, 8 replicas, ReLU and unit initial Gaussian std explicitly.
...
def validate_args(args):
    ...
    positive = (args.learning_rate, args.epsilon_eta, args.epsilon_alpha_mean,
                args.epsilon_alpha_covariance, args.initial_eta, args.initial_alpha_mean,
...
def replica_estep(advantages, eta, epsilon):
    """One shared learned eta; local top-half and normalization, as Appendix F."""
    ...
    # Subtraction is an exact shift of the dual objective, not advantage scaling.
    maximum = advantages.max(1, keepdim=True).values
    logits = torch.where(selected, (advantages - maximum) / eta, -torch.inf)
    log_normalizer = torch.logsumexp(logits, dim=1)
    log_weights = logits - log_normalizer[:, None]
    weights = log_weights.exp()
    temperature_loss = (maximum[:, 0] + eta * (epsilon + log_normalizer - selected_count.log())).mean()
    safe_log_weights = torch.where(selected, log_weights, 0.0)
    kl = (weights * (safe_log_weights + selected_count.log()[:, None])).sum(1)
    ess = weights.square().sum(1).reciprocal()
    return weights.detach(), temperature_loss, kl.detach(), ess.detach(), selected_count
...
def vmpo_loss(agent, duals, observations, actions, old_mean, old_std, advantages, normalized_returns, args):
    ...
    eta, alpha_mean, alpha_covariance = duals.clamp_min(DUAL_FLOOR).unbind()
    shard_advantages = replica_samples(advantages.detach(), args.num_steps, args.num_envs, args.num_replicas)
    weights, eta_loss, estep_kl, ess, selected_count = replica_estep(shard_advantages, eta, args.epsilon_eta)
    ...
    loss = policy_loss + eta_loss + mean_penalty + covariance_penalty + value_loss
    ...
    metrics = torch.stack((policy_loss.detach(), value_loss.detach(), eta_loss.detach(),
                           ...
                           std.detach().mean(), std.detach().amin(), std.detach().amax()))
    return loss, metrics
...
def main():
    ...
    with ExitStack() as resources:
        ...
        writer.add_text("paper_reference", "arXiv:1909.12238, Eqs 1-6/25-27, Appendix F/Table 7. "
                        f"Raw observations/rewards; n-step (lambda=1); learned shared eta; {args.num_replicas} env-axis replicas; "
        ...
        duals = nn.Parameter(torch.tensor([args.initial_eta, args.initial_alpha_mean,
        ...
        metric_names = ("losses/policy_loss", "losses/value_loss", "losses/temperature_loss",
                        ...
                        "debug/policy_std_mean", "debug/policy_std_min", "debug/policy_std_max")
            ...
            if should_log:
                with timer.span("diagnostics"), torch.no_grad():
                    ...
                    logged = gather_metrics({**dict(zip(metric_names, metrics)), **dict(zip(post_names, post)),
                                             "vmpo/eta": duals[0], "vmpo/alpha_mean": duals[1],
                                             "vmpo/alpha_covariance": duals[2],
