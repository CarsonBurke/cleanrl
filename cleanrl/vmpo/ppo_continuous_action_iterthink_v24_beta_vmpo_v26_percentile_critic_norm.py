# V-MPO v26 — D3 percentile-normalized scalar critic.
#
# Controlled derivative of v24 using its fresh P95(return)-P5(return) scale for
# both policy advantages and scalar critic targets. The critic predicts returns
# divided by that scale; its final layer is rescaled whenever the scale changes
# so every normalized-reward value prediction is preserved. Reward normalization
# remains vectorized and independent per environment; there is no centering or
# PopArt.
...
class Agent(nn.Module):
    ...
    critic_return_scale: torch.Tensor
    ...
    def __init__(self, envs, args):
        ...
        self.register_buffer("critic_return_scale", torch.ones(()))
    ...
    @torch.no_grad()
    def update_critic_return_scale(self, new_scale):
        old_scale = self.critic_return_scale.clone()
        scale_ratio = old_scale / new_scale
        self.value_head.weight.mul_(scale_ratio)
        self.value_head.bias.mul_(scale_ratio)
        self.critic_return_scale.copy_(new_scale)
...
if __name__ == "__main__":
    ...
    def rollout_model(observations):
        ...
        value = normalized_value.float() * agent.critic_return_scale
    ...
    def gae_model(
        ...
        next_values = (
            normalized_next_values.float() * agent.critic_return_scale
    ...
    def update_loss_model(
        ...
        value_loss = 0.5 * (
            normalized_values - normalized_value_targets
        ).square().mean()
        total_loss = (
            policy_loss
            + mean_constraint_loss
        ...
        value_error = (
            normalized_values - normalized_value_targets
        ) * agent.critic_return_scale
        raw_value_targets = (
            normalized_value_targets * agent.critic_return_scale
        )
        value_rmse = value_error.square().mean().sqrt()
        explained_variance = 1.0 - value_error.var(unbiased=False) / (
            raw_value_targets.var(unbiased=False) + 1e-8
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            agent.update_critic_return_scale(return_percentile_scale)
            normalized_value_targets = (
                value_targets / agent.critic_return_scale
            )
