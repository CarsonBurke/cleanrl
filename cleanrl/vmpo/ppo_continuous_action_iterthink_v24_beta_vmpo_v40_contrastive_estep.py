# V-MPO v40 — contrastive E-step score.
#
# Derived from v30 after v31-v33/v38 found no target-controller lever and v39
# found no reliable gain from historical-window denoising. V-MPO's sampled
# M-step assigns positive mass only to the globally elite state-action rows.
# With one action per state, subtracting the empirical behavior score is an
# exact on-policy control variate at the frozen target: it leaves the expected
# improvement gradient unchanged, gives rejected actions negative credit, and
# removes finite-batch self-imitation that can sharpen a locally stuck policy.
# The selected-set E-step radius is widened from 0.01 to 0.1; top-half support
# already contributes log(2) nats relative to the full behavior batch, so this
# is a moderate within-elite ranking change rather than an unconstrained tilt.
...
if __name__ == "__main__":
    ...
    def update_loss_model(
        ...
        # At the frozen behavior policy, E_a[grad log pi(a|s)] = 0. Subtracting
        # its empirical full-batch score therefore preserves the expected
        # V-MPO gradient while contrasting elite actions with rejected ones.
        contrastive_weights = weights - weights.new_full(
            weights.shape, 1.0 / args.batch_size
        )
        policy_loss = -(contrastive_weights * log_prob).sum()
