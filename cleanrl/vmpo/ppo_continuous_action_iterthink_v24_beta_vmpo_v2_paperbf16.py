# V-MPO v2 PAPER-BF16 — paper-faithful training semantics on IterThink v24 Beta.
#
# Keeps the v24 state-dependent unimodal Beta actor and IterThink trunk. Replaces
# PPO/GAE/multi-epoch reuse with V-MPO (Song et al., arXiv:1909.12238): one Adam
# update per fresh unroll, bootstrapped n-step returns, top-half exponential
# weighting, learned eta, a lagged target policy, PopArt value regression, and
# learned M-step KL multipliers. The Gaussian paper's decoupled mean/covariance
# constraints are adapted to Beta mean/concentration constraints.
#
# Paper alignment: 39-step Gym unrolls; rollout-wide top-half selection; target
# sync every 100 learner steps; fixed 1e-4 Adam LR; no GAE, advantage
# normalization/clipping, PPO ratio, policy early stop, entropy regularization,
# gradient clipping, or replay epochs. Gym batch size is unreported; 64 parallel
# trajectories follows the continuous-control batch size reported for humanoids.
#
# Performance path: FP32 master weights/duals and Beta/PopArt numerics, BF16
# autocast for network matmuls, fullgraph torch.compile reduce-overhead CUDA
# graphs, one full-batch loss graph, batched-expert GEMMs, fused Adam, and one
# packed host synchronization per logging interval.
...
if __name__ == "__main__":
    ...
    def update_loss_model(
        ...
        effective_sample_fraction = (
            weights.square().sum().reciprocal() / args.topk_size
        )
        metrics = torch.stack(
            (
                policy_loss.detach(),
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.log_interval == 0 or iteration == 1:
            ...
            writer.add_scalar("vmpo/weight_ess_fraction", ess_value, global_step)
