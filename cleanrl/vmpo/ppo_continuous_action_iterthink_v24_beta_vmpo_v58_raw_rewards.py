# V-MPO v58 — v54 with raw-scale rewards (single-factor ablation).
#
# Only factor from v54: advantages and value targets use RAW rewards, as in
# the paper (returns from actual rewards, PopArt confined to the value
# head). The running-std reward division is bypassed; the Dreamer3 support
# is untouched (its +-log1p(20000) symlog coords already span +-20000 raw).
# Hypothesis: input normalization compresses late-training advantage scale
# (~0.1) against fixed epsilon_eta, washing out E-step selection; raw scale
# restores it (eta solver is equivariant).
# lr, target hold, epsilons, global top-k, model are unchanged.
...
if __name__ == "__main__":
    ...
    # Support bounds are SYMLOG coordinates (Dreamer3 buckets span
    # symexp(+-coord)): +-log1p(20000) already covers raw HalfCheetah returns
    # to +-20000. A prior revision passed +-20000 as coordinates, overflowing
    # symexp to inf and NaNing every value from iteration 1 — reverted.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(args.num_steps):
            ...
            # Raw-reward ablation: bypass the running-std division so GAE
            # advantages and value targets keep raw scale (paper-like).
            # rew_norm stats still update in warmup but are unused below.
            reward = np.asarray(raw_reward, dtype=np.float64)
