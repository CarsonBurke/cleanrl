# V-MPO v35 — reward-normalized linear-coordinate HL-Gauss critic.
#
# Controlled derivative of v30 changing only critic support geometry: 51 raw
# scalar centers span -1,000 through exact zero to 1,000, Gaussian labels are
# projected in linear coordinates, and paired decoding is exactly zero-centered.
...
class Args:
    ...
    value_support_limit: float = 1_000.0
...
if __name__ == "__main__":
    ...
    if args.value_support_limit < 10.0 / (1.0 - args.gamma):
        raise ValueError(
            "value_support_limit must be at least 10 / (1 - gamma)"
        )
    ...
    half_value_bins = (args.num_value_bins - 1) // 2
    positive_value_support = hl_support.support[half_value_bins + 1 :]

    def linear_probs_to_scalar(probs):
        paired_probabilities = (
            probs[..., half_value_bins + 1 :]
            - probs[..., :half_value_bins].flip(-1)
        )
        return (paired_probabilities * positive_value_support).sum(dim=-1)

    def linear_logits_to_scalar(logits):
        return linear_probs_to_scalar(torch.softmax(logits, dim=-1))
    ...
    def rollout_model(observations):
        ...
        values = linear_logits_to_scalar(value_logits.float())
    ...
    def gae_model(
        ...
        next_values = linear_logits_to_scalar(next_value_logits.float()).view(
    ...
    def update_loss_model(
        ...
        values = linear_probs_to_scalar(value_probs)
