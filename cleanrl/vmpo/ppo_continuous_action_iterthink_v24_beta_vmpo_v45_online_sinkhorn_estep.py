# V-MPO v45 — online per-rollout balanced entropic projection for Beta policies.
#
# V44's exact positive-weight E-step was healthy, but freezing its policy target
# for eight rollouts allowed only one trust-region move per 19,968 transitions,
# under-updating the actor relative to v30.  V45 restores v30's per-rollout
# policy-target cadence: each rollout comes from one immutable deployed policy,
# then the critic update and the same exact balanced entropic E-step precede
# candidate fitting, full analytic Beta-KL projection, and atomic promotion.
# At the defaults this is a [num_steps, num_envs] transport matrix and one
# positive weighted-likelihood update opportunity every 2,496 transitions,
# without Q estimates, replay, simulator branches, counterfactual actions,
# top-k selection, signed weights, or contrastive loss.
...
class Args:
    ...
    # Default 1 restores v30's one-rollout target cadence and yields a
    # [num_steps, num_envs] transport matrix; larger windows remain supported.
...
if __name__ == "__main__":
    ...
    def transport_for_eta(centered_advantages, eta_value):
        """Compute the balanced entropic projection for the current policy-target cycle."""
