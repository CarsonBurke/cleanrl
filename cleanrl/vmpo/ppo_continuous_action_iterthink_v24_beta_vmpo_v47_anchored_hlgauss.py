# V-MPO v47 — reward-normalized anchored uniform-edge HL-Gauss critic.
#
# Controlled derivative of v30 replacing only the Dreamer3-bucket critic with
# the parameter-golf vapo HL-Gauss solution, rescaled from its [0, 1] reward
# range to our normalized-return range. Uniform edges partition [-22.125,
# 22.125] so exact zero and +/-20 are bin CENTERS, with 8 margin bins beyond
# each anchor keeping truncation bias negligible. Plain erf projection (no
# 32-iteration moment tilt), expected-scalar decode, sigma one bin width, and
# zero-anchored head init. Hypothesis: v30's +/-20,000 symexp support wastes
# resolution on normalized targets (percentile scale <= ~4.3, edge mass 0)
# and its raw-scale decode Jacobian (~750) destabilizes values; the anchored
# linear support restores resolution where targets live at ~1/64 the cost.
...
class Args:
    ...
    value_sigma_to_bin_ratio: float = 1.0
