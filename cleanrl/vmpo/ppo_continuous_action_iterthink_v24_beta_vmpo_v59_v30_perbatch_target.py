# V-MPO v59 — v30 with a per-batch target policy.
#
# Controlled ablation of v30. `--target-mode batch` makes the rollout snapshot
# the target: the online policy acts, and the M-step KL anchor is the policy
# that collected the batch (T_target = 1, as in PPO and TPO). `--target-mode
# periodic` reproduces v30's hard copy every 10 batches. The 39-step unroll
# restriction is lifted so the batch geometry can match base PPO (32 x 1024)
# while keeping v30's single full-batch gradient step per rollout.
#
# Hypothesis: v30's frozen target buys a KL sawtooth, not an evaluation phase.
# With one step per batch the leash is already non-binding, so a per-batch
# anchor should compound improvements every batch at no stability cost, and the
# PPO-geometry runs isolate how much of v30's behaviour is set by the number of
...
class Args:
    ...
    # "batch": the rollout snapshot is the target (T_target = 1).
    # "periodic": v30's hard copy, promoted on mean-KL or at max age.
...
if __name__ == "__main__":
    ...
    if args.target_mode not in ("batch", "periodic"):
        raise ValueError("target_mode must be 'batch' or 'periodic'")
    per_batch_target = args.target_mode == "batch"
    ...
    if per_batch_target:
        # The online policy acts; stored old_alpha/old_beta anchor the M-step.
        target_agent = agent
    else:
    ...
    def rollout_model(observations):
        with torch.autocast(device_type="cuda", dtype=autocast_dtype):
            if per_batch_target:
                target_alpha, target_beta, value_logits = agent(observations)
            else:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if should_log:
            ...
            promote_for_mean_kl = (
                not per_batch_target
                and iteration % args.log_interval == 0
                and mean_kl_value >= args.epsilon_alpha_mean
            )
            promote_for_max_age = (
                not per_batch_target
                and target_age_batches >= args.target_update_period
            )
