# V-MPO v12 target every batch — per-batch frozen-target refresh ablation.
#
# v10 GAE keeps its frozen policy for 100 learner batches. This isolated
# ablation refreshes that target after every learner batch.
#
# With one full-batch optimizer step, the target and online policies match
# before the update, so the pre-update parametric KL is zero. The KL dual has
# no same-batch corrective opportunity: it observes zero KL before the step,
# then the updated policy is immediately copied into the target.
#
# Exact eta, the top-half E-step, GAE at lambda=0.95, PopArt, the one-full-batch
# optimizer update, and the full-width ReLU-squared IterThink Beta policy remain
# unchanged.
