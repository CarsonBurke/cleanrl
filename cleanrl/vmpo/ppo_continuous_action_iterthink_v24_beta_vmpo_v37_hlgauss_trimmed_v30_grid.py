# V-MPO v37 — V30's active HL-Gauss grid with unused tails removed.
#
# V36 preserved V30's absolute Gaussian sigma but compressed the same 51
# centers into a smaller coordinate range, increasing target entropy and
# weakening value-logit gradients. This variant instead keeps V30's exact
# coordinate step, sigma ratio, and central bucket locations. It removes only
# the seven outermost buckets per side, leaving 37 centers and ±1,248.56 support.
...
class Args:
    ...
    num_value_bins: int = 37
    value_support_limit: float = 1_248.5603356271588
...
if __name__ == "__main__":
    ...
    if args.value_support_limit < 10.0 / (1.0 - args.gamma):
        raise ValueError("value_support_limit must be at least 10 / (1 - gamma)")
