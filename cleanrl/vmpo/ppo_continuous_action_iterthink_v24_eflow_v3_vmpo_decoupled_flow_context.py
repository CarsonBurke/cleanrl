# EFLOW v3 — eflow_v2 with the flow conditioner on its own observation MLP.
#
# eflow_v2 (job 4559: 367 final). The transform dual climbed to ~280 within
# 500k steps and the whole policy froze: base mean KL 0.001 per window, all
# promotions at max age. The spline conditioner read the shared 256-d policy
# features, so the transform KL moved whenever the trunk moved, for the base
# NLL or the critic, and a penalty on it became a penalty on the shared
# representation moving at all. eflow_v1 had the mirror problem: trunk drift
# showed up as transform movement that no dual held.
#
# v3: the conditioner reads a separate two-layer MLP on the observation.
# The trunk, Beta head, and critic graph are exactly v30's; the flow's only
# coupling to the base is the joint likelihood through u = T^-1(a). The
# transform KL now depends on flow parameters alone, so its dual constrains
# only the shape the flow adds, and the base moves at v30's rate.
import copy
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def policy(self, observations):
        ...
        return alpha, beta, self.flow_context(observations)
    ...
    def forward(self, observations):
        ...
        return alpha, beta, self.flow_context(observations), value_logits
