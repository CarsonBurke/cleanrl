# V-MPO v42 — cross-fitted conditional-dual native Beta policy optimization.
#
# For q(a|s) proportional to pi_old(a|s) exp(A(s,a)/eta), the conditional
# partition gives the exact dual eta*epsilon + eta*E_s[log Z(s)].  Its
# single-action variational estimate is log_z(s)-1+exp(A/eta-log_z(s)); two
# low-capacity log-Z networks train on disjoint environment folds and evaluate
# out of fold.  Every eight-rollout cycle (19,968 rows by default) freezes its
# deployed actor, solves this E-step once, and fits a fresh candidate by
# positive-weight maximum likelihood.  The deployed actor changes at the one
# atomic promotion point only after full-window analytic Beta-KL projection.
...
class Args:
    ...
    # Removing v30's top-half truncation removes log(2) nats of E-step change.
    # Preserve its total information budget, now conditionally within state.
    epsilon_eta: float = 0.7031471805599453
    initial_eta: float = 1.0
    dual_learning_rate: float = 3e-3
    dual_updates: int = 128
    dual_minibatch_size: int = 2048
    normalizer_hidden: int = 32
...
class StateLogPartition(nn.Module):
    """Low-capacity state normalizer; fold separation supplies cross-fitting."""

    def __init__(self, observation_dim, hidden):
        super().__init__()
        self.net = nn.Sequential(
            layer_init(nn.Linear(observation_dim, hidden), std=1.0),
            nn.Tanh(),
...
if __name__ == "__main__":
    ...
    if args.num_envs <= 0 or args.num_envs % 2:
        raise ValueError("num_envs must be positive and even for balanced folds")
    ...
    if args.epsilon_eta <= 0.0 or args.initial_eta <= DUAL_FLOOR:
        raise ValueError("epsilon_eta and initial_eta must be positive")
    if args.epsilon_policy_kl <= 0.0:
        raise ValueError("epsilon_policy_kl must be positive")
    if args.dual_updates <= 0 or args.m_step_epochs <= 0:
        raise ValueError("dual_updates and m_step_epochs must be positive")
    if args.dual_minibatch_size <= 0 or args.dual_minibatch_size % 2:
        raise ValueError("dual_minibatch_size must be positive and even")
    if args.dual_minibatch_size > args.window_size:
        raise ValueError("dual_minibatch_size cannot exceed the cycle window")
    ...
    if args.normalizer_hidden <= 0:
        raise ValueError("normalizer_hidden must be positive")
    if args.learning_rate <= 0.0 or args.dual_learning_rate <= 0.0:
    ...
    z_even = StateLogPartition(observation_dim, args.normalizer_hidden).to(device)
    z_odd = StateLogPartition(observation_dim, args.normalizer_hidden).to(device)
    ...
    def solve_conditional_dual(flat_observations, advantages, even_rows, eta_start):
        even_indices = torch.nonzero(even_rows, as_tuple=False).squeeze(-1)
        odd_indices = torch.nonzero(~even_rows, as_tuple=False).squeeze(-1)
        per_fold_minibatch = args.dual_minibatch_size // 2
        raw_eta = nn.Parameter(
            torch.tensor(
                inverse_softplus(max(eta_start - DUAL_FLOOR, DUAL_FLOOR)),
                device=device,
                dtype=torch.float32,
            )
        )
        dual_optimizer = optim.Adam(
            [*z_even.parameters(), *z_odd.parameters(), raw_eta],
            lr=args.dual_learning_rate,
            betas=(0.9, 0.999),
            eps=1e-8,
            fused=True,
        )
        for _ in range(args.dual_updates):
            even_batch = even_indices[
                torch.randint(even_indices.numel(), (per_fold_minibatch,), device=device)
            ]
            odd_batch = odd_indices[
                torch.randint(odd_indices.numel(), (per_fold_minibatch,), device=device)
            ]
            eta = F.softplus(raw_eta) + DUAL_FLOOR
            eta_for_normalizers = eta.detach()
            with torch.autocast(device_type="cuda", dtype=autocast_dtype):
                own_even_log_z = z_even(flat_observations[even_batch]).float()
                own_odd_log_z = z_odd(flat_observations[odd_batch]).float()
                crossfit_even_log_z = (
                    z_odd(flat_observations[even_batch]).float().detach()
                )
                crossfit_odd_log_z = (
                    z_even(flat_observations[odd_batch]).float().detach()
                )

            # Each normalizer sees only its own environment fold.  Eta is
            # detached here so its update cannot exploit in-fold fitting.
            normalizer_even_term = own_even_log_z - 1.0 + torch.exp(
                advantages[even_batch] / eta_for_normalizers - own_even_log_z
            )
            normalizer_odd_term = own_odd_log_z - 1.0 + torch.exp(
                advantages[odd_batch] / eta_for_normalizers - own_odd_log_z
            )
            normalizer_loss = 0.5 * eta_for_normalizers * (
                normalizer_even_term.mean() + normalizer_odd_term.mean()
            )

            # The shared eta sees only out-of-fold log partitions, detached
            # from both normalizer networks.
            eta_even_term = crossfit_even_log_z - 1.0 + torch.exp(
                advantages[even_batch] / eta - crossfit_even_log_z
            )
            eta_odd_term = crossfit_odd_log_z - 1.0 + torch.exp(
                advantages[odd_batch] / eta - crossfit_odd_log_z
            )
            eta_even_loss = eta * args.epsilon_eta + eta * eta_even_term.mean()
            eta_odd_loss = eta * args.epsilon_eta + eta * eta_odd_term.mean()
            eta_loss = 0.5 * (eta_even_loss + eta_odd_loss)
            dual_optimizer.zero_grad(set_to_none=True)
            (normalizer_loss + eta_loss).backward()
            dual_optimizer.step()

        with torch.no_grad():
            eta = (F.softplus(raw_eta) + DUAL_FLOOR).detach()
            with torch.autocast(device_type="cuda", dtype=autocast_dtype):
                crossfit_log_z = torch.empty_like(advantages)
                crossfit_log_z[odd_indices] = z_even(
                    flat_observations[odd_indices]
                ).float()
                crossfit_log_z[even_indices] = z_odd(
                    flat_observations[even_indices]
                ).float()
            even_log_z = crossfit_log_z[even_indices]
            odd_log_z = crossfit_log_z[odd_indices]
            even_term = even_log_z - 1.0 + torch.exp(
                advantages[even_indices] / eta - even_log_z
            )
            odd_term = odd_log_z - 1.0 + torch.exp(
                advantages[odd_indices] / eta - odd_log_z
            )
            even_loss = eta * args.epsilon_eta + eta * even_term.mean()
            odd_loss = eta * args.epsilon_eta + eta * odd_term.mean()
            raw_weights = torch.exp(advantages / eta - crossfit_log_z)
            raw_mean = raw_weights.mean()
            weights = raw_weights / raw_mean.detach()
            log_weights = weights.log()
            estimated_e_step_kl = (weights * log_weights).mean()
            mean_weight_residual = raw_mean - 1.0
        ...
        return (
            ...
            even_loss.detach(),
            odd_loss.detach(),
            estimated_e_step_kl.detach(),
            mean_weight_residual.detach(),
            ess_fraction.detach(),
            max_weight.detach(),
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            flat_advantages = advantages.reshape(-1).detach() / return_scale
            ...
            environment_rows = torch.arange(args.num_envs, device=device).repeat(
                args.window_batches * args.num_steps
            )
            even_rows = environment_rows.remainder(2).eq(0)

        z_even_before = clone_parameters(z_even)
        z_odd_before = clone_parameters(z_odd)
        ...
        (
            ...
            ess_fraction,
            max_weight,
        ) = solve_conditional_dual(
            flat_observations, flat_advantages, even_rows, solved_eta
        )
        ...
        dual_finite = bool(torch.isfinite(dual_metrics).all().item()) and bool(
            torch.isfinite(weights).all().item()
        )
        if dual_finite:
            solved_eta = float(eta.item())
        if not dual_finite:
            load_parameters(z_even, z_even_before)
            load_parameters(z_odd, z_odd_before)
        if dual_finite:
            (
                final_kl,
                candidate_gain,
            ...
            candidate_actor.load_state_dict(deployed_actor.state_dict())
            final_kl = torch.full((), torch.nan, device=device)
            candidate_gain = torch.full((), torch.nan, device=device)
            policy_concentration = torch.full((), torch.nan, device=device)
            policy_native_variance = torch.full((), torch.nan, device=device)
            actor_outputs_finite = False
        ...
        writer.add_scalar(
            "vmpo/estimated_mean_w_log_w",
            float(estimated_e_step_kl.item()),
            global_step,
        )
        writer.add_scalar(
            "vmpo/mean_weight_residual_before_normalization",
            float(mean_weight_residual.item()),
            global_step,
        )
        ...
        writer.add_scalar("vmpo/dual_even_loss", float(dual_even_loss.item()), global_step)
        writer.add_scalar("vmpo/dual_odd_loss", float(dual_odd_loss.item()), global_step)
