# V-MPO v18 — vectorized observation normalization + target-1 + eta-0.03.
#
# This explicit combined experiment inherits v17's behavior-equivalent
# vectorized observation normalization: 64 independent per-environment running
# statistics are stored in arrays, with ordinary rows updated in one NumPy hot
# path. Terminal observations are updated and normalized before same-step
# autoreset observations, matching the removed per-environment Gymnasium
# wrappers.
#
# The parametric target is refreshed every learner batch and epsilon_eta is
# 0.03. With one optimizer step per batch, period 1 makes the pre-update
# parametric target-to-online KL effectively zero.
#
# Observations are clipped to [-10, 10]. Rewards remain unnormalized so PopArt
# continues to normalize critic targets while preserving raw return units. Exact
# eta, the top-half E-step, Beta carrier, IterThink network, GAE, PopArt, the
# one-full-batch update, phase staggering, and diagnostics otherwise remain
# unchanged.
