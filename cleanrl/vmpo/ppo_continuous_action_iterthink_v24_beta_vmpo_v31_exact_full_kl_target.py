# V-MPO v31 — exact full-KL target promotion.
#
# Controlled derivative of v30 replacing its ten-batch, mean-coordinate target
# trigger with an every-batch full forward Beta KL on the exact online candidate
# being promoted. The candidate is copied before its next optimizer step, so the
# measured and deployed policies match. A two-update minimum hold preserves a
# nonzero M-step trust signal; 100 batches remains the maximum hold. The critic,
# reward normalization, and all other policy machinery are unchanged.
...
if __name__ == "__main__":
    ...
    if not 1 <= args.target_min_age_batches < args.target_update_period:
        raise ValueError(
            "target_min_age_batches must be positive and below target_update_period"
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        promote_for_full_kl = (
            target_age_batches >= args.target_min_age_batches
            and candidate_full_kl_value >= args.target_kl_threshold
        )
        promote_for_max_age = (
            target_rollout_batches + 1 >= args.target_update_period
        )
        target_promoted = promote_for_full_kl or promote_for_max_age
        if target_promoted:
            with torch.no_grad():
