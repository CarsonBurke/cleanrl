# V-MPO v22 — mean-KL-triggered target promotion.
#
# V17's online actor spends its mean-KL trust budget within roughly 10-20
# learner batches, but the fixed target remains deployed for 100 batches. V22
# promotes the updated online actor whenever the pre-update mean KL reaches its
# bound, checked at the existing logging cadence to avoid another device sync.
# The paper's 100-batch period remains a maximum hold. Concentration KL does not
# trigger promotion because its much smaller Beta-adapted bound is not
# equilibrated and would degenerate toward period-one updates.
#
# Vectorized per-environment observation normalization, exact eta, GAE, phase
# staggering, one fresh full-batch update, PopArt, and the full-width
# ReLU-squared IterThink Beta model remain unchanged.
