# V-MPO v6 RLAX-ALIGNED — global E-step on full-width IterThink Beta.
#
# RLax gathers advantages across mapped devices before choosing the top half
# and uses all-reduced sums for both normalized weights and temperature loss.
# This single-GPU implementation therefore performs one global E-step over the
# complete fresh rollout rather than treating device shards as separate losses.
#
# IterThink's four 64-wide feature stages remain intact, but their concatenated
# 256-wide representation is no longer projected through a 64-wide bottleneck
# before the separate 256-unit policy and value MLPs. All hidden activations,
# including those head MLPs, use ReLU-squared.
#
# The online network and V-MPO duals share the established IterThink PPO Adam
# learning rate of 3e-4. Training behavior returns are the only policy-return
# measurement; no additional policy-evaluation environments are run.
