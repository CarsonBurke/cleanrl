# EFLOW v2 — eflow_v1 with a real Lagrangian dual on the transform component.
#
# eflow_v1 (job 4558: 101 @1M, 3378 @4M, 4971 final vs v30's -59 / 5127 /
# 7548). The spline transform did all the moving: transform KL per target
# window ran 0.017-0.034 against its 0.0071 budget while the Beta base moved
# 0.0004-0.007 (v30: 0.015). Spline knots are far more sensitive per Adam
# step than the Beta head, and the transform dual sat at 1.0 like v30's
# duals, so nothing held it. The policy overshot the E-step's 0.01 target
# every window and partly fit the batch's top-half selection: faster start,
# lower plateau, saturation rising to 6.5%.
#
# v2: the transform dual leaves Adam and takes projected dual ascent in
# relative units after every step, alpha <- max(alpha + 0.1 (KL/eps - 1), 0).
# The constraint is measured exactly (20k exact log-densities per batch,
# ~8e-4 noise at budget), unlike fpo_v5's ELBO gap, so the dual tracks the
# real violation. The base keeps v30's dual mechanics untouched, so mean
# shift stays v30's job and the transform adds shape at a bounded rate.
import copy
import os
import random
import time
...
class Args:
    ...
    # Projected dual ascent step for the transform dual, in units of KL / eps - 1.
    transform_dual_step_size: float = 0.1
...
if __name__ == "__main__":
    ...
    transform_dual = torch.tensor(args.initial_alpha_transform, device=device)
    ...
    def update_loss_model(
        ...
        alpha_transform = transform_dual
        ...
        transform_constraint_loss = alpha_transform * transform_kl_average
        with torch.no_grad():
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        transform_dual_before = transform_dual.clone() if should_log else None
        ...
        with torch.no_grad():
            ...
            transform_dual.add_(
                args.transform_dual_step_size
                * (transform_kl_measured / args.epsilon_alpha_transform - 1.0)
            ).clamp_(min=0.0)
        target_age_batches += 1

        if should_log:
            assert duals_before is not None
            assert transform_dual_before is not None
            ...
            packed = torch.cat(
                (
                    ...
                    duals.detach(),
                    dual_delta,
                    transform_dual.view(1),
                    (transform_dual - transform_dual_before).view(1),
