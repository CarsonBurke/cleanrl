# sf_latdir_v1_diag -- INSTRUMENTATION-ONLY fork of ppo_continuous_action_sf_latdir_v1.py.
#
# TRAINING BEHAVIOUR IS BIT-IDENTICAL TO THE PARENT. Every addition is inside a
# torch.no_grad() block (or an autograd.grad whose only input is a detached action leaf),
# draws ZERO samples from any RNG (analytic Beta moments only), never enters a .backward(),
# and does not reorder any pre-existing computation. Nothing here is read by the loss.
#
# THE QUESTION IT ANSWERS. latdir maximizes log pi(a*) inside a clip trust region, with
# a* = a + eta*ghat. There are THREE ways a Beta policy can raise log pi(a*), and only the
# first is policy improvement:
#   (1) TRANSLATION toward a*  -- move the mean m = c1/(c1+c0) along ghat. What the method
#       claims to do.
#   (2) SHARPENING             -- raise the total concentration c = c1+c0 at fixed m.
#       Variance collapse, not improvement.
#   (3) SELF-IMITATION toward a -- move m toward the action ALREADY SAMPLED. a* sits a mere
#       eta away from a, so pulling m onto a raises log pi(a*) almost as much as (1) does
#       while carrying ZERO information from the critic. This is the per-state Dirac
#       collapse the parent's own loss comment (search "cannot fit 32,768 independent point
#       targets") names as the thing keeping the objective honest -- if it fails, this is
#       what the objective degenerates to.
# The loss cannot tell them apart; corr_g_adv cannot either (it never looks at the policy).
#
# THE READING KEY, MEASURED (scratchpad/verify_ceiling.py and
# scratchpad/verify_three_way.py -- same convention as the parent's
# scratchpad/verify_latdir.py: session-local, not checked in. Real geometry: act_dim 6,
# eta 0.3, per-actuator std ~0.46, so ||a - mu|| ~ 1.09 vs ||eta*ghat|| = 0.3). Do NOT read
# these as "1 means yes": the ceilings are NOT 1 and that is geometry, not weak evidence.
#
#                        align   align_null  align_taken  proj_real   dlogc   dlogp*   gap
#   (1) translate->a*    +0.24      ~0          +0.96       = frac      0      > 0     > 0
#   (2) sharpen           ~0        ~0           ~0          ~0        > 0     < 0     < 0
#   (3) self-imitate->a   ~0        ~0          +1.00        ~0         0      > 0     > 0
#
# THE TWO NUMBERS THAT ACTUALLY DECIDE IT are therefore
#   diag/proj_realized  -- <mu_new - mu_old, ghat>/eta. UNBIASED for "what fraction of the
#                          requested eta-step was delivered": a perfect 20% translation
#                          toward a* reads 0.192, a 50% one reads 0.480 (measured). This is
#                          the headline, NOT dmean_align.
#   diag/dmean_align    -- read ONLY against diag/dmean_align_null (the same statistic with
#                          ghat paired to a different row). Its ceiling under PERFECT
#                          translation is +0.24, because a* - mu = (a - mu) + eta*ghat and
#                          the (a - mu) term is 3.6x longer. align ~ 0.2 is therefore a
#                          STRONG positive, and align ~ null is the null result.
# and diag/dmean_align_taken is what separates (1) from (3): both raise dlogp_gap, only (3)
# pins the mean onto the sampled action while leaving proj_realized at zero.
#
# NOTE the spec'd sharpening signature ("dlogp_star > 0 with gap ~ 0") does NOT occur:
# raising c at fixed m LOWERS the density at a*, because a* sits ~0.85 std out (measured
# dlogp* = -0.16 at c x1.05, -1.09 at c x1.30). A positive dlogp_star therefore already
# rules (2) out on its own, and the live alternative to (1) is (3).
#
# The parametrization used throughout is (m, c): m = c1/(c1+c0) in (0,1) is the Beta mean,
# c = c1+c0 >= 2 is the total concentration. Var[z] = m(1-m)/(c+1), so c is monotonically
# "sharpness" at fixed m. E[a] = low + span*m and Std[a] = span*sqrt(Var[z]) exactly (the
# beta->action map is affine), so no sampling is needed anywhere in this file.
#
# SCALE FOR dlogp_gap. clip_coef_high = 0.28, so a sample stops contributing gradient once
# its cumulative log ratio at a* passes log(1.28) = 0.247 nats. dlogp_star and hence
# dlogp_gap have a per-sample ceiling of that order; "gap ~ 0" is only meaningful against it.
#
# diag/lin_* is the secondary check: is a* still in the LINEAR regime of log pi_old?
# lin_corr ~ 1 with lin_slope ~ 1 means yes; lin_corr -> 0 means the change in log pi at a*
# is unrelated to the first-order term, i.e. a* is in the tail and curvature owns the
# objective. Read lin_corr/lin_slope, NOT lin_ratio -- see their comments.
# =====================================================================================
...
class Args:
    ...
    # --- INSTRUMENTATION ONLY (this fork's entire addition) --------------------------
    latdir_diag: bool = True         # emit the diag/* panel: the translation-vs-sharpening
    #                                  probe described in the header. Costs TWO extra
    #                                  full-batch no_grad trunk forwards per iteration -- one
    #                                  at rollout time for the pre-update snapshot, one after
    #                                  the update phase -- plus one cheap autograd.grad over
    #                                  a detached action leaf, and touches NOTHING the loss
    #                                  reads. --no-latdir-diag
    #                                  makes this file byte-equivalent in behaviour to the
    #                                  parent AND skips the extra forward. Ignored when
    #                                  --no-latdir (there is no a* to compare against).
    latdir_diag_chunk: int = 0       # 0 = ONE forward over the whole rollout, which is the
    #                                  SAME (num_steps*num_envs) shape the file already
    #                                  feeds the trunk three times per iteration -- so under
    #                                  --compile it introduces no new shape and no recompile.
    #                                  Set >0 only if that forward does not fit; it must
    #                                  DIVIDE num_steps*num_envs, otherwise the ragged tail
    #                                  is a second shape. Chunking is exact IN EXACT
    #                                  ARITHMETIC -- the forward is per-row and every
    #                                  reduction is applied after reassembly -- but NOT
    #                                  bitwise in float32: cuBLAS selects different GEMM
    #                                  split strategies per batch size, measured at ~1e-5
    #                                  absolute on the concentrations
    #                                  (scratchpad/verify_diag_math.py,
    #                                  check 5c). Irrelevant for a diagnostic, and it is a
    #                                  second reason the default is one unchunked forward.
...
class Agent(nn.Module):
    ...
    def actor_diag(self, x, z, z_star):
        """INSTRUMENTATION ONLY. Analytic Beta moments + log-probs at z and z*.

        Returns (c1, c0, mean_action, std_action, log_prob_z, log_prob_z_star).

        Everything is ANALYTIC -- dist.mean and dist.variance, never a sample -- so calling
        this consumes no RNG and cannot shift the training trajectory by a single bit. It
        deliberately does NOT compute entropy (the gaussian branch's entropy needs an
        rsample, which WOULD advance the generator); latdir rejects the gaussian head at
        startup, so the beta branch is the only reachable one and this method asserts it.

        The beta->action map is the affine z -> low + span*z, so the action-space moments
        are exact:  E[a] = low + span*E[z],  Std[a] = span*Std[z]. log_det_fn is identically
        0 for beta, so the log-probs returned here are the SAME objects the loss uses.
        """
        if self.actor_dist != "beta":
            raise ValueError("actor_diag is beta-only (latdir rejects gaussian at startup)")
        actor_feat, _ = self._trunks(x)
        dist, to_action, log_det_fn = self._actor_dist(actor_feat)
        span = self.action_high - self.action_low
        mean_action = to_action(dist.mean)
        std_action = span * dist.variance.sqrt()
        log_prob = (dist.log_prob(z) - log_det_fn(z)).sum(1)
        log_prob_star = (dist.log_prob(z_star) - log_det_fn(z_star)).sum(1)
        return (
            dist.concentration1,
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # INSTRUMENTATION ONLY. Bound on every path so the diag/* panel is always
            # writable; diag_snap is the handoff to part (B), taken AFTER the update phase.
            diag_logp_star_minus_taken_pre = float("nan")
            diag_lin_pred = float("nan")
            diag_lin_ratio = float("nan")
            diag_lin_corr = float("nan")
            diag_lin_slope = float("nan")
            diag_lin_pred_delivered = float("nan")
            diag_ghat_norm_mean = float("nan")
            diag_snap = None
            ...
            if args.latdir:
                ...
                # ---- INSTRUMENTATION (A): is a* still in the linear regime, and what -----
                # ---- is the pre-update policy, snapshotted for the part-(B) diff? --------
                # This whole block is inside the enclosing torch.no_grad(); the one
                # enable_grad() below builds a graph over a DETACHED ACTION LEAF ONLY -- the
                # concentrations enter detached, so no policy parameter is in that graph and
                # torch.autograd.grad(inputs=(a_leaf,)) cannot populate any .grad.
                if args.latdir_diag:
                    span_a = agent.action_high - agent.action_low
                    flat_obs = obs.reshape(flat_obs_shape)
                    z_taken = latent_zs.reshape(n_flat, act_dim_sf)
                    d_chunk = args.latdir_diag_chunk if args.latdir_diag_chunk > 0 else n_flat
                    mu_old = torch.empty(n_flat, act_dim_sf, device=device)
                    logc_old = torch.empty(n_flat, act_dim_sf, device=device)
                    lp_old_taken = torch.empty(n_flat, device=device)
                    lp_old_star = torch.empty(n_flat, device=device)
                    lin_pred_t = torch.empty(n_flat, device=device)
                    lin_pred_del_t = torch.empty(n_flat, device=device)
                    # The DELIVERED displacement, reconstructed exactly from the z* the loss
                    # actually uses -- so it carries BOTH clamps (a* into the action box at
                    # parent line ~1980, then z* into [z_eps, 1-z_eps]). eta*ghat is the
                    # REQUESTED step and differs from this whenever latdir/frac_clamped > 0.
                    a_star_eff = agent.action_low + span_a * b_z_star
                    for s in range(0, n_flat, d_chunk):
                        sl = slice(s, min(s + d_chunk, n_flat))
                        c1, c0, mean_a, _std_a, lp_z, lp_zs = agent.actor_diag(
                            flat_obs[sl], z_taken[sl], b_z_star[sl]
                        )
                        mu_old[sl] = mean_a
                        # (m, c) reparametrization: c = c1 + c0 >= 2, so the log is safe
                        # without a clamp and a clamp would silently hide a collapse.
                        logc_old[sl] = (c1 + c0).log()
                        lp_old_taken[sl] = lp_z
                        lp_old_star[sl] = lp_zs
                        # FIRST-ORDER prediction of log pi_old(a*) - log pi_old(a).
                        # d/da is taken through the affine z = (a - low)/span, which is the
                        # exact chain rule for the action-space density: the affine map's
                        # log-Jacobian is a CONSTANT, so it shifts log pi by a constant and
                        # contributes nothing to d/da. No clamp on z here: the rollout sample
                        # was already clamped to [SAMPLE_EPS, 1-SAMPLE_EPS], so a is interior
                        # and a clamp could only zero a live gradient at the boundary.
                        with torch.enable_grad():
                            a_leaf = a_flat[sl].detach().requires_grad_(True)
                            lp_sum = (
                                Beta(c1.detach(), c0.detach())
                                .log_prob((a_leaf - agent.action_low) / span_a)
                                .sum()
                            )
                            grad_a = torch.autograd.grad(
                                lp_sum, a_leaf, create_graph=False
                            )[0]
                        lin_pred_t[sl] = args.latdir_eta * (ghat[sl] * grad_a).sum(-1)
                        lin_pred_del_t[sl] = (
                            (a_star_eff[sl] - a_flat[sl]) * grad_a
                        ).sum(-1)
                    d_pre = lp_old_star - lp_old_taken
                    diag_logp_star_minus_taken_pre = float(d_pre.mean())
                    diag_lin_pred = float(lin_pred_t.mean())
                    # RATIO OF THE MEANS, not the mean of per-sample ratios: the per-sample
                    # denominator passes through zero (ghat is orthogonal to grad log pi for
                    # some rows by construction), so the per-sample ratio is heavy-tailed and
                    # its sample mean does not estimate anything.
                    diag_lin_ratio = (
                        diag_logp_star_minus_taken_pre / diag_lin_pred
                        if abs(diag_lin_pred) > 1e-12
                        else float("nan")
                    )
                    diag_lin_pred_delivered = float(lin_pred_del_t.mean())
                    # READ lin_corr AND lin_slope, NOT lin_ratio. ghat is near-orthogonal to
                    # grad log pi for a typical row, so E_t[lin_pred_t] concentrates at ZERO
                    # while E_t[d_pre] does not -- the ratio of the means then divides a
                    # stable numerator by a noise-dominated denominator and swings over
                    # orders of magnitude with no signal in it (measured: +30, -242, +4.6 on
                    # consecutive iterations). The PAIRED statistics below are the same
                    # question asked in a way that survives that:
                    #   lin_corr  ~ 1  and  lin_slope ~ 1  => a* is in the linear regime
                    #   lin_corr  -> 0                     => the actual change in log pi at
                    #                                         a* is unrelated to the
                    #                                         first-order term, i.e. a* has
                    #                                         been thrown into the tail and
                    #                                         curvature owns the objective
                    # lin_ratio is kept exactly as specified so the requested number is on
                    # the dashboard; it is not the one to trust.
                    #
                    # Paired against lin_pred_DELIVERED, not lin_pred: d_pre is the change in
                    # log pi across the CLAMPED step, so linearizing the UNCLAMPED one would
                    # blame clamping on curvature. They differ exactly to the extent
                    # latdir/frac_clamped is nonzero.
                    #
                    # Both are mean-centred, so a constant offset between d_pre and the
                    # linear term is invisible here BY CONSTRUCTION -- read it off directly
                    # as logp_star_minus_taken_pre minus lin_pred_delivered.
                    _u = lin_pred_del_t - lin_pred_del_t.mean()
                    _v = d_pre - d_pre.mean()
                    _uu = _u.pow(2).mean()
                    _vv = _v.pow(2).mean()
                    # NaN, not 0, when the regressor is constant. At iteration 1 g is exactly
                    # zero so a* == a and lin_pred_del_t == d_pre == 0; reporting 0 there
                    # would be indistinguishable from the "curvature owns the objective"
                    # reading, which is the opposite conclusion.
                    if bool(_uu > 0) and bool(_vv > 0):
                        diag_lin_corr = float((_u * _v).mean() / (_uu.sqrt() * _vv.sqrt()))
                        diag_lin_slope = float((_u * _v).mean() / _uu)
                    else:
                        diag_lin_corr = float("nan")
                        diag_lin_slope = float("nan")
                    diag_ghat_norm_mean = float(ghat.norm(dim=-1).mean())
                    # Handoff to part (B). ghat is the direction the update was ASKED to move
                    # along; a_flat and a_star_eff are carried so (B) can separate a mean
                    # that moved toward a* from one that merely collapsed onto the sampled
                    # action a. All five are read-only from here on -- nothing in the update
                    # phase mutates them in place, so holding the references is safe.
                    diag_snap = (
                        mu_old, logc_old, lp_old_taken, lp_old_star,
                        ghat, a_flat, a_star_eff,
                    )
            else:
        ...
        # ---- INSTRUMENTATION (B): TRANSLATION vs SHARPENING ----------------------------
        # The decisive measurement, and the reason this fork exists. ONE extra no_grad
        # forward over the SAME states, with the policy that the update phase just produced,
        # compared against the pre-update snapshot taken at rollout time. No optimizer, no
        # loss and no RNG is touched between the snapshot and here beyond the update itself.
        #
        # Placed AFTER the whole epoch loop (all update_epochs x num_minibatches, including
        # an early target_kl break) and BEFORE the SSL step -- the SSL step trains the
        # encoder/predictor only and cannot move the actor, but ordering it this way keeps
        # "mu_new" unambiguously "the policy the next rollout will act with".
        diag_dmean_align = float("nan")
        diag_dmean_align_null = float("nan")
        diag_dmean_align_taken = float("nan")
        diag_dmean_align_astar = float("nan")
        diag_proj_realized = float("nan")
        diag_dmean_norm = float("nan")
        diag_dlogc = float("nan")
        diag_dlogp_star = float("nan")
        diag_dlogp_taken = float("nan")
        diag_dlogp_gap = float("nan")
        diag_c_mean = float("nan")
        diag_act_std_mean = float("nan")
        if diag_snap is not None:
            (mu_old, logc_old, lp_old_taken, lp_old_star,
             ghat_snap, a_snap, a_star_snap) = diag_snap
            n_d = mu_old.shape[0]
            d_chunk = args.latdir_diag_chunk if args.latdir_diag_chunk > 0 else n_d
            with torch.no_grad():
                mu_new = torch.empty_like(mu_old)
                logc_new = torch.empty_like(logc_old)
                std_new = torch.empty_like(mu_old)
                lp_new_taken = torch.empty_like(lp_old_taken)
                lp_new_star = torch.empty_like(lp_old_star)
                for s in range(0, n_d, d_chunk):
                    sl = slice(s, min(s + d_chunk, n_d))
                    c1, c0, mean_a, std_a, lp_z, lp_zs = agent.actor_diag(
                        b_obs[sl], b_latent_zs[sl], b_z_star[sl]
                    )
                    mu_new[sl] = mean_a
                    logc_new[sl] = (c1 + c0).log()
                    std_new[sl] = std_a
                    lp_new_taken[sl] = lp_z
                    lp_new_star[sl] = lp_zs

                dmu = mu_new - mu_old                                  # (N, act_dim)
                dmu_n = dmu.norm(dim=-1)
                ghat_n = ghat_snap.norm(dim=-1)

                def _align(v):
                    """mean_t cos(dmu_t, v_t), over rows where v is nonzero.

                    Shared by all four alignment readouts so they cannot drift apart in
                    masking or in the degenerate-row convention.
                    """
                    vn = v.norm(dim=-1)
                    ok = vn > 0
                    cs = (dmu * v).sum(-1) / (dmu_n * vn).clamp_min(1e-30)
                    n = ok.sum()
                    return (
                        float(torch.where(ok, cs, torch.zeros_like(cs)).sum() / n)
                        if bool(n > 0) else float("nan")
                    )

                # NULL BASELINE for dmean_align, and it is not optional. share_backbone is on
                # and the dual-backward at the end of every minibatch ADDS the clipped critic
                # gradient onto the same trunk that produces mu, so dmu = latdir signal +
                # critic-driven drift. Pairing each row's dmu with a DIFFERENT row's ghat
                # measures what that drift alone scores. The pairing is a fixed roll by N/2
                # rather than a permutation precisely because a permutation would need the
                # RNG, and this file may not touch it; N/2 lands in a different half of the
                # rollout, so the pair is temporally decorrelated.
                ghat_null = ghat_snap.roll(ghat_snap.shape[0] // 2, dims=0)
                # SELF-IMITATION probe. a - mu_old is where the mean would move if the
                # objective had degenerated into "make the action you already took more
                # likely" -- which raises log pi(a*) almost as much as a real improvement
                # step, since a* is only eta away from a, while carrying nothing from the
                # critic. dlogp_gap CANNOT tell that apart from a real step (measured: a 20%
                # move onto a gives gap = +0.51); proj_realized and this pair can.
                diag_dmean_align_null = _align(ghat_null)
                diag_dmean_align_taken = _align(a_snap - mu_old)
                diag_dmean_align_astar = _align(a_star_snap - mu_old)

                # THE HEADLINE. Per-sample cosine between the realized mean displacement and
                # the direction the objective asked for.
                #
                # ITS CEILING IS NOT +1. Under a PERFECT translation toward a* it reads
                # +0.24 in this file's geometry, because a* - mu = (a - mu) + eta*ghat and
                # ||a - mu|| ~ 1.09 against ||eta*ghat|| = 0.3 (measured, see the header).
                # Compare it to dmean_align_null, never to 1, and prefer proj_realized --
                # that one is unbiased for the delivered fraction of the eta-step.
                #
                # The two degenerate cases are handled DIFFERENTLY, on purpose. ||ghat|| == 0
                # means no direction was ever requested (iteration 1: g is identically zero
                # because ConditionalBlock is AdaLN-zero, see the parent header) -- the
                # cosine is undefined and the row is EXCLUDED, so the whole metric reads NaN
                # there rather than a spurious "no alignment". ||dmu|| == 0 means a direction
                # WAS requested and the mean did not move at all -- that is exactly zero
                # alignment, a real measurement, so the clamp_min below reports it as 0 and
                # the row is KEPT. Collapsing the two would make pure sharpening (mean
                # frozen, concentration rising) invisible, which is the hypothesis under test.
                diag_dmean_align = _align(ghat_snap)
                # THE OTHER HEADLINE, and the better-behaved one: the signed projection of
                # the realized mean displacement onto the requested step, in units of eta.
                # A perfect translation f of the way to a* reads exactly f (measured: 0.192
                # at f=0.2, 0.480 at f=0.5), because E_t[(a - mu).ghat] = 0 -- so unlike
                # dmean_align this has no geometric ceiling below 1.
                #
                # ONE CAVEAT, unhidden: ||ghat|| = min(1, ||g||/g_floor) <= 1, and the true
                # fraction of the intended step is <dmu,ghat>/(eta*||ghat||^2). The formula
                # below omits the ||ghat||^2, so while ||g|| < latdir_g_floor = 1e-3 (the
                # first handful of iterations, when the AdaLN-zero predictor has barely any
                # action sensitivity) it UNDER-reports by that factor. diag/ghat_norm_mean
                # is logged so the correction is available; it is 1.0 the rest of the time.
                #
                # Unlike dmean_align this does NOT exclude ||ghat|| == 0 rows -- they
                # contribute exactly 0, which is the correct contribution to a projection.
                diag_proj_realized = float((dmu * ghat_snap).sum(-1).mean() / args.latdir_eta)
                diag_dmean_norm = float(dmu_n.mean())
                # POSITIVE => the total concentration rose => the policy SHARPENED.
                diag_dlogc = float((logc_new - logc_old).mean())
                # What the objective directly maximizes. Ceiling ~log(1+clip_coef_high)
                # = 0.247 nats per sample before the clip kills its gradient.
                diag_dlogp_star = float((lp_new_star - lp_old_star).mean())
                # The density of the action ALREADY TAKEN. For an update that were
                # independent of this sample E[dlogp_taken] = -KL(pi_old||pi_new) <= 0; it is
                # positive exactly to the extent the update memorized its own batch, so a
                # large positive value here is itself the self-imitation signature.
                diag_dlogp_taken = float((lp_new_taken - lp_old_taken).mean())
                # Differential progress toward a* over a. NOTE it does NOT separate a real
                # step from self-imitation -- both make it positive (measured). Its
                # informative reading is the SIGN against sharpening, which drives it
                # negative (a* sits ~0.85 std out, where raising c LOWERS the density).
                diag_dlogp_gap = diag_dlogp_star - diag_dlogp_taken
                # Absolute levels, so a collapse accumulating over training is visible rather
                # than inferred from a sequence of per-iteration deltas.
                diag_c_mean = float(logc_new.exp().mean())
                diag_act_std_mean = float(std_new.mean())
        ...
        # ---- TRANSLATION vs SHARPENING panel (this fork's whole addition) --------------
        # Read proj_realized and (dmean_align vs dmean_align_null) FIRST. See the header
        # for the measured reading key and for why dmean_align's ceiling is +0.24, not +1.
        #   translate -> a*  : proj_realized = the delivered fraction, dmean_align >> null,
        #                      dmean_align_taken ~ +0.96, dlogc ~ 0, dlogp_gap > 0
        #   sharpen          : every align ~ 0, dlogc > 0, dlogp_star < 0, dlogp_gap < 0
        #   self-imitate -> a: proj_realized ~ 0, dmean_align ~ null, dmean_align_taken ~ +1,
        #                      dlogc ~ 0, dlogp_gap > 0  <-- looks like improvement, is not
        writer.add_scalar("diag/logp_star_minus_taken_pre", diag_logp_star_minus_taken_pre, global_step)
        writer.add_scalar("diag/lin_pred", diag_lin_pred, global_step)
        writer.add_scalar("diag/lin_ratio", diag_lin_ratio, global_step)
        writer.add_scalar("diag/lin_corr", diag_lin_corr, global_step)
        writer.add_scalar("diag/lin_slope", diag_lin_slope, global_step)
        writer.add_scalar("diag/lin_pred_delivered", diag_lin_pred_delivered, global_step)
        writer.add_scalar("diag/ghat_norm_mean", diag_ghat_norm_mean, global_step)
        writer.add_scalar("diag/dmean_align", diag_dmean_align, global_step)
        writer.add_scalar("diag/dmean_align_null", diag_dmean_align_null, global_step)
        writer.add_scalar("diag/dmean_align_taken", diag_dmean_align_taken, global_step)
        writer.add_scalar("diag/dmean_align_astar", diag_dmean_align_astar, global_step)
        writer.add_scalar("diag/proj_realized", diag_proj_realized, global_step)
        writer.add_scalar("diag/dmean_norm", diag_dmean_norm, global_step)
        writer.add_scalar("diag/dlogc", diag_dlogc, global_step)
        writer.add_scalar("diag/dlogp_star", diag_dlogp_star, global_step)
        writer.add_scalar("diag/dlogp_taken", diag_dlogp_taken, global_step)
        writer.add_scalar("diag/dlogp_gap", diag_dlogp_gap, global_step)
        writer.add_scalar("diag/c_mean", diag_c_mean, global_step)
        writer.add_scalar("diag/act_std_mean", diag_act_std_mean, global_step)
