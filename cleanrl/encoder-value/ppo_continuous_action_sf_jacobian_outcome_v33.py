...
#
# ==== V32 POST-MORTEM: THE LEARNED JACOBIAN DID NOT EARN ITS PLACE ====================
# Also measured at 0.85M, and the reason jac_use_learned now defaults to False:
#   jac/attrib_corr        0.169-0.215   vs a null s.d. of 0.0055 -- the attribution
#                                        genuinely predicts the realized advantage
#   jac/asf_delta_ev       0.001-0.002   held-out incremental EV of the ACTION-dependent
#                                        terms: the learned half predicts nothing
#   jac/learned_frac       0.66-0.70     ...while carrying two thirds of ||g||
# Magnitude without content: the learned half was diluting an exact analytic direction
# with estimation noise. That is the identification problem the design always had -- the
# head is fit on the narrow band of on-policy action noise around mu, where (a - mu) is
# the only variation separating c(s) from B(s)a. The analytic half needs no fitting and
# is exact, so v33 runs on it alone until delta_ev says otherwise. The head is still fit
# and still logged, purely so delta_ev keeps reporting.
#
...
class Args:
    ...
    jac_clip_match: str = "sqrt"     # per-coordinate clip bound: "sqrt" = FREQUENCY-matched
    #                                  to v9's joint clip, "linear" = product-matched (v32's
    #                                  failure, far too tight), "joint" = v9's bounds applied
    #                                  per coordinate (far too loose)
    jac_use_learned: bool = False    # include the LEARNED half of the action Jacobian.
    #                                  v32 measured jac/asf_delta_ev ~ 0.001 held-out while
    #                                  the learned half carried 66-70% of ||g||: magnitude
    #                                  without content, i.e. it dilutes the exact analytic
    #                                  direction. Off by default until delta_ev earns it.
...
def perdim_policy_loss(
    advantage, shift, omega, ratio_dims, clip_coef, clip_coef_high, clip_match
):
    """PPO surrogate with per-action-dimension credit and a GEOMETRY-MATCHED clip.

    The ratio must be the TRUE per-dimension ratio exp(log pi_j - log pi_j^old), not the
    joint ratio broadcast: d ratio_j / d theta = ratio_j * grad log pi_j, which is what
    makes A_j the credit that coordinate j actually receives. With A_j = A for every j
    and every ratio_j = 1 this reproduces the scalar PPO gradient EXACTLY -- pairing
    per-dimension ratios with A/act_dim instead divides the whole policy gradient by
    act_dim, which is silent because the direction is unchanged.

    THE PER-COORDINATE BOUND IS FREQUENCY-MATCHED, NOT PRODUCT-MATCHED. v32 set the
    bounds to the act_dim-th root so the PRODUCT of clamped ratios would land in v9's
    [1-clip_coef, 1+clip_coef_high] band. That reasoning confuses bounding a product with
    bounding each factor, and it is catastrophically tighter: with roughly independent
    coordinates sd(log r_j) = sd(log r)/sqrt(n), so a typical |log r_j| ~ 0.037 sits right
    against log(1.28)/6 = 0.041 and the clip binds on ~half of all coordinates, against
    v9's joint clipfrac of 0.023. v32 measured the consequence directly: entropy pinned at
    its initialization value (-1.01, exactly H(Beta) at init) and return at 859 @848k
    versus v9's ~3000 -- an actor that never moved.

    The operationally meaningful match is on HOW OFTEN the clip binds. Since log-ratios add
    across coordinates, that is the sqrt(n) root, giving [0.913, 1.106] at the defaults.
    The joint ratio is then no longer bounded by v9's band (it can reach 1.106^n), so
    target_kl remains the aggregate leash and jac/clipfrac_dim reports the realized rate --
    it should land near v9's losses/clipfrac, and a large gap means the match failed.
    """
    ...
    clipped = ((ratio_dims < lo) | (ratio_dims > hi)).float().mean()
    return loss, adv_dims, clipped
