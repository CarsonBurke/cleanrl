# PPO + the v9 successor-feature critic + an always-on Bellman-closed value embedding.
#
# The actor, SF value readout, SF targets, LeJEPA path, initialization stream, and PPO
# loss are exactly v9. A separate head predicts learned nonlinear coordinates of a
# return. A learned composer is affine in the next latent and is trained toward
# C(r, H(g)) = H(r + gamma*g), so expectation and composition commute. The state critic
# recursively bootstraps the exact scalar and all seven rich coordinates together; the
# rich path is never reduced to a scalar fallback. The exact v9 expected-return readout
# remains PPO's scalar objective coordinate.
# Cross-correlation aligns every predicted coordinate with a distinct target coordinate
# without manufacturing state variance. There is no gate, projection, or fallback.
#
...
class Args:
    ...
    # --- Bellman-closed rich state value --------------------------------------------
    return_code_dim: int = 7
    return_hidden: int = 64
    composer_hidden: int = 64
    composer_matrix_scale: float = 0.1
    return_scale: float = 100.0
    return_feature_reg_coef: float = 1.0
    return_algebra_coef: float = 1.0
    return_model_lr: float = 1e-4
    return_model_epochs: int = 4
    return_model_batch: int = 4096
    return_model_grad_clip: float = 1.0
    return_encoder_ema_rate: float = 0.05
    rich_value_coef: float = 6.0    # match v9's aggregate six-horizon SF loss scale
    rich_alignment_coef: float = 1.0
    rich_eval_envs: int = 2
...
class ReturnKernelEncoder(nn.Module):
    """Learn a stable nonlinear basis of a fixed-scale scalar return."""
    ...
    def forward(self, scaled_return):
        # Unlike clipping or tanh, asinh remains injective and unbounded. It compresses
        # rare return extremes before two squared activations without creating a saturated
        # region or making the learned code batch-relative.
        return self.net(torch.asinh(scaled_return).unsqueeze(-1))


class AffineReturnComposer(nn.Module):
    """Stable affine action on [exact scalar mean, learned rich coordinates]."""

    def __init__(self, rich_dim, hidden, gamma, matrix_scale):
        super().__init__()
        self.rich_dim = rich_dim
        self.matrix_scale = matrix_scale
        self.matrix_size = rich_dim * rich_dim
        self.context = nn.Sequential(
            layer_init(nn.Linear(1, hidden)),
            LeakyReLUSquared(),
            layer_init(
                nn.Linear(hidden, self.matrix_size + 2 * rich_dim),
                std=0.01,
            ),
        )
        initial_diagonal = min(gamma, 0.998)
        diagonal_logit = log(initial_diagonal / (0.999 - initial_diagonal))
        self.diagonal_logits = nn.Parameter(
            torch.full((rich_dim,), diagonal_logit)
        )

    def affine_map(self, reward_offset):
        parameters = self.context(reward_offset.unsqueeze(-1))
        matrix_residual = (
            self.matrix_scale
            * torch.tanh(parameters[..., : self.matrix_size])
        ).reshape(
            *parameters.shape[:-1], self.rich_dim, self.rich_dim
        )
        scalar_column = torch.tanh(
            parameters[
                ..., self.matrix_size : self.matrix_size + self.rich_dim
            ]
        )
        bias = torch.tanh(parameters[..., self.matrix_size + self.rich_dim :])
        diagonal = 0.999 * torch.sigmoid(self.diagonal_logits)
        rich_matrix = (
            torch.diag_embed(diagonal)
            + matrix_residual
        )
        row_l1 = rich_matrix.abs().sum(-1)
        row_scale = torch.clamp(
            0.999 / row_l1.clamp_min(1e-8),
            max=1.0,
        )
        rich_matrix = rich_matrix * row_scale.unsqueeze(-1)
        return rich_matrix, scalar_column, bias

    def forward(self, reward_offset, next_full_value):
        rich_matrix, scalar_column, bias = self.affine_map(reward_offset)
        scalar_contribution = (
            scalar_column * next_full_value[..., :1]
        )
        rich_contribution = torch.einsum(
            "...ij,...j->...i",
            rich_matrix,
            next_full_value[..., 1:],
        )
        return rich_contribution + scalar_contribution + bias
...
def return_code_regularization(coords, standardized_returns):
    """Noncollapse and true scale-invariant decorrelation."""
    ...
    # Do not detach std. Detaching creates a spurious radial gradient which repeatedly
    # shrinks the code while reporting a nominally scale-invariant correlation loss.
    normalized = centered / std.clamp_min(1e-4)
    corr = normalized.T @ normalized / coords.shape[0]
    ...
    scalar = scalar / scalar.std(unbiased=False).clamp_min(1e-4)
...
def cross_correlation_alignment(prediction, target):
    """Make each predicted coordinate track one distinct frozen target coordinate."""
    prediction_centered = prediction - prediction.mean(0, keepdim=True)
    target_centered = target - target.mean(0, keepdim=True)
    prediction_normalized = prediction_centered / prediction_centered.std(
        0, unbiased=False
    ).clamp_min(1e-4)
    target_normalized = target_centered / target_centered.std(
        0, unbiased=False
    ).clamp_min(1e-4)
    cross_correlation = (
        prediction_normalized.T @ target_normalized / prediction.shape[0]
    )
    diagonal = torch.diagonal(cross_correlation)
    diagonal_loss = (diagonal - 1.0).square().mean()
    offdiagonal = cross_correlation - torch.diag_embed(diagonal)
    offdiagonal_loss = offdiagonal.square().mean()
    return diagonal_loss + offdiagonal_loss, diagonal_loss, offdiagonal_loss
...
def compute_sampled_returns(
    ...
    """Reverse bootstrapped return sample with correct boundary censoring."""
    ...
    for t in reversed(range(rewards.shape[0])):
        ...
        sampled_returns[t] = (
            rewards[t] + gamma * (1.0 - terminations[t]) * future_return
        )
        sampled_valids[t] = current_valid
    return sampled_returns, sampled_valids


def compute_recursive_embedding_targets(
    reward_offsets,
    next_scalar_values,
    next_rich_codes,
    zero_rich_code,
    ...
    """Compose a full [exact scalar, rich coordinates] state value backward."""
    scalar_targets = torch.zeros_like(reward_offsets)
    rich_targets = torch.zeros_like(next_rich_codes)
    valids = torch.zeros_like(terminations, dtype=torch.bool)
    for t in reversed(range(reward_offsets.shape[0])):
        if t == reward_offsets.shape[0] - 1:
            future_scalar = next_scalar_values[t]
            future_rich = next_rich_codes[t]
            future_valid = bootstrap_valids[t].bool()
        else:
            crosses_boundary = boundaries[t].bool()
            future_scalar = torch.where(
                crosses_boundary,
                next_scalar_values[t],
                scalar_targets[t + 1],
            )
            future_rich = torch.where(
                crosses_boundary.unsqueeze(-1),
                next_rich_codes[t],
                rich_targets[t + 1],
            )
            future_valid = torch.where(
        ...
        terminal = terminations[t].bool()
        tail_scalar = torch.where(
            terminal,
            torch.zeros_like(future_scalar),
            future_scalar,
        )
        tail_rich = torch.where(
            terminal.unsqueeze(-1),
            zero_rich_code.expand_as(future_rich),
            future_rich,
        )
        full_tail_value = torch.cat(
            (tail_scalar.unsqueeze(-1), tail_rich), dim=-1
        )
        scalar_targets[t] = reward_offsets[t] + gamma * tail_scalar
        rich_targets[t] = composer(
            reward_offsets[t], full_tail_value
        )
        valids[t] = torch.logical_or(
            terminal,
            torch.logical_and(bootstrap_valids[t].bool(), future_valid),
        )
    return rich_targets, scalar_targets, valids
...
def effective_rank(x):
    ...
    if not eigenvalues.numel():
        return 0.0
    weights = eigenvalues / eigenvalues.sum().clamp_min(1e-12)
...
if __name__ == "__main__":
    ...
    if not 0 < args.rich_eval_envs < args.num_envs:
        raise ValueError("rich_eval_envs must be between zero and num_envs")
    if not args.separate_grad_clip:
        raise ValueError("v19 requires separate actor/critic gradient clipping")
    ...
    # Top-level auxiliary modules cannot enter v9's Agent optimizer or clipping groups.
    # Private initialization preserves the exact v9 CPU RNG stream.
    with torch.random.fork_rng(devices=[]):
        rich_value_head = layer_init(
            nn.Linear(args.hidden, args.return_code_dim, bias=False), std=0.01
        )
        return_encoder = ReturnKernelEncoder(
            args.return_code_dim, args.return_hidden
        )
        return_composer = AffineReturnComposer(
            args.return_code_dim,
            args.composer_hidden,
            args.gamma,
            args.composer_matrix_scale,
        )
    rich_value_head = rich_value_head.to(device)
    joint_critic_params = critic_params + list(rich_value_head.parameters())
    return_encoder = return_encoder.to(device)
    return_composer = return_composer.to(device)
    target_return_encoder = deepcopy(return_encoder).requires_grad_(False)
    target_return_composer = deepcopy(return_composer).requires_grad_(False)
    rich_optimizer = optim.Adam(
        rich_value_head.parameters(), lr=args.learning_rate, eps=1e-5
    )
    return_optimizer = optim.Adam(
        list(return_encoder.parameters())
        + list(return_composer.parameters()),
        lr=args.return_model_lr,
    )
    auxiliary_generator = torch.Generator(device=device)
    auxiliary_generator.manual_seed(args.seed + 170_017)
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            rich_optimizer.param_groups[0]["lr"] = lrnow
        ...
        with torch.no_grad():
            ...
            next_critic_feat_buf = agent._trunks(
                next_obses.reshape(flat_obs_shape)
            )[1]
            psi_next = psi_raw(
                agent.critic_head(next_critic_feat_buf).view(
            ...
            # Scalar samples train the embedding algebra. The state-value target
            # recursively bootstraps the full [exact scalar, rich coordinates] value.
            ...
            scaled_sampled_return = sampled_returns / args.return_scale
            sampled_next_return = (
                sampled_returns - rewards
            ) / args.gamma
            scaled_sampled_next_return = (
                sampled_next_return / args.return_scale
            )
            reward_offsets = rewards / args.return_scale
            next_scaled_values = (
                next_transition_values / args.return_scale
            )
            old_next_rich = rich_value_head(next_critic_feat_buf).reshape(
                args.num_steps, args.num_envs, args.return_code_dim
            )
            zero_code = target_return_encoder(
                torch.zeros(1, device=device)
            )[0]
            (
                rich_target,
                recursive_scaled_return,
                rich_valid,
            ) = compute_recursive_embedding_targets(
                reward_offsets,
                next_scaled_values,
                old_next_rich,
                zero_code,
                ...
                transition_boundaries,
                transition_valids,
                target_return_composer,
                args.gamma,
            )
            event_target = target_return_encoder(
                scaled_sampled_return.reshape(-1)
            ).reshape(args.num_steps, args.num_envs, args.return_code_dim)
            event_consistency_mse = float(
                (
                    rich_target[rich_valid] - event_target[rich_valid]
                ).square().mean()
            )
            scalar_recursion_max_error = float(
                (
                    recursive_scaled_return[rich_valid]
                    - scaled_sampled_return[rich_valid]
                ).abs().max()
            )

            # Pre-update temporal evaluation: the head has not trained on this rollout.
            environment_index = torch.arange(args.num_envs, device=device)
            evaluation_environment = environment_index < args.rich_eval_envs
            rich_evaluation = rich_valid & evaluation_environment.unsqueeze(0)
            preupdate_rich_prediction = rich_value_head(critic_feat_buf).reshape(
                args.num_steps, args.num_envs, args.return_code_dim
            )
            held_target = rich_target[rich_evaluation]
            held_prediction = preupdate_rich_prediction[rich_evaluation]
            target_variance = held_target.var(0, unbiased=False).clamp_min(1e-8)
            heldout_rich_nmse = float(
                (
                    (held_target - held_prediction).square().mean(0)
                    / target_variance
                ).mean()
            )
            heldout_rich_ev = 1.0 - heldout_rich_nmse
            target_effective_rank = effective_rank(held_target)
            prediction_effective_rank = effective_rank(held_prediction)
            target_coordinate_std = held_target.std(0)
            prediction_coordinate_std = held_prediction.std(0)
            prediction_feature_std = float(prediction_coordinate_std.mean())
        ...
        b_rich_target = rich_target.reshape(-1, args.return_code_dim)
        b_rich_valid = rich_valid.reshape(-1)
        b_rich_evaluation = rich_evaluation.reshape(-1)
        b_rich_train = b_rich_valid
        b_scaled_sampled_return = scaled_sampled_return.reshape(-1)
        b_scaled_sampled_next_return = scaled_sampled_next_return.reshape(-1)
        b_reward_offsets = reward_offsets.reshape(-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                rich_prediction = rich_value_head(critic_feat)
                rich_valid = b_rich_train[mb_inds]
                rich_weight = rich_valid.to(rich_prediction.dtype)
                rich_prediction_loss = (
                    (
                        rich_prediction - b_rich_target[mb_inds]
                    ).square().mean(-1)
                    * rich_weight
                ).sum() / rich_weight.sum().clamp_min(1.0)
                (
                    rich_alignment_loss,
                    rich_alignment_diagonal_loss,
                    rich_alignment_offdiagonal_loss,
                ) = cross_correlation_alignment(
                    rich_prediction[rich_valid],
                    b_rich_target[mb_inds][rich_valid].detach(),
                )
                rich_value_loss = (
                    rich_prediction_loss
                    + args.rich_alignment_coef * rich_alignment_loss
                )
                ...
                if args.separate_grad_clip:
                    # The SF and rich objectives are one critic. Rich supervision enters
                    # the trunk from update one and shares the full critic clip budget.
                    optimizer.zero_grad(set_to_none=True)
                    rich_optimizer.zero_grad(set_to_none=True)
                    joint_value_loss = (
                        v_loss + args.rich_value_coef * rich_value_loss
                    )
                    (args.vf_coef * joint_value_loss).backward(retain_graph=True)
                    critic_gn = nn.utils.clip_grad_norm_(
                        joint_critic_params, args.critic_grad_clip
                    )
                    value_grads = {
                        p: p.grad.detach().clone()
                        for p in joint_critic_params
                    ...
                    loss = (
                        ...
                        + (
                            v_loss
                            + args.rich_value_coef * rich_value_loss
                        )
                        * args.vf_coef
                    )
                    optimizer.zero_grad()
                    rich_optimizer.zero_grad()
                    loss.backward()
                    critic_gn = actor_gn = nn.utils.clip_grad_norm_(
                        list(agent.parameters())
                        + list(rich_value_head.parameters()),
                        args.max_grad_norm,
                    )
                    optimizer.step()
                    rich_optimizer.step()
        ...
        # Learn a finite-dimensional algebra of affine return transformations:
        # C(offset, H(g)) ~= H(offset + gamma*g). C is affine in H(g), so the
        # identity remains valid after conditional expectation.
        return_algebra_loss_sum = 0.0
        return_reg_loss_sum = return_var_loss_sum = 0.0
        return_corr_loss_sum = return_scalar_corr_loss_sum = 0.0
        return_mean_loss_sum = return_grad_norm_sum = 0.0
        return_model_steps = 0
        model_valid_indices = torch.nonzero(
            b_rich_valid, as_tuple=False
        ).squeeze(-1)
        for _ in range(args.return_model_epochs):
            permutation = model_valid_indices[
                torch.randperm(
                    model_valid_indices.numel(),
                    device=device,
                    generator=auxiliary_generator,
                )
            ]
            for model_start in range(
                0, permutation.numel(), args.return_model_batch
            ):
                idx = permutation[
                    model_start : model_start + args.return_model_batch
                ]
                online_code = return_encoder(b_scaled_sampled_return[idx])
                online_next_code = return_encoder(
                    b_scaled_sampled_next_return[idx]
                )
                full_next_value = torch.cat(
                    (
                        b_scaled_sampled_next_return[idx].unsqueeze(-1),
                        online_next_code,
                    ),
                    dim=-1,
                )
                composed_code = return_composer(
                    b_reward_offsets[idx], full_next_value
                )
                return_algebra_loss = (
                    composed_code - online_code
                ).square().mean()
                (
                    return_reg_loss,
                    return_var_loss,
        ...
        return_denominator = max(return_model_steps, 1)
        return_algebra_loss_mean = (
            return_algebra_loss_sum / return_denominator
        )
        return_reg_loss_mean = return_reg_loss_sum / return_denominator
        return_var_loss_mean = return_var_loss_sum / return_denominator
        return_corr_loss_mean = return_corr_loss_sum / return_denominator
        return_scalar_corr_loss_mean = (
            return_scalar_corr_loss_sum / return_denominator
        )
        return_mean_loss_mean = return_mean_loss_sum / return_denominator
        return_grad_norm_mean = return_grad_norm_sum / return_denominator
        with torch.no_grad():
            ema_update(
            ...
            post_zero_code = target_return_encoder(
                torch.zeros(1, device=device)
            )[0]
            post_recursive_target, _, _ = compute_recursive_embedding_targets(
                reward_offsets,
                next_scaled_values,
                old_next_rich,
                post_zero_code,
            ...
            return_encoder_frame_rmse = float(
                (
                    post_recursive_target[rich_evaluation]
                    - rich_target[rich_evaluation]
                ).square().mean().sqrt()
            )
            composer_matrix, _, _ = target_return_composer.affine_map(
                b_reward_offsets[b_rich_valid]
            )
            composer_diagonal = torch.diagonal(
                composer_matrix, dim1=-2, dim2=-1
            )
            composer_row_l1 = composer_matrix.abs().sum(-1)
        ...
        # ---- learned rich-return auxiliary diagnostics -------------------------------
        writer.add_scalar("rich_value/loss", rich_value_loss.item(), global_step)
        writer.add_scalar(
            "rich_value/prediction_loss", rich_prediction_loss.item(), global_step
        )
        writer.add_scalar(
            "rich_value/alignment_loss",
            rich_alignment_loss.item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/alignment_diagonal_loss",
            rich_alignment_diagonal_loss.item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/alignment_offdiagonal_loss",
            rich_alignment_offdiagonal_loss.item(),
            global_step,
        )
        writer.add_scalar("rich_value/heldout_nmse", heldout_rich_nmse, global_step)
        writer.add_scalar("rich_value/heldout_ev", heldout_rich_ev, global_step)
        writer.add_scalar(
            "rich_value/target_effective_rank",
        ...
        writer.add_scalar(
            "rich_value/target_std_min",
            target_coordinate_std.min().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/target_std_mean",
            target_coordinate_std.mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/target_std_max",
            target_coordinate_std.max().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/prediction_std_mean",
            prediction_feature_std,
            global_step,
        )
        writer.add_scalar(
            "rich_value/prediction_std_min",
            prediction_coordinate_std.min().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/prediction_std_max",
            prediction_coordinate_std.max().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/return_algebra_loss",
            return_algebra_loss_mean,
            global_step,
        )
        writer.add_scalar(
            "rich_value/return_regularization_loss",
            return_reg_loss_mean,
            global_step,
        )
        writer.add_scalar(
            "rich_value/return_variance_loss",
            return_var_loss_mean,
            global_step,
        )
        writer.add_scalar(
            "rich_value/return_correlation_loss",
            return_corr_loss_mean,
            global_step,
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "rich_value/return_mean_loss",
            return_mean_loss_mean,
            global_step,
        )
        writer.add_scalar(
            "rich_value/return_grad_norm",
            return_grad_norm_mean,
            global_step,
        )
        writer.add_scalar(
            "rich_value/encoder_frame_rmse",
            return_encoder_frame_rmse,
            global_step,
        )
        writer.add_scalar(
            "rich_value/recursive_event_mse",
            event_consistency_mse,
            global_step,
        )
        writer.add_scalar(
            "rich_value/scalar_recursion_max_error",
            scalar_recursion_max_error,
            global_step,
        )
        writer.add_scalar(
            "rich_value/composer_diagonal_min",
            composer_diagonal.min().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/composer_diagonal_mean",
            composer_diagonal.mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/composer_diagonal_max",
            composer_diagonal.max().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/composer_row_l1_mean",
            composer_row_l1.mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/composer_row_l1_max",
            composer_row_l1.max().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/valid_fraction",
            b_rich_valid.float().mean().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/scaled_return_absmax",
            b_scaled_sampled_return[b_rich_valid].abs().max().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/recursive_target_absmax",
            b_rich_target[b_rich_valid].abs().max().item(),
            global_step,
        )
