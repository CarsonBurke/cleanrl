# PPO + LeJEPA multiscale geometric successor actor, v9.
#
# Ten direct heads predict normalized successor means at effective horizons from 1
# through 334 steps. Every head receives its own longest-suffix target and decay, giving
# dense temporal supervision to the shared representation. Only the final, exact-gamma
# head supplies actor values and the 16-step vector innovation trace, so the policy still
# optimizes one reward objective with one complete current-state baseline.
# LeJEPA uses attached multi-horizon targets, action-trace conditioning, and SIGReg. Its
# aligned online embedding feeds both actor and critic. A trust-region line search
# backtracks each proposed SSL parameter displacement and accepts the largest aligned
# trial satisfying policy, embedding, successor, and value drift limits. A frozen
# representation is the matched control.
import copy
import os
import random
import time
...
SHORT_GEOMETRIC_HORIZONS = (1, 2, 4, 8, 16, 32, 64, 128, 256)
GEOMETRIC_BETAS = tuple(
    1.0 - 1.0 / horizon for horizon in SHORT_GEOMETRIC_HORIZONS
) + (GEOMETRIC_GAMMA,)
GEOMETRIC_EFFECTIVE_HORIZONS = tuple(1.0 / (1.0 - beta) for beta in GEOMETRIC_BETAS)
SSL_LINE_SEARCH_ALPHAS = tuple(0.5**power for power in range(8))
...
class Args:
    ...
    ssl_control_mode: str = "line_search"  # "line_search" | "frozen"
...
def build_multiscale_full_suffix_targets(
    phi, next_means, bootstrap, continuation, betas
):
    """Direct normalized longest-suffix targets for every geometric decay.

    phi: (T,B,D), next_means: (T,B,K,D), output: (T,B,K,D).
    """
    beta = torch.as_tensor(
        betas, device=phi.device, dtype=phi.dtype
    ).view(1, -1, 1)
    if next_means.shape[2] != beta.shape[1]:
        raise ValueError("next_means head count does not match betas")
    targets = torch.empty_like(next_means)
    running = torch.zeros_like(next_means[0])
    for t in reversed(range(phi.shape[0])):
        if t == phi.shape[0] - 1:
            next_estimate = bootstrap[t].view(-1, 1, 1) * next_means[t]
        else:
            cont = continuation[t].view(-1, 1, 1)
            boundary_bootstrap = (
                (1.0 - cont)
                * bootstrap[t].view(-1, 1, 1)
                * next_means[t]
            )
            next_estimate = cont * running + boundary_bootstrap
        running = (1.0 - beta) * phi[t].unsqueeze(1) + beta * next_estimate
        targets[t] = running
    return targets
...
@torch.no_grad()
def transport_reward_gauge(
    critic_head, old_covector, new_covector, sf_dim
):
    """Keep every successor head fixed after changing the residual-coordinate gauge."""
    delta = old_covector - new_covector
    num_heads = critic_head.weight.shape[0] // sf_dim
    weight = critic_head.weight.view(num_heads, sf_dim, -1)
    bias = critic_head.bias.view(num_heads, sf_dim)
    weight[:, -1].add_(torch.einsum("d,kdh->kh", delta, weight[:, :-1]))
    bias[:, -1].add_(torch.einsum("d,kd->k", delta, bias[:, :-1]))


@torch.no_grad()
def reset_optimizer_output_rows(optimizer, layer, sf_dim):
    """Clear optimizer moments for every analytically transformed residual row."""
    rows = torch.arange(
        sf_dim - 1, layer.weight.shape[0], sf_dim, device=layer.weight.device
    )
    for parameter in (layer.weight, layer.bias):
        ...
        for name in ("exp_avg", "exp_avg_sq", "max_exp_avg_sq"):
            ...
            if moment is not None:
                moment.index_fill_(0, rows, 0.0)


def interpolate_parameter_dict(before, proposed, alpha):
    """One displacement trial over named parameters only."""
    trial = {}
    for name, old in before.items():
        new = proposed[name]
        trial[name] = old + alpha * (new - old)
    return trial


@torch.no_grad()
def load_parameter_dict(module, parameters):
    current = dict(module.named_parameters())
    if current.keys() != parameters.keys():
        raise ValueError("parameter dictionaries do not match module")
    for name, value in parameters.items():
        current[name].copy_(value)


def select_line_search_alpha(alphas, evaluate):
    """Return the first (therefore largest) trial satisfying evaluate(alpha)."""
    for alpha in alphas:
        if evaluate(alpha):
            return alpha
    return 0.0
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Direct normalized successor means at every geometric scale. The final
        # coordinate in every head is the exact lagged reward-probe residual.
        self.base_dim = args.emb_dim + obs_dim + 2 * act_dim + 1
        self.sf_dim = self.base_dim + 1  # [e, s, a, a*a, 1, reward residual]
        self.num_successor_heads = len(GEOMETRIC_BETAS)
        self.critic_head = layer_init(
            nn.Linear(H, self.num_successor_heads * self.sf_dim), std=0.1
        )
        with torch.no_grad():
    ...
    def get_value(self, x):
        # Returns all normalized geometric successor means (B, K, sf_dim).
        _, critic_feat = self._trunks(x)
        flat = self.critic_head(critic_feat)
        return flat.view(*flat.shape[:-1], self.num_successor_heads, self.sf_dim)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        value_sf = self.critic_head(critic_feat).view(
            *critic_feat.shape[:-1], self.num_successor_heads, self.sf_dim
        )
        if self.actor_dist == "gaussian":
...
def shape_advantage(raw_advantage, args, device):
    """Map raw fixed-horizon advantage according to args.adv_transform.
...
if __name__ == "__main__":
    ...
    if args.ssl_control_mode not in ("line_search", "frozen"):
        raise ValueError("ssl_control_mode must be line_search or frozen")
    ...
    if args.auto_entropy:
        raise ValueError("auto_entropy is not implemented for fixed-horizon vector credit")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            sf_target = build_multiscale_full_suffix_targets(
                phi,
                successor_next,
                bootstrap,
                continuation,
            ...
            one_step_target = build_one_step_geometric_target(
                phi, successor_next[:, :, -1], bootstrap, args.gamma
            )
            delta_sf = one_step_target - successor_cur[:, :, -1]
            innovation_trace = fixed_horizon_sum(
                delta_sf, continuation, args.gamma, args.advantage_horizon
            )
            advantages = (
                innovation_trace @ reward_task
            ) / (1.0 - args.gamma)
            policy_adv = advantages
            returns = advantages + values

            # Exact sampled prefix, used only to expose how much of actor credit comes
            # from the learned baseline/tail rather than observed rewards.
            exact_prefix = fixed_horizon_sum(
                rewards, continuation, args.gamma, args.advantage_horizon
            )
            learned_tail = advantages - exact_prefix
            tail_contribution_ratio = float(
                learned_tail.std() / exact_prefix.std().clamp_min(1e-8)
            )
            td0_vector = (delta_sf @ reward_task) / (1.0 - args.gamma)
            ...
            sf_loss_scale = (
                sf_target.reshape(-1, len(GEOMETRIC_BETAS), sf_dim)
            ...
            preupdate_mse_by_head = (
                ((successor_cur - sf_target) / sf_loss_scale)
                .square()
                .mean((0, 1, 3))
            )
            marginal_target = sf_target.reshape(
                -1, len(GEOMETRIC_BETAS), sf_dim
            ).mean(0)
            marginal_scaled_mse = (
                (sf_target - marginal_target) / sf_loss_scale
            ).square().mean()
            marginal_mse_by_head = (
                ((sf_target - marginal_target) / sf_loss_scale)
                .square()
                .mean((0, 1, 3))
            )
            ...
            preupdate_skill_by_head = (
                1.0
                - preupdate_mse_by_head
                / marginal_mse_by_head.clamp_min(1e-12)
            )
            ...
            for t in reversed(range(args.num_steps)):
                ...
                mc_ret[t], mc_avail[t], mc_resid[t] = (
                    mc_run,
                    avail_run,
                    resid_run,
                )
        ...
        b_sf_target = sf_target.reshape(
            -1, len(GEOMETRIC_BETAS), sf_dim
        )
        # Policy advantage: shape fixed-horizon credit per `adv_transform`. With
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                sf_mse_by_head = sf_err.pow(2).mean((0, 2))
                # Preserve the actor-facing long head under critic gradient clipping:
                # half of the loss belongs to it; all deep-supervision heads share half.
                v_loss = (
                    0.5 * sf_mse_by_head[-1]
                    + 0.5 * sf_mse_by_head[:-1].mean()
                )
        ...
        reset_optimizer_output_rows(optimizer, agent.critic_head, sf_dim)
        ...
        ssl_parameters_before = {
            name: parameter.detach().clone()
            for name, parameter in ssl.named_parameters()
        }
        ...
        with torch.no_grad():
            ...
            old_probe_values = (
                old_successors[:, -1] @ reward_task
            ) / (1.0 - args.gamma)
            probe_split = probe_obs.shape[0] // 2
            frame_fit = slice(0, probe_split)
            trust_eval = slice(probe_split, None)
            ssl_parameters_proposed = {
                name: parameter.detach().clone()
                for name, parameter in ssl.named_parameters()
            }
            trial_results = {}

            def evaluate_ssl_trial(alpha):
                # Reset before every trial, then move once along the original proposal
                # displacement. Fixed quadrature buffers are restored, never interpolated.
                ssl.load_state_dict(ssl_state_before)
                load_parameter_dict(
                    ssl,
                    interpolate_parameter_dict(
                        ssl_parameters_before,
                        ssl_parameters_proposed,
                        alpha,
                    ),
                )
                trial_online = ssl.encoder(probe_obs).float()
                trial_rotation, trial_bias = affine_frame_transport(
                    trial_online[frame_fit],
                    probe_online_before[frame_fit],
                    control_rotation,
                    control_bias,
                )
                trial_control = trial_online @ trial_rotation + trial_bias
                trial_actor_feat, trial_critic_feat = agent._trunks(
                    torch.cat([probe_obs, trial_control], dim=-1)
                )
                trial_policy = agent._actor_dist(trial_actor_feat)[0]
                policy_kl = kl_divergence(
                    old_policy, trial_policy
                ).sum(-1)[trust_eval].mean()
                emb_drift = (
                    (trial_control[trust_eval] - probe_control_before[trust_eval])
                    .pow(2)
                    .mean()
                    .sqrt()
                    / probe_control_before[trust_eval]
                    .pow(2)
                    .mean()
                    .sqrt()
                    .clamp_min(1e-6)
                )
                trial_successors = agent.critic_head(trial_critic_feat).view(
                    -1, len(GEOMETRIC_BETAS), sf_dim
                )
                sf_drift_by_head = (
                    (
                        (
                            trial_successors[trust_eval]
                            - old_successors[trust_eval]
                        )
                        / sf_loss_scale
                    )
                    .pow(2)
                ...
                # No auxiliary head may hide drift in the actor-facing final head.
                sf_drift = sf_drift_by_head.max()
                trial_values = (
                    trial_successors[:, -1] @ reward_task
                ) / (1.0 - args.gamma)
                value_drift = (
                    (
                        trial_values[trust_eval]
                        - old_probe_values[trust_eval]
                    )
                    .pow(2)
                    .mean()
                    .sqrt()
                    / old_probe_values[trust_eval].std().clamp_min(1.0)
                )
                trial_results[alpha] = (
                    trial_rotation,
                    trial_bias,
                ...
                return bool(
                    policy_kl <= args.control_encoder_kl
                    and emb_drift <= args.control_encoder_emb_drift
                    and sf_drift <= args.control_encoder_sf_drift
                    and value_drift <= args.control_encoder_value_drift
                )

            if args.ssl_control_mode == "line_search":
                accepted_alpha = select_line_search_alpha(
                    SSL_LINE_SEARCH_ALPHAS, evaluate_ssl_trial
                )
            else:
                evaluate_ssl_trial(1.0)
                accepted_alpha = 0.0

            proposal_result = trial_results[1.0]
            (
                _proposal_rotation,
                _proposal_bias,
                _proposal_control,
                ...
                proposal_emb_drift,
                proposal_sf_drift,
                proposal_value_drift,
            ) = proposal_result
            accepted = accepted_alpha > 0.0
            if accepted:
                (
                    candidate_rotation,
                    candidate_bias,
                    ...
                    encoder_sf_drift,
                    encoder_value_drift,
                ) = trial_results[accepted_alpha]
                control_rotation = candidate_rotation
                control_bias = candidate_bias
                if accepted_alpha < 1.0:
                    # A partial trial is a new point, not the optimizer's full proposal.
                    # Retain the interpolated parameters but restart from preproposal
                    # moments so momentum cannot immediately replay the rejected step.
                    ssl_optimizer.load_state_dict(ssl_optimizer_state_before)
            else:
                ssl.load_state_dict(ssl_state_before)
                ssl_optimizer.load_state_dict(ssl_optimizer_state_before)
                probe_control_after = probe_control_before
                zero = probe_control_before.new_zeros(())
                encoder_policy_kl = encoder_emb_drift = zero
                encoder_sf_drift = encoder_value_drift = zero
            ...
            # SIGReg samples random projections even though only pred_loss is logged.
            # Restore the CUDA RNG so this diagnostic cannot change future policy draws.
            diagnostic_rng_state = torch.cuda.get_rng_state(device)
            _, selected_ssl_pred_l, _, _ = ssl(
                seq_obs[: args.ssl_batch],
                seq_act[: args.ssl_batch],
                seq_cont[: args.ssl_batch],
                args.sigreg_weight,
            )
            torch.cuda.set_rng_state(diagnostic_rng_state, device)
            selected_ssl_pred_l = selected_ssl_pred_l.item()
        ...
        for horizon, skill, mse in zip(
            GEOMETRIC_EFFECTIVE_HORIZONS,
            preupdate_skill_by_head,
            preupdate_mse_by_head,
        ):
            horizon_tag = f"{horizon:.3f}".replace(".", "p")
            writer.add_scalar(
                f"geometric/preupdate_skill_h{horizon_tag}",
                skill.item(),
                global_step,
            )
            writer.add_scalar(
                f"geometric/preupdate_mse_h{horizon_tag}",
                mse.item(),
                global_step,
            )
        writer.add_scalar(
        ...
        # Fixed-horizon vector credit diagnostics.
        ...
        writer.add_scalar(
            "tddelta/learned_tail_to_exact_prefix_std",
            tail_contribution_ratio,
            global_step,
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "tddelta/exact_prefix_std",
            exact_prefix.std().item(),
            global_step,
        )
        ...
        writer.add_scalar(
            "ssl/selected_pred_loss", selected_ssl_pred_l, global_step
        )
        ...
        writer.add_scalar(
            "ssl/proposal_policy_kl", proposal_policy_kl.item(), global_step
        )
        writer.add_scalar(
            "ssl/proposal_emb_drift", proposal_emb_drift.item(), global_step
        )
        writer.add_scalar(
            "ssl/proposal_sf_drift", proposal_sf_drift.item(), global_step
        )
        writer.add_scalar(
            "ssl/proposal_value_drift", proposal_value_drift.item(), global_step
        )
        writer.add_scalar(
            "ssl/control_encoder_accepted_alpha", accepted_alpha, global_step
        )
        writer.add_scalar(
            "ssl/control_encoder_optimizer_advanced",
            float(accepted_alpha == 1.0),
            global_step,
        )
        writer.add_scalar("ssl/control_encoder_update_accepted", float(accepted), global_step)
        writer.add_scalar(
            "ssl/control_encoder_update_rate", accepted_alpha, global_step
        )
