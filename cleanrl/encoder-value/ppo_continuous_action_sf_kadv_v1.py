...
class Args:
    ...
    # ---- LEARNED ADVANTAGE KERNEL (kadv) -- this file's change ----------------
    kadv: bool = True                # replace GAE's fixed scalar exponential kernel with a
    #                                  LEARNED per-coordinate exponential-SUM kernel. The vector
    #                                  TD residual recursion is run at K different lambdas at
    #                                  once; a closed-form ridge then picks the readout across
    #                                  the whole (K x sf_dim) stack. Expanding the recursion,
    #                                    A_t = sum_l sum_i [ sum_k w_{k,i} (gamma*lam_k)^l ] Delta^i_{t+l}
    #                                  so the effective kernel at lag l on coordinate i is
    #                                  c_l^i = sum_k w_{k,i} (gamma*lam_k)^l -- a K-term
    #                                  exponential sum, which spans essentially any smooth
    #                                  decaying kernel INCLUDING non-monotone and sign-changing
    #                                  ones, and gives the embedding block a different kernel
    #                                  from the velocity block. GAE is the exact special case
    #                                  w = (one lambda) x (w_r direction).
    kadv_lambdas: Tuple[float, ...] = (0.0, 0.5, 0.9, 0.97, 1.0)  # spread WIDE on purpose: adjacent lambdas
    #                                  give near-collinear columns and the ridge has to spend its
    #                                  budget separating them instead of fitting. lam=0 is the
    #                                  pure one-step TD residual (low variance / max bootstrap
    #                                  bias); lam=1 is the undiscounted-trace MC end.
    kadv_ridge: float = 1e-2         # scale-aware ridge on a K*sf_dim = 310-unknown system.
    kadv_val_frac: float = 0.2       # held-out fraction, for the REPORTED ev_mc only.
    kadv_split_floor: float = 2.0    # split-half reliability has a NONZERO noise floor, in units
    #                                  of 1/sqrt(n_params): two INDEPENDENT ridge solutions are two
    #                                  random directions in R^d, and corr of their predictions has
    #                                  std ~ 1/sqrt(d) -- it is not concentrated at 0. Measured over
    #                                  24 trials on a pure-noise target: 0.236 @d=25, 0.124 @d=60,
    #                                  0.073 @d=310 (this config), 0.037 @d=620. Uncorrected, a
    #                                  readout of PURE NOISE earns E[clamp(r,0,1)] = 0.073/sqrt(2pi)
    #                                  ~ 2.9% of the blend. Subtracting 2 sigma costs ~0.11 against
    #                                  a signal that measured 0.54-0.70 on this same target, and
    #                                  drives the noise case to ~0.
    kadv_prior_gae: bool = True      # shrink toward the SCALAR GAE on real rewards, NOT toward
    #                                  w_r . E. The scalar GAE carries no accumulated reward-probe
    #                                  residual, so at shrink -> 0 this arm degrades to literally
    #                                  PPO's advantage -- a known-good estimator (the family
    #                                  reference runs reach 9953-10071 on it). That makes the
    #                                  learned kernel strictly additive risk: it can only lose
    #                                  through the reliability estimate itself being wrong.
...
if __name__ == "__main__":
    ...
    kadv_on = args.kadv
    klam = torch.tensor(list(args.kadv_lambdas), device=device, dtype=torch.float32)
    n_klam = klam.numel()
    kadv_shrink_ema = 0.0
    if kadv_on:
        assert args.norm_adv, "kadv blends two unit-scaled advantages; norm_adv must be on"
        assert (klam >= 0).all() and (klam <= 1).all(), "kadv_lambdas must lie in [0, 1]"
    w_r_solved = False
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # The K-lambda stack rides the SAME delta_sf and the SAME two masks. It is
            # ADVANTAGE-ONLY: sf_residual (and therefore sf_target, the critic's regression
            # target) stays on lam_vec exactly as in the parent, so this arm does not move
            # the critic's target and the change is attributable to the policy path alone.
            if kadv_on:
                E_stack = torch.zeros(
                    args.num_steps, args.num_envs, n_klam, sf_dim, device=device
                )
                last_stack = torch.zeros(args.num_envs, n_klam, sf_dim, device=device)
            for t in reversed(range(args.num_steps)):
                ...
                if kadv_on:
                    last_stack = delta_sf.unsqueeze(1) + (
                        args.gamma * klam.view(1, n_klam, 1) * cont.unsqueeze(1) * last_stack
                    )
                    E_stack[t] = last_stack
            ...
            # ---- THE LEARNED KERNEL READOUT -------------------------------------
            # Solved, not gradient-fit, for the reason established on the per-actuator head:
            # an Adam-fit readout does NOT settle at zero when there is nothing to learn, it
            # equilibrates in a noise ball whose radius is set by the LR. A closed-form ridge
            # has no such ball, so "no signal" reads as an honest ~0.
            #
            # Shrinkage is SPLIT-HALF RELIABILITY, not held-out EV. Two independent solves on
            # disjoint halves of the training rows, correlated on rows NEITHER saw: that is the
            # reliability ratio of the ESTIMATOR, and unlike EV it is not capped by the noise
            # floor of the single-sample MC target. Measured on this same target, held-out EV
            # sat at -0.087 all run while split-half read 0.54-0.70 -- EV would have reported
            # "no signal" for a signal that was there.
            adv_learned = torch.zeros_like(adv_vector)
            ka_split_r = ka_ev_mc = ka_corr_gae = ka_horizon = 0.0
            ka_mass = [0.0] * n_klam
            kadv_active = kadv_on and w_r_solved and n_mc >= 256
            if kadv_active:
                with torch.no_grad():
                    Xf = E_stack.reshape(-1, n_klam * sf_dim)
                    # Per-column standardization is load-bearing: the lam=1 columns carry ~7x
                    # the scale of the lam=0 columns (sqrt(1/(1-(gamma*lam)^2))), so an
                    # unstandardized ridge penalty would silently be ~50x tighter on one end
                    # of the stack than the other and the "learned" kernel would just be a
                    # readout of which column happened to be biggest.
                    X_std = Xf.std(0).clamp_min(1e-6)
                    Xf = Xf / X_std
                    ...
                    n_val = max(64, int(args.kadv_val_frac * mc_idx.numel()))
                    val_idx, tr_idx = mc_idx[perm[:n_val]], mc_idx[perm[n_val:]]
                    w_a = pgvec_solve(Xf[tr_idx], tgt[tr_idx], args.kadv_ridge)
                    h = tr_idx.numel() // 2
                    a1 = pgvec_solve(Xf[tr_idx[:h]], tgt[tr_idx[:h]], args.kadv_ridge)
                    a2 = pgvec_solve(Xf[tr_idx[h:]], tgt[tr_idx[h:]], args.kadv_ridge)
                    ka_split_r = corr_score(Xf[val_idx] @ a1, Xf[val_idx] @ a2)
                    ka_floor = args.kadv_split_floor / math.sqrt(n_klam * sf_dim)
                    ka_rel = max(0.0, (ka_split_r - ka_floor) / (1.0 - ka_floor))
                    kadv_shrink_ema = 0.9 * kadv_shrink_ema + 0.1 * min(ka_rel, 1.0)
                    ka_ev_mc = ev_score(Xf[val_idx] @ w_a, tgt[val_idx])
                    adv_learned = (Xf @ w_a).reshape(args.num_steps, args.num_envs)
                    # What kernel did it actually learn? |w| mass per lambda, and the effective
                    # look-back horizon of c_l = sum_k m_k (gamma*lam_k)^l. GAE(lam=0.95) has
                    # H = gamma*lam/(1-gamma*lam) = 15.8, so this number is directly readable
                    # against the estimator being replaced.
                    wa_blk = w_a.view(n_klam, sf_dim).abs().sum(-1)
                    m = wa_blk / wa_blk.sum().clamp_min(1e-12)
                    ka_mass = [float(x) for x in m]
                    lag = torch.arange(400, device=device, dtype=torch.float32)
                    c_l = (m.unsqueeze(-1) * (args.gamma * klam).unsqueeze(-1).clamp_min(0)
                           ** lag.unsqueeze(0)).sum(0)
                    ka_horizon = float((lag * c_l).sum() / c_l.sum().clamp_min(1e-12))
            ...
            if kadv_active:
                # Both terms are put on unit scale BEFORE mixing. norm_adv re-standardizes the
                # sum afterwards, but that happens after the mix, so without this the blend
                # weight would not be the mixing weight -- the learned term's raw scale (set by
                # an arbitrary ridge on standardized columns) would set it instead.
                ka_s = kadv_shrink_ema
                g_n = adv_scalar_gae / adv_scalar_gae.std().clamp_min(1e-6)
                l_n = adv_learned / adv_learned.std().clamp_min(1e-6)
                advantages = (1.0 - ka_s) * g_n + ka_s * l_n
                _c, _d = adv_learned.reshape(-1), adv_scalar_gae.reshape(-1)
                ka_corr_gae = corr_score(_c, _d)
            elif args.vector_adv and w_r_solved:
        ...
        writer.add_scalar("kadv/active", float(kadv_active), global_step)
        writer.add_scalar("kadv/split_r", ka_split_r, global_step)
        writer.add_scalar("kadv/split_floor", math.sqrt(1.0 / (n_klam * sf_dim)) * args.kadv_split_floor, global_step)
        writer.add_scalar("kadv/shrink", kadv_shrink_ema, global_step)
        writer.add_scalar("kadv/ev_mc", ka_ev_mc, global_step)
        writer.add_scalar("kadv/corr_gae", ka_corr_gae, global_step)
        writer.add_scalar("kadv/horizon", ka_horizon, global_step)
        for _k, _lm in enumerate(args.kadv_lambdas):
            writer.add_scalar(f"kadv/mass_lam{_lm}", ka_mass[_k], global_step)
