...
#
# The PPO clip becomes per-coordinate, with bounds set to the act_dim-th roots of v9's,
# so the joint ratio -- the thing v9 actually clipped -- lands in the SAME band. Six
# independent [0.8, 1.28] gates would instead admit 0.8^6 .. 1.28^6, a 3.4x looser trust
# region smuggled in alongside the credit change and invisible to approx_kl/clipfracs,
# which are still computed on the joint ratio. omega=0 isolates this clip as its own arm.
#
...
def perdim_policy_loss(advantage, shift, omega, ratio_dims, clip_coef, clip_coef_high):
    ...
    The per-coordinate clip bounds are the act_dim-th roots of v9's, so the product over
    coordinates -- the joint ratio v9 actually clipped -- lands in exactly the same
    [1-clip_coef, 1+clip_coef_high] band. Without the root, six independent [0.8, 1.28]
    gates admit a joint ratio of 0.8^6 to 1.28^6, a 3.4x looser trust region smuggled in
    alongside the credit change, and approx_kl/clipfracs (still computed on the joint
    ratio) would not show it.
    """
    adv_dims = advantage.unsqueeze(-1) + omega * shift
    n = ratio_dims.shape[-1]
    lo = (1.0 - clip_coef) ** (1.0 / n)
    hi = (1.0 + clip_coef_high) ** (1.0 / n)
    loss = torch.max(
        -adv_dims * ratio_dims, -adv_dims * torch.clamp(ratio_dims, lo, hi)
    ).sum(-1).mean()
    return loss, adv_dims
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                pg_loss, mb_adv_dims = perdim_policy_loss(
                    mb_advantages,
                    jac_shift[mb_inds],
                    jac_omega_eff,
                    ratio_d,
