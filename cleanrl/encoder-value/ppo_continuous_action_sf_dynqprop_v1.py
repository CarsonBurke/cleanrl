# DynQProp v1 -- reward-anchored successor-Q control variate.
# =====================================================================================
# HYPOTHESIS. The exact reward coordinate gives a strong V(s), but PPO still estimates
# the local action slope of Q from sampled returns. Two one-step models predict
#
#   qhat(s,a) = rhat(s,a) + gamma * p_cont(s,a) * psi_reward_old(shat_next),
#
# and differentiate it at the OLD Beta mean. Their mean slope g(s) defines the centered
# control variate c = g(s) . (a - mu_old). The baseline clipped PPO loss on normalized A
# is untouched. Outside it we add alpha*ratio*c/A_std and subtract
# alpha*g.detach().(mu_new-mu_old)/A_std. These un-clipped terms cancel in expectation for
# every policy theta, for any frozen g and alpha. Scalar reward GAE, joint PPO clipping,
# and the exact reward-coordinate critic remain unchanged.
#
# SAFETY. Dynamics and reward models are used one rollout late and never updated during
# PPO. Alpha starts at zero and is capped conservatively; it opens only when both models
# generalize on held-out delta/reward targets and their action slopes agree in direction
# and magnitude. We never normalize c independently or force it to unit variance. Standard
# MuJoCo v4 control costs are inserted analytically; a reward head learns the remaining
# action-dependent reward (and the full reward on unknown environments). This preserves
# the exact environment reward anchor: the model affects only actor-gradient variance.
#
# RISKS. One-step normalized-observation dynamics drift with the observation normalizer;
# local derivatives can agree while sharing model bias (the twins differ in initialization,
# not training data); and finite-rollout estimates leave
# sampling error around the analytic cancellation. Diagnostics expose held-out fit, twin slope agreement,
# predicted variance reduction, correction scale, and the old-policy centering/add-back
# invariants. Alpha=0 recovers the v13 actor exactly.
# =====================================================================================
# PPO + reward-anchored successor features and LeJEPA auxiliary prediction.
#
...
class Args:
    ...
    aux_sf_coef: float = 1.0         # aggregate auxiliary loss weight relative to the exact h0
    #                                  standardized value loss; normalized by
    #                                  1 + aux_sf_coef so the overall critic-loss scale stays fixed
    # --- DynQProp: one-rollout-lagged local successor-Q slope -----------------------
    qprop_alpha_max: float = 0.25    # conservative cap; covariance fit may choose less or zero
    qprop_dyn_r2_gate: float = 0.80
    qprop_dyn_r2_full: float = 0.95
    qprop_reward_r2_gate: float = 0.60
    qprop_reward_r2_full: float = 0.90
    qprop_qgrad_rel_gate: float = 0.50
    qprop_qgrad_rel_full: float = 0.80
    qprop_grad_chunk: int = 4096
    dyn_hidden: int = 256
    dyn_lr: float = 3e-4
    dyn_steps_target: int = 64
    dyn_batch: int = 1024
    dyn_grad_clip: float = 1.0
...
class ObsDynamicsReward(nn.Module):
    """Action-conditioned normalized-observation delta, reward residual, continuation."""
    ...
    def __init__(self, obs_dim, act_dim, hidden):
        ...
        # A cold model is identity dynamics, zero non-control reward, and continuation.
        for head in (self.delta_head, self.reward_head):
    ...
    def forward(self, obs, action):
        h = self.body(torch.cat([obs, action], dim=-1))
        return self.delta_head(h), self.reward_head(h)[..., 0], self.cont_head(h)[..., 0]


def known_ctrl_cost_weight(env_id):
    """Exact default-v4 MuJoCo control cost; unknown/custom environments use zero."""
    return {
        "HalfCheetah-v4": 0.1,
        "Hopper-v4": 1e-3,
        "Walker2d-v4": 1e-3,
...
def model_reward(reward_residual, action, ctrl_cost_weight):
    return reward_residual - ctrl_cost_weight * action.square().sum(-1)


def gate_ramp(value, low, high):
    if not np.isfinite(value):
        return 0.0
    return float(np.clip((value - low) / max(high - low, 1e-8), 0.0, 1.0))


def successor_q_slopes(agent, models, obs, psi_raw, gamma, ctrl_cost_weight, chunk):
    """Frozen twin d qhat(s,a)/d a at the old Beta mean; no parameter gradients."""
    means, slopes, qhats = [], [[], []], [[], []]
    for start in range(0, obs.shape[0], chunk):
        s = obs[start : start + chunk]
        with torch.no_grad():
            actor_feat, _ = agent._trunks(s)
            dist, _, _ = agent._actor_dist(actor_feat)
            span = agent.action_high - agent.action_low
            mu = agent.action_low + span * dist.mean
        means.append(mu)
        for model_idx, model in enumerate(models):
            with torch.enable_grad():
                a0 = mu.detach().clone().requires_grad_(True)
                delta, reward_residual, cont_logit = model(s, a0)
                next_value = psi_raw(agent.get_value(s + delta)[:, 0])[:, 0]
                qhat = (
                    model_reward(reward_residual, a0, ctrl_cost_weight)
                    + gamma * torch.sigmoid(cont_logit) * next_value
                )
                slope = torch.autograd.grad(qhat.sum(), a0, create_graph=False)[0]
            slopes[model_idx].append(slope.detach())
            qhats[model_idx].append(qhat.detach())
    mu_old = torch.cat(means)
    g1, g2 = (torch.cat(parts) for parts in slopes)
    q1, q2 = (torch.cat(parts) for parts in qhats)
    n1, n2 = g1.norm(dim=-1), g2.norm(dim=-1)
    cosine = F.cosine_similarity(g1, g2, dim=-1, eps=1e-8)
    magnitude_agreement = 2.0 * torch.minimum(n1, n2) / (n1 + n2).clamp_min(1e-8)
    reliability = (cosine.clamp_min(0.0) * magnitude_agreement).mean()
    diagnostics = {
        "cosine": cosine.mean().item(),
        "reliability": reliability.item(),
        "norm": (0.5 * (n1 + n2)).mean().item(),
        "twin_rel_gap": ((q1 - q2).square().mean().sqrt() / q1.square().mean().sqrt().clamp_min(1e-8)).item(),
        "finite": float(
            torch.isfinite(g1).all()
            and torch.isfinite(g2).all()
            and torch.isfinite(mu_old).all()
            and torch.isfinite(q1).all()
            and torch.isfinite(q2).all()
        ),
    }
    return mu_old, 0.5 * (g1 + g2), diagnostics
...
def phi_features(reward, emb, obs, action):
    """phi = [reward, e, s, a, a*a, 1].

    The leading observed-reward coordinate is the anchor: its discounted successor sum
    is the value target by construction. It avoids compounding even a small correlated
    residual from a learned reward probe across the full return horizon. All remaining
    coordinates are auxiliary predictive structure for the shared critic trunk.
    ...
    return torch.cat(
        [reward.unsqueeze(-1), emb, obs, action, action * action, ones], dim=-1
    )
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # REWARD-ANCHORED successor-feature critic. Coordinate 0 is discounted reward;
        # the remaining coordinates are auxiliary predictive occupancies.
        ...
        self.sf_dim = 1 + args.emb_dim + obs_dim + 2 * act_dim + 1
        ...
        _paired_v9_head = layer_init(
            nn.Linear(H, args.critic_mtp_horizon * paired_v9_sf_dim, bias=False), std=0.1
        )
        del _paired_v9_head
    ...
    def get_action_and_value(self, x, z=None):
        ...
        if self.actor_dist == "beta":
            action_mean = self.action_low + (self.action_high - self.action_low) * dist.mean
        else:
        ...
        return action, z, log_prob, entropy, value_sf, action_mean
...
if __name__ == "__main__":
    ...
    if args.aux_sf_coef < 0.0:
        raise ValueError(f"aux_sf_coef must be nonnegative, got {args.aux_sf_coef}")
    if args.normalize_reward or args.clip_reward:
        raise ValueError("DynQProp requires the exact raw environment reward; normalization/clipping must stay off")
    if args.actor_dist != "beta":
        raise ValueError("DynQProp v1 requires the Beta policy's exact analytic action mean")
    if not args.norm_adv or args.norm_adv_scope != "batch" or args.adv_transform != "v10":
        raise ValueError("DynQProp v1 requires v10 plus batch advantage normalization for one shared scale")
    if args.ret_percnorm or args.vector_adv or args.per_dim_lambda:
        raise ValueError("DynQProp v1 retains scalar GAE and has no second advantage rescaling")
    ...
    # Isolate auxiliary-model initialization from the paired policy RNG stream. The two
    # sequential initializations inside the fork are distinct from each other.
    with torch.random.fork_rng(devices=[]):
        dyn_models = nn.ModuleList(
            [ObsDynamicsReward(obs_dim, act_dim_sf, args.dyn_hidden) for _ in range(2)]
        )
    dyn_models = dyn_models.to(device)
    dyn_optimizer = optim.Adam(dyn_models.parameters(), lr=args.dyn_lr)
    dyn_rng = torch.Generator(device=device)
    dyn_rng.manual_seed(args.seed + 7919)
    ctrl_cost_weight = known_ctrl_cost_weight(args.env_id)
    dyn_r2_last = reward_r2_last = -1.0
    dyn_mse_last = reward_mse_last = float("nan")
    dyn_cont_acc_last = 0.0
    ...
    # Fixed exact readout of the leading discounted-reward coordinate. The zero-initialized
    # critic head still makes V == 0 on the first rollout, without a learned-probe warmup.
    ...
    for iteration in range(1, args.num_iterations + 1):
        # Qhat must use the raw-coordinate transform that produced this rollout's old V,
        # not the target statistics that will be updated before PPO below.
        qprop_sf_mean_old = sf_mean.detach().clone()
        qprop_sf_std_old = sf_std.detach().clone()
        if args.anneal_lr:
            ...
            dyn_optimizer.param_groups[0]["lr"] = frac * args.dyn_lr
        ...
        with torch.no_grad():
            ...
            phi = phi_features(rewards, emb_buf, obs, actions)  # (T, B, sf_dim)
            ...
            # Coordinate 0 is reward exactly, so with scalar lambda this vector residual
            # must equal ordinary scalar GAE. Keep it as an online identity diagnostic;
            # the default policy path deliberately uses the independently computed GAE.
            adv_vector = sf_residual[..., 0]
            ...
            anchor_return_maxerr = (sf_target[..., 0] - returns).abs().max()
            anchor_adv_maxerr = (adv_vector - adv_scalar_gae).abs().max()
            ...
            # Exact immediate-reward reconstruction by coordinate 0. Unlike v9's ridge
            # reward probe, this has no fitted-frame drift or temporally correlated residual.
            flat_phi = phi.reshape(-1, sf_dim)
            flat_rew = rewards.reshape(-1)
            reward_resid = flat_rew - flat_phi @ w_r
            exact_reward_maxerr = reward_resid.abs().max()
        ...
        if args.norm_adv and args.norm_adv_scope == "batch":
            advantage_scale = b_policy_adv.std() + 1e-8
            b_policy_adv_normed = (b_policy_adv - b_policy_adv.mean()) / advantage_scale
        ...
        # Frozen local Q slopes from models trained only through the PREVIOUS rollout.
        # The coefficient fit below is recorded for the NEXT rollout; fitting and applying
        # alpha on the same sampled actions/returns would make alpha data-dependent inside
        # the score estimator and invalidate the control-variate identity.
        def psi_old_raw(sf_standardized):
            return sf_standardized * qprop_sf_std_old + qprop_sf_mean_old

        b_mu_old, b_qgrad, qgrad_diag = successor_q_slopes(
            agent,
            dyn_models,
            b_obs,
            psi_old_raw,
            args.gamma,
            ctrl_cost_weight,
            args.qprop_grad_chunk,
        )
        if not qgrad_diag["finite"]:
            # A closed control path must be a literal finite fallback, not 0*NaN.
            b_qgrad = torch.zeros_like(b_qgrad)
            b_mu_old = torch.where(torch.isfinite(b_mu_old), b_mu_old, b_actions)
            qgrad_diag.update({"cosine": 0.0, "reliability": 0.0, "norm": 0.0})
            qprop_alpha_lag = 0.0
        b_control_raw = (b_qgrad * (b_actions - b_mu_old)).sum(-1)
        b_control_scaled = b_control_raw / advantage_scale
        if not torch.isfinite(b_control_scaled).all():
            b_qgrad = torch.zeros_like(b_qgrad)
            b_control_raw = torch.zeros_like(b_control_raw)
            b_control_scaled = torch.zeros_like(b_control_scaled)
            qprop_alpha_lag = 0.0
            qgrad_diag.update({"finite": 0.0, "cosine": 0.0, "reliability": 0.0, "norm": 0.0})
        a_centered = b_policy_adv_normed - b_policy_adv_normed.mean()
        c_centered = b_control_scaled - b_control_scaled.mean()
        c_var = c_centered.square().mean()
        ac_cov = (a_centered * c_centered).mean()
        # This was fitted and gated on the preceding rollout, never this one.
        qprop_alpha = qprop_alpha_lag if np.isfinite(qprop_alpha_lag) else 0.0
        base_adv_var = b_policy_adv_normed.var().clamp_min(1e-12)
        qprop_var_red = float(
            1.0
            - (b_policy_adv_normed - qprop_alpha * b_control_scaled).var()
            / base_adv_var
        )
        qprop_corr = float(
            ac_cov
            / (a_centered.square().mean().sqrt() * c_var.sqrt()).clamp_min(1e-12)
        )
        qprop_center_maxerr = float((b_qgrad * (b_mu_old - b_mu_old)).sum(-1).abs().max())
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_sf, mb_mu_new = agent.get_action_and_value(
                ...
                base_pg_loss = torch.max(pg_loss1, pg_loss2).mean()
                # Keep the proven clipped PPO objective bit-for-bit. The control pair is
                # OUTSIDE clipping and uses the exact importance ratio:
                #   E_old[ratio*g.(a-mu_old)] = g.(mu_new-mu_old).
                # Hence these two terms cancel for every theta in expectation, not merely
                # at theta_old. g, mu_old, alpha, and the shared rollout scale are frozen.
                if qprop_alpha > 0.0:
                    cv_score_loss = qprop_alpha * (
                        ratio * b_control_scaled[mb_inds]
                    ).mean()
                    cv_addback_loss = -qprop_alpha * (
                        b_qgrad[mb_inds]
                        * (mb_mu_new - b_mu_old[mb_inds])
                        / advantage_scale
                    ).sum(-1).mean()
                else:
                    # Do not evaluate the model-derived tensors: 0 * NaN is still NaN.
                    cv_score_loss = base_pg_loss.new_zeros(())
                    cv_addback_loss = base_pg_loss.new_zeros(())
                pg_loss = base_pg_loss + cv_score_loss + cv_addback_loss

                # Anchored vector loss. The h0 reward coordinate actually consumed as V
                # gets half the critic budget at aux_sf_coef=1. Future-value MTP and all
                # embedding/state/action occupancies split the auxiliary half. Target
                # standardization makes this weighting about task relevance, not raw units.
                sf_tgt = b_sf_target[mb_inds]
                sf_err = value_sf - sf_tgt                                    # (B, mtp, sf_dim)
                value_mask = b_target_mask[mb_inds].to(sf_err.dtype)          # (B, mtp)
                direct_value_loss = sf_err[:, 0, 0].pow(2).mean()
                occupancy_loss = (
                    sf_err[..., 1:].pow(2).mean(-1) * value_mask
                ).sum() / value_mask.sum().clamp_min(1.0)
                future_value_mask = value_mask[:, 1:]
                future_value_denom = future_value_mask.sum()
                future_value_loss = (
                    sf_err[:, 1:, 0].pow(2) * future_value_mask
                ).sum() / future_value_denom.clamp_min(1.0)
                has_future_value = (future_value_denom > 0).to(sf_err.dtype)
                aux_loss = (
                    occupancy_loss + has_future_value * future_value_loss
                ) / (1.0 + has_future_value)
                # v9 sums rather than averages valid MTP horizons. Preserve that aggregate
                # scale while changing only the gradient allocation among target groups.
                mtp_scale = value_mask.sum(-1).mean().detach()
                v_loss = (
                    mtp_scale
                ...
                reward_per_h_mse = (
                    sf_err.detach()[..., 0].pow(2) * value_mask
                ).sum(0) / value_mask.sum(0).clamp_min(1)
                aux_per_h_mse = (
                    sf_err.detach()[..., 1:].pow(2).mean(-1) * value_mask
                ).sum(0) / value_mask.sum(0).clamp_min(1)
        ...
        # ---- one-step twin dynamics/reward fit --------------------------------------
        # Runs after PPO, so the models and every slope used above were frozen and trained
        # through at most the preceding rollout. Boundary final observations are valid
        # physics; transition_valids distinguishes them from unavailable successors.
        with torch.no_grad():
            dyn_s_all = obs.reshape(-1, obs_dim)
            dyn_a_all = actions.reshape(-1, act_dim_sf)
            dyn_delta_all = next_obses.reshape(-1, obs_dim) - dyn_s_all
            dyn_term_all = transition_terminations.reshape(-1)
            dyn_reward_all = rewards.reshape(-1) + ctrl_cost_weight * dyn_a_all.square().sum(-1)
            dyn_env_all = torch.arange(args.num_envs, device=device).repeat(args.num_steps)
            dyn_keep = transition_valids.reshape(-1) > 0.5
            dyn_s_all = dyn_s_all[dyn_keep]
            dyn_a_all = dyn_a_all[dyn_keep]
            dyn_delta_all = dyn_delta_all[dyn_keep]
            dyn_term_all = dyn_term_all[dyn_keep]
            dyn_reward_all = dyn_reward_all[dyn_keep]
            dyn_env_all = dyn_env_all[dyn_keep]
        n_all = dyn_s_all.shape[0]
        heldout_envs = torch.randperm(
            args.num_envs, device=device, generator=dyn_rng
        )[: max(1, args.num_envs // 8)]
        val_mask = (dyn_env_all[:, None] == heldout_envs[None, :]).any(-1)
        val_idx = val_mask.nonzero(as_tuple=False)[:, 0]
        train_idx = (~val_mask).nonzero(as_tuple=False)[:, 0]
        if train_idx.numel() >= args.dyn_batch and val_idx.numel() > 1:
            val_s, val_a = dyn_s_all[val_idx], dyn_a_all[val_idx]
            val_delta, val_term = dyn_delta_all[val_idx], dyn_term_all[val_idx]
            val_reward = dyn_reward_all[val_idx]
            train_s, train_a = dyn_s_all[train_idx], dyn_a_all[train_idx]
            train_delta, train_term = dyn_delta_all[train_idx], dyn_term_all[train_idx]
            train_reward = dyn_reward_all[train_idx]
            reward_scale = train_reward.std().clamp_min(0.1)
            n_train = train_s.shape[0]
            steps_per_epoch = max(1, n_train // args.dyn_batch)
            dyn_epochs = max(1, round(args.dyn_steps_target / steps_per_epoch))
            for model in dyn_models:
                for _ in range(dyn_epochs):
                    perm = torch.randperm(n_train, device=device, generator=dyn_rng)
                    for start in range(0, n_train - args.dyn_batch + 1, args.dyn_batch):
                        idx = perm[start : start + args.dyn_batch]
                        pred_delta, pred_reward, pred_cont = model(train_s[idx], train_a[idx])
                        dyn_loss = (
                            F.mse_loss(pred_delta, train_delta[idx])
                            + F.mse_loss(pred_reward / reward_scale, train_reward[idx] / reward_scale)
                            + F.binary_cross_entropy_with_logits(
                                pred_cont, 1.0 - train_term[idx]
                            )
                        )
                        dyn_optimizer.zero_grad(set_to_none=True)
                        dyn_loss.backward()
                        nn.utils.clip_grad_norm_(model.parameters(), args.dyn_grad_clip)
                        dyn_optimizer.step()
            with torch.no_grad():
                dyn_r2s, reward_r2s, dyn_mses, reward_mses, cont_accs = [], [], [], [], []
                for model in dyn_models:
                    pred_delta, pred_reward, pred_cont = model(val_s, val_a)
                    delta_resid = pred_delta - val_delta
                    reward_resid = pred_reward - val_reward
                    dyn_r2s.append(
                        (1.0 - delta_resid.square().mean(0) / val_delta.var(0).clamp_min(1e-8))
                        .clamp(-1.0, 1.0)
                        .mean()
                    )
                    reward_r2s.append(
                        1.0 - reward_resid.square().mean() / val_reward.var().clamp_min(1e-8)
                    )
                    dyn_mses.append(delta_resid.square().mean())
                    reward_mses.append(reward_resid.square().mean())
                    cont_accs.append(
                        ((pred_cont > 0).float() == (1.0 - val_term)).float().mean()
                    )
                dyn_r2_last = float(torch.stack(dyn_r2s).mean())
                reward_r2_last = float(torch.stack(reward_r2s).mean().clamp(-1.0, 1.0))
                dyn_mse_last = float(torch.stack(dyn_mses).mean())
                reward_mse_last = float(torch.stack(reward_mses).mean())
                dyn_cont_acc_last = float(torch.stack(cont_accs).mean())

        # Fit the coefficient for iteration i+1 with the SAME post-training model M_i,
        # critic, and policy that will generate its slopes. This second slope pass is
        # deliberate: reusing pre-PPO M_{i-1} slopes would silently mismatch alpha to the
        # model it scales. The fit still uses iteration-i data and is therefore lagged.
        fit_mu, fit_g, qgrad_fit_diag = successor_q_slopes(
            agent,
            dyn_models,
            b_obs,
            psi_raw,
        ...
        if qgrad_fit_diag["finite"]:
            fit_control_scaled = (fit_g * (b_actions - fit_mu)).sum(-1) / advantage_scale
            fit_c_centered = fit_control_scaled - fit_control_scaled.mean()
            fit_c_var = fit_c_centered.square().mean()
            fit_cov = (a_centered * fit_c_centered).mean()
            alpha_fit_tensor = (
                fit_cov / fit_c_var.clamp_min(1e-12)
            ).clamp(0.0, args.qprop_alpha_max)
            alpha_fit = float(alpha_fit_tensor) if torch.isfinite(alpha_fit_tensor) else 0.0
            fit_gate = (
                gate_ramp(dyn_r2_last, args.qprop_dyn_r2_gate, args.qprop_dyn_r2_full)
                * gate_ramp(
                    reward_r2_last, args.qprop_reward_r2_gate, args.qprop_reward_r2_full
                )
                * gate_ramp(
                    qgrad_fit_diag["reliability"],
                    args.qprop_qgrad_rel_gate,
                    args.qprop_qgrad_rel_full,
                )
            )
            qprop_alpha_next = alpha_fit * fit_gate
            qprop_var_red_next_fit = float(
                1.0
                - (b_policy_adv_normed - qprop_alpha_next * fit_control_scaled).var()
                / base_adv_var
            )
            if not np.isfinite(qprop_var_red_next_fit) or qprop_var_red_next_fit <= 0.0:
                qprop_alpha_next = fit_gate = alpha_fit = qprop_var_red_next_fit = 0.0
        else:
            alpha_fit = fit_gate = qprop_alpha_next = qprop_var_red_next_fit = 0.0
        qprop_alpha_lag = qprop_alpha_next
        ...
        writer.add_scalar("losses/direct_value_loss", direct_value_loss.item(), global_step)
        writer.add_scalar("losses/aux_sf_loss", aux_loss.item(), global_step)
        writer.add_scalar("losses/future_value_mtp_loss", future_value_loss.item(), global_step)
        writer.add_scalar("losses/occupancy_loss", occupancy_loss.item(), global_step)
        writer.add_scalar("losses/policy_loss", pg_loss.item(), global_step)
        # ---- DynQProp model, gate, variance, and exact-pair diagnostics ------------
        writer.add_scalar("qprop/model_heldout_delta_r2", dyn_r2_last, global_step)
        writer.add_scalar("qprop/model_heldout_reward_r2", reward_r2_last, global_step)
        writer.add_scalar("qprop/model_heldout_delta_mse", dyn_mse_last, global_step)
        writer.add_scalar("qprop/model_heldout_reward_mse", reward_mse_last, global_step)
        writer.add_scalar("qprop/model_continuation_accuracy", dyn_cont_acc_last, global_step)
        writer.add_scalar("qprop/alpha_applied", qprop_alpha, global_step)
        writer.add_scalar("qprop/alpha_fit_next", alpha_fit, global_step)
        writer.add_scalar("qprop/alpha_gate_next", fit_gate, global_step)
        writer.add_scalar("qprop/alpha_next", qprop_alpha_next, global_step)
        writer.add_scalar("qprop/control_adv_corr", qprop_corr, global_step)
        writer.add_scalar("qprop/predicted_var_reduction_applied", qprop_var_red, global_step)
        writer.add_scalar("qprop/predicted_var_reduction_next_fit", qprop_var_red_next_fit, global_step)
        writer.add_scalar("qprop/control_scaled_std", b_control_scaled.std().item(), global_step)
        writer.add_scalar("qprop/qgrad_norm_applied", qgrad_diag["norm"], global_step)
        writer.add_scalar("qprop/qgrad_twin_cosine_applied", qgrad_diag["cosine"], global_step)
        writer.add_scalar("qprop/qgrad_reliability_applied", qgrad_diag["reliability"], global_step)
        writer.add_scalar("qprop/qhat_twin_relative_gap_applied", qgrad_diag["twin_rel_gap"], global_step)
        writer.add_scalar("qprop/qgrad_reliability_next", qgrad_fit_diag["reliability"], global_step)
        writer.add_scalar("qprop/cv_score_loss", cv_score_loss.item(), global_step)
        writer.add_scalar("qprop/cv_addback_loss", cv_addback_loss.item(), global_step)
        writer.add_scalar(
            "qprop/correction_gross_to_ppo",
            (cv_score_loss.abs() + cv_addback_loss.abs()).item()
            / max(abs(base_pg_loss.item()), 1e-8),
            global_step,
        )
        writer.add_scalar("qprop/center_at_old_mean_maxerr", qprop_center_maxerr, global_step)
        writer.add_scalar(
            "qprop/old_policy_sample_pair_error",
            qprop_alpha * b_control_scaled.mean().abs().item(),
            global_step,
        )
        writer.add_scalar("qprop/known_ctrl_cost_weight", ctrl_cost_weight, global_step)
        ...
        # ---- reward-anchor and successor-feature diagnostics --------------------
        writer.add_scalar("sf/exact_reward_maxerr", exact_reward_maxerr.item(), global_step)
        writer.add_scalar("sf/anchor_return_maxerr", anchor_return_maxerr.item(), global_step)
        writer.add_scalar("sf/anchor_adv_maxerr", anchor_adv_maxerr.item(), global_step)
        for h in range(args.critic_mtp_horizon):
            writer.add_scalar(f"sf/psi_mse_h{h}", sf_per_h_mse[h].item(), global_step)
            writer.add_scalar(
                f"sf/reward_coordinate_mse_h{h}", reward_per_h_mse[h].item(), global_step
            )
            writer.add_scalar(f"sf/aux_mse_h{h}", aux_per_h_mse[h].item(), global_step)
        # ||Lambda|| per block against the tau ~= 20 prediction for a whitened,
        # zero-mean e block under gamma=0.99, lambda=0.95.
        emb_start = 1
        obs_start = emb_start + args.emb_dim
        act_start = obs_start + obs_dim
        writer.add_scalar("sf/lambda_std_reward", sf_std[0].item(), global_step)
        writer.add_scalar(
            "sf/lambda_std_emb", sf_std[emb_start:obs_start].mean().item(), global_step
        )
        ...
        writer.add_scalar("sf/lambda_std_obs",
                          sf_std[obs_start:act_start].mean().item(), global_step)
        writer.add_scalar("sf/lambda_std_act",
                          sf_std[act_start:-1].mean().item(), global_step)
        writer.add_scalar(
            "sf/lambda_absmean_emb",
            sf_mean[emb_start:obs_start].abs().mean().item(),
            global_step,
        )
        ...
        writer.add_scalar("advB/lam_reward", lam_vec[0].item(), global_step)
        writer.add_scalar(
            "advB/lam_emb", lam_vec[emb_start:obs_start].mean().item(), global_step
        )
        writer.add_scalar("advB/lam_obs",
                          lam_vec[obs_start:act_start].mean().item(), global_step)
        writer.add_scalar("advB/lam_act",
                          lam_vec[act_start:-1].mean().item(), global_step)
        ...
        writer.add_scalar("advB/tau_reward", tau_vec[0].item(), global_step)
        writer.add_scalar(
            "advB/tau_emb", tau_vec[emb_start:obs_start].mean().item(), global_step
        )
        writer.add_scalar("advB/tau_obs",
                          tau_vec[obs_start:act_start].mean().item(), global_step)
        writer.add_scalar("advB/tau_act",
                          tau_vec[act_start:-1].mean().item(), global_step)
