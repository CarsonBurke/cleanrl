...
#
# THE VECTOR-PATHWISE CHANNEL. Use the 62-dim successor-feature advantage AS A VECTOR,
# by routing it through the one gradient channel that has no scalar bottleneck.
#
# WHY EVERY PREVIOUS ATTEMPT COLLAPSED TO A SCALAR.
#   In a score-function estimator the quantity that finally multiplies grad log pi must be
#   a scalar -- that is the chain rule on a scalar loss, not a design choice. v23 tried to
#   escape it by replacing w_r with a LEARNED preference direction, but that still ends in
#   a scalar, and a worse one than the closed-form ridge probe: it paid for a vector critic
#   and threw the vector away at the last step. v32-v34 computed the exact action Jacobian
#   and then spent it re-weighting a score-function term, i.e. put it straight back through
#   the same bottleneck. v34's channel A additionally gated itself on attrib_corr, which
#   decays as entropy falls, so at 2M steps it multiplied by ~0 and the run WAS v9.
#
# THE CHANNEL THAT IS NOT BOTTLENECKED.
#   phi = [e(s), s, a, a*a, 1] is quadratic in the action, so Psi(s,a) = phi(s,a) +
#   gamma E[psi(s')|s,a] has an action Jacobian J = dPsi/da in R^(62 x 6). Contracting a
#   62-dim OUTCOME DIRECTION u with J gives a genuine 6-vector J^T u that depends on all 62
#   coordinates and never passes through a single scalar. This is the pathwise/DPG channel,
#   and it is orthogonal to (not a variant of) the score function.
#
# WHAT v9 THROWS AWAY, AND WHAT THIS PUTS BACK.
#   v9 computes the full vector GAE sf_residual (62-dim, per-component lambda) and then does
#   adv_vector = sf_residual @ w_r, destroying 61 of 62 dimensions every single iteration.
#   This channel recovers the 49 STATE coordinates of that vector; the a/a*a/const blocks are
#   deliberately excluded (see below), so it does not recover all 61.
#   Here that same vector becomes the outcome direction u, per sample -- not a fixed learned
#   direction, not a scalar.
#
# THE BUG THIS DESIGN EXISTS TO AVOID (it is not obvious and it is fatal).
#   sf_residual's ACTION blocks contain (a - mu) almost directly, and d phi_a/da = I at the
#   same step. So contracting the action blocks makes the update push mu toward whatever it
#   just sampled: a reward-free positive-feedback loop on the policy's own exploration
#   noise, i.e. variance amplification dressed up as a gradient. The STATE blocks [e, s]
#   have identically zero same-step action derivative, so for those the only route from
#   outcome to action is gamma dE[psi(s')]/da -- the learned part. u is therefore restricted
#   to the state blocks, and the action blocks are zeroed explicitly.
#
#   THAT ZEROING IS NOT SUFFICIENT ON ITS OWN, which is the subtlest thing in this file.
#   sf_residual's STATE coordinates contain gamma*psi(s_{t+1}), and s_{t+1} depends on a_t,
#   so the identical feedback loop re-enters through the successor term. Substituting the
#   head's own model gives E[g] contains gamma^2 J^T diag(asf_std/res_std) J dev, a PSD form,
#   so E[g.dev] >= 0 with no reward involved. Measured undebiased self-alignment
#   cos(g, dev) runs +0.55 to +0.92 on synthetic data with realistic structure;
#   vector_pathwise_debias drives it to within sampling noise of zero. A shuffle control
#   CANNOT catch this -- it destroys the self-alignment along with the signal -- so this had
#   to be fixed, not priced.
#
# MODES.
#   vecpath_mode="perp"  u has BOTH reward directions removed (see vector_pathwise_reward_
#                        basis), so the term has zero first-order effect on predicted return
#                        AND is the true complement of what adv_vector discards. Retained
#                        space is [e, s] = 49 coords minus 2 reward directions = 47, NOT 61:
#                        the a, a*a and constant blocks are excluded, not merely projected.
#                        Its whole claim is directed exploration in outcome space.
#   vecpath_mode="full"  u keeps the reward component, so the term also carries deterministic
#                        return ascent (a DPG term computed from psi rather than a Q net).
#
# FORM OF THE UPDATE. A regression onto a precomputed, trust-region-capped target
#   mu_target = clamp(mu + vecpath_step * sd(a) * ghat), NOT a linear -coef*(mu.g) term and
#   NOT backprop through the head. Linear terms push forever across 10 PPO epochs; the
#   regression is self-cancelling once mu reaches the target. Backprop through the head
#   would chase its extrapolation off the on-policy data manifold, where it was never fit.
#
# CONTROLS (vecpath_control). The regression carries an anchor toward the old mean even when
#   ghat = 0, so "coef > 0 helped" is NOT evidence the vector helped.
#   "anchor"   ghat := 0. Prices the proximal anchor alone.
#   "shuffle"  ghat permuted across the batch. Same marginal direction distribution, zero
#              per-sample alignment. THIS is the control that prices the vector.
#
# FALSIFIER. vecpath/jac_corr: correlation between the Jacobian's first-order predicted
#   outcome deviation and the realized one. If it sits at ~0 the Jacobian is noise and every
#   number this channel produces is noise -- the same failure mode that quietly disabled
#   v34, made loud instead of silent. vecpath/adv_align reports corr(ghat.dev, scalar adv);
#   under mode="perp" it should sit near zero by construction.
#
# vecpath_coef = 0 reproduces v9's UPDATE RULE (jac_perdim_omega and jac_improve_beta both
# default to 0 here), so it is a parity arm -- but not a bit-identical one: the action-
# successor head still trains, and its randperm consumes the same RNG stream the policy
# samples from, so the trajectories diverge. Same caveat, smaller, between the vecpath
# control arms: "shuffle" draws a randperm that "off" does not.
#
import os
import random
import time
...
class Args:
    ...
    # ---- v32: successor-feature ACTION JACOBIAN -----------------------------------
    jac_perdim_omega: float = 0.0   # strength of the zero-sum per-dimension credit
    #                                  redistribution, in units of the NORMALIZED advantage's
    #                                  std, BEFORE the correlation shrinkage below. omega=0
    #                                  leaves the per-coordinate trust region as the only change.
    jac_shift_clamp: float = 3.0     # winsorize the redistribution at this many std
    jac_clip_match: str = "gate"     # "gate" = v9's EXACT trust region (joint ratio, joint
    #                                  bounds) with per-dimension credit inside it. The
    #                                  per-coordinate-bound modes are kept only as the
    #                                  reproductions of two measured failures: "linear" is
    #                                  v32's (far too tight, actor frozen), "sqrt" is v33's
    #                                  (far too loose, joint clipfrac 0.17 vs v9's 0.023).
    jac_use_learned: bool = True     # include the LEARNED half of the action Jacobian
    jac_improve_beta: float = 0.0    # weight of the outcome-space improvement regression (channel B)
    # ---- v35: the vector-pathwise channel -------------------------------------------
    vecpath_coef: float = 0.5        # weight of the mu-regression onto the vector-pathwise
    #                                  target. 0 => v9 exactly (parity control arm).
    vecpath_step: float = 0.25       # target displacement, in units of the batch action std
    vecpath_mode: str = "perp"       # "perp" (strip both reward dirs) | "full" (keep them)
    vecpath_control: str = "off"     # "off" | "anchor" (ghat:=0) | "shuffle" (break alignment)
    vecpath_corr_ref: float = 0.2    # jac_corr at which the step reaches its full size
    jac_trust_kappa: float = 0.5     # improvement step cap, in units of the batch action std
    jac_ridge: float = 1e-2          # RELATIVE ridge on the outcome metric J_emb^T J_emb
    jac_chunk: int = 8192            # rows per Jacobian chunk (caps the (n, sf_dim, act_dim) temporary)
    asf_lr: float = 3e-4             # action-successor head LR (annealed with the PPO LR)
    asf_epochs: int = 4
    asf_batch: int = 4096
    asf_grad_clip: float = 1.0
...
def attribution_shift(attrib, calibration, clamp_sd):
    """Per-action-dimension credit shift: zero-sum across dimensions, shrunk by evidence.

    `attrib` is the exact action-dependent part of the advantage, decomposed by action
    coordinate. Subtracting its cross-dimension mean makes the shift ZERO-SUM, so adding
    it to a per-dimension copy of the scalar advantage leaves mean_j A_j == A exactly:
    the aggregate credit is v9's and only its attribution across coordinates moves.

    `calibration` is the shrinkage that makes this self-limiting. Z-scoring alone would
    inject a fixed-magnitude shift no matter how uninformative `attrib` is -- a useless
    Jacobian would still get full strength. Scaling by the measured correlation between
    the attribution and the realized advantage is exactly the shrinkage a least-squares
    predictor applies: no predictive content, no injection.
    """
    shift = attrib - attrib.mean(-1, keepdim=True)
    shift = shift / (shift.std() + 1e-8)
    # Winsorize, then RE-CENTRE: clamping a single wild coordinate breaks the zero-sum
    # property outright, and zero-sum is the entire blast-radius argument -- without it
    # the redistribution starts moving the aggregate credit, not just its attribution.
    # Re-centring can push a clamped coordinate past clamp_sd, but only by the row mean
    # of already-clamped values, and the row summed to zero before clamping.
    shift = shift.clamp(-clamp_sd, clamp_sd)
    shift = shift - shift.mean(-1, keepdim=True)
    return calibration * shift


def perdim_policy_loss(
    advantage, shift, omega, ratio_dims, ratio, clip_coef, clip_coef_high, clip_match
):
    """PPO surrogate with per-action-dimension credit inside v9's EXACT trust region.

    Per-dimension credit only reaches the policy through per-dimension ratios, since
    d ratio_j / d theta = ratio_j * grad log pi_j. Broadcasting the JOINT ratio instead
    collapses the gradient to (sum_j A_j) * r * sum_k grad log pi_k, and because the shift
    is zero-sum the redistribution cancels out of it entirely. So the credit must be
    per-coordinate -- but the TRUST REGION does not have to be, and two runs proved it
    must not be:

      "linear" (v32) -- per-coordinate bounds at the act_dim-th root, chosen so the
        PRODUCT of clamped ratios lands in v9's band. Bounding a product is not bounding
        each factor: sd(log r_j) = sd(log r)/sqrt(n), so the bound sat on the noise floor
        of the quantity it clipped. Actor entropy stayed at its initialization value for
        900k steps.
      "sqrt" (v33) -- bounds matched on clip FREQUENCY instead. The actor moved, but the
        joint ratio was no longer bounded at all: joint clipfrac 0.17 against v9's 0.023,
        approx_kl 0.011 against 0.004, and behind v9 at every matched step.

    "gate" keeps v9's clip EXACTLY: the same joint ratio, the same bounds, firing on the
    same samples. Where it fires, the per-dimension ratios are detached -- which is what
    PPO's clipped branch is, a term constant in theta, written per coordinate. At
    ratio_dims == 1 and omega == 0 the gradient is v9's identically.

    The returned loss VALUE differs from PPO's on clipped samples (PPO substitutes the
    clamped constant; this keeps the realized one). Only the gradient is load-bearing.
    """
    adv_dims = advantage.unsqueeze(-1) + omega * shift
    n = ratio_dims.shape[-1]
    if clip_match == "gate":
        clipped = ((advantage > 0) & (ratio > 1.0 + clip_coef_high)) | (
            (advantage < 0) & (ratio < 1.0 - clip_coef)
        )
        r = torch.where(clipped.unsqueeze(-1), ratio_dims.detach(), ratio_dims)
        return (-adv_dims * r).sum(-1).mean(), adv_dims, clipped.float().mean()
    root = {"sqrt": math.sqrt(n), "linear": float(n), "joint": 1.0}[clip_match]
    lo = (1.0 - clip_coef) ** (1.0 / root)
    hi = (1.0 + clip_coef_high) ** (1.0 / root)
    loss = torch.max(
        -adv_dims * ratio_dims, -adv_dims * torch.clamp(ratio_dims, lo, hi)
    ).sum(-1).mean()
    return loss, adv_dims, ((ratio_dims < lo) | (ratio_dims > hi)).float().mean()


def vector_pathwise_outcome_direction(sf_residual, state_end):
    """The 62-dim vector advantage, whitened, restricted to the state blocks.

    Returns (u, res_std). u is NOT yet usable as an outcome direction: it still carries the
    sampled action's own mechanical effect, which vector_pathwise_debias removes.

    METRIC. u is the vector GAE whitened by its own per-coordinate batch std, and it is
    contracted with the STANDARDIZED Jacobian d psi_std / da. The effective metric on
    outcome space is diag(1 / (std_k * asf_std_k)). That is a choice, not a derivation --
    but it is invariant to rescaling any phi coordinate, which the obvious alternative
    (raw phi units) is not: phi mixes a whitened latent, raw observations, actions and
    actions squared, so an unwhitened contraction is a vote by whichever block carries the
    largest units.

    The zeroing covers phi's `a`, `a*a` AND constant blocks, so the retained outcome space
    is [e, s], 49 coordinates for HalfCheetah, not 61.
    """
    res_std = sf_residual.std(0, keepdim=True)
    # RELATIVE floor, not an absolute epsilon. vector_pathwise_debias divides by res_std, so
    # a near-dead coordinate (a collapsed embedding dimension, say) would otherwise turn the
    # head's noise on that coordinate into an enormous spurious direction. An absolute 1e-6
    # does not bound the ratio; a floor tied to the state block's own scale does.
    res_std = res_std.clamp_min(1e-3 * res_std[:, :state_end].mean().clamp_min(1e-12))
    # The division allocates, so the in-place zeroing cannot alias sf_residual.
    u = sf_residual / res_std
    u[:, state_end:] = 0.0
    return u, res_std.squeeze(0)


def vector_pathwise_debias(u, jac_std, dev, asf_std, res_std, boot, gamma, state_end):
    """Remove the sampled action's OWN effect from the outcome direction.

    THIS IS THE CORRECTION THAT MAKES THE CHANNEL A GRADIENT RATHER THAN SELF-IMITATION.
    u is built from sf_residual, whose state coordinates contain gamma*boot*psi(s_{t+1}),
    and s_{t+1} depends on a_t. Substituting the head's own model
    E[psi_next_std | s,a] = head(feat, mu) + J.dev gives

        E[g] contains gamma^2 * J^T diag(asf_std/res_std) J * dev

    and that matrix is PSD, so E[g . dev] >= 0 REGARDLESS OF REWARD: the update would push
    mu toward whatever action it just sampled. That is variance amplification wearing a
    gradient's clothes, and it is invisible to a shuffle control, which destroys the
    self-alignment along with everything else.

    Subtracting gamma*boot*asf_std*(J.dev)/res_std re-centres the residual on the head's
    prediction AT THE MEAN ACTION instead of at the state, which is the correct baseline
    for a per-sample outcome direction.

    HONEST LIMIT. sf_residual is a multi-step GAE, so it also contains the effect of
    a_{t+1}, a_{t+2}, ... on their own successor states. Those are independent of a_t only
    to first order (they share the policy), so this removes the dominant one-step term, not
    every trace of self-alignment. vecpath/self_align measures what survives.
    """
    contamination = gamma * boot * (asf_std * torch.einsum("nkj,nj->nk", jac_std, dev)) / res_std
    out = u - contamination
    out[:, state_end:] = 0.0
    return out


def vector_pathwise_reward_basis(w_r, asf_std, res_std, state_end):
    """Orthonormal basis of the reward subspace that mode="perp" strips.

    TWO directions, not one, because two different statements both need to hold:
      b1 = w_r * asf_std   the functional being ascended is f(a) = <u, psi_std(s,a)>, and
                           predicted return is <w_r * asf_std, psi_std>. Killing this
                           component is what makes f carry no first-order return.
      b2 = w_r * res_std   in u-coordinates v9's scalar advantage is adv = <u, w_r*res_std>.
                           Killing this is what makes the arm the complement of what
                           adv_vector = sf_residual @ w_r actually discards.
    They are NOT parallel: res_std_k/asf_std_k = sqrt(1 - EV_k) and the critic's per-
    coordinate explained variance is far from uniform across [e, s]. Projecting out only b1
    leaves most of the advantage's variance in u, which would make a nonzero
    vecpath/adv_align look like a discovery when it is an artifact of the omission.
    """
    b1 = (w_r * asf_std).clone()
    b2 = (w_r * res_std).clone()
    b1[state_end:] = 0.0
    b2[state_end:] = 0.0
    b1 = b1 / b1.norm().clamp_min(1e-8)
    b2 = b2 - (b2 @ b1) * b1
    n2 = b2.norm()
    if float(n2) < 1e-6:  # degenerate (uniform EV): the two directions coincide
        return b1.unsqueeze(0)
    return torch.stack([b1, b2 / n2])


def vector_pathwise_target(means, g_vec, action_std, step, low, high):
    """Trust-region-capped regression target for the policy mean.

    A REGRESSION target rather than a linear -coef*(mu.g) term: a linear term keeps pushing
    for all update_epochs passes over the same batch, whereas this one stops contributing
    the moment mu reaches it. And a precomputed target rather than backprop through the
    head: the head is only ever fit on on-policy actions, so differentiating through it
    would chase its extrapolation off the data manifold with nothing bounding the step.

    g is normalised by its BATCH RMS, so `step` is the root-mean-square displacement in
    units of the batch action std. The PER-ROW clamp then bounds the tail: without it an
    outlier row is limited only by the action box, and TD residuals are heavy-tailed.
    """
    rms = g_vec.pow(2).sum(-1).mean().sqrt()
    disp = step * action_std * (g_vec / (rms + 1e-8))
    cap = step * action_std
    return (means + disp.clamp(-cap, cap)).clamp(low, high)


class ActionSuccessorHead(nn.Module):
    """E[psi(s') | s, a] in the critic head's STANDARDIZED units, quadratic in a.

        g(s, a) = c(s) + B(s) a + D(s) (a*a)

    Quadratic on purpose: phi is itself quadratic in a, so the model class matches the
    feature class it has to predict the future accumulation of. The payoff is that the
    action Jacobian is CLOSED FORM,

        d g / d a = B(s) + 2 D(s) diag(a),

    so a (sf_dim x act_dim) Jacobian at every one of 32768 rows costs one linear head
    evaluation instead of sf_dim backward passes.

    Zero-initialised (weights AND biases): the learned half of the Jacobian starts at
    exactly 0, so iteration 1 runs on the analytic d phi / d a alone and the variant
    departs from v9 only as this head earns it. The gradient is not dead -- the input
    features are nonzero, so the first fit moves it immediately.
    """

    def __init__(self, feat_dim, sf_dim, act_dim):
        super().__init__()
        self.sf_dim = sf_dim
        self.act_dim = act_dim
        self.const = nn.Linear(feat_dim, sf_dim)
        self.lin = nn.Linear(feat_dim, sf_dim * act_dim)
        self.quad = nn.Linear(feat_dim, sf_dim * act_dim)
        with torch.no_grad():
            for m in (self.const, self.lin, self.quad):
                m.weight.zero_()
                m.bias.zero_()

    def jacobian_coeffs(self, feat):
        """(B(s), D(s)) with shapes (n, sf_dim, act_dim) -- the a-dependent coefficients."""
        n = feat.shape[0]
        return (
            self.lin(feat).view(n, self.sf_dim, self.act_dim),
            self.quad(feat).view(n, self.sf_dim, self.act_dim),
        )

    def forward(self, feat, action):
        lin, quad = self.jacobian_coeffs(feat)
        a = action.unsqueeze(1)
        return self.const(feat) + (lin * a).sum(-1) + (quad * (a * a)).sum(-1)
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, z=None):
        ...
        action = to_action(z)
        # v32 keeps the PER-DIMENSION log-prob: channel A gives each action coordinate
        # its own advantage and its own PPO clip, which needs log pi_j, not their sum.
        # The two are consistent by construction -- sum_j log pi_j is the joint log-prob
        # used by the ratio, the KL diagnostics and the early stop, all unchanged.
        log_prob_dims = dist.log_prob(z) - log_det_fn(z)
        log_prob = log_prob_dims.sum(1)
        # Linearization point for the action Jacobian, and the per-dimension baseline.
        # For actor_dist=beta, to_action is an affine rescale, so this is EXACTLY E[a]
        # and Ahat_j = g_j (a_j - mean_j) is exactly zero-mean. The gaussian path squashes
        # through tanh, where it holds to first order only.
        mean_action = to_action(dist.mean)
        ...
        return action, z, log_prob, entropy, value_sf, log_prob_dims, mean_action
...
if __name__ == "__main__":
    ...
    # --- v32 action-successor head -------------------------------------------------
    # Top-level for the SAME reason as LeJepaSSL above: as an Agent submodule its params
    # would join agent.parameters() (the PPO optimizer) while sitting in NEITHER
    # actor_parameters() nor critic_parameters(), so with separate_grad_clip they would
    # go unclipped -- and worse, the dual-backward zeroes grads between the two passes,
    # so a module fed only by one of them would have its gradient silently discarded.
    asf_head = ActionSuccessorHead(args.hidden, sf_dim, act_dim_sf).to(device)
    asf_optimizer = optim.Adam(asf_head.parameters(), lr=args.asf_lr, eps=1e-5)
    # Standardizer SCALE snapshot taken at fit time and consumed by the next iteration's
    # Jacobian. Only the scale is needed: d(psi_std * sf_std + sf_mean)/da = sf_std * d psi_std/da,
    # so the mean drops out of every derivative and is deliberately not tracked.
    asf_std = torch.ones(sf_dim, device=device)
    asf_fitted = False
    # Action-space bounds for the channel-B improvement clamp.
    jac_action_low = torch.as_tensor(
        envs.single_action_space.low, dtype=torch.float32, device=device
    )
    jac_action_high = torch.as_tensor(
        envs.single_action_space.high, dtype=torch.float32, device=device
    )
    # phi block boundaries: phi = [e(s), s, a, a*a, 1]. Only the a and a*a blocks are
    # action-dependent, and their derivatives are I and 2 diag(a) exactly.
    jac_a0 = args.emb_dim + obs_dim
    jac_a1 = jac_a0 + act_dim_sf
    ...
    # v32: per-dimension old log-probs (channel A's ratio) and the rollout policy mean
    # (the Jacobian's linearization point AND the per-dimension advantage baseline).
    logprob_dims = torch.zeros(
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        asf_optimizer.param_groups[0]["lr"] = frac * args.asf_lr
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                (
                    action, z, logprob, ent, value_sf, logprob_d, mean_a
                ) = agent.get_action_and_value(next_obs)
                values[step] = psi_raw(value_sf[:, 0]) @ w_r
            actions[step] = action
            latent_zs[step] = z
            logprobs[step] = logprob
            logprob_dims[step] = logprob_d
            action_means[step] = mean_a
        ...
        with torch.no_grad():
            ...
            # v32 keeps the head's own STANDARDIZED output as well: the action-successor
            # head regresses onto psi_std(s') directly, so its predictions and the critic
            # head live in one frame and no cross-iteration standardizer mismatch can creep
            # in between the fit and the Jacobian that consumes it.
            psi_next_std = agent.get_value(next_obses.reshape(flat_obs_shape))[:, 0].reshape(
                args.num_steps, args.num_envs, sf_dim
            )
            psi_next = psi_raw(psi_next_std)
            ...
            # v32: freeze the readout the CREDIT PATH uses. w_r is re-solved further
            # down this same iteration, but values[], next_transition_values and
            # adv_vector above were all produced by THIS one. The action Jacobian is
            # contracted against w_r to score those same actions, so it has to read
            # the same readout -- otherwise the per-dimension attribution and the
            # advantage it redistributes come from two different reward probes.
            jac_w_r = w_r.clone()
            jac_w_r_solved = w_r_solved
        ...
        # ================= v32: SUCCESSOR-FEATURE ACTION JACOBIAN =====================
        # J = d Psi / d a at a = mu(s), where Psi(s,a) = phi(s,a) + gamma E[psi(s')|s,a].
        #
        # ANALYTIC HALF (always available, exact, zero variance). Only phi's a and a*a
        # blocks depend on the action, with derivatives I and 2 diag(a), so
        #     d(w_r . phi)/da = w_r[a-block] + 2 w_r[a*a-block] * mu.
        # For HalfCheetah the -0.1||a||^2 control cost is exactly representable in the
        # a*a block, so this term IS the true control-cost gradient.
        #
        # LEARNED HALF. The head outputs standardized psi, so its raw-unit Jacobian is
        # asf_std * (B + 2 D diag(mu)); contracting with w_r therefore contracts the
        # standardized Jacobian against (w_r * asf_std). Zero until the head is fit once.
        #
        # w_r here is the PREVIOUS solve -- the same readout that produced values[] and
        # adv_vector above. Mixing this iteration's w_r into a Jacobian scoring last
        # iteration's actions would put two different reward readouts in one credit term.
        with torch.no_grad():
            flat_actions = actions.reshape(-1, act_dim_sf)
            flat_means = action_means.reshape(-1, act_dim_sf)
            n_rows = flat_means.shape[0]
            # Psi is EXACTLY quadratic in a, so write w_r . Psi(s,a) coordinate-wise as
            #     L_j a_j + Q_j a_j^2  ->  w_r.(Psi(a) - Psi(mu)) = sum_j g_j dev_j + Q_j dev_j^2
            # with g_j = L_j + 2 Q_j mu_j and dev = a - mu. Both terms are exact; there is no
            # Taylor remainder to drop, and at sigma ~ 0.3 the quadratic term is not small.
            jac_q = jac_w_r[jac_a1 : jac_a1 + act_dim_sf].expand(n_rows, act_dim_sf).clone()
            jac_g = (
                jac_w_r[jac_a0:jac_a1]
                + 2.0 * jac_w_r[jac_a1 : jac_a1 + act_dim_sf] * flat_means
            )
            jac_g_analytic_norm = float(jac_g.norm(dim=-1).mean())
            jac_lev = torch.zeros_like(flat_means)
            jac_metric = None
            if asf_fitted and args.jac_use_learned:
                if args.jac_improve_beta > 0.0:
                    jac_metric = torch.zeros(n_rows, act_dim_sf, act_dim_sf, device=device)
                w_scaled = jac_w_r * asf_std      # readout expressed in standardized head units
                emb_scale = asf_std[: args.emb_dim].view(1, -1, 1)
                for s0 in range(0, n_rows, args.jac_chunk):
                    sl = slice(s0, min(s0 + args.jac_chunk, n_rows))
                    lin, quad = asf_head.jacobian_coeffs(critic_feat_buf[sl])
                    jac_std = lin + 2.0 * quad * flat_means[sl].unsqueeze(1)   # (n, sf, act)
                    jac_g[sl] += args.gamma * torch.einsum("k,nkj->nj", w_scaled, jac_std)
                    jac_q[sl] += args.gamma * torch.einsum("k,nkj->nj", w_scaled, quad)
                    # EMBEDDING rows only, in raw units: the geometry of "what about the
                    # future the representation actually resolves". phi's e block carries
                    # no explicit action, so this block is purely the learned half.
                    j_emb = args.gamma * jac_std[:, : args.emb_dim, :] * emb_scale
                    jac_lev[sl] = j_emb.pow(2).sum(1).sqrt()
                    if jac_metric is not None:
                        jac_metric[sl] = torch.einsum("nkj,nkl->njl", j_emb, j_emb) / args.emb_dim
            # Channel A: EXACT decomposition of the action-dependent part of the advantage
            # across action coordinates. The linear term is zero-mean per coordinate because
            # mu is the distribution mean; the quadratic term is not (E[dev^2] = var > 0), so
            # it is centred per coordinate across the batch to restore the baseline property.
            jac_dev = flat_actions - flat_means
            jac_quad_term = jac_q * jac_dev.pow(2)
            jac_attrib = jac_g * jac_dev + (
                jac_quad_term - jac_quad_term.mean(0, keepdim=True)
            )                                                                 # (N, act_dim)
            jac_g_norm = float(jac_g.norm(dim=-1).mean())
            # FALSIFIER: does the attribution predict the realized advantage at all?
            attrib_sum = jac_attrib.sum(-1)
            gae_flat = adv_scalar_gae.reshape(-1)
            jac_attrib_corr = float(
                (
                    ((attrib_sum - attrib_sum.mean()) * (gae_flat - gae_flat.mean())).mean()
                    / (attrib_sum.std() * gae_flat.std() + 1e-8)
                )
            )
            # SHRINKAGE. Scale the redistribution by the evidence that the attribution
            # predicts anything: at zero correlation it vanishes, so a Jacobian carrying no
            # information costs nothing. The null s.d. of a sample correlation over N rows
            # is 1/sqrt(N) ~ 0.0055 here, so the floor is set at 3 sigma rather than 0.
            jac_calib = max(0.0, jac_attrib_corr - 3.0 / (n_rows ** 0.5))
            jac_shift = attribution_shift(jac_attrib, jac_calib, args.jac_shift_clamp)
            # HELD-OUT head diagnostics, evaluated BEFORE this iteration's fit, on states
            # the head has never seen. The in-sample EV of the full prediction is useless as
            # a falsifier: psi(s') is almost entirely explained by c(s), so the head scores
            # high with lin and quad at exactly zero -- i.e. with no Jacobian at all. The
            # decisive number is the INCREMENT over a const-only prediction, which is the
            # only part the action Jacobian is made of.
            asf_ev_heldout = float("nan")
            asf_delta_ev = float("nan")
            if asf_fitted:
                ho = (
                    ((1.0 - transition_terminations) * transition_valids).reshape(-1) > 0.5
                ).nonzero(as_tuple=True)[0][: args.jac_chunk]
                if ho.numel() > 1:
                    ho_feat = critic_feat_buf[ho]
                    ho_tgt = psi_next_std.reshape(-1, sf_dim)[ho].reshape(-1)
                    asf_ev_heldout = ev_score(
                        asf_head(ho_feat, flat_actions[ho]).reshape(-1), ho_tgt
                    )
                    asf_delta_ev = asf_ev_heldout - ev_score(
                        asf_head.const(ho_feat).reshape(-1), ho_tgt
                    )
            # log-leverage spread: how anisotropic the outcome geometry is across action
            # coordinates. Near zero means channel B's metric is doing nothing.
            jac_lev_spread = float(
                torch.log(jac_lev + 1e-8).std(dim=-1).mean() if asf_fitted else 0.0
            )

            # Channel B: outcome-space improvement step, trust-region capped.
            jac_improved = flat_means
            jac_step_norm = 0.0
            if args.jac_improve_beta > 0.0 and jac_metric is not None:
                eye = torch.eye(act_dim_sf, device=device)
                # RELATIVE ridge: the metric's scale is set by the (drifting) embedding
                # scale, so an absolute ridge would be a no-op early and a wall later.
                trace_mean = jac_metric.diagonal(dim1=-2, dim2=-1).mean(-1).view(-1, 1, 1)
                mm = jac_metric + args.jac_ridge * (trace_mean + 1.0) * eye
                step = torch.linalg.solve(mm, jac_g.unsqueeze(-1)).squeeze(-1)
                cap = args.jac_trust_kappa * flat_actions.std(0, keepdim=True).clamp_min(1e-6)
                step = torch.max(torch.min(step, cap), -cap)
                jac_step_norm = float(step.norm(dim=-1).mean())
                jac_improved = (flat_means + step).clamp(jac_action_low, jac_action_high)
            # omega is held at 0 until w_r has been solved: with w_r == 0 the whole
            # Jacobian contraction is identically zero and the "redistribution" would be
            # a pure zero, i.e. harmless but also meaningless. Explicit is better.
            jac_omega_eff = args.jac_perdim_omega if jac_w_r_solved else 0.0

            # ---- THE VECTOR-PATHWISE CHANNEL ---------------------------------------
            # Everything here is under no_grad and consumed as a detached target.
            vecpath_on = args.vecpath_coef > 0.0 and asf_fitted and jac_w_r_solved
            vecpath_target = flat_means
            vecpath_g_rms = 0.0
            vecpath_jac_corr = float("nan")
            vecpath_adv_align = float("nan")
            vecpath_self_align = float("nan")
            vecpath_corr_shrunk = 0.0
            vecpath_step_eff = 0.0
            if vecpath_on:
                u, res_std = vector_pathwise_outcome_direction(
                    sf_residual.reshape(n_rows, sf_dim), jac_a0
                )
                vp_basis = (
                    vector_pathwise_reward_basis(jac_w_r, asf_std, res_std, jac_a0)
                    if args.vecpath_mode == "perp"
                    else None
                )
                if args.vecpath_mode not in ("perp", "full"):
                    raise ValueError(f"unknown vecpath_mode {args.vecpath_mode}")
                # Only bootstrapped transitions carry psi(s') at all -- and they are the
                # same rows the head was fit on.
                vp_boot = (
                    (1.0 - transition_terminations) * transition_valids
                ).reshape(n_rows, 1)
                vp_dev = flat_actions - flat_means
                g_vec = torch.zeros_like(flat_means)
                jc_num = jc_p = jc_r = jc_pp = jc_rr = None
                for s0 in range(0, n_rows, args.jac_chunk):
                    sl = slice(s0, min(s0 + args.jac_chunk, n_rows))
                    feat_c = critic_feat_buf[sl]
                    lin, quad = asf_head.jacobian_coeffs(feat_c)
                    jac_std_c = lin + 2.0 * quad * flat_means[sl].unsqueeze(1)
                    u_c = vector_pathwise_debias(
                        u[sl], jac_std_c, vp_dev[sl], asf_std, res_std,
                        vp_boot[sl], args.gamma, jac_a0,
                    )
                    if vp_basis is not None:
                        u_c = u_c - (u_c @ vp_basis.T) @ vp_basis
                    g_vec[sl] = args.gamma * torch.einsum("nk,nkj->nj", u_c, jac_std_c)

                    # FALSIFIER. Does this Jacobian predict the realized outcome at all?
                    # First-order predicted deviation vs the one actually observed, both
                    # against the head's prediction at the MEAN action. Accumulated as
                    # PER-COLUMN moments: pooling all 49 coordinates into one Pearson lets
                    # between-column mean spread inflate the denominator and biases the
                    # statistic toward zero -- which, since this number gates the step
                    # below, would quietly discard a channel that works.
                    pred_c = torch.einsum("nkj,nj->nk", jac_std_c, vp_dev[sl])[:, :jac_a0]
                    real_c = (
                        (
                            psi_next_std.reshape(n_rows, sf_dim)[sl]
                            - asf_head(feat_c, flat_means[sl])
                        )
                        * vp_boot[sl]
                    )[:, :jac_a0]
                    mom = [
                        (pred_c * real_c).sum(0), pred_c.sum(0), real_c.sum(0),
                        pred_c.pow(2).sum(0), real_c.pow(2).sum(0),
                    ]
                    if jc_num is None:
                        jc_num, jc_p, jc_r, jc_pp, jc_rr = mom
                    else:
                        jc_num, jc_p, jc_r, jc_pp, jc_rr = [
                            a + b for a, b in zip((jc_num, jc_p, jc_r, jc_pp, jc_rr), mom)
                        ]
                cov = jc_num - jc_p * jc_r / n_rows
                vp = (jc_pp - jc_p.pow(2) / n_rows).clamp_min(0.0)
                vr = (jc_rr - jc_r.pow(2) / n_rows).clamp_min(0.0)
                vecpath_jac_corr = float((cov / (vp * vr).sqrt().clamp_min(1e-12)).mean())

                if args.vecpath_control == "shuffle":
                    # Same marginal distribution of directions, zero per-sample alignment.
                    g_vec = g_vec[torch.randperm(n_rows, device=device)]
                elif args.vecpath_control == "anchor":
                    g_vec = torch.zeros_like(g_vec)
                elif args.vecpath_control != "off":
                    raise ValueError(f"unknown vecpath_control {args.vecpath_control}")

                # SELF-LIMITING STEP. A pure-noise Jacobian otherwise still buys a
                # full-size displacement on every row for every gradient step, because the
                # target normalises by the batch RMS. Shrink by the null sd of the pooled
                # correlation first, so "no measurable Jacobian" means "no step" rather
                # than "same step, random direction".
                vecpath_corr_shrunk = max(
                    0.0, vecpath_jac_corr - 3.0 / ((n_rows * jac_a0) ** 0.5)
                )
                vecpath_step_eff = args.vecpath_step * min(
                    1.0, vecpath_corr_shrunk / max(args.vecpath_corr_ref, 1e-8)
                )
                vecpath_g_rms = float(g_vec.pow(2).sum(-1).mean().sqrt())
                vecpath_self_align = (
                    float(
                        F.cosine_similarity(g_vec, vp_dev, dim=-1).mean()
                    )
                    if vecpath_g_rms > 0.0
                    else 0.0
                )
                vecpath_adv_align = (
                    float(
                ...
                vecpath_target = vector_pathwise_target(
                    flat_means,
                    g_vec,
                    flat_actions.std(0, keepdim=True).clamp_min(1e-6),
                    vecpath_step_eff,
                    jac_action_low,
                    jac_action_high,
                )
        ...
        b_logprob_dims = logprob_dims.reshape(-1, act_dim_sf)
        ...
        # How often the redistribution actually reverses a coordinate's credit relative
        # to the scalar advantage. Reads the SAME normalized advantage the loop consumes.
        with torch.no_grad():
            _sign_ref = (
                b_policy_adv_normed if (args.norm_adv and args.norm_adv_scope in
                                        ("batch", "batch_retstd")) else b_policy_adv
            )
            _a_dims = _sign_ref.unsqueeze(-1) + jac_omega_eff * jac_shift
            jac_sign_flip = float(
                (torch.sign(_a_dims) != torch.sign(_sign_ref).unsqueeze(-1)).float().mean()
            )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                (
                    _, _, newlogprob, entropy, value_sf, newlogprob_d, mean_action
                ...
                # ---- channel A: per-dimension credit, per-coordinate trust region ----
                # mean_j mb_adv_dims_j == mb_advantages EXACTLY (jac_shift is zero-sum
                # across dimensions), so the aggregate credit is v9's and only its
                # attribution across action coordinates differs. At omega == 0 the only
                # remaining change is the geometry-matched per-coordinate clip, which is
                # the control arm that prices that change on its own.
                ratio_d = (newlogprob_d - b_logprob_dims[mb_inds]).exp()
                pg_loss, mb_adv_dims, clipfrac_dim = perdim_policy_loss(
                    mb_advantages,
                    jac_shift[mb_inds],
                    jac_omega_eff,
                    ratio_d,
                ...
                clipfracs_dim += [float(clipfrac_dim)]

                # ---- channel B: outcome-space improvement regression -----------------
                # A REGRESSION onto a precomputed, trust-region-capped target, not a
                # pathwise gradient through the model: the head is only trained on
                # on-policy actions, so backpropagating through it would chase its
                # extrapolation off the data manifold with no bound on the step.
                if args.jac_improve_beta > 0.0 and asf_fitted:
                    improve_loss = (mean_action - jac_improved[mb_inds]).pow(2).sum(-1).mean()
                else:
                    improve_loss = torch.zeros((), device=device)

                # ---- the vector-pathwise regression (v35) ----------------------------
                if vecpath_on:
                    vecpath_loss = (
                        (mean_action - vecpath_target[mb_inds]).pow(2).sum(-1).mean()
                    )
                else:
                    vecpath_loss = torch.zeros((), device=device)
                ...
                if args.separate_grad_clip:
                    ...
                    (
                        pg_loss
                        + args.jac_improve_beta * improve_loss
                        + args.vecpath_coef * vecpath_loss
        ...
        # ---- v32 action-successor head: CROSS-FIT --------------------------------------
        # Fit AFTER the PPO update, consumed by the NEXT iteration's Jacobian. Fitting it
        # on the rollout it then scores would let it memorize this batch's action noise
        # and hand the actor back its own exploration as if it were signal.
        # Target is psi_std(s'), the critic head's OWN output frame, so the head and the
        # critic speak the same units and only ONE EMA step of standardizer drift separates
        # the snapshot below from the frame psi_next_std was produced in (sf_rate is 1/count
        # early and 0.01 thereafter, so this matters only for the first few iterations).
        # Only bootstrappable transitions are used: at a termination psi(s') is not the
        # continuation value, and at an invalid step there is no s' at all.
        asf_std = sf_std.clone()
        asf_loss_mean = float("nan")
        asf_gn_mean = float("nan")
        asf_ev = float("nan")
        asf_steps = 0
        asf_fit_mask = (
            ((1.0 - transition_terminations) * transition_valids).reshape(-1) > 0.5
        )
        n_asf = int(asf_fit_mask.sum())
        if n_asf >= args.asf_batch:
            asf_feat = critic_feat_buf[asf_fit_mask]                    # already detached
            asf_act = actions.reshape(-1, act_dim_sf)[asf_fit_mask]
            asf_tgt = psi_next_std.reshape(-1, sf_dim)[asf_fit_mask]
            asf_loss_sum = 0.0
            asf_gn_sum = 0.0
            for _ in range(args.asf_epochs):
                perm = torch.randperm(n_asf, device=device)
                for s0 in range(0, n_asf - args.asf_batch + 1, args.asf_batch):
                    idx = perm[s0 : s0 + args.asf_batch]
                    pred = asf_head(asf_feat[idx], asf_act[idx])
                    asf_loss = F.mse_loss(pred, asf_tgt[idx])
                    asf_optimizer.zero_grad(set_to_none=True)
                    asf_loss.backward()
                    asf_gn = nn.utils.clip_grad_norm_(asf_head.parameters(), args.asf_grad_clip)
                    asf_optimizer.step()
                    asf_loss_sum += float(asf_loss)
                    asf_gn_sum += float(asf_gn)
                    asf_steps += 1
            if asf_steps > 0:
                asf_loss_mean = asf_loss_sum / asf_steps
                asf_gn_mean = asf_gn_sum / asf_steps
                asf_fitted = True
                with torch.no_grad():
                    ev_pred = asf_head(asf_feat, asf_act)
                    asf_ev = ev_score(ev_pred.reshape(-1), asf_tgt.reshape(-1))
        ...
        # ---- v32 Jacobian channel -----------------------------------------------
        writer.add_scalar("jac/attrib_corr", jac_attrib_corr, global_step)
        writer.add_scalar("jac/g_norm", jac_g_norm, global_step)
        writer.add_scalar("jac/g_analytic_norm", jac_g_analytic_norm, global_step)
        writer.add_scalar(
            "jac/learned_frac",
            (jac_g_norm - jac_g_analytic_norm) / (jac_g_norm + 1e-8),
            global_step,
        )
        writer.add_scalar("jac/lev_log_spread", jac_lev_spread, global_step)
        writer.add_scalar("jac/omega_eff", jac_omega_eff, global_step)
        writer.add_scalar("jac/step_norm", jac_step_norm, global_step)
        writer.add_scalar("jac/asf_loss", asf_loss_mean, global_step)
        writer.add_scalar("jac/asf_ev", asf_ev, global_step)
        writer.add_scalar("jac/asf_ev_heldout", asf_ev_heldout, global_step)
        writer.add_scalar("jac/asf_delta_ev", asf_delta_ev, global_step)
        writer.add_scalar("vecpath/jac_corr", vecpath_jac_corr, global_step)
        writer.add_scalar("vecpath/corr_shrunk", vecpath_corr_shrunk, global_step)
        writer.add_scalar("vecpath/step_eff", vecpath_step_eff, global_step)
        writer.add_scalar("vecpath/self_align", vecpath_self_align, global_step)
        writer.add_scalar("vecpath/loss", float(vecpath_loss), global_step)
        writer.add_scalar("vecpath/adv_align", vecpath_adv_align, global_step)
        writer.add_scalar("vecpath/g_rms", vecpath_g_rms, global_step)
        writer.add_scalar(
            "vecpath/step_norm",
            float((vecpath_target - flat_means).norm(dim=-1).mean()),
            global_step,
        )
        writer.add_scalar("jac/calibration", jac_calib, global_step)
        writer.add_scalar("jac/shift_std", float(jac_shift.std()), global_step)
        writer.add_scalar("jac/sign_flip_frac", jac_sign_flip, global_step)
        writer.add_scalar(
            "jac/clipfrac_dim", float(np.mean(clipfracs_dim)) if clipfracs_dim else 0.0,
            global_step,
        )
        writer.add_scalar("jac/asf_grad_norm", asf_gn_mean, global_step)
        writer.add_scalar("jac/asf_steps", asf_steps, global_step)
