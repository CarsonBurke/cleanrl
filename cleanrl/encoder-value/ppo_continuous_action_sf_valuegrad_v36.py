...
#
# DPG ON SUCCESSOR FEATURES. The action-gradient of predicted value, taken through the
# 62-dim successor-feature model, used to steer the policy mean.
#
# WHAT v35 MEASURED, AND WHY IT SENT US HERE.
#   v35 used the PER-SAMPLE vector advantage as an outcome direction u and moved the mean
#   along gamma J^T u. Two things came out of it:
#     - the Jacobian is real: vecpath/jac_corr held 0.24-0.32, far above the ~0.0004 null.
#       The head genuinely predicts realized psi(s') deviations.
#     - the channel did nothing: at 900k the treatment (3613) did not separate from a
#       shuffle control (3704) despite step_norm sitting at full nominal.
#
#   The reason is structural, not a tuning failure. u is built from sf_residual, whose state
#   coordinates contain gamma*psi(s_{t+1}), and s_{t+1} depends on a_t. That makes
#   E[g] contain gamma^2 J^T diag(asf_std/res_std) J dev -- a PSD form, so E[g.dev] >= 0
#   with no reward involved: the policy gets pulled toward whatever it just sampled. The fix
#   (subtract the head's own action effect) removes that bias, but the action-attributable
#   part of the residual is ALSO the predicted advantage of the action. Debiasing therefore
#   deletes the improvement signal along with the artifact. Measured: self_align went
#   +0.55/+0.92 (undebiased, synthetic) -> -0.066 (debiased, live), i.e. straight past zero
#   into repulsion, and the return never moved either way.
#
# THE SEPARATION THAT DOES WORK.
#   The mechanical bias is a PSD form in dev: positive whether or not the outcome was GOOD.
#   Improvement needs a weighting by the reward direction. So use the reward covector rather
#   than the realized residual, and the per-sample dev disappears entirely:
#
#       g(s) = J(s)^T (w_r * asf_std)          the action-gradient of V
#
#   dev never enters, so self-alignment is structurally impossible -- not corrected, absent.
#   g is still per-STATE (J depends on s) and every one of the 62 successor coordinates
#   shapes the 6-dim action direction. This is DPG, with the Q-network replaced by the
#   successor-feature model, and it is NOT the scalar bottleneck: nothing here is
#   "collapse to a scalar advantage, then multiply grad log pi". The embedding sets the
#   DIRECTION of the action update, per state.
#
# THE ANALYTIC HALF IS EXACT AND FREE.
#   phi = [e(s), s, a, a*a, 1] is quadratic in the action, so d phi/d a_j is exactly
#   e_{a0+j} + 2 a_j e_{a1+j} and the analytic part of g is w_a[j] + 2 mu_j w_aa[j].
#   HalfCheetah's reward is x_vel - 0.1||a||^2 and the control cost is not a function of
#   state at all, so this recovers MuJoCo's control-cost gradient in closed form.
#
#   "EXACT" MEANS EXACT FOR THE FITTED MODEL, NOT FOR THE TRUE REWARD. It is the exact
#   derivative of w_r . phi, and w_r is a ridge fit. a = policy(s) + noise is partly
#   collinear with [e(s), s] without lying in its span, so w_a absorbs whatever reward the
#   49-dim state basis misses and projects it onto mu -- an omitted-variable bias that shows
#   up as a spurious constant push on every mean. sf/w_a_norm logs it.
#
#   AND THE ANALYTIC HALF ALONE IS NOT A DIRECTION WORTH FOLLOWING. Q = immediate +
#   gamma*future; this is only the immediate term, which on HalfCheetah is -0.2a, a pure
#   control-cost minimiser that drives actions toward zero. It is half of dQ/da, not a
#   conservative fallback -- which is why the falsifier gate below shrinks the COMBINED
#   direction rather than sparing the analytic part. --no-vg-use-learned prices exactly
#   this, and is expected to be BAD; it is a control, not a candidate.
#
# THIS CHANNEL HAS NEVER BEEN RUN. v32/v33/v34 all computed jac_g = J^T w_r and then spent
#   it re-weighting a score-function term (channel A), which put a vector back through the
#   scalar bottleneck it was built to avoid. Channel B, the one that consumes jac_g as an
#   actual action-space direction, defaulted to jac_improve_beta = 0 in every one of them.
#
# FORM. Same as v35: a regression of the policy mean onto a precomputed, trust-capped target
#   mu_target = clamp(mu + step * sd(a) * ghat), gated by the jac_corr falsifier so a
#   noise Jacobian buys a proportionally smaller step instead of a full-size random one.
#
# CONTROLS.
#   vg_control="shuffle"  permute g across the batch: same displacement distribution, wrong
#                         state. Prices the per-STATE direction, which is the whole claim.
#   vg_control="anchor"   g := 0. Prices the proximal anchor the regression carries anyway.
#   vg_coef=0             reproduces v9's update rule (channel A is off by default here).
#
# FALSIFIERS. vg/jac_corr (does J predict realized psi(s') deviations at all), vg/self_align
#   (must stay at ~0 BY CONSTRUCTION here -- if it ever drifts, dev has leaked into g and the
#   v35 failure has returned), vg/analytic_frac (how much of g is the exact analytic term vs
#   the learned one, so a collapsing head is visible immediately).
#
import os
import random
import time
...
class Args:
    ...
    # ---- v36: DPG on successor features ---------------------------------------------
    vg_coef: float = 0.5             # weight of the mu-regression onto the value-gradient
    #                                  target. 0 => v9's update rule.
    vg_step: float = 0.25            # target displacement, in units of the batch action std
    vg_control: str = "off"          # "off" | "anchor" (g:=0) | "shuffle" (break the state
    #                                  correspondence while keeping the displacements)
    vg_corr_ref: float = 0.2         # jac_corr at which the step reaches its full size
    vg_use_learned: bool = True      # add gamma * dE[psi(s')]/da to the exact analytic term
...
def value_gradient_target(means, g_vec, action_std, step, low, high):
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Only bootstrapped transitions carry psi(s'), and they are the rows the head
            # was fit on.
            vg_boot = ((1.0 - transition_terminations) * transition_valids).reshape(
                n_rows, 1
            )

            # ---- THE VALUE-GRADIENT CHANNEL ----------------------------------------
            # Everything here is under no_grad and consumed as a detached target.
            vg_on = args.vg_coef > 0.0 and jac_w_r_solved
            vg_target = flat_means
            vg_jac_corr = float("nan")
            vg_self_align = float("nan")
            vg_self_align_null = float("nan")
            vg_analytic_frac = float("nan")
            vg_corr_shrunk = 0.0
            vg_step_eff = 0.0
            vg_g_rms = 0.0
            if vg_on:
                # EXACT analytic half. d(w_r . phi)/d a_j = w_a[j] + 2 mu_j w_aa[j], from
                # phi = [e, s, a, a*a, 1] being quadratic in the action. No model and no
                # error: on HalfCheetah this IS the control-cost gradient, since the cost
                # -0.1||a||^2 is not a function of state and lands entirely in the a*a block.
                g_analytic = (
                    jac_w_r[jac_a0:jac_a1]
                    + 2.0 * jac_w_r[jac_a1 : jac_a1 + act_dim_sf] * flat_means
                )
                g_vec = g_analytic.clone()
                vg_dev = flat_actions - flat_means
                use_learned = asf_fitted and args.vg_use_learned
                jc_num = jc_p = jc_r = jc_pp = jc_rr = None
                if use_learned:
                    # psi_next_raw = asf_std * psi_next_std, so d(w_r . psi_next_raw)/da
                    # contracts the STANDARDIZED Jacobian against w_r * asf_std.
                    w_scaled = jac_w_r * asf_std
                    for s0 in range(0, n_rows, args.jac_chunk):
                        sl = slice(s0, min(s0 + args.jac_chunk, n_rows))
                        feat_c = critic_feat_buf[sl]
                        lin, quad = asf_head.jacobian_coeffs(feat_c)
                        jac_std_c = lin + 2.0 * quad * flat_means[sl].unsqueeze(1)
                        # Masked: a terminal/invalid transition has no s', so its future
                        # term is zero. Immaterial on HalfCheetah (no terminations), a few
                        # percent of rows on Hopper and Walker2d.
                        g_vec[sl] += args.gamma * vg_boot[sl] * torch.einsum(
                            "k,nkj->nj", w_scaled, jac_std_c
                        )
                        # FALSIFIER. Does J predict realized psi(s') deviations at all?
                        # Per-COLUMN moments: pooling all 49 state coordinates into one
                        # Pearson lets between-column mean spread inflate the denominator
                        # and biases the statistic toward zero, and this number gates the
                        # step below.
                        # pred_c carries the SAME mask as real_c: leaving it unmasked pits
                        # a nonzero prediction against a forced-zero target, inflating the
                        # denominator and biasing jac_corr down -- i.e. under-stepping.
                        pred_c = (
                            torch.einsum("nkj,nj->nk", jac_std_c, vg_dev[sl]) * vg_boot[sl]
                        )[:, :jac_a0]
                        real_c = (
                            (
                        ...
                        if jc_num is None:
                            ...
                            jc_num, jc_p, jc_r, jc_pp, jc_rr = [
                                a + b
                                for a, b in zip((jc_num, jc_p, jc_r, jc_pp, jc_rr), mom)
                            ]
                    cov = jc_num - jc_p * jc_r / n_rows
                    vp = (jc_pp - jc_p.pow(2) / n_rows).clamp_min(0.0)
                    vr = (jc_rr - jc_r.pow(2) / n_rows).clamp_min(0.0)
                    vg_jac_corr = float(
                        (cov / (vp * vr).sqrt().clamp_min(1e-12)).mean()
                    )
                else:
                    # Analytic-only ablation. NOT logged as 1.0: a literal 1.0 in
                    # TensorBoard is indistinguishable from "the head is perfect".
                    vg_jac_corr = float("nan")
                    vg_corr_shrunk = 1.0

                # The SHARE of g along the analytic half, <g_a, g>/||g||^2. A ratio of norms
                # is not a share: it exceeds 1 whenever the learned half opposes the analytic
                # one, which is exactly the interesting case.
                vg_analytic_frac = float(
                    (
                        (g_analytic * g_vec).sum(-1)
                        / g_vec.pow(2).sum(-1).clamp_min(1e-12)
                    ).mean()
                )
                if args.vg_control == "shuffle":
                    # Same displacement distribution, wrong state. Prices the per-state
                    # direction, which is the entire claim of this channel.
                    g_vec = g_vec[torch.randperm(n_rows, device=device)]
                elif args.vg_control == "anchor":
                    g_vec = torch.zeros_like(g_vec)
                elif args.vg_control != "off":
                    raise ValueError(f"unknown vg_control {args.vg_control}")

                # SELF-LIMITING STEP. Without it a pure-noise Jacobian still buys a
                # full-size displacement on every row, because the target normalises by the
                # batch RMS -- the mirror image of how v34's channel died.
                if use_learned:
                    # max(0.0, nan) == 0.0, so a NaN correlation fails safe to no step.
                    vg_corr_shrunk = max(
                        0.0, vg_jac_corr - 3.0 / ((n_rows * jac_a0) ** 0.5)
                    )
                vg_step_eff = args.vg_step * min(
                    1.0, vg_corr_shrunk / max(args.vg_corr_ref, 1e-8)
                )
                vg_g_rms = float(g_vec.pow(2).sum(-1).mean().sqrt())
                # MUST stay at ~0: g is built from w_r and J(s,mu), neither of which sees
                # dev. Any drift means the sampled action has leaked in and v35's
                # self-imitation failure has returned.
                # NULL-CORRECTED. Under the default Beta policy the action map is LINEAR,
                # so E[a] = to_action(E[z]) = mean_action and E[dev] = 0 exactly -- the raw
                # cosine is already unbiased. That is NOT true of the gaussian path, where
                # tanh-squashing makes E[dev|s] anti-parallel to mu while g_analytic ~ -0.2 mu
                # is too, manufacturing a positive cosine with no leak whatsoever. Logging the
                # permuted-dev null alongside makes the metric readable under either.
                if vg_g_rms > 0.0:
                    vg_self_align = float(
                        F.cosine_similarity(g_vec, vg_dev, dim=-1).mean()
                    )
                    vg_self_align_null = float(
                        F.cosine_similarity(
                            g_vec, vg_dev[torch.randperm(n_rows, device=device)], dim=-1
                        ).mean()
                    )
                else:
                    vg_self_align = vg_self_align_null = 0.0
                vg_target = value_gradient_target(
                    flat_means,
                    g_vec,
        ...
        clipfracs = []
        clipfracs_dim = []
        # Pre-initialised because the logging below reads the LAST minibatch's value:
        # every other vg_* metric is set before the update loop, and this one being the
        # exception is a NameError waiting for someone to add a `continue` to the body.
        vg_loss = torch.zeros((), device=device)
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # ---- the value-gradient regression (v36) ----------------------------
                if vg_on:
                    vg_loss = (mean_action - vg_target[mb_inds]).pow(2).sum(-1).mean()
                else:
                    vg_loss = torch.zeros((), device=device)
        ...
        writer.add_scalar("vg/jac_corr", vg_jac_corr, global_step)
        writer.add_scalar("vg/corr_shrunk", vg_corr_shrunk, global_step)
        writer.add_scalar("vg/step_eff", vg_step_eff, global_step)
        writer.add_scalar("vg/self_align", vg_self_align, global_step)
        writer.add_scalar("vg/self_align_null", vg_self_align_null, global_step)
        writer.add_scalar(
            "sf/w_a_norm", float(w_r[jac_a0 : jac_a1 + act_dim_sf].norm()), global_step
        )
        writer.add_scalar("vg/analytic_frac", vg_analytic_frac, global_step)
        writer.add_scalar("vg/g_rms", vg_g_rms, global_step)
        writer.add_scalar("vg/loss", float(vg_loss), global_step)
        writer.add_scalar(
            "vg/step_norm", float((vg_target - flat_means).norm(dim=-1).mean()), global_step
        )
