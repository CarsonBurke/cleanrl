...
class Args:
    ...
    phi_delta: bool = True           # add the transition delta s_{t+1}-s_t to phi. See
    #                                  phi_features: w_r is the only scalar bridge in this
    #                                  design and it measured r2=0.806 for the first 500k,
    #                                  which propagates to the measured 0.265 value error.
...
def phi_features(emb, obs, next_obs, action, use_delta):
    """phi = [e, s, ds, a, a*a, 1]   where ds = s_{t+1} - s_t.

    THE ds BLOCK IS THIS FILE'S CHANGE, and it exists to fix a MEASURED bias, not on
    aesthetics. HalfCheetah's reward uses the frame-skip AVERAGED velocity
    (x_{t+1}-x_t)/dt, while the observation carries only the INSTANTANEOUS qvel[0] at
    s_t. The gap is an acceleration term with a state-dependent coefficient, so a linear
    w_r on [e, s, a, a*a] cannot recover it -- and since w_r is the ONLY scalar bridge in
    this design (V = w_r . psi_0), that error enters every horizon of the value function.

    Measured on the v1 run, and this is the whole argument:
        reward_probe_r2   0.811 @100k  0.809 @300k  0.806 @500k  0.942 @1M  0.980 @4M
    i.e. the probe misses ~19% of immediate reward for the first 500k steps. The residual
    is TEMPORALLY WHITE while that is true (lag1 -0.108, lag5 +0.084, lag20 +0.015), so it
    amplifies through the discount by sqrt(1/(1-gamma^2)) = 7.089, and against a Lambda
    correlation time tau ~ 11.9 that predicts a relative value error of
        sqrt(1-0.806) * 7.089 / 11.9 = 0.2624
    The run measured sf/value_err_frac = 0.265 at 100k. The early value error of this
    family is therefore accounted for, to two digits, by this one missing block.

    R^2 recovers to 0.98 late only because a converged gait has small, smooth
    accelerations -- the probe's R^2 is really a readout of gait smoothness, so it is
    worst exactly when the value function is needed most. FALSIFIABLE PREDICTION:
    reward_probe_r2 should sit near 0.99 from iteration 1, not climb from 0.81.

    WHY IT WAS NOT ALREADY HERE. v1's own comment at the call site refused lagged obs on
    purpose -- "putting lagged obs in phi would let a linear w_r capture acceleration
    without the encoder, which would confound the lever with a much cheaper change." That
    was correct for keeping the encoder ablation clean, and that ablation has since been
    ANSWERED: gate/ev_latent_cap reached only 0.554 at 8M against gate/ev_trunk_probe
    0.756, so the encoder is not the winning lever and the handicap has no purpose left.

    phi may depend on the TRANSITION (s_t, a_t, s_{t+1}) without biasing the policy
    gradient: the reward itself is r(s,a,s'), successor features over transition features
    are standard, and psi still reads s_t ONLY -- so V = w_r . psi_0(s_t) stays a
    state-only function and remains a valid baseline. next_obs here is the TRANSITION
    next-obs (true terminal observation substituted at boundaries), not the reset obs.

    ORIGINAL phi = [e, s, a, a*a, 1] notes follow.
    ...
    parts = [emb, obs]
    if use_delta:
        parts.append(next_obs - obs)
    parts += [action, action * action, ones]
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.sf_dim = (args.emb_dim + obs_dim + (obs_dim if args.phi_delta else 0)
                       + 2 * act_dim + 1)  # [e, s, ds, a, a*a, 1]
...
if __name__ == "__main__":
    ...
    n_delta = obs_dim if args.phi_delta else 0
    w_r_solved = False
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            phi = phi_features(emb_buf, obs, next_obses, actions, args.phi_delta)  # (T,B,sf_dim)
        ...
        writer.add_scalar("sf/lambda_std_act",
                          sf_std[args.emb_dim + obs_dim + n_delta : -1].mean().item(), global_step)
        writer.add_scalar("sf/lambda_absmean_emb", sf_mean[: args.emb_dim].abs().mean().item(), global_step)
        if n_delta:
            writer.add_scalar("sf/lambda_std_delta",
                              sf_std[args.emb_dim + obs_dim : args.emb_dim + 2 * obs_dim].mean().item(),
                              global_step)
            writer.add_scalar("advB/lam_delta",
                              lam_vec[args.emb_dim + obs_dim : args.emb_dim + 2 * obs_dim].mean().item(),
                              global_step)
            writer.add_scalar("advB/tau_delta",
                              tau_vec[args.emb_dim + obs_dim : args.emb_dim + 2 * obs_dim].mean().item(),
                              global_step)
        ...
        writer.add_scalar("advB/lam_act",
                          lam_vec[args.emb_dim + obs_dim + n_delta : -1].mean().item(), global_step)
        ...
        writer.add_scalar("advB/tau_act",
                          tau_vec[args.emb_dim + obs_dim + n_delta : -1].mean().item(), global_step)
