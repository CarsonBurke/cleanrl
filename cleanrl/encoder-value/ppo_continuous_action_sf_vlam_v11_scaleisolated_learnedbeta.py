# PPO + successor features with scale-isolated learned SiTU cap allocation.
# v11 fixes v8's failure mode: 51 branch-local beta scalars became fast residual
# gain controls and moved some gate caps from 4 toward 2.6, increasing saturation.
# Here one bounded allocation scalar is shared by all 17 branches in each block:
#   delta = log(4) tanh(raw/log(4))
#   beta_g = 4 exp(delta), beta_u = 25 exp(-delta), beta_g beta_u = 100.
# Adaptive and official-beta activations share the same down projection. Each
# adaptive branch output is then rescaled to the official branch's per-state RMS,
# so beta can rotate feature direction but cannot attenuate/amplify residual energy.
# The three beta scalars accumulate task gradients over PPO's reused minibatches and
# receive one separate Adam step per rollout rather than 320 near-sign steps.
# Initialization exactly recovers v3's function, ordinary gradients, and RNG stream.
#
...
#
# V2 EXPERIMENTS. Its treatment used per-dimension lambda; --no-per-dim-lambda was
# the scalar-lambda arm used as this v3 activation experiment's requested reference.
# =====================================================================================
...
class Args:
    ...
    beta_learning_rate: float = 3e-3  # one averaged geometry step per rollout
    num_envs: int = 16
    num_steps: int = 2048
    anneal_lr: bool = True
    gamma: float = 0.99
    ...
    # False reproduces the requested sf_vlam_v2_scalarlam reference configuration.
    per_dim_lambda: bool = False     # derive lambda_d per phi coordinate from its MEASURED
...
@torch.no_grad()
def accumulate_scaled_grads(parameters, accumulators, scale):
    """Accumulate current grads with a shared clip factor, then clear them."""
    for parameter, accumulator in zip(parameters, accumulators, strict=True):
        if parameter.grad is not None:
            accumulator.add_(parameter.grad * scale)
            parameter.grad = None


def situ_glu(gate, up, beta_gate=4.0, beta_up=25.0):
    """SiTU-GLU product before the down projection."""
    capped_gate = beta_gate * torch.tanh(gate / beta_gate)
    capped_up = beta_up * torch.tanh(up / beta_up)
    return (capped_gate * torch.sigmoid(gate)) * capped_up


class SiTUGLUBranch(nn.Module):
    """Parameter- and initial-output-matched SiTU-GLU FFN."""

    def __init__(self, H):
        super().__init__()
        # The old two biased H->H linears contain 2H² + 2H parameters. Official
        # SiTU-GLU uses three bias-free matrices, so M=round(2(H+1)/3) is the
        # closest parameter-matched gated width (H=64: 8,256 vs 8,320, -0.77%).
        self.hidden_dim = max(1, round(2.0 * (H + 1) / 3.0))
        # At the inherited sqrt(2) gate/up gain and isotropic unit-variance input,
        # E[SiTU(g,u)^2] = 1.2630450818573506 (Gauss-Hermite quadrature). The old
        # ReLU² branch has output second moment 2 * E[ReLU(N(0,2))^4] = 12.
        # A rectangular orthogonal M->H down projection contributes M/H on
        # average, giving the gain below. This preserves initial residual scale;
        # the different Jacobian geometry remains the activation being tested.
        down_gain = np.sqrt(
            12.0 * H / (self.hidden_dim * 1.2630450818573506)
        )

        # Initialize the new-shaped branch from a private copy of the RNG stream,
        # then advance the public stream exactly as v2's two H->H linears did.
        # Routers, later blocks, and actor/critic heads therefore remain seed-paired
        # with the named v2 baseline even though this branch has different shapes.
        with torch.random.fork_rng(devices=[]):
            self.gate = layer_init(nn.Linear(H, self.hidden_dim, bias=False))
            self.up = layer_init(nn.Linear(H, self.hidden_dim, bias=False))
            self.down = layer_init(
                nn.Linear(self.hidden_dim, H, bias=False), std=down_gain
            )
        _ = layer_init(nn.Linear(H, H))
        _ = layer_init(nn.Linear(H, H))

    def forward(self, x, beta_gate, beta_up):
        gate = self.gate(x)
        up = self.up(x)
        adaptive = self.down(situ_glu(gate, up, beta_gate, beta_up))
        reference = self.down(situ_glu(gate, up))
        # Keep the ratio differentiable. Its beta derivative removes the radial
        # component exactly; detaching it would recreate v8's gain shortcut.
        adaptive_rms = adaptive.square().mean(dim=-1, keepdim=True).add(1e-8).sqrt()
        reference_rms = reference.square().mean(dim=-1, keepdim=True).add(1e-8).sqrt()
        return adaptive * (reference_rms / adaptive_rms)


def _branch_body(H):
    return SiTUGLUBranch(H)
...
class ThinkBlock(nn.Module):
    ...
    def __init__(self, in_dim, H, n_experts):
        ...
        self.beta_log_ratio_limit = log(4.0)
    ...
    def beta_allocation(self):
        delta = self.beta_log_ratio_limit * torch.tanh(
            self.beta_raw / self.beta_log_ratio_limit
        )
        return delta, 4.0 * torch.exp(delta), 25.0 * torch.exp(-delta)

    def forward(self, cat_feats, x0):
        x = self.in_proj(cat_feats)                                   # (B, H)
        g = torch.sigmoid(self.resid_gate)                            # (H,)
        x_in = g * x + (1.0 - g) * x0                                 # (B, H), convex
        _, beta_gate, beta_up = self.beta_allocation()

        d_dense = self.dense(
            self.dense_norm(x_in), beta_gate, beta_up
        )                                                             # (B, H)

        m_in = self.moe_norm(x_in)
        weights = torch.softmax(self.gate(m_in), dim=-1)              # (B, E)
        all_out = torch.stack(
            [expert(m_in, beta_gate, beta_up) for expert in self.experts], dim=1
        )                                                             # (B, E, H)
...
if __name__ == "__main__":
    ...
    beta_blocks = [module for module in agent.modules() if isinstance(module, ThinkBlock)]
    beta_params = [block.beta_raw for block in beta_blocks]
    beta_param_ids = {id(parameter) for parameter in beta_params}
    base_params = [
        parameter
        for parameter in agent.parameters()
        if id(parameter) not in beta_param_ids
    ]
    optimizer = optim.Adam(base_params, lr=args.learning_rate, eps=1e-5)
    beta_optimizer = optim.Adam(
        beta_params, lr=args.beta_learning_rate, eps=1e-5
    )
    ...
    # Beta is deliberately excluded: it accumulates the un-clipped task gradient
    # over the reused rollout and receives one separate geometry step below.
    actor_params = [
        parameter
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        beta_grad_sums = [torch.zeros_like(parameter) for parameter in beta_params]
        beta_accum_steps = 0
        beta_grad_norm = 0.0
        beta_update_count = 0
        if args.anneal_lr:
            ...
            beta_optimizer.param_groups[0]["lr"] = frac * args.beta_learning_rate
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    critic_clip_scale = (
                        args.critic_grad_clip / (critic_gn + 1e-12)
                    ).clamp(max=1.0)
                    accumulate_scaled_grads(
                        beta_params, beta_grad_sums, critic_clip_scale
                    )
                    ...
                    actor_clip_scale = (
                        args.actor_grad_clip / (actor_gn + 1e-12)
                    ).clamp(max=1.0)
                    accumulate_scaled_grads(
                        beta_params, beta_grad_sums, actor_clip_scale
                    )
                    ...
                    critic_gn = actor_gn = nn.utils.clip_grad_norm_(
                        base_params, args.max_grad_norm
                    )
                    global_clip_scale = (
                        args.max_grad_norm / (critic_gn + 1e-12)
                    ).clamp(max=1.0)
                    accumulate_scaled_grads(
                        beta_params, beta_grad_sums, global_clip_scale
                    )
                    optimizer.step()
                beta_accum_steps += 1
        ...
        if beta_accum_steps > 0:
            for parameter, gradient_sum in zip(
                beta_params, beta_grad_sums, strict=True
            ):
                parameter.grad = gradient_sum.div(beta_accum_steps)
            beta_grad_norm = float(
                torch.linalg.vector_norm(
                    torch.stack([parameter.grad for parameter in beta_params])
                )
            )
            beta_optimizer.step()
            beta_update_count = 1
        ...
        writer.add_scalar(
            "charts/beta_learning_rate",
            beta_optimizer.param_groups[0]["lr"],
            global_step,
        )
        ...
        writer.add_scalar("situ/beta_grad_norm", beta_grad_norm, global_step)
        writer.add_scalar(
            "situ/beta_updates_per_rollout", beta_update_count, global_step
        )
        for block_index, block in enumerate(beta_blocks):
            delta, beta_gate, beta_up = block.beta_allocation()
            writer.add_scalar(
                f"situ/block{block_index}_delta", delta.item(), global_step
            )
            writer.add_scalar(
                f"situ/block{block_index}_beta_gate", beta_gate.item(), global_step
            )
            writer.add_scalar(
                f"situ/block{block_index}_beta_up", beta_up.item(), global_step
            )
