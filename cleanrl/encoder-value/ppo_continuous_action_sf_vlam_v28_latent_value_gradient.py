# Calibrated latent value-GRADIENT policy credit from a LeJEPA successor critic.
#
# Base: v23_vector_preference (9,141 @8M on HalfCheetah-v4). Everything except the
# covector that scalarizes the vector TD(lambda) innovation is unchanged: same trunk,
# actor, rollout, LeJEPA workload, vector critic, per-dimension lambda machinery, PPO
# ratio clipping, and warm-up discipline.
#
# WHAT v23 GETS WRONG. v23's covector u(s) is a UNIT vector fit by a sign-only ordinal
# loss on pairs (i, j) of environments at the same rollout step:
#
#     want   sign( u . normalize(E_i - E_j) )  ==  sign( G_i - G_j )
#
# Three separate defects, all in that one line.
#
#   1. THE TWO SIDES ARE DIFFERENT OBJECTS. E is a TD(lambda) INNOVATION: if the critic
#      is unbiased then E[E_t | s_t] = 0 and its variation is action noise. G is a
#      state VALUE plus that noise. So G_i - G_j = [V(s_i) - V(s_j)] + [eps_i - eps_j],
#      and only the second bracket is visible in E_i - E_j. The first bracket is pure
#      label noise whose size is set by how far apart the paired states are -- and the
#      partner is the nearest of FIFTEEN candidates in a 32-dim embedding, which is not
#      near. The field is taught to rank STATES with a signal that can only carry
#      information about ACTIONS.
#   2. SIGN-ONLY. One bit per pair, one pair per state. The magnitude of the return gap,
#      which says how much to trust the label, is discarded.
#   3. UNIT NORM. The advantage is |E_t| cos(angle) -- its magnitude is set by the size
#      of the innovation, not by how much return a unit of innovation is worth. PPO
#      weights every sample by exactly that magnitude.
#
# WHAT REPLACES IT. A detached scalar readout f of the rich value vector, V_hat = f(psi),
# fit by dense REGRESSION on truncated Monte-Carlo returns. The policy credit is
#
#     A_t = < grad_psi f(psi(s_t)) , E_t >  ~=  f(psi_t + E_t) - f(psi_t)
#                                            =  f(Lambda_t) - f(psi_t),
#
# which IS the definition of a lambda-return advantage, read off the rich innovation
# instead of off the reward. The induced covector is still state-conditioned -- grad f
# varies with psi exactly where the value surface curves -- but it is now CALIBRATED:
# its magnitude is d(return)/d(psi), so <u, E> is in return units and PPO's per-sample
# weighting means what it is supposed to mean. Supervision goes from ~2k sign bits per
# rollout to ~16k magnitude-calibrated regression targets, and the regressed pair
# (psi -> G) is two views of one object instead of two different objects.
#
# WHAT IS PRESERVED. f is hard-detached from psi, the trunk and LeJEPA, so the critic
# remains a purely vector object trained only by vector TD; f cannot reshape the
# representation. f NEVER bootstraps -- its only target is the Monte-Carlo return, gated
# to states whose remaining episode is at least mc_window rewards deep inside the rollout
# (gamma^500 = 0.0066, so the return-to-episode-end truncation bias sits below the noise
# floor). The one-iteration cross-fit is kept verbatim: f is refit AFTER the policy
# update and consumed on the NEXT rollout, so it can never explain this rollout's action
# noise and hand the memorized explanation back as credit. Procrustes transport still
# pins LeJEPA's frame without an EMA teacher.
#
# HYPOTHESIS. Fixing credit MAGNITUDE (defect 3) and the object mismatch (defect 1) at
# once should close and pass the gap to the scalar-GAE control (v23_wgrad_scalargae,
# 9,905 @6.6M), because the same advantage is now read off a 32-dim TD-smoothed
# innovation rather than off one noisy scalar reward.
#
# DECISIVE FALSIFIER. credit/return_residual_corr -- the correlation between the credit
# the actor actually consumed and the horizon-matched realized return residual. All envs
# reset simultaneously on fixed-length MuJoCo tasks, so the cross-env mean return at a
# given rollout step is an exact horizon-matched baseline. If this does not rise well
# above the v23 ordinal field's value, the covector was not the bottleneck.
#
...
class Args:
    ...
    # --- Calibrated latent value-gradient policy credit -------------------------------
    ...
    # The readout is the ONLY place a return magnitude enters, its input is hard-detached,
    # and its target is Monte-Carlo (never a bootstrap), so it can neither reshape the
    # representation nor smuggle a scalar bootstrap into the vector critic. Its regression
    # gate is the existing mc_window mask, so a state contributes a target only when its
    # entire remaining episode -- at least mc_window rewards of it -- lies in this rollout.
...
class LatentValueReadout(nn.Module):
    """Detached scalar readout of the rich value vector, V_hat = f(psi).

    f exists ONLY to induce the covector u(s) = grad_psi f(psi(s)) that scalarizes the
    vector TD(lambda) innovation for the policy. Its input is hard-detached here rather
    than relying on callers to preserve the one-way boundary, and its only target is a
    Monte-Carlo return, so no gradient and no bootstrap of it can reach psi, the trunk or
    LeJEPA. The critic stays a purely vector object.
    """
...
def value_gradient_covector(readout, standardized_psi, chunk=8192):
    """u(s) = d f / d psi_standardized at psi(s).
...
def fit_latent_value_readout(
    ...
    """Dense regression of the truncated Monte-Carlo return onto the rich value vector."""
...
if __name__ == "__main__":
    ...
    with torch.random.fork_rng(devices=[]):
        readout = LatentValueReadout(sf_dim, args.readout_hidden).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # ---- calibrated value-gradient covector ----------------------------------
            # u(s) = grad_psi f(psi(s)) under the standardizer the readout was FIT with.
            # f consumes standardized psi, so the chain rule puts the 1/readout_std on the
            # innovation: <df/dpsi, E> = <u_standardized, E / readout_std>. Writing it that
            # way keeps the covector in exactly the units the readout was trained in.
            standardized_psi = (
                (psi_cur - readout_mean) / readout_std
            ).reshape(-1, sf_dim)
            frozen_credit_direction = value_gradient_covector(
            ...
            if readout_ready:
                ...
                # Iteration one trains the vector critic and the readout but deliberately
                # leaves the policy fixed. A random covector is not a defensible
                # exploration objective. (Same warm-up as v23's preference field.)
        ...
        with torch.no_grad():
            ...
            # init and only trains during that iteration's own PPO loop, so every row of
            # the fit input above is the same constant. An MSE loss on a constant input
            # constrains NOTHING about grad f -- but Adam still moves it, and 80 steps
            # grow |grad f| by more than an order of magnitude in a direction that means
            # nothing. Consuming that as a covector would spend the first real PPO update
            # on noise, which is precisely what the warm-up exists to prevent. Skip the
            # fit until psi actually varies; the warm-up simply lasts one iteration longer.
            psi_is_degenerate = bool(
                fit_psi.shape[0] < 2 or float(fit_psi.std(0).max()) < 1e-6
            )
            if fit_returns.numel() > 1 and not psi_is_degenerate:
                return_stat_count += 1
        ...
        with torch.no_grad():
            # Post-fit diagnostics. readout_ev is the in-sample R^2 of V_hat = f(psi)
            # against the Monte-Carlo return: it prices how much of the return the rich
            # value vector actually determines, and therefore how meaningful its gradient
            # can be. direction_update_cosine prices how far the covector moved in one
            # iteration -- the quantity the one-iteration cross-fit is spending.
            readout_ev = explained_variance_1d(readout(fit_psi), fit_targets)
            readout_target_count = int(fit_targets.numel())
        trained_credit_direction = value_gradient_covector(
