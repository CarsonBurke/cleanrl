# PPO + minimal reward-predictive successor features, v1.
#
# Hypothesis: a compact learned state-action feature space that both linearly
# predicts reward and is closed under one-step action-conditioned dynamics gives
# the critic a minimal approximate Bellman-complete subspace. A vector successor
# critic accumulates those frozen rollout features; value and policy advantage are
# read only through one global reward vector w.
#
# Algebraically, when r=phi*w exactly, projecting vector GAE through w is exactly
# ordinary scalar reward GAE. This method does not change the RL objective; any
# gain must come from preventing approximation errors in distinct future outcomes
# from cancelling inside a scalar critic.
import os
import random
import time
...
class RewardPredictiveFeatures(nn.Module):
    def __init__(self, obs_dim, act_dim, hidden, feature_dim):
        ...
        self.net = nn.Sequential(
            ...
            nn.SiLU(),
            layer_init(nn.Linear(hidden, feature_dim), std=0.5),
            nn.RMSNorm(feature_dim, elementwise_affine=False),
        )
...
def vector_gae(cumulants, psi, next_psi, terminations, boundaries, valids, gamma, gae_lambda):
    advantages = torch.zeros_like(psi)
    last = torch.zeros_like(psi[0])
    for t in reversed(range(cumulants.shape[0])):
        bootstrap = ((1.0 - terminations[t]) * valids[t]).unsqueeze(-1)
        continuation = (1.0 - boundaries[t]).unsqueeze(-1)
        delta = cumulants[t] + gamma * bootstrap * next_psi[t] - psi[t]
        last = delta + gamma * gae_lambda * continuation * last
        advantages[t] = last
    return advantages, psi + advantages
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        with torch.random.fork_rng(devices=[]):
            ...
            self.reward_vector = nn.Parameter(torch.zeros(self.sf_dim))
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            true_advantages, returns = scalar_gae(
                rewards, values, next_transition_values,
                transition_terminations, transition_boundaries, transition_valids,
                args.gamma, args.gae_lambda,
            )
            phis = agent.transition_features(raw_obses, actions)
            vector_advantages, psi_targets = vector_gae(
                phis, psis, next_psis,
                transition_terminations, transition_boundaries, transition_valids,
                args.gamma, args.gae_lambda,
            )
            advantages = vector_advantages @ w_old
            predicted_rewards = phis @ w_old
            readout_advantages, _ = scalar_gae(
                predicted_rewards, values, next_transition_values,
                transition_terminations, transition_boundaries, transition_valids,
                args.gamma, args.gae_lambda,
            )
            projection_identity_maxerr = (advantages - readout_advantages).abs().max()
            if not torch.isfinite(projection_identity_maxerr) or projection_identity_maxerr > 1e-4:
                raise RuntimeError(
                    "projected vector GAE identity failed: "
                    f"max error={projection_identity_maxerr.item():.6g}"
                )
        ...
        closure_next_raw = raw_obses[1:][pair_mask]
        ...
        # PPO first. Its frozen projected vector advantage is a plain tensor, so
        # policy backward has no path into phi, closure, psi targets, or w.
        ...
        # Critic/features second. Vector lambda targets remain the exact rollout
        # snapshot, while reward and closure train the current feature coordinates.
        critic_inds = np.arange(args.batch_size)
        closure_inds = np.arange(closure_raw.shape[0])
        for _ in range(args.update_epochs):
            np.random.shuffle(critic_inds)
            np.random.shuffle(closure_inds)
            closure_cursor = 0
            for start in range(0, args.batch_size, args.minibatch_size):
                mb_inds = critic_inds[start : start + args.minibatch_size]
                new_psi = agent.get_psi(b_obs[mb_inds])
                sf_loss = F.mse_loss(new_psi, b_psi_targets[mb_inds])
                phi = agent.transition_features(b_raw_obs[mb_inds], b_actions[mb_inds])
                reward_loss = F.mse_loss(agent.predict_reward(phi), b_rewards[mb_inds])

                closure_end = min(closure_cursor + args.minibatch_size, len(closure_inds))
                closure_mb = closure_inds[closure_cursor:closure_end]
                closure_cursor = closure_end
                phi_current = agent.transition_features(
                    closure_raw[closure_mb], closure_actions[closure_mb]
                )
                phi_next = agent.transition_features(
                    closure_next_raw[closure_mb], closure_next_actions[closure_mb]
                )
                phi_prediction = agent.predict_next_feature(
                    phi_current, closure_next_actions[closure_mb]
                )
                closure_loss = F.mse_loss(phi_prediction, phi_next)
                critic_loss = (
        ...
        with torch.no_grad():
            ...
            reward_r2 = 1.0 - (b_rewards - diagnostic_reward).var(unbiased=False) / reward_var.clamp_min(1e-8)
            diagnostic_phi_current = agent.transition_features(closure_raw, closure_actions)
            diagnostic_phi_next = agent.transition_features(
                closure_next_raw, closure_next_actions
            )
            diagnostic_phi_prediction = agent.predict_next_feature(
                diagnostic_phi_current, closure_next_actions
            )
            closure_mse = F.mse_loss(diagnostic_phi_prediction, diagnostic_phi_next)
            closure_mean_mse = F.mse_loss(
                diagnostic_phi_next,
                diagnostic_phi_next.mean(dim=0, keepdim=True).expand_as(diagnostic_phi_next),
            )
            ...
            vector_td_mse = F.mse_loss(post_psi, b_psi_targets)
            projected_corr = (
                ((b_advantages - b_advantages.mean()) * (true_advantages.reshape(-1) - true_advantages.mean())).mean()
                / (
                    b_advantages.std(unbiased=False)
                    * true_advantages.std(unbiased=False)
                ).clamp_min(1e-8)
            )
            true_projection_discrepancy = (
                b_advantages - true_advantages.reshape(-1)
            ).abs().max()
        ...
        writer.add_scalar("diagnostics/projection_identity_maxerr", projection_identity_maxerr.item(), global_step)
        writer.add_scalar("diagnostics/projected_true_gae_maxerr", true_projection_discrepancy.item(), global_step)
        writer.add_scalar("diagnostics/projected_gae_std", b_advantages.std().item(), global_step)
        writer.add_scalar("diagnostics/projected_true_gae_corr", projected_corr.item(), global_step)
