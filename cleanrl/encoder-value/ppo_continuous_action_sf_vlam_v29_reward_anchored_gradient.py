# REWARD-ANCHORED value-gradient policy credit from a LeJEPA successor critic.
#
# Base: v23_vector_preference (9,141 @8M on HalfCheetah-v4) and its sibling v28. The
# trunk, actor, rollout, LeJEPA workload, vector critic, per-dimension lambda machinery
# and PPO ratio clipping are unchanged.
#
# THE TWO CONTROLS THIS SITS BETWEEN. On the same chassis, two ways of turning the rich
# vector critic into policy credit have now been measured:
#
...
#
# The scalar control wins. That is the fact this variant is built on: a covector fit
# from returns is, so far, a worse scalarizer than the reward itself. But the two are not
# alternatives -- one is a special case of the other, and the vector version is throwing
# away the exact answer it is trying to approximate.
#
# THE CONSTRUCTION. Make the reward a coordinate of phi:
#
#     phi_t = [ r_t , (1-gamma) e_t ]          (sf_dim = 1 + emb_dim)
#
# Coordinate 0 of psi is then EXACTLY V(s) -- not a probe of it, not a regression onto
# it, the same discounted reward sum by definition -- and coordinate 0 of the vector
# TD(lambda) innovation E_t is EXACTLY the scalar GAE advantage A_t. The covector e_0 is
# therefore known analytically and is the best one measured to date. So pin it, and learn
# only what it leaves behind: the readout is
#
#     f(psi) = psi_0 + residual_std * g(psi_standardized),      g zero-ish at init,
#
# so the credit is
#
#     A_t = < grad_psi f , E_t >  =  A_t^GAE  +  residual_std * < grad g , E_t / s >.
#
# g is fit by dense regression on the part of the truncated Monte-Carlo return that
# psi_0 does NOT explain, G_t - psi_0(s_t). This is boosting, in the exact sense: stage
# one is the analytically correct reward advantage, stage two is a learned correction
# read off the 31 successor-embedding coordinates that the reward coordinate cannot see.
#
# WHY THE FLOOR IS THE CEILING OF THE CONTROLS. g's output layer is std=0.01-initialized,
# so at iteration 1 the credit is bit-equivalent to the scalar-GAE control. The vector
# path can only move it away from that as far as it can genuinely predict return the
# reward advantage missed. A covector that learns nothing costs nothing; the ordinal
# field of v23 could -- and by the numbers above, did -- cost something.
#
# WHAT THE CORRECTION IS FOR. A_t^GAE is a lambda-return residual of ONE noisy scalar.
# The other 31 coordinates are a TD-smoothed, SIGReg-whitened summary of where the
# trajectory went, with their own mixing times. Where the reward is a noisy readout of a
# smooth latent event (a gait phase completing, a fall beginning), the latent innovation
# sees it earlier and with less variance than the reward does. That is the only place a
# gain over the scalar control can come from, and it is what residual_std * <grad g, .>
# measures directly.
#
...
# coordinate the actor's advantage is built from 1/33 of the budget; the v13-v22 line
# that produced this chassis reserved half, and that value is kept.
#
# NO WARM-UP. v23 and v28 freeze the policy for iteration 1 because a random covector is
# not a defensible exploration objective. Here the covector is pinned to the exact reward
# direction from step 0 (psi zero-init => A^GAE degenerates to the plain discounted
# reward sum, which is a clean Monte-Carlo advantage), so the freeze is removed and the
# full critic trains from iteration 1, as in the scalar control.
#
# HYPOTHESIS. Anchoring at the best known covector and learning only the residual gets
# the scalar control's score as a floor and the vector critic's smoothing as upside.
#
# DECISIVE FALSIFIER. credit/anchor_correction_ratio -- the RMS of the learned correction
# over the RMS of the anchored GAE term -- read together with readout/residual_explained
# _variance (the R^2 of g on the return psi_0 does not explain). A correction that grows
# while the residual R^2 stays at zero is the covector re-injecting noise, and is the
# same failure the ordinal field had; kill on that pair, not on return alone.
#
# Historical base notes follow; references to the ordinal preference field and to a
# reward-free phi are superseded above.
#
...
class LatentValueResidual(nn.Module):
    """The learned half of the anchored readout, f(psi) = psi_0 + residual_std * g(psi).

    psi_0 is the reward coordinate of the successor vector, i.e. V(s) exactly, so the
    anchor needs no parameters and g is left with only the part of the return that the
    discounted reward sum does not already explain. The std=0.01 output init makes g
    negligible at iteration 1, so the credit starts bit-equivalent to scalar GAE.

    The input is hard-detached here rather than relying on callers to preserve the
    one-way boundary, and the only target is a Monte-Carlo residual, so no gradient and
    no bootstrap of g can reach psi, the trunk or LeJEPA.
    """

    def __init__(self, value_dim, hidden):
        super().__init__()
    ...
    def forward(self, standardized_psi):
        return self.body(standardized_psi).squeeze(-1)


def value_gradient_covector(readout, standardized_psi, chunk=8192):
    """u(s) = d g / d psi_standardized at psi(s).

    Differentiated w.r.t. the INPUT only. torch.autograd.grad with explicit inputs never
    touches .grad on the readout's parameters, so this is safe to call from inside the
    no-grad rollout block -- the enable_grad scope is local to the chunk.

    Chunked because the full rollout is 32,768 states: one graph over all of them would
    hold every intermediate activation of the readout at once for no benefit.
    """
    directions = []
    for start in range(0, standardized_psi.shape[0], chunk):
        block = standardized_psi[start : start + chunk].detach().requires_grad_(True)
        with torch.enable_grad():
            (gradient,) = torch.autograd.grad(readout(block).sum(), block)
        directions.append(gradient.detach())
    return torch.cat(directions, 0)


def fit_latent_value_readout(
    readout, optimizer, standardized_psi, targets, epochs, batch_size, grad_clip, generator
):
    """Dense regression of the ANCHOR-RESIDUAL return onto the rich value vector."""
    ...
    for _ in range(epochs):
        ...
        for start in range(0, n, batch_size):
            ...
            loss = F.mse_loss(readout(standardized_psi[index]), targets[index])
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            grad_norm = nn.utils.clip_grad_norm_(readout.parameters(), grad_clip)
            optimizer.step()
            loss_sum += loss.item()
...
if __name__ == "__main__":
    ...
    if not args.separate_grad_clip:
        raise ValueError("the value-readout warm-up requires separate_grad_clip")
    ...
    with torch.random.fork_rng(devices=[]):
        readout = LatentValueResidual(sf_dim, args.readout_hidden).to(device)
    ...
    # Scale of the return the ANCHOR leaves unexplained, G - psi_0. It conditions g's
    # regression and is multiplied back into the correction term, so g stays an O(1)
    # regression whatever the reward scale is. Same 1/count warmup as the psi
    # standardizer. Starting at 1.0 with a std=0.01 output init means the correction is
    # negligible on iteration 1 and the credit is the scalar GAE advantage.
    residual_std = torch.ones((), device=device)
    residual_stat_count = 0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # ---- reward-anchored value-gradient covector ------------------------------
            # f(psi) = psi_0 + residual_std * g(psi_standardized), so
            #
            #   grad_psi f = e_0 + (residual_std / s) * grad g,
            #   <grad_psi f, E> = E_0 + residual_std * <grad g, E / s>.
            #
            # Both terms are written as ONE covector contracted against the standardized
            # innovation E / s, so the minibatch loop stays a single dot product: the
            # anchor is e_0 scaled by s_0, which contracts to exactly E_0. Nothing about
            # the anchor is learned, and it is live from iteration 1.
            standardized_psi = (
                (psi_cur - readout_mean) / readout_std
            ).reshape(-1, sf_dim)
            residual_gradient = value_gradient_covector(
                readout, standardized_psi
            ).reshape(args.num_steps, args.num_envs, sf_dim)
            anchor_covector = torch.zeros_like(residual_gradient)
            anchor_covector[..., 0] = readout_std[0]
            frozen_credit_direction = anchor_covector + residual_std * residual_gradient
        ...
        #
        # The standardizer is snapshotted HERE, together with the fit, so the covector
        # evaluated at the top of the next iteration reads psi in the frame f was fit in.
        readout_mean = sf_mean.clone()
        readout_std = sf_std.clone()
        with torch.no_grad():
            refit_standardized_psi = (
                (psi_cur - readout_mean) / readout_std
            ).reshape(-1, sf_dim)
            # ---- fit gate ------------------------------------------------------------
            ...
            # The anchor already predicts psi_0 of the return by construction. g is fit
            # ONLY to what it leaves over, so a correction can only ever come from return
            # structure the discounted reward sum does not already carry. A constant
            # offset in this target is irrelevant -- only grad g reaches the policy.
            fit_residual = (mc_ret - psi_cur[..., 0]).reshape(-1)[fit_gate]
            # ---- degeneracy guard ------------------------------------------------------
            # psi is IDENTICALLY ZERO on iteration 1: the critic head is hard-zeroed at
            # init and only trains during that iteration's own PPO loop. Two things break
            # if g is fit on it. (1) Every row of the fit input is the same constant, so
            # an MSE loss constrains NOTHING about grad g -- yet Adam still moves it, by
            # more than an order of magnitude, in a direction that means nothing. (2) the
            # anchor explains none of the return yet, so residual_std would be set to the
            # FULL return std, its largest possible value. Together they put an untrained
            # covector into the credit at maximum amplitude on iteration 2 -- the exact
            # opposite of this design's floor guarantee, which is that the credit starts
            # as the scalar-GAE advantage. Skip both until psi actually varies.
            psi_is_degenerate = bool(
                fit_psi.shape[0] < 2 or float(fit_psi.std(0).max()) < 1e-6
            )
            if fit_residual.numel() > 1 and not psi_is_degenerate:
                residual_stat_count += 1
                residual_rate = max(args.sf_target_ema, 1.0 / residual_stat_count)
                residual_std = residual_std + residual_rate * (
                    fit_residual.std().clamp_min(1e-6) - residual_std
                )
            fit_targets = fit_residual / residual_std
        if psi_is_degenerate:
            readout_loss_mean = readout_grad_norm_mean = float("nan")
            readout_steps = 0
        else:
            readout_loss_mean, readout_grad_norm_mean, readout_steps = fit_latent_value_readout(
                readout,
                readout_optimizer,
                fit_psi,
                fit_targets,
        ...
        with torch.no_grad():
            # Post-fit diagnostics. readout_ev is the in-sample R^2 of g against the
            # return the ANCHOR does not explain: this is the entire claim of the design,
            # so it is also half of the kill criterion. direction_update_cosine prices how
            # far the covector moved in one iteration -- what the cross-fit is spending.
            readout_ev = explained_variance_1d(readout(fit_psi), fit_targets)
            readout_target_count = int(fit_targets.numel())
            # How large the learned correction is next to the exact anchored advantage.
            # Growing while readout_ev stays at zero is the covector injecting noise.
        ...
        refit_residual_gradient = value_gradient_covector(
            readout, refit_standardized_psi
        ).reshape(args.num_steps, args.num_envs, sf_dim)
        with torch.no_grad():
            # Rebuild the anchor in the REFIT standardizer's frame so the two covectors
            # being compared are expressed in the same units.
            refit_anchor_covector = torch.zeros_like(refit_residual_gradient)
            refit_anchor_covector[..., 0] = readout_std[0]
            trained_credit_direction = (
                refit_anchor_covector + residual_std * refit_residual_gradient
            )
            frozen_flat = frozen_credit_direction.reshape(-1, sf_dim)
            trained_flat = trained_credit_direction.reshape(-1, sf_dim)
            # These three are measured on the LEARNED half alone. The full covector
            # carries readout_std[0] in coordinate 0 -- a constant, sample-independent
            # entry at return scale -- so an uncentered cosine, resultant or norm over it
            # reads ~1 no matter what g does, i.e. is blind to the only part that moves.
            frozen_learned = residual_gradient.reshape(-1, sf_dim)
            trained_learned = refit_residual_gradient.reshape(-1, sf_dim)
            ...
            # Same statistic for the covector one iteration ahead: how much of the gain
            # is the refit, and how much was already available.
            trained_credit = (
        ...
        with torch.no_grad():
            ...
            credit_score_cosine = credit_score / (
                vector_trace_norm * frozen_flat.norm(dim=-1)
            ).clamp_min(1e-8)
        ...
        # ---- vector policy credit and value-gradient covector geometry ----------
        ...
        writer.add_scalar("readout/residual_std", residual_std.item(), global_step)
