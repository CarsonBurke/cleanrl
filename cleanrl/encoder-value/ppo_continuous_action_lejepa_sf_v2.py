# PPO + LeJEPA successor features v2: raw state joins the reward-feature basis.
# =====================================================================================
# v2 = v1 with ONE change: phi = [e, s, a, a*a, 1] instead of [e, a, a*a, 1].
#
# WHY. v1 measured reward_probe_r2 = 0.813 with reward_resid_ac_lag1 = 0.474 at 1M steps.
# R^2 alone would be survivable; a lag-1 residual autocorrelation of 0.47 is not. A white
# residual at R^2=0.81 costs ~1% EV, but a gait-phase-locked one at the same R^2 costs
# something closer to 10%, because V_err ~ sigma_eps * tau_eps while std_s(V) ~ sigma_r *
# tau_r. So w_r was carrying STRUCTURED error into every value estimate.
#
# The cause is structural, not a tuning miss. HalfCheetah's reward is x_vel - 0.1*||a||^2.
# The control cost was already exactly linear in the a*a block, so the residual is entirely
# the velocity term -- and the encoder has NO pressure to keep x_vel linearly decodable.
# It is trained by exactly two objectives, next-embedding prediction and SIGReg, neither of
# which has ever seen a reward. A whitened nonlinear map of the state is free to encode
# qvel[0] in a way no linear probe can read back, and SIGReg actively ROTATES the frame
# while doing it.
#
# The fix is not to teach the encoder about reward -- that would put a scalar-return
# gradient back into the latent and re-couple the two timescales this design separates.
# It is to notice that phi is a REWARD-FEATURE BASIS, and nothing ever required it to be
# only the learned latent. Raw (already-normalized) s makes the velocity term exactly
# linearly recoverable, so w_r . phi ~ r becomes near-exact and the residual goes white.
#
# THESIS IS UNCHANGED. phi grows from 45 to 62 dims; Lambda is still vector-valued
# occupancy, still the only bootstrapped object, still no scalar bootstrapping anywhere.
# One honest weakening: SIGReg's externally-clamped marginal now covers the e block only.
# The s block's marginal is pinned by NormalizeObservation's running stats instead -- a
# different and slower mechanism, whose drift is worth watching in sf/lambda_std_obs.
# =====================================================================================
