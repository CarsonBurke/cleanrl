# PPO + successor features with fixed-row-norm SiTU-GLU trunk branches.
# v7 keeps v3's official Kimi K3 SiTU-GLU product:
#   [4 tanh(g/4) sigmoid(g)] * [25 tanh(u/25)]
# but removes learned radial scale from the gate and up projections by using their
# differentiable per-row normalization at every forward. RMS-normalized branch
# inputs plus fixed sqrt(2)-norm projection rows remove learned projection scale
# without per-sample normalization after projection. The orthogonal v3
# initialization already has exactly this row norm, so initial behavior, parameter
# count, and the global RNG stream remain seed-paired. Hypothesis: preventing slow
# projection-norm growth keeps more SiTU units away from tanh's saturated extremes
# while retaining the activation's bounded multiplicative interaction.
#
...
class FixedRowNormLinear(nn.Linear):
    """Bias-free linear whose effective rows have a fixed sqrt(2) norm."""

    def __init__(self, in_features, out_features):
        super().__init__(in_features, out_features, bias=False)
        self.target_row_norm = np.sqrt(2.0)

    def forward(self, x):
        row_norm = self.weight.norm(dim=1, keepdim=True).clamp_min(
            torch.finfo(self.weight.dtype).eps
        )
        effective_weight = self.weight * (self.target_row_norm / row_norm)
...
class SiTUGLUBranch(nn.Module):
    ...
    def __init__(self, H):
        ...
        with torch.random.fork_rng(devices=[]):
            self.gate = layer_init(FixedRowNormLinear(H, self.hidden_dim))
            self.up = layer_init(FixedRowNormLinear(H, self.hidden_dim))
...
if __name__ == "__main__":
    ...
    if not rownorm_branches:
        raise RuntimeError("row-norm experiment constructed no SiTU-GLU branches")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            gate_raw_row_norms = torch.cat(
                [branch.gate.weight.norm(dim=1) for branch in rownorm_branches]
            )
            up_raw_row_norms = torch.cat(
                [branch.up.weight.norm(dim=1) for branch in rownorm_branches]
            )
        for projection, raw_row_norms in (
            ("gate", gate_raw_row_norms),
            ("up", up_raw_row_norms),
        ):
            writer.add_scalar(
                f"situ/{projection}_raw_row_norm_mean",
                raw_row_norms.mean().item(),
                global_step,
            )
            writer.add_scalar(
                f"situ/{projection}_raw_row_norm_min",
                raw_row_norms.min().item(),
                global_step,
            )
            writer.add_scalar(
                f"situ/{projection}_raw_row_norm_max",
                raw_row_norms.max().item(),
                global_step,
            )
