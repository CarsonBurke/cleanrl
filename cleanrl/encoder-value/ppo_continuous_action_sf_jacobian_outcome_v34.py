# =====================================================================================
# v32 -- SUCCESSOR-FEATURE ACTION JACOBIAN: a VECTOR policy-update channel.
#
# WHY THE PREFERENCE-DIRECTION LINE IS OVER (v23/v29/v30/v31 post-mortem).
# Every one of those variants tried to build a better covector w to turn the vector
# innovation E_t in R^{sf_dim} into a scalar advantage. All of them lost to v9, whose
# w_r is not learned at all -- it is the CLOSED-FORM ridge solution of w.phi == r. A
# learned direction was only ever a noisier estimate of something already solved
# analytically, so the whole family was strictly downhill:
#     v9 (closed-form w_r)                10292 @8M
#     v23_vector_preference (learned w)    9578 @8M
#     v31_anchored_minvariance             7586 @8M
#     v30_minvariance_innovation           7112 @4.6M (killed)
#     v29_reward_anchored_gradient         2540 @1.4M (killed)
#
# THE STRUCTURAL REASON, which no covector can fix. For a score-function (REINFORCE /
# PPO) estimator the gradient that reaches theta is
#       grad = E[ grad_theta log pi(a|s) * c ],   c a SCALAR.
# That is the chain rule, not a modelling choice. Whatever vector object you build, it
# reaches the policy only through its inner product with something -- i.e. as a scalar.
# "Use the embedding vectorially in the advantage" is therefore not an achievable goal;
# the advantage is a scalar by construction. To actually spend sf_dim numbers per
# timestep on the policy you must change the CHANNEL, not the covector.
#
# WHAT MAKES A VECTOR CHANNEL POSSIBLE HERE. phi is quadratic in the action by
# construction:  phi(s,a) = [e(s), s, a, a*a, 1]. So the action-conditioned successor
# feature
#       Psi(s,a) = phi(s,a) + gamma * E[psi(s') | s, a]
# is differentiable in a in CLOSED FORM for its analytic part, and we model the second
# term with a head that is quadratic in a as well (matching the feature class), so the
# whole action Jacobian
#       J(s,a) = d Psi / d a   in   R^{sf_dim x act_dim}
# is available analytically -- 62*6 = 372 numbers per timestep instead of 1, with no
# autograd pass over a batch of 32768.
#
# THE ANALYTIC PART IS EXACT AND FREE. HalfCheetah's reward is
# forward_velocity - 0.1*||a||^2, and the -0.1*||a||^2 term is EXACTLY representable in
# phi's a*a block, so w_r recovers it and d(w_r.phi)/da delivers the true control-cost
# gradient with ZERO variance. Standard PPO only ever learns that through noisy returns.
#
# CHANNEL A -- PER-DIMENSION CREDIT (jac_perdim_omega).
# PPO multiplies all act_dim scores grad log pi_j by ONE scalar A: it cannot say which
# COMPONENT of the action vector was the good part. Psi is exactly quadratic in a, so
# the action-dependent part of the advantage decomposes EXACTLY, with no Taylor
# remainder, as
#       w_r . (Psi(s,a) - Psi(s,mu)) = sum_j [ g_j dev_j + Q_j dev_j^2 ],  dev = a - mu,
# with g = J^T w_r at a = mu. The linear term is zero-mean per coordinate because mu is
# the mean of a FACTORIZED distribution (for actor_dist=beta the map to action space is
# affine, so to_action(E[z]) == E[a] exactly; the gaussian path's tanh makes it
# first-order); the quadratic term is centred per coordinate to restore that baseline.
# Each Ahat_j is thus a valid per-coordinate advantage, and
#       E[ grad_{mu_j} log pi * Ahat_j ] = g_j
# is the SAME expected update as the scalar advantage with the other act_dim-1
# coordinates' sampling noise removed. That is a variance reduction, and variance is the
# binding constraint at these sample sizes.
#
# It is injected as a ZERO-SUM-ACROSS-DIMENSIONS redistribution,
#       A_j = A + omega * calib * zscore(Ahat - mean_j Ahat),
# so mean_j A_j == A exactly: the aggregate credit is v9's and only its ATTRIBUTION
# across coordinates moves. Note A_j = A, NOT A/act_dim -- with true per-dimension
# ratios d ratio_j/d theta = ratio_j grad log pi_j, so dividing would scale the entire
# policy gradient by 1/act_dim while leaving its DIRECTION identical, i.e. silently.
#
# WHY IT IS SELF-LIMITING. A bare z-score would inject a fixed-magnitude shift however
# uninformative g is. `calib` is the measured correlation between the attribution and
# the realized advantage, floored at 3 null s.d. (1/sqrt(N) ~ 0.0055 at N = 32768), so
# an uninformative Jacobian contributes nothing. This is the shrinkage a least-squares
# predictor applies, and it makes the falsifier ACTUATING rather than merely observed.
#
# THE OBVIOUS OBJECTION, and the answer. Before the head is identified, g is its analytic
# part alone, and since HalfCheetah's reward has no linear-in-a term that is g ~ -0.2*mu:
# a pure control-cost signal already present in the reward, hence already in A. But it is
# NOT already in the ATTRIBUTION. PPO's scalar knows "this action was costly"; it does
# not know WHICH coordinate was the costly one except through sampling correlation. That
# per-coordinate fact is exactly what the zero-sum redistribution carries, it is exact,
# and it costs zero variance. The forward-velocity half of the reward reaches g only
# through gamma * d E[psi(s')|s,a]/da, which is the part jac/asf_delta_ev prices.
#
# The PPO clip becomes per-coordinate: per-dimension credit only reaches the policy
# through per-dimension ratios, since d ratio_j/d theta = ratio_j grad log pi_j. With the
# JOINT ratio broadcast instead, the gradient collapses to (sum_j A_j) r sum_k grad log
# pi_k, and because the shift is zero-sum the redistribution cancels out entirely. So the
# trust region has to become per-coordinate too, and its bounds are the one free choice.
#
# ==== V32 POST-MORTEM: WHAT THE BOUND MUST MATCH =====================================
# v32 set the per-coordinate bounds to the act_dim-th root of v9's, so that the PRODUCT
# of clamped ratios would land in v9's [0.8, 1.28]. That confuses bounding a product with
# bounding each factor, and it is far TIGHTER, not matched. Log-ratios add across
# coordinates, so sd(log r_j) = sd(log r)/sqrt(n); at v9's approx_kl ~ 0.004 the typical
# |log r_j| is ~0.037, sitting right on log(1.28)/6 = 0.041. The clip bound was at the
# noise floor of the thing it was clipping.
# Measured consequence, HalfCheetah-v4 seed 1, killed at 0.9M:
#   losses/entropy   -1.01   == H(Beta) at initialization, to 2 decimals: the actor
#                               never moved, across 900k steps and ~9000 minibatch steps
#   return @848k       859   vs v9's ~3000
#   losses/clipfrac  0.020   -- and this is why it was invisible: clipfrac and approx_kl
#                               are computed on the JOINT ratio and looked completely
#                               normal while every second coordinate was being clipped.
# v33 matches the bound on CLIP FREQUENCY instead, the operationally meaningful quantity:
# the sqrt(n) root, giving [0.913, 1.106]. jac/clipfrac_dim now reports the realized
# per-coordinate rate and must land near v9's losses/clipfrac ~ 0.023; a large gap means
# the match failed again. The joint ratio is no longer bounded by v9's band in exchange,
# so target_kl = 0.03 carries the aggregate leash. jac_clip_match selects the mode, and
# "linear" reproduces v32's failure exactly for anyone who wants to see it.
#
# ==== V33 POST-MORTEM: delta_ev WAS THE WRONG DIAGNOSTIC ==============================
# v33 turned the learned half of the Jacobian OFF, on the strength of v32's
# jac/asf_delta_ev ~ 0.001 -- the held-out incremental EV of the action-dependent terms
# when predicting psi(s'). That was the wrong target. psi(s') is 62-dimensional and
# almost entirely explained by c(s), so an action-dependent component can be worth
# essentially nothing there and still carry real information about the ADVANTAGE, which
# is a residual quantity. Running analytic-only settled it:
#   v32 (learned J on):    jac/attrib_corr  0.169 .. 0.215
#   v33 (analytic only):   jac/attrib_corr -0.003          <- indistinguishable from null
# So the informative half is the LEARNED one, and the analytic control-cost gradient --
# exact, zero-variance, and the part this design was proudest of -- predicts the realized
# advantage no better than chance on its own. That is not surprising in hindsight:
# HalfCheetah's advantage is dominated by forward-velocity outcomes, and the analytic
# term knows only the -0.1||a||^2 control cost.
# jac/attrib_corr is therefore the diagnostic that decides this question; asf_delta_ev is
# kept only as a secondary read on the head's fit. jac_use_learned defaults back to True.
#
# The shrinkage did its job through all of this: with the analytic-only Jacobian
# uninformative, jac/calibration went to exactly 0 and channel A disabled itself rather
# than injecting noise -- which is also why the v33 omega=0.25 arm was an exact duplicate
# of its own omega=0 control and was cancelled before it burned a slot.
#
# CHANNEL B -- OUTCOME-SPACE IMPROVEMENT (jac_improve_beta, default OFF).
#       da = (J_emb^T J_emb / emb_dim + ridge*I)^{-1} J^T w_r
# Move the action so the PREDICTED FUTURE FEATURE VECTOR travels as far as possible
# along the reward direction, measured in the metric the EMBEDDING defines. All 32
# embedding rows set the geometry; directions in which the action does not move the
# future get no step. Applied as a regression of mu(s) onto a trust-region-capped
# improved action, computed once per rollout and replayed from the buffer -- NOT a
# pathwise gradient through the model, which would be unbounded off the data manifold.
#
# CROSS-FIT. The action-successor head is fit AFTER the PPO update and consumed on the
# NEXT rollout, so it can never memorize the action noise of the batch it is scoring.
# It is a TOP-LEVEL module, not an Agent submodule, for the reason spelled out at the
# LeJepaSSL construction site: joining agent.parameters() would silently put it in the
# PPO optimizer and outside both grad-clip groups.
#
# SAFE FLOOR. The head is zero-initialised, so on iteration 1 J is purely analytic and
# the learned part contributes nothing; and omega is forced to 0 until w_r has been
# solved once (w_r == 0 would make g identically zero). The run therefore starts as
# exactly v9 and departs from it only as the head earns it.
#
# FALSIFIERS, in the order they bite.
#   jac/asf_delta_ev -- HELD-OUT explained variance of the action-dependent terms over a
#     const(s)-only prediction, measured on the next rollout before that rollout's fit.
#     The full-prediction EV is useless here: psi(s') is almost entirely explained by
#     c(s), so the head scores high with its Jacobian at exactly zero. If delta_ev sits
#     at zero, the LEARNED half of J is empty and only the analytic control-cost
#     attribution is live -- still a real signal, but a much smaller claim.
#   jac/attrib_corr -- corr(sum_j Ahat_j, scalar GAE). Null s.d. is 1/sqrt(32768) =
#     0.0055, so "zero" means below ~0.017, not below 0.1. This directly drives
#     jac/calibration, so a dead attribution silently disables channel A rather than
#     poisoning it; jac/shift_std then goes to zero and the run degenerates to the
#     per-coordinate-clip arm.
#   jac/sign_flip_frac -- how often the redistribution reverses a coordinate's credit
#     sign. Sustained above ~0.2 means omega is too high for the evidence.
# Kill on delta_ev == 0 AND attrib_corr below 0.02 with no return advantage; that is the
# configuration in which the whole Jacobian channel is doing nothing but the clip change.
#
# Naming: `z`/`latent_zs` is the Beta actor's native action sample (inherited). The
# encoder latent is `e`/`emb` throughout. Everything below the Jacobian machinery --
# LeJEPA SSL, the SF critic, per-dimension lambda, the closed-form reward probe, the
# dual-backward decoupled clipping -- is v9 unchanged.
import os
import random
import time
...
class Args:
    ...
    jac_perdim_omega: float = 0.25   # strength of the zero-sum per-dimension credit
