# Ordinal vector-moment policy transport from a LeJEPA latent-successor critic.
#
# The critic's complete 32D TD(lambda) innovation stays vector-valued. Equal-horizon
# cross-environment return comparisons orient robust higher-minus-lower innovation
# moments, while exact on-policy likelihood ratios move the actor's full innovation
# moment toward them. Symmetric environment-fold cross-fitting keeps each target
# independent of the actions whose ratios realize it. This directly uses the identifiable
# score-times-innovation moment:
# there is no scalar value/advantage, learned preference covector, action-effect model,
# RankGauss, or transfer of another state's action effect. Exact Beta KL rollback bounds
# every update; split-half score-moment agreement and held-out vector improvement are the
# decisive falsifiers. Procrustes transport pins the LeJEPA frame without an EMA teacher.
#
# Base: v23 vector preference. The learned covector and PPO scalarization are removed.
#
...
class Args:
    ...
    # --- Ordinal full-vector policy transport ---------------------------------------
    moment_horizon_tolerance: int = 32
    moment_target_norm: float = 1.0
    moment_kl_coef: float = 1.0
    moment_target_kl: float = 0.03
    moment_kl_tolerance: float = 0.25
    # Every Nth time group is held out from both actor and shared-critic updates. It
    # measures whether one update transports the same ordinal vector moment out of
    # sample rather than merely fitting a rollout partition.
    moment_holdout_stride: int = 4
...
@torch.no_grad()
def ordinal_vector_targets(vector_traces, returns, complete, horizons, tolerance):
    """Robust full-vector higher-minus-lower targets for each rollout timestep."""
    envs = vector_traces.shape[1]
    left, right = torch.triu_indices(envs, envs, offset=1, device=vector_traces.device)
    pair_valid = (
        complete[:, left]
        & complete[:, right]
        & ((horizons[:, left] - horizons[:, right]).abs() <= tolerance)
        & returns[:, left].ne(returns[:, right])
    )
    difference = vector_traces[:, left] - vector_traces[:, right]
    orientation = (returns[:, left] - returns[:, right]).sign().unsqueeze(-1)
    oriented = F.normalize(orientation * difference, dim=-1, eps=1e-6)
    weighted = oriented * pair_valid.unsqueeze(-1)
    count = pair_valid.sum(-1)
    mean = weighted.sum(1) / count.clamp_min(1).unsqueeze(-1)
    direction = F.normalize(mean, dim=-1, eps=1e-6)
    agreement = (oriented * direction.unsqueeze(1)).sum(-1)
    agreement = (agreement * pair_valid).sum(-1) / count.clamp_min(1)
    usable = count.gt(0) & mean.norm(dim=-1).gt(1e-6)
    return direction.detach(), usable, count, agreement


def grouped_vector_moment(ratio, vector_traces, baseline, scale):
    """Likelihood-ratio shift with an independent detached vector baseline."""
    centered = vector_traces.detach() - baseline.detach().unsqueeze(1)
    return ((ratio - 1.0).unsqueeze(-1) * centered).mean(1) / scale
...
@torch.no_grad()
def beta_raw_score(raw_parameters, native_actions, action_dim):
    raw_alpha, raw_beta = raw_parameters.split(action_dim, dim=-1)
...
class Agent(nn.Module):
    ...
    def _actor_dist(self, actor_feat, raw_policy=None):
    ...
    def policy_parameters(self, x):
        ...
        return self.raw_policy_parameters(actor_feat)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        raw_policy = self.raw_policy_parameters(actor_feat)
        dist, to_action, log_det_fn = self._actor_dist(actor_feat, raw_policy)
        if z is None:
            z = dist.sample()
...
if __name__ == "__main__":
    ...
    if args.num_envs < 4:
        raise ValueError("cross-fitted ordinal vector moments need at least four environments")
    if args.actor_dist != "beta":
        raise ValueError("exact vector-moment transport currently requires the Beta actor")
    if args.moment_target_norm <= 0 or args.moment_target_kl <= 0:
        raise ValueError("moment target norm and KL must be positive")
    if args.moment_holdout_stride < 2:
        raise ValueError("moment_holdout_stride must be at least two")
    if args.minibatch_size % args.num_envs:
        raise ValueError("each minibatch must contain complete cross-environment time groups")
    if not args.separate_grad_clip:
        raise ValueError("vector-moment transport requires separate_grad_clip")
    ...
    if auto_alpha:
        raise ValueError(
            ...
            "vector-moment design; use the Beta actor or disable auto_entropy"
        )
    if auto_alpha:
        act_dim = int(np.prod(envs.single_action_space.shape))
    ...
    # Consume exactly v23's LeJEPA initialization stream. There is no preference or
    # action-effect network: the behavior likelihood ratio is the transport model.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, _, _ = agent.get_action_and_value(next_obs)
            actions[step] = action
            latent_zs[step] = z
        ...
        with torch.no_grad():
            ...
            returns = mc_ret
            # Scalar max-entropy credit is intentionally unsupported: the policy consumes
            # only the complete ordinal vector moment below.
            if auto_alpha:
            ...
            # Symmetric environment-fold cross-fitting: each fold's actor moment moves
            # toward an ordinal direction estimated only from the other fold. This keeps
            # the target independent of the sampled actions whose likelihood ratios are
            # optimized, while using every environment for both roles.
            env_split = args.num_envs // 2
            direction_a, valid_a, pair_count_a, agreement_a = ordinal_vector_targets(
                vector_policy_trace[:, :env_split],
                mc_ret[:, :env_split],
                mc_complete[:, :env_split],
                mc_avail[:, :env_split],
                args.moment_horizon_tolerance,
            )
            direction_b, valid_b, pair_count_b, agreement_b = ordinal_vector_targets(
                vector_policy_trace[:, env_split:],
                mc_ret[:, env_split:],
                mc_complete[:, env_split:],
                mc_avail[:, env_split:],
            ...
            moment_group_valid = valid_a & valid_b
            moment_target_a = args.moment_target_norm * direction_b
            moment_target_b = args.moment_target_norm * direction_a
            moment_pair_count = pair_count_a + pair_count_b
            pair_agreement = 0.5 * (agreement_a + agreement_b)
            holdout_time = (
                torch.arange(args.num_steps, device=device)
                % args.moment_holdout_stride
            ).eq(0)
            moment_train_valid = moment_group_valid & ~holdout_time
            moment_holdout_valid = moment_group_valid & holdout_time
            trace_mean_a = vector_policy_trace[:, :env_split].mean(1)
            trace_mean_b = vector_policy_trace[:, env_split:].mean(1)
            cross_centered_a = vector_policy_trace[:, :env_split] - trace_mean_b.unsqueeze(1)
            cross_centered_b = vector_policy_trace[:, env_split:] - trace_mean_a.unsqueeze(1)
            moment_scale = torch.cat(
                [cross_centered_a, cross_centered_b], dim=1
            ).square().sum(-1).mean().sqrt().clamp_min(1e-6)

        b_obs = obs.reshape((-1,) + envs.single_observation_space.shape)
        b_logprobs = logprobs.reshape(-1)
        b_actions = actions.reshape((-1,) + envs.single_action_space.shape)
        b_latent_zs = latent_zs.reshape((-1,) + envs.single_action_space.shape)
        b_vector_trace = vector_policy_trace.reshape(-1, sf_dim)
        ...
        with torch.no_grad():
            old_raw_chunks = []
            for raw_start in range(0, args.batch_size, 4096):
                old_raw_chunks.append(agent.policy_parameters(b_obs[raw_start : raw_start + 4096]))
            b_old_raw = torch.cat(old_raw_chunks)

        moment_ready = iteration > 1
        active_critic_params = (
            critic_params if moment_ready else list(agent.critic_head.parameters())
        )
        ...
        times_per_minibatch = args.minibatch_size // args.num_envs
        for epoch in range(args.update_epochs):
            ...
            epoch_grad_steps = 0
            time_permutation = np.random.permutation(args.num_steps)
            for start in range(0, args.num_steps, times_per_minibatch):
                mb_times_np = time_permutation[start : start + times_per_minibatch]
                mb_times = torch.as_tensor(mb_times_np, device=device)
                mb_inds = (
                    mb_times.unsqueeze(1) * args.num_envs
                    + torch.arange(args.num_envs, device=device).unsqueeze(0)
                ).reshape(-1)

                _, _, newlogprob, entropy, value_sf, current_raw = agent.get_action_and_value(
                ...
                ratio_by_time = ratio.view(-1, args.num_envs)
                mb_moment_a = grouped_vector_moment(
                    ratio_by_time[:, :env_split],
                    vector_policy_trace[mb_times, :env_split],
                    trace_mean_b[mb_times],
                    moment_scale,
                )
                mb_moment_b = grouped_vector_moment(
                    ratio_by_time[:, env_split:],
                    vector_policy_trace[mb_times, env_split:],
                    trace_mean_a[mb_times],
                    moment_scale,
                )
                mb_valid = moment_train_valid[mb_times]
                if moment_ready and mb_valid.any():
                    moment_error = 0.25 * (
                        (mb_moment_a - moment_target_a[mb_times]).square().sum(-1)
                        + (mb_moment_b - moment_target_b[mb_times]).square().sum(-1)
                    )
                    improvement_loss = moment_error[mb_valid].mean()
                    train_time = ~holdout_time[mb_times]
                    train_kl = current_exact_kl.view(-1, args.num_envs).mean(-1)
                    actor_loss = (
                        improvement_loss
                        + args.moment_kl_coef * train_kl[train_time].mean()
                    )
                else:
                ...
                # The shared critic trunk also moves the policy. Exclude held-out time
                # groups from its loss, otherwise the purported actor holdout would leak
                # through critic gradients even though it is absent from moment_loss.
                value_mask = value_mask * (
                    ~holdout_time[mb_times]
                ).repeat_interleave(args.num_envs).unsqueeze(-1)
                v_loss = (
            ...
            with torch.no_grad():
                ...
                for guard_start in range(0, args.batch_size, 4096):
                    epoch_raw_chunks.append(agent.policy_parameters(b_obs[guard_start : guard_start + 4096]))
                epoch_raw = torch.cat(epoch_raw_chunks)
                epoch_exact_kl = mean_beta_kl(b_old_raw, epoch_raw, act_dim_sf)
            if moment_ready and float(epoch_exact_kl) > (
                args.moment_target_kl * (1.0 + args.moment_kl_tolerance)
            ):
                with torch.no_grad():
                    for parameter, saved in zip(agent.parameters(), epoch_parameters, strict=True):
        ...
        with torch.no_grad():
            final_raw_chunks = []
            final_logprob_chunks = []
            for metric_start in range(0, args.batch_size, 4096):
                metric_obs = b_obs[metric_start : metric_start + 4096]
                metric_z = b_latent_zs[metric_start : metric_start + 4096]
                final_raw = agent.policy_parameters(metric_obs)
                final_raw_chunks.append(final_raw)
                final_logprob_chunks.append(
                    beta_from_raw(final_raw, act_dim_sf).log_prob(metric_z).sum(-1)
                )
            final_raw = torch.cat(final_raw_chunks)
            final_logratio_flat = torch.cat(final_logprob_chunks) - b_logprobs
            final_ratio = final_logratio_flat.exp().view(
                args.num_steps, args.num_envs
            )
            achieved_moment_a = grouped_vector_moment(
                final_ratio[:, :env_split],
                vector_policy_trace[:, :env_split],
                trace_mean_b,
                moment_scale,
            )
            achieved_moment_b = grouped_vector_moment(
                final_ratio[:, env_split:],
                vector_policy_trace[:, env_split:],
                trace_mean_a,
                moment_scale,
            )
            old_moment_error = 0.25 * (
                moment_target_a.square().sum(-1) + moment_target_b.square().sum(-1)
            )
            new_moment_error = 0.25 * (
                (achieved_moment_a - moment_target_a).square().sum(-1)
                + (achieved_moment_b - moment_target_b).square().sum(-1)
            )

            def masked_mean(values, valid):
                return float(values[valid].mean()) if valid.any() else float("nan")

            train_error_reduction = masked_mean(
                old_moment_error - new_moment_error, moment_train_valid
            )
            heldout_error_reduction = masked_mean(
                old_moment_error - new_moment_error, moment_holdout_valid
            )
            train_moment_cosine = masked_mean(
                0.5 * (
                    F.cosine_similarity(achieved_moment_a, moment_target_a, dim=-1, eps=1e-8)
                    + F.cosine_similarity(achieved_moment_b, moment_target_b, dim=-1, eps=1e-8)
                ),
                moment_train_valid,
            )
            heldout_moment_cosine = masked_mean(
                0.5 * (
                    F.cosine_similarity(achieved_moment_a, moment_target_a, dim=-1, eps=1e-8)
                    + F.cosine_similarity(achieved_moment_b, moment_target_b, dim=-1, eps=1e-8)
                ),
                moment_holdout_valid,
            )
            exact_kl = mean_beta_kl(b_old_raw, final_raw, act_dim_sf)
            behavior_logprob = beta_from_raw(
                b_old_raw, act_dim_sf
            ).log_prob(b_latent_zs).sum(-1)
            behavior_logprob_parity_max = float(
                (behavior_logprob - b_logprobs).abs().max()
            )
            sample_approx_kl = float(
                ((final_logratio_flat.exp() - 1.0) - final_logratio_flat).mean()
            )
            ratio_flat = final_ratio.reshape(-1)
            ratio_ess = float(
                ratio_flat.sum().square()
                / ratio_flat.square().sum().clamp_min(1e-8)
                / ratio_flat.numel()
            )
            behavior_score = beta_raw_score(
                b_old_raw, b_latent_zs, act_dim_sf
            ).view(args.num_steps, args.num_envs, 2 * act_dim_sf)
            normalized_trace_a = cross_centered_a / moment_scale
            normalized_trace_b = cross_centered_b / moment_scale
            response_a = torch.einsum(
                "ted,tek->tdk", normalized_trace_a, behavior_score[:, :env_split]
            ) / env_split
            response_b = torch.einsum(
                "ted,tek->tdk", normalized_trace_b, behavior_score[:, env_split:]
            ) / (args.num_envs - env_split)
            preferred_score_a = torch.einsum("tdk,td->tk", response_a, direction_b)
            preferred_score_b = torch.einsum("tdk,td->tk", response_b, direction_a)
            preferred_raw_score_cosine = masked_mean(
                F.cosine_similarity(preferred_score_a, preferred_score_b, dim=-1, eps=1e-8),
                moment_group_valid,
            )
            response_frobenius_cosine = masked_mean(
                F.cosine_similarity(
                    response_a.flatten(1), response_b.flatten(1), dim=-1, eps=1e-8
                ),
                moment_group_valid,
            )
            target_crossfold_cosine = masked_mean(
                F.cosine_similarity(direction_a, direction_b, dim=-1, eps=1e-8),
                moment_group_valid,
            )
            enough_valid_targets = int(moment_group_valid.sum()) > 1
            valid_target_rank = (
                effective_rank(
                    torch.cat(
                        [moment_target_a[moment_group_valid], moment_target_b[moment_group_valid]]
                    )
                )
                if enough_valid_targets
                else float("nan")
            )
            valid_achieved_rank = (
                effective_rank(
                    torch.cat(
                        [achieved_moment_a[moment_group_valid], achieved_moment_b[moment_group_valid]]
                    )
                )
                if enough_valid_targets
                else float("nan")
            )
            achieved_norm = masked_mean(
                0.5 * (
                    achieved_moment_a.norm(dim=-1) + achieved_moment_b.norm(dim=-1)
                ),
                moment_group_valid,
            )
            actor_grad_norm_mean = accepted_actor_grad_norm_sum / max(accepted_grad_steps, 1)
            critic_grad_norm_mean = accepted_critic_grad_norm_sum / max(accepted_grad_steps, 1)
            final_policy_loss = masked_mean(new_moment_error, moment_train_valid) + (
                args.moment_kl_coef * float(exact_kl)
            )
        ...
        writer.add_scalar("losses/policy_loss", final_policy_loss, global_step)
        ...
        writer.add_scalar("losses/approx_kl", sample_approx_kl, global_step)
        writer.add_scalar("losses/exact_kl", float(exact_kl), global_step)
        writer.add_scalar("losses/actor_grad_norm", actor_grad_norm_mean, global_step)
        writer.add_scalar("losses/critic_grad_norm", critic_grad_norm_mean, global_step)
        ...
        # ---- ordinal full-vector moment transport -------------------------------
        ...
        writer.add_scalar("moment/valid_groups", moment_group_valid.sum().item(), global_step)
        writer.add_scalar("moment/pair_count_mean", moment_pair_count[moment_group_valid].float().mean().item() if moment_group_valid.any() else 0.0, global_step)
        writer.add_scalar("moment/pair_direction_agreement", masked_mean(pair_agreement, moment_group_valid), global_step)
        writer.add_scalar("moment/train_error_reduction", train_error_reduction, global_step)
        writer.add_scalar("moment/heldout_error_reduction", heldout_error_reduction, global_step)
        writer.add_scalar("moment/train_cosine", train_moment_cosine, global_step)
        writer.add_scalar("moment/heldout_cosine", heldout_moment_cosine, global_step)
        writer.add_scalar("moment/achieved_norm", achieved_norm, global_step)
        writer.add_scalar("moment/target_effective_rank", valid_target_rank, global_step)
        writer.add_scalar("moment/achieved_effective_rank", valid_achieved_rank, global_step)
        writer.add_scalar("moment/target_crossfold_cosine", target_crossfold_cosine, global_step)
        writer.add_scalar("moment/crossfit_preferred_raw_score_cosine", preferred_raw_score_cosine, global_step)
        writer.add_scalar("moment/crossfit_response_frobenius_cosine", response_frobenius_cosine, global_step)
        writer.add_scalar("moment/likelihood_ratio_ess", ratio_ess, global_step)
        writer.add_scalar("moment/behavior_logprob_parity_max", behavior_logprob_parity_max, global_step)
        writer.add_scalar("moment/actor_epochs_accepted", actor_epochs_accepted, global_step)
        writer.add_scalar("moment/kl_guard_rejections", kl_guard_rejections, global_step)
