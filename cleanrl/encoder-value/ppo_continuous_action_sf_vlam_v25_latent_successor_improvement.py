# Latent successor policy improvement with a LeJEPA vector critic.
#
# A detached action-conditioned bridge predicts the next rich state value from the
# current state embedding and action. Local return ordering selects the better observed
# successor among nearby states, and the Beta actor moves its predicted successor toward
# that complete D-dimensional target. Return is never decoded or used as a magnitude;
# there is no scalar advantage, policy-score target, RankGauss, or PPO ratio clipping.
# Exact Beta KL and epoch rollback provide the trust region. The bridge is trained after
# the actor for use one rollout later, and LeJEPA's frame remains Procrustes-aligned.
#
# Base: v23's 32D value representation and v9's RMSNorm -> Linear ->
# LeakyReLU(0.5)^2 IterThink trunk. Iteration one updates only the critic head and bridge.
import copy
import os
import random
import time
...
class Args:
    ...
    # --- Cross-fit latent successor improvement -------------------------------------
    bridge_hidden: int = 256
    bridge_lr: float = 3e-4
    bridge_epochs: int = 10
    bridge_time_batch: int = 256
    bridge_grad_clip: float = 1.0
    successor_horizon_tolerance: int = 0
...
class LatentSuccessorBridge(nn.Module):
    """Action-centered local linearization of the next rich state value."""

    def __init__(self, embedding_dim, action_dim, hidden):
        super().__init__()
        ...
        self.control_head = layer_init(
            nn.Linear(hidden, embedding_dim * action_dim), std=0.01
        )

    def components(self, embeddings):
        embeddings = embeddings.detach()
        features = self.body(embeddings)
        baseline = self.baseline_head(features)
        control = self.control_head(features).view(
            *embeddings.shape[:-1], self.embedding_dim, self.action_dim
        )
        return baseline, control

    def control_delta(self, embeddings, action_delta):
        _, control = self.components(embeddings)
        return torch.einsum("...da,...a->...d", control, action_delta)

    def forward(self, embeddings, actions, behavior_means):
        baseline, control = self.components(embeddings)
        return baseline + torch.einsum(
            "...da,...a->...d", control, actions - behavior_means.detach()
        )
...
def choose_better_successor_displacements(next_values, returns, valid, partner_env):
    """Select loser-only rich-value displacements using ordinal local comparisons."""
    partner_returns = returns.gather(1, partner_env)
    partner_valid = valid.gather(1, partner_env)
    loser = valid & partner_valid & (partner_returns > returns)
    partner_next = next_values.gather(
        1, partner_env.unsqueeze(-1).expand_as(next_values)
    )
    return (partner_next - next_values).detach(), loser


def latent_successor_error(prediction, target):
    return F.smooth_l1_loss(prediction, target.detach(), reduction="none").mean(-1)


def bridge_loss(bridge, embeddings, actions, behavior_means, next_values, valid):
    prediction = bridge(embeddings, actions.detach(), behavior_means)
    if not valid.any():
        return prediction.sum() * 0.0, prediction
    return latent_successor_error(prediction, next_values)[valid].mean(), prediction


@torch.no_grad()
def bridge_metrics(bridge, prediction, embeddings, next_values, valid):
    if not valid.any():
        return (float("nan"),) * 5
    baseline, _ = bridge.components(embeddings)
    action_effect = prediction - baseline
    target_residual = next_values - baseline
    full_error = latent_successor_error(prediction, next_values)
    baseline_error = latent_successor_error(baseline, next_values)
...
if __name__ == "__main__":
    ...
    if args.num_envs < 2:
        raise ValueError("latent successor improvement needs at least two parallel environments")
    if args.actor_dist != "beta":
        raise ValueError("latent successor improvement currently requires the Beta actor")
    if args.auto_entropy or args.ent_coef != 0.0:
        raise ValueError("latent successor improvement has no scalar entropy-value objective")
    if args.latent_actor_target_kl <= 0:
        raise ValueError("latent_actor_target_kl must be positive")
    if not args.separate_grad_clip:
        raise ValueError("latent-successor warm-up requires separate_grad_clip")
    ...
    # Consume exactly v13's LeJEPA initialization stream, then isolate only the bridge
    # so actor sampling remains paired with the strongest control.
    ssl = LeJepaSSL(obs_dim, act_dim_sf, args).to(device)
    with torch.random.fork_rng(devices=[]):
        bridge = LatentSuccessorBridge(
            args.emb_dim, act_dim_sf, args.bridge_hidden
        ).to(device)
    ...
    bridge_optimizer = optim.Adam(
        bridge.parameters(), lr=args.bridge_lr, eps=1e-5
    )
    bridge_generator = torch.Generator(device=device)
    bridge_generator.manual_seed(args.seed + 17021)
    ...
    if args.compile:
        ...
        bridge.body = CompiledModule(
            bridge.body, mode=args.compile_mode, cudagraphs=False
        )
        bridge.baseline_head = CompiledModule(
            bridge.baseline_head, mode=args.compile_mode, cudagraphs=False
        )
        bridge.control_head = CompiledModule(
            bridge.control_head, mode=args.compile_mode, cudagraphs=False
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            bridge_optimizer.param_groups[0]["lr"] = frac * args.bridge_lr
        ...
        with torch.no_grad():
            ...
            # Terminal/reset-crossing transitions are excluded: their final observations
            # are not ordinary rollout states on which the value critic was trained.
            successor_candidate = mc_complete & transition_boundaries.eq(0)
            successor_partner, has_successor_partner, neighbor_distance = (
                nearest_cross_env_pairs(
                    emb_buf,
                    mc_avail,
                    successor_candidate,
                    args.successor_horizon_tolerance,
                )
            )
            successor_sample_valid = successor_candidate & has_successor_partner
            desired_value_displacement, loser_mask = choose_better_successor_displacements(
                psi_next,
                returns,
                successor_sample_valid,
                successor_partner,
            )
            behavior_mean_actions = beta_mean_action(
                policy_raws, agent.action_low, agent.action_high
            )
            bridge_holdout_time = (
                (torch.arange(args.num_steps, device=device) + iteration) % 5 == 0
            )
            bridge_holdout_valid = transition_boundaries.eq(0) & bridge_holdout_time.unsqueeze(1)
            preactor_bridge_prediction = bridge(
                emb_buf, actions, behavior_mean_actions
            )
            (
                preactor_bridge_error,
                preactor_bridge_baseline_error,
                preactor_bridge_action_gain,
                preactor_bridge_cosine,
                preactor_bridge_effect_fraction,
            ) = bridge_metrics(
                bridge,
                preactor_bridge_prediction,
                emb_buf,
                psi_next,
                bridge_holdout_valid,
            )

        b_obs = obs.reshape((-1,) + envs.single_observation_space.shape)
        b_latent_zs = latent_zs.reshape((-1,) + envs.single_action_space.shape)
        b_embeddings = emb_buf.reshape(-1, args.emb_dim)
        b_desired_displacement = desired_value_displacement.reshape(-1, args.emb_dim)
        b_loser_mask = loser_mask.reshape(-1)
        b_behavior_means = behavior_mean_actions.reshape(-1, act_dim_sf)
        ...
        # Before a learned bridge exists, update only the zero-initialized vector head.
        # Critic gradients into the shared trunk would otherwise change the actor policy
        # even though latent successor improvement is intentionally off during warm-up.
        active_critic_params = (
            critic_params if bridge_ready else list(agent.critic_head.parameters())
        )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if bridge_ready:
                    current_mean_action = beta_mean_action(
                        current_raw, agent.action_low, agent.action_high
                    )
                    predicted_displacement = bridge.control_delta(
                        b_embeddings[mb_inds],
                        current_mean_action - b_behavior_means[mb_inds],
                    )
                    successor_error = latent_successor_error(
                        predicted_displacement, b_desired_displacement[mb_inds]
                    )
                    valid_actor = b_loser_mask[mb_inds]
                    improvement_loss = (
                        successor_error[valid_actor].mean()
                        if valid_actor.any()
        ...
        with torch.no_grad():
            ...
            frozen_predicted_displacement = bridge.control_delta(
                b_embeddings, final_mean_actions - b_behavior_means
            )
            old_successor_error = latent_successor_error(
                torch.zeros_like(b_desired_displacement), b_desired_displacement
            )
            new_successor_error = latent_successor_error(
                frozen_predicted_displacement, b_desired_displacement
            )
            if b_loser_mask.any():
                actor_old_error = float(old_successor_error[b_loser_mask].mean())
                actor_new_error = float(new_successor_error[b_loser_mask].mean())
                actor_predicted_improvement = actor_old_error - actor_new_error
                desired_displacement_norm = float(
                    b_desired_displacement[b_loser_mask].norm(dim=-1).mean()
                )
            else:
                actor_old_error = actor_new_error = actor_predicted_improvement = float("nan")
                desired_displacement_norm = float("nan")
            actor_mean_shift = float(
                (final_mean_actions - b_behavior_means).norm(dim=-1).mean()
            )
            ...
            full_improvement_loss = (
                actor_new_error if bridge_ready and b_loser_mask.any() else 0.0
            )
            full_policy_loss = (
                full_improvement_loss
            ...
            # Can the bridge's action tangent actually span the observed improvement
            # direction? This is measured on the exact frozen bridge consumed by the
            # actor, before the encoder and bridge are updated below. The least-squares
            # projection is diagnostic only and never supplies an actor target.
            loser_indices = b_loser_mask.nonzero(as_tuple=False).flatten()[:1024]
            if loser_indices.numel():
                _, actor_control = bridge.components(b_embeddings[loser_indices])
                actor_targets = b_desired_displacement[loser_indices]
                control_solution = torch.linalg.lstsq(
                    actor_control.float(), actor_targets.float().unsqueeze(-1)
                ).solution
                controllable_target = torch.matmul(
                    actor_control.float(), control_solution
                ).squeeze(-1)
                ...
                control_singular_values = torch.linalg.svdvals(actor_control.float())
                control_spectrum = control_singular_values / control_singular_values.sum(
                    -1, keepdim=True
                ).clamp_min(1e-12)
                actor_control_effective_rank = float(
                    torch.exp(
                        -(
                            control_spectrum
                            * (control_spectrum + 1e-12).log()
                        ).sum(-1)
                    ).mean()
                )
            else:
                actor_controllable_fraction = float("nan")
                actor_control_effective_rank = float("nan")
        ...
        # ---- fit the next rollout's bridge in the post-update aligned frame ----------
        # The actor consumed the bridge as it existed before this rollout. Recompute both
        # state inputs and critic values after the agent/SSL updates, then fit for k+1.
        with torch.no_grad():
            bridge_embeddings = encode_target(obs.reshape(flat_obs_shape)).reshape(
                args.num_steps, args.num_envs, args.emb_dim
            )
            bridge_next_values = psi_raw(
                agent.get_value(next_obses.reshape(flat_obs_shape))[:, 0]
            ).reshape(args.num_steps, args.num_envs, sf_dim)
            bridge_valid = transition_boundaries.eq(0)
            holdout_bridge_valid = bridge_valid & bridge_holdout_time.unsqueeze(1)
            training_bridge_valid = bridge_valid & ~bridge_holdout_time.unsqueeze(1)
            transfer_bridge_prediction = bridge(
                bridge_embeddings, actions, behavior_mean_actions
            )
            (
                transfer_bridge_error,
                transfer_bridge_baseline_error,
                transfer_bridge_action_gain,
                transfer_bridge_cosine,
                transfer_bridge_effect_fraction,
            ) = bridge_metrics(
                bridge,
                transfer_bridge_prediction,
                bridge_embeddings,
                bridge_next_values,
                holdout_bridge_valid,
            )
        for parameter in bridge.parameters():
            parameter.requires_grad_(True)
        bridge_loss_sum = bridge_grad_norm_sum = 0.0
        bridge_steps = 0
        for _ in range(args.bridge_epochs):
            time_permutation = torch.randperm(
                args.num_steps, device=device, generator=bridge_generator
            )
            for bridge_start in range(0, args.num_steps, args.bridge_time_batch):
                time_indices = time_permutation[
                    bridge_start : bridge_start + args.bridge_time_batch
                ]
                current_bridge_loss, _ = bridge_loss(
                    bridge,
                    bridge_embeddings[time_indices],
                    actions[time_indices],
                    behavior_mean_actions[time_indices],
                    bridge_next_values[time_indices],
                    training_bridge_valid[time_indices],
                )
                if not training_bridge_valid[time_indices].any():
                    continue
                bridge_optimizer.zero_grad(set_to_none=True)
                current_bridge_loss.backward()
                bridge_grad_norm = nn.utils.clip_grad_norm_(
                    bridge.parameters(), args.bridge_grad_clip
                )
                bridge_optimizer.step()
                bridge_loss_sum += current_bridge_loss.item()
                bridge_grad_norm_sum += float(bridge_grad_norm)
                bridge_steps += 1
        bridge_loss_mean = bridge_loss_sum / max(bridge_steps, 1)
        bridge_grad_norm_mean = bridge_grad_norm_sum / max(bridge_steps, 1)
        bridge_ready = bridge_steps > 0 or bridge_ready

        with torch.no_grad():
            trained_bridge_prediction = bridge(
                bridge_embeddings, actions, behavior_mean_actions
            )
            (
                trained_bridge_error,
                trained_bridge_baseline_error,
                trained_bridge_action_gain,
                trained_bridge_cosine,
                trained_bridge_effect_fraction,
            ) = bridge_metrics(
                bridge,
                trained_bridge_prediction,
                bridge_embeddings,
                bridge_next_values,
                holdout_bridge_valid,
            )
        ...
        with torch.no_grad():
            ...
            selected_neighbor_distance = neighbor_distance.gather(
                2, successor_partner.unsqueeze(-1)
            ).squeeze(-1)
            valid_neighbor_distance = selected_neighbor_distance[loser_mask]
            neighbor_p50 = (
            ...
            action_residual = (actions - behavior_mean_actions).reshape(-1, act_dim_sf)
            action_residual_rank = effective_rank(action_residual)
            action_residual_norm = float(action_residual.norm(dim=-1).mean())
        ...
        # ---- direct latent successor policy improvement --------------------------
        ...
        writer.add_scalar("latent_actor/improvement_loss", full_improvement_loss, global_step)
        writer.add_scalar("latent_actor/loser_count", b_loser_mask.sum().item(), global_step)
        writer.add_scalar("latent_actor/loser_fraction", b_loser_mask.float().mean().item(), global_step)
        writer.add_scalar("latent_actor/old_successor_error", actor_old_error, global_step)
        writer.add_scalar("latent_actor/new_successor_error", actor_new_error, global_step)
        writer.add_scalar("latent_actor/predicted_error_reduction", actor_predicted_improvement, global_step)
        writer.add_scalar("latent_actor/desired_displacement_norm", desired_displacement_norm, global_step)
        writer.add_scalar("latent_actor/mean_action_shift", actor_mean_shift, global_step)
        writer.add_scalar("latent_actor/controllable_fraction", actor_controllable_fraction, global_step)
        writer.add_scalar("latent_actor/control_effective_rank", actor_control_effective_rank, global_step)
        writer.add_scalar("latent_actor/beta_entropy", beta_entropy, global_step)
        writer.add_scalar("latent_actor/beta_alpha_mean", beta_alpha_mean, global_step)
        writer.add_scalar("latent_actor/beta_beta_mean", beta_beta_mean, global_step)
        writer.add_scalar("latent_actor/neighbor_distance_p50", neighbor_p50, global_step)
        writer.add_scalar("latent_actor/neighbor_distance_p90", neighbor_p90, global_step)

        # The held-out one-iteration-ahead panel is the causal test of whether the
        # frozen bridge used by this actor knew anything about the current rollout.
        writer.add_scalar("bridge/loss", bridge_loss_mean, global_step)
        writer.add_scalar("bridge/grad_norm", bridge_grad_norm_mean, global_step)
        writer.add_scalar("bridge/steps", bridge_steps, global_step)
        writer.add_scalar("bridge/heldout_one_iteration_ahead_error", preactor_bridge_error, global_step)
        writer.add_scalar("bridge/heldout_one_iteration_ahead_baseline_error", preactor_bridge_baseline_error, global_step)
        writer.add_scalar("bridge/heldout_one_iteration_ahead_action_gain", preactor_bridge_action_gain, global_step)
        writer.add_scalar("bridge/heldout_one_iteration_ahead_action_cosine", preactor_bridge_cosine, global_step)
        writer.add_scalar("bridge/heldout_one_iteration_ahead_effect_fraction", preactor_bridge_effect_fraction, global_step)
        writer.add_scalar("bridge/heldout_postupdate_transfer_error", transfer_bridge_error, global_step)
        writer.add_scalar("bridge/heldout_postupdate_transfer_baseline_error", transfer_bridge_baseline_error, global_step)
        writer.add_scalar("bridge/heldout_postupdate_transfer_action_gain", transfer_bridge_action_gain, global_step)
        writer.add_scalar("bridge/heldout_postupdate_transfer_action_cosine", transfer_bridge_cosine, global_step)
        writer.add_scalar("bridge/heldout_postupdate_transfer_effect_fraction", transfer_bridge_effect_fraction, global_step)
        writer.add_scalar("bridge/heldout_postfit_error", trained_bridge_error, global_step)
        writer.add_scalar("bridge/heldout_postfit_baseline_error", trained_bridge_baseline_error, global_step)
        writer.add_scalar("bridge/heldout_postfit_action_gain", trained_bridge_action_gain, global_step)
        writer.add_scalar("bridge/heldout_postfit_action_cosine", trained_bridge_cosine, global_step)
        writer.add_scalar("bridge/heldout_postfit_effect_fraction", trained_bridge_effect_fraction, global_step)
        writer.add_scalar("bridge/action_residual_effective_rank", action_residual_rank, global_step)
        writer.add_scalar("bridge/action_residual_norm", action_residual_norm, global_step)
