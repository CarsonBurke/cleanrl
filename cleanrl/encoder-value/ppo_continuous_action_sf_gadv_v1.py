# SF + STATE-CONDITIONAL ADVANTAGE READOUT (gadv_v1). Parent: lejepa_sf_v3.py, defaults.
#
# THE ONE CHANGE. The parent's critic is fully vector-valued -- a 62-dim TD(lambda) residual
# E_t -- but the POLICY side collapses at exactly one line, `adv_vector = sf_residual @ w_r`,
# through a single GLOBAL w_r that is constant across the whole batch. That collapse is not
# a lossy approximation, it is an identity: w_r . E_t == A_t whenever w_r . phi_t == r_t, so
# the entire vector machinery reduces to scalar GAE on a denoised reward. Measured on the
# parent: advB/adv_vec_corr = 0.97 -> 0.99. The vector never reaches the policy.
#
#   g(s_t) = w_r + dg(sg(critic_feat_t)) / res_rms        (sf_dim,) per state
#   adv_t  = E_t . g(s_t)  =  (E_t @ w_r)  +  (E_t . dg_eff(s_t))
#
# The first term is the parent's line, bit-for-bit. The second is the whole diff.
#
# WHY IT CANNOT SILENTLY REDUCE TO SCALAR GAE. dg is not free: it is fit each iteration by
# regressing the RESULTING advantage onto the TRUNCATED MONTE-CARLO advantage the parent
# already computes for its EV gate (mc_ret - values, on mc_mask). So dg only moves off zero
# where the global projection MISPREDICTS the realized return -- the projection's residual
# is precisely the reward structure w_r.phi cannot express (the frame-skip acceleration term
# the parent's header blames), and re-injecting it is the mechanism. The head stays a
# calibrated advantage estimator rather than an arbitrary state reweighting, and if the
# global projection were already optimal per-state the MSE fit would leave dg at zero.
#
# NOVELTY. Every SF-control method here reads the successor features through a single global
# reward vector. Making the READOUT DIRECTION state-conditional -- and anchoring it to a
# Monte-Carlo advantage rather than to a reward regression -- is the new object. It is what a
# scalar critic structurally cannot do: there is only one coordinate to reweight.
#
# CONSTRAINTS, each a correctness requirement (verified statically, see verify_gadv.py):
#   * dg's output layer is ZERO-init: g == w_r exactly at init, and the CPU RNG stream is
#     restored after construction, so iteration 1 is bit-identical to the parent.
#   * dg is DETACHED from the trunk and lives in its OWN AdamW. Its params are in no other
#     optimizer and in neither grad-clip budget -- otherwise this stops being one variable.
#   * g is a function of s_t ALONE. If it saw a_t the "advantage" would become an
#     action-dependent reweighting and the policy gradient would be biased.
#   * The fit runs ONCE per iteration, outside the 320-minibatch PPO loop, on the rollout
#     just collected, using the SAME (previous) w_r that produced values[] -- so the
#     projection stays self-consistent with the baseline it is differenced against.
#   * --no-gadv recovers the parent exactly (the head is never even constructed).
#
# HYPOTHESIS. A state-conditional readout recovers advantage signal the single global
# projection filters out, raising held-out MC-advantage EV over the base projection, and
# converting that into return on HalfCheetah-v4 above the parent's 10,071 @8M.
#
# NULL / KILL SIGNALS, in order of how cheaply they fire:
#   gadv/corr_vs_wr ~ 1.0        the head changed nothing; the run is a null experiment.
#   gadv/ev_mc <= gadv/ev_mc_base  the state-conditional readout does not predict the MC
#                                advantage better than the global one -- the premise is false.
#   gadv/dg_ratio_mean growing without bound alongside a FALLING ev_mc -- a runaway head
#                                fitting noise; expect a return regression.
#
# THE FAILURE MODE THAT IS NOT OBVIOUS, and the reason gadv_lr is what it is: the target is a
# single-sample Monte-Carlo advantage, so the fit's gradient is mostly noise, and Adam does
# NOT settle at dg = 0 when the global projection is already optimal -- it equilibrates in a
# noise ball whose radius is set by the learning rate. Measured on a synthetic replica of this
# loop, a control with nothing to learn still cost 0.029 held-out EV on average (0.069 at its
# worst) at lr=1e-3. gadv_lr's note records the sweep; the default makes that cost 0.002.
#
# THE SECOND FAILURE MODE, which the regression target actively INVITES. The fit target
# mc_ret - values decomposes as A_true + (V_true - values) + noise, and the middle term is the
# critic's own value error, which is STATE-ONLY. E_t . dg(s_t) can express a state-only
# component (through whatever part of E_t is predictable from s_t), so MSE will happily spend
# capacity there -- and a state-only term added to an advantage is a BASELINE: it contributes
# nothing to the policy gradient in expectation while adding variance, and under
# norm_adv_scope="batch" it shrinks the action-discriminating signal by the ratio of stds. So
# a rising gadv/ev_mc_gain is necessary but NOT sufficient. Read it against
# gadv/corr_std_ratio and gadv/cos_dg_wr: a large correction that is nearly collinear with
# w_r, or one that grows while return does not, is the head buying EV with a baseline.
# (Related and already handled: dg is divided by the per-dimension RMS of E, not its std,
# so no coordinate gets outsized leverage on the advantage -- see gadv_delta.)
#
# The honest risks this does NOT remove:
#   * mc_mask keeps only steps >= mc_window from the next boundary (~38% of a HalfCheetah
#     rollout), and those rows are a deterministic function of within-segment time index -- so
#     the fit set and the application set differ systematically in gait phase, not randomly.
#   * The head persists across iterations while the TRUNK it reads moves under it every PPO
#     update; 64 steps at lr 1e-4 cannot fully track that. It shows as gadv/fit_loss failing
#     to trend and ev_mc_gain hovering near zero -- which is indistinguishable from "there was
#     no signal", so a null result here is ambiguous between the two.
#   * On Hopper/Walker early training n_mc never reaches gadv_min_mc, so the head is never fit
#     and the file IS the parent. This is a HalfCheetah experiment.
#   * The correction is applied to every environment, but the 14 fit environments receive an
#     IN-SAMPLE correction and the 2 held-out ones an out-of-sample one, so the consumed
#     advantage carries env-blocked heteroscedasticity that norm_adv_scope="batch" does not
#     model. Second order while gadv/corr_std_ratio stays small, but it is a structural
#     asymmetry the (leak-prone) row split did not have; the leak-free split is worth it.
#   * res_rms is pooled over all 16 environments, held-out included, so the held-out scoring
#     is out-of-sample in the head's parameters but not in this one per-dimension scale.
#     32,768 rows into a 62-vector of second moments: negligible, but not literally zero.
# =====================================================================================
...
class Args:
    ...
    # --- gadv_v1: STATE-CONDITIONAL readout of the vector residual --------------------
    # g(s_t) = w_r + dg(sg(critic_feat_t))/res_std;  adv_t = E_t . g(s_t). See header.
    gadv: bool = True                # OFF recovers the parent EXACTLY -- the head is not even
    #                                  constructed, so the RNG stream is untouched too.
    gadv_hidden: int = 128           # dg trunk width. TANH activations, deliberately: bounded
    #                                  hidden units cap how fast the readout perturbation can
    #                                  grow if the MC target's noise is fit rather than its mean.
    gadv_lr: float = 1e-4            # own optimizer, own LR, annealed on the same frac as the
    #   PPO/SSL optimizers. THE load-bearing knob, and NOT for the usual reason. The target is
    #   a single-sample Monte-Carlo advantage, so the fit's stochastic gradient is mostly
    #   noise; Adam's normalized step means the head does not converge to dg = 0 when there is
    #   nothing to learn, it equilibrates in a NOISE BALL whose radius is set by the LR. Swept
    #   offline on a synthetic replica of this exact loop (244 annealed iterations, 64 steps
    #   each, fresh data per iteration, noise set so the base projection scores EV ~0.3 -- the
    #   order a single-sample MC-advantage target sits at). Both columns are held-out EV
    #   RELATIVE to the base projection, averaged over the whole run, not read off the last
    #   iteration; CONTROL is a world where the global w_r is already optimal, so every point
    #   of movement there is pure noise-fitting:
    #       lr        treat gain    control     worst    ratio mean/max
    #       1e-3        +0.714       -0.029     -0.069     0.29 / 0.49
    #       3e-4        +0.707       -0.006     -0.019     0.13 / 0.19
    #       1e-4        +0.659       -0.002     -0.007     0.071 / 0.091   <- default
    #       3e-5        +0.504       -0.0004    -0.003     0.033 / 0.038
    #   Read the CONTROL column as the load-bearing one: it is a property of the noise ball
    #   and reproduced to ~0.0005 across seeds and across two different synthetic fixtures.
    #   The treat column depends on how much state structure the fixture puts there and is
    #   only meaningful as a RANKING.
    #   3e-4 has the best raw gain and is the obvious next arm if the head reads as
    #   signal-starved (gadv/dg_ratio_mean pinned low with ev_mc_gain ~ 0). 1e-4 is the
    #   default because it costs ~7% of the gain for 3.5x less damage and a 2.7x smaller worst
    #   case, and because the parent's own per_dim_lambda post-mortem is that EARLY advantage
    #   damage compounds -- a head that stays near zero until evidence accumulates degrades
    #   gracefully to the parent, which is what makes a null result attributable.
    gadv_wd: float = 1e-4            # AdamW decay. NOMINAL, and measured to be so: at
    #   lr*wd = 1e-8 per step it moves ||dg|| by <1e-3 over a whole run, and sweeping it from
    #   1e-4 to 1e-1 changed the control's ratio by 0.002. What actually bounds the readout
    #   perturbation is gadv_lr (above) and its anneal, not this. Kept at the standard value
    #   rather than removed so the optimizer matches the SSL path's AdamW.
    gadv_steps: int = 64             # optimizer steps per ITERATION, outside the PPO loop
    gadv_batch: int = 4096           # samples per step, drawn WITH replacement from the masked
    #                                  train split (so any batch size is legal)
    gadv_grad_clip: float = 1.0      # own clip, fully separate from PPO's 0.25 budgets
    gadv_val_frac: float = 0.125     # held-out fraction, BY ENVIRONMENT (2 of 16), not by row.
    #   This is a correctness constraint, not tidiness. mc_ret[t] and mc_ret[t+1] share 499 of
    #   their 500 reward terms and trunk(s_t) ~ trunk(s_{t+1}) at dt=0.05, so a random ROW
    #   split leaves every held-out row with near-duplicate neighbours in the train set: the
    #   score would be in-sample in everything but name, and could not distinguish "found real
    #   state structure" from "memorized 12k autocorrelated Monte-Carlo returns" -- the exact
    #   distinction it exists to make. Whole environments are independent trajectories, so
    #   holding them out is the split that makes the number mean what the tag says. (This is
    #   the parent's own argument at the EV gate: a predictor scores well against a target
    #   built from data it has effectively already seen.)
    gadv_res_ema: float = 0.1        # EMA rate for res_rms. NOT sf_target_ema (0.01): ||E_t||
    #   FALLS by 1-2 orders of magnitude over the run (psi is zero-init, so the first
    #   rollout's residual is the whole discounted phi-sum), and a 100-iteration timescale
    #   would track a spread the critic left behind ~40% of the way into training. With the
    #   1/count warmup this is an exact running mean for 10 iterations, then a 10-iteration EMA.
    gadv_min_mc: int = 8192          # minimum masked transitions before the head is fit at all.
    #   The parent's n_mc >= 256 gate was sized for a CLOSED-FORM ridge probe; this head is
    #   ~33k parameters fit by SGD, and at 256 rows the loop would draw 64*4096 = 262k samples
    #   with replacement from ~220 of them. HalfCheetah yields n_mc ~ 12.4k per rollout
    #   (2048 steps, 1000-step episodes, mc_window=500 => ~777 rows/env * 16), so this passes
    #   with margin there and correctly refuses on Hopper/Walker early training, where
    #   episodes rarely reach 500 steps and the honest answer is "fall back to the parent".
...
def r2_score(pred, target):
    """Proper R^2: residual SUM of squares, so a constant offset COUNTS as error.

    Reported alongside ev_score because the two disagree here in an informative way. The
    truncated MC return is biased LOW by gamma^(mc_window+1)*V(s_{t+K+1}) (~0.7% of V at the
    defaults), a near-constant offset that ev_score is blind to and R^2 is not. A large
    ev/R^2 gap is that truncation bias, NOT a broken fit.
    """
...
def corr_score(a, b):
    """Pearson correlation of two flat tensors, as a python float."""
...
class GAdvHead(nn.Module):
    """dg: trunk feature -> a per-state PERTURBATION of the reward readout w_r.

    STATE ONLY, and that is a correctness constraint, not a style choice. adv_t = E_t.g(s_t)
    is a valid policy-gradient signal because g depends on s_t alone: E_t already carries the
    action, and a g that also saw a_t would make the "advantage" an action-dependent
    reweighting of itself -- the gradient would no longer point along the true advantage even
    at a perfect fit. verify_gadv.py AST-checks forward() for exactly this.

    The output layer is ZERO-init, so at construction dg == 0 exactly, g == w_r exactly, and
    the advantage is the parent's `sf_residual @ w_r` bit-for-bit. Tanh (not ReLU^2, the
    trunk's activation) because bounded hidden units bound how fast the readout can move
    when the Monte-Carlo target is noisy.
    """

    def __init__(self, feat_dim, sf_dim, hidden):
        super().__init__()
        ...
        self.out = nn.Linear(hidden, sf_dim)
        with torch.no_grad():
            self.out.weight.zero_()
            self.out.bias.zero_()

    def forward(self, critic_feat):
        return self.out(self.body(critic_feat))


def gadv_delta(head, critic_feat, res_rms):
    """g(s_t) - w_r, the state-conditional perturbation of the reward readout.

    res_rms is the per-dimension ROOT MEAN SQUARE of E_t, and it is the RMS rather than the
    standard deviation for a load-bearing reason. The correction is the bilinear form
    sum_d E_d * dg_d / res_d, so coordinate d's leverage on the advantage per unit of dg is
    RMS(E_d)/res_d. Dividing by the STD makes that leverage mean(E_d)/std(E_d), which is not
    O(1) at all in this feature space: phi's a*a block is strictly positive with a large mean
    and small variance, and the intercept's discounted sum is ~1/(1-gamma) with variance
    coming only from proximity to an episode boundary (the parent says exactly this at
    per_dim_lambda). Those coordinates would carry 10-100x the leverage of a whitened e
    coordinate, the fit would preferentially route through them, and the correction would be
    dominated by whichever coordinate happened to be loudest. Dividing by the RMS pins every
    coordinate's leverage at 1, which is also what makes ||dg|| a meaningful bound on the size
    of the perturbation.

    The sharper version, which is why this is not merely a conditioning preference. Write
    E_d = mu_d + e_d with e_d zero-mean. Coordinate d contributes dg_d(s)*(mu_d + e_d)/res_d,
    whose STATE-ONLY part has gain |mu_d|/res_d -- and a state-only term added to an advantage
    is a baseline: zero contribution to the policy gradient in expectation, pure added
    variance (see the header's second failure mode). Under RMS that gain is bounded by 1 for
    every coordinate, since |mu| <= sqrt(mu^2 + sigma^2). Under the std it is |mu_d|/std_d,
    which the intercept coordinate makes ~100. The std normalizer would have handed the fit a
    100x-amplified route to the one component that is worthless to the policy.

    The bound is EXACT only against this batch's RMS. res_rms is a lagged EMA
    (gadv_res_ema=0.1), so while a coordinate's scale is RISING -- and it does rise, since
    HalfCheetah returns grow through training and the reward-carrying phi coordinates grow with
    them -- the divisor sits low and the realised gain exceeds 1 by the per-window growth
    factor. That factor is order 1.0x, not the 100x the std normalizer gives away, so the
    ranking argument above survives; the claim to hold onto is "bounded by 1 up to the EMA's
    tracking lag", not "pinned at 1". gadv/corr_std_ratio measures the realised perturbation
    directly and is the monitor that does not depend on this argument at all.

    `critic_feat` is trunk(s_t) and NOTHING else enters -- no action, no phi, no reward.
    """
    return head(critic_feat) / res_rms


def gadv_correction(head, sf_residual_flat, critic_feat, res_rms):
    """E_t . (g(s_t) - w_r): the whole diff against the parent, as an ADDITIVE term.

    Additive on purpose. `adv = (sf_residual @ w_r) + correction` keeps the parent's exact
    expression for the base term, so a zero head reproduces it BITWISE rather than merely to
    within matmul-vs-broadcast reassociation.
    """
    return (sf_residual_flat * gadv_delta(head, critic_feat, res_rms)).sum(-1)
...
if __name__ == "__main__":
    ...
    # gadv is a modification OF the vector routing: it replaces the single global projection
    # w_r with a state-conditional one. With vector_adv off the policy consumes the scalar
    # GAE and the head would be fit every iteration and then silently discarded.
    if args.gadv and not args.vector_adv:
        raise ValueError(
            "gadv=True requires vector_adv=True: the state-conditional readout g(s) IS the "
            "projection of the vector residual onto the policy advantage. With --no-vector-adv "
            "the policy consumes the scalar GAE and g would never be read. Use --no-gadv."
        )
    # Same failure, one level less obvious: under auto_entropy the PPO loss reads `policy_adv`,
    # which is built by its OWN scalar soft-advantage recursion and never touches `advantages`.
    # The head would be fit every iteration, logged with healthy-looking metrics, and then
    # discarded -- a null experiment that reads as a real one. (vector_adv already rejects
    # auto_entropy below for a related reason; this catches the case explicitly.)
    if args.gadv and args.auto_entropy and args.actor_dist == "gaussian":
        raise ValueError(
            "gadv=True is incompatible with auto_entropy: the soft-advantage path builds "
            "`policy_adv` from a separate SCALAR GAE recursion, so the state-conditional "
            "readout would be computed, logged, and never read by the policy loss."
        )
    # The held-out split is BY ENVIRONMENT (see gadv_val_frac): with too few envs there is no
    # way to hold out an independent trajectory, and the reported EV would silently become an
    # in-sample number.
    if args.gadv and args.num_envs < 4:
        raise ValueError(
            f"gadv=True needs num_envs >= 4 to hold out whole environments for an honest "
            f"out-of-sample score; got {args.num_envs}."
        )
    ...
    # --- gadv: the state-conditional readout head ------------------------------------
    # A TOP-LEVEL module, deliberately not a submodule of Agent, for exactly the reason the
    # SSL nets are: as a submodule its parameters would enter agent.parameters() (the PPO
    # optimizer), actor_parameters()/critic_parameters(), and their 0.25 clip budgets --
    # which would make this two changes instead of one.
    #
    # None when disabled, not merely unused: constructing it consumes CPU RNG (orthogonal
    # init), and a shifted RNG stream would make --no-gadv fail to reproduce the parent for a
    # reason that has nothing to do with the readout. For the SAME reason the CPU generator
    # state is SNAPSHOT and RESTORED around the construction when it IS enabled: iteration 1
    # is then bit-identical to the parent (the head is zero, and the fit is gated on
    # w_r_solved, so it does not run until iteration 2), and every subsequent divergence is
    # attributable to dg rather than to a different exploration noise sequence.
    gadv_head = None
    gadv_optimizer = None
    gadv_gen = None
    if args.gadv:
        _rng_state = torch.get_rng_state()
        gadv_head = GAdvHead(args.hidden, sf_dim, args.gadv_hidden).to(device)
        torch.set_rng_state(_rng_state)
        gadv_optimizer = optim.AdamW(
            gadv_head.parameters(), lr=args.gadv_lr, weight_decay=args.gadv_wd
        )
        # The fit's shuffling and minibatch sampling draw from a DEDICATED generator, not the
        # global CUDA stream. Same motive as the CPU snapshot above, but it has to hold for
        # the whole run, not just construction: `dist.sample()` in the rollout draws from the
        # global CUDA stream, so 65 extra draws per iteration would give this arm a different
        # exploration noise sequence than the parent's from iteration 2 onward -- and at one
        # seed on HalfCheetah that difference alone is worth hundreds of return points. With
        # this, --gadv vs --no-gadv (and vs the parent) differ ONLY in dg.
        gadv_gen = torch.Generator(device=device)
        gadv_gen.manual_seed(args.seed)
    # Per-dimension ROOT MEAN SQUARE of the vector residual E_t (see gadv_delta for why RMS
    # and not std), EMA'd with the parent's 1/count warmup idiom: an exact running mean for
    # the first 1/gadv_res_ema iterations, then a plain EMA. This is dg's CONDITIONER, not a
    # target statistic -- it has to track ||E|| as the critic learns (or dg's weights stop
    # meaning the same thing) while staying smooth enough that they mean something at all.
    res_rms = torch.ones(sf_dim, device=device)
    res_stat_count = 0
    # Has the head EVER taken a step? Distinguishes "dg == 0 because it was never fit" from
    # "dg == 0 because the fit found nothing", which the direction metrics cannot tell apart
    # on their own (a zero head has an undefined, not orthogonal, angle to w_r).
    gadv_fitted = False
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            # Same reasoning for the readout head: late in training the policy is refining
            # and a still-moving g would keep redefining the advantage it is refining against.
            if gadv_optimizer is not None:
                gadv_optimizer.param_groups[0]["lr"] = frac * args.gadv_lr
        ...
        with torch.no_grad():
            ...
            # ---- gadv: MAKE THAT PROJECTION STATE-CONDITIONAL -----------------------
            # The line above is the parent's, untouched, and it is exactly where the vector
            # dies: 62 dimensions -> one scalar through a w_r that is CONSTANT across the
            # whole batch. Since w_r . E == A_hat identically whenever w_r . phi == r, that
            # collapse makes every dimension of the critic equivalent to scalar GAE on a
            # denoised reward (measured: adv_vec_corr 0.97 -> 0.99). Replace the constant
            # readout with a per-state one,
            #
            #     g(s_t) = w_r + dg(sg(critic_feat_t))/res_rms
            #     adv_t  = E_t . g(s_t) = (E_t @ w_r) + E_t . dg_eff(s_t)
            #
            # written ADDITIVELY so a zero head is bitwise the parent.
            #
            # dg IS NOT FREE. It is fit, right here and once per rollout, by regressing the
            # RESULTING advantage onto the TRUNCATED MONTE-CARLO advantage mc_ret - values --
            # the same mc_ret/mc_mask the EV gate below already consumes, deliberately reused
            # rather than re-derived (a second truncated-return estimator is exactly how an
            # off-by-one in the horizon accounting hides). That target is what keeps this a
            # CALIBRATED advantage instead of an arbitrary state reweighting: dg can only pay
            # for itself where the global projection genuinely mispredicts the realized
            # return, and if w_r were already per-state optimal the MSE fit leaves dg at zero.
            #
            # SELF-CONSISTENCY. w_r here is still the PREVIOUS iteration's solve -- the one
            # that produced values[] during the rollout, hence the one inside the regression
            # target's baseline. Fitting against a target built from one w_r while correcting
            # a projection built from another would make the residual meaningless.
            #
            # GATED ON w_r_solved. On iteration 1 w_r == 0, so the base projection is
            # identically zero and mc_ret - values is the RAW return: dg would be asked to
            # regress the whole return through E, land far from zero, and persist. The parent
            # already falls back to scalar GAE there for the same reason.
            gadv_ev = gadv_ev_base = gadv_r2 = gadv_r2_base = float("nan")
            gadv_loss_v = gadv_gn_v = gadv_fit_n = float("nan")
            gadv_dg_ratio_mean = gadv_dg_ratio_std = float("nan")
            gadv_corr_wr = gadv_corr_gae = float("nan")
            gadv_corr_std_ratio = gadv_cos_wr = float("nan")
            adv_gvector = adv_vector
            if args.gadv:
                flat_res = sf_residual.reshape(-1, sf_dim)
                res_stat_count += 1
                res_rate = max(args.gadv_res_ema, 1.0 / res_stat_count)
                res_rms = res_rms + res_rate * (
                    flat_res.pow(2).mean(0).sqrt().clamp_min(1e-6) - res_rms
                )
                # critic_feat_buf is (T*B, hidden) built from obs.reshape(-1, obs_dim), and the
                # buffers are T-MAJOR, so flat index i == t*num_envs + b in EVERY tensor below:
                # features, residual, mc_ret, values and the mask all address the same
                # transition. verify_gadv.py brute-forces this alignment.
                gfeat = critic_feat_buf.detach()
                # DETACHED, and doubly so: critic_feat_buf was built under no_grad, so the
                # trunk cannot receive gradient from the fit even under enable_grad below.
                gtarget = (mc_ret - values).reshape(-1)   # truncated MC advantage
                gbase = adv_vector.reshape(-1)
                if w_r_solved and n_mc >= args.gadv_min_mc:
                    # HELD OUT BY ENVIRONMENT, not by row -- see gadv_val_frac. Flat index
                    # i == t*num_envs + b, so i % num_envs recovers the environment, and whole
                    # environments are independent trajectories. A random row split would put
                    # each val row's own temporal neighbours (sharing 499/500 of their reward
                    # terms, and a near-identical trunk feature at dt=0.05) in the train set.
                    env_of = torch.arange(
                        args.num_steps * args.num_envs, device=device
                    ) % args.num_envs
                    # max/min are not decoration: round() is banker's rounding, so a small
                    # num_envs can round the val count to 0, and a large val_frac can take
                    # every environment and leave nothing to fit on.
                    n_val_env = max(1, min(args.num_envs - 1,
                                           round(args.gadv_val_frac * args.num_envs)))
                    val_envs = torch.randperm(args.num_envs, device=device,
                                              generator=gadv_gen)[:n_val_env]
                    is_val = (env_of.unsqueeze(-1) == val_envs).any(-1)
                    val_idx = (mc_mask & is_val).nonzero(as_tuple=True)[0]
                    tr_idx = (mc_mask & ~is_val).nonzero(as_tuple=True)[0]
                    # BOTH SIDES MUST BE NON-EMPTY, and unlike a row split an environment
                    # split does not guarantee it: mc_mask keeps only the rows with a full
                    # mc_window of future reward, and on variable-episode-length envs
                    # (Hopper/Walker) that subset can land entirely inside -- or entirely
                    # outside -- the drawn validation environments. An empty tr_idx makes
                    # randint(0, ...) raise; an empty val_idx makes r2_score return exactly
                    # 1.0 from 0/0, which would log a fabricated perfect fit for BOTH the
                    # head and the baseline. Skip the iteration and leave gadv/* at NaN.
                    if tr_idx.numel() > 0 and val_idx.numel() > 0:
                        gadv_fitted = True
                        gadv_fit_n = float(tr_idx.numel())
                        # Accumulated on DEVICE and synced once after the loop: an .item()
                        # inside a 64-step inner loop is 128 host syncs per iteration, which
                        # the SSL loop in this same file goes out of its way to avoid.
                        loss_acc = torch.zeros((), device=device)
                        gn_acc = torch.zeros((), device=device)
                        with torch.enable_grad():
                            for _ in range(args.gadv_steps):
                                sel = tr_idx[
                                    torch.randint(tr_idx.numel(), (args.gadv_batch,),
                                                  device=device, generator=gadv_gen)
                                ]
                                pred = gbase[sel] + gadv_correction(
                                    gadv_head, flat_res[sel], gfeat[sel], res_rms
                                )
                                gloss = F.mse_loss(pred, gtarget[sel])
                                gadv_optimizer.zero_grad(set_to_none=True)
                                gloss.backward()
                                ggn = nn.utils.clip_grad_norm_(
                                    gadv_head.parameters(), args.gadv_grad_clip
                                )
                                gadv_optimizer.step()
                                loss_acc += gloss.detach()
                                gn_acc += ggn.detach()
                        gadv_loss_v = float(loss_acc) / max(args.gadv_steps, 1)
                        gadv_gn_v = float(gn_acc) / max(args.gadv_steps, 1)
                        # HELD-OUT scoring, and both predictors on the SAME rows: `_base` is
                        # the parent's global projection, so ev_mc <= ev_mc_base is the direct
                        # falsifier for the premise that the readout should be state-conditional.
                        pred_val = gbase[val_idx] + gadv_correction(
                            gadv_head, flat_res[val_idx], gfeat[val_idx], res_rms
                        )
                        gadv_ev = ev_score(pred_val, gtarget[val_idx])
                        gadv_ev_base = ev_score(gbase[val_idx], gtarget[val_idx])
                        gadv_r2 = r2_score(pred_val, gtarget[val_idx])
                        gadv_r2_base = r2_score(gbase[val_idx], gtarget[val_idx])
                # Applied to the WHOLE rollout, including the rows mc_mask dropped -- see the
                # header's honest risk. Recomputed rather than reusing the fit's forward: the
                # optimizer stepped in between, so the advantage the policy consumes must come
                # from the head's POST-fit state.
                #
                # Gated on w_r_solved to match the selection below: on iteration 1 the base
                # projection is identically zero, every ratio and correlation here is 0/0, and
                # a logged 0.0 would read as a real measurement. NaN is the honest value.
                if w_r_solved:
                    # One forward for both the correction and the diagnostics below; the
                    # correction is written out of dg_eff rather than via gadv_correction so
                    # the head is not evaluated twice over 32,768 rows. Asserted equivalent to
                    # gadv_correction in verify_gadv.py.
                    dg_eff = gadv_delta(gadv_head, gfeat, res_rms)
                    corr_flat = (flat_res * dg_eff).sum(-1)
                    adv_gvector = adv_vector + corr_flat.reshape(
                        args.num_steps, args.num_envs
                    )
                    # THE runaway monitor, and it has to be this one: what matters is how much
                    # of the consumed advantage the head is responsible for, std(E.dg) relative
                    # to std(E.w_r). A ||dg||/||w_r|| ratio alone cannot see a head that is
                    # small in norm but aimed at a high-leverage coordinate (which is exactly
                    # what the RMS normalization in gadv_delta exists to prevent).
                    gadv_corr_std_ratio = float(
                        corr_flat.std() / adv_vector.std().clamp_min(1e-12)
                    )
                    # Secondary: ||g - w_r|| / ||w_r|| per state. Mean AND spread, because a
                    # head that is quiet on average and enormous on a few states is the failure
                    # mode a mean alone hides.
                    dg_ratio = dg_eff.norm(dim=-1) / w_r.norm().clamp_min(1e-12)
                    gadv_dg_ratio_mean = float(dg_ratio.mean())
                    gadv_dg_ratio_std = float(dg_ratio.std())
                    # Is the head finding a NEW readout direction, or just a per-state GAIN on
                    # the old one? cos(dg_eff, w_r) ~ +/-1 means g is (1+c(s))*w_r, i.e. a state
                    # -dependent step size rather than the vector routing this file exists to
                    # test -- and that case can leave corr_vs_wr near 1.0 while still changing
                    # the policy a lot, so the cheapest kill signal would misread it.
                    # Only meaningful once the head has moved: before the first fit dg_eff is
                    # identically zero and the clamp would report cos == 0.0, which reads as
                    # "found a fully orthogonal direction" -- the opposite of the truth.
                    if gadv_fitted:
                        gadv_cos_wr = float(
                            (dg_eff @ w_r
                             / (dg_eff.norm(dim=-1) * w_r.norm()).clamp_min(1e-12)).mean()
                        )
                    # corr(new adv, w_r.E) == 1.0 means the head did nothing and the run is a
                    # null experiment -- the SAME kill signal the parent applies to its own
                    # projection.
                    gadv_corr_wr = corr_score(adv_gvector, adv_vector)
            ...
            if args.vector_adv and w_r_solved:
                # adv_gvector IS adv_vector (the same object) whenever gadv is off or the
                # head has not been fit yet, so --no-gadv selects the parent's tensor.
                advantages = adv_gvector
            # The parent's kill signal, computed on the parent's quantity so the tag keeps
            # meaning exactly what it meant in v3 and the curves stay comparable. The new
            # readout is scored separately under gadv/*.
            if args.gadv and w_r_solved:
                gadv_corr_gae = corr_score(adv_gvector, adv_scalar_gae)
        ...
        # ---- gadv: the state-conditional readout ---------------------------------------
        # Written EVERY iteration from the first, NaN where the quantity does not exist yet
        # (iteration 1 has w_r == 0, so there is no base projection to correct and nothing to
        # fit). A tag that only appears late is a tag whose absence cannot be distinguished
        # from a crash.
        #
        # THE DECISIVE PAIR is ev_mc vs ev_mc_base: both are HELD-OUT explained variance
        # against the truncated Monte-Carlo advantage, on the same rows. ev_mc <= ev_mc_base
        # falsifies the premise outright -- the state-conditional readout does not predict the
        # advantage better than the single global w_r, so there was nothing in the other 61
        # dimensions for the policy to use.
        writer.add_scalar("gadv/ev_mc", gadv_ev, global_step)
        writer.add_scalar("gadv/ev_mc_base", gadv_ev_base, global_step)
        writer.add_scalar("gadv/ev_mc_gain", gadv_ev - gadv_ev_base, global_step)
        writer.add_scalar("gadv/r2_mc", gadv_r2, global_step)
        writer.add_scalar("gadv/r2_mc_base", gadv_r2_base, global_step)
        # 1.0 => the head changed nothing; the run is a null experiment. Read it TOGETHER with
        # cos_dg_wr: a pure per-state GAIN on w_r leaves this near 1.0 while still changing the
        # policy's effective step size a lot.
        writer.add_scalar("gadv/corr_vs_wr", gadv_corr_wr, global_step)
        writer.add_scalar("gadv/corr_vs_gae", gadv_corr_gae, global_step)
        writer.add_scalar("gadv/cos_dg_wr", gadv_cos_wr, global_step)
        # std(E.dg) / std(E.w_r): how much of the consumed advantage the head is responsible
        # for. THE runaway monitor -- growing while ev_mc falls means it is fitting MC noise.
        writer.add_scalar("gadv/corr_std_ratio", gadv_corr_std_ratio, global_step)
        # ||g - w_r|| / ||w_r||, per state. Secondary: it cannot see leverage, only norm.
        writer.add_scalar("gadv/dg_ratio_mean", gadv_dg_ratio_mean, global_step)
        writer.add_scalar("gadv/dg_ratio_std", gadv_dg_ratio_std, global_step)
        writer.add_scalar("gadv/fit_loss", gadv_loss_v, global_step)
        writer.add_scalar("gadv/fit_grad_norm", gadv_gn_v, global_step)
        writer.add_scalar("gadv/fit_n_train", gadv_fit_n, global_step)
        writer.add_scalar("gadv/res_rms_mean", res_rms.mean().item(), global_step)
        # The scale of the advantage the policy ACTUALLY consumed. advB/adv_vector_std above
        # still describes the parent's projection, so without this the treatment arm has no
        # logged scale at all.
        writer.add_scalar("gadv/adv_gvector_std", adv_gvector.std().item(), global_step)
