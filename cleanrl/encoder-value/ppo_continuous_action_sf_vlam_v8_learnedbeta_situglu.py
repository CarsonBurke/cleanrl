# PPO + successor features with learned SiTU-GLU cap allocation.
# v8 retains v3's parameter-matched Kimi K3 SiTU-GLU branches, but learns one
# scalar log-delta per branch:
#   beta_g = 4 exp(delta), beta_u = 25 exp(-delta)
#   [beta_g tanh(g/beta_g) sigmoid(g)] * [beta_u tanh(u/beta_u)]
# The zero initialization exactly recovers v3's (4, 25) caps, while
# beta_g*beta_u=100 preserves the product bound as optimization reallocates
# dynamic range between the gate and up paths. No clamp, schedule, or auxiliary
# loss constrains the ratio. Hypothesis: PPO can move saturation pressure away
# from whichever projection needs more range without surrendering SiTU's bounded
# forward product or changing any downstream initialization.
#
...
def situ_glu(gate, up, beta_gate, beta_up):
    ...
    # Factor the invariant product explicitly instead of multiplying separately
    # rounded reciprocal exponentials back together.
    return (
        100.0
...
class SiTUGLUBranch(nn.Module):
    """Parameter- and initial-output-matched SiTU-GLU FFN with learned caps."""

    def __init__(self, H):
        super().__init__()
        # A zero-centered log-delta keeps the official allocation bit-exact at
        # initialization while moving reciprocal scale between the two caps.
        ...
        # closest parameter-matched gated width (H=64: 8,256 matrix parameters
        # plus this variant's scalar ratio, versus 8,320 old parameters, -0.76%).
    ...
    def forward(self, x):
        gate = self.gate(x)
        up = self.up(x)
        return self.down(situ_glu(gate, up, self.beta_gate(), self.beta_up()))
...
if __name__ == "__main__":
    ...
    # Keep direct references before optional trunk compilation so the learned
    # allocation remains observable rather than turning this into a score-only
    # ablation whose mechanism cannot be diagnosed.
    situ_branches = [
        module for module in agent.modules() if isinstance(module, SiTUGLUBranch)
    ]
    if not situ_branches:
        raise RuntimeError("learned-beta experiment constructed no SiTU-GLU branches")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            beta_gates = torch.stack([branch.beta_gate() for branch in situ_branches])
            beta_ups = torch.stack([branch.beta_up() for branch in situ_branches])
        ...
        writer.add_scalar("situ/beta_up_min", beta_ups.min().item(), global_step)
        writer.add_scalar("situ/beta_up_max", beta_ups.max().item(), global_step)
