# PPO + reward-observable Koopman resolvent critic, v1.
#
...
#
# A full degree-2 Jacobi field under the old Beta policy predicts the detached
# per-reason TD innovation. Its expectation is exactly zero. PPO keeps scalar
# GAE as fallback and consumes Q-V only through a next-rollout reliability gate.
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        with torch.random.fork_rng(devices=[]):
            ...
            self.reason_action_head = layer_init(
                nn.Linear(self.latent_dim, self.action_basis_dim * self.reward_components),
                std=0.01,
            )
            with torch.no_grad():
                self.reason_action_head.weight.zero_()
                self.reason_action_head.bias.zero_()
    ...
    def action_advantage_reasons(self, latent, basis):
        coeff = self.reason_action_head(latent.detach()).view(
            -1, self.action_basis_dim, self.reward_components
        )
        return torch.einsum("bpr,bp->br", coeff, basis)
    ...
    def get_action_and_value(self, obs, raw_obs, native_action=None, return_q=False):
        ...
        if return_q:
            action_basis = self._orthogonal_beta_basis(
                native_action, dist.concentration1, dist.concentration0
            )
            action_advantage = self.action_advantage_reasons(model_state[..., 1:], action_basis)
        return (
            action,
            native_action,
            log_prob,
    ...
    def action_critic_parameters(self):
        return list(self.reason_action_head.parameters())
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # The action field sees the updated representation but a completely frozen
        # old-rollout TD target and old-policy Jacobi basis. Representation is detached.
        action_indices = np.flatnonzero(action_train_mask.cpu().numpy())
        action_acc = {"loss": 0.0, "grad": 0.0, "n": 0}
        for _ in range(args.action_epochs):
            ...
            for start in range(0, len(action_indices), args.minibatch_size):
                ...
                prediction = agent.action_advantage_reasons(latent, b_action_bases[mb])
                loss = ((prediction - b_old_reason_td[mb]) / td_scale).square().mean()
                action_optimizer.zero_grad(set_to_none=True)
                loss.backward()
                action_grad = nn.utils.clip_grad_norm_(action_params, args.critic_grad_clip)
                action_optimizer.step()
                action_acc["loss"] += loss.item()
        ...
        with torch.no_grad():
            ...
            postupdate_holdout_prediction = agent.action_advantage_reasons(
                agent.encode(b_raw_obs[eval_mask]), b_action_bases[eval_mask]
            ).sum(dim=-1)
        ...
        writer.add_scalar("losses/action_reason_td", action_acc["loss"] / action_n, global_step)
