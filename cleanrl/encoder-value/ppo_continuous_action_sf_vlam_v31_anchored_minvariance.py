# REWARD-ANCHORED MINIMUM-VARIANCE policy credit from a LeJEPA successor critic.
#
# Base: v23_vector_preference (9,141 @8M on HalfCheetah-v4). The trunk, actor, rollout,
# LeJEPA workload, vector critic, per-dimension lambda machinery and PPO ratio clipping
# are unchanged. This file is v29's reward-anchored chassis with v29's correction
# mechanism replaced, for a measured reason stated below.
#
# THE CONTROLS. Three ways of scalarizing the vector critic have now been measured on
# this chassis:
#
#   v23_wgrad_scalargae      9,905 @6.6M   V = w.psi, PLAIN scalar GAE on the reward.
#   v23_vector_preference    9,141 @8M     unit ordinal covector on the vector innovation.
#   v29_reward_anchored      killed @1.4M  scalar GAE + a value-GRADIENT correction.
#
# THE CONSTRUCTION THAT SURVIVES. Make the reward a coordinate of phi:
#
#     phi_t = [ r_t , (1-gamma) e_t ]          (sf_dim = 1 + emb_dim)
#
# Coordinate 0 of psi is then EXACTLY V(s) and coordinate 0 of the vector TD(lambda)
# innovation E_t is EXACTLY the scalar GAE advantage A_t -- bit for bit, not an
# approximation (pinned by test_reward_coordinate_innovation_is_exactly_the_scalar_gae).
# So the best covector measured to date is known analytically. Pin it and learn only the
# correction:
#
#     A_t = E_0,t  +  < u(psi_t) , E_t[1:] / s[1:] >
#
# WHY v29 FAILED, AND WHAT CHANGES. v29 built the correction as residual_std * grad_psi g,
# with g regressed on the return psi_0 leaves over. That is a STATE-LEVEL regression,
# differentiated and then rescaled by a return-scale constant, and then added to an
# ADVANTAGE-scale anchor. Nothing in that construction sets the correction's magnitude
# relative to the thing it corrects. The logs say exactly this: anchor_correction_ratio
# ran 0.12 -> 274 -> 40 -> 2.9 -> 2.5 while residual_explained_variance never passed 0.18,
# and -- decisively -- credit/anchor_return_residual_corr (0.60-0.68, the bare anchor) was
# consistently HIGHER than credit/return_residual_corr (0.06-0.47, anchor + correction).
# The learned half was strictly destroying the exact half.
#
# The fix is to stop differentiating a state model and let least squares set the scale
# directly. This is boosting against the full-horizon advantage G_t - V(s_t): stage one
# is the exact anchor E_0, stage two is fit to what stage one leaves over, by partialling
# out (double-ML / partially-linear regression):
#
#     G_t - V(s_t) - E_0,t   ~   b(psi_t)  +  < u(psi_t) , E_t[1:] / s[1:] >
#
# b absorbs everything about the STATE -- what is left of the level, which no action at t
# controls -- so u is identified only by within-state covariation between the innovation
# and the realized return. u then comes out at ADVANTAGE scale by construction, because
# that is the scale of the target it is regressed onto. If the embedding innovation
# explains nothing the least-squares solution is u = 0 and the credit collapses to the
# exact scalar GAE, which is the control that wins. A unit-norm ordinal covector cannot
# do that; neither could a rescaled gradient.
#
# V(s_t) is subtracted rather than left for b to learn because psi_0 IS V(s_t), exactly,
# by construction of phi_0 -- so it costs nothing and it decides whether the design works
# at all. Left in, the target is return-scale (mean in the hundreds on unnormalized
# HalfCheetah reward), b starts at std=0.01 and cannot climb to it for most of the run,
# and every unit of level b misses is available to u -- because E[E_t | s_t] is the
# critic's TD bias, not zero and state-varying. An MSE has no preference between the two
# splits, so u would silently come out at return scale: the v29 failure again.
#
# INIT. u's output layer is std=0.01-initialized, so at iteration 1 the correction is
# ~7% of the anchor's RMS -- under 1% of its variance. The scalar-GAE control is
# therefore the starting point, not something this has to relearn. Iteration 1's fit is
# SKIPPED (critic_head is zero-init and bias-free, so psi is constant, b degenerates to a
# scalar and cannot absorb the level), which keeps that guarantee through iteration 2.
#
# COORDINATE 0 IS WITHHELD FROM u. u spans only coordinates 1..sf_dim-1. Given coordinate
# 0 the regression could simply re-derive the anchor with a shrunk weight, and the design
# question -- does the latent innovation carry advantage information the reward does not?
# -- would be unanswerable from the logs. Withholding it makes the correction strictly
# additive information.
#
# WHAT THE CORRECTION IS FOR. A_t^GAE is a lambda-return residual of ONE noisy scalar.
# The other coordinates are a TD-smoothed, SIGReg-whitened summary of where the trajectory
# went, with their own mixing times. Where the reward is a noisy readout of a smooth
# latent event (a gait phase completing, a fall beginning), the latent innovation sees it
# earlier and with less variance. That is the only place a gain over the scalar control
# can come from.
#
# CROSS-FIT. u is fit AFTER the PPO update and consumed on the NEXT rollout, so it never
# sees the actions it scores. Without that, u can memorize this rollout's action noise and
# hand the actor back its own sampling error as advantage.
#
# CRITIC LOSS SPLIT. reward_coord_coef of the critic loss is reserved for coordinate 0,
# the rest shared over the embedding coordinates. Uniform weighting would give the one
# coordinate the actor's advantage is built from 1/33 of the budget.
#
# NO WARM-UP. The covector is pinned to the exact reward direction from step 0, so the
# iteration-1 policy freeze is removed and the full critic trains from iteration 1, as in
# the scalar control.
#
# HYPOTHESIS. Anchoring at the best known covector and fitting the correction by least
# squares -- rather than by differentiating a state model -- gets the scalar control's
# 9,905 as a floor and the vector critic's smoothing as upside.
#
# DECISIVE FALSIFIER. credit/correction_gain = credit/return_residual_corr minus
# credit/anchor_return_residual_corr: the credit the actor consumed must correlate with
# realized advantage BETTER than the bare anchor does. v29 was persistently negative here.
# Read with readout/innovation_delta_explained_variance (residual R^2 that the innovation
# adds over b alone) and credit/anchor_correction_ratio. A correction that grows while the
# delta-R^2 sits at zero is noise re-injection; kill on that pair, not on return alone.
#
# Historical base notes follow; references to the ordinal preference field, to a
# reward-free phi and to a value-gradient correction are superseded above.
#
...
class Args:
    ...
    # --- Reward-anchored value-gradient policy credit ---------------------------------
    readout_hidden: int = 128
    readout_lr: float = 3e-4
    readout_epochs: int = 20
    readout_batch: int = 4096        # ~16k gated samples per rollout => ~80 steps/iter,
    #                                  the same order as the ordinal field's 80 pair steps
    readout_grad_clip: float = 1.0
    reward_coord_coef: float = 0.5   # share of the critic loss reserved for phi's reward
    #                                  coordinate. It is the coordinate the actor's
    #                                  anchored advantage is built from, so uniform 1/33
    #                                  weighting would under-fit exactly the coordinate
    #                                  that has to be right. 0.5 is the v13-v22 value.
    # The residual head is the ONLY place a return magnitude enters, its input is
    # hard-detached, and its target is Monte-Carlo (never a bootstrap), so it can neither
    # reshape the representation nor smuggle a scalar bootstrap into the vector critic.
    # Its regression gate is the existing mc_window mask, so a state contributes a target
    # only when its entire remaining episode -- at least mc_window rewards of it -- lies
    # in this rollout.
...
class AnchoredInnovationRegressor(nn.Module):
    """Least-squares correction to the anchored advantage.

    Models the return the anchor leaves over as

        G_t - E_0,t  ~  b(psi_t) + < u(psi_t) , E_t[1:] / s[1:] >

    with b and u two heads on one detached body. b absorbs the STATE level, so u is left
    with only the part the innovation -- i.e. the action -- explains, and least squares
    fixes u's SCALE from the residual it actually predicts. That is the whole difference
    from a value-gradient correction, which is a state-level regression differentiated and
    then multiplied by a return-scale constant, and therefore arrives at the policy two
    orders of magnitude too large.

    u covers the embedding block ONLY. Coordinate 0 is the reward, and its innovation is
    already the anchor; letting u touch it would let the correction unpick the one term
    that is exact. So the correction can only ever add information the reward coordinate
    cannot carry, which is the design's stated claim.

    Inputs are hard-detached here rather than relying on callers, and the only target is
    Monte-Carlo, so nothing here can reshape psi, the trunk or LeJEPA, or bootstrap.
    """

    def __init__(self, value_dim, hidden, covector_dim):
        super().__init__()
        ...
        self.baseline_head = layer_init(nn.Linear(hidden, 1), std=0.01)
        self.covector_head = layer_init(nn.Linear(hidden, covector_dim), std=0.01)

    def forward(self, standardized_psi):
        feature = self.body(standardized_psi.detach())
        return self.baseline_head(feature).squeeze(-1), self.covector_head(feature)

    def covector(self, standardized_psi):
        return self.covector_head(self.body(standardized_psi.detach()))


def fit_anchored_innovation_regressor(
    regressor,
    optimizer,
    standardized_psi,
    standardized_trace,
    ...
    """Fit G - V - E_0 ~ b(psi) + <u(psi), E[1:]/s[1:]>. u is what the policy consumes.

    use_covector=False fits b ALONE, which is the honest control for the incremental R^2.
    Reading b's fit out of the JOINT model instead would be biased in the one direction
    that matters: whenever u wrongly absorbs state level, the joint b shrinks, its R^2
    drops, and the reported "innovation delta" rises -- the metric would look best exactly
    when the design is failing.
    """
    n = standardized_psi.shape[0]
    if n == 0:
        return float("nan"), float("nan"), 0
    loss_sum = grad_sum = 0.0
    steps = 0
    for _ in range(epochs):
        permutation = torch.randperm(n, device=standardized_psi.device, generator=generator)
        for start in range(0, n, batch_size):
            index = permutation[start : start + batch_size]
            baseline, covector = regressor(standardized_psi[index])
            prediction = baseline
            if use_covector:
                prediction = prediction + (covector * standardized_trace[index]).sum(-1)
            loss = F.mse_loss(prediction, targets[index])
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            grad_norm = nn.utils.clip_grad_norm_(regressor.parameters(), grad_clip)
            optimizer.step()
            loss_sum += loss.item()
            grad_sum += float(grad_norm)
            steps += 1
    return loss_sum / steps, grad_sum / steps, steps


@torch.no_grad()
def explained_variance_1d(prediction, target):
    variance = target.var(unbiased=False)
    if target.numel() < 2 or float(variance) < 1e-12:
        return float("nan")
    return float(1.0 - (prediction - target).var(unbiased=False) / variance)


@torch.no_grad()
def credit_return_correlation(credit, returns, valid):
    """corr(consumed policy credit, horizon-matched realized return residual).

    Every environment resets on the same rollout step on the fixed-length MuJoCo tasks,
    so the mean return over environments AT A GIVEN t is an exact horizon-matched
    baseline; subtracting it leaves the realized advantage. Any credit signal that
    carries policy information has to correlate with it, whatever route it took. This is
    the one metric on which this design and the ordinal-preference design are directly
    comparable, and it is the decisive falsifier for both.
    """
    residual = returns - returns.mean(1, keepdim=True)
    x, y = credit[valid], residual[valid]
    if x.numel() < 2:
        return float("nan")
    x = x - x.mean()
    y = y - y.mean()
    denominator = (x.square().sum().sqrt() * y.square().sum().sqrt()).clamp_min(1e-8)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Coordinate 0 is the observed reward, so psi_0 is V(s) by definition and the
        # coordinate-0 TD(lambda) innovation is the scalar GAE advantage exactly.
        self.sf_dim = args.emb_dim + 1
...
if __name__ == "__main__":
    ...
    if args.num_envs < 2:
        raise ValueError(
            "the horizon-matched return-residual falsifier needs at least two parallel "
            "environments"
        )
    if args.readout_batch < 1 or args.readout_epochs < 1:
        raise ValueError("the value readout needs at least one epoch over one minibatch")
    if not args.separate_grad_clip:
        raise ValueError(
            "the anchored covector's actor/critic gradient accounting requires "
            "separate_grad_clip"
        )
    ...
    if auto_alpha:
        raise ValueError(
            ...
            "vector-credit design; use the Beta actor or disable auto_entropy"
        )
    if auto_alpha:
        act_dim = int(np.prod(envs.single_action_space.shape))
    ...
    # value readout so actor sampling remains paired with the strongest control.
    ssl = LeJepaSSL(obs_dim, act_dim_sf, args).to(device)
    with torch.random.fork_rng(devices=[]):
        readout = AnchoredInnovationRegressor(
            sf_dim, args.readout_hidden, sf_dim - 1
        ).to(device)
        # Diagnostic only. Same architecture, same data, same schedule, but fit WITHOUT
        # the innovation term, so readout_ev - baseline_ev is a real incremental R^2
        # rather than a comparison against a head that was trained alongside a competitor.
        readout_control = AnchoredInnovationRegressor(
            sf_dim, args.readout_hidden, sf_dim - 1
        ).to(device)
    ...
    readout_optimizer = optim.Adam(readout.parameters(), lr=args.readout_lr, eps=1e-5)
    readout_control_optimizer = optim.Adam(
        readout_control.parameters(), lr=args.readout_lr, eps=1e-5
    )
    readout_generator = torch.Generator(device=device)
    readout_generator.manual_seed(args.seed + 17021)
    ...
    if args.compile:
        ...
        # The readout is deliberately NOT compiled: it is a 3-layer MLP whose dominant
        # use is a per-input autograd.grad inside the no-grad rollout block, which is
        # exactly the pattern dynamo recompiles on.
    ...
    sf_stat_count = 0
    # The readout's own frozen copy of the psi standardizer. sf_mean/sf_std move every
    # iteration, so evaluating the readout next iteration under the CURRENT standardizer
    # would feed it inputs from a distribution it was never fit on. Snapshotting at fit
    # time keeps the covector's input frame and the readout's training frame identical.
    readout_mean = sf_mean.clone()
    readout_std = sf_std.clone()
    # Return standardizer for the readout's regression target, same 1/count warmup as the
    # psi standardizer. Only conditioning: the credit is a DIFFERENCE of readout values,
    # so the location cancels and the scale is divided out by the credit RMS.
    # Per-coordinate critic loss weights. Coordinate 0 is the reward, and it is the one
    # the actor's anchored advantage is read from, so it gets its own share instead of
    # 1/sf_dim of a uniform mean.
    critic_coord_weight = torch.full(
        (sf_dim,), (1.0 - args.reward_coord_coef) / (sf_dim - 1), device=device
    )
    critic_coord_weight[0] = args.reward_coord_coef
    ...
    # No warm-up: the anchor covector is exact from step 0 (psi zero-init makes the
    # coordinate-0 innovation the plain discounted reward sum, a clean Monte-Carlo
    # advantage), so the policy and the full critic train from iteration 1 exactly as in
    # the scalar-GAE control. There is no iteration on which a random covector drives the
    # actor, which is the only thing v23's and v28's freeze existed to prevent.
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            readout_optimizer.param_groups[0]["lr"] = frac * args.readout_lr
            readout_control_optimizer.param_groups[0]["lr"] = frac * args.readout_lr
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, _ = agent.get_action_and_value(next_obs)
            actions[step] = action
            latent_zs[step] = z
        ...
        with torch.no_grad():
            ...
            psi_std_cur = (
                agent.critic_head(critic_feat_buf)
                .view(-1, args.critic_mtp_horizon, sf_dim)[:, 0]
                .reshape(args.num_steps, args.num_envs, sf_dim)
            )
            psi_cur = psi_raw(psi_std_cur)
            ...
            # phi = [r, (1-gamma) e]. The (1-gamma) keeps the embedding block's discounted
            # sum O(1) as gamma -> 1; the reward coordinate deliberately carries NO such
            # factor, so psi_0 is the ordinary discounted return and is directly
            # comparable with mc_ret. Per-coordinate standardization handles the scale gap
            # for the head.
            phi = torch.cat(
                [rewards.unsqueeze(-1), (1.0 - args.gamma) * emb_buf], dim=-1
            )
            ...
            # ---- reward-anchored least-squares covector -------------------------------
            # A_t = E_0,t + < u(psi_t) , E_t[1:] / s[1:] >.
            #
            # The anchor is exact and unlearned; u is the least-squares covector fit on
            # the PREVIOUS rollout, so its scale comes from the residual it predicts
            # rather than from a return-scale constant. Both terms are folded into ONE
            # covector contracted against the standardized innovation E / s, so the
            # minibatch loop stays a single dot product: the anchor is e_0 scaled by
            # s_0, which contracts to exactly E_0.
            # The head's OWN output, not psi re-standardized. psi_cur was built with the
            # standardizer in force at the top of this iteration, and the fit ran with the
            # one snapshotted at the END of the previous iteration -- which are the same
            # tensors, so (psi_cur - readout_mean)/readout_std would cancel here but NOT at
            # fit time, where sf_mean/sf_std have already been advanced. Using the head
            # output at both sites makes the regressor's input frame identical by
            # construction rather than by a cancellation that only half holds.
            standardized_psi = psi_std_cur.reshape(-1, sf_dim)
            correction_covector = readout.covector(standardized_psi).reshape(
                args.num_steps, args.num_envs, sf_dim - 1
            )
            frozen_credit_direction = torch.zeros(
                args.num_steps, args.num_envs, sf_dim, device=device
            )
            frozen_credit_direction[..., 0] = readout_std[0]
            frozen_credit_direction[..., 1:] = correction_covector
            standardized_trace = vector_policy_trace / readout_std
            policy_credit = (frozen_credit_direction * standardized_trace).sum(-1)
            anchor_credit = vector_policy_trace[..., 0]
            correction_credit = policy_credit - anchor_credit

        b_obs = obs.reshape((-1,) + envs.single_observation_space.shape)
        b_logprobs = logprobs.reshape(-1)
        b_actions = actions.reshape((-1,) + envs.single_action_space.shape)
        b_latent_zs = latent_zs.reshape((-1,) + envs.single_action_space.shape)
        # The covector is folded into the standardizer once here, so the minibatch loop
        # forms the credit with a single dot product and cannot drift from the value
        # computed above.
        b_vector_trace = (vector_policy_trace / readout_std).reshape(-1, sf_dim)
        b_credit_direction = frozen_credit_direction.reshape(-1, sf_dim)
        b_policy_credit = policy_credit.reshape(-1)
        ...
        # One rollout-wide divide-only scale. This is not coordinate whitening and does
        # not center away the covector's learned zero or sign.
        policy_credit_rms = b_policy_credit.square().mean().sqrt().clamp_min(1e-8)

        b_inds = np.arange(args.batch_size)
        clipfracs = []
        # No warm-up gate: see the note at readout construction. The critic trains in
        # full from iteration 1.
        active_critic_params = critic_params
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # The policy consumes the covector and the vector trace directly. The sum
                # is a local directional derivative of the value surface -- a lambda-return
                # advantage in return units -- not a decoded state value. Scale is
                # rollout-global and divide-only; no centering or coordinate whitening.
                mb_advantages = (
                    b_credit_direction[mb_inds] * b_vector_trace[mb_inds]
                ).sum(-1) / policy_credit_rms
                ...
                v_loss = (
                    (sf_err.square() * critic_coord_weight).sum(-1) * value_mask
        ...
        # ---- calibrated latent value readout ------------------------------------------
        # The actor consumed the covector as it existed BEFORE seeing this rollout. Refit
        # only after the policy update, for use next iteration. This one-iteration
        # cross-fit prevents the readout from explaining this rollout's action noise and
        # immediately feeding that memorized explanation back as policy credit.
        #
        # The standardizer is snapshotted HERE, together with the fit, so the INNOVATION
        # the covector is contracted against next iteration is divided by the same s it
        # was fit against. (u's psi input needs no standardizer at all: it is the critic
        # head's own output, which is already in standardized units at both sites.)
        readout_mean = sf_mean.clone()
        readout_std = sf_std.clone()
        with torch.no_grad():
            refit_standardized_psi = psi_std_cur.reshape(-1, sf_dim)
            # ---- fit gate ------------------------------------------------------------
            # mc_window keeps the Monte-Carlo truncation bias below the noise floor, but
            # on a task whose episodes END before mc_window the gate can be EMPTY. Fall
            # back to every state whose remaining episode is inside the rollout (a
            # truncation-biased target beats no correction at all), then to every state,
            # so the correction never silently stops being fit.
            fit_gate = mc_mask
            if not bool(fit_gate.any()):
                fit_gate = mc_complete.reshape(-1)
            if not bool(fit_gate.any()):
                fit_gate = torch.ones_like(mc_mask)
            fit_psi = refit_standardized_psi[fit_gate]
            # The embedding block of the standardized innovation -- the regressor's only
            # view of the action. Coordinate 0 is withheld: it is the anchor, and it is
            # already exact.
            fit_trace = (vector_policy_trace / readout_std).reshape(-1, sf_dim)[
                fit_gate
            ][:, 1:]
            # Boosting against the FULL-HORIZON advantage G_t - V(s_t). Stage one is the
            # anchor E_0 (exact, unlearned); the correction is fit to what stage one
            # leaves over. Subtracting psi_0 -- which IS V(s_t), by construction of phi_0 --
            # matters for conditioning, not for the population answer: it moves the whole
            # value function out of b's job, leaving b only the critic's TD bias.
            #
            # Without it the target is return-scale, mean in the hundreds on unnormalized
            # HalfCheetah reward, and b (std=0.01 init, lr 3e-4, ~80 steps per iteration)
            # cannot climb to it for most of the run. Every unit of level b misses is
            # available to u, because E[E_t | s_t] is the critic's TD bias -- not zero and
            # state-varying -- so u would represent state level, which is exactly the v29
            # failure with a different mechanism. Nothing in an MSE prefers the b split.
            fit_targets = (
                mc_ret - psi_cur[..., 0] - vector_policy_trace[..., 0]
            ).reshape(-1)[fit_gate]
            # Iteration 1: critic_head is zero-init and bias-free, so every fit row of psi
            # is the SAME constant. b then degenerates to a scalar and absorbs only the
            # target's mean, not its cross-sectional level -- so u is identified against a
            # return-scale target and comes out return-scale, and the next iteration's
            # actor spends a full PPO update on a correction that swamps the exact anchor
            # (policy_credit_rms is divide-only, so it dilutes rather than clips). Skip the
            # fit instead; u stays at its ~0 init and the credit stays the scalar GAE,
            # which is the floor this design is built to guarantee.
            psi_is_degenerate = bool(
                fit_psi.shape[0] < 2 or float(fit_psi.std(0).max()) < 1e-6
            )
        if psi_is_degenerate:
            readout_loss_mean = readout_grad_norm_mean = float("nan")
            readout_steps = 0
        else:
            readout_loss_mean, readout_grad_norm_mean, readout_steps = (
                fit_anchored_innovation_regressor(
                    readout,
                    readout_optimizer,
                    fit_psi,
                    fit_trace,
                    ...
                    args.readout_epochs,
                    args.readout_batch,
                    args.readout_grad_clip,
                    readout_generator,
                )
            )
            # Same data, same schedule, no innovation term. Diagnostic only -- its
            # parameters never reach the actor.
            fit_anchored_innovation_regressor(
                readout_control,
                readout_control_optimizer,
                fit_psi,
                fit_trace,
                fit_targets,
                args.readout_epochs,
        ...
        with torch.no_grad():
            # Post-fit diagnostics. baseline_ev comes from the SEPARATELY fit
            # baseline-only control, not from this model's own b -- see the note on
            # fit_anchored_innovation_regressor. Their difference is the ONLY thing this
            # design claims exists: residual advantage the embedding innovation explains
            # once the anchor and the state are both partialled out. If it is zero, the
            # correction should be zero too, and anchor_correction_ratio says whether it is.
            fit_baseline, fit_covector = readout(fit_psi)
            readout_ev = explained_variance_1d(
                fit_baseline + (fit_covector * fit_trace).sum(-1), fit_targets
            )
            baseline_ev = explained_variance_1d(readout_control(fit_psi)[0], fit_targets)
            innovation_delta_ev = readout_ev - baseline_ev
            readout_target_count = int(fit_targets.numel())
            # How large the learned correction is next to the exact anchored advantage.
            # v29 -- the same anchor with a value-GRADIENT correction -- sat at 2.5-270
            # here while explaining under 0.2 of the residual, and its total credit
            # correlated WORSE with realized advantage than the bare anchor did. That
            # comparison, anchor_credit_corr vs consumed_credit_corr, is the decisive
            # falsifier: a correction that does not beat its own anchor is damage.
            anchor_rms = anchor_credit.square().mean().sqrt()
            correction_rms = correction_credit.square().mean().sqrt()
            anchor_correction_ratio = float(correction_rms / anchor_rms.clamp_min(1e-8))
            anchor_credit_corr = credit_return_correlation(
                anchor_credit, returns, mc_complete
            )
            refit_correction_covector = readout.covector(refit_standardized_psi).reshape(
                args.num_steps, args.num_envs, sf_dim - 1
            )
            trained_credit_direction = torch.zeros_like(frozen_credit_direction)
            trained_credit_direction[..., 0] = readout_std[0]
            trained_credit_direction[..., 1:] = refit_correction_covector
            # Measured on the LEARNED half alone. The full covector carries readout_std[0]
            # in coordinate 0 -- a constant, sample-independent entry at return scale -- so
            # an uncentered cosine, resultant or norm over it reads ~1 no matter what the
            # correction does, i.e. is blind to the only part that moves.
            frozen_learned = correction_covector.reshape(-1, sf_dim - 1)
            trained_learned = refit_correction_covector.reshape(-1, sf_dim - 1)
            frozen_flat = frozen_credit_direction.reshape(-1, sf_dim)
            direction_update_cosine = float(
                F.cosine_similarity(frozen_learned, trained_learned, dim=-1).mean()
            )
            credit_direction_rank = effective_rank(trained_learned)
            credit_direction_resultant = float(
                F.normalize(trained_learned, dim=-1, eps=1e-8).mean(0).norm()
            )
            credit_direction_norm = float(trained_learned.norm(dim=-1).mean())
            # The falsifier, measured on the credit the ACTOR consumed this iteration.
            consumed_credit_corr = credit_return_correlation(
                policy_credit, returns, mc_complete
            )
            # Same statistic for the covector one iteration ahead. IN-SAMPLE -- the
            # covector was just fit on these states against targets built from these
            # returns -- so it is optimistically biased and is NOT the falsifier. Read it
            # only against consumed_credit_corr, whose covector never saw this rollout.
            trained_credit = (
                trained_credit_direction * (vector_policy_trace / readout_std)
            ).sum(-1)
            trained_credit_corr = credit_return_correlation(
                trained_credit, returns, mc_complete
            )
            ...
            # effective_rank centers but does not scale, and raw coordinate 0 (the
            # undiscounted return, deliberately without the (1-gamma) factor) carries
            # 10^3-10^5 times the variance of an embedding coordinate. Ranked raw, both
            # of these would read ~1 by construction and say nothing about the embedding
            # block. Standardize first, which is the space the head regresses in anyway.
            latent_target_rank = effective_rank((latent_target_flat - sf_mean) / sf_std)
            latent_prediction_rank = effective_rank(
                (latent_prediction_flat - sf_mean) / sf_std
            )
        ...
        with torch.no_grad():
            vector_trace_flat = b_vector_trace
            vector_trace_norm = vector_trace_flat.norm(dim=-1)
            credit_score = b_policy_credit
            credit_score_rms = credit_score.square().mean().sqrt()
            # On the LEARNED block only. The full covector carries readout_std[0] in
            # coordinate 0 -- raw return scale, orders of magnitude above the rest -- so a
            # cosine over it is pinned near 1 and blind to the correction.
            correction_flat = b_credit_direction[:, 1:]
            credit_score_cosine = (correction_flat * b_vector_trace[:, 1:]).sum(-1) / (
                correction_flat.norm(dim=-1) * b_vector_trace[:, 1:].norm(dim=-1)
            ).clamp_min(1e-8)
            coordinate_energy = (frozen_flat * vector_trace_flat).square()
            effective_coordinates = (
                coordinate_energy.sum(-1).square()
                / coordinate_energy.square().sum(-1).clamp_min(1e-12)
            ).mean()
        ...
        # ---- vector policy credit and least-squares covector geometry ----------
        ...
        writer.add_scalar("vector_adv/effective_contributing_coordinates", effective_coordinates.item(), global_step)
        writer.add_scalar("readout/mse", readout_loss_mean, global_step)
        writer.add_scalar("readout/residual_explained_variance", readout_ev, global_step)
        # THE decisive falsifier: the correction must beat its own anchor, not just exist.
        writer.add_scalar(
            "credit/correction_gain",
            consumed_credit_corr - anchor_credit_corr,
            global_step,
        )
        writer.add_scalar("readout/grad_norm", readout_grad_norm_mean, global_step)
        writer.add_scalar("readout/steps", readout_steps, global_step)
        writer.add_scalar("readout/regression_targets", readout_target_count, global_step)
        writer.add_scalar("readout/baseline_explained_variance", baseline_ev, global_step)
        writer.add_scalar(
            "readout/innovation_delta_explained_variance", innovation_delta_ev, global_step
        )
        writer.add_scalar("credit/anchor_correction_ratio", anchor_correction_ratio, global_step)
        writer.add_scalar("credit/anchor_return_residual_corr", anchor_credit_corr, global_step)
        writer.add_scalar("credit/direction_effective_rank", credit_direction_rank, global_step)
        writer.add_scalar("credit/direction_resultant_length", credit_direction_resultant, global_step)
        writer.add_scalar("credit/direction_norm_mean", credit_direction_norm, global_step)
        writer.add_scalar("credit/direction_update_cosine", direction_update_cosine, global_step)
        # THE FALSIFIER. Correlation of the credit the actor consumed with the
        # horizon-matched realized return residual, and the same for the covector one
        # iteration ahead.
        writer.add_scalar("credit/return_residual_corr", consumed_credit_corr, global_step)
        writer.add_scalar("credit/return_residual_corr_refit", trained_credit_corr, global_step)
        writer.add_scalar("credit/score_mean", credit_score.mean().item(), global_step)
        writer.add_scalar("credit/score_rms", credit_score_rms.item(), global_step)
        writer.add_scalar("credit/score_positive_fraction", (credit_score > 0).float().mean().item(), global_step)
        writer.add_scalar("credit/score_trace_cosine", credit_score_cosine.mean().item(), global_step)
