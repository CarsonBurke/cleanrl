# PPO + LeJEPA GHM with Rao-Blackwellized mean and grounded-prefix credit v3.
# =============================================================================
...
#
# MC-CFM remains the distributional objective.  Its one-geometric-future sample is
# unbiased for the successor mean but has almost the full variance of the outcome
# distribution, and v2 learned only +0.017 held-out skill over an unconditional
# marginal.  The mean head now receives the Rao-Blackwellized normalized occupancy:
#
#   m_t = (1-gamma)y_t + gamma m_{t+1}.
#
# Every observed suffix outcome contributes once.  A frozen GHM mean supplies only
# the unobserved tail at a truncation or rollout edge; a true termination has a zero
# absorbing tail.  This is a conditional mean of the infinite geometric outcome
# distribution, not an unnormalized cumulative embedding and not actor full-suffix
# credit.  The actor still uses v2's fixed 32-step innovation prefix.
#
...
def rao_blackwellized_occupancy_targets(
    outcomes,
    frozen_next_means,
    terminations,
    boundaries,
    ...
    """Exact suffix mean with a frozen learned tail only where observation ends.

    Normal transitions recurse into the next real outcome.  Time-limit
    truncations and the rollout edge bootstrap from the supplied final/next
    observation.  True terminations use the zero absorbing outcome.  Invalid
    outcome rows propagate invalidity backward through their current episode.
    """
    ...
    for t in range(outcomes.shape[0] - 1, -1, -1):
        ...
        rollout_edge = t == outcomes.shape[0] - 1
        if rollout_edge:
            tail = frozen_next_means[t]
...
def main():
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            rb_mean_targets, rb_mean_valids = rao_blackwellized_occupancy_targets(
                outcomes,
                next_means,
                termination_buf,
                boundary_buf,
        ...
        flat_rb_mean_target = rb_mean_targets.reshape(-1, outcome_dim)
        flat_rb_mean_valid = rb_mean_valids.reshape(-1)
        ...
        for _ in range(args.ghm_epochs):
            ...
            for start in range(0, train_indices.numel(), args.ghm_batch):
                ...
                ghm_mean_loss = cfm_regression_loss(
                    mean_pred,
                    flat_rb_mean_target[mb],
                    scales,
                    args.emb_dim,
                    act_dim,
                    flat_rb_mean_valid[mb],
                )
        ...
        with torch.no_grad():
            ...
            diag_rb_target = flat_rb_mean_target[holdout_indices]
            diag_rb_valid = flat_rb_mean_valid[holdout_indices]
            ghm_heldout_rb_mse = cfm_regression_loss(
                diag_pred,
                diag_rb_target,
                scales,
                args.emb_dim,
            ...
            valid_rb_targets = diag_rb_target[diag_rb_valid]
            if valid_rb_targets.shape[0] > 0:
                rb_marginal = valid_rb_targets.mean(0, keepdim=True).expand_as(
                    diag_rb_target
                )
                ghm_rb_marginal_mse = cfm_regression_loss(
                    rb_marginal,
                    diag_rb_target,
                    scales,
                    args.emb_dim,
                ...
                ghm_rb_marginal_mse = torch.zeros((), device=device)
        ...
        writer.add_scalar("ghm/heldout_rb_mse", ghm_heldout_rb_mse.item(), global_step)
        writer.add_scalar(
            "ghm/heldout_rb_unconditional_mse", ghm_rb_marginal_mse.item(), global_step
        )
        writer.add_scalar(
            "ghm/heldout_rb_skill_vs_unconditional",
            (1.0 - ghm_heldout_rb_mse / ghm_rb_marginal_mse.clamp_min(1e-8)).item(),
            global_step,
        )
