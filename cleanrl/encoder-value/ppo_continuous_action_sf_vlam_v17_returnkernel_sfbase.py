# PPO + the v9 successor-feature critic + a gated learned return-kernel auxiliary.
#
...
# bootstrapped sampled return. It is head-only until frozen held-out prediction has
# nontrivial NMSE, scale, and rank. Once admitted, its trunk gradient is orthogonalized
# against both the SF critic and actor gradients and norm-capped. The return encoder's
# decorrelation uses the true normalization derivative; v16 detached it and collapsed.
#
...
class Args:
    ...
    rich_grad_clip: float = 0.05
    rich_trunk_ratio: float = 0.25
    rich_holdout_envs: int = 2
    rich_gate_ev: float = 0.05
    rich_gate_target_rank: float = 2.0
    rich_gate_prediction_rank: float = 1.5
    rich_gate_min_target_std: float = 0.25
    rich_gate_max_target_std: float = 2.0
...
class ReturnKernelEncoder(nn.Module):
    """Learn nonlinear coordinates of one standardized bootstrapped return."""
...
def passes_rich_gate(
    heldout_ev,
    target_rank,
    prediction_rank,
    target_coordinate_std,
    prediction_coordinate_std,
    args,
):
    """Admission requires every coordinate to be live, not merely the average."""
    return (
        heldout_ev > args.rich_gate_ev
        and target_rank > args.rich_gate_target_rank
        and prediction_rank > args.rich_gate_prediction_rank
        and float(target_coordinate_std.min()) > args.rich_gate_min_target_std
        and float(target_coordinate_std.max()) < args.rich_gate_max_target_std
        and float(prediction_coordinate_std.min())
        > args.rich_gate_min_prediction_std
    )
...
def protect_auxiliary_trunk_gradient(
    ...
    """Remove the critic/actor gradient span, then cap the auxiliary norm."""
    device = trunk_parameters[0].device
    reference_sq = sum(
        (gradient.double().square().sum() for gradient in cap_reference_grads.values()),
        ...
        for parameter in trunk_parameters:
            if parameter in auxiliary_grads and parameter in basis:
                auxiliary_grads[parameter].sub_(
                    basis[parameter] * coefficient.to(basis[parameter].dtype)
                )
    ...
    scale = torch.clamp(
        max_ratio * reference_sq.sqrt()
        / projected_sq.sqrt().clamp_min(1e-20),
        max=1.0,
    )
    for parameter in trunk_parameters:
        if parameter in auxiliary_grads:
...
if __name__ == "__main__":
    ...
    if not args.separate_grad_clip:
        raise ValueError("v17 requires separate_grad_clip for protected rich gradients")
    ...
    with torch.random.fork_rng(devices=[]):
        rich_value_head = layer_init(
            nn.Linear(args.hidden, args.return_code_dim, bias=False), std=0.1
        )
        with torch.no_grad():
            rich_value_head.weight.zero_()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Separate rich-return target. It never changes the v9 scalar GAE, SF
            # recursion, readout, or policy advantage.
            ...
            # Temporal holdout: this head has not trained on the current rollout yet.
            environment_index = torch.arange(args.num_envs, device=device)
            holdout_environment = environment_index < args.rich_holdout_envs
            rich_holdout = sampled_valid & holdout_environment.unsqueeze(0)
            preupdate_rich_prediction = rich_value_head(critic_feat_buf).reshape(
                args.num_steps, args.num_envs, args.return_code_dim
            )
            held_target = rich_target[rich_holdout]
            held_prediction = preupdate_rich_prediction[rich_holdout]
            ...
            if rich_trunk_enabled:
                rich_gate_fail_count = (
                    0 if gate_passes else rich_gate_fail_count + 1
                )
        ...
        b_rich_holdout = rich_holdout.reshape(-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                rich_prediction = rich_value_head(
                    critic_feat if rich_trunk_enabled else critic_feat.detach()
                )
                ...
                if args.separate_grad_clip:
                    # Preserve v9's independently clipped SF and actor gradients. The
                    # rich head gets a third backward; its trunk component is absent
                    # while gated and orthogonalized/capped once admitted.
                    ...
                    rich_parameters = list(rich_value_head.parameters())
                    if rich_trunk_enabled:
                        rich_parameters = shared_trunk_params + rich_parameters
                    rich_gn = nn.utils.clip_grad_norm_(
                        rich_parameters, args.rich_grad_clip
                    )
                    rich_grads = {
                        p: p.grad.detach().clone()
                        for p in rich_parameters
                        if p.grad is not None
                    }
                    ...
                    if rich_trunk_enabled:
                        ...
                        primary = {}
                        for p in shared_trunk_params:
                            critic_gradient = critic_trunk_grads.get(p)
                            actor_gradient = actor_trunk_grads.get(p)
                            if critic_gradient is not None or actor_gradient is not None:
                                primary[p] = (
                                    torch.zeros_like(p)
                                    if critic_gradient is None
                                    else critic_gradient.detach().clone()
                                )
                                if actor_gradient is not None:
                                    primary[p].add_(actor_gradient)
                        cosines, retained = protect_auxiliary_trunk_gradient(
                            [critic_trunk_grads, actor_trunk_grads],
                            rich_grads,
                            shared_trunk_params,
                            primary,
                            args.rich_trunk_ratio,
                        )
                        rich_critic_cosines.append(cosines[0])
                        rich_actor_cosines.append(cosines[1])
                        rich_trunk_retained.append(retained)

                    # This addition is v9's exact update when the rich gate is closed.
                        ...
                        for p in shared_trunk_params:
                            if p in rich_grads:
                                p.grad = (
                                    rich_grads[p]
                                    if p.grad is None
                                    else p.grad + rich_grads[p]
                                )
                    for p in rich_value_head.parameters():
                        p.grad = rich_grads.get(p)
                    optimizer.step()
                    rich_optimizer.step()
        ...
        # Learn a nonlinear basis of scalar return outcomes only. The code is not
        # regressed toward the state predictor; doing so creates a moving
        # self-distillation fixed point and invites collapse.
        ...
        with torch.no_grad():
            frozen_target_before_ema = rich_target.reshape(
                -1, args.return_code_dim
            )[b_rich_holdout]
            ema_update(
            ...
            target_after_ema = target_return_encoder(
                b_standardized_sampled_return[b_rich_holdout]
            )
            return_encoder_frame_rmse = float(
                (
                    target_after_ema - frozen_target_before_ema
                ).square().mean().sqrt()
            )
        ...
        writer.add_scalar(
            "rich_value/trunk_enabled",
            float(rich_trunk_enabled),
            global_step,
        )
        writer.add_scalar(
            "rich_value/gate_count", rich_gate_count, global_step
        )
        writer.add_scalar(
            "rich_value/gate_fail_count", rich_gate_fail_count, global_step
        )
        writer.add_scalar(
            "rich_value/critic_cosine",
            float(torch.stack(rich_critic_cosines).mean())
            if rich_critic_cosines
            else 0.0,
            global_step,
        )
        writer.add_scalar(
            "rich_value/actor_cosine",
        ...
        writer.add_scalar(
            "rich_value/standardized_return_absmax",
            b_standardized_sampled_return[b_rich_valid].abs().max().item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/return_std", return_std.item(), global_step
        )
