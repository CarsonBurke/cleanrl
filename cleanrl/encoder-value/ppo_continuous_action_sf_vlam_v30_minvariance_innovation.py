# MINIMUM-VARIANCE innovation credit from a LeJEPA successor critic.
#
...
# and PPO ratio clipping are unchanged. Only the covector that scalarizes the vector
# TD(lambda) innovation differs.
#
# THE QUESTION THIS ANSWERS. Given a D-dimensional innovation E_t and a policy that can
# consume exactly one number per sample, which covector should collapse it? v23 answers
# "the unit direction that orders returns". v28 answers "the value gradient, grad_psi V".
# Both are wrong in the same way, and it is a statistical error rather than a modelling
# one.
#
# grad V says how much return a unit of psi is WORTH. It says nothing about how much of
# E is signal. Suppose one coordinate of the innovation is valuable but almost pure
# action noise, and another is half as valuable but nearly deterministic given the state.
# grad V weights the first one twice as heavily. The estimator that minimizes the
# variance of the credit weights them by the opposite consideration too -- and the object
# that does this is the REGRESSION of realized advantage on the innovation, not the
# gradient of the value surface:
#
#     u*(s) = argmin_u  E[ ( delta_t - < u , E_t > )^2 ]  =  Cov(E,delta) / Var(E),
#
# the best linear predictor of the realized return residual from the innovation. Where
# grad V multiplies noise coordinates by their worth, u* shrinks them by their unreliab-
# ility. They coincide only when the innovation is isotropic, which SIGReg whitens e
# toward but psi -- a discounted sum with per-coordinate mixing times spanning tau ~ 1 to
# tau ~ 20 -- is emphatically not.
#
# THE ESTIMATOR. delta_t is not observable, so do not estimate it separately: fit it
# jointly as a partially-linear model of the truncated Monte-Carlo return,
#
#     G_t  ~  b(psi_t)  +  < u(psi_t) , E_t / s >,
#
# with b and u two heads on one detached body over psi. b absorbs everything about the
# STATE, so u is left with exactly the part of the return that the innovation -- i.e. the
# action -- explains. This is the partialling-out step of double ML, and it is what makes
# u an advantage covector rather than a state-value one. The credit consumed by the
# policy is the second term alone.
#
# WHY THIS IS THE HONEST VERSION OF v23'S IDEA. v23 also wanted a state-conditioned
# covector supervised by returns, but paired the STATE-level label (G_i - G_j) with an
# ACTION-level feature (E_i - E_j) across two different states, so the baseline gap
# entered as label noise. Here the baseline is a fitted function rather than a nearest
# neighbour among fifteen, the label keeps its magnitude, and u is by construction the
# minimum-variance scalarization instead of a unit vector.
#
# WHAT IS PRESERVED. The body is hard-detached from psi, the trunk and LeJEPA; the
# target is Monte-Carlo, gated to states whose remaining episode is at least mc_window
# rewards deep in the rollout, so nothing here bootstraps. The one-iteration cross-fit is
# kept verbatim -- (b, u) is refit AFTER the policy update and consumed on the NEXT
# rollout -- which is what stops u from memorizing this rollout's action noise and
# feeding it straight back as credit. That cross-fit matters more here than anywhere
# else: u is fit to explain realized returns from realized innovations, which is exactly
# the quantity that overfits.
#
# HYPOTHESIS. Weighting each innovation coordinate by how reliably it predicts return,
# rather than by how much return it is worth, lowers the variance of the policy credit
# enough to beat both the ordinal field (9,141) and the scalar-GAE control (9,905).
#
# DECISIVE FALSIFIER. readout/innovation_delta_explained_variance -- the R^2 the
# innovation term adds on top of b(psi) alone. This is the exact quantity the design
# claims exists; if it sits at zero, the vector innovation carries no return information
# beyond the state and NO covector can help, which also retires v23 and v28's premise.
# credit/return_residual_corr is the shared cross-variant falsifier.
#
# Historical base notes follow; references to the ordinal preference field and to scalar
# evaluation are superseded above.
#
...
class Args:
    ...
    # --- Minimum-variance innovation policy credit ------------------------------------
    ...
    # The partially-linear regressor is the ONLY place a return magnitude enters, its
    # inputs are hard-detached, and its target is Monte-Carlo (never a bootstrap), so it
    # can neither reshape the representation nor smuggle a scalar bootstrap into the
    # vector critic. Its regression gate is the existing mc_window mask, so a state
    # contributes a target only when its entire remaining episode -- at least mc_window
    # rewards of it -- lies in this rollout.
...
class InnovationAdvantageRegressor(nn.Module):
    """Partially-linear model of the return: G ~ b(psi) + < u(psi) , E_standardized >.

    Two heads on one body. b is a state baseline that absorbs everything about WHERE the
    trajectory is; u is then the minimum-variance covector for the part of the return
    that the innovation -- i.e. the action -- explains. Only u reaches the policy.

    Both inputs are hard-detached here rather than relying on callers to preserve the
    one-way boundary, and the only target is a Monte-Carlo return, so no gradient and no
    bootstrap can reach psi, the trunk or LeJEPA. The critic stays a purely vector object.

    u's head is std=0.01-initialized, so the model starts as a pure state baseline and
    can only move weight onto the innovation by paying for it in fit.
    """
    ...
    def __init__(self, value_dim, hidden):
        ...
        self.covector_head = layer_init(nn.Linear(hidden, value_dim), std=0.01)

    def forward(self, standardized_psi):
        feature = self.body(standardized_psi)
        return self.baseline_head(feature).squeeze(-1), self.covector_head(feature)

    def covector(self, standardized_psi):
        return self.covector_head(self.body(standardized_psi))


def fit_innovation_regressor(
    regressor,
    optimizer,
    standardized_psi,
    standardized_trace,
    ...
    """Fit G ~ b(psi) + <u(psi), E/s> by least squares. u is what the policy consumes."""
    ...
    for _ in range(epochs):
        ...
        for start in range(0, n, batch_size):
            ...
            prediction = baseline + (covector * standardized_trace[index]).sum(-1)
...
if __name__ == "__main__":
    ...
    with torch.random.fork_rng(devices=[]):
        readout = InnovationAdvantageRegressor(sf_dim, args.readout_hidden).to(device)
    ...
    if args.compile:
        ...
        # The regressor is deliberately NOT compiled: it is a small MLP evaluated once
        # per rollout and once per fit minibatch, so compilation would buy nothing and
        # risk a recompile storm on the two distinct batch shapes it sees.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # ---- minimum-variance innovation covector --------------------------------
            # u(s) is read off the regressor fit on the PREVIOUS rollout, under the
            # standardizer it was fit with. The innovation enters the model already
            # divided by that standardizer, so the same division is applied here: the
            # covector is in exactly the units it was estimated in.
            standardized_psi = (
                (psi_cur - readout_mean) / readout_std
            ).reshape(-1, sf_dim)
            frozen_credit_direction = readout.covector(standardized_psi).reshape(
                args.num_steps, args.num_envs, sf_dim
            )
            if readout_ready:
                policy_credit = (
                    frozen_credit_direction * (vector_policy_trace / readout_std)
                ).sum(-1)
            else:
                # Iteration one trains the vector critic and the regressor but
                # deliberately leaves the policy fixed. A random covector is not a
                # defensible exploration objective. (Same warm-up as v23.)
                frozen_credit_direction.zero_()
        ...
        active_critic_params = (
            critic_params if readout_ready else list(agent.critic_head.parameters())
        )
        ...
        # ---- minimum-variance innovation regressor -------------------------------------
        # The actor consumed the covector as it existed BEFORE seeing this rollout. Refit
        # only after the policy update, for use next iteration. The cross-fit matters more
        # here than anywhere else on this line: u is fit to explain realized returns from
        # realized innovations, which is precisely the pairing that overfits. Consuming it
        # one rollout later means only a relationship that GENERALIZES becomes credit.
        #
        # The standardizer is snapshotted HERE, together with the fit, so the covector
        # evaluated at the top of the next iteration reads psi in the frame it was fit in.
        readout_mean = sf_mean.clone()
        readout_std = sf_std.clone()
        with torch.no_grad():
            ...
            # on a task whose episodes END before mc_window the gate can be EMPTY -- and
            # an empty gate is not merely a missed update. fit_* returns steps == 0, so
            # readout_ready never flips; with readout_ready False the credit is zero and
            # ent_coef is 0, so the actor gradient is exactly zero and episodes stay
            # short, so the gate stays empty. That deadlock would hold for the whole run
            # with nothing but readout/steps == 0 to show for it. Degrade instead: fall
            # back to every state whose remaining episode is inside the rollout (a
            # truncation-biased target beats a frozen actor), then to every state.
            #
            # Note the regressor needs NO degeneracy guard of the kind v28/v29 carry: psi
            # is constant on iteration 1, but E is not, so the fit collapses to a single
            # global least-squares covector -- the best linear scalarizer available at
            # that point, not an arbitrary one.
            fit_gate = mc_mask
            ...
            fit_trace = (vector_policy_trace / readout_std).reshape(-1, sf_dim)[fit_gate]
            fit_returns = mc_ret.reshape(-1)[fit_gate]
            if fit_returns.numel() > 1:
                return_stat_count += 1
                return_rate = max(args.sf_target_ema, 1.0 / return_stat_count)
                return_mean = return_mean + return_rate * (fit_returns.mean() - return_mean)
                return_std = return_std + return_rate * (
                    fit_returns.std().clamp_min(1e-6) - return_std
                )
            fit_targets = (fit_returns - return_mean) / return_std
        readout_loss_mean, readout_grad_norm_mean, readout_steps = fit_innovation_regressor(
            readout,
            readout_optimizer,
            fit_psi,
            fit_trace,
        ...
        readout_ready = readout_steps > 0 or readout_ready

        with torch.no_grad():
            # Post-fit diagnostics. readout_ev is the full model's in-sample R^2 against
            # the Monte-Carlo return; baseline_ev is the R^2 of b(psi) ALONE. Their
            # difference is the only thing this design claims exists: return variance the
            # INNOVATION explains once the state is partialled out. If it is zero, the
            # vector innovation carries no policy information and no covector can help.
            ...
            baseline_ev = explained_variance_1d(fit_baseline, fit_targets)
            innovation_delta_ev = readout_ev - baseline_ev
            readout_target_count = int(fit_targets.numel())
            trained_credit_direction = readout.covector(refit_standardized_psi).reshape(
                args.num_steps, args.num_envs, sf_dim
            )
            ...
            direction_update_cosine = float(
                F.cosine_similarity(frozen_flat, trained_flat, dim=-1).mean()
            )
            credit_direction_rank = effective_rank(trained_flat)
            credit_direction_resultant = float(
                F.normalize(trained_flat, dim=-1, eps=1e-8).mean(0).norm()
            )
            credit_direction_norm = float(trained_flat.norm(dim=-1).mean())
            ...
            trained_credit = (trained_credit_direction * (vector_policy_trace / readout_std)).sum(-1)
        ...
        writer.add_scalar("readout/return_explained_variance", readout_ev, global_step)
        ...
        writer.add_scalar("readout/return_std", return_std.item(), global_step)
        writer.add_scalar("readout/baseline_explained_variance", baseline_ev, global_step)
        # THE FALSIFIER for this design: return variance the innovation explains on top
        # of the state baseline.
        writer.add_scalar("readout/innovation_delta_explained_variance", innovation_delta_ev, global_step)
