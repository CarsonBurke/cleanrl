# PPO with a LeJEPA latent-successor critic and detached HL-Gauss evaluator.
#
# Genuine LeJEPA learns e(s) through action-conditioned next-embedding prediction and
# SIGReg; reward and return gradients never enter it. The critic predicts one
...
# one scalar return. A detached Dreamer3-bucket HL-Gauss decoder is trained on the
# frozen latent TARGET, then applied to predicted psi during rollout. No CE gradient can
# enter psi, the shared trunk, or LeJEPA. Procrustes transport pins LeJEPA's exported
...
class DetachedHLGaussEvaluator(nn.Module):
    """Categorical scalar interpreter with a hard stop at its latent input."""

    def __init__(self, emb_dim, hidden, num_bins):
        super().__init__()
        self.body = nn.Sequential(
            layer_init(nn.Linear(emb_dim, hidden)),
            LeakyReLUSquared(),
        )
        self.head = layer_init(nn.Linear(hidden, num_bins), std=0.01)
        with torch.no_grad():
            self.head.weight.zero_()
            self.head.bias.zero_()

    def forward(self, latent_value):
        return self.head(self.body(latent_value.detach()))


def detached_hlgauss_ce(evaluator, target_embedding, target_probabilities):
    """Fit the scalar interpreter to a frozen rich Bellman target."""
    logits = evaluator(target_embedding)
    return -(
...
if __name__ == "__main__":
    ...
    # evaluator so actor sampling remains paired with the strongest anchored control.
    ssl = LeJepaSSL(obs_dim, act_dim_sf, args).to(device)
    with torch.random.fork_rng(devices=[]):
        evaluator = DetachedHLGaussEvaluator(
    ...
    def decode_value(latent_value):
        return hl_support.to_scalar(evaluator(latent_value))
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # either the latent critic or LeJEPA.
        ...
        # Train on the intended latent TARGET, never on critic predictions. The hard
        # detach inside evaluator makes this a one-way semantic interpreter: CE cannot
        # improve, distort, or collapse the rich critic to make its own job easier.
        with torch.no_grad():
            post_update_latent = psi_raw(agent.get_value(b_obs)[:, 0])
            latent_target_flat = sf_target.reshape(-1, sf_dim)
            evaluator_target_probs = hl_support.project(b_returns.detach())
            evaluator_edge_mass = float(
                (
                    evaluator_target_probs[:, 0]
                    + evaluator_target_probs[:, -1]
                ).mean()
            )
        ...
        for _ in range(args.evaluator_epochs):
            ...
            for evaluator_start in range(0, args.batch_size, args.evaluator_batch):
                ...
                target_probabilities = evaluator_target_probs[
                    evaluator_indices
                ]
                evaluator_loss = detached_hlgauss_ce(
                    evaluator,
                    latent_target_flat[evaluator_indices],
        ...
        with torch.no_grad():
            ...
            evaluator_target_ev = ev_score(
                decode_value(latent_target_flat), b_returns
            )
        ...
        writer.add_scalar("evaluator/target_edge_mass", evaluator_edge_mass, global_step)
