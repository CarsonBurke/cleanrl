# PPO + successor features, v4: TRANSFORMER HISTORY ENCODER (H=16).
#
# v3's lever 2 gave the encoder H stacked frames through the existing single-state MLP.
# v4 replaces that with a CAUSAL TRANSFORMER over a 16-frame window: e_t = f(s_{t-15..t}),
# read out from the last position, no action conditioning.
#
# WHY A TRANSFORMER RATHER THAN A WIDER MLP. Concatenating 16 frames gives every lag its own
# private weight block, so "compare s_t to s_{t-k}" has to be relearned separately at each k
# and nothing ties the k's together. Attention expresses that comparison once, uniformly in
# k. At H=16 (0.8 s at dt=0.05, more than one HalfCheetah gait cycle) the target is not just
# acceleration -- it is gait PHASE, which a 2-frame difference cannot represent at all.
#
# WHAT THIS COSTS, STATED UP FRONT. psi reads trunk(s_t) alone. With H>1 the horizon-0 SF
# target stops being a function of the critic's own input, so from psi's seat the extra H-1
# frames are irreducible target noise -- and it grows with H. That noise reaches the policy
# through w_r . E. gate/ev_sf_online against the H=1 arms is the metric that prices it. This
# is the reason the cheap H=3 frame-stack arm is worth running alongside rather than instead:
# it tests the acceleration mechanism with far less of this penalty.
#
# The unconditional CausalTransformer is new in shared/lejepa.py and is NOT the existing
# Transformer: that one is built from AdaLN-zero ConditionalBlocks, which driven at c=0 are
# exact identities at init that can only escape through their modulation biases.
# =====================================================================================
# v3: MAKE THE ENCODER EARN ITS PLACE.
#
...
class Args:
    ...
    hist_len: int = 16               # H. Observation frames the ENCODER sees: e_t = f(s_{t-H+1..t}).
    ...
    #                                  test of the ENCODER.
    #   H=16 spans 0.8 s at dt=0.05 -- comfortably more than one HalfCheetah gait cycle, so
    #   e_t can in principle carry gait PHASE, not just the acceleration a 2-frame window
    #   gives. THE COST IS REAL AND IT IS NOT COMPUTE: psi reads trunk(s_t) alone, so with
    #   H>1 the horizon-0 SF target is no longer a function of the critic's own input. In a
    #   Markov env the extra H-1 frames are, from psi's seat, IRREDUCIBLE TARGET NOISE, and
    #   it grows with H. That noise reaches the policy through w_r . E. Watch gate/ev_sf_online
    #   against the H=1 arms: if it drops materially, this lever is buying representation at
    #   the cost of a noisier advantage, and the cheap-and-clean H=3 frame-stack arm is the
    #   better test of the acceleration mechanism on its own.
    hist_depth: int = 2              # causal transformer over the history window
    hist_heads: int = 4
    hist_dim_head: int = 32
    hist_mlp_dim: int = 256
...
class LeJepaSSL(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, args):
        ...
        # LEVER 2. hist_len==1 keeps the plain single-state MLP so the v2/v3 path is
        # untouched; hist_len>1 swaps in a CAUSAL TRANSFORMER over the H-frame window.
        # Concatenating 16 frames into one MLP would work but throws away the structure the
        # signal actually has -- a fixed-width flat vector gives every lag its own private
        # weights and no way to express "compare frame t to frame t-k" uniformly in k, which
        # is precisely the operation acceleration (and any higher difference) needs.
        ...
        if args.hist_len > 1:
            self.encoder = HistoryEncoder(
                ...
                depth=args.hist_depth,
                heads=args.hist_heads,
                dim_head=args.hist_dim_head,
                mlp_dim=args.hist_mlp_dim,
            )
        else:
    ...
    def encode(self, w):
        """(..., H, obs) -> (..., d), uniform across both encoder types.

        Keeps every call site shape-agnostic so the H=1 and H>1 paths cannot drift apart --
        the failure this guards against is one site encoding the window and another encoding
        a bare frame, which differs silently rather than raising.
        """
        lead = w.shape[:-2]
        flat = w.reshape(-1, w.shape[-2], w.shape[-1])
        # H==1 keeps the plain state MLP, which wants (M, obs) not (M, 1, obs).
        e = self.encoder(flat) if self.hist_len > 1 else self.encoder(flat[:, -1])
    ...
    def forward(self, obs_seq, act_seq, cum_cont, sigreg_weight):
        ...
        """
        emb = self.encode(obs_seq)                               # (N,L,d) from (N,L,H,obs)
...
def history_windows(obs, cont, hist_len):
    """(T,B,obs) -> (T,B,H,obs), TIME-ORDERED: [..., s_{t-1}, s_t], current frame LAST.

    stack_history emits newest-FIRST ([s_t, s_{t-1}, ...]) because it is built by appending
    successively older lags. A causal transformer needs the opposite: position 0 is the
    oldest frame and the readout is the last position. Feeding the unflipped order would run
    the encoder backwards through time, read out from the OLDEST frame, and -- because
    causal masking then hides everything except s_t itself -- silently degrade to a
    single-state encoder that still trains and still logs a plausible loss. Hence the flip,
    and hence the assertion in the verifier that window[..., -1, :] == obs.
    """
    if hist_len <= 1:
        return obs.unsqueeze(-2)
    t, b, od = obs.shape
    stacked = stack_history(obs, cont, hist_len)          # (T,B,H*obs), newest first
    return stacked.view(t, b, hist_len, od).flip(-2)      # -> oldest first
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            obs_hist = history_windows(obs, 1.0 - transition_boundaries, args.hist_len)
            emb_buf = ssl.encode(obs_hist)                # (T,B,H,obs) -> (T,B,d)
        ...
        with torch.no_grad():
            ...
            probe_emb_after = ssl.encode(
                obs_hist.reshape(-1, obs_hist.shape[-2], obs_hist.shape[-1])[drift_probe_idx]
            ).float()
