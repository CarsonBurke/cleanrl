# PPO + successor features with nGPT-style hyperspherical LeakyReLU(0.5)^2.
# v10 replaces the row-normalized SiTU treatment with a complete spherical trunk:
# every recurrent trunk state and branch direction has unit L2 norm; input
# projection rows and output projection columns are retracted to unit norm after
# every optimizer step; and dense/MoE directions enter through learned nGPT-style
# spherical interpolation.  Unit state and projection rows imply |w.h| <= 1, so
# LeakyReLU(0.5)^2 is bounded by 1 inside every branch without a saturating cap.
# The final direction is scaled to radius sqrt(2H), preserving the original trunk's
# head-facing initialization scale while keeping all squared activations internal.
#
# The named reference uses scalar lambda, so that setting is the default here.
# PPO, successor-feature targets, and the actor distribution are unchanged.
# =====================================================================================
...
def unit_sphere(x, dim=-1):
    """Normalize vectors in float32; zero vectors remain zero rather than becoming NaN."""
    dtype = x.dtype
    normalized = F.normalize(x.float(), p=2, dim=dim, eps=SPHERE_EPS)
    return normalized.to(dtype=dtype)


class SphericalLinear(nn.Linear):
    """Bias-free linear retracted along rows (input) or columns (output)."""

    def __init__(self, in_features, out_features, norm_dim):
        # Construct with a temporary bias so reset_parameters consumes exactly the
        # same RNG as the inherited biased Linear. spherical_linear() removes it
        # after layer_init, preserving downstream seed pairing without breaking
        # the bias-free Cauchy-Schwarz activation bound.
        super().__init__(in_features, out_features, bias=True)
        if norm_dim not in (0, 1):
            raise ValueError(f"norm_dim must be 0 or 1, got {norm_dim}")
        self.norm_dim = norm_dim

    @torch.no_grad()
    def project_(self):
        normalized = unit_sphere(self.weight, dim=self.norm_dim)
        self.weight.copy_(normalized)
        return self


def spherical_linear(in_features, out_features, norm_dim):
    layer = layer_init(SphericalLinear(in_features, out_features, norm_dim))
    layer.register_parameter("bias", None)
    return layer.project_()


class SphericalLeakySquaredBranch(nn.Module):
    """Unit-row projection, bounded square activation, unit-column readout."""

    def __init__(self, H):
        super().__init__()
        self.in_proj = spherical_linear(H, H, norm_dim=1)
        self.activation = LeakyReLUSquared()
        self.out_proj = spherical_linear(H, H, norm_dim=0)

    def forward(self, x):
        # x and every input-projection row are unit norm, hence each coordinate
        # entering the square lies in [-1, 1] and activation coordinates in [0, 1].
        hidden = self.activation(self.in_proj(x))
        return unit_sphere(self.out_proj(hidden))


class ThinkBlock(nn.Module):
    """Spherical feature mix followed by sequential dense and soft-MoE directions."""

    def __init__(self, in_dim, H, n_experts):
        super().__init__()
        self.n_experts = n_experts

        self.in_proj = spherical_linear(in_dim, H, norm_dim=1)
        # Bounded convex residual gate: g = sigmoid(resid_gate) per channel.
        # init +4 → g ≈ 0.982 → x_in ≈ x at start.
        self.resid_gate = nn.Parameter(torch.full((H,), 4.0))

        self.dense = SphericalLeakySquaredBranch(H)
        self.dense_alpha = nn.Parameter(torch.full((H,), 0.05))

        # Unit-row router with a fixed sqrt(2H) temperature: for isotropic unit
        # states its logits retain the old variance of about 2 without learned scale.
        self.gate = spherical_linear(H, n_experts, norm_dim=1)
        self.router_scale = np.sqrt(2.0 * H)
        self.experts = nn.ModuleList(
            [SphericalLeakySquaredBranch(H) for _ in range(n_experts)]
        )
        self.moe_alpha = nn.Parameter(torch.full((H,), 0.05))

    def forward(self, cat_feats, x0):
        x = unit_sphere(self.in_proj(cat_feats))                      # (B, H)
        g = torch.sigmoid(self.resid_gate)                            # (H,)
        h = unit_sphere(g * x + (1.0 - g) * x0)                       # (B, H)

        dense_direction = self.dense(h)                               # (B, H), unit
        dense_lr = self.dense_alpha.abs()
        h = unit_sphere(h + dense_lr * (dense_direction - h))

        weights = torch.softmax(
            self.router_scale * self.gate(h), dim=-1
        )                                                             # (B, E)
        all_out = torch.stack([expert(h) for expert in self.experts], dim=1)
        moe_direction = unit_sphere(
            (weights.unsqueeze(-1) * all_out).sum(dim=1)
        )                                                             # (B, H), unit
        moe_lr = self.moe_alpha.abs()
        h = unit_sphere(h + moe_lr * (moe_direction - h))
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, n_experts):
        super().__init__()
        self.H = H
        self.entry = spherical_linear(in_dim, H, norm_dim=1)
        self.blocks = nn.ModuleList()
        for k in range(K):
            block_in_dim = H * (k + 1)
            self.blocks.append(ThinkBlock(block_in_dim, H, n_experts))
        cat_dim = H * (K + 1)
        self.out_proj = spherical_linear(cat_dim, H, norm_dim=0)

    def forward(self, x):
        x0 = unit_sphere(self.entry(x))
        feats = [x0]
        for block in self.blocks:
            feats.append(block(torch.cat(feats, dim=-1), x0))
        direction = unit_sphere(self.out_proj(torch.cat(feats, dim=-1)))
        return np.sqrt(2.0 * self.H) * direction
...
if __name__ == "__main__":
    ...
    spherical_layers = [
        module for module in agent.modules() if isinstance(module, SphericalLinear)
    ]
    spherical_blocks = [
        module for module in agent.modules() if isinstance(module, ThinkBlock)
    ]
    if not spherical_layers or not spherical_blocks:
        raise RuntimeError("hyperspherical experiment constructed no spherical trunk")

    @torch.no_grad()
    def project_spherical_weights():
        for layer in spherical_layers:
            layer.project_()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            dense_lrs = torch.cat([block.dense_alpha.abs() for block in spherical_blocks])
            moe_lrs = torch.cat([block.moe_alpha.abs() for block in spherical_blocks])
            row_errors = []
            column_errors = []
            for layer in spherical_layers:
                norm_error = (
                    layer.weight.norm(dim=layer.norm_dim) - 1.0
                ).abs().max()
                if layer.norm_dim == 1:
                    row_errors.append(norm_error)
                else:
                    column_errors.append(norm_error)
        writer.add_scalar("sphere/dense_lr_mean", dense_lrs.mean().item(), global_step)
        writer.add_scalar("sphere/dense_lr_max", dense_lrs.max().item(), global_step)
        writer.add_scalar("sphere/moe_lr_mean", moe_lrs.mean().item(), global_step)
        writer.add_scalar("sphere/moe_lr_max", moe_lrs.max().item(), global_step)
        writer.add_scalar(
            "sphere/row_norm_max_error", torch.stack(row_errors).max().item(), global_step
        )
        writer.add_scalar(
            "sphere/column_norm_max_error",
            torch.stack(column_errors).max().item(),
            global_step,
        )
