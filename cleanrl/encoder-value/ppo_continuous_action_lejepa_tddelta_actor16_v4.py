# PPO + LeJEPA TDDelta direct-geometry actor-16 v4.
#
# This controlled iteration changes v3's policy credit from a fixed 32-step prefix to
# 16 steps while retaining the healthy direct-geometric critic. The critic still predicts
# through effective horizon 100; critic reach and actor-gradient variance are independent.
# It removes GAE and all
# action-value/Q learning. A state critic represents successor features as TDDelta bands
# at effective horizons {1,2,4,8,16,32,64,100}. The policy advantage is a fixed 16-step
...
# only as far as an analytic policy-KL leash permits. The critic predicts raw TDDelta
# bands. Each cumulative successor gets a direct fixed-n target at its own effective
# horizon, then adjacent cumulative targets are differenced into TDDelta bands. This
# removes the one-step TDDelta propagation delay that gave long bands only tiny,
# recursively bootstrapped targets early.
# Fixed band-width factors condition only the loss; no moving normalization is used.
#
...
def fixed_n_successor_target(
    phi, next_value, bootstrap, continuation, beta, horizon
):
    """Fixed-n successor target with correct termination, truncation, and tail bootstraps."""
    ...
    for k in range(horizon):
        ...
        if valid <= 0:
            break
        active = alive[:valid]
        out[:valid] += (beta**k) * active.unsqueeze(-1) * phi[k:]

        if k + 1 == horizon:
            stop = torch.ones_like(active)
        else:
            stop = 1.0 - continuation[k : k + valid]
            # A rollout tail is a data boundary, not an episode boundary: bootstrap
            # from the recorded next observation even when n has not been reached.
            stop = stop.clone()
            stop[-1] = 1.0
        out[:valid] += (
            (beta ** (k + 1))
            * active.unsqueeze(-1)
            * stop.unsqueeze(-1)
            * bootstrap[k : k + valid].unsqueeze(-1)
            * next_value[k:]
        )
        if k + 1 < horizon and valid > 1:
            alive = active[:-1] * continuation[k : k + valid - 1]
    return out


def build_direct_geometric_targets(
    phi, next_bands, bootstrap, continuation, betas, horizons
):
    """Direct fixed-n targets for cumulative successors, differenced into TDDelta bands."""
    if next_bands.shape[-2] != len(horizons) or betas.numel() != len(horizons):
        raise ValueError("TDDelta grid mismatch")
    cumulative_next = next_bands.cumsum(dim=-2)
    cumulative_targets = torch.stack(
        [
            fixed_n_successor_target(
                phi,
                cumulative_next[..., index, :],
                bootstrap,
                continuation,
                betas[index],
                horizon,
    ...
    band_targets = cumulative_targets.clone()
    band_targets[..., 1:, :] = (
        cumulative_targets[..., 1:, :] - cumulative_targets[..., :-1, :]
    )
    return band_targets
...
if __name__ == "__main__":
    ...
    # Band j spans the incremental effective horizon L_j-L_{j-1}. Dividing only the
    # critic residual by this fixed width compares average occupancy error per added
    # timestep. Unlike target normalization, this has no state and never enters values.
    sf_loss_scale = torch.as_tensor(
        TD_BAND_WIDTHS, device=device, dtype=torch.float32
    ).unsqueeze(-1)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            band_next = agent.get_value(flat_next_agent_obs).reshape(
            ...
            band_cur = agent.critic_head(critic_feat_buf).view(
                args.num_steps,
                args.num_envs,
            ...
            # Direct geometric fixed-n targets train every temporal octave immediately.
            # The policy still uses one-step Bellman innovations below; feeding these
            # multi-step critic targets into its 32-step accumulator would double-count.
            sf_target = build_direct_geometric_targets(
                phi,
                band_next,
                bootstrap,
                continuation,
            ...
            one_step_target = build_tddelta_targets(
                phi, band_next, bootstrap, td_betas
            )
            band_delta = one_step_target - band_cur
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # The head predicts raw successor bands. Detached block RMS values merely
                # precondition residual gradients and never enter the value definition.
                ...
                if args.sf_alpha < 1.0:
                    ...
                    scalar_err = (sf_raw_err * w_r).sum(-1).pow(2)
        ...
        with torch.no_grad():
            ...
            old_bands = agent.critic_head(old_critic_feat).view(
            ...
            old_probe_values = old_bands.sum(dim=1) @ w_r
            accepted = False
            for _ in range(8):
                ...
                new_bands = agent.critic_head(new_critic_feat).view(
                    -1, args.critic_mtp_horizon, sf_dim
                )
                encoder_sf_drift = (
                    ((new_bands - old_bands) / sf_loss_scale)
                    .pow(2)
                    .mean()
                    .sqrt()
                )
                new_probe_values = new_bands.sum(dim=1) @ w_r
                encoder_value_drift = (
        ...
        for h, horizon in enumerate(TD_HORIZONS):
            writer.add_scalar(
                f"tddelta/preconditioned_mse_horizon_{horizon}",
                sf_per_h_mse[h].item(),
                global_step,
            )
            writer.add_scalar(
                f"tddelta/raw_mse_horizon_{horizon}",
