# PPO with a primary learned state-value embedding and exact latent Bellman algebra.
#
# The critic emits z(s), never a separate scalar. PPO reads V(s)=scale*u.z(s). A learned
# reward encoder supplies every other coordinate, and the target is exactly
# phi(r)+gamma*continuation*z_ema(s'). Thus the free coordinates learn discounted reward
# features rather than actions, fixed quantiles, hand-chosen horizons, or state occupancy.
# phi is projected so decode(phi(r))=r exactly; the latent Bellman target therefore decodes
# to the scalar Bellman target to numerical precision. EMA targets stabilize the otherwise
# rotation-equivalent frame, while weak covariance control discourages nullspace collapse.
#
# The v9 LeakyReLU(0.5)^2 trunk, actor, PPO settings, initialization stream, and scalar
# GAE are retained. The old LeJEPA/SF workload is removed: z itself is the value object.
import os
import random
import time
...
class Args:
    ...
    # --- Learned reward-value embedding -----------------------------------------------
    value_latent_dim: int = 16       # one exact scalar direction + 15 learned value directions
    value_scale: float = 100.0       # fixed frame scale; avoids a moving latent gauge
    reward_input_scale: float = 5.0  # only conditions the reward encoder MLP
    reward_hidden: int = 64
    rich_value_coef: float = 1.0     # nullspace Bellman loss relative to scalar value loss
    reward_feature_std: float = 0.15 # Bellman-consistent innovation scale (~sqrt(1-gamma^2))
    reward_feature_reg_coef: float = 1.0
    reward_model_lr: float = 1e-4
    reward_model_epochs: int = 4
    reward_model_batch: int = 4096
    reward_model_grad_clip: float = 1.0
    target_ema_rate: float = 0.20    # once/rollout: ~5 iterations, not v15's stale ~20
...
class RewardValueEncoder(nn.Module):
    """Learned immediate-reward code with an exact scalar readout."""

    def __init__(self, latent_dim, hidden, reward_input_scale, value_scale):
        super().__init__()
        self.reward_input_scale = reward_input_scale
        ...
        self.net = nn.Sequential(
            ...
            LeakyReLUSquared(),
            layer_init(nn.Linear(hidden, latent_dim), std=0.01),
        )

    def forward(self, reward, decoder_direction):
        raw = self.net((reward / self.reward_input_scale).unsqueeze(-1))
        return project_null(raw, decoder_direction) + (
            reward / self.value_scale
        ).unsqueeze(-1) * decoder_direction


class TargetCritic(nn.Module):
    """EMA copy of only the critic pathway; it never receives gradients."""

    def __init__(self, agent):
        super().__init__()
        trunk = agent.trunk if agent.share_backbone else agent.critic_trunk
        self.trunk = deepcopy(trunk)
        self.critic_head = deepcopy(agent.critic_head)
...
def reward_feature_regularization(reward_code, null_basis, target_std):
    """VICReg-style grounding in an explicit decoder-null coordinate system."""
    coords = reward_code @ null_basis
    ...
    covariance_loss = offdiag.square().sum() / max(coords.shape[-1], 1)
    mean_loss = coords.mean(0).square().mean()
    return variance_loss + covariance_loss + mean_loss, variance_loss, covariance_loss, mean_loss
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        with torch.random.fork_rng(devices=[]):
            self.critic_head = layer_init(
                nn.Linear(H, args.value_latent_dim, bias=False), std=0.1
            )
            with torch.no_grad():
        ...
        self.value_scale = args.value_scale
    ...
    def get_value(self, x):
        ...
        latent = self.critic_head(critic_feat)
        return self.decode_value(latent)

    def get_value_latent(self, x):
        _, critic_feat = self._trunks(x)
        return self.critic_head(critic_feat)

    def decode_value(self, latent):
        return self.value_scale * (latent @ self.decoder_direction)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        value_latent = self.critic_head(critic_feat)
        value = self.decode_value(value_latent)
        if self.actor_dist == "gaussian":
...
if __name__ == "__main__":
    ...
    target_critic = TargetCritic(agent).to(device)
    with torch.random.fork_rng(devices=[]):
        reward_encoder = RewardValueEncoder(
    ...
    reward_optimizer = optim.AdamW(
        reward_encoder.parameters(), lr=args.reward_model_lr, weight_decay=1e-3
    )
    ...
    if args.compile:
        ...
        target_critic.trunk = CompiledModule(target_critic.trunk, cudagraphs=False)
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            reward_optimizer.param_groups[0]["lr"] = frac * args.reward_model_lr
        ...
        with torch.no_grad():
            ...
            # Frozen one-step latent Bellman targets for all PPO epochs. The validity
            # mask censors a missing time-limit final_observation but keeps true terminals,
            # whose next latent is correctly ignored.
            flat_next_obs = next_obses.reshape(flat_obs_shape)
            target_next_latent = target_critic(flat_next_obs).reshape(
                args.num_steps, args.num_envs, args.value_latent_dim
            )
            bootstrap = (1.0 - transition_terminations) * transition_valids
            latent_valid = torch.logical_or(
                transition_terminations.bool(), transition_valids.bool()
            )
            # Reward coordinates are snapshotted once per rollout, then held fixed through
            # all PPO epochs. They update only in the separate post-PPO model phase.
            reward_code = reward_encoder(
                rewards.reshape(-1), agent.decoder_direction
            ).reshape(args.num_steps, args.num_envs, args.value_latent_dim)
            latent_target = reward_code + (
                args.gamma * bootstrap.unsqueeze(-1) * target_next_latent
            )
            decoded_latent_target = agent.decode_value(latent_target)
            decoded_scalar_target = rewards + args.gamma * bootstrap * agent.decode_value(
                target_next_latent
            )
            bellman_identity_maxerr = (
                decoded_latent_target - decoded_scalar_target
            ).abs().max()

            if args.ret_percnorm and args.ret_perc_scope in ("ema", "batch"):
                flat_ret = returns.reshape(-1)[latent_valid.reshape(-1)]
        ...
        b_latent_target = latent_target.reshape(-1, args.value_latent_dim)
        b_latent_valid = latent_valid.reshape(-1)
        b_target_next_latent = target_next_latent.reshape(-1, args.value_latent_dim)
        b_bootstrap = bootstrap.reshape(-1)
        b_rewards = rewards.reshape(-1)
        b_policy_valid = b_latent_valid
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                valid = b_latent_valid[mb_inds]
                valid_weight = valid.to(newvalue.dtype)
                valid_denom = valid_weight.sum().clamp_min(1.0)
                scalar_error = (
                    newvalue / args.value_scale
                    - b_returns[mb_inds] / args.value_scale
                ).square()
                scalar_value_loss = (scalar_error * valid_weight).sum() / valid_denom
                null_error = project_null(
                    new_value_latent - b_latent_target[mb_inds],
                    agent.decoder_direction.detach(),
                )
                rich_value_loss = (
                    null_error.square().mean(-1) * valid_weight
                ).sum() / valid_denom
        ...
        # Learn the reward-side coordinates against the frozen target critic. This is
        # separate from PPO so covariance pressure cannot spend the actor/critic clip
        # budget. The scalar component is exact by construction and is projected out.
        with torch.no_grad():
            online_current_for_model = agent.get_value_latent(b_obs)
        reward_model_loss_sum = reward_reg_loss_sum = reward_grad_norm_sum = 0.0
        reward_var_loss_sum = reward_cov_loss_sum = reward_mean_loss_sum = 0.0
        reward_model_steps = 0
        model_valid_inds = torch.nonzero(b_latent_valid, as_tuple=False).squeeze(-1)
        for _ in range(args.reward_model_epochs):
            perm = model_valid_inds[torch.randperm(model_valid_inds.numel(), device=device)]
            for start in range(0, perm.numel(), args.reward_model_batch):
                idx = perm[start : start + args.reward_model_batch]
                online_code = reward_encoder(b_rewards[idx], agent.decoder_direction)
                online_target = online_code + (
                    args.gamma * b_bootstrap[idx].unsqueeze(-1) * b_target_next_latent[idx]
                )
                model_error = project_null(
                    online_target - online_current_for_model[idx],
                    agent.decoder_direction.detach(),
                )
                reward_model_loss = model_error.square().mean()
                (
                    reward_reg_loss,
                    reward_var_loss,
                    reward_cov_loss,
                    reward_mean_loss,
                ) = reward_feature_regularization(
                    online_code,
                    agent.decoder_null_basis,
                    args.reward_feature_std,
                )
                reward_loss = (
                    reward_model_loss
                    + args.reward_feature_reg_coef * reward_reg_loss
                )
                reward_optimizer.zero_grad(set_to_none=True)
                reward_loss.backward()
                reward_grad_norm = nn.utils.clip_grad_norm_(
                    reward_encoder.parameters(), args.reward_model_grad_clip
                )
                reward_optimizer.step()
                reward_model_loss_sum += reward_model_loss.item()
                reward_reg_loss_sum += reward_reg_loss.item()
                reward_var_loss_sum += reward_var_loss.item()
                reward_cov_loss_sum += reward_cov_loss.item()
                reward_mean_loss_sum += reward_mean_loss.item()
                reward_grad_norm_sum += float(reward_grad_norm)
                reward_model_steps += 1
        reward_model_loss_mean = reward_model_loss_sum / max(reward_model_steps, 1)
        reward_reg_loss_mean = reward_reg_loss_sum / max(reward_model_steps, 1)
        reward_var_loss_mean = reward_var_loss_sum / max(reward_model_steps, 1)
        reward_cov_loss_mean = reward_cov_loss_sum / max(reward_model_steps, 1)
        reward_mean_loss_mean = reward_mean_loss_sum / max(reward_model_steps, 1)
        reward_grad_norm_mean = reward_grad_norm_sum / max(reward_model_steps, 1)

        # EMA only after all targets have finished being consumed.
        online_critic_trunk = agent.trunk if agent.share_backbone else agent.critic_trunk
        ema_update(target_critic.trunk, online_critic_trunk, args.target_ema_rate)
        ema_update(target_critic.critic_head, agent.critic_head, args.target_ema_rate)

        with torch.no_grad():
            online_latent = online_current_for_model
            null_latent = project_null(online_latent, agent.decoder_direction)
            centered = null_latent - null_latent.mean(0, keepdim=True)
            cov = centered.T @ centered / max(centered.shape[0] - 1, 1)
            eig = torch.linalg.eigvalsh(cov.double()).clamp_min(0)
            eig = eig[eig > 1e-10]
            p_eig = eig / eig.sum().clamp_min(1e-12)
            latent_effective_rank = float(
                torch.exp(-(p_eig * (p_eig + 1e-12).log()).sum())
            ) if eig.numel() else 0.0
            latent_std = float(torch.sqrt(torch.diagonal(cov).clamp_min(0)).mean())
            latent_null_mean_norm = float(null_latent.mean(0).norm())
            post_reward_code = reward_encoder(b_rewards, agent.decoder_direction)
            reward_coords = post_reward_code @ agent.decoder_null_basis
            reward_feature_std = float(reward_coords.std(0).mean())
            reward_feature_mean_abs = float(reward_coords.mean(0).abs().mean())
            post_latent_target = post_reward_code + (
                args.gamma * b_bootstrap.unsqueeze(-1) * b_target_next_latent
            )
            target_null = project_null(post_latent_target, agent.decoder_direction)
            latent_target_var = target_null[b_latent_valid].var().clamp_min(1e-12)
            full_error = project_null(
                online_latent - post_latent_target, agent.decoder_direction
            ).square().mean(-1)
            reward_only_error = project_null(
                online_latent - post_reward_code, agent.decoder_direction
            ).square().mean(-1)
            in_sample_latent_nmse = float(
                (
                    full_error[b_latent_valid].mean()
                    / latent_target_var
                )
            )
            next_latent_gain = float(
                1.0
                - full_error[b_latent_valid].mean()
                / reward_only_error[b_latent_valid].mean().clamp_min(1e-12)
            )
            probe_idx = torch.arange(0, b_obs.shape[0], 31, device=device)[:1024]
            ema_probe = target_critic(b_obs[probe_idx])
            online_probe = online_latent[probe_idx]
            ema_frame_rmse = float((online_probe - ema_probe).square().mean().sqrt())
        ...
        writer.add_scalar("rich_value/reward_model_loss", reward_model_loss_mean, global_step)
        writer.add_scalar("rich_value/reward_regularization_loss", reward_reg_loss_mean, global_step)
        writer.add_scalar("rich_value/reward_variance_loss", reward_var_loss_mean, global_step)
        writer.add_scalar("rich_value/reward_covariance_loss", reward_cov_loss_mean, global_step)
        writer.add_scalar("rich_value/reward_mean_loss", reward_mean_loss_mean, global_step)
        writer.add_scalar("rich_value/reward_model_grad_norm", reward_grad_norm_mean, global_step)
        writer.add_scalar("rich_value/reward_model_steps", reward_model_steps, global_step)
        writer.add_scalar("rich_value/bellman_identity_maxerr", bellman_identity_maxerr, global_step)
        writer.add_scalar("rich_value/in_sample_latent_nmse", in_sample_latent_nmse, global_step)
        writer.add_scalar("rich_value/next_latent_predictive_gain", next_latent_gain, global_step)
        writer.add_scalar("rich_value/latent_effective_rank", latent_effective_rank, global_step)
        writer.add_scalar("rich_value/latent_std", latent_std, global_step)
        writer.add_scalar("rich_value/latent_null_mean_norm", latent_null_mean_norm, global_step)
        writer.add_scalar("rich_value/reward_feature_std", reward_feature_std, global_step)
        writer.add_scalar("rich_value/reward_feature_mean_abs", reward_feature_mean_abs, global_step)
        writer.add_scalar("rich_value/ema_frame_rmse", ema_frame_rmse, global_step)
        writer.add_scalar(
            "rich_value/valid_fraction", b_latent_valid.float().mean().item(), global_step
        )
