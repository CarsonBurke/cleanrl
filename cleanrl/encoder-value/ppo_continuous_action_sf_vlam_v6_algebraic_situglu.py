# PPO + successor features with parameter-matched algebraic SiTU-GLU branches.
# v6 replaces SiTU's exponentially saturating tanh caps with
#   C_beta(x) = beta*x / hypot(x, beta)
# and uses [where(g <= 0, g, C_4(g)) sigmoid(g)] * C_25(u). The product remains
# bounded at 100 (strictly below it over the reals, possibly equal after floating-
# point rounding), while the positive gate and both
# up-projection tails have polynomial rather than exponential cap derivatives.
# Width, public RNG advancement, and initial branch output scale remain matched
# to v2; scalar lambda is baked in. Hypothesis: retaining usable gradients after
# rare PPO-induced projection excursions improves SiTU's late-stage stability
# without giving up its protection from destructive activation tails.
#
...
class _AlgebraicCap(torch.autograd.Function):
    """Algebraic cap with a cancellation-free analytical backward."""

    @staticmethod
    def forward(ctx, x, beta):
        ctx.save_for_backward(x, beta)
        return beta * (x / torch.hypot(x, beta))

    @staticmethod
    def backward(ctx, grad_output):
        x, beta = ctx.saved_tensors
        slope = (1.0 + (x / beta).square()).pow(-1.5)
        return grad_output * slope, None


def algebraic_cap(x, beta):
    """Smoothly cap ``x`` at ±beta with polynomial derivative tails."""
    beta_tensor = torch.as_tensor(beta, dtype=x.dtype, device=x.device)
    return _AlgebraicCap.apply(x, beta_tensor)


def situ_glu(gate, up):
    """One-sided algebraic SiTU-GLU product before the down projection."""
    gate_factor = torch.where(gate <= 0.0, gate, algebraic_cap(gate, 4.0))
    return (gate_factor * torch.sigmoid(gate)) * algebraic_cap(up, 25.0)


def _algebraic_situ_gaussian_second_moment(quadrature_order=64):
    """E[situ_glu(g,u)^2] for independent g,u ~ N(0,2)."""
    nodes, weights = np.polynomial.legendre.leggauss(quadrature_order)

    def gaussian_integral(integrand, lower, upper):
        samples = 0.5 * (upper - lower) * nodes + 0.5 * (upper + lower)
        density = np.exp(-(samples**2) / 4.0) / np.sqrt(4.0 * np.pi)
        return 0.5 * (upper - lower) * np.dot(weights, integrand(samples) * density)

    def sigmoid(x):
        return 1.0 / (1.0 + np.exp(-x))

    gate_second_moment = gaussian_integral(
        lambda x: (x * sigmoid(x)) ** 2, -16.0, 0.0
    ) + gaussian_integral(
        lambda x: ((4.0 * x / np.hypot(x, 4.0)) * sigmoid(x)) ** 2,
        0.0,
        16.0,
    )
    up_second_moment = gaussian_integral(
        lambda x: (25.0 * x / np.hypot(x, 25.0)) ** 2, -16.0, 16.0
    )
    return gate_second_moment * up_second_moment


ALGEBRAIC_SITU_SECOND_MOMENT = _algebraic_situ_gaussian_second_moment()


class SiTUGLUBranch(nn.Module):
    """Parameter- and initial-output-matched algebraic SiTU-GLU FFN."""
    ...
    def __init__(self, H):
        ...
        # Deterministic split quadrature computes the algebraic product's
        # second moment under the inherited N(0,2) gate/up initialization. The
        # old ReLU² branch has output second moment 12. A rectangular orthogonal
        # M->H down projection contributes M/H on average, giving the gain below.
        down_gain = np.sqrt(
            12.0 * H / (self.hidden_dim * ALGEBRAIC_SITU_SECOND_MOMENT)
        )
