# PPO + reward-anchored successor features with a value-aware state embedding.
#
# The first feature remains observed reward, giving the critic an exact auxiliary value
# teacher. PPO does not read it. A learned linear readout of the state-only LeJEPA event
# embedding predicts expected immediate reward; applying that same readout to its
# successor embedding yields the full scalar value used by rollout bootstrap and GAE.
# Thus temporal prediction remains vector-valued and the scalar objective is decoded from
# the learned state embedding, with no action features in the readout.
#
# Unlike v13, the state encoder is not reward-blind. A linear readout of its full
# SIGReg-protected embedding is trained on standardized immediate reward alongside
# LeJEPA dynamics prediction. This pins a value-relevant direction without
# assigning semantics to coordinates, horizons, or quantiles; the remaining directions
# remain free to encode predictable state structure. A trailing constant successor
# coordinate carries the learned readout bias exactly.
# =====================================================================================
# Historical v9 design notes follow; the readout is now learned only from the state
# embedding and its constant coordinate as described above.
# =====================================================================================
...
class Args:
    ...
    ssl_reward_coef: float = 1.0     # learn state-only immediate reward in a free e direction
    ssl_reward_huber_delta: float = 1.0
    aux_sf_coef: float = 1.0         # aggregate occupancy loss relative to projected h0 value
...
class LeJepaSSL(nn.Module):
    """Encoder + causal dynamics predictor + SIGReg + state-reward readout.

    SIGReg keeps the full code noncollapsed; a linear probe makes at least one freely
    chosen direction predictive of expected immediate reward. PPO consumes the same
    linear direction only after the critic has accumulated the vector successor embedding.
    """
    ...
    def __init__(self, obs_dim, act_dim, args):
        ...
        # Keep v13's global RNG stream paired so the experiment changes representation
        # learning, not the first rollout's sampled actions.
        with torch.random.fork_rng(devices=[]):
            self.value_probe = layer_init(
                nn.Linear(args.emb_dim, 1), std=0.01
            )
        self.sigreg = SIGReg(
    ...
    def forward(
        ...
        reward_prediction = self.value_probe(emb).squeeze(-1)
        reward_loss = F.huber_loss(
            reward_prediction,
            reward_target_seq,
            delta=reward_huber_delta,
        )
        total = (
            pred_loss
        ...
        return total, pred_loss, sigreg_loss, reward_loss
...
if __name__ == "__main__":
    ...
    # The exact reward coordinate provides a clean zero-value first rollout. After the
    # first SSL update this vector is replaced by the state-embedding reward readout.
    w_r = torch.zeros(sf_dim, device=device)
    w_r[0] = 1.0
    embedding_readout_ready = False
    probe_reward_mean = torch.zeros((), device=device)
    probe_reward_std = torch.ones((), device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Train the critic in the direction the NEXT rollout will consume, while
            # leaving this rollout's values/GAE on the frozen old readout.
            next_w_r = w_r.clone()
            embedding_readout_active = embedding_readout_ready
            if embedding_readout_active:
                next_w_r.zero_()
                emb_start = 1
                emb_stop = emb_start + args.emb_dim
                next_w_r[emb_start:emb_stop] = (
                    probe_reward_std
                    * ssl.value_probe.weight.squeeze(0)
                )
                next_w_r[-1] = (
                    probe_reward_mean
                    + probe_reward_std
                    * ssl.value_probe.bias.squeeze(0)
                )
            ...
            # Coordinate 0 is the exact-reward auxiliary comparator. PPO's GAE uses the
            # learned state-embedding readout instead.
            ...
            # Measure the previous iteration's state-only learned reward readout on the
            # current policy distribution before updating its encoder/probe.
            flat_phi = phi.reshape(-1, sf_dim)
            flat_rew = rewards.reshape(-1)
            reward_resid = flat_rew - flat_phi @ w_r
            state_reward_maxerr = reward_resid.abs().max()
            state_reward_r2 = 1.0 - (
                reward_resid.var()
                / flat_rew.var().clamp_min(1e-12)
            )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # The learned state-embedding readout is the value PPO actually consumes,
                # so it receives the privileged scalar budget. Coordinate zero remains an
                # exact-reward auxiliary teacher but has no special loss allocation.
                sf_tgt = b_sf_target[mb_inds]
                sf_err = value_sf - sf_tgt                                    # (B, mtp, sf_dim)
                value_mask = b_target_mask[mb_inds].to(sf_err.dtype)          # (B, mtp)
                projected_value_error = (
                    (sf_err * sf_std) @ next_w_r
                ) / projected_value_scale
                direct_value_loss = projected_value_error[:, 0].pow(2).mean()
                occupancy_loss = (
                    sf_err.pow(2).mean(-1) * value_mask
                ).sum() / value_mask.sum().clamp_min(1.0)
                future_value_mask = value_mask[:, 1:]
                future_value_denom = future_value_mask.sum()
                future_value_loss = (
                    projected_value_error[:, 1:].pow(2)
                    * future_value_mask
                ...
                projected_value_per_h_mse = (
                    projected_value_error.detach().pow(2) * value_mask
                ).sum(0) / value_mask.sum(0).clamp_min(1)
                aux_per_h_mse = (
                    sf_err.detach().pow(2).mean(-1) * value_mask
                ).sum(0) / value_mask.sum(0).clamp_min(1)
        ...
        with torch.no_grad():
            ...
            reward_target_mean = rewards.mean()
            reward_target_std = rewards.std().clamp_min(1e-6)
            standardized_reward_target = (
                rewards - reward_target_mean
            ) / reward_target_std
            seq_reward_target = chunk_sequences(
                standardized_reward_target.unsqueeze(-1), args.seq_len
            ).squeeze(-1)
        ...
        ssl_pred_l = ssl_sig_l = ssl_reward_l = ssl_gn_sum = 0.0
        ssl_steps = 0
        ...
        for _ in range(args.ssl_epochs):
            ...
            for s in range(0, n_seq - args.ssl_batch + 1, args.ssl_batch):
                ...
                ssl_loss, pred_l, sig_l, reward_l = ssl(
                    seq_obs[idx],
                    seq_act[idx],
                    seq_mask[idx],
                    seq_reward_target[idx],
                    args.sigreg_weight,
        ...
        with torch.no_grad():
            ...
            probe_reward_target = rewards.reshape(-1)[
                drift_probe_idx
            ]
            probe_reward_r2 = float(
                1.0
                - (
                    probe_reward_target - probe_reward_prediction
                ).var()
                / probe_reward_target.var().clamp_min(1e-8)
            )
            probe_reward_mean.copy_(reward_target_mean)
            probe_reward_std.copy_(reward_target_std)
            embedding_readout_ready = True
        ...
        writer.add_scalar(
            "sf/state_reward_maxerr",
            state_reward_maxerr.item(),
            global_step,
        )
        writer.add_scalar(
            "sf/state_reward_r2", state_reward_r2.item(), global_step
        )
        writer.add_scalar(
            "sf/projected_value_scale",
            projected_value_scale.item(),
            global_step,
        )
        ...
        for h in range(args.critic_mtp_horizon):
            ...
            writer.add_scalar(
                f"sf/projected_value_mse_h{h}",
                projected_value_per_h_mse[h].item(),
                global_step,
            )
        ...
        writer.add_scalar("ssl/reward_loss", ssl_reward_l, global_step)
        writer.add_scalar("ssl/state_reward_probe_r2", probe_reward_r2, global_step)
        writer.add_scalar(
            "ssl/embedding_readout_active",
            float(embedding_readout_active),
            global_step,
        )
