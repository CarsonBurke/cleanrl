# PPO + successor features with SiTU-GLU and Kimi-style routed-path normalization.
# v4 adds the stabilizer paired with SiTU in Kimi K3's Stable LatentMoE: RMSNorm
# after routed-expert aggregation followed by a shared outer projection. Routed
# expert widths shrink from 43 to 42 so the complete trunk is within 0.04% of
# v2's parameter count; the outer projection is privately initialized so every
# otherwise-unchanged parameter remains seed-paired with v3. Hypothesis: removing
# routed-aggregate scale drift prevents routing concentration and expert
# co-adaptation from changing the MoE residual's strength. This is Kimi's
# companion stabilizer for SiTU, not a claim that tanh's asymptotic derivative
# stops vanishing inside an individual expert.
#
# PPO + successor features with parameter-matched SiTU-GLU trunk branches.
# v3 changes only the ThinkTrunk dense/MoE FFNs from Linear-ReLU²-Linear to the
# official Kimi K3 SiTU-GLU product:
#   [4 tanh(g/4) sigmoid(g)] * [25 tanh(u/25)]
# followed by a down projection. The gated width is chosen to match each old
# branch's parameter count (<1% difference at H=64), its down-projection gain
# matches the old branch's initial output second moment, and unchanged parameters
# retain their v2 seed-1 initialization. Relative to the requested
# sf_vlam_v2_scalarlam run, scalar lambda is baked in and the FFN is the only
# changed mechanism. Hypothesis: bounded products retain multiplicative state
# interactions while reducing destructive activation and gradient tails in PPO's
# heavily reused soft-MoE branches.
#
...
class SiTUGLUBranch(nn.Module):
    ...
    def __init__(self, H, hidden_dim=None):
        super().__init__()
        ...
        matched_hidden_dim = max(1, round(2.0 * (H + 1) / 3.0))
        self.hidden_dim = matched_hidden_dim if hidden_dim is None else hidden_dim
...
def _branch_body(H, hidden_dim=None):
    return SiTUGLUBranch(H, hidden_dim=hidden_dim)
...
class ThinkBlock(nn.Module):
    ...
    def __init__(self, in_dim, H, n_experts):
        super().__init__()
        if H != 64 or n_experts != 16:
            raise ValueError(
                "v4's parameter and routed-output scale matching is calibrated "
                "for hidden=64 and n_experts=16"
            )
        ...
        # One fewer gated channel per routed expert pays for the Kimi-style
        # post-aggregation projection: at H=64, E=16 this leaves only +1,024
        # parameters per block relative to v3 and -192 over the complete trunk
        # relative to the original v2 ReLU² trunk.
        routed_hidden_dim = max(1, round(2.0 * (H + 1) / 3.0) - 1)
        self.experts = nn.ModuleList(
            [_branch_body(H, hidden_dim=routed_hidden_dim) for _ in range(n_experts)]
        )
        self.moe_out_norm = nn.RMSNorm(H, elementwise_affine=False)
        # With N(0,2) router logits over 16 experts,
        # E[sum_i softmax(logits)_i²] = 0.203988672. Each expert is initialized
        # at output second moment 12, so this gain preserves v3's routed-output
        # RMS while RMSNorm removes its sample-dependent scale.
        moe_out_gain = np.sqrt(12.0 * 0.203988672)
        # This is a new v4-only matrix. Initialize it without consuming the
        # public RNG stream so later blocks and heads stay exactly paired to v3.
        with torch.random.fork_rng(devices=[]):
            self.moe_out_proj = layer_init(
                nn.Linear(H, H, bias=False), std=moe_out_gain
            )
    ...
    def forward(self, cat_feats, x0):
        ...
        routed = (weights.unsqueeze(-1) * all_out).sum(dim=1)         # (B, H)
        d_moe = self.moe_out_proj(self.moe_out_norm(routed))          # (B, H)
