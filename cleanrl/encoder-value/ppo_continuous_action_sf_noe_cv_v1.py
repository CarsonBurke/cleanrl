# PPO + successor features, NO LEARNED ENCODER: the attribution ablation for lejepa_sf_v2.
# =====================================================================================
...
class Args:
    ...
    # ---- STEIN / Q-PROP ANALYTIC CONTROL VARIATE (this file's change) ----------
    cv: bool = True                  # subtract an ACTION-DEPENDENT control variate from the
    #                                  advantage and add back its analytic gradient. Naively
    #                                  subtracting g~(s,a) with E_pi[g~|s]=0 is NOT unbiased:
    #                                  it shifts the gradient by -c.grad_theta E_pi_theta[g],
    #                                  because the baseline is constant in `a` but NOT in
    #                                  theta. The identity actually used is
    #                                    grad J = E[(A - c.g~) grad log pi] + c grad_th E_pi_th[g]
    #                                  and the second term is added back EXACTLY, closed form.
    #                                  That is what makes this zero-bias for ANY c -- the whole
    #                                  point after two arms died of bias injection (lam=1
    #                                  telescoping put the regression target inside its own
    #                                  design matrix; the split-half gate meant to catch that
    #                                  had a ~0.95 null on temporally-smoothed rows).
    #                                  A badly estimated c costs VARIANCE ONLY, never bias.
    #
    #                                  g = [a, a*a]. The right g for MuJoCo: reward is
    #                                  x_vel - 0.1*||a||^2, so the control-cost half of the
    #                                  advantage is an EXACTLY KNOWN function of the action
    #                                  that PPO currently pays Monte-Carlo variance to learn.
    #                                  Beta + AFFINE to_action makes both moments exact:
    #                                    E[a]   = lo + (hi-lo) * dist.mean
    #                                    E[a*a] = (hi-lo)^2 * dist.variance + E[a]^2
    cv_ridge: float = 1e-3           # ridge for the 2*act_dim = 12-unknown solve over ~32k
    #                                  rows. Over-determined; no optimizer, no LR, and no
    #                                  reliability gate is needed because c cannot bias.
    actor_dist: str = "beta"
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, z=None):
        ...
        # Analytic conditional moments of g = [a, a*a] under the CURRENT policy. Exact, not
        # estimated, because to_action is affine in the Beta sample. g_raw is the realized g
        # at the sampled action; g_mom is E_pi[g | s]. Their difference has zero conditional
        # mean by construction. g_mom stays ATTACHED to theta so the Q-Prop correction term
        # can be differentiated through the concentration heads.
        if self.actor_dist == "beta":
            span = self.action_high - self.action_low
            mean_a = self.action_low + span * dist.mean
            e_a2 = (span * span) * dist.variance + mean_a * mean_a
            g_mom = torch.cat([mean_a, e_a2], dim=-1)
        else:
            g_mom = torch.cat([action, action * action], dim=-1) * 0.0
        g_raw = torch.cat([action, action * action], dim=-1)
        if self.actor_dist == "gaussian":
        ...
        return action, z, log_prob, entropy, value_sf, g_raw, g_mom
...
if __name__ == "__main__":
    ...
    cv_on = args.cv
    if cv_on:
        assert args.actor_dist == "beta", "cv needs closed-form moments; beta only"
        assert args.norm_adv and args.norm_adv_scope == "batch", (
            "cv subtracts in BATCH-NORMALIZED advantage units so the analytic correction "
            "needs no rescaling; per-minibatch renormalization would silently decouple the "
            "two halves of the identity and REINTRODUCE the bias it exists to remove.")
    g_tildes = torch.zeros((args.num_steps, args.num_envs, 2 * act_dim)).to(device)
    cv_c = torch.zeros(2 * act_dim, device=device)
    cv_var_red = cv_corr = 0.0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, value_sf, g_raw, g_mom = agent.get_action_and_value(next_obs)
                values[step] = psi_raw(value_sf[:, 0]) @ w_r
            actions[step] = action
            latent_zs[step] = z
            logprobs[step] = logprob
            g_tildes[step] = (g_raw - g_mom).detach()
        ...
        with torch.no_grad():
            ...
            phi = phi_features(obs, actions)                  # (T, B, sf_dim)
        ...
        if args.norm_adv and args.norm_adv_scope == "batch":
            ...
            # Solve c by least squares of the NORMALIZED advantage on g~, then subtract.
            # 12 unknowns over ~32k rows. Because the analytic correction is added back in
            # the update, c is a pure variance knob: any c leaves the gradient unbiased.
            b_g_tilde = g_tildes.reshape(-1, 2 * act_dim)
            if cv_on:
                with torch.no_grad():
                    Xc = b_g_tilde.double()
                    gram = Xc.T @ Xc
                    sc = torch.diagonal(gram).mean().clamp_min(1e-12)
                    gram = gram + args.cv_ridge * sc * torch.eye(
                        gram.shape[0], device=gram.device, dtype=gram.dtype)
                    cv_c = torch.linalg.solve(
                        gram, Xc.T @ b_policy_adv_normed.double()).to(b_g_tilde.dtype)
                    fitted = b_g_tilde @ cv_c
                    v0 = b_policy_adv_normed.var()
                    b_policy_adv_normed = b_policy_adv_normed - fitted
                    cv_var_red = float(1.0 - b_policy_adv_normed.var() / v0.clamp_min(1e-12))
                    cv_corr = float(
                        ((fitted - fitted.mean()) * (b_policy_adv - b_policy_adv.mean())).mean()
                        / (fitted.std().clamp_min(1e-12) * b_policy_adv.std().clamp_min(1e-12)))
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_sf, _, mb_g_mom = agent.get_action_and_value(
                ...
                if cv_on:
                    # + c . grad_theta E_pi_theta[g] as a LOSS term (pg_loss is -objective).
                    # mb_g_mom is ATTACHED to theta; this is the exact bias correction.
                    pg_loss = pg_loss - (mb_g_mom @ cv_c).mean()
        ...
        writer.add_scalar("cv/var_reduction", cv_var_red, global_step)
        writer.add_scalar("cv/corr_fitted_adv", cv_corr, global_step)
        writer.add_scalar("cv/c_norm", float(cv_c.norm()), global_step)
        writer.add_scalar("cv/c_a2_mean", float(cv_c[act_dim:].mean()), global_step)
        ...
        writer.add_scalar("sf/lambda_std_obs", sf_std[:obs_dim].mean().item(), global_step)
        writer.add_scalar("sf/lambda_std_act", sf_std[obs_dim:-1].mean().item(), global_step)
        writer.add_scalar("sf/lambda_absmean_obs", sf_mean[:obs_dim].abs().mean().item(), global_step)
