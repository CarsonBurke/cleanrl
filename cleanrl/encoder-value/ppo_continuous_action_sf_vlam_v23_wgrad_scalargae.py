# PPO + SF factorization V = psi.w with LEARNED w and UNTOUCHED scalar-GAE PPO.
# v23, from the v9_leakyrelu05sq_scalarlam chassis (10,171 @8M on HalfCheetah-v4).
#
# THE PLAN THIS IMPLEMENTS. Keep PPO exactly as is; the value is V(s) = psi(s).w
# where psi is the d-dim successor-feature head trained by TD on a vector target
# (unchanged from v9, including the phi(s_t)-inclusive convention) and w is a
# LEARNED linear reward weight trained by regression against the step reward,
# r_t ~ w.phi(s_t, a_t). The critic loss is exactly two terms: SF-TD on psi and
# reward regression on w. Advantages/GAE and the PPO actor loss are untouched.
#
# TWO CHANGES vs the v9 reference run, both in the w/advantage pathway:
#   1. vector_adv=False. The reference routed the policy advantage through
#      w_r . E_t (the vector TD(lambda) residual). Here the advantage is the plain
#      scalar GAE recursion on V = psi.w -- PPO's actor loss never sees the vector
#      path. This removes the accumulated probe-residual injection the v9 header
#      priced as an "honest cost" (sum_k (gamma*lam)^k eps_{t+k} in the advantage).
#   2. w is nn.Parameter, zero-init, trained by MSE(phi.w, r) INSIDE the PPO
#      minibatch loop (320 steps/iter), replacing the closed-form ridge re-solve.
#      Rationale: the closed-form solve rewrites the entire value scale once per
#      iteration -- a per-iteration jump in V that GAE then differences against;
#      a gradient-trained w moves at the same cadence as psi, so the two factors
#      of V co-adapt smoothly. It also kills v9's ordering subtlety (solve AFTER
#      GAE so values[] use the previous solve) -- w is simply fixed during the
#      rollout like any critic parameter. Zero-init keeps iteration 1 sane:
#      V == 0, scalar GAE reduces to the discounted reward sum (no dead actor).
#
# GRADIENT TREATMENT. w's regression loss has a graph DISJOINT from the trunk
# (phi is built from sg(e), raw obs, actions), so one backward populates both
# critic terms; w is clipped in its OWN group (w_grad_clip) so the psi pathway's
# gradient treatment stays bit-identical to the reference -- the delta under test
# is the w mechanism, not a perturbed critic clip budget.
#
# CORRECTNESS CHECK (the plan's ground-truth test). The closed-form ridge solve
# is retained as a NO-GRAD diagnostic: sf/reward_solve_r2 is the R^2 ceiling any
# linear w could reach on this rollout, sf/reward_probe_r2 is the learned w's
# live R^2, and sf/w_cos_solve is their direction agreement. Learned-R^2
# converging to the ceiling with cos -> 1 means the gradient w recovers the
# optimal readout; a persistent gap prices the plan's "regression on w" choice.
#
# KILL SIGNALS. sf/reward_probe_r2 plateauing far below sf/reward_solve_r2
# (w too slow: the actor consumes a stale value scale); gate/ev_sf_online <<
# gate/ev_trunk_probe sustained (the SF critic losing to a scalar critic on the
# same trunk, v9's falsifier, unchanged).
# =====================================================================================
...
class Args:
    ...
    # --- v23: learned w + untouched scalar-GAE PPO ------------------------------------
    w_reward_coef: float = 1.0       # weight on the reward-regression loss that trains w
    w_grad_clip: float = 0.25        # w's OWN clip group (single sf_dim vector). Kept out of
    #                                  critic_grad_clip so the psi pathway's gradient
    #                                  treatment is bit-identical to the v9 reference.
    # --- B: vector-routed advantage + per-dimension lambda ---------------------------
    vector_adv: bool = False         # v23 default OFF: the plan keeps PPO untouched, so the
    #                                  policy advantage is the plain scalar GAE on V = psi.w.
    #                                  --vector-adv restores v9's w . E_t routing as a
    #                                  control arm (now with the learned w).
    ...
    sf_ridge: float = 1e-3           # ridge for the closed-form solves (v23: diagnostics + EV gate only)
...
def solve_reward_probe(phi, reward, ridge):
    """Closed-form ridge solve of a linear reward readout from phi -> immediate reward.

    v23: no longer the live value pathway (w is gradient-trained). Retained for the
    NO-GRAD diagnostics -- the per-rollout R^2 ceiling and direction check on the learned
    w, and the EV gate's trunk/latent probes. Done in float64; the normal equations
    square the condition number and phi's blocks have very different scales.
    """
    phi64 = phi.double()
    gram = phi64.T @ phi64
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v23: LEARNED reward weight, V(s) = w_r . psi_0(s). Trained by gradient
        # reward regression (r_t ~ w_r . phi_t) inside the PPO minibatch loop -- the
        # second critic term. Zero-init => V == 0 for the first rollout, so the
        # scalar GAE reduces to the discounted reward sum rather than random noise
        # driving the actor. NOT in critic_parameters(): it gets its own clip group.
        self.w_r = nn.Parameter(torch.zeros(self.sf_dim))
    ...
    def critic_parameters(self):
        # Params receiving the SF-TD gradient (incl. the shared trunk). w_r is
        # deliberately EXCLUDED: its reward-regression gradient is clipped in its
        # own group (w_grad_clip) so it cannot eat the psi pathway's clip budget.
...
if __name__ == "__main__":
    ...
    # v23: w_r lives in the Agent as a learned parameter (see Agent.__init__). While it
    # is still exactly zero (iteration 1, pre-update), adv_vector = sf_residual @ w_r is
    # identically zero, so the vector_adv control arm falls back to scalar GAE until w_r
    # has moved off zero-init; the DEFAULT scalar-GAE arm is never degenerate (V == 0
    # makes GAE the raw discounted reward sum).
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                values[step] = psi_raw(value_sf[:, 0]) @ agent.w_r
            actions[step] = action
            latent_zs[step] = z
        ...
        with torch.no_grad():
            # v23: w was FIXED during the rollout (it only moves in the minibatch loop),
            # so values[], next_transition_values and adv_vector are all self-consistent
            # with the same w by construction -- v9's solve-after-GAE ordering subtlety
            # no longer exists.
            w_live = agent.w_r.detach()
            ...
            next_transition_values = psi_next @ w_live
            ...
            #
            # v23: w_live is the same (gradient-trained) w that produced values[] during
            # the rollout and next_transition_values above, so the projection is
            # self-consistent with the baseline it is differenced against for free.
            #
            # HONEST COST (why this arm is OFF by default in v23). The identity is exact
            # only if the probe is exact. It is not, so this injects the accumulated
            # probe residual sum_k (gamma*lam)^k eps_{t+k} into the advantage -- the
            # quantity sf/value_err_frac tracks. The plan keeps PPO's advantage on the
            # scalar GAE; --vector-adv restores v9's routing as the control.
            adv_vector = sf_residual @ w_live
            ...
            if args.vector_adv and bool((w_live.abs().max() > 0).item()):
            ...
            # v23: w is gradient-trained in the minibatch loop, not solved here. The
            # residual that matters for the value pathway (value_err_frac, the AC lags,
            # mc_resid) is the LEARNED w's, since that is the w the actor's V consumed.
            flat_phi = phi.reshape(-1, sf_dim)
            flat_rew = rewards.reshape(-1)
            reward_resid = flat_rew - flat_phi @ w_live
            reward_r2 = 1.0 - (reward_resid.var() / flat_rew.var().clamp_min(1e-12)).item()
            # The closed-form ridge solve is retained as a NO-GRAD diagnostic: the R^2
            # ceiling any linear w could reach on this rollout, and the direction the
            # learned w should be converging toward (the plan's correctness check).
            w_solve = solve_reward_probe(flat_phi, flat_rew, args.sf_ridge)
            reward_solve_r2 = 1.0 - (
                (flat_rew - flat_phi @ w_solve).var() / flat_rew.var().clamp_min(1e-12)
            ).item()
            w_cos_solve = float(
                (w_live @ w_solve)
                / (w_live.norm().clamp_min(1e-12) * w_solve.norm().clamp_min(1e-12))
            )
            ...
            #
            # mc_ret / mc_avail / mc_mask are computed EARLIER (the per-dimension
            # tau estimator needs them before Lambda is formed). The probe-residual
            # recursion uses reward_resid, i.e. the LEARNED w's residual -- the error
            # of the value pathway the actor actually consumed this rollout.
        ...
        # v23: (phi, r) pairs for the gradient reward regression on w. phi is built from
        # sg(e), raw obs and actions -- a leaf tensor with NO graph into the trunk, so
        # w's loss can share the critic backward while staying gradient-disjoint from psi.
        b_phi = flat_phi
        b_rewards = flat_rew
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.sf_alpha < 1.0:
                    ...
                    # spent on the one direction that is actually read out. w detached:
                    # this term shapes psi along w, it must not drag w toward psi.
                    scalar_err = (sf_err @ (agent.w_r.detach() * sf_std)).pow(2)  # (B, mtp)
                    v_loss = args.sf_alpha * v_loss + (1.0 - args.sf_alpha) * (
                        scalar_err * value_mask
                    ).sum(dim=-1).mean()
                # v23: the SECOND critic term -- reward regression training w. Immediate
                # reward only, no bootstrapping; the only scalar regression in the design.
                w_loss = (b_phi[mb_inds] @ agent.w_r - b_rewards[mb_inds]).pow(2).mean()
                ...
                if args.separate_grad_clip:
                    ...
                    # One backward, two disjoint graphs: v_loss reaches trunk+critic_head,
                    # w_loss reaches only agent.w_r (phi is a leaf). Clipping the two
                    # groups separately therefore treats each gradient EXACTLY as if it
                    # had its own backward -- psi's clip budget is untouched by w.
                    (args.vf_coef * v_loss + args.w_reward_coef * w_loss).backward(retain_graph=True)
                    critic_gn = nn.utils.clip_grad_norm_(critic_params, args.critic_grad_clip)
                    w_gn = nn.utils.clip_grad_norm_([agent.w_r], args.w_grad_clip)
                    # Stash clipped value grads (must survive the zero before the
                    # policy backward; the shared trunk is in this set). w_r rides along.
                    value_grads = [
                        (p, p.grad.detach().clone())
                        for p in critic_params + [agent.w_r]
                    ...
                    loss = (
                        ...
                        + v_loss * args.vf_coef
                        + w_loss * args.w_reward_coef
                    )
                    optimizer.zero_grad()
                    loss.backward()
                    critic_gn = actor_gn = w_gn = nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                    optimizer.step()
        ...
        writer.add_scalar("losses/w_reward_loss", w_loss.item(), global_step)
        writer.add_scalar("losses/w_grad_norm", float(w_gn), global_step)
        ...
        # probe_r2 = the LEARNED w's live R^2; solve_r2 = the closed-form ridge ceiling
        # on the same rollout; w_cos_solve = direction agreement between the two. The
        # learned w earning its keep looks like probe_r2 -> solve_r2 with cos -> 1.
        writer.add_scalar("sf/reward_probe_r2", reward_r2, global_step)
        writer.add_scalar("sf/reward_solve_r2", reward_solve_r2, global_step)
        writer.add_scalar("sf/w_cos_solve", w_cos_solve, global_step)
        ...
        writer.add_scalar("sf/w_r_norm", agent.w_r.detach().norm().item(), global_step)
