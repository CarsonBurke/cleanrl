# PPO + reward-sufficient successor features with LeakyReLU(0.5)^2 branches.
# v12 ablates the learned embedding from the critic basis:
#     phi = [normalized observation, action, action^2, 1].
# This preserves the explicit sufficient statistics for HalfCheetah's velocity-minus-
# control-cost reward while removing 32 rotating, reward-redundant coordinates from every
# successor target.  The detached LeJEPA encoder/predictor is deliberately still trained
# exactly as in v9 and used for control diagnostics, so its workload and stochastic RNG
# stream remain paired; its embedding is never accepted by phi_features and cannot enter
# the SF target, reward readout, value, or vector-routed policy advantage.
#
# The critic head is narrower, but it is initialized under a private RNG context and an
# unused v9-sized head is initialized to advance the global RNG exactly as v9 did. Thus
# the downstream actor initialization is seed-paired. The v9 LeakyReLU^2 trunk, scalar
# lambda default, PPO logic, and actor are otherwise unchanged.
# =====================================================================================
# Historical v9/base design notes follow. Their embedding-in-phi claims describe the
# parent experiment, not v12; the executable definitions above and below are authoritative.
# =====================================================================================
...
class LeJepaSSL(nn.Module):
    ...
    It remains an intentionally unused control workload in v12. It never sees reward or
    receives value gradients; only detached health/capacity diagnostics read its embedding.
    """
...
def phi_features(obs, action):
    """Reward-sufficient critic basis phi = [s, a, a*a, 1].

    The learned embedding is intentionally not an argument. This makes it impossible to
    accidentally feed an embedding coordinate into the successor target or value path.
    The raw state is already scaled by NormalizeObservation.
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # REWARD-SUFFICIENT successor-feature head. v12 excludes the LeJEPA embedding;
        # each horizon predicts [s, a, a*a, 1].
        ...
        # The changed output width must not perturb the actor initialization. Construct
        # the real reduced head privately, then consume the exact RNG sequence of v9's
        # original [e,s,a,a²,1] head before creating the actor heads.
        ...
        original_sf_dim = args.emb_dim + obs_dim + 2 * act_dim + 1
        _paired_v9_head = layer_init(
            nn.Linear(H, args.critic_mtp_horizon * original_sf_dim, bias=False), std=0.1
        )
        del _paired_v9_head
...
if __name__ == "__main__":
    ...
    # Per-dimension EMA standardization of the SF target. The state, action, action², and
    # constant blocks remain strongly heteroscedastic; a global rescale would let the
    # largest-coordinate block dominate the MSE.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # ---- REWARD-SUFFICIENT SUCCESSOR-FEATURE BASIS --------------------------
            # Keep this encoder call in the same location as v9 even though phi excludes
            # it. emb_buf remains a detached control diagnostic and supplies the exact
            # pre-SSL embeddings used by the unchanged frame-drift/latent-cap probes.
            ...
            # obs block mixes at tau ~ 12, the action blocks are nearly white (tau ~ 1),
            # and the constant coordinate never
            ...
            # Per-dimension EMA standardization of the target. The 1/count warmup is an
            # exact running mean early (and exact initialization at count == 1), then
            # decays into the plain EMA once the state/action occupancy settles.
            sf_stat_count += 1
        ...
        # Reward-sufficient basis slices: [obs, action, action², constant].
        writer.add_scalar("sf/lambda_std_obs", sf_std[:obs_dim].mean().item(), global_step)
        writer.add_scalar("sf/lambda_std_action",
                          sf_std[obs_dim : obs_dim + act_dim_sf].mean().item(), global_step)
        writer.add_scalar("sf/lambda_std_action_sq",
                          sf_std[obs_dim + act_dim_sf : -1].mean().item(), global_step)
        writer.add_scalar("sf/lambda_std_const", sf_std[-1].item(), global_step)
        ...
        # experiment. The lam_* spread is the whole premise: if the basis groups converge
        ...
        writer.add_scalar("advB/lam_obs", lam_vec[:obs_dim].mean().item(), global_step)
        writer.add_scalar("advB/lam_action",
                          lam_vec[obs_dim : obs_dim + act_dim_sf].mean().item(), global_step)
        writer.add_scalar("advB/lam_action_sq",
                          lam_vec[obs_dim + act_dim_sf : -1].mean().item(), global_step)
        ...
        writer.add_scalar("advB/tau_obs", tau_vec[:obs_dim].mean().item(), global_step)
        writer.add_scalar("advB/tau_action",
                          tau_vec[obs_dim : obs_dim + act_dim_sf].mean().item(), global_step)
        writer.add_scalar("advB/tau_action_sq",
                          tau_vec[obs_dim + act_dim_sf : -1].mean().item(), global_step)
        writer.add_scalar("advB/tau_const", tau_vec[-1].item(), global_step)
