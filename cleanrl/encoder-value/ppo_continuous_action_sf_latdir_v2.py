# sf_latdir_v2: WEIGHT EACH SAMPLE BY ITS OWN PREDICTED GAIN. One change from v1.
#
# WHAT v1 MEASURED (HalfCheetah-v4, cancelled at 2.69M). The direction hypothesis SURVIVED
# and is the reason this file exists rather than a rollback:
#
#   latdir/corr_g_adv   0.078 @200k -> 0.243 @500k -> 0.255 @1M -> 0.285 @2M -> 0.374 @2.7M
#
# The kill criterion did not fire. The predictor's action Jacobian, projected onto the
# advantage-weighted latent surprise, carries real directional information and MORE of it
# as the predictor trains. But the returns were a disaster:
#
#   return   -326 @200k   -207 @500k   -58 @1M   +41 @2M   +259 @2.7M
#   sf_noe_v1 ref   304        2115      4045      6328
#
# Direction right, OPTIMIZATION wrong. Two logged numbers say exactly how:
#
#   latdir/a_star_step   0.2962  0.2938  0.2913  0.2843  0.2855   <- constant, == eta
#   latdir/g_norm_mean    1.71    19.4    17.6    24.9    27.2
#     with g_norm_p50 0.71 -> 19.4 and g_norm_p99 18.1 -> 130.6
#
# ||g|| spans TWO ORDERS OF MAGNITUDE across the batch and v1 threw all of it away: ghat is
# unit-L2, so every sample took an identical full-size 0.3 step in action units and had its
# likelihood raised with identical force -- whether the latent responded violently to the
# action in that state or barely at all. At corr_g_adv ~ 0.37 roughly 63% of each direction
# is noise, and v1 had the noise shouting exactly as loudly as the signal.
#
# THE FIX, and it needs no new machinery because the quantity is already computed. The gain
# the step buys, to first order, is
#
#       <g_t, eta*ghat_t>  =  eta * ||g_t||
#
# so ||g_t|| IS the per-sample predicted improvement. Weight the surrogate by it:
#
#       w_t     = ||g_t|| / mean_batch(||g||)          dimensionless, mean 1 by construction
#       w_t     = clamp(w_t, max=latdir_w_clip)        p99/p50 ~ 7x; a few rows would else
#                                                      carry the whole update
#       pg_loss = -( w_t * min(rho*, clip(rho*)) ).mean()
#
# Computed ONCE per rollout next to g, detached, stored (num_steps, num_envs), flattened and
# indexed by mb_inds exactly like every other per-sample tensor. NEVER recomputed per
# minibatch, and normalized by the FULL-BATCH mean -- a minibatch mean would make "weight 1"
# denote a different thing in each of the 320 minibatches and silently destroy the relative
# scaling the change exists to introduce.
#
# WHAT THIS IS NOT. Still no entropy bonus of any kind (ent_coef stays 0, auto_alpha stays
# rejected). Still NO blending with an advantage-weighted policy gradient -- the clip trust
# region remains the only trust region. Critic/SF loss, SSL loss, both TD masks and the
# dual-backward clip are untouched. --no-latdir-weight-by-gnorm reproduces v1 exactly.
#
# READ THESE BEFORE CONCLUDING ANYTHING, both are consequences of v1's `latdir_weight="adv"`
# patch interacting with this change:
#
#   1. w_t IS NOW DOUBLY WEIGHTED. Under latdir_weight="adv" the score is A_t*E^emb, so
#      g_t = A_t * g_raw_t and therefore ||g_t|| = |A_t|/std(A) * ||g_raw_t||. The weight is
#      thus |advantage| TIMES latent responsiveness. That is defensible and arguably the
#      right object -- the SIGN of the advantage sets the direction, its MAGNITUDE sets the
#      force, which is what an advantage is for -- but it is not "responsiveness" alone, and
#      the product is doubly heavy-tailed, which is why latdir_w_clip is not optional.
#   2. THE CLIP ALSO SHRINKS THE GLOBAL STEP -- but by almost nothing at 5.0, which was
#      checked rather than assumed. Clipping a mean-1 variable can only lower its mean, so
#      some of any v2 win would be a disguised eta reduction rather than reweighting. Fitting
#      a lognormal to v1's logged order statistics (p50 19.4, p99 130.6; the fit reproduces
#      the observed mean, 27.1 vs 27.2, so it is a good model of the real distribution):
#
#          clip   mean(w)   frac_clipped
#           3.0    0.944        4.0%
#           5.0    0.984        0.9%       <- the default
#          10.0    0.998        0.06%
#
#      so v2 at clip=5 takes a 1.6% smaller average step than v1 while redistributing it
#      over a 6.7x spread (w: p1 0.11, p50 0.72, p99 4.8). The change is therefore ~pure
#      RESHAPING and the v1-vs-v2 comparison is attributable. Do NOT lower the clip toward 3
#      without re-checking this -- at 3.0 the global shrink is 5.6% and the confound is real.
#      Verify against the run's own latdir/w_mean, which should sit at ~0.98; a materially
#      lower value means the distribution moved and the arithmetic above no longer holds.
#
# ALSO FIXED: a metric that changed meaning under the "adv" patch. cos_g_consecutive fell
# 0.46 -> 0.24 on the v1 run and was read as the direction becoming incoherent. It is not
# that simple: ghat = sign(A_t) * ghat_raw, so every advantage sign flip between consecutive
# steps flips the measured cosine even when the underlying geometric direction is perfectly
# stable. latdir/cos_g_consec_signfix undoes the sign and measures the raw geometry; compare
# the two before drawing any conclusion about coherence.
# =====================================================================================
...
class Args:
    ...
    # --- v2: per-sample weighting by the step's own first-order predicted gain -------
    latdir_weight_by_gnorm: bool = True  # scale each sample's surrogate by
    #                                  w_t = ||g_t|| / mean_batch(||g||). THE v2 CHANGE.
    #                                  v1's unit normalization discarded a ||g|| range of two
    #                                  orders of magnitude, so a state where the latent barely
    #                                  responds to the action pushed the policy exactly as
    #                                  hard as one where it responds violently.
    #                                  --no-latdir-weight-by-gnorm reproduces v1 EXACTLY (the
    #                                  weight is not merely set to 1; the multiply is skipped).
    latdir_w_clip: float = 5.0       # ceiling on w_t, applied AFTER the mean-1 normalization
    #                                  so the threshold reads as "times the average predicted
    #                                  gain". Measured p99/p50 ~ 7x on the v1 run, and under
    #                                  latdir_weight="adv" the weight is |A_t| TIMES the raw
    #                                  Jacobian norm, i.e. a product of two heavy-tailed
    #                                  factors. Without this a handful of rows would carry the
    #                                  entire update -- the same gradient-domination failure
    #                                  latdir_z_eps documents, arriving by a second route.
    #                                  Watch latdir/w_frac_clipped: if it is more than a few
    #                                  percent the weight distribution, not the clip, is the
    #                                  thing to reconsider.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if args.latdir:
                ...
                # ---- v2: the per-sample weight -----------------------------------------
                # THE ONE CHANGE IN THIS FILE. ghat above is unit-L2, so the STEP is
                # identical for every sample; ||g_t|| -- the first-order gain that step is
                # predicted to buy, <g_t, eta*ghat_t> = eta*||g_t|| -- is what v1 discarded.
                # Restore it as a weight on the surrogate rather than as a step size, so the
                # trust region still sees a bounded action perturbation while the OBJECTIVE
                # learns to care more about the states where the latent actually responds.
                #
                # Computed HERE, once, off the same g_norm the diagnostics read, and stored
                # (num_steps, num_envs) so its layout is manifestly the buffer layout that
                # b_obs / b_latent_zs / b_z_star are flattened from (T-major, i = t*B + e).
                # A minibatch-mean normalization would be the silent-corruption version of
                # this: "weight 1" would denote a different gain in each of the 320
                # minibatches, so the relative scaling this change exists to introduce would
                # be destroyed exactly where it is supposed to act.
                g_norm_flat = g_norm.reshape(-1)
                latdir_w_raw = (
                    g_norm_flat / g_norm_flat.mean().clamp_min(1e-12)
                ).reshape(args.num_steps, args.num_envs)          # mean == 1 by construction
                # Clipped AFTER normalizing, so latdir_w_clip reads in units of "times the
                # average predicted gain". Note this can only LOWER the mean, i.e. it shrinks
                # the global step as well as reshaping the weights -- see the header's
                # confound note, and latdir/w_mean.
                b_latdir_w = latdir_w_raw.clamp(max=args.latdir_w_clip).reshape(-1)
                ...
                step_norm = (a_star - a_flat).norm(dim=-1)
                a_star_step = float(step_norm.mean())
                # ---- v2 weight panel ----------------------------------------------------
                w_mean = float(b_latdir_w.mean())                 # POST-clip; < 1 by clipping
                w_p99 = float(torch.quantile(latdir_w_raw.reshape(-1), 0.99))  # PRE-clip: the
                #   post-clip p99 is just latdir_w_clip whenever the clip binds at all, which
                #   would report the arg back to itself instead of the tail it is bounding.
                w_frac_clipped = float((latdir_w_raw > args.latdir_w_clip).float().mean())
                # The realized average movement: v1's constant a_star_step said the ACTION
                # perturbation never varied, and this is the number that should now vary.
                eff_step_mean = float((b_latdir_w * step_norm).mean())
                # cos_g_consecutive under latdir_weight="adv" is contaminated: ghat carries
                # sign(A_t), so an advantage sign flip between t and t+1 flips the cosine even
                # when the geometry is perfectly stable. Undo the sign and measure the raw
                # direction. Free -- sign(A_t) is already known, no second Jacobian pass.
                if args.latdir_weight == "adv":
                    sgn = torch.sign(adv_vector.reshape(n_flat, 1))
                    ghat_geo = ghat * torch.where(sgn == 0, torch.ones_like(sgn), sgn)
                else:
                    ghat_geo = ghat
                geo_tb = ghat_geo.reshape(args.num_steps, args.num_envs, act_dim_sf)
                cos_g_consec_signfix = float(
                    ((geo_tb[:-1] * geo_tb[1:]).sum(-1) * cont_buf[:-1]).sum()
                    / cont_buf[:-1].sum().clamp_min(1.0)
                )
            else:
                ...
                w_mean = w_p99 = w_frac_clipped = eff_step_mean = float("nan")
                cos_g_consec_signfix = float("nan")
                g_norm_hist = None
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.latdir:
                    ...
                    if args.latdir_weight_by_gnorm:
                        # v2. w_t is the sample's own first-order predicted gain, computed
                        # ONCE per rollout and indexed by the SAME mb_inds as everything
                        # else. The v1 form above is left standing and overridden rather
                        # than edited, so --no-latdir-weight-by-gnorm is v1 by construction
                        # (the multiply is skipped, not multiplied by ones).
                        surrogate_star = torch.min(
                            ratio_star, torch.clamp(ratio_star, 1 - args.clip_coef, 1 + clip_hi)
                        )
                        pg_loss = -(b_latdir_w[mb_inds] * surrogate_star).mean()
                    with torch.no_grad():
        ...
        # Sign-corrected geometric coherence. Under latdir_weight="adv" this is the one to
        # read -- cos_g_consecutive above also counts advantage sign flips as incoherence.
        writer.add_scalar("latdir/cos_g_consec_signfix", cos_g_consec_signfix, global_step)
        # ---- v2 weight panel: w_mean < 1 quantifies how much the clip shrank the global
        # step, and eff_step_mean is the realized average action movement (v1's was pinned
        # at eta by construction, which is the pathology this file exists to remove).
        writer.add_scalar("latdir/w_mean", w_mean, global_step)
        writer.add_scalar("latdir/w_p99", w_p99, global_step)
        writer.add_scalar("latdir/w_frac_clipped", w_frac_clipped, global_step)
        writer.add_scalar("latdir/eff_step_mean", eff_step_mean, global_step)
