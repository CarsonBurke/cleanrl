# PPO + the v9 successor-feature critic + an always-on full-rank value embedding.
#
...
# bootstrapped sampled return. The rich loss is part of the critic from update one:
# there is no gate, gradient projection, or fallback path. Both event codes and predicted
# state codes receive live-std whitening losses, and the rich head has nonzero orthogonal
# initialization, so rank-one collapse is directly opposed rather than merely detected.
# The return encoder's decorrelation uses the true normalization derivative; v16 detached
# it and collapsed.
#
...
class ReturnKernelEncoder(nn.Module):
    ...
    def forward(self, standardized_return):
        raw_code = self.net(standardized_return.unsqueeze(-1))
        return standardize_code(raw_code)


def standardize_code(coords):
    """Fix every embedding coordinate's batch scale; correlations remain learned."""
    centered = coords - coords.mean(0, keepdim=True)
    return centered / centered.std(0, unbiased=False).clamp_min(1e-4)
...
if __name__ == "__main__":
    ...
    if not args.separate_grad_clip:
        raise ValueError("v18 requires separate actor/critic gradient clipping")
    ...
    # No weight decay: the code is scale-normalized, so decay would shrink raw
    # weights without changing the objective until numerical epsilon caused collapse.
    return_optimizer = optim.Adam(
        return_encoder.parameters(), lr=args.return_model_lr
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            rich_evaluation = sampled_valid & evaluation_environment.unsqueeze(0)
            preupdate_rich_prediction = standardize_code(
                rich_value_head(critic_feat_buf)
            ).reshape(args.num_steps, args.num_envs, args.return_code_dim)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                rich_prediction = standardize_code(
                    rich_value_head(critic_feat)
                )
                ...
                (
                    rich_state_reg_loss,
                    rich_state_var_loss,
                    rich_state_corr_loss,
                    rich_state_scalar_corr_loss,
                    rich_state_mean_loss,
                ) = return_code_regularization(
                    rich_prediction[rich_valid],
                    b_standardized_sampled_return[mb_inds][rich_valid],
                )
                rich_value_loss = (
                    rich_prediction_loss
                    + args.rich_state_reg_coef * rich_state_reg_loss
                )
        ...
        writer.add_scalar(
            "rich_value/state_regularization_loss",
            rich_state_reg_loss.item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/state_variance_loss",
            rich_state_var_loss.item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/state_correlation_loss",
            rich_state_corr_loss.item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/state_scalar_correlation_loss",
            rich_state_scalar_corr_loss.item(),
            global_step,
        )
        writer.add_scalar(
            "rich_value/state_mean_loss",
            rich_state_mean_loss.item(),
            global_step,
        )
