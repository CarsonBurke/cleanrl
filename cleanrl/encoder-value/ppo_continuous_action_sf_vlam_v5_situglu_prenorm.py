# PPO + successor features with fixed-scale SiTU-GLU preactivations.
# v5 adds non-affine RMSNorm independently after every gate/up projection and
# restores their initialization RMS sqrt(2) before applying SiTU. This prevents
# uniform learned projection-scale growth from driving whole branches into
# tanh/sigmoid saturation while preserving directional gradients, parameter
# count, aggregate initial RMS, and the RNG stream of v3. It does not prevent
# individual coordinates from concentrating and saturating. This is a novel
# anti-saturation ablation, separate from Kimi K3's post-aggregation norm.
#
...
class SiTUGLUBranch(nn.Module):
    ...
    def __init__(self, H):
        ...
        # No affine parameters: otherwise learned gamma could recreate the
        # projection-scale drift this ablation removes.
        self.gate_norm = nn.RMSNorm(self.hidden_dim, elementwise_affine=False)
        self.up_norm = nn.RMSNorm(self.hidden_dim, elementwise_affine=False)
        _ = layer_init(nn.Linear(H, H))
        _ = layer_init(nn.Linear(H, H))

    def preactivations(self, x):
        # sqrt(2) exactly matches layer_init's gate/up RMS on isotropic unit-RMS
        # inputs, so v5 changes scale dynamics after initialization, not the
        # starting operating point of SiTU.
        scale = np.sqrt(2.0)
        gate = scale * self.gate_norm(self.gate(x))
        up = scale * self.up_norm(self.up(x))
        return gate, up

    def forward(self, x):
        gate, up = self.preactivations(x)
