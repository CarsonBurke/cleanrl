# PPO + LeJEPA lagged geometric-tail actor, v7.
#
...
# The actor always retains the sampled 16-step prefix and current-state baseline. It
# admits the horizon-16 model bootstrap only through a scalar reliability coefficient.
# That coefficient is the clipped uncentered least-squares projection of a realized
# long-suffix continuation onto the pre-update model bootstrap, using rows with at least
# 256 observed same-episode rewards. Iteration t uses only the coefficient fitted on
# rollout t-1; the current fit is hard-assigned after policy optimization. Thus no
# same-rollout outcome can choose its own policy weight, and there is no moving average.
# Sixteen is only the actor estimator unroll; M retains the full geometric horizon.
...
class Args:
    ...
    tail_mode: str = "learned"       # "learned": prior-rollout LS fit; "zero": no model bootstrap
    tail_fit_min_steps: int = 256    # only fit where this many same-episode rewards are observed
    tail_fit_ridge: float = 1e-6     # numerical ridge in the scalar uncentered regression
...
def same_episode_suffix_steps(continuation):
    """Number of sampled rewards available before a boundary or rollout edge."""
...
def fit_tail_coefficient(
    bootstrap_tail, continuation_tail, eligible, ridge
):
    """Fit one nonnegative convex bootstrap weight by uncentered least squares.

    Returns (raw coefficient, coefficient in [0,1], fractional uncentered-MSE
    improvement over predicting a zero continuation). The fit is diagnostic data for
    the *next* rollout; callers must not use it on the rollout that produced it.
    """
    x = bootstrap_tail.reshape(-1)[eligible.reshape(-1)]
    y = continuation_tail.reshape(-1)[eligible.reshape(-1)]
    zero = bootstrap_tail.new_zeros(())
    if x.numel() == 0:
        return zero, zero, bootstrap_tail.new_full((), float("nan"))
    denominator = x.square().sum()
    if not torch.isfinite(denominator) or denominator <= ridge:
        return zero, zero, bootstrap_tail.new_full((), float("nan"))
    raw = (x * y).sum() / (denominator + ridge)
    coefficient = raw.clamp(0.0, 1.0)
    null_mse = y.square().mean()
    fit_score = 1.0 - (coefficient * x - y).square().mean() / null_mse.clamp_min(
        1e-12
    )
    return raw, coefficient, fit_score
...
if __name__ == "__main__":
    ...
    if args.tail_mode not in ("learned", "zero"):
        raise ValueError("tail_mode must be learned or zero")
    if not 1 <= args.tail_fit_min_steps <= args.num_steps:
        raise ValueError("tail_fit_min_steps must be in [1, num_steps]")
    if args.tail_fit_ridge < 0.0:
        raise ValueError("tail_fit_ridge must be nonnegative")
    ...
    ret_perc_scale = 1.0
    lagged_tail_coefficient = 0.0

    for iteration in range(1, args.num_iterations + 1):
        # Frozen for the complete rollout and its PPO update. The fit made from this
        # rollout is installed only after the actor has finished consuming policy_adv.
        rho_used = (
            lagged_tail_coefficient if args.tail_mode == "learned" else 0.0
        )
        if args.anneal_lr:
        ...
        with torch.no_grad():
            ...
            full_bootstrap_advantages = (
                innovation_trace @ reward_task
            ) / (1.0 - args.gamma)

            # The current-state baseline is never gated: suppressing it would turn a
            # reliability estimate for the future bootstrap into a high-variance
            # reinforce-like estimator. Only gamma^H V(s_H) is learned/rejected.
            exact_prefix = fixed_horizon_sum(
                rewards, continuation, args.gamma, args.advantage_horizon
            )
            baseline_advantage = exact_prefix - values
            bootstrap_tail = (
                full_bootstrap_advantages - exact_prefix + values
            )
            policy_adv = baseline_advantage + rho_used * bootstrap_tail
            returns = policy_adv + values

            # Fit a continuation reliability for the *next* rollout from frozen,
            # pre-update predictions. sf_target is the longest available sampled suffix
            # with a frozen edge bootstrap. Requiring a long observed suffix keeps that
            # edge from dominating the regression.
            suffix_steps = same_episode_suffix_steps(continuation)
            tail_fit_mask = suffix_steps >= args.tail_fit_min_steps
            realized_full_return = (
                sf_target @ reward_task
            ) / (1.0 - args.gamma)
            continuation_tail = realized_full_return - exact_prefix
            rho_est_raw, rho_est, tail_fit_score = fit_tail_coefficient(
                bootstrap_tail,
                continuation_tail,
                tail_fit_mask,
                args.tail_fit_ridge,
            )
            n_tail_fit = int(tail_fit_mask.sum().item())
            bootstrap_to_baseline_ratio = float(
                bootstrap_tail.std()
                / baseline_advantage.std().clamp_min(1e-8)
            )
            effective_bootstrap_ratio = (
                rho_used * bootstrap_to_baseline_ratio
            )
            td0_vector = (delta_sf @ reward_task) / (1.0 - args.gamma)
            _a, _b = full_bootstrap_advantages.reshape(-1), td0_vector.reshape(-1)
            adv_vec_corr = float(
        ...
        # A hard one-rollout lag, not temporal smoothing: the actor above used rho_used,
        # which was fixed before collection. Only the next rollout may consume this fit.
        lagged_tail_coefficient = (
            rho_est.item() if args.tail_mode == "learned" else 0.0
        )
        ...
        # Fixed-horizon vector credit and learned-bootstrap diagnostics.
        ...
        writer.add_scalar(
            "tail/bootstrap_to_baseline_std",
            bootstrap_to_baseline_ratio,
            global_step,
        )
        writer.add_scalar(
            "tail/effective_bootstrap_to_baseline_std",
            effective_bootstrap_ratio,
            global_step,
        )
        writer.add_scalar("tail/rho_used", rho_used, global_step)
        writer.add_scalar("tail/rho_est_raw", rho_est_raw.item(), global_step)
        writer.add_scalar("tail/rho_est", rho_est.item(), global_step)
        writer.add_scalar(
            "tail/fit_score_uncentered", tail_fit_score.item(), global_step
        )
        writer.add_scalar(
            "tail/fit_fraction",
            n_tail_fit / (args.num_steps * args.num_envs),
            global_step,
        )
        writer.add_scalar(
            "tddelta/full_bootstrap_advantage_std",
            full_bootstrap_advantages.std().item(),
            global_step,
        )
        writer.add_scalar(
            "tddelta/policy_advantage_std",
            policy_adv.std().item(),
            global_step,
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "tddelta/baseline_advantage_std",
            baseline_advantage.std().item(),
            global_step,
        )
