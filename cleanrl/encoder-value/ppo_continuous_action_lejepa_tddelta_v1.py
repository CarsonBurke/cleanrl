# PPO + LeJEPA TDDelta v1: state-only geometric successor bands and vector n-step policy credit.
#
# This version supersedes the inherited design notes below. It removes GAE and all
# action-value/Q learning. A state critic represents successor features as TDDelta bands
# at effective horizons {1,2,4,8,16,32,64,100}. The policy advantage is a fixed 32-step
# sum of the realized vector Bellman innovations, contracted with the lagged immediate-
...
# only as far as an analytic policy-KL leash permits, and PopArt-preserving band updates
# keep normalization changes from moving the raw state-value function.
#
...
class Args:
    ...
    critic_mtp_horizon: int = len(TD_HORIZONS)  # one TDDelta band per geometric horizon
...
def build_tddelta_targets(phi, next_bands, bootstrap, betas):
    """One-step TDDelta targets for state-only successor-feature bands.

    phi: (T,B,D), next_bands: (T,B,J,D), bootstrap: (T,B), betas: (J,).
    Band j represents M_beta_j - M_beta_{j-1}; summing bands reconstructs M_beta_j.
    """
    if next_bands.shape[-2] != betas.numel():
        raise ValueError("TDDelta band count does not match beta grid")
    targets = torch.empty_like(next_bands)
    boot = bootstrap.unsqueeze(-1)
    cumulative_next = next_bands.cumsum(dim=-2)
    targets[..., 0, :] = phi + betas[0] * boot * next_bands[..., 0, :]
    for j in range(1, betas.numel()):
        targets[..., j, :] = (
            (betas[j] - betas[j - 1]) * boot * cumulative_next[..., j - 1, :]
            + betas[j] * boot * next_bands[..., j, :]
        )
    return targets
...
def expand_band_block_scales(block_scales, emb_dim, obs_dim, act_dim):
    """Expand per-band, per-semantic-block scales without choosing a latent basis."""
    widths = (emb_dim, obs_dim, act_dim, act_dim, 1)
    return torch.cat(
        [block_scales[:, i : i + 1].expand(-1, width) for i, width in enumerate(widths)],
        dim=-1,
    )


@torch.no_grad()
def preserve_raw_popart_output(head, optimizer, old_mean, old_std, new_mean, new_std):
    """Change standardized-output coordinates without changing raw TDDelta bands."""
    scale = (old_std / new_std).reshape(-1)
    shift = ((old_mean - new_mean) / new_std).reshape(-1)
    head.weight.mul_(scale.unsqueeze(-1))
    head.bias.mul_(scale).add_(shift)
    for parameter, factor in ((head.weight, scale.unsqueeze(-1)), (head.bias, scale)):
        ...
        if "exp_avg_sq" in state:
            state["exp_avg_sq"].mul_(factor.square())
        if "max_exp_avg_sq" in state:
            state["max_exp_avg_sq"].mul_(factor.square())
...
if __name__ == "__main__":
    ...
    if args.critic_mtp_horizon != len(TD_BETAS):
        raise ValueError(
            f"critic_mtp_horizon must be {len(TD_BETAS)} for the fixed TDDelta grid, "
            f"got {args.critic_mtp_horizon}"
        )
    if abs(args.gamma - TD_BETAS[-1]) > 1e-12:
        raise ValueError(
            f"gamma must equal the longest TDDelta beta ({TD_BETAS[-1]:.6g}); "
    ...
    if args.per_dim_lambda:
        raise ValueError("TDDelta uses temporal bands; per-coordinate lambda is not supported")
    if not args.vector_adv:
        raise ValueError("TDDelta v1 always optimizes its vector fixed-horizon residual")
    ...
    if args.auto_entropy:
        raise ValueError("auto_entropy is not implemented for fixed-horizon TDDelta credit")
    ...
    # Each TDDelta band gets a covariant mean vector and one scalar scale per semantic
    # feature block [e,s,a,a^2,1]. A scalar over the whole LeJEPA block is invariant to
    # SIGReg's free orthogonal rotations; per-coordinate latent standardization is not.
    sf_mean = torch.zeros((len(TD_BETAS), sf_dim), device=device)
    sf_block_std = torch.ones((len(TD_BETAS), 5), device=device)
    sf_std = expand_band_block_scales(sf_block_std, args.emb_dim, obs_dim, act_dim_sf)
    sf_stat_count = 0
    ...
    def bands_raw(sf_standardized):
        """Un-standardize TDDelta bands back into feature-occupancy units."""
        return sf_standardized * sf_std + sf_mean

    def psi_raw(sf_standardized):
        """Reconstruct the longest-horizon successor feature M_gamma by summing bands."""
        return bands_raw(sf_standardized).sum(dim=-2)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                next_emb = control_encoder(next_obs)
        ...
        with torch.no_grad():
            ...
            emb_buf = control_encoder(flat_obs).reshape(
                args.num_steps, args.num_envs, args.emb_dim
            )
            next_emb_buf = control_encoder(flat_next_obs).reshape(
                args.num_steps, args.num_envs, args.emb_dim
            )
            ...
            band_next = bands_raw(agent.get_value(flat_next_agent_obs)).reshape(
                args.num_steps, args.num_envs, args.critic_mtp_horizon, sf_dim
            )
            psi_next = band_next.sum(dim=2).reshape(
            ...
            band_cur = bands_raw(
                agent.critic_head(critic_feat_buf).view(-1, args.critic_mtp_horizon, sf_dim)
            ).reshape(args.num_steps, args.num_envs, args.critic_mtp_horizon, sf_dim)
            psi_cur = band_cur.sum(dim=2)
            ...
            sf_target = build_tddelta_targets(phi, band_next, bootstrap, td_betas)
            band_delta = sf_target - band_cur
            # The bands telescope to the ordinary vector Bellman innovation for M_gamma.
            # The policy consumes a FIXED 32-step sum of those innovations, not a
            # lambda-weighted mixture of every possible stopping time.
            delta_sf = band_delta.sum(dim=2)
            ...
            # equals an ordinary 32-step reward TD residual even when the probe is poor.
            ...
            old_sf_mean = sf_mean.clone()
            old_sf_std = sf_std.clone()
            flat_tgt = sf_target.reshape(-1, args.critic_mtp_horizon, sf_dim)
            tgt_mean = flat_tgt.mean(0)
            sf_mean = sf_mean + sf_rate * (tgt_mean - sf_mean)
            centered = flat_tgt - tgt_mean
            block_slices = (
                slice(0, args.emb_dim),
                slice(args.emb_dim, args.emb_dim + obs_dim),
                slice(args.emb_dim + obs_dim, args.emb_dim + obs_dim + act_dim_sf),
                slice(args.emb_dim + obs_dim + act_dim_sf, -1),
                slice(-1, None),
            )
            tgt_block_std = torch.stack(
                [
                    centered[..., block].pow(2).mean(dim=(0, 2)).sqrt().clamp_min(1e-6)
                    for block in block_slices
                ],
                dim=-1,
            )
            sf_block_std = sf_block_std + sf_rate * (tgt_block_std - sf_block_std)
            sf_std = expand_band_block_scales(
                sf_block_std, args.emb_dim, obs_dim, act_dim_sf
            )
            preserve_raw_popart_output(
                agent.critic_head,
                optimizer,
                old_sf_mean,
                old_sf_std,
            ...
            sf_mtp = (sf_target - sf_mean) / sf_std
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.sf_alpha < 1.0:
                    ...
                    scalar_err = (
                        sf_err * (w_r.unsqueeze(0) * sf_std).unsqueeze(0)
                    ).sum(-1).pow(2)                                        # (B, bands)
        ...
        with torch.no_grad():
            ...
            # Line-search the online->control interpolation against the actual policy KL.
            # This directly leashes the representation-induced policy update that PPO's
            # likelihood ratio cannot observe.
            old_control_params = [
                parameter.detach().clone() for parameter in control_encoder.parameters()
            ]
            old_actor_feat, _ = agent._trunks(
                torch.cat([probe_obs, probe_control_before], dim=-1)
            )
            ...
            for _ in range(8):
                ...
                probe_control_after = control_encoder(probe_obs)
                new_actor_feat, _ = agent._trunks(
                ...
                if encoder_policy_kl <= args.control_encoder_kl:
                    break
                accepted_rate *= 0.5
            else:
                accepted_rate = 0.0
                ...
                encoder_policy_kl = probe_obs.new_zeros(())
        ...
        for h, horizon in enumerate(TD_HORIZONS):
            writer.add_scalar(f"tddelta/mse_horizon_{horizon}", sf_per_h_mse[h].item(), global_step)
            writer.add_scalar(
                f"tddelta/band_rms_horizon_{horizon}",
                sf_target[..., h, :].pow(2).mean().sqrt().item(),
                global_step,
            )
        writer.add_scalar("sf/lambda_std_emb", sf_block_std[:, 0].mean().item(), global_step)
        writer.add_scalar("sf/lambda_std_obs", sf_block_std[:, 1].mean().item(), global_step)
        writer.add_scalar("sf/lambda_std_act", sf_block_std[:, 2:4].mean().item(), global_step)
        ...
        # compares n=32 against TD0; no GAE is computed anywhere in this file.
