# PPO + successor features with a MULTI-DISCOUNT LADDER of vector targets.
# v24, from the v9_leakyrelu05sq_scalarlam chassis (10,171 @8M on HalfCheetah-v4).
#
# WHY THIS, WHY NOW. Four materially different value-pathway designs -- v2 (vector
# advantage, solved w), v9 (LeakyReLU^2 branches), v13 (reward-anchored), v23 (gradient
# w, scalar GAE) -- all land at 10.1k +/- 0.1k, and gate/ev_sf_online ~ ev_trunk_probe
# throughout: the SF critic is NOT more accurate than a scalar probe on the same trunk.
# So the family's +1.8k over the scalar-critic base (8,278) cannot be value accuracy;
# it must be the DENSE CREDIT channel -- 62 TD error signals per step shaping the shared
# trunk instead of 1. Every plateaued variant emits that credit at a SINGLE timescale
# (gamma=0.99, horizon 100), while the chassis's own mixing-time analysis says the
# coordinates live at tau ~ 1 (actions), ~7 (e), ~12 (obs). Per-dimension lambda tried
# to exploit that heterogeneity INSIDE one discount and was a null (scalarlam won).
#
# THE CHANGE. The critic head predicts the vector lambda-return of phi under a LADDER
# of discounts gamma_j in {0.99, 0.95, 0.80} (effective horizons 100 / 20 / 5), i.e.
# head output is (mtp, n_gammas, sf_dim). V, GAE, the vector advantage, w_r, and PPO
# read ONLY the gamma=0.99 slice -- the auxiliary discounts are pure dense credit:
# each adds sf_dim MORE error signals per step at a timescale the main discount
# blurs over. Short-horizon occupancy (gamma=0.8: "what happens in the next ~5
# steps") is exactly the gait-phase structure H2 says a scalar critic lacks, and it
# is far less policy-nonstationary than the 100-step return, so those targets are
# cleaner supervision for the trunk early in training.
#
# WHAT STAYS FIXED. Everything else is the v9 reference config verbatim: closed-form
# solved w_r (v23 measured gradient-w as a cold-start cost that only reconverges by
# ~6M), scalar lambda, vector_adv=True, same MTP masks, same per-(gamma,dim) EMA
# standardization (now a (G, sf_dim) table), same losses with the per-horizon MSE
# additionally MEANED over gamma so the critic gradient scale matches v9's clip budget.
#
# HYPOTHESIS. If the dense-credit channel has headroom, tripling the timescale
# diversity of the error signals moves the return curve above the 10.1k plateau.
# If it is a null, the channel is saturated and the next lever is actor-side
# (exploration), not value-side -- either outcome redirects the search.
#
# KILL SIGNALS. sf/psi_mse_g* collapsing to identical curves (the ladder targets are
# redundant -- no new signal); gate/ev_sf_online dropping below ev_trunk_probe
# (aux targets stealing capacity from the readout slice); return curve tracking v9
# exactly (null: channel saturated).
# =====================================================================================
...
class Args:
    ...
    # --- v24: multi-discount ladder ---------------------------------------------------
    sf_aux_gammas: tuple[float, ...] = (0.95, 0.8)  # auxiliary discounts (horizons ~20, ~5).
    #                                  The head predicts Lambda under [gamma] + these; V, GAE,
    #                                  w_r and the vector advantage read ONLY the main-gamma
    #                                  slice. Empty tuple () reproduces v9 exactly.
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v24: one Lambda prediction per (horizon, discount). Index 0 of the gamma axis
        # is ALWAYS the main args.gamma -- the only slice the value readout touches.
        self.n_gammas = 1 + len(args.sf_aux_gammas)
        self.critic_head = layer_init(
            nn.Linear(
                H, args.critic_mtp_horizon * self.n_gammas * self.sf_dim, bias=False
            ),
            std=0.1,
        )
        with torch.no_grad():
    ...
    def get_value(self, x):
        # Returns successor features (B, mtp, n_gammas, sf_dim); horizon 0 / gamma 0 is
        # psi(s_t) under the main discount, whose scalar readout w_r . psi_0 is V(s_t).
        _, critic_feat = self._trunks(x)
        return self.critic_head(critic_feat).view(
            -1, self.critic_mtp_horizon, self.n_gammas, self.sf_dim
        )
...
if __name__ == "__main__":
    ...
    # v24: the discount ladder. Index 0 is ALWAYS the main gamma -- every value-readout
    # site indexes [..., 0, :] and only the target construction and the critic loss see
    # the auxiliary discounts.
    n_gammas = agent.n_gammas
    gammas = torch.tensor([args.gamma] + list(args.sf_aux_gammas), device=device)
    gam_col = gammas.view(n_gammas, 1)  # broadcasts over (B, n_gammas, sf_dim)
    # Per-(gamma, dimension) EMA standardization of the SF target. The blocks of phi are
    # wildly heteroscedastic: for a whitened e block std(sum gamma^k e) ~ tau ~ 20 (the
    # effective autocorrelation horizon, NOT 1/(1-gamma)=100), while the a*a block is
    ...
    # block -- and the gamma axis is heteroscedastic too (the constant's Lambda is ~5 at
    # gamma=0.8 vs ~100 at 0.99), so the table is per-(gamma, dim).
    sf_mean = torch.zeros(n_gammas, sf_dim, device=device)
    sf_std = torch.ones(n_gammas, sf_dim, device=device)
    sf_stat_count = 0
    ...
    def psi_raw(sf_standardized):
        ...
        Expects trailing dims (..., n_gammas, sf_dim) so the (G, sf_dim) table
        broadcasts; every call site passes the full gamma axis and slices AFTER.
        """
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                values[step] = psi_raw(value_sf[:, 0])[:, 0] @ w_r
            actions[step] = action
            latent_zs[step] = z
        ...
        with torch.no_grad():
            ...
            # (T, B, n_gammas, sf_dim): horizon 0 of every discount, un-standardized.
            psi_next = psi_raw(agent.get_value(next_obses.reshape(flat_obs_shape))[:, 0]).reshape(
                args.num_steps, args.num_envs, n_gammas, sf_dim
            )
            ...
            psi_cur = psi_raw(
                agent.critic_head(critic_feat_buf).view(
                    -1, args.critic_mtp_horizon, n_gammas, sf_dim
                )[:, 0]
            ).reshape(args.num_steps, args.num_envs, n_gammas, sf_dim)
            next_transition_values = psi_next[:, :, 0] @ w_r
            ...
            # v24: one recursion, gamma as a broadcast axis. Shapes inside the loop are
            # (B, n_gammas, sf_dim); gam_col is (n_gammas, 1) so gam_col * lam_vec is the
            # per-(gamma, dim) trace decay. Same phi, same masks, same lambda for every
            # rung -- only the discount differs, which is the entire point.
            sf_residual = torch.zeros(
                args.num_steps, args.num_envs, n_gammas, sf_dim, device=device
            )
            last_sf = torch.zeros(args.num_envs, n_gammas, sf_dim, device=device)
            for t in reversed(range(args.num_steps)):
                boot = ((1.0 - transition_terminations[t]) * transition_valids[t]).view(-1, 1, 1)
                cont = (1.0 - transition_boundaries[t]).view(-1, 1, 1)
                delta_sf = phi[t].unsqueeze(1) + gam_col * boot * psi_next[t] - psi_cur[t]
                last_sf = delta_sf + gam_col * lam_vec * cont * last_sf
                sf_residual[t] = last_sf
            sf_target = sf_residual + psi_cur                 # Lambda_t, (T, B, G, sf_dim)
            ...
            flat_tgt = sf_target.reshape(-1, n_gammas, sf_dim)
            ...
            sf_mtp = sf_target.new_zeros((args.num_steps, args.num_envs, mtp, n_gammas, sf_dim))
        ...
        b_sf_target = sf_mtp.reshape(-1, args.critic_mtp_horizon, n_gammas, sf_dim)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                sf_err = value_sf - sf_tgt                            # (B, mtp, n_gammas, sf_dim)
                value_mask = b_target_mask[mb_inds].to(sf_err.dtype)  # (B, mtp)
                # MEAN over both dim and gamma: the per-(horizon,row) loss keeps v9's
                # scale, so the critic clip budget is not silently re-weighted by G.
                v_loss = (sf_err.pow(2).mean(-1).mean(-1) * value_mask).sum(dim=-1).mean()
                if args.sf_alpha < 1.0:
                    ...
                    scalar_err = (sf_err[:, :, 0] @ (w_r * sf_std[0])).pow(2)  # (B, mtp)
                    v_loss = args.sf_alpha * v_loss + (1.0 - args.sf_alpha) * (
                        scalar_err * value_mask
                    ).sum(dim=-1).mean()
                # Per-horizon / per-gamma psi MSE (last minibatch of the last epoch is
                # what gets logged). per_g near-identical across rungs is a kill signal:
                # the ladder targets would be redundant and buying no new credit.
                err2_g = sf_err.detach().pow(2).mean(-1)                      # (B, mtp, G)
                sf_per_h_mse = (err2_g.mean(-1) * value_mask).sum(0) / value_mask.sum(0).clamp_min(1)
                sf_per_g_mse = (err2_g * value_mask.unsqueeze(-1)).sum(dim=(0, 1)) / value_mask.sum().clamp_min(1)
        ...
        # v24: per-rung MSE in standardized units. Near-identical curves across rungs =
        # the ladder is redundant (kill signal); the aux rungs converging FASTER than
        # gamma=0.99 is the expected signature (shorter horizons are easier targets).
        for g in range(n_gammas):
            writer.add_scalar(
                f"sf/psi_mse_g{gammas[g].item():g}", sf_per_g_mse[g].item(), global_step
            )
        # ||Lambda|| per block against the tau ~= 20 prediction for a whitened,
        # zero-mean e block under gamma=0.99, lambda=0.95. Main-gamma slice.
        writer.add_scalar("sf/lambda_std_emb", sf_std[0, : args.emb_dim].mean().item(), global_step)
        ...
        writer.add_scalar("sf/lambda_std_obs",
                          sf_std[0, args.emb_dim : args.emb_dim + obs_dim].mean().item(), global_step)
        writer.add_scalar("sf/lambda_std_act",
                          sf_std[0, args.emb_dim + obs_dim : -1].mean().item(), global_step)
        writer.add_scalar("sf/lambda_absmean_emb", sf_mean[0, : args.emb_dim].abs().mean().item(), global_step)
