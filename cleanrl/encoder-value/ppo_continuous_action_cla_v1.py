# CLA v1 -- COUNTERFACTUAL LATENT ADVANTAGE: compute the baseline instead of learning it.
#
# THE ONE-LINE IDEA. GAE's baseline V(s) is a LEARNED estimate of E_{a~pi}[Q(s,a)]. With an
# action-conditioned one-step model that expectation can be COMPUTED:
#
#   s_hat'(a) = s + D(s, a)                                    # one-step model, OBS space
#   psi_cf(s) = (1/M) sum_m [ phi(s, a^m) + gamma * psi(s_hat'(a^m)) ],   a^m ~ pi(.|s)
#   delta_t   = phi_t + gamma*boot*psi(s_{t+1}) - psi_cf(s_t)   # full lambda recursion kept
#
# psi_cf averages over pi at s, so it carries NO dependence on the action actually taken: it
# is a valid state-value baseline and the policy gradient stays unbiased. What it needs, and
# what no amount of data can supply, is next-states for actions the agent never took.
#
# WHY NOT THE LATENT. The original design put psi_cf in LeJEPA's latent, using the JEPA
# predictor as the counterfactual model. The plan's own kill switch settled it first:
#
#   gate/ev_latent_cap  (ridge of MC return onto sg(e) -- the CEILING for ANY latent critic)
#       0.32 @1M / 0.66 late
#   gate/ev_trunk_probe (the same ridge on raw trunk features)
#       0.61 @1M / 0.88 late
#
# The plan's rule was "if the latent cap is far below the trunk probe, the latent is the
# bottleneck and this cannot win however good psi gets". It is, by ~25 EV points, at every
# checkpoint. SIGReg's whitening toward N(0,I) is ACTIVE PRESSURE against the scale structure
# value is made of, which is also why deleting the encoder (sf_noe_v1: 9953 +/-109 @8M) beat
# every variant that kept it. So the counterfactual next-state is valued by the TRUNK and the
# model lives in OBSERVATION space. The encoder survives only where it was measured to pay:
# inside phi, where it does reward denoising.
#
# WHAT THIS REPLACES. Vector routing (measured: adv_vec_corr 0.97 -> 0.99) is scalar GAE with
# r_t swapped for the probe's linear reconstruction w_r.phi_t -- a denoising filter, and a
# STRICTLY LINEAR functional of E. The identity w_r.E == A_hat holds exactly when w_r.phi == r,
# so improving the probe destroys the gain; that lever is self-limiting and was not built on.
# CLA is the first change in this family that alters the advantage NON-linearly: psi_cf is an
# expectation over a learned model, not a projection of anything already in the buffer.
#
# WHAT IS DELIBERATELY UNCHANGED. The lambda recursion and the horizon. The per-dim-lambda arm
# cut tau 16.8 -> 9 and lost 15%; short credit horizons are lethal here. This changes the
# BASELINE, not the horizon. The model is used at k=1 only, which is where it was measured
# reliable -- AR training to K=4 cost 5% return and bought nothing.
#
# DEAD LEVERS REMOVED, NOT FLAGGED OFF: hist_len (H in {1,3,16} monotonically harmful,
# reward_probe_r2 0.963/0.965/0.946) and per_dim_lambda default-off with its evidence inline.
#
# SAFETY, since the model now enters the value path:
#   - ObsDynamics predicts the DELTA and is ZERO-INIT at the output, so an untrained model is
#     the exact identity, never a noise source.
#   - A RAMP on held-out per-dim R^2 (cla_dyn_r2_gate -> cla_dyn_r2_full) scales how much of
#     psi_cf reaches the baseline. A hard threshold would toggle near the bar and swap the
#     critic's target between two Bellman operators at full amplitude every few iterations.
#   - cla_beta blends psi_cf with the learned psi; beta=0 recovers the parent exactly.
#   - ObsDynamics also predicts CONTINUATION, so psi_cf does not bootstrap through death
#     (a no-op on HalfCheetah, which never terminates; required for Hopper/Walker).
#   - psi_cf(s_{t+1}) is the SHIFTED psi_cf(s_t), not a second independent sample: off-boundary
#     next_obses[t] IS obs[t+1] bitwise, and drawing independently there would break the
#     lambda-return's telescoping and inflate injected noise ~4x in std.
# Falsifier: cla/baseline_gap. If psi_cf never differs from psi, the model knows nothing the
# critic did not already know and this is a null result, not a win.
# =====================================================================================
...
class Args:
    ...
    #   The baseline psi(s) is a LEARNED guess at E_{a~pi}[Q(s,a)]. With an action-conditioned
    #   model that expectation can be COMPUTED. M=0 disables and recovers the parent exactly.
...
def counterfactual_psi(agent, dyn, psi_raw, flat_obs, emb_flat, gamma, m_samples, w_r, chunk=32768):
    ...
    A COMPUTED baseline where GAE uses a learned one. psi(s) is trained to approximate
    E_{a~pi}[Q(s,a)]; this evaluates that expectation directly by pushing M counterfactual
    actions through the dynamics model and valuing the results. It is a one-step
    ...
    drawn independently of the executed a_t -- but it is not free: in the GAE expansion the
    CURRENT state's baseline carries coefficient exactly -1 (only the FUTURE values are
    attenuated by (1-lambda)), so the noise passes into the advantage essentially undamped.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Placed HERE, before next_transition_values, so the scalar-GAE path and the
            # vector SF recursion are built from the SAME baseline. Blending after that line
            # would leave the two advantage estimates disagreeing about what V(s) is.
            # Applied to BOTH ends of the TD residual on purpose: GAE telescopes only if the
            # same value function appears at t and t+1, so replacing psi(s_t) alone would
            # break the lambda-return into a biased sum of mismatched baselines.
            ...
            if cla_active:
                ...
                # THE TELESCOPING FIX, and it also halves the cost. In the parent
                ...
                # final_observation is substituted). That identity is exactly what makes
                # sum_k (gamma*lambda)^k delta_{t+k} collapse into a lambda-return.
                ...
                # noise that no longer cancels between consecutive residuals. It would then
                # accumulate with coefficient (gamma*lambda)^k, inflating the injected
                # variance by (1+gamma^2)/(1-(gamma*lambda)^2) ~ 17x -- about 4x in std, into
                # both adv_vector AND the critic's own regression target. Reusing the shifted
                # tensor restores the identity exactly; only the rows where next_obses really
                # differs from obs[t+1] (boundaries) plus the rollout tail are recomputed.
