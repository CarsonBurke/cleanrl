# PPO with warm-started one-step action feedback, v2.
#
# The policy is conditioned on the normalized action it produced at the
# previous environment step: pi(a_t | s_t, a_{t-1}). Unlike v1, the action
# columns of the input projection start at zero and the observation columns
# use the base PPO initialization. Training therefore begins as the tested
# observation-only policy, while the action channel has a live gradient and
# can learn only when it improves the objective.
#
...
#
# Hypothesis: preserving the base policy's initial function removes v1's
# early optimization shock while retaining the potential momentum channel.
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        actor_trunk = make_norm_residual_trunk(self.actor_input_dim, 64, **trunk_kwargs)
        # Preserve the base actor's observation initialization exactly in the
        # enlarged projection, but leave the feedback columns trainable.
        with torch.no_grad():
            nn.init.orthogonal_(actor_trunk.in_proj.weight[:, :observation_dim], np.sqrt(64 / observation_dim))
            actor_trunk.in_proj.weight[:, observation_dim:].zero_()
