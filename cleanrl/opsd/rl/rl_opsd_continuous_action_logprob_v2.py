# RL-OPSD v2: sampled-action log-density self-distillation for continuous control.
# =====================================================================================
...
#
# v1 projected each sampled action through a full Beta natural-gradient teacher. It stayed
# random through 1.15M steps despite a calibrated critic: the finite distribution projection
# changed both location and concentration, so its dense KL did not preserve the sampled
# action's signed credit strongly enough.
#
# v2 uses the paper's sampled-token view directly. The rollout records each Beta coordinate's
# chosen native action and old log density. Standardized GAE becomes privileged information:
#   teacher_logp_d = rollout_logp_d + teacher_logprob_step * advantage.
# On replay, the student minimizes squared per-coordinate log-density divergence to this
# detached teacher. At the rollout policy its gradient is exactly a scaled policy gradient,
# but the residual vanishes at the teacher instead of requiring PPO ratios or clipping.
# Two full CUDA-graphed passes improve target fitting while remaining 5x cheaper than the
# 10-epoch base; a 1e-3 LR compensates for the much smaller optimizer-step budget.
# =====================================================================================
import os
import random
import time
...
class Args:
    ...
    teacher_logprob_step: float = 0.5
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x):
        ...
        return action, z, distribution.log_prob(z), value_logits

    def get_policy_and_value(self, x, z):
        _, _, alpha, beta, value_logits = self._heads(x)
        distribution = Beta(alpha, beta, validate_args=False)
        return distribution.log_prob(z), distribution.entropy(), value_logits
...
class CudaGraphUpdater:
    """CUDA-graphed log-density distillation, value CE, and decoupled dual backward."""
    ...
    def __init__(self, agent, optimizer, args, obs_shape, act_dim, device):
        ...
        self.z = torch.full((mb, act_dim), 0.5, device=device)
        self.teacher_logprobs = torch.zeros((mb, act_dim), device=device)
    ...
    def _update(self):
        self.optimizer.zero_grad(set_to_none=False)
        student_logprobs, token_entropy, value_logits = self.agent.get_policy_and_value(
            self.obs, self.z
        )
        logprob_residual = student_logprobs - self.teacher_logprobs
        policy_loss = 0.5 * logprob_residual.square().sum(dim=-1).mean()
        residual_abs = logprob_residual.abs().mean()
        entropy = token_entropy.sum(dim=-1).mean()
        ...
        self.optimizer.step()
        return policy_loss, value_loss, entropy, residual_abs, actor_grad_norm, critic_grad_norm

    def step(self, obs, z, teacher_logprobs, target_probs, target_mask):
        self.obs.copy_(obs)
        self.z.copy_(z)
        self.teacher_logprobs.copy_(teacher_logprobs)
...
if __name__ == "__main__":
    ...
    assert args.teacher_logprob_step > 0.0
    assert args.distill_epochs > 0
    ...
    old_token_logprobs = torch.zeros_like(latent_zs)
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            learning_rate.fill_(frac * args.learning_rate)
        else:
            learning_rate.fill_(args.learning_rate)

        for step in range(args.num_steps):
            global_step += args.num_envs
            obs[step] = next_obs
            with torch.no_grad():
                action, z, token_logprobs, value_logits = agent.get_action_and_value(next_obs)
                values[step] = hl_support.to_scalar(value_logits[:, 0])
            latent_zs[step] = z
            old_token_logprobs[step] = token_logprobs
        ...
        with torch.no_grad():
            ...
            teacher_logprobs = old_token_logprobs.reshape(-1, act_dim) + (
                args.teacher_logprob_step * batch_advantage.unsqueeze(-1)
            )
        ...
        metric_totals = torch.zeros(6, device=device)
        ...
        (
            ...
            actor_grad_norm,
            critic_grad_norm,
        ) = (metric_totals / (args.num_minibatches * args.distill_epochs)).tolist()
        ...
        writer.add_scalar("charts/learning_rate", learning_rate.item(), global_step)
        writer.add_scalar("charts/SPS", sps, global_step)
        writer.add_scalar("losses/policy_logprob_divergence", policy_loss, global_step)
        ...
        writer.add_scalar("opsd/logprob_residual_abs", residual_abs, global_step)
        writer.add_scalar(
            "opsd/teacher_logprob_shift_abs",
            (args.teacher_logprob_step * batch_advantage).abs().mean().item(),
            global_step,
        )
