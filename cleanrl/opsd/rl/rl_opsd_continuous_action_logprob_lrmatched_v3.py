# RL-OPSD v3: LR-matched sampled-action log-density self-distillation.
# =====================================================================================
...
#
# V1's full-Beta natural-gradient teacher stayed random through 1.15M. V2 replaced it with
# sampled-action targets and reached 3456 @2M, but trailed the base's 5012. Its two 1e-3
# passes supply only 2/3 of the base's simple epoch×LR update budget (10 * 3e-4).
#
# V3 retains v2's cheap objective and two fully CUDA-graphed passes, but raises LR to 1.5e-3
# to match that budget without adding compute. The rollout records each chosen Beta action
# coordinate and old log density; standardized GAE defines the detached teacher:
#   teacher_logp_d = rollout_logp_d + teacher_logprob_step * advantage.
# The student minimizes squared per-coordinate log-density divergence. At the rollout policy
# this has the exact signed policy-gradient direction, but vanishes at the teacher without
# an importance ratio, PPO clipping, reference policy, or extra policy epochs.
# =====================================================================================
import os
import random
import time
