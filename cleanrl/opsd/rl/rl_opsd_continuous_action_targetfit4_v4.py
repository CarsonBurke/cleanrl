# RL-OPSD v4: four-pass sampled-action target fitting.
# =====================================================================================
...
#
# V2 established that sampled log-density targets learn (3456 @2M), but its residual stayed
# almost as large as the requested teacher shift: two passes did not fit the per-state target.
# V3 increased LR, improving throughput and preserving the rising curve, but a larger Adam
# step does not add the nonlinear revisits required to reduce that residual.
#
# V4 uses four fully CUDA-graphed passes at 7.5e-4. This preserves v3's epoch×LR budget
# (4 * 7.5e-4 == 2 * 1.5e-3) while giving each target twice as many fitting steps, and still
# uses 2.5x fewer passes than the 10-epoch base. The detached on-policy teacher remains:
#   teacher_logp_d = rollout_logp_d + teacher_logprob_step * standardized_advantage.
# Squared per-coordinate log-density divergence has the exact signed policy-gradient direction
# at rollout time and vanishes at the teacher. No importance ratio or PPO clipping is used.
# =====================================================================================
import os
import random
import time
