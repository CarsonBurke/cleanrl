# OPSD Residual-Teacher + Jacobian v1
# =====================================================================================
# The canonical OPSD teacher is the actor driver. A privileged residual adapter learns
# p(a_t | s_t, delta_t) without changing the base/student policy, then its frozen optimistic
# query is distilled into that zero-context student. OPSD supplies at least 95% directional
# cosine: the one-step pathwise Jacobian only rotates its frozen target (`jac_cos=0.95`).
#
# The adapter consumes detached zero-context base features and a PRESENT Fourier embedding
# of the realized one-step TD residual. Its final projection is initialized exactly to zero,
# so teacher and student are identical at initialization. Absence is not encoded as a value:
# the student bypasses the adapter, while every teacher call requires a present scalar
# context. Clone NLL owns only adapter parameters; the base actor has exactly one objective,
# the clipped forward KL from the final detached teacher. There is no PPO ratio, surrogate,
# advantage-weighted actor gradient, Q critic, candidate search, or counterfactual rollout.
#
# The Jacobian mechanism is inherited from OPSD-JacTeach v1: TransitionHead is fit only to
# observed one-step transitions, its dose is gated by held-out one-step R2, and the pathwise
# direction is rotated in the exact Fisher/std-coordinate plane. Degenerate rows are the
# identity. A per-iteration bisection matches delivered clipped KL to this arm's UNROTATED
# residual-OPSD teacher, so the Jacobian changes direction rather than dose.
# =====================================================================================
import math
import os
import random
...
class Agent(nn.Module):
    ...
    def base_parameters(self):
        return [
            ...
            if not name.startswith("residual_adapter.")
        ]
...
if __name__ == "__main__":
    ...
    base_params = agent.base_parameters()
    ...
    assert base_ids.isdisjoint(adapter_ids)
    assert base_ids | adapter_ids == all_ids
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            base_optimizer.param_groups[0]["lr"] = frac * args.learning_rate
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
            ...
            assert all(p.grad is None for p in base_params), "clone gradient reached base policy"
        ...
        distill_kls, v_losses, actor_grad_norms, jac_losses = [], [], [], []
        # The base actor's sole loss is distillation. Critic regression may share a step,
        # but there is no clone, Jacobian, PPO, or advantage-weighted term in actor loss.
        for epoch in range(max(args.actor_epochs, args.critic_epochs)):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if do_critic:
                    a_stu, b_stu, value_logits = agent.student_policy_and_value(obs_mb)
                else:
                ...
                (args.distill_coef * distill_loss).backward(retain_graph=do_critic)
