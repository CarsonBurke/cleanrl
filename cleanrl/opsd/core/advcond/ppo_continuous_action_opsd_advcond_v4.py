# OPSD-AdvCond v4 -- the advantage is a CONDITIONING INPUT, so stop normalizing it like a
# gradient. No PPO anywhere (no ratio, no clipping, no advantage-weighted surrogate).
# =====================================================================================
# WHY THIS EXISTS
#   v1-v3 batch-standardized delta before the Fourier features, inherited straight from
#   PPO's adv-norm. But PPO normalizes advantages to control the SCALE OF A GRADIENT, and
#   this method never multiplies a gradient by an advantage. Here delta is an INPUT to a
#   learned conditional, where batch mean/sd is actively wrong:
#     (a) mean subtraction destroys delta's natural zero. delta_t = 0 means "exactly as
#         V(s) expected". After subtracting the batch mean, the least-bad action in an
#         all-bad batch is relabelled POSITIVE: the conditional is taught a false sign.
#     (b) the batch sd makes the units non-stationary. adv_boost is quoted in those units,
#         so "beat yourself by 1" is a tiny real gain when sd is small and a large one
#         later -- we inflict input distribution shift on ourselves every iteration.
#   But delta cannot be fed raw either: AdvEmbed's frequencies are FIXED (0.5 .. 8), so
#   the channel only resolves an O(1) input.
#
# MEASURED (this file, HalfCheetah-v4, 131072 steps, mb=32, epochs=4, adv_boost=1)
#   mode      cond_scale_rms over 4 iters   clip_frac   entropy
#   batch     1.00 1.00 1.00 1.00 (pinned)    0.9%      -0.708
#   ema_rms   1.00 1.28 1.40 1.18             2.0%      -0.654
#   raw       0.61 1.34 2.17 1.90            10.8%      -0.512
#   Raw delta's RMS grows 3.5x in four iterations as V learns, so "raw" is already
#   saturating the +-3 clip at 11% and loses channel resolution as training proceeds.
#   Entropy preservation orders raw > ema_rms > batch, exactly as (a) predicts: the false
#   sign has to be paid for by sharpening the conditional.
#
# THE CHOICE: cond_scale = "ema_rms"
#   Divide by a slow (beta=0.99, ~100 iteration) RMS of delta, de-biased, and never
#   subtract the mean. Keeps the zero honest, keeps the Fourier input O(1), and lets the
#   meaning of a given delta drift only on a horizon far longer than one update.
#   "batch" (= v3) and "raw" are retained as arms so the claim stays falsifiable.
#
# NOTE reward normalization is already OFF in this lineage (normalize_reward=False); only
# NormalizeObservation is active, as in every durable baseline on this chassis.
# =====================================================================================
# INHERITED v3 NOTES (unchanged below)
# =====================================================================================
...
class Args:
    ...
    update_epochs: int = 1        # PAPER: Algorithm 1 takes ONE step per fresh minibatch.
    #                               Reuse is a PPO habit, not an OPSD one. Kept as a knob
    #                               only so the reuse hypothesis stays falsifiable.
    ...
    cond_scale: str = "ema_rms"   # "ema_rms" | "batch" | "raw" -- how delta is scaled before
    #                               the Fourier features. See the header. "batch" is v3.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # SCALING A CONDITIONING INPUT IS NOT SCALING A GRADIENT.
            # "batch" (v3, inherited from PPO's adv-norm) does two harmful things here:
            #   (a) mean subtraction destroys delta's natural zero. delta_t = 0 means
            #       "exactly as V expected"; after subtracting the batch mean, the
            #       least-bad action in an all-bad batch is relabelled positive, so the
            #       conditional is taught a false sign.
            #   (b) the batch sd makes the UNITS non-stationary: adv_boost is quoted in
            #       those units, so "beat yourself by 1" is a tiny real gain early (sd
            #       small) and a large one later. The conditional's input is rescaled
            #       every iteration -- distribution shift we inflict on ourselves.
            # "ema_rms" keeps the zero and divides by a slow RMS, so the Fourier features
            # still see an O(1) bounded input (they must: freqs span 0.5..8) while the
            # meaning of a given delta drifts only on a ~100-iteration horizon.
            if args.cond_scale == "batch":
                ...
                # de-bias the EMA so early iterations are not divided by a near-zero scale
        ...
        for _ in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with torch.no_grad():
                    ...
                    # If the margin has pushed most queries into the clamp, v2 has
                    # degenerated back into v1's constant query.
                    clip_fracs.append(
        ...
        writer.add_scalar("debug/cond_ms_ema", cond_ms.sqrt().item(), global_step)
