# OPSD-AdvCond v8 -- the privileged channel is made IDENTIFIABLE, not IMITATED.
# No PPO anywhere (no ratio, no clipped surrogate, no advantage-weighted policy gradient).
# =====================================================================================
# WHAT THIS FIXES, AND HOW IT WAS FOUND
#
# The paper's teacher needs no fitting: prefilling y* into a pretrained LLM makes it
# competent by construction. Our privileged channel is a scalar with no pre-existing
# semantics, so p(a | s, delta) has to be LEARNED, and v1-v7 bought that with a plain
# NLL on the taken action. That term is advantage-free behaviour cloning, and because
# the trunk is shared it also reshapes the acting policy through cross-context parameter
# coupling -- so the method was a hybrid of advantage-conditioned BC and self-distillation
# rather than the paper's distillation-only improvement.
#
# The champion's own logs show the term degenerating. Over its 2M run (a4c4 mb128):
#   clone_nll     -0.819  -1.858  -3.594  -5.144  -5.949
#   entropy       -0.715  -1.902  -3.696  -5.226  -5.976
#   distill_kl     0.162   0.096   0.066   0.046   0.035
# clone_nll == -entropy to within ~0.05 for the entire run. That identity is what you get
# when a model fits ITS OWN samples: the log-likelihood of on-policy actions equals the
# negative entropy exactly when no residual uncertainty is left to explain. So the term
# stopped carrying information almost immediately and ran as a pure sharpening loop --
# and the sharpening is what closed the teacher-student gap (5 nats of entropy gone,
# distill_kl down 4x) that the improvement operator consumes. The measured deceleration
# (slope +3754 -> +1547) is that gap vanishing, not a critic failure (EV 0.94-0.97).
#
# WHY THE NLL PRESSURE EXISTS AT ALL (I first got this wrong)
# It is NOT "the model fits its own samples, so it sharpens": fitting a distribution to
# its OWN samples is a stationary point by the score-function identity, expected gradient
# zero. The pressure comes from the ASYMMETRY plus the shared trunk, exactly the
# cross-context coupling this file is named after: the fit is the TEACHER conditional
# p(a | s, delta) while the data are the STUDENT's rollout actions. Conditioning on delta
# licenses a sharper fit (by up to I(a; delta | s)), the shared trunk hands that sharpness
# to the student, the student's own entropy drops, and the teacher can then sharpen
# again. That is a ratchet, and it measured 5 nats over 2M steps.
#
# THE FIX: score only the delta-DEPENDENT part of the fit.
# Contrast the true (a_t, delta_t) pairing against deltas belonging to OTHER states in the
# same minibatch:
#     L_info = -E[ score(delta_t) - logsumexp_j score(delta_j) ]
#
# A FALSIFIED CLAIM, KEPT AS A WARNING. I first wrote that uniform sharpening "raises the
# positive and every negative together and CANCELS in the softmax". THAT IS FALSE. With
# score = log p, a sharper density discriminates BETTER: sweeping concentration 4 -> 5000
# moves L_info 1.48 -> 0.17, and dL/dlog(conc) = +0.0067 at conc 40. Density scoring only
# BOUNDS the sink (floor 0, diminishing returns: 10 nats of entropy buys 0.79, the next
# 5 nats buys 0.19) where NLL is unbounded (-1.7 -> -21.7 over the same sweep).
#
# To actually remove it, use score = mean contrast at a DETACHED bandwidth,
#     score(delta_j) = -|| (a_t - mean(s_t, delta_j)) / sd(s_t, delta_t).detach() ||^2 / 2
# The improvement operator only ever consumes the MEAN SHIFT with delta, so nothing is
# lost: measured identical identifiability (acc 0.474 vs 0.475, |dL/dmean| 3.59e-4 vs
# 3.62e-4) while autograd has NO PATH to concentration -- logc.grad is None, not merely
# small. Selected with --info-score mean; `density` remains the default so the arms
# already queued against it keep their exact submitted meaning (verified bit-for-bit).
#
# SELF-LIMITING, AND IN THE RIGHT DIRECTION: identifiability gets EASIER as the
# delta-conditioned means separate (measured: slope 0.02 -> 0.5 moves L_info 1.47 -> 0.25
# at fixed sharpness). The champion's advantage_std GREW 3.2x over its run, so separation
# rises with training and the pressure to sharpen falls -- the opposite of NLL's dynamic.
#
# Safety: a Beta is compactly supported, so the unconstrained absolute scale cannot blow
# up; the worst case is uniform (entropy 0 per dim), which shows up as flat returns.
# `clone_coef` is retained as an optional absolute-density anchor, 0 by default.
#
# COST: info_negatives extra actor forwards per minibatch (4 by default => 6 contexts
# instead of 2). The update was ~4% of wall clock, so this is nearly free.
#
# NEW KILL-TELLS
#   debug/info_acc ~ 1/(1+info_negatives)  -> the channel carries nothing; chance level.
#   losses/info_nce ~ log(1+info_negatives) -> same, read from the loss side.
#   clone_nll tracking -entropy again      -> the sink is back; check clone_coef.
#
# CORROBORATING EVIDENCE: the actor-reuse curve is NON-MONOTONE (HalfCheetah, seed 1,
# mb128, matched steps, 125k-window means)
#   a1c4  worse than a4c4 by ~1260 @1M
#   a4c4  3774 @1.5M   entropy -6.00   <- champion, the peak
#   a8c4  3153 @1.5M   entropy -6.61   <- overshoots: more fitting, more collapse
# Two effects with opposite signs: extra clone passes make the conditional (hence the
# teacher) more accurate, but each pass is paid for in entropy. Past 4 the entropy cost
# wins. FALSIFIABLE PREDICTION FOR THIS FILE: if L_info removes the entropy cost of
# fitting, the optimum shifts RIGHT -- a8 should stop being worse than a4. If a8 still
# loses while entropy holds up, the failure is pairing MEMORIZATION instead (watch
# debug/info_acc saturating toward 1.0), which is a different problem needing held-out
# negatives rather than fewer passes.
#
# 2x2 DESIGN THIS ARM SITS IN (HalfCheetah-v4, seed 1, 2M, a4c4 mb128, matched otherwise)
#   v5 clone=1.0  -> champion: identifiable but sharpening (4452 @2M, decelerating)
#   v5 clone=0.25 -> weaker sink
#   v5 clone=0.0  -> no identifiability term at all; predicted INERT (cond_gap -> 0)
#   v8 info-only  -> identifiability WITHOUT the sink   <- this file's hypothesis
#   v8 info+0.25  -> both, if the absolute density needs an anchor
# =====================================================================================
# INHERITED v5 NOTES (unchanged below)
# =====================================================================================
# OPSD-AdvCond v5 -- SEPARATE REUSE BUDGETS for the actor and the critic.
# No PPO anywhere (no ratio, no clipped surrogate, no advantage-weighted policy gradient).
# =====================================================================================
...
class Args:
    ...
    adv_boost: float = 1.0        # margin, in sigma, each state is asked to beat ITSELF by
    adv_cond_clip: float = 3.0    # clamp on the standardized advantage used as conditioning
    cond_scale: str = "batch"     # "batch" | "ema_rms" | "raw". "batch" reproduces v5/v8's
                                  # original expression bit-for-bit; ema_rms measured
                                  # +27% @1M over batch on this chassis.
    ...
    info_coef: float = 1.0        # weight on the delta-contrastive identifiability term
    info_score: str = "density"   # "density" = log p contrast. Bounded sink but not free:
                                  #             measured dL/dlog(conc) = +0.0067.
                                  # "mean"    = mean contrast at a DETACHED bandwidth. Same
                                  #             identifiability, no gradient path to
                                  #             sharpness at all (logc.grad is None).
    info_negatives: int = 4       # deltas from other states contrasted against the true one;
                                  # costs info_negatives extra actor forwards per minibatch
    clone_coef: float = 0.0       # plain NLL anchor on the ABSOLUTE density. The champion
                                  # ran this at 1.0 and it satisfied clone_nll ~= -entropy
                                  # for 2M steps: pure self-fitting. Off by default here.
...
if __name__ == "__main__":
    ...
    cond_scale_used, cond_clipped = float("nan"), float("nan")
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Conditioning scale, ported from v6. "batch" is v5's original expression and
            # reproduces it bit-for-bit, so arms already queued against this file keep
            # their exact submitted meaning. Measured on HalfCheetah seed 1, a4c4 mb128:
            # ema_rms beats batch 664 vs 354 @500k, 1845 vs 1614 @750k, 2962 vs 2334 @1M.
        ...
        # forwards per step.
        query_all = (b_adv_cond + args.adv_boost).clamp(
        ...
        clip_fracs, info_losses, info_accs = [], [], []
        ...
        for epoch in range(max(args.actor_epochs, args.critic_epochs)):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Every context below runs on the SAME states in one batched forward.
                #   [0]        privileged at the REALIZED delta_t   (the positive pairing)
                #   [1]        privileged ABSENT                   (the student)
                #   [2..2+R]   privileged at OTHER STATES' deltas   (InfoNCE negatives)
                cond_absent = adv_mb.new_zeros((n, agent.cond_dim))
                conds = [agent.cond_present(adv_mb), cond_absent]
                for k in range(1, args.info_negatives + 1):
                    conds.append(agent.cond_present(torch.roll(adv_mb, k, dims=0)))
                reps = len(conds)
                alpha, beta, value_logits = agent.policy_and_value(
                    obs_mb.repeat(reps, 1), torch.cat(conds, 0)
                )
                a_cl, b_cl = alpha[:n], beta[:n]
                a_stu, b_stu = alpha[n : 2 * n], beta[n : 2 * n]

                # 1. IDENTIFIABILITY, not imitation.
                # The champion's plain NLL satisfied clone_nll ~= -entropy to within 0.05
                # for its whole 2M run. Reproduced from first principles: fitting a
                # distribution to on-policy samples gives NLL == negative entropy exactly.
                # The pressure is not self-fitting (that is a stationary point by the score
                # identity) but the ASYMMETRY: the fit is the TEACHER conditional while the
                # data are the STUDENT's rollout actions, and a shared trunk hands the
                # resulting sharpness back to the student, which lets the teacher sharpen
                # again. Measured ratchet: 5 nats over 2M steps, closing the very gap the
                # improvement operator consumes (cond_gap 3x down, distill_kl 4x down).
                #
                # So score only the delta-DEPENDENT part: contrast the true (a_t, delta_t)
                # pairing against deltas belonging to OTHER states. Two scorings, measured:
                #   density: logit_j = log p(a_t | s_t, delta_j).
                #     Bounded (floor 0) but NOT sharpness-free -- verified dL/dlog(conc)
                #     = +0.0067 at conc 40, and sweeping conc 4 -> 5000 moves the loss
                #     1.48 -> 0.17. It merely SATURATES where NLL diverges (-1.7 -> -21.7).
                #   mean: logit_j = -||(a_t - mean_j) / sd_pos.detach()||^2 / 2.
                #     The improvement operator only ever consumes the MEAN SHIFT with
                #     delta, so scoring means alone loses nothing: measured identical
                #     identifiability (acc 0.474 vs 0.475, |dL/dmean| 3.59e-4 vs 3.62e-4)
                #     while autograd has NO PATH to concentration at all (logc.grad is
                #     None, not merely small). Sharpness stops being a currency outright.
                if args.info_score == "mean":
                    bandwidth = (
                        Beta(a_cl, b_cl, validate_args=False).stddev.detach().clamp_min(1e-6)
                    )

                    def _score(a_i, b_i):
                        return -(
                            ((z_mb - a_i / (a_i + b_i)) / bandwidth) ** 2
                        ).sum(-1) * 0.5

                elif args.info_score == "density":

                    def _score(a_i, b_i):
                        return Beta(a_i, b_i, validate_args=False).log_prob(z_mb).sum(-1)

                else:
                    raise ValueError(f"unknown info_score {args.info_score!r}")
                log_cands = [_score(a_cl, b_cl)]
                for k in range(args.info_negatives):
                    lo = (2 + k) * n
                    log_cands.append(_score(alpha[lo : lo + n], beta[lo : lo + n]))
                log_cands = torch.stack(log_cands, 0)
                info_loss = -(log_cands[0] - torch.logsumexp(log_cands, dim=0)).mean()
                # Always the true NLL, independent of info_score: it is the anchor term
                # (clone_coef, 0 by default) and the sink's telemetry.
                clone_loss = -(
                ...
                loss = (
                    args.info_coef * info_loss
                    + args.clone_coef * clone_loss
                    + args.distill_coef * distill_loss
                )

                # 3. Critic: V(s) from the ZEROED-context block only -- contexts [2..] are
                # counterfactual deltas and must never reach the value regression.
                if do_critic:
                    log_value_probs = torch.log_softmax(value_logits[n : 2 * n], dim=-1)
                    value_ce = -(b_target_probs[mb] * log_value_probs).sum(dim=-1)
                    v_loss = (value_ce * b_target_mask[mb]).sum(dim=-1).mean()
                ...
                with torch.no_grad():
                    ...
                    # Direct identifiability read: how often does the TRUE delta explain
                    # the taken action better than another state's delta? 1/(R+1) is chance.
                    info_accs.append(
                        (log_cands.argmax(dim=0) == 0).float().mean().item()
                    )
                    ents.append(
        ...
        writer.add_scalar("losses/info_nce", float(np.mean(info_losses)), global_step)
        writer.add_scalar("debug/info_acc", float(np.mean(info_accs)), global_step)
        ...
        writer.add_scalar("debug/adv_boost", float(args.adv_boost), global_step)
