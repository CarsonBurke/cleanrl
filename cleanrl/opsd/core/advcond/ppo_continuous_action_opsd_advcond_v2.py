# OPSD-AdvCond v2 -- RELATIVE optimism. Same network, same two contexts, same losses as
# v1; the only change is WHERE the teacher is queried. No PPO anywhere.
# =====================================================================================
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for _ in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Teacher: same weights, asked to beat what this state ACTUALLY scored by
                # adv_boost sigma. Relative, so the query sits next to an observed point
                # instead of out in the Gaussian tail. Detached, so gradients reach only
                # the student -- exactly the paper's asymmetry.
                query = (adv_mb + args.adv_boost).clamp(
                    -args.adv_cond_clip, args.adv_cond_clip
                )
                cond_query = agent.cond_present(query)
                with torch.no_grad():
