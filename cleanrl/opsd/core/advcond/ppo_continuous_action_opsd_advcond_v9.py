# ============================ RESULT: v9's PREMISE IS FALSIFIED ======================
# The change works exactly as designed and the returns are WORSE. HalfCheetah-v4, seed 1,
# a4c4 mb128, cond_scale=ema_rms, matched steps:
#   arm                       @500k    @1M     entropy   cond_gap  clone_loss
#   v6 density clone (NLL)      664    2962      -7.35     0.007       -
#   v9 mean clone (this file)   589    2311      -4.64     0.013     2.971
# The mechanism did what it promised: clone_loss sits at act_dim/2 = 3.0 (calibrated by
# construction), entropy collapse is roughly HALVED, and the improvement step is 2x
# larger. The return is 22% lower at 1M.
#
# So the premise was wrong. I had diagnosed the clone term's unbounded sharpening as the
# thing killing the improvement operator (5-nat ratchet, teacher-student gap down 3x).
# But the 8M champion collapsed all the way to entropy -13.0 and scored 8718, beating
# PPO's 8278 -- the deepest collapse in the lineage produced the best score. On
# HalfCheetah a near-deterministic policy is close to optimal, so the sharpening the NLL
# buys is a FEATURE of this benchmark, not a pathology. Larger, better-calibrated steps
# with a broader policy lose to smaller steps with a sharper one.
#
# What survives from this line of work:
#   - the ablation that clone supplies CALIBRATION, not just identifiability (v8's
#     purely discriminative InfoNCE replacement scored -70 @1M with a MORE identifiable
#     channel; clone_coef=0 drove cond_gap to exactly 0.000 and died at 352k);
#   - the measurement that clone_nll == -entropy identically, i.e. the term is a
#     self-consistent sharpener, and WHY (teacher/student asymmetry through a shared
#     trunk, not self-fitting -- self-fitting is a stationary point);
#   - `--clone-score mean` as a working knob if a task ever needs a stochastic policy.
# Keep `--clone-score density` (the default-equivalent v6 behaviour) for HalfCheetah.
# One caveat on this verdict: with reward normalization ON, mean-clone reaches 3895 @1M,
# competitive with the champion, so the interaction is not settled at 2M.
# =====================================================================================
# OPSD-AdvCond v9 -- keep the clone term's CALIBRATION, cut its SHARPENING channel.
# No PPO anywhere (no ratio, no clipped surrogate, no advantage-weighted policy gradient).
# =====================================================================================
# WHAT THE CLONE TERM ACTUALLY SUPPLIES (established by ablation, not assumption)
#
# The rationalization fit -log p(a_t | s_t, delta_t) does two jobs at once:
#   CALIBRATION -- it makes E[a | s, delta] the TRUE posterior mean. This is the only
#     reason querying the teacher at delta+kappa is an IMPROVEMENT rather than an
#     arbitrary displacement. Load-bearing, and measured twice:
#       clone_coef=0 (remove it)  -> cond_gap exactly 0.000, run dead by 352k, ret -236.
#         The teacher becomes identically the student and the method is a no-op.
#       replace it with InfoNCE (v8, delta-contrastive identifiability) -> the channel got
#         MORE identifiable (info_acc 0.39 vs 0.20 chance, cond_gap 0.030 vs 0.008) and
#         returns were CATASTROPHIC (-70 @1M, -434 @2M with density scoring). Diagnosis:
#         a discriminative objective is invariant to any monotone reparameterization of
#         the mean-vs-delta map -- it needs the conditional means to be DISTINCT, not
#         correctly ORDERED -- so the improvement direction was free to be wrong. Bigger
#         steps, wrong direction. Identifiability is NOT a substitute for calibration.
#   SHARPENING -- log p also pays, without bound, for shrinking the density's spread.
#     On the batch-scale champion this ran as a 5-nat ratchet over 2M steps: clone_nll
#     tracked -entropy to within 0.05 the whole way (the exact identity you get when a
#     model has no residual uncertainty left to explain), while the teacher-student gap
#     the operator consumes fell 3x and distill_kl fell 4x.
#     The pressure is NOT self-fitting -- fitting a distribution to its own samples is a
#     stationary point by the score-function identity. It is the ASYMMETRY plus the shared
#     trunk: the fit is the TEACHER conditional while the data are the STUDENT's rollout
#     actions; conditioning on delta licenses a sharper fit, the shared trunk hands that
#     sharpness to the student, and the teacher can then sharpen again.
#
# THE CHANGE (one term, --clone-score mean)
#     L_clone = || (a_t - mean(s_t, delta_t)) / sd(s_t, delta_t).detach() ||^2 / 2
# Same calibrated conditional-mean regression, but the scale enters only as a detached
# constant, so autograd has NO PATH from this term to the concentration. Verified: the
# loss sits at act_dim/2 = 3.00 (the expected standardized squared residual, i.e. the fit
# is calibrated by construction) and cond_gap matches the density version (0.0074 vs
# 0.0072) with less entropy spent. `--clone-score density` reproduces v6 bit-for-bit.
#
# CHASSIS: built on the measured winner, not on the old champion. cond_scale=ema_rms
# (scale-only conditioning, no mean subtraction) beat batch standardization at every
# matched step -- 664 vs 354 @500k, 2962 vs 2334 @1M, 5453 vs 4390 @2M -- and 5453 @2M is
# the first arm in this lineage to pass PPO's 5012 @2M on the same chassis.
#
# KILL-TELLS
#   losses/clone_nll tracking -losses/entropy   -> the ratchet is back.
#   debug/cond_gap -> 0                          -> calibration lost, teacher == student.
#   losses/clone_loss drifting far from act_dim/2 -> the conditional is mis-scaled.
# =====================================================================================
# INHERITED v6 NOTES (unchanged below)
# =====================================================================================
...
class Args:
    ...
    # per-env seeding. Measured on this chassis (HalfCheetah, 16 envs):
    #   acting forward is LAUNCH-BOUND, not compute-bound -- eager act costs 1149 us at
    #   batch 16 and 1277 us at batch 256, i.e. flat. It dominated the 5.8 ms vec-step
    #   (env stepping was only 0.99 ms of it).
    #   compile mode=reduce-overhead: 1149 -> 347 us at batch 16 (3.3x).
    #   AsyncVectorEnv: 16131 -> 40357 samples/s of raw env stepping (2.5x).
    async_envs: bool = True       # step the MuJoCo envs in worker processes
    compile_act: bool = True      # CUDA-graph the rollout forward
    anneal_lr: bool = True
    gamma: float = 0.99
    ...
    clone_score: str = "mean"     # "density" = plain Beta NLL, reproduces v6 bit-for-bit.
                                  # "mean"    = conditional-mean regression at a DETACHED
                                  #             bandwidth: same calibrated direction, no
                                  #             gradient path to the density's spread.
...
if __name__ == "__main__":
    ...
    # Rollout forward only. The update keeps eager autograd: it is ~5% of wall clock
    # (512 minibatch steps per iteration against 2048 acting steps), so graphing it buys
    # little and would complicate the dual/telemetry paths.
    act_fn = (
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(max(args.actor_epochs, args.critic_epochs)):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v9: the term supplies TWO things and only one of them is wanted.
                #   CALIBRATION: it makes E[a | s, delta] the true posterior mean, which is
                #     the only reason querying at delta+kappa is an IMPROVEMENT rather than
                #     an arbitrary displacement. This is load-bearing. Measured: removing
                #     the term outright (clone_coef=0) drove cond_gap to exactly 0.000 and
                #     the run collapsed by 352k. Replacing it with a purely discriminative
                #     InfoNCE objective (v8) made the channel MORE identifiable
                #     (info_acc 0.39 vs 0.20 chance, cond_gap 0.030 vs 0.008) and returns
                #     CATASTROPHIC (-70 @1M): discrimination only needs the conditional
                #     means to be DISTINCT, not correctly ORDERED, so the improvement
                #     direction was free to be wrong. Calibration is the anchor.
                #   SHARPENING: log p also rewards shrinking the density's spread, without
                #     bound. On the batch-scale champion this ran as a 5-nat ratchet over
                #     2M steps (clone_nll tracked -entropy to within 0.05), closing the
                #     teacher-student gap the operator consumes.
                # So keep the conditional MEAN regression and cut the scale channel: score
                # at a DETACHED bandwidth. Identical calibrated direction, and autograd has
                # no path from this term to the concentration at all.
                if args.clone_score == "mean":
                    sd_ref = (
                        Beta(a_cl, b_cl, validate_args=False).stddev.detach().clamp_min(1e-6)
                    )
                    clone_loss = (
                        (((z_mb - a_cl / (a_cl + b_cl)) / sd_ref) ** 2).sum(-1).mean() * 0.5
                    )
                elif args.clone_score == "density":
                    clone_loss = -(
                        Beta(a_cl, b_cl, validate_args=False).log_prob(z_mb).sum(-1).mean()
                    )
                else:
                    raise ValueError(f"unknown clone_score {args.clone_score!r}")
                with torch.no_grad():
                    clone_nll_ref = -(
                        Beta(a_cl, b_cl, validate_args=False).log_prob(z_mb).sum(-1).mean()
                    )
                ...
                with torch.no_grad():
                    ...
                    clone_nlls.append(clone_nll_ref.item())
        ...
        # Always the true Beta NLL regardless of clone_score, so the 5-nat sharpening
        # ratchet stays directly comparable across modes: watch it vs losses/entropy.
        writer.add_scalar("losses/clone_nll", float(np.mean(clone_nlls)), global_step)
        writer.add_scalar("losses/clone_loss", float(np.mean(clone_losses)), global_step)
