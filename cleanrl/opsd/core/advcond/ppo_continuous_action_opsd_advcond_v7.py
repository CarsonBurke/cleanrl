# OPSD-AdvCond v7 -- the improvement DOSE is specified in information, not in delta units.
# No PPO anywhere (no ratio, no clipped surrogate, no advantage-weighted policy gradient).
# =====================================================================================
# THE PROBLEM v7 ATTACKS (measured, HalfCheetah-v4 seed 1, champion a4c4 mb128)
#   return: 899 @500k, 2581 @1M, 3832 @1.5M, 4452 @2M
#   slope:  +3754 per 1M over the first run's final third, decaying to +1547 by 2M.
#   PPO on this chassis: 3079 @1M, 5012 @2M. So the method closed to 89% of PPO at 2M
#   but is DECELERATING rather than converging upward.
#
# WHY IT DECELERATES -- MEASURED FROM THE CHAMPION'S OWN LOGGED INTERNALS, 0 -> 1.75M
#   advantage_std            3.94   6.25  14.56  14.44  14.72   (GREW 3.2x)
#   cond_gap                 0.030  0.022  0.016  0.011  0.010   (fell 3x)
#   distill_kl               0.162  0.096  0.066  0.046  0.039   (fell 4x)
#   student_entropy         -0.72  -1.90  -3.70  -5.23  -5.71    (collapsed 5 nats)
#   teacher minus student   -0.146 -0.093 -0.068 -0.049 -0.046   (gap closed 3x)
#   explained_variance       0.38   0.95   0.97   0.96   0.94    (critic is fine)
#
#   My first hypothesis was that delta's SPREAD COLLAPSES, shrinking a delta-quoted
#   margin. The data refutes it outright: advantage_std GREW 3.2x. The real mechanism runs
#   the other way. The POLICY SHARPENS (5 nats of entropy gone), and as it sharpens the
#   teacher and the student converge onto each other -- cond_gap 3x down, distill_kl 4x
#   down, the entropy gap between the two contexts 3x down. The improvement operator's
#   signal IS the teacher-student gap, so the operator runs out of things to say. The
#   critic is not the problem (EV 0.94-0.97 throughout).
#
# THE FIX
#   Do not quote the dose in delta units at all -- neither in delta's sd (which grows) nor
#   in nats of some proxy. Each iteration, bisect kappa so that the divergence the margin
#   actually buys equals a fixed target. That pins the exact quantity measured above to be
#   vanishing, on the real distributions, however sharp the policy has become. kappa then
#   floats (measured 0.11 -> 0.31 -> 0.20 -> 0.24 early) to hold the target, and the short
#   run shows distill_kl GROWING 0.097 -> 0.156 where the champion's decayed.
#
#   Expected late-training behaviour, and why both saturation flags exist: a very sharp
#   policy produces LARGE divergence for a small conditioning shift, so the dual may need
#   ever SMALLER kappa and could pin at kappa_min (kappa_floored). If instead the trunk
#   stops responding to the channel, it pins at kappa_max (kappa_saturated). Either end
#   means the dose is no longer controllable and the run should be read accordingly.
#
# A BUG WORTH RECORDING (first implementation was wrong)
#   Targeting KL(teacher || STUDENT) does not work. The student's conditioning block is
#   ZEROED, so that divergence has a floor set by the PRIVILEGE GAP itself, which kappa
#   does not control: measured 0.38-0.95 at kappa=kappa_min against a 0.02 target, so the
#   dual pinned at its floor with nothing to bisect. The correct reference is the
#   RATIONALIZATION context pi(.|s, delta) -- same context, only the margin differs -- for
#   which the divergence is exactly 0 at kappa=0 and monotone in kappa. Verified: kappa_kl
#   lands on 0.0196-0.0200 against a 0.02 target every iteration after the first.
#   (Iteration 1 saturates at kappa_max because an untrained trunk ignores the channel
#   entirely, so no margin produces divergence. Harmless: the teacher is the student there
#   anyway.)
# =====================================================================================
...
class Args:
    ...
    adv_boost: float = 1.0        # fixed margin in cond-scale units; used only when
    #                               kappa_dual is False
    kappa_dual: bool = True       # bisect the margin to hold a constant teacher<->student KL
    target_teacher_kl: float = 0.02  # per-state sum-over-dims KL the dose is held at
    kappa_min: float = 0.02
    kappa_max: float = 8.0
    dual_iters: int = 12          # bisection steps; ~2e-3 resolution on [0.02, 8]
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # forwards per step.
        # ===== THE DOSE IS SPECIFIED IN INFORMATION, NOT IN delta UNITS =================
        # Measured on the champion (a4c4 mb128): the return slope decayed +3754 (to 1M) to
        # +1547 (to 2M). Mechanism: the operator asks the teacher to beat delta_t by kappa,
        # but as the policy improves delta's SPREAD COLLAPSES, so a kappa quoted in any
        # delta-derived unit (batch sd, EMA rms, raw) requests an ever-smaller REAL
        # improvement. The step size self-extinguishes exactly when learning should
        # continue -- and no choice of cond_scale fixes it, since all three shrink with
        # delta.
        # So bisect kappa each iteration to hold the teacher<->student divergence at a
        # fixed target: constant information per step regardless of delta's spread.
        # The divergence is monotone in kappa, so bisection is well posed;
        # `kappa_saturated` reports when the +-clip has capped the achievable divergence
        # and the dual has run out of room at the top.
        if args.kappa_dual:
            with torch.no_grad():
                sub = torch.randperm(args.batch_size, device=device)[: args.minibatch_size]
                o_s, adv_s = b_obs[sub], b_adv_cond[sub]
                # REFERENCE IS THE RATIONALIZATION CONTEXT, NOT THE STUDENT.
                # Targeting KL(teacher || student) does not work: the student's block is
                # ZEROED, so that divergence has a floor set by the privilege gap itself,
                # which kappa does not control. Measured: at kappa=kappa_min the KL was
                # already 0.38-0.95 against a 0.02 target, so the dual pinned at its floor
                # with no room to bisect. Against pi(.|s, delta) -- same context, only the
                # margin differs -- the divergence is exactly 0 at kappa=0 and monotone in
                # kappa, so it measures precisely what the margin buys.
                a_ref, b_ref = agent.policy(o_s, agent.cond_present(adv_s))

                def _kl_at(k):
                    q = (adv_s + k).clamp(-args.adv_cond_clip, args.adv_cond_clip)
                    a_t_s, b_t_s = agent.policy(o_s, agent.cond_present(q))
                    return (
                        beta_kl_per_dim(a_t_s, b_t_s, a_ref, b_ref)
                        .clamp_min(0.0)
                        .sum(-1)
                        .mean()
                        .item()
                ...
                lo_k, hi_k = args.kappa_min, args.kappa_max
                # Bracket diagnostics. The divergence is steeply monotone near zero but
                # NOT globally monotone in kappa: AdvEmbed's features are periodic (freqs
                # to 8), so large margins move the conditioning vector around a torus, and
                # the +-clip flattens it completely. Measured: 8/120 monotonicity
                # violations on stress-perturbed networks, all at KL > 20, i.e. far above
                # any usable target. Bisection is therefore only trusted inside the
                # bracket, and both ends are reported so a silently wrong dose is visible:
                #   kappa_saturated -> even kappa_max cannot REACH the target (dose too
                #                      small, channel ignored)
                #   kappa_floored   -> even kappa_min already EXCEEDS it (dose too large,
                #                      channel hypersensitive)
                kl_hi = _kl_at(hi_k)
                kl_lo = _kl_at(lo_k)
                for _ in range(args.dual_iters):
                    mid_k = 0.5 * (lo_k + hi_k)
                    if _kl_at(mid_k) < args.target_teacher_kl:
                        lo_k = mid_k
                    else:
                        hi_k = mid_k
                kappa = 0.5 * (lo_k + hi_k)
                kappa_kl = _kl_at(kappa)
                kappa_saturated = float(kl_hi < args.target_teacher_kl)
                kappa_floored = float(kl_lo > args.target_teacher_kl)
        else:
            kappa = args.adv_boost
            kappa_kl = float("nan")
            kappa_saturated = 0.0
            kappa_floored = 0.0

        query_all = (b_adv_cond + kappa).clamp(
        ...
        writer.add_scalar("debug/kappa", float(kappa), global_step)
        writer.add_scalar("debug/kappa_kl", float(kappa_kl), global_step)
        writer.add_scalar("debug/kappa_saturated", kappa_saturated, global_step)
        writer.add_scalar("debug/kappa_floored", kappa_floored, global_step)
