# OPSD-AdvCond Distill-Only v1 -- remove clone/policy pressure.
# =====================================================================================
# PURPOSE
#   Test whether privileged advantage conditioning can be learned and transferred by
#   distillation alone. The teacher is a stale snapshot of the same network queried at
#   a relative optimistic TD residual; only the zero-condition student receives actor
#   gradients. There is deliberately no action likelihood / behavior-cloning term.
#
# IMPORTANT IDENTIFIABILITY LIMIT
#   The conditional input pathway is not trained by this objective: the teacher is
#   detached and the student always receives zero conditioning. A random-from-scratch
#   actor therefore has no data target that tells it how to use the privileged channel.
#   This is a falsifiable ablation, not an assumption that bare KL is sufficient.
#
# OBJECTIVE
#   L = KL[pi_snapshot(.|s, delta + boost) || pi(.|s, absent)] + vf_coef * L_V
#   The critic remains a separate state-value objective through the shared trunk.
#   If the run learns, the distillation target itself supplied useful pressure; if the
#   conditional gap remains random or collapses, the missing teacher-identification
#   signal is the reason clone loss was helping.
#
# INHERITED CHASSIS
#   v5 uses separate actor/critic reuse budgets and snapshots the teacher once per
#   rollout update. The only algorithmic change is deleting clone optimization.
# =====================================================================================
...
#
# RL translation of OPSD:
#   privileged signal -> standardized one-step TD residual
#   teacher          -> same policy queried at residual + adv_boost
#   student          -> same policy queried with the privileged block zeroed
#
# The teacher is detached and the student is the only actor branch in autograd. This
# faithfully tests distillation as the sole actor objective, but differs from the paper's
# pretrained teacher: a randomly initialized teacher has no learned privileged behavior.
#
# No action is fed as an input. The rollout action is deliberately absent from the loss;
# otherwise the experiment would silently reintroduce the clone/policy objective.
#
# KILL-TELLS:
#   cond_gap remains at its random initialization -> the privileged pathway is untrained.
#   distill_kl falls toward zero with flat returns -> student absorbed the random teacher.
#   entropy collapses with no return gain -> distillation is imposing arbitrary pressure.
# =====================================================================================
import os
import random
import time
...
class Args:
    ...
    actor_epochs: int = 1         # passes over the frozen teacher target for distillation
    critic_epochs: int = 4        # passes for the value regression only

    # --- OPSD advantage conditioning ---
    adv_boost: float = 1.0        # relative optimistic residual query
    ...
    cond_lambda: float = 0.0      # 1-step TD residual when zero; action-attributable signal
    distill_coef: float = 1.0     # weight on the per-dim teacher->student divergence
    distill_kl_clip: float = 2.0  # per-dim pointwise divergence clip
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(args.num_steps):
            ...
            obs[step] = next_obs
            with torch.no_grad():
                action, _, value_logits = agent.act(next_obs)
        ...
        distill_kls, v_losses, ents, gaps, tea_ents = [], [], [], [], []
        clip_fracs = []
        ...
        for epoch in range(max(args.actor_epochs, args.critic_epochs)):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                obs_mb, adv_mb = b_obs[mb], b_adv_cond[mb]
                n = obs_mb.shape[0]
                a_tea, b_tea = b_t_alpha[mb], b_t_beta[mb]
                query = query_all[mb]

                # Distillation-only actor update: the current student sees no privilege.
                # The realized-advantage branch is intentionally absent from autograd;
                # its only role is to produce the detached teacher snapshot above.
                cond_absent = adv_mb.new_zeros((n, agent.cond_dim))
                a_stu, b_stu, value_logits = agent.policy_and_value(obs_mb, cond_absent)
                ...
                loss = args.distill_coef * distill_loss
