# Q-OPSD causal v1 — matched A/B/C test of whether privileged Q scores add value over
# direct Q/MPO and state-only distillation. No PPO actor loss in any arm.
# =====================================================================================
# OWNERSHIP: isolated file. It does not modify the actively owned AdvCond v1/v2/v3 or
# HOPSD v44 sources. All three arms execute this same byte-identical program and differ
# only by --causal-arm.
#
# MOTIVATION FROM COMPLETED EVIDENCE. The owned single-action AdvCond v1 gave the model
# one tuple (s, a, TD-score) per state. At 1M it reached only 725; its conditional gap
# shrank 0.0122 -> 0.0048 and distill KL 0.0376 -> 0.0069. The score was not identifiable:
# with every state observed once, a rich state encoder can fit a without using the scalar.
#
# IDENTIFICATION FIX. At every STUDENT-ROLLOUT state, sample the same K actions from the
# frozen rollout policy and label all K with the same uncertainty-aware distributional
# twin-Q. The same state now appears with K different (score, action) pairs, so a conditional
# teacher cannot minimize NLL by ignoring score. Scores are within-state ranks, hence
# reward-scale invariant and always in support. Q is warmed before any actor improvement.
#
# SHARED PIPELINE (all arms): identical student Beta policy, 511-bin HL-Gauss MTP value
# critic, distributional twin-Q, replay ratio, 3-step targets, Q updates, K candidate
# actions, uncertainty penalty, teacher/shadow-teacher NLL steps, student KL steps,
# optimizers, KL budget, seed, checkpoints, and evaluation.
#
#   A direct_mpo: Q-ranked candidates -> weighted moment-matched Beta -> student KL.
#     A shadow score teacher receives identical optimizer steps but cannot affect policy.
#   B score_teacher: fit p_T(a|s,score) on all K candidates, query at the best in-support
#     rank, project to the same KL budget, then student KL.
#   C state_teacher: identical B teacher and steps, but score is zero in BOTH fit and query.
#     It estimates the unconditioned candidate marginal, isolating privileged-score value.
#
# CAUSAL READ:
#   B > A and B > C: privileged score-conditioned OPSD earns its machinery.
#   B ~= C > A: teacher/student separation helps; score privilege does not.
#   A ~= B: direct MPO already captures the useful Q information.
#   A > B: conditional rationalization is lossy or harder than direct policy improvement.
#
# Q UNCERTAINTY. Both Qs predict full categorical return distributions by NLL. Candidate
# score = min(expected Q1,Q2) - uncertainty_coef*q_frontier*JS(p1,p2)/log(2). The bounded
# Jensen-Shannon term detects distribution disagreement even when means match. It is used
# identically by all arms; this experiment tests OPSD structure, not uncertainty dosing.
# =====================================================================================
# --- HOPSD v44 lineage retained below as implementation provenance. ---
# --- v40 notes retained below; v44 otherwise inherits the complete v41 implementation. ---
...
# Q SUBSTRATE: replay performs 128 minibatch updates of 256 transitions per 32,768-step
# rollout (UTD 1.0), followed by four matched on-policy hardening epochs. The inherited
# value 32768 counted transitions as optimizer steps and accidentally implied UTD 256;
# all causal arms instead share the intended replay ratio and identical optimizer counts.
...
class Args:
    ...
    # --- Q-OPSD causal contract ---
    causal_arm: Literal["direct_mpo", "score_teacher", "state_teacher"] = "score_teacher"
    candidate_k: int = 8
    score_warmup_iters: int = 8
    uncertainty_coef: float = 1.0
    direct_temperature: float = 1.0
    target_teacher_kl: float = 0.6
    score_q_ev_min: float = 0.5
    target_project_iters: int = 20

    # Common Q substrate. Every causal arm uses distributional twin-Q NLL.
    q_coef: float = 0.0
    teacher_q_coef: float = 0.0
    search_coef: float = 0.0
    search_k: int = 8
    search_tau: float = 1.0
    q_nstep: int = 3
    dist_q: bool = True
    q_updates_per_iter: int = 128  # 128 * 256 samples / 32768 rollout samples = UTD 1.0
    q_batch: int = 256
    q_lr: float = 3e-4
    q_tau: float = 0.005
    q_onpol_coef: float = 1.0
    replay_capacity: int = 1_000_000
    q_onpol_only: bool = False
    q_onpol_epochs: int = 4
    ...
    # v44: the paper uses a fixed detached teacher. RL needs the privileged teacher to learn, so
    # train it online during warmup, then expose the student to a once-per-iteration Polyak target.
    teacher_target_warmup_iters: int = 8
    teacher_target_tau: float = 0.2
    ...
    # FOLLOWABLE and WINNING (4004@500k), so the dual's job is preventing a runaway (>3), not
    # enforcing the old tilt-era 0.27-0.30 band. eta 0.3->0.5 + floor 1e-3->0.02 for faster
    # re-engagement on a breach (the v40 dual was verified correct — this is a retune, not a fix).
    teacher_kl_budget: float = 1.5   # target mean KL(pi_T || pi_S) in nats
...
class QNet(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden=256, support=None):
        super().__init__()
        # v41: support=None -> scalar head (v40 behavior). support given -> HL-Gauss categorical
        # head over its buckets; forward() returns the EXPECTED value (so every Q call site is
        # unchanged), dist_logits() exposes the logits for the distributional TD loss.
...
def distributional_twin_lcb(q1, q2, obs, act, support, frontier, uncertainty_coef):
    """Expected twin-min minus bounded epistemic distribution disagreement.

    JS is normalized to [0,1] and converted to Q units with q_frontier, the measured
    value change under a radius-0.1 action perturbation. Unlike Gaussian variance NLL,
    this cannot be reduced by inflating aleatoric variance and distinguishes equal-mean,
    different-tail critics.
    """
    logp1 = torch.log_softmax(q1.dist_logits(obs, act), dim=-1)
    logp2 = torch.log_softmax(q2.dist_logits(obs, act), dim=-1)
    p1, p2 = logp1.exp(), logp2.exp()
    mix = 0.5 * (p1 + p2)
    logmix = mix.clamp_min(1e-12).log()
    js = 0.5 * (
        (p1 * (logp1 - logmix)).sum(-1) + (p2 * (logp2 - logmix)).sum(-1)
    )
    mean1 = support.probs_to_scalar(p1)
    mean2 = support.probs_to_scalar(p2)
    min_mean = torch.minimum(mean1, mean2)
    js_norm = (js / math.log(2.0)).clamp(0.0, 1.0)
    return min_mean - uncertainty_coef * frontier * js_norm, min_mean, js_norm


def beta_moments(alpha, beta):
    concentration = alpha + beta
    mean = alpha / concentration
    var = alpha * beta / (concentration.square() * (concentration + 1.0))
    return mean, var


def moments_to_beta(mean, var, concentration_cap):
    mean = mean.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
    max_var = 0.995 * mean * (1.0 - mean)
    var = torch.minimum(var.clamp_min(1e-8), max_var)
    concentration = mean * (1.0 - mean) / var - 1.0
    minimum = 1.0 / torch.minimum(mean, 1.0 - mean)
    concentration = torch.maximum(concentration, minimum).clamp(max=concentration_cap)
    return mean * concentration, (1.0 - mean) * concentration


def score_context(scores, act_dim):
    """Map one scalar Q rank into HindsightTeacher's fixed pooled-context width."""
    return torch.cat(
        [
            torch.zeros(*scores.shape, 4 * act_dim, device=scores.device, dtype=scores.dtype),
            scores.unsqueeze(-1),
        ],
        dim=-1,
    )
...
def project_beta_target(raw_a, raw_b, old_a, old_b, budget, iters, concentration_cap):
    """Largest global mixture moment step whose mean forward KL fits the shared budget."""
    old_mean, old_var = beta_moments(old_a, old_b)
    raw_mean, raw_var = beta_moments(raw_a, raw_b)
    old_second = old_var + old_mean.square()
    raw_second = raw_var + raw_mean.square()

    def at(eta):
        mean = old_mean + eta * (raw_mean - old_mean)
        second = old_second + eta * (raw_second - old_second)
        var = (second - mean.square()).clamp_min(1e-8)
        a, b = moments_to_beta(mean, var, concentration_cap)
        kl = beta_kl_per_dim(a, b, old_a, old_b).clamp_min(0.0).sum(-1).mean()
        return a, b, kl
...
@torch.no_grad()
def polyak_update_module(target, source, tau):
    """Move target parameters toward source once, copying non-parameter state exactly."""
    if not 0.0 <= tau <= 1.0:
        raise ValueError(f"teacher_target_tau must be in [0, 1], got {tau}")
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    if not args.dist_q:
        raise ValueError("Q-OPSD causal v1 requires --dist-q for uncertainty-aware labels")
    if args.candidate_k < 2:
        raise ValueError("--candidate-k must be at least 2 for within-state rank conditioning")
    if args.search_k != args.candidate_k:
        raise ValueError("--search-k and --candidate-k must match to preserve the shared search budget")
    if args.phi_mode != "pooled" or args.teacher_sees_g:
        raise ValueError("Q-OPSD causal v1 reserves pooled context exclusively for Q scores")
    ...
    # v44: deepcopy preserves v41's RNG stream, hence Q initialization and warmup behavior.
    # The target is never optimized; it supplies only detached full-distribution student targets.
    teacher_target = copy.deepcopy(teacher).requires_grad_(False)
    teacher_target.eval()
    ...
    q1 = QNet(obs_dim, act_dim, support=q_support).to(device)
    q2 = QNet(obs_dim, act_dim, support=q_support).to(device)
    q1_target = QNet(obs_dim, act_dim, support=q_support).to(device)
    q2_target = QNet(obs_dim, act_dim, support=q_support).to(device)
    ...
    def td_forward(s_obs, s_boot, s_act, s_nrew, s_disc):
        ...
        # distributional: target = projected n-step Bellman of the min-expected-value target net.
        with torch.no_grad():
            a_boot = student_env_action(s_boot, reparam=False)
            ...
            p_boot = torch.where((v1 <= v2).unsqueeze(-1), p1, p2)
        ...
        q1_mean = q_support.probs_to_scalar(torch.softmax(lg1, dim=-1)).detach().mean()
        return q_loss, q1_mean
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            for start in range(0, obs_flat_roll.shape[0], args.minibatch_size):
                ...
                summ = None
                pooled_sl = score_context(
                    torch.zeros(sl.stop - sl.start, device=device, dtype=obs.dtype),
                    act_dim,
                )
                vt_chunks.append(
        ...
        # Q-OPSD does not backpropagate Q through an actor. Q labels one shared candidate
        # set instead, so the old DPG guardrail is intentionally absent in all three arms.
        qgrad_ratio_teacher = 0.0
        teacher_dpg_atten = 1.0
        arsample_std = float("nan")
        ...
        # ================= Q-labeled candidate set (shared by A/B/C) =====================
        score_active = (
            iteration > args.score_warmup_iters
            and np.isfinite(q_ev)
            and q_ev >= args.score_q_ev_min
        )
        k = args.candidate_k
        rank_axis = torch.arange(k, device=device, dtype=torch.float32)
        rank_axis = (rank_axis - rank_axis.mean()) / rank_axis.std(unbiased=False)
        query_score = rank_axis[-1].item()
        frontier_t = torch.as_tensor(q_frontier_norm, device=device, dtype=torch.float32)
        cand_z_chunks, score_chunks = [], []
        old_a_chunks, old_b_chunks = [], []
        direct_a_chunks, direct_b_chunks = [], []
        js_chunks, top_gap_chunks, ess_chunks = [], [], []

        with torch.no_grad():
            for start in range(0, args.batch_size, args.minibatch_size):
                sl = slice(start, start + args.minibatch_size)
                _, _, _, _, _, old_a, old_b = agent.get_action_and_value(
                    b_obs[sl], b_latent_zs[sl]
                )
                m = old_a.shape[0]
                z_k = Beta(old_a, old_b).sample((k,)).clamp(
                    SAMPLE_EPS, 1.0 - SAMPLE_EPS
                )
                a_k = q_act_low + (q_act_high - q_act_low) * z_k
                obs_k = (
                    b_obs[sl]
                    .unsqueeze(0)
                    .expand(k, m, -1)
                ...
                lcb_k, _, js_k = distributional_twin_lcb(
                    q1,
                    q2,
                    obs_k,
                    a_k.reshape(k * m, -1),
                ...
                lcb_k = lcb_k.reshape(k, m)
                js_k = js_k.reshape(k, m)
                order = lcb_k.argsort(dim=0)
                ranks = torch.empty_like(lcb_k)
                ranks.scatter_(0, order, rank_axis[:, None].expand(-1, m))
                weights = torch.softmax(
                    (lcb_k - lcb_k.max(dim=0, keepdim=True).values)
                    / (args.direct_temperature * frontier_t),
                    dim=0,
                )
                direct_mean = (weights.unsqueeze(-1) * z_k).sum(0)
                direct_second = (weights.unsqueeze(-1) * z_k.square()).sum(0)
                direct_var = (direct_second - direct_mean.square()).clamp_min(1e-8)
                direct_a, direct_b = moments_to_beta(
                    direct_mean,
                    direct_var,
                    float(args.teacher_conc_cap) * 2.0,
                )
                behavior_lcb, _, _ = distributional_twin_lcb(
                    q1,
                    q2,
                    b_obs[sl],
                    b_actions[sl],
                ...
                cand_z_chunks.append(z_k)
                score_chunks.append(ranks)
                old_a_chunks.append(old_a)
                old_b_chunks.append(old_b)
                direct_a_chunks.append(direct_a)
                direct_b_chunks.append(direct_b)
                js_chunks.append(js_k.mean())
                top_gap_chunks.append((lcb_k.max(0).values - behavior_lcb).mean())
                ess_chunks.append(
                    (1.0 / weights.square().sum(0).clamp_min(1e-8)).mean()
                )

            b_cand_z = torch.cat(cand_z_chunks, dim=1)
            b_cand_score = torch.cat(score_chunks, dim=1)
            b_old_alpha = torch.cat(old_a_chunks)
            b_old_beta = torch.cat(old_b_chunks)
            b_direct_alpha = torch.cat(direct_a_chunks)
            b_direct_beta = torch.cat(direct_b_chunks)
            candidate_js = torch.stack(js_chunks).mean().item()
            candidate_top_gap = torch.stack(top_gap_chunks).mean().item()
            candidate_ess = torch.stack(ess_chunks).mean().item()

        # ================= Phase 1: teacher/shadow-teacher regression ====================
        # Every arm performs these exact optimizer steps. A's teacher is a compute-matched
        # shadow whose output is deliberately ignored. B receives Q ranks; C zeros them.
        ...
        for _epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                fit_scores = b_cand_score[:, mb]
                if args.causal_arm == "state_teacher" or not score_active:
                    fit_scores = torch.zeros_like(fit_scores)
                fit_ctx = score_context(fit_scores.reshape(-1), act_dim)
                ta, tb = teacher.actor_params_from(obs_k, fit_ctx, None, None)
                teacher_actor_loss = -Beta(ta, tb).log_prob(
                    b_cand_z[:, mb].reshape(k * m, act_dim)
                ).sum(-1).mean()

                zero_ctx = score_context(
                    torch.zeros(m, device=device, dtype=b_obs.dtype), act_dim
                )
                t_value_logits = teacher.critic_logits_from(
                    b_obs[mb], zero_ctx, None
                )
                t_target = b_target_probs[mb, 0].to(
                    device=device, non_blocking=True
                )
                t_v_loss = -(
                    t_target * torch.log_softmax(t_value_logits, dim=-1)
                ).sum(-1).mean()
                teacher_loss = (
                ...
                nn.utils.clip_grad_norm_(
                    teacher.parameters(), args.teacher_grad_clip
                )
                teacher_optimizer.step()
                teacher_nll_acc += teacher_actor_loss.detach().item()
                teacher_mb_count += 1
                last_t_v_loss = t_v_loss.detach()

        # One exact snapshot per iteration. Student targets never move during Phase 2.
        teacher_target.load_state_dict(teacher.state_dict())
        teacher_target.eval()

        # ================= Build one frozen full-distribution target ======================
        target_a_chunks, target_b_chunks, target_kl_chunks = [], [], []
        query_gap_chunks = []
        with torch.no_grad():
            for start in range(0, args.batch_size, args.minibatch_size):
                sl = slice(start, start + args.minibatch_size)
                old_a, old_b = b_old_alpha[sl], b_old_beta[sl]
                if not score_active:
                    raw_a, raw_b = old_a, old_b
                elif args.causal_arm == "direct_mpo":
                    raw_a, raw_b = b_direct_alpha[sl], b_direct_beta[sl]
                else:
                    query = query_score if args.causal_arm == "score_teacher" else 0.0
                    ...
                    raw_a, raw_b = teacher_target.actor_params_from(
                        b_obs[sl],
                        score_context(query_scores, act_dim),
                        None,
                        None,
                    )
                    zero_a, zero_b = teacher_target.actor_params_from(
                        b_obs[sl],
                        score_context(torch.zeros_like(query_scores), act_dim),
                        None,
                        None,
                    )
                    query_gap_chunks.append(
                        (
                            raw_a / (raw_a + raw_b)
                            - zero_a / (zero_a + zero_b)
                        )
                        .abs()
                        .mean()
                    )
                target_a, target_b, target_kl = project_beta_target(
                    raw_a,
                    raw_b,
                    old_a,
                    old_b,
                    args.target_teacher_kl,
                    args.target_project_iters,
                    float(args.teacher_conc_cap) * 2.0,
                )
                target_a_chunks.append(target_a)
                target_b_chunks.append(target_b)
                target_kl_chunks.append(target_kl)
            b_policy_target_a = torch.cat(target_a_chunks)
            b_policy_target_b = torch.cat(target_b_chunks)
            target_teacher_kl = torch.stack(target_kl_chunks).mean().item()
            query_gap = (
                torch.stack(query_gap_chunks).mean().item()
                if query_gap_chunks
                else 0.0
            )

        # ================= Phase 2: student pure KL + unchanged value critic =============
        acc_distill_kl = torch.zeros((), device=device)
        acc_clipfrac = torch.zeros((), device=device)
        student_mb_count = 0
        student_stopped = False
        last_v_loss = last_pg_loss = last_entropy = last_approx_kl = None
        for _epoch in range(args.update_epochs):
            np.random.shuffle(b_inds)
            for start in range(0, args.batch_size, args.minibatch_size):
                mb = b_inds[start : start + args.minibatch_size]
                tp_mb = b_target_probs[mb].to(
                    device=device, non_blocking=True
                )
                tm_mb = b_target_mask[mb].to(
                    device=device,
                    dtype=torch.float32,
                    non_blocking=True,
                )
                (
                    ...
                    tp_mb,
                    tm_mb,
                    b_policy_target_a[mb],
                    b_policy_target_b[mb],
                )
        ...
        teacher_nll_mean = teacher_nll_acc / max(teacher_mb_count, 1)
        distill_kl_mean = (acc_distill_kl / _sden).item()
        distill_clipfrac_mean = (acc_clipfrac / _sden).item()
        teacher_q_gap = candidate_top_gap
        total_teacher_kl = target_teacher_kl
        search_top_gap = candidate_top_gap
        search_ess = candidate_ess
        phi_dropout_frac = 0.0
        target_online_kl = 0.0
        target_mb_frac = 1.0
        teacher_target_active = score_active

        y_pred, y_true = b_values.cpu().numpy(), b_returns.cpu().numpy()
        var_y = np.var(y_true)
        explained_var = np.nan if var_y == 0 else 1 - np.var(y_true - y_pred) / var_y

        # Full-batch diagnostics use the exact causal contexts, never the inherited
        # future-action features.
        with torch.no_grad():
            t_vals, t_ents, s_ents, gaps = [], [], [], []
            diagnostic_query = (
                query_score
                if args.causal_arm == "score_teacher" and score_active
                else 0.0
            )
            for start in range(0, args.batch_size, args.minibatch_size):
                sl = slice(start, start + args.minibatch_size)
                n_diag = b_obs[sl].shape[0]
                diag_scores = torch.full(
                    (n_diag,),
                    diagnostic_query,
                    device=device,
                    dtype=b_obs.dtype,
                )
                diag_ctx = score_context(diag_scores, act_dim)
                ta, tb = teacher_target.actor_params_from(
                    b_obs[sl], diag_ctx, None, None
                )
                t_ents.append(
                    Beta(ta, tb).entropy().sum(-1).mean().item()
                )
                _, _, _, s_ent, _, sa, sb = agent.get_action_and_value(
                    b_obs[sl], b_latent_zs[sl]
                )
                ...
                zero_ctx = score_context(
                    torch.zeros_like(diag_scores), act_dim
                )
                t_vals.append(
                    hl_support.to_scalar(
                        teacher_target.critic_logits_from(
                            b_obs[sl], zero_ctx, None
                        )
                    )
                )
            ...
            teacher_ev = (
                np.nan
                if var_y == 0
                else 1 - np.var(y_true - t_vals) / var_y
            )
        ...
        writer.add_scalar("debug/teacher_target_online_kl", target_online_kl, global_step)
        writer.add_scalar("debug/teacher_target_active", float(teacher_target_active), global_step)
        writer.add_scalar("debug/teacher_target_mb_frac", target_mb_frac, global_step)
        writer.add_scalar("debug/score_active", float(score_active), global_step)
        writer.add_scalar("debug/candidate_js", candidate_js, global_step)
        writer.add_scalar("debug/candidate_top_lcb_gap", candidate_top_gap, global_step)
        writer.add_scalar("debug/candidate_ess", candidate_ess, global_step)
        writer.add_scalar("debug/query_gap", query_gap, global_step)
        writer.add_scalar("debug/projected_target_kl", target_teacher_kl, global_step)
