# OPSD-Conseq v1 -- decompose the credit scalar the teacher conditions on.
# =====================================================================================
# Parent: ppo_continuous_action_opsd_advcond_v6.py (family champion, 8718 @8M vs PPO 8278).
# ONE mechanism changes. `--cond-mode delta` reproduces the parent bit for bit.
#
# THE QUESTION THIS FILE ANSWERS
#   Where does optimization pressure come from, once you stop hand-coding a policy loss?
#
#   Every PG method picks a coefficient w_t multiplying grad log pi and argues about its
#   form: w = U (REINFORCE), U clipped by a ratio (PPO), f(U) (AWR/PMPO), sigma(U*l/eta)*U
#   (Osband's delight gradient). All of them are a HAND-DESIGNED map from credit to update.
#   Conditioning a generative actor on realized credit removes that map -- the network
#   decides how to use credit -- but it appears to remove the pressure with it, because
#   the objective becomes maximum likelihood on your own behaviour, and every policy is a
#   fixed point of "predict what you did".
#
#   IT DOES NOT. The pressure is Bayes, not a loss term. The teacher is queried at a
#   BETTER outcome than the one that happened, so
#       T(a|s) = p(a | s, outcome better)  propto  p(outcome better | s, a) * S(a|s),
#   i.e. the teacher is the posterior over actions given a good outcome and the student is
#   the prior. Distilling S <- T is posterior sharpening, and T = S if and only if
#   p(outcome better | s, a) does not depend on a. So the fixed points are exactly the
#   states where the outcome is action-independent -- local optimality -- and NOT every
#   policy. Nothing is hand-coded: the weighting is whatever likelihood the network fit.
#
#   The corollary is the whole design. Pressure and channel informativeness are THE SAME
#   QUANTITY: if the conditioning channel carries no information about the action, then
#   p(better | s, a) is flat in a, T = S, and the method is inert. So the way to get
#   pressure is not to add a term. It is to make the conditioning channel action-
#   attributable and keep it that way.
#
# WHY THE PARENT LOST ITS PRESSURE (its own telemetry, HalfCheetah-v4 seed 1)
#            step      0k    500k     1M    1.5M   1.75M
#     advantage_std   3.94    6.25  14.56   14.44   14.72     <- channel got 3.7x LOUDER
#     cond_gap       0.030   0.022  0.016   0.011   0.010     <- network used it 3x LESS
#     distill_kl     0.162   0.096  0.066   0.046   0.039     <- so the teacher became the student
#   The channel did not starve. The network learned to ignore it, and was right to.
#   delta_t = r_t + gamma V(s_{t+1}) - V(s_t) is a SUM of an exactly observed term with
#   zero noise and a difference of two large learned values whose error grows with the
#   reward scale. One scalar makes the clean part share a channel with the noisy part, and
#   the noisy part wins precisely as the policy sharpens (entropy -0.5 -> -9.6) and the
#   reward scale grows. The parent had already moved from GAE(0.95) to the 1-step residual
#   for exactly this reason and had already made the channel expensive to ignore with
#   fixed Fourier features. Neither helps: this is an SNR problem, not an encoding one.
#
# MEASURED, BEFORE WRITING THE FILE
#   1. The compressed scalar is un-learnable. Synthetic Beta policy, 6 dims, credit with
#      10% action-attributable variance: R^2(action | credit scalar) = 0.0000, against
#      R^2(action | surprisal) = 0.9506. A channel that predicts nothing about the action
#      is a channel MLE drops for free -- which is what cond_gap 0.010 is showing.
#   2. The observed part is learnable. 60k real HalfCheetah-v4 steps, gain in R^2 over
#      the state alone:
#              channel                    R^2(||a||^2)   R^2(a_1)
#              r_t                            +0.1001     +0.0839
#              reward_ctrl (= -0.1||a||^2)    +0.9995     +0.0000
#              s_{t+1}                        +0.0093     +0.4661
#      Reward variance is 94.4% forward-velocity, 6.1% control cost, and the control cost
#      is a DETERMINISTIC function of the action (corr with -||a||^2 is 1.000000). So r_t
#      is action-attributable, exactly observed, and its SNR does not decay as the critic
#      improves. It is the one channel that cannot go inert.
#
# THE CHANGE
#   Feed the teacher the three quantities delta is BUILT from, separately scaled:
#     r_res = r_t - EMA(r)                observed reward surprise. No critic in it.
#     dv    = gamma V(s_{t+1}) - V(s_t)   the critic's view of the state change.
#     adv   = GAE(gae_lambda)             long-horizon credit.
#   r_res + EMA(r) + dv = delta, so channels 1 and 2 SPAN the parent's scalar: v6 is the
#   special case with readout weights pinned at (1, 1) and horizon pinned at lambda=0.
#   Here the network learns the readout and the effective horizon, per state -- which is
#   the point. It is not a wider input; it is the removal of a hand-coded compression,
#   the same act as removing the hand-coded w_t.
#   The query still raises every channel by adv_boost: "same state, better outcome".
#
# FALSIFIABLE PREDICTIONS
#   1. debug/chan_use_r_res does NOT decay. If all three channels decay like v6's single
#      one did, the decomposition is not the cure and the disease is elsewhere.
#   2. debug/cond_gap stops collapsing. It is the same formula as v6's, so the two curves
#      are directly comparable, and v6's is on disk.
#   3. The edge over PPO stops DECAYING. The lineage signature is +24% @1M shrinking to
#      +6.4% @8M; a pressure fix must flatten that, not just lift the early curve.
#   4. Throughput roughly unchanged: two extra Fourier blocks on a 64-wide trunk, plus
#      n_channels no-grad probe forwards per ITERATION (not per minibatch). If it comes
#      out materially slower, something excluded crept in.
#   NOT CLAIMED: this is an estimator and allocation fix, not a new information source.
#   The improvement direction is still built from the action-credit correlation, which for
#   an exponential-family policy is the policy gradient's own information. What is bought
#   is a better-conditioned estimate of it, a learned rather than pinned readout, and
#   equal per-state allocation. At a FIXED 8M budget that is exactly the lever that moves
#   score, but it is not a claim about the asymptote.
# =====================================================================================
# INHERITED v6 NOTES (unchanged below; the RESULT banner refers to the PARENT, not to
# this file, which has no run yet)
# =====================================================================================
...
class Args:
    ...
    adv_embed_freqs: int = 8      # sinusoidal features per phase; each channel gets 2x this
    cond_lambda: float = 0.0      # GAE lambda for the delta channel; 0 = 1-step TD residual.
                                  # Only read when cond_mode == "delta".
    # --- v1: DECOMPOSE THE CREDIT SCALAR -------------------------------------------
    # "delta" reproduces v6 bit for bit (one channel, GAE(cond_lambda)).
    # "rvg"   splits that same information into the three quantities it is BUILT from:
    #           r_res = r_t - EMA(r)                    observed reward surprise. No critic
    #                                                   anywhere in it, so its noise is
    #                                                   exactly zero and does not grow.
    #           dv    = gamma*V(s_{t+1}) - V(s_t)       the critic's view of the state change.
    #           adv   = GAE(gae_lambda)                 long-horizon credit.
    #         delta = r_res + EMA(r) + dv, so channels 1+2 SPAN v6's scalar: v6 is the
    #         special case where the readout weights are pinned at (1, 1) and the horizon
    #         at lambda=0. Here the network learns both, per state.
    cond_mode: str = "rvg"        # "rvg" | "delta"
    cond_channel_probe: int = 4096  # states per iteration for the channel-usage telemetry;
                                    # 0 disables. One no-grad forward per channel.
    ...
    n_cond_channels: int = 0      # derived from cond_mode
...
class CreditEmbed(nn.Module):
    """Fixed Fourier features of EVERY credit channel, one Fourier block per channel.
    ...
    @property
    def dim(self):
        return 2 * self.n_freq * self.n_channels

    def forward(self, cond):
        # (B, C) -> (B, C, F) -> (B, C * 2F). At C == 1 the flattened layout is
        # element-for-element identical to v6's, so cond_mode="delta" is bit-exact.
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        H = args.hidden
        self.credit_embed = CreditEmbed(args.adv_embed_freqs, args.n_cond_channels)
        self.cond_dim = self.credit_embed.dim
    ...
    def cond_present(self, cond):
        """Privileged context: Fourier features of every scaled credit channel."""
        return self.credit_embed(cond)
...
if __name__ == "__main__":
    ...
    assert args.cond_mode in ("rvg", "delta"), f"unknown cond_mode {args.cond_mode!r}"
    args.n_cond_channels = 1 if args.cond_mode == "delta" else 3
    # Which channels the improvement query raises. Every channel has a natural zero and
    # is "advantage-like" (positive = better than expected), so the query raises all of
    # them: "the same state, but the outcome came out better than it did".
    COND_NAMES = ("delta",) if args.cond_mode == "delta" else ("r_res", "dv", "adv")
    query_boost_scale = 1.0 / math.sqrt(args.n_cond_channels)
    ...
    cond_ms = torch.zeros(args.n_cond_channels, device=device)
    rew_ema = torch.zeros((), device=device)   # natural zero of the reward-surprise channel
    # The channel probe is pure telemetry, so it MUST NOT touch the training RNG stream:
    # drawing its indices from the global generator shifts every subsequent minibatch
    # permutation and silently makes cond_mode="delta" non-reproducible. (Caught by the
    # bit-exactness gate, which failed on student_entropy at iteration 1 while
    # teacher_entropy still matched -- i.e. the teacher snapshot was fine and the update
    # order was not.)
    probe_gen = torch.Generator(device=device)
    probe_gen.manual_seed(args.seed + 0x5EED)
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.num_steps)):
                ...
                if args.cond_mode == "delta":
                    last_cond = (
                        delta + args.gamma * args.cond_lambda * lambda_nonterminal * last_cond
                    )
            ...
            # --- THE CREDIT CHANNELS ------------------------------------------------
            # v6 fed ONE scalar, delta_t = r_t + gamma V(s_{t+1}) - V(s_t), and measured
            # its own death: cond_gap 0.030 -> 0.010 over 1.75M steps while
            # advantage_std TRIPLED 3.94 -> 14.7. The channel did not starve; the network
            # learned to ignore it, and was right to. delta is a SUM of an exactly
            # observed term (r_t, zero noise) and a difference of two large learned values
            # (critic noise, and that noise grows with the reward scale). Sharing one
            # scalar means the clean part is swamped by the noisy part exactly when the
            # policy sharpens and the reward scale grows -- i.e. always, and increasingly.
            # Splitting them lets the network weight each per state instead of being
            # pinned at delta's implicit (1, 1).
            if args.cond_mode == "delta":
                b_cond = cond_adv.reshape(-1, 1)
            else:
                rew_batch_mean = rewards.mean()
                rew_ema.mul_(args.cond_ema_beta).add_((1.0 - args.cond_ema_beta) * rew_batch_mean)
                rew_ref = rew_ema / (1.0 - args.cond_ema_beta ** iteration)
                dv = (
                    args.gamma
                    * next_values
                ...
                b_cond = torch.stack(
                    [
                        (rewards - rew_ref).reshape(-1),  # observed reward surprise
                        dv.reshape(-1),                   # critic's view of the state change
                        advantages.reshape(-1),           # long-horizon credit, GAE(lambda)
                    ],
                    dim=-1,
                )
            ...
            # Scaling is PER CHANNEL: the three carry different units and different
            # non-stationarity, and one shared scale would let the largest of them decide
            # the resolution of the other two.
            if args.cond_scale == "batch":
                b_cond = (b_cond - b_cond.mean(0)) / (b_cond.std(0) + 1e-8)
            elif args.cond_scale == "ema_rms":
                ms = b_cond.square().mean(0)
                cond_ms.mul_(args.cond_ema_beta).add_((1.0 - args.cond_ema_beta) * ms)
                bias = 1.0 - args.cond_ema_beta ** iteration
                b_cond = b_cond / (cond_ms / bias).sqrt().clamp_min(1e-8)
            elif args.cond_scale == "raw":
                pass
            else:
                raise ValueError(f"unknown cond_scale {args.cond_scale!r}")
            cond_rms_per_ch = b_cond.square().mean(0).sqrt()
            # Overall conditioning scale across channels. At C == 1 this is exactly the
            # parent's value, so the tag stays comparable to v6's curve on disk instead of
            # silently narrowing to "the r_res scale".
            cond_scale_used = b_cond.square().mean().sqrt().item()
            b_adv_cond = b_cond.clamp(-args.adv_cond_clip, args.adv_cond_clip)
            cond_clipped = (b_cond.abs() >= args.adv_cond_clip).float().mean().item()
            cond_clip_per_ch = (b_cond.abs() >= args.adv_cond_clip).float().mean(0)
        ...
        # DOSE MUST BE MATCHED TO THE PARENT OR THE COMPARISON IS UNINTERPRETABLE.
        # adv_boost is a STEP SIZE, and this lineage measured how much that matters:
        # kappa 1/2/3/4 -> 5365/6077/5624/3128 @2M. Raising all C channels by the full
        # boost would displace the query by sqrt(C)x the parent's distance off the
        # observed data manifold, so an rvg-vs-v6 result would confound "decomposed the
        # channel" with "took a 1.7x bigger step". Dividing by sqrt(C) holds the query
        # displacement NORM equal to the parent's, so adv_boost keeps its meaning across
        # modes and C == 1 stays bit-exact. losses/distill_kl is the outcome check: it is
        # the same formula as v6's and v6's curve is on disk.
        query_all = (b_adv_cond + boost * query_boost_scale).clamp(
        ...
        # ===== CHANNEL USAGE: the falsification instrument =============================
        # v6's disease was measurable but not ATTRIBUTABLE: one cond_gap for one channel,
        # so "the trunk stopped using the credit signal" could not be localized. With the
        # scalar decomposed, ablate each channel out of the query and measure how far the
        # teacher's mean action moves. A channel whose removal moves nothing is a channel
        # the network has learned to ignore. Prediction under this file's hypothesis:
        # chan_use_r_res stays alive (r_t is exactly observed, and on HalfCheetah it is
        # measurably action-attributable: +0.100 R^2 on ||a||^2 and +0.084 on a_1 over the
        # state alone, 60k real steps), while the two critic-derived channels are the ones
        # free to decay. If ALL of them decay like v6's did, the decomposition is not the
        # cure and the disease is elsewhere.
        chan_use = [float("nan")] * args.n_cond_channels
        if args.cond_channel_probe > 0:
            with torch.no_grad():
                probe = torch.randperm(
                    args.batch_size, device=device, generator=probe_gen
                )[: args.cond_channel_probe]
                q_ref = query_all[probe]
                pa, pb = agent.policy(b_obs[probe], agent.cond_present(q_ref))
                m_ref = pa / (pa + pb)
                for channel in range(args.n_cond_channels):
                    q_ablated = q_ref.clone()
                    q_ablated[:, channel] = 0.0
                    qa, qb = agent.policy(b_obs[probe], agent.cond_present(q_ablated))
                    chan_use[channel] = (qa / (qa + qb) - m_ref).abs().mean().item()
        ...
        for channel, name in enumerate(COND_NAMES):
            writer.add_scalar(f"debug/cond_rms_{name}", cond_rms_per_ch[channel].item(), global_step)
            writer.add_scalar(f"debug/chan_use_{name}", chan_use[channel], global_step)
            writer.add_scalar(
                f"debug/cond_clip_{name}", cond_clip_per_ch[channel].item(), global_step
            )
