# OPSD-JacTeach v3-DeepValue -- the actor-side winner and the one value-side change,
# run together. One question: do the two gains ADD?
# =====================================================================================
# MECHANISM. This file is exactly the union of two parents and nothing else:
#
#   actor side  <- ppo_continuous_action_opsd_jacteach_v1.py   (VERBATIM, untouched)
#                  the analytic one-step-Jacobian teacher, rotated by an EXACT in-plane
#                  angle (`jac_cos`), dosed to the parent's own DELIVERED clipped KL by
#                  per-iteration bisection.
#   value side  <- ppo_continuous_action_opsd_advcond_v11.py   (VALUE HUNKS ONLY)
#                  the single zero-init Linear(H, mtp*num_bins) readout replaced by
#                  `DeepValueHead` -- a depth-B adaLN residual stack over the mtp horizon
#                  TOKENS -- plus `ValueTimeEmbed` and the args `value_blocks`/`value_mult`.
#
# THE PORT WAS DONE AS A DIFF, NOT BY HAND, AND THAT WAS CHECKED. `diff(jacteach_v1 -> this
# file)` is BYTE-IDENTICAL to `diff(advcond_v6 -> advcond_v11)` once v11's header block and
# its `+import math` are removed -- jacteach_v1 already imports math, for math.sqrt in the
# rotation. 117 changed lines, zero mismatches. So every jacteach control and mechanism is
# literally unmodified: the exact rotation, the delivered-KL bisection, the `fork_rng` head
# construction, the per-dim `b_valid` masking, and all `debug/jac_*` telemetry. Nothing
# else of v11 is here -- its header, its ladder table and its predictions belong to it and
# are not restated as this file's.
#
# WHY THIS COMBINATION AND NO OTHER -- INHERITED evidence (OPSD_FAMILY.md §4/§5/§6.7). I
# did NOT measure any number in this section:
#   * The 8M band contains EVERYTHING this family has built. Four arms land in 8812-8882 @8M
#     -- v6 8812, v11 8882, jacteach cos 0.85 8862, jacteach cos 0.95 8859; last-20
#     8718 +-498 / 8842 +-152 / 8854 +-114 / 8915 +-131. A 70-point spread against CIs of
#     +-114..+-498. §6.7 concludes the 8M asymptote is not set by the improvement direction.
#     STATED PRECISELY, BECAUSE §6.7's OWN PHRASING ("four materially different actor-side
#     mechanisms, one number") BLURS IT: three of those four (v6, cos 0.85, cos 0.95) differ
#     from each other ONLY on the actor side, but v11 differs from v6 ONLY on the value side
#     -- it is the readout arm, not an actor arm. So the band is not "the actor lever is
#     spent"; it is "every lever tried so far, actor OR value, lands in the same band". That
#     is a strictly worse prior for this file than §6.7's framing implies, and it is the
#     reason falsification 2 below is written as bluntly as it is.
#     (§4's table reads 8819 for the same v6 arm that §6.7's table reads 8812; the family doc
#     is internally inconsistent by 7 points, which is immaterial against a ~400 floor. The
#     inherited v6 table further down THIS file carries the 8819 figure.)
#   * §6.7 records the value pathway as the only axis that has moved the 8M number at all --
#     and records the size of that move as +124, which §5's noise floor (~400) puts INSIDE
#     NOISE. §4 lists the same result under FALSIFIED predictions: "deep value readout does
#     not transfer: +1355 on one chassis, +124 (inside noise) on advcond_v11", both deep-head
#     runs sharing a signature of higher EV early and LOWER EV at 8M (0.914 vs 0.973). That
#     qualifier is load-bearing and is kept rather than dropped: the value-side prior here is
#     WEAK, not established.
#
# HYPOTHESIS. The two levers touch disjoint pathways -- one changes where the teacher points,
# the other changes how the critic reads a distribution out of the same trunk feature. If the
# 8M wall is the sum of two separable deficits, the gains should add: ~8915 + ~124 ~= 9040.
#
# FALSIFICATION, written so the NULL is still informative:
#   1. If this file lands back inside the 8812-8882 band, the two levers do NOT compose and
#      the wall is ONE shared bottleneck, not two independent ones. That is the outcome the
#      priors above actually favour, and it is the more useful of the two answers.
#   2. THE POWER PROBLEM IS DISCLOSED UP FRONT. The value-side gain being added is +124
#      against a ~400 floor, so a null result CANNOT distinguish "the mechanisms do not
#      compose" from "the value-side gain was never real". A win is informative; a null is
#      weak evidence, and reading it as strong would be exactly the overclaim §5 exists to
#      prevent. This is a cheap combination test, not a decisive experiment.
#   3. Mechanism check, independent of return: `losses/explained_variance` must separate from
#      a matched jacteach_v1 reference. If the return moves and EV does not, the attribution
#      to the readout is wrong. This is v11's own prediction 3, repeated because it is the
#      only thing that distinguishes "the readout did it" from noise.
#   4. Disjointness check: `debug/jac_align` and `debug/jac_dose_scale` must behave as in
#      jacteach_v1 at matched config. If the deep readout perturbs them, the value change is
#      NOT disjoint from the actor mechanism and the "union of two independent levers"
#      framing is false at the outset.
#
# ---- WHAT I VERIFIED MYSELF, ON THIS FILE. Everything below is measured here. ----
#
# * `python -m py_compile` passes.
#
# * PARAMETER COUNT. Critic head 196,224 -> 163,648 params at H=64, 511 bins, mtp 6,
#   depth 3, mult 2: 0.834x, i.e. this lever REMOVES 32,576 parameters. A "more capacity"
#   explanation is therefore not available for anything it does.
#
# * INIT PROPERTIES, all exact, not approximate: logits are all-zero at init
#   (max|logits| = 0.000e+00), the bin softmax is exactly uniform (max|p - 1/511| = 0.0),
#   V(s) is exactly 0 on the symexp support (max|V| = 0.000e+00), and every block is exactly
#   the identity (max|z_B - z_start| = 0.000e+00). The critic starts where v6's zero-init
#   Linear started, so the actor sees no init shock.
#
# * THE ALL-ZERO FIXED POINT IS REAL, which is the whole reason `z_start` is a learned
#   nonzero parameter: with z_start forced to 0 the total gradient norm w.r.t. the head is
#   exactly 0.000e+00; with the learned start it is nonzero (1.34e-01 under my probe loss).
#   v11 documents 3.75e-02 for the nonzero case -- that magnitude is loss- and batch-
#   dependent, so I claim only the 0-vs-nonzero dichotomy the design rests on, not v11's
#   figure.
#
# * CONTROL IS EXACT. `--jac-cos 1.0 --env-id HalfCheetah-v4 --num-envs 4 --num-steps 256
#   --num-minibatches 4 --total-timesteps 20480 --seed 1` is BIT-IDENTICAL to
#   `ppo_continuous_action_opsd_advcond_v11.py` on the same line: 17 TensorBoard scalars,
#   ZERO differing, NaN-safe comparison (NaN == NaN counted equal), everything except
#   charts/SPS. v11 is this file's control for free.
#   THIS SUPERSEDES THE INHERITED CLAIM BELOW. jacteach_v1's own notes say `--jac-cos 1.0` is
#   bit-exact against `ppo_continuous_action_opsd_advcond_v6.py`, "so the champion's 8718 @8M
#   is this file's control for free". That sentence is TRUE OF jacteach_v1 AND FALSE HERE:
#   `DeepValueHead` replaces v6's zero-init Linear readout unconditionally, so `jac_cos 1.0`
#   reproduces v11, never v6. The inherited paragraph is left byte-unchanged deliberately --
#   editing inherited text would hide the composition rather than document it -- so read it
#   with this correction in hand. v6 is NOT a control for this file; v11 is.
#
# * MECHANISM ALIVE at `--jac-cos 0.95`, champion config (16 envs x 2048, mb128, a4/c4,
#   cond_scale ema_rms, adv_boost 1, seed 1, 8 iterations = 262,144 steps), against a
#   matched jacteach_v1 reference run on the same line:
#
#                          this file (v3)          jacteach_v1 ref
#     debug/jac_rot        0.95000 (0.95122 it2)   0.95000 all
#     debug/jac_gate       0 -> 0.97554 -> 1.0     0 -> 1.0
#     debug/jac_align      -0.264 .. +0.212        -0.307 .. +0.141
#     debug/jac_dose_scale 0.934 .. 1.000          0.940 .. 1.000
#     losses/distill_kl    0.0104 -> 0.0929        0.0151 -> 0.1024
#     losses/expl_variance 0.0 -> 0.8877           0.0 -> 0.8604
#
#   The one jac_rot value that is not 0.95000 is not drift: iteration 2's gate was 0.97554,
#   and the code sets cos_t = 1 - gate*(1 - jac_cos) = 1 - 0.97554*0.05 = 0.951223, which is
#   what was logged to 5 decimals. The gate ramp is the mechanism, exactly as specified.
#   Falsification 4 holds so far: align and dose_scale are indistinguishable from the
#   reference at this budget.
#   NON-FINITE SCALARS, honestly: five `debug/jac_*` tags are NaN at step 32768 and nowhere
#   else. That is by construction -- iteration 1's gate is 0 (untrained head, R2 ~ -0.0001),
#   the rotation block never runs, and those five keep their NaN initializers while the file
#   IS the parent for that iteration. The matched jacteach_v1 run has the identical five NaNs
#   at the identical step, so this is inherited, not introduced. Every scalar at every later
#   step is finite in both runs.
#
# * NOT A RESULT. Those 262k runs are 8 iterations of an 8M arm; `score_runs.py` last-20
#   reads 100.3 +-65.7 (v3) against 195.5 +-90.7 (v1 ref), which at this budget is noise
#   about a still-random policy and says NOTHING about either mechanism. They exist to prove
#   the machinery runs, not to rank anything. Falsification 3's EV separation (+0.027 here)
#   is likewise a 262k observation, and v11's own signature is "higher EV early, LOWER at
#   8M", so an early EV lead is precisely what must NOT be read as a win.
#
# * NO 8M NUMBER FOR THIS FILE EXISTS. The 8915 / 8882 / 8862 / 8859 / 8842 / 8812 figures
#   above are the PARENTS' and are quoted as the priors this file is built on, never as its
#   own result.
#
