# OPSD-JacTeach v2-stategate -- JacTeach v1 with a PER-STATE accuracy gate instead of a
# batch-mean one. Exactly one mechanism change against v1. Same single lever (`jac_cos`),
# same rotation, same dose match, NO new hyperparameter.
# =====================================================================================
# THE MECHANISM, IN ONE SENTENCE
#
# v1 gates the rotation dose on ONE number per iteration -- the batch-mean held-out
# one-step R2 (`debug/jac_gate`). v2 gates it ROW BY ROW on the same statistic decomposed
# per state, so the teacher turns the full `jac_cos` where the one-step model is locally
# trustworthy and does not turn at all where it is not.
#
# WHY. One-step model error is heteroscedastic across states, and a batch mean cannot see
# that. A single mean R2 spends the same angle at a state whose next observation the head
# predicts almost exactly as at a state where it is guessing. The family charter blesses
# gating dose on MEASURED one-step model accuracy; the measurement is per-transition, so
# spending it per-batch throws away everything except its average. MEASURED IN-RUN HERE
# (champion config, iteration 8, valid rows): the per-row normalized error has median
# 0.040 but p95 0.225, p99 0.451 and max 2.43 -- an 11x p99/median and a 61x max/median
# spread around a batch mean of 0.070. The heteroscedasticity the change is built on is
# real and large.
#
# HOW, EXACTLY (and why no new knob)
#
#   var_d   = sst_d / n_valid                       # per-dim valid-weighted delta variance
#   err_row = mean_d[ (ds_true - ds_pred)^2_d / var_d ]
#   gate_row = clamp( ((1 - err_row) - jac_r2_min) / (jac_r2_open - jac_r2_min), 0, 1 )
#
# Each dim's squared error is divided by THAT DIM's variance before averaging over dims --
# not sum-over-dims / sum-over-dims, which would be dominated by whichever obs dim has the
# largest delta variance and would not be the statistic the global gate is built from. With
# this normalization the identity
#
#   mean_over_valid_rows(err_row) == mean_d(sse_d / sst_d) == 1 - jac_r2
#
# is EXACT, not approximate (verified float64 offline, |diff| = 0.0; verified in-run in
# float32: iteration 2 err_mean 0.1942177 + jac_r2 0.8057823 = 1.0000000, iteration 8
# 0.0696753 + 0.9303247 = 1.0000000). So `1 - err_row` is a per-row R2 whose valid-row mean
# IS the number v1 already gates on: this is a DECOMPOSITION of the existing safety signal,
# not a second signal on a different scale. That is why `jac_r2_min`/`jac_r2_open` are
# reused verbatim per row and no new Arg exists -- "trustworthy enough to rotate" keeps one
# meaning at both scales. Same held-out quantities, same `b_valid` mask, no extra forward.
#
# The global gate is RETAINED as the outer guardrail: cos_t = 1 - jac_gate * gate_row *
# (1 - jac_cos), so a batch-mean R2 at or below `jac_r2_min` still makes this file exactly
# the parent everywhere, and `if jac_gate > 0` still skips the whole block. Because cos_t is
# now an (N,1) tensor, `sin_t` is `torch.sqrt((1 - cos_t^2).clamp_min(0))` rather than
# `math.sqrt`; cos_t lies in [jac_cos, 1] by construction so the radicand is non-negative
# before the clamp. The degenerate-row guard (`ok`) and the `torch.where` fallback to the
# unrotated displacement are unchanged.
#
# ============================ MY OWN MEASUREMENTS (v2) ===============================
# Everything in this block was run by me on this chassis. Nothing below it is mine.
#
# 1. BIT-EXACT CONTROL. `--jac-cos 1.0` vs `ppo_continuous_action_opsd_advcond_v6.py`,
#    HalfCheetah-v4 seed 1, every TensorBoard scalar except `charts/SPS`: 17 shared
#    scalars, ZERO differing, in BOTH configurations --
#      small  (4 envs x 256, mb4, 20480 steps)
#      champion (16 envs x 2048, mb128, a4/c4, 131072 steps)
#    So the champion's 8718 @8M remains this file's control for free.
#
# 2. MECHANISM ALIVE, `--jac-cos 0.95`, champion config, 8 iterations, seed 1:
#
#   it  jac_r2  jac_gate  state_mean  state_frac  jac_rot   jac_align  dose    distill_kl
#    1  -0.000    0.00      0.0346      0.026       --         --       --       0.01513
#    2   0.806    1.00      0.8750      0.907     0.95625    -0.3068   0.9450    0.73521
#    3   0.891    1.00      0.9354      0.940     0.95323    +0.1326   1.0000    0.11500
#    4   0.909    1.00      0.9678      0.976     0.95161    -0.0569   0.9834    0.15847
#    5   0.907    1.00      0.9655      0.974     0.95173    +0.1669   0.9989    0.12360
#    6   0.898    1.00      0.9574      0.967     0.95213    -0.0482   0.9881    0.13203
#    7   0.923    1.00      0.9778      0.983     0.95111    +0.0597   0.9949    0.13497
#    8   0.930    1.00      0.9762      0.981     0.95119    +0.0548   0.9943    0.09836
#
#    `debug/jac_rot` NO LONGER EQUALS `jac_cos` and must not: it is the valid-row mean of a
#    per-row cosine that is 0.95 where the gate is open and exactly 1 where it is shut. It
#    sits in (0.95, 0.957) at every iteration, i.e. strictly inside [jac_cos, 1] as
#    predicted. Paired v1 reference, identical config and seed: v1's jac_rot is
#    0.9499999 at EVERY iteration, and v1's distill_kl 0.1024 vs v2's 0.0984 @262144, so
#    the dose match survives the change.
#
# 3. IS THE PER-STATE GATE DEGENERATE? NO, BUT IT IS SKEWED WIDE OPEN, AND THAT IS THE
#    HONEST HEADLINE. Full valid-row distribution of `gate_row` (same run, instrumented
#    copy verified bit-identical to the run above):
#
#      it   frac(gate=0)  frac(0<gate<1)  frac(gate=1)   err_row p50 / p95 / p99 / max
#       1      0.903          0.095          0.002        0.929 / 1.828 / 2.344 / 4.098
#       2      0.027          0.337          0.636        0.163 / 0.422 / 0.673 / 3.577
#       4      0.009          0.086          0.905        0.060 / 0.263 / 0.482 / 1.958
#       6      0.013          0.103          0.884        0.068 / 0.295 / 0.562 / 2.966
#       8      0.008          0.055          0.937        0.040 / 0.225 / 0.451 / 2.429
#
#    So at steady state ~94% of states are fully open, ~5-6% are partially throttled and
#    ~1% are shut off completely, for a mean gate of 0.976. The mechanism is ALIVE (it is
#    neither ~all-zero nor ~all-one, and the shut rows are a real, persistent, ~1% tail
#    where the model's local error is 10-60x its median), but the dose it withholds is only
#    ~2.4% of nominal at this budget. READ THAT AS A SMALL EFFECT, NOT A BIG ONE. On
#    iteration 1 the gate is the opposite: 90% of rows shut, mean 0.035 -- but that
#    iteration is already the parent anyway because the global gate is 0, so it costs
#    nothing.
#
# 4. NaN SAFETY. Across all 28 logged scalars in the 0.95 run, the ONLY non-finite entries
#    are the five v1 already produces on iteration 1 (`jac_align`, `jac_rot`,
#    `jac_dose_scale`, `jac_dir_norm`, `jac_clamp_frac`), because the global gate is shut
#    and those five are only assigned inside the rotation branch. VERIFIED INHERITED, not
#    introduced: the paired v1 run at the same config logs exactly the same five NaNs at
#    exactly the same step. The two NEW scalars are finite at every iteration including the
#    first. Edge cases: all-invalid batch -> n_valid = 0 -> jac_r2 NaN -> jac_gate NaN ->
#    the branch is skipped and `gate_row` stays all-zero (it is zero-INITIALIZED, so it can
#    never be read as "open" from no data); single-valid-row -> n_valid = 1 < 2 -> same
#    path; a degenerate dim -> `sst` is clamped at 1e-12 so `var_d` is tiny but positive,
#    err_row goes large (or +inf), `1 - err_row` goes to -inf, and the clamp sends the row
#    to 0 -- the safe direction. The only way to get NaN into gate_row is a NaN in ds_pred,
#    which also NaNs jac_r2 and therefore shuts the outer gate first.
#
# FALSIFIABLE PREDICTIONS FOR THIS ARM
#  1. v2 >= v1 at matched `jac_cos` at 4M-8M. The mechanism only ever REMOVES rotation, so
#     if v1 > v2 the rows the model predicts worst are exactly the rows where rotating was
#     helping, and gating on model accuracy is the wrong charter instinct.
#  2. `debug/jac_gate_state_mean` stays in (0.9, 1.0) and `..._frac` in (0.9, 1.0) late in
#     training. If either reaches 1.000 the arm has become a no-op re-derivation of v1 and
#     the file should be deleted rather than benchmarked; if either collapses toward 0 the
#     head has stopped fitting on-policy data and the arm is silently the parent.
#  3. `debug/jac_rot` stays strictly inside (jac_cos, 1). Equal to jac_cos = gate open
#     everywhere = no-op; near 1 = gate shut everywhere = parent.
#  4. Because only ~2.4% of dose is withheld at this budget, the expected effect size is
#     SMALL and inside the seed-1 noise band (+-500 on this chassis). A large win at 1M
#     would be evidence of something other than this mechanism.
#
# WHAT I DID NOT MEASURE: any return at all. This file has been run for 8 iterations
# (262144 steps) only. Every return number below is v1's or v6's, not this file's.
#
# --------------- INHERITED JacTeach v1 NOTES BELOW, UNCHANGED --------------------------
# (v1's own header follows verbatim. Note that its "debug/jac_rot must equal jac_cos" and
# "debug/jac_gate pins at 1.0" tells describe the BATCH gate; in v2 jac_rot is bounded by
# jac_cos rather than equal to it, and the per-state gate is the number to watch.)
#
...
class Args:
    ...
    jac_cos: float = 0.85       # THE COSINE OF THE TEACHER ROTATION. 1.0 = the parent
                                ...
                                # Scale-free by construction. NOTE: in v2 debug/jac_rot no
                                # longer EQUALS this -- the rotation is per state, so
                                # jac_rot lands in [jac_cos, 1]. See jac_r2_min below.
    ...
    jac_r2_open: float = 0.8    # gate fully open at or above. Measured offline on real
                                ...
                                # v2 REUSES THIS SAME PAIR PER ROW -- deliberately no new
                                # knob. The row statistic is an exact decomposition of the
                                # batch one (its valid-row mean IS debug/jac_r2), so the
                                # two thresholds keep one meaning at both scales.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        jac_gate_state_mean = float("nan")
        jac_gate_state_frac = float("nan")
        if use_jac:
            ...
            with torch.no_grad():
                ...
                # gate_row: the PER-STATE dose gate, and the whole of v2. Zero-filled so a
                # batch with no usable transition leaves it shut rather than open.
                gate_row = torch.zeros_like(b_valid)
                if n_valid >= 2.0:
                    ...
                    # ===== PER-STATE ACCURACY, AS AN EXACT DECOMPOSITION OF jac_r2 ======
                    # NOT the suggested ratio-of-means `sum_d e^2 / sum_d (ds-mu)^2`. That
                    # normalizes a SUM over dims by a SUM over dims, so it is dominated by
                    # whichever obs dim has the largest delta variance and it is not the
                    # statistic the global gate is built from. Instead divide each dim's
                    # squared error by THAT DIM's valid-weighted variance first, exactly as
                    # `sse/sst` does, and only then average over dims:
                    #     var_d    = sst_d / n_valid
                    #     err_row  = mean_d[ e_{row,d}^2 / var_d ]
                    # Then, algebraically and not approximately,
                    #     mean_over_valid_rows(err_row) == mean_d(sse_d/sst_d) == 1 - jac_r2
                    # so `1 - err_row` is a per-row R2 whose valid-row mean IS the global R2
                    # this file already gates on. The change is therefore a decomposition of
                    # the existing safety signal, not a second, differently-scaled one.
                    # Same held-out quantities (ds_true, ds_pred, mu, sst), same `b_valid`
                    # mask, no extra forward pass.
                    var_d = sst / n_valid
                    err_row = (((ds_true - ds_pred) ** 2) / var_d).mean(-1)
                    # SAME RAMP as the global gate, per row -- so no new knob appears and
                    # "trustworthy enough to rotate" means the same thing at both scales.
                    gate_row = (
                        ((1.0 - err_row) - args.jac_r2_min)
                        / (args.jac_r2_open - args.jac_r2_min)
                    ).clamp(0.0, 1.0) * b_valid
                ...
                # gate_row stays all-zero for the same reason.
                if b_vsel.any():
                    jac_gate_state_mean = float(gate_row[b_vsel].mean().item())
                    jac_gate_state_frac = float((gate_row[b_vsel] > 0.5).float().mean().item())
            ...
            if jac_gate > 0.0:
                ...
                with torch.no_grad():
                    ...
                    # ===== v2: THE ROTATION IS NOW PER STATE =============================
                    # One-step model error is heteroscedastic across states, so a single
                    # batch-mean R2 spends the same angle at a state the model nails and at
                    # a state it is guessing at. `gate_row` is the same ramp applied to the
                    # same statistic, row by row, so cos_t becomes an (N,1) tensor: the
                    # teacher turns the full `jac_cos` where the model is locally
                    # trustworthy and not at all where it is not. `jac_gate` is retained as
                    # the OUTER guardrail -- global R2 <= jac_r2_min still means cos_t == 1
                    # everywhere, i.e. exactly the parent -- so the product of the two
                    # ramps keeps the parent's dose-attenuation semantics unchanged.
                    cos_req = min(max(args.jac_cos, -1.0), 1.0)
                    cos_t = 1.0 - (jac_gate * gate_row).unsqueeze(-1) * (1.0 - cos_req)
                    # cos_t in [cos_req, 1] by construction (the gate product is in [0,1]),
                    # so 1 - cos_t^2 >= 0 exactly; clamp_min is float32 hygiene only, and
                    # sqrt(0) is a value, not a NaN (no gradient flows here).
                    ...
                    # How far the teacher actually turned. IN v2 THIS NO LONGER EQUALS
                    # `jac_cos`: the rotation is per state, so this is the valid-row mean of
                    # a per-row cosine that is `jac_cos` where the gate is open and exactly
                    # 1 where it is shut. It must land in [jac_cos, 1], and where it lands
                    # inside that interval IS the mechanism's realized strength.
        ...
        if use_jac:
            ...
            # v2's two new numbers. Degenerate either way kills the arm: ~0 means the gate
            # is shut everywhere (mechanism off), ~1 everywhere means it agrees with the
            # global gate on every row and v2 is a no-op re-derivation of v1.
            writer.add_scalar("debug/jac_gate_state_mean", jac_gate_state_mean, global_step)
            writer.add_scalar("debug/jac_gate_state_frac", jac_gate_state_frac, global_step)
