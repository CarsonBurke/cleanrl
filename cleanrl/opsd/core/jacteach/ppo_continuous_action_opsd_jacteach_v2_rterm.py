# OPSD-JacTeach v2 -- v1 plus the term v1 left out. One lever (`--jac-reward`).
# =====================================================================================
# v1 IS THE BEST MODEL ON THIS DISK: 8953 last-20 on HalfCheetah-v4 seed 1 at `--jac-cos
# 0.95`, against the incumbent v6's 8718 and the previous best v11's 8842, leading at
# every matched step (@1M 4370, @2M 6701, @4M 8106, @6M 8835, @7M 9044).
#
# THE OMISSION. The charter's route-3 direction is
#
#       d = d/da [ r_hat(s,a) + gamma * V(f(s,a)) ]
#
# and v1 implemented only the second term, arguing that V already encodes discounted
# future reward. That argument holds for every reward component that is VISIBLE IN THE
# NEXT STATE and fails for every component that is not. HalfCheetah's reward is
#
#       r = forward_velocity - 0.1 * ||a||^2
#
# Forward velocity is obs[8], so its action-gradient survives through V(s'). The control
# cost does not appear in any next state, so its gradient -0.2*a is STRUCTURALLY INVISIBLE
# to grad_a V(f(s,a)) however good the transition model gets. Measured on 60k real steps:
# control cost is 6.1% of reward variance and correlates with -||a||^2 at 1.000000. It is
# perfectly learnable and v1 simply does not see it.
#
# MEASURED, CHAMPION CONFIG (16 envs x 2048, mb128, a4/c4, 8 iterations, seed 1):
#
#   metric                 v1 (gamma*V only)   v2 (+ r_hat)
#   debug/jac_rterm_r2                    --        0.95927   <- reward learnable held-out
#   debug/jac_rterm_frac                  --        0.26950   <- share of the direction
#   debug/jac_align                  0.07996        0.18356
#   debug/jac_rot                    0.95000        0.95000
#   debug/jac_dose_scale             0.99338        0.99275
#   losses/distill_kl                0.11897        0.11857   <- dose unchanged
#   debug/cond_gap                   0.03031        0.03004
#
# `jac_rterm_frac = 0.27` is the load-bearing number: the term v1 omitted carries 27% of
# the pathwise direction's magnitude, so this is not a rounding correction. Dose is
# untouched (distill_kl within 0.3%), so the arm is a pure DIRECTION change -- which is the
# only kind of change this lineage can attribute, since dose alone is known to move score
# (kappa 1/2/3/4 -> 5365/6077/5624/3128 @2M).
#
# jac_align rising 0.08 -> 0.18 is expected and worth stating: the reward gradient is part
# of what realized credit measures, so adding it moves the direction slightly TOWARD the
# credit direction. Still mostly orthogonal, so the route-3 information claim survives.
#
# CONTROL. `--no-jac-reward` is BIT-EXACT against
# `ppo_continuous_action_opsd_jacteach_v1.py` at the same `--jac-cos` -- 26 TensorBoard
# scalars, zero differing (NaN-safe comparison) -- so v1's 8953 is this file's control for
# free, and at `--jac-cos 1.0` the whole mechanism is skipped and the file is v6.
# The reward head's init draws are taken inside `fork_rng` so that holds.
#
# FALSIFIABLE PREDICTIONS
#  1. `debug/jac_rterm_r2` stays >0.9. The reward model is the easy object here; if it does
#     not, the head is mis-scaled rather than the idea being wrong.
#  2. `debug/jac_rterm_frac` stays materially above 0. If it decays toward 0 as the policy
#     sharpens, V(s') has absorbed the term and v2 collapses onto v1.
#  3. If v2 does NOT beat v1, the honest conclusion is that the control-cost direction is
#     already supplied by the credit channel (which sees the realized reward INCLUDING the
#     control cost), and the pathwise route buys only the parts credit cannot localize.
#     That would be a real result about WHICH information route 3 adds, not a null.
#
...
class Args:
    ...
    jac_reward: bool = True     # v2: include the charter's r_hat term in the pathwise
                                # derivative. --no-jac-reward is bit-identical to v1 at the
                                # same jac_cos, so v1's 8953 is this arm's control.
...
class RewardHead(nn.Module):
    """One-step action-conditioned reward model: r_hat(s, a).

    WHY THIS EXISTS. The charter's route-3 direction is grad_a [ r_hat + gamma*V(f(s,a)) ].
    v1 implemented only the second term, on the argument that V already encodes discounted
    future reward. That argument is wrong for exactly one component, and it is not a small
    one. HalfCheetah's reward is

        r = forward_velocity - 0.1 * ||a||^2

    The velocity term IS recoverable from the next observation (it is obs[8]), so grad_a of
    it survives through V(s'). The CONTROL COST is not: it depends on the action directly and
    never appears in any next state, so its gradient -0.2*a is structurally invisible to
    grad_a V(f(s,a)) no matter how good the transition model is. Measured on 60k real steps,
    control cost is 6.1% of reward variance and correlates with -||a||^2 at 1.000000 -- a
    perfectly learnable, always-present term that v1's direction simply omits.

    Same architecture and standardization discipline as TransitionHead, separate optimizer
    and separate grad clip, so `--no-jac-reward` reproduces v1 bit for bit.
    """
    ...
    def __init__(self, obs_dim, act_dim, hidden):
        ...
        self.r_mean: torch.Tensor
        self.r_var: torch.Tensor
        self.register_buffer("r_mean", torch.zeros(()))
        self.register_buffer("r_var", torch.ones(()))

    def forward(self, obs, action):
        """Standardized reward prediction (the space the regression loss lives in)."""
        return self.net(torch.cat([obs, action], dim=-1)).squeeze(-1)

    def reward(self, obs, action):
        """Prediction in RAW reward units, so it is commensurate with gamma*V(s')."""
        return self.forward(obs, action) * self.r_var.sqrt().clamp_min(1e-6) + self.r_mean

    @torch.no_grad()
    def update_stats(self, r, beta):
        self.r_mean.mul_(beta).add_((1.0 - beta) * r.mean())
        self.r_var.mul_(beta).add_((1.0 - beta) * r.var())
...
if __name__ == "__main__":
    ...
    rew_model = None
    rew_optimizer = None
    if use_jac and args.jac_reward:
        with torch.random.fork_rng(devices=[device] if args.cuda else []):
            rew_model = RewardHead(obs_shape[0], act_dim, args.jac_hidden).to(device)
        rew_optimizer = optim.Adam(rew_model.parameters(), lr=args.learning_rate, eps=1e-5)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        jac_rterm_r2 = float("nan")
        jac_rterm_frac = float("nan")
        if use_jac:
            ...
            if args.jac_reward:
                with torch.no_grad():
                    # HELD-OUT on the same terms as the transition R2: this head has only
                    # ever trained on earlier rollouts.
                    r_true = b_rewards
                    r_pred = rew_model.reward(b_obs, b_act)
                    if n_valid >= 2.0:
                        rmu = (r_true * b_valid).sum() / n_valid
                        rsse = (((r_true - r_pred) ** 2) * b_valid).sum()
                        rsst = (((r_true - rmu) ** 2) * b_valid).sum().clamp_min(1e-12)
                        jac_rterm_r2 = float((1.0 - rsse / rsst).item())
            ...
            if jac_gate > 0.0:
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    if args.jac_reward:
                        # Two separate grads rather than one, purely so the reward term's
                        # share of the direction is MEASURABLE (debug/jac_rterm_frac). The
                        # value head is the expensive part and is not recomputed.
                        (grad_v,) = torch.autograd.grad(
                            (args.gamma * v_next).sum(), z_var, retain_graph=True
                        )
                        (grad_r,) = torch.autograd.grad(
                            rew_model.reward(b_obs[sl], act).sum(), z_var
                        )
                        dirs.append((grad_v + grad_r).detach())
                        r_shares.append(grad_r.detach().norm(dim=-1))
                        v_shares.append(grad_v.detach().norm(dim=-1))
                    else:
                ...
                with torch.no_grad():
                    ...
                    if _delivered(u_new) > parent_kl:
                        ...
                        lo = hi = 1.0
                    if args.jac_reward and r_shares:
                        rs = torch.cat(r_shares)
                        vs = torch.cat(v_shares)
                        # THE DECISIVE READOUT FOR v2. The share of the pathwise direction's
                        # magnitude contributed by the term v1 omitted. ~0 means V(s') was
                        # already carrying the reward and this arm is a no-op; a material
                        # share means v1's direction was systematically incomplete.
                        jac_rterm_frac = float(
                            (rs[b_vsel] / (rs[b_vsel] + vs[b_vsel]).clamp_min(1e-12))
                            .mean()
                            .item()
                        )
        ...
        for epoch in range(max(args.actor_epochs, args.critic_epochs)):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if use_jac:
                    ...
                    if args.jac_reward:
                        r_target = (
                            b_rewards[mb] - rew_model.r_mean
                        ) / rew_model.r_var.sqrt().clamp_min(1e-6)
                        rmask = b_valid[mb]
                        rew_loss = (
                            ((rew_model(obs_mb, act_mb) - r_target) ** 2) * rmask
                        ).sum() / rmask.sum().clamp_min(1.0)
                        rew_optimizer.zero_grad(set_to_none=True)
                        (args.jac_coef * rew_loss).backward()
                        nn.utils.clip_grad_norm_(rew_model.parameters(), args.max_grad_norm)
                        rew_optimizer.step()
        ...
        if use_jac:
            ...
            with torch.no_grad():
                ...
                if int(keep.sum().item()) >= 2 and args.jac_reward:
                    rew_model.update_stats(
                        b_rewards[keep], 0.0 if iteration == 1 else 0.99
                    )
        ...
        if use_jac:
            ...
            writer.add_scalar("debug/jac_rterm_r2", jac_rterm_r2, global_step)
            writer.add_scalar("debug/jac_rterm_frac", jac_rterm_frac, global_step)
            writer.add_scalar(
