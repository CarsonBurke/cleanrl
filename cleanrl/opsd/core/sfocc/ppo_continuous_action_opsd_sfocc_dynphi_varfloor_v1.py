# OPSD-SFOcc DYNPHI VARFLOOR v1 -- preserve angular diversity while predicting phi's future.
# Parent: ppo_continuous_action_opsd_sfocc_dynphi_v1.py (sha256 3198960f4125d1d8).
# No PPO anywhere: no ratio, clipped surrogate, advantage, GAE, or policy gradient.
# =====================================================================================
# HYPOTHESIS AND SINGLE CHANGE
#
# dynphi v1 improved the occupancy channel (chan_action_r2 0.2078 -> 0.2940) and psi fit
# (psi_r2 0.2124 -> 0.3364), but phi_var_mean contracted from 0.3370 in the v3 control to
# 0.1256 and was still falling about 5% per iteration. When phi's non-affine LayerNorm
# keeps its post-normalization row mean-square near 1 (verified by new row-energy tags),
#
#   phi_var_mean = 1 - ||mean(phi)||^2 / phi_dim,
#
# up to LayerNorm's epsilon, so the measured contraction is angular concentration rather
# than harmless scale shrinkage. This arm adds exactly the
# VICReg variance hinge named by dynphi v1:
#
#   var_loss = mean_j relu(0.5 - sqrt(Var_batch(phi_j) + 1e-4)).
#
# The per-dimension std floor 0.5 is geometric rather than tuned. If every dimension meets
# it, phi_var_mean >= 0.25 directly; conditional on row mean-square remaining near 1, this
# retains about a 30-degree mean angular spread. That leaves slack below v3's measured
# 35 degrees while excluding dynphi's 21-degree endpoint. Coefficient 1.0 is one normalized
# value, matching the existing unit-weight cov and dyn terms without a grid; the new hinge
# itself is bounded by 0.5. It is inactive above the floor, so it cannot reward variance
# inflation or fight dynamics when diversity is healthy.
#
# DECISION READOUTS FOR THE 8M SEED-1 RUN, AT MATCHED CHECKPOINTS
# - sf/phi_floor_active_frac falling toward zero with phi_var_mean above 0.25: the floor is
#   currently inactive on the constrained trajectory. Only the matched untreated dynphi
#   control can say whether untreated contraction settles.
# - active_frac staying material or rising while phi_var_mean stays near 0.25: dynphi has
#   persistent collapse pressure and the floor is load-bearing.
# - phi_var_mean falling materially below 0.25 while active_frac is high: coefficient 1 is
#   insufficient; do not describe the representation as stabilized.
# In every branch, preserve dynphi's payoff only if chan_action_r2/channel_nats remain above
# the v3 incumbent, w_r2 stays above 0.9, and psi_r2 is read against the original 0.5 bar.
# Return is never inferred from the 65,536-step implementation lineage; it is compared only
# at matched long-run checkpoints. --no-var-floor is algorithmically the dynphi-v1 control:
# no module, parameter, RNG draw, optimizer entry, or extra tensor enters its loss. It still
# pays for the new telemetry and synchronizations, so SPS is not an exact parent comparison.
# =====================================================================================
...
# phi's non-affine LayerNorm fixes each ROW's variance ACROSS dims to approximately one
# (up to its epsilon) and bounds nothing about variance ACROSS SAMPLES. So this arm adds
# sf/phi_var_mean -- the mean per-dim BATCH
...
#
# WHAT THAT TAG IS. Non-affine LayerNorm centers every row exactly and makes its mean-square
# approximately one; epsilon leaves the exact value at pre-LN-var/(pre-LN-var + eps). Thus
#
#   phi_var_mean = E[row_mean_square] - ||E[phi]||^2 / phi_dim,
#
# and reduces to the earlier 1-minus-mean-direction identity only while row energy stays
# near one. v1 did not log row energy; varfloor v1 does. When it stays near one the decline
# is ANGULAR concentration, not scale. Both sides of the MSE remain O(1) per dim because
# the target is LayerNorm'd and the prediction regresses onto it.
#
# ------------------------------------------------------------------------------------
...
class Args:
    ...
    #                               non-affine LayerNorm keeps both sides of this MSE O(1)
    #                               per dim (up to its epsilon) -- the target IS a LayerNorm output and the
    ...
    # --- dynphi variance floor -------------------------------------------------------
    var_floor: bool = True        # THE VARFLOOR MECHANISM. --no-var-floor follows dynphi v1's
    #                               loss and optimizer path exactly.
    phi_std_floor: float = 0.5    # One geometric value, no sweep: all per-dim stds >= 0.5
    #                               directly imply phi_var_mean >= 0.25; when LayerNorm row
    #                               energy remains ~1, that is ~30 degrees of angular spread.
    var_floor_coef: float = 1.0   # Same normalized unit weight as sf_cov_coef and dyn_coef.
    #                               The linear hinge is bounded by phi_std_floor and is zero
    #                               when the diversity constraint is satisfied.
...
def phi_variance_floor(phi, std_floor):
    """VICReg's per-coordinate std hinge, adapted to LayerNorm-fixed feature energy.

    Population variance matches every other batch-statistic convention in this file.
    The epsilon is inside sqrt, as in VICReg, so the denominator remains finite near
    collapse. An exactly constant coordinate still has zero gradient because its centered
    deviations are zero; this term prevents ongoing contraction but cannot resurrect an
    exactly collapsed encoder by itself. The epsilon shifts the raw-std boundary by only
    0.02% at the default floor.
    """
    std = torch.sqrt(phi.var(dim=0, correction=0) + VAR_FLOOR_EPS)
    return F.relu(std_floor - std).mean()
...
def sf_losses(sf, obs, action, next_obs, a_bar_next, psi_target, reward, args):
    """SF regression, reward readout, decorrelation, dynamics, and angular variance floor.
    ...
    if args.dynphi:
        # THE TARGET SIDE IS DETACHED AND THAT IS THE LOAD-BEARING GRADIENT DIRECTION.
        # Without it, the target can move directly toward f in the same backward. Detaching
        # does NOT remove constant phi + constant f as a zero-error fixed point across
        # optimizer steps: the shared online encoder is recomputed each minibatch, and its
        # input-side gradient can still concentrate phi to make the next target easier.
        # That residual incentive is measured in dynphi v1 and is exactly what var_floor
        # opposes. no_grad rather than .detach() on the output: identical semantics, and the
        # target's activations are never stored.
        #
        ...
        dyn_mse = None
    if args.var_floor:
        var_floor = phi_variance_floor(phi_pred, args.phi_std_floor)
        total = total + args.var_floor_coef * var_floor
    else:
        var_floor = None
    return total, psi_mse, rew_mse, cov, dyn_mse, var_floor
...
if __name__ == "__main__":
    ...
    if args.var_floor:
        assert args.dynphi, "varfloor requires dynphi; use --no-dynphi --no-var-floor for v3"
        assert 0.0 < args.phi_std_floor <= 1.0, "phi_std_floor must be in (0, 1]"
        assert args.var_floor_coef > 0.0, "var_floor_coef must be positive"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # LayerNorm keeps each row's variance across dims near 1 when its pre-normalized
            # variance dominates epsilon, but bounds nothing about variance ACROSS SAMPLES:
            # phi(x) == const across samples is compatible with LayerNorm and reads 0 here.
            # Row-energy tags below verify the premise needed for the angular interpretation.
            phi_var = xc.square().mean(0)
            phi_var_mean = float(phi_var.mean().item())
            phi_std = phi_var.sqrt()
            phi_std_eff = torch.sqrt(phi_var + VAR_FLOOR_EPS)
            phi_std_min = float(phi_std.min().item())
            phi_std_mean = float(phi_std.mean().item())
            phi_floor_active_frac = float(
                (phi_std_eff < args.phi_std_floor).double().mean().item()
            )
            phi_var_floor = float(
                F.relu(args.phi_std_floor - phi_std_eff).mean().item()
            )
            phi_row_ms = phi_taken.double().square().mean(-1)
            phi_row_ms_mean = float(phi_row_ms.mean().item())
            phi_row_ms_min = float(phi_row_ms.min().item())
            phi_row_ms_p10 = float(torch.quantile(phi_row_ms, 0.1).item())
        ...
        sf_dyns, sf_var_floors, sf_grad_norms = [], [], []
        sf_grad_clipped = []
        ...
        for epoch in range(max(args.actor_epochs, args.critic_epochs)):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if do_sf:
                    ...
                    sf_total, sf_psi, sf_rew, sf_cov, sf_dyn, sf_var_floor = sf_losses(
                        sf,
                        obs_mb,
                        b_act[mb],
                        dyn_next,
                    ...
                    sf_total.backward()
                    sf_grad_norm = nn.utils.clip_grad_norm_(sf.parameters(), args.sf_grad_clip)
                    sf_optimizer.step()
                    with torch.no_grad():
                        sf_grad_norm_value = sf_grad_norm.item()
                        ...
                        if sf_var_floor is not None:
                            sf_var_floors.append(sf_var_floor.item())
                        sf_grad_norms.append(sf_grad_norm_value)
                        sf_grad_clipped.append(float(sf_grad_norm_value > args.sf_grad_clip))
        ...
        writer.add_scalar("losses/sf_grad_norm", float(np.mean(sf_grad_norms)), global_step)
        writer.add_scalar(
            "debug/sf_grad_clip_frac", float(np.mean(sf_grad_clipped)), global_step
        )
        if args.dynphi:
        ...
        if args.var_floor:
            writer.add_scalar(
                "losses/sf_var_floor", float(np.mean(sf_var_floors)), global_step
            )
        ...
        writer.add_scalar("sf/phi_std_min", phi_std_min, global_step)
        writer.add_scalar("sf/phi_std_mean", phi_std_mean, global_step)
        writer.add_scalar("sf/phi_floor_active_frac", phi_floor_active_frac, global_step)
        writer.add_scalar("sf/phi_var_floor", phi_var_floor, global_step)
        writer.add_scalar("sf/phi_row_ms_mean", phi_row_ms_mean, global_step)
        writer.add_scalar("sf/phi_row_ms_min", phi_row_ms_min, global_step)
        writer.add_scalar("sf/phi_row_ms_p10", phi_row_ms_p10, global_step)
        if args.dynphi:
