...
class SFCritic(nn.Module):
    ...
    def update_psi_stats(self, tgt, beta):
        """EMA of the target's per-dim mean and variance, same convention as
        update_chan_stats / the parent's jac_model.update_stats (beta=0 on the warmup
        iteration so iteration 1 is already on-level).
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # v2: refresh the standardization from the freshly built target, BEFORE the loss
        # standardizes it. Warmup beta=0 matches update_chan_stats and the parent's
        # jac_model.update_stats convention.
        sf.update_psi_stats(b_psi_target, 0.0 if iteration == 1 else args.cond_ema_beta)  # reuse: same ~100-iter horizon, no new knob
