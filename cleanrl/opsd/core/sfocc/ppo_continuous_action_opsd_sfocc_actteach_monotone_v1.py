# OPSD-SFOcc ActTeach Monotone v1 -- gain-safe fixed-concentration teacher.
# No PPO: the actor receives only hindsight clone and detached teacher distillation.
# =====================================================================================
# HYPOTHESIS
#
# The parent's exact-radius natural step can cross a curved successor-critic surface and
# produce a target whose finite predicted gain is nonpositive. Distilling such a target
# teaches the actor to move against the same updated critic that supplied its direction.
#
# ONE CHANGED MECHANISM
#
# `sf_teacher_kl` is a maximum radius. The parent's exact-radius float32 candidate is built
# first and is returned bit-for-bit whenever its predicted gain is positive. Only rows with
# nonpositive gain backtrack deterministically by halving their natural-step scale until the
# float32 target has strictly positive finite gain. If no positive representable step remains,
# that row returns the exact policy snapshot. Strict positivity is the sole acceptance rule:
# there is no gain threshold, coefficient, schedule, or new argument.
#
# UNCHANGED CONTROLS
#
# The policy and teacher remain on the fixed-concentration Beta manifold; the parent's exact
# natural direction, critic fitting and staging, hindsight clone NLL, detached teacher KL,
# coefficients, actor update, CUDA requirement, environment, horizon, and seed are unchanged.
# Existing comparable telemetry is retained; monotone-dose diagnostics are additive. The
# update order remains critic first, teacher second, actor last.
# =====================================================================================
import os
import random
import time
...
class Args:
    ...
    sf_teacher_kl: float = 0.10   # maximum total KL radius per state. One value, no sweep.
...
def sf_action_teacher(sf, agent, obs, target_kl, chunk):
    """Build a gain-safe fixed-concentration Beta teacher within a forward-KL radius.

    The parent's exact Fisher-natural direction and exact-radius candidate are constructed
    unchanged. A row whose float32 candidate does not improve the same frozen critic is
    backtracked by exact scale halvings until it strictly improves or the target can no
    longer differ representably from the snapshot.
    """
    ...
    full_dose_fracs, backtracked_fracs, zero_dose_backtrack_fracs = [], [], []
    kl_utilizations, backtrack_counts = [], []
    bracket_expansions = []
    ...
    for start in range(0, obs.shape[0], chunk):
        ...
        with torch.no_grad():
            ...
            # This is the parent's projection geometry verbatim. In particular, positive
            # full-dose rows retain the exact same float64 solve and float32 target.
            ...
            if not bool(full_teacher_ok.all()):
            ...
            if not bool(torch.isfinite(predicted_gain).all()):
                raise FloatingPointError(
                    "non-finite fixed-concentration full-dose predicted gain"
                )
            full_dose_predicted_gain = predicted_gain

            full_dose = active_rows & (predicted_gain > 0.0)
            backtracked = active_rows & ~full_dose
            searching = backtracked.clone()
            backtrack_count = torch.zeros_like(predicted_gain)
            zero_dose_after_backtrack = torch.zeros_like(searching)

            # No iteration cap or gain epsilon is needed: exact halving eventually leaves
            # both the float64 logit and float32 Beta target unchanged, which is the
            # deterministic exhaustion condition.
            while bool(searching.any()):
                search_rows = searching.unsqueeze(-1)
                scale = torch.where(search_rows, 0.5 * scale, scale)
                backtrack_count.add_(searching.to(backtrack_count.dtype))

                candidate_logit = mean_logit64 + scale * direction_logit
                logit_moved = (candidate_logit != mean_logit64).any(-1)
                _, candidate_alpha64, candidate_beta64 = at_scale(scale)
                candidate_alpha = candidate_alpha64.to(s_alpha.dtype)
                candidate_beta = candidate_beta64.to(s_beta.dtype)
                candidate_ok = (
                    torch.isfinite(candidate_alpha).all(-1)
                    & torch.isfinite(candidate_beta).all(-1)
                    & (candidate_alpha > 0.0).all(-1)
                    & (candidate_beta > 0.0).all(-1)
                )
                if not bool((~searching | candidate_ok).all()):
                    raise FloatingPointError(
                        "non-finite fixed-concentration backtrack target"
                    )

                target_moved = (
                    (candidate_alpha != s_alpha) | (candidate_beta != s_beta)
                ).any(-1)
                representable = searching & logit_moved & target_moved
                candidate_gain = torch.full_like(predicted_gain, float("nan"))
                if bool(representable.any()):
                    candidate_alpha_rows = candidate_alpha[representable]
                    candidate_beta_rows = candidate_beta[representable]
                    candidate_z = candidate_alpha_rows / (
                        candidate_alpha_rows + candidate_beta_rows
                    )
                    candidate_gain[representable] = sf.value(
                        obs_mb[representable], agent.z_to_action(candidate_z)
                    ) - predicted_value.detach()[representable]
                accept = (
                    representable
                    & torch.isfinite(candidate_gain)
                    & (candidate_gain > 0.0)
                )
                exhausted = searching & ~representable

                t_alpha = torch.where(accept.unsqueeze(-1), candidate_alpha, t_alpha)
                t_beta = torch.where(accept.unsqueeze(-1), candidate_beta, t_beta)
                predicted_gain = torch.where(accept, candidate_gain, predicted_gain)
                t_alpha = torch.where(exhausted.unsqueeze(-1), s_alpha, t_alpha)
                t_beta = torch.where(exhausted.unsqueeze(-1), s_beta, t_beta)
                predicted_gain = torch.where(
                    exhausted, torch.zeros_like(predicted_gain), predicted_gain
                )
                scale = torch.where(
                    exhausted.unsqueeze(-1), torch.zeros_like(scale), scale
                )
                zero_dose_after_backtrack |= exhausted
                searching &= ~(accept | exhausted)
            ...
            teacher_ok = (
                ...
                & torch.isfinite(predicted_gain)
                & (predicted_gain >= 0.0)
            )
            if not bool(teacher_ok.all()):
                raise FloatingPointError(
                    "invalid gain-safe fixed-concentration Beta teacher"
                )

            # Full-dose rows retain the parent's exact-radius check. Backtracked rows use
            # the new maximum-radius contract instead of being incorrectly required to
            # spend the full dose.
            full_dose_error = full_dose & (
                (delivered_kl - target_kl).abs() > 0.01 * target_kl
            )
            backtracked_dose_error = backtracked & (
                (delivered_kl < 0.0) | (delivered_kl > target_kl)
            )
            if bool((full_dose_error | backtracked_dose_error).any()):
                raise RuntimeError(
                    "fixed-concentration teacher violated its KL radius"
                )
            ...
            kl_shortfall_fracs.append(
                (active_rows & (delivered_kl < 0.99 * target_kl)).to(z0.dtype)
            )
            nonpositive_gain_fracs.append(
                (active_rows & (full_dose_predicted_gain <= 0.0)).to(z0.dtype)
            )
            ...
            full_dose_fracs.append(full_dose.to(z0.dtype))
            backtracked_fracs.append(backtracked.to(z0.dtype))
            zero_dose_backtrack_fracs.append(
                zero_dose_after_backtrack.to(z0.dtype)
            )
            kl_utilizations.append(delivered_kl / target_kl)
    ...
    metrics = {
        ...
        "full_dose_frac": cat_mean(full_dose_fracs),
        "backtracked_frac": cat_mean(backtracked_fracs),
        "zero_dose_after_backtrack_frac": cat_mean(zero_dose_backtrack_fracs),
        "kl_utilization": cat_mean(kl_utilizations),
        "backtrack_count": cat_mean(backtrack_counts),
        "backtrack_count_max": cat_max(backtrack_counts),
...
if __name__ == "__main__":
    ...
    assert (
        np.isfinite(args.sf_teacher_kl) and args.sf_teacher_kl > 0.0
    ), "teacher KL radius must be finite and positive"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # The teacher builder differentiates only a detached snapshot mean-logit leaf.
        # Clearing the last supervised gradients makes that isolation inspectable and
        # prevents stale `.grad` tensors from being mistaken for a teacher path.
        sf_optimizer.zero_grad(set_to_none=True)
        # These scores describe the exact UPDATED critic snapshot used below, not the
        # pre-update critic that built the rollout channel.
        with torch.no_grad():
        ...
        # Frozen once per iteration. Its mean follows the parent's exact fixed-concentration
        # natural direction under the UPDATED successor critic. The exact-radius candidate
        # is retained when improving; only nonpositive-gain rows halve within that KL radius.
        ...
        writer.add_scalar(
            "sf/teacher_full_dose_frac", teacher_metrics["full_dose_frac"], global_step
        )
        writer.add_scalar(
            "sf/teacher_backtracked_frac", teacher_metrics["backtracked_frac"], global_step
        )
        writer.add_scalar(
            "sf/teacher_zero_dose_after_backtrack_frac",
            teacher_metrics["zero_dose_after_backtrack_frac"],
            global_step,
        )
        writer.add_scalar(
            "sf/teacher_kl_utilization", teacher_metrics["kl_utilization"], global_step
        )
        writer.add_scalar(
            "sf/teacher_backtrack_count", teacher_metrics["backtrack_count"], global_step
        )
        writer.add_scalar(
            "sf/teacher_backtrack_count_max",
            teacher_metrics["backtrack_count_max"],
            global_step,
        )
        writer.add_scalar(
