# OPSD-SFOcc ActTeach FixedConc v3 -- boundary-capable mean-only Beta projection.
# No PPO: the actor receives only hindsight clone and detached teacher distillation.
# =====================================================================================
# V2 RESULT AND FAILURE
#
# FullBeta v2 fixed the teacher-dose collapse and was materially better than v1:
#
#   metric                              v2
#   peak 20-episode return              2939.6 at 2.992M
#   post-peak 20-episode trough          929.0 at 3.360M
#   final 20-episode return             1891.3 at 5.360M
#
# It nevertheless exposed a second geometric error. The critic objective depends only on
# the action mean, but the unconstrained full-Beta natural step increased concentration on
# every update (100% of updates, median teacher/snapshot ratio 1.089). Distillation therefore
# ratcheted the policy toward a near-delta distribution: total entropy fell from -3.54 at
# 0.5M to -14.68 at 2M and -17.63 at 5.34M. Once sharp, the actor stopped reliably following
# the teacher: over the second half, median one-update KL reduction was 0.001 and 48.8% of
# updates made KL worse. Return simultaneously oscillated by roughly 2000 points.
#
# High concentration also made float32 closed-form KL cancellation increasingly severe.
# Maximum per-state dose error first exceeded 1e-4 at 1.47M, 5e-4 at 2.59M, and 8e-4 at
# 3.51M; it eventually crossed the explicit 1% tolerance and terminated v2 at 5.34M.
#
# ONE CORRECTION
#
# Restrict both policy and teacher to the fixed-concentration Beta manifold
#
#   alpha = k * mu,  beta = k * (1 - mu),  k = 2 * (1 + log(2)) = 3.386...
#
# matching the original zero-logit policy's initial concentration. The actor exposes the
# raw mean logit and constructs paired parameters as `k*exp(logsigmoid(u))` and
# `k*exp(logsigmoid(-u))`. Either parameter may fall below 1 near an action boundary without
# the premature float32 sigmoid saturation that would recreate v1's infeasible interval.
# Fixed concentration removes the radial ratchet; it is not an entropy or variance floor,
# because either still falls as the mean itself approaches a boundary.
#
# The teacher differentiates w.psi(s, action(mu)) with respect to the mean logit u. On the
# fixed-k manifold its exact scalar Fisher metric per action dimension is
#
#   I_u = k^2 [trigamma(alpha) + trigamma(beta)] (d mu / d u)^2.
#
# The per-state natural direction is normalized to unit Fisher norm. A dynamically bracketed
# float64 bisection then moves the same raw u to forward
# KL(teacher || float32 snapshot)=0.10 before the detached target reaches the actor. The
# cast target is rechecked, and nonfinite logits or unrepresentable Beta parameters fail
# explicitly. Teacher concentration is structurally the snapshot concentration up to float
# rounding, so there is no radial concentration pressure.
#
# The actor still sees two supervised targets only:
#   1. hindsight rationalization NLL at the observed occupancy residual;
#   2. forward KL from the detached SF action teacher into the zero-context student.
# No value, advantage, ratio, policy-gradient term, fidelity gate, warm-up, entropy bonus,
# dose schedule, or threshold-controlled teacher enters the actor.
#
# UPDATE ORDER
#
# The occupancy channel is built before updating SF statistics. The SF optimizer fits fixed
# detached targets, the fixed-concentration teacher is frozen from that updated critic, and
# the actor runs last. This version passed static and focused mathematical verification but
# has no benchmark; it is not queued because the allowed corrective compute run was v2.
# =====================================================================================
import os
import random
import time
