# OPSD-SFOcc ActTeach CensoredGauss v1 -- bounded-mean, fixed-scale Gaussian ablation.
# =====================================================================================
# HYPOTHESIS
#
# The fixed-concentration Beta manifold couples action-space location to boundary-dependent
# variance and Fisher curvature. A fixed-scale Gaussian should isolate whether that singular
# geometry, rather than the SF teacher or hindsight channel, limits teacher following.
#
# ONE CHANGED MECHANISM
#
# Both student and privileged policies now generate an uncensored latent action
#
#   y ~ Normal(mean, sigma),  mean = action_mid + action_half_range * tanh(raw_head),
#   sigma = action_half_range / sqrt(BETA_CONCENTRATION + 1).
#
# The fixed sigma matches v3's zero-logit Beta action variance. Acting sends y clamped to the
# environment bounds while rollout storage retains y itself, so hindsight clone NLL is the
# exact generating Normal likelihood. The external teacher differentiates the updated SF
# value with respect to the bounded mean coordinate, takes the fixed-sigma Gaussian natural
# direction, and projects along its feasible bounded ray to at most 0.10 total KL. Boundary
# feasibility may therefore produce a measured KL shortfall; zero gradients preserve the
# exact snapshot. Distillation is exact same-sigma Gaussian forward KL.
#
# UNCHANGED CONTROLS
#
# The v3 defaults, SF critic and occupancy channel, fixed 0.10 teacher target, detached
# hindsight clone plus teacher losses, critic-first/teacher/actor update order, telemetry,
# CUDA requirement, HalfCheetah-v4, 8M steps, and seed 1 are unchanged. There is no PPO,
# value/advantage/ratio actor term, backtracking, staging, isolation, coefficient schedule,
# entropy bonus, fidelity gate, warm-up, or training result in this file.
# =====================================================================================
import os
import random
import time
...
class Args:
    ...
    #                               Scale is structural, so repeated fitting cannot recreate
    #                               v2's learned-concentration ratchet.
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        action_low = torch.as_tensor(
            envs.single_action_space.low, dtype=torch.float32
        ).reshape(-1)
        action_high = torch.as_tensor(
            envs.single_action_space.high, dtype=torch.float32
        ).reshape(-1)
        action_half_range = 0.5 * (action_high - action_low)
        if not bool(
            (
                torch.isfinite(action_low)
                & torch.isfinite(action_high)
                & (action_half_range > 0.0)
            ).all()
        ):
            raise ValueError("censored Gaussian requires finite, non-degenerate action bounds")
        self.action_low: torch.Tensor
        self.action_high: torch.Tensor
        self.action_mid: torch.Tensor
        self.action_half_range: torch.Tensor
        self.action_sigma: torch.Tensor
        self.register_buffer("action_low", action_low)
        self.register_buffer("action_high", action_high)
        self.register_buffer("action_mid", 0.5 * (action_low + action_high))
        self.register_buffer("action_half_range", action_half_range)
        self.register_buffer(
            "action_sigma",
            action_half_range / float(np.sqrt(BETA_CONCENTRATION + 1.0)),
        )
    ...
    def policy_raw_mean(self, obs, cond):
        return self.actor_mean_head(self._feat(obs, cond))

    def mean_from_raw(self, raw_mean):
        return self.action_mid + self.action_half_range * torch.tanh(raw_mean)

    def policy(self, obs, cond):
        """Return the bounded latent-Gaussian location and shared fixed scale."""
        mean = self.mean_from_raw(self.policy_raw_mean(obs, cond))
        return mean, self.action_sigma.expand_as(mean)

    def act(self, obs):
        """Sample latent y in action space; only the environment action is censored."""
        mean, sigma = self.policy(obs, self._zero_cond(obs))
        y = Normal(mean, sigma, validate_args=False).sample()
        return torch.maximum(torch.minimum(y, self.action_high), self.action_low), y

    def mean_action(self, obs):
        """The deterministic bounded Gaussian location used by psi and the channel."""
        mean, _ = self.policy(obs, self._zero_cond(obs))
        return mean
...
# =====================================================================================
def same_scale_gaussian_kl_per_dim(mean1, mean2, sigma):
    """Forward KL per dimension for Normals sharing the same fixed scale."""
    return 0.5 * ((mean1 - mean2) / sigma).square()


def sf_action_teacher(sf, agent, obs, target_kl, chunk):
    """Build a detached fixed-scale Gaussian teacher on the feasible bounded-mean ray.

    The bounded snapshot mean is the differentiated leaf. For fixed diagonal sigma, the
    mean-coordinate Fisher metric is diag(1 / sigma**2), so the normalized natural direction
    has unit Fisher norm and an unconstrained step sqrt(2 * target_kl) reaches the requested
    KL exactly. The action box can shorten that ray but can never be crossed.
    """
    teacher_means, student_means = [], []
    delivered_kls, predicted_gains = [], []
    mean_gaps, metric_grad_norms, step_scales, boundary_fracs = [], [], [], []
    boundary_limited_fracs, kl_shortfall_fracs = [], []
    nonpositive_gain_fracs, zero_grad_fracs, kl_shortfalls = [], [], []

    for start in range(0, obs.shape[0], chunk):
        obs_mb = obs[start : start + chunk]
        with torch.no_grad():
            s_mean, s_sigma = agent.policy(obs_mb, agent._zero_cond(obs_mb))
            snapshot_ok = (
                torch.isfinite(s_mean)
                & torch.isfinite(s_sigma)
                & (s_sigma > 0.0)
                & (s_mean >= agent.action_low)
                & (s_mean <= agent.action_high)
            )
            if not bool(snapshot_ok.all()):
                raise FloatingPointError("invalid bounded fixed-scale Gaussian snapshot")

        mean = s_mean.detach().requires_grad_(True)
        predicted_value = sf.value(obs_mb, mean)
        (grad_mean,) = torch.autograd.grad(
            predicted_value.sum(), (mean,), create_graph=False
        )

        with torch.no_grad():
            gradient_ok = torch.isfinite(predicted_value) & torch.isfinite(grad_mean).all(-1)
            if not bool(gradient_ok.all()):
                raise FloatingPointError("non-finite fixed-scale Gaussian teacher gradient")

            grad_scale = grad_mean.abs().amax(-1, keepdim=True)
            zero_grad = grad_scale == 0.0
            safe_grad_scale = torch.where(
                zero_grad, torch.ones_like(grad_scale), grad_scale
            )

            # Float64 geometry keeps the analytic Gaussian dose and the box intersection
            # accurate before the detached target is cast back to the actor's dtype.
            scaled_grad = (grad_mean / safe_grad_scale).double()
            sigma64 = s_sigma.double()
            natural_mean = scaled_grad * sigma64.square()
            scaled_metric_grad_sq = (scaled_grad * natural_mean).sum(-1, keepdim=True)
            metric_ok = torch.isfinite(scaled_metric_grad_sq) & (
                zero_grad | (scaled_metric_grad_sq > 0.0)
            )
            if not bool(metric_ok.all()):
                raise FloatingPointError("invalid fixed-scale Gaussian natural-gradient norm")
            safe_norm = torch.where(
                zero_grad, torch.ones_like(scaled_metric_grad_sq), scaled_metric_grad_sq
            ).sqrt()
            direction_mean = natural_mean / safe_norm
            metric_grad_norm = torch.where(
                zero_grad,
                torch.zeros_like(safe_norm),
                grad_scale.double() * safe_norm,
            )
            if not bool(torch.isfinite(direction_mean).all()):
                raise FloatingPointError("non-finite fixed-scale Gaussian natural direction")

            mean64 = s_mean.double()
            low64 = agent.action_low.double()
            high64 = agent.action_high.double()
            ray_limit = torch.full_like(direction_mean, torch.inf)
            ray_limit = torch.where(
                direction_mean > 0.0,
                (high64 - mean64) / direction_mean,
                ray_limit,
            )
            ray_limit = torch.where(
                direction_mean < 0.0,
                (low64 - mean64) / direction_mean,
                ray_limit,
            )
            feasible_scale = ray_limit.amin(-1, keepdim=True).clamp_min(0.0)
            requested_scale = mean64.new_full(
                (mean64.shape[0], 1), np.sqrt(2.0 * target_kl)
            )
            boundary_limited = (~zero_grad) & (feasible_scale < requested_scale)
            scale = torch.minimum(feasible_scale, requested_scale)
            scale = torch.where(zero_grad, torch.zeros_like(scale), scale)

            t_mean64 = mean64 + scale * direction_mean
            t_mean_float = t_mean64.to(s_mean.dtype)
            t_mean = torch.maximum(
                torch.minimum(t_mean_float, agent.action_high), agent.action_low
            )
            # Exact snapshot preservation is stronger than merely producing zero KL.
            t_mean = torch.where(zero_grad, s_mean, t_mean)
            delivered_kl = same_scale_gaussian_kl_per_dim(
                t_mean.double(), s_mean.double(), sigma64
            ).sum(-1)
            tolerance = max(1e-7, 1e-6 * target_kl)
            teacher_ok = (
                torch.isfinite(t_mean).all(-1)
                & torch.isfinite(delivered_kl)
                & (t_mean >= agent.action_low).all(-1)
                & (t_mean <= agent.action_high).all(-1)
                & (delivered_kl <= target_kl + tolerance)
            )
            if not bool(teacher_ok.all()):
                raise FloatingPointError(
                    "invalid or over-budget bounded fixed-scale Gaussian teacher"
                )

            teacher_value = sf.value(obs_mb, t_mean)
            action_gap = (t_mean - s_mean).abs().mean(-1)
            boundary = (
                (t_mean == agent.action_low) | (t_mean == agent.action_high)
            ).to(s_mean.dtype).mean(-1)
            ...
            if not bool(torch.isfinite(predicted_gain).all()):
                raise FloatingPointError("non-finite fixed-scale Gaussian predicted gain")
            ...
            boundary_limited_fracs.append(
                boundary_limited.squeeze(-1).to(s_mean.dtype)
            )
            kl_shortfall_fracs.append(
                ((~zero_grad).squeeze(-1) & (delivered_kl < 0.99 * target_kl)).to(
                    s_mean.dtype
                )
            )
            nonpositive_gain_fracs.append(
                ((~zero_grad).squeeze(-1) & (predicted_gain <= 0.0)).to(s_mean.dtype)
            )
            zero_grad_fracs.append(zero_grad.squeeze(-1).to(s_mean.dtype))
            kl_shortfalls.append((target_kl - delivered_kl).clamp_min(0.0))
    ...
    metrics = {
        ...
        "boundary_limited_frac": cat_mean(boundary_limited_fracs),
        "kl_shortfall_frac": cat_mean(kl_shortfall_fracs),
        "kl_shortfall": cat_mean(kl_shortfalls),
        "nonpositive_gain_frac": cat_mean(nonpositive_gain_fracs),
        "zero_grad_frac": cat_mean(zero_grad_fracs),
    }
    return (
        torch.cat(teacher_means).detach(),
        torch.cat(student_means).detach(),
        metrics,
    )


@torch.no_grad()
def teacher_follow_metrics(agent, obs, t_mean, s_mean_before, chunk):
    """Measure whether one actor update moved the student toward the frozen teacher."""
    post_means = []
    for start in range(0, obs.shape[0], chunk):
        obs_mb = obs[start : start + chunk]
        mean, _ = agent.policy(obs_mb, agent._zero_cond(obs_mb))
        post_means.append(mean)
    s_mean_after = torch.cat(post_means)
    post_policy_ok = (
        torch.isfinite(s_mean_after)
        & (s_mean_after >= agent.action_low)
        & (s_mean_after <= agent.action_high)
    )
    if not bool(post_policy_ok.all()):
        raise FloatingPointError("invalid post-update bounded Gaussian policy mean")

    target_step = t_mean - s_mean_before
    actual_step = s_mean_after - s_mean_before
    ...
    sigma64 = agent.action_sigma.double()
    pre_kl = same_scale_gaussian_kl_per_dim(
        t_mean.double(), s_mean_before.double(), sigma64
    ).sum(-1)
    post_kl = same_scale_gaussian_kl_per_dim(
        t_mean.double(), s_mean_after.double(), sigma64
    ).sum(-1)
    pre_gap = target_step.abs().mean(-1)
    post_gap = (t_mean - s_mean_after).abs().mean(-1)
...
if __name__ == "__main__":
    ...
    latent_ys = torch.zeros((args.num_steps, args.num_envs, act_dim), device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        b_y = latent_ys.reshape(-1, act_dim)
        b_rewards = rewards.reshape(-1)
        with torch.no_grad():
            b_act = torch.maximum(
                torch.minimum(b_y, agent.action_high), agent.action_low
            )
            action_clamp_frac = (b_y != b_act).float().mean().item()
        ...
        with torch.no_grad():
            ...
            # Recoverability of the CENSORED environment action from state plus channel.
            # This remains a leakage diagnostic for the simulator-facing behavior, but it
            # does not measure the uncensored latent y used by clone NLL.
            cond_action_r2 = _ls_r2(torch.cat([b_chan, b_obs], dim=-1), b_act)
            # Channel-only recoverability of that same environment action.
            chan_action_r2 = _ls_r2(b_chan, b_act)
            ...
            enc_action_r2 = _ls_r2(agent.cond_present(b_chan), b_act)
            # State-only control for the two channel probes. As the bounded mean becomes
            # predictable from state, every state-correlated feature can inherit action R2;
            # only gain beyond this control is attributable to the channel.
            state_action_r2 = _ls_r2(b_obs, b_act)
            # These probes target the action actually sent to the environment. Unlike the
            # parent's latent z, censored y is not affinely equivalent after clamping.
        ...
        # The teacher builder differentiates only a detached bounded snapshot-mean leaf.
        ...
        # Frozen once per iteration. Its mean follows the fixed-scale Gaussian natural
        # direction under the UPDATED successor critic, truncated only by the action box.
        ...
        for _ in range(args.actor_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mean, sigma = agent.policy(
                    torch.cat([obs_mb, obs_mb], 0),
                    torch.cat([agent.cond_present(chan_mb), cond_absent], 0),
                )
                mean_cl, sigma_cl = mean[:n], sigma[:n]
                mean_stu, sigma_stu = mean[n:], sigma[n:]

                # y_mb is the uncensored sample generated by this exact latent Normal.
                clone_loss = -Normal(
                    mean_cl, sigma_cl, validate_args=False
                ).log_prob(y_mb).sum(-1).mean()
                kl_dims = same_scale_gaussian_kl_per_dim(
                    t_mean, mean_stu, sigma_stu
                )
                ...
                with torch.no_grad():
                    ...
                    stu_nlls.append(
                        -Normal(
                            mean_stu, sigma_stu, validate_args=False
                        ).log_prob(y_mb).sum(-1).mean().item()
                    )
                    ...
                    tea_ents.append(
                        Normal(
                            t_mean,
                            agent.action_sigma.expand_as(t_mean),
                            validate_args=False,
                        ).entropy().sum(-1).mean().item()
                    )
        optimizer.zero_grad(set_to_none=True)
        ...
        # Comparable entropy tags now refer to the uncensored latent Gaussian.
        ...
        writer.add_scalar("debug/action_clamp_frac", action_clamp_frac, global_step)
        ...
        # Snapshot KL is the achieved teacher dose; mean gap is its environment-action gap.
        ...
        writer.add_scalar(
            "sf/teacher_boundary_limited_frac",
            teacher_metrics["boundary_limited_frac"],
            global_step,
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "sf/teacher_kl_shortfall", teacher_metrics["kl_shortfall"], global_step
        )
        writer.add_scalar(
