# OPSD-SFOcc ActTeach v1 -- successor features construct the teacher in ACTION space.
# No PPO: the actor still receives only hindsight clone and detached teacher distillation.
# =====================================================================================
# PROBLEM
#
# sfocc_v3 learned its supervised machinery but did not improve return:
#   return @8M        -366
#   psi_r2             0.875
#   w_r2               0.971
#   cond_gap            0.0009
#
# DynPhi made psi and phi more predictable without fixing return. The diagnosis is
# structural: p(a_taken | s, realized_occupancy_residual) is an inverse hindsight model.
# Feeding it a synthetic residual delta*w assumes, but never trains, the implication
# "this action increases w.psi". The teacher therefore became the student while the clone
# term concentrated the policy around bad behavior.
#
# ONE CHANGE
#
# After fitting the SF critic on the current rollout, construct a detached teacher directly:
#
#   z0        = student Beta mean in [0,1]
#   J(z)      = w . psi(s, action(z))
#   g         = dJ/dz at z0
#   u         = normalize(F^-1/2 g)                  exact local Beta-Fisher coordinates
#   z_teacher = z0 + scale * F^-1/2 u
#
# A vectorized bisection chooses `scale` per state so
# KL(Beta_teacher || Beta_student_snapshot) = sf_teacher_kl = 0.10 nats. Teacher and
# snapshot student use the same concentration; only the mean moves. The actor never sees
# the critic graph: alpha_teacher and beta_teacher are detached before the actor update.
# This is teacher-side local policy improvement followed by ordinary OPSD distillation,
# not a policy-gradient actor loss.
#
# NO GATE
#
# Critic fidelity is telemetry, never control flow. A fidelity gate would remove the only
# optimization pressure early, create a hidden curriculum, and conflate a failed direction
# with a gate that never opened. The fixed KL trust region bounds a bad early direction
# without switching it off. Exact-zero gradients naturally deliver zero KL; there is no
# threshold, schedule, warm-up, or confidence coefficient.
#
# UPDATE ORDER
#
# The SF optimizer runs first on the current rollout's fixed detached targets. The teacher
# is then built from that updated critic, and the actor runs last against one frozen teacher
# snapshot. This avoids paying a fixed KL dose to the critic's random initialization on
# iteration 1. The occupancy channel itself is still built before the SF update, preserving
# v3's no-statistics-mismatch invariant and its future-only anti-leakage construction.
#
# FIXED-DOSE GEOMETRY
#
# The teacher preserves alpha+beta from the student snapshot and keeps alpha,beta >= 1.
# At fixed concentration k, the exact Fisher diagonal for the Beta mean is
# F = k^2 * (trigamma(alpha) + trigamma(beta)). Maximizing g.dz at fixed local Fisher
# distance gives dz proportional to F^-1 g. Exact Beta KL, not the local quadratic,
# sets the final dose. Bisection has 16 fixed steps and is not a knob.
#
# PRE-REGISTERED DECISION
#
# This is the single compute iteration. HalfCheetah-v4, seed 1, 8M steps. No sibling,
# coefficient ladder, warm-up, or second mechanism.
#
#   T1 PRESSURE EXISTS: sf/teacher_snapshot_kl stays near 0.10 and
#      sf/teacher_mean_gap stays materially above zero. KL shortfall names boundary
#      infeasibility rather than silently weakening the dose.
#   T2 OPTIMIZER POINTS UP: sf/teacher_pred_gain > 0 and finite. This is an internal
#      consistency check, not proof that the critic gradient is correct.
#   T3 CRITIC SUPPORT: sf/teacher_psi_r2 and sf/teacher_w_r2 are measured after the critic
#      update that supplies the teacher and should clear 0.5. They are telemetry, no gate.
#   T4 POLICY FOLLOWS: post-update teacher/student KL and mean gap fall from their snapshot
#      baselines, with positive sf/teacher_follow_progress and alignment. The minibatch
#      distill loss alone is not this test because it is measured before each optimizer step.
#   T5 RETURN: reject if clearly behind sfocc_v3 at 1M-2M; success requires a positive,
#      sustained curve and should approach the JacTeach reference rather than merely make
#      -366 less negative.
#
# Known risk: on-policy psi can fit values while its action derivative is wrong. A positive
# predicted gain does not validate the derivative because the same critic produced both.
# The run is intentionally allowed to fail on that question; silently disabling the teacher
...
def sf_action_teacher(sf, agent, obs, target_kl, chunk):
    """Build one frozen Beta teacher by locally maximizing w.psi at fixed exact KL.

    The optimization graph ends at the latent action mean `z`: the student snapshot is
    created under no_grad, `z` is a detached leaf, and autograd.grad requests only dJ/dz.
    No critic gradient reaches the actor and no actor gradient reaches the critic.

    At fixed concentration k, the exact local Fisher diagonal for the Beta mean is
    k^2 * (trigamma(alpha) + trigamma(beta)). Maximizing g.dz at fixed Fisher norm
    gives dz proportional to F^-1*g. A fixed 16-step exact-KL bisection converts that
    direction into the requested total KL without a fidelity gate or adaptive schedule.
    """
    ...
    kl_shortfall_fracs, nonpositive_gain_fracs = [], []
    ...
    for start in range(0, obs.shape[0], chunk):
        ...
        with torch.no_grad():
            ...
            fisher_sqrt = concentration * (
                torch.polygamma(1, s_alpha) + torch.polygamma(1, s_beta)
            ).sqrt()
            inv_fisher_sqrt = fisher_sqrt.reciprocal()

        z = z0.detach().requires_grad_(True)
        predicted_value = sf.value(obs_mb, agent.z_to_action(z))
        grad = torch.autograd.grad(predicted_value.sum(), z, create_graph=False)[0]

        with torch.no_grad():
            metric_grad = grad * inv_fisher_sqrt
            metric_grad_norm = metric_grad.norm(dim=-1, keepdim=True)
            safe_norm = torch.where(
                metric_grad_norm == 0.0,
                torch.ones_like(metric_grad_norm),
                metric_grad_norm,
            )
            unit_metric_grad = metric_grad / safe_norm
            dz_unit = inv_fisher_sqrt * unit_metric_grad

            # Preserve the policy family's alpha,beta >= 1 invariant. The snapshot mean is
            # strictly inside this interval because the policy heads output 1+softplus.
            z_min = 1.0 / concentration
            z_max = 1.0 - z_min

            def at_scale(scale):
                z_teacher = torch.minimum(
                    torch.maximum(z0 + scale * dz_unit, z_min),
                    z_max,
                )
                return (
                    z_teacher,
                    z_teacher * concentration,
                    (1.0 - z_teacher) * concentration,
                )

            lo = z0.new_zeros((z0.shape[0], 1))
            hi = z0.new_full((z0.shape[0], 1), 4.0)
            for _ in range(16):
                mid = 0.5 * (lo + hi)
                _, mid_alpha, mid_beta = at_scale(mid)
                mid_kl = beta_kl_per_dim(
                    mid_alpha, mid_beta, s_alpha, s_beta
                ).sum(-1, keepdim=True)
                below = mid_kl < target_kl
                lo = torch.where(below, mid, lo)
                hi = torch.where(below, hi, mid)

            scale = 0.5 * (lo + hi)
            scale = torch.where(metric_grad_norm == 0.0, torch.zeros_like(scale), scale)
            z_teacher, t_alpha, t_beta = at_scale(scale)
            delivered_kl = beta_kl_per_dim(
                t_alpha, t_beta, s_alpha, s_beta
            ).sum(-1)
            ...
            boundary = (
                (z_teacher <= z_min + 1e-6) | (z_teacher >= z_max - 1e-6)
            ).to(z0.dtype).mean(-1)
            ...
            kl_shortfall_fracs.append((delivered_kl < 0.99 * target_kl).to(z0.dtype))
            nonpositive_gain_fracs.append((predicted_gain <= 0.0).to(z0.dtype))
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # The teacher builder uses autograd.grad only with respect to a detached latent
        # action leaf. Clearing the last supervised gradients makes that isolation directly
        ...
        # Frozen once per iteration. Its mean is a local fixed-KL improvement step under the
        # UPDATED successor critic; its concentration is the student snapshot's. The whole
        # 32768-row batch fits comfortably because the differentiated path is only psi's
        # two 64-wide layers; one large launch avoids 32 tiny action-gradient launches.
        (
            b_t_alpha,
            b_t_beta,
            b_s_alpha_before,
