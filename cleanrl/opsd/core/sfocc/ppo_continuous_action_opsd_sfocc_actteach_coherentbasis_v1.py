# OPSD-SFOcc ActTeach CoherentBasis v1 -- staged same-iteration SF coordinates.
# HYPOTHESIS: v3's joint update lets phi move while psi is fitted to targets expressed in
# the previous basis, so the teacher combines w and psi from inconsistent coordinates.
# ONE CHANGE: after the unchanged pre-update rollout channel, fit only phi+w for reward and
# decorrelation, rebuild feature lambda-returns in that updated basis, then fit only psi.
# UNCHANGED CONTROLS: fixed-concentration actor and exact 0.10-KL natural teacher, hindsight
# clone plus detached teacher KL, all defaults, telemetry, CUDA requirement, and actor-last
# update order remain those of fixedconc v3. There is no target net, EMA, schedule, or gate.
# =====================================================================================
import os
import random
import time
...
SAMPLE_EPS = 1e-6
BETA_CONCENTRATION = float(2.0 * (1.0 + np.log(2.0)))
...
class Args:
    ...
    actor_epochs: int = 1         # one pass over fixed hindsight-action and teacher targets.
    #                               Concentration is structural, so repeated fitting cannot
    #                               recreate v2's concentration ratchet.
    critic_epochs: int = 4        # passes for EACH staged SF regression. Both stages retain
    #                               v3's full supervised fitting budget; neither touches the
    #                               actor trunk, so they cannot fight the policy update.
    ...
    sf_coef: float = 0.5          # psi regression weight retained from v3 so the combined
    #                               staged sf_loss remains directly comparable.
    ...
    sf_grad_clip: float = 0.5     # matches max_grad_norm; applied independently to the
    #                               disjoint basis and psi optimizer parameter sets.
...
class SFCritic(nn.Module):
    ...
    module, split basis/psi optimizers and grad clips, and explicit parameter-set assertions
    make that isolation structural rather than assumed.
    ...
    def update_psi_stats(self, tgt, beta):
        ...
        statistics) only after the target is rebuilt in the updated frozen phi basis. The
        beta argument is kept so the existing batch-statistics implementation and buffer
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        self.actor_mean_head = layer_init(nn.Linear(H, act_dim), std=0.01)
    ...
    def policy_mean_logit(self, obs, cond):
        return self.actor_mean_head(self._feat(obs, cond))

    def beta_from_mean_logit(self, mean_logit):
        # Paired log-sigmoids keep both positive Beta parameters and their gradients
        # representable far beyond the range where float32 sigmoid rounds to 0 or 1.
        alpha = BETA_CONCENTRATION * F.logsigmoid(mean_logit).exp()
        beta = BETA_CONCENTRATION * F.logsigmoid(-mean_logit).exp()
        return alpha, beta

    def policy(self, obs, cond):
        return self.beta_from_mean_logit(self.policy_mean_logit(obs, cond))
...
def sf_action_teacher(sf, agent, obs, target_kl, chunk):
    """Build a detached fixed-concentration Beta teacher at exact forward KL.

    The critic graph ends at a detached snapshot mean-logit leaf. The exact Fisher metric
    of the fixed-concentration Beta manifold supplies a natural mean direction, normalized
    to unit Fisher norm per state. Dynamic bracketing and float64 bisection make the dose
    independent of both action-boundary proximity and float32 KL cancellation.
    """
    ...
    for start in range(0, obs.shape[0], chunk):
        ...
        with torch.no_grad():
            mean_logit0 = agent.policy_mean_logit(obs_mb, agent._zero_cond(obs_mb))
            if not bool(torch.isfinite(mean_logit0).all()):
                raise FloatingPointError(
                    "non-finite fixed-concentration policy mean logit"
                )
            s_alpha, s_beta = agent.beta_from_mean_logit(mean_logit0)
            concentration = s_alpha + s_beta
            snapshot_ok = (
                torch.isfinite(s_alpha)
                & torch.isfinite(s_beta)
                & (s_alpha > 0.0)
                & (s_beta > 0.0)
                & torch.isfinite(concentration)
                & (concentration > 0.0)
            )
            if not bool(snapshot_ok.all()):
                raise FloatingPointError(
                    "non-positive or non-finite fixed-concentration policy parameters"
                )
            z0 = s_alpha / concentration

        mean_logit = mean_logit0.detach().requires_grad_(True)
        alpha, beta = agent.beta_from_mean_logit(mean_logit)
        z = alpha / (alpha + beta)
        predicted_value = sf.value(obs_mb, agent.z_to_action(z))
        (grad_mean_logit,) = torch.autograd.grad(
            predicted_value.sum(), (mean_logit,), create_graph=False
        )

        with torch.no_grad():
            gradient_ok = torch.isfinite(predicted_value) & torch.isfinite(
                grad_mean_logit
            ).all(-1)
            if not bool(gradient_ok.all()):
                raise FloatingPointError("non-finite fixed-concentration teacher gradient")

            grad_scale = grad_mean_logit.abs().amax(-1, keepdim=True)
            zero_grad = grad_scale == 0.0
            safe_grad_scale = torch.where(
                zero_grad, torch.ones_like(grad_scale), grad_scale
            )

            # Projection geometry is deliberately float64. V2's increasingly concentrated
            # float32 Betas terminated when cancellation pushed max KL error past 1%.
            scaled_grad = (grad_mean_logit / safe_grad_scale).double()
            mean_logit64 = mean_logit0.double()
            z064 = F.logsigmoid(mean_logit64).exp()
            one_minus_z064 = F.logsigmoid(-mean_logit64).exp()
            geometry_alpha64 = BETA_CONCENTRATION * z064
            geometry_beta64 = BETA_CONCENTRATION * one_minus_z064
            dz_du = z064 * one_minus_z064
            fisher_logit = BETA_CONCENTRATION**2 * (
                torch.polygamma(1, geometry_alpha64)
                + torch.polygamma(1, geometry_beta64)
            ) * dz_du.square()
            snapshot_alpha64 = s_alpha.double()
            snapshot_beta64 = s_beta.double()
            fisher_ok = torch.isfinite(fisher_logit) & (fisher_logit > 0.0)
            if not bool(fisher_ok.all()):
                raise FloatingPointError(
                    "non-positive or non-finite fixed-concentration Beta Fisher metric"
                )

            natural_logit = scaled_grad / fisher_logit
            scaled_metric_grad_sq = (scaled_grad * natural_logit).sum(
                -1, keepdim=True
            )
            ...
            if not bool(metric_ok.all()):
                raise FloatingPointError(
                    "invalid fixed-concentration natural-gradient norm"
                )

            safe_norm = torch.where(
                zero_grad, torch.ones_like(scaled_metric_grad_sq), scaled_metric_grad_sq
            ).sqrt()
            direction_logit = natural_logit / safe_norm
            metric_grad_norm = torch.where(
                zero_grad,
                torch.zeros_like(safe_norm),
                grad_scale.double() * safe_norm,
            )
            if not bool(torch.isfinite(direction_logit).all()):
                raise FloatingPointError(
                    "non-finite fixed-concentration natural direction"
                )

            def at_scale(scale):
                teacher_logit = mean_logit64 + scale * direction_logit
                z_teacher = F.logsigmoid(teacher_logit).exp()
                one_minus_z_teacher = F.logsigmoid(-teacher_logit).exp()
                t_alpha = BETA_CONCENTRATION * z_teacher
                t_beta = BETA_CONCENTRATION * one_minus_z_teacher
            ...
            def checked_total_kl(candidate_alpha, candidate_beta):
                ...
                if not bool(torch.isfinite(kl_per_dim).all()):
                    raise FloatingPointError(
                        "non-finite fixed-concentration projection KL"
                    )
                return kl_per_dim.sum(-1, keepdim=True)

            active = ~zero_grad
            lo = mean_logit64.new_zeros((mean_logit64.shape[0], 1))
            hi = mean_logit64.new_ones((mean_logit64.shape[0], 1))
            expansions = mean_logit64.new_zeros((mean_logit64.shape[0], 1))
            for _ in range(24):
            ...
            if bool(unbracketed.any()):
                raise RuntimeError(
                    "fixed-concentration teacher failed to bracket target KL"
                )
            ...
            _, t_alpha64, t_beta64 = at_scale(scale)
            t_alpha = t_alpha64.to(s_alpha.dtype)
            t_beta = t_beta64.to(s_beta.dtype)
            # Preserve the exact snapshot contract for zero-gradient rows, including its
            # original float32 rounding.
            t_alpha = torch.where(zero_grad, s_alpha, t_alpha)
            t_beta = torch.where(zero_grad, s_beta, t_beta)
            z_teacher = t_alpha / (t_alpha + t_beta)
            # Verify the actual float32 target consumed by the actor, not only the float64
            # pre-cast solution.
            delivered_kl = beta_kl_per_dim(
                t_alpha.double(),
                t_beta.double(),
                s_alpha.double(),
                s_beta.double(),
            ...
            if not bool(teacher_ok.all()):
                raise FloatingPointError("non-finite fixed-concentration Beta teacher")
            dose_error = active.squeeze(-1) & (
                (delivered_kl - target_kl).abs() > 0.01 * target_kl
            )
            if bool(dose_error.any()):
                raise RuntimeError("fixed-concentration teacher missed target KL")
            ...
            boundary = (
                (z_teacher <= 2.0 * SAMPLE_EPS)
                | (z_teacher >= 1.0 - 2.0 * SAMPLE_EPS)
            ).to(z0.dtype).mean(-1)
            ...
            if not bool(torch.isfinite(predicted_gain).all()):
                raise FloatingPointError(
                    "non-finite fixed-concentration predicted gain"
                )
            ...
            kl_shortfall_fracs.append(
                (active.squeeze(-1) & (delivered_kl < 0.99 * target_kl)).to(
                    z0.dtype
                )
            )
            ...
            log_concentration_changes.append(
                concentration_ratio.double().log().abs().mean(-1)
            )
...
def teacher_follow_metrics(agent, obs, t_alpha, t_beta, s_alpha_before, s_beta_before, chunk):
    ...
    post_concentration = s_alpha_after + s_beta_after
    post_policy_ok = (
        torch.isfinite(s_alpha_after)
        & torch.isfinite(s_beta_after)
        & (s_alpha_after > 0.0)
        & (s_beta_after > 0.0)
        & torch.isfinite(post_concentration)
        & (post_concentration > 0.0)
    )
    if not bool(post_policy_ok.all()):
        raise FloatingPointError(
            "non-positive or non-finite post-update policy parameters"
        )
    ...
    # Follow telemetry uses float64 even though the actor loss remains float32: near-identical
    # Betas otherwise suffer the same closed-form cancellation that terminated v2.
    pre_kl = beta_kl_per_dim(
        t_alpha.double(),
        t_beta.double(),
        s_alpha_before.double(),
        s_beta_before.double(),
    ).sum(-1).clamp_min(0.0)
    post_kl = beta_kl_per_dim(
        t_alpha.double(),
        t_beta.double(),
        s_alpha_after.double(),
        s_beta_after.double(),
    ).sum(-1).clamp_min(0.0)
    ...
    active_dose = pre_kl > 1e-12
    reduction = torch.where(
        active_dose, (pre_kl - post_kl) / pre_kl, 0.0
    ).sum() / active_dose.sum().clamp_min(1)
    zero_dose_drift = torch.where(~active_dose, post_kl, 0.0).sum() / (
        (~active_dose).sum().clamp_min(1)
    )
    ...
    return {
        ...
        "student_kl_reduction": float(reduction.item()),
        ...
        "zero_dose_drift": float(zero_dose_drift.item()),
        "conc_ratio_post": float(conc_ratio.item()),
    }


# =====================================================================================
# BLOCK E -- STAGED SF LOSSES. Each phase owns a disjoint parameter set and grad clip.
# =====================================================================================
def sf_basis_losses(sf, obs, action, reward, args):
    """Reward readout plus phi decorrelation; psi is absent from this graph."""
    phi_pred = sf.phi(obs, action)
    rew_mse = (sf.w_head(phi_pred).squeeze(-1) - reward).square().mean()
    cov = offdiag_decorr(phi_pred)
    total = args.sf_rew_coef * rew_mse + args.sf_cov_coef * cov
    return total, rew_mse, cov


def sf_psi_loss(sf, obs, action, psi_target, args):
    """Psi regression in the updated, frozen phi coordinate system."""
    psi_pred = sf.psi_std(obs, action)
    psi_mse = (psi_pred - sf.standardize_psi_target(psi_target)).square().mean()
    return args.sf_coef * psi_mse, psi_mse
...
if __name__ == "__main__":
    ...
    # Two constant-lr optimizers make phase ownership structural rather than relying on
    # zeroed loss coefficients. The identity checks keep the sets disjoint and exhaustive
    # over every trainable SFCritic parameter.
    basis_parameters = [*sf.phi_net.parameters(), *sf.w_head.parameters()]
    psi_parameters = list(sf.psi_net.parameters())
    basis_parameter_ids = {id(parameter) for parameter in basis_parameters}
    psi_parameter_ids = {id(parameter) for parameter in psi_parameters}
    sf_parameter_ids = {id(parameter) for parameter in sf.parameters()}
    assert basis_parameter_ids.isdisjoint(psi_parameter_ids)
    assert basis_parameter_ids | psi_parameter_ids == sf_parameter_ids
    basis_optimizer = optim.Adam(basis_parameters, lr=args.learning_rate, eps=1e-5)
    psi_optimizer = optim.Adam(psi_parameters, lr=args.learning_rate, eps=1e-5)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ===== BLOCKS B+C: PRE-UPDATE rollout snapshot, unchanged from fixedconc v3 ======
        # These tensors exclusively define the observed hindsight channel and its comparable
        # pre-update diagnostics. They are never used as the staged psi regression target.
        phi_taken_pre, b_psi_target_pre, b_future_pre = sf_feature_targets(
            sf,
            agent,
            b_obs,
            b_next_obs,
        ...
        with torch.no_grad():
            ...
            psi_err = b_psi_target_pre - psi_taken
            psi_bias_frac = float(
                (
            ...
            w_r2 = _r2_per_dim(sf.w_head(phi_taken_pre), b_rewards.unsqueeze(-1))
            ...
            v_true = sf.w_head(b_psi_target_pre).squeeze(-1).cpu().numpy()
            variance = np.var(v_true)
            explained_variance = (
                np.nan if variance == 0 else 1.0 - np.var(v_true - v_pred) / variance
            )

        # ===== CRITIC FIRST, PHASE 1: establish the current phi+w basis ================
        # requires_grad is switched as well as optimizer ownership so psi cannot even
        # accumulate a gradient during this phase.
        basis_optimizer.zero_grad(set_to_none=True)
        psi_optimizer.zero_grad(set_to_none=True)
        for parameter in basis_parameters:
            parameter.requires_grad_(True)
        for parameter in psi_parameters:
            parameter.requires_grad_(False)
        sf_basis_totals, sf_rews, sf_covs = [], [], []
        # Consume exactly the parent's critic_epochs RNG draws, then reuse each epoch's
        # ordering in both stages. Drawing a second set for psi would shift actor minibatch
        # order and every later Beta sample, confounding the staged-update ablation.
        critic_permutations = [
            torch.randperm(args.batch_size, device=device)
            for _ in range(args.critic_epochs)
        ]
        for permutation in critic_permutations:
            for start in range(0, args.batch_size, args.minibatch_size):
                mb = permutation[start : start + args.minibatch_size]
                basis_total, sf_rew, sf_cov = sf_basis_losses(
                    sf, b_obs[mb], b_act[mb], b_rewards[mb], args
                )
                basis_optimizer.zero_grad(set_to_none=True)
                basis_total.backward()
                nn.utils.clip_grad_norm_(basis_parameters, args.sf_grad_clip)
                basis_optimizer.step()
                with torch.no_grad():
                    sf_basis_totals.append(basis_total.item())
        ...
        # ===== CRITIC FIRST, PHASE 2: rebuild targets, then fit only psi ================
        # The old target above has served the channel and diagnostics and is now dead.
        # Recompute with the just-updated frozen phi basis, then refresh standardization
        # from this exact target before any psi regression.
        phi_taken_updated, b_psi_target, _ = sf_feature_targets(
            sf,
            agent,
            b_obs,
            b_next_obs,
        ...
        coordinate_target_drift = float(
            (b_psi_target - b_psi_target_pre).square().mean().sqrt().item()
        )
        sf.update_psi_stats(b_psi_target, 0.0)
        for parameter in psi_parameters:
            parameter.requires_grad_(True)
        sf_psi_totals, sf_psis = [], []
        for permutation in critic_permutations:
            for start in range(0, args.batch_size, args.minibatch_size):
                mb = permutation[start : start + args.minibatch_size]
                psi_total, sf_psi = sf_psi_loss(
                    sf, b_obs[mb], b_act[mb], b_psi_target[mb], args
                )
                psi_optimizer.zero_grad(set_to_none=True)
                psi_total.backward()
                nn.utils.clip_grad_norm_(psi_parameters, args.sf_grad_clip)
                psi_optimizer.step()
                with torch.no_grad():
                    sf_psi_totals.append(psi_total.item())
                    sf_psis.append(sf_psi.item())
        psi_optimizer.zero_grad(set_to_none=True)
        for parameter in psi_parameters:
            parameter.requires_grad_(False)
        assert not any(parameter.requires_grad for parameter in sf.parameters())

        # These scores use the exact frozen critic and recomputed target pair supplied to
        # the teacher. The scalar EV therefore tests post-staging coordinate consistency,
        # while the parent EV above remains the comparable pre-update channel snapshot.
        with torch.no_grad():
            teacher_psi = _chunked(sf.psi, b_obs, b_act, chunk=args.batch_size)
            teacher_phi = phi_taken_updated
            ...
            teacher_v_pred = sf.w_head(teacher_psi).squeeze(-1).cpu().numpy()
            teacher_v_true = sf.w_head(b_psi_target).squeeze(-1).cpu().numpy()
            teacher_variance = np.var(teacher_v_true)
            teacher_explained_variance = (
                np.nan
                if teacher_variance == 0
                else 1.0
                - np.var(teacher_v_true - teacher_v_pred) / teacher_variance
            )

        # ===== ALWAYS-ACTIVE SF ACTION TEACHER ===========================================
        # Frozen once per iteration. Its mean follows a fixed-concentration natural
        # direction under the UPDATED successor critic, projected to exact forward KL.
        ...
        # Comparable parent tags retain their component definitions. sf_loss is the sum of
        # the independently averaged staged objectives, equal in weighting to v3's joint
        # objective; the two added tags expose how that sum split across the phases.
        writer.add_scalar(
            "losses/sf_loss",
            float(np.mean(sf_basis_totals) + np.mean(sf_psi_totals)),
            global_step,
        )
        writer.add_scalar(
            "losses/sf_basis_loss", float(np.mean(sf_basis_totals)), global_step
        )
        writer.add_scalar("losses/sf_psi_loss", float(np.mean(sf_psi_totals)), global_step)
        ...
        writer.add_scalar(
            "sf/teacher_explained_variance", teacher_explained_variance, global_step
        )
        # RMS coordinate movement from the channel snapshot's target to the target rebuilt
        # after phase 1. Both are raw lambda-return coordinates on the identical rollout.
        writer.add_scalar(
            "sf/coordinate_target_drift", coordinate_target_drift, global_step
        )
        ...
        writer.add_scalar(
            "sf/teacher_zero_dose_drift", follow_metrics["zero_dose_drift"], global_step
        )
        writer.add_scalar(
