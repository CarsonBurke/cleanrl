# OPSD-CovTilt v4 — v3's optimistic posterior self-teacher and feasible-mixture trust
# region, with the PRIVILEGED teacher context restored. No PPO ratio, no PG actor loss.
# =====================================================================================
...
#
# ------------------------------------- v4 --------------------------------------------
# v4 changes exactly ONE thing: WHAT THE TEACHER SEES. Everything downstream of the
# teacher's trunk input -- the heteroscedastic MLE, the upper-tail query, the
# feasible-mixture projection, the omega/eta bisections, the per-dim gates, the student
# loss -- is byte-faithful to v3, as are all v3 hyperparameter defaults.
#
# THE PRIVILEGE HYPOTHESIS. The distillation OPERATOR is already good enough on this
# chassis: hopsd_v27_sdgate = 8171@8M with this same self-distillation loss and a
# privileged hindsight teacher, against a PPO baseline of 8278@8M. CovTilt v1 -- the same
# distillation idea, but a teacher that sees only the scalar advantage rank -- finished
# 5743@8M. Operators reach ~8171; a rank-only teacher reaches 5743. The ~2400-point
# deficit is therefore teacher INFORMATION, not the distillation loss, and no amount of
# dose/trust-region tuning (v2, v3) addresses it. v3's "advantage rank is the only
# privilege" was an ablation of the paper's premise, not an implementation of it: OPSD's
# teacher conditions on y*, and a scalar rank is a 1-bit y*.
#
# WHICH CHANNEL, AND WHY NOT ANOTHER. Exactly one privilege channel in this lineage is
# verified to work: the pooled future-ACTION window phi (v19's near/far split, carried by
# the only 8M finisher, v27). Raw action lags, transformer contexts and outcome/return
# conditioning all failed (v21 1087@1M, v33 1736@500k, hopsd_v1 -86@2M). So v4 ports
# v27's build_future_features verbatim -- horizon H=20 (phi_horizon), NEAR = a_{t+1..t+5},
# FAR = a_{t+6..t+20}, mean+std pooled WITHIN each window, plus the validity fraction:
# phi_dim = 4*act_dim + 1 (= 25 on HalfCheetah-v4). Pooling keeps each window denoised and
# permutation-invariant; the near/far split restores one bit of temporal order (where the
# gait is drifting). Windows terminate at episode boundaries, so phi never reads actions
# from the next episode.
#
# WHY PHI IS SAFE HERE AND WAS NOT IN THE OUTCOME-CONDITIONED FAILURES. Those teachers fed
# the outcome in as one more MLP input BESIDE phi; the network explained a_t with phi, the
# outcome coordinate went inert, and the optimistic query became an out-of-distribution
# poke at a coordinate the model had learned to ignore. CovTilt is immune by construction:
# the rank u never enters the trunk, it MULTIPLIES Delta, so dropping the channel costs
# likelihood and phi cannot crowd it out. phi can only sharpen the state conditional that
# Delta(s, phi) and kappa(s, phi) are rationalized from. That immunity -- v3's central
# structural argument -- is precisely the precondition that lets the privilege back in.
#
# STRICT TEACHER-SIDE PRIVILEGE. phi is built once per iteration from the frozen rollout
# actions under no_grad: a detached constant. The teacher sees [obs, phi]; the student's
# get_action_and_value is obs-only at every call site, and phi appears nowhere in Phase 3.
# --privileged-teacher False sets phi_dim = 0, making the teacher input exactly obs and
# the run numerically identical to v3 (ablation arm).
#
# Prediction: teacher_nll drops materially below v3's at equal iteration (phi buys
# rationalization likelihood), debug/delta_abs_mean anneals more slowly than v3's, and the
# return curve tracks v27 rather than v1.
# Kill-tells: (a) nll improves but delta_abs_mean falls FASTER than v3's -> phi is
# absorbing the action's explainable variance and starving the tilt, i.e. crowd-out
# reappearing on the Delta head instead of on an input coordinate; (b) nll improves and
# returns still trace v1's 5743 shape -> rationalization quality is not the bottleneck and
# this operator caps out near 8171 whatever the teacher sees; (c) mix_eta pinned at 1.0
# with step_kl far below 1.2 -> the sharper teacher stopped proposing anything worth
# projecting, and the dose ladder, not the context, is the live dial.
# =====================================================================================
#
...
# v4 RESTORES the hindsight future-action window phi as the teacher's context (v27's
# near/far pooled channel, teacher-side only; see the v4 block above). STILL DROPPED: the
# privileged teacher critic, the AWR weights and the temperature dual.
#
...
class Args:
    ...
    # --- v4 privileged teacher context (v27's verified channel) ---
    phi_horizon: int = 20            # H: future action window (v27's hindsight_horizon)
    privileged_teacher: bool = True  # False => phi_dim = 0, i.e. numerically v3
...
class TiltTeacher(nn.Module):
    """Predicts the per-dim advantage/action correlation Delta(s, phi) and the conditional
    log-sd correction kappa(s, phi). The privileged rank u never enters here: it multiplies
    Delta downstream, so that channel is structurally load-bearing and cannot be crowded
    out by phi -- which is exactly why v4 can hand the trunk the privileged future-action
    context that every outcome-conditioned teacher in this lineage went inert beside.
    Zero-ish init (std=0.01 into tanh) => teacher == student at initialization."""

    def __init__(self, obs_dim, act_dim, phi_dim, args):
        super().__init__()
        H = args.hidden
        self.kappa_max = args.kappa_max
        self.in_dim = obs_dim + phi_dim   # phi_dim = 0 reproduces v3's obs-only teacher
        self.trunk = ThinkTrunk(self.in_dim, H, args.k_blocks, args.n_experts)
        self.delta_head = layer_init(nn.Linear(H, act_dim), std=0.01)
        self.kappa_head = layer_init(nn.Linear(H, act_dim), std=0.01)

    def forward(self, x):
        """x: [obs, phi] already concatenated (plain obs when privileged_teacher=False)."""
        feat = self.trunk(x)
...
if __name__ == "__main__":
    ...
    # v4 teacher context: near mean/std + far mean/std + validity fraction, all privileged.
    phi_dim = (4 * act_dim + 1) if args.privileged_teacher else 0
    teacher = TiltTeacher(obs_dim, act_dim, phi_dim, args).to(device)
    print(
        f"[covtilt-v4] privileged_teacher={args.privileged_teacher} "
        f"phi_horizon={args.phi_horizon} near_horizon={NEAR_HORIZON} phi_dim={phi_dim} "
        f"teacher_in={teacher.in_dim} (obs_dim={obs_dim})"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # --- the privileged channels ---------------------------------------------------
            # (a) standard-normal rank score of the advantage; enters multiplicatively.
            u_hat = normal_scores(advantages)                      # (T, B), exactly N(0,1)
            # (b) v4: v27's pooled future-ACTION windows, the only verified privilege channel
            # in this lineage. Built ONCE per iteration from the frozen rollout inside this
            # no_grad block, so phi is a detached constant with no gradient path anywhere and
            # no route into the student. Windows stop at episode boundaries.
            if args.privileged_teacher:
                near_mean, near_std, far_mean, far_std, fut_valid_frac = build_future_features(
                    actions, transition_boundaries, args.phi_horizon
                )
                phi = torch.cat(
                    [near_mean, near_std, far_mean, far_std, fut_valid_frac.unsqueeze(-1)], dim=-1
                )
            else:
                fut_valid_frac = torch.zeros_like(advantages)
                phi = advantages.new_zeros((args.num_steps, args.num_envs, 0))
        ...
        b_phi = phi.reshape(args.batch_size, phi_dim)          # detached; teacher-side only
        ...
        for _ in range(args.teacher_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                delta, kappa = teacher(torch.cat([b_obs[mb], b_phi[mb]], -1))
        ...
        with torch.no_grad():
            deltas, kappas = [], []
            for start in range(0, args.batch_size, args.minibatch_size):
                _sl = slice(start, start + args.minibatch_size)
                d_mb, k_mb = teacher(torch.cat([b_obs[_sl], b_phi[_sl]], -1))
                deltas.append(d_mb)
                kappas.append(k_mb)
        ...
        writer.add_scalar("debug/fut_valid_frac", float(fut_valid_frac.mean().item()), global_step)
