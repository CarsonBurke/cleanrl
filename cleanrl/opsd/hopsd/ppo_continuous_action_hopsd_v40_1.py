# HOPSD v40_1 — v40 + retuned privilege-budget dual (extracted + more responsive).
# =====================================================================================
# V40 500k RESULT: 4004 (best in the ledger; teacher_q_gap positive throughout, action_boundary_frac
#   0.008->0.30 = bang-bang regime entered, q_ev_replay 0.94). The design works.
# DUAL INVESTIGATION (v40_1): the KL dual was suspected asleep (teacher_kl_total hit 1.795 vs the 0.6
#   budget while lam_kl stayed ~0.05-0.10). VERDICT: NOT a bug. Reproducing the dual from the LOGGED
#   kl matches the logged lam BIT-FOR-BIT at every step — the controller tracks exactly as coded. It
#   sat low only because teacher_kl_total averaged ~0.5, just UNDER the 0.6 budget; the 1.795 was a
#   transient spike that DID produce the correct 1.82x jump (lam 0.0498->0.0904). Also found:
#   teacher_kl_total is bit-identical to losses/distill_kl (both are KL(pi_T||pi_S) on the same
#   pre-step-teacher / current-student pair) — redundant telemetry, not an error; the budget correctly
#   caps the teacher-student lead. v40_1 CHANGES (contingency for a runaway, if distill_kl climbs past
#   ~3 or the curve bends): (a) dual extracted to the pure, unit-tested dual_update_lam(); (b) eta
#   0.3->0.5 and lam floor 1e-3->0.02 so a decayed lam re-engages faster on a sustained breach
#   (kl=1.8 -> ceiling in ~7 iters vs ~12), near-inert in the current below-budget regime. Everything
#   else is byte-identical to v40.
# =====================================================================================
...
class Args:
    ...
    # v40_1: the v40 dual was VERIFIED correct (live lam reproduced bit-for-bit from the logged kl);
    # it simply sat low because kl averaged ~0.5, just UNDER budget. eta raised 0.3->0.5 and the floor
    # 1e-3->0.02 so a decayed lam re-engages faster on a true runaway (sustained kl=1.8 -> ceiling in
    # ~7 iters vs ~12; near-inert in the current below-budget regime where lam already sits ~0.05).
...
def dual_update_lam(lam, kl, budget, eta, lam_min, lam_max):
    """v40_1: one multiplicative dual-ascent step for the privilege-budget Lagrange multiplier.
    ...
    Extracted from the training loop as a pure function so the controller is unit-testable
    (v40 shipped this exact formula inline; verified bit-for-bit against the live run, so the
    behavior is unchanged — only eta/lam_min were retuned for a faster runaway response)."""
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # v40_1 ONE PRIVILEGE BUDGET dual update (extracted to dual_update_lam for testability):
        # multiplicative ascent on lam_kl driving total_teacher_kl toward teacher_kl_budget. Applied
        # AFTER the epochs — this iteration's penalty used the lam carried from the prior iteration,
        # so the dual never depends on the current update. NOTE: total_teacher_kl here is bit-identical
        # to losses/distill_kl (both measure KL(pi_T||pi_S) on the same pre-step teacher / current
        # student pair) — the budget correctly caps the teacher-student lead.
