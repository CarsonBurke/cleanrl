    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v34: DPG on the fresh on-policy states. Reparameterized student action
                # mapped to env space (same affine as rollout), twin-min Q, scale-
                # normalized by the EMA q_scale, and gated per-state by twin agreement so
                # DPG pushes off the realized frontier only where the twins agree.
                ...
                qterm = (w_rs * (-min_rs / q_scale)).mean()
                ent_bonus = (entropy / act_dim).mean()   # mean over dims (addendum): Q coverage
                actor_q_terms.append((args.q_coef * qterm).item())
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    (pg_loss + args.q_coef * qterm - args.ent_coef * ent_bonus).backward()
