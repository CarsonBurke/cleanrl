# HOPSD v24 — hindsight replay: the teacher trains off-policy on the last K batches.
# =====================================================================================
# The champion recipe is information-limited per iteration (v18: doubling teacher lr
# does nothing because the clip binds; the operator digests all the signal one batch
# holds). Meanwhile the on-policy loop discards every batch after one use — a
# structural handicap the architecture does not actually require: the teacher's
# objective is advantage-weighted MLE, which is off-policy safe BY CONSTRUCTION
# (this is AWR's core insight), and hindsight phi describes the stored trajectory, not
# the current policy. v24 keeps a ring buffer of the last replay_iters=8 batches
# (contexts, latent actions, lambda-returns; ~50MB GPU) and each epoch draws a fresh
# 32k virtual batch from the pool, recomputes advantages against the CURRENT student
# critic (adv = stored_return - V_now(s), the AWR recipe), and re-solves the
# winsorized KL-targeted tilt on that virtual batch. Equal optimizer steps and equal
# teacher FLOPs as v19 — but 8x the data diversity behind every tilt, and 10 distinct
# virtual batches per iteration instead of 10 passes over one.
# Student path unchanged: distills on the CURRENT batch toward the teacher evaluated
# at current contexts (extra no_grad forward); PPO-side leash/clips untouched.
# Risks, pre-registered: stale lambda-returns (bootstrapped with an old critic) bias
# the recomputed advantage — visible as early flatness vs control; stale-behavior
# exploitation is the POINT (good old actions get upweighted under the fresh critic).
# Read @1.5M vs v19 5531 / control 5288.
# =====================================================================================
#
...
class Args:
    ...
    replay_iters: int = 8            # v24: teacher replay pool depth, in batches (1 = on-policy v19)
...
def solve_tilt_weights(adv, adv_clip, tilt_eps, auto_temp, awr_temp):
    """v12.2 winsorized KL-targeted tilt on a flat advantage vector (see header).

    z-scores, winsorizes at +/-adv_clip, then geometric-bisects the softmax temp so
    KL(tilt || uniform) = tilt_eps. Returns (weights [N*softmax], temp, clip_frac).
    Caller wraps in no_grad.
    """
    adv_z = (adv.reshape(-1) - adv.mean()) / (adv.std() + 1e-8)
    a_flat = adv_z.clamp(-adv_clip, adv_clip)
    ...
    if auto_temp:
        ...
        temp = awr_temp
    w = torch.softmax(a_flat / temp, dim=0) * n_samp
    clip_frac = (adv_z.abs() > adv_clip).float().mean().item()
    return w, temp, clip_frac
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v24: the tilt is now solved PER EPOCH on replayed virtual batches
            # (solve_tilt_weights) — no iteration-level solve remains.
        ...
        # ---- v24: hindsight replay pool (ring buffer over the last replay_iters batches).
        # In v19 the teacher actor and critic contexts are IDENTICAL tensors
        # ([obs, windows, valid_frac]; teacher_sees_g False), so one copy serves both.
        assert not args.teacher_sees_g, "v24 replay pool assumes actor ctx == critic ctx"
        n_obs_dim = b_obs.shape[-1]
        if iteration == 1:
            pool_cap = args.replay_iters * args.batch_size
            pool_ctx = torch.zeros(pool_cap, b_teacher_actor_in.shape[-1], device=device)
            pool_z = torch.zeros(pool_cap, b_latent_zs.shape[-1], device=device)
            pool_ret = torch.zeros(pool_cap, device=device)
            pool_ptr, pool_n = 0, 0
        pool_sl = slice(pool_ptr, pool_ptr + args.batch_size)
        pool_ctx[pool_sl] = b_teacher_actor_in
        pool_z[pool_sl] = b_latent_zs
        pool_ret[pool_sl] = b_returns
        pool_ptr = (pool_ptr + args.batch_size) % pool_cap
        pool_n = min(pool_n + args.batch_size, pool_cap)
        ...
        for epoch in range(args.update_epochs):
            ...
            # ---- v24: draw this epoch's virtual batch from the replay pool with FRESH
            # advantages (stored lambda-return minus the CURRENT student critic's value
            # — the AWR recipe), then re-solve the tilt on it.
            with torch.no_grad():
                vb = torch.randint(0, pool_n, (args.batch_size,), device=device)
                v_fresh = []
                for vs in range(0, args.batch_size, args.minibatch_size):
                    vchunk = pool_ctx[vb[vs : vs + args.minibatch_size], :n_obs_dim]
                    v_fresh.append(value_logits_to_scalar(agent.get_value(vchunk)[:, 0]))
                adv_vb = pool_ret[vb] - torch.cat(v_fresh)
                awr_w_vb, temp_now, clip_frac_now = solve_tilt_weights(
                    adv_vb, args.adv_clip, args.tilt_eps, args.auto_temp, args.awr_temp
                )
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # ---- teacher update on the replayed virtual batch (own optimizer;
                # vb is already a uniform random draw, so contiguous slices are valid
                # minibatches and align with awr_w_vb positions) ----
                vb_mb = vb[start:end]
                t_alpha, t_beta = teacher.actor_params_for(pool_ctx[vb_mb])
                t_dist = Beta(t_alpha, t_beta)
                t_nll = -t_dist.log_prob(pool_z[vb_mb]).sum(-1)
                teacher_actor_loss = (awr_w_vb[start:end] * t_nll).mean()
                t_value_logits = teacher.critic_logits(pool_ctx[vb_mb])
                t_target = hl_support.project(pool_ret[vb_mb])
                ...
                # v24: the teacher trained on replay this step, so the distill target
                # needs its own forward at the CURRENT batch's contexts.
                with torch.no_grad():
                    t_alpha_cur, t_beta_cur = teacher.actor_params_for(b_teacher_actor_in[mb_inds])
                kl_dims = beta_kl_per_dim(t_alpha_cur, t_beta_cur, s_alpha, s_beta)
        ...
        writer.add_scalar("debug/awr_weight_max", awr_w_vb.max().item(), global_step)
        writer.add_scalar("debug/auto_temp", temp_now, global_step)
        writer.add_scalar("debug/clip_frac_adv", clip_frac_now, global_step)
        writer.add_scalar("debug/replay_pool_n", pool_n, global_step)
        with torch.no_grad():
            _p = awr_w_vb / awr_w_vb.sum()
            writer.add_scalar(
