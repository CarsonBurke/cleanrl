# HOPSD v8.1 — v8 + teacher->student critic distribution distillation (Rao-Blackwellized value targets).
# =====================================================================================
# NEW (v8.1): per-sample CE from the student's horizon-0 head toward the privileged
# teacher return distribution p_T(G | s, phi). Because phi carries ~no info about a_t
# given s (the dead-actor-channel diagnostic), averaging the sharp p_T(G|s,phi) over phi
# recovers the correct wide marginal p(G|s) (a mixture), so this is a VARIANCE-REDUCED
# version of the SAME return target, not a different one (Rao-Blackwellization).
# Kept auxiliary (critic_distill_coef=0.4): teacher errors are correlated across samples,
# and coupling V_S to V_T partially cancels the hindsight-baseline signal adv_hind =
# G_lambda - V_T. Prediction: faster critic EV rise, unchanged-or-better returns.
# Risk: if matched-step returns lag v8, the coupling is hurting -> drop critic_distill_coef.
# =====================================================================================
...
class Args:
    ...
    critic_distill_coef: float = 0.4  # weight of teacher->student critic distribution distillation, horizon 0 only
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Rao-Blackwellized critic distillation: pull the student's horizon-0 head
                # toward the privileged teacher return distribution p_T(G | s, phi). Computed
                # fresh under no_grad here (the teacher-training block above does not run in
                # epochs >= teacher_epochs, and its tensors belong to a freed graph); the
                # student horizon-0 logits are value_logits[:, 0] (dim-1 == horizon, matching
                # the MTP CE which softmaxes over dim=-1 == bins). Folded into v_loss so it
                # flows through the SAME critic backward/clip path (no third backward).
                with torch.no_grad():
                    t_cd_logits = teacher.critic_logits(b_teacher_critic_in[mb_inds])
                    t_cd_probs = torch.softmax(t_cd_logits, dim=-1)
                cd_loss = -(t_cd_probs * torch.log_softmax(value_logits[:, 0], dim=-1)).sum(-1).mean()
