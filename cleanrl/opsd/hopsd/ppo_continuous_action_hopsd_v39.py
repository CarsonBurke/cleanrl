# HOPSD v39 — privilege enters CREDIT ASSIGNMENT, never the action-conditional.
# =====================================================================================
...
# HYPOTHESIS. Hindsight enters credit assignment only; counterfactual advantages = luck-explained-
#   away credit -> sharper tilt -> better teacher targets at ZERO followability cost. Expected
#   signature: distill_kl ~ 0.27-0.30, adv_leak_nll -> 0, teacher_ev(u-critic) > pooled critic's,
#   return > 3139 @ 500k.
# JUDGMENT CALLS (flagged, not silently chosen). (a) HORIZON: default is the spec's PURE ONE-STEP
#   counterfactual (--cf-lambda 0.0); a GAE-lambda multi-step generalization over the SAME u-critic
#   is available behind --cf-lambda>0 for horizon-matching to GAE if the one-step baseline proves
#   too myopic. (b) BOUNDARY BOOTSTRAP: u_{t+1} for a truncated tail is unavailable, so the
...
class Args:
    ...
    # v39: counterfactual-advantage horizon for the TILT. 0.0 = pure one-step (spec default):
    # A_t = r_t + gamma V_T(s_{t+1}, u_{t+1})(1-boundary) - V_T(s_t, u_t). >0 = GAE-lambda multi-step
    # over the SAME hindsight u-critic (matches GAE's effective horizon; flag, not silently chosen).
    cf_lambda: float = 0.0
