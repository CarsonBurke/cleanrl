# HOPSD v23 — outcome-ONLY teacher context: state-delta windows REPLACE action windows.
# =====================================================================================
# Sibling of v22 (which APPENDS state-delta windows to the v19 action windows). v23
# strips the action windows entirely: the teacher's privilege is purely "how did the
# trajectory physically play out" — pooled near/far mean/std of s_{t+k} - s_t — never
# "what did the policy do". Completes the 2x2 with banked arms: actions-only (v19,
# 5531), states+actions (v22), states-only (v23), none (no-phi, 4726). Disentangles
# whether outcome context duplicates the action windows (v23 ~ v19, v22 ~ v19),
# complements them (v22 > v19 > v23), or dominates them (v23 >= v22 > v19).
# Also removes any smoothness shortcut: action windows correlate with a_t through
# policy smoothness (the v15 lesson); state deltas only through dynamics.
# =====================================================================================
#
# --- v22 rationale (retained below) ---
# In 21 versions the teacher's privilege has been future ACTIONS only — what the
# policy did, never what HAPPENED. This tests the outcome half of hindsight: state
# DELTAS (s_{t+k} - s_t, normalized obs space) pooled in the same near(1-5)/far(6-20)
# mean/std windows, appended to the v19 action windows for both teacher trunks. The
# teacher now conditions on achieved motion — velocity trends, posture drift — a
# distributed representation of "how did this play out".
# Why this shouldn't re-run the v1 collapse: v1's g was a SCALAR ~monotone in the AWR
# weight, so context cells pinned the weight and the tilted MLE degenerated. Pooled
# delta-s moments are 4*obs_dim physical features that only partially determine the
# GAE advantage (pooling discards the precise reward stream; GAE extends past the
# window) — within-cell weight contrast survives, attenuated not destroyed. And v21's
# screening result does not apply: the tilt operates on weights, not on a g-query.
# Hypothesis: outcome-conditioning shrinks within-cell action variance -> sharper
# teacher conditionals -> cleaner distill targets. Collapse tell: returns tracking
# control (5288) from below with distill_kl unusually SMALL (teacher = posterior).
# Read @1.5M vs v19 5531 / control 5288. Sibling v23 = state windows REPLACING action
# windows (isolates whether outcomes duplicate, complement, or beat actions in phi).
# =====================================================================================
#
...
class HindsightTeacher(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, args):
        super().__init__()
        H = args.hidden
        # v23: state-delta windows only (4S) + valid_frac — no action windows
        actor_in = obs_dim + 4 * obs_dim + 1 + (1 if args.teacher_sees_g else 0)
        critic_in = obs_dim + 4 * obs_dim + 1
...
def build_future_features(actions, boundaries, horizon, base=None):
    """Per-(t, env) mean/std pooled WITHIN a near and a far future window.

    actions: (T, B, D) sequence to pool; boundaries: (T, B) 1.0 where the transition
    at t ends an episode. Future step t+k (k>=1) is valid iff t+k <= T-1 and no
    boundary in transitions t..t+k-1.
    v22: if base is given, pools actions[t+k] - base[t] instead (state deltas).
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v23: pooled future state DELTAS only — the physical playout is the privilege
            s_near_mean, s_near_std, s_far_mean, s_far_std, fut_valid_frac = build_future_features(
                obs, transition_boundaries, args.hindsight_horizon, base=obs
            )
            ...
            actor_ctx = [obs, s_near_mean, s_near_std, s_far_mean, s_far_std]
            ...
            teacher_critic_in = torch.cat(
                [obs, s_near_mean, s_near_std, s_far_mean, s_far_std,
                 fut_valid_frac.unsqueeze(-1)],
                dim=-1,
            )
