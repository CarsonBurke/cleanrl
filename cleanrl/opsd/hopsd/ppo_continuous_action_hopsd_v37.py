# HOPSD v37 — outcome-privilege tokens + an epsilon-budgeted actor channel.
# =====================================================================================
# V35 AUTOPSY (INTERPOLATION-LEAK diagnosis). v35's non-lossy phi (transformer over future
#   ACTIONS) failed the gate: tf@300k=872 vs base 1461 (-40%), teacher_nll lagged at every
#   matched step, and distill_kl blew out to 0.45-0.50 vs base's 0.27-0.30 band. The deeper
#   cause: future actions are AUTOCORRELATED with a_t (policy smoothness), so a per-step
#   encoder can near-RECONSTRUCT a_t by interpolating its neighbors — the teacher rationalizes
#   by copying, not by insight, and spends its Beta sharpness on phi-NOISE. The result is a
#   pi_T(a|s,phi) that swings hard with phi per state -> the phi-free student cannot match the
#   marginal -> distill KL explodes -> anchor collapses. Pooling accidentally regularized this
#   away (a lossy phi is a followable phi), but pooling is a local minimum.
# V37 — two structural fixes (v36's dropout cap kept as an ablation, default OFF):
#   (1) PHI CONTENT SWAP. Encoder tokens are no longer future actions but future OUTCOMES:
#       token_{t+k} = concat( next_obs_{t+k} - obs_{t+k}  [state-delta in NORMALIZED obs
#       space; the shared running-mean cancels in the difference],  reward_{t+k}  [RAW, scaled
#       by the frozen q_reward_scale] ), for k=1..H, linear-projected to d_model. Where the
#       trajectory WENT and what it EARNED is improvement-relevant and CANNOT be used to cheat
#       imitation of a_t. Same RoPE / key-mask / null_summary / summary-token encoder and the
#       same compile tier; only the token content and projection width (obs_dim+1) change.
#       ENCODER BLOCK MODERNIZED (v37): SwiGLU FFN (silu-gated, hidden 8/3*d_model param-matched
#       to the old 4x MLP), per-head QK-norm (RMSNorm on Q/K before SDPA -> stable logits, flash
#       kernel preserved), pre-norm RMSNorm on both sublayers with a clean residual stream, RoPE
#       as specced, final RMSNorm before the summary readout, scaled residual-branch init
#       (out-projections * 1/sqrt(2*layers)). d_model 64 / 3 layers / 4 heads unchanged.
#       --phi-mode is retained: pooled = v34.1 ACTION near/far pooling EXACTLY (unchanged, the
#       control — pooled actions carry INTENT), attn = outcome-token summary only, hybrid
#       (default) = [obs, pooled action-features, outcome summary] (intent + outcome, complementary).
#   (2) ACTOR PRIVILEGE BUDGET (replaces dropout-as-cap). Each teacher step computes the actor
#       at FULL phi and at NULL phi (summary -> learned null; in hybrid the null side KEEPS the
#       pooled features + g, so the budget governs ONLY the summary channel). priv_kl = mean
#       per-state Beta KL(full || null) summed over dims; penalty lam_priv * priv_kl is added to
#       the teacher ACTOR loss. lam_priv is a dual variable updated AFTER the epoch loop toward a
#       target priv_eps (default 0.5 nats): lam <- clamp(lam*exp(eta*(priv_kl/priv_eps - 1)),
#       1e-3, 100), eta=0.05, lam0=0.1 — mirroring the tilt's KL-targeted E-step. The null side
#       is DETACHED (a trust-region anchor), so the penalty moves only the full-phi (distill
#       target) policy toward followability, never the anchor toward it.
# CORE DESIGN CLAIM (the asymmetry): the teacher CRITIC is UNBUDGETED — it always consumes full
#   phi with NO penalty and NO dropout. Privilege is FREE for value estimation (vdis/EV); the
#   followability constraint binds ONLY the actor channel the student must imitate. Outcome
#   privilege makes the value signal genuinely richer while the epsilon-budget keeps the policy
#   anchor followable — the two failure modes (uninformative vs unfollowable) are separated.
# HYPOTHESIS: outcome tokens + an epsilon-budgeted actor + unlimited critic privilege beats
#   both the pooled control and any dropout cap. Expected signature: teacher critic EV rises
#   ABOVE the pooled base's 0.94, distill_kl HOLDS the 0.27-0.30 followability band, and
#   debug/priv_kl rides priv_eps (debug/lam_priv self-adjusts to hold it there).
# JUDGMENT CALLS: (a) the null reference in priv_kl is stop-gradient (detached) — a fixed
#   anchor per step, so the budget constrains the full-phi policy rather than being satisfied by
#   moving the anchor. (b) the outcome state-delta uses the stored normalized obs/next_obses
#   (same tensors the replay path trusts); at truncation tails next_obses already holds
#   final_observation and the future-window mask drops post-boundary steps. (c) g is appended
#   last after phi in all modes (irrelevant at the default teacher_sees_g=False).
# =====================================================================================
...
class HindsightTeacher(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.encoder = None
        if self.needs_attn:
            # v37: encoder tokens are per-step OUTCOMES = [normalized state-delta, scaled
            # reward] -> token_dim = obs_dim + 1 (not act_dim).
            self.encoder = PhiEncoder(
                obs_dim + 1, d, args.teacher_layers, args.teacher_heads, args.teacher_ffn_mult,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if teacher.needs_attn:
                # Per-step outcome tokens. state-delta lives in NORMALIZED obs space (obs and
                # next_obses are both normalized; the shared mean cancels in the difference).
                # reward is RAW, scaled by the frozen q_reward_scale (matches the Q/return
                # units). next_obses already carries final_observation at truncation tails, and
                # the future-window validity mask drops steps past an episode boundary.
                step_tokens = torch.cat(
                    [next_obses - obs, (rewards / q_reward_scale).unsqueeze(-1)], dim=-1
                )
        ...
        b_future_tokens = ft_flat                           # (T*B, H, obs_dim+1) or None (pooled mode)
