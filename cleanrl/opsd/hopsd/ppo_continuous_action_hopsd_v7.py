# HOPSD v7 — champion (v2 awr05/kl005) + three surgical fixes from bottleneck analysis.
# =====================================================================================
# Base = v2 with awr_temp 0.5 (champion: 5539@2M, 7392@4M vs PPO baseline 5012/6716).
# The champion's binding constraint (expert review): forward-KL VARIANCE MATCHING is a
# ratchet — the AWR-weighted target's spread sits systematically below the student's,
# every iteration transfers the deficit, and the shrinking exploration spread shrinks
# the tilt's own step size (tilt is O(sigma)). Secondary: the epoch-end single-minibatch
# leash check both overshoots (p90 0.061 vs target 0.05, 49% of iters) and wastes all
# post-freeze student minibatches; and 10 teacher epochs on one batch drift the distill
# target toward plain reconstruction of taken actions (tilt decay).
#
# R1  Per-minibatch leash, EMA-triggered, budget raised to 0.06: check after every
#     student minibatch (kl_ema = 0.5*ema + 0.5*kl); exact stopping time, no overshoot,
#     no wasted updates. (0.03->0.05 was already a clear win; exact enforcement makes
#     0.06 safe.)
# R2  Mode-preserving entropy floor on the DISTILL TARGET: scale both concentration
#     excesses of the detached teacher by s = min(1, nu_cap/(ex_a+ex_b)) per dim.
#     Preserves the Beta mode (the tilt) exactly; caps sharpness (the ratchet). The
#     student's optimum IS the floored target — no gradient conflict, unlike ent_coef.
#     nu_cap=60/dim ~ entropy floor -1.35/dim (-8.1 total); champion is at -7.3@4.3M,
#     so sharpening continues, then holds. (v4 proved broad targets kill the mean-pull;
#     nu=60 is ~30x above that dead zone.)
# R3  Teacher trains only the first teacher_epochs=3 epochs; afterwards its params are
#     computed under no_grad — epochs 4-10 distill toward a STATIONARY, maximally
#     tilted target instead of one converging to batch BC. ~1.25x wall-clock speedup.
# Falsifiable per-mechanism: R1 -> approx_kl p90 within ~10% of 0.06 and near-zero
# skipped student minibatches; R2 -> student_entropy plateaus ~-8 by ~6M while
# teacher_student_mean_gap stops decaying; R3 -> teacher_nll epoch-0 vs epoch-2 split
# stays separated (no late reconstruction drift). Target: >= 8800@8M (baseline 8278).
# =====================================================================================
#
...
class Args:
    ...
    target_kl: float = 0.06          # student drift leash (R1: per-minibatch EMA-triggered stop)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # R1: per-minibatch EMA-triggered leash (exact stopping time, no epoch-end
        # overshoot, no wasted post-freeze minibatches). R3: the teacher trains only
        # the first teacher_epochs epochs; afterwards it is a frozen, maximally tilted
        # distill target.
        student_stopped = False
        kl_ema = None
        student_mb_done = 0
        for epoch in range(args.update_epochs):
            if student_stopped and epoch >= args.teacher_epochs:
                break
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                kl_ema = approx_kl.item() if kl_ema is None else 0.5 * kl_ema + 0.5 * approx_kl.item()
                if args.target_kl is not None and kl_ema > args.target_kl:
        ...
        writer.add_scalar("debug/kl_ema", kl_ema if kl_ema is not None else 0.0, global_step)
        writer.add_scalar(
            "debug/student_mb_frac",  # fraction of the full student budget actually used
            student_mb_done / float(args.update_epochs * args.num_minibatches),
            global_step,
        )
