# HOPSD v34.2 — v34.1 + two ablation-flagged off-policy scaling mechanisms (both OFF).
# =====================================================================================
# Context: the v34.1 qadv-renorm direction is endorsed and we are scaling the off-policy
# critic path harder (utd05 already finds ~6x more action-advantage than base at matched
# steps). Two orthogonal, independently-flagged levers, both default-OFF so this one file
# is the control AND each treatment arm:
#   (1) --q-nstep N (default 1 = v34.1 1-step). n-step TD targets for the REPLAY path.
#       The ring is a per-env contiguous stream (row = step*num_envs + env), so the k-th
#       same-env successor of ring index j is (j + k*num_envs) % rep_cap. Accumulate
#       discounted RAW rewards over up to n steps, STOP at the first episode boundary
#       (terminated OR truncated, from rep_boundary), bootstrap min-target-Q at that (or the
#       n-th) transition's next_obs with the TERMINATED mask (truncations bootstrap through
#       the stored real final obs; terminations do not). Window-starts whose n successors
#       would cross the ring write head are excluded (sample from the oldest
#       rep_filled-(n-1)*num_envs entries). HYPOTHESIS: n-step propagates return structure
#       through Q faster at fixed UTD (a known large win at low UTD). RISK: n-window boundary
#       arithmetic — the poison surface; the boundary mask + write-head exclusion are exact.
#   (2) --dpg-replay-frac F (default 0.0 = fresh-only). The DPG minibatch becomes a mix of
#       fresh on-policy states and an F fraction sampled from REPLAY (raw obs re-normalized
#       with the current pooled obs_rms; realized-action baseline = stored executed action).
#       Twin-agreement gate, qadv_scale normalizer, ceiling guardrail and all discriminator
#       telemetry operate over the mixed batch. HYPOTHESIS: replay-DPG multiplies the
#       improvement operator's coverage beyond the fresh-batch restriction. RISK: pushing the
#       policy at states the distill trust-region does not monitor — that is the ablation
#       question. Logs debug/dpg_replay_frac_effective.
# NOTE: the qgrad_ratio sensor and q_frontier probe remain on FRESH states (stable, arm-
# comparable diagnostics); ent_bonus (default coef 0) also stays on the fresh entropy.
# Everything at defaults (q_nstep=1, dpg_replay_frac=0.0) is byte-identical to v34.1.
# v34.1 header retained below.
# =====================================================================================
#
...
class Args:
    ...
    # v34.2: two ablation-flagged off-policy scaling mechanisms, both OFF by default so
    # this one file serves the control arm and each treatment arm.
    q_nstep: int = 1                 # n-step TD targets for the REPLAY path (1 = v34.1 1-step).
                                     # Accumulates discounted raw rewards over up to n consecutive
                                     # same-env ring transitions, stops at the first episode boundary
                                     # (terminated OR truncated), bootstraps min-target-Q at the n-th
                                     # (or boundary) next_obs with the terminated mask. Windows whose
                                     # n successors would cross the ring write head are excluded.
    dpg_replay_frac: float = 0.0     # fraction of each DPG minibatch drawn from REPLAY states (raw
                                     # obs re-normalized with the current pooled obs_rms) instead of
                                     # fresh on-policy states. 0.0 = fresh-only (v34.1). Realized-
                                     # action baseline at replay states uses the stored executed action.
...
if __name__ == "__main__":
    ...
    rep_boundary = torch.zeros((rep_cap,), device=device)  # v34.2: terminated OR truncated (n-step stop)
    rep_ptr = 0
    rep_filled = 0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            b_boundary = transition_boundaries.reshape(-1)  # v34.2: terminated OR truncated (episode end)

            # Ring write (wrap-aware; capacity need not divide batch_size). The flattened
            # order is row = step*num_envs + env, matching b_obs, so consecutive same-env
            # transitions land exactly num_envs apart in ring order (used by n-step below).
        ...
        def student_beta_params(norm_obs):
            """Student Beta (alpha, beta) over norm_obs, WITH grad -> for DPG at replay states.
            Same trunk+heads as get_action_and_value, so replay states enter the actor graph."""
            a_feat, _ = agent._trunks(norm_obs)
            alpha = 1.0 + F.softplus(agent.actor_alpha_head(a_feat))
            beta = 1.0 + F.softplus(agent.actor_beta_head(a_feat))
            return alpha, beta
        ...
        # v34.2 n-step (args.q_nstep): the ring is a per-env contiguous stream, so for a
        # window-start ring index j the k-th same-env successor is (j + k*num_envs) % rep_cap.
        # Discounted raw rewards accumulate until the FIRST episode boundary (terminated OR
        # truncated); the bootstrap sits at that (or the n-th) transition's next_obs with the
        # TERMINATED mask, so truncations bootstrap through the stored real final obs and true
        # terminations do not. Window-starts whose n successors would cross the write head are
        # excluded by sampling only from the oldest rep_filled - (n-1)*num_envs entries.
        n_step = max(1, args.q_nstep)
        ...
        for _qu in range(args.q_updates_per_iter):
            if n_step == 1:
                ...
                n_valid = max(rep_filled - (n_step - 1) * args.num_envs, 1)
                start_pos = (rep_ptr - rep_filled) % rep_cap
                a_off = torch.randint(0, n_valid, (args.q_batch,), device=device)
                j = (start_pos + a_off) % rep_cap                                       # [B] window starts
                ks = torch.arange(n_step, device=device)
                slot = (j.unsqueeze(1) + ks.unsqueeze(0) * args.num_envs) % rep_cap      # [B, n] successors
                s_obs = obs_renormalize(rep_obs[j], rms_mean, rms_var)
                s_act = rep_act[j]
                with torch.no_grad():
                    # active[:,k] = 1 iff no boundary occurred in transitions 0..k-1 (so we
                    # accumulate the reward at k). Cumprod of shifted (1-boundary) makes m =
                    # active.sum the effective horizon, stopping right after the first boundary.
                    not_bnd = 1.0 - rep_boundary[slot]                                   # [B, n]
                    ones_col = torch.ones((args.q_batch, 1), device=device)
                    active = torch.cumprod(torch.cat([ones_col, not_bnd[:, :-1]], dim=1), dim=1)
                    disc = (args.gamma ** ks.float()).unsqueeze(0)                       # [1, n]
                    G = (active * disc * (rep_rew[slot] / q_reward_scale)).sum(dim=1)    # [B]
                    m = active.sum(dim=1)                                                # [B] float, >= 1
                    last_slot = slot[torch.arange(args.q_batch, device=device), (m.long() - 1)]
                    boot_obs = obs_renormalize(rep_next_obs[last_slot], rms_mean, rms_var)
                    a_boot = student_env_action(boot_obs, reparam=False)
                    q_boot = torch.min(q1_target(boot_obs, a_boot), q2_target(boot_obs, a_boot))
                    y = G + (args.gamma ** m) * (1.0 - rep_term[last_slot]) * q_boot
        ...
        dpg_replay_fracs = []                        # v34.2: effective replay fraction of the DPG batch
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v34.2: DPG minibatch states. Fresh on-policy states as in v34.1, optionally
                # mixed with a dpg_replay_frac fraction of REPLAY states (raw obs re-normalized
                # with the current pooled obs_rms; realized-action baseline = stored executed
                # action). F=0 reduces byte-identically to the v34.1 fresh-only path (reuses the
                # distill forward's Beta and b_obs[mb_inds]/b_actions[mb_inds]).
                if args.dpg_replay_frac > 0.0 and rep_filled > 0:
                    n_rep = min(int(round(args.dpg_replay_frac * args.minibatch_size)), rep_filled)
                    n_fresh = args.minibatch_size - n_rep
                else:
                    n_rep, n_fresh = 0, args.minibatch_size
                if n_rep > 0:
                    r_idx = torch.randint(0, rep_filled, (n_rep,), device=device)
                    rep_obs_norm = obs_renormalize(rep_obs[r_idx], rms_mean, rms_var)
                    rep_alpha, rep_beta = student_beta_params(rep_obs_norm)   # WITH grad
                    dpg_obs = torch.cat([b_obs[mb_inds[:n_fresh]], rep_obs_norm], dim=0)
                    dpg_realized = torch.cat([b_actions[mb_inds[:n_fresh]], rep_act[r_idx]], dim=0)
                    dpg_alpha = torch.cat([s_alpha[:n_fresh], rep_alpha], dim=0)
                    dpg_beta = torch.cat([s_beta[:n_fresh], rep_beta], dim=0)
                else:
                    dpg_obs, dpg_realized = b_obs[mb_inds], b_actions[mb_inds]
                    dpg_alpha, dpg_beta = s_alpha, s_beta
                dpg_replay_fracs.append(n_rep / args.minibatch_size)

                # Reparameterized student action mapped to env space (same affine as rollout),
                # twin-min Q, gated per-state by twin agreement so DPG pushes off the realized
                # frontier only where the twins agree. Normalized by qadv_scale (an EMA of the
                # mean action-advantage of the proposal over the REALIZED executed action).
                s_dist_rs = Beta(dpg_alpha, dpg_beta)
                z_rs = s_dist_rs.rsample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                a_rs = q_act_low + (q_act_high - q_act_low) * z_rs
                q1_rs, q2_rs = q1(dpg_obs, a_rs), q2(dpg_obs, a_rs)
                min_rs = torch.min(q1_rs, q2_rs)
                with torch.no_grad():
                    gap_rs = (q1_rs - q2_rs).abs()
                    gm_rs = gap_rs.mean()
                    w_rs = gm_rs / (gap_rs + gm_rs + 1e-8)  # eps: avoid 0/0 if twins coincide
                    # Realized-action baseline on the SAME rows as dpg_obs (fresh: b_actions;
                    # replay: stored executed action). qadv = proposal advantage over realized.
                    q_real = torch.min(q1(dpg_obs, dpg_realized), q2(dpg_obs, dpg_realized))
        ...
        writer.add_scalar("debug/dpg_replay_frac_effective", float(np.mean(dpg_replay_fracs)) if dpg_replay_fracs else 0.0, global_step)
