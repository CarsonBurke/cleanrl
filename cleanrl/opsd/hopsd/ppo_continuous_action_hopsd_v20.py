# HOPSD v20 — three-window hindsight: dose-response on temporal granularity.
# =====================================================================================
# v19's near/far split beat the single-window control 5531 vs 5288 @1.5M (+4.6%) with
# clean dynamics — one bit of coarse temporal order in phi carries real improvement
# signal. v20 asks the obvious next question: is granularity a dial? Trisect the
# 20-step future at the SAME winning near boundary: near a_{t+1..t+5} (kept from v19),
# mid a_{t+6..t+12}, far a_{t+13..t+20}, mean/std pooled within each; context 6A+1 for
# both teacher inputs. Each window still pools >=5 steps, so per-window denoising stays
# comparable to v19's; the added mid/far contrast is the marginal information.
# Read vs v19 (2226@500k / 4416@1M / 5531@1.5M) and control (5288): if granularity is
# a dial, v20 > v19; if v19 captured the useful bit (drift direction) and further
# splits only thin the pooling, v20 ~= v19 or below — either way the curve tells us
# where phi's temporal information saturates. Jitter tell unchanged: distill_kl
# inflating toward 0.3+ with returns sagging.
# =====================================================================================
#
...
class HindsightTeacher(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, args):
        super().__init__()
        H = args.hidden
        actor_in = obs_dim + 6 * act_dim + 1 + (1 if args.teacher_sees_g else 0)  # v20: 3 windows
        critic_in = obs_dim + 6 * act_dim + 1
...
WINDOWS = (("near", 1, 5), ("mid", 6, 12), ("far", 13, 10**9))  # v20: trisected future


def build_future_features(actions, boundaries, horizon):
    """v20: per-(t, env) mean/std pooled WITHIN near/mid/far future windows.
    ...
    Returns (near_mean, near_std, mid_mean, mid_std, far_mean, far_std, valid_frac) with
    zeros where the respective window has no valid step; valid_frac covers the full horizon.
    """
    T, B, A = actions.shape
    names = [w[0] for w in WINDOWS]
    valid = torch.ones(T, B, device=actions.device)
    sums = {n: [torch.zeros(T, B, A, device=actions.device) for _ in range(2)] for n in names}
    cnts = {n: torch.zeros(T, B, device=actions.device) for n in names}
    for k in range(1, horizon + 1):
        ...
        a_k[: T - k] = actions[k:]
        w = next(n for n, lo, hi in WINDOWS if lo <= k <= hi)
    ...
    valid_frac = sum(cnts.values()) / float(horizon)
    return (*outs, valid_frac)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            (
                near_mean, near_std, mid_mean, mid_std, far_mean, far_std, fut_valid_frac
            ) = build_future_features(actions, transition_boundaries, args.hindsight_horizon)
            ...
            actor_ctx = [obs, near_mean, near_std, mid_mean, mid_std, far_mean, far_std]
            ...
            teacher_critic_in = torch.cat(
                [obs, near_mean, near_std, mid_mean, mid_std, far_mean, far_std,
