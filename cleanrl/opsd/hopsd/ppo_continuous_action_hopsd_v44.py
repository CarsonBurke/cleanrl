# HOPSD v44 — iteration-frozen target teacher for stable full-distribution distillation.
# =====================================================================================
# EVIDENCE. V41's online teacher-as-proposer is the strongest early HOPSD architecture
# (4964@500k, 6458@1M): Q ascent stays alive and sampled search finds global action
# improvements. Its student nevertheless chases teacher weights that move before every
# student minibatch; distill KL reaches 1.51@1M. The OPSD paper fixes its initial
# privileged teacher, motivating a detached target while not prescribing this RL schedule.
#
# METHOD. Keep v41's online proposal teacher, AWR, Q ascent, sampled search, replay,
# KL budget, and student loss unchanged. Add a detached target copy used only for the
# student's clipped forward KL. Preserve v41 exactly for eight warmup iterations while
# the online teacher learns a useful privileged conditional, hard-syncing the target
# after each iteration. Thereafter hold the target fixed within each iteration and update
# it once per iteration. The tau=1 arm isolates intra-iteration stationarity; tau=0.2 adds
# Polyak lag and tests whether damping the target further improves late stability. The
# online teacher remains constrained against the student by the existing privilege budget.
#
# HYPOTHESIS. Separating proposal learning from supervision removes minibatch target
# nonstationarity without weakening either improvement operator. Comparing tau=0.2 to
# tau=1 attributes any additional followability gain to target lag rather than stationarity.
# =====================================================================================
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        acc_target_online_kl = torch.zeros((), device=device)  # v44: KL(online teacher || target)
        ...
        teacher_target_active = iteration > args.teacher_target_warmup_iters
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v44 target schedule. Warmup is behavior-identical to v41: the student sees the
                # online teacher's pre-step full-phi distribution. Afterwards it sees one detached
                # target distribution held fixed for this entire iteration.
                if teacher_target_active and not student_stopped:
                    with torch.no_grad():
                        tt_summary = (
                            teacher_target.encode(b_future_actions[mb_inds], b_future_valid[mb_inds])
                            if teacher_target.needs_attn else None
                        )
                        t_alpha_q, t_beta_q = teacher_target.actor_params_from(
                            mb_obs, pooled_mb, tt_summary, t_g, None
                        )
                        acc_target_online_kl += beta_kl_per_dim(
                            t_alpha.detach(), t_beta.detach(), t_alpha_q, t_beta_q
                        ).clamp_min(0.0).sum(-1).mean()
                    target_mb_count += 1
                elif drop_mask is not None and not student_stopped:
                    with torch.no_grad():
                        t_alpha_q, t_beta_q = teacher.actor_params_from(
                            mb_obs, pooled_mb, t_summary, t_g, None
                        )
                else:
                ...
                if args.search_coef > 0.0:
                    ...
                    with torch.no_grad():
                        ...
                        ).reshape(k, Ssz)                                        # (k, S) expected values
                        w_k = torch.softmax(
        ...
        target_online_kl = (
            (acc_target_online_kl / target_mb_count).item() if target_mb_count > 0 else float("nan")
        )
        target_mb_frac = target_mb_count / _tden if teacher_target_active else 0.0
        ...
        # The student has consumed a target fixed for this iteration. Update only after all
        # minibatches: hard-sync through warmup, then make one Polyak move toward the proposal
        # teacher. Buffers are copied exactly; target parameters remain frozen.
        if iteration <= args.teacher_target_warmup_iters:
            teacher_target.load_state_dict(teacher.state_dict())
        else:
            polyak_update_module(teacher_target, teacher, args.teacher_target_tau)
