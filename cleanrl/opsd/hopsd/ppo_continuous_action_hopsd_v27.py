# HOPSD v27 — curvature-gated per-dim teacher entropy floor + per-dim distill KL budgets.
# =====================================================================================
# HYPOTHESIS: v19's fixed scalar KL budget (distill_kl_clip) and uniform teacher
# sharpness (scalar conc_cap) ignore WHERE the improvement signal lives across action
# dims. One shared per-dim signal s_d = 1 - Var_w[z_d]/Var[z_d] (AWR-tilt-weighted vs
# plain batch variance of the latent actions, EMA 0.9) measures which dims the tilt
# actually moves. Two gates, both driven by s_d_ema:
#   1) per-dim teacher concentration cap: cap_d = conc_cap_lo + (cap_hi - lo)*s_d_ema —
#      floors teacher entropy on FREE dims (the tilt doesn't move them; sharp targets
#      there buy nothing and kill exploration), keeps full sharpness on active dims.
#      Retains exploration at zero return cost.
#   2) per-dim distill budget: tau_d proportional to (s_d_ema + 0.25), normalized to
#      preserve the total budget A*distill_kl_clip (capped at 1.5x per dim) —
#      reallocates the KL budget toward the dims where the teacher carries signal.
# Kill-tells: uniform entropy rise + return sag = mis-gated (caps loosened on dims
# that mattered); s_d contrast collapsing toward uniform = signal-free (both
# mechanisms no-op back to v19). Logs: debug/s_d_0..{A-1}, debug/s_d_mean.
...
class Args:
    ...
    conc_cap_lo: float = 20.0        # v27: per-dim cap floor on free dims (s_d_ema -> 0)
...
class HindsightTeacher(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, args):
        ...
        with torch.no_grad():
            self.critic_head.weight.zero_()
        # v27: per-dim concentration cap, updated each iteration from s_d_ema
        # (cap_d = conc_cap_lo + (cap_hi - conc_cap_lo) * s_d_ema); init at cap_hi.
        self.register_buffer(
...
if __name__ == "__main__":
    ...
    # v27: EMA of the per-dim tilt signal s_d = 1 - Var_w[z_d]/Var[z_d] (init 0.5).
    s_d_ema = torch.full((act_dim,), 0.5, device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # --- v27: shared per-dim signal s_d = 1 - Var_w[z_d]/Var[z_d] over the
            # flat batch latents, with p = normalized tilt weights. High s_d = the
            # tilt concentrates that dim (improvement signal lives there); low s_d =
            # the dim is free. EMA'd (0.9) into s_d_ema, which drives BOTH the
            # per-dim teacher concentration cap and the per-dim distill budget,
            # updated here so they apply to THIS iteration's teacher/student epochs.
            z_flat = latent_zs.reshape(-1, act_dim)                       # (N, A)
            awr_w_flat = awr_w.reshape(-1)                                # (N,)
            p_w = (awr_w_flat / awr_w_flat.sum()).unsqueeze(-1)           # (N, 1)
            var_z = z_flat.var(dim=0, unbiased=False)                     # (A,)
            mu_w = (p_w * z_flat).sum(0)                                  # (A,)
            var_w = (p_w * (z_flat - mu_w).pow(2)).sum(0)                 # (A,)
            s_d = (1.0 - var_w / var_z.clamp_min(1e-12)).clamp(0.0, 1.0)  # (A,)
            s_d_ema.mul_(0.9).add_(0.1 * s_d)
            # Mechanism 1: per-dim teacher concentration cap (entropy floor on free dims).
            cap_d = args.conc_cap_lo + (args.teacher_conc_cap - args.conc_cap_lo) * s_d_ema
            teacher.conc_cap_d.copy_(cap_d)
            # Mechanism 2: per-dim distill KL budget, total budget preserved
            # (sum tau_d = A * distill_kl_clip before the 1.5x per-dim cap).
            tau_raw = s_d_ema + 0.25
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # scalar distill_kl_clip; total budget preserved, reallocated by s_d_ema.
        ...
        for _d in range(act_dim):
            writer.add_scalar(f"debug/s_d_{_d}", s_d[_d].item(), global_step)
        writer.add_scalar("debug/s_d_mean", s_d.mean().item(), global_step)
