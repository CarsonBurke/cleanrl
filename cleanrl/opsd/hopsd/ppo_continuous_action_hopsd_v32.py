# HOPSD v32 — leverage-weighted improvement operator (on top of v30's reward bonus).
# =====================================================================================
# v30 proved leverage-shaping through the REWARD channel (+~500 over cap20 matched).
# v32 adds the OPERATOR channel: the per-state advantage fed to the AWR tilt is scaled
# by normalized leverage lev_z = zscore(|V_T - V_S|):
#   adv_for_tilt = adv_z * clamp(1 + lev_coef * lev_z, 0.25, 3.0)
# applied BEFORE winsorize + KL-targeted temp solve, so the fixed 1.2-nat tilt budget
# is spent preferentially on high-leverage states (decisive where decisions matter,
# soft where they don't) while realized KL — and therefore ESS — stays exactly on
# target automatically. Negative advantages at high leverage get amplified too:
# decisive avoidance is also credit assignment. Composed with vdis_coef=0.1 (proven).
# Kill-tell: if debug/lev_factor telemetry saturates at the clamp rails, lev_coef is
# too hot; if v32 <= v30 at matched steps, the operator channel is redundant with the
# reward channel and gets isolated (lev only, vdis 0) before being dropped.
# =====================================================================================
#
# --- v30 (retained below) ---
...
class Args:
    ...
    lev_coef: float = 0.5            # v32: leverage scaling of the tilt advantage (0 = off)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v32: spend the tilt's KL budget on high-leverage states. Warmup-gated
            # with the same ramp as the reward bonus (teacher critic must be fit).
            lev_z = (vdis - vdis.mean()) / (vdis.std() + 1e-8)
            lev_gate = min(1.0, (iteration - 1) / max(1, args.vdis_warmup_iters))
            lev_factor = (1.0 + args.lev_coef * lev_gate * lev_z).clamp(0.25, 3.0)
            # Re-standardize after the multiply: the temp solve downstream is
            # scale-invariant but the winsorize is not — without this, lev_coef
            # would tighten the clamp (in sigma units) preferentially on the
            # high-leverage tails it is meant to emphasize.
            adv_lev = adv_z * lev_factor
            adv_lev = (adv_lev - adv_lev.mean()) / (adv_lev.std() + 1e-8)
            adv_c = adv_lev.clamp(-args.adv_clip, args.adv_clip)
        ...
        writer.add_scalar("debug/lev_factor_max", lev_factor.max().item(), global_step)
        writer.add_scalar(
            "debug/lev_factor_railfrac",
            ((lev_factor <= 0.25) | (lev_factor >= 3.0)).float().mean().item(),
            global_step,
        )
