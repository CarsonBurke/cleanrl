# HOPSD v29.1 — extragradient distillation, exact-preservation parametrization.
# v29's target rebuild (1+(nu-2)*mu*) center-biased the mean by (1-2mu)/nu — fatal
# while the early teacher is diffuse (nu~3): target pinned near z=0.5, student never
# commits, teacher never sharpens (157 @400k). v29.1 rebuilds as (mu**nu, (1-mu*)*nu)
# clamped >=1, which reproduces the teacher's Beta EXACTLY at d_mu=0; the momentum
# hypothesis itself is unchanged and still under test at extra_m=1.0.
# =====================================================================================
# HOPSD v29 — extragradient distillation: distill toward the teacher's MOMENTUM.
# =====================================================================================
# The improvement signal reaching the student each iteration is the teacher's move
# under one 1.2-nat tilt — small by design (the trust region). But the teacher's
# movement DIRECTION is persistent across iterations when improvement is real and
# noise-like when it isn't. v29 extrapolates: keep a frozen copy of last iteration's
# teacher; at distill time compute per-dim mean displacement d = mu_now - mu_prev on
# the current contexts and target mu* = mu_now + extra_m * d (extra_m=1.0 -> up to 2x
# the mean shift), concentration taken from teacher_now; alpha*/beta* rebuilt as
# 1 + (nu-2)*mu*. Persistent directions compound (Nesterov-style), noise cancels in
# the d-diff. The clamp story: |extrapolation| never exceeds |d| itself, so the
# distill target stays within 2x the teacher's own per-iteration trust region —
# bounded amplification, unlike direct AWR (3153, unleashed). First iteration d=0.
# Tells: distill_clipfrac spike + boundary-mean drift = amplifying teacher noise;
# debug/extra_shift tracks |d| (should fall as the run converges — if it doesn't,
# the teacher is oscillating and extra_m should drop). Read @1.5M vs v19 5531.
# =====================================================================================
#
...
class Args:
    ...
    extra_m: float = 1.0             # v29: extragradient momentum on the teacher's per-iter mean shift
...
if __name__ == "__main__":
    ...
    # v29: frozen previous-iteration teacher for the extragradient distill target
    teacher_prev = HindsightTeacher(obs_dim, act_dim, args).to(device)
    teacher_prev.load_state_dict(teacher.state_dict())
    for p in teacher_prev.parameters():
        p.requires_grad_(False)
    prev_dbar = None  # v29.1: last iteration's aggregate per-dim teacher displacement
    prev_dbar_perp = None  # v29.1: same, with the gap direction projected out
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # v29.1: the tilt's first-moment shift — the pure advantage-improving
        # direction in z space (teacher-independent), for the d-perp alignment screen
        with torch.no_grad():
            _z_flat = latent_zs.reshape(-1, latent_zs.shape[-1])
            _p_w = b_awr_w / b_awr_w.sum()
            tilt_dir = (_p_w.unsqueeze(-1) * _z_flat).sum(0) - _z_flat.mean(0)

        b_inds = np.arange(args.batch_size)
        distill_kls, distill_clipfracs, teacher_nlls, extra_shifts = [], [], [], []
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v29: extragradient target — extrapolate the teacher's per-iteration
                # mean displacement, concentration from teacher_now (see header).
                with torch.no_grad():
                    tp_alpha, tp_beta = teacher_prev.actor_params_for(b_teacher_actor_in[mb_inds])
                    mu_now = t_alpha / (t_alpha + t_beta)
                    mu_prev = tp_alpha / (tp_alpha + tp_beta)
                    d_mu = mu_now - mu_prev
                    mu_star = (mu_now + args.extra_m * d_mu).clamp(1e-4, 1.0 - 1e-4)
                    # v29.1: exact-preservation parametrization — at d_mu=0 this
                    # reproduces the teacher's Beta exactly (v29's 1+(nu-2)*mu form
                    # center-biased the target by (1-2mu)/nu, huge for the diffuse
                    # early teacher and a chicken-and-egg trap: see v29 @400k=157).
                    nu_tot = t_alpha + t_beta
                    tx_alpha = (mu_star * nu_tot).clamp_min(1.0)
                    tx_beta = ((1.0 - mu_star) * nu_tot).clamp_min(1.0)
                    extra_shifts.append(d_mu.abs().mean().item())
                kl_dims = beta_kl_per_dim(tx_alpha, tx_beta, s_alpha, s_beta)
        ...
        with torch.no_grad():
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v29.1 diagnostics: same-state teacher displacement vs improvement gap
                tpa, tpb = teacher_prev.actor_params_for(b_teacher_actor_in[sl])
                d_chunks.append(ta / (ta + tb) - tpa / (tpa + tpb))
                gap_chunks.append(ta / (ta + tb) - sa / (sa + sb))
            t_vals = torch.cat(t_vals).cpu().numpy()
            teacher_ev = np.nan if var_y == 0 else 1 - np.var(y_true - t_vals) / var_y
            # Alignment: is the teacher's per-iteration move pointed at the student->
            # teacher improvement direction, or is it policy-tracking noise?
            d_full = torch.cat(d_chunks).flatten()
            g_full = torch.cat(gap_chunks).flatten()
            extra_align_cos = float(F.cosine_similarity(d_full, g_full, dim=0))
            # Persistence: does the aggregate per-dim displacement direction survive
            # across iterations? (noise -> ~0, real improvement drift -> > 0)
            dbar = torch.cat(d_chunks).mean(0)
            if prev_dbar is None:
                extra_persist_cos = 0.0
            else:
                extra_persist_cos = float(F.cosine_similarity(dbar, prev_dbar, dim=0))
            prev_dbar = dbar.clone()
            # d-perp: displacement with the gap direction projected out — the only
            # component the norm-clipped student step can actually be rotated by.
            gbar = torch.cat(gap_chunks).mean(0)
            g_hat = gbar / gbar.norm().clamp_min(1e-8)
            dbar_perp = dbar - (dbar @ g_hat) * g_hat
            if prev_dbar_perp is None:
                extra_perp_persist_cos = 0.0
            else:
                extra_perp_persist_cos = float(
                    F.cosine_similarity(dbar_perp, prev_dbar_perp, dim=0)
                )
            prev_dbar_perp = dbar_perp.clone()
            # Is the teacher's orthogonal drift advantage-guided (aligned with the
            # tilt's first-moment shift) or tracking/refit residual?
            extra_perp_tilt_cos = float(F.cosine_similarity(dbar_perp, tilt_dir, dim=0))

        # v29: this iteration's trained teacher becomes next iteration's anchor
        # (after diagnostics, so the displacement above is the full-iteration move)
        teacher_prev.load_state_dict(teacher.state_dict())
        ...
        writer.add_scalar("debug/extra_shift", np.mean(extra_shifts), global_step)
        writer.add_scalar("debug/extra_align_cos", extra_align_cos, global_step)
        writer.add_scalar("debug/extra_persist_cos", extra_persist_cos, global_step)
        writer.add_scalar("debug/extra_perp_persist_cos", extra_perp_persist_cos, global_step)
        writer.add_scalar("debug/extra_perp_tilt_cos", extra_perp_tilt_cos, global_step)
