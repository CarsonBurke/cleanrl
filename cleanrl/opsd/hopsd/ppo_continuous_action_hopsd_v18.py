# HOPSD v18 — faster teacher: accelerate the improvement operator now that the
# teacher is known to BE the trust region.
# =====================================================================================
# v17 (no-teacher, direct AWR on the student) collapsed to 3153@1.5M with approx_kl
# 0.45-0.52 — 10x the teacher-mediated 0.05 — proving the teacher indirection is the
# load-bearing stabilizer, not cosmetics: the teacher absorbs the tilt into a smoothed
# conditional and the student's per-iteration movement is BOUNDED BY HOW FAR THE
# TEACHER MOVES. That makes the teacher's learning rate the direct throttle on the
# improvement operator — a lever we have never touched (teacher lr has always been
# tied 1:1 to the student's). v18: teacher_lr_mult (default 2.0) scales the teacher's
# Adam lr and its anneal. Prediction: approx_kl settles near 2x (~0.10-0.13) — still
# far below v17's 0.45 thrash zone — teacher_student_mean_gap grows, and matched-step
# returns beat control (2206@500k / 4159@1M / 5288@1.5M). Failure mode: teacher NLL
# noise amplified 2x outpaces what one distill epoch can transfer smoothly ->
# approx_kl > 0.2 with entropy crashing (the v13/v17 thrash signature) -> back off to
# 1.5. Everything else is byte-identical to v12.2 (winsorized KL-targeted tilt @ 1.2).
# =====================================================================================
#
...
class Args:
    ...
    teacher_lr_mult: float = 2.0     # v18: teacher lr = mult * student lr (the operator throttle)
...
if __name__ == "__main__":
    ...
    teacher_optimizer = optim.Adam(
        teacher.parameters(), lr=args.learning_rate * args.teacher_lr_mult, eps=1e-5
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            teacher_optimizer.param_groups[0]["lr"] = lrnow * args.teacher_lr_mult
