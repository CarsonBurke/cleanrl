# HOPSD v14 — heteroscedastic optimistic tilt: uncertainty-directed exploration
# through the improvement operator, paid for by the critic distribution we already
# compute and discard.
# =====================================================================================
# The recipe has ZERO exploration pressure: no entropy bonus, no PPO surrogate, and a
# sharpening teacher drags the student's entropy down monotonically. A flat optimism
# bonus would be erased by the tilt's z-centering; a HETEROSCEDASTIC one survives it.
# Mechanism: the student critic's HL-Gauss categorical gives a per-state predictive
# spread sigma(s) (symlog moments + delta method — same read as v12.1's precision).
# We run a SECOND GAE pass whose rewards carry an intrinsic bonus
# kappa * sigma(s_{t+1}) * nonterminal, and feed ONLY this optimistic advantage into
# the tilt (winsorize + KL-targeted dual unchanged, base v12.2). The critic targets,
# returns, and teacher-critic CE stay strictly honest — optimism must never touch the
# value function or it compounds. Net force: the teacher's improvement operator is
# re-ranked toward actions that land in states the critic is still unsure about —
# UCB-style directed exploration with no entropy term and no leash.
# Failure modes: (a) optimism leaks into value targets -> watch losses/explained_
# variance (must hold ~0.96) and debug/returns_mean vs the v12.2 control; (b) bonus
# too big -> tilt chases noise, matched-step return lags; too small -> no-op. Watch
# debug/opt_bonus_ratio (bonus std / advantage std) and debug/student_entropy (the
# intended effect: entropy stops free-falling while returns hold).
# =====================================================================================
#
...
class Args:
    ...
    opt_kappa: float = 0.1           # v14: intrinsic bonus = kappa * sigma_critic(s_{t+1}) (tilt-only)
    phi_zero: bool = False           # ablation: zero the hindsight context (teacher sees obs only);
                                     # with opt_kappa=0 this is the red-team phi-placebo test on v12.2
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v14: predictive spread of the landing-state value distribution — the
            # intrinsic bonus's per-state scale. Symlog moments + delta method
            # (sigma_raw ~= sigma_log * exp(|mu_log|)), from logits already computed
            # for the GAE bootstrap. Honest values stay untouched.
            ntv_probs = torch.softmax(next_transition_value_logits, dim=-1)
            coords = hl_support.coord_support
            ntv_mu_log = (ntv_probs * coords).sum(-1)
            ntv_var_log = ((ntv_probs * coords.pow(2)).sum(-1) - ntv_mu_log.pow(2)).clamp_min(1e-8)
            sigma_next = (ntv_var_log.sqrt() * ntv_mu_log.abs().exp()).reshape(
                args.num_steps, args.num_envs
            )

            advantages = torch.zeros_like(rewards).to(device)
            advantages_opt = torch.zeros_like(rewards).to(device)
            lastgaelam = 0
            lastgaelam_opt = 0
            for t in reversed(range(args.num_steps)):
                ...
                # Optimistic twin: same values, rewards + heteroscedastic bonus.
                # Feeds ONLY the tilt — never the critic targets.
                delta_opt = delta + args.opt_kappa * sigma_next[t] * bootstrap_nonterminal
                advantages_opt[t] = lastgaelam_opt = (
                    delta_opt + args.gamma * args.gae_lambda * lambda_nonterminal * lastgaelam_opt
                )
            ...
            if args.phi_zero:
                fut_mean = torch.zeros_like(fut_mean)
                fut_std = torch.zeros_like(fut_std)
                fut_valid_frac = torch.zeros_like(fut_valid_frac)
            # v14: the tilt runs on the OPTIMISTIC advantages; everything value-side
            # (returns, critic CE, g) stays on the honest ones.
            adv_z = (advantages_opt - advantages_opt.mean()) / (advantages_opt.std() + 1e-8)   # batch scope
        ...
        writer.add_scalar("debug/opt_sigma_mean", sigma_next.mean().item(), global_step)
        writer.add_scalar(
            "debug/opt_bonus_ratio",
            ((advantages_opt - advantages).std() / (advantages.std() + 1e-8)).item(),
            global_step,
        )
        with torch.no_grad():
            _p = awr_w / awr_w.sum()
