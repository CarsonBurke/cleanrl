...
#
# OPSD v31: teacher-aware learned full-batch gradient matching.
#
# v31 note: v28 learns well, but the weighter target only matches positive-advantage
# policy improvement while the real actor also receives signed hindsight-teacher KL.
# v31 trains the full learned distribution to align weighted-MLE gradients with the
# combined student actor-shaping gradient: positive-advantage improvement plus the
# active teacher KL term. No hard top-k or fixed exp-A actor rule.
#
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                match_alpha, match_beta = agent.get_action_params(mb_obs)
                target_hindsight_kl = beta_kl_per_dim(
                    b_fixed_teacher_alpha[mb_inds],
                    b_fixed_teacher_beta[mb_inds],
                    match_alpha,
                    match_beta,
                ).sum(-1)
                target_hindsight_loss = (
                    b_hindsight_weights[mb_inds].detach() * target_hindsight_kl.clamp(max=args.hindsight_kl_clip)
                ).mean()
                target_loss = -(positive_advantages * match_logprob).mean() + active_hindsight_beta * target_hindsight_loss
