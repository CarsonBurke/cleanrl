...
#
# OPSD v22: Teacher-weighted AWR.
#
# The privileged hindsight teacher outputs a scalar advantage estimate (from
# obs + future-conditioned hindsight context) that drives the AWR weights,
# replacing exp(GAE_z). The teacher is trained by smooth-L1 regression to the
# z-scored GAE advantage, acting as a future-informed, lower-variance denoiser
# of the per-sample advantage. No Beta distillation: the teacher influences the
# actor ONLY through the weights.
#
#   L_actor = - E[ w(s) * log pi_S(a|s) ],  w(s) = exp(A_teacher_z(s) / beta).
#
# Hypothesis: a learned, future-informed, lower-variance weighting moves the
# policy more reliably than raw per-sample GAE. The Dreamer3-bucket HL-Gauss
# critic and AWR actor (no PPO clipped surrogate) match v13; a small entropy
# floor (ent_coef) is kept since AWR has no trust-region clip.
import os
import random
import time
...
class Args:
    ...
    awr_temp: float = 0.5
    """AWR temperature beta in exp(A_z / beta); teacher advantage is z-scored first"""
...
if __name__ == "__main__":
    ...
    teacher.adv_head = maybe_compile(teacher.adv_head, "teacher.adv_head", args)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # z-scored GAE advantage: regression target for the teacher and the
        # reference signal it denoises.
        b_advantages_z = (
            (b_advantages - b_advantages.mean()) / (b_advantages.std(unbiased=False) + 1e-8)
        ).detach()

        # Fixed (no-grad) teacher scalar advantage over the whole batch, used to
        # form the AWR weights for this update. Computed in minibatch chunks.
        fixed_teacher_advs = []
        with torch.no_grad():
            for start in range(0, args.batch_size, args.minibatch_size):
                end = start + args.minibatch_size
                fixed_teacher_advs.append(teacher(b_obs[start:end], b_hindsight_context[start:end], args))
            b_fixed_teacher_adv = torch.cat(fixed_teacher_advs, dim=0)
            b_fixed_teacher_adv_z = (
                b_fixed_teacher_adv - b_fixed_teacher_adv.mean()
            ) / (b_fixed_teacher_adv.std(unbiased=False) + 1e-8)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Teacher-weighted AWR actor loss (no PPO ratio / clipped
                # surrogate): weighted MLE on the policy's own sampled actions,
                # weight = exp(A_teacher_z / beta). The teacher's future-informed
                # scalar advantage replaces the raw per-sample GAE z-score.
                # Weights are detached and clamped so a single high-advantage
                # sample cannot dominate the regression.
                t_adv_z = b_fixed_teacher_adv_z[mb_inds]
                awr_weights = torch.exp(t_adv_z / args.awr_temp).clamp(max=args.awr_weight_clip)
                ...
                # Teacher regression: privileged scalar advantage -> z-scored GAE
                # advantage (a future-informed denoiser of the per-sample target).
                mb_teacher_adv = teacher(b_obs[mb_inds], b_hindsight_context[mb_inds], args)
                teacher_target = b_advantages_z[mb_inds]
                teacher_loss = args.teacher_loss_coef * torch.nn.functional.smooth_l1_loss(
                    mb_teacher_adv, teacher_target
                )
                ...
                teacher_adv_means.append(mb_teacher_adv.detach().mean().item())
                teacher_adv_stds.append(mb_teacher_adv.detach().std(unbiased=False).item())
        ...
        writer.add_scalar("losses/hindsight_teacher_loss", np.mean(teacher_losses), global_step)
        writer.add_scalar(
        ...
        writer.add_scalar("debug/hindsight_teacher_adv_mean", np.mean(teacher_adv_means), global_step)
        writer.add_scalar("debug/hindsight_teacher_adv_std", np.mean(teacher_adv_stds), global_step)
