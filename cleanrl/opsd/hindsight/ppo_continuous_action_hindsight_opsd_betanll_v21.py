...
#
# OPSD v21: adaptive-temperature KL-controller AWR.
#
# v21 note: a live awr_temp is driven so realized per-update approx_kl tracks a
# setpoint (target_awr_kl) via a log-beta integrator, with an ESS-floor guard so
# the temperature is not lowered further when weights already collapse to few samples.
#
...
class Args:
    ...
    awr_temp: float = 1.0
    """AWR temperature beta in exp(A_z / beta); advantages are z-scored first"""
    awr_weight_clip: float = 20.0
    """maximum AWR exp-advantage weight, to bound a single sample's influence"""
    target_awr_kl: float = 0.02
    """setpoint for the realized per-update approx_kl that the temperature controller tracks"""
    beta_ctrl_lr: float = 0.1
    """integrator step size for the log-temperature KL controller"""
    awr_temp_min: float = 0.2
    """lower clip on the live AWR temperature"""
    awr_temp_max: float = 2.0
    """upper clip on the live AWR temperature"""
    awr_ess_min_frac: float = 0.3
    """ESS floor (as a fraction of minibatch) below which the controller will not lower temperature further"""
...
if __name__ == "__main__":
    ...
    # v21: persistent adaptive-temperature controller state (across iterations)
    log_beta = math.log(args.awr_temp)
    awr_temp_live = args.awr_temp
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # AWR actor loss (no PPO ratio / clipped surrogate): weighted MLE
                # on the policy's own sampled actions, weight = exp(A_z / beta),
                # i.e. forward-KL toward the analytic exp-advantage tilt of the old
                # policy. Weights are detached and clamped so a single high-advantage
                # sample cannot dominate the regression.
                awr_weights = torch.exp(mb_advantages / awr_temp_live).clamp(max=args.awr_weight_clip)
                awr_weights = awr_weights / (awr_weights.mean() + 1e-8)
                pg_loss = -(awr_weights.detach() * newlogprob).mean()
                # v21: track realized KL and weight second moment for the controller
                awr_kls.append(approx_kl.item())
                awr_w2s.append((awr_weights.detach() ** 2).mean().item())
        ...
        # v21: update the adaptive AWR temperature once per iteration.
        # ess_frac = 1/E[w^2] with w normalized to mean 1 is already the effective
        # sample FRACTION in (0,1]; compare it directly to the floor (not scaled by
        # minibatch_size). The guard only blocks the sharpening branch (err<0) when
        # effective sample fraction has already dropped below the floor.
        mean_kl = float(np.mean(awr_kls))
        ess_frac = 1.0 / max(float(np.mean(awr_w2s)), 1e-8)
        err = mean_kl / args.target_awr_kl - 1.0
        if not (err < 0 and ess_frac < args.awr_ess_min_frac):
            log_beta += args.beta_ctrl_lr * err
        awr_temp_live = float(np.clip(math.exp(log_beta), args.awr_temp_min, args.awr_temp_max))
        writer.add_scalar("debug/awr_temp_live", awr_temp_live, global_step)
        writer.add_scalar("debug/awr_mean_kl", mean_kl, global_step)
        writer.add_scalar("debug/awr_ess_frac", ess_frac, global_step)
