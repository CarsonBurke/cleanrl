...
#
# OPSD v26: learned full-batch first-order utility distribution.
#
# v26 note: v25 learns, but its ReLU-positive gradient target is still hand-shaped.
# v26 computes each sample's exact first-order utility for the current actor:
# utility_i = grad(-log pi_i) dot grad(E[-A log pi]). A useful sample is one whose
# weighted-MLE update would decrease the signed advantage policy-improvement loss.
# The weighter learns the full softmax utility distribution over all samples; the
# actor update uses only detached predicted weights. No fixed top-k or exp-A rule.
#
...
class Args:
    ...
    """entropy bonus for the learned sample distribution during utility distillation"""
    weighter_utility_temp: float = 0.5
    """temperature for the exact first-order utility target distribution"""
...
def actor_neglogprob_sample(actor_beta, action_low, action_high, actor_params, obs, action):
    beta_head = torch.func.functional_call(actor_beta, actor_params, (obs.unsqueeze(0),)).squeeze(0)
    ...
    z = ((action - action_low) / (action_high - action_low)).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
    return -beta_log_prob_per_dim(alpha, beta, z).sum()
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                per_sample_grads = torch.func.vmap(
                    torch.func.grad(actor_neglogprob_sample, argnums=3),
                    in_dims=(None, None, None, None, 0, 0),
                )(
                    agent.actor_beta,
                    agent.action_low,
                    agent.action_high,
                ...
                grad_rows = torch.cat([grad.reshape(mb_n, -1) for grad in per_sample_grads.values()], dim=1).detach()
                target_grad = (mb_advantages.detach().unsqueeze(1) * grad_rows).mean(0)
                utility = grad_rows @ target_grad
                utility_z = (utility - utility.mean()) / (utility.std(unbiased=False) + 1e-8)
                target_probs = torch.softmax(utility_z / args.weighter_utility_temp, dim=0)

                match_scores = weighter(mb_features, args)
                match_log_probs = torch.log_softmax(match_scores, dim=0)
                match_probs = match_log_probs.exp()
                utility_ce = -(target_probs * match_log_probs).sum()
                match_entropy = -(match_probs * match_log_probs).sum()
                match_entropy = match_entropy / max(np.log(mb_n), 1e-8)
                weighter_loss = utility_ce - args.weighter_entropy_coef * match_entropy
                ...
                # weighter is optimized above to predict exact first-order utility;
                # the real actor update treats the learned weights as fixed.
                with torch.no_grad():
                ...
                weighter_utility_losses.append(utility_ce.item())
        ...
        writer.add_scalar("losses/advantage_weighter_utility_ce", np.mean(weighter_utility_losses), global_step)
