...
#
# OPSD v20: V-MPO top-fraction softmax AWR weighting.
#
# v20 note: imitate only the top vmpo_topk_frac of the minibatch by advantage,
# softmax-weighted within (keeps relative magnitude) and zero elsewhere, dropping
# the dead uniform-BC component while preserving grad-norm/clip dynamics.
#
...
class Args:
    ...
    vmpo_topk_frac: float = 0.5
    """fraction of the minibatch (top by advantage) kept for the V-MPO softmax weighting"""
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v20: V-MPO-style top-fraction softmax. Imitate only the top
                # vmpo_topk_frac of the minibatch by advantage (softmax-weighted within,
                # retaining relative magnitude), zero the rest -> removes the dead
                # uniform-BC component, mean-1 normalized so grad-norm/clip dynamics hold.
                k = max(1, int(args.vmpo_topk_frac * mb_advantages.numel()))
                topv, topi = torch.topk(mb_advantages, k)
                awr_weights = torch.zeros_like(mb_advantages)
                awr_weights[topi] = torch.softmax(topv / args.awr_temp, dim=0)
                awr_weights = awr_weights * mb_advantages.numel()
