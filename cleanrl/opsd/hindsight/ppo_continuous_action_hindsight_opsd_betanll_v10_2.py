...
#
# PPO + Hindsight-Conditioned OPSD Teacher, Beta teacher beta-NLL v10.2 finite target.
#
# Hypothesis:
...
# v10.2 removes PPO actor optimization and treats OPSD as the actor objective:
...
# The teacher is trained separately to a finite-width privileged Beta target
# centered near y*, not by point-action NLL. This is the MuJoCo analogue of the
# paper's full-distribution privileged teacher and should preserve more entropy.
import os
import random
import time
...
class Args:
    ...
    """unused in v10.2; retained for CLI compatibility with point-target variants"""
    opsd_kl_clip: float = 2.0
    """per-action-dimension clip for the forward KL contribution before summing"""
    teacher_target_excess_concentration: float = 10.0
    """finite-width y* target Beta uses alpha=1+z*c, beta=1+(1-z)*c"""
...
def finite_width_ystar_beta_target(target_z, args):
    excess_concentration = max(args.teacher_target_excess_concentration, 1e-6)
    target_alpha = 1.0 + target_z * excess_concentration
    target_beta = 1.0 + (1.0 - target_z) * excess_concentration
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                target_alpha, target_beta = finite_width_ystar_beta_target(b_ystar_zs[mb_inds], args)
                teacher_loss = beta_kl_per_dim(
                    target_alpha.detach(),
                    target_beta.detach(),
                    mb_teacher_alpha,
                    mb_teacher_beta,
                ...
                teacher_target_concentrations.append((target_alpha.detach() + target_beta.detach()).mean().item())
                teacher_target_entropies.append(
                    beta_entropy_per_dim(target_alpha.detach(), target_beta.detach()).sum(-1).mean().item()
                )
        ...
        writer.add_scalar("losses/hindsight_teacher_target_kl", np.mean(teacher_losses), global_step)
        writer.add_scalar(
        ...
        writer.add_scalar("debug/teacher_target_concentration_sum", np.mean(teacher_target_concentrations), global_step)
        writer.add_scalar("debug/teacher_target_entropy", np.mean(teacher_target_entropies), global_step)
