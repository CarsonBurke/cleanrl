...
#
# OPSD v29: rank-aware learned full-batch gradient matching.
#
# v29 note: v28 is the strongest learned-distribution variant so far, but it only
# sees advantage magnitude directly. v29 adds advantage rank and positive-advantage
# indicator features, plus a learnable rank-logit skip. This lets the full softmax
# distribution learn v20-like elite sparsity from rank information without a fixed
# hard top-k, while retaining v28's student-gradient alignment objective.
#
# Goal: drive the policy entirely by distillation, with NO PPO clipped surrogate.
#
# Why the earlier PPO-free attempts failed: the pure-distillation line
# (v10/v11-old/v12) removed PPO but had no policy-improvement operator at all --
# forward-KL toward a learned teacher only reproduces/extrapolates that teacher
# and never guarantees mass moves toward higher-advantage actions, so all three
# collapsed (~0 to -200 on HalfCheetah). PPO is one improvement operator but not
# the only one. AWR is another, and it is itself a distillation:
#
#   The exp-advantage tilt of the old policy, pi_T(a|s) ~ pi_old(a|s) exp(A/beta),
#   is the closed-form solution of  max_pi E_pi[A]  s.t.  KL(pi||pi_old) <= eps.
#   Forward-KL of the student toward this analytic teacher reduces to weighted
#   maximum-likelihood on the policy's own on-policy actions:
#       L_actor = - E[ exp(A(s,a)/beta) * log pi_S(a|s) ].
#   No importance ratio, no clipped surrogate -- pure weighted self-distillation,
#   and a provable one-step policy improvement. A critic is still needed to form
#   A (value regression, not PPO).
#
# v13 = v11 with the PPO clipped surrogate replaced by this AWR actor loss. The
# signed hindsight-teacher distillation (positive advantage -> move toward the
# sampled action; negative advantage -> reflect away through the behavior mean)
# is kept as auxiliary dense shaping: it supplies the active repulsion from bad
# actions that AWR alone lacks (AWR can only down-weight bad actions, never push
# away from them), which also counteracts AWR's entropy-shrinking tendency. The
# Dreamer3-bucket HL-Gauss critic and all hindsight machinery match v11. A small
# entropy floor (ent_coef) is added since AWR has no trust-region clip.
import os
import random
import time
...
class Args:
    ...
    """the upper PPO actor ratio clipping coefficient; lower clipping uses clip_coef"""
    clip_vloss: bool = False
    """unused in v9; HL-Gauss value loss is an unclipped categorical CE"""
    ent_coef: float = 0.01
    """coefficient of the entropy (small floor; AWR has no trust-region clip)"""
    vf_coef: float = 0.5
    """coefficient of the value function"""
    weighter_hidden_size: int = 64
    """hidden size for the learned full-minibatch advantage weighter"""
    weighter_adv_logit_init: float = 2.0
    """initial learnable logit scale on normalized advantage before residual weighting"""
    weighter_rank_logit_init: float = 1.0
    """initial learnable logit scale on advantage rank before residual weighting"""
    weighter_learning_rate: float = 3e-4
    """learning rate for the learned sample-weight distribution"""
    weighter_entropy_coef: float = 0.001
    """entropy bonus for the learned sample distribution during gradient matching"""
    weighter_norm_coef: float = 0.05
    """penalty on mismatch between learned weighted-MLE gradient norm and target gradient norm"""
    weighter_score_clip: float = 5.0
    """symmetric clip on learned sample logits before the softmax distribution"""
...
def build_weighter_features(
    ...
    order = torch.argsort(flat_adv)
    ranks = torch.empty_like(flat_adv)
    ranks[order] = torch.linspace(0.0, 1.0, flat_adv.numel(), device=flat_adv.device, dtype=flat_adv.dtype)
    positive_advantage = (advantages > 0.0).to(advantages.dtype)
    teacher_mean = teacher_alpha / (teacher_alpha + teacher_beta)
    teacher_concentration = (teacher_alpha + teacher_beta).mean(-1)
    action_std = action_stds.mean(-1)
    action_residual = ((actions - action_means) / (action_stds + 1e-6)).square().mean(-1).sqrt()
    ystar_shift = (ystar_zs - action_zs).square().mean(-1).sqrt()
    teacher_action_shift = (teacher_mean - action_zs).square().mean(-1).sqrt()
    ...
    return normalize_batch(raw_features).detach()
...
def hindsight_weights(hindsight_advantage, args):
    # Symmetric in |advantage|: both confidently-good and confidently-bad states
    # carry a confident signed y*, so the student should be distilled toward the
    # teacher at both extremes; near-zero (ambiguous) advantage is downweighted.
    adv_z = hindsight_advantage_z(hindsight_advantage)
    weights = torch.exp(adv_z.abs() / args.hindsight_weight_temp)
...
def teacher_beta_nll_weights(hindsight_advantage, args):
    # Symmetric magnitude weighting: with signed y*, a strongly-negative advantage
    # carries an informative reflect-away target just as a strongly-positive one
    # carries a move-toward target, so weight teacher NLL by |advantage| strength.
    adv_z = hindsight_advantage_z(hindsight_advantage)
    abs_strength = torch.tanh(adv_z.abs() / args.ystar_adv_temp)
    weights = args.teacher_negative_anchor_weight + args.teacher_positive_weight_bonus * abs_strength
...
def build_ystar_actions(actions, behavior_mean, hindsight_advantage, args):
    # Signed steering target. strength in (-1, 1):
    #   positive advantage -> y* moves toward the sampled action (it should have
    #     happened);
    #   negative advantage -> y* reflects away from the sampled action through the
    #     behavior mean (the opposite is the should-have action);
    #   near-zero advantage -> y* stays at the behavior mean.
    # The reflected target may fall outside the action bounds; _action_to_z clamps
    # it into the valid latent range downstream, so no NaNs result.
    adv_z = hindsight_advantage_z(hindsight_advantage).reshape(actions.shape[:-1])
    strength = torch.tanh(adv_z / args.ystar_adv_temp).unsqueeze(-1)
...
class Agent(nn.Module):
    ...
    def get_action_params_with_actor_params(self, x, actor_params):
        beta_head = torch.func.functional_call(self.actor_beta, actor_params, (x,)).clone()
        head_alpha, head_beta = beta_head.chunk(2, dim=-1)
        alpha = 1.0 + torch.nn.functional.softplus(head_alpha)
        beta = 1.0 + torch.nn.functional.softplus(head_beta)
        return alpha, beta
    ...
    def get_action_logprob_with_actor_params(self, x, action, actor_params):
        alpha, beta = self.get_action_params_with_actor_params(x, actor_params)
        z = self._action_to_z(action)
        return beta_log_prob_per_dim(alpha, beta, z).sum(1)
...
class AdvantageWeighter(nn.Module):
    def __init__(self, feature_dim, hidden_size, adv_logit_init, rank_logit_init):
        super().__init__()
        self.adv_logit_scale = nn.Parameter(torch.tensor(float(adv_logit_init)))
        self.rank_logit_scale = nn.Parameter(torch.tensor(float(rank_logit_init)))
        self.net = nn.Sequential(
            layer_init(nn.Linear(feature_dim, hidden_size)),
            nn.SiLU(),
            layer_init(nn.Linear(hidden_size, hidden_size)),
            nn.SiLU(),
    ...
    def forward(self, features, args):
        scores = (
            self.adv_logit_scale * features[:, 0]
            + self.rank_logit_scale * features[:, 11]
            + self.net(features).squeeze(-1)
        )
        return scores.clamp(-args.weighter_score_clip, args.weighter_score_clip)
...
if __name__ == "__main__":
    ...
    weighter = AdvantageWeighter(
        feature_dim=13,
        hidden_size=args.weighter_hidden_size,
        adv_logit_init=args.weighter_adv_logit_init,
        rank_logit_init=args.weighter_rank_logit_init,
    ).to(device)
    with torch.no_grad():
    ...
    weighter_optimizer = optim.Adam(weighter.parameters(), lr=args.weighter_learning_rate, eps=1e-5)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.anneal_lr:
            ...
            weighter_optimizer.param_groups[0]["lr"] = frac * args.weighter_learning_rate
        ...
        with torch.no_grad():
            ...
            b_weighter_features = build_weighter_features(
                b_advantages,
                b_returns,
                b_values,
                b_logprobs,
                ...
                b_hindsight_weights,
                b_teacher_weights,
                b_fixed_teacher_alpha,
                b_fixed_teacher_beta,
            )
        ...
        weighter_align_losses = []
        weighter_losses = []
        weighter_entropies = []
        weighter_ess_fracs = []
        weighter_max_weights = []
        weighter_adv_corrs = []
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_features = b_weighter_features[mb_inds]
                mb_advantages = b_advantages[mb_inds]
                if args.norm_adv:
                    mb_advantages = (mb_advantages - mb_advantages.mean()) / (mb_advantages.std() + 1e-8)

                mb_n = mb_features.shape[0]
                match_logprob = agent.get_action_logprob_with_actor_params(
                    mb_obs,
                    mb_actions,
                    dict(agent.actor_beta.named_parameters()),
                )
                match_scores = weighter(mb_features, args)
                match_probs = torch.softmax(match_scores, dim=0)
                match_weights = match_probs * mb_n

                actor_params = list(agent.actor_beta.parameters())
                positive_advantages = mb_advantages.detach().clamp_min(0.0)
                target_loss = -(positive_advantages * match_logprob).mean()
                learned_loss = -(match_weights * match_logprob).mean()
                target_grads = torch.autograd.grad(target_loss, actor_params, retain_graph=True)
                learned_grads = torch.autograd.grad(learned_loss, actor_params, create_graph=True)
                target_vec = torch.cat([grad.detach().flatten() for grad in target_grads])
                learned_vec = torch.cat([grad.flatten() for grad in learned_grads])
                target_norm = target_vec.norm().detach()
                learned_norm = learned_vec.norm()
                align_loss = -(learned_vec * target_vec).sum() / (learned_norm * target_norm + 1e-8)
                norm_loss = (torch.log(learned_norm + 1e-8) - torch.log(target_norm + 1e-8)).square()
                match_entropy = -(match_probs * (match_probs + 1e-8).log()).sum()
                match_entropy = match_entropy / max(np.log(mb_n), 1e-8)
                weighter_loss = align_loss + args.weighter_norm_coef * norm_loss - args.weighter_entropy_coef * match_entropy

                weighter_optimizer.zero_grad()
                weighter_loss.backward()
                nn.utils.clip_grad_norm_(weighter.parameters(), args.max_grad_norm)
                ...
                # Learned full-minibatch weighting distribution: every sample is
                # eligible, weights are nonnegative and sum to minibatch_size. The
                # weighter is optimized above to match a policy-improvement actor
                # gradient; the real actor update treats the weights as fixed.
                with torch.no_grad():
                    awr_scores = weighter(mb_features, args)
                    awr_probs = torch.softmax(awr_scores, dim=0)
                    awr_weights = awr_probs * mb_n
                pg_loss = -(awr_weights.detach() * newlogprob).mean()
                ...
                weighter_align_losses.append(align_loss.item())
                weighter_losses.append(weighter_loss.item())
                weight_entropy = -(awr_probs * (awr_probs + 1e-8).log()).sum()
                weight_entropy = weight_entropy / max(np.log(mb_n), 1e-8)
                weighter_entropies.append(weight_entropy.item())
                weighter_ess_fracs.append((1.0 / (awr_weights.square().mean() + 1e-8)).item())
                weighter_max_weights.append(awr_weights.max().item())
                centered_weights = awr_weights - awr_weights.mean()
                centered_advantages = mb_advantages.detach() - mb_advantages.detach().mean()
                adv_corr = (centered_weights * centered_advantages).mean()
                adv_corr = adv_corr / (centered_weights.std(unbiased=False) * centered_advantages.std(unbiased=False) + 1e-8)
                weighter_adv_corrs.append(adv_corr.item())
        ...
        writer.add_scalar("losses/advantage_weighter_align", np.mean(weighter_align_losses), global_step)
        writer.add_scalar("losses/advantage_weighter_total", np.mean(weighter_losses), global_step)
        writer.add_scalar(
            "debug/value_target_edge_frac",
            ((b_value_target_probs[:, 0] + b_value_target_probs[:, -1]) > 0.05).float().mean().item(),
            global_step,
        )
        writer.add_scalar("debug/clip_coef_high", args.clip_coef_high, global_step)
        ...
        writer.add_scalar("debug/advantage_weighter_entropy", np.mean(weighter_entropies), global_step)
        writer.add_scalar("debug/advantage_weighter_ess_frac", np.mean(weighter_ess_fracs), global_step)
        writer.add_scalar("debug/advantage_weighter_max_weight", np.mean(weighter_max_weights), global_step)
        writer.add_scalar("debug/advantage_weighter_adv_corr", np.mean(weighter_adv_corrs), global_step)
