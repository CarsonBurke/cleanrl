...
#
# OPSD v23: CDF-rank AWR. Weight each transition by Phi^{-1}(F_s(G_t)), where
# F_s is the categorical critic's predicted CDF of returns at state s and G_t
# the realized (GAE) return. This is a per-state-normalized, distribution-aware
# advantage: it removes each state's value scale/offset and is automatically
# bounded and Gaussianized (percentile -> standard-normal quantile). The AWR
# weight becomes exp(z / awr_temp) with this z in place of the z-scored GAE
# advantage; everything else (teacher, entropy floor, value loss) matches v13.
# Hypothesis: a cleaner weighting signal than exp(z-scored GAE), especially
# where critic explained-variance decays and a single global advantage scale
# poorly reflects per-state uncertainty.
#
# OPSD v13: PPO-free actor via Advantage-Weighted Regression + signed hindsight.
#
...
class Args:
    ...
    awr_temp: float = 0.5
    """AWR temperature beta in exp(z / beta); z is the CDF-rank Gaussian advantage"""
    awr_weight_clip: float = 20.0
    """maximum AWR exp-advantage weight, to bound a single sample's influence"""
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # CDF-rank advantage precompute: for each transition, evaluate where the
        # realized return G_t falls in the critic's predicted return distribution
        # at that state (its percentile F_s(G_t)), then map the percentile through
        # the inverse standard-normal CDF to a bounded, per-state-normalized,
        # Gaussian-shaped advantage z = Phi^{-1}(F_s(G_t)). Detached: this is a
        # target weight, no gradient flows through the critic here. Computed in
        # minibatch-sized chunks to bound memory. hl_support.support already holds
        # the raw return-space bucket centers (symexp of the symlog coordinates),
        # so no extra symexp is applied.
        with torch.no_grad():
            centers = hl_support.support.unsqueeze(0)  # (1, num_bins), raw return space
            cdf_advs = []
            for start in range(0, args.batch_size, args.minibatch_size):
                end = start + args.minibatch_size
                probs = torch.softmax(agent.get_value_logits(b_obs[start:end]), dim=-1)  # (n, num_bins)
                G = b_returns[start:end].unsqueeze(1)  # (n, 1)
                cdf = (probs * (centers <= G).float()).sum(dim=-1)  # P(bin_center <= G), (n,)
                q = cdf.clamp(1e-3, 1 - 1e-3)
                z = math.sqrt(2.0) * torch.erfinv(2.0 * q - 1.0)  # Phi^{-1}(q)
                cdf_advs.append(z)
            b_cdf_adv = torch.cat(cdf_advs, dim=0)  # (batch_size,)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # CDF-rank AWR actor loss (no PPO ratio / clipped surrogate):
                # weighted MLE on the policy's own sampled actions, weight =
                # exp(z / beta) with z = Phi^{-1}(F_s(G_t)) the per-state-normalized
                # Gaussian advantage from the critic's predicted return CDF. Weights
                # are detached and clamped so a single sample cannot dominate.
                cdf_adv = b_cdf_adv[mb_inds]
                awr_weights = torch.exp(cdf_adv / args.awr_temp).clamp(max=args.awr_weight_clip)
                awr_weights = awr_weights / (awr_weights.mean() + 1e-8)
        ...
        writer.add_scalar("debug/cdf_adv_std", b_cdf_adv.std().item(), global_step)
