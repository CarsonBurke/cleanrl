...
#
# PPO + Hindsight-Conditioned OPSD Teacher, Beta teacher beta-NLL v6.
#
# Hypothesis:
...
# in the same latent z space. Teacher learning uses Beta NLL with variance.detach()
# ** beta reweighting, and OPSD distillation uses closed-form KL(T_Beta || S_Beta).
# The student never sees the hindsight context.
import os
import random
import time
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                student_dist, _, student_std = agent.get_action_distribution(b_obs[mb_inds])
                ...
                teacher_dist_fixed = Beta(b_fixed_teacher_alpha[mb_inds], b_fixed_teacher_beta[mb_inds])
                hindsight_kl = kl.kl_divergence(teacher_dist_fixed, student_dist).sum(-1)
                ...
                mb_teacher_dist = Beta(mb_teacher_alpha, mb_teacher_beta)
                beta_nll = beta_teacher_nll(mb_teacher_dist, b_ystar_zs[mb_inds], args.teacher_beta_nll_beta)
                ...
                teacher_latent_errors.append((mb_teacher_dist.mean.detach() - b_ystar_zs[mb_inds]).abs().mean().item())
                teacher_variances.append(mb_teacher_dist.variance.detach().mean().item())
                teacher_concentrations.append((mb_teacher_alpha.detach() + mb_teacher_beta.detach()).mean().item())
                student_concentrations.append(
                    (student_dist.concentration1.detach() + student_dist.concentration0.detach()).mean().item()
                )
