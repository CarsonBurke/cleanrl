...
#
# PPO + Hindsight-Conditioned OPSD Teacher, beta-NLL v2.
#
# Hypothesis:
...
# distribution is frozen once per rollout and distilled with KL(T || S). The
...
class Args:
    ...
    """scale on the behavior residual when forming y* = old_mean + scale * tanh(A*) * (a - old_mean)"""
    teacher_weight_min: float = 0.25
...
def teacher_beta_nll_weights(hindsight_advantage, args):
    adv_z = hindsight_advantage_z(hindsight_advantage)
    strength = torch.tanh(adv_z / args.ystar_adv_temp).abs()
    weights = 0.5 + strength
...
class Agent(nn.Module):
    ...
    def get_action_distribution(self, x):
        action_mean = self.actor_mean(x).clone()
