# Pan Goal Solver v15: rate-conditioned goal generator (learned inverse proposer).
#
# A reward-free LeJEPA learns observation tokens and temporal control belief.
# Pan hindsight and occupancy ground the follower in real trajectories. The
# proposer is the learned inverse map P(y) -> G: supervised regression from a
# state's sustained windowed reward rate y to its target-encoded goal latent,
# trained only on achieved (rate, latent) pairs. This inverts the failure mode
# of v11 (argmax/ascent over a learned scorer U(G), which needs accurate
# off-support values and invites exploitation): P is fit on-support and merely
# QUERIED above support, at y* = q99 + margin * (q99 - q50) of the current
# label distribution. Extrapolation is of a learned trend, self-correcting as
# faster real data lands in the previously extrapolated range; there is no
# gradient through any reward model and no fluke-anchor lock-in (quantiles
# track the live buffer). A detached rate head R(G) remains a diagnostics
...
# Proposer contract (FAMILY.md): input = aspiration coordinate y only (no
# embodied belief, so the ideal cannot chase the current state); objective =
# supervised latent MSE on achieved pairs; aspiration variable = the reward
# rate query y*; safeguards = on-support training, quantile-relative query,
# monotonicity sweep, and P(q99)-vs-P(y*) A/B evaluation arms.
# Hypothesis: the learned y -> G trend extrapolates a small margin beyond the
# achieved rate distribution, giving directed, self-renewing speed pressure.
...
class Args:
    ...
    goal_generator_coef: float = 1.0
    # Linear-in-rank upweight of high-rate samples in the generator loss: the
    # top of the batch gets (1 + this) times the weight of the bottom, so the
    # region P is actually queried in (upper tail) is not fit from ~1% of the
    # signal. Rank-based, so it is reward-scale free.
    generator_rank_weight: float = 3.0
    # Sustained reward-rate window for proposer labels and the rate head. Long
    # enough to span several gait cycles so transient bursts score poorly.
    anchor_reward_window: int = 100
    # Aspiration query: y* = q99 + margin * (q99 - q50) of the current label
    # distribution, so the commanded rate stays a distribution-relative step
    # beyond demonstrated capability regardless of reward scale or sign.
    aspiration_quantile_margin: float = 0.3
...
class GoalGenerator(nn.Module):
    """Learned inverse proposer P(y) -> G: map a sustained reward-rate query to
    the goal latent of states that achieve it. Trained by supervised MSE on
    achieved pairs only; the aspirational query above support is extrapolation
    of this trend, never the argmax of a learned scorer."""

    def __init__(self, dim):
        super().__init__()
        self.net = nn.Sequential(
            xavier_linear(nn.Linear(1, 256)),
            nn.SiLU(),
            xavier_linear(nn.Linear(256, 256)),
            nn.SiLU(),
            xavier_linear(nn.Linear(256, dim)),
        )

    def forward(self, rate):
        return self.net(torch.asinh(rate).unsqueeze(-1))
...
class PanGoalSolver(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.goal_generator = GoalGenerator(args.latent_dim)
    ...
    @torch.no_grad()
    def propose_goals(self, rate_queries, device):
        """One P(y) call per rate query; returns stacked goal latents."""
        queries = torch.as_tensor(rate_queries, device=device, dtype=torch.float32)
        return self.goal_generator(queries)
    ...
    def parameter_groups(self):
        ...
        proposer = list(self.goal_generator.parameters()) + list(
            self.reward_predictor.parameters()
        )
        ids = [{id(p) for p in group} for group in (world, follower, proposer)]
        if any(ids[i] & ids[j] for i in range(3) for j in range(i + 1, 3)):
            raise RuntimeError("world, follower, and proposer parameters must be disjoint")
        return world, follower, proposer
...
class VectorReplayBuffer:
    ...
    def sample_transitions(self, batch_size, history, rng, reward_window=0):
        ...
        if reward_window > 0:
            ...
            # pre-fall regime from both the rate head and the generator,
            # which then overestimate unstable fast gaits; revisit before
            # trusting the aspiration sweep off HalfCheetah.
    ...
    def best_windowed_anchor(self, window, history):
        """Scan all states whose obs history and reward window sit fully inside
        one episode. Returns (best rate, its raw obs history, q50, q99, max) of
        the sustained-rate distribution, or None."""
        ...
        for env in range(self.num_envs):
            ...
            if not valid.any():
                continue
            valid_rates.append(rates[valid])
        ...
        if best_history is None:
            return None
        all_rates = np.concatenate(valid_rates)
        q50, q99 = np.quantile(all_rates, (0.5, 0.99))
        return best_rate, best_history, float(q50), float(q99), float(all_rates.max())
...
# Sweep-point names for goal-semantics diagnostics: the rate probe's score at
# P(y) should rise monotonically across these queries while the generator's
# extrapolation holds; where it breaks marks the usable aspiration range.
SWEEP_NAMES = ("q50", "mid", "q99", "ystar", "beyond")
...
def train_proposer_step(agent, reward_optimizer, generator_optimizer, batch, args, device):
    """One shared batch trains both proposer heads on achieved data only:
    the rate probe R(G) (diagnostics) and the inverse generator P(y) -> G.
    Nothing in the world model or follower differentiates through either, and
    P never receives a gradient from R."""
    obs = as_tensor(batch["obs"], device)
    rate = as_tensor(batch["reward_rate"], device)
    rate_valid = as_tensor(batch["reward_rate_valid"], device)
    with torch.no_grad():
        _, goal_latent = agent.target_encode(obs)
    normalizer = rate_valid.sum().clamp_min(1.0)

    reward_prediction = agent.reward_predictor(goal_latent)
    reward_loss = ((reward_prediction - rate).square() * rate_valid).sum() / normalizer
    ...
    generated = agent.goal_generator(rate)
    ranks = torch.argsort(torch.argsort(rate)).float() / max(rate.shape[0] - 1, 1)
    generator_weight = (1.0 + args.generator_rank_weight * ranks) * rate_valid
    generator_loss = (
        (generated - goal_latent).square().mean(-1) * generator_weight
    ).sum() / generator_weight.sum().clamp_min(1e-6)
    generator_optimizer.zero_grad(set_to_none=True)
    (args.goal_generator_coef * generator_loss).backward()
    generator_grad_norm = nn.utils.clip_grad_norm_(
        agent.goal_generator.parameters(), args.max_grad_norm
    )
    ...
    return {
        ...
        "goal/generator_mse": generator_loss.item(),
        "goal/generator_grad_norm": float(generator_grad_norm),
...
def main():
    ...
    if args.aspiration_quantile_margin < 0:
        raise ValueError("aspiration quantile margin must be non-negative")
    ...
    if base_model is None or not hasattr(base_model, "nv"):
        raise TypeError("the forward-velocity diagnostic requires a MuJoCo env exposing model.nv")
    ...
    # No weight decay: decay pulls the sparse upper-tail fit toward zero,
    # exactly where the aspirational query reads P.
    generator_optimizer = torch.optim.AdamW(
        agent.goal_generator.parameters(),
        lr=args.learning_rate,
        weight_decay=0.0,
    )
    if args.compile:
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        # Refresh label quantiles and regenerate both goals AFTER the training
        # block: the EMA target encoder and P are frozen until the next block,
        # so these latents are exact for the upcoming rollout, the diagnostics
        # below, and eval alike (no snapshot mixing). Quantiles track the live
        # buffer, so there is no fluke-anchor lock-in; a transient regression
        # of q99 simply lowers the query until faster data returns.
        scan = replay.best_windowed_anchor(args.anchor_reward_window, args.history)
        if scan is not None:
            best_rate, best_history, rate_q50, rate_q99, rate_max = scan
            # The margin is applied in asinh space — the coordinate P actually
            # conditions on — so the aspiration step does not shrink toward
            # zero effective input distance as raw rates grow.
            a50, a99 = math.asinh(rate_q50), math.asinh(rate_q99)
            a_spread = a99 - a50
            a_star = a99 + args.aspiration_quantile_margin * a_spread
            y_star = math.sinh(a_star)
            sweep_queries = (
                rate_q50,
                math.sinh(0.5 * (a50 + a99)),
                rate_q99,
                y_star,
                math.sinh(a_star + args.aspiration_quantile_margin * a_spread),
            )
            with torch.no_grad():
                sweep_goals = agent.propose_goals(sweep_queries, device)
                sweep_rates = agent.reward_predictor(sweep_goals)
            base_goal, commanded_goal = sweep_goals[2], sweep_goals[3]
            for name, value in zip(SWEEP_NAMES, sweep_rates.tolist()):
                writer.add_scalar(
                    f"diagnostics/generator_predicted_rate_{name}", value, global_step
                )
            writer.add_scalar("goal/rate_q50", rate_q50, global_step)
            writer.add_scalar("goal/rate_q99", rate_q99, global_step)
            writer.add_scalar("goal/rate_max", rate_max, global_step)
            writer.add_scalar("goal/aspiration_query", y_star, global_step)
            writer.add_scalar("goal/anchor_buffer_best_rate", best_rate, global_step)
            writer.add_scalar(
                "goal/anchor_forward_velocity",
                float(best_history[:, -velocity_dims].mean()),
                global_step,
            )
            writer.add_scalar(
        ...
        if global_step >= args.random_warmup_steps and base_goal is not None:
            with torch.no_grad():
                diagnostic_obs = as_tensor(obs_history, device)
                diagnostic_incoming = as_tensor(incoming_history, device)
                diagnostic_belief, diagnostic_current_goal, _, _ = agent.follower_state(
                ...
                goal_action = agent.follower(
                    diagnostic_belief, diagnostic_current_goal, commanded
                )
                base_action = agent.follower(
                    diagnostic_belief, diagnostic_current_goal, base
                )
                # With goal_features(g, g) = 0 the null-goal action is exactly
                # zero, so goal-conditioned action power is the removal test.
                writer.add_scalar(
                    "diagnostics/goal_action_power",
                    goal_action.square().mean().item(),
                    global_step,
                )
                # Sensitivity of actions to aspiration alone (P(y*) vs P(q99)):
        ...
        if args.eval_interval > 0 and global_step >= next_eval_step and global_step >= args.random_warmup_steps:
            ...
            # A/B the aspiration itself: the same policy commanded P(q99),
            # the support edge. The commanded-minus-base gap is the direct
            # evidence for or against above-support extrapolation of P.
