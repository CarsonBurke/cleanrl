# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + RELATIVE-SURPRISAL DELIGHT GATE v13.
#
# WHY v13. The delight gate as a drop-in REPLACEMENT for the policy-gradient
# term inside PPO — on-policy, no aux losses, no replay, no EMAs.
#   ell_rel = clip(logpi_old(a|s) - logpi_theta(a|s), +-C)   (= -log ratio)
#   w = sigmoid(U * ell_rel / eta), detached;  U = minibatch-normalized GAE
...
# eta: gate transition width in logratio is eta/|U|; eta=0.1 saturates near
# PPO's clip boundary (primary arm), eta=0.3 ~ drift-matched to PPO (arm 2).
# target_kl 0.03 kept as carrier-standard insurance (gate self-throttles
# before it fires at eta=0.1). Health signatures (log + watch):
#   relgate_epoch0 ~ 0.5 always; relgate_epochN falling to 0.25-0.40 healthy;
#   pinned at 0.5 => eta too big (half-A2C); ~0 by epoch 3 => eta too small.
...
class Args:
    ...
    dg_eta: float = 0.1   # v13 relgate temperature (1.0 => gate inert at logratio scale)
