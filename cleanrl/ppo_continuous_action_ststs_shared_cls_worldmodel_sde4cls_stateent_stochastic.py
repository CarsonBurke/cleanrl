# PPO + Stochastic world model + SDE + state entropy bonus.
#
# Fork of worldmodel_sde4cls_stateent with stochastic transition model.
# The transition predicts Gaussian (mean, log_var) over next latent instead
# of a deterministic point prediction. During imagination, next latents are
# SAMPLED via reparameterization — different rollouts from the same seed
# diverge, forcing the actor to optimize expected return across possible
# futures. Trained with Gaussian NLL instead of MSE.
import os
import random
import time
...
class STSTSCLSBackbone(nn.Module):
    def __init__(self, obs_dim, context_len):
        ...
        self.state_pred_cls_index = obs_dim + 4
    ...
    def forward(self, obs_seq):
        ...
        cls_tokens = torch.stack([self.actor_cls, self.critic_cls, self.dynamics_cls, self.sde_cls, self.state_pred_cls], dim=0)
        ...
        return actor_cls, critic_cls, dynamics_cls, sde_cls, state_pred_cls
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # Stochastic transition: predicts Gaussian (mean, log_var) over next latent
        self.transition_trunk = nn.Sequential(
            nn.Linear(EMBED_DIM + action_dim, 256), nn.SiLU(),
            nn.Linear(256, 256), nn.SiLU(),
        )
        self.transition_mean = nn.Linear(256, EMBED_DIM)
        self.transition_log_var = nn.Linear(256, EMBED_DIM)
        nn.init.zeros_(self.transition_log_var.weight)
        nn.init.constant_(self.transition_log_var.bias, -2.0)  # init to low variance
    ...
    def _transition(self, za, sample=True):
        """Stochastic transition. Returns (next_z, mean, log_var).
        If sample=False, returns deterministic mean (for debugging)."""
        h = self.transition_trunk(za)
        mean = self.transition_mean(h)
        log_var = self.transition_log_var(h).clamp(-10.0, 2.0)
        if sample:
            std = (0.5 * log_var).exp()
            next_z = mean + std * torch.randn_like(mean)  # reparameterization
        else:
            next_z = mean
        return next_z, mean, log_var
    ...
    def get_value(self, obs_seq):
        _, critic_latent, _, _, _ = self._encode(obs_seq)
        return self.critic(critic_latent)

    def get_action_and_value(self, obs_seq, action=None):
        actor_latent, critic_latent, dynamics_latent, sde_latent, state_pred_latent = self._encode(obs_seq)
        action_mean = self.actor_mean(actor_latent)
        action_std = self._get_action_std(sde_latent)
        probs = Normal(action_mean, action_std)
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), self.critic(critic_latent), dynamics_latent, state_pred_latent
    ...
    def get_all_for_update(self, obs_seq, action):
        ...
        action_std = self._get_action_std(sde_latent)
        probs = Normal(action_mean, action_std)
        return probs.log_prob(action).sum(1), probs.entropy().sum(1), self.critic(critic_latent), dynamics_latent, state_pred_latent
    ...
    def imagine(self, z_start, horizon, gamma, gae_lambda):
        ...
        for _ in range(horizon):
            ...
            z, _, _ = self._transition(za, sample=True)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                newlogprob, entropy, newvalue, dynamics_latent, state_pred_latent = agent.get_all_for_update(b_obs_seqs[mb_inds], b_actions[mb_inds])
                ...
                # World model loss on fresh latents (stochastic transition)
                with torch.no_grad():
                    _, _, next_dynamics, _, _ = agent._encode(b_next_obs_seqs[mb_inds])
                mb_za = torch.cat([dynamics_latent, b_actions[mb_inds]], dim=-1)
                _, t_mean, t_log_var = agent._transition(mb_za, sample=False)
                t_var = t_log_var.exp()
                ...
                t_nll = F.gaussian_nll_loss(t_mean, next_dynamics, var=t_var, reduction='none')
                transition_loss = (t_nll.mean(-1) * mb_valid).sum() / (mb_valid.sum() + 1e-8)
        ...
        with torch.no_grad():
            _, _, z_seeds, _, _ = agent._encode(b_obs_seqs[seed_inds])
        ...
        with torch.no_grad():
            ...
            _, _, _, _, sp_latent = agent._encode(agent.obs_history)
            ...
            writer.add_scalar("worldmodel/transition_log_var_mean", t_log_var.mean().item(), global_step)
