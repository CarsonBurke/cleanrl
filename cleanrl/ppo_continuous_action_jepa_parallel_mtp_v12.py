# Direct parallel multi-target LeJEPA v12, cloned from frozen recurrent v9.
...
# Each horizon predicts directly from current projected64 plus its ordered action prefix.
# Horizon-specific action encoders add capacity; the shared deep predictor is batched
# across horizons, with no predicted-latent recurrence or cross-horizon dependency.
...
class LeWMBranch(nn.Module):
    """Direct horizon predictions share one batched predictor, not recurrent states.

    Ordered-prefix encoders distinguish horizons. Their initialization is RNG
    isolated; H1 and task/encoder initialization retain the recurrent control.
    Only current and factual H1 participate in the unchanged SIGReg statistic.
    """
    ...
    def __init__(self, action_dim, num_proj, proj_chunk, prediction_loss,
        ...
        with torch.random.fork_rng(devices=[]):
            self.prefix_encoders = nn.ModuleList(
                ActionEncoder(action_dim * horizon, 64) for horizon in prediction_horizons[1:]
            )
    ...
    def forward(self, current, following, native_actions, validity=None):
        ...
        if self.prediction_horizons == (1,):
            ...
            for index, horizon in enumerate(self.prediction_horizons):
                prefix = torch.where(validity[:, index, None, None],
                                     native_actions[:, :horizon].detach(), 0.5)
                encoder = self.action_encoder if index == 0 else self.prefix_encoders[index - 1]
                conditions.append(encoder((2.0 * prefix - 1.0).flatten(1)))
            conditions = torch.stack(conditions, dim=1)
            current_states = embeddings[0, :, None].expand_as(conditions)
            predictions = self.pred_proj(self.predictor(
                current_states.reshape(-1, 64), conditions.reshape(-1, 64),
...
def main():
    ...
    try:
        ...
        writer.add_text("ablation", f"{args.task_activation=}, {args.task_residual=}, "
                        ...
                        f"{args.prediction_horizons=}; direct parallel horizons with prefix encoders; fixed SSL512; predictions auxiliary only; "
