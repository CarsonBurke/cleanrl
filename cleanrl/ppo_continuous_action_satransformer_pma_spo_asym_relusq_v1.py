...
#   embed RMSNorm
#   1 self-attention block over obs tokens (RoPE, QK-norm, ReluSq² FFN, pre-norm,
...
#                                  (QK-norm, ReluSq² FFN, pre-norm, no RoPE,
#                                   learnable scalar residual gates)
#
# Sibling of satransformer_pma_spo_asym_v1 (SwiGLU); same FFN_MULT=2 to
# isolate the activation as the only variable. ReluSq² is non-gated
# (2 projections vs SwiGLU's 3), so this variant has fewer FFN params
# (2·d·ffn_dim vs 3·d·ffn_dim) and slightly less compute per FFN.
#   final RMSNorm
...
class SABlock(nn.Module):
    """Pre-norm SA + ReluSq² FFN (non-gated, w2(relu(w1(x))²)) over obs tokens.
    RoPE on Q/K. QK-norm. Learnable scalar gate per residual ('residual mixing')."""
    ...
    def __init__(self, dim, num_heads, ffn_mult, init_scale):
        ...
        for module in (self.wq, self.wk, self.wv, self.w1):
    ...
    def forward(self, x, rope_cos, rope_sin):
        ...
        h = self.ffn_norm(x)
        ffn_out = self.w2(F.relu(self.w1(h)).square())
...
class PMABlock(nn.Module):
    ...
    ReluSq² FFN on the seed stream. Learnable scalar residual gates."""
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic_head = layer_init(nn.Linear(MODEL_DIM, 1), std=1.0)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        return env_action, z, log_prob, entropy, value, std
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                env_action, z, logprob, _, value, _ = agent.get_action_and_value(next_obs)
