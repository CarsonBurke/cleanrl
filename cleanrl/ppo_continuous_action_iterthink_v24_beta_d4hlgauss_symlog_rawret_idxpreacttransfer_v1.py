# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic idx-preact-transfer v1.
#
...
#
# Index-preactivation-transfer variant: keep the existing DenseNet-style trunk
# concatenation and residual branch sum, but apply ReLU^2 before the historical
# transfer sum. For hidden channel j, the branch forms
#   relusq(current_linear(x)[j]) + sum_slot a[slot,j] * relusq(cat_feats[slot,j])
# before the branch output projection. There is no full cross-channel history
# projection inside the transfer path.
#
...
class IndexedPreActivationTransferBranch(nn.Module):
    ...
    def forward(self, x, history):
        current = self.act(self.current_linear(x))
        history = history.reshape(history.shape[0], self.history_slots, self.H)
        activated_history = self.act(history)
        same_index_transfer = (
            activated_history * self.history_weight.to(dtype=activated_history.dtype).unsqueeze(0)
        ).sum(dim=1)
        return self.out_linear(current + same_index_transfer)
...
class ThinkBlock(nn.Module):
    ...
    def __init__(self, in_dim, H, n_experts):
        ...
        self.dense = IndexedPreActivationTransferBranch(H, in_dim)

        # Soft MoE branch (softmax over all experts, no top-K).
        self.moe_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.gate = layer_init(nn.Linear(H, n_experts))
        self.experts = nn.ModuleList(
            [IndexedPreActivationTransferBranch(H, in_dim) for _ in range(n_experts)]
        )
