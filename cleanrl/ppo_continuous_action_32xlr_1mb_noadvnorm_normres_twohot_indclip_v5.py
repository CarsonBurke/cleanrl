# Pre-RMS SiTU-GLU PPO, v5: restored value-loss clipping for scalar MSE controls.
# Two-hot cross-entropy stays unclipped; identical trunks and zero bin head.
# Each network has its own 0.5 norm budget; joint Adam moments remain per parameter.
# Optional reward normalization; no return-target norm or critic EMA.
# Raw-space two-hot labels/expectation; uniform or symexp support geometry.
# Hypothesis: independent budgets remove actor/critic clipping interference.
import os
import random
import time
...
class Args:
    ...
    """toggle PPO value-loss clipping for scalar MSE only; ignored by two-hot CE"""
    ent_coef: float = 0.0
    """coefficient of the entropy"""
    vf_coef: float = 0.5
    """coefficient of the value function"""
    max_grad_norm: float = 0.5
    """independent maximum gradient norm for each of actor and critic"""
    target_kl: float | None = None
    """the target KL divergence threshold"""

    value_loss: Literal["twohot", "mse"] = "twohot"
    """plain two-hot cross-entropy or scalar half-MSE with optional PPO value clipping"""
...
def clip_gradients(actor_parameters, critic_parameters, max_grad_norm):
    """Clip disjoint networks independently; return their weighted preclip norms."""
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, targets, old_values, args):
    """PPO with plain two-hot CE or optionally clipped scalar MSE on fixed targets."""
