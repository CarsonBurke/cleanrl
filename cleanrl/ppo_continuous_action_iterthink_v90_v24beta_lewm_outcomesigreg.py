# PPO + IterThink v90 (v24 Beta + LeWM outcome SIGReg). From v89.
#
...
class Args:
    ...
    lewm_value_loss_coef: float = 0.25
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.value_v_min = args.v_min
        self.value_v_max = args.v_max
        self.value_sigma_ratio = 0.5
        self.num_outcome_tokens = 3
        ...
        self.reward_v_min = args.lewm_reward_v_min
        self.reward_v_max = args.lewm_reward_v_max
        self.reward_sigma_ratio = 0.5
        ...
        self.value_outcome_proj = xavier_linear(nn.Linear(self.value_num_bins, self.dim))
        ...
        self.value_outcome_unproj = xavier_linear(nn.Linear(self.dim, self.value_num_bins))
    ...
    def neutral_reward_probs(self, batch, device, dtype):
        ...
        sigma = self.reward_sigma_ratio * bin_width
        half_w = bin_width / 2.0
        target = torch.zeros(batch, 1, device=device, dtype=dtype)
        upper = (support.view(1, -1) + half_w - target) / sigma
        lower = (support.view(1, -1) - half_w - target) / sigma
        probs = 0.5 * (torch.erf(upper / np.sqrt(2.0)) - torch.erf(lower / np.sqrt(2.0)))
    ...
    def neutral_value_probs(self, batch, device, dtype):
        support = torch.linspace(self.value_v_min, self.value_v_max, self.value_num_bins, device=device, dtype=dtype)
        bin_width = (self.value_v_max - self.value_v_min) / (self.value_num_bins - 1)
        sigma = self.value_sigma_ratio * bin_width
        half_w = bin_width / 2.0
        target = torch.zeros(batch, 1, device=device, dtype=dtype)
        upper = (support.view(1, -1) + half_w - target) / sigma
        lower = (support.view(1, -1) - half_w - target) / sigma
    ...
    def encode_summary_with_outcomes(self, obs, reward_probs, continuations, value_probs=None):
        batch = obs.shape[0]
        if value_probs is None:
            value_probs = self.neutral_value_probs(batch, obs.device, obs.dtype)
        ...
        value_token = self.value_outcome_proj(value_probs.to(obs_tokens.dtype)).unsqueeze(1)
        return torch.cat([obs_tokens, reward_token, continuation_token, value_token], dim=1)
    ...
    def decode_value(self, summary, detach_summary=True):
        if detach_summary:
            summary = summary.detach()
        value_token = summary[:, self.num_obs_tokens + 2]
        return self.value_outcome_unproj(value_token)
    ...
    def imagine_step(self, summary, action):
        features = self._predictor_trunk(summary.unsqueeze(1), action.unsqueeze(1))[:, 0]
        next_summary = self.pred_next_proj(features)
        reward_logits, continuation_logits = self.decode_outcomes(next_summary, detach_summary=False)
        value_logits = self.decode_value(next_summary, detach_summary=False)
        return next_summary, reward_logits, continuation_logits, value_logits
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.wm_adapter_scale = args.wm_adapter_scale
        ...
        self.agent_latent_dim = self.world_model.num_latent_tokens * self.world_model.dim
        self.agent_input_dim = self.agent_latent_dim
        self.wm_adapter_norm = nn.RMSNorm(self.agent_latent_dim)
        self.wm_to_obs = nn.Linear(self.agent_latent_dim, obs_dim)
        nn.init.zeros_(self.wm_to_obs.weight)
        nn.init.zeros_(self.wm_to_obs.bias)
    ...
    def _obs_with_wm_residual(self, obs, agent_input):
        obs_flat = obs.reshape(obs.shape[0], -1)
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        wm_residual = self.wm_adapter_scale * self.wm_to_obs(self.wm_adapter_norm(agent_input))
        return obs_flat + wm_residual

    def _trunks_from_augmented_obs(self, augmented_obs):
        # Return (actor_feat, critic_feat). When sharing, the SAME trunk output
        # is fed to both heads, computed only once.
        if self.share_backbone:
            feat = self.trunk(augmented_obs)
            return feat, feat
        return self.actor_trunk(augmented_obs), self.critic_trunk(augmented_obs)

    def _trunks_from_agent_input(self, obs, agent_input):
        return self._trunks_from_augmented_obs(self._obs_with_wm_residual(obs, agent_input))
    ...
    def actor_parameters(self):
        ...
        adapter = list(self.wm_adapter_norm.parameters()) + list(self.wm_to_obs.parameters())
        return adapter + list(trunk.parameters()) + heads

    def critic_parameters(self):
        # Params receiving the VALUE gradient (incl. the shared trunk).
        trunk = self.trunk if self.share_backbone else self.critic_trunk
        adapter = list(self.wm_adapter_norm.parameters()) + list(self.wm_to_obs.parameters())
        return adapter + list(trunk.parameters()) + list(self.critic_head.parameters())
...
if __name__ == "__main__":
    ...
    wm_probe_params = (
        list(agent.world_model.reward_outcome_unproj.parameters())
        + list(agent.world_model.continuation_outcome_unproj.parameters())
        + list(agent.world_model.value_outcome_unproj.parameters())
    )
    wm_probe_param_ids = {id(p) for p in wm_probe_params}
    wm_latent_params = [p for p in world_model_params if id(p) not in wm_probe_param_ids]
    ...
    def value_probs_to_scalar(probs):
        ...
        if args.value_symlog:
            value = value.sign() * (value.abs().exp() - 1.0)
        return value
    ...
    def masked_token_sigreg(token_latents, token_valids):
        ...
        return sigreg(valid_tokens, generator=wm_torch_generator)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            current_prev_reward_probs = transition_reward_probs
            current_prev_continues = transition_continues
        ...
        wm_value_losses = []
        wm_sigreg_losses = []
        wm_obs_sigreg_losses = []
        wm_outcome_sigreg_losses = []
        wm_grad_norms = []
        wm_probe_grad_norms = []
        ...
        wm_imagine_latent_mse_sum = torch.zeros((), device=device)
        wm_imagine_reward_abs_error_sum = torch.zeros((), device=device)
        wm_imagine_reward_error_sum = torch.zeros((), device=device)
        wm_imagine_continuation_bce_sum = torch.zeros((), device=device)
        wm_imagine_probe_weight = torch.zeros((), device=device)
        wm_value_abs_error_sum = torch.zeros((), device=device)
        wm_value_probe_weight = torch.zeros((), device=device)
        wm_imagine_value_abs_error_sum = torch.zeros((), device=device)
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    safe_next_step_inds = (hist_step_inds + 1).clamp(max=max_start)
                    next_in_rollout = hist_step_inds + 1 < args.num_steps
                    next_value_target_probs = target_probs[safe_next_step_inds, env_inds]
                    bootstrap_value_target_probs = next_transition_value_probs[safe_hist_step_inds, env_inds]
                    terminal_zero_value_probs = hl_support.project(
                        torch.zeros(mb_size * horizon, device=device)
                    ).reshape(mb_size, horizon, -1)
                    value_target_probs = torch.where(
                        next_in_rollout.unsqueeze(-1),
                        next_value_target_probs,
                        bootstrap_value_target_probs,
                    )
                    value_target_probs = torch.where(
                        future_boundaries.bool().unsqueeze(-1),
                        bootstrap_value_target_probs,
                        value_target_probs,
                    )
                    value_target_probs = torch.where(
                        future_terminations.bool().unsqueeze(-1),
                        terminal_zero_value_probs,
                        value_target_probs,
                    )
                    value_target_probs_flat = value_target_probs.reshape(-1, args.num_bins)
                    ...
                    # Value labels are bootstrap targets, not observed transition facts.
                    # Keep them in the supervised target, but do not leak oracle values
                    # through the teacher-forced predictor context.
                    teacher_summaries = target_summaries.detach().clone()
                    neutral_teacher_values = agent.world_model.neutral_value_probs(
                        mb_size * horizon,
                        target_summaries.device,
                        target_summaries.dtype,
                    )
                    neutral_teacher_value_tokens = agent.world_model.value_outcome_proj(
                        neutral_teacher_values
                    ).reshape(mb_size, horizon, 1, agent.world_model.dim)
                    value_token_idx = agent.world_model.num_obs_tokens + 2
                    teacher_summaries[:, :, value_token_idx : value_token_idx + 1] = (
                        neutral_teacher_value_tokens.detach()
                    )
                    teacher_history = torch.cat(
                        [initial_summary.unsqueeze(1), teacher_summaries[:, :-1]], dim=1
                    ).detach()
                    ...
                    with torch.no_grad():
                        one_step_weight = latent_weight[:, 0]
                        (
                            imagine_next_summary,
                            imagine_reward_logits,
                            imagine_continuation_logits,
                            imagine_value_logits,
                        ) = (
                            agent.world_model.imagine_step(initial_summary.detach(), future_actions[:, 0])
                        )
                        imagine_latent_mse = F.mse_loss(
                            imagine_next_summary,
                            target_summaries[:, 0],
                            reduction="none",
                        ).mean(dim=(-1, -2))
                        imagine_reward_scalar = reward_support.to_scalar(imagine_reward_logits)
                        imagine_reward_error = imagine_reward_scalar - future_rewards[:, 0]
                        imagine_continuation_bce = F.binary_cross_entropy_with_logits(
                            imagine_continuation_logits,
                            1.0 - future_terminations[:, 0],
                            reduction="none",
                        )
                        imagine_value_scalar = hl_support.to_scalar(imagine_value_logits)
                        imagine_value_target_scalar = value_probs_to_scalar(value_target_probs[:, 0].detach())
                        wm_imagine_latent_mse_sum += (imagine_latent_mse * one_step_weight).sum()
                        wm_imagine_reward_abs_error_sum += (
                            imagine_reward_error.abs() * one_step_weight
                        ).sum()
                        wm_imagine_reward_error_sum += (imagine_reward_error * one_step_weight).sum()
                        wm_imagine_continuation_bce_sum += (
                            imagine_continuation_bce * one_step_weight
                        ).sum()
                        wm_imagine_value_abs_error_sum += (
                            (imagine_value_scalar - imagine_value_target_scalar).abs() * one_step_weight
                        ).sum()
                        wm_imagine_probe_weight += one_step_weight.sum()
                    reward_target_probs = reward_target_probs_flat.reshape(mb_size, horizon, -1)
                    # Detached probes: train outcome decoders for diagnostics/readout
                    # without turning CE/BCE into a second latent-shaping objective.
                    ...
                    pred_value_logits_all = agent.world_model.decode_value(
                        pred_mtp.reshape(
                            mb_size * horizon * mtp_len,
                            agent.world_model.num_latent_tokens,
                            agent.world_model.dim,
                        ...
                        detach_summary=True,
                    ).reshape(mb_size, horizon, mtp_len, -1)
                    ...
                    target_value_logits = agent.world_model.decode_value(
                        target_summaries.reshape(
                            mb_size * horizon,
                            agent.world_model.num_latent_tokens,
                            agent.world_model.dim,
                    ...
                    for mtp_idx in range(mtp_len):
                        ...
                        value_loss = -(
                            value_target_probs[:, mtp_idx:].detach()
                            * torch.log_softmax(pred_value_logits_all[:, :valid_horizon, mtp_idx], dim=-1)
                        ).sum(dim=-1)
                        value_losses.append((value_loss * offset_valid).sum() / denom)
                        with torch.no_grad():
                            ...
                            pred_value_scalar = hl_support.to_scalar(
                                pred_value_logits_all[:, :valid_horizon, mtp_idx]
                            )
                            target_value_scalar = value_probs_to_scalar(
                                value_target_probs[:, mtp_idx:].detach()
                            )
                            wm_value_abs_error_sum += (
                                (pred_value_scalar - target_value_scalar).abs() * offset_valid
                            ).sum()
                            wm_value_probe_weight += offset_valid.sum()
                    ...
                    wm_pred_value_loss = torch.stack(value_losses).mean()
                    ...
                    target_value_loss = (target_value_loss * latent_weight).sum() / target_denom
                    ...
                    wm_value_loss = 0.5 * (wm_pred_value_loss + target_value_loss)
                    wm_obs_sigreg_loss = masked_token_sigreg(
                        target_summaries[:, :, : agent.world_model.num_obs_tokens],
                        latent_weight,
                    )
                    wm_outcome_sigreg_loss = masked_token_sigreg(
                        target_summaries[:, :, agent.world_model.num_obs_tokens :],
                        latent_weight,
                    )
                    wm_sigreg_loss = wm_obs_sigreg_loss + wm_outcome_sigreg_loss
                    wm_loss = (
                        ...
                        + args.lewm_value_loss_coef * wm_value_loss
                    ...
                    if wm_accum_count >= wm_accum_steps or end >= args.batch_size:
                        ...
                        wm_probe_gn = nn.utils.clip_grad_norm_(wm_probe_params, args.max_grad_norm)
                        optimizer.step()
                        wm_grad_norms.append(float(wm_gn))
                        wm_probe_grad_norms.append(float(wm_probe_gn))
                        wm_accum_count = 0
                    ...
                    wm_obs_sigreg_losses.append(wm_obs_sigreg_loss.item())
                    wm_outcome_sigreg_losses.append(wm_outcome_sigreg_loss.item())
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                if wm_value_losses:
                    writer.add_scalar("lewm/value_ce", float(np.mean(wm_value_losses)), global_step)
                    if wm_value_probe_weight.item() > 0:
                        writer.add_scalar(
                            "lewm/value_scalar_mae",
                            (wm_value_abs_error_sum / wm_value_probe_weight).item(),
                            global_step,
                        )
                writer.add_scalar("lewm/sigreg", float(np.mean(wm_sigreg_losses)), global_step)
                writer.add_scalar("lewm/obs_sigreg", float(np.mean(wm_obs_sigreg_losses)), global_step)
                writer.add_scalar("lewm/outcome_sigreg", float(np.mean(wm_outcome_sigreg_losses)), global_step)
                writer.add_scalar("lewm/grad_norm", float(np.mean(wm_grad_norms)), global_step)
                writer.add_scalar("lewm/probe_grad_norm", float(np.mean(wm_probe_grad_norms)), global_step)
                ...
                if wm_imagine_probe_weight.item() > 0:
                    writer.add_scalar(
                        "lewm/imagine_step_latent_mse",
                        (wm_imagine_latent_mse_sum / wm_imagine_probe_weight).item(),
                        global_step,
                    )
                    writer.add_scalar(
                        "lewm/imagine_step_reward_scalar_mae",
                        (wm_imagine_reward_abs_error_sum / wm_imagine_probe_weight).item(),
                        global_step,
                    )
                    writer.add_scalar(
                        "lewm/imagine_step_reward_scalar_bias",
                        (wm_imagine_reward_error_sum / wm_imagine_probe_weight).item(),
                        global_step,
                    )
                    writer.add_scalar(
                        "lewm/imagine_step_continuation_bce",
                        (wm_imagine_continuation_bce_sum / wm_imagine_probe_weight).item(),
                        global_step,
                    )
                    writer.add_scalar(
                        "lewm/imagine_step_value_scalar_mae",
                        (wm_imagine_value_abs_error_sum / wm_imagine_probe_weight).item(),
                        global_step,
                    )
