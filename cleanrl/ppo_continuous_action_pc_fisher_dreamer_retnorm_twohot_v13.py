# Predictive-coding actor-critic with Dreamer return normalization and TwoHot critic v13.
#
# The actor is raw-reward v11: Fisher PC scores, exact TD(lambda) traces, and
# DreamerV3 percentile return scaling. The critic is an immediate categorical
# TD(0) learner over Dreamer's 255 raw symexp-spaced bins. Its exact free CE
# force drives one identity-output PC settle; the output head receives the exact
# free force directly. Critic traces and scalar TD modulation are intentionally absent.
import math
import os
import random
import time
...
class RunningReturnRange:
    """DreamerV3 percentile return scale, using an online bootstrap target batch."""

    def __init__(self, device, rate, limit, perclo, perchi):
        if not 0.0 < rate <= 1.0:
            raise ValueError("retnorm_rate must be in (0, 1]")
        if limit <= 0.0:
            raise ValueError("retnorm_limit must be positive")
        if not 0.0 <= perclo < perchi <= 100.0:
            raise ValueError("retnorm percentiles must satisfy 0 <= low < high <= 100")
        self.rate = rate
        self.limit = limit
        self.perclo = perclo
        self.perchi = perchi
        # Dreamer uses zero initialization with debias=False. In particular, it
        # does not replace these values with the first observed percentiles.
        self.lo = torch.zeros((), device=device)
        self.hi = torch.zeros((), device=device)
    ...
    def update(self, returns):
        ...
        count = finite.sum()
        has_finite = count > 0
        # Sort invalid entries to the end, then perform linear percentile
        # interpolation over only the finite prefix. This stays device-side and
        # gives every healthy environment row a chance to update the estimator.
        ordered = torch.where(finite, returns, torch.inf).sort().values
        max_index = (count - 1).clamp_min(0)
        positions = ordered.new_tensor(
            [self.perclo / 100.0, self.perchi / 100.0]
        ) * max_index
        lower = positions.floor().long().clamp_max(max_index)
        upper = positions.ceil().long().clamp_max(max_index)
        fraction = positions - lower
        quantiles = torch.lerp(ordered[lower], ordered[upper], fraction)
        quantiles = torch.where(has_finite, quantiles, torch.zeros_like(quantiles))
        lo = torch.lerp(self.lo, quantiles[0], self.rate)
        hi = torch.lerp(self.hi, quantiles[1], self.rate)
        self.lo.copy_(torch.where(has_finite, lo, self.lo))
        self.hi.copy_(torch.where(has_finite, hi, self.hi))
        return self.stats()

    @torch.no_grad()
    def stats(self):
        return self.lo.detach(), (self.hi - self.lo).clamp_min(self.limit).detach()
...
def dreamer_twohot_bins(device=None):
    """DreamerV3's 255 raw-value bins: symexp of a uniform [-20, 20] grid."""
    half = torch.linspace(
        -TWOHOT_SYMLOG_MAX,
        0.0,
        (TWOHOT_NUM_BINS - 1) // 2 + 1,
        dtype=torch.float32,
        device=device,
    )
    half = symexp(half)
...
def twohot_encode(values, bins):
    """Linearly interpolate raw scalar values between adjacent raw-value bins."""
    values = values.float().clamp(bins[0], bins[-1])
    upper = torch.searchsorted(bins, values).clamp_max(bins.numel() - 1)
    lower = (upper - 1).clamp_min(0)
    lower_value = bins[lower]
    upper_value = bins[upper]
    equal = lower == upper
    span = torch.where(equal, torch.ones_like(values), upper_value - lower_value)
    upper_weight = torch.where(equal, torch.zeros_like(values), (values - lower_value) / span)
    lower_weight = 1.0 - upper_weight
    return (
        F.one_hot(lower, bins.numel()).to(values.dtype) * lower_weight.unsqueeze(-1)
        + F.one_hot(upper, bins.numel()).to(values.dtype) * upper_weight.unsqueeze(-1)
    )


def twohot_decode(logits, bins):
    """Decode in raw units with Dreamer's symmetric sum for exact uniform zero."""
    probabilities = logits.softmax(dim=-1)
    middle = (bins.numel() - 1) // 2
    negative = probabilities[..., :middle] * bins[:middle]
    zero = probabilities[..., middle : middle + 1] * bins[middle : middle + 1]
    positive = probabilities[..., middle + 1 :] * bins[middle + 1 :]
    return zero.sum(dim=-1) + (negative.flip(-1) + positive).sum(dim=-1)


def twohot_ce_force(logits, targets, bins):
    target_probs = twohot_encode(targets, bins)
    probabilities = logits.softmax(dim=-1)
    return target_probs - probabilities, target_probs, probabilities


@torch.no_grad()
def compute_td_modulations(
    reward,
    terminated,
    next_value,
    value,
    ...
    """Compute raw TD error and actor-only Dreamer return-range scaling."""
    # Select the terminal branch instead of multiplying by zero: IEEE 0 * NaN
    # is NaN, even though terminal transitions must ignore their bootstrap.
    td_target = torch.where(terminated, reward, reward + gamma * next_value)
    td_error = td_target - value
    return_offset, return_scale = retnorm.update(td_target)
    actor_delta = td_error / return_scale
    if actor_clip > 0:
        actor_delta = actor_delta.clamp(-actor_clip, actor_clip)
    return td_target, td_error, actor_delta, return_offset, return_scale
        ...
        for layer_idx in range(len(self.edges)):
            ...
            if layer_idx + 1 < len(self.edges):
                ...
                if output_metric is None:
                    block = block + output_edge.weight.T @ output_edge.weight
                else:
...
@torch.no_grad()
def immediate_critic_directions(scores, vf_coef):
    """Immediate categorical TD(0) directions; no trace or scalar TD multiplier."""
    return [vf_coef * score.mean(dim=0) for score in scores]
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Dreamer initializes value logits identically to zero. This is crucial:
        # the exact support reaches +/-symexp(20), so arbitrary tail asymmetry
        # would create an enormous scalar bootstrap before the first update.
        self.critic_output = OutputPredictor(
            args.hidden_size, TWOHOT_NUM_BINS, std=0.0
        )
        ...
        self.register_buffer("critic_bins", dreamer_twohot_bins())
    ...
    @torch.no_grad()
    def critic_target(self, hidden, td_target, args):
        logits = self.critic_output(hidden)
        force, target_probs, probabilities = twohot_ce_force(
            logits, td_target, self.critic_bins
        )
        # Identity endpoint geometry deliberately isolates the categorical
        # critic. Fisher-natural logits would amplify a uniform-head force by
        # roughly the number of bins and are deferred to a separate version.
        target = logits + args.pc_critic_nudge * force
        return target, force, target_probs, probabilities, logits

    @torch.no_grad()
    def get_value(self, x):
        logits = self.critic_output(self.critic_pc.initial_states(x)[-1])
        return twohot_decode(logits, self.critic_bins)
    ...
    def act(self, x):
        ...
        value = twohot_decode(self.critic_output(critic_states[-1]), self.critic_bins)
    ...
    @torch.no_grad()
    def settle_critic(self, x, td_target, args, collect_diagnostics=True):
        free_states = self.critic_pc.initial_states(x)
        target, force, target_probs, probabilities, free_logits = self.critic_target(
            free_states[-1], td_target, args
        )
        ...
        zero = free_logits.new_zeros(())
        if collect_diagnostics:
            diagnostics["output_force_error"] = (
                (target - free_logits) / args.pc_critic_nudge - force
            ).square().mean().sqrt()
            displacement = torch.cat(
                [(state - free).flatten(1) for state, free in zip(states, free_states)],
                dim=1,
            )
            diagnostics["response_rms"] = (
                displacement / args.pc_critic_nudge
            ).square().mean().sqrt()
            diagnostics["cross_entropy"] = -(
                target_probs * F.log_softmax(free_logits, dim=-1)
            ).sum(dim=-1).mean()
            diagnostics["probability_tail_mass"] = (
                probabilities[:, 0] + probabilities[:, -1]
            ).mean()
            diagnostics["logit_rms"] = free_logits.square().mean().sqrt()
            diagnostics["logit_max"] = free_logits.abs().max()
            diagnostics["decoded_value_rms"] = twohot_decode(
                free_logits, self.critic_bins
            ).square().mean().sqrt()
            diagnostics["target_saturation_fraction"] = (
                (td_target <= self.critic_bins[0])
                | (td_target >= self.critic_bins[-1])
            ).float().mean()
        else:
            for name in (
                "output_force_error",
        ...
        # The categorical head is exact rather than a post-settlement PC
        # approximation: d(-CE)/dW = (tau - p) outer free features.
        output_score = force.unsqueeze(2) * self.critic_output.augmented_features(
            free_states[-1]
        ).unsqueeze(1)
        return states, residuals, output_score, diagnostics


def make_actor_trace_bank(agent, args, device):
    batch = args.num_envs
    return TraceBank([
...
def main():
    ...
    actor_trace = make_actor_trace_bank(agent, args, device)
    ...
    for update in range(1, args.num_updates + 1):
        ...
        with torch.no_grad():
            ...
            # Match Dreamer's update-before-use timing. The actor only uses the
            # percentile range; its offset is deliberately not subtracted.
            (
                td_target,
                td_error,
                actor_delta,
        ...
        critic_states, critic_residuals, critic_output_score, critic_diag = agent.settle_critic(
            obs, td_target, args, collect_diagnostics
        )
        with torch.no_grad():
            ...
            critic_valid = finite_example_rows(critic_scores + critic_residuals + [td_target])
            ...
            critic_directions = immediate_critic_directions(
                critic_scores, args.vf_coef
            )
            if collect_diagnostics:
        ...
        if collect_diagnostics:
            ...
            writer.add_scalar("retnorm/lo", return_offset, global_step)
            writer.add_scalar("retnorm/hi", retnorm.hi, global_step)
            writer.add_scalar("retnorm/scale", return_scale, global_step)
            writer.add_scalar("retnorm/actor_delta_rms", actor_delta.square().mean().sqrt(), global_step)
            ...
            writer.add_scalar("twohot/cross_entropy", critic_diag["cross_entropy"], global_step)
            writer.add_scalar("twohot/probability_tail_mass", critic_diag["probability_tail_mass"], global_step)
            writer.add_scalar("twohot/logit_rms", critic_diag["logit_rms"], global_step)
            writer.add_scalar("twohot/logit_max", critic_diag["logit_max"], global_step)
            writer.add_scalar("twohot/decoded_value_rms", critic_diag["decoded_value_rms"], global_step)
            writer.add_scalar("twohot/target_saturation_fraction", critic_diag["target_saturation_fraction"], global_step)
