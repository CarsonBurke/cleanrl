# nGPT hypersphere satransformer v3: v2 without positional embeddings (NoPE).
#
# Diff vs latent_ngpt_v2: RoPE removed entirely. The obs tokens form a SET
# with fixed identities, not a sequence: the per-dim affine tokenizer
# (obs_embed_w[i], obs_embed_b[i]) already gives every token a learned
# identity code, which is all attention needs to tell tokens apart. RoPE's
# relative-distance bias over obs INDICES is semantically arbitrary (the
# most related pair — joint i's position and joint i's velocity — sits at a
# fixed offset RoPE treats as "far"), and under nGPT it warps the justnorm'd
# q·k cosine geometry with a fixed per-index rotation the sqk scaling then
# has to learn around. Run head-to-head vs v2 (identical otherwise) as the
# RoPE/NoPE ablation.
#
# Diff vs latent_ngpt_v1 (shared with v2):
...
#   - 2 PMA seeds (actor, critic), not 3: the action distribution comes from
#     iterthink_v24_beta, where BOTH distribution heads project from one
#     actor feature. The separate sde seed is gone with the sde head.
#   - Action distribution ported from iterthink_v24_beta_d4hlgauss (dreamer4
#     faithful, --actor-dist toggle):
#       beta (DEFAULT): alpha,beta = 1 + softplus(head) (>=1 => unimodal),
...
# re-projection onto the hypersphere (rows for input projections, cols for
# output projections, embeddings per row), plain Adam (no weight decay).
# Tokenizer and heads stay unconstrained; seed features are scaled by
# sqrt(D) before the heads (RMS-1 head-input scale, stand-in for nGPT's s_z).
...
NUM_PMA_SEEDS = 2  # readout queries: actor (both distribution heads), critic (value)
...
class NGPTSABlock(nn.Module):
    ...
    comes from the per-dim tokenizer embeddings, not positions. Per-head
...
class NGPTPMABlock(nn.Module):
    ...
    (K, V). NoPE throughout (seeds positionless; token identity comes from
    the per-dim tokenizer embeddings).
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # PMA seed queries: separate learnable params for actor / critic
        ...
        # Linear-only heads (no MLP), unconstrained fp32. Both distribution
        # heads project from the single actor seed (iterthink_v24_beta).
        if self.actor_dist == "gaussian":
            # dreamer4 Gaussian: mean head + state-dependent log-VARIANCE head.
            self.actor_head = layer_init(nn.Linear(MODEL_DIM, self.action_dim), std=0.01)
            self.actor_logvar_head = layer_init(nn.Linear(MODEL_DIM, self.action_dim), std=0.01)
            ...
            self.actor_alpha_head = layer_init(nn.Linear(MODEL_DIM, self.action_dim), std=0.01)
            self.actor_beta_head = layer_init(nn.Linear(MODEL_DIM, self.action_dim), std=0.01)
        ...
        # NoPE: no positional encoding anywhere. Token identity comes from
        # the per-dim tokenizer embeddings; latents and seeds are positionless.
    ...
    def _encode(self, x):
        """Tokenize → cat[latent, obs] → justnorm → 4x SA → PMA(separate
        seeds Q over full SA output K/V) → sqrt(D) head scale. Returns the
        (actor, critic) PMA-seed features, each shape (B, MODEL_DIM)."""
        B = x.shape[0]
        ...
        # PMA output is unit-norm; scale to RMS-1 (L2 norm sqrt(D)) so head
        # inputs match the unconstrained-backbone lineage's final RMSNorm.
