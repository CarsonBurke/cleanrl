# 2x LR + 32 minibatches + file-level advantage normalization.
# Same host-actor trainer as ppo_continuous_action_32xlr_1mb_noadvnorm_v1,
# matching the HalfCheetah run ppo_32xlr_1mb_noadvnorm_file_advnorm (32x LR,
# 1 minibatch, norm_adv on: with a single minibatch, per-minibatch
# standardization is whole-batch standardization). Here LR is 2x the 3e-4
# PPO default and minibatches are back to the base count of 32, so the
# whole-batch property is preserved explicitly: GAE advantages are
# standardized once over the full rollout batch before minibatching, and
# per-minibatch restandardization in ppo_loss stays off. This keeps the
# relative advantage scale across minibatches intact, which per-minibatch
# normalization would destroy.
# Hidden activation is LeakyReluSq: leaky_relu(x, 0.5)^2.
import os
import random
import time
...
from cleanrl.shared.host_actor import HostMLP, LeakyReluSq
...
class Args:
    ...
    """2x the 3e-4 PPO default; one Adam covers actor and critic"""
    num_envs: int = 1
    ...
    """the number of mini-batches (PPO base default)"""
    update_epochs: int = 10
    """the K epochs to update the policy"""
    norm_adv: bool = False
    """raw GAE in the surrogate; no per-minibatch standardization"""
    file_norm_adv: bool = True
    """standardize GAE advantages once over the full rollout batch before minibatching"""
    clip_coef: float = 0.2
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(observation_dim, 64)), LeakyReluSq(),
            layer_init(nn.Linear(64, 64)), LeakyReluSq(),
...
def validate_args(args):
    ...
    if (args.norm_adv or args.file_norm_adv) and (args.minibatch_size < 2 or args.batch_size % args.minibatch_size == 1):
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; 2x LR; 32 minibatches; file-level advantage normalization")
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                if args.file_norm_adv:
                    # Whole-batch standardization, applied once before the
                    # epoch/minibatch loop. Per-minibatch restandardization in
                    # ppo_loss stays off so cross-minibatch scale is preserved.
