# PPO + IterThink v92 (v24 Beta + pure candidate-set TPO). From v24_dist.
#
...
# rollout policy at each state. A lightweight action-advantage scorer assigns a
# scalar score to every candidate; the executed candidate is anchored to its
# realized critic-consistent rollout advantage. TPO then constructs
...
#
# Hypothesis: v24's strong Beta distributional critic gives reliable signed
# state-action feedback, while the auxiliary scorer provides learned
# counterfactual scores for unexecuted candidates without environment cloning.
# Pure TPO then decouples "where probability mass should move" from PPO's scalar
# clipped-ratio mechanics. Because the group is formed in native Beta space,
# replay remains bounded and avoids tanh-squash pathologies.
#
# --- inherited v24 notes ---
...
class Args:
    ...
    # v92: pure TPO actor objective. A finite candidate group is sampled from the
    # behavior policy at rollout time. Candidate 0 is executed and anchored to
    # the realized advantage; all candidates are scored by an auxiliary
    # action-advantage head before q is frozen for replay epochs.
    tpo_group_size: int = 8
    tpo_eta: float = 1.0
    tpo_score_coef: float = 0.25
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.action_score_head = nn.Sequential(
            ...
            ReLUSquared(),
            layer_init(nn.Linear(H, 1), std=0.01),
        )
    ...
    def get_action_candidates_and_value(self, x, num_candidates):
        ...
        return action, z, z_candidates, candidate_log_probs, entropy, value_logits

    def _score_action_input(self, z):
        if self.actor_dist == "beta":
            return 2.0 * z - 1.0
        return torch.tanh(z)

    def _candidate_scores(self, critic_feat, z_candidates):
        num_candidates = z_candidates.shape[1]
        candidate_feat = critic_feat.unsqueeze(1).expand(-1, num_candidates, -1)
        score_input = torch.cat([candidate_feat, self._score_action_input(z_candidates)], dim=-1)
        return self.action_score_head(score_input).squeeze(-1)

    def get_candidate_scores(self, x, z_candidates):
        _, critic_feat = self._trunks(x)
        return self._candidate_scores(critic_feat, z_candidates)

    def get_candidate_log_probs_scores_and_value(self, x, z_candidates):
        actor_feat, critic_feat = self._trunks(x)
        num_candidates = z_candidates.shape[1]
        candidate_feat = actor_feat.unsqueeze(1).expand(-1, num_candidates, -1)
        dist, _, log_det_fn = self._actor_dist(candidate_feat)
        candidate_log_probs = (dist.log_prob(z_candidates) - log_det_fn(z_candidates)).sum(-1)
        candidate_scores = self._candidate_scores(critic_feat, z_candidates)
        value_logits = self.critic_head(critic_feat)
        if self.actor_dist == "gaussian":
            dist_1, _, log_det_1 = self._actor_dist(actor_feat)
            zr = dist_1.rsample()
            ...
            entropy = dist.entropy()[:, 0].sum(1)
        return candidate_log_probs, candidate_scores, entropy, value_logits
    ...
    def critic_parameters(self):
        ...
        return list(trunk.parameters()) + list(self.critic_head.parameters()) + list(self.action_score_head.parameters())
...
if __name__ == "__main__":
    ...
    assert args.tpo_score_coef >= 0.0, "tpo_score_coef must be non-negative"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, _, z_cands, cand_logprobs, ent, value_logits = agent.get_action_candidates_and_value(
        ...
        b_u = u.reshape(-1)
        b_candidate_scores = torch.zeros((args.batch_size, args.tpo_group_size), device=device)
        with torch.no_grad():
            for start in range(0, args.batch_size, args.minibatch_size):
                end = start + args.minibatch_size
                b_candidate_scores[start:end] = agent.get_candidate_scores(
                    b_obs[start:end], b_candidate_zs[start:end]
                )
            b_candidate_scores[:, 0] = b_advantages
        # Diagnostics: keep the inherited v24 advantage transform visible, but
        # pure TPO scores the executed candidate with raw critic-consistent policy
        # GAE. TPO then standardizes scores within each candidate group.
        gae = b_advantages
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                new_candidate_logprobs, new_candidate_scores, entropy, value_logits = agent.get_candidate_log_probs_scores_and_value(
                ...
                score_loss = F.smooth_l1_loss(new_candidate_scores[:, 0], b_advantages[mb_inds])
                tpo_score_losses.append(score_loss.item())
                ...
                if args.separate_grad_clip:
                    ...
                    (args.vf_coef * v_loss + args.tpo_score_coef * score_loss).backward(retain_graph=True)
                    ...
                    loss = policy_loss - ent_coef_eff * entropy_loss + v_loss * args.vf_coef + args.tpo_score_coef * score_loss
                    optimizer.zero_grad()
                    loss.backward()
        ...
        writer.add_scalar("losses/tpo_score_loss", np.mean(tpo_score_losses), global_step)
