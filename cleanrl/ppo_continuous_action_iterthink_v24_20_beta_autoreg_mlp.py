# PPO + IterThink v24.20 (STATE-CONDITIONED MLP AUTOREGRESSIVE BETA). From v24.16.
#
# Hypothesis: v24.16's global triangular AR residual works because bounded Beta
# exploration benefits from conditional action covariance. v24.20 keeps the exact
# teacher-forced AR-Beta density but makes the residual a zero-init state-conditioned
# prefix MLP. For each action dimension i, the residual sees actor features and the
# centered prefix z_<i only:
# Policy:
#   p(z|s) = prod_i Beta(z_i | alpha_i(s, z_<i), beta_i(s, z_<i)).
# The final residual head is zero-initialized, so startup is exactly v24 while the
# learned residual can become state-dependent and nonlinear. Rollout stores native
# z in (0,1); PPO replay teacher-forces stored z for exact probability ratios.
import os
import random
import time
...
class Args:
    ...
    # v24.16: autoregressive unimodal Beta actor. The base heads match v24's
    # independent Beta path; zero-initialized lower-triangular residual weights add
    # causal dependence on previous native z dimensions without changing init.
    autoreg_hidden: int = 64
    autoreg_residual_scale: float = 1.0
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v24.20 autoregressive Beta. Base heads are v24's independent Beta logits;
        # a state-conditioned prefix MLP adds residual concentration logits. The
        # residual head is zero-init, so startup is exactly v24.
        self.act_dim = act_dim
        self.actor_alpha_head = layer_init(nn.Linear(H, act_dim), std=0.01)
        self.actor_beta_head = layer_init(nn.Linear(H, act_dim), std=0.01)
        self.autoreg_residual_scale = float(args.autoreg_residual_scale)
        self.autoreg_mlp = nn.Sequential(
            layer_init(nn.Linear(H + act_dim, args.autoreg_hidden), std=0.5),
            nn.SiLU(),
            layer_init(nn.Linear(args.autoreg_hidden, args.autoreg_hidden), std=0.5),
            nn.SiLU(),
            layer_init(nn.Linear(args.autoreg_hidden, 2 * act_dim), std=0.0),
        )
        self.register_buffer("autoreg_prefix_mask", torch.tril(torch.ones(act_dim, act_dim), diagonal=-1))
    ...
    def _autoreg_residual_dim(self, actor_feat, z, dim):
        z_centered = z - 0.5
        prefix = z_centered * self.autoreg_prefix_mask[dim].unsqueeze(0)
        raw = self.autoreg_mlp(torch.cat([actor_feat, prefix], dim=-1))
        raw_alpha, raw_beta = raw.chunk(2, dim=-1)
        return (
            self.autoreg_residual_scale * raw_alpha[:, dim],
            self.autoreg_residual_scale * raw_beta[:, dim],
        )

    def _base_logits(self, actor_feat):
        return self.actor_alpha_head(actor_feat), self.actor_beta_head(actor_feat)

    def _beta_params_dim(self, actor_feat, z, dim, base_alpha=None, base_beta=None):
        if base_alpha is None or base_beta is None:
            base_alpha, base_beta = self._base_logits(actor_feat)
        alpha_resid, beta_resid = self._autoreg_residual_dim(actor_feat, z, dim)
        alpha = 1.0 + F.softplus(base_alpha[:, dim] + alpha_resid)
        beta = 1.0 + F.softplus(base_beta[:, dim] + beta_resid)
        return alpha, beta, alpha_resid, beta_resid

    def _beta_params(self, actor_feat, z):
        base_alpha = self.actor_alpha_head(actor_feat)
        base_beta = self.actor_beta_head(actor_feat)
        alpha_resids = []
        beta_resids = []
        for i in range(self.act_dim):
            alpha_resid, beta_resid = self._autoreg_residual_dim(actor_feat, z, i)
            alpha_resids.append(alpha_resid)
            beta_resids.append(beta_resid)
        alpha_resid = torch.stack(alpha_resids, dim=-1)
        beta_resid = torch.stack(beta_resids, dim=-1)
    ...
    def _sample_autoreg_z(self, actor_feat):
        ...
        base_alpha, base_beta = self._base_logits(actor_feat)
        for i in range(self.act_dim):
            alpha, beta, _, _ = self._beta_params_dim(actor_feat, z, i, base_alpha, base_beta)
            zi = Beta(alpha, beta).sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            z[:, i] = zi
        return z

    def _autoreg_logprob_entropy(self, actor_feat, z):
        base_alpha, base_beta = self._base_logits(actor_feat)
        log_prob = torch.zeros(actor_feat.shape[0], device=actor_feat.device, dtype=actor_feat.dtype)
        ...
        beta_resids = []
        for i in range(self.act_dim):
            alpha, beta, alpha_resid, beta_resid = self._beta_params_dim(actor_feat, z, i, base_alpha, base_beta)
            dist_i = Beta(alpha, beta)
            log_prob = log_prob + dist_i.log_prob(z[:, i])
            entropy = entropy + dist_i.entropy()
            alpha_vals.append(alpha)
            beta_vals.append(beta)
            alpha_resids.append(alpha_resid)
            beta_resids.append(beta_resid)
        alpha = torch.stack(alpha_vals, dim=-1)
        beta = torch.stack(beta_vals, dim=-1)
        alpha_resid = torch.stack(alpha_resids, dim=-1)
        beta_resid = torch.stack(beta_resids, dim=-1)
        return log_prob, entropy, alpha, beta, alpha_resid, beta_resid
    ...
    def get_action_and_value(self, x, z=None):
        # z is the native Beta sample in (0,1). During PPO replay, stored z is
        # teacher-forced through the lower-triangular head so each conditional
        # log-prob sees the same z_<i prefix used by the rollout policy.
        actor_feat, critic_feat = self._trunks(x)
        if z is None:
            z = self._sample_autoreg_z(actor_feat)
        action = self._z_to_action(z)
        # The linear z -> action rescale has constant Jacobian, so it is omitted
        # exactly as in v24 Beta; it cancels in PPO ratios.
        log_prob, entropy, _, _, _, _ = self._autoreg_logprob_entropy(actor_feat, z)
        value_logits = self.critic_head(critic_feat)
        return action, z, log_prob, entropy, value_logits

    def actor_parameters(self):
        # Params receiving the POLICY gradient (incl. the shared trunk). The base
        # concentration heads and conditional residual weights are clipped together
        # as one actor group, preserving v24's 2-way decoupled clip structure.
        trunk = self.trunk if self.share_backbone else self.actor_trunk
        heads = (
            list(self.actor_alpha_head.parameters())
            + list(self.actor_beta_head.parameters())
            + list(self.autoreg_mlp.parameters())
        )
    ...
    def actor_diagnostics(self, x, z):
        actor_feat, _ = self._trunks(x)
        _, _, alpha, beta, alpha_resid, beta_resid = self._autoreg_logprob_entropy(actor_feat, z)
        resid = torch.cat([alpha_resid, beta_resid], dim=-1)
        return {
            "cond_residual_rms": resid.pow(2).mean().sqrt(),
...
if __name__ == "__main__":
    ...
    # v24's auto-entropy path was Gaussian-only. Keep this ablation focused on
    # autoregressive Beta and fail loudly if that incompatible switch is requested.
    if args.auto_entropy:
        raise ValueError("--auto-entropy is not supported by the autoregressive Beta ablation")
    auto_alpha = False
    if auto_alpha:
        act_dim = int(np.prod(envs.single_action_space.shape))
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            diag_n = min(args.minibatch_size, b_obs.shape[0])
            actor_diag = agent.actor_diagnostics(b_obs[:diag_n], b_latent_zs[:diag_n])
        ...
        writer.add_scalar("debug/cond_residual_rms", actor_diag["cond_residual_rms"].item(), global_step)
        writer.add_scalar("debug/beta_alpha_mean", actor_diag["alpha_mean"].item(), global_step)
        writer.add_scalar("debug/beta_beta_mean", actor_diag["beta_mean"].item(), global_step)
