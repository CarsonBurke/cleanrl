# PPO + IterThink v95 (v24 Beta + short-branch TPO). From v94.
#
...
# rollout policy at each state. Unlike v94's myopic immediate-reward score, v95
...
# from the frozen rollout policy and scores the branch by dense discounted
# normalized rewards plus a frozen critic bootstrap. TPO then constructs
...
# dense reward intervals while avoiding the contextual-bandit failure of v94.
#
# --- inherited v24 notes ---
...
class Args:
    ...
    # v95: pure TPO actor objective over short counterfactual MuJoCo branches.
    # Candidate 0 is still the behavior action executed on the real wrapped env.
    tpo_group_size: int = 4
    tpo_branch_horizon: int = 4
    tpo_eta: float = 2.0
    tpo_score_floor: float = 0.1
    tpo_weighted_contexts: bool = True
...
def branch_tpo_rollouts(envs, agent, root_obs, first_zs, first_actions, first_logprobs, support, args, device):
    ...
    Returns:
      ...
      branch_old_logprobs: (N, K), sum old policy log-probs along the branch.
      branch_scores: (N, K), discounted normalized reward sum plus bootstrap.
    """
    ...
    for env_i, wrapped_env in enumerate(envs.envs):
        ...
        for cand_i in range(num_candidates):
            ...
            obs_mean, obs_var, obs_count = rms_after_one(
                norm_obs_wrapper.obs_rms.mean,
                norm_obs_wrapper.obs_rms.var,
                norm_obs_wrapper.obs_rms.count,
                raw_next_obs,
            )
            ret = norm_rew_wrapper.returns[0] * norm_rew_wrapper.gamma * (1 - terminated) + raw_reward
            rew_mean, rew_var, rew_count = rms_after_one(
                norm_rew_wrapper.return_rms.mean,
                norm_rew_wrapper.return_rms.var,
                norm_rew_wrapper.return_rms.count,
                np.asarray(ret, dtype=np.float64),
            )
            norm_reward = np.clip(raw_reward / np.sqrt(rew_var + norm_rew_wrapper.epsilon), -10.0, 10.0)
            ...
            obs_stats[(env_i, cand_i)] = (obs_mean, obs_var, obs_count)
            rew_stats[(env_i, cand_i)] = (ret, rew_mean, rew_var, rew_count)
        base.set_state(root_qpos, root_qvel)
        base.data.time = root_time
    ...
    for h in range(1, horizon):
        ...
        for env_i, cand_i in live_keys:
            obs_mean, obs_var, _ = obs_stats[(env_i, cand_i)]
            norm_obs_wrapper = find_wrapper(envs.envs[env_i], gym.wrappers.NormalizeObservation)
            obs_batch_np.append(normalize_raw_obs(raw_obs_next[env_i][cand_i], obs_mean, obs_var, norm_obs_wrapper.epsilon))
        obs_batch = torch.tensor(np.asarray(obs_batch_np), dtype=torch.float32, device=device)
        with torch.no_grad():
            action_batch, z_batch, logprob_batch, _, _ = agent.get_action_and_value(obs_batch)
        ...
        for j, (env_i, cand_i) in enumerate(live_keys):
            ...
            branch_old_logprobs[env_i, cand_i] += logprob_batch[j]
            ...
            obs_mean, obs_var, obs_count = obs_stats[(env_i, cand_i)]
            obs_mean, obs_var, obs_count = rms_after_one(obs_mean, obs_var, obs_count, raw_next_obs)
            ret, rew_mean, rew_var, rew_count = rew_stats[(env_i, cand_i)]
            ret = ret * norm_rew_wrapper.gamma * (1 - terminated) + raw_reward
            rew_mean, rew_var, rew_count = rms_after_one(rew_mean, rew_var, rew_count, np.asarray(ret, dtype=np.float64))
            norm_reward = np.clip(raw_reward / np.sqrt(rew_var + norm_rew_wrapper.epsilon), -10.0, 10.0)
...
if __name__ == "__main__":
    ...
    assert args.env_id == "HalfCheetah-v4", "v95 counterfactual probing is HalfCheetah-v4-specific"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                mb_branch_obs = b_branch_obs[mb_inds]
                mb_branch_zs = b_branch_zs[mb_inds]
                mb_branch_masks = b_branch_masks[mb_inds]
                flat_obs = mb_branch_obs.reshape((-1,) + envs.single_observation_space.shape)
                flat_zs = mb_branch_zs.reshape((-1,) + envs.single_action_space.shape)
                flat_masks = mb_branch_masks.reshape(-1)
                flat_logprobs = torch.zeros_like(flat_masks)
                entropy_loss = torch.zeros((), device=device)
                valid = flat_masks > 0.0
                if valid.any():
                    _, _, valid_logprobs, valid_entropy, _ = agent.get_action_and_value(flat_obs[valid], flat_zs[valid])
                    flat_logprobs[valid] = valid_logprobs
                    entropy_loss = valid_entropy.mean()
                new_step_logprobs = flat_logprobs.reshape(-1, args.tpo_group_size, args.tpo_branch_horizon)
                new_branch_logprobs = (new_step_logprobs * mb_branch_masks).sum(dim=-1)
                newlogprob = new_step_logprobs[:, 0, 0]
                ...
                with torch.no_grad():
                    ...
                    new_group_logp = F.log_softmax(new_branch_logprobs, dim=-1)
