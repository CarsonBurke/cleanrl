# PPO + IterThink v24 Beta + d3bucket MTP + BetaPlast v6: v4 + aggressive clip-higher.
# =====================================================================================
# v6 change over v4: clip_coef_high 0.28 -> 0.45. v4 (sign-neg / magnitude-pos PPO clip) was
# the clear ablation winner (~2x v2 at matched 1M steps), validating "reinforce good actions
# by magnitude, suppress bad ones by pure sign." v6 pushes that emphasis harder: a much looser
# UPPER clip (1+0.45) gives positive-advantage actions far more room to grow per update, while
# the lower clip (1-0.2) still caps how fast negative (sign=-1) actions are suppressed -- so the
# extra room accrues almost entirely to positive reinforcement. Tests whether v4 was clip-limited
# on the upside. Everything else identical to v4 (relative plasticity gate [0.25,4]).
#
# --- inherited v4 notes ---
...
class Args:
    ...
    clip_coef_high: float = 0.45     # "clip-higher" (DAPO): looser UPPER bound 1+clip_coef_high
    ent_coef: float = 0.0
