# Detached variance-matched HL-Gauss on our residual architecture v16.
# Loss-only comparison to v12 K501/sigma1.5: CE times the batch-mean predicted
# variance in decoded value units (target_scale^2 * Var_p(z)). Detach the gain
# so the critic cannot minimize the CE entropy floor by collapsing its variance.
# Hypothesis: restoring local mean-error gradient scale improves late PPO learning.
# Keep current-rollout frames, reward normalization, both trunks and shared Adam;
# no EMA, advantage normalization, value clipping, or new gradient clipping rule.
import os
import random
import time
...
class Args:
    ...
    """support radius in target standard deviations"""
    value_sigma_bins: float = 1.5
    """Gaussian smoothing in bin widths"""
    ent_coef: float = 0.0
...
class NormalizedEvaluationEnv(gym.Wrapper):
    """Single-environment evaluation using the training normalizer contracts."""
    def __init__(self, env, gamma):
        super().__init__(env)
        self.obs_norm = VectorObsNorm(1, env.observation_space.shape)
        self.rew_norm = VectorRewardNorm(1, gamma)
        self._reward = np.empty(1, dtype=np.float64)
        self._terminated = np.empty(1, dtype=np.bool_)

    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        return self.obs_norm.normalize(np.asarray(obs)[None])[0], info

    def step(self, action):
        obs, reward, terminated, truncated, info = self.env.step(action)
        self._reward[0], self._terminated[0] = reward, terminated
        normalized_reward = self.rew_norm.normalize(self._reward, self._terminated)[0]
        normalized_obs = self.obs_norm.normalize(np.asarray(obs)[None])[0]
        return normalized_obs, float(normalized_reward), terminated, truncated, info
...
def make_env(env_id, idx, capture_video, run_name, gamma):
    ...
    def thunk():
        return NormalizedEvaluationEnv(raw_factory(), gamma)
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        ...
        self.histogram = StandardizedHistogram(
            num_bins=args.value_atoms, half_span=args.value_half_span,
    ...
    def get_value(self, x):
        return self.histogram.decode(self.critic(x)).unsqueeze(-1)
...
def target_entropy(target_probs):
    """The Gaussian-label CE floor; zero-probability atoms contribute zero."""
    return -(target_probs * target_probs.clamp_min(1e-30).log()).sum(dim=-1).mean()


def categorical_value_terms(logits, target_probs, histogram):
    """Batch CE times a detached, batch-mean predicted value variance."""
    log_probs = logits.log_softmax(dim=-1)
    ce = -(target_probs.detach() * log_probs).sum(dim=-1).mean()
    with torch.no_grad():
        gain = histogram.value_gradient_gain(log_probs.detach().exp()).mean()
    return ce * gain, ce, gain
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, target_probs, args):
    ...
    v_loss, raw_ce, value_gain = categorical_value_terms(logits, target_probs, agent.histogram)
    entropy_loss = entropy.mean()
    loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef
    metrics = torch.stack((pg_loss.detach(), v_loss.detach(), entropy_loss.detach(),
                           old_approx_kl, approx_kl, clipfrac, raw_ce.detach(), value_gain))
    return loss, metrics
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", "residual SiTU-GLU; unit-L2 matrices and residual directions; sqrt(width) scaling; no LayerScale; HL-Gauss bandwidth/resolution ablation; support mean +/-5 target std; plain CE, no EMA or value clipping; original actor and critic residual trunks; entropy-floor and gradient-share diagnostics")
        ...
        @torch.no_grad()
        def prepare_targets(returns, previous_values):
            previous_mean = agent.histogram.mean.clone()
            previous_scale = agent.histogram.scale.clone()
            agent.histogram.observe(returns)
            labels = agent.histogram.project(returns)
            saturation = (agent.histogram.standardize(returns).abs() > agent.histogram.half_span).float().mean()
            label_error = (agent.histogram.decode_probs(labels) - returns).abs().mean()
            floor = target_entropy(labels)
            frame_shift = (agent.histogram.mean - previous_mean) / previous_scale
            frame_ratio = agent.histogram.scale / previous_scale
            # Reframe unchanged logits analytically: isolate optimizer progress from frame drift.
            reframed_values = agent.histogram.mean + frame_ratio * (previous_values - previous_mean)
            preupdate_mse = (reframed_values - returns).square().mean()
            return labels, saturation, label_error, floor, frame_shift, frame_ratio, preupdate_mse
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                (b_target_probs, saturated, label_error, entropy_floor, frame_shift,
                 frame_ratio, preupdate_mse) = prepare_targets(b_returns, b_values)
            updates = 0
                ...
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss, metrics = loss_model(
                            ...
                            b_advantages[indices], b_target_probs[indices],
                        )
                        optimizer.zero_grad(set_to_none=True)
                        loss.backward()
                        total_preclip_norm = nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                        optimizer.step()
                        normalize_matrices()
            ...
            with torch.no_grad():
                ...
                grad_norm = torch.sqrt(actor_grad_norm.square() + critic_grad_norm.square()).clamp_min(1e-30)
            logged = gather_metrics({
                ...
                "critic/target_mean": agent.histogram.mean, "critic/target_scale": agent.histogram.scale,
                "critic/label_entropy": entropy_floor, "critic/excess_ce": last[6] - entropy_floor,
                "critic/raw_ce": last[6], "critic/value_gain": last[7],
                "critic/preupdate_mse": preupdate_mse, "critic/rollout_mse": b_advantages.square().mean(),
                ...
                "critic/decoded_mse": post_error.square().mean(), "critic/value_bias": post_error.mean(),
                "critic/normalized_mse": (post_error / agent.histogram.scale).square().mean(),
                "critic/frame_shift_std": frame_shift, "critic/frame_scale_ratio": frame_ratio,
                "critic/support_low": agent.histogram.mean - agent.histogram.half_span * agent.histogram.scale,
                "critic/support_high": agent.histogram.mean + agent.histogram.half_span * agent.histogram.scale,
                "critic/sigma_value_units": agent.histogram.sigma * agent.histogram.scale,
