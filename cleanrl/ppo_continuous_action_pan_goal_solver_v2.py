# Pan Goal Solver v2: deterministic aspirational latent with direct actions.
#
# A reward-free LeJEPA learns observation tokens and temporal control belief.
# Hindsight trains a desired-successor model plus inverse dynamics, fused into
# one action call. Reward trains only a global affine goal ray P(y)=mu+s(y)d;
...
class Args:
    ...
    goal_overlap_coef: float = 0.05
    ema_decay: float = 0.995

    follower_hidden: int = 512
    successor_coef: float = 1.0
    inverse_coef: float = 1.0
    composed_coef: float = 1.0
    ray_consistency_coef: float = 0.25
    occupancy_coef: float = 0.1
    counterfactual_coef: float = 0.25
    counterfactual_match_radius: float = 0.75
...
class DirectGoalFollower(nn.Module):
    """Pan-style desired successor followed by inverse dynamics."""

    def __init__(self, observation_tokens, belief_dim, goal_dim, act_dim, hidden):
        super().__init__()
        goal_features = goal_dim + 1
        ...
        self.phase_readout = nn.Sequential(
            nn.RMSNorm(observation_tokens * belief_dim),
            xavier_linear(nn.Linear(observation_tokens * belief_dim, hidden)),
            nn.SiLU(),
            xavier_linear(nn.Linear(hidden, belief_dim)),
        )
        self.successor_base = nn.Sequential(
            nn.RMSNorm(belief_dim),
            xavier_linear(nn.Linear(belief_dim, hidden)),
            nn.SiLU(),
            xavier_linear(nn.Linear(hidden, belief_dim)),
        )
        self.successor_goal = nn.Sequential(
            nn.RMSNorm(goal_features),
            xavier_linear(nn.Linear(goal_features, hidden)),
            nn.SiLU(),
            xavier_linear(nn.Linear(hidden, belief_dim)),
        )
        self.inverse_trunk = nn.Sequential(
        ...
        self.inverse_head = xavier_linear(nn.Linear(hidden, 2 * act_dim), gain=0.01)
        ...
        self.register_buffer("goal_scale", torch.ones(goal_dim))

    def goal_features(self, current_goal, desired_goal):
        scaled = (desired_goal - current_goal) / self.goal_scale.clamp_min(1e-3)
        magnitude = scaled.square().mean(dim=-1, keepdim=True).add(1e-8).sqrt()
        # Suppress arbitrary directions from nearly identical overlapping
        # histories; asymptotically this becomes the unit reward-ray direction.
        direction = scaled / (magnitude + 0.1)
        capped_log_magnitude = torch.log1p(magnitude).clamp(max=np.log(9.0))
        return torch.cat([direction, capped_log_magnitude], dim=-1)

    def desired_successor(self, belief, current_goal, desired_goal):
        return self.successor_base(belief) + self.successor_goal(
            self.goal_features(current_goal, desired_goal)
        )

    def read_belief(self, observation_memory):
        return self.belief_readout(observation_memory.flatten(1))

    def read_phase(self, observation_tokens):
        return self.phase_readout(observation_tokens.flatten(1))

    def inverse_distribution(self, belief, desired_next):
        hidden = self.inverse_trunk(torch.cat([belief, desired_next], dim=-1))
        alpha_raw, beta_raw = self.inverse_head(hidden).chunk(2, dim=-1)
        return 1 + F.softplus(alpha_raw), 1 + F.softplus(beta_raw)

    def inverse_action(self, belief, desired_next):
        alpha, beta = self.inverse_distribution(belief, desired_next)
        native_mean = alpha / (alpha + beta)
        return 2 * native_mean - 1

    def action_nll(self, belief, desired_next, action):
        alpha, beta = self.inverse_distribution(belief, desired_next)
        native_action = ((action + 1) / 2).clamp(1e-5, 1 - 1e-5)
        return -torch.distributions.Beta(alpha, beta).log_prob(native_action).sum(-1).mean()
    ...
    def forward(self, belief, current_goal, desired_goal):
        desired_next = self.desired_successor(belief, current_goal, desired_goal)
        return self.inverse_action(belief, desired_next), desired_next
...
class PanGoalSolver(nn.Module):
    ...
    def direct_action(self, obs_history, incoming_action_history, cached_goal):
        ...
        action, desired_next = self.follower(belief, current_goal, goal)
        return action, current_goal, desired_next
...
class VectorReplayBuffer:
    ...
    def sample_hindsight(self, batch_size, history, max_offset, discount, rng):
        ...
        while sum(len(x[0]) for x in accepted) < batch_size:
            ...
            alternate_offset = self._geometric_offset(end.shape[0], max_offset, discount, rng)
            future = end + offset
            alternate_future = end + alternate_offset
            within = (future < self.total_steps) & (alternate_future < self.total_steps)
            future = np.minimum(future, self.total_steps - 1)
            alternate_future = np.minimum(alternate_future, self.total_steps - 1)
            valid = within & self._valid_end(env, end) & self._valid_end(env, end + 1)
            valid &= self._valid_end(env, future) & self._valid_end(env, alternate_future)
            ...
            alternate_episode = self._gather(self.episode, env, alternate_future[:, None]).reshape(-1)
            valid &= (source_episode == future_episode) & (source_episode == alternate_episode)
            accepted.append((env[valid], end[valid], future[valid], alternate_future[valid]))
        env = np.concatenate([x[0] for x in accepted])[:batch_size]
        end = np.concatenate([x[1] for x in accepted])[:batch_size]
        future = np.concatenate([x[2] for x in accepted])[:batch_size]
        alternate_future = np.concatenate([x[3] for x in accepted])[:batch_size]
        offsets = np.arange(history - 1, -1, -1)
        current_indices = end[:, None] - offsets
        next_indices = current_indices + 1
        future_indices = future[:, None] - offsets
        alternate_future_indices = alternate_future[:, None] - offsets
        return {
            "obs": self._padded_history(self.obs, env, end, history),
            "incoming": self._padded_history(self.incoming_action, env, end, history, zero_pad=True),
            "next_obs": self._padded_history(self.obs, env, end + 1, history),
            "future_obs": self._padded_history(self.obs, env, future, history),
            "alternate_future_obs": self._padded_history(
                self.obs, env, alternate_future, history
            ),
...
def train_follower_step(agent, optimizer, batch, args, device, rng):
    ...
    alternate_future_obs = as_tensor(batch["alternate_future_obs"], device)
    action = as_tensor(batch["action"], device)
    with torch.no_grad():
        all_tokens, all_goals = agent.target_encode(
            torch.cat([obs, next_obs, future_obs, alternate_future_obs], dim=0)
        )
        current_tokens, next_tokens, _, _ = all_tokens.chunk(4)
        current_goal, next_goal, future_goal, alternate_future_goal = all_goals.chunk(4)
        _, current_memory = agent.world_model.temporal(
            current_tokens, torch.zeros_like(incoming)
        )
    belief = agent.follower.read_belief(current_memory[:, :-1].detach())
    next_frame = agent.follower.read_phase(next_tokens[:, -1].detach())
    composed_action, desired_next = agent.follower(belief, current_goal, future_goal)
    alternate_action, alternate_next = agent.follower(
        belief, current_goal, alternate_future_goal
    )
    alpha = torch.exp(
        torch.empty(action.shape[0], 1, device=device).uniform_(0, np.log(8.0))
    )
    extrapolated_goal = current_goal + alpha * (future_goal - current_goal)
    extrapolated_action, extrapolated_next = agent.follower(
        belief, current_goal, extrapolated_goal
    )
    successor_loss = 0.5 * (
        F.mse_loss(desired_next, next_frame.detach())
        + F.mse_loss(alternate_next, next_frame.detach())
    )
    inverse_loss = agent.follower.action_nll(belief, next_frame, action)
    composed_loss = 0.5 * (
        agent.follower.action_nll(belief, desired_next, action)
        + agent.follower.action_nll(belief, alternate_next, action)
    )
    ray_successor_loss = F.mse_loss(extrapolated_next, desired_next.detach())
    ray_action_loss = F.mse_loss(extrapolated_action, composed_action.detach())
    ray_loss = ray_successor_loss + ray_action_loss
    occupancy_goals = torch.cat([future_goal, alternate_future_goal], dim=0)
    occupancy_logits = agent.follower.occupancy_logits(belief, occupancy_goals)
    positive = torch.stack(
        [occupancy_logits.diagonal(), occupancy_logits[:, action.shape[0] :].diagonal()], dim=1
    )
    occupancy_loss = (
        torch.logsumexp(occupancy_logits, dim=1) - torch.logsumexp(positive, dim=1)
    ).mean()
    with torch.no_grad():
        ...
        mutual = matched[matched] == torch.arange(action.shape[0], device=device)
        valid_match = (
    ...
    if valid_match.any():
        ...
        shared_belief = 0.5 * (belief[selected] + belief[partner])
        shared_current = 0.5 * (current_goal[selected] + current_goal[partner])
        action_a, next_a = agent.follower(
            shared_belief, shared_current, future_goal[selected]
        )
        action_b, next_b = agent.follower(
            shared_belief, shared_current, future_goal[partner]
        )
        counterfactual_loss = 0.5 * (
            F.mse_loss(next_a, next_frame[selected].detach())
            + F.mse_loss(next_b, next_frame[partner].detach())
            + agent.follower.action_nll(shared_belief, next_a, action[selected])
            + agent.follower.action_nll(shared_belief, next_b, action[partner])
        )
        counterfactual_action_sensitivity = (action_a - action_b).square().mean()
        counterfactual_successor_sensitivity = (next_a - next_b).square().mean()
    else:
        counterfactual_loss = successor_loss.new_zeros(())
        counterfactual_action_sensitivity = successor_loss.new_zeros(())
        counterfactual_successor_sensitivity = successor_loss.new_zeros(())
    loss = (
        args.successor_coef * successor_loss
        + args.inverse_coef * inverse_loss
        + args.composed_coef * composed_loss
        + args.ray_consistency_coef * ray_loss
    ...
    optimizer.step()
    with torch.no_grad():
        scale = torch.cat([current_goal, future_goal, alternate_future_goal]).std(
            dim=0, unbiased=False
        ).clamp_min(1e-3)
        agent.follower.goal_scale.lerp_(scale, 1 - args.goal_scale_decay)
        achieved_progress = (
            (current_goal - future_goal).square().mean(-1)
            - (next_goal - future_goal).square().mean(-1)
        )
        extrapolated_progress = (
            (current_goal - extrapolated_goal).square().mean(-1)
            - (next_goal - extrapolated_goal).square().mean(-1)
        )
    return {
        "follower/loss": loss.item(),
        "follower/successor_mse": successor_loss.item(),
        "follower/inverse_mse": inverse_loss.item(),
        "follower/composed_mse": composed_loss.item(),
        "follower/ray_consistency_mse": ray_loss.item(),
        "follower/ray_action_consistency_mse": ray_action_loss.item(),
        ...
        "follower/counterfactual_successor_sensitivity": counterfactual_successor_sensitivity.item(),
        ...
        "follower/extrapolated_progress": extrapolated_progress.mean().item(),
...
def train_goal_step(agent, optimizer, obs, reward_rate, args, device):
    ...
    band_id = physical_band_ids(reward_rate)
    band_tangents = []
    populated = sorted(int(index) for index in band_id.unique())
    for low, high in zip(populated, populated[1:]):
        low_mask, high_mask = band_id == low, band_id == high
        low_reward, high_reward = reward_rate[low_mask].mean(), reward_rate[high_mask].mean()
        coordinate_delta = (
            agent.proposer.reward_coordinate(high_reward)
            - agent.proposer.reward_coordinate(low_reward)
        ).clamp_min(1e-3)
        empirical_tangent = (
            goal_target[high_mask].mean(0) - goal_target[low_mask].mean(0)
        ) / coordinate_delta
        band_tangents.append(F.mse_loss(agent.proposer.direction, empirical_tangent))
    tangent_loss = (
        torch.stack(band_tangents).mean()
        if band_tangents
        else proposer_loss.new_zeros(())
    )
    ...
    return {
        ...
        "goal/direction_tangent_mse": tangent_loss.item(),
...
def evaluate_direct_policy(agent, cached_goal, args, device, run_name):
    ...
    while len(returns) < args.eval_envs:
        action, _, _ = agent.direct_action(
...
def main():
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        for step in range(rollout_steps):
            primitive_action = explorer.action()
            if global_step < args.random_warmup_steps:
                ...
                with torch.no_grad():
                    ...
                    policy_action, current_goal, _ = agent.direct_action(
