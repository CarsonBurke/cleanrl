# Pan Goal Solver v17: v16 made pure LeJEPA — no EMA teacher, attached target.
#
# v16 (unchanged here): gait-frequency explorer periods (6-30 steps),
# additive forward-velocity aspiration min((v_fwd + delta)/v_fwd, cap) for
# forward anchors, and a nonlinear follower action head over (belief, bounded
# goal delta) — each fixing a failure measured in the v14 run at 1.1M steps.
# v17 removes the EMA target encoder entirely. LeJEPA's SIGReg is precisely
# the anti-collapse mechanism that makes EMA/stop-gradient teachers
# unnecessary, so the transition-prediction target is now the online encoder,
# attached: gradient flows through both branches of the prediction loss.
# All goal coordinates (hindsight goals, rate-head labels, anchor encoding)
# come from the same online encoder, detached where the family contract
# requires gradient isolation (follower and reward-head updates must not
# touch the world model). One encoder, one coordinate system, no drift gap
# between the goals the follower trains on and the goal it is commanded.
# Proposer stays nonparametric (best sustained-rate replay anchor); reward
# touches only anchor selection and the detached rate probe.
# There is no PPO, action critic, policy gradient, search, MPC, or rollout.
# Hypothesis: v16's fixes engage the ratchet, and the two-sided LeJEPA
# gradient plus a single unlagged goal coordinate system tightens both the
# representation and goal-conditioning consistency.
