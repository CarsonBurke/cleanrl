...
# Hidden trunk is hypersphered residual LeakyReluSq: stiglu_sphere_v1
# geometry (unit-sphere streams, justnormed branch outputs mixed as
# A + g*(B - A), per-channel gates init ~0.18, justnorm only) with each
# SiTU-GLU replaced by a 0.5-matched LeakyReluSq pair. See lrelusq_res_v1
# for the pair; this is the SiTU-vs-LReluSq ablation on the sphere.
# Hypothesis: with justnorm bounding every contribution, LReluSq's
# saturation-free quadratic (gradient never vanishes) trains as well or
# better than multiplicative SiTU gating at 32x LR.
import os
import random
import time
...
from cleanrl.shared.host_actor import HostLReluSphereActor, make_lrelu_sphere_trunk
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; 32x LR; 1 minibatch; hypersphered residual LeakyReluSq trunk; no advantage normalization")
        ...
        host_actor = HostLReluSphereActor(agent.actor, args.num_envs)
