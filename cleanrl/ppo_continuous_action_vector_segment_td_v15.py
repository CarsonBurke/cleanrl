# Vector segment TD v15: state-action future predictions corrected by vector TD.
# Learned state-dependent joint-action features have exact Beta expectations.
# One frozen real-segment target per rollout, at actual endpoints; horizon
# queries cover the remaining episode. Fixed feature reference prevents policy
# changes from silently redefining the critic. Full batch, no GAE or replay.
import json
import math
import random
import time
...
@dataclass
class Args:
    exp_name: str = 'vector_segment_td_v15'
    seed: int = 1
...
def query_horizons(ages, horizon=1000, fractions=None):
    """One-step/full-remaining anchors and stratified integer queries in between.

    Every integer remaining horizon has nonzero training probability. All rows
    remain in the full fitting batch; this samples queries, not minibatches.
    """
    remaining = (horizon - ages).clamp_min(0)
    if fractions is None:
        fractions = torch.full((ages.shape[0], HORIZON_QUERIES - 2), .5, device=ages.device)
    strata = torch.arange(HORIZON_QUERIES - 2, device=ages.device)[None]
    middle = 1 + ((strata + fractions) / (HORIZON_QUERIES - 2) * remaining[:, None]).long()
    middle = torch.minimum(middle, remaining[:, None])
    return torch.cat((remaining.clamp_max(1)[:, None], middle, remaining[:, None]), -1)
...
def td_segments(components, ends, queries, td_steps=32):
    """Observed vector sums up to each query, with one common real endpoint.

    m is clipped by the next episode end and the rollout boundary. Queries
    shorter than m have fully observed targets and never use continuation.
    """
    ...
    # Short tiled scans avoid the long Inductor SplitScan lowering regression.
    ...
    queried_steps = torch.minimum(queries.view(steps, environments, -1), consumed[..., None])
    stop = positions[..., None] + queried_steps
    values = prefix[(stop - 1).clamp_min(0), environment_index[..., None]]
    before = prefix[(positions - 1).clamp_min(0), environment_index] * (positions > 0)[..., None]
    observed = torch.where((queried_steps > 0)[..., None], values - before[..., None, :], 0.)
    return (observed.flatten(0, 1).to(components.dtype), endpoint_index.flatten(),
            consumed.flatten(), endpoint_ends)


def segment_td_targets(observed, future, remaining_queries, endpoint_ends):
    """One detached multi-step vector TD target, without lambda/GAE mixing."""
    use_future = (remaining_queries > 0) & ~endpoint_ends[:, None]
    return (observed + torch.where(use_future[..., None], future.detach(), 0.)).detach()
...
class ResponseCritic(nn.Module):
    """Learn a state-action latent queried for vector future reward consequences.

    Every query predicts all physical reward components. State-conditioned
    integrable features include higher-order joint actions, with a fixed
    reference measure so their meaning survives changes to the actor.
    """
    def __init__(self, state_dim, action_dim, width=192, horizon=1000):
        ...
        self.features = 2 * action_dim + initial.shape[0]
        ...
        self.baseline_horizon = nn.Linear(2, width, bias=False)
        self.response_horizon = nn.Linear(2, width, bias=False)
        self.baseline_head = nn.Linear(width, self.components)
        self.response_head = nn.Linear(width, self.components * self.features)
    ...
    def hidden(self, observations, age, branch):
        with torch.autocast('cuda', dtype=torch.bfloat16, enabled=observations.dtype == torch.float32):
            hidden = branch(self.inputs(observations, age))
        return hidden.float() if observations.dtype == torch.float32 else hidden

    def horizon_features(self, horizons, dtype):
        h = horizons.to(dtype)
        return torch.stack((h / self.horizon, torch.log1p(h) / math.log1p(self.horizon)), -1)

    def baseline(self, observations, age, horizons):
        hidden = self.hidden(observations, age, self.baseline_trunk)
        query = self.baseline_horizon(self.horizon_features(horizons, hidden.dtype))
        prediction = self.baseline_head(F.silu(hidden[:, None] + query))
        return torch.where((horizons > 0)[..., None], prediction, 0.)

    def coefficients(self, observations, age, horizons):
        hidden = self.hidden(observations, age, self.response_trunk)
        query = self.response_horizon(self.horizon_features(horizons, hidden.dtype))
        prediction = self.response_head(F.silu(hidden[:, None] + query)).view(
            -1, horizons.shape[1], self.components, self.features)
        return torch.where((horizons > 0)[..., None, None], prediction, 0.)

    def powers(self, observations, age):
        hidden = self.hidden(observations, age, self.response_trunk)
        logits = self.power_head(hidden).view(-1, *self.power_mask.shape)
        return 4 * logits.sigmoid() * self.power_mask

    def response_profile(self, observations, age, horizons, scores, actions, basis_parameters):
        powers = self.powers(observations, age)
        mean, std = kernel_statistics(basis_parameters, powers)
        observed = kernels_observed(actions.double(), powers)
        features = torch.cat((scores.flatten(1), ((observed - mean) / std).to(scores.dtype)), -1)
        return torch.einsum('bhcf,bf->bhc', self.coefficients(observations, age, horizons), features)

    def expected_profile(self, observations, age, horizons, parameters):
        powers = self.powers(observations, age)
        ...
        expected = torch.cat((torch.stack((first, second), -1).flatten(1),
                              (current_mean - basis_mean) / basis_std), -1)
        response = torch.einsum('bhcf,bf->bhc', self.coefficients(observations, age, horizons),
                                expected.to(observations.dtype))
        return self.baseline(observations, age, horizons) + response
...
def main():
    ...
    query_rng = torch.Generator(device=device).manual_seed(args.seed + 800)
    ...
    with ExitStack() as resources:
        ...
        writer.add_text('method', 'Vector real-segment TD; fixed feature reference; learned joint-action powers; exact Beta expectations; fresh full batches; no GAE/replay/scalar value/imagined states/target sweeps.')
        ...
        critic = ResponseCritic(int(np.prod(obs_shape)), agent.action_dim, args.critic_width, horizon).to(device)
        ...
        def prediction(obs, age, queries, scores, actions, anchor):
            return critic.baseline(obs, age, queries) + critic.response_profile(
                obs, age, queries, scores, actions, anchor)

        def critic_loss(obs, age, queries, scores, actions, anchor, target, weights, scale):
            residual = prediction(obs, age, queries, scores, actions, anchor) - target
            loss = .5 * (residual.square().mean((-1, -2)) * weights).sum() / (weights.sum() * scale)
            return loss

        def continuation(obs, age, queries, logits):
            alpha, beta = (F.softplus(logits) + 1).chunk(2, -1)
            parameters = torch.stack((alpha, beta), -1)
            return critic.expected_profile(obs, age, queries, parameters)

        def actor_coefficients(obs, age):
            # Final actor objective only: full remaining horizon, signed components.
            queries = (horizon - age).clamp_min(0)[:, None]
            return critic.coefficients(obs, age, queries)[:, 0].sum(1)
        ...
        age_function, queries_function = resolved_ages, query_horizons
        segment_function = td_segments
        target_function = segment_td_targets
        statistics_function, credit_function = kernel_statistics, response_credit
        power_function = critic.powers
        if args.compile:
            ...
            queries_function = torch.compile(queries_function, fullgraph=True, mode=args.compile_mode)
            ...
            power_function = torch.compile(power_function, fullgraph=True, mode=args.compile_mode)
        ...
        for iteration in range(1, iterations + 1):
            ...
            with timer.span('targets'), torch.no_grad():
                ...
                age = age.clamp_min(0)
                fractions = torch.rand(batch_size, HORIZON_QUERIES - 2, device=device, generator=query_rng)
                queries = queries_function(age, horizon, fractions).clone()
                observed, endpoint_index, consumed, endpoint_ends = (x.clone() for x in segment_function(
                    components.view(args.num_steps, args.num_envs, -1), ends_tensor, queries, args.td_steps))
                endpoint_observations = next_observations[endpoint_index]
                remaining_queries = (queries - consumed[:, None]).clamp_min(0)
                # All real-segment continuations snapshot the same pre-fit critic.
                next_logits = policy_model(endpoint_observations).clone()
                future = continuation(endpoint_observations, age + consumed, remaining_queries, next_logits).clone()
                target = target_function(observed, future, remaining_queries, endpoint_ends).clone()
                ...
                scores, basis_factor, anchor = (x.clone() for x in basis_function(native_actions))
            ...
            with timer.span('update'):
                ...
                for _ in range(args.critic_epochs):
                    ...
                    loss = critic_loss(observations, age, queries, scores, native_actions, anchor, target, weights, scale)
                    loss.backward()
                ...
                with torch.no_grad():
                    if args.compile:
                        torch.compiler.cudagraph_mark_step_begin()
                    fitted = prediction(observations, age, queries, scores, native_actions, anchor).clone()
                    residual = fitted - target
                    diagnostics['critic/td_residual_rms'] = (residual.square().mean((-1, -2)) * weights).sum().div(weights.sum()).sqrt()
                    diagnostics['critic/immediate_residual_rms'] = (residual[:, 0].square().mean(-1) * weights).sum().div(weights.sum()).sqrt()
                    diagnostics['critic/remaining_residual_rms'] = (residual[:, -1].square().mean(-1) * weights).sum().div(weights.sum()).sqrt()
                    coefficients = actor_coefficients(observations, age).clone()
                    powers = power_function(observations, age).clone()
        ...
        report = dict(args=vars(args), status='cancelled' if stop_requested else 'completed',
                      ...
                      objective='Finite-horizon undiscounted vector action value, one frozen real-segment TD target per fresh rollout',
