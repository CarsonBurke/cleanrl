# PPO + IterThink v44 (PLAIN soft GAE -- drop the rank-shaping). From v43. Clean soft-actor-critic structure:
...
# ONE algorithmic change from v43 (a clean single-variable test):
#   (1) adv_transform "rankgauss_signmag" -> "v10": NO rank-shaping. v34's rank transform replaced the
#       advantage with the Gaussian quantile of its global rank -- outlier-immune but MAGNITUDE-DISCARDING.
#       Plain A_soft (z-scored by norm_adv, standard PPO) preserves magnitude. PRIME SUSPECT in the Walker drop.
# Entropy stays in BOTH channels exactly as v43 (NOT target-only): the soft target carries FUTURE entropy
# and the direct -alpha*H(s_t) actor term carries CURRENT entropy. direct_entropy is KEPT True because it is
# the ONLY channel where the dual's alpha is live within a rollout (the soft target bakes alpha_r once,
# pre-epoch, and never refreshes; with dual_freq="epoch" the per-mb read of alpha is already epoch-granular).
# v44 also adds toggleable instrumentation (--diag): raw A_soft moments, reward- vs entropy-return std split,
# pre-tanh |z| quantiles, policy mu/sigma min-max, and separate mean-head vs logvar-head grad norms.
...
class Args:
    ...
    target_entropy_coef: float = 0.0  # v41: REACHABLE per-dim entropy target (target = 0 nats). On-policy
                                    #      squashed H descends THROUGH 0 (the v32 free-descent level), so the
                                    #      dual gets a TWO-SIDED gradient and EQUILIBRATES alpha at a positive
                                    #      value instead of bleeding to the floor (v40's -|A| was unreachable).
                                    #      With entropy now in the C51 critic, a live alpha keeps a meaningful,
                                    #      dynamically-tuned future-entropy term in the soft value. Resolved
                                    #      target = 0.0*dim = 0 for all envs.
    alpha_lr: float = 1e-3
