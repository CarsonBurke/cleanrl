# PPO + IterThink v47 (live SAC temperature + state-entropy soft labels). From v46.
# Goal: implement SAC's max-entropy policy-gradient decomposition without letting PPO advantage
# normalization silently erase the current-entropy actor term.
...
# Changes from v46:
#   (1) Target entropy moves from unreachable -1.0*dim to -0.5*dim. v46's alpha path was intentionally
#       inert; v47 keeps the target below v44's over-exploratory 0 target while giving the dual a chance
#       to bind before the entropy channel becomes decorative.
#   (2) The soft critic label uses a state entropy estimate H_pi(s_{t+1}) from the rollout entropy buffer,
#       not -logpi of the successor action that happened to drive the next transition. This preserves SAC's
#       expected next-state entropy label and removes avoidable cross-action Monte Carlo noise.
#   (3) The direct current-state entropy term is divided by the shaped soft-advantage std when norm_adv is on.
#       PPO standardizes the policy advantage to unit scale; without the same conversion, alpha/return_std
#       is ~1e-3 and the direct SAC actor term is effectively erased.
#   (4) Default advantage shaping moves from rankgauss_signmag to tanh_std. Rankgauss was empirically robust
#       but not SAC-aligned because it discards additive reward/entropy magnitude. tanh_std keeps the raw
#       advantage sign and preserves local magnitude while bounding heavy tails by critic uncertainty.
...
class Args:
    ...
    # auto-tuned by SAC's log-alpha dual on the squashed policy entropy.
    ...
    target_entropy_coef: float = -0.5  # v47: middle target between v44's over-binding 0 and v46's inert -dim.
                                    #      The goal is a live SAC temperature that resists premature collapse
                                    #      without forcing Walker/Hopper to stay at the high-entropy regime that
                                    #      blocked convergence. target = -0.5*dim (-3 HC/Walker, -1.5 Hopper).
    alpha_lr: float = 5e-4          # v47: slower than v46; enough to bind, less Adam sign-stepping on reused PPO data.
    alpha_init: float = 0.1         # SAC-ish init (RAW units now -- alpha enters as alpha/return_std).
    alpha_min: float = 0.02         # v47: keep a real live floor through the one-sided pre-bind phase.
    ...
    scale_direct_entropy_by_adv: bool = True  # v47: norm_adv converts advantages to unit scale; divide the direct
                                    #      entropy coefficient by shaped A_soft.std() so alpha_eff remains in the same
                                    #      actor-loss units as the policy gradient.
    ...
    adv_transform: str = "tanh_std" # v47: SAC-aligned robust default. Unlike rankgauss, this keeps the raw
                                    #      advantage sign and preserves local magnitude, but bounds heavy tails by
                                    #      scaling with the distributional critic's state uncertainty sigma(s).
    ...
    norm_adv_center: bool = False   # v47: std-only normalization preserves the raw/shaped advantage sign.
                                    #      Mean-centering after rankgauss_signmag can flip small advantages,
                                    #      undoing the sign-correct transform. Set true for vanilla PPO parity.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        def _step_dual(entropy_mean):
            # SAC dual in log-alpha parameterization. Gradient on log_alpha is (H - target):
            # when H > target (entropy too high) alpha falls; when H < target it rises.
            # Unlike the alpha-scaled variant, this can recover from the floor promptly.
            alpha_loss = log_alpha * (entropy_mean - target_entropy)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.norm_adv:
                    if args.norm_adv_scope == "batch":
                        ...
                        if auto_alpha and args.direct_entropy and args.scale_direct_entropy_by_adv:
                            ...
                            direct_entropy_norms.append(float(mb_direct_entropy_norm.item()))
                        if args.norm_adv_center:
                ...
                with torch.no_grad():
                    clipfracs_low += [(ratio < 1.0 - args.clip_coef).float().mean().item()]
                    clipfracs_high += [(ratio > 1.0 + clip_hi).float().mean().item()]
                    clipfracs += [
                ...
                if auto_alpha and args.direct_entropy:
                    ...
                    direct_entropy_coefs.append(float(direct_entropy_coef.item()))
        ...
        with torch.no_grad():
            if auto_alpha and args.soft_adv:
                ...
                direct_entropy_coef_log = (
                    float(np.mean(direct_entropy_coefs))
                    if direct_entropy_coefs
                    else float(((alpha_r / return_std) / direct_entropy_norm).item())
                )
                direct_entropy_norm_log = (
                    float(np.mean(direct_entropy_norms)) if direct_entropy_norms else float(direct_entropy_norm.item())
                )
            else:
        ...
        writer.add_scalar("losses/clipfrac_low", np.mean(clipfracs_low), global_step)
        writer.add_scalar("losses/clipfrac_high", np.mean(clipfracs_high), global_step)
