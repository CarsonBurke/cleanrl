# PPO + IterThink v24 beta d4-HL-Gauss tokenized symlog-observation critic v2.
#
# Variant hypothesis: replace Gym's stateful observation normalization/clipping
# with a per-observation token interface:
#   raw obs_i -> non-shared Linear_i(1, L) -> per-token RMSNorm(no affine)
#             -> flatten tokens -> learned projection into the IterThink backbone width.
# No observation normalization, symlog observation transform, mean pooling, or
# attention readout is used. Reward normalization/clipping, value support, PPO,
# and the beta actor stay matched to the source run.
#
# Base run: HalfCheetah-v4__iterthink_v24_beta_d4hlgauss_symlog_v1_hc8m__1__1780787886.
#
# Derived from the wrapper-only symlog-observation v1 control.
#
...
class ObsTokenStem(nn.Module):
    """Per-observation affine token encoder, per-token RMSNorm, flatten, then project."""

    def __init__(self, obs_dim, token_dim, out_dim):
        super().__init__()
        self.obs_dim = obs_dim
        self.token_dim = token_dim
        self.obs_weight = nn.Parameter(torch.empty(obs_dim, token_dim))
        self.obs_bias = nn.Parameter(torch.empty(obs_dim, token_dim))
        self.token_norm = nn.RMSNorm(token_dim, elementwise_affine=False)
        self.out_proj = layer_init(nn.Linear(obs_dim * token_dim, out_dim))
        nn.init.normal_(self.obs_weight, mean=0.0, std=1.0)
        nn.init.normal_(self.obs_bias, mean=0.0, std=0.02)

    def encode_tokens(self, x):
        x = x.reshape(x.shape[0], self.obs_dim, 1)
        tokens = x * self.obs_weight.unsqueeze(0) + self.obs_bias.unsqueeze(0)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        token_dim = args.obs_token_dim
        if self.share_backbone:
            # One token stem + trunk feeds both heads (computed once per forward).
            self.obs_stem = ObsTokenStem(obs_dim, token_dim, H)
            self.trunk = ThinkTrunk(H, H, args.k_blocks, args.n_experts)
        else:
            self.critic_obs_stem = ObsTokenStem(obs_dim, token_dim, H)
            self.actor_obs_stem = ObsTokenStem(obs_dim, token_dim, H)
            self.critic_trunk = ThinkTrunk(H, H, args.k_blocks, args.n_experts)
            self.actor_trunk = ThinkTrunk(H, H, args.k_blocks, args.n_experts)
    ...
    def _trunks(self, x):
        ...
        if self.share_backbone:
            feat = self.trunk(self.obs_stem(x))
            return feat, feat
        return self.actor_trunk(self.actor_obs_stem(x)), self.critic_trunk(self.critic_obs_stem(x))
    ...
    def actor_parameters(self):
        ...
        if self.share_backbone:
            trunk = list(self.obs_stem.parameters()) + list(self.trunk.parameters())
        else:
            trunk = list(self.actor_obs_stem.parameters()) + list(self.actor_trunk.parameters())
        if self.actor_dist == "gaussian":
    ...
    def critic_parameters(self):
        ...
        if self.share_backbone:
            ...
            trunk = list(self.critic_obs_stem.parameters()) + list(self.critic_trunk.parameters())
        return trunk + list(self.critic_head.parameters())
