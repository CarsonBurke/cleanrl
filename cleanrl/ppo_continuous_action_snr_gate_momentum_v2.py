# Momentum-mean uncertainty gate on base PPO, v2.
# Gate = mu^2 / (mu^2 + tau * (rho_t * EMA_spread + eps)), tau=1.
# rho_t = sum(w_i^2) = (1-beta1)/(1+beta1) * (1+beta1^t)/(1-beta1^t),
# for normalized EMA weights. It starts at 1 and tends to 1/19 for beta1=0.9.
# Only change from v1: scale the matched-moment spread by this finite-history
# factor; numerical epsilon, Adam preconditioning and base PPO stay unchanged.
# Hypothesis: uncertainty of the averaged gradient is lower than single-sample
# noise, so useful coordinates survive without boosting any step above Adam.
# This is an IID stationary plug-in approximation, NOT calibrated uncertainty:
# EMA_spread is finite-sample biased; PPO correlations and drift are unmodeled.
import os
import random
import time
...
class Args:
    ...
    gate_tau: float = 1.0
    """SNR needed for half-strength updates"""
    gate_eps: float = 1e-12
    """numerical variance floor in squared-gradient units, not Adam epsilon"""
    num_envs: int = 32
...
def ema_mean_variance_factor(beta, bias_correction):
    """Sum of squared normalized EMA weights; caller supplies 1 - beta**t, t>=1."""
    return ((1.0 - beta) / (1.0 + beta)) * (2.0 - bias_correction) / bias_correction


def uncertainty_gate(mean, second_moment, tau, eps, variance_factor):
    """Gate the mean using IID plug-in uncertainty; leave the numerical floor unscaled."""
    signal = mean.square()
    # Matched weights imply E[g^2] >= E[g]^2. Only FP32 roundoff can violate this.
    variance = (second_moment - signal).clamp_min(0.0)
    # Algebraically snr/(snr+tau), without an intermediate division or overflow.
    return signal / (signal + tau * (variance_factor * variance + eps))


def snr_adam_update(params, grads, moments, seconds, gate_seconds, steps, lr,
                    beta1, beta2, eps, gate_tau, gate_eps):
    for parameter, gradient, moment, second, gate_second, step in zip(
        params, grads, moments, seconds, gate_seconds, steps
    ):
        step.add_(1)
        moment.lerp_(gradient, 1.0 - beta1)
        second.mul_(beta2).addcmul_(gradient, gradient, value=1.0 - beta2)
        # Gate moments share beta1; Adam's slow beta2 second moment is NOT a
        # valid variance partner for its fast first moment after a gradient jump.
        gate_second.mul_(beta1).addcmul_(gradient, gradient, value=1.0 - beta1)
        correction1 = 1.0 - beta1 ** step
        mean = moment / correction1
        variance_factor = ema_mean_variance_factor(beta1, correction1)
        gate = uncertainty_gate(mean, gate_second / correction1, gate_tau, gate_eps, variance_factor)
        denominator = (second / (1.0 - beta2 ** step)).sqrt().add_(eps)
        parameter.add_(-lr * gate * mean / denominator)


class SNRGatedAdam(optim.Optimizer):
    """Adam preconditioning gated by the IID plug-in variance of its momentum mean."""

    def __init__(self, params, lr=3e-4, betas=(0.9, 0.999), eps=1e-5,
                 gate_tau=1.0, gate_eps=1e-12, compile=True):
        params = list(params)
        if not params or not np.isfinite(lr) or lr < 0:
            raise ValueError("SNRGatedAdam requires parameters and a nonnegative finite LR")
        if not all(np.isfinite(value) and value > 0 for value in (eps, gate_tau, gate_eps)):
            raise ValueError("Adam epsilon, gate tau and gate epsilon must be positive and finite")
        if not all(0 <= beta < 1 for beta in betas):
            raise ValueError("SNRGatedAdam betas must be in [0, 1)")
        device = params[0].device
        if device.type != "cuda" or any(parameter.device != device for parameter in params):
            raise ValueError("SNRGatedAdam requires parameters on one CUDA device")
        super().__init__(params, dict(lr=torch.tensor(lr, device=device), betas=betas,
                                     eps=eps, gate_tau=gate_tau, gate_eps=gate_eps))
        self._update = (
            torch.compile(snr_adam_update, fullgraph=True, dynamic=False, options={"triton.cudagraphs": False})
            if compile else snr_adam_update
        )
        ...
        for group in self.param_groups:
            ...
            for parameter in group["params"]:
                ...
                if parameter.grad.is_sparse:
                    raise RuntimeError("SNRGatedAdam requires dense gradients")
                state = self.state[parameter]
                if not state:
                    state["step"] = torch.zeros((), device=parameter.device)
                    state["exp_avg"] = torch.zeros_like(parameter)
                    state["exp_avg_sq"] = torch.zeros_like(parameter)
                    state["gate_exp_avg_sq"] = torch.zeros_like(parameter)
                params.append(parameter)
                grads.append(parameter.grad)
                moments.append(state["exp_avg"])
                seconds.append(state["exp_avg_sq"])
                gate_seconds.append(state["gate_exp_avg_sq"])
                steps.append(state["step"])
            if params:
                beta1, beta2 = group["betas"]
                self._update(params, grads, moments, seconds, gate_seconds, steps, group["lr"],
                             beta1, beta2, group["eps"], group["gate_tau"], group["gate_eps"])
        return loss

    @torch.no_grad()
    def gate_statistics(self, parameters):
        """Coordinate-weighted final-update gate diagnostics for this single-group trainer."""
        group = self.param_groups[0]
        beta1 = group["betas"][0]
        summaries, count = [], 0
        for parameter in parameters:
            state = self.state[parameter]
            correction = 1.0 - beta1 ** state["step"]
            gate = uncertainty_gate(state["exp_avg"] / correction,
                                    state["gate_exp_avg_sq"] / correction,
                                    group["gate_tau"], group["gate_eps"],
                                    ema_mean_variance_factor(beta1, correction))
            summaries.append(torch.stack((gate.sum(), (gate < 0.1).float().sum())))
            count += parameter.numel()
        return torch.stack(summaries).sum(dim=0) / count
...
def main():
    ...
    try:
        ...
        optimizer = SNRGatedAdam(agent.parameters(), lr=args.learning_rate,
                                 gate_tau=args.gate_tau, gate_eps=args.gate_eps, compile=args.compile)
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            actor_gate = optimizer.gate_statistics(actor_parameters)
            critic_gate = optimizer.gate_statistics(critic_parameters)
            logged = gather_metrics({
                ...
                "gate/actor_mean": actor_gate[0], "gate/actor_below_0p1": actor_gate[1],
                "gate/critic_mean": critic_gate[0], "gate/critic_below_0p1": critic_gate[1],
            })
