# ============================================================================
# DG v8 -- v7's analytic-KL penalty + a HARD PER-MINIBATCH KL CAP (the spike fix).
#
# WHY (v7's failure mode). v7's adaptive-beta soft penalty controls the MEAN KL well (median
# ~0.017 ~ target 0.02) but CANNOT bound the MAX: beta adapts only ONCE per iteration (after the
# full 32-minibatch pass) and is bang-bang (x2/2). When beta walks to its floor and the advantage
# aligns, a single pass blows through to KL~0.2-1.0 (up to 50x target) before beta can react; beta
# then doubles, over-suppresses, walks down, and the cycle repeats -- "almost periodic" spikes.
# A soft penalty regulates the AVERAGE; it provably cannot cap the per-update MAX.
#
# FIX. Keep the soft penalty (it handles the typical step), and ADD trust-region ENFORCEMENT: the
# exact analytic KL is checked EVERY minibatch and the actor pass STOPS the moment KL exceeds a
# ceiling (kl_target * kl_cap_ratio). This is the robust "KL early-stopping" ingredient (Engstrom
# 2020) -- conceptually TRPO's reject-KL-violating-steps -- but on the EXACT Beta KL and checked
# per-minibatch (v7's per-epoch target_kl check fires only AFTER all 32 steps, far too late). The
# soft penalty bounds the mean; the hard cap bounds the max. Together = a real trust region.
# ----------------------------------------------------------------------------
...
class Args:
    ...
    kl_cap_ratio: float = 3.0        # HARD cap: stop the actor pass once a minibatch's exact KL
    #                                  exceeds kl_target*kl_cap_ratio -> bounds the per-update MAX
    #                                  KL (kills the soft-penalty spikes). 0 disables the cap.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.actor_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # HARD CAP: the exact KL already exceeds the ceiling -> STOP the actor pass before
                # stepping further. Bounds the per-update MAX KL -- the spike the soft penalty can't.
                if args.kl_trust and args.kl_cap_ratio > 0 and kl.item() > kl_cap:
