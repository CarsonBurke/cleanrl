# ============================================================================
# DG v25 -- noadvnorm / no-reward-norm base + compile + v24 edges HL-Gauss ablations.
#
# Algorithm base = dg_v23_noadvnorm (…1784850185):
#   raw rewards, no reward_rms, no advnorm, no retnorm, no DG gate, strong L2-hinge KL.
#
# On top:
#   * torch.compile(agent, mode="reduce-overhead", dynamic=False)  (CUDA graphs)
#   * v24 HL-Gauss: HLGaussSupport edges, ±20k via symlog coords, use_symlog, no MTP
#   * --critic-decode expected_scalar | scalar
#       expected_scalar = to_expected_scalar = E[symexp(z)]   (v24 / v215 default)
#       scalar          = to_scalar          = symexp(E[z])
#
# Prescribed ablations:
#   1) sigma=0.5, bins=511, expected_scalar
#   2) sigma=0.5, bins=51,  expected_scalar
#   3) sigma=0.5, bins=51,  scalar
# ----------------------------------------------------------------------------
# DG v24 -- edges HL-Gauss + reduce-overhead compile.
# ----------------------------------------------------------------------------
# DG v21 -- PURE L2-HINGE KL PENALTY trust region (penalty is the ONLY KL control).
#
# v18 carried THREE overlapping KL controls: (1) a soft penalty kl_beta*kl, (2) a per-minibatch
# step-throttle (kl_step_scale, ramp pg_loss->0 as KL nears kl_cap), and (3) adaptive kl_beta.
# v21 strips this down to a SINGLE mechanism: a soft KL PENALTY only -- no step-throttle, no
# post-hoc rescale, no hard early-stop -- and reshapes that penalty from linear to an L2 HINGE.
#
# WHY (the linear penalty's flaw). kl_beta*kl is ALWAYS-ON: it penalizes even tiny, on-target KL,
# so in the normal regime the actor is fighting a restoring force it doesn't need, and the term's
# magnitude (kl_beta can reach the hundreds) makes the loss landscape near zero KL high and
# confusing -- it distorts the small, well-behaved updates we WANT to take freely.
#
# WHAT (the L2 hinge). Penalty is EXACTLY ZERO below a deadband kl_pen_start (> 0.02, default 0.03)
# and grows QUADRATICALLY in the KL excess above it:
#       kl_excess = max(KL - kl_pen_start, 0);   kl_pen = kl_beta * kl_excess^2 .
# Below the deadband: free movement (zero penalty, undistorted small updates). Above it: a strong,
# rapidly-stiffening restoring force that pulls the step back toward the cap -- without ever
# throttling, rescaling, or aborting the update. The torch.clamp(min=0) keeps the gradient live in
# the active (excess>0) region and zeroes it inside the deadband -- exactly the hinge we want. The
# gradient flows through `kl` (computed from the CURRENT params vs stored old alpha,beta), so the
# penalty directly steers the policy back under the cap.
#
# CONFIG (penalty-only). kl_cap_ratio=0.0 fully disables the throttle/ceiling block (guarded by
# kl_cap_ratio>0); kl_step_scale=False; kl_target=0.03 so the adaptive kl_beta regulation target
# EQUALS the deadband -> realized KL settles right at the deadband edge. kl_trust stays True
# (penalty on) and the adaptive kl_beta machinery is kept -- it now tunes the hinge STRENGTH.
# kl_pen_form="linear" reproduces the exact old kl_beta*kl behavior for ablation.
# Launched with --no-dg-use-gate (pure PPO score, gate out of the picture). Everything else == v18.
# ----------------------------------------------------------------------------
...
from cleanrl.shared.hl_gauss import HLGaussSupport, Dreamer3BucketHLGaussSupport, symexp
...
class Args:
    ...
    hf_entity: str = ""
    # v24: monolithic agent compile (CUDA graphs when mode=reduce-overhead)
    compile: bool = True
    ...
    norm_adv: bool = False           # v24 default: no advnorm (winner of v23 ablations)
    ...
    # RIPO / RIC (betaplast_ripo_v1, Cai et al.): density-dependent ratio window
    #   |ρ-1| ≤ √(δ / π_eff), π_eff = exp(logπ / act_dim), dual-bounded [dual_clip_low, dual_clip_high].
    use_ric_clip: bool = False
    ric_sigma: float = 0.05          # δ in √(δ / π_eff) (betaplast_ripo_v1 default)
    dual_clip_low: float = 0.5       # hard floor on ratio_low
    dual_clip_high: float = 10.0     # hard ceil on ratio_high
    ric_density_floor: float = 1e-12
    # --- Analytic-KL trust region (v24 defaults = strong_mb_nodg_v2) ---
    kl_trust: bool = True            # add the KL penalty (exact Beta KL) to actor loss
    kl_target: float = 0.022         # adaptive-beta regulation target == deadband edge
    kl_beta_init: float = 50.0       # initial penalty coefficient beta
    kl_beta_min: float = 0.1         # adaptive-beta clamp (lower)
    kl_beta_max: float = 2000.0      # adaptive-beta clamp (upper)
    # v21: L2-HINGE penalty shape. penalty = 0 below kl_pen_start (deadband, MUST be > 0.02), then
    # kl_beta * (KL - kl_pen_start)^2 above it -- quadratic in the excess. "linear" reproduces the
    # old always-on kl_beta*kl for ablation.
    kl_pen_form: str = "l2_hinge"    # "l2_hinge" | "l4_hinge" (quartic, normalized excess) | "linear"
    kl_pen_start: float = 0.022      # deadband edge: zero penalty below this. MUST be > 0.02.
    # v21: step-throttle / ceiling DISABLED. kl_cap_ratio=0.0 fully skips the throttle block (guarded
    # by kl_cap_ratio>0); the soft penalty is the sole KL mechanism (no throttle, rescale, hard-stop).
    kl_cap_ratio: float = 0.0        # 0 disables the throttle/ceiling block entirely (v21 penalty-only).
    kl_step_scale: bool = False      # v21: OFF. No per-minibatch pg-step throttle.
    ...
    ret_ema_norm: bool = False       # OFF -- no return norm
    ...
    # HL-Gauss critic: ±20000 raw via symlog coords. Grid is either edges HLGauss (v24) or
    # Dreamer3Bucket (v23). Decode is independent (GAE only; CE still uses project()).
    critic_d3bucket: bool = False    # True -> v23 Dreamer3 raw-center buckets (odd bins, exact 0)
    critic_num_bins: int = 511
    critic_v_min: float = -9.90353755128617  # symlog(-20000)
    critic_v_max: float = 9.90353755128617   # symlog(+20000)
    critic_sigma_ratio: float = 0.5  # ablation default (v24 originally used 2.0; noadvnorm D3 used 0.75)
    critic_symlog: bool = True
    critic_support_is_edges: bool = True  # only for non-D3 HLGaussSupport
    # Value decode for GAE / bootstrap:
    #   "expected_scalar" -> E[raw] = E[symexp(z)]  (D3 native to_scalar)
    #   "scalar"          -> symexp(E[z])
    critic_decode: str = "expected_scalar"
    reward_norm: bool = False        # raw rewards (no gym NormalizeReward / clip)
    # v23: return-agnostic reward scale. r <- r / sqrt(EMA(r^2) + eps). No mean, no clip, no return stats.
    reward_rms: bool = False
    reward_rms_decay: float = 0.999  # EMA decay on second moment; higher = slower / more stable scale
    reward_rms_eps: float = 1e-8
    reward_rms_init: float = 1.0     # cold-start second moment (avoids huge first-step scale)

    # DG-specific (v24 default: gate OFF -- pure score + KL, matching noadvnorm winner)
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, z=None):
        """Returns (z_native, action, logp, ell, entropy, value_logits, alpha, beta).

        Tensor-only outputs (no Beta dist object) so torch.compile reduce-overhead / CUDA graphs
        stay pure. Caller rebuilds Beta(alpha, beta) only where analytic KL is needed.
        """
        ...
        action = 2.0 * z - 1.0
        return z, action, logp, ell, entropy, self.get_value(x), alpha, beta
...
if __name__ == "__main__":
    ...
    assert args.critic_decode in ("expected_scalar", "scalar"), f"bad critic_decode {args.critic_decode}"
    if args.use_ric_clip and args.use_ratio_clip:
        print("[v25] warning: use_ric_clip and use_ratio_clip both set; RIC takes precedence")
    ...
    # Train with CE on project() labels. Decode for GAE selected by critic_decode. No MTP.
    if args.critic_d3bucket:
        ...
        hlg = HLGaussSupport(
            ...
            support_is_edges=args.critic_support_is_edges,
        )

    def value_logits_to_scalar(logits):
        # expected_scalar: E[raw centers]; scalar: symexp(E[z])
        if args.critic_d3bucket:
            if args.critic_decode == "scalar":
                probs = torch.softmax(logits, dim=-1)
                e_z = (probs * hlg.coord_support).sum(dim=-1)
                return symexp(e_z)
            return hlg.to_scalar(logits)  # E[raw] (D3 native)
        if args.critic_decode == "scalar":
            return hlg.to_scalar(logits)
        return hlg.to_expected_scalar(logits)
    ...
    # Optimizers bind to the eager module params before compile wraps the module.
    ...
    if args.compile:
        # Monolithic agent compile; reduce-overhead enables CUDA graphs for fixed shapes
        # (rollout B=num_envs, update B=minibatch_size).
        agent = torch.compile(agent, mode=args.compile_mode, dynamic=False)
        grid = "d3bucket" if args.critic_d3bucket else "edges"
        print(
            f"[v25] torch.compile(agent, mode={args.compile_mode!r}, dynamic=False); "
            f"hlg grid={grid} bins={args.critic_num_bins} sigma={args.critic_sigma_ratio} "
            f"decode={args.critic_decode}"
        )
    ...
    # v23: global EMA of reward second moment (shared across envs/steps).
    reward_m2 = float(args.reward_rms_init)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                z, action, logprob, _, _, value_logits, alpha, beta = agent.get_action_and_value(next_obs)
            ...
            if args.reward_rms:
                # Update second-moment EMA on RAW rewards, then scale (mean-free, return-agnostic).
                batch_m2 = rewards[step].float().pow(2).mean().item()
                d = 1.0 - args.reward_rms_decay
                reward_m2 = args.reward_rms_decay * reward_m2 + d * batch_m2
                rewards[step] = rewards[step] / (reward_m2 ** 0.5 + args.reward_rms_eps)
        ...
        with torch.no_grad():
            if args.compile:
                torch.compiler.cudagraph_mark_step_begin()
            next_value = value_logits_to_scalar(agent.get_value(next_obs)).reshape(1, -1)
        ...
        kl_pen_terms = []  # v21: per-minibatch L2-hinge KL penalty magnitude (for logging)
        ...
        for epoch in range(args.actor_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                _, _, newlogprob, ell, entropy, _, new_alpha, new_beta = agent.get_action_and_value(
                ...
                if args.use_ric_clip:
                    # RIPO / RIC (betaplast_ripo_v1): density-dependent isometric ratio clip.
                    # Low-π samples get a wider window; high-π get a tighter one. Dual hard bounds.
                    with torch.no_grad():
                        log_pi_eff = b_logprobs[mb_inds] / act_dim
                        pi_eff = log_pi_eff.exp().clamp_min(args.ric_density_floor)
                        ric_eps = (args.ric_sigma / pi_eff).sqrt()
                        ratio_low = (1.0 - ric_eps).clamp_min(args.dual_clip_low)
                        ratio_high = (1.0 + ric_eps).clamp_max(args.dual_clip_high)
                    surr1 = -mb_adv * ratio
                    surr2 = -mb_adv * torch.clamp(ratio, ratio_low, ratio_high)
                    pg_loss = (w * torch.max(surr1, surr2)).mean()
                elif args.use_ratio_clip:
                    # v6 ablation: PPO clipped surrogate (leaky proxy trust region), gated by w.
                    clip_hi = args.clip_coef if args.clip_coef_high is None else args.clip_coef_high
                    ratio_low = 1.0 - args.clip_coef
                    ratio_high = 1.0 + clip_hi
                    surr1 = -mb_adv * ratio
                    surr2 = -mb_adv * torch.clamp(ratio, ratio_low, ratio_high)
                    pg_loss = (w * torch.max(surr1, surr2)).mean()
                else:
                ...
                # v21: L2-HINGE KL penalty -- ZERO below the deadband, quadratic in the excess above
                # it. torch.clamp(min=0) keeps the gradient live for excess>0 and zeroes it inside the
                # deadband; `kl` is grad-carrying (from new_dist = current params), so the penalty
                # gradient flows through it and pulls the policy back under the cap. No throttle/rescale.
                kl_excess = torch.clamp(kl - args.kl_pen_start, min=0.0)
                if args.kl_pen_form == "l4_hinge":
                    # Quartic hinge. Normalize the excess by the deadband so kl_beta stays in the same
                    # numeric range the adaptive controller already lives in -- raw excess^4 ~ 1e-8 would
                    # need kl_beta ~ 1e7. Grows 2x faster in the exponent than l2 => punishes the TAIL
                    # (hot-minibatch spikes) far harder while remaining ~0 in the normal 0.02-0.03 zone.
                    r = kl_excess / args.kl_pen_start
                    kl_pen = kl_beta * r * r * r * r
                elif args.kl_pen_form == "l2_hinge":
                    kl_pen = kl_beta * kl_excess * kl_excess  # L2 (quadratic) in the KL excess
                else:
                    kl_pen = kl_beta * kl                     # linear ablation (old always-on penalty)
                ...
                with torch.no_grad():
                    ...
                    if ratio_low is not None:
                        if torch.is_tensor(ratio_low):
                            clipfracs.append(((ratio < ratio_low) | (ratio > ratio_high)).float().mean().item())
                        else:
                            clipfracs.append(((ratio - 1.0).abs() > args.clip_coef).float().mean().item())
                    else:
                        clipfracs.append(0.0)
                    kl_terms.append(kl.item())
                    kl_pen_terms.append(kl_pen.item())
        ...
        writer.add_scalar("losses/kl_penalty", float(np.mean(kl_pen_terms)) if kl_pen_terms else 0.0, global_step)
        writer.add_scalar("losses/ret_ema_std", ema_ret_std, global_step)
        if args.reward_rms:
            writer.add_scalar("charts/reward_rms", reward_m2 ** 0.5, global_step)
