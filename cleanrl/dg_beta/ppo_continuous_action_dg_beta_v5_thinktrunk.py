# ============================================================================
# DG v5 -- faithful DG (dg_beta_v4 recipe) on iterthink_v24_beta's EXACT architecture.
# Swaps v4's 64x64 tanh MLPs for the iterthink ThinkTrunk (DenseNet stack of K
# residual-gated dense+softMoE blocks, ReLU^2/RMSNorm) + dreamer4 unimodal Beta, as
# SEPARATE actor/critic trunks (share_backbone=False) so the decoupled faithful recipe
# is untouched: HL-Gauss critic, raw U (no adv norm / no rankgauss), no PPO ratio/clip,
# 1 actor score-pass / critic_epochs regression refits, separate optimizers, ent_coef=0.
# HYPOTHESIS: representation capacity -- not the trust region -- is what separates the
# faithful score-function method (~4100) from the PPO+ThinkTrunk base (9490). Default
# dg_use_gate is left as-is, but the WINNING config to compare is --no-dg-use-gate.
#
...
#
# v4: RENORMALIZED GATE (pure reallocation). v1-v3 showed the gate consistently UNDERperforms
# the no-gate control: with mean(w)~0.5 it NET-DAMPS the gradient (and asymmetrically suppresses
# negative-advantage corrections), losing signal in dense on-policy control where every fresh
# sample is useful. v4 rescales the detached gate to mean(w)=1 (`dg_renorm`), so it REDISTRIBUTES
# gradient toward high-delight (rare-breakthrough) samples without shrinking the total -- the
# clean test of whether the gate's DIRECTION helps once its damping is removed. If renorm still
# loses to nogate, the gate's reweighting genuinely doesn't help on-policy (it likely needs the
# paper's redundant replay buffer to pay off).
#
...
class Args:
    ...
    actor_lr: float = None           # actor Adam LR; None -> learning_rate. Lower it to tame the
    #                                  high-capacity ThinkTrunk's over-concentration (faithful step
    #                                  control: bounds the actor step WITHOUT a trust region/KL).
    num_envs: int = 16
    num_steps: int = 128
    anneal_lr: bool = True
    gamma: float = 0.99
    ...
    dg_surprisal: str = "peak_ref"   # "peak_ref" (ell=logp(mode)-logp(a)>=0) | "mahalanobis" (moment-matched 0.5||(z-mu)/sigma||^2: E=0.5D for ANY concentration, incl. uniform) | "raw" (paper -logpi clip; INVERTS on beta)
    dg_eta: float = 1.0              # temperature eta in w = sigmoid(chi/eta)
    dg_clip: float = 10.0            # paper C: clip on the surprisal ell
    dg_renorm: bool = True           # v4: rescale gate to mean(w)=1 -> pure reallocation, no net damping
...
# of K ThinkBlocks (verbatim from cleanrl/iterthink/misc/ppo_continuous_action_iterthink_v24_dist.py).
# Each block: bounded-convex residual gate mixing x_in and x0, a dense pre-act
# MLP branch (RMSNorm + ReLU^2), and a soft (full-softmax, no top-K) MoE branch.
# Block k consumes the concatenation of all prior feats (H*(k+1) wide); the trunk
# output concatenates every feat and projects back to H. Used here as SEPARATE
# actor/critic trunks (iterthink's share_backbone=False variant) so the faithful
# decoupled DG recipe -- 1 actor score-pass, N critic regression refits, separate
# optimizers -- is preserved exactly; only the representation changes.
# ============================================================================
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, num_bins, hidden=64, k_blocks=3, n_experts=16,
        ...
        H = hidden
        # iterthink_v24_beta EXACT backbones: one ThinkTrunk each for actor & critic.
    ...
    def get_action_and_value(self, x, z=None):
        ...
        if getattr(self, "dg_surprisal", "peak_ref") == "mahalanobis":
            # Gaussian-ANALOGOUS moment-matched surprisal: 0.5*||(z-mu)/sigma||^2 using the
            # Beta's OWN mean/std. E[ell] = 0.5*D for ANY concentration (incl. uniform Beta),
            # giving the sigma-independent floor the Gaussian's 0.5||eps||^2 has but the Beta
            # log-density (peak_ref) lacks early on. Tail action -> large; central -> ~0; >=0.
            mean = dist.mean
            std = dist.stddev
...
if __name__ == "__main__":
    ...
    agent.dg_surprisal = args.dg_surprisal  # select the surprisal ell computed in get_action_and_value
        ...
        for epoch in range(args.actor_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if not args.dg_use_gate:
                    w = torch.ones_like(w)
                if args.dg_renorm:
                    # Pure REALLOCATION: rescale so mean(w)=1 -> the gate redistributes gradient
                    # toward high-delight samples WITHOUT the net ~0.5x damping that mean(w)<1
                    # imposes. Detached, so it only rescales magnitude (not the gradient path).
                    w = w / (w.mean() + 1e-8)
