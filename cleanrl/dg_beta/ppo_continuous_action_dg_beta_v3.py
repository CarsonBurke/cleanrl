...
#
# v3: EMA ADVANTAGE-SCALE NORM. NormalizeReward divides rewards by the running std of the
# discounted return; as the policy improves that std grows, COMPRESSING the reward scale, so
# the raw advantages (no return/adv norm) shrink over training -- the score gradient and the
# gate's chi=U*ell decay toward zero and learning stalls late. Fix: divide U by a slow EMA of
# its RMS (no mean-centering), keeping the global advantage scale ~unit throughout while
# preserving the sign, relative magnitudes, and zero anchor the gate needs. See `adv_ema_norm`.
#
...
class Args:
    ...
    norm_adv: bool = False           # per-minibatch standardize (PPO-style; CENTERS -> breaks gate anchor). Off.
    # v3: EMA advantage-SCALE normalization (reward-norm-style). Divide U by a slow EMA of its
    # RMS -- NO mean-centering -- so the global advantage scale stays ~unit over training (the
    # gradient and the gate's chi=U*ell don't shrink as NormalizeReward compresses the reward
    # scale and advantages decay toward the optimum), while sign and relative magnitudes (the
    # gate's breakthrough/blunder signal) and the zero anchor are preserved.
    adv_ema_norm: bool = True
    adv_ema_decay: float = 0.99      # per-iteration EMA decay for the advantage RMS
...
if __name__ == "__main__":
    ...
    global_step = 0
    adv_rms = 1.0  # EMA of the advantage mean-square (reward-norm-style global scale tracker)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # EMA advantage-scale normalization (reward-norm-style): rescale U to ~unit RMS by a
        # slow running second moment. No mean-centering -> sign, relative magnitude and the
        # zero anchor (the gate's breakthrough/blunder reference) are all preserved; only the
        # global scale drift from NormalizeReward / advantage decay is removed.
        if args.adv_ema_norm:
            adv_rms = args.adv_ema_decay * adv_rms + (1.0 - args.adv_ema_decay) * (b_advantages ** 2).mean().item()
            b_advantages = b_advantages / (adv_rms ** 0.5 + 1e-8)
        ...
        gate_means, surp_means, chi_stds, sign_agrees, actor_gns = [], [], [], [], []
        ...
        writer.add_scalar("charts/adv_rms", float(adv_rms ** 0.5), global_step)
