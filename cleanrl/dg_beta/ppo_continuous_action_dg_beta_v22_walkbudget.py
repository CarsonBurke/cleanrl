# ============================================================================
# DG v22 -- WALK-UNTIL-BUDGET + HARD STOP: the KL trust region that fixes BOTH v19 and v20.
#
# THE PROBLEM (from v19/v20). We want the realized per-update KL(pi_old||pi_new) to land at a target
# (~0.03) using a PURE policy-gradient direction -- NO soft KL penalty (the penalty gradient distorts
# the step direction) and NO post-hoc rescale of the step (interpolating params back is a fractional
# move that doesn't correspond to a gradient step). Two prior attempts both failed at opposite extremes:
#   * v19: 32 FULL-scale Adam steps unconstrained, then interpolate params back. The 32 steps WANDER
#     to KL~2.5 (kl_rescale_pre~2.5, analytic_kl_max~4.5): past step ~2 the policy is already far
#     off-policy so the later gradients are garbage. Rescaling kept alpha~0.09 of a BAD direction.
#   * v20: ONE accumulated-gradient Adam step, then rescale. But Adam is SCALE-INVARIANT in the
#     gradient (it divides by sqrt(v)), so a single step moves params by ~lr REGARDLESS of loss
#     scale -> KL~1.5e-5 (2000x UNDERSHOOT). alpha=1, no rescale fires, the policy is FROZEN
#     (return stuck ~-218). You cannot reach KL=0.03 in one Adam step.
# So: 1 step => KL 1e-5 (frozen); 32 unconstrained steps => KL 2.5 (off-policy wander). The budget
# 0.03 lives in between. The MISSING INGREDIENT is HOW FAR to walk along a GOOD (on-policy) direction.
#
# THE FIX (v22, "walk-until-budget + hard stop"). Take NORMAL full-scale Adam steps over the
# minibatches (pure DG-gated score, NO penalty, NO throttle, NO rescale -- the direction and the step
# magnitude are exactly a normal SGD step). After each step (or every --kl-check-every steps) measure
# the EXACT full-batch KL(theta_old||current). The instant a step would carry KL past kl_rescale_target,
# HARD STOP: DISCARD that overshooting step (revert the actor params to the last checkpoint that was
# CONFIRMED inside the budget) and end the pass. No interpolation, no fractional step -- every step we
# keep is a real, full, on-policy gradient step; we simply stop taking them once the budget is spent.
#
# WHY THIS BEATS v19, v20, AND THE ALTERNATIVES.
#   * Follows CURVATURE (vs v20 / single-direction line search). Each step is a FRESH gradient at the
#     current point -- exactly like normal PPO/SGD, a real multi-step trajectory, not one frozen ray
#     extrapolated by alpha~50. v22 never extrapolates and never rescales; it just walks and stops.
#   * Stays ON-POLICY (vs v19). v19 wandered because it took ALL 32 steps; its drift happened FAR
#     past KL=0.03. v22 STOPS at KL<=target, so the policy never moves more than ~target nats from
#     theta_old -- every kept gradient is essentially on-policy. v19's bug was not multi-stepping; it
#     was multi-stepping WAY past the budget. v22 multi-steps and stops AT the budget.
#   * Solves Adam scale-invariance (vs v20). We never rely on ONE step reaching 0.03. We take as MANY
#     ~lr-sized steps as the budget allows. Step count AUTO-ADAPTS: hot early advantages -> few steps
#     to 0.03; cold late advantages (high EV) -> more steps, up to the whole pass (then KL<target and
#     we keep it untouched, exactly like v18's sub-target case). Robust early AND late by construction.
#   * PURE STEPS, NO RESCALE (vs v19/v20). The trust region is enforced purely by step COUNT (a hard
#     stop on the exact realized batch KL), never by a penalty and never by a fractional rescale of a
#     step. With --kl-check-every 1 the kept policy sits one step below target -- a tight, clean bound.
#   * Cost: one extra full-batch actor-trunk forward pass per check (== one critic epoch's forward, and
#     we run 10 of those). --kl-check-every k amortizes it (check every k steps; the kept checkpoint
#     then lags by <k steps, so the realized KL bound is target + up to (k-1) steps' KL). Default k=1.
# (kl_rescale=False == v18 exactly: full pass, soft penalty + step-scale throttle.)
#
# --- inherited v19 notes ---
...
class Args:
    ...
    # --- v22: WALK-UNTIL-BUDGET + TRIM trust region (the fix for v19/v20) ---
    # When ON (--kl-rescale): take normal full-scale Adam steps over the minibatches (pure DG-gated
    # score, NO penalty, NO throttle -> undistorted PG direction). After each step (or every
    # kl_check_every steps) measure the EXACT full-batch KL(theta_old||current). The instant KL
    # reaches kl_rescale_target, STOP the pass and TRIM ONLY the final overshooting step by
    # interpolating  theta <- theta_before + beta*(theta_after - theta_before)  so realized KL lands
    # exactly at target (KL is monotone in beta along the segment; quadratic-seeded + backtracked line
    # search on the exact Beta KL). Earlier steps are KEPT at full scale. Sub-target passes (KL never
    # reaches target across all minibatches) are left untouched == v18. The realized per-update KL is
    # bounded WITHOUT a penalty (direction undistorted) and WITHOUT relying on one Adam step (v20's
    # frozen failure) or on letting 32 steps wander off-policy (v19's failure). One-sided: never up-scales.
    kl_rescale: bool = False         # master switch (disables soft penalty + step throttle when True)
    kl_rescale_target: float = 0.03  # per-update realized-KL budget the walk stops at / the trim lands on
    kl_rescale_iters: int = 8        # trim line-search steps (segment KL is monotone but only locally quadratic)
    kl_check_every: int = 1          # measure exact batch KL every k actor steps (1 = after every step) to
    #                                  amortize the check cost. When an overshoot is detected the trim
    #                                  interpolates the segment from the last CONFIRMED sub-target checkpoint
    #                                  to the overshoot, so the realized KL still lands exactly at target for
    #                                  any k -- k>1 only adds coarseness (the checkpoint may lag by <k steps).
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # v19/v22: snapshot the rollout policy params. v22 measures KL(theta_old||current) as it walks.
        theta_old = [p.detach().clone() for p in actor_params] if args.kl_rescale else None

        # v22: exact full-batch KL(pi_old || pi_current). Iterates the ENTIRE rollout once (the b_inds
        # ORDER is irrelevant -- every sample is visited exactly once), measuring the realized KL of
        # whatever params are CURRENTLY in the actor. Used both as the walk's stop test and the trim's
        # line-search objective. Cost: one actor-trunk forward pass over the 32768-sample batch.
        def _batch_kl():
            tot, n = 0.0, 0
            with torch.no_grad():
        ...
        # v22 diagnostics: realized batch KL before the trim (the walk's final overshoot) and after.
        rescale_alpha, rescale_kl_pre, rescale_kl_post, trimmed = 1.0, 0.0, 0.0, False
        # v22: the last params CONFIRMED sub-target (KL(theta_old||.) < target). The trim interpolates
        # the segment [theta_checkpoint, theta_after], guaranteeing KL(beta=0) < target <= KL(beta=1) so
        # a root in [0,1] exists for ANY kl_check_every (not just 1). Starts at theta_old (KL=0).
        theta_checkpoint = [p.detach().clone() for p in actor_params] if args.kl_rescale else None
        for epoch in range(args.actor_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v22: with the walk-until-budget trust region, the pass is PURE policy gradient (no
                # penalty) -- realized KL is bounded by STOPPING the walk + TRIMMING the last step, not
                # by distorting this gradient. v18 path keeps the soft penalty + step-scale throttle.
                kl_pen = 0.0 if args.kl_rescale else kl_beta * kl
                actor_loss = pg_loss + kl_pen - args.ent_coef * entropy.mean()

                if args.kl_rescale:
                    # v22 WALK-UNTIL-BUDGET + TRIM. Take ONE full-scale Adam step (a fresh on-policy
                    # gradient at the current point -> follows curvature). Every kl_check_every steps
                    # measure the EXACT batch KL(theta_old||current). When it reaches the target the walk
                    # overshot: trim the segment from the last CONFIRMED-sub-target checkpoint to here,
                    # [theta_checkpoint, theta_after], so KL lands exactly at target, then STOP the pass.
                    actor_opt.zero_grad()
                    actor_loss.backward()
                    nn.utils.clip_grad_norm_(actor_params, args.max_grad_norm)
                    actor_opt.step()
                    n_steps += 1
                    if (n_steps % args.kl_check_every == 0):
                        cur_kl = _batch_kl()  # KL(theta_old || current params)
                        if cur_kl < args.kl_rescale_target:
                            # Still inside the budget -> this is now the last confirmed sub-target point.
                            theta_checkpoint = [p.detach().clone() for p in actor_params]
                        else:
                            theta_after = [p.detach().clone() for p in actor_params]
                            rescale_kl_pre, trimmed = cur_kl, True

                            def _set_beta(bt):  # theta <- theta_checkpoint + bt*(theta_after - theta_checkpoint)
                                with torch.no_grad():
                                    for p, b0, b1 in zip(actor_params, theta_checkpoint, theta_after):
                                        p.data.copy_(b0 + bt * (b1 - b0))

                            # KL(theta_old||theta_checkpoint) < target at beta=0 (theta_checkpoint was
                            # confirmed sub-target -- or theta_old itself, KL=0) and == cur_kl>=target at
                            # beta=1; monotone along the segment -> a root in [0,1] for ANY kl_check_every.
                            # Seed from the local quadratic KL(beta) ~ beta^2 * cur_kl, backtrack on the
                            # EXACT KL; the trailing halving loop enforces the hard <=target bound.
                            beta = min(1.0, (args.kl_rescale_target / (cur_kl + 1e-12)) ** 0.5)
                            rescale_alpha, rescale_kl_post = beta, cur_kl
                            for _ in range(args.kl_rescale_iters):
                                ...
                                if rescale_kl_post <= args.kl_rescale_target:
                                    break
                                beta *= (args.kl_rescale_target / (rescale_kl_post + 1e-12)) ** 0.5
                            guard = 0
                            while rescale_kl_post > args.kl_rescale_target and guard < 30:
                                ...
                                rescale_kl_post = _batch_kl()
                                guard += 1
                            stop_actor = True  # budget spent -> end the pass
                else:
                    actor_opt.zero_grad()
                    actor_loss.backward()
                    nn.utils.clip_grad_norm_(actor_params, args.max_grad_norm)
                ...
                if stop_actor:  # v22: walk hit the KL budget -> stop after the trim
                    break
            if args.target_kl is not None and approx_kl > args.target_kl:
                break

        # v22: if the walk never reached the budget (sub-target pass -- typically late training with
        # cold advantages / small steps), no trim fired and the params are left UNTOUCHED == v18's
        # sub-target case. Report the realized KL of that full untouched pass.
        if args.kl_rescale and not trimmed:
            rescale_kl_pre = rescale_kl_post = _batch_kl()
        ...
        if args.kl_rescale:
            writer.add_scalar("losses/kl_rescale_alpha", rescale_alpha, global_step)  # v22: trim fraction beta
            writer.add_scalar("losses/kl_rescale_pre", rescale_kl_pre, global_step)   # batch KL at walk stop (pre-trim)
            writer.add_scalar("losses/kl_rescale_post", rescale_kl_post, global_step)  # realized batch KL after trim (<=target)
            # losses/actor_steps already logs the WALK LENGTH (how many full steps fit in the budget).
