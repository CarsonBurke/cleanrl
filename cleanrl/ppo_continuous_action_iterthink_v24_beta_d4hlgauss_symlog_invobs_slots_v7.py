# PPO + IterThink v24 beta d4-HL-Gauss symlog + REPRESENTATION-FIRST inverted-attention transformer v7.
#
# GOAL: beat iterthink v24 (the ~MLP DenseNet-MoE trunk, ~9.7k HalfCheetah) on
# ARCHITECTURE ALONE -- the PPO/critic/advantage stack (HL-Gauss symlog critic,
# rankgauss advantage, clip-higher, beta action dist, decoupled dual-backward grad
# clip, target_kl) is held identical to v24, so the only variable is the trunk.
#
# WHY v7 (the diagnosis). v6 was a faithful-MINIMAL inverted-attention transformer
# (Wu et al. 2024, openreview WgQZNoQ5AB) and trailed v24 by ~2k. Inspecting v24's
# winner showed its trunk is a non-residual, non-attention MLP whose strength is
# (a) soft-MoE capacity, (b) DenseNet concat, (c) a convex x0=entry(obs) residual
# gate, (d) ReLU^2. Subagent analysis (confirmed): (b) and (c) are PROSTHETICS that
# a plain MLP needs to get gradient flow + input persistence -- a pre-LN transformer
# already has both (residual stream carries every block forward; attention re-injects
# the obs every block), so porting them is double-counting. And capacity (a) is NOT
# the lever: 470k params on a ~17-D input is over-parameterized, and v6+MoE still
# trailed. The REAL bottleneck is the INPUT REPRESENTATION: v6's rank-1 linear
# tokenizer (x_i*w_i + pos_i) makes each token a scaled copy of one scalar, so the
# attention can only LINEARLY combine raw scalars before the first FFN -- strictly
# weaker than v24's first Linear (mixes dims, then ReLU^2 immediately).
#
# v7 therefore bets on REPRESENTATION + RELATIONAL MIXING, not capacity:
#   - Tokenizer: a NONLINEAR per-dim lift (shared 1->H->H ReLU^2 MLP + learned per-dim
#     identity embedding), so each token is a nonlinear function of its scalar and the
#     inverted attention finally has nonlinear cross-feature structure to mix. (MAIN BET.)
#   - INVERTED cross-attention is kept (paper + lucidrains: inverted/slot attention
#     converges faster + to lower loss than regular cross-attention), with QK-norm +
#     learned per-head gain as modern stabilizers.
#   - Block: pre-LN residual + LayerScale (per-channel gamma, small init) -- replaces
#     v24's convex x0-gate (which would anchor to a CONSTANT obs-free slot prior here);
#     plain ReLU^2 FFN, MODEST width (no MoE).
#   - Readout: DenseNet concat of slots0 + every block's slot output -> RMSNorm ->
#     out_proj. This is layer-MIXING (each depth read with its own weights, strictly
#     more expressive than the final residual), NOT residual-substitution, and is the
#     one v24 idea worth keeping in a transformer; it's cheap at K blocks.
#   - One SHARED slot stack feeds both heads (v24/v21 sharing) + decoupled clip.
# Capacity is deliberately MODEST (beating v24 with fewer params is the stronger claim).
# Slot-output pairwise cosine-sim is logged (debug/invslot_slot_cossim) as the real
# collapse metric -- assign-entropy can read healthy while slot OUTPUTS are redundant.
#
# Base: ppo_continuous_action_iterthink_v24_beta_d4hlgauss_symlog_invobs_slots_v6.py.
#
...
class ObsTokenizer(nn.Module):
    """Nonlinear per-dimension token lift (v7's main bet). Each scalar obs dim is
    mapped through a SHARED 1->H->H ReLU^2 MLP, then added to a learned per-dim
    identity embedding. v6's rank-1 linear lift (x_i*w_i + pos_i) made every token
    a scaled copy of one scalar, so inverted attention could only form LINEAR
    combinations of raw scalars before the first FFN -- strictly weaker than v24's
    MLP, whose first Linear mixes dims and immediately applies ReLU^2. Here each
    token is a NONLINEAR function of its own scalar, giving the attention real
    nonlinear cross-feature structure to mix (the diagnosed representation gap)."""
    ...
    def forward(self, x):
        h = torch.relu(self.fc1(x.unsqueeze(-1))).square()       # (B, obs_dim, H)
        h = self.fc2(h)
        return h + self.pos.unsqueeze(0)


class FFN(nn.Module):
    """Plain transformer FFN with ReLU^2 (v24's nonlinearity). Capacity is held
    MODEST on purpose: the diagnosis is that v24's edge is its training stack, not
    raw width/experts (470k params on a ~17-D input is heavily over-parameterized),
    and v6-with-MoE still trailed. The bet is representation + relational mixing,
    not capacity -- beating v24 with fewer params is the stronger architecture
    claim. (Soft-MoE was dropped here as cargo-culted capacity.)"""

    def __init__(self, H, mult):
        super().__init__()
        self.fc = nn.Linear(H, mult * H)
        self.proj = nn.Linear(mult * H, H)
...
class InvertedCrossAttention(nn.Module):
    """Multi-head INVERTED cross-attention (Wu et al. 2024): softmax over the SLOT
    (query) axis so slots COMPETE to explain each token, then renormalize each
    slot's weights over tokens into a convex combination. This is the relational,
    input-dependent token-mixing op that v24's flat-vector trunk structurally
    lacks. v7 adds QK-norm + a learned per-head gain (modern attention stabilizers)
    on top of v6's faithful inverted normalization."""

    def __init__(self, H, heads, qk_gain_init=1.0):
        super().__init__()
        assert H % heads == 0, "hidden must be divisible by slot_heads"
        self.H = H
        self.heads = heads
        ...
        self.q_gain = nn.Parameter(torch.full((heads,), float(qk_gain_init)))
        self._assign_entropy = None

    def forward(self, slots, tokens):
        B, S, _ = slots.shape
        T = tokens.shape[1]
        q = self.q(slots).view(B, S, self.heads, self.head_dim).transpose(1, 2)    # (B, h, S, d)
        k = self.k(tokens).view(B, T, self.heads, self.head_dim).transpose(1, 2)   # (B, h, T, d)
        v = self.v(tokens).view(B, T, self.heads, self.head_dim).transpose(1, 2)   # (B, h, T, d)
        q = F.rms_norm(q, (self.head_dim,)) * self.q_gain[None, :, None, None]
        k = F.rms_norm(k, (self.head_dim,))
        logits = torch.matmul(q, k.transpose(-1, -2)) / sqrt(self.head_dim)        # (B, h, S, T)
        assign = torch.softmax(logits, dim=-2)                  # INVERTED: token -> competing slots
        attn = assign / (assign.sum(dim=-1, keepdim=True) + 1e-8)  # slot -> token renorm (convex)
        y = torch.matmul(attn, v)                                                  # (B, h, S, d)
        y = y.transpose(1, 2).contiguous().view(B, S, self.H)
        with torch.no_grad():
            p = assign.detach().clamp_min(1e-8)
            self._assign_entropy = -(p * p.log()).sum(dim=-2).mean()
        return self.proj(y)


class InvertedSlotBlock(nn.Module):
    """Pre-LN inverted cross-attention decoder block with LayerScale residuals:
      slots += ls_attn * InvAttn(LN(slots), LN(tokens))   # relational token mixing
      slots += ls_ffn  * FFN(LN(slots))                   # per-slot ReLU^2 FFN
    LayerScale (per-channel gamma, small init) replaces v24's convex x0-gate: it
    gives the same 'start near identity, learn each sublayer's contribution'
    behaviour WITHOUT a spurious pull toward a constant obs-free prior. The
    transformer's residual stream already provides the identity path, and the
    attention re-injects the obs every block, so no x0 anchoring is needed."""

    def __init__(self, H, heads, ffn_mult, ls_init=0.1):
        super().__init__()
        self.slot_norm = nn.LayerNorm(H)
        self.token_norm = nn.LayerNorm(H)
        self.ffn_norm = nn.LayerNorm(H)
        self.attn = InvertedCrossAttention(H, heads)
        self.ffn = FFN(H, ffn_mult)
        self.ls_attn = nn.Parameter(torch.full((H,), ls_init))
        self.ls_ffn = nn.Parameter(torch.full((H,), ls_init))

    def forward(self, slots, tokens):
        slots = slots + self.ls_attn * self.attn(self.slot_norm(slots), self.token_norm(tokens))
        slots = slots + self.ls_ffn * self.ffn(self.ffn_norm(slots))
        return slots


class InvertedSlotStack(nn.Module):
    """Inverted-attention transformer refining learned slots over obs tokens, with
    v24's capacity machinery in each block and DenseNet cross-layer feature reuse:
    the initial slots and every block's slot state are concatenated (per slot) and
    projected for the readout (v24's concat-readout), rather than taking only the
    last block."""

    def __init__(self, n_slots, H, n_blocks, n_heads, ffn_mult):
        super().__init__()
        assert n_blocks >= 1
        ...
        self.blocks = nn.ModuleList(
            [InvertedSlotBlock(H, n_heads, ffn_mult) for _ in range(n_blocks)]
        )
        cat_dim = H * (n_blocks + 1)                              # slots0 + each block output
        self.out_norm = nn.RMSNorm(cat_dim, elementwise_affine=False)
        self.out_proj = nn.Linear(cat_dim, H)
        self._assign_entropy_sum = None
        self._assign_count = 0
        self._slot_cossim = None
    ...
    def consume_stats(self):
        # Returns (mean assign-entropy, mean pairwise slot-output cosine-sim) since
        # the last call, or (None, None) if no forward happened.
        if self._assign_count == 0:
            return None, None
        entropy = self._assign_entropy_sum / self._assign_count
        cossim = self._slot_cossim
        self._assign_entropy_sum = None
        self._assign_count = 0
        self._slot_cossim = None
        return entropy, cossim

    def _accumulate_block_stats(self, block):
        ent = block.attn._assign_entropy
        if ent is None:
            return
        if self._assign_entropy_sum is None:
            self._assign_entropy_sum = ent.detach().clone()
        else:
            self._assign_entropy_sum = self._assign_entropy_sum + ent.detach()
        self._assign_count += 1

    def forward(self, tokens):
        B = tokens.shape[0]
        slots0 = self.slots.unsqueeze(0).expand(B, -1, -1)
        slots = slots0
        feats = [slots0]
        for block in self.blocks:
            slots = block(slots, tokens)
            self._accumulate_block_stats(block)
            feats.append(slots)
        out = self.out_proj(self.out_norm(torch.cat(feats, dim=-1)))      # (B, n_slots, H)
        with torch.no_grad():
            # Slot-collapse metric: mean pairwise cosine similarity of slot OUTPUTS
            # (assign-entropy can read healthy while the slot outputs are redundant).
            s = F.normalize(out.detach(), dim=-1)                         # (B, K, H)
            sim = torch.matmul(s, s.transpose(1, 2))                      # (B, K, K)
            K = s.shape[1]
            if K > 1:
                off = (sim.sum(dim=(1, 2)) - K) / (K * (K - 1))          # mean off-diagonal
                self._slot_cossim = off.mean()
        return out.flatten(1)


class ThinkTrunk(nn.Module):
    """Shared tokenizer + a single inverted-attention slot stack. One trunk feeds
    both heads (v24's sharing); the decoupled dual-backward clip keeps the critic
    CE gradient from swamping the shared features."""

    def __init__(self, in_dim, H, n_slots, slot_blocks, slot_heads, ffn_mult):
        super().__init__()
        self.tokenizer = ObsTokenizer(in_dim, H)
        self.stack = InvertedSlotStack(n_slots, H, slot_blocks, slot_heads, ffn_mult)

    @property
    def out_dim(self):
        return self.stack.out_dim

    def forward(self, x):
        return self.stack(self.tokenizer(x))
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        H = args.hidden
        assert args.slot_dim == H, "slot_dim must equal hidden in v7"
        self.share_backbone = args.share_backbone
        if self.share_backbone:
            # One trunk (tokenizer + single inverted-attention slot stack) feeds
            # both heads; decoupled dual-backward clip keeps the critic CE gradient
            # from swamping the shared features (v24/v21 sharing).
            self.trunk = ThinkTrunk(
                obs_dim, H, args.n_slots, args.slot_blocks, args.slot_heads, args.ffn_mult
            )
            actor_feat_dim = critic_feat_dim = self.trunk.out_dim
        else:
            ...
            actor_feat_dim = self.actor_trunk.out_dim
            critic_feat_dim = self.critic_trunk.out_dim
    ...
    def _trunks(self, x):
        # Return (actor_feat, critic_feat). In shared mode both heads consume the
        # SAME shared slot-stack output (computed once); decoupled clip splits the
        # gradients. In non-shared mode each head has its own trunk.
        if self.share_backbone:
            feat = self.trunk(x)
            return feat, feat
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        stack = agent.trunk.stack if args.share_backbone else agent.critic_trunk.stack
        ent, cossim = stack.consume_stats()
        if ent is not None:
            writer.add_scalar("debug/invslot_assign_entropy", ent.item(), global_step)
        if cossim is not None:
            # Slot-collapse metric: ->1 means the slots learned the same thing.
            writer.add_scalar("debug/invslot_slot_cossim", cossim.item(), global_step)
