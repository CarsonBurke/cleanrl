# Fully on-policy Delightful Gradient with long-horizon GAE v25.
#
# Collects one 2,048-transition current-policy rollout, computes 128-step GAE,
# and makes exactly one DG actor update on a random 256-transition subset. The
# critic fits all fresh returns before the rollout is discarded. There is no
# replay, importance ratio, target actor, KL stop, or stale actor epoch.
# Hypothesis: v24 failed because batch-256 updates shortened GAE and shifted the
# actor every 16 environment steps; preserving v14's rollout horizon and actor
# cadence should retain DG's fully on-policy benefit without that instability.
...
class Args:
    ...
    num_envs: int = 16
    num_steps: int = 128
    target_rollout_batch_size: int = 2048
...
if __name__ == "__main__":
    ...
    if args.batch_size != args.target_rollout_batch_size:
        raise ValueError(f"fresh rollout batch must be {args.target_rollout_batch_size}, got {args.batch_size}")
    if not 0 < args.actor_batch_size <= args.batch_size:
        raise ValueError("actor_batch_size must be positive and no larger than the rollout")
    ...
    actor_rng = np.random.default_rng(args.seed)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Subsampling a fixed-policy rollout changes estimator variance, not
        # its on-policy distribution. A separate RNG preserves the critic's
        # established minibatch ordering and the policy's sampling stream.
        actor_inds = torch.as_tensor(
            actor_rng.choice(args.batch_size, args.actor_batch_size, replace=False),
            device=device,
        )
        actor_obs = b_obs[actor_inds]
        actor_raw_actions = b_raw_actions[actor_inds]
        actor_old_logprobs = b_logprobs[actor_inds]
        actor_advantages = b_advantages[actor_inds]
        ...
        actor_weights = (gate * actor_advantages).detach()
        ...
        post_logratio = post_action_logprob - actor_old_logprobs
    ...
    if args.save_model:
        ...
        if args.upload_model:
            ...
            push_to_hub(
                ...
                episodic_returns,
                repo_id,
                "FullyOnPolicyDelightfulGradientLongHorizon",
                f"runs/{run_name}",
