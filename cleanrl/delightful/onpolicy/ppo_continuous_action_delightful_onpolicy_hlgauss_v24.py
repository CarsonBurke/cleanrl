# Fully on-policy Delightful Gradient with a symlog HL-Gauss critic v24.
#
# Every actor score term comes from one fresh 256-transition rollout and is used
# exactly once. DG gates raw GAE advantages by the sampled action's detached
# surprisal; the batch is then discarded. The categorical critic may make
# repeated passes over those fresh return targets, but there is no replay,
# importance ratio, target actor, KL stop, or stale actor epoch.
# Hypothesis: DG's selective score weighting helps when its on-policy guarantee
# is preserved end to end, while HL-Gauss stabilizes the rapidly moving value.
    ...
    if args.save_model:
        ...
        if args.upload_model:
            ...
            push_to_hub(
                ...
                episodic_returns,
                repo_id,
                "FullyOnPolicyDelightfulGradient",
                f"runs/{run_name}",
