# Delightful PPO with directly bounded rewards and fresh actor updates v8.
#
...
# task: linear tolerance of forward speed, rising from 0 at speed 0 to 1 at the
# target speed 10. There is no reward EMA or other reward scaling. Raw Gym
# benchmark returns are unchanged.
# Advantages and delight are not normalized or whitened. State-dependent log
# sigma uses SAC's smooth bounds with log sigma=-1.5 initialization. Actions use
# SAC's tanh transform and the exact bounded-action density for both the policy
# score and surprisal.
#
# Hypothesis: matching DG's native reward support keeps raw delight in its
# intended operating range without advantage normalization.
