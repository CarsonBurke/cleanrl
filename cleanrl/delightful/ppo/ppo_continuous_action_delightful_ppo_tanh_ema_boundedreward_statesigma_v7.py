# Delightful PPO with EMA-bounded rewards and fresh actor updates v7.
#
...
# Once per learner batch, immediate rewards are divided by the paper's
# decay-0.999 bias-corrected EMA RMS and projected onto [0, 1], matching the
# reward support used by both DeepMind Control Suite and egg's DG experiment.
# Raw benchmark returns are unchanged. Advantages and delight are not
# normalized or whitened. State-dependent log sigma uses SAC's smooth bounds
# with log sigma=-1.5 initialization. Actions use SAC's tanh transform and the
# exact bounded-action density for both the policy score and surprisal.
#
# Hypothesis: matching DG's reward support and learner-step EMA semantics keeps
# raw delight in its intended operating range without advantage normalization.
...
class Args:
    ...
    """learner-step decay of the immediate-reward second-moment EMA"""
...
class EMABoundedRewardNormalizer:
    """EMA-scale one learner batch and project its rewards onto [0, 1]."""
    ...
    def normalize(self, rewards):
        ...
        scaled_rewards = rewards / (reward_rms + self.epsilon)
        lower_clip_fraction = float(np.mean(scaled_rewards < REWARD_LOWER_BOUND))
        upper_clip_fraction = float(np.mean(scaled_rewards > REWARD_UPPER_BOUND))
        bounded_rewards = np.clip(
            scaled_rewards, REWARD_LOWER_BOUND, REWARD_UPPER_BOUND
        )
        return (
            bounded_rewards,
            reward_rms,
...
if __name__ == "__main__":
    ...
    reward_normalizer = EMABoundedRewardNormalizer(
    ...
    reward_rms = 1.0
    reward_lower_clip_fraction = 0.0
    reward_upper_clip_fraction = 0.0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Match the paper's learner-step EMA cadence: update the statistics once
        # from the complete fresh batch, then bound the rewards consumed by GAE.
        raw_rewards_np = rewards.cpu().numpy()
        raw_reward_mean = float(raw_rewards_np.mean())
        raw_reward_min = float(raw_rewards_np.min())
        raw_reward_max = float(raw_rewards_np.max())
        (
            normalized_rewards,
            reward_rms,
            reward_lower_clip_fraction,
            reward_upper_clip_fraction,
        ) = reward_normalizer.normalize(raw_rewards_np)
        rewards.copy_(
            torch.as_tensor(normalized_rewards, dtype=torch.float32, device=device)
        )
        ...
        writer.add_scalar(
            "diagnostics/reward_lower_clip_fraction",
            reward_lower_clip_fraction,
            global_step,
        )
        writer.add_scalar(
            "diagnostics/reward_upper_clip_fraction",
            reward_upper_clip_fraction,
            global_step,
        )
        writer.add_scalar(
