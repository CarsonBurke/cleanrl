# Delightful PPO with return-calibrated DG and fresh batch-256 actor updates v3.
#
# DG multiplies each on-policy score term by sigmoid(A * ell / eta), where ell
# is clipped continuous-action surprisal. The actor takes exactly one full-batch
# step per fresh 256-transition rollout; only the critic uses minibatches.
# Immediate rewards use the paper's decay-0.999 EMA RMS and are multiplied by
# (1-gamma), putting dense discounted returns on the order-one scale of bounded
# episodic outcomes. Advantages and delight are not normalized or whitened.
# State-dependent log sigma uses SAC's smooth bounds but initializes at sigma=1
# because DG has no SAC entropy objective. Actions use SAC's tanh transform and
# the exact bounded-action density for both the policy score and surprisal.
#
# Hypothesis: matching the paper's actor update cadence and calibrating the
# return scale will keep delight responsive rather than saturated, while the
# higher initial sigma prevents negative density-surprisal from reversing DG.
...
class Args:
    ...
    target_actor_batch_size: int = 256
    """required fresh full-batch actor update size"""
    ...
    scale_discounted_return: bool = True
    """multiply unit-RMS dense rewards by (1-gamma) so returns are order one"""
...
class EMARewardNormalizer:
    """EMA-normalize immediate rewards and calibrate their discounted-return scale."""

    def __init__(self, decay=0.999, epsilon=1e-8, reward_scale=1.0):
    ...
    def normalize(self, rewards):
        ...
        normalized_rewards = self.reward_scale * rewards / (reward_rms + self.epsilon)
        return normalized_rewards, reward_rms
...
if __name__ == "__main__":
    ...
    reward_scale = 1.0 - args.gamma if args.scale_discounted_return else 1.0
