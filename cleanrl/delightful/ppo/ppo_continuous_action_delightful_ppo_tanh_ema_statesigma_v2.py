# Delightful PPO with tanh Gaussian actions and EMA reward scaling v2.
#
...
# step per fresh rollout; only the critic uses minibatches and multiple epochs.
# Immediate rewards are scaled by a
# bias-corrected EMA RMS (decay 0.999), replacing discounted-return reward
# normalization. Advantages are not normalized. A state-dependent diagonal
# Gaussian scale lets the policy adapt its exploration to each state. Actions
# use SAC's tanh transform and Jacobian-corrected bounded-action log-density, so
# the action executed by the environment is the action whose surprisal DG gates.
#
# Hypothesis: DG will suppress surprising negative-advantage actions while
# retaining rare improvements, and state-dependent scale will make surprisal
# informative across distinct locomotion phases.
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_logstd = layer_init(nn.Linear(64, action_dim), std=0.01)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        actor_loss.backward()
        nn.utils.clip_grad_norm_(actor_parameters, args.max_grad_norm)
