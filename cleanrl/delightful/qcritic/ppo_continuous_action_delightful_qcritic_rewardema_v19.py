# Delightful Policy Gradient with stable state-Gaussian policy v19.
#
# Keeps v18's batch-256 DG loop and one-sample same-state Q baseline. V19
# adopts the likely Acme control-policy parameterization missing from earlier
# versions: tanh-bounded action-space means and state-dependent softplus scales
# initialized at 0.7 with a 0.3 floor. Gaussian samples are clipped only for
# execution while the latent Gaussian score drives DG, preventing v18's
# variance collapse without falling back to a global standard deviation.
...
RAW_SCALE_INIT = float(np.log(np.expm1(INITIAL_POLICY_STD - MIN_POLICY_STD)))
...
def state_dependent_logstd(raw_scale):
    return torch.log(torch.nn.functional.softplus(raw_scale) + MIN_POLICY_STD)
...
class Actor(nn.Module):
    ...
    def parameters_for(self, observations):
        ...
        mean = torch.tanh(self.mean(features)) * self.action_scale + self.action_bias
        return mean, state_dependent_logstd(self.raw_scale(features))

    def transform(self, raw_actions):
        return torch.clamp(raw_actions, min=self.action_low, max=self.action_high)
    ...
    def logprobs(self, observations, raw_actions):
        ...
        return gaussian_logprob, gaussian_logprob, mean, logstd
...
if __name__ == "__main__":
    ...
    if args.env_id != "HalfCheetah-v4":
        raise ValueError("v19 is tuned for HalfCheetah-v4")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Approximate V(s) with one independent current-policy action at
            # each same state. This preserves a valid state-conditioned control
            # variate without v16's eight policy/Q samples per observation.
