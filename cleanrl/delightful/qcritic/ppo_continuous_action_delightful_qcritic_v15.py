# Delightful Policy Gradient with a paper-style Q critic v15.
#
# Paper-derived continuous-control stack: separate 2x256 Gaussian actor and
# Q(s,a) critic, replay, hard target networks, Adam 3e-4, clipped density
# surprisal, eta=1, and one actor-only DG update on fresh on-policy data.
# Project adaptations are explicit: 16 Gym actors and an 8M-step harness batch;
# the paper's reward EMA is replaced by the already-bounded Control Suite
# cheetah-run reward clip(forward_speed / 10, 0, 1); and Retrace is unnecessary
# because replay uses one-step Bellman targets, never off-policy multi-step
# traces. Raw Gym return remains the benchmark metric.
...
class Args:
    ...
    critic_updates_per_iteration: int = 32
    learning_starts: int = 2_048
    target_update_interval: int = 100
    actor_baseline_samples: int = 8
    dg_eta: float = 1.0
    dg_surprisal_clip: float = 10.0
    dmc_run_speed: float = 10.0
...
def smooth_logstd_bound(raw_logstd):
    return LOG_STD_MIN + 0.5 * (LOG_STD_MAX - LOG_STD_MIN) * (torch.tanh(raw_logstd) + 1.0)


def dmc_cheetah_run_reward(forward_speed, target_speed=10.0):
    """DeepMind Control cheetah-run's bounded linear speed tolerance."""
    if target_speed <= 0:
        raise ValueError("target_speed must be positive")
    return torch.clamp(forward_speed / target_speed, min=0.0, max=1.0)
...
class Actor(nn.Module):
    def __init__(self, obs_dim, action_dim, action_low, action_high):
        ...
        self.logstd = layer_init(nn.Linear(256, action_dim), std=0.01, bias_const=RAW_LOG_STD_INIT)
    ...
    def parameters_for(self, observations):
        ...
        return self.mean(features), smooth_logstd_bound(self.logstd(features))
...
@torch.no_grad()
def q_policy_baseline(actor, critic, observations, num_samples):
    if num_samples <= 0:
        raise ValueError("num_samples must be positive")
    estimates = []
    for _ in range(num_samples):
        actions, _ = actor.sample(observations)
        estimates.append(critic(observations, actions))
...
if __name__ == "__main__":
    ...
    if args.env_id != "HalfCheetah-v4":
        raise ValueError("v15's bounded Control Suite reward is defined only for HalfCheetah-v4")
    if args.batch_size != 2048:
        raise ValueError(f"fresh actor batch must be 2048, got {args.batch_size}")
    ...
    if args.actor_baseline_samples <= 0:
        raise ValueError("actor_baseline_samples must be positive")
    ...
    training_rewards = torch.zeros((args.num_steps, args.num_envs), device=device)
    raw_rewards = torch.zeros_like(training_rewards)
    next_observations_buffer = torch.zeros_like(observations)
    discounts = torch.zeros_like(training_rewards)
    bounded_episode_returns = torch.zeros(args.num_envs, device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        for step in range(args.num_steps):
            ...
            forward_speed = transition_info_values(infos, "x_velocity", args.num_envs)
            if not np.all(np.isfinite(forward_speed)):
                raise RuntimeError("HalfCheetah transition did not expose finite x_velocity")
            forward_speed_tensor = torch.as_tensor(forward_speed, dtype=torch.float32, device=device)
            bounded_reward = dmc_cheetah_run_reward(forward_speed_tensor, args.dmc_run_speed)
            training_rewards[step] = bounded_reward
            bounded_episode_returns += bounded_reward
            ...
            for env_index, info in enumerate(infos.get("final_info", ())):
                if info and "episode" in info:
                    ...
                    writer.add_scalar(
                        "charts/bounded_training_episodic_return",
                        bounded_episode_returns[env_index].item(),
                        global_step,
                    )
            ...
            bounded_episode_returns[done] = 0.0
        ...
        replay.add(
            ...
            training_rewards.reshape(args.batch_size).cpu().numpy(),
        ...
        if learner_step == 0:
            ...
            writer.add_scalar("reward/training_mean", training_rewards.mean().item(), global_step)
            writer.add_scalar("reward/training_min", training_rewards.min().item(), global_step)
            writer.add_scalar("reward/training_max", training_rewards.max().item(), global_step)
        ...
        with torch.no_grad():
            ...
            q_baseline = q_policy_baseline(actor, critic, b_obs, args.actor_baseline_samples)
            ...
            fresh_td_target = one_step_q_target(
                training_rewards.reshape(args.batch_size),
