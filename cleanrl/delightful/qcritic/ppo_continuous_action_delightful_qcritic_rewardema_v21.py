# Delightful Policy Gradient with deterministic same-state baseline v21.
#
# Keeps v20's batch-256 DG loop and smoothly bounded state-dependent Gaussian.
# V21 removes counterfactual sampling noise from the actor advantage by using
# Q(s, sampled action) - Q(s, deterministic policy mean). This remains a valid
# detached state-only score-function baseline, needs one Q call rather than
# v16's eight samples, and prevents random baseline noise from driving the
# covariance head toward opposite extremes.
...
if __name__ == "__main__":
    ...
    if args.env_id != "HalfCheetah-v4":
        raise ValueError("v21 is tuned for HalfCheetah-v4")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # A deterministic same-state control variate removes between-state
            # value and counterfactual sampling noise. It is detached, so its
            # actor dependence cannot create a pathwise gradient.
            baseline_actions = actor.deterministic(b_obs)
