# Delightful Policy Gradient with raw-reward EMA Q critic v16.
#
# Keeps v15's paper-derived 2x256 state-Gaussian actor, Q(s,a), replay, hard
# targets, clipped density surprisal, and eta=1 DG update. V16 fixes v15's
# objective mismatch: the critic learns Gym HalfCheetah's actual reward,
# including control cost, divided by the paper's bias-corrected decay-0.999
# immediate-reward EMA RMS. Replay stores raw rewards and applies the current
# scale at learning time, so historical transitions do not retain stale units.
# Retrace remains unnecessary because every replay backup is one-step TD(0).
...
if __name__ == "__main__":
    ...
    if args.env_id != "HalfCheetah-v4":
        raise ValueError("v16 is tuned for HalfCheetah-v4")
