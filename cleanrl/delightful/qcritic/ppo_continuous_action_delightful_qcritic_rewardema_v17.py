# Delightful Policy Gradient with paper-sized actor batches v17.
#
# Keeps v16's state-Gaussian actor, Q(s,a), replay, raw-reward EMA scaling,
# hard targets, clipped density surprisal, eta=1 DG update, and one-step TD(0).
# V17 uses the paper's batch size of 256 for each fresh on-policy actor update
# and centers taken-action Q values with an empirical batch baseline, avoiding
# v16's eight additional policy-action samples per state. With 16 environments
# this means 16 steps per policy iteration. Four replay critic updates per
# iteration preserve v16's 4:1 critic sample-to-data ratio.
...
if __name__ == "__main__":
    ...
    if args.env_id != "HalfCheetah-v4":
        raise ValueError("v17 is tuned for HalfCheetah-v4")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # The continuous-control appendix does not specify its advantage
            # baseline. Use the fresh batch's empirical mean, as the paper does
            # in its sequence experiment, without extra policy/Q evaluations.
            q_baseline = q_taken.mean()
