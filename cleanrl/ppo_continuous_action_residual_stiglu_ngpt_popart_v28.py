# Exact v26 raw-reward scalar-MSE, no-gradient/value-clipping control plus PopArt.
# Actor, critic, initialization order, raw GAE and PPO are unchanged. PopArt uses
# the VMPO reference EMA once per fresh rollout, after raw targets are frozen.
# The normalized critic is g * (W h + b) + shift. Compensate the existing gain
# and a nonlearned shift, NOT W: unit-matrix projection would undo W rescaling.
# Thus the frame change survives projection, including zero/negative gains,
# without another learned bias or RNG draw. Adam moments intentionally remain
# unchanged across frame changes, matching VMPO. Std bounds guard statistics;
# they do not clip rewards, values, targets or gradients. No symlog or VMPO actor.
import json
import math
import os
import random
...
from cleanrl.shared.ppo_loop import (
    ...
    explained_variance, gather_metrics, get_gae_fn,
)
...
class Args:
    ...
    # Matched job8627 v26 raw-reward, no-gradient/value-clipping control.
    ...
    """the shared actor and critic raw-return GAE lambda"""
    num_minibatches: int = 1
    """the number of mini-batches"""
    update_epochs: int = 10
    """the K epochs to update the policy"""
    norm_adv: bool = False
    """must remain false: actor advantages retain the control's raw units"""
    norm_reward: bool = False
    """must remain false: rewards are neither normalized nor clipped"""
    clip_coef: float = 0.2
    """lower policy ratio clipping coefficient"""
    clip_coef_upper: float = 0.2
    """upper policy ratio clipping coefficient"""
    clip_vloss: bool = False
    """must remain false: value clipping is unsupported in this matched arm"""
    critic_loss: Literal["mse"] = "mse"
    """scalar MSE in the adaptive PopArt frame; no alternative critic objectives"""
    critic_target: Literal["gae"] = "gae"
    """raw GAE returns; no alternative target or mask machinery"""
    ent_coef: float = 0.0
    """coefficient of the entropy"""
    vf_coef: float = 0.5
    """coefficient of the value function"""
    max_grad_norm: float = 0.5
    """control CLI field only; unused because gradient clipping is unsupported"""
    grad_clip: Literal["none"] = "none"
    """must remain none: every gradient is left unmodified"""
    target_kl: float | None = None
    """the optional v7 last-minibatch KL threshold, checked after each epoch"""

    popart_rate: float = 1e-4
    """VMPO EMA rate, applied once per fresh rollout, never per epoch or sample"""
    popart_std_min: float = 1e-2
    """VMPO lower statistical bound on PopArt standard deviation"""
    popart_std_max: float = 1e6
    """VMPO upper statistical bound on PopArt standard deviation"""
    ...
    compile: bool = True
    """compile deterministic policy statistics, PPO loss, PopArt and GAE"""
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        super().__init__()
        validate_args(Args() if args is None else args)
        ...
        # Preserve the control's RNG consumption, initialization order and gains.
        ...
        self.register_buffer("popart_shift", torch.zeros(()))
    ...
    def get_normalized_value(self, x):
        """Predict in the current PopArt frame, including its nonlearned offset."""
        return self.critic(x) + self.popart_shift

    def get_value(self, x):
        """Return raw V for rollout GAE, including final-observation bootstraps."""
        return self.popart_std * self.get_normalized_value(x) + self.popart_mean

    @torch.no_grad()
    def update_popart(self, returns, rate, std_min, std_max):
        """Update VMPO moments once per rollout while preserving every raw V.

        Leave the unit-projected head, all actor parameters and Adam state alone.
        Gain/shift compensation never divides by gain, so zero and negative
        readouts need no special case. Targets returned here are detached.
        """
        ...
        self.critic.readout_gain.mul_(old_std / new_std)
        self.popart_shift.mul_(old_std).add_(old_mean - new_mean).div_(new_std)
        self.popart_mean.copy_(new_mean)
        self.popart_sq_mean.copy_(new_sq_mean)
        self.popart_std.copy_(new_std)
        return ((returns - new_mean) / new_std).detach()
    ...
    def get_action_and_value(self, x, action=None):
        """Public API uses physical actions and raw V; training keeps native samples."""
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args):
    """Original PPO actor and normalized scalar MSE against detached raw GAE returns.

    old_values retains the control's call signature but is unused: no value clip.
    The rollout's PopArt frame is fixed throughout every optimization epoch.
    """
    alpha, beta = (F.softplus(agent.actor(observations)) + 1.0).chunk(2, dim=-1)
    normalized_value = agent.get_normalized_value(observations).view(-1)
    distribution = Beta(alpha, beta, validate_args=False)
    ...
    normalized_returns = ((returns - agent.popart_mean) / agent.popart_std).detach()
    v_loss = 0.5 * (normalized_value - normalized_returns).square().mean()
...
def validate_args(args):
    if args.critic_target != "gae":
        raise ValueError("this matched PopArt arm requires critic_target=gae")
    if args.critic_loss != "mse":
        raise ValueError("this matched PopArt arm requires critic_loss=mse; symlog is unsupported")
    if args.norm_reward:
        raise ValueError("this matched PopArt arm requires norm_reward=False; rewards must stay raw")
    if args.norm_adv:
        raise ValueError("this matched PopArt arm requires norm_adv=False; advantages must stay raw")
    if args.grad_clip != "none":
        raise ValueError("this matched PopArt arm requires grad_clip=none")
    if args.clip_vloss:
        raise ValueError("this matched PopArt arm requires clip_vloss=False")
    if not math.isfinite(args.popart_rate) or not 0 < args.popart_rate <= 1:
        raise ValueError("popart_rate must be finite and in (0, 1]")
    if not (math.isfinite(args.popart_std_min) and math.isfinite(args.popart_std_max)
            and 0 < args.popart_std_min <= args.popart_std_max):
        raise ValueError("PopArt standard-deviation bounds must be finite, positive and ordered")
...
def main():
    ...
    try:
        ...
        writer.add_text("critic_objective", "0.5 * normalized MSE; raw rewards, GAE, advantages and decoded V; no reward/advantage normalization or gradient/value clipping")
        writer.add_text("popart", f"VMPO EMA rate={args.popart_rate}, std bounds=[{args.popart_std_min}, {args.popart_std_max}]; once per fresh rollout after raw GAE is frozen; existing gain + nonlearned shift compensation; Adam moments unchanged")
        envs = make_training_env(args, run_name)
        resources.callback(envs.close)
        agent = Agent(envs, args).to(device)
        normalize_matrices = agent.normalize_matrices
        update_popart = agent.update_popart
        if args.compile:
            normalize_matrices = torch.compile(normalize_matrices, fullgraph=True,
                                               options={"triton.cudagraphs": False})
            update_popart = torch.compile(update_popart, fullgraph=True,
                                          options={"triton.cudagraphs": False})
        normalize_matrices()
        ...
        def rollout_statistics(observations, native):
            """Old log-probabilities and raw values for the entire uploaded rollout."""
        ...
        @torch.no_grad()
        def critic_statistics(observations, returns):
            """One logging-boundary critic forward; never inside an optimizer step."""
            normalized_value = agent.get_normalized_value(observations).flatten()
            raw_value = agent.popart_std * normalized_value + agent.popart_mean
            error = raw_value - returns
            normalized_returns = (returns - agent.popart_mean) / agent.popart_std
            return {
                "critic/decoded_mse": error.square().mean(),
                "critic/value_bias": error.mean(),
                "critic/postupdate_ev": explained_variance(raw_value, returns),
                "critic/decoded_mean": raw_value.mean(),
                "critic/decoded_min": raw_value.min(),
                "critic/decoded_max": raw_value.max(),
                "critic/decoded_finite_fraction": torch.isfinite(raw_value).float().mean(),
                "critic/normalized_mse": (normalized_value - normalized_returns).square().mean(),
            }
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                # Every bootstrap, GAE advantage and return stays in raw units.
                ...
                # Own both raw targets across compiled graph and PopArt mutations.
                b_advantages = advantages.flatten().detach().clone()
                b_returns = returns.flatten().detach().clone()
                del advantages, returns
            with timer.span("popart"), torch.no_grad():
                update_popart(b_returns, args.popart_rate, args.popart_std_min, args.popart_std_max)
                # Measure actual raw predictions before/after this frame change,
                # once per fresh rollout and strictly before optimizer updates.
                preservation_error = (value_model(b_obs).flatten() - b_values).abs().max()
            updates = 0
            ...
            with torch.no_grad():
                critic_logged = critic_statistics(b_obs, b_returns)
                ...
                total_grad_norm = torch.sqrt(actor_grad_norm.square() + critic_grad_norm.square())
            logged = gather_metrics({
                ...
                "critic/preupdate_mse": (b_values - b_returns).square().mean(),
                "critic/target_mean": b_returns.mean(),
                "critic/target_std": b_returns.std(unbiased=False),
                "critic/target_min": b_returns.min(), "critic/target_max": b_returns.max(),
                "critic/target_finite_fraction": torch.isfinite(b_returns).float().mean(),
                "critic/advantage_mean": b_advantages.mean(),
                "critic/advantage_std": b_advantages.std(unbiased=False),
                "grad/total_preclip_norm": total_grad_norm, "grad/total_postclip_norm": total_grad_norm,
                ...
                "popart/raw_preservation_max_error": preservation_error,
                **critic_logged,
            })
        ...
        checkpoint = {
            ...
            "semantics": {
                "critic": "normalized_value = critic(x) + popart_shift; raw_value = popart_std * normalized_value + popart_mean",
                "critic_loss": "0.5 * mean((normalized_value - (raw_gae_return - popart_mean) / popart_std)^2)",
                "rewards_advantages_bootstraps": "raw units; shared terminal/truncation-aware GAE",
                "popart_update": "VMPO EMA once per fresh rollout after raw GAE targets are frozen, before PPO epochs",
                "popart_compensation": "existing critic.readout_gain and nonlearned popart_shift; unit-projected matrices unchanged",
                "adam_moments": "unchanged across PopArt frame changes, matching VMPO",
                "clipping": "PPO policy ratios only; no reward, gradient, target or value clipping",
            },
