...
#
# V14. Make the substrate permutation/geometry equivariant by construction.
# Replace K-output cell-index field heads with shared per-cell laws over
# [base, h_i, z_i, seed_i], replace source/target edge heads with shared Q/K
# relational routing plus relative geometry, and replace absolute tick heads
# with shared recurrent halting. Add abstraction prototypes so equivalent
# cells can collapse into shared latent objects, with compute charged for
# active write intensity, sparse routes, and distinct active prototypes.
#
...
class Args:
    ...
    """latent abstraction objects that cells can collapse onto"""
    prototype_compute_weight: float = 0.25
    """relative cost of keeping many active abstraction prototypes distinct"""
    equivariant_residual_scale: float = 0.25
    """small residual-stream scale for stable shared recurrent substrate updates"""
    prototype_mix: float = 0.5
    """how strongly abstraction prototype messages influence each cell update"""
...
class MorphogenicSubstrate(nn.Module):
    """A fixed-capacity substrate whose shared laws are equivariant over cells."""
    ...
    def __init__(self, obs_dim, args):
        ...
        self.prototype_compute_weight = args.prototype_compute_weight
        self.residual_scale = args.equivariant_residual_scale
        self.prototype_mix = args.prototype_mix

        self.input = layer_init(nn.Linear(obs_dim, self.D))
        self.cell_pos = nn.Parameter(torch.randn(self.K, self.F) * 0.5)
        self.prototype_pos = nn.Parameter(torch.randn(self.P, self.F) * 0.5)
        self.edge_bias = nn.Parameter(torch.tensor(float(args.init_edge_bias)))
        self.lookback_edge_bias = nn.Parameter(torch.tensor(float(args.init_lookback_edge_bias)))
        self.null_route_bias = nn.Parameter(torch.tensor(float(args.init_null_route_bias)))
        self.lookback_null_route_bias = nn.Parameter(torch.tensor(float(args.init_lookback_null_route_bias)))

        self.query = layer_init(nn.Linear(self.D, self.F), std=0.1)
        self.cell_law = nn.Sequential(
            layer_init(nn.Linear(2 * self.D + 2, 2 * self.D)),
            ReLUSquared(),
            layer_init(nn.Linear(2 * self.D, self.D), std=0.5),
            ReLUSquared(),
        )
        self.growth = layer_init(nn.Linear(self.D, 1), std=0.01, bias_const=args.init_active_bias)
        self.shrink = layer_init(nn.Linear(self.D, 1), std=0.01)
        self.plasticity = layer_init(nn.Linear(self.D, 1), std=0.01)
        self.persistence = layer_init(nn.Linear(self.D, 1), std=0.01)
        self.route_query = layer_init(nn.Linear(self.D, self.F), std=0.01)
        self.route_key = layer_init(nn.Linear(self.D, self.F), std=0.01)
        self.prototype_bias = nn.Parameter(torch.zeros(self.P))
        self.halt = nn.Sequential(
            layer_init(nn.Linear(2 * self.D, self.D), std=0.5),
            ReLUSquared(),
            layer_init(nn.Linear(self.D, 1), std=0.01, bias_const=args.init_tick_bias),
        )
    ...
    def _cell_features(self, base, h, query_dist2):
        B = base.shape[0]
        base_cells = base[:, None, :].expand(B, self.K, self.D)
        centered_query_dist = query_dist2 - query_dist2.mean(dim=1, keepdim=True)
        geom = torch.stack([query_dist2, centered_query_dist], dim=-1)
        return self.cell_law(torch.cat([base_cells, h, geom], dim=-1))

    def _offdiag_mean(self, matrix):
        if self.K <= 1:
            return matrix.new_zeros(matrix.shape[0])
        eye = torch.eye(self.K, device=matrix.device, dtype=matrix.dtype)
        return (matrix * (1.0 - eye)[None, :, :]).sum(dim=(1, 2)) / (self.K * (self.K - 1))
    ...
    def forward(self, x):
        ...
        h = base[:, None, :].expand(B, self.K, self.D)
        coord_dist2 = (self.cell_pos[:, None, :] - self.cell_pos[None, :, :]).pow(2).mean(dim=-1)
        query_dist2 = (query[:, None, :] - self.cell_pos[None, :, :]).pow(2).mean(dim=-1)
        prototype_dist2 = (self.cell_pos[:, None, :] - self.prototype_pos[None, :, :]).pow(2).mean(dim=-1)
        ...
        history = []
        history_active = []
        active_cell_sum = x.new_zeros(B)
        active_tick_sum = x.new_zeros(B)
        write_intensity_sum = x.new_zeros(B)
        current_route_support_sum = x.new_zeros(B)
        current_compute_support_sum = x.new_zeros(B)
        ...
        prototype_support_sum = x.new_zeros(B)
        prototype_entropy_sum = x.new_zeros(B)
        prototype_pair_similarity_sum = x.new_zeros(B)
        route_pair_similarity_sum = x.new_zeros(B)
        final_active = None
        final_plasticity = None
        final_persistence = None
        final_write_intensity = None

        for _ in range(self.T):
            h_norm = self.norm(h)
            pooled_state = h_norm.mean(dim=1)
            cell_features = self._cell_features(base, h_norm, query_dist2)
            growth_pressure = self.growth(cell_features).squeeze(-1)
            shrink_pressure = F.softplus(self.shrink(cell_features)).squeeze(-1)
            active_logits = growth_pressure - shrink_pressure - 0.25 * query_dist2
            active = torch.sigmoid(active_logits)
            plasticity = torch.sigmoid(self.plasticity(cell_features)).squeeze(-1)
            persistence = torch.sigmoid(self.persistence(cell_features)).squeeze(-1)
            halt_input = torch.cat(
                [cell_features, pooled_state[:, None, :].expand(B, self.K, self.D)],
                dim=-1,
            )
            halt = torch.sigmoid(self.halt(halt_input).squeeze(-1))
            write_intensity = active * plasticity * halt

            q = self.route_query(cell_features)
            k = self.route_key(cell_features)
            relational_logits = torch.einsum("bif,bjf->bij", q, k) / np.sqrt(self.F)
            route_logits = (
                relational_logits
                + geometry_logits
                + self.edge_bias
                + torch.log(active[:, None, :] + 1e-6)
            )
            route_logits = route_logits.masked_fill(eye[None, :, :].bool(), -1e9)
            current_null_logits = self.null_route_bias.view(1, 1, 1).expand(B, self.K, 1)
            ...
            source_values = h_norm * active[:, :, None]
            msg = torch.bmm(route, source_values)
            lookback_msg = torch.zeros_like(h)
            current_support_sum = (write_intensity * route_support.sum(dim=-1)).sum(dim=1)
            current_effective_sum = (write_intensity * route_effective_support).sum(dim=1)
            current_route_support_sum = current_route_support_sum + current_support_sum
            current_compute_support_sum = current_compute_support_sum + (
                current_support_sum.detach() + current_effective_sum - current_effective_sum.detach()
            )
            route_entropy = -(route * torch.log(route + 1e-8)).sum(dim=-1)
            route_entropy_sum = route_entropy_sum + (route_entropy * read_weights).sum(dim=1)
            route_pair_similarity_sum = route_pair_similarity_sum + self._offdiag_mean(
                torch.bmm(route, route.transpose(1, 2))
            )

            prototype_logits = (
                -prototype_dist2[None, :, :] / temp[:, None, :]
                + self.prototype_bias.view(1, 1, self.P)
            )
            prototype_assign = torch.softmax(prototype_logits, dim=-1)
            prototype_mass = (write_intensity[:, :, None] * prototype_assign).sum(dim=1)
            prototype_state = torch.bmm(
                prototype_assign.transpose(1, 2),
                write_intensity[:, :, None] * h_norm,
            ) / (prototype_mass[:, :, None] + 1e-6)
            prototype_msg = torch.bmm(prototype_assign, prototype_state)
            prototype_support_sum = prototype_support_sum + effective_support(prototype_mass, dim=-1)
            prototype_entropy = -(prototype_assign * torch.log(prototype_assign + 1e-8)).sum(dim=-1)
            prototype_entropy_sum = prototype_entropy_sum + (prototype_entropy * read_weights).sum(dim=1)
            prototype_pair_similarity_sum = prototype_pair_similarity_sum + self._offdiag_mean(
                torch.bmm(prototype_assign, prototype_assign.transpose(1, 2))
            )

            if history:
                past = torch.stack(history, dim=1)
                past_active = torch.stack(history_active, dim=1)
                history_count = past.shape[1]
                past_norm = self.norm(past)
                source_key = self.route_key(self._cell_features(
                    base[:, None, :].expand(B, history_count, self.D).reshape(B * history_count, self.D),
                    past_norm.reshape(B * history_count, self.K, self.D),
                    query_dist2[:, None, :].expand(B, history_count, self.K).reshape(B * history_count, self.K),
                )).reshape(B, history_count, self.K, self.F)
                state_logits = torch.einsum("bif,bljf->bilj", q, source_key) / np.sqrt(self.F)
                lookback_logits = (
                    geometry_logits[:, :, None, :]
                    + state_logits
                    + self.lookback_edge_bias
                    + torch.log(past_active[:, None, :, :] + 1e-6)
                )
                lookback_route_logits = lookback_logits.reshape(B, self.K, history_count * self.K)
                lookback_null_logits = self.lookback_null_route_bias.view(1, 1, 1).expand(B, self.K, 1)
                ...
                past_values = past_norm * past_active[:, :, :, None]
                ...
                lookback_support_sum = (write_intensity * lookback_support.sum(dim=-1)).sum(dim=1)
                lookback_effective_sum = (write_intensity * lookback_effective_support).sum(dim=1)
                ...
                expected_active_lookback_edges = expected_active_lookback_edges + (
                    lookback_support_sum.detach() + lookback_effective_sum - lookback_effective_sum.detach()
                )
                lookback_entropy = -(lookback_route * torch.log(lookback_route + 1e-8)).sum(dim=-1)
                lookback_route_entropy_sum = lookback_route_entropy_sum + (lookback_entropy * read_weights).sum(dim=1)
                lookback_route_entropy_count += history_count * self.K

            mixed = h + persistence[:, :, None] * (
                self.route_mix * msg
                + self.lookback_mix * lookback_msg
                + self.prototype_mix * prototype_msg
            )
            delta = self.update(self.norm(mixed))
            h = h + self.residual_scale * budget[:, :, None] * write_intensity[:, :, None] * delta
            history.append(h)
            history_active.append(active)

            active_cell_sum = active_cell_sum + active.sum(dim=1)
            active_tick_sum = active_tick_sum + halt.mean(dim=1)
            ...
            final_write_intensity = write_intensity

        read_weights = final_write_intensity / (final_write_intensity.sum(dim=1, keepdim=True) + 1e-6)
        pooled = torch.sum(read_weights[:, :, None] * h, dim=1)
        out = self.readout(pooled)

        active_cells = active_cell_sum / self.T
        active_ticks = active_tick_sum
        active_edges = current_route_support_sum
        current_edge_capacity = max(self.T * self.K * (self.K - 1), 1)
        active_edge_frac = active_edges / current_edge_capacity
        expected_active_edges = current_compute_support_sum
        expected_active_edge_frac = expected_active_edges / current_edge_capacity
        ...
        route_entropy = route_entropy_sum / (self.T * np.log(max(self.K - 1, 2)))
        edge_entropy = route_entropy
        expected_edge_entropy = route_entropy
        lookback_norm = np.log(max(lookback_route_entropy_count, 2))
        lookback_edge_entropy = lookback_route_entropy_sum / lookback_norm
        expected_lookback_edge_entropy = lookback_edge_entropy
        lookback_route_entropy = lookback_route_entropy_sum / lookback_norm
        node_tick_compute = write_intensity_sum / max(self.T * self.K, 1)
        edge_tick_compute = expected_active_edge_frac
        prototype_support = prototype_support_sum / self.T
        prototype_support_frac = prototype_support / max(min(self.P, self.K), 1)
        prototype_compute = self.prototype_compute_weight * prototype_support_frac
        base_compute = (
            node_tick_compute
            + self.edge_compute_weight * edge_tick_compute
            + prototype_compute
        ) / (1.0 + self.edge_compute_weight + self.prototype_compute_weight)
        lookback_edge_compute = expected_active_lookback_edges / lookback_edge_capacity
        ...
        stats = {
            ...
            "plasticity": final_plasticity.mean(dim=1),
            "persistence": final_persistence.mean(dim=1),
            "budget": budget.squeeze(1),
            "temperature": temp.squeeze(1),
            "write_intensity": write_intensity_sum,
            "write_intensity_frac": node_tick_compute,
            "prototype_support": prototype_support,
            "prototype_support_frac": prototype_support_frac,
            "prototype_entropy": prototype_entropy_sum / (self.T * np.log(max(self.P, 2))),
            "prototype_pair_similarity": prototype_pair_similarity_sum / self.T,
            "route_pair_similarity": route_pair_similarity_sum / self.T,
        }
        return out, stats
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if last_stats is not None:
            ...
            for group in ("actor", "critic"):
                writer.add_scalar(f"plasticity/{group}_write_intensity", mean_stat(last_stats, group, "write_intensity"), global_step)
                writer.add_scalar(f"plasticity/{group}_write_intensity_frac", mean_stat(last_stats, group, "write_intensity_frac"), global_step)
                writer.add_scalar(f"symmetry/{group}_prototype_support", mean_stat(last_stats, group, "prototype_support"), global_step)
                writer.add_scalar(f"symmetry/{group}_prototype_support_frac", mean_stat(last_stats, group, "prototype_support_frac"), global_step)
                writer.add_scalar(f"symmetry/{group}_prototype_entropy", mean_stat(last_stats, group, "prototype_entropy"), global_step)
                writer.add_scalar(f"symmetry/{group}_prototype_pair_similarity", mean_stat(last_stats, group, "prototype_pair_similarity"), global_step)
                writer.add_scalar(f"symmetry/{group}_route_pair_similarity", mean_stat(last_stats, group, "route_pair_similarity"), global_step)
