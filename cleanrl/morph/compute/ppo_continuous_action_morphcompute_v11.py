...
#
# V11. Add a learned per-perceptron optimizer LR multiplier. Each substrate
# cell owns a learned multiplier on the global Adam update for cell-owned
# parameter slices. The same multiplier scales that cell's compute cost, so
# faster-learning cells pay more and slow/inert cells are cheaper.
#
...
class Args:
    ...
    cell_lr_min: float = 0.05
    """minimum learned per-cell LR multiplier"""
    cell_lr_max: float = 4.0
    """maximum learned per-cell LR multiplier"""
    cell_lr_init: float = 1.0
    """initial learned per-cell LR multiplier"""
...
class MorphogenicSubstrate(nn.Module):
    ...
    def __init__(self, obs_dim, args):
        ...
        self.cell_lr_min = args.cell_lr_min
        self.cell_lr_max = args.cell_lr_max
        ...
        cell_lr_frac = (args.cell_lr_init - args.cell_lr_min) / (args.cell_lr_max - args.cell_lr_min)
        cell_lr_frac = float(np.clip(cell_lr_frac, 1e-4, 1.0 - 1e-4))
        self.cell_lr_logit = nn.Parameter(
            torch.full((self.K,), float(np.log(cell_lr_frac / (1.0 - cell_lr_frac))))
        )
    ...
    def cell_lr_multiplier(self):
        return self.cell_lr_min + (self.cell_lr_max - self.cell_lr_min) * torch.sigmoid(self.cell_lr_logit)

    def capture_cell_owned_parameters(self):
        layers = {
            "growth": self.growth,
            "shrink": self.shrink,
        ...
        return {
            "cell_seed": self.cell_seed.detach().clone(),
            "cell_pos": self.cell_pos.detach().clone(),
            "edge_bias": self.edge_bias.detach().clone(),
            "lookback_edge_bias": self.lookback_edge_bias.detach().clone(),
            "null_route_bias": self.null_route_bias.detach().clone(),
            "lookback_null_route_bias": self.lookback_null_route_bias.detach().clone(),
            "layers": {
    ...
    def apply_cell_lr_update_multiplier(self, before):
        with torch.no_grad():
            lr = self.cell_lr_multiplier().detach()
            row_scale = lr[:, None]
            matrix_scale = 0.5 * (lr[:, None] + lr[None, :])
            self.cell_seed.copy_(before["cell_seed"] + row_scale * (self.cell_seed - before["cell_seed"]))
            self.cell_pos.copy_(before["cell_pos"] + lr[:, None] * (self.cell_pos - before["cell_pos"]))
            self.edge_bias.copy_(before["edge_bias"] + matrix_scale * (self.edge_bias - before["edge_bias"]))
            self.lookback_edge_bias.copy_(
                before["lookback_edge_bias"] + matrix_scale * (self.lookback_edge_bias - before["lookback_edge_bias"])
            )
            self.null_route_bias.copy_(
                before["null_route_bias"] + lr[:, None] * (self.null_route_bias - before["null_route_bias"])
            )
            self.lookback_null_route_bias.copy_(
                before["lookback_null_route_bias"]
                + lr[:, None] * (self.lookback_null_route_bias - before["lookback_null_route_bias"])
            )
            for name in ("growth", "shrink", "plasticity", "edge_source", "edge_target"):
                layer = getattr(self, name)
                old_weight, old_bias = before["layers"][name]
                layer.weight.copy_(old_weight + lr[:, None] * (layer.weight - old_weight))
                layer.bias.copy_(old_bias + lr * (layer.bias - old_bias))
    ...
    def forward(self, x):
        ...
        cell_lr = self.cell_lr_min + (self.cell_lr_max - self.cell_lr_min) * torch.sigmoid(self.cell_lr_logit)
        ...
        lr_weighted_active = active * cell_lr[None, :]
        current_route_support_sum = (lr_weighted_active * route_support.sum(dim=-1)).sum(dim=1)
        current_effective_support_sum = (lr_weighted_active * route_effective_support).sum(dim=1)
        ...
        for t in range(self.T):
            ...
            if history:
                ...
                lookback_support_sum = tick_gate * (
                    lr_weighted_active * lookback_support.sum(dim=-1)
                ).sum(dim=1)
                lookback_effective_support_sum = tick_gate * (
                    lr_weighted_active * lookback_effective_support
                ).sum(dim=1)
        ...
        lr_weighted_active_cell_frac = (active * cell_lr[None, :]).sum(dim=1) / self.K
        node_tick_compute = lr_weighted_active_cell_frac * active_tick_frac
        ...
        sorted_cell_lr = torch.sort(cell_lr).values
        p10_idx = min(int(0.10 * (self.K - 1)), self.K - 1)
        p50_idx = min(int(0.50 * (self.K - 1)), self.K - 1)
        p90_idx = min(int(0.90 * (self.K - 1)), self.K - 1)
        ...
        stats = {
            ...
            "cell_lr_multiplier": (active * cell_lr[None, :]).sum(dim=1) / (active.sum(dim=1) + 1e-6),
            "cell_lr_mean": cell_lr.mean().expand(B),
            "cell_lr_min": cell_lr.min().expand(B),
            "cell_lr_max": cell_lr.max().expand(B),
            "cell_lr_std": cell_lr.std(unbiased=False).expand(B),
            "cell_lr_p10": sorted_cell_lr[p10_idx].expand(B),
            "cell_lr_p50": sorted_cell_lr[p50_idx].expand(B),
            "cell_lr_p90": sorted_cell_lr[p90_idx].expand(B),
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                actor_lr_before = agent.actor.capture_cell_owned_parameters()
                critic_lr_before = agent.critic.capture_cell_owned_parameters()
                optimizer.step()
                agent.actor.apply_cell_lr_update_multiplier(actor_lr_before)
                agent.critic.apply_cell_lr_update_multiplier(critic_lr_before)
        ...
        if last_stats is not None:
            ...
            writer.add_scalar("morph/actor_cell_lr_multiplier", mean_stat(last_stats, "actor", "cell_lr_multiplier"), global_step)
            writer.add_scalar("morph/critic_cell_lr_multiplier", mean_stat(last_stats, "critic", "cell_lr_multiplier"), global_step)
            for group in ("actor", "critic"):
                writer.add_scalar(f"cell_lr/{group}_active_weighted_mean", mean_stat(last_stats, group, "cell_lr_multiplier"), global_step)
                writer.add_scalar(f"cell_lr/{group}_mean", mean_stat(last_stats, group, "cell_lr_mean"), global_step)
                writer.add_scalar(f"cell_lr/{group}_min", mean_stat(last_stats, group, "cell_lr_min"), global_step)
                writer.add_scalar(f"cell_lr/{group}_max", mean_stat(last_stats, group, "cell_lr_max"), global_step)
                writer.add_scalar(f"cell_lr/{group}_std", mean_stat(last_stats, group, "cell_lr_std"), global_step)
                writer.add_scalar(f"cell_lr/{group}_p10", mean_stat(last_stats, group, "cell_lr_p10"), global_step)
                writer.add_scalar(f"cell_lr/{group}_p50", mean_stat(last_stats, group, "cell_lr_p50"), global_step)
                writer.add_scalar(f"cell_lr/{group}_p90", mean_stat(last_stats, group, "cell_lr_p90"), global_step)
