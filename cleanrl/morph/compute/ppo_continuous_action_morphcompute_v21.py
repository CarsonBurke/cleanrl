# PPO + Morphogenic Compute v21.
#
# METHOD. v20 Property Flow plus zero-forward gradient bridges. Forward policy and
# value outputs remain the sparse v20 computation, but action/value logits include
# a train-time term of the form bridge - stopgrad(bridge). This has zero forward
# value and therefore does not perturb PPO behavior, while giving all cells a
# dense task-gradient river so suppressed cells/routes/update branches can learn
# before they are selected by sparse forward readouts.
import os
import random
import time
...
from cleanrl.morph.compute.ppo_continuous_action_morphcompute_v20 import (
    Agent as V20Agent,
    Args as V20Args,
    FLOW_CREDIT_FLOOR,
    make_env,
...
import cleanrl.morph.compute.ppo_continuous_action_morphcompute_v20 as v20mod
...
class Args(V20Args):
    ...
    bridge_coef: float = 0.05
    """zero-forward dense task-gradient bridge coefficient"""
    critic_null_route_bias: float = 0.25
    """lower critic no-read bias too, so value learning has the same flow as actor"""


class Agent(V20Agent):
    def __init__(self, envs, args=None):
        if args is None:
            args = Args()
        self.bridge_coef_value = args.bridge_coef
        super().__init__(envs, args)
        action_dim = int(np.prod(envs.single_action_space.shape))
        self.actor_bridge_alpha = nn.Linear(args.cell_dim, action_dim)
        self.actor_bridge_beta = nn.Linear(args.cell_dim, action_dim)
        self.critic_bridge_value = nn.Linear(args.cell_dim, 1)
        nn.init.orthogonal_(self.actor_bridge_alpha.weight, 0.01)
        nn.init.constant_(self.actor_bridge_alpha.bias, 0.0)
        nn.init.orthogonal_(self.actor_bridge_beta.weight, 0.01)
        nn.init.constant_(self.actor_bridge_beta.bias, 0.0)
        nn.init.orthogonal_(self.critic_bridge_value.weight, 0.01)
        nn.init.constant_(self.critic_bridge_value.bias, 0.0)
        with torch.no_grad():
            self.critic.null_route_bias.fill_(args.critic_null_route_bias)

    def _bridge(self, x):
        return self.bridge_coef_value * (x - x.detach())

    def _dense_cell_summary(self, cells):
    ...
    def get_value(self, x):
        ...
        bridge_value = self.critic_bridge_value(self._dense_cell_summary(critic_cells)).squeeze(-1)
    ...
    def _actor_dist(self, actor_features, actor_cells, actor_weights):
        ...
        summary = self._dense_cell_summary(actor_cells)
        ...
        alpha_logits = alpha_logits + self._bridge(self.actor_bridge_alpha(summary))
        beta_logits = beta_logits + self._bridge(self.actor_bridge_beta(summary))
    ...
    def get_action_and_value(self, x, z=None):
        ...
        actor_stats["bridge_coef"] = x.new_full(logprob.shape, self.bridge_coef_value)
        ...
        critic_stats["bridge_coef"] = x.new_full(logprob.shape, self.bridge_coef_value)
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    v20mod.FLOW_CREDIT_FLOOR = args.route_flow_credit_floor
