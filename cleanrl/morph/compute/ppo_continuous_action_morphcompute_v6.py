...
class MorphogenicSubstrate(nn.Module):
    ...
    def forward(self, x, edge_noise=None):
        B = x.shape[0]
        base = self.input(x)
...
class Agent(nn.Module):
    ...
    def get_value(self, x, critic_edge_noise=None):
        critic_features, _ = self.critic(x, critic_edge_noise)
    ...
    def get_action_and_value(self, x, z=None, actor_edge_noise=None, critic_edge_noise=None):
        actor_features, actor_stats = self.actor(x, actor_edge_noise)
        critic_features, critic_stats = self.critic(x, critic_edge_noise)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, _, value, _, _, actor_edge_noise, critic_edge_noise, last_stats = (
    ...
    if args.save_model:
        ...
        class EvalAgent(Agent):
            ...
            def get_action_and_value(self, x, z=None):
                action, _, logprob, entropy, value, _, _, _, _, _ = super().get_action_and_value(x, z)
