...
class Args:
    ...
    init_lookback_null_route_bias: float = 0.0
    ...
    route_dense_floor: float = 0.01
    """small softmax mixing floor so off-support sparse routes still receive task gradient"""
...
def sparse_route_with_floor(logits, dense_floor, dim=-1):
    sparse_route = sparsemax(logits, dim=dim)
    if dense_floor <= 0.0:
...
class MorphogenicSubstrate(nn.Module):
    ...
    def forward(self, x):
        ...
        route_with_null, sparse_route_with_null = sparse_route_with_floor(
        ...
        for t in range(self.T):
            ...
            if history:
                ...
                lookback_route_with_null, sparse_lookback_route_with_null = sparse_route_with_floor(
                ...
                active_lookback_edges = active_lookback_edges + tick_gate * (
                    active * lookback_support.sum(dim=-1)
                ).sum(dim=1)
                expected_active_lookback_edges = expected_active_lookback_edges + tick_gate * (
        ...
        expected_active_edges = current_effective_support_sum
