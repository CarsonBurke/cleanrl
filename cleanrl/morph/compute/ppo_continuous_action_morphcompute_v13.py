...
#
# V13. Starting directly from v9, charge plastic state rewrites instead of
# merely active cells. Plasticity remains a forward-pass write gate with
# immediate task-loss credit, while compute pressure pays for active plastic
# write intensity plus sparse current/lookback routing.
#
...
class MorphogenicSubstrate(nn.Module):
    ...
    def forward(self, x):
        ...
        write_intensity = active * plasticity
        history = []
        active_lookback_edges = x.new_zeros(B)
        expected_active_lookback_edges = x.new_zeros(B)
        current_route_support_sum = (write_intensity * route_support.sum(dim=-1)).sum(dim=1)
        current_effective_support_sum = (write_intensity * route_effective_support).sum(dim=1)
        ...
        for t in range(self.T):
            ...
            if history:
                ...
                lookback_support_sum = tick_gate * (
                    write_intensity * lookback_support.sum(dim=-1)
                ).sum(dim=1)
                lookback_effective_support_sum = tick_gate * (
                    write_intensity * lookback_effective_support
                ).sum(dim=1)
        ...
        write_intensity_frac = write_intensity.sum(dim=1) / self.K
        node_tick_compute = write_intensity_frac * active_tick_frac
        ...
        weighted_plasticity = write_intensity.sum(dim=1) / (active.sum(dim=1) + 1e-6)
        sorted_plasticity = torch.sort(plasticity, dim=1).values
        ...
        stats = {
            ...
            "write_intensity": write_intensity.sum(dim=1),
            "write_intensity_frac": write_intensity_frac,
            "plasticity_active_weighted_mean": weighted_plasticity,
            "plasticity_min": plasticity.min(dim=1).values,
            "plasticity_max": plasticity.max(dim=1).values,
            "plasticity_std": plasticity.std(dim=1, unbiased=False),
            "plasticity_p10": sorted_plasticity[:, p10_idx],
            "plasticity_p50": sorted_plasticity[:, p50_idx],
            "plasticity_p90": sorted_plasticity[:, p90_idx],
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if last_stats is not None:
            ...
            for group in ("actor", "critic"):
                writer.add_scalar(f"plasticity/{group}_active_weighted_mean", mean_stat(last_stats, group, "plasticity_active_weighted_mean"), global_step)
                writer.add_scalar(f"plasticity/{group}_mean", mean_stat(last_stats, group, "plasticity"), global_step)
                writer.add_scalar(f"plasticity/{group}_min", mean_stat(last_stats, group, "plasticity_min"), global_step)
                writer.add_scalar(f"plasticity/{group}_max", mean_stat(last_stats, group, "plasticity_max"), global_step)
                writer.add_scalar(f"plasticity/{group}_std", mean_stat(last_stats, group, "plasticity_std"), global_step)
                writer.add_scalar(f"plasticity/{group}_p10", mean_stat(last_stats, group, "plasticity_p10"), global_step)
                writer.add_scalar(f"plasticity/{group}_p50", mean_stat(last_stats, group, "plasticity_p50"), global_step)
                writer.add_scalar(f"plasticity/{group}_p90", mean_stat(last_stats, group, "plasticity_p90"), global_step)
