# PPO + Morphogenic Symmetry v5.
#
# METHOD. v4 (gated ReGLU^2 positional graph) plus LEARNED TOKEN SIZES. Until now
# every unit carried a fixed D-wide vector: token capacity was hard-coded. v5 makes
# each unit's effective width emergent. A per-unit channel gate g_u in [0,1]^D is
# derived from the SAME learned positional field, g_u = sigmoid(W_size @ phi(p_u) +
# bias), and applied to each tick's attention and FFN *deltas* (not the residual
# stream -- gating the stream fights LayerNorm). A channel with g~0 never updates,
# so the unit is effectively narrower. Its effective size is sum_c g_{u,c}, charged
# to compute, so -- exactly like distinct-template count and attention support --
# the network pays for width only where it helps and discovers per-unit capacity.
# Position now sets a unit's size, its weights (tying), and its routing: one learned
# geometry drives all three. Init bias is high so every unit starts near full width
# (~= v4); the compute tax then shrinks widths that don't earn their keep.
#
# HYPOTHESIS. Letting the field allocate representation width per unit -- more where
# the sub-problem is rich, less where it is simple -- improves capacity efficiency
# and performance over the uniform-width v4 at matched cost.
import os
import random
import time
...
from cleanrl.morph.sym.ppo_continuous_action_morphsym_v4 import (
    Args as V4Args,
    Agent as V4Agent,
    SymGraph4,
)
...
class Args(V4Args):
    ...
    size_init_bias: float = 3.0
    """initial per-channel width-gate logit; sigmoid(3)~0.95 starts near full width (~=v4)"""
    size_compute_weight: float = 1.0
    """weight on the effective-token-size (per-unit width) compute cost"""


class SymGraph5(SymGraph4):
    """v4 positional graph with a per-unit learned channel-width gate (learned token sizes)."""

    def __init__(self, obs_dim, args, n_out):
        super().__init__(obs_dim, args, n_out)
        self.size_compute_weight = args.size_compute_weight
        self.size_logits = layer_init(nn.Linear(self.phi_dim, self.D), std=0.01)
...
class Agent(V4Agent):
    def __init__(self, envs, args=None):
        ...
        self.actor = SymGraph5(obs_dim, args, n_out=action_dim)
        self.critic = SymGraph5(obs_dim, args, n_out=1)
