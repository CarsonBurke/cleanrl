# PPO + Morphogenic Symmetry v8.
#
# METHOD. v7 (per-template learned token width) with a WIDE channel budget: cell_dim
# is raised to 128 so each template's effective width can range from ~1 up to 128
# active channels. Width is still a per-template property inherited through the entmax
# assignment, g_u = A_u @ sigmoid(template_width) with template_width shape (M, 128).
# Tokens start near full width (~128) and the compute tax carves each template down
# toward as few channels as it needs (down to ~1), while leaving room to keep up to
# 128 where the sub-problem is rich. This gives token size a real dynamic range
# instead of the narrow, near-inert band it had at D=64.
#
# HYPOTHESIS. With genuine head-room (1..128) and per-template symmetry, the network
# will differentiate template widths meaningfully -- a few fat templates for the hard
# sub-problems, many thin ones elsewhere -- improving capacity allocation per unit of
# priced compute. (Note: D=128 roughly doubles real FLOPs/memory vs D=64; the priced
# width cost is still not yet a real FLOP saving -- see the sparse-compute TODO line.)
import os
import random
import time
...
from cleanrl.morph.compute.ppo_continuous_action_morphcompute_v9 import mean_stat, make_env
from cleanrl.morph.compute.ppo_continuous_action_morphcompute_v18 import signed_loss_with_safe_compute
from cleanrl.morph.sym.ppo_continuous_action_morphsym_v7 import Args as V7Args, Agent as V7Agent
...
# Agent/SymGraph7 read self.D = args.cell_dim, so cell_dim=128 gives the full range.
Agent = V7Agent
