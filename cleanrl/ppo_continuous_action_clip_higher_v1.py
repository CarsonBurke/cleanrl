# Ablation of Beta PPO: asymmetric clip-higher policy clipping.
# Ratio is clipped to [1-0.2, 1+0.28] so advantageous probability increases
# may travel farther than decreases. Value clipping stays symmetric at 0.2.
# Otherwise identical to cleanrl/ppo_continuous_action.py.
import os
import random
import time
...
class Args:
    ...
    clip_coef: float = 0.2
    """symmetric value-clip width; also the default lower policy clip"""
    clip_coef_low: float = 0.2
    """policy ratio lower clip: ratio >= 1 - clip_coef_low"""
    clip_coef_high: float = 0.28
    """policy ratio upper clip: ratio <= 1 + clip_coef_high (clip-higher)"""
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args):
    ...
    with torch.no_grad():
        ...
        clipfrac = (
            (ratio < (1.0 - args.clip_coef_low)) | (ratio > (1.0 + args.clip_coef_high))
        ).float().mean()
    if args.norm_adv:
        advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8)
    pg_loss1 = -advantages * ratio
    pg_loss2 = -advantages * torch.clamp(
        ratio, 1.0 - args.clip_coef_low, 1.0 + args.clip_coef_high
    )
...
def validate_args(args):
    ...
    if min(args.clip_coef_low, args.clip_coef_high) <= 0:
        raise ValueError("policy clip coefficients must be positive")
