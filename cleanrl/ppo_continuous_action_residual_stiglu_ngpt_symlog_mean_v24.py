# Scalar critic-coordinate ablation of historical job8254/v7: normalized rewards,
# GAE/V, unchanged residual SiTU-GLU actor/critic, fused Adam and matrix projection.
# Hypothesis: symlog coordinates help without changing the conditional-mean target.
# symlog_mean has dL/dz = symexp(z) - y, hence z* = symlog(E[y]); symlog_mse
# instead fits E[symlog(y)]. All arms retain critic-first initialization and gains:
# identical initial head coordinates do NOT imply identical initial decoded V.
import json
import math
import os
import random
...
def scalar_value_loss(prediction, targets, mode):
    """Mean scalar loss against detached normalized-return targets.

    For symlog_mean, the convex potential's derivative is symexp(z), so the
    per-example gradient is symexp(z) - y (divided by N after reduction).
    The target-only conjugate makes the minimum zero without changing gradients.
    """
    targets = targets.detach()
    if mode == "mse":
        return 0.5 * ((prediction - targets) ** 2).mean()
    if mode == "symlog_mse":
        return 0.5 * ((prediction - symlog(targets)) ** 2).mean()
    if mode == "symlog_mean":
        ...
        return (potential - targets * prediction + conjugate).mean()
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args):
    """Pure clipped PPO loss; a single critic forward supplies its scalar coordinate."""
    ...
    if args.clip_vloss:
        ...
        v_loss = scalar_value_loss(newvalue, returns, args.critic_loss)
    ...
    try:
        ...
        def critic_statistics(observations, returns):
            ...
            return {
                ...
                "critic/postupdate_ev": explained_variance(decoded, returns),
                "critic/decoded_mean": decoded.mean(),
                "critic/decoded_min": decoded.min(),
                "critic/decoded_max": decoded.max(),
                "critic/decoded_finite_fraction": torch.isfinite(decoded).float().mean(),
                "critic/coordinate_mean": coordinate.mean(),
                "critic/coordinate_min": coordinate.min(),
                "critic/coordinate_max": coordinate.max(),
                "critic/coordinate_abs_max": coordinate.abs().max(),
                "critic/coordinate_finite_fraction": torch.isfinite(coordinate).float().mean(),
            }
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            logged = gather_metrics({
                ...
                "critic/target_mean": b_returns.mean(), "critic/target_std": b_returns.std(unbiased=False),
