# PPO + IterThink v185 projspace-workingrep-k6 (fix A for v184's regression vs nGPT).
#
# v185 = v184 (std pre-LN transformer WM) with the JEPA projector emb promoted to be THE
#   WORKING REPRESENTATION, making the WM le-wm-faithful end to end. This is "fix A" from
#   the v184-vs-le-wm audit; v186 is the orthogonal "fix B" (double-norm + predictor warm-
#   start).
#   THE BUG (v184). In le-wm the projector emb is the latent: predictor, JEPA target, and
#   SIGReg all operate in emb space. v184 instead kept the RAW encoder summary as the
#   recurrent/belief/agent latent and applied the projector only INSIDE the loss, to an
#   OFF-PATH copy. Net effect: SIGReg and the JEPA MSE regularized a representation NOTHING
#   else used, while the actual working latent (what the predictor recurses on and the agent
#   reads) was shaped by no isotropy/predictability pressure at all. nGPT v182 never had this
#   gap -- its unit-sphere normalization constrained the on-path latent directly.
#   THE FIX (v185).
#     - encode_summary now applies jepa_projector per token, so its output emb IS the latent.
#       The recurrent stream, belief, agent input, JEPA target, and SIGReg all share emb.
#     - The loss drops the second projector pass: target = encoder emb, pred = the predictor's
#       pred_next_proj output (the asymmetric le-wm pred head, emb->emb), MSE between them.
#       SIGReg runs on emb directly. (le-wm: target=emb, pred=pred_proj(predictor(emb)).)
#     - Projector BatchNorm -> LayerNorm. The projector now runs on the CONTROL path,
#       including single-step rollout/imagination (batch = num_envs), so its internal norm
#       must be batch-independent. LayerNorm gives identical encode at train/rollout/imagine
#       and is cudagraph-safe (no running-stat buffers). SIGReg's N(0,I) target stays well-
#       posed (it shapes the raw fc2 output; the unit sphere BN was escaping is gone since v184).
#   WHY THIS SHOULD HELP. The predictor recurses on, and the agent consumes, a latent that is
#   now directly pushed toward isotropic-Gaussian + one-step-predictable. Imagination is self-
#   consistent (predict next emb, feed emb back). Scale matches: pred_next_proj output (~unit
#   var, xavier) vs emb (~N(0,I) via SIGReg), so MSE and the train-vs-imagine input agree.
#   RISK. emb depth grows (encoder -> target_proj -> projector); watch encode cost and early
#   stability. SIGReg now directly constrains the agent's input distribution -- re-check the
#   lewm_sigreg_coef balance and KL at ~1M.
#
# -----------------------------------------------------------------------------
...
class JEPAProjector(nn.Module):
    """le-wm projector head (Linear -> LayerNorm -> GELU -> Linear).

    v185: the projector emb is now THE WORKING REPRESENTATION (encode_summary applies this
    head; the recurrent/belief/agent stream and the JEPA objective ALL live in emb space).
    Because it now runs on the control path -- including single-step rollout/imagination
    where the batch is just num_envs -- the internal norm must be BATCH-INDEPENDENT.
    v183/v184 used BatchNorm1d (batch-stat whitening, fine off-path with a large pooled
    batch); v185 swaps it for LayerNorm so encode is identical at train, rollout, and
    imagine time (no batch-size-dependent statistics). The SIGReg N(0,I) target stays
    well-posed: the projector OUTPUT (fc2, raw) is what SIGReg shapes, and the unit-sphere
    constraint that BN was introduced to escape is already gone (v184). LayerNorm accepts
    arbitrary leading dims, so callers may pass (..., dim) directly."""
    ...
    def forward(self, x):  # x: (..., dim)
        return self.fc2(F.gelu(self.norm(self.fc1(x))))


class StdSelfAttention(nn.Module):
    # v184: standard multi-head self-attention. The nGPT machinery is removed -- no sqk,
    # no per-head q/k unit-normalization, and the cosine-logit scale (head_dim**0.5) is
    # replaced by the conventional softmax temperature head_dim**-0.5 (sdpa's default, so
    # `scale` is omitted). Pre-norm + residual are owned by the enclosing block.
...
class LeWMTransformerBlock(nn.Module):
    # v184: standard pre-LayerNorm transformer block (encoder). The nGPT eigen-learning-rate
    # residual interpolation, unit-sphere block-entry norm, and gated-SwiGLU-with-sqrt(D)
    # MLP are replaced by the canonical x = x + attn(LN(x)); x = x + mlp(LN(x)) with a GELU
    # MLP. Each sublayer carries one pre-norm; the attention/MLP have no internal norm.
...
class AdaLNAxialPredictorBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, ffn_mult, axis, rope_fraction):
        ...
        self.axis = axis
        self.dim = dim
        # v184: le-wm ConditionalBlock (DiT AdaLN-zero) -- affine-free pre-LayerNorms; the
        # zero-init modulation MLP supplies (shift, scale, gate) per sublayer. At init the
        # gates are 0 so each sublayer is an exact identity (action influence ramps in),
        # and the residual is a STANDARD gated add (the nGPT unit-sphere eigen-lr
        ...
        self.w1 = xavier_linear(nn.Linear(dim, dim * ffn_mult))
        self.w2 = xavier_linear(nn.Linear(dim * ffn_mult, dim))
        # AdaLN-zero: SiLU then a zero-init linear to 6*dim = (shift, scale, gate) x2.
...
class LeWMBackbone(nn.Module):
    ...
    def encode_summary(self, obs, incoming_outcome):
        ...
        # v185 (fix A): the projector emb IS the working representation. le-wm's predictor,
        # JEPA target and SIGReg all operate in projector emb space; v184 instead left the
        # raw encoder summary as the recurrent/belief/agent latent and SIGReg'd only an
        # OFF-PATH projector emb, so the actual working space was regularized by nothing.
        # Applying the projector here (per token) makes emb the single latent everything
        # shares -- the recurrent stream, belief, agent input, JEPA MSE and SIGReg.
        return self.jepa_projector(summary)  # (B,19,D) in emb space
...
if __name__ == "__main__":
    ...
    def wm_supervised_loss_forward(
        ...
        # v185 (fix A): encoded_sequence is ALREADY in projector emb space (encode_summary
        # applies the projector), and the predictor's pred_next_proj is the ASYMMETRIC
        # prediction head mapping the belief to emb space -- so one_step_pred and the encoder
        # emb are directly comparable. No second projector pass: target = encoder emb,
        # pred = predicted emb, MSE between them (le-wm: target=emb, pred=pred_proj(predictor)).
        emb_sequence = encoded_sequence
        pred_emb = one_step_pred
        ...
        # v185: SIGReg over the FULL summary in le-wm projector emb space. emb_sequence IS
        # the working latent (encode_summary applies the LayerNorm projector), so SIGReg
        # drives the SAME on-path emb the MSE target and the agent input live in toward
        # isotropic N(0,I). No sqrt(D) rescale -- emb is shaped to N(0,I) by SIGReg itself.
    ...
    def wm_supervised_metrics_forward(
        ...
        one_step_pred = pred_mtp[:, :, 0, :]
        # v185 (fix A): encoded_sequence is already in projector emb space; mirror the loss
        # path -- target = encoder emb, pred = predicted emb, MSE direct (no second projector).
        emb_sequence = encoded_sequence
        pred_emb = one_step_pred
        target_emb = emb_sequence[:, 1:]
        one_step_latent_mse = F.mse_loss(pred_emb, target_emb, reduction="none").mean(dim=(-1, -2))
