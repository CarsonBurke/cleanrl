# PPO + EPO (Evolutionary Policy Optimization) v4
#
...
#
# v4 fixes a *growing* / out-of-control KL (0.015 -> 0.3, spikes to 2.0) that v3
# still showed, while healthy baselines keep KL ~0.01 and DECAYING. Root cause:
# the shared off-policy follower rows feed the same actor as on-policy rows, and
# PPO's clip only zeroes the gradient when advantage and ratio move together. For
# off-policy rows with NEGATIVE advantage and a huge importance ratio (exactly
# what arises as genomes diverge), torch.max picks the UNCLIPPED -adv*ratio term
# -> a large unbounded gradient that perturbs the shared actor for every genome,
# escalating as the population specializes. Two fixes:
#   * Dual-clip PPO (Ye et al. 2020): cap the negative-advantage loss at
#     dual_clip*|adv|, bounding those destabilizing gradients. Standard robust PPO.
#   * Honest KL/clipfrac metrics: report them over the ON-POLICY rows only (the
#     follower rows are intentionally off-policy and don't belong in the
#     trust-region metric); off-policy KL/clipfrac are logged separately.
#
# v3 fixed an off-policy SCALE mismatch found by a parity audit that caused
# extreme KL (~0.13 vs healthy ~0.01) and clipfrac (~0.30). The off-policy share
# is a fraction 1/(K+1) of the batch; the reference uses K=64 blocks (24576 envs)
# so off-policy mass is ~1.5%, but with only 16 envs we use K=4 -> 20%, ~13x too
# much. Reusing the behavior log-prob/mu/sigma for relabeled rows (faithful to
# the reference, a2c_common.py:993-995) is only safe when that mass is tiny; at
# 20% those high-KL rows dominate the batch, pollute advantage normalization, and
# waste clipped gradient. v3 caps the shared off-policy data to the reference's
# per-update mass (~batch/64) by subsampling the follower block, keeping diverse
# std-1 genomes. This is the true-parity fix at our env scale (K=64 is infeasible
# with 16 envs, so we match the off-policy *fraction* instead).
#
# v2 closed three fidelity gaps an alignment audit found against the reference
# `--epo` path:
...
class Args:
    ...
    off_policy_frac: float = 0.03
    """cap each share's off-policy rows to this fraction of the on-policy batch (reference ~1/64 at K=64); keeps off-policy mass near the reference instead of the 1/(K+1) we'd get at small K"""
    dual_clip: float = 3.0
    """dual-clip PPO coefficient (>1): caps the negative-advantage policy loss at dual_clip*|adv|, bounding the large unclipped gradients off-policy rows would otherwise inject. <=1 disables"""
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # makes this a safe off-policy update. We then SUBSAMPLE the relabeled rows
        # down to `off_policy_frac` of the on-policy batch, matching the reference's
        # tiny per-update off-policy mass (~1/64 at K=64) instead of the 1/(K+1)=20%
        # we'd otherwise inject at K=4 — which is what drove the extreme KL/clipfrac.
        n_off = max(1, int(args.off_policy_frac * args.batch_size)) if args.share_repeats > 0 else 0
        with torch.no_grad():
            for rep in range(args.share_repeats):
                ...
                # flatten the block, then subsample to the reference off-policy mass
                f_obs = o_blk.reshape((-1,) + envs.single_observation_space.shape)
                f_act = actions[:, lo:hi].reshape((-1,) + envs.single_action_space.shape)
                f_logp = logprobs[:, lo:hi].reshape(-1)  # behavior old log-prob, reused
                f_adv = adv_f.reshape(-1)
                f_ret = returns_f.reshape(-1)
                f_val = vf.reshape(-1)
                n_avail = f_obs.shape[0]
                sel = torch.randperm(n_avail, device=device)[: min(n_off, n_avail)]

                b_obs = torch.cat([b_obs, f_obs[sel]], 0)
                b_actions = torch.cat([b_actions, f_act[sel]], 0)
                b_logprobs = torch.cat([b_logprobs, f_logp[sel]], 0)
                b_advantages = torch.cat([b_advantages, f_adv[sel]], 0)
                b_returns = torch.cat([b_returns, f_ret[sel]], 0)
                b_values = torch.cat([b_values, f_val[sel]], 0)
                b_genome = torch.cat([b_genome, fg[sel]], 0)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, total_size, args.minibatch_size):
                ...
                # on-policy rows are the first `batch_size` entries of the batch (followers appended after)
                mb_on = torch.as_tensor(mb_inds < args.batch_size, device=device)
                ...
                with torch.no_grad():
                    # KL/clipfrac as a trust-region metric over ON-POLICY rows only;
                    # follower rows are intentionally off-policy (logged separately).
                    kl_row = (ratio - 1) - logratio
                    clip_row = ((ratio - 1.0).abs() > args.clip_coef).float()
                    old_approx_kl = (-logratio)[mb_on].mean()
                    approx_kl = kl_row[mb_on].mean()
                    clipfracs += [clip_row[mb_on].mean().item()]
                    if (~mb_on).any():
                        offpol_kls.append(kl_row[~mb_on].mean().item())
                        offpol_clipfracs.append(clip_row[~mb_on].mean().item())

                mb_advantages = b_advantages[mb_inds]
                if args.norm_adv:
                    mb_advantages = (mb_advantages - mb_advantages.mean()) / (mb_advantages.std() + 1e-8)

                # Policy loss (PPO clip + dual-clip on negative-advantage rows)
                pg_loss1 = -mb_advantages * ratio
                pg_loss2 = -mb_advantages * torch.clamp(ratio, 1 - args.clip_coef, 1 + args.clip_coef)
                pg_clip = torch.max(pg_loss1, pg_loss2)
                if args.dual_clip > 1.0:
                    # for adv < 0, cap the loss at dual_clip*|adv| to bound unclipped
                    # gradients from high-ratio off-policy rows (Ye et al. 2020)
                    pg_clip = torch.where(
                        mb_advantages < 0,
                        torch.min(pg_clip, -args.dual_clip * mb_advantages),
                        pg_clip,
                    )
        ...
        if offpol_kls:
            writer.add_scalar("losses/offpolicy_approx_kl", np.mean(offpol_kls), global_step)
            writer.add_scalar("losses/offpolicy_clipfrac", np.mean(offpol_clipfracs), global_step)
