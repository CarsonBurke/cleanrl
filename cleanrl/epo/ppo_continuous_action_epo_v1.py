# PPO + EPO (Evolutionary Policy Optimization) v1
#
...
class Args:
    ...
    genome_dim: int = 8
    """dimension of each learnable genome latent z_k"""
    ent_coef_max: float = 0.005
    """top of the per-genome entropy-bonus spectrum (genome 0 = explorer); decays linearly to 0 at the exploiter genome"""
    share_repeats: int = 1
    """leader-follower off-policy repeats: how many additional genome relabelings to add to the PPO batch each update (0 disables sharing)"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, num_genomes, genome_dim):
        ...
        self.genomes = nn.Parameter(0.1 * torch.randn(num_genomes, genome_dim))
    ...
    def _condition(self, x, genome_idx):
        # genome_idx: None -> exploiter genome (last, entropy coef 0); else LongTensor (batch,)
    ...
    def get_action_and_value(self, x, genome_idx=None, action=None):
        cond = self._condition(x, genome_idx)
        head_alpha, head_beta = self.actor(cond).chunk(2, dim=-1)
        alpha = 1.0 + F.softplus(head_alpha)
        beta = 1.0 + F.softplus(head_beta)
        ...
        return action, probs.log_prob(z).sum(1), probs.entropy().sum(1), self.critic(cond)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ---- EPO leader-follower experience sharing -------------------------------
        # Relabel every transition under a *shifted* genome, recompute value and a
        # 1-step TD return/advantage with the follower's critic, keep the behavior
        # old log-prob. PPO clipping makes this a safe off-policy update that shares
        # exploratory data across the whole population.
        with torch.no_grad():
            for rep in range(args.share_repeats):
                roll = random.randint(1, args.num_genomes - 1)  # which other genome each env borrows from
                follower_env_genome = (env_genome + roll) % args.num_genomes  # (num_envs,)
                fg = follower_env_genome  # alias

                # follower critic values over the whole rollout + bootstrap state
                vf = agent.get_value(
                    obs.reshape(-1, obs.shape[-1]),
                    fg.repeat(args.num_steps),
                ).reshape(args.num_steps, args.num_envs)
                vf_last = agent.get_value(next_obs, fg).reshape(-1)
                ...
                nnt[:-1] = 1.0 - dones[1:]
                nnt[-1] = 1.0 - next_done
                returns_f = rewards + args.gamma * vf_next * nnt
                adv_f = returns_f - vf

                b_obs = torch.cat([b_obs, obs.reshape((-1,) + envs.single_observation_space.shape)], 0)
                b_actions = torch.cat([b_actions, actions.reshape((-1,) + envs.single_action_space.shape)], 0)
                b_logprobs = torch.cat([b_logprobs, logprobs.reshape(-1)], 0)  # behavior old log-prob, reused
                ...
                b_genome = torch.cat([b_genome, fg.repeat(args.num_steps)], 0)
