# PPO + IterThink v214 affine-RMS forecast-only policy recurrence.
#
# Preserve v213's affine-RMS observation encoder, attached LeJEPA/SIGReg,
# action-free temporal dynamics, action-conditioned policy-moment forecast,
# exact-Beta PPO guards, and imagination. Remove every direct observation and
# temporal-memory path into the actor and critic: both consume only the detached
# incoming forecast [predicted native policy mean, clamped native log-std]. The
# forecast is generated from (WM history through s_t, actual a_t) and becomes the
# sole agent state at s_(t+1); resets receive zero. Independent affine RMSNorm +
# dense interfaces are live from step zero (no zero residual or warmup bypass).
# Rollout successor targets are the no-exploration actor distribution evaluated
# on that exact detached successor forecast. Dream seeds recompute the incoming
# forecast with the current WM from the preceding real state/action window,
# avoiding stale forecast/current-WM hybrids.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if not self.detach_world_model_from_agent:
            raise ValueError("v214 requires a strictly detached forecast/agent interface")
        self.share_backbone = args.share_backbone
        if self.share_backbone:
            raise ValueError("v214 requires separate actor/critic forecast interfaces and trunks")
        self.explore_code_dim = args.explore_code_dim
        self.explore_logit_scale = args.explore_logit_scale
        self.forecast_dim = 2 * act_dim
        # v214's agent state is exactly the detached incoming policy forecast.
        # The WM belief remains private to dynamics and forecast generation.
        self.agent_input_dim = self.forecast_dim
        self.actor_forecast_norm = nn.RMSNorm(self.forecast_dim)
        self.critic_forecast_norm = nn.RMSNorm(self.forecast_dim)
        self.actor_forecast_input = layer_init(
            nn.Linear(self.forecast_dim, args.lewm_dim), std=np.sqrt(2)
        )
        self.critic_forecast_input = layer_init(
            nn.Linear(self.forecast_dim, args.lewm_dim), std=np.sqrt(2)
        )
    ...
    def pack_agent_input(self, incoming_forecast):
        if incoming_forecast.shape[-1] != self.forecast_dim:
            raise ValueError("incoming policy forecast has the wrong width")
        if self.detach_world_model_from_agent:
            incoming_forecast = incoming_forecast.detach()
        return incoming_forecast

    def agent_input_from_forecast(self, incoming_forecast):
        return self.pack_agent_input(incoming_forecast)
    ...
    def _base_actor_feat_from_agent_input(self, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        return self.actor_trunk(
            self.actor_forecast_input(self.actor_forecast_norm(agent_input))
        )
    ...
    def _critic_feat_from_agent_input(self, agent_input):
        ...
        return self.critic_trunk(
            self.critic_forecast_input(self.critic_forecast_norm(agent_input))
        )

    def _trunks(self, x):
        return self._trunks_from_agent_input(self.agent_input_from_forecast(x))
    ...
    def get_value(self, x):
        ...
        critic_feat = self._critic_feat_from_agent_input(self.agent_input_from_forecast(x))
    ...
    def get_action_and_value(self, x, z=None, explore_code=None):
        return self.get_action_and_value_from_agent_input(
            self.agent_input_from_forecast(x), z, explore_code
        )

    def get_action_and_value_from_agent_input(
        self, agent_input, z=None, explore_code=None, return_beta_params=False
    ):
    ...
    def actor_parameters(self):
        ...
        return (
            list(self.actor_forecast_norm.parameters())
            + list(self.actor_forecast_input.parameters())
    ...
    def critic_parameters(self):
        ...
        return (
            list(self.critic_forecast_norm.parameters())
            + list(self.critic_forecast_input.parameters())
            + list(self.critic_trunk.parameters())
            + list(self.critic_head.parameters())
        )

    @torch.no_grad()
    def assert_live_forecast_interface_init(self):
        for name, module in (
            ("actor", self.actor_forecast_input),
            ("critic", self.critic_forecast_input),
        ):
            if torch.count_nonzero(module.weight).item() == 0:
                raise AssertionError(f"{name} forecast interface must initialize live")
...
if __name__ == "__main__":
    ...
    assert args.detach_world_model_from_agent, "v214 requires PPO to be detached from the WM"
    assert args.lewm_dyn_horizon == args.lewm_context, (
        "v214 forecast targets require the teacher-forced final row to have the "
    ...
    assert not args.share_backbone, "v214 separates actor/critic forecast interfaces and trunks"
    ...
    agent.assert_live_forecast_interface_init()
    ...
    def build_wm_history_and_agent_input(obs_tensor, incoming_forecast=None):
        ...
        return (
            agent.agent_input_from_forecast(incoming_forecast),
            current_latent,
            belief_latents,
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                agent_input, current_latent, belief_latents = build_wm_history_and_agent_input(
            ...
            with torch.no_grad():
                # Strict forecast-only successor state: neither the observed
                # successor nor its belief is consulted by actor or critic.
                next_transition_agent_input = agent.agent_input_from_forecast(
                    predicted_successor_forecast
                )
                next_transition_agent_inputs[step] = next_transition_agent_input
                # This is deliberately a coupled fixed-point/self-distillation
                # label: pi_base(F_theta(history_t, a_t)). There is no direct
                # successor-observation policy teacher once belief input is removed.
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # The fixed label is pi_base applied to the rollout-time
                    # forecast itself, not a belief/observation-teacher policy.
                    forecast_weight = (
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            # env e) is clean if [j-W .. j] lies within one episode and j>=W.
            # The extra predecessor lets v214 freshly reconstruct f_j from a
            # full W-state context ending at s_(j-1), without leaking s_j.
            # dones[s]=1 marks an episode-start obs, so a reset between window
            # steps p-1,p shows as dones[p]=1; require dones==0 at j, j-1, ...,
            # j-W+1. We re-encode sampled obs windows with the current WM.
            ...
            for _ in range(args.imagine_batches):
                ...
                with torch.no_grad():
                    ...
                    obs_start = agent.world_model.obs_token_start
                    if clean_flat.numel() > 0:
                        win_d = torch.arange(W_im, -1, -1, device=device)             # [W,...,0]
                        win_steps = step_idx.unsqueeze(0) - win_d.unsqueeze(1)        # (W+1,B): s_{j-W}..s_j
                        win_rollouts = rollout_idx.unsqueeze(0).expand(W_im + 1, -1)
                        win_envs = env_idx.unsqueeze(0).expand(W_im + 1, -1)
                        win_summ = agent.world_model.encode_summary(
                            bank_obs[win_rollouts, win_steps, win_envs].reshape((-1,) + obs_shape),
                        ).detach().reshape(
                            W_im + 1, B_im, agent.world_model.num_latent_tokens, agent.world_model.dim
                        )
                        # Dynamics seed: W states s_{j-W+1}..s_j and W-1 actions
                        # a_{j-W+1}..a_{j-1}. The distinct forecast seed context
                        # is the preceding W states s_{j-W}..s_{j-1}.
                        # (action[d] is taken FROM state[d], pairing as the predictor expects).
                        hist_latents = [win_summ[d][:, obs_start:] for d in range(1, W_im + 1)]
                        win_actions = bank_actions[
                            win_rollouts[1:-1], win_steps[1:-1], win_envs[1:-1]
                        ]
                        hist_actions = [win_actions[d] for d in range(W_im - 1)]
                        # Reconstruct the sole agent state for s_j with the CURRENT
                        # WM from the causally preceding real history through
                        # s_(j-1) and the actual outbound a_(j-1). Never combine a
                        # stored old forecast with freshly re-encoded WM latents.
                        _, _, _, seed_forecast = (
                            agent.world_model.policy_forecast_from_history(
                                win_summ[:-1, :, obs_start:].transpose(0, 1).contiguous(),
                                bank_actions[
                        ...
                        # No causally preceding transition is available; use the
                        # same intentional zero state as an environment reset.
                    ...
                    for t in range(H_im):
                        ...
                        agent_input = agent.agent_input_from_forecast(
                            dream_incoming_forecast
                        )
                        action, z, logprob, _, value_logits = (
                            agent.get_action_and_value_from_agent_input(
                                agent_input, explore_code=im_explore_code
                            )
                        )
                    ...
                    # Bootstrap solely from the forecast carried into final state s_H.
                    value_probs_H = torch.softmax(
                        agent.get_value_from_agent_input(
                            agent.agent_input_from_forecast(dream_incoming_forecast),
                        )[:, 0],  # horizon 0 = V(s_H)
                        dim=-1,
                    )
        ...
        with torch.no_grad():
            ...
            diag_forecast = b_agent_inputs[:diag_n]
            actor_forecast_activation = agent.actor_forecast_input(
                agent.actor_forecast_norm(diag_forecast)
            )
            critic_forecast_activation = agent.critic_forecast_input(
                agent.critic_forecast_norm(diag_forecast)
            )
            forecast_centered = diag_forecast - diag_forecast.mean(dim=0, keepdim=True)
            forecast_cov = forecast_centered.T @ forecast_centered / max(1, diag_n - 1)
            forecast_variance = forecast_cov.diagonal().sum()
            forecast_effective_rank = forecast_variance.square() / (
                forecast_cov.square().sum().clamp_min(1e-12)
            )
            ...
            noexplore_action_mean = to_action_diag(dist_zero.mean)
            critic_value_diag = value_logits_to_scalar(
        ...
        writer.add_scalar(
            "policy_forecast/actor_interface_activation_rms",
            actor_forecast_activation.square().mean().sqrt().item(),
            global_step,
        )
        writer.add_scalar(
            "policy_forecast/critic_interface_activation_rms",
            critic_forecast_activation.square().mean().sqrt().item(),
            global_step,
        )
        writer.add_scalar(
            "policy_forecast/actor_interface_weight_rms",
            agent.actor_forecast_input.weight.square().mean().sqrt().item(),
            global_step,
        )
        writer.add_scalar(
            "policy_forecast/critic_interface_weight_rms",
            agent.critic_forecast_input.weight.square().mean().sqrt().item(),
            global_step,
        )
        writer.add_scalar(
            "policy_forecast/feature_rms",
            diag_forecast.square().mean().sqrt().item(),
            global_step,
        )
        writer.add_scalar(
            "policy_forecast/feature_batch_std",
            diag_forecast.std(dim=0, unbiased=False).mean().item(),
            global_step,
        )
        writer.add_scalar(
            "policy_forecast/feature_effective_rank",
            forecast_effective_rank.item(),
            global_step,
        )
        writer.add_scalar(
            "policy_forecast/noexplore_action_mean_batch_std",
            noexplore_action_mean.std(dim=0, unbiased=False).mean().item(),
            global_step,
        )
        writer.add_scalar(
            "policy_forecast/noexplore_actor_entropy",
            dist_zero.entropy().sum(dim=-1).mean().item(),
            global_step,
        )
        writer.add_scalar(
            "policy_forecast/critic_value_batch_std",
            critic_value_diag.std(unbiased=False).item(),
            global_step,
        )
        writer.add_scalar(
