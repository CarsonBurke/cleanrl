# PPO + IterThink v134 rawsymlog-mlp-scalar-k6 (v133 + dense symlog preprocessor).
#
# Key change from v132:
#   - Remove Gym's stateful NormalizeObservation/clip wrappers.
#   - Feed raw flattened MuJoCo observations through a Dreamer3-like fixed
#     symlog preprocessor before scalar tokenization:
#       z = symlog(raw_obs)
#       h = Linear(z) -> RMSNorm -> Linear -> ReLU^2 -> Linear
#       token_i = sum_k f_k(h_i) * W_{i,k} + b_i
#     where f_k = [h, |h|, h^2].
...
#   symlog MLP can provide Dreamer3-style dense cross-feature conditioning while
#   still outputting one scalar per feature for the existing tokenized WM.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # a running normalizer; per-feature weights keep each scalar identifiable.
        ...
        self.obs_pre_linear1 = xavier_linear(nn.Linear(obs_dim, self.dim))
        self.obs_pre_norm = nn.RMSNorm(self.dim)
        self.obs_pre_linear2 = xavier_linear(nn.Linear(self.dim, self.dim))
        self.obs_pre_linear3 = xavier_linear(nn.Linear(self.dim, obs_dim))
        self.obs_scalar_weight = nn.Parameter(torch.empty(obs_dim, 3, self.dim))
        self.obs_feature_bias = nn.Parameter(torch.empty(obs_dim, self.dim))
        nn.init.xavier_uniform_(self.obs_scalar_weight.reshape(obs_dim * 3, self.dim))
        nn.init.xavier_uniform_(self.obs_feature_bias)
    ...
    def encode_summary(self, obs):
        ...
        z = symlog(obs_flat)
        z = self.obs_pre_linear3(relu_sq(self.obs_pre_linear2(self.obs_pre_norm(self.obs_pre_linear1(z)))))
        obs_scalar_features = torch.stack([z, z.abs(), z.square()], dim=-1)
        obs_tokens = (
            obs_scalar_features.unsqueeze(-1) * self.obs_scalar_weight.to(obs_scalar_features.dtype).unsqueeze(0)
        ).sum(dim=-2)
        obs_tokens = obs_tokens + self.obs_feature_bias.to(obs_tokens.dtype)
