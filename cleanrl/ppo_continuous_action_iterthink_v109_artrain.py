        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # v109: AUTOREGRESSIVE rollout along the REAL action sequence is
                    # now a TRAINING path, le-wm style: NOTHING detached. The seed
                    # encoder summary and every fed-back prediction stay attached, so
                    # gradient flows through the whole rollout -- step h learns to be
                    # a good INPUT for step h+1 (multi-step consistency) and the
                    # encoder hears about rollout failures, exactly as the attached
                    # obs context does in the teacher-forced path. Per-step losses
                    # (latent MSE vs attached real targets, reward CE vs projected
                    # labels, continuation BCE) inject gradient at every depth, so
                    # effective path lengths are 1..H with most mass on short paths.
                    # The same pass still feeds the per-horizon drift probes (which
                    # are now train metrics, not held-out).
                    ar_hist_lat = [initial_summary[:, obs_start:]]
                    ar_hist_act = []
                    ...
                    for h in range(horizon):
                        ...
                        ar_hist_lat.append(ar_next_summary[:, obs_start:])
