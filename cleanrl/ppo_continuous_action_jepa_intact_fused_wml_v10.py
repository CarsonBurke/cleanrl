"""INTACT fused weighted-likelihood execution: algorithm-identical v9 comparator.

Retains v9's AWR-style executed-action likelihood, frozen conditional Beta law,
attached JEPA/local and hindsight goal-law learning, real critic, optimizer
ownership, update schedule and SIGReg projection/chunk defaults. Changes only
execution: minibatch indexing and metric packing live inside the compiled loss;
three independent gradient clips use one compiled tensor-only function; metrics
accumulate as one vector. No manual CUDA capture or optimizer replacement.

The deterministic proposal still has no reachability guarantee, and world/law
drift prevents a monotonic improvement claim. This compares execution costs;
it does not introduce a new goal selector or temporal-difference algorithm.
"""
import os
import random
import copy
...
def actor_objective(agent, observations, native_actions, weights, args):
    """Only prescriber parameters receive the weighted-action control gradient."""
    alpha, beta = agent.direct_policy(observations)
    return -(weights * agent.action_logprob(alpha, beta, native_actions)).mean()
...
def training_loss(agent, observations, native_actions, weights, returns, old_values,
                  next_observations, goal_observations, rewards, terminations, args):
    actor_loss = actor_objective(agent, observations, native_actions, weights, args)
    ...
    metrics.update({
        "losses/weighted_action_nll": actor_loss.detach(),
        "losses/value_loss": real_value_loss.detach(),
    })
    return model_loss + actor_loss + args.vf_coef * real_value_loss, metrics


# Stable log names are resolved once, outside the optimizer path. Packing inside
# the compiled loss avoids a separate eager kernel per scalar per minibatch.
METRIC_NAMES = (
    "world/prediction_loss", "world/sigreg_loss", "world/local_nll", "world/goal_nll",
    "world/reward_loss", "world/continuation_loss", "world/latent_std",
    "world/local_intent_norm", "world/goal_intent_norm", "world/total_loss",
    "losses/weighted_action_nll", "losses/value_loss",
)


def indexed_training_loss(agent, indices, batch, args):
    """Gather the nine factual rollout fields and pack diagnostics in one graph.

    Batch order follows training_loss: observations, native actions, weights,
    returns, old values, next observations, goal observations, rewards, terms.
    Compiled outputs must be consumed before the next graph step: backward the
    loss and add the packed metrics into independently owned storage.
    """
    loss, metrics = training_loss(agent, *(field[indices] for field in batch), args)
    return loss, torch.stack(tuple(metrics[name] for name in METRIC_NAMES))


def clip_gradient_groups(gradient_groups, max_norm):
    """In-place independent L2 clipping of explicit nonempty gradient groups.

    Same two-stage norm and 1e-6 denominator as nn.utils.clip_grad_norm_. The
    caller supplies gradients, so tracing never reads mutable Parameter.grad
    attributes and cannot capture stale gradient storage. Compile without CUDA
    graphs: parameter gradients belong to the separately compiled backward.
    """
    norms = []
    for gradients in gradient_groups:
        if not gradients:
            raise ValueError("each gradient owner must supply a nonempty tuple")
        total_norm = torch.linalg.vector_norm(torch.stack(torch._foreach_norm(gradients, 2.0)), 2.0)
...
def validate_args(args):
    ...
    if not args.cuda:
        raise ValueError("the weighted-action learner requires CUDA")
    return args
...
def main():
    ...
    try:
        ...
        writer.add_text("method", "Algorithm-identical v9: compiled gathers, packed metrics, and tensor-only independent gradient clipping.")
        ...
        def rollout_statistics(observations, native, following, goals, rewards, terminations):
            """Factual diagnostics before fitting this rollout; no actor model derivatives."""
            latent = agent.encode(observations)
            next_latent = agent.encode(following)
            previous = observations[..., agent.observation_dim:]
            intent = agent.prescriber(torch.cat((latent, previous), dim=-1))
            alpha, beta = agent.law_from_intent(latent, intent, previous)
            physical = agent.action_low + agent.action_scale * native
            goal_intent = agent.encode(goals) - latent
            diagnostics = {
                ...
                "goals/prescribed_norm": intent.norm(dim=-1).mean(),
                "goals/factual_norm": goal_intent.norm(dim=-1).mean(),
                "goals/local_norm": (next_latent - latent).norm(dim=-1).mean(),
                "policy/entropy": (Beta(alpha, beta, validate_args=False).entropy()
                                   + agent.log_action_scale).sum(-1).mean(),
                "policy/concentration": (alpha + beta).mean(),
            }
        ...
        def loss_model(indices, rollout_batch):
            return indexed_training_loss(agent, indices, rollout_batch, args)

        gradient_clipper = clip_gradient_groups
        ...
        if args.compile:
            ...
            gradient_clipper = torch.compile(
                gradient_clipper, fullgraph=True, dynamic=False,
                options={"triton.cudagraphs": False},
            )
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_values, b_logprobs, preupdate_metrics = rollout_statistics(
                    b_obs, b_native, b_next_obs, b_goals, b_rewards, b_terminations,
                )
                ...
                preupdate_metrics["losses/prefit_weighted_action_nll"] = -(b_weights * b_logprobs).mean()
            ...
            updates = 0
            metric_sums.zero_()
            rollout_batch = (b_obs, b_native, b_weights, b_returns, b_values,
                             b_next_obs, b_goals, b_rewards, b_terminations)
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss, metrics = loss_model(indices, rollout_batch)
                        # No optimizer may mutate aliased frozen weights before this.
                        loss.backward()
                        gradients = tuple(tuple(parameter.grad for parameter in parameters
                                                if parameter.grad is not None)
                                          for parameters in parameter_groups)
                        gradient_norms[updates].copy_(gradient_clipper(gradients, args.max_grad_norm))
                        for optimizer in optimizers:
                            optimizer.step()
                        # Consume CUDA-graph outputs before the next mark_step.
                        metric_sums.add_(metrics.detach())
                        updates += 1
                        del loss, metrics, gradients

            with timer.span("host_refresh"):
                host_actor.refresh()
            metric_means = metric_sums / updates
            metric_values = dict(zip(METRIC_NAMES, metric_means.unbind()))
            ...
            if any(not np.isfinite(value) for name, value in logged.items()
                ...
                raise FloatingPointError("nonfinite weighted-action learner metrics")
