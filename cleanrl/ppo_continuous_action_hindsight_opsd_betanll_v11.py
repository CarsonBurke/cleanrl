...
#
# PPO + Hindsight-Conditioned OPSD Teacher v11: signed hindsight steering.
#
# Lineage / motivation:
# The pure-OPSD-actor line (v10/v11-old/v12) removed PPO and tried to drive the
# Beta policy by teacher distillation alone; all three collapsed (~0 to -200 on
# HalfCheetah) while v9 -- PPO actor + a learned hindsight teacher used only as a
# small auxiliary distillation loss -- reaches ~3600. The lesson: in MuJoCo the
# teacher has no pretrained competence, so it cannot be the sole improvement
# operator. PPO must remain the grounded improver; OPSD is dense shaping on top.
#
# v11 therefore returns to the v9 backbone (PPO actor, Dreamer3-bucket HL-Gauss
# critic, learned hindsight teacher over future obs-deltas / discounted reward
# sums / GAE) and only improves the *teacher target*. v9's y* used relu(adv): a
# bad action simply regressed y* to the behavior mean, teaching nothing about
# what should have happened (pure posterior-reconstruction failure mode). v11
# makes the steering SIGNED: positive advantage moves y* toward the sampled
# action (it should have happened); negative advantage reflects y* away from it
# through the behavior mean (the opposite is the should-have action); near-zero
# advantage leaves y* at the mean. To exploit this, the teacher-NLL weighting and
# the student distillation weighting both become symmetric in |advantage|, so
# confidently-bad transitions inform the policy as much as confidently-good ones,
# instead of v9's positive-only weighting. Everything else matches v9.
import os
import random
import time
