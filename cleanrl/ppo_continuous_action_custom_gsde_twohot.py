...
class Agent(nn.Module):
    def __init__(self, envs, sde_dim=64, rpo_alpha=0.05, num_bins=255, bin_range=8.0):
        ...
        # Shared actor trunk (mean + covariance share feature extraction)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        # Shared trunk → mean head + cov adapter
        features = self.actor_trunk(x)                     # [B, 64]
        action_mean = self.actor_mean(features)            # [B, ad]

        # Covariance: 2*softsign bounds shared features to [-2,2] → Cholesky factor
        cov_features = 2.0 * features / (1.0 + features.abs())
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            features_log = agent.actor_trunk(b_obs[mb_inds])
            cov_feat_log = 2.0 * features_log / (1.0 + features_log.abs())
