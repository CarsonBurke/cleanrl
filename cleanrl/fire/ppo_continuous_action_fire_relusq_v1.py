# PPO + FIRE + ReLU² hidden activation
#
# Same FIRE schedule (Muon-style sqrt(d_out/d_in) scaled Newton-Schulz at
# fixed step checkpoints) as fire_v1, but hidden activations are ReLU²
# (Primer-style: f(x) = relu(x)²) instead of Tanh on both critic and actor
# trunks. Action head still uses Normal(mean, std) with ClipAction wrapper.
#
# Hypothesis: ReLU² gives more aggressive nonlinear gating than Tanh and
# loses effective rank faster (dead units), so periodic FIRE bursts have
# more headroom to recover plasticity than under saturating Tanh.
import os
import random
import time
