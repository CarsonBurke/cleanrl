# PPO + FIRE on actor only
#
# Same schedule [2M, 4M, 6M] and NS mechanics as fire_v1, but FIRE is
# applied ONLY to the actor_mean submodule. Critic weights are left
# untouched. Pairs with fire_critic_only_v1 to localize FIRE's win:
# value-function plasticity vs. policy plasticity.
import os
import random
import time
...
@torch.no_grad()
def fire_reinit_actor_only(agent: nn.Module, num_iters: int = 10):
    """FIRE applied only to agent.actor_mean (policy head + trunk). Critic
    weights are deliberately left untouched."""
    for m in agent.actor_mean.modules():
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        while fire_pending and global_step >= fire_pending[0]:
            ...
            fire_reinit_actor_only(agent, num_iters=args.fire_ns_iters)
            # Reset Adam state ONLY for actor_mean params (also includes
            # actor_logstd? No — only actor_mean, since logstd wasn't FIREd)
