# Parallel hierarchical MTP v13: direct fine/coarse projected-space predictions.
# Off is an UNCOUPLED TWO-LEVEL CAPACITY CONTROL; off/on isolates coupling.
# Each level batches its selected horizons from the current latent and ordered
# action prefixes, without temporal recurrence. Extra prefix encoders add capacity.
# Both clocks have observed-future anchors; agreement is NOT prediction accuracy.
# Consistency: Cproj(fine[h]) -> stop(coarse[h]), independently of the
...
class Args:
    ...
    """observed fine-level anchors; strictly increasing and always including H1"""
    coarse_stride: int = 4
    """coarse horizon spacing constraint; direct forecasts consume full ordered prefixes"""
    ...
    """off retains identical two-level direct predictor capacity, anchors and regularizers"""
...
class LeWMBranch(nn.Module):
    """Two observed-anchored direct multi-target levels with optional coupling.

    Selected horizons are independent within each level. Cross-level agreement
    stops the coarse teacher, while both levels retain factual future anchors.
    """
    ...
    def __init__(self, action_dim, num_proj, proj_chunk, prediction_loss, prediction_target_gradient,
        ...
        with torch.random.fork_rng(devices=[]):
            ...
            self.coarse_action_encoder = ActionEncoder(self.coarse_horizons[0] * action_dim, 64)
        ...
        with torch.random.fork_rng(devices=[]):
            self.fine_prefix_encoders = nn.ModuleList(
                ActionEncoder(action_dim * horizon, 64) for horizon in self.target_horizons[1:]
            )
            self.coarse_prefix_encoders = nn.ModuleList(
                ActionEncoder(action_dim * horizon, 64) for horizon in self.coarse_horizons[1:]
            )

    def rollouts(self, fine_current, coarse_current, native_actions, valid=None):
        """Direct forecasts: one batched deep predictor call per latent level."""
        if valid is None:
            valid = torch.ones((native_actions.shape[0], len(self.target_horizons)),
                               dtype=torch.bool, device=native_actions.device)
        level_predictions = []
        for current, horizons, first_encoder, other_encoders, predictor, projection in (
            (fine_current, self.target_horizons, self.action_encoder,
             self.fine_prefix_encoders, self.predictor, self.pred_proj),
            (coarse_current, self.coarse_horizons, self.coarse_action_encoder,
             self.coarse_prefix_encoders, self.coarse_predictor, self.coarse_pred_proj),
        ):
            conditions = []
            for index, horizon in enumerate(horizons):
                target_index = self.target_horizons.index(horizon)
                prefix = torch.where(valid[:, target_index, None, None],
                                     native_actions[:, :horizon].detach(), 0.5)
                encoder = first_encoder if index == 0 else other_encoders[index - 1]
                conditions.append(encoder((2.0 * prefix - 1.0).flatten(1)))
            conditions = torch.stack(conditions, dim=1)
            states = current[:, None].expand_as(conditions)
            predictions = projection(predictor(
                states.reshape(-1, 64), conditions.reshape(-1, 64),
            )).reshape_as(conditions)
            level_predictions.append({horizon: predictions[:, index]
                                      for index, horizon in enumerate(horizons)})
        return tuple(level_predictions)
    ...
    def forward(self, current, following, native_actions, valid):
        ...
        fine_predictions, coarse_predictions = self.rollouts(
            embeddings[:, 0], coarse_embeddings[:, 0], native_actions, valid,
        )
        metrics = {}
...
def main():
    ...
    try:
        ...
        writer.add_text("ablation", f"{args.task_activation=}, {args.task_residual=}, "
                        ...
                        "off/on compares identical direct two-level capacity, anchors and two SIGReg calls; "
                        "flat parallel v12 comparison confounds capacity; no temporal recurrence; "
