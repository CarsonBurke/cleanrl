# PPO + LEARNED ADVANTAGE WEIGHT as a STOCHASTIC BETA meta-policy (v5).
#
# GOAL. Replace the STATIC rankgauss advantage transform with a LEARNED per-sample
# weight w_i that multiplies the (standardized) advantage in the PPO surrogate:
# effective adv = w_i * A_i. We do NOT distill rankgauss; the weighting is learned
# on-policy from how much it IMPROVES the policy. The weight is FREE to compress OR
# amplify any sample -- nothing in the objective prefers one direction.
#
# WHY A BETA. The weight is a STOCHASTIC meta-policy with the SAME template as the
# actor -- a Beta whose own sampling IS the noise -- trained by REINFORCE, so it learns
# "weights like this paid off" rather than overfitting a noisy point estimate.
#
# v4 -> v5: THE TEACHER CHANGES. v4 rewarded each weight by the FIRST-ORDER predicted
# held-out improvement r_i = w_i*A_i*s_i  (s_i = grad logpi_i . h_hat). That signal is
# (a) blind to the curvature/trust-region effect that makes rankgauss good, (b) faintly
# biased toward AMPLIFYING large |A| (its systematic part scales with A_i^2), and (c) so
# noisy that corr(w,|A|) never left 0 over 2.7M steps -- the meta-policy stayed inert.
#
# v5 trains the weight on the REALIZED held-out RETURNS DIFFERENCE from ACTUALLY applying
# the weighting. Per minibatch, split T (train) / V (held-out):
#   g_w = grad_theta sum_{i in T} w_i * L_i^PPO     (weighted train-loss gradient)
#   g_0 = grad_theta sum_{i in T}       L_i^PPO     (uniform baseline gradient)
#   reward = J_V(theta - a*g_w) - J_V(theta - a*g_0)            (a = lw_probe_lr)
# i.e. take the weighted vs the uniform train step, and measure which one improves the
# held-out PPO surrogate J_V MORE. This is the realized (2nd-order-accurate) effect of the
# weighting, not a 1st-order prediction: amplifying a noisy outlier overshoots and LOWERS
# J_V, so compression can win -- the curvature term IS the trust-region signal rankgauss
# exploits. The reward is SIGN-NEUTRAL: compress or amplify, whatever raises J_V wins.
#
# REINFORCE on the weight policy:
#   L_w = - ( mean_{i in T} log pi_w(b_i) ) * reward
# No separate baseline is needed: J_V(uniform) does NOT depend on the sampled weights, so
# the paired subtraction IS an exact REINFORCE control variate -- reward is already the
# centered advantage of this weighting over uniform. The realized reward is a SCALAR per
# minibatch (the curvature term is an inherently cross-sample interaction, so genuine
# per-sample realized credit would cost O(M) probes). The paired weighted-vs-uniform
# structure (shared theta, T, V, sampled weights) cancels common-mode variance; the
# CONDITIONED Beta (inputs [actor_feat, A, value, z]) carries the per-sample structure,
# learned statistically across minibatches.
#
# THE WEIGHT META-POLICY.
#   alpha,beta = 1 + softplus(head([actor_feat, A, value, z]))   (unimodal Beta)
#   b_i ~ Beta(alpha_i, beta_i) in (0,1);  w_i = b_i * N / sum(b)   (LINEAR mean-1, detached)
# mean-1 normalization also keeps |g_w| ~ |g_0|, so the probe compares DIRECTION/
# concentration of the step, not its size. The actor takes its normal PPO step with the
# sampled w detached (mean-1 => only RE-WEIGHTS, no learning-rate confound).
#
...
class Args:
    ...
    # ---- Learned advantage weight: stochastic Beta meta-policy + REINFORCE ----
    # v5: reward = REALIZED held-out returns difference (weighted vs uniform train step).
    learnweight: bool = True
    lw_holdout: float = 0.5      # fraction of each minibatch held out (V) for the J_V probe
    lw_probe_lr: float = 3e-3    # probe step size a in reward = J_V(theta-a*g_w) - J_V(theta-a*g_0)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # mean-normalized weights ≈ 1 at init (starts as standard PPO).
        if args.learnweight:
            # Weight meta-policy: a Beta over the per-sample weight, SAME template as
            # the actor (alpha,beta = 1 + softplus => unimodal). Small last-layer init
            # => alpha~beta at start => symmetric Beta(~1.7,~1.7) => mean-1 weights with
            # built-in spread (the exploration noise). Input: [actor_feat, advantage].
            # Inputs: [actor_feat (H), advantage (1), value (1), sampled action z (act_dim)].
            self.weight_body = nn.Sequential(nn.Linear(H + 2 + act_dim, args.lw_hidden), nn.Tanh())
            self.weight_alpha_head = layer_init(nn.Linear(args.lw_hidden, 1), std=0.01, bias_const=0.0)
            self.weight_beta_head = layer_init(nn.Linear(args.lw_hidden, 1), std=0.01, bias_const=0.0)
    ...
    def weight_dist(self, actor_feat, adv, value, action):
        """Beta meta-policy over the per-sample weight, from the (all-detached) inputs
        [actor_feat, advantage, value, sampled_action z]. The sampled action is the
        per-sample input that the held-out signal s_i = grad logpi(z|s).h depends on.
        Unimodal (alpha,beta >= 1), same template as the actor."""
        h = torch.cat([actor_feat, adv.unsqueeze(-1), value.unsqueeze(-1), action], dim=-1)
        body = self.weight_body(h)
        alpha = 1.0 + F.softplus(self.weight_alpha_head(body)).squeeze(-1)
        beta = 1.0 + F.softplus(self.weight_beta_head(body)).squeeze(-1)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.learnweight:
                    # ===== Beta weight meta-policy + REINFORCE on the REALIZED =====
                    # ===== held-out RETURNS DIFFERENCE (weighted vs uniform step) =====
                    M = len(mb_inds)
                    nV = min(M - 1, max(1, int(round(args.lw_holdout * M))))
                    T_pos = slice(0, M - nV)         # train head (gets the weighting + step)
                    V_pos = slice(M - nV, M)         # held-out tail (scores the step)

                    # Forward #1 (signal graph): logpi, value logits, actor feature.
                    ...
                    # Beta weight meta-policy over the full minibatch. Inputs all detached
                    # (no grad to actor/critic); value = critic scalar at this state.
                    value_s = hl_support.to_scalar(value_logits1).detach()
                    dist_w = agent.weight_dist(
                        actor_feat1.detach(), adv1.detach(), value_s, b_latent_zs[mb_inds]
                    )
                    b_w = dist_w.sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)     # (M,), detached
                    # Applied weighting for the REAL update + diagnostics: LINEAR mean-1 over
                    # the full minibatch (bounded; cannot over-concentrate like softmax).
                    w_full = (b_w * (M / (b_w.sum() + 1e-8))).detach()           # mean 1 over M
                    # Probe weighting: mean-1 over T ONLY, so the reward is a pure function of
                    # b_w[T] (exact control variate + exact REINFORCE credit on logp_w_T), and
                    # sum(w_T) = |T| = sum(uniform) so |g_w| ~ |g_0| -- the probe compares step
                    # DIRECTION/concentration, not step size.
                    nT = M - nV
                    w_T = (b_w[T_pos] * (nT / (b_w[T_pos].sum() + 1e-8))).detach()  # mean 1 over T

                    # Per-sample PPO TRAIN loss on T (to be minimized); grads -> actor params.
                    pg1_T = adv1[T_pos] * ratio1[T_pos]
                    pg2_T = adv1[T_pos] * torch.clamp(ratio1[T_pos], 1 - args.clip_coef, 1 + clip_hi)
                    loss_T = -torch.min(pg1_T, pg2_T)                            # (|T|,)
                    # Weighted vs uniform train-loss gradients wrt actor params. Retain the
                    # weighted graph so the uniform grad reuses it; both consumed here so the
                    # in-place probe below cannot corrupt a live graph.
                    L_w = (w_T * loss_T).mean()
                    L_0 = loss_T.mean()
                    g_w = torch.autograd.grad(L_w, actor_params, retain_graph=True, allow_unused=True)
                    g_0 = torch.autograd.grad(L_0, actor_params, allow_unused=True)
                    g_w = [torch.zeros_like(p) if g is None else g for p, g in zip(actor_params, g_w)]
                    g_0 = [torch.zeros_like(p) if g is None else g for p, g in zip(actor_params, g_0)]

                    # REALIZED held-out returns difference. Take each candidate SGD step
                    # theta -= a*g, score the held-out PPO surrogate J_V; the step that
                    # improves V more wins. Fully in-place + restored under no_grad. The
                    # difference cancels J_V(theta), so this is purely the weighting's effect
                    # (2nd-order accurate: overshooting a noisy outlier LOWERS J_V).
                    a = args.lw_probe_lr
                    adv_V = adv1[V_pos].detach()
                    obs_V = b_obs[mb_inds][V_pos]
                    z_V = b_latent_zs[mb_inds][V_pos]
                    oldlp_V = b_logprobs[mb_inds][V_pos]

                    def _surr_V():
                        lp = agent.lw_logprob(obs_V, z_V)
                        r = (lp - oldlp_V).exp()
                        return torch.min(
                            adv_V * r, adv_V * torch.clamp(r, 1 - args.clip_coef, 1 + clip_hi)
                        ).mean()
                    ...
                    with torch.no_grad():
                        ...
                        for p, g in zip(actor_params, g_w):
                            p.add_(g, alpha=a)
                        for p, g in zip(actor_params, g_0):
                            p.add_(g, alpha=-a)
                        jv_0 = _surr_V()
                        for p, g in zip(actor_params, g_0):
                            p.add_(g, alpha=a)
                    # reward = J_V(weighted) - J_V(uniform). The uniform term does NOT depend
                    # on the sampled weights, so it is an EXACT REINFORCE baseline (control
                    # variate) -- no EMA needed; reward is already the centered advantage of
                    # this weighting over uniform. Sign-neutral: compress OR amplify, whatever
                    # raises the realized held-out return wins. One scalar shared over the
                    # sampled weights (the curvature term is cross-sample, so per-sample
                    # realized credit is unavailable without O(M) probes).
                    reward = (jv_w - jv_0).item()      # >0 => this weighting beat uniform on V

                    logp_w_T = dist_w.log_prob(b_w)[T_pos]
                    ent_w = dist_w.entropy()                                      # diagnostic only
                    lw_loss = -(logp_w_T.mean()) * reward
                    ...
                    with torch.no_grad():
                        ...
                        # DECISIVE diagnostic: corr(applied w, |A|). NEGATIVE => compresses
                        # outliers (rankgauss-like). POSITIVE => amplifies them. v4 was pinned
                        # at 0; the realized-returns signal should now move it OFF zero.
        ...
        if args.learnweight:
            ...
            writer.add_scalar("lw/beta_entropy", lw_entropy, global_step)
