# PPO + IterThink v91 (v24 Beta + LeWM outcome heads). From v90.
#
...
if __name__ == "__main__":
    ...
    wm_outcome_io_params = (
        list(agent.world_model.reward_outcome_proj.parameters())
        + list(agent.world_model.continuation_outcome_proj.parameters())
        + list(agent.world_model.value_outcome_proj.parameters())
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    with torch.no_grad():
                        ...
                        imagine_latent_mse = F.mse_loss(
                            imagine_next_summary[:, : agent.world_model.num_obs_tokens],
                            target_summaries[:, 0, : agent.world_model.num_obs_tokens],
                            reduction="none",
                        ).mean(dim=(-1, -2))
                    ...
                    # Outcome heads are the training signal for outcome tokens.
                    # Obs tokens are trained by latent MSE; outcome tokens are not.
                    ...
                    for mtp_idx in range(mtp_len):
                        ...
                        pred = pred_mtp[:, :valid_horizon, mtp_idx, : agent.world_model.num_obs_tokens]
                        target = target_summaries[:, mtp_idx:, : agent.world_model.num_obs_tokens]
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                if wm_imagine_probe_weight.item() > 0:
                    writer.add_scalar(
                        "lewm/imagine_step_obs_latent_mse",
