# PPO + dynamics sensitivity credit-GAE + SB3-style tanh Gaussian + asymmetric SPO + ReLU^2 v2.
#
...
# v2 uses the SB3 gSDE/SAC numerical convention for the tanh Jacobian:
# compute the squash correction from the pre-tanh Gaussian action when
# available, and only use clipped inverse tanh as a fallback.
...
class Args:
    ...
    """clip range for normalized dynamics-sensitivity credit-GAE residuals"""
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration >= args.dynamics_credit_start:
            ...
            if raw_credit_residual_rms.item() < 1e-6:
                ...
                credit_residuals = credit_residuals / (raw_credit_residual_rms + 1e-8)
