# PPO + IterThink v58 (v24 Beta + append-only skip-token loop ledger). From v24.
#
...
#     parameter shapes.
#   - The loop bank contains observation/work tokens only. Learned skip tokens are
#     written from each loop layer as summaries of that layer's current
#     information, appended to the ledger, and never used as recurrent current
#     state.
...
class ThinkTrunk(nn.Module):
    ...
    def _run_work_loop(self, initial, x0):
        ...
        skip_ledger = [self.skip_writer(skip_seed, initial)]
        for _ in range(self.K):
            context = torch.cat([current, torch.cat(skip_ledger, dim=1)], dim=1)
            current = self.block(current, context, initial)
            # Skip tokens summarize the just-computed layer; they are appended as
            # future KV context but never become the next recurrent current state.
            skip_ledger.append(self.skip_writer(skip_seed, current))
        return skip_ledger
    ...
    def _read_trace(self, ledger, x0, role):
        ...
        traces = []
        skip_ledger = ledger
        for depth in range(len(skip_ledger)):
            context = torch.cat(skip_ledger[: depth + 1], dim=1)
            read = initial_read
    ...
    def forward(self, x):
        ...
        with ctx:
            ...
            ledger = self._run_work_loop(initial, x0)
