# TD7-CTX v3_5 — v3_4 + TOP-ERL-STYLE DENSE PER-PATCH n-STEP VALUE READOUT (random horizons).
# --------------------------------------------------------------------------------------------------
# WHY v3_5: in v3_4 the history body's ONLY learning signal is a SINGLE gradient — the critic's decode
# query at the last patch position, truncated to k_live=32. Every other token position the body composes
# is shaped only indirectly. TOP-ERL (ICLR 2025) shows that training a CAUSAL value-transformer with a
# per-position N-step-return readout at EVERY token — with RANDOM horizon lengths — is the key to its
# performance (their ablation: fixing the horizon instead of randomizing it dropped success 80% -> 35%).
# We adapt that as a DENSE value-learning signal for our body: a plain-init `patch_value_head` reads the
# body output at EVERY patch position inside sampled training windows and regresses a random-horizon
# n-step return. This is NOT self-supervised prediction — the target is a return/TD bootstrap (the
# critic's OWN objective: discounted patch reward-sums + a detached TD7 target-critic minQ), so it is the
# same value gradient the body already serves, just delivered densely at every history position instead
# of once at the decode query. HYPOTHESIS: a dense return-gradient at every position teaches the body
# "what did well previously in previous states" far more sample-efficiently than the single truncated
# decode-query gradient, without changing WHAT the body optimizes (downstream value utility).
#
# WHAT v3_5 ADDS (everything else BYTE-IDENTICAL to v3_4; the head is TRAIN-TIME ONLY — not in the
# checkpoint/eval path, not in the KV caches):
#  (1) patch_value_head = nn.Linear(d_model, 1), PLAIN init (it does not feed actor/critic, so the exact
#      plain-TD7 floor at init is unaffected). Its params live in the CRITIC optimizer (like the body).
#  (2) patch_value_update(): at critic cadence (gated by pv_every) sample pv_windows contiguous windows of
#      pv_window_len patch positions from the WRITTEN token history, recompute the body IN-GRAPH over each
#      window via body.forward_seq (self-contained full-causal within the window — the EXACT left-context
#      scheme v3's lm_update used: NO frozen-cache left context), and regress head(h_p) -> z(G_p).
#  (3) TARGET G_p (random horizon n in {1..pv_max_horizon}, ONE n per window per update, TOP-ERL-style):
#        G_p = Σ_{i=0}^{n-1} γ^(50 i) R_{p+i} + γ^(50 n) V_boot(p+n),  γ = gamma (0.99), 50 = patch_len.
#      R_{p+i} = raw patch reward-sum (as stored in the ring). V_boot = DETACHED TD7 target-critic minQ at
#      the boundary state of patch p+n (last-step obs + in-patch hist reconstruction, reused verbatim from
#      v3_2's qdistill) evaluated with the CURRENT actor's action. Per position, n is TRUNCATED to what the
#      written ring can back (never index past min(n_patch,max_patches) or the raw ring); positions with 0
#      available horizon (or an unreconstructable boundary, len<hist_k+1) are DROPPED, not masked into loss.
#      Targets are z-scored by an EXPONENTIAL-FORGETTING running mean/var (decay ~0.999). Loss = pv_coef *
#      MSE(head(h_p), z(G_p)) over valid positions, ADDED TO critic_loss BEFORE the single existing backward,
#      so the existing body-only 0.5 grad-clip (unchanged) now also bounds gradients arriving from this loss.
#  (4) RNG DISCIPLINE: window starts + horizons are drawn from a DEDICATED np.random.Generator, so with
#      pv_coef=0 (pv_every still firing) the main torch/numpy RNG streams are untouched and the run matches
#      v3_4 to float noise. No dropout/rand anywhere in forward_seq or the boundary reconstruction.
# --------------------------------------------------------------------------------------------------
# TD7-CTX v3_4 — USER-SHAPED body (v3_3) + CHECKPOINT ERA-CONSISTENCY FIX.
# --------------------------------------------------------------------------------------------------
# WHY v3_4 (substrate-level lesson): live td7ctx_v3_3 had a HEALTHY online policy (last-20 ~12,211 @251k,
# matched-step competitive with hist/baselines) but its CHECKPOINT EVAL COLLAPSED: 12,155@150k ->
# 11,483@200k -> -291@250k, while ctx_gate climbed (1.88) and body_grad_norm stayed pinned at the 0.5 clip.
# ROOT CAUSE — CHECKPOINT/BODY ERA MISMATCH. TD7 deploys a *checkpoint* policy (frozen actor/encoder
# adapters snapshotted at the best worst-case assessment). In v3_1/v3_2 the body is aux-trained and drifts
# SLOWLY, so decoding checkpoint-era adapters through the current body was an accepted, benign deviation.
# In v3_3 the body is RL-trained and MOVES AT THE CLIP CAP EVERY STEP: by eval time the live body (and live
# KV cache) are eras away from the checkpoint adapters, which then read a context distribution they were
# never trained on -> deployed policy is garbage, even though the ONLINE actor (whose adapters co-adapt with
# the body every step) is fine. Self-reinforcing because the checkpoint only updates on improved worst-case
# online episodes (collection uses the ONLINE actor, so learning itself was unaffected; only the deployed/
# evaluated policy broke). LESSON: a fast-moving RL-trained body makes TD7 checkpoint DEPLOYMENT era-sensitive;
# the slow aux bodies of v3_1/v3_2 masked it.
# THE FIX (this file; everything else byte-identical to v3_3): snapshot a checkpoint_body ATOMICALLY with the
# checkpoint actor/encoder, and route EVERY checkpoint-path decode through checkpoint_body + a dedicated
# K_ckpt/V_ckpt cache rebuilt under checkpoint_body at eval start (prepare_checkpoint_cache; ~one reprefill,
# negligible at the 5k-step eval cadence). The ONLINE path, the critic/body RL path, and the zero-init floor
# are untouched. (Only evaluate() consumes the checkpoint policy here — collection acts with the online actor.)
# --------------------------------------------------------------------------------------------------
...
class Args:
    ...
    # --- v3_5 DENSE PER-PATCH n-STEP VALUE READOUT (TOP-ERL-style; train-time only) ---
    pv_coef: float = 0.1
    """weight of the dense per-patch n-step value loss added to the critic loss (0 -> exact v3_4 behaviour)"""
    pv_every: int = 1
    """run patch_value_update every k-th critic update (1 = every step; raise if the extra cost demands)"""
    pv_windows: int = 2
    """contiguous patch-position windows sampled per patch_value_update"""
    pv_window_len: int = 128
    """patch positions per sampled window (the body is recomputed in-graph over each window)"""
    pv_max_horizon: int = 8
    """max n-step horizon (in patches); one random n in {1..pv_max_horizon} is drawn per window per update"""
...
class EMANorm:
    """Exponential-forgetting running mean/var of a scalar target stream (v3_5 dense-value normalizer).

    Unlike ReturnNorm's non-forgetting Welford, this tracks a SLOWLY-DRIFTING target (the n-step value
    return keeps growing as the policy improves), so a decayed estimate is preferred: each update folds the
    batch mean/var in with weight (1-decay). `normalize` returns the clipped z-score (all detached: the
    target is a constant to the loss). Pre-approved in review to replace the non-forgetting Welford here."""
    ...
    def update(self, batch_mean, batch_var):
        if not self.inited:
            self.mean, self.var, self.inited = float(batch_mean), float(batch_var), True
        else:
            d = self.decay
            self.mean = d * self.mean + (1.0 - d) * float(batch_mean)
            self.var = d * self.var + (1.0 - d) * float(batch_var)
...
class TD7Agent:
    def __init__(self, state_dim, action_dim, max_action, args: Args, device, writer: SummaryWriter, step_dim):
        ...
        # v3_4 ERA-CONSISTENCY FIX: the checkpoint policy must decode through the BODY of its OWN era, not the
        # live (fast-moving, RL-trained) body. checkpoint_body is snapshotted ATOMICALLY with checkpoint_actor/
        # _encoder whenever the checkpoint is updated, and the checkpoint-path decode reads a dedicated K_ckpt/
        # V_ckpt cache reprefilled under checkpoint_body at eval start. See the header failure-mode note.
        self.checkpoint_body = copy.deepcopy(self.body)
        # v3_5 DENSE VALUE HEAD: a plain-init readout of the body output at every patch position inside the
        # sampled training windows (patch_value_update). It does NOT feed actor/critic (so the plain-TD7 floor
        # is untouched) and is NOT part of checkpointing/eval or the KV caches — it is a train-time-only signal.
        # Construct it under a SAVED/RESTORED global torch RNG state so this extra module does NOT advance the
        # main stream: with pv_coef=0 the run is then BIT-IDENTICAL to v3_4 from the same seed (not merely a
        # re-seeded variant). The head's own weights come from a dedicated generator so they are still random.
        _rng_state = torch.get_rng_state()
        _cuda_rng_state = torch.cuda.get_rng_state_all() if torch.cuda.is_available() else None
        self.patch_value_head = nn.Linear(args.tok_dim, 1).to(device)
        with torch.no_grad():  # re-init deterministically off the main stream (plain Linear init, dedicated gen)
            _g = torch.Generator().manual_seed(args.seed + 12345)
            self.patch_value_head.weight.copy_(
                (torch.rand(self.patch_value_head.weight.shape, generator=_g) * 2 - 1) * (args.tok_dim ** -0.5))
            self.patch_value_head.bias.copy_(
                (torch.rand(self.patch_value_head.bias.shape, generator=_g) * 2 - 1) * (args.tok_dim ** -0.5))
        torch.set_rng_state(_rng_state)  # restore the main stream so downstream construction is unperturbed
        if _cuda_rng_state is not None:
            torch.cuda.set_rng_state_all(_cuda_rng_state)
        # CRITIC optimizer OWNS the body params (v3_3) AND the v3_5 patch_value_head. actor_optimizer excludes
        # them, so actor steps never move the body; any actor-deposited body grad is wiped by the next critic
        # zero_grad (grad hygiene). The head is per-parameter in Adam, so with pv_coef=0 (its grad is exactly
        # 0) it neither moves nor perturbs the critic/body updates -> exact v3_4 equivalence.
        self.critic_optimizer = torch.optim.Adam(
            list(self.critic.parameters()) + list(self.body.parameters())
            + list(self.patch_value_head.parameters()),
            lr=args.critic_lr,
        )
        # v3_5 dense-value bookkeeping: per-patch discount (one patch == patch_len env steps), a forgetting
        # target normalizer, a DEDICATED RNG (so window/horizon draws never touch the main torch/numpy
        # streams -> pv_coef=0 stays bit-identical to v3_4), and the ring column layout for boundary recon.
        self.gamma_patch = args.gamma ** args.patch_len
        self.pv_norm = EMANorm(decay=0.999)
        self.pv_rng = np.random.default_rng(args.seed)
        self.action_dim = action_dim
        self.raw_state_dim = step_dim - action_dim - 1  # ring row = [raw_obs, normalized_action, reward]
        assert self.raw_state_dim + args.hist_k * action_dim == state_dim, (
            "state_dim must equal raw_state_dim + hist_k*action_dim for in-patch boundary hist reconstruction")
        # the ring stores normalized_action = raw_action / max_action; V_boot feeds ring actions to the critic
        # via the reconstructed boundary state, so a non-unit action scale must fail loudly (matches v3_2).
        assert float(max_action) == 1.0, "v3_5 boundary V_boot assumes max_action==1 (ring action == raw action)"
        self.last_pv_loss = 0.0
        self.last_pv_target_std = 0.0
        ...
        # v3_4 CHECKPOINT cache: KV produced by checkpoint_body over the resident tokens, rebuilt at eval
        # start (prepare_checkpoint_cache) so the checkpoint actor decodes an era-consistent context.
        self.K_ckpt = torch.zeros(n_layers, MAX_PATCHES, args.tok_dim, device=device)
        self.V_ckpt = torch.zeros(n_layers, MAX_PATCHES, args.tok_dim, device=device)
        self.ckpt_n_written = 0
    ...
    def select_action(self, state, use_checkpoint=False, use_exploration=True):
        with torch.no_grad():
            ...
            if use_checkpoint:
                # v3_4: decode the checkpoint policy through its OWN-era body + cache (prepare_checkpoint_cache
                # rebuilds K_ckpt/V_ckpt under checkpoint_body at eval start). NEVER the live body/cache.
                zs = self.checkpoint_encoder.zs(state)
                action = self.checkpoint_actor(
                    state, zs, self.checkpoint_body, self.K_ckpt, self.V_ckpt, patch_idx, self.ckpt_n_written
                )
            else:
    ...
    @torch.no_grad()
    def prepare_checkpoint_cache(self):
        """v3_4 ERA-CONSISTENCY: (re)build the checkpoint KV cache under checkpoint_body over the CURRENT
        resident tokens, so the checkpoint policy's decode reads a context produced by the body of its own
        era (not the drifted live body). Called at eval start (evaluate()); ~one full_reprefill (~30ms),
        negligible at the 5k-step eval cadence. No new patches are added during eval, so a single rebuild
        to the current n_patch covers the whole evaluation. Passing K_ckpt as both the live and frozen
        target of full_reprefill is intentional: the final live->frozen copy is a harmless self-copy."""
        self.checkpoint_body.full_reprefill(
            self.store.tokens, self.store.n_patch, self.K_ckpt, self.V_ckpt, self.K_ckpt, self.V_ckpt,
            self.store.raw_patches,
        )
        self.ckpt_n_written = min(self.store.n_patch, MAX_PATCHES)
    ...
    def patch_value_update(self):
        """v3_5 DENSE PER-PATCH n-STEP VALUE readout (TOP-ERL-style). Sample pv_windows contiguous windows
        of pv_window_len patch positions from the WRITTEN history, recompute the body IN-GRAPH over each
        window (self-contained full-causal, exactly v3's forward_seq left-context = none), and regress the
        plain patch_value_head at every position onto a random-horizon n-step return G_p. Returns the raw
        (un-weighted) pv MSE loss tensor (in-graph, so critic backward moves body + head) or None when there
        is not enough written/resident history yet. Draws ONLY from self.pv_rng; no main-RNG consumption."""
        T = self.args.pv_window_len
        nw_tok = min(self.store.n_patch, self.store.max_patches)  # written token positions (cache-backed)
        if self.store.n_patch < T + 1 or nw_tok < T + 1:
            return None
        # windows must (a) fit fully inside the written token cache [0, nw_tok) and (b) leave >=1 patch of
        # bootstrap headroom; sampled starts also stay resident in the raw ring (lo_s), so every reward/
        # boundary lookup at positions [s, s+T-1+horizon] is backed by raw storage.
        lo_s = max(0, self.store.n_patch - self.store.raw_patches)
        hi_s = nw_tok - T - 1  # last position s+T-1 <= nw_tok-2 -> at least its 1-step bootstrap exists
        if hi_s < lo_s:
            return None
        nwin = self.args.pv_windows
        Hmax = self.args.pv_max_horizon
        rp = self.store.raw_patches
        gp = self.gamma_patch

        starts = self.pv_rng.integers(lo_s, hi_s + 1, size=nwin)           # (nwin,)
        horizons = self.pv_rng.integers(1, Hmax + 1, size=nwin)            # (nwin,) one random n per window

        # (nwin, T) absolute patch index per window position; forward_seq recomputes the body over these.
        pos_np = starts[:, None] + np.arange(T)[None, :]                   # (nwin, T)
        pos_t = torch.as_tensor(pos_np, device=self.device, dtype=torch.long)
        tok_seq = self.store.tokens[pos_t].detach()                       # (nwin, T, tok_dim) in-graph from here
        h = self.body.forward_seq(tok_seq, pos_t)                         # (nwin, T, tok_dim); grads -> body
        pred = self.patch_value_head(h).squeeze(-1)                       # (nwin, T)

        p = pos_np.reshape(-1)                                            # (P,) flat abs patch idx, P = nwin*T
        n_draw = np.repeat(horizons, T)                                   # (P,) per-position drawn horizon
        avail = (nw_tok - 1) - p                                          # max bootstrap horizon that is written
        n_eff = np.minimum(n_draw, avail)                                 # truncate n to available (>=... )
        valid = n_eff >= 1                                               # drop positions with 0 available horizon

        # --- reward part: Σ_{i=0}^{n-1} gamma_patch^i * R_{p+i} (raw ring reward-sums) ---
        G_reward = np.zeros(p.shape[0], dtype=np.float64)                 # (P,)
        disc = 1.0
        for i in range(Hmax):
            active = i < n_eff                                            # positions still accumulating patch i
            if not active.any():
                break
            R_i = self.store.raw_rsum[(p + i) % rp]                       # (P,) resident by construction
            G_reward += np.where(active, disc * R_i, 0.0)
            disc *= gp

        # --- bootstrap patch index b = p + n_eff (a WRITTEN, resident patch by avail construction) ---
        b = p + n_eff                                                     # (P,)
        slot = b % rp                                                     # (P,)
        L = self.store.raw_lens[slot]                                     # (P,) valid steps in the boundary patch
        hk = self.args.hist_k
        sd, ad = self.raw_state_dim, self.action_dim
        recon_ok = L >= (hk + 1)                                          # in-patch hist needs rows L-2..L-(hk+1)
        valid = valid & recon_ok                                         # drop unreconstructable boundaries
        if not valid.any():
            return None

        # reconstruct the boundary state (last-step obs + in-patch hist) for the VALID bootstrap patches only,
        # reusing v3_2's qdistill scheme verbatim, then evaluate a DETACHED TD7 target-critic minQ with the
        # CURRENT actor's action (TOP-ERL uses the current policy for the bootstrap value).
        vslot = slot[valid]
        vb = b[valid]
        vL = L[valid]
        obs_row = np.clip(vL - 1, 0, self.patch_len - 1)                 # last valid step (boundary proxy obs)
        obs = self.store.raw_blocks[vslot, obs_row, :sd]                 # (Pv, sd)
        offsets = np.arange(2, 2 + hk)                                   # rows L-2 (most recent) .. L-(hk+1)
        hist_rows = np.clip(vL[:, None] - offsets[None, :], 0, self.patch_len - 1)  # (Pv, hk)
        hist_acts = self.store.raw_blocks[vslot[:, None], hist_rows, sd:sd + ad]    # (Pv, hk, ad) most-recent-first
        s_b = np.concatenate([obs, hist_acts.reshape(len(vslot), -1)], axis=-1).astype(np.float32)  # (Pv, state_dim)
        s_b_t = torch.as_tensor(s_b, device=self.device)
        boot_pos = torch.as_tensor(vb, device=self.device, dtype=torch.long)        # decode query = boundary patch idx
        fnw = self.frozen_n_written
        with torch.no_grad():  # DETACHED bootstrap: deposits NO grad into body / actor / critic / encoders
            fixed_zs = self.fixed_encoder.zs(s_b_t)
            a_b = self.actor(s_b_t, fixed_zs, self.body, self.K_frozen, self.V_frozen, boot_pos, fnw)
            ftzs = self.fixed_encoder_target.zs(s_b_t)
            ftzsa = self.fixed_encoder_target.zsa(ftzs, a_b)
            Qb, _ = self.critic_target(
                s_b_t, a_b, ftzsa, ftzs, self.body_target, self.K_frozen, self.V_frozen, boot_pos, fnw
            )
            v_boot = Qb.min(dim=1).values.detach().cpu().numpy().astype(np.float64)  # (Pv,)

        # --- assemble G_p = reward part + gamma_patch^n * V_boot(p+n) over VALID positions ---
        G = G_reward[valid] + (gp ** n_eff[valid].astype(np.float64)) * v_boot       # (Pv,)
        # forgetting-normalizer z-score (target is a detached constant to the loss)
        self.pv_norm.update(G.mean(), G.var())
        z = np.clip((G - self.pv_norm.mean) / (self.pv_norm.std + 1e-6), -5.0, 5.0).astype(np.float32)
        z_t = torch.as_tensor(z, device=self.device)                                 # (Pv,)

        valid_t = torch.as_tensor(valid, device=self.device)                         # (P,) bool
        pred_valid = pred.reshape(-1)[valid_t]                                        # (Pv,) in-graph
        pv_loss = F.mse_loss(pred_valid, z_t)
        self.last_pv_loss = pv_loss.detach()
        self.last_pv_target_std = float(G.std())
        return pv_loss
    ...
    def train(self):
        ...
        # v3_5: dense per-patch n-step value loss, added to the critic loss BEFORE the single backward so
        # its body gradients pass through the SAME body-only 0.5 clip (nothing else changes). Gated by
        # pv_every; draws only from self.pv_rng so pv_coef=0 stays bit-identical to v3_4. patch_value_head
        # grads (unclipped, like the critic) also flow from here. None early on / when no valid positions.
        total_critic_loss = critic_loss
        if self.training_steps % self.args.pv_every == 0:
            pv_loss = self.patch_value_update()
            if pv_loss is not None:
                total_critic_loss = critic_loss + self.args.pv_coef * pv_loss
        ...
        if self.training_steps % 500 == 0:
            ...
            self.writer.add_scalar("debug/pv_loss", float(self.last_pv_loss), self.training_steps)
            self.writer.add_scalar("debug/pv_target_std", self.last_pv_target_std, self.training_steps)
    ...
    def maybe_train_and_checkpoint(self, ep_timesteps, ep_return):
        ...
        # update checkpoint
            ...
            # v3_4: snapshot the body ATOMICALLY with the checkpoint actor/encoder so the deployed policy
            # and the body it decodes through belong to the same era (the eval-start cache rebuild uses it).
            self.checkpoint_body.load_state_dict(self.body.state_dict())
...
def evaluate(agent: TD7Agent, eval_env, eval_eps, use_checkpoint, hist_k, action_dim, max_action):
    ...
    # v3_4: rebuild the checkpoint-era KV cache under checkpoint_body ONCE before the eval episodes, so the
    # checkpoint policy decodes an era-consistent context (no patches are added during eval).
    if use_checkpoint:
        agent.prepare_checkpoint_cache()
