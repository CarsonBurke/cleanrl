"""FPO standardized affine bridge v5: residual flow in normalized coordinates.

Fix v4's scale mismatch: define z=(x_t-(1-t)*mean)/s_t and u=u_affine+s_t*r(z).
Integrate z directly, then decode mean+std*z. The residual has unit-scale input
and its physical velocity contracts with the bridge, rather than dominating a
small affine std. Compute epsilon predictions analytically to avoid cancellation.
PPO and FPO objective unchanged. --no-residual-flow is the affine-only control.
"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        self.residual_flow = args.residual_flow
    ...
    def velocity_with_affine(self, observations, states, times, mean, std):
        ...
        scale = variance.sqrt()
        standardized = (states-(1.0-times)*mean)/scale
        return base + scale*self.residual_velocity(observations, standardized, times)
    ...
    def sample(self, observations, noise):
        ...
        mean, std = self.affine_parameters(observations)
        standardized = noise.detach().clone()
        for step in range(self.flow_steps):
            times = standardized.new_full((len(standardized), 1), 1.0-step/self.flow_steps)
            standardized = standardized-self.residual_velocity(observations, standardized, times)/self.flow_steps
        native = mean+std*standardized
...
def cfm_errors(agent, observations, native, times, noise, mse):
    ...
    scale = variance.sqrt()
    standardized = (states-(1.0-times)*mean)/scale
    correction = agent.residual_velocity(conditions, standardized, times)
    if mse == "epsilon":
        residual = times/scale*standardized+(1.0-times)*scale*correction-noise
    else:
        coefficient = (times-(1.0-times)*std.square())/variance
        velocity = coefficient*(states-(1.0-times)*mean)-mean+scale*correction
...
class HostSampler:
    """Euler in standardized coordinates with exact affine endpoint decoding."""
    def __init__(self, agent, num_rows, steps):
        ...
        self.residual_flow = agent.residual_flow
        ...
        self.native, self.action, self.work, self.mean, self.std = (
            np.empty(shape, np.float32) for _ in range(5))
