# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v10.
#
# WHAT CHANGES vs DSRG v8: the message NONLINEARITY only -- SiLU -> ReLU^2 (squared ReLU).
# Everything else (the v8 PURE ADDITIVE post-norm residual `norm(h + hhat)`, competitive
# routing, obs-conditional readout, load-balance, triple-backward) is byte-for-byte v8, so
# this is a clean A/B on the activation. SiLU was the inherited Dreamer/iterthink default,
# never a choice made for this graph. ReLU^2 = max(0,x)^2 is harder and sparser: it zeros
# every negative pre-activation EXACTLY (vs SiLU's soft -0.278 floor) and grows super-
# linearly above 0 -- a selectivity that matches the sparse top-m routing theme (few, crisp
# active messages rather than a soft everywhere-nonzero field). The super-linear magnitude
# is safe here precisely BECAUSE the residual is post-norm: norm(h + hhat) re-normalizes the
# larger update every tick, so it can't run away; signed negatives (from tick_t and the
# signed W_shared/U message) are what ReLU^2 gates to 0. (v9, a pre-norm variant, regressed
# at every checkpoint through 1.5M -- pre-norm decayed late-tick contributions; reverted.)
#
# --- v8 lineage note ---
# WHAT CHANGED vs DSRG v7: the per-tick CARRY became a PURE ADDITIVE RESIDUAL. v7's tick
# update was an LSTM-style gated carry `norm((1-f)*hhat + f*h)`; the sigmoid f can SATURATE
# (f->1 kills the update gradient, f->0 kills the carry gradient), the same magnitude-
# saturation pathology v6 removed from routing. v8 uses `norm(h + hhat)` -- the iterthink-
# validated additive residual (`x = x + delta`, no gate). The `+h` is an unconditional
# identity highway, so gradient flows straight back across all T ticks to the obs-injected
# perceptrons, and no gate can saturate to zero out the update or carry path. (v7 and v8
# tracked closely through 2M, v7 ~5% ahead late -- a small single-seed gap.)
#
# --- v7 lineage note ---
...
class RoutedGraphCell(nn.Module):
    ...
    def forward(self, h, tick_t, Ksrc):
        ...
        # v10: ReLU^2 (squared ReLU) instead of SiLU. Harder, sparser activation that
        # pairs with the sparse top-m routing; zeros all negative pre-activations exactly
        # (vs SiLU's soft -0.278 floor) and grows super-linearly above 0. Safe on the
        # POST-norm residual: the per-tick norm(h + hhat) below re-normalizes the (larger,
        # super-linear) update each tick, so the magnitude can't run away. tick_t (and the
        # signed W_shared/U message) supply the negatives that ReLU^2 then gates to 0.
        pre = m + tick_t
        hhat = F.relu(pre) * F.relu(pre)                                  # ReLU^2 (tick-aware); NO inner norm
