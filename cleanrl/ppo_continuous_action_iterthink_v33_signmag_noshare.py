# PPO + IterThink v33 (SIGN-CORRECT advantage + UNSHARED trunk + parity target). From v32.
#
# WHY v33. v32's equilibrium dual won big on HalfCheetah (~8800) but FAILED to learn the
# TERMINATING envs (Walker2d ~1550, Hopper ~600); a no-entropy control failed identically, so
# the base pipeline -- not the entropy mechanism -- could not learn them. Four deep math audits
# (critic, GAE/returns/bootstrap, policy ratio/numerics, advantage transform) localised the cause
# to ONE place: the ADVANTAGE TRANSFORM, with the others proven correct (critic mean-consistent
# with scalar GAE to ~1e-5; ratio's tanh Jacobian cancels exactly; no dones off-by-one / autoreset
# corruption). The bug:
#   * SIGN ANCHORED AT THE MEDIAN. rankgauss maps advantages to Gaussian quantiles by RANK; its
#     zero-crossing is the batch MEDIAN, not 0. On HalfCheetah the per-step advantage distribution
#     is ~symmetric (median ~= 0) so the PG sign is right and it wins. On Walker2d/Hopper episodes
#     TERMINATE (falling): the advantage distribution is strongly LEFT-SKEWED (many small-positive
#     survival steps, rare large-negative termination spikes) so median >> 0, and ~P(gae<0)-0.5 of
#     the batch (~10-20%) is reinforced with the WRONG SIGN every update. clip_z (the first probe)
#     centers on the MEAN -- milder, same error class.
# v33's fixes (all confirmed by diagnostics on Hopper/Walker, same critic throughout):
#   (1) SIGN-CORRECT TRANSFORM: adv_transform = "rankgauss_signmag" -- global-rank MAGNITUDE
#       (outlier-immune, identical to rankgauss so the HC win is preserved) with the TRUE sign(gae)
#       reattached, so the zero-crossing is exactly gae=0. Strictly better than clip_z (which still
#       mean-centers). Diagnostics: clip_z lifted Hopper 580->785, Walker ->1334; signmag is the
#       sign-exact version of the same idea.
#   (2) UNSHARED TRUNK: share_backbone = False. Diagnostic (separate actor/critic ThinkTrunks)
#       lifted Hopper 580->920, the single strongest lever -- the shared trunk starves the actor
#       when the critic dominates the gradient on terminating envs.
#   (3) BATCH-SCOPE norm_adv: norm_adv_scope = "batch". The per-minibatch re-standardize wobbles
#       the zero-point by O(1/sqrt(mb)) and, for clamped transforms, re-inflates the bounded tails;
#       a self-normalizing transform should be standardized ONCE over the full rollout.
#   (4) PARITY TARGET (from v32 tail): target_entropy = target_entropy_coef * action_dim (SAC's
#       -|A|), so the dual target scales per-env instead of a HC-tuned absolute.
# Hypothesis: sign-correct credit + an un-starved actor let the pipeline LEARN the terminating
# envs (Walker2d/Hopper) for the first time, without regressing the HalfCheetah win (signmag == the
# winning rankgauss in magnitude, differing only in re-signing the ~9% near-median samples).
#
# ---- inherited v32 notes ----
...
class Args:
    ...
    direct_entropy: bool = True     # v31: inject entropy as a DIRECT actor-loss bonus (alpha*H in the
                                    #      objective) instead of via the indirect soft-advantage GAE.
                                    #      When True the soft-adv channel is OFF (policy_adv = reward adv).
    target_entropy: Optional[float] = None   # absolute override; if None, resolved PER-DIM from
                                    #      target_entropy_coef * action_dim (SAC parity, scales across envs).
    target_entropy_coef: float = -1.0  # PER-DIM target (SAC parity: SAC uses -|A| == -1.0/dim). Resolved
                                    #      target = coef * action_dim => -6 (HC/Walker, 6-dim), -3 (Hopper,
                                    #      3-dim). Keeps the entropy setpoint CONSISTENT across envs instead
                                    #      of a hand-set absolute that means different things per action dim.
    alpha_lr: float = 1e-3
    ...
    share_backbone: bool = False     # v33: SEPARATE actor/critic ThinkTrunks. Shared trunk starves the
                                    #      actor when the critic dominates the gradient on terminating
                                    #      envs (Hopper diagnostic 580->920 unsharing). True = v32 behavior.
    ...
    adv_transform: str = "rankgauss_signmag"  # v33: sign-CORRECT (zero-crossing at gae=0, not the batch
                                    #      median) with rankgauss's outlier-immune global-rank magnitude.
                                    #      Plain "rankgauss" anchors the sign at the median => wrong-signed
                                    #      gradients on the LEFT-SKEWED advantages of terminating envs.
    ...
    norm_adv_scope: str = "batch"    # v33: standardize the shaped advantage ONCE over the full rollout.
                                    #      Per-minibatch re-standardize wobbles the zero-point and re-inflates
                                    #      clamped tails; a self-normalizing transform wants batch scope.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if auto_alpha and not args.direct_entropy:
                # SOFT-ADV channel (v30): entropy enters the policy advantage via GAE.
                ...
                # v31 DIRECT channel: policy advantage is the pure reward advantage; entropy is
                # injected straight into the actor loss below (next_value_bonus computed but unused).
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if auto_alpha and args.direct_entropy:
                    pg_loss = pg_loss - log_alpha.exp().detach() * entropy.mean()
