# Lattice-Tanh: Learnable covariance feature map for lattice exploration.
# Based on: Latent Exploration for Reinforcement Learning https://arxiv.org/abs/2305.20065
#
# Feature pipeline: cov_trunk(obs) → tanh(x * prescale + shift) → z-norm
#
# Why this works (vs z-norm alone or tanh alone):
#   - tanh provides per-dim soft-clipping: the network can selectively saturate
#     certain dims to create structured, non-uniform covariance features.
#   - z-norm after tanh ensures unit RMS: prevents total covariance explosion
#     and gives actor_logstd a short gradient path for magnitude control.
#   - prescale (init=1.5): learnable per-dim saturation level (linear vs binary).
#   - shift (init=0): learnable per-dim bias for asymmetric saturation.
#
# tanh alone causes entropy instability (covariance grows on Walker2d).
# z-norm alone erases per-dim importance (all dims contribute equally).
# tanh+z-norm: selective importance WITH magnitude control.
#
# Architecture: split mean/cov trunks, shared W (actor_mean.weight), RMSNorm.
# Split trunks prevent gradient conflict between mean and covariance optimization.
# Asymmetric PPO clipping (low=0.2, high=0.28) relaxes upward policy updates.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, alpha=1.0, full_std=True, use_expln=False,
        ...
        # Learnable tanh + z-norm feature map for covariance
        # prescale: controls per-dim saturation (how binary vs linear each dim is)
        # shift: per-dim bias for asymmetric saturation behavior
        # z-norm (applied after tanh): ensures unit RMS, prevents covariance explosion
        self.cov_prescale = nn.Parameter(torch.full((latent_dim,), 1.5))
        self.cov_shift = nn.Parameter(torch.zeros(latent_dim))
    ...
    def get_action_and_value(self, x, action=None):
        ...
        # Tanh soft-clips per dim, then z-norm controls total magnitude
        latent_cov_tanh = torch.tanh(latent_cov_raw * self.cov_prescale + self.cov_shift)
        cov_rms = torch.sqrt(torch.mean(latent_cov_tanh ** 2, dim=-1, keepdim=True) + 1e-8)
        latent_cov = latent_cov_tanh / cov_rms
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Policy loss (asymmetric clipping: tighter floor, looser ceiling)
