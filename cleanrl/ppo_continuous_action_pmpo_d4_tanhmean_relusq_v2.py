# PMPO-D4 + ReluSq + soft-clip mean + bounded sigma v2.
#
# v1 lessons: μ = mean_scale·tanh(raw_mean) capped runaway successfully but starved
# gradient flow once raw_mean drifted past ~2 (dμ/d(raw) = scale·sech²(raw) → 0).
# Result: KL collapsed from 1.6e-3 → 2e-4 over 8M steps; policy converged too
# conservatively. Also v1 left σ unbounded — a second saturation-harvest path
# remained open via wide-σ samples reaching tanh's flat region.
#
# v2 fixes:
#   1) Proper soft-clip: μ = k · tanh(raw_mean / k). Linear region scales with k,
#      gradient at origin = 1 (not k), saturation only beyond ±k. Preserves
#      gradient flow until raw_mean genuinely needs to be capped.
#   2) Bounded log_std: clamp(raw_logstd, LOG_STD_MIN, LOG_STD_MAX). Closes the
#      σ-side saturation harvest path architecturally (SAC idiom).
#   3) k=2.0: tanh(2)=0.964 already deep into squash boundary; saves gradient
#      range vs k=3.0.
#   4) mean_scale registered as buffer (state_dict consistency).
#   5) Logs actor_logstd_mean and mean_cap_util to TB to spot σ-drift / cap
#      saturation early.
...
class Args:
    ...
    """soft-clip cap k for mean head: μ = k·tanh(raw_mean/k)"""
...
class Agent(nn.Module):
    def __init__(self, envs, mean_scale=2.0):
        super().__init__()
        ...
        self.register_buffer("mean_scale", torch.tensor(mean_scale, dtype=torch.float32))
    ...
    def get_action_distribution(self, x):
        ...
        # Proper soft-clip: μ = k · tanh(raw / k). Linear near origin, capped at ±k.
        action_mean = self.mean_scale * torch.tanh(raw_mean / self.mean_scale)
        raw_logstd = self.actor_logstd.expand_as(action_mean)
        action_logstd = torch.clamp(raw_logstd, LOG_STD_MIN, LOG_STD_MAX)
...
if __name__ == "__main__":
    ...
    agent = Agent(envs, mean_scale=args.mean_scale).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                rollout_action_means_abs.append(action_dist.mean.abs().mean().item())
            actions[step] = action
        ...
        # v2 diagnostics: σ drift and μ-cap saturation
        writer.add_scalar("diag/actor_logstd_mean", agent.actor_logstd.mean().item(), global_step)
        writer.add_scalar("diag/mean_cap_util", float(np.mean(rollout_action_means_abs)) / args.mean_scale, global_step)
