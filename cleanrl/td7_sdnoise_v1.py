...
#
# SDNOISE v1 — SAC's state-dependent exploration noise, WITHOUT SAC's maxent objective.
#
# Motivation: the full TD7->SAC actor swap (sac_v11) underperformed the TD7 StockSIG line
# (15280 vs 16454) because SAC's entropy term enters the *value targets* and biases them. Here we
# transplant only SAC's exploration ingredient: the actor grows a state-dependent log_std head
# (fc_logstd, exactly as sac_continuous_action.py) and behaves as a diagonal Gaussian whose noise
# scale varies with state. Everything on the TD/critic side stays byte-identical TD3/TD7 (min-twin
# targets, fixed 0.2 target-policy smoothing on the target actor's mean, LAP, target clipping,
# checkpointing). Entropy shapes ONLY the behavior sampling and the actor loss, never the targets.
# The temperature alpha is autotuned toward target entropy act_dim*log(target_sigma) so the average
# per-dim sigma is held near TD3's fixed 0.1 exploration budget: this is a state-conditioned
# *reallocation* of the same noise, not extra noise. With --no-sd-noise the file reduces to the
# exact TD7 baseline (no log_std head is even constructed, so parameter-init RNG is unchanged).
#
# Hypothesis: injecting exploration where the policy is genuinely uncertain (state-dependent sigma),
# while keeping the value function unbiased, captures SAC's exploration benefit without its target
# bias, improving over both fixed-noise TD7 and the full SAC swap.
import copy
import os
import random
import time
        ...
        with torch.no_grad():
            ...
            if use_exploration:
                if self.args.sd_noise:
                    # State-dependent Gaussian: a = mu + sigma(s) * eps, clamped to action bounds.
                    action = mu + log_std.exp() * torch.randn_like(mu)
                else:
                    action = mu + torch.randn_like(mu) * self.args.exploration_noise
            else:
