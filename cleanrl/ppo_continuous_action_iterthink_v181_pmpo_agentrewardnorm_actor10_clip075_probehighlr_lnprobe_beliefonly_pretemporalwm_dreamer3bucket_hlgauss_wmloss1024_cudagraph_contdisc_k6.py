# PPO + IterThink v181 pmpo-agentrewardnorm-actor10-clip075-probehighlr-lnprobe-beliefonly-pretemporalwm-dreamer3bucket-hlgauss-wmloss1024-cudagraph-contdisc-k6.
#
# v181 = v180 (PMPO) + v24-style reward normalization for the AGENT ONLY.
#   HYPOTHESIS. v180 PMPO collapsed (-55) not because of the WM but because PMPO's
#   sign-AWR weight adv.tanh().abs() is SCALE-SENSITIVE and v180 fed it the RAW,
#   un-normalized GAE advantage (NormalizeReward disabled, adv_transform=v10 identity,
#   norm_adv bypassed). At raw HalfCheetah scale tanh saturates to ~1 => degenerate
#   uniform weighting. v24 PMPO works (7780) precisely because gym NormalizeReward
#   rescales reward so the advantage lands in tanh's graded region; v179.x ppo-clip
#   survives raw reward only because norm_adv standardizes the advantage.
#   FIX (per user directive "reward norm like v24 for the agent, but the WM reward pred
#   must stay unnormalized"): maintain a gym-NormalizeReward-parity running std of the
#   discounted return and divide ONLY the agent's critic/GAE reward by it:
#     - agent_rewards[t] = rewards[t] / running_return_std  (real GAE + critic target)
#     - imagined GAE divides the WM's RAW predicted reward by the same reward_std
#     - rewards[t] (WM target future_rewards) + prev_reward_in (grounding) stay RAW
#   The symlog HL-Gauss critic support (+/-9.9 ~= raw +/-20k) covers the smaller
#   normalized-return scale without a retune (symlog is dense near 0; matches v24).
#
# -----------------------------------------------------------------------------
# v180 = v179.3 with the actor objective switched to PMPO (policy_objective=pmpo default,
#   ported from v178): sign/tanh|adv| pos/neg log-likelihood + reverse-KL(old||new) coef 0.3
#   trust region, raw (un-normalized) GAE, Beta actor. actor_update_epochs 1->10. Imagination
#   still ON. Tests whether v179's accurate WM (detached probes, discount~0.995) lets clip-free
#   PMPO survive on dreams where v178 failed (-300). policy_objective="ppo" recovers v179.3 clip.
#
# -----------------------------------------------------------------------------
...
class Args:
    ...
    actor_update_epochs: int = 10  # v180: 10 actor epochs (PMPO leans on reverse-KL leash, not clip)
    ...
    # v180-PMPO: actor objective selector and PMPO trust-region knobs.
    ...
    #   "ppo":  recovers the v179 clip-higher ratio objective (internal A/B fallback).
...
class Agent(nn.Module):
    ...
    def actor_dist_params(self, dist):
        # v180-PMPO: distribution-agnostic (p1, p2) extraction so old actor dists can be
    ...
    def make_actor_dist(self, p1, p2):
        # v180-PMPO: inverse of actor_dist_params. Rebuild a Beta/Normal from buffered
    ...
    def get_action_and_value_from_agent_input(self, obs, agent_input, z=None, return_dist=False):
        ...
        # v180-PMPO: return_dist=True additionally returns (p1, p2, log_prob_per_dim) where
...
if __name__ == "__main__":
    ...
    # v180-PMPO: buffer the OLD actor-dist params (the policy that generated each action) so
    ...
    # v181: v24-style reward normalization for the AGENT ONLY. v180 deliberately
    # disabled NormalizeReward so the WM reward head trains on RAW HalfCheetah
    # rewards (see header). But PMPO's sign-AWR feeds the RAW advantage through
    # tanh().abs(); at raw HalfCheetah scale that saturates to ~1 and degenerates
    # (v180: -55). v24 works (7780) because gym NormalizeReward already scales the
    # reward so the discounted return -- and hence the advantage -- sits in tanh's
    # graded range. We replicate gym's NormalizeReward (running std of the
    # discounted return) but apply it ONLY to the agent's critic/GAE inputs:
    #   - agent_rewards[t] = rewards[t] / running_return_std   (critic + GAE)
    #   - rewards[t] (WM target future_rewards) and prev_reward_in (grounding)
    #     stay RAW, exactly as the user requires.
    # The symlog HL-Gauss critic support (+/-9.9 ~= raw +/-20k) gracefully covers
    # the smaller normalized-return scale (symlog is dense near 0), matching v24's
    # working d3bucket critic, so no support retune is needed.
    class RewardReturnRMS:
        """Welford/Chan running mean+var of the discounted return (gym parity)."""

        def __init__(self, dev, epsilon=1e-4):
            self.mean = torch.zeros((), device=dev)
            self.var = torch.ones((), device=dev)
            self.count = epsilon

        def update(self, x):  # x: (num_envs,) current discounted-return accumulator
            batch_mean = x.mean()
    ...
    ret_rms = RewardReturnRMS(device)
    return_acc = torch.zeros(args.num_envs, device=device)
    reward_std = torch.ones((), device=device)
    agent_rewards = torch.zeros((args.num_steps, args.num_envs), device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            rewards[step] = reward_tensor  # RAW: WM reward target + grounding
            # v181: gym-NormalizeReward-parity update of the running return std, then
            # normalize THIS step's reward for the agent. Order matches gym: accumulate
            # discounted return -> update RMS -> normalize reward -> reset return at
            # episode boundary. reward_std persists to the imagination phase below.
            return_acc.mul_(args.gamma).add_(reward_tensor)
            ret_rms.update(return_acc)
            reward_std = (ret_rms.var + 1e-8).sqrt()
            agent_rewards[step] = reward_tensor / reward_std
            return_acc.mul_(1.0 - boundary_tensor_f)
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(args.num_steps)):
                ...
                delta = agent_rewards[t] + args.gamma * next_transition_values[t] * bootstrap_nonterminal - values[t]
        ...
        b_old_p1 = old_action_p1.reshape((-1,) + envs.single_action_space.shape)  # v180-PMPO: old-dist params
        ...
        reverse_kl = torch.zeros((), device=device)        # v180-PMPO: mean reverse-KL(old||new)
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if actor_active:
                    if args.policy_objective == "pmpo":
                        # v180-PMPO: forward also returns the new actor-dist params (rebuilt to a
        ...
        if reverse_kl_count > 0:
            reverse_kl = reverse_kl_sum / reverse_kl_count   # v180-PMPO: mean over minibatches
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            im_reverse_kls = []   # v180-PMPO: imagined reverse-KL(old||new) per actor update
            im_blocks = 0
            im_actor_updates = 0
            im_critic_updates = 0
            for _ in range(args.imagine_batches):
                with torch.no_grad():
                    ...
                    im_p1s, im_p2s = [], []   # v180-PMPO: dream-generation (old) actor-dist params
                    ...
                    for t in range(H_im):
                        ...
                        im_p1s.append(gen_p1)   # v180-PMPO: old (dream-generation) dist params
                    ...
                    im_rewards = torch.stack(im_rs, dim=0)                      # (H,B) RAW (WM probe)
                    # v181: the WM reward probe predicts RAW reward; convert to the
                    # agent's normalized units (same reward_std as the real critic/GAE)
                    # so the imagined advantage feeding PMPO's tanh().abs() matches the
                    # real-path scale and the shared normalized critic stays consistent.
                    im_rewards_n = im_rewards / reward_std
                    ...
                    for t in reversed(range(H_im)):
                        ...
                        delta = im_rewards_n[t] + discount * next_values[t] - values_t[t]
                    ...
                    f_old_p1 = torch.stack(im_p1s, dim=0).reshape(H_im * B_im, -1)  # v180-PMPO old dist
                ...
                for im_epoch in range(args.update_epochs):
                    ...
                    if actor_active_im:
                        ...
                        im_reverse_kls.append(reverse_kl_im.item())   # v180-PMPO
                        im_actor_updates += 1
        ...
        writer.add_scalar("charts/reverse_kl", reverse_kl.item(), global_step)   # v180-PMPO trust-region size
