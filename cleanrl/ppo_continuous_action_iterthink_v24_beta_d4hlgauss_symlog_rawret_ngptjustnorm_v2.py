# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic nGPT-justnorm v2.
#
...
# per-channel eigen learning rates initialized at max strength (1.0). Observation handling
# remains the rawret env-normalized/clipped observation vector; we normalize the
# first observation-entry matrix, not the observation vector itself, to avoid
# discarding state magnitude.
#
...
def ngpt_residual(x, branch, alpha):
    lr = alpha.abs().to(dtype=x.dtype).view(*([1] * (x.ndim - 1)), x.shape[-1])
    x_u = justnorm(x)
...
class ThinkBlock(nn.Module):
    ...
    def __init__(self, in_dim, H, n_experts):
        ...
        self.dense_alpha = nn.Parameter(torch.full((H,), NGPT_ALPHA_INIT))
        self.moe_alpha = nn.Parameter(torch.full((H,), NGPT_ALPHA_INIT))
    ...
    def forward(self, cat_feats, x0):
        ...
        x = ngpt_residual(x_in, d_dense, self.dense_alpha)
        return ngpt_residual(x, d_moe, self.moe_alpha)
