# Graph-executed tangent value model plus KL-only tangent policy, v9.
# Compose v7's state-preserving CUDA graph with v8's likelihood-ratio surrogate.
# One sealed held-out policy decision; exact Beta KL bounds both splits.
# Log clipped/unclipped/linear held gains to distinguish pessimistic clipping
# from gradient noise. No gradient/value clipping or critic step-size history.
import os
import random
import time
...
class Args:
    ...
    """KL-constrained likelihood-ratio actor or baseline minibatch Adam PPO actor"""
...
class TangentValueAdam(optim.Optimizer):
    ...
    update returns device scalars in this order: actual gain, model gain, absolute
    ...
    @torch.no_grad()
    def update(self, observations, targets):
            ...
            try:
                with torch.cuda.stream(stream):
                    for _ in range(warmup):
                        self.optimizer.update(self._observations, self._targets)
                ...
                with torch.cuda.graph(self._graph, stream=stream):
                    self.metrics = self.optimizer.update(self._observations, self._targets)
            finally:
                with torch.cuda.stream(stream):
                    for value, original in zip(mutable, saved):
                        value.copy_(original)
...
class TangentPolicyOptimizer:
    ...
    def __init__(self, actor, num_envs, num_steps, kl_budget=0.01, damping=0.01,
        ...
        def check(output, old_alpha, old_beta, native, old_logprob, advantage):
            ...
            # Exact policy KL supplies proximity control for this single step.
            # Unlike minibatch PPO, do not add a second, clipped-ratio objective.
            gains = torch.expm1(logratio) * advantage
    ...
    def step(self, observations, native, advantages, iteration):
        ...
        # These are diagnostics only: the held-out labels never select a new
        # candidate after the single final accept/reject decision.
        held_ratio_change = torch.expm1(
            Beta(actual_alpha, actual_beta, validate_args=False).log_prob(actions).sum(-1) - old_logprob)
        clipped_gains = torch.minimum(held_ratio_change * adv,
                                      held_ratio_change.clamp(-0.2, 0.2) * adv)
        score_alpha = actions.log() - old_alpha.digamma() + (old_alpha + old_beta).digamma()
        score_beta = torch.log1p(-actions) - old_beta.digamma() + (old_alpha + old_beta).digamma()
        logit_scores = torch.cat((score_alpha, score_beta), -1) * old_logits.sigmoid()
        linear_gains = adv * (logit_scores * output_displacement).sum(-1)
        clipped_streams = clipped_gains.view(self.num_steps, self.num_envs).mean(0)
        linear_streams = linear_gains.view(self.num_steps, self.num_envs).mean(0)
        metrics = {
            ...
            "candidate_heldout_clipped_gain": clipped_streams[self.fit_envs:].mean(),
            "candidate_heldout_linear_gain": linear_streams[self.fit_envs:].mean(),
            "candidate_heldout_clipping_penalty": held_gain.mean() - clipped_streams[self.fit_envs:].mean(),
            "candidate_fit_clipped_gain": clipped_streams[:self.fit_envs].mean(),
...
def main():
    ...
    try:
        ...
        if args.actor_update == "tangent":
            policy_optimizer = TangentPolicyOptimizer(
        ...
        actor_metrics = torch.empty((max_updates if actor_optimizer is not None else 0, 5), device=device)
        timer = PhaseTimer()
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            if policy_optimizer is not None:
                with timer.span("actor_update"):
                    policy_metrics = policy_optimizer.step(b_obs, b_native, b_advantages, iteration)
            updates = 0
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        prediction_metrics[updates].copy_(value_update.update(mb_obs, mb_returns))
                        updates += 1
            if actor_optimizer is not None:
