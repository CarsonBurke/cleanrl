"""INTACT one-step stochastic model control: test horizon error, not coefficients.

v5's H3 model-prescriber modes collapsed while their same-law PPO controls
learned. v6's real-return-corrected JEPA gradient reached 5,554 at matched 6M,
against 4,920 for the v5 PPO control. Yet its fresh-rollout value-expansion MSE
was 0.0034 versus advantage variance 0.0102, and the model control variate did
not reduce scalar residual variance. Good reward prediction is not evidence
that optimizing the terminal-value derivative is safe.

v7 therefore tests a specific alternative cause: was free H3 state compounding
necessary for collapse? The actor maximizes one sampled immediate model reward
plus gamma*continuation*V(predicted successor, executed action), with frozen
world/action-law/critic weights. It retains true model-driven stochastic control,
not PPO or a mean-action surrogate. H1 is the minimal bootstrapped model-control
case; only its factual starting state is used. The critic is real-return only.
There is no KL penalty, return correction, coefficient sweep, or model-loss
normalization. Same seed, environment, budget and execution architecture as v6.

Pre-fit prediction/reward/value errors and isolated encoder/law/prescriber KL
are carried forward. If real returns collapse despite low one-step errors,
shorter rollout alone is falsified; the remaining issue is trusting local model
and value derivatives under repeated actor optimization. Search-free native
conditional-Beta deployment is unchanged. This is not an INTACT reproduction.
"""
import os
import random
import copy
...
class Args:
    ...
    """unsupported for a pure model actor"""

    control_mode: Literal["model"] = "model"
    """pure one-step JEPA model-gradient prescriber"""
    goal_horizon: int = 8
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if args.control_mode not in {"model"}:
...
def model_action_value(agent, latent, physical, args, *, frozen=False):
    following = agent.predict_next(latent, physical, frozen=frozen)
    return agent.predict_reward(latent, physical, frozen=frozen) + args.gamma * agent.predict_continuation(
        latent, physical, frozen=frozen,
    ) * agent.value_from_latent(following, physical, frozen=frozen).squeeze(-1)
...
def actor_objective(agent, observations, args):
    ...
    return -model_action_value(agent, latent, physical, args, frozen=True).mean()
...
def training_loss(agent, observations, native_actions, old_logprobs, advantages, returns,
    ...
    """Pure H1 model actor; real-return critic and identical INTACT world losses."""
    ...
    actor_loss = actor_objective(agent, observations, args)
    ...
    with torch.no_grad():
        metrics.update({
            ...
            "model/hypothetical_correction": correction.detach(),
            "model/imagined_actor_return": -actor_loss.detach(),
...
def validate_args(args):
    ...
    if args.target_kl is not None:
        raise ValueError("a PPO KL stopping rule is not defined for the pure model objective")
