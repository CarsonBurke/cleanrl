...
#
# v2 vs v1: idiomatic tanh-squash matching ppo_continuous_action_tanhsq_v1.py.
#   - rollout buffer stores POST-squash env_action (not pre-squash z).
#   - replay path inverts via affine + atanh: u = (a - center)/half_range; z = atanh(u).
#   - drops gym.wrappers.ClipAction (tanh already bounds the action).
#   - adds an affine [-1, 1] → [low, high] action map (Box action space).
#   - simple Jacobian form: log(1 - u² + SQUASH_EPS) instead of softplus form.
#
...
#
# Rollout storage: post-squash env_action (idiomatic, matches tanhsq_v1).
# Replay path inverts via affine + atanh: u = (a - center)/half_range; z = atanh(u).
# u is clamped to [-1+SQUASH_EPS, 1-SQUASH_EPS] before atanh for numerical safety.
# ClipAction wrapper is dropped — tanh already bounds u ∈ [-1, 1].
#
...
SQUASH_EPS = 1e-6  # numerical floor for log(1 - u²) and u clamp on replay atanh
...
class Agent(nn.Module):
    ...
    def _action_to_u(self, action):
        center = 0.5 * (self.action_low + self.action_high)
        half_range = 0.5 * (self.action_high - self.action_low)
        u = (action - center) / half_range
        return u.clamp(-1.0 + SQUASH_EPS, 1.0 - SQUASH_EPS)
    ...
    def get_action_and_value(self, x, action=None):
        """Idiomatic tanh-squashed PPO (matches ppo_continuous_action_tanhsq_v1).

        If `action` is None: sample fresh z ~ Normal(mean, std), return
            env_action = affine(tanh(z)) along with log_prob, entropy, value.
        Else: invert the affine map and tanh to recover z = atanh(u), then
            recompute log_prob at that z under the current policy.

        Jacobian: log|det da/dz| = sum_i log(1 - u_i² + eps). The constant
        affine half_range factor cancels in the PPO ratio, so we omit it.
        """
        ...
        return env_action, log_prob, entropy, value_logits, std
...
if __name__ == "__main__":
    ...
    # `actions` stores the post-squash env_action (matches tanhsq_v1 idiom);
    # the replay path inverts via affine + atanh inside get_action_and_value.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                env_action, logprob, _, value_logits, _ = agent.get_action_and_value(next_obs)
                values[step] = hl_support.to_scalar(value_logits)
            actions[step] = env_action  # store post-squash env_action for replay
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # b_actions[mb_inds] is the stored post-squash env_action;
                # get_action_and_value inverts the affine+tanh to recover z.
