# Fixed-task successor-action score geometry PPO v17.
# The unchanged scalar Tanh64x64 PPO actor/critic gains a separate Tanh64x64
# control c=b0+d.z, where S is the behavior Beta score, F=E[SS^T]=LL^T,
# z=L^-1 S and b=L^-T d. Frozen natural coefficients integrate c exactly under
# the new ideal Beta policy. No SF/return/latent prediction objective is used.
# Let J=d(alpha,beta)/d logits, K=Jac_actor Jac_actor^T, W=J K J and
# D=J[S,SS^T-F]. The corrected parameter gradient has output form J S A-D b.
# Its coefficient-independent conditional mean makes second-moment minimization
# equivalent to variance-trace minimization. The exact normal H=E[D^T K D] is
# H00=tr(WF), H0b=W:kappa3, Hbb=tr(WF)F+FWF+W:kappa4.
# Independent Beta action cumulants require only 2x2 blocks, not a rank-4 tensor.
# Integrate H in float64, then whiten via B=diag(1,L^-T): Hwhite=B^T H B.
# Analytic fitting uses 0.5*A^2*q-A*d.e+0.5*d^T*Hwhite*d, with
# q=S^T W S and e=B^T[q, S*q-F W S]. This unbiased population surrogate can
# be negative on a finite minibatch; it is NOT a realized squared gradient.
# Sampled fitting uses the same whitened basis and full actor correction, but
# samples the quadratic term. PPO fits the unused analytic control as a match.
# The optimum is conditional, ideal-Beta, unclipped and unpreconditioned, not
# an Adam/clipped-PPO/reused-batch global optimum or unbiased-GAE guarantee.
# Minibatch-normalized PPO advantages are also control targets, without scaling.
# Finite-precision clipped sampling, target normalization and adaptive reuse
# limit the population identity; diagnostics are prefit, not future guarantees.
# Precedents: Q-Prop (https://arxiv.org/abs/1611.02247) and direct variance training
# in LAX/RELAX (https://arxiv.org/abs/1711.00123); no generic CV novelty claim.
import os
import random
import time
...
class Args:
    ...
    control_mode: Literal["ppo", "sampled", "analytic"] = "analytic"
    """PPO fits unused analytic control; sampled/analytic apply the full control."""
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        super().__init__()
        if args.control_mode not in {"ppo", "sampled", "analytic"}:
            raise ValueError("control_mode must be ppo, sampled or analytic")
...
def _beta_block_matrix(aa, ab, bb):
    """Dense independent-action blocks in alpha-all, beta-all coordinate order."""
    cross = torch.diag_embed(ab)
    return torch.cat((
        torch.cat((torch.diag_embed(aa), cross), dim=-1),
        torch.cat((cross, torch.diag_embed(bb)), dim=-1),
    ), dim=-2)


@torch.no_grad()
def beta_control_geometry(alpha, beta, gram):
    """Return detached packed Fisher whitening and exact population normal.

    For A actions, outputs are [...,3,A] and [...,1+2A,1+2A].
    Packed entries are (1/l11, -l21/(l11*l22), 1/l22).
    All cancellation-sensitive computation stays float64 on the input device;
    only the whitened result is cast back. No sample score enters this normal.
    """
    dtype = alpha.dtype
    alpha, beta, gram = (value.detach().to(torch.float64) for value in (alpha, beta, gram))
    diagonal, cross = beta_fisher_components(alpha, beta)
    faa, fbb = diagonal.chunk(2, dim=-1)
    l11 = faa.sqrt()
    l21 = cross / l11
    l22 = (fbb - l21.square()).sqrt()
    inv_l11, mixed, inv_l22 = l11.reciprocal(), -l21 / (l11 * l22), l22.reciprocal()
    whitening = torch.stack((inv_l11, mixed, inv_l22), dim=-2)
    fisher = _beta_block_matrix(faa, cross, fbb)
    jacobian = beta_logit_jacobian(alpha, beta)
    weight = jacobian.unsqueeze(-1) * gram * jacobian.unsqueeze(-2)
    actions = alpha.shape[-1]
    waa, wbb = weight.diagonal(dim1=-2, dim2=-1).chunk(2, dim=-1)
    wab = weight[..., :actions, actions:].diagonal(dim1=-2, dim2=-1)
    block_sum = waa + 2 * wab + wbb
    trace = (waa * faa + 2 * wab * cross + wbb * fbb).sum(-1)
    third_common = torch.polygamma(2, alpha + beta) * block_sum
    third = torch.cat((torch.polygamma(2, alpha) * waa - third_common,
                       torch.polygamma(2, beta) * wbb - third_common), dim=-1)
    fourth_common = torch.polygamma(3, alpha + beta) * block_sum
    fourth = _beta_block_matrix(
        torch.polygamma(3, alpha) * waa - fourth_common,
        -fourth_common, torch.polygamma(3, beta) * wbb - fourth_common,
    )
    normal_bb = trace[..., None, None] * fisher + fisher @ weight @ fisher + fourth
    normal = torch.cat((
        torch.cat((trace[..., None, None], third.unsqueeze(-2)), dim=-1),
        torch.cat((third.unsqueeze(-1), normal_bb), dim=-1),
    ), dim=-2)
    # Right- then left-multiply by B without allocating a dense basis transform.
    right = torch.cat((
        normal[..., :1], normal[..., 1:1 + actions] * inv_l11.unsqueeze(-2),
        normal[..., 1:1 + actions] * mixed.unsqueeze(-2)
        + normal[..., 1 + actions:] * inv_l22.unsqueeze(-2),
    ), dim=-1)
    normal_white = torch.cat((
        right[..., :1, :], right[..., 1:1 + actions, :] * inv_l11.unsqueeze(-1),
        right[..., 1:1 + actions, :] * mixed.unsqueeze(-1)
        + right[..., 1 + actions:, :] * inv_l22.unsqueeze(-1),
    ), dim=-2)
    return whitening.to(dtype), normal_white.to(dtype)


def natural_coefficients(coefficients, whitening):
    """Convert [b0,d_alpha,d_beta] to natural [b0,b_alpha,b_beta].

    Only coefficients are differentiable; packed behavior whitening is a teacher.
    """
    inv_l11, mixed, inv_l22 = whitening.detach().unbind(dim=-2)
    da, db = coefficients[..., 1:].chunk(2, dim=-1)
    return torch.cat((coefficients[..., :1], da * inv_l11 + db * mixed, db * inv_l22), dim=-1)


@torch.no_grad()
def control_linear_features(score, alpha, beta, gram, whitening):
    """Detached (q,e_white), independent of advantage normalization or targets."""
    dtype = score.dtype
    score, alpha, beta, gram, whitening = (
        value.detach().to(torch.float64) for value in (score, alpha, beta, gram, whitening)
    )
    jacobian = beta_logit_jacobian(alpha, beta)
    weighted_score = jacobian * (gram @ (jacobian * score).unsqueeze(-1)).squeeze(-1)
    score_power = (score * weighted_score).sum(-1)
    response = beta_fisher_product(alpha, beta, weighted_score)
    ea, eb = (score * score_power.unsqueeze(-1) - response).chunk(2, dim=-1)
    inv_l11, mixed, inv_l22 = whitening.unbind(dim=-2)
    eligibility = torch.cat((
        score_power.unsqueeze(-1), inv_l11 * ea, mixed * ea + inv_l22 * eb,
    ), dim=-1)
    return score_power.to(dtype), eligibility.to(dtype)


def analytic_control_loss(coefficients, advantages, normal, eligibility, score_power, actor_parameter_count):
    """Population-integrated quadratic surrogate per actor parameter.

    Only current white coefficients attach. The finite-sample value may be
    negative: do not interpret it as the realized gradient second moment.
    """
    advantages = advantages.detach()
    linear = (coefficients * eligibility.detach()).sum(-1)
    quadratic = (coefficients.unsqueeze(-2) @ normal.detach() @ coefficients.unsqueeze(-1)).flatten()
    return (0.5 * advantages.square() * score_power.detach() - advantages * linear
            + 0.5 * quadratic).mean() / actor_parameter_count
...
def policy_loss(agent, observations, native_actions, advantages, returns, old_data, args):
    ...
    if args.control_mode == "sampled":
        fit_objective = control_variance_loss(
            natural_coefficients(coefficients, old_data["whitening"]), ppo_advantages,
            old_data["score"], old_data["alpha"], old_data["beta"],
        ...
        fit_objective = analytic_control_loss(
            coefficients, ppo_advantages, old_data["normal"], old_data["eligibility"],
            old_data["score_power"], agent.actor_parameter_count,
        )
    ...
    objective = pg_loss + correction_loss - args.ent_coef * entropy + args.vf_coef * value_loss + fit_objective
    with torch.no_grad():
        metrics = {
            ...
            "control/fit_objective": fit_objective.detach(), "control/correction_loss": correction_loss.detach(),
...
def rollout_statistics(agent, observations, native_actions):
    ...
    white_coefficients = agent.control(observations.detach())
    gram = actor_jacobian_gram(agent.actor, observations)
    whitening, normal = beta_control_geometry(alpha, beta, gram)
    coefficients = natural_coefficients(white_coefficients, whitening)
    score_power, eligibility = control_linear_features(score, alpha, beta, gram, whitening)
    ...
    data = {
        ...
        "coefficients": coefficients, "white_coefficients": white_coefficients, "gram": gram,
        "whitening": whitening, "normal": normal, "eligibility": eligibility, "score_power": score_power,
...
def control_diagnostics(agent, observations, advantages, old_data, exact, norm_adv=True):
    ...
    if exact:
        ...
        metrics.update({
            ...
            "control/preupdate_sampled_second_moment": 0.5 * corrected_second / agent.actor_parameter_count,
            "control/preupdate_analytic_fit_objective": analytic_control_loss(
                old_data["white_coefficients"], advantages, old_data["normal"], old_data["eligibility"],
                old_data["score_power"], agent.actor_parameter_count,
            ),
        })
...
def validate_args(args):
    if args.env_id != "HalfCheetah-v4" or args.env_backend != "native":
        raise ValueError("v17 matched fixed-task runs require native HalfCheetah-v4")
    if not args.cuda or not args.compile:
        raise ValueError("v17 requires CUDA and compiled learner execution")
    if args.track:
        raise ValueError("v17 matched runs use local TensorBoard, not WandB")
    if args.control_mode not in {"ppo", "sampled", "analytic"}:
...
def main():
    ...
    with ExitStack() as resources:
        ...
        contract = {
            "version": 17, "control_mode": args.control_mode, "critic_mode": "scalar",
            ...
            "control_basis": "[1,z]; z=L^-1*S; F=L*L^T; natural_b=L^-T*d; alpha_all_then_beta_all",
            "control_units": "PPO_minibatch_normalized_advantage_if_norm_adv_else_raw_GAE; actor_correction_unscaled",
            "training_objective": "analytic_and_ppo: mean(0.5*A^2*q-A*d.e+0.5*d^T*Hwhite*d)/P; "
                                  "sampled: 0.5*mean(g^T*K*g)/P; control_weight=1",
            "geometry": "exact_Beta_cumulants_through_order4; float64_on_CUDA_then_cast_after_whitening; "
                        "Hwhite=B^T*E[D^T*K*D|s]*B; B=diag(1,L^-T); no_sample_score_outer_or_normal_solve",
            "fit_metric": "analytic_finite_sample_surrogate_can_be_negative; not_realized_squared_gradient",
            "matched_comparison": "sampled_vs_analytic_same_white_basis_and_full_actor_correction; "
                                  "v16_comparison_does_not_isolate_exact_geometry",
            ...
            "actor_correction": "mean(ratio*c_old-E_new[c_old]); sampled_and_analytic_use_full_control; ppo_uses_none",
            ...
            "teacher_freezing": "alpha_beta_logmoments_scores_natural_and_white_coefficients_samples_Gram_Fisher_J_"
                                "whitening_normal_eligibility_scorepower_values_logprobs_cached_before_any_fit",
            "optimum_scope": "exact_conditional_trace_for_ideal_Beta_unclipped_behavior_objective_unpreconditioned_actor_gradient",
            "not_claimed": "SF_or_return_or_latent_prediction; Adam_or_clipped_PPO_or_reused_batch_global_optimum; "
                           "unbiased_GAE; generic_control_variate_novelty",
        ...
        metric_names = ("losses/policy_loss", "losses/value_loss", "losses/entropy", "losses/old_approx_kl",
                        "losses/approx_kl", "losses/clipfrac", "control/fit_objective", "control/correction_loss",
