# PPO + IterThink v53 (v24 Beta + actor/critic role-memory loop-bank). From v24.
#
...
#     role-token gradient leakage, while sharing observation embeddings, recurrent
#     block weights, and the final projection.
#   - Final readout flattens/concats every token from every loop bank before one
#     projection, preserving v24's wide final feature path.
#
...
class ThinkTrunk(nn.Module):
    ...
    def _run_stream(self, initial):
        ...
        flat = torch.cat([bank.reshape(bank.shape[0], -1) for bank in ledger], dim=-1)
        return self.out_proj(self.out_norm(flat))
    ...
    def forward(self, x):
        ...
        with ctx:
            actor_feat = self._run_stream(self._initial_bank(x, "actor"))
            critic_feat = self._run_stream(self._initial_bank(x, "critic"))
