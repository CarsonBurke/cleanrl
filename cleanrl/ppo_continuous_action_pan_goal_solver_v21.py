# Pan Goal Solver v21: v20 with far-frame waypoints as the successor target.
#
# v19 AND v20 both measured the goal channel dead (goal_removal_action_mse
# pinned at 0.001 vs 0.05+ for v16/v17): the one-step frame target is
# near-goal-blind because, given the belief, the behavior mixture's next
# action (scripted explorer + fixed-goal policy) is almost independent of
# where the trajectory ends up 50 steps later — and attaching the composed
# loss (v20) cannot help, since with a good inverse model the S that best
# explains a_t is still the true next frame. The conditioning signal that
# worked in v16/v17 was far-horizon: regressing the first action on the
# ~50-step hindsight goal. v21 brings that signal into the factored design:
# sample a waypoint frame f_{t+j} (j uniform in [1, min(k, 16)]) on the
# actual path to the hindsight goal at offset k. S predicts the waypoint
# (a target with ~j-fold stronger goal-dependence), and I gains a
# first-action-toward-far-frame hindsight term I(b, f_t, f_{t+j}) -> a_t
# alongside the exact one-step inverse loss. Composition and act-time path
# unchanged: a = I(b, f, S(b, f, g, G)), composed loss attached through S.
#
...
def train_follower_step(agent, optimizer, batch, args, device):
    ...
    # The waypoint target carries ~j-fold stronger goal-dependence than the
    # one-step frame (v19/v20 measured the one-step target goal-blind:
    # goal_removal_action_mse pinned at 0.001 vs 0.05+ for direct far-goal
    # action regression).
