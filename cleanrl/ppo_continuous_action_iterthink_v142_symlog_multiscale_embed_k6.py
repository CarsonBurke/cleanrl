# PPO + IterThink v142 symlog-multiscale-embed-k6 (v141 with immediate obs symlog).
#
...
#     Project that basis directly to token dim, add learned feature slot
#     embedding, then apply embedding RMSNorm before the encoder stack.
...
#
# Hypothesis:
#   Immediate symlog controls raw MuJoCo outliers before the GPT-style scalar
#   embedding basis, while the multiscale features preserve enough directional
#   magnitude information for embedding RMSNorm to be safe.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # GPT-style symlog-observation tokenizer. Apply symlog immediately to raw
        # scalars, map through a fixed multiscale basis, project directly to token
        # dim, add slot identity, then embedding-normalize the token stream.
