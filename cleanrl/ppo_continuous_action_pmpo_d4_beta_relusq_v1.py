# PMPO-D4 + ReluSq + Beta-distribution policy v1.
#
# Sidesteps the entire saturation-harvest pathology by replacing Normal(μ,σ)
# + tanh-squash with Beta(α,β) on [0,1] linearly remapped to the action box.
# Beta has bounded support natively → no Jacobian, no `log|det J| ≈ 2|z|` bonus
# to harvest. Removes the need for μ-cap and σ-clamp.
#
# Parametrization (per Della Libera 2024 / arxiv:2409.04971):
#   α = 1 + exp(clamp(head_α, -20, 2))
#   β = 1 + exp(clamp(head_β, -20, 2))
# +1 shift forces α,β ≥ 1 → unimodal, concave Beta (Chou et al. 2017 trick).
# Default-init head outputs ≈ 0 → α=β=2, Beta(2,2) symmetric mean=0.5
# (=action center after remap), entropy ≈ -0.12 per dim.
#
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Actor outputs 2*action_dim: [log(α-1) heads, log(β-1) heads].
