# Predictive-coding actor-critic with bounded Beta Fisher geometry, v7.
#
...
# inference uses a conservative quadratic output node centered on the free output.
# Its frozen metric is the exact expected Beta Fisher pulled back through smooth
# bounded mean/concentration coordinates plus explicit numerical damping; the
# sampled score supplies a direct linear nudge, avoiding an inverse-Fisher solve.
...
# per-environment, per-parameter TD(lambda) traces. Directions are applied by a
# streaming Adam optimizer with zero default weight decay. This is prospective
# predictive coding plus temporal eligibility, not a conjugate Bayesian posterior.
import math
import os
import random
import time
...
def bounded_beta_parameters(raw, edge_offset, concentration_min, concentration_max):
    """Map mean/concentration coordinates to a smooth, finite Beta family."""
    if edge_offset <= 0:
...
def bounded_beta_jacobian(raw, allocation, concentration, concentration_min, concentration_max):
    """Per-action Jacobian d(alpha,beta)/d(mean_logit,concentration_raw)."""
    ...
    jacobian = raw.new_empty(*allocation.shape, 2, 2)
    jacobian[..., 0, 0] = concentration * allocation_derivative
    jacobian[..., 1, 0] = -concentration * allocation_derivative
    jacobian[..., 0, 1] = allocation * concentration_derivative
    jacobian[..., 1, 1] = (1.0 - allocation) * concentration_derivative
    return jacobian
...
def bounded_beta_score(
    ...
    """Exact sampled log-likelihood score in bounded raw coordinates."""
    ...
    parameter_score = torch.stack([alpha_score, beta_score], dim=-1)
    jacobian = bounded_beta_jacobian(
        raw, allocation, concentration, concentration_min, concentration_max
    )
    raw_score = torch.einsum("baij,bai->baj", jacobian, parameter_score)
    return torch.cat([raw_score[..., 0], raw_score[..., 1]], dim=1)


def bounded_beta_fisher_metric(
    raw,
    alpha,
    beta,
    allocation,
    ...
    """Exact J^T F_beta J metric, with full 2x2 blocks per action."""
    if damping <= 0:
        raise ValueError("pc_fisher_damping must be positive")
    common = torch.polygamma(1, alpha + beta)
    faa = torch.polygamma(1, alpha) - common
    fbb = torch.polygamma(1, beta) - common
    fab = -common
    fisher_ab = raw.new_empty(*alpha.shape, 2, 2)
    fisher_ab[..., 0, 0] = faa
    fisher_ab[..., 0, 1] = fab
    fisher_ab[..., 1, 0] = fab
    fisher_ab[..., 1, 1] = fbb
    ...
    blocks = jacobian.transpose(-1, -2) @ fisher_ab @ jacobian
    ...
    metric[:, idx, idx] = blocks[..., 0, 0]
    metric[:, action_dim + idx, action_dim + idx] = blocks[..., 1, 1]
    metric[:, idx, action_dim + idx] = blocks[..., 0, 1]
    metric[:, action_dim + idx, idx] = blocks[..., 1, 0]
    metric.diagonal(dim1=-2, dim2=-1).add_(damping)
    return metric


def fixed_endpoint_force(output, free_output, metric, score, nudge):
    """Negative energy gradient for the frozen quadratic-plus-linear endpoint."""
    if nudge <= 0:
        raise ValueError("PC nudges must be positive")
    return nudge * score - metric_times(metric, output - free_output)
        ...
        for _ in range(num_steps):
            for layer_idx in reversed(range(num_layers)):
                ...
                if layer_idx + 1 < num_layers:
                    ...
                    output = F.linear(states[layer_idx], output_weight, output_bias)
                    output_force = fixed_endpoint_force(
                        output, free_output, metric, output_score, nudge
                    )
...
class Agent(nn.Module):
    ...
    def actor_distribution(self, hidden):
        ...
        return raw, alpha, beta, allocation, concentration, Beta(alpha, beta)

    @torch.no_grad()
    def actor_endpoint(self, hidden, action_z, args):
        output, alpha, beta, allocation, concentration, _ = self.actor_distribution(hidden)
        ...
        return output, metric, score

    @torch.no_grad()
    def critic_endpoint(self, hidden, args):
        output = self.critic_output(hidden)
        metric = output.new_ones(output.shape[0], 1, 1)
        score = torch.ones_like(output)
        return output, metric, score
    ...
    def act(self, x):
        ...
        _, _, _, _, _, dist = self.actor_distribution(actor_states[-1])
    ...
    def settle_actor(self, x, action_z, args, collect_diagnostics=True):
        ...
        free_output, metric, score = self.actor_endpoint(free_states[-1], action_z, args)
        ...
        settled_output = self.actor_output(states[-1])
        output_force = fixed_endpoint_force(
            settled_output, free_output, metric, score, args.pc_actor_nudge
        )
        if collect_diagnostics:
            ...
            fisher_eigenvalues = torch.linalg.eigvalsh(metric)
            diagnostics["fisher_min_eigenvalue"] = fisher_eigenvalues[:, 0].mean()
            diagnostics["fisher_max_eigenvalue"] = fisher_eigenvalues[:, -1].mean()
            diagnostics["fisher_condition"] = (
                fisher_eigenvalues[:, -1]
                / fisher_eigenvalues[:, 0].clamp_min(args.pc_fisher_damping)
            ).mean()
            undamped_trace = (
                metric.diagonal(dim1=-2, dim2=-1) - args.pc_fisher_damping
            ).clamp_min(0.0).mean()
            diagnostics["fisher_damping_fraction"] = (
                args.pc_fisher_damping
                / (undamped_trace + args.pc_fisher_damping).clamp_min(1e-12)
            )
            effective_score = output_force / args.pc_actor_nudge
            diagnostics["effective_score_cosine"] = F.cosine_similarity(
                effective_score.flatten(1), score.flatten(1), dim=1
            ).mean()
            diagnostics["effective_score_relative_error"] = (
                (effective_score - score).flatten(1).norm(dim=1)
                / score.flatten(1).norm(dim=1).clamp_min(1e-12)
            ).mean()
        else:
            ...
            diagnostics["fisher_min_eigenvalue"] = states[0].new_zeros(())
            diagnostics["fisher_max_eigenvalue"] = states[0].new_zeros(())
            diagnostics["fisher_condition"] = states[0].new_zeros(())
            diagnostics["fisher_damping_fraction"] = states[0].new_zeros(())
            diagnostics["effective_score_cosine"] = states[0].new_zeros(())
            diagnostics["effective_score_relative_error"] = states[0].new_zeros(())
        output_score = (
            output_force.unsqueeze(2)
            * self.actor_output.augmented_features(states[-1]).unsqueeze(1)
            / args.pc_actor_nudge
        )
    ...
    def settle_critic(self, x, args, collect_diagnostics=True):
        ...
        free_output, metric, score = self.critic_endpoint(free_states[-1], args)
        ...
        if collect_diagnostics:
            diagnostics["output_force_error"] = (
                fixed_endpoint_force(
                    free_output, free_output, metric, score, args.pc_critic_nudge
                )
                / args.pc_critic_nudge
                - score
            ).square().mean().sqrt()
        ...
        output_score = (
            output_force.unsqueeze(2)
            * self.critic_output.augmented_features(states[-1]).unsqueeze(1)
            / args.pc_critic_nudge
        )
        return states, residuals, output_score, diagnostics
...
def main():
    ...
    if args.pc_actor_nudge <= 0 or args.pc_critic_nudge <= 0:
        raise ValueError("PC nudges must be positive")
    if args.pc_fisher_damping <= 0:
        raise ValueError("pc_fisher_damping must be positive")
    # Validate the bounded family before constructing CUDA state.
    bounded_beta_parameters(
        torch.zeros(1, 2),
        args.beta_edge_offset,
        args.beta_concentration_min,
        args.beta_concentration_max,
    )
    if args.compile:
    ...
    for update in range(1, args.num_updates + 1):
        ...
        if collect_diagnostics:
            ...
            behavior_concentration = (
                behavior_dist.concentration1
                + behavior_dist.concentration0
                - 2.0 * args.beta_edge_offset
            )
            writer.add_scalar("policy/concentration_mean", behavior_concentration.mean(), global_step)
            writer.add_scalar("policy/concentration_max", behavior_concentration.max(), global_step)
            for key in (
                "fisher_min_eigenvalue",
                "fisher_max_eigenvalue",
                "fisher_condition",
                "fisher_damping_fraction",
                "effective_score_cosine",
                "effective_score_relative_error",
            ):
                writer.add_scalar(f"pc/actor_{key}", actor_diag[key], global_step)
