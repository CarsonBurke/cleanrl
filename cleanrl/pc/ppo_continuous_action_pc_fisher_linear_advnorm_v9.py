# Fisher-clamped predictive coding with exact linear streaming advantage normalization, v9.
#
...
# per-environment, per-parameter TD(lambda) traces. Lagged reward-independent
# postsynaptic-neuron score-RMS factors and creation-time learning rates enter
# two actor traces, so immediate linear updates sum exactly to centered, normalized forward
# lambda-return updates. The critic retains streaming AdamW. This is prospective
# predictive coding plus temporal eligibility, not a conjugate posterior update.
import math
import os
import random
import time
...
class Args:
    ...
    actor_learning_rate: float = 8e-4
    ...
    actor_neuron_rms_decay: float = 0.999
    actor_neuron_rms_floor: float = 0.1
...
class StreamingAdvantageMoments:
    """Lagged moments of a causal, consistently clipped lambda-return proxy."""
    ...
    @torch.no_grad()
    def assign_and_update(self, delta, done, trace_decay, clip):
        ...
        clipped_delta = (
            delta.clamp(-clip * assigned_std, clip * assigned_std)
            if clip > 0
            else delta
        )
        clipped_delta = torch.where(finite, clipped_delta, 0.0)
        next_proxy = torch.where(
            finite, clipped_delta + trace_decay * self.proxy, 0.0
        )
        ...
        return assigned_mean, assigned_std, clipped_delta


class PostsynapticNeuronScoreRMS:
    """Lagged score RMS shared by every incoming synapse of one neuron."""

    def __init__(self, templates, decay, minimum):
        self.decay = decay
        self.minimum = minimum
        self.mean_squares = [
            torch.ones(
                template.shape[1], 1, device=template.device, dtype=template.dtype
            )
            for template in templates
        ]
        self.initialized = torch.zeros(
            (), dtype=torch.bool, device=self.mean_squares[0].device
        )

    @torch.no_grad()
    def precondition_and_update(self, scores, valid):
        if len(scores) != len(self.mean_squares):
            raise ValueError("score and preconditioner structures differ")
        if scores[0].shape[0] != valid.shape[0]:
            raise ValueError("valid shape must match score batch")
        scales = [
            torch.where(
                self.initialized,
                mean_square.sqrt().clamp_min(self.minimum).reciprocal(),
                torch.ones_like(mean_square),
            )
            for mean_square in self.mean_squares
        ]
        row_mask = valid.view(valid.shape[0], 1, 1)
        preconditioned = [
            torch.where(row_mask, score * scale, 0.0)
            for score, scale in zip(scores, scales)
        ]

        count = valid.sum().clamp_min(1)
        has_valid = valid.any()
        for score, mean_square in zip(scores, self.mean_squares):
            batch_mean_square = (
                torch.where(row_mask, score, 0.0)
                .square()
                .sum(dim=(0, 2))
                .unsqueeze(1)
                / (count * score.shape[2])
            ...
            candidate = torch.where(
                self.initialized,
                torch.lerp(mean_square, batch_mean_square, 1.0 - self.decay),
                batch_mean_square,
            )
            mean_square.copy_(torch.where(has_valid, candidate, mean_square))
        self.initialized.logical_or_(has_valid)
        return preconditioned, scales

    def layer_rms(self):
        return [mean_square.mean().sqrt() for mean_square in self.mean_squares]
...
class AugmentedLinearAscent:
    """Unit-step linear ascent; directions already contain creation-time rates."""

    def __init__(self, edges):
        self.edges = list(edges)

    @torch.no_grad()
    def step(self, directions, collect_diagnostics=False):
        ...
        for edge, direction in zip(self.edges, directions):
            edge.weight.add_(direction[:, :-1])
            edge.bias.add_(direction[:, -1])
            if collect_diagnostics:
                update_rms.append(direction.square().mean().sqrt())
        return update_rms
...
def make_trace_banks(agent, args, device):
    ...
    return actor_reward, actor_baseline, critic, actor_templates
...
def main():
    ...
    if not 0 <= args.actor_neuron_rms_decay < 1:
        raise ValueError("actor_neuron_rms_decay must be in [0, 1)")
    if args.actor_neuron_rms_floor <= 0:
        raise ValueError("actor_neuron_rms_floor must be positive")
    if args.compile:
    ...
    (
        ...
        critic_trace,
        actor_templates,
    ) = make_trace_banks(agent, args, device)
    actor_optimizer = AugmentedLinearAscent(agent.actor_edges())
    actor_neuron_rms = PostsynapticNeuronScoreRMS(
        actor_templates, args.actor_neuron_rms_decay, args.actor_neuron_rms_floor
    )
    ...
    for update in range(1, args.num_updates + 1):
        ...
        with torch.no_grad():
            ...
            (
                assigned_advantage_mean,
                assigned_advantage_std,
                actor_delta,
            ) = advantage_moments.assign_and_update(
                td_error, done, trace_decay, args.actor_td_clip
            )
        ...
        with torch.no_grad():
            ...
            preconditioned_actor_scores, actor_neuron_scales = (
                actor_neuron_rms.precondition_and_update(actor_scores, actor_valid)
            )
            creation_scale = (
                args.actor_learning_rate * frac / assigned_advantage_std
            )
            actor_reward_trace.accumulate(
                [score * creation_scale for score in preconditioned_actor_scores],
                trace_decay,
            )
            actor_baseline_trace.accumulate(
                [
                    score * (creation_scale * assigned_advantage_mean)
                    for score in preconditioned_actor_scores
                ],
                trace_decay,
            )
            ...
            actor_directions = normalized_actor_directions(
                ...
                actor_baseline_trace,
                actor_delta,
                done | ~actor_valid | (update == args.num_updates),
                trace_decay,
            )
            critic_directions = [
            ...
            actor_update_rms = actor_optimizer.step(actor_directions, collect_diagnostics)
            critic_update_rms = critic_optimizer.step(
                critic_directions, args.critic_learning_rate * frac, collect_diagnostics
            )
            actor_reward_trace.reset(done | ~actor_valid)
            actor_baseline_trace.reset(done | ~actor_valid)
        ...
        if collect_diagnostics:
            ...
            writer.add_scalar("td/error_instant_rms", td_error.square().mean().sqrt(), global_step)
            ...
            writer.add_scalar(
                "advnorm/diagnostic_centered_onestep_delta_mean",
                instant_normalized_delta.mean(),
                global_step,
            )
            writer.add_scalar(
                "advnorm/diagnostic_centered_onestep_delta_rms",
            ...
            writer.add_scalar("direction/actor_parameter_delta", actor_direction_norm, global_step)
            ...
            actor_neuron_rms_values = actor_neuron_rms.layer_rms()
            ...
            for idx in range(len(agent.actor_edges())):
                ...
                writer.add_scalar(
                    f"pc_actor/edge_{idx}_neuron_score_rms",
                    actor_neuron_rms_values[idx],
                    global_step,
                )
                writer.add_scalar(
                    f"pc_actor/edge_{idx}_assigned_preconditioner_rms",
                    actor_neuron_scales[idx].square().mean().sqrt(),
                    global_step,
                )
                writer.add_scalar(f"pc_actor/edge_{idx}_linear_update_rms", actor_update_rms[idx], global_step)
