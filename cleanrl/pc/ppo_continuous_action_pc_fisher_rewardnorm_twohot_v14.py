# Reward-normalized predictive-coding actor-critic with TwoHot critic v14.
#
# This is clamped TD(lambda) v3 with zero weight decay and exactly one algorithmic
# replacement: the scalar critic becomes an immediate categorical TD(0) learner
# over Dreamer's 255 raw symexp-spaced bins. The v3 reward wrappers, actor TD RMS,
# actor Fisher PC scores, and actor TD(lambda) trace remain unchanged.
import math
import os
import random
import time
...
class RunningTDRMS:
    """The exact v3 actor-only running TD-error RMS normalizer."""
...
def compute_td_modulations(
    ...
    """Compute the robust target and exact v3 actor TD-RMS modulation."""
    ...
    actor_delta = td_rms.normalize(td_error, actor_clip)
    return td_target, td_error, actor_delta
...
def main():
    ...
    for update in range(1, args.num_updates + 1):
        ...
        with torch.no_grad():
            ...
            # Preserve v3 TD-RMS modulation; the robust branch ignores an
            # invalid bootstrap at true terminals instead of evaluating 0 * NaN.
            (
                td_target,
                td_error,
                actor_delta,
