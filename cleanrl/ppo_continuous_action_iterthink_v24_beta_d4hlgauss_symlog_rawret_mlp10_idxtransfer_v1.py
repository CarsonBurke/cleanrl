# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic MLP10 idx-transfer v1.
#
...
#
# MLP10 idx-transfer variant: remove the DenseNet-style trunk concatenation,
# residual branch sum, x0 convex residual, and MoE. The actor/critic trunk is a
# plain depth-10, width-128 ReLU-squared MLP. Starting at hidden layer 3, each
# hidden perceptron also receives learned same-index transfer residuals from all
# earlier non-immediate hidden layers in its preactivation:
#   z_l[j] = W_l h_{l-1}[j] + b_l[j] + sum_{i=1}^{l-2} a_{l,i,j} h_i[j]
#
...
class TransferMLPTrunk(nn.Module):
    def __init__(self, in_dim, H, depth):
        ...
        for layer_idx in range(depth):
            layer_in_dim = in_dim if layer_idx == 0 else H
            history_slots = max(0, layer_idx - 1)
            self.layers.append(IndexedTransferLayer(layer_in_dim, H, history_slots))
