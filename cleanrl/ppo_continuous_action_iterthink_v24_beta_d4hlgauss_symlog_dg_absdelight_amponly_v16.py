# PPO + IterThink v24 beta d4-HL-Gauss critic + AMPLIFY-ONLY DELIGHT GATE v16.
#
# WHY v16. Two faithful delight-gate forms failed for orthogonal reasons:
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.dg_absdelight:
                    # v16: AMPLIFY-ONLY (non-suppressing) absolute delight gate.
                    # ell~ = (-logpi_theta) - H(s) is the realized within-state
                    # typicality (zero-mean per state since E[-logpi]=H): ell~>0
                    # a surprising draw, ell~<0 modal. std-normalize so eta is a
                    # clean trust width; clamp into chi so boundary samples cannot
                    # saturate the gate.
                    #
                    # WHY AMPLIFY-ONLY (the v15 fix): v15's symmetric gate
                    # 2*sigmoid(chi/eta) collapsed the policy to uniform. In
                    # dense-reward CC, good actions are TYPICAL (critic+policy
                    # co-adapt), so cov(U,ell~)<0: U>0 => ell~<0 => chi<0 =>
                    # w<0.5, i.e. the gate SUPPRESSED the confident-good gradient
                    # PPO rides. Measured: entropy->0 (max for Beta), KL->1e-6,
                    # actor_grad_norm 1.33->0.003 by 400k - a degenerate
                    # max-entropy fixed point. FIX: clamp the gate to >=1 so it
                    # can ONLY amplify, never cut below standard PG. Confident-good
                    # keeps full weight (equilibrium unreachable); good-SURPRISES
                    # (U>0,ell~>0) and typical-BAD (U<0,ell~<0) get a bounded bonus
                    # up to 2x. Self-annealing: an adopted good-surprise becomes
                    # typical -> bonus->0. No ratio, no clip; single actor loss;
                    # target_kl is the only drift backstop. relgate stores the
                    # post-clamp weight (>=1, mean>1) for telemetry.
