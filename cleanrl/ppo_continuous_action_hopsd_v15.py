# HOPSD v15 — temporally-ORDERED hindsight context: give the teacher gait phase.
# =====================================================================================
# The privileged context phi has been pooled mean/std of a_{t+1..t+20} — a permutation-
# invariant reduction that annihilates temporal order. Locomotion is periodic: WHERE in
# the stride the future sits (mid-swing vs about-to-plant) is precisely what decides
# whether a_t was good, and precisely what pooling destroys. Under a slow-moving policy
# the pooled stats are nearly a function of s_t itself (red-team: phi may be a placebo).
# v15 appends the ORDERED lag actions a_{t+2}, a_{t+4}, a_{t+8} (validity-masked, zeros
# where the window crosses a boundary) to BOTH teacher inputs (actor and critic —
# conditioning equalized; the cohex failure taught us not to split contexts). Lag 1 is
# deliberately EXCLUDED: a_{t+1} ~= a_t for a smooth policy, and feeding it makes the
# teacher a near-identity copier (the v1-style fixed point: mean_gap -> 0, no tilt).
# Tilt/dual/distill machinery unchanged from v12.2 (winsorize +-2sigma, KL-targeted).
# Read at 1.5M vs v12.2 control: if ordered phi carries improvement signal the pooled
# version cannot, matched-step returns should beat ~3100@750k / ~5600@1.5M; watch
# debug/teacher_student_mean_gap (should be LARGER than control — a sharper conditional
# prescribes bigger corrections) and losses/distill_kl. If gap -> 0 instead, the lags
# leaked enough of a_t to trigger the copier mode; drop lag 2, keep 4/8.
# =====================================================================================
#
...
class HindsightTeacher(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, args):
        super().__init__()
        H = args.hidden
        n_lags = len(ORDERED_LAGS)
        actor_in = obs_dim + (2 + n_lags) * act_dim + 1 + (1 if args.teacher_sees_g else 0)
        critic_in = obs_dim + (2 + n_lags) * act_dim + 1
...
ORDERED_LAGS = (2, 4, 8)  # v15: gait-phase lags; lag 1 excluded (copier risk)


def build_future_features(actions, boundaries, horizon):
    """Per-(t, env) mean/std of a_{t+1..t+H}, valid-horizon fraction, and ordered lags.
    ...
    Returns (mean, std, valid_frac, lags) with zeros where no valid future exists;
    lags is (T, B, len(ORDERED_LAGS)*A): the masked actions a_{t+k} for k in ORDERED_LAGS.
    """
    T, B, A = actions.shape
    ...
    lags = torch.cat(
        [lag_feats.get(k, torch.zeros_like(actions)) for k in ORDERED_LAGS], dim=-1
    )
    return mean, std, valid_frac, lags
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            fut_mean, fut_std, fut_valid_frac, fut_lags = build_future_features(
            ...
            actor_ctx = [obs, fut_mean, fut_std, fut_lags]
            ...
            teacher_critic_in = torch.cat(
                [obs, fut_mean, fut_std, fut_lags, fut_valid_frac.unsqueeze(-1)], dim=-1
            )
