"""PPO with Trading-Bot-Inspired Architecture — Learned Log-Std v2 (tbot_scalar_lstd)

v2 changes from v1 (HC 6771 last-50, 6958 best-R50, 7235 best-R10):

1. **Z-normalization after tanh**: Critical insight from lattice-tanh — tanh alone causes
   entropy instability (Walker2d entropy grew 5.3→6.5 in v3, causing collapse). Z-norm
   after tanh gives log_std_param a short gradient path for total magnitude control.
   Pipeline: sde_fc → RMSNorm → /prescale → tanh → z-norm

2. **Re-introduce shared W in noise path**: v1 lost policy-aligned exploration by using
   log_std_param independently of actor_out.weight. Now noise variance uses BOTH:
   action_var = (latent^2) @ (std^2 * W.T^2), coupling exploration to policy structure
   while retaining per-element learned magnitudes.

3. **Separate covariance trunk**: Lattice-tanh used separate mean/cov trunks to avoid
   gradient conflict. Now sde path has its own 2-layer trunk instead of sharing the
   actor backbone + single sde_fc layer.

v1 base (from trading_bot_0 commit 5b24cee4):
...
class Agent(nn.Module):
    ...
    def _get_action_std(self, x):
        """State-dependent action std via separate cov trunk + tanh + z-norm + shared W."""
        # Separate covariance trunk
        cov_h = F.silu(self.cov_norm1(self.cov_fc1(x)))
        cov_h = F.silu(self.cov_norm2(self.cov_fc2(cov_h)))

        # Soft-sign: bounded [-1, 1] but never fully saturates (unlike tanh)
        sde_latent = cov_h / (cov_h.abs() + 1.0)  # (batch, hidden_dim)
        ...
        # SDE variance: sum over hidden dim of (latent^2 * combined_weights)
        action_var = (sde_latent.pow(2)) @ noise_weights  # (batch, act_dim)
