# Critic-first HL-Gauss on the exact 50M-step, 32x-LR sphere PPO baseline.
# Source: decoupled_v5; HL-Gauss: arxiv.org/abs/2403.03950, Appendix A.
# Fit fixed old-lambda labels, then refresh GAE before any actor update.
# Hypothesis: refreshed credit assignment reduces categorical critic lag.
# Actor percentile scaling follows refresh; critic labels never change.
import os
import random
import time
...
def make_optimizers(agent, args):
    """Disjoint optimizers keep the behavior actor fixed during critic fitting."""
    actor_optimizer = optim.Adam(agent.actor.parameters(), lr=args.learning_rate, eps=1e-5, fused=True)
    critic_optimizer = optim.Adam(agent.critic.parameters(), lr=args.critic_learning_rate, eps=1e-5, fused=True)
...
def actor_loss(agent, observations, native_actions, old_logprobs, advantages, args):
    """Native-action PPO using immutable behavior likelihoods and refreshed GAE."""
...
def critic_loss(agent, observations, target_probs, args):
    """Raw Gaussian cross entropy against labels frozen before critic fitting."""
    logits = agent.critic(observations)
    cross_entropy = -(target_probs.detach() * logits.log_softmax(dim=-1)).sum(dim=-1).mean()
    return args.vf_coef * cross_entropy, cross_entropy.detach()
...
def main():
    ...
    try:
        ...
        actor_optimizer, critic_optimizer = make_optimizers(agent, args)
        ...
        def critic_loss_model(observations, target_probs):
            return critic_loss(agent, observations, target_probs, args)
        ...
        critic_losses = torch.empty(max_updates, device=device)
        actor_grad_norms = torch.empty(max_updates, device=device)
        critic_grad_norms = torch.empty(max_updates, device=device)
        actor_parameters = actor_optimizer.param_groups[0]["params"]
        critic_parameters = critic_optimizer.param_groups[0]["params"]
        timer = PhaseTimer()
        ...
        for iteration in range(1, args.num_iterations + 1):
            if args.anneal_lr:
                ...
                critic_optimizer.param_groups[0]["lr"] = fraction * args.critic_learning_rate
            bootstraps.reset()
            host_actor.refresh()
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_old_advantages = advantages.flatten().detach().clone()
                b_returns = returns.flatten().detach().clone()
                del advantages, returns
                # Neither inference compiler uses CUDA graph trees, so these labels
                # own their storage; only the replayed GAE outputs need snapshots.
                b_target_probs = target_model(b_returns)
                projection_error = agent.value_support.probs_to_scalar(b_target_probs) - b_returns
                precritic_error = b_values - b_returns
                value_diagnostics = {
                    ...
                    "hlgauss/preupdate_decoded_value_mse": precritic_error.square().mean(),
                    "hlgauss/precritic_fixed_target_bias": precritic_error.mean(),
                    "hlgauss/precritic_fixed_target_rmse": precritic_error.square().mean().sqrt(),
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    epoch_start = critic_updates
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        critic_optimizer.zero_grad(set_to_none=True)
                        if args.compile:
                            torch.compiler.cudagraph_mark_step_begin()
                        loss, cross_entropy = critic_loss_model(b_obs[indices], b_target_probs[indices])
                        # Copy graph-owned metrics before backward or the next forward.
                        critic_losses[critic_updates].copy_(cross_entropy)
                        loss.backward()
                        critic_grad_norms[critic_updates].copy_(
                            nn.utils.clip_grad_norm_(critic_parameters, args.max_grad_norm).detach()
                        )
                        critic_optimizer.step()
                        critic_updates += 1
                    epoch_gradients[f"grad/critic_preclip_norm_epoch_{epoch + 1}"] = critic_grad_norms[
                        epoch_start:critic_updates
                    ].mean()

            with timer.span("gae"), torch.no_grad():
                # Reevaluate every bootstrap using the fitted critic, including time
                # limits and the rollout tail; the cached final observations persist.
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                b_refreshed_values = value_model(b_obs).flatten()
                tail_value = value_model(next_obs).flatten()
                truncation_values = bootstraps.resolve(value_model, device)
                advantages, returns = gae_fn(
                    batch.rewards,
                    b_refreshed_values.view(args.num_steps, args.num_envs),
                    batch.terminations,
                    batch.truncations,
                    truncation_values,
                    tail_value,
                ...
                b_refreshed_returns = returns.flatten().detach().clone()
                del advantages, returns
                advantage_drift = b_advantages - b_old_advantages
                postcritic_error = b_refreshed_values - b_returns
                credit_diagnostics = {
                    "actor/old_advantage_mean": b_old_advantages.mean(),
                    "actor/old_advantage_rms": b_old_advantages.square().mean().sqrt(),
                    "actor/refreshed_advantage_mean": b_advantages.mean(),
                    "actor/refreshed_advantage_rms": b_advantages.square().mean().sqrt(),
                    "actor/advantage_drift_mean": advantage_drift.mean(),
                    "actor/advantage_drift_rms": advantage_drift.square().mean().sqrt(),
                    "hlgauss/refreshed_return_mean": b_refreshed_returns.mean(),
                    "hlgauss/refreshed_return_std": b_refreshed_returns.std(unbiased=False),
                    "hlgauss/postcritic_fixed_target_bias": postcritic_error.mean(),
                    "hlgauss/postcritic_fixed_target_rmse": postcritic_error.square().mean().sqrt(),
                    "hlgauss/refreshed_explained_variance": explained_variance(b_refreshed_values, b_refreshed_returns),
                }
                # Only refreshed returns update the actor's percentile EMA. Its
                # denominator scales refreshed advantages, never critic labels.
                actor_scale = (
                    actor_scale_model(b_refreshed_returns)
                    if actor_scale_model is not None
                    else b_refreshed_returns.new_ones(())
                )
                unscaled_advantage_rms = credit_diagnostics["actor/refreshed_advantage_rms"]
                b_advantages.div_(actor_scale)

            actor_updates = 0
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    epoch_start = actor_updates
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        update_metrics[actor_updates].copy_(metrics)
                        loss.backward()
                        actor_grad_norms[actor_updates].copy_(
                            nn.utils.clip_grad_norm_(actor_parameters, args.max_grad_norm).detach()
                        )
                        actor_optimizer.step()
                        actor_updates += 1
                    epoch_gradients[f"grad/actor_preclip_norm_epoch_{epoch + 1}"] = actor_grad_norms[
                        epoch_start:actor_updates
                    ].mean()
                    # Last-minibatch KL is checked only after a complete actor epoch.
                    if args.target_kl is not None and update_metrics[actor_updates - 1, 3] > args.target_kl:
                        break

            last = update_metrics[actor_updates - 1]
            logged = gather_metrics(
                {
                    "losses/policy_loss": last[0],
                    "losses/value_loss": critic_losses[critic_updates - 1],
                    "losses/entropy": last[1],
                    "losses/old_approx_kl": last[2],
                    "losses/approx_kl": last[3],
                    "losses/clipfrac": update_metrics[:actor_updates, 4].mean(),
                    "losses/explained_variance": explained_variance(b_values, b_returns),
                    "grad/actor_preclip_norm": actor_grad_norms[:actor_updates].mean(),
                    "grad/critic_preclip_norm": critic_grad_norms[:critic_updates].mean(),
                    ...
                    "hlgauss/postupdate_value_bias": postcritic_error.mean(),
                    "hlgauss/postupdate_value_rmse": postcritic_error.square().mean().sqrt(),
                    **value_diagnostics,
                    **credit_diagnostics,
                    **epoch_gradients,
                }
            ...
            writer.add_scalar("charts/critic_learning_rate", critic_optimizer.param_groups[0]["lr"], global_step)
            writer.add_scalar("charts/actor_updates", actor_updates, global_step)
            writer.add_scalar("charts/critic_updates", critic_updates, global_step)
