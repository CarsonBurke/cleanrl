# Peri-RMS scalar PPO, v12: absolute physical-density Delightful truegate.
# Starting from no-final-norm v8, gate only raw policy GAE once per rollout.
# Keep branch norms, residual gates, readout scaling, normalized rewards,
# scalar clipped MSE, LR 0.0096 and no gradient clipping; default one epoch.
import os
import random
import time
...
class Args:
    ...
    norm_adv: bool = False
    """gate-weighted raw GAE in the surrogate; no minibatch standardization"""
    clip_coef: float = 0.2
    ...
    dg_eta: float = 1.0
    """fixed truegate temperature eta in w = sigmoid(raw_GAE * surprisal / eta)"""
    dg_surprisal_clip: float = 10.0
    """symmetric clip bound on negative old physical log-density"""
    reward_norm: bool = True
    """normalize/clip rewards before GAE; never normalize GAE return targets"""

    placement: Literal["pre", "post", "peri"] = "peri"
    """peri normalizes branch inputs and outputs before identity residual addition"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, placement="peri", norm_kind="rms", activation="stiglu"):
        super().__init__()
...
def truegate_weights(advantages, physical_logprobs, eta, surprisal_clip):
    """Fixed old-policy gate; physical log-density already includes the Jacobian."""
...
def main():
    ...
    try:
        ...
        writer.add_text(
            ...
            f"clip_heads={args.clip_heads}; 1 minibatch; absolute physical-density truegate; "
            f"gated raw GAE; eta={args.dg_eta}; surprisal_clip={args.dg_surprisal_clip}",
        )
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                # Old physical log-density already includes the action-scale Jacobian.
                # Freeze the gate before any optimizer step; value targets stay ungated.
            ...
            logged = gather_metrics(
                {
                    ...
                    "debug/dg_w_mean": dg_w.mean(),
                    "debug/dg_w_std": dg_w.std(unbiased=False),
