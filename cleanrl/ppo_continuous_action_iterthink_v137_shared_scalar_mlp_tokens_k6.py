# PPO + IterThink v137 shared-scalar-mlp-tokens-k6 (v136 without scalar feature expansion).
#
# Key change from v133:
#   - Remove Gym's stateful NormalizeObservation/clip wrappers.
#   - Encode each raw observation scalar with a shared tokenizer:
#       z_i = symlog(raw_i)
#       h_i = Linear_shared(z_i) -> per-token RMSNorm -> Linear_shared -> ReLU^2
#       token_i = Linear_shared(h_i) + learned_feature_embed_i
#     This keeps tokenization scalable to many observations while preserving
#     feature identity through learned slot embeddings.
#   - The rolling prompt bank now stores raw env observations directly; no stale
#     normalized coordinates and no collection/current RMS mismatch.
#
# Hypothesis:
#   A shared symlog scalar tokenizer provides bounded, reusable per-sensor
#   encoding without hand-written scalar features, unscalable per-feature weights,
#   or the full dense remapping of v135. The hidden linear after RMSNorm lets
#   magnitude become direction before the nonlinearity/final token projection.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # Fixed raw-observation tokenizer. symlog bounds MuJoCo velocities without
        # a running normalizer. The scalar MLP is shared across obs dimensions for
        # scalability; learned slot embeddings provide feature identity.
        ...
        self.obs_scalar_in = xavier_linear(nn.Linear(1, self.dim))
        self.obs_scalar_norm = nn.RMSNorm(self.dim)
        self.obs_scalar_hidden = xavier_linear(nn.Linear(self.dim, self.dim))
        self.obs_scalar_out = xavier_linear(nn.Linear(self.dim, self.dim))
        self.obs_slot_embed = nn.Parameter(torch.empty(self.num_obs_tokens, self.dim))
    ...
    def encode_summary(self, obs):
        ...
        z = symlog(obs_flat)
        obs_tokens = self.obs_scalar_in(z.unsqueeze(-1))
        obs_tokens = self.obs_scalar_norm(obs_tokens)
        obs_tokens = self.obs_scalar_hidden(obs_tokens)
        obs_tokens = self.obs_scalar_out(relu_sq(obs_tokens))
        obs_tokens = obs_tokens + self.obs_slot_embed.to(obs_tokens.dtype).unsqueeze(0)
