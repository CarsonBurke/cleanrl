# PPO + IterThink v24.8 (PROTECTED FULL-COVARIANCE TANH NORMAL). From v24.7.
#
# v24.3-v24.5 showed that SAC-Beta's shifted-log concentration family is not the
# missing mechanism: widening the cap lets the policy sharpen, but returns still
# lag v24. The stronger hypothesis is that v24's remaining ceiling comes from
# independent per-actuator exploration. HalfCheetah/Walker improvements often
# require coordinated phase/action changes; a factorized Beta can represent a
# final action vector, but its sampled search directions remain axis-aligned.
#
# v24.7 recovered from the v24.6 over-broad init, but its covariance gradients
# dominated the actor update (grad norms 50-110, KL spikes >0.1, large clipfrac)
# and the run lagged v24 badly. v24.8 keeps full covariance as a coordinated
# exploration side channel, but protects mean/trunk learning:
#   - covariance heads read detached actor features, so covariance likelihood
#     gradients do not reshape the shared actor representation;
#   - covariance heads use a lower optimizer LR;
#   - covariance heads are clipped separately with a smaller norm budget.
#
# The default actor remains:
#     z ~ N(mu(s), L(s)L(s)^T), action = tanh(z)
#     log_prob = log N(z; mu, LL^T) - sum_i log |d tanh(z_i)/dz_i|
# The Cholesky diagonal is a SAC-style bounded log_std head initialized at std≈0.5.
# Off-diagonal Cholesky entries are row-std-scaled tanh heads, so correlation is
# learnable but cannot explode early.
#
# --- inherited v24 notes ---
#
...
#
#   actor_dist="fullcov"  (DEFAULT, coordinated exploration path):
#       state-dependent full-covariance Normal in latent z-space, tanh-squashed
#       to action bounds with the exact squash-Jacobian correction. Full covariance
#       is cheap for MuJoCo action dims and directly tests whether independent
#       action noise is v24's architectural ceiling.
#
#   actor_dist="beta"  (v24 control, the "performs much better" path):
...
#       bounds do not.
#
# PARITY NOTES (all dists): the rollout buffers the distribution-NATIVE sample
# (latent_zs) — pre-tanh z for gaussian/fullcov, z in (0,1) for beta — and replays it on
...
class Args:
    ...
    # v24.8: action distribution. "fullcov" (protected state-dependent full-cov tanh Normal)
    # | "beta" (v24 unimodal control) | "gaussian" (diagonal dreamer4 control).
    # (dreamer4 state-dependent log-VARIANCE head, not SAC log_std; soft tanh-rescale bound).
    actor_dist: str = "fullcov"
    ...
    fullcov_log_std_min: float = -4.0
    fullcov_log_std_max: float = 2.0
    fullcov_log_std_init: float = -0.7
    fullcov_offdiag_scale: float = 0.25
    fullcov_detach_cov: bool = True
    fullcov_cov_lr_mult: float = 0.25
    fullcov_cov_grad_clip: float = 0.03
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v24.8: protected full-covariance tanh Normal default, with v24 beta/diagonal controls.
        ...
        elif self.actor_dist == "fullcov":
            self.actor_head = layer_init(nn.Linear(H, act_dim), std=0.01)
            if not (args.fullcov_log_std_min < args.fullcov_log_std_init < args.fullcov_log_std_max):
                raise ValueError(
                    "fullcov_log_std_init must lie inside "
                    f"({args.fullcov_log_std_min}, {args.fullcov_log_std_max}); "
                    f"got {args.fullcov_log_std_init}"
                )
            raw_logstd_init = float(
                np.arctanh(
                    rescale(
                        args.fullcov_log_std_init,
                        (args.fullcov_log_std_min, args.fullcov_log_std_max),
                        (-1.0, 1.0),
                    )
                )
            )
            self.actor_logstd_head = layer_init(nn.Linear(H, act_dim), std=0.01, bias_const=raw_logstd_init)
            self.actor_tril_head = layer_init(nn.Linear(H, act_dim * (act_dim - 1) // 2), std=0.01)
            self.fullcov_log_std_min = args.fullcov_log_std_min
            self.fullcov_log_std_max = args.fullcov_log_std_max
            self.fullcov_offdiag_scale = args.fullcov_offdiag_scale
            self.fullcov_detach_cov = args.fullcov_detach_cov
            tril_rows, tril_cols = torch.tril_indices(act_dim, act_dim, offset=-1)
            self.register_buffer("fullcov_tril_rows", tril_rows)
            self.register_buffer("fullcov_tril_cols", tril_cols)
    ...
    def _actor_dist(self, actor_feat):
        ...
        if self.actor_dist == "fullcov":
            mean = self.actor_head(actor_feat)
            cov_feat = actor_feat.detach() if self.fullcov_detach_cov else actor_feat
            raw_logstd = self.actor_logstd_head(cov_feat)
            log_std = rescale(
                raw_logstd.tanh(),
                (-1.0, 1.0),
                (self.fullcov_log_std_min, self.fullcov_log_std_max),
            )
            std = log_std.exp()
            scale_tril = torch.diag_embed(std)
            raw_offdiag = self.actor_tril_head(cov_feat)
            offdiag = raw_offdiag.tanh() * self.fullcov_offdiag_scale
            scale_tril[:, self.fullcov_tril_rows, self.fullcov_tril_cols] = (
                std[:, self.fullcov_tril_rows] * offdiag
            )
            dist = MultivariateNormal(mean, scale_tril=scale_tril, validate_args=False)
            to_action = torch.tanh
            log_det_fn = lambda z: 2.0 * (log(2.0) - z - F.softplus(-2.0 * z))
            return dist, to_action, log_det_fn
        # beta
    ...
    def get_fullcov_stats(self, x):
        if self.actor_dist != "fullcov":
            raise RuntimeError("get_fullcov_stats is only valid for fullcov actor_dist")
        ...
        marginal_std = cov.diagonal(dim1=-2, dim2=-1).clamp_min(1e-12).sqrt()
        corr = cov / (marginal_std.unsqueeze(-1) * marginal_std.unsqueeze(-2)).clamp_min(1e-12)
        corr_offdiag = corr[:, self.fullcov_tril_rows, self.fullcov_tril_cols]
        return log_std, offdiag, marginal_std, corr_offdiag
    ...
    def get_action_and_value(self, x, z=None):
        # z is the distribution-NATIVE sample (pre-tanh for gaussian/fullcov; in (0,1) for
        ...
        squash_log_det = log_det_fn(z)
        if base_log_prob.ndim == 1:
            log_prob = base_log_prob - squash_log_det.sum(1)
        else:
            log_prob = (base_log_prob - squash_log_det).sum(1)
        value_logits = self.critic_head(critic_feat)
        if self.actor_dist in ("gaussian", "fullcov"):
            ...
            zr = dist.rsample()
            zr_log_prob = dist.log_prob(zr)
            zr_log_det = log_det_fn(zr)
            if zr_log_prob.ndim == 1:
                entropy = (zr_log_prob - zr_log_det.sum(1)).neg()
            else:
                entropy = (zr_log_prob - zr_log_det).sum(1).neg()
        else:
    ...
    def actor_parameters(self):
        ...
        # decoupled clip; no separate std budget — gaussian/fullcov scale heads and
        ...
        if self.actor_dist == "gaussian":
            ...
            heads = list(self.actor_head.parameters()) + self.actor_cov_parameters()
        else:
            heads = list(self.actor_alpha_head.parameters()) + list(self.actor_beta_head.parameters())
        return list(trunk.parameters()) + heads

    def actor_core_parameters(self):
        trunk = self.trunk if self.share_backbone else self.actor_trunk
        if self.actor_dist == "fullcov":
            return list(trunk.parameters()) + list(self.actor_head.parameters())
        return self.actor_parameters()

    def actor_cov_parameters(self):
        if self.actor_dist != "fullcov":
            return []
        return list(self.actor_logstd_head.parameters()) + list(self.actor_tril_head.parameters())
...
if __name__ == "__main__":
    ...
    cov_params = agent.actor_cov_parameters()
    if args.actor_dist == "fullcov" and cov_params:
        cov_param_ids = {id(p) for p in cov_params}
        base_params = [p for p in agent.parameters() if id(p) not in cov_param_ids]
        optimizer = optim.Adam(
            [
                {"params": base_params, "lr": args.learning_rate},
                {"params": cov_params, "lr": args.learning_rate * args.fullcov_cov_lr_mult},
            ],
            eps=1e-5,
        )
    else:
    ...
    actor_params = agent.actor_core_parameters()
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            if len(optimizer.param_groups) > 1:
                optimizer.param_groups[1]["lr"] = lrnow * args.fullcov_cov_lr_mult
        ...
        cov_gn = torch.tensor(0.0, device=device)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    if args.actor_dist == "fullcov" and cov_params:
                        cov_gn = nn.utils.clip_grad_norm_(cov_params, args.fullcov_cov_grad_clip)
                    ...
                    loss.backward()
                    if args.actor_dist == "fullcov" and cov_params:
                        cov_grad_norms = [p.grad.detach().norm(2) for p in cov_params if p.grad is not None]
                        cov_gn = (
                            torch.norm(torch.stack(cov_grad_norms), 2)
                            if cov_grad_norms
        ...
        if args.actor_dist == "fullcov" and len(optimizer.param_groups) > 1:
            writer.add_scalar("charts/fullcov_cov_learning_rate", optimizer.param_groups[1]["lr"], global_step)
        ...
        if args.actor_dist == "fullcov":
            writer.add_scalar("losses/fullcov_cov_grad_norm", float(cov_gn), global_step)
        ...
        if args.actor_dist == "fullcov":
            with torch.no_grad():
                log_std, offdiag, marginal_std, corr_offdiag = agent.get_fullcov_stats(b_obs)
                writer.add_scalar("debug/fullcov_log_std_mean", log_std.mean().item(), global_step)
                writer.add_scalar("debug/fullcov_log_std_min", log_std.min().item(), global_step)
                writer.add_scalar("debug/fullcov_log_std_max", log_std.max().item(), global_step)
                writer.add_scalar("debug/fullcov_cholesky_diag_mean", log_std.exp().mean().item(), global_step)
                writer.add_scalar("debug/fullcov_cholesky_offdiag_abs_mean", offdiag.abs().mean().item(), global_step)
                writer.add_scalar("debug/fullcov_cholesky_offdiag_abs_max", offdiag.abs().max().item(), global_step)
                writer.add_scalar("debug/fullcov_marginal_std_mean", marginal_std.mean().item(), global_step)
                writer.add_scalar("debug/fullcov_marginal_std_max", marginal_std.max().item(), global_step)
                writer.add_scalar("debug/fullcov_corr_abs_mean", corr_offdiag.abs().mean().item(), global_step)
                writer.add_scalar("debug/fullcov_corr_abs_max", corr_offdiag.abs().max().item(), global_step)
