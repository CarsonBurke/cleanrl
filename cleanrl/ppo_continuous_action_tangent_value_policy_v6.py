# Fresh tangent-value optimizer with selectable PPO actor, v6.
# Hypothesis: a per-minibatch value JVP gives a useful Gauss-Newton step without
# inherited LR collapse or noisy subtraction of aggregate critic losses.
# Adam supplies a direction; six fresh ray trials compete with the origin by
# paired per-example value gains. No actor/critic grad clipping or value clipping.
# Actor: unchanged v5 analytic-KL tangent policy, or scheduled baseline Adam PPO.
# Both arms use the same rollout values, log-probabilities and frozen GAE targets.
import os
import random
import time
