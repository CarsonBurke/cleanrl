# SPO-asym + transformer + HL-Gauss + SA latent tokens + fp32 (no autocast).
#
# Diff vs satransformer_pma_spo_asym_hlgauss_latent_v1: drops the bf16
# autocast on the encoder forward (line 471-477 of v1). Everything else
# identical (same SA + PMA + heads + state-dep std-head).
#
# Hypothesis. The transformer (89k params, ~8x MLP) only beats the MLP
# (5028) by ~2% on HC. fixedstd_v1 confirmed state-dep std is helping
# (removing it dropped to 3265 at 4M vs v1's 3751). So the bottleneck
# is elsewhere. bf16 has 7-bit mantissa (~1% relative error per matmul);
# after SA + PMA depth ~4, accumulated noise on mean/std/value_logits
# could be 2-4%. fp32 throughout removes this noise. If precision is the
# limiter, expect a clear bump above 5138. If not, look at deeper
# architecture (heads, tokenizer, SA depth).
#
...
class Agent(nn.Module):
    ...
    def _encoder_with_autocast(self, x):
        # fp32_v1 variant: NO bf16 autocast. Precision-loss test.
