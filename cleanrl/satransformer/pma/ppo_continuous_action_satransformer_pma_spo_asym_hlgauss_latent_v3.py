# SPO-asym + transformer + HL-Gauss + SA latent tokens + separate PMA queries.
#
# v3 vs v2: fixes a round-trip-bias bug introduced in v2.
#
# v2 stored post-squash env_action and recovered z via atanh on replay.
# atanh(tanh(z)) is not bit-exact in fp32 (~1-2 ULP drift), and with
# bf16 autocast on the encoder (~1% noise on mean/std) plus a state-
# dependent SDE head whose std reaches ~3, the round-trip bias
# accumulated into a small but persistent ~5-10% score regression vs
# v1 over 5M+ steps. (KL/SPO-penalty/EV/std-distribution all looked
# identical to v1, ruling out trust-region or critic explanations.)
#
# v3 reverts storage/replay to v1's pre-squash z (exact, no round-trip),
# but keeps v2's other "idiomatic" cleanups:
#   - drops gym.wrappers.ClipAction (tanh already bounds u ∈ [-1, 1]).
#   - adds an affine [-1, 1] → [low, high] action map for general Box
#     action spaces (identity for HC; meaningful for other envs).
#   - keeps the simple Jacobian form log(1 - u² + ε) — equivalent to
#     the softplus form for non-saturated z, and with z stored exactly
#     there is no clamp-on-replay needed.
#
...
#
# Distribution: tanh-squashed Normal(mean, exp(t) + STD_MIN), with affine map
# from u = tanh(z) ∈ [-1, 1] to the env action range [low, high].
#   z      ~ Normal(mean, std)
#   u      = tanh(z)
#   action = center + half_range * u         (affine; for HC center=0, range=1)
#   log_prob(action) = Normal.log_prob(z) - sum(log(1 - u^2 + SQUASH_EPS))
#                                            (Jacobian for tanh squash; the
#                                             affine half_range is a constant
#                                             scale that cancels in the PPO
#                                             ratio, so we omit it here.)
#
...
#
# Rollout storage: pre-squash sample z (exact). On replay we feed z back
# directly — no atanh(tanh(z)) round-trip, so no fp32 / bf16-noise drift in
# the PPO ratio. ClipAction wrapper is dropped — tanh already bounds u.
#
...
SQUASH_EPS = 1e-6  # numerical floor for log(1 - u²) Jacobian term
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        # Affine map [-1, 1] (tanh range) → [low, high] (env action range).
    ...
    def get_action_and_value(self, x, z=None):
        """Tanh-squashed PPO with pre-squash z storage.

        If `z` is None: sample z ~ Normal(mean, std); return
            env_action = affine(tanh(z)) along with z, log_prob, entropy, value.
        Else: treat `z` as the stored pre-squash sample (exact, no round-trip)
            and recompute log_prob under the current policy.

        Storing z exactly avoids the atanh(tanh(z)) round-trip bias that
        accumulates with bf16-noisy mean/std (see v3 header).
        """
        ...
        u = torch.tanh(z)
        env_action = self._u_to_action(u)

        # Jacobian for tanh squash: log|d tanh/dz| summed over action dims.
        # The constant affine half_range cancels in the PPO ratio, omit.
        squash_correction = torch.log(1.0 - u.pow(2) + SQUASH_EPS).sum(-1)
        log_prob = dist.log_prob(z).sum(-1) - squash_correction
...
if __name__ == "__main__":
    ...
    # `actions` stores the pre-squash sample z (exact, no atanh round-trip).
    # tanh(z) → affine([low, high]) is what the env sees; z is what we replay.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            actions[step] = z  # store pre-squash z (exact; used for replay)
