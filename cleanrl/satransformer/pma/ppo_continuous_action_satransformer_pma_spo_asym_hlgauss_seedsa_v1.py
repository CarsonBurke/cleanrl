# SPO-asym + shared SA→PMA transformer + HL-Gauss + seeds-in-SA.
#
# Sibling of satransformer_pma_spo_asym_hlgauss_v1; difference is that the
# 3 readout seeds (actor / sde / critic) are concatenated to the obs tokens
# *before* the SA block, so they participate in full self-attention with
# the obs tokens (bidirectional mixing). The PMA block then cross-attends
# the seeds (taken from the SA output) over the obs tokens (also from the
# SA output) for a focused readout.
#
# Token layout entering SA: [seed_actor, seed_sde, seed_critic, obs_0, ..., obs_{D-1}]
# RoPE: identity (cos=1, sin=0) on seed positions, standard RoPE freqs on
#       obs positions. Same trick as cleanrl/hlgauss/transformer/...sa_transformer_base_v1.py.
#
...
def build_rope_cache(num_obs_positions, head_dim, device, num_special_tokens=0, base=10000.0):
    ...
    the standard RoPE freqs starting from frequency-of-position-0."""
    ...
    freqs = torch.outer(positions, theta)  # (num_obs_positions, head_dim//2)
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        # RoPE cache for SA over [3 seeds | obs_dim obs tokens]. Seed
        # positions (0..2) get identity rotation; obs positions (3..3+obs_dim-1)
        # get the standard RoPE frequencies starting from index 0.
        head_dim = MODEL_DIM // NUM_HEADS
        rope_cos, rope_sin = build_rope_cache(
            self.obs_dim, head_dim, torch.device("cpu"), num_special_tokens=3,
        )
        self.register_buffer("rope_cos", rope_cos)
        self.register_buffer("rope_sin", rope_sin)

    def _encode(self, x):
        """Tokenize → concat[seeds, obs] → embed_norm → SA → split → PMA →
        final_norm. Returns the three seed features (actor, sde, critic)
        each shape (B, MODEL_DIM)."""
        B = x.shape[0]
        # (B, obs_dim) → (B, obs_dim, MODEL_DIM) via per-dim affine.
        obs_tokens = x.unsqueeze(-1) * self.obs_embed_w + self.obs_embed_b
        seeds_init = self.seeds.unsqueeze(0).expand(B, -1, -1)  # (B, 3, D)
        # Seeds participate in SA (bidirectional mixing with obs).
        all_tokens = torch.cat([seeds_init, obs_tokens], dim=1)  # (B, 3+obs_dim, D)
        all_tokens = self.embed_norm(all_tokens)
        all_tokens = self.sa_block(all_tokens, self.rope_cos, self.rope_sin)

        seeds_after_sa = all_tokens[:, :3]   # (B, 3, D)
        obs_after_sa = all_tokens[:, 3:]     # (B, obs_dim, D)
        # PMA: refined seed readout via cross-attention over the obs portion.
        seeds = self.pma_block(seeds_after_sa, obs_after_sa)
