# nGPT hypersphere satransformer v8-actioncritic: v7 decoupled role-token
# encoder/decoders with current-action-conditioned critic head.
#
# Diff vs v7: project the current env action into the critic head input. During
# rollout and PPO replay, the critic conditions on the sampled/replayed action.
# For bootstrap-only value calls, it conditions on the current policy's
# deterministic action (Beta mean or squashed Gaussian mean). This tests whether
# giving the value learner the actuator choice improves credit assignment,
# while keeping the v7 token backbone and rankgauss learner unchanged.
#
...
CRITIC_HEAD_DIM = MODEL_DIM * (CRITIC_READOUT_TOKENS + 1)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.critic_action_encoder = nn.Linear(self.action_dim, MODEL_DIM, bias=False)
        init_linear_(self.critic_action_encoder, init_scale=1.0)
    ...
    def normalize_weights(self):
        ...
        self.critic_action_encoder.weight.data.copy_(
            justnorm(self.critic_action_encoder.weight.data, dim=1)
        )
    ...
    def critic_parameters(self):
        ...
        params += list(self.critic_action_encoder.parameters())
    ...
    def _dist_deterministic_action(self, dist):
        if self.actor_dist == "gaussian":
            return torch.tanh(dist.mean)
        z_mean = dist.concentration1 / (dist.concentration1 + dist.concentration0)
        return self.action_low + (self.action_high - self.action_low) * z_mean

    def _critic_logits(self, critic_feat, env_action):
        action_feat = justnorm(self.critic_action_encoder(env_action)) * (MODEL_DIM ** 0.5)
        critic_feat = torch.cat([critic_feat, action_feat], dim=-1)
        return self.critic_head(critic_feat)

    def get_value_logits(self, x, env_action=None):
        actor_feat, critic_feat = self._encoder_with_autocast(x)
        if env_action is None:
            dist, _, _ = self._actor_dist(actor_feat)
            env_action = self._dist_deterministic_action(dist)
        return self._critic_logits(critic_feat, env_action)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        value_logits = self._critic_logits(critic_feat, env_action)  # (B, num_bins)
