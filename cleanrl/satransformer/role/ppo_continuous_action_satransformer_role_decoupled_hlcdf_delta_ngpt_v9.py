# nGPT hypersphere satransformer v9-hlcdf-delta: v7 decoupled role-token
# encoder/decoders with baseline-centered critic-CDF policy advantages.
#
...
#
# Diff vs v8-HLCDF: center the CDF advantage on the critic baseline instead of
# the critic median. For each sample, compute z_ret = probit(F(return)) and
# z_base = probit(F(old_value)) under the same HL-Gauss distribution, then use
# tanh((z_ret - z_base) / kappa). This preserves distribution-aware scaling but
# makes zero policy advantage match PPO's baseline. Minibatches use fixed batch
# target RMS scaling (default 0.7), not per-minibatch unit RMS, so uncertainty
# is not immediately inflated away. No target-KL stopping.
#
...
class Args:
    ...
    adv_rms_norm: bool = False
    """sign-preserving post-shaping RMS scaling for policy advantages"""
    adv_transform: str = "hlcdf_delta"
    """policy advantage shaping: 'hlcdf_delta', 'hlcdf_delta_signmag', 'hlcdf_tanh', 'hlcdf_probit', 'rankgauss', or 'v10'"""
    cdf_probit_clamp: float = 0.995
    """CDF/probit clamp before erfinv"""
    hlcdf_tanh_kappa: float = 1.5
    """temperature for tanh(CDF-probit delta / kappa)"""
    hlcdf_target_rms: float = 0.7
    """fixed batch RMS target for HLCDF advantages; <=0 disables batch scaling"""
    clip_coef: float = 0.2
...
def hl_gauss_cdf_values(logits, targets, hl_support, args):
    """Piecewise-uniform CDF and probit for scalar targets under HL-Gauss logits."""
    probs = torch.softmax(logits, dim=-1)
    targets = symlog(targets) if hl_support.use_symlog else targets
    ...
    z = ((2.0 ** 0.5) * torch.erfinv(centered.cpu())).to(targets.device)
    return u, z


def hl_gauss_cdf_advantage(logits, returns, baselines, raw_gae, hl_support, args):
    """Distribution-aware policy advantage from critic CDF coordinates.

    Delta modes compare the lambda-return CDF coordinate to the old scalar
    baseline CDF coordinate under the same critic distribution. That fixes the
    v8 issue where zero advantage meant "return equals critic median" rather
    than "return equals PPO baseline".
    """
    u, z = hl_gauss_cdf_values(logits, returns, hl_support, args)
    u_base, z_base = hl_gauss_cdf_values(logits, baselines, hl_support, args)
    z_delta = z - z_base

    if args.adv_transform == "hlcdf_delta":
        adv = torch.tanh(z_delta / args.hlcdf_tanh_kappa)
    elif args.adv_transform == "hlcdf_delta_signmag":
        adv = torch.sign(raw_gae) * torch.tanh(z_delta.abs() / args.hlcdf_tanh_kappa)
    elif args.adv_transform == "hlcdf_probit":
        adv = z
    elif args.adv_transform == "hlcdf_tanh":
        adv = torch.tanh(z / args.hlcdf_tanh_kappa)
    else:
        raise ValueError(f"unknown adv_transform {args.adv_transform}")
    return adv, u, z_delta, u_base, z_base
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        b_hlcdf_u_base = None
        b_hlcdf_z_base = None
        if args.adv_transform in ("hlcdf_delta", "hlcdf_delta_signmag", "hlcdf_tanh", "hlcdf_probit"):
            with torch.no_grad():
                value_logits_chunks = []
        ...
        is_hlcdf_delta_mode = args.adv_transform in ("hlcdf_delta", "hlcdf_delta_signmag")
        hlcdf_adv_pre_scale = b_policy_advantages
        if args.hlcdf_target_rms > 0 and is_hlcdf_delta_mode:
            adv_rms = b_policy_advantages.square().mean().sqrt().clamp_min(1e-8)
            b_policy_advantages = b_policy_advantages * (args.hlcdf_target_rms / adv_rms)
        if (not is_hlcdf_delta_mode) and args.norm_adv and args.norm_adv_scope == "batch":
            b_policy_advantages = (
        ...
        with torch.no_grad():
            ...
            adv_pre_scale_mean_abs = hlcdf_adv_pre_scale.abs().mean()
            adv_pre_scale_saturation = (hlcdf_adv_pre_scale.abs() > 0.95).float().mean()
            ...
            if b_hlcdf_u is not None:
                ...
                hlcdf_u_base_mean = b_hlcdf_u_base.mean()
                hlcdf_u_base_std = b_hlcdf_u_base.std()
                hlcdf_z_base_abs = b_hlcdf_z_base.abs().mean()
                hlcdf_z_delta_sign_agree = (
                    torch.sign(b_advantages) == torch.sign(b_hlcdf_z)
                ).float().mean()
                hlcdf_large_flip_frac = (
                    (torch.sign(b_advantages) != torch.sign(b_hlcdf_z))
                    & (b_hlcdf_z.abs() > 0.5)
                ).float().mean()
            else:
                ...
                hlcdf_u_base_mean = torch.zeros((), device=device)
                hlcdf_u_base_std = torch.zeros((), device=device)
                hlcdf_z_base_abs = torch.zeros((), device=device)
                hlcdf_z_delta_sign_agree = torch.zeros((), device=device)
                hlcdf_large_flip_frac = torch.zeros((), device=device)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if (not is_hlcdf_delta_mode) and args.adv_rms_norm:
                    mb_advantages = mb_advantages / mb_advantages.square().mean().sqrt().clamp_min(1e-8)
                elif (not is_hlcdf_delta_mode) and args.norm_adv and args.norm_adv_scope == "minibatch":
                    mb_advantages = (
        ...
        writer.add_scalar("diag/adv_pre_scale_mean_abs", adv_pre_scale_mean_abs.item(), global_step)
        writer.add_scalar("diag/adv_pre_scale_saturation", adv_pre_scale_saturation.item(), global_step)
        ...
        writer.add_scalar("diag/adv_hlcdf_u_base_mean", hlcdf_u_base_mean.item(), global_step)
        writer.add_scalar("diag/adv_hlcdf_u_base_std", hlcdf_u_base_std.item(), global_step)
        writer.add_scalar("diag/adv_hlcdf_z_base_abs", hlcdf_z_base_abs.item(), global_step)
        writer.add_scalar("diag/adv_hlcdf_z_delta_sign_agree", hlcdf_z_delta_sign_agree.item(), global_step)
        writer.add_scalar("diag/adv_hlcdf_large_flip_frac", hlcdf_large_flip_frac.item(), global_step)
