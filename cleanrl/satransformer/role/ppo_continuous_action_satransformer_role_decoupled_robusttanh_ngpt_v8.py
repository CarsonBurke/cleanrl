# nGPT hypersphere satransformer v8-robusttanh: v7 decoupled role-token
# encoder/decoders with robust bounded raw-GAE advantages.
#
...
#
# Diff vs v7: replace batch rankgauss with robust bounded raw-GAE shaping:
# adv = tanh(A / (kappa * robust_scale)), where robust_scale is the median
# absolute advantage converted to normal-equivalent sigma. This keeps raw zero
# as the sign boundary, suppresses noisy tails, and removes empirical rank
# pressure. No target-KL stopping. HL-Gauss remains Dreamer4-style edge-bin
# support with sigma_to_bin_ratio=2 and our reward-normalized raw range
# [-10, 10].
#
...
class Args:
    ...
    adv_transform: str = "robust_tanh"
    """policy advantage shaping: 'robust_tanh', 'rankgauss', or 'v10'"""
    cdf_probit_clamp: float = 0.995
    """rankgauss clamp before erfinv"""
    robust_tanh_kappa: float = 1.5
    """temperature for tanh(raw_advantage / (kappa * robust_scale))"""
    robust_scale_floor_frac: float = 0.05
    """minimum robust scale as a fraction of standard deviation"""
    clip_coef: float = 0.2
...
def shape_advantage(gae, args, device):
    if args.adv_transform == "v10":
        return gae
    if args.adv_transform == "robust_tanh":
        abs_med = gae.abs().median()
        std = gae.std()
        normal_equiv_scale = abs_med / 0.67448975
        floor = args.robust_scale_floor_frac * std
        scale = torch.maximum(normal_equiv_scale, floor).clamp_min(1e-8)
        return torch.tanh(gae / (args.robust_tanh_kappa * scale))
