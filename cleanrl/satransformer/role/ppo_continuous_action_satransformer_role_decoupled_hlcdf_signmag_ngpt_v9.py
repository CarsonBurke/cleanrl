# nGPT hypersphere satransformer v9-hlcdf-signmag: v7 decoupled role-token
# encoder/decoders with raw-sign anchored critic-CDF magnitude advantages.
#
...
#
# Diff vs v8-HLCDF: keep the original absolute CDF magnitude signal
# tanh(abs(probit(F(return))) / kappa), but force the policy sign to match raw
# GAE. This preserves the useful distributional percentile pressure from v8
# while preventing median-vs-baseline sign flips. Update scaling/RMS behavior
# stays aligned with original HLCDF v8. No target-KL stopping.
#
...
class Args:
    ...
    adv_transform: str = "hlcdf_signmag"
    """policy advantage shaping: 'hlcdf_signmag', 'hlcdf_tanh', 'hlcdf_probit', 'rankgauss', or 'v10'"""
...
def hl_gauss_cdf_advantage(logits, returns, raw_gae, hl_support, args):
    ...
    if args.adv_transform == "hlcdf_signmag":
        adv = torch.sign(raw_gae) * torch.tanh(z.abs() / args.hlcdf_tanh_kappa)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        b_hlcdf_u = None
        b_hlcdf_z = None
        if args.adv_transform in ("hlcdf_signmag", "hlcdf_tanh", "hlcdf_probit"):
            with torch.no_grad():
                value_logits_chunks = []
        ...
        with torch.no_grad():
            ...
            if b_hlcdf_u is not None:
                ...
                hlcdf_abs_sign_agree = (
                    torch.sign(b_advantages) == torch.sign(b_hlcdf_z)
                ).float().mean()
                hlcdf_abs_sign_flip_frac = (
                    torch.sign(b_advantages) != torch.sign(b_hlcdf_z)
                ).float().mean()
            else:
                ...
                hlcdf_abs_sign_agree = torch.zeros((), device=device)
                hlcdf_abs_sign_flip_frac = torch.zeros((), device=device)
        ...
        writer.add_scalar("diag/adv_hlcdf_abs_sign_agree", hlcdf_abs_sign_agree.item(), global_step)
        writer.add_scalar("diag/adv_hlcdf_abs_sign_flip_frac", hlcdf_abs_sign_flip_frac.item(), global_step)
