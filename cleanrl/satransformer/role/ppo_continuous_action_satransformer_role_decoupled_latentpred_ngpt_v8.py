# nGPT hypersphere satransformer v8-latentpred: v7 decoupled role-token
# encoder/decoders with auxiliary latent next-observation prediction.
#
# Diff vs v7: add two learned global latent prediction tokens. They decode from
# the shared obs/latent memory and feed a latent head that predicts the next
# normalized observation vector. The auxiliary MSE loss is masked on terminal
# transitions and gets its own clipped backward pass, so it trains shared
# representation plus the latent decoder/head without directly updating
# actor/critic private readout paths.
#
...
# support with sigma_to_bin_ratio=2, but uses our reward-normalized raw value
# range [-10, 10] before symlog conversion.
#
...
LATENT_HEAD_DIM = MODEL_DIM * NUM_LATENT_PRED_TOKENS
...
class Args:
    ...
    latent_pred_coef: float = 0.1
    """coefficient for auxiliary next-observation prediction loss"""
    latent_grad_clip: float = 0.25
    """max norm for latent prediction gradient clipping when separate_grad_clip=True"""
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.latent_pred_type_embed = nn.Parameter(torch.empty(1, MODEL_DIM))
        ...
        self.latent_pred_tokens = nn.Parameter(
            torch.randn(NUM_LATENT_PRED_TOKENS, MODEL_DIM) * seed_std
        )
        ...
        self.latent_pred_cross_blocks = nn.ModuleList(
            [NGPTCrossAttentionBlock(MODEL_DIM, NUM_HEADS, FFN_MULT) for _ in range(NUM_DECODER_BLOCKS)]
        )
        ...
        self.latent_head = layer_init(nn.Linear(LATENT_HEAD_DIM, self.obs_dim), std=0.01)
    ...
    def normalize_weights(self):
        ...
        ngpt_blocks = (
            ...
            + list(self.latent_pred_cross_blocks)
        )
        ...
        self.latent_pred_type_embed.data.copy_(justnorm(self.latent_pred_type_embed.data, dim=-1))
        ...
        self.latent_pred_tokens.data.copy_(justnorm(self.latent_pred_tokens.data, dim=-1))

    def _encode(self, x, include_latent_pred=False):
        ...
        scale. Returns actor features (B, action_dim, 3D), critic feature
        (B, 2D), and optional latent prediction feature (B, 2D)."""
        B = x.shape[0]
        ...
        latent_pred_feat = None
        if include_latent_pred:
            latent_pred_tokens = self.latent_pred_tokens + self.latent_pred_type_embed
            latent_pred_tokens = justnorm(latent_pred_tokens).unsqueeze(0).expand(B, -1, -1)
            for block in self.latent_pred_cross_blocks:
                latent_pred_tokens = block(latent_pred_tokens, memory)
            latent_pred_feat = latent_pred_tokens.reshape(B, LATENT_HEAD_DIM) * (MODEL_DIM ** 0.5)
        ...
        return actor_feats, critic_feat, latent_pred_feat

    def _encoder_with_autocast(self, x, include_latent_pred=False):
        if x.is_cuda and torch.cuda.is_bf16_supported():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                actor_feat, critic_feat, latent_pred_feat = self._encode(x, include_latent_pred)
        else:
            actor_feat, critic_feat, latent_pred_feat = self._encode(x, include_latent_pred)
        if latent_pred_feat is not None:
            latent_pred_feat = latent_pred_feat.float()
        return actor_feat.float(), critic_feat.float(), latent_pred_feat
    ...
    def latent_parameters(self):
        ...
        params += [
            self.latent_pred_tokens,
            self.latent_pred_type_embed,
        ]
        params += list(self.latent_pred_cross_blocks.parameters())
        params += list(self.latent_head.parameters())
        return params
    ...
    def get_value_logits(self, x):
        _, critic_feat, _ = self._encoder_with_autocast(x)
    ...
    def get_action_distribution(self, x):
        actor_feat, _, _ = self._encoder_with_autocast(x)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        actor_feat, critic_feat, _ = self._encoder_with_autocast(x)
    ...
    def get_action_value_and_prediction(self, x, z):
        actor_feat, critic_feat, latent_pred_feat = self._encoder_with_autocast(
            x,
            include_latent_pred=True,
        )
        dist, to_action, log_det_fn = self._actor_dist(actor_feat)
        value_logits = self.critic_head(critic_feat)
        next_obs_pred = self.latent_head(latent_pred_feat)
        env_action = to_action(z)
        log_prob = dist.log_prob(z).sum(1) - log_det_fn(z)
        entropy = dist.entropy().sum(1)
        return env_action, z, log_prob, entropy, value_logits, next_obs_pred

    def get_next_obs_prediction(self, x):
        _, _, latent_pred_feat = self._encoder_with_autocast(x, include_latent_pred=True)
        return self.latent_head(latent_pred_feat)
...
if __name__ == "__main__":
    ...
    latent_params = agent.latent_parameters()
    support_min, support_max = value_support_bounds(args)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            next_done_tensor = torch.Tensor(next_done).to(device)
            rewards[step] = torch.tensor(reward).to(device).view(-1)
            next_observations[step] = next_obs_tensor
            next_dones[step] = next_done_tensor
            next_obs, next_done = next_obs_tensor, next_done_tensor
        ...
        latent_gn = torch.zeros((), device=device)
        latent_loss = torch.zeros((), device=device)
        latent_nonterminal_frac = 0.0
        for epoch in range(args.update_epochs):
            np.random.shuffle(b_inds)
            clipfracs = []
            latent_nonterminal_fracs = []
            for start in range(0, args.batch_size, args.minibatch_size):
                end = start + args.minibatch_size
                mb_inds = b_inds[start:end]

                # b_actions[mb_inds] is the stored native sample; pass as `z`.
                _, _, newlogprob, entropy, newvalue_logits, pred_next_obs = agent.get_action_value_and_prediction(
                ...
                latent_mask = (1.0 - b_next_dones[mb_inds]).float()
                latent_mse = (pred_next_obs - b_next_obs[mb_inds]).square().mean(dim=-1)
                latent_loss = (latent_mse * latent_mask).sum() / latent_mask.sum().clamp_min(1.0)
                latent_nonterminal_fracs.append(latent_mask.mean().item())

                entropy_loss = entropy.mean()  # logged only; no entropy term in the loss

                if args.separate_grad_clip:
                    optimizer.zero_grad(set_to_none=True)
                    (args.latent_pred_coef * latent_loss).backward(retain_graph=True)
                    latent_gn = nn.utils.clip_grad_norm_(latent_params, args.latent_grad_clip)
                    latent_grads = [
                    ...
                    loss = pg_loss + v_loss * args.vf_coef + latent_loss * args.latent_pred_coef
                    optimizer.zero_grad(set_to_none=True)
                    loss.backward()
                    actor_gn = critic_gn = latent_gn = nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                optimizer.step()
            ...
            latent_nonterminal_frac = float(np.mean(latent_nonterminal_fracs)) if latent_nonterminal_fracs else 0.0
        ...
        with torch.no_grad():
            ...
            latent_pred_attn_lr = block_lr(agent.latent_pred_cross_blocks, "attn_alpha")
            latent_pred_mlp_lr = block_lr(agent.latent_pred_cross_blocks, "mlp_alpha")

        # TRY NOT TO MODIFY: record rewards for plotting purposes
        writer.add_scalar("charts/learning_rate", optimizer.param_groups[0]["lr"], global_step)
        writer.add_scalar("losses/value_loss", v_loss.item(), global_step)
        writer.add_scalar("losses/latent_pred_loss", latent_loss.item(), global_step)
        ...
        writer.add_scalar("grad/latent_norm", latent_gn.item(), global_step)
        writer.add_scalar("losses/explained_variance", explained_var, global_step)
        writer.add_scalar("diag/latent_nonterminal_frac", latent_nonterminal_frac, global_step)
        ...
        writer.add_scalar("diag/ngpt_latent_pred_attn_lr", latent_pred_attn_lr, global_step)
        writer.add_scalar("diag/ngpt_latent_pred_mlp_lr", latent_pred_mlp_lr, global_step)
