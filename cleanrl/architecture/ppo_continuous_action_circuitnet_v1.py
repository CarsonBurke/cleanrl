# CircuitNet v1 — PPO + recurrent sparse MoE over a residual workspace.
#
# Spec: specs/circuit_net_spec.md (§1.1-1.8, §2, §3, Appendix A). Successor to DSRG.
#
# IDEA: a giant pool of M small SwiGLU experts; per input a learned top-2 router
# activates a sparse subset and composes them over T recurrent steps into an
# input-specific CIRCUIT, on a single residual workspace w (no O(N^2) mixing).
#
#   obs --encoder--> w in R^{B,D}
#   for t in 1..T:                         (experts WEIGHT-TIED across steps = reuse substrate)
#       w_hat = LayerNorm(w) + phase_emb[t]    (per-step PHASE code -> ordered circuit)
#       logits = router(w_hat)  (+ warmup noise sigma*randn, sigma=max(0,1-step/75k))
#       top-2 by logit; softmax over the 2 kept logits only
#       Delta = sum_{e in top2} gate_e * Expert_e(w_hat)
#       w = w + gamma * Delta                  (per-channel LayerScale gamma, init 0.1)
#   head_in = LayerNorm(w_T)               (DENSE readout; no routing on the heads)
#
...
# that genuinely breaks the weight-tied iterated-map fixed point.
#
...
#
# Kept from v15 (byte-for-byte): PPO loop, GAE, Beta actor, HL-Gauss symlog critic,
# obs/reward norm, env wrappers, triple-backward decoupled grad-clip, Args/tyro pattern.
import os
import random
import time
...
class Args:
    ...
    n_slots: int = 1                 # K: workspace slots (1 in v1; >1 = backlog blackboard)
...
class CircuitNetTrunk(nn.Module):
    """obs -> encoder -> workspace w in R^{B,D}; T recurrent steps of top-k sparse-MoE
    over the residual workspace (experts WEIGHT-TIED across steps); LayerNorm(w_T) is the
    DENSE head input. §1.2 + Appendix A.

    MoE forward = dense-compute-all-M then mask to top-2 (no gather/scatter at M=64).
    Anti-collapse package (LB + z-loss + warmup logit noise) is first-class from step 0.
    Per-step phase_emb makes the weight-tied T-step selection an ORDERED circuit.

    Stashes (read by the update loop / metrics each forward):
      self.route_loss            : lb_coef*(sum_t LB_t)/T + z_coef*(sum_t z_t)/T
      self._gate_full            : (T, B, M) full gate-mass per step (0 off top-k) for routing_mi/util
      self._expert_out_stack     : (M, B, D) per-expert output on the LAST step (for eff_rank)
      self._expert_absh          : (T, M) per-expert mean |hidden| (for ReDo / dead-expert)
      self._delta_ratio          : (T,) ||gamma*Delta_t|| / ||w_T|| per step
      self._router_logits_last   : (B, M) last-step logits (router_entropy diagnostic)
    """
    ...
    def __init__(self, obs_dim, args):
        ...
        # M SwiGLU experts, weight-tied across T (shared module reused each step).
        ...
        self.final_ln = nn.LayerNorm(self.D)          # LayerNorm(w_T) -> head input
    ...
    def _all_experts(self, w_hat):
        # Dense compute of ALL M experts on the full batch. Returns (M, B, D) and (M, B) |hidden|.
        outs, absh = [], []
        for e in self.experts:
            hid = F.silu(e.W_gate(w_hat)) * e.W_in(w_hat)   # (B, h)
            outs.append(e.W_out(hid))                       # (B, D)
            absh.append(hid.abs().mean(dim=-1))             # (B,) per-sample mean |hidden|
        return torch.stack(outs, dim=0), torch.stack(absh, dim=0)   # (M,B,D), (M,B)

    def forward(self, x):
        B = x.shape[0]
        w = self.encoder(x)                                 # (B, D) workspace
        ...
        for t in range(self.T):
            w_hat = self.pre_ln(w) + self.phase_emb[t]      # (B, D) pre-LN + ordered phase code
            expert_out, absh = self._all_experts(w_hat)     # (M,B,D), (M,B)
            absh_steps.append(absh.mean(dim=1))             # (M,) per-expert mean |hidden| over batch

            if self.dense_control:
                # CONTROL: no routing. All M experts summed as a plain residual MLP
                # (top_k = M effectively, no router grad, no LB/z). Matched active FLOPs
                # only when n_experts is set to the control's matched count by the caller.
                delta = expert_out.sum(dim=0)               # (B, D)
                gate_full = expert_out.new_zeros(B, self.M) # routing metrics undefined -> zeros
            else:
                logits = self.router(w_hat)                 # (B, M)
                ...
                gate_k = torch.softmax(topv, dim=-1)                     # (B, k) over kept logits
                # scatter the kept gate mass back to full M (0 elsewhere).
                gate_full = logits.new_zeros(B, self.M).scatter(1, topi, gate_k)   # (B, M)
                # Delta = sum_{e in topk} gate_e * Expert_e(w_hat); dense-all-M masked to top-k.
                delta = (gate_full.t().unsqueeze(-1) * expert_out).sum(dim=0)      # (B, D)

                # --- anti-collapse aux (batch-level, summed over T then /T) ---
                P = torch.softmax(logits, dim=-1).mean(0)                # (M,) full-M soft mass
                onehot = logits.new_zeros(B, self.M).scatter(1, topi, 1.0)
            ...
            w = w + self.gamma * delta                      # LayerScale residual update
            last_expert_out = expert_out
            # delta_ratio computed against the eventual w_T below (need w_T norm) -> store delta now
            delta_ratio_steps.append((self.gamma * delta).norm(dim=-1))   # (B,) numerator per step

        w_T = self.final_ln(w)                              # (B, D) head input
        wT_norm = w.norm(dim=-1, keepdim=False).clamp_min(1e-8)          # pre-LN w_T norm (the residual stream)
        ...
        self._gate_full = torch.stack(gate_full_steps, dim=0)           # (T, B, M)
        self._expert_out_stack = last_expert_out.detach()              # (M, B, D) last step
        self._expert_absh = torch.stack(absh_steps, dim=0).detach()     # (T, M)
        self._delta_ratio = torch.stack(
            [dr.mean() / wT_norm.mean() for dr in delta_ratio_steps]    # (T,) mean over batch
        ).detach()
        self._router_logits_last = (last_logits.detach() if last_logits is not None
                                    else w.new_zeros(B, self.M))
        return w_T
...
def circuit_metrics(trunk):
    ...
    routing_mi   : H(mean_b q_b) - mean_b H(q_b), q_b in R^M = per-sample top-k gate-mass.
                   We average the per-sample gate-mass over the T steps (mean over steps)
                   to get one q_b per sample, then take the entropy gap. Natural log (nats).
    eff_rank     : exp(entropy of normalized singular values) of the stacked expert-output
                   matrix (M x D) -- the per-expert mean output over the probe batch (the
                   §2 "stacked expert outputs over a probe batch" reading). Illusory-width
                   / collapse detector.
    dead_expert_frac : fraction of experts with dormancy s_e = E|h_e| / mean_e E|h_e| < tau.
    delta_ratio[t] : ||gamma*Delta_t|| / ||w_T|| per step (already a per-step mean over batch).
    plus mean_gamma, router_entropy, expert_util_entropy diagnostics.
    """
    M = trunk.M
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        D = args.d_model
        # §1.1: SEPARATE actor & critic trunks (non-negotiable). v1 exercises only the
        # separate path; share_backbone is kept for compat but must be False.
        assert not args.share_backbone, "CircuitNet v1 requires share_backbone=False (separate trunks)"
    ...
    def actor_parameters(self):
        # actor_trunk (encoder + experts + phase + gamma + router + LNs) + actor head.
        if self.actor_dist == "gaussian":
...
if __name__ == "__main__":
    ...
    if args.compile:
        # Compile the CircuitNetTrunk.forward (fixed shapes, dense-compute-all-M -> no
        # recompiles, T=2 -> no grad-checkpoint). donated_buffer off: the triple-backward
        # backprops value/route with retain_graph=True, incompatible with donated buffers.
        # Stashing self._* attrs from a compiled forward graph-breaks at those points but
        # still runs correctly; if compile fails at all, fall back to eager (never break
        # the run).
