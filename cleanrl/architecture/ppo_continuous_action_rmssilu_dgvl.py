# RMSNorm+SiLU + HL-Gauss + Direct Log-Std + DG(value-loss surprisal)
#
# Combines:
#   - RMSNorm+SiLU architecture (replacing Tanh activations)
#   - HL-Gauss categorical value head (no value clipping)
#   - State-dependent log_std (dlogstd)
#   - Modified DG: gate = sigmoid(advantage * value_loss_per_sample / eta)
#     Uses per-sample HL-Gauss CE loss as surprisal signal.
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        # Critic with HL-Gauss categorical output, RMSNorm+SiLU
        ...
        # Shared actor trunk with RMSNorm+SiLU, outputs both mean and log_std
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Compute per-sample value loss first (used as surprisal for DG gate)
                ...
                v_loss_per_sample = -(target_probs * log_probs).sum(dim=-1)
                v_loss = v_loss_per_sample.mean()

                # DG gate: sigmoid(raw_advantage * value_loss_surprisal / eta)
                # Value loss per sample measures how surprising the outcome was
                # to the critic — stays meaningful even with low policy entropy
                # Computed BEFORE advantage normalization
                with torch.no_grad():
                    vl_surprisal = v_loss_per_sample.detach()
                    delight = mb_advantages * vl_surprisal
