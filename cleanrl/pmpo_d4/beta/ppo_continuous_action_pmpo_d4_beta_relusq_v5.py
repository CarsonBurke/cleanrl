# PMPO-D4 + ReluSq + Beta-distribution policy v2 — softplus parametrization.
#
# v1 used the Della Libera (arxiv:2409.04971) recipe:
#   α = 1 + exp(clamp(head, -20, 2))
# exp+clamp is unstable: exp's gradient explodes for large head, and the hard
# clamp creates dead gradient outside the interval. The +2 cap forces α ≤ ~8.4,
# putting an artificial ceiling on policy peakedness.
#
# v2 uses softplus (lucidrains' empirical recommendation for Beta policies):
#   α = 1 + softplus(head_α)
#   β = 1 + softplus(head_β)
# Properties:
#   - dα/dhead = sigmoid(head) ∈ (0, 1) — gradient natively bounded, no explosion
#   - smooth everywhere, no clamp discontinuity
#   - α grows linearly in head for head ≫ 0 (vs exponentially with exp)
#   - identical lower-end: head → -∞ gives α → 1 (uniform Beta)
#   - no artificial upper cap — α grows as needed for sharp policies
# The paper's `no_clip` ablation failed because exp had unbounded gradient,
# not because the +1 floor was insufficient. softplus structurally doesn't
# need the clamp.
#
# +1 shift still forces α,β ≥ 1 → unimodal, concave Beta (Chou et al. 2017).
# Init: head≈0 → α=β=1+softplus(0)=1+log(2)≈1.69, slightly less peaked than
# v1's Beta(2,2). Still spread, mean=0.5, action center after remap.
#
