# PMPO-D4 + ReluSq + Beta v3 — delight-gated PMPO (dreamer4 ablation).
#
# Builds on v2 (softplus Beta). Adds the "delight gate" from
# lucidrains/dreamer4 (dreamer4.py:4258), a per-action-dim multiplicative
# weight on log_probs before the PMPO advantage scaling:
#
#   centered_surprise = -(log_prob + entropy)   # per-dim, mean 0 under policy
#   delight_gate      = sigmoid(centered_surprise * adv / T).detach()
#   gated_log_prob    = log_prob * delight_gate
#   signed_log_prob   = gated_log_prob * |tanh(adv)|
#
# Why center on entropy. dreamer4's original gate `sigmoid(-log_prob * adv)`
# assumes categorical log-probs (≤ 0) where -log_prob ≥ 0 measures sample
# surprise. For our continuous Beta, log_prob is a *density* whose mean under
# the policy equals -entropy. Without centering, two failure modes appeared:
#   (a) plain `-log_prob * adv` near-canceled across the bulk of samples
#       (mean ≈ 0), driving gate ≈ 0.5 with no information content. Empirically
#       observed: gate_mean stuck at 0.500 throughout training, Beta
#       concentration drifted *down* from 3.4 → 2.7 (anti-learning).
#   (b) `clamp_max(log_prob, 0) * adv` had the same degeneracy because
#       Beta(1.69,1.69) has positive density at the mode → clamp zeros most
#       samples → gate ≈ 0.5.
# Centering on entropy gives a per-sample quantity with mean 0 and proper
# variance: positive for tail samples (rare under policy), negative for
# mode samples. Multiplied by adv this restores dreamer4's intended
# semantics:
#   - pos-adv mode (settled good)   → logit < 0 → gate < 0.5 (down-weight)
#   - pos-adv tail (rare good)      → logit > 0 → gate > 0.5 (up-weight)
#   - neg-adv mode (settled bad)    → logit > 0 → gate > 0.5 (up-weight)
#   - neg-adv tail (already-rare)   → logit < 0 → gate < 0.5 (down-weight)
# Detached, no gradient through gate.
#
# Interpretation. The gate's logit is -log_prob * adv:
#   - adv > 0, action already likely (log_prob ≈ 0)   → logit ≈ 0  → gate ≈ 0.5
#   - adv > 0, action unlikely (log_prob ≪ 0)         → logit ≫ 0  → gate ≈ 1
#   - adv < 0, action already unlikely (log_prob ≪ 0) → logit ≪ 0  → gate ≈ 0
#   - adv < 0, action likely (log_prob ≈ 0)           → logit ≈ 0  → gate ≈ 0.5
#
# The gate suppresses gradient on "settled" samples (good action already
# probable, or bad action already improbable) and concentrates gradient where
# the policy still disagrees with the advantage signal. Similar in spirit to
# focal-loss focusing — but it is sample-wise and symmetric across sign of
# advantage, not a clipping ratio.
#
# Detached: gate flows no gradient. Pure loss-mass reweighting.
#
# Hypothesis. v2 collapses on Hopper (α+β → 829, exploration dies) because
# PMPO keeps pushing all neg-advantage probability mass down even after the
# policy has stopped putting mass there — squeezing the Beta tighter and
# tighter around its current mode. The delight gate kills exactly that
# pathological gradient: once the bad action is already improbable, gate → 0,
# concentration stops growing. On HalfCheetah (where v2 already worked) the
# gate should be near-neutral on saturated samples and negligible elsewhere,
# so we expect ≈ parity or modest gain.
#
# Single new hyperparameter: `delight_temperature` (default 1.0, matches
# dreamer4 default).
...
class Args:
    ...
    # delight gate (dreamer4 ablation)
    use_delight_gating: bool = True
    """multiply log_prob by sigmoid(-log_prob*adv/T).detach() before PMPO weighting"""
    delight_temperature: float = 1.0
    """temperature T in the delight gate logit"""
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        reverse_kl_count = 0
        delight_gate_sum = torch.zeros((), device=device)
        delight_gate_count = 0
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.policy_objective == "ppo":
                    ...
                    # Delight gate (dreamer4 dreamer4.py:4258), entropy-centered for
                    # continuous Beta. See header comment for the why. The gate
                    # input is `centered_surprise = -(log_prob + entropy)`, mean
                    # 0 under the policy with proper per-sample variance even
                    # for nearly-uniform Beta. Detached → no grad through gate.
                    if args.use_delight_gating:
                        entropy_per_dim_new = new_dist.entropy()  # [N, action_dim]
                        centered_surprise = -(log_prob_per_dim + entropy_per_dim_new)
                        delight_gate = (
                            centered_surprise * adv_per_dim / args.delight_temperature
                        ).sigmoid().detach()
                        gated_log_prob = log_prob_per_dim * delight_gate
                        delight_gate_sum = delight_gate_sum + delight_gate.mean().detach() * len(mb_inds)
                        delight_gate_count += len(mb_inds)
                    else:
                        gated_log_prob = log_prob_per_dim

                    signed_log_probs = gated_log_prob * adv_weight
        ...
        if args.use_delight_gating and delight_gate_count > 0:
            writer.add_scalar("diag/delight_gate_mean", (delight_gate_sum / delight_gate_count).item(), global_step)
