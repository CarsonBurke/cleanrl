# PMPO-D4 + ReluSq + Beta — clip-higher v1 (DAPO/OpenRLHF asymmetric clip).
#
# Sibling to advnorm_v3 (KL hard-stop). Same Beta + ReluSq + adv_norm
# backbone; trust region swapped from per-dim reverse-KL hard-cap to the
# canonical PPO ratio-clipped surrogate with asymmetric bounds:
#
#   pg_loss1 = -adv_norm * ratio
#   pg_loss2 = -adv_norm * clamp(ratio, 1 - clip_low, 1 + clip_high)
#   pg_loss  = max(pg_loss1, pg_loss2).mean()
#
# clip_coef_low = 0.20, clip_coef_high = 0.28 (DAPO-flavored asymmetry, also
# used in this repo's lstd_ablations/lstd_gated.py). Looser ceiling lets
# positive-advantage actions push the policy harder; tighter floor keeps
# negative-advantage suppression conservative.
#
# Why no /action_dim. PPO clipping operates on the joint density ratio
# `exp(sum_d log_prob_d_new - sum_d log_prob_d_old)`. The clip bounds the
# *joint* drift directly, so action_dim is already absorbed — no per-dim
# rescaling needed. Compare to v3 where the soft-KL trust region was
# coupled to per-dim KL and required action_dim invariance manually.
#
# What's dropped from v3:
#   - kl_coef * KL term (no soft penalty)
#   - target_kl per-mb hard cap (no early-stop)
#   - kl_break / n_updates diagnostics
#   - /action_dim division on PG (and KL is gone entirely)
# What's added: standard PPO `clipfrac` diagnostic + `losses/clipfrac`.
#
# Hypothesis. Clip-higher is the canonical, well-tuned trust region;
# v3's KL hard-cap was a workaround for the soft-coef KL approach failing.
# At parity on Beta + ReluSq + adv_norm, expect comparable or better HC
# performance than v3, with simpler machinery and no early-stop hyperparam.
...
class Args:
    ...
    clip_coef_low: float = 0.20
    """policy ratio floor: clip(ratio, 1 - low, 1 + high)"""
    clip_coef_high: float = 0.28
    """policy ratio ceiling (DAPO/OpenRLHF asymmetric clip)"""
    ent_coef: float = 0.0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # PPO clip-higher (DAPO/OpenRLHF asymmetric).
                pg_loss1 = -mb_advantages * ratio
                pg_loss2 = -mb_advantages * torch.clamp(
                    ratio, 1 - args.clip_coef_low, 1 + args.clip_coef_high
                )
