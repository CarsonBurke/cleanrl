# PMPO-D4 + ReluSq + Beta — adv-norm ablation v2 (action-dim invariant).
#
# v1 collapsed (NaN) on HC. Sign-stripped objective + kl_coef=0.3 left the
# reverse-KL term too weak relative to the PG term, drift compounded across
# 320 grad steps per iteration, parameters runaway → NaN.
#
# v2 changes vs v1:
#   1. Action-dim invariant losses. PG and KL each divided by action_dim D
#      so the same kl_coef transfers across envs. log_prob storage stays
#      joint (sum-over-dims) so ratio / approx_kl diagnostics keep standard
#      joint-density semantics — only loss assembly divides by D.
#   2. kl_coef 0.3 → 3.0 to match v3's effective trust-region tightness:
#        TR = kl_coef · |∂kl_loss/∂θ| / |∂pg_loss/∂θ|
#        v3 (HC):       0.3 · 6g / 0.5g = 3.6
#        v2 (any env):  kl_coef · g / 0.8g  = kl_coef / 0.8
#        match → kl_coef ≈ 0.8 · 3.6 = 2.88, round to 3.0
#      Side effect: every env now gets the same TR ≈ 3.75 (v3 was env-
#      coupled: HC=3.6, Hopper=1.8). Hopper was where v3 collapsed, so
#      tighter TR there is desired.
#
# Final objective:
#   adv_norm = (adv - adv.mean()) / (adv.std() + eps)        # per-minibatch
#   pg_loss  = -(adv_norm * log_prob_joint / D).mean()        # D = action_dim
#   kl_loss  = KL(old || new).sum(-1).mean() / D              # exact Beta KL
#   loss     = pg_loss + kl_coef * kl_loss + 0.5 * v_loss
#
# Hypothesis. With trust region scale-matched to v3, PG + reverse KL on
# normalized advantages should not collapse. If it still diverges, soft-
# coefficient KL is fundamentally insufficient and a target_kl early-stop
# (or ratio clipping) is needed — the canonical Schulman 2017 §4.1 finding.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # so kl_coef is interpretable as a per-dim quantity that
                # transfers across envs.
