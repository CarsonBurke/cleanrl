# PMPO-D4 + ReluSq + Beta — adv-norm ablation v3 (KL hard-stop).
#
# Lineage:
#   v1 (sum-PG, kl_coef=0.3)        → NaN; soft KL too weak, drift compounded
#   v2 (per-dim PG/KL, kl_coef=3.0) → no NaN but catastrophic early spike:
#                                     reverse_kl 0.007 → 23.9 → 137 (per-dim!)
#                                     across iters 2-3, then settled. Networks
#                                     spent first ~100k steps in garbage
#                                     param space, plateaued at ~140 return
#                                     at 1.4M / 8M.
#
# Diagnosis. Soft kl_coef penalty is a Lagrangian: gradient descent can
# violate it transiently. With 320 grad steps per iteration and no per-step
# enforcement, a few outlier minibatches early in training are enough to
# blow the trust region wide open. This is exactly Schulman 2017 §4.1:
# fixed-coef KL penalty abandoned in favor of clipping.
#
# v3 fix: hard per-minibatch reverse-KL early-stop. After computing
# reverse_kl in each minibatch, if it exceeds `target_kl` we skip this mb's
# update AND all remaining minibatches/epochs in this iteration. The soft
# kl_coef stays as a smoother *within* the trust region; target_kl is the
# hard cap that catches the cases where the soft pull-back loses the race.
#
# target_kl = 0.02 per-dim (≈ 0.12 joint on HC). Calibrated from v2 logs:
#   - normal steady-state per-dim reverse_kl: 0.0005 - 0.005
#   - v2 catastrophic spike:                  23.9 - 137
#   - threshold sits ~5× above normal, ~1000× below failure.
#
# Objective unchanged from v2:
#   adv_norm = (adv - adv.mean()) / (adv.std() + eps)
#   pg_loss  = -(adv_norm * log_prob_joint / D).mean()        # D = action_dim
#   kl_loss  = KL(old || new).sum(-1).mean() / D              # per-dim
#   loss     = pg_loss + kl_coef * kl_loss + 0.5 * v_loss
#   if reverse_kl > target_kl: skip this mb update, break to next iteration
#
# Hypothesis. With the hard cap, the early spike is prevented; rest-of-
# training resembles v2 steady-state but starting from a clean policy
# instead of a damaged one. Expect substantially closer to HC baseline
# (~1500-3000) than v2's plateau.
...
class Args:
    ...
    kl_coef: float = 3.0
    """coefficient on KL(old rollout policy || new policy) penalty (per-dim scale)"""
    reverse_kl: bool = True
    """if true use KL(old||new); if false use KL(new||old) for ablation"""
    target_kl: float = 0.02
    """per-dim KL hard cap; if a minibatch's reverse_kl exceeds this, skip
    the update and break out of remaining minibatches/epochs this iteration"""
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        reverse_kl_count = 0
        # Pre-init loss tensors so logging is safe if the very first minibatch
        # of the very first iteration trips the kl_break (no update applied).
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Action-dim invariant: divide joint log_prob and joint KL by D
                # so kl_coef / target_kl are interpretable as per-dim quantities
                # that transfer across envs.
                action_dim = agent.action_dim

                # Vanilla advantage-weighted log-likelihood (no ratio clipping).
                pg_loss = -(mb_advantages * newlogprob).mean() / action_dim

                # Exact Beta KL as the trust region.
                old_dist = Beta(b_old_alphas[mb_inds], b_old_betas[mb_inds])
                if args.reverse_kl:
                    kl_loss = kl_divergence(old_dist, new_dist).sum(-1).mean() / action_dim
                else:
                    kl_loss = kl_divergence(new_dist, old_dist).sum(-1).mean() / action_dim
                reverse_kl = kl_loss
                reverse_kl_sum = reverse_kl_sum + reverse_kl.detach() * len(mb_inds)
                reverse_kl_count += len(mb_inds)

                # Hard trust-region cap. Skip this mb's update and any
                # remaining work this iteration if drift exceeded threshold.
                # The forward pass is wasted but it's the cost of admission.
                if args.target_kl > 0.0 and reverse_kl.item() > args.target_kl:
                    kl_break = True
                    break
        ...
        writer.add_scalar("diag/n_updates", n_updates, global_step)
        writer.add_scalar("diag/kl_break", int(kl_break), global_step)
