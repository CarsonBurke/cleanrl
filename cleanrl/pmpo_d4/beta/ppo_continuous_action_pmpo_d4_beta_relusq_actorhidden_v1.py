# PMPO-D4 + ReluSq + Beta-distribution policy + ActorHidden-to-Critic v1.
#
# Ablation of pmpo_d4_beta_relusq_v1: the critic receives the actor's detached
# final hidden activation (the second ReluSq(64) output, just before the
# concentration head) concatenated with the observation. Actor gradients are
# stopped at the cat — the critic gets a privileged *view* of the actor's
# pre-head latent, but its loss cannot reshape that latent.
#
# Hypothesis: the actor's pre-head representation is an action-relevant
# summary of the state. Giving the critic that summary should make value
# fitting easier on visited states, especially when the actor's latent
# captures structure (gait phase, contact, etc.) that a from-scratch critic
# body would have to rediscover. Same intent as the lnmean_relusq_actorhidden
# ablation; here the underlying policy is Beta(α,β) so there is no LayerNorm
# at the head — the trunk's second ReluSq output is the exposed hidden.
#
# Only the critic input dim and the actor-trunk/head split change vs the
# parent v1 file.
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Actor split into trunk (body, up to and including final ReluSq) and
        # head (final Linear → 2*action_dim concentrations). Trunk output is
        # the "final hidden" exposed to the critic.
        ...
        self.actor_head = layer_init(nn.Linear(ACTOR_HIDDEN_DIM, 2 * action_dim), std=0.01)

        # Critic input is obs concatenated with detached actor hidden.
        self.critic = nn.Sequential(
            layer_init(nn.Linear(obs_dim + ACTOR_HIDDEN_DIM, 64)),
            ReluSq(),
            layer_init(nn.Linear(64, 64)),
            ReluSq(),
    ...
    def critic_from_obs_and_hidden(self, x, hidden):
        return self.critic(torch.cat([x, hidden.detach()], dim=-1))

    def get_value(self, x):
        hidden = self.actor_trunk(x)
        return self.critic_from_obs_and_hidden(x, hidden)

    def _dist_from_hidden(self, hidden):
        head = self.actor_head(hidden)
        log_alpha_m1, log_beta_m1 = head.chunk(2, dim=-1)
        log_alpha_m1 = torch.clamp(log_alpha_m1, LOG_CONC_MIN, LOG_CONC_MAX)
        log_beta_m1 = torch.clamp(log_beta_m1, LOG_CONC_MIN, LOG_CONC_MAX)
        alpha = 1.0 + torch.exp(log_alpha_m1)
        beta = 1.0 + torch.exp(log_beta_m1)
        return Beta(alpha, beta)

    def get_action_distribution(self, x):
        hidden = self.actor_trunk(x)
        return self._dist_from_hidden(hidden)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        hidden = self.actor_trunk(x)
        dist = self._dist_from_hidden(hidden)
        if action is None:
        ...
        value = self.critic_from_obs_and_hidden(x, hidden)
        return action, log_prob_per_dim.sum(1), entropy_per_dim.sum(1), value
