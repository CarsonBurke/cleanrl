# PMPO-D4 + ReluSq + Beta — SPO asym v1 (asymmetric ε per drift direction).
#
# Sibling to spo_v1. Same Beta + ReluSq + adv_norm backbone; SPO objective
# kept (quadratic penalty in (r−1), linear gradient pull-back), but the
# constraint bound ε is split asymmetrically based on whether the drift
# direction agrees with the advantage sign — analogue of PPO clip-higher.
#
#   J(θ) = E[ r·A − (|A|/(2·ε(A,r))) · (r − 1)² ]
#
#   ε(A, r) = ε_high  if sign(A) == sign(r − 1)   ("with the advantage")
#             ε_low   otherwise                   ("against the advantage")
#
# Fixed points: r* = 1 + sign(A)·ε_high (loosened in the helpful direction).
#
# Motivation. spo_v1 ran with approx_kl ~0.003 — far below clipdapo's
# 0.013. The linear-gradient SPO trust region was over-constraining,
# starving the policy of update signal. Asymmetric ε is the SPO analogue
# of clipdapo's clip_high=0.28 / clip_low=0.20 setup: loosen the
# constraint when r is moving in the same direction as the advantage,
# keep it tight in the opposite direction.
#
# Hyperparameters: ε_low = 0.20, ε_high = 0.28 (match clipdapo defaults).
#
# Diff vs spo_v1:
#   - spo_eps (single)  →  spo_eps_low + spo_eps_high
#   - per-sample ε computed from sign(A·(r−1)), broadcast into penalty
#
# Hypothesis. With ε raised in the helpful direction, the policy drifts
# more per step toward high-advantage actions; approx_kl rises to a
# level comparable with clipdapo (~0.01–0.02), and learning speed
# narrows the gap to or surpasses clipdapo.
