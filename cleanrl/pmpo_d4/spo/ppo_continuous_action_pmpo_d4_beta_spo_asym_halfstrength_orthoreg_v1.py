# PMPO-D4 + Beta + ReluSq + SPO asym (half-strength) + soft ortho reg
#
# Base: pmpo_d4_beta_relusq_spo_asym_v1 with half-strength SPO bounds
# (ε_low=0.40, ε_high=0.56 — 2x baseline, half-penalty).
#
# Addition: BigGAN-style off-diagonal Gram penalty added to the loss,
#     L_ortho = ortho_coef * Σ_W ||(W^T W) ⊙ (1−I)||_F²    (tall W)
#                          + Σ_W ||(W W^T) ⊙ (1−I)||_F²    (wide W)
# applied to every hidden Linear (final critic value head + final actor
# 2*action_dim head excluded). Off-diagonal-only is scale-invariant, so
# the canonical std=sqrt(2) orthogonal init starts at penalty 0.
#
# Companion experiment: orthoreg_v1 on plain PPO landed at 1545 (regression
# vs FIRE-baseline 2422). Test here whether the strong SPO halfstrength
# backbone (4720 unmodified) tolerates / benefits from the same penalty,
# or whether the orthogonality pressure is fundamentally counterproductive
# for continuous-control PPO regardless of base.
#
# Reference: Brock et al. "Large Scale GAN Training" (BigGAN), 2018.
...
class Args:
    ...
    """soft orthogonality regularizer coefficient (Brock et al BigGAN-style off-diag Gram penalty)"""
