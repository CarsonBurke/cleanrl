# PMPO-D4 + Beta + ReluSq + SPO asym (half-strength) + WeightCenter dim=0
#
# Base: pmpo_d4_beta_relusq_spo_asym_v1 with half-strength SPO bounds
# (ε_low=0.40, ε_high=0.56 — 2x baseline, half-penalty).
#
# Addition: after every optimizer.step(), subtract the column-wise mean
# from each HIDDEN Linear weight: W ← W − W.mean(dim=0, keepdim=True).
# After centering, mean_i W[i,j] = 0 for every input column j. Final actor
# and critic heads are NOT centered — shifting their outputs by a
# data-dependent constant would actively change the policy / value (no
# softmax to absorb the shift, unlike OEC).
#
# Hypothesis (analogue of OEC arXiv:2601.02031): hidden weights drift
# along a rank-1 mean direction, eating effective rank. Removing that
# direction every step is a constant, free gauge fix relative to the
# bias term. Cheap (one mean+sub per Linear per step), no Newton-Schulz.
#
...
if __name__ == "__main__":
    ...
    CENTER_DIM = 0  # v1: subtract column-wise mean (across output rows)
