# PMPO-D4 + Beta + ReluSq + SPO asym (half-strength) + scale-only advantage norm.
#
# Fork of pmpo_d4_beta_spo_asym_halfstrength_v1 (4720.9 baseline).
# Single-line algorithmic change: replace standard PPO advantage normalization
#     A_norm = (A − A.mean()) / (A.std() + 1e-8)
# with **scale-only** normalization
#     A_norm = A / (A.std() + 1e-8)
#
# Why. Standard PPO normalization does TWO things — recenter (subtract mean)
# and rescale (divide by std). The recentering is what causes signs to flip:
# in a minibatch of all-positive raw advantages, e.g. A_raw=[3,5,7,9] with
# mean=6, the smallest advantages become NEGATIVE z-scores. SPO-asym's
# objective reads sign(A) at three places —
#   (1) ε branch selection:     ε_high if sign(A)==sign(r−1) else ε_low
#   (2) PG surrogate direction: pg_surrogate = A · ratio
#   (3) penalty intensity:      |A| · (r−1)² / (2·ε)
# A sign-flipped sample is pushed toward the wrong fixed point r* in (2),
# is mis-bracketed by the asymmetric ε in (1), and has its penalty intensity
# compressed (because |A_norm| is much smaller than |A_raw|) in (3).
#
# Scale-only normalization keeps the variance-reduction (std-divide) benefit
# that gives scale-invariance for hyperparameter transfer, while preserving
# every sample's sign exactly and keeping |A| proportional to raw |A| so the
# SPO penalty intensity still tracks per-sample importance.
#
# Half-strength SPO bounds carry over: ε_low = 0.40, ε_high = 0.56 (2x
# baseline 0.20/0.28, half-penalty), matching the unmodified halfstrength_v1
# 4720 baseline.
#
# Hypothesis. The asymmetry-distortion fix is a strict improvement on every
# surface where SPO touches the advantage: the asymmetric ε reads the true
# sign, the PG drives r in the right direction, and the penalty correctly
# intensifies for high-|A| samples. Should beat halfstrength_v1 on HC.
...
class Args:
    ...
    clip_coef: float = 0.2
    """value-loss clipping coefficient (symmetric); SPO has no policy ratio clip"""
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.norm_adv:
                    # Scale-only normalization: divide by std, do NOT subtract
                    # mean. Preserves sign(A) (so SPO-asym's ε branch reads
                    # the true advantage sign) and keeps |A| proportional to
                    # raw |A| (so the penalty intensity tracks per-sample
                    # importance). Mean-subtract would flip signs in skewed
                    # minibatches and compress the |A| spread.
