# PMPO-D4 + ReluSq + Beta — SPO cubic v1 (quadratic-gradient soft hinge).
#
# Sibling to spo_v1. Replaces the SPO quadratic-in-(r−1) penalty (linear
# gradient) with a cubic-in-|r−1| penalty (quadratic gradient) chosen so
# that the boundary force at r = 1 ± ε is identical to standard SPO, but
# the in-band gradient stays much closer to full strength.
#
#   penalty(r) = (|A| / (3·ε²)) · |r − 1|³
#
#   J(θ) = E[ r·A − penalty(r) ]
#
#   ∂(penalty)/∂r = (|A| / ε²) · |r − 1| · (r − 1)
#       = quadratic in (r − 1), sign-respecting
#
#   ∂J/∂r = A − sign(r−1) · (|A|/ε²) · (r−1)²
#
# Boundary calibration. At r = 1 + ε with A > 0: penalty force = |A|·ε²/ε²
# = |A| = A → cancels the PG term → fixed point r* = 1 + ε (same as
# linear-gradient SPO and PPO clip). For A < 0, fixed point at r = 1 − ε.
#
# Why "soft hinge". Compare per-sample gradient magnitudes (A > 0):
#
#       r          PPO-clip   SPO linear   SPO cubic
#       1          A          A            A
#       1 + ε/2    A          A/2          3A/4   ← stays strong in-band
#       1 + ε      0          0            0
#       1 + 1.5ε   0 (clip)   −A/2         −5A/4  ← sharper recovery
#       1 + 2ε     0 (clip)   −A           −3A    ← much sharper out-of-band
#
# So cubic SPO ≈ PPO-clip-shaped (full strength near r=1, zero at boundary)
# but smooth and with nonzero out-of-band pull-back. This is the
# "softer hinge" approximation of clipping the user asked for.
#
# Motivation. spo_v1 ran with approx_kl ~ 0.003 (vs clipdapo's 0.013),
# i.e., the linear-gradient SPO penalty was active throughout the in-band
# region and softening every sample's PG. Cubic shifts most of the
# penalty mass to the boundary, restoring near-full PG strength in-band
# while keeping the smooth out-of-band pull-back that SPO is designed for.
#
# Diff vs spo_v1:
#   - penalty exponent 2 → 3 (with re-derived 1/(3·ε²) coefficient)
...
class Args:
    ...
    spo_eps: float = 0.2
    """SPO cubic-penalty bound ε; penalty = |A|·|r−1|³ / (3ε²); fixed point r* = 1 ± ε"""
    ent_coef: float = 0.0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # SPO cubic. Penalty = |A| · |r−1|³ / (3·ε²). Gradient
                # magnitude on r is quadratic in (r−1), giving a soft-hinge
                # trust region: near-full PG strength in-band, smooth zero
                # at r = 1 ± ε, fast-growing pull-back beyond.
                pg_surrogate = mb_advantages * ratio
                ratio_diff = ratio - 1.0
                spo_penalty = (
                    mb_advantages.abs() * ratio_diff.abs().pow(3)
                    / (3.0 * args.spo_eps ** 2)
                )
