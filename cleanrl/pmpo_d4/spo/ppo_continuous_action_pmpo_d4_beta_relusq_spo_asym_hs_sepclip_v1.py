# PMPO-D4 + Beta + ReluSq + SPO asym (half-strength) + separate actor/critic clip.
#
# Base: pmpo_d4_beta_relusq_spo_asym_v1 with half-strength SPO bounds
# (ε_low=0.40, ε_high=0.56 — 2x baseline, half-penalty), matching the
# unmodified `pmpo_d4_beta_spo_asym_halfstrength_v1` 4720 baseline.
#
# Addition. The single global clip
#     clip_grad_norm_(agent.parameters(), 0.5)
# is replaced by two independent clips on the disjoint actor and critic
# subnets:
...
#
# This is the orbit-wars
# `_backward_actor_critic_with_separate_clips` pattern, simplified: there
# is no shared trunk here, so a single combined backward suffices and the
# retain_graph / clone-grad dance isn't needed. After the combined
# backward, actor params only carry ∂pg/∂θ_actor (the value loss has no
# path through them) and critic params only carry ∂vf/∂θ_critic, so
# clipping each subnet's grads separately is well-defined.
#
# Hypothesis. With one global clip, a value-loss surge pulls the global
# norm above the 0.5 threshold and shrinks the policy gradient by the
# same factor — even though the policy gradient itself was healthy.
# Separate clips decouple the two: each subnet's update step is governed
# by its own gradient magnitude, so the policy stops paying for critic
# transients (e.g. early HL-Gauss CE spikes / HC-rich-reward bursts).
