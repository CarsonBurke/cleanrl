# PMPO-D4 + ReluSq + TanhNormal policy v3 — z storage + dynamic KL early-stop.
#
# v1 followed Della Libera (arxiv:2409.04971) SAC-TanhNormal recipe verbatim:
# state-dependent (μ, log σ) heads, log σ clamp [-20, 2], tanh squash, Jacobian
# `2(log 2 − z − softplus(−2z))`. On HC 8M with PMPO it crashed at 3.95M with
# NaN: returns swung wildly (-431..+580) before σ blew up. Paper's recipe relies
# on SAC's fixed entropy bonus α=0.2 to hold σ steady; PMPO has no such bonus.
#
# v3 fixes two things:
#
# 1) Store z (pre-tanh latent) directly in the rollout buffer, drop atanh.
#    Tianshou's TanhNormal does this — they `rsample()` and cache `pre_tanh_value`.
#    v1 did atanh(action.clamp(-1+1e-6, 1-1e-6)) during update epochs, which is
#    LOSSY near saturation: tanh(z) → ±1 fast, but the stored action gets clipped
#    by ±(1−ε), so atanh recovers a smaller |z'| ≠ z. log_prob is then evaluated
#    at the wrong z under new params. Combined with state-dependent σ, this
#    roundtrip noise amplifies any drift. Storing z directly removes it.
#
# 2) Dynamic trust region via reverse_kl early-stop (analog of PPO's target_kl).
#    Static log_std clamp is emergency-stop only — never bites in healthy
#    training, can't react to dynamics. Reverse_kl excess directly indicates
#    trust-region violation; breaking the update epoch when running-mean
#    reverse_kl exceeds `pmpo_target_kl` caps per-rollout drift adaptively.
#    The paper's wide [-20, 2] log_std clamp is preserved as a numerical
#    backstop (harmless when never hit).
#
# Paper recipe preserved:
#   - state-dependent (μ, log σ) heads
#   - log σ clamp [-20, 2]
#   - independent action dims, sum log_prob per dim
#   - canonical stable Jacobian formula
#
# Intentional deviations (PPO/PMPO not SAC):
#   - score-function gradient (sample(), not rsample())
#   - 2×64 ReluSq body (CleanRL convention)
#   - PMPO-D4 actor loss; no replay, no Q net, no entropy temperature
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Actor outputs 2*action_dim: [mean heads, raw log-std heads].
        # Both state-dependent per the paper's SAC-TanhNormal recipe.
    ...
    def get_action_distribution(self, x):
        head = self.actor(x)
        mean, raw_logstd = head.chunk(2, dim=-1)
        logstd = raw_logstd.clamp(LOG_STD_MIN, LOG_STD_MAX)
        std = logstd.exp()
    ...
    def squash_log_abs_det_jacobian(self, z):
        ...
        return self.log_action_scale + 2.0 * (log(2.0) - z - nn.functional.softplus(-2.0 * z))
