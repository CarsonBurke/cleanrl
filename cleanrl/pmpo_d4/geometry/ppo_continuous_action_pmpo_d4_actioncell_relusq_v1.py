# PMPO-D4 + ReluSq + TanhNormal action-cell policy v1.
#
...
#
# The PMPO KL is the corresponding categorical KL over all action cells. There
# is no probability floor or clamp; interval probabilities are computed in
# log-space via a CUDA-safe log-CDF approximation and log-diff-exp. PPO mode
# still uses density log-probs.
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.policy_objective == "ppo":
                    ...
                    if args.pmpo_reverse_kl:
                        cell_kl_per_dim = kl_from_log_probs(old_cell_log_probs, new_cell_log_probs)
                    else:
                        cell_kl_per_dim = kl_from_log_probs(new_cell_log_probs, old_cell_log_probs)
                    reverse_kl = cell_kl_per_dim.sum(-1).mean()
