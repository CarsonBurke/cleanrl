# PMPO-D4 + ReluSq + tanh-bounded mean head v1.
#
# Anti-saturation fix for unbounded hidden activations under PMPO's tanh-squashed
# actor. Architectural approach (Tianshou / DreamerV3 / TorchRL idiom): the
# actor's mean head outputs are wrapped in `mean_scale * tanh(...)` so the
# pre-squash latent mean μ is hard-capped to ±mean_scale. Once the cap saturates,
# `dμ/d(Wh) = mean_scale * (1 - tanh²(Wh)) → 0`, so the head's gradient flow
# vanishes along the saturation-harvesting direction — the perverse incentive
# becomes self-defeating instead of runaway. mean_scale=3.0 spans tanh-squash's
# active region (tanh(3)≈0.995 reaches near-boundary actions cleanly).
#
# Compared to pmpo_d4_lnmean_relusq_v1 (LayerNorm before mean head): LN bounded
# the *input* to the head but the head's *weights* could grow over training and
# eventually push μ into saturation anyway. tanh-cap bounds the *output*
# directly, with the self-defeating-gradient property as a bonus.
...
class Agent(nn.Module):
    ...
    def get_action_distribution(self, x):
        ...
        action_mean = self.mean_scale * torch.tanh(raw_mean)
