# PMPO-D4 + ReluSq + Gaussian-constrained categorical bins v1.
#
# Catbins made PMPO optimize fixed action-space probability mass, but the free
# 51-way logits per dimension were too unstructured: PMPO had to learn a
# histogram rather than a smooth control law. This variant keeps the categorical
# PMPO machinery but constrains each per-dim categorical distribution to be the
# bin masses of a truncated Gaussian:
#
#   p_i(s) = integral_{bin_i} TruncNormal(mu(s), sigma_global)
#
# The actor controls only the state-dependent action-space location mu(s); a
# single learned global sigma vector controls exploration, like PPO's global
# log-std. There is no latent tanh/Jacobian term, no Beta concentration channel,
# and no arbitrary categorical simplex to collapse into unsupported histograms.
#
# No delight gate. The PMPO objective is the direct D4-style sign-split
# categorical log-prob objective plus categorical reverse KL over bin masses.
...
def normal_log_interval_prob(mean, std, lower_edges, upper_edges):
    lower = (lower_edges - mean.unsqueeze(-1)) / std.unsqueeze(-1)
    upper = (upper_edges - mean.unsqueeze(-1)) / std.unsqueeze(-1)
    out = torch.empty_like(lower)
    width = upper - lower
    small_width_mask = width < 1e-4

    if small_width_mask.any():
        midpoint = 0.5 * (lower[small_width_mask] + upper[small_width_mask])
        out[small_width_mask] = width[small_width_mask].log() - 0.5 * midpoint.square() - 0.5 * log(2.0 * pi)

    regular_mask = ~small_width_mask
    left_mask = (lower <= 0.0) & regular_mask
    if left_mask.any():
        log_cdf_upper = normal_log_cdf(upper[left_mask])
        log_cdf_lower = normal_log_cdf(lower[left_mask])
        out[left_mask] = logdiffexp(log_cdf_upper, log_cdf_lower)

    right_mask = (lower > 0.0) & regular_mask
    if right_mask.any():
        log_sf_lower = normal_log_cdf(-lower[right_mask])
        log_sf_upper = normal_log_cdf(-upper[right_mask])
        out[right_mask] = logdiffexp(log_sf_lower, log_sf_upper)
    return out
...
class Agent(nn.Module):
    def __init__(self, envs, action_bins=51):
        ...
        # Actor outputs state-dependent action-space Gaussian means. Bin logits
        # are derived from truncated Gaussian interval masses, not free logits.
        ...
        self.actor_logsigma = nn.Parameter(torch.zeros(1, action_dim))
    ...
    def get_action_params(self, x):
        unit_mu = torch.sigmoid(self.actor_mu(x))
        mean = self.action_low + (self.action_high - self.action_low) * unit_mu
        logsigma = self.actor_logsigma.expand_as(mean)
        sigma = logsigma.exp()
        return mean, sigma

    def get_action_logits(self, x):
        mean, sigma = self.get_action_params(x)
        edges = self.action_edges.unsqueeze(0).to(dtype=mean.dtype, device=mean.device)
        raw_log_probs = normal_log_interval_prob(mean.double(), sigma.double(), edges[..., :-1].double(), edges[..., 1:].double())
        support_log_prob = normal_log_interval_prob(
            mean.double(),
            sigma.double(),
            edges[..., :1].double(),
            edges[..., -1:].double(),
        )
        return (raw_log_probs - support_log_prob).to(mean.dtype)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Gaussian-constrained categorical actor diagnostics
        with torch.no_grad():
            ...
            current_mu, current_sigma = agent.get_action_params(b_obs)
            mean_mu = current_mu.mean().item()
            mean_abs_mu = current_mu.abs().mean().item()
            mean_sigma = current_sigma.mean().item()
            mean_logsigma = agent.actor_logsigma.mean().item()
        ...
        writer.add_scalar("diag/gaussbins_mean_max_prob", mean_max_prob, global_step)
        writer.add_scalar("diag/gaussbins_mean_bin_entropy", mean_bin_entropy, global_step)
        writer.add_scalar("diag/gaussbins_mean_selected_bin", mean_selected_bin, global_step)
        writer.add_scalar("diag/gaussbins_mean_logit_range", mean_logit_range, global_step)
        writer.add_scalar("diag/gaussbins_mean_min_log_prob", mean_min_log_prob, global_step)
        writer.add_scalar("diag/gaussbins_mean_edge_prob", mean_edge_prob, global_step)
        writer.add_scalar("diag/gaussbins_selected_edge_frac", selected_edge_frac, global_step)
        writer.add_scalar("diag/gaussbins_mean_mu", mean_mu, global_step)
        writer.add_scalar("diag/gaussbins_abs_mean_mu", mean_abs_mu, global_step)
        writer.add_scalar("diag/gaussbins_mean_sigma", mean_sigma, global_step)
        writer.add_scalar("diag/gaussbins_mean_logsigma", mean_logsigma, global_step)
