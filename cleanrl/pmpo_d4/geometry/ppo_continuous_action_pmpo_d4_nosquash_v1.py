# PMPO-D4 NoSquash v1: align with dreamer4's PMPO actor by dropping the tanh
# action squash. Actions are raw Normal(mu, sigma) samples; the env's
# ClipAction wrapper enforces action-space bounds before stepping.
#
# This removes the log|det J| Jacobian term from log_prob, so the reverse_KL
# penalty on the actor's Gaussian is now equal to the action-space KL —
# closing the gap that lets unbounded hidden activations decouple latent KL
# from action-space KL. With Tanh hidden activations (this file) the result
# is an apples-to-apples comparison against pmpo_d4_v1; the change also
# unlocks safe use of unbounded hidden activations (relu^2 / lrelusq) in
# follow-on variants.
#
# Diff vs pmpo_d4_v1:
#  - get_squashed_action_and_value: action = latent sample (no tanh), no Jacobian
#  - logprob = Normal.log_prob (no squash correction)
#  - action_center / action_scale registered buffers retained but unused
...
class Agent(nn.Module):
    ...
    def get_squashed_action_and_value(self, x, action=None):
        # NoSquash: action is the raw Normal sample. Env wrapper clips to bounds.
        # Naming kept (`get_squashed_*`) to avoid touching the rest of the loop.
        latent_dist = self.get_action_distribution(x)
        if action is None:
            action = latent_dist.sample()
        log_prob_per_dim = latent_dist.log_prob(action)
