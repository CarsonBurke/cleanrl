# PMPO-D4 + ReluSq + TanhNormal policy v4 — relax soft KL brake.
#
# v3 added z-storage + dynamic KL early-stop (target_kl=0.05). On HC at 3M:
# σ contracted to 0.95 (no v1-style blowup) but return tracked at ~75% of
# Beta v2's pace. Realized reverse_kl was only ~0.005/iter (PPO normally
# allows ~0.015). The hard cap at 0.05 rarely fires; the dominant brake is
# the *soft* `pmpo_kl_coef * KL` term inside the loss. At 0.3 it over-
# regularizes μ, σ updates and chokes learning.
#
# v4 lowers pmpo_kl_coef 0.3 → 0.1. The 0.05 hard cap stays as the
# runaway-σ defense; v1 showed σ can spiral with PMPO + state-dep σ, so we
# keep that backstop. Hypothesis: lighter soft penalty lets policy move per
# update; if σ tries to spiral like v1, the hard cap catches it.
#
