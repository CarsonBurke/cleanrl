# PMPO-D4 + signed-magnitude actor loss + ReluSq v1.
#
# Ablation: keep PMPO's tanh-squashed-Gaussian actor head, per-action log_prob
# (Jacobian included), and reverse-KL trust region. Replace the bucket-split
# sign-only weighting with PPO-style magnitude-conditioned form `-(adv * r).mean()`
# (no clip — reverse-KL is the trust region). Hidden activations are relu(x)^2.
#
# Compare against:
#   - relusq_v1 (PPO + relu^2, no squash):              works, ~3856
#   - pmpo_d4_relusq_v1 (PMPO bucket + relu^2 + squash): collapsed
# Isolates whether replacing the sign-only bucket weight with PPO-style magnitude
# weighting (while keeping squash + reverse-KL) recovers stability under relu^2.
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.policy_objective == "ppo":
                    ...
                    # Ablation: PPO-style magnitude-conditioned actor loss on the
                    # squashed-actor log_probs. Replaces PMPO's sign-bucket weight
                    # with `-(adv * ratio).mean()` (no clip). Trust region is
                    # PMPO's reverse-KL, kept from pmpo_d4_v1. Jacobians cancel
                    # inside the ratio (same action evaluated under both policies).
