# PMPO-D4 + ReluSq + TanhNormal action-cell policy with global std v2.
#
...
# global log-std vector like PPO's baseline. v2 also turns the PMPO target KL
# into a minibatch-level hard stop before the next optimizer step, instead of
# waiting until the end of an epoch.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if (
                    ...
                    and (not torch.isfinite(reverse_kl) or reverse_kl.item() > args.pmpo_target_kl)
                ):
                    stopped_by_kl = True
                    break
                ...
                optimizer.step()
                update_fraction = epoch + (end / args.batch_size)
        ...
        writer.add_scalar("losses/stopped_by_kl", float(stopped_by_kl), global_step)
        writer.add_scalar("losses/update_fraction", update_fraction, global_step)
