# PMPO-D4 NoSquash + LeakyReluSq v1: dreamer4-aligned PMPO (no tanh action
# squash) combined with (0.5*x + 0.5*relu(x))^2 hidden activations. Removing
# the squash makes latent KL == action KL, so unbounded hidden activations no
# longer decouple the reverse_KL trust region from the actual policy
# distribution. This is the principled way to use unbounded hidden
# activations under PMPO.
...
class LeakyReluSq(nn.Module):
    """f(x) = (0.5*x + 0.5*relu(x))^2 — x^2 for x>=0, 0.25*x^2 for x<0."""
    def forward(self, x):
