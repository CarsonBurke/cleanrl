# PMPO-D4 + LeakyReluSq + LayerNorm-before-mean v1.
#
# Path 2 fix for the unbounded-activation collapse: keep PMPO's tanh action
# squash, but insert nn.LayerNorm(64) immediately before the actor's final
# mean head. This re-imposes the bounded-latent precondition that the squash's
# Jacobian relies on, without giving up (0.5*x+0.5*relu(x))^2 capacity in the
# body. Actor only — critic is unchanged.
