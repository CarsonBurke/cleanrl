# PMPO-D4 + ReluSq + TanhNormal policy v1.
#
# Faithful port of Della Libera et al. (arxiv:2409.04971) SAC-TanhNormal recipe
# (their strongest baseline: 12198 ± 1054 on HalfCheetah-v4) into the PPO/PMPO
# stack. SAC-TanhNormal is a single-layer normalizing flow: sample z ~ N(μ,σ),
# action = action_center + action_scale · tanh(z). The PMPO-D4 trunk supplies
# the per-dim log-prob masking, advantage tanh weighting, and reverse-KL term
# from pmpo_d4_v1.
#
# Paper recipe details:
#   - Policy net outputs state-dependent (μ, log σ) — two heads.
#   - Log σ clipped to [-20, 2] (matches SAC's LOG_STD_MIN/MAX, Table 2).
#   - Independent action dims; latent dist = Normal(μ, σ) per dim.
#   - tanh squash, then linear remap [0,1] → [low, high] (envs are [-1,1] so
#     center=0, scale=1, but we keep the general form for portability).
#
# Differences from paper (intentional, to fit PPO/PMPO):
#   - 2×64 ReluSq body (CleanRL convention) instead of paper's 2×256 ReLU.
#   - PPO-style on-policy update with PMPO-D4 actor objective; no replay,
#     no Q network, no entropy temperature.
#
