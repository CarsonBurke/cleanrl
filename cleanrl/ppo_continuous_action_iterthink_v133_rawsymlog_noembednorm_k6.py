# PPO + IterThink v133 rawsymlog-noembednorm-k6 (v132 without obs-token RMSNorm).
#
...
#       z = symlog(raw_obs)
#       token_i = sum_k f_k(z_i) * W_{i,k} + b_i
#     where f_k = [z, |z|, z^2]. This bounds token scale without a running
#     normalizer while preserving scalar magnitude before the existing encoder
#     norms.
...
#   symlog tokenizer may keep optimization stable while making replayed prompt
#   windows invariant to when they are sampled. Unlike v132, do not normalize each
#   observation token immediately, because that may discard magnitude information.
#
# -----------------------------------------------------------------------------
# Previous header:
