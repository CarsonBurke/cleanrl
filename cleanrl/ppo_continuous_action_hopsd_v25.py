# HOPSD v25 — coherent exploration: AR(1)-correlated Beta deviations (OU in z space).
# =====================================================================================
...
class Args:
    ...
    explore_rho: float = 0.85        # v25: AR(1) coefficient for correlated exploration (0 = white/v19)
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if args.explore_rho > 0.0:
                    # v25: AR(1)-correlated exploration — carry the deviation from the
                    # policy mean across steps (killed on episode boundary), inject the
                    # fresh Beta deviation at sqrt(1-rho^2) so the marginal spread stays
                    # ~the policy's own. log_prob re-evaluated at the blended z (only
                    # diagnostics + the inert leash consume it; MLE/distill use z itself).
                    mean_z = r_alpha / (r_alpha + r_beta)
                    rho = args.explore_rho
                    dev = rho * explore_dev * (1.0 - next_done).unsqueeze(-1) + (
                        1.0 - rho * rho
                    ) ** 0.5 * (z - mean_z)
                    z = (mean_z + dev).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                    explore_dev = z - mean_z
