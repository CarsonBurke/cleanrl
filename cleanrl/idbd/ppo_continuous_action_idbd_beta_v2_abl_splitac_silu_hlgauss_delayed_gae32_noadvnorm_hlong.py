...
"""
**Long IDBD h-memory (20×)** on delayed mature GAE-32 + rewnorm + **noadvnorm**.

Paper: h ← [1−α]₊ h + Δw  → mean mem ~1/α ≈ 20 opt steps @α=0.05
Here:  h ← [1−κ·α]₊ h + Δw  with κ=0.05 → mem ~20× longer (~400 steps).
...
class Args:
    ...
    """κ in h←[1−κ·α]₊h+Δw; 1=paper (~20-step), this run=20× (~400-step @α0=0.05)"""
...
if __name__ == "__main__":
    ...
    print(
        f"delayed_gae32_noadvnorm_hlong: DELAYED GAE H={H} λ={args.gae_lambda} "
