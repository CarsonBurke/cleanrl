...
"""
Ablation: silu_hlgauss training loop + **ThinkTrunk** representation (separate).

Keep IDBD splitac, HL-Gauss critic, Beta actor, mb=num_envs, θ=0.05 — but replace
64×64 SiLU MLPs with iterthink_v24 ThinkTrunk (DenseNet residual + soft MoE,
ReLU²/RMSNorm) as separate actor/critic trunks (share_backbone=False).

Hypothesis: capacity/representation (not trust region or SiLU) is what separates
thin MLP IDBD (~4k) from ThinkTrunk PPO bases (~9k). Isolate trunk only.
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            ...
            f"ThinkTrunk(H={args.think_hidden},K={args.think_k},E={args.think_experts}) + HL-Gauss"
        )
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_thinktrunk: minibatch_size={args.minibatch_size} "
