...
"""
**Autostep-normalized, per-tensor IDBD** on the sepclip delayed-GAE-32 stream.

Why: vanilla IDBD's θ is not dimensionless — Δβ = θ·g·h assumes the per-weight
gradient·trace product is O(1), as in Sutton '92's binary-feature LMS testbed.
In this net the product is ~1e-7, so β froze at init in every prior run (paper
θ values ≡ θ_effective ~1e-9). Sutton's group hit exactly this and built
Autostep (Mahmood et al. 2012): normalize the meta gradient by a running max
so β moves O(θ) per step regardless of scale.

Second fix: v3_metascale showed the *per-weight* correlation signal is ~1e-4 of
full scale (dense backprop credit rotates; no per-weight "drifting feature"
structure). Meta signal is therefore aggregated **per tensor** (one β per
weight matrix): s = Σᵢ(-gᵢhᵢ), normalized by a slow-decaying running max of
|s|, so Δβ = θ·s/vmax ∈ [−θ, θ]. Noise cancels ~√N within a tensor; a
persistent layer-level "step too small/large" signal survives.

β bounded to [−10, ln(max_α)]; h trace per-param as before ((1−α)₊ decay).
Everything else identical to sepclip.
"""
import math
import os
import random
...
class IDBD(Optimizer):
    """Per-tensor Autostep-normalized Incremental Delta-Bar-Delta.

    Weight update is Sutton '92 IDBD; the meta update is aggregated per tensor
    and normalized by a running max (Autostep, Mahmood et al. 2012) so θ is
    dimensionless: Δβ = θ·Σ(-g·h)/vmax ∈ [−θ, θ]. β is shared per tensor.
    ...
    def __init__(
        ...
        if autostep_tau <= 1.0:
            raise ValueError(f"Invalid autostep_tau: {autostep_tau}")
        defaults = dict(
            lr=lr, meta_lr=meta_lr, max_alpha=max_alpha, eps=eps,
            paper_beta_bounds=paper_beta_bounds, autostep_tau=autostep_tau,
        )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            autostep_tau = group["autostep_tau"]
            ...
            for p in group["params"]:
                ...
                if len(state) == 0:
                    ...
                    state["vmax"] = torch.zeros((), device=p.device, dtype=p.dtype)
                ...
                # Per-tensor Autostep meta: s = Σᵢ(-gᵢhᵢ), normalized by a
                # slow-decaying running max of |s| ⇒ Δβ = θ·s/vmax ∈ [−θ, θ],
                # one shared β step per tensor (β starts uniform, stays uniform)
                s = meta_signal.sum()
                vmax.copy_(torch.maximum(vmax * (1.0 - 1.0 / autostep_tau), s.abs()))
                d_beta = meta_lr * s / (vmax + 1e-12)
                beta.add_(d_beta)
...
class Args:
    ...
    # IDBD-specific
    idbd_meta_lr: float = 0.01
    """meta step-size θ; with Autostep normalization Δβ∈[−θ,θ] per step, so θ
    is the max fractional α change per optim step (dimensionless by design)"""
    idbd_autostep_tau: float = 1e4
    """decay horizon (optim steps) of the running max normalizer vmax"""
    ...
    idbd_meta_lr_stream: float = 0.0
    """θ for correlated delayed stream (0 = use idbd_meta_lr; normalized meta
    doesn't need the tiny stream θ)"""
    paper_beta_bounds: bool = False
    """if True: clamp |Δβ|≤2 and β≥−10 (paper practical bounds)"""
...
if __name__ == "__main__":
    ...
    actor_optimizer = IDBD(
        ...
        autostep_tau=args.idbd_autostep_tau,
    )
    ...
    print(
        f"idbd_autostep_v1 (per-tensor Autostep meta, θ={args.idbd_meta_lr}, "
        f"τ_vmax={args.idbd_autostep_tau:g}): DELAYED GAE H={H} λ={args.gae_lambda} "
        f"every-step optim oldest (n={args.num_envs}), separate actor/critic grad clip "
        f"(max_norm={args.max_grad_norm} each), "
        f"meta_lr={args.idbd_meta_lr}, bounds={args.paper_beta_bounds}, "
        f"α0={args.learning_rate}, τ={args.target_tau}, rewnorm, noadvnorm"
    )
