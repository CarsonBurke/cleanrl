...
"""
**v3 base**: delayed GAE-32 + rewnorm + noadvnorm with:
  - separate actor/critic **weight** grad clips
  - IDBD meta (β) on **raw** g; clip_coef only on weight path

Queue variants via CLI:
  1) control
  2) --idbd-meta-scale unit_rms
  3) --paper-theta  (θ=0.05, stream 0, β bounds)
  4) --paper-x
  5) --paper-lms
  6) --max-grad-norm 0
"""
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            f"torch.compile + v3 H={H} sep-wclip meta-raw scale={args.idbd_meta_scale} "
    ...
    print(
        f"delayed_v3: H={H} λ={args.gae_lambda} n={args.num_envs} "
