...
"""
Incremental + env reward norm + linear HL-Gauss + **no advnorm**.

Parallel to incr_d3retnorm_norewnorm (raw + d3 retnorm). This arm keeps the
batch-noadvnorm recipe that worked offline (~4.9k):

- **NormalizeReward** on (returns stay on normalized scale)
- HL-Gauss **linear** support [−20, 20], no symlog — matches reward-norm range
- **norm_adv=False** — raw 1-step TD advantages (no mean/std on n=16)
- no d3 retnorm

Still: 1×num_envs step→optim, splitac IDBD, SiLU, θ=0.05, α_init=0.05.
"""
import math
import os
import random
...
class Args:
    ...
    norm_adv: bool = False
    """no mean/std advnorm — raw 1-step TD advantages (batch noadvnorm win)"""
    clip_coef: float = 0.2
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_incr_noadvnorm: INCREMENTAL "
        f"(1×{args.num_envs} env-step → optim), rewnorm, noadvnorm, "
        f"update_epochs={args.update_epochs}, meta_lr={args.idbd_meta_lr}, "
        f"init_alpha={args.learning_rate}, "
        f"HL-Gauss bins={args.num_bins} linear=[{args.v_min},{args.v_max}], SiLU"
    )
