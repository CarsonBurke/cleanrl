# TD7 baseline + IDBD optimizer (Sutton 1992).
# Based on cleanrl/td7_continuous_action.py; Adam → IDBD for actor/critic/encoder.
# Defaults: α_init=0.05 (paper), total_timesteps=1e6 for ablation queue.
#
# TD7 = TD3 + 4 additions:
...
from cleanrl.idbd.idbd_opt import IDBD
...
class Args:
    ...
    batch_size: int = 256
    """the batch size of sample from the replay memory (try 32 for more IDBD-like updates)"""
    gamma: float = 0.99
    ...
    actor_lr: float = 0.05
    """initial IDBD α for actor (Sutton 1992 paper init)"""
    critic_lr: float = 0.05
    """initial IDBD α for critic"""
    encoder_lr: float = 0.05
    """initial IDBD α for encoder"""
    idbd_meta_lr: float = 0.05
    """IDBD meta step-size θ"""
    idbd_max_alpha: float = 1.0
    """hard cap on per-parameter α"""
    paper_beta_bounds: bool = False
    """if True: clamp |Δβ|≤2 and β≥−10 (paper practical bounds)"""
    paper_theta: bool = False
    """if True: force θ=0.05 and paper_beta_bounds=True at startup"""
...
if __name__ == "__main__":
    ...
    print(
        f"td7_idbd: batch={args.batch_size} θ={args.idbd_meta_lr} "
        f"bounds={args.paper_beta_bounds} α0_a/c/e={args.actor_lr}/{args.critic_lr}/{args.encoder_lr} "
