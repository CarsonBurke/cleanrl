...
"""
**v4 rankwin**: sepclip delayed-GAE-32 stream + running-window rank→probit advantages.

Motivation: rankgauss (rank over the full 2048-step rollout) is the best 8M
finisher (6240) despite far staler data than the delayed-32 stream (5831).
Rank shaping is bounded and outlier-proof — a mean/std or EMA-RMS divisor is a
noisy scalar that amplifies outliers at mb=16 and amplifies noise when |A|
collapses; a rank map has fixed, deterministic magnitudes regardless of SNR.

Here the mature advantages (16/step) are ranked within a sliding window of the
last `adv_rank_window` (4096 ≈ 256 steps) mature advantages, then mapped
uniform-quantile → probit N(0,1). Combines rankgauss's shaping with the fresh
delayed stream. Everything else identical to sepclip (IDBD kept, inert).
"""
import math
import os
import random
...
class Args:
    ...
    adv_rank_window: int = 4096
    """sliding window of mature advantages for rank→probit shaping (0 = raw adv)"""
...
if __name__ == "__main__":
    ...
    # sliding window of recent mature advantages for rank→probit shaping
    W = args.adv_rank_window
    adv_win = torch.zeros(max(W, 1), device=device)
    adv_win_len = 0
    adv_win_ptr = 0
    ...
    print(
        f"v4_rankwin (rank window W={args.adv_rank_window}): DELAYED GAE H={H} λ={args.gae_lambda} "
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if W > 0:
                # rank each mature advantage within the last W mature advantages,
                # then uniform quantile → probit N(0,1) (bounded, outlier-proof)
                flat = mb_adv.view(-1)
                k = flat.numel()
                ins = (adv_win_ptr + torch.arange(k, device=device)) % W
                adv_win[ins] = flat
                adv_win_ptr = (adv_win_ptr + k) % W
                adv_win_len = min(adv_win_len + k, W)
                sorted_win, _ = torch.sort(adv_win[:adv_win_len])
                ranks = torch.searchsorted(sorted_win, flat).to(torch.float32)
                uq = ((ranks + 0.5) / adv_win_len).clamp(1e-4, 1.0 - 1e-4)
                mb_adv = math.sqrt(2.0) * torch.erfinv(2.0 * uq - 1.0)
