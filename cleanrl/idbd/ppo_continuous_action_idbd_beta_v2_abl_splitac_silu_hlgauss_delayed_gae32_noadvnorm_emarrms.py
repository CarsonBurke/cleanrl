...
"""
**EMA-RMS advantage scale** on delayed GAE-32 + rewnorm + noadvnorm.

Not d3 retnorm: scale from **advantages** A, not return percentiles.
  s ← (1-τ)s + τ·RMS(A)     (slow EMA; effective N ≫ n=16)
  A ← A / max(ε, s)         (keep actor SNR ~O(1) as raw |A| shrinks)

Critic targets (returns) unscaled. No batch mean/std, no rankgauss, no floor=1.
Hypothesis: delayed plateau is |A| collapse under fixed α; slow adv RMS fixes it.
"""
import math
import os
import random
...
class Args:
    ...
    norm_adv: bool = False
    """no per-batch mean/std advnorm — EMA-RMS is the sole A scaler"""
    ema_rms_adv: bool = True
    """scale policy advantages by slow EMA of RMS(A)"""
    ema_rms_tau: float = 1e-3
    """EMA rate on rms(A); small ⇒ long memory, stable on n=num_envs"""
    ema_rms_eps: float = 1e-8
    """floor on scale (div0 only; unlike d3 floor=1 this still amplifies small A)"""
    clip_coef: float = 0.2
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            f"torch.compile + delayed GAE H={H} + EMA-RMS(A) "
    ...
    ema_rms_s = 1.0  # EMA of rms(A); start at 1 so early A unchanged until data arrives
    ema_rms_inited = False
    ...
    print(
        f"delayed_gae32_noadvnorm_emarrms: DELAYED GAE H={H} λ={args.gae_lambda} "
        f"every-step optim oldest (n={args.num_envs}), EMA-RMS(A) τ={args.ema_rms_tau}, "
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.ema_rms_adv:
            with torch.no_grad():
                batch_rms = mb_adv.detach().pow(2).mean().sqrt().item()
                if not ema_rms_inited:
                    ema_rms_s = max(batch_rms, args.ema_rms_eps)
                    ema_rms_inited = True
                else:
                    ema_rms_s = (1.0 - args.ema_rms_tau) * ema_rms_s + args.ema_rms_tau * batch_rms
                scale = max(ema_rms_s, args.ema_rms_eps)
            mb_adv = mb_adv / scale
        ...
        if global_step % args.log_every < args.num_envs or iteration == args.num_iterations:
            ...
            writer.add_scalar("charts/ema_rms_adv_scale", ema_rms_s, global_step)
