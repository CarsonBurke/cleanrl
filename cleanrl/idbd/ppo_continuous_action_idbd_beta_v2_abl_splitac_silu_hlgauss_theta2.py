...
"""
Ablation: silu_hlgauss base with θ=2.0 (IDBD meta_lr).

Extreme meta rate (40× paper demo θ=0.05). Forces α off the init plateau
or exposes instability. Pair with theta05 for θ sensitivity.

Hypothesis: if α still frozen → unit-feature IDBD has no usable g·h signal;
if α explodes/returns crash → θ must sit in a narrow band (or needs bounds).
"""
import math
import os
import random
...
class Args:
    ...
    # IDBD-specific
    idbd_meta_lr: float = 2.0
    """meta step-size θ = 2.0 (extreme; force α adaptation or blow-up)"""
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_theta2: minibatch_size={args.minibatch_size} "
        f"(~{args.batch_size // args.minibatch_size} steps/epoch × {args.update_epochs} epochs), "
        f"meta_lr={args.idbd_meta_lr} (θ=2.0), init_alpha={args.learning_rate}, "
