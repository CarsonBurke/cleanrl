...
"""
**Long IDBD h-memory (200×)** on delayed mature GAE-32 + rewnorm + **noadvnorm**.

Paper: h ← [1−α]₊ h + Δw  → mean mem ~1/α ≈ 20 opt steps @α=0.05
Here:  h ← [1−κ·α]₊ h + Δw  with κ=0.005 → mem ~200× longer (~4000 steps).

Same delayed cadence / scale stack as delayed_gae32_noadvnorm winner. Not rankgauss.
"""
import math
import os
import random
...
class IDBD(Optimizer):
    ...
    def __init__(
        ...
        defaults = dict(
            lr=lr, meta_lr=meta_lr, max_alpha=max_alpha,
            h_decay_scale=h_decay_scale, eps=eps,
        )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Long h-memory: h ← [1 − κ·α]₊ h + Δw  (κ=1 → paper)
...
class Args:
    ...
    idbd_h_decay_scale: float = 0.005
    """κ in h←[1−κ·α]₊h+Δw; 1=paper (~20-step), this run=200× (~4000-step @α0=0.05)"""
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            f"torch.compile + delayed GAE H={H} + LONG h κ={args.idbd_h_decay_scale} "
    ...
    print(
        f"delayed_gae32_noadvnorm_hlong200: DELAYED GAE H={H} λ={args.gae_lambda} "
        f"every-step optim oldest (n={args.num_envs}), "
        f"κ_h={args.idbd_h_decay_scale} (~{1.0/(args.idbd_h_decay_scale*args.learning_rate):.0f}-step h mem), "
