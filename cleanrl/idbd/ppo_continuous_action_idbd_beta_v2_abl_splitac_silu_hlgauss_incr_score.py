...
"""
Incremental + pure score-function policy loss (post-incr improvement).

Base: 1×num_envs env-step → optim, 1-step TD. Each sample is used once
on-policy, so PPO's ratio clip is a leftover trust region from multi-epoch
PPO — not well-motivated here. Replace clipped surrogate with A2C-style
  L_π = -E[A · log π(z|s)]
(joint critic CE unchanged). Hypothesis: if incr works, unclipped score
uses full gradient; clip only shrinks already-small single-pass steps.
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_incr_score: INCREMENTAL A2C-score "
        f"(1×{args.num_envs} env-step → optim, no PPO clip), "
        f"meta_lr={args.idbd_meta_lr}, init_alpha={args.learning_rate}, "
        f"HL-Gauss bins={args.num_bins}, SiLU"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            # Pure score (A2C): no ratio / clip. ratio kept only for KL diagnostics.
            pg_loss = -(mb_advantages.detach() * newlogprob).mean()
