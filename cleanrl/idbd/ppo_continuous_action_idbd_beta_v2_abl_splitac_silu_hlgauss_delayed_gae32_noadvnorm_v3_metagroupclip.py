...
"""
**v3 + meta as its own grad-clip group** (meta non-raw).

v3 base: delayed GAE-32, rewnorm, noadvnorm, sep **weight** clips for actor vs critic.

Change vs v3 (meta-raw):
  - backward leaves raw g
  - **Meta clip group** = all actor+critic params together → c_meta
  - **Weight groups** = actor alone → c_actor, critic alone → c_critic
  - β update uses g_meta = c_meta · g_raw   (non-raw; own group)
  - w update uses g_w = c_head · g_raw
  - h tracks actual Δw from g_w

Hypothesis: meta should track the clipped joint scale (one group), not pre-clip
spikes; weight heads still clipped separately.
"""
import math
import os
import random
...
class IDBD(Optimizer):
    ...
    @torch.no_grad()
    def step(self, closure=None, clip_coef: float = 1.0, clip_coef_meta: float = 1.0):
        """IDBD step. `p.grad` must be **raw** (unclipped).

        clip_coef_meta: scales g for β meta-update (meta's own clip group).
        clip_coef: scales g for weight update / actual Δw → h.
        """
        loss = None
        if closure is not None:
            with torch.enable_grad():
                loss = closure()
        if clip_coef <= 0.0 or clip_coef_meta <= 0.0:
            raise ValueError(f"Invalid clip_coef={clip_coef} meta={clip_coef_meta}")
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Meta non-raw: apply meta-group clip scale first
                g_for_meta = grad if clip_coef_meta == 1.0 else grad * clip_coef_meta
                g_meta = g_for_meta / g_rms if meta_scale == "unit_rms" else g_for_meta
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            f"torch.compile + v3_metagroupclip H={H} sep-wclip meta-group scale={args.idbd_meta_scale} "
    ...
    print(
        f"delayed_v3_metagroupclip: H={H} λ={args.gae_lambda} n={args.num_envs} "
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for opt in optimizers:
            opt.zero_grad()
        loss.backward()
        # Meta = own clip group (all params). Weights = per-head groups.
        clip_coef_actor = 1.0
        ...
        if args.max_grad_norm > 0.0:
            ...
            # Meta group: joint actor+critic (non-raw meta)
            clip_coef_meta = _coef(actor_params + critic_params)
            if args.weight_clip_sep:
                clip_coef_actor = _coef(actor_params)
                clip_coef_critic = _coef(critic_params)
            else:
                clip_coef_actor = clip_coef_critic = clip_coef_meta
        actor_optimizer.step(clip_coef=clip_coef_actor, clip_coef_meta=clip_coef_meta)
        critic_optimizer.step(clip_coef=clip_coef_critic, clip_coef_meta=clip_coef_meta)
        ...
        if global_step % args.log_every < args.num_envs or iteration == args.num_iterations:
            ...
            writer.add_scalar("idbd/clip_coef_meta", clip_coef_meta, global_step)
            ...
            print(
                f"SPS: {sps}  ca={clip_coef_actor:.2f} cc={clip_coef_critic:.2f} cm={clip_coef_meta:.2f}  "
