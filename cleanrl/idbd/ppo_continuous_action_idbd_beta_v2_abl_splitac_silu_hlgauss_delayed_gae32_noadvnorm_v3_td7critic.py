...
"""
**v3 + TD7-ish critic** (no rankgauss / PMPO / MPO).

v3 base: delayed GAE-32, rewnorm, noadvnorm, sep weight clip, meta on raw g.

Critic-side scale control inspired by TD7 target clipping, adapted for soft Polyak:

  **EMA target value clip** — bootstrap V and critic return targets are clamped
  to [min_target, max_target]. Bounds track unclamped return extrema with a
  slow EMA (not TD7's periodic hard snapshot — that couples to hard target
  nets; we use Polyak, so EMA is the natural analogue).

No AvgL1Norm (little benefit with NormalizeObservation + V(s) head).
Actor still uses raw GAE (no rankgauss / PMPO).
Hypothesis: lagging clamp on V/returns curbs bootstrap explosion without
...
class Args:
    ...
    td7_target_clip: bool = True
    """clamp bootstrap V / returns to EMA [min_target, max_target] of return extrema"""
    td7_target_clip_tau: float = 0.005
    """EMA rate for min/max bounds (aligned with Polyak τ; TD7 uses hard snapshot every 250)"""
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            f"torch.compile + v3_td7critic H={H} sep-wclip meta-raw scale={args.idbd_meta_scale} "
    ...
    # EMA target-value clip bounds (TD7 idea; soft update for Polyak regime)
    min_target = 0.0
    max_target = 0.0
    range_inited = False

    print(
        f"delayed_v3_td7critic: H={H} λ={args.gae_lambda} n={args.num_envs} "
        ...
        f"td7clip={args.td7_target_clip} τ_clip={args.td7_target_clip_tau}"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # TD7: clamp bootstrap through snapshotted target range
            if args.td7_target_clip and range_inited:
                next_value = next_value.clamp(min_target, max_target)
            ...
            for t in reversed(range(H)):
                if t == H - 1:
                    ...
                    if args.td7_target_clip and range_inited:
                        nextvalues = nextvalues.clamp(min_target, max_target)
            ...
            # Track unclamped return range (TD7 tracks Q_target extrema)
            if args.td7_target_clip:
                rmin = float(mb_ret.min().item())
                rmax = float(mb_ret.max().item())
                tau = args.td7_target_clip_tau
                if not range_inited:
                    min_target, max_target = rmin, rmax
                    range_inited = True
                else:
                    # EMA of batch extrema (smooth; expands/contracts without hard lag)
                    min_target = (1.0 - tau) * min_target + tau * rmin
                    max_target = (1.0 - tau) * max_target + tau * rmax
                    if max_target < min_target:
                        min_target, max_target = max_target, min_target
                # Critic / bootstrap use EMA clamp; policy A stays raw GAE
                mb_ret = mb_ret.clamp(min_target, max_target)
        ...
        if global_step % args.log_every < args.num_envs or iteration == args.num_iterations:
            ...
            writer.add_scalar("charts/td7_min_target", min_target, global_step)
            writer.add_scalar("charts/td7_max_target", max_target, global_step)
            ...
            print(
                f"SPS: {sps}  ca={clip_coef_actor:.2f} cc={clip_coef_critic:.2f}  "
