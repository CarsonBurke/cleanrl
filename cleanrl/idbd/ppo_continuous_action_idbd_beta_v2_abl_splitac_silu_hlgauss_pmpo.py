...
"""
Ablation: silu_hlgauss + dreamer4 PMPO (no advnorm), env-step minibatch (16).

Same splitac IDBD + SiLU + HL-Gauss + mb=num_envs, but policy objective is PMPO:
  - sign-soft weight |tanh(A)| on per-dim Beta log-prob of replayed z
  - balanced pos/neg averages, w=0.5
  - reverse KL(old||new) trust region (coef 0.3) using rollout concentrations
  - norm_adv=False (raw GAE → tanh; no mean/std, no rankgauss)

IDBD-friendly: many small steps of size 16 (not large Adam minibatches).
"""
import math
import os
import random
...
class Args:
    ...
    norm_adv: bool = False
    """PMPO uses raw GAE → tanh(|·|); no mean/std advnorm"""
    clip_coef: float = 0.2
    """unused for PMPO actor (kept for clipfrac diagnostic / optional vloss)"""
    ...
    """unused under PMPO (KL trust region via pmpo_kl_coef)"""
    # dreamer4 PMPO
    pmpo_pos_to_neg_weight: float = 0.5
    """weight on positive-advantage class; negative uses 1-w"""
    pmpo_kl_coef: float = 0.3
    """reverse KL(old rollout || new) coefficient"""
    pmpo_reverse_kl: bool = True
    """True: KL(old||new); False: KL(new||old)"""
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            f"torch.compile(agent, mode={args.compile_mode!r}) + splitac IDBD + SiLU + HL-Gauss + PMPO "
            f"(no advnorm, mb={args.minibatch_size})"
        )
    ...
    # Rollout Beta concentrations for closed-form reverse KL trust region
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_pmpo: minibatch_size={args.minibatch_size} "
        ...
        f"HL-Gauss bins={args.num_bins}, SiLU, PMPO noadvnorm kl_coef={args.pmpo_kl_coef}"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # ALGO LOGIC: action logic (store native Beta z + rollout concentrations)
            with torch.no_grad():
                action, z, logprob, _, value, dist = agent.get_action_and_value(next_obs, hl_support)
                values[step] = value.flatten()
                old_alphas[step] = dist.concentration1
                old_betas[step] = dist.concentration0
            actions[step] = action
            zs[step] = z
        ...
        # Optimizing the policy and value network (many small IDBD steps, mb=num_envs)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, newvalue, new_dist = agent.get_action_and_value(
                ...
                with torch.no_grad():
                    ...
                    beta_conc_means.append(
                        (0.5 * (new_dist.concentration1 + new_dist.concentration0)).mean().item()
                    )
                    beta_ent_means.append(entropy.mean().item())

                # PMPO: raw GAE → soft sign weights; no advnorm
                mb_advantages = b_advantages[mb_inds]
                log_prob_per_dim = new_dist.log_prob(b_zs[mb_inds])  # (mb, act_dim)
                ...
                if args.pmpo_kl_coef > 0.0:
                    kl_loss = (
                        ...
                        else kl_divergence(new_dist, old_dist).sum(-1).mean()
                    )
        ...
        if reverse_kl_count > 0:
            writer.add_scalar(
                "losses/reverse_kl", (reverse_kl_sum / reverse_kl_count).item(), global_step
            )
        print(
