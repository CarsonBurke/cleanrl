...
"""
Incremental + Polyak target critic (post-incr improvement).

Base: 1×num_envs env-step → optim, 1-step TD, splitac+SiLU+HL-Gauss.

Online TD bootstraps V(s') from the same net that just trained → moving
target / feedback. Soft-copy critic (τ=0.005) for bootstrap only; train
online critic on projected TD targets. Hypothesis: stabler δ → cleaner
IDBD g/h correlation and better late returns if pure incr works.
"""
import math
import os
import random
...
class Args:
    ...
    """Polyak τ for critic target: θ̄ ← (1-τ)θ̄ + τθ after each optim"""
...
if __name__ == "__main__":
    ...
    # Target critic: same init, soft-updated; used only for TD bootstrap V(s').
    # Keep online_critic ref BEFORE compile so Polyak always sees real params.
    import copy
    ...
    if args.compile:
        ...
        print(
            f"torch.compile(agent, mode={args.compile_mode!r}) + incr + target-V "
            f"(τ={args.target_tau})"
        )
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_incr_targetv: INCREMENTAL+targetV "
        f"(1×{args.num_envs} env-step → optim, τ={args.target_tau}), "
        f"update_epochs={args.update_epochs}, meta_lr={args.idbd_meta_lr}, "
        f"init_alpha={args.learning_rate}, HL-Gauss bins={args.num_bins}, SiLU"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # 1-step TD with target-critic bootstrap (online; λ unused)
        with torch.no_grad():
            next_logits = critic_target(next_obs)
            next_value = hl_support.to_scalar(next_logits).view(-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            # Polyak update target critic toward online critic
            with torch.no_grad():
