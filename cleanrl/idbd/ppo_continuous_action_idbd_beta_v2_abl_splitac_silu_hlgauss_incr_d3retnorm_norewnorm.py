...
"""
Incremental (1×num_envs step→optim) + **no advnorm** + **no env reward norm**
+ **DreamerV3 retnorm**.

Pure incr used norm_adv=True and gym NormalizeReward — brittle on n=16 and
distorts the scale retnorm/HL-Gauss should handle. This rebuild:

- **norm_adv=False** (no mean/std on the 16 parallel TD advantages)
- **no NormalizeReward** — raw rewards, clip ±10 only
- **d3 retnorm** — policy A /= max(floor, EMA(P95−P5)) over TD returns;
  critic targets stay unscaled
- HL-Gauss **value_symlog=True**, support [−10,10] in symlog (raw returns)

Still: splitac IDBD, SiLU, 1-step TD, mb=num_envs, θ=0.05, α_init=0.05.
"""
import math
import os
import random
...
class Args:
    ...
    num_steps: int = 1
    """unused for collection length; kept for CLI compat (incremental = 1 env-step)"""
    anneal_lr: bool = False
    """LR annealing (off: IDBD adapts α; if on, re-inits β toward annealed α)"""
    gamma: float = 0.99
    """the discount factor gamma"""
    gae_lambda: float = 0.0
    """unused: incremental mode uses 1-step TD (λ=0)"""
    ...
    norm_adv: bool = False
    """no mean/std advnorm — d3 retnorm is the sole policy advantage scaler"""
    # DreamerV3-style return percentile scale (retnorm, impl=perc)
    ret_percnorm: bool = True
    """scale policy advantages by return percentile spread S=max(floor, P95-P5)"""
    ...
    # HL-Gauss critic on raw returns via symlog
    num_bins: int = 101
    """categorical value bins"""
    v_min: float = -10.0
    """HL-Gauss support min in **symlog** coords (raw returns)"""
    v_max: float = 10.0
    """HL-Gauss support max in **symlog** coords"""
    ...
    log_every: int = 2048
    """log SPS / IDBD stats every N env-steps (global_step units = num_envs * steps)"""
    # to be filled in runtime
    batch_size: int = 0
    """samples per optim step (= num_envs)"""
    minibatch_size: int = 0
    """same as batch_size (one mb of all parallel envs)"""
    num_iterations: int = 0
    """number of env-steps (vectorized) = total_timesteps // num_envs"""
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    # One vectorized env-step → optim on all parallel envs
    ...
    hl_support = HLGaussSupport(
        ...
        args.sigma_ratio,
        device,
        use_symlog=args.value_symlog,  # True: raw returns via symlog support
    )
    agent = Agent(envs, args.num_bins).to(device)
    # Build optimizers on uncompiled modules so param identity is stable.
    ...
    if args.compile:
        ...
        print(f"torch.compile(agent, mode={args.compile_mode!r}) + split actor/critic IDBD + SiLU + HL-Gauss")
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_incr_d3retnorm_norewnorm: INCREMENTAL "
        f"(1×{args.num_envs} env-step → optim), noadvnorm, d3retnorm, NO rewnorm, "
        f"update_epochs={args.update_epochs}, meta_lr={args.idbd_meta_lr}, "
        f"init_alpha={args.learning_rate}, "
        f"HL-Gauss bins={args.num_bins} symlog=[{args.v_min},{args.v_max}], SiLU"
    )

    # DreamerV3 retnorm (perc): EMA of return P5/P95
    ema_ret_lo, ema_ret_hi = 0.0, 1.0
    ema_perc_inited = False
    ret_perc_scale = 1.0

    # last optim metrics for periodic logging
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # 1-step TD advantage / bootstrap return (online; λ unused)
        with torch.no_grad():
            next_value = agent.get_value(next_obs, hl_support).view(-1)
            # bootstrap through truncations/terminations like CleanRL next_done
            td_target = reward_t + args.gamma * next_value * (1.0 - next_done)
            advantages = td_target - value
            returns = td_target  # critic target (unscaled; HL-Gauss on raw/symlog)

            # DreamerV3 retnorm: scale *policy* advantages only by EMA return spread
            if args.ret_percnorm:
                qs = torch.tensor([args.ret_perc_lo, args.ret_perc_hi], device=device)
                lo, hi = torch.quantile(returns.detach(), qs).tolist()
                ...
                advantages = advantages / ret_perc_scale
        ...
        for epoch in range(args.update_epochs):
            ...
            # no mean/std advnorm — A already retnorm-scaled
            ...
            newvalue_logits = agent.get_value_logits(b_obs)
            if args.clip_vloss:
                newvalue_expected = newvalue.view(-1)
                v_clipped = b_values + torch.clamp(
                    newvalue_expected - b_values,
                    -args.clip_coef,
                    args.clip_coef,
                )
                target_unclipped = b_returns
                target_clipped = v_clipped + (b_returns - b_values)
                log_probs = torch.log_softmax(newvalue_logits, dim=-1)
                v_loss_unclipped = -(hl_support.project(target_unclipped) * log_probs).sum(-1)
                v_loss_clipped = -(hl_support.project(target_clipped) * log_probs).sum(-1)
                v_loss = torch.max(v_loss_unclipped, v_loss_clipped).mean()
            else:
                target_probs = hl_support.project(b_returns)
                log_probs = torch.log_softmax(newvalue_logits, dim=-1)
        ...
        # periodic logging (every log_every env-steps of progress)
