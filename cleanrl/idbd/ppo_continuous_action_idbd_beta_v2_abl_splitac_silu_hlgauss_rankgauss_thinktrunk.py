...
"""
Ablation: **rankgauss** advantages + **ThinkTrunk** (separate A/C).

Combines the overnight winner (rankgauss ~6.3k on 64×64 SiLU) with iterthink_v24
ThinkTrunks for actor and critic (share_backbone=False), still IDBD splitac +
HL-Gauss + Beta + mb=num_envs + θ=0.05.

- norm_adv=False; batch rank→probit GAE (erfinv) before PPO clip
- ThinkTrunk H=64, K=3, E=16 (DenseNet residual + soft MoE, ReLU²/RMSNorm)

Hypothesis: rankgauss's magnitude-free shaping stacks with trunk capacity
that alone was middling (~3.5k mid-run).
"""
import math
import os
import random
...
class Args:
    ...
    norm_adv: bool = False
    """off: rankgauss replaces mean/std advnorm"""
    adv_transform: str = "rankgauss"
    """rank → U(0,1) quantile → N(0,1) probit (batch-level)"""
    cdf_probit_clamp: float = 0.999
    """clamp |2u-1| before erfinv to bound tails"""
    clip_coef: float = 0.2
    ...
    # ThinkTrunk (iterthink_v24 defaults)
    think_hidden: int = 64
    """ThinkTrunk width H"""
    think_k: int = 3
    """number of ThinkBlocks K"""
    think_experts: int = 16
    """soft MoE experts per block"""
    compile: bool = True
...
class BetaActor(nn.Module):
    """ThinkTrunk + unimodal Beta concentration heads (splitac actor module)."""

    def __init__(self, obs_dim: int, act_dim: int, H: int, K: int, n_experts: int):
        super().__init__()
        self.trunk = ThinkTrunk(obs_dim, H, K, n_experts)
...
class CriticTrunk(nn.Module):
    """ThinkTrunk + HL-Gauss logits head (splitac critic module)."""

    def __init__(self, obs_dim: int, num_bins: int, H: int, K: int, n_experts: int):
        super().__init__()
        self.trunk = ThinkTrunk(obs_dim, H, K, n_experts)
        self.head = layer_init(nn.Linear(H, num_bins), std=1.0)
...
class Agent(nn.Module):
    """Unimodal Beta actor + HL-Gauss critic on separate ThinkTrunks."""

    def __init__(self, envs, num_bins: int, H: int, K: int, n_experts: int):
        super().__init__()
        obs_dim = int(np.array(envs.single_observation_space.shape).prod())
        act_dim = int(np.prod(envs.single_action_space.shape))
        self.act_dim = act_dim
        self.num_bins = num_bins
        self.critic = CriticTrunk(obs_dim, num_bins, H, K, n_experts)
        self.actor = BetaActor(obs_dim, act_dim, H, K, n_experts)
...
if __name__ == "__main__":
    ...
    hl_support = HLGaussSupport(
        ...
        args.sigma_ratio,
        device,
        use_symlog=False,  # reward-normalized returns stay in linear space
    )
    agent = Agent(
        envs, args.num_bins, args.think_hidden, args.think_k, args.think_experts
    ).to(device)
    ...
    if args.compile:
        ...
        print(
            f"torch.compile(agent, mode={args.compile_mode!r}) + splitac IDBD + "
            f"ThinkTrunk(H={args.think_hidden},K={args.think_k},E={args.think_experts}) + "
            f"HL-Gauss + rankgauss"
        )
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_rankgauss_thinktrunk: "
        f"mb={args.minibatch_size} (~{args.batch_size // args.minibatch_size} steps/epoch), "
        f"meta_lr={args.idbd_meta_lr}, init_alpha={args.learning_rate}, "
        f"HL-Gauss bins={args.num_bins}, ThinkTrunk H={args.think_hidden} K={args.think_k} E={args.think_experts}, "
        f"adv=rankgauss"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, _, value, dist = agent.get_action_and_value(next_obs, hl_support)
        ...
        # rankgauss: rank → uniform quantile → probit Gaussian (erfinv on CPU)
        if args.adv_transform == "rankgauss":
            ...
            b_advantages = ((2.0 ** 0.5) * torch.erfinv(centered.cpu())).to(device)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, newvalue, dist = agent.get_action_and_value(
                    b_obs[mb_inds], hl_support, z=b_zs[mb_inds]
                )
