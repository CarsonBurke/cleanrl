...
"""
**Weight-only grad clip** on delayed GAE-32 + rewnorm + noadvnorm.

Standard stack clips p.grad before IDBD → meta sees rescale factor that
changes every step and corrupts corr(g, h).

Here:
  - backward leaves raw grads
  - compute global clip_coef = min(1, max_norm / ||g||)
  - IDBD meta (β) uses **raw** g
  - weight step uses g_w = clip_coef · g; h tracks **actual** Δw = −α g_w

Hypothesis: unclipped meta path lets β/α move; weight path stays stable.
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            f"torch.compile + delayed GAE H={H} + weight-only clip "
    ...
    print(
        f"delayed_gae32_noadvnorm_wclip: DELAYED GAE H={H} λ={args.gae_lambda} "
        f"every-step optim oldest (n={args.num_envs}), weight-only grad clip "
        f"(max_norm={args.max_grad_norm}), meta on raw g, "
        f"meta_lr={args.idbd_meta_lr}, α0={args.learning_rate}, τ={args.target_tau}, rewnorm, noadvnorm"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for opt in optimizers:
            opt.zero_grad()
        loss.backward()
        # Global clip coef for **weights only**; leave p.grad raw for IDBD meta.
        clip_coef = 1.0
        if args.max_grad_norm > 0.0:
            params_with_grad = [p for p in (actor_params + critic_params) if p.grad is not None]
            if params_with_grad:
                total_norm = torch.norm(
                    torch.stack([p.grad.detach().norm(2) for p in params_with_grad]), 2
                )
                clip_coef = float(
                    torch.clamp(args.max_grad_norm / (total_norm + 1e-6), max=1.0).item()
                )
        for opt in optimizers:
            opt.step(clip_coef=clip_coef)
        ...
        if global_step % args.log_every < args.num_envs or iteration == args.num_iterations:
            ...
            writer.add_scalar("idbd/clip_coef", clip_coef, global_step)
