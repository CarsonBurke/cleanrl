...
"""
Truncated **32-step GAE** (λ=0.95) on splitac+SiLU+HL-Gauss+noadvnorm.

Between pure incr (num_steps=1, 1-step TD) and batch (num_steps=2048):
  collect 32 env-steps × num_envs → GAE λ=0.95 → IDBD optim with mb=num_envs.

Horizon is truncated: early indices in the window get longer multi-step
backups; the last step is ~1-step + V(s'). Bootstrap V closes the window.

Defaults otherwise match batch noadvnorm: rewnorm, linear HL-Gauss [−20,20],
norm_adv=False, α_init=0.05, θ=0.05, update_epochs=1.
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_gae32: num_steps={args.num_steps} "
        ...
        f"HL-Gauss linear=[{args.v_min},{args.v_max}], SiLU, noadvnorm, rewnorm"
    )
