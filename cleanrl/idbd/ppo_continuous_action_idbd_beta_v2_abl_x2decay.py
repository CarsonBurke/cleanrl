...
def install_linear_input_hooks(root: nn.Module):
    """Store per-Linear last-input energy x² (mean over batch) for IDBD decay."""
    x2_by_weight_id = {}

    def make_hook(mod: nn.Linear):
        def hook(_module, inputs, _output):
            x = inputs[0].detach()
            # (..., in) -> mean square over all but last dim
            flat = x.reshape(-1, x.shape[-1])
            x2_by_weight_id[id(mod.weight)] = (flat * flat).mean(dim=0)
        return hook
    ...
    return x2_by_weight_id, handles
...
class IDBD(Optimizer):
    ...
    def __init__(
        ...
        max_alpha: float = 0.1,
        eps: float = 1e-8,
        x2_by_weight_id: Optional[Dict[int, torch.Tensor]] = None,
    ):
        if lr <= 0.0:
        ...
        self.x2_by_weight_id = x2_by_weight_id if x2_by_weight_id is not None else {}
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Trace: h ← [1 − α ⊙ x²]₊ h + Δw  (paper eq.6; x² from last Linear input)
                x2 = self.x2_by_weight_id.get(id(p))
                if x2 is not None and p.dim() == 2 and x2.numel() == p.shape[1]:
                    # weight (out, in): broadcast x² over out rows
                    decay = (1.0 - alpha * x2.unsqueeze(0)).clamp(min=0.0)
                else:
...
if __name__ == "__main__":
    ...
    # Hooks must bind to the real modules (before compile wraps them).
    x2_by_weight_id, _hook_handles = install_linear_input_hooks(agent)
    if args.compile:
        agent = torch.compile(agent, mode=args.compile_mode)
        print(f"torch.compile(agent, mode={args.compile_mode!r}) + x² IDBD decay")
    optimizer = IDBD(
        agent.parameters(),
        lr=args.learning_rate,
        meta_lr=args.idbd_meta_lr,
        max_alpha=args.idbd_max_alpha,
        x2_by_weight_id=x2_by_weight_id,
    )
    ...
    print(
        f"IDBD Beta v2 abl_x2decay: minibatch_size={args.minibatch_size} "
