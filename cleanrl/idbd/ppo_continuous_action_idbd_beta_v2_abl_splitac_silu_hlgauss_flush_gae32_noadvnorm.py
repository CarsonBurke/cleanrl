...
"""
**Flush GAE-32**: collect 32 → GAE → optim full batch → **throw away**.

Tests polish hypothesis vs delayed mature GAE:
  - Frozen π during 32-step collect (on-policy batch)
  - GAE λ=0.95 on full window
  - One epoch, mb=num_envs, **time-ordered** (no shuffle) over all 32×envs
  - Discard buffer; next collect is fresh
  - Polyak target V for bootstrap; same rewnorm + noadvnorm + HL linear as winner

Contrast delayed: every-step optim on 1 mature sample only (under-reuse ceiling).
"""
import math
import os
import random
...
class Args:
    ...
    """Polyak τ for bootstrap V (0 = online V)"""
    shuffle_minibatches: bool = False
    """if False, optim in temporal order (older→newer within the 32)"""
...
if __name__ == "__main__":
    ...
    if args.compile:
        ...
        print(
            f"torch.compile + FLUSH GAE-32 (collect→optim all→throw away) "
            f"τ={args.target_tau} shuffle={args.shuffle_minibatches}"
        )
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_flush_gae32_noadvnorm: FLUSH collect-{args.num_steps} "
        f"gae_λ={args.gae_lambda} mb={args.minibatch_size} "
        f"(~{args.batch_size // args.minibatch_size} IDBD steps/epoch × {args.update_epochs} epochs), "
        f"meta_lr={args.idbd_meta_lr}, init_alpha={args.learning_rate}, "
        f"HL-Gauss linear=[{args.v_min},{args.v_max}], SiLU, noadvnorm, rewnorm, throw-away each batch"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            if args.target_tau > 0:
                next_value = hl_support.to_scalar(critic_target(next_obs)).reshape(1, -1)
            else:
        ...
        for epoch in range(args.update_epochs):
            if args.shuffle_minibatches:
                np.random.shuffle(b_inds)
            # else: temporal order (step-major flatten is t0 all envs, t1 all envs, ...)
