...
"""
Delayed mature GAE H=32 + rewnorm + noadvnorm + **PMPO** (no retnorm).

Delayed ring optim every step on oldest full-horizon sample. Scale stack matches
winning noadvnorm: NormalizeReward, no advnorm, no retnorm, HL linear [−20,20].
Policy objective is dreamer4 PMPO (|tanh(A)|, balanced pos/neg, reverse KL 0.3).
"""
import math
import os
import random
...
class Args:
    ...
    """unused under PMPO (KL via pmpo_kl_coef)"""
...
if __name__ == "__main__":
    ...
    buf_old_alphas = torch.zeros((H, args.num_envs) + act_dim, device=device)
    buf_old_betas = torch.zeros((H, args.num_envs) + act_dim, device=device)
    filled = 0
    ...
    print(
        f"splitac_silu_hlgauss_delayed_gae32_noadvnorm_pmpo: DELAYED mature GAE H={H} λ={args.gae_lambda} "
        f"every-step optim on oldest only (n={args.num_envs}), "
        f"meta_lr={args.idbd_meta_lr}, α0={args.learning_rate}, τ={args.target_tau}, rewnorm, noadvnorm, PMPO, HL linear"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if filled < H:
            ...
            buf_old_alphas[:-1].copy_(buf_old_alphas[1:].clone())
            buf_old_betas[:-1].copy_(buf_old_betas[1:].clone())
            idx = H - 1
        ...
        buf_old_alphas[idx] = dist.concentration1
        buf_old_betas[idx] = dist.concentration0
        ...
        with torch.no_grad():
            ...
            mb_old_alphas = buf_old_alphas[0]
            mb_old_betas = buf_old_betas[0]
        ...
        # PMPO: raw A → |tanh(A)| weights; balanced pos/neg; reverse KL to rollout Beta
        new_dist = dist
        log_prob_per_dim = new_dist.log_prob(mb_zs)  # (num_envs, act_dim)
        adv_weight = mb_adv.tanh().abs().unsqueeze(-1)
        signed_log_probs = log_prob_per_dim * adv_weight
        pos_mask = (mb_adv >= 0).unsqueeze(-1).expand_as(log_prob_per_dim)
        neg_mask = ~pos_mask
        ...
        old_dist = Beta(mb_old_alphas, mb_old_betas)
