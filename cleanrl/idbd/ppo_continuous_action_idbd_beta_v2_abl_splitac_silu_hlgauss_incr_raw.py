...
"""
Incremental + **fully raw** advantages (no rewnorm, no retnorm, no advnorm).

A/B vs incr_d3retnorm_norewnorm (retnorm) and incr_noadvnorm (env rewnorm):

- **no NormalizeReward** — raw rewards, clip ±10 only
- **no d3 retnorm** — policy sees raw 1-step TD advantages
- **norm_adv=False** — no mean/std either
...
class Args:
    ...
    """off: no d3 retnorm — raw TD advantages only"""
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_incr_raw: INCREMENTAL "
        f"(1×{args.num_envs} env-step → optim), RAW A (no rewnorm/retnorm/advnorm), "
