...
"""
Ablation: silu_hlgauss + rankgauss advantages (instead of mean/std advnorm).

Replace per-minibatch (A-mean)/std with batch-level rank → uniform quantile
→ probit Gaussian (erfinv). Magnitude-free, outlier-immune; v19/gsde win.

norm_adv=False; adv_transform=rankgauss on full rollout batch before PPO clip.
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_rankgauss: minibatch_size={args.minibatch_size} "
        ...
        f"HL-Gauss bins={args.num_bins}, SiLU, adv=rankgauss (no mean/std)"
    )
