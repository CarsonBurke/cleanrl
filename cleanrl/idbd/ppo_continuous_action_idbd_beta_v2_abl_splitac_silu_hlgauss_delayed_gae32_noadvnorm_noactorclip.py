...
if __name__ == "__main__":
    ...
    print(
        f"splitac_silu_hlgauss_delayed_gae32_noadvnorm_noactorclip: DELAYED mature GAE H={H} λ={args.gae_lambda} "
        f"every-step optim on oldest only (n={args.num_envs}), "
        f"meta_lr={args.idbd_meta_lr}, α0={args.learning_rate}, τ={args.target_tau}, rewnorm, noadvnorm, NO actor gradclip"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for opt in optimizers:
            opt.zero_grad()
        loss.backward()
        # critic-only grad clip; actor grads unclipped
