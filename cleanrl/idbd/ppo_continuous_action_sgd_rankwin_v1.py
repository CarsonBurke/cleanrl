...
"""
**Consolidation: SGD stream + rank-window advantages (+ anneal via flag)**.

Composes the two independent wins from the streaming ablation campaign:
  - sgdstream_anneal (5259 last-20, +20-30% faster to 4.4k @2M than sepclip's
    constant step): linear LR anneal on the explicit-SGD sepclip loop.
  - rank→probit advantage shaping over a sliding window (v4_rankwin; rankgauss
    lineage — bounded, outlier-proof credit weights at any advantage SNR).

Hypothesis: anneal's mid-training speed and rank's late-training slope are
mechanistically independent (step-size policy vs credit robustness) and stack.
Run with --anneal-lr. Everything else identical to sgd_stream_v1.
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    print(
        f"sgd_rankwin_v1 (rank window W={args.adv_rank_window}): DELAYED GAE H={H} λ={args.gae_lambda} "
