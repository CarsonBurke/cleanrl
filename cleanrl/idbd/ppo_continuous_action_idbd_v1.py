...
"""
PPO + Incremental Delta-Bar-Delta (IDBD) optimizer (v1).

Key idea
--------
Replace Adam with Sutton's IDBD (1992): per-parameter log step-sizes β with
α = exp(β), meta-learned online from the correlation between the current
gradient and a decaying memory h of recent weight updates. Same-sign
successive gradients increase α; opposing signs decrease it.

Hypothesis
----------
IDBD wants *incremental* updates (one example at a time). PPO's large
minibatches average away the signal that adapts α. Using minibatch size =
num_envs (one vectorized env-step, "batch size 1 × env count") yields many
more backprops per rollout and should let per-parameter step-sizes track
nonstationarity in the policy/value losses. LR annealing is off by default
because IDBD already adapts α.

Reference: Sutton 1992, "Adapting bias by gradient descent: An incremental
version of delta-bar-delta". Linear demo: ../IncrementalDeltaBarDelta/idbd.py
"""
import math
import os
import random
...
def log_idbd_diagnostics(writer, idbd: dict, step: int, prefix: str = "idbd"):
    """Write IDBD health metrics. `step` is usually cumulative optimizer updates."""
    writer.add_scalar(f"{prefix}/alpha_mean", idbd["alpha_mean"], step)
    writer.add_scalar(f"{prefix}/alpha_median", idbd["alpha_median"], step)
    writer.add_scalar(f"{prefix}/alpha_std", idbd["alpha_std"], step)
    writer.add_scalar(f"{prefix}/alpha_p10", idbd["alpha_p10"], step)
    writer.add_scalar(f"{prefix}/alpha_p90", idbd["alpha_p90"], step)
    writer.add_scalar(f"{prefix}/alpha_min", idbd["alpha_min"], step)
    writer.add_scalar(f"{prefix}/alpha_max", idbd["alpha_max"], step)
    writer.add_scalar(f"{prefix}/alpha_geom_mean", idbd["alpha_log_mean"], step)
    writer.add_scalar(f"{prefix}/alpha_vs_init_ratio", idbd["alpha_vs_init_ratio"], step)
    writer.add_scalar(f"{prefix}/frac_at_max", idbd["frac_at_max"], step)
    writer.add_scalar(f"{prefix}/frac_above_init", idbd["frac_above_init"], step)
    writer.add_scalar(f"{prefix}/frac_below_init", idbd["frac_below_init"], step)
    writer.add_scalar(f"{prefix}/meta_dot_mean", idbd["meta_dot_mean"], step)
    writer.add_scalar(f"{prefix}/meta_abs_mean", idbd["meta_abs_mean"], step)
    writer.add_scalar(f"{prefix}/grow_frac", idbd["grow_frac"], step)
    writer.add_scalar(f"{prefix}/h_abs_mean", idbd["h_abs_mean"], step)
    writer.add_scalar(f"{prefix}/eff_step_mean", idbd["eff_step_mean"], step)
...
class Args:
    ...
    """initial per-parameter step-size α = exp(β) for IDBD"""
    num_envs: int = 1
    ...
    """meta step-size θ for updating log step-sizes β (Sutton 1992 uses 0.05)"""
    idbd_max_alpha: float = 0.1
    """hard cap on per-parameter α = exp(β)"""
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD v1: minibatch_size={args.minibatch_size} "
        f"(~{args.batch_size // args.minibatch_size} steps/epoch × {args.update_epochs} epochs), "
        f"meta_lr={args.idbd_meta_lr}, init_alpha={args.learning_rate}"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # IDBD snapshot once per rollout (aligned with env global_step / episodic returns)
        idbd = optimizer.pop_diagnostics()
        log_idbd_diagnostics(writer, idbd, global_step)
