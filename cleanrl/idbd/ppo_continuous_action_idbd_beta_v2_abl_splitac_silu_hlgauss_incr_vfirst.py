...
"""
Incremental + critic-first (post-incr improvement).

Base: 1×num_envs env-step → optim. Joint actor+critic step uses a stale V
for advantages while both nets move. Here: (1) critic-only IDBD step on TD
target, (2) recompute V and A with updated critic, (3) actor-only PPO step.

Hypothesis: fresher advantages every step → better policy gradient and less
value–policy interference under aggressive online IDBD.
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_incr_vfirst: INCREMENTAL critic-first "
        f"(1×{args.num_envs} env-step → V then π), "
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ----- critic-first, then actor (split phases) -----
        clipfracs = []
        beta_conc_means = []
        beta_ent_means = []

        # (1) Critic-only step on fixed TD returns
        ...
        # (2) Refresh V and advantages with updated critic
        with torch.no_grad():
            value = agent.get_value(b_obs, hl_support).view(-1)
            next_value = agent.get_value(next_obs, hl_support).view(-1)
            td_target = reward_t + args.gamma * next_value * (1.0 - next_done)
            b_advantages = td_target - value
            b_returns = td_target
            b_values = value

        # (3) Actor-only PPO step
        _, _, newlogprob, entropy, _, dist = agent.get_action_and_value(
