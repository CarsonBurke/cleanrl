...
"""
**Long IDBD h-memory** on rankgauss base (not GAE horizon).

Standard unit-feature IDBD:
  h ← [1 − α]₊ h + Δw     with α≈0.05 ⇒ decay 0.95, mean memory ~1/α ≈ 20 opt steps

Here:
  h ← [1 − κ·α]₊ h + Δw   with κ=idbd_h_decay_scale≪1
  κ=0.05 ⇒ decay ≈0.9975, mean memory ~1/(κα) ≈ 400 opt steps (~20× longer)

Hypothesis: short h makes meta-signal meta_dot≈0 / α stuck; longer reward/update
memory lets successive grads correlate so β actually moves and asymptote lifts.
Base: rankgauss + splitac + SiLU + HL-Gauss + rewnorm (current best ~6.3k).
"""
import math
import os
import random
...
class IDBD(Optimizer):
    """Parameter-wise Incremental Delta-Bar-Delta (Sutton 1992) + long h-memory.

    Unit-feature port with optional slow h decay:

        β  ← β - θ · g · h
        α  ← exp(β)   (clamped)
        w  ← w - α · g
        h  ← [1 - κ·α]₊ · h + Δw     (Δw = -α g; κ=1 recovers paper)

    κ≪1 lengthens h (reward/update memory) without changing the weight step.
    """
    ...
    def __init__(
        ...
        if h_decay_scale <= 0.0:
            raise ValueError(f"Invalid h_decay_scale: {h_decay_scale}")
        defaults = dict(
            lr=lr,
            meta_lr=meta_lr,
            max_alpha=max_alpha,
            h_decay_scale=h_decay_scale,
            eps=eps,
        )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            h_decay_scale = group["h_decay_scale"]
            ...
            for p in group["params"]:
                ...
                # Long memory: h ← [1 − κ·α]₊ h + Δw  (κ=1 → paper)
                decay = (1.0 - h_decay_scale * alpha).clamp_(min=0.0)
...
class Args:
    ...
    idbd_h_decay_scale: float = 0.05
    """κ in h←[1−κ·α]₊h+Δw; 1=paper (~20-step mem @α=0.05), 0.05≈400-step mem"""
...
if __name__ == "__main__":
    ...
    actor_optimizer = IDBD(
        ...
        h_decay_scale=args.idbd_h_decay_scale,
    )
    ...
    if args.compile:
        ...
        print(
            f"torch.compile(agent, mode={args.compile_mode!r}) + rankgauss + "
            f"LONG h-memory κ={args.idbd_h_decay_scale}"
        )
    ...
    print(
        f"IDBD Beta v2 rankgauss_hlong: minibatch_size={args.minibatch_size} "
        f"(~{args.batch_size // args.minibatch_size} steps/epoch × {args.update_epochs} epochs), "
        f"meta_lr={args.idbd_meta_lr}, α0={args.learning_rate}, κ_h={args.idbd_h_decay_scale} "
        f"(~{1.0 / (args.idbd_h_decay_scale * args.learning_rate):.0f}-step h mem @α0), "
        f"HL-Gauss bins={args.num_bins}, SiLU, adv=rankgauss"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # GPU erfinv flaky on some boxes). Self-normalizing, magnitude-free.
