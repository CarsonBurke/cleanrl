...
"""
**Delayed mature GAE H=256** (much longer horizon vs H=32 plateau).

Hypothesis: H=32 still truncates GAE mass ((γλ)^32≈0.14 residual); H=256
fully saturates multi-step credit under λ=0.95 / γ=0.99. Same delayed
cadence (optim oldest only every step), rewnorm + noadvnorm + HL linear.

Tradeoff: mature sample is H steps stale (more off-policy) but near-MC GAE.
"""
import math
import os
import random
...
class Args:
    ...
    num_steps: int = 256
    """GAE / ring horizon H; 256 saturates (γλ)^H≪1 vs residual ~0.14 at H=32"""
...
if __name__ == "__main__":
    ...
    print(
        f"splitac_silu_hlgauss_delayed_gae256_noadvnorm: DELAYED mature GAE H={H} λ={args.gae_lambda} "
        f"every-step optim on oldest only (n={args.num_envs}), "
        f"meta_lr={args.idbd_meta_lr}, α0={args.learning_rate}, τ={args.target_tau}, rewnorm, noadvnorm, HL linear"
    )
