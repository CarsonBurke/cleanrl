...
"""
PPO + IDBD optimizer + unimodal Beta action policy (v1).

Same as idbd_v1 (Sutton 1992 per-parameter step-sizes, incremental minibatches)
but the Gaussian actor is replaced by dreamer4 / iterthink-v215 unimodal Beta:

    alpha = 1 + softplus(head_a)
    beta  = 1 + softplus(head_b)     # both >= 1 => unimodal on (0,1)
    z ~ Beta(alpha, beta)
    action = low + (high - low) * z  # affine; constant Jacobian drops from PPO ratio

Native z is stored and replayed for log_prob (v215 z-replay). Entropy is the
closed-form Beta entropy (not squashed).

Hypothesis: Beta explores with bounded support and state-dependent concentration;
IDBD should allocate larger α to concentration heads that consistently reduce
policy loss under incremental updates.
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v1: minibatch_size={args.minibatch_size} "
