...
"""
Ablation: d3retnorm without env reward normalization (IDBD Beta v2).

Base combo (splitac + SiLU + HL-Gauss) plus:

- **noadvnorm** — GAE advantages not mean/std-normalized per batch
- **d3retnorm** — DreamerV3 retnorm: scale advantages by EMA(P95−P5) of
  returns (floor=1); critic targets stay unscaled
- **norewnorm** — drop gym `NormalizeReward`; keep per-step reward clip ±10.
  Raw-scale returns ⇒ HL-Gauss with value_symlog=True, support [−10,10]
  in symlog coords (~±2e4 raw).

Hypothesis: env reward norm distorts the return scale that retnorm and
HL-Gauss already handle; raw rewards + retnorm may give cleaner GAE
magnitudes and a critic that sees true return magnitudes via symlog.

Unimodal Beta (v215); α_init=0.05, θ=0.05, max_α=1, epochs=1, mb=num_envs.
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_d3retnorm_norewnorm: minibatch_size={args.minibatch_size} "
        ...
        f"HL-Gauss bins={args.num_bins} symlog=[{args.v_min},{args.v_max}], SiLU, noadvnorm, d3retnorm, NO rewnorm"
    )
