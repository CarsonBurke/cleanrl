...
"""
**32-step truncated GAE** + d3 retnorm + no env reward norm.

- num_steps=32, gae_lambda=0.95
- no NormalizeReward (clip ±10); HL-Gauss symlog [−10,10]
- norm_adv=False; d3 retnorm on GAE advantages only
- critic targets unscaled
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_gae32_d3retnorm_norewnorm: num_steps={args.num_steps} "
        f"gae_λ={args.gae_lambda} mb={args.minibatch_size} "
        f"(~{args.batch_size // args.minibatch_size} IDBD steps/epoch), "
        f"meta_lr={args.idbd_meta_lr}, init_alpha={args.learning_rate}, "
        f"HL-Gauss symlog=[{args.v_min},{args.v_max}], SiLU, noadvnorm, d3retnorm, NO rewnorm"
    )
