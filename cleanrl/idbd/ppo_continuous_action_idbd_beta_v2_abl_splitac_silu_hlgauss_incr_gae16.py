...
"""
Semi-online short GAE (post-incr improvement).

If pure 1-step incr works (IDBD likes frequent updates), pure TD(0) still has
high bias / myopic credit. Collect **num_steps=16** (not 2048), GAE λ=0.95,
then optim with mb=num_envs — ~128× more IDBD steps per env-step than the
2048 rollout, but multi-step advantages.

Hypothesis: sweet spot between pure incr (λ=0, every step) and batch PPO
(λ=0.95, 2048) for credit assignment + h timescale.
"""
import math
import os
import random
...
class Args:
    ...
    num_steps: int = 16
    """short horizon: 16-step GAE rollouts (semi-online vs 2048 / pure-1-step)"""
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_incr_gae16: short-GAE "
        f"num_steps={args.num_steps} mb={args.minibatch_size} "
