...
"""
Ablation: splitac + SiLU + HL-Gauss critic (IDBD Beta v2).

Combines three changes on top of IDBD Beta v2 paper defaults
(α_init=0.05, θ=0.05, max_α=1.0, update_epochs=1, mb=num_envs):

1. **splitac** — separate actor/critic IDBD optimizers so policy and value
   meta-learning of α do not share state / gradient mixing in one optimizer.
2. **SiLU** — nn.SiLU() instead of Tanh in actor trunk and critic MLP
   (smoother grads than Tanh saturations under large IDBD steps).
3. **HL-Gauss** — categorical critic (num_bins=101, support [-20,20] on
   reward-normalized returns, σ=0.75·bin_width, no symlog); value loss is
   cross-entropy on projected return targets; GAE still uses scalar E[bin].

Unimodal Beta (v215 / dreamer4): concentrations = 1 + softplus; z ~ Beta;
action = affine of z. Optional whole-agent torch.compile(reduce-overhead).
Optimizers are built on uncompiled modules so param identity stays stable.
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_incr: INCREMENTAL "
        f"(1×{args.num_envs} env-step → optim), "
        f"update_epochs={args.update_epochs}, meta_lr={args.idbd_meta_lr}, "
        f"init_alpha={args.learning_rate}, "
        f"HL-Gauss bins={args.num_bins} support=[{args.v_min},{args.v_max}], SiLU"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            returns = td_target  # critic target
