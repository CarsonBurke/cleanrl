...
class Args:
    ...
    """scale GAE advantages by return percentile spread S=max(floor, P95-P5)"""
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_d3retnorm: minibatch_size={args.minibatch_size} "
        ...
        f"HL-Gauss bins={args.num_bins} support=[{args.v_min},{args.v_max}], SiLU, noadvnorm, d3retnorm"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # DreamerV3 retnorm: scale policy advantages by return percentile spread
        # (critic targets `returns` stay unscaled for HL-Gauss)
        if args.ret_percnorm:
            flat_ret = returns.reshape(-1)
            qs = torch.tensor([args.ret_perc_lo, args.ret_perc_hi], device=device)
            lo, hi = torch.quantile(flat_ret.detach(), qs).tolist()
