...
"""
**v3 + one-sided advantage scale floor** (fix |A| starvation, not full whitening).

Why full EMA-RMS advnorm failed (delayed noadvnorm + A/=EMA[rms(A)]):
  - Early: large raw A divided by S_A~5–9 → **kills** delayed's fast early climb
  - Mid/late: S_A collapses to ~0.15–0.2 → **5–7× amplify** noisy n=16 A every step
  - That is unit-variance forcing, not a floor: destroys magnitude-as-confidence early
    and over-boosts residual noise late (then max_grad_norm clips to a noise sphere)
  - Same family of mistake as d3 retnorm on the actor (gain on A), worse timing

This fix — **one-sided**:
  s ← EMA[ mean(|A|) ]   (robust-ish scale on n=16)
  A ← A * min( max_boost, max(1, s_ref / s) )
  - never shrinks large early A (gain ≥ 1)
  - only boosts when scale falls below s_ref
  - max_boost caps late noise amplification

v3 base otherwise: delayed GAE-32, rewnorm, noadvnorm, sep weight clip, meta raw g.
"""
import math
import os
import random
...
class IDBD(Optimizer):
    ...
    -δx. Updates (elementwise):
        ...
        β  ← β - θ · g_raw · h          (meta: unclipped grad)
        α  ← exp(β)   (clamped)
        w  ← w - α · (c · g_raw)         (weights: clipped by coef c)
        h  ← [1 - α]₊ · h + Δw           (h tracks actual weight change)

    Optional clip_coef c∈(0,1] scales only the weight path.
    ...
    def __init__(
        ...
        if meta_scale not in ("none", "unit_rms"):
            raise ValueError(f"Invalid meta_scale: {meta_scale}")
        defaults = dict(
            lr=lr, meta_lr=meta_lr, max_alpha=max_alpha, eps=eps,
            meta_scale=meta_scale, paper_beta_bounds=paper_beta_bounds, paper_x=paper_x,
        )
    ...
    @torch.no_grad()
    def step(self, closure=None, clip_coef: float = 1.0):
        """IDBD step. `p.grad` must be **unclipped**.

        clip_coef scales only the weight update (and thus h via actual Δw).
        Meta-update β uses raw g so global-norm rescaling cannot wipe correlations.
        """
        loss = None
        if closure is not None:
            with torch.enable_grad():
                loss = closure()
        if clip_coef <= 0.0:
            raise ValueError(f"Invalid clip_coef: {clip_coef}")

        for group in self.param_groups:
            meta_lr = group["meta_lr"]
            max_alpha = group["max_alpha"]
            meta_scale = group["meta_scale"]
            paper_beta_bounds = group["paper_beta_bounds"]
            paper_x = group["paper_x"]
            eps = group["eps"]
            init_beta = math.log(group["lr"])

            need_rms = meta_scale == "unit_rms" or paper_x
            if need_rms:
                sq = [p.grad.detach().pow(2).mean() for p in group["params"] if p.grad is not None]
                g_rms = torch.sqrt(torch.stack(sq).mean()).item() if sq else 1.0
                g_rms = max(g_rms, eps)
                self._acc["meta_g_rms"] += g_rms
                self._acc["n_groups"] += 1
            else:
                g_rms = 1.0
            ...
            for p in group["params"]:
                ...
                g_meta = grad / g_rms if meta_scale == "unit_rms" else grad
                meta_signal = -(g_meta * h)
                n = grad.numel()
                self._acc["n"] += n
                ...
                grad_w = grad if clip_coef == 1.0 else grad * clip_coef
                step = alpha * grad_w
                self._acc["eff_step"] += step.abs().sum().item()
                p.add_(step, alpha=-1.0)

                if paper_x:
                    # paper-like: decay only when "feature" active; x² ∝ g² normalized
                    x2 = (grad.detach().pow(2) / (g_rms * g_rms + eps)).clamp(
                        max=(1.0 / alpha.clamp(min=eps))
                    )
                    decay = (1.0 - alpha * x2).clamp(min=0.0)
                else:
                    decay = (1.0 - alpha).clamp(min=0.0)
    ...
    def pop_diagnostics(self):
        ...
        ng = max(self._acc["n_groups"], 1)
        out["meta_g_rms"] = self._acc["meta_g_rms"] / ng
...
class Args:
    ...
    """θ for correlated delayed stream (0 = use idbd_meta_lr). Use 0 for paper θ."""
    idbd_meta_scale: str = "none"
    """none | unit_rms — scale g for β only to unit RMS"""
    paper_beta_bounds: bool = False
    """paper: β≥−10 and |Δβ|≤2 per step"""
    paper_x: bool = False
    """h decay with activity x²∝g² instead of unit x≡1"""
    paper_lms: bool = False
    """A2C score + MSE value (closer to LMS δ²)"""
    paper_theta: bool = False
    """if True: force θ=0.05, stream=0, paper_beta_bounds=True at startup"""
    weight_clip_sep: bool = True
    """separate actor/critic weight clip coefs (meta still on raw g)"""
    onesided_adv: bool = True
    """one-sided A scale floor: boost only when EMA|A| < s_ref"""
    onesided_adv_tau: float = 1e-3
    """EMA rate on mean(|A|)"""
    onesided_adv_ref: float = 1.0
    """target scale in rewnorm-GAE units; boost when s < ref"""
    onesided_adv_max_boost: float = 4.0
    """cap on amplification (prevents late n=16 noise blow-up)"""
    onesided_adv_eps: float = 1e-6
    """floor on EMA scale for division"""
...
if __name__ == "__main__":
    ...
    if args.paper_theta:
        args.idbd_meta_lr_stream = 0.0
        args.idbd_meta_lr = 0.05
        args.paper_beta_bounds = True
    ...
    actor_optimizer = IDBD(
        ...
        meta_scale=args.idbd_meta_scale,
        paper_beta_bounds=args.paper_beta_bounds,
        paper_x=args.paper_x,
    )
    ...
    if args.compile:
        ...
        print(
            f"torch.compile + v3_onesidedA H={H} sep-wclip meta-raw scale={args.idbd_meta_scale} "
            f"paper_x={args.paper_x} lms={args.paper_lms} θ={args.idbd_meta_lr} "
            f"max_gn={args.max_grad_norm} τ={args.target_tau}"
        )
    ...
    # one-sided A scale: EMA of mean(|A|); gain = clamp(ref/s, 1, max_boost)
    ema_abs_a = 1.0
    ema_abs_inited = False
    last_adv_gain = 1.0

    print(
        f"delayed_v3_onesidedA: H={H} λ={args.gae_lambda} n={args.num_envs} "
        f"meta_scale={args.idbd_meta_scale} paper_x={args.paper_x} paper_lms={args.paper_lms} "
        f"bounds={args.paper_beta_bounds} θ={args.idbd_meta_lr} max_gn={args.max_grad_norm} "
        f"sep={args.weight_clip_sep} α0={args.learning_rate} rewnorm noadvnorm "
        f"onesidedA ref={args.onesided_adv_ref} maxboost={args.onesided_adv_max_boost} τ={args.onesided_adv_tau}"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.onesided_adv:
            with torch.no_grad():
                batch_abs = mb_adv.detach().abs().mean().item()
                if not ema_abs_inited:
                    ema_abs_a = max(batch_abs, args.onesided_adv_eps)
                    ema_abs_inited = True
                else:
                    ema_abs_a = (
                        (1.0 - args.onesided_adv_tau) * ema_abs_a
                        + args.onesided_adv_tau * batch_abs
                    )
                s = max(ema_abs_a, args.onesided_adv_eps)
                # gain ≥ 1: never dampen; ≤ max_boost: no late explode
                gain = min(
                    args.onesided_adv_max_boost,
                    max(1.0, args.onesided_adv_ref / s),
                )
                last_adv_gain = gain
            mb_adv = mb_adv * gain
        elif args.norm_adv and mb_adv.numel() > 1:
        ...
        if args.paper_lms:
            v_pred = hl_support.to_scalar(newvalue_logits)
        ...
        for opt in optimizers:
            opt.zero_grad()
        loss.backward()
        # Meta always raw. Weight path: separate (or joint) clip coefs.
        clip_coef_actor = 1.0
        clip_coef_critic = 1.0
        if args.max_grad_norm > 0.0:
            def _coef(params):
                ps = [p for p in params if p.grad is not None]
                if not ps:
                    return 1.0
                tot = torch.norm(torch.stack([p.grad.detach().norm(2) for p in ps]), 2)
                return float(torch.clamp(args.max_grad_norm / (tot + 1e-6), max=1.0).item())

            if args.weight_clip_sep:
                clip_coef_actor = _coef(actor_params)
                clip_coef_critic = _coef(critic_params)
            else:
                c = _coef(actor_params + critic_params)
                clip_coef_actor = clip_coef_critic = c
        actor_optimizer.step(clip_coef=clip_coef_actor)
        critic_optimizer.step(clip_coef=clip_coef_critic)
        ...
        if global_step % args.log_every < args.num_envs or iteration == args.num_iterations:
            ...
            writer.add_scalar("idbd/clip_coef_actor", clip_coef_actor, global_step)
            writer.add_scalar("charts/onesided_adv_ema", ema_abs_a, global_step)
            writer.add_scalar("charts/onesided_adv_gain", last_adv_gain, global_step)
            writer.add_scalar("idbd/clip_coef_critic", clip_coef_critic, global_step)
            writer.add_scalar("idbd/meta_g_rms", idbd.get("meta_g_rms", 0.0), global_step)
            ...
            print(
                f"SPS: {sps}  ca={clip_coef_actor:.2f} cc={clip_coef_critic:.2f}  gA={last_adv_gain:.2f} sA={ema_abs_a:.3g}  "
