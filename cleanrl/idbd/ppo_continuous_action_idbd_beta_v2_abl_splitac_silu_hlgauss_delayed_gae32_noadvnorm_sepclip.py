...
"""
**Separate actor/critic grad clips** on delayed GAE-32 + rewnorm + noadvnorm.

Baseline: one joint clip over actor+critic
  c = min(1, max_norm / ||[g_a; g_c]||)
so a large critic grad throttles the actor (and vice versa).

Here: independent clips (same max_norm=0.5 each)
  clip_grad_norm_(actor_params, 0.5)
  clip_grad_norm_(critic_params, 0.5)

Unlike v3: clip is in-place before IDBD, so β and w both see clipped g.

Optional: --paper-theta → θ=0.05, stream θ off, β bounds (paper § after Fig.2).
"""
import math
import os
import random
...
if __name__ == "__main__":
    ...
    print(
        f"delayed_gae32_noadvnorm_sepclip: DELAYED GAE H={H} λ={args.gae_lambda} "
