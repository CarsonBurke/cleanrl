...
"""
Ablation: silu_hlgauss base with θ=0.5 (IDBD meta_lr).

α is frozen near init (0.05) with default θ=0.05; raise θ 10× so the
β-update signal θ·g·h can leave the noise floor. Same splitac+SiLU+HL-Gauss.

Hypothesis: if α_p10/p90 still collapse to 0.05, meta signal is essentially
zero (not merely under-scaled); if α spreads, default θ was too timid.
"""
import math
import os
import random
...
class Args:
    ...
    # IDBD-specific
    idbd_meta_lr: float = 0.5
    """meta step-size θ = 0.5 (10× paper demo; probe α adaptation)"""
...
if __name__ == "__main__":
    ...
    print(
        f"IDBD Beta v2 abl_splitac_silu_hlgauss_theta05: minibatch_size={args.minibatch_size} "
        f"(~{args.batch_size // args.minibatch_size} steps/epoch × {args.update_epochs} epochs), "
        f"meta_lr={args.idbd_meta_lr} (θ=0.5), init_alpha={args.learning_rate}, "
