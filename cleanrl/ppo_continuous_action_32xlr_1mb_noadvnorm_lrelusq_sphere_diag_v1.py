...
# Instrumented control: byte-identical learner to lrelusq_sphere_v1, whose 50M
# HalfCheetah run trails stiglu_sphere by ~7% at 30M after matching it to 10M.
# Purpose is attribution, not a new method: it carries the probes so the two
# candidate causes below can be read off a run instead of argued.
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; 32x LR; 1 minibatch; hypersphered residual LeakyReluSq trunk; no advantage normalization; trunk probes")
