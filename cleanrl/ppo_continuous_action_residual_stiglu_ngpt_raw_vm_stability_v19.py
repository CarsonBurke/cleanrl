# Value-trust-region and variance-gain EMA ablations of unclipped PPO v19.
# Keep v18 raw K101/sigma0.5, mean-matched labels and detached raw variance gain.
# Optional value clipping matches kragg_hlgauss: stop a sample's CE gradient
# when its clipped decoded value has greater squared target error than its current value.
# Optional gain EMA uses retention 0.5 per optimizer step, initialized by the first gain.
# This smooths only the loss multiplier, not support, rewards, targets or critic weights.
# Hypothesis: stabilize unclipped optimization without changing either residual trunk.
import math
import os
import random
import time
...
class Args:
    ...
    """stop CE gradients on the reference decoded-value clipping sample set"""
    value_gain_ema: bool = False
    """smooth the detached predicted raw-variance multiplier each optimizer step"""
    value_gain_momentum: float = 0.5
    """EMA retention; first update adopts the current gain without cold-start bias"""
    grad_clip: Literal["global", "actor", "none"] = "none"
    """joint norm clipping, actor-only clipping, or no gradient clipping"""
...
class ValueGainEMA(nn.Module):
    """Detached scalar gain state, updated once per optimizer loss evaluation."""
    ...
    def update(self, gain):
        self.count.add_(1.0)
        weight = torch.minimum(torch.full_like(self.count, self.momentum),
                               1.0 - self.count.reciprocal())
        self.value.mul_(weight).add_(gain.detach() * (1.0 - weight))
        # Backward must retain this step's multiplier, not the mutable EMA buffer.
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        ...
        self.value_gain_ema = ValueGainEMA(args.value_gain_momentum) if args.value_gain_ema else None
...
def categorical_value_terms(logits, target_probs, histogram, targets, old_values,
                            clip_vloss=False, clip_coef=0.2, gain_ema=None):
    """Variance-matched CE with optional reference value gate and detached gain EMA."""
    log_probs = logits.log_softmax(dim=-1)
    cross_entropy = -(target_probs.detach() * log_probs).sum(dim=-1)
    raw_ce = cross_entropy.mean()
    with torch.no_grad():
        ...
        gain = raw_gain if gain_ema is None else gain_ema.update(raw_gain)
        if clip_vloss:
            new_values = histogram.probs_to_scalar(probabilities)
            clipped = old_values + (new_values - old_values).clamp(-clip_coef, clip_coef)
            frozen = (clipped - targets).square() > (new_values - targets).square()
            clipfrac = frozen.float().mean()
        else:
            clipfrac = torch.zeros((), device=logits.device)
    if clip_vloss:
        cross_entropy = cross_entropy * (~frozen)
    return cross_entropy.mean() * gain, raw_ce, gain, raw_gain, clipfrac


def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, target_probs, targets, old_values, args):
    ...
    v_loss, raw_ce, value_gain, raw_gain, value_clipfrac = categorical_value_terms(
        logits, target_probs, agent.histogram, targets, old_values,
        args.clip_vloss, args.clip_coef, agent.value_gain_ema,
    )
    entropy_loss = entropy.mean()
    loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef
    metrics = torch.stack((pg_loss.detach(), v_loss.detach(), entropy_loss.detach(),
                           old_approx_kl, approx_kl, clipfrac, raw_ce.detach(), value_gain, raw_gain, value_clipfrac))
    return loss, metrics


def apply_gradient_clipping(parameters, actor_parameters, max_norm, mode):
    """Return the selected pre-clip norm, or None when gradients are untouched."""
    if mode == "global":
        return nn.utils.clip_grad_norm_(parameters, max_norm)
    if mode == "actor":
        return nn.utils.clip_grad_norm_(actor_parameters, max_norm)
    if mode == "none":
        return None
    raise ValueError("grad_clip must be global, actor, or none")


def validate_args(args):
    if not 0.0 <= args.value_gain_momentum < 1.0:
        raise ValueError("value_gain_momentum must lie in [0, 1)")
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", f"original residual SiTU-GLU trunks and matrix projection; raw rewards; fixed symexp-spaced raw nodes; {args.value_target} mean-preserving labels; paired raw-expectation decode; detached predicted raw-variance CE gain; gradient clipping={args.grad_clip}; value clipping={args.clip_vloss}; gain EMA={args.value_gain_ema}, retention={args.value_gain_momentum}; no return frame")
        ...
        def loss_model(observations, native, old_logprobs, advantages, target_probs, targets, old_values):
            return ppo_loss(agent, observations, native, old_logprobs, advantages, target_probs, targets, old_values, args)
        ...
        for iteration in range(1, args.num_iterations + 1):
                ...
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss, metrics = loss_model(
                            ...
                            b_advantages[indices], b_target_probs[indices], b_returns[indices], b_values[indices],
                        )
                        optimizer.zero_grad(set_to_none=True)
                        loss.backward()
                        selected_preclip_norm = apply_gradient_clipping(
                            parameters, actor_parameters, args.max_grad_norm, args.grad_clip,
                        )
                        optimizer.step()
                        normalize_matrices()
            ...
            with torch.no_grad():
                ...
                elif args.grad_clip == "actor":
                    total_preclip_norm = torch.sqrt(selected_preclip_norm.square() + critic_grad_norm.square())
                else:
            ...
            logged = gather_metrics({
                ...
                "critic/value_gain_raw": last[8], "losses/value_clipfrac": update_metrics[:updates, 9].mean(),
