...
# Both fixes for the LeakyReluSq sphere trunk at once: the zero-mean signed
# square f(x) = x|x| in place of leaky_relu(x,0.5)^2, and every
# scale-invariant trunk matrix pinned to its initial Frobenius norm.
...
# The two causes are independent (representation versus optimization), so this
# arm tests whether their fixes compose; signsq is still degree-2 homogeneous,
# so the pin is exactly as load-bearing here as for LeakyReluSq.
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; 32x LR; 1 minibatch; hypersphered zero-mean signed-square trunk, Frobenius-pinned; no advantage normalization; trunk probes")
