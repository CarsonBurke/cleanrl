# PPO + IterThink v135 rawsymlog-dense-tokens-k6 (v133 + dense tokenizing encoder).
#
# Key change from v133:
#   - Remove Gym's stateful NormalizeObservation/clip wrappers.
#   - Feed raw flattened MuJoCo observations through a dense symlog tokenizer:
#       z = symlog(raw_obs)
#       h = Linear(z) -> RMSNorm -> Linear -> ReLU^2
#       obs_tokens = Linear(h) reshaped into learned latent slots
#     This keeps Dreamer3-style dense cross-feature processing, but projects
#     directly into the token interface instead of manufacturing pseudo-scalars.
...
#
# Hypothesis:
#   Running obs norm is the source of prompt-bank coordinate ambiguity. A fixed
#   symlog dense tokenizer can recover Dreamer3's stable proprioception encoding
#   while preserving the existing tokenized WM/policy interface.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # a running normalizer; dense layers mix proprioceptive coordinates before
        # projecting directly into learned latent slots.
        ...
        self.obs_dense1 = xavier_linear(nn.Linear(obs_dim, self.dim))
        self.obs_dense_norm = nn.RMSNorm(self.dim)
        self.obs_dense2 = xavier_linear(nn.Linear(self.dim, self.dim))
        self.obs_token_proj = xavier_linear(nn.Linear(self.dim, self.num_obs_tokens * self.dim))
    ...
    def encode_summary(self, obs):
        ...
        z = symlog(obs_flat)
        h = self.obs_dense_norm(self.obs_dense1(z))
        h = relu_sq(self.obs_dense2(h))
        obs_tokens = self.obs_token_proj(h).reshape(batch, self.num_obs_tokens, self.dim)
