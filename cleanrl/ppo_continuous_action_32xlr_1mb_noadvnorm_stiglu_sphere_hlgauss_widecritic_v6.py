# HL-Gauss capacity study on the exact 32x-LR sphere PPO baseline.
# Paper: arxiv.org/abs/2403.03950, classification benefits with larger critics.
# Widen ONLY the critic to256; preserve the width64 actor and its initial RNG.
...
class Args:
    ...
    """categorical critic width; actor remains64"""
    critic_feature_scale: float = 16.0
    """unit-L2 width256 features times16 have RMS1"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, value_config: HLGaussConfig | None = None, critic_feature_scale=16.0, critic_width=256):
        super().__init__()
        ...
        # Build the wider critic only AFTER the exact baseline actor initialization.
        value_trunk = make_situ_sphere_trunk(observation_dim, critic_width)
        value_head = ScaledValueHead(critic_width, value_config.num_bins, critic_feature_scale)
        ...
        self.critic = nn.Sequential(value_trunk, value_head)
