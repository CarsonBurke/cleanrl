# Pan Goal Solver v7: direct one-step MSE descent toward an evolving ideal.
#
# A reward-free LeJEPA learns observation tokens and temporal control belief.
# Pan hindsight and occupancy ground the follower in real trajectories. A frozen
# reward-blind one-step world model amortizes Euclidean descent toward P(y*).
# P's deterministic point is recomputed every step and updated every train cycle.
...
class PanGoalSolver(nn.Module):
    ...
    def direct_action(self, obs_history, incoming_action_history, goal):
        ...
        desired_goal = goal.expand(obs_history.shape[0], -1)
        action = self.follower(belief, current_goal, desired_goal)
        return action, current_goal

    def act(self, obs_history, incoming_action_history, aspiration_reward):
        goal = self.proposer(aspiration_reward)[0]
        action, current_goal = self.direct_action(
            obs_history, incoming_action_history, goal
        )
...
def train_follower_step(agent, optimizer, batch, args, device, rng):
    ...
    with torch.no_grad():
        ...
        _, combined_memory = agent.world_model.temporal(
            torch.cat([current_tokens, occupancy_tokens, current_tokens], dim=0),
            torch.cat(
                [
                    torch.zeros_like(incoming),
                    torch.zeros_like(occupancy_incoming),
                    incoming,
                ],
                dim=0,
            ),
        )
        current_memory, occupancy_memory, control_memory = combined_memory.chunk(3)
    ...
    goal_action = agent.follower(
        belief, current_goal, aspiration_goal.expand_as(current_goal)
    )
    transition_parameters = list(agent.world_model.transition.parameters())
    for parameter in transition_parameters:
        parameter.requires_grad_(False)
    predicted_next_tokens = agent.world_model.transition(
        control_memory.detach(), goal_action
    )
    ...
    with torch.no_grad():
        zero_next_tokens = agent.world_model.transition(
            control_memory.detach(), torch.zeros_like(goal_action)
        )
        zero_history = torch.cat(
            [frame_history[:, 1:], zero_next_tokens.mean(dim=-2)[:, None]], dim=1
        )
        zero_next_goal, _ = agent.target_representation["goal_projector"].encode(
            zero_history
        )
        current_distance = (current_goal - aspiration_goal).square().mean(-1)
        zero_distance = (zero_next_goal - aspiration_goal).square().mean(-1)
    predicted_distance = (predicted_next_goal - aspiration_goal).square().mean(-1)
    goal_descent_loss = (
        (predicted_distance - zero_distance)
        / current_distance.clamp_min(1e-3)
    ).mean()
    loss = (
        args.action_coef * action_loss
        + args.occupancy_coef * occupancy_loss
        + args.goal_descent_coef * goal_descent_loss
    )
    optimizer.zero_grad(set_to_none=True)
    loss.backward()
    for parameter in transition_parameters:
    ...
    return {
        ...
        "follower/goal_descent_loss": goal_descent_loss.item(),
        "follower/predicted_goal_progress": (current_distance - predicted_distance).mean().item(),
        "follower/predicted_advantage_over_zero": (zero_distance - predicted_distance).mean().item(),
        "follower/goal_action_abs_mean": goal_action.abs().mean().item(),
...
def main():
    ...
    aspiration = torch.tensor([args.aspiration_reward], device=device)
    with torch.no_grad():
        current_goal_point = agent.proposer(aspiration)[0].detach()
    previous_goal = current_goal_point.clone()
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        for step in range(rollout_steps):
            ...
            with torch.no_grad():
                ...
                policy_action, current_goal, step_goal = agent.act(
                    obs_tensor, incoming_tensor, aspiration
                )
                goal_mse[step] = (
                    current_goal - step_goal.unsqueeze(0)
        ...
        if global_step >= args.follower_warmup_steps:
            for _ in range(args.follower_updates_per_iteration):
                ...
                # P/U receive a fresh reward-goal optimizer step on every
                # follower optimization cycle, never an occasional batch.
                ...
                metrics.append(
                    train_goal_step(
                        agent, goal_optimizer, goal_obs, reward_rate, args, device
                    )
                )
        if global_step >= args.random_warmup_steps:
        ...
        with torch.no_grad():
            current_goal_point = agent.proposer(aspiration)[0].detach()
            predicted_utility = agent.utility(current_goal_point.unsqueeze(0)).item()
            raw_revision = (current_goal_point - previous_goal).square().mean().item()
            direction_norm = agent.proposer.direction.norm().item()
            goal_norm = current_goal_point.norm().item()
        ...
        if revision_anchor_obs is not None:
            with torch.no_grad():
                ...
                nearest = nearest_anchor_mse(anchor_latents, current_goal_point)
                ...
                if previous_anchor_latents is not None:
                    ...
                    rotation = u @ vh
                    aligned_goal = (current_goal_point - current_mean) @ rotation + previous_mean
