# PPO + IterThink v94 (v24 Beta + reward-interval TPO). From v93.
#
...
# rollout policy at each state. Unlike v92's learned scorer and v93's one-step
# TD bootstrap, v94 groups exactly at MuJoCo's lowest reward interval: for each
# candidate action it restores the simulator state, steps one action, and scores
# the candidate by that immediate raw reward. TPO then constructs
...
#
# Hypothesis: v93 failed because adding gamma V(s') - V(s) turned the target into
# noisy local Q-fitting. Dense HalfCheetah already supplies a reward every step;
# grouping at that reward interval should give TPO a direct within-state action
...
class Args:
    ...
    # v94: pure TPO actor objective over immediate-reward counterfactual MuJoCo probes.
...
def counterfactual_reward_scores(envs, candidate_actions, device):
    """Immediate HalfCheetah reward scores for a (num_envs, K, act_dim) action set.

    This intentionally bypasses wrappers while probing and restores MuJoCo state before
    the real wrapped env step. The actor target standardizes within each candidate group,
    so raw reward scale is sufficient and avoids wrapper-stat side effects.
    """
    scores = []
    ...
    for env_i, wrapped_env in enumerate(envs.envs):
        ...
        for cand_i in range(actions_np.shape[1]):
            ...
            _, raw_reward, _, _, _ = base.step(actions_np[env_i, cand_i])
            scores.append(raw_reward)
        base.set_state(qpos, qvel)
        base.data.time = sim_time

    return torch.tensor(np.asarray(scores), dtype=torch.float32, device=device).reshape(actions_np.shape[0], actions_np.shape[1])
...
if __name__ == "__main__":
    ...
    assert args.env_id == "HalfCheetah-v4", "v94 counterfactual probing is HalfCheetah-v4-specific"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                cand_scores = counterfactual_reward_scores(envs, cand_actions, device)
        ...
        # pure TPO uses the immediate-reward counterfactual score vector collected
