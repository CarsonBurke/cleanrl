# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic headnorm v1.
#
...
# target rather than v149's MTP critic. This sibling variant also removes actor
# output-projection biases and applies affine-free RMSNorm immediately before
# the actor and critic output projections.
#
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.actor_head_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.critic_head_norm = nn.RMSNorm(H, elementwise_affine=False)
        ...
        if self.actor_dist == "gaussian":
            ...
            self.actor_head = layer_init(nn.Linear(H, act_dim, bias=False), std=0.01)
            self.actor_logvar_head = layer_init(nn.Linear(H, act_dim, bias=False), std=0.01)
            ...
            self.actor_alpha_head = layer_init(nn.Linear(H, act_dim, bias=False), std=0.01)
            self.actor_beta_head = layer_init(nn.Linear(H, act_dim, bias=False), std=0.01)
    ...
    def _actor_dist(self, actor_feat):
        ...
        if self.actor_dist == "gaussian":
            actor_feat = self.actor_head_norm(actor_feat)
    ...
    def get_value(self, x):
        ...
        return self.critic_head(self.critic_head_norm(critic_feat))
    ...
    def get_action_and_value(self, x, z=None):
        ...
        value_logits = self.critic_head(self.critic_head_norm(critic_feat))
        if self.actor_dist == "gaussian":
