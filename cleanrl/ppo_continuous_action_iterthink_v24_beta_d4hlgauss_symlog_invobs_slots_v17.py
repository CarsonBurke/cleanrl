# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted-attention slot transformer v17.
#
# WHY v17 (SimbaV2 ORTHOGONAL INIT + fold-ins, ablation over v12). Brings over SimbaV2's
# weight-init recipe and two micro-tweaks, all vs the v11/v12 champion (alpha init stays
# 0.05; v16 separately tests the alpha change). Three changes, bundled per request:
#   (a) ORTHOGONAL INIT of every trunk weight matrix (q,k,v,proj,mlp.fc,mlp.proj,
#       tokenizer value/pos), replacing Kaiming. Applied ONCE; the existing per-optimizer
#       -step justnorm then maintains unit-norm rows/cols (drifting off orthogonality) --
#       exactly SimbaV2's "orthogonal init + per-gradient-step column-normalize". NOTE both
#       SimbaV2 and we normalize weights PER OPTIMIZER STEP (per gradient update), not per
#       env step or per forward. This is NOT v13's orthonormalize-every-step (which lost).
#   (b) SIGNED eigen-lr: drop the .abs() in the lerp step so a channel can move away from
#       its sublayer output (SimbaV2's alpha scaler is signed). [PRIME SUSPECT if v17 regresses]
#   (c) +eps before normalizing the sublayer update (SimbaV2's relu(x)+eps), a no-op except
#       for the degenerate zero-vector case.
# HYPOTHESIS: better-conditioned orthogonal starting directions improve over Kaiming+justnorm.
# Base: v12 (compiled, behavior-identical to v11 champion).
#
...
class InvertedSlotBlock(nn.Module):
    ...
    def _lerp(self, slots, update, alpha, init_value, init_scaling):
        ...
        # v17 fold-in #1: SIGNED eigen-lr (no .abs()) -- SimbaV2's alpha scaler is signed,
        # letting a channel step AWAY from its sublayer output, not just toward it.
        lr = alpha * (init_value / init_scaling)
        a = justnorm(slots)
        # v17 fold-in #2: +eps before normalizing the sublayer output, so a (near-)zero
        # update has a defined direction (SimbaV2's `relu(x)+eps` before l2normalize).
        # Negligible for normal updates (entries ~1e-2..1); only bites the degenerate
        # all-zero case. Applied here at the residual justnorm since our SwiGLU output
        # (unlike SimbaV2's bare relu) is normalized in the lerp, not inside the MLP.
        b = justnorm(update + 1e-8)
...
class ThinkTrunk(nn.Module):
    ...
    @torch.no_grad()
    def orthogonal_init_weights(self):
        """v17: SimbaV2-style ORTHOGONAL initialization of every trunk weight matrix
        (replacing nn.Linear's default Kaiming-uniform). This is applied ONCE at init;
        the per-optimizer-step justnorm (normalize_weights) then takes over -- weights
        drift off exact orthogonality but stay unit-norm, exactly SimbaV2's recipe
        (orthogonal init + per-gradient-step column normalization). NOT v13's failed
        orthonormalize-every-step. Orthogonal init gives better-conditioned, decorrelated
        starting feature directions; the subsequent init justnorm re-normalizes the
        embedding dim, matching normalize_weights so the start is on the unit hypersphere."""
        nn.init.orthogonal_(self.tokenizer.value_weight)
        nn.init.orthogonal_(self.tokenizer.pos)
        for stack in (self.actor_stack, self.critic_stack):
            if stack is None:
                continue
            for block in stack.blocks:
                nn.init.orthogonal_(block.attn.q.weight)
                nn.init.orthogonal_(block.attn.k.weight)
                nn.init.orthogonal_(block.attn.v.weight)
                nn.init.orthogonal_(block.attn.proj.weight)
...
class Agent(nn.Module):
    ...
    def orthogonal_init_trunk_weights(self):
        """v17: orthogonal-initialize every trunk's weight matrices once at construction
        (before the init justnorm). SimbaV2-style ortho init; heads excluded."""
        if self.share_backbone:
            self.trunk.orthogonal_init_weights()
        else:
            self.actor_trunk.orthogonal_init_weights()
            self.critic_trunk.orthogonal_init_weights()
...
if __name__ == "__main__":
    ...
    agent.orthogonal_init_trunk_weights()   # v17: SimbaV2-style orthogonal init...
    agent.normalize_trunk_weights()   # nGPT: ...then justnorm onto the unit hypersphere
    if args.compile:
