# Reward-normalized, fixed-support symlog categorical critic v14.
# Keep v13 raw-mean-preserving labels, but values now use normalized-return
# units: fixed +/-10 support, 501 nodes, sigma 1.5 bins, no target frame or EMA.
# Gaussian moment matching and raw-space twohot share the same scalar decoder.
# Vector reward normalization covers warmup, rollout and public evaluation.
# Our residual trunks, matrix projection and fullbatch asymmetric PPO stay fixed.
# Hypothesis: normalized rewards retain stable actor/critic scale without
# sacrificing target-mean correctness or chasing a per-rollout value frame.
import math
import os
import random
import time
...
class Args:
    ...
    """mean-matched Gaussian labels or twohot in normalized-return units"""
    value_atoms: int = 501
    """number of categorical critic atoms"""
    value_min: float = -10.0
    """lower support bound in reward-normalized return units"""
    value_max: float = 10.0
    """upper support bound in reward-normalized return units"""
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        ...
        # Both target encodings share pre-transform nodes and the same decoder.
...
def make_target_projector(args, histogram):
    ...
    # The dual coefficient has inverse-value units. A fixed +/-1 search is
    # too narrow near small support endpoints; scale it by node spacing.
    return partial(gaussian.project_moment_matched, tilt_bound=64.0 / gaussian.bin_width)
...
def validate_args(args):
    ...
    if not math.isfinite(args.value_max) or args.value_max <= 0 or args.value_min != -args.value_max:
        raise ValueError("support bounds must be finite, positive-radius and symmetric")
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", f"original residual SiTU-GLU trunks and matrix projection; vector-normalized rewards; fixed symexp-spaced normalized-return nodes; {args.value_target} mean-preserving labels; paired raw-expectation decode; no return frame or EMA")
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            logged = gather_metrics({
                ...
                "critic/label_entropy": label_entropy, "critic/excess_ce": last[1] - label_entropy,
