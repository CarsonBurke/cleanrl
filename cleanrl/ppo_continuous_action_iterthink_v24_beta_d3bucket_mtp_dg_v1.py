# PPO + IterThink v24 Beta + v162 critic + Dreamer3-bucket HL-Gauss MTP + DELIGHTFUL POLICY GRADIENT (DG-Beta) v1.
# =====================================================================================
# v1 = the d3bucket_mtp_pmpo_v1 sibling with PMPO's |tanh(adv)| pos/neg weighting REPLACED by a faithful
# Beta port of Osband's Delightful Policy Gradient (DG, Algorithm 2). EVERYTHING else is byte-identical to
# pmpo_v1 (DreamerV3 511-bucket symexp HL-Gauss distributional MTP critic, iterthink ThinkTrunk, MTP
# machinery, unimodal Beta actor, reward GAE, decoupled grad clipping max_grad_norm=0.5, the analytic
# reverse Beta-KL trust region, old-dist storage at rollout, optimizer, hyperparameters, logging).
#
# DG GATE: each policy-gradient term is gated by w = sigmoid(chi/eta), chi = U * ell, where U is the RAW
# GAE advantage and ell is the action SURPRISAL. The score is then -(w * U * logpi).mean().
#   - SURPRISAL = scale-invariant Mahalanobis z-score^2: ell = 0.5 * sum_dim( ((z - mean)/std)^2 ).
#     This is EXACTLY the Gaussian's -logpi tail term (drift-free, >=0, zero at the mode). The paper's
#     ell = clip(-log pi, -10, 10) is correct for a Gaussian, but a NAIVE Beta port using raw -log pi_Beta
#     is WRONG: the Beta's per-state normalizer -log B(alpha,beta) shifts -logpi by a state-dependent
#     constant, corrupting chi. The Mahalanobis form strips that normalizer and keeps only the tail.
#   - WHITEN: chi is whitened per-minibatch (paper 2.2) before the sigmoid so the gate stays GRADED rather
#     than saturating to a hard {0,1} mask; this gives chi an adaptive scale across minibatches.
#   - RENORM: the gate is mean-preserving (w /= w.mean()), so it REWEIGHTS gradient mass toward
#     high-delight terms without globally shrinking the effective learning rate.
#   - TRUST REGION: retained UNCHANGED from pmpo_v1 -- + pmpo_kl_coef * KL(old_rollout || new) closed-form
#     reverse Beta-KL (coef 0.3), plus the analytic-KL early stop. The ONLY actor-grad paths are newlogprob
#     (in the score) and reverse_kl (the leash); the entire gate (m, s, ell, chi, w) is detached.
# policy_objective="ppo" still reproduces the base EXACTLY (the DG path lives under pmpo_active).
#
# HYPOTHESIS: a GRADED surprisal-gate -- chi = advantage x z-score^2 -- reallocates gradient toward
# high-surprisal "breakthroughs" (the agent did something improbable AND it paid off) and away from
# high-surprisal "blunders", while leaving low-surprisal routine actions near a neutral weight. On the same
# Beta base + KL leash this should beat PMPO's pure-sign |tanh(adv)| weighting, which discards both the
# advantage magnitude and the action surprisal. BASELINE TO BEAT: PMPO v1 = 7781 (HalfCheetah, 8M).
# =====================================================================================
#
...
class Args:
    ...
    critic_update_epochs: Optional[int] = None  # if set, DECOUPLE the critic from the actor: critic trains this many
                                                # epochs, actor only `update_epochs`. The critic runs FIRST (well-fit
                                                # values -> good advantages for the gate AND score), then the actor takes
                                                # its `update_epochs` gated steps LAST, so no critic update perturbs the
                                                # policy after it steps (matters because share_backbone=True couples them
                                                # through the trunk). None => coupled (both = update_epochs), exactly the
                                                # original behavior. Faithful to the paper, where the off-policy Retrace
                                                # critic trains continuously while the actor does ~single updates.
    ...
    target_kl: Optional[float] = 0.03  # KL early-stop leash (the iterthink-line winner; shared trunk runs hot without it). None => off.
    ...
    policy_objective: str = "pmpo"   # "pmpo" = dreamer4 PMPO | "ppo" = base clipped surrogate
    ...
    # --- Delightful Policy Gradient (DG) gate (faithful Beta analog of Osband Algorithm 2).
    # Replaces PMPO's |tanh(adv)| pos/neg weighting with a graded sigmoid gate w = sigmoid(chi/eta),
    # chi = U * ell (advantage x action-surprisal). The surprisal ell is the SCALE-INVARIANT
    # Mahalanobis z-score^2 (the Gaussian's -logpi tail term), NOT raw -logpi_Beta (corrupted by the
    # Beta per-state normalizer -log B(alpha,beta)). pg_loss = -(w * U * logpi).mean(). ---
    dg_eta: float = 1.0              # gate temperature (paper uses 1)
    dg_clip: float = 10.0            # clip bound C on surprisal ell (paper clips to [-C,C])
    dg_offpolicy_is: bool = False    # Retrace-style TRUNCATED importance correction on the score: weight each sample by
                                     # rho_bar = min(dg_is_clip, pi_new/pi_old) (DETACHED). This is the paper's OWN
                                     # off-policy mechanism (Retrace, c=1) -- NOT a KL/clip trust region. It lets the
                                     # bare score reuse a rollout over MANY epochs without overshoot: as the policy moves,
                                     # stale samples (rho<1) are downweighted, surprising ones capped at the truncation.
                                     # This supplies the sample efficiency the paper gets from replay+Retrace, which a
                                     # pure 1-epoch on-policy score lacks (it is too weak to bootstrap the policy).
    dg_is_clip: float = 1.0          # Retrace truncation c for rho_bar=min(c,rho); paper uses c=1.
    dg_mode: str = "score"           # "score" = faithful Algorithm 2 bare PG -(w*U*logpi) -- NEEDS a trust region
                                     #   (reverse-KL or 1 epoch); with neither it overshoots/collapses over 10 epochs.
                                     # "gate_ppo" = multiply PPO's CLIPPED surrogate by the detached delight gate w.
                                     #   The clip IS the trust region (built-in importance correction over 10 epochs),
                                     #   so DG REWEIGHTS PPO's gradient budget toward delightful samples instead of
                                     #   REPLACING PPO with bare REINFORCE. Use with dg_renorm so w is mean-preserving.
    dg_surprisal: str = "mahalanobis"  # "logpi" = paper-literal Algorithm 2 surprisal ell=clip(-log pi(a|s), -C, C),
                                       #   CONCENTRATION-DEPENDENT (includes the Beta -log B(a,b) normalizer): as the
                                       #   policy peaks, -log pi for confident actions goes NEGATIVE -> chi flips -> the
                                       #   gate stops amplifying already-confident actions -> self-limits concentration.
                                       # "mahalanobis" = scale-INVARIANT z^2 tail term (>=0); removes that self-regulation
                                       #   and lets entropy run away. Kept only for the ablation.
                                       # "entref" = clip(-log pi - H[pi], -C, C): centered at 0 per state (E[-log pi]=H),
                                       #   so it measures TAIL POSITION only -- cancels the log B normalizer AND the
                                       #   action-space Jacobian constant, keeps the confident-action self-regulation.
                                       #   RECOMMENDED (independently prescribed by Beta-math + adversarial reviews).
    dg_whiten: bool = True           # whiten chi per-minibatch before gating (paper 2.2) -- keeps gate graded vs saturated
    dg_renorm: bool = True           # mean-preserving gate (divide by w.mean()): reweight without shrinking LR
    dg_floor: float = 0.0            # optional constant added to surprisal (Gaussian-style normalizer floor); 0=off
    dg_normadv: bool = False         # UPSTREAM advantage normalization: divide mb_advantages by the batch-global adv std
                                     # ONCE, so the SAME O(1) U flows to BOTH the gate (chi=U*ell) and the score (w*U*logpi).
                                     # Preserves the delight identity (gate measures the delight of the ACTUAL update),
                                     # unlike the gate-only dg_advnorm modes. This is the faithful fix for raw-GAE scale
                                     # (~5 off the symexp critic) that would otherwise saturate the entref sigmoid.
    dg_advnorm: bool = False         # scale the GATE's advantage by the batch-global adv std (paper's reward-EMA analog) so chi=U/std*ell stays graded WITHOUT whitening; score keeps raw U
    dg_advnorm_ema: bool = False     # paper-faithful variant of dg_advnorm: divide the GATE advantage by a PERSISTENT EMA (decay below) of adv-std instead of the per-update batch std. Slowly-varying global reward-scale (the paper normalizes rewards by an EMA decay 0.999 for critic stability), so the gate sees true cross-update delight magnitude rather than a per-batch-whitened unit signal. Takes precedence over dg_advnorm when set.
    dg_advnorm_ema_decay: float = 0.999  # EMA decay for dg_advnorm_ema (paper uses 0.999 on the reward scale)
    dg_advnorm_perc: bool = False    # scale the GATE's advantage by the DreamerV3 percentile RETURN scale S=max(floor,P95-P5)
                                     # (the base's ret_percnorm machinery, normally bypassed under PMPO/DG). A principled,
                                     # robust, slowly-varying (EMA) global return scale -- unlike per-batch adv-std it is
                                     # outlier-insensitive and doesn't re-whiten to unit each update. Uses ret_perc_scope/
                                     # ret_perc_rate/ret_perc_floor. Highest precedence among the dg_advnorm_* modes. Score keeps raw U.
...
if __name__ == "__main__":
    ...
    # DG persistent EMA of the advantage scale (paper-faithful gate normalization, dg_advnorm_ema).
    dg_adv_ema = None
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, value_logits, act_dist = agent.get_action_and_value(next_obs)
        ...
        with torch.no_grad():
            ...
            # Compute the DreamerV3 percentile return scale S=max(floor,P95-P5) when EITHER the standard
            # non-PMPO ret_percnorm is on, OR the DG gate wants it (dg_advnorm_perc, which runs under PMPO).
            # Only the non-PMPO PG path divides policy_adv by it; under DG the score keeps raw U and the
            # gate consumes ret_perc_scale below.
            if args.ret_perc_scope in ("ema", "batch") and (
                (not pmpo_active and args.ret_percnorm) or (pmpo_active and args.dg_advnorm_perc)):
        ...
        # DG paper-faithful gate scale: update a PERSISTENT EMA (decay dg_advnorm_ema_decay) of the
        # batch adv-std ONCE per update. Used below as a slowly-varying global reward-scale for the gate
        # (vs dg_advnorm's per-update batch std, which whitens chi to unit each update).
        if args.dg_advnorm_ema:
            _cur_adv_std = b_advantages.std().detach()
            dg_adv_ema = _cur_adv_std if dg_adv_ema is None else (
                args.dg_advnorm_ema_decay * dg_adv_ema + (1.0 - args.dg_advnorm_ema_decay) * _cur_adv_std)
        ...
        clipfracs = []
        # DG diagnostics (last-minibatch values; harmless defaults for the ppo path).
        reverse_kl_val = torch.zeros((), device=device)
        dg_gate_mean_val = torch.zeros((), device=device)
        dg_gate_min_val = torch.zeros((), device=device)
        dg_gate_max_val = torch.zeros((), device=device)
        dg_chi_std_val = torch.zeros((), device=device)
        dg_ell_mean_val = torch.zeros((), device=device)
        # Decoupled actor/critic epochs: critic trains first (epochs 0..n_critic-1), actor takes its
        # gated steps in the LAST `update_epochs` epochs so no critic trunk update follows the actor step.
        # critic_update_epochs=None reproduces the coupled default (do_actor==do_critic==True every epoch).
        n_critic_epochs = args.critic_update_epochs if args.critic_update_epochs is not None else args.update_epochs
        n_total_epochs = max(args.update_epochs, n_critic_epochs)
        actor_gn = critic_gn = torch.zeros((), device=device)
        for epoch in range(n_total_epochs):
            do_critic = epoch < n_critic_epochs
            do_actor = epoch >= n_total_epochs - args.update_epochs
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if pmpo_active:
                    # ===== Delightful Policy Gradient (DG) gate: faithful Beta analog of Algorithm 2. =====
                    ...
                    if args.dg_normadv:
                        # Upstream O(1) normalization: SAME U to gate and score (preserves the delight identity,
                        # unlike the gate-only dg_advnorm modes). Divide-only (no mean-subtract) to keep GAE's zero-point.
                        mb_advantages = mb_advantages / (b_advantages.std() + 1e-8)
                    # --- DG gate (faithful Beta analog of Algorithm 2) ---
                    z_mb = b_latent_zs[mb_inds]
                    newlogprob = new_dist.log_prob(z_mb).sum(1)                    # full-action logprob (Beta-native z)
                    with torch.no_grad():
                        if args.dg_surprisal == "entref":
                            # Entropy-referenced surprisal: ell = clip(-log pi - H[pi], -C, C). Since
                            # E_a[-log pi] = H, this is centered at 0 PER STATE -> gate ~0.5 for typical actions
                            # REGARDLESS of the policy's current sharpness. It cancels the Beta normalizer
                            # log B(alpha,beta) (the concentration term that made raw -log pi read sharpness, not
                            # surprise) AND the constant action-space Jacobian offset (H carries the same constant),
                            # so it measures *tail position* alone. Beta self-regulation is preserved: a confident
                            # action (logpi > H) gives ell<0 -> chi<0 (for U>0) -> gate<0.5 -> update down-weighted.
                            ent_mb = new_dist.entropy().sum(1)
                            ell = (-newlogprob.detach() - ent_mb.detach()).clamp(-args.dg_clip, args.dg_clip)
                        elif args.dg_surprisal == "logpi":
                            # Paper-literal Algorithm 2: ell = clip(-log pi(a|s), -C, C). SYMMETRIC clip allows
                            # NEGATIVE surprisal (peaked Beta -> pdf>1 -> -log pi<0 for confident actions), which is
                            # the self-regulating, concentration-DEPENDENT mechanism the Mahalanobis variant lacked.
                            # NOTE: conflates concentration (log B normalizer) with tail position; prefer "entref".
                            ell = (-newlogprob.detach()).clamp(-args.dg_clip, args.dg_clip)
                        else:  # "mahalanobis": scale-invariant z^2 tail term (>=0), upper-clip only
                            m = new_dist.mean
                            s = new_dist.stddev.clamp_min(1e-6)
                            ell = 0.5 * (((z_mb - m) / s) ** 2).sum(1)             # Mahalanobis z-score^2 surprisal, >=0
                            ell = ell.clamp(max=args.dg_clip)
                        if args.dg_floor != 0.0:
                            ell = ell + args.dg_floor                             # optional Gaussian-style constant floor
                        # delight = U * ell. Optionally scale U by the batch-GLOBAL adv std (absolute, not
                        # minibatch-relative) so chi sits in sigmoid's graded band without whitening (paper's
                        # reward-EMA analog). This affects ONLY the gate input; the score below keeps raw U.
                        if args.dg_advnorm_perc:
                            chi_adv = mb_advantages / ret_perc_scale             # DreamerV3 percentile return scale S=max(floor,P95-P5)
                        elif args.dg_advnorm_ema:
                            chi_adv = mb_advantages / (dg_adv_ema + 1e-8)         # persistent EMA scale (paper reward-norm faithful)
                        elif args.dg_advnorm:
                            chi_adv = mb_advantages / (b_advantages.std() + 1e-8) # per-update batch std
                        else:
                            chi_adv = mb_advantages                              # raw GAE -> gate sees true (possibly saturating) magnitude
                        chi = chi_adv * ell
                        if args.dg_whiten:
                            chi = (chi - chi.mean()) / (chi.std() + 1e-8)         # paper 2.2: whiten before gating (adaptive scale)
                        w = torch.sigmoid(chi / args.dg_eta)                      # gate, detached (whole block is no_grad)
                        if args.dg_renorm:
                            w = w / (w.mean() + 1e-8)                             # mean-preserving: reweight without shrinking LR
                    if args.dg_mode == "gate_ppo":
                        # DG reweights PPO's CLIPPED surrogate (clip = built-in trust region + importance correction
                        # over the 10 epochs). w is detached; only ratio carries grad. With dg_renorm, w.mean()==1 so
                        # this is PPO with per-sample budget redistributed toward delightful samples -- not a regression.
                        clip_hi = args.clip_coef if args.clip_coef_high is None else args.clip_coef_high
                        surr = torch.max(-mb_advantages * ratio,
                                         -mb_advantages * torch.clamp(ratio, 1 - args.clip_coef, 1 + clip_hi))
                        pg_loss = (w * surr).mean()
                    else:  # "score": faithful Algorithm 2 bare PG
                        if args.dg_offpolicy_is:
                            # Retrace-style truncated IS: rho_bar = min(c, pi_new/pi_old), DETACHED weight (off-policy
                            # correction, NOT a trust region). Lets the bare score safely reuse the rollout over many
                            # epochs: epoch 0 has rho=1 (on-policy); later epochs downweight stale samples. The paper's
                            # own efficiency engine (replay+Retrace), which pure 1-epoch on-policy DG lacks.
                            rho_bar = ratio.detach().clamp(max=args.dg_is_clip)
                            pg_loss = -(w * rho_bar * mb_advantages * newlogprob).mean()
                        else:
                            pg_loss = -(w * mb_advantages * newlogprob).mean()       # faithful Algorithm 2 score: -(w * U * logpi)
                    ...
                    dg_gate_mean_val = w.mean().detach()
                    dg_gate_min_val = w.min().detach()
                    dg_gate_max_val = w.max().detach()
                    dg_chi_std_val = chi.std().detach()       # post-whiten chi if whitened, else raw
                    dg_ell_mean_val = ell.mean().detach()
                ...
                if args.separate_grad_clip:
                    ...
                    # do_actor/do_critic gate which tower steps (decoupled epochs).
                    optimizer.zero_grad(set_to_none=True)
                    if do_critic and do_actor:
                    ...
                    terms = []
                    if do_actor:
                        terms.append(pg_loss - ent_coef_eff * entropy_loss)
                    if do_critic:
                        terms.append(v_loss * args.vf_coef)
                    if terms:
                        optimizer.zero_grad()
                        sum(terms).backward()
            ...
            # Only the actor epochs are subject to the KL early-stop; critic-only epochs never move the
            # policy (separate backbone) or move it only via the trunk, so they must not trigger the break.
            if do_actor and args.target_kl is not None and approx_kl > args.target_kl:
                break
        ...
        if pmpo_active:
            ...
            # DG gate diagnostics: gate_mean ~ how much total gradient mass survives; gate_min/max
            # track saturation (a gate pinned at 0 or 1 means the sigmoid is no longer graded).
            writer.add_scalar("losses/dg_gate_mean", dg_gate_mean_val.item(), global_step)
            writer.add_scalar("losses/dg_gate_min", dg_gate_min_val.item(), global_step)
            writer.add_scalar("losses/dg_gate_max", dg_gate_max_val.item(), global_step)
            writer.add_scalar("losses/dg_chi_std", dg_chi_std_val.item(), global_step)
            writer.add_scalar("losses/dg_ell_mean", dg_ell_mean_val.item(), global_step)
            with torch.no_grad():
                writer.add_scalar("losses/adv_abs_mean", b_advantages.abs().mean().item(), global_step)
