# PMPO-D4 + ReluSq + LayerNorm-before-mean + ActorHidden-to-Critic v1.
#
# Ablation of pmpo_d4_lnmean_relusq_v1: the critic now receives the actor's
# final hidden activation (the LayerNorm(64) output, just before the action
# mean head) concatenated with the observation. The hidden is *detached* so
# critic gradients cannot flow back into the actor — the critic is only being
# offered a privileged *view* of what the actor is "thinking" at this state.
#
# Hypothesis: the actor's pre-head latent is a compressed, action-relevant
# summary of the state. Giving the critic that summary should make the value
# function easier to fit on states the policy actually visits, especially when
# the actor's representation captures structure (e.g. gait phase, contact
# pattern) that a from-scratch critic body would have to rediscover.
#
# Only the critic input dim and the actor-trunk/head split change vs v1.
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # Actor split into trunk (up to and including LayerNorm) and head.
        # The trunk output is the "final hidden" we expose to the critic.
        ...
        self.actor_head = layer_init(nn.Linear(ACTOR_HIDDEN_DIM, act_dim), std=0.01)
    ...
    def actor_mean_from_hidden(self, hidden):
    ...
    def get_action_distribution(self, x):
        hidden = self.actor_trunk(x)
        action_mean = self.actor_mean_from_hidden(hidden)
    ...
    def get_squashed_action_and_value(self, x, action=None):
        ...
        action_logstd = self.actor_logstd.expand_as(action_mean)
        action_std = torch.exp(action_logstd)
        latent_dist = Normal(action_mean, action_std)
        if action is None:
