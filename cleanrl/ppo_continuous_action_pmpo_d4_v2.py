# PMPO-D4 v2: PMPO with PPO-style ablations to localize what matters.
#
# This is a PARTIAL ablation: we keep tanh-squashed actions and the reverse-KL
# term (the latter acts as the trust region in place of PPO's ratio clipping).
# We change ONLY the two PPO-style features the user named:
#   - pmpo_norm_adv: applies PPO's per-minibatch advantage normalization
#     `(adv - mean) / (std + 1e-8)` BEFORE the PMPO weighting / sign-split.
#     Identical formula to `cleanrl/ppo_continuous_action.py` ~line 277. Tests
#     whether PMPO's tanh(|adv|) saturation is tuned to unnormalized GAE scale.
#   - pmpo_use_signed: replaces the bucket-balanced sign-split + tanh(|adv|)
#     pg_loss with `pg_loss = -(adv * ratio).mean()`. Signed magnitude is used
#     directly via the importance ratio — no sign-split bucket, no tanh
#     saturation, and no clip (reverse KL handles the trust region). Tests
#     whether PMPO's custom magnitude/sign handling is what wins.
# Reverse KL and tanh-squashing are intentionally untouched. `policy_objective
# =ppo` still keeps the base CleanRL PPO path for reference.
...
class Args:
    ...
    pmpo_norm_adv: bool = False
    """v2 ablation: normalize advantage (subtract mean, divide std) before PMPO weighting"""
    pmpo_use_signed: bool = False
    """v2 ablation: drop sign-split + tanh saturation; use pg_loss = -mean(adv * ratio) (no clip; reverse KL is the trust region)"""
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.policy_objective == "ppo":
                    ...
                    if args.pmpo_use_signed:
                        # PPO-like loss form, no clip: signed adv * importance ratio.
                        # No sign split, no tanh saturation; reverse KL is the trust region.
