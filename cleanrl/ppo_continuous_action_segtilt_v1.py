# Segment-tilted PPO: grade generations, not timesteps.
#
# The part of MaxRL worth keeping is structural, not its formulae.  A generation is a
# LATENT variable and only its OUTCOME is graded; the gradient is then the score averaged
# over the outcome-conditioned posterior.  Every earlier version here (tail, bestof v1/v2)
# applied an outcome-level weight to individual TIMESTEPS instead, which is a different
# object: v2's empirical CDF ranks a timestep against other STATES' advantages, so it is
# a pooled marginal over (s,a) -- neither a per-state conditional nor a trajectory outcome.
# The measured cost of that mismatch: with a perfect baseline the ratio of per-state
# std(w) across five state groups ran 19.6 / 22.4 / 56.7 / 261.7 at T=1/2/4/8, and the
# effective sample size fell to ~42 of 32768 at the kurtosis these runs actually log.
# The transform was never the problem.  The object it acted on was.
#
# So grade a generation.  Cut each env's rollout into contiguous segments of
# `segment_len` steps; a segment's action sequence is the latent, and its realized
# performance is the grade.  Score it by the mean GAE advantage over the segment -- a
# critic-normalized "how much better than predicted did this stretch of behaviour do",
# which is comparable across segments starting from different states and is safe at
# episode boundaries because GAE has already handled them.  Then rank the segments and
# tilt the batch toward the good ones:
#
#     w_t = T * F(segment score)^(T-1) * A_t
#
# This is pass@T applied to the right object.  `T * F^(T-1)` is the density of the best of
# T draws, bounded in [0, T] and renormalized to mean exactly 1, so the tilt cannot smuggle
# in a scale change -- the defect that silently turned an earlier sweep into a
# learning-rate sweep.  At T=1 the multiplier is identically 1 and w_t = A_t, so PPO is a
# strict MEMBER of this family rather than a control standing beside it.
#
# Nothing else moves.  GAE, the critic, the advantage's own per-timestep credit
# assignment, gradient clipping and every hyperparameter are the baseline's.  The only
# thing replaced is which distribution over behaviour the policy gradient is tilted toward.
#
# Conditioning.  16 envs x 1024 steps at segment_len=64 gives 256 segments per batch --
# a well-conditioned ECDF, against 16 episodes (too few to rank) or 16384 pooled timesteps
# (the wrong object).  `segment_len` is swept because it sets what counts as one attempt.
#
# Hypothesis.  If outcome-conditioning is worth anything here, tilting toward
# better-performing stretches of behaviour beats PPO on HalfCheetah, and the `lower` arm
# -- which tilts toward the worst segments -- is symmetrically worse.  If `upper` and
# `lower` are both indistinguishable from T=1, the tilt is not reaching the policy and the
# objective has nothing to offer this domain at any granularity.
#
...
class Args:
    ...
    tilt_order: float = 1.0
    """T in best-of-T: 1.0 is exactly PPO, larger tilts harder toward good segments"""
    segment_len: int = 64
    """steps per graded generation; must divide num_steps"""
    tilt_side: str = "upper"
    """'upper' tilts toward the best segments; 'lower' is the falsification arm"""
...
def segment_multiplier(advantages, segment_len, order, sign):
    """Rank whole segments of behaviour and return a mean-1 per-timestep tilt.

    ``advantages`` is ``(num_steps, num_envs)``; segments are contiguous in time within an
    env. The multiplier is ``T * F^(T-1)``, the density of the maximum of T draws, so a
    segment's share of the gradient is its probability of being the best of T attempts.
    Renormalized to mean exactly 1: a tilt must move gradient BETWEEN segments, never
    change how much there is in total.
    """
    steps, envs = advantages.shape
    grouped = advantages.view(steps // segment_len, segment_len, envs)
    scores = (sign * grouped.mean(dim=1)).flatten()
    count = scores.numel()
    ranks = torch.empty_like(scores)
    ranks[scores.argsort()] = (
        torch.arange(count, device=scores.device, dtype=scores.dtype) + 0.5) / count
    multiplier = order * ranks.pow(order - 1.0)
    multiplier = multiplier / multiplier.mean().clamp_min(1e-12)
    return multiplier.view(steps // segment_len, 1, envs).expand(
        steps // segment_len, segment_len, envs).reshape(steps, envs)
...
def validate_args(args):
    ...
    if args.tilt_order < 1.0:
        raise ValueError("tilt_order must be at least 1; T=1 is PPO and T<1 is not an order")
    if args.segment_len <= 0 or args.num_steps % args.segment_len:
        raise ValueError("segment_len must be positive and divide num_steps")
    if args.tilt_side not in {"upper", "lower"}:
        raise ValueError("tilt_side must be 'upper' or 'lower'")
    return args
...
def main():
    ...
    try:
        ...
        tilt_sign = 1.0 if args.tilt_side == "upper" else -1.0
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                # Cloned before anything else reads it: `advantages` is a CUDA-graph
                # output and is overwritten by the next launch.
                tilted = advantages.clone()
                b_returns = returns.flatten().clone()
                if args.tilt_order <= 1.0:
                    # T=1: the multiplier is identically 1, so this is PPO to the bit and
                    # no ranking is computed at all.
                    multiplier = None
                else:
                    multiplier = segment_multiplier(
                        tilted, args.segment_len, args.tilt_order, tilt_sign)
                    tilted = tilted * multiplier
                b_advantages = tilted.flatten()
                # Read from tensors already materialised above. An earlier variant logged
                # a counterfactual by calling the GAE kernel a second time; under CUDA
                # graphs that raises on the overwritten output and killed every treated
                # arm at iteration 1, so diagnostics here never launch anything.
                raw = advantages.flatten()
                if multiplier is None:
                    multiplier_ess = torch.ones((), device=device)
                    multiplier_max = torch.ones((), device=device)
                    weight_adv_rho = torch.ones((), device=device)
                    segment_score_std = torch.zeros((), device=device)
                else:
                    flat_multiplier = multiplier.flatten()
                    # Share of segments actually carrying gradient, in [0, 1]. The failure
                    # mode of every earlier version was this collapsing toward zero.
                    multiplier_ess = (flat_multiplier.sum() ** 2) / (
                        flat_multiplier.numel() * (flat_multiplier ** 2).sum().clamp_min(1e-12))
                    multiplier_max = flat_multiplier.max()
                    grouped = advantages.view(
                        args.num_steps // args.segment_len, args.segment_len, args.num_envs)
                    segment_score_std = grouped.mean(dim=1).std()
                    centred_w = b_advantages - b_advantages.mean()
                    centred_a = raw - raw.mean()
            ...
            logged = gather_metrics({
                ...
                "tilt/multiplier_ess": multiplier_ess,
                "tilt/multiplier_max": multiplier_max,
                "tilt/segment_score_std": segment_score_std,
                "tilt/weight_adv_rho": weight_adv_rho,
            })
