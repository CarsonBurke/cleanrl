# IterThink v24 beta (strong on-policy base) + FAITHFUL Delightful Policy Gradient,
# with the APPROPRIATE non-negative surprisal. From arXiv:2603.14608v1, Algorithm 2.
#
# DG replaces the per-sample score weight: w_t = sigmoid(U_t * ell_t / eta), eta=1,
# detached; update = -(w * U * log pi(a)).mean(). No PPO ratio, no clipped surrogate,
# no advantage normalization (U is the raw GAE advantage, the paper's U_t), NO whitening.
# The critic (511-bin distributional), GAE, Beta actor, shared trunk, decoupled grad
# clip, and target_kl early-stop are the UNCHANGED v24_beta base; only the actor loss
# changes. On-policy, no replay. The actor takes a SINGLE faithful pass per batch
# (actor_epochs=1, Algorithm 2); the critic keeps its update_epochs=10 so advantages U
# stay well-estimated. target_kl gates only the actor, never truncating critic epochs.
#
# THE FIX. The paper's continuous surprisal ell = -log pi(a|s) is a log-DENSITY: on a
# sharp Beta (this base's default actor) the density at the mode is >>1, so -log pi goes
# strongly NEGATIVE there and chi = U*ell carries the OPPOSITE sign of U -> the gate
# suppresses breakthroughs and amplifies blunders (measured: sign_agree -> 0). The
# paper's clip(+-10) bounds magnitude but NOT sign; its optional whitening only recenters
# chi (inert gate ~ 0.5). The appropriate surprisal is PEAK-REFERENCED:
#     ell = log pi(mode) - log pi(a) >= 0   (native space, summed over dims)
# The per-state log-normalizer cancels, leaving the pure distance-from-mode (Beta
# tail-depth / Gaussian Mahalanobis): favored action -> ell~0 (gate ~1/2), tail action
# -> ell large (gate opens/closes by sign(U)) -- exactly the discrete behavior the gate
# assumes, and scale-invariant (fixes the paper's own action-scaling caveat at the source).
#
# HYPOTHESIS. Prior on-policy gate failures used either the WEAK clean Beta base or the
# INVERTED raw surprisal; this is the untested combination -- strong base + faithful gate
# + correct (>=0) surprisal + no whitening. Falsifiable: if it matches/beats the base
# (iterthink_v24_beta_s1), the gate mechanism transfers on-policy once the surprisal is
# sign-correct; if it still trails, it confirms the gate needs the off-policy/replay
# redundancy the paper's own control setup (2M replay buffer) uses.
#
...
class Args:
    ...
    update_epochs: int = 10          # CRITIC epochs (keeps the distributional critic well-fit -> good U)
    actor_epochs: int = 1            # DG actor passes per batch: faithful Algorithm 2 = single pass
    norm_adv: bool = False           # DG: raw advantage U (paper uses U_t directly, no PPO norm)
    ...
    adv_transform: str = "v10"       # DG: identity -> U is the raw GAE advantage (paper's U_t)
    ...
    # Faithful Delightful Policy Gradient (Algorithm 2). The actor uses the gated
    # score function (no PPO ratio/clip); the critic/GAE/trunk are the unchanged base.
    dg_eta: float = 1.0              # gate temperature eta (paper default 1)
    dg_clip: float = 10.0            # surprisal magnitude clip (paper C=10)
    dg_surprisal: str = "peak_ref"   # "mahalanobis" (Gaussian-analog moment-matched, Beta-safe floor) | "peak_ref" (mode-ref, >=0; ~0 for uniform Beta) | "raw_clip" (literal -log density)
    dg_whiten: bool = False          # OFF: no per-minibatch whitening of chi (paper's optional patch)
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, z=None):
        ...
        # DG surprisal ell (>= 0), summed over dims, in the distribution's NATIVE space.
        if self.dg_surprisal == "mahalanobis":
            # Gaussian-ANALOGOUS moment-matched surprisal: 0.5*||(z-mu)/sigma||^2 using
            # the distribution's OWN mean/std. For ANY dist (incl. a UNIFORM Beta),
            # E[ell] = 0.5*Sum E[(z-mu)^2]/sigma^2 = 0.5*D -- the sigma-INDEPENDENT floor
            # the Gaussian's 0.5*||eps||^2 has but the Beta's log-density LACKS (a near-
            # uniform Beta on bounded support has density ~1 -> log-prob surprisal ~0).
            # Tail action -> large; central action -> ~0; always >=0 (no gate inversion).
            mean = dist.mean
            std = dist.stddev
            surprisal = (0.5 * ((z - mean) / (std + 1e-6)) ** 2).sum(1)
        else:
            # peak-referenced ell = log pi(mode) - log pi(z): the per-state log-normalizer
            # cancels, leaving distance-from-mode. NOTE: collapses to ~0 for a near-uniform
            # Beta (flat density) -> dead gate; use "mahalanobis" for the Beta.
            if self.actor_dist == "gaussian":
                mode = dist.mean
            else:  # beta: mode = (a-1)/(a+b-2) for a,b>1; flat (a=b=1) -> denom~0 -> ell~0
                a_c, b_c = dist.concentration1, dist.concentration0
                denom = (a_c + b_c - 2.0).clamp_min(1e-6)
                mode = ((a_c - 1.0) / denom).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            surprisal = (dist.log_prob(mode) - dist.log_prob(z)).sum(1).clamp_min(0.0)
        return action, z, log_prob, entropy, value_logits, surprisal
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, value_logits, _ = agent.get_action_and_value(next_obs)
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                _, _, boot_logprob, _, boot_logits, _ = agent.get_action_and_value(next_obs)
        ...
        clipfracs = []
        actor_gn = 0.0       # may stay 0 on critic-only epochs (logging)
        actor_done = False   # set when target_kl is hit; stops actor updates, critic still finishes
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_logits, surprisal = agent.get_action_and_value(
                ...
                # FAITHFUL Delightful Policy Gradient (Algorithm 2, arXiv:2603.14608v1),
                # with the APPROPRIATE non-negative surprisal so the gate does not invert.
                #   U   = mb_advantages (raw advantage; adv_transform=v10, norm_adv=off)
                #   ell = peak-referenced surprisal >= 0 (from get_action_and_value),
                #         clipped to [0, dg_clip] for magnitude parity with the paper.
                #   chi = U * ell ;  w = sigmoid(chi/eta) (DETACHED) ;  eta=1
                #   pg  = -(w * U * log pi(a)).mean()   -- gated score function.
                # No PPO importance ratio, no clipped surrogate, no whitening. The detach
                # makes autograd match the gated score-function estimator exactly. ratio /
                # approx_kl above remain only for logging and the target_kl early-stop.
                ell = surprisal
                if args.dg_surprisal == "raw_clip":  # literal-paper ablation: -log density
                    ell = (-newlogprob).clamp(-args.dg_clip, args.dg_clip)
                else:  # "mahalanobis" / "peak_ref": already >= 0, just bound magnitude
                    ell = ell.clamp(0.0, args.dg_clip)
                U = mb_advantages
                chi = U * ell
                if args.dg_whiten:  # OFF by default; kept only as an ablation knob
                    chi = (chi - chi.mean()) / (chi.std(unbiased=False) + 1e-8)
                w = torch.sigmoid(chi / args.dg_eta).detach()
                pg_loss = -(w * U * newlogprob).mean()
                ...
                # DG = faithful single-pass actor (Algorithm 2): apply the policy
                # gradient only for the first actor_epochs; later epochs train the
                # CRITIC only (keeps it well-fit -> clean advantages U) without further
                # moving the policy off the batch it was estimated on.
                do_actor = (epoch < args.actor_epochs) and not actor_done
                if args.separate_grad_clip:
                    ...
                    (args.vf_coef * v_loss).backward(retain_graph=do_actor)
                    ...
                    loss = (pg_loss - ent_coef_eff * entropy_loss if do_actor else 0.0) + v_loss * args.vf_coef
                    optimizer.zero_grad()
                    loss.backward()
            ...
            # target_kl stops the ACTOR (single-pass DG rarely trips it) but must NOT
            # truncate the critic's remaining epochs, so set a flag instead of breaking.
            if (epoch < args.actor_epochs) and args.target_kl is not None and approx_kl > args.target_kl:
                actor_done = True
        ...
        # DG gate diagnostics (last minibatch). gate_mean~0.5 = inert; spread away from
        # 0.5 = the gate is actually selecting. sign_agree = frac(sign(chi)==sign(U)):
        # must be ~1.0 (peak-ref ell>=0 keeps chi aligned with U); a drop signals the
        # surprisal inverted the gate. surprisal_mean/chi_std confirm the gate has range.
        with torch.no_grad():
            writer.add_scalar("charts/dg_gate_mean", w.mean().item(), global_step)
            writer.add_scalar("charts/dg_gate_std", w.std().item(), global_step)
            writer.add_scalar("charts/dg_surprisal_mean", ell.mean().item(), global_step)
            writer.add_scalar("charts/dg_chi_std", chi.std().item(), global_step)
            writer.add_scalar("charts/dg_sign_agree",
                              (torch.sign(chi) == torch.sign(U)).float().mean().item(), global_step)
