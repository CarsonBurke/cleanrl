# LeWM head-only PPO v3: selected 64x64 backbones learn only prediction + SIGReg.
# PPO heads read detached backbone features. Both JEPA time branches stay attached.
# Training-only unbounded projectors, action embeddings, and AdaLN-zero MLPs;
# no attention/causal transformer, EMA, target network, or extra inference parameters.
# Separate PPO Adam / representation AdamW and clipping prevent cross-loss scaling.
import os
import random
import time
...
class Args:
    ...
    """which backbones learn from LeWM only; unselected branches retain ordinary PPO"""
...
class LeWMBranch(nn.Module):
    """Training-only LeWM projection/prediction stack on an unchanged PPO backbone.
...
class Agent(nn.Module):
    def __init__(self, envs, jepa_mode="both", sigreg_num_proj=1024, sigreg_proj_chunk=256):
        ...
        # Construct after both unchanged PPO trunks: the inference initialization is identical.
        self.actor_ssl = LeWMBranch(self.action_dim, sigreg_num_proj, sigreg_proj_chunk) if jepa_mode in {"actor", "both"} else None
        self.critic_ssl = LeWMBranch(self.action_dim, sigreg_num_proj, sigreg_proj_chunk) if jepa_mode in {"critic", "both"} else None

    @staticmethod
    def encode(trunk, observations):
        return trunk[3](trunk[2](trunk[1](trunk[0](observations))))

    def parameter_groups(self):
        """Disjoint owners: PPO heads/unselected trunks versus selected backbones + SSL."""
        policy, representation = [], []
        for name in ("actor", "critic"):
            trunk, branch = getattr(self, name), getattr(self, name + "_ssl")
            if branch is None:
                policy.extend(trunk.parameters())
            else:
                policy.extend(trunk[4].parameters())
                representation.extend(trunk[:4].parameters())
                representation.extend(branch.parameters())
        return tuple(policy), tuple(representation)

    def parameter_counts(self):
        counts = {}
        for name in ("actor", "critic"):
            trunk = getattr(self, name)
            counts[name + "_backbone"] = sum(parameter.numel() for parameter in trunk[:4].parameters())
            counts[name + "_head"] = sum(parameter.numel() for parameter in trunk[4].parameters())
    ...
    def ssl_losses(self, actor_latent, critic_latent, next_observations, native_actions):
        zero = actor_latent.new_zeros(())
        actor_terms = critic_terms = (zero, zero, zero, zero)
        if self.actor_ssl is not None:
            actor_terms = self.actor_ssl(actor_latent, self.encode(self.actor, next_observations), native_actions)
        if self.critic_ssl is not None:
            critic_terms = self.critic_ssl(critic_latent, self.encode(self.critic, next_observations), native_actions)
        metrics = torch.stack(tuple(term.detach() for term in actor_terms + critic_terms))
        return actor_terms[0], actor_terms[1], critic_terms[0], critic_terms[1], metrics

    def get_value(self, x):
        latent = self.encode(self.critic, x)
        return self.critic[4](latent.detach() if self.critic_ssl is not None else latent)
    ...
    def get_policy_value_latents(self, x):
        actor_latent = self.encode(self.actor, x)
        critic_latent = self.encode(self.critic, x)
        logits = self.actor[4](actor_latent.detach() if self.actor_ssl is not None else actor_latent)
        value = self.critic[4](critic_latent.detach() if self.critic_ssl is not None else critic_latent)
...
def loss_components(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args, next_observations=None):
    """PPO actor/value, then actor prediction/SIGReg and critic prediction/SIGReg."""
    ...
    actor_prediction = actor_regularization = critic_prediction = critic_regularization = observations.new_zeros(())
    ...
    if args.jepa_mode != "none":
        actor_prediction, actor_regularization, critic_prediction, critic_regularization, ssl_metrics = agent.ssl_losses(
            actor_latent, critic_latent, next_observations, native_actions,
        )
        metrics = torch.cat((metrics, ssl_metrics))
    components = torch.stack((actor_objective, critic_objective,
                              actor_prediction, args.sigreg_weight * actor_regularization,
                              critic_prediction, args.sigreg_weight * critic_regularization))
...
def gradient_balance(agent, components):
    """Sample the two representation objectives on shared backbone parameters only."""
    metrics = {}
    for index, name in enumerate(("actor", "critic")):
        if getattr(agent, name + "_ssl") is None:
            continue
        parameters = tuple(getattr(agent, name)[:4].parameters())
        prediction = torch.autograd.grad(components[2 + 2 * index], parameters, retain_graph=True)
        regularization = torch.autograd.grad(components[3 + 2 * index], parameters, retain_graph=True)
        with torch.no_grad():
            ...
            metrics[prefix + "_prediction_grad_norm"] = prediction_norm
            metrics[prefix + "_sigreg_grad_norm"] = sigreg_norm
            metrics[prefix + "_sigreg_to_prediction_grad_ratio"] = sigreg_norm / prediction_norm.clamp_min(1e-12)
            metrics[prefix + "_grad_cosine"] = dot / (prediction_norm * sigreg_norm).clamp_min(1e-12)
    return metrics
...
def main():
    ...
    try:
        ...
        update_metrics = torch.empty((max_updates, 14 if use_jepa else 6), device=device)
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            if use_jepa:
                ...
                for index, branch in enumerate(("actor", "critic")):
                    if getattr(agent, branch + "_ssl") is None:
                        continue
                    for offset, name in enumerate(("prediction_loss", "sigreg_loss", "projected_std", "backbone_std")):
                        metric_values[f"ssl/{branch}_{name}"] = update_metrics[:updates, 6 + 4 * index + offset].mean()
