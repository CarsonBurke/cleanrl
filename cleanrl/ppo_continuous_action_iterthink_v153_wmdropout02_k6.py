# PPO + IterThink v153 wmdropout02-k6 (v152 + LeWM-style WM dropout).
#
# Key change from v152:
#   - Add dropout=0.2 inside the world-model transformer attention and FFN
#     branches, matching ../le-wm's placement: SDPA attention dropout, attention
#     output dropout, FFN hidden dropout, and FFN output dropout. Dropout is active
#     for supervised WM training and imagination rollouts/training, but disabled
#     for real-environment agent-input encoding.
#
# -----------------------------------------------------------------------------
# Previous header:
...
def ngpt_gated_silu_mlp(x, w1, w2, act_scale, dropout_p=0.0, training=False):
    uv = (w1(x) * act_scale.to(x.dtype)) * (x.shape[-1] ** 0.5)
    u, v = torch.chunk(uv, 2, dim=-1)
    h = F.dropout(u * F.silu(v), p=dropout_p, training=training)
    return F.dropout(w2(h), p=dropout_p, training=training)
...
class NGPTSelfAttention(nn.Module):
    ...
    def forward(self, x, causal=False):
        ...
        attn_out = F.scaled_dot_product_attention(
            q, k, v, is_causal=causal, scale=self.head_dim ** 0.5, dropout_p=drop
        )
        attn_out = attn_out.transpose(1, 2).reshape(batch, seq_len, width)
        return F.dropout(self.proj(attn_out), p=self.dropout, training=self.training)


class LeWMTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_mult, dropout=0.0):
        super().__init__()
        self.attn = NGPTSelfAttention(dim, num_heads, dropout)
    ...
    def forward(self, x, causal=False):
        ...
        ffn = ngpt_gated_silu_mlp(
            x, self.w1, self.w2, self.ffn_act_scale, dropout_p=self.dropout, training=self.training
        )
        return ngpt_residual(x, ffn, self.ffn_alpha)
...
class _SpaceAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        k = sqk * unit_norm(k)
        drop = self.dropout if self.training else 0.0
        attn_out = F.scaled_dot_product_attention(q, k, v, scale=self.head_dim ** 0.5, dropout_p=drop)
        attn_out = attn_out.transpose(1, 2).reshape(batch, seq_len, width)
        return F.dropout(self.proj(attn_out), p=self.dropout, training=self.training)
...
class _RoPEAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, rope_fraction, dropout=0.0, rope_theta=10000.0):
        super().__init__()
        if dim % num_heads != 0:
            raise ValueError(f"dim {dim} not divisible by num_heads {num_heads}")
        self.dim = dim
    ...
    def forward(self, x, causal=True):
        # PyTorch's efficient CUDA SDPA dropout path cannot generate valid RNG
        # offsets when the flattened batch exceeds 65535. Imagination time-axis
        # attention has shape (B_im * space_tokens, T, D), so chunk only this rare
        # dropout-active path and keep the deterministic/eval path untouched.
        if self.training and self.dropout > 0.0 and x.shape[0] > 65535:
            return torch.cat([self.forward(chunk, causal=causal) for chunk in x.split(32768, dim=0)], dim=0)
        batch, seq_len, width = x.shape
        qkv = self.qkv(x).reshape(batch, seq_len, 3, self.num_heads, self.head_dim)
        qkv = qkv.permute(2, 0, 3, 1, 4)                    # (3, B, heads, T, head_dim)
        q, k, v = qkv[0], qkv[1], qkv[2]
...
class AdaLNAxialPredictorBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, ffn_mult, axis, rope_fraction, dropout=0.0):
        super().__init__()
        ...
        if axis == "time":
            self.attn = _RoPEAttention(dim, num_heads, rope_fraction, dropout)
        else:
            self.attn = _SpaceAttention(dim, num_heads, dropout)
        self.w1 = xavier_linear(nn.Linear(dim, 2 * dim * ffn_mult, bias=False))
        self.w2 = xavier_linear(nn.Linear(dim * ffn_mult, dim, bias=False))
    ...
    def forward(self, x, action_features):
        ...
        if self.axis == "space":
            ...
            y = ngpt_modulated_residual(
                y,
                ngpt_gated_silu_mlp(
                    h, self.w1, self.w2, self.ffn_act_scale, dropout_p=self.dropout, training=self.training
                ),
                bc(m2),
                self.ffn_eta,
            )
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.encoder_layers = nn.ModuleList(
            [
                LeWMTransformerBlock(self.dim, args.lewm_heads, args.lewm_ffn_mult, args.lewm_dropout)
                for _ in range(args.lewm_encoder_layers)
            ]
        )
...
if __name__ == "__main__":
    ...
    # WM dropout is a training/dream regularizer. Real-environment rollout uses a
    # deterministic WM state representation for stable policy inputs.
