# Ctrl-base PPO with one-step action feedback, v1.
#
# Keep ctrl_base's additive pre-RMS SiTU-GLU trunk, Beta policy, full-batch
# updates, raw GAE, and 32x learning rate. The actor becomes
# pi(a_t | s_t, a_{t-1}) by receiving the previous native Beta action. Native
# feedback is reset to 0.5 at episode boundaries; the critic remains V(s) so
# time-limit bootstrapping stays identical to ctrl_base.
#
# Hypothesis: the actor's previous action supplies actuator momentum information
# without recurrence or a larger rollout model.
import os
import random
import time
...
class Args:
    ...
    activation: Literal["lrelusq", "stiglu"] = "lrelusq"
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, placement="pre", norm_kind="layer", activation="lrelusq"):
        super().__init__()
        ...
        self.critic = nn.Sequential(
            make_norm_residual_trunk(observation_dim, 64, placement=placement,
                                     norm_kind=norm_kind, activation=activation),
            layer_init(nn.Linear(64, 1), std=1.0),
        )
        self.actor_input_dim = observation_dim + self.action_dim
        self.actor = nn.Sequential(
            make_norm_residual_trunk(self.actor_input_dim, 64, placement=placement,
                                     norm_kind=norm_kind, activation=activation),
    ...
    def get_policy_and_value(self, observations, actor_inputs):
    ...
    def get_action_and_value(self, observations, action=None, previous_native_action=None):
        """Public API uses physical actions; feedback is the previous native action."""
        ...
        alpha, beta, value = self.get_policy_and_value(observations, actor_inputs)
        if action is None:
...
def ppo_loss(agent, actor_inputs, observations, native_actions, old_logprobs, advantages, returns, old_values, args):
    """Pure clipped PPO loss on native Beta samples; no inverse action scaling."""
    alpha, beta, newvalue = agent.get_policy_and_value(observations, actor_inputs)
    distribution = Beta(alpha, beta, validate_args=False)
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", f"Beta host actor with previous native action feedback; "
                        f"additive {args.placement}-{args.norm_kind}norm {args.activation} residual trunk; "
                        "32x LR; 1 minibatch; raw GAE")
        envs = make_training_env(args, run_name)
        resources.callback(envs.close)
        agent = Agent(envs, placement=args.placement, norm_kind=args.norm_kind,
        ...
        def rollout_statistics(actor_inputs, observations, native):
            """Evaluate the exact conditional actor inputs used by the rollout."""
            alpha, beta, value = agent.get_policy_and_value(observations, actor_inputs)
            return value.flatten(), agent.action_logprob(alpha, beta, native)

        def loss_model(actor_inputs, observations, native, old_logprobs, advantages, returns, old_values):
            return ppo_loss(
                agent, actor_inputs, observations, native, old_logprobs, advantages, returns, old_values, args
            )
        ...
        # The host actor consumes observation plus previous native action.
            ...
            for step in range(args.num_steps):
                ...
                with timer.span("normalize_transfer", use_cuda=False):
                    ...
                    transfer.push(step, reward, terms, truncs, observations=obs_step,
                                  actor_inputs=actor_inputs, native_actions=native)
                previous_native[...] = native
                ...
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss, metrics = loss_model(
                            b_actor_inputs[indices], b_obs[indices], b_native[indices], b_logprobs[indices],
