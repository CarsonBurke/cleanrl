# HOPSD v35 — non-lossy hindsight via a transformer phi-encoder (vs pooled windows).
# =====================================================================================
...
class HindsightTeacher(nn.Module):
    """Separate teacher actor + teacher critic sharing one transformer phi-encoder.

    encoder: (future actions, validity) -> summary phi (d_model). SHARED by both heads.
    Actor input:  [obs, summary, g (z-scored lambda-return, only if teacher_sees_g)].
    Critic input: [obs, summary]  (no return features -> no return-copier degeneracy).
    """
    ...
    def __init__(self, obs_dim, act_dim, args):
        ...
        actor_in = obs_dim + d + (1 if args.teacher_sees_g else 0)
        critic_in = obs_dim + d
    ...
    def actor_params_from(self, obs, summary, g=None):
        ctx = [obs, summary] if g is None else [obs, summary, g]
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # v35: the raw future-action window + validity mask are built here (they do not
            # depend on advantages) and encoded by the transformer to the teacher-critic
            # value. The actor context (which may need g) is assembled below.
            fut_actions, fut_valid, fut_valid_frac = build_future_window(
            ...
            vt_chunks = []
            for start in range(0, fa_flat.shape[0], args.minibatch_size):
                sl = slice(start, start + args.minibatch_size)
                summ = teacher.encode(fa_flat[sl], fv_flat[sl])
                vt_chunks.append(hl_support.to_scalar(teacher.critic_logits_from(obs_flat_roll[sl], summ)))
        ...
        b_future_actions = fut_actions.reshape(-1, args.hindsight_horizon, act_dim)
        b_future_valid = fut_valid.reshape(-1, args.hindsight_horizon)
        ...
        with torch.no_grad():
            _summ_tel = teacher.encode(b_future_actions[_tel], b_future_valid[_tel])
            _ta, _tb = teacher.actor_params_from(
                b_obs[_tel], _summ_tel, b_g[_tel] if args.teacher_sees_g else None
            )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Encode the future window ONCE per minibatch; the summary feeds both the
                # actor and the HL-Gauss critic, so gradient from both losses flows into the
                # shared transformer encoder. (Teacher stays eager; its encoder is compiled
                # via --compile-teacher.)
                t_summary = teacher.encode(b_future_actions[mb_inds], b_future_valid[mb_inds])
                t_alpha, t_beta = teacher.actor_params_from(
                    b_obs[mb_inds], t_summary, b_g[mb_inds] if args.teacher_sees_g else None
                )
                t_dist = Beta(t_alpha, t_beta)
                t_nll = -t_dist.log_prob(b_latent_zs[mb_inds]).sum(-1)
                teacher_actor_loss = (b_awr_w[mb_inds] * t_nll).mean()
                t_value_logits = teacher.critic_logits_from(b_obs[mb_inds], t_summary)
                ...
                (v_loss, distill_loss, qterm, ent_bonus, entropy_mean, approx_kl,
                 distill_kl_g, clipfrac_g, qadv_abs_g, qadv_sig_g, gate_g) = student_step(
                    ...
                    tp_mb, tm_mb, t_alpha.detach(), t_beta.detach(), qadv_scale_t,
                )
        ...
        with torch.no_grad():
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                sl = slice(start, start + args.minibatch_size)
                summ = teacher.encode(b_future_actions[sl], b_future_valid[sl])
                ta, tb = teacher.actor_params_from(
                    b_obs[sl], summ, b_g[sl] if args.teacher_sees_g else None
                )
