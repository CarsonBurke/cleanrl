# Checked directional-curvature Adam on base PPO, v2.
# Predict using the current UNCLIPPED gradient and actual parameter displacement.
# Actor/critic have separate lengths and losses; one same-batch trial estimates
# directional curvature, a second checks its predicted optimum (or doubles the
# length if no positive curvature is observed). Keep the best checked point,
# including the origin. No LR ceiling, schedule, ratio EMA or layer attribution.
# Adam supplies directions; uphill momentum uses the current preconditioned
# gradient instead. Joint gradient clipping remains 0.5 for Adam observations.
# Hypothesis: directional loss geometry, not momentum magnitude, chooses useful
# step lengths. PPO's surrogate is not policy return; monitor both KL and score.
# Two extra no-grad forwards; all trials share frozen minibatch targets. Adam
# moments advance once per minibatch, including when neither trial is accepted.
import os
import random
import time
...
def prepare_trial(params, gradients, moments, seconds, steps, origins, directions,
    ...
    norm = torch.stack([g.square().sum() for group in gradients for g in group]).sum().sqrt()
...
class CurvatureAdam(optim.Optimizer):
    """Two separable objectives; one Adam observation and two checked trials per batch.
    ...
    def __init__(self, actor_params, critic_params, lr=3e-4, betas=(0.9, 0.999),
        ...
        groups = [list(actor_params), list(critic_params)]
        flat = [p for group in groups for p in group]
        if any(not group for group in groups) or len({id(p) for p in flat}) != len(flat):
            raise ValueError("CurvatureAdam requires two nonempty disjoint parameter groups")
        ...
        if device.type != "cuda" or any(p.device != device for p in flat):
            raise ValueError("CurvatureAdam requires parameters on one CUDA device")
        super().__init__([dict(params=group) for group in groups], dict(betas=betas, eps=eps))
        self.groups = groups
        self.max_grad_norm = max_grad_norm
        self.lrs = torch.full((2,), lr, device=device)
        ...
        self._prepare = torch.compile(prepare_trial, fullgraph=True, dynamic=False, options=options) if compile else prepare_trial
        self._propose = torch.compile(propose_trial, fullgraph=True, dynamic=False, options=options) if compile else propose_trial
        self._finish = torch.compile(finish_trial, fullgraph=True, dynamic=False, options=options) if compile else finish_trial
    ...
    def probe(self):
        ...
        self._prepare(self.groups, gradients, self.moments, self.seconds, self.steps,
                      ...
                      self.probe_prediction, self.fallback, *settings["betas"], settings["eps"],
                      self.max_grad_norm)
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args):
    ...
    objectives = torch.stack((pg_loss - args.ent_coef * entropy_loss, v_loss * args.vf_coef))
    loss = objectives.sum()
    metrics = torch.stack((pg_loss.detach(), v_loss.detach(), entropy_loss.detach(),
                           old_approx_kl, approx_kl, clipfrac))
    return loss, metrics, objectives


def curvature_metrics(checks):
    metrics = {}
    for slot, name in enumerate(("actor", "critic")):
        values = checks[:, slot]
...
def main():
    ...
    try:
        ...
        optimizer = CurvatureAdam(agent.actor.parameters(), agent.critic.parameters(),
        ...
        def check_model(observations, native, old_logprobs, advantages, returns, old_values):
            return ppo_loss(agent, observations, native, old_logprobs, advantages, returns, old_values, args)[2]
        ...
        prediction_metrics = torch.empty((max_updates, 2, 10), device=device)
        timer = PhaseTimer()
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        optimizer.probe()
                        with torch.no_grad():
                            probe_loss = check_model(*minibatch).clone()
                            optimizer.propose(before_loss, probe_loss)
                            candidate_loss = check_model(*minibatch)
            ...
            writer.add_scalar("charts/learning_rate", logged["curvature/actor/next_lr"], global_step)
