# nGPT hypersphere satransformer v6: v5 role/value-token backbone with the
# iterthink learner stack.
#
# Diff vs v5: replace SPO+plain minibatch advantage standardization with the
# iterthink winner policy machinery requested here: batch rankgauss advantage
# shaping, clip-higher PPO (lower 0.2, upper 0.28), 10 update epochs, and dual
# backward actor/critic clipping. No target-KL stopping. HL-Gauss remains
# Dreamer4-style edge-bin support with sigma_to_bin_ratio=2, but uses our
# reward-normalized raw value range [-10, 10] before symlog conversion.
#
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # readouts. Unlike PMA seeds, these participate in every SA layer from
        # the beginning, so readout tokens can iteratively query and write back
        # into the shared body/muscle token field.
    ...
    def _encode(self, x):
        """Tokenize → cat[role, latent, obs] → justnorm → 4x SA → pick final
        ...
        role_tokens = torch.cat(
            [action_roles, global_actor, actor_context, critic, critic_context],
            dim=0,
        ...
        action_feats = sa_tokens[:, : self.action_dim]
        global_actor_feat = sa_tokens[:, self.action_dim]
        actor_context_feat = sa_tokens[:, self.action_dim + 1]
        critic_feat = sa_tokens[:, self.action_dim + 2]
        critic_context_feat = sa_tokens[:, self.action_dim + 3]
    ...
    def actor_parameters(self):
        ...
        params += [
            ...
            # Critic role tokens live in the same SA stream, so policy loss can
            # reach them through attention even though actor heads do not read
            # them directly. Include them here so policy gradients on these
            # tokens are actor-clipped before clipped critic grads are added.
            self.critic_token,
