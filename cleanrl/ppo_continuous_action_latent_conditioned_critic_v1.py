# Latent-conditioned critic PPO v1.
# The Beta actor is unchanged, but its pre-action 64-wide latent is detached and
# concatenated with the observation for value prediction. This gives the critic
# access to the actor's representation without allowing value loss to reshape it.
# Hypothesis: actor features expose policy-relevant state structure that improves
# value approximation while preserving independent policy-gradient dynamics.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_latent_dim = 64
        self.critic = nn.Sequential(
            layer_init(nn.Linear(observation_dim + self.actor_latent_dim, 64)), nn.Tanh(),
        ...
        self.actor_head = layer_init(nn.Linear(self.actor_latent_dim, 2 * self.action_dim), std=0.01)

    @property
    def actor(self):
        """Flat sequential actor required by the host mirror graph."""
        return nn.Sequential(*self.actor_trunk, self.actor_head)

    def get_actor_latent(self, x):
        return self.actor_trunk(x)

    def get_value(self, x):
        actor_latent = self.get_actor_latent(x).detach()
        return self.critic(torch.cat((x, actor_latent), dim=-1))

    def get_policy_and_value(self, x):
        actor_latent = self.get_actor_latent(x)
        logits = self.actor_head(actor_latent)
        alpha, beta = (F.softplus(logits) + 1.0).chunk(2, dim=-1)
        value_input = torch.cat((x, actor_latent.detach()), dim=-1)
        return alpha, beta, self.critic(value_input)
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta PPO; critic input=observation concat detached actor pre-action latent; FP32; native-action storage; host actor mirror")
