# PPO + IterThink v24 — RAW REWARD / RAW ADVANTAGE, NO RATIO CLIP v2. From rawreward_rawadv_v1.
#
# v2 CHANGE: remove the PPO SURROGATE RATIO CLIP only (clip_ratio=False). The policy
# objective becomes the plain importance-weighted policy gradient, pg_loss = -A*ratio,
# with NO clamp on the ratio. The GRAD-NORM clip is KEPT (actor/critic_grad_clip=0.25,
# unchanged from v1) — that's the deliberate distinction: we drop PPO's trust region
# but keep the gradient-magnitude leash. Everything else is identical to v1 (raw
# reward, raw advantage, update_epochs=1, target_kl off).
#
# WHY. With update_epochs=1 the ratio starts at ~1 for every sample (z-replay on the
# just-collected policy), so the clip rarely binds on the FIRST minibatches; but as the
# 32 minibatches of the single pass walk the policy, later minibatches are evaluated
# off the drifted policy and the clip begins to bite. Removing it asks: does PPO's
# clip add anything here once the step is already bounded by the 0.25 grad-norm clip
# and a single epoch? Vanilla PG (-A*ratio) is the more faithful policy-gradient
# object; the clip is a heuristic trust region. EXPECT either parity with v1 (clip was
# redundant given grad-norm clip + 1 epoch) or instability from unconstrained
# importance weights on high-|adv| raw samples. Clean clip-vs-noclip A/B; bar: v1.
#
# NORMALIZATION-FREE STUDY: strip BOTH the reward normalization and the advantage
# normalization and let the policy gradient see the natural, growing scale of the
# task. The critic support is widened to absorb the raw return magnitude.
#
#   (1) RAW REWARD. Drop the NormalizeReward wrapper (divide-by-running-return-std)
#       AND its companion reward clip(-10,10) — that clip is calibrated to the
#       normalized scale and would shred raw HalfCheetah rewards (>10/step when
#       running). The critic now regresses to the UNSCALED discounted return.
#       Observation normalization is UNCHANGED (only the reward path is stripped).
#
#   (2) RAW ADVANTAGE. adv_scale_norm="off" (no EMA-RMS), norm_adv=False,
#       adv_transform="v10" (identity). The PG sees the raw GAE: its magnitude is
#       the genuine TD-residual scale and GROWS as the policy improves (the exact
#       drift the EMA-RMS variant existed to cancel). PPO's ratio clip + target_kl
#       are the only leashes on the resulting (advantage-magnitude-scaled) step.
#
#   (3) CRITIC RANGE. The whole C51/GAE machinery here lives in RAW value space
#       (atoms = r + γ·support; value_symlog is vestigial — support is never
#       symexp'd), so the fix for the raw return scale is simply to WIDEN the linear
#       support. Raw HalfCheetah value ≈ per-step_reward/(1-γ), capped by the
#       episode: ~+800..1200 at a strong policy, down to ~-1000 for an early policy
#       running backward. Support = [-1500, 1500], 511 bins (bin_width≈5.9), with
#       critic_init_tau scaled proportionally (same 0.025·range relative sharpness
#       as the [-10,10] base) so the peaked-at-0 init still prevents the
#       distributional-bootstrap blowup. Watch debug/target_edge_mass: if it climbs
#       off ≈0 the support is clipping returns and must widen further.
#
# HYPOTHESIS: removing reward norm eliminates the divisor's drift/noise; the raw
# advantage carries true magnitude (PPO's original signal), and with a correctly
# sized critic this can match or beat the normalized variants — or expose the
# instability that motivated normalization in the first place. Either way it is the
# clean control for the whole advnorm ladder.
#
...
class Args:
    ...
    update_epochs: int = 1           # RAW v1: single pass per rollout (no sample reuse)
    norm_adv: bool = False           # RAW-ADV v1: off — no per-minibatch standardize (raw GAE magnitude)
    clip_ratio: bool = False         # NOCLIP v2: OFF — no PPO surrogate ratio clip; raw -A*ratio
    clip_coef: float = 0.2           # PPO clip (lower bound 1-clip_coef; also upper if no high). Only used when clip_ratio=True (still drives the clipfrac diagnostic).
    ...
    target_kl: Optional[float] = None  # RAW v1: KL early-stop DISABLED (moot anyway with update_epochs=1)
    ...
    actor_grad_clip: float = 0.25    # max-norm for the policy gradient (KEPT — only the ratio clip is removed)
    critic_grad_clip: float = 0.25   # max-norm for the value gradient (KEPT)

    # Distributional critic support. RAW-REWARD v1: WIDENED to cover the unscaled
    # discounted return (raw HalfCheetah V ≈ ±(per-step_reward/(1-γ)), capped by the
    # episode). 511 bins over [-1500,1500] => bin_width≈5.9 (a per-step reward spans
    # ~1.4 bins). Re-tune v_min/v_max per env if debug/target_edge_mass climbs off ≈0.
    num_bins: int = 511
    v_min: float = -1500.0
    v_max: float = 1500.0
    critic_init_tau: float = 75.0  # init Z ≈ N(0, tau^2): same 0.025·range relative sharpness as the [-10,10] base
    ...
    adv_transform: str = "v10"       # RAW-ADV v1: identity — raw GAE is fed straight to the PG
    ...
    # ADVNORM v1: EMA-RMS advantage scaling. Applied once per rollout to the (shaped)
    # batch advantage, BEFORE the minibatch loop. "off" => disabled (recover base by
    # also restoring adv_transform="rankgauss", norm_adv=True). "ema_rms" divides by a
    # slow EMA of the batch advantage RMS and rescales to adv_scale_target; LINEAR, no
    # recentering, so relative magnitude differences and the raw GAE sign are preserved.
    adv_scale_norm: str = "off"          # "off" | "ema_rms"  (RAW-ADV v1 default = off: raw GAE magnitude)
    adv_scale_decay: float = 0.99        # EMA decay for the global advantage RMS
    adv_scale_target: float = 1.0        # target RMS the scaled advantage is pinned to
...
def make_env(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        ...
        # RAW REWARD v1: NormalizeReward + reward clip(-10,10) intentionally DROPPED.
        # The critic regresses to the unscaled discounted return; obs norm is kept.
        return env
...
if __name__ == "__main__":
    ...
    adv_scale_ema = None  # ADVNORM v1: slow global EMA of the advantage RMS (lazy-init to 1st batch)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ADVNORM v1: EMA-RMS scaling. Pin the (shaped) advantage's magnitude to a
        # fixed target via a slow GLOBAL EMA of its RMS — linear (ratios + sign of the
        # shaped adv preserved), no recentering, batch-scoped so the scale is identical
        # across every minibatch this rollout. This is the normalization step; with
        # adv_scale_norm != "off" we run with norm_adv=False so nothing double-normalizes.
        raw_gae_rms = gae.pow(2).mean().sqrt().item()  # diagnostic: watch reward-norm drift
        if args.adv_scale_norm == "ema_rms":
            batch_rms = b_policy_adv.pow(2).mean().sqrt().item()
            if adv_scale_ema is None:
                adv_scale_ema = batch_rms  # lazy-init: no lag on the first rollout
            else:
                adv_scale_ema = args.adv_scale_decay * adv_scale_ema + (1.0 - args.adv_scale_decay) * batch_rms
            b_policy_adv = b_policy_adv / (adv_scale_ema + 1e-8) * args.adv_scale_target
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.clip_ratio:
                    ...
                    # NOCLIP v2: no PPO surrogate clip — plain importance-weighted policy
                    # gradient -A*ratio. The trust region is now ONLY the grad-norm clip
                    # (actor_grad_clip) and update_epochs=1; nothing bounds the per-sample
                    # ratio, so off-policy drift within the pass is unconstrained.
        ...
        # How many of the configured update_epochs actually ran before the KL leash
        # tripped. With raw (un-normalized) advantages the per-epoch step is large, so
        # this can collapse toward 1 — watch it to know the effective sample reuse.
        epochs_ran = epoch + 1
        ...
        writer.add_scalar("losses/epochs_ran", epochs_ran, global_step)
        ...
        # ADVNORM v1 diagnostics: raw_gae_rms should DRIFT DOWN as reward-norm grows the
        # return scale; adv_scale_ema tracks it; applied_adv_rms should sit near target.
        writer.add_scalar("charts/raw_gae_rms", raw_gae_rms, global_step)
        if args.adv_scale_norm != "off":
            writer.add_scalar("charts/adv_scale_ema", float(adv_scale_ema), global_step)
            writer.add_scalar("charts/applied_adv_rms", b_policy_adv.pow(2).mean().sqrt().item(), global_step)
