# Sparser-JIT NN PPO v2: restore width while retaining fixed sparse connectivity.
# This capacity control keeps the v1 50%-density masks but uses width 64, matching
# the base hidden width. It isolates whether v1's learning loss came from sparse
# connectivity or from simultaneously shrinking the representation.
# Masked gradients keep inactive edges permanently zero; torch.compile fuses the
# learner graphs while the host mirror preserves native rollout throughput.
# Hypothesis: full width recovers representational capacity while sparsity
# regularizes noisy PPO gradients without changing the action distribution.
import os
import random
import time
...
class Args:
    ...
    hidden_width: int = 64
    """hidden width for the sparse actor and critic capacity control"""
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        ...
        self.critic = nn.Sequential(
            layer_init(SparseLinear(observation_dim, width, density)), nn.Tanh(),
            layer_init(SparseLinear(width, width, density)), nn.Tanh(),
...
def validate_args(args):
    if min(args.num_envs, args.num_steps, args.num_minibatches, args.update_epochs, args.hidden_width) <= 0:
        raise ValueError("environment, rollout, minibatch, epoch and hidden-width counts must be positive")
...
def main():
    ...
    try:
        ...
        writer.add_text(
            "policy",
            "Beta PPO; width-64 fixed 50%-density sparse hidden layers; "
            "masked gradients; FP32; native-action storage; host actor mirror; torch.compile",
        )
