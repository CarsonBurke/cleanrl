...
#
# SOFTGAUSS v1 — proper SAC soft Bellman backup on TD7 (isolated), squashed-Gaussian actor.
#
# An earlier full TD7->SAC swap lost (15280 vs 16454), but it changed EVERYTHING at once (actor,
# target, and lost SALE/LAP/checkpointing tuning). This is the isolated test: ONLY the value target
# and the actor loss go soft. SALE encoders, LAP, min-twin targets, target clipping, delayed policy
# updates, and checkpointing are all untouched from TD7. The actor becomes SAC's squashed Gaussian
# (mean + log_std head, a = tanh(mu + sigma*eps)); its entropy enters the BACKUP:
#   Q_target = r + gamma*not_done*( minQ_target(s', a').clamp(min,max) - alpha*logpi(a'|s') ),
# with a' sampled FRESH from the CURRENT actor (SAC semantics: no target actor and no 0.2 smoothing
# noise for a' selection — the entropy term provides the target smoothing). The clamp tracks observed
# Q extremes and applies to the minQ term only; the entropy term rides outside it. The actor loss is
# SAC's mean(alpha*logpi - Q) (same twin-mean reduction TD7's actor uses; logpi IS the entropy term,
# so no separate bonus). alpha is autotuned to the sigma=0.1-equivalent action-space entropy budget.
# Prior arms (sdnoise/betanoise) kept TD7's HARD backup and used entropy only in the actor loss;
# --no-soft-bellman here recovers that sibling regime (baseline hard target incl. target actor + 0.2
# smoothing, no entropy in the backup) with this file's squashed actor, isolating the backup change.
#
# Numerical note: logpi in the BACKUP can spike near |a|->1 (tanh Jacobian log(1-tanh^2) blows up),
# a new risk surface since entropy now enters the target; it is sanity-clamped to [-50, 50] there
# (actor-loss logpi is left unclamped, standard SAC). Hypothesis: the soft backup's smoother,
# uncertainty-aware value target improves TD7 where the full swap's confound hid it.
import copy
import os
import random
import time
...
class Actor(nn.Module):
    ...
    def sample(self, state, zs):
        # Reparameterized squashed-Gaussian sample with the tanh log-prob correction. Actions live in
        # [-1, 1] (action_scale = 1), so a = tanh(x_t) directly. Uses the numerically stable
        # log(1 - tanh^2 + 1e-6) form from cleanrl/sac_continuous_action.py.
...
class TD7Agent:
    def __init__(self, state_dim, action_dim, max_action, args: Args, device, writer: SummaryWriter):
        ...
        if args.alpha_autotune:
            self.target_entropy = action_dim * float(
                np.log(args.target_sigma) + 0.5 * np.log(2 * np.pi * np.e)
            )
    ...
    def train(self):
        ...
        if self.training_steps % self.args.policy_freq == 0:
            # Reparameterized squashed sample; logpi is the entropy term (no separate bonus).
            actor_action, actor_logpi = self.actor.sample(state, fixed_zs)
            ...
            # SAC actor loss with TD7's twin reduction (mean over both Q heads), per-sample so it
            # pairs with logpi; equals mean(alpha*logpi) - mean(Q) == -Q - alpha*entropy.
            ...
            if self.args.alpha_autotune:
                ...
                # -logpi is the (squashed) action-space entropy proxy; drive it to target_entropy.
                alpha_loss = (
                    -self.log_alpha.exp() * (self.target_entropy + logpi_detached)
                ).mean()
