# Pre-RMS SiTU-GLU PPO (normres_indclip_v4 baseline) with the shared GatePolar optimizer, v1.
# Mechanism: every hidden matrix of the SiTU-GLU residual trunk (in_proj plus each block's
# gate/up/down) takes the polar (Muon-style) direction of Nesterov momentum at fixed
# Frobenius norm; biases, per-channel block gates and the actor/critic heads keep Adam.
# Decoupled decay on those matrices is released per coordinate by the resistance gate
# Phi(z), so a coordinate only shrinks when its slow gradient mean does not resist.
# Novelty: the polar/gated-decay split is applied to a normed residual trunk rather than a
# Linear stack, so the group builder here traverses the trunk explicitly instead of ndim.
# Hypothesis: spectrally normalized trunk steps plus resisted decay keep the trunk plastic
# under PPO's drifting state distribution and beat 32x-LR Adam at 8M steps.
# `--optimizer adam` is the exact baseline control in this same file.
import os
import random
import time
...
from cleanrl.shared.gate_polar import GatePolar, parameter_count
...
class Args:
    ...
    optimizer: Literal["adam", "gate_polar", "gate_adam"] = "gate_polar"
    """adam (the exact baseline control), gate_polar (polar trunks, Adam heads, gated decay) or gate_adam (Adam everywhere, gated decay)"""
    optimizer_learning_rate: float = 3e-3
    """base LR for gate_polar/gate_adam; the polar step is norm-fixed, so it does not share Adam's LR scale"""
    weight_decay: float = 0.1
    """decoupled decay on weight matrices, released per coordinate by the gate; 0 is plain polar"""
    gate_beta: float = 0.9999
    """pole of the gate's gradient mean/power EMAs"""
    head_lr_scale: float = 1.0
    """output-head learning-rate multiplier for gate_polar"""
    polar_scale: float = 0.2
    """Frobenius norm of a polar direction per sqrt(rows * columns)"""
    num_envs: int = 16
...
def trunk_matrices(trunk):
    """The SiTU-GLU trunk's hidden weight matrices, in forward order.

    `mlp_groups` cannot be used here: the trunk is not a Linear stack. Selection is by
    module traversal (in_proj plus each block's gate/up/down), never by `ndim == 2`.
    """
    if not isinstance(trunk, NormResidualTrunk):
        raise TypeError("GatePolar grouping expects a NormResidualTrunk")
    if trunk.activation != "stiglu" or not all(isinstance(block, SiTUGLUBranch) for block in trunk.blocks):
        raise TypeError("GatePolar grouping expects SiTU-GLU branches; use --activation stiglu")
    matrices = [trunk.in_proj.weight]
    for block in trunk.blocks:
        matrices.extend((block.gate.weight, block.up.weight, block.down.weight))
    if any(matrix.ndim != 2 for matrix in matrices):
        raise ValueError("trunk hidden weights must be two-dimensional")
    return matrices


def norm_residual_groups(*stacks, head_lr_scale=1.0):
    """Groups for `nn.Sequential(NormResidualTrunk, head Linear)`: polar trunk, Adam elsewhere.

    Hidden trunk matrices take the polar direction and the gated decoupled decay; the
    in_proj bias, the per-channel block gates and any learned norm scalar take Adam at
    the base LR; the output head takes Adam at lr * head_lr_scale. Any unrouted trunk
    matrix is an error rather than a silent Adam parameter.
    """
    matrices, vectors, heads = [], [], []
    for stack in stacks:
        if not isinstance(stack, nn.Sequential) or len(stack) != 2 or not isinstance(stack[1], nn.Linear):
            raise TypeError("GatePolar grouping expects nn.Sequential(trunk, Linear head)")
        trunk = stack[0]
        stack_matrices = trunk_matrices(trunk)
        selected = {id(matrix) for matrix in stack_matrices}
        matrices.extend(stack_matrices)
        heads.extend(stack[1].parameters())
        for name, parameter in trunk.named_parameters():
            if id(parameter) in selected:
                continue
            if parameter.ndim > 1:
                raise ValueError(f"unrouted trunk matrix {name}; every hidden matrix must be polar")
            vectors.append(parameter)
    # Only the trunk matrices carry gated decay. The Adam groups pin weight_decay
    # to zero explicitly: GatePolar decays any parameter with ndim >= 2, which
    # would otherwise shrink the std=0.01 actor head and the critic head as well.
    groups = [{"params": matrices, "polar": True}]
    if vectors:
        groups.append({"params": vectors, "polar": False, "weight_decay": 0.0})
    groups.append({"params": heads, "polar": False, "lr_scale": head_lr_scale, "weight_decay": 0.0})
    return groups


def assert_group_coverage(groups, module):
    """The groups must partition the module's parameters exactly: no duplicate, no omission."""
    grouped = [id(parameter) for group in groups for parameter in group["params"]]
    if len(grouped) != len(set(grouped)):
        raise ValueError("duplicate parameters across optimizer groups")
    if set(grouped) != {id(parameter) for parameter in module.parameters()}:
        raise ValueError("optimizer groups must cover exactly the agent's parameters")
    return groups


def base_learning_rate(args):
    """Adam and the polar step have different LR scales, so each arm carries its own."""
    return args.learning_rate if args.optimizer == "adam" else args.optimizer_learning_rate


def make_optimizer(args, agent):
    if args.optimizer == "adam":
        return optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5, fused=True)
    if args.optimizer == "gate_polar":
        groups = norm_residual_groups(agent.actor, agent.critic, head_lr_scale=args.head_lr_scale)
    else:
        groups = [{"params": list(agent.parameters()), "polar": False}]
    assert_group_coverage(groups, agent)
    return GatePolar(
        groups,
        ...
        polar_scale=args.polar_scale,
        compile=args.compile,
    )


def describe_optimizer(args, optimizer):
    if args.optimizer == "adam":
        return f"Adam eps 1e-5 fused, lr {args.learning_rate} (baseline control)"
    sizes = [
        (len(group["params"]), "polar" if group["polar"] else "adam", group["lr_scale"])
        for group in optimizer.param_groups
    ]
    return (
        f"GatePolar lr {args.optimizer_learning_rate}; groups {sizes}; "
        f"{parameter_count(optimizer.param_groups)} parameters; "
        f"weight_decay {args.weight_decay}; gate_beta {args.gate_beta}; polar_scale {args.polar_scale}"
    )


def set_learning_rate(optimizer, value):
    """GatePolar rewrites its device LR scalars in place, so annealing never recompiles."""
    if isinstance(optimizer, GatePolar):
        optimizer.set_lr(value)
    else:
        optimizer.param_groups[0]["lr"] = value


def read_learning_rate(optimizer):
    """Read the LR back off the optimizer itself, host-side, once per iteration."""
    return optimizer.base_lr if isinstance(optimizer, GatePolar) else optimizer.param_groups[0]["lr"]
...
def validate_args(args):
    ...
    if args.optimizer not in {"adam", "gate_polar", "gate_adam"}:
        raise ValueError("optimizer must be adam, gate_polar or gate_adam")
    if base_learning_rate(args) <= 0:
        raise ValueError("the active optimizer's learning rate must be positive")
    if args.optimizer == "gate_polar" and args.activation != "stiglu":
        raise ValueError("gate_polar grouping is defined for the SiTU-GLU trunk only")
    return args
...
def main():
    ...
    try:
        ...
        optimizer = make_optimizer(args, agent)
        writer.add_text("optimizer", describe_optimizer(args, optimizer))
        ...
        for iteration in range(1, args.num_iterations + 1):
            learning_rate = base_learning_rate(args)
            if args.anneal_lr:
                learning_rate *= 1.0 - (iteration - 1.0) / args.num_iterations
                # GatePolar scales every group from one base LR; Adam keeps its single group.
            ...
            writer.add_scalar("charts/optimizer_learning_rate", read_learning_rate(optimizer), global_step)
