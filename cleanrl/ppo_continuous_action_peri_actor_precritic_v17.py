# Independent actor/critic normalization ablation, v17; not a paper claim.
# Hypothesis: actor branch-output RMS controls policy-update scale, whereas a
# scalar critic benefits from magnitude-sensitive residual updates. Relative to
# no-final Peri v8, only critic branch-output RMS becomes Identity: branch-input
# RMS, SiTU-GLU, gates and 1/8 readout scaling stay intact in both networks.
# Matched one-epoch PPO: LR .0096, normalized rewards, clipped scalar MSE,
# no gradient clipping, 50M transitions, seed 1; the Peri actor is unchanged.
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def __init__(self, envs):
        ...
        critic_trunk = self.critic[0]
        assert isinstance(critic_trunk, NormResidualTrunk)
        critic_trunk.block_output_norms = nn.ModuleList(
            nn.Identity() for _ in critic_trunk.blocks
        )
...
def main():
    ...
    try:
        ...
        writer.add_text(
            "policy",
            f"Beta Peri-RMS actor; branch-input-only RMS scalar critic; "
            f"SiTU-GLU residual trunks, no final norm; LR={args.learning_rate}; "
