"""PMPO-lite + LSTD: sign(advantage) PPO with state-dependent exploration.

Fork of lstd_linear_tanh with PMPO's sign-only advantage weighting.
Single change: replace mb_advantages with sign(mb_advantages) in policy loss.

Original: Fork of tbot_scalar with learned log_std_param matrix, adapted from trading_bot_0
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Policy loss (PMPO-lite: sign-only advantages)
                sign_adv = torch.sign(mb_advantages)
                pg_loss1 = -sign_adv * ratio
                pg_loss2 = -sign_adv * torch.clamp(ratio, 1 - args.clip_coef_low, 1 + args.clip_coef_high)
