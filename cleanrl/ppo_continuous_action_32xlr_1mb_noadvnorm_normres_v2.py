# No-sphere normalized residual PPO, v2. Same 32x LR / full-batch / raw GAE
# learner as the sphere controls. Three additive gated residual blocks replace
# normalized branch interpolation and dense sphere skips. Pre-norm preserves an
# identity gradient path; post-norm controls the stream after every update.
# LayerNorm vs RMSNorm isolates channel centering, not batch-mean removal.
# Fixed readout scaling preserves sphere head update scale, not just init logits.
# Hypothesis: controlling branch inputs without projecting branch outputs can
# retain useful magnitude information and avoid squared-activation blowups.
import os
import random
import time
