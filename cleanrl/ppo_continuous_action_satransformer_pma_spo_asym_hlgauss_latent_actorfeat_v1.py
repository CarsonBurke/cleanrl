# SPO-asym + transformer + HL-Gauss + latent_actorfeat: critic sees actor latent.
#
# Architecture diff vs hlgauss_latent_v1:
#   - critic_head input expanded from MODEL_DIM to 2*MODEL_DIM:
#       value_logits = critic_head(cat([critic_feat, actor_feat.detach()], -1))
#     The critic's linear head sees the actor's PMA-output latent in addition
#     to its own. .detach() prevents value-loss gradients from flowing back
#     into the actor (no policy-feedback corruption). The actor_feat slice of
#     the critic_head weight matrix is zero-initialized so this variant
#     starts identical to latent_v1 and can only learn to use actor_feat as
#     the value loss demands it.
#
#   Why pre-sample latent (not the sampled action z): actor_feat is a
#   deterministic function of s, so V(s, actor_feat(s)) is still V(s) with a
#   richer parameterization — no bootstrap marginalization issue (GAE works
#   exactly as before), no on-policy-coverage problem (latent space is fully
#   covered everywhere π visits). Collapses the "policy non-stationarity"
#   component of value-target variance (critic always knows what policy
#   it's evaluating across drift) but leaves the action-sampling component
#   untouched — strictly weaker than full Q-style critic, strictly safer.
#
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        # Critic head: input = cat(critic_feat, actor_feat.detach()).
        # Init: critic_feat slice = orthogonal(std=1.0) (matches latent_v1's head),
        #       actor_feat slice = zeros, bias = 0. So at t=0 this is identical
        #       to latent_v1; learning to use actor_feat is a strict addition.
        self.critic_head = nn.Linear(2 * MODEL_DIM, num_bins)
        torch.nn.init.zeros_(self.critic_head.weight)
        torch.nn.init.zeros_(self.critic_head.bias)
        torch.nn.init.orthogonal_(self.critic_head.weight[:, :MODEL_DIM], gain=1.0)
    ...
    def get_value_logits(self, x):
        actor_feat, _, critic_feat = self._encoder_with_autocast(x)
        return self.critic_head(torch.cat([critic_feat, actor_feat.detach()], dim=-1))
    ...
    def get_action_and_value(self, x, z=None):
        ...
        mean = self.actor_mean_head(actor_feat)
        std = self.sde_logstd_head(sde_feat).exp() + STD_MIN
        value_logits = self.critic_head(torch.cat([critic_feat, actor_feat.detach()], dim=-1))  # (B, num_bins)
