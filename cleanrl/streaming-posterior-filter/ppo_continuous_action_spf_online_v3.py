# Single-use online fast-memory actor-critic v3.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# Sixty-four independent environment streams contribute one fresh transition
# each. Every transition is consumed by exactly one optimizer update immediately
# after it arrives: no rollout replay, shuffled minibatches, repeated epochs,
# batch advantage normalization, or BPTT. The critic target and policy advantage
# are one-step TD estimates. The inherited PPO ratio is exactly one before this
# sole update, so this is honestly an online actor-critic control rather than a
# claim that clipping remains meaningful.
#
# A per-environment Student-t Delta memory still reads before action and writes
# only after the corresponding valid transition.
import os
import random
import time
...
def validate_online_contract(args):
    """Reject configurations that would invalidate the single-use experiment."""
    required = {
        "num_steps": 1,
        "num_minibatches": 1,
        "update_epochs": 1,
    ...
    if violations:
        raise ValueError(
            "online_v3 requires a single fresh transition per stream and one "
            f"optimizer use; invalid settings: {', '.join(violations)}"
        )
