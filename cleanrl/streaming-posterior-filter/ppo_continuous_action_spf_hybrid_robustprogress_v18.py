# Hybrid robust learning-progress PPO v18.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# v17's standardized surprise kept unit-scale exploration pressure after the
# fast model had converged, winning decisively through 4M but giving back the
# final lead. v18 uses the raw lagged-centered robust surprise instead. The
# signal is initially the same scale, then self-anneals as prediction surprise
# becomes stationary. It remains actor-only, replay-stored, Student-t bounded,
# V-only, and adds no live-state read inside PPO epochs.
...
class Args:
    ...
    # Self-annealing actor-only learning progress from the fast dynamics update.
    surprise_coef: float = 0.25
    surprise_baseline_rate: float = 1e-3
    surprise_bonus_clip: float = 1.0
...
class RobustFastDynamics:
    ...
    def update(self, obs, next_obs, valid):
        ...
        surprise_bonus = surprise_error.clamp(
    ...
    def diagnostics(self):
        ...
        return {
            ...
            "robust_surprise/centered": values[6].item(),
...
def validate_surprise_contract(args):
    ...
    if violations:
        raise ValueError(
            "hybrid robustprogress v18 invalid settings: " + ", ".join(violations)
        )
