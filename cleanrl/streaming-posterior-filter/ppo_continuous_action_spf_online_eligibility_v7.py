# Streaming V-critic actor-head eligibility trace v7.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# Each of 64 streams is observed once. A backward-view eligibility recurrence
# carries only the Beta actor-head score gradients, so future one-step V(s) TD
# errors credit past actions without replaying observations or retaining an
# autograd graph. The trunk and distributional V critic remain one-step. Episode
# boundaries clear only their stream's trace. No Q critic, rollout, repeated
