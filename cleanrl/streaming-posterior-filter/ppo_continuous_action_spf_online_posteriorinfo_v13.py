# Action-conditioned posterior information exploration v13.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# Each environment stream carries an independent robust Bayesian linear
# dynamics posterior over normalized (state, action) features. Before an action
# is executed, its posterior covariance yields the expected information gain
# from observing the successor. A lagged per-stream baseline turns this into a
# bounded actor-only exploration advantage; the raw one-step TD target and the
# distributional V critic remain unchanged. The posterior is updated only after
# the physical successor is available, with a Student-t-weighted Joseph update.
# There is no Q function, replay, batch statistic, repeated optimizer use, or
# cross-step autograd graph.
import os
import random
import time
...
class Args:
    ...
    # Action-conditioned robust dynamics posterior and actor-only information bonus.
    posterior_info_coef: float = 0.5
    posterior_prior_variance: float = 1.0
    posterior_reset_rate: float = 1e-4
    posterior_student_df: float = 5.0
    posterior_scale_rate: float = 1e-3
    posterior_baseline_rate: float = 1e-3
    posterior_bonus_clip: float = 3.0
...
def validate_online_contract(args):
    """Reject configurations that invalidate posterior-information isolation."""
    required = {
        "num_steps": 1,
        "num_minibatches": 1,
        "update_epochs": 1,
    ...
    positive = (
        "posterior_info_coef",
        "posterior_prior_variance",
        "posterior_reset_rate",
        "posterior_student_df",
        "posterior_scale_rate",
        "posterior_baseline_rate",
        "posterior_bonus_clip",
    )
    violations.extend(
        f"{name}={getattr(args, name)!r} (required > 0)"
        for name in positive
        if getattr(args, name) <= 0
    )
    if args.posterior_scale_rate > 1:
        violations.append("posterior_scale_rate must be <= 1")
    if args.posterior_baseline_rate > 1:
        violations.append("posterior_baseline_rate must be <= 1")
    if args.posterior_reset_rate > 1:
        violations.append("posterior_reset_rate must be <= 1")
    if violations:
        raise ValueError(
            "online posteriorinfo v13 requires one fresh transition per stream "
            f"and one optimizer use; invalid settings: {', '.join(violations)}"
        )
...
class RobustPosteriorDynamics:
    """Independent robust linear dynamics posteriors with causal information gain."""

    def __init__(self, num_envs, obs_dim, act_dim, args, device):
        self.obs_dim = obs_dim
        self.act_dim = act_dim
        self.feature_dim = obs_dim + act_dim + 1
        self.reset_rate = args.posterior_reset_rate
        self.student_df = args.posterior_student_df
        self.scale_rate = args.posterior_scale_rate
        self.baseline_rate = args.posterior_baseline_rate
        self.bonus_clip = args.posterior_bonus_clip
        self.weight = torch.zeros(
            (num_envs, obs_dim, self.feature_dim), device=device
        )
        identity = torch.eye(self.feature_dim, device=device)
        self.identity = identity.unsqueeze(0).expand(num_envs, -1, -1)
        self.prior_covariance = (
            self.identity.clone().mul_(args.posterior_prior_variance)
        )
        self.covariance = self.prior_covariance.clone()
        self.scale = torch.ones(num_envs, device=device)
        self.info_mean = torch.zeros(num_envs, device=device)
        self.info_var = torch.ones(num_envs, device=device)
        self.diagnostic_sum = torch.zeros(7, device=device)
        self.diagnostic_steps = 0

    def _key(self, obs, action):
        state = F.normalize(obs, dim=-1)
        scaled_action = action / self.act_dim**0.5
        bias = torch.ones_like(action[:, :1])
        return F.normalize(torch.cat((state, scaled_action, bias), dim=-1), dim=-1)

    def _info_std(self):
        # A 5%-of-location relative floor prevents an almost deterministic
        # information signal from turning floating-point residue into a clipped
        # full-strength bonus.
        relative_floor = 0.05 * self.info_mean.abs()
        return (self.info_var + relative_floor.square()).sqrt().clamp_min(1e-3)

    @torch.no_grad()
    def information_gain(self, obs, action):
        key = self._key(obs, action)
        projected = torch.bmm(self.covariance, key.unsqueeze(2)).squeeze(2)
        leverage = (key * projected).sum(1).clamp_min(0.0)
        raw_info = 0.5 * torch.log1p(leverage / self.scale)
        bonus = (raw_info - self.info_mean) / self._info_std()
        return bonus.clamp(-self.bonus_clip, self.bonus_clip), raw_info

    @torch.no_grad()
    def update(self, obs, action, next_obs, valid, raw_info):
        key = self._key(obs, action)
        prediction = torch.einsum("boi,bi->bo", self.weight, key)
        innovation = next_obs - obs - prediction
        projected = torch.bmm(self.covariance, key.unsqueeze(2)).squeeze(2)
        leverage = (key * projected).sum(1).clamp_min(0.0)
        predictive_var = (self.scale + leverage).clamp_min(
            torch.finfo(self.scale.dtype).tiny
        )
        mahalanobis = innovation.square().sum(1) / predictive_var
        student_weight = (
            (self.student_df + self.obs_dim)
            / (self.student_df + mahalanobis)
        ).clamp_max(1.0)
        effective_noise = self.scale / student_weight.clamp_min(1e-6)
        denominator = (effective_noise + leverage).clamp_min(
            torch.finfo(self.scale.dtype).tiny
        )
        gain = projected / denominator.unsqueeze(1)

        valid_bool = valid.to(dtype=torch.bool)
        weight_update = innovation.unsqueeze(2) * gain.unsqueeze(1)
        self.weight.add_(weight_update * valid_bool.view(-1, 1, 1))

        contraction = self.identity - gain.unsqueeze(2) * key.unsqueeze(1)
        joseph = torch.bmm(
            torch.bmm(contraction, self.covariance),
            contraction.transpose(1, 2),
        )
        joseph.add_(
            effective_noise.view(-1, 1, 1)
            * gain.unsqueeze(2)
            * gain.unsqueeze(1)
        )
        next_covariance = torch.lerp(
            joseph, self.prior_covariance, self.reset_rate
        )
        invalid_covariance = torch.lerp(
            self.covariance, self.prior_covariance, self.reset_rate
        )
        self.covariance.copy_(
            torch.where(valid_bool.view(-1, 1, 1), next_covariance, invalid_covariance)
        )
        self.covariance.copy_(
        ...
        info_error = raw_info - self.info_mean
        normalized_info = info_error / self._info_std()
        next_info_mean = self.info_mean + self.baseline_rate * info_error
        next_info_var = torch.lerp(
            self.info_var,
            info_error * (raw_info - next_info_mean),
            self.baseline_rate,
        ).clamp_min(1e-6)
        self.info_mean.copy_(
            torch.where(valid_bool, next_info_mean, self.info_mean)
        )
        self.info_var.copy_(
            torch.where(valid_bool, next_info_var, self.info_var)
        )
        ...
        self.diagnostic_sum.add_(
            torch.stack(
                (
                    (raw_info * valid_float).sum() / count,
                    (normalized_info * valid_float).sum() / count,
                    (student_weight * valid_float).sum() / count,
                    (self.scale * valid_float).sum() / count,
                    (leverage * valid_float).sum() / count,
                    (
                        self.covariance.diagonal(dim1=1, dim2=2).mean(1)
                        * valid_float
                    ).sum()
                    / count,
                    (weight_update.square().mean((1, 2)).sqrt() * valid_float).sum()
                    / count,
                )
            )
        )
    ...
    def diagnostics(self):
        ...
        self.diagnostic_steps = 0
        return {
            "posterior_info/raw": values[0].item(),
            "posterior_info/normalized": values[1].item(),
            "posterior_info/student_weight": values[2].item(),
            "posterior_info/scale": values[3].item(),
            "posterior_info/leverage": values[4].item(),
            "posterior_info/covariance_mean": values[5].item(),
            "posterior_info/update_rms": values[6].item(),
            "posterior_info/weight_rms": self.weight.square().mean().sqrt().item(),
        }
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, fast_context, z=None):
        # z is the distribution-NATIVE sample (pre-tanh for gaussian; in (0,1) for
        # beta). Stored z and fast_context reproduce the behavior-policy sample.
...
if __name__ == "__main__":
    ...
    posterior_dynamics = RobustPosteriorDynamics(
        args.num_envs, obs_dim, act_dim, args, device
    )
    ...
    intrinsic_bonuses = torch.zeros_like(rewards)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            dist_p1[step] = p1
            dist_p2[step] = p2
            info_bonus, raw_info = posterior_dynamics.information_gain(obs[step], action)
            intrinsic_bonuses[step] = info_bonus
            ...
            intrinsic_bonuses[step].mul_(transition_valid_tensor)
            posterior_dynamics.update(
                obs[step],
                action,
                transition_next_obs_tensor,
                transition_valid_tensor,
                raw_info,
            )
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                _, _, next_logprob, _, _, _ = agent.get_action_and_value(
                    next_obses.reshape((-1,) + envs.single_observation_space.shape),
                    next_contexts.reshape((-1,) + envs.single_observation_space.shape),
                )
            ...
            # The information term is actor-only. It scores the sampled action
            # under the pre-transition posterior, while the raw V target above
            # remains the environment-reward TD target.
            policy_adv = (
                policy_adv + args.posterior_info_coef * intrinsic_bonuses
            )
        ...
        for k, v in posterior_dynamics.diagnostics().items():
            writer.add_scalar(k, v, global_step)
        writer.add_scalar(
            "posterior_info/actor_bonus_mean",
            (args.posterior_info_coef * intrinsic_bonuses).mean().item(),
            global_step,
        )
        writer.add_scalar(
