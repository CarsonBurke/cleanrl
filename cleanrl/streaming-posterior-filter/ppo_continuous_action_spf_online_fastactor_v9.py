# Streaming local fast-actor plasticity v9.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# Each environment owns two small linear fast-weight maps from normalized state
# to Beta concentration-logit corrections. After a one-step V(s) TD error, an
# exact local Beta score outer-product updates those maps; the correction was
# read before that transition and is stored exactly for the slow actor update.
# This adds state-correlated within-run policy adaptation without a Q critic,
# replay, rollout, repeated epoch, batch advantage normalization, or BPTT.
import os
import random
import time
...
def validate_online_contract(args):
    ...
    if violations:
        raise ValueError(
            "online fastactor v9 requires one fresh transition per stream and "
...
class LocalFastActor:
    """Per-stream local Beta-logit maps updated by one-step V-TD scores."""
    ...
    def read(self, obs):
        key = F.normalize(obs, dim=-1)
        alpha_offset = torch.einsum("bao,bo->ba", self.alpha_weight, key)
        beta_offset = torch.einsum("bao,bo->ba", self.beta_weight, key)
        return (
            alpha_offset.clamp(-self.offset_clip, self.offset_clip),
            beta_offset.clamp(-self.offset_clip, self.offset_clip),
        )

    @torch.no_grad()
    def update(self, obs, z, alpha, beta, td_error):
        key = F.normalize(obs, dim=-1)
        z = z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        alpha_excess = (alpha - 1.0).clamp_min(SAMPLE_EPS)
        beta_excess = (beta - 1.0).clamp_min(SAMPLE_EPS)
        alpha_jacobian = -torch.expm1(-alpha_excess)
        beta_jacobian = -torch.expm1(-beta_excess)
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, fast_context, z=None, fast_offsets=None):
        ...
        dist, to_action, log_det_fn = self._actor_dist(actor_feat, fast_offsets)
        if z is None:
            z = dist.sample()
