# Hybrid posterior-information PPO v16.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# Builds on the strongest v2 carrier rather than forcing exploration through
# the unstable single-use actor. Each environment maintains an independent
# robust diagonal Bayesian linear dynamics posterior over normalized
# (state, action) features. Its pre-transition expected information gain becomes
# a stored actor-only intrinsic advantage and is propagated through the rollout;
# the raw distributional V target remains environment reward. PPO replays the
# stored scalar bonus and behavior-time fast context, never live posterior state.
# There is no Q function or posterior BPTT.
...
class Args:
    ...
    # Robust action-conditioned posterior information bonus.
    posterior_info_coef: float = 0.25
...
class RobustDiagonalPosteriorDynamics:
    """Per-stream robust diagonal linear dynamics posterior."""

    def __init__(self, num_envs, obs_dim, act_dim, args, device):
        self.obs_dim = obs_dim
        self.act_dim = act_dim
        self.feature_dim = obs_dim + act_dim + 1
        self.prior_variance = args.posterior_prior_variance
        ...
        self.variance = torch.full(
            (num_envs, self.feature_dim), self.prior_variance, device=device
        )
        self.scale = torch.ones(num_envs, device=device)
        self.info_mean = torch.zeros(num_envs, device=device)
        self.info_var = torch.ones(num_envs, device=device)
        self.diagnostic_sum = torch.zeros(6, device=device)
        self.diagnostic_steps = 0
    ...
    def information_gain(self, obs, action):
        ...
        leverage = (self.variance * key.square()).sum(1).clamp_min(0.0)
    ...
    def update(self, obs, action, next_obs, valid, raw_info):
        ...
        gain = self.variance * key / (effective_noise + leverage).unsqueeze(1)
        valid_bool = valid.to(dtype=torch.bool)
        weight_update = innovation.unsqueeze(2) * gain.unsqueeze(1)
        self.weight.add_(weight_update * valid_bool.view(-1, 1, 1))

        posterior_variance = (
            self.variance * (1.0 - gain * key)
        ).clamp_min(torch.finfo(self.variance.dtype).tiny)
        posterior_variance = torch.lerp(
            posterior_variance,
            torch.full_like(posterior_variance, self.prior_variance),
            self.reset_rate,
        )
        self.variance.copy_(
            torch.where(valid_bool.unsqueeze(1), posterior_variance, self.variance)
        )
        ...
        self.info_mean.copy_(torch.where(valid_bool, next_info_mean, self.info_mean))
        self.info_var.copy_(torch.where(valid_bool, next_info_var, self.info_var))
        ...
        self.diagnostic_sum.add_(
            torch.stack(
                (
                    ...
                    (self.variance.mean(1) * valid_float).sum() / count,
                )
            )
        )
        self.diagnostic_steps += 1
    ...
    def diagnostics(self):
        ...
        return {
            ...
            "posterior_info/variance_mean": values[5].item(),
...
def propagate_information_advantage(bonuses, boundaries, gamma, gae_lambda):
    """Discount actor-only information rewards without crossing resets."""
    advantages = torch.zeros_like(bonuses)
    last_advantage = 0
    for t in reversed(range(bonuses.shape[0])):
...
def validate_posterior_contract(args):
    required = {
        "ent_alpha": 0.0,
        "ent_coef": 0.0,
        "auto_entropy": False,
    ...
    if violations:
        raise ValueError(
            "hybrid posteriorinfo v16 invalid settings: " + ", ".join(violations)
        )
...
if __name__ == "__main__":
    ...
    posterior_dynamics = RobustDiagonalPosteriorDynamics(
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Propagate the stored, causally centered information rewards through
            # this rollout for the actor only. The distributional V targets above
            # remain raw environment-return targets.
            info_advantages = propagate_information_advantage(
                intrinsic_bonuses,
                transition_boundaries,
                args.gamma,
                args.gae_lambda,
            )
            policy_adv = policy_adv + args.posterior_info_coef * info_advantages
        ...
        writer.add_scalar(
            ...
            (args.posterior_info_coef * info_advantages).mean().item(),
            global_step,
        )
        writer.add_scalar(
