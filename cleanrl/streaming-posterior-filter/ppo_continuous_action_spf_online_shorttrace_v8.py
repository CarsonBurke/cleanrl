# Streaming short-horizon V-critic actor-head eligibility trace v8.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# V7's lambda=0.9 trace improved 1M reward but accumulated stale score directions
# and collapsed after 3M. V8 reduces the trace horizon to about two transitions
# while retaining exact backward-view credit on the Beta heads. Each stream is
# observed once; only numerical score traces persist. The trunk and V(s) critic
# remain one-step. No Q critic, replay, rollout, repeated epoch, batch advantage
# normalization, or BPTT.
import os
import random
import time
...
class Args:
    ...
    # Backward-view trace on the small Beta policy heads only.
    actor_trace_lambda: float = 0.5
...
def validate_online_contract(args):
    """Reject configurations that invalidate the streaming V-critic trace."""
    required = {
        "num_steps": 1,
        "num_minibatches": 1,
        "update_epochs": 1,
    ...
    if not 0 <= args.actor_trace_lambda <= 1:
        violations.append("actor_trace_lambda must be in [0, 1]")
    if violations:
        raise ValueError(
            "online eligibility v7 requires one fresh transition per stream "
...
class BetaHeadEligibility:
    """Per-stream backward-view score traces for the two linear Beta heads."""

    def __init__(self, num_envs, act_dim, hidden, args, device):
        self.decay = args.gamma * args.actor_trace_lambda
        weight_shape = (num_envs, act_dim, hidden)
        bias_shape = (num_envs, act_dim)
        self.alpha_weight = torch.zeros(weight_shape, device=device)
        self.alpha_bias = torch.zeros(bias_shape, device=device)
        self.beta_weight = torch.zeros(weight_shape, device=device)
        self.beta_bias = torch.zeros(bias_shape, device=device)
        self.diagnostic_sum = torch.zeros(3, device=device)
        self.diagnostic_steps = 0
    ...
    def gradients(
        ...
        alpha_score = torch.sigmoid(raw_alpha) * (
            z.log() - torch.digamma(alpha) + common
        )
        beta_score = torch.sigmoid(raw_beta) * (
            torch.log1p(-z) - torch.digamma(beta) + common
        )
        current_alpha_weight = torch.einsum(
            "ba,bh->bah",
            alpha_score,
            actor_feat,
        )
        current_beta_weight = torch.einsum(
            "ba,bh->bah",
            beta_score,
            actor_feat,
        )

        self.alpha_weight.mul_(self.decay).add_(current_alpha_weight)
        self.alpha_bias.mul_(self.decay).add_(alpha_score)
        self.beta_weight.mul_(self.decay).add_(current_beta_weight)
        self.beta_bias.mul_(self.decay).add_(beta_score)

        weighted = advantage.view(-1, 1, 1)
        bias_weighted = advantage.view(-1, 1)
        gradients = (
            -(weighted * self.alpha_weight).mean(0),
            -(bias_weighted * self.alpha_bias).mean(0),
            -(weighted * self.beta_weight).mean(0),
            -(bias_weighted * self.beta_bias).mean(0),
        )

        current_rms = (
            0.5
            * (
                current_alpha_weight.square().mean()
                + current_beta_weight.square().mean()
            )
        ).sqrt()

        keep = (1.0 - boundary).view(-1, 1, 1)
        bias_keep = (1.0 - boundary).view(-1, 1)
        self.alpha_weight.mul_(keep)
        self.beta_weight.mul_(keep)
        self.alpha_bias.mul_(bias_keep)
        self.beta_bias.mul_(bias_keep)
        ...
        self.diagnostic_sum.add_(
            torch.stack(
                (
                    current_rms,
                    trace_rms,
                    boundary.to(dtype=actor_feat.dtype).mean(),
                )
            )
        )
        self.diagnostic_steps += 1
    ...
    def diagnostics(self):
        ...
        self.diagnostic_steps = 0
        return {
            "eligibility/current_score_rms": values[0].item(),
            "eligibility/trace_rms": values[1].item(),
            "eligibility/boundary_fraction": values[2].item(),
        }
...
class Agent(nn.Module):
    ...
    @torch.no_grad()
    def beta_trace_inputs(self, x, fast_context):
        actor_feat, _ = self._trunks(x, fast_context)
        return (
            actor_feat,
            self.actor_alpha_head(actor_feat),
            self.actor_beta_head(actor_feat),
        )
...
if __name__ == "__main__":
    ...
    act_dim = int(np.prod(envs.single_action_space.shape))
    actor_eligibility = BetaHeadEligibility(
        args.num_envs,
        act_dim,
        args.hidden,
        args,
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        b_boundaries = transition_boundaries.reshape(-1)
        ...
        for epoch in range(args.update_epochs):
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                trace_actor_feat, trace_raw_alpha, trace_raw_beta = (
                    agent.beta_trace_inputs(
                        b_obs[mb_inds],
                        b_contexts[mb_inds],
                    )
                )
                trace_gradients = actor_eligibility.gradients(
                    trace_actor_feat,
                    trace_raw_alpha,
                    trace_raw_beta,
                    b_latent_zs[mb_inds],
                    mb_advantages.detach(),
                    b_boundaries[mb_inds],
                )
                ...
                if args.separate_grad_clip:
                    ...
                    agent.actor_alpha_head.weight.grad = trace_gradients[0]
                    agent.actor_alpha_head.bias.grad = trace_gradients[1]
                    agent.actor_beta_head.weight.grad = trace_gradients[2]
                    agent.actor_beta_head.bias.grad = trace_gradients[3]
        ...
        for k, v in actor_eligibility.diagnostics().items():
            writer.add_scalar(k, v, global_step)
        writer.add_scalar(
