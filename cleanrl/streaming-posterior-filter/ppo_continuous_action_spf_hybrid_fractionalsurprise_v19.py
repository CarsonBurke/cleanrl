# Hybrid fractional robust-progress PPO v19.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# v17's full variance normalization never annealed its exploration pressure,
# while v18's unnormalized progress lost most of v17's 2-4M acceleration. v19
# divides lagged-centered robust surprise by sqrt(surprise_std), halfway between
# those endpoints. While variance dominates, amplitude decays with its fourth
# root; once the mean anchor dominates, the residual itself decays to zero.
# This preserves slower self-annealing without a timestep schedule. The bonus
# remains causal, replay-stored, actor-only, Student-t bounded, and Q-free.
...
class RobustFastDynamics:
    ...
    def update(self, obs, next_obs, valid):
        ...
        surprise_error = robust_surprise - self.surprise_mean
        surprise_std = (
            self.surprise_var
            + (0.05 * self.surprise_mean.abs()).square()
        ).sqrt().clamp_min(1e-3)
        # Fractional normalization: unit-scale initially, then amplitude decays
        # as sqrt(surprise_std) when the residual process becomes predictable.
        surprise_bonus = (surprise_error / surprise_std.sqrt()).clamp(
            -self.surprise_clip, self.surprise_clip
        )
        update = (
        ...
        next_surprise_mean = (
            self.surprise_mean + self.surprise_rate * surprise_error
        )
        next_surprise_var = torch.lerp(
            self.surprise_var,
            surprise_error * (robust_surprise - next_surprise_mean),
            self.surprise_rate,
        ).clamp_min(1e-6)
        self.surprise_mean.copy_(
            torch.where(valid_bool, next_surprise_mean, self.surprise_mean)
        )
        self.surprise_var.copy_(
            torch.where(valid_bool, next_surprise_var, self.surprise_var)
        )
        ...
        return surprise_bonus * valid_float
...
def validate_surprise_contract(args):
    ...
    if violations:
        raise ValueError(
            "hybrid robustsurprise v17 invalid settings: " + ", ".join(violations)
        )
