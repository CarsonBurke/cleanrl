# Checkpointed robust fast-dynamics reference v3.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# Training math is exactly v2. This reproducibility carrier adds only final
# eval-compatible model and fast-state/optimizer lineage artifacts.
# A per-environment Delta-rule memory learns the policy-induced one-step flow
# online from ordered transitions. Before acting, the policy reads a predicted
# successor observation and concatenates it with the current observation.
# After the transition, a Student-t gate robustly writes the clean temporal
# innovation. PPO stores the exact read context with each behavior sample, so
# minibatch replay cannot leak later memory state or change the likelihood ratio.
#
# Slow weights use Adam and the exact centered-run PPO losses/rollout contract.
# The fast state is inference-time recurrent memory, not an optimizer surrogate:
# it receives one causal update per valid environment transition, never BPTT.
