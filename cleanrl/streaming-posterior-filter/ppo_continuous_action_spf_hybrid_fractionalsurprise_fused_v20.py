# Hybrid fused fractional robust-progress PPO v20.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# v19 wins at every HalfCheetah checkpoint but gives up 28% throughput to its
# behavior-time surprise update. v20 preserves the exact Q-free v19 objective
# and state recurrence while compiling pure functional fast-memory read/update
# kernels. This targets launch overhead without changing causal ordering,
# stored rollout contexts, actor-only surprise credit, or raw-reward V targets.
# Runs emit an eval-compatible model plus fast-state/optimizer lineage sidecar.
...
class Args:
    ...
    # Actor-only robust surprise from the existing fast dynamics update.
    surprise_coef: float = 0.25
    surprise_baseline_rate: float = 1e-3
    surprise_bonus_clip: float = 3.0
    # Compile only the pure recurrent fast-memory kernels. PPO/distributions
    # remain eager, avoiding graph breaks and output-buffer aliasing.
    compile_fast_memory: bool = True
    fast_memory_compile_mode: str = "default"
...
def robust_fast_read(memory, obs, context_clip):
    key = F.normalize(obs, dim=-1)
    prediction = torch.einsum("bij,bj->bi", memory, key)
    return prediction.clamp(-context_clip, context_clip)


def robust_fast_update(
    memory,
    scale,
    surprise_mean,
    surprise_var,
    ...
    normalized_error = innovation.square().mean(1) / scale
    student_weight = (
        (student_df + 1.0) / (student_df + normalized_error)
    ).clamp_max(1.0)
    robust_surprise = student_weight * torch.log1p(normalized_error)
    surprise_error = robust_surprise - surprise_mean
    surprise_std = (
        surprise_var + (0.05 * surprise_mean.abs()).square()
    ).sqrt().clamp_min(1e-3)
    surprise_bonus = (surprise_error / surprise_std.sqrt()).clamp(
        -surprise_clip, surprise_clip
    )
    update = (
        (eta * student_weight).view(-1, 1, 1)
        * innovation.unsqueeze(2)
        * key.unsqueeze(1)
    )
    valid_bool = valid.to(dtype=torch.bool)
    valid_float = valid_bool.to(dtype=prediction.dtype)
    next_memory = memory + update * valid_bool.view(-1, 1, 1)
    scale_target = student_weight * innovation.square().mean(1)
    scale_candidate = torch.lerp(scale, scale_target, scale_rate)
    next_scale = torch.where(valid_bool, scale_candidate, scale).clamp_min(
        torch.finfo(scale.dtype).tiny
    )
    mean_candidate = surprise_mean + surprise_rate * surprise_error
    var_candidate = torch.lerp(
        surprise_var,
        surprise_error * (robust_surprise - mean_candidate),
        surprise_rate,
    ).clamp_min(1e-6)
    next_surprise_mean = torch.where(valid_bool, mean_candidate, surprise_mean)
    next_surprise_var = torch.where(valid_bool, var_candidate, surprise_var)
    ...
    diagnostics = torch.stack(
        (
            ...
            (next_scale * valid_float).sum() / count,
            (update.square().mean((1, 2)).sqrt() * valid_float).sum() / count,
            (prediction.square().mean(1).sqrt() * valid_float).sum() / count,
            (robust_surprise * valid_float).sum() / count,
            (surprise_bonus * valid_float).sum() / count,
        )
    )
    return (
        next_memory,
...
class RobustFastDynamics:
    def __init__(self, num_envs, obs_dim, args, device):
        ...
        self.surprise_rate = args.surprise_baseline_rate
        self.surprise_clip = args.surprise_bonus_clip
        self.memory = torch.zeros((num_envs, obs_dim, obs_dim), device=device)
        self.scale = torch.ones(num_envs, device=device)
        self.surprise_mean = torch.zeros(num_envs, device=device)
        self.surprise_var = torch.ones(num_envs, device=device)
        self.diagnostic_sum = torch.zeros(7, device=device)
        self.diagnostic_steps = 0
        self.compiled = args.compile_fast_memory
        self._read_impl = robust_fast_read
        self._update_impl = robust_fast_update
        if self.compiled:
            self._read_impl = torch.compile(
                robust_fast_read,
                mode=args.fast_memory_compile_mode,
                dynamic=False,
                fullgraph=True,
            )
            self._update_impl = torch.compile(
                robust_fast_update,
                mode=args.fast_memory_compile_mode,
                dynamic=False,
                fullgraph=True,
    ...
    @torch.no_grad()
    def read(self, obs):
        return self._read_impl(self.memory, obs, self.context_clip)
    ...
    def update(self, obs, next_obs, valid):
        ...
        self.diagnostic_sum.add_(diagnostics)
    ...
    def diagnostics(self):
        ...
        return {
            ...
            "robust_surprise/raw": values[5].item(),
            "robust_surprise/fractional": values[6].item(),
            "fast_memory/memory_rms": self.memory.square().mean().sqrt().item(),
        }

    def release_compiled_graphs(self):
        self._read_impl = robust_fast_read
        self._update_impl = robust_fast_update
        self.compiled = False
...
def propagate_surprise_advantage(bonuses, boundaries, gamma, gae_lambda):
...
def validate_surprise_contract(args):
    required = {
        "ent_alpha": 0.0,
        "ent_coef": 0.0,
        "auto_entropy": False,
    ...
    if args.surprise_coef <= 0:
        violations.append("surprise_coef must be > 0")
    if not 0 < args.surprise_baseline_rate < 1:
        violations.append("surprise_baseline_rate must be in (0, 1)")
    if args.surprise_bonus_clip <= 0:
        violations.append("surprise_bonus_clip must be > 0")
    if args.compile_fast_memory and args.fast_memory_compile_mode != "default":
        violations.append("fast_memory_compile_mode must be 'default'")
    if violations:
        raise ValueError(
            "hybrid fractional-surprise fused v20 invalid settings: "
            + ", ".join(violations)
        )
...
if __name__ == "__main__":
    ...
    surprise_bonuses = torch.zeros_like(rewards)

    global_step = 0
    # Intentionally include first-call compilation in SPS and wall-clock cost.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            surprise_bonuses[step] = fast_memory.update(
                obs[step], transition_next_obs_tensor, transition_valid_tensor
            )
        ...
        with torch.no_grad():
            ...
            surprise_advantages = propagate_surprise_advantage(
                surprise_bonuses,
                transition_boundaries,
                args.gamma,
                args.gae_lambda,
            )
            policy_adv = policy_adv + args.surprise_coef * surprise_advantages
        ...
        writer.add_scalar(
            "robust_surprise/actor_bonus_mean",
            (args.surprise_coef * surprise_advantages).mean().item(),
            global_step,
        )
        writer.add_scalar(
    ...
    if args.save_model:
        ...
        model_path = run_dir / f"{args.exp_name}.cleanrl_model"
        fast_state_path = run_dir / f"{args.exp_name}.faststate.pt"
        ...
        torch.save(
            {
                ...
                "fast_memory": {
                    "memory": fast_memory.memory.detach(),
                    "scale": fast_memory.scale.detach(),
                    "surprise_mean": fast_memory.surprise_mean.detach(),
                    "surprise_var": fast_memory.surprise_var.detach(),
                },
        ...
        print(f"saved_fast_state={fast_state_path}")

    if fast_memory.compiled:
        fast_memory.release_compiled_graphs()
