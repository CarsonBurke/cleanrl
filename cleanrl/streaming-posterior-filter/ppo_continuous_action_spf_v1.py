# Streaming Posterior Filter (SPF) v1.
# Family contract and derivation: cleanrl/streaming-posterior-filter/FAMILY.md
#
# THESIS: a streaming optimizer should estimate a latent descent direction and
# its uncertainty instead of learning competitive gradient multipliers. Each
# output row gets an independent robust filter. Adam's first moment is replaced
# by an empirical-Bayes Student-t measurement update; Adam's second moment stays
# as a controlled coordinate preconditioner.
#
# For a rowwise latent gradient M:
#   Q  = ((1-beta1)^2 / beta1) * R
#   P- = P + Q
#   w  = min(1, (nu + d) / (nu + ||g-M||^2 / (P-+R)))
#   K  = w*P- / (w*P- + R)
#   M  = M + K*(g-M)
#   P  = (1-K)*P-
#   theta <- theta - temperature * M / sqrt(V_adam)
# Fixed Q/R gives a scale-invariant forgetting prior. R is estimated online
# from the robust post-update residual. Isolated extremes lower K; rows never
# compete. This is an approximate posterior over a latent gradient state, not
# a falsely claimed EKF posterior over network weights.
#
# v1 deliberately changes only the optimizer on the exact centered-run PPO
# program. It removes the activation controller entirely and leaves rollout,
# losses, architecture, dual gradient clipping, and seed unchanged.
#
# References:
#   exact centered run: 1435 @500k, 3482 @1M, 5310 @1.6M (ended there)
#   plain PPO reference: 1576 @500k, 3079 @1M, 4507 @1.6M, 5012 @2M,
#                        6716 @4M, 8278 @8M-window; final-20 8455
# Required telemetry: spf/gain_*, spf/student_weight_mean,
# spf/outlier_fraction, spf/p_over_r_mean, spf/innovation_per_dim_mean,
# spf/prior_cosine_mean, spf/update_rms_mean and charts/SPS.
# Kill: non-finite state; gain collapse; or >10% below plain PPO at two
# consecutive matched checkpoints after 1M. Throughput target: >=85% of the
# exact centered run's final cumulative 2221 SPS after startup.
...
class Args:
    ...
    learning_rate: float = 3e-4      # global temperature after filtered Adam-RMS direction.
    num_envs: int = 16
    num_steps: int = 2048
    anneal_lr: bool = False          # streaming filter tracks forever; no terminal collapse.
    gamma: float = 0.99
    ...
    # Streaming Posterior Filter: Adam-sized M/V plus O(output rows) P/R.
    # beta1 defines the scale-invariant Q/R forgetting prior; Student-t
    # innovation precision makes the row gain noise-dependent and one-sided.
    spf_beta1: float = 0.9
    spf_beta2: float = 0.999
    spf_student_df: float = 5.0
    spf_diagnostic_interval: int = 64  # amortize telemetry-only row reductions.
...
class StreamingPosteriorFilter(optim.Optimizer):
    """Row-isotropic robust Kalman filter over a latent gradient direction.

    Matrices use one filter per output row. A vector or scalar tensor uses one
    filter for the tensor. M and Adam's controlled RMS path are full-size; P and
    R are float64 row scalars. Rows are independent, so plasticity is not
    zero-sum.
    """
    ...
    def __init__(
        ...
        beta2=0.999,
        student_df=5.0,
        diagnostic_interval=64,
    ):
        if lr <= 0.0:
            raise ValueError(f"lr must be positive, got {lr}")
        if not 0.0 < beta1 < 1.0 or not 0.0 <= beta2 < 1.0:
            raise ValueError(f"beta1 must be in (0, 1) and beta2 in [0, 1), got {(beta1, beta2)}")
        if student_df <= 2.0:
            raise ValueError(f"student_df must exceed 2 for finite variance, got {student_df}")
        if diagnostic_interval <= 0:
            raise ValueError(f"diagnostic_interval must be positive, got {diagnostic_interval}")
        defaults = dict(
            lr=lr,
            beta1=beta1,
            beta2=beta2,
    ...
    @classmethod
    def _row_rms(cls, x):
        """Overflow-safe row RMS for every finite floating-point input."""
        rows = cls._rows(x)
        scale = rows.abs().amax(dim=1)
        safe_scale = torch.where(scale > 0, scale, torch.ones_like(scale))
        normalized = rows / safe_scale.unsqueeze(1)
        return scale * normalized.square().mean(dim=1).sqrt()

    @staticmethod
    def _expand_rows(rows, target):
        if target.ndim == 0:
            return rows.reshape(())
        if target.ndim == 1:
            return rows.reshape(1)
        return rows.view(rows.shape[0], *([1] * (target.ndim - 1)))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            beta2 = group["beta2"]
            nu = group["student_df"]
            diagnostic_interval = group["diagnostic_interval"]
            nominal_gain = 1.0 - beta1
            q_over_r = nominal_gain * nominal_gain / beta1
            variance_floor = torch.finfo(torch.float64).tiny
            ...
            for p in group["params"]:
                ...
                grad = p.grad
                if grad.is_sparse:
                    raise RuntimeError("StreamingPosteriorFilter does not support sparse gradients")
                if not grad.is_floating_point():
                    raise RuntimeError("StreamingPosteriorFilter requires floating-point gradients")

                state = self.state[p]
                if len(state) == 0:
                    observation_var = (
                        self._row_rms(grad).double().square().clamp_min(variance_floor)
                    )
                    state["step"] = 0
                    state["mean"] = grad.clone()
                    # Store sqrt(V), not V. This is algebraically Adam's raw
                    # second moment but avoids overflow/underflow in g^2.
                    state["second_moment_rms"] = torch.zeros_like(p)
                    state["posterior_var"] = observation_var * nominal_gain
                    state["observation_var"] = observation_var

                mean = state["mean"]
                second_moment_rms = state["second_moment_rms"]
                posterior_var = state["posterior_var"]
                observation_var = state["observation_var"]

                innovation = grad - mean
                predicted_var = posterior_var + q_over_r * observation_var
                innovation_scale = predicted_var + observation_var
                innovation_rms = self._row_rms(innovation).double()
                row_width = self._rows(grad).shape[1]
                delta = row_width * innovation_rms.square() / innovation_scale
                student_weight = ((nu + row_width) / (nu + delta)).clamp_max(1.0)

                # Information-form gain remains finite when an extreme finite
                # innovation makes w underflow to zero: K -> 0 without R / w.
                weighted_predicted_var = student_weight * predicted_var
                kalman_gain = weighted_predicted_var / (
                    weighted_predicted_var + observation_var
                )
                gain = self._expand_rows(kalman_gain.to(dtype=grad.dtype), grad)

                collect_diagnostics = state["step"] % diagnostic_interval == 0
                if collect_diagnostics:
                    state["last_prior_cosine"] = F.cosine_similarity(
                        self._rows(mean),
                        self._rows(grad),
                        dim=1,
                    )

                mean.add_(innovation * gain)
                # Algebraically identical to the scalar Joseph update here, but
                # this form never evaluates 0 * inf in the w=0 rejection limit.
                posterior_var.mul_(1.0 - kalman_gain).add_(
                    q_over_r * observation_var * (1.0 - kalman_gain)
                )

                residual = grad - mean
                sqrt_weight = self._expand_rows(
                    student_weight.sqrt().to(dtype=grad.dtype),
                    residual,
                )
                weighted_residual_rms = self._row_rms(residual * sqrt_weight).double()
                robust_noise_target = (
                    weighted_residual_rms.square()
                    + student_weight * posterior_var
                )
                observation_var.lerp_(robust_noise_target, 1.0 - beta2)
                observation_var.clamp_min_(variance_floor)

                state["step"] += 1
                second_moment_rms.mul_(beta2 ** 0.5)
                scaled_grad = grad.abs() * ((1.0 - beta2) ** 0.5)
                torch.hypot(second_moment_rms, scaled_grad, out=second_moment_rms)
                bias_correction_rms = (1.0 - beta2 ** state["step"]) ** 0.5
                denominator = second_moment_rms / bias_correction_rms
                denominator = torch.where(
                    denominator > 0,
                    denominator,
                    torch.ones_like(denominator),
                )
                direction = mean / denominator
                if collect_diagnostics:
                    parameter_rms = self._row_rms(p)
                    update_rms = self._row_rms(direction) * lr
                    relative_denominator = torch.where(
                        parameter_rms > 0,
                        parameter_rms,
                        torch.ones_like(parameter_rms),
                    )
                    state["last_update_rms"] = update_rms
                    state["last_relative_update"] = update_rms / relative_denominator
                p.add_(direction, alpha=-lr)

                state["last_gain"] = kalman_gain
                state["last_student_weight"] = student_weight
                state["last_delta_per_dim"] = (delta / row_width).clamp_max(
    ...
    def load_state_dict(self, state_dict):
        ...
        # Re-read the original serialized row-filter tensors so a float32 model
        # does not silently lose the float64 covariance precision.
        double_keys = (
            "posterior_var",
            "observation_var",
            "last_gain",
        ...
        for saved_group, current_group in zip(
            ...
            for saved_id, parameter in zip(
                ...
                current_state = self.state[parameter]
                for key in double_keys:
                    if key in saved_state:
                        current_state[key] = saved_state[key].to(
    ...
    def diagnostics(self):
        ...
        for state in self.state.values():
            if "last_gain" not in state:
                continue
            fields["gain"].append(state["last_gain"].reshape(-1))
            fields["weight"].append(state["last_student_weight"].reshape(-1))
            fields["p_over_r"].append(
                (state["posterior_var"] / state["observation_var"]).reshape(-1)
            )
            fields["delta"].append(state["last_delta_per_dim"].reshape(-1))
            fields["cosine"].append(state["last_prior_cosine"].reshape(-1))
            fields["update"].append(state["last_update_rms"].reshape(-1))
            fields["relative_update"].append(state["last_relative_update"].reshape(-1))
        if not fields["gain"]:
            return {}

        values = {name: torch.cat(parts) for name, parts in fields.items()}
        gain = values["gain"]
        weight = values["weight"]
        return {
            "spf/gain_mean": gain.mean().item(),
            "spf/gain_std": gain.std(correction=0).item(),
            "spf/gain_p10": gain.quantile(0.1).item(),
            "spf/gain_p90": gain.quantile(0.9).item(),
            "spf/student_weight_mean": weight.mean().item(),
            "spf/outlier_fraction": (weight < 0.5).float().mean().item(),
            "spf/p_over_r_mean": values["p_over_r"].mean().item(),
            "spf/innovation_per_dim_mean": values["delta"].mean().item(),
            "spf/prior_cosine_mean": values["cosine"].mean().item(),
            "spf/update_rms_mean": values["update"].mean().item(),
            "spf/relative_update_mean": values["relative_update"].mean().item(),
        }
...
if __name__ == "__main__":
    ...
    optimizer = StreamingPosteriorFilter(
        agent.parameters(),
        lr=args.learning_rate,
        beta1=args.spf_beta1,
        beta2=args.spf_beta2,
        student_df=args.spf_student_df,
        diagnostic_interval=args.spf_diagnostic_interval,
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for k, v in optimizer.diagnostics().items():
