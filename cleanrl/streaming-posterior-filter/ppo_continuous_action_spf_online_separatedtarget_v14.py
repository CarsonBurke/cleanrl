# Separated actor/target-V online actor-critic v14.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# The actor and distributional V critic use independent trunks, removing the
# shared-feature interference observed when one-step value errors destabilize
# the policy representation. The successor bootstrap still comes from a frozen
# Polyak V trunk/head read before the sole optimizer step and updated afterward.
# Every transition is fresh and single-use; there is no Q function, replay,
# rollout statistic, repeated epoch, or cross-step autograd graph.
import copy
import os
import random
import time
...
class Args:
    ...
    # Disjoint trunks keep actor and target-V representation learning isolated.
    ...
    # Polyak target for the V trunk and distributional value head.
    target_value_tau: float = 0.005
...
def validate_online_contract(args):
    """Reject configurations that invalidate the streaming target-V experiment."""
    required = {
        "num_steps": 1,
        "num_minibatches": 1,
        "update_epochs": 1,
    ...
    if not 0 < args.target_value_tau <= 1:
        violations.append("target_value_tau must be in (0, 1]")
    if violations:
        raise ValueError(
            "online separatedtarget v14 requires disjoint actor/value trunks, "
...
@torch.no_grad()
def polyak_update_value(target_agent, source_agent, tau):
    target_parameters = target_agent.critic_parameters()
    source_parameters = source_agent.critic_parameters()
    if len(target_parameters) != len(source_parameters) or any(
        target.shape != source.shape
        for target, source in zip(target_parameters, source_parameters)
    ):
        raise ValueError("target and source V parameter structures differ")
    for target, source in zip(target_parameters, source_parameters):
        target.lerp_(source, tau)


def one_step_target(reward, current_value, target_next_value, termination, valid, gamma):
    """Return the V-only TD advantage and raw-reward target for one transition."""
    bootstrap = (1.0 - termination) * valid
    advantage = reward + gamma * target_next_value * bootstrap - current_value
    return advantage, advantage + current_value
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            next_transition_value_logits = target_agent.get_value(
            ...
            # The contract fixes T=1 and disables entropy augmentation. Both
            # actor and critic therefore use the same causal target-V bootstrap.
            advantages, returns = one_step_target(
                rewards,
                values,
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    # The contract makes the actor and critic parameter sets
                    # disjoint. Their graphs can be freed independently and
                    # their clipped gradients coexist until the one optimizer
                    # step without cloning or recombination.
            ...
            if args.target_kl is not None and approx_kl > args.target_kl:
                break
        polyak_update_value(target_agent, agent, args.target_value_tau)
