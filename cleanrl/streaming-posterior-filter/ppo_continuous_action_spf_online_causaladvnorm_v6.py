# Single-use online fast-memory actor-critic with lagged advantage norm v6.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# A shared robust location and scale summarize only prior one-step V(s) TD
# errors. The current 64 fresh transitions are centered and scaled with that
# frozen pre-update state; only afterward do they update it for the next step.
# This approximates PPO advantage normalization without a temporal rollout or
# current-batch leakage, while preserving all within-step relative magnitudes.
# Raw TD targets still train V(s). No Q critic, replay, repeated epoch, BPTT, or
# current-batch mean/variance enters the actor update.
import os
import random
import time
...
class Args:
    ...
    # Lagged robust location/scale for the actor TD error only.
    causal_norm_student_df: float = 5.0
    causal_norm_location_rate: float = 0.01
    causal_norm_scale_rate: float = 0.01
    causal_norm_initial_scale: float = 1.0
    causal_norm_min_scale: float = 0.1
    causal_norm_max_scale: float = 100.0
...
def validate_online_contract(args):
    """Reject configurations that invalidate the single-use V-critic experiment."""
    required = {
        "num_steps": 1,
        "num_minibatches": 1,
        "update_epochs": 1,
    ...
    if args.causal_norm_student_df <= 0:
        violations.append("causal_norm_student_df must be positive")
    if not 0 < args.causal_norm_location_rate <= 1:
        violations.append("causal_norm_location_rate must be in (0, 1]")
    if not 0 < args.causal_norm_scale_rate <= 1:
        violations.append("causal_norm_scale_rate must be in (0, 1]")
    if not (
        0
        < args.causal_norm_min_scale
        <= args.causal_norm_initial_scale
        <= args.causal_norm_max_scale
    ):
        violations.append(
            "causal normalization scales must satisfy 0 < min <= initial <= max"
        )
    if violations:
        raise ValueError(
            "online causaladvnorm v6 requires one fresh transition per stream "
...
class CausalAdvantageNormalizer:
    """Lagged robust location/scale; current outputs use prior state only."""

    def __init__(self, args, device):
        self.student_df = args.causal_norm_student_df
        self.location_rate = args.causal_norm_location_rate
        self.scale_rate = args.causal_norm_scale_rate
        self.min_log_scale = log(args.causal_norm_min_scale)
        self.max_log_scale = log(args.causal_norm_max_scale)
        self.location = torch.zeros((), device=device)
        self.log_scale = torch.full(
            (),
            log(args.causal_norm_initial_scale),
            device=device,
        )
    ...
    @torch.no_grad()
    def transform(self, td_error):
        location = self.location.clone()
        scale = self.log_scale.exp().clone()
        centered = td_error - location
        normalized = centered / scale
        z2 = normalized.square()
        ...
        self.location.add_(self.location_rate * scale * location_score)
        self.log_scale.add_(self.scale_rate * scale_score)
    ...
    def diagnostics(self):
        ...
        self.diagnostic_steps = 0
        return {
            "causal_adv_norm/location": values[0].item(),
            "causal_adv_norm/scale": values[1].item(),
            "causal_adv_norm/raw_mean": values[2].item(),
            "causal_adv_norm/centered_mean": values[3].item(),
            "causal_adv_norm/normalized_mean": values[4].item(),
            "causal_adv_norm/normalized_std": values[5].item(),
        }
...
if __name__ == "__main__":
    ...
    advantage_normalizer = CausalAdvantageNormalizer(args, device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            raw_policy_adv = policy_adv
            policy_adv = advantage_normalizer.transform(policy_adv)
        ...
        b_advantages = policy_adv.reshape(-1)            # lagged causal-normalized policy TD error
        b_raw_advantages = raw_policy_adv.reshape(-1)
        ...
        gae = b_raw_advantages
        b_policy_adv = shape_advantage(b_advantages, b_sigma, b_u, args, device)
        ...
        for k, v in advantage_normalizer.diagnostics().items():
            writer.add_scalar(k, v, global_step)
        writer.add_scalar(
