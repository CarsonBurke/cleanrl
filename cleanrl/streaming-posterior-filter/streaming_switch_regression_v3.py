# Temporal-hypergradient robust posterior v3.
# Fully streaming predict -> score -> update. This version propagates forward
# sensitivities of posterior mean and variance to process noise, so Q is credited
# for persistent tracking rather than only inflating the current variance.
...
def make_temporal_hyperfilter_step(args, device):
    weight = torch.zeros((args.output_dim, args.input_dim), device=device)
    posterior_var = torch.full_like(weight, args.filter_initial_variance)
    observation_var = torch.full((args.output_dim,), args.noise_std**2, device=device)
    eta0 = math.log(args.filter_process_variance)
    log_q = torch.full((args.output_dim,), eta0, device=device)
    mean_sensitivity = torch.zeros_like(weight)
    var_sensitivity = torch.zeros_like(posterior_var)
    nu = args.filter_student_df
    log_q_min = math.log(args.filter_q_min)
    log_q_max = math.log(args.filter_q_max)

    def transition(mean, variance, eta, x, observed):
        process_var = eta.exp()
        predicted_var = variance + process_var.unsqueeze(1)
        prediction = F.linear(x, mean)
        residual = observed - prediction
        feature_square = x.square()
        a_i = predicted_var * feature_square.unsqueeze(0)
        projected_var = a_i.sum(1)
        ...
        denominator = projected_var + effective_noise
        gain = predicted_var * x.unsqueeze(0) / denominator.unsqueeze(1)
        next_mean = mean + gain * residual.unsqueeze(1)
        b_i = effective_noise.unsqueeze(1) + (
            projected_var.unsqueeze(1) - a_i
        ).clamp_min(0.0)
        one_minus_kx = 1.0 - gain * x.unsqueeze(0)
        next_var = predicted_var * one_minus_kx.square() + gain.square() * b_i
    ...
    def step(x, observed, latent):
        ...
        residual = observed - prediction
        feature_square = x.square()
        projected_var = (predicted_var * feature_square.unsqueeze(0)).sum(1)
        innovation_var = observation_var + projected_var
        delta = residual.square() / innovation_var
        raw_weight = (nu + 1.0) / (nu + delta)
        ...
        residual_sensitivity = -(mean_sensitivity * x.unsqueeze(0)).sum(1)
        innovation_var_sensitivity = (
            (var_sensitivity + process_var.unsqueeze(1))
            * feature_square.unsqueeze(0)
        ).sum(1)
        loss_residual_gradient = raw_weight * residual / innovation_var
        loss_variance_gradient = 0.5 * (
            1.0 - raw_weight * delta
        ) / innovation_var
        q_score = (
            loss_residual_gradient * residual_sensitivity
            + loss_variance_gradient * innovation_var_sensitivity
            + args.filter_q_prior * (log_q - eta0)
        )

        jvp_result = torch.func.jvp(
            lambda mean, variance, eta: transition(
                mean,
                variance,
                eta,
                x,
            ...
            (weight, posterior_var, log_q),
            (
                mean_sensitivity,
                var_sensitivity,
                torch.ones_like(log_q),
            ),
        )
        next_weight, next_var = jvp_result[0]
        next_mean_sensitivity, next_var_sensitivity = jvp_result[1]
        update = next_weight - weight
        weight.copy_(next_weight)
        posterior_var.copy_(next_var)
        mean_sensitivity.copy_(next_mean_sensitivity)
        var_sensitivity.copy_(next_var_sensitivity)
        log_q.add_(q_score, alpha=-args.filter_q_rate)
        log_q.clamp_(log_q_min, log_q_max)
...
def main():
    ...
    run_name = (
        f"StreamingSwitch-v3__{args.exp_name}_{args.method}_{args.condition}"
