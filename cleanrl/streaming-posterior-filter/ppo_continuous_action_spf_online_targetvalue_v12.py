# Streaming Polyak target V(s) critic v12.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# The online actor and distributional V critic still update from each fresh
# transition once, but the successor bootstrap comes from a slowly moving copy
# of the V trunk/head. The target is read before the optimizer step and updated
# afterward, stabilizing the moving TD target without replay or a Q function.
# No rollout, repeated epoch, batch advantage normalization, or BPTT.
import copy
import os
import random
import time
...
def validate_online_contract(args):
    ...
    if violations:
        raise ValueError(
            "online targetvalue v12 requires one fresh transition per stream "
