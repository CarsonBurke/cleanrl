# Episodic epistemic-token policy exploration v15.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# Each environment samples one Gaussian policy token at episode start and keeps
# it fixed until the boundary. Learned low-rank Beta-logit sensitivity heads map
# that token into a smooth, bounded, state-dependent policy perturbation. This
# creates temporally coherent parameter-space exploration without a fast policy
# update, eligibility trace, replay, or cross-step graph. The critic never sees
# the token, so V(s) remains a marginal state baseline and the one-step policy
# gradient can reinforce useful episode-level perturbation directions.
# Every transition is fresh and single-use. There is no Q function, rollout
# statistic, repeated epoch, batch advantage normalization, or BPTT.
import os
import random
import time
...
class Args:
    ...
    # Episode-persistent low-rank parameter-space exploration.
    policy_token_rank: int = 8
    policy_token_scale: float = 0.5
    policy_token_offset_clip: float = 0.5
...
def validate_online_contract(args):
    """Reject configurations that invalidate epistemic-token isolation."""
    required = {
        "num_steps": 1,
        "num_minibatches": 1,
        "update_epochs": 1,
    ...
    if args.policy_token_rank < 1:
        violations.append("policy_token_rank must be >= 1")
    if args.policy_token_scale <= 0:
        violations.append("policy_token_scale must be > 0")
    if args.policy_token_offset_clip <= 0:
        violations.append("policy_token_offset_clip must be > 0")
    if violations:
        raise ValueError(
            "online epistemictoken v15 requires one episode-persistent token, "
            "one fresh transition per stream, and one optimizer use; invalid "
            f"settings: {', '.join(violations)}"
        )


@torch.no_grad()
def resample_episode_tokens(tokens, boundaries):
    """Keep continuing-stream tokens and refresh only reset streams."""
    return torch.where(
        boundaries.to(dtype=torch.bool).unsqueeze(1),
        torch.randn_like(tokens),
        tokens,
    )
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        H = args.hidden
        self.policy_token_rank = args.policy_token_rank
        self.policy_token_scale = args.policy_token_scale
        self.policy_token_offset_clip = args.policy_token_offset_clip
        self.register_buffer("last_token_offset_rms", torch.zeros(()))
        ...
        if self.actor_dist == "gaussian":
            ...
            self.actor_alpha_token_head = layer_init(
                nn.Linear(H, act_dim * self.policy_token_rank), std=0.05
            )
            self.actor_beta_token_head = layer_init(
                nn.Linear(H, act_dim * self.policy_token_rank), std=0.05
            )
    ...
    def _actor_dist(self, actor_feat, actor_token=None):
        ...
        alpha_logits = self.actor_alpha_head(actor_feat)
        beta_logits = self.actor_beta_head(actor_feat)
        if actor_token is not None:
            batch, act_dim = alpha_logits.shape
            alpha_sensitivity = self.actor_alpha_token_head(actor_feat).view(
                batch, act_dim, self.policy_token_rank
            )
            beta_sensitivity = self.actor_beta_token_head(actor_feat).view(
                batch, act_dim, self.policy_token_rank
            )
            normalizer = self.policy_token_rank**0.5
            alpha_raw = (
                self.policy_token_scale
                * torch.einsum("bar,br->ba", alpha_sensitivity, actor_token)
                / normalizer
            )
            beta_raw = (
                self.policy_token_scale
                * torch.einsum("bar,br->ba", beta_sensitivity, actor_token)
                / normalizer
            )
            clip = self.policy_token_offset_clip
            alpha_offset = clip * torch.tanh(alpha_raw / clip)
            beta_offset = clip * torch.tanh(beta_raw / clip)
            alpha_logits = alpha_logits + alpha_offset
            beta_logits = beta_logits + beta_offset
            with torch.no_grad():
                self.last_token_offset_rms.copy_(
                    torch.cat((alpha_offset, beta_offset), dim=1)
                    .square()
                    .mean()
                    .sqrt()
                )
    ...
    def get_action_and_value(self, x, fast_context, z=None, actor_token=None):
        # The stored native action, context, and episode token reproduce the
        # exact behavior-policy sample during its sole optimizer use.
        actor_feat, critic_feat = self._trunks(x, fast_context)
        dist, to_action, log_det_fn = self._actor_dist(actor_feat, actor_token)
        if z is None:
            z = dist.sample()
    ...
    def actor_parameters(self):
        ...
        if self.actor_dist == "gaussian":
            ...
            heads = (
                ...
                + list(self.actor_alpha_token_head.parameters())
                + list(self.actor_beta_token_head.parameters())
            )
...
if __name__ == "__main__":
    ...
    actor_tokens = torch.zeros(
        (args.num_steps, args.num_envs, args.policy_token_rank), device=device
    )
    ...
    next_actor_token = torch.randn(
        args.num_envs, args.policy_token_rank, device=device
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            dist_p1[step] = p1
            dist_p2[step] = p2
            actor_tokens[step] = next_actor_token
            ...
            next_context = fast_memory.read(next_obs)
            next_actor_token = resample_episode_tokens(
                next_actor_token, transition_boundaries[step]
            )
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                _, _, next_logprob, _, _, _ = agent.get_action_and_value(
                    next_obses.reshape((-1,) + envs.single_observation_space.shape),
                    next_contexts.reshape((-1,) + envs.single_observation_space.shape),
                    actor_token=actor_tokens.reshape(-1, args.policy_token_rank),
                )
        ...
        b_actor_tokens = actor_tokens.reshape(-1, args.policy_token_rank)
        ...
        writer.add_scalar(
            "epistemic_token/offset_rms",
            agent.last_token_offset_rms.item(),
            global_step,
        )
        writer.add_scalar(
            "epistemic_token/boundary_fraction",
            transition_boundaries.mean().item(),
            global_step,
        )
        writer.add_scalar(
