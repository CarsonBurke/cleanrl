# Single-use online fast-memory actor-critic with causal robust TD gating v5.
...
# attenuates only surprising V(s)-critic advantages. Unlike v4, the gate
# preserves the raw TD-error units instead of dividing by the adaptive scale, so
# ordinary magnitude information cannot collapse into sign-SGD. The raw one-step
# TD target still trains V(s). No Q critic, replay, multi-step rollout, repeated
# epoch, cross-stream advantage statistic, or BPTT.
import os
import random
import time
...
def validate_online_contract(args):
    ...
    if violations:
        raise ValueError(
            "online robustgate v5 requires one fresh transition per stream and "
...
class CausalStudentAdvantage:
    """Per-stream Student-t outlier gate with pre-update scale reads."""
    ...
    def transform(self, td_error):
        ...
        robust_advantage = td_error * student_weight.sqrt()
    ...
    def diagnostics(self):
        ...
        return {
            ...
            "robust_adv/gated_abs_mean": values[3].item(),
        }
