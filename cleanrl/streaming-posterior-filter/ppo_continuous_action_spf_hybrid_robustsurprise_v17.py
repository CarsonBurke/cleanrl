# Hybrid robust-surprise PPO v17.
# Family contract: cleanrl/streaming-poster-filter/FAMILY.md
#
# Reuses the strongest v2 fast dynamics memory as the exploration model instead
# of adding another per-step posterior. Its already-computed Student-t innovation
# yields a bounded learnable-surprise reward: w*log(1+normalized_error), which
# vanishes for gross outliers. A lagged per-stream baseline centers that signal;
# stored bonuses propagate through GAE actor-only while the distributional V
# critic remains on environment reward. This preserves v2 replay semantics and
# adds no live-state read inside PPO epochs. There is no Q function.
...
class RobustFastDynamics:
    ...
    def update(self, obs, next_obs, valid):
        ...
        surprise_bonus = (surprise_error / surprise_std).clamp(
    ...
    def diagnostics(self):
        ...
        return {
            ...
            "robust_surprise/normalized": values[6].item(),
