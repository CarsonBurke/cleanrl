# Single-use online fast-memory actor-critic with causal robust TD scaling v4.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# Sixty-four independent streams contribute one fresh transition each. A
# per-stream Student-t scale, read before and updated after the current TD error,
# converts the V(s)-critic advantage into a bounded policy influence. The raw
# one-step TD target still trains the value critic. No Q critic, replay,
# multi-step rollout, repeated epoch, cross-stream advantage statistic, or BPTT.
# The hypothesis is that bounding each stream's TD influence prevents noisy
# transitions from dominating the online policy direction without discarding
# sign or temporal order.
import os
import random
import time
...
class Args:
    ...
    # Causal per-stream Student-t normalization for the policy TD error only.
    robust_adv_student_df: float = 5.0
    robust_adv_scale_rate: float = 0.01
    robust_adv_initial_scale: float = 1.0
    robust_adv_min_scale: float = 0.1
    robust_adv_max_scale: float = 100.0
...
def validate_online_contract(args):
    ...
    if args.robust_adv_student_df <= 0:
        violations.append("robust_adv_student_df must be positive")
    if not 0 < args.robust_adv_scale_rate <= 1:
        violations.append("robust_adv_scale_rate must be in (0, 1]")
    if not (
        0
        < args.robust_adv_min_scale
        <= args.robust_adv_initial_scale
        <= args.robust_adv_max_scale
    ):
        violations.append(
            "robust advantage scales must satisfy 0 < min <= initial <= max"
        )
    if violations:
        raise ValueError(
            "online robustadv v4 requires one fresh transition per stream and "
...
class CausalStudentAdvantage:
    """Per-stream robust scale and bounded influence with pre-update reads."""

    def __init__(self, num_envs, args, device):
        self.student_df = args.robust_adv_student_df
        self.scale_rate = args.robust_adv_scale_rate
        self.min_log_scale = log(args.robust_adv_min_scale)
        self.max_log_scale = log(args.robust_adv_max_scale)
        self.log_scale = torch.full(
            (num_envs,),
            log(args.robust_adv_initial_scale),
            device=device,
        )
        self.diagnostic_sum = torch.zeros(4, device=device)
        self.diagnostic_steps = 0
    ...
    def transform(self, td_error):
        ...
        z = td_error / scale
        z2 = z.square()
        student_weight = self.student_df / (self.student_df + z2)
        robust_advantage = z * student_weight.sqrt()

        scale_score = (
            (self.student_df + 1.0) * z2 / (self.student_df + z2) - 1.0
        )
    ...
    def diagnostics(self):
        ...
        self.diagnostic_steps = 0
        return {
            "robust_adv/scale": values[0].item(),
            "robust_adv/student_weight": values[1].item(),
            "robust_adv/raw_abs_mean": values[2].item(),
            "robust_adv/bounded_abs_mean": values[3].item(),
        }
...
if __name__ == "__main__":
    ...
    robust_advantage = CausalStudentAdvantage(args.num_envs, args, device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            policy_adv = robust_advantage.transform(policy_adv.squeeze(0)).unsqueeze(0)
        ...
        b_advantages = policy_adv.reshape(-1)            # causal robust policy TD influence
        ...
        for k, v in robust_advantage.diagnostics().items():
            writer.add_scalar(k, v, global_step)
        writer.add_scalar(
