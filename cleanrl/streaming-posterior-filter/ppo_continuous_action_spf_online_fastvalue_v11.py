# Streaming dual-timescale V(s) residual v11.
# Family contract: cleanrl/streaming-posterior-filter/FAMILY.md
#
# Each environment owns a robust linear fast residual on top of the slow
# distributional V critic. Actor TD errors use V_slow+V_fast, while the slow
# critic keeps its raw one-step target; the fast residual is read for both
# endpoints before the current TD error updates it. This targets critic lag
# without directly plasticizing the policy. No Q critic, replay, rollout,
# repeated epoch, batch advantage normalization, or BPTT.
import os
import random
import time
...
class Args:
    ...
    # Robust per-stream linear residual on V(s).
    fast_value_eta: float = 0.01
    fast_value_leak: float = 0.9999
    fast_value_student_df: float = 5.0
    fast_value_scale_rate: float = 0.001
    fast_value_correction_clip: float = 20.0
...
def validate_online_contract(args):
    """Reject configurations that invalidate the streaming V-residual experiment."""
    required = {
        "num_steps": 1,
        "num_minibatches": 1,
        "update_epochs": 1,
    ...
    if args.fast_value_eta <= 0:
        violations.append("fast_value_eta must be positive")
    if not 0 < args.fast_value_leak <= 1:
        violations.append("fast_value_leak must be in (0, 1]")
    if args.fast_value_student_df <= 0:
        violations.append("fast_value_student_df must be positive")
    if not 0 < args.fast_value_scale_rate <= 1:
        violations.append("fast_value_scale_rate must be in (0, 1]")
    if args.fast_value_correction_clip <= 0:
        violations.append("fast_value_correction_clip must be positive")
    if violations:
        raise ValueError(
            "online fastvalue v11 requires one fresh transition per stream and "
...
class FastValueResidual:
    """Per-stream robust linear correction to the slow state-value critic."""

    def __init__(self, num_envs, obs_dim, args, device):
        self.weight = torch.zeros((num_envs, obs_dim), device=device)
        self.scale = torch.ones(num_envs, device=device)
        self.eta = args.fast_value_eta
        self.leak = args.fast_value_leak
        self.student_df = args.fast_value_student_df
        self.scale_rate = args.fast_value_scale_rate
        self.correction_clip = args.fast_value_correction_clip
    ...
    def read(self, obs):
        key = F.normalize(obs, dim=-1)
        return (self.weight * key).sum(-1).clamp(
            -self.correction_clip,
            self.correction_clip,
        )

    @torch.no_grad()
    def update(self, obs, td_error):
        key = F.normalize(obs, dim=-1)
        z2 = td_error.square() / self.scale.square().clamp_min(
            torch.finfo(self.scale.dtype).tiny
        )
        student_weight = (
            (self.student_df + 1.0) / (self.student_df + z2)
        ).clamp_max(1.0)
        update = (
            self.eta
            * student_weight.unsqueeze(1)
            * td_error.unsqueeze(1)
            * key
        )
        self.weight.mul_(self.leak).add_(update)
        scale_target = student_weight * td_error.square()
        self.scale.square_().lerp_(scale_target, self.scale_rate).sqrt_()
        self.scale.clamp_min_(torch.finfo(self.scale.dtype).tiny)

        correction = self.read(obs)
        self.diagnostic_sum.add_(
            torch.stack(
                (
                    td_error.abs().mean(),
                    ...
                    correction.square().mean().sqrt(),
                    self.weight.square().mean().sqrt(),
                )
            )
        )
        self.diagnostic_steps += 1
    ...
    def diagnostics(self):
        ...
        self.diagnostic_steps = 0
        return {
            "fast_value/td_abs_mean": values[0].item(),
            "fast_value/student_weight": values[1].item(),
            "fast_value/scale": values[2].item(),
            "fast_value/update_rms": values[3].item(),
            "fast_value/correction_rms": values[4].item(),
            "fast_value/weight_rms": values[5].item(),
        }
...
if __name__ == "__main__":
    ...
    fast_value = FastValueResidual(args.num_envs, obs_dim, args, device)
    ...
    fast_values = torch.zeros((args.num_steps, args.num_envs), device=device)
    next_fast_values = torch.zeros_like(fast_values)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            fast_values[step] = fast_value.read(next_obs)
            ...
            next_fast_values[step] = fast_value.read(transition_next_obs_tensor)
        ...
        with torch.no_grad():
            ...
            policy_adv = (
                advantages
                + args.gamma * next_fast_values * bootstrap_nonterminal
                - fast_values
            )
            fast_value.update(obs[0], policy_adv[0])
        ...
        for k, v in fast_value.diagnostics().items():
            writer.add_scalar(k, v, global_step)
        writer.add_scalar(
