# Successor forecasts plus explicit JEPA future-target stop-gradient v11.
...
# Default combination stops only the observed future prediction target AFTER
# projection. Current prediction and both SIGReg branches still train the
# encoder/projector. SF teacher targets always stop; SF student attachment is
# independent. The attached-prediction control reproduces v8's objective.
import os
import random
import time
...
class Args:
    ...
    prediction_target_gradient: Literal["attached", "stopped"] = "stopped"
    """JEPA prediction future target only, after projection; SIGReg remains attached"""
...
class LeWMBranch(nn.Module):
    ...
    def __init__(self, action_dim, num_proj, proj_chunk, prediction_target_gradient):
        super().__init__()
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.ssl = LeWMBranch(self.action_dim, args.sigreg_num_proj, args.sigreg_proj_chunk,
                              args.prediction_target_gradient)
...
def main():
    ...
    try:
        ...
        writer.add_text("prediction_target_contract", f"{args.prediction_target_gradient=}; "
                        "only observed future prediction targets stop after projection; "
                        "current prediction and both SIGReg branches remain attached")
