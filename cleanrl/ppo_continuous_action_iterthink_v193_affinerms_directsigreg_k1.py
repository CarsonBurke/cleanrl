# PPO + IterThink v193 affine-RMSNorm direct-SIGReg k1.
#
# v193 combines v145's proven raw-affine -> tokenwise affine-RMSNorm input with
# v186's warm temporal predictor and v182's detached predicted-latent control path.
# The encoder output is the working latent used by recurrence, direct attached MSE,
# per-token SIGReg, imagination, and (through a stop-gradient) PPO. The v183/v186
# off-path BatchNorm projector is removed. Higher WM MTP offsets are removed because
# only offset 0 received a latent gradient; k1 makes every consumed WM head trained.
# No running observation normalization, EMA target, or target detach is introduced.
#
# -----------------------------------------------------------------------------
...
class Args:
    ...
    lewm_mtp_len: int = 1  # v193: remove untrained higher-offset WM projections
    ...
    lewm_sigreg_num_proj: int = 1024  # v193: true per-token D=64 SIGReg (v145/le-wm density)
    lewm_sigreg_knots: int = 17
    lewm_sigreg_min_valid: int = 32
    lewm_sigreg_ref_n: int = 128  # v193: preserve le-wm's coefficient calibration independent of WM batch size
...
def init_affine_tokenizer(weight, bias):
    """Initialize scalar affine tokens with magnitude-sensitive angular geometry.

    RMSNorm(x*w+b) is sign-only when b=0.  Give every feature independent,
    equal-norm signal and bias directions so magnitude is encoded from step zero.
    """
    nn.init.xavier_uniform_(weight)
    nn.init.xavier_uniform_(bias)
    with torch.no_grad():
        proj = (bias * weight).sum(dim=-1, keepdim=True) / weight.square().sum(dim=-1, keepdim=True).clamp_min(1e-8)
        bias.sub_(proj * weight)
        bias.mul_(weight.norm(dim=-1, keepdim=True) / bias.norm(dim=-1, keepdim=True).clamp_min(1e-8))
...
class StdSelfAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads):
        ...
        self.norm = nn.Identity()  # v193: one affine RMS pre-norm lives in the enclosing block
...
class LeWMTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, ffn_mult):
        super().__init__()
        self.norm1 = nn.RMSNorm(dim, elementwise_affine=True, eps=1e-6)
        self.attn = StdSelfAttention(dim, num_heads)
        self.norm2 = nn.RMSNorm(dim, elementwise_affine=True, eps=1e-6)
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        init_affine_tokenizer(self.outcome_feature_weight, self.outcome_feature_bias)
        ...
        init_affine_tokenizer(self.obs_feature_weight, self.obs_feature_bias)
        self.embed_norm = nn.RMSNorm(self.dim, elementwise_affine=True, eps=1e-6)
        self.encoder_layers = nn.ModuleList(
            [LeWMTransformerBlock(self.dim, args.lewm_heads, args.lewm_ffn_mult) for _ in range(args.lewm_encoder_layers)]
        )
        # Final RMS is followed by raw learned target projections, so the actual
        # SIGReg latent retains variable radius and is not sphere-constrained.
        self.encoder_norm = nn.RMSNorm(self.dim, elementwise_affine=True, eps=1e-6)
        ...
        self.predictor_norm = nn.RMSNorm(self.dim, elementwise_affine=True, eps=1e-6)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if not self.detach_world_model_from_agent:
            raise ValueError("v193 requires a strictly detached world-model/agent interface")
        ...
        self.agent_input_ln = nn.RMSNorm(args.lewm_dim, elementwise_affine=True, eps=1e-6)
...
if __name__ == "__main__":
    ...
    assert args.lewm_mtp_len == 1, "v193 deliberately supports only the trained one-step WM head"
    assert args.obs_cond_mode == "none", "v193 isolates the detached belief-only agent interface"
    ...
    # Decay only ordinary matrix weights. Tokenizer signal/bias directions and
    # affine RMS gains encode geometry but have scale-invariant forward paths;
    # shrinking them toward RMS epsilon is harmful and provides no useful prior.
    wm_decay_params = [
        p for name, p in wm_latent_named
        if p.ndim >= 2 and "feature_" not in name and not name.endswith("bias")
    ]
    wm_no_decay_params = [
        p for name, p in wm_latent_named
        if not (p.ndim >= 2 and "feature_" not in name and not name.endswith("bias"))
    ]
    optimizer = optim.AdamW(
        [
            {"params": non_wm_params, "weight_decay": 0.0},
            {"params": wm_decay_params, "weight_decay": args.lewm_weight_decay},
            {"params": wm_no_decay_params, "weight_decay": 0.0},
    ...
    def masked_token_sigreg(token_latents, token_valids):
        ...
        for h in range(horizon):
            ...
            step_losses.append(sigreg(step_flat, A=A, n_scale=args.lewm_sigreg_ref_n))
        if not step_losses:
    ...
    def _sigreg_group_static(token_latents, token_valids, A):
        ...
        for h in range(token_latents.shape[1]):
            ...
            # Epps-Pulley is extensive in N. Pin N to le-wm's batch-128
            # calibration so a 1024-row WM minibatch does not make SIGReg 8x stronger.
            statistic = (err @ weights) * args.lewm_sigreg_ref_n
    ...
    def masked_token_sigreg_static(token_latents, token_valids, A_obs, A_out):
        # v193: true per-slot SIGReg. Each token's batch marginal is regularized
        # independently in D dimensions; token identity is never folded into the sample
        # axis and slots are not forced jointly independent. Vectorize the slot axis:
        # this is mathematically identical to 19 separate D-dimensional SIGReg calls,
        # but keeps the compiled graph at one body per horizon instead of 19 bodies.
        del A_out  # one shared random sketch basis is sufficient for identical N(0,I) targets
        t = sigreg.t.to(device=token_latents.device, dtype=token_latents.dtype)
        phi = sigreg.phi.to(device=token_latents.device, dtype=token_latents.dtype)
        weights = sigreg.weights.to(device=token_latents.device, dtype=token_latents.dtype)
        step_losses = []
        for h in range(token_latents.shape[1]):
            # (B,S,D) @ (D,P) -> (S,B,P); every slot retains its own batch marginal.
            projected = torch.einsum("bsd,dp->sbp", token_latents[:, h], A_obs)
            valid = token_valids[:, h].to(token_latents.dtype)
            valid_count = valid.sum().clamp_min(1.0)
            ...
            err = (cos_mean - phi).square() + sin_mean.square()  # (S,P,knots)
            step_loss = ((err @ weights) * args.lewm_sigreg_ref_n).mean()
            active = (valid.sum() >= args.lewm_sigreg_min_valid).to(token_latents.dtype)
            step_losses.append(step_loss * active)
        active_count = (
            (token_valids.sum(dim=0) >= args.lewm_sigreg_min_valid)
            .to(token_latents.dtype)
            .sum()
            .clamp_min(1.0)
        )
        return torch.stack(step_losses).sum() / active_count

    def masked_token_sigreg_metric_static(token_latents, token_valids, A_obs, A_out):
        # The full differentiable SIGReg is intentionally expensive. Metrics need only
        # a stable diagnostic estimate; use the reference 128 samples and 128 sketches
        # instead of repeating the full B=1024,P=1024 trigonometric pass under no_grad.
        metric_n = min(args.lewm_sigreg_ref_n, token_latents.shape[0])
        metric_p = min(128, A_obs.shape[1])
        return masked_token_sigreg_static(
            token_latents[:metric_n],
            token_valids[:metric_n],
            A_obs[:, :metric_p],
            A_out[:, :metric_p],
        )

    def masked_token_sigreg_train_static(token_latents, token_valids, A_obs, A_out):
        # SIGReg's reference statistic and coefficient are calibrated at N=128.
        # WM minibatches are shuffled, so their first 128 rows form a fresh unbiased
        # subset; using all 1024 only spends 8x compute to reduce estimator noise.
        train_n = min(args.lewm_sigreg_ref_n, token_latents.shape[0])
        return masked_token_sigreg_static(
            token_latents[:train_n], token_valids[:train_n], A_obs, A_out
        )
    ...
    def wm_supervised_loss_forward(
        ...
        # v193: direct prediction in the exact recurrent/control coordinates. Both
        # history and target encoder branches stay attached; no target stop-grad, EMA,
        # BN projector, or normalization exists between this MSE and the working latent.
        one_step_latent_mse = F.mse_loss(
            one_step_pred, target_summaries, reduction="none"
        ).mean(dim=(-1, -2))
        ...
        # SIGReg acts directly on the attached working latent, including anchor and targets.
        sigreg_latents = encoded_sequence
        wm_sigreg_loss = masked_token_sigreg_train_static(
            sigreg_latents, sigreg_weight, sigreg_A_obs, sigreg_A_out
        )
    ...
    def wm_supervised_metrics_forward(
        ...
        sigreg_latents = encoded_sequence
        wm_sigreg_loss = masked_token_sigreg_metric_static(
            sigreg_latents, sigreg_weight, sigreg_A_obs, sigreg_A_out
        )
        wm_discount_bce = discount_bce_sum
        wm_discount_bce_excess = discount_bce_sum - discount_entropy_sum
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for _ in range(args.lewm_update_epochs):
            ...
            for start in range(0, args.batch_size, wm_minibatch_size):
                ...
                # v193: D-dimensional sketches shared across independently regularized token slots.
