...
#
# BETANOISE v2 — v1 (v215-style unimodal Beta actor head) + warm-start init at the entropy budget.
#
# v1 paid a 25k-40k transient: alpha_ent started cold at 1.0 and drove concentrations from ~1.06 up
# to ~18 while the entropy autotuner settled (alpha_ent 1.0->0.04), so early actor updates mostly
# burned entropy rather than value (629 @50k vs sdnoise 5404, before catching up to 10400 @98k once
# converged). v2 removes that transient with ONE change: warm-start the Beta at its v1 equilibrium.
# The concentration heads are zero-weight with bias = inv_softplus(conc_init - 1), so the policy is
# exactly Beta(conc_init, conc_init) in every state at init (entropy already at the target budget,
# deterministic mean at action 0), and log_alpha starts at ln(alpha_ent_init) (v1's settled value)
# instead of 0. Everything else (autotune, target math, diagnostics, byte-identical --no-beta-noise
# fallback) is identical to v1. Registered prediction: >=5000 @50k and >= sdnoise within noise @100k;
# otherwise the Beta parameterization itself is inferior to the Gaussian sdnoise and the arm closes.
#
...
class Args:
    ...
    """v2 warm-start: initial Beta concentration in every state. 49.5 puts a SYMMETRIC Beta exactly
    at the -9.46 entropy target (action-std ~0.0995); v1's equilibrium mean-conc ~18 corresponds to
    an asymmetric boundary-pushed policy and would start a symmetric init above budget (std 0.164)"""
    alpha_ent_init: float = 0.1
    """v2 warm-start: initial alpha_ent (v1 settled ~0.1-0.17 from a cold start at 1.0)"""
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, zs_dim=256, hdim=256, use_beta=False, conc_init=18.0):
        super().__init__()
        ...
        if use_beta:
            ...
            # v2 warm-start: zero the head weights and set the bias to inv_softplus(conc_init - 1),
            # so 1 + softplus(bias) = conc_init in every state at init. The policy starts as
            # Beta(conc_init, conc_init) everywhere -> entropy already at the target budget and a
            # deterministic mean at action 0 (like a zero-init baseline head), eliminating the
            # v1 25k-40k transient where actor updates mostly grew concentration/burned entropy.
...
class TD7Agent:
    def __init__(self, state_dim, action_dim, max_action, args: Args, device, writer: SummaryWriter):
        ...
        self.actor = Actor(
            ...
            use_beta=args.beta_noise, conc_init=args.conc_init,
        ).to(device)
        ...
        if args.beta_noise:
            if args.alpha_autotune:
                ...
                # v2 warm-start: begin at v1's settled alpha_ent instead of a cold 1.0.
                self.log_alpha = torch.full(
                    (1,), float(np.log(args.alpha_ent_init)), requires_grad=True, device=device
                )
