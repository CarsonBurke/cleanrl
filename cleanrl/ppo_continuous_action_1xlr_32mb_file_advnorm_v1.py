# Base LR + 32 minibatches + file-level advantage normalization.
# Same host-actor trainer as ppo_continuous_action_32xlr_1mb_noadvnorm_v1.
# Follow-up to ppo_2xlr_32mb_file_advnorm_v1, which underperformed base
# (6382 vs 7468 at 8M): its per-iteration update distance (LR x updates =
# 6e-4 x 320) was 2x base and the reference, with 10x the approx-KL. Here
# LR is back to the 3e-4 PPO default so total movement per iteration matches
# base and the 32xLR/1mb reference (3e-4 x 320 = 0.096), isolating the norm
# scope effect: GAE advantages are standardized once over the full rollout
# batch before minibatching, per-minibatch restandardization stays off.
...
class Args:
    ...
    """the PPO default base LR; one Adam covers actor and critic"""
    num_envs: int = 1
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; base LR; 32 minibatches; file-level advantage normalization")
