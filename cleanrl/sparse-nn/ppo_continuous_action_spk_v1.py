# PPO + Sparse-K Perceptrons v1 (spk).
#
# METHOD. Fixed fan-in K hard connections per unit (not soft routing). Separate
# actor/critic SiLU trunks; unimodal Beta policy (α,β = 1+softplus). Connectivity
# is either previous-layer only or all-prior (prefix concat). Optional edge
# rewiring on env episode end: SET (mag drop + random regrow) or threshold
# (rewire slots with utility < X). Connectivity frozen during PPO updates.
#
# Density: K matched to dense-64 fan-in; N=256 so prev-layer is incomplete.
# Unit of rewiring = connections only.
#
# META REWIRE (rewire=meta). IDBD-flavored learning-usefulness without IDBD
# step-sizes: per edge, h tracks recent (-g), s = (-g)·h (grad agreement),
# u = EMA(s). On episode end, among age≥T mature edges, rewire bottom-q% by u.
# No soft gate. New edges reset w,h,u,age + Adam state.
#
# HYPOTHESIS. Gradient-agreement utility + age-protected rewiring finds better
# topologies than magnitude SET / absolute thresholds under PPO.
import math
import os
import random
import time
...
class SparseKLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B, in] -> y: [B, out]
        gathered = x[:, self.indices]  # [B, out, K]
        contrib = gathered * self.weight
        if self.rewire_mode == "learned" and not self.fully_dense:
            ...
            contrib = contrib * gate
        y = contrib.sum(dim=-1) + self.bias
        if self._track_utility and self.training:
            with torch.no_grad():
                pre = gathered.detach().abs().mean(dim=0)  # [out, K]
                u = pre * self.weight.detach().abs()
...
if __name__ == "__main__":
    ...
    assert args.rewire in ("none", "set", "thresh", "learned", "meta"), (
        f"rewire must be none|set|thresh|learned|meta, got {args.rewire}"
    )
        ...
        with torch.no_grad():
            ...
            for trunk in (raw_agent().actor_trunk, raw_agent().critic_trunk):
                for layer in trunk.sparse_layers():
                    abs_w.append(layer.weight.detach().abs().mean().item())
