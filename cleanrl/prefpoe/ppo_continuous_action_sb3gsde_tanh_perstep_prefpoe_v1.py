# PPO + SB3 gSDE per-step + Tanh MLP + PrefPoE v1
#
# Hypothesis:
...
class Args:
    ...
    prefpoe_alpha_entropy: float = 0.1
    """α in paper eq (2). Entropy regularization on the preference head.
       For Gaussians, ∂H/∂σ = 1/σ → self-correcting against collapse."""
    ...
    pref_log_std_min: float = -3.0
    """Numerical floor on pref log std (σ_pref ≥ exp(-3) ≈ 0.05). Prevents
       σ_pref → 0 making 1/σ²_pref blow up the fused PoE precision."""
    pref_log_std_max: float = 2.0
    """Numerical ceiling on pref log std (mild — paper-faithful Gaussian entropy reg
       does the real work, this is just safety)."""
    pref_log_std_init: float = -0.5
    """Initial bias for the pref_log_std head. With Tanh-MLP latents the gSDE
       baseline starts at σ_main ≈ 0.6 (sum-of-latent² · exp(-4) under default
       gsde_log_std_init=-2). Initializing pref at σ_pref ≈ exp(-0.5) = 0.61
       matches main's scale — otherwise fused is dominated by the sharper of
       the two heads and behavior at step 0 is biased toward pref's random init."""
...
class Agent(nn.Module):
    def __init__(
        ...
        self.pref_log_std = layer_init(nn.Linear(latent_dim, action_dim), std=0.01)
        with torch.no_grad():
            self.pref_log_std.bias.fill_(pref_log_std_init)
    ...
    def _pref_distribution(self, latent_sde):
        ...
        pref_log_std = self.pref_log_std(latent_sde).clamp(self.pref_log_std_min, self.pref_log_std_max)
        return Normal(pref_mu, pref_log_std.exp())
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with torch.no_grad():
                    ...
                    pref_log_std_raw = agent.pref_log_std(agent.actor_latent(b_obs[mb_inds]))
                    pref_log_std_mean_log.append(pref_log_std_raw.mean().item())
                    pref_log_std_min_log.append(pref_log_std_raw.min().item())
                    pref_log_std_max_log.append(pref_log_std_raw.max().item())
