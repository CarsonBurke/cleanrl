# PrefPoE (Preference-of-Experts) on vanilla PPO — v2: rebalanced L_pref.
#
# v1 → v2 changes (diagnosis: pref MLE term dominated PPO clip by 100-1000×):
#   - β (pref_loss_coef): 1.0 → 0.05   (~20× smaller so |L_pref| ~ |PPO loss|)
#   - α (pref_ent_coef):  0.01 → 0.05  (5× stronger anti-collapse on π_pref)
#   - pref_std_min:       1e-3 → 0.10  (prevent PoE precision dominating fusion;
#                                       PoE: σ_pref << σ_main ⇒ fused ≈ pref)
# Everything else (architecture, fusion math, losses) matches v1 / vis.py.
#
...
class Args:
    ...
    pref_std_min: float = 0.10
    pref_std_max: float = 2.0
    fused_std_min: float = 1e-3
    fused_std_max: float = 2.0
    pref_loss_coef: float = 0.05  # β: advantage-weighted MLE on π_pref
    pref_ent_coef: float = 0.05   # α: entropy bonus on π_pref
