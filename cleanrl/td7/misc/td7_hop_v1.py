# TD7-HOP v1 — hindsight-privileged teacher + distillation student on the TD7 substrate.
# =====================================================================================
...
class Args:
    ...
    policy_freq: int = 2
    """the frequency of the policy-side updates (teacher + student [+ actor], delayed)"""
    lap_alpha: float = 0.4
...
class TD7HopAgent:
    def __init__(self, state_dim, action_dim, max_action, args: Args, device, writer: SummaryWriter):
        ...
        self.kl_count = 0
        self.hop_stats = {}               # last policy-update telemetry
    ...
    def train(self):
        ...
        # ---------- policy side (delayed): teacher -> student [-> actor] ----------
        if self.training_steps % self.args.policy_freq == 0:
            self._policy_update(state, fixed_zs)
        ...
        if self.training_steps % 500 == 0:
            ...
            for tag, val in self.hop_stats.items():
                self.writer.add_scalar(f"debug/{tag}", val, self.training_steps)

    def _policy_update(self, lap_state, lap_fixed_zs):
        args = self.args
        ...
        # ---- explore-only: TD7's own actor update, byte-preserved, on the LAP minibatch ----
        if args.explore_only:
