...
#
# TD7 DELIGHTFUL PG v1 — stochastic score-function DG on TD7's proven learner.
#
# Retains SALE representations, twin Q, LAP replay, target clipping, hard
# targets, checkpointing, and the successful soft-Gaussian Bellman/entropy
# system. The actor's pathwise -Q term is replaced by DG: at each replay state,
# compare one fresh policy action against an independent same-state policy
# action, then apply the detached sigmoid(advantage * clipped surprisal) gate to
# the fixed-action Gaussian score. The SAC entropy term remains solely as a
# stochastic-policy stabilizer, analogous to the reference KL in LLM training.
#
# Hypothesis: TD7's robust critic prevents the Q exploitation and covariance
# collapse seen in the standalone DG critic while preserving an LLM-transferable
# stochastic score update. No importance ratio or pathwise Q gradient is used.
import copy
import os
import random
import time
...
class Args:
    ...
    """signed continuous-density surprisal clip"""
    max_actor_grad_norm: float = 0.5
    """clip the DG plus entropy actor gradient before its optimizer step"""
    soft_bellman: bool = True
    """use the SAC soft Bellman backup (entropy in the target, fresh current-actor a', no target
    actor/smoothing); False = baseline TD7 hard target (target actor mean + 0.2 smoothing, no
    entropy), i.e. the sibling entropy-in-actor-loss-only regime with this file's squashed actor"""
    target_sigma: float = 0.1
    """action-space std whose Gaussian differential entropy sets the entropy target (budget)"""
    alpha: float = 0.2
...
# Squashed-Gaussian policy log-std bounds. Actions live in [-1, 1], so the upper bound is 0
# (sigma <= 1) rather than SAC's 2; the lower bound matches SAC (sigma >= ~6.7e-3).
LOG_STD_MIN = -5
LOG_STD_MAX = 0
# Sanity bound for logpi entering the soft BACKUP (the tanh Jacobian can spike near |a| -> 1).
LOGPI_CLAMP = 50.0
...
def delightful_gate(advantages, action_logprobs, eta=1.0, surprisal_clip=10.0):
    if eta <= 0:
        raise ValueError("DG eta must be positive")
    if surprisal_clip <= 0:
...
class Actor(nn.Module):
    """TD7-conditioned squashed Gaussian policy (SAC parameterization on the SALE zs embedding)."""
    ...
    def __init__(self, state_dim, action_dim, zs_dim=256, hdim=256):
        ...
        self.logstd_head = nn.Linear(hdim, action_dim)
    ...
    def forward(self, state, zs):
        ...
        mean = self.mean_head(a)
        log_std = torch.tanh(self.logstd_head(a))
        log_std = LOG_STD_MIN + 0.5 * (LOG_STD_MAX - LOG_STD_MIN) * (log_std + 1)
        return mean, log_std

    @staticmethod
    def _squashed_log_prob(normal, raw_action):
        action = torch.tanh(raw_action)
        log_prob = normal.log_prob(raw_action) - torch.log(1.0 - action.pow(2) + 1e-6)
        return log_prob.sum(1, keepdim=True)

    def sample(self, state, zs):
        """Reparameterized sample used only by the soft Bellman and entropy terms."""
        mean, log_std = self(state, zs)
        normal = torch.distributions.Normal(mean, log_std.exp())
        raw_action = normal.rsample()
        action = torch.tanh(raw_action)
        return action, self._squashed_log_prob(normal, raw_action)

    def sample_score(self, state, zs):
        """Non-reparameterized action and latent for the fixed-action DG score."""
        mean, log_std = self(state, zs)
        normal = torch.distributions.Normal(mean, log_std.exp())
        raw_action = normal.sample()
        action = torch.tanh(raw_action)
        return action, raw_action, self._squashed_log_prob(normal, raw_action)

    def log_prob_from_raw(self, state, zs, raw_action):
        mean, log_std = self(state, zs)
        normal = torch.distributions.Normal(mean, log_std.exp())
        return self._squashed_log_prob(normal, raw_action)

    def deterministic(self, state, zs):
        mean, _ = self(state, zs)
...
class TD7Agent:
    def __init__(self, state_dim, action_dim, max_action, args: Args, device, writer: SummaryWriter):
        ...
        # the action-space differential entropy of a Gaussian with std target_sigma.
        if args.alpha_autotune:
            self.target_entropy = action_dim * float(np.log(args.target_sigma) + 0.5 * np.log(2 * np.pi * np.e))
    ...
    def select_action(self, state, use_checkpoint=False, use_exploration=True):
        with torch.no_grad():
            ...
            if use_checkpoint:
                ...
                if use_exploration:
                    action, _ = self.checkpoint_actor.sample(state, zs)
                else:
                    action = self.checkpoint_actor.deterministic(state, zs)
            else:
                zs = self.fixed_encoder.zs(state)
                if use_exploration:
                    # Squashed-Gaussian behavior sample (a = tanh(mu + sigma*eps)), natively bounded.
                    action, _ = self.actor.sample(state, zs)
                else:
                    # Deterministic acting (eval / checkpoint): the squashed mean tanh(mu).
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            if self.args.soft_bellman:
                ...
                # smoothing noise — entropy provides the smoothing). Condition on the same SALE
                # target embedding the critic target reads (SALE encoders untouched).
                next_action, next_logpi = self.actor.sample(next_state, fixed_target_zs)
                target_logpi = next_logpi.clamp(-LOGPI_CLAMP, LOGPI_CLAMP)
            else:
                # Baseline TD7 hard target: target actor's deterministic (squashed) mean + clipped
                # smoothing noise; no entropy in the backup (sibling entropy-in-actor-loss regime).
        ...
        if self.training_steps % self.args.policy_freq == 0:
            with torch.no_grad():
                actor_action, actor_raw_action, actor_action_logpi = self.actor.sample_score(state, fixed_zs)
                actor_fixed_zsa = self.fixed_encoder.zsa(fixed_zs, actor_action)
                actor_q = self.critic(state, actor_action, actor_fixed_zsa, fixed_zs).min(1, keepdim=True)[0]

                baseline_action, _, _ = self.actor.sample_score(state, fixed_zs)
                baseline_fixed_zsa = self.fixed_encoder.zsa(fixed_zs, baseline_action)
                baseline_q = self.critic(state, baseline_action, baseline_fixed_zsa, fixed_zs).min(1, keepdim=True)[0]
                advantages = actor_q - baseline_q
                gate, surprisal, delight = delightful_gate(
                    advantages,
                    actor_action_logpi,
                    eta=self.args.dg_eta,
                    surprisal_clip=self.args.dg_surprisal_clip,
                )

            # The fixed latent action makes this a pure score-function gradient:
            # Q, SALE, the baseline draw, advantage, and the gate are detached.
            score_logpi = self.actor.log_prob_from_raw(state, fixed_zs, actor_raw_action)
            dg_loss = -(gate.detach() * advantages.detach() * score_logpi).mean()

            # Keep SoftGauss's reparameterized entropy term only to stabilize the
            # stochastic policy. There is no pathwise Q term.
            _, entropy_logpi = self.actor.sample(state, fixed_zs)
            actor_loss = dg_loss + self.alpha * entropy_logpi.mean()

            self.actor_optimizer.zero_grad()
            actor_loss.backward()
            actor_grad_norm = nn.utils.clip_grad_norm_(self.actor.parameters(), self.args.max_actor_grad_norm)
            self.actor_optimizer.step()

            # autotune alpha toward the target entropy (holds average sigma near target_sigma)
            if self.args.alpha_autotune:
                with torch.no_grad():
                    _, logpi_detached = self.actor.sample(state, fixed_zs)
                alpha_loss = (-self.log_alpha.exp() * (self.target_entropy + logpi_detached)).mean()
            ...
            if self.training_steps % 500 == 0:
                ...
                self.writer.add_scalar("losses/dg_actor_loss", dg_loss.item(), self.training_steps)
                self.writer.add_scalar("losses/actor_grad_norm", actor_grad_norm.item(), self.training_steps)
                self.writer.add_scalar("advantage/mean", advantages.mean().item(), self.training_steps)
                self.writer.add_scalar("advantage/std", advantages.std(unbiased=False).item(), self.training_steps)
                self.writer.add_scalar("dg/gate_mean", gate.mean().item(), self.training_steps)
                self.writer.add_scalar("dg/surprisal_mean", surprisal.mean().item(), self.training_steps)
                self.writer.add_scalar("dg/delight_mean", delight.mean().item(), self.training_steps)
            if self.training_steps % 1000 == 0:
                with torch.no_grad():
                    ent_proxy = -entropy_logpi.mean()
                ...
                if target_logpi is not None:
                    self.writer.add_scalar("charts/target_logpi_mean", target_logpi.mean().item(), self.training_steps)
                    self.writer.add_scalar("charts/target_logpi_max", target_logpi.max().item(), self.training_steps)
