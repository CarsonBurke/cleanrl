...
#
# BETANOISE v4 — stretched Beta support (the boundary loophole) on top of v3's mode-greedy head.
#
# Base: td7_betanoise_v3 (mode-greedy deterministic action for the v215 unimodal Beta head; see that
# file and v1). v4 keeps greedy_mode=True and adds ONE new mechanism: a STRETCHED action support.
#
# Why (hypothesis 2, the boundary loophole): the Gaussian sdnoise variant wins the late game partly
# because its unclamped mean mu can exceed +-1. Post-clamp, the executed action is deterministically
# pinned at the bound, but the entropy budget is charged on the PRE-clamp Gaussian — so a dim that
# wants to saturate gets free precision exactly where HalfCheetah saturates (~26% of dims) without
# paying concentration. A Beta on native (-1,1) support has no such loophole: to sharpen against a
# boundary it must pay real concentration, which the sigma=0.1 entropy budget caps. v4 gives the Beta
# the same loophole: map z through a stretched affine a_raw = 2*stretch*z - stretch (support
# (-stretch, stretch)) and clamp back to [-1, 1] for anything the env or the value target sees. A
# boundary-seeking dim can then park its MODE past +-1 and saturate with finite concentration, while
# entropy and log-prob stay measured on the unclamped stretched variable. The stretch is a pure
# affine reparam of z, so it adds only a constant ln(stretch) Jacobian: gradients, the z-space Beta
# entropy, the entropy target, and alpha autotune are all unchanged (both sides stay z-space).
#
# Where the stretch clamps vs stays open (mirrors sdnoise's clamped-env / unclamped-critic split):
#   - Behavior + all DETERMINISTIC actions (eval, checkpoint, and the target-actor action in the
#     backup): a = a_raw.clamp(-1, 1). The env and the value target only ever see in-bounds actions,
#     identical to baseline TD7 — the loophole never corrupts the backup.
#   - Actor-update action into the critic: a_r = 2*stretch*z_r - stretch UNCLAMPED (|a_r| <= stretch,
#     mild extrapolation), mirroring sdnoise feeding its unclamped a_r to keep the reparam gradient
#     alive past the bound so a saturating dim keeps receiving a boundary-ward gradient.
#   - Replay buffer stores the CLAMPED executed action (what the env got); TD7 never recomputes
#     behavior log-probs, so no z or stretched action needs to be replayed.
#
# action_stretch=1.0 recovers v3 exactly at every site. Diagnostic charts/frac_pinned measures the
# fraction of deterministic dims with |a_raw| > 1 — how hard the loophole is actually used.
import copy
import os
import random
import time
...
class Args:
    ...
    """deterministic action = Beta MODE (v3) instead of the Beta mean (v1); used at eval/checkpoint
    acting and the target actor's action in the critic backup. --no-greedy-mode recovers v1 exactly"""
    action_stretch: float = 1.1
    """stretched Beta support (v4): z in (0,1) maps to a_raw in (-stretch, stretch); env and
    deterministic/backup actions are clamped to [-1,1], the actor-update action stays unclamped so a
    boundary-seeking dim can saturate with finite concentration. 1.0 recovers v3 exactly"""
...
def beta_mode(conc_a, conc_b):
    """Mode of a unimodal Beta (both concentrations >= 1), in the Beta's own (0,1) support.

    mode = (conc_a - 1) / (conc_a + conc_b - 2). The denominator vanishes only for the uniform
    Beta(1,1); a small-denominator guard falls back to 0.5 there (torch.where evaluates both
    branches, so the division is also clamped to avoid a non-finite value flowing into the unused
    branch). Clamped to (1e-6, 1-1e-6) for parity with how the mean was consumed downstream."""
    denom = conc_a + conc_b - 2.0
    mode = torch.where(denom > 1e-6, (conc_a - 1.0) / denom.clamp(min=1e-6), torch.full_like(conc_a, 0.5))
    return mode.clamp(1e-6, 1 - 1e-6)


def stretch_action(z, stretch):
    """Stretched affine map z in (0,1) -> a_raw in (-stretch, stretch) (v4 boundary loophole).

    a_raw = 2*stretch*z - stretch = stretch*(2z - 1). With stretch=1.0 this is the identity map
    2z-1 used in v1/v3. Callers clamp a_raw to [-1,1] for anything the env or value target sees, and
    leave it unclamped for the actor-update action so boundary-seeking dims keep a live gradient."""
    return stretch * (2.0 * z - 1.0)
...
class TD7Agent:
    ...
    def select_action(self, state, use_checkpoint=False, use_exploration=True):
        with torch.no_grad():
            ...
            if self.args.beta_noise:
                # out_a, out_b are the Beta concentrations.
                if use_exploration:
                    z = torch.distributions.Beta(out_a, out_b).sample().clamp(1e-6, 1 - 1e-6)
                elif self.args.greedy_mode:
                    # Deterministic acting (eval / checkpoint): Beta MODE (v3).
                    z = beta_mode(out_a, out_b)
                else:
                    # v1 behavior: Beta mean.
                    z = out_a / (out_a + out_b)
                # v4: stretched support, then clamp to bounds — the env only ever sees in-bounds
                # actions (stretch=1.0 -> a_raw already in (-1,1), clamp is a no-op == v3).
                action = stretch_action(z, self.args.action_stretch).clamp(-1.0, 1.0)
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            if self.args.beta_noise:
                # v3: bootstrap the Beta MODE (greedy_mode), not the mean, avoiding undervaluation of
                # boundary-saturating dims. v4: map through the stretch and clamp to bounds BEFORE
                # smoothing so the value target only ever sees in-bounds actions (same as baseline);
                # the loophole is confined to the actor-update path and never corrupts the backup.
                if self.args.greedy_mode:
                    z_det = beta_mode(t_a, t_b)
                else:
                    z_det = t_a / (t_a + t_b)
                next_mean = stretch_action(z_det, self.args.action_stretch).clamp(-1.0, 1.0)
            else:
                next_mean = t_a
        ...
        if self.training_steps % self.args.policy_freq == 0:
            ...
            if self.args.beta_noise:
                ...
                # v4: UNCLAMPED stretched action into the critic (|a_r| <= stretch), mirroring
                # sdnoise's unclamped a_r so a saturating dim keeps a boundary-ward reparam gradient.
                actor_action = stretch_action(z_r, self.args.action_stretch)
            else:
                actor_action = out_a
            ...
            if self.args.beta_noise and self.training_steps % 1000 == 0:
                with torch.no_grad():
                    ...
                    # Bias v3 removes: |a_mode - a_mean| = 2*|mode - mean| in action space.
                    mode_action = 2.0 * beta_mode(out_a, out_b) - 1.0
                    mode_mean_gap = (mode_action - det_action).abs().mean()
                    # v4 loophole usage: fraction of deterministic dims whose stretched a_raw parks
                    # past the bound (|a_raw| > 1), i.e. saturating dims exploiting the stretch.
                    z_det = beta_mode(out_a, out_b) if self.args.greedy_mode else out_a / (out_a + out_b)
                    a_raw = stretch_action(z_det, self.args.action_stretch)
                    frac_pinned = (a_raw.abs() > 1.0).float().mean()
                ...
                self.writer.add_scalar("charts/mode_mean_gap", mode_mean_gap.item(), self.training_steps)
                self.writer.add_scalar("charts/frac_pinned", frac_pinned.item(), self.training_steps)
