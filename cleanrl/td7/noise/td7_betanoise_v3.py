...
#
# BETANOISE v3 — mode-greedy deterministic action for the unimodal Beta actor head on TD7.
#
# Base: this is td7_betanoise_v1 (v215-style unimodal Beta exploration head; see that file) with ONE
# change — the DETERMINISTIC action is the Beta MODE instead of the Beta mean, at every site the
# deterministic action is consumed: eval/checkpoint acting AND the target actor's action in the
# critic backup. Exploration sampling, the actor loss, and the entropy/autotune path are unchanged.
#
# Why: betanoise_v1 matches/beats the Gaussian sdnoise variant until ~300k then falls behind (15414
# vs 16178 @500k on HalfCheetah). The gap opens as the policy sharpens: ~26% of action dims sit near
# the bounds, and a boundary-skewed Beta's MEAN is systematically less extreme than its MODE — e.g.
# Beta(30,2) has mean->a=0.875 but mode->a=0.933. Because the TD backup bootstraps Q(s',
# deterministic_action), a mean-greedy deterministic point UNDERVALUES saturating dims, and that bias
# GROWS as concentrations rise — matching the late divergence. The Beta mode is the mass peak (the
# actual "best guess" action), so it is the correct greedy point for a bounded distribution the way
# the Gaussian mean is for an unbounded one.
#
# mode = (conc_a - 1) / (conc_a + conc_b - 2), valid for the unimodal 1+softplus parameterization
# (both concentrations >= 1). A small-denominator guard (torch.where on conc_a+conc_b-2 > 1e-6, else
# 0.5) covers the near-uniform early-training regime; the mode is clamped to (1e-6, 1-1e-6) then
# mapped a = 2*mode - 1, exactly as the mean was. --no-greedy-mode recovers v1 behavior exactly
# (mean-greedy), a clean A/B.
#
# Registered prediction: v3 tracks v1 through ~300k and closes most of the 400-500k gap vs sdnoise.
# If it does not, the residual is the entropy-budget boundary loophole (a Gaussian gets free
# post-clamp precision at |a|=1 that a bounded Beta cannot), and the fix is boundary handling, not
# the greedy point. Diagnostic charts/mode_mean_gap measures |a_mode - a_mean| directly: it should
# start ~0 (near-uniform Beta) and grow with saturation, tracking the bias this change removes.
import copy
import os
import random
import time
...
class TD7Agent:
        ...
        with torch.no_grad():
            ...
            if self.args.beta_noise:
                ...
                if use_exploration:
                    ...
                    # Deterministic acting (eval / checkpoint): Beta MODE mapped to (-1,1) (v3).
                    z = beta_mode(out_a, out_b)
                else:
                    # v1 behavior: Beta mean mapped to (-1,1).
                    z = out_a / (out_a + out_b)
                action = 2.0 * z - 1.0
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            if self.args.beta_noise:
                # v3: the deterministic point that gets bootstrapped is the Beta MODE (greedy_mode),
                # not the mean; mode-greedy avoids undervaluing boundary-saturating action dims.
                if self.args.greedy_mode:
                    next_mean = 2.0 * beta_mode(t_a, t_b) - 1.0
                else:
        ...
        if self.training_steps % self.args.policy_freq == 0:
            ...
            if self.args.beta_noise and self.training_steps % 1000 == 0:
                with torch.no_grad():
                    ...
                    # Bias this change removes: |a_mode - a_mean| = 2*|mode - mean| in action space.
