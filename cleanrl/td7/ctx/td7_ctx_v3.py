...
class HistoryTransformer(nn.Module):
    ...
    def decode(self, query, positions, K_cache, V_cache, n_written):
        ...
        for li, layer in enumerate(self.layers):
            ...
            res = self._attend(q, positions, K_cache[li], V_cache[li], n_written, lo_off=1, need_ent=last)
            attn_out, ent = res if last else (res, ent)
...
class TD7Agent:
    ...
    def train(self):
        ...
        if self.training_steps % self.args.target_update_rate == 0:
            ...
            # snapshot the token cache, then EXACTLY rebuild both KV caches so body_target + frozen cache
            # are a consistent pair for the next 250 steps. Time the rebuild for telemetry.
