...
class Args:
    ...
    # --- WPPG (arXiv 2603.02576) transfer: Wasserstein-proximal actor. sd_noise only. ---
    # All default to current behavior, so existing variants trace byte-identically.
    #
    # WHY: with a single inner step TD7's actor update and WPPG's direction
    # matching are the SAME gradient (WPPG's Delta is identically zero at the
    # evaluation point, leaving -2*eta*(da/dtheta)^T grad_a Q against TD7's
    # -(da/dtheta)^T grad_a Q; Adam erases the scale). So importing WPPG's actor
    # rule verbatim is a no-op. The part that does NOT collapse is the proximal
    # term itself -- but only measured against a LAGGED policy, since against the
    # current policy it is zero with zero gradient. That gives a per-state bound
    # on how far the action distribution may move, which Adam's per-parameter
    # normalization structurally cannot provide (cf. why PPO's ratio clip is not
    # redundant with Adam).
    #
    # SDNoiseActor is a diagonal Gaussian in (mean, log_std) before its clamp, so
    # W2^2 is CLOSED FORM: ||mu-mu_old||^2 + ||sigma-sigma_old||^2. That is exact
    # rather than the paper's sampled approximation, needs no autograd.grad, and
    # stays inside torch.compile(fullgraph=True). It is also computed pre-clamp,
    # which matters: the actor's .clamp(-1,1) zeroes the gradient in saturated
    # coordinates, exactly where a trust region most needs to act.
    wppg_trust_region: bool = False
    """add a closed-form W2^2 proximal penalty against a lagged policy snapshot (sd_noise only)"""
    wppg_tr_coef: float = 1.0
    """coefficient on the W2^2 penalty, i.e. 1/(2*eta) in the paper's proximal step"""
    wppg_tr_tau: float = 0.01
    """Polyak rate for the lagged policy snapshot; smaller = longer trust-region horizon"""
    wppg_action_samples: int = 1
    """K actions per state transported by the actor update (paper uses 32); 1 = current behavior"""
    wppg_actor_min_q: bool = False
    """use the per-sample min over twin critics in the actor objective instead of their mean"""
...
class ActorLossCore(nn.Module):
    ...
    def __init__(
        ...
        # WPPG transfer. Plain Python attrs => Dynamo specializes on them at trace
        # time, so the default (all off) traces to byte-identical code.
        self.wppg_k = int(args.wppg_action_samples)
        self.wppg_min_q = args.wppg_actor_min_q
        self.wppg_trust_region = args.wppg_trust_region
        self.wppg_tr_coef = args.wppg_tr_coef
        if self.wppg_trust_region:
            # Registered submodule, not built per call, so Dynamo does not retrace.
            self.actor_old = copy.deepcopy(actor).requires_grad_(False)
        if endpoint_sampler is not None:
            self.register_buffer("control_mix", control_mix)

    @torch.no_grad()
    def update_policy_snapshot(self, tau):
        """Polyak the lagged policy the trust region is measured against."""
        for p, op in zip(self.actor.parameters(), self.actor_old.parameters()):
            op.mul_(1.0 - tau).add_(p, alpha=tau)
        for b, ob in zip(self.actor.buffers(), self.actor_old.buffers()):
            ob.copy_(b)
    ...
    def forward(self, state, fixed_control_zs, policy_noise, alpha):
        ...
        if self.sac_policy:
            ...
            if self.wppg_k > 1 or self.wppg_trust_region:
                # sample_additive inlined so (mean, log_std) are available for the
                # closed-form W2 term without a second policy forward. Identical ops.
                mean, log_std = self.actor.policy_stats(state, fixed_control_zs)
                if self.wppg_trust_region:
                    with torch.no_grad():
                        mean_old, log_std_old = self.actor_old.policy_stats(
                            state, fixed_control_zs
                        )
                    # Closed-form W2^2 between diagonal Gaussians, PRE-clamp.
                    w2_penalty = (
                        (mean - mean_old).pow(2).sum(dim=1)
                        + (log_std.exp() - log_std_old.exp()).pow(2).sum(dim=1)
                    ).mean()
                log_pi = log_std.sum(dim=1, keepdim=True)
                if self.wppg_k > 1:
                    # Transport K actions per state instead of one. zsa depends on
                    # the action so it is recomputed per sample; zs does not.
                    k = self.wppg_k
                    mean = mean.repeat_interleave(k, dim=0)
                    log_std = log_std.repeat_interleave(k, dim=0)
                    state = state.repeat_interleave(k, dim=0)
                    fixed_control_zs = fixed_control_zs.repeat_interleave(k, dim=0)
                actor_action = (mean + log_std.exp() * policy_noise).clamp(-1, 1)
            else:
        ...
        if self.sac_policy:
            ...
            # min over the twin critics is the paper's (and TD3's) pessimism;
            # TD7 stock averages them. Behind wppg_actor_min_q so the default is unchanged.
            q_term = q_values.min(dim=1)[0].mean() if self.wppg_min_q else q_values.mean()
            actor_loss = -q_term - alpha * log_pi.mean()
            if w2_penalty is not None:
                # Q here is unnormalized -- O(1e3) on HalfCheetah at convergence --
                # while W2^2 between policies lagged 1/wppg_tr_tau updates is O(1e-2).
                # A raw coefficient would make the trust region a rounding error and
                # the experiment would silently test nothing. Scaling by detached
                # |Q| makes wppg_tr_coef dimensionless: the fraction of current Q
                # value that one unit of W2^2 is worth.
                q_scale = q_values.detach().abs().mean().clamp_min(1e-3)
                actor_loss = actor_loss + self.wppg_tr_coef * q_scale * w2_penalty
        else:
            actor_loss = -q_values.mean()
        w2_stat = (
            w2_penalty.detach() if w2_penalty is not None else actor_loss.new_zeros(())
        )
        return actor_loss, log_pi.mean(), w2_stat
...
class LeSALEAgent:
    def __init__(self, state_dim, action_dim, max_action, args: Args, device, writer: SummaryWriter):
        ...
        # Uncompiled handle: torch.compile returns an OptimizedModule wrapper, and
        # the snapshot Polyak must run eagerly outside the traced graph.
        self.actor_loss_core_raw = self.actor_loss_core
        ...
        if self.training_steps % self.args.policy_freq == 0:
            ...
            for _ in range(actor_updates):
                if self.args.sac_policy:
                    ...
                    # K independent draws per state when transporting K actions.
                    noise_shape = (
                        action.shape[0] * self.args.wppg_action_samples,
                        action.shape[1],
                    )
                ...
                if self.args.pc_actor:
                    ...
                    actor_loss, actor_log_pi, actor_w2 = self.actor_loss_core(
                    ...
                    if self.args.wppg_trust_region:
                        # Lag the reference policy AFTER the step, so the penalty
                        # measures cumulative drift over ~1/wppg_tr_tau updates
                        # rather than being identically zero.
                        self.actor_loss_core_raw.update_policy_snapshot(
                            self.args.wppg_tr_tau
                        )
            ...
            if self.training_steps % self.args.latent_log_freq == 0:
                ...
                if self.args.wppg_trust_region and actor_w2 is not None:
                    self.writer.add_scalar(
                        "wppg/w2_penalty", actor_w2.item(), self.training_steps
                    )
                if self.args.sac_policy:
                    self.writer.add_scalar(
