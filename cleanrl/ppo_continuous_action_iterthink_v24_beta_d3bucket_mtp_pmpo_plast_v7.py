# PPO + IterThink v24 Beta + d3bucket MTP + BetaPlast v7: v4 + half-strength suppression.
# =====================================================================================
# v7 change over v4: signneg_neg_scale 1.0 -> 0.5, i.e. negative advantages get eff_adv=-0.5
# instead of -1. v4 (magnitude-pos / sign-neg) won the ablation; v7 tilts the pos/neg balance
# further toward reinforcement: bad actions are still suppressed (by sign) but at HALF the
# per-update pressure, so positive-advantage magnitude dominates the gradient. Directly tests
# the "upweight good, downweight bad" thesis on the suppression side (a different lever than
# v6's clip room). Positives keep divide-by-std magnitude; plasticity gate = v4's relative [0.25,4].
#
# --- inherited v4 notes ---
...
class Args:
    ...
    # Magnitude of the negative-advantage suppression signal (eff_adv = -signneg_neg_scale for
    # negatives). 1.0 = full unit sign (v4). <1 weakens suppression so positive reinforcement
    # dominates the update (positive-emphasis via advantage scaling). v7 default 0.5.
    signneg_neg_scale: float = 0.5
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if pmpo_active:
                    ...
                    neg_val = -args.signneg_neg_scale
                    if args.signneg_pos_mode == "ppo":
                        ...
                        eff_adv = torch.where(mb_adv_raw >= 0, pos_mag, torch.full_like(mb_adv_raw, neg_val))
                    elif args.signneg_pos_mode == "sign":
                        # pure sign: positives +1, negatives -neg_scale. NO advnorm.
                        eff_adv = torch.where(mb_adv_raw >= 0, torch.sign(mb_adv_raw), torch.full_like(mb_adv_raw, neg_val))
                    else:
