# JEPA x pre-RMS residual v1: the strongest 50M HalfCheetah base
# (ppo_normres_v2_pre_rms_stiglu_50M: additive pre-RMSNorm SiTU-GLU trunks,
# 32x LR 9.6e-3, full 32k batch, raw GAE advantages) whose actor reads a
# SIGReg-JEPA encoder trained ONLY by stop-target latent prediction.
# Why: the stop-target JEPA gave a tanh actor stationary (0.6%/rollout drift),
# isotropic, dynamics-predictive inputs (+~500 at 8M, +~600 at 2-4M vs its
# matched baseline); this base wins long runs by surviving large policy steps.
# Test whether better actor inputs compound with that step regime at 50M.
# Latents enter as sqrt(64)*unit vectors: unit per-element second moment, the
# trunk's input-projection design point. actor_input=obs is the matched base.
# The critic always reads raw observations. SSL LR anneals with PPO LR.
import os
import random
import time
...
WIDTH = 64
LATENT_SCALE = 8.0  # sqrt(WIDTH): unit directions carry unit per-element RMS
...
class Args:
    ...
    actor_input: Literal["latent", "concat", "obs"] = "latent"
    """latent: JEPA features only; concat: raw observations alongside them; obs: matched base"""
...
class HostPolicy:
    ...
    Mirror outputs are permanent buffers, so the latent is copied into our own
    feature buffer before the actor mirror consumes it.
    """

    def __init__(self, agent, num_rows):
        self.actor = make_host_mirror(agent.actor, num_rows)
        self.encoder = None if agent.actor_input == "obs" else make_host_mirror(agent.encoder, num_rows)
    ...
    def __call__(self, observations):
        ...
        np.multiply(self.latent, np.float32(LATENT_SCALE), out=self.latent)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        actor_dim = {"latent": WIDTH, "concat": self.observation_dim + WIDTH,
                     "obs": self.observation_dim}[args.actor_input]
        self.critic = nn.Sequential(
            make_norm_residual_trunk(self.observation_dim, WIDTH, placement="pre",
                                     norm_kind="rms", activation="stiglu"),
            layer_init(nn.Linear(WIDTH, 1), std=1.0),
        )
        self.actor = nn.Sequential(
            make_norm_residual_trunk(actor_dim, WIDTH, placement="pre",
                                     norm_kind="rms", activation="stiglu"),
            layer_init(nn.Linear(WIDTH, 2 * self.action_dim), std=0.01),
        )
    ...
    def actor_features(self, x):
        ...
        if self.actor_input == "obs":
            return x
        latent = LATENT_SCALE * F.normalize(self.encoder(x).detach(), p=2, dim=-1)
...
def validate_args(args):
    ...
    if args.actor_input not in {"latent", "concat", "obs"}:
...
def main():
    ...
    try:
        ...
        actor_reads = ("raw observations" if args.actor_input == "obs" else
                       f"detached sqrt(64)*unit SIGReg-JEPA latent ({args.actor_input})")
        writer.add_text("architecture", f"pre-RMSNorm SiTU-GLU residual actor on {actor_reads}; "
                        "same trunk critic on raw observations")
        ...
        train_ssl = args.actor_input != "obs"
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        # the obs control never reads the encoder, so it skips SSL.
                        for ssl_indices in indices.split(args.ssl_minibatch_size) if train_ssl else ():
                            if args.compile:
                                torch.compiler.cudagraph_mark_step_begin()
