# MaxRL-PPO v1: maximum-likelihood reweighting of the policy gradient.
#
# MaxRL (arXiv 2602.02710) shows expected-reward RL is only the FIRST-ORDER term of
# maximum likelihood: log p = -sum_k fail@k / k, so grad log p = sum_k (1/k) grad pass@k
# and REINFORCE keeps k=1 alone. Truncating at order T gives the BOUNDED weight
#     w_T(p) = (1-(1-p)^T)/p = sum_{k=1..T} (1-p)^{k-1}   in [1, T],
# w(1)=1, w(0)=T. T=1 is PPO, T->inf is exact ML. The paper's Alg. 1 is one line:
# advantage (r-rbar)/rbar, where rbar is the mean over N rollouts of the same prompt.
#
# Port to dense-reward control. MuJoCo cannot re-roll from a state, so there is no
# group to average; the CRITIC is the amortised replacement for rbar. Returns are also
# signed, so we cannot divide by a mean return -- we need a genuine p in (0,1].
# Key idea: recover a binary success event from the dense return.
#   success_t = 1{Ghat_t > tau},  Ghat_t = Ahat_t + V(s_t)   (the GAE return target)
#   tau       = EMA of the q-quantile of Ghat over the batch -- an absolute bar that
#               ratchets upward as the policy improves, i.e. a self-generated verifier.
#   phat(s)   = sigmoid(pass_head(s)), trained by BCE on success_t. BCE on a success
#               indicator IS maximum likelihood on that event, so this is the paper's
#               objective rather than a proxy for it.
# Two mechanisms, selected by --maxrl-mode:
#   weight: A = (w_T(phat)/mean w) * A_GAE. Pure reallocation -- PPO applies w=1
#           uniformly; MaxRL spends capacity where good outcomes are rare.
#   tail:   A = A_GAE + beta * scaled(w_T(phat) * (success - phat)). The added term is
#           exactly Alg. 1's advantage with the truncated weight and the critic in
#           place of the group mean. Failures push down by at most 1, rare successes
#           push up by up to T: a tail-seeking gradient on top of PPO's mean-seeking one.
#   both:   product of the two. off: bit-identical control (pass head gets no gradient).
#
# Hypothesis: PPO's uniform w=1 under-weights states where above-bar outcomes are rare,
# which is exactly where exploration signal lives. MaxRL's w_T reallocates the gradient
# there and, per the paper's Fig. 14, keeps the outcome distribution broad instead of
# sharpening it. Note T here is a pure weight-SHAPE knob: the paper couples larger T to
# more rollouts, whose variance reduction (Fig. 28) we do not get, so keep T moderate.
# The pass head reads a DETACHED trunk by default so any gain is attributable to the
# reweighting and not to auxiliary-task representation learning in the critic.
import math
import os
import random
import time
...
class Args:
    ...
    maxrl_mode: Literal["off", "weight", "tail", "both"] = "both"
    """off reproduces baseline PPO exactly; weight rescales A_GAE; tail adds Alg. 1's advantage"""
    maxrl_order: int = 8
    """truncation order T of the Maclaurin expansion; caps the weight at w_T(0)=T"""
    maxrl_beta: float = 0.5
    """mixing coefficient of the tail-likelihood advantage, in units of A_GAE's std"""
    ...
    """read a detached critic trunk, so the BCE loss cannot reshape the value function"""
...
def maxrl_advantages(advantages, returns, pass_logits, bar, args):
    ...
    if args.maxrl_mode in ("weight", "both"):
        ...
        training_advantages = training_advantages * (weight / weight.mean())
    if args.maxrl_mode in ("tail", "both"):
        # Algorithm 1's advantage: centre by the per-state pass rate, then weight.
        tail = weight * (success - probability)
        # Express beta in units of the dense advantage so its meaning is scale-free.
        tail = tail * (advantages.std() / (tail.std() + 1e-8))
        training_advantages = (training_advantages + args.maxrl_beta * tail) / math.sqrt(
            1.0 + args.maxrl_beta ** 2
        )
...
def validate_args(args):
    ...
    if args.maxrl_beta < 0.0 or args.maxrl_pass_coef < 0.0:
        raise ValueError("maxrl_beta and maxrl_pass_coef must be non-negative")
    return args
...
def main():
    ...
    try:
        ...
        writer.add_text("maxrl", f"mode={args.maxrl_mode}; T={args.maxrl_order}; beta={args.maxrl_beta}; "
