# MaxRL v4: the critic as the latent marginal.  Optimise log E[return | s] per state.
#
# Idea.  MaxRL (arXiv 2602.02710) observes that RL's E[p] is the first-order term of
# ML's log E[p], and that the gap matters because a latent generation model reaches an
# outcome through MANY routes: p(y|x) = sum_z m(z|x) I{f(z)=y}.  Credit belongs to that
# marginal, not to any single route, and the ML gradient is the route-average weighted
# by 1/p -- low-probability inputs get amplified.  Appendix M.4 drops the indicator
# entirely: for r >= 0 the objective is log E[r] with advantage (r - mu)/mu, mu the
# Monte-Carlo mean reward for that input.  The paper leaves continuous rewards and
# PPO-style training to future work; this file does both.
#
# The translation.  Take x = a state s and z = the trajectory continuing from it.  Many
# continuations reach the same return, and V(s) = E_{tau|s}[R] IS the marginal over
# those latent routes -- computed by a learned amortiser over all past experience rather
# than by N fresh Monte-Carlo rollouts from a cloned state.  So mu -> V(s) and M.4's
# advantage (r - mu)/mu becomes (G - V(s))/V(s) = A(s,a)/V(s).  Exactly:
#
#     grad log E[R|s] = grad E[R|s] / E[R|s] = E[A(s,a) grad log pi] / V(s)
#
# i.e. the ordinary policy gradient with each state's advantage divided by its value.
# No groups, no state cloning, no binarisation, no Maclaurin truncation.  v1-v3 all
# bolted a group-sampled outcome advantage onto PPO's and lost dose-monotonically; the
# group mean over 8 branches is simply a noisier V(s) than the critic already provides.
#
# Positivity.  log E[R] needs R >= 0.  A constant reward shift c is exactly policy-
# invariant for a non-terminating task under truncation bootstrapping, shifts every
# value by c/(1-gamma), and leaves GAE advantages EXACTLY unchanged (delta_t is
# invariant: the +c cancels the (gamma-1)c/(1-gamma) from the shifted values).  So
# subtracting a running value floor is that reward shift expressed in value space --
# free, and it perturbs nothing else.  (On Hopper/Walker, where episodes terminate, the
# shift is a survival bonus and is no longer policy-invariant.)
#
# The RL <-> ML dial.  w(s) = 1 / (Vshift(s) + lambda * mean(Vshift)), normalised to
# mean 1.  lambda -> infinity recovers uniform weighting (plain PPO); lambda -> 0
# recovers exact 1/V maximum-likelihood weighting.  lambda is the continuous analogue
# of MaxRL's truncation order T.  It holds the denominator at or above lambda *
# mean(Vshift) > 0, so the weight is finite by construction and nothing needs clipping;
# the resulting ratio is 1 + kappa/lambda, where kappa = max(Vshift)/mean(Vshift) is the
# batch's own value dispersion (~2 when values are roughly symmetric).  lambda scales
# the dynamic range but does not bound it alone, so `maxrl/weight_ratio` is logged.
# The shift itself is the batch value minimum, recomputed each iteration and stored
# nowhere, so the dial is the only thing setting the dose.
#
# Advantage normalisation.  Per-minibatch standardisation would re-standardise random
# subsets of the reweighted advantages and partially undo the reweighting, which is why
# it must go.  Standardising ONCE over the full batch before weighting keeps PPO's
# scale conditioning (worth ~1000 return at the v3 config) while leaving every relative
# weight intact, so `adv_norm_scope=batch` replaces `norm_adv`, and the combination of
# a live weight with minibatch scope is rejected outright.
#
# Hypothesis.  Spending more gradient on states the policy currently handles badly is a
# robustness/exploration prior that plain PPO lacks; if it is real, `invert` (the same
# weight multiset, correspondence reversed) must lose to the control while `ml` beats
# it, and `shuffle` (same multiset, correspondence destroyed) must land between them.
import os
import random
import time
...
class Args:
    ...
    """RL<->ML dial: infinity is uniform weighting, 0 is exact 1/V; holds the denominator positive"""
...
def maxrl_weights(values, args):
    """Per-state inverse-value weights: the exact gradient weighting of log E[R | s].

    The floor is the current batch minimum, recomputed every iteration and carried
    nowhere.  Subtracting it is the constant reward shift; it makes ``shifted >= 0``
    exactly, with no clamp, and keeps ``shifted.min() == 0`` so the realised weight
    ratio is the advertised ``1 + kappa/lambda``.  An earlier revision smoothed the
    floor with an EMA for robustness: that made it a ratchet, dropping instantly to any
    outlier batch minimum and recovering over ~100 iterations (a fifth of a run), which
    silently halved the dose and broke scale invariance.  Statelessness is the fix.
    """
    floor = values.min()
    shifted = values - floor
    scale = shifted.mean().clamp_min(1e-6)
    weights = (shifted + args.maxrl_lambda * scale).reciprocal()
    return weights / weights.mean(), floor


def maxrl_controls(weights, values, mode, generator):
    ...
    try:
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                weights, floor = maxrl_weights(b_values, args)
                weights = maxrl_controls(weights, b_values, args.maxrl_mode, weight_generator)
                ...
                centred_r = batch.rewards.flatten() - batch.rewards.mean()
                weight_reward_rho = (centred_w * centred_r).mean() / (
                    centred_w.std(unbiased=False) * centred_r.std(unbiased=False) + 1e-12)
            updates = 0
