# MaxRL v6: allocate EXPLORATION by the 1/p weight, not the gradient.
#
# Five versions applied the paper's weight (grad J = E[w(p) grad p]) to the policy
# gradient across states, and every one lost to a uniform control:
#   v1 p = P(return > threshold | s)      6057 vs 7991    v3 p = group pass fraction  4948 vs 5793
#   v4 p = V(s)                           6266 vs 8466    v5 p = E[(A)_+ | s]         7220 vs 8466
# The controls say this is not an estimator problem.  v5's head genuinely predicts
# improvability (upside_ev ~0.20) and beats a random weighting of identical dispersion
# by ~1000 at every checkpoint, so the signal is real -- and reweighting still costs
# more than the signal is worth.  Across v4 and v5 the damage tracks weight dispersion
# (ratio 4.7 cost ~300 at random; ratio 58 cost ~2300), which is the signature of a
# variance price, not of a wrong p.
#
# Why the gradient was the wrong target.  MaxRL reweights over x ~ rho because in a
# bandit that is the ONLY lever: you cannot choose which prompts to see, so "spend more
# effort where success is rare" can only mean "upweight that term in the sum".  An MDP
# has a lever a bandit does not -- the policy chooses its own data.  So the MDP-native
# reading of the same sentence is not gradient reweighting but EXPLORATION ALLOCATION:
# sample more widely where success is rare.  That leaves the policy gradient estimator
# exactly alone, which is the part that kept paying the variance price.
#
# Mechanism.  v5's head already estimates the reachable improvement p(s) = E[(A)_+|s].
# Here it modulates a per-state entropy coefficient of mean one, so the AVERAGE
# exploration pressure is fixed by ent_coef and only its ALLOCATION across states
# varies.  `ml` follows the paper (1/p: widen where improvement is rare); `invert`
# tests the opposite (widen where improvement is abundant), which v4's controls hint
# may be the right sign in an MDP; `flat` holds the coefficient uniform and is the
# control that isolates allocation from level.  The advantages are never touched.
#
# Hypothesis.  If allocation is the right lever, `ml` or `invert` beats `flat` at equal
# mean entropy pressure.  If both tie with `flat`, exploration allocation is worth
# nothing here and the 1/p weight has no purchase on this domain in any form -- which,
# given five prior versions, is the result that would close the line.
import os
import random
import time
...
class Args:
    ...
    """mean coefficient of the entropy bonus; its ALLOCATION is what maxrl_mode varies"""
    vf_coef: float = 0.5
    ...
    maxrl_mode: str = "ml"
    """flat (uniform entropy coefficient), ml (1/p allocation), shuffle, invert, off (no bonus)"""
...
def maxrl_advantages(advantages, args):
    """v6 does not reweight the policy gradient at all; only the batch-scope
    standardisation that every arm shares survives from v5."""
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns,
             old_values, upside_target, entropy_weight, args):
    ...
    # entropy_weight has mean one over the batch, so ent_coef alone fixes the average
    # exploration pressure and maxrl_mode changes only where it is spent.
    entropy_loss = (entropy * entropy_weight).mean()
    ...
    if args.maxrl_mode in ("off", "flat"):
...
def validate_args(args):
    ...
    if args.maxrl_mode not in {"off", "flat", "ml", "shuffle", "invert"}:
    ...
    try:
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                # Read before the update, trained after it: a state's allocation never
                # sees the realisation of the action taken there.
                if args.maxrl_mode in ("off", "flat"):
                    pass_rate = torch.ones_like(b_advantages)
                    weights = torch.ones_like(b_advantages)
                else:
                    pass_rate = upside_model(b_obs)
                    weights = maxrl_weights(pass_rate, args)
                    weights = maxrl_controls(weights, pass_rate, args.maxrl_mode, weight_generator)
                b_advantages = maxrl_advantages(b_advantages, args)
                ...
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss, metrics = loss_model(
                            ...
                            b_upside_target[indices], weights[indices],
                        )
