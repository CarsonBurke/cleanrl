# MaxRL difficulty weighting v4: the paper's cross-prompt weight w_T(p) on dense PPO.
#
# MaxRL (arXiv 2602.02710) writes every objective as E_x[w(p_x) grad p_x]: RL has w = 1,
# maximum likelihood w = 1/p, and the rollout-truncated MaxRL w_T(p) = (1 - (1-p)^T)/p.
# Its gains come from this weight across prompts, which amplifies hard ones, not from
# changing the direction within a prompt. v1-v3 tried shift-invariant exponential success
# likelihoods. Shift invariance removes any absolute notion of difficulty, so they reduced
# to within-state tail tilting and lost to the control.
#
# Here "success" is absolute and the within-state signal stays dense:
#   success event   G_t >= tau, the lambda-return reaching the batch's q-quantile, so
#                   success means beating the current policy's typical return
#   p(s)            Phi((V(s) - tau) / sigma), with sigma the batch residual spread of
#                   G - V; the existing critic gives the success probability, with no
#                   new head and no lag
#   advantage       w_T(p(s)) / mean(w) * A_GAE; a state-only weight, so the expected
#                   direction within each state is untouched
# grad V(s) plays the role of grad p for dense rewards. States from which beating the
# current policy is unlikely get up to T times the gradient of easy ones. T = 1 is the
# control. Actor and critic gradients are clipped separately (the control; see FAMILY.md).
import math
import os
import random
import time
...
class Args:
    ...
    success_quantile: float = 0.9
    """success = lambda-return above this quantile of the batch's lambda-returns"""
    maxrl_truncation: float = 8.0
    """T in w_T(p) = (1 - (1-p)^T)/p: 1 is RL, infinity is maximum likelihood"""
...
def success_probability(values, returns, quantile):
    """Gaussian success model around the critic: P(G >= tau | s) with tau the batch quantile."""
    tau = torch.quantile(returns, quantile)
    sigma = (returns - values).std()
    return 0.5 * torch.erfc((tau - values) / (sigma * math.sqrt(2.0))), tau, sigma


def truncated_ml_weight(p, truncation):
    """w_T(p) = (1 - (1-p)^T)/p, continuous at p = 0 where it equals T."""
    safe = p.clamp_min(1e-12)
    weight = -torch.expm1(truncation * torch.log1p(-safe.clamp_max(1.0 - 1e-7))) / safe
    return torch.where(p < 1e-12, torch.full_like(p, truncation), weight)


def difficulty_weights(values, returns, args):
    p, tau, sigma = success_probability(values, returns, args.success_quantile)
    weight = truncated_ml_weight(p, args.maxrl_truncation)
    normalized = weight / weight.mean()
    diagnostics = {
        "maxrl/p_mean": p.mean(), "maxrl/p_below_5pct": (p < 0.05).float().mean(),
        "maxrl/tau": tau, "maxrl/sigma": sigma, "maxrl/weight_max": normalized.max(),
        "maxrl/weight_ess_frac": normalized.mean().square() / normalized.square().mean(),
    }
...
def validate_args(args):
    ...
    if not 0.0 < args.success_quantile < 1.0 or args.maxrl_truncation < 1.0:
        raise ValueError("success_quantile must be in (0, 1) and maxrl_truncation >= 1")
    return args
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta; separate actor/critic clipping; MaxRL difficulty weight "
                        f"q={args.success_quantile} T={args.maxrl_truncation}")
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                weights, difficulty = difficulty_weights(b_values, b_returns, args)
                b_advantages = weights * advantages.flatten()
            updates = 0
