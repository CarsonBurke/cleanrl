# MaxRL-CGF v1: maximum-likelihood RL (Tajwar et al. 2026, arXiv 2602.02710) for dense
# rewards, with the critic as the amortized per-state normalizer.
#
# MaxRL: with a binary success event of probability p(x), RL ascends p while maximum
# likelihood ascends log p; grad log p = E[score | success], estimated per prompt with
# advantage (r - mean)/mean. Dense rewards: the success likelihood must be g(G) with
# log E[g(G + c)] = const + log E[g(G)] for our arbitrary reward offset, which forces
# g = exp(beta G). The objective is then the return's cumulant generating function,
#     (1/beta) log E[exp(beta G)] = mean + beta/2 var + ...,
# whose first-order term is ordinary RL; beta plays the role of the paper's truncation T.
# MuJoCo is the paper's latent-generation model exactly: dynamics are a deterministic
# decoder of the policy's action noise, so this optimism is over controllable outcomes.
#
# Critic = MaxRL's per-prompt mean. LINEX regression, (e^{bu} - bu - 1)/b^2 with
# u = target - psi, is minimized at psi = (1/b) log E[e^{b target}]: the soft value, from
# single samples. Soft GAE over psi gives A; the policy advantage is (e^{bA} - 1)/b.
# At the critic's fixed point E[e^{bA} | s] = 1 for every state (MaxRL's per-prompt
# normalization; the -1 is its control variate), unlike AWR/V-MPO's batch-level softmax.
# With lambda-return targets that fixed point is a mixture of the recursive entropic value
# (lambda=0) and the discounted return's CGF (lambda=1), not the CGF itself. An error in
# psi(s_t) only rescales state s_t's weight; errors at later states enter the exponent
# action-dependently, as GAE's lambda bias does additively.
# beta = kappa / std(A) per batch keeps the temperature dimensionless; kappa -> 0 is PPO.
# Ablations: critic_objective=mse (exponential weights over a mean critic, so per-state
# mass is unnormalized); advantage_transform=linear (soft critic only, PPO weighting).
# Actor and critic gradients are clipped separately.
import math
import os
import random
import time
...
class Args:
    ...
    """dimensionless temperature: beta = kappa / std(soft advantage) per batch; kappa -> 0 is PPO"""
    critic_objective: str = "linex"
    """linex: soft-value critic (MaxRL normalizer); mse: mean critic (ablation)"""
    advantage_transform: str = "exp"
    """exp: (e^{bA} - 1)/b posterior weights; linear: A itself (critic-only ablation)"""
...
def maxrl_advantages(soft_advantages, beta):
    """Success-posterior weight minus its control variate, (e^{bA} - 1)/b; b -> 0 gives A."""
    return torch.expm1(beta * soft_advantages) / beta
...
def value_loss(prediction, target, temperature, args):
    if args.critic_objective == "linex":
        return linex_loss(prediction, target, temperature)
    return 0.5 * (prediction - target) ** 2


def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, temperature, args):
    """Clipped PPO on MaxRL advantages; critic regresses the soft (LINEX) or mean value."""
    ...
    v_loss_unclipped = value_loss(newvalue, returns, temperature, args)
    if args.clip_vloss:
        v_clipped = old_values + torch.clamp(newvalue - old_values, -args.clip_coef, args.clip_coef)
        v_loss = torch.max(v_loss_unclipped, value_loss(v_clipped, returns, temperature, args)).mean()
    else:
...
def validate_args(args):
    ...
    if not args.maxrl_kappa > 0 or args.critic_objective not in {"linex", "mse"}:
        raise ValueError("maxrl_kappa must be positive; critic_objective is linex or mse")
    if args.advantage_transform not in {"exp", "linear"}:
        raise ValueError("advantage_transform is exp or linear")
    return args
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); FP32; native-action storage; host actor mirror; "
                        f"MaxRL-CGF kappa={args.maxrl_kappa} critic={args.critic_objective} "
                        f"transform={args.advantage_transform}")
        ...
        def loss_model(observations, native, old_logprobs, advantages, returns, old_values, temperature):
            return ppo_loss(agent, observations, native, old_logprobs, advantages, returns, old_values, temperature, args)
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                # Soft GAE: the unchanged recursion over psi (or V for the mse ablation).
                ...
                if args.advantage_transform == "exp":
                    b_advantages = maxrl_advantages(soft_advantages, beta)
                else:
                    b_advantages = soft_advantages.clone()
                ...
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss, metrics = loss_model(
                            ...
                            b_advantages[indices], b_returns[indices], b_values[indices], beta,
                        )
                        ...
                        optimizer.step()
                        update_metrics[updates, :6].copy_(metrics)
                        update_metrics[updates, 6] = actor_norm
                        update_metrics[updates, 7] = critic_norm
                        updates += 1
            ...
            logged = gather_metrics({
                ...
                "losses/actor_grad_norm": update_metrics[:updates, 6].mean(),
                "losses/critic_grad_norm": update_metrics[:updates, 7].mean(),
                **posterior,
            })
