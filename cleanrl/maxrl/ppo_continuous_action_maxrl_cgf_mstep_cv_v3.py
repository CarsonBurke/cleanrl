# MaxRL-CGF M-step v3: the M-step with its self-fitting noise replaced by its expectation.
#
# v2 fitted  mean_i w_i log pi(a_i|s_i)  with w = e^{bA}/mean(w) > 0 and one action per
# state. Write w = (w - 1) + 1. The "+1" part, mean_i log pi(a_i|s_i), is a single-sample
# Monte Carlo estimate of E_{a~pi_old}[log pi(a|s)]. Its population value never moves pi,
# since the MLE of pi_old's own samples is pi_old, but its finite-sample value regresses
# the policy toward the sampled noise. That can only raise concentration (one point per
# state is fitted best by a delta). Replace it by its exact value:
#     E_old[log pi] = -H(pi_old) - KL(pi_old || pi)
# so the M-step objective is identical in expectation and becomes
#     maximize  mean_i (w_i - 1) log pi(a_i|s_i)  -  KL(pi_old || pi)  (+ const),
# evaluated on the decoupled mean and concentration steps. The -1 is MaxRL's control
# variate. The unit KL is the analytic remainder of the positive weights, not a tuned
# penalty. The Lagrangian budgets add on top of it. As kappa -> 0, (w - 1) -> bA, so this
# becomes a KL-regularized policy gradient. Everything else is as in v2.
import math
import os
import random
import time
...
def mstep_loss(agent, observations, native_actions, old_alpha, old_beta, weights, returns,
    ...
    """Control-variate M-step on the decoupled Beta steps, dual-descent KL budgets, LINEX critic."""
    ...
    # Control-variate M-step: E_old[log pi] enters analytically as -KL (see header).
    fit_loss = -((weights - 1.0) * log_likelihood).mean()
    ...
    policy_loss = fit_loss + ((1.0 + multipliers.detach()) * kl).sum()
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); FP32; native-action storage; host actor mirror; "
                        f"MaxRL-CGF control-variate M-step kappa={args.maxrl_kappa} eps_mean={args.eps_mean} eps_conc={args.eps_conc}")
