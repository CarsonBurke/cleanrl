# Successor-latent critic v3: a fixed-policy mechanism experiment, not an RL claim.
...
@dataclass
class Args:
    exp_name: str = "successor_latent_v3_fixed_policy_gate"
    seed: int = 1
    env_id: str = "HalfCheetah-v4"
    gate_checkpoint: str = "runs/HalfCheetah-v4__ppo_normres_action_quadratic_v1_ordinary_8M__1__1789524779/ppo_normres_action_quadratic_v1_ordinary_8M.cleanrl_model"
    num_envs: int = 16
    num_steps: int = 2048
    gamma: float = 0.99
...
def summarize_gate(rows, seed):
    ...
    passed = snr >= 3 and relative_upper < .5 and cosine_lower > .5 and reward_error < .05 and representation_error < .1
    return {
        'passed': bool(passed), 'status': 'passed' if passed else ('inconclusive' if snr < 3 else 'failed'),
        'criterion': 'reference SNR >=3; upper95 relative gradient error <0.5; lower95 cosine >0.5; reward NMSE <0.05; representation NMSE <0.1',
...
def main():
    ...
    try:
        ...
        agent.actor.load_state_dict({k[len('actor.'):]: v for k, v in checkpoint['model'].items() if k.startswith('actor.')})
        agent.critic.load_state_dict({k[len('critic.'):]: v for k, v in checkpoint['model'].items()
                                     if k.startswith('critic.trunk.') or k.startswith('critic.value_head.')})
        ...
        reward_decoder = RewardDecoder(successor.latent_dim, args.critic_width).to(device)
        ...
        def reward_loss(latents, rewards):
            return .5 * (reward_decoder(latents.detach()) - rewards).square().mean()
        ...
        report.update(args=vars(args), actor_unchanged=True, models_frozen_for_holdout=True, holdout=rows,
