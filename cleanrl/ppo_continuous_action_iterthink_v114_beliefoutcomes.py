...
class Args:
    ...
    lewm_reward_loss_coef: float = 1.0   # v114: one consumed reward CE head, not a 4-head MTP sum
    lewm_termination_loss_coef: float = 1.0
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # v114 consumed outcome path: transition reward/continue are read from the
        # same flattened action-conditioned predictor belief that imagination uses
        # to produce the next latent. This avoids a separate semantic-token decoder
        # whose CE can look fine while the scalar path consumed by dreams drifts.
        ...
        # prepends these queries at each step. v114 decodes outcomes from the
        # flattened belief, so the queries can help pool transition information
        # without becoming a separate reward-token decoder.
    ...
    def imagine_step_from_history(self, latent_history, action_history):
        ...
        reward_logits, continuation_logits = self.decode_belief_outcomes(
            features, action_history[:, -1], detach_belief=False
        )
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        wm_reward_abs_error_sums = torch.zeros(1, device=device)
        wm_reward_error_sums = torch.zeros(1, device=device)
        wm_reward_pred_edge_sums = torch.zeros(1, device=device)
        wm_reward_target_edge_sums = torch.zeros(1, device=device)
        wm_reward_probe_weights = torch.zeros(1, device=device)
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    pred_mtp, pred_beliefs = agent.world_model.predict_mtp_from_history(
                    ...
                    # v114: train the exact consumed outcome path. The transition
                    # reward/continue head reads the action-conditioned predictor
                    # belief for (s_t, a_t), and imagination consumes that same
                    # head inside imagine_step_from_history. Future reward MTP is
                    # deliberately not trained from a current belief without future
                    # dynamics/actions; latent MTP remains the dynamics objective.
                    pred_reward_logits, pred_continuation_logits = agent.world_model.decode_belief_outcomes(
                        pred_beliefs,
                        future_actions,
                        detach_belief=False,
                    )
                    ...
                    outcome_denom = latent_weight.sum().clamp_min(1.0)
                    reward_loss = -(
                        reward_target_probs.detach() * torch.log_softmax(pred_reward_logits, dim=-1)
                    ).sum(dim=-1)
                    wm_pred_reward_loss = (reward_loss * latent_weight).sum() / outcome_denom
                    termination_loss = F.binary_cross_entropy_with_logits(
                        pred_continuation_logits,
                        1.0 - future_terminations,
                        reduction="none",
                    )
                    wm_pred_termination_loss = (termination_loss * latent_weight).sum() / outcome_denom
                    with torch.no_grad():
                        pred_reward_scalar = reward_support.to_scalar(pred_reward_logits)
                        reward_error = pred_reward_scalar - future_rewards
                        wm_reward_abs_error_sums[0] += (reward_error.abs() * latent_weight).sum()
                        wm_reward_error_sums[0] += (reward_error * latent_weight).sum()
                        pred_reward_probs = torch.softmax(pred_reward_logits, dim=-1)
                        pred_edge_mass = pred_reward_probs[..., 0] + pred_reward_probs[..., -1]
                        wm_reward_pred_edge_sums[0] += (pred_edge_mass * latent_weight).sum()
                        target_edge_frac = (
                            (future_rewards <= args.lewm_reward_v_min)
                            | (future_rewards >= args.lewm_reward_v_max)
                        ).to(latent_weight.dtype)
                        wm_reward_target_edge_sums[0] += (target_edge_frac * latent_weight).sum()
                        wm_reward_probe_weights[0] += outcome_denom
