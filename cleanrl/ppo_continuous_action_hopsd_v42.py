# HOPSD v42 — Q-substrate pack: value-scale norm + pessimism dial + SimBa Q-block + LAP replay.
# =====================================================================================
# V42 folds four mechanism transplants from the SOTA critic literature onto the v41 base (search +
# dist-Q + KL-dual). Each is independently flagged for attribution; all four target the SAME lever —
# a better-conditioned, scale-stable, less-pessimistic value ridge — because in v40/v41 the teacher is
# the SOLE improvement engine and it can only climb what the twin-Q substrate represents. Provenance:
#   (1) VALUE-SCALE NORMALIZATION (default ON) — SimBaV2 / MR.Q reward-scaling. A running std of
#       discounted returns (EMA, eps-floored) normalizes TD targets AND every Q magnitude the teacher
#       consumes (DPG ascent, search softmax weights, advantage tilt). As raw returns grow 6k->12k the
#       teacher's improvement pressure and the 1.2-nat KL budget hold constant EFFECTIVE temperature.
#       Dedup vs q_reward_scale: value_scale subsumes the global-magnitude role; q_frontier is kept
#       ONLY as a local action-sensitivity conditioner (it divides a Q-GAP within a state, an axis
#       value_scale does not touch) — see notes at the frontier site.
#   (2) PESSIMISM DIAL --q-pessimism {min,mean,optimist} — BRO ablation core, licensed here by our LN
#       critics + the KL budget (both cap the exploitation a looser bound could invite). min = twin
#       minimum (v41 behavior); mean = twin average; optimist = mean + q_beta*std (std of two twins =
#       |q1-q2|/2, q_beta default 0.5). Applied uniformly to TD targets AND teacher ascent/search
#       ranking so the whole improvement signal shifts together. Default: mean.
#   (3) SIMBA Q-BLOCK (default ON) — SimBa architecture. Twin Q nets: RSNorm(inputs) -> linear embed ->
#       2x pre-LN residual inverted-bottleneck(4x) blocks -> final LN -> head, at ~parameter parity
#       with the [256,256] MLP (embed width h=66; trunk 16h^2+40h ~= 72.3k vs MLP 73.0k, within 1%).
#       RSNorm (SimBa running-stats input normalizer, no affine) standardizes the (obs, action) Q
#       input (obs is env-normalized ~unit; action in [-1,1]); the residual pre-LN stack conditions
#       the ridge the operators ascend.
#   (4) LAP PRIORITIZED REPLAY (default ON) — TD7 Loss-Adjusted Prioritized replay. priority =
#       clamp(max-over-twins |n-step TD|, min=1)^0.4, sampling proportional; critic loss = Huber(delta=1)
#       (MSE below 1, linear above) which cancels the prioritization bias WITHOUT importance weights.
#       Applies to the replay TD loop only; focuses UTD compute on the high-error transitions.
#   PERF STACK (from the v41 perf addendum, carried in this build): bf16 autocast (--bf16, default ON),
#       fused Adam, TF32, monolithic torch.compile tiers, async vector envs (--async-envs, default ON,
#       per-env-index seeding args.seed+i), TD block-graph decision (see td loop). Determinism tradeoff
#       of async envs: single-seed matched-step comparisons stay valid; bit-exact relaunch repro is lost.
#   TELEMETRY: debug/value_scale, debug/q_std_over_mean (twin disagreement; feeds optimist), and
#       debug/lap_priority_mean, alongside all v40/v41 tags.
#   KL BUDGET: raised 0.6 -> 1.2 nats (default). distill_kl equilibrated at ~1.5 nats against the 0.6
#       budget with the dual pinned at ceiling TWICE, and the ~1.5-nat teacher lead won both times
#       (v41_search 6693@1M, new best). The budget now sanctions the empirical winner; the dual stays
#       a runaway guard ABOVE 1.2. Search stays default ON (the confirmed keeper).
# HYPOTHESIS: a better-conditioned, scale-stable, less-pessimistic ridge — with a teacher allowed a
#   ~1.2-nat lead — raises what the teacher's operators (DPG + sampled search) can climb, compounding
#   with v41, targeting the 1M gate well above v40's 6353 (v41_search already banked 6693@1M).
# =====================================================================================
# --- v41 (the base this builds on) ---
# =====================================================================================
# V40 RESULT: 4004@500k — best in the ledger; mechanism CONFIRMED (teacher_q_gap positive throughout,
#   action_boundary_frac 0.008->0.30 = bang-bang regime entered, q_ev_replay 0.94). The teacher-as-
#   sole-improvement-engine design works. The KL dual was verified correct (live lam reproduced
#   bit-for-bit from the logged kl) — v41 KEEPS the proven 0.6-nat budget; the search operator's new
#   pressure spends within the SAME envelope and the dual arbitrates between DPG and search.
# V41 UPGRADES the teacher's IMPROVEMENT OPERATOR itself:
#   (1) SAMPLED SEARCH (core leap). v40's teacher improves only by LOCAL DPG ascent — a gradient
#       nudge, ill-conditioned at the action rails where we now live (boundary_frac 0.30). Add a
#       GLOBAL MPO/AWAC-style E-step: on the same fresh+replay minibatches, draw k=8 actions/state
#       from the CURRENT teacher Beta (detached), score by min(Q1,Q2), w_i = softmax((Q_i - max_j
#       Q_j)/(tau*q_frontier)), and add -search_coef * sum_i w_i log pi_T(a_i|s). Trains the density
#       toward the Q-best of its OWN proposals — a proper policy-improvement step that works where
#       DPG gradients are sick, and COMPOUNDS with DPG (local + global). Both inside the KL budget.
#   (2) DUAL hygiene (budget KEPT at 0.6; eta 0.3->0.5, floor 1e-3->0.02; extracted to unit-tested
#       dual_update_lam). Faster runaway engagement, inert at the proven ~0.5 equilibrium.
#   (3) DISTRIBUTIONAL twin-Q (--dist-q, default OFF): both Q nets emit HL-Gauss categorical logits
#       over the critic's Dreamer3 bucket support; replay TD target = projected n-step distributional
#       Bellman (C51 projection on the uniform symlog coord grid); DPG/search ranking = min of the two
#       EXPECTED values. The ridge-quality arm — a better-conditioned Q raises the ceiling the
#       operators climb. Scalar-MSE path byte-identical to v40 when OFF.
#   (4) TELEMETRY: debug/search_top_gap = E[max_i Q_i - minQ(s, a_realized)] (headroom the search
#       finds — new aliveness metric alongside teacher_q_gap); debug/search_ess = mean effective
#       sample size of w (softmax concentration; healthy ~2-4 of 8). All v40 tags kept.
# HYPOTHESIS: local+global improvement operators compound (search escapes the rails DPG can't climb);
#   the distributional ridge raises the value ceiling both operators ascend.
# =====================================================================================
# --- v40 notes (the base this builds on) ---
...
class Args:
    ...
    # v41 SAMPLED-SEARCH improvement operator (MPO/AWAC-style E-step): on the same teacher-DPG
    # minibatches (fresh+replay 50/50), draw search_k actions per state from the CURRENT teacher
    # Beta (detached), rank by min(Q1,Q2), softmax to per-state weights over the k samples, and add
    # -search_coef * sum_i w_i log pi_T(a_i|s) to the teacher actor loss. A GLOBAL improvement step
    # that works at the action rails where DPG gradients are ill-conditioned; compounds with DPG.
    search_coef: float = 0.5         # weight of the sampled-search NLL term (0 disables)
    search_k: int = 8                # action samples per state
    search_tau: float = 1.0          # softmax temperature (in q_frontier units) for the search weights
    q_nstep: int = 3                 # v40: n-step returns for the replay TD targets (Q quality)
    # v41 DISTRIBUTIONAL twin-Q (ridge-quality arm; default OFF). Both Q nets emit HL-Gauss
    # categorical logits over the critic's Dreamer3 bucket support; the replay TD target is the
    # projected n-step distributional Bellman target; DPG/search ranking uses min of the two
    # EXPECTED values. Scalar MSE path is unchanged when OFF.
    dist_q: bool = False
    ...
    # v41 perf: bf16 autocast over all network forward/backward (fp32 master weights, no GradScaler
    # — bf16's fp32-range exponent makes scaling unnecessary). Distribution math (Beta log_prob/KL)
    # and HL-Gauss cross-entropy stay fp32 (autocast keeps softmax/log/lgamma/sum reductions in
    # fp32; only the Linear/matmul GEMMs run bf16). v42 DEFAULT OFF: the SPS matrix showed bf16
    # autocast NEARLY HALVES throughput here (232 vs 448 SPS) — the Q/actor GEMMs are tiny and
    # launch-bound, so autocast's per-op fp32<->bf16 cast kernels cost more than the bf16 matmul
    # saves. Parity is clean, so it stays available behind --bf16 for larger-net experiments.
    bf16: bool = False               # bf16 autocast for all forward/backward; fp32 master weights
    # v42 Q-SUBSTRATE PACK -----------------------------------------------------------------------
    q_pessimism: Literal["min", "mean", "optimist"] = "mean"  # twin-Q combine for TD targets AND
                                     # teacher ascent/search ranking. min=v41 (SAC/BRO pessimism),
                                     # mean=twin average, optimist=mean+q_beta*std (BRO ablation core).
    q_beta: float = 0.5              # optimist upside coefficient (std of two twins = |q1-q2|/2)
    value_scale: bool = True         # SimBaV2/MR.Q running return-scale normalizer for TD/onpol
                                     # targets (subsumes the frozen reward-std). Keeps Q ~O(1) and the
                                     # teacher's effective temperature constant as raw returns grow.
    value_scale_momentum: float = 0.99   # EMA momentum for the running std of discounted returns
    simba_q: bool = True             # SimBa Q-block: RSNorm -> embed -> 2x pre-LN residual
                                     # inverted-bottleneck(4x) -> LN -> head (param-parity with MLP)
    simba_q_hidden: int = 66         # SimBa embed width h (16h^2+40h ~= 72.3k ~= MLP 73.0k)
    lap: bool = True                 # TD7 LAP prioritized replay (priority^alpha, Huber(1) loss)
    lap_alpha: float = 0.4           # LAP priority exponent
    async_envs: bool = True          # AsyncVectorEnv (subprocess rollout) vs SyncVectorEnv
    ...
    # v41: budget raised 0.6 -> 1.5 nats. The v40 live run showed ~1.8 nats of teacher lead is
    # FOLLOWABLE and WINNING (4004@500k). The dual was VERIFIED correct (live lam reproduced
    # bit-for-bit from the logged kl) — the "asleep" read sampled a transient spike; the equilibrium
    # ~0.5 under the 0.6 budget IS the winning regime. So KEEP the budget at 0.6 (proven setpoint);
    # the v41 search operator spends within the SAME envelope and the dual arbitrates DPG vs search
    # automatically. Adopt only the v40_1 HYGIENE: eta 0.3->0.5 + floor 1e-3->0.02 (faster runaway
    # engagement on a breach, inert at equilibrium) + the pure unit-tested dual_update_lam().
    teacher_kl_budget: float = 1.2   # target mean KL(pi_T || pi_S) in nats. v42: raised 0.6->1.2 —
                                     # distill_kl equilibrated at ~1.5 nats against the 0.6 budget with
                                     # the dual pinned at ceiling TWICE, and the ~1.5-nat teacher lead
                                     # won both times (v41_search 6693@1M, new best). Sanction the
                                     # empirical winner; the dual stays a runaway guard ABOVE 1.2.
    teacher_kl_eta: float = 0.5      # dual-ascent step
...
def combine_q(q1, q2, mode, beta=0.5):
    """v42 PESSIMISM DIAL (BRO ablation core). Combine the twin Q values into the single value the
    TD target / teacher ascent / search ranking consume. min = twin minimum (v41, SAC/BRO pessimism);
    mean = twin average; optimist = mean + beta*std, where std of the two twins = |q1-q2|/2 so
    optimist = 0.5(q1+q2) + beta*0.5*|q1-q2|. Licensed here by the LN critics + KL budget, both of
    which cap the exploitation a looser bound could invite. `mode` is a python str -> a static branch
    under torch.compile (specialized, no runtime overhead)."""
    if mode == "min":
        return torch.min(q1, q2)
    half_sum = 0.5 * (q1 + q2)
    if mode == "mean":
        return half_sum
    return half_sum + beta * 0.5 * (q1 - q2).abs()   # optimist


class RSNorm(nn.Module):
    """v42 SimBa running-statistics input normalizer (no learnable affine). forward() is READ-ONLY
    (just standardizes with the current buffers) — the stats are advanced explicitly via update() ONCE
    per iteration, OUTSIDE the compiled/CUDA-graphed td_step. This is deliberate: an in-place buffer
    write inside forward would (a) be double-counted by CompiledTier's eager-then-compiled parity
    probe and desync the two calls (failing parity, disabling the td-tier compile), and (b) risk a
    cudagraph input-mutation error. Read-only forward sidesteps both while staying faithful to SimBa.
    update() uses a batched Welford (Chan's parallel) merge; the count grows unbounded so the stats
    converge (new-batch weight b/count -> 0). Target nets get their buffers copied from online in the
    same per-iteration step (buffers escape the param polyak)."""
    ...
    def update(self, x):
        ...
        delta = bmean - self.running_mean
        tot = self.count + b
        self.running_mean += delta * (b / tot)
        M2 = self.running_var * self.count + bvar * b + delta * delta * (self.count * b / tot)
        # In-place: preserve buffer identity so the snapshotted target buffer-lists AND the CUDA-graph
        # capture see the update (a rebind would orphan the original tensor -> stale target var / graph).
        self.running_var.copy_(M2 / tot)
        self.count += b
...
class SimBaBlock(nn.Module):
    """v42 SimBa pre-LN residual inverted-bottleneck (expansion 4x): x + Linear(4h->h)(ReLU(Linear(
    h->4h)(LN(x)))). Pre-LN + residual conditions the value ridge the teacher operators ascend."""

    def __init__(self, h, expansion=4):
        super().__init__()
        self.norm = nn.LayerNorm(h)
        self.fc1 = layer_init(nn.Linear(h, expansion * h))
        self.fc2 = layer_init(nn.Linear(expansion * h, h))
...
class QNet(nn.Module):
    """v34: SAC-style state-action value net with LayerNorm (high-UTD stabilizer).
    ...
    v42: --simba-q swaps the [256,256] MLP for a SimBa block (RSNorm(inputs) -> linear embed ->
    2x pre-LN residual inverted-bottleneck(4x) -> final LN -> head) at ~parameter parity. support=None
    -> scalar head (v40 behavior); support given -> HL-Gauss categorical head, forward() returns the
    EXPECTED value (every Q call site unchanged), dist_logits() exposes the logits for the dist TD loss.
    """

    def __init__(self, obs_dim, act_dim, hidden=256, support=None, simba=False, simba_h=66, simba_blocks=2):
        super().__init__()
        self.support = support
        self.simba = simba
        out_dim = support.num_bins if support is not None else 1
        in_dim = obs_dim + act_dim
        if simba:
            self.rsnorm = RSNorm(in_dim)
            self.embed = layer_init(nn.Linear(in_dim, simba_h))
            self.blocks = nn.Sequential(*[SimBaBlock(simba_h) for _ in range(simba_blocks)])
            self.final_norm = nn.LayerNorm(simba_h)
            self.head = layer_init(nn.Linear(simba_h, out_dim), std=1.0)
        else:
    ...
    def update_rsnorm(self, obs, act):
        """Advance the RSNorm running stats on a representative (obs, act) batch. Called once per
        iteration outside the compiled td_step; no-op when --simba-q is off."""
        if self.simba:
            self.rsnorm.update(torch.cat([obs, act], dim=-1))

    def _trunk(self, obs, act):
        x = torch.cat([obs, act], dim=-1)
        if self.simba:
            return self.head(self.final_norm(self.blocks(self.embed(self.rsnorm(x)))))
        return self.net(x)

    def forward(self, obs, act):
        out = self._trunk(obs, act)
        if self.support is None:
            return out.squeeze(-1)
        return self.support.probs_to_scalar(torch.softmax(out, dim=-1))

    def dist_logits(self, obs, act):
        """Categorical logits (B, num_bins); distributional mode only."""
        return self._trunk(obs, act)
...
def dual_update_lam(lam, kl, budget, eta, lam_min, lam_max):
    """v40_1/v41: one multiplicative dual-ascent step for the privilege-budget Lagrange multiplier.

    lam_{t+1} = clip( lam_t * exp(eta * (kl/budget - 1)), [lam_min, lam_max] ).
    kl > budget -> multiplier > 1 (tighten toward the student); kl < budget -> lam decays.
    Extracted as a pure function so the controller is unit-testable. (The v40 dual was VERIFIED
    correct against the live run — lam reproduced bit-for-bit from the logged kl — so this is not a
    bugfix; v41 raises the budget to 1.5 nats and eta to 0.5 per the winning run's evidence.)"""
    lam = lam * math.exp(eta * (kl / budget - 1.0))
    return float(min(max(lam, lam_min), lam_max))


def project_nstep_categorical(p_boot, nrew, disc, support):
    """v41: C51-style categorical projection of the n-step distributional Bellman target onto the
    (symexp-spaced Dreamer3) support.

      p_boot : (B, N) target-net categorical probs at the bootstrap state/action.
      nrew   : (B,)   scaled n-step reward sum.
      disc   : (B,)   bootstrap discount = gamma^k * (1-term) * valid.
      support: the Dreamer3 bucket support (raw centers + UNIFORM symlog coord grid).

    Each support atom z_j (raw center) is shifted Tz_j = nrew + disc*z_j, mapped to the uniform
    coord grid via symlog, and its mass p_boot_j split between the two neighbor bins by linear
    interpolation THERE (exact, since coord spacing is uniform). Mass-conserving. Returns (B, N)."""
    z = support.support                                              # (N,) raw centers
    N = z.shape[0]
    Tz = nrew.unsqueeze(-1) + disc.unsqueeze(-1) * z.unsqueeze(0)    # (B, N) raw shifted atoms
    cz = symlog(Tz).clamp(support.coord_min, support.coord_max)      # (B, N) coord (symlog) space
    b = (cz - support.coord_min) / support.coord_bin_width           # (B, N) grid position in [0, N-1]
    lo = b.floor().clamp(0, N - 1)
    frac = b - lo                                                    # in [0, 1)
    lo = lo.long()
    hi = (lo + 1).clamp(max=N - 1)
    m = torch.zeros_like(p_boot)
    m.scatter_add_(1, lo, p_boot * (1.0 - frac))
    m.scatter_add_(1, hi, p_boot * frac)
    return m
...
if __name__ == "__main__":
    ...
    # v41 perf: TF32 baseline for the fp32 matmul/conv paths (Ampere+); free accuracy-for-speed on
    # the small MLP GEMMs. Kept off when torch_deterministic is requested (TF32 is nondeterministic).
    if not args.torch_deterministic:
        torch.backends.cuda.matmul.allow_tf32 = True
        torch.backends.cudnn.allow_tf32 = True

    # v42 perf: AsyncVectorEnv (subprocess-per-env) parallelizes the CPU MuJoCo rollout (36.5% of
    # wall time in the v41 profile) across cores. Determinism tradeoff: single-seed matched-step
    # comparisons stay valid, but bit-exact relaunch reproduction is lost (subprocess scheduling).
    # Per-env-index seeding is preserved: gymnasium's vector reset(seed=args.seed) seeds sub-env i
    # with args.seed+i for BOTH Async and Sync, so the seed derivation is identical to v41.
    _env_fns = [
        make_env(
            args.env_id,
            i,
    ...
    envs = gym.vector.AsyncVectorEnv(_env_fns) if args.async_envs else gym.vector.SyncVectorEnv(_env_fns)
    ...
    # v41 perf: fused=True collapses the per-parameter Adam elementwise ops into a few kernels — a
    # meaningful launch-cost cut, most of all for the 32768x/iter q_optimizer.step (launch-bound).
    ...
    teacher_optimizer = optim.Adam(teacher.parameters(), lr=args.learning_rate, eps=1e-5, fused=True)

    # --- v34: twin Q nets + polyak targets (independent inits) + separate optimizer ---
    # v41: shared HL-Gauss bucket support (built here so the distributional Q heads can reuse it).
    ...
    q_support = hl_support if args.dist_q else None   # v41: distributional twin-Q when --dist-q
    _qkw = dict(support=q_support, simba=args.simba_q, simba_h=args.simba_q_hidden)
    q1 = QNet(obs_dim, act_dim, **_qkw).to(device)
    q2 = QNet(obs_dim, act_dim, **_qkw).to(device)
    q1_target = QNet(obs_dim, act_dim, **_qkw).to(device)
    q2_target = QNet(obs_dim, act_dim, **_qkw).to(device)
    q1_target.load_state_dict(q1.state_dict())
    q2_target.load_state_dict(q2.state_dict())
    # v42: target RSNorm must stay FROZEN (eval -> no running-stat update on bootstrap forwards);
    # its buffers are copied from the online nets after each TD block (buffers escape param polyak).
    q1_target.eval()
    q2_target.eval()
    # Buffer lists for the post-TD-block normalizer sync (RSNorm running_mean/var/count). Empty when
    # --simba-q is off (MLP QNet has no buffers), so the sync foreach is a no-op then.
    q1_buf_list, q1_tbuf_list = list(q1.buffers()), list(q1_target.buffers())
    q2_buf_list, q2_tbuf_list = list(q2.buffers()), list(q2_target.buffers())
    q_optimizer = optim.Adam(list(q1.parameters()) + list(q2.parameters()), lr=args.q_lr, eps=1e-5, fused=True)
    ...
    def pooled_obs_rms():
        """Pooled (over the 16 identical envs) current obs_rms mean/var as device tensors. Async envs
        have no local `.envs`, so the NormalizeObservation stats are fetched over IPC via get_attr
        (once per iteration -> negligible vs the rollout)."""
        if args.async_envs:
            rms_list = envs.get_attr("obs_rms")
            means = np.stack([r.mean for r in rms_list])
            varis = np.stack([r.var for r in rms_list])
        else:
    ...
    if not args.q_onpol_only:
        ...
        # v42 LAP: per-transition priority (aligned with the replay ring). Sampled proportionally;
        # new entries inserted at the running max priority so they are seen at least once.
        rep_prio = torch.ones((rep_cap,), device=device)
    rep_ptr = 0
    rep_filled = 0
    lap_max_prio = 1.0   # running max LAP priority for newly-inserted transitions

    # Frozen raw-reward scale for Q targets (running std over the warmup, then frozen). v42: superseded
    # by the running value_scale when --value-scale (the actual normalizer is q_scale, set per-iter).
    q_rew_sum = 0.0
    q_rew_sumsq = 0.0
    q_rew_count = 0
    q_reward_scale = 1.0
    q_reward_frozen = False
    # v42 VALUE-SCALE: running std of discounted returns (EMA), eps-floored; the single global
    # target/Q-magnitude normalizer when --value-scale. value_scale_init seeds it from the first
    # iteration's return std (so no slow ramp from 1.0). q_scale (set each iteration) is what the
    # TD/onpol targets and telemetry divide by: value_scale when ON, else the frozen reward std.
    value_scale = 1.0
    value_scale_init = True
    q_scale = q_reward_scale
    ...
    def td_forward(s_obs, s_boot, s_act, s_nrew, s_disc):
        """v40/v41: n-step twin-Q TD forward+loss (static q_batch shape). Replay sampling/indexing
        and the optimizer/polyak/clip stay OUTSIDE; this is the graphed region. s_disc carries the
        whole bootstrap coefficient (gamma^k*(1-term)*valid), so no per-iteration scalar is baked in.
        v41: distributional (--dist-q) branch projects the n-step distributional Bellman target and
        trains both categorical heads by cross-entropy; scalar (default) branch is the v40 MSE.
        v42: the bootstrap uses the PESSIMISM DIAL (combine_q) instead of a hard twin-min; --lap swaps
        the MSE for Huber(delta=1) (the LAP loss that cancels the prioritization bias without IS
        weights); returns the per-sample max-over-twins |TD| so the caller can refresh LAP priorities."""
        if q_support is None:
            with torch.no_grad():
                a_boot = student_env_action(s_boot, reparam=False)
                q_boot = combine_q(q1_target(s_boot, a_boot), q2_target(s_boot, a_boot), args.q_pessimism, args.q_beta)
                y = s_nrew + s_disc * q_boot
            q1_pred = q1(s_obs, s_act)
            q2_pred = q2(s_obs, s_act)
            if args.lap:
                q_loss = F.huber_loss(q1_pred, y, delta=1.0) + F.huber_loss(q2_pred, y, delta=1.0)
            else:
                q_loss = F.mse_loss(q1_pred, y) + F.mse_loss(q2_pred, y)
            td_abs = torch.maximum((q1_pred - y).abs(), (q2_pred - y).abs()).detach()
            return q_loss, q1_pred.detach().mean(), td_abs
        # distributional: target = projected n-step Bellman under the pessimism dial. min -> lower-EV
        # target dist (v41); optimist -> higher-EV dist; mean -> average of the two projected targets.
        with torch.no_grad():
            a_boot = student_env_action(s_boot, reparam=False)
            p1 = torch.softmax(q1_target.dist_logits(s_boot, a_boot), dim=-1)
            p2 = torch.softmax(q2_target.dist_logits(s_boot, a_boot), dim=-1)
            v1 = q_support.probs_to_scalar(p1)
            v2 = q_support.probs_to_scalar(p2)
            if args.q_pessimism == "mean":
                m = 0.5 * (project_nstep_categorical(p1, s_nrew, s_disc, q_support)
                           + project_nstep_categorical(p2, s_nrew, s_disc, q_support))
            else:
                pick1 = (v1 >= v2) if args.q_pessimism == "optimist" else (v1 <= v2)
                p_boot = torch.where(pick1.unsqueeze(-1), p1, p2)
                m = project_nstep_categorical(p_boot, s_nrew, s_disc, q_support)
            tgt_ev = q_support.probs_to_scalar(m)
        lg1 = q1.dist_logits(s_obs, s_act)
        lg2 = q2.dist_logits(s_obs, s_act)
        q_loss = -(m * torch.log_softmax(lg1, dim=-1)).sum(-1).mean() \
                 - (m * torch.log_softmax(lg2, dim=-1)).sum(-1).mean()
        pred1 = q_support.probs_to_scalar(torch.softmax(lg1, dim=-1))
        pred2 = q_support.probs_to_scalar(torch.softmax(lg2, dim=-1))
        td_abs = torch.maximum((pred1 - tgt_ev).abs(), (pred2 - tgt_ev).abs()).detach()
        return q_loss, pred1.detach().mean(), td_abs
    ...
    # v41 perf: one reusable bf16 autocast context (enabled toggles it — no nullcontext scattering).
    # Wraps the network forward regions; the Linear/matmul GEMMs run bf16 while autocast keeps the
    # distribution math (Beta lgamma/log_prob, softmax/log_softmax, sum reductions, HL-Gauss CE) in
    # fp32 by its own op-type rules. Weights stay fp32 (master); bf16's fp32-range exponent removes
    # the need for a GradScaler. Backward runs outside the context (saved-activation dtypes carry the
    # bf16 GEMMs into the backward automatically — the textbook AMP pattern).
    amp = torch.autocast("cuda", dtype=torch.bfloat16, enabled=args.bf16)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad(), amp:  # v41 perf: bf16 over the value bootstrap + teacher-critic warmup
        ...
        with torch.no_grad():
            ...
            if not args.q_onpol_only:
                ...
                if args.lap:
                    rep_prio[idx] = lap_max_prio   # v42 LAP: new transitions enter at the running max
        ...
        # v42 VALUE-SCALE: update the running std of discounted returns (EMA, eps-floored) and pick
        # the actual target normalizer q_scale. This is the single global magnitude normalizer: it
        # keeps Q ~O(1) as raw returns grow so the teacher's effective temperature (DPG /q_frontier,
        # search softmax /q_frontier, KL budget) stays constant. Dedup vs the other normalizers is
        # deliberate — q_frontier (local action-sensitivity, in these already-normalized Q units) and
        # the batch z-scored advantage tilt (adv_z) are orthogonal axes, so neither stacks with this.
        if args.value_scale:
            _rstd = b_returns_raw.std(unbiased=False).item()
            if value_scale_init:
                value_scale = max(_rstd, 1e-3)
                value_scale_init = False
            else:
                value_scale = max(args.value_scale_momentum * value_scale + (1 - args.value_scale_momentum) * _rstd, 1e-3)
            q_scale = value_scale
        else:
            q_scale = q_reward_scale

        # v42 RSNorm: advance the online Q input-normalizer stats ONCE per iteration (forward stays
        # read-only for CUDA-graph/parity safety) on the executed (obs, act) batch — the same input
        # space the TD forwards see — then copy them into the frozen target nets (buffers escape the
        # param polyak). No-op when --simba-q is off (MLP QNet has no buffers).
        if args.simba_q:
            q1.update_rsnorm(b_obs, b_actions)
            q2.update_rsnorm(b_obs, b_actions)
            torch._foreach_copy_(q1_tbuf_list, q1_buf_list)
            torch._foreach_copy_(q2_tbuf_list, q2_buf_list)
        ...
        # v42 LAP: proportional sampling. Per-update multinomial over ~1M priorities x q_updates_per_iter
        # is intractable, so ONE draw per iteration yields all indices (updates, q_batch); priorities
        # refresh per-update via an eager scatter outside the graphed td_step (canonical LAP freshness).
        q_loss_accum = torch.zeros((), device=device)
        q_val_accum = torch.zeros((), device=device)
        lap_prio_sum = torch.zeros((), device=device)
        if not args.q_onpol_only:
            if args.lap:
                _flat_idx = torch.multinomial(
                    rep_prio[:rep_filled], args.q_updates_per_iter * args.q_batch, replacement=True
                )
                idx_all = _flat_idx.view(args.q_updates_per_iter, args.q_batch)
            for _qu in range(args.q_updates_per_iter):
                s_idx = idx_all[_qu] if args.lap else torch.randint(0, rep_filled, (args.q_batch,), device=device)
                s_obs = obs_renormalize(rep_obs[s_idx], rms_mean, rms_var)
                s_boot = obs_renormalize(rep_nstep_obs[s_idx], rms_mean, rms_var)
                s_act = rep_act[s_idx]
                s_nrew = rep_nstep_rew[s_idx] / q_scale
                s_disc = rep_nstep_disc[s_idx]
                with amp:
                    q_loss, q1_mean, td_abs = td_step(s_obs, s_boot, s_act, s_nrew, s_disc)
                ...
                with torch.no_grad():
                    ...
                    if args.lap:
                        # v42 fix: LAP's floor-at-1 is calibrated to RAW return-scale TD (TD7). td_abs
                        # is in value_scale-normalized units (~O(1)), so flooring it directly would pin
                        # every priority to 1 -> uniform sampling, silently cancelling LAP against the
                        # value-scale normalizer. Rescale to raw units before the floor so priorities
                        # actually differentiate high-error transitions.
                        new_prio = (td_abs * q_scale).clamp_min(1.0).pow(args.lap_alpha)
                        rep_prio[s_idx] = new_prio
                        lap_prio_sum += new_prio.mean()
                    q_loss_accum += q_loss.detach()
                    q_val_accum += q1_mean
            if args.lap:
                lap_max_prio = max(lap_max_prio, float(rep_prio[:rep_filled].max().item()))
        ...
        q_targets_onpol = b_returns_raw / q_scale
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with amp:
                    if q_support is None:
                        ...
                        lg1 = q1.dist_logits(b_obs[mb1], b_actions[mb1])
                        lg2 = q2.dist_logits(b_obs[mb2], b_actions[mb2])
                        with torch.no_grad():
                            t1 = q_support.project(q_targets_onpol[mb1])
                            t2 = q_support.project(q_targets_onpol[mb2])
                        onpol_loss = args.q_onpol_coef * (
                            -(t1 * torch.log_softmax(lg1, dim=-1)).sum(-1).mean()
                            - (t2 * torch.log_softmax(lg2, dim=-1)).sum(-1).mean()
                        )
                q_optimizer.zero_grad(set_to_none=True)
                onpol_loss.backward()
                nn.utils.clip_grad_norm_(q_params, 0.5)
        ...
        with torch.no_grad():
            ...
            # v42: measure the frontier gap under the SAME pessimism combine as the DPG numerator, so
            # the ascent term (numerator/normalizer) stays in one consistent Q unit.
            _fqr = combine_q(q1(_fs, _fa), q2(_fs, _fa), args.q_pessimism, args.q_beta)
            _fqp = combine_q(q1(_fs, _fp), q2(_fs, _fp), args.q_pessimism, args.q_beta)
        ...
        _tminq = combine_q(q1(b_obs[_tel], _ta_env), q2(b_obs[_tel], _ta_env), args.q_pessimism, args.q_beta)
        ...
        with torch.no_grad():
            ...
            q_tgt = b_returns_raw / q_scale
            _num = (q_tgt - q_on).var(unbiased=False)
            _den = q_tgt.var(unbiased=False)
            q_ev = float("nan") if _den.item() == 0 else (1.0 - (_num / _den)).item()
            q_mean = q_on.mean().item() * q_scale
            q_twin_gap = (torch.cat(gap_chunks).mean() / (torch.cat(absq_chunks).mean() + 1e-8)).item()
            # v42: twin std / |mean| (std of two twins = |q1-q2|/2). Feeds the optimist pessimism mode
            # (mean + q_beta*std) — a running read on how much upside the dial adds.
            q_std_over_mean = (torch.cat(gap_chunks).mean() * 0.5 / (q_on.mean().abs() + 1e-8)).item()
            ...
            if args.q_onpol_only:
                ...
                _y = rep_nstep_rew[_r] / q_scale + rep_nstep_disc[_r] * combine_q(
                    q1_target(_sb, _ab), q2_target(_sb, _ab), args.q_pessimism, args.q_beta
                )
                _pred = combine_q(q1(_so, rep_act[_r]), q2(_so, rep_act[_r]), args.q_pessimism, args.q_beta)
        ...
        if args.q_onpol_only:
            ...
            lap_priority_mean = float("nan")
        else:
            q_loss_mean = (q_loss_accum / args.q_updates_per_iter).item()
            q_value_mean = (q_val_accum / args.q_updates_per_iter).item()
            lap_priority_mean = (lap_prio_sum / args.q_updates_per_iter).item() if args.lap else float("nan")
        ...
        acc_search_top_gap = torch.zeros((), device=device) # v41: E[max_i Q_i - minQ(s, a_realized)] (search headroom)
        acc_search_ess = torch.zeros((), device=device)     # v41: mean effective sample size of the search weights
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                with amp:  # v41 perf: bf16 autocast over the teacher forwards (GEMMs bf16; Beta/CE fp32)
                    ...
                    minq_fresh = combine_q(q1(mb_obs, t_a_env), q2(mb_obs, t_a_env), args.q_pessimism, args.q_beta)
                    used_replay = teacher_replay_ok and rep_filled > 0
                    if used_replay:
                        ...
                        minq_rep = combine_q(q1(r_obs, r_a_env), q2(r_obs, r_a_env), args.q_pessimism, args.q_beta)
                        minq_mix = torch.cat([minq_fresh, minq_rep])
                    else:
                        minq_mix = minq_fresh
                    teacher_dpg = args.teacher_q_coef * teacher_dpg_atten * (-minq_mix.mean() / q_frontier_norm_t)

                    # v41 SAMPLED-SEARCH operator (global E-step): on the SAME fresh+replay states, draw
                    # search_k detached actions per state from the CURRENT teacher Beta, rank by
                    # min(Q1,Q2), softmax (in q_frontier units) to per-state weights, and pull the
                    # teacher density toward its own Q-best proposals via weighted NLL. Well-conditioned
                    # at the action rails where the DPG gradient is sick; grad flows to the teacher actor
                    # only (samples + weights carry no grad). Compounds with the local DPG term.
                    if args.search_coef > 0.0:
                        if used_replay:
                            s_obs_srch = torch.cat([mb_obs, r_obs], dim=0)
                            s_alpha_srch = torch.cat([t_alpha, r_alpha], dim=0)
                            s_beta_srch = torch.cat([t_beta, r_beta], dim=0)
                        else:
                            s_obs_srch, s_alpha_srch, s_beta_srch = mb_obs, t_alpha, t_beta
                        Ssz, k = s_obs_srch.shape[0], args.search_k
                        with torch.no_grad():
                            z_k = Beta(s_alpha_srch.detach(), s_beta_srch.detach()).sample((k,)).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                            a_k = q_act_low + (q_act_high - q_act_low) * z_k          # (k, S, A)
                            obs_rep = s_obs_srch.unsqueeze(0).expand(k, Ssz, -1).reshape(k * Ssz, -1)
                            q_k = combine_q(
                                q1(obs_rep, a_k.reshape(k * Ssz, -1)), q2(obs_rep, a_k.reshape(k * Ssz, -1)),
                                args.q_pessimism, args.q_beta,
                            ).reshape(k, Ssz)                                        # (k, S) ranking values
                            w_k = torch.softmax(
                                (q_k - q_k.max(dim=0, keepdim=True).values) / (args.search_tau * q_frontier_norm_t), dim=0
                            )                                                        # (k, S), sums to 1 over k
                            search_ess_mb = (1.0 / (w_k * w_k).sum(dim=0).clamp_min(1e-8)).mean()
                            q_k_fresh_max = q_k[:, : mb_obs.shape[0]].max(dim=0).values
                        logp_k = Beta(s_alpha_srch, s_beta_srch).log_prob(z_k).sum(-1)  # (k, S) grad -> teacher
                        search_term = args.search_coef * (-(w_k * logp_k).sum(dim=0).mean())
                    else:
                        search_term = mb_obs.new_zeros(())
                    ...
                    # direction as the distill anchor; grad flows to the teacher only. Both the DPG and
                    # search operators live INSIDE this envelope.
                    with torch.no_grad():
                    ...
                    teacher_loss = (
                        teacher_actor_loss + args.teacher_vf_coef * t_v_loss
                        + teacher_dpg + search_term + kl_penalty
                    )
                ...
                with torch.no_grad():
                    minq_real = combine_q(q1(mb_obs, b_actions[mb_inds]), q2(mb_obs, b_actions[mb_inds]), args.q_pessimism, args.q_beta)
                    acc_teacher_qgap += minq_fresh.detach().mean() - minq_real.mean()
                    acc_teacher_kl += teacher_kl_mb.detach()
                    if args.search_coef > 0.0:
                        acc_search_top_gap += (q_k_fresh_max - minq_real).mean()
                        acc_search_ess += search_ess_mb
        ...
        search_top_gap = (acc_search_top_gap / _tden).item() if args.search_coef > 0.0 else float("nan")
        search_ess = (acc_search_ess / _tden).item() if args.search_coef > 0.0 else float("nan")
        ...
        if teacher_mb_count > 0:
            lam_kl = dual_update_lam(
                lam_kl, total_teacher_kl, args.teacher_kl_budget, args.teacher_kl_eta,
                args.teacher_kl_lam_min, args.teacher_kl_lam_max,
            )
        ...
        # v42 Q-substrate telemetry: the running return-scale normalizer, twin disagreement (feeds
        # optimist mode), and the mean LAP priority (post-^alpha; >1 means high-error focus is active).
        writer.add_scalar("debug/value_scale", value_scale, global_step)
        writer.add_scalar("debug/q_std_over_mean", q_std_over_mean, global_step)
        writer.add_scalar("debug/lap_priority_mean", lap_priority_mean, global_step)
        ...
        writer.add_scalar("debug/search_top_gap", search_top_gap, global_step)
        writer.add_scalar("debug/search_ess", search_ess, global_step)
        ...
        # v40 ONE PRIVILEGE BUDGET telemetry. (debug/teacher_kl_total dropped in v41 — it was
        # bit-identical to losses/distill_kl; one canonical tag is cleaner.)
