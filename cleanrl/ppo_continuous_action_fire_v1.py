# PPO + FIRE (Frobenius-Isometry Re-initialization)
#
# FIRE: orthogonalize Linear weights via Newton-Schulz iteration at scheduled
# checkpoints during training. The closed-form solution is the nearest
# semi-orthogonal matrix to the current weights (minimizes Squared Frobenius
# Error subject to Deviation-from-Isometry = 0). FIRE approximates it with a
# few Newton-Schulz steps, scaled Muon-style by sqrt(d_out / d_in).
#
# Hypothesis: even with PPO's modest update budget, repeated re-orthogonalization
# preserves plasticity / effective rank and unlocks more learning over an 8M
# horizon vs. baseline PPO that gradually loses isotropy.
#
...
def fire_reinit(model: nn.Module, num_iters: int = 10):
    """Apply FIRE: replace each Linear weight with its nearest semi-orthogonal
    matrix (Newton-Schulz approx), scaled Muon-style by sqrt(d_out / d_in).
