# PPO + IterThink v24.19 (AUTOREG BETA SOURCE + Y-REPLAY LOGIT FLOW).
# From v24.16 and v24.18.
#
# Actor ablation: replace v24.18's independent Beta source with v24.16's
# teacher-forced autoregressive Beta source, then apply the same exact y-replay
# additive logit flow and positive-only CFM auxiliary:
#     z0 ~ p_AR(z|s); y0 = logit(z0); y1 = Flow_s(y0)
#     action = low + (high-low) * sigmoid(y1)
# PPO replay stores y1, inverts the current flow to recover z0, and evaluates
# p_AR(z0|s) with z0 as the teacher-forced prefix. This composes AR bounded source
# covariance with v24.18's residual logit-space transport while preserving exact
# PPO ratios up to the usual numerical Beta sample clamp.
#
...
# flow. v24.18 stores y1, so replay remains exact even when y1 is far from zero.
#
...
class Args:
    ...
    # v24.9: action distribution. "beta_flow" (Beta source + logit coupling flow)
...
class AdditiveVelocityCouplingBlock(nn.Module):
    def __init__(self, action_dim, cond_dim, flip=False):
        super().__init__()
        ...
        self.net = nn.Sequential(
            ...
            nn.SiLU(),
            layer_init(nn.Linear(hidden_dim, right_dim), std=0.0),
        )
    ...
    def _velocity(self, left, cond, t):
        if t.ndim == 0:
            t = t.expand(left.shape[0], 1)
        elif t.ndim == 1:
            t = t.unsqueeze(-1)
        t = t.to(dtype=left.dtype, device=left.device)
        return self.net(torch.cat((cond, left, t), dim=-1))

    def forward(self, y, cond, t, dt):
        left, right = self._split(y)
        right = right + dt * self._velocity(left, cond, t)
    ...
    def cfm_loss(self, y_t, cond, t, target_velocity, weights):
        left, _ = self._split(y_t)
        _, target_right = self._split(target_velocity)
        pred_right = self._velocity(left, cond, t)
        per_sample = (pred_right - target_right).square().mean(dim=-1)
        return (weights * per_sample).mean()


class BetaLogitActionFlow(nn.Module):
    def __init__(self, action_dim, cond_dim, flow_steps):
        super().__init__()
        self.flow_steps = int(flow_steps)
        self.blocks = nn.ModuleList(
            AdditiveVelocityCouplingBlock(action_dim, cond_dim, flip=bool(i % 2))
    ...
    def forward(self, y0, cond):
        ...
        for i, block in enumerate(self.blocks):
            y = block(y, cond, self._time(i, y.shape[0], y.device, y.dtype), dt)
        return y
    ...
    def inverse(self, y1, cond):
        ...
        for i in reversed(range(self.flow_steps)):
            block = self.blocks[i]
            y = block.inverse(y, cond, self._time(i, y.shape[0], y.device, y.dtype), dt)
        return y
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v24.9: Beta logit-flow default, with v24 beta/diagonal controls.
        ...
        if self.actor_dist == "gaussian":
            ...
            self.autoreg_alpha_weight = nn.Parameter(torch.zeros(act_dim, act_dim))
            self.autoreg_beta_weight = nn.Parameter(torch.zeros(act_dim, act_dim))
            self.register_buffer("autoreg_mask", torch.tril(torch.ones(act_dim, act_dim), diagonal=-1))
            ...
            if self.actor_dist == "beta_flow":
                self.action_flow = BetaLogitActionFlow(act_dim, H, args.beta_flow_steps)
    ...
    def _autoreg_residual(self, z):
        z_centered = z - 0.5
        alpha_resid = F.linear(z_centered, self.autoreg_alpha_weight * self.autoreg_mask)
        beta_resid = F.linear(z_centered, self.autoreg_beta_weight * self.autoreg_mask)
        return alpha_resid, beta_resid

    def _beta_source_dist(self, actor_feat, z=None):
        base_alpha = self.actor_alpha_head(actor_feat)
        base_beta = self.actor_beta_head(actor_feat)
        if z is None:
            alpha_resid = torch.zeros_like(base_alpha)
            beta_resid = torch.zeros_like(base_beta)
        else:
            alpha_resid, beta_resid = self._autoreg_residual(z)
        alpha = 1.0 + F.softplus(base_alpha + alpha_resid)
        beta = 1.0 + F.softplus(base_beta + beta_resid)
        return Beta(alpha, beta)

    def _sample_autoreg_z(self, actor_feat):
        z = actor_feat.new_full((actor_feat.shape[0], self.act_dim), 0.5)
        for i in range(self.act_dim):
            dist = self._beta_source_dist(actor_feat, z)
            zi = Beta(dist.concentration1[:, i], dist.concentration0[:, i]).sample()
            z[:, i] = zi.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        return z

    def _autoreg_mean_z(self, actor_feat):
        z = actor_feat.new_full((actor_feat.shape[0], self.act_dim), 0.5)
        for i in range(self.act_dim):
            dist = self._beta_source_dist(actor_feat, z)
            z[:, i] = dist.mean[:, i].clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        dist = self._beta_source_dist(actor_feat, z)
        return z, dist
    ...
    def _flow_logprob(self, dist, z0, y0, y1):
        return (
    ...
    def beta_flow_cfm_loss(self, x, target_y1, advantages, weight_clip):
        if self.actor_dist != "beta_flow":
        ...
        with torch.no_grad():
            z0 = self._sample_autoreg_z(actor_feat.detach())
            y0 = safe_logit(z0)
            target_z = torch.sigmoid(target_y1.detach()).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            ...
            y1 = safe_logit(Beta(target_alpha, target_beta).sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS))
        ...
        return self.action_flow.cfm_loss(y0, y1, cfm_feat, weights)

    def get_beta_flow_stats(self, x):
        if self.actor_dist != "beta_flow":
            raise RuntimeError("get_beta_flow_stats is only valid for beta_flow actor_dist")
        actor_feat, _ = self._trunks(x)
        source_mean, dist = self._autoreg_mean_z(actor_feat)
        y0 = safe_logit(source_mean)
        y1 = self.action_flow(y0, actor_feat)
        z1 = torch.sigmoid(y1).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        alpha_resid, beta_resid = self._autoreg_residual(source_mean)
        return {
            "source_alpha": dist.concentration1,
            "source_beta": dist.concentration0,
            "autoreg_residual": torch.cat([alpha_resid, beta_resid], dim=-1),
            "flow_delta": y1 - y0,
            "flowed_mean_z": z1,
        }
    ...
    def get_action_and_value(self, x, z=None):
        ...
        if self.actor_dist == "beta_flow":
            if z is None:
                z0 = self._sample_autoreg_z(actor_feat)
                y0 = safe_logit(z0)
                y1 = self.action_flow(y0, actor_feat)
                z1 = torch.sigmoid(y1)
                replay_z = y1
                ...
                y1 = z
                y0 = self.action_flow.inverse(y1, actor_feat)
                z0 = torch.sigmoid(y0).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                z1 = torch.sigmoid(y1)
                replay_z = y1
            dist = self._beta_source_dist(actor_feat, z0)
            action = self._z_to_action(z1)
            log_prob = self._flow_logprob(dist, z0, y0, y1)
            entropy = -log_prob
    ...
    def actor_parameters(self):
        ...
        if self.actor_dist == "gaussian":
            ...
            heads = (
                ...
                + [self.autoreg_alpha_weight, self.autoreg_beta_weight]
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.actor_dist == "beta_flow":
            with torch.no_grad():
                ...
                writer.add_scalar(
                    "debug/autoreg_residual_rms",
                    flow_stats["autoreg_residual"].pow(2).mean().sqrt().item(),
                    global_step,
                )
                writer.add_scalar("debug/beta_flow_flowed_mean_z_std", flow_stats["flowed_mean_z"].std().item(), global_step)
                replay_y = b_latent_zs
                replay_z = torch.sigmoid(replay_y)
                writer.add_scalar("debug/beta_flow_replay_y_abs_mean", replay_y.abs().mean().item(), global_step)
                writer.add_scalar("debug/beta_flow_replay_y_abs_max", replay_y.abs().max().item(), global_step)
                writer.add_scalar(
                    "debug/beta_flow_replay_z_boundary_frac",
                    ((replay_z <= SAMPLE_EPS) | (replay_z >= 1.0 - SAMPLE_EPS)).float().mean().item(),
                    global_step,
                )
