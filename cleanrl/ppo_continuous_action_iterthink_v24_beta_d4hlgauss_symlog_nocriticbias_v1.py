# PPO + IterThink v24 beta d4-HL-Gauss symlog critic, no critic bias v1.
#
...
# and PPO settings stay matched to d4-HL-Gauss. This ablation removes only the
# Gaussian logit bias from the critic init, leaving layer_init's random weights
# and zero bias in place. The support edges are linear in
# symlog(v_min)..symlog(v_max), matching hl_gauss_pytorch's transform contract
# while preserving d4's raw normalized-return range.
#
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Categorical critic. This ablation deliberately leaves layer_init's
        # zero bias in place instead of applying d4hlgauss' peaked zero-return
        # Gaussian-logit prior.
