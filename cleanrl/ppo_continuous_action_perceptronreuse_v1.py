# PPO + Perceptron Reuse v1: re-applying earlier perceptrons on a PLAIN MLP trunk.
#
# METHOD. A plain pre-LN residual MLP trunk. Each block i introduces ONE perceptron
# f_i = _mlp_body(H) (Linear->ReLU^2->Linear). Crucially, at block i the network may
# re-apply ANY perceptron introduced so far {f_0..f_i} to its CURRENT running state z,
# then combine the results into the residual update. The perceptrons become reusable
# subroutines composed into an input-specific circuit. Three combination modes
# (--route-mode):
#   "own"   : weight 1.0 on f_i only (plain sequential residual MLP; control). No extra
#             bodies are evaluated -> a true plain-MLP control with no extra FLOPs.
#   "static": w = softmax(alphas[i]) over the available {f_0..f_i}; input-INDEPENDENT
#             convex combination of perceptron outputs.
#   "dyn"   : w = softmax(routers[i](z)) over {f_0..f_i}; input-DEPENDENT convex
#             combination (the current state selects which perceptrons to compose).
# Optional top_k sparsifies static/dyn: keep the top_k weights, renormalize to a convex
# combination (run only the kept bodies for "dyn"/static). Block update at i:
#   z = lns[i](h); update = sum_{j<=i} w_j * bodies[j](z); h = h + update.
# Output = final_ln(h); output dim = H. The same trunk class builds two independent
# instances (separate actor/critic trunks), wired into the unchanged Beta actor /
# HL-Gauss symlog distributional critic heads.
#
# NOVELTY vs depthmix. depthmix reused previous *representations* (read earlier layer
# OUTPUTS via depth mixing). This reuses previous *perceptrons*: it RE-APPLIES earlier
# layers' FUNCTIONS/weights to the current state. "own" is exactly depthmix's "seq"
# plain-MLP control (shared baseline). The lever under test is function reuse, not
# output reuse.
#
# HYPOTHESIS. On dense low-dimensional proprioceptive states, composing/reusing a
# cumulative library of perceptrons (calling earlier subroutines on fresh state) gives
# greater parameter-efficient capacity than a feed-forward stack of equal depth, and an
# input-dependent router lets the circuit specialize per state.
#
...
# all hyperparameters are inherited byte-for-byte from the depthmix base;
# ONLY the trunk network is replaced.
import os
import random
import time
...
class Args:
    ...
    share_backbone: bool = True      # one PerceptronReuseTrunk for both actor and critic heads
    ...
    # PerceptronReuse trunk (plain MLP + reuse of earlier perceptrons; NO MoE).
    #   route_mode: "own" (sequential residual; plain MLP control)
    #               | "static" (input-independent softmax over available perceptrons)
    #               | "dyn" (input-dependent router over available perceptrons).
    route_mode: str = "dyn"
    reuse_top_k: int = 0             # 0 = dense soft over all available; >0 = keep top_k
    trunk_hidden: int = 128          # H
    trunk_blocks: int = 8            # number of perceptron-reuse blocks
    trunk_heads: int = 4             # UNUSED by this trunk (kept for Args compat; harmless)
    trunk_d_qk: int = 32             # UNUSED by this trunk (kept for Args compat; harmless)
...
class PerceptronReuseTrunk(nn.Module):
    """Plain-MLP trunk that RE-APPLIES earlier perceptrons to the current state.

    Each block i introduces one perceptron f_i = _mlp_body(H). At block i the network
    may apply ANY perceptron introduced so far {f_0..f_i} to its current running state
    z = lns[i](h), and combine the results into the residual update. Modes:
      "own"   : weight 1.0 on f_i only (plain sequential residual MLP; control). No
                other bodies are evaluated.
      "static": w = softmax(alphas[i]) over the i+1 available perceptrons
                (input-independent convex combination).
      "dyn"   : w = softmax(routers[i](z)) over the i+1 available perceptrons
                (input-dependent convex combination).
    Optional top_k sparsifies static/dyn: keep the top_k weights, renormalize to sum
    to 1 (still a convex combination), and for the sparse path only run the kept
    bodies. Block update: h = h + sum_{j<=i} w_j * bodies[j](z). Output = final_ln(h).
    Output dim = H. Reuse: applying bodies[j] (j<i) at block i re-invokes perceptron j
    on fresh state; lns[i] is the per-call-site (per-block) norm, bodies[j] the reused
    function.
    """

    def __init__(self, in_dim, H, n_blocks, route_mode, mlp_mult, top_k):
        super().__init__()
        assert route_mode in ("own", "static", "dyn"), f"unknown route_mode {route_mode}"
        ...
        # The perceptron library: f_i introduced at block i (reused at later blocks).
        self.bodies = nn.ModuleList([_mlp_body(H, mlp_mult) for _ in range(n_blocks)])
        # Per-block (per-call-site) pre-LN applied to the running state before reuse.
        self.lns = nn.ModuleList([nn.LayerNorm(H) for _ in range(n_blocks)])

        if route_mode == "static":
            # Static input-independent combination weights over the i+1 available
            # perceptrons at block i (indices 0..i).
            self.alphas = nn.ParameterList(
                [nn.Parameter(torch.zeros(i + 1)) for i in range(n_blocks)]
            )
        elif route_mode == "dyn":
            # Input-dependent router producing i+1 logits over the available perceptrons.
            self.routers = nn.ModuleList(
                [layer_init(nn.Linear(H, i + 1)) for i in range(n_blocks)]
            )

        self.final_ln = nn.LayerNorm(H)

    def _combine_weights(self, i, z):
        # Returns combination weights w over the i+1 available perceptrons (0..i).
        # For "static"/"dyn" w is a length-(i+1) convex combination (nonneg, sums to 1).
        if self.route_mode == "static":
            w = torch.softmax(self.alphas[i], dim=0)          # (i+1,)
        else:  # dyn
            w = torch.softmax(self.routers[i](z), dim=-1)     # (B, i+1)
        if 0 < self.top_k < i + 1:
            # Keep only the top_k weights, zero the rest, renormalize to sum to 1.
            topv, topi = torch.topk(w, self.top_k, dim=-1)
    ...
    def _block_update(self, i, z):
        # Apply the available perceptrons {f_0..f_i} to the SAME current-state input z
        # and combine them by the per-mode weights. update has shape (B, H).
        if self.route_mode == "own":
            # Plain-MLP control: only f_i, no extra bodies/FLOPs.
            return self.bodies[i](z)
        w = self._combine_weights(i, z)                       # static:(i+1,) dyn:(B,i+1)
        if 0 < self.top_k < i + 1:
            # Sparse: run only the kept bodies. For "dyn" the kept set is per-sample, so
            # we cannot prune the unique-body set generally; run all available but mask.
            # For "static" the kept set is shared -> run only those bodies.
            if self.route_mode == "static":
                topi = torch.nonzero(w > 0, as_tuple=False).flatten().tolist()
                outs = torch.stack([self.bodies[j](z) for j in topi], dim=1)  # (B,k,H)
                wk = w[topi].view(1, -1, 1)                                   # (1,k,1)
                return (wk * outs).sum(dim=1)
            # dyn sparse: per-sample kept set differs -> evaluate all available, mask via w.
            outs = torch.stack([self.bodies[j](z) for j in range(i + 1)], dim=1)  # (B,i+1,H)
            return (w.unsqueeze(-1) * outs).sum(dim=1)
        # Dense soft over all available perceptrons.
        outs = torch.stack([self.bodies[j](z) for j in range(i + 1)], dim=1)  # (B,i+1,H)
        if self.route_mode == "static":
            return (w.view(1, -1, 1) * outs).sum(dim=1)
        return (w.unsqueeze(-1) * outs).sum(dim=1)            # dyn: (B,i+1,1)*(B,i+1,H)

    def forward(self, x):
        h = self.entry(x)
        for i in range(self.n_blocks):
            z = self.lns[i](h)
            h = h + self._block_update(i, z)
