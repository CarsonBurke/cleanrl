# PPO + IterThink v66 (v24 Beta + parameter-golf affine final norm). From v24.
#
...
#     length, not channel width, so every current obs/latent token can attend to
#     all previous loop-bank tokens plus the current bank without changing block
#     parameter shapes.
#   - The recurrent loop bank contains observation tokens plus shared,
#     non-stratified latent tokens. These are the tokens that carry information
...
#     traces, but are never appended to the future latent ledger and do not carry
...
#     This is PMA-style pooling over loop-prefix token sets: learned role seeds
#     cross-attend to each prefix and concatenate the resulting per-depth summaries.
#   - v63 ports the parameter-golf residual-stability recipe: learned two-lane
#     residual mixing against the initial token bank, attention/FFN residual
#     scales start at 1.0, and the recurrent work block's residual-producing
#     projections start at zero while the inner branch weights use Xavier. The
#     PMA read block keeps Xavier output projections because its unconditioned
#     role seed would otherwise be state-blind at initialization.
#   - v64 adds a final role-feature RMSNorm after the actor/critic readout
#     projections. v63 normalized before the role projection but not before the
#     PPO heads; parameter-golf normalizes the final representation before the
#     head, which should reduce late actor/critic gradient scale drift.
#   - v65 keeps v64's learning behavior but removes per-minibatch host syncs from
#     the debug raw-grad breakdown.
#   - v66 makes the final actor/critic feature RMSNorm affine. v65's non-affine
#     final norm fixed raw gradient scale but underfit by preventing the Beta heads
#     from using readout magnitude to sharpen the policy.
#
...
class Args:
    ...
    hidden: int = 64
    k_blocks: int = 3
    loop_latent_tokens: int = 8
    transformer_heads: int = 4
    transformer_ffn_mult: int = 4
    transformer_resid_init: float = 1.0
    transformer_q_gain_init: float = 1.0
...
class AffineRMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return x * rms * self.weight.to(dtype=x.dtype)
...
class LoopBankBlock(nn.Module):
    ...
    def __init__(self, H, num_heads, ffn_mult, resid_init, q_gain_init, zero_residual_proj=False):
        super().__init__()
        ...
        self.resid_mix = nn.Parameter(torch.stack((torch.ones(H), torch.zeros(H))).float())
        ...
        for module in (self.wo, self.ff2):
            if zero_residual_proj:
                zero_init_linear_(module)
            else:
                init_linear_(module, init_scale=1.0)
    ...
    def forward(self, current, context, initial):
        B, S, H = current.shape
        C = context.shape[1]
        mix = self.resid_mix.to(dtype=current.dtype)
        x_in = mix[0].view(1, 1, H) * current + mix[1].view(1, 1, H) * initial
...
class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, args):
        ...
        self.H = H
        self.K = K
        self.num_latent_tokens = args.loop_latent_tokens
        self.num_read_tokens = 1
        self.use_bf16 = args.transformer_bf16
        self.entry = init_linear_(nn.Linear(in_dim, H))
        ...
        self.latent_tokens = nn.Parameter(torch.empty(self.num_latent_tokens, H))
        ...
        self.actor_out_proj = init_linear_(nn.Linear(read_dim, H))
        self.critic_out_proj = init_linear_(nn.Linear(read_dim, H))
        self.actor_feat_norm = AffineRMSNorm(H)
        self.critic_feat_norm = AffineRMSNorm(H)
    ...
    def _initial_work_bank(self, x):
        ...
        latent_tokens = (
            self.latent_tokens.unsqueeze(0).expand(B, -1, -1)
            + self.type_embed[1].view(1, 1, -1)
            + x0.unsqueeze(1)
        )
        return torch.cat([obs_tokens, latent_tokens], dim=1), x0
    ...
    def _read_trace(self, ledger, x0, role):
        ...
        return feat_norm(out_proj(out_norm(flat)))
...
class Agent(nn.Module):
    ...
    def _trunks(self, x):
        ...
        # computes separate actor/critic role-CLS streams with shared loop weights.
        if self.share_backbone:
            return self.trunk(x)
    ...
    def actor_parameters(self):
        ...
        return list(trunk.parameters()) + self.actor_head_parameters()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                actor_trunk_raw_gn = None
                actor_head_raw_gn = None
                critic_trunk_raw_gn = None
                critic_head_raw_gn = None
                ...
                if args.separate_grad_clip:
                    ...
                    critic_trunk = agent.trunk if agent.share_backbone else agent.critic_trunk
                    critic_trunk_raw_gn = grad_norm_tensor(critic_trunk.parameters())
                    critic_head_raw_gn = grad_norm_tensor(agent.critic_head.parameters())
                    ...
                    actor_trunk = agent.trunk if agent.share_backbone else agent.actor_trunk
                    actor_trunk_raw_gn = grad_norm_tensor(actor_trunk.parameters())
                    actor_head_raw_gn = grad_norm_tensor(agent.actor_head_parameters())
        ...
        writer.add_scalar("debug/actor_trunk_grad_norm_raw", actor_trunk_raw_gn if actor_trunk_raw_gn is not None else 0.0, global_step)
        writer.add_scalar("debug/actor_head_grad_norm_raw", actor_head_raw_gn if actor_head_raw_gn is not None else 0.0, global_step)
        writer.add_scalar("debug/critic_trunk_grad_norm_raw", critic_trunk_raw_gn if critic_trunk_raw_gn is not None else 0.0, global_step)
        writer.add_scalar("debug/critic_head_grad_norm_raw", critic_head_raw_gn if critic_head_raw_gn is not None else 0.0, global_step)
