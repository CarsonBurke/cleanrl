# Continuous Kaggriculture-style TPO on the no-batch residual SiTU-GLU/nGPT v7 control.
# Use the newest v25 runtime, but default to original actor/critic GAE lambda=.95
# and scalar MSE. Reward normalization is bypassed in warmup and training; raw
# rewards are neither rescaled nor clipped. Observation normalization stays on.
# Hypothesis: removing evolving reward scale can improve learning, with separate
# tests of global gradient clipping and value clipping. The value radius remains
# +/-0.2 in the chosen reward units, intentionally raw units when norm_reward=False.
import json
import math
import os
import random
...
class Args:
    ...
    """normalize and clip rewards with VectorRewardNorm; false keeps raw rewards"""
    clip_coef: float = 0.2
    ...
    """scalar critic objective; values decode to the chosen reward-return units"""
    critic_target: Literal["gae", "vapo", "episode_mc"] = "gae"
    ...
    policy_objective: Literal["tpo", "clip"] = "tpo"
    """Kaggriculture-style continuous TPO, or asymmetric clipped control"""
    tpo_eta: float = 2.0
    """TPO target temperature"""
    utility_clip: float | None = 3.0
    """clip rollout-whitened utilities to bound target tilt"""
    tpo_cell_width: float = 1e-3
    """per-coordinate action-cell width for density-to-mass conversion"""
...
class Agent(nn.Module):
    ...
    def get_value(self, x):
        """Return decoded V in the same discounted-return units as training rewards."""
...
def tpo_kagg_loss(logratio_components, old_logprobs, utility, eta, cell_width):
    """Kaggriculture target with continuous-density scale normalization.

    Kaggriculture's categorical KL gradient is ``p_new - q``.  A continuous
    action cell has tiny mass ``p_old = density * width``, so using that raw
    gradient makes the actor signal vanish in magnitude and lets Adam's
    normalization over-steer.  Dividing the cell KL by p_old preserves its
    exact target direction and yields the density-ratio gradient
    ``r - q/p_old``.  The target is still q from Kaggriculture, rather than
    the direct exp(utility / eta) target used by the older TPO family.
    """
    if not 0.0 < cell_width < 1.0:
        raise ValueError("tpo_cell_width must be in (0, 1)")
    log_cell = math.log(cell_width)
    old_mass = (old_logprobs.detach() + log_cell).clamp(-30.0, -1e-6)
    ...
    log_target_ratio = torch.log(q.clamp_min(1e-12)) - old_mass
    target_ratio = log_target_ratio.exp()
    logratio = logratio_components
    ratio = logratio.clamp_max(20.0).exp()
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args,
             critic_mask=None, utilities=None):
    ...
    if args.policy_objective == "tpo":
        ...
        pg_loss = tpo_kagg_loss(logratio_components, old_component_logprobs, utility, args.tpo_eta, args.tpo_cell_width).sum(-1).mean()
    elif args.policy_objective == "clip":
        effective = torch.where(advantages >= 0, logratio.clamp_max(float(np.log(1 + args.clip_coef_upper))), logratio.clamp_min(float(np.log(1 - args.clip_coef))))
        pg_loss = -(effective.exp() * advantages).mean()
    else:
...
def validate_args(args):
    if args.policy_objective not in {"tpo", "clip"} or args.tpo_eta <= 0.0 or not math.isfinite(args.tpo_eta):
        raise ValueError("invalid TPO objective or eta")
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", f"Beta Kaggriculture-style TPO; eta={args.tpo_eta}; cell={args.tpo_cell_width}; no-batch v7 residual actor")
        ...
        writer.add_text("critic_objective", f"{args.critic_loss}; gradient clipping={args.grad_clip}; reward normalization={args.norm_reward}; value clipping={args.clip_vloss}; decoded GAE/V in training reward units")
        ...
        def loss_model(observations, native, old_logprobs, advantages, returns, old_values, critic_mask, utilities):
            return ppo_loss(agent, observations, native, old_logprobs, advantages, returns, old_values, args,
                            critic_mask=critic_mask, utilities=utilities)
        ...
        rew_norm = VectorRewardNorm(args.num_envs, args.gamma) if args.norm_reward else None
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            for step in range(args.num_steps):
                ...
                with timer.span("normalize_transfer", use_cuda=False):
                    reward = raw_reward if rew_norm is None else rew_norm.normalize(raw_reward, terms)
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                # GAE uses decoded values in the same units as the staged rewards;
                # symlog, when selected, changes critic coordinates only.
                ...
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss, metrics = loss_model(
                            ...
                            b_critic_mask[indices], b_utilities[indices],
                        )
        ...
        checkpoint = {
            ...
            } if rew_norm is not None else None,
