# nGPT hypersphere satransformer v9-hlcdf-delta-signmag: v7 decoupled
# role-token encoder/decoders with sign-anchored baseline-centered critic-CDF
# policy advantages.
#
...
#
# Diff vs v9-HLCDF-delta: use the baseline-centered CDF delta only as a
# magnitude, while forcing the policy sign to match raw GAE:
# sign(raw_gae) * tanh(abs(z_ret - z_base) / kappa). This tests whether HLCDF
# magnitude is useful before critic CDF calibration is reliable enough to own
# PPO sign. Minibatches use fixed batch target RMS scaling (default 0.7), not
# per-minibatch unit RMS. No target-KL stopping.
#
...
class Args:
    ...
    adv_transform: str = "hlcdf_delta_signmag"
