# PPO + IterThink v24 beta d4-C51 DGAE v3 (C51-CORRECT distributional critic).
#
# WHY v3. v2 (symlog + recursive HL-Gauss distributional lambda-target) REGRESSED
# badly vs v1 on HalfCheetah (3878 vs v1's 4642@1.3M, climbing to 7275). The only
# diff was the critic target, and the trace showed exactly why:
#   - critic width sigma_mean 0.79 vs v1's 0.265 (3x too WIDE),
#   - explained_variance collapsed 0.6 -> 0.075,
#   - decode_mae 0.10 vs v1's 0.015, value_loss 4.48 vs 2.66.
# Two implementation errors, both diagnosed by comparison to CleanRL's c51.py:
#   (1) PROJECTION-DIFFUSION. v2 built the target by RECURSIVE re-projection with a
#       sigma=2-bin GAUSSIAN (HL-Gauss) projection. Gaussian smoothing adds width on
#       EVERY one of the ~2048 recursion steps; the steady state is ~3x the proj floor
#       even for a self-consistent point-mass value fn. C51 instead uses a 2-bin
#       LINEAR-INTERPOLATION projection (c51.py:217-227) that is MEAN-EXACT and adds
#       ~zero width (a target landing on a grid point stays a point mass).
#   (2) DECODE SPLIT. On a symlog support the value bootstrap uses symexp(E[z]) while
#       the advantage uses E[symexp(z)]; on a wide Z these diverge (Jensen). C51 uses
#       a LINEAR support, where the only mean is E[Z]=sum(p*atom) -- one decode, used
#       everywhere, no Jensen gap.
#
# v3 = "do it the C51 way", extrapolated to PPO+GAE:
#   - LINEAR support (value_symlog=False), atoms in raw normalized-return units,
#     v_min/v_max=+-20 (returns settle ~3, absmax peaked 15.8 -> comfortable margin).
#     => to_scalar == distributional_raw_mean == E[Z]; the decode split VANISHES.
#   - C51 2-bin LINEAR-INTERPOLATION projection (`c51_project`) for the critic target,
#     replacing the per-atom HL-Gauss Gaussian => mean-exact, diffusion-free.
#   - Distributional TD(lambda) target via the same backward mixture recursion but with
#     c51_project. `--dist-target-mode` selects "td_lambda" (GAE-lambda mixture) or
#     "td0" (pure 1-step C51 Bellman, lambda=0) to ABLATE whether residual recursive
#     diffusion still matters; "scalar" falls back to v1's scalar-return projection.
# Expectation: critic width drops back toward the true return spread (~0.27), EV
# recovers, and v3 at least MATCHES v1 -- a precondition before layering a genuinely
# shape-aware (risk/distortion) advantage in v4, since with a clean sharp Z the
# distribution shape is finally trustworthy.
#
# NOTE on the advantage: with a linear support the DGAE advantage (E[symexp]) and the
# scalar GAE (symexp(E)) both collapse to E[Z], so they are now IDENTICAL and correct.
# Linear-cost DGAE is mean-only by construction (paper Def.2); shape-aware advantages
# are deferred to v4 once this clean critic is validated.
#
# --- inherited DGAE notes (arXiv:2507.17530) ---
# The actor advantage recursion uses the paper's distributional TD error
#   delta_G = r + d(gamma Z(s'), Z(s)),
# which for a 1-D categorical critic + the paper's LINEAR directional inverse-CDF
# metric collapses exactly to d(gamma Z', Z) = gamma E[Z'] - E[Z] (mean difference).
#
...
def distributional_raw_mean(probs, raw_support):
    """Return E[Z] in raw reward/return units for a categorical value distribution.

    DGAE's directional inverse-CDF metric with linear cost is an integral over
    matched quantiles. For discrete one-dimensional distributions this integral
    equals the difference of raw means, so the exact metric is
    gamma * E[Z_next] - E[Z_current]. Do not use hl_support.to_scalar here:
    for symlog critics that decodes as symexp(E[z]), not E[symexp(z)].
    """
    return (probs * raw_support).sum(dim=-1)


def c51_project(loc, weights, atoms):
    """C51 2-bin linear-interpolation projection (Bellemare et al.; cleanrl/c51.py).

    Redistributes mass `weights` placed at continuous locations `loc` onto the fixed
    grid `atoms` (uniform spacing) by splitting each source atom between its two
    nearest grid bins in proportion to distance. This is MEAN-EXACT (preserves
    E[Z] up to clamping) and adds ~zero spurious width -- a source atom landing on
    a grid point stays a point mass -- unlike a Gaussian projection which always
    smears. `loc`/`weights` are (..., K); `atoms` is (n,); returns (..., n).
    """
    n = atoms.numel()
    v_lo, v_hi = atoms[0], atoms[-1]
    delta = atoms[1] - atoms[0]
    tz = loc.clamp(v_lo, v_hi)
    b = (tz - v_lo) / delta
    l = b.floor().clamp(0, n - 1)
    u = b.ceil().clamp(0, n - 1)
    li, ui = l.long(), u.long()
    d_l = (u + (l == u).float() - b) * weights   # mass to lower bin
    d_u = (b - l) * weights                       # mass to upper bin
    out = torch.zeros(loc.shape[:-1] + (n,), device=loc.device, dtype=weights.dtype)
    out.scatter_add_(-1, li, d_l)
    out.scatter_add_(-1, ui, d_u)
    return out


def distributional_lambda_target(
    rewards, dones, next_done, value_probs, bootstrap_probs, atoms, gamma, lam
):
    """Distributional TD(lambda) return target as a categorical over the LINEAR grid `atoms`.

    Backward mixture recursion (distributional analog of the scalar GAE lambda-return):
        Z^lambda_t = C51proj( r_t + gamma * nonterm * [ (1-lambda) Z(s_{t+1})
                                                        + lambda    Z^lambda_{t+1} ] )
    The convex combination is a mixture of categoricals on the SAME grid (prob-weighted
    average), `gamma*. + r` shifts/scales each atom in raw return units, and the C51
    linear-interp projection (mean-exact, diffusion-free) snaps the shifted atoms back
    onto the grid. lam=0 gives the pure 1-step C51 Bellman target (no recursion). When
    the next state is terminal (nonterm=0) every atom collapses to r_t, so the mixture
    weights wash out and the target is C51proj(r_t) -- a (near) point mass at r_t.

    Returns lam_probs (T, B, n) and the per-state target std (raw units).
    """
    T, B, n = value_probs.shape
    lam_probs = torch.zeros_like(value_probs)
    target_std = torch.zeros((T, B), device=value_probs.device, dtype=value_probs.dtype)
    next_val = bootstrap_probs    # Z(s_{t+1}) at the boundary = bootstrap value dist
    next_lam = bootstrap_probs    # Z^lambda_{t+1} at the boundary = bootstrap value dist
    a = atoms.unsqueeze(0)        # (1, n)
    for t in reversed(range(T)):
        nonterminal = (1.0 - next_done) if t == T - 1 else (1.0 - dones[t + 1])  # (B,)
        m = (1.0 - lam) * next_val + lam * next_lam                              # (B, n) mixture
        loc = rewards[t].unsqueeze(-1) + gamma * nonterminal.unsqueeze(-1) * a    # (B, n) shifted atoms
        probs_t = c51_project(loc, m, atoms)                                     # (B, n)
        lam_probs[t] = probs_t
        mean = (probs_t * atoms).sum(-1, keepdim=True)
        target_std[t] = (probs_t * (atoms - mean) ** 2).sum(-1).clamp_min(0).sqrt()
        next_lam = probs_t
        next_val = value_probs[t]
    return lam_probs, target_std
...
class Args:
    ...
    # Distributional critic support. v3: LINEAR (C51-style) raw-return support, sized
    # to the normalized-return range (returns settle ~3, absmax peaked ~15.8).
    num_bins: int = 511
    v_min: float = -20.0
    v_max: float = 20.0
    critic_init_tau: float = 0.5   # init Z ≈ N(0, tau^2), sharp at 0
    value_symlog: bool = False     # v3: linear support => single decode E[Z]=sum(p*atom)
    value_sigma_to_bin_ratio: float = 2.0  # Dreamer4 / hl_gauss_pytorch default (HL-Gauss fallback only)
    dgae_policy_adv: bool = True           # use distributional TD recursion for actor advantages
    # v3 critic target: "td_lambda" = distributional GAE(lambda) C51 mixture,
    # "td0" = pure 1-step C51 Bellman (lambda=0; ablates recursive diffusion),
    # "scalar" = v1 scalar-return HL-Gauss projection (A/B baseline).
    dist_target_mode: str = "td_lambda"
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            scalar_deltas = torch.zeros_like(rewards).to(device)
            lastgaelam = 0
            for t in reversed(range(args.num_steps)):
                ...
                scalar_deltas[t] = delta
                advantages[t] = lastgaelam = delta + args.gamma * args.gae_lambda * nextnonterminal * lastgaelam
            returns = advantages + values
            # DGAE POLICY ADVANTAGE: arXiv:2507.17530 replaces scalar TD errors
            # with a directional metric between return distributions. With
            # one-dimensional categorical supports and linear inverse-CDF cost,
            # mean_q[Q_{gamma Z'}(q) - Q_Z(q)] is exactly
            # gamma * E_raw[Z'] - E_raw[Z]. Use raw bin-center means, not
            # symlog scalar decode symexp(E[z]), because rewards/advantages are
            # in raw normalized-return units.
            dist_values = distributional_raw_mean(value_probs, raw_support)
            next_dist_value = distributional_raw_mean(bootstrap_probs, raw_support).reshape(1, -1)
            dgae_advantages = torch.zeros_like(rewards).to(device)
            dgae_deltas = torch.zeros_like(rewards).to(device)
            lastdgaelam = 0
            for t in reversed(range(args.num_steps)):
                if t == args.num_steps - 1:
                    nextnonterminal = 1.0 - next_done
                    nextdistvalues = next_dist_value
                else:
                    nextnonterminal = 1.0 - dones[t + 1]
                    nextdistvalues = dist_values[t + 1]
                delta_g = rewards[t] + args.gamma * nextdistvalues * nextnonterminal - dist_values[t]
                dgae_deltas[t] = delta_g
                dgae_advantages[t] = lastdgaelam = delta_g + args.gamma * args.gae_lambda * nextnonterminal * lastdgaelam
            ...
            if auto_alpha:
                ...
                for t in reversed(range(args.num_steps)):
                    if t == args.num_steps - 1:
                        nextnonterminal = 1.0 - next_done
                        nextvalues = next_dist_value if args.dgae_policy_adv else next_value
                    else:
                        nextnonterminal = 1.0 - dones[t + 1]
                        nextvalues = dist_values[t + 1] if args.dgae_policy_adv else values[t + 1]
                    currentvalues = dist_values[t] if args.dgae_policy_adv else values[t]
                    delta = rewards[t] + args.gamma * (nextvalues + next_value_bonus[t]) * nextnonterminal - currentvalues
                    policy_adv[t] = lastgaelam = delta + args.gamma * args.gae_lambda * nextnonterminal * lastgaelam
            elif args.dgae_policy_adv:
                policy_adv = dgae_advantages
            else:
                policy_adv = advantages
            # Critic target. v3: C51-correct distributional return target on the LINEAR
            # grid. "td_lambda"/"td0" build the categorical Bellman mixture via the
            # mean-exact C51 linear-interp projection (diffusion-free); "scalar" falls
            # back to v1's scalar-return HL-Gauss projection (A/B baseline).
            if args.dist_target_mode in ("td_lambda", "td0"):
                lam_eff = args.gae_lambda if args.dist_target_mode == "td_lambda" else 0.0
                target_probs, target_dist_std = distributional_lambda_target(
                    rewards, dones, next_done, value_probs, bootstrap_probs,
                    raw_support, args.gamma, lam_eff,
                )
            elif args.dist_target_mode == "scalar":
                target_probs = hl_support.project(returns)
                mean_raw = (target_probs * raw_support).sum(-1, keepdim=True)
                target_dist_std = (target_probs * (raw_support - mean_raw) ** 2).sum(-1).clamp_min(0).sqrt()
            else:
                raise ValueError(f"unknown dist_target_mode {args.dist_target_mode}")
            ...
            if args.value_symlog:
                ...
                sigma_floor = args.sigma_floor_bins * bin_width  # float (uniform linear grid)
            sigma = sigma.clamp_min(sigma_floor) if isinstance(sigma_floor, float) else torch.maximum(sigma, sigma_floor)
        ...
        b_dist_values = dist_values.reshape(-1)
        b_scalar_advantages = advantages.reshape(-1)
        b_scalar_deltas = scalar_deltas.reshape(-1)
        b_dgae_advantages = dgae_advantages.reshape(-1)
        b_dgae_deltas = dgae_deltas.reshape(-1)
        ...
        scalar_delta_z = (b_scalar_deltas - b_scalar_deltas.mean()) / (b_scalar_deltas.std() + 1e-8)
        dgae_delta_z = (b_dgae_deltas - b_dgae_deltas.mean()) / (b_dgae_deltas.std() + 1e-8)
        scalar_adv_z = (b_scalar_advantages - b_scalar_advantages.mean()) / (b_scalar_advantages.std() + 1e-8)
        dgae_adv_z = (b_dgae_advantages - b_dgae_advantages.mean()) / (b_dgae_advantages.std() + 1e-8)
        dgae_delta_scalar_corr = (scalar_delta_z * dgae_delta_z).mean().item()
        dgae_adv_scalar_corr = (scalar_adv_z * dgae_adv_z).mean().item()
        dgae_delta_scalar_mae = (b_dgae_deltas - b_scalar_deltas).abs().mean().item()
        dgae_adv_scalar_mae = (b_dgae_advantages - b_scalar_advantages).abs().mean().item()
        dgae_decode_mae = (b_dist_values - b_values).abs().mean().item()
        ...
        # v2: how DISTRIBUTIONAL is the target? target_dist_std (raw units) >> the
        # projection floor (~sigma_ratio*bin_width) means Z is learning real spread,
        # not a smoothed point mass. Compare to sigma_mean (the critic's own spread).
        target_dist_std_mean = target_dist_std.mean().item()
        ...
        writer.add_scalar("debug/target_dist_std", target_dist_std_mean, global_step)
        writer.add_scalar("debug/dgae_delta_mean", b_dgae_deltas.mean().item(), global_step)
        writer.add_scalar("debug/dgae_delta_std", b_dgae_deltas.std().item(), global_step)
        writer.add_scalar("debug/dgae_adv_mean", b_dgae_advantages.mean().item(), global_step)
        writer.add_scalar("debug/dgae_adv_std", b_dgae_advantages.std().item(), global_step)
        writer.add_scalar("debug/dgae_delta_scalar_corr", dgae_delta_scalar_corr, global_step)
        writer.add_scalar("debug/dgae_adv_scalar_corr", dgae_adv_scalar_corr, global_step)
        writer.add_scalar("debug/dgae_delta_scalar_mae", dgae_delta_scalar_mae, global_step)
        writer.add_scalar("debug/dgae_adv_scalar_mae", dgae_adv_scalar_mae, global_step)
        writer.add_scalar("debug/dgae_rawmean_decode_mae", dgae_decode_mae, global_step)
