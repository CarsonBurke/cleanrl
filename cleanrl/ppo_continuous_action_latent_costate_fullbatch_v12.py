# Latent costate fullbatch v12: vector credit without minibatches or repeated backups.
...
# Fit one detached Bellman target per fresh rollout; all fitting steps use the full batch.
# Hypothesis: higher full-batch learning rates recover useful vector credit without sweeps.
...
@dataclass
class Args:
    exp_name: str = 'latent_costate_fullbatch_v12'
    seed: int = 1
    ...
    model_epochs: int = 10
    critic_epochs: int = 8
    critic_target_refreshes: int = 1
    gamma: float = 1.
    full_batch: bool = True
    minibatch_size: int = 4096
    model_learning_rate: float = .003
    critic_learning_rate: float = .0009
...
def main():
    ...
    with ExitStack() as resources:
        ...
        report = dict(args=vars(args), fresh_initialization=True, checkpoint_loaded=False,
                      ...
                      limitations='No episode-age discount in actor weighting: not an exact episode-start discounted gradient. Full-batch fitting uses 10 model and 8 critic steps per rollout with default epoch settings; one fixed critic target. Learning-rate changes do not reproduce minibatch Adam trajectories. Single seed and training returns. Learned dynamics derivatives may be inaccurate. Differential Bellman recursion need not contract. Fixed affine normalization after stochastic warmup. State occupancy includes ongoing-episode policy changes. Natural actor solve is approximate.')
