# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic MLP10 obs-idx-transfer justnorm v1.
#
...
#
# MLP10 obs-idx-transfer justnorm variant: remove the DenseNet-style trunk
# concatenation, residual branch sum, x0 convex residual, and MoE. The actor/
# critic trunk uses a hidden-width ReLU-squared observation stem x0, then a plain
# depth-10, width-128 ReLU-squared MLP with RMSNorm after every ReLU-squared
# activation. Starting at hidden layer 2, each hidden perceptron also receives
# learned same-index transfer residuals from x0 and earlier non-immediate hidden
# layers in its preactivation:
#   x0[j] = stem(obs)[j]
#   z_l[j] = W_l h_{l-1}[j] + b_l[j] + a_{l,0,j} x0[j] + sum_{i=1}^{l-2} a_{l,i,j} h_i[j]
# The trunk's dense matrices are nGPT-style justnorm projected at initialization
# and after optimizer steps, matching the ngptjustnorm ablation lineage. Transfer
# weights and actor/critic heads are left unconstrained.
#
...
#
# WHY v21. v19 used two independent actor/critic trunks. The
...
#   - share_backbone: one trunk feeds both the actor head and the
...
class Args:
    ...
    share_backbone: bool = True      # one MLP trunk for both actor and critic heads
...
class IndexedTransferLayer(nn.Module):
    def __init__(self, in_dim, H, history_slots):
        super().__init__()
        self.H = H
        self.history_slots = history_slots
        self.input_norm = nn.RMSNorm(in_dim, elementwise_affine=False)
        self.current_linear = layer_init(nn.Linear(in_dim, H))
        self.act = ReLUSquared()
        self.output_norm = nn.RMSNorm(H, elementwise_affine=False)
        if history_slots:
            self.history_weight = nn.Parameter(torch.empty(history_slots, H))
            nn.init.normal_(self.history_weight, mean=0.0, std=np.sqrt(2.0 / (H + history_slots)))
        else:
            self.register_parameter("history_weight", None)

    def forward(self, x, history_feats):
        preact = self.current_linear(self.input_norm(x))
        if self.history_slots:
            history = torch.stack(history_feats, dim=1)               # (B, slots, H)
            transfer = (history * self.history_weight.to(dtype=history.dtype).unsqueeze(0)).sum(dim=1)
            preact = preact + transfer
        return self.output_norm(self.act(preact))

    @torch.no_grad()
    def normalize_ngpt_weights(self):
        self.current_linear.weight.copy_(justnorm(self.current_linear.weight, dim=1))


class TransferMLPTrunk(nn.Module):
    def __init__(self, in_dim, H, depth):
        super().__init__()
        if depth < 1:
            raise ValueError(f"mlp_depth must be >= 1, got {depth}")
        self.stem = IndexedTransferLayer(in_dim, H, 0)
        self.layers = nn.ModuleList()
        for layer_idx in range(depth):
            history_slots = layer_idx
            self.layers.append(IndexedTransferLayer(H, H, history_slots))
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            history_feats = feats[:-1]
            x = layer(x, history_feats)
            feats.append(x)
        return x

    @torch.no_grad()
    def normalize_ngpt_weights(self):
        self.stem.normalize_ngpt_weights()
        for layer in self.layers:
            layer.normalize_ngpt_weights()
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if self.share_backbone:
            ...
            self.trunk = TransferMLPTrunk(obs_dim, H, args.mlp_depth)
        else:
            self.critic_trunk = TransferMLPTrunk(obs_dim, H, args.mlp_depth)
            self.actor_trunk = TransferMLPTrunk(obs_dim, H, args.mlp_depth)
