# Residual SiTU-GLU with unit-L2 weights AND residual activations v5.
# Controlled addition to ngpt_weights_v4: h=N(stage1(obs)); b=N(stage2(h));
# output=head(N(h+b)). N is rowwise unit L2; zero vectors remain zero.
# Actor and critic share this geometry. Keep plain addition, no learned mixing,
# gain parameters, or PPO changes. Weight projection stays init + post-Adam.
# Hypothesis: directional residual updates reduce branch-scale-driven policy KL.
# Caveat: normalized heads bound state-dependent logits/value contributions by 1.
import os
import random
import time
...
class ResidualMLP(nn.Module):
    """Two width-64 stages with normalized stream, branch, and residual sum."""
    ...
    def forward(self, x):
        ...
        branch = F.normalize(self.second(h), p=2, dim=-1)
        return self.head(F.normalize(h + branch, p=2, dim=-1))
...
class ResidualHostMirror:
    ...
    def __call__(self, observations):
        ...
        np.copyto(self.branch, self.second(self.hidden))
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", "residual SiTU-GLU; unit-L2 weights plus stream, branch, and residual-sum normalization; no learned mixing")
