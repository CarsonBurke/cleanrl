# ============================================================================
# DG v24 -- no-advnorm winner + v215 HL-Gauss ±20k critic + monolithic reduce-overhead compile.
#
# From dg_v23_noadvnorm (best of the v23 ablations): raw rewards, no advnorm, no retnorm,
# no DG gate, strong L2-hinge KL. Changes:
#   1) Critic: iterthink v215-style HLGaussSupport (NOT Dreamer3Bucket, NOT MSE):
#      - raw return range ±20000 via symlog coords v_min/v_max = ±symlog(20000)
#      - support_is_edges=True, 511 bins, sigma_ratio=2.0, use_symlog=True
#      - decode with to_expected_scalar (E[symexp(z)]), CE HL-Gauss targets via project()
#      - single-step return (no MTP)
#   2) torch.compile(agent, mode="reduce-overhead", dynamic=False) for CUDA graphs.
#      Agent methods return tensors only (alpha/beta not Beta dist) so graphs stay pure.
# ----------------------------------------------------------------------------
# DG v23 -- v21 + return-agnostic reward RMS scale (r / sqrt(EMA(r^2))).
# ----------------------------------------------------------------------------
...
class Args:
    ...
    # v24 / v215 HL-Gauss critic: symlog coords for raw return range ±20000, edges support, no MTP
    critic_num_bins: int = 511
    critic_v_min: float = -9.90353755128617  # symlog(-20000)
    critic_v_max: float = 9.90353755128617   # symlog(+20000)
    critic_sigma_ratio: float = 2.0  # v215 value_sigma_to_bin_ratio (HL-Gauss projection width)
    critic_symlog: bool = True
    critic_support_is_edges: bool = True  # v215 HLGaussSupport edges layout
...
if __name__ == "__main__":
    ...
    # v24 / v215 HL-Gauss: uniform-in-symlog edges support over raw returns ±20000.
    # Decode with E[symexp(z)] (to_expected_scalar); train with CE on project() labels. No MTP.
    hlg = HLGaussSupport(
    ...
    if args.compile:
        ...
        print(f"[v24] torch.compile(agent, mode={args.compile_mode!r}, dynamic=False)")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                # v215: E[symexp(z)] decode (not symexp(E[z]))
                values[step] = hlg.to_expected_scalar(value_logits).flatten()
            zs[step] = z
            alphas[step] = alpha
            betas[step] = beta
        ...
        with torch.no_grad():
            if args.compile:
                torch.compiler.cudagraph_mark_step_begin()
            next_value = hlg.to_expected_scalar(agent.get_value(next_obs)).reshape(1, -1)
