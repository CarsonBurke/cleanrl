# PPO + IterThink v24.fpo.1 (FLOW POLICY OPTIMIZATION). From v24 beta.
#
# FPO replaces v24's explicit actor density with a time-conditioned flow-matching
# policy. Actions are generated by integrating a learned velocity field from
# Gaussian noise at t=1 to action space at t=0. PPO's policy ratio is replaced by
# the FPO surrogate ratio exp(L_old(action, eps, t) - L_new(action, eps, t)), where
# L is the conditional flow-matching reconstruction loss on rollout-sampled
# noise/timestep pairs. The v24 distributional critic, rankgauss advantage
# shaping, clip-higher PPO objective, and decoupled actor/critic gradient clipping
# are otherwise preserved.
#
# Hypothesis: FPO's denoising policy can express coordinated, multimodal action
# structure without Gaussian sigma pathologies or Beta boundary constraints, while
# v24's robust critic/advantage machinery supplies a stable scalar learning signal.
#
...
class Args:
    ...
    # v24.fpo.1: "fpo" is the default flow-matching policy. "beta" and "gaussian"
