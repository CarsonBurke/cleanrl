# PPO + IterThink v24 beta + SimBaV2 hyperspherical backbone (simbav2_v1).
#
# KEY IDEA. Every trunk this lineage has tried (DenseNet+MoE, perceptron-reuse,
# recurrent program libraries) plateaus ~9.4-9.8k on HalfCheetah, and WIDE always
# regressed. SimBaV2 (ICLR'25 successor to SimBa) is the published recipe that makes
# scale *help* on continuous control: keep the entire representation on the unit
# hypersphere so capacity converts to score instead of destabilizing. We port that
# backbone faithfully and drop it into the v24_beta PPO machinery as a clean trunk swap.
#
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if self.share_backbone:
            ...
            self.trunk = SimbaV2Backbone(obs_dim, args.shared_hidden, args.shared_blocks, args.c_shift)
        else:
            # SimBaV2 default: SEPARATE asymmetric actor/critic backbones.
            actor_H = args.actor_hidden
            critic_H = args.critic_hidden
            self.actor_trunk = SimbaV2Backbone(obs_dim, actor_H, args.actor_blocks, args.c_shift)
            self.critic_trunk = SimbaV2Backbone(obs_dim, critic_H, args.critic_blocks, args.c_shift)
