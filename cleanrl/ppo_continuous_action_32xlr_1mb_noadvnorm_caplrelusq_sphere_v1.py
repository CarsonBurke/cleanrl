...
# Capped LeakyReluSq: leaky_relu(4*tanh(x/4), 0.5)^2 in place of
# leaky_relu(x, 0.5)^2. Bounded by 16; still non-negative, so it keeps the
# 0.31 branch DC energy fraction that justnorm cannot remove.
...
# This is the cap-only cell of the {leaky, signed} x {uncapped, capped} 2x2:
# it isolates boundedness from zero-meanness. Design point E[f^2] = 3.38142
# at v = 2 (uncapped: 6.375).
# Hypothesis: bounded weight-norm growth restores late-training plasticity and
# recovers most of the gap to stiglu_sphere even with the DC direction intact.
...
from cleanrl.shared.host_actor import make_caplrelusq_sphere_trunk
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            make_caplrelusq_sphere_trunk(observation_dim, 64),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; 32x LR; 1 minibatch; hypersphered tanh-capped LeakyReluSq trunk; no advantage normalization; trunk probes")
