# PPO + IterThink v24.fpo.2 (BOUNDED-ENDPOINT FPO). From v24.fpo.1.
#
...
# v2 instead treats the bounded env action itself as the flow endpoint: x_1 ~ N(0,I),
# x_0 in [-1,1]. The stored action, CFM target, and env action are the same bounded
# tensor. This is a deliberate bounded-support FPO adaptation, not raw-action parity.
#
# Hypothesis: bounded-endpoint FPO keeps FPO's coordinated flow policy while removing
# raw-action/tanh saturation degeneracy. Defaults also use separate actor/critic
# trunks and plain batch-normalized GAE to avoid critic trunk and rank transform
# interactions with the CFM pseudo-ratio.
#
...
class Args:
    ...
    share_backbone: bool = False     # FPO default: separate policy/value features keep CFM ratios cleaner
    separate_grad_clip: bool = False # FPO default: one global PPO-style clip, not v24's 0.25 actor cap
    ...
    fpo_target_kl: Optional[float] = None
