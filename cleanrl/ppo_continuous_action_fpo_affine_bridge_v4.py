"""FPO affine-bridge residual flow v4: learn mean and contraction explicitly.

PPO critic/GAE/optimizer/clipping unchanged; FPO epsilon-MSE and fixed MC pairs.
A state-only 64x64 Tanh head predicts Gaussian endpoint mean and positive std.
Its analytic conditional-flow field transports N(0,I) to that Gaussian along
x_t=t*eps+(1-t)*action. A nonlinear residual velocity retains non-Gaussian flow
expressivity. Integrating-factor Euler solves the affine component exactly, so
zero residual samples exactly mean+std*noise at any step count; the residual is
integrated with the same ten flow evaluations. Both heads learn ONLY via FPO.
Hypothesis: explicit mean/scale gradients learn useful noise contraction faster
than asking a generic small MLP to discover state-dependent affine dynamics.
"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        residual = nn.Sequential(*layers)
        # Zero residual starts at a calibrated Gaussian bridge, not a field that
        # happens to preserve endpoint noise but misfits the CFM interpolation.
        nn.init.zeros_(residual[-1].weight)
        nn.init.zeros_(residual[-1].bias)
        affine = nn.Sequential(
            layer_init(nn.Linear(self.obs_dim, 64)), nn.Tanh(),
            layer_init(nn.Linear(64, 64)), nn.Tanh(),
            layer_init(nn.Linear(64, 2 * self.action_dim), std=0.01),
        )
        self.actor = nn.ModuleDict({"affine": affine, "residual": residual})

    def get_value(self, observations):
        return self.critic(observations.reshape(-1, self.obs_dim))

    def affine_parameters(self, observations):
        mean, raw_std = self.actor["affine"](observations).chunk(2, -1)
        std = torch.nn.functional.softplus(raw_std + 0.541324854612918)
        return mean, std

    def residual_velocity(self, observations, states, times):
        phases = times * self.time_frequencies
        embedding = torch.cat((phases.cos(), phases.sin()), -1)
        return self.output_scale * self.actor["residual"](torch.cat((observations, states, embedding), -1))

    def velocity_with_affine(self, observations, states, times, mean, std):
        variance = times.square() + (1.0 - times).square() * std.square()
        coefficient = (times - (1.0 - times) * std.square()) / variance
        base = coefficient * (states - (1.0 - times) * mean) - mean
        return base + self.residual_velocity(observations, states, times)

    def velocity(self, observations, states, times):
        mean, std = self.affine_parameters(observations)
        return self.velocity_with_affine(observations, states, times, mean, std)
    ...
    def sample(self, observations, noise):
        ...
        for step in range(self.flow_steps):
            t, tn = 1.0 - step / self.flow_steps, 1.0 - (step + 1) / self.flow_steps
            scale = (t*t + (1.0-t)**2 * std.square()).sqrt()
            next_scale = (tn*tn + (1.0-tn)**2 * std.square()).sqrt()
            times = native.new_full((len(native), 1), t)
            residual = self.residual_velocity(observations, native, times)
            native = (1.0-tn)*mean + (next_scale/scale) * (native-(1.0-t)*mean-residual/self.flow_steps)
...
def cfm_errors(agent, observations, native, times, noise, mse):
    ...
    mean, std = agent.affine_parameters(observations.detach())
    velocity = agent.velocity_with_affine(conditions, states, times, mean[:, None, :], std[:, None, :])
    if mse == "epsilon":
...
class HostSampler:
    """Exact affine bridge plus integrating-factor Euler residual, permanent buffers."""
    def __init__(self, agent, num_rows, steps):
        self.obs_dim, self.action_dim, self.steps = agent.obs_dim, agent.action_dim, steps
        self.output_scale, self.action_transform = agent.output_scale, agent.action_transform
        self.affine = make_host_mirror(agent.actor["affine"], num_rows)
        self.actor = make_host_mirror(agent.actor["residual"], num_rows)
        ...
        self.native, self.action, self.work, self.mean, self.std, self.variance, self.next_variance = (
            np.empty(shape, np.float32) for _ in range(7))
        self.inputs = np.empty((num_rows, agent.obs_dim + agent.action_dim + 8), np.float32)
        times = (1.0 - np.arange(steps, dtype=np.float32)/steps)[:, None]
        phases = times * agent.time_frequencies.detach().cpu().numpy()[None, :]
        self.time_embeddings = np.concatenate((np.cos(phases), np.sin(phases)), -1)
    ...
    def __call__(self, observations, rng, *, noise=None):
        ...
        outputs = self.affine(observations)
        np.copyto(self.mean, outputs[:, :self.action_dim])
        np.add(outputs[:, self.action_dim:], np.float32(0.541324854612918), out=self.std)
        np.logaddexp(self.std, np.float32(0.0), out=self.std)
        np.square(self.std, out=self.std)
        self.inputs[:, :self.obs_dim] = observations
        for step in range(self.steps):
            t, tn = np.float32(1.0-step/self.steps), np.float32(1.0-(step+1)/self.steps)
            np.multiply(self.std, np.float32((1.0-t)**2), out=self.variance)
            self.variance += t*t
            np.sqrt(self.variance, out=self.variance)
            np.multiply(self.std, np.float32((1.0-tn)**2), out=self.next_variance)
            self.next_variance += tn*tn
            np.sqrt(self.next_variance, out=self.next_variance)
            self.next_variance /= self.variance
            self.inputs[:, self.obs_dim:self.obs_dim+self.action_dim] = self.native
            self.inputs[:, -8:] = self.time_embeddings[step]
            np.multiply(self.actor(self.inputs), np.float32(self.output_scale/self.steps), out=self.work)
            self.native -= self.work
            np.multiply(self.mean, np.float32(1.0-t), out=self.work)
            self.native -= self.work
            self.native *= self.next_variance
            np.multiply(self.mean, np.float32(1.0-tn), out=self.work)
            self.native += self.work
        if self.action_transform == "tanh":
...
def main():
    ...
    with ExitStack() as resources:
        ...
        writer.add_text("policy", "FPO Algorithm1: frozen MC error ratios; signed clipped GAE; "
                        f"affine Gaussian bridge plus nonlinear residual; {args.action_transform} actions; FPO only")
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with torch.no_grad():
                ...
                bounded = ((decoded.clamp(agent.action_low, agent.action_high) - agent.action_bias)
                           / agent.action_scale)
                affine_mean, affine_std = agent.affine_parameters(b_obs)
                logged = gather_metrics({
                    "sampling/affine_std": affine_std.mean(),
                    "sampling/affine_mean_abs": affine_mean.abs().mean(),
