# NextLat critic PPO v1, based on normres_indclip_v4 50M Beta PPO.
# The critic keeps an observation-only latent h_t and learns an action-conditioned
# residual dynamics model h_hat_{t+1}=f(h_t,a_t)+h_t against the stopped online
# next-state critic latent. This tests predictive critic representations without
# feeding actor features into the value function.
# Hypothesis: one-step latent prediction regularizes the critic toward transition
# structure and improves value estimates without changing actor inputs or logits.
import os
import random
import time
...
class Args:
    ...
    """weight of the one-step critic NextLat SmoothL1 auxiliary"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, placement="pre", norm_kind="rms", activation="stiglu"):
        ...
        self.next_latent_predictor = nn.Sequential(
            layer_init(nn.Linear(64 + self.action_dim, 64)),
            nn.Tanh(),
            layer_init(nn.Linear(64, 64), std=0.1),
        )
    ...
    def get_critic_latent(self, x):
        return self.critic[0](x)

    def predict_next_critic_latent(self, critic_latent, native_action):
        action_input = native_action * 2.0 - 1.0
        delta = self.next_latent_predictor(torch.cat((critic_latent, action_input), dim=-1))
        return critic_latent + delta

    def get_value(self, x):
        return self.critic[1](self.get_critic_latent(x))
...
def ppo_loss(agent, observations, native_actions, next_observations, transition_valid, old_logprobs, advantages, targets, old_values, args):
    ...
    critic_latent = agent.get_critic_latent(observations)
    with torch.no_grad():
        target_next_latent = agent.get_critic_latent(next_observations)
    predicted_next_latent = agent.predict_next_critic_latent(critic_latent, native_actions)
    latent_error = F.smooth_l1_loss(predicted_next_latent, target_next_latent, reduction="none")
    latent_mask = transition_valid.to(dtype=latent_error.dtype).unsqueeze(-1)
    nextlat_loss = (latent_error * latent_mask).sum() / latent_mask.expand_as(latent_error).sum().clamp_min(1.0)
    loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef + args.nextlat_coef * nextlat_loss
    metrics = torch.stack(
        (pg_loss.detach(), v_loss.detach(), entropy_loss.detach(), old_approx_kl, approx_kl, clipfrac, nextlat_loss.detach())
    )
    return loss, metrics
...
def validate_args(args):
    ...
    if args.nextlat_coef < 0.0:
        raise ValueError("nextlat_coef must be nonnegative")
    return args
...
def main():
    ...
    try:
        ...
        writer.add_text(
            ...
            f"critic NextLat coef={args.nextlat_coef}",
        )
        ...
        critic_parameters = tuple(agent.critic.parameters()) + tuple(agent.next_latent_predictor.parameters())
        ...
        def loss_model(observations, native, next_observations, transition_valid, old_logprobs, advantages, targets, old_values):
            return ppo_loss(
                agent, observations, native, next_observations, transition_valid,
                old_logprobs, advantages, targets, old_values, args
            )
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            for step in range(args.num_steps):
                ...
                with timer.span("normalize_transfer", use_cuda=False):
                    ...
                    transfer.push(
                        step, reward, terms, truncs, observations=obs_step,
                        next_observations=transition_obs, native_actions=native
                    )
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_transition_valid = ~(
                    (batch.terminations > 0.5) | (batch.truncations > 0.5)
                ).flatten()
            ...
            logged = gather_metrics(
                {
                    ...
                    "losses/nextlat_pred_loss": last[6],
