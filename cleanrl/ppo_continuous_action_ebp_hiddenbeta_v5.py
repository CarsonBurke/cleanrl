# PPO + Energy-Based Prediction Hidden-Beta v5.
#
# Predictive-coding variant of base continuous PPO. Hidden activations are explicit
# bounded state variables, not an ordinary end-to-end MLP path. Each local hidden
# predictor outputs a Beta distribution over the next hidden state; states settle
# by minimizing adjacent Beta negative log likelihoods plus terminal PPO/value
# energy. After settling, states are detached: local predictors learn only adjacent
# settled-state prediction, and output heads learn only terminal PPO/value losses.
#
    ...
    def settle_states(self, hierarchy, x, initial_states, args, terminal_energy_fn, steps=None):
        ...
        for _ in range(steps):
            ...
            for state, grad in zip(states, grads):
                ...
                next_state = next_state.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
