# PPO + IterThink v24 Beta + v162 critic + Dreamer3-bucket HL-Gauss MTP + D3 PERCNORM v1.
# =====================================================================================
...
class Args:
    ...
    # --- DreamerV3 percentile advantage normalization (the sole advantage scaler) ---
    ret_percnorm: bool = True        # scale policy advantage by S = max(floor, EMA(P95)-EMA(P5)) of returns
    ret_perc_rate: float = 0.01      # EMA rate on the percentiles (DreamerV3 default; decay 0.99)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # DreamerV3 percentile advantage normalization: scale the POLICY advantage by an EMA
            # of the return percentile spread, S = max(floor, EMA(P95) - EMA(P5)). Divide-only (the
            # GAE advantage is already value-centered); preserves sign/order. The critic target
            # `returns` stays RAW (the Dreamer3 symexp-bucket head absorbs scale), as in v15/v16
            # and DreamerV3 itself (valnorm=none). This REPLACES rankgauss/norm_adv as the scaler.
