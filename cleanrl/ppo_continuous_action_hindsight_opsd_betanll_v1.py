...
#
# PPO + Hindsight-Conditioned OPSD Teacher, beta-NLL v1.
#
# Hypothesis:
...
# future observation deltas, discounted reward sums, and a 16-step bootstrapped
# advantage. The teacher learns the rollout action with Gaussian beta-NLL
# (variance.detach() ** beta reweighting, as in ../beta-nll), then the student
# receives a detached full-distribution KL from teacher to student. The student
# never sees the hindsight context.
import math
import os
import random
import time
...
def build_hindsight_context(obs, rewards, dones, values, gamma):
    continuous_features = []
    valid_features = []
    ...
    h = HINDSIGHT_REWARD_HORIZONS[-1]
    h_valid = future_valid_mask(dones, h)
    future_values = shifted_tensor(values, h)
    hindsight_advantage = reward_sums[h] + (gamma**h) * future_values * h_valid - values
    continuous_features.append(hindsight_advantage.unsqueeze(-1))
    ...
    return context, hindsight_advantage


def hindsight_weights(hindsight_advantage, args):
    flat_adv = hindsight_advantage.reshape(-1)
    adv_z = (flat_adv - flat_adv.mean()) / (flat_adv.std(unbiased=False) + 1e-8)
...
class HindsightTeacher(nn.Module):
    ...
    def forward(self, obs, hindsight_context, args):
        hidden = self.backbone(torch.cat([obs, hindsight_context], dim=-1))
        mean = self.mean(hidden)
        logvar_min = math.log(args.teacher_var_min)
        logvar_max = math.log(args.teacher_var_max)
        logvar = self.logvar(hidden).clamp(logvar_min, logvar_max)
        var = logvar.exp()
        return mean, var
...
if __name__ == "__main__":
    ...
    agent.actor_mean = maybe_compile(agent.actor_mean, "agent.actor_mean", args)
    ...
    optimizer = optim.Adam(list(agent.parameters()) + list(teacher.parameters()), lr=args.learning_rate, eps=1e-5)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            hindsight_context, hindsight_advantage = build_hindsight_context(obs, rewards, dones, values, args.gamma)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                student_probs, student_mean, student_std = agent.get_action_distribution(b_obs[mb_inds])
                ...
                teacher_loss = (mb_hindsight_weights * beta_nll).mean()

                with torch.no_grad():
                    teacher_mean = mb_teacher_mean.detach()
                    teacher_std = mb_teacher_var.detach().sqrt()
                    min_teacher_std = args.teacher_std_floor_ratio * student_std.detach()
                    teacher_std = torch.maximum(teacher_std, min_teacher_std)
                    teacher_action_error = (teacher_mean - b_actions[mb_inds]).abs().mean()
                ...
                loss = (
                    ...
                    + v_loss * args.vf_coef
                    + args.teacher_loss_coef * teacher_loss
                    + active_hindsight_beta * hindsight_kl_loss
                )

                optimizer.zero_grad()
                loss.backward()
                nn.utils.clip_grad_norm_(list(agent.parameters()) + list(teacher.parameters()), args.max_grad_norm)
                optimizer.step()

                teacher_losses.append(teacher_loss.item())
                hindsight_kl_losses.append(hindsight_kl_loss.item())
                teacher_action_errors.append(teacher_action_error.item())
                teacher_stds.append(teacher_std.mean().item())
