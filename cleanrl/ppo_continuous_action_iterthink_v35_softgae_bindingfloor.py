...
class Args:
    ...
    soft_alpha_frac: float = 0.5    # v35: when BOTH channels run, fraction of alpha given to the soft-adv
                                    #      (in-GAE/future) channel; the direct (current-step) channel gets
                                    #      (1-frac). Splits the entropy budget so each state's entropy is
                                    #      weighted ~alpha total (not 2x) -- fixes the on-policy double-count.
    soft_adv: bool = True           # v35: TRUE => FUTURE entropy enters the policy advantage through the
                                    #      soft-advantage GAE (SAC soft Bellman bootstrap V(s')+alpha*H(s')),
                                    #      i.e. "max entropy IN GAE". Decoupled from direct_entropy so BOTH
                                    #      the in-GAE (future) and direct (current-step) terms run, as in SAC.
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if auto_alpha:
                ...
                # v35 alpha-SPLIT (fixes double-counting). On an on-policy GAE, running the soft-adv
                # (future-entropy) AND direct (current-step) channels BOTH credit each interior state's
                # entropy H(s_k) -- once directly at step k, once as the successor entropy of step k-1.
                # That over-weights vs SAC's single-alpha budget. Split alpha across the two channels
                # so each state's total entropy weight stays ~alpha: soft gets soft_alpha_frac, direct
                # gets (1-soft_alpha_frac). When only one channel is active it gets the full alpha.
                both_channels = args.soft_adv and args.direct_entropy
                soft_frac = args.soft_alpha_frac if both_channels else 1.0
                alpha_eff = alpha_r * soft_frac
            ...
            if auto_alpha and args.soft_adv:
                # SOFT-ADV channel (v30): FUTURE entropy enters the policy advantage via GAE.
                # v35: gated on soft_adv (decoupled from direct_entropy) so it runs alongside the
                # direct current-step bonus (both terms of the SAC soft objective).
                ...
                # soft_adv OFF: policy advantage is the pure reward advantage. Any entropy then enters
                # only via the direct current-step bonus below (next_value_bonus computed but unused).
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if auto_alpha and args.direct_entropy:
                    # v35 alpha-split: direct channel gets (1-soft_alpha_frac) of alpha when the soft
                    # channel is also live, else the full alpha (see the soft-bonus block above).
                    direct_frac = (1.0 - args.soft_alpha_frac) if (args.soft_adv and args.direct_entropy) else 1.0
