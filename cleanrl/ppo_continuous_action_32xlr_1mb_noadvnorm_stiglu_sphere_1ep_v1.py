...
# No-advnorm + hypersphered SiTU-GLU trunk with ONE actor epoch per rollout.
# Same 32x LR (9.6e-3) and nGPT-style sphere trunk as
# _noadvnorm_stiglu_sphere_v1, but actor and critic train on separate
# optimizers and schedules: the policy takes a single full-batch PPO pass
# (minimal trust-region movement on raw GAE) while the critic keeps 10
# epochs of value fitting. Tests whether one raw-advantage policy step
# suffices when the step count, not the weighting, drives KL.
import os
import random
import time
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; 32x LR; 1 minibatch; hypersphered SiTU-GLU trunk; 1 actor epoch + 10 critic epochs; no advantage normalization")
