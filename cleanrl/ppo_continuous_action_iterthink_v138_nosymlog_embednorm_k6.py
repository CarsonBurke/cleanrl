# PPO + IterThink v138 nosymlog-embednorm-k6 (v137 with normalized obs and embed norm).
#
# Key change from v137:
#   - Restore Gym's NormalizeObservation/clip wrappers and remove symlog from
#     observation tokenization.
#   - Encode each normalized observation scalar with a shared tokenizer, add a
#     learned feature slot embedding, then apply a standard token embedding
#     RMSNorm before the encoder stack. This avoids v137's pre-hidden RMSNorm
#     that can erase scalar magnitude before the value is represented.
#
# Hypothesis:
#   Normalized observations plus conventional embedding normalization should
#   recover the stable reward-learning behavior of the normalized-observation
#   variants while keeping v137's scalable shared scalar tokenizer.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # Normalized-observation tokenizer. The scalar MLP is shared across obs
        # dimensions for scalability; learned slot embeddings provide feature
        # identity. Apply embedding RMSNorm after value and slot embeddings are
        # combined, not before the scalar value has been represented.
