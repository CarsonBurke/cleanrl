...
# Combined with RPO (Robust Policy Optimization) for training-time action mean perturbation
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, rpo_alpha=0.5, alpha=1.0, full_std=True, use_expln=False,
                 std_clip=(1e-3, 1.0), std_reg=0.0):
        super().__init__()
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is not None:
            # RPO: perturb latent space instead of action space
            z = torch.FloatTensor(latent.shape).uniform_(-self.rpo_alpha, self.rpo_alpha).to(x.device)
            action_mean = self.actor_mean(latent + z)
        else:
