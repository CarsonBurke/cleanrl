# PPO + dynamics sensitivity action credit + ReLU^2 v1.
#
...
# redistributes pressure across action dimensions.
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.get_value(x), action_mean, action_std
...
def compute_dynamics_credit(
    ...
    try:
        for start in range(0, b_obs.shape[0], credit_batch_size):
            ...
            mb_model_actions = mb_actions.clamp(action_low, action_high)
            pred_delta, pred_reward = dynamics_model(mb_obs, mb_model_actions)
            ...
            mb_scores = (b_actions[start:end] - b_action_means[start:end]) / (b_action_stds[start:end] + 1e-8)
            mb_credit = action_grad * b_action_stds[start:end] * mb_scores
...
if __name__ == "__main__":
    ...
    executed_actions = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, action_mean, action_std = agent.get_action_and_value(next_obs)
                values[step] = value.flatten()
            actions[step] = action
            executed_actions[step] = action.clamp(action_low, action_high)
        ...
        b_executed_actions = executed_actions.reshape((-1,) + envs.single_action_space.shape)
        ...
        if len(valid_model_inds) > 0:
            ...
            for _ in range(args.dynamics_update_epochs):
                ...
                for start in range(0, len(valid_model_inds), dynamics_minibatch_size):
                    ...
                    mb_actions = b_executed_actions[mb_inds]
        ...
        writer.add_scalar("losses/executed_action_clipfrac", (b_actions != b_executed_actions).float().mean().item(), global_step)
