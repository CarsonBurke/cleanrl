...
#
# OPSD v30: learned full-batch gradient matching with soft ESS sparsity.
#
# v30 note: v28 learns well but still uses a softmax with an entropy bonus, so some
# behavioral-cloning mass remains on weak samples. v30 keeps the full learned
# distribution over every sample and v28's advantage-logit cold start, but adds a
# soft ESS ceiling to the weighter objective. This encourages learned sparsity only
# when the distribution is too diffuse; there is still no hard top-k or fixed exp-A.
#
...
class Args:
    ...
    weighter_ess_target_frac: float = 0.35
    """soft upper target for learned distribution ESS fraction; penalize only more diffuse weights"""
    weighter_ess_coef: float = 0.05
    """coefficient for the soft ESS sparsity penalty"""
...
class AdvantageWeighter(nn.Module):
    def __init__(self, feature_dim, hidden_size, adv_logit_init):
        super().__init__()
    ...
    def forward(self, features, args):
        scores = self.adv_logit_scale * features[:, 0] + self.net(features).squeeze(-1)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                match_ess_frac = 1.0 / ((match_weights.square().mean() + 1e-8))
                ess_loss = (match_ess_frac - args.weighter_ess_target_frac).clamp_min(0.0).square()
                weighter_loss = (
                    align_loss
                    + args.weighter_norm_coef * norm_loss
                    + args.weighter_ess_coef * ess_loss
                    - args.weighter_entropy_coef * match_entropy
                )
