# PPO + IterThink v24 Beta + SF FACTORIZED CRITIC + SF-MTP (sffactor v2).
# =====================================================================================
# v2 = v1 + SF-MTP: the psi head predicts the successor features at horizons
# h=0..critic_mtp_horizon-1 (psi(s_t) -> Psi_{t+h}), with the SAME boundary masks the
# 8278 baseline used for its return-MTP head. V(s) = psi_0(s).w exactly as v1; only
# the auxiliary horizons are new.
#
# WHY (4M evidence from v1 + its scalar-MSE control): v1 beat its matched control by
# +15% return / +0.18 EV @4M — the vector TD target works — but BOTH trail the
# baseline (v1 -15%, control -27%). The control has ONE output dim, so per-dim loss
# heterogeneity cannot explain its gap; what v1/control both removed from the 8278
# recipe was the distributional-CE head AND the MTP auxiliary (returns at t..t+5).
# v2 restores the MTP component in SF-native form — one change vs v1, attributing
# how much of the remaining gap was MTP vs loss family. If v2 closes most of it,
# CE was incidental; if not, v3 goes distributional (per-dim symlog or HL-Gauss).
# =====================================================================================
...
class Args:
    ...
    critic_mtp_horizon: int = 6      # SF-MTP: psi(s_t) predicts Psi_{t+h}, h=0..H-1 (base's MTP=6)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v2: the head emits ALL horizons; horizon 0 is the value's psi.
        self.psi_head = layer_init(
            nn.Linear(H, self.critic_mtp_horizon * self.sf_dim, bias=True), std=0.1
        )
        with torch.no_grad():
    ...
    def get_psi(self, x):
        # Successor features, all horizons: (B, mtp, sf_dim). Horizon 0 is psi(s_t).
        _, critic_feat = self._trunks(x)
        return self.psi_head(critic_feat).view(-1, self.critic_mtp_horizon, self.sf_dim)

    def get_value(self, x):
        # V(s) = psi_0(s) . w   (w intentionally NOT detached here; callers that must
        # not backprop through w use torch.no_grad()).
        return self.get_psi(x)[:, 0] @ self.w
    ...
    def get_action_and_value(self, x, z=None):
        ...
        psi = self.psi_head(critic_feat).view(-1, self.critic_mtp_horizon, self.sf_dim)
        if self.actor_dist == "gaussian":
            zr = dist.rsample()
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            next_psis = agent.get_psi(flat_next_obses)[:, 0].reshape(
            ...
            # v2 SF-MTP targets: horizon h regresses Psi_{t+h} from s_t's features,
            # valid only when no reset boundary lies between t and t+h and t+h stays
            # inside the rollout — the exact mask construction of the base's return-MTP.
            mtp = args.critic_mtp_horizon
            psi_target_mtp = psi_targets.new_zeros((args.num_steps, args.num_envs, mtp, agent.sf_dim))
            psi_mtp_mask = torch.zeros(
            ...
            for h in range(mtp):
                ...
                psi_target_mtp[:valid_len, :, h] = psi_targets[h : h + valid_len]
                psi_mtp_mask[:valid_len, :, h] = valid_h
        ...
        b_psi_target_mtp = psi_target_mtp.reshape(-1, args.critic_mtp_horizon, agent.sf_dim)
        b_psi_mtp_mask = psi_mtp_mask.reshape(-1, args.critic_mtp_horizon)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # SF critic loss: vector TD on psi (all MTP horizons, boundary-masked,
                # summed over valid horizons per row like the base's return-MTP CE) +
                # reward regression on w. GAE above already consumed V=psi_0.w.
                td_err = (new_psi - b_psi_target_mtp[mb_inds]).pow(2).mean(dim=-1)  # (B, mtp)
                mb_mask = b_psi_mtp_mask[mb_inds].to(td_err.dtype)
                sf_td_loss = (td_err * mb_mask).sum(dim=-1).mean()
                td_err_per_dim = (new_psi[:, 0] - b_psi_target_mtp[mb_inds][:, 0]).pow(2).mean(dim=0)  # (sf_dim,) h=0 diag
