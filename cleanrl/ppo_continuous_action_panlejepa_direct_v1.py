# Pan-LeJEPA Direct v1: aspirational latent goals with one-pass actions.
#
# A deterministic proposer maps the current v215-style temporal belief to an
# observation-only aspirational goal. A hindsight-trained Pan cascade predicts
# the desired next observation latent under that goal, then a zero-initialized
# inverse-action residual adjusts the base Beta policy. Runtime is one fused
# feed-forward policy call: no action search, MPC, transition rollout, reward
# prediction, or imagined policy data. Real environment reward remains only in
# PPO and in the training-only elite-future selector; it is never a policy input.
# Hindsight goals and latents are auxiliary targets, never PPO relabels.
#
# Hypothesis: increasingly strong achieved future states give the proposer a
# grounded direction in latent space; modest extrapolation beyond that elite
# state supplies an unreachable but useful command, while PPO remains the actual
# improvement engine and exact Beta KL protects the deployed composite policy.
#
# -----------------------------------------------------------------------------
# Parent: PPO + IterThink v215 affine-RMS shifted-action belief + fresh dreams.
#
# Preserve v213, but append a shifted incoming-action slot only after the
# observation encoder/SIGReg boundary. The existing temporal blocks process it
# causally as an independent per-slot stream, so it cannot write observation
# tokens; final actor, critic, transition, and policy-forecast queries may read
# it. Outbound a_t remains invisible until after policy sampling. Dream seeds
# reconstruct both belief and incoming forecast with the current WM rather than
# mixing fresh beliefs with stored stale forecasts. Add action sensitivity,
# shuffle, and fresh-policy WM-drift KL diagnostics.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    lewm_reward_loss_coef: float = 0.0  # Pan v1 has no learned reward-model objective
    lewm_termination_loss_coef: float = 0.0
    ...
    diagnostics_warm_ar: bool = False
    ...
    imagine_enable: bool = False       # direct policy only; never train on WM rollouts
    ...
    imagine_prompt_bank_rollouts: int = 0  # direct v1 keeps no unused dream prompt bank

    # Pan-LeJEPA direct policy. Goals contain exactly the 17 observation tokens;
    # reward/continuation and shifted-action tokens are excluded by construction.
    pan_aspiration_scale: float = 1.5
    pan_goal_min_offset: int = 8
    pan_goal_horizon: int = 300
    pan_quality_window: int = 32
    pan_aux_epochs: int = 1
    pan_aux_minibatch_size: int = 512
    pan_current_loss_coef: float = 1.0
    pan_elite_loss_coef: float = 1.0
    pan_next_loss_coef: float = 1.0
    pan_next_contrast_coef: float = 0.1
    pan_next_contrast_margin: float = 0.01
    pan_inverse_nll_coef: float = 0.05
    pan_inverse_contrast_coef: float = 0.01
    pan_inverse_contrast_margin: float = 0.05
    pan_composed_nll_coef: float = 0.025
    pan_aux_grad_clip: float = 1.0
    pan_logit_delta_limit: float = 2.0
    pan_aux_target_kl: float = 0.01
    pan_aux_trust_anchor_n: int = 512
...
class TransitionQueryReadout(nn.Module):
    ...
    def __init__(self, num_obs_tokens, num_semantic_tokens, dim, num_heads, ffn_mult,
        ...
        self.num_memory_tokens = num_obs_tokens if num_memory_tokens is None else num_memory_tokens
    ...
    def forward(self, state_memory, action_features):
        ...
        if space_len != self.num_memory_tokens:
            raise ValueError(f"expected {self.num_memory_tokens} memory tokens, got {space_len}")
        ...
        obs_query = obs_query.expand(batch * time_len, -1, -1) + memory[:, : self.num_obs_tokens]
...
class GoalTokenDecoder(nn.Module):
    """Decode aligned observation tokens by cross-attending belief memory."""

    def __init__(self, num_queries, dim, num_heads, ffn_mult):
        super().__init__()
    ...
    def forward(self, memory):
        if memory.ndim != 3 or memory.shape[-1] != self.dim:
            raise ValueError("goal decoder memory must be (B,S,D)")
        ...
        queries = queries + attn_out
        queries = queries + self.w2(relu_sq(self.w1(self.ffn_norm(queries))))
        return self.out_norm(queries)


class AspirationalGoalDecoder(nn.Module):
    """Predict current and elite achieved observation latents from current belief."""

    def __init__(self, num_obs_tokens, dim, num_heads, ffn_mult):
        super().__init__()
        self.num_obs_tokens = num_obs_tokens
        self.dim = dim
        self.decoder = GoalTokenDecoder(
            2 * num_obs_tokens, dim, num_heads, ffn_mult
        )
        self.current_residual = nn.Linear(dim, dim, bias=False)
        self.elite_residual = nn.Linear(dim, dim, bias=False)
        nn.init.zeros_(self.current_residual.weight)
        nn.init.zeros_(self.elite_residual.weight)

    def forward(self, memory, aspiration_scale):
        if memory.shape[1] < self.num_obs_tokens:
            raise ValueError("belief memory is missing observation-token slots")
        decoded = self.decoder(memory)
        base = memory[:, : self.num_obs_tokens]
        current = base + self.current_residual(
            decoded[:, : self.num_obs_tokens]
        )
        elite = base + self.elite_residual(
            decoded[:, self.num_obs_tokens :]
        )
        goal = current + aspiration_scale * (elite - current)
        return current, elite, goal


class GoalNextPredictor(nn.Module):
    """Pan-like H(B,G): desired immediate observation latent, without actions."""

    def __init__(self, num_obs_tokens, dim, num_heads, ffn_mult):
        super().__init__()
        self.num_obs_tokens = num_obs_tokens
        self.decoder = GoalTokenDecoder(
            num_obs_tokens, dim, num_heads, ffn_mult
        )
        self.residual = xavier_linear(nn.Linear(dim, dim, bias=False))

    def forward(self, belief_memory, goal_tokens):
        if goal_tokens.shape[1] != self.num_obs_tokens:
            raise ValueError("Pan goals must contain observation tokens only")
        memory = torch.cat([belief_memory, goal_tokens], dim=1)
        decoded = self.decoder(memory)
        return belief_memory[:, : self.num_obs_tokens] + self.residual(decoded)
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # Dreamer4-style shifted action token: row t receives a_(t-1), never a_t.
        # It is appended after the encoder/SIGReg representation and temporal
        # attention is independent per slot, so this stream cannot alter obs slots.
        self.incoming_action_embed = xavier_linear(nn.Linear(act_dim, self.dim, bias=False))
        ...
        self.transition_readout = TransitionQueryReadout(
            ...
            args.lewm_heads,
            args.lewm_ffn_mult,
            num_memory_tokens=self.num_obs_tokens + 1,
        )
    ...
    def _temporal_state_trunk(self, latent_history, incoming_action_history):
        ...
        if context_len > self.context:
            ...
            incoming_action_history = incoming_action_history[:, -self.context :]
        if incoming_action_history.shape[:2] != latent_history.shape[:2]:
            raise ValueError("incoming actions must align with latent history")
        action_slot = self.incoming_action_embed(incoming_action_history)
        tokens = torch.cat([latent_history, action_slot.unsqueeze(2)], dim=2)
        for layer in self.predictor_layers:
            tokens = layer(tokens)
        return self.predictor_norm(tokens)

    def belief_from_history(self, latent_history, incoming_action_history):
        return self._temporal_state_trunk(latent_history, incoming_action_history)[:, -1]
    ...
    def policy_forecast_from_history(self, latent_history, incoming_action_history, outbound_action):
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
            incoming_action_history = incoming_action_history[:, -self.context :]
        state_memory = self._temporal_state_trunk(latent_history, incoming_action_history)
        return self.policy_moment_outcome(
            state_memory[:, -1:], outbound_action[:, None]
        )

    def predict_mtp_from_history(self, latent_history, incoming_action_history,
                                 action_history, return_beliefs=False):
        if latent_history.shape[1] > self.context:
            latent_history = latent_history[:, -self.context :]
            incoming_action_history = incoming_action_history[:, -self.context :]
        ...
        if return_beliefs:
            return preds, state_memory
        return preds

    def imagine_step_from_history(self, latent_history, incoming_action_history, action_history):
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.pan_aspiration_scale = args.pan_aspiration_scale
        self.pan_logit_delta_limit = args.pan_logit_delta_limit
        ...
        self.agent_latent_dim = (obs_dim + 1) * args.lewm_dim
        self.agent_input_dim = self.agent_latent_dim + self.forecast_dim
        self.actor_readout = AgentQueryReadout(
            obs_dim + 1, args.lewm_dim, args.lewm_heads, args.lewm_ffn_mult
        )
        ...
        self.aspirational_goal_decoder = AspirationalGoalDecoder(
            obs_dim, args.lewm_dim, args.lewm_heads, args.lewm_ffn_mult
        )
        self.goal_next_predictor = GoalNextPredictor(
            obs_dim, args.lewm_dim, args.lewm_heads, args.lewm_ffn_mult
        )
        self.goal_inverse_readout = AgentQueryReadout(
            2 * obs_dim + 1,
            args.lewm_dim,
            args.lewm_heads,
            args.lewm_ffn_mult,
        ...
        if self.actor_dist == "gaussian":
            ...
            self.goal_alpha_residual = nn.Linear(args.lewm_dim, act_dim)
            self.goal_beta_residual = nn.Linear(args.lewm_dim, act_dim)
            nn.init.zeros_(self.goal_alpha_residual.weight)
            nn.init.zeros_(self.goal_alpha_residual.bias)
            nn.init.zeros_(self.goal_beta_residual.weight)
            nn.init.zeros_(self.goal_beta_residual.bias)
        ...
        assert self.agent_latent_dim == (self.world_model.num_obs_tokens + 1) * self.world_model.dim
        with torch.random.fork_rng():
    ...
    def _actor_dist(self, actor_feat, explore_code=None, goal_logit_delta=None):
        ...
        if goal_logit_delta is not None:
            delta_alpha, delta_beta = goal_logit_delta
            raw_alpha = raw_alpha + delta_alpha
            raw_beta = raw_beta + delta_beta
    ...
    def agent_input_from_history(self, latent_history, incoming_action_history, incoming_forecast=None):
        latent = self.world_model.belief_from_history(
            latent_history, incoming_action_history
        )
    ...
    def agent_input_from_obs(self, x):
        ...
        zero_action = latent.new_zeros(latent.shape[0], 1, self.world_model.act_dim)
        return self.agent_input_from_history(latent.unsqueeze(1), zero_action)
    ...
    def _memory_from_agent_input(self, agent_input):
        if self.detach_world_model_from_agent:
            agent_input = agent_input.detach()
        flat_memory, _ = self.split_agent_input(agent_input)
        return flat_memory.reshape(
            flat_memory.shape[0], self.obs_dim + 1, self.world_model.dim
        )

    def propose_aspirational_goal(self, agent_input, aspiration_scale=None):
        memory = self._memory_from_agent_input(agent_input)
        scale = (
            self.pan_aspiration_scale
            if aspiration_scale is None
            else aspiration_scale
        )
        return self.aspirational_goal_decoder(memory, scale)

    def predict_goal_next(self, agent_input, goal_tokens):
        memory = self._memory_from_agent_input(agent_input)
        return self.goal_next_predictor(memory, goal_tokens)

    def goal_logit_delta(self, agent_input, desired_next_tokens):
        memory = self._memory_from_agent_input(agent_input)
        inverse_memory = torch.cat([memory, desired_next_tokens], dim=1)
        feature = self.goal_inverse_readout(inverse_memory.reshape(memory.shape[0], -1))
        limit = self.pan_logit_delta_limit
        return (
            limit * torch.tanh(self.goal_alpha_residual(feature) / limit),
            limit * torch.tanh(self.goal_beta_residual(feature) / limit),
        )

    def pan_path(self, agent_input, goal_tokens=None):
        if goal_tokens is None:
            _, _, goal_tokens = self.propose_aspirational_goal(agent_input)
            # The proposer is a deterministic, training-only command source. PPO
            # treats its collection-version command as fixed policy context.
            goal_tokens = goal_tokens.detach()
        desired_next = self.predict_goal_next(agent_input, goal_tokens)
        return goal_tokens, desired_next, self.goal_logit_delta(
            agent_input, desired_next
        )

    def pan_hindsight_nll(self, agent_input, desired_next_tokens, native_action):
        """Goal-only self-imitation; the base actor is a fixed reference."""
        with torch.no_grad():
            actor_feat = self._actor_feat_from_agent_input(agent_input)
            raw_alpha = self.actor_alpha_head(actor_feat)
            raw_beta = self.actor_beta_head(actor_feat)
        delta_alpha, delta_beta = self.goal_logit_delta(
            agent_input, desired_next_tokens
        )
        alpha = 1.0 + F.softplus(raw_alpha + delta_alpha)
        beta = 1.0 + F.softplus(raw_beta + delta_beta)
        return -Beta(alpha, beta).log_prob(native_action).sum(dim=-1)
    ...
    def get_beta_params_from_agent_input(self, agent_input, explore_code, goal_tokens=None):
        actor_feat = self._actor_feat_from_agent_input(agent_input)
        _, _, goal_delta = self.pan_path(agent_input, goal_tokens)
        dist, _, _ = self._actor_dist(actor_feat, explore_code, goal_delta)
    ...
    def get_action_and_value_from_agent_input(
        ...
        _, _, goal_delta = self.pan_path(agent_input, goal_tokens)
        dist, to_action, log_det_fn = self._actor_dist(
            actor_feat, explore_code, goal_delta
        )
        if z is None:
            z = dist.sample()
    ...
    def actor_parameters(self):
        ...
        if self.actor_dist == "gaussian":
            ...
            heads = (
                ...
                + list(self.goal_next_predictor.parameters())
                + list(self.goal_inverse_readout.parameters())
                + list(self.goal_alpha_residual.parameters())
                + list(self.goal_beta_residual.parameters())
            )
        return (
    ...
    def pan_auxiliary_parameters(self):
        return (
            list(self.aspirational_goal_decoder.parameters())
            + list(self.goal_next_predictor.parameters())
            + list(self.goal_inverse_readout.parameters())
            + list(self.goal_alpha_residual.parameters())
            + list(self.goal_beta_residual.parameters())
    ...
    def assert_zero_forecast_residual_init(self):
        ...
        for name, module in (
            ("goal alpha", self.goal_alpha_residual),
            ("goal beta", self.goal_beta_residual),
        ):
            if torch.count_nonzero(module.weight).item() != 0:
                raise AssertionError(f"{name} residual must initialize zero")
            if torch.count_nonzero(module.bias).item() != 0:
                raise AssertionError(f"{name} residual bias must initialize zero")
...
@torch.no_grad()
def select_pan_elite_indices(
    rewards, transition_boundaries, min_offset, horizon, quality_window
):
    """Select the highest sustained-reward future state within each episode.

    The selector is training-only. It returns state indices, never latents, and
    excludes reset observations by comparing episode ids derived from boundary
    transitions. Distance does not enter the score.
    """
    if rewards.ndim != 2 or transition_boundaries.shape != rewards.shape:
        raise ValueError("Pan selector expects (time, env) reward/boundary tensors")
    time_len, num_envs = rewards.shape
    if not (1 <= min_offset <= horizon < time_len):
        raise ValueError("invalid Pan future-goal offset range")

    episode_ids = torch.zeros_like(transition_boundaries, dtype=torch.long)
    if time_len > 1:
        episode_ids[1:] = transition_boundaries[:-1].long().cumsum(dim=0)

    quality_sum = torch.zeros_like(rewards)
    quality_count = torch.zeros_like(rewards)
    for offset in range(quality_window):
        valid_len = time_len - offset
        if valid_len <= 0:
            break
        same_episode = (
            episode_ids[:valid_len] == episode_ids[offset : offset + valid_len]
        )
        quality_sum[:valid_len] += (
            rewards[offset : offset + valid_len] * same_episode
        )
        quality_count[:valid_len] += same_episode
    quality = quality_sum / quality_count.clamp_min(1.0)
    min_quality_count = max(1, quality_window // 2)

    offsets = torch.arange(min_offset, horizon + 1, device=rewards.device)
    source_index = torch.arange(time_len, device=rewards.device).unsqueeze(1)
    candidate_index = source_index + offsets.unsqueeze(0)
    in_bounds = candidate_index < time_len
    candidate_index = candidate_index.clamp_max(time_len - 1)

    episode_by_env = episode_ids.transpose(0, 1)
    quality_by_env = quality.transpose(0, 1)
    candidate_episode = episode_by_env[:, candidate_index]
    candidate_quality = quality_by_env[:, candidate_index]
    candidate_quality_count = quality_count.transpose(0, 1)[:, candidate_index]
    valid = in_bounds.unsqueeze(0) & (
        candidate_episode == episode_by_env.unsqueeze(-1)
    ) & (candidate_quality_count >= min_quality_count)
    scores = candidate_quality.masked_fill(~valid, -torch.inf)
    best_slot = scores.argmax(dim=-1)
    any_valid = valid.any(dim=-1)
    best_index = candidate_index.unsqueeze(0).expand(num_envs, -1, -1).gather(
        -1, best_slot.unsqueeze(-1)
    ).squeeze(-1)
    identity = torch.arange(time_len, device=rewards.device).unsqueeze(0).expand(
        num_envs, -1
    )
    best_index = torch.where(any_valid, best_index, identity)
    best_quality = quality_by_env.gather(1, best_index)
    return (
        best_index.transpose(0, 1),
        any_valid.transpose(0, 1),
        best_quality.transpose(0, 1),
        quality,
        episode_ids,
    )
...
@torch.no_grad()
def sample_pan_hindsight_indices(
    transition_boundaries, min_offset, horizon, generator
):
    """Uniformly sample an achieved future goal, independent of reward."""
    if transition_boundaries.ndim != 2:
        raise ValueError("Pan hindsight sampler expects (time, env) boundaries")
    time_len, num_envs = transition_boundaries.shape
    episode_ids = torch.zeros_like(transition_boundaries, dtype=torch.long)
    episode_ids[1:] = transition_boundaries[:-1].long().cumsum(dim=0)
    offsets = torch.arange(min_offset, horizon + 1, device=transition_boundaries.device)
    source_index = torch.arange(time_len, device=transition_boundaries.device).unsqueeze(1)
    candidate_index = source_index + offsets.unsqueeze(0)
    in_bounds = candidate_index < time_len
    candidate_index = candidate_index.clamp_max(time_len - 1)
    episode_by_env = episode_ids.transpose(0, 1)
    valid = in_bounds.unsqueeze(0) & (
        episode_by_env[:, candidate_index] == episode_by_env.unsqueeze(-1)
    )
    ...
    sampled_slot = random_scores.argmax(dim=-1)
    any_valid = valid.any(dim=-1)
    sampled_index = candidate_index.unsqueeze(0).expand(num_envs, -1, -1).gather(
        -1, sampled_slot.unsqueeze(-1)
    ).squeeze(-1)
    identity = torch.arange(time_len, device=transition_boundaries.device).unsqueeze(0).expand(
        num_envs, -1
    )
    sampled_index = torch.where(any_valid, sampled_index, identity)
    return sampled_index.transpose(0, 1), any_valid.transpose(0, 1), episode_ids
...
if __name__ == "__main__":
    ...
    assert args.pan_aspiration_scale >= 1.0
    assert 1 <= args.pan_goal_min_offset <= args.pan_goal_horizon
    assert args.pan_goal_horizon < args.num_steps
    assert args.pan_quality_window > 0
    assert args.pan_aux_epochs >= 0
    assert args.pan_aux_minibatch_size > 0
    assert args.pan_next_contrast_coef >= 0
    assert args.pan_next_contrast_margin >= 0
    assert args.pan_inverse_contrast_coef >= 0
    assert args.pan_inverse_contrast_margin >= 0
    assert args.pan_logit_delta_limit > 0
    assert 0 < args.pan_aux_target_kl <= args.target_kl
    assert args.pan_aux_trust_anchor_n > 0
    assert not args.imagine_enable, "Pan direct v1 forbids imagined policy rollouts"
    assert args.lewm_reward_loss_coef == 0.0
    assert args.lewm_termination_loss_coef == 0.0
    ...
    pan_aux_params = agent.pan_auxiliary_parameters()
    pan_aux_param_ids = {id(p) for p in pan_aux_params}
    base_actor_param_ids = {
        id(p)
        for module in (
            agent.actor_readout,
            agent.actor_trunk,
    ...
    assert pan_aux_param_ids.isdisjoint(base_actor_param_ids), (
        "Pan hindsight loss must not update the base actor"
    )
    ...
    def snapshot_optimizer_subset(parameters):
        param_values = [p.detach().clone() for p in parameters]
        state_values = []
        for p in parameters:
            state_values.append(
                {
    ...
    @torch.no_grad()
    def restore_optimizer_subset(parameters, snapshot):
        param_values, state_values = snapshot
        for p, value, saved_state in zip(parameters, param_values, state_values):
            p.copy_(value)
            current_state = optimizer.state[p]
            current_state.clear()
            current_state.update(saved_state)
    ...
    pan_hindsight_generator = torch.Generator(device=device)
    pan_hindsight_generator.manual_seed(args.seed + 50000)
    ...
    observation_latents = torch.zeros(
        (
            args.num_steps,
            args.num_envs,
    ...
    incoming_actions = torch.zeros_like(actions)
    ...
    rollout_incoming_action_history = []
    ...
    current_incoming_action = torch.zeros(
        args.num_envs, agent.world_model.act_dim, device=device
    )
    imagine_prompt_bank = []

    def build_belief_agent_input(obs_tensor, incoming_action, incoming_forecast=None):
        ...
        past_incoming_actions = rollout_incoming_action_history[-n_past:] if n_past > 0 else []
        ...
        if n_past > 0:
            ...
            past_incoming_actions = [
                action * valid.view(-1, 1).to(action.dtype)
                for action, valid in zip(past_incoming_actions, past_valids)
            ]
        belief_latents = torch.stack(past_latents + [current_latent], dim=1)
        belief_incoming_actions = torch.stack(
            past_incoming_actions + [incoming_action], dim=1
        )
        if incoming_forecast is None:
        ...
        return (
            agent.agent_input_from_history(
                belief_latents, belief_incoming_actions, incoming_forecast
            ),
            current_latent,
            belief_latents,
            belief_incoming_actions,
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            incoming_actions[step] = current_incoming_action

            with torch.no_grad():
                agent_input, current_latent, belief_latents, belief_incoming_actions = build_belief_agent_input(
                    next_obs, current_incoming_action, current_incoming_forecast
                )
                ...
                observation_latents[step] = current_latent
                value_probs[step] = p
                ...
                _, _, _, predicted_successor_forecast = (
                    agent.world_model.policy_forecast_from_history(
                        belief_latents, belief_incoming_actions, action
                    )
                )
            ...
            rollout_incoming_action_history.append(current_incoming_action.detach())
            ...
            if len(rollout_latent_history) > args.lewm_context - 1:
                ...
                rollout_incoming_action_history = rollout_incoming_action_history[-(args.lewm_context - 1) :]
            ...
            with torch.no_grad():
                next_transition_agent_input, _, _, _ = build_belief_agent_input(
                    transition_next_obs_t, action, predicted_successor_forecast
                )
            ...
            current_incoming_action = action.detach()
            if bool(boundary_tensor.any()):
                ...
                current_incoming_action = current_incoming_action.clone()
                current_incoming_action[boundary_tensor] = 0.0
        ...
        with torch.no_grad():
            ...
            (
                pan_goal_indices,
                pan_goal_mask,
                pan_selected_quality,
                pan_current_quality,
                pan_episode_ids,
            ) = select_pan_elite_indices(
                rewards,
                transition_boundaries,
                args.pan_goal_min_offset,
                args.pan_goal_horizon,
                args.pan_quality_window,
            )
            (
                pan_hindsight_indices,
                pan_hindsight_mask,
                pan_hindsight_episode_ids,
            ) = sample_pan_hindsight_indices(
                transition_boundaries,
                args.pan_goal_min_offset,
                args.pan_goal_horizon,
                pan_hindsight_generator,
            )
            if not torch.equal(pan_episode_ids, pan_hindsight_episode_ids):
                raise RuntimeError("Pan selectors disagree on episode boundaries")
            pan_next_valid = torch.zeros_like(pan_goal_mask)
            pan_next_valid[:-1] = (
                pan_episode_ids[:-1] == pan_episode_ids[1:]
            )
            pan_aux_mask = pan_goal_mask & pan_hindsight_mask & pan_next_valid
        ...
        wm_fresh_policy_kl_sum = torch.zeros((), device=device)
        wm_fresh_policy_kl_weight = torch.zeros((), device=device)
        wm_grad_norms = []
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    teacher_incoming_actions = torch.cat(
                        [
                            incoming_actions[mb_step_inds, mb_env_inds].unsqueeze(1),
                            future_actions[:, :-1],
                        ],
                        dim=1,
                    )
                    ...
                    pred_mtp, state_memory = agent.world_model.predict_mtp_from_history(
                        teacher_history,
                        teacher_incoming_actions,
                        future_actions,
                        return_beliefs=True,
                    )
                    ...
                    with torch.no_grad():
                        # Representation-drift guard diagnostic: compare the
                        # rollout base policy with the policy induced by a fully
                        # fresh current-WM belief and freshly reconstructed
                        # incoming forecast for the same final state.
                        if state_memory.shape[1] >= 2:
                            _, _, _, fresh_incoming_forecast = (
                                agent.world_model.policy_forecast_from_state_memory(
                                    state_memory[:, -2:-1], future_actions[:, -2:-1]
                                )
                            )
                            fresh_agent_input = agent.pack_agent_input(
                                state_memory[:, -1], fresh_incoming_forecast[:, 0]
                            )
                            fresh_alpha, fresh_beta = (
                                agent.get_base_beta_params_from_agent_input(fresh_agent_input)
                            )
                            rollout_alpha = base_behavior_alphas[
                                safe_hist_step_inds[:, -1], env_inds[:, -1]
                            ]
                            rollout_beta = base_behavior_betas[
                                safe_hist_step_inds[:, -1], env_inds[:, -1]
                            ]
                            fresh_policy_kl = torch.distributions.kl_divergence(
                                Beta(rollout_alpha, rollout_beta),
                                Beta(fresh_alpha, fresh_beta),
                            ).sum(dim=-1)
                            fresh_weight = forecast_weight[:, 0]
                            wm_fresh_policy_kl_sum += (fresh_policy_kl * fresh_weight).sum()
                            wm_fresh_policy_kl_weight += fresh_weight.sum()
                    ...
                    with torch.no_grad():
                        def accumulate_labeled_ar(
                            ...
                            for h in range(horizon):
                                ...
                                (
                                    ...
                                    torch.stack(hist_incoming_actions, dim=1),
                                ...
                                hist_incoming_actions.append(future_actions[:, h])
                        ...
                        if args.diagnostics_length1_ar:
                            accumulate_labeled_ar(
                                ...
                                [incoming_actions[mb_step_inds, mb_env_inds]],
                                [],
                        ...
                        if args.diagnostics_warm_ar:
                            ...
                            if bool(warm_valid.any()):
                                ...
                                win_incoming_actions = incoming_actions[win_steps, win_envs]
                                accumulate_labeled_ar(
                                    [win_summaries[:, d, obs_start:].detach() for d in range(warm_context)],
                                    [win_incoming_actions[:, d] for d in range(warm_context)],
                    ...
                    # Pan direct v1 deliberately has no learned reward or
                    # continuation objective. Avoid even decoding those heads;
                    # real reward is consumed only by PPO and the elite selector.
                    wm_reward_loss = wm_latent_loss.new_zeros(())
                    wm_termination_loss = wm_latent_loss.new_zeros(())
        ...
        def log_policy_forecast_diagnostics():
            ...
            writer.add_scalar(
                "policy_forecast/wm_induced_fresh_policy_kl",
                (wm_fresh_policy_kl_sum / wm_fresh_policy_kl_weight.clamp_min(1.0)).item(),
                global_step,
            )
        ...
        b_observation_latents = observation_latents.reshape(
            -1, agent.world_model.num_obs_tokens, agent.world_model.dim
        )
        ...
        b_u = u.reshape(-1)
        pan_env_indices = torch.arange(args.num_envs, device=device).view(1, -1)
        b_pan_goal_flat_indices = (
            pan_goal_indices * args.num_envs + pan_env_indices
        ).reshape(-1)
        b_pan_hindsight_flat_indices = (
            pan_hindsight_indices * args.num_envs + pan_env_indices
        ).reshape(-1)
        b_pan_aux_mask = pan_aux_mask.reshape(-1)
        b_pan_selected_quality = pan_selected_quality.reshape(-1)
        b_pan_current_quality = pan_current_quality.reshape(-1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.step()
                    if attempted_actor_update:
                        post_step_beta_kl = measure_behavior_beta_kl(trust_anchor_inds)
                        post_step_base_kl = measure_base_beta_kl(trust_anchor_inds)
        ...
        def measure_real_anchor_beta_kls():
            return (
                measure_behavior_beta_kl(trust_anchor_inds),
                measure_base_beta_kl(trust_anchor_inds),
            )
        ...
        if args.imagine_enable and global_step >= args.imagine_warmup_steps:
            ...
            # env e) is clean if [j-W-1 .. j] lies within one episode and
            # j>=W+1. The extra predecessor supplies the oldest incoming action
            # for a full W-row action-aware forecast context.
            ...
            for _ in range(args.imagine_batches):
                ...
                with torch.no_grad():
                    ...
                    obs_start = agent.world_model.obs_token_start
                    if clean_flat.numel() > 0:
                        win_d = torch.arange(W_im + 1, -1, -1, device=device)
                        win_steps = step_idx.unsqueeze(0) - win_d.unsqueeze(1)        # (W+2,B): s_{j-W-1}..s_j
                        win_rollouts = rollout_idx.unsqueeze(0).expand(W_im + 2, -1)
                        win_envs = env_idx.unsqueeze(0).expand(W_im + 2, -1)
                        win_summ = agent.world_model.encode_summary(
                            bank_obs[win_rollouts, win_steps, win_envs].reshape((-1,) + obs_shape),
                        ).detach().reshape(
                            W_im + 2, B_im, agent.world_model.num_latent_tokens, agent.world_model.dim
                        )
                        # Dynamics: states indices 2..W+1 ending at s_j;
                        # incoming actions indices 1..W; between-state actions
                        # indices 2..W.
                        hist_latents = [win_summ[d][:, obs_start:] for d in range(2, W_im + 2)]
                        seed_incoming = bank_actions[
                            win_rollouts[1:-1], win_steps[1:-1], win_envs[1:-1]
                        ]
                        hist_incoming_actions = [seed_incoming[d] for d in range(W_im)]
                        win_actions = bank_actions[
                            win_rollouts[2:-1], win_steps[2:-1], win_envs[2:-1]
                        ]
                        hist_actions = [win_actions[d] for d in range(W_im - 1)]
                        # Fresh f_j: W states indices 1..W ending s_(j-1),
                        # incoming actions indices 0..W-1, outbound index W.
                        forecast_incoming = bank_actions[
                            win_rollouts[:-2], win_steps[:-2], win_envs[:-2]
                        ].transpose(0, 1).contiguous()
                        _, _, _, seed_forecast = agent.world_model.policy_forecast_from_history(
                            win_summ[1:-1, :, obs_start:].transpose(0, 1).contiguous(),
                            forecast_incoming,
                            bank_actions[win_rollouts[-2], win_steps[-2], win_envs[-2]],
                        )
                        dream_incoming_forecast = seed_forecast[:, 0]
                    else:
                        ...
                        hist_incoming_actions = [torch.zeros(
                            B_im, agent.world_model.act_dim, device=device
                        )]
                        hist_actions = []                   # no real history available
                        dream_incoming_forecast = torch.zeros(
                            B_im, agent.forecast_dim, device=device
                        )
                    for t in range(H_im):
                        ...
                        incoming_hist = torch.stack(hist_incoming_actions, dim=1)
                        belief = agent.world_model.belief_from_history(latent_hist, incoming_hist)
                        ...
                        _, _, _, next_dream_forecast = (
                            agent.world_model.policy_forecast_from_history(
                                latent_hist, incoming_hist, action
                            )
                        )
                        ...
                        next_s, reward_logits, cont_logits = (
                            agent.world_model.imagine_step_from_history(
                                latent_hist, incoming_hist, dyn_actions
                            )
                        )
        ...
        # ======================= PAN HINDSIGHT AUXILIARY =======================
        # This runs only after the behavior batch has been fully consumed by PPO.
        # Achieved future goals supervise H(B,G) and the goal-specific inverse
        # residual; they are never substituted into the PPO likelihood ratio.
        pan_aux_losses = []
        pan_current_losses = []
        pan_elite_losses = []
        pan_next_losses = []
        pan_next_contrast_losses = []
        pan_inverse_nlls = []
        pan_inverse_contrast_losses = []
        pan_composed_nlls = []
        pan_aux_grad_norms = []
        pan_aux_kls = []
        pan_aux_updates_rejected = 0
        pan_aux_trust_exhausted = False
        pan_valid_indices = torch.nonzero(b_pan_aux_mask, as_tuple=False).squeeze(-1)
        if args.pan_aux_epochs > 0 and pan_valid_indices.numel() > 0:
            pan_anchor_n = min(args.pan_aux_trust_anchor_n, args.batch_size)
            pan_anchor_inds = torch.as_tensor(
                np.linspace(0, args.batch_size - 1, pan_anchor_n, dtype=np.int64),
                device=device,
                dtype=torch.long,
            )
            with torch.no_grad():
                pan_reference_alpha, pan_reference_beta = (
                    agent.get_beta_params_from_agent_input(
                        b_agent_inputs[pan_anchor_inds],
                        b_explore_codes[pan_anchor_inds],
                    )
                )
                pan_reference_dist = Beta(
                    pan_reference_alpha, pan_reference_beta
                )
            for _ in range(args.pan_aux_epochs):
                permutation = pan_valid_indices[
                    torch.randperm(pan_valid_indices.numel(), device=device)
                ]
                for start in range(0, permutation.numel(), args.pan_aux_minibatch_size):
                    mb_inds = permutation[start : start + args.pan_aux_minibatch_size]
                    mb_agent_input = b_agent_inputs[mb_inds]
                    current_target = b_observation_latents[mb_inds]
                    elite_target = b_observation_latents[
                        b_pan_goal_flat_indices[mb_inds]
                    ]
                    achieved_goal_target = b_observation_latents[
                        b_pan_hindsight_flat_indices[mb_inds]
                    ]
                    next_target = b_observation_latents[mb_inds + args.num_envs]

                    current_pred, elite_pred, _ = agent.propose_aspirational_goal(
                        mb_agent_input
                    )
                    desired_next = agent.predict_goal_next(
                        mb_agent_input, achieved_goal_target.detach()
                    )
                    shuffled_achieved_goal = achieved_goal_target.roll(1, dims=0)
                    shuffled_desired_next = agent.predict_goal_next(
                        mb_agent_input, shuffled_achieved_goal.detach()
                    )
                    current_loss = F.mse_loss(current_pred, current_target.detach())
                    elite_loss = F.mse_loss(elite_pred, elite_target.detach())
                    next_error = F.mse_loss(
                        desired_next, next_target.detach(), reduction="none"
                    ).mean(dim=(-1, -2))
                    shuffled_next_error = F.mse_loss(
                        shuffled_desired_next,
                        next_target.detach(),
                        reduction="none",
                    ).mean(dim=(-1, -2))
                    next_loss = next_error.mean()
                    next_contrast_loss = F.relu(
                        next_error
                        - shuffled_next_error
                        + args.pan_next_contrast_margin
                    ).mean()
                    inverse_nll_values = agent.pan_hindsight_nll(
                        mb_agent_input,
                        next_target.detach(),
                        b_latent_zs[mb_inds],
                    )
                    shuffled_inverse_nll = agent.pan_hindsight_nll(
                        mb_agent_input,
                        next_target.roll(1, dims=0).detach(),
                        b_latent_zs[mb_inds],
                    )
                    inverse_nll = inverse_nll_values.mean()
                    inverse_contrast_loss = F.relu(
                        inverse_nll_values
                        - shuffled_inverse_nll
                        + args.pan_inverse_contrast_margin
                    ).mean()
                    composed_nll = agent.pan_hindsight_nll(
                        mb_agent_input,
                        desired_next,
                        b_latent_zs[mb_inds],
                    ).mean()
                    pan_loss = (
                        args.pan_current_loss_coef * current_loss
                        + args.pan_elite_loss_coef * elite_loss
                        + args.pan_next_loss_coef * next_loss
                        + args.pan_next_contrast_coef * next_contrast_loss
                        + args.pan_inverse_nll_coef * inverse_nll
                        + args.pan_inverse_contrast_coef * inverse_contrast_loss
                        + args.pan_composed_nll_coef * composed_nll
                    )
                    pan_snapshot = snapshot_optimizer_subset(pan_aux_params)
                    optimizer.zero_grad(set_to_none=True)
                    pan_loss.backward()
                    pan_gn = nn.utils.clip_grad_norm_(
                        pan_aux_params, args.pan_aux_grad_clip
                    )
                    optimizer.step()
                    with torch.no_grad():
                        pan_current_alpha, pan_current_beta = (
                            agent.get_beta_params_from_agent_input(
                                b_agent_inputs[pan_anchor_inds],
                                b_explore_codes[pan_anchor_inds],
                            )
                        )
                        pan_aux_kl = torch.distributions.kl_divergence(
                            pan_reference_dist,
                            Beta(pan_current_alpha, pan_current_beta),
                        ).sum(dim=-1).mean()
                    pan_aux_kls.append(float(pan_aux_kl))
                    if (
                        not torch.isfinite(pan_aux_kl)
                        or pan_aux_kl > args.pan_aux_target_kl
                    ):
                        restore_optimizer_subset(
                            pan_aux_params, pan_snapshot
                        )
                        pan_aux_updates_rejected += 1
                        pan_aux_trust_exhausted = True
                        break
                    pan_aux_losses.append(pan_loss.item())
                    pan_current_losses.append(current_loss.item())
                    pan_elite_losses.append(elite_loss.item())
                    pan_next_losses.append(next_loss.item())
                    pan_next_contrast_losses.append(next_contrast_loss.item())
                    pan_inverse_nlls.append(inverse_nll.item())
                    pan_inverse_contrast_losses.append(
                        inverse_contrast_loss.item()
                    )
                    pan_composed_nlls.append(composed_nll.item())
                    pan_aux_grad_norms.append(float(pan_gn))
                if pan_aux_trust_exhausted:
                    break
        ...
        with torch.no_grad():
            ...
            # Deterministic batch reversal avoids perturbing the sampling RNG.
            memory_tokens = diag_memory.reshape(
                diag_n,
                agent.world_model.num_obs_tokens + 1,
                agent.world_model.dim,
            )
            shuffled_tokens = memory_tokens.clone()
            shuffled_tokens[:, -1] = memory_tokens.flip(0)[:, -1]
            shuffled_agent_input = torch.cat(
                [shuffled_tokens.reshape(diag_n, -1), diag_forecast], dim=-1
            )
            shuffled_alpha, shuffled_beta = agent.get_base_beta_params_from_agent_input(
                shuffled_agent_input
            )
            action_shuffle_kl = torch.distributions.kl_divergence(
                Beta(dist_zero.concentration1, dist_zero.concentration0),
                Beta(shuffled_alpha, shuffled_beta),
            ).sum(dim=-1).mean()
            shuffled_mean = agent.action_low + (
                agent.action_high - agent.action_low
            ) * (shuffled_alpha / (shuffled_alpha + shuffled_beta))
            base_mean = to_action_diag(dist_zero.mean)
            action_shuffle_mean_delta = (shuffled_mean - base_mean).abs().mean()
            base_value = value_logits_to_scalar(
                agent.get_value_from_agent_input(None, b_agent_inputs[:diag_n])[:, 0]
            )
            shuffled_value = value_logits_to_scalar(
                agent.get_value_from_agent_input(None, shuffled_agent_input)[:, 0]
            )
            action_shuffle_value_delta = (shuffled_value - base_value).abs().mean()

            pan_current, pan_elite, pan_goal = agent.propose_aspirational_goal(
                b_agent_inputs[:diag_n]
            )
            pan_desired_next = agent.predict_goal_next(
                b_agent_inputs[:diag_n], pan_goal
            )
            pan_delta = agent.goal_logit_delta(
                b_agent_inputs[:diag_n], pan_desired_next
            )
            pan_dist, _, _ = agent._actor_dist(
                actor_feat_diag, None, pan_delta
            )
            pan_removal_kl = torch.distributions.kl_divergence(
                pan_dist, dist_zero
            ).sum(dim=-1).mean()
            shuffled_goal = pan_goal.flip(0)
            shuffled_goal_next = agent.predict_goal_next(
                b_agent_inputs[:diag_n], shuffled_goal
            )
            shuffled_goal_delta = agent.goal_logit_delta(
                b_agent_inputs[:diag_n], shuffled_goal_next
            )
            shuffled_goal_dist, _, _ = agent._actor_dist(
                actor_feat_diag, None, shuffled_goal_delta
            )
            pan_goal_shuffle_kl = torch.distributions.kl_divergence(
                pan_dist, shuffled_goal_dist
            ).sum(dim=-1).mean()
            _, _, achieved_scale_goal = agent.propose_aspirational_goal(
                b_agent_inputs[:diag_n], aspiration_scale=1.0
            )
            achieved_scale_next = agent.predict_goal_next(
                b_agent_inputs[:diag_n], achieved_scale_goal
            )
            achieved_scale_delta = agent.goal_logit_delta(
                b_agent_inputs[:diag_n], achieved_scale_next
            )
            achieved_scale_dist, _, _ = agent._actor_dist(
                actor_feat_diag, None, achieved_scale_delta
            )
            pan_aspiration_kl = torch.distributions.kl_divergence(
                achieved_scale_dist, pan_dist
            ).sum(dim=-1).mean()
            pan_goal_delta_rms = torch.cat(pan_delta, dim=-1).square().mean().sqrt()
            pan_extrapolation_rms = (pan_goal - pan_elite).square().mean().sqrt()
        ...
        writer.add_scalar(
            "shifted_action/agent_policy_shuffle_kl",
            action_shuffle_kl.item(),
            global_step,
        )
        writer.add_scalar(
            "shifted_action/agent_mean_shuffle_delta",
            action_shuffle_mean_delta.item(),
            global_step,
        )
        writer.add_scalar(
            "shifted_action/critic_value_shuffle_delta",
            action_shuffle_value_delta.item(),
            global_step,
        )
        writer.add_scalar(
            "shifted_action/slot_rms",
            memory_tokens[:, -1].square().mean().sqrt().item(),
            global_step,
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "pan/elite_valid_fraction", b_pan_aux_mask.float().mean().item(), global_step
        )
        writer.add_scalar(
            "pan/selected_sustained_reward",
            b_pan_selected_quality[b_pan_aux_mask].mean().item()
            if bool(b_pan_aux_mask.any())
            else 0.0,
            global_step,
        )
        writer.add_scalar(
            "pan/current_sustained_reward",
            b_pan_current_quality[b_pan_aux_mask].mean().item()
            if bool(b_pan_aux_mask.any())
            else 0.0,
            global_step,
        )
        writer.add_scalar("pan/goal_removal_kl", pan_removal_kl.item(), global_step)
        writer.add_scalar("pan/goal_shuffle_kl", pan_goal_shuffle_kl.item(), global_step)
        writer.add_scalar("pan/aspiration_scale_kl", pan_aspiration_kl.item(), global_step)
        writer.add_scalar("pan/goal_logit_delta_rms", pan_goal_delta_rms.item(), global_step)
        writer.add_scalar("pan/extrapolation_rms", pan_extrapolation_rms.item(), global_step)
        writer.add_scalar(
            "pan/aux_exact_beta_kl_max",
            max(pan_aux_kls) if pan_aux_kls else 0.0,
            global_step,
        )
        writer.add_scalar(
            "pan/aux_updates_rejected", pan_aux_updates_rejected, global_step
        )
        writer.add_scalar(
            "pan/aux_trust_exhausted", float(pan_aux_trust_exhausted), global_step
        )
        if pan_aux_losses:
            writer.add_scalar("pan/aux_loss", float(np.mean(pan_aux_losses)), global_step)
            writer.add_scalar("pan/current_mse", float(np.mean(pan_current_losses)), global_step)
            writer.add_scalar("pan/elite_mse", float(np.mean(pan_elite_losses)), global_step)
            writer.add_scalar("pan/next_mse", float(np.mean(pan_next_losses)), global_step)
            writer.add_scalar(
                "pan/next_contrast_loss",
                float(np.mean(pan_next_contrast_losses)),
                global_step,
            )
            writer.add_scalar("pan/inverse_nll", float(np.mean(pan_inverse_nlls)), global_step)
            writer.add_scalar(
                "pan/inverse_contrast_loss",
                float(np.mean(pan_inverse_contrast_losses)),
                global_step,
            )
            writer.add_scalar("pan/composed_nll", float(np.mean(pan_composed_nlls)), global_step)
            writer.add_scalar(
                "pan/aux_grad_norm", float(np.mean(pan_aux_grad_norms)), global_step
            )
        ...
        writer.add_scalar("losses/dream_actor_updates_used", dream_actor_updates_used, global_step)
        writer.add_scalar(
            "losses/dream_actor_updates_rejected", dream_actor_updates_rejected, global_step
        )
        writer.add_scalar(
