# PPO + IterThink v152.1 hlscalar-decode-k6 (v152 + library-style scalar decode).
#
# Key change from v152:
#   - Keep v152 architecture and training unchanged, but consume HL-Gauss value
#     and reward predictions with library-style `symexp(E[z])` scalar decode.
#     Uncertainty shaping still uses raw-space `E[symexp(z)]` as the distribution
#     center, so this isolates scalar consumption from target projection and
#     spread diagnostics.
#
# -----------------------------------------------------------------------------
# Previous header:
