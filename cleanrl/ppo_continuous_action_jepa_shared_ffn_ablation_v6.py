# Shared-encoder JEPA scientific ablations v6, derived from shared_ffn_v4.
# PPO readouts retain two width-64 stages; reward supervision is real one-step
# normalized reward, never a policy input or planning signal. PPO minibatches
# may grow while temporal prediction/SIGReg retain fixed-size update batches.
# Projection is nGPT-style weight-only: no spherical features or learned gates.
# Diagnostics reuse the training graph (v4 drew extra SIGReg directions); compare
# v6 arms against matched v6 off/detached controls, not v4 training trajectories.
import os
import random
import time
...
class Args:
    ...
    reward_mode: Literal["off", "detached", "attached"] = "off"
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.actor_uses_encoder = args.jepa_mode in {"actor", "both"}
        self.critic_uses_encoder = args.jepa_mode in {"critic", "both"}
        self.reward_mode = args.reward_mode
        self.weight_projection = args.weight_projection
        critic_input = 64 if self.critic_uses_encoder else observation_dim
        actor_input = 64 if self.actor_uses_encoder else observation_dim
        # Advance the legacy RNG in the original critic -> actor -> encoder ->
        # JEPA order, including in the GLU arm. Its replacement readouts below
        # are RNG-isolated, so encoder/JEPA initialization stays paired too.
        self.critic = TaskFFN(critic_input, 1, 1.0, "tanh", args.task_residual)
        self.actor = TaskFFN(actor_input, 2 * self.action_dim, 0.01, "tanh", args.task_residual)
        self.encoder = nn.Sequential(
            layer_init(nn.Linear(observation_dim, 64)), nn.Tanh(),
            layer_init(nn.Linear(64, 64)), nn.Tanh(),
        ) if args.jepa_mode != "none" else None
        ...
        if args.task_activation == "stiglu":
            with torch.random.fork_rng(devices=[]):
                self.critic = TaskFFN(critic_input, 1, 1.0, "stiglu", args.task_residual)
                self.actor = TaskFFN(actor_input, 2 * self.action_dim, 0.01, "stiglu", args.task_residual)
        # Off/detached/attached have identical auxiliary capacity and leave the
        # legacy random stream untouched. None remains the exact PPO baseline.
    ...
    def parameter_groups(self):
        ...
        reward = () if self.reward_head is None else tuple(self.reward_head.parameters())
    ...
    def parameter_counts(self):
        counts = {
            ...
            "reward_head": 0 if self.reward_head is None else sum(p.numel() for p in self.reward_head.parameters()),
        }
...
def policy_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args):
    ...
    return _policy_objective(agent, agent.get_policy_and_value(observations), native_actions,
...
def representation_components(agent, observations, native_actions, next_observations, rewards, args, *, latent=None):
    ...
    if agent.encoder is None:
        return observations.new_zeros(3), {}
    ...
    try:
        ...
        def joint_model(observations, native, old_logprobs, advantages, returns, old_values,
            ...
            components, world_metrics = representation_components(
                ...
                next_observations, rewards, args,
                latent=None if latent is None else latent[:args.ssl_minibatch_size],
            )
            return ppo, components, ppo_metrics, world_metrics

        def world_model(observations, native, next_observations, rewards):
        ...
        max_updates = args.update_epochs * (args.batch_size // (
            args.ssl_minibatch_size if use_jepa else args.minibatch_size
        ))
        ...
        world_metric_names = (("ssl/shared_prediction_loss", "ssl/shared_sigreg_loss",
                               "ssl/shared_projected_std", "ssl/shared_backbone_std") if use_jepa else ())
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    if use_jepa:
                        ...
                        batches = ((indices, None) for indices in device_minibatches(
                    ...
                    for ppo_indices, ssl_indices in batches:
                        ...
                        if policy_step:
                            ...
                            ppo, components, ppo_metrics, world_metrics = evaluate(
                                ...
                                b_next_obs[ssl_indices] if use_jepa else None,
                                b_rewards[ssl_indices] if use_jepa else None,
                            )
            ...
            exposure = {
                ...
                "ssl_minibatch_size": args.ssl_minibatch_size if use_jepa else 0,
