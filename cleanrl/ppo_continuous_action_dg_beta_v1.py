...
# 64x64 tanh MLP, SCALAR value critic (MSE) on SEPARATE networks, standard max_grad_norm=0.5.
# The actor takes ONE faithful score pass per rollout (each sample used once, on-policy --
# nothing to drift against, so no ratio needed); the critic, being pure regression with no
# off-policy bias, refits for `critic_epochs` to keep the advantages informative.
#
# Hypothesis: with the confounds of the heavy iterthink base removed and the beta surprisal
# corrected (ell>=0, magnitude preserved), the delight gate (vs the no-gate REINFORCE control)
# reallocates gradient toward rare breakthroughs and improves over plain on-policy score.
# ============================================================================
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, z=None):
        """Returns (z_native, action, logp, ell_peak_ref, entropy, value).

        ell_peak_ref = log pi(mode) - log pi(z) >= 0 is the >=0 surprisal; the caller clips it
        to [0, dg_clip] and applies the gate. value is the critic estimate (grad-attached for
        the critic loss; the rollout path calls under no_grad).
        """
        dist = self._dist(x)
        if z is None:
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                z, action, logprob, _, _, value = agent.get_action_and_value(next_obs)
