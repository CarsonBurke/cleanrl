# nGPT hypersphere satransformer v9-latentread: v7 decoupled role-token
# encoder/decoders with shared latent next-observation prediction tokens.
#
# Diff vs v8-latentpred: the two learned global latent prediction tokens are
# inserted directly into the shared SA memory, not decoded by a separate latent
# cross-attention stack. Actor and critic decoders can attend to those final
# prediction-token states as part of memory, while the latent head reads the
# same tokens to predict the next normalized observation vector. The auxiliary
# MSE loss is masked on terminal transitions and gets its own clipped backward
# pass. No second backbone or private latent decoder.
#
...
class Agent(nn.Module):
    ...
    def _encode(self, x, include_latent_pred=False):
        ...
        # Shared encoder memory is obs, latent workspace, and the two dynamics
        # prediction tokens. Actor and critic readout tokens are deliberately
        # excluded so they cannot write into the representation they consume,
        # but they can read the final prediction-token states via cross-attn.
        latent = (self.latent_tokens + self.latent_type_embed).unsqueeze(0).expand(B, -1, -1)
        latent_pred_seed = (
            self.latent_pred_tokens + self.latent_pred_type_embed
        ).unsqueeze(0).expand(B, -1, -1)
        memory = torch.cat([latent, latent_pred_seed, obs_tokens], dim=1)
        ...
        if include_latent_pred:
            latent_pred_tokens = memory[:, NUM_LATENTS : NUM_LATENTS + NUM_LATENT_PRED_TOKENS]
