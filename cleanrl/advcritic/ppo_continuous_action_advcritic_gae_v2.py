# Advantage Critic GAE PPO v2
#
# Ablation hypothesis:
# - Keep a state-value bootstrap baseline only to compute TD(lambda)/GAE targets.
# - Train an action-conditional advantage critic A(s, a) to distill those targets.
# - Use detached predicted advantages in PPO's clipped policy loss, with an
#   optional mix back to raw GAE for stability.
# - Regularize E_{a~pi(.|s)}[A(s, a)] ~= 0 so the advantage critic remains
#   action-relative instead of collapsing into a second value function.
#
# This tests whether GAE can provide the low-variance TD training signal while an
# action-conditioned critic supplies smoother, denoised policy advantages.
import os
import random
import time
...
class Args:
    ...
    policy_advantage_mix: float = 0.75
    """1.0 uses learned advantages for the policy; 0.0 uses raw GAE targets"""
