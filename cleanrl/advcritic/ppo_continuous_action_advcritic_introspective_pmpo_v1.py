# Introspective Advantage Critic + PMPO-D4 PPO v1
#
# Combined hypothesis:
# - From advcritic_introspective_v3: keep V(s) only as a TD(lambda)/GAE
#   bootstrap baseline; train an action-conditional A(s, a) with a
#   heteroscedastic logvar head; blend predicted A(s, a) with raw GAE based on
#   the critic's predicted confidence; regularize E_pi[A(s,a)]^2 ~ 0.
# - From pmpo_d4_v1: replace ratio-clipped PPO with PMPO-D4 actor — squashed
#   actions via tanh + log|det J|, raw-advantage sign split with tanh(|adv|)
#   weighting on per-action-dim log-probs, plus reverse KL(old || new).
# - The introspective signal feeds PMPO's tanh weight: when the critic is
#   confident, the policy follows its own action-conditioned model; when it
#   isn't, PMPO rides raw GAE.
import os
import random
import time
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # PMPO-D4 actor with introspective advantage signal.
