# Advantage Critic PPO v1
#
# Ablation hypothesis:
# - Replace PPO's state-value critic V(s) with an action-conditional critic A(s, a).
# - Fit A(s, a) to centered Monte Carlo reward-to-go targets, then use detached
#   predicted advantages in the clipped PPO policy loss.
# - Enforce E_{a~pi(.|s)}[A(s, a)] ~= 0 with sampled policy actions, so the critic
#   learns action-relative advantage rather than leaking a state-value baseline.
#
# This intentionally removes value bootstrapping/GAE to isolate whether a learned,
# denoised advantage critic can drive PPO updates on MuJoCo continuous control.
import os
import random
import time
...
class Args:
    ...
    """unused in this ablation; kept for CLI compatibility with PPO"""
    ...
    """Toggles whether or not to use a clipped loss for the advantage critic."""
    ent_coef: float = 0.0
    ...
    normalize_advantage_targets: bool = True
    """normalize Monte Carlo advantage targets over the rollout batch"""
    policy_advantage_mix: float = 1.0
    """1.0 uses learned advantages for the policy; 0.0 uses Monte Carlo targets"""
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    if not 0.0 <= args.policy_advantage_mix <= 1.0:
        raise ValueError("policy_advantage_mix must be in [0, 1]")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Monte Carlo reward-to-go targets. There is no value bootstrap in this
        # ablation because the critic predicts A(s, a), not V(s).
        with torch.no_grad():
            returns = torch.zeros_like(rewards).to(device)
            next_return = torch.zeros(args.num_envs).to(device)
            for t in reversed(range(args.num_steps)):
                if t == args.num_steps - 1:
                    nextnonterminal = 1.0 - next_done
                else:
                    nextnonterminal = 1.0 - dones[t + 1]
                next_return = rewards[t] + args.gamma * nextnonterminal * next_return
                returns[t] = next_return
            advantage_targets = returns.clone()
        ...
        with torch.no_grad():
            b_old_advantages = agent.get_advantage(b_obs, b_actions).view(-1)
        ...
        with torch.no_grad():
            ...
            b_advantages = (
                args.policy_advantage_mix * b_predicted_advantages
                + (1.0 - args.policy_advantage_mix) * b_advantage_targets
            )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.clip_vloss:
                    v_loss_unclipped = (newvalue - b_advantage_targets[mb_inds]) ** 2
                    v_clipped = b_old_advantages[mb_inds] + torch.clamp(
                        newvalue - b_old_advantages[mb_inds],
                        -args.clip_coef,
                        args.clip_coef,
                    )
                    v_loss_clipped = (v_clipped - b_advantage_targets[mb_inds]) ** 2
                    v_loss_max = torch.max(v_loss_unclipped, v_loss_clipped)
                    adv_loss = 0.5 * v_loss_max.mean()
                else:
                    adv_loss = 0.5 * ((newvalue - b_advantage_targets[mb_inds]) ** 2).mean()
                zero_mean_loss = advantage_zero_mean_loss(agent, b_obs[mb_inds], args.advantage_zero_mean_samples)
        ...
        writer.add_scalar("losses/advantage_explained_variance", explained_var, global_step)
