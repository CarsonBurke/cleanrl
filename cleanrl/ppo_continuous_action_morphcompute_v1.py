...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        ...
        self.actor_mean = layer_init(nn.Linear(args.cell_dim, action_dim), std=0.01)
        self.actor_logstd = layer_init(nn.Linear(args.cell_dim, action_dim), std=0.01, bias_const=-1.0)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        action_logstd = self.actor_logstd(actor_features).clamp(-5.0, 2.0)
        ...
        stats = {
            "actor": actor_stats,
            "critic": critic_stats,
            "action_clip_frac": (action.abs() > 1.0).float().mean(dim=1),
        }
        return (
            action,
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if last_stats is not None:
            ...
            writer.add_scalar("morph/action_clip_frac", last_stats["action_clip_frac"].detach().mean().item(), global_step)
    ...
    if args.save_model:
        ...
        class EvalAgent(Agent):
            ...
            def get_action_and_value(self, x, action=None):
                action, logprob, entropy, value, _, _, _ = super().get_action_and_value(x, action)
