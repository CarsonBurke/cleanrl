"""PPO Transformer with Dual-Decoder Tanh Factors (v28).

Goal: decouple exploitation and exploration learning while preserving expressive,
state-dependent correlated noise.

Distribution:
  u = mu(s) + F(s) z + sigma_diag(s) ⊙ eps
  a = tanh(u)
  z ~ N(0, I_R), eps ~ N(0, I_A)

Covariance:
  Cov[u|s] = F(s) F(s)^T + diag(sigma_diag^2)

Where:
  - mean decoder tokens specialize for pre-tanh action means
  - exploration decoder tokens specialize for covariance construction
  - action<->source attention mixes source-basis vectors into per-action factors
  - state-gated learnable floors prevent exploration collapse in hard states

Exact pre-tanh Gaussian log_prob uses Woodbury + determinant lemma; action-space
log_prob adds tanh Jacobian correction.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # === Actor: shared trunk + decoupled mean/explore decoders ===
        ...
        self.actor_trunk1 = TransformerBlock(d_token, 4, hidden_dim, use_bf16_attn=args.use_bf16_attn)
        self.actor_trunk2 = TransformerBlock(d_token, 4, hidden_dim, use_bf16_attn=args.use_bf16_attn)

        self.mean_action_anchor_tokens = nn.Parameter(torch.randn(self._act_dim, d_token) * 0.02)
        self.mean_action_tokenize = nn.Linear(hidden_dim, self._act_dim * d_token)
        self.mean_dec1 = TransformerBlock(d_token, 4, hidden_dim, use_bf16_attn=args.use_bf16_attn)
        self.mean_dec2 = TransformerBlock(d_token, 4, hidden_dim, use_bf16_attn=args.use_bf16_attn)

        self.explore_action_anchor_tokens = nn.Parameter(torch.randn(self._act_dim, d_token) * 0.02)
        self.explore_action_tokenize = nn.Linear(hidden_dim, self._act_dim * d_token)
        ...
        self.explore_dec1 = TransformerBlock(d_token, 4, hidden_dim, use_bf16_attn=args.use_bf16_attn)
        self.explore_dec2 = TransformerBlock(d_token, 4, hidden_dim, use_bf16_attn=args.use_bf16_attn)

        # Action-token decoded mean and pooled residual diagonal scale.
        self.action_mean_out = layer_init(nn.Linear(d_token, 1), std=0.01)
        self.mean_global_out = layer_init(nn.Linear(d_token, self._act_dim), std=0.01)
        ...
        nn.init.zeros_(self.diag_floor_gate_out.weight)
        nn.init.zeros_(self.diag_floor_gate_out.bias)
        nn.init.zeros_(self.action_floor_gate_out.weight)
    ...
    def _get_actor_tokens(self, x):
        ...
        policy_tokens = self.actor_trunk1(policy_tokens)
        policy_tokens = self.actor_trunk2(policy_tokens)

        mean_action_state = self.mean_action_tokenize(h).reshape(B, self._act_dim, self._d_token)
        mean_action_anchor = self.mean_action_anchor_tokens.unsqueeze(0).expand(B, -1, -1)
        mean_action_tokens = _normalize(mean_action_state + mean_action_anchor)

        mean_tokens = torch.cat([policy_tokens, mean_action_tokens], dim=1)
        mean_tokens = self.mean_dec1(mean_tokens)
        mean_tokens = self.mean_dec2(mean_tokens)
        mean_action_tokens = mean_tokens[:, self._n_policy_tokens:]

        explore_action_state = self.explore_action_tokenize(h).reshape(B, self._act_dim, self._d_token)
        explore_action_anchor = self.explore_action_anchor_tokens.unsqueeze(0).expand(B, -1, -1)
        explore_action_tokens = _normalize(explore_action_state + explore_action_anchor)
        ...
        explore_tokens = torch.cat([policy_tokens, explore_action_tokens, source_tokens], dim=1)
        explore_tokens = self.explore_dec1(explore_tokens)
        explore_tokens = self.explore_dec2(explore_tokens)
        p0 = self._n_policy_tokens
        p1 = p0 + self._act_dim
        explore_action_tokens = explore_tokens[:, p0:p1]
        source_tokens = explore_tokens[:, p1:]
        return policy_tokens, mean_action_tokens, explore_action_tokens, source_tokens

    def _get_dist_params(self, x):
        policy_tokens, mean_action_tokens, explore_action_tokens, source_tokens = self._get_actor_tokens(x)
        ...
        q = self.factor_q(explore_action_tokens).reshape(B, self._act_dim, self.factor_heads, self.factor_head_dim).transpose(1, 2)
        ...
        factor_res = self.factor_row_out(explore_action_tokens)  # (B, A, R)
    ...
    def _gaussian_log_prob_and_entropy(self, delta, diag_std, F_mat):
        B = delta.shape[0]
        A = self._act_dim
    ...
    def _atanh(self, x):
        x = x.clamp(min=-1.0 + self.tanh_eps, max=1.0 - self.tanh_eps)
    ...
    def get_action_and_value(self, x, action=None):
        mean, diag_std, F_mat, _, _, _ = self._get_dist_params(x)
        ...
        if action is None:
            ...
            pre_tanh = mean + sub_noise + diag_noise
            action = torch.tanh(pre_tanh)
        else:
            pre_tanh = self._atanh(action)

        delta = pre_tanh - mean
        base_log_prob, base_entropy = self._gaussian_log_prob_and_entropy(delta, diag_std, F_mat)
        log_det_jac = torch.log(1.0 - action.pow(2) + self.tanh_eps).sum(-1)
        log_prob = base_log_prob - log_det_jac
        # Use base Gaussian entropy as a stable proxy for policy entropy regularization.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            mean_probe, diag_std_probe, F_probe, action_gain_probe, source_scale_probe, floor_probe = agent._get_dist_params(b_obs[:256])
            ...
            pre_tanh_probe = mean_probe + noise_probe
            action_probe = torch.tanh(pre_tanh_probe)
            ...
            writer.add_scalar("explore/action_abs_mean", action_probe.abs().mean().item(), global_step)
            writer.add_scalar("explore/action_sat_frac", (action_probe.abs() > 0.95).float().mean().item(), global_step)
