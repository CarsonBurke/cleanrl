"""PPO Transformer with State-Conditioned Normalizing Flow v3

v3 over v1: init flow log_scale bias to compress N(0,I) base noise at start.
Coupling layers start active (not identity), giving fast early learning while
preserving the expressive ceiling from transforming wide base noise.
Wider log_scale range (tanh * 2.0).
"""
import os
import random
import time
...
class ExplorationHead(nn.Module):
    """Learned exploration strategy via cross-attention + self-attention.

    1. Cross-attention: exploration queries attend to policy tokens
       → each actuator gathers state-relevant information
    2. Self-attention: exploration queries attend to each other
       → actuators coordinate exploration (direction + magnitude)
    3. Output: unclamped log_std per action dim

    The self-attention step is critical — it lets the network reason about
    cross-actuator exploration structure (e.g. "hip exploring right →
    knee should also explore more").
    """
    def __init__(self, d_token, act_dim, n_heads=4):
        ...
        # Output: per-token → shift and log_scale for affine coupling layers
        # We use 3 coupling layers, each transforms half the dims
        self.n_flow_layers = 3
        # Each coupling layer needs shift + log_scale for act_dim//2 dims
        # conditioned on the exploration token representations
        self._act_dim = act_dim
        self.flow_params = nn.Linear(d_token, 2 * act_dim * self.n_flow_layers, bias=True)
        nn.init.zeros_(self.flow_params.weight)
        # Init log_scale bias to compress N(0,I) → ~exp(LOG_STD_INIT) scale at start
        # Bias layout: (n_flow_layers * 2 * act_dim), reshaped to (n_layers, 2, act_dim)
        # Index [:, 1, :] = log_scale slots
        with torch.no_grad():
            bias = self.flow_params.bias.view(self.n_flow_layers, 2, act_dim)
            bias[:, 0, :] = 0.0   # shifts start at 0
            bias[:, 1, :] = -0.7  # log_scale: tanh(-0.7)*2≈-1.24, across 3 layers ≈ exp(-1.24*1.5) ≈ 0.15 total scale

    def forward(self, policy_tokens):
        # policy_tokens: (B, n_tokens, d_token)
        B = policy_tokens.shape[0]
        A = self.explore_queries.shape[0]  # act_dim
        ...
        x = self.cross_out_proj(cross_out)  # (B, act_dim, d_token)
        ...
        # Pool exploration tokens → conditioning vector for flow
        cond = x.mean(dim=1)  # (B, d_token)
        flow_raw = self.flow_params(cond)  # (B, 2 * act_dim * n_flow_layers)
        # Reshape to (n_flow_layers, B, 2, act_dim) — shift and log_scale per layer
        flow_raw = flow_raw.reshape(B, self.n_flow_layers, 2, A)
        flow_raw = flow_raw.permute(1, 0, 2, 3)  # (n_layers, B, 2, act_dim)

        return flow_raw  # shifts and log_scales for each coupling layer
...
class Agent(nn.Module):
    ...
    def _flow_forward(self, z, flow_params, mean):
        """Transform base noise z through affine coupling layers.
        Returns action, sum of log_det_jacobian."""
        x = z  # (B, act_dim)
        log_det = torch.zeros(z.shape[0], device=z.device)
        A = z.shape[1]
        mid = A // 2

        for i in range(flow_params.shape[0]):
            shift = flow_params[i, :, 0, :]  # (B, act_dim)
            log_scale = flow_params[i, :, 1, :].tanh() * 2.0  # wider range: exp([-2, 2])

            # Alternate which half gets transformed
            if i % 2 == 0:
                x_fixed, x_transform = x[:, :mid], x[:, mid:]
                s, t = log_scale[:, mid:], shift[:, mid:]
    ...
    def _flow_inverse(self, action, flow_params, mean):
        """Inverse transform: action → base noise z.
        Returns z, sum of log_det_jacobian (negative direction)."""
        x = action - mean
        log_det = torch.zeros(action.shape[0], device=action.device)
        A = action.shape[1]
        mid = A // 2

        # Reverse order of coupling layers
        for i in reversed(range(flow_params.shape[0])):
            shift = flow_params[i, :, 0, :]
            log_scale = flow_params[i, :, 1, :].tanh() * 2.0
    ...
    def get_action_and_value(self, x, action=None):
        ...
        flow_params = self.explore_head(tokens)  # (n_layers, B, 2, act_dim)

        if action is None:
            # Sample: z ~ N(0, I), transform through flow
            z = torch.randn_like(mean)
            action, log_det_fwd = self._flow_forward(z, flow_params, mean)
            # log_prob = log_prob_base(z) + log_det_inverse = log_prob_base(z) - log_det_fwd
            ...
            z, log_det_inv = self._flow_inverse(action, flow_params, mean)
            base_log_prob = -0.5 * (z.pow(2) + np.log(2 * np.pi)).sum(-1)
            log_prob = base_log_prob + log_det_inv

        # Entropy approximation: use base entropy + E[log_det]
        # Exact entropy is intractable for flows; base entropy is a lower bound
        base_entropy = 0.5 * mean.shape[-1] * (1 + np.log(2 * np.pi))
        entropy = torch.full((x.shape[0],), base_entropy, device=x.device)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            fp = agent.explore_head(tokens)
            log_scales = fp[:, :, 1, :].tanh() * 2.0  # (n_layers, 256, act_dim)
            writer.add_scalar("explore/flow_log_scale_mean", log_scales.mean().item(), global_step)
            writer.add_scalar("explore/flow_log_scale_std", log_scales.std().item(), global_step)
