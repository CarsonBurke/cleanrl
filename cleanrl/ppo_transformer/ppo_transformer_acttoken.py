"""PPO Transformer with Action-Space Tokenization (Variant B)

nGPT-style hypersphere transformer where each token = one actuator.
Self-attention between action tokens learns cross-actuator correlations
for both policy mean and state-dependent exploration noise.

Architecture:
- obs → Linear(obs_dim, 64) → RMSNorm → SiLU → obs_features
- Per-actuator tokens: learnable_embed[i] + Linear(64, d_token)(obs_features)
- nGPT transformer block: tokens attend to each other
- Per-token output heads: mean_i, log_std_i = heads(token_i)

Self-attention directly learns cross-actuator structure:
"when exploring hip torque, also explore knee torque similarly."
Unified mean+noise in one forward pass — no separate noise path.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # === Actor: action-space tokenization ===
        # Observation encoder (shared features for conditioning tokens)
        self.actor_fc1 = layer_init(nn.Linear(obs_dim, hidden_dim))
        self.actor_norm1 = RMSNorm(hidden_dim)

        # Per-actuator learnable embeddings + obs conditioning
        self.actuator_embed = nn.Parameter(torch.randn(act_dim, d_token) * 0.02)
        self.obs_to_token = nn.Linear(hidden_dim, d_token, bias=False)

        # Transformer: tokens attend to each other → cross-actuator reasoning
        self.actor_transformer = TransformerBlock(d_token=d_token, n_heads=4, ffn_hidden=hidden_dim)

        # Per-token output: mean + log_std from each actuator's token
        self.mean_head = nn.Linear(d_token, 1, bias=True)
        nn.init.normal_(self.mean_head.weight, std=0.01)
        nn.init.zeros_(self.mean_head.bias)

        self.noise_head = nn.Linear(d_token, 1, bias=True)
        nn.init.zeros_(self.noise_head.weight)
        nn.init.constant_(self.noise_head.bias, LOG_STD_INIT)

        # === Critic: same virtual-token architecture (not action-tokenized) ===
        n_critic_tokens = 8
        self.critic_fc1 = layer_init(nn.Linear(obs_dim, hidden_dim))
        self.critic_norm1 = RMSNorm(hidden_dim)
        self.critic_tokenize = nn.Linear(hidden_dim, n_critic_tokens * d_token)
        ...
        self._n_critic_tokens = n_critic_tokens

    def _actor_forward(self, x):
        """Returns (action_mean, action_std) — both state-dependent."""
        B = x.shape[0]
        h = F.silu(self.actor_norm1(self.actor_fc1(x)))  # (B, 64)

        # Build per-actuator tokens: learnable embed + obs conditioning
        obs_cond = self.obs_to_token(h)  # (B, d_token)
        tokens = self.actuator_embed.unsqueeze(0) + obs_cond.unsqueeze(1)  # (B, act_dim, d_token)
        tokens = _normalize(tokens)
        tokens = self.actor_transformer(tokens)  # (B, act_dim, d_token)

        # Per-token heads
        action_mean = self.mean_head(tokens).squeeze(-1)  # (B, act_dim)
        log_std = self.noise_head(tokens).squeeze(-1).clamp(LOG_STD_MIN, LOG_STD_MAX)  # (B, act_dim)
    ...
    def _critic_features(self, x):
        ...
        tokens = self.critic_tokenize(h).reshape(B, self._n_critic_tokens, self._d_token)
        tokens = _normalize(tokens)
    ...
    def get_action_and_value(self, x, action=None):
        action_mean, action_std = self._actor_forward(x)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # noise metrics from last minibatch — approximate
        with torch.no_grad():
            _, std_sample = agent._actor_forward(b_obs[:256])
            writer.add_scalar("explore/log_std_mean", std_sample.log().mean().item(), global_step)
            writer.add_scalar("explore/log_std_std", std_sample.log().std().item(), global_step)
