"""PPO Transformer with State-Conditioned Normalizing Flow v2

v2 changes over v1:
1. Learnable base_log_std (init LOG_STD_INIT) — tighter initial noise, faster early learning
2. MC entropy estimate (-log_prob) — enables actual entropy regularization
3. Softclamp log_scale (tanh * 2.0) — wider dynamic range for flow transforms
"""
import os
import random
import time
...
class ExplorationHead(nn.Module):
    ...
    def __init__(self, d_token, act_dim, n_heads=4):
        ...
        nn.init.zeros_(self.flow_params.bias)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Learnable base noise std — tighter init for faster early learning
        self.base_log_std = nn.Parameter(torch.full((act_dim,), LOG_STD_INIT))
    ...
    def _flow_forward(self, z, flow_params, mean):
        ...
        z is already scaled by base_std. Returns action, sum of log_det_jacobian."""
        x = z  # (B, act_dim)
    ...
    def _flow_inverse(self, action, flow_params, mean):
        ...
        Returns z (in base_std-scaled space), sum of log_det_jacobian (negative direction)."""
        x = action - mean
    ...
    def get_action_and_value(self, x, action=None):
        ...
        base_std = self.base_log_std.exp()  # learnable base noise scale

        if action is None:
            # Sample: z ~ N(0, base_std^2), transform through flow
            eps = torch.randn_like(mean)
            z = eps * base_std
            action, log_det_fwd = self._flow_forward(z, flow_params, mean)
            # log_prob = log N(z; 0, base_std^2) - log_det_fwd
            base_log_prob = (-0.5 * eps.pow(2) - 0.5 * np.log(2 * np.pi) - self.base_log_std).sum(-1)
        ...
        # MC entropy estimate: E[-log p(a|s)] ≈ -log_prob per sample
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            log_scales = fp[:, :, 1, :].tanh()  # (n_layers, 256, act_dim)
