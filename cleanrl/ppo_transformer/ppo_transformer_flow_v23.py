"""PPO Transformer with Directional Low-Rank Exploration (v23).
...
Where transformer-decoded mode tokens produce:
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.mix_block1 = TransformerBlock(d_token, 4, hidden_dim)
        self.mix_block2 = TransformerBlock(d_token, 4, hidden_dim)
    ...
    def _get_policy_and_mode_tokens(self, x):
        ...
        all_tokens = torch.cat([policy_tokens, mode_tokens], dim=1)
        all_tokens = self.mix_block1(all_tokens)
        all_tokens = self.mix_block2(all_tokens)
        policy_out = all_tokens[:, :self._n_policy_tokens]
        mode_out = all_tokens[:, self._n_policy_tokens:]
        return policy_out, mode_out
    ...
    def _get_dist_params(self, x):
        ...
        diag_raw = self.diag_std_out(mean_h) + self.diag_log_std_offset.unsqueeze(0)
