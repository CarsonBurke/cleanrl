"""PPO Transformer with Scaled-Up Backbone-Native Noise (v8)

Scales up v6's proven architecture (noise tokens in backbone + L matrix).
v6 scored ~6005 on HalfCheetah — the best result so far.

Key changes from v6:
1. 3 TRANSFORMER BLOCKS (was 2): More rounds of attention for noise tokens
   to exchange cross-dim info when producing L matrix rows.
2. d_token = 48 (was 32): Richer token representations for both policy
   and noise tokens. Each noise token has more capacity to encode its
   row of the L matrix.
3. n_heads = 6 (was 4): Keeps head_dim=8, adds more attention heads.

What stays from v6 (proven to work):
- Fixed noise token parameters (NOT obs-conditioned — v7 showed this is better)
- Lower-triangular L matrix with exact log_prob
- Mean correction from noise tokens
- nGPT normalization on hypersphere
- Zero-init readout heads (starts as identity/diagonal Gaussian)
"""
import os
import random
import time
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, d_token=48, n_heads=6, ffn_hidden=96):
        super().__init__()
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        hidden_dim = 64
        n_policy_tokens = 8
        d_token = 48  # wider tokens (was 32)
        n_heads = 6   # more heads (was 4), head_dim=8

        # === Actor: obs → policy tokens + noise tokens → 3 transformer blocks ===
        ...
        # Fixed noise token parameters (one per action dim)
        self.noise_tokens = nn.Parameter(torch.randn(act_dim, d_token) * 0.02)

        # 3 transformer blocks (was 2)
        self.actor_transformer1 = TransformerBlock(d_token=d_token, n_heads=n_heads, ffn_hidden=hidden_dim)
        self.actor_transformer2 = TransformerBlock(d_token=d_token, n_heads=n_heads, ffn_hidden=hidden_dim)
        self.actor_transformer3 = TransformerBlock(d_token=d_token, n_heads=n_heads, ffn_hidden=hidden_dim)
        ...
        # === Critic backbone (separate, also scaled up) ===
        ...
        self.critic_transformer = TransformerBlock(d_token=d_token, n_heads=n_heads, ffn_hidden=hidden_dim)
