"""PPO Transformer with Residual Action Prediction (v15)

Instead of predicting absolute actions, predicts action DELTAS:
  action_t = prev_action_{t-1} + delta(s_t)
  delta ~ N(mean_delta(s), L(s)L(s)^T)

This creates natural temporal smoothness in actions -- good for locomotion
where joint torques should change gradually. Unlike v9 (fixed eps per rollout
which was catastrophic), this operates at the action level: the noise is
resampled each step but the action has momentum through the residual.

Key changes from v6:
  1. Agent takes (obs, prev_action) as input
  2. prev_action embedded and added to policy tokens
  3. Network outputs delta (not absolute action)
  4. Training loop tracks and stores prev_action per env
  5. Log prob computed on delta, not action

v12 (mixture): ~12% behind v6 at 5M, widening
v13 (gated noise): killed at 1.2M, redundant with L matrix
v14 (obs-to-L skip): ~8% behind v6 at 4M
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # === Actor: (obs, prev_action) → policy tokens + noise tokens → transformer → delta ===
        self.actor_fc1 = layer_init(nn.Linear(obs_dim + act_dim, hidden_dim))  # obs + prev_action
    ...
    def _get_actor_tokens(self, x, prev_action):
        """Process (obs, prev_action) through actor backbone."""
        B = x.shape[0]
        actor_input = torch.cat([x, prev_action], dim=-1)
        h = F.silu(self.actor_norm1(self.actor_fc1(actor_input)))
    ...
    def _get_L_and_correction(self, noise_tokens):
        """From noise tokens, produce L matrix and mean correction."""
        A = self._act_dim
    ...
    def get_action_and_value(self, x, prev_action, action=None):
        """Predict action delta and compute log_prob.

        Args:
            x: observations (B, obs_dim)
            prev_action: previous action (B, act_dim) — zeros at episode start
            action: if provided, compute log_prob for this action (training)
        """
        policy_tokens, noise_tokens = self._get_actor_tokens(x, prev_action)
        base_mean = self._get_mean(policy_tokens)
        L, log_det, mean_correction = self._get_L_and_correction(noise_tokens)
        mean_delta = base_mean + mean_correction

        if action is None:
            eps = torch.randn_like(mean_delta)
            noise = torch.bmm(L, eps.unsqueeze(-1)).squeeze(-1)
            delta = mean_delta + noise
            action = prev_action + delta
        else:
            # Recover delta from stored action and prev_action
            delta = action - prev_action
            noise = delta - mean_delta
            eps = torch.linalg.solve_triangular(
                L, noise.unsqueeze(-1), upper=False
            ).squeeze(-1)

        # Log prob of the delta (not the absolute action)
...
if __name__ == "__main__":
    ...
    prev_actions_buf = torch.zeros((args.num_steps, args.num_envs) + act_shape).to(device)
    ...
    # Track previous action per env (zeros at episode start)
    curr_prev_action = torch.zeros(args.num_envs, *act_shape).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            prev_actions_buf[step] = curr_prev_action

            # ALGO LOGIC: action logic (agent outputs delta, action = prev + delta)
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(next_obs, curr_prev_action)
                values[step] = value.flatten()
            actions[step] = action
            logprobs[step] = logprob

            # Update prev_action for next step
            curr_prev_action = action.clone()
            ...
            # Reset prev_action for envs that terminated/truncated
            done_mask = next_done.bool()
            if done_mask.any():
                curr_prev_action[done_mask] = 0.0
        ...
        b_prev_actions = prev_actions_buf.reshape((-1,) + act_shape)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(
                    b_obs[mb_inds], b_prev_actions[mb_inds], b_actions[mb_inds]
                )
        ...
        # noise metrics
        with torch.no_grad():
            pt, nt = agent._get_actor_tokens(b_obs[:256], b_prev_actions[:256])
            L_probe, _, mcorr_probe = agent._get_L_and_correction(nt)
