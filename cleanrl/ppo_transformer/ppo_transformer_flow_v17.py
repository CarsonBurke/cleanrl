"""PPO Transformer with Stabilized Noise (v17)

v6 architecture + two stability improvements for Hopper/Walker:

1. target_kl=0.02: Limits policy change per update epoch, preventing
   the rapid noise collapse observed on Hopper (diag 0.97->0.24) and
   Walker (0.97->0.40) where noise collapses before optimal policy found.

2. Diag clamp (-3.0, 2.0): Noise floor of exp(-3)=0.05 instead of
   exp(-5)=0.007. Prevents complete noise elimination.

v6 results: HC=6005, Hop=1575, Walk=3751
Best baselines: HC=6005, Hop=1871(gsde), Walk=4866(lattice)
v6 underperforms on Hopper/Walker due to premature noise collapse.
"""
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def _get_L_and_correction(self, noise_tokens):
        ...
        log_diag = diag.clamp(-3.0, 2.0)  # noise floor: exp(-3) ≈ 0.05
