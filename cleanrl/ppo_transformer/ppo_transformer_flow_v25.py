"""PPO Transformer with Action-Token Factor Exploration (v25).
...
Where:
  ...
  - action<->source token attention defines signed factor loadings in F(s)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.actor_block1 = TransformerBlock(d_token, 4, hidden_dim)
        self.actor_block2 = TransformerBlock(d_token, 4, hidden_dim)

        # Action-token decoded mean + diagonal scale.
    ...
    def _get_dist_params(self, x):
        _, action_tokens, source_tokens = self._get_actor_tokens(x)

        mean = self.action_mean_out(action_tokens).squeeze(-1)

        diag_raw = self.diag_std_out(action_tokens).squeeze(-1) + self.diag_log_std_offset.unsqueeze(0)
        ...
        signed_weights = torch.tanh(logits)

        F_mat = action_gain.unsqueeze(-1) * source_scale.unsqueeze(1) * signed_weights  # (B, A, R)
