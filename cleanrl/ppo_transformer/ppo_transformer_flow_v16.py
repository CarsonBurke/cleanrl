"""PPO Transformer with Standard Pre-Norm (v16) -- nGPT ablation

Tests whether nGPT's hypersphere constraint helps or hurts. Replaces:
  - L2 normalize -> RMSNorm pre-normalization
  - Geodesic residual (alpha * (h - x)) -> Standard additive residual
  - Weight normalization on QKV/FFN -> Standard initialization

Everything else identical to v6: same token structure (8 policy + 6 noise),
same L matrix readout, same noise mechanism, same hyperparameters.

If standard pre-norm outperforms nGPT, we can improve v6 by removing the
hypersphere constraint. If nGPT is better, its normalization is genuinely
beneficial for this architecture.

v12 final: 4807 (80% of v6). v14 degraded. v15 catastrophic.
v6 remains at 6005 -- the target to beat.
"""
import os
import random
import time
...
class TransformerBlock(nn.Module):
    """Standard pre-norm Transformer block (no nGPT hypersphere constraint)."""
    def __init__(self, d_token=32, n_heads=4, ffn_hidden=64):
        ...
        nn.init.zeros_(self.o_proj.weight)  # zero-init for clean residual start
        ...
        nn.init.zeros_(self.ffn_out.weight)  # zero-init for clean residual start

    def forward(self, x):
        B, T, D = x.shape

        # Pre-norm attention with additive residual
        h = self.attn_norm(x)
        qkv = self.qkv(h)
        ...
        # Pre-norm SwiGLU FFN with additive residual
        h = self.ffn_norm(x)
        gate_value = self.ffn_gate_value(h)
...
class Agent(nn.Module):
    ...
    def _get_actor_tokens(self, x):
        ...
        # No L2 normalization — standard pre-norm handles it in the transformer

        # Expand noise tokens to batch (no L2 normalization)
        noise_tokens = self.noise_tokens.unsqueeze(0).expand(B, -1, -1)
