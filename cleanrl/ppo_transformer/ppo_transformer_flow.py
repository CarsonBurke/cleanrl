"""PPO Transformer with State-Conditioned Normalizing Flow

nGPT-style hypersphere transformer backbone. Exploration head (cross-attn +
self-attn) conditions affine coupling layers that transform base Gaussian
noise into an expressive, potentially multimodal, asymmetric, heavy-tailed
action distribution. Log-prob is tractable via change-of-variables.
"""
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def _flow_forward(self, z, flow_params, mean):
        ...
        for i in range(flow_params.shape[0]):
            ...
            log_scale = flow_params[i, :, 1, :].tanh()  # (B, act_dim) bounded
    ...
    def _flow_inverse(self, action, flow_params, mean):
        ...
        for i in reversed(range(flow_params.shape[0])):
            ...
            log_scale = flow_params[i, :, 1, :].tanh()
