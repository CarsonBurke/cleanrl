"""PPO Transformer with Per-Dimension Action Tokens (v18)

Adds learnable action tokens (one per action dim) alongside noise tokens.
Each action token attends to policy tokens and directly predicts per-dim mean,
mirroring how noise tokens predict per-dim L matrix rows.

Architecture:
  1. Obs -> FC -> 4 policy tokens (encode state, shared representation)
  2. + 6 action tokens (one per dim, learnable params -> per-dim mean)
  3. + 6 noise tokens (one per dim, learnable params -> L matrix rows)
  4. = 16 tokens total (vs v6's 14)
  5. 2 nGPT transformer blocks
  6. Action tokens -> per-dim means (Linear(d_token, 1) each)
  7. Noise tokens -> L matrix + mean correction (same as v6)

Hypothesis: v6 pools 8 policy tokens and uses an FC for mean prediction.
This aggregation may lose per-dimension info. Separate action tokens can
specialize per action dim, with cross-token attention for coordination.

v17 failed: target_kl + noise floor hurt all 3 envs.
v6 results: HC=5803(last25%), Hop=1575, Walk=3753.
"""
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        hidden_dim = 64
        n_policy_tokens = 4  # reduced from 8 to keep total tokens similar
        d_token = 32

        # === Actor: obs → policy + action + noise tokens → transformer → outputs ===
        ...
        # Learnable action tokens — one per action dimension (predict per-dim mean)
        self.action_tokens = nn.Parameter(torch.randn(act_dim, d_token) * 0.02)

        # Learnable noise tokens — one per action dimension (predict L matrix rows)
        self.noise_tokens = nn.Parameter(torch.randn(act_dim, d_token) * 0.02)

        # 2 transformer blocks: [4 policy + 6 action + 6 noise] = 16 tokens
        ...
        # Per-dim mean readout from action tokens (each token → 1 scalar)
        self.mean_out = nn.Linear(d_token, 1, bias=True)
    ...
    def _get_actor_tokens(self, x):
        """Process obs, return (action_tokens, noise_tokens)."""
        B = x.shape[0]
        ...
        # Expand action and noise tokens to batch
        action_tokens = _normalize(self.action_tokens.unsqueeze(0).expand(B, -1, -1))
        noise_tokens = _normalize(self.noise_tokens.unsqueeze(0).expand(B, -1, -1))

        # [policy(4), action(6), noise(6)] = 16 tokens total
        all_tokens = torch.cat([policy_tokens, action_tokens, noise_tokens], dim=1)

        all_tokens = self.actor_transformer1(all_tokens)
        all_tokens = self.actor_transformer2(all_tokens)

        # Split: skip policy tokens, return action and noise
        n_p = self._n_policy_tokens
        n_a = self._act_dim
        action_out = all_tokens[:, n_p:n_p + n_a]
        noise_out = all_tokens[:, n_p + n_a:]
        return action_out, noise_out

    def _get_mean(self, action_tokens):
        """Each action token predicts one scalar mean."""
        return self.mean_out(action_tokens).squeeze(-1)  # (B, act_dim)
    ...
    def get_action_and_value(self, x, action=None):
        action_tokens, noise_tokens = self._get_actor_tokens(x)
        base_mean = self._get_mean(action_tokens)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # noise metrics
        with torch.no_grad():
            at, nt = agent._get_actor_tokens(b_obs[:256])
