"""PPO Transformer with Cross-Attention Exploration Queries (Variant A)

nGPT-style hypersphere transformer backbone with learned exploration queries
that cross-attend to policy backbone tokens to produce state-dependent noise.

Noise mechanism:
- act_dim learnable exploration query tokens (d_token=32 each)
- Cross-attention: Q=explore_queries, KV=policy_backbone_tokens
- Each query learns which state features matter for its actuator's exploration
- Output: per-action-dim log_std, state-dependent

This replaces diagonal Gaussian (state-independent) noise with a learned,
state-dependent exploration strategy that leverages the transformer's
attention mechanism.
"""
import os
import random
...
class ExplorationHead(nn.Module):
    ...
    def __init__(self, d_token, act_dim, n_heads=4):
        ...
        # Output: per-action log_std (no clamp — let the network learn bounds)
    ...
    def forward(self, policy_tokens):
        ...
        # Per-action log_std — unclamped, network learns its own bounds
        log_std = self.noise_head(x).squeeze(-1)  # (B, act_dim)
        return log_std
...
class Agent(nn.Module):
    ...
    def _actor_forward(self, x):
        ...
        # State-dependent noise from exploration head (unclamped)
        log_std = self.explore_head(tokens)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # noise metrics logged from last minibatch — approximate
        with torch.no_grad():
