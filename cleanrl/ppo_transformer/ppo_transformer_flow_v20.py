"""PPO Transformer with Transformer-Native Noise Tokens (v6)

Noise tokens are embedded directly in the transformer backbone alongside
policy tokens. The transformer's self-attention is the ONLY mechanism for
cross-dim coordination — no separate exploration head.

Architecture:
  1. Obs → FC → 8 policy tokens (encode state)
  2. + 6 learnable noise tokens (one per action dim)
  3. All 14 tokens processed by 2 nGPT transformer blocks
  4. Policy tokens → mean (pool + FC)
  5. Noise tokens → L (lower-triangular scale) + mean correction

The noise tokens attend to policy tokens in the transformer, gathering
state-relevant information through the same attention mechanism that
processes the policy. No bolt-on exploration head needed.

Exact log_prob via change-of-variables: log N(eps;0,I) - sum(log(diag(L))).
v20: Adaptive entropy tuning (SAC-style) to prevent noise collapse.
v6 had ent_coef=0 with detached entropy — L matrix diagonal collapses from
~1.0 to ~0.24 with no counteracting gradient. Hopper/Walker fail because
they need sustained exploration past the collapse point.

This adds a learnable alpha parameter (like SAC's automatic entropy tuning):
- alpha = exp(log_alpha), adjusted to maintain target entropy
- If entropy < target: alpha increases (stronger entropy bonus)
- If entropy > target: alpha decreases (less entropy bonus)
- target_entropy calibrated per action dim: 0.5 * act_dim * (1 + log(2π)) - act_dim
  = ~2.5 for 6-dim (HC/Walker), ~1.3 for 3-dim (Hopper)
  This keeps avg diag(L) ≈ 0.37 instead of collapsing to 0.24.
"""
import os
import random
import time
...
class Args:
    ...
    ent_coef: float = 0.0
    """coefficient of the entropy (ignored when adaptive_alpha=True)"""
    adaptive_alpha: bool = True
    """use SAC-style adaptive entropy coefficient"""
    alpha_lr: float = 3e-4
    """learning rate for log_alpha"""
    target_entropy_scale: float = 0.3
    """target entropy = scale * max_entropy (where max_entropy = 0.5*d*(1+log(2pi)))"""
    vf_coef: float = 0.5
...
if __name__ == "__main__":
    ...
    # Adaptive entropy tuning (SAC-style)
    if args.adaptive_alpha:
        act_dim = np.prod(envs.single_action_space.shape)
        max_entropy = 0.5 * act_dim * (1 + np.log(2 * np.pi))  # entropy of N(0,I)
        target_entropy = args.target_entropy_scale * max_entropy
        log_alpha = torch.tensor(np.log(0.01), dtype=torch.float32, device=device, requires_grad=True)
        alpha_optimizer = optim.Adam([log_alpha], lr=args.alpha_lr)
        print(f"Adaptive entropy: target={target_entropy:.2f}, max={max_entropy:.2f}, init_alpha={np.exp(np.log(0.01)):.4f}")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.adaptive_alpha:
                    alpha = log_alpha.exp().clamp(min=1e-4, max=1.0)
