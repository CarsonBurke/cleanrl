"""PPO Transformer with State-Dependent MVN (Correlated Gaussian)

nGPT-style hypersphere transformer backbone. Exploration head (cross-attn +
self-attn) outputs a Cholesky factor L(s) so the action distribution is
N(mean(s), L(s) @ L(s)^T) — state-dependent correlated noise.

Self-attention between exploration queries directly shapes the cross-actuator
correlation structure. Log-prob via the Cholesky parameterization is tractable.
"""
import os
import random
import time
...
class ExplorationHead(nn.Module):
    ...
    def __init__(self, d_token, act_dim, n_heads=4):
        ...
        # Output: each token produces one row of the Cholesky factor L
        # Row i has i+1 non-zero entries (lower triangular)
        # We output act_dim values per token and mask to lower-triangular
        self.chol_head = nn.Linear(d_token, act_dim, bias=True)
        nn.init.zeros_(self.chol_head.weight)
        # Initialize bias so diagonal ≈ exp(LOG_STD_INIT), off-diag ≈ 0
        nn.init.zeros_(self.chol_head.bias)

        self._act_dim = act_dim
        # Pre-compute lower triangular mask and diagonal indices
        self.register_buffer('tril_mask', torch.tril(torch.ones(act_dim, act_dim)).bool())
        self.register_buffer('diag_idx', torch.arange(act_dim))
    ...
    def forward(self, policy_tokens):
        ...
        # Build Cholesky factor L: each token i → row i of L
        raw_L = self.chol_head(x)  # (B, act_dim, act_dim)
        # Mask to lower-triangular
        L = raw_L.masked_fill(~self.tril_mask, 0.0)
        # Positive diagonal via softplus offset by LOG_STD_INIT
        diag = F.softplus(L[:, self.diag_idx, self.diag_idx] + LOG_STD_INIT) + SDE_EPS
        L = L.clone()
        L[:, self.diag_idx, self.diag_idx] = diag

        return L  # (B, act_dim, act_dim)
...
class Agent(nn.Module):
    ...
    def _actor_forward(self, x):
        """Returns (action_mean, L) — mean and Cholesky factor."""
        B = x.shape[0]
        ...
        # Mean from pooled tokens
        pooled = tokens.mean(dim=1)
        h = F.silu(self.actor_agg_norm(self.actor_agg(pooled)))
        action_mean = self.actor_out(h)

        # Cholesky factor from exploration head
        L = self.explore_head(tokens)  # (B, act_dim, act_dim)

        return action_mean, L
    ...
    def get_action_and_value(self, x, action=None):
        action_mean, L = self._actor_forward(x)

        probs = MultivariateNormal(action_mean, scale_tril=L)
        if action is None:
            action = probs.sample()
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # noise metrics — Cholesky diagonal gives per-dim std
        with torch.no_grad():
            _, L_sample = agent._actor_forward(b_obs[:256])
            diag_std = L_sample.diagonal(dim1=-2, dim2=-1)  # (256, act_dim)
            writer.add_scalar("explore/log_std_mean", diag_std.log().mean().item(), global_step)
            writer.add_scalar("explore/offdiag_norm", (L_sample.tril(-1) ** 2).sum(-1).sum(-1).sqrt().mean().item(), global_step)
