"""FPO epoch-refreshed Monte Carlo v6; PPO backbone and v1 actor unchanged.

Use velocity-MSE (best measured v1 setting), clip=.05, eight MC pairs. Freeze an
old-policy snapshot for the WHOLE rollout update. Redraw time/noise pairs each
epoch, recomputing OLD and NEW losses on identical pairs. The old policy never
moves between epochs: refreshing old losses against current weights would erase
the trust region. This deviates deliberately from paper Algorithm1's fixed pairs
across epochs to test the measured fitting of a finite MC training draw.
"""
...
class Args:
    ...
    flow_steps: int = 10
    mc_samples: int = 8
    mse: Literal["epsilon", "velocity"] = "velocity"
...
class RolloutCFM:
    """Permanent device buffers; redraw pairs before each optimization epoch.

    Old losses always use the same rollout-frozen policy snapshot. The current
    epoch indexes those pairs and old losses together for every minibatch.
    """
...
def main():
    ...
    with ExitStack() as resources:
        ...
        writer.add_text("policy", "FPO: epoch-refreshed pairs against a rollout-frozen old policy; signed clipped GAE; "
        ...
        provenance = {
            ...
            "time_convention": "noise_at_0_data_at_1", "mc_refresh": "per_epoch_frozen_old_policy", "args": vars(args),
        }
        ...
        old_agent = copy.deepcopy(agent).requires_grad_(False)
        ...
        def rollout_statistics(observations, native, times, noise):
            return (old_agent.get_value(observations).flatten(),
                    cfm_loss(old_agent, observations, native, times, noise, args.mse))
