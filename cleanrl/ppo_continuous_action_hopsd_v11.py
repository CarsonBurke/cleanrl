# HOPSD v11 — hindsight-anchored critic distillation, done right (v8.1 post-mortem fix).
# =====================================================================================
...
class Args:
    ...
    critic_distill_coef: float = 0.3 # aux CE of student critic toward V_T's distribution (MC-valid states)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v11: hindsight-anchored critic distillation on MC-valid states.
                with torch.no_grad():
                    t_cd_probs = torch.softmax(
                        teacher.critic_logits(b_teacher_critic_in[mb_inds]), dim=-1
                    )
                cd_mask = b_mc_valid[mb_inds].to(device=device, dtype=torch.float32)
                cd_ce = -(t_cd_probs * value_log_probs[:, 0]).sum(-1)
                cd_loss = (cd_ce * cd_mask).sum() / cd_mask.sum().clamp_min(1.0)
