...
#
# OPSD v24: learned full-batch advantage-weight distribution.
#
# v24 note: replace v20's fixed top-k V-MPO weights with a learned categorical
# distribution over every sample in the minibatch. The weighter is not trained to
# regress advantages. It is meta-trained: choose train-sample weights, simulate a
# differentiable one-step actor_beta update, then optimize the weights by held-out
# clipped policy-improvement on meta samples. The real actor update still uses
# detached nonnegative mean-1 weights, preserving PPO-free weighted distillation.
#
...
class Args:
    ...
    weighter_inner_lr: float = 3e-4
    """differentiable one-step actor_beta SGD step used by the weighter meta-objective"""
    weighter_meta_frac: float = 0.25
    """fraction of each minibatch held out to train the weighter by actor-improvement meta-loss"""
    weighter_entropy_coef: float = 0.01
    """entropy bonus for the learned sample distribution during meta-training"""
...
if __name__ == "__main__":
    ...
    weighter = AdvantageWeighter(feature_dim=11, hidden_size=args.weighter_hidden_size).to(device)
    with torch.no_grad():
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if mb_n >= 2:
                    meta_n = max(1, min(mb_n - 1, int(args.weighter_meta_frac * mb_n)))
                    perm = torch.randperm(mb_n, device=device)
                    meta_idx = perm[:meta_n]
                    train_idx = perm[meta_n:]

                    actor_params = dict(agent.actor_beta.named_parameters())
                    train_scores = weighter(mb_features[train_idx], args)
                    train_probs = torch.softmax(train_scores, dim=0)
                    train_weights = train_probs * train_idx.numel()
                    train_logprob = agent.get_action_logprob_with_actor_params(
                        mb_obs[train_idx],
                        mb_actions[train_idx],
                        actor_params,
                    )
                    inner_loss = -(train_weights * train_logprob).mean()
                    actor_param_items = list(actor_params.items())
                    inner_grads = torch.autograd.grad(
                        inner_loss,
                        [param for _, param in actor_param_items],
                        create_graph=True,
                    )
                    fast_actor_params = {
                        name: param - args.weighter_inner_lr * grad
                        for (name, param), grad in zip(actor_param_items, inner_grads)
                    }
                    meta_logprob = agent.get_action_logprob_with_actor_params(
                        mb_obs[meta_idx],
                        mb_actions[meta_idx],
                        fast_actor_params,
                    )
                    meta_logratio = meta_logprob - mb_old_logprobs[meta_idx]
                    meta_ratio = meta_logratio.exp()
                    meta_advantages = mb_advantages[meta_idx].detach()
                    meta_pg1 = meta_ratio * meta_advantages
                    meta_pg2 = torch.clamp(
                        meta_ratio,
                        1.0 - args.clip_coef,
                        1.0 + args.clip_coef_high,
                    ) * meta_advantages
                    meta_policy_loss = -torch.min(meta_pg1, meta_pg2).mean()
                    train_entropy = -(train_probs * (train_probs + 1e-8).log()).sum()
                    train_entropy = train_entropy / max(np.log(train_idx.numel()), 1e-8)
                    weighter_loss = meta_policy_loss - args.weighter_entropy_coef * train_entropy
                    ...
                    meta_policy_loss = torch.zeros((), device=device)
                    weighter_loss = meta_policy_loss
                ...
                # weighter is optimized above by held-out actor improvement; the
                # real actor update treats the learned weights as fixed targets.
                with torch.no_grad():
                ...
                weighter_meta_losses.append(meta_policy_loss.item())
        ...
        writer.add_scalar("losses/advantage_weighter_meta", np.mean(weighter_meta_losses), global_step)
