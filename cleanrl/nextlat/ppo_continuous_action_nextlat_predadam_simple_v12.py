# NEXTLAT PREDADAM SIMPLE v12: minimal optimizer-level predictive coding control.
# =====================================================================================
# This is the deliberately SIMPLE optimizer arm against the deeper PredAdam v9. It is not a
# single-variable ablation against clip025: policy-head weights are also frozen in the auxiliary
# decoder and the dynamics predictor has a private Adam. NextLat targets and PPO are unchanged.
# The shared-trunk auxiliary gradient is handled in raw gradient space: within each logical
# ThinkTrunk block, remove its component
# along a strictly lagged EMA of the primary actor+critic gradient (Bloop-style), cap its raw
# gradient norm to 5% of the current primary block norm (plus a strict global 5% raw-gradient
# cap), and mix it into the SAME task Adam step. There is no second trunk Adam, rollback/proposal
# transaction, meta-prediction gate, or TD target. Hypothesis: a small task-orthogonal
# representation pressure is the minimum useful optimizer-level mechanism behind clip025.
# Watch simplepc/* diagnostics; the raw-gradient cap is not an Adam parameter-step trust bound.
# =====================================================================================
...
class Args:
    ...
    nextlat_trunk_grad_clip: float = 0.25  # clip025 incumbent; applied before SimplePC admission
    nextlat_predictor_grad_clip: float = 0.25  # private predictor budget, independent of trunk size

    # v12 simple optimizer control. Projection and mixing happen in RAW gradient space;
    # both logical-block and global auxiliary norms are capped relative to the current
    # primary trunk gradient before the single shared task-Adam step. Adam can amplify a
    # newly active coordinate, so this is deliberately named as a RAW-gradient ratio rather
    # than an update-space trust ratio.
    simplepc_raw_grad_ratio: float = 0.05
    simplepc_ema_decay: float = 0.99  # Bloop paper accumulation rho=0.01 => decay=1-rho
...
class Agent(nn.Module):
    ...
    def nextlat_parameters(self):
        # v9: only the representation and predictor receive the auxiliary gradient.
        # Policy-head weights are frozen in _actor_dist_frozen_head.
        trunk = self.trunk if self.share_backbone else self.actor_trunk
    ...
    def nextlat_trunk_parameter_blocks(self):
        """Parameters grouped by the ThinkTrunk modules that define one logical block."""
        trunk = self.trunk if self.share_backbone else self.actor_trunk
        ...
        if {id(p) for block in blocks for p in block} != {
            id(p) for p in trunk.parameters()
        }:
            raise RuntimeError("logical trunk blocks do not partition trunk parameters")
        return blocks
...
@torch.no_grad()
def make_gradient_block_layout(parameter_blocks):
    """Precompute segment metadata for logical-block projection and norm caps."""
    if not parameter_blocks or any(not block for block in parameter_blocks):
        raise ValueError("at least one non-empty parameter block is required")
    flat_parameters = [parameter for block in parameter_blocks for parameter in block]
    device = flat_parameters[0].device
    lengths_tuple = tuple(parameter.numel() for parameter in flat_parameters)
    ...
    return lengths_tuple, block_lengths_tuple, block_lengths, groups
...
@torch.no_grad()
def _optimizer_safe_tensors(tensors, reference):
    """Return whether tensors are finite and safe for a squared-moment optimizer.

    Adam squares gradients. A gradient such as ``1e30`` is finite in float32 but is not
    a valid optimizer input because its second moment overflows. Treating this condition
    exactly like NaN/Inf keeps the fail-closed auxiliary path ahead of optimizer state.
    """
    safe = reference.new_ones((), dtype=torch.bool)
    for tensor in tensors:
        tensor_safe = torch.isfinite(tensor).all() & torch.isfinite(tensor.square()).all()
        if tensor_safe.device != safe.device:
            tensor_safe = tensor_safe.to(safe.device)
        safe = safe & tensor_safe
    return safe


@torch.no_grad()
def _repair_optimizer_state_(optimizer, reference):
    """Repair nonfinite floating optimizer state and report whether it was pristine.

    Auxiliary failure containment must also recover moments poisoned by an earlier
    transaction. Invalid entries, including a corrupted scalar clock, are reset to zero;
    valid state is bit-for-bit untouched. The caller decides whether a proposal made
    after a repair is trustworthy enough to accept.
    """
    state_was_finite = reference.new_ones((), dtype=torch.bool)
    for state in optimizer.state.values():
        for value in state.values():
            ...
            value_was_finite = finite.all()
            if value_was_finite.device != state_was_finite.device:
                value_was_finite = value_was_finite.to(state_was_finite.device)
            state_was_finite = state_was_finite & value_was_finite
            value.copy_(torch.where(finite, value, torch.zeros_like(value)))
    return state_was_finite


@dataclass
class SimplePCGradientEMAState:
    """Primary-gradient EMA state used only as a lagged projection direction."""
...
@torch.no_grad()
def update_primary_gradient_ema_(ema_gradients, primary_gradients, decay):
    """Update an uncorrected primary-gradient EMA after the optimizer transaction."""
    if len(ema_gradients) != len(primary_gradients):
        raise ValueError("EMA and primary gradient lists must have equal length")
    if not 0.0 <= decay < 1.0:
        raise ValueError("EMA decay must be in [0, 1)")
    for ema, primary in zip(ema_gradients, primary_gradients):
        if ema.shape != primary.shape or ema.device != primary.device:
            raise ValueError("EMA and primary gradients must share shape and device")
    torch._foreach_mul_(ema_gradients, decay)
    torch._foreach_add_(ema_gradients, primary_gradients, alpha=1.0 - decay)
...
def project_and_cap_auxiliary_gradients(
    ...
    primary_ema,
    auxiliary_gradients,
    max_raw_gradient_ratio,
    *,
    layout=None,
):
    """Bloop-style block projection followed by local and global primary-relative caps.

    The auxiliary component parallel or antiparallel to the primary-gradient EMA is
    removed in every logical block. The result is then capped against the CURRENT primary
    gradient. These are raw-gradient caps before Adam, not parameter-update bounds.
    """
    if not primary_gradients:
        raise ValueError("at least one trunk gradient is required")
    if not (
        len(primary_gradients) == len(primary_ema) == len(auxiliary_gradients)
    ):
        raise ValueError("primary, EMA, and auxiliary lists must align one-to-one")
    if max_raw_gradient_ratio < 0.0:
        raise ValueError("max_raw_gradient_ratio must be non-negative")
    for primary, ema, auxiliary in zip(
        primary_gradients, primary_ema, auxiliary_gradients
    ):
        if not (
            primary.shape == ema.shape == auxiliary.shape
            and primary.device == ema.device == auxiliary.device
        ):
            raise ValueError("aligned gradients must share shape and device")

    if layout is None:
        singleton_blocks = [[gradient] for gradient in primary_gradients]
        layout = make_gradient_block_layout(singleton_blocks)
    lengths_tuple, block_lengths_tuple, block_lengths, groups = layout
    if lengths_tuple != tuple(gradient.numel() for gradient in primary_gradients):
        raise ValueError("gradients do not match the supplied logical-block layout")
    if sum(block_lengths_tuple) != sum(lengths_tuple):
        raise ValueError("invalid logical-block layout")

    flat_primary = torch.cat([gradient.reshape(-1) for gradient in primary_gradients])
    flat_ema = torch.cat([gradient.reshape(-1) for gradient in primary_ema])
    flat_auxiliary = torch.cat(
        [gradient.reshape(-1) for gradient in auxiliary_gradients]
    )

    # Check the raw auxiliary once. This catches finite float32 values whose square would
    # overflow before any such value can be mixed into the task optimizer. Task gradients
    # are already protected by the actor/critic ``error_if_nonfinite`` clip boundaries.
    numeric_valid = _optimizer_safe_tensors([flat_auxiliary], flat_auxiliary)

    primary_sq_by_block = _segment_sum(flat_primary.square(), block_lengths)
    ema_sq_by_block = _segment_sum(flat_ema.square(), block_lengths)
    aux_sq_by_block = _segment_sum(flat_auxiliary.square(), block_lengths)
    ema_aux_dot = _segment_sum(flat_ema * flat_auxiliary, block_lengths)

    tiny = torch.finfo(flat_ema.dtype).tiny
    ema_valid = ema_sq_by_block > tiny
    coefficient = torch.where(
        ema_valid,
        ema_aux_dot / ema_sq_by_block.clamp_min(tiny),
        torch.zeros_like(ema_aux_dot),
    )
    flat_projected = flat_auxiliary - coefficient[groups] * flat_ema

    # A second residual correction makes the device-side projection robust to float32
    # accumulation error without synchronizing for scalar decisions.
    residual = _segment_sum(flat_ema * flat_projected, block_lengths)
    correction = torch.where(
        ema_valid,
        residual / ema_sq_by_block.clamp_min(tiny),
        torch.zeros_like(residual),
    )
    flat_projected = flat_projected - correction[groups] * flat_ema

    projected_sq_by_block = _segment_sum(flat_projected.square(), block_lengths)
    primary_norm_by_block = primary_sq_by_block.sqrt()
    projected_norm_by_block = projected_sq_by_block.sqrt()
    local_scale = torch.clamp(
        max_raw_gradient_ratio
        * primary_norm_by_block
        / projected_norm_by_block.clamp_min(tiny),
        max=1.0,
    )
    local_scale = local_scale * (primary_sq_by_block > 0.0)
    flat_local = flat_projected * local_scale[groups]

    primary_norm = primary_sq_by_block.sum().sqrt()
    local_norm = flat_local.square().sum().sqrt()
    global_scale = torch.clamp(
        max_raw_gradient_ratio * primary_norm / local_norm.clamp_min(tiny),
        max=1.0,
    )
    global_scale = global_scale * (primary_norm > 0.0)
    candidate_admitted = flat_local * global_scale
    numeric_valid = (
        numeric_valid
        & torch.isfinite(flat_projected).all()
        & torch.isfinite(candidate_admitted).all()
        & torch.isfinite(local_scale).all()
        & torch.isfinite(global_scale).all()
    )
    # Scalar ``where`` is intentionally used instead of multiplying by a zero veto:
    # IEEE arithmetic would otherwise preserve NaNs through ``NaN * 0``.
    flat_admitted = torch.where(
        numeric_valid, candidate_admitted, torch.zeros_like(candidate_admitted)
    )
    admitted = [
        flat.view_as(reference)
        for flat, reference in zip(
            flat_admitted.split(lengths_tuple), primary_gradients
        )
    ]

    admitted_sq_by_block = _segment_sum(flat_admitted.square(), block_lengths)
    admitted_norm_by_block = admitted_sq_by_block.sqrt()
    ema_norm = ema_sq_by_block.sum().sqrt()
    auxiliary_norm = aux_sq_by_block.sum().sqrt()
    raw_dot = (flat_ema * flat_auxiliary).sum()
    zero = flat_primary.new_zeros(())

    def finite_or_zero(value):
        return torch.where(torch.isfinite(value).all(), value, torch.zeros_like(value))

    return admitted, {
        "primary_norm": primary_norm,
        "ema_norm": finite_or_zero(ema_norm),
        "auxiliary_norm": finite_or_zero(auxiliary_norm),
        "projected_norm": finite_or_zero(flat_projected.square().sum().sqrt()),
        "admitted_norm": flat_admitted.square().sum().sqrt(),
        "raw_ema_cosine": finite_or_zero(
            raw_dot / (ema_norm * auxiliary_norm).clamp_min(tiny)
        ),
        "post_ema_dot_absmax": _segment_sum(
            flat_ema * flat_admitted, block_lengths
        ).abs().max(),
        "accepted_fraction": finite_or_zero(
            flat_admitted.square().sum().sqrt()
            / auxiliary_norm.clamp_min(tiny)
        ),
        "global_cap_scale": finite_or_zero(global_scale),
        "max_raw_grad_block_ratio": finite_or_zero(
            (admitted_norm_by_block / primary_norm_by_block.clamp_min(tiny)).max()
        ),
        "numeric_valid": numeric_valid.to(dtype=zero.dtype),
    }


@torch.no_grad()
def apply_private_optimizer_step(parameters, optimizer, gradients):
    """Atomically apply a finite private auxiliary step and report its norm.

    An invalid gradient is replaced by an all-zero transaction before the optimizer is
    called. The optimizer clock may advance and finite momentum may decay, but parameters
    are restored exactly. Pre-existing invalid moments are repaired and also veto the
    proposal made in that transaction.
    """
    ...
    if not bool(parameters_are_finite.item()):
        raise FloatingPointError("private parameter was nonfinite before transaction")

    optimizer.zero_grad(set_to_none=True)
    before = [parameter.detach().clone() for parameter in parameters]
    flat_gradient = torch.cat([gradient.reshape(-1) for gradient in gradients])
    gradient_valid = _optimizer_safe_tensors([flat_gradient], parameters[0])
    safe_gradient = torch.where(
        gradient_valid, flat_gradient, torch.zeros_like(flat_gradient)
    )
    state_was_finite = _repair_optimizer_state_(optimizer, parameters[0])
    for parameter, gradient in zip(parameters, safe_gradient.split(lengths)):
    ...
    proposal_valid = _optimizer_safe_tensors([flat_update], parameters[0])
    resulting_state_finite = _repair_optimizer_state_(optimizer, parameters[0])
    transaction_valid = (
        gradient_valid & state_was_finite & proposal_valid & resulting_state_finite
    )
    update_sq = parameters[0].new_zeros(())
    for parameter, previous in zip(parameters, before):
        accepted = torch.where(transaction_valid, parameter.detach(), previous)
        parameter.copy_(accepted)
        update_sq = update_sq + (accepted - previous).square().sum()
    optimizer.zero_grad(set_to_none=True)
    return update_sq.sqrt()


@torch.no_grad()
def gradient_dot_and_cosine(left_gradients, right_gradients):
    """Return global first-order dot and cosine for two aligned gradient lists."""
    if not left_gradients or len(left_gradients) != len(right_gradients):
        raise ValueError("aligned non-empty gradient lists are required")
    dot = left_gradients[0].new_zeros(())
    left_sq = left_gradients[0].new_zeros(())
    right_sq = left_gradients[0].new_zeros(())
    for left, right in zip(left_gradients, right_gradients):
        ...
        left_sq = left_sq + left.square().sum()
        right_sq = right_sq + right.square().sum()
    cosine = dot / (left_sq.sqrt() * right_sq.sqrt()).clamp_min(
...
@torch.no_grad()
def apply_simplepc_optimizer_transaction(
    *,
    trunk_parameters,
    predictor_parameters,
    task_optimizer,
    predictor_optimizer,
    primary_trunk_gradients,
    actor_trunk_gradients,
    critic_trunk_gradients,
    auxiliary_trunk_gradients,
    predictor_gradients,
    primary_gradient_ema,
    ema_decay,
    max_raw_gradient_ratio,
    layout,
):
    """Apply one causal SimplePC/Bloop optimizer transaction.

    Bloop's stochastic guarantee requires the projection direction to be independent of
    the current minibatch. Accordingly, auxiliary admission uses only the EMA left by
    PREVIOUS transactions. The first transaction admits no shared-trunk auxiliary, but
    still trains the private predictor. Only after task Adam consumes the mixed direction
    do we incorporate the current primary gradient into the EMA.

    The returned shared-trunk step norm measures the actual mixed task-Adam parameter
    update. It cannot attribute a marginal update to the auxiliary gradient because that
    gradient shares Adam's moments and coordinate-wise preconditioner with the task.
    """
    if len(trunk_parameters) != len(primary_trunk_gradients):
        raise ValueError("one primary gradient is required per trunk parameter")
    if len(trunk_parameters) != len(auxiliary_trunk_gradients):
        raise ValueError("one auxiliary gradient is required per trunk parameter")
    if len(predictor_parameters) != len(predictor_gradients):
        raise ValueError("one auxiliary gradient is required per predictor parameter")
    if not isinstance(primary_gradient_ema, SimplePCGradientEMAState):
        raise TypeError("primary_gradient_ema must be SimplePCGradientEMAState")

    ema_was_initialized = primary_gradient_ema.initialized
    # A zero admission ratio makes the cold-start transaction task-only without allowing
    # the current minibatch to leak into its own projection direction.
    admitted_trunk_gradients, stats = project_and_cap_auxiliary_gradients(
        primary_trunk_gradients,
        primary_gradient_ema.gradients,
        auxiliary_trunk_gradients,
        max_raw_gradient_ratio if ema_was_initialized else 0.0,
        layout=layout,
    )
    stats["ema_was_initialized"] = primary_trunk_gradients[0].new_tensor(
        float(ema_was_initialized)
    )
    (
        stats["actor_admitted_dot"],
        stats["actor_admitted_cosine"],
    ) = gradient_dot_and_cosine(actor_trunk_gradients, admitted_trunk_gradients)
    (
        stats["critic_admitted_dot"],
        stats["critic_admitted_cosine"],
    ) = gradient_dot_and_cosine(critic_trunk_gradients, admitted_trunk_gradients)

    for parameter, primary_gradient, admitted_gradient in zip(
        trunk_parameters, primary_trunk_gradients, admitted_trunk_gradients
    ):
        if (
            parameter.shape != primary_gradient.shape
            or parameter.device != primary_gradient.device
        ):
            raise ValueError("trunk parameters and gradients must share shape and device")
        # Assign the exact mixed direction instead of depending on whichever gradient was
        # left resident by the preceding actor/critic backward passes.
        parameter.grad = primary_gradient + admitted_gradient
    ...
    trunk_before = [parameter.detach().clone() for parameter in trunk_parameters]
    task_optimizer.step()
    shared_trunk_step_sq = trunk_parameters[0].new_zeros(())
    for parameter, previous in zip(trunk_parameters, trunk_before):
        shared_trunk_step_sq = (
            shared_trunk_step_sq
            + (parameter.detach() - previous).square().sum()
        )

    # Paper Algorithm 1 order: direction from lagged EMA -> optimizer step -> EMA update.
    update_primary_gradient_ema_(
        primary_gradient_ema.gradients,
        primary_trunk_gradients,
        ema_decay,
    )
    primary_gradient_ema.initialized = True
    return stats, predictor_step_norm, shared_trunk_step_sq.sqrt()
...
if __name__ == "__main__":
    ...
    assert args.nextlat_depth >= 1
    assert 0.0 <= args.simplepc_raw_grad_ratio <= 1.0
    assert 0.0 <= args.simplepc_ema_decay < 1.0
    assert args.separate_grad_clip, "SimplePC requires decoupled task/auxiliary gradients"
    ...
    # Exactly two Adam streams. The task Adam owns the shared trunk and consumes the
    # primary gradient plus a protected raw auxiliary gradient. Predictor-only parameters
    # use private Adam state and are absent from task Adam.
    ...
    simplepc_layout = make_gradient_block_layout(nextlat_trunk_blocks)
    primary_gradient_ema = SimplePCGradientEMAState(
        [torch.zeros_like(parameter) for parameter in nextlat_trunk_params]
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.nextlat:
                    ...
                    with torch.no_grad():
                        ...
                        ).reshape(args.nextlat_depth, len(mb_inds), args.hidden)
                    h_hat = mb_actor_feat
                    ...
                    for i in range(1, args.nextlat_depth + 1):
                        ...
                        # Frozen decoder weights: KL supplies the policy's local Fisher
                        # geometry to imagined latents but never rewrites the action head.
                        s_dist, _, _ = agent._actor_dist_frozen_head(h_hat)
                ...
                # Three backwards, two Adam streams. Actor and critic are independently
                # clipped then summed into the primary task gradient. NextLat trunk and
                # predictor gradients have separate clip budgets; only the predictor uses
                # private Adam. The admitted trunk gradient joins the SAME task Adam step.
                ...
                if args.nextlat:
                    # Both components are already resident after their independent clips.
                    # Keep them only for first-order alignment diagnostics; admission is
                    # intentionally based on their aggregate primary direction below.
                    actor_trunk_grads = [
                        parameter.grad.detach().clone()
                        if parameter.grad is not None
                ...
                if args.nextlat:
                    ...
                    (
                        simplepc_stats,
                        simplepc_predictor_step_norm,
                        simplepc_shared_trunk_step_norm,
                    ) = apply_simplepc_optimizer_transaction(
                        trunk_parameters=nextlat_trunk_params,
                        predictor_parameters=nextlat_predictor_params,
                        task_optimizer=task_optimizer,
                        predictor_optimizer=predictor_optimizer,
                        primary_trunk_gradients=primary_trunk_grads,
                        actor_trunk_gradients=actor_trunk_grads,
                        critic_trunk_gradients=critic_trunk_grads,
                        auxiliary_trunk_gradients=nextlat_trunk_grads,
                        predictor_gradients=nextlat_predictor_grads,
                        primary_gradient_ema=primary_gradient_ema,
                        ema_decay=args.simplepc_ema_decay,
                        max_raw_gradient_ratio=args.simplepc_raw_grad_ratio,
                        layout=simplepc_layout,
                    )
                else:
                    task_optimizer.step()
        ...
        if args.nextlat:
            ...
            # Preserve the incumbent tag for matched-run dashboards; it now denotes the
            # representation/trunk budget, the only auxiliary gradient that reaches PPO.
            writer.add_scalar(
            ...
            writer.add_scalar(
                "simplepc/primary_grad_norm",
                float(simplepc_stats["primary_norm"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/primary_ema_norm",
                float(simplepc_stats["ema_norm"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/raw_auxiliary_grad_norm",
                float(simplepc_stats["auxiliary_norm"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/projected_auxiliary_grad_norm",
                float(simplepc_stats["projected_norm"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/admitted_auxiliary_grad_norm",
                float(simplepc_stats["admitted_norm"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/numeric_valid",
                float(simplepc_stats["numeric_valid"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/predictor_step_norm",
                float(simplepc_predictor_step_norm),
                global_step,
            )
            # This is the actual mixed task-Adam step, not an attribution of the
            # auxiliary gradient's marginal effect under Adam's shared moments.
            writer.add_scalar(
                "simplepc/shared_trunk_step_norm",
                float(simplepc_shared_trunk_step_norm),
                global_step,
            )
            writer.add_scalar(
                "simplepc/raw_ema_cosine",
                float(simplepc_stats["raw_ema_cosine"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/actor_admitted_dot",
                float(simplepc_stats["actor_admitted_dot"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/actor_admitted_cosine",
                float(simplepc_stats["actor_admitted_cosine"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/critic_admitted_dot",
                float(simplepc_stats["critic_admitted_dot"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/critic_admitted_cosine",
                float(simplepc_stats["critic_admitted_cosine"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/post_ema_dot_absmax",
                float(simplepc_stats["post_ema_dot_absmax"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/accepted_fraction",
                float(simplepc_stats["accepted_fraction"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/global_cap_scale",
                float(simplepc_stats["global_cap_scale"]),
                global_step,
            )
            writer.add_scalar(
                "simplepc/max_raw_grad_block_ratio",
                float(simplepc_stats["max_raw_grad_block_ratio"]),
                global_step,
            )
        if auto_alpha:
