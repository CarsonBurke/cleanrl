# NEXTLAT FUNCTION-TRUST LIVE-HEAD CUDA v16: v15 semantics, GPU-resident PPO epochs.
# =====================================================================================
# PERFORMANCE DELTA vs v15 (algorithm unchanged): HL-Gauss MTP labels and masks stay on
# CUDA for all PPO epochs, removing about 3.7 GiB/iteration of pageable label H2D at the
# default shape. Minibatch telemetry stays device-side and is copied once per iteration;
# PPO norm checks use an asynchronous device assertion instead of synchronizing the host.
# One NumPy permutation is transferred per epoch rather than implicitly converting indices
# on every tensor slice. Static HL-Gauss projection joins the compiled reduce-overhead
# forward paths and its retained CUDA-graph output is cloned immediately.
# Float32 matmul precision is set to "high" before model construction/compilation, enabling
# TF32-class CUDA kernels where PyTorch supports them. TF32 and fused projection math can
# change last-bit numerics and therefore exact trajectories, but preserve dtype, objectives,
# masks, and optimizer math.
# =====================================================================================
...
def clip_grad_norm_async_fail_loud_(parameters, max_norm, norm_type=2.0):
    """Clip gradients and enqueue a nonfinite assertion without a host sync.

    ``error_if_nonfinite=True`` eventually calls ``.item()`` on the aggregate norm in
    PyTorch's eager implementation. At hundreds of PPO minibatches per iteration that
    serializes the CPU and CUDA streams. The device assertion is ordered after clipping
    and before the optimizer kernels on the same stream: finite gradients take the exact
    ordinary clipping path, while NaN/Inf still aborts before a usable optimizer step.
    """
    ...
    torch._assert_async(
        ...
        "The total gradient norm is non-finite; refusing the PPO optimizer step",
    )
    return total_norm
...
def synchronize_scalar_telemetry(statistics):
    """Materialize device scalar telemetry with one device-to-host transfer.

    Values must already be scalar tensors on one device. Keeping this boundary explicit
    prevents an innocent ``float(tensor)`` or ``writer.add_scalar(tensor)`` from adding a
    separate CUDA synchronization for every metric.
    """
    if not statistics:
        return {}
    ...
    # ``stack`` performs the necessary common-dtype promotion while writing the packed
    # buffer; avoid materializing one converted tensor per metric beforehand.
    packed = torch.stack([value.detach().reshape(()) for value in scalars])
    # This is the PPO iteration's sole telemetry synchronization. ``tolist`` operates
    # after the one packed D2H copy and therefore causes no additional CUDA transfers.
    host_values = packed.cpu().tolist()
    return dict(zip(names, host_values))


@torch.no_grad()
def retain_projected_targets(projected_targets, *, compiled):
    """Clone a long-lived projection when its producer may reuse CUDA-graph storage."""
    if not torch.is_tensor(projected_targets):
        raise TypeError("projected targets must be a tensor")
    return (
        projected_targets.detach().clone()
        if compiled
        else projected_targets.detach()
    )
...
def merge_live_head_auxiliary_gradients(parameters, auxiliary_gradients, *, valid):
    ...
    PPO gradients are authoritative and were checked by
    ``clip_grad_norm_async_fail_loud_`` before this call. Invalid auxiliary input adds exact
    zeros, producing task-only Adam parameters and moments without a CUDA host sync.
    """
    parameters = list(parameters)
...
if __name__ == "__main__":
    ...
    # Must precede model construction and torch.compile. On recent NVIDIA GPUs, "high"
    # permits TF32-class internal matmul kernels while retaining float32 inputs/outputs.
    # The throughput win costs last-bit parity with v15, not algorithmic precision.
    ...
    def project_value_targets_fn(targets_):
        # Pure, static-shape HL-Gauss projection: safe to compile independently of the
        # eager optimizer transaction and its deliberate parameter mutations.
    ...
    # DreamerV3 percentile statistics remain on CUDA too. Float64 preserves the old
    # Python-float EMA arithmetic closely without synchronizing quantiles to the host.
    ema_ret_lo = torch.zeros((), dtype=torch.float64, device=device)
    ema_ret_hi = torch.ones((), dtype=torch.float64, device=device)
    ret_perc_scale = torch.ones((), dtype=torch.float64, device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if args.ret_percnorm and args.ret_perc_scope in ("ema", "batch"):
                ...
                lo, hi = torch.quantile(flat_ret, qs).to(dtype=torch.float64).unbind()
                if args.ret_perc_scope == "ema":
                    if not ema_perc_inited:
                        ema_ret_lo.copy_(lo)
                        ema_ret_hi.copy_(hi)
                        ema_perc_inited = True
                    else:
                        r = args.ret_perc_rate
                        ema_ret_lo.add_(r * (lo - ema_ret_lo))
                        ema_ret_hi.add_(r * (hi - ema_ret_hi))
                    spread = ema_ret_hi - ema_ret_lo
                else:  # "batch": fresh whole-rollout percentile spread, no EMA
                    spread = hi - lo
                ret_perc_scale.copy_(spread.clamp_min(args.ret_perc_floor))
            ...
            # The fixed (T,B,MTP,bins) target is large (~383 MiB by default), but CUDA
            # has ample capacity. Retaining it here avoids retransferring the entire
            # rollout once per PPO epoch. A compiled CUDA-graph output is ephemeral, so
            # clone it immediately before later compiled calls can reuse graph storage.
            if args.compile:
                torch.compiler.cudagraph_mark_step_begin()
            projected_target_probs = project_value_targets_fn(return_mtp.detach())
            target_probs = retain_projected_targets(
                projected_target_probs,
            ...
            if args.nextlat:
                ...
                latent_batch_std = latents.reshape(-1, args.hidden).std(dim=0).mean()
        ...
        if args.nextlat:
            nextlat_action_offsets = (
                torch.arange(args.nextlat_depth, device=device, dtype=torch.int64)[:, None]
                * args.num_envs
            )
        ...
        # v15 converted each float32 mean to Python then averaged in float64. Retain that
        # diagnostic precision while accumulating asynchronously on-device.
        clipfrac_sum = torch.zeros((), dtype=torch.float64, device=device)
        clipfrac_count = 0
        for epoch in range(args.update_epochs):
            np.random.shuffle(b_inds)
            # Preserve v15's NumPy permutation exactly, but pay for one compact index
            # transfer per epoch rather than repeated implicit NumPy->CUDA conversion on
            # every indexed buffer and NextLat horizon.
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.ret_percnorm and args.ret_perc_scope == "minibatch":
                    ...
                    ret_perc_scale.copy_(mb_perc_scale.detach())
                ...
                value_mask = b_target_mask[mb_inds]
        ...
        edge_mass = (edge_per_h * edge_mask_f).sum() / edge_mask_f.sum().clamp_min(1)

        # Keep all CUDA telemetry live until one packed D2H transfer. The target-KL
        # epoch decision above is the only intentional per-epoch host control-flow check.
        device_telemetry = {
            "losses/value_loss": v_loss,
            "losses/policy_loss": pg_loss,
            "losses/entropy": entropy_loss,
            "charts/ret_perc_scale": ret_perc_scale,
        ...
        if args.nextlat:
            device_telemetry.update(
                {
                    "losses/nextlat_pred_loss": nextlat_pred_loss,
                    "losses/nextlat_kl_loss": nextlat_kl_loss,
                    "losses/nextlat_trunk_grad_norm": nextlat_trunk_gn,
                    # Preserve the incumbent dashboard tag for the predictive trunk.
                    "losses/nextlat_grad_norm": nextlat_trunk_gn,
                    "losses/nextlat_predictor_grad_norm": nextlat_predictor_gn,
                    "debug/latent_batch_std": latent_batch_std,
                    "function_trust/predictor_step_norm": function_trust_predictor_step_norm,
                }
            )
            livehead_tags = {
            ...
            for tag, statistic in livehead_tags.items():
                device_telemetry[f"livehead/{tag}"] = statistic
            function_trust_tags = {
            ...
            for tag, stat in function_trust_tags.items():
                device_telemetry[f"function_trust/{tag}"] = function_trust_stats[stat]
        if auto_alpha:
        ...
        # Release the retained projection clone before producing the next iteration's
        # clone; the compiled graph owns its separate static output buffer.
        del target_probs, b_target_probs, target_probs_mb
