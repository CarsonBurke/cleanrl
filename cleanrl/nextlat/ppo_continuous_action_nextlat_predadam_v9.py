# NEXTLAT PREDADAM v9: optimizer-level predictive coding for PPO.
# =====================================================================================
# The winning NextLat auxiliary is always gradient-clipped, so it behaves as a persistent
# update direction rather than an ordinary weighted loss. v9 makes that mechanism explicit:
# task, predictive-trunk, and predictor gradients have independent Adam moments. Every
# predictive trunk tensor is projected into the intersection of the actor/critic descent
# half-spaces, locally trust-capped, then subjected to a strict global task-step cap. The
# dynamics predictor still receives its full private Adam update.
# Policy-head weights are frozen in the auxiliary decoder: future-policy KL shapes imagined
# latents without rewriting the behavior decoder. An optional causal meta-gate admits each
# tensor only when its preceding ungated-safe proposal forecasts the next task update.
# Hypothesis: selective update-space admission keeps the useful representation pressure of
# clip025 while removing destructive optimizer interference. Watch predadam/* diagnostics.
# =====================================================================================
...
class Args:
    ...
    # v9 optimizer-level admission. Task and prediction have independent Adam moments;
...
def _repair_optimizer_moments(optimizer, reference):
    """Return whether Adam moments were finite, repairing invalid entries fail-closed."""
    moments_finite = reference.new_ones((), dtype=torch.bool)
    for state in optimizer.state.values():
        for name, value in state.items():
            # Adam's scalar clock is not a parameter proposal moment. It is expected to
            # advance even when an invalid auxiliary transaction is rejected.
...
@torch.no_grad()
def update_predictive_meta_gates(
    previous_proposals,
    task_updates,
    cross_ema,
    proposal_sq_ema,
    *,
    ...
    """Update causal proposal/task statistics and return one gate per tensor.

    ``previous_proposals`` must contain the prior *ungated* safe proposal. Keeping the
    counterfactual proposal, rather than the update admitted by the previous gate, lets a
    closed gate continue collecting evidence and reopen when predictive/task alignment
    becomes positive again.
    """
    count = len(previous_proposals)
    if not (
        len(task_updates) == len(cross_ema) == len(proposal_sq_ema) == count
    ):
        raise ValueError("meta-gate state and update lists must align")
    if not 0.0 <= decay < 1.0:
        raise ValueError("meta-gate decay must lie in [0, 1)")
    gates = []
    for previous, task_update, cross, proposal_sq in zip(
        previous_proposals,
        task_updates,
        cross_ema,
        proposal_sq_ema,
        ...
        if (
            previous.shape != task_update.shape
            or previous.device != task_update.device
            or previous.dtype != task_update.dtype
        ):
            raise ValueError(
                "previous proposals and task updates must share shape, device, and dtype"
            )
        if cross.numel() != 1 or proposal_sq.numel() != 1:
            raise ValueError("meta-gate moments must be scalar")
        observed_cross = (previous * task_update).sum()
        observed_sq = previous.square().sum()
        cross.mul_(decay).add_(observed_cross, alpha=1.0 - decay)
        proposal_sq.mul_(decay).add_(observed_sq, alpha=1.0 - decay)
        learned_gate = torch.clamp(
            cross / proposal_sq.clamp_min(1e-20),
            min=0.0,
            max=1.0,
        )
        gates.append(learned_gate if warm else torch.ones_like(learned_gate))
    return gates
...
def admit_predictive_updates(
    ...
    *,
    ...
    """Protect and trust-cap predictive Adam parameter deltas.

    Actor/critic inputs are clipped loss gradients, whereas task/predictive inputs are
    optimizer parameter deltas. Each tensor is independently projected so its auxiliary
    delta cannot increase either loss to first order, then capped to ``max_ratio`` of that
    tensor's actual task delta. A final global cap makes the same ratio strict overall.
    """
    ...
    if gates is not None and len(gates) != len(predictive_updates):
        raise ValueError("one meta-gate is required per predictive update tensor")
    if max_ratio < 0.0:
        raise ValueError("max_ratio must be non-negative")
    for task, predictive in zip(task_updates, predictive_updates):
        if (
            task.shape != predictive.shape
            or task.device != predictive.device
            or task.dtype != predictive.dtype
        ):
            raise ValueError(
                "paired task and predictive updates must share shape, device, and dtype"
            )

    # A negative task delta is a useful fallback gradient proxy for standalone tests and
    # callers that do not retain loss gradients. Training always supplies both explicitly.
    ...
    for task, actor, critic in zip(task_updates, actor_gradients, critic_gradients):
        if (
            actor.shape != task.shape
            or critic.shape != task.shape
            or actor.device != task.device
            or critic.device != task.device
            or actor.dtype != task.dtype
            or critic.dtype != task.dtype
        ):
            raise ValueError(
                "actor/critic gradients must match their update shape, device, and dtype"
            )

    if layout is None:
        layout = make_update_tensor_layout(task_updates)
    lengths_tuple, lengths, groups = layout
    if lengths_tuple != tuple(update.numel() for update in task_updates):
    ...
    gate_values = flat_task.new_ones(len(task_updates))
    if gates is not None:
        gate_tensors = [
            torch.as_tensor(gate, device=flat_task.device, dtype=flat_task.dtype)
            for gate in gates
        ]
        if any(gate.numel() != 1 for gate in gate_tensors):
            raise ValueError("each meta-gate must be scalar")
        gate_values = torch.stack([gate.reshape(()) for gate in gate_tensors])

    # Any nonfinite optimizer/projection input vetoes the complete trunk transaction.
    # Sanitizing the whole vector, rather than individual entries, prevents a partially
    # valid layer from slipping through and keeps all following diagnostics finite.
    input_finite = torch.stack(
        [
            torch.isfinite(flat_task).all(),
            torch.isfinite(flat_predictive).all(),
            torch.isfinite(flat_actor).all(),
            torch.isfinite(flat_critic).all(),
            torch.isfinite(gate_values).all(),
        ]
    ).all()
    flat_task = torch.where(input_finite, flat_task, torch.zeros_like(flat_task))
    flat_predictive = torch.where(
        input_finite, flat_predictive, torch.zeros_like(flat_predictive)
    )
    flat_actor = torch.where(input_finite, flat_actor, torch.zeros_like(flat_actor))
    flat_critic = torch.where(input_finite, flat_critic, torch.zeros_like(flat_critic))
    gate_values = torch.where(input_finite, gate_values, torch.zeros_like(gate_values))
    gated = flat_predictive * gate_values[groups]
    ...
    task_sq_local = _segment_sum(flat_task.square(), lengths)
    pred_sq_local = _segment_sum(gated.square(), lengths)
    ...
    actor_pred = _segment_sum(flat_actor * gated, lengths)
    critic_pred = _segment_sum(flat_critic * gated, lengths)
    ...
    original_feasible = (
        actor_pred <= _projection_dot_tolerance(actor_sq, pred_sq_local)
    ) & (critic_pred <= _projection_dot_tolerance(critic_sq, pred_sq_local))
    ...
    projected_sq_local = _segment_sum(flat_projected.square(), lengths)
    task_norm_local = task_sq_local.sqrt()
    projected_norm_local = projected_sq_local.sqrt()
    local_scale = torch.clamp(
        max_ratio * task_norm_local / projected_norm_local.clamp_min(1e-20), max=1.0
    )
    local_scale = local_scale * (task_sq_local > 0.0)
    flat_local_admitted = flat_projected * local_scale[groups]

    task_sq = task_sq_local.sum()
    raw_pred_sq = flat_predictive.square().sum()
    raw_dot = (flat_task * flat_predictive).sum()
    local_sq = flat_local_admitted.square().sum()
    task_norm = task_sq.sqrt()
    local_norm = local_sq.sqrt()
    global_cap_scale = torch.clamp(
        max_ratio * task_norm / local_norm.clamp_min(1e-20), max=1.0
    )
    global_cap_scale = global_cap_scale * (task_sq > 0.0).to(global_cap_scale.dtype)
    flat_admitted = flat_local_admitted * global_cap_scale

    # Both caps are non-negative scalings and therefore preserve each half-space in exact
    # arithmetic. Check the actual capped tensors once more with their smaller scale so a
    # cap multiplication cannot turn a boundary projection into a spurious veto/violation.
    admitted_sq_local = _segment_sum(flat_admitted.square(), lengths)
    admitted_actor_dot = _segment_sum(flat_actor * flat_admitted, lengths)
    admitted_critic_dot = _segment_sum(flat_critic * flat_admitted, lengths)
    admitted_feasible = (
        admitted_actor_dot <= _projection_dot_tolerance(actor_sq, admitted_sq_local)
    ) & (
        admitted_critic_dot <= _projection_dot_tolerance(critic_sq, admitted_sq_local)
    )
    flat_admitted = flat_admitted * admitted_feasible[groups]
    admitted_sq_local = _segment_sum(flat_admitted.square(), lengths)
    admitted = [
        flat.view_as(reference)
        for flat, reference in zip(flat_admitted.split(lengths_tuple), task_updates)
    ]

    admitted_sq = flat_admitted.square().sum()
    ...
    admitted_norm_local = admitted_sq_local.sqrt()
    return admitted, {
        "task_norm": task_norm,
        "predictive_norm": raw_pred_sq.sqrt(),
        "raw_cosine": raw_cosine,
        "cap_scale": global_cap_scale,
        ...
        "actor_conflict_fraction": (actor_pred > 0.0).to(flat_task.dtype).mean(),
        "critic_conflict_fraction": (critic_pred > 0.0).to(flat_task.dtype).mean(),
        "local_nonzero_fraction": (admitted_norm_local > 0.0).to(flat_task.dtype).mean(),
        "numeric_valid": input_finite.to(flat_task.dtype),
        "max_local_ratio": (
...
def apply_predictive_trunk_transaction(
    ...
    *,
    ...
    """Advance predictive Adam and return gated and counterfactual-safe deltas.

    The third returned update list is the actor/critic-projected, trust-capped proposal
    with no meta gate. It is retained solely as the next causal meta-gate observation, so
    a zero gate cannot erase the evidence required to reopen itself.
    """
    ...
    for parameter, gradient in zip(parameters, gradients):
        if (
            ...
            raise ValueError(
                "predictive gradients must match their parameter shape, device, and dtype"
            )
    ...
    flat_gradient = torch.where(
        gradient_finite, flat_gradient, torch.zeros_like(flat_gradient)
    )
    for parameter, gradient in zip(parameters, flat_gradient.split(lengths)):
        parameter.grad = gradient.view_as(parameter)
    optimizer.step()
    flat_before = torch.cat([previous.reshape(-1) for previous in before])
    flat_raw = torch.cat(
        [parameter.detach().reshape(-1) for parameter in parameters]
    ) - flat_before
    proposal_finite = torch.isfinite(flat_raw).all()
    moments_finite = _repair_optimizer_moments(optimizer, parameters[0])
    transaction_valid = gradient_finite & proposal_finite & moments_finite
    flat_raw = torch.where(transaction_valid, flat_raw, torch.zeros_like(flat_raw))
    raw_updates = [
        update.view_as(reference)
        for update, reference in zip(flat_raw.split(lengths), parameters)
    ]
    ...
    if gates is None:
        ungated_safe_updates = admitted_updates
    else:
        # The optional meta path learns from the safe counterfactual that would have been
        # admitted at gate=1. This extra projection is intentionally absent when the gate
        # is disabled, preserving the default v9 execution and numerical behavior.
        ungated_safe_updates, _ = admit_predictive_updates(
            task_updates,
            raw_updates,
            max_ratio,
            actor_gradients=actor_gradients,
    ...
    transaction_valid = transaction_valid & stats["numeric_valid"].to(torch.bool)
    admitted_updates = [
        torch.where(transaction_valid, update, torch.zeros_like(update))
        for update in admitted_updates
    ]
    ungated_safe_updates = [
        torch.where(transaction_valid, update, torch.zeros_like(update))
        for update in ungated_safe_updates
    ]
    stats["numeric_valid"] = transaction_valid.to(parameters[0].dtype)
    for parameter, update in zip(parameters, admitted_updates):
        parameter.add_(update)
    optimizer.zero_grad(set_to_none=True)
    return raw_updates, admitted_updates, ungated_safe_updates, stats


@torch.no_grad()
def apply_private_optimizer_step(parameters, optimizer, gradients):
    """Atomically apply a finite private optimizer step and report its step norm."""
    if len(parameters) != len(gradients):
    ...
    for parameter, gradient in zip(parameters, gradients):
        if (
            ...
            raise ValueError(
                "private gradients must match their parameter shape, device, and dtype"
            )
    optimizer.zero_grad(set_to_none=True)
    before = [parameter.detach().clone() for parameter in parameters]
    flat_gradient = torch.cat([gradient.reshape(-1) for gradient in gradients])
    ...
    proposal_finite = torch.isfinite(flat_update).all()
    moments_finite = _repair_optimizer_moments(optimizer, parameters[0])
    transaction_valid = gradient_finite & proposal_finite & moments_finite
...
if __name__ == "__main__":
    ...
    predadam_layout = make_update_tensor_layout(nextlat_trunk_params)

    # Optional one-step causal meta-gate state: the preceding ungated, protected
    # predictive Adam proposal forecasts the current task Adam update. State is per
    # tensor, not pooled across unrelated layers whose optimizer geometry differs.
    # Allocate none of this O(P) state on the default disabled path.
    if args.predadam_meta_gate:
        meta_previous = [torch.zeros_like(p) for p in nextlat_trunk_params]
        meta_cross = [torch.zeros((), device=device) for _ in nextlat_trunk_params]
        meta_pred_sq = [torch.zeros((), device=device) for _ in nextlat_trunk_params]
    else:
        meta_previous = meta_cross = meta_pred_sq = None
    predadam_steps = 0 if args.predadam_meta_gate else None
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.nextlat:
                    # Optional causal forecast compares the prior ungated safe predictive
                    # proposal with the newly observed task delta. Retaining this
                    # counterfactual means a zero gate can still gather evidence and
                    # reopen. The disabled path allocates and computes no meta state.
                    gates = None
                    if args.predadam_meta_gate:
                        gates = update_predictive_meta_gates(
                            meta_previous,
                            task_updates,
                            meta_cross,
                            meta_pred_sq,
                            decay=args.predadam_meta_decay,
                            # Python control decision; introduces no device sync.
                            warm=predadam_steps >= args.predadam_meta_warmup,
                        )

                    agent.zero_grad(set_to_none=True)
                    predadam_predictor_step_norm = apply_private_optimizer_step(
                    ...
                    (
                        ...
                        gates=gates,
                        layout=predadam_layout,
                    )
                    if args.predadam_meta_gate:
                        for previous, proposal in zip(
                            meta_previous, ungated_safe_updates
                        ):
                            previous.copy_(proposal)
                        predadam_gate_mean = torch.stack(gates).mean()
                    else:
                        predadam_gate_mean = task_updates[0].new_ones(())
                    if args.predadam_meta_gate:
                        predadam_steps += 1
                else:
                    ...
                    predadam_stats = {
                        ...
                        "local_nonzero_fraction": zero_stat,
                        "numeric_valid": zero_stat,
                        "max_local_ratio": zero_stat,
                    }
        ...
        if args.nextlat:
            ...
            writer.add_scalar(
                "predadam/max_local_ratio",
                float(predadam_stats["max_local_ratio"]),
                global_step,
            )
            writer.add_scalar(
                "predadam/local_nonzero_fraction",
                float(predadam_stats["local_nonzero_fraction"]),
                global_step,
            )
            writer.add_scalar(
