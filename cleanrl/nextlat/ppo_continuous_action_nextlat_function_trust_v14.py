# NEXTLAT FUNCTION-TRUST v14: behavioral optimizer-level predictive coding for PPO.
# =====================================================================================
...
# exact function-space trust regions on a deterministic 256-state probe: Beta-policy KL and
...
class Agent(nn.Module):
    ...
    def get_behavior_probe(self, x):
        """Return deterministic Beta and categorical-critic outputs without sampling."""
        ...
        return alpha, beta, critic_logits
...
@torch.no_grad()
def clone_behavior_probe(probe):
    ...
    if isinstance(probe, BehaviorProbe):
        ...
        if len(probe) != 3:
            raise ValueError("a behavior probe must contain alpha, beta, and critic logits")
        tensors = probe
    return BehaviorProbe(*(tensor.detach().clone() for tensor in tensors))
...
def behavioral_kls(
    ...
    return (
        beta_policy_kl(reference, candidate),
...
if __name__ == "__main__":
    ...
    assert not args.nextlat or args.actor_dist == "beta", (
        "function-space policy trust is defined for the Beta actor"
    )
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.nextlat:
                    ...
                    with torch.no_grad():
                        ...
                        pre_task_probe = BehaviorProbe(
                            probe_dist.concentration1.detach().clone(),
                            probe_dist.concentration0.detach().clone(),
                ...
                if args.nextlat:
                    ...
                    # Task-induced behavior is the ruler for this auxiliary update. Clone
                    # compiled outputs because later probe calls may reuse graph buffers.
                    with torch.no_grad():
                    ...
                    (
                        ...
                        evaluate_probe=lambda: behavior_probe_fn(probe_obs),
