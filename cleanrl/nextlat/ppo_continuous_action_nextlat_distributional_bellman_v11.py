# NEXTLAT DISTRIBUTIONAL BELLMAN v11: value-grounded predictive coding for PPO.
# =====================================================================================
...
# Watch bellman/*, nextlat/*, and predadam/* diagnostics.
# =====================================================================================
import copy
import os
import random
...
def build_nstep_distributional_targets(
    ...
    This is the same recursive lambda-return learned by the PPO critic, rather than the
    full-return ``r + gamma Z`` operator (which is recovered when lambda=1). The returned
    mask has shape ``(T, B, len(horizons))``.
    """
    if rewards.ndim != 2:
    ...
    for horizon_index, horizon in enumerate(horizons):
        ...
        for step in range(unroll_steps):
            ...
            if truncated.any():
                prefix_destination = prefix[:valid_length]
                prefix_destination[truncated] += (
                    trace_discount * gamma * next_values[step:][truncated]
                )
            ...
            if step == unroll_steps - 1:
                ...
                if bootstrapped.any():
                    destination = bootstrap_probs[:valid_length]
                    destination[bootstrapped] = next_value_probabilities[step:][bootstrapped]
                    destination_scale = bootstrap_scale[:valid_length]
                    destination_scale[bootstrapped] = (gamma * gae_lambda) ** (
                        step + 1
                    )

        flat_valid = valid.reshape(-1)
        valid_indices = flat_valid.nonzero(as_tuple=False).squeeze(-1)
        flat_target = targets[:, :, horizon_index].reshape(-1, num_bins)
        flat_probs = bootstrap_probs.reshape(-1, num_bins)
        flat_prefix = prefix.reshape(-1)
        flat_scale = bootstrap_scale.reshape(-1)
        for start in range(0, valid_indices.numel(), projection_chunk):
            indices = valid_indices[start : start + projection_chunk]
            flat_target[indices] = categorical_affine_projection(
                flat_probs[indices], flat_prefix[indices], flat_scale[indices], support
            )
...
class Args:
    ...
    # v11 value-grounded NextLat. Comma-separated powers of two keep the CLI compact.
    nextlat: bool = True
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Project on CPU, then keep the immutable table resident on CUDA for all PPO
            # epochs. Defaults are ~383 MiB (T*B*6*511 fp32); repeated pageable-host
            # advanced indexing and H2D copies would otherwise move ~3.8 GiB/iteration.
            target_probs = hl_support_cpu.project(return_mtp.detach().cpu()).to(device)
            ...
            if args.nextlat:
                bellman_targets, bellman_masks = build_nstep_distributional_targets(
                    rewards.detach().cpu(),
                    transition_terminations.detach().cpu(),
                    transition_boundaries.detach().cpu(),
                    transition_valids.detach().cpu(),
                    frozen_next_value_probs.detach().cpu(),
                    next_transition_values.detach().cpu(),
                    hl_support_cpu.support,
                    args.gamma,
                    args.gae_lambda,
                    bellman_horizons,
                    args.bellman_projection_chunk,
                )
                ...
                # minibatch, eliminating another ~3.8 GiB of pageable H2D per iteration.
                # No second full-sized device copy is kept.
                bellman_targets = bellman_targets.to(
                    device=device, dtype=torch.float16
                )
                bellman_masks = bellman_masks.to(device)
                label_storage_mib = (
