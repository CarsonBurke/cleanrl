# PPO + IterThink v24 beta no-critic-bias v1.
#
# Variant hypothesis: remove v24's peaked Gaussian-logit critic-head bias and
# leave the categorical value head at standard zero-bias init. This tests whether
# the narrow initial Z(s) prior is stabilizing or whether it slows early value
# adaptation by over-anchoring returns near zero.
#
# Base: ppo_continuous_action_iterthink_v24_dist.py / iterthink_v24_beta_s1.
# Only intended algorithmic change: critic_head.bias is no longer overwritten.
#
...
class Args:
    ...
    v_min: float = -10.0
    v_max: float = 10.0
    critic_init_tau: float = 0.5   # kept for CLI parity with v24_dist; unused in this no-bias ablation
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Categorical critic. This ablation deliberately removes v24's
        # Gaussian-logit bias prior, leaving the layer_init zero bias in place.
