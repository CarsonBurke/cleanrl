# Raw-return variance-matched categorical critic on residual SiTU PPO v17.
# Loss-only ablation of v13 raw mean-matched HL-Gauss K101/sigma0.5.
# Multiply CE by detached batch-mean predicted variance of RAW support values,
# not variance of symlog coordinates. Detachment prevents variance-collapse incentives.
# Hypothesis: restoring local mean-error sensitivity improves critic fitting;
# shared Adam/global clipping can still change actor-critic gradient balance.
# Keep raw rewards, fixed +/-20000 support, original labels, trunks and projection.
import math
import os
import random
import time
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", f"original residual SiTU-GLU trunks and matrix projection; raw rewards; fixed symexp-spaced raw nodes; {args.value_target} mean-preserving labels; paired raw-expectation decode; detached predicted raw-variance CE gain; no return frame or EMA")
