# PPO + FIRE on critic only
#
# Same schedule [2M, 4M, 6M] and NS mechanics as fire_v1, but FIRE is
# applied ONLY to the critic submodule. Actor weights are not touched
# (so they continue accruing whatever spectral pathology drives baseline
# stagnation). Pairs with fire_actor_only_v1 to localize FIRE's win:
# value-function plasticity vs. policy plasticity. If critic-only ≈
# vanilla FIRE, then value-net rank loss is the bottleneck.
import os
import random
import time
...
@torch.no_grad()
def fire_reinit_critic_only(agent: nn.Module, num_iters: int = 10):
    """FIRE applied only to agent.critic (value head + trunk). Actor
    weights are deliberately left untouched."""
    for m in agent.critic.modules():
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        while fire_pending and global_step >= fire_pending[0]:
            ...
            fire_reinit_critic_only(agent, num_iters=args.fire_ns_iters)
            # Reset Adam state ONLY for critic params (leave actor moments alone)
