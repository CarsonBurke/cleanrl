# ENT-PPO CORR-NOISE v1 -- temporally CORRELATED action noise with an EXACT conditional density.
# =====================================================================================
# THE MECHANISM. The chassis draws its action noise independently at every step. Here the
# noise is generated by a per-env AR(1) latent that is part of the AGENT's state:
#
#   z_t = rho (*) z_{t-1} + sqrt(1 - rho^2) (*) xi_t,   xi_t ~ N(0, I_d)   [(*) = elementwise]
#   u_t = mu(s_t) + L(s_t) z_t,        a_t = tanh(u_t)          [tanh = the chassis's map]
#
# Conditioned on the RECORDED z_{t-1}, u_t is EXACTLY Gaussian:
#   u_t | s_t, z_{t-1} ~ N( mu(s_t) + L(s_t)(rho (*) z_{t-1}),  M M^T ),  M = L diag(sqrt(1-rho^2))
# so log_prob / entropy / the PPO importance ratio are closed form and need NO approximation.
# z_{t-1} is stored in the rollout buffer next to obs/actions and replayed in the update pass,
# so the recomputed log_prob is the same conditional density that generated the sample. z is the
# agent's own memory -- no simulator state is read anywhere.
#   L(s) = diag(sigma(s)) (I_d + Uhat(s) V(s)^T)   [== diag(sigma) + U V^T with U = diag(sigma) Uhat]
# with rank k = --corr-rank (default 2). log|det M| uses the matrix-determinant lemma and
# M^{-1}r uses Woodbury on the k x k capacitance K = I_k + V^T Uhat, so the density path never
# forms or factorizes a (d x d) matrix, and the k x k capacitance is factorized ONCE per
# distribution instance and reused by log|det| and every solve. (d, k) = (6, 2) here. The ONE
# place a dense d x d object appears is the exact all-action KL, whose tr(Sigma_old^{-1} Sigma_new)
# is a d x d contraction by nature: ~180 mult/sample at d = 6, MEASURED at 2.58 ms of the
# 10.85 ms champion minibatch, i.e. 24% of the corr update -- launch-bound, not FLOP-bound. The
# whole corr path is nonetheless FASTER than both controls (gate H), because it replaces
# torch.distributions objects (validate_args=True => per-call support checks and syncs) and the
# gaussian arm's extra rsample-based entropy estimate with closed forms.
# rho = tanh(raw_rho), one per action dim, initialized at EXACTLY 0, learned by the ordinary
# PG + KL gradient. No schedule, no hand-tuning.
#
# WHY THIS SHOULD BEAT WHITE NOISE AT EQUAL ENTROPY. The nats the chassis spends are spent on the
# CONDITIONAL entropy (that is what -log pi_old and the alpha*KL term both measure):
#   H(u_t | s_t, z_{t-1}) = H_marginal(u_t | s_t) + 0.5 * sum_i log(1 - rho_i^2),
# i.e. at a FIXED conditional-entropy budget the marginal noise scale satisfies
#   log|det L| = budget - 0.5 * sum_i log(1 - rho_i^2),
# which GROWS as |rho| -> 1. White per-step noise in HalfCheetah largely self-cancels: consecutive
# independent kicks integrate to a random walk of size sqrt(T), so entropy buys jitter around the
# mean trajectory, not displacement from it. The AR(1) latent spends the same nats on excursions
# that persist for ~1/(1-rho) steps and are coordinated across joints by the low-rank part of L,
# i.e. sustained limb pushes -> O(T) displacement -> more state coverage per nat. The mechanism is
# therefore a re-ALLOCATION of the chassis's existing entropy budget, not an increase of it.
#
# THE ENTROPY HEADWIND ON rho -- PREDICTED LARGE, MEASURED SMALL. This was the mechanism most
# likely to silently null the file, so it was measured rather than argued. Since
#   H(u_t | s_t, z_{t-1}) = const + log|det L| + 0.5 * sum_i log(1 - rho_i^2),
# H is STRICTLY DECREASING in |rho_i| (dH/d raw_rho_i = -rho_i exactly), so anything that rewards
# entropy pushes rho back to 0. ent_coef is 0 here, so the candidate channel is the alpha*KL
# proximal term via KL(new||old) = -H(new) + E_new[-log pi_old]: penalizing +kl_coef*KL adds
# -kl_coef*H(new), which looks like an entropy reward at coefficient kl_coef (~0.354 measured)
# and hence a LINEAR restoring force kl_coef*rho_i toward 0.
# THAT READING IS WRONG, and reading only that half over-states the headwind by ~100x. The two
# halves of the SAME KL almost exactly cancel. Measured on 4096 real on-policy states at
# kl_coef = 0.3543, after 20 real Adam steps so the policy has drifted to exact_kl ~= 0.030
# (the champion config's own realized drift is 0.023 at iteration 1), per-dim gradient norms:
#   rho   |KL-entropy half|  |KL-cross half|  |NET KL|  |PG channel|   |PG| / |NET KL|
#   0.4        0.3488            0.3429        0.0068     0.0198            2.9
#   0.7        0.6083            0.6034        0.0066     0.0337            5.1
#   0.8        0.6952            0.6895        0.0089     0.0312            3.5
# and at zero drift (epoch 1, minibatch 1, new == old) the NET KL gradient on raw_rho is
# identically 0 to fp32 (|NET| = 1e-8 against halves of 0.61) because the KL is at its minimum.
# The KL-entropy half does track the analytic +kl_coef*rho to 4 digits (0.2483 vs 0.2480 at
# rho = 0.7), so the derivation is right; it is the CANCELLATION that kills the headwind.
# CONCLUSION: KL(new||old) is a PROXIMAL ANCHOR, not an entropy bonus. It resists CHANGING rho
# within an iteration; it has no preference for rho = 0. The net trust-region force is ~1% of
# either half and is out-weighted 3-5x by the policy gradient, so whether rho grows is decided by
# whether correlated exploration actually EARNS advantage -- which is exactly the experiment.
# Consequences for reading the result:
#   (a) There is no strong restoring force to overcome, so a learned rho that stays near 0 means
#       the PG did not find correlated exploration useful -- a real negative result about the
#       mechanism, NOT an artifact of the entropy term. That is what P2 tests.
#   (b) The residual net force does grow with drift (0.0068 -> 0.0089 across rho 0.4 -> 0.8), so
#       the coupling to the sample-mask is real but second-order: correlated noise widens the
#       per-sample ratio spread, the mask fires more, drift rises, and the anchor stiffens.
#       debug/tr_mask_frac must be read NEXT TO debug/corr_rho_absmean, not alone.
#   (c) The soft-MDP reward r~ = r + alpha*(-log pi_old) DOES pay less for correlated noise
#       (E[-log pi] = H_cond, lower by |0.5 sum log(1-rho^2)|; at rho = 0.8 that is 3.07 nats,
#       i.e. 0.92 reward/step against ~6 task reward/step). But log pi_old is DETACHED, so this
#       is not a gradient on rho at all: at fixed rho it is a constant per-step shift, which the
#       chassis's own header notes cancels in delta_t once the critic fits. It can only act
#       indirectly and slowly, through the critic's valuation of a high-rho regime.
# The entropy stays the TRUE conditional entropy: it is the correct object and hacking it would
# break the Ent-PPO derivation. --corr-rho-fixed FLOAT instead PINS rho on every action dim
# (raw_rho frozen, requires_grad False, excluded from Adam and from the actor clip) so the
# mechanism can be delivered at full dose regardless of what the learning signal decides. It is
# an experimental arm, not a default: the default path learns rho.
#
# GATING AND THE HONEST CONFOUND. --corr-noise (default ON) overrides the actor readout to the
# correlated Gaussian. --no-corr-noise restores args.actor_dist VERBATIM and constructs none of
# the new parameters, so it is the chassis bit-for-bit (gate B). Two controls exist and they are
# not the same experiment:
#   * --no-corr-noise                      == the chassis's DEFAULT Beta champion (10362 @8M).
#   * --no-corr-noise --actor-dist gaussian == the chassis's Gaussian arm, and the ONLY matched
#     control for this file: same trunk/critic/mean/logvar init (identical RNG prefix), white
#     diagonal noise instead of correlated low-rank noise. Exactly one mechanism differs.
# Against the Beta champion this file changes TWO things (Beta -> tanh-Gaussian, and white ->
# correlated), because the mechanism requires a location-scale family. Any comparison to 10362
# is confounded by that and P4 below is the falsifier for it.
#
...
# grad clipping, and every hyperparameter value (max_grad_norm included). None of these were
# touched. VERIFIED IN THIS FILE: the items in the block at the bottom of this header.
#
# PRE-REGISTERED PREDICTIONS (chassis reference: 10362 +-92 @8M; matched checkpoints
# @1M 3041 / @2M 5465 / @3M 7281 / @4M 8890; benchmark noise floor ~400 points).
#   P1 RETURN. PASS: >= 10762 @8M (chassis + one noise floor) AND >= 5865 @2M.
#      FAIL: <= 9962 @8M, or <= 5065 @2M and <= 8490 @4M (two consecutive checkpoints a full
#      noise floor below the chassis). In between = INCONCLUSIVE, report it as such.
#   P2 THE MECHANISM IS ALIVE. PASS: mean_i |rho_i| >= 0.20 and debug/corr_lag1_ac_mean >= 0.15
#      by 2M. FAIL: debug/corr_rho_absmax < 0.05 at 4M -- rho never left its init, the AR(1) term
#      is inert, and ANY return difference is attributable to Beta -> Gaussian, not to correlation.
#   P3 NATS ARE RE-ALLOCATED, NOT ADDED. PASS: debug/corr_marginal_minus_cond_logdet >= 0.30 nats
#      by 4M (the policy really is buying marginal scale with correlation) while
#      debug/entropy_bonus_mean stays within +-1.0 nat of the matched Gaussian control's.
#      FAIL: the entropy bonus is > 1.0 nat ABOVE the control's -- then the arm simply bought more
#      entropy and P1 is not a test of the displacement story.
#   P4 THE GAIN IS THE CORRELATION, NOT THE FAMILY SWITCH. PASS: corr arm >= matched Gaussian
#      control + 400 @4M. FAIL: the two are within 400 @4M -- the correlated latent contributed
#      nothing on top of the tanh-Gaussian readout, and this file should be abandoned in favor of
#      whatever the family switch alone is worth.
#
# --- VERIFIED IN THIS FILE (measured, not claimed) -----------------------------------
# Every number below was produced by this exact file. Nothing here is an 8M result: NO 8M run
# was performed. Short config S = --num-envs 4 --num-steps 256 --num-minibatches 4
# --total-timesteps 20480 --seed 1. Champion config C = --num-envs 16 --num-steps 2048
# --num-minibatches 32 --update-epochs 10 --total-timesteps 262144 --seed 1.
#
...
#   36 shared tags, 0 tags unique to either side, 720 points compared, 0 DIFFERING, in all three:
#     parent(default beta)   vs  --no-corr-noise                        -> 0/720
#     parent(default beta)   vs  --no-corr-noise --corr-rho-fixed 0      -> 0/720
#     parent --actor-dist gaussian vs --no-corr-noise --actor-dist gaussian -> 0/720
#   charts/episodic_return (20 pts) is among the compared tags, so the TRAJECTORIES are identical,
#   not merely the summary statistics. The OFF path constructs no new parameter and draws no extra
#   RNG, at init or in the rollout, which is why this is exact rather than close.
#
# C DENSITY CORRECTNESS -- the load-bearing gate. rho = 0.7 on all 6 dims, corr_rank 2, one fixed
#   state and one fixed z_prev, N = 1e6 samples drawn through the ACTUAL sampling path.
#   (i)   mean: max abs err 1.043e-3 = 1.21 MC sigma vs analytic mu + L(rho*z_prev).
#         CorrGaussian.mean (Woodbury) vs a dense reference: 2.98e-8.
#   (ii)  covariance vs analytic (1-rho^2) L L^T: MAX ABS ELEMENTWISE ERR 1.3701e-3
#         (rel 1.854e-3 of ||Sigma||_max = 0.7389; MC 1-sigma 1.045e-3, i.e. 1.31 sigma).
#         dense_M @ dense_M^T vs the same reference: 1.19e-7.
#   (iii) normalization: E[-log p] = 6.45742138 +- 1.73e-3 (MC SE) vs closed-form entropy
#         6.45821714 -> diff -7.96e-4 = 0.46 MC sigma. Independent 0.5*logdet(2 pi e Sigma)
#         = 6.45821723, i.e. the entropy formula itself is right to 8.6e-8.
#   (iv)  log_prob vs torch.distributions.MultivariateNormal on the dense Sigma (a completely
#         independent implementation): max abs diff 5.07e-6 over 200k samples (|log p| up to
#         20.2), mean 3.99e-7 -- fp32 round-off.
#   (v)   sampling path: recovered xi has mean 1.5e-3 and per-dim std 0.9988..1.0005;
#         |u - (mu + L z_t)| = 9.5e-7; Var(z_t | z_prev) = 0.5099 vs 1 - rho^2 = 0.51.
#   (vi)  rollout/replay: |log_prob(rollout) - log_prob(replay)| = 0.0 EXACTLY, action == tanh(u)
#         exactly, entropy == closed form exactly. The PPO ratio therefore replays the same
#         conditional density that generated the sample.
#   (vii) exact KL: self-KL = 0.0; corr_kl vs a torch MultivariateNormal KL between the same two
#         Gaussians = 1.62798834 vs 1.62798844, rel err 6.2e-8.
#   (viii) as a bonus consistency check across the pinned-rho runs on C, the logged
#         debug/corr_marginal_minus_cond_logdet is 3.065 / 4.982 / 6.984 at rho 0.8 / 0.9 / 0.95
#         against the analytic -3*log(1-rho^2) = 3.0650 / 4.9821 / 6.9838, and losses/entropy
#         reproduces 8.51 + log|det L| - that quantity to 3 decimals.
#
# D rho IS ALIVE AND LEARNS.
#   init: max|rho| = 0.000e+00 EXACTLY (raw_rho is a zeros Parameter).
#   grad wrt raw_rho at init = [+0.4216 +0.1487 -0.1867 -0.8704 -0.0993 +0.3313]; finite, nonzero,
#     max 8.70e-1. V-head grad 3.54e-2 (> 0, so the low-rank part is not dead); Uhat-head grad
#     exactly 0 at init, as expected since dL/dUhat ~ V = 0.
#   after 800 real Adam steps (S): rho = [+0.0082 +0.0082 -0.0011 +0.0029 -0.0038 -0.0093].
#   after 2560 real Adam steps (C): rho = [-0.0041 -0.0080 +0.0002 +0.0162 -0.0089 -0.0129],
#     absmax 0.0182, absmean 0.0084, rising roughly monotonically from 0.0029.
#   HONEST READING: rho is unambiguously off 0 and moving, but at 262k steps (3% of the 8M
#   horizon) it is only ~1e-2. This file CANNOT locate the equilibrium; P2 is checked at 2M.
#   Run-to-run spread at 262k is ~2x (0.018 to 0.036 absmax across builds differing only in
#   fp32-level linear-algebra ordering), so treat the magnitude as order-of-magnitude only.
#   THREE-WAY GRADIENT DECOMPOSITION (see the headwind section): the KL's -H(new) half is large
#   and matches +kl_coef*rho analytically, but is cancelled to ~1% by its own cross-entropy half;
#   net trust-region force is 3-5x SMALLER than the PG channel. rho's fate is decided by the PG.
#
# E TEMPORAL CORRELATION IS REAL. Lag-1 autocorrelation of the noise u_t - mu(s_t), within-episode
#   pairs only, per action dim.
#   From REAL training rollouts on C with rho pinned: debug/corr_lag1_ac_mean = 0.7992 / 0.8994 /
#     0.9497 at rho = 0.8 / 0.9 / 0.95 -- the realized action-space autocorrelation equals rho to
#     3-4 digits, so the plumbing carries the intended dynamics end to end.
#   Matched white-noise control (--no-corr-noise --actor-dist gaussian): per-dim
#     [0.0047 -0.0310 -0.0097 -0.0080 +0.0147 +0.0211], mean -0.0014, |max| 0.0310 against a
#     1/sqrt(n) scale of 0.0204 -- indistinguishable from 0, as white noise must be.
#   corr arm with rho FORCED to 0: mean +0.0019, |max| 0.0331 -- matches the control, so the
#     low-rank part alone introduces no temporal structure.
#   corr arm with rho FORCED to 0.7: per-dim [0.7073 0.7050 0.7096 0.6987 0.7250 0.7016].
#   LEARNED arm at 262k: lag1_ac_mean 0.0002 -> -0.0053, i.e. ~0. This is STATED PLAINLY, not
#     papered over: at rho ~ 0.016 there is nothing to see yet, so the demonstration above is the
#     forced-rho one, exactly as the acceptance criterion permits.
#
# F RESET CORRECTNESS. 1500 steps x 16 envs with a 40-step time limit (same env, shorter limit,
#   purely to generate boundaries) at rho pinned to 0.95 so an un-reset latent is unmistakable;
#   592 boundary events.
#   non-boundary steps with stored z_prev == carried z: 23408/23408 (must be all).
#   boundary steps with stored z_prev == carried z:     0/592      (must be none).
#   corr(stored post-reset latent, carried pre-reset latent) per dim, max |.| = 0.0734 against
#     1/sqrt(592) = 0.0411, i.e. 1.78 sigma -> independent.
#   the stored post-reset latent IS a standard normal draw: per-dim mean within 0.043 of 0
#     (MC SE 0.0411), std 0.953..1.028, KS p-values vs N(0,1) 0.362..0.963.
#   control on non-reset steps: corr(z_t, z_prev) = 0.9475..0.9516, i.e. exactly the pinned rho.
#   stationarity (the reason for fresh-draw rather than zeros): latent variance at boundary steps
#     0.9820 vs 1.0054 elsewhere. Zeroing would have given 0.0 vs ~1.0 and shrunk the first
#     post-reset action's noise to sqrt(1-rho^2) of its stationary scale.
#
# G NO NaN/Inf over 262144 steps at champion config C. Five arms: learned rho, --no-corr-noise,
#   and --corr-rho-fixed 0.8 / 0.9 / 0.95. EVERY scalar of EVERY arm scanned (968 points per corr
#   arm over 59 tags; 792 over 37 tags for the control): NONFINITE = NONE anywhere.
#   debug/epochs_completed = 10/10 in every iteration of every arm; kl_breaker_tripped 0
#   everywhere; debug/target_edge_mass 0 everywhere.
#   losses/value_loss first->last: learned 20.46->10.85, control 24.0->22.9, rho0.8 28.08->18.62,
#     rho0.9 25.24->16.19, rho0.95 22.17->13.31 -- all finite and falling.
#   losses/explained_variance: learned 0->0.719, control 0->0.851, rho0.8 0->0.300,
#     rho0.9 0->0.347, rho0.95 0->0.310. Finite and sane BUT the pinned-rho arms are materially
#     WORSE than the control at 262k. Reported as a warning, not explained away: a louder action
#     process makes the value target harder, and whether it recovers by 1M is untested.
#   SAMPLE-MASK SENSITIVITY (the chassis's known-fragile axis), mean/max over the 8 iterations:
#     arm            tr_mask_frac      approx_kl        ratio_max  breaker  epochs
#     control        0.0062 / 0.0083   0.0101 / 0.0126    5.55        0     10/10
#     learned rho    0.0121 / 0.0214   0.0128 / 0.0223    9.60        0     10/10
#     rho 0.8        0.0342 / 0.0431   0.0213 / 0.0257    3.60        0     10/10
#     rho 0.9        0.0518 / 0.0640   0.0261 / 0.0310    3.87        0     10/10
#     rho 0.95       0.0656 / 0.0883   0.0361 / 0.0554    5.46        0     10/10
#   rho = 0.8 is CLEAN: no NaN, no breaker trip (max approx_kl 0.0257 is 5.8x under the 0.15
#   breaker), full epoch budget, and a LOWER ratio_max than the control. Degradation with rho is
#   monotone but gentle; even 0.95 does not trip anything. The real ceiling is not NaN, it is the
#   actor clip: pre-clip losses/actor_grad_norm runs 1.0 (control) / 3.4 (learned) / 5.9 (rho0.8)
#   / 12.8 (rho0.9) / 17.7 (rho0.95) against actor_grad_clip = 0.25, so at 0.95 the step is
#   renormalized ~70x and its direction is set almost entirely by clipping. RECOMMENDATION: 0.8
#   is the right full dose, 0.9 is acceptable, 0.95 is the numerical ceiling. No default changed.
#
# H THROUGHPUT AND VRAM at champion config C.
#   Peak VRAM (nvidia-smi, process-attributed): corr arms 2208 MiB, --no-corr-noise beta
#     2204 MiB, gaussian control 2206 MiB. The mechanism costs 2-4 MiB.
#   SPS: the GPU was SHARED with an unrelated 16-22 GB training job for most of these
#     measurements, so absolute SPS is a contended lower bound and is reported as a range.
#     Lighter-contention pass: learned 1678, beta control 825, rho0.8 2192, rho0.9 1694,
#     rho0.95 1043. Heavier-contention pass: corr 653-708, beta 488-521, gaussian 483-486.
#   Because SPS was unreliable, the mechanism's cost was ALSO measured directly and uncontended,
#     as one champion-size (B = 1024) minibatch fwd+bwd of the full actor objective:
#       corr rank2 rho=0    10.85 ms      corr rank2 rho=0.7   10.74 ms
#       corr rank0 rho=0.7   6.63 ms      no-corr beta         23.01 ms
#       no-corr gaussian    18.48 ms      trunk-only floor      5.99 ms
#     and one rollout forward at B = 16: corr 2.74 ms, gaussian 12.10 ms, beta 10.60 ms.
#     The exact all-action KL alone is 2.577 ms of the 10.887 ms corr update (23.7%).
#   So the corr path is 1.7-2.1x FASTER per update than either control. That is not the mechanism
#     being free -- it is the mechanism being cheaper than what it replaces: torch.distributions
#     runs with validate_args=True (per-call support checks, each a GPU->CPU sync) and the
#     gaussian arm additionally pays an rsample + second log_prob for its sampled squashed
#     entropy, both of which the closed-form path does not need.
#   TIME LIMIT GUIDANCE for an 8M queue slot: 1.3 h at the observed 1678 SPS, 3.4 h at the
#     contended 653 SPS. Budget >= 5 h.
#
# I The parent chassis was never modified: md5 f6c545f51d6c49901bd739f6f11aabca before and after.
#   All scratch run directories created for these measurements were deleted.
#
# NOT VERIFIED HERE, AND NOT CLAIMED: any return at 1M/2M/4M/8M; that the mechanism helps at all.
#   At 262k the corr arms are BEHIND the beta control on episodic return (last-16 mean: control
#   +225, learned -96, rho0.8 -179, rho0.9 -176, rho0.95 -192). 262k is 3% of the horizon and the
#   chassis itself is only at ~3041 by 1M, so this is weak evidence -- but it is evidence pointing
#   the wrong way, and it is recorded here rather than omitted.
# ------------------------------------------------------------------------------------
#
...
# CORR-NOISE numerical guard (NOT a tuning knob). The low-rank factors are smoothly rescaled so
# that ||Uhat||_F * ||V||_F < CORR_SPEC_CAP. Then every eigenvalue of I_d + Uhat V^T lies in the
# open disk |w - 1| < CORR_SPEC_CAP, so it has positive real part, det > 0, and
# (1-cap)^k <= det(I_k + V^T Uhat) <= (1+cap)^k. With cap = 0.9, k = 2 that is [0.01, 3.61]:
# log|det L| is bounded within [sum log sigma - 4.61, sum log sigma + 1.29] and
# ||(I + Uhat V^T)^{-1}||_2 <= 10. L is PROVABLY invertible for every parameter value, so the
# density can never blow up -- which an unconstrained diag + U V^T cannot promise.
CORR_SPEC_CAP = 0.9
LOG_2PI = float(np.log(2.0 * np.pi))


class CorrGaussian:
    """Conditional action distribution of the correlated-noise policy (see file header).

        u | s, z_prev ~ N( mu + L (rho * z_prev),  M M^T ),   M = L diag(sqrt(1 - rho^2))
        L = diag(sigma) (I_d + Uhat V^T)

    Everything on the DENSITY path (mean, log|det M|, M^{-1} r, entropy, sampling) is computed
    with the matrix-determinant lemma / Woodbury on the k x k capacitance K = I_k + V^T Uhat.
    No (d x d) matrix is formed. `dense_M` exists only for the exact all-action KL.

    rho is a (d,) tensor shared across the batch (a policy parameter, not a state readout);
    everything else is batched (B, ...). k == 0 is supported: L is then exactly diagonal and the
    only mechanism left is the temporal AR(1) memory.
    """

    def __init__(self, mu, sigma, uhat, v, rho, z_prev):
        self.mu, self.sigma, self.uhat, self.v = mu, sigma, uhat, v
        self.rho, self.z_prev = rho, z_prev
        self.d = mu.shape[-1]
        self.k = uhat.shape[-1]
        if self.k > 0:
            eye = torch.eye(self.k, device=mu.device, dtype=mu.dtype)
            cap = eye + v.transpose(-1, -2) @ uhat                 # (B, k, k) capacitance
            # ONE factorization, reused by log|det| and by every solve on this instance
            # (log_prob, the KL's two applies). torch.linalg.solve would re-factorize per call.
            # |det| = prod |U_ii| since P and L are unit-determinant, and the contraction bound
            # guarantees |det| >= (1 - CORR_SPEC_CAP)^k, so the log is always finite.
            self.cap_lu, self.cap_piv = torch.linalg.lu_factor(cap)
            self.log_absdet_cap = (
                self.cap_lu.diagonal(dim1=-2, dim2=-1).abs().log().sum(-1)
            )
        else:
            self.cap_lu = self.cap_piv = None
            self.log_absdet_cap = mu.new_zeros(mu.shape[0])
        # sqrt(1 - rho^2): the AR(1) innovation scale. rho = tanh(raw) in (-1,1) => c in (0,1].
        self.c = (1.0 - rho * rho).clamp_min(1e-8).sqrt()           # (d,)
        # log|det M| = sum log sigma + log|det(I_k + V^T Uhat)| + sum log c
        self.log_det_l = sigma.log().sum(-1) + self.log_absdet_cap  # (B,) marginal factor
        self.log_det_m = self.log_det_l + self.c.log().sum()        # (B,) conditional factor
        self.mean = mu + self.apply_l(rho * z_prev)
        self.z_sample = None

    def apply_l(self, vec):
        # L vec = sigma * (vec + Uhat (V^T vec))
        if self.k == 0:
            return self.sigma * vec
        t = self.v.transpose(-1, -2) @ vec.unsqueeze(-1)            # (B, k, 1)
        return self.sigma * (vec + (self.uhat @ t).squeeze(-1))

    def solve_l(self, r):
        # L^{-1} r = (I + Uhat V^T)^{-1} (r / sigma), by Woodbury on the k x k capacitance.
        y = r / self.sigma
        if self.k == 0:
            return y
        t = torch.linalg.lu_solve(
            self.cap_lu, self.cap_piv, self.v.transpose(-1, -2) @ y.unsqueeze(-1)
        )
        return y - (self.uhat @ t).squeeze(-1)

    def solve_m_mat(self, rhs):
        # M^{-1} rhs for rhs (B, d, m). M^{-1} = diag(1/c) L^{-1}.
        y = rhs / self.sigma.unsqueeze(-1)
        if self.k > 0:
            t = torch.linalg.lu_solve(self.cap_lu, self.cap_piv, self.v.transpose(-1, -2) @ y)
            y = y - self.uhat @ t
        return y / self.c.unsqueeze(-1)

    def dense_M(self):
        # (B, d, d) = diag(sigma) (I + Uhat V^T) diag(c). ONLY the exact all-action KL needs
        # this: tr(Sigma_old^{-1} Sigma_new) is a d x d contraction by nature. d = 6 here.
        eye = torch.eye(self.d, device=self.mu.device, dtype=self.mu.dtype)
        a = eye if self.k == 0 else self.uhat @ self.v.transpose(-1, -2) + eye
        return self.sigma.unsqueeze(-1) * a * self.c

    def sample(self):
        # Returns (u, z_t). z_t is the AR(1) latent to carry into the next step.
        xi = torch.randn_like(self.mu)
        z = self.rho * self.z_prev + self.c * xi
        return self.mu + self.apply_l(z), z

    def log_prob(self, u):
        # Exact log N(u; mean, M M^T). Whitening uses M^{-1} = diag(1/c) L^{-1}.
        x = self.solve_l(u - self.mean) / self.c
        return -0.5 * (x * x).sum(-1) - 0.5 * self.d * LOG_2PI - self.log_det_m

    def entropy(self):
        # Closed-form differential entropy of the CONDITIONAL Gaussian (item 5 of the contract):
        # H = 0.5 d log(2 pi e) + log|det M|. Note H = H_marginal + 0.5 sum log(1 - rho^2), i.e.
        # correlation LOWERS the conditional entropy at fixed L -- the budget the chassis's
        # -log pi_old reward bonus and alpha*KL term act on. See the header's equal-entropy
        # argument. Native (pre-tanh) space, matching the chassis's Beta convention of dropping
        # the action-map Jacobian from the entropy (it is a per-sample constant in the ratio).
        return 0.5 * self.d * (1.0 + LOG_2PI) + self.log_det_m


def corr_kl(new, old):
    """Exact all-action KL(new || old) between two CorrGaussian conditionals at the same z_prev.

    KL = 0.5 [ log det S_old/S_new - d + tr(S_old^{-1} S_new) + dm^T S_old^{-1} dm ], S = M M^T,
    so tr(...) = ||M_old^{-1} M_new||_F^2 and the quadratic term = ||M_old^{-1} dm||^2.
    Invariant under the tanh action map (a bijection), exactly as in the chassis's gaussian path.
    """
    # ONE batched triangular solve for both terms: M_new and the mean difference are stacked
    # into a single (B, d, d+1) right-hand side.
    rhs = torch.cat((new.dense_M(), (old.mean - new.mean).unsqueeze(-1)), dim=-1)
    w = old.solve_m_mat(rhs)
    x, dm = w[..., : new.d], w[..., new.d]                        # M_old^{-1} M_new ; M_old^{-1} dm
    quad = (x * x).sum((-2, -1)) + (dm * dm).sum(-1)
    return 0.5 * (quad - new.d) + old.log_det_m - new.log_det_m
...
class Args:
    ...
    # --- CORR-NOISE v1: the ONE new mechanism (see header). ---
    # ON: the actor readout becomes the correlated Gaussian (the chassis's gaussian mean +
    # log-variance heads, built with an IDENTICAL RNG prefix, plus the low-rank factor heads
    # and the per-dim AR(1) memory coefficient). This OVERRIDES actor_dist.
    # OFF: args.actor_dist is used verbatim and none of the new parameters are constructed, so
    # the file is the chassis bit-for-bit (no extra RNG draws at init or at rollout).
    corr_noise: bool = True
    corr_rank: int = 2               # rank k of the low-rank part of L(s); k=0 => L diagonal
    # ENTROPY HEADWIND ESCAPE HATCH. The conditional entropy is
    #   H = const + log|det L| + 0.5 * sum_i log(1 - rho_i^2),
    # strictly DECREASING in |rho_i|. In this chassis ent_coef is 0, so the pressure does not
    # come from an explicit entropy bonus -- it comes from the alpha*KL proximal term, because
    # KL(new||old) = -H(new) + E_new[-log pi_old]: penalizing +kl_coef*KL adds -kl_coef*H(new)
    # to the loss, i.e. rewards entropy at coefficient kl_coef (~0.35 measured). Since
    # dH/d raw_rho_i = -rho_i exactly, that channel is a LINEAR restoring force kl_coef*rho_i
    # pulling rho to 0 -- zero at rho = 0 (escape from init is not blocked) but growing with
    # |rho|. The entropy stays the true conditional entropy (it is the correct object; hacking
    # it would break the derivation). Instead: pin rho when the mechanism must be delivered at
    # full dose regardless of what the learning signal decides.
    # None => rho is LEARNED as specified. Set => rho is pinned to this value on EVERY action
    # dim and raw_rho is frozen (requires_grad False, so Adam and the actor clip skip it).
    corr_rho_fixed: Optional[float] = None
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # CORR-NOISE: the mechanism needs a location-scale family, so it OVERRIDES actor_dist
        # with the chassis's gaussian readout. With corr_noise off, args.actor_dist is used
        # verbatim and nothing below consumes any extra init RNG => chassis bit-for-bit.
        self.corr_noise = args.corr_noise
        self.corr_rank = args.corr_rank
        self.actor_dist = "gaussian" if args.corr_noise else args.actor_dist
        ...
        if self.corr_noise:
            # Built AFTER the chassis heads so the trunk/critic/mean/logvar init RNG prefix is
            # IDENTICAL to `--no-corr-noise --actor-dist gaussian` -- the matched control.
            # L = diag(sigma)(I + Uhat V^T): V starts at EXACTLY zero, so L = diag(sigma) at
            # init and the arm begins as the chassis's gaussian arm. Only ONE factor is zeroed:
            # dL/dV ~ Uhat != 0 lifts V off zero, which then unfreezes Uhat (dL/dUhat ~ V).
            self.actor_uhat_head = layer_init(nn.Linear(H, act_dim * args.corr_rank), std=0.01)
            self.actor_v_head = layer_init(nn.Linear(H, act_dim * args.corr_rank), std=0.01)
            with torch.no_grad():
                self.actor_v_head.weight.zero_()
                self.actor_v_head.bias.zero_()
            # rho = tanh(raw_rho): exactly 0 at init, always in (-1,1), one per action dim,
            # LEARNED by the ordinary PG + KL gradient. No schedule, no hand-tuning.
            self.raw_rho = nn.Parameter(torch.zeros(act_dim))
            self.corr_rho_fixed = args.corr_rho_fixed
            if self.corr_rho_fixed is not None:
                # PINNED dose. Same tanh parameterization (so the forward path is byte-identical
                # to the learned case), raw value set to atanh(target) and frozen. requires_grad
                # False => grad stays None => Adam and clip_grad_norm_ both skip it, and the
                # entropy headwind through the alpha*KL term cannot walk it back to 0.
                with torch.no_grad():
                    self.raw_rho.fill_(float(np.arctanh(self.corr_rho_fixed)))
                self.raw_rho.requires_grad_(False)

    def corr_rho(self):
        return torch.tanh(self.raw_rho)

    def _corr_actor_dist(self, actor_feat, z_prev, rho=None):
        # The chassis's gaussian mean + soft-bounded log-variance readout, verbatim, plus the
        # low-rank factors of L and the AR(1) coefficient.
        ...
        sigma = (0.5 * lv).exp()
        b, d, k = actor_feat.shape[0], mean.shape[-1], self.corr_rank
        uhat = self.actor_uhat_head(actor_feat).view(b, d, k)
        v = self.actor_v_head(actor_feat).view(b, d, k)
        if k > 0:
            # Smooth contraction to ||Uhat||_F ||V||_F < CORR_SPEC_CAP (see CORR_SPEC_CAP).
            # clamp_min keeps the n -> 0 limit finite AND its gradient alive: at n = 0 the
            # factor evaluates to 1.0 (not 0/0), so V's gradient is not killed at init.
            n = uhat.flatten(1).norm(dim=1) * v.flatten(1).norm(dim=1)
            n_safe = n.clamp_min(1e-6)
            scale = CORR_SPEC_CAP * torch.tanh(n_safe / CORR_SPEC_CAP) / n_safe
            v = v * scale.view(b, 1, 1)
        if rho is None:
            rho = self.corr_rho()
        return CorrGaussian(mean, sigma, uhat, v, rho, z_prev)
    ...
    def get_action_and_value(self, x, z=None, z_prev=None, rho=None):
        # z is the distribution-NATIVE sample (pre-tanh for gaussian/corr; in (0,1) for
        ...
        # z_prev (corr only) is the RECORDED AR(1) latent from before the step; it is the
        # conditioning variable of the exact conditional density and must be the same value
        # in the update pass as it was during the rollout. rho overrides the live rho (used
        # to rebuild the BEHAVIOR policy in the update pass).
        actor_feat, critic_feat = self._trunks(x)
        if self.corr_noise:
            dist = self._corr_actor_dist(actor_feat, z_prev, rho)
            if z is None:
                z, dist.z_sample = dist.sample()
            action = torch.tanh(z)
            # The tanh log-det is the chassis's gaussian-path Jacobian, kept verbatim so
            # -log pi is an action-space surprisal (the soft-MDP reward). It depends only on
            # the replayed sample, so it cancels EXACTLY in the PPO log-ratio.
            log_det = 2.0 * (log(2.0) - z - F.softplus(-2.0 * z))
            log_prob = dist.log_prob(z) - log_det.sum(1)
            value_logits = self.critic_head(critic_feat).view(
                -1, self.critic_mtp_horizon, self.num_bins
            )
            return action, z, log_prob, dist.entropy(), value_logits, dist
    ...
    def dist_params(self, dist):
        if self.corr_noise:
            # The corr conditional is fully described by (mu, sigma, Uhat, V) + (rho, z_prev);
            # rho is a policy parameter snapshotted once per rollout and z_prev comes from its
            # own rollout buffer, so only the four state-dependent tensors are stored here.
            # V is stored POST-contraction, so the rebuild is a pure reconstruction.
    ...
    def rebuild_corr_dist(self, mu, sigma, uhat, v, rho, z_prev):
        return CorrGaussian(mu, sigma, uhat, v, rho, z_prev)
    ...
    def actor_parameters(self):
        ...
        if self.corr_noise:
            heads = (
                ...
                + list(self.actor_logvar_head.parameters())
                + list(self.actor_uhat_head.parameters())
                + list(self.actor_v_head.parameters())
                # Excluded when pinned: torch.autograd.grad (the log_grad_split probe) REFUSES
                # an input that does not require grad, and a frozen param has nothing to clip.
                + ([self.raw_rho] if self.raw_rho.requires_grad else [])
            )
...
if __name__ == "__main__":
    ...
    # CORR-NOISE guards.
    assert args.corr_rank >= 0, "corr_rank must be >= 0 (0 = diagonal L, temporal memory only)"
    # auto_entropy's dual targets the SAMPLED squashed entropy and its `args.actor_dist ==
    # "gaussian"` gate would silently read False here (corr_noise overrides the readout, it does
    # not rewrite args), so refuse the combination outright rather than half-enable it.
    assert not (args.corr_noise and args.auto_entropy), \
        "--corr-noise is incompatible with --auto-entropy (its closed-form conditional entropy replaces the sampled squashed-entropy estimate the SAC dual assumes)"
    assert args.corr_rho_fixed is None or abs(args.corr_rho_fixed) < 1.0, \
        "corr_rho_fixed must lie strictly inside (-1, 1): |rho| = 1 makes the conditional density degenerate"
    if args.corr_rho_fixed is not None and not args.corr_noise:
        # Permitted and IGNORED, not an error: with the mechanism off there is no rho to pin and
        # no new parameter is constructed, so `--no-corr-noise --corr-rho-fixed X` is still the
        # chassis bit-for-bit (verified in the VERIFIED block). Kept permissive so a queued
        # control arm can carry the flag without a crash.
        print("NOTE: --corr-rho-fixed is ignored because --no-corr-noise is set (no rho exists).")
    ...
    # `actor_dist` in this table is the RAW arg. Under --corr-noise it is overridden by the
    # correlated Gaussian, so the effective readout is recorded as its own row -- reading
    # `actor_dist|beta` off a corr-noise run and concluding the actor is Beta is otherwise an
    # easy and expensive mistake.
    effective_actor = "corr_gaussian" if args.corr_noise else args.actor_dist
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s" % (
            "\n".join(
                [f"|{key}|{value}|" for key, value in vars(args).items()]
                + [f"|effective_actor_readout|{effective_actor}|"]
            )
        ),
    )
    ...
    if args.corr_noise:
        # CORR-NOISE rollout state. `latent_zs` already holds the NATIVE (pre-tanh) sample u_t;
        # these add the AR(1) conditioning latent z_{t-1} and the two low-rank factors of the
        # BEHAVIOR policy's L(s_t), so the update pass rebuilds the exact same conditional.
        corr_act_dim = int(np.prod(envs.single_action_space.shape))
        latent_prevs = torch.zeros((args.num_steps, args.num_envs, corr_act_dim)).to(device)
        dist_uhat = torch.zeros(
            (args.num_steps, args.num_envs, corr_act_dim, args.corr_rank)
        ).to(device)
        dist_v = torch.zeros(
            (args.num_steps, args.num_envs, corr_act_dim, args.corr_rank)
        ).to(device)
        # z is stationary N(0, I) under the recursion for ANY rho, so it is seeded from its
        # stationary law (see the reset comment in the rollout loop for why not zeros).
        z_carry = torch.randn(args.num_envs, corr_act_dim, device=device)
    ...
    # Accumulated on-device (one .item() per iteration, not per step: a per-step sync would
    # cost more than the mechanism itself).
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if args.corr_noise:
                # EPISODE-BOUNDARY RESET of the agent's own AR(1) memory, keyed on the chassis's
                # existing `next_done` = terminations | truncations of the PREVIOUS step (the
                # same signal `dones[step]` records). The latent is REPLACED by a fresh N(0, I)
                # draw, NOT zeroed: N(0, I) is the stationary law of z_t = rho*z + sqrt(1-rho^2)*xi,
                # so the MARGINAL action law N(mu, L L^T) -- and hence the entropy telemetry, the
                # -log pi_old soft-MDP reward scale and the noise magnitude -- is identical at
                # every step of an episode INCLUDING the first. Zeroing would make the first
                # post-reset step's noise only sqrt(1-rho^2) of its stationary scale and bias
                # exploration at exactly the moment the episode starts. Carrying z across the
                # boundary would instead leak the previous episode's momentum into a fresh state.
                z_carry = torch.where(
                    next_done.unsqueeze(1) > 0, torch.randn_like(z_carry), z_carry
                )
                latent_prevs[step] = z_carry
                z_reset_acc += next_done.sum()

            with torch.no_grad():
                action, z, logprob, ent, value_logits, dist = agent.get_action_and_value(
                    next_obs, z_prev=z_carry if args.corr_noise else None
                )
            ...
            dist_p2[step] = p2
            if args.corr_noise:
                z_carry = dist.z_sample
                dist_uhat[step] = dist.uhat
                dist_v[step] = dist.v
        ...
        if args.corr_noise:
            b_latent_prevs = latent_prevs.reshape(-1, corr_act_dim)
            # Explicit batch dim, not -1: at corr_rank 0 these tensors have zero elements and
            # `-1` is ambiguous against a zero-size dimension.
            b_dist_uhat = dist_uhat.reshape(args.batch_size, corr_act_dim, args.corr_rank)
            b_dist_v = dist_v.reshape(args.batch_size, corr_act_dim, args.corr_rank)
            # The BEHAVIOR rho. No optimizer step runs during a rollout, so the live rho is
            # still exactly the one that generated every sample in this buffer; snapshot it
            # once (it is (d,), state-independent) instead of buffering T*B copies.
            rho_old = agent.corr_rho().detach()
            rho_now = rho_old.cpu().numpy()
            # TEMPORAL-CORRELATION PROBE: lag-1 autocorrelation of the realized noise
            # component u_t - mu(s_t) = L(s_t) z_t, per action dim, over WITHIN-episode pairs
            # only (transition_boundaries[t] == 1 means step t+1 starts a fresh episode and a
            # fresh z). This is the direct measurement of whether the mechanism is doing
            # anything; it reads ~0 for white noise regardless of what rho claims.
            with torch.no_grad():
                noise = latent_zs - dist_p1                       # (T,B,d), dist_p1 == mu(s_t)
                pair_ok = (transition_boundaries[:-1] == 0).unsqueeze(-1).to(noise.dtype)
                n_pairs = pair_ok.sum().clamp_min(1.0)
                n0, n1 = noise[:-1], noise[1:]
                m0 = (n0 * pair_ok).sum((0, 1)) / n_pairs
                m1 = (n1 * pair_ok).sum((0, 1)) / n_pairs
                d0, d1 = n0 - m0, n1 - m1
                v0 = ((d0 * d0) * pair_ok).sum((0, 1)) / n_pairs
                v1 = ((d1 * d1) * pair_ok).sum((0, 1)) / n_pairs
                cov = ((d0 * d1) * pair_ok).sum((0, 1)) / n_pairs
                corr_lag1 = (cov / (v0.sqrt() * v1.sqrt() + 1e-12)).cpu().numpy()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, value_logits, new_dist = agent.get_action_and_value(
                    b_obs[mb_inds],
                    b_latent_zs[mb_inds],
                    z_prev=b_latent_prevs[mb_inds] if args.corr_noise else None,
                )
                ...
                if args.corr_noise:
                    # Exact all-action KL between the two CONDITIONALS at the SAME recorded
                    # z_prev, so it is the same object the chassis penalizes: analytic, no
                    # importance weight, invariant under the tanh action map.
                    exact_kl = corr_kl(
                        new_dist,
                        agent.rebuild_corr_dist(
                            b_dist_p1[mb_inds],
                            b_dist_p2[mb_inds],
                            b_dist_uhat[mb_inds],
                            b_dist_v[mb_inds],
                            rho_old,
                            b_latent_prevs[mb_inds],
                        ),
                    )
                else:
        ...
        if args.corr_noise:
            # CORR-NOISE telemetry. rho is read AFTER the update, so the trace shows learning;
            # `rho_now` (pre-update, the behavior policy) is what the lag-1 probe corresponds to.
            rho_post = agent.corr_rho().detach().cpu().numpy()
            writer.add_scalar("debug/corr_rho_absmax", float(np.abs(rho_post).max()), global_step)
            writer.add_scalar("debug/corr_rho_absmean", float(np.abs(rho_post).mean()), global_step)
            for i, r in enumerate(rho_post):
                writer.add_scalar(f"debug/corr_rho_{i}", float(r), global_step)
            writer.add_scalar("debug/corr_lag1_ac_mean", float(corr_lag1.mean()), global_step)
            writer.add_scalar("debug/corr_lag1_ac_absmax", float(np.abs(corr_lag1).max()), global_step)
            for i, a in enumerate(corr_lag1):
                writer.add_scalar(f"debug/corr_lag1_ac_{i}", float(a), global_step)
            # The nats-reallocation readout (P3): log|det L| (marginal) minus log|det M|
            # (conditional) == -0.5 * sum_i log(1 - rho_i^2) >= 0. Read off the BEHAVIOR rho so
            # it matches the rollout that was actually collected.
            writer.add_scalar(
                "debug/corr_marginal_minus_cond_logdet",
                float(-0.5 * np.log(np.clip(1.0 - rho_now ** 2, 1e-8, None)).sum()),
                global_step,
            )
            with torch.no_grad():
                writer.add_scalar("debug/corr_logdet_L_mean", float(new_dist.log_det_l.mean()), global_step)
                writer.add_scalar("debug/corr_logdet_M_mean", float(new_dist.log_det_m.mean()), global_step)
                # log|det(I_k + V^T Uhat)|: 0 => the low-rank part of L is still inert.
                writer.add_scalar(
                    "debug/corr_lowrank_logdet_absmean",
                    float(new_dist.log_absdet_cap.abs().mean()),
                    global_step,
                )
                writer.add_scalar("debug/corr_sigma_mean", float(new_dist.sigma.mean()), global_step)
            writer.add_scalar(
                "debug/corr_z_reset_frac",
                float(z_reset_acc.item()) / float(args.num_steps * args.num_envs),
                global_step,
            )
