# PPO + action-logprob DG gate clip-hi no-advnorm HL-Gauss critic v7.
#
...
# - Gate raw PPO advantages with the paper's continuous-action DG gate in
#   environment action coordinates:
#     ell = clamp(-old_logprob_z + log|da/dz|, -beta_nll_clip, beta_nll_clip)
#     gate = sigmoid(gate_coef * A * ell)
#   The log-Jacobian is not used in PPO ratios because it cancels.
...
# Using action-space log-density surprisal should be the closest bounded-Beta
# analogue to the Gaussian action-density gate used by DG.
...
def logprob_to_gate(advantages, logprob, clip, gate_coef, logdet=0.0):
    clipped_surprisal = (-logprob + logdet).clamp(-clip, clip)
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        ...
        action_width = self.action_high - self.action_low
        self.register_buffer("action_logdet", torch.log(action_width).sum())
    ...
    def logprob_gate(self, advantages, logprob, clip, gate_coef):
        return logprob_to_gate(advantages, logprob, clip, gate_coef, self.action_logdet)
...
if __name__ == "__main__":
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("ppo_continuous_action_beta_nll_hlgauss_actionlogp_cliphi_noadvnorm_v7 requires CUDA")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                raw_surprisal, beta_nll_gate = agent.logprob_gate(
                    mb_advantages,
                    b_logprobs[mb_inds],
                    args.beta_nll_clip,
        ...
        writer.add_scalar("losses/action_surprisal_mean", np.mean(raw_surprisal_means), global_step)
        writer.add_scalar("losses/action_surprisal_std", np.mean(raw_surprisal_stds), global_step)
        ...
        writer.add_scalar("losses/action_surprisal_gate_arg_mean", np.mean(raw_surprisal_gate_arg_means), global_step)
        writer.add_scalar("losses/action_surprisal_gate_arg_std", np.mean(raw_surprisal_gate_arg_stds), global_step)
        ...
        writer.add_scalar("losses/action_surprisal_gate_p05", np.mean(raw_surprisal_gate_p05s), global_step)
        writer.add_scalar("losses/action_surprisal_gate_p50", np.mean(raw_surprisal_gate_p50s), global_step)
        writer.add_scalar("losses/action_surprisal_gate_p95", np.mean(raw_surprisal_gate_p95s), global_step)
        ...
        writer.add_scalar("losses/action_logdet", agent.action_logdet.item(), global_step)
