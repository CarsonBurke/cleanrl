# PPO + paper-shaped Beta-NLL gate clip-hi no-advnorm HL-Gauss critic v5.
#
...
# - Gate raw PPO advantages with a DG-shaped sigmoid gate:
#     gate = sigmoid(gate_coef * A * clipped_beta_nll)
...
# A paper-shaped bounded surprisal gate should preserve useful rare successes
# and suppress rare failures without adding non-paper advantage z-scoring.
...
def beta_nll_to_gate(advantages, beta_nll, clip, gate_coef):
    ...
    gate = torch.sigmoid(gate_coef * advantages * beta_nll)
    return beta_nll, gate.detach()
...
if __name__ == "__main__":
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("ppo_continuous_action_beta_nll_hlgauss_paperdg_cliphi_noadvnorm_v5 requires CUDA")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                beta_nll, beta_nll_gate = beta_nll_to_gate(
                    mb_advantages,
                    b_beta_nlls[mb_inds],
                    args.beta_nll_clip,
