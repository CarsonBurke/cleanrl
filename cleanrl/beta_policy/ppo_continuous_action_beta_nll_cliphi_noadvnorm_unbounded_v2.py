# PPO + Beta-NLL clip-hi no-advnorm unbounded-weights v2.
#
...
# - Weight raw PPO advantages by detached, peak-referenced Beta NLL:
#     log pi(mode) - log pi(z) >= 0
#   rather than raw -log pi(z), which can invert sign semantics once a Beta policy
...
# while peak-referenced NLL allocates more actor gradient to surprising sampled
# actions without changing the PPO trust-ratio mechanics.
import os
import random
import time
...
class Args:
    ...
    beta_nll_weight_min: float = 0.0
    """minimum detached beta-NLL advantage weight"""
    beta_nll_weight_max: float = 1e9
    """maximum detached beta-NLL advantage weight"""
...
def beta_nll_to_weights(beta_nll, clip, weight_min, weight_max):
...
class Agent(nn.Module):
    ...
    def beta_nll_weights(self, dist, z, clip, weight_min, weight_max):
        ...
        return beta_nll_to_weights(beta_nll, clip, weight_min, weight_max)
    ...
    def get_beta_action_and_value(self, x, z=None):
        ...
        return action, z, logprob, dist.entropy().sum(1), self.critic(x), beta_nll

    def get_action_and_value(self, x, action=None):
        z = None if action is None else self._action_to_z(action)
        action, _, logprob, entropy, value, _ = self.get_beta_action_and_value(x, z)
...
if __name__ == "__main__":
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("ppo_continuous_action_beta_nll_cliphi_noadvnorm_unbounded_v2 requires CUDA")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, action_z, logprob, _, value, beta_nll = agent.get_beta_action_and_value(next_obs)
        ...
        clipfracs = []
        beta_nll_means = []
        beta_nll_weight_means = []
        beta_nll_weight_stds = []
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, _, newlogprob, entropy, newvalue, _ = agent.get_beta_action_and_value(
                    b_obs[mb_inds],
                    b_action_zs[mb_inds],
                )
                ...
                beta_nll, beta_nll_weights = beta_nll_to_weights(
                    b_beta_nlls[mb_inds],
                    args.beta_nll_clip,
                    args.beta_nll_weight_min,
                    args.beta_nll_weight_max,
                )
                ...
                with torch.no_grad():
                    ...
                    beta_nll_weight_means.append(beta_nll_weights.mean().item())
                    beta_nll_weight_stds.append(beta_nll_weights.std(unbiased=False).item())
                ...
                mb_advantages = mb_advantages * beta_nll_weights.detach()
        ...
        writer.add_scalar("losses/beta_nll_weight_mean", np.mean(beta_nll_weight_means), global_step)
        writer.add_scalar("losses/beta_nll_weight_std", np.mean(beta_nll_weight_stds), global_step)
