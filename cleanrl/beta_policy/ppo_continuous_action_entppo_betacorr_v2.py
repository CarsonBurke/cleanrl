...
class Args:
    ...
    # ANNEAL the perturbation dose to 0 over this FRACTION of total training, linearly, then hold
    # 0. None => no annealing (v1 behaviour, bitwise). v1 measured WHY this is needed: a FIXED dose
    # helps while the policy is coarse (g=0.5 was +9.3% @2M, +6.1% @3M) but then acts as an
    # irreducible noise floor that blocks late convergence -- its approx_kl stalled at 0.0665 @8M
    # against the chassis's 0.0061, tr_mask_frac 0.0168 vs 0.0039, and its return tail std was
    # 1568 vs 561 while REACHING the same peak (max 10755 vs 10696). The dose must retire itself.
    corr_gain_anneal_frac: Optional[float] = None
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # Anneal multiplier on g, driven from the training loop. Exactly 1.0 unless
        # --corr-gain-anneal-frac is set, so the no-anneal path is bitwise v1.
        self.corr_gain_scale = 1.0
        if self.beta_corr:
    ...
    def corr_gain(self):
        ...
        # corr_gain_scale is exactly 1.0 with annealing off, so this is bitwise v1 there.
        g = self.corr_gain_max * torch.tanh(self.raw_gain)
        if self.corr_gain_scale != 1.0:
            g = g * self.corr_gain_scale
        return g
...
if __name__ == "__main__":
    ...
    if args.beta_corr:
        ...
        assert args.corr_gain_anneal_frac is None or 0.0 < args.corr_gain_anneal_frac <= 1.0, \
            "corr_gain_anneal_frac must lie in (0, 1]"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.beta_corr:
            z_reset_acc.zero_()
            if args.corr_gain_anneal_frac is not None:
                # Linear dose retirement: 1 -> 0 across the first corr_gain_anneal_frac of
                # training, then exactly 0. Uses (iteration-1) so the first iteration runs at
                # full dose, matching the un-annealed arm's opening step.
                _prog = (iteration - 1.0) / max(
                    1.0, args.corr_gain_anneal_frac * args.num_iterations
                )
                agent.corr_gain_scale = float(max(0.0, 1.0 - _prog))
                writer.add_scalar(
                    "debug/corr_gain_scale", agent.corr_gain_scale, global_step
                )
