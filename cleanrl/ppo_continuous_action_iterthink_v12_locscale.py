# PPO + IterThink v12 (LOCATION-SCALE distributional critic). From v10/v11.
#
# THE MEAN PROBLEM. v10's distributional critic wins late (EV 2-5x, peak 6062 vs
# v7's 5480, baseline 4710) but LOSES early (-33% @500k). Root cause (two
# subagents, independently): a fixed-support categorical critic represents the
# value as the MEAN of a probability distribution over fixed bins, so moving the
# mean requires TRANSPORTING probability mass across near-empty bins. The CE
# gradient through that corridor is ~10x weaker than a scalar/flat head's, so the
# mean tracks a fast-rising (non-stationary) early return target slowly -> laggy
# advantages -> early policy deficit. The mean is an emergent, slow property.
#
# v11 annealed in a scalar MSE term -- a SCHEDULE that papers over the lag, not a
# fix. v12 makes a real structural change: STOP MAKING THE MEAN EMERGENT.
# Decouple location from shape (location-scale parameterization of the value
# distribution):
#
#   value(s) = mu(s)            <- scalar head, regressed by MSE: the mean tracks
#                                  at scalar-regression speed, PERMANENTLY. No mass
#                                  transport, no schedule. Drives GAE / advantage /
#                                  bootstrap -- so the CONTROL path is plain PPO on
#                                  a fast scalar value and advantages never lag.
#   shape(s) ~ residual R = G^lambda - mu   <- categorical head over a TIGHT
#                                  relative support c, an AUXILIARY distributional
#                                  model of the return residual sharing the critic
#                                  trunk. This is the representation-shaping channel
#                                  behind v10's late EV win, now WITHOUT gating the
#                                  value readout behind slow mass transport.
#
# THE ELEGANT PART. The shape target is built by the distributional lambda-recursion
# run in the location-relative frame: the per-step affine shift becomes exactly the
# location TD-error delta_t = r_t + gamma*mu(s_{t+1}) - mu(s_t) (the delta GAE
# already computes), applied to the residual distribution:
#   R_t =_D delta_t + gamma*nonterm*[ (1-lambda)*shape(s_{t+1}) + lambda*R_{t+1} ].
# delta is small/bounded so a tight relative support is adequate throughout (no
# wide-support tuning, no v9 blowup). This is distributional RL ON THE RESIDUAL:
# the shape's mean tracks the advantage and vanishes at convergence, but it is a
# pure auxiliary -- it shapes the shared critic representation and CANNOT
# destabilise PPO, since control uses only the fast scalar mu.
#
# Peaked init kept (on c) for a sharp initial residual. The deliberately-omitted
# next step (v13, if this wins): fold the shape mean back into the value
# (value = mu + E[shape]) to USE the distribution in control. Here we first
# isolate the structural mean fix. Control: iterthink_v10_distreturn / v11.
import os
import random
import time
...
class Args:
    ...
    # Location-scale critic. mu = scalar value head (fast mean). shape = categorical
    # over a TIGHT relative support c=[-rel_range, rel_range] modelling the return
    # residual R = G^lambda - mu. delta TD-errors are small so this stays in-range.
    num_bins: int = 511
    rel_range: float = 5.0
    critic_init_tau: float = 0.5   # init residual shape ~ N(0, tau^2): sharp at 0
    shape_coef: float = 1.0        # weight of the distributional shape CE loss
...
def distributional_residual_returns(
    ...
    where delta_t = r_t + γ·nonterm·mu(s_{t+1}) - mu(s_t) is the location TD-error
    (== GAE delta, since value == mu). The residual mean tracks the advantage and
    vanishes at convergence; this is an AUXILIARY distributional target and does
    not feed control. Shapes: deltas/dones (T, B); shape_probs (T, B, n);
    bootstrap_shape (B, n) = residual shape of Z(s_T). Returns (T, B, n).
    """
    T = deltas.shape[0]
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Scalar GAE on mu (the fast location). Drives advantage AND yields the
            # per-step location TD-errors delta_t reused by the residual recursion.
            ...
            # Distributional λ-return RESIDUAL target (mean-matches `advantages`).
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # LOCATION loss: direct scalar MSE on mu -> fast, lag-free mean.
        ...
        # Residual-support adequacy: edge-bin mass of the shape target should stay ≈ 0.
