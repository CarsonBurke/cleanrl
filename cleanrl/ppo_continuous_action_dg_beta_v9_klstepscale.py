# ============================================================================
# DG v9 -- v8's KL ceiling, but as a STEP SCALE instead of a HARD STOP (use all the data).
#
# WHY (v8's cost). v8 bounds the per-update MAX KL by checking the exact analytic KL every
# minibatch and BREAKING the actor pass the moment it crosses kl_target*kl_cap_ratio. That works
# (max KL fell from ~1.07 to ~0.10), but it pays in sample efficiency: on the ~27% of iterations
# that fire, the actor STOPS and DISCARDS every remaining minibatch of the pass (sometimes after
# only 6 of 32 steps). The early minibatches get full-strength steps; the late ones get nothing --
# a data-ordering bias, and the discarded gradient signal is forgone for that whole rollout.
#
# FIX. Same exact-KL ceiling, but instead of a binary stop, SCALE each minibatch's policy-gradient
# step by a factor that ramps linearly from 1 at kl_target (no damping in the normal range) down
# to 0 at the ceiling kl_cap = kl_target*kl_cap_ratio:
#     scale = clamp( (kl_cap - KL) / (kl_cap - kl_target), 0, 1 )          # detached scalar
#     pg_loss <- scale * pg_loss
# Every minibatch is still processed -- NO data discarded, no ordering bias -- but as the policy
# nears the trust-region boundary its steps shrink to zero, so it cannot overshoot. Crucially the
# soft KL penalty (kl_beta * KL) stays FULL-STRENGTH and UNSCALED, so at/past the boundary the only
# live gradient is the penalty PULLING BACK toward pi_old: the update is self-correcting, KL can
# DECREASE on later minibatches, rather than terminally frozen. This is a smooth trust region
# (continuous throttle) vs v8's hard reject -- it spends the full KL budget across ALL the data
# instead of front-loading it onto the first few minibatches and throwing the rest away.
# HYPOTHESIS: bounds the max KL as well as v8 while using every minibatch -> better sample
# efficiency / higher return at equal KL safety. (--no-kl-step-scale falls back to v8's hard cap.)
# ----------------------------------------------------------------------------
...
class Args:
    ...
    kl_cap_ratio: float = 3.0        # KL ceiling = kl_target*kl_cap_ratio. v9: the step scale hits 0
    #                                  here (v8: hard stop here). 0 disables. Bounds per-update MAX KL.
    kl_step_scale: bool = True       # v9: scale each minibatch's pg step by (kl_cap-KL)/(kl_cap-kl_target)
    #                                  clamped to [0,1] -- a smooth trust region that uses ALL minibatches.
    #                                  False -> v8's hard per-minibatch stop (discards the rest of the pass).
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ---- Actor update: DG-gated score + ANALYTIC-KL penalty + HARD per-minibatch KL cap ----
        gate_means, surp_means, chi_stds, clipfracs, kl_terms = [], [], [], [], []
        scale_terms = []  # v9: per-minibatch policy-gradient step scale in [0,1]
        ...
        for epoch in range(args.actor_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # KL ceiling enforcement. v9 (kl_step_scale): SMOOTH -- scale this minibatch's
                # policy-gradient step down as the exact KL approaches the ceiling, ramping from 1
                # at kl_target to 0 at kl_cap. No minibatch is discarded; the soft penalty below
                # stays full-strength and pulls back. v8 fallback: HARD stop at the ceiling.
