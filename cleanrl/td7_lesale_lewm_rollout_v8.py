# TD7-LeSALE LeWM rollout v8 — multi-step private world model with a control gradient firewall.
#
# StockSIG remains the actor/critic interface. A private LayerNorm world-model branch shares only
# the early state trunk, predicts three recurrent action-conditioned steps, uses full SIGReg and
# JEDI, and adaptively caps its combined shared-trunk gradient at 0.35x the Stock SALE gradient.
import runpy
import sys
...
_enable_unless_overridden("--lewm-rollout-aux")
...
if not _has_option("--exp-name"):
    sys.argv.extend(["--exp-name", "td7_lesale_lewm_rollout_v8"])
