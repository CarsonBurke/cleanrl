# PPO + DepthMix v1: depth-mixing residuals on a PLAIN MLP trunk.
#
# METHOD. A plain pre-LN MLP trunk where each block, instead of taking only the
# previous block's output, MIXES over ALL previous layer outputs ("the bank")
# before its body. The body is a single plain MLP (Linear->ReLU^2->Linear); there
# is NO mixture-of-experts. Three reach-back MIX modes (--reach-mode):
#   "seq"  : MIX = bank[-1]                          (plain sequential residual; control)
#   "dwa"  : MIX = sum_j softmax(alpha_i)[j]*bank[j] (DenseFormer depth-weighted add;
#            static, input-independent convex combo of all previous outputs)
#   "attn" : per-head depth attention. Query from the running state bank[-1], keys/
#            values from ALL bank entries; softmax over depth gives an INPUT-DEPENDENT
#            convex combination per head. MIX = Wo(concat_heads(ctx)).
# Block: h = mixed + body(LN(mixed)); body = Linear(H, m*H)->ReLU^2->Linear(m*H, H).
# Trunk output = final_ln(bank[-1]); output dim = H. Same trunk class builds two
# independent instances (separate actor/critic trunks), wired into the unchanged
# Beta actor / HL-Gauss symlog distributional critic heads.
#
# NOVELTY. DenseFormer ("dwa") is the static-scalar special case of depth mixing;
# "attn" is its input-dependent generalization (the convex weights over previous
# layers become a function of the current state, per attention head). "seq" is the
# degenerate control (reach back = 1). This isolates the depth-mixing residual on a
# plain MLP, with NO MoE, so the lever under test is the reach-back mechanism alone.
#
# HYPOTHESIS. On dense low-dimensional proprioceptive states, an input-dependent
# reach-back over a deep plain MLP improves credit assignment and feature reuse
# (a block can re-read any earlier representation, gated by the current state),
# outperforming the sequential residual and the static DenseFormer add.
#
...
# all hyperparameters are inherited byte-for-byte from the v24 beta symlog base;
...
class DepthMixTrunk(nn.Module):
    ...
    Each block mixes over ALL previous layer outputs (the "bank") via one of:
      "seq" : plain sequential residual (reach back = 1).
      "dwa" : DenseFormer static depth-weighted add (input-independent softmax
              convex combination of all previous outputs).
      "attn": per-head depth attention (input-dependent convex combination over
              previous layers; query from the running state, keys/values from all).
    Then a plain pre-LN MLP block: h = mixed + body(LN(mixed)). Output = final_ln(h).
    Output dim = H.
    """
    ...
    def __init__(self, in_dim, H, n_blocks, reach_mode, n_heads, d_qk, mlp_mult):
        ...
        self.lns = nn.ModuleList()       # pre-LN before each block body
        self.bodies = nn.ModuleList()    # plain MLP body per block
    ...
    def _mix(self, i, bank):
        ...
        B = bank[-1].shape[0]
        L = len(bank)
        nh, dqk, H = self.n_heads, self.d_qk, bank[-1].shape[-1]
        dv = H // nh
        q = self.Wq[i](bank[-1]).view(B, nh, dqk)                    # (B, nh, dqk)
        stack = torch.stack(bank, dim=1)                             # (B, L, H)
        K = self.Wk[i](stack).view(B, L, nh, dqk)                   # (B, L, nh, dqk)
        V = self.Wv[i](stack).view(B, L, nh, dv)                   # (B, L, nh, dv)
        # scores over depth j: (B, nh, L)
        scores = torch.einsum("bhd,blhd->bhl", q, K) / (dqk ** 0.5)
        w = torch.softmax(scores, dim=-1)                           # (B, nh, L)
        ctx = torch.einsum("bhl,blhd->bhd", w, V)                   # (B, nh, dv)
        ctx = ctx.reshape(B, H)                                     # concat heads
        return self.Wo[i](ctx)                                       # (B, H)
