# PPO + Beta-NLL gate clip-hi no-advnorm HL-Gauss critic v4.
#
...
# - Gate raw PPO advantages by detached, peak-referenced Beta NLL and the sign
#   of the advantage:
#     gate = 2 * sigmoid(gate_coef * zscore(A) * normalized_beta_nll)
#   This keeps neutral/common samples near 1, boosts rare successes, and
#   suppresses rare failures without unbounded actor weights.
...
# A sign-aware, bounded surprisal gate should preserve useful rare successes
# without the KL pressure caused by the previous sign-blind NLL multiplier.
...
class Args:
    ...
    """clip for peak-referenced beta NLL advantage weights"""
    beta_nll_gate_coef: float = 1.0
    """scale for the sign-aware beta-NLL advantage gate"""
    beta_nll_gate_adv_clip: float = 5.0
    """clip for minibatch advantage z-scores used only inside the gate"""
    beta_nll_gate_surprisal_clip: float = 5.0
    """clip for mean-normalized beta-NLL used only inside the gate"""
    num_bins: int = 511
...
def beta_nll_to_gate(advantages, beta_nll, clip, gate_coef, adv_clip, surprisal_clip):
    ...
    surprisal = torch.where(
        ...
        torch.zeros_like(beta_nll),
    ).clamp(0.0, surprisal_clip)
    gate = 2.0 * torch.sigmoid(gate_coef * adv_z * surprisal)
    return beta_nll, gate.detach(), adv_z.detach(), surprisal.detach()
...
class Agent(nn.Module):
    ...
    def beta_nll_gate(self, advantages, beta_nll, clip, gate_coef, adv_clip, surprisal_clip):
        return beta_nll_to_gate(advantages, beta_nll, clip, gate_coef, adv_clip, surprisal_clip)
...
if __name__ == "__main__":
    ...
    if not args.cuda or not torch.cuda.is_available():
        raise RuntimeError("ppo_continuous_action_beta_nll_hlgauss_gate_cliphi_noadvnorm_v4 requires CUDA")
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        beta_nll_gate_means = []
        beta_nll_gate_stds = []
        beta_nll_gate_adv_z_means = []
        beta_nll_gate_surprisal_means = []
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                beta_nll, beta_nll_gate, beta_nll_gate_adv_z, beta_nll_gate_surprisal = beta_nll_to_gate(
                    mb_advantages,
                    b_beta_nlls[mb_inds],
                    args.beta_nll_clip,
                    args.beta_nll_gate_coef,
                    args.beta_nll_gate_adv_clip,
                    args.beta_nll_gate_surprisal_clip,
                )
                ...
                with torch.no_grad():
                    ...
                    beta_nll_gate_adv_z_means.append(beta_nll_gate_adv_z.mean().item())
                    beta_nll_gate_surprisal_means.append(beta_nll_gate_surprisal.mean().item())
        ...
        writer.add_scalar("losses/beta_nll_gate_adv_z_mean", np.mean(beta_nll_gate_adv_z_means), global_step)
        writer.add_scalar("losses/beta_nll_gate_surprisal_mean", np.mean(beta_nll_gate_surprisal_means), global_step)
