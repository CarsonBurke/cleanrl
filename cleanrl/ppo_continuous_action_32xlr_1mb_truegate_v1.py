# 32x LR + 1 minibatch + truegate delight gate (v9/v10, arXiv:2603.14608 Alg. 2).
# w = sigmoid(U * l / eta) with l = clip(-true log-density, -C, C), eta=1, C=10.
# Stored log-prob already includes the action-range Jacobian, so it is the
# physical density; do not subtract logdet again. Gate raw GAE, then PPO.
import os
import random
import time
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; truegate w=sigmoid(U*l/eta) on physical log-density")
