# MSOU-gSDE (Sticky): Multi-Scale OU gSDE with Continuous (Non-Reset) Noise State
#
# Major iteration hypothesis:
# - The MSOU-gSDE v1 script resamples exploration matrices at every rollout
#   start. For balance-sensitive tasks (Hopper/Walker), these large "exploration
#   state jumps" can repeatedly destabilize the policy and cap returns.
# - Keeping a continuous OU process across rollouts (and only resetting per-env
#   when an episode ends) should reduce catastrophic exploration bursts while
#   preserving rich time correlation.
#
# This keeps the same marginal gSDE log_prob used by the lineage, but changes
# the *state management* of the OU process.
import math
import os
import random
import time
...
class Args:
    ...
    """if True, resample exploration matrices at rollout start (v1 behavior)"""
...
class MSOUGSDEDistribution:
    ...
    def effective_matrix(self, weights):
        # weights: (B,K) -> normalize to preserve marginal variance
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None, update_noise=True):
        ...
        if action is None:
            if update_noise:
                self.gsde.update_weights(self.log_std, self.rhos, batch_size=weights.shape[0])
...
if __name__ == "__main__":
    ...
    # Initialize exploration state once, keep it "sticky" across rollouts.
