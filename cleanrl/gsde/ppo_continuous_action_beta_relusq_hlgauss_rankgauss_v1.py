# PPO + ReLUSq + Beta + HL-Gauss critic + rankgauss advantages v1
#
# Purpose:
# This is the HL-Gauss row of a 2x2 ablation {scalar, HL-Gauss critic} x
# {raw, rankgauss advantages}. It is forked from beta_relusq_rankgauss_v1 and
# changes EXACTLY ONE factor vs that file: the scalar MSE critic is replaced by
# a categorical HL-Gauss distributional critic (num_bins logits, value read out
# as E[z] = sum(softmax(logits) * support), trained by cross-entropy against a
# Gaussian-smoothed two-hot projection of the returns). Everything else -- Beta
# actor, ReLUSq MLP, all hyperparameters, and the rankgauss advantage transform
# -- is identical.
#
# Question being answered:
# Our iterthink_v19 attribution found that bounded-advantage shaping (rankgauss),
# not the distributional critic, drove the win. rankgauss is rank-only and so is
# largely insensitive to value *magnitude*/calibration, which is most of what an
# HL-Gauss critic improves. This run measures whether swapping in the HL-Gauss
# critic changes how much rankgauss helps on the Beta/ReLUSq base.
#
# adv_transform="raw" recovers the HL-Gauss + raw cell (the matched baseline for
# this row); adv_transform="rankgauss" is the HL-Gauss + rankgauss cell.
#
# Novelty in this repo:
# - rankgauss advantage transform combined with an HL-Gauss critic on a Beta actor
# - ReLUSq hidden activations in both actor and critic
import os
import random
import time
...
class Args:
    ...
    """advantage transform applied to GAE before the surrogate: "raw" | "rankgauss" """
    cdf_probit_clamp: float = 0.999
    """clamp on the centered quantile (2u-1) before erfinv, to bound the tails"""
    num_bins: int = 51
    """number of bins in the HL-Gauss categorical value support"""
    v_min: float = -10.0
    """minimum value of the HL-Gauss support"""
    v_max: float = 10.0
    """maximum value of the HL-Gauss support"""
...
class HLGaussSupport:
    """Discretized support with HL-Gauss projection for a categorical value head.

    Inlined (verbatim, non-symlog path) from cleanrl/shared/hl_gauss.py to keep
    this a self-contained single file. Value read out as E[z] = sum(softmax(logits)
    * support); scalar targets projected onto bins via a Gaussian kernel.
    """
    ...
    def project(self, targets):
        targets = targets.clamp(self.v_min, self.v_max).unsqueeze(-1)
...
class Agent(nn.Module):
    ...
    def get_action_distribution(self, x):
        head_alpha, head_beta = self.actor(x).chunk(2, dim=-1)
    ...
    def get_action_and_value(self, x, hl_support, action=None):
        ...
        return action, probs.log_prob(z).sum(1), probs.entropy().sum(1), value
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.adv_transform == "rankgauss":
            # rank -> uniform quantile -> probit Gaussian (erfinv on CPU; GPU erfinv
            # is broken on this box). Self-normalizing, outlier-immune, magnitude-free.
            gae = b_advantages
            n = gae.numel()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.clip_vloss:
                    # Clip expected value, then build a clipped target for HL-Gauss
                    # projection; take the pessimistic (higher-loss) of the two.
