# PMPO-D4 + normadv + LeakyReluSq + HL-Gauss critic v1.
#
# Stacks three changes on the PMPO-D4 actor:
#   - pmpo_norm_adv (default True): PPO-style per-minibatch advantage
#     normalization before the PMPO bucket-split / tanh weighting.
#   - LeakyReluSq hidden activation: f(x) = (0.5*x + 0.5*relu(x))^2 replaces
#     all hidden Tanh in the actor and critic.
#   - HL-Gauss critic from pmpo_d4_hlgauss_v2: distributional value head, GAE
#     uses expected scalar from the categorical, value loss is cross-entropy
#     against Gaussian-projected returns.
# Otherwise PMPO actor (squashed actions, bucket-balanced sign-split + tanh
# magnitude weighting, reverse KL).
...
class Args:
    ...
    """kept for CLI parity; HL-Gauss trains the critic with projected-return cross-entropy"""
    ent_coef: float = 0.0
    ...
    pmpo_norm_adv: bool = True
    """normalize advantage per-minibatch before PMPO weighting (PPO-style)"""
    ...
    """apply symlog/symexp around HL-Gauss value targets and expectations"""
    d4_value_support: bool = True
    """use Dreamer4-style edge bins and no target clamping for the value support"""
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins=51):
        super().__init__()
    ...
    def get_action_and_value(self, x, hl_support, action=None):
        ...
        if action is None:
            action = probs.sample()
        log_prob_per_dim = probs.log_prob(action)
        entropy_per_dim = probs.entropy()
        return action, log_prob_per_dim.sum(1), entropy_per_dim.sum(1), self.get_value(x, hl_support)
    ...
    def get_squashed_action_and_value(self, x, hl_support, action=None):
...
def evaluate_policy(
    ...
    hl_support = HLGaussSupport(
        ...
        sigma_ratio,
        device,
        use_symlog=value_symlog,
        support_is_edges=d4_value_support,
        clamp_targets=not d4_value_support,
    )
    ...
    while len(episodic_returns) < eval_episodes:
        with torch.no_grad():
            ...
            if policy_objective == "pmpo":
                actions, _, _, _, _, _ = agent.get_squashed_action_and_value(obs_tensor, hl_support)
            else:
                actions, _, _, _ = agent.get_action_and_value(obs_tensor, hl_support)
...
if __name__ == "__main__":
    ...
    hl_support = HLGaussSupport(
        ...
        support_is_edges=args.d4_value_support,
        clamp_targets=not args.d4_value_support,
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                if args.policy_objective == "pmpo":
                    action, logprob, _, value, action_dist, _ = agent.get_squashed_action_and_value(next_obs, hl_support)
                else:
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                value_logits = agent.get_value_logits(b_obs[mb_inds])
        ...
        writer.add_scalar("losses/value_pred_mean", newvalue.mean().item(), global_step)
        writer.add_scalar("losses/return_mean", b_returns.mean().item(), global_step)
    ...
    if args.save_model:
        ...
        episodic_returns = evaluate_policy(
            ...
            value_symlog=args.value_symlog,
            d4_value_support=args.d4_value_support,
        )
