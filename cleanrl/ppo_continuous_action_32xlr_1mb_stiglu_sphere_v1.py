# 32x LR + 1 minibatch with a hypersphered SiTU-GLU trunk (sTiGLU) instead
...
# unchanged. The trunk ports nGPT (arXiv:2412.13587) to the dense layout:
# every stream lives on the unit hypersphere, each stage mixes unit-normed
# branch outputs into the stream as A + g*(B - A) steps with learned
# per-channel gates (init sigmoid(-1.5) ~= 0.18), then re-projects. Justnorm
# only -- no RMSNorm or affine norms. Normalizing branch outputs before
# mixing bounds every contribution no matter how hot SiTU runs, so the KL
# blowup that killed denseres cannot recur by construction. Hypothesis:
# direction-only learning on the sphere is the right geometry for
# multiplicative gating at 32x LR.
import os
import random
import time
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); FP32; hypersphered SiTU-GLU trunk; native-action storage; host actor mirror")
