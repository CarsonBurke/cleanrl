# SiTU factorized residual amplitude, v7: preserve the winning pre-RMS graph.
# Replace redundant sigmoid(g)*Wdown rows by softplus(g)*unit_rows(Wdown).
# Reuse existing gates as amplitudes; match the initial effective matrices.
# Same function class, parameter/layer counts, LR and PPO schedule. Hypothesis:
# separating branch magnitude from direction avoids coupled Adam amplification.
# Actor-only by default; exact post-update Beta KL measures actual policy jumps.
import os
import random
import time
...
class Args:
    ...
    actor_factorized: bool = True
    """separate actor residual amplitude from down-projection direction"""
    critic_factorized: bool = False
    """optional independent critic reparameterization; unchanged by default"""
...
def make_trunk(observation_dim, factorized):
    if factorized:
        return make_situ_factorized_trunk(observation_dim, 64)
    return make_norm_residual_trunk(observation_dim, 64, placement="pre",
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, actor_factorized=True, critic_factorized=False):
        super().__init__()
        ...
        self.critic = nn.Sequential(
            make_trunk(observation_dim, critic_factorized),
            layer_init(nn.Linear(64, 1), std=1.0),
        )
        self.actor = nn.Sequential(
            make_trunk(observation_dim, actor_factorized),
...
def beta_forward_kl(old_alpha, old_beta, alpha, beta):
    """Analytic KL(old || new), FP64 to avoid concentrated-Beta cancellation."""
    old_alpha, old_beta = old_alpha.double(), old_beta.double()
    ...
    return (torch.lgamma(alpha) + torch.lgamma(beta) - torch.lgamma(new_sum)
            ...
            + (new_sum - old_sum) * torch.digamma(old_sum)).sum(-1)


def policy_geometry(actor, observations, old_alpha, old_beta):
    """Post-update policy movement; mean/concentration interventions are not additive."""
    alpha, beta = (F.softplus(actor(observations)) + 1.0).double().chunk(2, dim=-1)
    old_alpha, old_beta = old_alpha.double(), old_beta.double()
    old_concentration, concentration = old_alpha + old_beta, alpha + beta
    old_mean, mean = old_alpha / old_concentration, alpha / concentration
    old_variance = old_mean * (1 - old_mean) / (old_concentration + 1)
    kl = beta_forward_kl(old_alpha, old_beta, alpha, beta)
    ordered = kl.sort().values
    mean_only = beta_forward_kl(old_alpha, old_beta, mean * old_concentration,
                                (1 - mean) * old_concentration)
    concentration_only = beta_forward_kl(old_alpha, old_beta, old_mean * concentration,
                                         (1 - old_mean) * concentration)
    return torch.stack((
        kl.mean(), ordered[int(0.95 * (kl.numel() - 1))],
        ordered[int(0.99 * (kl.numel() - 1))], ordered[-1],
        ((mean - old_mean).square() / old_variance).mean().sqrt(),
        (concentration / old_concentration).log().square().mean().sqrt(),
        mean_only.mean(), concentration_only.mean(),
    ))
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", f"SiTU factorized; actor={args.actor_factorized}; critic={args.critic_factorized}; "
                        "32x LR; 1 minibatch; raw GAE")
        envs = make_training_env(args, run_name)
        resources.callback(envs.close)
        agent = Agent(envs, actor_factorized=args.actor_factorized,
                      critic_factorized=args.critic_factorized).to(device)
        ...
        def geometry_model(observations, old_alpha, old_beta):
            return policy_geometry(agent.actor, observations, old_alpha, old_beta)
        ...
        if args.compile:
            ...
            geometry_model = torch.compile(geometry_model, mode=args.compile_mode,
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("policy_geometry"), torch.no_grad():
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                geometry = geometry_model(b_obs, b_alpha, b_beta)
            ...
            logged = gather_metrics({
                ...
                "policy/exact_kl_post": geometry[0],
                "policy/exact_kl_post_p95": geometry[1],
                "policy/exact_kl_post_p99": geometry[2],
                "policy/exact_kl_post_max": geometry[3],
                "policy/mean_shift_old_std_rms": geometry[4],
                "policy/log_concentration_ratio_rms": geometry[5],
                "policy/mean_only_kl": geometry[6],
                "policy/concentration_only_kl": geometry[7],
            })
