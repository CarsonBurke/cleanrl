# Vanilla-trunk Beta PPO with independent actor/critic gradient clipping, v1.
# Each complete network has its own 0.5 norm budget after the weighted joint
# backward. Scalar critic retains PPO value-loss clipping, with no EMA or return norm.
# Retains the base two-layer tanh trunks, initialization, optimizer and schedule.
import os
import random
import time
...
class Args:
    ...
    """retain PPO scalar value-loss clipping independently of gradient clipping"""
    ent_coef: float = 0.0
    """coefficient of the entropy"""
    vf_coef: float = 0.5
    """coefficient of the value function"""
    max_grad_norm: float = 0.5
    """independent maximum gradient norm for each actor and critic"""
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args):
    """Original clipped policy and scalar value objectives on native Beta samples."""
...
def clip_gradients(actor_parameters, critic_parameters, max_grad_norm):
    """Clip disjoint complete networks and return their weighted preclip norms."""
    actor_norm = nn.utils.clip_grad_norm_(actor_parameters, max_grad_norm, foreach=True)
    critic_norm = nn.utils.clip_grad_norm_(critic_parameters, max_grad_norm, foreach=True)
...
def main():
    ...
    try:
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss.backward()
                        actor_norm, critic_norm = clip_gradients(
                            actor_parameters, critic_parameters, args.max_grad_norm
                        )
            ...
            performed_norms = update_grad_norms[:updates]
            mean_norms = performed_norms.mean(dim=0)
            clip_fractions = (performed_norms > args.max_grad_norm).float().mean(dim=0)
            logged = gather_metrics(
                {
                    ...
                    "grad/actor_preclip_norm": mean_norms[0],
                    "grad/critic_preclip_norm": mean_norms[1],
                    "grad/actor_clip_fraction": clip_fractions[0],
                    "grad/critic_clip_fraction": clip_fractions[1],
                }
            )
