# Vector TD response v14: state-action future predictions corrected by vector TD.
# Learned state-dependent joint-action features have exact Beta expectations.
# One frozen one-step target per rollout, at real next observations; horizon
...
@dataclass
class Args:
    exp_name: str = 'vector_td_response_v14'
    seed: int = 1
...
def td_targets(components, next_baseline, horizons, ends):
    """Frozen vector one-step targets; next_baseline is the FULL expected critic.

    h=0 is identically zero. Episode ends suppress continuation, whereas an
    arbitrary rollout boundary does not. Never differentiate the target side.
    """
    continuation = (horizons > 1) & ~ends[:, None]
    target = components[:, None] + torch.where(continuation[..., None], next_baseline.detach(), 0.)
    return torch.where((horizons > 0)[..., None], target, 0.).detach()
...
def main():
    ...
    if min(args.total_timesteps, args.critic_width, args.critic_epochs, args.critic_learning_rate,
    ...
    with ExitStack() as resources:
        ...
        writer.add_text('method', 'Vector one-step TD; fixed feature reference; learned joint-action powers; exact Beta expectations; fresh full batches; no GAE/replay/scalar value/imagined states/target sweeps.')
        ...
        for iteration in range(1, iterations + 1):
            ...
            with timer.span('targets'), torch.no_grad():
                ...
                # Snapshot all continuation predictions BEFORE any critic fitting.
                next_logits = policy_model(next_observations).clone()
                future = continuation(next_observations, age + 1, (queries - 1).clamp_min(0), next_logits).clone()
                target = target_function(components, future, queries, ends_tensor.flatten()).clone()
        ...
        report = dict(args=vars(args), status='cancelled' if stop_requested else 'completed',
                      ...
                      scalar_value_network_used=False, learned_dynamics_used=False, gamma=1., td_steps=1,
                      ...
                      objective='Finite-horizon undiscounted vector action value, one frozen one-step TD target per fresh rollout',
