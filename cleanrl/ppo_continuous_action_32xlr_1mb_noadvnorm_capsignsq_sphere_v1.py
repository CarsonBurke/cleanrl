...
# Capped signed square: t = 4*tanh(x/4), f = t|t|. Odd (zero-mean, branch DC
# energy fraction 0.0003 against 0.31 for LeakyReluSq), bounded by 16,
# quadratic and saturation-free near 0.
# Measured on the uncapped 50M arms (jobs 5198-5202): a squared pair between two
# justnorms is exactly degree-2 homogeneous, so its weight norms have no
# restoring force and inflate to 3.9x (LeakyReluSq) / 4.9x (x|x|) of init by
# 17M steps, against 1.9x for tanh-capped SiTU-GLU. That inflation IS the
# implicit trust region of a scale-invariant parameterization at 32x LR --
# pinning ||W||_F at init instead produced approx_kl ~1.2 blowups by 8M -- and
# it costs plasticity, because for a scale-invariant map the angular step per
# Adam update decays like 1/||W||.
# Fix under test: take stability from saturation, the way SiTU-GLU does, instead
# of from norm inflation. Capping the preactivation at t = 4*tanh(x/4) (SiTU's
# own gate cap, 2.83 sigma at the v=2 design point) makes the block bounded and
# therefore scale-DEPENDENT, restoring a gradient signal on ||W|| that holds the
# activation near its design point without forbidding growth outright.
# This is the both-fixes cell of the 2x2: it is SiTU-GLU's stabilization
# recipe (bounded factors) and its zero-mean branch output, on a squared pair
# instead of a multiplicative gate. Design point E[f^2] = 6.35657 at v = 2
# (uncapped x|x|: 12.0).
# Hypothesis: the closest match to stiglu_sphere's dynamics of the four cells,
# and the arm that can beat it, since a monotone injective odd square keeps
# more input information per channel than a saturating multiplicative gate.
...
from cleanrl.shared.host_actor import make_capsignsq_sphere_trunk
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            make_capsignsq_sphere_trunk(observation_dim, 64),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; 32x LR; 1 minibatch; hypersphered tanh-capped zero-mean signed-square trunk; no advantage normalization; trunk probes")
