# PPO + IterThink v24 Beta + d3bucket MTP + PMPO + BetaPlast v3: PURE SIGN (no tanh).
# =====================================================================================
# v3 change over v2 (ablation): drop the tanh magnitude weight from the PMPO update.
# v2 weighted each taken action's per-dim log-prob by adv.tanh().abs() in [0,1) (a soft
# sign that still leaks a little magnitude near 0). v3 sets pmpo_adv_weight="sign": the
# weight is a CONSTANT 1, so the update is a PURE sign update -- every positive-advantage
# action is pushed up equally and every negative one down equally, magnitude fully removed;
# the sign enters ONLY through the pos/neg bucket split (balanced by count, KL-to-rollout
# leash unchanged). Tests whether even the tanh's residual magnitude was carrying credit or
# whether a strict sign is enough. Keeps v2's relative plasticity gate ([0.25,4]) intact.
#
# --- inherited v2 notes ---
...
class Args:
    ...
    # PMPO advantage magnitude weight. "tanh": adv.tanh().abs() in [0,1) (soft sign, v2 default).
    # "sign": constant 1 (PURE sign -- magnitude removed entirely; the sign enters only via the
    # pos/neg bucket split). v3 default.
    pmpo_adv_weight: str = "sign"
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if pmpo_active:
                    ...
                    # Advantage magnitude weight: "tanh" = soft sign in [0,1); "sign" = pure sign
                    # (constant 1; magnitude removed, only the pos/neg bucket carries the sign).
                    if args.pmpo_adv_weight == "tanh":
                        adv_weight = mb_advantages.tanh().abs().unsqueeze(-1)
                    elif args.pmpo_adv_weight == "sign":
                        adv_weight = torch.ones_like(mb_advantages).unsqueeze(-1)
                    else:
                        raise ValueError(f"unknown pmpo_adv_weight {args.pmpo_adv_weight}")
