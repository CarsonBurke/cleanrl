# PPO + Energy-Based Prediction Gaussian-Edge v1.1.
#
...
#
# v1.1 changes only the local hidden predictor granularity: instead of one dense
# layer-level Gaussian mean per target, every source perceptron predicts every
# downstream target perceptron with its own Gaussian edge. Edge means use a fixed
# fan-in scale calibration so rollout hidden-state std stays comparable to v1;
# energies are averaged across source-target edges to keep local-loss scale close.
#
...
class EdgeGaussianPredictor(nn.Module):
    def __init__(self, in_dim, out_dim, fixed_logvar=0.0, learn_logvar=False):
        super().__init__()
        self.aggregate_gain = in_dim / np.sqrt(2)
        self.weight = nn.Parameter(torch.empty(in_dim, out_dim))
        self.bias = nn.Parameter(torch.zeros(in_dim, out_dim))
        nn.init.orthogonal_(self.weight, np.sqrt(2))
        self.learn_logvar = learn_logvar
        if learn_logvar:
            self.logvar_weight = nn.Parameter(torch.empty(in_dim, out_dim))
            self.logvar_bias = nn.Parameter(torch.full((in_dim, out_dim), fixed_logvar))
            nn.init.orthogonal_(self.logvar_weight, 0.01)
        else:
            self.register_buffer("fixed_logvar", torch.full((in_dim, out_dim), fixed_logvar))

    def forward(self, x, logvar_min, logvar_max):
        source = x.unsqueeze(-1)
        mean = self.aggregate_gain * torch.tanh(source * self.weight.unsqueeze(0) + self.bias.unsqueeze(0))
        if self.learn_logvar:
            logvar = (source * self.logvar_weight.unsqueeze(0) + self.logvar_bias.unsqueeze(0)).clamp(
                logvar_min,
                logvar_max,
            )
        else:
...
class PCHierarchy(nn.Module):
    def __init__(self, input_dim, hidden_size, args):
        super().__init__()
        self.edge0 = EdgeGaussianPredictor(input_dim, hidden_size, args.pc_fixed_logvar, args.pc_learn_logvar)
        self.edge1 = EdgeGaussianPredictor(hidden_size, hidden_size, args.pc_fixed_logvar, args.pc_learn_logvar)

    def aggregate_edge_mean(self, edge, x, args):
        edge_mean, edge_logvar = edge(x, args.pc_logvar_min, args.pc_logvar_max)
        precision = torch.exp(-edge_logvar)
        return (edge_mean * precision).sum(dim=1) / precision.sum(dim=1).clamp_min(1e-8)

    def initial_states(self, x, args):
        h1_mean = self.aggregate_edge_mean(self.edge0, x, args)
        h2_mean = self.aggregate_edge_mean(self.edge1, h1_mean, args)
    ...
    def local_energies(self, x, states, args):
        ...
        e1 = 0.5 * (((h1.unsqueeze(1) - mean1) ** 2) * torch.exp(-logvar1) + logvar1)
        e2 = 0.5 * (((h2.unsqueeze(1) - mean2) ** 2) * torch.exp(-logvar2) + logvar2)
