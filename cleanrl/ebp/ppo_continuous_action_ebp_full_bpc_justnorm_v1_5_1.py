# PPO + Energy-Based Prediction Full-BPC JustNorm v1.5.1.
#
...
#
# v1.5.1 keeps v1.5's dense Bayesian local regression and full residual
# precision, but replaces per-coordinate hidden-state clipping with nGPT-style
# JustNorm projection. The BPC local models, actor, and critic all see the
# projected hidden direction.
#
# v1.5 keeps v1.2's dense Gaussian generative form, mean = W f(prev) + b, but
# replaces Adam local-predictor learning with Bayesian predictive-coding style
# sufficient-stat regression. Each local model maintains dense feature covariance
# A, target-feature cross-stat B, posterior mean W = B A^-1, and a full output
# precision matrix estimated from residual covariance. PPO/value heads still use
# Adam because the terminal actor/value objective is outside the local BPC model.
#
...
class Args:
    ...
    """unused for JustNorm hidden states; retained for CLI compatibility"""
    ...
    bpc_stat_update_rate: float = 0.05
    """EMA rate for Bayesian local sufficient-stat updates"""
    bpc_precision_update_rate: float = 0.05
    """EMA rate for output precision updates from residual covariance"""
    bpc_prior_precision: float = 1.0
    """ridge/prior precision for augmented local regression features"""
    bpc_precision_ridge: float = 1e-3
    """residual covariance ridge before inversion"""
    bpc_precision_min: float = 0.05
    """minimum eigenvalue of each local output precision matrix"""
    bpc_precision_max: float = 20.0
    """maximum eigenvalue of each local output precision matrix"""
    pc_justnorm_eps: float = 1e-6
    """minimum hidden-state L2 norm for JustNorm projection"""
...
def justnorm(x, eps=1e-6):
    ...
    return (x.float() / norm).to(dtype=x.dtype)
...
class LocalPredictor(nn.Module):
    def __init__(self, in_dim, out_dim, fixed_logvar=0.0, learn_logvar=False, activation="identity", args=None):
        super().__init__()
        del learn_logvar
        ...
        init_linear = layer_init(nn.Linear(in_dim, out_dim))
        weight = init_linear.weight.detach()
        bias = init_linear.bias.detach()
        precision0 = float(np.exp(-fixed_logvar))
        prior = float(args.bpc_prior_precision)

        augmented_weight = torch.cat([weight, bias[:, None]], dim=1)
        self.register_buffer("weight", weight.clone())
        self.register_buffer("bias", bias.clone())
        self.register_buffer("prior_augmented_weight", augmented_weight.clone())
        self.register_buffer("feature_stat", torch.zeros(self.feature_dim, self.feature_dim))
        self.register_buffer("empirical_target_feature_stat", torch.zeros(out_dim, self.feature_dim))
        self.register_buffer("feature_precision", torch.eye(self.feature_dim) * prior)
        self.register_buffer("target_feature_stat", augmented_weight @ self.feature_precision)
        self.register_buffer("output_precision", torch.eye(out_dim) * precision0)
        self.register_buffer("logdet_precision", torch.tensor(out_dim * np.log(precision0), dtype=torch.float32))
    ...
    def forward(self, x, logvar_min, logvar_max):
        del logvar_min, logvar_max
        features = predictor_activation(x, self.activation)
        return F.linear(features, self.weight, self.bias), self.output_precision

    def energy(self, x, target, args):
        mean, precision = self(x, args.pc_logvar_min, args.pc_logvar_max)
        err = target - mean
        quad = (err @ precision * err).sum(dim=1)
        return 0.5 * (quad - self.logdet_precision) / self.out_dim

    @torch.no_grad()
    def bpc_update(self, x, target, args):
        phi = self.features(x.detach())
        y = target.detach()
        batch = max(phi.shape[0], 1)
        eye = torch.eye(self.feature_dim, device=phi.device, dtype=phi.dtype)
        stat_rate = args.bpc_stat_update_rate
        precision_rate = args.bpc_precision_update_rate

        batch_feature_stat = (phi.T @ phi) / batch
        batch_target_feature = (y.T @ phi) / batch
        self.feature_stat.lerp_(batch_feature_stat, stat_rate)
        self.empirical_target_feature_stat.lerp_(batch_target_feature, stat_rate)
        self.feature_precision.copy_(args.bpc_prior_precision * eye + self.feature_stat)
        self.target_feature_stat.copy_(
            args.bpc_prior_precision * self.prior_augmented_weight + self.empirical_target_feature_stat
        )

        augmented_weight = torch.linalg.solve(
            self.feature_precision,
            self.target_feature_stat.T,
        ).T
        self.weight.copy_(augmented_weight[:, :-1])
        self.bias.copy_(augmented_weight[:, -1])

        mean = F.linear(phi[:, :-1], self.weight, self.bias)
        residual = y - mean
        covariance = (residual.T @ residual) / batch
        covariance = 0.5 * (covariance + covariance.T)
        covariance = covariance + args.bpc_precision_ridge * torch.eye(
            self.out_dim,
            device=covariance.device,
            dtype=covariance.dtype,
        )
        eigvals, eigvecs = torch.linalg.eigh(covariance)
        precision_eigvals = eigvals.clamp_min(1.0 / args.bpc_precision_max).reciprocal()
        precision_eigvals = precision_eigvals.clamp(args.bpc_precision_min, args.bpc_precision_max)
        precision = (eigvecs * precision_eigvals.unsqueeze(0)) @ eigvecs.T
        precision = 0.5 * (precision + precision.T)
        self.output_precision.lerp_(precision, precision_rate)
        _, logabsdet = torch.linalg.slogdet(self.output_precision)
        self.logdet_precision.copy_(logabsdet)


class PCHierarchy(nn.Module):
    def __init__(self, input_dim, hidden_size, args):
        super().__init__()
        self.edge0 = LocalPredictor(
            input_dim,
            hidden_size,
    ...
    def initial_states(self, x, args):
        h1_mean, _ = self.edge0(x, args.pc_logvar_min, args.pc_logvar_max)
        h1_mean = justnorm(h1_mean, args.pc_justnorm_eps)
        h2_mean, _ = self.edge1(h1_mean, args.pc_logvar_min, args.pc_logvar_max)
        h2_mean = justnorm(h2_mean, args.pc_justnorm_eps)
        return [h1_mean, h2_mean]

    def local_energies(self, x, states, args):
        h1, h2 = states
        return self.edge0.energy(x, h1, args).mean(), self.edge1.energy(h1, h2, args).mean()
    ...
    @torch.no_grad()
    def bpc_update(self, x, states, args):
        h1, h2 = states
        self.edge0.bpc_update(x, h1, args)
        self.edge1.bpc_update(h1, h2, args)
    ...
    def settle_states(self, hierarchy, x, initial_states, args, terminal_energy_fn, steps=None):
        ...
        for _ in range(steps):
            ...
            for layer_idx, (state, grad) in enumerate(zip(states, grads)):
                ...
                next_state = justnorm(next_state, args.pc_justnorm_eps)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                agent.actor_pc.bpc_update(b_obs[mb_inds], actor_states, args)
                agent.critic_pc.bpc_update(b_obs[mb_inds], critic_states, args)
