# PPO + Energy-Based Prediction Signed Hidden-Beta v6.
#
# Predictive-coding variant of base continuous PPO. Hidden activations are explicit
# signed bounded state variables, not an ordinary end-to-end MLP path. Each local
# hidden predictor outputs a Beta distribution over z=(h+1)/2, while the exposed
# hidden state h stays in (-1, 1). This preserves v1's centered latent geometry
# while using Beta hidden prediction as intended.
#
...
class PCHierarchy(nn.Module):
    ...
    def local_energies(self, x, states, args):
        h1, h2 = states
        h1 = h1.clamp(-1.0 + SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        h2 = h2.clamp(-1.0 + SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        z1 = ((h1 + 1.0) * 0.5).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        z2 = ((h2 + 1.0) * 0.5).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
