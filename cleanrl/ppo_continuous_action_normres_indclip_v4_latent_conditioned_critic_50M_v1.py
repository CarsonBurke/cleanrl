# Latent-conditioned critic PPO v1, based on normres_indclip_v4 50M Beta PPO.
# The actor keeps the base normalized-residual trunk and Beta head. The critic
# receives observation concatenated with the actor's detached pre-action latent,
# so value learning can use policy features without reshaping the actor.
# Hypothesis: policy features expose value-relevant state structure and improve
# long-horizon value estimates without changing policy-gradient incentives.
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, placement="pre", norm_kind="rms", activation="stiglu"):
        ...
        self.critic = nn.Sequential(
            make_norm_residual_trunk(
                observation_dim + self.actor_latent_dim,
                64,
                placement=placement,
                norm_kind=norm_kind,
        ...
        self.actor = nn.Sequential(
            ...
            layer_init(nn.Linear(self.actor_latent_dim, 2 * self.action_dim), std=0.01),
        )

    def get_actor_latent(self, x):
        return self.actor[0](x)
    ...
    def get_policy_and_value(self, x):
        ...
        alpha, beta = (F.softplus(self.actor[1](actor_latent)) + 1.0).chunk(2, dim=-1)
...
def main():
    ...
    try:
        ...
        writer.add_text(
            "policy",
            f"Latent-conditioned critic; Beta host actor; additive {args.placement}-{args.norm_kind}norm "
            f"{args.activation} residual trunk; detached actor latent concat; 32x LR; raw GAE",
        )
