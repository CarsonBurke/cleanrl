# HL-Gauss bandwidth/resolution ablations on our residual architecture v12.
# Same v10 learner: current-rollout standardized support, plain CE, no EMA.
# Default K101/sigma2; compare K201/sigma4 at identical sigma_z=0.2, then
# K201/sigma1.5 (sigma_z=0.075). Support remains mean +/-5 target std.
# Preserve both trunks, matrix projection, shared Adam and global gradient clip.
# Diagnostics separate the label-entropy floor from excess CE, decoded error,
# frame drift and actor/critic gradient shares. They do not alter updates.
import os
import random
import time
...
def categorical_value_loss(logits, target_probs):
    """Laboratory plain CE: sum over atoms, mean over samples, frozen labels."""
    return -(target_probs.detach() * logits.log_softmax(dim=-1)).sum(dim=-1).mean()
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, target_probs, args):
    ...
    v_loss = categorical_value_loss(logits, target_probs)
...
def main():
    ...
    try:
        ...
        def prepare_targets(returns):
            ...
            return labels, saturation, label_error, floor, frame_shift, frame_ratio
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_target_probs, saturated, label_error, entropy_floor, frame_shift, frame_ratio = prepare_targets(b_returns)
            updates = 0
            ...
            logged = gather_metrics({
                ...
                "critic/label_entropy": entropy_floor, "critic/excess_ce": last[1] - entropy_floor,
