# PPO + IterThink v106 rewscalar (v105 + scalar-space loss on the WM reward head).
#
# Change from v105:
#   - SCALAR-SPACE REWARD LOSS: the WM reward head keeps the softmax-over-bins
#     parameterization and expectation decode E[Z], but is trained ONLY by
#     Huber(delta=1) between the decoded scalar and the actual raw reward -- no
#     distribution-space CE term. The loss is now exactly the quantity the dream
#     consumes. Hypothesis from the v105 A/B: softmax-CE fits tightly (decoded
#     MAE 0.10) but prices mass misplacement independently of how far it shifts
#     the decode; the CDF/BCE head had decode-aligned geometry (led every matched
#     step to ~2.7M) but diluted its signal over 100 thresholds (MAE 2.7x worse,
#     plateaued ~3.7k vs 6.2k). Direct regression on the decode concentrates all
#     signal on the consumed error. Known risk (monitored, not patched): the
#     expectation can be satisfied by degenerate mass placement, and the cheapest
#     gradient path to move the mean is via far bins -- watch
#     lewm/reward_pred_edge_mass_* and lewm/reward_scalar_bias_*. Scalar targets
#     are clamped to the support so the bounded decode can always reach them.
#     Logged as lewm/reward_huber.
#
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # Scalar-space reward targets, clamped to the support so the
                    # bounded expectation decode can always reach them.
                    scalar_reward_targets = future_rewards.clamp(
                        args.lewm_reward_v_min, args.lewm_reward_v_max
                    )
                    ...
                    target_reward_loss = F.huber_loss(
                        reward_support.to_scalar(target_reward_logits),
                        scalar_reward_targets,
                        reduction="none",
                        delta=1.0,
                    )
                    ...
                    for mtp_idx in range(mtp_len):
                        ...
                        reward_scalar_err = F.huber_loss(
                            pred_reward_scalar,
                            scalar_reward_targets[:, mtp_idx:],
                            reduction="none",
                            delta=1.0,
                        )
                        reward_losses.append((reward_scalar_err * offset_valid).sum() / denom)
                        with torch.no_grad():
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/reward_huber", float(np.mean(wm_reward_losses)), global_step)
