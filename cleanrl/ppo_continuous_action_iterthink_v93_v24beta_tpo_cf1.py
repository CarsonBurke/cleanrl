# PPO + IterThink v93 (v24 Beta + one-step counterfactual TPO). From v92.
#
...
# rollout policy at each state. Unlike v92's learned counterfactual scorer, v93
# directly probes HalfCheetah's MuJoCo state: for each candidate action it restores
# the simulator state, steps one action, normalizes the counterfactual observation
# and reward with the current wrappers' statistics, and scores the candidate by
# one-step TD advantage r + gamma V(s') - V(s). TPO then constructs
# q_i ∝ p_old_i exp(zscore(score_i) / eta) on the sampled-candidate simplex and
# fits the current policy's simplex probabilities by cross-entropy. PPO ratio/
# clip are kept only as diagnostics; they do not enter the actor loss.
#
# Hypothesis: v92 failed because learned unexecuted-action scores were too weak
# and extrapolative. HalfCheetah exposes cheap simulator state restore, so a real
# one-step counterfactual scorer should give TPO a meaningful within-state action
# ranking while preserving v24's strong critic and bounded Beta replay.
#
# --- inherited v24 notes ---
...
def tpo_skill(scores, eps=1e-6):
    centered = scores - scores.mean(dim=-1, keepdim=True)
    std = centered.std(dim=-1, keepdim=True, unbiased=False)
    return torch.where(std > eps, centered / std.clamp_min(eps), centered)


def tpo_target(old_log_scores, scores, eta):
    skill = tpo_skill(scores)
    return torch.softmax(F.log_softmax(old_log_scores, dim=-1) + skill / eta, dim=-1)


def tpo_cross_entropy_loss(new_log_scores, old_log_scores, scores, eta):
    q = tpo_target(old_log_scores.detach(), scores.detach(), eta).detach()
...
class Args:
    ...
    # v93: pure TPO actor objective over one-step counterfactual MuJoCo probes.
...
def counterfactual_tpo_scores(envs, agent, candidate_actions, current_values, support, gamma, device):
    """One-step HalfCheetah counterfactual TD scores for a (num_envs, K, act_dim) action set.

    This intentionally bypasses wrappers while probing and restores MuJoCo state before
    the real wrapped env step. Observation/reward normalization are computed from the
    wrappers' current running statistics without updating those statistics.
    """
    norm_obs_batch = []
    norm_rewards = []
    nonterminals = []
    actions_np = candidate_actions.detach().cpu().numpy()
    for env_i, wrapped_env in enumerate(envs.envs):
        ...
        obs_eps = norm_obs_wrapper.epsilon
        rew_return = norm_rew_wrapper.returns[0]
        rew_mean = norm_rew_wrapper.return_rms.mean
        rew_var = norm_rew_wrapper.return_rms.var
        rew_count = norm_rew_wrapper.return_rms.count
        rew_eps = norm_rew_wrapper.epsilon
        ...
        for cand_i in range(actions_np.shape[1]):
            base.set_state(qpos, qvel)
            base.data.time = sim_time
            raw_next_obs, raw_reward, terminated, truncated, _ = base.step(actions_np[env_i, cand_i])
            if elapsed_steps is not None and max_episode_steps is not None:
                truncated = truncated or (elapsed_steps + 1 >= max_episode_steps)
            obs_mean_c, obs_var_c = rms_after_one(obs_mean, obs_var, obs_count, raw_next_obs)
            norm_next_obs = (raw_next_obs - obs_mean_c) / np.sqrt(obs_var_c + obs_eps)
            norm_obs_batch.append(np.clip(norm_next_obs, -10.0, 10.0))
            candidate_return = rew_return * norm_rew_wrapper.gamma * (1 - terminated) + raw_reward
            _, rew_var_c = rms_after_one(rew_mean, rew_var, rew_count, np.asarray(candidate_return, dtype=np.float64))
            norm_reward = raw_reward / np.sqrt(rew_var_c + rew_eps)
            norm_rewards.append(np.clip(norm_reward, -10.0, 10.0))
            nonterminals.append(0.0 if (terminated or truncated) else 1.0)
        base.set_state(qpos, qvel)
        base.data.time = sim_time

    with torch.no_grad():
        obs_t = torch.tensor(np.asarray(norm_obs_batch), dtype=torch.float32, device=device)
        next_probs = torch.softmax(agent.get_value(obs_t), dim=-1)
        next_values = (next_probs * support).sum(dim=-1).reshape(actions_np.shape[0], actions_np.shape[1])
        rewards_t = torch.tensor(np.asarray(norm_rewards), dtype=torch.float32, device=device).reshape_as(next_values)
        nonterm_t = torch.tensor(np.asarray(nonterminals), dtype=torch.float32, device=device).reshape_as(next_values)
        return rewards_t + gamma * nonterm_t * next_values - current_values.unsqueeze(-1)
...
if __name__ == "__main__":
    ...
    assert args.env_id == "HalfCheetah-v4", "v93 counterfactual probing is HalfCheetah-v4-specific"
    ...
    candidate_scores = torch.zeros((args.num_steps, args.num_envs, args.tpo_group_size)).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                cand_scores = counterfactual_tpo_scores(envs, agent, cand_actions, values[step], support, args.gamma, device)
            candidate_zs[step] = z_cands
            candidate_logprobs[step] = cand_logprobs
            candidate_scores[step] = cand_scores
        ...
        b_candidate_zs = candidate_zs.reshape((-1, args.tpo_group_size) + envs.single_action_space.shape)
        b_candidate_logprobs = candidate_logprobs.reshape(-1, args.tpo_group_size)
        b_candidate_scores = candidate_scores.reshape(-1, args.tpo_group_size)
        ...
        b_u = u.reshape(-1)
        # Diagnostics: keep the inherited v24 advantage transform visible, but
        # pure TPO uses the one-step counterfactual TD score vector collected
        # during rollout. TPO then standardizes scores within each candidate group.
        gae = b_advantages
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                new_candidate_logprobs, entropy, value_logits = agent.get_candidate_log_probs_and_value(
                    b_obs[mb_inds], b_candidate_zs[mb_inds]
                )
                ...
                tpo_scores = b_candidate_scores[mb_inds]
