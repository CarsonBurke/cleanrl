# PPO + IterThink v10 (distributional λ-return critic, CORRECTED). From v9.
#
# v9 implemented the distributional λ-return target correctly (mass/mean verified)
# but FAILED for two fixable config reasons, not a flaw in the idea:
#   (1) Diffuse init over a wide support. At init the categorical critic is ~
#       uniform over [-20,20] (std ≈ 11). The λ-recursion compounds that spread,
#       the affine x↦r+γx pushes mass past ±20, and clamping mangles the target →
#       corrupted early value (EV 0.21 @200k vs v7's 0.61). DOMINANT bug.
#   (2) Resolution too coarse to even be distributional: ±20/255 = 0.157/bin, but
#       the conditional return spread is ~0.25 → the target was ~1.6 bins wide
#       (barely two-hot), carrying no real distributional signal.
#
# v10 fixes both so the distributional target is sharp, faithful, and well-posed:
#   - PEAKED critic-head init: bias_i = -0.5*(z_i/tau)^2 (tau small) → initial Z
#     is a sharp Gaussian at 0, never uniform. Kills the blowup at the source.
#     Head weight kept small (std 0.1) so bias dominates the init but the trunk
#     still receives value gradients.
#   - TIGHT, drift-safe, well-resolved support: returns are reward-normalized so
#     the value MEAN drifts to ~3-5 at peak (≈ raw_return / σ_G), so we keep
#     headroom with [-10, 10] but use 511 bins → 0.039/bin ≈ 6 bins per
#     conditional-std: genuinely distributional, negligible clamping.
#   - Keep the verified λ-Bellman recursion (mean == scalar GAE λ-return), now
#     fed sharp inputs. Mean used for the (unchanged) GAE advantage.
#   - Instrumentation: log return mean/std and edge-bin mass to confirm the
#     support is adequate rather than assume it.
#
# Control: ppo_continuous_action_iterthink_v7_hlgauss (scalar smoothed target).
# Caveat: actor/critic trunks are SEPARATE, so this can only help via a better
# value mean → better advantages. If it merely matches v7, the next lever is a
# shared trunk so the richer critic objective reaches the policy.
import os
import random
import time
