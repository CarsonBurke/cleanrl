# PPO + IterThink v24 beta d4-HL-Gauss symlog critic + ENTROPY-REFERENCED Delightful gate v1.
#
# WHY. dg_v1 / dg_paper_v1 ported DG (arXiv:2603.14608v1) with raw surprisal
# ell = -log pi(a|s). In discrete settings ell ~= 0 for common actions, so the
# gate sigmoid(U*ell) is neutral (0.5) on typical samples and only gates rare
# ones. On-policy continuous samples instead ALL sit at ell ~= H(s) (+2.5..4
# nats here), so the gate degenerated to sigmoid(2.5*U): a pure advantage-sign
# gate that mutes nearly all negative-advantage gradient (measured gate
# pos/neg = 0.66/0.18 early). dg_v1 lost ~40% vs baseline because of it.
#
# FIX (novel continuous-DG semantics): center surprisal at its per-state
# expectation, the differential entropy: ell~ = -log pi(a|s) - H(s), so
# E[ell~|s] = 0 exactly and the discrete-case gate semantics are restored:
#   typical action            -> ell~ ~= 0 -> weight ~= 1 (neutral)
#   tail action, U>0          -> breakthrough, amplified (up to 2x)
#   tail action, U<0          -> blunder, muted (down to 0x)
#   mode action, U<0          -> confident bad action, pushed down harder
# For the Beta policy both -log pi and H are native-space closed forms; the
# env-rescale Jacobian constant cancels in the subtraction.
#   U = existing rankgauss/minibatch-normalized policy advantage (std ~= 1).
#   chi = U * clip(ell~, -C, C);  weight = 2*sigmoid(chi/eta), eta=1.
#   PPO's clipped policy terms are multiplied by stopgrad(weight); 2x makes
#   the neutral point an exact identity (chi=0 -> weight=1), so the gate is a
#   pure RELATIVE reallocation of gradient budget on top of the proven PPO
#   carrier (ratio clip 0.2/0.28, 10 epochs, target_kl 0.03 all kept).
# dg_gate_mode="amplify_only" is the attribution ablation: gate only U>0
# samples (rare-success amplification), leave all U<0 weights at 1.
#
...
class Args:
    ...
    # Entropy-referenced Delightful gate (continuous-corrected arXiv:2603.14608v1).
...
def delightful_entref_gate(advantages, logprob, entropy, eta=1.0, clip_bound=10.0):
    """Entropy-referenced DG gate for continuous actions.

    ell~ = clip(-log pi(a|s) - H(s), -C, C), chi = U * ell~, w = sigmoid(chi/eta).
    Centering at the per-state differential entropy makes E[ell~|s] = 0, so a
    TYPICAL on-policy sample gets w = 0.5 (neutral after the 2x rescale) and
    only atypical samples are gated. logprob and entropy must share the same
    coordinate space (both Beta-native here: the env-rescale Jacobian constant
    cancels in the subtraction). Caller detaches both inputs; the training loss
    uses stopgrad(w) so the gradient stays w * U * grad log pi.
    """
    if eta <= 0:
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.delightful_pg:
                    ...
                    dg_gate, dg_surprisal, dg_raw_surprisal, dg_delight = delightful_entref_gate(
                        mb_advantages,
                        newlogprob.detach(),
                        entropy.detach(),
                        eta=args.dg_eta,
