# PMPO-D4 + normadv + LeakyReluSq activation v1.
#
# Combines two changes that individually moved the needle on HalfCheetah:
#   - pmpo_norm_adv (default True here): PPO-style per-minibatch advantage
#     normalization before the PMPO bucket-split / tanh weighting.
#   - LeakyReluSq hidden activation: f(x) = (0.5*x + 0.5*relu(x))^2 replaces
#     all hidden Tanh in the actor and critic networks. Equals x^2 for x>=0
#     and 0.25*x^2 for x<0; smooth at 0, always non-negative, unbounded.
# Otherwise this is the PMPO-D4 actor (tanh-squashed actions, bucket-balanced
# sign-split + tanh(|adv|) magnitude weighting, reverse KL with old policy).
# `pmpo_use_signed` and `policy_objective=ppo` flags remain available for
# completeness but are off by default for this ablation.
