...
#
# Adds --reward-rms: scale immediate rewards by a global EMA of reward second moment.
# Mean-free, clip-free, return-agnostic (no R / gamma in the scale). Critic targets and
# GAE use the scaled rewards; episodic_return logging stays raw. Orthogonal to
# --norm-adv / ret_ema_norm (those still optional).
#
# Ablations vs strong_mb_nodg_v2 baseline (nodg, raw rew, minibatch advnorm, L2-hinge KL):
#   1) --no-norm-adv
#   2) --no-norm-adv --reward-rms
#   3) --reward-rms  (advnorm ON)
# ----------------------------------------------------------------------------
...
class Args:
    ...
    # --- Analytic-KL trust region (v21: the soft PENALTY is the ONLY KL control) ---
    kl_trust: bool = True            # add the KL penalty (exact Beta KL) to actor loss
    kl_target: float = 0.03          # v21: adaptive-beta regulation target == the deadband, so the
    #                                  realized KL settles right at the deadband edge (kl_pen_start).
    ...
    kl_pen_start: float = 0.03       # deadband edge: zero penalty below this. MUST be > 0.02.
