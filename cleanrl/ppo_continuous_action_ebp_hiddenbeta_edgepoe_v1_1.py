# PPO + Energy-Based Prediction Hidden-Beta Edge-PoE v1.1.
#
# Predictive-coding variant of base continuous PPO. Hidden activations are explicit
# signed state variables, not an ordinary end-to-end MLP path. Each local edge has
# per-source/per-target Beta evidence predictors over z=(h+1)/2; source-wise
# evidence is combined as a target-wise product of experts. During PPO updates,
# actor and critic states settle by minimizing local Beta prediction energies plus
...
# backprop through the hidden hierarchy. Edge-local Beta product-of-experts should
# preserve centered hidden features while giving each source perceptron a direct
# opinion about each target. The actor uses v168's bounded unimodal Beta policy,
# alpha/beta = 1 + softplus(head), sampled in native action z in (0, 1).
import os
import random
import time
...
class Args:
    ...
    pc_state_clip: float = 1.0
    """unused compatibility flag; hidden states are always clamped to (-1, 1)"""
...
def signed_to_unit_interval(h):
    return ((h + 1.0) * 0.5).clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)


def unit_interval_to_signed(z):
    return (2.0 * z - 1.0).clamp(-1.0 + SAMPLE_EPS, 1.0 - SAMPLE_EPS)


class EdgePoEBetaPredictor(nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.alpha_weight = nn.Parameter(torch.empty(in_dim, out_dim))
        self.alpha_bias = nn.Parameter(torch.empty(in_dim, out_dim))
        self.beta_weight = nn.Parameter(torch.empty(in_dim, out_dim))
        self.beta_bias = nn.Parameter(torch.empty(in_dim, out_dim))
        self.reset_parameters()

    def reset_parameters(self):
        nn.init.normal_(self.alpha_weight, mean=0.0, std=0.01)
        nn.init.constant_(self.alpha_bias, -6.0)
        nn.init.normal_(self.beta_weight, mean=0.0, std=0.01)
        nn.init.constant_(self.beta_bias, -6.0)

    def forward(self, x):
        source = x.unsqueeze(-1)
        alpha_evidence = F.softplus(source * self.alpha_weight.unsqueeze(0) + self.alpha_bias.unsqueeze(0))
        beta_evidence = F.softplus(source * self.beta_weight.unsqueeze(0) + self.beta_bias.unsqueeze(0))
...
class PCHierarchy(nn.Module):
    def __init__(self, input_dim, hidden_size, args):
        super().__init__()
        self.edge0 = EdgePoEBetaPredictor(input_dim, hidden_size)
        self.edge1 = EdgePoEBetaPredictor(hidden_size, hidden_size)

    def edge_dist(self, edge, x):
        alpha_evidence, beta_evidence = edge(x)
        alpha = 1.0 + alpha_evidence.sum(dim=1)
        beta = 1.0 + beta_evidence.sum(dim=1)
        return Beta(alpha, beta)

    def initial_states(self, x, args):
        h1_dist = self.edge_dist(self.edge0, x)
        h1_mean = unit_interval_to_signed(h1_dist.mean)
        h2_dist = self.edge_dist(self.edge1, h1_mean)
        h2_mean = unit_interval_to_signed(h2_dist.mean)
        return [h1_mean, h2_mean]

    def local_energies(self, x, states, args):
        h1, h2 = states
        z1 = signed_to_unit_interval(h1)
        z2 = signed_to_unit_interval(h2)
        dist1 = self.edge_dist(self.edge0, x)
        dist2 = self.edge_dist(self.edge1, h1.clamp(-1.0 + SAMPLE_EPS, 1.0 - SAMPLE_EPS))
    ...
    def settle_states(self, hierarchy, x, initial_states, args, terminal_energy_fn, steps=None):
        ...
        for _ in range(steps):
            ...
            for state, grad in zip(states, grads):
                ...
                next_state = next_state.clamp(-1.0 + SAMPLE_EPS, 1.0 - SAMPLE_EPS)
