# PPO + IterThink v24.16 (autoregressive Beta ablation). From v24_dist.
#
# Hypothesis: bounded Beta exploration can improve if later action dimensions
# condition on earlier native z samples instead of assuming diagonal independence.
# Policy:
#   p(z|s) = prod_i Beta(z_i | alpha_i(s, z_<i), beta_i(s, z_<i)).
# The base alpha/beta heads match v24's independent Beta actor, and a masked
# lower-triangular residual over centered z prefixes is zero-initialized, so init
# is exactly v24. Rollout stores native z in (0,1); PPO replay teacher-forces the
# stored prefixes through the autoregressive head for exact probability ratios.
# Actions are only linearly rescaled from z to the env action range. Critic,
# advantage shaping, gradient clipping, and PPO training behavior are preserved.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v24.16 autoregressive Beta. The base heads are exactly v24's independent
        # Beta logits. The conditional residual is a masked linear embedding of the
        # already generated native z prefix; zero init makes startup exactly v24.
    ...
    def _sample_autoreg_z(self, actor_feat):
        ...
        for i in range(self.act_dim):
            alpha, beta, _, _ = self._beta_params(actor_feat, z)
            zi = Beta(alpha[:, i], beta[:, i]).sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            z[:, i] = zi
        return z
    ...
    def actor_diagnostics(self, x, z):
        ...
        alpha, beta, alpha_resid, beta_resid = self._beta_params(actor_feat, z)
