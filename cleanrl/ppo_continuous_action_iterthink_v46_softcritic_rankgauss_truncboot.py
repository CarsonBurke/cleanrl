# PPO + IterThink v46 (REVERT the advantage transform to rankgauss_signmag = v40's). From v45.
# Soft-actor-critic structure (unchanged; soft-entropy channel is ~inert -- alpha_eff ~6e-4 -- and kept only
# for diff continuity):
...
# ONE algorithmic change from v45: adv_transform "v10" -> "rankgauss_signmag".
# DIAGNOSIS (corrected -- the transform, NOT the entropy target, is the lever):
#   * The v45 target test was NULL: alpha stayed ~0.08 and alpha_eff ~6e-4 (== v44), because the once/epoch
#     dual is too slow to move alpha far in 8M; target 0 vs -dim therefore produced ~identical policies
#     (v44 Walker 463 ~ v45 617, both broken). The soft-entropy apparatus is effectively OFF.
#   * The REAL split is the advantage transform, holding target fixed:
#       target -dim:  rankgauss(v40) Walker 4964  vs  plain(v45) 617   (8x)
#       target  0:    rankgauss(v43) Walker  854  vs  plain(v44) 463   (2x)
#     and HC: rankgauss(v40) 7121 vs plain ~4750. WHY: HC/Walker advantages are ~10x heavier-tailed than
#     Hopper's (A_soft_std ~0.08 vs ~0.007). Plain GAE + z-score lets outlier steps (big strides / falls)
#     dominate each minibatch gradient -> no convergence. rankgauss maps to the Gaussian quantile of the
#     global rank: sign-correct, zero-crossing-preserving, tail-taming (outliers -> ~3 sigma). This recovers
#     v40's HC 7121 / Walker 4964; with v43's truncation bootstrap on top it should match or slightly beat v40.
# TRADE-OFF: plain GAE's Hopper gain (1235 vs v40's 779) is likely forfeited here -- rankgauss flattens
# Hopper's well-behaved magnitudes. The Hopper-vs-(HC,Walker) transform tension is the next experiment: a
# magnitude-PRESERVING robust transform (tanh_gae / rankgauss_signed) that tames tails without discarding scale.
# Inherited: v43 truncation bootstrap + gamma-on-entropy; direct_entropy True; --diag instrumentation
# (raw A_soft moments, reward/entropy-return std split, pre-tanh |z| quantiles, mu/sigma min-max, per-head GNs).
# Inherited from v43: time-limit truncations bootstrap V(final_obs); gamma on the entropy term. Inherited
# from v42: alpha/return_std scaling. Inherited from v41: single-counted denoised label, ONE alpha, target 0.
#
# ---- inherited v43 notes ----
...
class Args:
    ...
    soft_adv_coef: float = 0.25     # v38: FIXED weight of the (scale-matched) future-entropy advantage in
                                    #      policy_adv = reward_adv + coef * ent_adv_rescaled. ent_adv is first
                                    #      rescaled to the reward advantage's std, so coef is the entropy
                                    #      reorder's size as a fraction of the reward scale (ratio ~ sqrt(1+coef^2)).
                                    #      Replaces alpha-weighting the in-GAE channel (scale-unstable). 0 => off.
    ...
    direct_entropy: bool = True     # v44: ON (kept). SAC's current-state -alpha*H(s_t) actor term, alpha
                                    #      DETACHED, read fresh each minibatch. With dual_freq="epoch" alpha is
                                    #      piecewise-constant per epoch, so this read is ALREADY epoch-granular
                                    #      (alpha only moves at epoch boundaries). It is the ONLY channel where
                                    #      the dual's alpha is LIVE inside the rollout: the soft TARGET bakes
                                    #      alpha_r once (pre-epoch) and never refreshes, so target-only entropy
                                    #      would leave the per-epoch dual inert until the next rollout. Keeping
                                    #      the direct term restores SAC's exact split AND makes the dual bite.
    ...
    target_entropy_coef: float = -1.0  # v45: REVERT to SAC's -|A| per-dim target (= v40's setting). The v41
                                    #      change to 0.0 was the WALKER REGRESSION: target 0 BINDS (squashed H sits
                                    #      BELOW 0 on every env -> dual cranks alpha to push entropy UP), forcing
                                    #      exploration on a sensitive balancer that must SHARPEN to converge. v44
                                    #      Walker was dead flat (425->459 over 4M) under it. With -|A| the target is
                                    #      (intentionally) UNREACHABLE: squashed H stays ABOVE -dim, the dual bleeds
                                    #      alpha to alpha_min, and the max-ent pressure gets OUT OF THE WAY so the
                                    #      reward gradient can drive the policy deterministic -- exactly v40's regime
                                    #      (Walker 4964, a LATE bloomer: 637->1405->2188 at 4M, then ~5000). target =
                                    #      -1.0*dim (-6 HC/Walker, -3 Hopper). The soft channel goes ~inert (alpha at
                                    #      floor); kept for the clean single-variable diff vs v44 (plain GAE retained,
                                    #      which gave Hopper 1235 >> v40's 779).
    alpha_lr: float = 1e-3
    alpha_init: float = 0.1         # SAC-ish init (RAW units now -- alpha enters as alpha/return_std).
    alpha_min: float = 0.001        # v41: low floor (was 0.02). With a reachable target the dual equilibrates
                                    #      alpha itself; the floor is just a positivity guard, not a support prop.
    alpha_max: float = 0.5          # v42: upper clamp (RAW units). With alpha_eff=alpha/return_std the soft-return
                                    #      entropy level is ~alpha*H_bar*100/return_std; at the worst corner
                                    #      (return_std at floor 20, H_bar~4) alpha_max=0.5 caps it at the support
                                    #      edge (~10). The dual sits at ~0.08 so this never binds -- pure safety.
    ...
    adv_transform: str = "rankgauss_signmag"  # v46: REVERT to v40's transform. The v44/v45 plain-GAE detour
                                    #      was the real regression: HC/Walker advantages are ~10x heavier-tailed
                                    #      than Hopper's (A_soft_std ~0.08 vs ~0.007), so plain GAE + z-scoring lets
                                    #      a few outlier steps (big strides / falls) dominate each gradient -> the
                                    #      policy can't converge (Walker stuck ~500-600 in v44/v45). rankgauss maps
                                    #      the advantage to the Gaussian quantile of its global rank: sign-correct,
                                    #      zero-crossing-preserving, and TAIL-TAMING (outliers -> ~3 sigma). v40 with
                                    #      this hit HC 7121 / Walker 4964. The entropy target barely matters (v44 vs
                                    #      v45 confirmed alpha stays ~0.08, alpha_eff ~6e-4 inert either way), so the
                                    #      transform -- not the target -- is the lever. NOTE: this likely costs the
                                    #      Hopper gain plain GAE gave (1235 -> ~780); the Hopper-vs-Walker transform
                                    #      tension is a separate follow-up (a magnitude-preserving robust transform).
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            truncs[step] = torch.as_tensor(truncations.astype(np.float32)).to(device)
            if truncations.any() and "final_observation" in infos:
                fobs_arr = infos["final_observation"]
                for i in np.nonzero(truncations)[0]:
        ...
        with torch.no_grad():
            ...
            if truncs_b.any():
                f_logits, _ = agent.get_value(final_obs_buf[truncs_b])           # (K, n)
            ...
            if auto_alpha:
                # Sample a' ~ π(·|s_T) for the bootstrap value AND the bootstrap entropy (SAC's single sample).
                ...
                boot_logits, _ = agent.get_value(next_obs)
                bootstrap_probs = torch.softmax(boot_logits, dim=-1)            # (B, n) = Z(s_T)
            ...
            if auto_alpha and args.soft_adv:
                ...
                ent_bonus[:-1] = (-logprobs[1:]) * (1.0 - dones[1:])    # H(s_{t+1}); 0 if s_{t+1} starts a new episode
                ent_bonus[-1] = (-boot_logprob) * (1.0 - next_done)     # bootstrap step at t = T-1
            ...
            ent_target = ent_values          # V_ent retired: no-op target (head untrained; see update loop)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if auto_alpha and args.direct_entropy:
                    pg_loss = pg_loss - (log_alpha.exp().detach() / return_std) * entropy.mean()
                ...
                # v41: V_ent head RETIRED (entropy now lives in the main C51 critic). The head is still
                # defined for call-signature stability but is no longer trained or used. Logged as 0.
                ent_v_loss = torch.zeros((), device=device)
        ...
        with torch.no_grad():
            if auto_alpha and args.soft_adv:
                ent_bonus_frac = ((alpha_eff * ent_bonus).abs().mean() / (rewards.abs().mean() + 1e-8)).item()
        ...
        if auto_alpha:
            writer.add_scalar("losses/alpha", log_alpha.exp().item(), global_step)         # RAW (SAC) temperature
            ...
            # squashed entropy H(s) = -logπ. With the REACHABLE target (0), H should equilibrate NEAR 0
            # (dual two-sided), and alpha should settle at a POSITIVE value (not bleed to the floor as v40).
