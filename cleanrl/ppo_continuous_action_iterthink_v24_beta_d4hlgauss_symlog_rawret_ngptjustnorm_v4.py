# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic nGPT-justnorm v4.
#
...
# per-channel eigen learning rates initialized to the reference nGPT scale (0.05).
# v3 combined
...
def ngpt_residual(x, branch, alpha):
    lr = alpha.clamp(0.0, 1.0).to(dtype=x.dtype).view(*([1] * (x.ndim - 1)), x.shape[-1])
    x_u = justnorm(x)
...
class ThinkBlock(nn.Module):
    ...
    def __init__(self, in_dim, H, n_experts):
        ...
        self.input_alpha = nn.Parameter(torch.full((H,), NGPT_ALPHA_INIT))
        self.branch_alpha = nn.Parameter(torch.full((H,), NGPT_ALPHA_INIT))
    ...
    def forward(self, cat_feats, x0):
        x_in = ngpt_residual(x0, self.in_proj(cat_feats), self.input_alpha)
        ...
        return ngpt_residual(x_in, branch, self.branch_alpha)
