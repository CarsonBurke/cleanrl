# Standardized conditional transport PPO: exact density, no ODE or reward-weighted regression.
# Two alternating triangular maps learn action dependence around a trainable Gaussian.
# Hypothesis: retain PPO's signed, ratio-controlled policy improvement while learning
# nonlinear joint exploration; standardized coordinates keep shape updates noise-relative.
# Zero transport heads start at the matched Gaussian. Samples are detached; inverse
# scores MUST be differentiated. This is normalizing transport, NOT flow matching.
import hashlib
import json
import math
import os
...
class Args:
    ...
    flow_layers: int = 2
    """alternating triangular transport stages; zero is the exact Gaussian control"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, args):
        ...
        for stage in range(args.flow_layers):
            ...
            self.couplings.append(nn.Sequential(
                ...
                layer_init(nn.Linear(64, 2 * changed_dim), std=0.0),
            ))
    ...
    def transport(self, observations, source, inverse=False):
        """Triangular maps: exact inverse and forward log-determinant.

        Inverse inputs MUST retain gradients when scoring actions: subsequent
        conditioners depend on values produced by earlier inverse stages.
        tanh bounds diagonal logscales, not the full Jacobian condition number.
        """
        value = source
        ...
        for stage in stages:
            ...
            shift, raw_scale = self.couplings[stage](torch.cat((observations, fixed), -1)).chunk(2, -1)
            logscale = raw_scale.tanh()
            changed = (changed - shift) * (-logscale).exp() if inverse else changed * logscale.exp() + shift
            value = torch.cat((fixed, changed), -1) if stage % 2 == 0 else torch.cat((changed, fixed), -1)
            logdet = logdet + logscale.sum(-1)
        return value, logdet
...
class HostSampler:
    ...
    def __init__(self, agent, rows):
        ...
        self.couplings = [make_host_mirror(module, rows) for module in agent.couplings]
        ...
        self.inputs, self.scales = [], []
        for stage in range(len(self.couplings)):
            fixed_dim = self.split if stage % 2 == 0 else agent.action_dim - self.split
            self.inputs.append(np.empty((rows, self.obs_dim + fixed_dim), np.float32))
            self.scales.append(np.empty((rows, agent.action_dim - fixed_dim), np.float32))
    ...
    def __call__(self, observations, rng, source=None):
        ...
        for stage, coupling in enumerate(self.couplings):
            ...
            inputs, scale = self.inputs[stage], self.scales[stage]
            inputs[:, :self.obs_dim] = observations
            inputs[:, self.obs_dim:] = fixed
            shift, raw_scale = np.split(coupling(inputs), 2, -1)
            np.tanh(raw_scale, out=scale)
            self.logdet += scale.sum(-1)
...
def provenance(agent, args):
    return {
        ...
        "likelihood": "exact inverse transport joint density; actual host source/Jacobian behavior score; FP32 rounding drift logged",
        "method": "PPO on conditional affine coupling transport in standardized Gaussian coordinates; NOT flow matching",
        "gradient": "sampled action detached; exact score differentiates the inverse map, never the rollout sampling graph",
        "limitations": ["PPO clipping is not a hard KL bound", "sampled KL, not analytic KL", "bounded diagonal logscale does not bound full Jacobian conditioning", "one seed is not an optimality result"],
    }
