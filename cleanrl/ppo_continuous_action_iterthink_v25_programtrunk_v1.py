# PPO + IterThink v25 ProgramTrunk v1 — emergent reusable circuitry.
#
# METHOD (TRUNK ONLY; the entire v24 PPO/critic/advantage stack is UNCHANGED).
# Replaces v24's DenseNet trunk (48 PRIVATE per-block experts, softmax-over-all,
# fresh per block) with a depth-recurrent PROGRAM over ONE SHARED expert library:
#   - A single library of E experts (Linear(H,H)->ReLU^2->Linear(H,H)) is invoked
#     at N weight-tied steps (unrolled recurrence / universal-transformer style).
#   - At each step a TOP-K sparse router, conditioned on the running state AND a
#     per-step `step_embed`, selects k experts and combines them by softmax over
#     the top-k logits. State update is a bounded convex residual
#     z = g*(z + upd) + (1-g)*x0 with a per-step gate. Output dim = H.
#
# WHY REUSABLE CIRCUITS EMERGE. The optimizer CANNOT give each depth a private
# function — the only way to realize N distinct transforms is to RECOMBINE the
# shared experts differently per step. `step_embed` breaks tie-symmetry so each
# call-site runs a different "program line" over the same subroutine library;
# top-k forces specialization (vs v24's softmax-over-all averaging, which blurs
# experts into a within-block mean). Reuse paid off at narrow width in screening;
# this spends the SAME ~470-600K budget on a reusable library + 2x effective depth.
#
# OPTIMIZATION (session finding: deep trunks under-optimized). Fan-in-correct init
# everywhere (std = gain/sqrt(fan_in), He gain on ReLU^2 layers; NOT constant
# sqrt(2)), and each expert's 2nd linear scaled by 1/sqrt(2*N) at init so the sum
# of N residual branches keeps the stream ~unit-variance — LIVE small, not ReZero.
#
# LOAD BALANCE. Switch-style aux loss (coef lb_coef~1e-2) from the router probs is
# added to the total loss so top-k experts don't collapse to a few always-on ones.
#
# DIAGNOSTICS (circuits/*): cross-depth expert reuse rate + multi-depth fraction,
# per-step router entropy, and inter-step usage KL — measuring emergent reuse.
#
# --- inherited v24 notes ---
# v24 beta d4-HL-Gauss symlog critic: scalar GAE return -> symlog -> Gaussian-
# smoothed categorical target; logits -> E[symlog bin center] -> symexp -> scalar
# value for GAE/bootstrap. Support edges linear in symlog(v_min)..symlog(v_max).
#
...
class Args:
    ...
    # ProgramTrunk: a depth-recurrent program over a SHARED expert library.
    # Defaults are param-matched to ~484K trunk params (vs v24's ~470K), spending
    # the budget on a reusable library called at N weight-tied steps instead of
    # 48 private per-block experts.
    hidden: int = 128                # H, trunk feature/residual-stream width
    n_steps: int = 6                 # N weight-tied program steps (unrolled recurrence)
    n_experts: int = 14              # E shared subroutines in the library
    top_k: int = 4                   # sparse experts called per step (softmax over top-k)
    step_embed_dim: int = 16         # per-step call-site context dim feeding the router
    lb_coef: float = 1e-2            # Switch-style load-balance aux-loss coefficient
...
def fanin_linear(in_dim, out_dim, gain=1.0, bias_const=0.0):
    # Fan-in-correct init: std = gain / sqrt(fan_in). The session's optimization
    # finding: constant std=sqrt(2) over-/under-scales projections whose fan-in
    # is not H (concat/router/in-projections), starving deeper trunks. gain=sqrt(2)
    # recovers the ReLU-family He variance; gain=1 is the linear/identity default.
    layer = nn.Linear(in_dim, out_dim)
    std = gain / math.sqrt(in_dim)
...
class Expert(nn.Module):
    """A reusable subroutine: Linear(H,H) -> ReLU^2 -> Linear(H,H).

    Fan-in-correct init throughout. The SECOND linear is additionally scaled by
    1/sqrt(2*N) at init so that summing N weight-tied residual branches keeps the
    residual-stream variance ~unit at init (NOT zero-init/ReZero, which the session
    found starved the branches — the branch is initialized LIVE but small)."""

    def __init__(self, H, n_steps):
        super().__init__()
        self.fc1 = fanin_linear(H, H, gain=math.sqrt(2.0))  # He for ReLU^2
        self.act = ReLUSquared()
        self.fc2 = fanin_linear(H, H, gain=1.0)
        with torch.no_grad():
            self.fc2.weight.mul_(1.0 / math.sqrt(2.0 * n_steps))
            self.fc2.bias.zero_()
...
class ProgramTrunk(nn.Module):
    """A depth-recurrent program over a SHARED library of reusable experts.

    One library of E experts is invoked at N weight-tied steps (unrolled recurrence
    = universal-transformer style). At each step a top-k sparse router (conditioned
    on the running state AND a per-step `step_embed`) selects k experts and combines
    them by softmax over the top-k logits. The state is updated by a bounded convex
    residual `z = g*(z + upd) + (1-g)*x0` with a per-step gate. Output dim = H.

    Reusable circuits EMERGE because the optimizer CANNOT allocate a private function
    per depth: the only way to realize N distinct transformations is to recombine the
    shared experts differently per step. `step_embed` breaks the tie symmetry so each
    call-site can run a different "program line" over the same subroutine library;
    top-k forces specialization (vs v24's softmax-over-all blur).

    The most recent forward stashes:
      - `last_aux_loss`: graph-connected Switch-style load-balance loss (mean over
        steps), to be added to the total loss by the caller.
      - `last_route_*`: detached per-step routing stats for the reuse diagnostics.
    """

    def __init__(self, in_dim, H, n_steps, n_experts, top_k, step_embed_dim=16):
        super().__init__()
        self.H = H
        ...
        self.entry = fanin_linear(in_dim, H, gain=math.sqrt(2.0))

        # Per-step call-site context: lets the SAME library implement a different
        # program line per step while keeping subroutines shared.
        self.step_embed = nn.Embedding(n_steps, step_embed_dim)
        nn.init.normal_(self.step_embed.weight, std=0.02)

        # Router: state (post-norm) + step embed -> expert logits. Fan-in init.
        self.router = fanin_linear(H + step_embed_dim, n_experts, gain=1.0)
        self.route_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.body_norm = nn.RMSNorm(H, elementwise_affine=False)

        # SHARED expert library — defined once, called at every step.
        self.experts = nn.ModuleList([Expert(H, n_steps) for _ in range(n_experts)])

        # Per-step (per-call-site) convex residual gate; init +4 => g~0.982 => the
        # update is added at full strength while the x0 anchor is preserved.
        self.gate_w = nn.Parameter(torch.full((n_steps, H), 4.0))

        self.out_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.out_proj = fanin_linear(H, H, gain=1.0)

        # Stash slots (populated each forward).
        self.last_aux_loss = None
        self.last_route_probs = None   # (N, E) mean soft router prob per step (detached)
        self.last_route_onehot = None  # (N, E) mean top-k selection freq per step (detached)
    ...
    def forward(self, x):
        ...
        # Stack all experts once per step is unavoidable for dense param reuse;
        # we compute expert outputs only on the gathered top-k via masking-free
        # dense eval (E small) then select. Dense eval keeps it simple & vectorized.
        aux_terms = []
        route_probs_log = []
        route_onehot_log = []
        for t in range(self.n_steps):
            zn = self.route_norm(z)
            s = self.step_embed.weight[t].expand(B, -1)            # (B, S)
            logits = self.router(torch.cat([zn, s], dim=-1))       # (B, E)
            full_probs = torch.softmax(logits, dim=-1)             # (B, E) — for aux loss

            # Top-k sparse routing; softmax over the top-k VALUES only.
            topv, topi = logits.topk(self.top_k, dim=-1)           # (B, k)
            w = torch.softmax(topv, dim=-1)                        # (B, k)

            # Evaluate the shared library densely on the normed state, gather top-k.
            bn = self.body_norm(z)
            all_out = torch.stack([e(bn) for e in self.experts], dim=1)  # (B, E, H)
            sel = all_out.gather(
                1, topi.unsqueeze(-1).expand(-1, -1, self.H)
            )                                                       # (B, k, H)
            upd = (w.unsqueeze(-1) * sel).sum(dim=1)               # (B, H)

            g = torch.sigmoid(self.gate_w[t])                      # (H,)
            z = g * (z + upd) + (1.0 - g) * x0                     # bounded convex residual

            # ---- Switch-style load balance (per step): E * sum_e f_e * P_e ----
            # f_e = fraction of tokens routing e into its top-k; P_e = mean router prob.
            with torch.no_grad():
                onehot = torch.zeros_like(full_probs).scatter_(1, topi, 1.0)  # (B, E)
            f = onehot.mean(dim=0)                                  # (E,)
            P = full_probs.mean(dim=0)                              # (E,)
            aux_terms.append(self.n_experts * (f.detach() * P).sum())

            route_probs_log.append(P.detach())
            route_onehot_log.append(onehot.mean(dim=0).detach())

        self.last_aux_loss = torch.stack(aux_terms).mean()
        self.last_route_probs = torch.stack(route_probs_log)       # (N, E)
        self.last_route_onehot = torch.stack(route_onehot_log)     # (N, E)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        def _make_trunk():
            return ProgramTrunk(
                obs_dim, H, args.n_steps, args.n_experts, args.top_k, args.step_embed_dim
            )
    ...
    def _program_trunks(self):
        # The ProgramTrunk(s) whose stashed aux/route stats reflect the last forward.
        if self.share_backbone:
            return [self.trunk]
        return [self.actor_trunk, self.critic_trunk]

    def trunk_aux_loss(self):
        # Graph-connected Switch-style load-balance loss from the most recent forward,
        # averaged over active trunks. Caller multiplies by args.lb_coef.
        terms = [t.last_aux_loss for t in self._program_trunks()]
        return torch.stack(terms).mean()

    @torch.no_grad()
    def trunk_route_stats(self):
        # Detached (N, E) per-step router probs and top-k selection freqs, averaged
        # over active trunks. Basis for the emergent-reuse diagnostics.
        probs = torch.stack([t.last_route_probs for t in self._program_trunks()]).mean(0)
        onehot = torch.stack([t.last_route_onehot for t in self._program_trunks()]).mean(0)
        return probs, onehot
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Switch-style load-balance aux loss on the ProgramTrunk router (from
                # this minibatch's forward). Keeps the top-k experts from collapsing to
                # a few always-on subroutines; coef ~1e-2. Graph-connected to the trunk.
                lb_loss = agent.trunk_aux_loss()
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    # Fold the load-balance regularizer into the policy backward: it
                    # shapes the shared trunk's router and is clipped within the actor
                    # group's budget (the trunk is in actor_params).
                    (pg_loss - ent_coef_eff * entropy_loss + args.lb_coef * lb_loss).backward()
                    ...
                    loss = pg_loss - ent_coef_eff * entropy_loss + v_loss * args.vf_coef + args.lb_coef * lb_loss
                    optimizer.zero_grad()
                    loss.backward()
        ...
        writer.add_scalar("losses/load_balance", lb_loss.item(), global_step)

        # ---- Emergent reusable-circuitry diagnostics (from the last minibatch) ----
        # Computed under no_grad from the stashed per-step (N, E) router stats.
        with torch.no_grad():
            route_probs, route_onehot = agent.trunk_route_stats()  # (N, E) each
            n_steps, n_experts = route_onehot.shape
            top_k = args.top_k
            # (1) Cross-depth reuse rate. An expert is "active" at step t if it is
            # selected for >5% of the batch there. reuse_rate = mean over experts of
            # (#active steps / N); > 1/N => experts genuinely called at multiple depths
            # (the literal signature of a reused subroutine). max possible = top_k/E.
            active = (route_onehot > 0.05).float()                  # (N, E)
            steps_active = active.sum(dim=0)                        # (E,)
            reuse_rate = (steps_active / n_steps).mean().item()
            multi_depth_frac = (steps_active >= 2).float().mean().item()
            # (2a) Per-step router entropy (mean over steps), normalized to [0,1].
            #   low intra-step entropy + specialization => discrete subroutine calls.
            ent = -(route_probs * (route_probs + 1e-9).log()).sum(dim=1)  # (N,)
            route_entropy = (ent / math.log(n_experts)).mean().item()
            # (2b) Inter-step usage KL: mean pairwise symmetric KL between the top-k
            # selection distributions of distinct steps. HIGH => different program
            # lines call different subsets (a structured program, not one repeated op).
            sel = route_onehot / (route_onehot.sum(dim=1, keepdim=True) + 1e-9)  # (N, E)
            sel = sel + 1e-9
            logsel = sel.log()
            # KL(i||j) = sum sel_i*(log sel_i - log sel_j); symmetrize and average off-diagonal.
            kl = (sel.unsqueeze(1) * (logsel.unsqueeze(1) - logsel.unsqueeze(0))).sum(-1)  # (N,N)
            sym_kl = 0.5 * (kl + kl.transpose(0, 1))
            off = ~torch.eye(n_steps, dtype=torch.bool, device=sym_kl.device)
            inter_step_kl = sym_kl[off].mean().item() if n_steps > 1 else 0.0
        writer.add_scalar("circuits/reuse_rate", reuse_rate, global_step)
        writer.add_scalar("circuits/multi_depth_expert_frac", multi_depth_frac, global_step)
        writer.add_scalar("circuits/router_entropy_norm", route_entropy, global_step)
        writer.add_scalar("circuits/inter_step_usage_kl", inter_step_kl, global_step)
