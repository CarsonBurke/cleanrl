...
class Args:
    ...
    edge_gate_temp: float = 0.67
    """temperature for stochastic stretched hard-concrete edge gates"""
    edge_stretch_low: float = -0.1
    """lower stretch bound for hard-concrete edge gates"""
    edge_stretch_high: float = 1.1
    """upper stretch bound for hard-concrete edge gates"""
...
def hard_concrete_gate(logits, temperature, low, high, training, noise=None):
    if training:
        if noise is None:
            uniform = torch.rand_like(logits).clamp(1e-6, 1.0 - 1e-6)
            noise = torch.log(uniform) - torch.log1p(-uniform)
        gate = torch.sigmoid((logits + noise) / temperature)
    else:
        noise = None
        gate = torch.sigmoid(logits / temperature)
    gate_stretched = gate * (high - low) + low
    gate_clipped = gate_stretched.clamp(0.0, 1.0)
    gate = gate_clipped + gate_stretched - gate_stretched.detach()
    if low < 0.0 < high:
        expected_l0 = torch.sigmoid(logits - temperature * float(np.log(-low / high)))
    else:
        expected_l0 = torch.sigmoid(logits)
    return gate, expected_l0, noise
...
class MorphogenicSubstrate(nn.Module):
    ...
    def __init__(self, obs_dim, args):
        ...
        self.edge_gate_temp = args.edge_gate_temp
        self.edge_stretch_low = args.edge_stretch_low
        self.edge_stretch_high = args.edge_stretch_high
    ...
    def forward(self, x, edge_noise=None, lookback_edge_noise=None):
        B = x.shape[0]
        base = self.input(x)
        ...
        edge_gate, expected_edge, edge_noise = hard_concrete_gate(
            edge_logits,
            self.edge_gate_temp,
            self.edge_stretch_low,
            self.edge_stretch_high,
            self.training,
            edge_noise,
        )
        edge_gate = edge_gate * nonself[None, :, :]
        expected_edge = expected_edge * nonself[None, :, :]

        route_logits = geometry_logits + torch.log(active[:, None, :] + 1e-6)
        route_logits = route_logits.masked_fill(eye[None, :, :].bool(), -1e9)
        route_exp = torch.exp(route_logits - route_logits.max(dim=-1, keepdim=True).values)
        route_unnorm = edge_gate * route_exp
        route = route_unnorm / (route_unnorm.sum(dim=-1, keepdim=True) + 1e-6)
        ...
        history = []
        lookback_noise_chunks = []
        lookback_noise_cursor = 0
        active_lookback_edges = x.new_zeros(B)
        expected_active_lookback_edges = x.new_zeros(B)
        lookback_edge_entropy_sum = x.new_zeros(B)
        expected_lookback_edge_entropy_sum = x.new_zeros(B)
        ...
        for t in range(self.T):
            ...
            if history:
                ...
                noise_slice = None
                if lookback_edge_noise is not None:
                    noise_slice = lookback_edge_noise[
                        :, lookback_noise_cursor : lookback_noise_cursor + history_count, :, :
                    ]
                lookback_gate, expected_lookback, noise_slice = hard_concrete_gate(
                    lookback_logits,
                    self.edge_gate_temp,
                    self.edge_stretch_low,
                    self.edge_stretch_high,
                ...
                if noise_slice is not None:
                    lookback_noise_chunks.append(noise_slice)
                lookback_noise_cursor += history_count

                source_activity = active[:, None, None, :]
                lookback_route_logits = lookback_logits + torch.log(source_activity + 1e-6)
                lookback_route_exp = torch.exp(
                    lookback_route_logits
                    - lookback_route_logits.amax(dim=(1, 3), keepdim=True)
                )
                lookback_unnorm = lookback_gate * lookback_route_exp
                lookback_route = lookback_unnorm / (lookback_unnorm.sum(dim=(1, 3), keepdim=True) + 1e-6)
                lookback_msg = torch.einsum("blij,bljd->bid", lookback_route, past_norm)

                tick_gate = tick_gates[:, t]
                lookback_gate_metric = lookback_gate.clamp(0.0, 1.0)
                active_lookback_edges = active_lookback_edges + tick_gate * lookback_gate_metric.sum(dim=(1, 2, 3))
                expected_active_lookback_edges = (
                    expected_active_lookback_edges + tick_gate * expected_lookback.sum(dim=(1, 2, 3))
                )
                lookback_edge_entropy = -(
                    lookback_gate_metric * torch.log(lookback_gate_metric + 1e-8)
                    + (1.0 - lookback_gate_metric) * torch.log(1.0 - lookback_gate_metric + 1e-8)
                )
                expected_lookback_edge_entropy = -(
                    expected_lookback * torch.log(expected_lookback + 1e-8)
                    + (1.0 - expected_lookback) * torch.log(1.0 - expected_lookback + 1e-8)
                )
                lookback_edge_entropy_sum = (
                    lookback_edge_entropy_sum + tick_gate * lookback_edge_entropy.sum(dim=(1, 2, 3))
                )
                expected_lookback_edge_entropy_sum = (
                    expected_lookback_edge_entropy_sum
                    + tick_gate * expected_lookback_edge_entropy.sum(dim=(1, 2, 3))
                )
                lookback_route_entropy = -(lookback_route * torch.log(lookback_route + 1e-8)).sum(dim=(1, 3))
        ...
        edge_gate_metric = edge_gate.clamp(0.0, 1.0)
        active_edges = edge_gate_metric.sum(dim=(1, 2))
        active_edge_frac = active_edges / max(self.K * (self.K - 1), 1)
        expected_active_edges = expected_edge.sum(dim=(1, 2))
        ...
        edge_entropy = -(
            edge_gate_metric * torch.log(edge_gate_metric + 1e-8)
            + (1.0 - edge_gate_metric) * torch.log(1.0 - edge_gate_metric + 1e-8)
        )
        edge_entropy = (edge_entropy * nonself[None, :, :]).sum(dim=(1, 2)) / max(self.K * (self.K - 1), 1)
        expected_edge_entropy = -(
            expected_edge * torch.log(expected_edge + 1e-8)
            + (1.0 - expected_edge) * torch.log(1.0 - expected_edge + 1e-8)
        )
        expected_edge_entropy = (
            expected_edge_entropy * nonself[None, :, :]
        ).sum(dim=(1, 2)) / max(self.K * (self.K - 1), 1)
        lookback_edge_entropy = lookback_edge_entropy_sum / lookback_edge_capacity
        expected_lookback_edge_entropy = expected_lookback_edge_entropy_sum / lookback_edge_capacity
        ...
        if lookback_noise_chunks:
            lookback_edge_noise_out = torch.cat(lookback_noise_chunks, dim=1)
        else:
            lookback_edge_noise_out = x.new_empty(B, 0, self.K, self.K)
        ...
        stats = {
            ...
            "lookback_edge_noise": lookback_edge_noise_out,
...
class Agent(nn.Module):
    ...
    def get_value(self, x, critic_edge_noise=None, critic_lookback_edge_noise=None):
        critic_features, _ = self.critic(x, critic_edge_noise, critic_lookback_edge_noise)
    ...
    def get_action_and_value(
        ...
        x,
        z=None,
        actor_edge_noise=None,
        critic_edge_noise=None,
        actor_lookback_edge_noise=None,
        critic_lookback_edge_noise=None,
    ):
        actor_features, actor_stats = self.actor(x, actor_edge_noise, actor_lookback_edge_noise)
        critic_features, critic_stats = self.critic(x, critic_edge_noise, critic_lookback_edge_noise)
        ...
        return (
            ...
            actor_stats["edge_noise"],
            critic_stats["edge_noise"],
            actor_stats["lookback_edge_noise"],
            critic_stats["lookback_edge_noise"],
            stats,
        )
...
if __name__ == "__main__":
    ...
    lookback_edge_pairs = args.max_ticks * (args.max_ticks - 1) // 2
    obs = torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space.shape).to(device)
    zs = torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device)
    actor_edge_noises = torch.zeros((args.num_steps, args.num_envs, args.max_cells, args.max_cells)).to(device)
    critic_edge_noises = torch.zeros((args.num_steps, args.num_envs, args.max_cells, args.max_cells)).to(device)
    lookback_noise_shape = (args.num_steps, args.num_envs, lookback_edge_pairs, args.max_cells, args.max_cells)
    actor_lookback_edge_noises = torch.empty(lookback_noise_shape, dtype=torch.float32)
    critic_lookback_edge_noises = torch.empty(lookback_noise_shape, dtype=torch.float32)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                (
                    ...
                    actor_edge_noise,
                    critic_edge_noise,
                    actor_lookback_edge_noise,
                    critic_lookback_edge_noise,
                    last_stats,
                ) = agent.get_action_and_value(next_obs)
                values[step] = value.flatten()
            zs[step] = z
            actor_edge_noises[step] = actor_edge_noise
            critic_edge_noises[step] = critic_edge_noise
            actor_lookback_edge_noises[step].copy_(actor_lookback_edge_noise.cpu())
            critic_lookback_edge_noises[step].copy_(critic_lookback_edge_noise.cpu())
        ...
        b_actor_edge_noises = actor_edge_noises.reshape((-1, args.max_cells, args.max_cells))
        b_critic_edge_noises = critic_edge_noises.reshape((-1, args.max_cells, args.max_cells))
        b_actor_lookback_edge_noises = actor_lookback_edge_noises.reshape(
            (-1, lookback_edge_pairs, args.max_cells, args.max_cells)
        )
        b_critic_lookback_edge_noises = critic_lookback_edge_noises.reshape(
            (-1, lookback_edge_pairs, args.max_cells, args.max_cells)
        )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                (
                    ...
                    b_obs[mb_inds],
                    b_zs[mb_inds],
                    b_actor_edge_noises[mb_inds],
                    b_critic_edge_noises[mb_inds],
                    b_actor_lookback_edge_noises[mb_inds].to(device),
                    b_critic_lookback_edge_noises[mb_inds].to(device),
                )
    ...
    if args.save_model:
        ...
        class EvalAgent(Agent):
            ...
            def get_action_and_value(self, x, z=None):
                action, _, logprob, entropy, value, _, _, _, _, _, _, _ = super().get_action_and_value(x, z)
