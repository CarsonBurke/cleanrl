# PPO + action-conditioned future-reward innovation Q-Prop, v1 (RNG-paired).
#
# A state critic predicts the raw future-reward path and scalar tail. A separate cheap
# critic learns the local action slope of every future coordinate from realized suffix
# residuals. PPO uses that frozen action effect as an exact Q-Prop control variate outside
# clipping: a sampled importance term minus its analytic expectation under the new Beta
# policy. The population correction is identically zero, while its gradient removes
# action-correlated noise. No reward normalization or model
# rollout is used; the state critic, action field, and actor have isolated gradient paths.
import os
import random
import time
...
class Args:
    ...
    prefix_return_loss_coef: float = 1.0
    mc_window: int = 500             # gamma^500 ~= .0066: independent truncated-MC EV gate

    # Action-conditioned successor innovation. The field is affine in physical action,
    # which is the exact local model appropriate inside PPO's trust region and permits
    # an analytic Beta-policy expectation without hand-built action monomials.
    innovation_rank: int = 8
    innovation_hidden: int = 64
    innovation_learning_rate: float = 3e-4
    innovation_grad_clip: float = 0.25
    qprop_coef: float = 1.0
...
def tail_innovation_target(
    current_tails, next_last_rewards, next_tails, terminations, valids, gamma
):
    """Action innovation for the return beginning at the prediction horizon."""
    valid = target_valid_mask(terminations, valids)
    bootstrap = (1.0 - terminations) * valids
    target = bootstrap * (next_last_rewards + gamma * next_tails) - current_tails
    return torch.where(valid, target, torch.zeros_like(target)), valid
...
def build_action_future_reward_targets(rewards, terminations, boundaries, horizon):
    """Future rewards for an action-value target, including known zeros after termination."""
    time_steps, num_envs = rewards.shape
    targets = rewards.new_zeros((time_steps, num_envs, horizon))
    valid = torch.zeros_like(targets, dtype=torch.bool)
    # Status per source row: 0 is inside the episode, 1 is after a true terminal,
    # and 2 is after a time-limit/unknown boundary. Only status 2 is censored.
    status = torch.zeros((time_steps, num_envs), dtype=torch.int8, device=rewards.device)
    source = torch.arange(time_steps, device=rewards.device)
    for offset in range(min(horizon, time_steps)):
        available = source + offset < time_steps
        active = available[:, None] & (status == 0)
        terminal_zero = status == 1
        shifted_index = (source + offset).clamp_max(time_steps - 1)
        shifted_rewards = rewards[shifted_index]
        targets[:, :, offset] = torch.where(active, shifted_rewards, 0.0)
        valid[:, :, offset] = active | terminal_zero
        shifted_terminations = terminations[shifted_index].bool()
        shifted_boundaries = boundaries[shifted_index].bool()
        status = torch.where(active & shifted_terminations, 1, status)
        status = torch.where(
            active & shifted_boundaries & ~shifted_terminations,
            2,
            status,
        )
    return targets, valid
...
def discounted_projection_mse(prediction, target, valid, discount_weights):
    """Penalize correlated vector error in the final objective direction."""
    row_valid = valid.all(dim=-1)
    if not row_valid.any():
        return prediction.sum() * 0.0
    predicted_return = prediction[row_valid] @ discount_weights
    target_return = target[row_valid] @ discount_weights
    return F.mse_loss(predicted_return, target_return)


def masked_standardized_mse(prediction, target, valid, scale=None):
    """Dimensionless MSE using only the supervised population's target scale."""
    if not valid.any():
        return prediction.sum() * 0.0
    if scale is None:
        scale = target[valid].std(unbiased=False).clamp_min(1e-4).detach()
    standardized_error = (prediction - target) / scale
    return standardized_error[valid].square().mean()


def masked_feature_scale(target, valid):
    """Per-coordinate rollout scale without allowing masked zeros into moments."""
    count = valid.sum(dim=0).clamp_min(1)
    mean = (target * valid).sum(dim=0) / count
    variance = ((target - mean).square() * valid).sum(dim=0) / count
    return variance.sqrt().clamp_min(1e-4).detach()


def qprop_control_variate_loss(
    ratio, sampled_innovation, expected_new_innovation, advantage_scale, coefficient
):
    """Population-zero Q-Prop term kept outside PPO's nonlinear clipping."""
    return coefficient * (
        (ratio * sampled_innovation).mean() - expected_new_innovation.mean()
    ) / advantage_scale.detach()


def masked_r2(prediction, target, valid):
    if not valid.any():
        return prediction.new_tensor(float("nan"))
    valid_prediction = prediction[valid]
    valid_target = target[valid]
    variance = valid_target.var(unbiased=False)
    if variance <= 1e-12:
        return prediction.new_tensor(float("nan"))
    return 1.0 - (valid_prediction - valid_target).square().mean() / variance
...
class ActionInnovation(nn.Module):
    """State-conditioned local action slopes for the future-reward path and tail."""

    def __init__(self, obs_dim, act_dim, horizon, rank, hidden, discount_weights, tail_discount):
        super().__init__()
        ...
        self.coefficient_slope_head = layer_init(
            nn.Linear(hidden, rank * act_dim), std=0.01
        )
        self.temporal_basis = layer_init(nn.Linear(rank, horizon, bias=False), std=0.1)
        self.tail_slope_head = layer_init(nn.Linear(hidden, act_dim), std=0.01)
        with torch.no_grad():
            self.coefficient_slope_head.weight.zero_()
            self.coefficient_slope_head.bias.zero_()
            self.tail_slope_head.weight.zero_()
            self.tail_slope_head.bias.zero_()
        self.register_buffer("discount_weights", discount_weights.detach().clone())
        self.tail_discount = tail_discount

    def _features_and_coefficient_slopes(self, observations):
        features = self.encoder(observations)
        return features, self.coefficient_slope_head(features).reshape(
            -1, self.rank, self.act_dim
        )

    def slopes(self, observations):
        features, coefficient_slopes = self._features_and_coefficient_slopes(observations)
        future_slopes = torch.einsum(
            "hr,bra->bha", self.temporal_basis.weight, coefficient_slopes
        )
        tail_slopes = self.tail_slope_head(features)
        total_slopes = torch.einsum(
            "h,bha->ba", self.discount_weights, future_slopes
        ) + self.tail_discount * tail_slopes
        return future_slopes, tail_slopes, total_slopes

    def total(self, observations, actions, behavior_means):
        features = self.encoder(observations)
        coefficient_slopes = self.coefficient_slope_head(features).reshape(
            -1, self.rank, self.act_dim
        )
        discounted_basis = self.discount_weights @ self.temporal_basis.weight
        total_slopes = torch.einsum(
            "r,bra->ba", discounted_basis, coefficient_slopes
        ) + self.tail_discount * self.tail_slope_head(features)
        total_innovations = (total_slopes * (actions - behavior_means)).sum(dim=-1)
        return total_innovations, total_slopes

    def forward(self, observations, actions, behavior_means):
        future_slopes, tail_slopes, total_slopes = self.slopes(observations)
        centered_actions = actions - behavior_means
        future_innovations = torch.einsum(
            "bha,ba->bh", future_slopes, centered_actions
        )
        tail_innovations = (tail_slopes * centered_actions).sum(dim=-1)
        total_innovations = (total_slopes * centered_actions).sum(dim=-1)
        return future_innovations, tail_innovations, total_innovations, total_slopes
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, z=None):
        ...
        action = to_action(z)
        action_mean = to_action(dist.mean)
        ...
        return action, z, log_prob, entropy, value, future_rewards, action_mean
...
if __name__ == "__main__":
    ...
    assert args.innovation_rank > 0
    assert args.mc_window > 0
    if args.actor_dist != "beta":
        raise ValueError("reward_innovation_qprop_v1 requires the Beta actor")
    ...
    act_dim = int(np.prod(envs.single_action_space.shape))
    # Keep the treatment's initialization from perturbing the paired actor or global RNG.
    with torch.random.fork_rng(devices=[]):
        action_innovation = ActionInnovation(
            obs_dim,
            act_dim,
            args.prediction_horizon,
            args.innovation_rank,
            args.innovation_hidden,
            agent.discount_weights,
            agent.tail_discount,
        ).to(device)
    if args.compile:
    ...
    innovation_optimizer = optim.Adam(
        action_innovation.parameters(), lr=args.innovation_learning_rate, eps=1e-5
    )
    ...
    action_means = torch.zeros_like(actions)
    ...
    for iteration in range(1, args.num_iterations + 1):
        if args.anneal_lr:
            ...
            innovation_optimizer.param_groups[0]["lr"] = (
                frac * args.innovation_learning_rate
            )
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, z, logprob, ent, value, _, action_mean = (
        ...
        with torch.no_grad():
            ...
            action_reward_vector_targets, action_reward_vector_valid = (
                build_action_future_reward_targets(
                    rewards,
                    transition_terminations,
                    transition_boundaries,
                    args.prediction_horizon,
        ...
        b_action_reward_vector_targets = action_reward_vector_targets.reshape(
            -1, args.prediction_horizon
        )
        b_action_reward_vector_valid = action_reward_vector_valid.reshape(
            -1, args.prediction_horizon
        )
        ...
        with torch.no_grad():
            _, b_old_future_predictions, b_old_tails, _ = agent.get_critic(b_obs)
            b_old_action_innovations, b_old_action_slopes = action_innovation.total(
                b_obs, b_actions, b_action_means
            )
            b_future_innovation_targets = (
                b_action_reward_vector_targets - b_old_future_predictions
            )
            b_tail_innovation_targets, b_tail_innovation_valid = (
                tail_innovation_target(
                    b_old_tails,
                    b_next_future_predictions[:, -1],
                    b_next_tails,
                    transition_terminations.reshape(-1),
                    transition_valids.reshape(-1),
                    args.gamma,
                )
            )
            b_innovation_future_valid = (
                b_action_reward_vector_valid & b_actor_valid.unsqueeze(-1)
            )
            b_innovation_future_scale = masked_feature_scale(
                b_future_innovation_targets, b_innovation_future_valid
            )
            if b_tail_innovation_valid.any():
                b_innovation_tail_scale = b_tail_innovation_targets[
                    b_tail_innovation_valid
                ].std(unbiased=False).clamp_min(1e-4)
            else:
                b_innovation_tail_scale = torch.ones((), device=device)
            b_projected_innovation_targets = (
                b_future_innovation_targets @ agent.discount_weights
                + agent.tail_discount * b_tail_innovation_targets
            )
            b_projected_innovation_valid = (
                b_innovation_future_valid.all(dim=-1)
                & b_tail_innovation_valid
            )
        ...
        if args.norm_adv and args.norm_adv_scope == "batch":
            ...
            valid_advantages = b_advantages[b_actor_valid]
            b_adv_scale = valid_advantages.std(unbiased=False).clamp_min(1e-8)
        else:
            b_adv_scale = torch.ones((), device=device)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, prefix_predictions, _, _ = agent.get_critic(b_obs[mb_inds])
                prefix_return_loss = discounted_projection_mse(
                    prefix_predictions,
                ...
                weighted_prefix_loss = (
                    args.prefix_return_loss_coef * prefix_return_loss
                )
                if args.separate_grad_clip:
                    weighted_prefix_loss.backward()
                    prefix_gn = nn.utils.clip_grad_norm_(
                        critic_params, args.prefix_grad_clip
                    )
                    prefix_grads = [
                        (parameter, parameter.grad.detach().clone())
                        for parameter in critic_params
                ...
                v_loss = scalar_value_loss + weighted_prefix_loss + predictive_loss
                ...
                (
                    predicted_future_innovations,
                    predicted_tail_innovations,
                    _,
                    _,
                ) = action_innovation(
                    b_obs[mb_inds],
                ...
                innovation_future_loss = masked_standardized_mse(
                    predicted_future_innovations,
                    b_future_innovation_targets[mb_inds],
                    b_innovation_future_valid[mb_inds],
                    b_innovation_future_scale,
                )
                innovation_tail_loss = masked_standardized_mse(
                    predicted_tail_innovations,
                    b_tail_innovation_targets[mb_inds],
                    b_tail_innovation_valid[mb_inds],
                    b_innovation_tail_scale,
                )
                innovation_loss = innovation_future_loss + innovation_tail_loss
                innovation_optimizer.zero_grad(set_to_none=True)
                innovation_loss.backward()
                innovation_gn = nn.utils.clip_grad_norm_(
                    action_innovation.parameters(), args.innovation_grad_clip
                )
                innovation_optimizer.step()

                _, _, newlogprob, entropy, _, _, new_action_mean = agent.get_action_and_value(
                ...
                if args.norm_adv:
                    if args.norm_adv_scope == "batch":
                        ...
                        mb_adv_scale = b_adv_scale
                    else:
                        raw_mb_advantages = b_advantages[mb_inds][mb_actor_valid]
                        mb_advantages = normalize_valid_advantages(
                            raw_mb_advantages,
                            torch.ones_like(
                                raw_mb_advantages, dtype=torch.bool
                            ),
                        )
                        mb_adv_scale = raw_mb_advantages.std(
                            unbiased=False
                        ).clamp_min(1e-8)
                else:
                    mb_advantages = b_advantages[mb_inds][mb_actor_valid]
                    mb_adv_scale = torch.ones((), device=device)
                ...
                clipped_pg_loss = torch.max(pg_loss1, pg_loss2).mean()
                frozen_sample_innovation = b_old_action_innovations[mb_inds][mb_actor_valid]
                expected_new_innovation = (
                    b_old_action_slopes[mb_inds][mb_actor_valid]
                    * (
                        new_action_mean[mb_actor_valid]
                        - b_action_means[mb_inds][mb_actor_valid]
                    )
                ).sum(dim=-1)
                qprop_cv_loss = qprop_control_variate_loss(
                    ratio,
                    frozen_sample_innovation,
                    expected_new_innovation,
                    mb_adv_scale,
                    args.qprop_coef,
                )
                pg_loss = clipped_pg_loss + qprop_cv_loss
                ...
                with torch.no_grad():
                    ...
                    acc["prefix_return"] += prefix_return_loss.item()
                    acc["pg"] += pg_loss.item()
                    acc["innovation_future"] += innovation_future_loss.item()
                    acc["innovation_tail"] += innovation_tail_loss.item()
                    acc["innovation_gn"] += float(innovation_gn)
                    acc["qprop_cv"] += qprop_cv_loss.item()
                    acc["n"] += 1
                ...
                if args.separate_grad_clip:
                    ...
                    for parameter, gradient in (
                        value_grads + prefix_grads + predictive_grads
                    ):
                        parameter.grad = (
                            gradient
        ...
        writer.add_scalar(
            ...
            (
                ...
                + args.prefix_return_loss_coef * acc["prefix_return"]
        ...
        writer.add_scalar(
            "losses/prefix_return", acc["prefix_return"] / n_mb, global_step
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "losses/innovation_future", acc["innovation_future"] / n_mb, global_step
        )
        writer.add_scalar(
            "losses/innovation_tail", acc["innovation_tail"] / n_mb, global_step
        )
        writer.add_scalar("losses/qprop_cv", acc["qprop_cv"] / n_mb, global_step)
        ...
        writer.add_scalar(
            "losses/prefix_grad_norm", acc["prefix_gn"] / n_mb, global_step
        )
        writer.add_scalar(
            "losses/innovation_grad_norm", acc["innovation_gn"] / n_mb, global_step
        )
        with torch.no_grad():
            ...
            full_target_rows = b_reward_vector_valid.all(dim=-1)
            target_rank_sample = b_reward_vector_targets[full_target_rows]
            if target_rank_sample.shape[0] > 1:
                target_rank_stride = max(target_rank_sample.shape[0] // 4096, 1)
                target_rank_sample = target_rank_sample[::target_rank_stride]
                centered_targets = target_rank_sample - target_rank_sample.mean(dim=0)
                target_singular_values = torch.linalg.svdvals(centered_targets.float())
                target_singular_weights = target_singular_values.square()
                target_singular_weights = target_singular_weights / target_singular_weights.sum().clamp_min(1e-8)
                target_effective_rank = torch.exp(
                    -(
                        target_singular_weights
                        * target_singular_weights.clamp_min(1e-8).log()
                    ).sum()
                )
            else:
                target_effective_rank = diagnostic_future.new_tensor(float("nan"))
            ...
            fitted_action_innovations, fitted_action_slopes = action_innovation.total(
                b_obs, b_actions, b_action_means
            )
            valid_actor_advantages = b_advantages[b_actor_valid]
            valid_old_innovations = b_old_action_innovations[b_actor_valid]
            centered_advantages = valid_actor_advantages - valid_actor_advantages.mean()
            centered_innovations = valid_old_innovations - valid_old_innovations.mean()
            innovation_correlation = (
                (centered_advantages * centered_innovations).mean()
                / (
                    centered_advantages.square().mean().sqrt()
                    * centered_innovations.square().mean().sqrt()
                ).clamp_min(1e-8)
            )
            residual_variance_ratio = (
                (valid_actor_advantages - valid_old_innovations).var(unbiased=False)
                / valid_actor_advantages.var(unbiased=False).clamp_min(1e-8)
            )
            innovation_projected_r2 = masked_r2(
                fitted_action_innovations,
                b_projected_innovation_targets,
                b_projected_innovation_valid,
            )
        ...
        for bucket_start, bucket_end in ((0, 8), (8, 32), (32, 64), (64, 128)):
            ...
            writer.add_scalar(
                f"diagnostics/future_reward_r2_{bucket_start}_{bucket_end}",
                bucket_r2.item(),
                global_step,
            )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "diagnostics/future_target_effective_rank",
            target_effective_rank.item(),
            global_step,
        )
        writer.add_scalar(
        ...
        writer.add_scalar(
            "diagnostics/innovation_gae_correlation",
            innovation_correlation.item(),
            global_step,
        )
        writer.add_scalar(
            "diagnostics/innovation_residual_variance_ratio",
            residual_variance_ratio.item(),
            global_step,
        )
        writer.add_scalar(
            "diagnostics/innovation_projected_target_r2",
            innovation_projected_r2.item(),
            global_step,
        )
        writer.add_scalar(
            "diagnostics/innovation_abs_mean",
            b_old_action_innovations.abs().mean().item(),
            global_step,
        )
        writer.add_scalar(
            "diagnostics/innovation_slope_norm",
            fitted_action_slopes.norm(dim=-1).mean().item(),
            global_step,
        )
