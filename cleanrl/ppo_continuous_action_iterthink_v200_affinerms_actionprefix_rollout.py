# PPO + IterThink v200 affine-RMS action-prefix rollout world model.
#
# Keep v198's observation-only spatial encoder, attached encoder SIGReg,
# action-free temporal policy memory, separate detached actor/critic readouts,
# outbound-action transition queries, Beta policy, and exact-KL rollback. Add a
# fully attached H-step replay-action rollout through the shared transition model:
# z_t --a_t--> zhat_{t+1} --a_{t+1}--> ... . Every predicted observation token,
# reward, and continuation is supervised at every depth while gradients propagate
# through all earlier predicted states. The stable teacher-forced one-step loss is
# retained as an anchor. There is no prediction SIGReg, EMA, target detach,
# representation normalization, or flow objective.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class Args:
    ...
    # The attached H-step prefix retains every transition graph. Accumulate four
    # 256-row microbatches to preserve the proven 1024-row effective update.
    ...
    lewm_prefix_loss_coef: float = 0.5  # fully attached replay-action rollout loss
...
class LeWMBackbone(nn.Module):
    ...
    def rollout_action_prefix(self, initial_obs_tokens, action_prefix):
        """Roll out an entire replay-action prefix without severing its graph.

        The initial encoder tokens, every predicted observation token set, and all
        shared transition applications remain attached. This is deliberately the
        same one-step transition used by imagination; the only difference is that
        replay supplies the future action prefix and labels during WM training.
        """
        latent_history = initial_obs_tokens.unsqueeze(1)
        predicted_summaries = []
        reward_logits = []
        continuation_logits = []
        for h in range(action_prefix.shape[1]):
            action = action_prefix[:, h : h + 1]
            state_memory = self._temporal_state_trunk(latent_history)
            features = self.transition_readout(
                state_memory[:, -1:], self.action_cond(action)
            )[:, -1]
            next_summary = self.pred_next_proj(features)
            next_reward_logits, next_continuation_logits = self.decode_belief_outcomes(
                next_summary, action[:, 0], detach_belief=False
            )
            predicted_summaries.append(next_summary)
            reward_logits.append(next_reward_logits)
            continuation_logits.append(next_continuation_logits)
            # No detach: later losses backpropagate through the complete prefix.
            latent_history = torch.cat(
                [latent_history, next_summary[:, self.obs_token_start :].unsqueeze(1)],
                dim=1,
            )
        return (
            torch.stack(predicted_summaries, dim=1),
            torch.stack(reward_logits, dim=1),
...
if __name__ == "__main__":
    ...
    assert args.lewm_prefix_loss_coef >= 0.0, "v200 prefix loss coefficient must be nonnegative"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        wm_prefix_latent_losses = []
        wm_prefix_reward_losses = []
        wm_prefix_termination_losses = []
        wm_sigreg_losses = []
        wm_grad_norms = []
        ...
        if args.lewm_loss_coef > 0.0:
            ...
            for _ in range(args.lewm_update_epochs):
                ...
                for start in range(0, args.batch_size, wm_minibatch_size):
                    ...
                    # v200: train the exact path consumed by imagination on complete
                    # replay-action prefixes. Only the first observation is real;
                    # every later observation input is the preceding prediction.
                    # Neither encoder endpoint nor any recurrent prediction is
                    # detached, so a depth-h error trains all transitions 1..h.
                    (
                        prefix_summaries,
                        prefix_reward_logits,
                        prefix_continuation_logits,
                    ) = agent.world_model.rollout_action_prefix(
                        initial_summary[:, obs_start:], future_actions
                    )
                    prefix_valid = latent_weight
                    prefix_denom = prefix_valid.sum().clamp_min(1.0)
                    prefix_latent_mse = F.mse_loss(
                        prefix_summaries[:, :, obs_start:],
                        target_summaries[:, :, obs_start:],
                        reduction="none",
                    ).mean(dim=(-1, -2))
                    prefix_reward_ce = -(
                        reward_target_probs.detach()
                        * torch.log_softmax(prefix_reward_logits, dim=-1)
                    ).sum(dim=-1)
                    prefix_continuation_bce = F.binary_cross_entropy_with_logits(
                        prefix_continuation_logits,
                        1.0 - future_terminations,
                        reduction="none",
                    )
                    wm_prefix_latent_loss = (
                        prefix_latent_mse * prefix_valid
                    ).sum() / prefix_denom
                    wm_prefix_reward_loss = (
                        prefix_reward_ce * prefix_valid
                    ).sum() / prefix_denom
                    wm_prefix_termination_loss = (
                        prefix_continuation_bce * prefix_valid
                    ).sum() / prefix_denom
                    ...
                    wm_loss = (
                        ...
                        + args.lewm_prefix_loss_coef
                        * (
                            args.lewm_loss_coef * wm_prefix_latent_loss
                            + args.lewm_reward_loss_coef * wm_prefix_reward_loss
                            + args.lewm_termination_loss_coef * wm_prefix_termination_loss
                        )
                    )
                    if wm_accum_count == 0:
                    ...
                    wm_prefix_latent_losses.append(wm_prefix_latent_loss.item())
                    wm_prefix_reward_losses.append(wm_prefix_reward_loss.item())
                    wm_prefix_termination_losses.append(wm_prefix_termination_loss.item())
        ...
        if world_model_only:
            ...
            if wm_losses:
                ...
                writer.add_scalar("lewm/prefix_latent_mse", float(np.mean(wm_prefix_latent_losses)), global_step)
                writer.add_scalar("lewm/prefix_reward_ce", float(np.mean(wm_prefix_reward_losses)), global_step)
                writer.add_scalar(
                    "lewm/prefix_continuation_bce",
                    float(np.mean(wm_prefix_termination_losses)),
                    global_step,
                )
