...
#
# SDNOISE v3 — antithetic sigma-gradient on top of v2's OU + decouple mechanisms.
#
# Base: td7_sdnoise_v2 (SAC state-dependent log_std head on TD7 + two orthogonal flags: noise_corr
# AR(1)/OU exploration and decouple_greedy; see that file and v1). v3 adds ONE more orthogonal,
# independently-flagged mechanism, keeping noise_corr and decouple_greedy unchanged.
#
# (C) antithetic_sigma (default True): in v1/v2 the log_std-head gradient comes from a SINGLE noisy
# sample Q(a_center + sigma*eps). Per sample that gradient is dominated by the first-order term
# eps_i * dQ/da_i — zero-mean but with magnitude ~|dQ/da_i|*sigma_i — which buries the SECOND-order
# curvature signal sigma_i^2 * H_ii that the head is actually meant to follow (its equilibrium is
# sigma_i^2 = alpha/|H_ii|). An antithetic pair Q(a_center + sigma*eps) and Q(a_center - sigma*eps)
# evaluated with the SAME eps cancels every ODD-order term in sigma*eps exactly, pathwise; averaging
# the two leaves a near-pure, low-variance curvature signal per update. Cost is one extra critic
# forward (still ONE randn draw). Prediction: the sigma map differentiates much earlier (v1 needed
# ~40k steps to open its 100x sigma_min/sigma_max spread) and the arm tracks-or-beats v1 throughout;
# charts/sigma_spread = log10(sigma_max/sigma_min) tracks this. Combines with decouple_greedy exactly
# as v2: decoupled -> a_center = mu.detach() (log_std-only gradient); coupled -> a_center = mu, so the
# antithetic average ALSO symmetrizes mu's gradient toward the deterministic one (an O(sigma^2) side
# effect, acceptable). antithetic_sigma=False recovers v2 exactly (single-sample, same RNG draw).
#
# --- v2 mechanisms (unchanged) ---
# v1's diagnostics show sigma is genuinely state-dependent (sigma_min ~ 0.01, sigma_max ~ 1.0,
# sigma_mean ~ 0.13 stable), i.e. the per-step noise budget is already allocated near-optimally
# across states — curvature-matched sigma_i^2 ~ alpha/|H_ii| is the optimal per-step allocation. But
# that noise is WHITE: independent each step. Over a locomotion stride the per-step perturbations
# average out, so at sigma ~ 0.13 exploration barely displaces the trajectory and the policy stops
# discovering new state regions — the diagnosed 8M bottleneck.
#
# The two v2 flags (both default-off-able, so this file also serves the OU-only/decouple-only arms):
#
...
#
# Everything else — entropy/alpha autotune, all value targets, LAP, checkpointing — is byte-identical
# to v1. With noise_corr=0 AND decouple_greedy=False AND antithetic_sigma=False the file reduces to
# v1 exactly; antithetic_sigma=False alone recovers v2 exactly.
#
...
# gives a small late-game EVAL gain (unbiased greedy target), within noise early. (C) antithetic
# opens the sigma_spread earlier than v1's ~40k and tracks-or-beats v1 throughout. Watch-item: TD7
...
class Args:
    ...
    antithetic_sigma: bool = True
    """v3 (orthogonal): estimate the log_std-head gradient from an antithetic pair
    0.5*(Q(a_center+sigma*eps)+Q(a_center-sigma*eps)) with a SHARED eps, cancelling the odd-order
    (first-order) noise that buries the sigma^2*H curvature signal. False recovers v2 exactly"""
...
class TD7Agent:
    ...
    def train(self):
        ...
        if self.training_steps % self.args.policy_freq == 0:
            ...
            if self.args.sd_noise:
                # Exploration-sample center: detached in decoupled mode (mu is trained separately on
                # the deterministic greedy Q below, so the noisy eval must not move it), else mu
                # itself (coupled: the sample also carries mu's gradient, as in v1/v2).
                a_center = mu.detach() if self.args.decouple_greedy else mu
                eps = torch.randn_like(mu)  # ONE draw, reused for both antithetic branches
                sigma = log_std.exp()
                if self.args.antithetic_sigma:
                    # Antithetic pair with a SHARED eps: Q(a_center + sigma*eps) and
                    # Q(a_center - sigma*eps). Averaging cancels ALL odd-order terms in sigma*eps
                    # pathwise -- in particular the first-order eps_i*dQ/da_i noise (magnitude
                    # ~|dQ/da_i|*sigma_i) that buries the curvature signal sigma_i^2*H_ii the log_std
                    # head follows (equilibrium sigma_i^2 = alpha/|H_ii|). The remaining even-order
                    # signal gives a near-pure, low-variance curvature gradient per update. Same ONE
                    # randn draw as the single-sample path (RNG-count parity with v2).
                    a_plus = a_center + sigma * eps
                    a_minus = a_center - sigma * eps
                    zsa_plus = self.fixed_encoder.zsa(fixed_zs, a_plus)
                    zsa_minus = self.fixed_encoder.zsa(fixed_zs, a_minus)
                    noisy_Q = 0.5 * (
                        self.critic(state, a_plus, zsa_plus, fixed_zs)
                        + self.critic(state, a_minus, zsa_minus, fixed_zs)
                    )
                else:
                    # v2 single-sample noisy eval a_r = a_center + sigma*eps (unclamped; mu in (-1,1)
                    # plus small Gaussian noise, well inside the critic's domain).
                    noisy_action = a_center + sigma * eps
                    noisy_zsa = self.fixed_encoder.zsa(fixed_zs, noisy_action)
                    noisy_Q = self.critic(state, noisy_action, noisy_zsa, fixed_zs)
                if self.args.decouple_greedy:
                    # mu ascends the DETERMINISTIC Q(s, mu) (exact baseline DPG fixed point); log_std
                    # gets its gradient from the (a_center-detached) noisy term. Per-parameter each
                    # gets exactly one -Q gradient.
                ...
                # Baseline TD7 DPG (no state-dependent noise): deterministic actor action, no entropy.
                actor_action = mu
            ...
            if self.args.sd_noise and self.training_steps % 1000 == 0:
                with torch.no_grad():
                    sigma = log_std.exp()
                    # log10 spread across action dims/batch: how fast state-differentiation develops
                    # (v3's antithetic curvature signal should widen this earlier than v1's ~40k).
                    sigma_spread = torch.log10(sigma.max() / sigma.min())
                ...
                self.writer.add_scalar("charts/sigma_spread", sigma_spread.item(), self.training_steps)
