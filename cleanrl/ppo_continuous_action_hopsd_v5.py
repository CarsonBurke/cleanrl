# HOPSD v5 "plangrad+awr" — advantage-weighted decoder + hindsight plan-gradient ascent.
# =====================================================================================
# v5 = v4 with the AWR weighting RESTORED on the decoder NLL (temp 0.5, the setting
# winning in the live ablation: awr05_r2 7325@3.6M vs kl005 6878). v4's lesson (dead
# at -190@1.7M): the unweighted decoder converges to the posterior of the CURRENT
# policy, whose width is the policy's own noise — the distill target never sharpens
# (teacher entropy stuck at init -0.58), and forward KL toward a broad Beta with a
# slightly-shifted mean has a vanishing gradient (approx_kl sat at 0.006, 8x under
# the leash). AWR weighting is not just the improvement tilt; it is the SHARPENING
# mechanism that gives distillation its gradient strength. The plan-ascent machinery
# itself worked as designed (plan_delta at radius, plan_dv > 0) but had no vehicle.
#
# v5 composes the two improvement operators:
#   DECODER  pi_T(a_t | s_t, phi_t): separate ThinkTrunk over [obs, phi, valid_frac],
#     phi = [future-action mean/std over H=20]. Trained by ADVANTAGE-WEIGHTED Beta NLL
#     (w = exp(adv_z/temp).clamp(20), mean-normalized) on the taken z: within a plan
#     context, tilt toward better-than-taken actions (v2's proven operator).
...
#
# plan_radius halved vs v4 (0.5 sigma): the AWR-sharpened decoder is less trustworthy
# off-manifold, so the counterfactual query stays closer to the achieved plan.
# Cold start unchanged: zero-init plan-critic head -> phi* = phi initially.
# target_kl=0.05 freezes the student only (decoder+critic always train full epochs).
#
# HYPOTHESIS: AWR supplies sharp within-context improvement; the plan ascent adds a
# temporally coherent cross-context shift (a whole-gait nudge). Composed, they beat
# AWR alone. Falsifiable against the live ablation: hopsd_v2_kl005_awr05_r2 IS v5
# minus the plan ascent — if v5 does not beat its curve (7325@3.6M), the ascent adds
# nothing at this radius and the plan-critic direction is noise or an OOD artifact.
import os
import random
import time
...
class Args:
    ...
    awr_temp: float = 0.5            # decoder NLL weight temperature on batch-z-scored GAE
    ...
    plan_radius: float = 0.5         # trust region: ||(phi*-phi)/sigma_batch|| cap (sigma units)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # --- Achieved plan features + AWR weights ---
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                teacher_actor_loss = (b_awr_w[mb_inds] * t_nll).mean()  # AWR tilt (v2's operator)
