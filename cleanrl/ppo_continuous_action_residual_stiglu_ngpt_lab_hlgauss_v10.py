# Laboratory HL-Gauss head on OUR normalized residual SiTU architecture v10.
# std_proxy_v2's best HL-Gauss candidate: K101, z +/-5, sigma_z=0.5 (5 bins),
# shared StandardizedHistogram with half-infinite edge bins and plain CE.
# No EMA: current rollout target mean/std, frozen after GAE across epochs.
# Preserve both trunks, actor initialization, matrix projection, shared Adam,
# global grad clip, fullbatch PPO, advnorm and asymmetric policy clipping.
# Replace only the scalar value readout with categorical logits and expectation.
import os
import random
import time
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", "residual SiTU-GLU; unit-L2 matrices and residual directions; sqrt(width) scaling; no LayerScale; laboratory K101 HL-Gauss head, sigma 5 bins, plain CE, no EMA or value clipping; original actor and critic residual trunks")
