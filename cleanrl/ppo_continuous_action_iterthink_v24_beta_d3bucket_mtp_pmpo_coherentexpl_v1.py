# PPO + IterThink v24 Beta + d3bucket MTP + PMPO + MAGPOS + COHERENT (colored/OU) EXPLORATION v1.
# =====================================================================================
# NOVELTY: temporally-COHERENT (colored / Ornstein-Uhlenbeck-style AR(1)) action-space
# exploration layered on top of the HalfCheetah-best magpos recipe (pos=magnitude, neg=tanh).
# Standard PPO/PMPO draws an i.i.d. Beta action every step, so the exploratory jitter is white
# noise: it averages out over a few steps and rarely produces the sustained, directional motor
# commands that discover new gaits/velocities. Here, during ROLLOUT ONLY, each sampled action is
# additively perturbed by a per-env AR(1) noise process expl_eps that PERSISTS across timesteps:
#   expl_eps <- rho*expl_eps + sqrt(1-rho^2)*N(0,1)      (unit stationary variance => scale is
#                                                          interpretable in action units)
#   taken_action = clip(sampled_action + expl_ou_scale * expl_eps, action_low, action_high)
# The perturbed+clipped action is the TAKEN action: it steps the env AND is the action the PMPO
# Beta log-prob is scored on (the perturbed action is mapped back to its native Beta z via the
# SAME linear inverse + SAMPLE_EPS clamp and REPLAYED through get_action_and_value, so stored
# action / latent z / rollout log-prob stay mutually consistent -- no second log-prob code path).
# expl_eps is reset to 0 for any env whose episode just terminated. rho in [0,1) sets the
# correlation time (higher = smoother, longer-lived pushes); scale sets the magnitude.
# HYPOTHESIS: temporally-extended, smoother exploration covers the state space more effectively
# than white per-step jitter on HalfCheetah's dense, termination-free dynamics, nudging the
# magpos recipe above its 9221 @ 8M ceiling. Everything else -- the magpos PMPO objective, the
# Beta actor, the d3bucket MTP critic, the training/update loop -- is IDENTICAL to magpos_v1.
# DEFAULTS OFF: expl_ou_rho=0.0 AND expl_ou_scale=0.0 => the perturbation is gated off entirely
# (no extra RNG draws), so the file is behaviorally BYTE-FOR-BYTE identical to magpos_v1.
#
# === RESULT (seed 1, HalfCheetah, matched-step vs magpos ref @2M/3M = 3513/5040 -> 9221): NEGATIVE. ===
#   Coherent noise only HURTS, monotonically with magnitude:
#     rho0.85 scale0.05 (gentlest): @1M/2M/3M = 1328/2873/3629  (~72% of magpos, still below)
#     rho0.95 scale0.10           : @1M/2M/3M =  185/ 931/1916  (far below)
#     rho0.85 scale0.10           : @1M/2M/3M =  202/ 680/1610  (far below)
#   Best = NO noise (magpos). => Exploration is NOT the bottleneck on HalfCheetah's dense, termination-
#   free reward; injecting persistent action noise just perturbs the well-optimized policy. The Beta
#   actor's own entropy already supplies adequate exploration. Coherent/OU action noise is a DEAD END
#   for dense-reward continuous control here. Next lever must be optimization/credit-assignment quality
#   (critic denoising, MTP actor for temporal credit), not exploration noise. Deliverable stands:
#   magpos_v1 tanhneg = 9221 (+18.5%).
# =====================================================================================
# --- inherited magpos_v1 notes ---
# PPO + IterThink v24 Beta + d3bucket MTP + PMPO + MAGPOS v1 (clean base, NO plasticity).
# =====================================================================================
# CONTEXT: a long line of plasticity+sign ablations (pmpo_plast_v2..v10) all UNDERPERFORMED the
# clean pmpo_v1 base (~7780 @ 8M vs ~6500). Isolation showed the PLASTICITY graft was the
# regression (same objective + plasticity = pmpo_plast_v2 ~5650). BUT within that handicapped
# setting, giving POSITIVE advantages real MAGNITUDE (divide-by-std) instead of PMPO's tanh-squash
# consistently helped (plast_v4/v8 ~6500 vs tanh-both plast_v2 ~5650). magpos tests that ONE
# insight on the CLEAN, strong base with ZERO plasticity: pmpo_pos_weight="magnitude" makes the
# positive PMPO log-prob weight |A|/std (unbounded, proportional reinforcement) while negatives
# keep the bounded tanh (or hard sign). Everything else -- the PMPO pos/neg log-prob objective,
# the count balancing, the reverse-KL trust region -- is IDENTICAL to pmpo_v1. HYPOTHESIS:
# proportional positive reinforcement beats sign-only squash and pushes clean PMPO above 7780.
#
# === RESULT (seed 1, 8M, matched-step, all knobs swept) ===
#   HalfCheetah: magpos(pos=magnitude, neg=tanh, posw0.5, kl0.3, LINEAR+UNCLAMPED) = 9221  vs  tanh
#     baseline 7780  =>  +1441 (+18.5%). Optimum: linear unclamped pos-magnitude (clamp5 HURT, pow1.5/2.0
#     no gain), tanh neg beats hard sign (9221 vs 8633), posw 0.5 best (0.65/0.75/0.85 worse), kl 0.3 best.
#   >>> This win DOES NOT GENERALIZE. It is HalfCheetah-SPECIFIC. <<<
#   Walker2d: magpos REGRESSES ~2.8-3.5x. Matched @1M/2M/3M -> tanh baseline 516/2120/2049  vs
#     magpos 385/594/738. The base works fine on Walker (baseline ~2000+); magpos SPECIFICALLY breaks it.
#   Hopper: base-limited -- BOTH arms fail (~88 avg over 2M; brief 985 spike then falls); the iterthink
#     PMPO base is not tuned for Hopper's early-termination dynamics -- orthogonal to magpos.
#   MECHANISM: HalfCheetah has no early termination + dense forward reward + smooth advantages, so an
#     UNBOUNDED |A|/std positive weight productively amplifies the best actions. Walker is a termination-
#     sensitive gait task: unbounded positive weighting over-amplifies high-variance lucky episodes near
#     the fall boundary and destabilizes the gait -- the tanh BOUND is protective there. The two envs want
#     OPPOSITE things (HC wants unbounded, clamping hurt it; Walker wants bounded, tanh), so NO single
#     static positive-weight knob wins both. A general variant needs adaptive/rank-bounded positive
#     weighting -- see magpos_v2 (rank-percentile positive weight: proportional in the bulk but bounded).
#
# --- inherited pmpo_v1 notes ---
...
class Args:
    ...
    # --- magpos: per-sign shaping of the PMPO log-prob weight (default tanh|A| for BOTH matches
    # the original pmpo_v1). The winning insight from the plasticity ablations was that POSITIVE
    # advantages want REAL MAGNITUDE, not tanh-squash. Here we test it on the clean base within
    # PMPO's own log-prob + KL framework (no PPO clip, no plasticity):
    #   pos weight in {"tanh": |tanh A| (bounded, original), "magnitude": |A|/std (unbounded,
    #     reinforces good actions proportionally)}
    #   neg weight in {"tanh": |tanh A| (bounded graded, original), "sign": 1 (pure sign)}
    # The pos/neg split and pmpo_pos_to_neg_weight balancing and KL leash are UNCHANGED.
    pmpo_pos_weight: str = "magnitude"
    pmpo_neg_weight: str = "tanh"
    # Optional cap on the positive magnitude weight |A|/std (0.0 = no cap, the default, matching
    # the runs that already beat pmpo_v1). >0 clamps the heavy positive-advantage tail so a few
    # huge-advantage actions cannot dominate an update -- a robustness/tuning knob.
    pmpo_pos_mag_clamp: float = 0.0
    # Power applied to the positive magnitude weight: w_pos = (|A|/std)**pow (1.0 = linear default).
    # clamp5 showed the heavy positive tail HELPS, so pow>1 sharpens reinforcement of the best
    # actions (heavier tail); pow<1 flattens it toward sign. Applied before the optional clamp.
    pmpo_pos_mag_pow: float = 1.0

    # --- Coherent (colored / OU) action-space exploration (ROLLOUT ONLY). A per-env AR(1)
    # noise process is added to each sampled action and clipped to the env bounds, giving
    # temporally-correlated (smooth, persistent) exploration instead of white per-step jitter.
    # BOTH defaults 0.0 => perturbation gated OFF (no extra RNG) => magpos_v1 EXACTLY. ---
    expl_ou_rho: float = 0.0    # AR(1) correlation of the colored noise in [0,1); 0.0 = i.i.d.-equivalent
    expl_ou_scale: float = 0.0  # additive noise magnitude in action units; 0.0 = OFF (no perturbation)
...
if __name__ == "__main__":
    ...
    # Coherent/OU exploration: per-env AR(1) colored-noise state (num_envs, act_dim), carried
    # across rollout steps and zeroed on episode termination. sqrt(1-rho^2) keeps unit stationary
    # variance so expl_ou_scale is interpretable in action units. Gated by expl_ou_scale != 0.
    expl_eps = torch.zeros((args.num_envs,) + envs.single_action_space.shape, device=device)
    expl_ou_sigma = (1.0 - args.expl_ou_rho ** 2) ** 0.5
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if args.expl_ou_scale != 0.0:
                    # Coherent/OU exploration (rollout only): advance the AR(1) noise, perturb the
                    # sampled action in ACTION space, clip to the env bounds, map the perturbed
                    # action back to its native Beta z (linear inverse of to_action + the SAME
                    # SAMPLE_EPS clamp as sampling), and REPLAY it through get_action_and_value so
                    # the stored action / latent z / rollout log-prob are mutually consistent (the
                    # taken action is exactly what the PMPO Beta log-prob is scored on). scale==0
                    # skips this block entirely (no extra RNG draw) => magpos_v1 EXACTLY.
                    expl_eps = args.expl_ou_rho * expl_eps + expl_ou_sigma * torch.randn_like(expl_eps)
                    perturbed = (action + args.expl_ou_scale * expl_eps).clamp(
                        agent.action_low, agent.action_high
                    )
                    z = ((perturbed - agent.action_low) / (agent.action_high - agent.action_low)).clamp(
            ...
            if args.expl_ou_scale != 0.0:
                # Reset the colored-noise state for envs whose episode just ended so exploration
                # in a fresh episode does not inherit the previous episode's correlated push.
                expl_eps[next_done.bool()] = 0.0
        ...
        with torch.no_grad():
            ...
            if not pmpo_active and args.ret_percnorm and args.ret_perc_scope in ("ema", "batch"):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if pmpo_active:
                    ...
                    # Per-sign PMPO weight. Original pmpo_v1 used |tanh A| for BOTH (magnitude
                    # squashed away -> "essentially sign"). magpos lets POSITIVE advantages carry
                    # real magnitude (|A|/std, unbounded) so genuinely better actions are reinforced
                    # proportionally, while NEGATIVE stays bounded (tanh graded or hard sign).
                    absA = mb_advantages.abs()
                    adv_std = mb_advantages.std() + 1e-8
                    if args.pmpo_pos_weight == "tanh":
                        w_pos = absA.tanh()
                    elif args.pmpo_pos_weight == "magnitude":
                        w_pos = absA / adv_std
                        if args.pmpo_pos_mag_pow != 1.0:
                            w_pos = w_pos.pow(args.pmpo_pos_mag_pow)
                        if args.pmpo_pos_mag_clamp > 0.0:
                            w_pos = w_pos.clamp(max=args.pmpo_pos_mag_clamp)
                    else:
                        raise ValueError(f"unknown pmpo_pos_weight {args.pmpo_pos_weight}")
                    if args.pmpo_neg_weight == "tanh":
                        w_neg = absA.tanh()
                    elif args.pmpo_neg_weight == "sign":
                        w_neg = torch.ones_like(absA)
                    else:
                        raise ValueError(f"unknown pmpo_neg_weight {args.pmpo_neg_weight}")
                    adv_weight = torch.where(mb_advantages >= 0, w_pos, w_neg).unsqueeze(-1)
