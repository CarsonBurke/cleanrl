# PPO + IterThink v40 (SAC max-entropy IN GAE -- FIXED SAC TARGET -|A|, fully DYNAMIC dual). From v38.
#
# WHY v40. v38 fixed the soft channel (centered + scale-controlled, ratio ~1.0 forever) but kept v35's
# BINDING FLOOR target (target_entropy = +0.5/dim => +6 HC / +3 Hopper / +3 Walker), set deliberately
# ABOVE each policy's free-collapse entropy so the dual ALWAYS binds and force-injects exploration.
# That RESCUED the terminating envs but CAPPED HalfCheetah: H is pinned at +3.0, forbidding the
# low-entropy high-speed gait, so HC plateaus ~2450 vs v32's ~8800. The "fix" we then tried (v39) was
# to ANNEAL the target +0.5 -> -1.0 over training -- but a hand-tuned schedule is exactly the kind of
# tuning SAC AVOIDS. SAC never schedules anything: it sets ONE fixed target (target_entropy = -|A|,
# negative and REACHABLE) and lets the temperature dual find the optimal alpha DYNAMICALLY, per-env,
# from the data. That is the right mechanism; v40 adopts it verbatim.
#
# v40 CHANGE (one isolated constant): target_entropy_coef +0.5 -> -1.0 (SAC's -|A| per dim). No floor,
# no schedule. The dual now equilibrates alpha wherever the SAC target dictates:
#   * HC: squashed H (~4) stays FAR ABOVE -6, so the dual bleeds alpha to alpha_min (0.02). The direct
#     -alpha*logpi term fades => HC reverts toward pure-reward PPO and should recover toward ~8800
#     (the v32 behaviour, now reached DYNAMICALLY rather than by an annealed schedule).
#   * Hopper/Walker: H stays above -3 too, so alpha also bleeds -- BUT unlike the naked -|A| that
#     collapsed these envs in v35, v40 retains v38's ALPHA-INDEPENDENT soft channel: policy_adv =
#     reward_adv + soft_adv_coef * (scale-matched future-entropy advantage). That fixed 0.25-of-reward
#     exploration NUDGE persists regardless of alpha -- the on-policy stand-in for SAC's soft-Q
#     future-entropy term -- so structured exploration survives the alpha bleed. This is the honest
#     test of whether the SAC mechanism (fixed reachable target + dynamic dual + entropy-in-value)
#     works on-policy once the soft channel is no longer buggy. If the terminating envs still collapse,
#     that is real evidence of the on-policy/off-policy exploration gap, not a tuning failure.
# Everything else (centered+scale-controlled soft channel, V_ent baseline, alpha_min=0.02, dual cadence)
# is unchanged from v38.
#
# ---- inherited v38 notes ----
...
class Args:
    ...
    target_entropy_coef: float = -1.0  # v40: SAC's EXACT per-dim target -|A| (negative, reachable). No floor,
                                    #      no schedule -- the dual finds alpha DYNAMICALLY per-env. H stays far
                                    #      above -|A| on all three envs => alpha bleeds toward alpha_min, fading
                                    #      the direct -alpha*logpi term so HC can exploit (recover ~8800). The
                                    #      alpha-INDEPENDENT soft channel (soft_adv_coef * scale-matched
                                    #      future-entropy adv) persists as the on-policy stand-in for SAC's
                                    #      soft-Q future-entropy term, sustaining structured exploration on the
                                    #      terminating envs. Resolved target = -1.0*dim => -3 (Hopper), -6 (HC/Walker).
    alpha_lr: float = 1e-3
