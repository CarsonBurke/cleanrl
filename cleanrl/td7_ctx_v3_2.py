...
# --------------------------------------------------------------------------------------------------
# v3_2 — VALUE-AWARE LM HEADS (the "useful representation" arm of the accurate-vs-useful ablation vs
# v3_1). HEADS-ONLY delta: the body, decode, attention, RL path, prefill/reprefill, and PatchEncoder
# reward-sum aux are BYTE-IDENTICAL to v3_1. The existing next-token + next-rsum LM heads STAY. We add
# TWO new per-position heads to the SAME LM window forward (same body activations `h`), so the body's
# self-supervision now also encodes "what did well previously", not just next-step predictive accuracy:
#   HEAD 1 (reward-to-go, ring-only, cheap): lm_rtg_head predicts the discounted sum of the NEXT
#     rtg_horizon patch reward-sums, target_rtg(g) = sum_{k=1..H} gamma_patch^(k-1) * raw_rsum[g+k] with
#     gamma_patch = 0.99**patch_len (~0.605). z-scored by a running Welford (rtg_norm), clipped [-5,5].
#     Positions whose horizon overflows the resident/written band (g + rtg_horizon >= n_written) are
#     MASKED OUT of the loss (not truncated) — exactly zero loss + zero grad.
#   HEAD 2 (critic-distillation, value-grounded): lm_qdistill_head predicts the CURRENT critic's minQ at
#     each patch BOUNDARY, computed under no_grad + DETACHED (deposits NO grad into body/actor/critic).
#     Boundary proxy state = the LAST valid step's obs of patch g (the ring stores obs-at-step, not
#     next-obs; last-step obs is the boundary proxy). The hist-augmented input is reconstructed IN-PATCH
#     ONLY from the same patch's last-4-executed actions (rows L-2..L-5, most-recent first); positions
#     with L<5 (lookback would cross the patch boundary) are MASKED OUT — no fabricated hist. Target is
#     the on-policy minQ(s_b_aug, actor(s_b_aug)) under the FIXED embeddings + FROZEN cache (same
#     (weights,cache) pair the RL path uses), z-scored by qdistill_norm, clipped [-5,5]. ASSUMES
#     max_action==1 (HalfCheetah/Hopper/Walker2d clip actions to [-1,1]) so the ring's normalized action
#     equals the raw action the critic expects. Cost: one batched actor+critic forward over nw*T frozen-
#     cache boundary positions per LM update (measured vs v3_1 in the delta report).
# Total lm_loss = next_tok + next_rsum + rtg + qdistill (equal weight; the two new terms are masked-means).
# --------------------------------------------------------------------------------------------------
...
class Args:
    ...
    rtg_horizon: int = 8
    """number of FUTURE patches summed for the value-aware reward-to-go LM head (per-patch discount)"""
...
def welford_update_batch(norm: ReturnNorm, vals: np.ndarray):
    """Exact chunked Welford merge of a 1-D array of samples into a ReturnNorm (host-side, O(1) work).

    Identical count/mean/M2 to feeding the same samples one-by-one through ReturnNorm.update, so the
    two new value-aware LM heads can normalize their (~nw*T) per-call targets without a python loop.
    """
    n_b = int(np.asarray(vals).size)
    if n_b == 0:
        return
    vals = np.asarray(vals, dtype=np.float64)
    mu_b = float(vals.mean())
    m2_b = float(((vals - mu_b) ** 2).sum())
    n_a = norm.count
    if n_a == 0:
        norm.count = n_b
        norm.mean = mu_b
        ...
        return
    n = n_a + n_b
    delta = mu_b - norm.mean
    norm.mean = norm.mean + delta * n_b / n
    norm.M2 = norm.M2 + m2_b + delta * delta * n_a * n_b / n
    norm.count = n


def masked_mse(pred, target, mask):
    """Masked-mean MSE over a boolean mask. Masked-out positions contribute EXACTLY zero to the loss and
    zero gradient. A fully-masked component returns a finite 0 that is still connected to `pred` (so the
    LM backward stays valid and no NaN appears when every position of a component is masked)."""
    m = mask.reshape(-1)
    if not bool(m.any()):
        return pred.reshape(-1).sum() * 0.0
    return F.mse_loss(pred.reshape(-1)[m], target.reshape(-1)[m])
...
class TD7Agent:
    def __init__(self, state_dim, action_dim, max_action, args: Args, device, writer: SummaryWriter, step_dim):
        ...
        # v3_2 VALUE-AWARE heads (same LM window forward `h`): reward-to-go + critic-distillation.
        self.lm_rtg_head = nn.Linear(args.tok_dim, 1).to(device)
        self.lm_qdistill_head = nn.Linear(args.tok_dim, 1).to(device)
        self.rtg_norm = ReturnNorm()  # running z-score of the raw reward-to-go targets
        self.qdistill_norm = ReturnNorm()  # running z-score of the raw critic minQ targets
        # per-patch discount: one patch == patch_len env steps, so gamma_patch = gamma_step**patch_len.
        self.gamma_patch = args.gamma ** args.patch_len
        # ring column layout: row = [raw_obs(:raw_state_dim), normalized_action, reward]. state_dim here is
        # the HIST-AUGMENTED dim (raw_state_dim + hist_k*action_dim); recover the raw pieces for qdistill.
        self.action_dim = action_dim
        self.step_dim = step_dim
        self.raw_state_dim = step_dim - action_dim - 1
        assert self.raw_state_dim + args.hist_k * action_dim == state_dim, (
            "state_dim must equal raw_state_dim + hist_k*action_dim for in-patch hist reconstruction")
        # the ring stores normalized_action = raw_action / max_action; the critic expects raw actions in
        # [-1,1]. With max_action==1 (HalfCheetah/Hopper/Walker2d) the two coincide, so ring actions and
        # actor outputs feed the critic directly. (asserted here so a non-unit action scale fails loudly.)
        assert float(max_action) == 1.0, "v3_2 qdistill assumes max_action==1 (ring action == raw action)"
        self.lm_optimizer = torch.optim.Adam(
            list(self.body.parameters()) + list(self.lm_tok_head.parameters())
            + list(self.lm_rsum_head.parameters()) + list(self.lm_rtg_head.parameters())
            + list(self.lm_qdistill_head.parameters()),
            lr=3e-4,
        )
        self.last_lm_loss = 0.0
        self.last_lm_next_tok = 0.0
        self.last_lm_rtg = 0.0
        self.last_lm_qdistill = 0.0
    ...
    def lm_update(self):
        ...
        predict (per position) the NEXT patch token + NEXT z-scored reward-sum (accuracy heads), PLUS the
        v3_2 VALUE-AWARE heads at each position g_t: the discounted reward-to-go over the raw ring, and a
        critic-distillation of the current minQ at the patch boundary. This is the ONLY objective that
        moves the body params (+ the four LM heads). PatchEncoder tokens + the qdistill target are
        DETACHED (no backprop into the encoder, body, actor, or critic through the target path)."""
        ...
        # ---- v3_2 VALUE-AWARE heads: predict AT each window position g_t (not the next one) ----
        Hz = self.args.rtg_horizon
        rp = self.store.raw_patches
        g_np = starts[:, None] + np.arange(T)[None, :]  # (nw, T) global patch idx per window position (== pos)

        # HEAD 1 (reward-to-go, ring-only): discounted sum of the next Hz patch reward-sums.
        ks = np.arange(1, Hz + 1)  # future patch offsets 1..Hz
        gk = g_np[:, :, None] + ks[None, None, :]  # (nw, T, Hz) future global patch indices
        rsum_future = self.store.raw_rsum[gk % rp].astype(np.float64)  # (nw, T, Hz)
        disc = (self.gamma_patch ** np.arange(Hz))  # (Hz,) per-step discounts gamma_patch^(k-1), k=1..Hz
        rtg_raw = (rsum_future * disc[None, None, :]).sum(-1)  # (nw, T) raw reward-to-go target
        # MASK (do NOT truncate) positions whose horizon overflows the written/resident band.
        rtg_mask_np = (g_np + Hz) < n_written  # (nw, T) bool; valid iff the full horizon is written
        welford_update_batch(self.rtg_norm, rtg_raw[rtg_mask_np])  # z-score stats from RAW valid targets
        rtg_z = np.clip((rtg_raw - self.rtg_norm.mean) / (self.rtg_norm.std + 1e-6), -5.0, 5.0).astype(np.float32)
        rtg_z_t = torch.as_tensor(rtg_z, device=self.device)  # (nw, T)
        rtg_mask_t = torch.as_tensor(rtg_mask_np, device=self.device)  # (nw, T) bool

        # HEAD 2 (critic-distillation): reconstruct the boundary state (last-step obs + in-patch hist),
        # run the CURRENT actor+critic over the FROZEN cache under no_grad, distill minQ into body/head.
        slot = g_np % rp  # (nw, T)
        L = self.store.raw_lens[slot]  # (nw, T) valid step count per patch
        hk = self.args.hist_k
        # in-patch lookback needs rows L-2 .. L-(hk+1); they exist only for L>=hk+1, else MASK (no fabricated hist)
        qd_mask_np = L >= (hk + 1)
        sd, ad = self.raw_state_dim, self.action_dim
        obs_row = np.clip(L - 1, 0, self.patch_len - 1)  # last valid step (clamped; masked rows ignored)
        obs = self.store.raw_blocks[slot, obs_row, :sd]  # (nw, T, sd) boundary-proxy obs
        offsets = np.arange(2, 2 + hk)  # rows L-2 (most recent) .. L-(hk+1), most-recent-first
        hist_rows = np.clip(L[:, :, None] - offsets[None, None, :], 0, self.patch_len - 1)  # (nw,T,hk)
        hist_acts = self.store.raw_blocks[slot[:, :, None], hist_rows, sd:sd + ad]  # (nw,T,hk,ad) most-recent first
        hist_flat = hist_acts.reshape(nw, T, -1)  # (nw, T, hk*ad)
        s_b_aug = np.concatenate([obs, hist_flat], axis=-1).reshape(nw * T, sd + hk * ad).astype(np.float32)
        s_b_t = torch.as_tensor(s_b_aug, device=self.device)
        g_flat = torch.as_tensor(g_np.reshape(-1), device=self.device, dtype=torch.long)
        fnw = self.frozen_n_written
        with torch.no_grad():  # DETACHED target: deposits NO grad into body / actor / critic
            fixed_zs = self.fixed_encoder.zs(s_b_t)
            a_b = self.actor(s_b_t, fixed_zs, self.body, self.K_frozen, self.V_frozen, g_flat, fnw)
            fixed_zsa = self.fixed_encoder.zsa(fixed_zs, a_b)
            Q, _ = self.critic(s_b_t, a_b, fixed_zsa, fixed_zs, self.body, self.K_frozen, self.V_frozen, g_flat, fnw)
            target_q = Q.min(dim=1).values.reshape(nw, T)  # minQ over the twins, (nw, T)
        qd_mask_t = torch.as_tensor(qd_mask_np, device=self.device)  # (nw, T) bool
        tq_np = target_q.detach().cpu().numpy()
        welford_update_batch(self.qdistill_norm, tq_np[qd_mask_np])  # z-score stats from RAW valid targets
        target_q_z = ((target_q - self.qdistill_norm.mean) / (self.qdistill_norm.std + 1e-6)).clamp(-5.0, 5.0)

        pred_rtg = self.lm_rtg_head(h).squeeze(-1)  # (nw, T)
        pred_qd = self.lm_qdistill_head(h).squeeze(-1)  # (nw, T)
        rtg_mse = masked_mse(pred_rtg, rtg_z_t, rtg_mask_t)
        qdistill_mse = masked_mse(pred_qd, target_q_z, qd_mask_t)

        lm_loss = next_tok_mse + next_rsum_mse + rtg_mse + qdistill_mse  # equal-weight (masked-mean for the two new)
        ...
        self.last_lm_next_tok = next_tok_mse.detach()
        self.last_lm_rtg = rtg_mse.detach()
        self.last_lm_qdistill = qdistill_mse.detach()
    ...
    def train(self):
        ...
        if self.training_steps % 500 == 0:
            ...
            self.writer.add_scalar("debug/lm_next_tok", float(self.last_lm_next_tok), self.training_steps)
            self.writer.add_scalar("debug/lm_rtg", float(self.last_lm_rtg), self.training_steps)
            self.writer.add_scalar("debug/lm_qdistill", float(self.last_lm_qdistill), self.training_steps)
