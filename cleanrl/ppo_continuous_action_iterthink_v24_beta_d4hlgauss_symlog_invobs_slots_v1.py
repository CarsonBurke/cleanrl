# PPO + IterThink v24 beta d4-HL-Gauss symlog + inverted obs slots v1.
#
# Variant hypothesis: replace the IterThink MLP/MoE backbone with a purely spatial
# inverted cross-attention tokenizer inspired by TF-Inv / Slot Attention's core
# normalization axis. Each scalar observation dimension becomes an LLM-style token:
# value embedding + learned position embedding. Separate actor and critic slot
# banks compete to explain these obs tokens with inverted attention
# (softmax over slots for each token, then per-slot renorm over tokens). No GRU,
# recurrence, time horizon, or iterative slot updates. Actor reads four slots;
# critic reads four slots; everything downstream is the symlog_v1 PPO machinery.
#
...
class ObsTokenizer(nn.Module):
    """Scalar observation dimensions -> ordered tokens."""
    ...
    def __init__(self, obs_dim, H):
        ...
        self.norm = nn.RMSNorm(H, elementwise_affine=False)

    def forward(self, x):
        tokens = self.value(x.unsqueeze(-1)) + self.pos.unsqueeze(0)
        return self.norm(tokens)


class InvertedSlotReadout(nn.Module):
    """Single-step inverted cross-attention slots over obs tokens.

    Normal cross-attention lets each slot independently choose tokens. Here every
    token assigns probability mass across slots first, creating slot competition;
    a per-slot renormalization then forms each slot's token-weighted update.
    """

    def __init__(self, n_slots, H, ff_mult):
        super().__init__()
        ...
        self.slot_norm0 = nn.LayerNorm(H)
        self.slot_norm1 = nn.LayerNorm(H)
        self.ff = nn.Sequential(
            layer_init(nn.Linear(H, ff_mult * H)),
            nn.GELU(),
            layer_init(nn.Linear(ff_mult * H, H), std=1.0),
        )
        self.out_norm = nn.RMSNorm(n_slots * H, elementwise_affine=False)
        self.out_proj = layer_init(nn.Linear(n_slots * H, H))
    ...
    def forward(self, tokens):
        ...
        k = self.k(tokens)
        v = self.v(tokens)
        logits = torch.matmul(q, k.transpose(-1, -2)) / sqrt(q.shape[-1])  # (B, S, T)
        assign = torch.softmax(logits, dim=1)                              # token -> competing slots
        attn = assign / (assign.sum(dim=-1, keepdim=True) + 1e-6)           # slot -> token renorm
        updates = torch.matmul(attn, v)
        slots = self.slot_norm0(slots + updates)
        slots = self.slot_norm1(slots + self.ff(slots))
        with torch.no_grad():
            p = assign.detach().clamp_min(1e-8)
            self._assign_entropy = -(p * p.log()).sum(dim=1).mean()
            if self._assign_entropy_sum is None:
                self._assign_entropy_sum = self._assign_entropy.clone()
            else:
                self._assign_entropy_sum = self._assign_entropy_sum + self._assign_entropy
            self._assign_count += 1
        return self.out_proj(self.out_norm(slots.flatten(1)))


class ThinkTrunk(nn.Module):
    def __init__(self, in_dim, H, K, n_experts, actor_slots=4, critic_slots=4, slot_ff_mult=2):
        super().__init__()
        del K, n_experts
        self.tokenizer = ObsTokenizer(in_dim, H)
        self.actor_readout = InvertedSlotReadout(actor_slots, H, slot_ff_mult)
        self.critic_readout = InvertedSlotReadout(critic_slots, H, slot_ff_mult)

    def forward(self, x):
        tokens = self.tokenizer(x)
        actor_feat = self.actor_readout(tokens)
        critic_feat = self.critic_readout(tokens)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        H = args.hidden
        assert args.slot_dim == H, "slot_dim must equal hidden in v1"
    ...
    def _trunks(self, x):
        # Return (actor_feat, critic_feat). In shared mode, actor and critic read
        # separate inverted-attention slot banks over the same obs tokens.
        if self.share_backbone:
            return self.trunk(x)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.share_backbone:
            actor_ent = agent.trunk.actor_readout.consume_stats()
            critic_ent = agent.trunk.critic_readout.consume_stats()
