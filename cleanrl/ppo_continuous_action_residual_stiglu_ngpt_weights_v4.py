# Residual SiTU-GLU with nGPT-style weight projection v4.
# Same residual architecture and Beta PPO as residual_activation_v1; only project
# weight vectors to unit L2 norm at initialization and after every Adam step.
# ../ngpt/train.py: gate/up rows, down columns, output-head rows. Biases unchanged.
# This is a weight-only ablation, not spherical activations or learned nGPT gates.
# Unit head norms deliberately replace the default 0.01 policy-head init scale.
# Compare at full rollout batches, learning_rate=0.0024, and ten update epochs.
import os
import random
import time
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", "residual SiTU-GLU; nGPT unit gate/up/head rows and down columns; init + post-Adam projection")
