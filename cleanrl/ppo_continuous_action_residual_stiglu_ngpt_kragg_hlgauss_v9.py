# Standardized Kragg HL-Gauss critic atop scale-restored residual SiTU PPO v9.
# Reuse the strongest recorded 50M HL-Gauss recipe: K255, z +/-6, sigma=3 bins,
# current-rollout target frame (no EMA), reflected Gaussian labels/scalar expectation,
# detached current-variance CE calibration; no softcap or per-sample weighting.
# Keep actor/rollout/full-batch PPO; advnorm on, asymmetric clip, no value clip.
# Initialize categorical logits near uniform with live trunk gradients. Frames
# and labels update once AFTER old-frame GAE, then stay frozen across epochs.
# Hypothesis: smoothed distributional targets improve critic learning under PPO.
import math
import os
import random
import time
...
class KraggHistogram(nn.Module):
    ...
    The head is a distribution over `z`; raw value is `mean + scale * E[z]`.
    Mean and scale are the current rollout's target statistics, updated once
    after GAE and frozen for the duration of the optimizer epochs.
    ...
    def observe(self, targets: torch.Tensor) -> None:
        """Adopt the current rollout's target frame without running averages."""
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        ...
        self.histogram = KraggHistogram(
            atoms=args.value_atoms, half_span=args.value_half_span,
...
def categorical_value_loss(histogram, logits, target_probs):
    """Reuse Kragg's detached batch-current-variance CE calibration."""
    log_probs = logits.log_softmax(dim=-1)
    with torch.no_grad():
        gain = histogram.scale.square() * histogram.standardized_variance(log_probs.exp()).mean()
    return -(target_probs.detach() * log_probs).sum(dim=-1).mean() * gain
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, target_probs, args):
    ...
    v_loss = categorical_value_loss(agent.histogram, logits, target_probs)
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", "residual SiTU-GLU; unit-L2 matrices and residual directions; sqrt(width) scaling; no LayerScale; standardized Kragg HL-Gauss critic with matched CE, no value clipping")
        ...
        def prepare_targets(returns):
            ...
            saturation = agent.histogram.saturated(returns)
            label_error = (agent.histogram.decode_probs(labels) - returns).abs().mean()
            return labels, saturation, label_error
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_target_probs, saturated, label_error = prepare_targets(b_returns)
            updates = 0
            ...
            logged = gather_metrics({
                ...
                "critic/saturated": saturated, "critic/label_error": label_error,
