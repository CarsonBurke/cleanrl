# Critic-Metric OPSD v3 — learn critic-grounded action-space geometry.
# =====================================================================================
# v1's binary KL/success gates stalled the actor. Removing them in v2 exposed a deeper
# failure: free entropy and policy-coordinate branches overwhelmed dQ/da, drove every action
# to the tanh boundary, and kept writing enormous gradients after the critic path vanished.
#
# v3 makes critic grounding structural. A learned positive-definite action-space metric
# preconditions the frozen inner critic's dQ/da. The metric is trace-normalized and initialized
# to identity, so the initial actor gradient is EXACTLY direct -Q and the meta-learner can
# change geometry but cannot create a critic-independent entropy, likelihood, mean, or scale
# gradient. The direction vanishes when dQ/da vanishes and remains first-order Q-ascent in
# action space by construction.
#
# The metric is trained through the exact functional Adam direction. Every finite actor and
# metric update is applied. Adam steps below the policy-KL cap remain unchanged; larger steps
# are continuously rescaled to the cap with its implicit meta-gradient, while optimizer
# moments advance normally. KL is update geometry, never an admission gate.
#
# The outer objective uses validation-critic predicted gain minus disagreement with the
# training critic's predicted gain. Absolute critic calibration cancels in both gains, and
# shared extrapolation is penalized continuously rather than converted into a success gate.
# A direct-Q shadow uses identical clipping, optimizer state, policy-space normalization,
# states, and action noise. No teacher, outcome conditioning, candidate comparison, or fixed
# policy-gradient objective exists.
#
# PASS: positive conservative and next-fitted-critic gains, finite nonzero dQ/da, positive
# metric ascent dot, applied KL never above its cap and no skipped updates, learned gain above
# the direct-Q shadow, and return beyond the 10,362 chassis. FAIL: action saturation approaches
# 1, metric ascent breaks, critic fit fails, or two matched checkpoints trail the chassis by >5%.
# =====================================================================================
...
if __name__ == "__main__":
    ...
    if args.env_id != "HalfCheetah-v4":
        raise ValueError("critic-metric v3 is scoped to HalfCheetah-v4")
    ...
    with torch.random.fork_rng(devices=[device]):
        ...
        gradient_metric = CriticGradientMetric(
            obs_dim, action_dim, args.metric_hidden
        ).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            validation_meta_score_before = validation_score_critic(
                meta_obs, meta_action_before
            ).mean()
            training_meta_score_before = score_critic(
                meta_obs, meta_action_before
            ).mean()
        ...
        validation_meta_score_after = validation_score_critic(
            meta_obs, shadow_action
        ).mean()
        training_meta_score_after = score_critic(
            meta_obs, shadow_action
        ).mean()
        ...
        with torch.no_grad():
            ...
            direct_validation_score_after = validation_score_critic(
                meta_obs, direct_shadow_action
            ).mean()
            direct_training_score_after = score_critic(
                meta_obs, direct_shadow_action
            ).mean()
