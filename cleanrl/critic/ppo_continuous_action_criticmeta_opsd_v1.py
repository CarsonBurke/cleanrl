# Critic-Meta OPSD v1 — the actor's gradient is learned from predicted return.
# =====================================================================================
...
# Adam shadow is logged under the identical states and optimizer state. The learned actor
# update commits only when the independent critic predicts improvement and the exact
# transformed-Gaussian KL is within budget; negative safe proposals still train the loss
# generator. No teacher exists.
#
...
if __name__ == "__main__":
    ...
    if args.max_actor_kl <= 0.0:
        raise ValueError("max_actor_kl must be positive")
    ...
    previous_update_accepted = False
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            actor_kl_safe = bool(proposed_actor_kl <= args.max_actor_kl)
        ...
        with torch.no_grad():
            ...
            actor_update_accepted = bool(
                actor_kl_safe and meta_predicted_gain.detach() > 0.0
            )

        meta_grad_norm = torch.full((), float("nan"), device=device)
        shadow_commit_error = torch.full((), float("nan"), device=device)
        if actor_kl_safe:
        ...
        if actor_update_accepted:
            # Commit exactly the update scored above. The learned-loss optimizer step cannot
            # alter these already-constructed actor gradients.
            ...
            actor_optimizer.zero_grad(set_to_none=True)
        previous_update_accepted = actor_update_accepted
        ...
        writer.add_scalar(
            "meta/actor_update_accepted",
            float(actor_update_accepted),
            global_step,
        )
        writer.add_scalar(
