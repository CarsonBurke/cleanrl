# PPO + IterThink v24 Beta + d3bucket MTP + ENT-PPO v4 samplemask + DEEP VALUE READOUT v1.
# =====================================================================================
# ONE LEVER: the value READOUT. Everything else is the incumbent chassis, byte for byte.
#
# THE MECHANISM. The chassis reads the value distribution off the shared ThinkTrunk with a
# single bias-free zero-init Linear(H, mtp*num_bins): ONE affine map from a 64-d trunk
# feature to 6 horizons x 511 Dreamer3 symexp buckets. --deep-value replaces that map with
# an END-TO-END RESIDUAL BLOCK STACK over MTP-token conditioning:
#     cond_t = critic_feat + h_embed[t]          T = critic_mtp_horizon tokens per state
#     z_0    = z_start (learned, NONZERO)
#     z_b    = z_{b-1} + f_b(z_{b-1}, cond, t_emb(0))      b = 0 .. value_blocks-1
#     logits = decoder(z_B)                      decoder = Linear(H, num_bins), ZERO-init
# one block being  u = in_proj([RMSNorm(z), RMSNorm(cond)]) ; u = u*(1+scale)+shift ;
# F = out_proj(ReLU(u)^2)  -- a DiT block minus attention (the horizon tokens are
# conditionally independent given cond, so there is nothing to attend over). ONE backward
# through the B composed blocks. Support, HL-Gauss labels, MTP mask, value loss, reduction,
# GAE, advantage normalization and the whole actor are UNCHANGED.
#
# Note the readout gets CHEAPER in parameters, not richer: the decoder is SHARED across
# horizons (Linear(64,511) = 32,704) instead of one 6x-wide output (Linear(64,3066) =
# 196,224). The depth, the adaLN conditioning and the per-horizon token embedding are the
# added capacity. MEASURED in this file: 196,224 -> 163,648 critic-readout parameters.
#
# THE DONOR EVIDENCE (measured, HalfCheetah-v4, seed 1, identical core config
# --num-envs 16 --num-steps 2048 --num-minibatches 32 --update-epochs 10 -lr 3e-4)
#                                             @2M    @4M    @6M    @8M
#   iterthink_v24_beta_d3bucket_mtp_ppoadvnorm_batch_v1   (linear readout)
#                                             5012   6716   7548   8455
#   edmvalue_e2e_8m_v1  (THIS readout, --no-dblock, PPO actor UNCHANGED)
#                                             5257   8112   9138   9810
#   delta                                       +5%   +21%   +21%   +16%
# +1355 at 8M and the gain GROWS with steps. That HORIZON SIGNATURE is the whole reason
# this is worth a run: the actor-direction lever measured on this same chassis (31-dose
# sweep) tied at 8M with its gain DECAYING to zero by 4M. Opposite signature, opposite
# conclusion.
#
# EXPLAINED VARIANCE CANNOT SEE THIS BOTTLENECK. Measured at 8M on the donor pair:
# EV = 0.970 for the 8455 linear-readout ancestor and EV = 0.952 for the 9810 deep-value
# arm. The LOWER-EV arm scores 16% HIGHER. Report EV, but do NOT use it to argue the head
# is or is not working -- it is not the axis this lever moves. The plausible reading is
# that the deep readout trades a little global regression fit for better-CONDITIONED
# advantages (relative ordering / local TD structure), which is what the PG consumes.
#
# WHAT IS PORTED AND WHAT IS NOT. The donor file is a DiffusionBlocks A/B; its --dblock arm
# (the diffusion ladder) is the arm that LOST. This file ports ONLY the code that is live at
# dblock=False. NOT ported, and NOT reachable here: the EDM sigma schedule and equal-mass
# block bins (block_sigma_edges/inference_sigmas/cdf_lo/cdf_hi/block_bins/sample_sigma),
# EDM preconditioning (c_skip/c_out/c_in/c_noise, denoise()), the EDM lambda weighting,
# the Fourier bin latents and latent_of(), the frozen z_seed, per-block CE (block_losses)
# and its telemetry, and the Euler inference loop (_ode_logits, dblock_inference_steps).
# grep this file for `sigma`, `edm`, `ladder`, `dblock` -- there is nothing.
#
# FALSIFIABLE PREDICTIONS (pre-registered)
#   P1  At matched steps on the incumbent line, --deep-value >= entppo_v4_samplemask_a03_mask
#       (10362 +-92 @8M). PASS: >= 10362 at 8M. FAIL: >5% below at two consecutive
#       checkpoints (4M and 6M, or 6M and 8M).
#   P2  The advantage is TIME-EXTENSIVE: the deep-value gain at 4M is at least as large in
#       percent as at 2M. If the gain instead peaks early and decays, this is the same
#       failure mode as the actor-direction sweep and the lever is dead, regardless of the
#       8M point estimate.
#   P3  ALREADY MEASURED, AND IT DID NOT HOLD AS STATED -- see VERIFIED below. Under the
#       chassis default share_backbone=True the actor-side gates DO shift (approx_kl +75%,
#       tr_mask_frac +120%, entropy -20%, reproducible across 3 seeds with disjoint
#       ranges). Isolated with --no-share-backbone, those same gaps collapse to <=1.8%,
#       so the CHANNEL is the shared trunk's value gradient and the clip group it shares
#       with the head, NOT the actor objective (which is diff-verified byte-identical).
#       The live prediction is therefore: the 8M A/B is a value-readout A/B PLUS a
#       reallocation of the critic clip budget away from the trunk. If the deep arm loses,
#       --critic-grad-clip is the first thing to look at, not the readout.
#   P4  EV is NOT the gate (see above). A drop in losses/explained_variance relative to the
#       control does NOT falsify anything; only return does.
#
# --- INHERITED (assumed, measured elsewhere, NOT re-verified in this file) ---
#   * 10362 +-92 @8M for the chassis (entppo_v4_samplemask_a03_mask).
#   * The 8455 -> 9810 donor ladder and its per-checkpoint deltas.
#   * EV 0.970 vs 0.952 @8M.
#   * The donor's architecture rationale: adaLN-only block, RMSNorm on both inputs,
#     TimeEmbed frequency range [0.5, 8], shared zero-init decoder, and the NONZERO
#     z_start (with a zero start, a zero-init decoder and zero-init out_proj put the e2e
#     stack on an all-zero fixed point where NOTHING learns -- donor measured grad norm
#     exactly 0.00 for 4k steps).
#   * value_blocks=3 / value_mult=2 are the DONOR DEFAULTS (dblock_num_blocks=3,
#     dblock_mult=2), carried over unchanged. They are not tuned here.
#
# --- VERIFIED IN THIS FILE (measured at port time, HalfCheetah-v4, seed 1, RTX 5090) ---
#   * py_compile clean; both flag states train to completion.
#   * --no-deep-value is BIT-EXACT to the chassis parent over a 20480-step run
#     (--num-envs 4 --num-steps 256 --num-minibatches 4 --seed 1): 36 shared scalar tags,
#     0 tags unique to either side, 720 points compared, ZERO differing points
#     (charts/SPS excluded; NaN vs NaN counted equal; no NaN present).
#   * Critic-readout parameters: 196,224 (chassis linear) -> 163,648 (deep), -32,576.
#   * At init the deep readout emits logits EXACTLY 0 (max|logit| = 0.000000e+00), probs
#     are exactly uniform (max|p - 1/511| = 0.0e+00) and V(s) = 0.000000e+00 exactly on the
#     symmetric symexp support -- the identical neutral start to the head it replaces,
#     which also measures max|logit| = 0. Init readout grad-norm under the real masked
#     HL-Gauss CE: 4.246 (deep) vs 2.314 (chassis); in BOTH arms it sits ENTIRELY in the
#     final linear (decoder.weight / critic_head.weight) and the trunk grad is exactly 0.
#     The block stack's step-0 grad is 0 (out_w is zero-init) but becomes nonzero at
#     step 1 (|g blocks| 0.0 -> 0.725 -> 1.432 -> 2.223 over the first 4 Adam steps), so
#     z_start does escape the all-zero fixed point the donor warns about.
#   * MTP axis is not transposed: logits(feat, None)[:, h, :] == logits(feat, (h,))[:, 0, :]
#     to 1.2e-07 (kernel noise) while the MISMATCHED-horizon comparison differs by 5e-03,
#     and perturbing h_embed[3] moves ONLY slice [:, 3, :] (all other horizons: exactly 0).
#   * 262k champion-config A/B (3 seeds/arm, --num-envs 16 --num-steps 2048
#     --num-minibatches 32 --update-epochs 10): no NaN/Inf in ANY scalar after warmup in
#     either arm; debug/epochs_completed = 10/10 everywhere (the leash never fires in
#     either arm); losses/value_loss and losses/explained_variance OVERLAP between arms.
#     But the ACTOR-side gates SEPARATE, reproducibly and disjointly over 3 seeds:
#       losses/approx_kl    0.0178 (deep) vs 0.0097 (chassis)   gap/within-arm-sd +23
#       debug/tr_mask_frac  0.0138       vs 0.0060             gap/sd +38
#       losses/entropy      -0.519       vs -0.406             gap/sd -7.1
#       critic_grad_norm    5.70         vs 1.48               gap/sd +15
#     Re-run with --no-share-backbone (value gradient cannot reach the actor) the same
#     gaps become entropy -1.2%, tr_mask_frac +0.7%, approx_kl -1.8%, ratio_max -0.2%,
#     actor_grad_norm +1.3%, while critic_grad_norm stays +351%. So the deep head's
#     LARGER value gradient (pre-clip norm 5.7 vs 1.5, i.e. clip factor 0.25/5.7 = 0.044
#     vs 0.25/1.5 = 0.160 on the SHARED trunk+head group) is the whole channel. This is
#     an unavoidable property of the chassis's shared trunk and single critic clip group,
#     not a leak in the actor code -- but it means the 8M comparison is not purely a
#     readout swap, and the requester was right to ask.
#   * THROUGHPUT: the deep readout costs 1.40x per value train step (7.58 vs 5.43 ms, min
#     of 8 alternating reps at the champion minibatch) and 2.60x per bootstrap pass
#     (10.52 vs 4.05 ms), peak CUDA 224 vs 166 MiB (train step) and 912 vs 600 MiB
#     (bootstrap) -- roughly +4-5% of champion iteration time. End-to-end SPS is NOT
#     usable evidence here: on this contended GPU the SAME arm spanned 946-2076 SPS
#     across seeds (+-120%), which dwarfs the readout's cost.
#   * 262k CANNOT decide P1 or P2. The donor's advantage was +5% at 2M and 262k is 1/8 of
#     that FIRST checkpoint; at 262k this chassis is still in its initial transient. The
#     short run is a PLUMBING test (shapes, gradients, actor non-interference, throughput),
#     not evidence about the lever.
# =====================================================================================
#
# --- inherited entppo_v4_samplemask notes (chassis, unchanged) ---
...
class Args:
    ...
    # --- DEEP VALUE READOUT (the one lever; see header) -----------------------------
    # THE A/B SWITCH. --no-deep-value is the CHASSIS, bit-exact: the value readout is the
    # single bias-free zero-init Linear(H, mtp*num_bins). --deep-value replaces that map
    # with the end-to-end residual block stack (ValueNet) over MTP-token conditioning.
    # Nothing else in the file reads this flag: same support, same HL-Gauss labels, same
    # MTP mask, same CE, same reduction, same grad-clip grouping, same actor.
    deep_value: bool = True
    # DONOR DEFAULTS, carried over unchanged and NOT tuned here (dblockcritic_v1:
    # dblock_num_blocks=3, dblock_mult=2). The 9810 @8M donor arm ran exactly these.
    value_blocks: int = 3            # depth of the residual value stack
...
# =====================================================================================
# DEEP VALUE READOUT (ported verbatim from the dblock=False path of
# ppo_continuous_action_iterthink_v24_beta_d3bucket_mtp_ppoadvnorm_batch_dblockcritic_v1.py,
# whose --no-dblock arm scored 9810 @8M against its 8455 linear-readout ancestor).
# NOTHING diffusion-side is ported: no EDM sigma schedule, no preconditioning, no per-block
# CE, no Euler inference loop, no per-block sigma bins, no bin latents. See header.
# =====================================================================================

class TimeEmbed(nn.Module):
    """DiT TimestepEmbedder. Code ported verbatim; the CONDITIONING INPUT here is the
    constant 0 (the donor's e2e path evaluates every block at c_noise == 0), so this module
    contributes a single learned bias vector to the adaLN modulation rather than a schedule.
    It is kept because it is part of the block signature that was measured at 9810 @8M, and
    because ada_w/ada_b are zero-init so the modulation starts as the identity either way.

    INHERITED RATIONALE for the frequency range (moot at a constant input, retained for
    lineage): DiT embeds integer timesteps in [0, 1000] with geometric frequencies in
    [1e-4, 1]; the donor's c_noise spans only [-1.55, 1.10], so that ladder would be nearly
    constant, while a 2**arange(16) ladder HASHES its input instead of encoding it.
    Geometric in [0.5, 8] resolves the donor's range while staying smooth in it."""
...
class BlockStack(nn.Module):
    """The value network's blocks, with a leading BLOCK axis on every parameter.

    One block is a DiT block minus attention -- there is no sequence to attend over here:
    the MTP horizon tokens are conditionally independent given the conditioning, so an
    attention mixer over 6 independent value distributions would buy nothing.

        u = in_proj([RMSNorm(z), RMSNorm(cond)])          # z: residual state, cond: obs code
        u = u * (1 + scale(t)) + shift(t)                 # adaLN conditioning, ZERO-init
        F = out_proj(ReLU(u)^2)                           # out_proj ZERO-init  =>  F == 0 at init

    The leading (num_blocks,) axis and the einsums over it are the donor's batched-sibling
    layout, ported verbatim. This file only ever calls one block at a time (ids = b, G = 1)
    because the readout COMPOSES the blocks into a residual stack -- the batched all-blocks
    call is the diffusion regime's, and it is not reachable here.
    """

    def __init__(self, num_blocks, H, mult):
        super().__init__()
        Hm = mult * H
        self.num_blocks, self.H, self.Hm = num_blocks, H, Hm
        # Affine-free RMSNorm on both inputs (file convention): the trunk feature is
        # unnormalized and z grows along the stack, so the ReLU^2 needs both scales bounded
    ...
    def forward(self, z, cond, t_emb, ids=None):
        """z (G,T,H) -- one residual state per (block, token); cond (T,H) -- shared by every
        block, re-read at every depth; t_emb (G,T,H); ids (G,) selects blocks, None = all."""
        w_in, b_in, w_ada, b_ada, w_out, b_out = (
        ...
        n_z = self.z_norm(z)
        n_c = self.c_norm(cond).unsqueeze(0).expand_as(n_z)
        cat = torch.cat([n_z, n_c], dim=-1)                            # (G,T,2H)
        u = torch.einsum("gtk,gmk->gtm", cat, w_in) + b_in.unsqueeze(1)
        mod = torch.einsum("gth,gmh->gtm", t_emb, w_ada) + b_ada.unsqueeze(1)
        shift, scale = mod.chunk(2, dim=-1)
        u = u * (1.0 + scale) + shift
        return torch.einsum("gtm,ghm->gth", torch.relu(u).pow(2), w_out) + b_out.unsqueeze(1)


class ValueNet(nn.Module):
    """The deep value READOUT: an end-to-end residual block stack over MTP tokens.

        cond_t = critic_feat + h_embed[t]              T = mtp tokens per state
        z_0    = z_start                              learned, NONZERO (see below)
        z_b    = z_{b-1} + f_b(z_{b-1}, cond, t_emb(0))
        logits = decoder(z_B)                         (B, mtp, num_bins)

    Drop-in for the chassis's Linear(H, mtp*num_bins): same output shape, same Dreamer3
    symexp support, same HL-Gauss labels, same masked CE, same reduction. The horizon axis
    lives in the TOKEN (h_embed), never in the decoder, so the decoder is shared across
    horizons and the readout is SMALLER in parameters than the linear head it replaces.

    z_start MUST be nonzero. With a zero start, a zero-init decoder and zero-init out_proj,
    the stack sits on an all-zero fixed point -- dL/d decoder = dL/dlogits (x) z == 0 and
    dL/d out_proj = decoder^T dL/dlogits (x) . == 0 -- and NOTHING learns (donor measured
    gradient norm exactly 0.00 for 4k steps). A learned nonzero query breaks the deadlock
    exactly the way the trunk feature does for the chassis's zero-init linear head, and
    V(s) is still EXACTLY 0 at init.

    t_emb is evaluated at c_noise == 0 for every block (the donor's e2e path; the block
    signature keeps its conditioning input so the ported block code is unmodified). ada_w
    and ada_b are zero-init, so the adaLN modulation starts as the identity.
    """

    def __init__(self, H, num_bins, mtp, args):
        super().__init__()
        self.H, self.num_bins, self.mtp = H, num_bins, mtp
        self.num_blocks = args.value_blocks
        # Per-horizon token embedding: the M MTP heads become M TOKENS sharing one
        # conditioning. Horizon identity lives in the conditioning, never in the decoder.
        self.h_embed = nn.Embedding(mtp, H)
        nn.init.normal_(self.h_embed.weight, std=0.02)
        self.t_embed = TimeEmbed(H)
        self.blocks = BlockStack(self.num_blocks, H, args.value_mult)
    ...
    def logits(self, critic_feat, horizons=None):
        """Value-bucket logits, (B, T, num_bins). The ONLY value readout, so the rollout,
        the bootstrap and the regression target always see the same network."""
        cond = self.cond_tokens(critic_feat, horizons)
        n_tok = cond.shape[0] // critic_feat.shape[0]
        return self._e2e_logits(cond).view(critic_feat.shape[0], n_tok, self.num_bins)

    def _e2e_logits(self, cond):
        T = cond.shape[0]
        t_emb = self.t_embed(torch.zeros(T, device=cond.device)).unsqueeze(0)
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # THE LEVER. Both arms emit (B, mtp, num_bins) logits over the SAME Dreamer3
        # symexp support and are trained by the SAME masked HL-Gauss CE; only the map from
        # the trunk feature to those logits changes. Exactly ONE of the two heads is
        # constructed, so --no-deep-value consumes the chassis's RNG stream unchanged and
        # is bit-exact to the parent file.
        self.num_bins = args.num_bins
        self.critic_mtp_horizon = args.critic_mtp_horizon
        self.deep_value = args.deep_value
        if self.deep_value:
            # Deep readout: end-to-end residual block stack over MTP-token conditioning.
            self.critic = ValueNet(H, args.num_bins, args.critic_mtp_horizon, args)
        else:
    ...
    def _value_logits(self, critic_feat):
        # Value LOGITS (B, mtp, num_bins) from the critic conditioning feature. THE ONLY
        # place the lever acts: identical shape, support and downstream loss in both arms.
        if self.deep_value:
            return self.critic.logits(critic_feat)
    ...
    def get_value(self, x):
        ...
        return self._value_logits(critic_feat)
    ...
    def get_action_and_value(self, x, z=None):
        ...
        value_logits = self._value_logits(critic_feat)
        if self.actor_dist == "gaussian":
    ...
    def critic_parameters(self):
        ...
        # CHASSIS GROUPING, unchanged: trunk + the whole value head form ONE critic group
        # clipped to critic_grad_clip. (The donor grouped identically -- list(trunk) +
        # list(head) -- so there was nothing to reconcile.)
        trunk = self.trunk if self.share_backbone else self.critic_trunk
        head = self.critic if self.deep_value else self.critic_head
        return list(trunk.parameters()) + list(head.parameters())
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    # stashed clip_critic(d vl / d trunk). The value head (linear or deep
                    # block stack) gets the value gradient only.
