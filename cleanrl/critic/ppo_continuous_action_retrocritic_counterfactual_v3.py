# Retrospective Counterfactual-Credit PPO v3
#
# Key idea:
...
# - Train A_r to reconstruct GAE advantages, but regularize it to be
#   action-relative under the same hindsight context:
#       E_{a~pi(.|s)}[A_r(s, a, z_future)] ~= 0
# - Use its detached prediction as a confidence weight on raw GAE. The policy
#   gradient sign still comes from PPO/GAE; hindsight only gates credit.
#
# Hypothesis:
# MuJoCo locomotion has delayed, phase-coupled consequences. The zero-mean
# counterfactual constraint should prevent the hindsight critic from becoming a
# survival/outcome classifier and make it closer to marginal action credit.
import os
import random
import time
...
class Args:
    ...
    retro_zero_mean_coef: float = 0.05
    """coefficient for counterfactual E_pi[A_r(s,a,z)]^2 regularization"""
    retro_zero_mean_samples: int = 4
    """number of counterfactual policy actions per state for zero-mean regularization"""
...
def retro_zero_mean_loss(agent, obs, retro_features, sample_count):
    if sample_count <= 0:
        return torch.zeros((), device=obs.device)

    with torch.no_grad():
        sampled_actions = agent.get_action_dist(obs).sample((sample_count,))

    repeated_obs = obs.unsqueeze(0).expand(sample_count, *obs.shape).reshape(-1, obs.shape[-1])
    repeated_features = retro_features.unsqueeze(0).expand(sample_count, *retro_features.shape).reshape(
        -1, retro_features.shape[-1]
    )
    flat_actions = sampled_actions.reshape(-1, sampled_actions.shape[-1])
    sampled_advantages = agent.get_retro_advantage(repeated_obs, flat_actions, repeated_features).view(
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        retro_zm_loss = torch.zeros((), device=device)
        if args.retro_coef > 0 and args.retro_pretrain_epochs > 0:
            for pretrain_epoch in range(args.retro_pretrain_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    retro_zm_loss = retro_zero_mean_loss(
                        agent, b_obs[mb_inds], b_retro_features[mb_inds], args.retro_zero_mean_samples
                    )

                    optimizer.zero_grad()
                    loss = retro_loss + args.retro_zero_mean_coef * retro_zm_loss
                    loss.backward()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                loss = (
                    ...
                    + v_loss * args.vf_coef
                    + retro_loss * args.retro_coef
                    + retro_zm_loss * args.retro_zero_mean_coef
                )
        ...
        writer.add_scalar("losses/retro_zero_mean_loss", retro_zm_loss.item(), global_step)
