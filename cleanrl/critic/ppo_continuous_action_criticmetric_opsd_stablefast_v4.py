# Critic-Metric OPSD StableFast v4 — bounded geometry and faster CUDA execution.
# =====================================================================================
# v3's return spikes were policy-regime changes, not episode length or failed critic fitting:
# Adam was gradient-clipped and then rescaled to the 0.01 KL ceiling on essentially every
# update, so each 1,000-step episode crossed 62-63 maximum-size policy displacements. Its
# learned metric was also extremely anisotropic despite preserving positive Q-ascent.
#
# v4 makes the KL cap exceptional instead of the effective learning-rate controller. The actor
# rate is below v3's median cap-scaled effective rate, and an identity floor bounds the
# trace-normalized action metric's condition number at 19 for HalfCheetah's six actions. This
# preserves critic-grounded positive-definite preconditioning while reducing gait-regime churn.
#
# Throughput changes preserve the actor/meta numerics: MuJoCo workers run asynchronously,
# replay remains on CUDA, score critics reuse frozen fitted models instead of copied snapshots,
# critic-fit GEMMs use BF16 autocast with FP32 outputs/losses/targets, compatible Adam instances
# use fused kernels, and expensive diagnostics are emitted every log_interval iterations.
# Actor Adam, functional-Adam shadow scoring, KL measurement, and metric meta-gradients remain
# FP32 because their small differences determine the committed policy update.
#
# PASS: the KL cap is usually inactive, applied KL remains below its ceiling, actor raw-gradient
# clipping is no longer nearly universal, metric eigenvalues remain in their structural bounds,
# critic fit remains finite, and return regimes persist across multiple episode cohorts.
# FAIL: cap activation or clipping remains dominant, action saturation approaches one, critic
# fit degrades, or throughput fails to improve against v3 on the same workstation.
# =====================================================================================
