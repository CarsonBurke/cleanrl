# KL/VALUE EPOCH DECOUPLE v1 — on the ppoadvnorm_batch base (8278 @8M reference).
# =====================================================================================
# ONE CHANGE vs the reference recipe (this file + --norm-adv --norm-adv-scope batch
# --no-ret-percnorm): the target_kl=0.03 early stop no longer BREAKS the epoch loop.
# It freezes the POLICY (and entropy/alpha) updates and continues CRITIC-ONLY epochs
# (value backward, 0.25 clip, optimizer step) for the remainder of the 10-epoch budget.
#
# MOTIVATION (from the sffactor line's post-mortem): the reference run trips the KL
# leash in ~84% of iterations (96% in the first half), so the bucket-CE MTP critic
# forfeits most of its epoch budget to policy drift it did not cause. Across sffactor
# variants, policy-epochs-completed was the strongest mediator of final return; this
# decouples the two budgets so the critic trains to its full allowance every iteration.
#
# RISK (pre-registered): critic-only epochs still move the shared trunk, drifting the
# actor without PPO clipping beyond what normal epochs already do; if extra critic
# epochs cause net policy drift, next-iteration approx_kl will trip EARLIER (watch
# debug/policy_epochs_completed falling below the reference's implied ~2-4).
# PASS: >= baseline (5012 @2M / 6716 @4M / 8278 @8M) at matched steps.
# FAIL: >5% below baseline at two consecutive checkpoints.
# =====================================================================================
#
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        clipfracs = []
        # v1 KL/value decouple: the KL leash gates POLICY updates only. On breach,
        # freeze the policy loss and keep running critic-only epochs — the critic's
        # epoch budget should not be taxed by policy drift. (Baseline trips the leash
        # in ~84% of iterations, forfeiting most critic epochs along with the policy's.)
        policy_frozen = False
        policy_epochs_completed = args.update_epochs
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if auto_alpha and not policy_frozen:
                    ...
                    ent_coef_eff = log_alpha.exp().detach() if auto_alpha else args.ent_coef

                if policy_frozen:
                    # Critic-only update: value backward + clip, no policy/entropy
                    # gradient and no alpha update. Actor-head params get no grad
                    # (set_to_none) so Adam skips them; the shared trunk still moves,
                    # which is the same exposure it has in normal dual-backward epochs.
                    optimizer.zero_grad(set_to_none=True)
                    (args.vf_coef * v_loss).backward()
                    frozen_clip = args.critic_grad_clip if args.separate_grad_clip else args.max_grad_norm
                    critic_gn = nn.utils.clip_grad_norm_(critic_params, frozen_clip)
                    optimizer.step()
                elif args.separate_grad_clip:
            ...
            if (not policy_frozen) and args.target_kl is not None and approx_kl > args.target_kl:
                policy_frozen = True
                policy_epochs_completed = epoch + 1
        ...
        writer.add_scalar("debug/policy_epochs_completed", policy_epochs_completed, global_step)
