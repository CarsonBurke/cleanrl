# Dreamer two-hot value head on the exact pre-RMS SiTU-GLU PPO base.
# Base: HalfCheetah-v4__ppo_normres_v2_pre_rms_stiglu_50M__1__1788831191.
# Raw-space labels/expectation; symexp or uniform raw centers; zero head.
# Preserve both trunks, readout scaling, actor RNG, joint Adam/global clipping,
# GAE and update order. Clipped/unclipped MSE controls isolate the loss change.
# Dreamer's actor scaling, optimizer and slow-value regularizer are not imported.
# Hypothesis: categorical regression improves control without changing base PPO.
import os
import random
import time
...
class Args:
    ...
    value_loss: Literal["twohot", "mse", "mse_unclipped"] = "twohot"
    """critic objective; scalar clipping applies only to mse with clip_vloss"""
    reward_norm: bool = True
    """base reward normalization/clipping; false leaves environment rewards raw"""
    value_num_bins: int = 255
    """Dreamer categorical atoms; ignored by scalar MSE controls"""
    value_max_abs: float = 10.0
    ...
    def __init__(
        ...
        *,
        ...
        if value_loss == "twohot":
            ...
            self.critic[-1] = head
        elif value_loss not in ("mse", "mse_unclipped"):
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, targets, old_values, args):
    ...
    if args.value_loss == "twohot":
        ...
        if args.value_loss == "mse" and args.clip_vloss:
...
def main():
    ...
    try:
        ...
        update_grad_norms = torch.empty(max_updates, device=device)
        timer = PhaseTimer()
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss.backward()
                        total_grad_norm = nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                        update_grad_norms[updates].copy_(total_grad_norm)
                        optimizer.step()
            ...
            logged = gather_metrics(
                {
                    ...
                    "grad/joint_preclip_norm": update_grad_norms[:updates].mean(),
                    "grad/joint_clip_fraction": (update_grad_norms[:updates] > args.max_grad_norm).float().mean(),
                    **value_diagnostics,
                }
            )
