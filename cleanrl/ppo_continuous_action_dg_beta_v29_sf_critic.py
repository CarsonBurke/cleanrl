# ============================================================================
# DG v29 -- v18 shell + lejepa_sf_v2 successor-feature critic/TD for advantages.
#
# REFERENCE: ppo_continuous_action_dg_beta_v18_ppoadvnorm.py
#   * reward_norm=False (raw rewards, no NormalizeReward / reward clip)
#   * 16 envs x 128 steps, separate actor/critic ThinkTrunks, Beta actor
#   * actor_epochs=1, 32 minibatches, pure score -(U log pi) + analytic KL trust
#   * critic_epochs=10, minibatches
#   * default: nodg + batch_retstd cool U (the advantage-path ablation arm)
#
# ONLY CHANGE vs v18 critic path: replace HL-Gauss scalar-return CE with lejepa_sf_v2's
# successor-feature TD that defines V for GAE:
#   phi = [e, s, a, a*a, 1]          (e from detached LeJEPA encoder; s is raw normed obs)
#   Lambda = TD-lambda of phi        (ONLY bootstrapped object -- vector)
#   psi(s) predicts Lambda           (critic head; MSE, standardized)
#   w_r <- ridge(phi -> r)           (immediate reward only, closed-form each iter)
#   V = w_r . psi                    (scalar used in GAE advantages)
#
# SSL (encoder+predictor+SIGReg) is outside the PPO loop, once per iteration -- defines e
# only; no reward gradient into the encoder. Hypothesis: SF V fixes the advantage/EV
# pathology of scalar HL-Gauss bootstrap under v18's raw-reward short-rollout stack.
# ============================================================================
import os
import random
import time
...
class Args:
    ...
    dg_use_gate: bool = False  # default nodg for clean advantage transplant
...
if __name__ == "__main__":
    ...
    print(
        f"[v29] SF critic on v18 shell: reward_norm={args.reward_norm} "
        f"T={args.num_steps} sf_dim={sf_dim} nodg={not args.dg_use_gate} "
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ---- Critic: multi-epoch MSE on SF targets ------------------------------------
        ...
        for epoch in range(args.critic_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.sf_alpha < 1.0:
                    # In std space, reward direction is w_r * sf_std; in raw space just w_r.
        ...
        # ---- Actor: v18 score + KL trust, 1 epoch x minibatches ------------------------
