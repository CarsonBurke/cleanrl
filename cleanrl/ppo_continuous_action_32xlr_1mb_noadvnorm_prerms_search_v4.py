# Pre-RMSNorm architecture search, v4. Identity residuals and the PPO learner
# stay fixed. Vary normalization inside the squared branch: projected inputs,
# squared features, or branch output; alternatively learn input gains, preserve
# the critic's final stream magnitude, or match sphere input-matrix scale.
# Inner RMS scales preserve initial second-moment targets (2, 6.375, or .5),
# avoiding an accidental global branch-size sweep. All rollouts use the fused
# host mirror, with separate CUDA actor/critic and unchanged bounded Beta heads.
import os
import random
import time
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, norm_position="input", learned_input_gain=False,
                 actor_final_norm=True, critic_final_norm=True, branch_input_scale=1.0, n_blocks=3):
        super().__init__()
        ...
        self.critic = nn.Sequential(
            make_pre_rms_trunk(observation_dim, 64, n_blocks, norm_position=norm_position,
                               learned_input_gain=learned_input_gain, final_norm=critic_final_norm,
                               branch_input_scale=branch_input_scale),
            layer_init(nn.Linear(64, 1), std=1.0),
        )
        self.actor = nn.Sequential(
            make_pre_rms_trunk(observation_dim, 64, n_blocks, norm_position=norm_position,
                               learned_input_gain=learned_input_gain, final_norm=actor_final_norm,
                               branch_input_scale=branch_input_scale),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", f"Beta host actor; additive preRMS {args.norm_position} trunk; "
        ...
        agent = Agent(envs, norm_position=args.norm_position, learned_input_gain=args.learned_input_gain,
                      ...
                      branch_input_scale=args.branch_input_scale, n_blocks=args.n_blocks).to(device)
