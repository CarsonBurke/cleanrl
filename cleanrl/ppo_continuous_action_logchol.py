# Log-Cholesky PPO with Fixed-Determinant Covariance
#
# Key idea: Covariance signal comes ENTIRELY from Mahalanobis (which noise was
# good/bad), NOT from log_det (entropy pressure). We achieve this structurally
# by fixing det(Σ) = 1 via zero-sum log-diagonal:
#   log_diag = log_diag_raw - mean(log_diag_raw)
#   → prod(exp(log_diag)) = 1 → det(L) = 1 → det(Σ) = 1
#
# The network can only redistribute noise across dimensions and set correlations.
# It cannot inflate/deflate total noise scale. All gradient signal comes from
# Mahalanobis: "for this state, which noise directions matched good vs bad actions."
#
# Architecture:
...
#
# Why this works:
#   - No log_det in loss at all (it's constant = 0)
#   - No norms, bounds, or attenuation needed
#   - Covariance can't explode or collapse — determinant is structurally fixed
#   - Network learns noise SHAPE (allocation + correlations) per state
...
class Agent(nn.Module):
    def __init__(self, envs, rpo_alpha=0.05, num_bins=255, bin_range=8.0):
        ...
        # Separate heads for diagonal (magnitude) and off-diagonal (correlations)
        self.log_diag_head = layer_init(nn.Linear(64, action_dim), std=0.01)
        n_offdiag = action_dim * (action_dim - 1) // 2
        self.offdiag_head = layer_init(nn.Linear(64, n_offdiag), std=0.01)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        # Covariance path: separate heads for log-diagonal and off-diagonal
        ...
        log_det = log_diag.sum(dim=-1)  # ≈ 0 always, but keep for numerics
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            log_diag = log_diag_raw - log_diag_raw.mean(dim=-1, keepdim=True)
            writer.add_scalar("losses/effective_logstd_mean", log_diag_raw.mean().item(), global_step)
            writer.add_scalar("losses/log_diag_std", log_diag.std().item(), global_step)
