# 32x LR + 1 minibatch with a residual LeakyReluSq trunk instead of SiTU-GLU.
# Same host-actor trainer as ppo_continuous_action_32xlr_1mb_v1: lr=9.6e-3,
# num_minibatches=1; PPO loss, clip, critic and Beta head are unchanged. The
# trunk is the exact res_v1 scaffolding (in-proj, two gated blocks, U-net long
# skip, layer-wide scalar gates init sigmoid(-1.5) ~= 0.18, no normalization)
# with each SiTU branch replaced by a LeakyReluSq Linear pair matched to the
# same 0.5 output scale. Clean SiTU-vs-LReluSq ablation: scaffolding, scales,
# and gates identical, only the block nonlinearity differs. Hypothesis: with
# residuals fixing the grad-norm pathology that killed v1 SiTU, the
# saturation-free LReluSq (gradient never vanishes) trains as well or better
# than multiplicative gating here.
import os
import random
import time
...
from cleanrl.shared.host_actor import HostLReluResActor, make_lrelu_res_trunk
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            make_lrelu_res_trunk(observation_dim, 64),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); FP32; residual LeakyReluSq trunk; native-action storage; host actor mirror")
        ...
        host_actor = HostLReluResActor(agent.actor, args.num_envs)
