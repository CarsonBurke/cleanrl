# Critic-only checked curvature Adam on base PPO, v3.
# Isolate v2's useful critic behavior: actor is ordinary fused Adam with the
# BASELINE linear LR schedule. Only the critic uses measured directional
# curvature and checked probe/candidate lengths; no critic LR ceiling/schedule.
# Current raw weighted critic gradient predicts actual parameter displacement.
# Keep the best checked critic point, including origin; uphill momentum uses
# its current preconditioned gradient. Actor updates are never trial/rejected.
# Preserve joint actor+critic gradient clipping (0.5). Adam moments advance once
# per minibatch. Two extra critic-only forwards share frozen PPO value targets.
# Hypothesis: better value fitting improves GAE without moving the policy into
# the saturated uniform-policy attractor observed with actor curvature in v2.
import os
import random
import time
...
def prepare_trial(params, gradients, actor_gradients, moments, seconds, steps, origins, directions,
                  lrs, probe_lrs, predictions, fallback, beta1, beta2, eps, max_grad_norm):
    # Keep the true derivative intact; clipping only conditions Adam's observations.
    all_gradients = actor_gradients + [g for group in gradients for g in group]
    norm = torch.stack([g.square().sum() for g in all_gradients]).sum().sqrt()
    coefficient = (max_grad_norm / (norm + 1e-6)).clamp(max=1.0)
    ...
    for slot, (group, raw, means, variances, counters, saved, vectors) in enumerate(
        ...
        for parameter, gradient, mean, variance, step, origin in zip(group, raw, means, variances, counters, saved):
            ...
            step.add_(1)
            mean.lerp_(clipped, 1.0 - beta1)
            variance.mul_(beta2).addcmul_(clipped, clipped, value=1.0 - beta2)
            denominator = (variance / (1.0 - beta2 ** step)).sqrt() + eps
            vector = (mean / (1.0 - beta1 ** step)) / denominator
            adam.append(vector)
            current.append(clipped / denominator)
...
class CriticCurvatureAdam(optim.Optimizer):
    """Critic-only trials; actor gradients share clipping, not curvature control.
    ...
    def __init__(self, actor_params, critic_params, lr=3e-4, betas=(0.9, 0.999),
                 eps=1e-5, max_grad_norm=0.5, compile=True):
        self.actor_params = list(actor_params)
        groups = [list(critic_params)]
        flat = self.actor_params + groups[0]
        if not self.actor_params or not groups[0] or len({id(p) for p in flat}) != len(flat):
            raise ValueError("CriticCurvatureAdam requires nonempty disjoint actor and critic parameters")
        if not all(np.isfinite(v) and v > 0 for v in (lr, eps, max_grad_norm)):
            raise ValueError("LR, epsilon and gradient norm must be positive and finite")
    ...
    @torch.no_grad()
    def probe(self):
        if any(p.grad is None or p.grad.is_sparse for p in self.actor_params):
            raise RuntimeError("Each actor parameter needs a dense current gradient")
        actor_gradients = [p.grad for p in self.actor_params]
        gradients = []
        ...
        self._prepare(self.groups, gradients, actor_gradients, self.moments, self.seconds, self.steps,
...
def value_loss(newvalue, returns, old_values, clip_coef, clip_vloss):
    newvalue = newvalue.view(-1)
    squared_error = (newvalue - returns) ** 2
    if clip_vloss:
        clipped = old_values + torch.clamp(newvalue - old_values, -clip_coef, clip_coef)
        squared_error = torch.max(squared_error, (clipped - returns) ** 2)
    return 0.5 * squared_error.mean()


def critic_objective(agent, observations, returns, old_values, args):
    """Same weighted value objective as PPO; no actor/distribution computation."""
    return (args.vf_coef * value_loss(agent.get_value(observations), returns, old_values,
                                     args.clip_coef, args.clip_vloss)).reshape(1)
...
def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args):
    ...
    v_loss = value_loss(newvalue, returns, old_values, args.clip_coef, args.clip_vloss)
...
def main():
    ...
    try:
        ...
        optimizer = CriticCurvatureAdam(agent.actor.parameters(), agent.critic.parameters(),
                                       lr=args.learning_rate, max_grad_norm=args.max_grad_norm, compile=args.compile)
        ...
        def check_model(observations, returns, old_values):
            return critic_objective(agent, observations, returns, old_values, args)
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        loss.backward()
                        # Joint clipping is performed once by probe; critic raw
                        # gradients survive for prediction, actor grads are clipped.
                        optimizer.probe()
                        actor_optimizer.step()
                        with torch.no_grad():
                            probe_loss = check_model(minibatch[0], minibatch[4], minibatch[5]).clone()
                            optimizer.propose(before_loss, probe_loss)
                            candidate_loss = check_model(minibatch[0], minibatch[4], minibatch[5])
