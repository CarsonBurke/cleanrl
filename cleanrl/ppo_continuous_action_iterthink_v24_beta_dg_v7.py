...
#
# ============================ DG variant (v7) =================================
# v7 = the FAITHFUL-DG FIX, from an adversarial audit of v6 against the paper's Algorithm 1
# (dtheta += w_t * U_t * grad log pi(A_t); w_t = sigmoid(U_t * ell_t / eta); eta=1; raw U=R-b).
# ONE root cause, ONE fix:
#
#  THE IMPLEMENTATION BUG -- the gate was computed on the WRONG advantage. v6 defaulted to
#  adv_transform="rankgauss" (quantizes the advantage to its batch-RANK, destroying the
#  magnitude) AND norm_adv=True (per-minibatch re-standardization). So chi = U*ell used RANK,
#  not the raw breakthrough/blunder size R-b the paper's gate needs ("up-weight rare LARGE
#  successes"). FIX: adv_transform="v10" (raw GAE) + norm_adv=False, so the SAME raw U=R-b flows
#  into BOTH the gate chi AND the score term, exactly as Algorithm 1.
#
# NO TRUST REGION, NO ENTROPY CRUTCH (faithful). With a SINGLE actor epoch (--actor-epochs 1)
# each sample is used exactly ONCE -> the update is on-policy and there is nothing to drift
# against, so no ratio/clip and no KL leash are needed (unlike PPO's 10-epoch reuse). v6's
# no-gate "divergence" (approx_kl~437) was an ARTIFACT of the SAME bug: standardized advantages
# make every positive sample U~+1, so the 32 minibatch steps all push concentration UP in
# lockstep and compound into a blow-up. RAW signed/magnitude-weighted U does not align the steps
# that way, so 1 epoch is stable on its own. A PPO-penalty KL leash remains available as an
# OFF-by-default ablation (--dg-kl-coef >0), not part of faithful DG.
#
# dg_surprisal defaults to "cdf_tail" (pure Beta action tail, E~1, concentration-invariant) --
# the purest faithful surprisal. dg_use_gate True vs False is the gate A/B (does the delight gate
# help vs plain REINFORCE-score), both now in the SAME faithful raw-advantage regime.
#
# ============================ DG variant (v6) =================================
...
class Args:
    ...
    dg_surprisal: str = "cdf_tail"    # v7: pure Beta action tail (E~1, concentration-invariant) -- the purest faithful surprisal. "hybrid" (0.5*(policy+critic), v5) | "critic" (v4) | "cdf_tail" | "mode_ref" | "raw_clip" | "entref"
