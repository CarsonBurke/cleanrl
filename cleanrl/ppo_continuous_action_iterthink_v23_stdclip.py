# PPO + IterThink v23 (DIRECT LOG-STD + SEPARATE STD GRAD-CLIP). From v22.
#
# WHY v23. v22 added a state-dependent logstd HEAD but left it inside the actor's
# single decoupled grad-clip group (trunk + mean head + logstd head all share one
# actor_grad_clip norm). Because the shared trunk dominates that joint norm, the
# logstd head's gradient is renormalized by a trunk-driven factor — its effective
# learning step is whatever survives, i.e. it gets SWAMPED, exactly the failure
# mode decoupled clipping was meant to fix (just one level down). v23 promotes the
# std head to its OWN clip group: a THREE-way decoupled dual-backward clip —
#     critic_grad_clip : value grad on (trunk + critic_head)
#     actor_grad_clip  : policy grad on (trunk + mean head)
#     logstd_grad_clip : policy grad on the logstd head ALONE
# so the exploration-scale update has an independent budget and can adapt without
# being dominated by (or dominating) the mean/trunk gradient. The trunk's budget
# is unchanged (still actor+critic clips; the std head doesn't touch it), so this
# is a clean one-factor change vs v22. `logstd_grad_clip` is the new knob.
# Bar to beat: v22 (shared-clip std head) and v21 (~8774, flat logstd).
#
# --- inherited v22 notes ---
...
class Args:
    ...
    actor_grad_clip: float = 0.25    # max-norm for the policy gradient on (trunk + mean head)
    critic_grad_clip: float = 0.25   # max-norm for the value gradient (incl. its trunk part)
    logstd_grad_clip: float = 0.25   # v23: SEPARATE max-norm for the logstd head's gradient
...
class Agent(nn.Module):
    ...
    def logstd_parameters(self):
        # v23: the exploration-scale params, clipped to their OWN norm budget.
        return (
            list(self.actor_logstd_head.parameters()) if self.direct_logstd else [self.actor_logstd]
        )

    def actor_mean_parameters(self):
        # v23: policy-gradient params EXCLUDING the logstd group (trunk + mean head).
        trunk = self.trunk if self.share_backbone else self.actor_trunk
        return list(trunk.parameters()) + list(self.actor_head.parameters())

    def actor_parameters(self):
        # All params receiving the POLICY gradient (mean group + logstd group).
        return self.actor_mean_parameters() + self.logstd_parameters()
...
if __name__ == "__main__":
    ...
    actor_mean_params = agent.actor_mean_parameters()
    logstd_params = agent.logstd_parameters()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.separate_grad_clip:
                    ...
                    # v23: THREE-way decoupling. The mean group (trunk + mean head)
                    # and the logstd head are disjoint param sets, so clipping each
                    # to its own max-norm independently rescales their grads — the
                    # std head no longer rides the trunk-dominated actor norm.
                    actor_gn = nn.utils.clip_grad_norm_(actor_mean_params, args.actor_grad_clip)
                    logstd_gn = nn.utils.clip_grad_norm_(logstd_params, args.logstd_grad_clip)
                    ...
                    optimizer.zero_grad()
                    loss.backward()
                    critic_gn = actor_gn = logstd_gn = nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
                    optimizer.step()
        ...
        writer.add_scalar("losses/logstd_grad_norm", float(logstd_gn), global_step)
