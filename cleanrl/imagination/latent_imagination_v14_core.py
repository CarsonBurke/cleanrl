...
#
# v14 adds Dreamer4-style relevant-sequence sampling on top of the v13 path:
# 1. behavior and imagination batches are a mixture of uniform and relevant
#    replay contexts,
# 2. relevant means high short-horizon return among oversampled candidates,
# 3. behavior cloning is applied only to the relevant fraction of the batch.
import copy
import os
import random
import time
...
class Args:
    ...
    behavior_prior_coef: float = 0.3
    """weight on reverse KL to the rollout-time real-policy distribution during behavior learning"""
    ...
    behavior_relevance_threshold: float = 0.5
    """fraction of each batch reserved for relevant replay segments"""
    behavior_relevance_candidate_multiplier: int = 8
    """candidate oversampling factor used to choose relevant replay segments"""
...
class SequenceReplayBuffer:
    def __init__(self, capacity_steps: int, obs_shape, action_shape, num_envs: int):
        ...
        self.policy_mean = np.zeros((capacity_steps, num_envs) + action_shape, dtype=np.float32)
        self.policy_logstd = np.zeros((capacity_steps, num_envs) + action_shape, dtype=np.float32)
        self.policy_valid = np.zeros((capacity_steps, num_envs), dtype=np.float32)
    ...
    def add(self, obs, next_obs, actions, rewards, dones, policy_mean, policy_logstd, policy_valid):
        self.obs[self.pos] = obs
        self.next_obs[self.pos] = next_obs
        self.actions[self.pos] = actions
        self.rewards[self.pos] = rewards
        self.dones[self.pos] = dones
        self.policy_mean[self.pos] = policy_mean
        self.policy_logstd[self.pos] = policy_logstd
        self.policy_valid[self.pos] = policy_valid
    ...
    def _compute_relevance_scores(self, time_indices, env_indices, horizon: int, gamma: float) -> np.ndarray:
        scores = np.zeros(len(time_indices), dtype=np.float32)
        current_step_ids = self._gather_step_ids(time_indices, env_indices)
        alive = np.ones(len(time_indices), dtype=bool)
        discount = 1.0
        for offset in range(horizon):
            ...
            if np.any(valid):
                scores[valid] += discount * self.rewards[future_slots[valid], env_indices[valid]]
                alive[valid] &= self.dones[future_slots[valid], env_indices[valid]] < 0.5
            alive &= valid
            discount *= gamma
        return scores

    def sample_mixed_indices(self, batch_size: int, horizon: int, gamma: float, relevant_fraction: float, candidate_multiplier: int):
        num_relevant = int(round(batch_size * relevant_fraction))
        num_uniform = batch_size - num_relevant

        uniform_t, uniform_e = self.sample_indices(num_uniform) if num_uniform > 0 else (np.empty(0, dtype=np.int64), np.empty(0, dtype=np.int64))

        if num_relevant <= 0:
            relevant_mask = np.zeros(batch_size, dtype=bool)
            return uniform_t, uniform_e, relevant_mask

        candidate_count = max(num_relevant, num_relevant * max(candidate_multiplier, 1))
        cand_t, cand_e = self.sample_indices(candidate_count)
        scores = self._compute_relevance_scores(cand_t, cand_e, horizon, gamma)
        topk = np.argsort(scores)[-num_relevant:]
        rel_t, rel_e = cand_t[topk], cand_e[topk]

        time_indices = np.concatenate([uniform_t, rel_t], axis=0)
        env_indices = np.concatenate([uniform_e, rel_e], axis=0)
        relevant_mask = np.zeros(batch_size, dtype=bool)
        relevant_mask[num_uniform:] = True
        if batch_size > 1:
            perm = np.random.permutation(batch_size)
            time_indices = time_indices[perm]
            env_indices = env_indices[perm]
            relevant_mask = relevant_mask[perm]
        return time_indices, env_indices, relevant_mask
    ...
    def get_policy_tensors(self, time_indices, env_indices, device: torch.device):
        policy_mean = torch.tensor(self.policy_mean[time_indices, env_indices], device=device, dtype=torch.float32)
        policy_logstd = torch.tensor(self.policy_logstd[time_indices, env_indices], device=device, dtype=torch.float32)
        policy_valid = torch.tensor(self.policy_valid[time_indices, env_indices], device=device, dtype=torch.float32)
        return policy_mean, policy_logstd, policy_valid
...
def compute_behavior_clone_loss(
    ...
    old_action_logstd: torch.Tensor,
    old_policy_valid: torch.Tensor,
    args: Args,
    ...
    current_mask = behavior_action_mask[:, 0] & relevant_mask
    if current_mask.any():
        current_loss = current_loss[current_mask].mean()
    else:
        current_loss = current_loss.mean() * 0.0

    if agent.multi_token_pred_len <= 1:
        old_dist = Normal(old_action_mean.detach(), torch.exp(old_action_logstd.detach()))
        valid = (old_policy_valid > 0.5) & relevant_mask
        prior_kl = kl_divergence(old_dist, dist).sum(-1)
        if valid.any():
            prior_kl_loss = prior_kl[valid].mean()
        else:
            prior_kl_loss = current_loss * 0.0
        return current_loss, prior_kl_loss, relevant_mask.float().mean()
    ...
    future_mask = behavior_action_mask[:, 1:] & relevant_mask.unsqueeze(1)
    ...
    if valid.any():
        ...
        prior_kl_loss = current_loss * 0.0
    return current_loss + future_loss, prior_kl_loss, relevant_mask.float().mean()
...
def compute_imagination_losses(
    ...
    with torch.no_grad():
        ...
        for _ in range(args.imagination_horizon):
            ...
            step_value = twohot_predict(step_value_logits, agent.value_bins)

            latent_seq.append(latent)
            context_seq.append(context_state)
            action_seq.append(action)
            old_value_seq.append(step_value)
            old_mean_seq.append(dist.loc)
            old_std_seq.append(dist.scale)
            alive_weights.append(alive)

            env_action = agent.clamp_action(action)
            ...
            reward_seq.append(reward_pred)
            continuation_seq.append(continuation)
    ...
    context_seq = torch.stack(context_seq)
    action_seq = torch.stack(action_seq)
    reward_seq = torch.stack(reward_seq)
    continuation_seq = torch.stack(continuation_seq)
    old_value_seq = torch.stack(old_value_seq)
    old_mean_seq = torch.stack(old_mean_seq)
    old_std_seq = torch.stack(old_std_seq)
    alive_weights = torch.stack(alive_weights).detach()

    returns = lambda_returns(
        reward_seq,
        continuation_seq,
        old_value_seq.detach(),
        bootstrap.detach(),
        args.gamma,
        args.imagination_lambda,
    )
    advantages = (returns - old_value_seq).detach()

    flat_latent = latent_seq.reshape(-1, latent_seq.shape[-1])
    flat_context = context_seq.reshape(-1, context_seq.shape[-1])
    flat_actions = action_seq.reshape(-1, action_seq.shape[-1])
    flat_old_means = old_mean_seq.reshape(-1, old_mean_seq.shape[-1])
    flat_old_stds = old_std_seq.reshape(-1, old_std_seq.shape[-1])

    current_dist = agent.get_actor_dist_from_latent_context(flat_latent, flat_context)
    current_logprobs = current_dist.log_prob(flat_actions.detach()).sum(-1)
    current_value_logits = agent.get_value_logits_from_latent_context(flat_latent, flat_context)
    old_dist = Normal(flat_old_means.detach(), flat_old_stds.detach())
    prior_kls = kl_divergence(old_dist, current_dist).sum(-1)

    flat_advantages = advantages.reshape(-1)
    flat_logprobs = current_logprobs.reshape(-1)
    flat_weights = alive_weights.reshape(-1)
    flat_prior_kls = prior_kls.reshape(-1)
    flat_value_logits = current_value_logits.reshape(-1, current_value_logits.shape[-1])
...
def main(args_class=Args):
    ...
    last_imag_stats = {
        ...
        "behavior_prior_kl_loss": torch.zeros((), device=device),
        "behavior_relevance": torch.zeros((), device=device),
    ...
    for iteration in range(1, args.num_iterations + 1):
        for _ in range(args.num_steps):
            ...
            if global_step < args.learning_starts:
                ...
                policy_mean = np.zeros_like(actions_np, dtype=np.float32)
                policy_logstd = np.zeros_like(actions_np, dtype=np.float32)
                policy_valid = np.zeros((args.num_envs,), dtype=np.float32)
            else:
                with torch.no_grad():
                    ...
                    policy_mean = dist.loc.cpu().numpy().astype(np.float32)
                    policy_logstd = torch.log(dist.scale).cpu().numpy().astype(np.float32)
                    policy_valid = np.ones((args.num_envs,), dtype=np.float32)
        ...
        for _ in range(args.world_model_gradient_steps):
            time_indices, env_indices, _ = replay.sample_mixed_indices(
                args.world_model_batch_size,
                args.multi_token_pred_len,
                args.gamma,
                args.behavior_relevance_threshold,
                args.behavior_relevance_candidate_multiplier,
            )
        ...
        if global_step >= args.learning_starts:
            for _ in range(args.imagination_gradient_steps):
                time_indices, env_indices, relevant_mask_np = replay.sample_mixed_indices(
                    args.imagination_batch_size,
                    args.multi_token_pred_len,
                    args.gamma,
                    args.behavior_relevance_threshold,
                    args.behavior_relevance_candidate_multiplier,
                )
                relevant_mask = torch.tensor(relevant_mask_np, device=device, dtype=torch.bool)
                start_obs, _, _, _, _ = replay.get_tensors(time_indices, env_indices, device)
                old_action_mean, old_action_logstd, old_policy_valid = replay.get_policy_tensors(time_indices, env_indices, device)
                ...
                bc_loss, behavior_prior_kl_loss, behavior_relevance = compute_behavior_clone_loss(
                    agent,
                    start_obs,
                    start_context,
                    action_target_batch,
                ...
                behavior_loss = bc_loss + args.behavior_prior_coef * behavior_prior_kl_loss
                total_imag_loss = imag_stats["total_loss"] * imag_coef + current_behavior_clone_coef(args, global_step) * behavior_loss
                ...
                last_imag_stats["behavior_prior_kl_loss"] = behavior_prior_kl_loss.detach()
                last_imag_stats["behavior_relevance"] = behavior_relevance.detach()
        ...
        writer.add_scalar("imagination/behavior_prior_kl", last_imag_stats["behavior_prior_kl_loss"].item(), global_step)
        writer.add_scalar("imagination/behavior_relevance", last_imag_stats["behavior_relevance"].item(), global_step)
