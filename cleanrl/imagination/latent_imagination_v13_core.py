...
class Args:
    ...
    """top quantile cutoff used to upweight relevant short-horizon replay segments"""
    behavior_relevance_weight: float = 2.0
    """extra multiplicative weight for relevant replay segments"""
...
def compute_behavior_clone_loss(
    ...
    discounts = torch.pow(args.gamma, torch.arange(reward_targets.shape[1], device=reward_targets.device, dtype=torch.float32))
    masked_rewards = reward_targets * reward_target_mask.float()
    relevance_scores = (masked_rewards * discounts.unsqueeze(0)).sum(dim=1)
    threshold = torch.quantile(relevance_scores.detach(), args.behavior_relevance_threshold)
    relevance_weights = 1.0 + args.behavior_relevance_weight * (relevance_scores >= threshold).float()

    current_loss = -dist.log_prob(behavior_actions[:, 0].detach()).sum(-1)
    current_mask = behavior_action_mask[:, 0]
    if current_mask.any():
        current_loss = (relevance_weights[current_mask] * current_loss[current_mask]).sum() / relevance_weights[current_mask].sum().clamp_min(1e-6)
    else:
    ...
    if future_mask.any():
        expanded_weights = relevance_weights.unsqueeze(1).expand_as(future_nll)
        future_loss = (expanded_weights[future_mask] * future_nll[future_mask]).sum() / expanded_weights[future_mask].sum().clamp_min(1e-6)
    else:
    ...
    if valid.any():
        ...
        prior_kl_loss = current_loss * 0.0
    return current_loss + future_loss, prior_kl_loss, relevance_scores.mean()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if global_step >= args.learning_starts:
            for _ in range(args.imagination_gradient_steps):
                ...
                action_target_batch, reward_target_batch, action_target_mask = replay.get_mtp_targets(
                    time_indices, env_indices, args.multi_token_pred_len, device
                )
