...
#
# v22 keeps the scaled v21 setup, but replaces plain MultiheadAttention with
# an explicit SDPA attention block that supports grouped query heads. This is
# closer to Dreamer4's attention path while keeping the smaller MuJoCo-sized
# world model from v21.
import math
import os
import random
import time
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None, key_padding_mask=None):
        h = self.norm1(x)
        attn_out = self.attn(h, attn_mask=attn_mask, key_padding_mask=key_padding_mask)
        x = x + attn_out
...
def main(args_class=Args):
    ...
    print(f"Starting latent_imagination_v22 on {args.env_id} with {args.num_envs} envs")
