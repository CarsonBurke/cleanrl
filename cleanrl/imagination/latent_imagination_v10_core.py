...
#
# v10 fixes two bootstrap failures in v9:
# 1. the PMPO loss now uses score-function gradients instead of
#    `log_prob(rsample())`, which was cancelling the actor-mean gradient, and
# 2. the actor gets a Dreamer-style replay-action cloning anchor so it can learn
#    grounded behavior before imagination dominates.
#
# The resulting loop stays PPO-free:
# 1. collect real transitions with the current actor,
# 2. store them in a sequence replay buffer,
# 3. train the encoder + world model on replay,
# 4. freeze the world model for an imagination window,
# 5. improve the same actor/value heads with PMPO, twohot value learning, a
#    frozen prior, and replay-action cloning.
import copy
import os
import random
import time
...
class Args:
    ...
    policy_learning_starts: int = 20_000
    """real env steps before imagination policy training starts"""
    prior_snapshot_step: int = 200_000
    """real env step when the persistent prior is snapshotted"""
    ...
    """weight on KL to the persistent prior"""
    ...
    """linear decay length for replay-action cloning after prior snapshot"""
...
class SequenceReplayBuffer:
    ...
    def build_context_states(
        ...
        for offset in range(context_len, 0, -1):
            ...
            if self.full:
                valid = np.ones_like(prev_indices, dtype=bool)
                prev_indices = np.mod(prev_indices, size)
            else:
                valid = prev_indices >= 0
            if not np.any(valid):
                continue
            prev_obs = torch.tensor(self.obs[prev_indices[valid], env_indices[valid]], device=device, dtype=torch.float32)
            prev_actions = torch.tensor(
                self.actions[prev_indices[valid], env_indices[valid]], device=device, dtype=torch.float32
            )
            prev_dones = torch.tensor(self.dones[prev_indices[valid], env_indices[valid]], device=device, dtype=torch.float32)
...
def build_policy_prior(agent: Agent):
    ...
    prior_actor_context_proj = copy.deepcopy(agent.actor_context_proj).eval()
    for parameter in prior_actor_mean.parameters():
        parameter.requires_grad = False
    for parameter in prior_actor_context_proj.parameters():
        parameter.requires_grad = False
    prior_logstd = agent.actor_logstd.detach().clone()
    return prior_actor_mean, prior_actor_context_proj, prior_logstd
...
def compute_behavior_clone_loss(
    ...
    return -dist.log_prob(behavior_actions.detach()).sum(-1).mean()
...
def compute_imagination_losses(
    ...
    for _ in range(args.imagination_horizon):
        ...
        if prior is not None:
            prior_actor_mean, prior_actor_context_proj, prior_logstd = prior
            prior_latent = latent + prior_actor_context_proj(context_state)
    ...
    if prior_kls:
        ...
        prior_kls = torch.zeros_like(logprobs)
...
def main(args_class=Args):
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if imagination_prior is None and global_step >= prior_snapshot_step(args):
        ...
        if global_step >= args.policy_learning_starts:
            for _ in range(args.imagination_gradient_steps):
                ...
                start_obs, _, start_actions, _, _ = replay.get_tensors(time_indices, env_indices, device)
                start_context = replay.build_context_states(agent, time_indices, env_indices, args.context_len, device)
                imag_stats = compute_imagination_losses(agent, start_obs, start_context, args, imagination_prior)
                bc_loss = compute_behavior_clone_loss(agent, start_obs, start_context, start_actions)
                total_imag_loss = imag_stats["total_loss"] + current_behavior_clone_coef(args, global_step) * bc_loss
                imagination_optimizer.zero_grad()
                (current_imagination_coef(args, global_step) * total_imag_loss).backward()
        ...
        writer.add_scalar("imagination/prior_active", 1.0 if imagination_prior is not None else 0.0, global_step)
