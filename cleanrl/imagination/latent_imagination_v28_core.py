...
#
# v28: Fix PMPO actor loss explosion from v27, aligned with d4 SimTrainer.
#
# Fixes:
# - 2-epoch actor training per train_step (matching d4 SimTrainer epochs=2)
#   KL(old||new) is zero in epoch 1 but nonzero in epoch 2, providing constraint
# - Reduce wm_only_steps to 10k (d4 does 10k BehaviorClone before dreaming)
# - Enable return normalization by default
# - Clamp log_probs to [-20, 2] in PMPO to prevent unbounded gradients
import math
import os
import random
import time
...
class GroupedQueryAttention(nn.Module):
    ...
    def _append_kv_cache(self, k_new, v_new, kv_cache, cache_lengths, max_seq_len):
        ...
        prev_cache_len = k_prev.shape[-2]
        total_len = min(max_seq_len, prev_cache_len + new_len)
        max_keep = total_len - new_len

        # Vectorized: compute how many cached tokens to keep per batch element
        prev_len = cache_lengths.clamp(max=prev_cache_len).clamp(max=max_keep)  # (batch,)
        keep_from = (prev_len - max_keep).clamp(min=0)  # (batch,)
        kept = prev_len - keep_from  # (batch,)

        # Build index tensors for scatter/gather
        device = k_new.device
        pos = torch.arange(total_len, device=device).unsqueeze(0)  # (1, total_len)

        # Mask for prev cached positions: pos < kept[b]
        prev_mask = pos < kept.unsqueeze(1)  # (batch, total_len)
        # Mask for new positions: kept[b] <= pos < kept[b] + new_len
        new_mask = (pos >= kept.unsqueeze(1)) & (pos < (kept + new_len).unsqueeze(1))

        # Source indices for prev cache: keep_from[b] + pos
        prev_src_idx = keep_from.unsqueeze(1) + pos  # (batch, total_len)
        prev_src_idx = prev_src_idx.clamp(max=prev_cache_len - 1)
        # Source indices for new: pos - kept[b]
        new_src_idx = pos - kept.unsqueeze(1)  # (batch, total_len)
        new_src_idx = new_src_idx.clamp(min=0, max=new_len - 1)

        # Expand for heads dim
        prev_src = prev_src_idx.unsqueeze(1).unsqueeze(-1).expand(-1, self.heads, -1, self.dim_head)
        new_src = new_src_idx.unsqueeze(1).unsqueeze(-1).expand(-1, self.heads, -1, self.dim_head)
        prev_m = prev_mask.unsqueeze(1).unsqueeze(-1).expand(-1, self.heads, -1, self.dim_head)
        new_m = new_mask.unsqueeze(1).unsqueeze(-1).expand(-1, self.heads, -1, self.dim_head)

        k_from_prev = k_prev.gather(2, prev_src)
        v_from_prev = v_prev.gather(2, prev_src)
        k_from_new = k_new.gather(2, new_src)
        v_from_new = v_new.gather(2, new_src)

        k_all = torch.where(prev_m, k_from_prev, torch.where(new_m, k_from_new, k_new.new_zeros(1)))
        v_all = torch.where(prev_m, v_from_prev, torch.where(new_m, v_from_new, v_new.new_zeros(1)))
        return k_all, v_all
...
class Actor(nn.Module):
    """Actor matching dreamer4's architecture: policy_head MLP + linear unembed.

    The policy_head transforms features to a policy embedding, then a simple
    linear projection produces (mean, log_var). The linear unembed limits how
    fast the distribution can change, preventing PMPO loss explosion.
    """
    def __init__(self, feat_dim, act_dim, units, layers, act="silu"):
        super().__init__()
        # Policy head MLP: feat_dim → policy_embed_dim (like dreamer4's policy_head)
        policy_embed_dim = units
        self.mlp = MLP(feat_dim, policy_embed_dim, units, layers, act=act, out_scale=1.0)
        # Linear unembed: policy_embed → (mean, log_var) per action dim
        # Small init (1e-2) like dreamer4's continuous_action_unembed
        self.unembed = nn.Linear(policy_embed_dim, act_dim * 2, bias=False)
        nn.init.normal_(self.unembed.weight, std=1e-2)
        self.act_dim = act_dim

    def stats(self, feat):
        policy_embed = self.mlp(feat)
        out = self.unembed(policy_embed)
...
class Args:
    ...
    pmpo_kl_weight: float = 0.3  # reverse KL constraint weight (dreamer4 default)
    actor_epochs: int = 2  # match dreamer4 SimTrainer epochs=2
    gae_discount: float = 0.997
    keep_return_ema_stats: bool = True
    ...
    wm_only_steps: int = 25000  # WM-only training before imagination begins
...
def main(args_class=Args):
    ...
    # Target actor for KL constraint — stores previous step's parameters
    # so KL(old_policy || new_policy) is meaningful (not zero)
    import copy
    ...
    # torch.compile
    # Compile actor forward (MLP + unembed) separately (Normal distribution breaks torch.compile)
    def actor_forward(feat):
        return actor.unembed(actor.mlp(feat))
    actor_mlp_compiled = actor_forward
    if args.compile:
        ...
        actor_mlp_compiled = torch.compile(actor_forward)
    ...
    def train_step(wm_only=False):
        ...
        with torch.no_grad():
            ...
            # Build seed tensors using fully vectorized gather
            # ctx_start: (B, K), ctx_len: (B, K)
            # Build (BK, hist_len) index tensor for gathering from (B, T, ...)
            time_idx_range = torch.arange(hist_len, device=device).unsqueeze(0).unsqueeze(0)  # (1, 1, hist_len)
            cs_expanded = ctx_start.unsqueeze(2)  # (B, K, 1)
            cl_expanded = ctx_len.unsqueeze(2)  # (B, K, 1)
            # gather_idx[b,k,h] = cs[b,k] + h, clamped to valid range
            gather_idx = (cs_expanded + time_idx_range).clamp(max=T - 1)  # (B, K, hist_len)
            # valid_pos[b,k,h] = h < cl[b,k]
            valid_pos = time_idx_range < cl_expanded  # (B, K, hist_len)

            # Flatten to (BK, hist_len)
            gather_idx_flat = gather_idx.reshape(BK, hist_len)
            valid_pos_flat = valid_pos.reshape(BK, hist_len)

            # Batch indices for advanced indexing
            batch_idx = torch.arange(B, device=device).unsqueeze(1).expand(B, K).reshape(BK)  # (BK,)

            # Gather tokens: tokens_d[batch_idx[bk], gather_idx_flat[bk, h]]
            gi = gather_idx_flat.unsqueeze(-1).expand(-1, -1, token_dim)  # (BK, hist_len, token_dim)
            seed_obs_tokens = tokens_d[batch_idx].gather(1, gi)  # (BK, hist_len, token_dim)
            seed_obs_tokens = seed_obs_tokens * valid_pos_flat.unsqueeze(-1)

            gi_act = gather_idx_flat.unsqueeze(-1).expand(-1, -1, act_dim)
            seed_prev_act = prev_act_d[batch_idx].gather(1, gi_act)
            seed_prev_act = seed_prev_act * valid_pos_flat.unsqueeze(-1)

            seed_prev_rew = rew_d[batch_idx].gather(1, gather_idx_flat)
            seed_prev_rew = seed_prev_rew * valid_pos_flat

            seed_is_first_raw = is_first[batch_idx].gather(1, gather_idx_flat)
            seed_is_first = seed_is_first_raw | ~valid_pos_flat  # invalid positions treated as first

            seed_len = ctx_len.reshape(BK)
            seed_valid = (k_idx < num_seeds.unsqueeze(1)).reshape(BK)
            ...
            # Fixed gamma discount like dreamer4 — no continue head in imagination GAE
        ...
        # ===== Actor (PMPO) with epochs — matching d4 SimTrainer epochs=2 =====
        # Center advantages so ~50% are positive — prevents PMPO degeneration
        # when all advantages share the same sign (common with early/inaccurate critic)
        adv_detached = advantages.detach()
        adv_mean = adv_detached[seed_rollout_mask].mean() if seed_rollout_mask.any() else adv_detached.new_zeros(())
        adv_centered = adv_detached - adv_mean
        pos_mask = (adv_centered >= 0) & seed_rollout_mask
        neg_mask = (adv_centered < 0) & seed_rollout_mask
        alpha = args.pmpo_pos_neg_weight

        # Capture target actor stats BEFORE any updates (like d4's old_action_unembeds)
        with torch.no_grad():
            old_mu, old_logvar = target_actor.stats(imag_actor_feats.detach())
            old_logvar = old_logvar.clamp(-10, 4)
            old_std = (0.5 * old_logvar).exp()

        for _actor_epoch in range(args.actor_epochs):
            actor_opt.zero_grad()

            policy_dist = actor(imag_actor_feats.detach())
            logpi_raw = policy_dist.log_prob(imag_acts_flat.detach()).reshape(BK, args.imag_horizon, act_dim)
            logpi = logpi_raw.clamp(-20, 2)  # prevent unbounded PMPO gradients
            entropy = policy_dist.entropy().reshape(BK, args.imag_horizon, act_dim).sum(-1)

            pos_mask_3d = pos_mask.unsqueeze(-1).expand_as(logpi)
            neg_mask_3d = neg_mask.unsqueeze(-1).expand_as(logpi)
            pos_term = logpi[pos_mask_3d].mean() if pos_mask.any() else logpi.new_zeros(())
            neg_term = -logpi[neg_mask_3d].mean() if neg_mask.any() else logpi.new_zeros(())
            pmpo_loss = -(alpha * pos_term + (1 - alpha) * neg_term)

            # KL(old || new) — old stats fixed from before epoch loop
            # Epoch 1: KL≈0 (target just copied). Epoch 2+: KL>0 (actor updated)
            kl_loss = logpi.new_zeros(())
            if args.pmpo_kl_weight > 0:
                new_mu, new_logvar = actor.stats(imag_actor_feats.detach())
                new_logvar = new_logvar.clamp(-10, 4)
                new_std = (0.5 * new_logvar).exp()
                old_dist = Normal(old_mu, old_std)
                new_dist = Normal(new_mu, new_std)
                kl_per_dim = torch.distributions.kl.kl_divergence(old_dist, new_dist)
                kl_sum = kl_per_dim.sum(-1)
                kl_loss = kl_sum.reshape(BK, args.imag_horizon)[seed_rollout_mask].mean()
        ...
        # ===== Critic (clipped value loss, matching dreamer4) =====
        ...
        # Simple twohot CE value loss — dreamer4's clipping is broken in twohot space
        # (treats twohot as logits → loss ≈ log(255) always, zero gradients)
        value_loss_per_step = -(return_twohot * F.log_softmax(val_logits, dim=-1)).sum(-1).reshape(
            BK, args.imag_horizon
        )
        critic_loss = value_loss_per_step[seed_rollout_mask].mean()
    ...
    print(f"Starting latent_imagination_v27 on {args.env_id} with {args.num_envs} envs")
    ...
    while global_step < args.total_timesteps:
        ...
        if not prefilling and replay.can_sample(args.batch_length):
            ...
            # Hard copy target actor once per env step (like d4's stale old_action_unembeds)
            # KL becomes nonzero in actor epoch 2+ within each train_step
            if not wm_warming:
