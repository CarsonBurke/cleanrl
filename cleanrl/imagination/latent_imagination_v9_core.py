...
#
# v9 removes PPO entirely. The real environment is used only to gather fresh
# context sequences and update the world model. Policy improvement happens in
# imagination on top of a frozen world model:
...
# 5. sample real contexts from replay and improve the same actor/value heads via
#    PMPO sign loss, lambda-return value learning, and an optional frozen prior.
#
# This is not a full Dreamer4 reproduction, but the training structure is much
# closer: one actor, one imagination value head, one persistent prior snapshot,
# and no PPO handoff pathology.
import copy
import os
import random
import time
...
class Args:
    ...
    prior_snapshot_fraction: float = 0.25
    """fraction of total timesteps when the persistent prior is snapshotted"""
...
def prior_snapshot_step(args: Args) -> int:
    return int(args.total_timesteps * args.prior_snapshot_fraction)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if global_step >= args.policy_learning_starts:
            for _ in range(args.imagination_gradient_steps):
                ...
                (current_imagination_coef(args, global_step) * imag_stats["total_loss"]).backward()
