# v17.2 — Bug fixes for dreamerv3/dreamer4 alignment
#
# Fixes from v17 (v16):
...
class Args:
    ...
    # Training (dreamerv3: 1e-4 for all)
    ...
    horizon: int = 333
    lam: float = 0.95
    actor_entropy: float = 0.01  # dreamer4 uses 0.01
    pmpo_pos_neg_weight: float = 0.5  # α in PMPO
    pmpo_kl_weight: float = 0.3  # reverse KL constraint weight
    actor_minstd: float = 0.1
    actor_maxstd: float = 1.0
    value_clip: float = 0.4  # dreamer4 PPO-style value clipping
    slow_target_rate: float = 0.02  # EMA rate for slow critic (used for value targets)
    return_norm_decay: float = 0.99  # dreamerv3 rate=0.01
...
def main(args_class=Args):
    ...
    def train_step():
        ...
        with torch.no_grad():
            ...
            # Dreamer4 PMPO: raw advantages, no return normalization
            # Return norm is only used for EMA tracking, not advantage computation
            return_norm.update(returns)
            advantages = returns - img_slow_val[:, :-1]

            # Discount weight: cumprod of continuation probs (dreamerv3 style)
            weight = torch.cumprod(img_con[:, :-1], dim=1)

            # Pre-compute actor/critic input feats
            imag_actor_feats = img_feats[:, :-1].reshape(BK * args.imag_horizon, -1)
            imag_acts_flat = img_acts.reshape(BK * args.imag_horizon, -1)

            # Store old critic values for value clipping
            old_val_logits = critic(imag_actor_feats)
            old_val_pred = twohot_predict(old_val_logits.detach(), bins).reshape(BK, args.imag_horizon)

        # ===== Actor (PMPO — dreamer4 style: boolean mask + masked_mean) =====
        actor_opt.zero_grad()
        ...
        # masked_mean: mean of logpi where mask is True
        ...
        # Entropy bonus (uniform mean — matches dreamer4)
        entropy_loss = -entropy.float().mean()

        # KL constraint: KL(old || new) — mode-covering direction (dreamer4 convention)
        ...
        # ===== Critic (dreamer4 style: value clipping, no discount weighting) =====
        ...
        # Value clipping (dreamer4 PPO-style)
        old_val_flat = old_val_pred.detach().reshape(-1)
        clipped_val = old_val_flat + (twohot_predict(val_logits, bins).reshape(-1) - old_val_flat).clamp(
            -args.value_clip, args.value_clip)

        # Unclipped loss
        loss_unclipped = twohot_loss(val_logits, returns_flat, bins)
        # Clipped loss
        loss_clipped = twohot_loss(val_logits, clipped_val.detach(), bins)
        # Take max (pessimistic, like PPO)
        ...
        # Update slow critic (still used for value targets in imagination)
        with torch.no_grad():
    ...
    print(f"Starting latent_imagination_v17.2 on {args.env_id} with {args.num_envs} envs")
