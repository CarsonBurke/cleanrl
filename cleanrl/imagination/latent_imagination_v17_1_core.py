...
def main(args_class=Args):
    ...
    def train_step():
        ...
        with torch.no_grad():
            ...
            # Dreamer4: normalize both returns and values, then subtract
            advantages = return_norm.normalize(returns) - return_norm.normalize(img_slow_val[:, :-1])

            # Discount weight: cumprod(disc*con) / disc (dreamerv3 style)
            # img_con already includes disc since continue target = (~term)*disc
            # Dividing by cont_target_val removes discount from first step (current state)
            weight = torch.cumprod(img_con[:, :-1], dim=1) / cont_target_val
        ...
        # PMPO: separate positive and negative advantages, discount-weighted
        adv_detached = advantages.detach()
        w = weight.detach()
        pos_mask = (adv_detached >= 0).float()
        neg_mask = (adv_detached < 0).float()

        alpha = args.pmpo_pos_neg_weight
        pos_w = w * pos_mask
        neg_w = w * neg_mask
        pos_term = (logpi * pos_w).sum() / pos_w.sum().clamp(min=1)
        neg_term = -(logpi * neg_w).sum() / neg_w.sum().clamp(min=1)
        ...
        if args.pmpo_kl_weight > 0:
            ...
            # KL(old || new) = sum_d [log(sig_new/sig_old) + (sig_old^2 + (old_mu-new_mu)^2)/(2*sig_new^2) - 0.5]
        ...
        # ===== Critic (dreamerv3 style: discount-weighted + slow-critic regularization) =====
        ...
        value_loss = (w_critic * loss_returns).mean()

        # Slow-critic regularization (dreamerv3): pull critic toward slow target
        slow_val_target = slow_val_for_critic.detach().reshape(-1)
        slow_reg_loss = twohot_loss(
            val_logits, slow_val_target, bins
        ).reshape(BK, args.imag_horizon)
        slow_reg_term = (w_critic * slow_reg_loss).mean()

        critic_loss = value_loss + args.slow_reg * slow_reg_term
    ...
    print(f"Starting latent_imagination_v17.1 on {args.env_id} with {args.num_envs} envs")
