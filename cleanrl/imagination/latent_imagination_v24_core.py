...
#
# v24 ports the main remaining Dreamer4-style transformer ideas that fit this
# codebase: softclamped grouped attention, SwiGLU feedforwards, value-residual
# mixing, learned reward embeddings into the agent token, register tokens, and
# loss normalization, while keeping v23's batched observe and cached online
# path.
import math
import os
import random
...
class TransformerWorldModel(nn.Module):
    ...
    def _embed_action(self, action):
        ...
        dim_ids = torch.arange(self.act_dim, device=action.device)
        dim_emb = self.action_dim_emb(dim_ids).unsqueeze(0).expand(batch, -1, -1)
...
def main(args_class=Args):
    ...
    def build_aux_targets(values, valid_mask, is_first_mask, steps):
        ...
        for horizon in range(steps):
            ...
            for t in range(base_len):
                target_t = t + offset
                if target_t >= values.shape[1]:
                    break
                same_episode = valid_mask[:, t] & valid_mask[:, target_t]
                if target_t > t:
                    for k in range(t + 1, target_t + 1):
                        same_episode = same_episode & ~is_first_mask[:, k]
                target_mask[:, t, horizon] = same_episode
                targets[:, t, horizon] = values[:, target_t]
    ...
    def train_step():
        ...
        if reward_aux_head is not None:
            reward_aux_targets, reward_aux_mask = build_aux_targets(rew, valid, is_first, args.aux_reward_steps)
        ...
        if action_aux_head is not None:
            action_aux_targets, action_aux_mask = build_aux_targets(prev_act, valid, is_first, args.aux_action_steps)
    ...
    print(f"Starting latent_imagination_v24 on {args.env_id} with {args.num_envs} envs")
