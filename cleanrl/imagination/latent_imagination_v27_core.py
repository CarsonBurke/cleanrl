...
#
# v27: Bug fixes and perf improvements over v26, aligned with dreamer4 reference.
# Key changes from v26:
# - Remove continue head from imagination GAE (dreamer4 uses fixed gamma=0.997)
# - Add PPO-style value clipping (max of unclipped/clipped CE, clip=0.4)
# - Fix critic clipped value loss using twohot_encode on clipped scalar values
# - Actually vectorize seed construction (v26 had Python for-loops despite claim)
# - Vectorize KV cache append (eliminate per-batch Python loop)
# - PMPO + reverse KL constraint aligned with dreamer4
import math
import os
import random
import time
...
class Args:
    ...
    wm_only_steps: int = 100000  # WM-only training before imagination begins
...
def main(args_class=Args):
    ...
    def train_step(wm_only=False):
        ...
        # PMPO: split by advantage sign, masked_mean of log_probs per partition
        # Exactly matching dreamer4 lines 2821-2835
        pos_mask = (adv_detached >= 0) & seed_rollout_mask  # (BK, H)
        neg_mask = (adv_detached < 0) & seed_rollout_mask   # (BK, H)
        # Broadcast to (BK, H, act_dim) like dreamer4: advantage = rearrange(advantage, '... -> ... 1')
        ...
        # masked_mean matching dreamer4's masked_mean: t[mask].mean()
        ...
        # KL constraint: KL(target_actor || current_actor) — reverse KL
        # Uses target_actor (previous step's params) so KL is nonzero,
        # matching dreamer4 where generate() uses previous policy
        ...
        if args.pmpo_kl_weight > 0:
            ...
            # reverse KL: KL(old || new) penalizes new policy collapse
    ...
    while global_step < args.total_timesteps:
        ...
        if not prefilling and replay.can_sample(args.batch_length):
            ...
            # Update target actor once per env step (not per train step)
            # so KL(target || current) is nonzero within the training cycle
            if not wm_warming:
