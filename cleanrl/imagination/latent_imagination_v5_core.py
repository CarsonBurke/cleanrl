# Shared implementation for latent-imagination v5 and later forks from it.
...
class Args:
    ...
    use_multi_horizon_actor_stats: bool = False
    """estimate imagined actor targets from multiple horizons instead of a single rollout horizon"""
    ...
    """use only the sign of the imagined correction for actor updates"""
    model_coef: float = 0.25
    """overall weight on the latent-model loss"""
    model_coef_after_imag: float = 0.25
    """overall model-loss weight once imagination starts contributing to actor updates"""
    transition_coef: float = 1.0
    """weight on the stochastic latent transition loss"""
    use_multi_horizon_model_loss: bool = False
    """train the world model with direct multi-horizon supervision instead of only the one-step objective"""
    multi_horizon_steps: tuple[int, ...] = (1, 2, 4, 8)
    """horizons used for direct multi-step world-model supervision and optional actor statistics"""
    multi_horizon_coef: float = 1.0
    """weight on the direct multi-horizon supervision loss"""
...
def current_model_coef(args: Args, global_step: int) -> float:
    return args.model_coef_after_imag if current_imagination_coef(args, global_step) > 0.0 else args.model_coef
...
def build_multi_horizon_targets(
    ...
    action_shape = env_actions.shape[2:]
    obs_shape = next_obses.shape[2:]

    action_sequences = torch.zeros((max_horizon, num_steps, num_envs) + action_shape, device=rewards.device)
    for offset in range(max_horizon):
        if offset >= num_steps:
            break
        action_sequences[offset, : num_steps - offset] = env_actions[offset:]

    reward_targets = torch.zeros((num_horizons, num_steps, num_envs), device=rewards.device)
    continuation_targets = torch.zeros((num_horizons, num_steps, num_envs), device=rewards.device)
    next_obs_targets = torch.zeros((num_horizons, num_steps, num_envs) + obs_shape, device=rewards.device)
    valid_masks = torch.zeros((num_horizons, num_steps, num_envs), device=rewards.device)

    for horizon_idx, horizon in enumerate(horizons):
        for step in range(0, num_steps - horizon + 1):
            discounted_return = torch.zeros(num_envs, device=rewards.device)
            continuation = torch.ones(num_envs, device=rewards.device)
            valid = torch.ones(num_envs, device=rewards.device)
            discount = 1.0

            for offset in range(horizon):
                discounted_return = discounted_return + discount * rewards[step + offset]
                continuation = continuation * (1.0 - next_dones[step + offset])
                if offset < horizon - 1:
                    valid = valid * (1.0 - next_dones[step + offset])
                discount *= gamma

            reward_targets[horizon_idx, step] = discounted_return
            continuation_targets[horizon_idx, step] = continuation
            next_obs_targets[horizon_idx, step] = next_obses[step + horizon - 1]
            valid_masks[horizon_idx, step] = valid

    return action_sequences, reward_targets, continuation_targets, next_obs_targets, valid_masks
...
class Agent(nn.Module):
    ...
    def imagine_multihorizon_returns(
        self,
        obs,
        first_env_action,
        horizons: tuple[int, ...],
        ...
        batch_size = latent.shape[0]
        max_horizon = max(horizons)
        horizon_to_index = {horizon: index for index, horizon in enumerate(horizons)}

        latent = latent.unsqueeze(1).expand(-1, branches, -1).reshape(batch_size * branches, self.latent_dim)
        env_action = first_env_action.unsqueeze(1).expand(-1, branches, -1).reshape(batch_size * branches, -1)

        discounted_returns = torch.zeros(batch_size * branches, device=latent.device)
        discount = torch.ones(batch_size * branches, device=latent.device)
        continuation = torch.ones(batch_size * branches, device=latent.device)
        horizon_returns = torch.zeros(len(horizons), batch_size * branches, device=latent.device)
        ...
        for step in range(max_horizon):
            ...
            discounted_returns = discounted_returns + discount * continuation * reward_pred
            continuation = continuation * (1.0 - done_prob)
            discount = discount * gamma
            latent = next_latent

            horizon = step + 1
            if horizon in horizon_to_index:
                bootstrap = self.get_value_from_latent(latent).squeeze(-1)
                horizon_returns[horizon_to_index[horizon]] = discounted_returns + discount * continuation * bootstrap

            if step < max_horizon - 1:
                policy_action = self.get_dist_from_latent(latent).sample()
                env_action = self.clamp_action(policy_action)

        return horizon_returns.view(len(horizons), batch_size, branches)
...
def main(args_class=Args):
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        b_multi_action_sequences = multi_action_sequences.reshape(
            multi_action_sequences.shape[0],
            args.batch_size,
            *envs.single_action_space.shape,
        )
        b_multi_reward_targets = multi_reward_targets.reshape(len(args.multi_horizon_steps), args.batch_size)
        b_multi_continuation_targets = multi_continuation_targets.reshape(len(args.multi_horizon_steps), args.batch_size)
        b_multi_next_obs_targets = multi_next_obs_targets.reshape(
            (len(args.multi_horizon_steps), args.batch_size) + envs.single_observation_space.shape
        )
        b_multi_valid_masks = multi_valid_masks.reshape(len(args.multi_horizon_steps), args.batch_size)

        with torch.no_grad():
            imag_coef = current_imagination_coef(args, global_step)
            if imag_coef > 0.0:
                if args.use_multi_horizon_actor_stats:
                    b_horizon_returns = agent.imagine_multihorizon_returns(
                        b_obs,
                        b_env_actions,
                    ...
                    b_horizon_returns_mean = b_horizon_returns.mean(dim=-1)
                    b_imag_returns = b_horizon_returns_mean.mean(dim=0)
                    b_imag_stds = b_horizon_returns_mean.std(dim=0, unbiased=False)
                else:
                ...
                if args.sign_only_imag_actor:
                    b_imag_actor_signal = torch.sign(b_imag_advantages_capped)
                else:
                    b_imag_actor_signal = b_imag_advantages_capped

                if args.use_imag_conf_gate:
                    b_policy_advantages = b_advantages + imag_coef * b_total_gate * b_imag_actor_signal
                else:
                    b_policy_advantages = b_advantages + imag_coef * b_imag_actor_signal
            else:
                ...
                b_imag_actor_signal = torch.zeros_like(b_values)
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.use_multi_horizon_model_loss:
                    ...
                    for horizon_index, horizon in enumerate(args.multi_horizon_steps):
                        valid_mask = b_multi_valid_masks[horizon_index, mb_inds] > 0.5
                        ...
                        first_step_std = None
                        for offset in range(horizon):
                            horizon_actions = b_multi_action_sequences[offset, mb_inds][valid_mask]
                        ...
                        target_next_latent = agent.encode_target(b_multi_next_obs_targets[horizon_index, mb_inds][valid_mask])
                        target_reward = b_multi_reward_targets[horizon_index, mb_inds][valid_mask]
                        target_continuation = b_multi_continuation_targets[horizon_index, mb_inds][valid_mask]

                        if horizon == 1 and first_step_mean is not None and first_step_std is not None:
                            transition_loss_terms.append(gaussian_nll(target_next_latent, first_step_mean, first_step_std).mean())
                        else:
                    ...
                    transition_loss = (
                        torch.stack(transition_loss_terms).mean()
                        if transition_loss_terms
                        else torch.zeros((), device=device)
                    )
                    reward_loss = (
                        torch.stack(reward_loss_terms).mean()
                        if reward_loss_terms
                        else torch.zeros((), device=device)
                    )
                    done_loss = (
                        torch.stack(done_loss_terms).mean()
                        if done_loss_terms
                        else torch.zeros((), device=device)
                    )
                    value_consistency_loss = (
                        torch.stack(value_consistency_terms).mean()
                        if value_consistency_terms
                        else torch.zeros((), device=device)
                    )
                    model_loss = args.multi_horizon_coef * (
                ...
                model_coef_now = current_model_coef(args, global_step)
        ...
        writer.add_scalar("losses/model_coef", model_coef_now, global_step)
        ...
        writer.add_scalar("imagination/imagined_actor_signal", b_imag_actor_signal.mean().item(), global_step)
