...
#
# v15 aligns more closely with the Dreamer4 phase split:
# 1. world model and action/reward MTP are trained before imagination,
# 2. relevant-sequence sampling is used only for the behavior phase,
# 3. imagination starts later on uniform replay contexts,
# 4. the standalone replay-actor BC term is removed from imagination.
import copy
import os
import random
import time
...
def current_behavior_clone_coef(args: Args, global_step: int) -> float:
    return args.behavior_clone_coef if args.learning_starts <= global_step < args.policy_learning_starts else 0.0
...
def compute_behavior_clone_loss(
    ...
    if agent.multi_token_pred_len <= 1:
        return current_loss, relevant_mask.float().mean()
    ...
    return current_loss + future_loss, relevant_mask.float().mean()
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if global_step >= args.learning_starts:
            for _ in range(args.imagination_gradient_steps):
                if global_step < args.policy_learning_starts:
                    ...
                    bc_loss, behavior_relevance = compute_behavior_clone_loss(
                        agent,
                        start_obs,
                        start_context,
                        action_target_batch,
                    ...
                    imag_stats = {
                        ...
                        "value_loss": torch.zeros((), device=device),
                        "prior_kl_loss": torch.zeros((), device=device),
                        "behavior_clone_loss": bc_loss.detach(),
                        "behavior_relevance": behavior_relevance.detach(),
                        "mean_return": torch.zeros((), device=device),
                        "mean_advantage": torch.zeros((), device=device),
                        "positive_fraction": torch.zeros((), device=device),
                    }
                    total_imag_loss = current_behavior_clone_coef(args, global_step) * bc_loss
                else:
                    ...
                    imag_stats["behavior_clone_loss"] = torch.zeros((), device=device)
                    imag_stats["behavior_relevance"] = torch.zeros((), device=device)
                    total_imag_loss = imag_stats["total_loss"] * current_imagination_coef(args, global_step)
