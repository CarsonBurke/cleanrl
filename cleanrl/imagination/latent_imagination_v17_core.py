...
def main(args_class=Args):
    ...
    def train_step():
        ...
        # ===== Critic (clipped value loss, dreamer4 style) =====
        ...
        loss_unclipped = twohot_loss(
            val_logits, returns_flat, bins
        ).reshape(BK, args.imag_horizon)

        # Clipped value loss: clip scalar value toward old (slow) value,
        # then compute loss against returns using the clipped scalar as target.
        # Gradients still flow through val_logits since we take max with unclipped.
