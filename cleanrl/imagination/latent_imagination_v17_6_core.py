# v17.6 — Bug fixes for dreamerv3/dreamer4 alignment
#
# Fixes from v17 (v16):
...
class Args:
    ...
    # Imagination (d4: horizon=8)
    imag_horizon: int = 8

    # Actor-Critic (d4 hyperparams)
    horizon: int = 100  # d4 gamma=0.99 → horizon=1/(1-0.99)=100
    lam: float = 0.95
    actor_entropy: float = 0.0  # d4: no entropy in actor loss
    pmpo_pos_neg_weight: float = 0.5  # d4: imagination_alpha=0.5
    pmpo_kl_weight: float = 0.3  # d4: imagination_prior_coef=0.3
    value_coef: float = 0.5  # d4: imagination_value_coef=0.5
    actor_minstd: float = 0.05  # d4: model_min_std=0.05
...
def main(args_class=Args):
    ...
    def train_step():
        ...
        with torch.no_grad():
            ...
            # d4: raw advantages, no return normalization
            advantages = returns - img_slow_val[:, :-1]
        ...
        # ===== Actor (PMPO — dreamer4 style: discount-weighted masked means) =====
        actor_opt.zero_grad()

        policy_dist = actor(imag_actor_feats.detach())
        logpi = policy_dist.log_prob(imag_acts_flat.detach()).sum(-1)
        entropy = policy_dist.entropy().sum(-1)

        # Flatten everything for indexed masking (dreamer4 convention)
        flat_logpi = logpi
        flat_w = weight.detach().reshape(-1)
        ...
        # Discount-weighted masked means (dreamer4: alive_weights[mask] * logprobs[mask])
        if pos_mask.any():
            pos_w = flat_w[pos_mask]
            pos_loss = -(pos_w * flat_logpi[pos_mask]).sum() / pos_w.sum().clamp(min=1e-6)
        else:
            pos_loss = torch.zeros(1, device=device)

        if neg_mask.any():
            neg_w = flat_w[neg_mask]
            neg_loss = (neg_w * flat_logpi[neg_mask]).sum() / neg_w.sum().clamp(min=1e-6)
        else:
            neg_loss = torch.zeros(1, device=device)

        pmpo_loss = alpha * pos_loss + (1 - alpha) * neg_loss

        # KL to prior (dreamer4): KL(detached_old || new) — regularize toward rollout-time policy
        ...
        # KL(old || new) per-dim, summed over action dims
        kl_div = (torch.log(new_sig / old_sig) +
                  (old_sig**2 + (old_mu - new_mu)**2) / (2 * new_sig**2) - 0.5).sum(-1)
        # Discount-weighted KL
        kl_loss = (flat_w * kl_div).sum() / flat_w.sum().clamp(min=1e-6)

        # d4 total: policy_loss + prior_coef * kl + entropy (0 by default)
        entropy_loss = -entropy.mean()
        actor_loss = pmpo_loss + args.pmpo_kl_weight * kl_loss + args.actor_entropy * entropy_loss
        ...
        critic_loss = args.value_coef * (w_critic * (loss_returns + args.slow_reg * loss_slow)).mean()
        ...
        return {
            ...
            'ret_scale': advantages.abs().mean().item(),
        }
    ...
    print(f"Starting latent_imagination_v17.6 on {args.env_id} with {args.num_envs} envs")
