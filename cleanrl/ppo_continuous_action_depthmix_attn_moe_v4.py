# PPO + DepthMix attn-MoE v4: depth-attention trunk + soft-MoE FFN.
#
# WHY v4. The stabilized depth-attention (v2) trains cleanly but, run to 8M, only
# ~matches a plain-MLP residual trunk (9248 vs 8880 on HalfCheetah — within
# single-seed noise) and sits BELOW the v24 MoE baseline (~9500-9750). Across the
# whole depthmix line the lone clear winner is v24's *pointwise input-dependent
# capacity* (soft local experts), not richer depth readout (multi-slot REGRESSED).
#
# CHANGE (single variable from v2). Keep v2's pre-LN add-form depth-attention
# EXACTLY. Replace ONLY the per-block FFN sublayer's single fixed MLP body with a
# v24-style SOFT mixture of experts (softmax gate over E experts, NO top-k), sized
# so the experts together match v2's body params (E * 2*H*de == 2*mult*H^2, i.e.
# E*de == mult*H). The depth-attention still routes over PREVIOUS LAYERS (depth);
# the MoE now routes over EXPERTS (pointwise). Both mixes are input-dependent.
#
# HYPOTHESIS. Pointwise conditioning is the missing capacity. If depth-attention +
# param-matched soft-MoE FFN beats v24, input-dependent depth mixing adds value ON
# TOP of the MoE that already wins; if it only matches v24, the attention is dead
# weight over v24's static DenseNet reach-back and the MoE alone is the story.
#
# ---- v2 docstring (depth-attention mechanism, UNCHANGED) ----
# depthmix_v1's "attn" mode collapsed (exploding grads / KL). Two structural
...
@dataclass
class Args:
    exp_name: str = "depthmix_attn_moe_v4"
    seed: int = 1
    ...
    trunk_mlp_mult: int = 2          # FFN body width budget (m*H); MoE matches its params
    qk_norm: bool = False            # attn: L2-normalize q/K per head + learnable logit scale
    # Soft-MoE FFN (attn mode only). E experts each Linear(H,de)->ReLU^2->Linear(de,H),
    # softmax gate over experts (no top-k). Param-matched to the v2 body when
    # trunk_experts * trunk_expert_dim == trunk_mlp_mult * trunk_hidden (default 8*32==2*128).
    trunk_experts: int = 8           # E soft experts per block FFN (1 == v2 plain body)
    trunk_expert_dim: int = 32       # de: per-expert hidden width
...
def _expert_body(H, de):
    # One soft-MoE expert: Linear(H, de) -> ReLU^2 -> Linear(de, H). Same activation
    # and sqrt(2) init as v24's expert body; de is the per-expert hidden width.
    return nn.Sequential(
        layer_init(nn.Linear(H, de)),
        ReLUSquared(),
        layer_init(nn.Linear(de, H)),
    )
...
class DepthMixTrunk(nn.Module):
    ...
    attn (v2) is the standard pre-LN transformer-decoder-over-depth form: ONE
    ...
    def __init__(self, in_dim, H, n_blocks, reach_mode, n_heads, d_qk, mlp_mult,
                 qk_norm=False, n_experts=8, expert_dim=32):
        super().__init__()
        ...
        if reach_mode == "attn":
            # v4: FFN sublayer is a soft MoE (softmax gate over E experts, no top-k),
            # param-matched to v2's single body (E*de == mlp_mult*H). E==1 recovers v2.
            self.gate = nn.ModuleList(
                [layer_init(nn.Linear(H, n_experts), std=0.01) for _ in range(n_blocks)]
            )
            self.experts = nn.ModuleList([
                nn.ModuleList([_expert_body(H, expert_dim) for _ in range(n_experts)])
                for _ in range(n_blocks)
            ])
        else:
            # seq / dwa keep v2's single fixed MLP body per block.
            self.bodies = nn.ModuleList(
                [_mlp_body(H, mlp_mult) for _ in range(n_blocks)]
            )
        ...
        if reach_mode == "attn":
            ...
            self.Wo = nn.ModuleList(
                [layer_init(nn.Linear(H, H)) for _ in range(n_blocks)]
            )
            if qk_norm:
    ...
    def _attn_forward(self, x):
        # Stable pre-LN ADD-form depth-attention trunk. `s` is the persistent
        # residual stream; `bank` snapshots its state after each block. LayerNorm
        # is applied ONLY to branch inputs (q_in / kv / ffn), so `s` flows through
        # an exact identity skip (no throttle) and every branch output is bounded.
        nh, dqk = self.n_heads, self.d_qk
        s = self.entry(x)                                          # (B, H)
        B, H = s.shape
        dv = H // nh
        ...
        for i in range(self.n_blocks):
            L = len(bank)
            # --- sublayer 1: depth-attention (pre-LN, residual ADD) ---
            q_in = self.ln_q[i](s)                                 # (B, H)
            kv = self.ln_kv[i](torch.stack(bank, dim=1))           # (B, L, H)
            q = self.Wq[i](q_in).view(B, nh, dqk)                  # (B, nh, dqk)
            K = self.Wk[i](kv).view(B, L, nh, dqk)                 # (B, L, nh, dqk)
            V = self.Wv[i](kv).view(B, L, nh, dv)                  # (B, L, nh, dv)
            if self.qk_norm:
                q = F.normalize(q, dim=-1)
                K = F.normalize(K, dim=-1)
                scale = self.logit_scale[i].exp().view(1, nh, 1)   # (1, nh, 1)
                scores = torch.einsum("bhd,blhd->bhl", q, K) * scale  # (B, nh, L)
            else:
                scores = torch.einsum("bhd,blhd->bhl", q, K) / (dqk ** 0.5)
            w = torch.softmax(scores, dim=-1)                      # over depth L; (B, nh, L)
            ctx = torch.einsum("bhl,blhd->bhd", w, V).reshape(B, H)  # (B, H)
            s = s + self.Wo[i](ctx)                                # residual ADD (identity preserved)
            # --- sublayer 2: soft-MoE FFN (pre-LN, residual ADD) ---
            f_in = self.ln_ffn[i](s)                               # (B, H)
            gw = torch.softmax(self.gate[i](f_in), dim=-1)         # (B, E); over experts
            e_out = torch.stack([e(f_in) for e in self.experts[i]], dim=1)  # (B, E, H)
            ffn = (gw.unsqueeze(-1) * e_out).sum(dim=1)            # (B, H)
            s = s + ffn                                            # residual ADD
            bank.append(s)
