# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return idx-transfer + TPO-MD v4 PROBE-CRITIC.
#
# DELTA vs tpomd_v1: the K probe transitions per state — currently used ONLY to
# score candidates and then DISCARDED — are recycled as EXTRA critic supervision.
# tpomd_v1 already runs each candidate one tick through the RAW MuJoCo physics,
# producing real (r_k, s'_k, term_k) and (for scoring) the FROZEN pre-update value
# of every probe next-state, V_old(s'_k). We add an auxiliary critic loss over
# these probe NEXT-STATES.
#
# FORMULATION (anchoring / self-distillation — chosen because it is the only
# NON-BIASING use of these probes for a V-only critic). The probe gives K
# off-policy candidate targets y_k = r_k + gamma*(1-term_k)*V_old(s'_k) for the
# SAME rollout state s. Regressing V(s) on the mean of those K targets would bias
# V(s) toward the MEAN-candidate value, not the on-policy value the main loss
# already learns from the executed transition (candidate 0). A pure V-critic
# cannot consume off-policy candidate TD targets without that bias (it needs a
# Q-function). So instead we treat each probe NEXT-STATE s'_k as an ordinary extra
# state and train the CURRENT critic to reproduce the FROZEN critic's value there:
# project the stop-gradient scalar V_old(s'_k) — the same v_next scoring already
# computed — through the SAME symlog HL-Gauss projector used for the main target,
# and add CE(project(V_old(s'_k)), log_softmax(V(s'_k))) to the value loss. This
# carries NO fresh reward signal (target = frozen net's own value), so it cannot
# bias V away from the on-policy value; it only anchors the net + extends state
# coverage to off-policy candidate next-states (a stability/regularization term,
# zero at the start of each update since the net == V_old there). Weight =
# tpo_probe_vf_coef; the aux loss enters ONLY the critic backward (critic_params +
# shared trunk, under critic_grad_clip), NEVER the actor branch. At
# tpo_probe_vf_coef=0.0 (guarded by an `if`) the file is bit-identical to tpomd_v1.
#
# --- inherited tpomd_v1 notes ---
...
# pushing full-strength updates on noise-level groups. The trust region lives in
# the TARGET, MPO-style: each update an adaptive temperature eta is solved
# (vectorized bisection on log-eta over all groups at once) so that batch-mean
# KL(p_old || q) = tpo_eps, where q = softmax(K-restricted log p_old + u/eta);
# full CE convergence to q is then an eps-sized step by construction. q is solved
# once, detached, frozen across epochs. Actor loss = tpo_coef * CE(q, softmax of
# new K-restricted logprobs) - ent_coef * entropy.
#
...
#
# KILL CRITERIA: debug/tpo_kl_achieved persistently far from tpo_eps;
# debug/actor_epochs_completed pinned at 1; entropy freefall vs the base run.
#
...
class Args:
    ...
    tpo_eps: float = 0.03        # target batch-mean KL(p_old || q): the per-update trust region
    ...
    # PROBE-CRITIC (v4): recycle the K discarded probe transitions as EXTRA critic
    # supervision. Auxiliary value loss = CE(project(stopgrad V_old(s'_k)),
    # log_softmax(V(s'_k))) over the probe NEXT-STATES s'_k, weighted by this coef
    # relative to the main value loss; added ONLY to the critic backward. This is a
    # NON-BIASING anchoring/self-distillation term (target is the frozen net's own
    # value, no fresh reward), extending state coverage. 0.0 => bit-identical to v1.
    tpo_probe_vf_coef: float = 0.5
...
if __name__ == "__main__":
    ...
    assert args.tpo_probe_vf_coef >= 0.0, "tpo_probe_vf_coef must be non-negative"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # PROBE-CRITIC aux target: project the FROZEN-net scalar value of every
            # probe next-state V_old(s'_k) (the SAME v_next scoring uses) through the
            # SAME symlog HL-Gauss projector as the main return target. This is a
            # stop-gradient anchor (v_next was computed under no_grad / pre-update);
            # carries no fresh reward, so it cannot bias V off the on-policy value.
            # Stored (B, K, num_bins) for per-minibatch indexing. coef=0 => skipped
            # entirely (no extra work, no graph/RNG perturbation).
            if args.tpo_probe_vf_coef > 0.0:
                b_probe_target_probs = hl_support.project(v_next.reshape(-1)).reshape(
                    args.batch_size, args.tpo_k, args.num_bins
                )
                # Probe next-states grouped per rollout state for minibatch indexing.
                b_probe_next_obs = flat_probe_obs.reshape(args.batch_size, args.tpo_k, obs_dim)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Critic backward scalar. PROBE-CRITIC adds an auxiliary CE over the
                # K probe next-states s'_k of this minibatch: a SEPARATE critic
                # forward (gradient flows through critic_params + the shared trunk
                # ONLY — never the actor) toward the frozen-net anchor target
                # b_probe_target_probs. Folded into the critic loss so it rides every
                # critic backward (dual / breaker / global) and the critic grad clip.
                # Guarded by coef>0 so coef=0 is bit-identical to v1 (no extra
                # forward, no graph/RNG perturbation, scalar == args.vf_coef * v_loss).
                critic_loss = args.vf_coef * v_loss
                if args.tpo_probe_vf_coef > 0.0:
                    probe_obs_mb = b_probe_next_obs[mb_inds].reshape(-1, obs_dim)
                    probe_value_logits = agent.get_value(probe_obs_mb)
                    probe_value_log_probs = torch.log_softmax(probe_value_logits, dim=-1)
                    probe_targets_mb = b_probe_target_probs[mb_inds].reshape(-1, args.num_bins)
                    probe_v_loss = -(probe_targets_mb * probe_value_log_probs).sum(dim=-1).mean()
                    critic_loss = critic_loss + args.tpo_probe_vf_coef * probe_v_loss
                ...
                if args.separate_grad_clip:
                    ...
                    # critic_loss == args.vf_coef * v_loss (+ probe aux when coef>0).
                    # The aux term depends only on agent.get_value, so its gradient
                    # flows through critic_params + the shared trunk, never the actor
                    # heads — "critic-only" holds even in this combined backward.
                    loss = critic_loss
                    if actor_active:
        ...
        if args.tpo_probe_vf_coef > 0.0:
            writer.add_scalar("losses/probe_value_loss", probe_v_loss.item(), global_step)
