# MAPPO-style per-action-dimension critic on the indclip v4 baseline.
#
# MECHANISM: action dimensions play the role of MAPPO's agents. The critic keeps
# the shared trunk and widens its head to Linear(64, D) -- MAPPO's *shared* mode,
# where agent identity enters only at the last layer. GAE runs independently per
# head over (T, N*D), with reward/termination/truncation broadcast identically to
# all heads (MAPPO's shared team reward). Beta log-probs stay per-dimension (B, D),
# so the surrogate uses per-dimension ratios, per-dimension clipping and per-head
# advantages, reduced as sum-over-D then mean-over-batch. The value loss is MAPPO's
# clipped max(0.5*e_orig^2, 0.5*e_clip^2) meaned over both axes.
#
# NOVELTY: PPO's independent-clipping idea pushed down to the action dimension.
# The sum over D is the gradient-scale-MATCHED choice, not a D-times amplification:
# the baseline's joint log-prob is already the sum over dims, so d(pg_loss)/d(logprob_d)
# is -A*ratio in both. The actual change is (i) per-dimension instead of joint
# clipping and (ii) per-head advantages. No 1/D rescale is applied. The reported
# losses/policy_loss does read ~D x the baseline's; the gradient does not.
#
# HYPOTHESIS: per-dimension clipping is strictly tighter than joint clipping (one
# dimension's large ratio can no longer be masked by the others), which should cut
# destructive updates at 32x LR; the D heads only differ through their own bootstrap
# terms, so head_value_spread reports whether they stay degenerate copies.
#
# NOT PORTED (no single-agent analogue): active_masks, use_centralized_V, agent-id
# one-hots, PopArt, Huber loss, advantage standardization (norm_adv stays False).
# --value-heads scalar restores the baseline exactly.
#
# Pre-RMS SiTU-GLU PPO, v4: restored PPO scalar value-loss clipping,
# and independent actor/critic gradient clipping after the weighted backward.
# Joint fused Adam, network initialization and schedules stay unchanged.
# Reward normalization is optional; no additional return-target norm or critic EMA.
import os
import random
import time
...
class Args:
    ...
    value_heads: Literal["scalar", "per_action"] = "per_action"
    """one critic head per action dimension (MAPPO agents), or the baseline scalar head"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, placement="pre", norm_kind="rms", activation="stiglu", value_heads="per_action"):
        super().__init__()
        ...
        if value_heads not in {"scalar", "per_action"}:
            raise ValueError("value_heads must be 'scalar' or 'per_action'")
        # MAPPO shared mode: one trunk, agent identity injected only at the last
        # layer. With per_action heads, head d is "agent" d = action dimension d.
        self.per_action_heads = value_heads == "per_action"
        self.value_heads = self.action_dim if self.per_action_heads else 1
        self.critic = nn.Sequential(
            make_norm_residual_trunk(
                observation_dim, 64, placement=placement, norm_kind=norm_kind, activation=activation
            ),
            layer_init(nn.Linear(64, self.value_heads), std=1.0),
        )
    ...
    def reduce_logprob(self, terms):
        """``(B, D)`` per-dimension log-probabilities, or the joint ``(B, 1)``."""
        return terms if self.per_action_heads else terms.sum(-1, keepdim=True)

    def action_logprob(self, alpha, beta, native_action):
        distribution = Beta(alpha, beta, validate_args=False)
        return self.reduce_logprob(distribution.log_prob(native_action) - self.log_action_scale)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        distribution = Beta(alpha, beta, validate_args=False)
        logprob = self.reduce_logprob(distribution.log_prob(native) - self.log_action_scale)
...
def broadcast_heads(signal, heads):
    """``(T, N)`` rollout signal broadcast to ``(T, N*H)``.

    MAPPO hands every agent the same team reward and the same episode mask; the
    heads differ only through their own bootstrap terms. ``heads == 1`` returns
    the input unchanged, keeping the scalar arm on the baseline's exact tensors.
    """
    if heads == 1:
        return signal
    steps, envs = signal.shape
    return signal.unsqueeze(-1).expand(steps, envs, heads).reshape(steps, envs * heads).contiguous()


def resolve_head_bootstraps(bootstraps, value_fn, device, heads):
    """``(T, N*H)`` truncation bootstrap values for the multi-head critic.

    ``TruncationBootstrapCache.resolve`` contracts for a scalar critic (``value_fn``
    must return exactly one value per observation), so each head is scattered by
    its own projected call. Truncations are rare and the batch is tiny, so H
    passes over it are far cheaper than duplicating the cache's scatter logic.
    """
    if heads == 1:
        return bootstraps.resolve(value_fn, device)
    columns = [bootstraps.resolve(lambda obs, head=head: value_fn(obs)[:, head], device) for head in range(heads)]
    return torch.stack(columns, dim=-1).flatten(1, 2)


def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, targets, old_values, args):
    """Per-head clipped surrogate and optionally clipped MSE over ``(B, H)`` heads.

    Every tensor carries a trailing head axis: ``H == action_dim`` for the MAPPO
    arm, ``H == 1`` for the scalar arm, where each reduction degenerates to the
    baseline's. The surrogate sums over heads and means over the batch (MAPPO's
    ``sum(dim=-1).mean()``); that sum is gradient-scale matched to the baseline,
    whose joint log-ratio is itself the sum of the per-dimension terms.
    """
    alpha, beta, newvalue = agent.get_policy_and_value(observations)
    distribution = Beta(alpha, beta, validate_args=False)
    newlogprob = agent.reduce_logprob(distribution.log_prob(native_actions) - agent.log_action_scale)
    entropy = (distribution.entropy() + agent.log_action_scale).sum(-1)
    logratio = newlogprob - old_logprobs
    ratio = logratio.exp()
    with torch.no_grad():
        # Diagnostics stay on the JOINT log-ratio so approx_kl/clipfrac remain
        # comparable to the baseline dashboard; only the surrogate is per-head.
        ...
        clipfrac = ((joint_ratio - 1.0).abs() > args.clip_coef).float().mean()
        # Cross-head dispersion of the D value predictions: zero for H == 1, and
        # the direct measure of whether the MAPPO heads stay degenerate copies.
        head_value_spread = newvalue.std(dim=-1, correction=0).mean()
    if args.norm_adv:
...
def main():
    ...
    try:
        ...
        writer.add_text(
            ...
            f"{args.value_heads} critic head(s)",
        )
        ...
        heads = agent.value_heads

        def rollout_statistics(observations, native):
            """Old per-head values ``(B, H)`` and per-dimension log-probabilities."""
            alpha, beta, value = agent.get_policy_and_value(observations)
            return value, agent.action_logprob(alpha, beta, native)
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                # (B, H) -> (T, N*H): head h of env n occupies column n*H + h, so
                # the Triton (T, N) recurrence runs one independent GAE per head.
                values = b_values.view(args.num_steps, args.num_envs * heads)
                next_obs = transfer.observation(next_obs_np)
                tail_value = value_model(next_obs).flatten()
                truncation_values = resolve_head_bootstraps(bootstraps, value_model, device, heads)
                advantages, returns = gae_fn(
                    broadcast_heads(batch.rewards, heads),
                    values,
                    broadcast_heads(batch.terminations, heads),
                    broadcast_heads(batch.truncations, heads),
                    truncation_values,
                    tail_value,
                    args.gamma,
                    args.gae_lambda,
                )
                b_advantages = advantages.reshape(args.batch_size, heads).clone()
                b_returns = returns.reshape(args.batch_size, heads).clone()
            updates = 0
            ...
            logged = gather_metrics(
                {
                    ...
                    "losses/head_value_spread": last[6],
                    "losses/explained_variance": explained_variance(b_values.mean(-1), b_returns.mean(-1)),
