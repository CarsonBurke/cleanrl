# SpanNorm observation-token PPO, v15.
# Shared learned token offsets start N(0,1), avoiding an all-zero RMS fixed point.
# Hypothesis: a block-spanning skip stabilizes the high-LR residual update.
# Normalize the initial attention input; both post-norm sublayers skip from the
# same block input (the FFN does not skip from the attention output); no final norm.
...
class Args:
    ...
    norm_method: Literal["peri", "siamese", "span", "hybrid"] = "span"
...
class Agent(nn.Module):
    ...
    def __init__(
        self, envs, *, norm_method="span", token_width=32, n_tokens=4, n_heads=2, n_blocks=3
    ):
        super().__init__()
