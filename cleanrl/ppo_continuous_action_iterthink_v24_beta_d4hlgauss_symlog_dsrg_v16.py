# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v16.
#
# WHAT CHANGES vs DSRG v8: v16 STACKS the two structural wins, both orthogonal (one touches
# the cell message, the other the actor readout path):
#   (1) DENDRITIC SUBUNITS (from v15): each neuron is a 2-layer net -- the K=16 fan-in is
#       partitioned into nb=4 nonlinear branch subunits, soma sums them (Poirazi-Mel), zero
#       new params. See detail below.
#   (2) DIRECT obs->ACTOR residual (from v14): actor_feat += Linear(obs), small orthogonal
#       init (gain 0.1). v14 (obs-actor on the v8 base) was the first variant to beat v8 at
#       EVERY mature checkpoint -- 4755 vs 4500 @2M, 6077 vs 5683 @4M, 7343 vs 6931 @8M
#       (+6%). It gives the policy a direct, low-variance obs->action path instead of
#       laundering the observation through the sparse graph + readout. Critic untouched.
# HYPOTHESIS: the two gains are mechanistically independent (richer per-neuron compute in the
# shared graph vs a direct obs->action shortcut for the actor), so they should STACK. v16
# tests additivity on top of the v14 SOTA base (7343 @8M).
#
# --- DENDRITIC SUBUNITS detail (v15) ---
# WHAT CHANGES vs DSRG v8: each NEURON becomes a 2-LAYER net (DENDRITIC SUBUNITS), zero new
# params. BIOLOGY: a real pyramidal neuron is not a perceptron -- its dendritic branches are
# nonlinear subunits (clustered, co-active inputs on one branch interact supralinearly via
# NMDA/Ca spikes), and the soma sums those subunit outputs. Poirazi-Mel: a pyramidal cell
# ~= a 2-layer network (branches = hidden units, soma = output). DSRG's neuron was the
# perceptron special case: pool all K inputs LINEARLY -> one nonlinearity. v15 partitions
# the K=16 fan-in into nb=4 branches of kb=4; each branch pools ITS inputs (shared + low-
# rank message) and passes them through a SiLU subunit nonlinearity (inner layer), then the
# soma SUMS the subunit outputs and applies the SiLU soma nonlinearity (outer layer). The
# routing (global competitive top-m of K), parameters, and active-edge count are byte-for-
# byte v8 -- only the message reorganizes, so this is a clean single-variable test of the
# dendritic 2-layer structure. Clustered selected edges now interact supralinearly within a
# branch instead of being averaged flat. (Same SiLU at both layers -> structure is the only
# variable; a sharper NMDA-like branch nonlinearity e.g. ReLU^2 is the natural v16.)
#
# --- v8 lineage note ---
...
class Args:
    ...
    obs_actor_std: float = 0.1       # v14: orthogonal-init gain of the direct obs->actor-feature residual
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v14: direct low-variance obs->actor-feature residual (see _trunks). Small
        # orthogonal init so it starts as a gentle contribution and grows if it helps.
        self.obs_actor = layer_init(nn.Linear(obs_dim, H), std=args.obs_actor_std)
    ...
    def _trunks(self, x):
        ...
        # v14: DIRECT obs->actor-feature residual. A plain linear obs->H skip gives the
        # policy direct, low-variance access to the raw observation without laundering it
        # through the sparse graph + readout. Small orthogonal init => starts ~10% and
        # grows only as it earns return. Actor ONLY (critic already excellent).
