# Raw-reward wide-support HL-Gauss on normalized residual SiTU PPO v11.
# Preserve our actor/critic trunks and optimizer. Rewards enter GAE unchanged:
# no reward normalization/clipping, return standardization, or EMA.
# Reuse the tested raw-symlog-v4 support: 511 centers across raw +/-20000,
# sigma/bin-width=0.75, plain CE, decode E[raw center] (not symexp(E[z])).
# No advnorm/value clip; asymmetric policy clip, fullbatch, 50M LR schedule.
# Hypothesis: fixed raw target coordinates avoid per-rollout frame drift.
# Wide finite support can still saturate; report out-of-range targets and bias.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, args=None):
        ...
        self.histogram = HLGaussConfig(
            v_min=args.value_min, v_max=args.value_max, num_bins=args.value_atoms,
            sigma_ratio=args.value_sigma_bins, transform="symlog",
            bin_type="centers", decode="scalar",
        ).build()
...
def main():
    ...
    try:
        ...
        writer.add_text("architecture", "residual SiTU-GLU; unit-L2 matrices and residual directions; sqrt(width) scaling; no LayerScale; raw rewards; fixed raw +/-20000 symlog K511 HL-Gauss head, sigma 0.75 bins, raw expectation, plain CE; no return frame or EMA; original actor and critic residual trunks")
        ...
        def prepare_targets(returns):
            ...
            label_error = (agent.histogram.probs_to_scalar(labels) - returns).abs().mean()
