...
class Args:
    ...
    edge_gate_temp: float = 0.5
    """temperature for deterministic stretched hard-concrete edge gates"""
...
def stretched_hard_concrete(logits, temperature, low, high):
    gate = torch.sigmoid(logits / temperature)
    gate = gate * (high - low) + low
...
class MorphogenicSubstrate(nn.Module):
    ...
    def forward(self, x):
        ...
        edge_gate = stretched_hard_concrete(
            edge_logits,
            self.edge_gate_temp,
        ...
        active_edges = edge_gate.sum(dim=(1, 2))
        ...
        edge_entropy = -(
            edge_gate * torch.log(edge_gate + 1e-8)
            + (1.0 - edge_gate) * torch.log(1.0 - edge_gate + 1e-8)
        )
        ...
        edge_tick_compute = active_edge_frac * active_tick_frac
