# PPO + IterThink v24.22 (v24.5 SAC-Beta + time-limit bootstrap). From v24.5.
#
# Ablation: port v43/v47 time-limit truncation handling into the v24.5 Beta stack
# without soft-critic or max-entropy changes. MuJoCo time-limit cuts are bootstrapped
# from infos["final_observation"] instead of treated as true terminals in scalar GAE
# and the C51 lambda target.
#
# --- inherited v24.5 notes ---
#
# PPO + IterThink v24.5 (SAC-BETA SHIFTED LOG-CONCENTRATIONS, PPO WIDE CAP). From v24.3.
#
...
def distributional_lambda_returns(
    ...
    Mean-matches the scalar GAE λ-return before support clipping. Shapes: rewards/dones/truncs (T, B);
    value_probs/final_probs (T, B, n); bootstrap_probs (B, n) = Z(s_T). Returns (T, B, n).
    TIME-LIMIT: truncated steps bootstrap from Z(final_observation) instead of being zeroed.
    ...
    for t in reversed(range(T)):
        ...
        trunc_t = truncs[t]
        boot_mask = 1.0 - (done_t - trunc_t)            # zero only for true termination
        mix = (1.0 - gae_lambda) * z_next + gae_lambda * g_next          # (B, n)
        dist_t = trunc_t.unsqueeze(-1) * final_probs[t] + (1.0 - trunc_t).unsqueeze(-1) * mix
        gn = (gamma * boot_mask).unsqueeze(-1)          # (B, 1)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            truncs[step] = torch.as_tensor(time_limit_truncations.astype(np.float32), device=device)
            final_obs_buf[step].zero_()
            if time_limit_truncations.any():
                if "final_observation" not in infos:
                    raise RuntimeError("time-limit truncation missing infos['final_observation']; cannot bootstrap")
        ...
        with torch.no_grad():
            ...
            if truncs_b.any():
                _, _, _, _, f_logits = agent.get_action_and_value(final_obs_buf[truncs_b])
            ...
            for t in reversed(range(args.num_steps)):
                ...
                delta = rewards[t] + args.gamma * boot - values[t]
            ...
            if auto_alpha:
                ...
                for t in reversed(range(args.num_steps)):
                    if t == args.num_steps - 1:
                        ...
                        nextnonterminal = 1.0 - dones[t + 1]
                        nextvalues = values[t + 1]
                    boot = (nextvalues + next_value_bonus[t]) * nextnonterminal + final_vals[t] * truncs[t]
                    delta = rewards[t] + args.gamma * boot - values[t]
