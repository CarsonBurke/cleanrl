# PPO + IterThink v143 symlog-multiscale-noembednorm-k6 (v142 without input embed norm).
#
# Key change from v141:
#   - Remove Gym's stateful NormalizeObservation/clip wrappers.
#   - Apply symlog immediately to each raw observation scalar, then encode with
#     the fixed multiscale basis [clip(z/s), tanh(z/s)] for s in {1, 5, 25}.
#     Project that basis directly to token dim and add learned feature slot
#     embedding. No input embedding RMSNorm; encoder pre-norm and encoder_norm
#     handle scale after the scalar value has entered the stream.
...
#
# Hypothesis:
#   If embedding RMSNorm is still suppressing useful scalar amplitude, removing
#   it should improve early reward/latent prediction while symlog still controls
#   raw MuJoCo outliers.
#
# -----------------------------------------------------------------------------
# Previous header:
...
class LeWMBackbone(nn.Module):
    def __init__(self, obs_dim, act_dim, args):
        ...
        # Symlog-observation tokenizer. Apply symlog immediately to raw scalars,
        # map through a fixed multiscale basis, project directly to token dim,
        # and add slot identity. There is intentionally no input embed norm.
