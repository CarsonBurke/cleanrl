# PPO temporal fully-decoupled ablation: actor, critic, and state-dependent
# exploration each receive their own temporal transformer backbone and their own
# learned CLS readout token.
#
# This tests whether the backbone itself, not just the readout, needs to be
# specialized for policy, value estimation, and exploration.
import os
import random
import time
...
class TemporalCLSBackbone(nn.Module):
    def __init__(self, obs_dim, context_len):
        ...
        self.cls = nn.Parameter(torch.randn(1, 1, EMBED_DIM) * cls_std)

        head_dim = EMBED_DIM // NUM_Q_HEADS
        rope_cos, rope_sin = build_rope_cache(context_len + 1, head_dim, torch.device("cpu"))
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        self.actor_backbone = TemporalCLSBackbone(obs_dim, self.context_len)
        self.critic_backbone = TemporalCLSBackbone(obs_dim, self.context_len)
        self.sde_backbone = TemporalCLSBackbone(obs_dim, self.context_len)
    ...
    def get_value(self, obs_seq):
        return self.critic(self.critic_backbone(obs_seq))
    ...
    def get_action_and_value(self, obs_seq, action=None):
        ...
        sde_features = self.sde_backbone(obs_seq)
