# HOPSD v33 — outcome-conditioning as a steering channel on the teacher (+phi-dropout).
# =====================================================================================
# v30's teacher rationalizes a_t from the realized future phi; v33 adds ONE mechanism:
# a scalar OUTCOME score u wired into the teacher actor as an extra steering input, with
# phi-dropout forcing the teacher to actually use it. The teacher keeps its AWR tilt
# (the proven improvement carrier); u is a second, continuous knob.
#   u (training) = z-scored NEXT-STATE advantage, u_t = normalized A_{t+1}. The one-step
#     SHIFT is the leak fix: u must exclude a_t's own reward, so it grades how the
#     future-AFTER-a_t went, not a_t itself (u=0 at the rollout's last step and wherever
#     the transition at t crossed an episode boundary). Winsorized to +/-3 after z-score.
#   phi-dropout (prob phi_drop): per sample, zero the teacher's phi block and set
#     phi_flag=0 while u STAYS — so on dropped samples the teacher can only shape its
#     conditional through u, which forces u to become a real, honored channel rather than
#     an ignored input the phi features make redundant. Applied to actor AND critic
#     (critic gets phi_flag + dropout but NO u input).
#   QUERY (student distill targets): teacher evaluated at (s, phi_realized, flag=1,
#     u=query_z) — a CONSTANT optimistic outcome u* for every state, so the student is
#     distilled toward "act as if the good-outcome future is coming".
#   BONUS (purified): vdis = |V_T(s,phi,flag=1) - V_T(s,0,flag=0)| (two teacher-critic
#     passes; dropout makes the flag=0 branch a trained, meaningful no-phi value). Same
#     z-norm / coef / warmup as v30 — measures how much phi changes the teacher's value.
# GO/NO-GO: debug/steer_dmu = mean_dim |mu_T(u=query_z) - mu_T(u=0)| on a fixed 1024-state
#   subsample (phi realized, flag=1). If steer_dmu stays ~0 the teacher ignores u and the
#   arm is dead; if it grows, u is a live steering axis. Also logs debug/u_std (spread of
#   the shifted advantage pre-normalization). Args: phi_drop=0.3, query_z=1.5.
# =====================================================================================
#
...
class Args:
    ...
    vdis_coef: float = 0.1           # v30: intrinsic bonus scale on the purified |V_full - V_zero| / std
    vdis_warmup_iters: int = 20      # v30: linear bonus ramp while the teacher critic trains
    phi_drop: float = 0.3            # v33: per-sample prob of zeroing the teacher phi block (flag=0, u stays)
    query_z: float = 1.5             # v33: constant optimistic outcome u* the student is distilled toward
...
class HindsightTeacher(nn.Module):
    ...
    Actor input:  [obs, near/far future-action mean/std, (g?), valid frac, u, phi_flag]
    Critic input: [obs, near/far future-action mean/std, valid frac, phi_flag]  (no u, no g)
    v33: actor gains u (outcome score) + phi_flag; critic gains phi_flag. The phi block
    (the 4A window features + valid frac) is per-sample zeroed under phi-dropout.
    """

    def __init__(self, obs_dim, act_dim, args):
        super().__init__()
        H = args.hidden
        # v33: +2 actor dims (u, phi_flag); +1 critic dim (phi_flag).
        actor_in = obs_dim + 4 * act_dim + 1 + (1 if args.teacher_sees_g else 0) + 2  # v19 windows; v33 u+flag
        critic_in = obs_dim + 4 * act_dim + 1 + 1  # v33: +phi_flag
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # --- v33: purified privileged-mismatch bonus (pre-GAE) ---
            # Windows do not depend on advantages, so they (and the phi-dropout mask,
            # reused below) are built here. The bonus is now |V_T(s,phi,flag=1) -
            # V_T(s,0,flag=0)|: two teacher-critic passes over the SAME critic, so it
            # measures how much phi moves the teacher's own value (no student critic).
            ...
            vf1 = fut_valid_frac.unsqueeze(-1)          # (T,N,1) valid-frac column
            ones_col = torch.ones_like(vf1)
            zeros_col = torch.zeros_like(vf1)
            zA = torch.zeros_like(near_mean)            # (T,N,A) zero window block
            teacher_critic_full_in = torch.cat(
                [obs, near_mean, near_std, far_mean, far_std, vf1, ones_col], dim=-1
            )  # phi realized, flag=1
            teacher_critic_zero_in = torch.cat(
                [obs, zA, zA, zA, zA, zeros_col, zeros_col], dim=-1
            )  # phi block zeroed, flag=0
            tcf = teacher_critic_full_in.reshape(-1, teacher_critic_full_in.shape[-1])
            tcz = teacher_critic_zero_in.reshape(-1, teacher_critic_zero_in.shape[-1])
            vt_full_chunks, vt_zero_chunks = [], []
            for start in range(0, tcf.shape[0], args.minibatch_size):
                sl = slice(start, start + args.minibatch_size)
                vt_full_chunks.append(hl_support.to_scalar(teacher.critic_logits(tcf[sl])))
                vt_zero_chunks.append(hl_support.to_scalar(teacher.critic_logits(tcz[sl])))
            v_teacher_full = torch.cat(vt_full_chunks).reshape(args.num_steps, args.num_envs)
            v_teacher_zero = torch.cat(vt_zero_chunks).reshape(args.num_steps, args.num_envs)
            vdis = (v_teacher_full - v_teacher_zero).abs()
            ...
            # --- HOPSD privileged context --- (windows already built above)
            adv_z = (advantages - advantages.mean()) / (advantages.std() + 1e-8)   # batch scope
            g = (returns - returns.mean()) / (returns.std() + 1e-8)                # batch scope

            # --- v33: outcome score u = z-scored NEXT-STATE advantage (leak fix) ---
            # u_t = normalized A_{t+1}: the one-step forward shift excludes a_t's own
            # reward, so u grades the future-after-a_t, not a_t. Zero at the rollout's
            # last step and wherever the transition at t crossed an episode boundary
            # (same boundary gate build_future_features uses for its k=1 window).
            u_shift = torch.zeros_like(advantages)
            u_shift[:-1] = advantages[1:] * (1.0 - transition_boundaries[:-1])
            u_std_raw = u_shift.std().item()  # pre-normalization spread (telemetry)
            u_z = ((u_shift - u_shift.mean()) / (u_shift.std() + 1e-8)).clamp(-3.0, 3.0)
            ...
            # --- v33: phi-dropout mask (per-iteration, per-sample); reused by the
            # teacher actor AND critic training inputs. phi_keep=1 -> phi present. ---
            phi_keep = (torch.rand(args.num_steps, args.num_envs, device=device) >= args.phi_drop).float()
            km = phi_keep.unsqueeze(-1)  # (T,N,1); zeros the phi block where dropped

            # Teacher ACTOR training input: phi (dropout-masked), then u (ALWAYS present,
            # the leak-fixed outcome), then phi_flag. u surviving dropout is what forces
            # the teacher to honor it. Column order is shared with the query input below.
            actor_ctx = [obs, near_mean * km, near_std * km, far_mean * km, far_std * km]
            if args.teacher_sees_g:
                actor_ctx.append(g.unsqueeze(-1))
            actor_ctx.append(fut_valid_frac.unsqueeze(-1) * km)
            actor_ctx.append(u_z.unsqueeze(-1))
            actor_ctx.append(km)  # phi_flag == keep mask
            teacher_actor_in = torch.cat(actor_ctx, dim=-1)

            # Teacher CRITIC training input: same phi-dropout, phi_flag, NO u.
            teacher_critic_in = torch.cat(
                [obs, near_mean * km, near_std * km, far_mean * km, far_std * km,
                 fut_valid_frac.unsqueeze(-1) * km, km],
                dim=-1,
            )

            # QUERY input (student distill targets): phi realized (no dropout), flag=1,
            # u = constant optimistic query_z. u and phi_flag are the last two columns.
            query_ctx = [obs, near_mean, near_std, far_mean, far_std]
            if args.teacher_sees_g:
                query_ctx.append(g.unsqueeze(-1))
            query_ctx.append(fut_valid_frac.unsqueeze(-1))
            query_ctx.append(torch.full_like(vf1, args.query_z))
            query_ctx.append(ones_col)
            teacher_query_in = torch.cat(query_ctx, dim=-1)
        ...
        b_teacher_query_in = teacher_query_in.reshape(-1, teacher_query_in.shape[-1])
        b_teacher_critic_full_in = teacher_critic_full_in.reshape(-1, teacher_critic_full_in.shape[-1])
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v33: distill from the u*-conditioned QUERY (phi realized, flag=1,
                # u=query_z), not the teacher's own dropout/true-u training input.
                with torch.no_grad():
                    q_alpha, q_beta = teacher.actor_params_for(b_teacher_query_in[mb_inds])
                kl_dims = beta_kl_per_dim(q_alpha, q_beta, s_alpha, s_beta)
        ...
        # teacher/student entropies, mean-action gap in native z space. v33: the actor
        # diagnostics use the QUERY input (the actual distill source, flag=1/u=query_z),
        # and teacher_ev uses the clean full-phi/flag=1 critic input.
        with torch.no_grad():
            t_vals, t_ents, s_ents, gaps = [], [], [], []
            for start in range(0, args.batch_size, args.minibatch_size):
                sl = slice(start, start + args.minibatch_size)
                ta, tb = teacher.actor_params_for(b_teacher_query_in[sl])
                ...
                t_vals.append(hl_support.to_scalar(teacher.critic_logits(b_teacher_critic_full_in[sl])))
            t_vals = torch.cat(t_vals).cpu().numpy()
            teacher_ev = np.nan if var_y == 0 else 1 - np.var(y_true - t_vals) / var_y

            # v33 GO/NO-GO steerability: on a fixed 1024-state subsample, how far the
            # teacher's mean action moves between u=0 and u=query_z (phi realized, flag=1).
            n_steer = min(1024, args.batch_size)
            steer_hi = b_teacher_query_in[:n_steer]          # u = query_z (last-but-one col)
            steer_lo = steer_hi.clone()
            steer_lo[:, -2] = 0.0                            # u = 0
            ah, bh = teacher.actor_params_for(steer_hi)
            al, bl = teacher.actor_params_for(steer_lo)
            steer_dmu = (ah / (ah + bh) - al / (al + bl)).abs().mean().item()
        ...
        writer.add_scalar("debug/steer_dmu", steer_dmu, global_step)
        writer.add_scalar("debug/u_std", u_std_raw, global_step)
