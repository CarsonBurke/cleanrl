# Base Beta PPO with the shared GatePolar optimizer, v2: v1 plus `--optimizer gate_adam`
# (the resistance gate on the Adam direction, no polar), isolating the gate from the direction.
# Hidden trunk matrices take the polar direction of Nesterov momentum; biases and
# heads take Adam. Decoupled weight decay on matrices is released per coordinate
# by the resistance gate Phi(z): a coordinate decays at full strength unless its
# slow gradient mean pushes back against the pull (plasticity proxy model v12).
# Hypothesis: PPO's trunks carry weight mass on observation directions that never
# earn a restoring gradient; shrinking it improves the policy's generalization
# across the drifting state distribution. `--optimizer adam` is the base control
# in the same trainer; `--weight-decay 0` is plain polar. LR is swept per arm.
import os
import random
import time
...
class Args:
    ...
    optimizer: str = "gate_polar"
    """adam (base control), gate_polar (polar trunks, Adam heads, gated decay) or gate_adam (Adam everywhere, gated decay)"""
...
def make_optimizer(args, agent):
    ...
    if args.optimizer == "gate_polar":
        groups = mlp_groups(agent.actor, agent.critic, head_lr_scale=args.head_lr_scale)
        return GatePolar(groups, lr=args.learning_rate, betas=(0.9, 0.999), eps=1e-5, weight_decay=args.weight_decay,
                         gate_beta=args.gate_beta, polar_scale=args.polar_scale, compile=args.compile)
    if args.optimizer == "gate_adam":
        return GatePolar([{"params": list(agent.parameters()), "polar": False}], lr=args.learning_rate, betas=(0.9, 0.999),
                         eps=1e-5, weight_decay=args.weight_decay, gate_beta=args.gate_beta, compile=args.compile)
    raise ValueError(f"unknown optimizer {args.optimizer}")


def describe_optimizer(args, optimizer):
    if args.optimizer == "adam":
        return "Adam eps 1e-5 fused (base)"
    sizes = [(len(group["params"]), "polar" if group["polar"] else "adam", group["lr_scale"]) for group in optimizer.param_groups]
    return f"GatePolar groups {sizes}; weight_decay {args.weight_decay}; gate_beta {args.gate_beta}"
