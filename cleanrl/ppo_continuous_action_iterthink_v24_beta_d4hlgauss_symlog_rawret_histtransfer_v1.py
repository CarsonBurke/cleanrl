# PPO + IterThink v24 beta d4-HL-Gauss symlog raw-return critic hist-transfer v1.
#
...
#
# Hist-transfer variant: keep the existing DenseNet-style trunk concatenation
# and residual branch sum, but also feed every previous trunk feature directly
# into each branch MLP's first preactivation. In each ThinkBlock the dense branch
# and every MoE expert now compute phi(W [current_state, past_features] + b)
# before their output projection, so earlier perceptron values participate in
# the transfer function rather than only in the block input/readout residuals.
#
...
class HistoryTransferBranch(nn.Module):
    def __init__(self, H, history_dim):
        super().__init__()
        self.in_linear = layer_init(nn.Linear(H + history_dim, H))
        self.act = ReLUSquared()
        self.out_linear = layer_init(nn.Linear(H, H))

    def forward(self, x, history):
        return self.out_linear(self.act(self.in_linear(torch.cat((x, history), dim=-1))))
...
class ThinkBlock(nn.Module):
    ...
    def __init__(self, in_dim, H, n_experts):
        ...
        self.history_norm = nn.RMSNorm(in_dim, elementwise_affine=False)
        self.dense = HistoryTransferBranch(H, in_dim)

        # Soft MoE branch (softmax over all experts, no top-K).
        self.moe_norm = nn.RMSNorm(H, elementwise_affine=False)
        self.gate = layer_init(nn.Linear(H, n_experts))
        self.experts = nn.ModuleList([HistoryTransferBranch(H, in_dim) for _ in range(n_experts)])

    def forward(self, cat_feats, x0):
        x = self.in_proj(cat_feats)                                   # (B, H)
        g = torch.sigmoid(self.resid_gate)                            # (H,)
        x_in = g * x + (1.0 - g) * x0                                 # (B, H), convex
        history = self.history_norm(cat_feats)                        # (B, in_dim)

        d_dense = self.dense(self.dense_norm(x_in), history)          # (B, H)

        m_in = self.moe_norm(x_in)
        weights = torch.softmax(self.gate(m_in), dim=-1)              # (B, E)
        all_out = torch.stack([e(m_in, history) for e in self.experts], dim=1)  # (B, E, H)
