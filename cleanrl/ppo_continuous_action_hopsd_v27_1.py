# HOPSD v27.1 — advantage-covariance gated per-dim teacher caps + per-dim distill KL budgets.
# =====================================================================================
# HYPOTHESIS: v27's second-order signal s_d = 1 - Var_w[z_d]/Var[z_d] flatlined (~0.004)
# — a variance-ratio estimator is underpowered at the ~1.2-nat tilt dose. The tilt's
# per-dim footprint is FIRST-moment: cov_d = corr(adv, z_d) detects which dims the
# advantage actually loads on. Sharpen those dims, broaden the rest. The gate is a
# CENTERED softmax over |cov_ema| so that the flat-signal limit reduces EXACTLY to the
# uniform-cap-20 arm (v27's de facto test, on champion pace):
#   signal: cov_d = corr(adv_z, z_d) over the flat batch latents, EMA 0.9 into cov_ema
#           (init ZEROS, so the run starts precisely at the flat/uniform-cap limit).
#   gate:   s_norm = softmax(|cov_ema| / 0.5); g_d = clamp(s_norm - 1/A, min=0)/((A-1)/A)
#           in [0, 1]; g_d == 0 for every dim when cov_ema is flat.
#   1) per-dim teacher concentration cap: cap_d = conc_cap_lo + (cap_hi - lo)*g_d —
#      floors teacher entropy on FREE dims, keeps full sharpness on dims advantage
#      loads on. Flat limit: all caps == conc_cap_lo (20), i.e. the uniform-cap arm.
#   2) per-dim distill budget: tau_d = distill_kl_clip*(1 + g_d - g_d.mean()), clamped
#      to [0.5, 1.5]*distill_kl_clip. Flat limit: tau_d == distill_kl_clip exactly.
# Kill-tells: uniform entropy rise + return sag = mis-gated; cov_ema staying ~0 = the
# tilt has no first-moment per-dim footprint either (both mechanisms no-op to the
# uniform-cap arm). Logs: debug/cov_d_0..{A-1}, debug/cov_ema_mean_abs, debug/g_d_max.
# Everything else byte-identical to v27.
# =====================================================================================
#
# --- v19 method (retained below) ---
# two-window hindsight: coarse temporal structure without per-step jitter.
...
class Args:
    ...
    conc_cap_lo: float = 20.0        # v27.1: per-dim cap floor on free dims (g_d -> 0)
...
class HindsightTeacher(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, args):
        ...
        with torch.no_grad():
            self.critic_head.weight.zero_()
        # v27.1: per-dim concentration cap, updated each iteration from the gate g_d
        # (cap_d = conc_cap_lo + (cap_hi - conc_cap_lo) * g_d); init at cap_hi.
        self.register_buffer(
            "conc_cap_d", torch.full((act_dim,), float(args.teacher_conc_cap))
        )

    def actor_params_for(self, x_priv):
        feat = self.actor_trunk(x_priv)
        alpha = (1.0 + F.softplus(self.alpha_head(feat))).clamp(max=self.conc_cap_d)
        beta = (1.0 + F.softplus(self.beta_head(feat))).clamp(max=self.conc_cap_d)
        return alpha, beta
...
if __name__ == "__main__":
    ...
    # v27.1: EMA of the per-dim advantage-covariance signal cov_d = corr(adv_z, z_d).
    # Init ZEROS so the run starts exactly at the flat/uniform-cap-20 limit.
    cov_ema = torch.zeros((act_dim,), device=device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # --- v27.1: first-order per-dim signal cov_d = corr(adv_z, z_d) over the
            # flat batch latents. adv_z is already z-scored (mean 0, unit std), so
            # cov_d = mean_N(adv_z * (z_d - mean z_d)) / std(z_d) is the correlation
            # between advantage and latent dim d — a first-moment detector for which
            # dims the tilt loads on. EMA'd (0.9) into cov_ema; a CENTERED softmax gate
            # g_d in [0, 1] (zero for every dim when cov_ema is flat) then drives BOTH
            # the per-dim teacher concentration cap and the per-dim distill budget,
            # updated here so they apply to THIS iteration's teacher/student epochs.
            z_flat = latent_zs.reshape(-1, act_dim)                       # (N, A)
            adv_z_flat = adv_z.reshape(-1).unsqueeze(-1)                  # (N, 1)
            z_centered = z_flat - z_flat.mean(0)                          # (N, A)
            cov_d = (adv_z_flat * z_centered).mean(0) / z_flat.std(0).clamp_min(1e-8)  # (A,)
            cov_ema.mul_(0.9).add_(0.1 * cov_d)
            # Centered softmax gate: flat cov_ema -> s_norm == 1/A -> g_d == 0 for all d.
            s_norm = torch.softmax(cov_ema.abs() / 0.5, dim=0)           # (A,)
            g_d = (s_norm - 1.0 / act_dim).clamp_min(0.0) / ((act_dim - 1.0) / act_dim)  # (A,)
            # Mechanism 1: per-dim teacher concentration cap (entropy floor on free dims).
            # Flat limit (g_d == 0): all caps == conc_cap_lo (the uniform-cap-20 arm).
            cap_d = args.conc_cap_lo + (args.teacher_conc_cap - args.conc_cap_lo) * g_d
            teacher.conc_cap_d.copy_(cap_d)
            # Mechanism 2: per-dim distill KL budget, centered so the mean is preserved.
            # Flat limit (g_d == 0): tau_d == distill_kl_clip exactly.
            tau_d = (args.distill_kl_clip * (1.0 + g_d - g_d.mean())).clamp(
                min=0.5 * args.distill_kl_clip, max=1.5 * args.distill_kl_clip
            )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # v27: per-dim budget tau_d (broadcast (A,) over (mb, A)) replaces the
                # scalar distill_kl_clip; centered on distill_kl_clip, reallocated by g_d.
        ...
        for _d in range(act_dim):
            writer.add_scalar(f"debug/cov_d_{_d}", cov_d[_d].item(), global_step)
        writer.add_scalar("debug/cov_ema_mean_abs", cov_ema.abs().mean().item(), global_step)
        writer.add_scalar("debug/g_d_max", g_d.max().item(), global_step)
