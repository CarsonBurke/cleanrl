# KL-constrained tangent policy without redundant surrogate clipping, v8.
# v6 predicted policy changes accurately but rejected many held-out clipped gains.
# Hypothesis: PPO clipping adds a pessimistic penalty even to useful directions;
# one full-rollout trust-region step does not need minibatch PPO ratio clipping.
# Fit and one sealed held-out decision use the likelihood-ratio surrogate, while
# exact Beta KL remains constrained separately on both trajectory splits.
# Log linear, unclipped and clipped held-out gain to separate gradient noise from
# clipping bias. Critic retains the fresh value-JVP optimizer; no grad/value clip.
import os
import random
import time
            ...
            with timer.span("update"):
                for epoch in range(args.update_epochs):
                    for indices in device_minibatches(args.batch_size, args.minibatch_size, device, shuffle_generator):
                        ...
                        prediction_metrics[updates].copy_(optimizer.step(mb_obs, mb_returns))
                        updates += 1
            if args.actor_update == "adam":
