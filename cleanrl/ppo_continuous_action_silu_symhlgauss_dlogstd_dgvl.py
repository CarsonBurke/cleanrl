# RMSNorm+SiLU + SymLog HL-Gauss (no vclip) + Direct Log-Std + DGVL
# Same as DPG variant but uses per-sample value CE loss as surprisal signal
# instead of -logprob. Value loss is decorrelated from policy entropy, avoiding
# the feedback loop where the gate just tracks policy confidence.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, num_bins):
        ...
        # Dreamer4-style: Linear → SiLU → Linear → RMSNorm → head
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Value loss: HL-Gauss cross-entropy, no vclip
                # Compute per-sample CE BEFORE policy loss so we can use it as surprisal
                ...
                # DGVL: Delight gate using value CE loss as surprisal
                # Uses RAW advantages (before normalization)
                with torch.no_grad():
