# SiameseNorm observation-token PPO, v14.
# Shared learned token offsets start N(0,1), avoiding an all-zero RMS fixed point.
# Hypothesis: coupled post-norm and identity streams preserve gradient flow at high LR.
# Shared deltas update both streams; depth-scaled post stream, normalized Q/K/V,
# and the essential final identity-stream RMS follow the adapted Siamese topology.
# Observation-token adaptation, not a full LLM recipe: noncausal attention only
# within each observation's four learned tokens; no temporal or cross-env attention.
# All methods retain the existing SiTU-GLU FFN and learned sigmoid residual gates.
# Papers: SiameseNorm https://arxiv.org/abs/2602.08064
#         SpanNorm    https://arxiv.org/abs/2601.22580
#         HybridNorm  https://arxiv.org/abs/2503.04598
# Common ScaleInit: orthogonal token projection gain sqrt(tokens*width/in_dim),
# Q/K/V gain 1; attention output and SiTU down weights scaled by 1/sqrt(blocks).
# SiTU target output variance 0.5; norm gains 1, gates -1.5, readout 1/sqrt(tokens*width).
# Matched PPO: one epoch, LR 0.0096, reward normalization, clipped scalar MSE,
# raw GAE, one minibatch, no gradient clipping; unchanged CUDA learner/native rollout.
import os
import random
import time
...
from cleanrl.shared.paper_norm import make_paper_norm_trunk
...
class Args:
    ...
    norm_method: Literal["peri", "siamese", "span", "hybrid"] = "siamese"
    """paper normalization topology for both observation-token actor and critic"""
    token_width: int = 32
    """features per learned observation token"""
    n_tokens: int = 4
    """noncausal attention tokens per observation; no temporal or cross-env mixing"""
    n_heads: int = 2
    """attention heads per block; must divide token_width"""
    n_blocks: int = 3
    """attention/SiTU-GLU blocks, with common 1/sqrt(n_blocks) output-weight scaling"""
...
class Agent(nn.Module):
    ...
    def __init__(
        self, envs, *, norm_method="siamese", token_width=32, n_tokens=4, n_heads=2, n_blocks=3
    ):
        super().__init__()
        ...
        feature_dim = n_tokens * token_width
        self.critic = nn.Sequential(
            make_paper_norm_trunk(
                observation_dim, token_width, n_tokens, n_blocks, n_heads, method=norm_method
            ),
        ...
        self.actor = nn.Sequential(
            ...
            layer_init(nn.Linear(feature_dim, 2 * self.action_dim), std=0.01),
        )
...
def main():
    ...
    try:
        ...
        writer.add_text(
            "policy",
            f"Beta host actor; observation-token attention; norm_method={args.norm_method}; "
            f"tokens={args.n_tokens}; token_width={args.token_width}; heads={args.n_heads}; "
            f"blocks={args.n_blocks}; SiTU-GLU; learned sigmoid residual gates; "
            f"common ScaleInit output weights=1/sqrt(blocks), readout=1/sqrt(tokens*width); "
            f"LR={args.learning_rate}; epochs={args.update_epochs}; grad_clip={args.grad_clip}; "
            f"clip_heads={args.clip_heads}; clip_vloss={args.clip_vloss}; reward_norm={args.reward_norm}; "
            f"minibatches={args.num_minibatches}; norm_adv={args.norm_adv}",
        )
