# Fixed-capacity SiTU reorganization, v6. Preserve width64, three hidden43
# SiTU branches, all weights/gates, four RMS norms per trunk, and Beta heads.
# Move existing normalization to gate/up/product/output, or change additive
# residuals to interpolation / input anchoring. No added parameters or layers.
# Actor/critic can select independently; PPO and the 50M schedule stay fixed.
import os
import random
import time
...
class Args:
    ...
    actor_variant: Literal["baseline", "gate_only", "up_only", "product_norm", "branch_norm", "blend", "anchor"] = "baseline"
    """same-capacity actor normalization/residual organization"""
    critic_variant: Literal["baseline", "gate_only", "up_only", "product_norm", "branch_norm", "blend", "anchor"] = "baseline"
    """same-capacity critic normalization/residual organization"""
...
class Agent(nn.Module):
    ...
    def __init__(self, envs, *, actor_variant="baseline", critic_variant="baseline"):
        super().__init__()
        ...
        self.critic = nn.Sequential(
            make_situ_reorg_trunk(observation_dim, 64, variant=critic_variant),
            layer_init(nn.Linear(64, 1), std=1.0),
        )
        self.actor = nn.Sequential(
            make_situ_reorg_trunk(observation_dim, 64, variant=actor_variant),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", f"SiTU fixed capacity; actor={args.actor_variant}; critic={args.critic_variant}; "
                        "32x LR; 1 minibatch; raw GAE")
        envs = make_training_env(args, run_name)
        resources.callback(envs.close)
        agent = Agent(envs, actor_variant=args.actor_variant,
                      critic_variant=args.critic_variant).to(device)
