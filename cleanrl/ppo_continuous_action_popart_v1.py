# Ablation of Beta PPO: PopArt on the scalar critic, following the V-MPO family.
# The critic emits standardized values. GAE and bootstrap use unnormalized
# values. After each rollout, return moments are EMA-updated (rate 1e-4) and
# the value head plus Adam moments are rescaled so unnormalized predictions
# stay put. PPO's value loss then trains in normalized coordinates.
import os
import random
import time
...
class Args:
    ...
    """EMA rate for PopArt return mean/second-moment, matching V-MPO"""
    popart_std_min: float = 1e-2
    """lower clamp on PopArt return standard deviation"""
    popart_std_max: float = 1e6
    """upper clamp on PopArt return standard deviation"""
...
class Agent(nn.Module):
    ...
    def unnormalize_value(self, normalized_value):
        return normalized_value * self.popart_std + self.popart_mean

    def get_value(self, x):
        return self.unnormalize_value(self.critic(x))
    ...
    @torch.no_grad()
    def update_popart(self, returns, rate, std_min, std_max, optimizer=None):
        """Match V-MPO PopArt stats, and keep Adam moments in the new coordinates."""
        ...
        new_std = (
            (new_sq_mean - new_mean.square()).clamp_min(0.0).sqrt().clamp(std_min, std_max)
        )
        ...
        if optimizer is not None:
            for parameter, factor in ((head.weight, scale), (head.bias, scale)):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        return action, logprob, entropy, self.unnormalize_value(value)
...
def validate_args(args):
    ...
    if not 0.0 < args.popart_rate <= 1.0:
        raise ValueError("popart_rate must be in (0, 1]")
    if not 0.0 < args.popart_std_min <= args.popart_std_max:
        raise ValueError("PopArt std clamps must satisfy 0 < min <= max")
    return args
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); FP32; native-action storage; PopArt critic")
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            for step in range(args.num_steps):
                ...
                with torch.no_grad():
                    if args.compile:
                        torch.compiler.cudagraph_mark_step_begin()
                    alpha, beta, normalized_value = policy_model(next_obs)
                    value = agent.unnormalize_value(normalized_value)
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                b_returns = agent.update_popart(
                    returns.flatten(), args.popart_rate, args.popart_std_min,
                    args.popart_std_max, optimizer,
                ).clone()
            b_obs, b_native = obs.flatten(0, 1), native_actions.flatten(0, 1)
            b_logprobs = logprobs.flatten()
            b_values = (values.flatten() - agent.popart_mean) / agent.popart_std
            updates = 0
