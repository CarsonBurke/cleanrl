# 32x LR + 1 minibatch with a residual SiTU-GLU trunk (sTiGLU) instead of
# LeakyReluSq. Same host-actor trainer as ppo_continuous_action_32xlr_1mb_v1:
# lr=9.6e-3, num_minibatches=1; PPO loss, clip, critic and Beta head are
# unchanged. The trunk is in_proj -> two gated SiTU blocks with per-block
# residuals plus a U-net long skip re-injecting the early stream late (as in
# modded-nanogpt, scalar gates init sigmoid(-1.5) ~= 0.18, no normalization).
# Residuals keep trunk outputs O(1) so head gradients stay in the tanh regime
# (v1 without them blew the trust region: KL ~0.2, clipfrac ~0.4); the unit-
# variance stream also puts SiTU gate/up preacts exactly at their v=2 design
# point. Hypothesis: fade-in multiplicative gating explores joint structure
# that fixed activations miss, without destabilizing PPO's trust region.
import os
import random
import time
...
from cleanrl.shared.host_actor import HostSiTUResActor, make_situ_res_trunk
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            make_situ_res_trunk(observation_dim, 64),
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta: alpha,beta=1+softplus(head); FP32; residual SiTU-GLU trunk; native-action storage; host actor mirror")
        ...
        host_actor = HostSiTUResActor(agent.actor, args.num_envs)
