"""PPO with Trading-Bot-Inspired Architecture (tbot_tight)

Based on tbot with two changes for tighter initial exploration:

1. **Tighter noise scale**: sde_noise_scale_init lowered from 1.0 to 0.5
   (softplus maps 0.5 -> ~1.0 vs 1.0 -> ~1.31), starting with smaller noise.

2. **RMSNorm on SDE features**: Added RMSNorm after sde_fc before soft-sign
   bounding, stabilizing the feature magnitudes feeding into the noise pathway.

Otherwise identical to tbot:
- Soft-sign bounded SDE noise with actor weight modulation
- Distributional critic (symlog two-hot, DreamerV3-style)
- RMSNorm + SiLU activations
- Mean-scale decoupling
...
class Args:
    ...
    # tbot-specific
    sde_noise_scale_init: float = 0.5
...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        ...
        sde_raw = self.sde_norm(self.sde_fc(h))  # (batch, hidden_dim)
