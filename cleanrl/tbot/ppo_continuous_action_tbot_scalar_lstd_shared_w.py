"""PPO with Trading-Bot-Inspired Architecture — Shared W Ablation (tbot_scalar_lstd_shared_w)

Ablation of tbot_scalar_lstd: adds back actor_out weight coupling to the noise variance.

In tbot_scalar_lstd, action_var is computed purely from the SDE latent and learned log_std:
    action_var = (sde_latent^2) @ std_sq

THIS ABLATION adds actor weight modulation on top:
    action_var = ((sde_latent^2) @ std_sq) * (W^2).sum(dim=1)
where W = actor_out.weight. This scales each action dimension's variance by the squared
norm of its corresponding actor weight row, coupling exploration magnitude to the actor's
output structure (policy-aligned exploration).

This tests whether the decoupled noise (no actor_out weight influence) in tbot_scalar_lstd
is better than coupling variance to actor weights as in the lattice/tbot_scalar lineage.

Everything else is identical to tbot_scalar_lstd:
- Learned log_std_param [hidden_dim, act_dim] matrix with clamp
- RMSNorm + tanh SDE latent pipeline
...
class Agent(nn.Module):
    ...
    def _get_action_std(self, h):
        """State-dependent action std via RMSNorm->tanh SDE + learned log_std + shared W modulation."""
        ...
        # Shared W modulation: scale each action dim by squared norm of its actor weight row
        W = self.actor_out.weight  # (act_dim, hidden_dim)
        w_sq_norm = (W ** 2).sum(dim=1)  # (act_dim,)
        action_var = action_var * w_sq_norm  # (batch, act_dim)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        W = agent.actor_out.weight
        writer.add_scalar("tbot/w_sq_norm_mean", (W ** 2).sum(dim=1).mean().item(), global_step)
