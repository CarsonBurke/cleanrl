# Predictive-coding actor-critic with Fisher output geometry and Dreamer return normalization v11.
#
# Exact fork of raw-reward v10, replacing actor TD-error RMS normalization with
# DreamerV3's zero-initialized EMA of the 5th-to-95th percentile return range.
# The online return sample is the unclipped one-step bootstrap target. Rewards and
# critic TD errors remain raw, actor/critic TD clamps remain unchanged, and decay is zero.
import math
import os
import random
import time
...
def compute_td_modulations(
    ...
    td_target = reward + gamma * (~terminated).float() * next_value
    ...
    critic_delta = td_error.clamp(-critic_clip, critic_clip)
    return td_target, td_error, actor_delta, critic_delta, return_offset, return_scale
