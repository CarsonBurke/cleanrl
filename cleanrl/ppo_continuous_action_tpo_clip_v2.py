# TPO objective study control: Beta PPO with the asymmetric clipped surrogate used
# by the `tpo-objective-20260920-retry/clip/ppo` reference run.  The reference
# calls this DAPO-style band (0.80, 1.28): positive-advantage probability
# increases are allowed to 1.28x while decreases stop at 0.80x; negative
# advantages retain the opposite one-sided bound.  This preserves more room for
# low-probability exploratory actions than a symmetric PPO band.  The TPO
# Bernoulli target itself is categorical and is intentionally not transplanted
# onto continuous Beta densities, where a selected density is not a probability.
# Positive Beta parameters replace the Gaussian head; native samples are stored
# for old/new densities and all computations stay in FP32.
import os
import random
import time
import math
...
class Args:
    ...
    clip_coef: float = 0.2
    """legacy symmetric clip; retained for value-loss clipping"""
    clip_low: float = 0.80
    """lower probability-ratio bound for the policy surrogate"""
    clip_high: float = 1.28
    """upper probability-ratio bound for the policy surrogate"""
    ...
    policy_objective: str = "tpo"
    """continuous TPO target objective or the reference asymmetric clip control"""
    tpo_eta: float = 2.0
    """TPO temperature; the joint target ratio is exp(utility / eta)"""
    utility_clip: float | None = 3.0
    """bound whitened TPO utilities; None disables the bound"""
    logratio_guard: float = 20.0
    """log-density guard used by the optional ratio clip diagnostics"""
    tpo_cell_width: float = 1e-3
    """per-coordinate action-cell width used to turn Beta density into mass"""
...
class Agent(nn.Module):
    ...
    def action_logprobs(self, alpha, beta, native_action):
        distribution = Beta(alpha, beta, validate_args=False)
        return distribution.log_prob(native_action) - self.log_action_scale

    def action_logprob(self, alpha, beta, native_action):
        return self.action_logprobs(alpha, beta, native_action).sum(-1)
...
def tpo_kagg_loss(logratio_components, old_logprobs, utility, eta, cell_width):
    """Kaggriculture-style single-sample TPO for a continuous Beta action.

    A sampled continuous action is treated as a small cell.  For each Beta
    coordinate, p = density(action) * cell_width is the cell probability;
    TPO shifts its logit by the rollout utility and fits the resulting Bernoulli
    target with KL(q || p_theta).  The entropy term of q is constant and omitted.
    Unlike the intra-TPO family, this keeps the target in probability space.
    """
    if cell_width <= 0.0 or cell_width >= 1.0:
        raise ValueError("tpo_cell_width must be in (0, 1)")
    log_cell = math.log(cell_width)
    old_mass = (old_logprobs.detach() + log_cell).clamp(min=-30.0, max=-1e-6)
    new_mass = (old_logprobs.detach() + logratio_components + log_cell).clamp(min=-30.0, max=-1e-6)
    old_logit = old_mass - torch.log(-torch.expm1(old_mass))
    target_logit = old_logit + utility.detach().unsqueeze(-1) / eta
    q = torch.sigmoid(target_logit)
    # Current mass is represented by its log probability for stable BCE.
    return -(q * new_mass + (1.0 - q) * torch.log(-torch.expm1(new_mass)))


def ppo_loss(agent, observations, native_actions, old_logprobs, advantages, returns, old_values, args, utilities=None):
    alpha, beta, newvalue = agent.get_policy_and_value(observations)
    distribution = Beta(alpha, beta, validate_args=False)
    new_component_logprobs = agent.action_logprobs(alpha, beta, native_actions)
    old_component_logprobs = old_logprobs.detach()
    logratio_components = new_component_logprobs - old_component_logprobs
    logratio = logratio_components.sum(-1)
    ratio = logratio.clamp_max(args.logratio_guard).exp()
    with torch.no_grad():
        old_approx_kl = (-logratio).mean()
        approx_kl = ((ratio - 1) - logratio).mean()
        clipfrac = ((ratio < args.clip_low) | (ratio > args.clip_high)).float().mean()
    if args.policy_objective == "tpo":
        utility = tpo_utility(advantages, args.utility_clip) if utilities is None else utilities
        pg_loss = tpo_kagg_loss(
            logratio_components, old_component_logprobs, utility, args.tpo_eta, args.tpo_cell_width
        ).mean()
    elif args.policy_objective == "clip":
        effective_logratio = torch.where(
            advantages >= 0.0,
            logratio.clamp_max(float(np.log(args.clip_high))),
            logratio.clamp_min(float(np.log(args.clip_low))),
        )
        pg_loss = -(effective_logratio.exp() * advantages).mean()
    else:
        raise ValueError("policy_objective must be 'tpo' or 'clip'")
...
def validate_args(args):
    ...
    if not (0.0 < args.clip_low < 1.0 < args.clip_high):
        raise ValueError("clip_low must be in (0, 1) and clip_high must exceed 1")
    if args.policy_objective not in {"tpo", "clip"}:
        raise ValueError("policy_objective must be 'tpo' or 'clip'")
    if args.tpo_eta <= 0.0 or not math.isfinite(args.tpo_eta):
        raise ValueError("tpo_eta must be finite and positive")
    if not 0.0 < args.tpo_cell_width < 1.0:
        raise ValueError("tpo_cell_width must be in (0, 1)")
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", f"Kaggriculture-style Beta TPO cell target (eta={args.tpo_eta}, cell={args.tpo_cell_width}) with asymmetric clip control [{args.clip_low}, {args.clip_high}]; FP32; native-action storage; host actor mirror")
        ...
        def rollout_statistics(observations, native):
            ...
            return value.flatten(), agent.action_logprobs(alpha, beta, native)

        def loss_model(observations, native, old_logprobs, advantages, returns, old_values, utilities):
            return ppo_loss(agent, observations, native, old_logprobs, advantages, returns, old_values, args, utilities)
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                # Kaggriculture computes the TPO target utility once from the
                # complete rollout; recomputing it per minibatch changes q
                # during the epoch and creates optimizer-order-dependent drift.
