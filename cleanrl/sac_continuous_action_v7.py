# SAC v7: v4 optimistic HL-Gauss critic + scalar TD auxiliary.
#
# Keeps v4's optimistic categorical initialization, which drove strong early
# HalfCheetah learning, but adds a lightly weighted scalar Huber loss on the
# decoded Q expectation. Hypothesis: CE gives stable value classification but
# under-corrects fine actor-facing Q ranking later; the scalar auxiliary restores
# TD-error-proportional correction without abandoning HL-Gauss smoothing.
#
# Diagnostics log scalar TD error, projection bias, support usage, and critic
...
class Args:
    ...
    scalar_td_coef: float = 0.05
    """weight for scalar decoded-Q Huber auxiliary, normalized by bin width"""
...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            qf1_ce_loss = -(target_probs * qf1_log_probs).sum(dim=-1).mean()
            qf2_ce_loss = -(target_probs * qf2_log_probs).sum(dim=-1).mean()
            qf1_scalar_values = hl_support.to_scalar(qf1_logits)
            qf2_scalar_values = hl_support.to_scalar(qf2_logits)
            scalar_loss_scale = max(hl_support.bin_width, 1.0)
            qf1_scalar_loss = F.smooth_l1_loss(qf1_scalar_values / scalar_loss_scale, next_q_value / scalar_loss_scale)
            qf2_scalar_loss = F.smooth_l1_loss(qf2_scalar_values / scalar_loss_scale, next_q_value / scalar_loss_scale)
            qf1_loss = qf1_ce_loss + args.scalar_td_coef * qf1_scalar_loss
            qf2_loss = qf2_ce_loss + args.scalar_td_coef * qf2_scalar_loss
            ...
            if global_step % 100 == 0:
                ...
                writer.add_scalar("losses/qf1_ce_loss", qf1_ce_loss.item(), global_step)
                writer.add_scalar("losses/qf2_ce_loss", qf2_ce_loss.item(), global_step)
                writer.add_scalar("losses/qf1_scalar_loss", qf1_scalar_loss.item(), global_step)
                writer.add_scalar("losses/qf2_scalar_loss", qf2_scalar_loss.item(), global_step)
