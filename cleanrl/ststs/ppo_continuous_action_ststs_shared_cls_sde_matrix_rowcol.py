...
# - state-conditioned matrix modulation through latent-row gates, action-column
#   gates, and a global temperature shift
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        self.base_log_std = nn.Parameter(torch.ones(EMBED_DIM, action_dim) * LOG_STD_INIT)
        self.row_proj = layer_init(nn.Linear(EMBED_DIM, EMBED_DIM), std=0.01)
        self.col_proj = layer_init(nn.Linear(EMBED_DIM, action_dim), std=0.01)
        self.temp_proj = layer_init(nn.Linear(EMBED_DIM, 1), std=0.01)
        nn.init.zeros_(self.row_proj.weight)
        nn.init.zeros_(self.row_proj.bias)
        nn.init.zeros_(self.col_proj.weight)
        nn.init.zeros_(self.col_proj.bias)
    ...
    def _effective_log_std(self, latent_sde):
        row_delta = LOG_STD_DELTA * torch.tanh(self.row_proj(latent_sde))
        col_delta = LOG_STD_DELTA * torch.tanh(self.col_proj(latent_sde))
        temp_delta = LOG_STD_DELTA * torch.tanh(self.temp_proj(latent_sde))
        return self.base_log_std.unsqueeze(0) + row_delta.unsqueeze(-1) + col_delta.unsqueeze(1) + temp_delta.unsqueeze(-1)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            row_delta = LOG_STD_DELTA * torch.tanh(agent.row_proj(latent_sde))
            col_delta = LOG_STD_DELTA * torch.tanh(agent.col_proj(latent_sde))
            temp_delta = LOG_STD_DELTA * torch.tanh(agent.temp_proj(latent_sde))
            ...
            writer.add_scalar("sde/row_delta_rms", row_delta.pow(2).mean().sqrt().item(), global_step)
            writer.add_scalar("sde/col_delta_rms", col_delta.pow(2).mean().sqrt().item(), global_step)
            writer.add_scalar("sde/temp_delta_rms", temp_delta.pow(2).mean().sqrt().item(), global_step)
