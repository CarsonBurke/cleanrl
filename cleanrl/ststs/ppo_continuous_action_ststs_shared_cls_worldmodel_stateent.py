# PPO + World model + Dreamer4-style state prediction entropy bonus. No SDE.
#
# 4 CLS tokens: actor, critic, dynamics, state_pred.
# Diagonal Gaussian policy (learned log_std vector, not state-dependent).
# World model with fresh latents (v2 fix). State entropy bonus on reward.
import os
import random
import time
...
class STSTSCLSBackbone(nn.Module):
    def __init__(self, obs_dim, context_len):
        ...
        for p in [self.actor_cls, self.critic_cls, self.dynamics_cls, self.state_pred_cls]:
    ...
    def forward(self, obs_seq):
        ...
        cls_tokens = torch.stack([self.actor_cls, self.critic_cls, self.dynamics_cls, self.state_pred_cls], dim=0)
        ...
        return (
            ...
            tokens[:, -1, self.state_pred_cls_index],
        )
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, obs_seq, action=None):
        actor_latent, critic_latent, dynamics_latent, state_pred_latent = self._encode(obs_seq)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            _, _, _, sp_latent = agent._encode(agent.obs_history)
