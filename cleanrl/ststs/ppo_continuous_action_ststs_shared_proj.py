# PPO with a shared axial space-time transformer backbone.
#
...
#
# Ablation:
# - one shared STSTS backbone for actor and critic
# - flatten the final token grid
# - separate linear projections into actor and critic heads
#
# PPO base does not include an SDE head, so this variant keeps the standard
# state-independent diagonal Gaussian exploration of `ppo_continuous_action.py`.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        self.backbone = STSTSBackbone(obs_dim, self.context_len)
    ...
    def _encode(self, obs_seq):
        tokens = self.backbone(obs_seq)
        flat = tokens.reshape(tokens.shape[0], -1)
        actor_latent = torch.tanh(self.actor_proj(flat))
        critic_latent = torch.tanh(self.critic_proj(flat))
