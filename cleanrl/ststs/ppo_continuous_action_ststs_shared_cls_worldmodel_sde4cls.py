# PPO + World model + SDE exploration via 4th CLS token.
#
# 4 CLS tokens: actor, critic, dynamics, SDE.
# Key fix: world model trains on FRESH latents recomputed each minibatch,
# not stale latents from rollout. This prevents the transition model from
# chasing a non-stationary target as PPO updates the backbone.
# Rollout buffer stores next_obs_seqs instead of latents.
import os
import random
import time
...
class Agent(nn.Module):
    def __init__(self, envs, num_envs):
        ...
        # SDE: learned exploration matrix (EMBED_DIM -> action_dim variance)
    ...
    def _action_std_fixed(self):
        """Fixed std for imagination rollouts (no SDE latent available)."""
        log_std = (self.log_std_param + LOG_STD_INIT).clamp(LOG_STD_MIN, LOG_STD_MAX)
        # Use mean variance across latent dims as a scalar approximation
    ...
    def get_all_for_update(self, obs_seq, action):
        """Unified forward pass for PPO + WM update. Returns PPO outputs + fresh dynamics latent."""
        actor_latent, critic_latent, dynamics_latent, sde_latent = self._encode(obs_seq)
        action_mean = self.actor_mean(actor_latent)
        action_std = self._get_action_std(sde_latent)
        probs = Normal(action_mean, action_std)
        return probs.log_prob(action).sum(1), probs.entropy().sum(1), self.critic(critic_latent), dynamics_latent
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, _ = agent.get_action_and_value(agent.obs_history)
        ...
        # Validity mask for world model: transition invalid if next step is new episode
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Unified forward pass: PPO outputs + fresh dynamics latent
                newlogprob, entropy, newvalue, dynamics_latent = agent.get_all_for_update(b_obs_seqs[mb_inds], b_actions[mb_inds])
                ...
                # Target: encode next obs_seq with current backbone, detached
                with torch.no_grad():
                    _, _, next_dynamics, _ = agent._encode(b_next_obs_seqs[mb_inds])
