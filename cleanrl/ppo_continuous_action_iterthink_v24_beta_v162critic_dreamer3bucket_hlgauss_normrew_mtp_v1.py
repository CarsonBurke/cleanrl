# PPO + IterThink v24 Beta + v162 critic + Dreamer3-bucket HL-Gauss norm-reward MTP v1.
#
...
# Dreamer3-style exponentially spaced value buckets over normalized-return range:
#   - NormalizeReward + clipped normalized rewards bound targets near ±10/(1-gamma)
#   - value bucket centers are symexp(linspace(symlog(-1000), symlog(1000), 511))
...
# the PPO critic on the environment reward return after optional reward scaling.
# Entropy enters the actor two ways:
...
# calibrated. In this variant the target is normalized/clipped-reward scalar
# return HL-Gauss MTP over Dreamer3-spaced buckets.
#
...
# v24's distributional lambda-return to v162 normalized scalar-return HL-Gauss
# MTP over Dreamer3 buckets; only the policy advantage transforms are selected by
...
class Args:
    ...
    # ADVANTAGE only, NEVER the critic's target. The critic regresses to the scaled reward
    ...
    # Defaults match normalized/clipped return support ±10/(1-gamma) = ±1000,
    # expressed in symlog coordinates.
    num_bins: int = 511
    ...
    # Match CleanRL PPO's reward scaling. With clip_reward=True and gamma=0.99,
    # normalized critic targets are bounded near ±1000 discounted return units.
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            # scaled return units. The default rankgauss path does not consume this.
