# 32x LR + 1 minibatch + V-MPO v30 critic *targets*, no HL-Gauss.
# v1 was wrong: it froze a critic and scored V(s') from that copy while
# V(s) came from the online net, held 100 rollouts (1000 steps at 32x LR).
# v30 scores both V(s) and V(s') with the online critic; the frozen copy
# is the policy. The critic-side piece that is not HL-Gauss is explicit
# next-observation GAE: one batched online forward on every transition's
# actual next obs, including finals before autoreset.
import os
import random
import time
...
def main():
    ...
    try:
        ...
        writer.add_text("policy", "Beta host actor; online critic V(s) and V(s'); explicit-next GAE")
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            with timer.span("gae"), torch.no_grad():
                ...
                next_values = next_value_model(batch.transition_observations.flatten(0, 1)).view(
