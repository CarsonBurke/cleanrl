# PPO + IterThink v24.17 (DIRECT BETA SPLINE ACTION FLOW). From v24.14.
#
# Ablation: remove the logit/sigmoid flow entirely. The source policy remains
# v24's unimodal independent Beta, but the flow now maps z0 in (0,1) directly to
# z1 in (0,1):
#     z0 ~ Beta(alpha(s), beta(s)); z1 = SplineFlow_s(z0); action = low + (high-low) * z1
# PPO replay stores the bounded post-flow coordinate z1. During updates the
# current spline is inverted exactly to recover z0 and the density is
#     log pi(z1|s) = log Beta(z0|s) - log |d SplineFlow_s(z0) / dz0|.
#
# This intentionally uses a monotone piecewise-linear coupling spline, not a
# rational-quadratic spline. Widths are fixed uniform bins; state-conditioned
# positive bin heights define an exact monotone map on [0,1]. Zero final-layer
# initialization gives uniform heights and therefore the identity map.
#
# Diagnostics log spline logdet mean, replay boundary fraction, and inverse
# reconstruction error to catch density/replay mistakes near the bounded support.
#
# --- inherited v24.14 notes ---
#
# PPO + IterThink v24.14 (BETA LOGIT ACTION FLOW, EXACT Y-REPLAY). From v24.9.
#
# v24.12 tried y-replay but mixed unclamped replay logits with clamped bounded
# actions. That can make the stored density coordinate differ from the executed
# action near boundaries. v24.14 redoes y-replay correctly: the rollout stores
# post-flow logit y1 and the env action is computed from the same sigmoid(y1)
# coordinate without an extra clamp. PPO replay uses y1 directly and inverts the
# current flow to compute the exact transformed density.
#
# --- inherited v24.9 notes ---
# PPO + IterThink v24.9 (BETA LOGIT ACTION FLOW). From v24.
#
# Full-covariance tanh Normal gave coordinated noise but broke PPO's actor
# gradients. v24.9 keeps v24's bounded independent Beta as the source policy and
# adds multivariate dependence through an exactly invertible additive coupling
# flow in logit action space:
#     z0 ~ Beta(alpha(s), beta(s)); y0 = logit(z0)
#     y1 = Flow_s(y0); z1 = sigmoid(y1); action = low + (high-low) * z1
# PPO ratios use the exact transformed density by inverting z1 under the current
# flow. The rollout buffer stores z1, so action replay remains exact.
#
# A detached conditional-flow-matching auxiliary trains the same flow toward
# beta-smoothed rollout-action endpoints, weighted by positive advantage. This
# gives the flow coordinated exploration structure without letting the auxiliary
# move the v24 actor trunk or critic.
#
# --- inherited v24 notes ---
#
...
class Args:
    ...
    # v24.17: action distribution. "beta_flow" means a Beta source plus direct
    # bounded monotone PL spline coupling flow (legacy name retained for CLI parity).
    # | "beta" (v24 control) | "gaussian" (diagonal dreamer4 control).
    # (dreamer4 state-dependent log-VARIANCE head, not SAC log_std; soft tanh-rescale bound).
    actor_dist: str = "beta_flow"
    ...
    beta_flow_steps: int = 4
    beta_flow_spline_bins: int = 8
    beta_flow_spline_min_bin_height: float = 1e-4
    beta_flow_cfm_coef: float = 0.01
    beta_flow_cfm_weight_clip: float = 5.0
    beta_flow_cfm_target_concentration: float = 64.0
    beta_flow_cfm_detach_trunk: bool = True
...
class MonotoneLinearSplineCouplingBlock(nn.Module):
    """State-conditioned monotone piecewise-linear coupling on z in (0,1).

    The transformed half uses fixed uniform x-bins and predicted positive y-bin
    heights that sum to one. This is less expressive than rational-quadratic
    splines, but the forward map, inverse map, and log determinant are exact and
    cheap. Zero output logits make the heights uniform, so the block initializes
    to identity.
    """

    def __init__(self, action_dim, cond_dim, num_bins, min_bin_height, flip=False):
        super().__init__()
        ...
        if self.num_bins < 2:
            raise ValueError("beta_flow_spline_bins must be >= 2")
        if not (0.0 <= self.min_bin_height < 1.0 / self.num_bins):
            raise ValueError("beta_flow_spline_min_bin_height must be in [0, 1 / bins)")
        ...
        self.net = nn.Sequential(
            layer_init(nn.Linear(cond_dim + left_dim, hidden_dim), std=0.5),
            nn.SiLU(),
            layer_init(nn.Linear(hidden_dim, hidden_dim), std=0.5),
            nn.SiLU(),
            layer_init(nn.Linear(hidden_dim, right_dim * self.num_bins), std=0.0),
        )
    ...
    def _heights_and_edges(self, left, cond):
        right_dim = self.split if self.flip else self.action_dim - self.split
        logits = self.net(torch.cat((cond, left), dim=-1)).view(left.shape[0], right_dim, self.num_bins)
        heights = F.softmax(logits, dim=-1)
        if self.min_bin_height > 0.0:
            heights = self.min_bin_height + (1.0 - self.num_bins * self.min_bin_height) * heights
        heights = heights / heights.sum(dim=-1, keepdim=True)
        edges = F.pad(heights.cumsum(dim=-1), (1, 0), value=0.0)
        return heights, edges

    def _gather_bin_params(self, values, heights, edges, inverse):
        x = values.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        if inverse:
            # Count completed y-bins. The index is discrete, but gradients still
            # flow through the selected bin height/edge parameters.
            bin_idx = (x.unsqueeze(-1) >= edges[..., 1:]).sum(dim=-1).clamp(max=self.num_bins - 1)
        else:
            bin_idx = torch.floor(x * self.num_bins).long().clamp(0, self.num_bins - 1)
        gather_idx = bin_idx.unsqueeze(-1)
        y_left = edges.gather(-1, gather_idx).squeeze(-1)
        height = heights.gather(-1, gather_idx).squeeze(-1)
        x_left = bin_idx.to(dtype=x.dtype) / float(self.num_bins)
        slope = height * float(self.num_bins)
        return x, x_left, y_left, slope

    def forward_with_logdet(self, z, cond):
        left, right = self._split(z)
        heights, edges = self._heights_and_edges(left, cond)
        right, x_left, y_left, slope = self._gather_bin_params(right, heights, edges, inverse=False)
        out_right = y_left + (right - x_left) * slope
        logdet = slope.clamp_min(1e-12).log().sum(dim=-1)
        return self._merge(left, out_right), logdet

    def inverse_with_logdet(self, z, cond):
        left, right = self._split(z)
        heights, edges = self._heights_and_edges(left, cond)
        right, x_left, y_left, slope = self._gather_bin_params(right, heights, edges, inverse=True)
        out_right = x_left + (right - y_left) / slope.clamp_min(1e-12)
        logdet = slope.clamp_min(1e-12).log().sum(dim=-1)
        return self._merge(left, out_right), logdet


class BetaSplineActionFlow(nn.Module):
    def __init__(self, action_dim, cond_dim, flow_steps, num_bins, min_bin_height):
        super().__init__()
        self.flow_steps = int(flow_steps)
        self.blocks = nn.ModuleList(
            MonotoneLinearSplineCouplingBlock(
                action_dim,
                cond_dim,
                num_bins,
                min_bin_height,
    ...
    def forward_with_logdet(self, z0, cond):
        z = z0
        logdet = torch.zeros(z0.shape[0], device=z0.device, dtype=z0.dtype)
        for block in self.blocks:
            z, block_logdet = block.forward_with_logdet(z, cond)
            logdet = logdet + block_logdet
        return z, logdet

    def inverse_with_logdet(self, z1, cond):
        z = z1
        logdet = torch.zeros(z1.shape[0], device=z1.device, dtype=z1.dtype)
        for i in reversed(range(self.flow_steps)):
            z, block_logdet = self.blocks[i].inverse_with_logdet(z, cond)
            logdet = logdet + block_logdet
        return z, logdet
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        # v24.17: direct bounded Beta spline-flow default, with v24 beta/diagonal controls.
        ...
        if self.actor_dist == "gaussian":
        ...
        elif self.actor_dist in ("beta", "beta_flow"):
            ...
            if self.actor_dist == "beta_flow":
                self.action_flow = BetaSplineActionFlow(
                    act_dim,
                    H,
                    args.beta_flow_steps,
                    args.beta_flow_spline_bins,
                    args.beta_flow_spline_min_bin_height,
                )
                self.beta_flow_cfm_target_concentration = float(args.beta_flow_cfm_target_concentration)
                self.beta_flow_cfm_detach_trunk = bool(args.beta_flow_cfm_detach_trunk)
        else:
            raise ValueError(f"unknown actor_dist {self.actor_dist}")

    def _beta_source_dist(self, actor_feat):
    ...
    def _flow_logprob(self, dist, z0, forward_logdet):
        return dist.log_prob(z0.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)).sum(dim=-1) - forward_logdet

    def beta_flow_cfm_loss(self, x, target_z1, advantages, weight_clip):
        if self.actor_dist != "beta_flow":
            raise RuntimeError("beta_flow_cfm_loss is only valid for beta_flow actor_dist")
        actor_feat, _ = self._trunks(x)
        cfm_feat = actor_feat.detach() if self.beta_flow_cfm_detach_trunk else actor_feat
        with torch.no_grad():
            source_dist = self._beta_source_dist(actor_feat.detach())
            z0 = source_dist.sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            target_z = target_z1.detach().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
            target_conc = torch.as_tensor(
                self.beta_flow_cfm_target_concentration,
                device=target_z.device,
                dtype=target_z.dtype,
            )
            target_alpha = 1.0 + target_conc * target_z
            target_beta = 1.0 + target_conc * (1.0 - target_z)
            sampled_target_z = Beta(target_alpha, target_beta).sample().clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        ...
        pred_z, _ = self.action_flow.forward_with_logdet(z0, cfm_feat)
        per_sample = (pred_z - sampled_target_z).square().mean(dim=-1)
        return (weights * per_sample).mean()

    def get_beta_flow_stats(self, x):
        if self.actor_dist != "beta_flow":
            raise RuntimeError("get_beta_flow_stats is only valid for beta_flow actor_dist")
        actor_feat, _ = self._trunks(x)
        dist = self._beta_source_dist(actor_feat)
        source_mean = dist.mean.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
        z1, logdet = self.action_flow.forward_with_logdet(source_mean, actor_feat)
        recon_z0, _ = self.action_flow.inverse_with_logdet(z1, actor_feat)
        return {
            "source_alpha": dist.concentration1,
            "source_beta": dist.concentration0,
            "flow_delta": z1 - source_mean,
            "flowed_mean_z": z1,
            "flow_logdet": logdet,
            "inverse_recon_error": (recon_z0 - source_mean).abs(),
        }
    ...
    def get_action_and_value(self, x, z=None):
        ...
        if self.actor_dist == "beta_flow":
            ...
            if z is None:
                ...
                z1, forward_logdet = self.action_flow.forward_with_logdet(z0, actor_feat)
                replay_z = z1
            else:
                z1 = z.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                z0, inverse_logdet = self.action_flow.inverse_with_logdet(z1, actor_feat)
                forward_logdet = -inverse_logdet
                z0 = z0.clamp(SAMPLE_EPS, 1.0 - SAMPLE_EPS)
                replay_z = z1
            action = self._z_to_action(z1)
            log_prob = self._flow_logprob(dist, z0, forward_logdet)
            entropy = -log_prob
            value_logits = self.critic_head(critic_feat)
            return action, replay_z, log_prob, entropy, value_logits
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                if args.actor_dist == "beta_flow" and args.beta_flow_cfm_coef > 0.0:
                    cfm_loss = agent.beta_flow_cfm_loss(
                        b_obs[mb_inds],
                        b_latent_zs[mb_inds],
                        mb_raw_adv,
                        args.beta_flow_cfm_weight_clip,
                    )
                    cfm_losses.append(cfm_loss.item())
                ...
                if args.separate_grad_clip:
                    ...
                    optimizer.zero_grad(set_to_none=True)
                    (pg_loss - ent_coef_eff * entropy_loss + args.beta_flow_cfm_coef * cfm_loss).backward()
                    ...
                    loss = (
                        ...
                        + args.beta_flow_cfm_coef * cfm_loss
                        + v_loss * args.vf_coef
                    )
                    optimizer.zero_grad()
                    loss.backward()
        ...
        if args.actor_dist == "beta_flow":
            writer.add_scalar("losses/beta_flow_cfm_loss", float(np.mean(cfm_losses)) if cfm_losses else 0.0, global_step)
        ...
        if args.actor_dist == "beta_flow":
            with torch.no_grad():
                flow_stats = agent.get_beta_flow_stats(b_obs)
                flow_delta = flow_stats["flow_delta"]
                writer.add_scalar("debug/beta_flow_delta_abs_mean", flow_delta.abs().mean().item(), global_step)
                writer.add_scalar("debug/beta_flow_delta_abs_max", flow_delta.abs().max().item(), global_step)
                writer.add_scalar("debug/beta_flow_source_alpha_mean", flow_stats["source_alpha"].mean().item(), global_step)
                writer.add_scalar("debug/beta_flow_source_beta_mean", flow_stats["source_beta"].mean().item(), global_step)
                writer.add_scalar("debug/beta_flow_flowed_mean_z_std", flow_stats["flowed_mean_z"].std().item(), global_step)
                writer.add_scalar("debug/beta_spline_logdet_mean", flow_stats["flow_logdet"].mean().item(), global_step)
                writer.add_scalar(
                    "debug/beta_spline_inverse_recon_error",
                    flow_stats["inverse_recon_error"].max().item(),
                    global_step,
                )
                replay_z = b_latent_zs
                writer.add_scalar("debug/beta_spline_replay_z_abs_mean", replay_z.abs().mean().item(), global_step)
                writer.add_scalar("debug/beta_spline_replay_z_abs_max", replay_z.abs().max().item(), global_step)
                boundary_frac = ((replay_z <= SAMPLE_EPS) | (replay_z >= 1.0 - SAMPLE_EPS)).float().mean().item()
                writer.add_scalar(
                    "debug/beta_spline_boundary_frac",
                    boundary_frac,
                    global_step,
                )
                writer.add_scalar(
                    "debug/beta_flow_replay_z_boundary_frac",
                    boundary_frac,
                    global_step,
                )
