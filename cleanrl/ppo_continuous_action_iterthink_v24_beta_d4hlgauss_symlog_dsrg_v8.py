# PPO + IterThink v24 beta d4-HL-Gauss symlog + Dynamic Sparse Recurrent Graph (DSRG) v8.
#
# WHAT CHANGES vs DSRG v7: the per-tick CARRY becomes a PURE ADDITIVE RESIDUAL (and
# NOTHING else). v7's tick update was an LSTM-style gated carry `norm((1-f)*hhat + f*h)`;
# the sigmoid f can SATURATE (f->1 kills the update gradient = dead compute, f->0 kills
# the carry gradient), the same magnitude-saturation pathology v6 removed from routing.
# v8 uses `norm(h + hhat)` -- the iterthink-validated additive residual (`x = x + delta`,
# no gate). The `+h` is an UNCONDITIONAL identity highway, so gradient flows straight back
# across all T ticks to the obs-injected perceptrons (better credit assignment, especially
# at depth), and no gate can saturate to zero out the update or carry path. The single
# post-add RMSNorm holds h at unit scale for the next tick's routing dot-products and stays
# well-conditioned via the always-nonzero +h floor. f_proj is removed; everything else
# (competitive routing, obs-conditional readout, load-balance, triple-backward) is v7.
#
# --- v7 lineage note ---
