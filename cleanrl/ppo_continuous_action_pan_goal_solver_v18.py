# Pan Goal Solver v18: v17 with occupancy InfoNCE shaping the belief.
#
# v17 measurement at 3M steps: the occupancy InfoNCE sits ~1 nat above
# chance (5.1 vs ln(512)=6.24) after 3M steps of training — because it only
# trains follower-side readout heads on a DETACHED belief, it can merely
# read out whatever future-information next-step prediction happens to leave
# in the temporal memory, and that is close to none. The belief is the
# conditioning substrate of the action head, so this is the weakest link of
# the whole goal-conditioning pathway.
# v18 change: the occupancy branch of the follower loss backpropagates into
# the world model (belief side attached; goal side stays detached so the
# contrastive task cannot restructure the goal space). The temporal belief
# is now trained to encode which future goals are reachable from here —
# reward-free, contract-legal, and the most Pan-1-flavored objective in the
# family. The hindsight action-MSE branch stays fully detached from the
# world model. Everything else is v17: pure LeJEPA world model (no EMA,
# attached online prediction target, token-space SIGReg), gait-frequency
# explorer, additive velocity aspiration, nonlinear follower head.
# There is no PPO, action critic, policy gradient, search, MPC, or rollout.
# Hypothesis: a belief shaped by reachability gives the action head a
# substrate that actually distinguishes futures, turning the intermittent
# commanded-goal ignitions seen in v16/v17 into consolidated running.
...
class DirectGoalFollower(nn.Module):
    ...
    def __init__(self, observation_tokens, belief_dim, goal_dim, act_dim, hidden):
        ...
        # Nonlinear head over (belief, bounded goal delta). The v14 bilinear
        # servo (belief-conditioned Jacobian applied to the goal direction) is
        # linear in the goal delta, which cannot express the sign-nonlinear
        # corrections goal tracking needs (measured 50% frozen-goal progress).
    ...
    def action(self, belief, current_goal, desired_goal):
        ...
        return torch.tanh(self.action_net(torch.cat([belief, direction], dim=-1)))
...
class PanGoalSolver(nn.Module):
    ...
    def follower_state(self, obs_history, incoming_action_history):
        ...
        online_tokens, goal, world_belief, memory = (
            outputs[0], outputs[2], outputs[5], outputs[6],
        )
        # TemporalBelief keeps feature streams separate, so the follower readout
        # excludes its action slot. One online pass supplies both the trained
        # belief and the shared P/R/follower goal coordinates.
...
def train_follower_step(agent, optimizer, world_optimizer, batch, args, device):
    obs = as_tensor(batch["obs"], device)
    incoming = as_tensor(batch["incoming"], device)
    next_obs = as_tensor(batch["next_obs"], device)
    ...
    # The world-model forward runs attached: the occupancy InfoNCE below
    # backpropagates through the occupancy belief into the temporal stack,
    # training the belief to encode reachable futures (reward-free). The
    # hindsight action branch and the goal side stay detached.
    ...
    occupancy_belief = agent.follower.read_belief(occupancy_memory[:, :-1])
    ...
    world_grad_norm = nn.utils.clip_grad_norm_(
        agent.world_model.parameters(), args.max_grad_norm
    )
    optimizer.step()
    ...
    return {
        ...
        "follower/occupancy_world_grad_norm": float(world_grad_norm),
    }
