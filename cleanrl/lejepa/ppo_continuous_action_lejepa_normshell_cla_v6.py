# PPO + LeJEPA normalized geometric successor shells + counterfactual baseline v6.
#
# Parent v5 reached 6,648 at 2.8M without GAE. V6 changes only its state baseline: once a
# held-out observation-dynamics model is reliable, it averages one-step successor-shell
# targets over counterfactual actions sampled from the current policy. The baseline remains
# state-only, while the executed action is compared against that policy marginal over the
# same fixed 16-transition innovation. The normalized shells avoid the deterministic-
# coordinate scale explosion seen in sfinnovation_v4.
#
...
# contracted with the lagged immediate-
# reward covector only at the PPO boundary. The accumulated reward-probe residual is added
# exactly, so imperfect reward decoding cannot silently change the control objective.
...
class Args:
    ...
    cla_m: int = 4
    cla_beta: float = 1.0
    cla_dyn_r2_gate: float = 0.85
    cla_dyn_r2_full: float = 0.95
    dyn_hidden: int = 256
    dyn_lr: float = 3e-4
...
class ObsDynamics(nn.Module):
    """Action-conditioned one-step observation delta and continuation model."""
    ...
    def forward(self, state, action):
        hidden = self.body(torch.cat([state, action], dim=-1))
        return self.delta_head(hidden), self.cont_head(hidden)[..., 0]
...
def counterfactual_shells(
    ...
    """Policy-marginalized successor-shell baseline, independent of behavior actions."""
    shell_outputs, standard_errors = [], []
    for start in range(0, states.shape[0], chunk):
        state = states[start : start + chunk]
        embedding = embeddings[start : start + chunk]
        agent_obs = torch.cat([state, embedding], dim=-1)
        actor_features, _ = agent._trunks(agent_obs)
        distribution, to_action, _ = agent._actor_dist(actor_features)
        shell_sum = None
        centered_value_sum = torch.zeros(state.shape[0], device=state.device)
        centered_value_sq = torch.zeros_like(centered_value_sum)
        first_value = None
        for _ in range(samples):
            latent_action = distribution.sample()
            if agent.actor_dist == "beta":
                latent_action = latent_action.clamp(
                    SAMPLE_EPS, 1.0 - SAMPLE_EPS
                )
            action = to_action(latent_action)
            state_delta, continuation_logit = dynamics(state, action)
            predicted_state = state + state_delta
            predicted_embedding = encode_control(predicted_state)
            predicted_agent_obs = torch.cat(
                [predicted_state, predicted_embedding], dim=-1
            )
            next_shells = agent.get_value(predicted_agent_obs)
            continuation = torch.sigmoid(continuation_logit)
            shells = build_normalized_shell_targets(
                phi_features(embedding, state, action),
                next_shells,
                continuation,
                betas,
                widths,
            )
            shell_sum = shells if shell_sum is None else shell_sum + shells
            value = psi_raw(shells) @ reward_weights
            first_value = value if first_value is None else first_value
            centered = value - first_value
            centered_value_sum += centered
            centered_value_sq += centered.square()
        shell_outputs.append(shell_sum / samples)
        variance = (
            centered_value_sq
            - centered_value_sum.square() / samples
        ).clamp_min(0.0) / max(samples - 1, 1)
        standard_errors.append((variance / samples).sqrt())
    return torch.cat(shell_outputs), torch.cat(standard_errors)
...
if __name__ == "__main__":
    ...
    if args.cla_m < 0:
        raise ValueError("cla_m must be nonnegative")
    if not 0.0 <= args.cla_beta <= 1.0:
        raise ValueError("cla_beta must be in [0, 1]")
    if args.cla_dyn_r2_full <= args.cla_dyn_r2_gate:
        raise ValueError("cla_dyn_r2_full must exceed cla_dyn_r2_gate")
    ...
    dynamics = ObsDynamics(obs_dim, act_dim_sf, args.dyn_hidden).to(device)
    dynamics_optimizer = optim.Adam(dynamics.parameters(), lr=args.dyn_lr)
    dynamics_r2 = 0.0
    dynamics_mse = 0.0
    dynamics_continuation_accuracy = 0.0
    ...
    td_betas = torch.as_tensor(TD_BETAS, device=device, dtype=torch.float32)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            beta_eff = args.cla_beta * min(
                max(
                    (dynamics_r2 - args.cla_dyn_r2_gate)
                    / max(
                        args.cla_dyn_r2_full - args.cla_dyn_r2_gate,
                        1e-8,
                    ),
                    0.0,
                ),
            ...
            cla_active = args.cla_m > 0 and beta_eff > 0.0
            if cla_active:
                flat_embeddings = emb_buf.reshape(-1, args.emb_dim)
                counterfactual_cur, counterfactual_se = counterfactual_shells(
                    agent,
                    dynamics,
                    encode_control,
                    psi_raw,
                ...
                counterfactual_cur = counterfactual_cur.reshape_as(
                    successor_cur
                )
                counterfactual_next = torch.empty_like(successor_next)
                counterfactual_next[:-1] = counterfactual_cur[1:]
                needs_recompute = transition_boundaries > 0.5
                needs_recompute[-1] = True
                recompute_indices = (
                    needs_recompute.reshape(-1).nonzero(as_tuple=True)[0]
                )
                recompute_states = flat_next_obs[recompute_indices]
                recompute_embeddings = next_emb_buf.reshape(
                    -1, args.emb_dim
                )[recompute_indices]
                recomputed, _ = counterfactual_shells(
                    agent,
                    dynamics,
                    encode_control,
                    psi_raw,
                ...
                counterfactual_next.view(
                    -1, args.critic_mtp_horizon, sf_dim
                )[recompute_indices] = recomputed
                cla_baseline_gap = float(
                    (
                        psi_raw(counterfactual_cur - successor_cur)
                        @ w_r
                    ).abs().mean()
                ) * beta_eff
                cla_standard_error = (
                    float(counterfactual_se.mean()) * beta_eff
                )
                successor_cur = (
                    (1.0 - beta_eff) * successor_cur
                    + beta_eff * counterfactual_cur
                )
                successor_next = (
                    (1.0 - beta_eff) * successor_next
                    + beta_eff * counterfactual_next
                )
                values.copy_(psi_raw(successor_cur) @ w_r)
            else:
                cla_baseline_gap = 0.0
                cla_standard_error = 0.0
            ...
            # The lagged reward probe is the same covector used to produce rollout values.
            # Its error is corrected exactly over the same fixed horizon, so policy credit
            # equals an ordinary fixed-n reward TD residual even when the probe is poor.
            ...
            advantages = adv_vector + residual_trace
        ...
        # Fit the counterfactual model after consuming this rollout. Its held-out
        # per-coordinate delta R2 gates baseline use on the next iteration.
        with torch.no_grad():
            dynamics_states = obs.reshape(flat_obs_shape)
            dynamics_actions = actions.reshape(-1, act_dim_sf)
            dynamics_next = next_obses.reshape(flat_obs_shape)
            dynamics_delta = dynamics_next - dynamics_states
            dynamics_terminated = transition_terminations.reshape(-1)
            dynamics_keep = transition_valids.reshape(-1) > 0.5
            dynamics_states = dynamics_states[dynamics_keep]
            dynamics_actions = dynamics_actions[dynamics_keep]
            dynamics_delta = dynamics_delta[dynamics_keep]
            dynamics_terminated = dynamics_terminated[dynamics_keep]
        dynamics_count = dynamics_states.shape[0]
        validation_count = dynamics_count // 10
        if dynamics_count - validation_count >= args.dyn_batch:
            split = torch.randperm(dynamics_count, device=device)
            validation_indices = split[:validation_count]
            training_indices = split[validation_count:]
            validation_states = dynamics_states[validation_indices]
            validation_actions = dynamics_actions[validation_indices]
            validation_delta = dynamics_delta[validation_indices]
            validation_terminated = dynamics_terminated[validation_indices]
            dynamics_states = dynamics_states[training_indices]
            dynamics_actions = dynamics_actions[training_indices]
            dynamics_delta = dynamics_delta[training_indices]
            dynamics_terminated = dynamics_terminated[training_indices]
            training_count = dynamics_states.shape[0]
            steps_per_epoch = max(1, training_count // args.dyn_batch)
            epochs = max(
                1, round(args.dyn_steps_target / steps_per_epoch)
            )
            for _ in range(epochs):
                permutation = torch.randperm(training_count, device=device)
                for start in range(
                    0, training_count - args.dyn_batch + 1, args.dyn_batch
                ):
                    indices = permutation[start : start + args.dyn_batch]
                    predicted_delta, continuation_logit = dynamics(
                        dynamics_states[indices],
                        dynamics_actions[indices],
                    )
                    dynamics_loss = F.mse_loss(
                        predicted_delta, dynamics_delta[indices]
                    ) + F.binary_cross_entropy_with_logits(
                        continuation_logit,
                        1.0 - dynamics_terminated[indices],
                    )
            ...
            with torch.no_grad():
                predicted_delta, continuation_logit = dynamics(
                    validation_states, validation_actions
                )
                residual = predicted_delta - validation_delta
                dynamics_r2 = float(
                    (
                        1.0
                        - residual.square().mean(0)
                        / validation_delta.var(0).clamp_min(1e-8)
                    )
                    .clamp(-1.0, 1.0)
                    .mean()
                )
                dynamics_mse = float(residual.square().mean())
                dynamics_continuation_accuracy = float(
                    (
                        (continuation_logit > 0).float()
                        == (1.0 - validation_terminated)
                    )
                    .float()
                    .mean()
                )
