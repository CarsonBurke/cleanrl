# ============================================================================
# LeJEPA + learned embedding advantage + policy gradient v1
#
# NOT AWR. NOT static EDBR / cos / pure ridge-as-advantage.
#
# Learned stack:
#   1) JEPA: e=f(s), ê'=g(e,a)  (SSL: next-e MSE + SIGReg)
#   2) Learned V_ψ(e): MLP on embeddings, fit to TD(λ) returns from real transitions
#   3) Embedding advantage (model-based, g load-bearing):
#        A_t = r_t + γ V(sg(g(e_t, a_t))) − V(e_t)
#      optional mix with real-next residual r + γ V(e') − V(e)
#   4) Policy: standard score PG  −A log π   (or PPO clip on A) + KL trust / entropy
#      — NOT exp(A) weighted BC / AWR
#
# No Q-network, no max_a. Advantage is a learned TD residual in JEPA latent space;
# policy is ordinary on-policy PG with that A.
# ============================================================================
import os
import random
import time
...
class Args:
    ...
    norm_adv: bool = True
    use_ratio_clip: bool = False  # False: score PG; True: PPO clip on emb advantages
    clip_coef: float = 0.2
...
if __name__ == "__main__":
    ...
    print(
        f"[lejepa_embadv_pg_v1] learned V(e) + model emb A + score/PPO PG (NO AWR)  "
        f"adv_real_mix={args.adv_real_mix} warmup={args.ssl_warmup_iters} "
        f"ratio_clip={args.use_ratio_clip}"
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # ---- Policy gradient on embedding advantages (NOT AWR) -------------------------
            ...
            for epoch in range(args.actor_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    if args.use_ratio_clip:
                        ...
                        surr1 = -mb_adv * ratio
                        surr2 = -mb_adv * torch.clamp(
                            ratio, 1 - args.clip_coef, 1 + clip_hi
                        )
                        pg_loss = torch.max(surr1, surr2).mean()
                    else:
                        # On-policy score function with embedding advantage
        ...
        phase = "ssl_warmup" if not actor_active else ("ppo" if args.use_ratio_clip else "score_pg")
        print(
            ...
            f"ssl_pred={ssl_pred:.4f} ent={ent_mean:.3f}"
        )
