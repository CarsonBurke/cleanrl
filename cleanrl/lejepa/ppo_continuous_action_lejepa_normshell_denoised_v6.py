# PPO + LeJEPA normalized geometric successor shells, denoised credit v6.
#
# Parent v5 reached 6,648 at 2.8M without GAE. V6 isolates the mechanism that its exact
# reward-residual correction removed: the actor defaults to the reward covector projected
# through the 16-step successor-embedding innovation, intentionally denoising reward
# components that phi cannot reconstruct. The matched control restores the exact residual.
#
...
# contracted with the lagged immediate-reward covector only at the PPO boundary. By
# default, reward components the embedding cannot reconstruct are intentionally omitted;
# --reward-residual-correction restores their exact fixed-16 scalar contribution.
...
class Args:
    ...
    reward_residual_correction: bool = False
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        with torch.no_grad():
            ...
            corrected_advantage = adv_vector + residual_trace
            correction_active = (
                args.reward_residual_correction or not w_r_solved
            )
            advantages = (
                corrected_advantage if correction_active else adv_vector
            )
            policy_adv = advantages
            returns = advantages + values
            _embedded = adv_vector.reshape(-1)
            _corrected = corrected_advantage.reshape(-1)
