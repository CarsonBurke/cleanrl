...
from cleanrl.pc.ppo_continuous_action_pc_momentum_streaming_advnorm_v6 import (
    Agent,
    Args,
    AugmentedAdam,
    LocalPredictor,
...
def test_streaming_advantage_moments_are_lagged_and_track_lambda_scale():
    moments = StreamingAdvantageMoments(
        torch.device("cpu"), num_envs=2, decay=0.5, minimum=0.1, stats_winsor=0.0
    )
    mean, std = moments.assign_and_update(
        torch.tensor([1.0, 3.0]), torch.tensor([False, False]), trace_decay=0.5
    )
    torch.testing.assert_close(mean, torch.tensor(0.0))
    torch.testing.assert_close(std, torch.tensor(1.0))
    torch.testing.assert_close(moments.proxy, torch.tensor([1.0, 3.0]))
    torch.testing.assert_close(moments.mean, torch.tensor(2.0))
    torch.testing.assert_close(moments.variance, torch.tensor(1.0))

    mean, std = moments.assign_and_update(
        torch.tensor([10.0, 12.0]), torch.tensor([False, False]), trace_decay=0.5
    )
    # The current transition cannot affect the moments assigned to its score.
    torch.testing.assert_close(mean, torch.tensor(2.0))
    torch.testing.assert_close(std, torch.tensor(1.0))
    torch.testing.assert_close(moments.proxy, torch.tensor([10.5, 13.5]))
    torch.testing.assert_close(moments.mean, torch.tensor(7.0))
    torch.testing.assert_close(moments.variance, torch.tensor(26.625))


def test_streaming_advantage_proxy_includes_terminal_sample_then_resets():
    moments = StreamingAdvantageMoments(
        torch.device("cpu"), num_envs=2, decay=0.9, minimum=0.1, stats_winsor=0.0
    )
    moments.assign_and_update(
        torch.tensor([1.0, 2.0]), torch.tensor([False, True]), trace_decay=0.9
    )
    torch.testing.assert_close(moments.mean, torch.tensor(1.5))
    torch.testing.assert_close(moments.variance, torch.tensor(0.25))
    torch.testing.assert_close(moments.proxy, torch.tensor([1.0, 0.0]))


def test_streaming_advantage_moments_ignore_nonfinite_batches():
    moments = StreamingAdvantageMoments(
        torch.device("cpu"), num_envs=2, decay=0.9, minimum=0.1, stats_winsor=20.0
    )
    moments.assign_and_update(
        torch.tensor([1.0, 3.0]), torch.tensor([False, False]), trace_decay=0.9
    )
...
def test_two_trace_direction_equals_offline_normalized_lambda_return():
    trace_decay = 0.7
    scores = torch.tensor([[1.0, -0.5], [0.2, 2.0], [-1.0, 0.3], [0.7, -0.2]])
    deltas = torch.tensor([0.4, -1.2, 0.8, 2.0])
    assigned_means = torch.tensor([0.1, -0.4, 0.7, 0.3])
    assigned_stds = torch.tensor([0.5, 2.0, 1.5, 0.8])
    reward_trace = TraceBank([torch.empty(1, 2)])
    baseline_trace = TraceBank([torch.empty(1, 2)])
    online = torch.zeros(2)

    for step in range(len(deltas)):
        score = scores[step].unsqueeze(0)
        reward_trace.accumulate([score / assigned_stds[step]], trace_decay)
        baseline_trace.accumulate(
            [score * (assigned_means[step] / assigned_stds[step])], trace_decay
        )
        # The final sample is a training cutoff, not an environment termination.
        terminal = torch.tensor([step == len(deltas) - 1])
        online += normalized_actor_directions(
            reward_trace,
            baseline_trace,
            deltas[step].view(1),
            terminal,
    ...
    offline = torch.zeros(2)
    for score_step in range(len(deltas)):
        lambda_return = sum(
            trace_decay ** (future_step - score_step) * deltas[future_step]
            for future_step in range(score_step, len(deltas))
        )
        offline += scores[score_step] * (
            (lambda_return - assigned_means[score_step]) / assigned_stds[score_step]
        )
    torch.testing.assert_close(online, offline, rtol=1e-6, atol=1e-6)


def test_two_trace_direction_centers_once_at_each_episode_boundary():
    trace_decay = 0.6
    scores = torch.tensor([1.0, 2.0, -1.0, 0.5, 3.0])
    deltas = torch.tensor([0.2, 0.7, -0.4, 1.1, 0.3])
    means = torch.tensor([0.5, -0.2, 0.1, 0.4, -0.3])
    stds = torch.tensor([1.0, 0.5, 2.0, 0.8, 1.5])
    done = torch.tensor([False, True, False, False, True])
    reward_trace = TraceBank([torch.empty(1, 1)])
    baseline_trace = TraceBank([torch.empty(1, 1)])
    online = torch.zeros(())

    for step in range(len(deltas)):
        score = scores[step].view(1, 1)
        reward_trace.accumulate([score / stds[step]], trace_decay)
        baseline_trace.accumulate([score * means[step] / stds[step]], trace_decay)
        online += normalized_actor_directions(
            reward_trace,
            baseline_trace,
            deltas[step].view(1),
        ...
        reward_trace.reset(done[step].view(1))
        baseline_trace.reset(done[step].view(1))

    offline = torch.zeros(())
    episode_start = 0
    for episode_end in torch.nonzero(done).flatten().tolist():
        for score_step in range(episode_start, episode_end + 1):
            ...
            offline += scores[score_step] * (
                (lambda_return - means[score_step]) / stds[score_step]
            )
