...
SCRIPT = (
    ...
    / "ppo_continuous_action_embopt_freegoal_frozen_chart_v3.py"
)
SPEC = importlib.util.spec_from_file_location("freegoal_frozen_chart_v3", SCRIPT)
...
def test_default_warmup_and_representation_boundary_are_exact():
    args = MODULE.Args()
    assert args.warmup_steps == 262_144
    assert not MODULE.controlled_learning_started(262_143, args.warmup_steps)
    assert MODULE.controlled_learning_started(262_144, args.warmup_steps)
    assert MODULE.should_update_representation(False)
    assert not MODULE.should_update_representation(True)
    assert not MODULE.warmup_boundary_reached(
        False, 262_143, args.warmup_steps
    )
    assert MODULE.warmup_boundary_reached(
        False, 262_144, args.warmup_steps
    )
    assert not MODULE.warmup_boundary_reached(
        True, 262_144, args.warmup_steps
    )


def test_frozen_world_retains_action_jacobian_without_parameter_gradients():
    args = small_args()
    world = MODULE.CausalHistoryWorldModel(
        obs_dim=3,
        action_dim=2,
    ...
    MODULE.freeze_world_model(world)
    belief = torch.randn(5, args.latent_dim)
    action = torch.randn(5, 2, requires_grad=True)
    world.predict_next(belief, action).square().mean().backward()
...
def test_route_diagnostics_are_observational_and_finite():
    args = small_args()
    ...
    plain_loss, _, plain_actions = MODULE.factual_st_actor_loss(
        plain_agent,
        beliefs,
        next_beliefs,
        action_excitation,
    ...
    expected = {
        "goal_return_gradient_norm",
        "policy_return_gradient_norm",
        "policy_delivery_gradient_norm",
        "policy_action_cost_gradient_norm",
        "policy_uncertainty_gradient_norm",
        "policy_return_delivery_gradient_cosine",
    ...
    assert metrics["goal_return_gradient_norm"] > 0
    assert metrics["policy_return_gradient_norm"] > 0
    assert metrics["policy_delivery_gradient_norm"] > 0
    assert metrics["policy_action_cost_gradient_norm"] > 0
    assert metrics["policy_uncertainty_gradient_norm"] > 0
...
def test_source_excludes_forbidden_mechanisms():
    ...
    assert '"model/chart_freeze_transition", 1.0, global_step' in source
    assert "warmup_boundary_reached(" in source
    assert "should_update_representation(controlled_iteration)" in source
