...
from cleanrl.ppo_continuous_action_residual_stiglu_ngpt_layerscale_v8 import (
...
def test_compiled_normalized_policy_matches_host_after_projected_adam_updates():
    ...
    for _ in range(3):
        with torch.no_grad():
            ...
            agent.actor.layer_scale.add_(torch.linspace(-0.2, 0.3, 64, device="cuda"))
        mirror.refresh()
        with torch.no_grad():
...
def test_zero_layerscale_preserves_skip_and_nonzero_scale_changes_direction():
    configure_runtime(matmul_precision="highest", allow_tf32=False)
    torch.manual_seed(1)
    trunk = ResidualMLP(17, 64, output_std=1.0).cuda()
    x = torch.randn(16, 17, device="cuda")
    with torch.no_grad():
        trunk.head[0].weight.copy_(torch.eye(64, device="cuda"))
        trunk.head[0].bias.zero_()
        trunk.layer_scale.zero_()
        expected = 8.0 * torch.nn.functional.normalize(trunk.first(x), dim=-1)
        torch.testing.assert_close(trunk(x), expected)
        trunk.second[0].down.weight.neg_()
        torch.testing.assert_close(trunk(x), expected)
        trunk.layer_scale.fill_(0.1)
        small_residual = trunk(x)
        trunk.layer_scale.fill_(1.0)
        full_residual = trunk(x)
        # Normalizing AFTER the gain on the branch would erase this difference.
        assert not torch.allclose(small_residual, full_residual)
