...
SCRIPT = (
    ...
    / "ppo_continuous_action_spf_hybrid_robustprogress_v18.py"
)
spec = importlib.util.spec_from_file_location(
    "ppo_continuous_action_spf_hybrid_robustprogress_v18", SCRIPT
)
...
def device():
    assert torch.cuda.is_available(), "robust-progress tests require CUDA"
...
def test_repeated_predictable_surprise_self_anneals(device):
    ...
    assert 0 <= final_bonus.item() < 1e-4 * first_bonus.item()
