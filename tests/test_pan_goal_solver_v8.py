...
from cleanrl.pan_goal_solver.ppo_continuous_action_pan_goal_solver_v8 import (
    Args,
    DirectGoalFollower,
    PanGoalSolver,
    VectorReplayBuffer,
...
def test_each_action_recomputes_one_goal_and_calls_one_follower():
    ...
    original_proposer = agent.proposer.forward
    ...
    def counted_proposer(*args, **kwargs):
        nonlocal proposer_calls
        proposer_calls += 1
        return original_proposer(*args, **kwargs)

    def forbidden(*_args, **_kwargs):
        raise AssertionError("training-only module was called during action inference")

    agent.proposer.forward = counted_proposer
    ...
    with torch.no_grad():
        action, current_goal, goal = agent.act(
            torch.randn(4, 8, 17), torch.randn(4, 8, 6), torch.tensor([25.0])
        )
...
def test_one_step_goal_descent_updates_only_the_direct_follower():
    args = _args()
    args.action_coef = 0.0
    args.occupancy_coef = 0.0
    args.goal_descent_coef = 1.0
    agent = _agent()
    world, follower, goal = agent.parameter_groups()
