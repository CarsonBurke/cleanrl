...
SCRIPT = (
    ...
    / "cleanrl"
    / "encoder-value"
    / "ppo_continuous_action_sf_koopman_resolvent_v1.py"
)
SPEC = importlib.util.spec_from_file_location("sf_koopman_resolvent_v1", SCRIPT)
...
def test_full_degree_two_beta_basis_is_orthonormal():
    ...
    torch.testing.assert_close(
        basis.T @ basis / basis.shape[0], torch.eye(5), atol=3e-2, rtol=0.0
    )
...
def test_action_loss_isolated_from_encoder_and_operator_snapshot():
    ...
    prediction = agent.action_advantage_reasons(latent, basis)
