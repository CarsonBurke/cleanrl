...
SCRIPT = Path(__file__).parents[1] / "cleanrl" / "delightful" / "qcritic" / "ppo_continuous_action_delightful_qcritic_rewardema_v22.py"
SPEC = importlib.util.spec_from_file_location("delightful_qcritic_rewardema_v22", SCRIPT)
...
def test_v22_uses_current_ema_scaled_raw_rewards_and_no_retrace():
    ...
    assert "baseline_actions = actor.deterministic(b_obs)" in source
...
def test_v22_uses_256_fresh_actor_samples_and_preserves_replay_ratio():
    args = MODULE.Args()
...
def test_policy_scale_bounds_and_squashed_density():
    ...
    assert torch.isfinite(action_logprob).all()
    assert torch.isfinite(gaussian_logprob).all()
    assert not torch.allclose(action_logprob, gaussian_logprob)
