"""Tests for v34's successor-feature action Jacobian.
...
SRC = (
    ...
    / "cleanrl"
    / "encoder-value"
    / "ppo_continuous_action_sf_jacobian_outcome_v34.py"
)

_spec = importlib.util.spec_from_file_location("v34", SRC)
...
def test_head_is_identically_zero_at_init_so_iteration_one_is_analytic_only():
    head = v34.ActionSuccessorHead(64, SF_DIM, ACT).to(DEV)
...
def test_closed_form_head_jacobian_matches_autograd():
    torch.manual_seed(0)
    head = v34.ActionSuccessorHead(64, SF_DIM, ACT).to(DEV).double()
    _randomize(head)
...
def test_analytic_phi_jacobian_matches_autograd():
    ...
    for i in range(3):
        auto = torch.autograd.functional.jacobian(
            lambda a: (v34.phi_features(emb[i], obs[i], a) * w).sum(), action[i]
        )
...
def test_phi_has_no_action_dependence_outside_the_two_action_blocks():
    ...
    p1 = v34.phi_features(emb, obs, a1)
    p2 = v34.phi_features(emb, obs, a2)
...
def test_reward_probe_recovers_the_quadratic_control_cost_from_the_aa_block():
    ...
    phi = v34.phi_features(emb, obs, action)
    ...
    w_r = v34.solve_reward_probe(phi, reward, 1e-6)
    ...
    assert v34.ev_score(phi @ w_r, reward) > 0.999
...
def _agent(dist_name):
    ...
    return v34.Agent(_Envs(), args).to(DEV), args
...
def test_perdim_loss_reproduces_the_scalar_ppo_GRADIENT_at_the_start_of_an_epoch():
    ...
    ratio_d = (lp - old).exp()
    loss, adv_dims, _ = v34.perdim_policy_loss(
        adv,
...
def test_perdim_loss_gradient_is_sensitive_to_the_sign_of_the_shift():
    ...
    adv = torch.randn(n, device=DEV, dtype=torch.float64)
    shift = v34.attribution_shift(
        torch.randn(n, ACT, device=DEV, dtype=torch.float64), 1.0, 3.0
    )

    def g(sign):
        loss, _, _ = v34.perdim_policy_loss(
            adv, sign * shift, 0.5, ratio_d, ratio_d.prod(-1), 0.2, 0.28, "gate"
        )
...
def test_shift_is_exactly_zero_sum_so_mean_credit_is_the_scalar_advantage():
    ...
    for omega in (0.0, 0.25, 1.0, 25.0):
        shift = v34.attribution_shift(attrib, 1.0, 3.0)
        assert shift.sum(-1).abs().max() < 1e-12
        _, adv_dims, _ = v34.perdim_policy_loss(adv, shift, omega, ratio_d, ratio_d.prod(-1), 0.2, 0.28, "gate")
...
def test_shrinkage_disables_the_channel_when_the_attribution_predicts_nothing():
    ...
    attrib = torch.randn(4096, ACT, device=DEV, dtype=torch.float64)
    assert torch.equal(
        v34.attribution_shift(attrib, 0.0, 3.0),
        torch.zeros_like(attrib),
    )
    half = v34.attribution_shift(attrib, 0.5, 3.0)
    full = v34.attribution_shift(attrib, 1.0, 3.0)
...
def test_shift_is_winsorized_without_losing_the_zero_sum_property():
    ...
    clamped = v34.attribution_shift(attrib, 1.0, 3.0)
    unclamped = v34.attribution_shift(attrib, 1.0, 1e9)
...
def test_clip_match_modes_apply_the_bounds_they_advertise():
    ...
    for mode, root in [("joint", 1.0), ("linear", float(ACT)), ("sqrt", ACT**0.5)]:
        hi = hi_j ** (1.0 / root)
        loss, _, frac = v34.perdim_policy_loss(adv, zero, 0.0, big, big.prod(-1), 0.2, 0.28, mode)
...
def test_frequency_matched_clip_binds_near_v9s_rate_where_the_product_match_does_not():
    ...
    for mode in ("linear", "sqrt"):
        _, _, frac = v34.perdim_policy_loss(
            adv, zero, 0.0, per_dim.exp(), per_dim.exp().prod(-1), 0.2, 0.28, mode
        )
...
def _improve(j_emb, g, ridge, kappa, act_std):
    """Exactly v34's channel-B arithmetic."""
...
def test_head_recovers_a_known_quadratic_action_map():
    torch.manual_seed(0)
    head = v34.ActionSuccessorHead(16, 8, ACT).to(DEV)
    ...
    for _ in range(600):
        ...
        opt.step()
    assert v34.ev_score(head(feat, action).reshape(-1), target.reshape(-1)) > 0.95
...
def test_action_successor_head_is_not_in_the_ppo_optimizer_or_clip_groups():
    ...
    head = v34.ActionSuccessorHead(args.hidden, SF_DIM, ACT).to(DEV)
...
# ------------------------------------------------------- the "gate" trust region (v34)


def _toy(n=8192, seed=0):
    torch.manual_seed(seed)
    mu = torch.zeros(ACT, device=DEV, dtype=torch.float64, requires_grad=True)
    a = torch.randn(n, ACT, device=DEV, dtype=torch.float64)
    lp = torch.distributions.Normal(mu, 1.0).log_prob(a)
    return mu, lp, torch.randn(n, device=DEV, dtype=torch.float64)


def test_gate_reproduces_v9s_gradient_exactly_at_the_start_of_an_epoch():
    mu, lp, adv = _toy()
    old = lp.detach()
    r = (lp.sum(1) - old.sum(1)).exp()
    v9 = torch.max(-adv * r, -adv * torch.clamp(r, 0.8, 1.28)).mean()
    (g9,) = torch.autograd.grad(v9, mu, retain_graph=True)
    rd = (lp - old).exp()
    loss, _, _ = v34.perdim_policy_loss(
        adv, torch.zeros_like(rd), 0.0, rd, rd.prod(-1), 0.2, 0.28, "gate"
    )
    (g34,) = torch.autograd.grad(loss, mu)
    assert torch.allclose(g9, g34, rtol=1e-9), (g9, g34)


def test_gate_fires_on_exactly_the_samples_v9_clips():
    # v9's max picks the CLIPPED branch iff (A>0 and r>1+hi) or (A<0 and r<1-lo).
    torch.manual_seed(0)
    n = 100000
    adv = torch.randn(n, device=DEV, dtype=torch.float64)
    r = (torch.randn(n, device=DEV, dtype=torch.float64) * 0.35).exp()
    v9_picks_clipped = (-adv * torch.clamp(r, 0.8, 1.28)) > (-adv * r)
    gate = ((adv > 0) & (r > 1.28)) | ((adv < 0) & (r < 0.8))
    assert torch.equal(v9_picks_clipped, gate)
    assert 0.01 < float(gate.float().mean()) < 0.30      # the test is not vacuous


def test_clipped_samples_contribute_no_gradient_under_the_gate():
    mu, lp, adv = _toy(n=4096)
    old = (lp.detach() - 1.5)                     # force every joint ratio far above 1.28
    rd = (lp - old).exp()
    r = rd.prod(-1)
    assert float((r > 1.28).float().mean()) == 1.0
    pos = adv.abs()                               # A > 0 everywhere => all clipped
    loss, _, frac = v34.perdim_policy_loss(
        pos, torch.zeros_like(rd), 0.0, rd, r, 0.2, 0.28, "gate"
    )
    assert float(frac) == 1.0
    (g,) = torch.autograd.grad(loss, mu)
    assert float(g.abs().max()) == 0.0


def test_gate_clip_rate_tracks_v9s_not_the_per_coordinate_modes():
    # The whole point of v34: the realized clip rate must be v9's, unlike v32/v33.
    torch.manual_seed(0)
    n = 200000
    adv = torch.randn(n, device=DEV, dtype=torch.float64)
    per_dim = torch.randn(n, ACT, device=DEV, dtype=torch.float64) * (0.09 / ACT**0.5)
    rd, r = per_dim.exp(), per_dim.sum(-1).exp()
    zero = torch.zeros_like(rd)
    rates = {
        m: float(v34.perdim_policy_loss(adv, zero, 0.0, rd, r, 0.2, 0.28, m)[2])
        for m in ("gate", "sqrt", "linear")
    }
    v9_rate = float(
        (((adv > 0) & (r > 1.28)) | ((adv < 0) & (r < 0.8))).float().mean()
    )
    assert abs(rates["gate"] - v9_rate) < 1e-12
    assert rates["linear"] > 20 * max(rates["gate"], 1e-6)
