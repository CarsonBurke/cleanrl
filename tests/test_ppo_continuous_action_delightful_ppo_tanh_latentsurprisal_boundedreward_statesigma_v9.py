...
SCRIPT = (
    ...
    / "cleanrl" / "delightful" / "ppo" / "ppo_continuous_action_delightful_ppo_tanh_latentsurprisal_boundedreward_statesigma_v9.py"
)
SPEC = importlib.util.spec_from_file_location(
    "delightful_ppo_tanh_latentsurprisal_boundedreward_statesigma_v9", SCRIPT
)
...
def test_sac_midpoint_initialization_exposes_negative_density_surprisal():
    ...
    _, squashed_logprob, gaussian_logprob = agent._action_and_logprobs_from_raw(
        distribution, raw_action
    )

    assert torch.allclose(logstd, torch.full_like(logstd, dg.INITIAL_LOG_STD))
    assert (-gaussian_logprob).item() < 0.0
    assert torch.equal(squashed_logprob, gaussian_logprob)
...
def test_squashed_action_and_replayed_logprob_match_sac_change_of_variables():
    ...
    (
        ...
        raw_actions,
        sampled_logprob,
        sampled_gaussian_logprob,
        _,
        _,
    ) = agent.sample_action_and_value(observations)
    ...
    expected_gaussian_logprob = base_distribution.log_prob(raw_actions).sum(1)
    expected_logprob = expected_gaussian_logprob - (
        log_tanh_jacobian + torch.log(agent.action_scale)
    ).sum(1)
    ...
    assert torch.allclose(
        sampled_gaussian_logprob, expected_gaussian_logprob, atol=1e-5
    )
...
def test_saturated_tanh_action_preserves_exact_fresh_ratio_via_raw_latent():
    ...
    actions, raw_actions, rollout_logprob, _, _, _ = agent.sample_action_and_value(
        observations
    )
    _, replayed_logprob, _, _ = agent.get_action_and_value(
        observations, actions, raw_action=raw_actions
...
def test_tanh_jacobian_cancels_from_ratios_and_policy_score_gradients():
    torch.manual_seed(17)
    agent = dg.Agent(DummyVectorEnv())
    observations = torch.tensor([[0.5, -0.25, 1.0], [-1.0, 0.75, 0.25]])
    raw_actions = torch.tensor([[0.3, -0.7], [1.1, -0.2]])

    old_distribution, _ = agent.get_action_distribution(observations)
    _, old_squashed_logprob, old_gaussian_logprob = (
        agent._action_and_logprobs_from_raw(old_distribution, raw_actions)
    )
    old_squashed_logprob = old_squashed_logprob.detach()
    old_gaussian_logprob = old_gaussian_logprob.detach()

    with torch.no_grad():
        agent.actor_mean.bias.add_(0.1)
    new_distribution, _ = agent.get_action_distribution(observations)
    _, new_squashed_logprob, new_gaussian_logprob = (
        agent._action_and_logprobs_from_raw(new_distribution, raw_actions)
    )

    squashed_logratio = new_squashed_logprob - old_squashed_logprob
    gaussian_logratio = new_gaussian_logprob - old_gaussian_logprob
    assert torch.allclose(squashed_logratio, gaussian_logratio, atol=1e-6)

    score_parameters = (agent.actor_mean.bias, agent.actor_logstd.bias)
    squashed_score = torch.autograd.grad(
        new_squashed_logprob.sum(), score_parameters, retain_graph=True
    )
    gaussian_score = torch.autograd.grad(
        new_gaussian_logprob.sum(), score_parameters
    )
    for squashed_gradient, gaussian_gradient in zip(
        squashed_score, gaussian_score, strict=True
    ):
        assert torch.allclose(squashed_gradient, gaussian_gradient, atol=1e-6)


def test_gate_uses_latent_gaussian_surprisal_not_squashed_action_surprisal():
    agent = dg.Agent(DummyVectorEnv())
    observations = torch.zeros((1, 3))
    distribution, _ = agent.get_action_distribution(observations)
    # Stay inside the surprisal clip so this isolates the coordinate change.
    raw_action = torch.full((1, 2), 0.3)
    _, squashed_logprob, gaussian_logprob = agent._action_and_logprobs_from_raw(
        distribution, raw_action
    )
    advantages = torch.ones(1)

    gaussian_gate, gaussian_surprisal, _ = dg.delightful_gate(
        advantages, gaussian_logprob
    )
    squashed_gate, squashed_surprisal, _ = dg.delightful_gate(
        advantages, squashed_logprob
    )

    assert not torch.equal(gaussian_surprisal, squashed_surprisal)
    assert not torch.equal(gaussian_gate, squashed_gate)
