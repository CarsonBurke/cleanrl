...
SCRIPT = (
    ...
    / "ppo_continuous_action_velocity_chunk_intact_direct_v6.py"
)
SPEC = importlib.util.spec_from_file_location("velocity_chunk_intact_direct_v6", SCRIPT)
...
def test_chunk_law_shapes_fixed_variance_and_shared_routes():
    agent = MODULE.Agent(17, 2, small_args())
    z = torch.randn(5, 4)
    local_intent = torch.randn(5, 4)
    goal_condition = torch.randn(5, 4)
    previous_action = torch.randn(5, 2)
    local_mean, local_log_std = agent.intact_action_law.parameters_for(
        z, local_intent, previous_action
    )
    ...
    expected_log_std = torch.full_like(local_log_std, np.log(0.2))
    torch.testing.assert_close(local_log_std, expected_log_std)
    torch.testing.assert_close(goal_log_std, expected_log_std)
    source = inspect.getsource(MODULE.chunk_intact_nll_losses)
    assert source.count("agent.intact_action_law.nll") == 2


def test_chunk_nll_prioritizes_first_action_over_each_tail_action():
...
def test_local_endpoints_attach_and_goal_encoder_receives_goal_nll_gradient():
    agent = MODULE.Agent(17, 2, small_args())
    z_start = torch.randn(6, 4, requires_grad=True)
    z_future = torch.randn(6, 4, requires_grad=True)
    current = torch.randn(6, requires_grad=True)
    achieved = torch.randn(6, requires_grad=True)
    previous = torch.randn(6, 2)
    chunk = torch.randn(6, 3, 2)
    local, goal = MODULE.chunk_intact_nll_losses(
        agent, z_start, z_future, current, achieved, previous, chunk
    )
    ...
    goal_grads = torch.autograd.grad(
        goal.sum(),
        (z_start, z_future, current, achieved),
        allow_unused=True,
        retain_graph=True,
    )
    assert goal_grads[0] is not None and goal_grads[0].abs().sum() > 0
    assert goal_grads[1] is None
    assert goal_grads[2] is None and goal_grads[3] is None
    goal.sum().backward()
    assert any(
        parameter.grad is not None and parameter.grad.abs().sum() > 0
        for parameter in agent.velocity_goal_encoder.parameters()
    )


def test_velocity_command_features_use_exact_achieved_mean_and_detach_labels():
    current = torch.tensor([1.0, -0.5], requires_grad=True)
    intervals = torch.tensor([[1.0, 2.0, 3.0], [-1.0, 0.0, 1.0]])
    achieved = intervals.mean(dim=1).requires_grad_()
    features = MODULE.velocity_command_features(current, achieved)
    expected = torch.tensor([[1.0, 2.0, 1.0], [-0.5, 0.0, 0.5]])
    torch.testing.assert_close(features, expected)
    assert not features.requires_grad


def test_command_tracker_is_deterministic_and_never_exceeds_current_support():
    first = MODULE.VelocityCommandTracker(0.5, 0.5, 0.0, 10.0)
    second = MODULE.VelocityCommandTracker(0.5, 0.5, 0.0, 10.0)
    updates = [
        torch.tensor([0.0, 1.0, 2.0]),
        torch.tensor([-2.0, -1.0, 0.0]),
        torch.tensor([2.0, 3.0, 4.0]),
    ]
    first_values = [first.update(values) for values in updates]
    second_values = [second.update(values) for values in updates]
    for left, right in zip(first_values, second_values):
        torch.testing.assert_close(left, right, rtol=0.0, atol=0.0)
    torch.testing.assert_close(first_values[0], torch.tensor(1.0))
    torch.testing.assert_close(first_values[1], torch.tensor(0.0))
    torch.testing.assert_close(first_values[2], torch.tensor(1.5))
    assert first_values[-1] <= updates[-1].max()


def test_command_normalizer_updates_deterministically():
    first = MODULE.Agent(17, 2, small_args())
    second = MODULE.Agent(17, 2, small_args())
    features = torch.tensor([[0.0, 1.0, 1.0], [2.0, 3.0, 1.0]])
    first.update_command_normalizer(features, ema=0.9)
    second.update_command_normalizer(features, ema=0.9)
    torch.testing.assert_close(first.command_feature_mean, second.command_feature_mean)
    torch.testing.assert_close(
        first.command_feature_variance, second.command_feature_variance
    )
    torch.testing.assert_close(first.command_feature_mean, torch.tensor([1.0, 2.0, 1.0]))


def test_direct_chunk_is_deterministic_bounded_and_has_no_search_or_rng():
    torch.manual_seed(1)
    agent = MODULE.Agent(17, 2, small_args())
    z = torch.randn(4, 4)
    current = torch.randn(4)
    target = torch.randn(4)
    previous = torch.randn(4, 2)
    first_action, first_chunk, first_metrics = MODULE.direct_action_chunk(
        agent, z, current, target, previous
    )
    ...
    for forbidden in ("rand", "sample", "cem", "optim", "terminal"):
...
def test_training_updates_world_model_velocity_heads_shared_law_and_goal_encoder():
    args = small_args(
        forward_coef=1.0,
        sigreg_coef=0.02,
        local_nll_coef=0.1,
    ...
    batch = make_batch()
    features = MODULE.velocity_command_features(
        MODULE.forward_velocity(batch.observations[:, 0]),
        batch.interval_velocities.mean(dim=1),
    )
    agent.update_command_normalizer(features, 0.99)
    ...
    torch.testing.assert_close(
        metrics["achieved_chunk_velocity"], batch.interval_velocities.mean()
    )
    for key in (
        "local_nll",
        "goal_nll",
...
def test_defaults_are_h12_direct_with_fixed_chunk_variance():
    args = MODULE.Args()
    ...
    assert args.chunk_tail_weight == 0.1
    assert not hasattr(args, "target_velocity_margin")
...
def test_command_statistics_update_once_per_rollout_after_pooling_minibatches():
    source = SCRIPT.read_text()
    assert source.count("command_tracker.update(") == 1
    assert source.count("agent.update_command_normalizer(") == 1
    assert "command_tracker.update(torch.cat(achieved_velocity_samples))" in source
