...
SCRIPT = Path(__file__).parents[1] / "cleanrl" / "delightful" / "qcritic" / "ppo_continuous_action_delightful_qcritic_rewardema_v18.py"
SPEC = importlib.util.spec_from_file_location("delightful_qcritic_rewardema_v18", SCRIPT)
...
def test_v18_uses_current_ema_scaled_raw_rewards_and_no_retrace():
...
def test_v18_uses_256_fresh_actor_samples_and_preserves_replay_ratio():
    args = MODULE.Args()
