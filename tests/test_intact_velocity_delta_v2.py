...
SCRIPT = (
    ...
    / "ppo_continuous_action_intact_velocity_delta_v2.py"
)
SPEC = importlib.util.spec_from_file_location("intact_velocity_delta_v2", SCRIPT)
...
def test_h1_score_includes_exact_analytic_action_cost():
    model = QuadraticIntervalModel([0.5, -0.25])
    z = torch.zeros(1, 1)
    action = torch.tensor([[0.5, -0.25]])
    score = MODULE.interval_velocity_score(model, z, action, 0.1)
    torch.testing.assert_close(score, torch.tensor([2.96875]))


def test_projected_optimizer_is_deterministic_and_improves_concave_score():
    model = QuadraticIntervalModel([0.6, -0.4])
    z = torch.zeros(4, 1)
    start = torch.zeros(4, 2)
    first, first_metrics = MODULE.optimize_action_h1(
        model, z, start, optimizer_args()
    )
    second, second_metrics = MODULE.optimize_action_h1(
        model, z, start, optimizer_args()
    )
...
def test_projected_optimizer_never_returns_below_warm_start_score():
    model = NonconcaveIntervalModel()
    z = torch.zeros(1, 1)
    start = torch.full((1, 1), 0.94)
    selected, metrics = MODULE.optimize_action_h1(
        model, z, start, optimizer_args(action_opt_iterations=12)
    )
    initial_score = model.predict_interval_velocity(z, start)
    selected_score = model.predict_interval_velocity(z, selected)
...
def test_deployed_optimizer_has_no_rng_cem_or_intact_dependency():
    source = inspect.getsource(MODULE.optimize_action_h1)
