...
SCRIPT = Path(__file__).parents[1] / "cleanrl" / "delightful" / "qcritic" / "ppo_continuous_action_delightful_qcritic_rewardema_v17.py"
SPEC = importlib.util.spec_from_file_location("delightful_qcritic_rewardema_v17", SCRIPT)
...
def test_v17_uses_current_ema_scaled_raw_rewards_and_no_retrace():
    ...
    assert "q_baseline = q_taken.mean()" in source
...
def test_v17_uses_256_fresh_actor_samples_and_preserves_replay_ratio():
    args = MODULE.Args()
