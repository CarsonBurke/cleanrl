...
SCRIPT = (
    ...
    / "cleanrl" / "delightful" / "ppo" / "ppo_continuous_action_delightful_ppo_tanh_ema_boundedreward_statesigma_v7.py"
)
SPEC = importlib.util.spec_from_file_location(
    "delightful_ppo_tanh_ema_boundedreward_statesigma_v7", SCRIPT
)
...
def test_args_encode_requested_scale_choices():
    ...
    assert args.reward_ema_decay == 0.999
    assert not hasattr(args, "scale_gae_kernel")
    assert args.delight_temperature == 1.0
    assert args.surprisal_clip == 10.0


def test_reward_normalizer_ema_scales_then_bounds_each_learner_batch():
    normalizer = dg.EMABoundedRewardNormalizer(decay=0.5)

    first, first_rms, first_lower_clip, first_upper_clip = normalizer.normalize(
        np.array([-3.0, 4.0], dtype=np.float32)
    )
    assert np.isclose(first_rms, np.sqrt(12.5))
    assert np.allclose(first, np.array([0.0, 1.0]))
    assert first_lower_clip == 0.5
    assert first_upper_clip == 0.5
    assert normalizer.num_updates == 1

    second, second_rms, second_lower_clip, second_upper_clip = normalizer.normalize(
        np.array([0.0, 2.0], dtype=np.float32)
    )
    expected_m2 = (0.5 * 6.25 + 0.5 * 2.0) / (1.0 - 0.5**2)
    assert np.isclose(second_rms, np.sqrt(expected_m2))
    assert np.allclose(second, np.array([0.0, 2.0]) / np.sqrt(expected_m2))
    assert second_lower_clip == 0.0
    assert second_upper_clip == 0.0
    assert normalizer.num_updates == 2


def test_reward_normalizer_preserves_requested_post_norm_support():
    normalizer = dg.EMABoundedRewardNormalizer(decay=0.999)
    normalized, _, _, _ = normalizer.normalize(
        np.array([-100.0, -0.1, 0.0, 0.25, 1.0, 100.0], dtype=np.float32)
    )
