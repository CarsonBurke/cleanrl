...
SCRIPT = (
    ...
    / "ppo_continuous_action_spf_online_epfastactor_v10.py"
)
spec = importlib.util.spec_from_file_location(
    "ppo_continuous_action_spf_online_epfastactor_v10",
    SCRIPT,
)
...
def test_fast_actor_reads_before_update_and_isolates_streams(device):
    ...
    fast_actor.update(
        ...
        alpha,
        beta,
        -torch.expm1(-(alpha - 1.0)),
        -torch.expm1(-(beta - 1.0)),
...
def test_smooth_offset_update_matches_active_tanh_autograd(device):
    args = Args(
        num_envs=1,
        fast_actor_eta=0.01,
        fast_actor_leak=1.0,
        fast_actor_offset_clip=2.0,
        fast_actor_signal_clip=100.0,
        fast_actor_score_clip=100.0,
    )
    fast_actor = LocalFastActor(1, 1, 2, args, device)
    fast_actor.alpha_weight[0, 0, 0] = 4.0
    obs = torch.tensor(((1.0, 0.0),), device=device)
    z = torch.tensor(((0.8,),), device=device)
    raw_weight = torch.tensor(4.0, device=device, requires_grad=True)
    alpha_offset = args.fast_actor_offset_clip * torch.tanh(
        raw_weight / args.fast_actor_offset_clip
    )
    alpha = 1.0 + F.softplus(alpha_offset)
    beta = torch.tensor(2.0, device=device)
    expected_score = torch.autograd.grad(
        torch.distributions.Beta(alpha, beta).log_prob(z[0, 0]),
        raw_weight,
    )[0]
    ...
    fast_actor.update(
        ...
        alpha.view(1, 1),
        beta.view(1, 1),
        torch.sigmoid(alpha_offset).view(1, 1),
        (-torch.expm1(-(beta - 1.0))).view(1, 1),
        torch.ones(1, device=device),
        torch.zeros(1, device=device),
    )

    expected_weight = 4.0 + args.fast_actor_eta * expected_score
    torch.testing.assert_close(
        fast_actor.alpha_weight[0, 0, 0],
        expected_weight,
    )


def test_boundary_credits_then_resets_only_finished_stream(device):
    args = Args(num_envs=2, fast_actor_leak=1.0)
    fast_actor = LocalFastActor(2, 1, 2, args, device)
    obs = torch.tensor(((1.0, 0.0), (0.0, 1.0)), device=device)
    ...
    fast_actor.update(
        ...
        torch.full((2, 1), 2.0, device=device),
        torch.full((2, 1), 3.0, device=device),
        torch.full((2, 1), 0.6321205588, device=device),
        torch.full((2, 1), 0.8646647168, device=device),
    ...
    torch.testing.assert_close(
        fast_actor.alpha_weight[0],
        torch.zeros_like(fast_actor.alpha_weight[0]),
    )
    assert fast_actor.alpha_weight[1].abs().sum() > 0


def test_stored_jacobian_controls_near_unit_concentration(device):
    args = Args(
        num_envs=1,
        fast_actor_eta=1.0,
        fast_actor_leak=1.0,
    ...
    fast_actor = LocalFastActor(1, 1, 1, args, device)

    fast_actor.update(
        torch.ones((1, 1), device=device),
        torch.full((1, 1), 0.8, device=device),
        torch.ones((1, 1), device=device),
        torch.full((1, 1), 2.0, device=device),
        torch.full((1, 1), 1e-12, device=device),
        torch.full((1, 1), 0.6321205588, device=device),
        torch.ones(1, device=device),
        torch.zeros(1, device=device),
    )

    assert fast_actor.alpha_weight.abs().max() < 1e-10
