...
def test_shared_signal_learns_negative_nonunit_amplitude_one_observation_at_a_time():
    ...
    state = AdaptiveStructureState(weight)
...
def test_scalar_fit_does_not_snap_positive_signal_to_one():
    ...
    state = AdaptiveStructureState(weight, control="no_dense")
    for index in range(100):
...
def test_shared_representation_does_not_force_equal_coefficient_magnitudes():
    weight = torch.zeros(1, 4, device="cuda")
    state = AdaptiveStructureState(weight)
    step = torch.compile(state.step, fullgraph=True)
    inputs = torch.eye(4, device="cuda")
    targets = (0.4, -1.3, 2.1, -0.8)
    for index in range(3000):
        x = inputs[index % 4:index % 4 + 1]
        residual = (weight * x).sum() - targets[index % 4]
        step(weight, residual * x, x.square())
    predictions = inputs @ weight[0]
    torch.testing.assert_close(predictions, torch.tensor(targets, device="cuda"), rtol=0.05, atol=0.05)
