...
from cleanrl.beta_policy.ppo_continuous_action_beta_nll_hlgauss_paperdg_cliphi_noadvnorm_v5 import (
    Agent,
    Args,
    SAMPLE_EPS,
    beta_nll_to_gate,
...
def test_v5_defaults_use_requested_ablations_hlgauss_and_paper_gate():
    args = Args()
...
def test_beta_nll_gate_is_finite_detached_bounded_and_sign_aware_without_rescale():
    advantages = torch.tensor([-1.0, 0.0, 1.0])
    beta_nll = torch.ones(3)

    clipped_nll, gate = beta_nll_to_gate(
        advantages,
        beta_nll,
        clip=10.0,
        gate_coef=1.0,
...
def test_beta_nll_gate_uses_clipped_surprisal_to_strengthen_rare_successes_and_failures():
    advantages = torch.tensor([-1.0, -1.0, 1.0, 1.0])
    beta_nll = torch.tensor([0.1, 20.0, 0.1, 20.0])
    ...
    assert torch.allclose(clipped_nll, torch.tensor([0.1, 2.0, 0.1, 2.0]))
...
def test_update_gate_uses_stored_behavior_nll_not_live_policy():
    ...
    _, behavior_gate = beta_nll_to_gate(
        advantages,
        stored_nll,
        clip=10.0,
        gate_coef=1.0,
    )
    _, live_gate = beta_nll_to_gate(
        advantages,
        live_policy_nll,
        clip=10.0,
        gate_coef=1.0,
