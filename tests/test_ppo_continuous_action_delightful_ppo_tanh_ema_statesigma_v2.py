...
SCRIPT = (
    ...
    / "cleanrl" / "delightful" / "ppo" / "ppo_continuous_action_delightful_ppo_tanh_ema_statesigma_v2.py"
)
SPEC = importlib.util.spec_from_file_location(
    "delightful_ppo_tanh_ema_statesigma_v2", SCRIPT
)
...
def test_policy_log_standard_deviation_is_state_dependent_and_bounded():
    ...
    assert torch.allclose(logstd[0], torch.full((2,), -1.5))
    logstd.sum().backward()
