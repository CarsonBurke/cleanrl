...
from cleanrl.iterthink.v24_d4hlgauss.dg.ppo_continuous_action_iterthink_v24_beta_d4hlgauss_symlog_dg_rawgate_normupdate_v1 import (
    Agent as DGAgent,
    Args as DGArgs,
...
def test_hybrid_dg_gates_on_raw_advantage_but_updates_with_normalized_advantage():
    raw_advantage = torch.tensor([0.1, -0.1])
    normalized_advantage = torch.tensor([1.0, -1.0])
    new_logprob = torch.tensor([-2.0, -2.0], requires_grad=True)

    raw_gate, _, _, _ = delightful_gate(raw_advantage, new_logprob.detach(), eta=1.0, clip_bound=10.0)
    norm_gate, _, _, _ = delightful_gate(
    ...
    loss = -(raw_gate.detach() * normalized_advantage * new_logprob).mean()
    loss.backward()

    assert not torch.allclose(raw_gate, norm_gate)
    assert torch.allclose(new_logprob.grad, -raw_gate.detach() * normalized_advantage / raw_advantage.numel())
