...
SCRIPT = (
    ...
    / "cleanrl" / "delightful" / "ppo" / "ppo_continuous_action_delightful_ppo_tanh_ema_statesigma_v6.py"
)
SPEC = importlib.util.spec_from_file_location(
    "delightful_ppo_tanh_ema_statesigma_v6", SCRIPT
)
...
def test_reward_normalizer_uses_only_bias_corrected_immediate_reward_rms():
    normalizer = dg.EMARewardNormalizer(decay=0.5)

    first, first_rms = normalizer.normalize(np.array([3.0, 4.0], dtype=np.float32))
    assert np.isclose(first_rms, np.sqrt(12.5))
    assert np.allclose(first, np.array([3.0, 4.0]) / np.sqrt(12.5))
