...
SCRIPT = (
    ...
    / "cleanrl"
    / "encoder-value"
    / "ppo_continuous_action_lejepa_geometric_td_actor_v7.py"
)
SPEC = importlib.util.spec_from_file_location("lejepa_geometric_td_actor_v7", SCRIPT)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(MODULE)


def test_long_geometric_horizon_and_fixed_actor_unroll_are_distinct():
    args = MODULE.Args()
    assert args.advantage_horizon == 16
    assert args.tail_fit_min_steps == 256
    assert args.tail_mode == "learned"
...
def test_same_episode_suffix_steps_stop_at_boundaries_and_rollout_edge():
...
def test_uncentered_tail_fit_recovers_coefficient_and_honors_mask():
    bootstrap = torch.tensor([1.0, 2.0, -3.0, 1000.0])
    realized = 0.37 * bootstrap
    realized[-1] = -1000.0
    eligible = torch.tensor([True, True, True, False])
    raw, coefficient, score = MODULE.fit_tail_coefficient(
        bootstrap, realized, eligible, ridge=0.0
    )
    torch.testing.assert_close(raw, torch.tensor(0.37))
    torch.testing.assert_close(coefficient, torch.tensor(0.37))
    assert score > 0.999999


def test_tail_fit_is_a_convex_reliability_weight():
    x = torch.tensor([1.0, 2.0, 3.0])
    eligible = torch.ones(3, dtype=torch.bool)
    _, high, _ = MODULE.fit_tail_coefficient(x, 4.0 * x, eligible, ridge=0.0)
    _, low, _ = MODULE.fit_tail_coefficient(x, -x, eligible, ridge=0.0)
    assert high == 1.0
    assert low == 0.0


def test_gating_only_bootstrap_preserves_current_state_baseline():
    prefix = torch.tensor([3.0, -2.0])
    value = torch.tensor([7.0, 5.0])
    full_advantage = torch.tensor([1.0, -4.0])
    baseline = prefix - value
    bootstrap = full_advantage - prefix + value
    torch.testing.assert_close(baseline + bootstrap, full_advantage)
    torch.testing.assert_close(baseline + 0.0 * bootstrap, prefix - value)
...
def test_current_rollout_tail_fit_is_installed_only_after_actor_update():
    source = SCRIPT.read_text()
    read_lag = source.index("rho_used = (")
    actor_credit = source.index(
        "policy_adv = baseline_advantage + rho_used * bootstrap_tail"
    )
    actor_update = source.index("for epoch in range(args.update_epochs)")
    hard_assign = source.index("lagged_tail_coefficient = (", actor_update)
    assert read_lag < actor_credit < actor_update < hard_assign
    assert "lagged_tail_coefficient =" in source
    assert ".lerp" not in source
