...
SCRIPT = (
    ...
    / "cleanrl"
    / "encoder-value"
    / "ppo_continuous_action_lejepa_ghm_rbmean_prefix_v3.py"
)
SPEC = importlib.util.spec_from_file_location("lejepa_ghm_rbmean_prefix_v3", SCRIPT)
...
def test_rb_mean_recursion_uses_every_observed_suffix_and_rollout_tail():
    gamma = 0.8
    outcomes = torch.tensor([1.0, 2.0, 3.0]).reshape(3, 1, 1)
    frozen_next = torch.tensor([10.0, 20.0, 7.0]).reshape(3, 1, 1)
    targets, valid = MODULE.rao_blackwellized_occupancy_targets(
        outcomes,
        frozen_next,
        torch.zeros(3, 1),
        torch.zeros(3, 1),
    ...
    expected_2 = (1.0 - gamma) * 3.0 + gamma * 7.0
    expected_1 = (1.0 - gamma) * 2.0 + gamma * expected_2
    expected_0 = (1.0 - gamma) * 1.0 + gamma * expected_1
    torch.testing.assert_close(
        targets[:, 0, 0], torch.tensor([expected_0, expected_1, expected_2])
    )
    assert valid.all()


def test_rb_mean_cuts_reset_and_bootstraps_final_observation_at_truncation():
    gamma = 0.9
    outcomes = torch.tensor([1.0, 2.0, 100.0, 200.0]).reshape(4, 1, 1)
    frozen_next = torch.zeros_like(outcomes)
    frozen_next[1] = 5.0
    boundaries = torch.zeros(4, 1)
    boundaries[1] = 1.0
    targets, valid = MODULE.rao_blackwellized_occupancy_targets(
        outcomes,
        frozen_next,
    ...
    expected_1 = (1.0 - gamma) * 2.0 + gamma * 5.0
    expected_0 = (1.0 - gamma) * 1.0 + gamma * expected_1
    torch.testing.assert_close(targets[1, 0, 0], torch.tensor(expected_1))
    torch.testing.assert_close(targets[0, 0, 0], torch.tensor(expected_0))
    assert valid.all()


def test_rb_mean_true_terminal_has_zero_tail():
    gamma = 0.95
    outcomes = torch.tensor([2.0, 3.0, 100.0]).reshape(3, 1, 1)
    frozen_next = torch.full_like(outcomes, 999.0)
    boundaries = torch.zeros(3, 1)
    boundaries[1] = 1.0
    terminations = boundaries.clone()
    targets, valid = MODULE.rao_blackwellized_occupancy_targets(
        outcomes,
        frozen_next,
        terminations,
    ...
    expected_1 = (1.0 - gamma) * 3.0
    expected_0 = (1.0 - gamma) * 2.0 + gamma * expected_1
    torch.testing.assert_close(targets[1, 0, 0], torch.tensor(expected_1))
    torch.testing.assert_close(targets[0, 0, 0], torch.tensor(expected_0))
    assert valid.all()


def test_rb_mean_invalid_outcome_propagates_only_to_current_episode():
    outcomes = torch.ones(4, 1, 1)
    boundaries = torch.zeros(4, 1)
    boundaries[2] = 1.0
    valids = torch.ones(4, 1)
    valids[2] = 0.0
    _, valid = MODULE.rao_blackwellized_occupancy_targets(
        outcomes,
    ...
    assert valid.squeeze(1).tolist() == [False, False, False, True]
...
def test_source_has_fixed_prefix_joint_ppo_and_mc_only_ghm():
    ...
    assert "ghm_epochs: int = 8" in source
    assert "flat_rb_mean_target[mb]" in source
    assert "flat_mc_cfm_loss(" in source
    frozen_actor = source.index("means = ghm_target.mean(")
    actor_credit = source.index("one_step_credits = successor_innovation(")
    mean_fit = source.index("mean_pred = ghm.mean(")
    install_target = source.index("hard_update(ghm_target, ghm)")
    assert frozen_actor < actor_credit < mean_fit < install_target
