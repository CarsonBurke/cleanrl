...
SCRIPT = (
    ...
    / "cleanrl"
    / "encoder-value"
    / "ppo_continuous_action_lejepa_tddelta_v1.py"
)
SPEC = importlib.util.spec_from_file_location("lejepa_tddelta_v1", SCRIPT)
...
def test_popart_rescale_preserves_raw_bands_and_optimizer_coordinates():
    torch.manual_seed(3)
    head = torch.nn.Linear(4, 6)
    optimizer = torch.optim.Adam(head.parameters(), lr=1e-3)
    x = torch.randn(5, 4)
    # Materialize Adam state.
    head(x).square().mean().backward()
    optimizer.step()
    old_mean = torch.randn(2, 3)
    old_std = torch.rand(2, 3) + 0.2
    new_mean = torch.randn(2, 3)
    new_std = torch.rand(2, 3) + 0.2
    before = head(x).reshape(5, 2, 3) * old_std + old_mean
    MODULE.preserve_raw_popart_output(
        head, optimizer, old_mean, old_std, new_mean, new_std
    )
    after = head(x).reshape(5, 2, 3) * new_std + new_mean
