...
from cleanrl.critic import (
    ppo_continuous_action_criticmetric_opsd_rawreward_v5 as stablefast,
)


def test_raw_replay_rewards_reach_bellman_targets_without_rescaling():
    args = stablefast.Args()
    assert (
        args.exp_name
        == "ppo_continuous_action_criticmetric_opsd_rawreward_v5"
    )
    assert not hasattr(args, "reward_ema_decay")
    assert not hasattr(args, "reward_norm_eps")
    assert not hasattr(stablefast, "EMARewardNormalizer")
    ...
    rewards = torch.tensor([-16.0, 3.5, 32.0])
    discounts = torch.tensor([0.0, 0.5, 0.25])
    replay.add(
        rewards[:, None],
        torch.zeros(3, 1),
        rewards,
    ...
    torch.testing.assert_close(
        sampled_observations.squeeze(-1),
        sampled_rewards,
        rtol=0,
        atol=0,
    )

    next_values = 2.0 * sampled_observations.squeeze(-1)
    target = stablefast.one_step_q_target(
        sampled_rewards,
        sampled_discounts,
        next_values,
    )
    torch.testing.assert_close(
        target - sampled_discounts * next_values,
        sampled_rewards,
        rtol=0,
        atol=0,
    )
