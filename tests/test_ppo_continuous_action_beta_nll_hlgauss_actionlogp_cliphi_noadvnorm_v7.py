...
from cleanrl.beta_policy.ppo_continuous_action_beta_nll_hlgauss_actionlogp_cliphi_noadvnorm_v7 import (
    Agent,
    Args,
    SAMPLE_EPS,
    beta_nll_to_gate,
...
def test_v7_defaults_use_requested_ablations_hlgauss_and_action_logprob_gate():
    args = Args()
...
def test_logprob_gate_is_finite_detached_bounded_and_sign_aware_without_rescale():
    advantages = torch.tensor([-1.0, 0.0, 1.0])
    logprob = -torch.ones(3)

    clipped_surprisal, gate = logprob_to_gate(
        advantages,
        logprob,
        clip=10.0,
        gate_coef=1.0,
    ...
    assert torch.allclose(clipped_surprisal, torch.ones(3))
    assert torch.isfinite(gate).all()
    assert gate.requires_grad is False
    assert torch.all(gate > 0.0)
    assert torch.all(gate < 1.0)
    assert gate[0] < 0.5
    assert torch.allclose(gate[1], torch.tensor(0.5))
    assert gate[2] > 0.5


def test_logprob_gate_uses_raw_clipped_surprisal_to_strengthen_rare_successes_and_failures():
    advantages = torch.tensor([-1.0, -1.0, 1.0, 1.0])
    logprob = torch.tensor([-0.1, -20.0, -0.1, -20.0])
    ...
    assert torch.allclose(clipped_surprisal, torch.tensor([0.1, 2.0, 0.1, 2.0]))
    assert gate[1] < gate[0] < 0.5
    assert gate[3] > gate[2] > 0.5


def test_logprob_gate_preserves_negative_continuous_density_surprisal():
    advantages = torch.tensor([-1.0, 1.0])
    logprob = torch.tensor([2.0, 2.0])
    ...
    assert torch.allclose(clipped_surprisal, torch.tensor([-2.0, -2.0]))
    assert gate[0] > 0.5
    assert gate[1] < 0.5


def test_logprob_gate_can_shift_to_action_space_density_with_logdet():
    advantages = torch.tensor([-1.0, 1.0])
    logprob_z = torch.tensor([2.0, 2.0])
...
def test_agent_action_logdet_matches_linear_beta_to_action_transform():
    agent = Agent(DummyVecEnv(), Args(num_bins=31))

    expected = torch.log(torch.tensor([5.0, 2.0])).sum()

    assert torch.allclose(agent.action_logdet.cpu(), expected)


def test_agent_logprob_gate_uses_action_space_density_shift():
    agent = Agent(DummyVecEnv(), Args(num_bins=31))
    advantages = torch.tensor([1.0])
    logprob_z = torch.tensor([agent.action_logdet.item()])

    clipped_surprisal, gate = agent.logprob_gate(
        advantages,
        logprob_z,
        clip=10.0,
        gate_coef=1.0,
    ...
    assert torch.allclose(clipped_surprisal.cpu(), torch.zeros(1), atol=1e-6)
    assert torch.allclose(gate.cpu(), torch.tensor([0.5]), atol=1e-6)
...
def test_update_gate_uses_stored_behavior_logprob_not_live_policy():
    advantages = torch.tensor([-1.0, -1.0, 1.0, 1.0])
    stored_logprob = torch.tensor([-0.5, -1.0, -1.5, -2.0])
    live_policy_logprob = torch.tensor([-9.0, -1.0, -1.0, -1.0])

    _, behavior_gate = logprob_to_gate(
        advantages,
        stored_logprob,
        clip=10.0,
        gate_coef=1.0,
    )
    _, live_gate = logprob_to_gate(
        advantages,
        live_policy_logprob,
        clip=10.0,
        gate_coef=1.0,
