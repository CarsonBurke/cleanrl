...
from cleanrl.pan_goal_solver.ppo_continuous_action_pan_goal_solver_v4 import (
    Args,
    DirectGoalFollower,
    PanGoalSolver,
    VectorReplayBuffer,
...
def _batch(batch_size=16, history=8):
    ...
    return {
        ...
        "diagnostic_reward": rng.normal(size=batch_size).astype(np.float32),
    }
...
def test_direct_inference_does_not_call_proposer_utility_or_transition():
    ...
    with torch.no_grad():
        action, current_goal = agent.direct_action(
...
def test_diagnostic_reward_cannot_change_follower_update():
    args = _args()
    first = _agent()
    second = copy.deepcopy(first)
    first_batch = _batch()
    second_batch = copy.deepcopy(first_batch)
    second_batch["diagnostic_reward"] = second_batch["diagnostic_reward"][::-1].copy()
    first_parameters = first.parameter_groups()[1]
    second_parameters = second.parameter_groups()[1]
    first_optimizer = torch.optim.AdamW(first_parameters, lr=1e-3)
    second_optimizer = torch.optim.AdamW(second_parameters, lr=1e-3)
    train_follower_step(
        first,
        first_optimizer,
        first_batch,
    ...
    for first_parameter, second_parameter in zip(
        first_parameters, second_parameters, strict=True
    ):
        torch.testing.assert_close(first_parameter, second_parameter, rtol=0, atol=0)
...
def test_replay_hindsight_and_reward_windows_do_not_cross_episodes():
    ...
    for step in range(300):
        ...
        segment = np.full(2, step // 11, dtype=np.int64)
    ...
    np.testing.assert_array_equal(
        hindsight["obs"][:, -1, 1], hindsight["future_obs"][:, -1, 1]
    )
    assert np.all(
