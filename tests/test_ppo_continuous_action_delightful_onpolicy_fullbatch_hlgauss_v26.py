...
SCRIPT = Path(__file__).parents[1] / "cleanrl" / "delightful" / "onpolicy" / "ppo_continuous_action_delightful_onpolicy_fullbatch_hlgauss_v26.py"
SPEC = importlib.util.spec_from_file_location("delightful_onpolicy_fullbatch_hlgauss_v26", SCRIPT)
...
def test_defaults_define_one_fresh_full_rollout_actor_batch():
    args = v26.Args()

    assert args.num_envs * args.num_steps == 2048
    assert args.target_actor_batch_size == 2048
...
def test_delightful_gate_matches_detached_definition():
    ...
    gate, surprisal, delight = v26.delightful_gate(advantages, action_logprobs, eta=2.0)
...
def test_delightful_gate_clips_surprisal_and_validates_scale():
    _, surprisal, _ = v26.delightful_gate(
        torch.ones(2),
    ...
    with pytest.raises(ValueError, match="dg_eta"):
        v26.delightful_gate(torch.ones(1), torch.zeros(1), eta=0.0)
    with pytest.raises(ValueError, match="dg_surprisal_clip"):
        v26.delightful_gate(torch.ones(1), torch.zeros(1), surprisal_clip=0.0)


def test_single_critic_head_has_neutral_value_initialization():
    args = v26.Args()
    agent = v26.Agent(DummyVectorEnv(), args)
...
def test_critic_decodes_expectation_in_symlog_coordinates_before_symexp():
    args = v26.Args(value_num_bins=3, value_min=-2.0, value_max=2.0)
    support = torch.tensor([-1.0, 0.0, 1.0])
    agent = v26.Agent(DummyVectorEnv(), args, value_support=support)
    probabilities = torch.tensor([[0.1, 0.2, 0.7]])
    logits = probabilities.log()

    decoded = agent.decode_value_logits(logits)
    expected = v26.symexp((probabilities * support).sum(dim=-1))
    wrong_raw_space_expectation = (probabilities * v26.symexp(support)).sum(dim=-1)
...
def test_gae_bootstraps_truncation_but_stops_trace():
    ...
    advantages, returns = v26.compute_gae(
        rewards,
        values,
        terminations,
        truncations,
