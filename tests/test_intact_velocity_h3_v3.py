...
SCRIPT = (
    ...
    / "ppo_continuous_action_intact_velocity_h3_v3.py"
)
SPEC = importlib.util.spec_from_file_location("intact_velocity_h3_v3", SCRIPT)
...
def test_h3_score_sums_each_predicted_reward_and_exact_action_cost():
    model = QuadraticIntervalModel([0.5, -0.25])
    z = torch.zeros(1, 1)
    actions = torch.tensor([[[0.5, -0.25], [0.5, -0.25], [0.5, -0.25]]])
    score, velocities = MODULE.action_sequence_score(model, z, actions, 0.1)
    torch.testing.assert_close(score, torch.tensor([8.90625]))
    torch.testing.assert_close(velocities, torch.full((1, 3), 3.0))


def test_h3_score_backpropagates_future_rewards_through_dynamics():
    agent = MODULE.Agent(17, 2, small_args())
    z = torch.randn(5, 4)
...
def test_default_planner_horizon_is_three():
    assert MODULE.Args().planner_horizon == 3
