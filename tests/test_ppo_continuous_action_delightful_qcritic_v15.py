...
SCRIPT = Path(__file__).parents[1] / "cleanrl" / "delightful" / "qcritic" / "ppo_continuous_action_delightful_qcritic_v15.py"
SPEC = importlib.util.spec_from_file_location("delightful_qcritic_v15", SCRIPT)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(MODULE)


def test_dmc_cheetah_reward_is_bounded_linear_speed_tolerance():
    speeds = torch.tensor([-3.0, 0.0, 2.5, 10.0, 14.0])
    rewards = MODULE.dmc_cheetah_run_reward(speeds)
    torch.testing.assert_close(rewards, torch.tensor([0.0, 0.0, 0.25, 1.0, 1.0]))
...
def test_actor_has_state_dependent_diagonal_scale_and_bounded_actions():
    ...
    assert actor.logstd.weight.shape == (2, 256)
...
def test_v15_uses_one_step_q_targets_without_retrace_implementation():
