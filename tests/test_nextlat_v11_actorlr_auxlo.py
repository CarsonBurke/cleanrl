"""Structural tests for the v11 actorlr_auxlo version.

Covers what the v8 functional tests cannot: split-LR defaults (incl. aux group), unshared
backbone default, full-batch defaults, and the optimizer-group invariant the
v11 build asserts (disjoint actor/critic/aux groups covering all params exactly
...
def _load_v11():
    spec = importlib.util.spec_from_file_location(
        "nextlat_v11_actorlr_auxlo",
        ROOT / "cleanrl/iterthink/v24_d3bucket/ppo/ppo_continuous_action_iterthink_v24_beta_d3bucket_mtp_ppoadvnorm_batch_nextlat_v11_actorlr_auxlo.py",
    )
...
def test_v10_fullbatch_defaults():
    ...
    assert args.actor_lr == pytest.approx(1e-2)
    assert args.critic_lr == pytest.approx(1e-3)
    assert args.nextlat_lr == pytest.approx(3e-4)
    assert args.target_kl == pytest.approx(0.03)


def _optimizer_groups(agent):
    # Mirrors the v11 build: core actor/critic groups plus the aux-only params
    # derived by set difference (predictor / dyn+readout).
    actor = list(agent.actor_parameters())
    critic = list(agent.critic_parameters())
    actor_ids = {id(p) for p in actor}
    critic_ids = {id(p) for p in critic}
    aux = [p for p in agent.nextlat_actor_parameters() if id(p) not in actor_ids]
    aux += [p for p in agent.nextlat_critic_parameters() if id(p) not in critic_ids]
...
def test_optimizer_group_partition_matches_guard(share_backbone):
    ...
    actor, critic, aux = _optimizer_groups(agent)
    actor_ids = {id(p) for p in actor}
    critic_ids = {id(p) for p in critic}
    ...
    if not share_backbone:
        ...
        assert not (actor_ids & aux_ids)
        assert not (critic_ids & aux_ids)
        assert all_ids == (actor_ids | critic_ids | aux_ids)
        # Aux group is non-trivial: the predictors actually left the core groups.
        assert len(aux_ids) > 0
    else:
        # Shared trunk lands in both groups: the v11 build guard must fire.
