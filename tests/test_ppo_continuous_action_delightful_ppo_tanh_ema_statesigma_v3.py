...
SCRIPT = (
    ...
    / "cleanrl" / "delightful" / "ppo" / "ppo_continuous_action_delightful_ppo_tanh_ema_statesigma_v3.py"
)
SPEC = importlib.util.spec_from_file_location(
    "delightful_ppo_tanh_ema_statesigma_v3", SCRIPT
)
...
def test_args_encode_requested_scale_choices():
    ...
    assert args.num_envs * args.num_steps == args.target_actor_batch_size == 256
    assert args.num_minibatches == 4
    assert not args.anneal_lr
    assert args.reward_ema_decay == 0.999
    assert args.scale_discounted_return
    assert args.delight_temperature == 1.0
    assert args.surprisal_clip == 10.0


def test_reward_normalizer_uses_bias_corrected_rms_and_return_scale():
    normalizer = dg.EMARewardNormalizer(decay=0.5, reward_scale=0.01)

    first, first_rms = normalizer.normalize(np.array([3.0, 4.0], dtype=np.float32))
    assert np.isclose(first_rms, np.sqrt(12.5))
    assert np.allclose(first, 0.01 * np.array([3.0, 4.0]) / np.sqrt(12.5))
    ...
    assert np.allclose(second, 0.01 * np.array([0.0, 2.0]) / np.sqrt(expected_m2))
