...
MODULE_PATH = (
    ...
    / "ppo_continuous_action_embopt_atlas_thompson_v3.py"
)
SPEC = importlib.util.spec_from_file_location("embopt_atlas_thompson_v3", MODULE_PATH)
...
def test_learned_arrival_does_not_depend_on_current_support():
    rates = torch.zeros(1, 3)
    duration = torch.full((1, 3), math.log(1.0))
    unsupported = torch.full((1, 3), -100.0)
...
def test_state_bridge_is_goal_free_while_conditional_bridge_accepts_terminal_edge():
    env = SimpleNamespace(
        single_observation_space=gym.spaces.Box(-1.0, 1.0, (5,), dtype=np.float32),
        single_action_space=gym.spaces.Box(-1.0, 1.0, (2,), dtype=np.float32),
    )
    ...
    state_prediction = agent.navigate_state_only(y)
    conditional_prediction = agent.navigate(y, goal, delta)

    assert state_prediction.shape == (6, 2, 4)
    assert conditional_prediction.shape == (6, 2, 4)
