...
def test_geometric_horizon_mapping_includes_td0_fixed95_and_full_gamma():
    gamma = 0.99
    assert MODULE.lambda_from_effective_horizon(1.0, gamma) == 0.0
    assert abs(
        MODULE.lambda_from_effective_horizon(16.8067, gamma) - 0.95
    ) < 1e-6
    assert MODULE.lambda_from_effective_horizon(100.0, gamma) == 1.0
...
def test_vectorized_candidate_gae_matches_individual_recursions():
    torch.manual_seed(16)
    delta = torch.randn(7, 3, 4)
    continuation = torch.randint(0, 2, (7, 3)).float()
    candidate_lambdas = torch.rand(5, 7, 3)
    actual = MODULE.learned_vector_gae_candidates(
        delta, continuation, 0.99, candidate_lambdas
    )
    expected = torch.stack(
        [
            MODULE.learned_vector_gae(
                delta, continuation, 0.99, candidate_lambda
            )
...
def test_variance_temperature_lengthens_trace_when_batch_noise_is_discounted():
    bootstrap = torch.tensor([[2.0]])
    direct = torch.tensor([[0.0]])
    recursive = torch.tensor([[0.0]])
    variance = torch.tensor([[4.0]])
    per_transition = MODULE.optimal_recursive_lambda(
        bootstrap,
        direct,
        recursive,
        variance,
    ...
    batch_gradient = MODULE.optimal_recursive_lambda(
        bootstrap,
        direct,
        recursive,
        variance,
        variance_temperature=1.0 / 16.0,
    )
    assert batch_gradient > per_transition
    torch.testing.assert_close(per_transition, torch.tensor([0.5]))
    torch.testing.assert_close(batch_gradient, torch.tensor([16.0 / 17.0]))


def test_cross_environment_gradient_risk_preserves_unclamped_u_statistic():
    reference = torch.zeros(4, 2, 3)
    constant_bias = torch.full_like(reference, 2.0)
    mask = torch.ones(4, dtype=torch.bool)
    bias, variance, total = MODULE.cross_environment_gradient_risk(
        constant_bias, reference, mask
    )
    torch.testing.assert_close(bias, torch.tensor(24.0))
    torch.testing.assert_close(variance, torch.tensor(0.0))
    torch.testing.assert_close(total, bias)

    alternating = reference.clone()
    alternating[0] = 1.0
    alternating[1] = -1.0
    alternating[2] = 1.0
    alternating[3] = -1.0
    negative_bias, positive_variance, _ = MODULE.cross_environment_gradient_risk(
        alternating, reference, mask
    )
    assert negative_bias < 0.0
    assert positive_variance > 0.0


def test_environment_gradient_tensor_keeps_environments_independent():
    score = torch.tensor(
        [
            [[1.0], [10.0]],
            [[2.0], [20.0]],
    ...
    actual = MODULE.environment_gradient_tensors(score, advantage, valid)
    expected = torch.tensor(
        [
            [[6.5, 8.0]],
            [[300.0, 400.0]],
...
def test_routed_environment_gradient_uses_actuator_specific_credit():
    projected_score = torch.tensor(
        [[[[1.0], [2.0]]], [[[3.0], [4.0]]]]
    ).reshape(2, 1, 2, 1)
    routed_advantage = torch.tensor(
        [[[[5.0, 7.0], [11.0, 13.0]]], [[[17.0, 19.0], [23.0, 29.0]]]]
    ).reshape(2, 1, 2, 2)
    valid = torch.ones(2, 1, dtype=torch.bool)
    actual = MODULE.environment_routed_gradient_tensors(
        projected_score, routed_advantage, valid
    )
    expected = torch.tensor([[[85.0, 103.0]]])
    torch.testing.assert_close(actual, expected)
