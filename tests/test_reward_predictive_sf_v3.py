...
SCRIPT = (
    ...
    / "cleanrl"
    / "encoder-value"
    / "ppo_continuous_action_reward_predictive_sf_v3.py"
)
SPEC = importlib.util.spec_from_file_location("reward_predictive_sf_v3", SCRIPT)
...
def test_successor_loss_preserves_full_scalar_td_strength_independent_of_dimension():
    reward_vector = torch.full((8,), 8**-0.5)
    scalar_error = torch.tensor([2.0, -3.0, 4.0])
    target = torch.randn(3, 8)
    prediction = target + scalar_error.unsqueeze(-1) * reward_vector

    scalar_loss, orthogonal_loss = MODULE.successor_td_losses(
        prediction, target, reward_vector
    )

    torch.testing.assert_close(scalar_loss, scalar_error.square().mean())
    torch.testing.assert_close(orthogonal_loss, torch.zeros(()), atol=1e-6, rtol=0.0)


def test_reward_observability_starts_require_complete_in_episode_sequences():
    ...
    starts = MODULE.reward_observability_starts(boundaries, horizon=2)

    expected = torch.tensor([[0, 0], [1, 0], [2, 1], [3, 1]])
...
def test_successor_and_feature_gradient_clip_groups_are_disjoint():
    args = MODULE.Args(
        hidden=8, k_blocks=1, n_experts=2, feature_dim=5, feature_hidden=12
    )
    agent = MODULE.Agent(DummyVectorEnv(), args)
    successor_ids = {id(parameter) for parameter in agent.successor_parameters()}
    feature_ids = {id(parameter) for parameter in agent.feature_parameters()}

    assert successor_ids
    assert feature_ids
    assert successor_ids.isdisjoint(feature_ids)
