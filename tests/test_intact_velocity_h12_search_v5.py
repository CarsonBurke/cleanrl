...
SCRIPT = (
    ...
    / "ppo_continuous_action_intact_velocity_h12_search_v5.py"
)
SPEC = importlib.util.spec_from_file_location("intact_velocity_h12_search_v5", SCRIPT)
...
def cem_args(**overrides):
    values = dict(
        action_cost_coef=0.0,
        cem_population=64,
        cem_iterations=3,
        cem_elites=8,
        cem_global_fraction=0.125,
        cem_initial_std=0.6,
        cem_min_std=0.08,
        cem_update_rate=0.7,
    )
...
def test_h12_score_sums_each_predicted_reward_and_exact_action_cost():
...
def test_cem_population_scoring_delegates_to_exact_sequence_score():
    model = QuadraticIntervalModel([0.5, -0.25])
    z = torch.zeros(2, 1)
    candidates = torch.randn(2, 3, 4, 2).clamp(-1.0, 1.0)
    population_scores, population_velocities = (
        MODULE.action_sequence_population_score(
            model, z, candidates, 0.1, return_velocities=True
        )
    )
    flat_scores, flat_velocities = MODULE.action_sequence_score(
        model,
        z[:, None].expand(-1, 3, -1).reshape(6, 1),
        candidates.reshape(6, 4, 2),
        0.1,
    )
    torch.testing.assert_close(population_scores, flat_scores.reshape(2, 3))
    torch.testing.assert_close(
        population_velocities, flat_velocities.reshape(2, 3, 4)
    )
    assert "action_sequence_score(" in inspect.getsource(
        MODULE.action_sequence_population_score
    )


def test_seeded_cem_is_deterministic_bounded_and_keeps_optimal_incumbent():
    model = QuadraticIntervalModel([0.6, -0.4])
    z = torch.zeros(3, 1)
    warm_start = torch.tensor([[[0.6, -0.4]]]).expand(3, 12, 2).clone()
    first, first_metrics = MODULE.optimize_action_sequence_cem(
        model,
        z,
        warm_start,
        cem_args(),
    ...
    second, second_metrics = MODULE.optimize_action_sequence_cem(
        model,
        z,
        warm_start,
        cem_args(),
    ...
    torch.testing.assert_close(first, warm_start, rtol=0.0, atol=0.0)
    torch.testing.assert_close(
    ...
    _, adam_metrics = MODULE.optimize_action_sequence(
        model, z, warm_start, optimizer_args(action_opt_iterations=1)
    )
    assert set(first_metrics) == set(adam_metrics)
    assert torch.all(first.abs() <= 1.0)
    assert torch.all(first_metrics["optimizer_improvement"] >= 0.0)


def test_planner_switch_dispatches_only_the_selected_search_method():
    source = inspect.getsource(MODULE.plan_action_sequence)
    assert 'args.planner == "adam"' in source
    assert 'args.planner == "cem"' in source
    assert "optimize_action_sequence(" in source
    assert "optimize_action_sequence_cem(" in source


def test_sequence_optimizer_is_deterministic_bounded_and_improves_score():
    model = QuadraticIntervalModel([0.6, -0.4])
    z = torch.zeros(4, 1)
    start = torch.zeros(4, 12, 2)
...
def test_factorial_defaults_are_h12_adam_without_continuing_random_actions():
    args = MODULE.Args()
    assert args.model_horizon == 12
    assert args.planner_horizon == 12
    assert args.planner == "adam"
    assert args.random_action_probability == 0.0
    assert args.cem_population == 256
    assert args.cem_iterations == 4
    assert args.cem_elites == 32
    assert args.cem_global_fraction == 0.125
    assert args.cem_initial_std == 0.6
    assert args.cem_min_std == 0.08
    assert args.cem_update_rate == 0.7


def test_zero_probability_random_replacement_consumes_no_rng():
    generator = torch.Generator().manual_seed(17)
    state_before = generator.get_state().clone()
    action = torch.randn(4, 2)
    plan = torch.randn(4, 12, 2)
    selected, selected_plan, replaced = MODULE.apply_random_action_replacement(
        action, plan, 0.0, generator
    )
...
def test_random_replacement_is_bounded_and_resets_only_selected_plans():
    action = torch.zeros(64, 2)
    plan = torch.ones(64, 12, 2)
    selected, selected_plan, replaced = MODULE.apply_random_action_replacement(
        action, plan, 0.5, torch.Generator().manual_seed(23)
    )
    assert replaced.any() and (~replaced).any()
    assert torch.all(selected[replaced].abs() <= 1.0)
    torch.testing.assert_close(selected[~replaced], action[~replaced])
    assert torch.count_nonzero(selected_plan[replaced]) == 0
    torch.testing.assert_close(selected_plan[~replaced], plan[~replaced])
