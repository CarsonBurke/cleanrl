...
from cleanrl.pan_goal_solver.ppo_continuous_action_pan_goal_solver_v13 import (
    Args,
    DirectGoalFollower,
    PanGoalSolver,
    VectorReplayBuffer,
...
def test_reward_prediction_is_invariant_to_goal_distance():
    ...
    with torch.no_grad():
        near = agent.predict_reward(goal)
        far = agent.predict_reward(100.0 * goal)
    torch.testing.assert_close(near, far, rtol=1e-5, atol=1e-5)


def test_frozen_decoder_passes_reward_gradient_only_to_goal_input():
    agent = _agent()
    goal = torch.randn(5, 16, requires_grad=True)
    agent.predict_reward(goal).sum().backward()
    assert goal.grad is not None
    assert goal.grad.abs().sum() > 0
    assert all(
        parameter.grad is None
        for parameter in agent.target_representation["goal_projector"].decoder.parameters()
    )
...
def test_goal_features_preserve_local_magnitude_and_saturate_for_far_goals():
    follower = DirectGoalFollower(
        observation_tokens=2, belief_dim=4, goal_dim=4, act_dim=2, hidden=16
    )
    current = torch.zeros(1, 4)
    direction = torch.tensor([[1.0, -2.0, 0.5, 0.0]])
    near = follower.goal_features(current, 0.01 * direction)
    medium = follower.goal_features(current, direction)
    far = follower.goal_features(current, 1_000.0 * direction)
    assert near.norm() < medium.norm() < far.norm()
    torch.testing.assert_close(
        torch.nn.functional.normalize(far),
        torch.nn.functional.normalize(direction),
    )
    torch.testing.assert_close(far.norm(), torch.tensor(1.0), atol=5e-4, rtol=0)


def test_desired_successor_is_structurally_goal_conditioned():
    follower = DirectGoalFollower(
        observation_tokens=2, belief_dim=4, goal_dim=4, act_dim=2, hidden=16
    )
    belief = torch.randn(3, 4)
    current = torch.randn(3, 4)
    same = follower.desired_successor(belief, current, current)
    shifted = follower.desired_successor(belief, current, current + 1.0)
    torch.testing.assert_close(same, current, rtol=0, atol=0)
    assert not torch.equal(shifted, current)
