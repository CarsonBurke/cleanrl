...
from cleanrl.ppo_continuous_action_latent_costate_fullbatch_v12 import (
    Args, Agent, ActorNaturalGradient, Dynamics, LatentCostate, FixedAffineObsNorm,
    ...
    incoming_projection, fitting_batches, critic_fitting_schedule,
)
...
@pytest.mark.parametrize('gamma', [0., .99, 1.])
@pytest.mark.parametrize('continuation', [0., 1.])
def test_vector_bellman_matches_full_reward_derivative_including_policy(continuation, gamma):
    model = LinearPhysics()
    ...
    full_return = output[:, -1] - .1 * action.square().sum(-1) + gamma * continuation * .5 * output[:, :2].square().sum(-1)
    ...
    actual, qa, eta = differential_targets(model, actor, state, action.detach(), jac,
                                          output[:, :2], torch.full((32,), continuation, device='cuda'), .1, gamma)
    torch.testing.assert_close(actual, expected, atol=1e-10, rtol=1e-9)
    expected_qa = model.u - .2 * action.detach() + gamma * continuation * output[:, :2].detach() @ model.B
...
def test_compiled_costate_learning_targets_and_two_actor_steps():
    ...
    target_fn = torch.compile(lambda x, a, j, n, mask: differential_targets(model, actor, x, a, j, n, mask, .1, .99),
    ...
    for _ in range(2):
        ...
        frozen_next_value = next_value.clone()
        for _ in range(Args().critic_epochs):
            ...
            optimizer.step()
            del loss
        # Compiled fitting must neither alias nor mutate the bootstrap snapshot.
        torch.testing.assert_close(target, frozen_target, rtol=0, atol=0)
        torch.testing.assert_close(next_value, frozen_next_value, rtol=0, atol=0)
...
def test_fitting_batches_full_batch_does_not_partition_or_advance_shuffle_rng():
    generator = torch.Generator(device='cuda').manual_seed(17)
    before = generator.get_state().clone()
    full = list(fitting_batches(128, 32, torch.device('cuda'), generator, True))
    assert full == [slice(None)]
    assert torch.equal(before, generator.get_state())
    mini = list(fitting_batches(128, 32, torch.device('cuda'), generator, False))
    assert len(mini) == 4
    torch.testing.assert_close(torch.cat(mini).sort().values, torch.arange(128, device='cuda'))
    assert not torch.equal(before, generator.get_state())


@pytest.mark.parametrize('refreshes,expected', [
    (1, (True, False, False, False, False, False, False, False)),
    (2, (True, False, False, False, True, False, False, False)),
    (8, (True,) * 8),
])
def test_critic_target_refreshes_are_separate_from_fitting_passes(refreshes, expected):
    assert critic_fitting_schedule(8, refreshes) == expected
    for full_batch, expected_steps in [(False, 64), (True, 8)]:
        generator = torch.Generator(device='cuda').manual_seed(1)
        updates = sum(len(list(fitting_batches(32768, 4096, torch.device('cuda'), generator, full_batch)))
                      for _ in critic_fitting_schedule(8, refreshes))
        assert updates == expected_steps


@pytest.mark.parametrize('epochs,refreshes', [(0, 1), (8, 0), (8, 3), (8, 9)])
def test_invalid_critic_target_schedules_rejected(epochs, refreshes):
    with pytest.raises(ValueError):
        critic_fitting_schedule(epochs, refreshes)
