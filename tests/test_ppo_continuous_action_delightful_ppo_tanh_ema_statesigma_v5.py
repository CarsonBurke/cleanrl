...
SCRIPT = (
    ...
    / "cleanrl" / "delightful" / "ppo" / "ppo_continuous_action_delightful_ppo_tanh_ema_statesigma_v5.py"
)
SPEC = importlib.util.spec_from_file_location(
    "delightful_ppo_tanh_ema_statesigma_v5", SCRIPT
)
...
def test_args_encode_requested_scale_choices():
    ...
    assert args.scale_gae_kernel
    assert args.delight_temperature == 1.0
    assert args.surprisal_clip == 10.0


def test_reward_normalizer_uses_bias_corrected_rms_and_gae_kernel_scale():
    reward_scale = dg.gae_kernel_scale(gamma=0.99, gae_lambda=0.95)
    normalizer = dg.EMARewardNormalizer(decay=0.5, reward_scale=reward_scale)

    assert np.isclose(reward_scale, 0.0595)

    first, first_rms = normalizer.normalize(np.array([3.0, 4.0], dtype=np.float32))
    assert np.isclose(first_rms, np.sqrt(12.5))
    assert np.allclose(
        first, reward_scale * np.array([3.0, 4.0]) / np.sqrt(12.5)
    )

    second, second_rms = normalizer.normalize(np.array([0.0, 2.0], dtype=np.float32))
    expected_m2 = (0.5 * 6.25 + 0.5 * 2.0) / (1.0 - 0.5**2)
    assert np.isclose(second_rms, np.sqrt(expected_m2))
    assert np.allclose(
        second, reward_scale * np.array([0.0, 2.0]) / np.sqrt(expected_m2)
    )
...
def test_initial_squashed_density_has_positive_surprisal_at_the_mode():
    agent = dg.Agent(DummyVectorEnv())
    ...
    assert torch.allclose(logstd, torch.zeros_like(logstd))
