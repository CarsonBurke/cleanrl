...
def test_qdagger_dqn_atari_impalacnn():
    subprocess.run(
        "python cleanrl/qdagger_dqn_atari_impalacnn.py --learning-starts 10 --total-timesteps 16 --buffer-size 10 --batch-size 4 --teacher-steps 16 --offline-steps 16 --offline-checkpoint-interval-seconds 600 --teacher-eval-episodes 1",
        shell=True,
        check=True,
    )
...
def test_qdagger_dqn_atari_impalacnn_eval():
    subprocess.run(
        "python cleanrl/qdagger_dqn_atari_impalacnn.py --save-model --learning-starts 10 --total-timesteps 16 --buffer-size 10 --batch-size 4 --teacher-steps 16 --offline-steps 16 --offline-checkpoint-interval-seconds 600 --teacher-eval-episodes 1",
        shell=True,
        check=True,
    )
