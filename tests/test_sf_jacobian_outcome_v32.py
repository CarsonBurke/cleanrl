"""Tests for v32's successor-feature action Jacobian.
...
SRC = (
    ...
    / "cleanrl"
    / "encoder-value"
    / "ppo_continuous_action_sf_jacobian_outcome_v32.py"
)

_spec = importlib.util.spec_from_file_location("v32", SRC)
...
def test_head_is_identically_zero_at_init_so_iteration_one_is_analytic_only():
    head = v32.ActionSuccessorHead(64, SF_DIM, ACT).to(DEV)
...
def test_closed_form_head_jacobian_matches_autograd():
    torch.manual_seed(0)
    head = v32.ActionSuccessorHead(64, SF_DIM, ACT).to(DEV).double()
    _randomize(head)
...
def test_analytic_phi_jacobian_matches_autograd():
    ...
    for i in range(3):
        auto = torch.autograd.functional.jacobian(
            lambda a: (v32.phi_features(emb[i], obs[i], a) * w).sum(), action[i]
        )
...
def test_phi_has_no_action_dependence_outside_the_two_action_blocks():
    ...
    p1 = v32.phi_features(emb, obs, a1)
    p2 = v32.phi_features(emb, obs, a2)
...
def test_reward_probe_recovers_the_quadratic_control_cost_from_the_aa_block():
    ...
    phi = v32.phi_features(emb, obs, action)
    ...
    w_r = v32.solve_reward_probe(phi, reward, 1e-6)
    ...
    assert v32.ev_score(phi @ w_r, reward) > 0.999
...
def _agent(dist_name):
    ...
    return v32.Agent(_Envs(), args).to(DEV), args
...
def test_perdim_loss_reproduces_the_scalar_ppo_GRADIENT_at_the_start_of_an_epoch():
    ...
    ratio_d = (lp - old).exp()
    loss, adv_dims = v32.perdim_policy_loss(
        adv, torch.zeros(n, ACT, device=DEV, dtype=torch.float64), 0.0, ratio_d, 0.2, 0.28
    )
...
def test_perdim_loss_gradient_is_sensitive_to_the_sign_of_the_shift():
    ...
    adv = torch.randn(n, device=DEV, dtype=torch.float64)
    shift = v32.attribution_shift(
        torch.randn(n, ACT, device=DEV, dtype=torch.float64), 1.0, 3.0
    )

    def g(sign):
        loss, _ = v32.perdim_policy_loss(adv, sign * shift, 0.5, ratio_d, 0.2, 0.28)
...
def test_shift_is_exactly_zero_sum_so_mean_credit_is_the_scalar_advantage():
    ...
    for omega in (0.0, 0.25, 1.0, 25.0):
        shift = v32.attribution_shift(attrib, 1.0, 3.0)
        assert shift.sum(-1).abs().max() < 1e-12
        _, adv_dims = v32.perdim_policy_loss(adv, shift, omega, ratio_d, 0.2, 0.28)
...
def test_shrinkage_disables_the_channel_when_the_attribution_predicts_nothing():
    ...
    attrib = torch.randn(4096, ACT, device=DEV, dtype=torch.float64)
    assert torch.equal(
        v32.attribution_shift(attrib, 0.0, 3.0),
        torch.zeros_like(attrib),
    )
    half = v32.attribution_shift(attrib, 0.5, 3.0)
    full = v32.attribution_shift(attrib, 1.0, 3.0)
...
def test_shift_is_winsorized_without_losing_the_zero_sum_property():
    ...
    clamped = v32.attribution_shift(attrib, 1.0, 3.0)
    unclamped = v32.attribution_shift(attrib, 1.0, 1e9)
...
def test_per_coordinate_clip_bounds_match_v9s_joint_trust_region():
    # The product of act_dim per-coordinate bounds must reproduce [1-clip, 1+clip_high]
    # exactly. Without the act_dim-th root this is a 3.4x looser trust region.
    lo = (1.0 - 0.2) ** (1.0 / ACT)
    hi = (1.0 + 0.28) ** (1.0 / ACT)
    assert abs(lo**ACT - 0.8) < 1e-12
    assert abs(hi**ACT - 1.28) < 1e-12
    # and the shipped loss must actually use those bounds: drive every ratio far past
    # them and check the clipped branch caps the surrogate at the matched value.
    # The clip only binds on the improvement side: with A > 0, max(-A r, -A clamp(r))
    # picks the CLIPPED branch, capping the surrogate at the matched joint bound.
    n = 256
    adv = torch.ones(n, device=DEV, dtype=torch.float64)
    zero = torch.zeros(n, ACT, device=DEV, dtype=torch.float64)
    ratio_d = torch.full((n, ACT), 10.0, device=DEV, dtype=torch.float64)
    loss, _ = v32.perdim_policy_loss(adv, zero, 0.0, ratio_d, 0.2, 0.28)
    assert abs(float(loss) + ACT * hi) < 1e-9, float(loss)
    # ...and on the downside, at the matched lower bound.
    ratio_d = torch.full((n, ACT), 0.01, device=DEV, dtype=torch.float64)
    loss, _ = v32.perdim_policy_loss(-adv, zero, 0.0, ratio_d, 0.2, 0.28)
    assert abs(float(loss) - ACT * lo) < 1e-9, float(loss)
...
def _improve(j_emb, g, ridge, kappa, act_std):
    """Exactly v32's channel-B arithmetic."""
...
def test_head_recovers_a_known_quadratic_action_map():
    torch.manual_seed(0)
    head = v32.ActionSuccessorHead(16, 8, ACT).to(DEV)
    ...
    for _ in range(600):
        ...
        opt.step()
    assert v32.ev_score(head(feat, action).reshape(-1), target.reshape(-1)) > 0.95
...
def test_action_successor_head_is_not_in_the_ppo_optimizer_or_clip_groups():
    ...
    head = v32.ActionSuccessorHead(args.hidden, SF_DIM, ACT).to(DEV)
